...
Процедура УдалитьЛишниеСтрокиНастроекСинхронизации(НастройкиСинхронизацииИзМенеджераСервиса, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Режим = "НеНастроенныеОбмены" Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ТаблицаНастроек.Значение КАК Значение
			|ИЗ
			|	Справочник.НастройкиТранспортаСообщенийОбмена.Настройки КАК ТаблицаНастроек
			|ГДЕ
			|	ТаблицаНастроек.Настройка = ""ОбластьДанныхКорреспондента""";
		
		ОбластьДанныхВнутренняяПубликация = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Значение");
