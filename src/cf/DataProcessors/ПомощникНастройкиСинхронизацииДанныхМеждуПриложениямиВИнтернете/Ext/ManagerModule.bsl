...
Процедура УдалитьЛишниеСтрокиНастроекСинхронизации(НастройкиСинхронизацииИзМенеджераСервиса, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Режим = "НеНастроенныеОбмены" Тогда
		...
		// Области с настроенной синхронизацией через внутр публикацию
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               |	Настройки.WSОбластьДанныхКорреспондента КАК WSОбластьДанныхКорреспондента
		               |ИЗ
		               |	РегистрСведений.НастройкиТранспортаОбменаДанными КАК Настройки
		               |ГДЕ
		               |	Настройки.WSОбластьДанныхКорреспондента > 0";
		
		ОбластьДанныхВнутренняяПубликация = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("WSОбластьДанныхКорреспондента");
		...
		Для Каждого НастройкаИзМенеджераСервиса Из НастройкиСинхронизацииИзМенеджераСервиса Цикл
			...
			Если ОбластьДанныхВнутренняяПубликация.Найти(НастройкаИзМенеджераСервиса.ОбластьДанных) <> Неопределено Тогда
				НастройкаИзМенеджераСервиса.УдалитьНастройку = Истина;
				Продолжить;
			КонецЕсли;
