...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ОтчетОбОшибке <> Неопределено Тогда
		СтандартныеПодсистемыКлиент.ОтправитьОтчетОбОшибке(ОтчетОбОшибке, ИнформацияОбОшибке);
	КонецЕсли;
...
#Область ПроцедурыИФункцииФормированияСпискаДелПользователя
...
Процедура СформироватьСписокТекущихДелВФонеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		...
		ТекстОшибки          = ОбработкаОшибок.СообщениеОбОшибкеДляПользователя(Результат.ИнформацияОбОшибке);
		ТекстОшибкиПодробный = ОбработкаОшибок.ПодробноеПредставлениеОшибки(Результат.ИнформацияОбОшибке);
		ИнформацияОбОшибке   = Результат.ИнформацияОбОшибке;
		ОтчетОбОшибке        = Новый ОтчетОбОшибке(Результат.ИнформацияОбОшибке);
		СтандартныеПодсистемыКлиент.НастроитьВидимостьИЗаголовокСсылкиОтправкиОтчетаОбОшибке(Элементы.СформироватьОтчетОбОшибке, Результат.ИнформацияОбОшибке);
		Элементы.ГруппаПодробнее.Видимость = Не Элементы.СформироватьОтчетОбОшибке.Видимость;
		Возврат;
...
Процедура ДобавитьДелаСОповещением(НастройкиОтображения, ТекущиеДела)
	...
	Если НастройкиОтображения.ДеревоДел.Колонки.Найти("ВыводитьВОповещениях") <> Неопределено Тогда
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если Не ЗначениеЗаполнено(Дело.Форма) Тогда
				ОтборДочерних = Новый Структура;
				ОтборДочерних.Вставить("ИдентификаторВладельца", Дело.Идентификатор);
				ДочерниеДела = ТекущиеДела.НайтиСтроки(ОтборДочерних);
				Для Каждого ДочернееДело Из ДочерниеДела Цикл
					Если Не ДочернееДело.ЕстьДела Тогда
						Продолжить;
					КонецЕсли;
					
					Если Не ЗначениеЗаполнено(ДочернееДело.Форма) Тогда
						Продолжить;
					КонецЕсли;
					...
					Если ДочернееДело.Количество <> 0 Тогда
						ДобавкаДочернее = " (" + ДочернееДело.Количество + ")";
					Иначе
						ДобавкаДочернее = "";
					КонецЕсли;
					
					ДелоСОповещением = ДелаСОповещением.Добавить();
					ДелоСОповещением.Идентификатор = ДочернееДело.Идентификатор;
					ДелоСОповещением.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления,
						Строка.Представление,
						ДочернееДело.Представление + ДобавкаДочернее);
				КонецЦикла;
