...
Процедура ЗаполнитьДеревоДел(НастройкиОтображения)
	...
	Для Каждого Дело Из ТекущиеДела Цикл
		...
		Если СтрокаДела = Неопределено Тогда
			...
			Если СохраненноеДеревоДел <> Неопределено
				И СохраненноеДеревоДел.Колонки.Найти("ВыводитьВОповещениях") <> Неопределено Тогда
				НайденнаяСтрока = СохраненноеДеревоДел.Строки.Найти(Дело.Идентификатор, "Идентификатор", Истина);
				Если НайденнаяСтрока <> Неопределено Тогда
					СтрокаДела.ВыводитьВОповещениях = НайденнаяСтрока.ВыводитьВОповещениях;
				КонецЕсли;
			КонецЕсли;
