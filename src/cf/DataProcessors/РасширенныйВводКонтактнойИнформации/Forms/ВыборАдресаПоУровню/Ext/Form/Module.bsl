...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ДополнительныйРодитель", ДополнительныйРодитель);
	Параметры.Свойство("ДополнительныйУровень",  ДополнительныйУровень);
...
Функция ДанныеКлассификатораПоИдентификатору(Знач ПараметрыПоиска)
	...
	Если ЗначениеЗаполнено(ДополнительныйРодитель) Тогда
		ДополнительныеДанныеКлассификатора = Обработки.РасширенныйВводКонтактнойИнформации.АдресаДляИнтерактивногоВыбора(ДополнительныйРодитель,
			ДополнительныйУровень, ТипАдреса, ПараметрыПоиска);
		Для Каждого СтрокаИсточник Из ДополнительныеДанныеКлассификатора.Данные Цикл
			ЗаполнитьЗначенияСвойств(ДанныеКлассификатора.Данные.Вставить(0), СтрокаИсточник);
		КонецЦикла;
	КонецЕсли;
	...
	ИначеЕсли ДанныеКлассификатора.Данные.Количество() = 0 Тогда
		...
		ДополнительнаяИнформацияАдреса = Новый Структура("АдресВJSON", Истина);
		
		Адрес = РаботаСАдресами.АдресПоИдентификатору(Родитель, ДополнительнаяИнформацияАдреса);
		...
		Если РаботаСАдресами.ЭтоГородФедеральногоЗначения(НаименованиеРегиона) Тогда
			...
			КонецЕсли;
		Иначе
			АдресПоПолям = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
		
			НаименованиеРодительскогоОбъекта = РаботаСАдресамиКлиентСервер.НаименованиеРодительскогоОбъекта(АдресПоПолям, Родитель);
			
			Если Уровень = 1 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствует информация о регионах.'");
			ИначеЕсли Уровень = 2 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют районы для %1.'");
			ИначеЕсли Уровень = 3 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют муниципальные районы для %1.'");
			ИначеЕсли Уровень = 4 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют поселения для %1.'");
			ИначеЕсли Уровень = 5 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют города для %1.'");
			ИначеЕсли Уровень = 6 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют населенные пункты для %1.'");
			ИначеЕсли Уровень = 7 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют территории для %1.'");
			ИначеЕсли Уровень = 8 Тогда
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Отсутствуют улицы для %1.'");
			КонецЕсли;
			
			Результат.КраткоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.КраткоеПредставлениеОшибки, 
