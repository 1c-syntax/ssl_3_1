...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НаселенныйПунктДетально <> Неопределено Тогда
		...
		Если НаселенныйПунктДетально.Свойство("country")
			 И (РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(НаселенныйПунктДетально.Country) 
			 ИЛИ ПустаяСтрока(НаселенныйПунктДетально.Country)) Тогда
	...
	Иначе
		...
		Элементы.Регион.ПодсказкаВвода = ПодсказкаВводаПоИмениУровня().Получить("Регион");
	КонецЕсли;
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(ТипАдреса) Тогда
		Территория = ПредставлениеУровняАдреса(НаселенныйПунктДетально, "territory");
		Элементы.ТипАдреса.Видимость = Ложь;
		Элементы.Регион.КнопкаВыбора = Ложь;
	...
	ИначеЕсли ИспользоватьАвтоподбор Тогда
		УровниСАдреснымиОбъектами = ОпределитьУровниВКоторыхДоступенВыборАдресныхОбъектов(НаселенныйПунктДетально);
		
		ИменаЭлементов = ИменаУровней();
		ПодсказкаВводаПоИмениУровня = ПодсказкаВводаПоИмениУровня();
	
		Для Каждого ИмяУровняАдреса Из ИменаЭлементов Цикл
			ИмяЭлемента = ИмяУровняАдреса.Ключ;
			Если УровниСАдреснымиОбъектами.Найти(ИмяУровняАдреса.Значение) <> Неопределено
			 Или ЗначениеЗаполнено(НаселенныйПунктДетально[ИмяУровняАдреса.Значение + "Id"])
			 Или ЭтоУровеньРегиона(ИмяУровняАдреса.Значение) Тогда
				Элементы[ИмяЭлемента].ПодсказкаВвода = ПодсказкаВводаПоИмениУровня[ИмяЭлемента];
			Иначе 
...
Процедура ТипАдресаПриИзменении(Элемент)
	...
	Уровни = ОпределитьУровниВКоторыхДоступенВыборАдресныхОбъектов(НаселенныйПунктДетально);
	УстановитьПодсказкиДляПолей(Уровни);
...
Процедура ОбработкаВыбораУровня(Элемент, Знач ВыбранноеЗначение, СтандартнаяОбработка)
	...
	НаселенныйПунктПоИдентификатору = НаселенныйПунктДетальноПоИдентификатору(СведенияОбАдресе);
	
	НаселенныйПунктДетально = НаселенныйПунктПоИдентификатору.СписокРеквизитовНаселенныйПункт;
	
	ОпределитьПоляАдреса(ВыбранноеЗначение.Уровень);
	Элемент.ОбновитьТекстРедактирования();

	УровниСАдреснымиОбъектами = НаселенныйПунктПоИдентификатору.УровниСАдреснымиОбъектами;
	УстановитьДоступностьВыбораАдресныхОбъектов(УровниСАдреснымиОбъектами);
...
&НаКлиенте
Процедура УстановитьДоступностьВыбораАдресныхОбъектов(УровниСАдреснымиОбъектами = Неопределено)

	ИменаЭлементов = ИменаУровней();
	ПодсказкаВводаПоИмениУровня = ПодсказкаВводаПоИмениУровня();
	
	Для Каждого УровеньИмя Из ИменаЭлементов Цикл
		ИмяЭлемента = УровеньИмя.Ключ;
		Если ЗначениеЗаполнено(НаселенныйПунктДетально[УровеньИмя.Значение + "Id"]) Тогда
			Элементы[ИмяЭлемента].ПодсказкаВвода = ПодсказкаВводаПоИмениУровня[ИмяЭлемента];
		ИначеЕсли УровниСАдреснымиОбъектами <> Неопределено И УровниСАдреснымиОбъектами.Найти(УровеньИмя.Значение) <> Неопределено Тогда
			Элементы[ИмяЭлемента].ПодсказкаВвода = ПодсказкаВводаПоИмениУровня[ИмяЭлемента];
		Иначе 
			Элементы[ИмяЭлемента].ПодсказкаВвода = "";
		КонецЕсли;
...
Функция ИмяУровняПоНазваниюЭлемента(ИмяЭлемента)
	...
	ИменаУровней = ИменаУровней();
	Возврат ИменаУровней[ИмяЭлемента];
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИменаУровней()
	
	ИменаУровней = Новый Соответствие;
	ИменаУровней.Вставить("Регион",             "area");
	...
	Возврат ИменаУровней;
	
КонецФункции


&НаКлиентеНаСервереБезКонтекста
Функция ПодсказкаВводаПоИмениУровня()
	
	Уровни = Новый Соответствие;
	Уровни.Вставить("Регион", НСтр("ru = 'Выберите или введите регион'"));
	Уровни.Вставить("Район", НСтр("ru = 'Выберите или введите район'"));
	Уровни.Вставить("МуниципальныйРайон", НСтр("ru = 'Выберите или введите муниципальный район'"));
	Уровни.Вставить("Город", НСтр("ru = 'Выберите или введите город'"));
	Уровни.Вставить("Поселение", НСтр("ru = 'Выберите или введите поселение'"));
	Уровни.Вставить("НаселенныйПункт", НСтр("ru = 'Выберите или введите населенный пункт'"));
	Уровни.Вставить("Территория", НСтр("ru = 'Выберите или введите территорию'"));
...
Функция ВозвращаемыеДанныеОбАдресе()
	...
	Идентификатор = РаботаСАдресамиКлиентСервер.ИдентификаторАдреса(НаселенныйПунктДетально, Ложь);
	Результат.УАдресаЕстьУлицы = УАдресаЕстьУлицы(Идентификатор, 
...
Процедура ОткрытьФормуВыбораАдресныхОбъектов(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИмяУровня = "locality" И РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса) Тогда
		ДополнительныйРодитель = ИдентификаторРодителяУровняАдреса("city", НаселенныйПунктДетально, ТипАдреса);
		ДополнительныйУровень = РаботаСАдресамиКлиентСервер.СопоставлениеНаименованиеУровнюАдреса("city");
	КонецЕсли;
	...
	ПараметрыОткрытия.Вставить("ДополнительныйРодитель", ДополнительныйРодитель);
	ПараметрыОткрытия.Вставить("ДополнительныйУровень",  ДополнительныйУровень);
...
Функция ПредставлениеУровняАдреса(Адрес, ИмяУровня)
	...
	Возврат РаботаСАдресамиКлиентСервер.ПредставлениеУровняАдреса(Адрес, ИмяУровня);
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если СтрСравнить(УправлениеКонтактнойИнформациейКлиентСервер.АдресЕАЭС(), ТипАдреса) = 0
	 Или СтрСравнить(УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес(), ТипАдреса) = 0 Тогда
		
		Элементы.ТипАдреса.Видимость            = Ложь;
		Элементы.МуниципальныйРайон.Видимость   = Ложь;
		Элементы.Поселение.Видимость            = Ложь;
		Элементы.Территория.Видимость           = Ложь;
		Элементы.Регион.Заголовок               = НСтр("ru = 'Регион'");
		Элементы.Регион.КнопкаВыбора            = Ложь;
		Элементы.Район.КнопкаВыбора             = Ложь;
		Элементы.Город.КнопкаВыбора             = Ложь;
		Элементы.НаселенныйПункт.КнопкаВыбора   = Ложь;
		Элементы.Город.Заголовок                = НСтр("ru = 'Город / населенный пункт'");
		Элементы.НаселенныйПункт.Заголовок      = НСтр("ru = 'Внутригородской район / квартал'");
...
Функция АвтоПодборВариантов(Текст, ИмяУровня, ТипАдреса, НаселенныйПунктДетально)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса) И ИмяУровня = "locality" Тогда
			РодительскийИдентификатор = ИдентификаторРодителяУровняАдреса("city", НаселенныйПунктДетально, ТипАдреса);
			МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
			УровеньГорода = РаботаСАдресамиКлиентСервер.СопоставлениеНаименованиеУровнюАдреса("city"); 
			Результат = МодульАдресныйКлассификаторСлужебный.АдресныеОбъектыУровня(РодительскийИдентификатор, УровеньГорода, ТипАдреса, Текст);
			Если Не Результат.Отказ Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьСписок(ДанныеВыбора, Результат.Данные);
			КонецЕсли;
		КонецЕсли;
...
Функция ИдентификаторРодителяУровняАдреса(Знач ИмяУровня, Знач НаселенныйПунктДетально, Знач ТипАдреса)
	...
	Для каждого ИмяУровняАдреса Из ИменаУровнейАдреса Цикл
		...
		Если ТекущийУровня = Неопределено Или ТекущийУровня > Уровень Или ИмяУровняАдреса = ИмяУровня Тогда
			Возврат РодительскийИдентификатор;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(НаселенныйПунктДетально[ИмяУровняАдреса + "Id"]) Тогда
			Если ЭтоМуниципальныйАдрес И СтрСравнить(ИмяУровняАдреса, "district") = 0 Тогда
				// В муниципальном адресе не учитывается уровень района, но иногда там может располагаться город.
				Продолжить;
			КонецЕсли;
...
Функция НаселенныйПунктДетальноПоИдентификатору(СведенияОбАдресе)
	...
	УровниСАдреснымиОбъектами = Новый Массив;
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		УровниСАдреснымиОбъектами = МодульАдресныйКлассификаторСлужебный.ДочерниеУровниСАдреснымиОбъектами(
			СведенияОбАдресе.Идентификатор, СведенияОбАдресе.Муниципальный);
	КонецЕсли;
	
	СписокРеквизитовНаселенныйПункт = Обработки.РасширенныйВводКонтактнойИнформации.СписокРеквизитовНаселенныйПункт(СведенияОбАдресе);
	
	Результат.Вставить("УровниСАдреснымиОбъектами", УровниСАдреснымиОбъектами);
	Результат.Вставить("СписокРеквизитовНаселенныйПункт", СписокРеквизитовНаселенныйПункт);
	Возврат Результат;
...
&НаСервереБезКонтекста
Функция ОпределитьУровниВКоторыхДоступенВыборАдресныхОбъектов(НаселенныйПунктДетально)
	
	УровниСАдреснымиОбъектами = Новый Массив;
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		ЭтоМуниципальныйАдрес = (НаселенныйПунктДетально.AddressType = РаботаСАдресамиКлиентСервер.МуниципальныйАдрес());
		Идентификатор = РаботаСАдресамиКлиентСервер.ИдентификаторАдреса(НаселенныйПунктДетально, Ложь, Ложь);
	
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		УровниСАдреснымиОбъектами = МодульАдресныйКлассификаторСлужебный.ДочерниеУровниСАдреснымиОбъектами(
			Идентификатор, ЭтоМуниципальныйАдрес);
	КонецЕсли;
	
	Возврат УровниСАдреснымиОбъектами;
	
КонецФункции

&НаКлиенте
Процедура УстановитьПодсказкиДляПолей(УровниСАдреснымиОбъектами)
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоУровеньРегиона(ИмяУровня)
	Возврат СтрСравнить(ИмяУровня, "area") = 0;
КонецФункции

&НаСервереБезКонтекста
Функция УАдресаЕстьУлицы(Идентификатор, ТипАдреса)
	Возврат РаботаСАдресами.АдресСодержитУлицы(Идентификатор, ТипАдреса);
КонецФункции
