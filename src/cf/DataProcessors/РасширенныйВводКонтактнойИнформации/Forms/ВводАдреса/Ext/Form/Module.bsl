...
Процедура ИнформацияОЗагрузкеАдресныеСведенийОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если НавигационнаяСсылкаФорматированнойСтроки = "load" Тогда
...
&НаКлиенте
Процедура УстановитьСтрану(ПредставлениеСтраны)
	
	Если ЗначениеЗаполнено(ПредставлениеСтраны) Тогда
		Страна = ОпределитьСтрануПоНаименованию(ПредставлениеСтраны);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ОпределитьСтрануПоНаименованию(Страна)
	Возврат УправлениеКонтактнойИнформацией.ДанныеСтраныМира(, Страна).Ссылка;
КонецФункции
...
Процедура ПеренестиЗначенияПолейАдресВПоляНовогоТипа()
	...
	Если ЭтоИностранныйАдрес(НаселенныйПунктДетально.AddressType)
...
Процедура ОбработкаИзмененияАдреса()
	...
	Если СтрСравнить(Страна, НаселенныйПунктДетально.Country) <> 0 Тогда
		УстановитьСтрану(НаселенныйПунктДетально.Country);
	КонецЕсли;
...
Функция ПредставлениеНаселенногоПунктаАдреса(Адрес)
	...
	Для каждого ИмяУровня Из РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(Адрес.AddressType, Ложь) Цикл
		...
		Если ЗначениеЗаполнено(Адрес[ИмяУровня]) Тогда
			Если Адрес.AddressType = РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес() 
				И СтрСравнить(ИмяУровня, "Locality") = 0
				И СтрСравнить(СокрЛП(Адрес[ИмяУровня] + " " + Адрес[ИмяУровня + "Type"]), СокрЛП(Адрес["city"] + " " + Адрес["cityType"])) = 0 Тогда
					Продолжить;
			КонецЕсли;
...
Процедура ОткрытьФормуЗагрузкиКлассификатора()
	...
	Перем ДополнительныеПараметры, СведенияОРегионе;
