...
Процедура ПослеВыбораАдресаПоИндексу(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			ПолученныйАдрес = ОпределитьАдресПоИдентификатору(Результат, ИсключатьГородИзМуниципальногоАдреса);
...
Процедура ПослеВыбораУлицы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Не Результат.Отказ Тогда
			ИсключатьГородИзМуниципальногоАдреса = Ложь;
			НаселенныйПунктДетально = ОпределитьАдресПоИдентификатору(Результат, ИсключатьГородИзМуниципальногоАдреса);
			ОбработкаИзмененияАдреса();
		КонецЕсли;
	Иначе
...
Процедура УлицаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, СведенияОбАдресе, Истина, ИсключатьГородИзМуниципальногоАдреса);
	ОбработкаИзмененияАдреса();
...
Процедура УстановитьМуниципальныеСведения(НаселенныйПунктДетально)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторАдреса) Тогда
			...
			Результат = МодульАдресныйКлассификаторСлужебный.АктуальныеАдресныеСведения(Новый Структура("Идентификатор", ИдентификаторАдреса));
			Если Не Результат.Отказ Тогда
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	НаселенныйПунктДетально.Value = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(НаселенныйПунктДетально, ВидКИ.ВключатьСтрануВПредставление,, Контекст.ИсключатьГородИзМуниципальногоАдреса);
...
Процедура УстановитьЗначенияНациональныхРеквизитовКонтактнойИнформации(ДанныеАдреса)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		ИсключатьГородИзМуниципальногоАдреса = МодульАдресныйКлассификаторСлужебный.ИсключатьГородИзМуниципальногоАдреса(ДанныеАдреса);
	Иначе 
		ИсключатьГородИзМуниципальногоАдреса = Ложь;;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, Знач ВыбранноеЗначение, Знач ВключатьУлица, ИсключатьГородИзМуниципальногоАдреса)
	...
	Иначе
		ПолученныйАдрес = ОпределитьАдресПоИдентификатору(ВыбранноеЗначение, ИсключатьГородИзМуниципальногоАдреса);
	КонецЕсли;
...
Процедура ЗаполнитьПоляДомовСтроенийЗемельныхУчастков(Знач ВыбранноеЗначение)
	...
	УстановитьМуниципальныеЧастиАдреса(НаселенныйПунктДетально, ТипЧисло.ПривестиЗначение(ВыбранноеЗначение.ОКТМО), ВыбранноеЗначение.Идентификатор);
	ОбработкаИзмененияАдреса();
...
&НаСервереБезКонтекста
Функция ОпределитьАдресПоИдентификатору(СведенияОбАдреса, ИсключатьГородИзМуниципальногоАдреса = Ложь)
	Возврат Обработки.РасширенныйВводКонтактнойИнформации.СписокРеквизитовНаселенныйПункт(СведенияОбАдреса, ИсключатьГородИзМуниципальногоАдреса);
КонецФункции
...
Процедура УстановитьНаселенныйПунктПослеВыбора(Знач ДанныеОбАдресе)
	...
	НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, ДанныеОбАдресе, Ложь, ИсключатьГородИзМуниципальногоАдреса);
	ОбработкаИзмененияАдреса();
	УстановитьФлагиВМенюПоТипуАдреса();
...
Процедура ОбновитьПредставлениеАдреса()
	...
	РаботаСАдресамиКлиентСервер.ОбновитьПредставлениеАдреса(НаселенныйПунктДетально, ВидКонтактнойИнформации.ВключатьСтрануВПредставление, ИсключатьГородИзМуниципальногоАдреса);
	ПредставлениеАдреса = НаселенныйПунктДетально.Value;
		...
		Иначе
			...
			Для ИндексАдреса = 0 По ИменаУровнейАдреса.Количество() - 1 Цикл
				Если ЭлементыАдреса.Количество() > 0 Тогда
					...
					Если Позиция > 0 И НЕ РаботаСАдресамиКлиент.ПредставлениеУровняБезСокращения(ИменаУровнейАдреса[ИндексАдреса]) Тогда
...
Функция ПредставлениеНаселенногоПунктаАдреса(Адрес)
	...
	Для каждого ИмяУровня Из РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(Адрес.AddressType, Ложь,, ИсключатьГородИзМуниципальногоАдреса) Цикл
...
&НаСервереБезКонтекста
Процедура УстановитьМуниципальныеЧастиАдреса(НаселенныйПунктДетально, ОКТМО, ИдентификаторАдреса)
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		МодульАдресныйКлассификаторСлужебный.УстановитьМуниципальныеСведения(НаселенныйПунктДетально, ОКТМО, ИдентификаторАдреса);
