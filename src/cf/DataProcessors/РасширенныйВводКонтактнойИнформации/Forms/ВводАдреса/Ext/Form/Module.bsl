...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначенияПолей = ОпределитьЗначениеАдреса();
	...
	ОпределитьОтображениеЭлементовИЦветов(Параметры.ТолькоПросмотр);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		Если ЕстьПравоЗагружатьКлассификатор Тогда
			ТексСообщения = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Для автоподбора и проверки адресов загрузите адресные сведения. <a href=load>Загрузить</a>'"));
			Элементы.ИнформацияОЗагрузкеАдресныеСведений.Заголовок = ТексСообщения;
		Иначе
	...
	Иначе
		Элементы.ЗагрузитьКлассификатор.Видимость = Ложь;
		Элементы.СведенияОбАдресе.Видимость = Ложь;
	КонецЕсли;
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType)
		...
		УстановитьЗначенияПолейПослеИзмененияПредставленияНаСервере(ПредставлениеАдреса, Неопределено);
	КонецЕсли;
	
	ИдентификаторНаселенногоПункта = ИдентификаторАдреса(НаселенныйПунктДетально, Ложь);
	СодержитУлицы = РаботаСАдресами.АдресСодержитУлицы(ИдентификаторНаселенногоПункта, НаселенныйПунктДетально.AddressType);
	Элементы.Улица.ПодсказкаВвода = УстановитьПодсказкуВводаДляУлиц(СодержитУлицы);
...
Процедура ДомПриИзменении(Элемент)
	...
	НаселенныйПунктДетально.apartmentId = "";
...
Процедура ТипПомещенияПриИзменении(Элемент)
	...
	ОбновитьМенюДобавленияСтроенийИПомещений();
	ЗаполнитьИдентификаторПомещения();
	ОбработкаИзмененияАдреса();
...
Процедура НаселенныйПунктАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если МеждународныйФорматАдреса Или УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПредставлениеАдресаПриИзменении(Элемент)
	...
	ПредставлениеАдресаКеш = "";
	НаселенныйПунктДетально.value = ПредставлениеАдреса;
	ПодключитьОбработчикОжидания("УстановитьЗначенияПолейПослеИзмененияПредставления", 0.1, Истина);
...
&НаКлиенте
Процедура ПомещениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	ПоказатьСписокПомещений();
	
КонецПроцедуры

&НаКлиенте
Процедура ПомещениеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ЗаполнитьПоляПомещения(ВыбранноеЗначение);
...
Процедура ПомещениеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ЭтоНациональныйАдрес = РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(НаселенныйПунктДетально.addressType)
		Или РаботаСАдресамиКлиентСервер.ЭтоАдминистративноТерриториальныйАдрес(НаселенныйПунктДетально.addressType);
			
	Если Не ЭтоНациональныйАдрес Тогда
		Возврат;
	КонецЕсли;
	
	ИдентификаторДома = ИдентификаторАдреса(НаселенныйПунктДетально, Истина, Истина);

	Если ЗначениеЗаполнено(ИдентификаторДома) Тогда
		
		ДанныеВыбора = СписокАвтоподбораПомещений(ИдентификаторДома, Текст + "%");
...
Процедура ОписаниеОбращенияВПоддержкуОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку") Тогда
		...
		Если НавигационнаяСсылка = "ВопросВПоддержку" Тогда
			...
			ПараметрыОбращения.Тема = РаботаСАдресамиСлужебныйКлиент.ТемаОбращенияВПоддержку();
			ПараметрыОбращения.ТехнологическаяИнформация = ИнформацияДляПоддержки(
				ПредставлениеАдреса,
				НаселенныйПунктДетально);
			
			ОписаниеОшибок = Новый Массив;
			ОшибкиЗаполнения = СписокОшибокЗаполнения(НаселенныйПунктДетально, ВидКонтактнойИнформации, Ложь, Ложь);
			
			Для Каждого ОшибкаЗаполнения Из ОшибкиЗаполнения Цикл
				ОписаниеОшибок.Добавить(ОшибкаЗаполнения.ТекстОшибки);
			КонецЦикла;
			
			ОписаниеОшибки = СтрСоединить(ОписаниеОшибок, "; ");
			
			ПараметрыОбращения.Сообщение = РаботаСАдресамиСлужебныйКлиент.ТекстОбращенияВПоддержку(
				ПредставлениеАдреса,
				ОписаниеОшибки);
...
&НаКлиенте
Процедура СведенияОбАдресеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	
	Если НавигационнаяСсылка = "СведенияОбАдресе"
		И ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
...
&НаКлиенте
Процедура РегионИностранногоАдресаПриИзменении(Элемент)
	ЗаполнитьПолеИностранногоАдреса("area", РегионИностранногоАдреса);	
КонецПроцедуры

&НаКлиенте
Процедура РайонИностранногоАдресаПриИзменении(Элемент)
	ЗаполнитьПолеИностранногоАдреса("district", РайонИностранногоАдреса);
КонецПроцедуры

&НаКлиенте
Процедура ВнутригородскойРайонИностранногоАдресаПриИзменении(Элемент)
	ЗаполнитьПолеИностранногоАдреса("locality", ВнутригородскойРайонИностранногоАдреса);
КонецПроцедуры
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	ЭтоНациональныйАдрес = ЭтоНациональныйАдрес();
		
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ВключаемыйТипАдреса = ?(УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType),
			Неопределено, УправлениеКонтактнойИнформациейКлиентСервер.АдресВСвободнойФорме()); 
		ПереключитьНаСтруктурированныйВвод(ВключаемыйТипАдреса);
...
Процедура АдресныйКлассификаторУстарел(Команда)
	...
	СообщениеОПроверкеАдреса = НСтр("ru = 'Адресные сведения, загруженные в приложение, устарели.
...
Функция ИнформацияДляПоддержки(ПредставлениеАдреса, НаселенныйПунктДетально)
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		СведенияОбАдресе = СведенияОбАдресе(НаселенныйПунктДетально);
		
		ИнформацияДляПоддержки = МодульАдресныйКлассификаторСлужебный.ИнформацияДляПоддержки(
			ПредставлениеАдреса,
			СведенияОбАдресе);
	...
	Возврат ИнформацияДляПоддержки
...
Процедура ОтключитьВозможностьВыбораАдресныхСведенийИзКлассификатора()
	...
	Элементы.Помещение.КнопкаВыбора                = Ложь;
...
&НаКлиенте
Процедура ЗаполнитьИдентификаторПомещения()
	
	НаселенныйПунктДетально.ApartmentId = "";
	ПодключитьОбработчикОжидания("ЗаполнитьИдентификаторПомещенияДляПервогоПомещения", 0.2, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьИдентификаторПомещенияДляПервогоПомещения()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторДома) Тогда
		Возврат;
	КонецЕсли;
	
	СписокПомещений = СписокАвтоподбораПомещений(ИдентификаторДома, Помещение);
	
	Если СписокПомещений = Неопределено Или СписокПомещений.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИдентификаторПомещения = "";
	Для Каждого СвойстваПомещения Из СписокПомещений Цикл
		Если СвойстваПомещения.Значение.ТипПомещения = ТипПомещения Тогда
			ИдентификаторПомещения = СвойстваПомещения.Значение.ИдентификаторПомещения;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	НаселенныйПунктДетально.ApartmentId = ИдентификаторПомещения;
...
Процедура ПереключитьАдресВСвободнойФорму()
	...
	ПереключитьОтображениеАдресВСвободнойФорме();
...
Процедура ОчиститьНаселенныйПункт()
	...
	НаселенныйПунктДетально.areaValue = "";
...
Процедура ОтобразитьЭлементыФормыВСоответствиеСоСтраной()
	...
	Если Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		...
		ТипАдресаИзАдреса = НаселенныйПунктДетально.AddressType;
		ПереопределитьТипаАдресаПоСтране(Страна, НаселенныйПунктДетально, СведенияОСтране, МеждународныйФорматАдреса);
		ПеренестиЗначенияПолейАдресВПоляНовогоТипа(ТипАдресаИзАдреса);
		ТипАдресаИзАдреса = НаселенныйПунктДетально.AddressType;
	
	КонецЕсли;

	УстановитьЗначенияНациональныхРеквизитовКонтактнойИнформации(НаселенныйПунктДетально);
	
	ОтобразитьПоляПоТипуАдреса();
	НужноОбновитьПреставление = ПривестиИндексКФорматуСтраны();
...
&НаСервере
Функция ВосстановитьАдрес(ПредставлениеАдреса, НаселенныйПункт2, ТипАдреса)
...
Процедура ПереопределитьТипаАдресаПоСтране(Страна, НаселенныйПунктДетально, СведенияОСтране, МеждународныйФорматАдреса)
	...
	Если РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(Страна) И Не МеждународныйФорматАдреса Тогда
		Если СтрСравнить(НаселенныйПунктДетально.AddressType, РаботаСАдресамиКлиентСервер.МуниципальныйАдрес()) <> 0 Тогда
...
Процедура ПеренестиЗначенияПолейАдресВПоляНовогоТипа(ТипАдреса)
	...
	Для Счетчик = 0 По СведенияАдреса.Количество() - 1 Цикл
		...
		Если ИменаУровнейНовогоТипа.Количество() > Счетчик Тогда
	...
	Если НаселенныйПунктДетально.Свойство("street") Тогда
		Если ЭтоИностранныйАдрес(НаселенныйПунктДетально.AddressType) Тогда
			НаселенныйПунктДетально["street"] = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(
				НаселенныйПунктДетально, "street", "Type");
			НаселенныйПунктДетально["StreetType"] = ""; // В иностранном адресе нет сокращений.
		Иначе
			Если Не ЗначениеЗаполнено(НаселенныйПунктДетально["streetType"]) Тогда
				ЧастиАдресногоОбъекта = СтрРазделить(НаселенныйПунктДетально["street"], " ");
				Если ЧастиАдресногоОбъекта.Количество() > 1 Тогда
					НаселенныйПунктДетально["StreetType"] = ЧастиАдресногоОбъекта[ЧастиАдресногоОбъекта.ВГраница()];
					ЧастиАдресногоОбъекта.Удалить(ЧастиАдресногоОбъекта.ВГраница());
					НаселенныйПунктДетально["street"] = СтрСоединить(ЧастиАдресногоОбъекта, " ");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЭтоИностранныйАдрес(НаселенныйПунктДетально.AddressType) Тогда
		...
		Если НаселенныйПунктДетально.Свойство("buildings") Тогда
			Если ТипЗнч(НаселенныйПунктДетально["buildings"]) = Тип("Массив") И НаселенныйПунктДетально["buildings"].Количество() > 0 Тогда
				ПерваяЗапись = НаселенныйПунктДетально["buildings"].Получить(0);
				НаселенныйПунктДетально["buildings"] = Новый Массив;
				НаселенныйПунктДетально["buildings"].Добавить(ПерваяЗапись);
			Иначе
				НаселенныйПунктДетально["buildings"] = Новый Массив;	 
			КонецЕсли;
		КонецЕсли;
		
		Если НаселенныйПунктДетально.Свойство("apartments") Тогда
			Если ТипЗнч(НаселенныйПунктДетально["apartments"]) = Тип("Массив") И НаселенныйПунктДетально["apartments"].Количество() > 0 Тогда
				ПерваяЗапись = НаселенныйПунктДетально["apartments"].Получить(0);
				НаселенныйПунктДетально["apartments"] = Новый Массив;
				НаселенныйПунктДетально["apartments"].Добавить(ПерваяЗапись);
			Иначе
				НаселенныйПунктДетально["apartments"] = Новый Массив;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОтобразитьВыбранныйТипАдреса(ВыбранныйТипАдреса = Неопределено)
	...
	ПроверкаОбновленияПолейПослеИзмененияПредставления();
	ПредставлениеАдресаКеш = Неопределено;
	
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType)
		И Элементы.АдресВСвободнойФорме.Пометка Тогда
			...
			Иначе
				...
				ИменаУровней = РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса("Все", Истина);
				...
				ТипАдреса = ?(СведенияОСтране.УчастникЕАЭС,
				...
				Если ЧастиАдреса.Количество() > 0 Тогда
					ОписаниеТипаЧисло = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10, 0));
					ИндексИзАдреса = ОписаниеТипаЧисло.ПривестиЗначение(ЧастиАдреса.Получить(0));
					Если ИндексИзАдреса >= 0 Тогда
						НаселенныйПунктДетально["ZIPCode"] = СокрЛП(ЧастиАдреса.Получить(0));
						ЧастиАдреса.Удалить(0);
					КонецЕсли;
				КонецЕсли;
				
				ИменаУровнейНовогоТипа = РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(ТипАдреса, Истина);
				Для Счетчик = 0 По ЧастиАдреса.Количество() - 1 Цикл
					
					Если ИменаУровнейНовогоТипа.Количество() > Счетчик Тогда
						ИмяУровня = ИменаУровнейНовогоТипа.Получить(Счетчик);
						НаселенныйПунктДетально[ИмяУровня] = СокрЛП(ЧастиАдреса.Получить(0));
					Иначе
						Прервать;
					КонецЕсли;
					ЧастиАдреса.Удалить(0);
				...
				Если ЧастиАдреса.Количество() > 0 Тогда
					...
					СведенияОДоме = СтрРазделить(СокрЛП(ЧастиАдреса.Получить(0)), " ");
					Если СведенияОДоме.Количество() > 1 Тогда
						НаселенныйПунктДетально["HouseType"] = СведенияОДоме.Получить(0);
						СведенияОДоме.Удалить(0);
					КонецЕсли;
					НаселенныйПунктДетально["HouseNumber"] = СтрСоединить(СведенияОДоме, " ");
					ЧастиАдреса.Удалить(0);
				...
				Если ЧастиАдреса.Количество() > 0 Тогда
					...
					СведенияОЗдании = СтрРазделить(СокрЛП(ЧастиАдреса.Получить(0)), " ");
					НаселенныйПунктДетально["buildings"] = Новый Массив;
					ТипЗданияИзАдреса =  СведенияОЗдании.Получить(0);
					СведенияОЗдании.Удалить(0);
					ДанныеЗдания = СтрСоединить(СведенияОЗдании, " ");
					ЗданиеИзАдреса = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(ТипЗданияИзАдреса, ДанныеЗдания);
					НаселенныйПунктДетально["buildings"].Добавить(ЗданиеИзАдреса);
					ЧастиАдреса.Удалить(0);
				...
				Если ЧастиАдреса.Количество() > 0 Тогда
					...
					СведенияОПомещении = СтрРазделить(СокрЛП(ЧастиАдреса.Получить(0)), " ");
					НаселенныйПунктДетально["apartments"] = Новый Массив;
					ТипПомещенияИзАдреса =  СведенияОПомещении.Получить(0);
					СведенияОПомещении.Удалить(0);
					ДанныеПомещения = СтрСоединить(СведенияОПомещении, " ");
					ПомещениеИзАдреса = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(ТипПомещенияИзАдреса, ДанныеПомещения);
					НаселенныйПунктДетально["apartments"].Добавить(ПомещениеИзАдреса);
					ЧастиАдреса.Удалить(0);
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		ПроизвольныйВводАдреса = Ложь;
		ПереключитьОтображениеАдресВСвободнойФорме();
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоИностранныйАдресИлиЕАЭС(НаселенныйПунктДетально.AddressType) Тогда
		ВидимостьПолейИностраногоАдреса(Истина);
	КонецЕсли;
...
Процедура ПоказатьСписокДомовИлиЗемельныхУчастков(ВключитьОтбор)
	...
	Если ЗначениеЗаполнено(Идентификатор) Тогда
		...
		НаименованиеАдресногоОбъекта = РаботаСАдресамиКлиентСервер.НаименованиеРодительскогоОбъекта(НаселенныйПунктДетально, Идентификатор);
		...
		ПараметрыФормы.Вставить("НаименованиеАдресногоОбъекта", НаименованиеАдресногоОбъекта);
		...
		ОповещениеОЗакрытие = Новый ОписаниеОповещения("ВыбранДом", ЭтотОбъект);
		ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ВыборДома", ПараметрыФормы, ЭтотОбъект,,,, ОповещениеОЗакрытие, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
...
&НаКлиенте
Процедура ПоказатьСписокПомещений()
	
	ИдентификаторДома = ИдентификаторАдреса(НаселенныйПунктДетально, Истина, Истина);
	
	Если ЗначениеЗаполнено(ИдентификаторДома) Тогда
		
		ИдентификаторУлицы = ИдентификаторАдреса(НаселенныйПунктДетально, Истина);
		НаименованиеАдресногоОбъекта = "";
		Если ЗначениеЗаполнено(ИдентификаторУлицы) Тогда
			НаименованиеАдресногоОбъекта = РаботаСАдресамиКлиентСервер.НаименованиеРодительскогоОбъекта(НаселенныйПунктДетально, ИдентификаторУлицы);
		КонецЕсли;
		НаименованиеДома = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Дом, НРег(ТипДома), Ложь);
		НаименованиеАдресногоОбъекта = НаименованиеАдресногоОбъекта + " " + НаименованиеДома;
		
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("Идентификатор", ИдентификаторДома);
		ПараметрыФормы.Вставить("НаименованиеАдресногоОбъекта", НаименованиеАдресногоОбъекта);
		ПараметрыФормы.Вставить("НомерПомещения", Помещение);
		ПараметрыФормы.Вставить("ТипПомещения", ТипПомещения);
		
		ОповещениеОЗакрытие = Новый ОписаниеОповещения("ВыбраноПомещение", ЭтотОбъект);
		ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ВыборПомещения", ПараметрыФормы,
			ЭтотОбъект,,,, ОповещениеОЗакрытие, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр("ru = 'Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации помещений для введенного адреса.'");
		Иначе
...
&НаКлиенте
Процедура ВыбранДом(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ВыбраноПомещение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		ЗаполнитьПоляПомещения(Результат);
	КонецЕсли;
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	Если ТипЗнч(ДанныеВыбора) = Тип("Структура") Тогда
		...
		УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнуюИнформациюИзJSONВXML(ДанныеВыбора, 
			КонтактнаяИнформация, Перечисления.ТипыКонтактнойИнформации.Адрес);
	Иначе
		Значение             = "";
	КонецЕсли;
...
Функция АдресныеСведенияКлассификатораЗагружены(НаименованиеРегиона)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		СведенияОАдресномКлассификаторе = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.СведенияОДоступностиАдресногоКлассификатора();
		Результат.ТекстОшибки           = СведенияОАдресномКлассификаторе["ТекстОшибки"];
...
&НаКлиенте
Процедура ЗаполнитьПолеИностранногоАдреса(ИмяПоля, ЗначениеПоля)
	
	НаселенныйПунктДетально[ИмяПоля]          = ЗначениеПоля;
	НаселенныйПунктДетально[ИмяПоля + "Type"] = "";
	НаселенныйПунктДетально[ИмяПоля + "ID"]   = "";
	
	ОбновитьСписокУровней(НаселенныйПунктДетально, ИмяПоля, ЗначениеЗаполнено(ЗначениеПоля));
	ОбновитьПредставлениеАдреса();
...
&НаКлиенте
Функция ПривестиИндексКФорматуСтраны()
	
	ИндексБылИзменен = Ложь;
	
	МаксимальнаяДлинаПочтовогоИндекса = ?(ЭтоНациональныйАдрес(), 6, 10);
	
	Элементы.Индекс.ОграничениеТипа = Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(МаксимальнаяДлинаПочтовогоИндекса));
	
	Если ЭтоНациональныйАдрес() И ЗначениеЗаполнено(Индекс) Тогда
		ЛишниеСимволы = СтрСоединить(СтрРазделить(Индекс, "0123456789"), "");
		Индекс        = СтрСоединить(СтрРазделить(Индекс, ЛишниеСимволы), "");
		Если ЗначениеЗаполнено(ЛишниеСимволы) Тогда
			ИндексБылИзменен = Истина;
		КонецЕсли;
		Если СтрДлина(Индекс) > МаксимальнаяДлинаПочтовогоИндекса Тогда
			Индекс = Лев(Индекс, МаксимальнаяДлинаПочтовогоИндекса);
			НаселенныйПунктДетально.Zipcode = Индекс;
			ИндексБылИзменен = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ИндексБылИзменен;
	
КонецФункции

&НаКлиенте
Функция ЭтоНациональныйАдрес()
	Возврат РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(Страна) И Не МеждународныйФорматАдреса;
КонецФункции
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	УлицаЗаголовок = НСтр("ru = 'Улица'");
	ОтображатьПоляИностранногоАдреса = Ложь;
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Элементы.Строение.КнопкаВыбора               = Ложь;
		Элементы.Помещение.КнопкаВыбора              = АвтоподборДоступен;
		Элементы.Строение.Доступность                = Не ЭтоЗемельныйУчасток;
		Элементы.ТипСтроения.Доступность             = Не ЭтоЗемельныйУчасток;
		Элементы.НаселенныйПункт.ПодсказкаВвода      = НСтр("ru = 'Выберите или введите название населенного пункта'");
	...
	Иначе
		...
		Если Не МеждународныйФорматАдреса Тогда
			
			ВидимостьСтроений                     = Истина;
			Элементы.Дом.КнопкаВыбора             = Ложь;
			Элементы.Строение.КнопкаВыбора        = Ложь;
			ОтображатьПоляИностранногоАдреса      = Истина;
			Элементы.Помещение.КнопкаВыбора       = Ложь;
			
		Иначе
			УлицаЗаголовок                        = НСтр("ru = 'Адрес'");
	...
	Элементы.Улица.Заголовок                        = УлицаЗаголовок;
	ВидимостьПолейИностраногоАдреса(ОтображатьПоляИностранногоАдреса);
	
	// Проверять, вводить в свободной форме и искать по индексу можем только российские адреса.
	Элементы.Улица.МногострочныйРежим                          = Не ВидимостьСтроений; // Если нет строений, то улица многострочная
	...
	Если ВидКонтактнойИнформации.ВключатьСтрануВПредставление И ЭтоНациональныйАдрес() Тогда
		ОбновитьПредставлениеАдреса();
	КонецЕсли;
...
&НаКлиенте
Процедура ВидимостьПолейИностраногоАдреса(Видимость)
	
	Элементы.РегионИностранногоАдреса.Видимость     = Видимость;
	Элементы.РайонИностранногоАдреса.Видимость      = Видимость;
	Элементы.ВнутригородскойРайонИностранногоАдреса.Видимость = Видимость;
...
&НаСервере
Процедура ОпределитьОтображениеЭлементовИЦветов(ТолькоПросмотр)
	...
	Иначе
		...
		Если СведенияОЗагрузке.АдресныйКлассификаторДоступен Тогда
			Если Не СведенияОЗагрузке.РегионЗагружен Тогда
				ТекстОшибки = ДополнитьТекстОшибкиСсылкойДляЗагрузки(СведенияОЗагрузке);
				Элементы.ИнформацияОЗагрузкеАдресныеСведений.Заголовок = ТекстОшибки;
...
&НаСервере
Функция ДополнитьТекстОшибкиСсылкойДляЗагрузки(СведенияОЗагрузке)
	
	Если ПустаяСтрока(СведенияОЗагрузке.ТекстОшибки) Тогда
		Возврат "";
	КонецЕсли;
	...
	Если  Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
			МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
			ИППодключена = МодульАдресныйКлассификаторСлужебный.ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки();
			Если Не ИППодключена Тогда
				ТекстОшибки = СтроковыеФункции.ФорматированнаяСтрока(СведенияОЗагрузке.ТекстОшибки + " <a href=""load"">%1</a>", НСтр("ru = 'Загрузить'"));
				Возврат  ТекстОшибки;
			КонецЕсли;
		КонецЕсли;
	...
	Возврат СведенияОЗагрузке.ТекстОшибки;
...
Процедура УстановитьЗначенияНациональныхРеквизитовКонтактнойИнформации(ДанныеАдреса)
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоИностранныйАдресИлиЕАЭС(ДанныеАдреса.addressType) Тогда
		Если МеждународныйФорматАдреса Тогда
			НаселенныйПункт = 
				УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(ДанныеАдреса, "area", "Type");
		Иначе
			РегионИностранногоАдреса = 
				УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(ДанныеАдреса, "area", "Type");
			РайонИностранногоАдреса = 
				УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(ДанныеАдреса, "district", "Type", Истина);
			ВнутригородскойРайонИностранногоАдреса = 
				УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(ДанныеАдреса, "locality", "Type", Истина);
			НаселенныйПункт = 
				УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(ДанныеАдреса, "city", "Type", Истина);
		КонецЕсли;
	Иначе
	...
	Улица = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(ДанныеАдреса, "street", "Type", Истина);
...
Функция ДобавитьПолеСтроенияИлиПомещения(Идентификатор, Тип, Значение)
	...
	Если Тип = "Помещение" Тогда
		...
		НовыйЭлемент.УстановитьДействие("НачалоВыбора", "Подключаемый_ПомещениеНачалоВыбора");
		НовыйЭлемент.УстановитьДействие("ОбработкаВыбора", "Подключаемый_ПомещениеОбработкаВыбора");
		НовыйЭлемент.УстановитьДействие("АвтоПодбор", "Подключаемый_ПомещениеАвтоПодбор");
		
		ЭтоНациональныйАдрес = РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(НаселенныйПунктДетально.addressType)
			Или РаботаСАдресамиКлиентСервер.ЭтоАдминистративноТерриториальныйАдрес(НаселенныйПунктДетально.addressType);
			
		НовыйЭлемент.КнопкаВыбора = ЭтоНациональныйАдрес;
...
Процедура ОтобразитьКоличествоЗаписейВИсторииИзменений()
	...
	НайденныеСтроки = КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Отбор);
	Если Параметры.ИзФормыИстории Тогда
...
Процедура Подключаемый_ТипПомещенияПриИзменении(Элемент)
	...
	Для каждого СвойстваПомещения Из НаселенныйПунктДетально.Apartments Цикл
		Если СтрСравнить(СвойстваПомещения.Type, СтароеЗначение) = 0 Тогда
			СвойстваПомещения.Type = ЭтотОбъект[Элемент.Имя];
			ОбновитьПредставлениеАдреса();
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура Подключаемый_ПомещениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура Подключаемый_ПомещениеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура Подключаемый_ПомещениеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИдентификаторДома = НаселенныйПунктДетально.houseId;
		Если ЗначениеЗаполнено(ИдентификаторДома) Тогда
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если НЕ ВебСервисИспользуется И ТребуетсяОбновление Тогда
			Если ЕстьПравоЗагружатьКлассификатор Тогда
				СообщениеОПроверкеАдреса = НСтр("ru = 'Адрес введен корректно, но адресные сведения, загруженные в приложение, давно не обновлялись.
				|Рекомендуется выполнить обновление и повторить проверку адреса заново.'");
				ВопросОбОбновлениеАдресногоКлассификатора(СообщениеОПроверкеАдреса);
			Иначе
				СообщениеОПроверкеАдреса = НСтр("ru = 'Адрес введен корректно, но адресные сведения, загруженные в приложение,
...
&НаСервереБезКонтекста
Функция НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, Знач СведенияОбАдресе, Знач ВключатьУлица)
	...
	Если ЗначениеЗаполнено(ПолученныйАдрес.houseNumber) Тогда
		ЗаполнитьЗначенияСвойств(НаселенныйПунктДетально, ПолученныйАдрес, 
			"houseNumber,houseType,houseId");
	Иначе
		НаселенныйПунктДетально.houseId     = "";
	КонецЕсли;
	
	Если ТипЗнч(ПолученныйАдрес.buildings) = Тип("Массив") И ПолученныйАдрес.buildings.Количество() > 0 Тогда
		НаселенныйПунктДетально.buildings = ПолученныйАдрес.buildings;
	КонецЕсли;
	
	Если ТипЗнч(ПолученныйАдрес.apartments) = Тип("Массив") И ПолученныйАдрес.apartments.Количество() > 0 Тогда
		НаселенныйПунктДетально.apartments = ПолученныйАдрес.apartments;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(НаселенныйПунктДетально, ПолученныйАдрес,
		"areaValue,addressType,admLevels,munLevels,version,stead,steadId");
	
	Если ПустаяСтрока(ПолученныйАдрес.street) Тогда
		НаселенныйПунктДетально.streetId = "";
	ИначеЕсли ЗначениеЗаполнено(ПолученныйАдрес.streetId) Тогда
		НаселенныйПунктДетально.id = ПолученныйАдрес.streetId;
	КонецЕсли;
	
	РаботаСАдресами.ДобавитьУровеньУлицыДляАдреса(НаселенныйПунктДетально);
	...
	ИдентификаторНаселенногоПункта = ИдентификаторАдреса(НаселенныйПунктДетально, Ложь, Ложь);
	СодержитУлицы = РаботаСАдресами.АдресСодержитУлицы(ИдентификаторНаселенногоПункта, 
...
//
&НаКлиенте
Процедура ПослеПроверкиДоступностиКлассификатораВФоне(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ДоступностьКлассификатора = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
...
Процедура ЗаполнитьПоляДомовСтроенийЗемельныхУчастков(Знач ВыбранноеЗначение)
	...
	УстановитьМуниципальныеЧастиАдресаИИдентификаторы(НаселенныйПунктДетально, ВыбранноеЗначение);
	ОбработкаИзмененияАдреса();
	
КонецПроцедуры

// Параметры:
//  ВыбранноеЗначение - см. АдресныйКлассификаторСлужебный.ОписаниеПомещения
//
&НаКлиенте
Процедура ЗаполнитьПоляПомещения(Знач ВыбранноеЗначение)

	Модифицированность = Истина;
	НаселенныйПунктДетально.Apartments.Очистить();

	ТипПомещения = ВыбранноеЗначение.ТипПомещения;
	Помещение = ВыбранноеЗначение.НомерПомещения;
	НаселенныйПунктДетально.ApartmentId = ВыбранноеЗначение.ИдентификаторПомещения;
	
	ИнформацияОПомещении = ЗначениеСтроенияИлиПомещения(ТипПомещения, Помещение);
	НаселенныйПунктДетально.Apartments.Добавить(ИнформацияОПомещении);
	
	ОбновитьМенюДобавленияСтроенийИПомещений();
	ОбработкаИзмененияАдреса(Истина);
...
Процедура ЗаполнитьПредопределенныеВариантыАдреса()
	...
	Если ЗначениеЗаполнено(Параметры.ТипПомещения)
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.УточнениеОшибки = НСтр("ru = 'Автоподбор и проверка адреса недоступны:'");
	
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.РасширенныйВводКонтактнойИнформации.ПроверитьДоступностьКлассификатора",
		Новый Структура,
		НастройкиЗапуска);
...
Процедура ПоказатьАдресВСвободнойФорме()
	...
	Если Не СведенияОСтране.УчастникЕАЭС Или МеждународныйФорматАдреса Тогда
		ОчиститьАдресКлиент();
	КонецЕсли;
	УдалитьСтрануИзПредставления();
	...
	Если Элементы.Страна.Доступность Тогда
		Элементы.Страна.Видимость = Не ВидКонтактнойИнформации.ТолькоНациональныйАдрес;
	КонецЕсли;
	...
	ВидимостьПолейИностраногоАдреса(Ложь);
...
&НаКлиенте
Процедура ПереключитьОтображениеАдресВСвободнойФорме()
	
	ЭтоАдресВСвободнойФорме = УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType);
	Элементы.ПредставлениеАдреса.ПодсказкаВвода = ?(ЭтоАдресВСвободнойФорме, НСтр("ru = 'Введите или вставьте адрес'"), "");
	Элементы.ПредставлениеАдреса.ТолькоПросмотр = Не ЭтоАдресВСвободнойФорме;
	Элементы.Улица.КнопкаВыбора = Не ЭтоАдресВСвободнойФорме;
	Элементы.Дом.КнопкаВыбора = Не ЭтоАдресВСвободнойФорме;
	Элементы.Строение.КнопкаВыбора = Не ЭтоАдресВСвободнойФорме;
	Элементы.Помещение.КнопкаВыбора = Не ЭтоАдресВСвободнойФорме;
	Элементы.ТипДома.РедактированиеТекста = ЭтоАдресВСвободнойФорме;
	Элементы.ТипСтроения.РедактированиеТекста = ЭтоАдресВСвободнойФорме;
	Элементы.ТипДома.РежимВыбораИзСписка = Не ЭтоАдресВСвободнойФорме;
	Элементы.ТипСтроения.РежимВыбораИзСписка = Не ЭтоАдресВСвободнойФорме;
	Элементы.ГруппаИнформационныеСообщения.Видимость =  Не ЭтоАдресВСвободнойФорме;
	Если Не ЭтоАдресВСвободнойФорме Тогда
		Если Элементы.ТипДома.СписокВыбора.НайтиПоЗначению(ТипДома) = Неопределено Тогда
			ТипДома = ПервыйИлиПустой(Элементы.ТипДома.СписокВыбора);
		КонецЕсли;
		Если Элементы.ТипСтроения.СписокВыбора.НайтиПоЗначению(ТипСтроения) = Неопределено Тогда
			ТипСтроения = ПервыйИлиПустой(Элементы.ТипСтроения.СписокВыбора);
		КонецЕсли;
	КонецЕсли;
	
	Если ЭтоАдресВСвободнойФорме Тогда
		ВебКлассификаторДоступен          = Ложь;
		ИспользуетсяАдресныйКлассификатор = Ложь;
	Иначе	
		СведенияОАдресномКлассификаторе    = СведенияОАдресномКлассификаторе();
		ЕстьЗагруженныйКлассификатор       = СведенияОАдресномКлассификаторе.Получить("ИспользоватьЗагруженные");
		ВебКлассификаторДоступен           = СведенияОАдресномКлассификаторе.Получить("КлассификаторДоступен");
		ИспользуетсяАдресныйКлассификатор  = СведенияОАдресномКлассификаторе.Получить("ИспользуетсяАдресныйКлассификатор");
		ВебСервисИспользуется              = ВебКлассификаторДоступен;
	...
	Если Не ЭтоНациональныйАдрес() Тогда
		ЭтоАдресВСвободнойФорме = УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType);
		Элементы.Улица.Видимость = Не ЭтоАдресВСвободнойФорме;
		Элементы.НаселенныйПункт.Видимость = Не ЭтоАдресВСвободнойФорме;
		Элементы.Индекс.Видимость = Не ЭтоАдресВСвободнойФорме;
		
		Элементы.ГруппаСтроенияИИнформация.Видимость = Не ЭтоАдресВСвободнойФорме;
		Элементы.Улица.КнопкаВыбора = ЭтоНациональныйАдрес;

	Иначе
		Элементы.Улица.Видимость = Истина;
		Элементы.НаселенныйПункт.Видимость = Истина;
...
&НаСервереБезКонтекста
Функция СведенияОАдресномКлассификаторе()
	Возврат УправлениеКонтактнойИнформациейСлужебныйПовтИсп.СведенияОДоступностиАдресногоКлассификатора();
КонецФункции
...
Функция СписокАвтоподбораНаселенногоПункта(Текст, ТипАдреса)
	...
	ДополнительныеПараметры.ПрямойПорядок = Ложь;
...
&НаСервереБезКонтекста
Функция СписокАвтоподбораПомещений(Знач Идентификатор, Знач Текст)
	Возврат Обработки.РасширенныйВводКонтактнойИнформации.СписокАвтоподбораПомещений(Идентификатор, Текст);
КонецФункции
...
&НаКлиенте
Функция ЭтоИностранныйАдрес(ТипАдреса)
...
Процедура УстановитьНаселенныйПунктПослеВыбора(Знач ДанныеОбАдресе)
	...
	СодержитУлицы = НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, ДанныеОбАдресе, Ложь);
	Элементы.Улица.ПодсказкаВвода = УстановитьПодсказкуВводаДляУлиц(СодержитУлицы);
...
&НаКлиентеНаСервереБезКонтекста
Функция УстановитьПодсказкуВводаДляУлиц(СодержитУлицы)
	Возврат ?(СодержитУлицы, НСтр("ru = 'Выберите или введите улицу'"), "");
КонецФункции
...
&НаСервере
Функция ОпределитьЗначениеАдреса()
	
	ЗначенияПолей = ?(ЗначениеЗаполнено(Параметры.Значение), Параметры.Значение, Параметры.ЗначенияПолей);
...
Процедура ОбновитьПредставлениеАдреса()
	...
	ПредставлениеИзПолей = ?(Не СведенияОСтране.УчастникЕАЭС Или МеждународныйФорматАдреса, Ложь, Истина);
	ПредставлениеАдреса = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(НаселенныйПунктДетально, ВидКонтактнойИнформации.ВключатьСтрануВПредставление, ,ПредставлениеИзПолей);
	НаселенныйПунктДетально.Value = ПредставлениеАдреса;
...
Процедура ИзменитьСведенияОНаселенномПункте()
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоИностранныйАдресИлиЕАЭС(НаселенныйПунктДетально.AddressType) Тогда
		Если МеждународныйФорматАдреса Тогда
			ЗаполнитьПолеИностранногоАдреса("area", "");
		Иначе
			ЗаполнитьПолеИностранногоАдреса("city", "");
		КонецЕсли;
	Иначе
		ОчиститьНаселенныйПункт();
	КонецЕсли;
	...
	Если Не АдресВосстановлен Тогда
		...
		ИначеЕсли Не ЭтоРФАдрес() И Не РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(Страна) Тогда
			
			ЗаполнитьПолеИностранногоАдреса("city", НаселенныйПункт);
			ОбновитьПредставлениеАдреса();
			Возврат;
...
Процедура ПоказатьПодсказкуПоТипАдреса()
	...
	Если МеждународныйФорматАдреса Тогда
		...
		ЦветТекстаПредставления = Новый Цвет(); 
	ИначеЕсли РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(НаселенныйПунктДетально.AddressType) Тогда
		...
		Элементы.ГруппаТребуетсяПомощь.Видимость = Истина;
		ЦветРамки               = ЦветФонаФормы;
		ЦветТекстаПредставления = ЦветПоясняющегоТекста; 
	ИначеЕсли РаботаСАдресамиКлиент.ЭтоАдминистративноТерриториальныйАдрес(НаселенныйПунктДетально.AddressType) Тогда
		...
		ЦветРамки               = ЦветФонаФормы;
		ЦветТекстаПредставления = ЦветПоясняющегоТекста; 
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		Элементы.СтраницаТипАдреса.ТекущаяСтраница = Элементы.СтраницаАдресВСвободнойФорме;
		Элементы.ГруппаТребуетсяПомощь.Видимость = Истина;
		ЦветРамки               = Новый Цвет();
		ЦветТекстаПредставления = Новый Цвет(); 
	Иначе
	...
	Элементы.ПредставлениеАдреса.ЦветРамки = ЦветРамки;
	Элементы.ПредставлениеАдреса.ЦветТекста = ЦветТекстаПредставления;
...
Процедура ОбработкаИзмененияАдреса(ОбновитьДополнительныеЗдания = Истина)
	...
	Иначе
		РегионИностранногоАдреса               = НаселенныйПунктДетально.area;
		РайонИностранногоАдреса                = НаселенныйПунктДетально.district;
		НаселенныйПункт                        = НаселенныйПунктДетально.city;
		ВнутригородскойРайонИностранногоАдреса = НаселенныйПунктДетально.locality;
	КонецЕсли;
...
Процедура ПереключитьНаСтруктурированныйВводЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(СокрЛП(ПредставлениеАдреса)) 
	   И СтрНайти(ВРег(Комментарий), ВРег(ПредставлениеАдреса)) = 0 Тогда
...
Функция ИдентификаторАдреса(Адрес, ВключатьУлицу, ВключатьУровеньДома = Ложь)
	...
	Возврат РаботаСАдресамиКлиентСервер.ИдентификаторАдреса(Адрес, ВключатьУлицу, ВключатьУровеньДома);
...
&НаСервереБезКонтекста
Процедура УстановитьМуниципальныеЧастиАдресаИИдентификаторы(НаселенныйПунктДетально, ВыбранноеЗначение)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		ЕстьИдентификаторПомещения = Ложь;
		Если НаселенныйПунктДетально.apartments.Количество() = 1 Тогда
			ЕстьИдентификаторПомещения = Истина;
			МодульАдресныйКлассификаторСлужебный.УстановитьИдентификаторыАдреса(НаселенныйПунктДетально);
		КонецЕсли;
		...
		Если ЗагруженныеДанные Тогда
			ИдентификаторАдреса = ИдентификаторАдреса(НаселенныйПунктДетально, Истина);
		Иначе
			Если ЗначениеЗаполнено(ВыбранноеЗначение.ИдентификаторДома) И Не ЕстьИдентификаторПомещения Тогда
...
Процедура ПослеВводаСведенийНаселенногоПункта(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Элементы.Улица.ПодсказкаВвода = УстановитьПодсказкуВводаДляУлиц(Результат.УАдресаЕстьУлицы);
		ОбработкаИзмененияАдреса();
...
&НаКлиенте
Процедура УстановитьЗначенияПолейПослеИзмененияПредставления()

	Если Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType)
		И Не Элементы.АдресВСвободнойФорме.Пометка Тогда
		Возврат;
	КонецЕсли;
	
	Если ПредставлениеАдресаКеш = Неопределено Или (ПредставлениеАдресаКеш = ПредставлениеАдреса 
	   И ЗначениеЗаполнено(ПредставлениеАдресаКеш))  Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьЗначенияПолейПослеИзмененияПредставленияНаСервере(ПредставлениеАдреса, НаселенныйПунктДетально);
	ПредставлениеАдресаКеш = ПредставлениеАдреса;

КонецПроцедуры

&НаСервере
Процедура УстановитьЗначенияПолейПослеИзмененияПредставленияНаСервере(ПредставлениеАдреса, Знач Адрес = Неопределено)
	...
	ПредставлениеАдресаДоУстановкиРеквизитов = ПредставлениеАдреса;
	СтранаДоИзменения                        = НаселенныйПунктДетально.country;
	Комментарий                              = НаселенныйПунктДетально.comment;

	ДанныеАдреса = РаботаСАдресами.РазложитьПредставлениеАдресаПоПолям(ПредставлениеАдреса, Адрес);

	НаселенныйПунктДетально = РаботаСАдресами.ПодготовитьАдресДляВвода(ДанныеАдреса);
	НаселенныйПунктДетально.AddressType = УправлениеКонтактнойИнформациейКлиентСервер.АдресВСвободнойФорме();
	НаселенныйПунктДетально.country = ?(ЗначениеЗаполнено(НаселенныйПунктДетально.country),
		НаселенныйПунктДетально.country, СтранаДоИзменения);
	НаселенныйПунктДетально.comment = Комментарий;
	...
	РазрешитьВводАдресаВСвободнойФормеКеш = РазрешитьВводАдресаВСвободнойФорме;
	УстановитьЗначениеРеквизитовПоКонтактнойИнформации(НаселенныйПунктДетально);
	ПредставлениеАдреса = ПредставлениеАдресаДоУстановкиРеквизитов;
	РазрешитьВводАдресаВСвободнойФорме = РазрешитьВводАдресаВСвободнойФормеКеш;
	НаселенныйПунктДетально.value = ПредставлениеАдреса;
	
КонецПроцедуры

&НаКлиенте
Процедура ПроверкаОбновленияПолейПослеИзмененияПредставления()
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType)
		И Элементы.АдресВСвободнойФорме.Пометка Тогда
		УстановитьЗначенияПолейПослеИзмененияПредставления();
	КонецЕсли;
КонецПроцедуры
