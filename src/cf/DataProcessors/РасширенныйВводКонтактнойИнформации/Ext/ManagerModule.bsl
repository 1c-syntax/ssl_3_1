	...
	Для Каждого РезультатПроверки Из РезультатыАнализа.Данные Цикл
			Для Каждого ОшибкаАдреса Из РезультатПроверки.Ошибки Цикл
				...
				Если Обработанные[Ключ] = Неопределено Тогда
					...
					ОписаниеОшибки.Вставить("ПутьXPath",     Ключ);
					ОписаниеОшибки.Вставить("СущностьПоля",  Неопределено);
					ОписаниеОшибки.Вставить("АдресПроверен", РезультатПроверки.АдресПроверен);
					ОписаниеОшибки.Вставить("Варианты",      РезультатПроверки.Варианты);
					
					Результат.Добавить(ОписаниеОшибки, СокрЛП(ОшибкаАдреса.Текст + Символы.ПС + ОшибкаАдреса.Подсказка));
