...
//
Функция СписокРеквизитовНаселенныйПункт(СведенияОбАдресномОбъекте = Неопределено, ИсключатьГородИзМуниципальногоАдреса = Ложь) Экспорт
	...
	ИсключатьГородИзМуниципальногоАдреса = МодульАдресныйКлассификаторСлужебный.ИсключатьГородИзМуниципальногоАдреса(АдресныеСведения.Данные);
...
Функция СформироватьАдресПоПредставлению(Представление)
	...
	Возврат УправлениеКонтактнойИнформациейСлужебный.АдресXMLВXDTO(АдресВXML, Представление, Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	Для каждого ИмяПоля Из СписокПолей Цикл
		...
		Если ОбъектАдреса.Свойство(ИмяПоля) И ЗначениеЗаполнено(ОбъектАдреса[ИмяПоля]) Тогда
			...
			ПредставлениеУровня = НаименованиеУровня + " " + ОбъектАдреса[ИмяПоля + "Type"];
...
// Параметры:
//  Адрес - Строка
//  СтрокаТабличнойЧасти - СтрокаТаблицыЗначений: см. Справочник.Пользователи.КонтактнаяИнформация
// 
