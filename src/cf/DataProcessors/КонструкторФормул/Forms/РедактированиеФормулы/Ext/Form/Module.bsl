...
Процедура ПроверитьФормулу(Команда)
	...
	ШаблонСообщения = НСтр("ru = 'Проверяемое выражение:
	|%1
	|
	|Результат проверки:
	|%2'");
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения, Выражение, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонСообщения, Выражение, Результат);
	
	ПоказатьПредупреждение(, НСтр("ru = 'Проверка выполнена успешно.'") + Символы.ПС + Символы.ПС + ТекстСообщения);
...
Функция СписокОператоров()
	...
	Если Не ЗначениеЗаполнено(СписокОператоров) Тогда
		СписокОператоров = КонструкторФормулСлужебный.СписокОператоров();
	КонецЕсли;
...
Функция ВыражениеДляВставки(Оператор)
	...
	Если Оператор.Родитель.Имя = "ЛогическиеОператорыИКонстанты"
		Или Оператор.Родитель.Имя = "Операторы" Тогда
		Результат = Оператор.Заголовок;
	КонецЕсли;
