...
Процедура ВыполнитьПроверку()
	...
	Иначе
		...
		Попытка
			Результат = Вычислить(Выражение); // Не нарушает безопасность информации, так как на клиенте вычисляется произвольный код, введенный только самим пользователем.
		Исключение
			ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ВызватьИсключение ТекстСообщенияОбОшибкеПриПроверкеФормулы(ТекстОшибки);
		КонецПопытки;
