...
Функция АктуальностьКлассификатораБанков() Экспорт
	...
	Результат.Вставить("ВеличинаПросрочкиСтрокой", ОбщегоНазначения.ИнтервалВремениСтрокой(
		ПоследнееОбновление, ТекущаяДатаСеанса()));
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаСКлассификаторами") Тогда
		МодульРаботаСКлассификаторами = ОбщегоНазначения.ОбщийМодуль("РаботаСКлассификаторами");
		Классификаторы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторКлассификатора());
		Обновления = МодульРаботаСКлассификаторами.ДоступныеОбновленияКлассификаторов(Классификаторы);
		Если Не ЗначениеЗаполнено(Обновления.КодОшибки) Тогда
			ДоступноОбновление = Обновления.ДоступныеВерсии.Количество() > 0;
			Результат.ДоступноОбновление = ДоступноОбновление;
			Результат.КлассификаторПросрочен = ДоступноОбновление И Результат.КлассификаторПросрочен;
		КонецЕсли;
	КонецЕсли;
