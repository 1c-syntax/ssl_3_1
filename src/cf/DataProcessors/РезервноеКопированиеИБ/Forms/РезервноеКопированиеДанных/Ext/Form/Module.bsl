...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.КаталогСРезервнымиКопиями = ?(АвтоматическоеВыполнение, 
		?(Не ПустаяСтрока(НастройкиРезервногоКопирования.КаталогХраненияРезервныхКопий), 
			НастройкиРезервногоКопирования.КаталогХраненияРезервныхКопий,
			НастройкиРезервногоКопирования.КаталогХраненияРезервныхКопийПриРучномЗапуске),
		?(Не ПустаяСтрока(НастройкиРезервногоКопирования.КаталогХраненияРезервныхКопийПриРучномЗапуске),
			НастройкиРезервногоКопирования.КаталогХраненияРезервныхКопийПриРучномЗапуске,
			НастройкиРезервногоКопирования.КаталогХраненияРезервныхКопий));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ТекущаяСтраница <> Элементы.СтраницаИнформацииИВыполненияРезервногоКопирования Тогда
		Возврат;
	КонецЕсли;
...
Процедура Далее(Команда)
	...
	Если ТекущаяСтраницаПомощника = Элементы.СтраницаВыполненияРезервногоКопирования Тогда
		ПерейтиНаСтраницу(Элементы.СтраницаИнформацииИВыполненияРезервногоКопирования);
		УстановитьПутьАрхиваСКопиями(Объект.КаталогСРезервнымиКопиями, АвтоматическоеВыполнение);
	Иначе
		Закрыть();
	КонецЕсли;
...
Процедура ПерейтиНаСтраницу(НоваяСтраница)
	...
	ПерейтиДалее = Истина;
	Если НоваяСтраница = Элементы.СтраницаИнформацииИВыполненияРезервногоКопирования Тогда
		ПерейтиНаСтраницуИнформацииИВыполненияРезервногоКопирования(ПерейтиДалее);
	ИначеЕсли НоваяСтраница = Элементы.СтраницаОшибокПриКопировании 
		Или НоваяСтраница = Элементы.СтраницаУспешногоВыполненияКопирования Тогда
...
#Иначе
	...
	ИначеЕсли НайтиФайлы(Объект.КаталогСРезервнымиКопиями).Количество() = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Указан несуществующий каталог ""%1"". 
			|Если резервное копирование было настроено на другом компьютере, укажите сетевой каталог, доступный с любого компьютера, где ведется работа в приложении.'"),
			Объект.КаталогСРезервнымиКопиями);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.КаталогСРезервнымиКопиями");
		РеквизитыЗаполнены = Ложь;
	Иначе
...
#Область ОбработчикиСобытийФормыНаСервереИИзмененияНастроекРезервногоКопирования

&НаСервереБезКонтекста
Процедура УстановитьПутьАрхиваСКопиями(Знач Путь, Знач АвтоматическоеВыполнение)
	
	НастройкиПути = РезервноеКопированиеИБСервер.НастройкиРезервногоКопирования();
	Если АвтоматическоеВыполнение Тогда
		НастройкиПути.КаталогХраненияРезервныхКопий = Путь;
	Иначе
		НастройкиПути.КаталогХраненияРезервныхКопийПриРучномЗапуске = Путь;
	КонецЕсли;
...
#Область ПроцедурыИФункцииПодготовкиРезервногоКопирования
...
Функция СформироватьФайлыСкрипта()
	...
	ПараметрыСкрипта.ПутьКCOMСоединителю = КаталогПрограммы() + "comcntr.dll";
