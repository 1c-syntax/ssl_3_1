...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Если поле ИспользуемоеИмяФайла есть, то это внешняя обработка.
...
#Если ВебКлиент Тогда
	Если ЗначениеЗаполнено(ПутьКФайлуВнешнейОбработкиНаКлиенте) Тогда
		ТекстОшибки = НСтр("ru = 'Для выполнения действия требуется запустить клиентское приложение.'");
...
Процедура СформироватьОтчетОбОшибкеНажатие(Элемент)
	...
	Если ОтчетОбОшибке <> Неопределено Тогда
		МодульСтандартныеПодсистемыКлиент.ПоказатьОтчетОбОшибке(ОтчетОбОшибке);
	КонецЕсли;
...
Процедура НастроитьВидимостьИЗаголовокСсылкиОтправкиОтчетаОбОшибке()
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		МодульСтандартныеПодсистемыКлиент.НастроитьВидимостьИЗаголовокСсылкиОтправкиОтчетаОбОшибке(
...
Процедура ПриЗавершенииОперацииИзменения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус <> "Выполнено" Тогда
		НазадСервер();
		ВывестиИнформациюОбОшибке(Результат.ИнформацияОбОшибке);
		Возврат;
	КонецЕсли;
	...
	Для Каждого РезультатИзменения Из РезультатыИзменений Цикл
		...
		Если РезультатВыполненияПорции.Статус = СтатусОшибка Тогда
			ВывестиИнформациюОбОшибке(РезультатВыполненияПорции.ИнформацияОбОшибке);
		Иначе
...
Функция ИсключаемыеТипыРеквизитов()
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Для каждого ИмяСправочника Из МодульРаботаСФайламиСлужебный.ИменаСправочниковХраненияФайлов(
			МенеджерОбъектаПоПолномуИмени(ИмяОбъекта).ПустаяСсылка(), Истина) Цикл
...
Процедура ДобавитьВнешниеРеквизитыВНабор(НаборыРеквизитов, СписокВидовИзменяемыхОбъектов)
	...
	МодульПользователиСлужебный = ОбщийМодуль("ПользователиСлужебный");
	Если МодульПользователиСлужебный = Неопределено
	 Или Не ВерсияБСПСоответствуетТребованиям Тогда
		Возврат;
	КонецЕсли;
	
	ВнешниеРеквизиты = Новый Массив; // Массив из см. ПользователиСлужебный.ВнешнийРеквизит
	МодульПользователиСлужебный.ПриЗаполненииВнешнихРеквизитов(СписокВидовИзменяемыхОбъектов, ВнешниеРеквизиты);
...
//
// Пример:
//	Минимальная версия приложения <b>1.1</b>. <a href = "Обновление">Обновите</a> приложение.
//
// Возвращаемое значение:
...
Процедура ОтправитьОтчетОбОшибке()
	...
	Для Каждого Элемент Из СведенияОбОшибках() Цикл
		...
		МодульСтандартныеПодсистемыКлиент.ОтправитьОтчетОбОшибке(СведенияОбОшибке.ОтчетОбОшибке, СведенияОбОшибке.ИнформацияОбОшибке);
	КонецЦикла;
...
&НаКлиенте
Процедура ВывестиИнформациюОбОшибке(ИнформацияОбОшибке)
	
	Если ВерсияБСПСоответствуетТребованиям Тогда
		МодульСтандартныеПодсистемыКлиент = ОбщийМодуль("СтандартныеПодсистемыКлиент");
		МодульСтандартныеПодсистемыКлиент.ВывестиИнформациюОбОшибке(ИнформацияОбОшибке);
	Иначе
