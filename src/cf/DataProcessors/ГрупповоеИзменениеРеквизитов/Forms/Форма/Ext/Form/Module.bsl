...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег(ПолноеИмяФормы("ДополнительныеПараметры")) Тогда
		...
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			ОбновитьВидимостьЭлементов();
			СохранитьНастройкиОбработки(Объект.ИзменятьВТранзакции, Объект.ПрерыватьПриОшибке,
...
Процедура СохранитьНастройкиОбработки(ИзменятьВТранзакции, ПрерыватьПриОшибке,
	ТОНастройкаПорции, ТОПроцентОбъектовВПорции, ТОЧислоОбъектовВПорции, ОбрабатыватьРекурсивно)
	...
	СтруктураНастроек.ТОНастройкаПорции			= ТОНастройкаПорции;
	СтруктураНастроек.ТОПроцентОбъектовВПорции	= ТОПроцентОбъектовВПорции;
	СтруктураНастроек.ТОЧислоОбъектовВПорции	= ТОЧислоОбъектовВПорции;
...
Процедура РазрешитьРедактированиеРеквизитов(ВыбранныйРеквизит)
	...
	Иначе
		...
		Если ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
			МодульЗапретРедактированияРеквизитовОбъектовКлиентМодуль = ОбщийМодуль("ЗапретРедактированияРеквизитовОбъектовКлиент");
			Если МодульЗапретРедактированияРеквизитовОбъектовКлиентМодуль <> Неопределено Тогда
				МодульЗапретРедактированияРеквизитовОбъектовКлиентМодуль.ПроверитьСсылкиНаОбъект(
					Новый ОписаниеОповещения("РазрешитьРедактированиеРеквизитовЗавершение", ЭтотОбъект,
...
Функция ФильтруемыеРеквизиты()
	...
	Для Каждого ИмяТипа Из СтрРазделить(ВидыИзменяемыхОбъектов, ",", Ложь) Цикл
		...
		Для Каждого ОписаниеФО Из Метаданные.ФункциональныеОпции Цикл
			...
			ОписаниеРеквизитов = ОбъектМетаданных.Реквизиты;// массив из ОбъектМетаданныхРеквизит
			Для Каждого РеквизитОМ Из ОписаниеРеквизитов Цикл
...
Процедура ОбновитьСчетчикиИзменяемыхРеквизитов(Знач ТаблицаФормы = Неопределено)
	...
	Для Каждого ТаблицаФормы Из СписокТаблиц Цикл
		...
		Страница = ТаблицаФормы.Родитель;// СтраницаПанели
		...
		Если Не ПустаяСтрока(Пояснение) Тогда
			...
			Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
				Если ЕстьОтборПоСтрокам Тогда 
					Пояснение = Пояснение + " " 
						+ НСтр("ru = 'Изменения будут выполнены в тех строках выбранных элементов, которые удовлетворяют <a href = ""ПерейтиКНастройкеОтбора"">условиям отбора</a>.'")
				Иначе
					Пояснение = Пояснение + " " 
						+ НСтр("ru = 'Изменения будут выполнены <a href = ""ПерейтиКНастройкеОтбора"">во всех строках</a> выбранных элементов.'")
				КонецЕсли;
			КонецЕсли;
		Иначе
