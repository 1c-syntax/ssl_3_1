	...
	Попытка
		...
		Для Каждого ДанныеОбъекта Из ОбрабатываемыеОбъекты Цикл
			...
			Попытка
				...
				Иначе
					// @skip-check query-in-loop - порционная запись по одному объекту на транзакцию.
...
Функция ВнестиИзменения(Знач ДанныеОбъекта, Знач ИзменяемыйОбъект, Знач Параметры)
	...
	Для Каждого Операция Из Параметры.ИзменяемыеРеквизиты Цикл
		...
		ИначеЕсли Операция.ВидОперации = 2 Тогда // изменение дополнительного реквизита
			...
			Если ЗначениеЗаполнено(Значение) Тогда
				...
				ДобавленныеРеквизиты.Добавить(НайденнаяСтрока);
			Иначе
				Если НайденнаяСтрока <> Неопределено Тогда
	...
	Если ДобавленныеРеквизиты.Количество() > 0 Тогда
		СписокСвойств = ИзменяемыйОбъект.ДополнительныеРеквизиты.Выгрузить(ДобавленныеРеквизиты).ВыгрузитьКолонку("Свойство");
		МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
		ОписанияСвойств = МодульОбщегоНазначения.ЗначенияРеквизитовОбъектов(СписокСвойств, "ТипЗначения, МногострочноеПолеВвода");
		...
		Для Каждого ДобавленныйРеквизит Из ДобавленныеРеквизиты Цикл
			ОписаниеСвойства = ОписанияСвойств[ДобавленныйРеквизит.Свойство];
			Если МодульУправлениеСвойствамиСлужебный.ИспользоватьНеограниченнуюСтроку(ОписаниеСвойства.ТипЗначения, ОписаниеСвойства.МногострочноеПолеВвода) Тогда
				ДобавленныйРеквизит.ТекстоваяСтрока = Значение;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ВерсияБСПСоответствуетТребованиям() Экспорт
	...
	Возврат СравнитьВерсии(ВерсияБСП, "3.1.8.25") >= 0;
