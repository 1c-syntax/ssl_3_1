...
Функция ВерсияБСППоддерживаетВнешниеРеквизитыПользователей() Экспорт
	...
	ВерсияБСП = МодульСтандартныеПодсистемыСервер.ВерсияБиблиотеки();
	Возврат СравнитьВерсии(ВерсияБСП, "3.1.10.32") >= 0;
...
Функция РезультатИзмененияПорцииОбъектов(ОбрабатываемыеОбъекты, РезультатИзменения, НастройкиИзменения) Экспорт
	...
	Если МодульПользователиСлужебный = Неопределено
	 Или Не ВерсияБСППоддерживаетВнешниеРеквизитыПользователей() Тогда
		МодульПользователиСлужебный = Неопределено;
	КонецЕсли;
	...
	Попытка
		...
		Для Каждого ДанныеОбъекта Из ОбрабатываемыеОбъекты Цикл
			...
			Попытка
				...
				Если МодульПользователиСлужебный <> Неопределено
				   И Изменения <> Неопределено
				   И ЗначениеЗаполнено(Изменения.ИзменяемыеВнешниеРеквизиты) Тогда
					
					МодульПользователиСлужебный.ПриИзмененииВнешнихРеквизитов(ИзменяемыйОбъект,
