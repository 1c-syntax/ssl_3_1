...
Функция ВнестиИзменения(Знач ДанныеОбъекта, Знач ИзменяемыйОбъект, Знач Параметры)
	...
	Для Каждого Операция Из Параметры.ИзменяемыеРеквизиты Цикл
		...
		ИначеЕсли Операция.ВидОперации = 2 Тогда // изменение дополнительного реквизита
			...
			ЗначенияСвойств.Вставить(Операция.Свойство, Значение);
			МодульУправлениеСвойствами.УстановитьСвойстваУОбъекта(ИзменяемыйОбъект, ЗначенияСвойств);
		...
		ИначеЕсли Операция.ВидОперации = 3 Тогда // изменение дополнительного сведения
			...
			ОписаниеСвойства = ТаблицаСвойств.Добавить();
			ОписаниеСвойства.Свойство = Операция.Свойство;
			ОписаниеСвойства.Значение = Значение;
			
			МодульУправлениеСвойствами.ЗаписатьСвойстваУОбъекта(ИзменяемыйОбъект.Ссылка, ТаблицаСвойств);
...
Процедура ВыполнитьКодАлгоритма(Знач Объект, Знач КодАлгоритма, Знач ВыполнитьВБезопасномРежиме)
	...
	// Произвольный алгоритм доступен только администратору в неразделенной информационной базе.
	Если РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Выполнение произвольного алгоритма недоступно в разделенной информационной базе.'");
	КонецЕсли;

	Если НЕ ПравоДоступа("Администрирование", Метаданные) Тогда
		ВызватьИсключение НСтр("ru = 'Выполнение произвольного алгоритма недоступно.
			|Требуется право Администрирование.'");
	КонецЕсли;

	Если ВыполнитьВБезопасномРежиме Тогда
...
Функция ВыполнитьИзменениеОбъектовВНесколькоПотоков(Параметры, ОбрабатываемыеОбъекты, РезультатИзменения,
	...
	РезультатВыполнения = ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков(
		"Обработка.ГрупповоеИзменениеРеквизитов.МодульОбъекта.РезультатИзмененияПорцииОбъектов",
		ПараметрыВыполнения,
		ПараметрыМетода);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РезультатИзмененияПорцииОбъектов", Истина);
