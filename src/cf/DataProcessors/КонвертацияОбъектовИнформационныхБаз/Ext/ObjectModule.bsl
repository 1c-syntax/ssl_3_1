...
Процедура ВнешнееСоединениеПередЗагрузкойДанных() Экспорт
	...
	Если ПараметрыАрхива <> Неопределено И ПараметрыАрхива.КоличествоФайлов > 0 Тогда
		...
		НомерПакета = 0;
		КаталогВременногоХранилищаФайлов = ОбменДаннымиСервер.КаталогВременногоХранилищаФайлов();
		ВременныйКаталог = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, "{" + Строка(Новый УникальныйИдентификатор) + "}"); //АПК: 441 временный каталог будет удалена в процедуре СобратьИПоместитьСообщениеОбменаВАрхив 
...
// 
// Возвращаемое значение:
//  Строка - см. СтрокаСообщенияОбОшибке
//
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	ИмяПланаОбменаДляПерехода = Неопределено;
	
	Если ТипЗнч(НастройкиСтарогоПланаОбмена) = Тип("Строка") Тогда
		ИмяПланаОбменаДляПерехода = НастройкиСтарогоПланаОбмена;
	ИначеЕсли ТипЗнч(НастройкиСтарогоПланаОбмена) = Тип("Структура") Тогда
		ИмяПланаОбменаДляПерехода = НастройкиСтарогоПланаОбмена.ИмяПланаОбменаДляПереходаНаНовыйОбмен;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ИмяПланаОбменаДляПерехода) Тогда
		СтрокаСообщения = НСтр("ru = 'Не удалось определить план обмена для перехода на EnterpriseData.'");
		ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Ошибка);
		ВызватьИсключение НСтр("ru = 'Автоматический переход на EnterpriseData не выполнен.'");
	КонецЕсли;
...
Процедура ЗаписатьРегистрыБухгалтерии(Регистратор, ТаблицыОтложеннойЗаписиДвижений)
	...
	Если НЕ ЗначениеЗаполнено(Регистратор.Пустая()) Тогда
...
// Параметры:
//  Источник - Строка - Имя временного файла с правилами
// 
// Возвращаемое значение:
//  - Неопределено
//  - Структура:
//    * ВерсияФорматаХраненияПравил - Число 
//    * Конвертация - см. Конвертация
//    * ТаблицаНастройкиПараметров - см. ИнициализацияТаблицыНастройкиПараметров
//    * ТаблицаПравилВыгрузки - см. КоллекцияПравилаВыгрузкиДанных
//    * ТаблицаПравилКонвертации - см. КоллекцияПравилаКонвертации
//    * Алгоритмы - Структура 
//    * Параметры - Структура  
//    * Запросы - Структура 
//    * XMLПравила - Строка - xml-строка правил обмена
//    * СтрокаТиповДляПриемника - Строка - xml-строка с типами
//
...
Процедура ИнициализацияРеквизитовИМодульныхПеременных()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
		РазделителиКонфигурации = МодульРаботаВМоделиСервиса.РазделителиКонфигурации();
	Иначе
...
Процедура ЗаписатьИнформациюОбОбменеДаннымиЧерезПланыОбмена(Знач НомерОтправленного)
	...
	НомерПринятого = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелДляОбмена, "НомерПринятого");
	
	// Атрибуты механизма квитирования сообщений обмена.
	УстановитьАтрибут(Приемник, "НомерИсходящегоСообщения", НомерОтправленного);
	УстановитьАтрибут(Приемник, "НомерВходящегоСообщения",  НомерПринятого);
...
Функция НеобходимоВыполнитьКорректировкуИнформацииСопоставления()
	...
	Возврат РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.НеобходимоВыполнитьКорректировкуИнформацииСопоставления(
		УзелДляОбмена, НомерОтправленного + 1);
