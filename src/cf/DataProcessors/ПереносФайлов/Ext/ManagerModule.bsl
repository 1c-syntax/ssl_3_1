...
Функция ВыполнитьПереносФайлов(ФайлыДляПереноса, Параметры) Экспорт
	...
	Если Параметры.Действие = "ПеренестиМеждуТомами" Тогда
		ДействиеДляЖурнала = НСтр("ru = 'между томами на сервере.'");
	ИначеЕсли Параметры.Действие = "ПеренестиВТома" Тогда
		ДействиеДляЖурнала = НСтр("ru = 'в тома на сервере.'")
	Иначе
...
Процедура ПеренестиФайл(Файл, СвойстваТома, Параметры, ОшибкиПереноса, ПеренесеноФайлов)
	...
	Исключение
		...
		ИмяДляЖурнала = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(СвойстваФайла.Наименование,
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка переноса файла.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|по причине:
				|%3"".'"),
				ОбщегоНазначения.ПредметСтрокой(ФайлСсылка), 
...
Процедура ПеренестиВБазу(ФайлОбъект, СвойстваТома, Параметры)
	...
	ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(ФайлОбъект);

	РаботаСФайламиСлужебный.ЗаписатьФайлВИнформационнуюБазу(ФайлОбъект.Ссылка, ДанныеФайла, ПараметрыЗаписиВИБ);
...
//
Функция ОшибкаПереноса(ФайлСсылка, ИнформацияОбОшибке) Экспорт
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьПереносФайлов", Истина);
