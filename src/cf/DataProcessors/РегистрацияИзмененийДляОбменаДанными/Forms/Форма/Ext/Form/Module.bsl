...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Иначе
		...
		ВерсияДанных = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбменаСсылка, "ВерсияДанных");
		...
		Если УзелОбменаСсылка <> Неопределено 
		    ...
		    И ПустаяСтрока(ВерсияДанных) Тогда
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыФоновогоЗадания)
	...
	Если ПолноеИмяОбработки = "ВнешняяОбработка.РегистрацияИзмененийДляОбменаДанными" Тогда
		Результат = МодульДлительныеОперации.ВыполнитьВФоне("ВнешняяОбработка.РегистрацияИзмененийДляОбменаДанными.МодульОбъекта.ВыполнитьИзменениеРегистрации", ПараметрыФоновогоЗадания, ПараметрыВыполнения);
	Иначе
		Результат = МодульДлительныеОперации.ВыполнитьВФоне("Обработка.РегистрацияИзмененийДляОбменаДанными.МодульОбъекта.ВыполнитьИзменениеРегистрации", ПараметрыФоновогоЗадания, ПараметрыВыполнения);
	КонецЕсли;
...
Функция ПолучитьИменаМетаданныхТекущейСтроки(БезУчетаАвторегистрации) 
	...
	Если ПустаяСтрока(МетаИмя) Тогда
		// Корень конфигурации без пометки
	...
	Иначе
		...
		Если ПустаяСтрока(МетаРодительИмя) Тогда
			...
			Если СтруктураИменМетаданных.Свойство(МетаИмя) Тогда
				Результат.МетаИмена = СтруктураИменМетаданных[МетаИмя];
			Иначе
...
Процедура ЗаполнитьДополнительнуюИнформацию()
	...
	МодульОбменДаннымиРегистрацияПовтИсп = ОбщийМодуль("ОбменДаннымиРегистрацияПовтИсп");
	МодульОбменДаннымиРегистрацияСервер = ОбщийМодуль("ОбменДаннымиРегистрацияСервер");
...
// АПК:488-выкл "Вычислить" вместо "ОбщегоНазначения.ВычислитьВБезопасномРежиме()", так как обработка автономная
