...
Процедура НастроитьСписокСсылок(ИмяТаблицы, Наименование)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ #ИмяТаблицы# КАК ОсновнаяТаблица
	|		ПО (ОсновнаяТаблица.Ссылка = ТаблицаИзменений.Ссылка)
	|ГДЕ
	|	ТаблицаИзменений.Узел = &ВыбранныйУзел";
	
	СвойстваСписка.ТекстЗапроса = СтрЗаменить(СвойстваСписка.ТекстЗапроса, "#ИмяТаблицы#", ИмяТаблицы);
	СвойстваСписка.ТекстЗапроса = СтрЗаменить(СвойстваСписка.ТекстЗапроса, "#ИмяТаблицыИзменения#", ИмяТаблицы + ".Изменения");
...
Процедура ДополнитьИменаСкрываемыхМетаданных()
	...
	Для Каждого РегистрСведенийМетаданные Из Метаданные.РегистрыСведений Цикл
		Для Каждого ИзмерениеРегистра Из Метаданные.РегистрыСведений[РегистрСведенийМетаданные.Имя].Измерения Цикл
			Если НРег(ИзмерениеРегистра.Имя) = "узел" Тогда
				ИменаСкрываемыхМетаданных.Добавить("РегистрСведений." + РегистрСведенийМетаданные.Имя);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
