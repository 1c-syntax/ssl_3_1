...
//         * Команда - значение входящего параметра Команда для упрощения обработки результатов.
//
Функция ИзменитьРегистрациюНаСервере(Команда, БезУчетаАвторегистрации, Узел, Данные, ИмяТаблицы = Неопределено, СтруктураИменМетаданных = Неопределено) Экспорт
	...
	Для Каждого Элемент Из ДанныеРегистрации Цикл
		...
		Если Элемент = Неопределено Тогда
			...
			Если ТипЗнч(Команда) = Тип("Булево") И Команда Тогда
				...
				ДобавитьРезультаты(Результат, ИзменитьРегистрациюНаСервере(Команда, БезУчетаАвторегистрации, Узел, "Константы", ИмяТаблицы, СтруктураИменМетаданных));
		...
		ИначеЕсли Тип = Тип("Строка") Тогда
			...
			Если НадоФильтрБСП Тогда
				ДобавитьРезультаты(Результат, БСП_РегистрацияИзмененийМетаданных(Узел, Описание, БезУчетаАвторегистрации, СтруктураИменМетаданных));
				Продолжить;
...
// Возвращает структуру с полями "Всего" и "Успешно", описывающей количества регистраций.
//
Функция БСП_РегистрацияИзмененийМетаданных(Узел, Описание, БезУчетаАвторегистрации, СтруктураИменМетаданных)
	
	Результат = Новый Структура("Всего, Успешно", 0, 0);
	
	Если ТипЗнч(СтруктураИменМетаданных) = Тип("Структура")
		И Описание.ЭтоКонстанта
		И НЕ СтруктураИменМетаданных.Свойство("Константы") Тогда
		
		// Если нужно зарегистрировать все изменения, при этом в составе плана обмена нет констант.
		Возврат Результат;
		
	КонецЕсли;
	
	Если СтруктураИменМетаданных <> Неопределено
		И Описание.ЭтоКонстанта 
		И Не Описание.ЭтоКоллекция 
		И СтруктураИменМетаданных.Константы.Найти(Описание.Имятаблицы) = Неопределено Тогда
	...
	Если Описание.ЭтоКоллекция Тогда
		Для Каждого МетаВид Из Описание.Метаданные Цикл
			...
			ДобавитьРезультаты(Результат, БСП_РегистрацияИзмененийМетаданных(Узел, ТекОписание, БезУчетаАвторегистрации, СтруктураИменМетаданных));
		КонецЦикла;
	Иначе;
