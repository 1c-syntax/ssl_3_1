...
//
Функция ИзмеренияНабораЗаписей(ИмяТаблицы) Экспорт
...
Функция ИзменитьРегистрациюНаСервере(Команда, БезУчетаАвторегистрации, Узел, Данные, ИмяТаблицы = Неопределено, ИменаМетаданных = Неопределено) Экспорт
	...
	Если ТипЗнч(Данные) = Тип("Массив") Тогда
		...
		Если ЭтоРегистрацияПоОтбору Тогда
			...
			МодульОбменДаннымиСобытия = ОбщийМодуль("ОбменДаннымиСобытия");
	...
	Для Каждого Элемент Из ДанныеРегистрации Цикл
		...
		Если Элемент = Неопределено Тогда
			...
			Для Каждого МД Из ИменаМетаданных Цикл
				ИзменитьРегистрациюНаСервере(Команда, БезУчетаАвторегистрации, Узел, МД.Значение, ИмяТаблицы, ИменаМетаданных);
			КонецЦикла;
...
Функция ОбщийМодуль(Имя) 
	...
	Если Метаданные.ОбщиеМодули.Найти(Имя) <> Неопределено Тогда 
		...
		Модуль = Вычислить(Имя); // АПК:488 "Вычислить" вместо "ОбщегоНазначения.ВычислитьВБезопасномРежиме()", так как обработка автономная
	Иначе
		Модуль = Неопределено;
	КонецЕсли;
...
Функция ЭтоУзелПланаОбменаБСП(Узел)
	...
	Если МодульОбменДаннымиПовтИсп = Неопределено Тогда
