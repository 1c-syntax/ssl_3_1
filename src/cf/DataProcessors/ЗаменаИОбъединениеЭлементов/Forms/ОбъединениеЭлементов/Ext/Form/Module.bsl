...
// Параметры формы:
//     НаборСсылок - Массив из ЛюбаяСсылка - элементы для замены на другой элемент ссылочного типа.
//
...
#Область СлужебныеПроцедурыОбъединенияЭлементов
...
Функция ПроверитьВозможностьЗаменыСсылок()
	...
	Для Каждого МестоИспользования Из МестаИспользования Цикл
		НаборСсылок.Добавить(МестоИспользования.Ссылка);
		ПарыЗамен.Вставить(МестоИспользования.Ссылка, ОсновнойЭлемент);
	КонецЦикла;
...
Функция ЗаменитьСсылки()
	...
	Для Каждого МестоИспользования Из МестаИспользования Цикл
		ПараметрыМетода.ПарыЗамен.Вставить(МестоИспользования.Ссылка, ОсновнойЭлемент);
	КонецЦикла;
...
Процедура ПослеЗавершенияЗаменыСсылок(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ПоказатьОповещениеПользователя(СообщениеОЗавершении(), ПолучитьНавигационнуюСсылку(ОсновнойЭлемент),
			Строка(ОсновнойЭлемент), БиблиотекаКартинок.ДиалогИнформация);
		ОповеститьОбУспешнойЗамене(МестаИспользованияВМассив());
		Закрыть();
	КонецЕсли
...
Процедура ЗаполнитьМестаИспользования(Знач АдресРезультата)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаИспользования Цикл
		
		СтрокаИспользования = НовыеМестаИспользования.Найти(СтрокаТаблицы.Ссылка, "Ссылка");
		Если СтрокаИспользования = Неопределено Тогда
			СтрокаИспользования = НовыеМестаИспользования.Добавить();
			СтрокаИспользования.Ссылка = СтрокаТаблицы.Ссылка;
		КонецЕсли;
		
		Мест = СтрокаТаблицы.Вхождения;
		Если Мест > МаксМест И Не ПометкиУдаления[СтрокаТаблицы.Ссылка] Тогда
			МаксСсылка = СтрокаТаблицы.Ссылка;
			МаксМест   = Мест;
		КонецЕсли;
		
		СтрокаИспользования.МестИспользования = Мест;
		СтрокаИспользования.Код      = КодОбъекта(СтрокаТаблицы.Ссылка, КэшМетаданных);
		СтрокаИспользования.Владелец = ВладелецОбъекта(СтрокаТаблицы.Ссылка, КэшМетаданных);
