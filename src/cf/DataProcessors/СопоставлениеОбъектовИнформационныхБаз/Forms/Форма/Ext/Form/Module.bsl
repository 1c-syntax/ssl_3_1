...
#Область СлужебныеПроцедурыИФункцииПоставляемаяЧасть
...
Процедура ЗаполнитьПоляПоиска()
	...
	Если Объект.СписокИспользуемыхПолей.Количество() = 0 Тогда
		
		// для справочников
		ДобавитьПолеПоиска("Наименование");
		ДобавитьПолеПоиска("Код");
		ДобавитьПолеПоиска("Владелец");
		ДобавитьПолеПоиска("Родитель");
		
		// для документов и БП
		ДобавитьПолеПоиска("Дата");
		ДобавитьПолеПоиска("Номер");
		
		// популярные поля поиска
		ДобавитьПолеПоиска("Организация");
		ДобавитьПолеПоиска("ИНН");
		ДобавитьПолеПоиска("КПП");
		
		Если Объект.СписокИспользуемыхПолей.Количество() = 0 
			И Объект.СписокПолейТаблицы.Количество() > 0 Тогда
			
			Значение = Объект.СписокПолейТаблицы[0].Значение;
			ДобавитьПолеПоиска(Значение);
...
&НаКлиенте
Процедура ДобавитьПолеПоиска(Значение)
	
	Элемент = Объект.СписокПолейТаблицы.НайтиПоЗначению(Значение);
	
	Если Элемент <> Неопределено Тогда
		
		Элемент.Пометка = Истина;
		Объект.СписокИспользуемыхПолей.Добавить(Значение, Значение, Истина);
