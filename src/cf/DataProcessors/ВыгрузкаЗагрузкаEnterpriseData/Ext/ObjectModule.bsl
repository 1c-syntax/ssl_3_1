...
//   РасширениеФорматаПриЗагрузке - Строка - пространство имен расширения формата, которое используется при загрузке.
//
...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "", РасширениеФорматаПриЗагрузке = "") 
	...
	Если ЗначениеЗаполнено(УзелОбмена) И НаправлениеОбмена = "Отправка" Тогда
		...
		ОбменДаннымиСлужебный.ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелОбмена, НаправлениеОбмена);
...
Процедура ДополнитьСписокОбъектовКВыгрузке(МассивМетаданныхОтбор)
	...
	Если КоллекцияРезультат.Колонки.Найти("ОбъектСсылка") <> Неопределено Тогда
