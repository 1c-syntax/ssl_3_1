...
Процедура ПроверитьПодключениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПодключениеУстановлено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Подключение успешно установлено.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	Иначе
		
		ШаблонТекстаПредупреждения = НСтр("ru = 'Не удалось установить подключение.
			|
			|%1'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
		ТекстПредупреждения = СтрШаблон(ШаблонТекстаПредупреждения,
			НСтр("ru = 'Подробное описание см. в журналах регистрации синхронизируемых приложений.'"));
...
Процедура ПроверитьПодключениеНаСервере(ПодключениеУстановлено)
	...
	Если Не ПодключениеУстановлено Тогда
		...
		ШаблонТекста = НСтр("ru = 'Соединение не установлено.
			|%1'", ОбщегоНазначения.КодОсновногоЯзыка());
		ПодробноеПредставлениеОшибки = СтрШаблон(ШаблонТекста, СокрЛП(ОбработкаОбъект.СообщениеОбОшибке));
