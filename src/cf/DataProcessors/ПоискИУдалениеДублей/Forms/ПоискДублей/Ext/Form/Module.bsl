...
Процедура ШагМастераДалее()
	...
	Если ТекущаяСтраница = Элементы.ШагПоискНеВыполнялся Тогда
		...
		Если ОписаниеПрикладныхПравил <> Неопределено И УчитыватьПрикладныеПравила Тогда
			ПравилаПоискаУказаны = Истина;
		Иначе
			ПравилаПоискаУказаны = Ложь;
			Для Каждого СтрокаПравил Из ПравилаПоиска Цикл
				Если ЗначениеЗаполнено(СтрокаПравил.Правило) Тогда
					ПравилаПоискаУказаны = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		Если Не ПравилаПоискаУказаны Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите хотя бы одно правило поиска дублей.'"));
			Возврат;
		КонецЕсли;
...
Функция ПараметрыУдаленияДублей()
	...
	ОбщееКоличествоМестИспользования = 0;
	Для Каждого ГруппыДублей Из ДеревоДублей.Строки Цикл
		...
		Для Каждого Дубль Из ГруппыДублей.Строки Цикл
			Если Дубль.Пометка = 1 И Дубль.Ссылка <> Оригинал Тогда
				ОбщееКоличествоМестИспользования = ОбщееКоличествоМестИспользования + Дубль.Количество;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	// Эмпирическое значение, ниже него экономия от отключения итогов не окупает накладных расходов.
	ПорогОтключенияИтогов = 500;
	ОтключатьИтоги = ОбщееКоличествоМестИспользования >= ПорогОтключенияИтогов;
	...
	Для Каждого ГруппыДублей Из ДеревоДублей.Строки Цикл
		...
		ПараметрыПроцедуры.Вставить("ОтключатьИтоги", ОтключатьИтоги);
...
Функция НайтиИУдалитьДубли()
	...
	Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Тогда
		...
		Возврат ДлительныеОперации.ВыполнитьВФоне(
			"Обработка.ПоискИУдалениеДублей.МодульОбъекта.ФоновыйПоискДублей",
			ПараметрыПроцедуры,
			НастройкиЗапуска);
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		Возврат ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков(
			"Обработка.ПоискИУдалениеДублей.МодульОбъекта.ФоновоеУдалениеДублей",
			НастройкиЗапуска,
			ПараметрыУдаления);
