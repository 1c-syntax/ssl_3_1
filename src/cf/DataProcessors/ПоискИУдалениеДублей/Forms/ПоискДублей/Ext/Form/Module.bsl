...
Процедура ИнициализироватьКомпоновщикОтбораИПравила(НастройкиФормы)
	...
	Если ТипЗнч(НастройкиКД) = Тип("НастройкиКомпоновкиДанных") Тогда
		...
		Для Сч = -(НастройкиКД.Отбор.Элементы.Количество()-1) По 0 Цикл
			...
			Если СтрНайти(Отбор.Представление, НСтр("ru='среди которых возможны дубли'")) > 0 Тогда
	...
	Если ЭтоАдресВременногоХранилища(АдресВыбранныхМестИспользования) Тогда
		...
		Для каждого РеквизитЗначениеОтбора Из ВыбранныеМестаИспользования.ЗначениеОтбора Цикл
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
				ГруппаОтбора,
				РеквизитЗначениеОтбора.Ключ,
				РеквизитЗначениеОтбора.Значение,
...
Процедура ЗаполнитьВозможныеДубли()
	...
	Для каждого МестоИспользования Из ПредполагаемыеВозможныеДубли Цикл
		...
		КлючиСвязи = КлючиСвязиМетаданных(ИдентификаторОбрабатываемогоОбъекта, МестоИспользования);
...
Функция СформироватьОтборВозможныхДублей(ДеревоДублей)
	...
	Для каждого ГруппаДублей Из ДеревоДублей.Строки Цикл
		...
		Для каждого СтрокаДубля Из ГруппаДублей.Строки Цикл
			...
			Если СтрокаДубля.Основной Тогда
...
// Функция - Степень родства метаданных
//
// 
// Возвращаемое значение:
...
//
&НаСервереБезКонтекста
Функция КлючиСвязиМетаданных(Знач ИдентификаторОбрабатываемогоОбъекта, Знач МестоИспользования)
	...
	Для каждого РеквизитМестаИспользования Из МестоИспользования.Реквизиты Цикл
		...
		Если РеквизитМестаИспользования.Тип.Типы().Найти(ТипЗнч(ИдентификаторОбрабатываемогоОбъекта.ЗначениеПустойСсылки)) <> Неопределено Тогда
...
&НаСервере
Процедура ОтобратьВыбранныеМестаИспользования(ВыбранныйТип)
	...
	ИдентификаторМетаданных = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ВыбранныйТип);
	ВыбранныеМетаданные = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(ИдентификаторМетаданных);
	ВыбранныеМестаИспользования.ПредставлениеОтбора = НСтр("ru='Элементы, среди которых возможны дубли'");
