		...
		Если СтруктураПолейПодобия.Количество() > 0 Тогда
			...
			Если FuzzySearch = Неопределено Тогда
				Результат.ОписаниеОшибки = 
					НСтр("ru = 'Не удалось подключить внешнюю компоненту FuzzyStringMatchExtension из макета ""ОбщийМакет.КомпонентаПоискаСтрок""
				...
				Для Каждого ИндексСтроки Из СтрРазделить(ИндексыСтрок, ",") Цикл
					...
					Если ИспользоватьПрикладныеПравила Тогда
						...
						Если ТаблицаКандидатов.Количество() = РазмерПрикладнойПорции Тогда
							ЗарегистрироватьДублиПоПрикладнымПравилам(ТаблицаДублей, МенеджерОбластиПоиска, ЭталонныйЭлемент, ТаблицаКандидатов, 
								СтруктураПолей, ДополнительныеПараметры);
...
//
Процедура ЗарегистрироватьДублиПоПрикладнымПравилам(СтрокиДереваРезультата, Знач МенеджерОбластиПоиска, Знач ОсновныеДанные, Знач ТаблицаКандидатов, Знач СтруктураПолей, Знач ДополнительныеПараметры)
