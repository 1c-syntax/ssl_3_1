...
Функция ГруппыДублей(Знач ПараметрыПоиска, Знач ЭталонныйОбъект = Неопределено) Экспорт
	...
	ПараметрыПоискаПохожихСтрок = Неопределено;
	КоличествоДублей = 0;
	...
	Пока СледующийЭлементВыборки(СхемаЗапроса.ВыборкаЭталонныхОбъектов) Цикл
		...
		Если СтруктураЗапроса.ИменаПолейДляСравненияНаПодобие.Количество() > 0 Тогда
			...
			Если ПараметрыПоискаПохожихСтрок = Неопределено Тогда
				Попытка
			...
			Для Каждого ИмяПоля Из СтруктураЗапроса.ИменаПолейДляСравненияНаПодобие Цикл
				...
				Для Каждого ИндексСтроки Из СтрРазделить(ИндексыСтрок, ",") Цикл
					Если ПустаяСтрока(ИндексСтроки) Тогда
						Продолжить;
					КонецЕсли;
					ЭлементДубль = КандидатыДублей[Число(ИндексСтроки)];
					
					Если ЕстьПравилаПоиска Тогда
						ДобавитьСтрокуКандидатов(КоллекцияДублей, ТаблицаКандидатов, ЭталонныйЭлемент, ЭлементДубль, 
							СтруктураЗапроса);
						Если ТаблицаКандидатов.Количество() = КоличествоЭлементовДляСравнения Тогда
							ЗарегистрироватьДублиПоПрикладнымПравилам(КоллекцияДублей, ПолноеИмяОбъектаМетаданных, 
								МенеджерОбластиПоиска, ЭталонныйЭлемент, ТаблицаКандидатов, СтруктураЗапроса, 
		...
		Если КоличествоДублей <> ТаблицаДублей.Количество() Тогда
			КоличествоДублей = ТаблицаДублей.Количество();
			ДлительныеОперации.СообщитьПрогресс(КоличествоДублей, "ЗарегистрироватьДубль");
		КонецЕсли;
	...
	Если РассчитыватьМестаИспользования Тогда
		...
		МестаИспользования = ОбщегоНазначения.МестаИспользования(СсылкиОбъектов);
...
Функция ЕстьПрикладныеПравилаОбластиПоискаДублей(Знач ИмяОбъекта) Экспорт
	...
	Возврат СведенияОбОбъекте <> Неопределено 
		И (СведенияОбОбъекте = "" Или СтрНайти(СведенияОбОбъекте, "ПараметрыПоискаДублей") > 0);
	
КонецФункции

// Обработчик фонового поиска дублей, вызывается из процедуры НайтиИУдалитьДубли формы ПоискДублей.
//
// Параметры:
//   ПараметрыПоиска - см. ПоискИУдалениеДублей.ПараметрыПоискаДублей
//   АдресРезультата - Строка - адрес во временном хранилище для возврата результата.
//
Процедура ФоновыйПоискДублей(Знач Параметры, Знач АдресРезультата) Экспорт
	
	НачалоПоискаДублей = ТекущаяДатаСеанса();
	ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начат поиск дублей для: %1.'"), Параметры.ОбластьПоискаДублей));
	...
	ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершен поиск дублей для ""%1"" за %2 мин.'"), 
			Параметры.ОбластьПоискаДублей,
			Формат((ТекущаяДатаСеанса() - НачалоПоискаДублей) / 60, "ЧДЦ=2")));

КонецПроцедуры

// Обработчик фонового удаления дублей, вызывается из процедуры НайтиИУдалитьДубли формы ПоискДублей.
//
// Параметры:
...
Функция ФоновоеУдалениеДублей(Знач Параметры) Экспорт
	...
	НачалоУдаленияДублей = ТекущаяДатаСеанса();
	КоличествоДублей = Параметры.ПарыЗамен.Количество(); 
	ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начато удаление дублей: %1.'"), КоличествоДублей));
	...
	ОтключатьИтоги = Параметры.ОтключатьИтоги;
	Если ОтключатьИтоги Тогда
	...
	ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершено удаление дублей %1 за %2 мин.'"),
			КоличествоДублей,
			Формат((ТекущаяДатаСеанса() - НачалоУдаленияДублей) / 60, "ЧДЦ=2")));
	
	Возврат Результат;
	
КонецФункции

// Включает или отключает использование итогов регистров накопления (вид "Остатки")
// и регистров бухгалтерии. При включении однократно пересчитывает текущие итоги.
//
// Параметры:
//  ФлагИспользования - Булево - Ложь отключает итоги, Истина включает и пересчитывает.
//
Процедура УстановитьИспользованиеИтоговРегистров(Знач ФлагИспользования)
...
//
Процедура ДобавитьСтрокуКандидатов(КоллекцияДублей, ТаблицаКандидатов,  Знач ДанныеОсновногоЭлемента, 
	Знач ДанныеКандидата, Знач СтруктураЗапроса)
	
	Если КоллекцияДублей.НезначимыеДубли[ДанныеОсновногоЭлемента.Ссылка] <> Неопределено
		Или КоллекцияДублей.НезначимыеДубли[ДанныеКандидата.Ссылка] <> Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗарегистрироватьДубль(КоллекцияДублей, Знач Элемент1, Знач Элемент2, Знач СтруктураЗапроса)
	...
	// Определить, какой элемент уже является дублем.
	СтрокаДублей1 = ТаблицаДублей.Найти(Элемент1.Ссылка, "Ссылка");
	СтрокаДублей2 = ТаблицаДублей.Найти(Элемент2.Ссылка, "Ссылка");
	
	ЕстьНезначимыйДубль1 = КоллекцияДублей.НезначимыеДубли[Элемент1.Ссылка] <> Неопределено;
	ЕстьНезначимыйДубль2 = КоллекцияДублей.НезначимыеДубли[Элемент2.Ссылка] <> Неопределено;
	
	Дубль1Зарегистрирован = (СтрокаДублей1 <> Неопределено) Или ЕстьНезначимыйДубль1;
	Дубль2Зарегистрирован = (СтрокаДублей2 <> Неопределено) Или ЕстьНезначимыйДубль2;
	
	// Если оба элемента есть дубли, то ничего делать не надо.
	...
	Если Дубль1Зарегистрирован И Дубль2Зарегистрирован
		Или ЕстьНезначимыйДубль1 Или ЕстьНезначимыйДубль2 Тогда
		Возврат;
	КонецЕсли;
	
	// Перед регистрацией дубля определить группу дублей.
...
// Удаляет дубли из коллекции, если дубль помечен на удаление и не имеет мест использования.
Процедура УдалитьНезначимыеДубли(КоллекцияДублей)
	...
	Если ТаблицаДублей.Количество() - ДублиКПроверке.Количество() <= КоллекцияДублей.КоличествоЭлементовДляСравнения Тогда
		...
		МестаИспользования = ОбщегоНазначения.МестаИспользования(ДублиКПроверке);
		МестаИспользования.Индексы.Добавить("Ссылка, ВспомогательныеДанные");
		ОтборДублей = Новый Структура("Ссылка", Неопределено);
		ОтборМестИспользования = Новый Структура("Ссылка, ВспомогательныеДанные", Неопределено, Ложь);
		 
		Для Индекс = 0 По ДублиКПроверке.ВГраница() Цикл
			
			ОтборДублей.Ссылка = ДублиКПроверке[Индекс];
			ПроверяемыеДубли = ТаблицаДублей.НайтиСтроки(ОтборДублей);
			Для каждого Дубль Из ПроверяемыеДубли Цикл
				...
				ОтборМестИспользования.Ссылка = Дубль.Ссылка;
				МестаИспользованияВнутренний = МестаИспользования.НайтиСтроки(ОтборМестИспользования);
				Если МестаИспользованияВнутренний.Количество() = 0 И Дубль.ПометкаУдаления Тогда
					КоллекцияДублей.НезначимыеДубли[Дубль.Ссылка] = Истина;
					КоллекцияДублей.НезначимыеДубли[Дубль.Родитель] = Истина;
...
Процедура УдалитьНезначимыеГруппы(КоллекцияДублей)
	...
	ТаблицаДублей = КоллекцияДублей.ТаблицаДублей;
	ОтборДублей = Новый Структура("Родитель", Неопределено);
	Для каждого ГруппаДублей Из КоллекцияДублей.ОбработанныеГруппы Цикл
		
		ОтборДублей.Родитель = ГруппаДублей.Ключ;
		Дубли = ТаблицаДублей.НайтиСтроки(ОтборДублей);
...
// Возвращаемое значение:
//   Структура:
//     * НезначимыеДубли - Соответствие из КлючИЗначение:
//       ** Ключ - ЛюбаяСсылка
//       ** Значение - Булево
...
Функция КоллекцияДублей(Знач ПараметрыПоиска, ИменаПолейДляСравненияНаПодобие, ИменаПолейДляСравненияНаРавенство)
	...
	КоллекцияДублей.Вставить("НезначимыеДубли", Новый Соответствие);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ФоновыйПоискДублей", Истина);
	Методы.Вставить("ФоновоеУдалениеДублей", Истина);
