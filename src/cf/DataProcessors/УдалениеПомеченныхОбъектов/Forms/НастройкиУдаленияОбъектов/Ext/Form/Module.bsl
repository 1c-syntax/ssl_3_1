...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Элементы.ИспользоватьРегламентноеУдаление.Заголовок = НСтр("ru = 'Автоматически удалять помеченные объекты'");
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("НачатьПроверкуБлокировкиУдаляемыхОбъектов",0.1, Истина);
КонецПроцедуры
...
Процедура ДобавитьНастройку(Команда)
	...
	ПараметрыФормы.КоллекцииВыбираемыхОбъектовМетаданных = КоллекцияВыбираемыхОбъектов;
	ПараметрыФормы.ТолькоПодсистемыСКИ = Истина;
	ПараметрыФормы.ВыбранныеОбъектыМетаданных = ВыбранныеОбъекты;

	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ДобавитьНастройкуЗавершение", ЭтотОбъект);
	СтандартныеПодсистемыКлиент.ВыбратьОбъектыМетаданных(ПараметрыФормы, ОповещениеОЗакрытии);
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьНастройкуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ДобавитьНастройкуСервер(Результат);
КонецПроцедуры

&НаСервере
Процедура ДобавитьНастройкуСервер(Результат)
	ДеревоМестПоиска = РеквизитФормыВЗначение("НастройкиОтображенияОбъектов");
...
&НаКлиенте
Процедура НачатьПроверкуБлокировкиУдаляемыхОбъектов()

	Оповещение = Новый ОписаниеОповещения("ЗавершитьПроверкуБлокировкиУдаляемыхОбъектов", ЭтотОбъект);
	ДлительнаяОперация = НачатьПроверкуБлокировкиУдаляемыхОбъектовСервер();
...
&НаСервере
Функция НачатьПроверкуБлокировкиУдаляемыхОбъектовСервер()
...
//
&НаКлиенте
Процедура ЗавершитьПроверкуБлокировкиУдаляемыхОбъектов(Результат, ДополнительныеПараметры) Экспорт
