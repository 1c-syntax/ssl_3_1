...
&НаКлиенте
Перем СведенияОРезультатахУдаления; // см. НовыйСведенияОРезультатахУдаления
...
&НаКлиенте
Перем ОтчетДляОтправки; // ОтчетОбОшибке 

&НаКлиенте
Перем СвязиНеУдаленныхДействие;
...
Процедура ПриОткрытии(Отказ)
	СведенияОРезультатахУдаления = НовыйСведенияОРезультатахУдаления(); 
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	Если Параметры.ОтборМетаданных.Количество() > 0 Тогда
		Настройки["ОтборМетаданных"] = Неопределено;
	КонецЕсли;
КонецПроцедуры
...
Процедура СсылкаПодробнееНажатие(Элемент)
	...
	Если ЭтоОшибкаУстановкиМонопольногоРежима(ИнформацияОбОшибкеФоновогоЗадания) Тогда
		ОткрытьФормуЗавершенияРаботыПользователей();
		Возврат;
	КонецЕсли;

	Если Элемент.Имя = Элементы.ПодробнееОшибкаФоновогоЗадания.Имя Тогда
		Если ОтчетДляОтправки = Неопределено Тогда
			ОтчетДляОтправки = Новый ОтчетОбОшибке(ИнформацияОбОшибкеФоновогоЗадания);
		КонецЕсли;
...
Процедура МестаИспользованияНеудаленныхОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ТекущаяСтрокаДерева = ТекущаяСтрокаМестИспользованияНеудаленных();
	ТекущиеДанные = Элементы.МестаИспользованияНеудаленных.ТекущиеДанные;
	ТекстСообщения = УстановитьЗаменитьНаСервере(ТекущиеДанные.УдаляемыйСсылка, ВыбранноеЗначение);
	Если Не ПустаяСтрока(ТекстСообщения) Тогда
		ПоказатьПредупреждение(, ТекстСообщения);
	КонецЕсли;
	СтандартныеПодсистемыКлиент.РазвернутьУзлыДерева(ЭтотОбъект, "МестаИспользованияНеудаленных", "*", Истина);
	УстановитьТекущуюСтрокуМестИспользованияНеудаленных(ТекущаяСтрокаДерева);
...
Процедура СвязиНеУдаленныхДействиеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СвязиНеУдаленныхДействие = Новый Структура;
	СвязиНеУдаленныхДействие.Вставить("ТекущаяСтрокаДерева", ТекущаяСтрокаМестИспользованияНеудаленных());
	СвязиНеУдаленныхДействие.Вставить("ИдентификаторСтроки", Элементы.МестаИспользованияНеудаленных.ТекущаяСтрока);
	СвязиНеУдаленныхДействие.Вставить("ВыбранноеЗначение", ВыбранноеЗначение);
	
	ИдентификаторСтроки = Элементы.МестаИспользованияНеудаленных.ТекущаяСтрока;
	ВыделеннаяСтрока = МестаИспользованияНеудаленных.НайтиПоИдентификатору(ИдентификаторСтроки);
	ВыделеннаяСтрока.ДействиеПредставление = "";

	Если ВыбранноеЗначение = "ЗаменитьСсылку" Тогда
		УстановитьЗаменитьНа(Неопределено);
	ИначеЕсли ВыбранноеЗначение = "Удалить" И НЕ ВыделеннаяСтрока.СсылочногоТипа Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нельзя удалить выбранное значение: %1'"),
			ВыделеннаяСтрока.Представление));
	Иначе	
		ПодключитьОбработчикОжидания("СвязиНеУдаленныхДействиеОбработкаВыбораЗавершение", 0.1, Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СвязиНеУдаленныхДействиеОбработкаВыбораЗавершение()

	Если СвязиНеУдаленныхДействие = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТекстСообщения = УстановитьДействиеДляМестаИспользования(СвязиНеУдаленныхДействие.ИдентификаторСтроки, 
		СвязиНеУдаленныхДействие.ВыбранноеЗначение);
	УстановитьТекущуюСтрокуМестИспользованияНеудаленных(СвязиНеУдаленныхДействие.ТекущаяСтрокаДерева);
	Если Не ПустаяСтрока(ТекстСообщения) Тогда
		ПоказатьПредупреждение(, ТекстСообщения);
	КонецЕсли;
	СвязиНеУдаленныхДействие = Неопределено;
...
Процедура ДеревоПомеченныхНаУдалениеУстановитьВсе(Команда)
	...
	ДеревоПомеченныхНаУдалениеУстановитьСнятьВсе(Истина);
...
Процедура ДеревоПомеченныхНаУдалениеСнятьВсе(Команда)
	...
	ДеревоПомеченныхНаУдалениеУстановитьСнятьВсе(Ложь);
...
Процедура УстановитьУдалить(Команда)
	...
	ТекущаяСтрокаДерева = ТекущаяСтрокаМестИспользованияНеудаленных();
	ВыделенныеСтрокиТаблицы = Новый Массив;
	НессылочныеЗначения = Новый Массив;
	ВсеВыделенныеСтрокиТаблицы = Элементы.МестаИспользованияНеудаленных.ВыделенныеСтроки;
	КоличествоСтрокТаблицы = ВсеВыделенныеСтрокиТаблицы.Количество();
	Для Каждого ИдентификаторСтрокиТаблицы Из ВсеВыделенныеСтрокиТаблицы Цикл
		ТекущиеДанные = МестаИспользованияНеудаленных.НайтиПоИдентификатору(ИдентификаторСтрокиТаблицы);
		Если ТекущиеДанные.СсылочногоТипа Тогда
			ВыделенныеСтрокиТаблицы.Добавить(ИдентификаторСтрокиТаблицы);
		Иначе	
			НессылочныеЗначения.Добавить(ТекущиеДанные.Представление);
		КонецЕсли;
	КонецЦикла;

	Если ВыделенныеСтрокиТаблицы.Количество() > 0 Тогда
		УстановитьДействиеДляМестИспользования(ВыделенныеСтрокиТаблицы, "Удалить");
	КонецЕсли;
	Если НессылочныеЗначения.Количество() = 1 Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нельзя удалить выбранное значение: %1.'"),
			НессылочныеЗначения[0]));
	ИначеЕсли НессылочныеЗначения.Количество() > 1 И ВыделенныеСтрокиТаблицы.Количество() > 0 Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нельзя удалить все выбранные значения (%1 из %2).'"),
			НессылочныеЗначения.Количество(), КоличествоСтрокТаблицы));
 	ИначеЕсли НессылочныеЗначения.Количество() > 1 Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нельзя удалить выбранные значения (%1 шт.).'"),
			НессылочныеЗначения.Количество()));
	КонецЕсли;
...
Процедура УстановитьЗаменитьНа(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Состояние(НСтр("ru='Для выбранной строки нельзя указать действие.'"),,, БиблиотекаКартинок.ДиалогВосклицание);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция ТекущаяСтрокаМестИспользованияНеудаленных()

	ТекущиеДанные = Элементы.МестаИспользованияНеудаленных.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат ТекущиеДанные.ОбнаруженныйСсылка;
	
КонецФункции

&НаКлиенте
Процедура УстановитьТекущуюСтрокуМестИспользованияНеудаленных(Значение)

	ЭлементыДерева = МестаИспользованияНеудаленных.ПолучитьЭлементы();
	Если Значение = Неопределено Или ЭлементыДерева.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ИдентификаторСтрокиДерева = 0;
	ОбщегоНазначенияКлиентСервер.ПолучитьИдентификаторСтрокиДереваПоЗначениюПоля("ОбнаруженныйСсылка",
		ИдентификаторСтрокиДерева, ЭлементыДерева, Значение, Ложь);
	Элементы.МестаИспользованияНеудаленных.ТекущаяСтрока = ИдентификаторСтрокиДерева;
КонецПроцедуры

&НаСервере
Функция УстановитьЗаменитьНаСервере(УдаляемыйСсылка, ВыбранноеЗначение)
	...
	ВыделенныеСтрокиТаблицы = Новый Массив;
	Для Каждого ИдентификаторСтрокиТаблицы Из Элементы.МестаИспользованияНеудаленных.ВыделенныеСтроки Цикл

		ВыделеннаяСтрока = МестаИспользованияНеудаленных.НайтиПоИдентификатору(ИдентификаторСтрокиТаблицы);
		Если ВыделеннаяСтрока.УдаляемыйСсылка <> УдаляемыйСсылка Тогда
			Продолжить;
		КонецЕсли;
	...
	Возврат УстановитьДействиеДляМестИспользования(ВыделенныеСтрокиТаблицы, "ЗаменитьСсылку", ВыбранноеЗначение);

КонецФункции

&НаСервере
Функция УстановитьДействиеДляМестИспользования(Знач ИдентификаторыСтрокТаблицы, Знач Действие, Знач Параметр = Неопределено)
	
	ДействиеПредставление = ?(Параметр = Неопределено,
		ДействияМестИспользованияНеудаленных.НайтиПоЗначению(Действие).Представление,
		"");
	ТекстСообщения = "";
	ТаблицаЗначений = ТаблицаДействий();
	Успешно = Ложь;

	Для каждого ИдентификаторСтрокиТаблицы Из ИдентификаторыСтрокТаблицы Цикл
		МестоИспользования = МестаИспользованияНеудаленных.НайтиПоИдентификатору(ИдентификаторСтрокиТаблицы);
		Если Параметр <> Неопределено Тогда
			ДействиеПредставление = ПредставлениеКомандыЗаменитьНа(МестоИспользования, Параметр);
		КонецЕсли;
		
		Если Действие = "Удалить" Тогда

			Фильтр = Новый Структура("ОбнаруженныйСсылка", МестоИспользования.ОбнаруженныйСсылка);
			Для каждого УдаляемоеДействие Из ТаблицаЗначений.НайтиСтроки(Фильтр) Цикл
				ТаблицаЗначений.Удалить(УдаляемоеДействие);
			КонецЦикла;
			
			НовоеДействие = ТаблицаЗначений.Добавить();
			НовоеДействие.Действие = Действие;
			НовоеДействие.ОбнаруженныйСсылка = МестоИспользования.ОбнаруженныйСсылка;
			НовоеДействие.ПараметрДействия = Параметр;
			НовоеДействие.Источник = МестоИспользования.УдаляемыйСсылка;
			Успешно = Истина;
		...
		ИначеЕсли Действие = "ЗаменитьСсылку" Тогда
			...
			Фильтр = Новый Структура("Источник, Действие", МестоИспользования.УдаляемыйСсылка, "ЗаменитьСсылку");
			ИзменяемыеДействия = ТаблицаЗначений.НайтиСтроки(Фильтр);	
			Для каждого ИзменяемоеДействие Из ИзменяемыеДействия Цикл
				ИзменяемоеДействие.ПараметрДействия = Параметр;
			КонецЦикла;
			
			Если ИзменяемыеДействия.Количество() = 0 Тогда
				НовоеДействие = ТаблицаЗначений.Добавить();
				НовоеДействие.Действие = Действие;
				НовоеДействие.ОбнаруженныйСсылка = МестоИспользования.ОбнаруженныйСсылка;
				НовоеДействие.ПараметрДействия = Параметр;
		...
		Иначе
			Если ПустаяСтрока(ТекстСообщения) Тогда
				ТекстСообщения = НСтр("ru='Для места использования %1 нельзя выбрать действие %2.'");
	...
	Если Успешно Тогда // Если есть хотя бы одно измененное действие.
		
		Если Действие = "ЗаменитьСсылку" Тогда
			// Замена ссылки сразу во всех местах использования, кроме удаляемых.
			Фильтр = Новый Структура("ОбнаруженныйСсылка");
			Для каждого МестоИспользования Из МестаИспользованияНеудаленных.ПолучитьЭлементы() Цикл
				Фильтр.ОбнаруженныйСсылка = МестоИспользования.ОбнаруженныйСсылка;
				ТекущееДействие = ТаблицаЗначений.НайтиСтроки(Фильтр);
		...
		ЗначениеВРеквизитФормы(ТаблицаЗначений, "ТаблицаДействий");
		ЗаполнитьМестаИспользованияНеудаленныхОбъектов(ТаблицаЗначений);
	КонецЕсли;
...
&НаСервере
Функция УстановитьДействиеДляМестаИспользования(Знач ИдентификаторСтрокиТаблицы, Знач Действие, Знач Параметр = Неопределено)

	Возврат УстановитьДействиеДляМестИспользования(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторСтрокиТаблицы),
		Действие, Параметр);
...
//
&НаСервере
Функция ТаблицаДействий()
	ТаблицаЗначений = РеквизитФормыВЗначение("ТаблицаДействий", Тип("ТаблицаЗначений")); // ТаблицаЗначений
	ТаблицаЗначений.Индексы.Добавить("ОбнаруженныйСсылка");
	ТаблицаЗначений.Индексы.Добавить("ОбнаруженныйСсылка, Действие");
	ТаблицаЗначений.Индексы.Добавить("Источник, Действие");
...
Процедура ОткрытьФормуЗавершенияРаботыПользователей()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
		...
		ПараметрыФормы.Заголовок = НСтр("ru = 'Не удалось удалить помеченные объекты'");
		ПараметрыФормы.ТекстСообщенияОбОшибке = НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. в приложении работают другие пользователи:'");
		ПараметрыФормы.ТекстСообщенияОбОшибкеЗавершенияРаботы = НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. не удалось завершить работу пользователей:'");
		ПараметрыФормы.ЗавершатьВсеСеансыКромеТекущего = Истина;
		ПараметрыФормы.СообщениеПриВходеВПриложение = НСтр("ru = 'Работа с приложением временно недоступна для удаления помеченных объектов.'");
...
&НаСервере
Процедура ЗаполнитьМестаИспользованияНеудаленныхОбъектов(Знач ТаблицаДействий = Неопределено)

	НеУдаленныеИдентификаторТекущейСтроки = Элементы.НеУдаленные.ТекущаяСтрока;
	СтрокаДерева = ?(НеУдаленныеИдентификаторТекущейСтроки <> Неопределено,
		НеУдаленные.НайтиПоИдентификатору(НеУдаленныеИдентификаторТекущейСтроки), Неопределено);
	
	ОтображаемыеСтроки = МестаИспользованияНеудаленных.ПолучитьЭлементы();
	ОтображаемыеСтроки.Очистить();
	КешПричинНеудаления.Очистить();
	...
	Если ТаблицаДействий = Неопределено Тогда
		ТаблицаДействий = ТаблицаДействий();
	КонецЕсли;

	ТаблицаСвязейНеудаленных = РеквизитФормыВЗначение("СвязиНеудаленных", Тип("ТаблицаЗначений")); // ТаблицаЗначений
	ТаблицаСвязейНеудаленных.Индексы.Добавить("УдаляемыйСсылка");
	
	// Выбрана ссылка не удаленного объекта.
	Показываемые = ТаблицаСвязейНеудаленных.НайтиСтроки(Новый Структура("УдаляемыйСсылка", СтрокаДерева.УдаляемыйСсылка));
	Для Каждого СтрокаТаблицы Из Показываемые Цикл
		...
		ДобавитьПричиныНеудаленияРекурсивно(ТаблицаДействий, ТаблицаСвязейНеудаленных, ОтображаемыеСтроки, СтрокаТаблицы);
...
&НаСервере
Процедура ЗаполнитьДействиеВСтрокеМестаИспользованияНеудаленных(ТаблицаДействий, Знач ПричинаНеудаления, Действие = "")
	Если ПричинаНеудаления.ЭтоОшибка Тогда
		Возврат;
	КонецЕсли;
	
	Фильтр = Новый Структура("ОбнаруженныйСсылка, Действие", ПричинаНеудаления.ОбнаруженныйСсылка, "Удалить");
	ВыбранныеДействия = ТаблицаДействий.НайтиСтроки(Фильтр);
	
	Если ВыбранныеДействия.Количество() = 0 Тогда
		Фильтр = Новый Структура("Источник, Действие", ПричинаНеудаления.УдаляемыйСсылка, "ЗаменитьСсылку");
		ВыбранныеДействия = ТаблицаДействий.НайтиСтроки(Фильтр);
	КонецЕсли;
	...
	ИначеЕсли Действие = "Удалить" Тогда
		ПричинаНеудаления.ДействиеПредставление = ДействияМестИспользованияНеудаленных.НайтиПоЗначению(Действие).Представление;
	Иначе
...
&НаСервере
Процедура ДобавитьПричиныНеудаленияРекурсивно(ТаблицаДействий, ТаблицаСвязейНеудаленных, КоллекцияСтрокРодителя, ДанныеСтроки)
	...
	Показываемые = ТаблицаСвязейНеудаленных.НайтиСтроки(Новый Структура("УдаляемыйСсылка", ДанныеСтроки.ОбнаруженныйСсылка));
	ЕстьТолькоОшибки = Истина;
	Для Каждого СтрокаТаблицы Из Показываемые Цикл
		Если НЕ СтрокаТаблицы.ЭтоОшибка Тогда
			ЕстьТолькоОшибки = Ложь;
		КонецЕсли;
		
		Если КешПричинНеудаления.НайтиПоЗначению(СтрокаТаблицы.ОбнаруженныйСсылка) = Неопределено Тогда
			КешПричинНеудаления.Добавить(СтрокаТаблицы.УдаляемыйСсылка);
			ДобавитьПричиныНеудаленияРекурсивно(ТаблицаДействий, ТаблицаСвязейНеудаленных, ПодчиненныеСтроки, СтрокаТаблицы);
		КонецЕсли;
	КонецЦикла;

	НоваяСтрока.ОсновнаяПричина = НЕ НоваяСтрока.ЭтоОшибка 
		И (ПодчиненныеСтроки.Количество() = 0 Или ЕстьТолькоОшибки Или Показываемые.Количество() = 0);
									
	ЗаполнитьДействиеВСтрокеМестаИспользованияНеудаленных(ТаблицаДействий, НоваяСтрока, ?(Показываемые.Количество() <> 0, "Удалить", ""));
...
Процедура МестаИспользованияНеудаленныхПриАктивизацииСтроки(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ОбнаруженныйСсылка) Тогда
		...
		Если НЕ(Действия.Количество() > 0 
			...
			Элементы.СвязиНеУдаленныхДействие.СписокВыбора.Добавить("ЗаменитьСсылку", 
				ПредставлениеКомандыЗаменитьНа(ТекущиеДанные));
		КонецЕсли;
...
Функция ПредставлениеКомандыЗаменитьНа(ТекущиеДанные, Параметр = Неопределено)
	...
	ПредставлениеСтрокиВерхнегоУровня = ?(СтрокаВерхнегоУровня <> Неопределено,
		СтрокаВерхнегоУровня.Представление, ТекущиеДанные.ПредставлениеУдаляемый);

	Если Параметр <> Неопределено Тогда
		Результат = СтрЗаменить(НСтр("ru='Заменить %1 на %2'"), "%1", ПредставлениеСтрокиВерхнегоУровня);
		Возврат СтрЗаменить(Результат, "%2", Параметр);
	Иначе
		Возврат СтрЗаменить(НСтр("ru='Заменить %1 на...'"), "%1", ПредставлениеСтрокиВерхнегоУровня);
	КонецЕсли;
...
Функция НачатьПоискПомеченныхСервер(ОтборМетаданных, УникальныйИдентификаторФормы, ИскатьТехнологическиеОбъекты = Ложь)
	...
	АктуализироватьНастройкиДополнительныхРеквизитовПередПоискомПомеченных();
	...
	Задание = ДлительныеОперации.ВыполнитьФункцию(ПараметрыМетода,
		"УдалениеПомеченныхОбъектовСлужебный.ПомеченныеНаУдаление",
		ОтборМетаданных, 
		ДополнительныеРеквизитыПомеченныхНаУдаления.Выгрузить(), 
...
Процедура ПослеЗавершенияПоискаПомеченных(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполняетсяЗадание = Ложь;
	Если Результат.Статус = "Ошибка" Тогда
		ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(ЭтотОбъект, Результат);
		УстановитьСостояниеВыбораПомеченныхНаУдалениеСПанельюСостояния();
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(Форма, Результат)
	Форма.ИнформацияОбОшибкеФоновогоЗадания = Результат.ИнформацияОбОшибке;
	Форма.Элементы.ДекорацияПредставлениеЗавершенияНеУспешно.Заголовок = 
		?(ЭтоОшибкаУстановкиМонопольногоРежима(Результат.ИнформацияОбОшибке),
			НСтр("ru='Не удалось установить монопольный режим'"),
			ОбработкаОшибок.СообщениеОбОшибкеДляПользователя(Результат.ИнформацияОбОшибке));
...
Процедура НачатьУдалениеПомеченных(Параметр = Неопределено)
	...
	ВыполняемаяОперация = НачатьУдалениеПомеченныхСервер(УникальныйИдентификатор, РезультатПредыдущегоШага,
...
Функция НачатьУдалениеПомеченныхСервер(УникальныйИдентификаторФормы, РезультатПредыдущегоШага, ПовторныйПоиск = Ложь)
	...
	Если ИтоговыйРежимУдаления = "Монопольный" Тогда
		ДопустимыеРежимыУдаления = УдалениеПомеченныхОбъектовСлужебный.ДопустимыеРежимыУдаления();
		Задание = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
			"УдалениеПомеченныхОбъектовСлужебный.УдалитьПомеченныеОбъекты",
	...
	Иначе
		...
		Задание = ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков(
			"УдалениеПомеченныхОбъектовСлужебный.УдалитьПомеченныеОбъекты",
...
Процедура ПослеЗавершенияУдаленияПомеченных(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СведенияОРезультатахУдаления.Успешно Тогда
		Если УдалятьПриОткрытии Тогда
...
Функция ОбработатьРезультатУдаленияОшибка(Результат)
	...
	ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(ЭтотОбъект, Результат);
	УстановитьМонопольныйРежимНаСервере(Ложь);
	Возврат НовыйСведенияОРезультатахУдаления();
КонецФункции
...
// Параметры:
//   СведенияОРезультатахУдаления - см. НовыйСведенияОРезультатахУдаления
//
&НаСервере
...
//   
// Возвращаемое значение:
//   см. НовыйСведенияОРезультатахУдаления
//
&НаСервере
...
Функция СформироватьРезультатУдаления(СведенияОРезультате, РезультатВыполненияВФоне, ИдентификаторХранилищаРезультата)
	Результат = ?(СведенияОРезультате = Неопределено, НовыйСведенияОРезультатахУдаления(), СведенияОРезультате);
...
Функция НачатьВыполнениеДополнительнойОбработкиСервер()
	...
	Задание = ДлительныеОперации.ВыполнитьФункцию(ПараметрыМетода,
		"УдалениеПомеченныхОбъектовСлужебный.ВыполнитьОбработкуПричинНеудаления",
		ТаблицаДействий.Выгрузить());
	Возврат Задание;
...
Процедура ПриОбновленииПрогрессаФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	Если Результат.Прогресс <> Неопределено Тогда
		...
		Если Не ДополнительныеПараметры.ЭтоПроцессУдаления Или ПараметрыПрогресса = Неопределено Тогда
			ТекстПрогресса = Результат.Прогресс.Текст;
		Иначе
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоОшибкаУстановкиМонопольногоРежима(ИнформацияОбОшибке)
	Если ИнформацияОбОшибке = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	ТекстОшибкиМонопольно = НСтр("ru='Ошибка разделенного доступа к базе'");
	Возврат СтрНайти(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке), ТекстОшибкиМонопольно) <> 0;
КонецФункции
...
Процедура ЗапуститьЗадание(Задание = "", Параметр = Неопределено)
	...
	Если НЕ ПустаяСтрока(ТекущееЗаданиеФормы.Имя) Тогда
		...
		Если РежимУдаления = "Монопольный" 
			...
			ЗаполнитьЗначенияСвойств(Результат, УстановитьМонопольныйРежимНаСервере(Истина));
			Если Результат.Статус = "Ошибка" Тогда
				ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(ЭтотОбъект, Результат);
				Элементы.СтраницыОтображениеСостояния.Видимость = Истина;
...
&НаКлиенте
Процедура ДеревоПомеченныхНаУдалениеУстановитьСнятьВсе(Значение)
	
	Если Элементы.ДеревоПомеченныхНаУдаление.ВыделенныеСтроки.Количество() > 1 Тогда
		Для Каждого ИдентификаторСтроки Из Элементы.ДеревоПомеченныхНаУдаление.ВыделенныеСтроки Цикл
			ВыделенныйЭлемент = ДеревоПомеченныхНаУдаление.НайтиПоИдентификатору(ИдентификаторСтроки);
			Если ВыделенныйЭлемент = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ВыделенныйЭлемент.Пометка = Значение;
			ЭлементыСтроки = ВыделенныйЭлемент.ПолучитьЭлементы();
			Для Каждого ЭлементСтроки Из ЭлементыСтроки Цикл
				ЭлементСтроки.Пометка = Значение;
				ДеревоПомеченныхНаУдалениеУстановитьПометкуВСписке(ЭлементСтроки, Значение, Ложь);
			КонецЦикла;
			
			Родитель = ВыделенныйЭлемент.ПолучитьРодителя();
			Если Родитель = Неопределено Тогда
				ДеревоПомеченныхНаУдалениеПроверитьРодителя(ВыделенныйЭлемент);
			Иначе
				ДеревоПомеченныхНаУдалениеПроверитьРодителя(Родитель);
			КонецЕсли;
	...
	Для Каждого Элемент Из ЭлементыСписка Цикл
		ДеревоПомеченныхНаУдалениеУстановитьПометкуВСписке(Элемент, Значение, Истина);
...
//
&НаКлиентеНаСервереБезКонтекста
Функция НовыйСведенияОРезультатахУдаления()
	Результат = Новый Структура;
...
&НаСервере
Процедура АктуализироватьНастройкиДополнительныхРеквизитовПередПоискомПомеченных();
	
	НастройкиДополнительныхРеквизитов = ДополнительныеРеквизитыПомеченныхНаУдаления.Выгрузить();
	УдаляемыеСтроки = Новый Массив;
	Для Каждого Настройка Из НастройкиДополнительныхРеквизитов Цикл
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(Настройка.Метаданные);
		Если ОбъектМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОбъектМетаданных.Реквизиты, Настройка.Реквизит) Тогда
			УдаляемыеСтроки.Добавить(Настройка);
		КонецЕсли;
	КонецЦикла;
	
	Если УдаляемыеСтроки.Количество() > 0 Тогда
		Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
			НастройкиДополнительныхРеквизитов.Удалить(УдаляемаяСтрока);
		КонецЦикла;
		ДополнительныеРеквизитыПомеченныхНаУдаления.Загрузить(НастройкиДополнительныхРеквизитов);
	КонецЕсли;
