...
Процедура ОтменитьОперацию(Команда)
	...
	Если ТекущееЗадание() = "ВыполнениеДополнительнойОбработки" Тогда
...
Процедура УстановитьУдалить(Команда)
	Для Каждого ИдентификаторСтроки Из Элементы.МестаИспользованияНеудаленных.ВыделенныеСтроки Цикл
		...
		Если ВыделеннаяСтрока.ОсновнаяПричина Тогда
			ЗарегистрироватьДействиеМестаИспользования(ВыделеннаяСтрока, "Удалить");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗарегистрироватьДействиеМестаИспользования(МестоИспользования, Действие, Параметр = Неопределено)
	...
	МестоИспользования.ДействиеПредставление = ?(Параметр = Неопределено,
		ДействияМестИспользованияНеудаленных.НайтиПоЗначению(МестоИспользования.Действие),
	...
	Если Действие <> Неопределено Тогда
		...
		Если ЗарегистрированноеДействие.Количество() = 0 Тогда
			...
			ЗарегистрированноеДействие.Действие = МестоИспользования.Действие;
		...
		Иначе
			ЗарегистрированноеДействие = ТаблицаДействий[0];
			ЗарегистрированноеДействие.Действие = МестоИспользования.Действие;
			ЗарегистрированноеДействие.ПараметрДействия = Параметр;
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборОформления = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОформления.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("МестаИспользованияНеудаленных.СсылочногоТипа");
	ОтборОформления.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборОформления.ПравоеЗначение = Истина;
	
	ОтборОформления = ГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОформления.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("МестаИспользованияНеудаленных.ОбнаруженныйСсылка");
...
&НаСервере
Процедура УстановитьСписокДействийМестИспользования()
	ДействияМестИспользованияНеудаленных.Добавить("Удалить", "Удалить");
	ДействияМестИспользованияНеудаленных.Добавить("ЗаменитьСсылку", "Заменить %ПредставлениеСсылка% на");
	
	Элементы.СвязиНеУдаленныхДействие.СписокВыбора.Очистить();
	Для Каждого ЭлементСписка Из ДействияМестИспользованияНеудаленных Цикл
		Элементы.СвязиНеУдаленныхДействие.СписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
	КонецЦикла;
КонецПроцедуры
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВыбранноеДействиеНадПричинойНеудаления(Форма, ПричинаНеудаления, Действие = "")
...
Процедура ДобавитьПричиныНеудаленияРекурсивно(Форма, КоллекцияСтрокРодителя, ДанныеСтроки)
	...
	НоваяСтрока.ОсновнаяПричина = (ПодчиненныеСтроки.Количество() = 0 Или СодержатьсяТолькоОшибки)
		И НоваяСтрока.СсылочногоТипа И Показываемые.Количество() = 0;
КонецПроцедуры
...
Процедура МестаИспользованияНеудаленныхПриАктивизацииСтроки(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ОбнаруженныйСсылка) Тогда
		Элементы.СвязиНеУдаленныхДействие.СписокВыбора[1].Представление = ПредставлениеКомандыЗаменитьНа(ТекущиеДанные);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура НачатьПоискПомеченных()
...
&НаКлиенте
Процедура НачатьУдалениеПомеченных()
...
&НаСервере
Функция НачатьУдалениеПомеченныхСервер(УникальныйИдентификаторФормы, РезультатПредыдущегоШага)
	Если УдалятьПриОткрытии Тогда
...
&НаКлиенте
Процедура НачатьВыполнениеДополнительнойОбработки()
...
&НаКлиенте
Процедура ДобавитьЗадание(ИмяОперации)
	ОчередьЗаданийФормы.Добавить(ИмяОперации);
КонецПроцедуры

&НаКлиенте
Процедура УдалитьТекущееЗадание()
...
Функция ТекущееЗадание()
	Возврат ?(ОчередьЗаданийФормы.Количество() = 0, "", ОчередьЗаданийФормы[0]);
КонецФункции
...
&НаКлиенте
Процедура ЗапуститьЗадание(Задание = "")
	Результат = Новый Структура("Успешно", Истина);
	
	Если НЕ ПустаяСтрока(Задание) И Задание <> ТекущееЗадание() Тогда
	...
	Если НЕ ПустаяСтрока(ТекущееЗаданиеФормы) Тогда
		
		Если РежимУдаления = "Монопольный" 
				И ТекущееЗаданиеФормы <> ЗаданияФормы().ПоискПомеченных Тогда
		...
		Если ТекущееЗаданиеФормы = ЗаданияФормы().УдалениеПомеченных Тогда
			НачатьУдалениеПомеченных();
		ИначеЕсли ТекущееЗаданиеФормы = ЗаданияФормы().ПоискПомеченных Тогда 
			НачатьПоискПомеченных();
		ИначеЕсли ТекущееЗаданиеФормы = ЗаданияФормы().ВыполнениеДополнительнойОбработки Тогда 
			НачатьВыполнениеДополнительнойОбработки();
		КонецЕсли;
