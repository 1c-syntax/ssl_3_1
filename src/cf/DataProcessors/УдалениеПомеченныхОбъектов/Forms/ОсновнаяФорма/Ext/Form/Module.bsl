...
Процедура УдалитьВсе(Команда)
	ОповещениеПодтверждениеУдалитьВсе = Новый ОписаниеОповещения("ПриПодтвержденииУдалитьВсе", ЭтотОбъект, Новый Структура("Команда", Команда));
	
	Если ВыполняемаяОперация = Неопределено И ВыбраныНеВсеОбъектыВДеревеПомеченных() Тогда
	    ПоказатьВопрос(
			ОповещениеПодтверждениеУдалитьВсе,
			НСтр("ru='Удалить все помеченные на удаление?'"),
			РежимДиалогаВопрос.ДаНет);
	Иначе
		ВыполнитьОбработкуОповещения(ОповещениеПодтверждениеУдалитьВсе, КодВозвратаДиалога.Да);
	КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьВыбранные(Команда)
	ОтправитьСтатистику(Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.ИмяКоманды);
...
Процедура УстановитьСнятьПометкуУдаленияПродолжение(Результат, СсылкиДляОбработки) Экспорт
	...
	Если ТипЗнч(Изменения) = Тип("Строка") Тогда
		Ошибка = НСтр("ru='Произошла ошибка при изменении пометки удаления по причине:'")
					+ Символы.ПС + Изменения;
		ПоказатьПредупреждение(, Ошибка,,
			НСтр("ru='Ошибка при изменении пометки удаления.'"));
	Иначе
...
Функция УстановитьСнятьПометкуУдаленияНаСервере(Ссылки)
	...
	Для каждого Изменение Из Изменения Цикл
		...
		ИнформацияОТипе = УдалениеПомеченныхОбъектовСлужебный.ИнформацияОТипе(
							ТипЗнч(Изменение.Ссылка), 
							Кеш);
		СтрокиМетаданных = ДеревоНеудаленные.Строки.НайтиСтроки(
							Новый Структура("УдаляемыйСсылка", ВРег(ИнформацияОТипе.ПолноеИмя)))[0].Строки;
		Данные = СтрокиМетаданных.НайтиСтроки(
					Новый Структура("УдаляемыйСсылка", Изменение.Ссылка))[0];
		
		Данные.НомерКартинки = УдалениеПомеченныхОбъектовСлужебный.НомерКартинки(
								Изменение.Ссылка,
								ИнформацияОТипе.Ссылочный,
								ИнформацияОТипе.Вид,
...
&НаКлиенте
Процедура ОтправитьСтатистику(ИмяКоманды)

	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЦентрМониторинга") Тогда
		Возврат;
	КонецЕсли;	
	
	ОбщегоНазначенияКлиент.ОбщийМодуль("ЦентрМониторингаКлиент").ЗаписатьОперациюБизнесСтатистики("УдалениеПомеченных.Интерфейс." + ИмяКоманды, 1);
...
&НаКлиенте
Функция ВыбраныНеВсеОбъектыВДеревеПомеченных()
	Результат = Ложь;
	
	Для каждого СтрокаГруппы Из ДеревоПомеченныхНаУдаление.ПолучитьЭлементы() Цикл
	
		Если СтрокаГруппы.Пометка <> 1 Тогда
			Результат = Истина;
			Прервать;
		КонецЕсли;
...
&НаКлиенте
Процедура ПриПодтвержденииУдалитьВсе(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	ОтправитьСтатистику(Элементы.ДеревоПомеченныхНаУдалениеУдалитьВсе.ИмяКоманды);
	ДобавитьЗадание(ЗаданияФормы().УдалениеПомеченных, "ПолноеУдаление");
	ДеревоПомеченныхНаУдалениеУстановитьВсе(ДополнительныеПараметры.Команда);
	ЗапуститьЗадание();
КонецПроцедуры
