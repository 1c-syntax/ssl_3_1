...
&НаКлиенте
Перем ВыполняемаяОперация;// Неопределено, см. ДлительныеОперации.ВыполнитьФункцию
...
Процедура УдалитьВыбранные(Команда)
	...
	Если СведенияОВыбранныхОбъектах.КоличествоВыбранных = СведенияОВыбранныхОбъектах.КоличествоВсего Тогда
	    ТекстВопроса = НСтр("ru='Удалить все помеченные на удаление?'");
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Удалить помеченные на удаление (%1)?'"),
							СведенияОВыбранныхОбъектах.КоличествоВыбранных);
	КонецЕсли;
	
	ОповещениеПодтверждениеУдалитьВсе = Новый ОписаниеОповещения(
											"ПриПодтвержденииУдалить",
											ЭтотОбъект,
											СведенияОВыбранныхОбъектах);
...
Функция ОтборМетаданныхТолькоСуществующие(ОтборМетаданных)
	...
	Для каждого ВыбранныеМетаданные Из ОтборМетаданных Цикл
		Если Метаданные.НайтиПоПолномуИмени(ВыбранныеМетаданные.Значение) = Неопределено Тогда
			Продолжить;
		КонецЕсли;     
...
Функция НачатьУдалениеПомеченныхСервер(УникальныйИдентификаторФормы, РезультатПредыдущегоШага, ПовторныйПоиск = Ложь)
	...
	ПараметрыМетода = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификаторФормы);
	ПараметрыМетода.НаименованиеФоновогоЗадания = НСтр("ru = 'Удаление помеченных объектов'");
	ПараметрыМетода.ЗапуститьВФоне = Истина;
...
Процедура ПослеЗавершенияУдаленияПомеченных(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		...
		СведенияОРезультатахУдаления = ОбработатьРезультатУдаленияОшибка(ЭтотОбъект, Результат);
	ИначеЕсли Результат.Статус = "Выполнено" Тогда 
		
		СведенияОРезультатахУдаления = ОбработатьРезультатУдаленияУспех(
										Результат, 
										УникальныйИдентификаторСУчетомВладельца(), 
...
&НаКлиенте
Функция ОбработатьРезультатУдаленияОшибка(ЭтотОбъект, Результат)
	УстановитьСостояниеВыбораПомеченныхНаУдалениеСПанельюСостояния();
	ЗаполнитьИнформациюОшибкамиФоновогоЗадания(ЭтотОбъект, Результат);
	УстановитьМонопольныйРежимНаСервере(Ложь);
