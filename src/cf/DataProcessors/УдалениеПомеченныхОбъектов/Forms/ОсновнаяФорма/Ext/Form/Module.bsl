...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РегламентноеЗадание = РегламентныеЗаданияНайтиПредопределенное();
	...
	ВидимостьРасписания = ВидимостьФлажка
	...
	Если Не ВидимостьРасписания Тогда 
		Элементы.УдалениеПомеченныхИспользование.Заголовок = НСтр("ru = 'Автоматически удалять помеченные объекты'");
	КонецЕсли;
...
Процедура УдалениеПомеченныхИспользованиеПриИзменении(Элемент)
	Если УдалениеПомеченныхИспользование И Элементы.УдалениеПомеченныхПредставлениеРасписания.Видимость Тогда
		Если Элементы.УдалениеПомеченныхПредставлениеРасписания.Видимость Тогда
			РегламентныеЗаданияИзменитьРасписание();
			Возврат;
		КонецЕсли;
	КонецЕсли;
	Изменения = Новый Структура("Использование", УдалениеПомеченныхИспользование);
	РегламентныеЗаданияСохранить(Изменения);
КонецПроцедуры
...
Процедура КомандаДалее(Команда)
	ЗапуститьФоновоеЗаданиеКлиент(1);
КонецПроцедуры

&НаКлиенте
Процедура КомандаНазад(Команда)
	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления;
	ВидимостьДоступность(ЭтотОбъект);
КонецПроцедуры
...
Процедура ДеревоПомеченныхНаУдалениеОбновить(Команда)
	ЗапуститьФоновоеЗаданиеКлиент(2);
КонецПроцедуры
...
Процедура ПоказатьСвязиНеУдаленныхНаКлиенте()
	Если НеУдаленныеИдентификаторТекущейСтроки = Элементы.НеУдаленные.ТекущаяСтрока Тогда
		Возврат;
	КонецЕсли;
	ПоказатьСвязиНеУдаленныхНаСервере(ЭтотОбъект);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ПоказатьСвязиНеУдаленныхНаСервере(Форма)
	...
	Иначе
		...
		Скрываемые = Форма.СвязиНеУдаленных.НайтиСтроки(Новый Структура("Видимость", Истина));
		Для Каждого СтрокаТаблицы Из Скрываемые Цикл
			СтрокаТаблицы.Видимость = Ложь;
		КонецЦикла;
		
		Форма.Элементы.СвязиНеУдаленных.ОтборСтрок = Новый ФиксированнаяСтруктура("УдаляемыйСсылка", СтрокаДерева.УдаляемыйСсылка);
		Форма.НеУдаленныеПодсказка = " ";
		...
		Для Каждого СтрокаТаблицы Из Показываемые Цикл
			...
			Иначе
				Если ПоказыватьТекстОшибки Тогда
					Форма.Элементы.СвязиНеУдаленных.ТекущаяСтрока = СтрокаТаблицы.ПолучитьИдентификатор();
	...
	КонецЕсли;
	Если ПоказыватьТекстОшибки Тогда
		ТекущаяСтраница = Форма.Элементы.СтраницаТекстОшибки;
		Форма.Элементы.СсылкаПодробнее.Видимость = ЗначениеЗаполнено(ПодробныйТекстОшибки);
		Форма.Элементы.ТекстОшибки.Заголовок = ТекстОшибки;
	Иначе
		ТекущаяСтраница = Форма.Элементы.СтраницаПричиныНеУдаления;
		Шаблон = НСтр("ru = 'Места использования объекта ""%1"" (%2):'");
		Форма.НеУдаленныеПодсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, СтрокаДерева.Представление, Формат(СтрокаДерева.КоличествоСвязей, "ЧН=0; ЧГ="));
	КонецЕсли;
	Если Форма.Элементы.СтраницыВариантовОтображенияПричин.ТекущаяСтраница <> ТекущаяСтраница Тогда
		Форма.Элементы.СтраницыВариантовОтображенияПричин.ТекущаяСтраница = ТекущаяСтраница;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	ПараметрыВыполнения.Вставить("МассивСтрокТаблицы", МассивСтрокТаблицы);
	ПараметрыВыполнения.Вставить("МассивСсылок", МассивСсылок);
	ПараметрыВыполнения.Вставить("ИмяРеквизитаЗначение", ИмяРеквизитаЗначение);
	ПараметрыВыполнения.Вставить("УстанавливатьПометку", УстанавливатьПометку);
	
	Обработчик = Новый ОписаниеОповещения("ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение", ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	ОповещениеОбИзменениях = ИзменитьПометкуУдаленияОбъектов(ПараметрыВыполнения.МассивСсылок, ПараметрыВыполнения.УстанавливатьПометку);
	
	КоличествоОбъектов = ПараметрыВыполнения.МассивСсылок.Количество();
	Если КоличествоОбъектов > 0 Тогда
		СтандартныеПодсистемыКлиент.РазвернутьУзлыДерева(ЭтотОбъект, "НеУдаленные", "*", Истина);
	КонецЕсли;
	
	СтандартныеПодсистемыКлиент.ОповеститьФормыОбИзменении(ОповещениеОбИзменениях,
		Новый Структура("ПометкаУдаления", ПараметрыВыполнения.УстанавливатьПометку));
	...
	Иначе
		Если ПараметрыВыполнения.УстанавливатьПометку Тогда
		...
		Если КоличествоОбъектов = 1 Тогда
			ОповещениеСсылка = ПараметрыВыполнения.МассивСсылок[0];
...
&НаКлиенте
Процедура РегламентныеЗаданияИзменитьРасписание()
	Обработчик = Новый ОписаниеОповещения("РегламентныеЗаданияПослеИзмененияРасписания", ЭтотОбъект);
	Диалог = Новый ДиалогРасписанияРегламентногоЗадания(УдалениеПомеченныхРасписание);
...
Процедура РегламентныеЗаданияПослеИзмененияРасписания(Расписание, ПараметрыВыполнения) Экспорт
	...
	УдалениеПомеченныхРасписание = Расписание;
...
Процедура ВидимостьДоступность(Форма)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаУспешноЗавершено Тогда
		...
		Элементы.КнопкаДалее.Видимость = Ложь;
		Элементы.КнопкаЗакрыть.КнопкаПоУмолчанию = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВозниклаОшибка Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.ПодсказкаМонопольногоРежима.Видимость = Форма.Монопольно;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция НомерКартинки(Знач СсылкаИлиДанные, Знач СсылочногоТипа, Знач Вид, Знач Статус)
...
Функция ИзменитьПометкуУдаленияОбъектов(МассивСсылок, ПометкаУдаления)
	...
	Попытка
		...
		Для Номер = 1 По Количество Цикл
			Ссылка = МассивСсылок[Количество - Номер];
	...
	Если КоличествоОбъектов > 0 Тогда
		...
		Для Каждого Ссылка Из МассивСсылок Цикл
			...
			Для Каждого СтрокаДерева Из Найденные Цикл
				...
				СтрокаДерева.НомерКартинки   = НомерКартинки(Ссылка, Истина, ОбщегоНазначения.ВидОбъектаПоСсылке(Ссылка),
			...
			Для Каждого СтрокаТаблицы Из Найденные Цикл
				...
				СтрокаТаблицы.НомерКартинки   = НомерКартинки(Ссылка, Истина, ОбщегоНазначения.ВидОбъектаПоСсылке(Ссылка),
					?(ПометкаУдаления, "Удален", ""));
				СтрокаТаблицы.ПометкаУдаленияМодифицирована = Истина;
			КонецЦикла;
		КонецЦикла;
		
		ТаблицаВРеквизитФормы("НеУдаленные", ДеревоЗначений, "УдаляемыйСсылка");
		ТаблицаВРеквизитФормы("СвязиНеУдаленных", ТаблицаЗначений, "УдаляемыйСсылка, ОбнаруженныйСсылка");
		НеУдаленныеИдентификаторТекущейСтроки = Элементы.НеУдаленные.ТекущаяСтрока;
	КонецЕсли;
	
	Возврат СтандартныеПодсистемыСервер.ПодготовитьОповещениеФормОбИзменении(МассивСсылок);
	
КонецФункции

&НаСервере
Процедура ТаблицаВРеквизитФормы(ИмяТаблицы, ДанныеТаблицы, КлючевыеКолонки)
...
&НаСервере
Процедура РегламентныеЗаданияСохранить(Изменения)
	РегламентныеЗаданияСервер.ИзменитьЗадание(УдалениеПомеченныхИдентификатор, Изменения);
	УстановитьДоступность();
КонецПроцедуры

&НаСервере
Функция РегламентныеЗаданияНайтиПредопределенное()
	Фильтр = Новый Структура("Метаданные", "УдалениеПомеченных");
...
Процедура УстановитьДоступность()
	...
	Если Элементы.УдалениеПомеченныхНастроитьРасписание.Видимость Тогда
...
Процедура ЗаполнитьКоллекцииОставшихсяОбъектов(РезультатВыполненияВФоне)
	...
	Для Каждого Причина Из ПрепятствующиеУдалению Цикл
		...
		Если СтрокаНеУдаленного = Неопределено Тогда
			...
			СтрокаНеУдаленного.НомерКартинки = НомерКартинки(СтрокаНеУдаленного.УдаляемыйСсылка,
		...
		СтрокаПрепятствующегоУдалению.ПометкаУдаления    = (Причина.ОбнаруженныйСтатус = "Удален");
		...
		Иначе
			...
			СтрокаПрепятствующегоУдалению.НомерКартинки = НомерКартинки(СтрокаПрепятствующегоУдалению.ОбнаруженныйСсылка,				
				СтрокаПрепятствующегоУдалению.СсылочногоТипа, ОбнаруженныйИнформация.Вид, Причина.ОбнаруженныйСтатус);
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	ПараметрыМетода = Новый Структура;
	ПараметрыМетода.Вставить("ИскатьПомеченные", Ложь);
	ПараметрыМетода.Вставить("УдалятьПомеченные", Ложь);
	ПараметрыМетода.Вставить("ПрочитатьПомеченныеСоСтраницыРезультатов", Ложь);
	ПараметрыМетода.Вставить("ПрочитатьПомеченныеСоСтраницыУстановкиФлажков", Ложь);
	
	Если Режим = 1 Тогда
		ТекущаяСтраница = Элементы.Страницы.ТекущаяСтраница;
		Если ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления Тогда
			Если РежимУдаления = "Полный" Тогда
				ПараметрыМетода.ИскатьПомеченные = Истина;
				ПараметрыМетода.УдалятьПомеченные = Истина;
			Иначе
				ПараметрыМетода.ИскатьПомеченные = Истина;
			КонецЕсли;
		ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление Тогда
			ПараметрыМетода.ПрочитатьПомеченныеСоСтраницыУстановкиФлажков = Истина;
			ПараметрыМетода.УдалятьПомеченные = Истина;
		ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
			Если РежимУдаления = "Полный" Тогда
				ПараметрыМетода.ИскатьПомеченные = Истина;
			Иначе
				ПараметрыМетода.ПрочитатьПомеченныеСоСтраницыРезультатов = Истина;
			КонецЕсли;
			ПараметрыМетода.УдалятьПомеченные = Истина;
		КонецЕсли;
	...
	Если ПараметрыМетода.ИскатьПомеченные Тогда
	...
	Задание = ЗапуститьФоновоеЗадание(ПараметрыМетода);
	Если Задание = Неопределено Тогда
		Возврат; // Удаление не требуется.
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеУстановкиМонопольногоРежима", ЭтотОбъект, Режим);
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заблокировать работу в программе по причине: 
				           |%1'"),
				Задание.ТекстОшибкиУстановкиМонопольногоРежима);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	Если ПараметрыМетода.УдалятьПомеченные И Монопольно Тогда
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			Результат = Новый Структура;
			Результат.Вставить("ОшибкаПриУстановкеМонопольногоРежима", Истина);
			Результат.Вставить("ТекстОшибкиУстановкиМонопольногоРежима", КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	
	// Запуск фонового задания
	Если Не ПараметрыМетода.ИскатьПомеченные Тогда
		Если ПараметрыМетода.ПрочитатьПомеченныеСоСтраницыРезультатов Тогда
			ПользовательскиеОбъекты = ПомеченныеНаУдалениеСоСтраницыРезультатов();
		ИначеЕсли ПараметрыМетода.ПрочитатьПомеченныеСоСтраницыУстановкиФлажков Тогда
			ПользовательскиеОбъекты = ПомеченныеНаУдалениеСоСтраницыУстановкиФлажков();
		КонецЕсли;
		Если ПользовательскиеОбъекты.Количество() = 0 Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаУдалениеНеТребуется;
			ВидимостьДоступность(ЭтотОбъект);
			Возврат Неопределено;
		КонецЕсли;
		ПараметрыМетода.Вставить("ПользовательскиеОбъекты", ПользовательскиеОбъекты);
	КонецЕсли;
	
	ПараметрыМетода.Вставить("ПериодЗаписи", 3); // Секунд
	ПараметрыМетода.Вставить("Монопольно", Монопольно);
	
	// Переключение на страницу длительной операции.
	...
	Если ПоказыватьБублик Тогда
		...
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Лево;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
	ИначеЕсли ПараметрыМетода.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru = 'Удаляются объекты, помеченные на удаление...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
	...
	ИмяМетода = "Обработки.УдалениеПомеченныхОбъектов.УдалитьПомеченныеОбъектыИнтерактивно";
	
	НастройкиЗапуска = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Удаление помеченных объектов (интерактивное)'");
	
	Задание = ДлительныеОперации.ВыполнитьВФоне(ИмяМетода, ПараметрыМетода, НастройкиЗапуска);
...
&НаКлиенте
Процедура ПослеУстановкиМонопольногоРежима(Результат, Режим) Экспорт
	Если Результат = Ложь Тогда // Монопольный режим установлен.
		ЗапуститьФоновоеЗаданиеКлиент(Режим);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Результат = ЗагрузитьРезультатФоновогоЗадания(Задание.АдресРезультата);
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ru = 'Не удалось прочитать результат удаления помеченных'"), "");
			Возврат;
		КонецЕсли;
		
		СтандартныеПодсистемыКлиент.ОповеститьФормыОбИзменении(
			Результат.ОповещениеОбИзменениях,
			Новый Структура("УдалениеПомеченных", Истина));
		
		Если Не ПустаяСтрока(Результат.ОповещениеТекст) Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Удаление помеченных'"),
				НавигационнаяСсылка,
				Результат.ОповещениеТекст,
				Результат.ОповещениеКартинка);
		КонецЕсли;
		
		Если Результат.РазвернутьДеревоПомеченныхНаУдаление Тогда
			СтандартныеПодсистемыКлиент.РазвернутьУзлыДерева(ЭтотОбъект, "ДеревоПомеченныхНаУдаление");
		КонецЕсли;
		
	Иначе
		
		ВывестиОшибку(Задание.КраткоеПредставлениеОшибки, Задание.ПодробноеПредставлениеОшибки);
...
&НаСервере
Функция ЗагрузитьРезультатФоновогоЗадания(АдресРезультата)
	Если Монопольно Тогда
		СнятьМонопольныйРежим();
	КонецЕсли;
	...
	Результат = Новый Структура;
	Результат.Вставить("ОповещениеОбИзменениях", Новый Соответствие);
	Результат.Вставить("ОповещениеТекст", "");
	Результат.Вставить("ОповещениеКартинка", Неопределено);
	Результат.Вставить("РазвернутьДеревоПомеченныхНаУдаление", Ложь);
	
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		
		Результат.ОповещениеОбИзменениях = СтандартныеПодсистемыСервер.ПодготовитьОповещениеФормОбИзменении(РезультатВыполненияВФоне.Удаленные);
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаУспешноЗавершено;
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Удалено объектов: %1.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="));
			Элементы.НадписьУспешноЗавершено.Заголовок = Результат.ОповещениеТекст;
		Иначе
			Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления;
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Удалено: %1, не удалено: %2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
				Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Результат.ОповещениеКартинка = БиблиотекаКартинок.Предупреждение32;
	...
	Иначе
		...
		Для Каждого УдаляемыйСсылка Из ПользовательскиеОбъекты Цикл
			...
			УдаляемыйИнформация = Обработки.УдалениеПомеченныхОбъектов.СформироватьИнформациюОТипах(РезультатВыполненияВФоне, ТипУдаляемого);
		...
		Если КоличествоТипов = 0 Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаУдалениеНеТребуется;
		Иначе
			Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление;
			Если КоличествоТипов = 1 Тогда
				Результат.РазвернутьДеревоПомеченныхНаУдаление = Истина;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ru = 'Удаление не выполнено по причине:'") + Символы.ПС + КраткоеПредставлениеОшибки;
	ПодробныйТекстОшибки = Кратко + Символы.ПС + Символы.ПС + ПодробноеПредставлениеОшибки;
	Элементы.НадписьТекстОшибки.Заголовок = Кратко;
	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВозниклаОшибка;
	ВидимостьДоступность(ЭтотОбъект);
КонецПроцедуры
...
&НаКлиенте
Процедура УскоритьУдалениеПомеченныхПриИзменении(Элемент)
