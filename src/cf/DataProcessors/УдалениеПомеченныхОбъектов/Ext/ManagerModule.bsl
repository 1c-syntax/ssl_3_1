...
Функция ОбработатьПакет(ПараметрыУдаления, Пакет)
	...
	Буфер.Вставить("Элементы", Новый Массив);
	Буфер.Вставить("Код", КодыРезультатаУдаления().Успешно);
	Буфер.Вставить("РезультатУдаления");
	Для Каждого Элемент Из Пакет Цикл
		Буфер.Элементы.Добавить(Элемент);
		...
		Попытка
			...
			Для каждого УдаляемыйСсылка Из Буфер.Элементы Цикл
				...
				РезультатУдаления = УдалитьЭлементПакета(
										...
										НельзяУдалятьОсновнойОбъект);
			
				Если РезультатУдаления.Код <> КодыРезультатаУдаления().Успешно И НЕ НельзяУдалятьОсновнойОбъект Тогда
		...
		Буфер.Элементы.Очистить();
		Буфер.РезультатУдаления = Неопределено;
		Буфер.Код = КодыРезультатаУдаления().Успешно;
...
// 	                     - ИнформацияОбОшибке
//
Функция УдалитьЭлементПакета(ПараметрыУдаления, Элемент, МестаИспользования, НеУдалятьЭлемент)
	...
	Если НЕ НеУдалятьЭлемент Тогда
		Попытка
			Блокировка = Новый БлокировкаДанных();
