...
Процедура ОтразитьНеУдаленные(ПараметрыУдаления, НеУдаленные)
	...
	Для Каждого Элемент Из НеУдаленные Цикл
		...
		// @skip-check query-in-loop - Легкий запрос для штучной проверки удаления объекта.
...
Функция ОбработатьУдаляемыеОбъекты(ПараметрыУдаления, УдаляемыеОбъекты, СведенияОМетаданных)
	...
	Для Каждого ЭлементОчереди Из УдаляемыеОбъекты Цикл
		...
		Если Пакет.Количество() >= ПараметрыУдаления.РазмерПакета
			...
			ОбъединитьРезультатУдаления(РезультатыУдаления, РезультатОбработкиПакета);
			Пакет.Очистить();
		КонецЕсли;
	КонецЦикла;
	
	РезультатУдаленияИзНеобработанногоПакета = РезультатУдаленияИзНеобработанногоПакета(ПараметрыУдаления, ОбъектыСЗапретомУдаления);
	ОбъединитьРезультатУдаления(РезультатыУдаления, РезультатУдаленияИзНеобработанногоПакета);
...
Процедура ОчиститьСсылкиВМестеИспользования(МестоИспользования, УдаляемыеОбъекты)
	...
	Попытка
		...
		Элемент = Блокировка.Добавить(МестоИспользования.Метаданные().ПолноеИмя());
		Элемент.УстановитьЗначение("Ссылка", МестоИспользования);
		Блокировка.Заблокировать();
		
		МестоИспользованияОбъект = МестоИспользования.ПолучитьОбъект();
		Если МестоИспользованияОбъект = Неопределено Тогда
			ЗафиксироватьТранзакцию();
			Возврат;
		КонецЕсли;
		
		ОчиститьСсылкиВКоллекцииРеквизитов(МестоИспользованияОбъект, МетаданныеОбъекта.СтандартныеРеквизиты, УдаляемыеОбъекты);
		ОчиститьСсылкиВКоллекцииРеквизитов(МестоИспользованияОбъект, МетаданныеОбъекта.Реквизиты, УдаляемыеОбъекты);
		
		Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
			ОчиститьСсылкиВТабличнойЧасти(МестоИспользованияОбъект[ТабличнаяЧасть.Имя], ТабличнаяЧасть.Реквизиты, УдаляемыеОбъекты);
		КонецЦикла;
		...
		Для Каждого ТабличнаяЧасть Из СтандартныеТабличныеЧасти Цикл
			ОчиститьСсылкиВТабличнойЧасти(МестоИспользованияОбъект[ТабличнаяЧасть.Имя], ТабличнаяЧасть.СтандартныеРеквизиты, УдаляемыеОбъекты);
		КонецЦикла;
		
		МестоИспользованияОбъект.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых");
		МестоИспользованияОбъект.ОбменДанными.Загрузка = Истина;
		МестоИспользованияОбъект.Записать();

		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ЗаписатьПредупреждение(НСтр("ru = 'Не удалось очистить ссылки на удаляемый объект в ""%1"" по причине:'"), 
			МестоИспользования, ИнформацияОбОшибке());
	КонецПопытки;
	
КонецПроцедуры

Процедура ОчиститьСсылкиВТабличнойЧасти(МестоИспользованияОбъект, КоллекцияРеквизитов, УдаляемыеОбъекты)
	Для Каждого СтрокаТЧ Из МестоИспользованияОбъект Цикл
		ОчиститьСсылкиВКоллекцииРеквизитов(СтрокаТЧ, КоллекцияРеквизитов, УдаляемыеОбъекты);
	КонецЦикла;
КонецПроцедуры

Процедура ОчиститьСсылкиВКоллекцииРеквизитов(МестоИспользованияОбъект, КоллекцияРеквизитов, УдаляемыеОбъекты)
	Для Каждого Реквизит Из КоллекцияРеквизитов Цикл
		Если УдаляемыеОбъекты.Найти(МестоИспользованияОбъект[Реквизит.Имя]) <> Неопределено Тогда
			МестоИспользованияОбъект[Реквизит.Имя] = Реквизит.Тип.ПривестиЗначение(Неопределено);
		КонецЕсли;
	КонецЦикла;	
КонецПроцедуры
...
//
Процедура ОбъединитьРезультатУдаления(РезультатыУдаления, РезультатОбработки)
	
	РезультатыУдаления.НеудаленныеОбъекты = ОбъединениеТаблиц(РезультатыУдаления.НеудаленныеОбъекты, РезультатОбработки.НеудаленныеОбъекты);
	РезультатыУдаления.МестаИспользования = ОбъединениеТаблиц(РезультатыУдаления.МестаИспользования, РезультатОбработки.МестаИспользования);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РезультатыУдаления.УдаленныеОбъекты, РезультатОбработки.УдаленныеОбъекты);
...
Функция ОбъектУдален(УдаляемыйСсылка)
	...
	Если Не ЗначениеЗаполнено(УдаляемыйСсылка) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|ИЗ
		|	&ПолноеИмяОбъектаМетаданных КАК УдаляемыйОбъект
		|ГДЕ
		|	УдаляемыйОбъект.Ссылка = &Ссылка";
...
Функция ОбработатьПакетВОднойТранзакции(ПараметрыУдаления, Пакет)
	...
	Попытка
		Для Каждого ЭлементПакета Из Пакет Цикл
			...
			УдаляемыйЭлемент = ЭлементПакета.УдаляемыйСсылка;
			ПредставлениеЭлемента = ОбщегоНазначения.ПредметСтрокой(УдаляемыйЭлемент);
			
			Блокировка = Новый БлокировкаДанных();
			Элемент = Блокировка.Добавить(УдаляемыйЭлемент.Метаданные().ПолноеИмя());
			Элемент.УстановитьЗначение("Ссылка", УдаляемыйЭлемент);
			Блокировка.Заблокировать();
			
			Объект = УдаляемыйЭлемент.ПолучитьОбъект();
			Если Объект <> Неопределено Тогда
				УдалитьОбъект(Объект);
				Результат.УдаленныеОбъекты.Добавить(УдаляемыйЭлемент);
			КонецЕсли;
		КонецЦикла;
	...
	Исключение
		...
		ЗаписатьПредупреждение(НСтр("ru = 'Не удалось удалить ""%1"" по причине:'"),
			ПредставлениеЭлемента, ИнформацияОбОшибке());
	КонецПопытки;
...
Функция УдалитьЭлементПакета(ПараметрыУдаления, Элемент, МестаИспользования, НеудаленныеЭлементы)
	...
	ПредставлениеЭлемента = ОбщегоНазначения.ПредметСтрокой(Элемент.УдаляемыйСсылка);
	Попытка
		Блокировка = Новый БлокировкаДанных();
	...
	Исключение
		...
		ЗаписатьПредупреждение(НСтр("ru = 'Не удалось удалить ""%1"" по причине:'"), ПредставлениеЭлемента, ИнформацияОбОшибке);
...
Функция ОставшиесяМестаИспользованияУдаляемогоОбъекта(ПараметрыУдаления, МестаИспользования, СсылкаИсключение = Неопределено)
	...
	Для Каждого СведенияОМестеИспользования Из МестаИспользования Цикл
		...
		Если ЭтоСсылка Тогда
			// @skip-check query-in-loop - Легкий запрос для штучной проверки удаления объекта.
			ОбъектУдален = ОбъектУдален(СведенияОМестеИспользования.МестоИспользования);
			Если (НЕ ОбъектУдален ИЛИ СведенияОМестеИспользования.МестоИспользования = СсылкаИсключение)
...
Функция СсылкиТолькоНаЭлементыВнутриПакета(Пакет, МестаИспользования)
	...
	Для Каждого Элемент Из Пакет Цикл
		...
		МестаИспользованияЭлемента = МестаИспользования.Скопировать(
			Новый Структура("УдаляемыйСсылка", Элемент.УдаляемыйСсылка), "МестоИспользования");
		// @skip-check query-in-loop - Легкий запрос для штучной проверки удаления объекта.
		Если ИсточникаПодмножествоПриемника(Пакет, МестаИспользованияЭлемента, "УдаляемыйСсылка", "МестоИспользования")
			ИЛИ МестаИспользования.Количество() = 0 Тогда
...
Процедура УдалитьПомеченныеОбъектыМонопольно(УдаляемыеОбъекты, ПараметрыВыполнения)
	...
	Для Индекс = -(УдаляемыеОбъекты.Количество() - 1) По 0 Цикл
		...
		// @skip-check query-in-loop - Легкий запрос для штучной проверки удаления объекта.
		Если НЕ ОбъектУдален(УдаляемыйОбъект) И НЕ СведенияОбУдаляемыхОбъектах[УдаляемыйОбъект].ПометкаУдаления Тогда
	...
	Пока УдаляемыеОбъекты.Количество() > 0 Цикл
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписатьПредупреждение(НСтр("ru = 'Не удалось удалить элементы по причине:'"), Неопределено, ИнформацияОбОшибке);
...
Процедура ЗаписатьПредупреждение(Знач Описание, Знач ПредставлениеЭлемента, Знач ИнформацияОбОшибке)
	...
	КонецЕсли;
	ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, ПредставлениеЭлемента) 
		+ Символы.ПС + ТекстДляЖурнала;
