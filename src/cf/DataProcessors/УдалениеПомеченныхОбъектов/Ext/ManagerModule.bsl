...
Функция ОбработатьУдаляемыеОбъекты(ПараметрыУдаления, УдаляемыеОбъекты, СведенияОМетаданных)
	...
	Для Каждого ЭлементОчереди Из УдаляемыеОбъекты Цикл
		Если Пакет.Найти(ЭлементОчереди.УдаляемыйСсылка,"УдаляемыйСсылка") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Если РезультатыУдаления.НеудаленныеОбъекты.Количество() > 0 Тогда
...
Функция ЭлементОчередиВПакет(ЭлементОчереди, СведенияОМетаданных, Исключения)
	...
	Если НЕ ОбъектУдален(ЭлементОчереди.УдаляемыйСсылка) 
		...
		ИначеЕсли РазрешеноУдалениеЭлемента(ДочерниеПодчиненныеЭлементы) Тогда
			...
			ДополнитьПакетДочернимиИПодчиненнымиЭлементами(Пакет, ДочерниеПодчиненныеЭлементы, Исключения);
		Иначе
...
Функция ОбработатьПакет(ПараметрыУдаления, Пакет)
	...
	Если ЗначениеЗаполнено(ПараметрыУдаления.ИдентификаторЗадания) Тогда
	...
	Для Каждого Элемент Из Пакет Цикл
		РезультатУдаления = УдалитьЭлементПакета(ПараметрыУдаления,
			Элемент,
			МестаИспользования.Скопировать(Новый Структура("УдаляемыйСсылка", Элемент.УдаляемыйСсылка)));
		ОтразитьРезультатУдаленияЭлементаПакета(ПараметрыУдаления, РезультатОбработкиПакета, Элемент, РезультатУдаления);
	КонецЦикла;
...
Функция ДочерниеЭлементы(ОсновнойЭлемент, РезультатПакета)
	...
	Для Каждого СтрокаДерева Из РезультатПакета.Строки Цикл
		...
		Иначе
			...
			Результат = ОбъединениеТаблиц(Результат, ЗаписьРезультата.ДочерниеЭлементы);
		КонецЕсли;
...
Процедура ОтразитьРезультатУдаленияЭлементаПакета(ПараметрыУдаления, РезультатОбработкиПакета, Элемент, РезультатУдаления)
	...
	Иначе
		РезультатОбработкиПакета.УдаленныеОбъекты.Добавить(Элемент.УдаляемыйСсылка);
...
Функция УдалитьЭлементПакета(ПараметрыУдаления, Элемент, МестаИспользования)
	...
	Попытка
		Блокировка = Новый БлокировкаДанных();
		ЭлементБлокировки = Блокировка.Добавить(Объект.Метаданные().ПолноеИмя());
		ЭлементБлокировки.УстановитьЗначение("Ссылка", Объект.Ссылка);
		Блокировка.Заблокировать();
	
		УдалитьОбъект(Объект);
		ОставшиесяМестаИспользования = ОставшиесяМестаИспользованияУдаляемогоОбъекта(ПараметрыУдаления,
			МестаИспользования);
...
Функция ПараметрыУдаления(УдаляемыеОбъекты, Параметры)
	...
	ПараметрыОбработки.Вставить("ИдентификаторЗадания", Новый УникальныйИдентификатор);
...
Процедура УдалитьПомеченныеОбъектыМонопольно(УдаляемыеОбъекты, ПараметрыВыполнения)
	...
	Для Сч = -(УдаляемыеОбъекты.Количество() - 1) По 0 Цикл
		...
		Для Каждого ПодчиненныйОбъект Из ДочерниеПодчиненныеОбъекты Цикл
			Если ПодчиненныйОбъект.ПометкаУдаления Тогда
...
Процедура ОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Процент = (ПараметрыВыполнения.КоличествоОбработанных)
...
Процедура ОтправитьСтатистику(РежимУдаления, ВремяВыполнения, КоличествоУдаляемыхЭлементов)
	...
	ЦентрМониторингаМенеджер = ОбщегоНазначения.ОбщийМодуль("ЦентрМониторинга");
	ЦентрМониторингаМенеджер.ЗаписатьОперациюБизнесСтатистики(
