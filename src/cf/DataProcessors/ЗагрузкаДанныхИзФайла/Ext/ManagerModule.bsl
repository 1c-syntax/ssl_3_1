...
Процедура СоздатьСписокСправочниковДляЗагрузки(СписокСправочниковДляЗагрузки) Экспорт
	...
	Для каждого ОбъектМетаданныхДляВывода Из Метаданные.Справочники Цикл
		...
		Если ФункциональныеОпции.Получить(ПолноеИмя) = Ложь 
			Или ИнформацияОСправочниках.Найти(ПолноеИмя, "ПолноеИмя") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	
		Если ЗагрузкаДанныхИзФайлаДопустима(ОбъектМетаданныхДляВывода) Тогда
			
			Представление = ОбъектМетаданныхДляВывода.Представление();

			СтрокаТаблицы = ИнформацияОСправочниках.Найти(Представление, "Представление");
			Если СтрокаТаблицы <> Неопределено Тогда
				СтрокаТаблицы.Представление = Представление + " (" + СтрокаТаблицы.ПолноеИмя +")";
				Представление               = Представление + " (" + ПолноеИмя +")";
			КонецЕсли;
			
			ИнформацияОСправочнике = ИнформацияОСправочниках.Добавить();
			ИнформацияОСправочнике.Представление = Представление;
			...
			ИнформацияОТипеЗагрузки.Вставить("ПолноеИмяОбъектаМетаданных", ПолноеИмя);
