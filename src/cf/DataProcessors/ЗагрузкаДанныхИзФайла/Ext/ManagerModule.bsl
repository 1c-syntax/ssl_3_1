...
Процедура СоздатьСписокСправочниковДляЗагрузки(СписокСправочниковДляЗагрузки) Экспорт
	...
	ЗагружаемыеСправочники = ИнформацияОСправочниках();
	...
	Для каждого ОбъектМетаданныхДляВывода Из Метаданные.Справочники Цикл
		...
		Если ЗагрузкаДанныхИзФайлаДопустима(ОбъектМетаданныхДляВывода) Тогда
			...
			ИнформацияОСправочнике = ЗагружаемыеСправочники.Добавить();
			ИнформацияОСправочнике.Представление = ОбъектМетаданныхДляВывода.Представление();
			ИнформацияОСправочнике.ПолноеИмя     = ПолноеИмя;
			
		КонецЕсли;
		
	КонецЦикла;
	
	ИнтеграцияПодсистемБСП.ПриОпределенииСправочниковДляЗагрузкиДанных(ЗагружаемыеСправочники);
	ЗагрузкаДанныхИзФайлаПереопределяемый.ПриОпределенииСправочниковДляЗагрузкиДанных(ЗагружаемыеСправочники);
	
	ИнформацияОСправочниках = РасширеннаяИнформацияОСправочниках();
	
	Для каждого ИнформацияОСправочнике Из ЗагружаемыеСправочники Цикл
		
		СтрокаТаблицы = ИнформацияОСправочниках.Найти(ИнформацияОСправочнике.ПолноеИмя, "ПолноеИмя");
		Если СтрокаТаблицы = Неопределено Тогда
			
			СтрокаТаблицы = ИнформацияОСправочниках.Добавить();
			СтрокаТаблицы.ПолноеИмя      = ИнформацияОСправочнике.ПолноеИмя;
			
			СтрокаТаблицыПоПредставлению = ИнформацияОСправочниках.Найти(ИнформацияОСправочнике.Представление, "Представление");
			Если СтрокаТаблицыПоПредставлению <> Неопределено Тогда
				СтрокаТаблицыПоПредставлению.Представление = СтрокаТаблицыПоПредставлению.Представление + " (" + СтрокаТаблицыПоПредставлению.ПолноеИмя + ")";
				СтрокаТаблицы.Представление                = ИнформацияОСправочнике.Представление + " (" + ИнформацияОСправочнике.ПолноеИмя + ")";
			Иначе
				СтрокаТаблицы.Представление                = ИнформацияОСправочнике.Представление;
			КонецЕсли;
			
		ИначеЕсли СтрокаТаблицы.ПрикладнаяЗагрузка Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаТаблицы.ПрикладнаяЗагрузка = ИнформацияОСправочнике.ПрикладнаяЗагрузка; 
		
		ИнформацияОТипеЗагрузки     = НоваяИнформацияОТипеЗагрузки();
		ИнформацияОТипеЗагрузки.Тип = ?(ИнформацияОСправочнике.ПрикладнаяЗагрузка, "ПрикладнаяЗагрузка", "УниверсальнаяЗагрузка");
		ИнформацияОТипеЗагрузки.Вставить("ПолноеИмяОбъектаМетаданных", ИнформацияОСправочнике.ПолноеИмя);
		СтрокаТаблицы.ИнформацияОТипеЗагрузки = ИнформацияОТипеЗагрузки;
		
	КонецЦикла;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		МодульДополнительныеОтчетыИОбработки = ОбщегоНазначения.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
		ТаблицаКоманд = МодульДополнительныеОтчетыИОбработки.ДополнительныеКомандыЗагрузкиДанныхИзФайла();
		Для Каждого СтрокаТаблицы Из ТаблицаКоманд Цикл
			
			ИнформацияОТипеЗагрузки = НоваяИнформацияОТипеЗагрузки();
			ИнформацияОТипеЗагрузки.Тип = "ВнешняяЗагрузка";
			ИнформацияОТипеЗагрузки.ПолноеИмяОбъектаМетаданных  = СтрокаТаблицы.Модификатор;
			ИнформацияОТипеЗагрузки.Ссылка = СтрокаТаблицы.Ссылка;
			ИнформацияОТипеЗагрузки.Идентификатор = СтрокаТаблицы.Идентификатор;
			ИнформацияОТипеЗагрузки.Представление = СтрокаТаблицы.Представление;
			
			ИнформацияОСправочнике = ИнформацияОСправочниках.Добавить();
			ИнформацияОСправочнике.ПолноеИмя = ОбъектМетаданныхДляВывода.ПолноеИмя();
			ИнформацияОСправочнике.ИнформацияОТипеЗагрузки = ИнформацияОТипеЗагрузки;
			ИнформацияОСправочнике.Представление = СтрокаТаблицы.Представление;
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого ИнформацияОСправочнике Из ИнформацияОСправочниках Цикл 
		...
		Представление = "";
		Если ИнформацияОСправочнике.ПрикладнаяЗагрузка И ПустаяСтрока(ИнформацияОСправочнике.Представление) Тогда
		...
		Если ПустаяСтрока(ИнформацияОСправочнике.ИнформацияОТипеЗагрузки.Представление) Тогда
			ИнформацияОСправочнике.ИнформацияОТипеЗагрузки.Представление = Представление;
		КонецЕсли;
...
//   * ПрикладнаяЗагрузка - Булево
// 
Функция ИнформацияОСправочниках()
	
	ТипСтрока = Новый ОписаниеТипов("Строка");
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("ПолноеИмя",          ТипСтрока);
	Результат.Колонки.Добавить("Представление",      ТипСтрока);
	Результат.Колонки.Добавить("ПрикладнаяЗагрузка", Новый ОписаниеТипов("Булево"));
...
//   * Представление           - Строка
//   * ПрикладнаяЗагрузка      - Булево
//   * ИнформацияОТипеЗагрузки - Структура
// 
Функция РасширеннаяИнформацияОСправочниках()
	
	ТипСтруктура = Новый ОписаниеТипов("Структура");
	
	Результат = ИнформацияОСправочниках();
	Результат.Колонки.Добавить("ИнформацияОТипеЗагрузки", ТипСтруктура);
...
//   * Ссылка - Неопределено, СправочникСсылка
//   * Идентификатор - Строка 
//   * Представление - Строка
// 
Функция НоваяИнформацияОТипеЗагрузки() Экспорт
	
	ИнформацияОТипеЗагрузки = Новый Структура;
	ИнформацияОТипеЗагрузки.Вставить("Тип",                        "");
	ИнформацияОТипеЗагрузки.Вставить("ПолноеИмяОбъектаМетаданных", "");
	ИнформацияОТипеЗагрузки.Вставить("Ссылка",                     Неопределено);
	ИнформацияОТипеЗагрузки.Вставить("Идентификатор",              "");
	ИнформацияОТипеЗагрузки.Вставить("Представление",              "");
	
	Возврат ИнформацияОТипеЗагрузки;
...
Процедура СопоставитьЗначениеКолонкиАвто(ТаблицаСопоставления, ИмяКолонки) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаСопоставления Цикл 
		...
		Значение = ДокументПоПредставлению(СтрокаТаблицы[ИмяКолонки], Типы);
		НоваяСтрока = ЗначенияДляСопоставления.Добавить();
		НоваяСтрока.ЗначениеПоиска =?(Значение = Неопределено, СокрЛП(СтрокаТаблицы[ИмяКолонки]), Значение);
		НоваяСтрока.Ключ = ТаблицаСопоставления.Индекс(СтрокаТаблицы);
				...
				Иначе
					Если ТипДанных = Тип("Булево") Тогда
						...
						Если ЗначениеЯчейки = "1" 
						   Или СтрСравнить(ЗначениеЯчейки, ЗагрузкаДанныхИзФайла.ПредставлениеТекстаДаДляБулево()) = 0
					...
					ИначеЕсли ТипДанных = Тип("Дата") Тогда
						ДанныеЯчейки = ОбщегоНазначенияКлиентСервер.СтрокаВДату(Ячейка.Текст);
					Иначе
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(Тип("Булево")) Тогда 
			ОписаниеТипаКолонки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Флаг, %1 или 1 / Нет или 0'"), ЗагрузкаДанныхИзФайла.ПредставлениеТекстаДаДляБулево());
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				Если Не ИнформацияОбОшибке.ЯвляетсяОшибкойКатегории(КатегорияОшибки.ОшибкаДоступаКЛокальномуФайлу) Тогда
					ВызватьИсключение;
				КонецЕсли;
				...
				УточненныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1
					           |
					           |Убедитесь в корректности данных в файле.'"), Уточнение.Текст);
			...
			ОбластьВывода = ШаблонСДанными.ПолучитьОбласть(НомерСтрокиСШапкойТаблицы + 1, 1, НомерСтрокиСШапкойТаблицы + 1, 
				ЗагруженныеДанные.Колонки.Количество());
...
Функция СоздатьCSVВоВременномХранилище(ИнформацияПоКолонкам, АдресВоВременномХранилище = "") Экспорт
	
	ОбщегоНазначенияКлиентСервер.Проверить(
		ИнформацияПоКолонкам <> Неопределено И ИнформацияПоКолонкам.Количество() > 0,
		НСтр("ru = 'Не переданы колонки для формирования CSV'"));
	
	Поток = Новый ПотокВПамяти();
	
	Попытка
		ЗаписатьCSVЗаголовокВПоток(Поток, ИнформацияПоКолонкам);
	...
	Если ДвоичныеДанные = Неопределено Или ДвоичныеДанные.Размер() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось сформировать CSV файл. Получен пустой результат'");
	КонецЕсли;
	
	МаксимальныйРазмерФайла = МаксимальныйРазмерФайлаШаблона();
	Если ДвоичныеДанные.Размер() > МаксимальныйРазмерФайла Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Размер CSV файла (%1 байт) превышает допустимый предел (%2 байт)'"),
			Формат(ДвоичныеДанные.Размер(), "ЧГ=0"), Формат(МаксимальныйРазмерФайла, "ЧГ=0"));
	КонецЕсли;
...
Процедура ЗаписатьCSVЗаголовокВПоток(Поток, ИнформацияПоКолонкам)
	...
	Для Каждого Колонка Из ИнформацияПоКолонкам Цикл
		ПредставлениеЭкранированное = ЭкранироватьЗначениеCSV(Колонка.ПредставлениеКолонки);
		МассивКолонок.Добавить(ПредставлениеЭкранированное);
	КонецЦикла;
	
	Заголовок = СтрСоединить(МассивКолонок, ";");
	Запись.ЗаписатьСтроку(Заголовок);
...
Функция ЭкранироватьЗначениеCSV(Значение)
	...
	ЗначениеСтрока = СокрЛП(Строка(Значение));
	
	Если СтрНайти(ЗначениеСтрока, ";") > 0 
		Или СтрНайти(ЗначениеСтрока, """") > 0
		Или СтрНайти(ЗначениеСтрока, Символы.ПС) > 0 
		Или СтрНайти(ЗначениеСтрока, Символы.ВК) > 0 Тогда
...
Функция МаксимальныйРазмерФайлаШаблона() Экспорт
	Возврат 200 * 1024 * 1024;
КонецФункции
...
Процедура ЗаписатьСопоставленныеДанные(ПараметрыВыгрузки, АдресХранилища) Экспорт
	...
	НаборРеквизитов = Новый Соответствие;
	...
	Для каждого СтрокаТаблицы Из СопоставленныеДанные Цикл
		...
		Попытка
			...
			Для каждого Колонка Из ИнформацияПоКолонкам Цикл 
				Если Колонка.Видимость Тогда
					...
					ИначеЕсли СтрНачинаетсяС(Колонка.ИмяКолонки, "ДополнительныйРеквизит_") Тогда
						ДобавитьЗначениеСвойства(НаборРеквизитов, Свойства, "ДополнительныйРеквизит_", Колонка.ИмяКолонки,  СтрокаТаблицы[Колонка.ИмяКолонки]);
			...
			Если НаборРеквизитов.Количество() > 0 И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
				МодульУправлениеСвойствами = ОбщегоНазначения.ОбщийМодуль("УправлениеСвойствами");
				МодульУправлениеСвойствами.УстановитьСвойстваУОбъекта(ЭлементСправочника, НаборРеквизитов);
			КонецЕсли;
...
Процедура ДобавитьЗначениеСвойства(НаборСвойств, Свойства, Префикс, ИмяКолонки, Значение)
	...
	Если Свойство <> Неопределено Тогда
		Если ТипЗнч(НаборСвойств) = Тип("ТаблицаЗначений") Тогда
			НоваяСтрокаСвойств = НаборСвойств.Добавить();
			НоваяСтрокаСвойств.Свойство = Свойство.Ссылка;
			НоваяСтрокаСвойств.Значение = Значение;
		Иначе
			НаборСвойств.Вставить(Свойство.Ссылка, Значение);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СформироватьОтчетОЗагрузкеФон", Истина);
	Методы.Вставить("ЗаписатьСопоставленныеДанные", Истина);
	Методы.Вставить("ЗаполнитьТаблицуСопоставленияДаннымиИзШаблонаФон", Истина);
	Методы.Вставить("ЗагрузитьФайлВТаблицу", Истина);
