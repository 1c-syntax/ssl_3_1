...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТипЗагрузки = "ТабличнаяЧасть" И ЗначениеЗаполнено(Параметры.ПолноеИмяТабличнойЧасти) Тогда 
		СписокНеоднозначностей = Новый Массив; // Массив из СправочникСсылка
		...
		ИначеЕсли МассивОбъекта[0] = "Обработка" Тогда
			МенеджерОбъекта = Обработки[МассивОбъекта[1]];
		Иначе
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	Высота = Параметры.СтрокаИзТаблицы.Количество() + СписокНеоднозначностей.Количество() + 7;
...
Процедура ЭлементыСправочникаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ТекущиеДанныеВыбора = Элементы.ЭлементыСправочника.ТекущиеДанные; // ЛюбаяСсылка
	Закрыть(ТекущиеДанныеВыбора.Ссылка);
КонецПроцедуры
