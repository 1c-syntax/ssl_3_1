...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		...
		ИнформацияПоКолонкамЗагружаемыхДанных = РеквизитФормыВЗначение("ИнформацияПоКолонкам");

		СохраненнаяИнформациейПоКолонкам  = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЗагрузкаДанныхИзФайла", ИмяОбъектаСопоставления,, ИмяПользователя()); // ТаблицаЗначений
		
		Если ТипЗнч(СохраненнаяИнформациейПоКолонкам) = Тип("ТаблицаЗначений") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СохраненнаяИнформациейПоКолонкам, ИнформацияПоКолонкамЗагружаемыхДанных);
		КонецЕсли;
		...
		Если Параметры.КолонкиМакета <> Неопределено Тогда
			ОпределитьДинамическийМакет(ИнформацияПоКолонкамЗагружаемыхДанных, Параметры.КолонкиМакета);
		...
		Иначе
			Обработки.ЗагрузкаДанныхИзФайла.ОпределитьИнформацияПоКолонкам(ПараметрыЗагрузкиДанных, ИнформацияПоКолонкамЗагружаемыхДанных);
			...
			КонецЕсли;
		КонецЕсли;
		ЗначениеВРеквизитФормы(ИнформацияПоКолонкамЗагружаемыхДанных, "ИнформацияПоКолонкам");
...
Процедура ЗакрытьФормуИВернутьМассивСсылок()
	...
	Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектСопоставления) Тогда
			МассивСсылок.Добавить(СтрокаТаблицы.ОбъектСопоставления);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ВыборСправочникаДляЗагрузки Тогда
		Если Элементы.ВидЗагрузкиДанных.ТекущиеДанные = Неопределено Тогда
			Возврат;
		КонецЕсли;
...
// Параметры:
//  ОписаниеСтрокиВыбора - см. Обработки.ЗагрузкаДанныхИзФайла.НоваяИнформацияОТипеЗагрузки
//
&НаСервере
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	Если ТипЗнч(ОписаниеСтрокиВыбора) = Тип("Структура") Тогда
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеПроверки()
	...
	Иначе
		...
		ДлительнаяОперация = СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка();
...
Процедура ЗаписатьЗагружаемыеДанныеКлиент()
	...
	Иначе
		...
		ДлительнаяОперация = ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка();
		Если ДлительнаяОперация.Статус = "Выполняется" Тогда
			Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ДлительныеОперации;
		КонецЕсли;
...
Процедура ФоновоеЗаданиеОтчетНаКлиенте(НеВыводитьОкноОжидания = Истина)
	...
	ДлительнаяОперация = СформироватьОтчетОЗагрузке(ФильтрОтчет, НеВыводитьОкноОжидания);
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Или Статистика.Неоднозначных > 0 Тогда
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ru = 'Строки, для которых в приложении имеется несколько вариантов, будут пропущены: %1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ru = 'Строки, для которых в приложении нет соответствующих данных, будут пропущены: %1'");
...
Процедура СопоставитьДанныеПрикладнаяЗагрузка(ТаблицаСопоставленияДанныхСервер)
	...
	Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанныхСервер Цикл 
		Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектСопоставления) Тогда 
			СтрокаТаблицы.РезультатСопоставленияСтроки = ЗагрузкаДанныхИзФайлаКлиентСервер.СтатусСопоставлен();
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		Для каждого Колонка Из ТабличнаяЧасть.Колонки Цикл
			Если Колонка.Имя <> "Идентификатор" Тогда
				НоваяСтрока[ПрефиксТабличнойЧасти + Колонка.Имя] = СтрокаТаблицы[Колонка.Имя];
			КонецЕсли;
			
			Если ЗначениеЗаполнено(КолонкиОбязательныеДляЗаполнения.Получить(Колонка.Имя))
				И ЗаполненыВсеОбязательныеКолонки
				И НЕ ЗначениеЗаполнено(СтрокаТаблицы[Колонка.Имя]) Тогда
		...
		Отбор = Новый Структура("Идентификатор", СтрокаТаблицы.Идентификатор); 
		...
		Если Строки.Количество() > 0 Тогда 
			...
			Для каждого Колонка Из ЗагружаемыеДанные.Колонки Цикл
				Если Колонка.Имя <> "Идентификатор" И Колонка.Имя <> "РезультатСопоставленияСтроки" И Колонка.Имя <> "ОписаниеОшибки" Тогда
					НоваяСтрока["ФЛ_" + Колонка.Имя] = СтрокаТаблицы[Колонка.Имя];
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		Для Каждого Колонка Из ТаблицаДляТЧ.Колонки Цикл
			...
			ИначеЕсли Колонка.Имя <> "РезультатСопоставленияСтроки" И Колонка.Имя <> "ОписаниеОшибки" Тогда
				НоваяСтрока[Колонка.Имя] = СтрокаТаблицы[ПрефиксТабличнойЧасти + Колонка.Имя];
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция НезаполненныеОбязательныеКолонки()
	...
	ВысотаШапки = Шапка.ВысотаТаблицы;
	ШиринаТаблицы = Шапка.ШиринаТаблицы;
	...
	Для НомерСтроки = ВысотаШапки + 1 По ШаблонСДанными.ВысотаТаблицы Цикл
		...
		Для НомерКолонки = 1 По ШиринаТаблицы Цикл
			Если ЗначениеЗаполнено(ШаблонСДанными.Область(НомерСтроки, НомерКолонки, НомерСтроки, НомерКолонки).Текст) Тогда
				СтрокаПустая = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого Колонка Из ОбязательныеКолонки Цикл
			Если Не ЗначениеЗаполнено(ШаблонСДанными.Область(НомерСтроки, Колонка.Позиция, НомерСтроки, Колонка.Позиция).Текст) Тогда
				ПредставлениеКолонки = ?(ЗначениеЗаполнено(Колонка.Синоним), Колонка.Синоним, Колонка.ПредставлениеКолонки);
				Если НазванияКолонокБезДанных.Найти(ПредставлениеКолонки) = Неопределено Тогда
					НазванияКолонокБезДанных.Добавить(ПредставлениеКолонки);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Функция ОбластьЗаголовкаШаблонаТаблицы(Шаблон)
	...
	Если МетаданныеОбластьЗаголовокТаблицы = Неопределено Тогда 
		...
		Если КолонкиИмеютГруппировку Тогда 
			ОбластьЗаголовокТаблицы = Шаблон.ПолучитьОбласть("R1:R2");
		Иначе
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		ТекстПодсказки = НСтр("ru = 'Для загрузки данных необходимо сохранить бланк в файл для заполнения в другом приложении.
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			Если РасширениеФайла = "csv" Тогда
				...
				АдресВоВременномХранилище = СоздатьCSVВоВременномХранилище(ИнформацияПоКолонкам);
				...
				ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, АдресВоВременномХранилище,
					ПутьКФайлу, ПараметрыСохраненияФайла);
...
Процедура ЗагрузитьДанныеИзФайлаВШаблон(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ДлительнаяОперация = ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение);
		НастройкиОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		НастройкиОжидания.ВыводитьОкноОжидания = Ложь;
...
Процедура ПослеВыбораРасширенияФайла(Результат, Параметр) Экспорт
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		СохранитьШаблонВоВременноеХранилище(Результат, ШаблонСДанными, ИнформацияПоКолонкам, АдресВоВременномХранилище);
		ПараметрыСохраненияФайла = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
		ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, АдресВоВременномХранилище,
...
&НаСервереБезКонтекста
Процедура СохранитьШаблонВоВременноеХранилище(РасширениеФайла, Знач ШаблонСДанными, Знач ИнформацияПоКолонкам, АдресВоВременномХранилище)

	РасширениеФайла = НРег(СокрЛП(РасширениеФайла));
	
	ДопустимыеРасширения = Новый Соответствие;
	ДопустимыеРасширения.Вставить("csv",  Истина);
	ДопустимыеРасширения.Вставить("xlsx", Истина);
	ДопустимыеРасширения.Вставить("xls",  Истина);
	ДопустимыеРасширения.Вставить("ods",  Истина);
	ДопустимыеРасширения.Вставить("mxl",  Истина);
	
	Если ДопустимыеРасширения.Получить(РасширениеФайла) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сохранение шаблона отменено. Недопустимое расширение файла: %1'"), РасширениеФайла);
	КонецЕсли;
	...
	Попытка
		...
		ИначеЕсли РасширениеФайла = "xls" Тогда
			...
			Если Не ФайлНаДиске.Существует() Тогда
				ВызватьИсключение НСтр("ru='Сохранение шаблона отменено. Не удалось создать временный файл'");
			КонецЕсли;
		...
		Иначе
			...
			Попытка
				...
				Если РасширениеФайла = "xlsx" Тогда
					ШаблонСДанными.Записать(Поток, ТипФайлаТабличногоДокумента.XLSX);
				ИначеЕсли РасширениеФайла = "ods" Тогда
					ШаблонСДанными.Записать(Поток, ТипФайлаТабличногоДокумента.ODS);
				Иначе
					ШаблонСДанными.Записать(Поток, ТипФайлаТабличногоДокумента.MXL);
				КонецЕсли;
			...
			Исключение
				Попытка
					Поток.Закрыть();
				Исключение
					ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(ЗагрузкаДанныхИзФайла.СобытиеЖурналаРегистрации(),
		...
		Если ДвоичныеДанные = Неопределено Или ДвоичныеДанные.Размер() = 0 Тогда
			ВызватьИсключение НСтр("ru='Сохранение шаблона отменено. Получен пустой файл.'");
		КонецЕсли;

		Если ДвоичныеДанные.Размер() > Обработки.ЗагрузкаДанныхИзФайла.МаксимальныйРазмерФайлаШаблона() Тогда
			ВызватьИсключение НСтр("ru='Сохранение шаблона отменено из-за превышения установленного ограничения по размеру файла'");
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СоздатьCSVВоВременномХранилище(Знач ИнформацияПоКолонкам)
	Возврат Обработки.ЗагрузкаДанныхИзФайла.СоздатьCSVВоВременномХранилище(ИнформацияПоКолонкам);
КонецФункции
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыПроцедуры.Вставить("ШаблонСДанными", ШаблонСДанными);
	ПараметрыПроцедуры.Вставить("ИмяВременногоФайла", ИмяВременногоФайла);
	ПараметрыПроцедуры.Вставить("ИнформацияПоКолонкам", РеквизитФормыВЗначение("ИнформацияПоКолонкам"));
	...
	ПараметрыВыполнения.УточнениеОшибки =
		НСтр("ru = 'Не удалось загрузить данные по причине:'");
	
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайла.ЗагрузитьФайлВТаблицу",
...
//
&НаКлиенте
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонСДанными = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	СопоставитьЗагружаемыеДанные();
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("ШаблонСДанными", ШаблонСДанными);
	ПараметрыПроцедуры.Вставить("ТаблицаСопоставления", РеквизитФормыВЗначение("ТаблицаСопоставленияДанных"));
	ПараметрыПроцедуры.Вставить("ИнформацияПоКолонкам", РеквизитФормыВЗначение("ИнформацияПоКолонкам"));
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru = 'Заполнение таблицы сопоставления загруженными данными из файла.'");
	ПараметрыВыполнения.УточнениеОшибки =
		НСтр("ru = 'Не удалось сопоставить данные по причине:'");
...
//
&НаКлиенте
Процедура ПослеСопоставленияЗагружаемыхДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеСопоставленияНаСервере(Результат.АдресРезультата);
	ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент();
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("СопоставленныеДанные", РеквизитФормыВЗначение("ТаблицаСопоставленияДанных"));
	ПараметрыПроцедуры.Вставить("ПараметрыЗагрузки", ПараметрыЗагрузки);
	ПараметрыПроцедуры.Вставить("ИмяОбъектаСопоставления", ИмяОбъектаСопоставления);
	ПараметрыПроцедуры.Вставить("ИнформацияПоКолонкам", РеквизитФормыВЗначение("ИнформацияПоКолонкам"));
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Запись загруженных данных из файла'");
	ПараметрыВыполнения.УточнениеОшибки = НСтр("ru = 'Не удалось записать данные по причине:'");
...
//
&НаКлиенте
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(Результат, ДополнительныеПараметры) Экспорт
	...
	СсылкиНаОбъектыОповещения = Новый Массив;
	ЗаполнитьТаблицуСопоставленияИзВременногоХранилища(Результат.АдресРезультата, СсылкиНаОбъектыОповещения);
	ОповеститьФормыОбИзменении(СсылкиНаОбъектыОповещения);
	ФоновоеЗаданиеОтчетНаКлиенте();
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	ПараметрыПроцедуры = Новый Структура();
	ПараметрыПроцедуры.Вставить("ТаблицаОтчет", ТаблицаОтчет);
	ПараметрыПроцедуры.Вставить("ТипОтчета", ТипОтчета);
	ПараметрыПроцедуры.Вставить("СопоставленныеДанные", СопоставленныеДанные);
	ПараметрыПроцедуры.Вставить("ШаблонСДанными", ШаблонСДанными);
	ПараметрыПроцедуры.Вставить("ИмяОбъектаСопоставления", ИмяОбъектаСопоставления);
	ПараметрыПроцедуры.Вставить("РассчитыватьПроцентПрогресса", РассчитыватьПроцентПрогресса);
	ПараметрыПроцедуры.Вставить("ИнформацияПоКолонкам", ТаблицаИнформацияПоКолонкам);
...
//
&НаКлиенте
Процедура ПослеСозданияОтчета(Результат, ДополнительныеРезультаты) Экспорт
	...
	ПоказатьОтчет(Результат.АдресРезультата);
	ПодтверждениеЗакрытияФормы = Истина;
...
Процедура ПрогрессВыполнения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполняется"
	   И Результат.Прогресс <> Неопределено Тогда
		...
		ФоновоеЗаданиеПроцент = Результат.Прогресс.Процент;
	КонецЕсли;
