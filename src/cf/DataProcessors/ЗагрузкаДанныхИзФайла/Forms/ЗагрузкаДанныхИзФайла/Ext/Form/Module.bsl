...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		Если ПустаяСтрока(ШаблонСДаннымиОднаКолонка) Тогда
			ПоказатьПредупреждение(, (НСтр("ru = 'Для вставки сопоставленных данных в список, необходимо заполнить текстовое поле.'")));
			Возврат;
		КонецЕсли;
		
		СкопироватьОднуКолонкуВШаблонСДанными();
		
	ИначеЕсли ТаблицаСДаннымиПустая() Тогда
		
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ru = 'Для сопоставления и загрузки данных, необходимо заполнить таблицу.'")));
		Иначе
		...
		Если Элемент.Пометка Тогда
			...
			Если Колонка <> Неопределено Тогда
				...
				Если СтрНачинаетсяС(Колонка.ИмяКолонки, "КонтактнаяИнформация_") Тогда
					...
					КонецЕсли;
					ОтборГде = ОтборГде + " И СправочникСопоставления.Представление <> """"";
					РазделительИ = " И ";
					Продолжить;
				...
				ИначеЕсли СтрНачинаетсяС(Колонка.ИмяКолонки, "ДополнительныйРеквизит_") Тогда
					...
					Если ОбъектаТипаКолонки <> Неопределено Тогда
						ОтборГде = ОтборГде + " И СправочникСопоставления.Значение <> ЗНАЧЕНИЕ(" +  ОбъектаТипаКолонки.ПолноеИмя() + ".ПустаяСсылка)";
					КонецЕсли;
					РазделительИ = " И ";
					Продолжить;
				КонецЕсли;
				...
				Если ТипКолонки = Тип("Строка") Тогда
					Если Колонка.ТипКолонки.КвалификаторыСтроки.Длина = 0 Тогда
						СтрокаУсловие = СтрокаУсловие + РазделительИ + "ВЫРАЗИТЬ(СправочникСопоставления." + ИмяКолонкиСправочника 
							+ " КАК Строка(500)) = ВЫРАЗИТЬ(ТаблицаСопоставления." + Колонка.ИмяКолонки + " КАК  Строка(500))";
						ОтборГде = ОтборГде + " И СправочникСопоставления." + ИмяКолонкиСправочника + " <> """"";
					Иначе
						...
						ОтборГде = ОтборГде + РазделительИ + " СправочникСопоставления." + ИмяКолонкиСправочника + " <> """"";
					КонецЕсли;
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахАвто(ОписаниеТипаОбъектаСопоставления)
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		НовыйЭлемент = Элементы.Добавить("ТаблицаСопоставленияДанных_" + Колонка.Имя, Тип("ПолеФормы"), Элементы.ТаблицаСопоставленияДанных); // РасширениеПоляФормыДляПоляВвода
