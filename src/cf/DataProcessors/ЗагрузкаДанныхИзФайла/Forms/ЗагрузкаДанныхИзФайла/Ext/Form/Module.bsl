		...
		Если Элемент.Пометка Тогда
			...
			Если Колонка <> Неопределено Тогда
				...
				Если СтрНачинаетсяС(Колонка.ИмяКолонки, "КонтактнаяИнформация_") Тогда
					...
					ШаблонСтрокаУсловие = "ВЫРАЗИТЬ(СправочникСопоставления.Представление КАК Строка(500)) = ВЫРАЗИТЬ(ТаблицаСопоставления.%1 КАК Строка(500))";
					СтрокаУсловие = СтрокаУсловие + РазделительИ
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтрокаУсловие, Колонка.ИмяКолонки);
