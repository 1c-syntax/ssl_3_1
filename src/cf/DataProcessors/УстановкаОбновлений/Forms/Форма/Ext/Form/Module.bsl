...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления() Тогда
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для обновления приложения. Обратитесь к администратору.'"),
	...
	СтатусНевыполненныхОбработчиков = ОбновлениеИнформационнойБазыСлужебный.СтатусНевыполненныхОбработчиков();
	Элементы.НадписьНезавершенныеОбработчики.Видимость = ЗначениеЗаполнено(СтатусНевыполненныхОбработчиков);
	...
	Иначе 
		...
		ПоказатьУстановленныеИсправления = Истина;
	КонецЕсли;
	...
	Если ЭтоВебКлиент Или ЭтоПодчиненныйУзелРИБ Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не Результат.Поддерживается
		...
		Элементы.ГруппаСтраницыОсновное.ТекущаяСтраница = Элементы.СтраницаНедоступно;
		Элементы.ФормаЗакрыть.Видимость = Истина;
		Элементы.НадписьНедоступно.Заголовок = Результат.ОписаниеОшибки;
		Возврат;
	КонецЕсли;
...
#Область СтраницаФайлОбновления
...
#Область СтраницаПослеУстановкиИсправлений
...
#Область СтраницаВыборРежимаОбновленияФайл
...
#Область СтраницаВыборРежимаОбновленияСервер
...
Процедура НадписьОтложенныеОбработчикиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОбновлениеИнформационнойБазыКлиент.ПоказатьРезультатыОбновления();
...
&НаКлиенте
Процедура ПерезапуститьПрограммуПриИзменении(Элемент)
	
	Если ПерезапуститьПрограмму Тогда
		Элементы.КнопкаДалее.Заголовок = НСтр("ru = 'Перезапустить'");
	Иначе
...
&НаКлиенте
Процедура ДекорацияУведомлятьПользователейОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	СписокВыбора = Новый СписокЗначений;
	СписокВыбора.Добавить("5", НСтр("ru = '5 минут'"));
	СписокВыбора.Добавить("10", НСтр("ru = '10 минут'"));
	СписокВыбора.Добавить("15", НСтр("ru = '15 минут'"));
	СписокВыбора.Добавить("30", НСтр("ru = '30 минут'"));
	
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораИнтервалаУведомленияПользователей", ЭтотОбъект);
	ПоказатьВыборИзМеню(Оповещение, СписокВыбора, Элементы.ДекорацияДляПодменю);
КонецПроцедуры

&НаКлиенте
Процедура УведомлятьПользователейПриИзменении(Элемент)
	Элементы.ДекорацияУведомлятьПользователей.Доступность = УведомлятьПользователей;
КонецПроцедуры
...
Процедура ПередОткрытиемСтраницы(Знач НоваяСтраница = Неопределено)
	...
	ИначеЕсли НоваяСтраница = Страницы.ВыборРежимаОбновленияСервер Тогда
		...
		Элементы.ПанельУведомлятьПользователей.Видимость = (Объект.РежимОбновления = 2);
...
#Иначе
	...
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Или ЭтоПодчиненныйУзелРИБ Тогда 
...
Процедура УстановитьОбновление()
	...
	Если УведомлятьПользователей Тогда
		ПараметрыОбновления.Вставить("ИнтервалУведомленияПользователей", ИнтервалУведомления);
	Иначе
		ПараметрыОбновления.Вставить("ИнтервалУведомленияПользователей", 0);
	КонецЕсли;
...
Процедура ОбработатьНажатиеКнопкиДалее()
	...
	Иначе
		Закрыть();
		Если ПоказатьУстановленныеИсправления Тогда
			ОбновлениеКонфигурацииКлиент.ПоказатьУстановленныеИсправления();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиСоСтраницыФайлаОбновления()
	...
	ПользователиСлужебныйКлиент.ПоказатьПредупреждениеБезопасности(Обработчик,
		ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередВыборомФайлаОбновления);
...
Процедура УстановитьИсправленияНаСервере(ПомещенныеФайлы)
	...
	ПараметрыУстановкиИсправлений = ОбновлениеКонфигурации.ПараметрыУстановкиИсправлений();
	ПараметрыУстановкиИсправлений.ВФоне = Ложь;
	ПараметрыУстановкиИсправлений.ОбновитьПараметрыРаботыРасширений = Истина;
	ПараметрыУстановкиИсправлений.ПроверятьПрименимостьПоМанифесту = Истина;

	Результат = ОбновлениеКонфигурации.УстановкаИУдалениеИсправлений(Исправления, ПараметрыУстановкиИсправлений);
	...
	Если ЕстьОшибки Тогда
		Если УстановленоИсправлений > 0 Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установлено исправлений: %1 из %2, изменения вступят в силу после перезапуска приложения'"),
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, ЭтоПереходДалее) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Не удалось установить обновление приложения, т.к. введены неправильные имя или пароль администратора 
...
#Область ПланированиеОбновленияВУказанноеВремя
...
&НаКлиенте
Процедура ПослеВыбораИнтервалаУведомленияПользователей(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ШаблонЗаголовка = НСтр("ru = 'Уведомлять пользователей о планируемом обновлении за <a href=""Ссылка"">%1 минут</a>'");
	Элементы.ДекорацияУведомлятьПользователей.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ШаблонЗаголовка, Результат.Значение);
	
#Если Не ВебКлиент Тогда
	ИнтервалУведомления = XMLЗначение(Тип("Число"), Результат.Значение);
#КонецЕсли
КонецПроцедуры
