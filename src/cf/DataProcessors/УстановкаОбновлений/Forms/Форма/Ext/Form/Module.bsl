...
Процедура ЗаполнитьПанельИнформации()
	...
	Информация = МодульКонтрольВеденияУчетаСлужебный.ИнформацияОПоследнейПроверкеВеденияУчета();
	Если Информация.ПредупреждатьОНеобходимостиПовторнойПроверки Тогда
		...
		Если ЗначениеЗаполнено(Информация.ДатаПоследнейПроверки) Тогда
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подсказка, Информация.ДатаПоследнейПроверки);
		Иначе
			Подсказка = "";
		КонецЕсли;
