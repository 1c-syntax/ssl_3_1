...
Процедура ПриОткрытии(Отказ)
	...
	ИнициализироватьФайлыИсправлений(ФайлыИсправленийСтрокой);
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ФайлОбновления Тогда
		...
		ИначеЕсли НуженФайлОбновления = 1 Тогда
			...
			Если ПустаяСтрока(ВыбранныеФайлы) И Не ЭтоТестКлиент Тогда
...
Процедура ВыбратьФайлОбновления()
	...
	Если ЭтоБазоваяВерсия И Не ДоступнаУстановкаПатчейВБазовойВерсии Тогда 
		...
		ФайловаяСистемаКлиент.ЗагрузитьФайл(
			Новый ОписаниеОповещения("ПослеВыбораФайлаОбновления", ЭтотОбъект),
			ПараметрыЗагрузки);
...
#Иначе
	...
	Иначе
		...
		Если ЭтоБазоваяВерсия И Не ДоступнаУстановкаПатчейВБазовойВерсии Тогда
			ОписаниеФильтраДиалога.Вставить(0, НСтр("ru = 'Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu'"));
		Иначе
...
//
&НаКлиенте
Процедура ПослеВыбораФайлаОбновления(ОписаниеВыбранногоФайла, ДополнительныеПараметры) Экспорт 
	
	Если ТипЗнч(ОписаниеВыбранногоФайла) <> Тип("Структура") Тогда 
		Возврат;
	КонецЕсли;
	
	ОписаниеИмениФайла = СтрРазделить(ОписаниеВыбранногоФайла.Имя, ПолучитьРазделительПути());
	ОписаниеВыбранногоФайла.Вставить("ИмяФайла", ОписаниеИмениФайла[ОписаниеИмениФайла.ВГраница()]);
	
	ОписаниеВыбранныхФайлов = Новый Массив;
	ОписаниеВыбранныхФайлов.Добавить(ОписаниеВыбранногоФайла);
