...
Функция ОшибкиЗаполненияТелефона()
	...
	НомерТелефонаТолькоЦифры = УправлениеКонтактнойИнформациейКлиент.ОставитьТолькоЦифрыВСтроке(ПолныйНомерТелефона);
	...
	Если ИспользоватьДополнительныеПроверки Тогда
		МодульРаботаСАдресамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСАдресамиКлиент");
		МодульРаботаСАдресамиКлиент.ПроверитьКорректностьКодовСтраныИГорода(КодСтраны, КодГорода, НомерТелефона, СписокОшибок);
	КонецЕсли;
