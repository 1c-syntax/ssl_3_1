...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазрешитьВводАдресаВСвободнойФорме = УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.addressType);
...
Процедура ПослеЗакрытияФормыИстории(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВводНовогоАдреса Тогда
		...
		Если Не РазрешитьВводАдресаВСвободнойФорме Тогда
			НаселенныйПунктДетально.addressType = УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес();
		КонецЕсли;
		ОчиститьПоляАдресаНаФорме();
	Иначе
		Отбор = Новый Структура("Вид",  ОписаниеВидаКонтактнойИнформации(ЭтотОбъект).Ссылка);
		НайденныеСтроки = КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Отбор);
...
&НаКлиенте
Процедура ОчиститьПоляАдресаНаФорме()
	Улица                           = "";
	ДополнительнаяИнформация        = "";
	Город                           = "";
	Регион                          = "";
	ПочтовыйИндекс                  = "";
	ПредставлениеИностранногоАдреса = "";
	ПредставлениеАдреса             = "";
КонецПроцедуры
