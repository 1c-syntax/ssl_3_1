...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначения.ЭтоLinuxКлиент()
		...
		Если ЗначениеЗаполнено(КаталогПлатформы) Тогда
			...
			УточнениеПерезапуск = Символы.ПС + Символы.ПС + НСтр("ru = 'Перезапустите приложение на подходящей версии платформы (кнопка <b>Перезапустить</b>).'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТаблицаВерсийПлатформы(Платформы, ВсеНерекомендуемыеВерсии)
	...
	Для Каждого Платформа Из Платформы Цикл 
		...
		Если СтрРазделить(Платформа.ИмяБезРасширения, ".").Количество() <> 3 Тогда
			Продолжить;
		КонецЕсли;
		
		// Проверка установленных платформ у пользователя. Должен быть указан минимальный поддерживаемый режим совместимости.
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(Платформа.ИмяБезРасширения, "8.3.24") < 0 Тогда
			Продолжить;
		КонецЕсли;
...
Функция НайденнаяПлатформа(ТаблицаВерсийПлатформы, ТекущаяВерсия)
	...
	Пока ПодходящаяСборка = Неопределено Цикл
		...
		Если НомерВерсии < 24 Тогда // 8.3.24
			Возврат Неопределено;
		КонецЕсли;
