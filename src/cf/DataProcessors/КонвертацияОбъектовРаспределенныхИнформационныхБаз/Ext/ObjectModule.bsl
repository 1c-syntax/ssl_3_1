...
Процедура ЗаписатьСсылкуПредопределенныхДанных(Данные)
	...
	ИначеЕсли Объект.ИмяПредопределенныхДанных <> Данные.ИмяПредопределенныхДанных Тогда
		...
		Попытка
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить(ОбщегоНазначения.ИмяТаблицыПоСсылке(Объект.Ссылка));
		...
		Исключение
			...
			Событие = НСтр("ru = 'Предопределенные элементы.Запись'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область ОписанияОшибокКонтекстаВыполнения
