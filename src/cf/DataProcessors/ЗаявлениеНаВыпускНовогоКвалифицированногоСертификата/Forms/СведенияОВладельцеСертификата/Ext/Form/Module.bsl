...
Процедура СохранитьИзмененияИЗакрыть()
	...
	Если Не Модифицированность Тогда
		Закрыть(Новый Структура("СведенияОВладельце, Обновить", СведенияОВладельце, Ложь));
	ИначеЕсли СохранитьИзмененияНаСервере() Тогда
		Модифицированность = Ложь;
		Закрыть(Новый Структура("СведенияОВладельце, Обновить", СведенияОВладельце, Истина));
	КонецЕсли;
