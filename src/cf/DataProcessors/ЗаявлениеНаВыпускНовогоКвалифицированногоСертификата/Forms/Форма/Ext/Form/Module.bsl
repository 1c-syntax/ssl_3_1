...
Процедура ИнструкцияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Иначе
		ТекстПредупреждения =
			...
			|5. Копия* страхового свидетельства обязательного пенсионного страхования (СНИЛС) представителя организации, указанного для выпуска сертификата.
			|6. Копия* документа, подтверждающего полномочия руководителя, подписавшего заявление (протокол собрания учредителей, решение собственника, устав) или актуальная выписка из ЕГРЮЛ, заверенная налоговым органом.
			|
...
&НаСервере
Функция ТекущееЗначениеРеквизита(Реквизит, РеквизитТип)
...
Процедура ЗаполнитьРеквизитыОрганизации()
	...
	Организация = ТекущееЗначениеРеквизита(Реквизиты.Организация, ОрганизацияТип);
	УточнитьПустоеЗначение(Организация, ОрганизацияТип);
...
Процедура ПриИзмененииВладельцаСертификатаНаСервере(Загрузка = Ложь)
	...
	Реквизиты.Вставить("Организация", ТекущееЗначениеРеквизита(Организация, ОрганизацияТип));
	Реквизиты.Вставить("Сотрудник",   ТекущееЗначениеРеквизита(Сотрудник, СотрудникТип));
...
Функция ПроверитьЗаполнениеОсновныхРеквизитов()
	...
	Возврат Не ОбработкаМодуль.ПроверитьЗаполнениеРеквизитов(
...
Функция ПроверитьЗаполнениеДополнительныхРеквизитов()
	...
	Результат = Не ОбработкаМодуль.ПроверитьЗаполнениеРеквизитов(
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "Объект.Программа");
		Результат = Ложь;
	КонецЕсли;
...
Функция ОписаниеЗапросаНаКвалифицированныйСертификат(Поля, ТекстОшибки)
	...
	ТекстОшибки = НСтр("ru = 'Некорректный юридический адрес:'") + Символы.ПС;
	...
	Поля.Вставить("L", Лев(ПолеНаселенныйПункт, 255));
	...
	Поля.Вставить("Street", Лев(ПолеУлица, 255));
...
Функция ЗаявлениеXML(Билет)
	...
	ЗаписьXML.ЗаписатьАтрибут("ВерсПрог", "1С");
		...
		ЗаписьXMLЭлемент(ЗаписьXML, "ТелефонОсновной",
			Обработки.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.ТелефонДляОтправкиВСервис(ТелефонXML));
...
Процедура ПолучитьИмяНовогоКонтейнераКлюча(Оповещение)
	...
	// Дата используется только для уникальности имени, поэтому требуется время компьютера.
