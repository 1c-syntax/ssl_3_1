...
&НаКлиенте
Процедура ВводСведенийОВладельце(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Если Результат.Обновить Или Не ЗначениеЗаполнено(ВладелецСертификатаСтрокой) Тогда
		Сведения = Результат.СведенияОВладельце;
		Модифицированность = Истина;
		ДокументыПечатались = Ложь;
...
Функция ОписаниеЗапросаНаКвалифицированныйСертификат(Поля)
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный юридический адрес ""%1"": 
				|%2'"), ТекстОшибки);
...
Процедура СоздатьОбъектКриптографииПослеПодключенияКомпоненты(Результат, Контекст) Экспорт
	...
	Если Не Результат.Подключено Тогда
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
			...
			ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
		Иначе
			ТекстСообщения = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС 
...
Функция ИмяРегионаРФПоРекомендациямДляСКПЭП(КодРегиона)
	...
	Если ПустаяСтрока(КодРегиона) Тогда
		ВызватьИсключение НСтр("ru = 'Не указан регион РФ.'");
	КонецЕсли;
