...
Процедура НапечататьДокументы()
	...
	ЗаписатьЗаявлениеНаСервере(Документ);
...
&НаСервере
Процедура ЗаписатьЗаявлениеНаСервере(Знач Документ)
	
	ПотокЗаявлениеНаПодключение = Новый ПотокВПамяти();
	Документ.Записать(ПотокЗаявлениеНаПодключение, ТипФайлаТабличногоДокумента.PDF);
	
	ЧтениеПотока = Новый ЧтениеДанных(ПотокЗаявлениеНаПодключение);
	РезультатЧтения = ЧтениеПотока.Прочитать();
	
	ДвоичныеДанныеЗаявлениеНаПодключение = ПотокЗаявлениеНаПодключение.ЗакрытьИПолучитьДвоичныеДанные();
	АдресЗаявлениеНаПодключение = ПоместитьВоВременноеХранилище(ДвоичныеДанныеЗаявлениеНаПодключение, УникальныйИдентификатор);
...
Процедура СформироватьИОтправитьФайлыЗаявления(Результат, ДополнительныеПараметры) Экспорт
	...
	ОписаниеДанных.ОписаниеДляПакета.ТипСодержимого = "pdf";
	ИмяФайла = НовыйСжатыйУникальныйИдентификатор();
	ОписаниеДанных.ОписаниеДляПакета.ИмяФайла = ИмяФайла + ".bin";
	ОписаниеДанных.Представление.Представление = НСтр("ru = 'Заявление на подключение (файл сформирован)'");
	ОписаниеДанных.Представление.Значение = Новый ОписаниеОповещения(
		"ОткрытьФайлОбработкаОповещения", ЭтотОбъект, Новый Структура("АдресФайла, ИмяФайла", АдресЗаявлениеНаПодключение, ИмяФайла + ".pdf"));
