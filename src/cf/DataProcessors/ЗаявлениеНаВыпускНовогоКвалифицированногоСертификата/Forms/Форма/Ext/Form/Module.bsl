...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.СертификатСсылка) Тогда
		КлючНаТокене = Параметры.НаТокене;
	...
	Элементы.ОчиститьСНИЛС.Видимость = Ложь;
	...
	Если ЗначениеЗаполнено(Параметры.СертификатСсылка) Тогда
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ДанныеЗаявления.СертификатОснование) Тогда
				ПродлениеСертификатаСтороннегоУЦ = СертификатВыпущенСтороннимУЦ(ДанныеЗаявления.СертификатОснование);
	...
	// Новое заявление по кнопке "Перевыпустить..."
	ИначеЕсли ЗначениеЗаполнено(Параметры.СертификатОснование) Тогда
		ПродлениеСертификатаСтороннегоУЦ = СертификатВыпущенСтороннимУЦ(Параметры.СертификатОснование);
		СкопироватьЗаявление(Параметры.СертификатОснование);
		ПриИзмененииОрганизацииНаСервере(Истина);
	// Новое заявление по кнопке "Выпустить сертификат..."
	Иначе
		ДокументыПартнерЭтоИП = Ложь;
		
		Объект.Добавил = Пользователи.ТекущийПользователь();
		Объект.Пользователь = Объект.Добавил;
		ПользовательТаблицы = Объект.Пользователи.Добавить();
		ПользовательТаблицы.Пользователь = Объект.Добавил;
	...
	ПодсказкаОбслуживающейОрганизации = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'Узнать контакты партнера можно <a href = ""%1"">на сайте</a>.'"), СсылкаНаКонтактыПартнеров());
	ПодсказкаДляПрограммы = СтроковыеФункции.ФорматированнаяСтрока(
	НСтр("ru = 'Для создания запроса на сертификат используется приложение электронной подписи и шифрования <b>%1</b> или <b>%2</b> или активный ключевой носитель (<b>Рутокен</b> или <b>JaCarta</b>).
		|По вопросам работы приложения обратитесь к его документации или воспользуйтесь <a href=""СсылкаНаСтатьюИТС"">дополнительной инструкцией на сайте 1С</a>.
		|Для работы с <b>Рутокеном</b> скачайте и установите <a href=""СсылкаНаДрайверРутокена"">библиотеку с сайта производителя</a> и вставьте токен в компьютер.
		|Для работы с <b>JaCarta</b> скачайте и установите <a href=""СсылкаНаДрайверJaCarta"">библиотеку с сайта производителя</a> и вставьте токен в компьютер.
		|
		|<b>Ключ электронной подписи</b> - это секретная информация, которая сохраняется на компьютер, флешку, устройство или другой носитель информации и используется в дальнейшем для создания электронных подписей.
		|
	...
	Элементы.ДокументыПартнерРасширеннаяПодсказка.Заголовок = ПодсказкаОбслуживающейОрганизации;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ТолькоПросмотрЗаявления Тогда
		...
		Если ДанныеЗаявления.СостояниеЗаявления = ПредопределенноеЗначение("Перечисление.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено") Тогда
			ОтобразитьПоискПриложений(Истина);
...
Процедура ЭлектронноеПродлениеПриИзменении(Элемент)
	...
	ОчиститьСообщения();
	ПриПотереГотовности(ЭтотОбъект);
	ПодключитьОбработчикОжидания("ЭлектронноеПродлениеПриИзмененииОбработчикОжидания", 0.1, Истина);
...
Процедура СертификатОснованиеПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаявления.СертификатОснование) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьДокументОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ВыбратьДокументОбработкаНавигационнойСсылкиПродолжение(Элемент, НавигационнаяСсылкаФорматированнойСтроки);
	
КонецПроцедуры

&НаКлиенте
Асинх Процедура ВыбратьДокументОбработкаНавигационнойСсылкиПродолжение(Элемент, НавигационнаяСсылкаФорматированнойСтроки)
...
&НаКлиенте
Процедура ПредставлениеТарифаПриИзменении(Элемент)
	
	Если ПустаяСтрока(ДанныеЗаявления.ПредставлениеТарифа) Тогда
		Если Тарифы.Количество() > 0 Тогда
			СвойстваТарифа = Тарифы[0];
		Иначе
			СвойстваТарифа = СвойстваБазовогоТарифа();
			БазовыйТариф = Тарифы.Добавить();
			ЗаполнитьЗначенияСвойств(БазовыйТариф, СвойстваТарифа);
		КонецЕсли;
		ДанныеЗаявления.КодТарифа			= СвойстваТарифа.Код;
		ДанныеЗаявления.ПредставлениеТарифа = СвойстваТарифа.Представление;
		ДанныеЗаявления.ОписаниеТарифа		= СвойстваТарифа.Описание;
	КонецЕсли;
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("Представление", ДанныеЗаявления.ПредставлениеТарифа);
	ВыбранныйТариф = Тарифы.НайтиСтроки(ПараметрыОтбора)[0];
	ДанныеЗаявления.КодТарифа = ВыбранныйТариф.Код;
	ДанныеЗаявления.ОписаниеТарифа = ВыбранныйТариф.Описание;
	
	Элементы.ПредставлениеТарифаРасширеннаяПодсказка.Заголовок = ПодробноеОписаниеТарифа();
...
#Если ВебКлиент Тогда
	ПодключитьОбработчикОжидания("ЗаполнитьПрограммуПослеИзмененияТарифа", 0.3, Истина);
#Иначе
	ПодключитьОбработчикОжидания("ЗаполнитьПрограммуПослеИзмененияТарифа", 0.1, Истина);
#КонецЕсли
...
&НаКлиенте
Процедура ДекорацияСертификатыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(
		Неопределено,
		?(НавигационнаяСсылкаФорматированнойСтроки = "Сертификат", АдресСертификата, АдресКорневогоСертификата),
		?(НавигационнаяСсылкаФорматированнойСтроки = "Сертификат", ИмяФайлаБезРасширения(), НСтр("ru = 'Корневой сертификат'")));
...
Процедура ПрограммаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Строка") Тогда
		...
		Найдено = СписокТокенов.НайтиСтроки(Новый Структура("ПрограммаИдентификатор", ВыбранноеЗначение));
		Если Найдено.Количество() > 0 Тогда
			
			Если ЗначениеЗаполнено(Найдено[0].Ошибка) Тогда
				ТекстОшибки = Найдено[0].Ошибка;
				
				ПриПотереГотовности(ЭтотОбъект);
				ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(НСтр("ru = 'Выбор токена'"), "",
					Новый Структура("ОписаниеОшибки", ТекстОшибки), Новый Структура);
				
				Возврат;
			КонецЕсли;
			
			Если ПрограммаИдентификатор <> ВыбранноеЗначение Тогда
				КонтейнерКлючаИмя = "";
				ПриПотереГотовности(ЭтотОбъект);
			КонецЕсли;
			...
			Объект.Программа = Неопределено;
			ПрограммаИдентификатор = ВыбранноеЗначение;
			ПрограммаИмя = Найдено[0].ПрограммаИмя;
			ПрограммаТип = Найдено[0].ПрограммаТип;
			ПредставлениеПрограммы = Найдено[0].Представление;
			УстановитьВидимостьПрограммы();
			КлючНаТокене = Истина;
			ПодключитьОбработчикОжидания("ПослеВыбораПрограммыОбработчикОжидания", 0.1, Истина);
			Возврат;
...
Процедура ПрограммаРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Если НавигационнаяСсылка = "СсылкаНаДрайверРутокена" Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://www.rutoken.ru/support/download/pkcs/");
	ИначеЕсли НавигационнаяСсылка = "СсылкаНаДрайверJaCarta" Тогда	
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://aladdin-rd.ru/support/downloads/jacarta/");
	Иначе
...
&НаКлиенте
Процедура ДекорацияОбновитьСписокПриложенийНажатие(Элемент)
	ПодключитьОбработчикОжидания("ПриИзмененииСоставаИлиНастроекПрограмм", 0.1, Истина);
КонецПроцедуры
...
Процедура ОбновитьСостояниеОбработкиЗаявления(Команда)
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеЗаявления.СостояниеОбработкиЗаявления) Тогда
		ПоказатьПредупреждение(Неопределено, ДанныеЗаявления.СостояниеОбработкиЗаявления);
	КонецЕсли;
...
Процедура УстановитьСертификат(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) И Не ЗначениеЗаполнено(ПрограммаИдентификатор) Тогда
		ТекстОшибки = НСтр("ru = 'Приложение или токен: Поле не заполнено.'");
...
Процедура ПослеЗаписи()
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПослеДобавленияВСправочникСертификатовЭлектроннойПодписи(Объект.Ссылка, Контекст);
...
Процедура ОбновитьДанныеЗаявления()
	...
	Если ТипЗнч(Содержание) = Тип("Структура") Тогда
		...
		Если Содержание.Свойство("ЗаявлениеНаПодключение")
			И ТипЗнч(Содержание.ЗаявлениеНаПодключение) = Тип("ДвоичныеДанные") Тогда
			
			АдресЗаявлениеНаПодключение = ПоместитьВоВременноеХранилище(Содержание.ЗаявлениеНаПодключение, УникальныйИдентификатор);
		КонецЕсли;
...
Процедура СкопироватьЗаявление(СертификатОснование)
	...
	Если ТипЗнч(Содержание) = Тип("Структура") Тогда
		...
		Для Каждого Реквизит Из РеквизитыВладельцаСертификата() Цикл
			Если Реквизит.Ключ = "ВладелецАдресРегистрации" Тогда
				Продолжить;
			КонецЕсли;
			КопируемыеСвойства = КопируемыеСвойства + ", " + Реквизит.Ключ;
		КонецЦикла;
	...
	ДанныеЗаявленияМенеджерЗаписи.ОформленоВЭлектронномВиде = ПроверитьСертификатНаВозможностьЭлектронногоПродления(ДанныеСертификатаОснования, Ложь);
...
&НаСервере
Функция ПроверитьСертификатНаВозможностьЭлектронногоПродления(Знач ДанныеСертификата, Знач СообщатьОбОшибках = Истина)
	...
	Если ДанныеСертификата.СостояниеЗаявления = Неопределено И Не ПродлениеСертификатаСтороннегоУЦ Тогда
		
		// Если не нашли в ИБ учетную запись, то подготавливаем заявление как при продлении сертификата стороннего УЦ.
		ПродлениеСертификатаСтороннегоУЦ = Истина;
		УстановитьОтображениеРеквизитовЗависимыхОтВидаПродления();
		
	ИначеЕсли ДанныеСертификата.СостояниеЗаявления <> Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено
		И Не ПродлениеСертификатаСтороннегоУЦ Тогда
...
Процедура ЗаписатьЗаявление()
	...
	Если ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено Тогда
		...
		ОбновитьЗначение(Содержание, "КлючНаТокене");
	...
	ИначеЕсли ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Подготовлено Тогда
		...
		ОбновитьЗначение(Содержание, "ЗаявлениеНаПодключение", ПолучитьИзВременногоХранилища(АдресЗаявлениеНаПодключение));
		...
		ОбновитьЗначение(Содержание, "ВключатьЛицензиюКриптоПроВСертификат");
	...
	ДанныеЗаявленияМенеджерЗаписи.СодержаниеЗаявления = Новый ХранилищеЗначения(Содержание, Новый СжатиеДанных(9));
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ГруппаТарифы.ТолькоПросмотр = ТолькоПросмотрЭлементов;
	...
	Если ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отправлено Тогда
		Элементы.ДекорацияСостояние.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Отправлено. Уточнить срок отработки заявки можно у <a href = ""%1"">обслуживающей организации</a>.'"), СсылкаНаКонтактыПартнеров());
	Иначе
	...
	Элементы.СведенияОЗаявленииИВладельцеСертификата.Видимость = ЗаявлениеОтправлено И ЭлектронноеПродление И Не ПродлениеСертификатаСтороннегоУЦ;
	Элементы.СодержаниеСертификата.Видимость = СертификатПолучен;
	
	Элементы.ГруппаПаспорт.Видимость = Не ЗаявлениеОтправлено;
	Элементы.ГруппаСНИЛС.Видимость = Не ЗаявлениеОтправлено;
	...
	Элементы.ГруппаЛицензияКриптоПро.Видимость = ВыбранаПрограммаКриптоПро(ПрограммаИдентификатор, СписокПрограмм, Объект.Программа);
	
	Элементы.ДекорацияСертификаты.Видимость = СертификатПолучен;
	
	УстановитьЗаголовокДекорацияТекстЗаявления(ЗаявлениеОтправлено, ЭлектронноеПродление);
...
Процедура УстановитьОтображениеРеквизитовЗависимыхОтВидаПродления()
	...
	Элементы.ГруппаБумажноеПродление.Видимость = Не ЭлектронноеПродление Или ПродлениеСертификатаСтороннегоУЦ;
	Элементы.ДекорацияПередачаКомплекта.Видимость = Не ЭлектронноеПродление Или ПродлениеСертификатаСтороннегоУЦ;
	Элементы.ГруппаЭлектронноеПродление.Видимость = ЭлектронноеПродление;
	Элементы.ГруппаСНИЛС.Видимость = ЭлектронноеПродление И ПродлениеСертификатаСтороннегоУЦ;
...
&НаКлиентеНаСервереБезКонтекста
Функция СсылкаНаКонтактыПартнеров()
	
	Возврат "https://its.1c.ru/db/aboutitsnew/content/23/hdoc?order[comment]=%D0%97%D0%B0%D0%BA%D0%B0%D0%B7%20%D0%BD%D0%B0%201%D0%A1:%D0%9F%D0%BE%D0%B4%D0%BF%D0%B8%D1%81%D1%8C";
...
Процедура ЗаполнитьРеквизитыОрганизации(Загрузка = Ложь)
	...
	Если ОрганизацияТип = Неопределено Или ЭтоФизическоеЛицо Тогда
		Если ЭлектроннаяПодпись.ОбщиеНастройки().ОрганизацияИспользуется Тогда
...
Процедура ПриИзмененииВладельцаСертификатаНаСервере(Загрузка = Ложь)
	...
	Реквизиты.ВладелецАдресРегистрации = "";
...
Процедура ДозаполнитьПоПредыдущемуЗаявлениюИлиСертификату(ДанныеСертификата)
	...
	Если РеквизитыЗаявления <> Неопределено Тогда
		...
		СведенияОВладельце = РеквизитыВладельцаСертификата();
		СведенияОВладельце.Удалить("ВладелецАдресРегистрации");
		СведенияОВладельце.Вставить("ГражданствоОКСМКодАльфа3");
		СведенияОВладельце.Вставить("ГражданствоПредставление");
		СведенияОВладельце.Вставить("ВладелецТелефонXML");
		...
		Если ТипЗнч(ДвоичныеДанныеСертификата) = Тип("ДвоичныеДанные") Тогда
			...
			Если ЗначениеЗаполнено(СвойстваСубъектаСертификата.ЭлектроннаяПочта)
				...
				СвойстваСубъектаСертификата.ЭлектроннаяПочта = МодульУправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(
					СвойстваСубъектаСертификата.ЭлектроннаяПочта, ПараметрыВидаКонтактнойИнформации.Тип);
			КонецЕсли;
...
Функция ПроверитьЗаполнениеДополнительныхРеквизитов()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) И ПустаяСтрока(ПрограммаИдентификатор) Тогда
		
		Если Не ПустаяСтрока(ПредставлениеПрограммы) Тогда
			ПредставлениеПрограммы = "";
		КонецЕсли;
		
		ТекстОшибки = НСтр("ru = 'Приложение:'") + " " + ОбработкаМодуль.ТекстОшибкиПолеНеЗаполнено();
...
Функция ОписаниеЗапросаНаКвалифицированныйСертификат(Поля, ТекстОшибки)
	...
	// шаблон - "1.2.643.2.2.50.1.9.8791462.1882306.9191997.3049782.15111.41178" - Калуга-Астрал,
	// "1.2.643.2.2.50.1.9.48013.6498827.11094235.2141279.19877.56253" - 1С.
	
	Если ВключатьЛицензиюКриптоПроВСертификат И ВыбранаПрограммаКриптоПро(ПрограммаИдентификатор, СписокПрограмм, Объект.Программа) Тогда
		ШаблонСертификата = "1.2.643.2.2.50.1.9.48013.6498827.11094235.2141279.19877.56253";
	Иначе
...
&НаКлиенте
Функция ЗаполнитьПоляСертификата()
	...
	// Для физического лица заполнение не обязательно.
	
	Если ЗначениеЗаполнено(ВладелецАдресРегистрацииXML) И ЭтоФизическоеЛицо Или Не ЭтоФизическоеЛицо Тогда
...
Процедура НапечататьДокументы()
	...
	ПодготовленныеДокументы = ПодготовленныеДокументы();
	Если ПодготовленныеДокументы.ДокументыНаОтправку.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОбработатьДокументыНаОтправку(ПодготовленныеДокументы.ДокументыНаОтправку);
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		ПодготовленныеДокументы.ДокументНаПечать.Показать(НазваниеПечатнойФормы);
	...
	ПараметрыПечати = МодульУправлениеПечатьюКлиент.ПараметрыПечати();
	ПараметрыПечати.ВладелецФормы = ЭтотОбъект;
	ПараметрыПечати.ЗаголовокФормы = ?(ДанныеЗаявления.ОформленоВЭлектронномВиде,
		НСтр("ru = 'Просмотр заявления'"),
		НСтр("ru = 'Печать заявления'"));
	...
	ПечатнаяФорма.ТабличныйДокумент = ПодготовленныеДокументы.ДокументНаПечать;
	ПечатнаяФорма.ИмяФайлаПечатнойФормы = НазваниеПечатнойФормы;
	
	ОбластиОбъектов = Новый СписокЗначений;
	МодульУправлениеПечатьюКлиент.ПечатьДокументов(КоллекцияПечатныхФорм, ОбластиОбъектов, ПараметрыПечати);
...
&НаСервере
Процедура ОбработатьДокументыНаОтправку(Знач ДокументыНаОтправку)
	
	ИмяВременногоКаталога = ПолучитьИмяВременногоФайла() + ПолучитьРазделительПути();
	СоздатьКаталог(ИмяВременногоКаталога);
	Архив = Новый ЗаписьZipФайла;
	Для Каждого Документ Из ДокументыНаОтправку Цикл
		ИмяВременногоФайла = ИмяВременногоКаталога + НовыйСжатыйУникальныйИдентификатор() + ".pdf";
		Документ.Записать(ИмяВременногоФайла, ТипФайлаТабличногоДокумента.PDF);
		Архив.Добавить(ИмяВременногоФайла, РежимСохраненияПутейZIP.НеСохранятьПути);
	КонецЦикла;
	
	ДвоичныеДанныеАрхива = Архив.ПолучитьДвоичныеДанные("", "", МетодСжатияZIP.Сжатие, УровеньСжатияZIP.Максимальный);
	АдресЗаявлениеНаПодключение = ПоместитьВоВременноеХранилище(ДвоичныеДанныеАрхива, УникальныйИдентификатор);
...
&НаСервере
Функция ПодготовленныеДокументы()
	
	ДокументыНаОтправку = Новый Массив;
	ДокументНаПечать = Новый ТабличныйДокумент;
	Результат = Новый Структура("ДокументыНаОтправку, ДокументНаПечать");
	...
	Если Не ДокументыПоляСертификата.Свойство("S") Тогда
		Макет.УдалитьОбласть(Макет.Области.S, Смещать);
	КонецЕсли;
	...
	Если ЭтоИндивидуальныйПредприниматель Или ЭтоФизическоеЛицо Тогда
		...
		ДокументНаПечать.Вывести(ОбластьЗаявление);
		ОбластьСогласиеНаОбработкуПДн = Макет.ПолучитьОбласть("СогласиеНаОбработкуПДн");
		
		Если ДанныеЗаявления.ОформленоВЭлектронномВиде Тогда
			
			ДокументНаПечать.ВывестиГоризонтальныйРазделительСтраниц();
			ДокументНаПечать.Вывести(ОбластьСогласиеНаОбработкуПДн);
			
			Заявление = Новый ТабличныйДокумент;
			Заявление.Вывести(ОбластьЗаявление);
			ДокументыНаОтправку.Добавить(Заявление);
			
			СогласиеНаОбработкуПДн = Новый ТабличныйДокумент;
			СогласиеНаОбработкуПДн.Вывести(ОбластьСогласиеНаОбработкуПДн);
			ДокументыНаОтправку.Добавить(СогласиеНаОбработкуПДн);
			
		Иначе
			ДокументНаПечать.Вывести(ОбластьСогласиеНаОбработкуПДн);
			ДокументыНаОтправку.Добавить(ДокументНаПечать);
		КонецЕсли;
		
	Иначе
		
		ОбластьЗаявление = Макет.ПолучитьОбласть("ЗаявлениеШапка");
		ДокументНаПечать.Вывести(ОбластьЗаявление);
		
		Если Должность <> ДокументыРуководительДолжность
			Или СокрЛП(Фамилия + " " + Имя + " " + Отчество) <> СокрЛП(ДокументыРуководитель) Тогда
			
			ДругойПодписант = Макет.ПолучитьОбласть("ДругойПодписант");
			ДокументНаПечать.Вывести(ДругойПодписант);
			
		КонецЕсли;
		
		ОбластьЗаявлениеПодвал = Макет.ПолучитьОбласть("ЗаявлениеПодвал");
		ДокументНаПечать.Вывести(ОбластьЗаявлениеПодвал);
		
		ОбластьСогласиеНаОбработкуПДн = Макет.ПолучитьОбласть("СогласиеНаОбработкуПДн");
		
		Если ДанныеЗаявления.ОформленоВЭлектронномВиде Тогда
			
			Заявление = ДокументНаПечать;
			СогласиеНаОбработкуПДн = Новый ТабличныйДокумент;
	...
	Результат.ДокументыНаОтправку = ДокументыНаОтправку;
	Результат.ДокументНаПечать = ДокументНаПечать;
...
Функция ПечатьИнформацииЗаявления()
	...
	Если ВключатьЛицензиюКриптоПроВСертификат Тогда
		Макет.Параметры.ЛицензияКриптоПро = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Включить в сертификат лицензию %1'"),
			ЭлектроннаяПодписьКлиентСерверЛокализация.ИмяПрограммыКриптоПро());
	КонецЕсли;
...
Функция ПечатьИнформацииСертификата()
	...
	Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
		РасширенныеДанныеДляПечатиСертификата = Новый Структура("ТипИдентификации, СКЗИ, Заключение, СКЗИ_УЦ, ЗаключениеУЦ, Подпись, ИспользованиеКлюча");
		РасширенныеДанныеДляПечатиСертификата.ТипИдентификации = СертификатКриптографии.ДополненияСертификата.Получить(
			"1.2.643.100.114");
		РасширенныеДанныеДляПечатиСертификата.Подпись = СертификатКриптографии.ЗначениеПодписи;

		ДанныеУЦ = СертификатКриптографии.ДополненияСертификата.Получить("1.2.643.100.112");
		Если ДанныеУЦ <> Неопределено И ДанныеУЦ.Количество() = 4 Тогда
			РасширенныеДанныеДляПечатиСертификата.СКЗИ = ДанныеУЦ[0];
			РасширенныеДанныеДляПечатиСертификата.СКЗИ_УЦ = ДанныеУЦ[1];
			РасширенныеДанныеДляПечатиСертификата.Заключение = ДанныеУЦ[2];
			РасширенныеДанныеДляПечатиСертификата.ЗаключениеУЦ = ДанныеУЦ[3];
		КонецЕсли;
		
		ИспользованиеКлючаЧисло = СертификатКриптографии.ДополненияСертификата.Получить("2.5.29.15");
		
		Если ИспользованиеКлючаЧисло <> Неопределено Тогда
			Макет.Параметры.ОбластьИспользования = ЭлектроннаяПодписьСлужебный.ИспользованиеКлючаСтрокой(ИспользованиеКлючаЧисло);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СертификатКриптографии.УниверсальнаяДатаНачалаЗакрытогоКлюча) И ЗначениеЗаполнено(
			СертификатКриптографии.УниверсальнаяДатаОкончанияЗакрытогоКлюча) Тогда

			Макет.Параметры.ПериодДействияЗакрытогоКлюча = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 по %2'"), Формат(СертификатКриптографии.УниверсальнаяДатаНачалаЗакрытогоКлюча, НСтр(
				"ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss ""UTC""'''")), Формат(
				СертификатКриптографии.УниверсальнаяДатаОкончанияЗакрытогоКлюча, НСтр("ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss ""UTC""'''")));

		КонецЕсли;
		Макет.Параметры.Алгоритм = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПредставлениеАлгоритмаПодписи(
				СвойстваСертификата.АлгоритмОткрытогоКлюча, Истина, Истина);
	Иначе
		
		РасширенныеДанныеДляПечатиСертификата = ЭлектроннаяПодписьСлужебный.РасширенныеДанныеДляПечатиСертификата(
			ДанныеСертификата);

		Если ЗначениеЗаполнено(СвойстваСертификата.ДатаНачалаЗакрытогоКлюча) И ЗначениеЗаполнено(
			СвойстваСертификата.ДатаОкончанияЗакрытогоКлюча) Тогда

			Макет.Параметры.ПериодДействияЗакрытогоКлюча = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 по %2'"), Формат(СвойстваСертификата.ДатаНачалаЗакрытогоКлюча
					- ЭлектроннаяПодписьСлужебный.РазницаСУниверсальнымВременем(), НСтр(
					"ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss ""UTC""'''")), Формат(
					СвойстваСертификата.ДатаОкончанияЗакрытогоКлюча
					- ЭлектроннаяПодписьСлужебный.РазницаСУниверсальнымВременем(), НСтр(
					"ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss ""UTC""'''")));

		КонецЕсли;
		
		Если РасширенныеДанныеДляПечатиСертификата.ИспользованиеКлюча = "030203F8" Тогда
			Макет.Параметры.ОбластьИспользования = НСтр("ru = 'Цифровая подпись, Неотрекаемость, Шифрование ключей, Шифрование данных, Согласование ключей'");
		Иначе
	...
	Макет.Параметры.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 по %2'"), 
		Формат(СертификатКриптографии.ДатаНачала, НСтр("ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss ""UTC""'''")),
		Формат(СертификатКриптографии.ДатаОкончания, НСтр("ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss ""UTC""'''")));
...
&НаКлиенте
Асинх Процедура СоздатьКлючИЗапросНаСертификатНаТокене()
	
	Контекст = Новый Структура;
	
	Контекст.Вставить("ПоляСертификата", ЗаполнитьПоляСертификата());
	
	Найдено = СписокТокенов.НайтиСтроки(Новый Структура("ПрограммаИдентификатор", ПрограммаИдентификатор));
	Если Найдено.Количество() = 0 Тогда
		Ждать ЗаполнитьСписокУстройств();
	КонецЕсли;
	Найдено = СписокТокенов.НайтиСтроки(Новый Структура("ПрограммаИдентификатор", ПрограммаИдентификатор));
	Если Найдено.Количество() = 0 Тогда
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: %2.
			|%3'"), ПредставлениеПрограммы, ОшибкаНеНайденТокен(), РекомендацииДляТокена());
		ОткрытьФормуОшибки(НовыйОписаниеОшибки(НСтр("ru = 'Создание ключа электронной подписи'"), Ошибка));
		ОбработатьЗаявлениеПослеСозданияКлючаИЗапросаНаСертификат(Ложь, Контекст);
		Возврат;
	КонецЕсли;
	
	СвойстваТокена = Найдено[0];
	
	Если ЗначениеЗаполнено(СвойстваТокена.Ошибка) Тогда
		
		ОткрытьФормуОшибки(НовыйОписаниеОшибки(НСтр("ru = 'Создание ключа электронной подписи'"), СвойстваТокена.Ошибка));
		ОбработатьЗаявлениеПослеСозданияКлючаИЗапросаНаСертификат(Ложь, Контекст);
	...
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для создания ключа электронной подписи требуется установка внешней компоненты %1.'"), "ExtraCryptoAPI");
	...
	Контекст.Вставить("СвойстваТокена", СвойстваТокена);
	Ждать ЭлектроннаяПодписьКлиентЛокализация.ОчиститьОшибкиКомпоненты(ОбъектКомпоненты);
	
	Попытка
		НастройкиТокена = Ждать ОбъектКомпоненты.PKCS11ПолучитьНастройкиБэкендаПоУмолчаниюАсинх(СвойстваТокена.Токен);
		Ждать ЭлектроннаяПодписьКлиентЛокализация.ОшибкаКомпоненты(ОбъектКомпоненты);
		НастройкиТокена = НастройкиТокена.Значение;
	Исключение
		Ошибка = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ОткрытьФормуОшибки(НовыйОписаниеОшибки(НСтр("ru = 'Создание ключа электронной подписи'"), Ошибка));
		ОбработатьЗаявлениеПослеСозданияКлючаИЗапросаНаСертификат(Ложь, Контекст);
	...
	Ошибка = "";
	Попытка
		Ждать ОбъектКомпоненты.PKCS11ИнициализацияАсинх(НастройкиТокена);
	Исключение
		Ошибка = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если Не ПустаяСтрока(Ошибка) И Не ЭлектроннаяПодписьКлиентЛокализация.БиблиотекаУжеИнициализирована(Ошибка) Тогда
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьПослеВводаПинКода", ЭтотОбъект, Контекст);
	ЭлектроннаяПодписьКлиентЛокализация.ВвестиПинКодТокена(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Функция ОшибкаНеНайденТокен()
	Возврат ЭлектроннаяПодписьСлужебныйКлиент.ОшибкаНеНайденТокен();
КонецФункции

&НаКлиенте
Асинх Процедура ПродолжитьПослеВводаПинКода(ПинКод, Контекст) Экспорт
	...
	СвойстваТокена = Контекст.СвойстваТокена;
	
	Если Не ЗначениеЗаполнено(КонтейнерКлючаИмя) Тогда
		
		КонтейнерКлючаИмя =  Прав(СтрЗаменить(Строка(Новый УникальныйИдентификатор), "-", ""), 32);
		
		ТипМеханизмаГенерацииКлючевойПары = СвойстваТокена.ТипМеханизмаГенерацииКлючевойПары; 
		Ждать ЭлектроннаяПодписьКлиентЛокализация.ОчиститьОшибкиКомпоненты(ОбъектКомпоненты);
		
		Попытка
			Результат = Ждать ОбъектКомпоненты.PKCS11СоздатьКлючевуюПаруАсинх(СвойстваТокена.Слот, ПинКод, ТипМеханизмаГенерацииКлючевойПары, КонтейнерКлючаИмя);
			Ждать ЭлектроннаяПодписьКлиентЛокализация.ОшибкаКомпоненты(ОбъектКомпоненты);
		Исключение
			Ошибка = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			КонтейнерКлючаИмя = "";
	...
	Атрибуты.Вставить("2.5.4.3", СтрШаблон("UTF8String:%1", Контекст.ПоляСертификата.CN));
	Атрибуты.Вставить("2.5.4.4",  СтрШаблон("UTF8String:%1",  Контекст.ПоляСертификата.SN));
	Атрибуты.Вставить("2.5.4.42", СтрШаблон("UTF8String:%1",  Контекст.ПоляСертификата.G));
	Атрибуты.Вставить("1.2.643.100.3",  Контекст.ПоляСертификата.SNILS);
	Атрибуты.Вставить("1.2.643.3.131.1.1",  Контекст.ПоляСертификата.INN);
	Атрибуты.Вставить("2.5.4.6", "RU");
	Атрибуты.Вставить("1.2.840.113549.1.9.1",  Контекст.ПоляСертификата.E);
	
	АтрибутыJSON = СтрокаJSON(Атрибуты);
	
	РасширенныеАтрибуты = Новый Соответствие;
	РасширенныеАтрибуты.Вставить("keyUsage", "critical,digitalSignature,nonRepudiation,keyEncipherment,dataEncipherment,keyAgreement");
	РасширенныеАтрибуты.Вставить("extendedKeyUsage", "1.3.6.1.5.5.7.3.2,1.3.6.1.5.5.7.3.4");
	РасширенныеАтрибуты.Вставить("certificatePolicies", "1.2.643.100.113.1");  
	РасширенныеАтрибуты.Вставить("1.2.643.100.111", СтрШаблон("ASN1:UTF8String:%1", ИмяСредства(СвойстваТокена.Токен)));
	РасширенныеАтрибуты.Вставить("1.3.6.1.4.1.311.21.7", "DER:30:10:06:08:2A:85:03:02:02:2E:00:08:02:01:01:02:01:00"); //CertTemplate  
	РасширенныеАтрибуты.Вставить("1.2.643.100.114", СтрШаблон("DER:02:01:0%1",  Контекст.ПоляСертификата.IdentKind));
	
	РасширенныеАтрибутыJSON =  СтрокаJSON(РасширенныеАтрибуты);
	
	Ждать ЭлектроннаяПодписьКлиентЛокализация.ОчиститьОшибкиКомпоненты(ОбъектКомпоненты);
	
	Попытка
		
		РезультатЗапрос = Ждать ОбъектКомпоненты.PKCS11СоздатьCSRАсинх(СвойстваТокена.Слот, ПинКод, КонтейнерКлючаИмя, АтрибутыJSON, РасширенныеАтрибутыJSON);
		РезультатОткрытыйКлюч = Ждать ОбъектКомпоненты.PKCS11ПолучитьОткрытыйКлючАсинх(СвойстваТокена.Слот, ПинКод, КонтейнерКлючаИмя);
		Ждать ЭлектроннаяПодписьКлиентЛокализация.ОшибкаКомпоненты(ОбъектКомпоненты);
	...
	Контекст.Вставить("ТекущаяОткрытаяЧастьКлючаЭП", ПолучитьHexСтрокуИзДвоичныхДанных(РезультатОткрытыйКлюч.Значение));
	Контекст.Вставить("ТекущийИКС", ИдентификаторКлючаСубъектаHexСтрокой(Контекст.ТекущаяОткрытаяЧастьКлючаЭП));
	Контекст.Вставить("ИмяФайлаЗапроса", РезультатЗапрос.Значение);
	Контекст.Вставить("ПомещаемыеФайлы", Новый Массив);
	Контекст.Вставить("ЗапросСоздан", Истина);
	
	Контекст.ПомещаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(РезультатЗапрос.Значение));
	...
	НачатьЗапросРазрешенияПользователя(Новый ОписаниеОповещения(
		"СоздатьКлючИЗапросНаСертификатНаТокенеПослеПолученияРазрешений", ЭтотОбъект, Контекст), Вызовы);

КонецПроцедуры

&НаКлиенте
Асинх Процедура СоздатьКлючИЗапросНаСертификатНаТокенеПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	ФайловаяСистемаКлиент.ЗагрузитьФайлы(Новый ОписаниеОповещения(
		"СоздатьКлючИЗапросНаСертификатНаТокенПослеПомещенияФайлов", ЭтотОбъект, Контекст,
		"СоздатьКлючИЗапросНаСертификатНаТокенПослеОшибкиПомещенияФайлов", ЭтотОбъект),
		ПараметрыЗагрузки, Контекст.ПомещаемыеФайлы);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьКлючИЗапросНаСертификатНаТокенПослеОшибкиПомещенияФайлов(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
...
&НаКлиенте
Асинх Процедура СоздатьКлючИЗапросНаСертификатНаТокенПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Попытка
		Ждать УдалитьФайлыАсинх(Контекст.ИмяФайлаЗапроса);
	Исключение
		ИмяСобытия = НСтр("ru = 'Электронная подпись.Заявление на выпуск сертификата'",
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытия, "Ошибка",
			НСтр("ru = 'Не удалось удалить файл запроса на сертификат.'") + Символы.ПС 
			+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
	КонецПопытки;
	
	ОбработатьЗаявлениеПослеСозданияКлючаИЗапросаНаСертификат(Истина, Контекст);
...
Процедура ПроверитьДоступностьОтправки(ЭтоПроверкаПередОтправкой = Ложь)
	...
	Если Не ЭтоФизическоеЛицо И Не ЭтоИндивидуальныйПредприниматель
		...
		ПоказыватьИнформациюОДоступностиОтправкиЗаявления = ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекЗагрузить(
...
Процедура ПроверитьЗаявлениеВЭлектронномВидеПередОтправкой()
	...
	Если Не ДокументыПечатались Тогда
		Если ДанныеЗаявления.ОформленоВЭлектронномВиде Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Перед подписанием и отправкой Заявления необходимо создать ключ электронной подписи.'"),,,, Отказ);
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Документы еще не печатались. Заявление не будет принято пока не будут получены печатные документы.'"),,,, Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(АдресФайлаСНИЛС) И ПродлениеСертификатаСтороннегоУЦ Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран файл скана СНИЛС'"),,,, Отказ);
	КонецЕсли;
	...
	Если ДокументыПечатались И Не Модифицированность Тогда
		Если ПродлениеСертификатаСтороннегоУЦ Тогда
			ПродолжитьОтправкуЗаявлениеНаПродлениеСертификатаСтороннегоУЦ();
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ПродолжитьОтправкуЗаявлениеНаПродлениеСертификатаСтороннегоУЦ()
...
Процедура СформироватьИОтправитьФайлыЗаявления(Результат, ДополнительныеПараметры) Экспорт
	...
	ОписаниеДанных = НовыйОписаниеДанных();
	ОписаниеДанных.Данные = АдресЗаявлениеНаПодключение;
	ОписаниеДанных.ОписаниеДляПакета.ИдентификаторДокумента = НовыйСжатыйУникальныйИдентификатор();
	ОписаниеДанных.ОписаниеДляПакета.ТипДокумента = "ЗаявлениеНаПодключение";
	ОписаниеДанных.ОписаниеДляПакета.ТипСодержимого = "zip";
	ИмяФайла = НовыйСжатыйУникальныйИдентификатор();
	ОписаниеДанных.ОписаниеДляПакета.ИмяФайла = ИмяФайла + ".bin";
	ОписаниеДанных.Представление.Представление = НСтр("ru = 'Заявление на подключение (файл сформирован)'");
	ОписаниеДанных.Представление.Значение = Новый ОписаниеОповещения(
		"ОткрытьФайлОбработкаОповещения", ЭтотОбъект, Новый Структура("АдресФайла, ИмяФайла", АдресЗаявлениеНаПодключение, ИмяФайла + ".zip"));
	...
	Если ПродлениеСертификатаСтороннегоУЦ Тогда
		ПриложитьДокумент(ДанныеДляПодписания, "СНИЛС");
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФайлОбработкаОповещения(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		
		ФайловаяСистемаКлиент.ОткрытьФайл(ДополнительныеПараметры.АдресФайла,, ДополнительныеПараметры.ИмяФайла);
...
Процедура ОткрытьФормуОшибки(ОписаниеОшибки, ОбработкаПродолжения = Неопределено)
	...
	ПараметрыОшибки.Вставить("ЗаголовокПредупреждения", ОписаниеОшибки.Описание);
	ПараметрыОшибки.Вставить("ТекстОшибкиКлиент", "" + ОписаниеОшибки.Текст + ?(ЗначениеЗаполнено(ОписаниеОшибки.Код), " (" + ОписаниеОшибки.Код + ")", ""));

	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыОшибки, ЭтотОбъект, ОбработкаПродолжения);
...
//
&НаКлиенте
Функция НовыйОписаниеОшибки(Описание = "", Текст = "", Код = "")
	
	ОписаниеОшибки = Новый Структура;
	ОписаниеОшибки.Вставить("Код", Код);
	ОписаниеОшибки.Вставить("Текст", Текст);
...
//
&НаСервере
Функция ПакетЗаявления(Знач ОписанияФайловПакета, Знач ТипТранзакции)
	...
	Для Каждого ОписаниеФайла Из ОписанияФайловПакета Цикл
		...
		Если ОписаниеФайла.Свойство("СвойстваПодписи") Тогда
			...
			Если ТипЗнч(ОписаниеФайла.СвойстваПодписи) = Тип("Строка")Тогда
				ОписаниеФайла.СвойстваПодписи = ПолучитьИзВременногоХранилища(ОписаниеФайла.СвойстваПодписи); // см. ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваПодписи
			КонецЕсли;
...
&НаКлиенте
Асинх Процедура ПослеПолученияПрограммКриптографии(Результат, Параметры) Экспорт
	...
	Ждать ЗаполнитьСписокУстройств(Ложь); 
	
	ОтобразитьПоискПриложений(Ложь);
...
&НаКлиенте
Асинх Функция ЗаполнитьСписокУстройств(ПредлагатьУстановить = Истина)
	
	ОтобразитьПоискПриложений(Истина, Истина);
	
	СписокТокенов.Очистить();
	
	УстановленныеТокеныРезультат = Ждать ЭлектроннаяПодписьКлиентЛокализация.УстановленныеТокены(Неопределено, ПредлагатьУстановить);
	ТекстОшибкиЗаполненияУстройств = УстановленныеТокеныРезультат.Ошибка;
	
	Для Каждого Токен Из УстановленныеТокеныРезультат.Токены Цикл
		
		Найдено = СписокТокенов.НайтиСтроки(Новый Структура("ПрограммаИдентификатор", Токен.СерийныйНомер));
		Если Найдено.Количество() > 0 Тогда
			НоваяСтрока = Найдено[0];
		Иначе
			НоваяСтрока = СписокТокенов.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Токен);
			НоваяСтрока.ПрограммаИмя = ИмяПрограммы(НоваяСтрока.Токен);
			НоваяСтрока.ПрограммаТип = ТипПрограммы(НоваяСтрока.Токен);
			НоваяСтрока.ПрограммаИдентификатор = Токен.СерийныйНомер;
			Если Не ПустаяСтрока(НоваяСтрока.Ошибка) И СтрНайти(НоваяСтрока.Ошибка, ОшибкаНеНайденТокен()) <> 0 Тогда
				НоваяСтрока.Ошибка = СтрШаблон("%1%2%3", НоваяСтрока.Ошибка, Символы.ПС, РекомендацииДляТокена());
			КонецЕсли;
		КонецЕсли;
		
		Если Токен.Слот = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(НоваяСтрока.ТипМеханизмаГенерацииКлючевойПары) Тогда
			Попытка
				ТипМеханизмаГенерацииКлючевойПары = ТипМеханизмаГенерацииКлючевойПары(Токен.Механизмы, Токен.Представление);
			Исключение
				НоваяСтрока.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: Не удалось получить информацию о механизмах токена в слоте %2. %3'"),
					НоваяСтрока.Представление, Токен.Слот, ОбработкаОшибок.КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Продолжить;
			КонецПопытки;
			
			Если ТипЗнч(ТипМеханизмаГенерацииКлючевойПары) = Тип("Строка") Тогда
				НоваяСтрока.Ошибка = ТипМеханизмаГенерацииКлючевойПары;
			Иначе
				НоваяСтрока.Представление = Токен.Представление;
				НоваяСтрока.Слот = Токен.Слот;
				НоваяСтрока.ТипМеханизмаГенерацииКлючевойПары = ТипМеханизмаГенерацииКлючевойПары;
				НоваяСтрока.Ошибка = "";
			КонецЕсли;
		КонецЕсли;
	...
	ОтобразитьПоискПриложений(Ложь, Истина);

	Возврат УстановленныеТокеныРезультат.ПроверкаВыполнена;
	
КонецФункции

&НаКлиенте
Процедура ОтобразитьПоискПриложений(ИдетПоиск, ЭтоТокен = Ложь)
	
	Элементы.ПредставлениеПрограммы.Доступность = Не ИдетПоиск;
	Элементы.Программа.Доступность = Не ИдетПоиск;
	Элементы.ГруппаПоискПриложений.Видимость = ИдетПоиск;
	Элементы.ДекорацияОбновитьСписокПриложений.Видимость = Не ИдетПоиск;

	Если ИдетПоиск Тогда
		Если ЭтоТокен Тогда
			Элементы.ДекорацияПоискПриложенийТекст.Заголовок = НСтр(
				"ru = 'Идет поиск токенов...'");
		Иначе
			Элементы.ДекорацияПоискПриложенийТекст.Заголовок = НСтр(
				"ru = 'Идет поиск приложений электронной подписи...'");
		КонецЕсли;
	КонецЕсли;
	
	Элементы.ДекорацияПоискПриложенийТекст.Видимость = ИдетПоиск;
	
КонецПроцедуры

&НаКлиенте
Функция РекомендацииДляТокена()
	
	Возврат ЭлектроннаяПодписьСлужебныйКлиент.РекомендацииДляТокена();
	
КонецФункции

&НаКлиенте
Функция ТипМеханизмаГенерацииКлючевойПары(Механизмы, Представление)
	
	Если Не ЗначениеЗаполнено(Механизмы) Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Токен %1 предназначен только для хранения ключей электронной подписи, для создания ключа выберите %2 или %3.'"),
			Представление,
			ЭлектроннаяПодписьКлиентСерверЛокализация.ИмяПрограммыКриптоПро(),
			ЭлектроннаяПодписьКлиентСерверЛокализация.ИмяПрограммыVipNet());
	КонецЕсли;
	
	СписокМеханизмов = Прочитать(Механизмы);
	СписокМеханизмов = СписокМеханизмов.Получить("support_mechanisms");
	СписокПоддерживаемыхМеханизмов = Новый Массив;

	Для Каждого Элемент Из СписокМеханизмов Цикл
		ТипМеханизма = Элемент.Получить("type");
		Если ТипМеханизма = 4608 Тогда
			Возврат ТипМеханизма;
		КонецЕсли;

		ИмяМеханизма = СокрЛП(Элемент.Получить("name"));
		Если ИмяМеханизма = "CKM_GOSTR3410_256_KEY_PAIR_GEN" Тогда
			Возврат ТипМеханизма;
		КонецЕсли;

		Если ЗначениеЗаполнено(ИмяМеханизма) Тогда
			СписокПоддерживаемыхМеханизмов.Добавить(СтрШаблон("%1 (%2)", ИмяМеханизма, ТипМеханизма));
		КонецЕсли;
	КонецЦикла;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'На токене %1 не поддерживается механизм создания ключевой пары с алгоритмом ГОСТ 34.10-2012 (256).
			 |Поддерживаемые механизмы: %2'"), Представление, СтрСоединить(СписокПоддерживаемыхМеханизмов, ", "));
	
КонецФункции

&НаКлиенте
Функция ИмяПрограммы(Модель)
	
	Если СтрНачинаетсяС(НРег(Модель), "rutoken") Тогда
		Возврат "Aktiv ruToken CSP 2.0";
	ИначеЕсли СтрНачинаетсяС(НРег(Модель), "jacarta") Тогда
		Возврат "JaCarta GOST R 34.10-2012";
	КонецЕсли;
...
&НаКлиенте
Функция ИмяСредства(Модель)
	
	Если СтрНачинаетсяС(НРег(Модель), "rutoken") Тогда
		Возврат "СКЗИ Рутокен ЭЦП"; // АПК:1297 Название для внешней системы.
	ИначеЕсли СтрНачинаетсяС(НРег(Модель), "jacarta") Тогда
		Возврат "SKZI Kriptotoken 2 EP";
	КонецЕсли;
...
Процедура ПоказатьПодсказкуКриптопровайдеры(Результат)
	...
	Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр(
		"ru = '<a href = ""%1"">Нажмите здесь</a>, чтобы определить установленные приложения и токены для работы с электронной подписью.'"),
	...
	Элементы.ДекорацияПроверкаУстановкиКриптопровайдераРасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось определить установленные приложения электронной подписи и шифрования по причине:
		|%1'"), Результат.Ошибка);
...
Процедура ПоказатьДобавлениеВСправочник(ВыбранноеЗначение)
	...
	Если Не ЭтоПолноправныйПользователь Тогда
		...
		ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Приложение %1 не включено в список используемых приложений и не установлено на компьютере.
	...
	ИначеЕсли ЭтоПодчиненныйУзелРИБ Тогда
		...
		ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Приложение %1 не включено в список используемых приложений и не установлено на компьютере.
					 |Добавьте приложение в главном узле распределенной информационной базы.'"), Представление));
...
Функция ЗаполнитьСписокПрограмм(ОписаниеОшибки = "")
	...
	ЗаполнитьСписокПрограммНаСервере(НовыйСписокПрограмм, ВариантАлгоритмов,
	...
	ТипПровайдера = ТипПровайдераВыбранногоТарифа();
	
	Для Каждого Элемент Из СписокТокенов Цикл
		Если ЗначениеЗаполнено(Элемент.Ошибка) Тогда
			ОписаниеОшибки =  ОписаниеОшибки + Символы.ПС + Элемент.Ошибка;
		КонецЕсли;
		ЕстьУстройства = Истина;
	КонецЦикла;
	
	Если ТипПровайдера <> "hardware" Тогда
		Для Каждого Элемент Из НовыйСписокПрограмм Цикл
			ЗаполнитьЗначенияСвойств(СписокПрограмм.Добавить(), Элемент);
		КонецЦикла;
		ЕстьПрограммы = Истина;
	КонецЕсли;
	
	Если ТипПровайдера <> "software" Тогда
		Для Каждого Элемент Из СписокТокенов Цикл
			НоваяСтрока = СписокПрограмм.Добавить();
			НоваяСтрока.Идентификатор = Элемент.ПрограммаИдентификатор;
			НоваяСтрока.Представление = Элемент.Представление;
			НоваяСтрока.ИмяПрограммы = Элемент.ПрограммаИмя;
			НоваяСтрока.ТипПрограммы = Элемент.ПрограммаТип;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ЕстьУстройства Или ЕстьПрограммы;
	
КонецФункции

&НаСервереБезКонтекста
Процедура ЗаполнитьСписокПрограммНаСервере(СписокПрограмм, ВариантАлгоритмов,
	...
	ЭтоСеансWindows = ОбщегоНазначения.ЭтоWindowsКлиент();
	ЭтоСеансLinux = ОбщегоНазначения.ЭтоLinuxКлиент();
	ЭтоСеансMacOS = ОбщегоНазначения.ЭтоMacOSКлиент();
	...
	Для Каждого Настройка Из Настройки Цикл
		...
		Если ЭтоСеансWindows И Настройка.НетВWindows
			Или ЭтоСеансLinux И Настройка.НетВLinux
			Или ЭтоСеансMacOS И Настройка.НетВMacOS
			Или Настройка.Неактуальная Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьПрограмму(Оповещение = Неопределено)
	...
	Если КлючНаТокене И СписокТокенов.Количество() > 0 Тогда
		ЗаполнитьТокен(СписокТокенов[0]);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьТокен(СвойстваТокена)
	
	Если ПрограммаИдентификатор = СвойстваТокена.ПрограммаИдентификатор Тогда
		Возврат;
	КонецЕсли;
	
	ПрограммаИдентификатор = СвойстваТокена.ПрограммаИдентификатор;
	Объект.Программа = Неопределено;
	ПрограммаИмя = СвойстваТокена.ПрограммаИмя;
	ПрограммаТип = СвойстваТокена.ПрограммаТип;
	ПредставлениеПрограммы = СвойстваТокена.Представление;
	УстановитьВидимостьПрограммы();
	КлючНаТокене = Истина;
	ПодключитьОбработчикОжидания("ПослеВыбораПрограммыОбработчикОжидания", 0.1, Истина);
	...
	Если СписокПрограмм.Количество() <= Контекст.Индекс + 1 Тогда
		...
		Если Контекст.ПерваяДобавленнаяПрограмма <> Неопределено Тогда
			Если ТипЗнч(Контекст.ПерваяДобавленнаяПрограмма) = Тип("СправочникСсылка.ПрограммыЭлектроннойПодписиИШифрования") Тогда
				...
				НайденаСтрока = СписокПрограмм.НайтиСтроки(Новый Структура("Ссылка", Контекст.ПерваяДобавленнаяПрограмма));
				Если НайденаСтрока.Количество() > 0 Тогда
					КлючНаТокене = Ложь;
					ПрограммаИмя = НайденаСтрока[0].ИмяПрограммы;
					ПрограммаТип = НайденаСтрока[0].ТипПрограммы;
				КонецЕсли;
			Иначе
				ПрограммаИдентификатор = Контекст.ПерваяДобавленнаяПрограмма;
				
				НайденаСтрока = СписокПрограмм.НайтиСтроки(Новый Структура("Идентификатор", ПрограммаИдентификатор));
				Если НайденаСтрока.Количество() > 0 Тогда
					КлючНаТокене = НайденаСтрока[0].ТипПрограммы = 100;
					ПрограммаИмя = НайденаСтрока[0].ИмяПрограммы;
					ПрограммаТип = НайденаСтрока[0].ТипПрограммы;
				КонецЕсли;
			КонецЕсли;
...
Процедура ПрограммаОбработкаВыбораПродолжение(Ответ, ИдентификаторПоставляемойНастройки) Экспорт
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы(ИдентификаторПоставляемойНастройки);
...
Процедура ПослеВыбораПрограммы(ПоказатьОшибку, Оповещение = Неопределено)
	...
	Если КлючНаТокене 
		Или Не ЗначениеЗаполнено(Объект.Программа) И Не ЗначениеЗаполнено(ПрограммаИдентификатор)
...
Процедура ПослеВыбораПрограммыПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.ПоказатьОшибку Тогда
			...
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
				НСтр("ru = 'Выбор приложения электронной подписи'"), "", Менеджер,
				Новый Структура, Новый Структура("ПоказатьИнструкцию", Истина));
...
Процедура ЗаполнитьДанныеВыбораПрограммы(ДанныеВыбора, СтандартнаяОбработка)
	...
	Для Каждого СтрокаТаблицы Из СписокПрограмм Цикл
		Значение = ?(ЗначениеЗаполнено(СтрокаТаблицы.Ссылка), СтрокаТаблицы.Ссылка, СтрокаТаблицы.Идентификатор);
		ДанныеВыбора.Добавить(Значение, СтрокаТаблицы.Представление);
	КонецЦикла;
...
Процедура УстановитьВидимостьПрограммы()
	...
	Если Не ВидимостьПрограммы Тогда
		Если Не ЗначениеЗаполнено(ПрограммаИдентификатор) Тогда
			Элементы.ДекорацияПрограмма.Заголовок = НСтр("ru = 'Приложение не удалось определить автоматически.'");
		ИначеЕсли ЗначениеЗаполнено(ПрограммаИдентификатор) И КлючНаТокене Тогда
			Элементы.ДекорацияПрограмма.Видимость = Ложь;
		ИначеЕсли ЗначениеЗаполнено(ПрограммаИдентификатор) И ПрограммыАвто.Получить(ПрограммаИдентификатор) <> Неопределено Тогда
			Элементы.ДекорацияПрограмма.Заголовок = НСтр("ru = 'Приложение установлено на компьютере и определено автоматически.'");
		Иначе
			Если ПрограммыАвто.Количество() > 0 Тогда
				Элементы.ДекорацияПрограмма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Приложение %1 не установлено на компьютере.'"), ПредставлениеПрограммы);
			ИначеЕсли ДанныеЗаявления.СостояниеЗаявления = ПредопределенноеЗначение("Перечисление.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено") Тогда
				Элементы.ДекорацияПрограмма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Приложение %1 не удалось определить автоматически.'"), ПредставлениеПрограммы);
			Иначе
				Элементы.ДекорацияПрограмма.Видимость = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ЗаявлениеXMLДляБумажногоПродления(Билет)
	...
	ЗаписьXML.ЗаписатьАтрибут("ВерсПрог", "1С"); // АПК:1036 внешний идентификатор
		...
		ЗаписьXMLЭлемент(ЗаписьXML, "ВидСертификатаКод", ДанныеЗаявления.КодТарифа);
...
Функция ЗаявлениеXMLДляЭлектронногоПродления(ОписаниеОшибки)
	...
	ЗаписьXML.ЗаписатьАтрибут("ВерсПрог", "1С"); // АПК:1036 внешний идентификатор
		...
		ЗаписьXMLЭлемент(ЗаписьXML, "ТипЗаявления", ?(ПродлениеСертификатаСтороннегоУЦ, "2", "3"));
		ЗаписьXMLЭлемент(ЗаписьXML, "ВидСертификатаКод", ДанныеЗаявления.КодТарифа);
		Если Не ПродлениеСертификатаСтороннегоУЦ Тогда
...
Процедура СоздатьКлючИЗапросНаСертификат(Оповещение)
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат установите расширение для работы с 1С:Предприятием.'"),
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат установите внешнюю компоненту.'"));
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваСоздатьКонтейнерПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(Путь) Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ключ электронной подписи с помощью приложения %1 не создан.'"),
...
Процедура СоздатьКлючИЗапросНаСертификатПослеУточненияОшибкиВызоваСоздатьКонтейнер(ТекстОшибки, Контекст) Экспорт
	...
	ШаблонОшибки = НСтр("ru = 'Не удалось выполнить операцию по причине:
			|%1
			|
			|Возможно, операция отменена или завершилась неудачей в приложении электронной подписи.
			|Повторите попытку.'");
...
&НаКлиенте
Асинх Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваПолучитьОткрытыйКлючПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
...
Процедура ПроверитьСуществованиеКонтейнераКлючейПослеПолученияИменКонтейнеров(ОписаниеИмен, Контекст) Экспорт
	...
	Если ВыбранаПрограммаКриптоПро(ПрограммаИдентификатор, СписокПрограмм, Объект.Программа) Тогда
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
			...
			Иначе
				...
				ПоказатьПредупреждение(
					Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке", ЭтотОбъект, Контекст),
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Приложение электронной подписи не доступно по причине:
		...
		Если ПрограммаАвто = Неопределено Тогда
			...
			ПоказатьПредупреждение(
				Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке", ЭтотОбъект, Контекст),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не установлено приложение %1(%2).'"), ПредставлениеПрограммы, ПрограммаИдентификатор));
			Возврат;
		КонецЕсли;
...
Функция ДанныеПрограммы()
	...
	Если Строки.Количество() = 0 Тогда
		Возврат Новый Структура("ИмяПрограммы, ТипПрограммы");
	КонецЕсли;
...
Процедура УстановитьПолученныйСертификат()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) И Не ЗначениеЗаполнено(ПрограммаИдентификатор) Тогда
		ТекстОшибки = НСтр("ru = 'Поле не заполнено.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки,, "Объект.Программа");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура УстановитьСертификатПослеПодтвержденияИнформации()
	...
	ИначеЕсли КлючНаТокене Тогда
		УстановитьСертификатНаТокен();
	Иначе
...
&НаКлиенте
Асинх Процедура УстановитьСертификатНаТокен()
	...
	Если Найдено.Количество() = 0 Тогда
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: %2. Вставьте токен в компьютер и повторите попытку установить сертификат.
			|%3'"), ПредставлениеПрограммы, ОшибкаНеНайденТокен(), РекомендацииДляТокена());
		ПослеУстановкиСертификата(Ложь, Контекст);
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СвойстваТокена.Ошибка) Тогда
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: %2.'"), ПредставлениеПрограммы, СвойстваТокена.Ошибка);
		ПослеУстановкиСертификата(Ложь, Контекст);
		Возврат;
	КонецЕсли;
	
	ДД = ПолучитьИзВременногоХранилища(АдресСертификата);
	
	СертификатКриптографии = Новый СертификатКриптографии();
	Ждать СертификатКриптографии.ИнициализироватьАсинх(ДД);
	
	СертификатНайденНаТокене = Ложь;
	СертификатыНаТокене = Ждать ЭлектроннаяПодписьКлиентЛокализация.СертификатыНаТокене(СвойстваТокена,,Истина);
	Если ЗначениеЗаполнено(СертификатыНаТокене.Ошибка) Тогда
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: %2.'"), ПредставлениеПрограммы, СертификатыНаТокене.Ошибка);
		ПослеУстановкиСертификата(Ложь, Контекст);
	Иначе
		Для Каждого СертификатНаТокене Из СертификатыНаТокене.Сертификаты Цикл
			СертификатКриптографииНаТокене = Ждать ЭлектроннаяПодписьСлужебныйКлиент.СертификатИзСтроки(СертификатНаТокене);
			Если ТипЗнч(СертификатКриптографииНаТокене) = Тип("Строка") Тогда
				Продолжить;
			КонецЕсли;
			Если СертификатКриптографииНаТокене.Отпечаток = СертификатКриптографии.Отпечаток
				И СертификатКриптографииНаТокене.СерийныйНомер = СертификатКриптографии.СерийныйНомер Тогда
				СертификатНайденНаТокене = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Если СертификатНайденНаТокене Тогда
		УстановитьСертификатПослеУстановкиНаТокен(Контекст);
		Возврат;
	КонецЕсли;
	
	Контекст.Вставить("СвойстваТокена", СвойстваТокена);
	Контекст.Вставить("ДанныеСертификата", ДД);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьУстановкуПослеВводаПинКода", ЭтотОбъект, Контекст);
	ЭлектроннаяПодписьКлиентЛокализация.ВвестиПинКодТокена(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Асинх Процедура ПродолжитьУстановкуПослеВводаПинКода(ПинКод, Контекст) Экспорт
	
	СвойстваТокена = Контекст.СвойстваТокена;
	СертификатBase64 = Base64Строка(Контекст.ДанныеСертификата);
	
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
		"ru = 'Для импорта сертификата на токен требуется установка внешней компоненты %1.'"),
		"ExtraCryptoAPI");
	
	Попытка
		ОбъектКомпоненты = Ждать ЭлектроннаяПодписьСлужебныйКлиент.ОбъектВнешнейКомпонентыExtraCryptoAPI(Истина, ТекстПояснения);
	Исключение
		ОшибкаУстановкиСертификата = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ПослеУстановкиСертификата(Ложь, Контекст);
		Возврат;
	КонецПопытки;
	...
	Попытка
		Результат = Ждать ОбъектКомпоненты.PKCS11ИмпортироватьСертификатАсинх(СвойстваТокена.Слот, ПинКод, СертификатBase64, КонтейнерКлючаИмя);
		Ждать ЭлектроннаяПодписьКлиентЛокализация.ОшибкаКомпоненты(ОбъектКомпоненты);
	Исключение
		ОшибкаУстановкиСертификата = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ПослеУстановкиСертификата(Ложь, Контекст);
...
&НаКлиенте
Процедура УстановитьСертификатПослеУстановкиНаТокен(Контекст)
	ПараметрыСоздания = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыСозданияМенеджераКриптографии();
	ПараметрыСоздания.ПоказатьОшибку = Неопределено;
		
	ЭлектроннаяПодписьСлужебныйКлиент.СоздатьМенеджерКриптографии(
		Новый ОписаниеОповещения("УстановитьВЛичноеХранилищеПослеСозданияМенеджераКриптографии",
		ЭтотОбъект, Контекст),
		"ПолучениеСертификатов", ПараметрыСоздания);
КонецПроцедуры

&НаКлиенте
Асинх Процедура УстановитьВЛичноеХранилищеПослеСозданияМенеджераКриптографии(МенеджерКриптографии, Контекст) Экспорт

	Если ТипЗнч(МенеджерКриптографии) <> Тип("МенеджерКриптографии") Тогда
		ПослеУстановкиСертификата(Истина, Контекст);
		Возврат;
	КонецЕсли;
	
	Ждать УстановитьСертификатВХранилище(МенеджерКриптографии, Ложь);
	Ждать УстановитьСертификатВХранилище(МенеджерКриптографии, Истина);
...
&НаКлиенте
Асинх Функция УстановитьСертификатВХранилище(МенеджерКриптографии, Доверенный)
	
	АдресДанныхСертификата = ?(Доверенный, АдресКорневогоСертификата, АдресСертификата);
	ТипХранилища = ?(Доверенный, ТипХранилищаСертификатовКриптографии.КорневыеСертификаты,
	ЭлектроннаяПодписьСлужебныйКлиентСервер.ТипХранилищаДляПоискаСертификата(Истина));
	
	ТекстОшибки = "";
	СертификатИнициализирован = Ложь;
	Если ЗначениеЗаполнено(АдресДанныхСертификата) Тогда
		Попытка
			СертификатКриптографии = Новый СертификатКриптографии;
			СертификатДляУстановки = ПолучитьИзВременногоХранилища(АдресДанныхСертификата);
			Ждать СертификатКриптографии.ИнициализироватьАсинх(СертификатДляУстановки);
			СертификатИнициализирован = Истина;
		Исключение
			ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;

		Если СертификатИнициализирован Тогда
			Попытка
				ХранилищеСертификатовКриптографии = Ждать МенеджерКриптографии.ПолучитьХранилищеСертификатовАсинх(ТипХранилища);
				Найдено = Ждать ХранилищеСертификатовКриптографии.НайтиПоОтпечатку(СертификатКриптографии.Отпечаток);
				Если Найдено = Неопределено Тогда
					Ждать ХранилищеСертификатовКриптографии.ДобавитьАсинх(СертификатКриптографии);
				КонецЕсли;
			Исключение
				ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			КонецПопытки;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ТекстШаблона = ?(Доверенный,
			НСтр("ru = 'Не удалось установить корневой сертификат в доверенные корневые сертификаты: %1'"),
			НСтр("ru = 'Не удалось установить сертификат в личное хранилище: %1'"));
		ПараметрыОшибки.ЗаголовокПредупреждения = НСтр("ru = 'Не удалось установить сертификат'");
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстШаблона, ТекстОшибки);
		ПараметрыОшибки.ТекстОшибкиКлиент = ТекстОшибки;
		
		ПараметрыОшибки.ДополнительныеДанные = Новый Структура("ДанныеСертификата", АдресДанныхСертификата);
		ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыОшибки);
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись.УстановкаСертификата'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Ошибка", ТекстОшибки + ?(СертификатИнициализирован, Символы.ПС + СведенияОСертификате, ""),, Истина);
	КонецЕсли;
...
Процедура УстановитьСертификатПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для установки сертификата на компьютер установите расширение для работы с 1С:Предприятием.'"),
...
Процедура УстановитьСертификатПослеСозданияОбъектаКриптографии(Результат, Контекст) Экспорт
	...
	Если Результат <> Истина Тогда
		...
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(Новый ОписаниеОповещения(
...
Процедура ПослеУстановкиСертификата(Установлен, Контекст) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		...
		ОбработкаПродолжения = Новый ОписаниеОповещения("ПослеЗакрытияФормыОповещения", ЭтотОбъект);
		ОткрытьФормуОшибки(ОписаниеОшибки, ОбработкаПродолжения);
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ПослеЗакрытияФормыОповещения(Результат, Контекст) Экспорт
	Ответ = Ждать ВопросАсинх(НСтр("ru = 'Сертификат будет установлен вручную?'"), РежимДиалогаВопрос.ДаНет);
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ПослеУстановкиСертификатаНаСервере(Истина);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ОтобразитьИнформациюОТарифах()
	
	Элементы.ПредставлениеТарифа.СписокВыбора.Очистить();
	ТолькоПросмотрЭлементов = ТолькоПросмотрЗаявления Или ЗаявлениеОтправлено(ЭтотОбъект);
	
	Если ТолькоПросмотрЭлементов Тогда
		
		Если ПустаяСтрока(ДанныеЗаявления.КодТарифа) Тогда
			Элементы.ГруппаТарифы.Видимость = Ложь;
		Иначе
			ВыбранныйТариф = Тарифы.Добавить();
			ВыбранныйТариф.Код = ДанныеЗаявления.КодТарифа;
			ВыбранныйТариф.Представление = ДанныеЗаявления.ПредставлениеТарифа;
			ВыбранныйТариф.Описание = ДанныеЗаявления.ОписаниеТарифа;
			
			Элементы.ПредставлениеТарифа.Вид = ВидПоляФормы.ПолеНадписи;
			Элементы.ПредставлениеТарифа.Ширина = 20;
			Элементы.ПредставлениеТарифа.РастягиватьПоГоризонтали = Ложь;
		КонецЕсли;
		
	Иначе
		
		ПолучитьИнформациюОТарифахНаСервере();
		
		Если Тарифы.Количество() = 1 Тогда
			Элементы.ПредставлениеТарифа.Вид = ВидПоляФормы.ПолеНадписи;
			Элементы.ПредставлениеТарифа.Ширина = 20;
			Элементы.ПредставлениеТарифа.РастягиватьПоГоризонтали = Ложь;
		Иначе
			Элементы.ПредставлениеТарифа.Вид = ВидПоляФормы.ПолеВвода;
			Для Каждого Тариф Из Тарифы Цикл
				Элементы.ПредставлениеТарифа.СписокВыбора.Добавить(Тариф.Представление);
			КонецЦикла;
		КонецЕсли;
		
		Если ПустаяСтрока(ДанныеЗаявления.КодТарифа) Тогда
			ДанныеЗаявления.КодТарифа = Тарифы[0].Код;
			ДанныеЗаявления.ПредставлениеТарифа = Тарифы[0].Представление;
			ДанныеЗаявления.ОписаниеТарифа = Тарифы[0].Описание;
		КонецЕсли;
		
		Элементы.ВключатьЛицензиюКриптоПроВСертификат.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Включить лицензию КриптоПро CSP (%1)'"),
			?(СтоимостьЛицензииКриптоПро > 0, Строка(СтоимостьЛицензииКриптоПро) + " ₽", "платно"));
...
&НаСервере
Процедура ПолучитьИнформациюОТарифахНаСервере()
	...
	Попытка
		...
		СвойстваФункции = Фабрика.Пакеты.Получить(URIПространстваИмен).КорневыеСвойства.Получить("GetTarifInfo1C");
		Если СвойстваФункции = Неопределено Тогда
		...
		Ответ = ВебСервис.GetTarifInfo1C(ЗначениеПустогоПараметра);
		
		Для Каждого ИнформацияОТарифе Из Ответ.TarifInfo1CData.TarifInfo1CResult.Tariffs.Tariff Цикл
			Если ИнформацияОТарифе.IsCryptoPro = Истина Тогда
				СтоимостьЛицензииКриптоПро = ИнформацияОТарифе.Price;
			Иначе
				НовыйТариф = Тарифы.Добавить();
				НовыйТариф.Код = ИнформацияОТарифе.Code;
				НовыйТариф.Представление = ИнформацияОТарифе.Name + ?(ИнформацияОТарифе.Price > 0, " (" + ИнформацияОТарифе.Price + " ₽)", "");
				НовыйТариф.Описание = ИнформацияОТарифе.Description;
				НовыйТариф.ТипПровайдера = ИнформацияОТарифе.ProviderType;
			КонецЕсли;
		КонецЦикла;
	...
	Исключение
		...
		Тарифы.Очистить();
		НовыйТариф = Тарифы.Добавить();
		СвойстваБазовогоТарифа = СвойстваБазовогоТарифа();
		ЗаполнитьЗначенияСвойств(НовыйТариф, СвойстваБазовогоТарифа);
...
&НаКлиенте
Функция ПодробноеОписаниеТарифа()
	
	Возврат СтроковыеФункцииКлиент.ФорматированнаяСтрока(ДанныеЗаявления.ОписаниеТарифа);
	
КонецФункции

&НаКлиенте
Функция ТипПровайдераВыбранногоТарифа()
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("Код", ДанныеЗаявления.КодТарифа);
	
	НайденныеТарифы = Тарифы.НайтиСтроки(ПараметрыОтбора);
	...
	Если НайденныеТарифы.Количество() = 0 Тогда
		...
		Иначе
			...
			ДанныеЗаявления.ПредставлениеТарифа = Тарифы[0].Представление;
			ДанныеЗаявления.ОписаниеТарифа = Тарифы[0].Описание;
			Возврат ?(ПустаяСтрока(Тарифы[0].ТипПровайдера), "all", Тарифы[0].ТипПровайдера);
		КонецЕсли;
	КонецЕсли;
	
	ВыбранныйТариф = НайденныеТарифы[0];
	
	Возврат ВыбранныйТариф.ТипПровайдера;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция СвойстваБазовогоТарифа()
	
	Результат = Новый Структура;
	
	Результат.Вставить("Код", "base");
	Результат.Вставить("Представление", НСтр("ru = '1С:Подпись.Базовый'"));
	Результат.Вставить("Описание", НСтр("ru = 'Подходит для ЭДО с контрагентами, сдачи отчетности, работы с Гос. Порталами.'"));
	Результат.Вставить("ТипПровайдера", "all");
...
&НаКлиенте
Процедура ЗаполнитьПрограммуПослеИзмененияТарифа()
...
Процедура ВыгрузитьТехническуюИнформацию(ВыгрузитьАрхив)
	...
	Если ЗаявлениеОтправлено Тогда
		Сведения.Добавить(НСтр("ru = 'Идентификатор документооборота'")
			+ ": " + Строка(ДанныеЗаявления.ИдентификаторДокументооборота));
		Сведения.Добавить(НСтр("ru = 'Состояние обработки заявления'")
			+ " (" + Формат(ДанныеЗаявления.ДатаОбновленияСостояния, "ДЛФ=DT") + ")"
			+ ":" + Символы.ПС + ДанныеЗаявления.СостояниеОбработкиЗаявления + Символы.ПС);
	Иначе
		Сведения.Добавить(НСтр("ru = 'Заявление не отправлено'")
			+ ":" + Символы.ПС + ДанныеЗаявления.СостояниеОбработкиЗаявления + Символы.ПС);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(КонтейнерКлючаПуть) Тогда
		Сведения.Добавить(НСтр("ru = 'Путь к контейнеру ключа'")
			+ ":" + Символы.ПС + КонтейнерКлючаПуть + Символы.ПС);
	КонецЕсли;
	Если ЗначениеЗаполнено(КонтейнерКлючаИмя) Тогда
		Сведения.Добавить(НСтр("ru = 'Имя контейнера ключа'")
			+ ":" + Символы.ПС + КонтейнерКлючаИмя + Символы.ПС);
	КонецЕсли;
	Если ЗначениеЗаполнено(ПрограммаИмя) Тогда
		Сведения.Добавить(НСтр("ru = 'Имя приложения'")
			+ ":" + Символы.ПС + ПрограммаИмя + Символы.ПС);
	КонецЕсли;
	Если ЗначениеЗаполнено(ПредставлениеПрограммы) Тогда
		Сведения.Добавить(НСтр("ru = 'Приложение'")
			+ ":" + Символы.ПС + ПредставлениеПрограммы + Символы.ПС);
	КонецЕсли;
	
	ОписаниеФайлов = ОписаниеФайлов(Сведения, ВыгрузитьАрхив);
	
	Если ВыгрузитьАрхив Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			СтрСоединить(Сведения, Символы.ПС), Неопределено, , ОписаниеФайлов);
	Иначе
		ПараметрыСообщения = Новый Структура("Тема, Сообщение", НСтр("ru = 'Проблема при подаче заявления на сертификат электронной подписи'"));
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			СтрСоединить(Сведения, Символы.ПС), ПараметрыСообщения, , ОписаниеФайлов);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция ОписаниеФайлов(Сведения, ВыгрузитьАрхив)
...
Функция СвойстваУЦ(ВариантУдостоверяющегоЦентра)
	...
	СвойстваУЦ = Новый Структура("КраткоеНаименованиеУЦ, ПолноеНаименованиеУЦ,
		...
		|ПолныйЮридическийАдресУЦ, ЮридическийАдресУЦ, ФактическийАдресУЦ, ЭлектроннаяПочтаУЦ");
	...
	Если ВариантУдостоверяющегоЦентра = "KA" Тогда
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '248023, г. Калуга, пер. Теренинский, д. 6'");
	...
	ИначеЕсли ВариантУдостоверяющегоЦентра = "RT" Тогда
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '115114, Москва, ул. Кожевническая, д. 14, стр. 5'");
	...
	ИначеЕсли ВариантУдостоверяющегоЦентра = "TK" Тогда
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '119034, г. Москва, Барыковский пер., д. 4, стр. 2'");
	...
	Иначе
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '119285, г. Москва, ул. Мосфильмовская, д. 42 стр. 1, эт/пом/ком 1/1/7'");
...
Процедура СозданиеКлючаИЗапросаНаСертификатНачало()
	...
	ИначеЕсли КлючНаТокене Тогда
		ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(
			Новый ОписаниеОповещения("СоздатьКлючИЗапросНаСертификатНаТокенеПослеПодключенияРасширенияРаботыСФайлами", ЭтотОбъект),
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат установите расширение для работы с 1С:Предприятием.'"),
			Ложь);
		Возврат;
	Иначе	
...
&НаКлиенте
Процедура СоздатьКлючИЗапросНаСертификатНаТокенеПослеПодключенияРасширенияРаботыСФайлами(Результат, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатНаТокене();
...
&НаКлиентеНаСервереБезКонтекста
Функция ВыбранаПрограммаКриптоПро(ПрограммаИдентификатор, СписокПрограмм, Программа)
	
	Если ЗначениеЗаполнено(ПрограммаИдентификатор) Тогда
		Возврат СтрНачинаетсяС(ПрограммаИдентификатор , "CryptoPro");
	КонецЕсли;
	
	СтрокаПрограммы = СписокПрограмм.НайтиСтроки(Новый Структура("Ссылка", Программа));
	Если СтрокаПрограммы.Количество() <> 0 Тогда
		Возврат СтрНачинаетсяС(СтрокаПрограммы[0].Идентификатор , "CryptoPro");
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СертификатВыпущенСтороннимУЦ(Знач СертификатОснование)
	
	ДанныеСертификата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СертификатОснование, "ДанныеСертификата").Получить();
	Если ДанныеСертификата = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Сертификат = Новый СертификатКриптографии(ДанныеСертификата);
	СвойстваИздателяСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваИздателяСертификата(Сертификат);
	
	Возврат СвойстваИздателяСертификата.ОГРН <> "1047796526546"; // ООО "НПЦ "1С"
	
КонецФункции

&НаСервере
Процедура УстановитьЗаголовокДекорацияТекстЗаявления(ЗаявлениеОтправлено, ЭлектронноеПродление)
	
	Если ЗаявлениеОтправлено Тогда
		Элементы.ДекорацияТекстЗаявления.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru='<a href = ""%1"">Текст заявления</a>'"), "ТекстЗаявления");
	Иначе
		Элементы.ДекорацияТекстЗаявления.Заголовок = ?(ЭлектронноеПродление,
			СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='<a href = ""%1"">Создать ключ электронной подписи</a>'"), "ТекстЗаявления"),
			СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='Печать заявления: <a href = ""%1"">текст заявления</a>'"), "ТекстЗаявления"));
	КонецЕсли;
