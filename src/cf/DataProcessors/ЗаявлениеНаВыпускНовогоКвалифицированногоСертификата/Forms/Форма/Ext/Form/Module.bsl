...
Процедура ДозаполнитьПоПредыдущемуЗаявлениюИлиСертификату(ДанныеСертификата)
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(СведенияОВладельце, ДанныеСертификата, "Фамилия, Имя, Отчество, Должность");
...
Процедура ЗаполнитьПрограмму(Оповещение = Неопределено)
	...
	Если ЗначениеЗаполнено(Объект.Программа) Или ЗначениеЗаполнено(ПрограммаИдентификатор) Тогда
