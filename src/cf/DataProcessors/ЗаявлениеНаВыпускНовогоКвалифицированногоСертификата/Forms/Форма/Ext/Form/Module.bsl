...
Функция ПодготовленныйДокумент()
	...
	ИначеЕсли Не ЭтоИндивидуальныйПредприниматель
	        И Не ДокументыПоляСертификата.Свойство("OU") Тогда
...
Функция НастройкаУЦ(ТекстОшибки, ПолучитьВариантАлгоритмов)
	...
	Попытка
		...
		Иначе
			УзелDOM = ПостроительDOM.ПолучитьЭлементыПоИмени("CA");
			Если УзелDOM.Количество() > 0 Тогда
				ПолученныйВариантУдостоверяющегоЦентра = ВРег(УзелDOM[0].ТекстовоеСодержимое);
				Если ПолученныйВариантУдостоверяющегоЦентра <> "1C"
				   И ПолученныйВариантУдостоверяющегоЦентра <> "KA" Тогда
