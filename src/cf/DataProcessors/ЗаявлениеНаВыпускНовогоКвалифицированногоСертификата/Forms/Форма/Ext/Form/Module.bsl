...
Асинх Процедура СоздатьКлючИЗапросНаСертификатНаТокене()
	...
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
		"ru = 'Для создания ключа электронной подписи требуется установка внешней компоненты %1.'"),
		"ExtraCryptoAPI");
...
&НаКлиенте
Процедура СоздатьКлючИЗапросНаСертификатНаТокенеПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат
			           |установите расширение для работы с 1С:Предприятием.'"),
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат
...
Асинх Процедура ПродолжитьУстановкуПослеВводаПинКода(ПинКод, Контекст) Экспорт
	...
	СертификатBase64 = Base64Строка(ДД);
	ОбъектКомпоненты = Контекст.ОбъектКомпоненты;
...
Процедура СозданиеКлючаИЗапросаНаСертификатНачало()
	...
	ИначеЕсли КлючНаТокене Тогда
		ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(
		Новый ОписаниеОповещения("СоздатьКлючИЗапросНаСертификатНаТокенеПослеПодключенияРасширенияРаботыСФайлами",
			ЭтотОбъект), НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат
							  |установите расширение для работы с 1С:Предприятием.'"), Ложь);
		Возврат;
	Иначе	
