...
Процедура ИнструкцияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭтоФизическоеЛицо Тогда
		ТекстПредупреждения =
			...
			|4. Копия страхового свидетельства обязательного пенсионного страхования (СНИЛС) физического лица или уведомления о регистрации в системе персонифицированного учета (Форма АДИ-РЕГ).'");
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|6. Копия* документа, подтверждающего полномочия представителя организации, указанного для выпуска сертификата (решение собственника, протокол собрания учредителей, выписка ЕГРЮЛ или <a href=""%1"">доверенность</a>).
			|
...
Функция ОписаниеЗапросаНаКвалифицированныйСертификат(Поля, ТекстОшибки)
	...
	Если ЗапросПосле_01_09_2021() Тогда
		Если ЭтоФизическоеЛицо ИЛИ ЭтоИндивидуальныйПредприниматель Тогда
			// Поле INN - ИНН индивидуального предпринимателя или физического лица.
			Поля.Вставить("INN", ?(ЭтоФизическоеЛицо, ВладелецИНН, ИНН));
...
Функция ПодготовленныйДокумент()
	...
	Если ДокументыПоляСертификата.Свойство("INNLE") Тогда
		Макет.Параметры.INN = ДокументыПоляСертификата.INNLE;
	КонецЕсли;
...
Функция ЗаявлениеXML(Билет)
	...
	ЗаписьXML.ЗаписатьАтрибут("ВерсПрог", "1С"); // АПК:1036 внешний идентификатор
		...
		ЗаписьXMLЭлемент(ЗаписьXML, "ПризнакЮридическогоЛица", Не ЭтоИндивидуальныйПредприниматель И Не ЭтоФизическоеЛицо);
...
Процедура УстановитьПараметрыКомпоненты(ПолноеИмяМакета, ПараметрыПодключения)
	...
	ОписаниеКомпоненты = ЭлектроннаяПодписьСлужебныйКлиент.ОписаниеКомпоненты();
