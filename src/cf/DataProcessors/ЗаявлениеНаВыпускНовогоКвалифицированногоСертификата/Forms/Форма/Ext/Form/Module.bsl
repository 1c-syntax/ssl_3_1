...
Процедура ЗагрузитьЗаявление()
	...
	Если ДоступныНапоминанияПользователя
		...
		НапомнитьОГотовности = РегистрыСведений.ОповещенияПользователейСертификатов.ПрочитатьОтметкуОНапоминании(Объект.Ссылка);
	КонецЕсли;
...
Процедура СкопироватьЗаявление(СертификатОснование)
	...
	Если ПроверитьСертификатНаВозможностьЭлектронногоПродления(ДанныеСертификатаОснования, Ложь) Тогда
		ДанныеЗаявления.ОформленоВЭлектронномВиде = Истина;
	КонецЕсли;
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ТехническаяИнформация.Видимость = ЗаявлениеОтправлено;
...
Процедура УстановитьОтображениеРеквизитовЗависимыхОтВидаПродления()
	...
	Элементы.ОтправитьЗаявление.Заголовок = ?(ЭлектронноеПродление, 
		НСтр("ru='Подписать и отправить заявление'", "ru"), НСтр("ru='Отправить заявление'", "ru"));
...
Процедура ОтправитьЗаявлениеВБумажномВидеЗавершение(Результат, Контекст) Экспорт
	...
	Иначе
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось отправить заявление по причине: %1'"), ОписаниеОшибки.Текст);
		ПоказатьПредупреждение(ОбработкаПродолжения,
			УстановитьГиперссылку(ТекстОшибки, "its.1c.ru", "http://its.1c.ru"));
...
Процедура ПослеПодтвержденияИнформации(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры = "СодержаниеСертификата" Тогда
...
Процедура СформироватьИОтправитьФайлыЗаявления(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗаявлениеXML = Неопределено Тогда
		...
		Иначе
			ОбработкаПродолжения = Новый ОписаниеОповещения(
				"ПослеПодтвержденияИнформации", ЭтотОбъект);
		КонецЕсли;
		
		ПоказатьПредупреждение(ОбработкаПродолжения,
			УстановитьГиперссылку(ОписаниеОшибки.Описание, "its.1c.ru", "http://its.1c.ru"));
...
&НаКлиенте
Процедура ОткрытьФормуИнформацииСертификата(ОписаниеОшибки = Неопределено)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеПодтвержденияИнформации", ЭтотОбъект, "СодержаниеСертификата");
...
&НаКлиенте
Процедура ОткрытьФормуОшибки(ОписаниеОшибки)
...
Функция ЗаполнитьСписокПрограмм(ОписаниеОшибки = "")
	...
	ТекстОшибки = ЗаполнитьСписокПрограммНаСервере(НовыйСписокПрограмм, ВариантАлгоритмов,
		ДанныеЗаявления.СостояниеЗаявления, КонтейнерКлючаПуть, ПрограммаОблачногоСервиса);
...
Функция ВебСервисУЦ()
	...
	Возврат Обработки.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.ВебСервисУЦ();
...
Процедура УстановитьПолученныйСертификат()
	...
	Если ДанныеЗаявления.СостояниеЗаявления = 
		...
		Если ДанныеЗаявления.ОформленоВЭлектронномВиде Тогда
			СформироватьИОтправитьФайлРасписки();
		Иначе
			ОткрытьФормуИнформацииСертификата();
		КонецЕсли;
