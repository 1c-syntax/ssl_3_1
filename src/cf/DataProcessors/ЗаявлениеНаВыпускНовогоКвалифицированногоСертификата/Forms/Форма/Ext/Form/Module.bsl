...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НСтр("ru = 'Для создания запроса на сертификат используется приложение электронной подписи и шифрования <b>%1</b> или <b>%2</b> или активный ключевой носитель (<b>Рутокен</b>).
		...
		Если Найдено.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Найдено[0].Ошибка) Тогда
				...
				ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(НСтр("ru = 'Выбор токена'"), "",
					Новый Структура("ОписаниеОшибки", Найдено[0].Ошибка), Новый Структура);
				Возврат;
			КонецЕсли;
...
Асинх Процедура СоздатьКлючИЗапросНаСертификатНаТокене()
	...
	Если Найдено.Количество() = 0 Тогда
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найден токен %1.'"), ПредставлениеПрограммы);
...
Асинх Процедура ПродолжитьПослеВводаПинКода(ПинКод, Контекст) Экспорт
	...
	РасширенныеАтрибуты.Вставить("1.2.643.100.111", СтрШаблон("ASN1:UTF8String: СКЗИ %1", ПрограммаИмя));
...
Асинх Функция ЗаполнитьСписокУстройств(ПредлагатьУстановить = Истина)
	...
	Попытка
		ОбъектКомпоненты = Ждать ЭлектроннаяПодписьСлужебныйКлиент.ОбъектВнешнейКомпонентыExtraCryptoAPI(ПредлагатьУстановить, ТекстПояснения);
	Исключение
		ТекстОшибкиЗаполненияУстройств = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Для Каждого ТипУстройства Из ТипыУстройств Цикл
		...
		СтрокаТокена = СписокТокенов.Добавить();
		СтрокаТокена.Токен = ТипУстройства;
		СтрокаТокена.Представление = МодельТокена(ТипУстройства);
		СтрокаТокена.ПрограммаИдентификатор = ТипУстройства;
		СтрокаТокена.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: Не найдены подключенные токены.'"), СтрокаТокена.Представление);
		...
		Попытка
			...
			НастройкиТокена = НастройкиТокена.Значение;
		Исключение
			СтрокаТокена.Ошибка = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Продолжить;
		КонецПопытки;
		...
		Исключение
			СтрокаТокена.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1: Не найдены подключенные токены. %2'"), СтрокаТокена.Представление, 
				ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			Продолжить;
		КонецПопытки;
		
		Если СписокСлотов.Значение = Неопределено Или СписокСлотов.Значение = "null" Тогда
			СтрокаТокена.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1: Не найдены подключенные токены.'"), СтрокаТокена.Представление);
			Продолжить;
		КонецЕсли;
		
		СписокСлотов = Прочитать(СписокСлотов.Значение);
		...
		Для каждого Слот Из СписокСлотов Цикл
			...
			Для каждого ЗначениеСлота Из ЗначенияСлотов Цикл
				...
				НоваяСтрока.Слот = ЗначениеСлота;
				НоваяСтрока.Токен = ТипУстройства;
				НоваяСтрока.Представление = СтрШаблон("%1 %2", СтрокаТокена.Представление, НоваяСтрока.Слот);
				
				Попытка
					ИнформацияОСлоте = Ждать ОбъектКомпоненты.PKCS11ПолучитьИнформациюОСлотеАсинх(ЗначениеСлота);
					Ждать ПолучитьОшибкуКомпоненты(ОбъектКомпоненты);
				Исключение
					НоваяСтрока.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1: Ошибка получения информации о слоте %2. %3'"), СтрокаТокена.Представление,
						ЗначениеСлота, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
					Продолжить;
				КонецПопытки;
				
				НоваяСтрока.Описание = Прочитать(ИнформацияОСлоте.Значение).Получить("description");
				...
				Исключение
					НоваяСтрока.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1: Ошибка получения информации о токене в слоте %2. %3'"), СтрокаТокена.Представление,
						ЗначениеСлота, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
					Продолжить;
				КонецПопытки;
				
				Информация = Прочитать(ИнформацияОТокене.Значение);
				НоваяСтрока.ПрограммаИдентификатор = Информация.Получить("serial_number");
				Модель = Информация.Получить("model");
				НоваяСтрока.Модель = МодельТокена(Модель);
				НоваяСтрока.ПрограммаИмя = ИмяПрограммы(Модель);
				НоваяСтрока.ПрограммаТип = ТипПрограммы(Модель);
				НоваяСтрока.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 	НСтр("ru = '%1 (серийный номер %2)'"), НоваяСтрока.Модель, НоваяСтрока.ПрограммаИдентификатор);
				
				Попытка
					СписокМеханизмов = Ждать ОбъектКомпоненты.PKCS11ПолучитьСписокПоддерживаемыхМеханизмовАсинх(ЗначениеСлота);
					Ждать ПолучитьОшибкуКомпоненты(ОбъектКомпоненты);
					ТипМеханизмаГенерацииКлючевойПары = ТипМеханизмаГенерацииКлючевойПары(СписокМеханизмов.Значение, Модель);
				Исключение
					НоваяСтрока.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1: Ошибка получения информации о механизмах токена в слоте %2. %3'"), СтрокаТокена.Представление,
						ЗначениеСлота, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
					Продолжить;
				КонецПопытки;
		...
		Если ЕстьТокены Тогда
			СписокТокенов.Удалить(СтрокаТокена);
		КонецЕсли;
...
&НаКлиенте
Функция ТипМеханизмаГенерацииКлючевойПары(Механизмы, Модель)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не поддерживается механизм создания ключевой пары с алгоритмом ГОСТ 34.10-2012 (256).
		 |Поддерживаемые механизмы: %1'"), СтрСоединить(СписокПоддерживаемыхМеханизмов, ", "));
	
КонецФункции

&НаКлиенте
Функция МодельТокена(Модель)
