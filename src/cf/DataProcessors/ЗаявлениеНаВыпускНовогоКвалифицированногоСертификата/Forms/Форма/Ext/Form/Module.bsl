...
Функция ЗаполнитьСписокПрограмм(ОписаниеОшибки = "")
	...
	ТекстОшибкиЗаполненияПрограмм = ЗаполнитьСписокПрограммНаСервере(НовыйСписокПрограмм, ВариантАлгоритмов,
		ДанныеЗаявления.СостояниеЗаявления, КонтейнерКлючаПуть, ВстроенныйКриптопровайдер, ПрограммыАвто);

	СписокПрограмм.Очистить();

	ЕстьУстройства = Ложь; ЕстьПрограммы = Истина;
	...
	Если ЗначениеЗаполнено(ТекстОшибкиЗаполненияПрограмм) Тогда
		ЕстьПрограммы = Ложь;
		ОписаниеОшибки = ТекстОшибкиЗаполненияПрограмм;
		Если ЕстьУстройства = Ложь Тогда
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Заполнение списка приложений электронной подписи'"), "", Новый Структура("ОписаниеОшибки",
				ТекстОшибкиЗаполненияПрограмм), Новый Структура);
		КонецЕсли;
	КонецЕсли;
