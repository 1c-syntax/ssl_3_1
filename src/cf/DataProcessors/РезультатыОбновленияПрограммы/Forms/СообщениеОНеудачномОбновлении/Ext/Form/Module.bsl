...
Функция ПодключитьВнешнююОбработку(АдресВоВременномХранилище)
	...
	ИмяОбработки = Менеджер.Подключить(АдресВоВременномХранилище, , Ложь, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
	Возврат Менеджер.Создать(ИмяОбработки).Метаданные().ПолноеИмя(); // Не нарушает безопасность информации, так как создается уже подключенный объект.
	// АПК:556-вкл
	// АПК:552-вкл
