...
Процедура ОтложенныеОбработчикиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ОтложенныеОбработчикиОбработчикДополнение
	   И Элемент.ТекущиеДанные.ЭтоОбработчикОчисткиУстаревшихДанных Тогда
...
Процедура ВопросВПоддержку(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку") Тогда
		...
		ПараметрыОбращения.Тема = НСтр("ru = 'Не удалось обновить приложение на новую версию'");
		
		ТехнологическаяИнформация = Новый Массив;
		ТехнологическаяИнформация.Добавить(ПараметрыОбращения.ТехнологическаяИнформация);
		
		СтатусОшибка = ПредопределенноеЗначение("Перечисление.СтатусыОбработчиковОбновления.Ошибка");
		ПараметрОтбора = Новый Структура("Статус", СтатусОшибка);
		НайденныеОшибки = ОтложенныеОбработчики.НайтиСтроки(ПараметрОтбора);
		
		Для Каждого НайденнаяОшибка Из НайденныеОшибки Цикл
			ТехнологическаяИнформация.Добавить(НайденнаяОшибка.ИнформацияОПроцедуреОбновления);
			ТехнологическаяИнформация.Добавить(Символы.ПС);
		КонецЦикла;
		
		ПараметрыОбращения.ТехнологическаяИнформация = СтрСоединить(ТехнологическаяИнформация, Символы.ПС);
		
		ПолучитьСведенияОбОбновлении();
		ПараметрыОбращения.ОтборЖурналаРегистрации.Вставить("ДатаНачала", ВремяНачалаОтложенногоОбновления);
		ПараметрыОбращения.ОтборЖурналаРегистрации.Вставить("ДатаОкончания", ВремяОкончанияОтложенногоОбновления);
		ПараметрыОбращения.ОтборЖурналаРегистрации.Вставить("Сеанс", НомерТекущегоСеанса);
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОтложенныеОбработчикиОбработчикДополнение.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОтложенныеОбработчики.ОбработчикДополнение");
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	СтрокаОбработчикаОчисткиУстаревшихДанных = Неопределено;
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Если Не ПустаяСтрока(СтрокаПоиска) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ДобавитьОтложенныйОбработчик(Обработчик, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики,
			НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета, СтрокаОбработчикаОчисткиУстаревшихДанных);
	...
	Если СтрокаОбработчикаОчисткиУстаревшихДанных <> Неопределено Тогда
		ИндексСтроки = ОтложенныеОбработчики.Индекс(СтрокаОбработчикаОчисткиУстаревшихДанных);
		ОтложенныеОбработчики.Сдвинуть(ИндексСтроки, ОтложенныеОбработчики.Количество() - 1 - ИндексСтроки);
		СтрокаОбработчикаОчисткиУстаревшихДанных.ЭтоОбработчикОчисткиУстаревшихДанных = Истина;
		СтрокаОбработчикаОчисткиУстаревшихДанных.ОбработчикДополнение =
			НСтр("ru = 'Посмотреть и очистить устаревшие данные самостоятельно.'");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку") Тогда
		...
		ПараметрОтбора = Новый Структура("Статус", Перечисления.СтатусыОбработчиковОбновления.Ошибка);
		НайденныеОшибки = ОтложенныеОбработчики.НайтиСтроки(ПараметрОтбора);
...
&НаСервере
Процедура ДобавитьОтложенныйОбработчик(СтрокаОбработчика, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики,
			НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета, СтрокаОбработчикаОчисткиУстаревшихДанных)
	...
	КонецЕсли;
	Если ОбновлениеИнформационнойБазыСлужебный.ЭтоОбработчикОчисткиУстаревшихДанных(СтрокаОбработчика) Тогда
		СтрокаОбработчикаОчисткиУстаревшихДанных = СтрокаСписка;
	КонецЕсли;
...
Функция ЗапускУстановкиИсправлений()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"ОбновлениеИнформационнойБазыСлужебный.ЗагрузитьИУстановитьИсправления");
