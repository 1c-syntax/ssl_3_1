...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПустойИдентификатор = Строка(ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	...
	Элементы.ТаблицаРегламентныеЗаданияЗаблокироватьРаботуСВнешнимиРесурсами.Доступность = НЕ РаботаСВнешнимиРесурсамиЗаблокирована;
	
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		
		Элементы.ТаблицаРегламентныеЗаданияОбновитьДанные.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
		Элементы.ТаблицаРегламентныеЗаданияНастроитьРасписание.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "РазрешенаРаботаСВнешнимиРесурсами" Тогда
		...
		Элементы.ТаблицаРегламентныеЗаданияЗаблокироватьРаботуСВнешнимиРесурсами.Доступность = Истина;
	...
	ИначеЕсли ИмяСобытия = "ЗапрещенаРаботаСВнешнимиРесурсами" Тогда
		...
		Элементы.ТаблицаРегламентныеЗаданияЗаблокироватьРаботуСВнешнимиРесурсами.Доступность = Ложь;
		СтандартныеПодсистемыКлиент.УстановитьРасширенныйЗаголовокПриложения();
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Задания не выполнены (%1 из %2)'"),
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	ПоказатьПредупреждение(,
		НСтр("ru = 'Задание отменено, но состояние отмены будет установлено сервером только через некоторое время,
...
Процедура ЗапуститьРегламентноеЗаданиеНеВФоне(ИдентификаторЗадания)
	...
	ИмяЗадания = МетаданныеЗадания.Имя;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступомСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступомСлужебный");
		МодульУправлениеДоступомСлужебный.ПередЗапускомРегламентногоЗаданияНеВФоне(Задание);
	КонецЕсли;
	
	ПараметрыВыполнения = РегламентныеЗаданияСервер.НовыеПараметрыВыполненияЗадания();
	ПараметрыВыполнения.ВФоне = Ложь;
	
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить("РегламентныеЗадания", ИмяМетода, Истина);
	РегламентныеЗаданияСервер.ВыполнитьЗадание(Задание, ПараметрыВыполнения);
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить("РегламентныеЗадания", ИмяМетода, Неопределено);
...
Процедура СобытияЖурналаРегистрации(Команда)
	...
	Если Элементы.Задания.ТекущаяСтраница = Элементы.РегламентныеЗадания Тогда
		ТекущиеДанные = Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные;
		Если ТекущиеДанные = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Выберите регламентное задание.'"));
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ДатаНачала) И Не ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончания)
			Или (ТекущиеДанные.ДатаНачала = ТекстНеОпределено И ТекущиеДанные.ДатаОкончания = ТекстНеОпределено) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Нет событий для просмотра, так как регламентное задание не выполнялось.'"));
			Возврат;
		КонецЕсли;
		ОтборСобытий.Вставить("ДатаНачала", ТекущиеДанные.ДатаНачала);
		Если ТекущиеДанные.ДатаОкончания <> ПредставлениеПустойДаты() Тогда
			ОтборСобытий.Вставить("ДатаОкончания", ТекущиеДанные.ДатаОкончания);
		КонецЕсли;
		ДобавитьОтборПоСеансу(ОтборСобытий, ТекущиеДанные.Идентификатор);
	ИначеЕсли Элементы.Задания.ТекущаяСтраница = Элементы.ФоновыеЗадания Тогда
		ТекущиеДанные = Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные;
		Если ТекущиеДанные = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Выберите фоновое задание.'"));
			Возврат;
		КонецЕсли;
		ОтборСобытий.Вставить("ДатаНачала", ТекущиеДанные.Начало);
		ОтборСобытий.Вставить("ДатаОкончания", ТекущиеДанные.Конец);
		ДобавитьОтборПоСеансу(ОтборСобытий, ТекущиеДанные.ИдентификаторРегламентногоЗадания);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаблокироватьРаботуСВнешнимиРесурсами(Команда)
	
	ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ЗаблокироватьРаботуСВнешнимиРесурсамиЗавершение", ЭтотОбъект);
	ТекстСообщения = НСтр("ru = 'Работа со всеми внешними ресурсами (синхронизация данных, отправка почты и т.п.), выполняемая по расписанию, будет заблокирована.
	|
	|Возможные причины для блокировки:
	|• Ошибочная отметка базы, как перемещенной
	|• Необходимость временной блокировки регламентных заданий'");
	СписокКнопок = Новый СписокЗначений();
	СписокКнопок.Добавить(КодВозвратаДиалога.Да, "Заблокировать");
	СписокКнопок.Добавить(КодВозвратаДиалога.Нет, "Отмена");
	ПоказатьВопрос(ОбработчикОповещенияОЗакрытии, 
		ТекстСообщения, 
		СписокКнопок, , 
		КодВозвратаДиалога.Нет, НСтр("ru = 'Блокировка работы с внешними ресурсами'"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ПредставлениеПустойДаты());
...
Процедура ЗаполнитьНастройкиФормы(Знач Настройки)
	...
	Если Настройки.Получить("ОтборПоСостояниюЗавершено") = Неопределено Тогда
		Настройки.Вставить("ОтборПоСостояниюЗавершено", Ложь);
	КонецЕсли;
	
	Если Настройки.Получить("ОтборПоСостояниюЗавершеноАварийно") = Неопределено Тогда
		Настройки.Вставить("ОтборПоСостояниюЗавершеноАварийно", Ложь);
	КонецЕсли;

	Если Настройки.Получить("ОтборПоСостояниюОтменено") = Неопределено Тогда
		Настройки.Вставить("ОтборПоСостояниюОтменено", Ложь);
	КонецЕсли;
	...
	Если Настройки.Получить("ВидОтбораПоПериоду") = Неопределено Тогда
		
		Настройки.Вставить("ВидОтбораПоПериоду", 3); // за сегодня
...
Процедура УстановитьИспользованиеРегламентногоЗадания(Включено)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки Цикл
		ТекущиеДанные = ТаблицаРегламентныеЗадания.НайтиПоИдентификатору(ВыделеннаяСтрока);
		РегламентныеЗаданияСервер.ИзменитьРегламентноеЗадание(ТекущиеДанные.Идентификатор,
			Новый Структура("Использование", Включено));
...
&НаКлиенте
Процедура ЗаблокироватьРаботуСВнешнимиРесурсамиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		ЗаблокироватьРаботуСВнешнимиРесурсамиНаСервере();
		Оповестить("ЗапрещенаРаботаСВнешнимиРесурсами");
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ЗаблокироватьРаботуСВнешнимиРесурсамиНаСервере()
	РегламентныеЗаданияСервер.ЗаблокироватьРаботуСВнешнимиРесурсами();
КонецПроцедуры

#Область ФоноваяЗагрузкаРегламентныхЗаданий
...
Процедура ЗагрузитьРегламентныеЗадания(ИдентификаторЗадания = Неопределено, ОбновитьНезаметно = Ложь)
	...
	КонецЕсли;
	Результат = ЗагрузитьРегламентныеЗаданияСервер(ИдентификаторЗадания);
...
&НаСервере
Функция ЗагрузитьРегламентныеЗаданияСервер(ИдентификаторЗадания)
...
Процедура ОбработатьРезультат(ПараметрыЗадания)
	...
	УдалитьИзВременногоХранилища(ПараметрыЗадания.АдресРезультата);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеПустойДаты()
	Возврат "<>";
КонецФункции

#КонецОбласти

#Область ФоноваяЗагрузкаФоновыхЗаданий
...
Функция СформироватьТаблицуФоновыхЗаданийВФоне()
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне("РегламентныеЗаданияСлужебный.ЗаполнитьТаблицуСвойствФоновыхЗаданийВФоне",
		ПередаваемыеПараметры, ПараметрыВыполнения);
		
	Если Результат.Статус = "Выполнено" Тогда
		ОбновитьТаблицуФоновыхЗаданий(Результат.АдресРезультата);
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ВызватьИсключение Результат.ИнформацияОбОшибке;
	КонецЕсли;
...
Функция ОтборФоновыхЗаданий()
	...
	Если МассивСостояний.Количество() <> 4 Тогда
		Отбор.Вставить("Состояние", ?(МассивСостояний.Количество() = 1, МассивСостояний[0], МассивСостояний));
	КонецЕсли;
...
Процедура ОбновитьТаблицуФоновыхЗаданий(АдресРезультата = Неопределено)
	...
	Для каждого Задание Из ТекущаяТаблица Цикл
		...
		Если Индекс >= Таблица.Количество()
		 ИЛИ Таблица.Получить(Индекс).Идентификатор <> Задание.Идентификатор Тогда
...
&НаСервереБезКонтекста
Процедура ДобавитьОтборПоСеансу(Отбор, Идентификатор)
	
	Если Идентификатор = РегламентныеЗаданияСлужебный.ТекстНеОпределено() Тогда
		Возврат;
	КонецЕсли;
	
	ОтборЗадания = Новый Структура;
	ОтборЗадания.Вставить("УникальныйИдентификатор", Новый УникальныйИдентификатор(Идентификатор));
	...
	Для Каждого НайденноеЗадание Из НайденныеЗадания Цикл
		РегламентноеЗадание = НайденноеЗадание;
		Прервать;
	КонецЦикла;
	...
	КопияОтбора = ОбщегоНазначения.СкопироватьРекурсивно(Отбор); // Структура
	КопияОтбора.Вставить("Метаданные", РегламентноеЗадание.Метаданные.ПолноеИмя());
	
	Результат = Новый ТаблицаЗначений;
	ВыгрузитьЖурналРегистрации(Результат, КопияОтбора, , , 1);
	Для Каждого Строка Из Результат Цикл
		Сеансы = Новый СписокЗначений;
		Сеансы.Добавить(Строка.Сеанс);
