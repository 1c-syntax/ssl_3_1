...
Процедура ПечатьСоШтампом(Команда)
	...
	Файл = ТекущаяСсылкаНаФайл();
	Если ЗначениеЗаполнено(Файл) Или ОбработатьКомандуЗаписиФайла() Тогда
		ДокументСоШтампом = РаботаСФайламиСлужебныйВызовСервера.ТабличныйДокументСоШтампом(Файл);
...
Процедура Освободить(Команда)
	...
	ОсвободитьФайл();
	ОповеститьОбИзменении(Файл);
	Оповестить("Запись_Файл", Новый Структура("Событие", "РедактированиеОтменено"), Файл);
