...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФайлПеремещенВАрхив = РаботаСФайловымАрхивомСервер.ФайлПеренесенВФайловыйАрхив(ПрисоединенныйФайлОбъект.Ссылка);
	...
	ДоступнаЭлектроннаяПодпись = РаботаСФайламиСлужебный.ДоступнаЭлектроннаяПодпись(ТипЗнч(ПрисоединенныйФайлОбъект));
...
Процедура СтандартныеСкопировать(Команда)
	...
	РаботаСФайламиКлиент.ОткрытьФормуФайла(Неопределено,, ПараметрыФормы);
...
Процедура Печать(Команда)
	...
	Если ЗначениеЗаполнено(Файл) Или ОбработатьКомандуЗаписиФайла() Тогда
		Файлы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущаяСсылкаНаФайл()); // Заново получаем ссылку, если новый файл записан.
...
Процедура ПечатьСоШтампом(Команда)
	...
	Файл = ТекущаяСсылкаНаФайл();
	Если ЗначениеЗаполнено(Файл) Или ОбработатьКомандуЗаписиФайла() Тогда
		РаботаСФайламиСлужебныйКлиент.НапечататьФайлСоШтампом(Файл, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ОтправитьНаПодписание(Команда)
	...
	ПараметрыПодписания = МодульЭлектроннаяПодписьКлиент.НовыйТипПодписи();
	ПараметрыПодписания.ВыборДоверенности = Истина;
	РаботаСФайламиКлиент.ОтправитьФайлНаПодписание(ТекущаяСсылкаНаФайл(), УникальныйИдентификатор, ДополнительныеПараметры, ПараметрыПодписания);
...
Процедура ПродлитьДействиеПодписей(Команда)
	...
	Если Модифицированность Тогда
		ПеренестиРезультатыПроверкиПодписей();
	КонецЕсли;
...
Процедура СохранитьПодпись(Команда)
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаПодписи) Тогда
		...
		ИмяФайлаПодписи = МодульЭлектроннаяПодписьСлужебныйКлиентСервер.ИмяФайлаПодписи(ЭтотОбъект.Объект.Наименование,
...
Процедура УстановитьВидимостьКомандФормы()
	...
	// Обработка внешнего признака ТолькоЧтениеДанныхФайла,
	// см. например Взаимодействия.ПриСозданииФормыЭлементаФайлов.
	...
	Элементы.ФормаУдалить.Видимость = (ЭтотОбъект.Объект.Автор = ТекущийПользователь) И Элементы.ФормаУдалить.Видимость;
...
Процедура ОткрытьФайл()
	...
	ЭтотОбъектОбъект = ЭтотОбъект["Объект"];
	
	ФайлРедактируется = ЗначениеЗаполнено(ЭтотОбъектОбъект.Редактирует) И ЭтотОбъектОбъект.Редактирует = ТекущийПользователь;

	ПараметрыПолученияФайла = РаботаСФайламиКлиент.ПараметрыАсинхронногоПолученияФайла("ОткрытьФайл");
	ПараметрыПолученияФайла.ПрисоединенныйФайл	= ЭтотОбъектОбъект.Ссылка;
...
Процедура СохранитьКак()
	...
	Если ЭтоНовый() Или ЭтотОбъектОбъект.Зашифрован Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопок(Форма, Элементы)
	...
	Если ИменаКоманд.Найти("Редактировать") <> Неопределено Тогда
...
Функция ИменаКомандИзмененияДанныхФайла()
	...
	ИменаЭлементов = Новый Массив;
	ИменаЭлементов.Добавить("ФормаОтправитьНаПодписание");
	ИменаЭлементов.Добавить("ЭлектронныеПодписиОтправитьНаПодписание");
	ИменаКоманд.Вставить("ОтправитьНаПодписание", ИменаЭлементов);
...
Функция ДоступныеКомандыФормы(Форма)
	...
	Если ФайлЗашифрован Тогда
		...
		СделатьКомандуНедоступной(ИменаКоманд, "ОтправитьНаПодписание");
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗначениеКопирования) Тогда
		...
		Попытка
			...
			РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(ЗаписываемыйОбъект, ДвоичныеДанные, РеквизитыЗначенияКопирования.Размер, 
									РеквизитыЗначенияКопирования.Расширение);

			Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ЗаписываемыйОбъект.ТипХраненияФайла) Тогда
				СсылкаНового = Справочники[ИмяСправочника].ПолучитьСсылку();
				ЗаписываемыйОбъект.УстановитьСсылкуНового(СсылкаНового);
				ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(ЗаписываемыйОбъект);
				РаботаСФайламиСлужебный.ЗаписатьФайлВИнформационнуюБазу(СсылкаНового, ДвоичныеДанные, ПараметрыЗаписиВИБ);
			Иначе
				РаботаСФайламиВТомахСлужебный.ДобавитьФайл(ЗаписываемыйОбъект, ДвоичныеДанные,,,ЗаписываемыйОбъект.Том);
			КонецЕсли;
			...
			Если ДоступнаЭлектроннаяПодпись И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
				...
				МодульЭлектроннаяПодпись.ДобавитьПодпись(ЗаписываемыйОбъект, ПодписиОбъекта);				
			КонецЕсли;
...
&НаСервере
Процедура ПеренестиРезультатыПроверкиПодписей()
	РаботаСФайламиСлужебный.ПеренестиРезультатыПроверкиПодписей(ЭлектронныеПодписи, РеквизитФормыВЗначение("Объект").Ссылка);
КонецПроцедуры
