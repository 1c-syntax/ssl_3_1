...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла(ТекущаяСсылкаНаФайл());
...
Процедура СтандартныеСкопировать(Команда)
	...
	ОткрытьФорму(
		"Обработка.РаботаСФайлами.Форма.ПрисоединенныйФайл",
...
Процедура Печать(Команда)
	...
	Если ЗначениеЗаполнено(ТекущаяСсылкаНаФайл()) Или ОбработатьКомандуЗаписиФайла() Тогда
		Файлы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущаяСсылкаНаФайл());
...
Процедура ПечатьСоШтампом(Команда)
	...
	СсылкаНаТекущийОбъект = ТекущаяСсылкаНаФайл();
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект)
		Или ОбработатьКомандуЗаписиФайла() Тогда
		
		ДокументСоШтампом = РаботаСФайламиСлужебныйВызовСервера.ТабличныйДокументСоШтампом(
			СсылкаНаТекущийОбъект, СсылкаНаТекущийОбъект);
...
Процедура Отправить(Команда)
	...
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект)
		Или ОбработатьКомандуЗаписиФайла() Тогда
		Файлы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СсылкаНаТекущийОбъект);
...
Процедура Подписать(Команда)
	...
	РаботаСФайламиКлиент.ПодписатьФайл(
		ТекущаяСсылкаНаФайл(), УникальныйИдентификатор, ДополнительныеПараметры);
...
Процедура Редактировать(Команда)
	...
	ДанныеФайла = ДанныеФайла(СсылкаНаТекущийОбъект, УникальныйИдентификатор);
...
Процедура Освободить(Команда)
	...
	ОсвободитьФайл();
	ОповеститьОбИзменении(СсылкаНаТекущийОбъект);
	Оповестить("Запись_Файл", Новый Структура("Событие", "РедактированиеОтменено"), СсылкаНаТекущийОбъект);
...
Процедура ОткрытьФайлДляПросмотра()
	...
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
		ТекущаяСсылкаНаФайл(), Неопределено, УникальныйИдентификатор);
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Оповестить("Запись_Файл", Новый Структура("ЭтоНовый, Событие", ФайлБылСоздан, "Запись"), ЭтотОбъект.Объект.Ссылка);
...
Процедура ОбновитьИзФайлаНаДискеЗавершение(Результат, ПараметрыВыполнения) Экспорт
	...
	Оповестить("Запись_Файл", Новый Структура, ТекущийФайл);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Если СведенияОСинхронизации.Количество() > 0 Тогда
			...
			Элементы.ДекорацияДатаСинхронизации.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Синхронизирован: <a href=""OpenJournal"">%1</a>'"),
