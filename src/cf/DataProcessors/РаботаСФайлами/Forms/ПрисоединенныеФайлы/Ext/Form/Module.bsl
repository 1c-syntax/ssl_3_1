...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ЭлементФормы Из Элементы Цикл
		...
		Если ИменаКомандФормы.Найти(ЭлементФормы.ИмяКоманды) <> Неопределено
			Или ИменаКомандФормы.Найти(ЭлементФормы.Имя) <> Неопределено Тогда
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если МассивИменФайлов.Количество() > 0 Тогда
		...
		Иначе
			...
			Оповестить("Запись_Файл", Новый Структура("ЭтоНовый, ВладелецФайла", Истина, Параметры.ВладелецФайла),
				МассивИменФайлов);
...
Процедура ПечатьСоШтампом(Команда)
	...
	Если ТекущиеДанные.Расширение = "mxl" Тогда
	...
	ИначеЕсли ТекущиеДанные.Расширение = "docx" Тогда
		ДокументСоШтампом = РаботаСФайламиСлужебныйВызовСервера.ОфисныйДокументСоШтампом(ТекущиеДанные.Ссылка, ТекущиеДанные.Ссылка);
		ФайловаяСистемаКлиент.ОткрытьФайл(ДокументСоШтампом, , ТекущиеДанные.Наименование+".docx");
	КонецЕсли;
КонецПроцедуры
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьДоступныеКоманды(ТекущиеДанныеФайла, ФайлыРедактируютсяВОблачномСервисе, ВозможностьОсвободитьФайл, АвторизованныйПользователь)
	...
	Если ФайлРедактируется Тогда
		...
		УдалитьКомандуИзМассива(ИменаКоманд, "Удалить");
