...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Файл" Тогда
		...
		Если ТипЗнч(СсылкаНаФайл) <> ТипСправочникаСФайлами Тогда
			Если Параметр.Событие = "ФайлыИзмененыПриРаботеСАрхивом" Тогда
				УстановитьДоступностьФайловыхКоманд();
			КонецЕсли;
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			Если ФайловыеКомандыДоступны() И ТекущиеДанные <> Неопределено Тогда
				 Если СсылкаНаФайл = ТекущиеДанные.Ссылка Или Параметр.Событие = "ФайлыИзмененыПриРаботеСАрхивом" Тогда
					УстановитьДоступностьФайловыхКоманд();
				 КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование Тогда
		...
		Если ТекущиеДанные.НомерКартинкиЭтоАрхив = 0 Тогда
			Возврат;
		КонецЕсли;
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	ДанныеСтрокиПриемника = Элемент.ДанныеСтроки(Строка);

	ПеретаскиваемыеЭлементы = ПараметрыПеретаскивания.Значение;
	Если ТипЗнч(ПеретаскиваемыеЭлементы) <> Тип("Массив") Тогда
		ПеретаскиваемыеЭлементы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыПеретаскивания.Значение);
	КонецЕсли;
	
	ПеретаскиваемыеФайлыНаДиске = ПеретаскиваемыеФайлыНаДиске(ПеретаскиваемыеЭлементы);
	Если ЗначениеЗаполнено(ПеретаскиваемыеФайлыНаДиске) Тогда
		СтандартнаяОбработка = Ложь;
		
		ГруппаПриемник = Неопределено;
		Если ЕстьГруппыФайлов Тогда
			Если ЗначениеЗаполнено(Строка) Тогда
				Если ДанныеСтрокиПриемника.ЭтоГруппа Тогда
					ГруппаПриемник = ДанныеСтрокиПриемника.Ссылка;
				Иначе
					ГруппаПриемник = ЗначениеРеквизитаОбъекта(Строка, "Родитель");
				КонецЕсли;
			Иначе
				ГруппаПриемник = Элемент.ТекущийРодитель;
			КонецЕсли;
		КонецЕсли;

		РаботаСФайламиСлужебныйКлиент.ДобавитьФайлыПеретаскиванием(ВладелецФайла, УникальныйИдентификатор, 
			ПеретаскиваемыеФайлыНаДиске, ГруппаПриемник);
		Возврат;
	КонецЕсли;
	
	ПеретаскиваемыеСсылкиИногоВладельца = ПеретаскиваемыеСсылкиИногоВладельца(ПеретаскиваемыеЭлементы);
	Если ЗначениеЗаполнено(ПеретаскиваемыеСсылкиИногоВладельца) Тогда
		Если ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Перемещение Тогда
			РаботаСФайламиСлужебныйВызовСервера.ПеренестиФайлы(ПеретаскиваемыеСсылкиИногоВладельца, ВладелецФайла);
		ИначеЕсли ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Копирование Тогда
			РаботаСФайламиСлужебныйВызовСервера.СкопироватьФайлы(ПеретаскиваемыеСсылкиИногоВладельца, ВладелецФайла);
		Иначе
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
		...
		Оповестить("Запись_Файл", ПараметрыОповещенияЗаписиФайла, ПеретаскиваемыеСсылкиИногоВладельца);
	Иначе
		ЭтоПеретаскиваниеВЭлемент = Истина;
		Если ЕстьГруппыФайлов Тогда
			ЭтоПеретаскиваниеВЭлемент = ЗначениеЗаполнено(Строка) И НЕ ДанныеСтрокиПриемника.ЭтоГруппа;
		КонецЕсли;
		Если ЭтоПеретаскиваниеВЭлемент Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Функция ПеретаскиваемыеСсылкиИногоВладельца(ПеретаскиваемыеЭлементы)
	
	Результат = Новый Массив();
	ЭтоОдинаковыйВладелец = Неопределено;
	Для Каждого ПеретаскиваемыйЭлемент Из ПеретаскиваемыеЭлементы Цикл
		Если ТипЗнч(ПеретаскиваемыйЭлемент) <> ТипСправочникаСФайлами Тогда
			Продолжить;
		КонецЕсли;
		
		Если ЭтоОдинаковыйВладелец = Неопределено Тогда
			ЭтоОдинаковыйВладелец = ВладелецФайла = ЗначениеРеквизитаОбъекта(ПеретаскиваемыйЭлемент, "ВладелецФайла");
			Если ЭтоОдинаковыйВладелец Тогда
				Прервать;
			КонецЕсли;
		КонецЕсли;
		
		Результат.Добавить(ПеретаскиваемыйЭлемент);
	КонецЦикла;
...
&НаКлиенте
Функция ПеретаскиваемыеФайлыНаДиске(ПеретаскиваемыеЭлементы)
	Результат = Новый Массив();
	Для Каждого ПеретаскиваемыйЭлемент Из ПеретаскиваемыеЭлементы Цикл
		ЭтоФайлНаДиске = ТипЗнч(ПеретаскиваемыйЭлемент) = Тип("Файл") И ПеретаскиваемыйЭлемент.ЭтоФайл();
		Если ЭтоФайлНаДиске Тогда
			Результат.Добавить(ПеретаскиваемыйЭлемент.ПолноеИмя);
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого Строка Из Строки Цикл
		Если ТипЗнч(Строка.Ключ) = Тип("СправочникСсылка.Файлы") Тогда
			Возврат;
		Иначе
			Прервать;
		КонецЕсли;
	...
	ПрисоединенныеФайлы = Строки.ПолучитьКлючи();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Файлы", ПрисоединенныеФайлы);
	Запрос.Текст = "ВЫБРАТЬ
	               |	СведенияОФайлах.Файл КАК Файл,
	               |	ВЫБОР
	               |		КОГДА СведенияОФайлах.ДатаПереносаВАрхив = ДАТАВРЕМЯ(1, 1, 1)
	               |			ТОГДА -1
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК НомерКартинкиЭтоАрхив
	               |ИЗ
	               |	РегистрСведений.СведенияОФайлах КАК СведенияОФайлах
	               |ГДЕ
	               |	СведенияОФайлах.Файл В(&Файлы)";
	...
	Пока Выборка.Следующий() Цикл
		Строка = Строки.Получить(Выборка.Файл);
		Строка.Данные["НомерКартинкиЭтоАрхив"] = Выборка.НомерКартинкиЭтоАрхив;
	КонецЦикла;	
...
Процедура ОткрытьФайл()
	...
	Если ТекущиеДанные.ЭтоГруппа Тогда
		ОткрытьКарточкуФайла();
	Иначе
		ОткрытьПрисоединенныйФайл(ДополнительныеПараметры);
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьПрисоединенныйФайл(ПараметрыФайла)

	ТекущиеДанные = ПараметрыФайла.ТекущиеДанные; // см. ОбработкаОбъект.ПоискИУдалениеДублей.ГруппыДублей
	...
	ПараметрыДействия = ПараметрыПолученияФайла.ПараметрыДействия;
	ПараметрыДействия.ОповещатьЕслиЗашифрован	= Истина;
	ПараметрыДействия.ФайлРедактируется			= ФайлРедактируется;
...
Процедура УстановитьПометкуУдаленияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		УстановитьСнятьПометкуУдаления(ДополнительныеПараметры.Файлы, ВладелецФайла);
		ПараметрыОповещенияЗаписиФайла = РаботаСФайламиСлужебныйКлиент.ПараметрыОповещенияЗаписиФайла("ДанныеФайлаИзменены");
...
Процедура НастроитьДинамическийСписок()
	...
	|	&НомерКартинки КАК НомерКартинкиЭтоАрхив
	|ИЗ
	...
	ПолноеИмяСправочника = "Справочник." + ИмяСправочникаХранилищаФайлов; // @query-part
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяСправочника", ПолноеИмяСправочника);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Служебный", ?(ЕстьРеквизитСлужебный, "Файлы.Служебный", "ЛОЖЬ")); // @query-part-2, @query-part-3
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ГруппаФайла", ?(ЕстьГруппыФайлов, "Файлы.Родитель", "НЕОПРЕДЕЛЕНО")); // @query-part-2, @query-part-3
    ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НомерКартинки", 
		?(ИмяСправочникаХранилищаФайлов = "Файлы", 
		"ВЫБОР
		|		КОГДА ЕСТЬNULL(Файлы.ТекущаяВерсия.ДатаПереносаВАрхив, ДАТАВРЕМЯ(1, 1, 1)) = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА -1
		|		ИНАЧЕ 0
		|	КОНЕЦ", "-1"));
	
	СвойстваСписка.ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЭтоГруппа",
		?(ЕстьГруппыФайлов, "Файлы.ЭтоГруппа", "ЛОЖЬ")); // @query-part-2, @query-part-3
...
Функция ИменаКомандИзмененияОбъектов()
	...
	Результат.Вставить("ОтправитьНаПодписание", Истина);
...
Функция ДоступныеКоманды(ТекущиеДанныеФайла, ФайлыРедактируютсяВОблачномСервисе, ВозможностьОсвободитьФайл, 
	...
	ФайлПеремещенВАрхив				   = ТекущиеДанныеФайла.НомерКартинкиЭтоАрхив = 0;
	...
	Если ФайлРедактируется Тогда
		...
		ИменаКоманд["ОтправитьНаПодписание"] = Ложь;
...
&НаСервереБезКонтекста
Процедура УстановитьСнятьПометкуУдаления(Файлы, ВладелецФайла)
	
	ФайлыВладельца = РаботаСФайламиСлужебный.ОтобратьПрисоединенныеФайлыВладельца(Файлы, ВладелецФайла);
...
&НаСервере
Функция ПараметрыНастройкиСинхронизации(Знач ВладелецФайла)
...
Функция ТекущиеДанные()
	Если Не СтандартныеПодсистемыКлиент.ЭтоЭлементДинамическогоСписка(Элементы.Список) 
		Или Элементы.Список.ТекущиеДанные.ВладелецФайла <> ВладелецФайла Тогда
...
Функция ГруппаФайла(Файл)
	Возврат ?(Файл.Метаданные().Иерархический, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Файл, "Родитель"),
		Неопределено);
КонецФункции
...
Функция ВыделенныеСтроки()
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.Список.ВыделенныеСтроки Цикл
		Если ТипЗнч(ВыделеннаяСтрока) <> Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
			...
			Если ДанныеСтроки.ВладелецФайла = ВладелецФайла Тогда
