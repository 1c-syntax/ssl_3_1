...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Заголовок = НСтр("ru = 'Сохранение папки на устройство'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	КаталогДляСохранения = РаботаСФайламиСлужебныйКлиент.КаталогВыгрузки();
...
Процедура ПапкаДляЭкспортаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВыборФайла.Выбрать() Тогда
		КаталогДляСохранения = ВыборФайла.Каталог + ПолучитьРазделительПути();
	КонецЕсли;
...
Процедура СохранитьПапку()
	...
	Если ПустаяСтрока(КаталогДляСохранения) Или КаталогДляСохранения = ПолучитьРазделительПути() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Укажите папку.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ СтрЗаканчиваетсяНа(КаталогДляСохранения, ПолучитьРазделительПути()) Тогда
		КаталогДляСохранения = КаталогДляСохранения + ПолучитьРазделительПути();
	КонецЕсли;
	
	// Проверим - каталог выгрузки существует? если нет - создадим.
	КаталогВыгрузки = Новый Файл(КаталогДляСохранения);
	...
	ПапкаДляЭкспортаПолная = КаталогДляСохранения + Строка(ЧтоСохраняем) + ПолучитьРазделительПути();
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать подкаталог ""%1"" в каталоге ""%2"" по причине:
				           |%3'"),
				Строка(ЧтоСохраняем),
				КаталогДляСохранения,
...
Процедура СохранитьПапкуЗавершение(Результат, ПараметрыВыполнения) Экспорт
	Если Результат.Успех = Истина Тогда
		СохраняемыйПуть = КаталогДляСохранения;
		ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить("ИмяПапкиВыгрузки", "ИмяПапкиВыгрузки",  СохраняемыйПуть);
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Экспорт папки'"),,
		             СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		               ...
		               Строка(ЧтоСохраняем), Строка(КаталогДляСохранения) ) );
