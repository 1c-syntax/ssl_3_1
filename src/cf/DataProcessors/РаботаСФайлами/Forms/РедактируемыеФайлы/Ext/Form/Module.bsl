...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗавершениеРаботыПрограммы = Параметры.ЗавершениеРаботыПрограммы;
	Элементы.ПоказыватьЗанятыеФайлыПриЗавершенииРаботы.Видимость = ЗавершениеРаботыПрограммы;
	Элементы.ГруппаКоманднойПанели.Видимость                     = ЗавершениеРаботыПрограммы;
...
Процедура СохранитьКак(Команда)
	...
	ПараметрыПолученияФайла.ФайлВАрхиве						= Ложь; // Файлы в архиве должны быть недоступны для редактирования.
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранитьМассив(МассивСтруктур, Истина);
	Закрыть();
...
Процедура НастроитьДинамическийСписок()
	...
	Для Каждого ТипСправочника Из МассивТипов Цикл
		...
		Если Не ПравоДоступа("Изменение", МетаданныеСправочника) Тогда
			Продолжить;
		КонецЕсли;
		Если МетаданныеСправочника.Имя <> "ВерсииФайлов" Тогда
			ФрагментЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	Файлы.Редактирует,
			...
			|	Файлы.Размер / 1024
			|ИЗ
			|	&ИмяТаблицы КАК Файлы
			|ГДЕ
			|	Файлы.Редактирует = &Редактирует";
			ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "&ИмяТаблицы", МетаданныеСправочника.ПолноеИмя());
			Если Не ПустаяСтрока(ТекстЗапроса) Тогда
				ТекстЗапроса = ТекстЗапроса + "
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|";
				ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "ВЫБРАТЬ РАЗРЕШЕННЫЕ", "ВЫБРАТЬ"); // @query-part-1, @query-part-2
			КонецЕсли;
