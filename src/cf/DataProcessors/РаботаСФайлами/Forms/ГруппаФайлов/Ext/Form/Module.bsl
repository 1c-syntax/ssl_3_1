...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНоваяГруппа Тогда
		...
		ОбъектЗначение.ДатаСоздания  = ТекущаяДатаСеанса();
	...
	МетаданныеСправочника = ОбъектЗначение.Метаданные();
	ИмяСправочника = МетаданныеСправочника.Имя;

	НастроитьОбъектФормы(ОбъектЗначение);

	РазрешеноРедактирование = НЕ ТолькоПросмотр
		И ПравоДоступа("Изменение", МетаданныеСправочника)
		И ПравоДоступа("Изменение", ОбъектЗначение.ВладелецФайла.Метаданные());
	Если РазрешеноРедактирование Тогда
		Если ЗначениеЗаполнено(ОбъектЗначение.Ссылка) Тогда
			ЗаблокироватьДанныеДляРедактирования(ОбъектЗначение.Ссылка, , УникальныйИдентификатор);
		КонецЕсли;
	Иначе
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект)
	...
	Если Параметры.Родитель <> Неопределено
			И ТипЗнч(Параметры.Родитель) = ТипЗнч(НовыйОбъект.Ссылка) Тогда
