...
Процедура ЛокальныйКаталогФайловЭкспортаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЕстьБСП Тогда
		...
		МодульФайловаяСистемаКлиент = Вычислить("ФайловаяСистемаКлиент"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только имя общего модуля.
		Если ТипЗнч(МодульФайловаяСистемаКлиент) = Тип("ОбщийМодуль") Тогда
			МодульФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьРазрешенияНаДоступКВнешнимРесурсам(ЗакрытьФорму)
	...
	Если ВнешниеРесурсыРазрешены <> Истина Тогда
		...
		Если ЕстьПрофилиБезопасности Тогда
			...
			МодульРаботаВБезопасномРежимеКлиент = Вычислить("РаботаВБезопасномРежимеКлиент"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только имя общего модуля.
