...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ПКО.ЕстьОбработчикПослеВыгрузкиВФайл Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(76, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, Источник, "ЕстьОбработчикПослеВыгрузкиВФайл");
		КонецПопытки;				
...
Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных(Свойства, ИмяТипа, 
	...
	Если ИмяТипа = "Справочник"
		...
		Запрос.Текст =
		"ВЫБРАТЬ // АвтозаменаРазрешенные
		| * // АвтозаменаПолей
		|ИЗ
		|	&ИмяТаблицыМетаданных КАК ОбъектДляВыгрузки";
		
		Если НЕ ПустаяСтрока(СтрокаРазрешения) Тогда
			
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "// АвтозаменаРазрешенные", СтрокаРазрешения);
			
		КонецЕсли;
		
		Если СокрЛП(СтрокаВыбораПолей) <> "*" Тогда
			
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "* // АвтозаменаПолей", СтрокаВыбораПолей);
		...
		Для Каждого СтрокаТаблицы Из КоллекцияПравилаКонвертации() Цикл
			...
			Если Источник <> Неопределено Тогда
				...
				Попытка
					...
					КонецЕсли;
				Исключение
					ЗаписатьИнформациюОбОшибкеВПротокол(11, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Строка(Источник));
				КонецПопытки;
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ПКГС.ЕстьОбработчикПередОбработкойВыгрузки Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПКС(48, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, ПКГС,
	...
	Иначе  // это простая группировка
		...
		Если ПКГС.ЕстьОбработчикПослеОбработкиВыгрузки Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(49, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, ПКГС,
	...
	ИначеЕсли ПКГС.ПолучитьИзВходящихДанных Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПКС(66, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, ПКГС, Источник,,,Ложь);
	...
	Для Каждого ОбъектКоллекции Из КоллекцияОбъектов Цикл
		...
		Если ПКГС.ЕстьОбработчикПередВыгрузкой Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(50, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, ПКГС,
		...
		Если ПКГС.ЕстьОбработчикПриВыгрузке Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(51, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, ПКГС,
		...
		Если ПКГС.ЕстьОбработчикПослеВыгрузки Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(52, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, ПКГС,
...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено, Свойства = Неопределено)
	...
	Если ЭтоВыгрузкаРегистра Тогда
		...
		Если НЕ ПустаяСтрока(СтрокаРазрешения) Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "// АвтозаменаРазрешенные", СтрокаРазрешения);
	...
	Иначе
		...
		Если Правило.ВыбиратьДанныеДляВыгрузкиОднимЗапросом <> Истина Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "* // АвтозаменаПолей", "ОбъектДляВыгрузки.Ссылка КАК Ссылка");
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилКонвертации, СтруктураДляУдаленияРегистрацииИзменений)
				...
				Если ПоследняяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если Не ПустаяСтрока(ПоследняяСтрокаПравилаВыгрузки.ПослеОбработки) Тогда
						...
						Исключение
							...
							ЗаписатьИнформациюОбОшибкеОбработчикиПВД(32, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПоследняяСтрокаПравилаВыгрузки.Имя, "ПослеОбработкиВыгрузкиДанных");
				...
				Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если Не ПустаяСтрока(ТекущаяСтрокаПравилаВыгрузки.ПередОбработкой) Тогда
						...
						Исключение
							...
							ЗаписатьИнформациюОбОшибкеОбработчикиПВД(31, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ТекущаяСтрокаПравилаВыгрузки.Имя, "ПередОбработкойВыгрузкиДанных");
	...
	Исключение
		...
		ЗП = ПолучитьСтруктуруЗаписиПротокола(72, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			ИначеЕсли ИмяУзла = "Алгоритм" Тогда
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Исключение
						...
						ЗП = ПолучитьСтруктуруЗаписиПротокола(39, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция РаспаковатьZipФайл(ИмяФайлаДляРаспаковки)
	...
	Исключение
		...
		ЗП = ПолучитьСтруктуруЗаписиПротокола(2, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД, ИмяТипа, Менеджер, ПрефиксИмениТипа, ВозможенПоискПоПредопределенным = Ложь)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Истина // АвтозаменаУсловия", "");
