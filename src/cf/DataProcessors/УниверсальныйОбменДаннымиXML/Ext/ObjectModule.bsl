...
#Область ВспомогательныеПеременныеМодуляДляНаписанияАлгоритмовОбщиеДляВыгрузкиИЗагрузки
...
#Область ФлагиНаличияГлобальныхОбработчиковСобытий
...
#Область ПеременныеОбработокОбменаОбщиеДляВыгрузкиИЗагрузки
...
#Область ПеременныеМодуляОбработкиВыгрузки
...
#Область ПеременныеМодуляОбработкиЗагрузки
...
#Область ПеременныеДляХраненияМодулейСтандартныхПодсистем
...
Функция КопироватьУзел(Узел) Экспорт
	...
	Если ВестиДополнительныйКонтрольЗаписиВXML Тогда
		...
		Стр = ЗаменитьНедопустимыеСимволыXML(Стр, "");
...
Процедура УстановитьАтрибут(Узел, Имя, Значение) Экспорт
	...
	Если ВестиДополнительныйКонтрольЗаписиВXML Тогда
		...
		СтрокаЗаписи = ЗаменитьНедопустимыеСимволыXML(СтрокаЗаписи, "");
...
Процедура УдалениеОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(Правило.ПередУдалением) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(29, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Исключение
		...
		ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(24, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			Правило.Имя, Объект, "");
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если ПКО.ЕстьОбработчикПередВыгрузкой Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(41, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если ПКО.ЕстьОбработчикПриВыгрузке Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(42, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если ПКО.ЕстьОбработчикПослеВыгрузки Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(43, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если ПКО.ЕстьОбработчикПослеВыгрузкиВФайл Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(76, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
//  Уровень - Число
//  Выравнивание - Число
//  БезусловнаяЗаписьВПротоколОбмена - Булево
// 
// Возвращаемое значение:
//  Строка
//
			...
			Иначе
				...
				Если Не ПустаяСтрока(Значение) Тогда
					...
					Исключение
						...
						ЗаписатьИнформациюОбОшибкеВПротокол(11, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Процедура ОчиститьДанныеПоПравилу(Правило)
	...
	Если Не ПустаяСтрока(Правило.ПередОбработкой) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(27, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если Не ПустаяСтрока(Правило.ПослеОбработки) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(28, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип)
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеВПротокол(26, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			Объект, Тип);
...
//
// Возвращаемое значение:
//  - ЛюбаяСсылка - новый или найденный объект информационной базы.
//  - Неопределено - если не удалось создать объект
//  
...
Функция ПрочитатьОбъект()
	...
	Если ЕстьОбработчикПередЗагрузкой Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(19, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Пока ФайлОбмена.Прочитать() Цикл
			...
			Если НЕ ЭтоПараметрДляОбъекта
				...
				Если ЕстьОбработчикПриЗагрузке Тогда
					...
					Исключение
						...
						ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(20, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			Если ЕстьОбработчикПослеЗагрузки Тогда
				...
				Исключение
					УдалитьИзСтекаНеЗаписанныхОбъектов(НПП, ГНПП);
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(21, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ПКГС.ЕстьОбработчикПередОбработкойВыгрузки Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПКС(48, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Иначе  // это простая группировка
		...
		Если ПКГС.ЕстьОбработчикПослеОбработкиВыгрузки Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(49, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	ИначеЕсли ПКГС.ПолучитьИзВходящихДанных Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПКС(66, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Для Каждого ОбъектКоллекции Из КоллекцияОбъектов Цикл
		...
		Если ПКГС.ЕстьОбработчикПередВыгрузкой Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(50, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
		...
		Если ПКГС.ЕстьОбработчикПриВыгрузке Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(51, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
		...
		Если ПКГС.ЕстьОбработчикПослеВыгрузки Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(52, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Процедура ПолучитьЗначениеСвойства(Значение, ОбъектКоллекции, ПКО, ПКС, ВходящиеДанные, Источник)
	...
	Исключение
		...
		Если КодОшибки <> 14 Тогда
			ЗаписатьИнформациюОбОшибкеОбработчикиПКС(КодОшибки, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ПКО, ПКС, Источник, "");
		КонецЕсли;
...
Процедура ВыгрузитьСубконто(Источник,
	...
	Если ПКС.ЕстьОбработчикПередВыгрузкой Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПКС(55, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Для каждого КлючИЗначение Из Значение Цикл
		...
		Если ПКС.ЕстьОбработчикПриВыгрузке Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(56, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
		...
		Если ПКС.ЕстьОбработчикПослеВыгрузки Тогда
			...
			Исключение
				...
				ЗаписатьИнформациюОбОшибкеОбработчикиПКС(57, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			...
			ИмяОбработчика = НСтр("ru = '%1 (глобальный)'");
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
				Правило.Имя,
	...
	Если Не ПустаяСтрока(Правило.ПередВыгрузкой) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(33, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			...
			ИмяОбработчика = НСтр("ru = '%1 (глобальный)'");
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				Правило.Имя, 
	...
	Если Не ПустаяСтрока(Правило.ПослеВыгрузки) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(34, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если Не ПустаяСтрока(Правило.ПередОбработкой) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(31, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если Не ПустаяСтрока(Правило.ПослеОбработки) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(32, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
// Параметры:
//   Данные - КонстантаМенеджерЗначения
//          - СправочникОбъект
//          - ДокументОбъект
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			...
			ИмяОбработчика = НСтр("ru = '%1 (конвертация)'");
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьZipФайла(ИмяАрхива, ПарольДляСжатияФайлаОбмена, НСтр("ru = 'Файл обмена данными'"));
		Архиватор.Добавить(ИмяФайлаОбмена);
		Архиватор.Записать();
		
		УдалитьФайлы(ИмяФайлаОбмена);
		
		ИмяФайлаОбмена = ИмяАрхива;
	...
	Попытка
		...
		Если Не Отказ Тогда
			...
			Исключение
				...
				ИмяОбработчика = НСтр("ru = '%1 (конвертация)'");
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
			...
			Если Не Отказ Тогда
				...
				Исключение
					...
					ИмяОбработчика = НСтр("ru = '%1 (конвертация)'");
					ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
...
Процедура ИнициализацияРеквизитовИМодульныхПеременных()
	...
	Попытка
		МодульДатыЗапретаИзменения = ВычислитьВыражение("ДатыЗапретаИзменения");
	Исключение
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = ПодставитьПараметрыВСтроку(Текст, Метаданные().Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
...
#Область ПроцедурыИФункцииИзБазовойФункциональностиДляОбеспеченияАвтономности
