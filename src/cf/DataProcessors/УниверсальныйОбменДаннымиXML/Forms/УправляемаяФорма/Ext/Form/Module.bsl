...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru = 'Универсальный обмен данными в формате XML (%1)'");
	ЗаголовокФормы = СтрШаблон(ЗаголовокФормы, ВерсияОбъектаНаСервере());
...
#Если ВебКлиент Тогда
		...
		ЭтоКлиент = Истина;
		Элементы.РежимОбработки.Доступность = Ложь;
#КонецЕсли
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайла, 
			ПодготовленныеПараметрыДиалогаПомещенияФайлов(), УникальныйИдентификатор);	
		
	Иначе
		
		ДополнительныеПараметры = ДополнительныеПараметрыОбработкиОповещения();
		
		ДополнительныеПараметры.АдресФайлаНаСервере = АдресФайла;
		
		ПолучитьИнформациюОФайлеОбменаЗавершение(Истина, ДополнительныеПараметры);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайлаОбменаВХранилище, 
			ПодготовленныеПараметрыДиалогаПомещенияФайлов(), УникальныйИдентификатор);
	...
	Иначе
		...
		ДополнительныеПараметры.АдресФайлаНаСервере = АдресФайлаПравилВХранилище;
		
		НастройкаОтладкиЗагрузкиЗавершение(Истина, ДополнительныеПараметры);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.'"));
		Возврат;
	КонецЕсли;
	
	Если ЭтоКлиент Тогда
		
		Если Не ПустаяСтрока(АдресФайлаПравилВХранилище)
			И ЭтоАдресВременногоХранилища(АдресФайлаПравилВХранилище) Тогда
			УдалитьИзВременногоХранилища(АдресФайлаПравилВХранилище);
			АдресФайлаПравилВХранилище = "";
		КонецЕсли;
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПрочитатьПравилаОбменаЗавершение", ЭтотОбъект); 
				
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайлаПравилВХранилище, 
			ПодготовленныеПараметрыДиалогаПомещенияФайлов(), УникальныйИдентификатор);
	...
	Иначе
		...
		ПрочитатьПравилаОбменаЗавершение(Истина, ДополнительныеПараметры);
...
Процедура НастройкаОтладкиЗагрузкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") 
		И ДополнительныеПараметры.Свойство("АдресФайлаНаСервере") Тогда
		
		Адрес = ДополнительныеПараметры.АдресФайлаНаСервере;
	...
	Объект.ИмяФайлаОбмена = ИмяФайлаНаСервереИлиКлиенте(ИмяФайлаОбмена ,Адрес);
...
&НаКлиенте
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") 
		...
		АдресФайлаПравилВХранилище = ДополнительныеПараметры.АдресФайлаНаСервере;
		
	Иначе
		
		АдресФайлаПравилВХранилище = Результат.Адрес;
		ИмяВыбранногоФайла = Результат.СсылкаНаФайл.Имя;
		
	КонецЕсли;
	
			
	ВыполнитьЗагрузкуПравилОбмена(АдресФайлаПравилВХранилище, ИмяВыбранногоФайла);
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Если ЭтоКлиент Тогда
		...
		Объект.ИмяФайлаОбмена = ПолучитьИмяВременногоФайла(".xml"); // АПК:441 удаление в ВыполнитьВыгрузкуНаСервере()
		ДвоичныеДанные.Записать(Объект.ИмяФайлаОбмена);
...
Процедура ВыполнитьЗагрузкуИзФормы()
	...
	Иначе
		...
		ВыполнитьЗагрузкуИзФормыЗавершение(Истина, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ВыполнитьЗагрузкуИзФормыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьЗагрузкуНаСервере(Адрес, ИмяВыбранногоФайла);
...
&НаКлиенте
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		Если ЭтоАдресВременногоХранилища(АдресФайлаПравилЗагрузкиВХранилище) Тогда
			...
			Объект.ИмяФайлаПравилОбмена = ПолучитьИмяВременногоФайла("xml"); // АПК:441 удаление в ЗагрузитьПравилаОбменаИПараметрыНаСервере()
			ДвоичныеДанные.Записать(Объект.ИмяФайлаПравилОбмена);
		Иначе
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Если ЭтоКлиент Тогда
		...
		Если СоздатьНовый Тогда
			...
			ИмяФайла = ПолучитьИмяВременногоФайла(Расширение); // АПК:441 удаление в ЗагрузитьПравилаОбменаИПараметрыНаСервере() или ВыполнитьВыгрузкуНаСервере()
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ПолучитьФайлВыгрузкиНаКлиенте(АдресФайлаДанныхВХранилище);
...
&НаКлиенте
Процедура ПолучитьФайлВыгрузкиНаКлиенте(Адрес)
	
	ИмяСохраняемогоФайла = НСтр("ru = 'Файл выгрузки'");
	
	ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	
	ДиалогВыбора.Заголовок = НСтр("ru = 'Выберите путь для сохранения файла выгрузки'");
	ДиалогВыбора.Расширение = ?(Объект.АрхивироватьФайл, "zip", "xml");
	ДиалогВыбора.Фильтр = ?(Объект.АрхивироватьФайл, "(*.zip)|*.zip", "(*.xml)|*.xml");
	
    Оповещение = Новый ОписаниеОповещения;
	
	ПолучаемыеФайлы = Новый Массив;
	
	ПолучаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(ИмяСохраняемогоФайла, Адрес));
	
	НачатьПолучениеФайлов(Оповещение, ПолучаемыеФайлы, ДиалогВыбора);
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка Тогда
		...
		АдресФайлаДанных = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ОбъектДляСервера.ИмяФайлаОбмена), 
...
Процедура ОткрытьФормуНастройкиОтладкиОбработчиковЗавершение(ПараметрыОтладки, ОбработчикиСобытийИзФайлаПравил) Экспорт
	...
	Если ПараметрыОтладки <> Неопределено Тогда
		...
		Если ЭтоКлиент Тогда
			...
			Оповещение = Новый ОписаниеОповещения("ОткрытьФормуНастройкиОтладкиОбработчиковЗавершениеУдалениеФайлов", ЭтотОбъект);
...
Процедура ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	ЧастиВерсии = СтрРазделить(Информация.ВерсияПриложения, ".");
	Если Не (Не (ЧастиВерсии.Количество() < 2 Или Число(ЧастиВерсии[0]) < 8 Или Число(ЧастиВерсии[1]) < 3)
		...
		ВызватьИсключение НСтр("ru = 'Обработка предназначена для запуска на версии платформы
			|1С:Предприятие 8.3.3 с отключенным режимом совместимости или выше'");
...
#КонецЕсли
	...
	Иначе 
		...
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайла, ИмяФайлаПравилЗагрузки, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ПоместитьФайлПравилЗагрузкиВХранилищеЗавершить(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Или Результат.ПомещениеФайлаОтменено Тогда
		Возврат;
	КонецЕсли;
	
	АдресФайлаПравилЗагрузкиВХранилище = Результат.Адрес;
...
&НаКлиенте
Функция ПодготовленныеПараметрыДиалогаПомещенияФайлов()
	
	ПараметрыДиалогаПомещения = Новый ПараметрыДиалогаПомещенияФайлов;
	
	ПараметрыДиалогаПомещения.МножественныйВыбор = Ложь;
	ПараметрыДиалогаПомещения.Фильтр = "Все файлы (*.*)|*.*";
	ПараметрыДиалогаПомещения.Заголовок = НСтр("ru = 'Выберите файл для загрузки'");
	
	Возврат ПараметрыДиалогаПомещения;
	
КонецФункции 

// Возвращаемое значение:
//   Структура:
//     * АдресФайлаНаСервере - Строка
//
&НаКлиенте
Функция ДополнительныеПараметрыОбработкиОповещения()
	
	Структура = Новый Структура; 
	
	Структура.Вставить("АдресФайлаНаСервере", "");
