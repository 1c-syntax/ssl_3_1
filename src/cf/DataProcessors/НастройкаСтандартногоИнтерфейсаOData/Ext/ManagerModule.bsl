...
Функция МодельДанныхПредоставляемыхДляСтандартногоИнтерфейсаOData() Экспорт
	...
	ОбъектыВключаемыеВСтандартныйИнтерфейсаOData = ОбъектыВключаемыеВСтандартныйИнтерфейсаOData();
	Для Каждого ИсключаемыйОбъект Из ОбъектыИсключаемыеИзСтандартногоИнтерфейсаOData() Цикл
		Если ОбъектыВключаемыеВСтандартныйИнтерфейсаOData.Найти(ИсключаемыйОбъект) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	СвойстваРазделенияДанных = Неопределено;
	...
	Для Каждого Элементы Из ИнтерфейсODataСлужебныйПовтИсп.ОписаниеМоделиДанныхКонфигурации() Цикл
		Для Каждого КлючИЗначение Из Элементы.Значение Цикл
			...
			Если Исключаемые[ОписаниеОбъекта.ПолноеИмя] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если Не ЭтоРазделенныйОбъект(ОписаниеОбъекта, СвойстваРазделенияДанных) Тогда
				Продолжить;
			КонецЕсли;
			ЗаполнитьМодельДанныхПредоставляемыхДляСтандартногоИнтерфейсаOData(Результат, ОписаниеОбъекта.ПолноеИмя, 
				Исключаемые, СвойстваРазделенияДанных);
		КонецЦикла;
	КонецЦикла;
...
Функция ЭтоРазделенныйОбъект(Знач ОписаниеОбъекта, СвойстваРазделенияДанных = Неопределено)
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ВыгрузкаЗагрузкаДанных") Тогда
		Если СвойстваРазделенияДанных = Неопределено Тогда
			МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса"); // АПК:1386 Базовая функциональность БТС

			СвойстваРазделенияДанных = Новый Структура("РазделительОсновныхДанных,РазделительВспомогательныхДанных");
			СвойстваРазделенияДанных.РазделительОсновныхДанных = МодульРаботаВМоделиСервиса.РазделительОсновныхДанных();
			СвойстваРазделенияДанных.РазделительВспомогательныхДанных = МодульРаботаВМоделиСервиса.РазделительВспомогательныхДанных();
		КонецЕсли;
		
		Возврат (ОписаниеОбъекта.РазделениеДанных.Свойство(СвойстваРазделенияДанных.РазделительОсновныхДанных)
				Или ОписаниеОбъекта.РазделениеДанных.Свойство(СвойстваРазделенияДанных.РазделительВспомогательныхДанных));
	КонецЕсли;
	...
	Если ЭтоСсылочныеДанные(ОбъектМетаданных) 
		...
		Возврат ПравоДоступа("Изменение", ОбъектМетаданных, Метаданные.Роли.УдаленныйДоступOData);
	КонецЕсли;
	
	Если ОбщегоНазначения.ЭтоЖурналДокументов(ОбъектМетаданных)
			ИЛИ ОбщегоНазначения.ЭтоПеречисление(ОбъектМетаданных) Тогда
			
		Возврат ПравоДоступа("Чтение", ОбъектМетаданных, Метаданные.Роли.УдаленныйДоступOData);
	КонецЕсли;
...
Функция ТипыИзОписанийТипов(ОписанияТипов)
...
Процедура ЗаполнитьМодельДанныхПредоставляемыхДляСтандартногоИнтерфейсаOData(Знач Результат, Знач ПолноеИмя, 
	Знач Исключаемые, СвойстваРазделенияДанных)
	...
	ЭтоРазделенныйОбъектМетаданных = ЭтоРазделенныйОбъект(СвойстваОбъектаМетаданных, СвойстваРазделенияДанных);
	ОбъектМетаданныхДоступенТолькоДляЧтения = ОбъектМетаданныхДоступенТолькоДляЧтения(ОбъектМетаданных);
	...
	ИначеЕсли ОбщегоНазначения.ЭтоЖурналДокументов(ОбъектМетаданных) Тогда
		Строка.Изменение = Ложь;
	Иначе
		Строка.Изменение = ЭтоРазделенныйОбъектМетаданных И Не ОбъектМетаданныхДоступенТолькоДляЧтения;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СвойстваОбъектаМетаданных.Зависимости Цикл
		...
		Если ЗависимыйОбъектМетаданных <> Неопределено И ЭтоРазделенныйОбъект(ЗависимыйОбъектМетаданных) 
			И Не ОбщегоНазначения.ЭтоПеречисление(ОбъектМетаданных) И Не ОбъектМетаданныхДоступенТолькоДляЧтения(ОбъектМетаданных) Тогда
			Продолжить;
		КонецЕсли;
		...
		ЗаполнитьМодельДанныхПредоставляемыхДляСтандартногоИнтерфейсаOData(Результат, ПолноеИмяЗависимости, 
			Исключаемые, СвойстваРазделенияДанных);
...
Функция ОбъектыИсключаемыеИзСтандартногоИнтерфейсаOData()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ВыгрузкаЗагрузкаДанных") Тогда
		...
		ИсключаемыеТипыИзВыгрузкиЗагрузки = МодульВыгрузкаЗагрузкаДанныхСлужебныйСобытия.ПолучитьТипыИсключаемыеИзВыгрузкиЗагрузки();
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеТипы, ИсключаемыеТипыИзВыгрузкиЗагрузки);
		
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.РаботаСФайламиВМоделиСервиса") Тогда
			МодульРаботаСФайламиСлужебныйВМоделиСервисаПовтИсп = ОбщегоНазначения.ОбщийМодуль("РаботаСФайламиСлужебныйВМоделиСервисаПовтИсп");
			ОбъектыХранения = МодульРаботаСФайламиСлужебныйВМоделиСервисаПовтИсп.СправочникиФайловИОбъектыХранения().ОбъектыХранения;
			МодульВыгрузкаЗагрузкаДанных = ОбщегоНазначения.ОбщийМодуль("ВыгрузкаЗагрузкаДанных");
			ТипыДляУдаленияИзИсключения = Новый Массив;
			Для Каждого ОбъектХранения Из ОбъектыХранения Цикл
				МодульВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки(
					ТипыДляУдаленияИзИсключения,
					ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОбъектХранения.Ключ),
					МодульВыгрузкаЗагрузкаДанных.ДействиеСоСсылкамиНеИзменять());
			КонецЦикла;
			ТипыДляУдаленияИзИсключения = ТипыИзОписанийТипов(ТипыДляУдаленияИзИсключения);
			ИсключаемыеТипы = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ИсключаемыеТипы, ТипыДляУдаленияИзИсключения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьВозможностьСозданияПользователяДляВызововСтандартногоИнтерфейсаOData()
	...
	Если КоличествоПользователей = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в приложении отсутствуют другие пользователи.'");
	КонецЕсли;
...
Функция ОбъектыВключаемыеВСтандартныйИнтерфейсаOData()
	
	ВключаемыеТипы = Новый Массив;
	ВключаемыеТипы.Добавить(Метаданные.Справочники.ИдентификаторыОбъектовРасширений);
	ВключаемыеТипы.Добавить(Метаданные.РегистрыСведений.ИдентификаторыОбъектовВерсийРасширений);
	
	Возврат ВключаемыеТипы;
	
КонецФункции

Функция ОбъектМетаданныхДоступенТолькоДляЧтения(ОбъектМетаданных)
	
	Возврат ОбъектыВключаемыеВСтандартныйИнтерфейсаOData().Найти(ОбъектМетаданных) <> Неопределено;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПодготовитьПараметрыНастройкиСоставаСтандартногоИнтерфейсаOData", Истина);
