...
#Область НастройкаКомпонентовОбменаНаРаботуСУзлом
	ОбменДаннымиСлужебный.ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелДляОбмена, "Отправка");
	
	ОбменДаннымиОценкаПроизводительности.Инициализировать(КомпонентыОбмена);
	
	КомпонентыОбмена.ТолькоНастройкиXDTO = Не ОбменДаннымиСервер.НастройкаСинхронизацииЗавершена(КомпонентыОбмена.УзелКорреспондента);
	Если КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
		
		ЭтоУзелОбъектнойКонвертации = ОбменДаннымиПовтИсп.СпособКонвертацииОбъектовXDTO(КомпонентыОбмена.УзелКорреспондента);
		Если ЭтоУзелОбъектнойКонвертации = Истина Тогда
			
			// Если настройки не отправлялись, то отправляем настройки
			// иначе отправляем данные. Признак отправки данных - у нас есть поддерживаемые объекты от корреспондента,
			// значит мы получили файл настроек от корреспондента.
			НастройкиКорреспондента = 
				ОбменДаннымиXDTOСервер.ПоддерживаемыеОбъектыФорматаКорреспондента(КомпонентыОбмена.УзелКорреспондента, "ОтправкаПолучение");
			
			КомпонентыОбмена.ТолькоНастройкиXDTO = НЕ (НастройкиКорреспондента.Количество() > 0);
	...
	Если Не КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
		...
		КомпонентыОбмена.ВерсияФорматаОбмена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КомпонентыОбмена.УзелКорреспондента, "ВерсияФорматаОбмена");
		...
		Иначе
			Если Не КомпонентыОбмена.ФлагОшибки Тогда
				...
				Иначе
					...
					ИмяВременногоФайла = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, Строка(Новый УникальныйИдентификатор) + ".xml");
...
Процедура ВыполнитьЗагрузкуДанных(ПараметрыЗагрузки = Неопределено) Экспорт
	...
	ОбменДаннымиСлужебный.ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелОбменаЗагрузкаДанных, "Получение");
	ПослеИнициализацииКомпонентыОбменаПриПолучении(КомпонентыОбмена, Истина, Истина);
	...
	Если ПараметрыЗагрузки.Свойство("КлючОбрабатывающегоЗадания", КлючОбрабатывающегоЗадания) Тогда
		КомпонентыОбмена.КлючОбрабатывающегоЗадания = ПараметрыЗагрузки.КлючОбрабатывающегоЗадания;
	КонецЕсли;
...
#Если ВнешнееСоединение Тогда
	РегистрыСведений.АрхивСообщенийОбменов.ПоместитьСообщениеВАрхив(
		УзелОбменаЗагрузкаДанных, ИмяФайлаОбмена);
#КонецЕсли
	...
	ОбменДаннымиОценкаПроизводительности.ЗавершитьЗамер(
			ВремяНачала, "ОткрытиеФайлаСообщенияОбмена", "", КомпонентыОбмена, 
	...
	ОбменДаннымиОценкаПроизводительности.ЗавершитьЗамер(
			ВремяНачала, "ПослеОткрытияФайлаОбмена", "", КомпонентыОбмена, 
			ОбменДаннымиОценкаПроизводительности.ТипСобытияБиблиотека());
	...
	Исключение
		...
		ШаблонТекстаСообщения = НСтр("ru = 'Ошибка при загрузке данных: %1'");
		СтрокаСообщения = СтрШаблон(ШаблонТекстаСообщения, ОписаниеОшибки);
		ОбменДаннымиXDTOСервер.ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщения, , , , , Истина);
		
		Если КомпонентыОбмена.Свойство("ОбъектнаяКонвертацияXDTO")
			И КомпонентыОбмена.ОбъектнаяКонвертацияXDTO = Истина Тогда
			
			ОтразитьОшибкуЗагрузкиДанных(КомпонентыОбмена, СтрокаСообщения);
...
//                         ТаблицыДляЗагрузки.Добавить("СправочникСсылка.Валюты");
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	ПослеИнициализацииКомпонентыОбменаПриПолучении(КомпонентыОбмена, Ложь, Истина);
	ОбменДаннымиСлужебный.ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелОбменаЗагрузкаДанных, "Получение");
...
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	ПослеИнициализацииКомпонентыОбменаПриПолучении(КомпонентыОбмена, Истина, Ложь, Истина);
	ОбменДаннымиСлужебный.ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелОбменаЗагрузкаДанных, "Получение");
...
Процедура ПоместитьСообщениеДляСопоставленияДанных(ДанныеВыгрузкиXML) Экспорт
	...
	ПослеИнициализацииКомпонентыОбменаПриПолучении(КомпонентыОбмена, Истина, Ложь);
	ОбменДаннымиСлужебный.ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелОбменаЗагрузкаДанных, "Получение");
...
Процедура ЗагрузитьДанныеСообщенияОбмена(ДанныеВыгрузкиXML) Экспорт
	...
	КаталогВременногоХранилищаФайлов = ОбменДаннымиСервер.КаталогВременногоХранилищаФайлов();
	ИмяФайлаОбмена = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, Строка(Новый УникальныйИдентификатор) + ".xml");
...
Процедура ОтразитьОшибкуЗагрузкиДанных(КомпонентыОбмена, Знач ТекстОшибки)
	
	ВремяНачала = ОбменДаннымиОценкаПроизводительности.НачатьЗамер();
	
	Если КомпонентыОбмена.Свойство("ИсторияПримененияПКО")
		И ЗначениеЗаполнено(КомпонентыОбмена.ИсторияКонвертацииОбъекта.ПолученныйОбъект)
		Тогда
		
		ПолученныйОбъект = КомпонентыОбмена.ИсторияКонвертацииОбъекта.ПолученныйОбъект;
		ОбъектФормата    = КомпонентыОбмена.ИсторияКонвертацииОбъекта.ОбъектФормата;
	...
	КомпонентыОбмена.ИсторияКонвертацииОбъекта.ЕстьОшибкаНепредвиденногоИсключения = Истина;
	...
	ДанныеЗаписи.ПричинаДобавления         = Перечисления.ПричиныДобавленияВРегистрОбъектыКЗагрузке.ОшибкаКонвертации;
	ДанныеЗаписи.УзелПланаОбмена           = КомпонентыОбмена.УзелКорреспондента;
	ДанныеЗаписи.ПолноеПредставлениеОшибки = ТекстОшибки;
...
Процедура ПослеИнициализацииКомпонентыОбменаПриПолучении(КомпонентыОбмена, ИнициироватьВедениеПротокола, ИнициализироватьЗамеры, АнализСообщения = Ложь)
	
	КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = КлючСообщенияЖурналаРегистрации;
	КомпонентыОбмена.ВедениеПротоколаДанных.ВыводВПротоколИнформационныхСообщений = ВыводВПротоколИнформационныхСообщений;
	
	Если ИнициироватьВедениеПротокола = Истина Тогда
		
		ОбменДаннымиXDTOСервер.ИнициализироватьВедениеПротоколаОбмена(КомпонентыОбмена, ИмяФайлаПротоколаОбмена);
		
	КонецЕсли;
	
	Если ИнициализироватьЗамеры = Истина Тогда
		
		ОбменДаннымиОценкаПроизводительности.Инициализировать(КомпонентыОбмена, АнализСообщения);
