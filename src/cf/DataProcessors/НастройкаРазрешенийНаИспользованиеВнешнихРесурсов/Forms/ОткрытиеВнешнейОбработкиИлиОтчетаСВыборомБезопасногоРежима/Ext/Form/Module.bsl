...
Функция ПодключитьНаСервере(ЭтоВнешняяОбработка)
	...
	Возврат Менеджер.Подключить(ФайлОбработкиАдрес,, БезопасныйРежим); // АПК:552; АПК:553 Безопасное подключение. Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
