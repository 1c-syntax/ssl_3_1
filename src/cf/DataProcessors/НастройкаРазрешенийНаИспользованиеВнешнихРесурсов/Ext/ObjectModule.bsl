...
Перем ОперацииАдминистрирования; // см. ОперацииАдминистрирования

// Текущий план применения запросов на использование внешних ресурсов.
Перем ПланПримененияЗапросов; // см. ПланПримененияЗапросов

// Исходный срез разрешений (в разрезе владельцев разрешений).
Перем ИсходныйСрезРазрешенийВРазрезеВладельцев; // см. РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.НовыйСрезРазрешенийВРазрезеВладельцев

// Исходный срез разрешений (без учета владельцев разрешений).
Перем ИсходныйСрезРазрешенийБезУчетаВладельцев; // см. РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.НовыйСрезРазрешений

// Срез разрешений в результате применения запросов (в разрезе владельцев разрешений).
Перем РезультатПримененияЗапросовВРазрезеВладельцев; // см. РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.НовыйСрезРазрешенийВРазрезеВладельцев

// Срез разрешений в результате применения запросов (в разрезе владельцев разрешений).
Перем РезультатПримененияЗапросовБезУчетаВладельцев; // см. РегистрыСведений.РазрешенияНаИспользованиеВнешнихРесурсов.НовыйСрезРазрешений

// Дельта между исходным и результирующим срезами разрешений (в разрезе владельцев разрешений).
Перем ДельтаВРазрезеВладельцев; // см. ДельтаВРазрезеВладельцев
...
Процедура ДобавитьЗапросРазрешенийНаИспользованиеВнешнихРесурсов(
	...
	Если Строка = Неопределено Тогда
		...
		Если ТипПрограммногоМодуля = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
			...
			Имя = РаботаВБезопасномРежиме.ПрофильБезопасностиИнформационнойБазы();
...
//      *** Ключ - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения
//      *** Значение - ОбъектXDTO - XDTO-описание разрешения,
//    ** ДополненияРазрешений - Соответствие из КлючИЗначение:
//      *** Ключ - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения
//      *** Значение - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.ДополнениеРазрешения
//
Функция ДельтаВРазрезеВладельцев()
		
	Результат = Новый Структура();
	
	Результат.Вставить("Добавляемые", Новый ТаблицаЗначений);
	Результат.Добавляемые.Колонки.Добавить("ТипПрограммногоМодуля", Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	Результат.Добавляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля", Новый ОписаниеТипов("УникальныйИдентификатор"));
	Результат.Добавляемые.Колонки.Добавить("ТипВладельца", Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	Результат.Добавляемые.Колонки.Добавить("ИдентификаторВладельца", Новый ОписаниеТипов("УникальныйИдентификатор"));
	Результат.Добавляемые.Колонки.Добавить("Тип", Новый ОписаниеТипов("Строка"));
	Результат.Добавляемые.Колонки.Добавить("Разрешения", Новый ОписаниеТипов("Соответствие"));
	Результат.Добавляемые.Колонки.Добавить("ДополненияРазрешений", Новый ОписаниеТипов("Соответствие"));
	
	Результат.Вставить("Удаляемые", Новый ТаблицаЗначений);
	Результат.Удаляемые.Колонки.Добавить("ТипПрограммногоМодуля", Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	Результат.Удаляемые.Колонки.Добавить("ИдентификаторПрограммногоМодуля", Новый ОписаниеТипов("УникальныйИдентификатор"));
	Результат.Удаляемые.Колонки.Добавить("ТипВладельца", Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
	Результат.Удаляемые.Колонки.Добавить("ИдентификаторВладельца", Новый ОписаниеТипов("УникальныйИдентификатор"));
	Результат.Удаляемые.Колонки.Добавить("Тип", Новый ОписаниеТипов("Строка"));
	Результат.Удаляемые.Колонки.Добавить("Разрешения", Новый ОписаниеТипов("Соответствие"));
	Результат.Удаляемые.Колонки.Добавить("ДополненияРазрешений", Новый ОписаниеТипов("Соответствие"));
...
Процедура РассчитатьДельтуВРазрезеВладельцев()
	...
	ДельтаВРазрезеВладельцев = ДельтаВРазрезеВладельцев();
...
//      *** Значение - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.ДополнениеРазрешения
//
Функция ДельтаБезУчетаВладельцев()
...
Процедура РассчитатьДельтуБезУчетаВладельцев()
	...
	ДельтаБезУчетаВладельцев = ДельтаБезУчетаВладельцев();
...
//   * Имя - Строка - имя профиля безопасности
//
Функция ОперацииАдминистрирования()
	...
	Возврат ОперацииАдминистрирования;
...
//      ** ИдентификаторВладельца - УникальныйИдентификатор,
//      ** Тип - Строка - имя XDTO-типа, описывающего разрешения,
//      ** Разрешения - Соответствие из КлючИЗначение - описание добавляемых разрешений:
//         *** Ключ - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения
//         *** Значение - ОбъектXDTO - XDTO-описание добавляемого разрешения,
//      ** ДополненияРазрешений - Соответствие из КлючИЗначение - описание дополнений добавляемых разрешений:
//         *** Ключ - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения
//         *** Значение - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.ДополнениеРазрешения
...
//      * ИдентификаторВладельца - УникальныйИдентификатор,
//      * Тип - Строка - имя XDTO-типа, описывающего разрешения,
//      * Разрешения - Соответствие из КлючИЗначение - описание удаляемых разрешений:
//         * Ключ - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения
//         * Значение - ОбъектXDTO - XDTO-описание удаляемого разрешения,
//      * ДополненияРазрешений - Соответствие из КлючИЗначение - описание дополнений удаляемых разрешений:
//         * Ключ - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.КлючРазрешения
//         * Значение - см. РегистрСведений.РазрешенияНаИспользованиеВнешнихРесурсов.ДополнениеРазрешения
//
Функция ПланПримененияЗапросов()
	...
	Возврат ПланПримененияЗапросов;
...
ПланПримененияЗапросов = ПланПримененияЗапросов();

ОперацииАдминистрирования = ОперацииАдминистрирования();
