...
Процедура ПерейтиНаСтраницуОшибки()
	...
	ОтборСтроки.Вставить("ИмяОсновнойСтраницы", "ОшибкаВыполнения");
	
	НайденныеСтроки = ТаблицаПереходов.НайтиСтроки(ОтборСтроки);
	
	ПорядковыйНомерПерехода = НайденныеСтроки[0].ПорядковыйНомерПерехода;
...
&НаСервере
Функция ОпределитьСозданноеСОшибкойАвтономноеРабочееМесто(КодСбойногоУзла)
	
	АвтономноеРабочееМесто = Неопределено;
	ИдентификаторУзлаПланаОбменаКУдалению = "";
	
	// Объект.АвтономноеРабочееМесто
	ИнформацияОбУзлеСОшибкой = ПолучитьИзВременногоХранилища(АдресВременногоХранилищаНачальногоОбраза);
	Если ТипЗнч(ИнформацияОбУзлеСОшибкой) = Тип("Структура")
		И ИнформацияОбУзлеСОшибкой.Свойство("ИдентификаторУзлаПланаОбменаКУдалению", ИдентификаторУзлаПланаОбменаКУдалению)
		Тогда
		
		Если Не ПустаяСтрока(ИдентификаторУзлаПланаОбменаКУдалению) Тогда
			
			ИдентификаторУзлаАРМ = Новый УникальныйИдентификатор(Строка(ИдентификаторУзлаПланаОбменаКУдалению));
			АвтономноеРабочееМесто = ПланыОбмена[ИмяПланаОбмена].ПолучитьСсылку(ИдентификаторУзлаАРМ);
			
		КонецЕсли;
		
	КонецЕсли;
	
	Если АвтономноеРабочееМесто = Неопределено
		И Не ПустаяСтрока(Строка(КодСбойногоУзла))
		Тогда
		
		АвтономноеРабочееМесто = ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(Строка(КодСбойногоУзла));
		
	КонецЕсли;
	
	Возврат АвтономноеРабочееМесто;
	
КонецФункции

&НаСервере
Процедура УдалитьАвтономноеРабочееМесто(Отказ, Знач КодСбойногоУзла, СообщениеОбОшибке = "")
	
	ОтключаемоеАвтономноеРабочееМесто = ОпределитьСозданноеСОшибкойАвтономноеРабочееМесто(КодСбойногоУзла);
	
	КонтекстУдаления = Новый Структура;
	КонтекстУдаления.Вставить("АвтономноеРабочееМесто", ОтключаемоеАвтономноеРабочееМесто);
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Удаление автономного рабочего места'");
	ПараметрыВыполнения.ЗапуститьНеВФоне = Ложь;
	
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне("АвтономнаяРаботаСлужебный.УдалитьАвтономноеРабочееМесто",
		КонтекстУдаления, ПараметрыВыполнения);
...
&НаКлиенте
Функция Подключаемый_ОжиданиеУдаленияНачальногоОбраза_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	
	// В код узла плана обмена АРМ пишется УИД фонового задания длительной операции.
	КодСбойногоУзла = ИдентификаторЗадания;
	...
	СообщениеОбОшибке = "";
	УдалитьАвтономноеРабочееМесто(Отказ, КодСбойногоУзла, СообщениеОбОшибке);
	
	Если Отказ Тогда
		
		ШаблонТекста = НСтр("ru = 'При удалении автономного рабочего места возникли ошибки: %1'");
		ПоказатьПредупреждение(Неопределено, СтрШаблон(ШаблонТекста, СообщениеОбОшибке));
...
&НаКлиенте
Функция Подключаемый_ОжиданиеУдаленияНачальногоОбразаДлительнаяОперация_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
...
&НаКлиенте
Функция Подключаемый_ЗавершениеУдаленияНачальногоОбраза_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
...
Процедура СценарийСозданияАвтономногоРабочегоМеста()
	...
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ОшибкаВыполнения", "СтраницаНавигацииНачало");
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ОжиданиеУдаленияНачальногоОбраза", "СтраницаНавигацииОжидание");
	НовыйПереход.ДлительнаяОперация = Истина;
	НовыйПереход.ИмяОбработчикаДлительнойОперации = "Подключаемый_ОжиданиеУдаленияНачальногоОбраза_ОбработкаДлительнойОперации";
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ОжиданиеУдаленияНачальногоОбраза", "СтраницаНавигацииОжидание");
	НовыйПереход.ДлительнаяОперация = Истина;
	НовыйПереход.ИмяОбработчикаДлительнойОперации = "Подключаемый_ОжиданиеУдаленияНачальногоОбразаДлительнаяОперация_ОбработкаДлительнойОперации";
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ЗавершениеУдаленияНачальногоОбраза", "СтраницаНавигацииОкончание");
	НовыйПереход.ДлительнаяОперация = Истина;
	НовыйПереход.ИмяОбработчикаДлительнойОперации = "Подключаемый_ЗавершениеУдаленияНачальногоОбраза_ОбработкаДлительнойОперации";
