...
Процедура ЗаписатьПакетУстановкиВоВременноеХранилище(ПараметрыЗаписи)
	...
	Исключение
		...
		// Вернем в помощник отрицательный результат для последующего удаления сбойного узла
		ДанныеОшибки = Новый Структура;
		ДанныеОшибки.Вставить("ИдентификаторУзлаПланаОбменаКУдалению", АвтономноеРабочееМесто.УникальныйИдентификатор());
		
		ПоместитьВоВременноеХранилище(ДанныеОшибки, ПараметрыЗаписи.АдресНачальногоОбраза);
