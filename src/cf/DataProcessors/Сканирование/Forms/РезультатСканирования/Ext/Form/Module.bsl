...
Процедура ПередОткрытиемАвтомат(Результат, ПараметрыОткрытия) Экспорт
	...
	Если ПараметрыОткрытия.ТекущийШаг = 1 Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ПередОткрытиемАвтоматШаг1ПослеИнициализации", ЭтотОбъект, ПараметрыОткрытия);
		РаботаСФайламиСлужебныйКлиент.ПроинициализироватьКомпоненту(ОписаниеОповещения, Истина);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПередОткрытиемАвтоматШаг1ПослеИнициализации(РезультатПроверкиИнициализации, ПараметрыОткрытия) Экспорт
	...
	Если Не ПодключаемыйМодуль.ЕстьУстройства() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПередОткрытиемАвтоматПродолжение(ПараметрыОткрытия)
	...
	Если ПараметрыОткрытия.ТекущийШаг = 2 Тогда
		...
		Если ПараметрыОткрытия.ВыбранноеУстройство = "" Тогда
			...
			ОткрытьФорму("Обработка.Сканирование.Форма.НастройкаСканирования", ПараметрыОткрытияФормы , ЭтотОбъект, , , , Обработчик, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
			Возврат;
		КонецЕсли;
	...
	Если ПараметрыОткрытия.ТекущийШаг = 3 Тогда
		...
		Если Разрешение = -1 ИЛИ Цветность = -1 ИЛИ Поворот = -1 ИЛИ РазмерБумаги = -1 Тогда
			...
			Разрешение = РаботаСФайламиСлужебныйКлиент.ПолучитьНастройку(ПодключаемыйМодуль,
				ПараметрыОткрытия.ВыбранноеУстройство,
				"XRESOLUTION");
			
			Цветность = РаботаСФайламиСлужебныйКлиент.ПолучитьНастройку(ПодключаемыйМодуль,
				ПараметрыОткрытия.ВыбранноеУстройство,
				"PIXELTYPE");
			
			Поворот = РаботаСФайламиСлужебныйКлиент.ПолучитьНастройку(ПодключаемыйМодуль,
				ПараметрыОткрытия.ВыбранноеУстройство,
				"ROTATION");
			
			РазмерБумаги = РаботаСФайламиСлужебныйКлиент.ПолучитьНастройку(ПодключаемыйМодуль,
				ПараметрыОткрытия.ВыбранноеУстройство,
				"SUPPORTEDSIZES");
...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	НастройкиСканирования = РаботаСФайламиСлужебныйВызовСервера.ПараметрыСканирования();
	...
	ПримитивныеНастройкиСканирования = РаботаСФайламиСлужебныйВызовСервера.КонвертироватьНастройкиСканирования(НастройкиСканирования);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ПримитивныеНастройкиСканирования, "Разрешение, Цветность, Поворот, РазмерБумаги, СжатиеTIFF");
...
Процедура СохранитьПослеОбъединенияЗавершение(Контекст)
	...
	ИначеЕсли ТипРезультата = РаботаСФайламиКлиент.ТипРезультатаКонвертацииИмяФайла() Тогда
		...
		Для ИндексФайла = 0 По ПараметрыВыполнения.МассивФайловКопия.ВГраница() Цикл
			Если ПараметрыВыполнения.МассивФайловКопия[0].ПутьКФайлу = Контекст.ПутьКФайлуЛокальный Тогда
				ИндексыУдаления.Вставить(0, ИндексФайла);
			КонецЕсли;
		КонецЦикла;	
		
		Для Каждого ИндексУдаления Из ИндексыУдаления Цикл
			ПараметрыВыполнения.МассивФайловКопия.Удалить(ИндексУдаления);
		КонецЦикла;
...
Процедура НачатьСканированиеПослеПолученияКомпоненты(РезультатИнициализации, Контекст) Экспорт
	Если РезультатИнициализации.Подключено Тогда
		...
		РезультатИнициализации.ПодключаемыйМодуль.НачатьСканирование(
