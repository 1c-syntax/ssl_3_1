...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ПараметрыСканирования) Тогда
...
Процедура РекомендацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "ОткрытьНастройки" Тогда
		СтандартнаяОбработка = Ложь;
		РаботаСФайламиКлиент.ОткрытьФормуНастройкиСканирования();
	ИначеЕсли НавигационнаяСсылка = "Запустить32БитныйКлиент" Тогда
		СтандартнаяОбработка = Ложь;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку") Тогда
		...
		Если НавигационнаяСсылка = "ВопросВПоддержку" Тогда
			...
			Если Не ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
				ПредставлениеОшибки = НСтр("ru='Вызвана помощь из окна сканирования.'");
			КонецЕсли;
...
Процедура Настройка(Команда)
	...
	АвтоматическаяПодачаДокументовЧисло = РаботаСФайламиСлужебныйКлиент.НастройкаСканера(ЭтотОбъект, ПодключаемыйМодуль,
		АвтоматическаяПодачаДокументов, "FEEDER");
	ДоступностьАвтоматическаяПодачаДокументов = (АвтоматическаяПодачаДокументовЧисло <> -1);
	...
	ПараметрыФормы.Вставить("АвтоматическаяПодачаДокументов", АвтоматическаяПодачаДокументов);
	
	ПараметрыФормы.Вставить("ИспользоватьImageMagickДляПреобразованияВPDF", ИспользоватьImageMagickДляПреобразованияВPDF);
	...
	ПараметрыФормы.Вставить("ДоступностьАвтоматическаяПодачаДокументов", ДоступностьАвтоматическаяПодачаДокументов);
...
Процедура ПродолжитьОтправкуВопросаВПоддержку(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыОбращения.Тема = НСтр("ru = 'Проблема при сканировании файла'");
...
Процедура УстановитьТекстРекомендации()
	...
	Шаблон = НСтр("ru = 'Сканирование выполняется с помощью %1.'");
	Рекомендация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяУстройстваСканирования);
	...
	Шаблон = " • " + НСтр("ru = 'Укажите в <a href = ""%1"">настройках сканирования</a> доступный сканер.'");
	Рекомендация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, "ОткрытьНастройки");
	Рекомендации.Добавить(Рекомендация);
	...
	Если ПоказыватьДиалогСканера Или РазрешениеПеречисление = РазрешениеИзображения Тогда
...
Процедура ПодготовкаКСканированиюПослеИнициализации(РезультатПроверкиИнициализации, ПараметрыОткрытия) Экспорт
	...
	ОповещениеПродолжения = Новый ОписаниеОповещения("ПодготовкаКСканированиюПослеВключенияЖурнала", 
		ЭтотОбъект, ПараметрыОткрытия);
	РаботаСФайламиСлужебныйКлиент.ВключитьЖурналСканирования(ПодключаемыйМодуль, ОповещениеПродолжения, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ПодготовкаКСканированиюПослеВключенияЖурнала(Результат, ПараметрыОткрытия) Экспорт
...
Процедура ПередОткрытиемАвтоматПродолжение(ПараметрыОткрытия)
	...
	Если ПараметрыОткрытия.ТекущийШаг = 3 Тогда
		...
		АвтоматическаяПодачаДокументовЧисло = РаботаСФайламиСлужебныйКлиент.НастройкаСканера(ЭтотОбъект, ПодключаемыйМодуль,
			ПараметрыОткрытия.ВыбранноеУстройство, "FEEDER");
...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	Если Не ПоказыватьДиалогСканера Тогда
		...
		Если ДвустороннееСканирование Тогда
			Представление = Представление +  НСтр("ru = 'Двустороннее сканирование'") + ". ";
		КонецЕсли;	
		
		Если АвтоматическаяПодачаДокументов Тогда
			Представление = Представление +  НСтр("ru = 'Автоподача документов'") + ". ";
		КонецЕсли;	
	Иначе
		Если СохранятьВPDF Тогда
			ФорматКартинки = Строка(ФорматСканированногоИзображения);
...
#Если НЕ ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Если Источник = "TWAIN" И Событие = "ImageAcquired" Тогда
		...
		Если ТаблицаФайлов.Количество() > 1 И Элементы.ТаблицаФайлов.Видимость = Ложь Тогда
			...
			Элементы.ФормаПринятьВсеКакОтдельныеФайлы.Видимость = Не ТолькоОдинФайл;
...
Процедура НачатьНепосредственноеСканирование()
	...
	ПараметрыСканирования.АвтоматическаяПодачаДокументов = АвтоматическаяПодачаДокументов;
...
#Если Не ВебКлиент Тогда
	АдресКартинки = ПоместитьВоВременноеХранилище(БиблиотекаКартинок.ДлительнаяОперация48.ПолучитьДвоичныеДанные(), 
		УникальныйИдентификатор);
#КонецЕсли
	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить("КомпонентаСканирования", "ПараметрыЗаданияСканирования", 
