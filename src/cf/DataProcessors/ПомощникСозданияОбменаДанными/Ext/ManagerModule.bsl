...
Процедура ПроверитьПодключениеККорреспонденту(Параметры, АдресРезультата) Экспорт
	...
	ИначеЕсли НастройкиПодключения.ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.WS Тогда
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено Тогда
			...
			СообщениеОбОшибке = "";
			ПараметрыИнформационнойБазы = WSПрокси.GetIBParameters(НастройкиПодключения.ИмяПланаОбмена, НастройкиПодключения.ИдентификаторИнформационнойБазыИсточника, СообщениеОбОшибке);
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкиПодключения)
	...
	Попытка
		...
		Если НастройкиПодключения.ВосстановлениеНастроекОбмена <> "" Тогда
			ОбновитьКод = Истина;
		КонецЕсли;
...
Процедура ЗаполнитьНастройкиПодключенияИзXML(НастройкиПодключения,
	...
	СтруктураНастроек.Свойство("НомерОтправленного", НастройкиПодключения.НомерОтправленного);
	СтруктураНастроек.Свойство("НомерПринятого", НастройкиПодключения.НомерПринятого);
