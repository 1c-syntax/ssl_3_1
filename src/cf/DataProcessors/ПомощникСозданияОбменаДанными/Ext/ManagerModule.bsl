...
#Область ЗагрузкаПриоритетныхДанныхВПодчиненныйУзелРИБ

Процедура ПриНачалеЗагрузкиСообщенияОбменаДаннымиСОбновлением(ПараметрыЗагрузки, ПараметрыОбработчика, ПродолжитьОжидание = Ложь) Экспорт
	
	ПриНачалеЗапускаОбменаДанными(ПараметрыЗагрузки, ПараметрыОбработчика, "Обработки.ПомощникСозданияОбменаДанными.ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБВФоне", ПродолжитьОжидание);
		
КонецПроцедуры   

// Для внутреннего использования.
//
Процедура ПриОжиданииЗагрузкиСообщенияОбменаДаннымиСОбновлением(ПараметрыОбработчика, ПродолжитьОжидание) Экспорт
...
// Для внутреннего использования.
//
Процедура ПриЗавершенииЗагрузкиСообщенияОбменаДаннымиСОбновлением(ПараметрыОбработчика, СтатусЗавершения) Экспорт
	
	ПриЗавершенииДлительнойОперации(ПараметрыОбработчика, СтатусЗавершения);
	
КонецПроцедуры

Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБВФоне(ПараметрыЗагрузки, АдресХранилища) Экспорт
	
	Результат = ИнициализироватьРезультатДлительнойОперации();

	Попытка
		ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(ПараметрыЗагрузки);
	Исключение
		...
		Результат.СообщениеОбОшибке = ПредставлениеОшибки;
	КонецПопытки;
	
	Если ПараметрыЗагрузки.ЕстьОшибки Тогда
		Результат.Отказ = Истина;
		Результат.СообщениеОбОшибке = НСтр("ru = 'При выполнении загрузки данных возникли ошибки (см. Журнал регистрации).'")
	КонецЕсли;
...
#Область ЗагрузкаСообщенияПередОбновлениемИнформационнойБазы

Процедура ПриНачалеЗагрузкиСообщенияОбменаДаннымиБезОбновленияИБ(ПараметрыЗагрузки, ПараметрыОбработчика, ПродолжитьОжидание = Ложь) Экспорт
	
	ПриНачалеЗапускаОбменаДанными(ПараметрыЗагрузки, ПараметрыОбработчика, "Обработки.ПомощникСозданияОбменаДанными.ЗагрузитьСообщениеПередОбновлениемИнформационнойБазыВФоне", ПродолжитьОжидание);
	
КонецПроцедуры

// Для внутреннего использования.
//
Процедура ПриОжиданииЗагрузкиСообщенияПередОбновлениемИнформационнойБазы(ПараметрыОбработчика, ПродолжитьОжидание) Экспорт
...
// Для внутреннего использования.
//
Процедура ПриЗавершенииЗагрузкиСообщенияПередОбновлениемИнформационнойБазы(ПараметрыОбработчика, СтатусЗавершения) Экспорт
	
	ПриЗавершенииДлительнойОперации(ПараметрыОбработчика, СтатусЗавершения);
	
КонецПроцедуры

Процедура ЗагрузитьСообщениеПередОбновлениемИнформационнойБазыВФоне(ПараметрыПроцедуры, АдресХранилища) Экспорт
	
	Результат = ИнициализироватьРезультатДлительнойОперации();
	
	Попытка
		ЗагрузитьСообщениеПередОбновлениемИнформационнойБазы(ПараметрыПроцедуры);
	Исключение
		ПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если ПараметрыПроцедуры.ЕстьОшибки Тогда
		Результат.Отказ = Истина;
		Результат.СообщениеОбОшибке = НСтр("ru = 'При выполнении загрузки данных возникли ошибки (см. Журнал регистрации).'");
	КонецЕсли;
	...
	Попытка
		ВыгрузитьСообщениеПослеОбновленияИнформационнойБазы(ПараметрыПроцедуры);
	Исключение
...
Процедура ПриНачалеПолученияВариантовНастроекОбменаДанными(УникальныйИдентификатор, ПараметрыОбработчика, ПродолжитьОжидание) Экспорт
	...
	ПриПолученииДоступныхНастроекСинхронизацииДанных(ТаблицаСтандартныеНастройки, ПараметрыОбработчика);
...
Процедура ПриПолученииДоступныхНастроекСинхронизацииДанных(ТаблицаНастройки, ПараметрыОбработчика)
	...
	Для Каждого ИмяПланаОбмена Из СписокПлановОбмена Цикл
		...
		ЗаполнитьТаблицуВариантамиНастроекПланаОбмена(ТаблицаНастройки, ПараметрыОбработчика, ИмяПланаОбмена);
...
Процедура ЗаполнитьТаблицуВариантамиНастроекПланаОбмена(ТаблицаНастройки, ПараметрыОбработчика, ИмяПланаОбмена)
	
	ОтборПоВариантамНастроек = Неопределено;
	Если ПараметрыОбработчика <> Неопределено
			И ЗначениеЗаполнено(ПараметрыОбработчика.ОтборПоВариантамНастроек)Тогда
		ОтборПоВариантамНастроек = ПараметрыОбработчика.ОтборПоВариантамНастроек;
	КонецЕсли;
	...
	Для Каждого ВариантНастройки Из НастройкиОбмена.ВариантыНастроекОбмена Цикл
		Если ОтборПоВариантамНастроек <> Неопределено
			И ОтборПоВариантамНастроек.НайтиПоЗначению(ВариантНастройки.ИдентификаторНастройки) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Функция СохранитьНастройкиПодключения(НастройкиПодключения) Экспорт
	...
	// Выгрузка для off-line обмена через универсальный формат
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкиПодключения)
	...
	Если ОбменДаннымиПовтИсп.ЭтоПланОбменаXDTO(НастройкиПодключения.ИмяПланаОбмена) Тогда
		...
		Если НастройкиПодключения.ПоддерживаемыеОбъектыФормата <> Неопределено Тогда
			...
			РегистрыСведений.НастройкиОбменаДаннымиXDTO.ОбновитьНастройки(НовыйУзел.Ссылка,
				"ПоддерживаемыеОбъекты", НастройкиПодключения.ПоддерживаемыеОбъектыФормата.Получить());
			
		КонецЕсли;
		
		Если НастройкиПодключения.ПоддерживаемыеОбъектыФорматаКорреспондента <> Неопределено Тогда
			
			РегистрыСведений.НастройкиОбменаДаннымиXDTO.ОбновитьНастройкиКорреспондента(НовыйУзел.Ссылка,
				"ПоддерживаемыеОбъекты", НастройкиПодключения.ПоддерживаемыеОбъектыФорматаКорреспондента.Получить());
	...
	// Общие данные узлов.
	Префикс = ?(НастройкиПодключения.ИспользоватьПрефиксыДляНастройкиОбмена Или НастройкиПодключения.ИспользоватьПрефиксыДляНастройкиОбменаКорреспондента,
		НастройкиПодключения.ПрефиксИнформационнойБазыИсточника, "");
		
	ОбъектнаяКонвертацияXDTO = ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(НастройкиПодключения.ИмяПланаОбмена,
		"ОбъектнаяКонвертацияXDTO", НастройкиПодключения.ВариантНастройкиОбмена);
		
	СтруктураЗаписи = Новый Структура;
	СтруктураЗаписи.Вставить("УзелИнформационнойБазы", НастройкиПодключения.УзелИнформационнойБазы);
	СтруктураЗаписи.Вставить("Префикс", Префикс);
	СтруктураЗаписи.Вставить("ПрефиксКорреспондента", НастройкиПодключения.ПрефиксИнформационнойБазыПриемника);
	СтруктураЗаписи.Вставить("ИмяПланаОбменаКорреспондента", НастройкиПодключения.ИмяПланаОбменаКорреспондента);
	СтруктураЗаписи.Вставить("ОбъектнаяКонвертацияXDTO", ОбъектнаяКонвертацияXDTO);
...
Процедура ПриНачалеЗапускаОбменаДанными(ПараметрыЗагрузки, ПараметрыОбработчика, ИмяМетода, ПродолжитьОжидание = Ложь)
	
	ИмяПланаОбмена = ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(ПараметрыЗагрузки.УзелИнформационнойБазы);
	ИдентификаторТранспорта = ТранспортСообщенийОбмена.ТранспортПоУмолчанию(ПараметрыЗагрузки.УзелИнформационнойБазы);
	
	КлючФоновогоЗадания = ОбменДаннымиСервер.КлючФоновогоЗадания(ИмяПланаОбмена,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Загрузка сообщения обмена данными: %1'"), ИдентификаторТранспорта));

	Если ОбменДаннымиСервер.ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Загрузка сообщения обмена данными: %1 уже выполняется.'"), ИдентификаторТранспорта);
	КонецЕсли;
	...
	Если ИмяМетода = "ЗагрузитьСообщениеПередОбновлениемИнформационнойБазыВФоне" Тогда
		
		ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
			"Обработки.ПомощникСозданияОбменаДанными.ЗагрузитьСообщениеПередОбновлениемИнформационнойБазыВФоне",
			ПараметрыЗагрузки,
			ПараметрыВыполнения);
	...
	Иначе
		...
		ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
			"Обработки.ПомощникСозданияОбменаДанными.ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБВФоне",
			ПараметрыЗагрузки,
			ПараметрыВыполнения);
...
//   * СообщениеОбОшибке - Строка
//
Функция ИнициализироватьРезультатДлительнойОперации()
...
Процедура ЗагрузитьСообщениеПередОбновлениемИнформационнойБазы(ПараметрыЗагрузки)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Если ПараметрыЗагрузки.УзелИнформационнойБазы <> Неопределено Тогда
			...
			Если ПараметрыЗагрузки.ПродолжитьЗагрузкуСообщениемИзАрхива Тогда
				ОбменДаннымиСервер.СохранитьСообщениеИзАрхиваВКэш(ПараметрыЗагрузки.УзелИнформационнойБазы);
			КонецЕсли;
			
			// Только загрузка.
			ПараметрыОбмена = ОбменДаннымиСервер.ПараметрыОбмена();
			ПараметрыОбмена.ИдентификаторТранспорта = ИдентификаторТранспорта;
			ПараметрыОбмена.ДанныеАутентификации = ПараметрыЗагрузки.ДанныеАутентификации;
			ПараметрыОбмена.ВыполнятьЗагрузку = Истина;
			ПараметрыОбмена.ВыполнятьВыгрузку = Ложь;
			
			ОбменДаннымиСервер.ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(ПараметрыЗагрузки.УзелИнформационнойБазы, ПараметрыОбмена, ПараметрыЗагрузки.ЕстьОшибки);
...
Процедура ВыгрузитьСообщениеПослеОбновленияИнформационнойБазы(ПараметрыВыгрузки)
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
			...
			Если УзелИнформационнойБазы <> Неопределено Тогда
				...
				Если ВыполнитьВыгрузку Тогда
					...
					ПараметрыОбмена = ОбменДаннымиСервер.ПараметрыОбмена();
					ПараметрыОбмена.ИдентификаторТранспорта = ИдентификаторТранспорта;
					ПараметрыОбмена.ДанныеАутентификации    = ПараметрыВыгрузки.ДанныеАутентификации;
					ПараметрыОбмена.ВыполнятьЗагрузку       = Ложь;
					ПараметрыОбмена.ВыполнятьВыгрузку       = Истина;
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(ПараметрыЗагрузки)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Если УзелИнформационнойБазы <> Неопределено Тогда
			...
			ОбновлениеПравилДляОбменаДанными(ПараметрыЗагрузки.УзелИнформационнойБазы);
			...
			ОбменДаннымиСервер.ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(УзелИнформационнойБазы, ПараметрыОбмена, ПараметрыЗагрузки.ЕстьОшибки);
...
Процедура ОбновлениеПравилДляОбменаДанными(УзелИнформационнойБазы)
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПолучениеПараметровКорреспондента", Истина);
	Методы.Вставить("ПроверитьПодключениеККорреспонденту", Истина);
	Методы.Вставить("СохранитьНастройкиПодключения", Истина);
	Методы.Вставить("ЗарегистрироватьДанныеДляНачальнойВыгрузки", Истина);
	Методы.Вставить("ЗагрузитьНастройкиXDTOКорреспондента", Истина);
	Методы.Вставить("УдалитьНастройкуСинхронизации", Истина);
	Методы.Вставить("СохранитьНастройкиСинхронизации", Истина);
	Методы.Вставить("ИзменитьТранспортУзлаНаWS", Истина);
	Методы.Вставить("ИзменитьТранспортУзлаКорреспондентаНаWS", Истина);
	Методы.Вставить("ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБВФоне", Истина);
	Методы.Вставить("ЗагрузитьСообщениеПередОбновлениемИнформационнойБазыВФоне", Истина);
