...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьТекущуюСтрокуТаблицы();
	
	Если НЕ Элементы.ВариантРаботы.Видимость Тогда
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаНоваяНастройка
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаПродолжениеНастройки
...
&НаКлиенте
Процедура УстановитьТекущуюСтрокуТаблицы()
	
	Если ИдентификаторСтрокиНовая <> 0
		И Элементы.ТаблицаНоваяНастройка.ПроверитьСтроку(ИдентификаторСтрокиНовая) = Истина Тогда
		
		Элементы.ТаблицаНоваяНастройка.ТекущаяСтрока = ИдентификаторСтрокиНовая;
		
	КонецЕсли;
	
	Если ИдентификаторСтрокиПродолжение <> 0 
		И Элементы.ТаблицаПродолжениеНастройки.ПроверитьСтроку(ИдентификаторСтрокиПродолжение) = Истина Тогда
		
		Элементы.ТаблицаПродолжениеНастройки.ТекущаяСтрока = ИдентификаторСтрокиПродолжение;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.СинхронизацияДанныхЖирныйШрифт);
...
Процедура ИнициализацияРеквизитовФормы()
	...
	ВариантРаботыМастера = Параметры.ВариантРаботыМастера;
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если НЕ Строка.ПассивныйРежим Тогда
			...
			Если Строка.ИдентификаторТранспорта = ИдентификаторТранспорта Тогда
				...
				ИдентификаторСтрокиНовая = НоваяСтрока.ПолучитьИдентификатор();
			КонецЕсли;
		...
		Если Не Строка.ПрямоеПодключение Тогда
			...
			Если Строка.ИдентификаторТранспорта = ИдентификаторТранспорта Тогда
				НоваяСтрока.Выбран = Истина;
				ИдентификаторСтрокиПродолжение = НоваяСтрока.ПолучитьИдентификатор();
			КонецЕсли;
...
Процедура ИнициализацияЭлементовФормы()
	...
	Если Элементы.ВариантРаботы.Видимость Тогда
...
&НаСервере
Функция ПрочитатьИПроверитьНастройкиПодключения(Знач ИмяФайлаНастроек, Знач Адрес, СообщениеОбОшибке)
