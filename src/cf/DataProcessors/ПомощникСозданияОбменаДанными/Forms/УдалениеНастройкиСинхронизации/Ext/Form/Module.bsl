...
Процедура ПриНачалеУдаленияНастройкиСинхронизацииНаСервере(ПродолжитьОжидание)
	...
	Иначе
		...
		НастройкиУдаления.Вставить("ДанныеАутентификации", ДанныеАутентификации);
...
Процедура ИнициализироватьРеквизитыФормы()
	...
	ИдентификаторТранспорта = ТранспортСообщенийОбмена.ТранспортПоУмолчанию(УзелОбмена);

	ПрямоеПодключение = Ложь;
	ПассивныйРежим = Ложь;
	...
	Если ЗначениеЗаполнено(ИдентификаторТранспорта) Тогда
		...
		ПрямоеПодключение = ПараметрыТранспорта.ПрямоеПодключение;
		ПассивныйРежим = ПараметрыТранспорта.ПассивныйРежим;
	КонецЕсли;
	
	ПолучитьПараметрыКорреспондента = МодельСервиса
		И Не ЭтоОбменСПриложениемВСервисе
		И Не ПрямоеПодключение
		И Не ПассивныйРежим;
		
	ДанныеАутентификации = ТранспортСообщенийОбмена.ПарольСинхронизацииДанных(УзелОбмена);
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	НовыйПереход.ИмяОбработчикаПриПереходеДалее = "Подключаемый_СтраницаНачало_ПриПереходеДалее";
...
Функция Подключаемый_СтраницаНачало_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Элементы.ГруппаНачалоДополнительно.Видимость = ПрямоеПодключение;
		
	ШаблонЗаголовка = НСтр("ru = 'Удалить настройку также в программе ""%1"".'");
	Элементы.УдалитьНастройкуВКорреспонденте.Заголовок = СтрШаблон(ШаблонЗаголовка, НаименованиеКорреспондента);
...
Функция Подключаемый_СтраницаНачало_ПриПереходеДалее(Отказ)
	...
	Если УдалитьНастройкуВКорреспонденте И Не ПарольЗаполнен Тогда
		ПараметрыАутентификации = ТранспортСообщенийОбменаКлиент.ПараметрыАутентификации();
		ПараметрыАутентификации.Корреспондент = УзелОбмена;
		ПараметрыАутентификации.ИдентификаторТранспорта = ИдентификаторТранспорта;
		
		ТребуетсяАутентификация = Ложь;
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Корреспондент", УзелОбмена);
		...
		ПарольЗаполнен = Не ТребуетсяАутентификация;
		
		Отказ = ТребуетсяАутентификация;
	КонецЕсли;
