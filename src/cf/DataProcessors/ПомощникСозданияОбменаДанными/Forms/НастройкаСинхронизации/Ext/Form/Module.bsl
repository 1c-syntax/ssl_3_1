...
Процедура ВыполнитьЗагрузкуНастроекXDTOПриНеобходимости(ПрерватьНастройку, ОповещениеПродолжения)
	...
	МассивЗначений[2] = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.WSПассивныйРежим");
	
	ВидТранспортаПодддерживаетПрямоеПодключение = (МассивЗначений.Найти(ВидТранспорта) <> Неопределено);
	
	СостояниеНастройки = СостояниеНастройкиСинхронизации(УзелОбмена);
	Если (Не СостояниеНастройки.НастройкаСинхронизацииЗавершена
			И Не СостояниеНастройки.ПолученыНастройкиXDTOКорреспондента)
		ИЛИ (ВидТранспортаПодддерживаетПрямоеПодключение
			И НЕ ПоддерживаетсяСопоставлениеДанных) Тогда
