...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазрешенДоступКИнтернетСервисам() Тогда
	...
	Иначе
		ВыборСервераВзаимодействия = 1;
		Элементы.ВыборСервераВзаимодействияДиалог.Доступность = Ложь;
		Элементы.ГруппаКомментарийРазрешенДоступКИнтернетСервисам.Видимость = Истина;
		ОбновитьЭлементыВыбораСервера(ЭтотОбъект);
		Элементы.ГруппаВыборСервераВзаимодействия.Показать();
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_НаборКонстант" И Источник = "РазрешитьДоступКИнтернетСервисам" Тогда
		ПриИзмененииРазрешенияДоступаКИнтернетСервисам();
	КонецЕсли;
...
Процедура ПриПолученииКодаРегистрации()
	...
	Если ВыборСервераВзаимодействия = 0 Тогда
		ТекстОшибки = РазрешенДоступКИнтернетСервисам();
	...
	Если ВыборСервераВзаимодействия = 1 Тогда
		ПараметрыРегистрации.КодАктивации = Формат(ОбщегоНазначенияКлиент.ДатаСеанса(), НСтр("ru='ДФ=''yyyMMddHHmmss'''"));
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция РазрешенДоступКИнтернетСервисам()
...
&НаСервере
Процедура ПриИзмененииРазрешенияДоступаКИнтернетСервисам()
	РазрешенДоступКИнтернетСервисам = ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
	Элементы.ВыборСервераВзаимодействияДиалог.Доступность = РазрешенДоступКИнтернетСервисам;
