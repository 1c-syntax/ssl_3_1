...
Процедура ВыполнитьСкрытиеНаСервере(Параметры, АдресХранилища) Экспорт
	...
	Для Каждого ТипМетаданных Из ДеревоОбрабатываемыхОбъектов.Строки Цикл
		...
		Для Каждого ОбъектМетаданных Из ТипМетаданных.Строки Цикл
			...
			Если СтрНайти(ОбъектМетаданных.Тип, "Регистры") > 0 Тогда
				ОбработатьРегистры(ОбъектМетаданных, ПараметрыОтбора); // @skip-check query-in-loop - пакетное скрытие конфиденциальной информации.
			Иначе
				ОбработатьОбъектМетаданных(ОбъектМетаданных, ПараметрыОтбора); // @skip-check query-in-loop - пакетное скрытие конфиденциальной информации.
			КонецЕсли;
	...
	Если Метаданные.РегистрыСведений.Найти(ИмяРегистраВерсииОбъектов) <> Неопределено 
...
Процедура ОбработатьОбъектМетаданных(ОбъектМетаданных, ПараметрыОтбора)
	...
	Если ЗначениеЗаполнено(Субъекты) Тогда
		...
		Объект = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОбъектМетаданных.ПолноеИмя);
...
Функция ПроверитьРегистрДляУдаленияФайловВыбран(ДеревоОбрабатываемыхОбъектов)
	...
	Если Метаданные.РегистрыСведений.Найти(ИмяРегистраДвоичныхДанных) = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
