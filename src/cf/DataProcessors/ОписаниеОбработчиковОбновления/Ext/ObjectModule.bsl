...
Функция ЕстьЦиклПорядкаВыполненияОбработчика(Знач ЭтотОбработчик, Знач ПриоритетыОбработчика = Неопределено, СообщатьОбОшибках = Ложь) Экспорт
	...
	Если ЕстьЦикл Тогда
		...
		Путь.Добавить(ОписаниеОбработчика[0].Процедура);
...
Процедура УстановитьНомерОчереди(ИтерацииОбновления)
	...
	Для Каждого Библиотека Из ИтерацииОбновления Цикл
		...
		ЗаполнятьОчередь = РазрабатываемыеПодсистемы.Найти(Библиотека.Подсистема) <> Неопределено;
...
// 	 * Ссылка - Строка
//
Функция ОбработчикиСПереопределеннойОчередью(ИменаБиблиотек = Неопределено)
	
	Если ИменаБиблиотек = Неопределено Тогда
		ИменаБиблиотек = РазрабатываемыеПодсистемы();
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Параллельно", "Параллельно");
	Запрос.УстановитьПараметр("Отложенно", "Отложенно");
	Запрос.УстановитьПараметр("РазрабатываемыеПодсистемы", ИменаБиблиотек);
...
Функция ТекстЗапросаПереопределенныхОчередейОтложенныхОбработчиков()
	...
	|	Обработчики КАК Обработчики
	|ГДЕ
	|	НЕ Обработчики.Подсистема В (&РазрабатываемыеПодсистемы)
	|	И Обработчики.РежимВыполненияОтложенныхОбработчиков = &Параллельно
	|	И Обработчики.РежимВыполнения = &Отложенно
...
//
// Параметры:
// 	ВремяКон - ДатаВремя - время окончания.
// 	ВремяНач - ДатаВремя - время начала.
