...
//
Функция ЕстьЦиклВыполненияОбработчиков() Экспорт
...
// Проверяет наличие зацикленности выполнения по данным приоритетов для конкретного обработчика.
//
// Параметры:
// 	ЭтотОбработчик - СтрокаТабличнойЧасти.ОбработчикиОбновления - описание обработчика
// 	ПриоритетыОбработчика - ТаблицаЗначений - выгрузки табличной части ПриоритетыВыполнения
...
Функция ЕстьЦиклПорядкаВыполненияОбработчика(Знач ЭтотОбработчик, Знач ПриоритетыОбработчика = Неопределено, СообщатьОбОшибках = Ложь) Экспорт
	...
	Для Каждого Приоритет Из ПриоритетыОбработчика Цикл
		Связанный = НовыйОбработчик(Приоритет.Обработчик2, Приоритет.Процедура2);
	...
	Если СвязанныеОбработчикиДо.Количество() > 0 Тогда
		ЕстьЦикл = ЕстьЦиклВПорядкеОбработчиков(ЭтотОбработчик, СвязанныеОбработчикиДо, "До", Путь, ПолныйПуть);
	КонецЕсли;
	
	Если НЕ ЕстьЦикл И СвязанныеОбработчикиПосле.Количество() > 0 Тогда
		ЕстьЦикл = ЕстьЦиклВПорядкеОбработчиков(ЭтотОбработчик, СвязанныеОбработчикиПосле, "После", Путь, ПолныйПуть);
	КонецЕсли;
	
	Если ЕстьЦикл Тогда
		ОписаниеОбработчика = ОбработчикиОбновления.НайтиСтроки(Новый Структура("Ссылка", ЭтотОбработчик));
		ПолныйПуть.Вставить(0, ОписаниеОбработчика[0].Процедура);
		...
		ДобавитьОшибку(ОписаниеОбработчика[0].Процедура, ТекстСообщения);
...
Процедура ЗагрузитьОбъектыОбработчика(ИмяТабличнойЧасти, СсылкаОбработчика, ОбъектыСтрокой)
	...
	Для Каждого ИмяОбъекта Из ИменаОбъектов Цикл
		...
		Если ИмяТабличнойЧасти = "БлокируемыеОбъекты" Тогда
			ВзведенФлагБлокировки = ВзвестиФлагБлокировки("ИзменяемыеОбъекты", ИмяОбъекта);
			Если НЕ ВзведенФлагБлокировки Тогда
				ВзведенФлагБлокировки = ВзвестиФлагБлокировки("ЧитаемыеОбъекты", ИмяОбъекта);
			КонецЕсли;
		КонецЕсли;
...
Функция ВзвестиФлагБлокировки(ИмяТЧ, ИмяОбъекта)
...
Процедура ВывестиСообщениеОбОшибках(БросатьИсключение = Истина)
	...
	Если БросатьИсключение Тогда
		...
		ВызватьИсключение НСтр("ru = 'При построении очереди отложенных обработчиков обнаружены ошибки.
			...
			|Для этого требуется запустить конфигурацию с параметром запуска ""ОтключитьЛогикуНачалаРаботыСистемы"".
			|Ошибки перечислены в журнале регистрации.
			|
			|Список проблемных обработчиков обновления:'") + Символы.ПС + СписокПроблемныхОбработчиков;
	Иначе
		Для Каждого Ошибка Из Ошибки Цикл
...
// Параметры:
// 	Обработчик - СтрокаТабличнойЧасти.ОбработчикиОбновления - описание обработчика
// 	СвязанныеОбработчики - Массив из ДокументСсылка
// 	НаправлениеОбхода - Строка
// 	Путь - Массив
// Возвращаемое значение:
// 	Булево - истина если есть цикл в порядке выполнения обработчиков
//
Функция ЕстьЦиклВПорядкеОбработчиков(Обработчик, СвязанныеОбработчики, НаправлениеОбхода, Путь, ПолныйПуть)
	
	Для каждого СвязанныйОбработчик Из СвязанныеОбработчики Цикл
		
		Если ПолныйПуть.Найти(СвязанныйОбработчик.Процедура) <> Неопределено Тогда
			ПолныйПуть.Добавить(СвязанныйОбработчик.Процедура);
			Возврат Ложь;
		КонецЕсли;
		ПолныйПуть.Добавить(СвязанныйОбработчик.Процедура);
		...
		Если СвязанныеОбработчикиТекущего.Количество() > 0 Тогда 
			Если ЕстьЦиклВПорядкеОбработчиков(Обработчик, СвязанныеОбработчикиТекущего, НаправлениеОбхода, Путь, ПолныйПуть) Тогда
				Путь.Добавить(СвязанныйОбработчик.Процедура);
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
		ПолныйПуть.Удалить(ПолныйПуть.ВГраница());
...
Функция НовыйОбработчик(Ссылка = "", ИмяПроцедуры = "")
...
//
// Возвращаемое значение:
// 	Массив из Строк - имена подсистем в том виде, в котором они заданы в общих модулях ОбновлениеИнформационнойБазыХХХ
//
Функция РазрабатываемыеПодсистемы() Экспорт
	
	Результат = Новый Массив;
	ОбновлениеИнформационнойБазыПереопределяемый.ПриФормированиеСпискаРазрабатываемыхПодсистем(Результат);
	Возврат Результат;
