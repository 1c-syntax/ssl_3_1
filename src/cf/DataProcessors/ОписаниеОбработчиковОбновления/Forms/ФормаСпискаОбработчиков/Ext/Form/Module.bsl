...
Процедура УстановитьНомерСборкиОбработчикам(Команда)
	...
	ПоказатьВводЧисла(ОбработчикОтвета, Число(НомераВерсии[3]), НСтр("ru = 'Укажите новый номер сборки'"));
...
Функция ДобавитьПроцедуруМодуля(Код)
	...
	ЕстьПроцедураОписания = Ложь;
	Если Код.ЭтоПроцедураОписания Тогда
		Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
			ЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Процедура ОписаниеОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
			Если СтрокиМодуля.Найти(ЗаголовокПроцедуры) <> Неопределено Тогда
				ЕстьПроцедураОписания = Истина;
				Прервать;
			КонецЕсли;
			ЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Процедура ПриДобавленииОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
			Если СтрокиМодуля.Найти(ЗаголовокПроцедуры) <> Неопределено Тогда
				ЕстьПроцедураОписания = Истина;
				Прервать;
			КонецЕсли;
	...
	Иначе
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, Код.ИмяМодуля, ИмяПроцедуры);
...
Процедура ЗаменитьПроцедуруМодуля(Код)
	...
	Пока СтрокаМодуля <> Неопределено Цикл
		...
		// АПК:1297-выкл строки для вставки в описание процедур.
		...
		Если НЕ ВыполненаЗамена 
			...
			И СтрНайти(СтрокаМодуля, ЗаголовокПроцедуры) = 0 Тогда
			Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
				ЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Процедура ОписаниеОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
				Если СтрНайти(СтрокаМодуля, ЗаголовокПроцедуры) > 0 Тогда
					ЭтоПроцедураОписания = Истина;
					Прервать;
				КонецЕсли;
				ЗаголовокПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Процедура ПриДобавленииОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
				Если СтрНайти(СтрокаМодуля, ЗаголовокПроцедуры) > 0 Тогда
		...
		Если СокрЛП(СтрокаМодуля) = ЗаголовокПроцедуры 
			ИЛИ (Код.ЭтоПроцедураОписания И СокрЛП(СтрокаМодуля) = "Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт")
			ИЛИ ЭтоПроцедураОписания Тогда
...
Функция ОписанияОбработчиковПоМодулямОбновления(АдресНастроек)
	...
	Для Каждого ИмяКонфигурации Из ИменаКонфигураций Цикл
		...
		МодулиОбработчиков = МодулиОбработчиковОбновления(ОбработчикиМодуля, РазметкаКода);
		Если РазметкаКода.УстановитьТэги Тогда
		...
		Для Каждого Измененный Из ИзмененныеОбработчики Цикл
			...
			Для Каждого Конфликт Из КонфликтыОбработчика Цикл
				...
				Если Обработчик <> Неопределено Тогда
					НовыйМодуль = КонфликтующиеОбработчики.Добавить();
					...
					ЗаполнитьТэгиМодулейПоПодсистемам(КонфликтующиеОбработчики, РазметкаКода);
				КонецЕсли;
			КонецЦикла;
...
Процедура ЗаполнитьОписание(ОписаниеКонфигураций, ИменаКонфигураций)
	...
	Для Каждого Подсистема Из ПодсистемыКонфигурации.ПоИменам Цикл
		Сведения = НовыеСведенияОБиблиотеке();
...
// Возвращает пустую структуру сведений о библиотеке
// 
// Возвращаемое значение:
// 	Структура:
//   * Имя - Строка - имя библиотеки.
...
//   * ПараллельноеОтложенноеОбновлениеСВерсии - Строка - номер версии в формате из 4-х цифр с которой применяется
//                                                        параллельное отложенное обновление.
...
//
&НаСервере
Функция НовыеСведенияОБиблиотеке()
	
	Сведения = Новый Структура;
	Сведения.Вставить("Имя", "");
	Сведения.Вставить("Версия", "");
	Сведения.Вставить("ИдентификаторИнтернетПоддержки", "");
	Сведения.Вставить("РежимВыполненияОтложенныхОбработчиков", "");
	Сведения.Вставить("ПараллельноеОтложенноеОбновлениеСВерсии", "");
	Сведения.Вставить("ТребуемыеПодсистемы", Новый Массив);
