...
Процедура УстановитьНомерСборкиОбработчикам(Команда)
	...
	ТекстПодсказки = НСтр("ru = 'Текущая версия: %1; Укажите новый номер сборки:'");
	ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПодсказки, ВерсияКонфигурации);
	ПоказатьВводЧисла(ОбработчикОтвета, Число(НомераВерсии[3]) + 1, ТекстПодсказки);
