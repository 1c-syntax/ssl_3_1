...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПустаяСтрока(Параметры.АдресОбработчика) Тогда
		...
		Если НЕ ПустаяСтрока(ПроцедураОбработки) Тогда
			...
			АвтоЗаголовок = Ложь;
		КонецЕсли;
		ОсновнойОбъектМетаданных = ИмяОсновногоОбъектаМетаданных(Объект.ОбработчикиОбновления[0].Процедура, ЕдинственноеЧисло);
...
Процедура ЗавершениеЗакрытияФормы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ЕстьЦиклПорядкаВыполненияОбработчиков() Тогда
			Возврат;
		КонецЕсли;
...
Процедура ПроцедураПриИзменении(Элемент)
	...
	Если НЕ ПустаяСтрока(ПроцедураОбработки) Тогда
		Заголовок = ПроцедураОбработки;
		АвтоЗаголовок = Ложь;
		ОсновнойОбъектМетаданных = ИмяОсновногоОбъектаМетаданных(ПроцедураОбработки, ЕдинственноеЧисло);
	КонецЕсли;
...
Процедура ОбъектМетаданныхОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = ТипЗнч(ВыбранноеЗначение) = Тип("Строка");
	ИмяТаблицы = Элемент.Родитель.Родитель.Имя;
	ПараметрыОбработчикаВыбора = ПараметрыОбработчикаВыбора(ИмяТаблицы);
	ОбработкаВыбораОбъектаМетаданных(ВыбранноеЗначение, ПараметрыОбработчикаВыбора);
...
Функция ИмяОсновногоОбъектаМетаданных(ИмяПроцедуры, ЕдинственноеЧисло)
...
Процедура ЗагрузитьПриоритетыОбработчика(НовыеПриоритеты)
	...
	ИмяОсновногоОбъекта = ИмяОсновногоОбъектаМетаданных(Объект.ОбработчикиОбновления[0].Процедура, ЕдинственноеЧисло);
...
Процедура ПодобратьОбъектыМетаданных(Текст, ДанныеВыбора, СтандартнаяОбработка, ВключатьОбщиеМодули = Ложь)
	...
	СтандартнаяОбработка = Ложь;
	ВключатьОбщиеМодули = Истина;
	ОбъектыМетаданных = НайтиОбъектыМетаданных(Текст, ВключатьОбщиеМодули);
...
&НаСервереБезКонтекста
Функция НайтиОбъектыМетаданных(ЧастьИмени, ВключатьОбщиеМодули)
...
&НаКлиенте
Процедура ДобавитьТипыРеквизита(ИмяТаблицы)
...
Процедура ЗавершениеВыбораОбъектаМетаданных(Результат, ДополнительныеПараметры) Экспорт
	...
	ПолноеИмяРеквизита = Результат.Значение;
	ДобавитьТипыРеквизитаВТаблицу(ДополнительныеПараметры.ИмяТаблицы, ПолноеИмяРеквизита);
...
Функция РеквизитыОбъектаМетаданных(ИмяОбъекта)
	...
	Для Каждого Коллекция Из ЕщеРеквизитыОбъекта Цикл
		...
		Для Каждого Реквизит Из Коллекция.Значение Цикл
			ПолноеИмяРеквизита = ПолноеИмяОбъекта + "." + Коллекция.Ключ;
...
Процедура ДобавитьТипыРеквизитаВТаблицу(ИмяТаблицы, ПолноеИмяРеквизита)
	...
	ИменаРеквизита = СтрРазделить(ПолноеИмяРеквизита, ".");
	ИменаРеквизита[0] = МножественноеЧисло[ИменаРеквизита[0]];
	Реквизит = Метаданные.НайтиПоПолномуИмени(СтрСоединить(ИменаРеквизита, "."));
	Для Каждого Тип Из Реквизит.Тип.Типы() Цикл
		...
		НоваяСтрока = ТабличнаяЧасть.Добавить(); // СтрокаТаблицыЗначений - 
		НоваяСтрока.Ссылка = Объект.ОбработчикиОбновления[0].Ссылка;
...
Функция ЕстьЦиклПорядкаВыполненияОбработчиков(Сообщать = Истина)
	...
	ЕстьЦикл = ЭтотОбъект.ВладелецФормы.ЕстьЦиклПорядкаВыполненияОбработчиков(ЭтотОбработчик, Объект.ПриоритетыВыполнения);
	Возврат ЕстьЦикл;
...
Процедура УстановитьПорядокПоУмолчанию()
	...
	ВыборПорядка.Вставить("До","Обработчик1");
	ВыборПорядка.Вставить("После","Обработчик2");
	ВыборПорядка.Вставить("Любой","Любой");
