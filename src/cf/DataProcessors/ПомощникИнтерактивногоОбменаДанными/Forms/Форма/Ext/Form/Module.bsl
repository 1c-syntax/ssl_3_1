...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ ПустаяСтрока(Параметры.КодУзлаИнформационнойБазы)
		И НЕ ПустаяСтрока(Параметры.ИмяПланаОбмена) Тогда
	...
	ЗакрытьФормуБезусловно = (Параметры.ЗакрытьФормуБезусловно = ИСТИНА);
	ИнициализироватьНастройкиТранспортаСообщенийОбмена();
	
	Если Не ЗначениеЗаполнено(Объект.ИдентификаторТранспорта) Тогда
		
		Текст = НСтр("ru = 'Не заданы настройки подключения. Синхронизация не может быть продолжена.'", 
	...
	ВсеУзлы = ОбменДаннымиПовтИсп.УзлыПланаОбмена(Объект.ИмяПланаОбмена);
	...
	ЭтоУзелОбъектнойКонвертации = ОбменДаннымиПовтИсп.СпособКонвертацииОбъектовXDTO(Объект.УзелИнформационнойБазы);
	...
	Параметры.Свойство("ИдентификаторТранспорта", Объект.ИдентификаторТранспорта);
	Параметры.Свойство("ОбластьДанныхКорреспондента",  ОбластьДанныхКорреспондента);
	...
	НастройкиТранспорта = ТранспортСообщенийОбмена.НастройкиТранспортаПоУмолчанию(
		Объект.УзелИнформационнойБазы,
		Объект.ИдентификаторТранспорта);

	Если Объект.ИдентификаторТранспорта = "SM" И НастройкиТранспорта.ВнутренняяПубликация Тогда
	...
	ДанныеАутентификации = ТранспортСообщенийОбмена.ПарольСинхронизацииДанных(Объект.УзелИнформационнойБазы);
	
	ИспользоватьПрогрессБар = ТранспортСообщенийОбмена.ПараметрТранспорта(Объект.ИдентификаторТранспорта, "ИспользоватьПрогресс");

	Элементы.ГруппаПредупреждениеПриОбменеЧерезВнутреннююПубликацию.Видимость = 
		ОбменЧерезВнутреннююПубликацию И Не ПолученоСообщениеДляСопоставленияДанных;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ПолученоСообщениеДляСопоставленияДанных И Не ОтменаСопоставления Тогда
		Если (ЗакончитьСопоставлениеДанных И Не ПропуститьПолучениеДанных)
			...
			УдалитьСообщениеДляСопоставленияДанных(Объект.УзелИнформационнойБазы, Объект.ИдентификаторТранспорта);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПерейтиКПредупреждениямВыгрузки(Команда)
	
	// Выгрузка только в пакетном режиме проверка ЭтоУзелОбъектнойКонвертации не нужна.
	ОткрытьРезультатыПакетнойСинхронизации(Объект.УзелИнформационнойБазы, "СобытияВыгрузки")
	
КонецПроцедуры

&НаКлиенте
Процедура ПерейтиКПредупреждениямЗагрузки(Команда)
	
	Если ЭтоУзелОбъектнойКонвертации Тогда
		
		ОткрытьРезультатыОбъектнойСинхронизации(Объект.УзелИнформационнойБазы);
		
	Иначе
		
		ОткрытьРезультатыПакетнойСинхронизации(Объект.УзелИнформационнойБазы, "СобытияЗагрузки")
...
&НаКлиенте
Процедура ОткрытьРезультатыОбъектнойСинхронизации(УзелИнформационнойБазы)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("УзелИнформационнойБазы", УзелИнформационнойБазы);
	
	ОткрытьФорму("Обработка.РаботаСРезультатамиОбмена.Форма.РазборРезультатовПообъектногоОбмена", ПараметрыОткрытия, ЭтотОбъект, УзелИнформационнойБазы);
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьРезультатыПакетнойСинхронизации(УзелИнформационнойБазы, ИмяНабораСобытий)
	
	МассивУзловПланаОбмена = Новый Массив;
	МассивУзловПланаОбмена.Добавить(УзелИнформационнойБазы);
	
	ОтборУзловПланаОбмена = Новый Массив;
	ОтборУзловПланаОбмена.Добавить(УзелИнформационнойБазы);
	...
	Если ПустаяСтрока(ИмяНабораСобытий)
		...
		Если ПодсистемаВерсионированияСуществует Тогда
			...
			ОтборТипыПредупреждений.Добавить(ПредопределенноеЗначение("Перечисление.ТипыВерсийОбъекта.НепринятыеДанныеПоКоллизии"));
			ОтборТипыПредупреждений.Добавить(ПредопределенноеЗначение("Перечисление.ТипыВерсийОбъекта.ПринятыеДанныеПоКоллизии"));
			ОтборТипыПредупреждений.Добавить(ПредопределенноеЗначение("Перечисление.ТипыВерсийОбъекта.НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе"));
			ОтборТипыПредупреждений.Добавить(ПредопределенноеЗначение("Перечисление.ТипыВерсийОбъекта.НепринятыйПоДатеЗапретаОбъектСуществуетВБазе"));
...
Процедура ИнициализироватьНастройкиТранспортаСообщенийОбмена()
	...
	ИдентификаторТранспорта = ТранспортСообщенийОбмена.ТранспортПоУмолчанию(Объект.УзелИнформационнойБазы);
	
	ПропускатьСтраницуТранспорта = Истина;
	
	Если Не ЗначениеЗаполнено(Объект.ИдентификаторТранспорта) Тогда
		Объект.ИдентификаторТранспорта = ИдентификаторТранспорта;
	КонецЕсли;
...
Процедура ИнициализироватьРеквизитыДополненияВыгрузки()
	...
	НастройкиДополненияВыгрузки = ОбменДаннымиСервер.ИнтерактивноеИзменениеВыгрузки(
		Объект.УзелИнформационнойБазы, УникальныйИдентификатор, ДополнениеВыгрузкиРасширенныйРежим);
...
&НаСервереБезКонтекста
Процедура УдалитьСообщениеДляСопоставленияДанных(Знач УзелОбмена, Знач ИдентификаторТранспорта = Неопределено)
	...
	Если ЗначениеЗаполнено(ОбщиеНастройки.СообщениеДляСопоставленияДанных) Тогда
		...
		ПассивныйРежимФайловаяИБ = ОбщегоНазначения.ИнформационнаяБазаФайловая() 
			И ТранспортСообщенийОбмена.ПараметрТранспорта(ИдентификаторТранспорта, "ПассивныйРежим");
			
		ИмяФайлаСообщенияВХранилище = ОбменДаннымиСервер.ПолучитьФайлИзХранилища(ОбщиеНастройки.СообщениеДляСопоставленияДанных, ПассивныйРежимФайловаяИБ);
...
&НаКлиенте
Процедура ЗапуститьФоновоеЗаданиеПолучитьСообщениеОбменаВоВременныйКаталог(ПараметрыЗадания, Отказ, ПолучитьПарольИзСеансовыхДанных)
	
	Результат = ЗапуститьФоновоеЗаданиеПолучитьСообщениеОбменаВоВременныйКаталогНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных);
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		ЗапуститьОжиданиеЗавершения(Результат);
...
&НаКлиенте
Процедура ЗапуститьФоновоеЗаданиеВыполнитьАвтоматическоеСопоставлениеДанных(ПараметрыЗадания, Отказ, ПолучитьПарольИзСеансовыхДанных)
	
	Результат = ЗапуститьФоновоеЗаданиеВыполнитьАвтоматическоеСопоставлениеДанныхНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных);
...
&НаКлиенте
Процедура ЗапуститьФоновоеЗаданиеВыполнитьЗагрузкуДанных(ПараметрыЗадания, Отказ, ПолучитьПарольИзСеансовыхДанных)
	
	Результат = ЗапуститьФоновоеЗаданиеВыполнитьЗагрузкуДанныхНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных);
...
&НаКлиенте
Процедура ЗапуститьФоновоеЗаданиеВыполнитьВыгрузкуДанных(ПараметрыЗадания, Отказ, ПолучитьПарольИзСеансовыхДанных)
	
	Результат = ЗапуститьФоновоеЗаданиеВыполнитьВыгрузкуДанныхНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных);
...
&НаКлиенте
Процедура ЗапуститьОжиданиеЗавершения(Результат)
	
	ДлительнаяОперация = Истина;
	ИдентификаторЗадания = РезультатВыполненияФоновогоЗадания.ИдентификаторЗадания;
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеПолучитьСообщениеОбменаВоВременныйКаталогНаСервере(Знач ПараметрыЗадания, Знач ПолучитьПарольИзСеансовыхДанных)
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ПомощникИнтерактивногоОбменаДанными.ПолучитьСообщениеОбменаВоВременныйКаталог",
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьАвтоматическоеСопоставлениеДанныхНаСервере(Знач ПараметрыЗадания, Знач ПолучитьПарольИзСеансовыхДанных)
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьЗагрузкуДанныхНаСервере(Знач ПараметрыЗадания, Знач ПолучитьПарольИзСеансовыхДанных)
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьВыгрузкуДанныхНаСервере(Знач ПараметрыЗадания, Знач ПолучитьПарольИзСеансовыхДанных)
...
Функция Подключаемый_СтраницаОжиданияАнализаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПолучитьПарольИзСеансовыхДанных = Ложь;
	...
	ПараметрыМетода.Вставить("ИдентификаторТранспорта", Объект.ИдентификаторТранспорта);
	ПараметрыМетода.Вставить("ДанныеАутентификации", ДанныеАутентификации);
	...
	ЗапуститьФоновоеЗаданиеПолучитьСообщениеОбменаВоВременныйКаталог(ПараметрыЗадания, Отказ, ПолучитьПарольИзСеансовыхДанных);
...
Функция Подключаемый_АнализДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ЗапуститьФоновоеЗаданиеВыполнитьАвтоматическоеСопоставлениеДанных(ПараметрыЗадания, Отказ, Ложь);
...
Функция Подключаемый_ЗагрузкаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ЗапуститьФоновоеЗаданиеВыполнитьЗагрузкуДанных(ПараметрыЗадания, Отказ, Ложь);
...
Процедура ПриНачалеВыгрузкиДанных(Отказ)
	...
	Иначе
		...
		ЗапуститьФоновоеЗаданиеВыполнитьВыгрузкуДанных(ПараметрыЗадания, Отказ, ПолучитьПарольИзСеансовыхДанных);
	КонецЕсли;
...
Функция Подключаемый_СтраницаОкончанияСопоставления_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Если РезультатЗагрузкиДанных = "Успех"
		И Параметры.ОбъектнаяКонвертацияXDTO = Истина
		Тогда
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	Если ОбменМеждуПриложениямиВСервисе Тогда
		...
		Если ПолученоСообщениеДляСопоставленияДанных Тогда
			...
			Если НЕ Параметры.ОбъектнаяКонвертацияXDTO Тогда
