...
Функция СостояниеДлительнойОперации(Знач ИдентификаторОперации, УзелОбмена, ПолучитьПарольИзСеансовыхДанных, СтрокаСообщенияОбОшибке = "")
	...
	Попытка
		...
		СохраненныеПараметры = РегистрыСведений.НастройкиТранспортаОбменаДанными.НастройкиТранспортаWS(
			УзелОбмена, ПараметрыАутентификации);
...
Процедура ПроверитьПодключениеИСохранитьНастройки(Отказ)
	...
	ЕстьПодключение = ОбменДаннымиСервер.ЕстьПодключениеККорреспонденту(Объект.УзелИнформационнойБазы,
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	ТаблицаПереходовНоваяСтрока("СтраницаНачала", "СтраницаНавигацииНачало", "СтраницаНачала_ПриОткрытии", "СтраницаНачала_ПриПереходеДалее");
