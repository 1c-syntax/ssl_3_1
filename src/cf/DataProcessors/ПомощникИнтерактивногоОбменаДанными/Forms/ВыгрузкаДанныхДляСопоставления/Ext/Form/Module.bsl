...
Процедура НастройкиПодключения(Команда)
	...
	ПараметрыФормы = Новый Структура("Корреспондент", УзелОбмена);
	
	ОткрытьФорму("Справочник.НастройкиТранспортаСообщенийОбмена.Форма.ПанельНастроекТранспорта",
		ПараметрыФормы,,,,, ОповещениеОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПриНачалеПроверкиПодключенияНаСервере(ПродолжитьОжидание)
	...
	НастройкиПодключения.Вставить("Корреспондент", УзелОбмена);
	НастройкиПодключения.Вставить("ИдентификаторТранспорта", ИдентификаторТранспорта);
	...
	НастройкиПодключения.Вставить("ВариантНастройкиОбмена", "");
	НастройкиПодключения.Вставить("ДанныеАутентификации", ДанныеАутентификации);
...
Процедура ПриНачалеВыгрузкиДанныхДляСопоставленияНаСервере(ПродолжитьОжидание)
	...
	Иначе
		...
		НастройкиВыгрузки.Вставить("ИдентификаторТранспорта", ИдентификаторТранспорта);
		НастройкиВыгрузки.Вставить("НастройкиТранспорта", НастройкиТранспорта);
		НастройкиВыгрузки.Вставить("ДанныеАутентификации", ДанныеАутентификации);
...
Процедура ИнициализироватьПараметрыТранспорта()
	...
	ИспользоватьПрогресс = ТранспортСообщенийОбмена.ПараметрТранспорта(ИдентификаторТранспорта, "ИспользоватьПрогресс");
	
	ЗапроситьПароль              = Ложь;
	ПроверкаПодключенияВыполнена = Ложь;
	ВыгрузкаДанныхВыполнена      = Ложь;
	ДоступнаНастройкаТранспорта  = Ложь;
	
	ПассивныйРежим = ТранспортСообщенийОбмена.ПараметрТранспорта(УзелОбмена, "ПассивныйРежим");
	
	Если ПассивныйРежим Тогда
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	ПассивныйРежим = ТранспортСообщенийОбмена.ПараметрТранспорта(ИдентификаторТранспорта, "ПассивныйРежим");
	Если НЕ ПассивныйРежим Тогда
