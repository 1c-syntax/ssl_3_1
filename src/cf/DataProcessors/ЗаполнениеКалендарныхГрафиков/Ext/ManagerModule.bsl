...
Функция ПериодыНерабочихДней(ДанныеКалендаря, Период) Экспорт

	Периоды = Новый Массив;
	
	Если СокрЛП(ДанныеКалендаря.Код) = "РФ" Тогда
		Периоды.Добавить(
			ОписаниеПериодаНерабочихДней(
				Дата(2020, 3, 30), Дата(2020, 4, 3), НСтр("ru = 'Указ Президента РФ от 25.03.2020 № 206'")));
		Периоды.Добавить(
			ОписаниеПериодаНерабочихДней(
				Дата(2020, 4, 4), Дата(2020, 4, 30), НСтр("ru = 'Указ Президента РФ от 02.04.2020 № 239'")));
	КонецЕсли;
...
Функция ОписаниеПериодаНерабочихДней(Начало, Окончание, Основание = "")
	
	ОбщегоНазначенияКлиентСервер.Проверить(Начало <= Окончание, НСтр("ru = 'Начало периода позже его завершения'"));
	
	ОписаниеПериода = Новый Структура("Период, Основание, Даты, Представление");
