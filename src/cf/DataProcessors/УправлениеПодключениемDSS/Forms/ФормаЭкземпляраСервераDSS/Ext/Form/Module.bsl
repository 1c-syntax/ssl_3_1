...
&НаКлиенте
Перем НовыйЭлемент, ЗащищенноеСоединениеГОСТ;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоНаИзменения = СервисКриптографииDSSСлужебный.ПроверитьПраво("ЭкземплярыСервераDSS", "Изменение");
	
	Если НЕ ПравоНаИзменения Тогда
		ВызватьИсключение НСтр("ru = 'Пользователь не обладает правами редактирования сведений о сервере DSS.'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ЗащищенноеСоединениеГОСТ = Объект.ЗащищенноеСоединениеГОСТ;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Изменен Тогда
		ОбработатьДанныеЗащищенногоХранилища(Истина);
		ПриСозданииЧтенииНаСервере();
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ЗащищенноеСоединениеГОСТПриИзменении(Элемент)
	
	Если ЗащищенноеСоединениеГОСТ И Не Объект.ЗащищенноеСоединениеГОСТ Тогда

		СистемнаяИнформация = Новый СистемнаяИнформация;

		Ответ = Ждать ВопросАсинх(НСтр("ru = 'Для создания защищенного соединения с сервером DSS рекомендуется использовать сертифицированные средства криптографии и сертификаты ГОСТ.
		|Продолжить?'", 
		СервисКриптографииDSSСлужебныйКлиент.КодЯзыка()),
		РежимДиалогаВопрос.ДаНет,
		30);
		...
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			Объект.ЗащищенноеСоединениеГОСТ = ЗащищенноеСоединениеГОСТ;
			Возврат;
		КонецЕсли;
...
Процедура АдресСервераПриИзменении(Элемент)
	...
	ВидимостьЗащищенноеСоединениеГОСТ(ЭтотОбъект);
	Если Параметры.Ключ.Пустая() И Элементы.ЗащищенноеСоединениеГОСТ.Видимость Тогда
		Объект.ЗащищенноеСоединениеГОСТ = Истина;
	КонецЕсли;
...
Процедура ПолучитьОписаниеСервиса(Интерактивно = Ложь)
	...
	Если Интерактивно Тогда
		ПараметрыОперации	= СервисКриптографииDSSСлужебныйКлиент.ПодготовитьПараметрыОперации();
		ПараметрыОперации.ЗащищенноеСоединениеГОСТ = Объект.ЗащищенноеСоединениеГОСТ;
...
Процедура СформироватьНаименование()
	...
	Если Объект.Наименование = ПрежнийСервер И ПравоНаИзменения Тогда
...
Процедура УправлениеФормой(КонтекстФормы)
	...
	Элементы.СекретСервисногоКлиента.Доступность = КонтекстФормы.ПравоНаИзменения;
	...
	ВидимостьЗащищенноеСоединениеГОСТ(КонтекстФормы);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ВидимостьЗащищенноеСоединениеГОСТ(Форма)
	
	ЗащищенноеСоединение = СтрНачинаетсяС(Форма.Объект.АдресСервера, "https://");
	Форма.Элементы.ЗащищенноеСоединениеГОСТ.Видимость = ЗащищенноеСоединение;
	Форма.Элементы.ДекорацияЗащищенноеСоединение.Видимость = Не ЗащищенноеСоединение;
...
Процедура ОбновитьСтатусЗащищенногоХранилища()
	...
	Если НЕ ТолькоПросмотр И ПравоНаИзменения Тогда
		СекретСервисногоКлиента = 
			СервисКриптографииDSSКлиентСервер.ПолучитьЗначениеПароля(ОбработатьДанныеЗащищенногоХранилища().СекретСервисногоКлиента, Неопределено);
	КонецЕсли;

КонецПроцедуры

&НаСервере
Функция ОбработатьДанныеЗащищенногоХранилища(Записать = Ложь)
	
	ПустыеНастройки = СервисКриптографииDSSСлужебный.НастройкиПользователяПоУмолчанию();
	СервисКриптографииDSSСлужебный.ДополнитьНастройкиПользователя(ПустыеНастройки, Объект, "");
	КлючХранения = СервисКриптографииDSS.КлючСервисногоКлиента(ПустыеНастройки);

	Результат = СервисКриптографииDSSСлужебный.ПолучитьЗакрытыеДанные(КлючХранения, , Истина);
		
	Если Записать Тогда
		Результат.СекретСервисногоКлиента = СервисКриптографииDSSКлиентСервер.ОбъектПароля(СекретСервисногоКлиента, 0);
		СервисКриптографииDSSСлужебный.СохранитьЗакрытыеДанные(КлючХранения, Результат, , Истина);
	КонецЕсли;
