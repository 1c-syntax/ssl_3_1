...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НовыйЭлемент Тогда
		...
		ПоказатьВопрос(ОповещениеСледующее, 
			НСтр("ru = 'Добавлена информация о новом сервере облачной подписи.
...
&НаКлиенте
Процедура ДополнитьСписокСервером(Элемент, ТекстРедактирования, СтандартнаяОбработка = Истина)
	...
	НачалоПозиции = 0;
	Элемент.ПолучитьГраницыВыделения(,НачалоПозиции,,);
	АдресСервера = Объект.АдресСервера;
