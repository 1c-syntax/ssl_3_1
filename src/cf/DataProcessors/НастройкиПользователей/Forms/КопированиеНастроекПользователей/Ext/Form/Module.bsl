...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		Если Источник <> ЭтотОбъект.ИмяФормы Тогда
			Возврат;
		КонецЕсли;
...
Процедура ОткрытьФормуВыбораПользователей(ВыбранныйТипПользователей, ПараметрыОтбора)
	...
	ИначеЕсли ВыбранныйТипПользователей = "ВнешниеПользователи"
		...
		ОткрытьФорму("Справочник.ВнешниеПользователи.ФормаСписка", ПараметрыОтбора, Элементы.ПользовательСсылка);
	КонецЕсли;
...
Процедура ВыбратьПользователей(Элемент)
	...
	ПараметрыФормы.Вставить("Источник", ЭтотОбъект.ИмяФормы);
...
Процедура Скопировать(Команда)
	...
	Если РезультатПроверки = "ЕстьАктивныеПользователиПолучатели"
		Или (ЗначениеЗаполнено(ОткрытыеКопируемыеФормы) И РезультатПроверки = "ТекущийПользовательПолучатель") Тогда
		...
		Если ПереключательКопируемыеНастройки = "СкопироватьВсе" 
			...
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("СкопироватьПродолжение", ЭтотОбъект);
			...
			ОткрытьФорму("Обработка.НастройкиПользователей.Форма.ПредупреждениеОКопированииНастроек", ПараметрыФормы, , , , , ОповещениеОЗакрытии);
			Возврат;
...
&НаКлиенте
Процедура СкопироватьПродолжение(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Структура")
		И Результат.Свойство("Действие")
		И Результат.Действие = "СкопироватьИЗакрыть" Тогда
		СкопироватьНастройки(Результат.Действие);
	КонецЕсли;
