...
Функция СписокНастроекФорм(СписокФорм, ИмяПользователя)
	...
	СтрокаРезультата = Неопределено; // СтрокаТаблицыЗначений: см. ПалитраСвойствСпискаНастроекФорм
...
Процедура УдалитьУстаревшуюНастройку(Выборка, Хранилище, Контекст)
	...
	Если ТипОбъектовПоддерживается Тогда
		...
		Если ИмяОбъектаКорректно = Неопределено Тогда
			ИмяОбъектаКорректно = Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта) <> Неопределено;
...
Функция СледующаяНастройка(Выборка, ОбработкаОшибок = "ПриОшибкеПропустить", Хранилище = Неопределено)
	...
	ТекущийКлюч = Выборка.КлючОбъекта + " " + Выборка.КлючНастроек;
	...
	СледующийКлюч = Выборка.КлючОбъекта + " " + Выборка.КлючНастроек;
	Если ЕстьОшибка Тогда
		ЕстьСледующий = ТекущийКлюч <> СледующийКлюч;
	КонецЕсли;
