...
//    * Пометка - Булево - 
//    * Картинка - Картинка - 
//    * СписокКлючей - СписокЗначений - где:
//        ** Значение - Строка - 
//
...
Процедура ЗаполнитьФормыОбъектовМетаданных(СписокОбъектовМетаданных, ТипОбъектаМетаданных,
	...
	Для Каждого Объект Из СписокОбъектовМетаданных Цикл
		...
		Для Каждого ИмяСтандартнойФормы Из ИменаСтандартныхФорм Цикл
			...
			Форма = Объект["Основная" + ИмяСтандартнойФормы.Значение];
...
Функция ПользователиДляКопирования(ПользовательИсточник, ТаблицаПользователей, ПользовательВнешний, Очистка = Ложь) Экспорт
	...
	Если Очистка Тогда
		Запрос.Текст = ТекстЗапросаСпискаВсехПользователей(Ложь, Очистка)
			+ Символы.ПС + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС
			+ ТекстЗапросаСпискаВсехПользователей(Истина, Очистка);
	Иначе
		Запрос.Текст = ТекстЗапросаСпискаВсехПользователей(ПользовательВнешний, Очистка);
	КонецЕсли;
...
Функция ТекстЗапросаСпискаВсехПользователей(ПользовательВнешний, Очистка)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КромеСлужебныхПользователей",
		?(Очистка И Не ОбщегоНазначения.РазделениеВключено() Или ПользовательВнешний,
...
//    * НастройкиОтчетовДерево - ДеревоЗначений - где:
...
Процедура ЗаполнитьСписокНастроекОтчетов(Параметры)
	...
	Для Каждого Настройка Из Настройки Цикл
		...
		Если Не ПустаяСтрока(Параметры.Поиск) Тогда
			Если СтрНайти(ВРег(ПредставлениеВариантаОтчета), ВРег(Параметры.Поиск)) = 0
				И СтрНайти(ВРег(НазваниеНастройки), ВРег(Параметры.Поиск)) = 0 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого ВариантОтчета Из ТаблицаВариантовОтчетов Цикл
		...
		Если Не ПустаяСтрока(Параметры.Поиск) Тогда
			...
			Если СтрНайти(ВРег(ВариантОтчета.Представление), ВРег(Параметры.Поиск)) = 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьСписокНастроекВнешнегоВида(Параметры)
	...
	Для Каждого НастройкаФормы Из НастройкиФорм Цикл
		...
		Если Не ПустаяСтрока(Параметры.Поиск) Тогда
			...
			Если СтрНайти(ВРег(НастройкаФормы.Представление), ВРег(Параметры.Поиск)) = 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьСписокПрочихНастроек(Параметры)
	...
	Если Ключи.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(Параметры.Поиск) Тогда
			Если СтрНайти(ВРег(НСтр("ru = 'Персональные настройки'")), ВРег(Параметры.Поиск)) = 0 Тогда
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru = 'Настройки печати табличных документов'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru = 'Избранное'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
		...
		Для Каждого ПрочаяНастройка Из ПрочиеНастройки Цикл
			...
			Если Результат.СписокНастроек.Количество() <> 0 Тогда
				...
				Если Не ПустаяСтрока(Параметры.Поиск) Тогда
					...
					Если СтрНайти(ВРег(Результат.НазваниеНастройки), ВРег(Параметры.Поиск)) = 0 Тогда
						ВыводитьНастройку = Ложь;
					КонецЕсли;
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru = 'Прочие настройки'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
...
Процедура ДобавитьНастройкиРабочегоСтолаИКомандногоИнтерфейса(Параметры, ДеревоНастроек)
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		Если СтрНайти(ВРег(НСтр("ru = 'Командный интерфейс и начальная страница'")), ВРег(Параметры.Поиск)) = 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
