...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение'"),, Строка(Объект.Ссылка), БиблиотекаКартинок.ДиалогИнформация);
...
#Область ПреобразованиеРезультатовЗаполненияАнкетыВТабличнуюЧастьДокумента
...
Процедура ЗаполнитьОтветыТабличныйВопросОтветыВСтроках(СтрокаДерева, Таблица)
	...
	ИмяТаблицы          = АнкетированиеКлиентСервер.ИмяВопроса(СтрокаДерева.КлючСтроки) + "_Таблица";
	ИмяКолонкиБезНомера = ИмяТаблицы + "_Колонка_";
