...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Объект.ДатаПолучения = Параметры.ДатаСогласия;
		...
		Если ЗначениеЗаполнено(Параметры.Субъект) Тогда
...
Процедура НачатьПредупреждениеОНеобходимостиУведомленияРКН(ПараметрыЗаписи)
	...
	ПараметрыДиалога.Картинка = БиблиотекаКартинок.ДиалогВосклицание;
...
Функция ТекстПредупрежденияОбУведомленииРКН()
	Возврат Документы.СогласиеНаОбработкуПерсональныхДанных.ПолучитьМакет(
		"ПредупреждениеОбУведомленииРКН" + "_"+ ОбщегоНазначения.КодОсновногоЯзыка()).ПолучитьТекст();
КонецФункции
