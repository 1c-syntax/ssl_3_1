...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Элементы.УточнитьКонтакты.Заголовок                  = НСтр("ru = 'Сохранить адреса в контакты'");
		Элементы.УточнитьКонтакты.Отображение                = ОтображениеКнопки.Текст;
		Элементы.ГруппаЛево.ВыравниваниеЭлементовИЗаголовков = ВариантВыравниванияЭлементовИЗаголовков.ЭлементыПравоЗаголовкиЛево;
		Элементы.Тема.ПоложениеЗаголовка                     = ПоложениеЗаголовкаЭлементаФормы.Верх;
		Элементы.ГруппаВложения.ОтображатьЗаголовок          = Истина;
		Элементы.ГруппаПолучатели.ОтображатьЗаголовок        = Истина;
		Элементы.СохранитьВложение.Отображение               = ОтображениеКнопки.Картинка;
		Элементы.УдалитьВложение.Отображение                 = ОтображениеКнопки.Картинка;
		Элементы.УдалитьВложение.ПоложениеВКоманднойПанели   = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
	КонецЕсли
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, РежимЗаписи, РежимПроведения)
	...
	Если ТекущийОбъект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик Тогда
...
Процедура УправлениеДоступностью()
	...
	Если Объект.СтатусПисьма = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Исходящее") Тогда
		Элементы.ДатаОтправления.Заголовок = НСтр("ru = 'Передано на отправку'");
	Иначе
		Элементы.ДатаОтправления.Заголовок = НСтр("ru = 'Отправлено'");
	КонецЕсли;
...
Процедура ОпределитьВидимостьДоступностьЭлементовВЗависимостиОтСтатусаПисьма()
	...
	Если Объект.СтатусПисьма <> Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик Тогда
		...
		Если Объект.ТипТекста = Перечисления.ТипыТекстовЭлектронныхПисем.HTML Тогда
			...
			ТекстПисьма = Взаимодействия.ОбработатьТекстHTML(Объект.Ссылка, Ложь, Ложь);
...
Процедура СохранитьВложение(Команда)
	...
	Если (ТекущиеДанные.Расположение = 0) ИЛИ (ТекущиеДанные.Расположение = 1) Тогда
		...
		ПараметрыПолученияФайла = РаботаСФайламиКлиент.ПараметрыАсинхронногоПолученияФайла("СохранитьКак", "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный");
	...
	ИначеЕсли ТекущиеДанные.Расположение = 2 Тогда
		...
		СохранитьВложениеИзВременногоХранилища(АдресВоВременномХранилище, ТекущиеДанные.ИмяФайла);
		
	ИначеЕсли ТекущиеДанные.Расположение = 4 Тогда
		
		СохранитьВложениеИзВременногоХранилища(ТекущиеДанные.ИмяФайлаНаКомпьютере, ТекущиеДанные.ИмяФайла);
...
&НаКлиенте
Процедура СохранитьВложениеИзВременногоХранилища(АдресВоВременномХранилище, ИмяФайла)
	
	ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru = 'Сохранить вложение'");
	
	ФайловаяСистемаКлиент.СохранитьФайл(, АдресВоВременномХранилище, ИмяФайла, ПараметрыСохранения);
	...
	Если ПереданныеПараметры.Свойство("Вложения") И ПереданныеПараметры.Вложения <> Неопределено Тогда
		...
		Если ТипЗнч(ПереданныеПараметры.Вложения) = Тип("СписокЗначений") Или ТипЗнч(ПереданныеПараметры.Вложения) = Тип("Массив") Тогда
			Для Каждого Вложение Из ПереданныеПараметры.Вложения Цикл
				...
				ОписаниеВложения.ИндексКартинки = РаботаСФайламиСлужебныйКлиентСервер.ИндексПиктограммыФайла(Расширение);
			КонецЦикла;
		КонецЕсли;
...
Процедура ОтправитьВыполнить()
	...
	Если НайденныеСтроки[0].УдалятьПослеОтправки Тогда
		...
		ТекстВопроса = НСтр("ru = 'В настройках почты не предусмотрено сохранение отправленных писем в приложении.
		                    |Продолжить?'");
...
Функция ВыполнитьОтправкуНаСервере()
	...
	Исключение
		...
		ТекстОшибки = РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(
			ИнформацияОбОшибке, ОбщегоНазначения.КодОсновногоЯзыка(), , ПисьмоОбъект.УчетнаяЗапись);
