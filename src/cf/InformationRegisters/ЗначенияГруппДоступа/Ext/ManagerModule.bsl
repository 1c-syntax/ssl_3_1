...
Процедура ОбновитьДанныеРегистра(ГруппыДоступа = Неопределено, ЕстьИзменения = Неопределено) Экспорт
	...
	Если УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
		УправлениеДоступомСлужебный.ОчиститьРегистрСведений(
			Метаданные.РегистрыСведений.ЗначенияГруппДоступа.ПолноеИмя(), ЕстьИзменения);
		УправлениеДоступомСлужебный.ОчиститьРегистрСведений(
			Метаданные.РегистрыСведений.ЗначенияГруппДоступаПоУмолчанию.ПолноеИмя(), ЕстьИзменения);
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ОграничениеДоступаВключено = УправлениеДоступомСлужебный.КонстантаОграничиватьДоступНаУровнеЗаписей();
...
Процедура ОбновитьРазрешенныеЗначения(ИспользуемыеВидыДоступа,
	...
	|			ТОГДА ЕСТЬNULL(ВыбранныеЗначенияДоступа.ЗначениеВИерархии, НастройкиЗначений.ЗначениеДоступа)
