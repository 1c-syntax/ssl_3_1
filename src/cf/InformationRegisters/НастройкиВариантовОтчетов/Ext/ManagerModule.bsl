...
#Область ОбработчикиЧтенияЗаписиНастроекДоступностиВариантаОтчета
...
Функция КоличествоПользователейВариантОтчета(ПользователиВарианта) Экспорт 
	...
	Для Каждого ПользовательВарианта Из ПользователиВарианта Цикл 
		...
		Если ВыбранныйПользователь = Неопределено Тогда 
			ВыбранныйПользователь = ГруппаВсеПользователи;
		КонецЕсли;
...
Процедура ДобавитьПользователейВариантаОтчета(ВариантОтчета, ПользователиВарианта, УведомитьПользователей)
	...
	ОбщиеГруппыПользователей.Добавить(Пользователи.ГруппаВсеПользователи());
	ОбщиеГруппыПользователей.Добавить(ВнешниеПользователи.ГруппаВсеВнешниеПользователи());
...
#Область ОбработчикиОповещенияПользователейВариантаОтчета
...
//  Замещение - РежимЗамещения, Булево
//
Процедура ОповеститьПользователейВариантаОтчета(Записи, Замещение) Экспорт 
	...
	СтарыеЗаписи = ОбщегоНазначения.ЗаписиНабораИзБазыДанных(Записи, Замещение, "Вариант");
	Если СтарыеЗаписи.Количество() = 0 Тогда
		ЭлементОтбора = Записи.Отбор.Найти("Вариант");
		Варианты = ЭлементОтбора.Значение;
	Иначе
		Варианты = СтарыеЗаписи.ВыгрузитьКолонку("Вариант");
	КонецЕсли;
	
	ПользователиВарианта = ПользователиВариантаОтчета(Варианты);
	
	Пока ПользователиВарианта.Следующий() Цикл 
		
		ВариантОтчета = ПользователиВарианта.Вариант;
		...
		Попытка
			МодульОбсуждения.ОтправитьСообщение(Пользователи.ТекущийПользователь(), ПользователиВарианта.Ссылка,
...
Функция ПользователиВариантаОтчета(ВариантОтчета, ВыбранныеПользователи = Неопределено) Экспорт 
	...
	|		ПО Пользователи.Ссылка = Составы.Пользователь
	|ГДЕ
	|	Настройки.Вариант В (&ВариантОтчета)
	|	И Настройки.Подсистема = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)
