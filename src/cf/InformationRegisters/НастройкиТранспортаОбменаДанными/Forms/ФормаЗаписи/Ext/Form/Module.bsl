...
Процедура ПроверитьПодключениеНаСервере(Отказ, ВидТранспортаСтрокой, НовыйПароль)
	...
	ПаролиДляПроверки = Новый Структура;
	
	Если COMПарольПользователяИзменен Тогда
		ПаролиДляПроверки.Вставить("COMПарольПользователя", COMПарольПользователя);
	КонецЕсли;
	
	Если FTPСоединениеПарольИзменен Тогда
		ПаролиДляПроверки.Вставить("FTPСоединениеПароль", FTPСоединениеПароль);
	КонецЕсли;
	
	Если WSПарольИзменен Тогда
		ПаролиДляПроверки.Вставить("WSПароль", WSПароль);
	КонецЕсли;
	
	Если ПарольАрхиваСообщенияОбменаИзменен Тогда
		ПаролиДляПроверки.Вставить("ПарольАрхиваСообщенияОбмена", ПарольАрхиваСообщенияОбмена);
	КонецЕсли;
