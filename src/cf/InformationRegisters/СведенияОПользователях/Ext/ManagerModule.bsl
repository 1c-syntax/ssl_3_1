...
Процедура ОбновитьДанныеРегистра(Пользователь = Неопределено, ЕстьИзменения = Неопределено) Экспорт
	...
	ТекущаяПорцияСсылок = Неопределено;
	ВсеПользователиИБ = Новый Соответствие;
	Если Пользователь = Неопределено
	 Или Выборка.Количество() > 1000 Тогда
		ПользователиИБ = ПользователиИнформационнойБазы.ПолучитьПользователей();
		Для Каждого ПользовательИБ Из ПользователиИБ Цикл
			ВсеПользователиИБ.Вставить(ПользовательИБ.УникальныйИдентификатор, ПользовательИБ);
		КонецЦикла;
	КонецЕсли;
	
	Пока Истина Цикл
		ПорцияСсылок = Неопределено;
		Пока Выборка.Следующий() Цикл
			Свойства = НовыеСвойстваПользователя(Выборка.Ссылка, Выборка,,
				ВсеПользователиИБ.Получить(Выборка.ИдентификаторПользователяИБ));
			Если Свойства = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если ТекущийТип <> ТипЗнч(Выборка.Ссылка)
			 Или ТекущаяПорцияСсылок.Количество() >= 100 Тогда
				ПорцияСсылок = ТекущаяПорцияСсылок;
				ТекущийТип = ТипЗнч(Выборка.Ссылка);
				ТекущаяПорцияСсылок = Новый ТаблицаЗначений;
				ТекущаяПорцияСсылок.Колонки.Добавить("Ссылка",
					Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущийТип)));
			КонецЕсли;
			ТекущаяПорцияСсылок.Добавить().Ссылка = Выборка.Ссылка;
			Если ЗначениеЗаполнено(ПорцияСсылок) Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если Не ЗначениеЗаполнено(ПорцияСсылок) Тогда
			Если Не ЗначениеЗаполнено(ТекущаяПорцияСсылок) Тогда
				Прервать;
			КонецЕсли;
			ПорцияСсылок = ТекущаяПорцияСсылок;
			ТекущаяПорцияСсылок = ПорцияСсылок.Скопировать(Новый Массив);
		КонецЕсли;
		
		Блокировка = Новый БлокировкаДанных;
		Если ТипЗнч(ПорцияСсылок[0].Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		КонецЕсли;
		ЭлементБлокировки.ИсточникДанных = ПорцияСсылок;
		...
		Запрос = ЗапросСвойств(ПорцияСсылок, Истина);
		РежимСлияния  = ОбщегоНазначения.РежимСлиянияНабораЗаписей();
		ПакетныйРежим = РежимСлияния <> Неопределено;
		...
		Попытка
			...
			ТекущаяВыборка = Запрос.Выполнить().Выбрать();
			Пока ТекущаяВыборка.Следующий() Цикл
				Свойства = НовыеСвойстваПользователя(ТекущаяВыборка.Ссылка, ТекущаяВыборка);
				Если Свойства = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если Не ПакетныйРежим Тогда
					НаборЗаписей.Отбор.Пользователь.Установить(ТекущаяВыборка.Ссылка);
				Иначе
					Запись = НаборЗаписей.Добавить();
				КонецЕсли;
				Если ТекущаяВыборка.НомерКартинкиСостояния = Null Тогда
					Запись.Пользователь = ТекущаяВыборка.Ссылка;
				Иначе
					ЗаполнитьЗначенияСвойств(Запись, ТекущаяВыборка);
				КонецЕсли;
				...
				КонецЕсли;
			КонецЦикла;
			Если ПакетныйРежим И ЗначениеЗаполнено(НаборЗаписей) Тогда
				НаборЗаписей.Записать(РежимСлияния);
				ЕстьИзменения = Истина;
			КонецЕсли;
...
//                 - Неопределено
//
//  ТекущиеСвойства - Неопределено
//                  - Структура - возвращаемое значение
//
Функция НовыеСвойстваПользователя(Пользователь, Выборка, ПользовательОбъект = Неопределено,
			ПользовательИБ = Неопределено, ТекущиеСвойства = Неопределено) Экспорт
	
	ТекущиеСвойства = ?(ТекущиеСвойства = Неопределено, Новый Структура, ТекущиеСвойства);
	ТекущиеСвойства.Вставить("ПометкаУдаления", Ложь);
	ТекущиеСвойства.Вставить("Недействителен", Ложь);
	ТекущиеСвойства.Вставить("ИдентификаторПользователяИБ",
		ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	ТекущиеСвойства.Вставить("Подразделение");
	ТекущиеСвойства.Вставить("ФизическоеЛицо");
	ТекущиеСвойства.Вставить("СтарыеЗначения", Неопределено);
	
	Если Выборка <> Неопределено И Выборка.ПометкаУдаления <> Null Тогда
		ТекущиеСвойства.СтарыеЗначения = Новый Структура("ПометкаУдаления,
			|Недействителен, ИдентификаторПользователяИБ, Подразделение, ФизическоеЛицо");
		ЗаполнитьЗначенияСвойств(ТекущиеСвойства.СтарыеЗначения, Выборка);
	КонецЕсли;
	
	Свойства = НовыеСвойства();
		...
		Если ТипЗнч(ПользовательОбъект) <> Тип("ДанныеФормыСтруктура")
		   ...
		   И ТипЗнч(ПользовательОбъект.ДополнительныеСвойства.РасширенныеСвойстваПользователяИБ) = Тип("Структура") Тогда
			...
			Если УровеньДоступа.НастройкиДляВхода Тогда
				ДопустимыеРасширенныеСвойства = Новый Структура(
					"ПотребоватьСменуПароляПриВходе,
					|СрокДействияНеОграничен,
					|СрокДействия,
					|ПросрочкаРаботыВПрограммеДоЗапрещенияВхода");
			Иначе
				ДопустимыеРасширенныеСвойства = Новый Структура(
					"ПотребоватьСменуПароляПриВходе");
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ДопустимыеРасширенныеСвойства, Свойства);
			ЗаполнитьЗначенияСвойств(ДопустимыеРасширенныеСвойства,
				ПользовательОбъект.ДополнительныеСвойства.РасширенныеСвойстваПользователяИБ);
			ЗаполнитьЗначенияСвойств(Свойства, ДопустимыеРасширенныеСвойства);
		КонецЕсли;
	...
	Иначе
		ТекущиеСвойства.ИдентификаторПользователяИБ = ПользовательИБ.УникальныйИдентификатор;
	КонецЕсли;
	...
	Если ТекущиеСвойства.ПометкаУдаления И ПользовательИБ = Неопределено Тогда
		Свойства.НомерКартинкиСостояния = 19 + ?(ЭтоВнешнийПользователь, 1, 0);
	
	ИначеЕсли ТекущиеСвойства.ПометкаУдаления Тогда
	...
	Если Выборка = Неопределено
	 Или ТекущиеСвойства.Свойство("ЗаписатьИзмененияВЖурнал") Тогда
		Возврат Свойства;
	КонецЕсли;
...
//                 - Неопределено
//
//  ЗаписатьИзмененияВЖурнал - Булево - записать изменения в журнал регистрации, так как было
//    изменение одного из свойств: ПометкаУдаления, Недействителен, ИдентификаторПользователяИБ.
//
//  ЕстьИзменения - Булево - возвращаемое значение.
//
Процедура ОбновитьСведенияОПользователе(Пользователь, ПользовательОбъект,
			ПользовательИБ = Неопределено, ЗаписатьИзмененияВЖурнал = Ложь, ЕстьИзменения = Ложь) Экспорт
	...
	Попытка
		...
		Если Не Выборка.Следующий() Тогда
			Выборка = Неопределено;
		КонецЕсли;
		ТекущиеСвойства = ?(ЗаписатьИзмененияВЖурнал,
			Новый Структура("ЗаписатьИзмененияВЖурнал"), Неопределено);
		Свойства = НовыеСвойстваПользователя(Пользователь,
			Выборка, ПользовательОбъект, ПользовательИБ, ТекущиеСвойства);
		...
		Если Свойства <> Неопределено Тогда
			...
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(Запись, Свойства);
			НаборЗаписей.ДополнительныеСвойства.Вставить("СвойстваПользователя", ТекущиеСвойства);
			НаборЗаписей.Записать();
			ЕстьИзменения = Истина;
		КонецЕсли;
		ЗафиксироватьТранзакцию();
...
//               - СправочникСсылка.ВнешниеПользователи
//
Процедура УдалитьСведенияОПользователе(Пользователь) Экспорт
...
Функция ЗапросСвойств(Пользователь, ВсеПоляРегистра = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Пользователь)
	 Или ТипЗнч(Пользователь) <> Тип("СправочникСсылка.Пользователи")
	   И ТипЗнч(Пользователь) <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		ТекстЗапросаСсылок = ТекстЗапросаСсылок();
		Если Пользователь = Неопределено Тогда
			ТекстЗапросаСсылок = СтрЗаменить(ТекстЗапросаСсылок,
				"Пользователи.Ссылка В(&Пользователь)", "ИСТИНА");
			ТекстЗапросаСсылок = СтрЗаменить(ТекстЗапросаСсылок,
				"СведенияОПользователях.Пользователь В(&Пользователь)", "ИСТИНА");
		КонецЕсли;
		Запрос.Текст = ТекстЗапросаСсылок;
		ТекстЗапросаСсылок = СтрЗаменить(ТекстЗапросаСсылок,
			"СсылкиПользователей", "СсылкиВнешнихПользователей");
		ТекстЗапросаСсылок = СтрЗаменить(ТекстЗапросаСсылок,
			"Справочник.Пользователи", "Справочник.ВнешниеПользователи");
		ТекстЗапроса = ?(ВсеПоляРегистра, ТекстЗапросаСвойствВсеПоляРегистра(), ТекстЗапросаСвойств());
		Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов()
			+ ТекстЗапросаСсылок + ОбщегоНазначения.РазделительПакетаЗапросов()
			+ ТекстЗапроса;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Пользователи.Подразделение", "НЕОПРЕДЕЛЕНО");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Пользователи.ФизическоеЛицо", "НЕОПРЕДЕЛЕНО");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "СсылкиПользователей", "СсылкиВнешнихПользователей");
		Запрос.Текст = Запрос.Текст + ОбщегоНазначения.ТекстОбъединитьВсе()
			+ СтрЗаменить(ТекстЗапроса, "Справочник.Пользователи", "Справочник.ВнешниеПользователи");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапросаСвойств(),
			"СсылкиПользователей", "(ВЫБРАТЬ &Пользователь КАК Ссылка)");
...
// Для функции ЗапросСвойств.
Функция ТекстЗапросаСсылок()
	
	Возврат
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Ссылки.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ СсылкиПользователей
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	
	|	ВЫБРАТЬ
	|		ВЫРАЗИТЬ(СведенияОПользователях.Пользователь КАК Справочник.Пользователи)
	|	ИЗ
	|		РегистрСведений.СведенияОПользователях КАК СведенияОПользователях
	|	ГДЕ
	|		ТИПЗНАЧЕНИЯ(СведенияОПользователях.Пользователь) = ТИП(Справочник.Пользователи)
	|		И СведенияОПользователях.Пользователь <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|		И СведенияОПользователях.Пользователь В(&Пользователь)) КАК Ссылки";
	
КонецФункции

// Для функции ЗапросСвойств.
Функция ТекстЗапросаСвойств()
	...
	|ИЗ
	|	СсылкиПользователей КАК Ссылки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
	|		ПО (Пользователи.Ссылка = Ссылки.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОПользователях КАК СведенияОПользователях
	|		ПО (СведенияОПользователях.Пользователь = Ссылки.Ссылка)
	|ГДЕ
	|	НЕ(Пользователи.Ссылка ЕСТЬ NULL
	|				И СведенияОПользователях.Пользователь ЕСТЬ NULL)";
	
КонецФункции

// Для функции ЗапросСвойств.
Функция ТекстЗапросаСвойствВсеПоляРегистра()
	...
	Для Каждого ПользовательИБ Из ВсеПользователиИБ Цикл
		...
		Попытка
			...
			Если Строка <> Неопределено Тогда
				// @skip-check query-in-loop - Порционная обработка данных в транзакции
				ОбновитьСведенияОПользователе(Строка.Ссылка, Неопределено);
			КонецЕсли;
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьСведенияОПользователяхИОтключитьАутентификацию");
