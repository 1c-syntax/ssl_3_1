...
Процедура УстановитьСсылкуНаОригинал() 
	...
	Если ЗначениеЗаполнено(НомерНаПечать) И ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда
		СсылкаНаДокумент = СтрШаблон(НСтр("ru='%1 № %2 от %3'"), ТипДокумента, НомерНаПечать,
	...
	ИначеЕсли ЗначениеЗаполнено(НомерНаПечать) И Не ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда 
		СсылкаНаДокумент = СтрШаблон(НСтр("ru='%1 № %2'"), ТипДокумента, НомерНаПечать);
	Иначе 
		СсылкаНаДокумент = СтрШаблон(НСтр("ru='%1 от %2'"), ТипДокумента, Формат(СведенияОДокументе.Дата, НСтр("ru='ДЛФ=DD'")));
	КонецЕсли;
