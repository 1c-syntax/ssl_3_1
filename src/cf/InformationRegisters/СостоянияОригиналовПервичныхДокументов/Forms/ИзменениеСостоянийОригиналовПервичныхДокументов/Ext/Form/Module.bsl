...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкиПодсистемы = УчетОригиналовПервичныхДокументов.НастройкиПодсистемы();
	Если Не НастройкиПодсистемы.ОтображатьПодсказкуВФормеИзмененияСостояний Тогда
		Элементы.ГруппаДекорацияПредупреждение.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли (ИмяСобытия = "Запись_РегистрСведенийСостоянияОригиналовПервичныхДокументов"
		И ((Источник <> Неопределено И ТипЗнч(Источник) = Тип("Массив") И Источник.Найти(Запись.Владелец) <> Неопределено)
		Или Источник = Запись.Владелец))
...
Процедура КомплектПечатныхФормВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ОригиналПолученКартинка" Тогда
		УстановитьСостояниеОригинала("УстановкаСостоянияОригиналПолучен");
	КонецЕсли;
...
Процедура ОК(Команда)
	...
	Если КомплектПечатныхФорм.Количество()<> 0  Тогда
		...
		Оповестить("Запись_РегистрСведенийСостоянияОригиналовПервичныхДокументов",, Запись.Владелец);
		Закрыть();
...
&НаКлиенте
Процедура УстановитьСостояниеОригинала(ИмяКоманды)
	...
	Если ИмяКоманды = "УстановкаСостоянияОригиналПолучен" Тогда
		СостояниеОригинала = ОригиналПолучен;
	Иначе
	...
	Для Каждого СтрокаСписка Из Элементы.КомплектПечатныхФорм.ВыделенныеСтроки Цикл
		...
		ДанныеСтроки.Состояние = СостояниеОригинала;
		ДанныеСтроки.ОригиналПолученКартинка = ?(ДанныеСтроки.Состояние = ОригиналПолучен, 1, 0);
		ДанныеСтроки.АвторИзменения = ПользователиКлиент.ТекущийПользователь();
		ДанныеСтроки.ДатаПоследнегоИзменения = НСтр("ru = '<только что>'");
	КонецЦикла;
...
Процедура ЗаполнитьПервоначальныйСписокПечатныхФорм()
	...
	ОбъектМетаданных = Запись.Владелец.Метаданные();
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		ТаблицаКоманд = МодульДополнительныеОтчетыИОбработки.ДополнительныеКомандыПечати(ПолноеИмяОМД, Истина);
	КонецЕсли;
	...
	Если ТабличнаяЧасть <> "" Тогда
		...
		Для Каждого Сотрудник Из МассивСотрудников Цикл
			...
			ФИО = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(НаименованиеСотрудника);
			Для Каждого ТекСтрока Из СписокПечатныхФорм Цикл
				...
				НоваяСтрока.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 %2'"), ТекСтрока.Представление, ФИО);
	...
	НенужныеСтроки = КомплектПечатныхФорм.НайтиСтроки(
		Новый Структура("Представление", НСтр("ru='Комплект документов на принтер'")));
	Для Каждого НеНужнаяСтрока Из НенужныеСтроки Цикл
		КомплектПечатныхФорм.Удалить(НеНужнаяСтрока);
	КонецЦикла;

	НенужныеСтроки = КомплектПечатныхФорм.НайтиСтроки(
		Новый Структура("Представление", НСтр("ru='Комплект документов с настройкой...'")));
	Для Каждого НеНужнаяСтрока Из НенужныеСтроки Цикл
		КомплектПечатныхФорм.Удалить(НеНужнаяСтрока);
	КонецЦикла;
...
Процедура ОчиститьНеОтслеживаемые()
	...
	Для Каждого ПечатнаяФорма Из КомплектПечатныхФорм.НайтиСтроки(Отбор) Цикл 
...
Процедура УдалитьДубликатыФорм()
	...
	Для Каждого ПечатнаяФорма Из КомплектПечатныхФорм Цикл
		ТабличнаяЧасть = УчетОригиналовПервичныхДокументов.ТабличнаяЧастьССотрудниками(Запись.Владелец); 
		Если ТабличнаяЧасть <> "" Тогда
			Отбор = Новый Структура("Представление", ПечатнаяФорма.Представление);
		Иначе
			Отбор = Новый Структура("ИмяМакета", ПечатнаяФорма.ИмяМакета);
		КонецЕсли;
...
Процедура УстановитьСсылкуНаОригинал() 
	...
	ТипДокумента = ОбщегоНазначения.ПредставлениеОбъекта(Запись.Владелец.Метаданные());
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрефиксацияОбъектов") Тогда
		НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СведенияОДокументе.Номер, Истина, Истина);
	Иначе
		СсылкаНаДокумент = Строка(Запись.Владелец);
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НомерНаПечать) И Не ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда
		 СсылкаНаДокумент = ТипДокумента; 
		 Возврат;
	КонецЕсли;
	 
	Если ЗначениеЗаполнено(НомерНаПечать) И ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда
		СсылкаНаДокумент = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 № %2 от %3'"),
			ТипДокумента, НомерНаПечать, Формат(СведенияОДокументе.Дата, НСтр("ru='ДЛФ=DD'")));
	ИначеЕсли ЗначениеЗаполнено(НомерНаПечать) И Не ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда 
		СсылкаНаДокумент = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 № %2'"), ТипДокумента,
			НомерНаПечать);
	Иначе 
		СсылкаНаДокумент = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2'"),
			ТипДокумента, Формат(СведенияОДокументе.Дата, НСтр("ru='ДЛФ=DD'")));
	КонецЕсли;
...
Процедура УстановитьОбщееСостояние()
	...
	Для Каждого ПечатнаяФорма Из КомплектПечатныхФорм Цикл
		Если ЗначениеЗаполнено(ПечатнаяФорма.Состояние) Тогда
			Отбор = Новый Структура("Состояние",ПечатнаяФорма.Состояние);
			...
			Если НайденныеСтроки.Количество() = КомплектПечатныхФорм.Количество() Или КоличествоЗаписываемых = 1  Тогда
				Запись.Состояние = ПечатнаяФорма.Состояние;
			Иначе
...
Процедура ЗаписатьСостоянияОригиналовПервичныхДокументов()
	...
	ЭлементБлокировки.УстановитьЗначение("Владелец", Запись.Владелец); 
	...
	Попытка
		...
		НаборЗаписей.Отбор.Владелец.Установить(Запись.Владелец);
		
		ИмяТабличнойЧасти = УчетОригиналовПервичныхДокументов.ТабличнаяЧастьССотрудниками(Запись.Владелец);

		Для Каждого ПечатнаяФорма Из КомплектПечатныхФорм Цикл
			
			Если ЗначениеЗаполнено(ПечатнаяФорма.Состояние) Тогда 
				Если ИмяТабличнойЧасти <> "" Тогда
					НаборЗаписей.Отбор.Сотрудник.Установить(ПечатнаяФорма.Сотрудник);
				КонецЕсли;
				НаборЗаписей.Отбор.ПервичныйДокумент.Установить(ПечатнаяФорма.ИмяМакета);
				НаборЗаписей.Прочитать();
				Если НаборЗаписей.Количество() > 0 Тогда
					Если НаборЗаписей[0].Состояние <> ПечатнаяФорма.Состояние Тогда
						УчетОригиналовПервичныхДокументов.ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(Запись.Владелец,
		...
		УчетОригиналовПервичныхДокументов.ЗаписатьОбщееСостояниеОригиналаДокумента(Запись.Владелец, 
			Запись.Состояние);
