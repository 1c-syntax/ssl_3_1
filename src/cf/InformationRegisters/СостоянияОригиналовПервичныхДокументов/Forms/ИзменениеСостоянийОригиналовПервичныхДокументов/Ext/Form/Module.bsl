...
Процедура ЗаполнитьПервоначальныйСписокПечатныхФорм()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		КоллекцияПечатныхФорм = УправлениеПечатью.КомандыПечатиОбъекта(ОбъектМетаданных);
		...
		Если ТЧ <> "" Тогда
			Для Каждого Сотрудник Из Запись.Владелец[ТЧ] Цикл
			...
			Если ТаблицаКоманд.Количество() > 0 Тогда
				...
				МодульФизическиеЛицаКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ФизическиеЛицаКлиентСервер");
					...
					Иначе
						ФИО = Сотрудник.Сотрудник.Наименование;
					КонецЕсли;
					ФИО = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(Сотрудник.Сотрудник.Наименование);
