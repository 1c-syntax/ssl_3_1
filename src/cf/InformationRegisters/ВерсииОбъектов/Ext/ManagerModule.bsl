...
// 
Процедура ПрогрессУдалениеПредупрежденийСинхронизации(Знач ТекущийШаг, Максимум)
...
Процедура ОчиститьПредупрежденияВерсий(ПараметрыУдаления, ИмяСобытия) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыУдаления.ОтборПоДатеВозникновения) Тогда
		...
		Запрос.УстановитьПараметр("ДатаНачала", ПараметрыУдаления.ОтборПоДатеВозникновения.ДатаНачала);
		Запрос.УстановитьПараметр("ДатаОкончания", ПараметрыУдаления.ОтборПоДатеВозникновения.ДатаОкончания);
	...
	Пока ВыборкаОбъекты.Следующий() Цикл
		...
		Попытка
			...
			ПрогрессУдалениеПредупрежденийСинхронизации(ПараметрыУдаления.КоличествоОперацийТекущийШаг, ПараметрыУдаления.КоличествоОперацийМаксимум);
