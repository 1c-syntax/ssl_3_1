...
Процедура СформироватьОтчетПоИзменениям(ПараметрыОтчета, АдресРезультата) Экспорт
	...
	Для ИндексВерсии = 2 По МассивНомеровВерсий.Количество() Цикл
		...
		Для Каждого ЭлементСоответствия Из ВерсияОбъекта.ТабличныеЧасти Цикл
			...
			СсылкаНаВерсиюТаблицы.Колонки.Добавить(ИмяКолонкиИдСтроки());
			Для Каждого СтрокаТаблицы Из СсылкаНаВерсиюТаблицы Цикл
				СтрокаТаблицы[ИмяКолонкиИдСтроки()] = СтрокаТаблицы.НомерСтроки;
			КонецЦикла;
			
			СсылкаНаВерсиюТаблицы.Колонки.Добавить(ИмяКолонкиМодификация());
			СсылкаНаВерсиюТаблицы.ЗаполнитьЗначения(Ложь, ИмяКолонкиМодификация());
			
			СсылкаНаВерсиюТаблицы.Колонки.Добавить(ИмяКолонкиИзменения(), Новый ОписаниеТипов("Массив"));
			...
			Если ТаблицаСИзменениями <> Неопределено Тогда
				...
				Для Каждого ЭлементТЧ Из ИзмененныеСтроки Цикл
					...
					СсылкаНаВерсиюТаблицы[ЭлементТЧ.ИндексВТЧ1-1][ИмяКолонкиИдСтроки()] = СтрокаТИЗ[ИмяКолонкиИдСтроки()];
					СсылкаНаВерсиюТаблицы[ЭлементТЧ.ИндексВТЧ1-1][ИмяКолонкиМодификация()] = "И";
					СсылкаНаВерсиюТаблицы[ЭлементТЧ.ИндексВТЧ1-1][ИмяКолонкиИзменения()] = ЭлементТЧ.Различия;
				КонецЦикла;
				
				Для Каждого ЭлементТЧ Из ДобавленныеСтроки Цикл
					СсылкаНаВерсиюТаблицы[ЭлементТЧ.ИндексВТЧ1-1][ИмяКолонкиИдСтроки()] = УвеличитьСчетчик(счетчикУникальныйИд, ИмяТаблицы);
					СсылкаНаВерсиюТаблицы[ЭлементТЧ.ИндексВТЧ1-1][ИмяКолонкиМодификация()] = "Д";
				КонецЦикла;
				...
				Для Индекс = 1 По СсылкаНаВерсиюТаблицы.Количество() Цикл
					Если СсылкаНаВерсиюТаблицы[Индекс-1][ИмяКолонкиИдСтроки()] = Неопределено Тогда
						...
						Для Каждого ИмяКолонки Из ОбщиеКолонки Цикл
							Если (ИмяКолонки <> ИмяКолонкиИдСтроки()) И (ИмяКолонки <> ИмяКолонкиМодификация()) Тогда
						...
						ПараметрыОтбора.Вставить(ИмяКолонкиМодификация(), Неопределено);
						...
						Для ИндПоТЗ_Текущей = 1 По МассивСтрокТекущейТЧ.Количество() Цикл
							Если ИндПоТЗ_Текущей <= МассивСтрокПредыдущейТЧ.Количество() Тогда
								МассивСтрокТекущейТЧ[ИндПоТЗ_Текущей-1][ИмяКолонкиИдСтроки()] = МассивСтрокПредыдущейТЧ[ИндПоТЗ_Текущей-1][ИмяКолонкиИдСтроки()];
							КонецЕсли;
							МассивСтрокТекущейТЧ[ИндПоТЗ_Текущей-1][ИмяКолонкиМодификация()] = Ложь;
						КонецЦикла;
					КонецЕсли;
				КонецЦикла;
				Для Каждого ЭлементТЧ Из УдаленныеСтроки Цикл
					СтрокаМнимая = СсылкаНаВерсиюТаблицы.Добавить();
					СтрокаМнимая[ИмяКолонкиИдСтроки()] = ТаблицаИзмененийТабличныхЧастей[ИмяТаблицы][НомерПредыдущейВерсии][ЭлементТЧ.ИндексВТЧ0-1][ИмяКолонкиИдСтроки()];
					СтрокаМнимая[ИмяКолонкиМодификация()] = "У";
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
Процедура ВывестиИзмененияРеквизитов(ТЧОтчета, ТаблицаИзмененийРеквизитов, МассивНомеровВерсий, ОбщийШаблон, СсылкаНаОбъект)
	...
	Для Каждого ЭлементМодРеквизит Из ТаблицаИзмененийРеквизитов Цикл
		Если ЭлементМодРеквизит[ИмяКолонкиМодификация()] = Истина Тогда
	...
	Для Каждого ЭлементИзмененнаяТЧ Из ТаблицаИзмененийТабличныхЧастей Цикл
		...
		Для ТекСчетчикУникальныйИд = 1 По СчетчикУникальныйИд[ИмяТабличнойЧасти] Цикл
			...
			Пока ИндексПоВерсиям >= 1 Цикл
				...
				Если ТекущаяТЧВерсии.Колонки.Найти(ИмяКолонкиИдСтроки()) <> Неопределено Тогда
					НайденнаяСтрока = ТекущаяТЧВерсии.Найти(ТекСчетчикУникальныйИд, ИмяКолонкиИдСтроки());
				КонецЕсли;
				
				Если НайденнаяСтрока <> Неопределено Тогда
					Если (НайденнаяСтрока[ИмяКолонкиМодификация()] <> Неопределено) Тогда
						Если (ТипЗнч(НайденнаяСтрока[ИмяКолонкиМодификация()]) = Тип("Строка")
							ИЛИ (ТипЗнч(НайденнаяСтрока[ИмяКолонкиМодификация()]) = Тип("Булево")
							      И НайденнаяСтрока[ИмяКолонкиМодификация()] = Истина)) Тогда
			...
			Пока ИндексПоВерсиям >= 1 Цикл
				...
				Если НайденнаяСтрока <> Неопределено Тогда
					...
					Модификация = НайденнаяСтрока[ИмяКолонкиМодификация()];
					...
					Для Каждого Колонка Из ТекущаяТЧВерсии.Колонки Цикл
						...
						ИменаКолонокСИзменениями = НайденнаяСтрока[ИмяКолонкиИзменения()]; // Массив
						МассивЗаполнения.Добавить(ИменаКолонокСИзменениями.Найти(Колонка.Имя) <> Неопределено, Представление);
					КонецЦикла;
...
Процедура ПодготовитьКолонкиТаблицИзмененийРеквизитов(ТаблицаЗначений,
	...
	ТаблицаЗначений.Колонки.Добавить(ИмяКолонкиМодификация());
	ТаблицаЗначений.Колонки.Добавить(ИмяКолонкиТипЗначения()); // Предполагаемый тип значения.
...
Функция СчитатьНачальныеЗначенияРеквизитовИТабличныхЧастей(ТаблицаРеквизитов, ТаблицаТЧ, КоличествоВерсий, МассивНомеровВерсий, СсылкаНаОбъект)
	...
	Для Каждого СтрокаТаблицыЗначений Из Реквизиты Цикл
		...
		НоваяСтрока[ИмяКолонкиМодификация()] = Ложь;
		НоваяСтрока[ИмяКолонкиТипЗначения()] = СтрокаТаблицыЗначений.ТипРеквизита;
	...
	Для Каждого ЭлементТЧ Из ТабличныеЧасти Цикл
		...
		ТекущаяТЗ.Колонки.Добавить(ИмяКолонкиИдСтроки());
		ТекущаяТЗ.Колонки.Добавить(ИмяКолонкиМодификация());
		ТекущаяТЗ.Колонки.Добавить(ИмяКолонкиИзменения(), Новый ОписаниеТипов("Массив"));
		
		Для Индекс = 1 По ТекущаяТЗ.Количество() Цикл
			ТекущаяТЗ[Индекс-1][ПрефиксСлужебнойКолонки() + "ИдСтроки"] = Индекс;
			ТекущаяТЗ[Индекс-1][ПрефиксСлужебнойКолонки() + "Модификация"] = Ложь;
		КонецЦикла;
...
Функция РассчитатьЧислоИзменившихсяРеквизитов(ТаблицаИзмененийРеквизитов, МассивНомеровВерсий)
	...
	Для Каждого ЭлементТЗ Из ТаблицаИзмененийРеквизитов Цикл
		Если ЭлементТЗ[ИмяКолонкиМодификация()] <> Неопределено И ЭлементТЗ[ИмяКолонкиМодификация()] = Истина Тогда
...
Процедура ЗаполнитьХарактеристикуИзмененияРеквизита(ТаблицаИзмененийРеквизита, 
	...
	Для Каждого Элемент Из ТаблицаИзмененийРеквизита Цикл
		...
		ИзменениеРеквизита[ИмяКолонкиМодификация()] = Истина;
	КонецЦикла;
...
Функция НайтиПохожиеСтрокиТаблиц(Знач Таблица1, Знач Таблица2, Знач ТребуемоеКоличествоРазличий = 0, Знач МаксимумРазличий = Неопределено, СоответствиеСтрокТаблицы1СтрокамТаблицы2 = Неопределено)
	
	Пропустить = "Пропустить_";
	
	Если Таблица1.Колонки.Найти(Пропустить) = Неопределено Тогда
		Таблица1 = Таблица1.Скопировать();
		Таблица1.Колонки.Добавить(Пропустить, Новый ОписаниеТипов("Булево"));
		Таблица1.Индексы.Добавить(Пропустить);
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы1 Из Таблица1.НайтиСтроки(Новый Структура(Пропустить, Ложь)) Цикл
		Для Каждого СтрокаТаблицы2 Из Таблица2.НайтиСтроки(Новый Структура(Пропустить, Ложь)) Цикл
			КоличествоРазличий = КоличествоРазличийВСтрокахТаблиц(СтрокаТаблицы1, СтрокаТаблицы2, ОбщиеКолонки, ОстальныеКолонки);
			...
			Если КоличествоРазличий = ТребуемоеКоличествоРазличий Тогда
				...
				СтрокаТаблицы1[Пропустить] = Истина;
...
Процедура УдалитьПредыдущиеПредупрежденияОбменов(СведенияОВерсииОбъекта) Экспорт
	
	// Определены следующие стратегии:
	// 1. Если тип предупреждения относиться к синхронизации, то чистим по типу;
	// 2. Если есть комментарий синхронизации, то чистим все аналогичные комментарии.
	//
	// При этом в двух стратегиях удаляем записи, где условия совпадают, а автором является пользователь,
	// потому что уже нет возможности идентифицировать обмен, к которому относиться предупреждение.
	
	ДопустимыеТипыПредупреждений = Новый Массив;
	ДопустимыеТипыПредупреждений.Добавить(Перечисления.ТипыВерсийОбъекта.ПринятыеДанныеПоКоллизии);
	ДопустимыеТипыПредупреждений.Добавить(Перечисления.ТипыВерсийОбъекта.НепринятыеДанныеПоКоллизии);
	ДопустимыеТипыПредупреждений.Добавить(Перечисления.ТипыВерсийОбъекта.НепринятыйПоДатеЗапретаОбъектСуществуетВБазе);
	ДопустимыеТипыПредупреждений.Добавить(Перечисления.ТипыВерсийОбъекта.НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе);
	
	Попытка
		
		Если ДопустимыеТипыПредупреждений.Найти(СведенияОВерсииОбъекта.ТипВерсииОбъекта) <> Неопределено Тогда
			
			УдалитьПредыдущиеПредупрежденияОбменовПоТипу(СведенияОВерсииОбъекта, ДопустимыеТипыПредупреждений);
			
		КонецЕсли;
		
		Если НЕ ПустаяСтрока(СведенияОВерсииОбъекта.ПредупреждениеСинхронизации) Тогда
			
			УдалитьПредыдущиеПредупрежденияОбменовПоКомментарию(СведенияОВерсииОбъекта);
...
Процедура УдалитьПредыдущиеПредупрежденияОбменовПоТипу(СведенияОВерсииОбъекта, ДопустимыеТипыПредупреждений)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Объект", СведенияОВерсииОбъекта.Объект);
	Запрос.УстановитьПараметр("ТипВерсииОбъекта", СведенияОВерсииОбъекта.ТипВерсииОбъекта);
	
	Если ЗначениеЗаполнено(СведенияОВерсииОбъекта.АвторВерсии)
		И ТипЗнч(СведенияОВерсииОбъекта.АвторВерсии) <> Тип("СправочникСсылка.Пользователи") Тогда
		
		Запрос.УстановитьПараметр("АвторВерсииУзел", СведенияОВерсииОбъекта.АвторВерсии);
		
	Иначе
		
		Запрос.УстановитьПараметр("АвторВерсииУзел", Неопределено);
	...
	Запрос.Текст =
		...
		|	И ВерсииОбъектов.ТипВерсииОбъекта = &ТипВерсииОбъекта
		|	И (ВерсииОбъектов.АвторВерсии = &АвторВерсииУзел
		|			ИЛИ ВерсииОбъектов.АвторВерсии ССЫЛКА Справочник.Пользователи)
		|
		|УПОРЯДОЧИТЬ ПО
...
Процедура УдалитьПредыдущиеПредупрежденияОбменовПоКомментарию(СведенияОВерсииОбъекта)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Объект", СведенияОВерсииОбъекта.Объект);
	Запрос.УстановитьПараметр("ПредупреждениеСинхронизации", СведенияОВерсииОбъекта.ПредупреждениеСинхронизации);
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ВерсииОбъектов.Объект = &Объект
		|	И (ВЫРАЗИТЬ(ВерсииОбъектов.ПредупреждениеСинхронизации КАК СТРОКА(512))) = &ПредупреждениеСинхронизации
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВерсииОбъектов.НомерВерсии";
...
Функция ПрефиксСлужебнойКолонки()
	
	Возврат "Версионирование_";
	
КонецФункции

Функция ИмяКолонкиИдСтроки()

	Возврат ПрефиксСлужебнойКолонки() + "ИдСтроки";

КонецФункции

Функция ИмяКолонкиМодификация()

	Возврат ПрефиксСлужебнойКолонки() + "Модификация";

КонецФункции

Функция ИмяКолонкиИзменения()

	Возврат ПрефиксСлужебнойКолонки() + "Изменения";

КонецФункции

Функция ИмяКолонкиТипЗначения()

	Возврат ПрефиксСлужебнойКолонки() + "ТипЗначения";
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СформироватьОтчетПоИзменениям", Истина);
