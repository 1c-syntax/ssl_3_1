...
Процедура УстановитьПараметрРаботыРасширения(ИмяПараметра, Значение, БезУчетаВерсииРасширений = Ложь) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"РегистрыСведений.ПараметрыРаботыВерсийРасширений.УстановитьПараметрРаботыРасширения");
	...
	НоваяЗапись.ХранилищеПараметра = Новый ХранилищеЗначения(Содержимое, Новый СжатиеДанных(9));
	
	НаборЗаписей.Записать();
	
КонецПроцедуры

// См. СтандартныеПодсистемыСервер.УдалитьПараметрРаботыРасширения.
Процедура УдалитьПараметрРаботыРасширения(ИмяПараметра, БезУчетаВерсииРасширений = Ложь, ЕстьИзменения = Ложь) Экспорт
	
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"РегистрыСведений.ПараметрыРаботыВерсийРасширений.УдалитьПараметрРаботыРасширения");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИмяПараметра", ИмяПараметра);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПараметрыРаботыВерсийРасширений.ХранилищеПараметра КАК ХранилищеПараметра
	|ИЗ
	|	РегистрСведений.ПараметрыРаботыВерсийРасширений КАК ПараметрыРаботыВерсийРасширений
	|ГДЕ
	|	ПараметрыРаботыВерсийРасширений.ИмяПараметра = &ИмяПараметра
	|	И &ОтборВерсии";
	
	Если БезУчетаВерсииРасширений <> Неопределено Тогда
		ВерсияРасширений = ?(БезУчетаВерсииРасширений,
			Справочники.ВерсииРасширений.ПустаяСсылка(), ПараметрыСеанса.ВерсияРасширений);
		Запрос.УстановитьПараметр("ВерсияРасширений", ВерсияРасширений);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборВерсии",
			"ПараметрыРаботыВерсийРасширений.ВерсияРасширений = &ВерсияРасширений");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборВерсии", "ИСТИНА");
	КонецЕсли;
...
//      ** ИерархияГруппПользователей - см. СвойстваПараметраОбновления
//      ** СоставыГруппПользователей - см. СвойстваПараметраОбновления
//      ** СведенияОПользователях - см. СвойстваПараметраОбновления
...
Функция ПараметрыОбновления(Обновлять = Ложь) Экспорт
	...
	ПараметрыПодсистемы = Новый Структура;
	ПараметрыПодсистемы.Вставить("ИерархияГруппПользователей", НовыеСвойстваПараметраОбновления(Обновлять));
	ПараметрыПодсистемы.Вставить("СоставыГруппПользователей", НовыеСвойстваПараметраОбновления(Обновлять));
	ПараметрыПодсистемы.Вставить("СведенияОПользователях", НовыеСвойстваПараметраОбновления(Обновлять));
...
Процедура ВыполнитьОбновлениеРазделенныхДанныхВФоне(Параметры, ИдентификаторФормы) Экспорт
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
		"РегистрыСведений.ПараметрыРаботыВерсийРасширений.ОбработчикДлительнойОперацииВыполнитьОбновлениеРазделенныхДанных",
		Параметры,
		ПараметрыОперации);
...
Процедура ОбработчикДлительнойОперацииВыполнитьОбновлениеРазделенныхДанных(Параметры, АдресРезультата) Экспорт
	...
	// СтандартныеПодсистемы Пользователи
	Если Параметры.Пользователи.ИерархияГруппПользователей.Обновлять Тогда
		РегистрыСведений.ИерархияГруппПользователей.ОбновитьДанныеРегистра(
			Параметры.Пользователи.ИерархияГруппПользователей.ЕстьИзменения);
	КонецЕсли;
	...
	КонецЕсли;
	Если Параметры.Пользователи.СведенияОПользователях.Обновлять Тогда
		РегистрыСведений.СведенияОПользователях.ОбновитьДанныеРегистра(,
			Параметры.Пользователи.СведенияОПользователях.ЕстьИзменения);
	КонецЕсли;
...
Процедура ВключитьЗаполнениеПараметровРаботыРасширений(Запустить = Истина, ВключитьБезусловно = Ложь) Экспорт
	...
	Если Запустить Тогда
		ЗапуститьЗаполнениеПараметровРаботыРасширений(
			НСтр("ru = 'Запуск при включении заполнения параметров работы приложения'"));
	КонецЕсли;
...
Процедура ЗапуститьЗаполнениеПараметровРаботыРасширений(Комментарий, ОжидатьЗавершения = Ложь) Экспорт
	...
	ПараметрыВыполненияЗадания.ЭтоЗапускВручную = Ложь;
	ФоновоеЗадание = РегламентныеЗаданияСервер.ВыполнитьЗадание(
		Метаданные.РегламентныеЗадания.ЗаполнениеПараметровРаботыРасширений,
		ПараметрыВыполненияЗадания);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработчикДлительнойОперацииВыполнитьОбновлениеРазделенныхДанных", Истина);
