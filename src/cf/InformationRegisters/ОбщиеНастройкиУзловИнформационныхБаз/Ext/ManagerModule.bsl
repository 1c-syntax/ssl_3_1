...
// Метод проверяет наличие минимум одного настроенного обмена с объектной моделью загрузки данных.
//
// Возвращаемое значение:
//   Булево - ИСТИНА если есть минимум одна настроенная синхронизация, иначе ЛОЖЬ.
//
Функция ЕстьУзлыИспользующиеПообъектныйОбмен() Экспорт
	...
	|ГДЕ
	|	ОбщиеНастройкиУзловИнформационныхБаз.ОбъектнаяКонвертацияXDTO
	|	И НЕ ОбщиеНастройкиУзловИнформационныхБаз.СинхронизацияНедоступна";
...
//
Функция ИнтервалОбновленияСтратегииПоискаПоКлючам(УзелОбмена) Экспорт
	
	ИнтервалОбновленияСтратегииПоискаПоКлючам = 0;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОбщиеНастройкиУзловИнформационныхБаз.ИнтервалОбновленияСтратегииПоискаПоКлючам КАК ИнтервалОбновленияСтратегииПоискаПоКлючам
	|ИЗ
	|	РегистрСведений.ОбщиеНастройкиУзловИнформационныхБаз КАК ОбщиеНастройкиУзловИнформационныхБаз
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		ИнтервалОбновленияСтратегииПоискаПоКлючам = Выборка.ИнтервалОбновленияСтратегииПоискаПоКлючам;
		
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ИнтервалОбновленияСтратегииПоискаПоКлючам) Тогда
		
		ИнтервалОбновленияСтратегииПоискаПоКлючам = 
			КонвертацияОбъектноСервер.СтандартныйИнтервалОбновленияСтратегииПоискаПоКлючам();
		
	КонецЕсли;
	
	Возврат ИнтервалОбновленияСтратегииПоискаПоКлючам;
...
//
// Возвращаемое значение:
//   Булево - значение настройки "ОбъектнаяКонвертацияXDTO", которое сохранено в регистре ОбщиеНастройкиУзловИнформационныхБаз.
//
Функция СпособОбъектнойКонвертацииXDTO(УзелОбмена) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОбщиеНастройкиУзловИнформационныхБаз.ОбъектнаяКонвертацияXDTO КАК ОбъектнаяКонвертацияXDTO
	|ИЗ
	|	РегистрСведений.ОбщиеНастройкиУзловИнформационныхБаз КАК ОбщиеНастройкиУзловИнформационныхБаз
	|ГДЕ
	|	ОбщиеНастройкиУзловИнформационныхБаз.УзелИнформационнойБазы = &УзелОбмена";
	...
	Если Выборка.Следующий() Тогда
		...
		Результат = Выборка.ОбъектнаяКонвертацияXDTO;
...
Процедура ИзменитьСпособОбъектнойКонвертацииXDTO(УзелОбмена, НовоеЗначение) Экспорт
	
	СтруктураЗаписи = Новый Структура;
	СтруктураЗаписи.Вставить("УзелИнформационнойБазы", УзелОбмена);
	СтруктураЗаписи.Вставить("ОбъектнаяКонвертацияXDTO", НовоеЗначение);
...
Процедура ОбновитьОбщиеНастройкиКорреспондента(УзелИнформационнойБазы)
