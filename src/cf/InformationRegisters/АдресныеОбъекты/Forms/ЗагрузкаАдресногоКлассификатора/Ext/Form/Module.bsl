...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьВидимостьКнопокПослеСменыСтраницы(ЭтотОбъект);
	
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Элементы.ГруппаПодвалСубъектов.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		Элементы.Переместить(Элементы.ГруппаВыбраноРегионовДляЗагрузки, Элементы.ГруппаПодвалСубъектов, Элементы.ГруппаСтрокаПодвала);
		Элементы.СубъектыРФ.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ВСтрокахТаблицы;
		Элементы.СубъектыРФ.ВысотаВСтрокахТаблицы = 6;
	КонецЕсли;
...
//
&НаКлиенте
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		Даты = ДатыОбновленияАдресногоКлассификатор(Результат.АдресРезультата);
...
Процедура ЗакрытиеФормыЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПодтверждениеЗакрытияФормы = Истина;
		ЗавершитьФоновыеЗадания(ИдентификаторЗадания);
		Закрыть();
	Иначе 
		ПодтверждениеЗакрытияФормы = Неопределено;
	КонецЕсли;
...
Процедура ЗавершитьФоновыеЗадания(ИдентификаторЗадания)
	
	Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
		ДлительныеОперации.ОтменитьВыполнениеЗадания(ИдентификаторЗадания);
	КонецЕсли;
	
	ОчиститьКаталогВременногоХранилищаФайлов();
...
#Область ЗагрузкаАдресногоКлассификатор
...
Процедура ЗагрузитьКлассификаторИзКаталога(Знач КаталогДанных)
	...
	ПараметрыЗагрузки.Вставить("КодИсточникаЗагрузки",  КодИсточникаЗагрузки); 
	ПараметрыЗагрузки.Вставить("ПолеОшибки", "АдресЗагрузки");
	ПараметрыЗагрузки.Вставить("ЗагружатьПомещения", ЗагружатьПомещения);
...
Процедура ЗагрузитьКлассификаторИзКаталогаЗавершение(РезультатАнализа, ДополнительныеПараметры) Экспорт
	...
	Если РезультатАнализа.Ошибки <> Неопределено Тогда
		...
		ОчиститьСообщения();
		Если РезультатАнализа.Ошибки.СписокОшибок.Количество() = 1 Тогда
			ПоказатьПредупреждение(, РезультатАнализа.Ошибки.СписокОшибок[0].ТекстДляОднойОшибки);
		Иначе
...
//
&НаКлиенте
Процедура ПослеФоновойЗагрузкиССайта(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Статус = "Ошибка" Тогда
		Если СтрНайти(Результат.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Результат.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения. Возможные причины:
				...
				| • Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки приложения подключиться к Интернету;
				| • Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в приложении.
				|
				|Техническая информация:'") + Символы.ПС + СтрПолучитьСтроку(Результат.КраткоеПредставлениеОшибки, 1);
		Иначе
		...
		Если ТипЗнч(Результаты) = Тип("Соответствие") Тогда
			Для Каждого РезультатПорции Из Результаты Цикл
				Если РезультатПорции.Значение.Статус = "Ошибка" Тогда
					ВывестиСообщениеОбОшибке(РезультатПорции.Значение);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе // выход из цикла
		...
		ОповещениеОПрогрессеВыполнения = Новый ОписаниеОповещения("ПрогрессВыполнения", ЭтотОбъект);
		Результат = ЗапуститьФоновуюЗагрузкуНаСервере(ДополнительныеПараметры.ПараметрыЗагрузки.КодыРегионов, ПомещенныеФайлы,
			ЗагружатьИсториюИзмененийАдресныхОбъектов, УникальныйИдентификатор, ЗагружатьПомещения);
...
//
&НаКлиенте
Процедура ПослеФоновойЗагрузкиИзКаталога(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Статус = "Ошибка" Тогда
		...
		Если ПустаяСтрока(ТекстОшибки) Тогда
			ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения из папки.
				|Сохраните файлы с сайта ""1С"" (http://its.1c.ru/download/fias2) на компьютер, а затем загрузить в приложение.
				|
				|Техническая информация:'") + Символы.ПС + Результат.КраткоеПредставлениеОшибки;
		КонецЕсли;
...
&НаКлиенте
Процедура ВывестиСообщениеОбОшибке(Результат)
...
&НаСервереБезКонтекста
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, ЗагружатьИсторию, УникальныйИдентификатор, ЗагружатьПомещения)
	...
	ПараметрыЗагрузки.ЗагружатьПомещения      = ЗагружатьПомещения;
...
&НаСервереБезКонтекста
Процедура ОчиститьКаталогВременногоХранилищаФайлов()
	
	Попытка
		
		КаталогВременныФайлов = АдресныйКлассификаторСлужебный.КаталогВременногоХранилищаФайлов();
...
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, 
		Знач ЗагружатьИсториюИзмененийАдресныхОбъектов, Знач УникальныйИдентификатор, Знач ЗагружатьПомещения)
	...
	РезультатИзвлечения = АдресныйКлассификаторСлужебный.ИзвлечьФайлыВКаталог(ОписаниеФайловЗагрузки, КаталогФайлов, ЗагружатьПомещения);
	Если РезультатИзвлечения.ПроизошлаОшибка Тогда
		Результат.ТекстОшибки     = РезультатИзвлечения.ТекстОшибки;
		Результат.ПроизошлаОшибка = РезультатИзвлечения.ПроизошлаОшибка;
		Возврат Результат;
	КонецЕсли;
...
Процедура УстановитьВидимостьКнопокПослеСменыСтраницы(Форма)
	
	ТекущаяСтраница = Форма.Элементы.ШагиЗагрузки.ТекущаяСтраница;
	
	Форма.Элементы.Загрузить.Видимость = Ложь;
	Форма.Элементы.ПрерватьЗагрузку.Видимость = Ложь;
	Форма.Элементы.ОбновитьКлассификатор.Видимость = Ложь;
	
	Если ТекущаяСтраница = Форма.Элементы.ВыборРегионовЗагрузки Тогда
		Форма.Элементы.Загрузить.Видимость = Истина;
	ИначеЕсли ТекущаяСтраница = Форма.Элементы.ОжиданиеЗагрузки Тогда
		Форма.Элементы.ПрерватьЗагрузку.Видимость = Истина;
	ИначеЕсли ТекущаяСтраница = Форма.Элементы.ОбновлениеКлассификатора Тогда
		Форма.Элементы.ОбновитьКлассификатор.Видимость = Истина;
	КонецЕсли;
