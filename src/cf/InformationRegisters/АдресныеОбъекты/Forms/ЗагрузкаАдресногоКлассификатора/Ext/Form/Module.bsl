...
Процедура Загрузить(Команда)
	...
	Если КодИсточникаЗагрузки = "КАТАЛОГ" Тогда
		Текст = НСтр("ru = 'Для загрузки адресного классификатора из папки необходимо установить расширение для работы с 1С:Предприятием.'");
...
Процедура ЗагрузитьКлассификаторИзКаталога(Знач КаталогДанных)
	...
	ЗагружаемыйРегион = КодыРегионов.Получить(0);
	КодыЗагружаемыхРегионов = Новый СписокЗначений;
	КодыЗагружаемыхРегионов.Добавить(ЗагружаемыйРегион.Значение, ЗагружаемыйРегион.Представление);
	...
	АдресныйКлассификаторКлиент.АнализДоступностиФайловКлассификатораВКаталоге(ОписаниеОповещения, КодыЗагружаемыхРегионов, КаталогДанных, ПараметрыЗагрузки);
...
Процедура ПослеФоновойЗагрузкиИзКаталога(Задание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Задание.Статус = "Выполнено" Тогда
		...
		КодыРегионов = ДополнительныеПараметры.ПараметрыЗагрузки.КодыРегионов;
		
		Для Каждого КодРегиона Из КодыРегионов Цикл
		
			НайденныеСтроки = СубъектыРФ.НайтиСтроки(ФильтрОтбораПоКодуРегиона(КодРегиона.Значение));
			Если НайденныеСтроки.Количество() > 0 Тогда
				НайденныеСтроки[0].Загружать = Ложь;
				НайденныеСтроки[0].Загружено = Истина;
			КонецЕсли;
			
		КонецЦикла;
		
		ЗагрузитьКлассификаторИзКаталога(АдресЗагрузки);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция ФильтрОтбораПоКодуРегиона(КодРегиона)
	
	Отбор = Новый Структура;
	Отбор.Вставить("КодСубъектаРФ", КодРегиона);
	Возврат Отбор;
