...
Процедура ОчиститьКлассификатор()
	...
	ДлительнаяОперация = ЗапуститьФоновуюОчисткуНаСервере(КодыРегионовДляОчистки(), УникальныйИдентификатор);
...
//
&НаКлиенте
Процедура ПослеФоновойОчистки(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПрогрессВыполнения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполняется" И Результат.Прогресс <> Неопределено Тогда
		ТекстСостоянияОчистки = Результат.Прогресс.Текст;
	КонецЕсли;
...
Функция ЗапуститьФоновуюОчисткуНаСервере(Знач КодыРегионов, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.УточнениеОшибки = НСтр("ru = 'Не удалось очистить адресный классификатор по причине:'");
