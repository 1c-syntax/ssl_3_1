...
#Если МобильныйКлиент Тогда
	Выполнить("ДлительнаяОперация = ОбновитьСнимкиОтчетовНаСервере(ИдентификаторыСтрок)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов функции модуля конфигурации.
	Выполнить("ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов функции модуля конфигурации.
	Выполнить("ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОписаниеОповещения, ПараметрыОжидания)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов процедуры модуля конфигурации.
#Иначе
...
#Если МобильныйКлиент Тогда
		Выполнить("СтандартныеПодсистемыКлиент.ВывестиИнформациюОбОшибке(Результат.ИнформацияОбОшибке)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов процедуры модуля конфигурации.
#Иначе
