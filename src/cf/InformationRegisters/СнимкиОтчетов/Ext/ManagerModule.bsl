...
Процедура СохранитьСнимокОтчетаПользователя(РезультатОтчета, НастройкиОтчета) Экспорт
	...
	МенеджерЗаписи.ПользовательскаяНастройка = Новый ХранилищеЗначения(
		НастройкиОтчета.СвойстваРезультата.КомпоновщикНастроек.ПользовательскиеНастройки,
		Новый СжатиеДанных(9));

	МенеджерЗаписи.РезультатОтчета = Новый ХранилищеЗначения(РезультатОтчета, Новый СжатиеДанных(9));
...
Процедура ОбновитьСнимкиОтчетовПользователя(ПараметрыЗаполнения, АдресХранилища) Экспорт
	...
	Если ПараметрыЗаполнения.Свойство("СнимкиОтчетов") Тогда
		...
		|	СнимкиОтчетов.Отчет КАК СсылкаОтчета,
		...
		|			И СнимкиОтчетов.Отчет = СохраненныеСнимкиОтчетов.СсылкаОтчета
	...
	ИмяОтчета = "ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СнимкиОтчетов.Отчет) = ТИП(Справочник.ИдентификаторыОбъектовМетаданных)
	|			ТОГДА ВЫРАЗИТЬ(СнимкиОтчетов.Отчет КАК Справочник.ИдентификаторыОбъектовМетаданных).Имя
	|		КОГДА ТИПЗНАЧЕНИЯ(СнимкиОтчетов.Отчет) = ТИП(Справочник.ИдентификаторыОбъектовРасширений)
	|			ТОГДА ВЫРАЗИТЬ(СнимкиОтчетов.Отчет КАК Справочник.ИдентификаторыОбъектовРасширений).Имя
	|		ИНАЧЕ СнимкиОтчетов.Отчет
	|	КОНЕЦ";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда 
		...
		ИмяОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ВЫБОР
			...
			|		КОГДА ТИПЗНАЧЕНИЯ(СнимкиОтчетов.Отчет) = ТИП(Справочник.ИдентификаторыОбъектовРасширений)
			|			ТОГДА ВЫРАЗИТЬ(СнимкиОтчетов.Отчет КАК Справочник.ИдентификаторыОбъектовРасширений).Имя
			|		КОГДА ТИПЗНАЧЕНИЯ(СнимкиОтчетов.Отчет) = ТИП(%1)
			|			ТОГДА ВЫРАЗИТЬ(СнимкиОтчетов.Отчет КАК %1).ИмяОбъекта
			|		ИНАЧЕ СнимкиОтчетов.Отчет
	...
	Пока Выборка.Следующий() Цикл
		...
		МенеджерЗаписи = РегистрыСведений.СнимкиОтчетов.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Выборка, "Пользователь, Вариант, ХешПользовательскойНастройки");
		МенеджерЗаписи.Отчет = Выборка.СсылкаОтчета;
		...
		ПараметрыФормированияОтчета.СсылкаОтчета = Выборка.СсылкаОтчета;
		...
		Если Не ПустаяСтрока(ИмяСправочникаВариантыОтчетов) Тогда
			КлючВарианта = Выборка.ИмяОтчета;
			ПараметрыФормированияОтчета.КлючВарианта = КлючВарианта;
			ПараметрыФормированияОтчета.СсылкаВарианта = ВариантыОтчетов.ВариантОтчета(Выборка.СсылкаОтчета, КлючВарианта);
		КонецЕсли;
		...
		Если Не Формирование.Успех Тогда
			...
			ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(Выборка.СсылкаОтчета),
			Выборка.Вариант,
			Формирование.ТекстОшибки);
		КонецЕсли;
		
		МенеджерЗаписи.РезультатОтчета = Новый ХранилищеЗначения(Формирование.ТабличныйДокумент, Новый СжатиеДанных(9));
...
Функция СнимкиОтчетовПользователя(Пользователь, ИмяСправочникаВариантыОтчетов) Экспорт
	...
	СнимкиОтчетовПользователя = Запрос.Выполнить().Выгрузить();
	СнимкиОтчетовПользователя.Колонки.СсылкаОтчета.Имя = "Отчет";

	Возврат СнимкиОтчетовПользователя;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьСнимкиОтчетовПользователя", Истина);
