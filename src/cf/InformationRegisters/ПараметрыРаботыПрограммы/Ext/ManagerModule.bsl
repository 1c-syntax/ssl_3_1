...
Процедура ОбновитьПараметрРаботыПрограммы(ИмяПараметра, Значение, ЕстьИзменения = Ложь, СтароеЗначение = Неопределено) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"РегистрыСведений.ПараметрыРаботыПрограммы.ОбновитьПараметрРаботыПрограммы");
...
// См. СтандартныеПодсистемыСервер.УдалитьПараметрРаботыПрограммы.
Процедура УдалитьПараметрРаботыПрограммы(ИмяПараметра, ЕстьИзменения) Экспорт
	
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"РегистрыСведений.ПараметрыРаботыПрограммы.УдалитьПараметрРаботыПрограммы");
	...
	Запрос.УстановитьПараметр("ИмяПараметраХраненияИзменений", ИмяПараметраХраненияИзменений);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	РегистрСведений.ПараметрыРаботыПрограммы КАК ПараметрыРаботыПрограммы
	|ГДЕ
	|	ПараметрыРаботыПрограммы.ИмяПараметра = &ИмяПараметраХраненияИзменений";
	...
	Если Выборка.Следующий() Тогда
		НаборЗаписей = СлужебныйНаборЗаписей(РегистрыСведений.ПараметрыРаботыПрограммы);
		НаборЗаписей.Отбор.ИмяПараметра.Установить(ИмяПараметраХраненияИзменений);
		НаборЗаписей.Записать();
		ЕстьИзменения = Истина;
	КонецЕсли;
...
Функция ИзмененияПараметраРаботыПрограммы(ИмяПараметра) Экспорт
	...
	Если Не ЭтоИзмененияПараметраРаботыПрограммы(ПоследниеИзменения) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для параметра работы приложения ""%1"" не найдены изменения.'"), ИмяПараметра)
...
Процедура ДобавитьИзмененияПараметраРаботыПрограммы(ИмяПараметра, Знач Изменения) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"РегистрыСведений.ПараметрыРаботыПрограммы.ДобавитьИзмененияПараметраРаботыПрограммы");
...
Процедура ЗагрузитьОбновитьПараметрыРаботыПрограммы() Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения)
		...
		Если ТипЗнч(ОбработанныйРезультат.ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
			ВызватьИсключение ОбработкаОшибок.ПодробноеПредставлениеОшибки(ОбработанныйРезультат.ИнформацияОбОшибке);
		КонецЕсли;
	Иначе
		Попытка
...
Функция ЗагрузитьПараметрыРаботыПрограммыВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ru = 'Фоновая загрузка параметров работы приложения'");
	...
	Если ОбщегоНазначения.РежимОтладки()
	 Или СтандартныеПодсистемыСервер.ОтключенаЛогикаНачалаРаботыСистемы() Тогда
...
Функция ОбновитьПараметрыРаботыПрограммыВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ru = 'Фоновое обновление параметров работы приложения'");
	...
	Если ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения)
	   И Не ДоступноВыполнениеФоновыхЗаданий() Тогда
		...
		ТекстОшибки =
			НСтр("ru = 'Обновление параметров работы приложения, когда подключены расширения приложения,
			           ...
			           |
			           |Необходимо запустить обновление в сеансе 1С:Предприятия или временно отключать расширения приложения.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция ОбработанныйРезультатДлительнойОперации(Результат, Операция) Экспорт
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		Если Операция = "ЗагрузкаПараметровРаботыПрограммы" Тогда
			КраткоеПредставлениеОшибки =
				НСтр("ru = 'Не удалось загрузить параметры работы приложения по причине:
				           |Фоновое задание, выполняющее загрузку, отменено.'");
			
		ИначеЕсли Операция = "ОбновлениеПараметровРаботыПрограммы" Тогда
			КраткоеПредставлениеОшибки =
				НСтр("ru = 'Не удалось обновить параметры работы приложения по причине:
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
			ИнформацияОбОшибке = РезультатВыполнения.ИнформацияОбОшибке;
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		ИнформацияОбОшибке = ИнформацияОбОшибкеРезультатаДлительнойОперации(КраткоеПредставлениеОшибки);
	КонецЕсли;
...
Процедура ВыполнитьОбновлениеНеразделенныхДанныхВФоне(Параметры, ИдентификаторФормы) Экспорт
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
		"РегистрыСведений.ПараметрыРаботыПрограммы.ОбработчикДлительнойОперацииВыполнитьОбновлениеНеразделенныхДанных",
		Параметры,
		ПараметрыОперации);
...
Процедура ОбработчикДлительнойОперацииВыполнитьОбновлениеНеразделенныхДанных(Параметры, АдресРезультата) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) Тогда
		ТекстОшибки =
			НСтр("ru = 'Не удалось обновить параметры работы приложения, т.к. подключены расширения.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		Исключение
			...
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При получении параметра работы приложения
				           |%1
				           |возникала ошибка извлечения значения из хранилища:
				           |%2'"),
				ИмяПараметра,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ИмяСобытия = НСтр("ru = 'Параметры работы приложения.Получение параметра'",
				ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УстановитьХранимыеДанныеПараметраРаботыПрограммы(ИмяПараметра, ХранимыеДанные)
	...
	НоваяЗапись.ХранилищеПараметра = Новый ХранилищеЗначения(ХранимыеДанные, Новый СжатиеДанных(9));
...
Процедура ПроверитьВозможностьОбновленияВМоделиСервиса(Знач ИмяПараметра, НовоеЗначение, Знач Операция)
	...
	ИмяСобытия = НСтр("ru = 'Параметры работы приложения.Не выполнено обновление в неразделенном режиме'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
//  
// Возвращаемое значение:
//  - РегистрСведенийНаборЗаписей.ПараметрыРаботыВерсийРасширений
//  - РегистрСведенийНаборЗаписей.ПараметрыРаботыПрограммы
//  - РегистрСведенийНаборЗаписей.ИдентификаторыОбъектовВерсийРасширений
//  
...
// Для вызова из ОбработанныйРезультатДлительнойОперации.
Функция ИнформацияОбОшибкеРезультатаДлительнойОперации(ПредставлениеОшибки)
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработчикДлительнойОперацииЗагрузкиПараметровРаботыПрограммы", Истина);
	Методы.Вставить("ОбработчикДлительнойОперацииОбновленияПараметровРаботыПрограммы", Истина);
	Методы.Вставить("ОбработчикДлительнойОперацииОбновленияПараметровВерсийРасширений", Истина);
	Методы.Вставить("ОбработчикДлительнойОперацииВыполнитьОбновлениеНеразделенныхДанных", Истина);
