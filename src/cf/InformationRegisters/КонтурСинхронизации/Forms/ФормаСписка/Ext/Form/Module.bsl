...
&НаСервере
Процедура ОчиститьКонтурНаСервере()
	
	Набор = РегистрыСведений.КонтурСинхронизации.СоздатьНаборЗаписей();
	Набор.Записать();
...
&НаКлиенте
Процедура ОчиститьКонтур(Команда)
	ОчиститьКонтурНаСервере();
КонецПроцедуры
