...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаОтправкиВГосключ.Видимость = ЗначениеЗаполнено(Запись.НастройкаОтправкиВГосключ);
	Если ЗначениеЗаполнено(Запись.НастройкаОтправкиВГосключ) Тогда
		Заголовок = НСтр("ru='Статус подписания в сервисе Госключ'");
	Иначе
		Заголовок = НСтр("ru='Статус подписания в сервисе Моя подпись (ФНС)'");
	КонецЕсли;
...
Процедура ПослеЧтенияСвойствПодписи(СвойстваПодписи, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваПодписи.ТекстОшибки) Тогда
		НовыеСвойстваПодписи.РезультатПроверки.ОшибкаМатематическойПроверкиПодписи = СвойстваПодписи.ТекстОшибки;
		НовыеСвойстваПодписи.Статус = СвойстваПодписи.ТекстОшибки;
	КонецЕсли;
