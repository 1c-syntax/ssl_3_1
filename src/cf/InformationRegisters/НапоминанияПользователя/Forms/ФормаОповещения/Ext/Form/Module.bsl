...
Процедура ПриОткрытии(Отказ)
	...
	Если СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ОтображатьНапоминанияВЦентреОповещений Тогда
		Отказ = Истина;
	Иначе
		Активизировать();
	КонецЕсли;
...
Процедура ОбновитьТаблицуНапоминаний() 
	...
	Если СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ОтображатьНапоминанияВЦентреОповещений Тогда
		...
		СсылкиНаСообщения = Новый Массив;
		НапоминанияСообщений = Новый Соответствие;
		Для Каждого Напоминание Из Напоминания Цикл
			СсылкиНаСообщения.Добавить(Напоминание.НавигационнаяСсылка);
			НапоминанияСообщений.Вставить(Напоминание.НавигационнаяСсылка, Напоминание);
		КонецЦикла;
		
		ПредставленияСсылок = ПолучитьПредставленияНавигационныхСсылок(СсылкиНаСообщения);
		Для Каждого ПредставлениеСсылки Из ПредставленияСсылок Цикл
			Напоминание = НапоминанияСообщений[ПредставлениеСсылки.НавигационнаяСсылка];
			...
			Контекст.Вставить("НавигационнаяСсылка", ПредставлениеСсылки.НавигационнаяСсылка);
			
			ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьОткрытиеОповещения", НапоминанияПользователяСлужебныйКлиент, Контекст);
			
			ПоказатьОповещениеПользователя(
				Напоминание.Описание, ОписаниеОповещения,
				ПредставлениеСсылки.Текст, ПредставлениеСсылки.Картинка,
				СтатусОповещенияПользователя.Важное,
				Напоминание.НавигационнаяСсылка);
...
Процедура ОткрытьНапоминание()
	...
	ИначеЕсли НапоминанияПользователяКлиентСервер.ЭтоНавигационнаяСсылкаСообщения(
		Элементы.Напоминания.ТекущиеДанные.Идентификатор) Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(Элементы.Напоминания.ТекущиеДанные.НавигационнаяСсылка);
	Иначе
		РедактироватьНапоминание();
	КонецЕсли;
КонецПроцедуры
...
Процедура РедактироватьНапоминание()
	...
	ОткрытьФорму("РегистрСведений.НапоминанияПользователя.Форма.Напоминание", 
		Новый Структура("ДанныеКлюча", ПараметрыНапоминания));
КонецПроцедуры
