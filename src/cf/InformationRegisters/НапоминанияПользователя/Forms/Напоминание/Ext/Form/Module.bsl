...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДанныеКлюча <> Неопределено Тогда
		КлючЗаписи = РегистрыСведений.НапоминанияПользователя.СоздатьКлючЗаписи(Параметры.ДанныеКлюча);
		
		Запись = РегистрыСведений.НапоминанияПользователя.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(Запись, Параметры.ДанныеКлюча);
		Запись.Прочитать();
		
		ПриЧтенииНаСервере(Запись);
		ЗначениеВРеквизитФормы(Запись, "Объект");
	КонецЕсли;
	...
	Если Параметры.ДанныеКлюча <> Неопределено Тогда
		...
		ЗаполнитьЗначенияСвойств(ИсходныеПараметры, Параметры.ДанныеКлюча);
	...
	Если НапоминанияПользователяКлиентСервер.ЭтоНавигационнаяСсылкаСообщения(Объект.Идентификатор) Тогда
		Элементы.Текст.Видимость = Ложь;
		Элементы.Сообщение.Видимость = Истина;
		ИдентификаторСообщения = Новый ИдентификаторСообщенияСистемыВзаимодействия(СтрЗаменить(Объект.Идентификатор, "e1ccs/data/msg?id=",""));
		...
		Если НачалоДня(Сообщение.Дата) = НачалоДня(ТекущаяДатаСеанса()) Тогда
			ДатаТекстом = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'сегодня %1'"), Формат(Сообщение.Дата, НСтр("ru='ДФ=HH:mm'")));
		Иначе
			ДатаТекстом = Формат(Сообщение.Дата, НСтр("ru='ДФ=dd.MM.yyyy HH:mm'"));
		КонецЕсли;
		
		ОписаниеСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сообщение от %1, полученное %2'"),
			Автор.ПолноеИмя, ДатаТекстом);
		
		Элементы.ОписаниеСообщения.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ОписаниеСообщения);
		
		ПредставлениеСсылок = ПолучитьПредставленияНавигационныхСсылок(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Идентификатор));
		
		ТекстСсылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("<a href=""%1"">%2</a>",
		Объект.Идентификатор, ПредставлениеСсылок[0].Текст);
		Элементы.СсылкаНаСообщение.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ТекстСсылки);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИначеЕсли ТекущийОбъект.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.Периодически Тогда
		БлижайшееВремяНапоминания = НапоминанияПользователяСлужебный.БлижайшаяДатаСобытияПоРасписанию(Расписание);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	СтруктураПараметров.НавигационнаяСсылка = ПараметрыЗаписи.НавигационнаяСсылка;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрыЗаписи.Вставить("НавигационнаяСсылка", 
		НапоминанияПользователяСлужебный.НавигационнаяСсылкаНапоминания(ТекущийОбъект));
КонецПроцедуры
