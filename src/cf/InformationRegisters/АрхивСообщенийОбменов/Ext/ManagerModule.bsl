...
Процедура ПоместитьСообщениеВАрхив(УзелИнформационнойБазы, ПутьКИсходномуФайлу) Экспорт
	...
	Если ПланыОбменаБСП.Найти(УзелИнформационнойБазы.Метаданные().Имя) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ТекстПричины = НСтр("ru ='Файл не был помещен в архив.'") + Символы.ПС + СообщениеОбОшибке;
		
		ПараметрыЗаписи = Новый Структура;
		ПараметрыЗаписи.Вставить("УзелИнформационнойБазы", УзелИнформационнойБазы);
		ПараметрыЗаписи.Вставить("Причина", ТекстПричины);
...
Процедура ПоместитьВАрхив(УзелИнформационнойБазы, ПутьКИсходномуФайлу, Настройки)
	...
	Если ОбменДаннымиСлужебный.ПутьКФайлуСодержитНедопустимыеСимволы(ПутьКИсходномуФайлу) Тогда
	...
	ХешСуммаСообщенияОбмена = "";
	
	Если РасширениеФайла = "zip" Тогда
		ИмяВременногоКаталога = ПолучитьИмяВременногоФайла("");
		
		Если ОбменДаннымиСервер.РаспаковатьZipФайл(ПутьКИсходномуФайлу, ИмяВременногоКаталога) Тогда
			СписокРаспакованныхФайлов = НайтиФайлы(ИмяВременногоКаталога, ПолучитьМаскуВсеФайлы(), Истина);
			
			Если СписокРаспакованныхФайлов.Количество() Тогда
				ДвоичныеДанныеСообщенияОбмена = Новый ДвоичныеДанные(СписокРаспакованныхФайлов[0].ПолноеИмя);
				ХешСуммаСообщенияОбмена = ОбменДаннымиСервер.ХешСуммаСообщениеОбмена(ДвоичныеДанныеСообщенияОбмена);
			КонецЕсли;
		КонецЕсли; 
		
		УдалитьФайлы(ИмяВременногоКаталога);
	Иначе
		ДвоичныеДанныеСообщенияОбмена = Новый ДвоичныеДанные(ПутьКИсходномуФайлу);
		ХешСуммаСообщенияОбмена = ОбменДаннымиСервер.ХешСуммаСообщениеОбмена(ДвоичныеДанныеСообщенияОбмена);
	КонецЕсли;
	...
	НовыйАрхив.ХешСуммаФайла = ХешСуммаСообщенияОбмена;
