...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПравоНаЧтениеВерсииОбъектов = Ложь;
	
	Если ПодсистемаВерсионированияСуществует Тогда
		
		РегистрыСведенийВерсииОбъектов = Метаданные.РегистрыСведений.Найти("ВерсииОбъектов");
		ЕстьПравоНаЧтениеВерсииОбъектов = ПравоДоступа("Чтение", РегистрыСведенийВерсииОбъектов);
		Если ЕстьПравоНаЧтениеВерсииОбъектов Тогда
			
			МенеджерПеречисления = Перечисления["ТипыВерсийОбъекта"];
			КэшЗначений.Вставить("НепринятыйПоДатеЗапретаОбъектСуществуетВБазе",
				МенеджерПеречисления.НепринятыйПоДатеЗапретаОбъектСуществуетВБазе);
			КэшЗначений.Вставить("НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе",
				МенеджерПеречисления.НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе);
	...
	Если ПодсистемаВерсионированияСуществует И ЕстьПравоНаЧтениеВерсииОбъектов Тогда
...
Процедура ОпределитьСобытияВыгрузкиЗагрузки(ЕстьСобытияВыгрузки, ЕстьСобытияЗагрузки)
	...
	Если Не ЕстьСобытияЗагрузки И ПодсистемаВерсионированияСуществует Тогда
		...
		РегистрыСведенийВерсииОбъектов = Метаданные.РегистрыСведений.Найти("ВерсииОбъектов");
		Если ПравоДоступа("Чтение", РегистрыСведенийВерсииОбъектов) Тогда

			МенеджерПеречисления = Перечисления["ТипыВерсийОбъекта"];

			ЕстьСобытияЗагрузки = Отбор.Найти(МенеджерПеречисления.НепринятыйПоДатеЗапретаОбъектСуществуетВБазе) <> Неопределено
				Или Отбор.Найти(МенеджерПеречисления.НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе) <> Неопределено
				Или Отбор.Найти(МенеджерПеречисления.НепринятыеДанныеПоКоллизии) <> Неопределено Или Отбор.Найти(
				МенеджерПеречисления.ПринятыеДанныеПоКоллизии) <> Неопределено;
...
Процедура ОтборПоТипамПредупреждений()
	...
	Если ТипЗнч(КэшЗначений.ОтборТипыПредупреждений) = Тип("Строка") Тогда
		
		Если КэшЗначений.ОтборТипыПредупреждений = "Получение" Тогда
			
			КэшЗначений.ОтборТипыПредупреждений = ТипыПредупрежденийПолучения();
			
		ИначеЕсли КэшЗначений.ОтборТипыПредупреждений = "Отправка" Тогда
			
			КэшЗначений.ОтборТипыПредупреждений = ТипыПредупрежденийОтправки();
			
		Иначе
			
			КэшЗначений.ОтборТипыПредупреждений = Новый Массив;
...
&НаСервере
Функция ТипыПредупрежденийПолучения()
	
	ТипыПредупрежденийПолучения = Новый Массив;
	
	ТипыПредупрежденийПолучения.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.НезаполненныеРеквизиты"));
	ТипыПредупрежденийПолучения.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.НепроведенныйДокумент"));
	ТипыПредупрежденийПолучения.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.СообщениеОбменаНеПомещеноВАрхив"));
	ТипыПредупрежденийПолучения.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.ОшибкаВыполненияКодаОбработчиковПриПолученииДанных"));
	
	ПодсистемаВерсионированияСуществует = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВерсионированиеОбъектов");
	Если ПодсистемаВерсионированияСуществует Тогда
		
		ТипыПредупрежденийПолучения.Добавить(КэшЗначений.НепринятыйПоДатеЗапретаОбъектСуществуетВБазе);
		ТипыПредупрежденийПолучения.Добавить(КэшЗначений.НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе);
		ТипыПредупрежденийПолучения.Добавить(КэшЗначений.НепринятыеДанныеПоКоллизии);
		ТипыПредупрежденийПолучения.Добавить(КэшЗначений.ПринятыеДанныеПоКоллизии);
		
	КонецЕсли;
	
	Возврат ТипыПредупрежденийПолучения;
	
КонецФункции

&НаСервере
Функция ТипыПредупрежденийОтправки()
	
	ТипыПредупрежденийОтправки = Новый Массив;
	
	ТипыПредупрежденийОтправки.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.АдминистративнаяОшибкаПриложения"));
	ТипыПредупрежденийОтправки.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.ОшибкаВыполненияКодаОбработчиковПриОтправкеДанных"));
	ТипыПредупрежденийОтправки.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.ОшибкаПроверкиСконвертированногоОбъекта"));
	
	Возврат ТипыПредупрежденийОтправки;
...
Процедура ИзменитьСостояниеПредупрежденияНаСервере(НаборСтруктур, СкрытьОтобразить)
	...
	Для Каждого СтруктураСДаннымиСтроки Из НаборСтруктур Цикл
		...
		ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВерсионированиеОбъектов") Тогда
			...
			Если ПравоДоступа("Чтение", РегистрыСведенийВерсииОбъектов) Тогда
				МодульВерсионированиеОбъектов = ОбщегоНазначения.ОбщийМодуль("ВерсионированиеОбъектов");
				МодульВерсионированиеОбъектов.ИгнорироватьВерсиюОбъекта(СтруктураСДаннымиСтроки.ПроблемныйОбъект,
					СтруктураСДаннымиСтроки.ВерсияИзДругойПрограммы, СкрытьОтобразить);
			КонецЕсли;
