...
Процедура ЖурналРегистрации(Команда)
	...
	ПараметрыОтбораЖурнала.Вставить("СобытиеЖурналаРегистрации", ОбменДаннымиКлиент.СобытиеЖурналаРегистрацииОбменДанными());
...
Процедура ОткрытьКарточкуПроблемы(ПараметрыОткрытия)
	...
	Если ТипПредупреждения = ПредопределенноеЗначение("Перечисление.ТипыПроблемОбменаДанными.НезаполненныеРеквизиты") Тогда
...
Процедура ДинамическийСписокСПредупреждениямиВерсионирования()
	...
	МодульВерсионированиеОбъектов.ТекстЗапросаСпискаПредупрежденияВерсий(ТекстЗапроса);
	
	// ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка() нельзя использовать из-за того,
	// что метод не умеет очищать свойство динамического списка "ОсновнаяТаблица"
	Список.ТекстЗапроса = Список.ТекстЗапроса + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + ТекстЗапроса;
