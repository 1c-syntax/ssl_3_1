...
//   ВключатьПроблемыПообъектногоОбмена - Булево - признак того, что требуется включить данные пообъектного обмена
//
...
// 
Функция КоличествоПредупрежденийДетально(УзлыСинхронизации = Неопределено, ВключатьПроблемыПообъектногоОбмена = Ложь) Экспорт
	...
	Если ВключатьПроблемыПообъектногоОбмена Тогда
		
		ДанныеПообъектногоОбмена = РегистрыСведений.ОбъектыКЗагрузке.КоличествоЗаписейПоУзламПланаОбмена(УзлыСинхронизации);
		
		Если ДанныеПообъектногоОбмена = Неопределено Тогда
			КоличествоПроблемПообъектныйОбмен = 0;
		Иначе
			КоличествоПроблемПообъектныйОбмен = ДанныеПообъектногоОбмена.Получить(УзлыСинхронизации);
			КоличествоПроблемПообъектныйОбмен = ?(КоличествоПроблемПообъектныйОбмен = Неопределено, 0, КоличествоПроблемПообъектныйОбмен);
		КонецЕсли;
	...
	Если НЕ РезультатЗапроса[1].Пустой() Тогда
		...
		Если ВыборкаПроблемыПолучения.Следующий() Тогда
			...
			СтруктураЗаголовков.КоличествоПредупрежденийПолучения = ВыборкаПроблемыПолучения.КоличествоПредупрежденийПолучение + КоличествоПроблемПообъектныйОбмен;
			
		КонецЕсли;
		
	ИначеЕсли ВключатьПроблемыПообъектногоОбмена Тогда
		
		СтруктураЗаголовков.КоличествоПредупрежденийПолучения = КоличествоПроблемПообъектныйОбмен;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчиститьПредупрежденияСинхронизации", Истина);
