...
Процедура ОбновитьВспомогательныеДанныеРегистраПоИзменениямКонфигурации() Экспорт
	...
	Если УправлениеДоступомСлужебный.КонстантаОграничиватьДоступНаУровнеЗаписей() Тогда
...
Процедура ОбновитьДанныеРегистра(ЕстьИзменения = Неопределено) Экспорт
	...
	Если УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
		УправлениеДоступомСлужебный.ОчиститьРегистрСведений(
			Метаданные.РегистрыСведений.ГруппыЗначенийДоступа.ПолноеИмя(), ЕстьИзменения);
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьГруппыПустыхЗначенийДоступа(ЕстьИзменения = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ЗначенияДоступаСГруппами.ИменаТаблицДляОбновления) Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если Не РезультатыЗапроса[Индекс].Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				НаборЗаписей.Отбор.ЗначениеДоступа.Установить(Выборка.ЗначениеДоступа);
				НаборЗаписей.Отбор.ГруппаЗначенийДоступа.Установить(Выборка.ГруппаЗначенийДоступа);
				ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(НаборЗаписей); // Удаление лишних записей о связях.
		...
		Если Не РезультатыЗапроса[Индекс + 1].Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(НаборЗаписей); // Добавление недостающих записей о связях.
