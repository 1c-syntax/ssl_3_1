...
Процедура УстановитьНазваниеПрограммыДляОткрытияМакета()
	...
	ОписаниеДействия = ?(Параметры.ТолькоОткрытие, НСтр("ru = 'открытия'"), НСтр("ru = 'внесения изменений'"));
	...
	Для Каждого Элемент Из ЗаполняемыеЭлементы Цикл
		Элемент.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Элемент.Заголовок, ПредставлениеМакета,
			НазваниеПрограммыДляОткрытияМакета, ОписаниеДействия);
	КонецЦикла;
