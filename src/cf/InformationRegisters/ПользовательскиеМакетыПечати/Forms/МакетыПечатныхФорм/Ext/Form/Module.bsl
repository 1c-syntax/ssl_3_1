...
Функция ОбновитьОтображениеМакета(Знач Параметр)
	...
	ИсточникиДанных = Параметр.ИсточникиДанных;
...
Функция ПолучитьНавигационнуюСсылкуНаФормуСписка(ИдентификаторОбъектаМетаданных)
	Возврат  "e1cib/list/" + ИдентификаторОбъектаМетаданных.ПолноеИмя;
КонецФункции
...
Процедура ЗаполнитьТаблицуМакетовПечатныхФорм(Ветка)
	...
	ИзмененныеМакеты = РегистрыСведений.ПользовательскиеМакетыПечати.ИзмененныеМакеты();
	...
	Для Каждого ОписаниеМакета Из УправлениеПечатью.МакетыПечатныхФорм(Истина) Цикл
		...
		ТипМакета = РегистрыСведений.ПользовательскиеМакетыПечати.ТипМакета(ОбъектМетаданных.Имя, ИмяВладельца);
		
		Макет = СписокМакетов.Добавить();
		Макет.ИсточникМакета = ИмяВладельца;
		Макет.ИсточникиДанных = ИмяВладельца;
		...
		Макет.Картинка = РегистрыСведений.ПользовательскиеМакетыПечати.ИндексКартинки(ТипМакета);
		Макет.КартинкаГруппы = РегистрыСведений.ПользовательскиеМакетыПечати.КартинкаМакета(ТипМакета);
	...
	СоответствиеВладельцевМакетов = ОбщегоНазначения.ИдентификаторыОбъектовМетаданных(СписокМакетов.ВыгрузитьКолонку("Владелец"), Ложь);
	
	Для Каждого Макет Из СписокМакетов Цикл
		Если Макет.Владелец <> Неопределено И СоответствиеВладельцевМакетов[Макет.Владелец.ПолноеИмя()] <> Неопределено Тогда
			Макет.Владелец = СоответствиеВладельцевМакетов[Макет.Владелец.ПолноеИмя()];
	...
	Для Каждого Владелец Из ВладельцыМакетов Цикл
		ГруппаМакетов = Ветка.ПолучитьЭлементы().Добавить();
...
Процедура ЗаписатьКэшМакетов(МакетыОбъектов)
	...
	ИдентификаторыОбъектовМетаданных = ОбщегоНазначения.ИдентификаторыОбъектовМетаданных(ОбъектыСКомандамиПечати.ВыгрузитьЗначения());
...
Процедура ЗагрузитьВетку(Ветка)
	...
	Для Каждого Элемент Из Ветка.ПолучитьЭлементы() Цикл
		...
		Для Каждого ПодчиненныйЭлемент Из Элемент.ПолучитьЭлементы() Цикл
			ЗагрузитьВетку(ПодчиненныйЭлемент);
		КонецЦикла;
	КонецЦикла;
