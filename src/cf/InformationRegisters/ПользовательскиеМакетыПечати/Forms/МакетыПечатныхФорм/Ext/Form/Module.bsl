...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ПозицияМакетаВДереве = ПозицияМакетаВДереве(Параметры.ПутьКМакету);
	
	Если ЗначениеЗаполнено(ПозицияМакетаВДереве) Тогда
		ПозицияВДереве = ПозицияМакетаВДереве;
	КонецЕсли;
...
Функция ИдентификаторВладельцаМакета(ПутьКМакету)
	...
	ИдентификаторВладельцаМакета = Неопределено;
	
	Для Каждого ИсточникДанных Из ИсточникиДанныхМакета Цикл
		Если ОбъектыМетаданных[ИсточникДанных] <> Неопределено Тогда
			ИдентификаторВладельцаМакета = ОбъектыМетаданных[ИсточникДанных].ПолноеИмя();
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ИдентификаторВладельцаМакета;
