...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МакетыВыгрузки = Параметры.МакетыВыгрузки;
	ЗагрузитьНастройкиСпособГруппировкиОбъектов();
	
	ЗаполнитьОбъектыСКомандамиПечати();
	
	Если ГруппировкаПоРазделам Тогда
		ЗаполнитьСписокМакетовПоРазделам();
	Иначе
		ЗаполнитьСписокМакетовПоОбъектам();
	КонецЕсли;
	...
	Элементы.МакетыФорматСохраненияВыгрузки.Видимость = МакетыВыгрузки;
	Элементы.МакетыДобавитьМакет.Видимость = НЕ МакетыВыгрузки;
	Элементы.МакетыДобавитьМакетOfficeOpenXML.Видимость = НЕ МакетыВыгрузки;
	Элементы.МакетыДобавитьМакетВыгрузки.Видимость = МакетыВыгрузки;
	
	Если МакетыВыгрузки Тогда
		
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru = 'Макеты файлов выгрузки'");
		Элементы.МакетыУсловияДоступности.Заголовок = Нстр("ru = 'Видимость в подменю ""Выгрузить в файл""...'");
		
		ТекстПодсказки = НСтр("ru = 'Показывать в подменю ""Выгрузить в файл""'");
		Элементы.МакетыИспользуется.Подсказка = ТекстПодсказки;
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Владелец) Тогда
		...
		ПозицияВДереве = ПозицияВладельцаВДереве(Параметры.Владелец);
		Если ЗначениеЗаполнено(ПозицияВДереве) Тогда
			ПерейтиКЭлементу(ПозицияВДереве);
		КонецЕсли;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Не ЗначениеЗаполнено(Параметры.ПутьКМакету)
	   И Не ЗначениеЗаполнено(Параметры.Владелец) Тогда
		ПерейтиКЭлементу(ПозицияВДереве);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "Запись_ТабличныйДокумент" Или ИмяСобытия = "Запись_ОфисныйДокумент" 
		...
		ИсточникСоответствуетПриемнику = Истина;
		Если Параметр.МакетДляВыгрузкиОбъекта Тогда
			ИсточникСоответствуетПриемнику = (МакетыВыгрузки = Параметр.МакетДляВыгрузкиОбъекта);
		КонецЕсли;
		
		Если Не ТолькоПросмотр
		   И ИсточникСоответствуетПриемнику Тогда
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Для Каждого Ветка Из ВеткиВладельцаМакетов Цикл
			...
			Если МакетыВыгрузки Тогда
				...
				ФорматСохраненияВыгрузки = Параметр.ФорматСохраненияВыгрузки;
				Расширение = СоответствиеФорматаВыгрузкиРасширению[ФорматСохраненияВыгрузки];
				Макет.ИндексКартинкиФорматаСохранения =
					РегистрыСведений.ПользовательскиеМакетыПечати.ИндексПиктограммыФайла(Расширение);
				Макет.ФорматСохраненияВыгрузки = ФорматСохраненияВыгрузки;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ Отказ Тогда
		СохранитьНастройкиСпособГруппировкиОбъектов(ГруппировкаПоРазделам);
	КонецЕсли;
...
Процедура ОбработкаПолученияНавигационнойСсылки(Ключ, Представление, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПараметрМакетыВыгрузки = Новый Структура("МакетыВыгрузки", МакетыВыгрузки);
	НавигационнаяСсылка = НавигационнаяСсылкаПоПараметру(Представление, ПараметрМакетыВыгрузки);
...
&НаКлиенте
Процедура ГруппировкаПоРазделамПриИзменении(Элемент)
	...
	ИзмененСпособГруппировки = Истина;
	УстановитьОтборМакетов();
...
Процедура ДобавитьМакет(Команда)
	...
	ПараметрыОповещения.Вставить("Копирование", Ложь);
	ПараметрыОповещения.Вставить("ТипМакета", "MXL");
...
Процедура ДобавитьМакетOfficeOpenXML(Команда)
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Копирование", Ложь);
	ПараметрыОповещения.Вставить("ТипМакета", "DOCX");
	ПараметрыОповещения.Вставить("МакетДляВыгрузкиОбъекта", Ложь);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриВыбореИмениМакета", ЭтотОбъект, ПараметрыОповещения);
	УникальноеНаименование = ПолучитьУникальноеНаименование(НСтр("ru = 'Новая печатная форма'"), Ложь);
	ПоказатьВводСтроки(ОписаниеОповещения, УникальноеНаименование, НСтр("ru = 'Введите наименование макета'"), 100, Ложь)
...
Процедура УсловияДоступности(Команда)
	...
	ПараметрыОткрытия.Вставить("ЭтоМакетВыгрузки", МакетыВыгрузки);
	ПараметрыОткрытия.Вставить("ПредставлениеМакетаВыгрузки", ТекущиеДанные.Представление);
...
&НаКлиенте
Процедура ДобавитьМакетВыгрузки(Команда)
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Копирование", Ложь);
	ПараметрыОповещения.Вставить("ТипМакета", "MXL");
	ПараметрыОповещения.Вставить("МакетДляВыгрузкиОбъекта", Истина);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриВыбореИмениМакета", ЭтотОбъект, ПараметрыОповещения);
	УникальноеНаименование = ПолучитьУникальноеНаименование(НСтр("ru = 'Новая форма выгрузки'"), Ложь);
	Подсказка = НСтр("ru = 'Введите наименование макета'");
	ПоказатьВводСтроки(ОписаниеОповещения, УникальноеНаименование, Подсказка, 100, Ложь);
...
Процедура ОткрытьМакетПечатнойФормыДляРедактированияПродолжение(ПереключитьИспользование, ТекущиеДанные) Экспорт
	...
	ПараметрыОткрытия.Вставить("ОсновнаяПечатнаяФорма", ТекущиеДанные.ОсновнаяПечатнаяФорма);
	ПараметрыОткрытия.Вставить("НаименованиеПечатнойФормы", ТекущиеДанные.НаименованиеПечатнойФормы);
	ПараметрыОткрытия.Вставить("ДоступноНазначение", Не ТекущиеДанные.Поставляемый);
	ПараметрыОткрытия.Вставить("МакетДляВыгрузкиОбъекта", ТекущиеДанные.МакетДляВыгрузкиОбъекта);
	ПараметрыОткрытия.Вставить("ФорматСохраненияВыгрузки", ТекущиеДанные.ФорматСохраненияВыгрузки);
...
Процедура УстановитьДоступностьКнопокКоманднойПанели()
	...
	Элементы.МакетыДобавитьМакетВыгрузки.Доступность = ТекущийМакетВыбран И ТекущийМакет.ДоступноСоздание;
...
Процедура Скопировать(Команда)
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Копирование", Истина);
	ПараметрыОповещения.Вставить("ТипМакета");
	
	ТекущийМакет = Элементы.Макеты.ТекущиеДанные;
	Если ТекущийМакет.МакетДляВыгрузкиОбъекта Тогда
		
		ПараметрыОповещения.Вставить("МакетДляВыгрузкиОбъекта", Истина); 
		ПараметрыОповещения.Вставить("ФорматСохраненияВыгрузки", ТекущийМакет.ФорматСохраненияВыгрузки);
		
	Иначе
		
		ПараметрыОповещения.Вставить("МакетДляВыгрузкиОбъекта", Ложь); 
		ПараметрыОповещения.Вставить("ФорматСохраненияВыгрузки");
...
Процедура ПриВыбореИмениМакета(ИмяМакета, ПараметрыОповещения) Экспорт
	...
	ПараметрыОткрытия.Вставить("МакетДляВыгрузкиОбъекта", Ложь);
	
	Если  ПараметрыОповещения.МакетДляВыгрузкиОбъекта Тогда
		
		ПараметрыОткрытия.МакетДляВыгрузкиОбъекта = Истина;
		Если Копирование Тогда
			ПараметрыОткрытия.Вставить("ФорматСохраненияВыгрузки", ТекущиеДанные.ФорматСохраненияВыгрузки);
		КонецЕсли;
...
Процедура УдалитьМакет(Команда)
	...
	ИмяОбъектаМетаданных = Родитель.Идентификатор;
	ОчиститьКэш(ИмяОбъектаМетаданных, ОбъектыСКомандамиПечати);
...
Процедура УстановитьУсловноеОформление()
	...
	ОформляемоеПоле = ЭлементОформления.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(Элементы.МакетыИндексКартинкиФорматаСохранения.Имя);
	
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Макеты.ФорматСохраненияВыгрузки");
...
&НаСервере
Процедура ЗаполнитьСписокМакетовПоОбъектам()
...
Функция МакетыОбъекта(ИмяОбъектаМетаданных)
	...
	Отбор.Вставить("МакетДляВыгрузкиОбъекта", МакетыВыгрузки);
	
	Возврат МакетыОбъекта.НайтиСтроки(Отбор);
...
Процедура ЗаполнитьОбъектыСКомандамиПечати()
	...
	МетаданныеДляВыгрузки = Новый Массив; // Массив Из Строка
	МетаданныеДляВыгрузки.Добавить("Справочники");
	МетаданныеДляВыгрузки.Добавить("Документы");
	...
	Для Каждого ОбъектМетаданных Из УправлениеПечатью.ИсточникиКомандПечати() Цикл
		...
		Если МакетыВыгрузки
		   И МетаданныеДляВыгрузки.Найти(ИмяБазовогоТипа) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Объект Из ОбъектыСМакетами Цикл
		...
		Если Объект.Ключ <> Метаданные.ОбщиеМакеты Тогда
			...
			ИмяБазовогоТипа = ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(Объект.Ключ);
			Если МакетыВыгрузки
			   И МетаданныеДляВыгрузки.Найти(ИмяБазовогоТипа) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗагрузитьСписокМакетов()
	...
	Если (ВыполняетсяЗагрузкаДанных
	 Или Не ЕстьОбъектыБезКэша())
	       И НЕ ИзмененСпособГруппировки Тогда
		Возврат;
	КонецЕсли;
	
	ИзмененСпособГруппировки = Ложь;
...
&НаСервере
Функция ПозицияВладельцаВДереве(Знач ИдентификаторВладельца)
	
	ВладелецМакета = НайтиЭлементВДеревеМакетов(ИдентификаторВладельца);
	
	Если ВладелецМакета <> Неопределено Тогда
		Результат = ПутьКЭлементуВДереве(ВладелецМакета);
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьНастройкиСпособГруппировкиОбъектов()
	
	ЗначениеНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкаОткрытияМакетовСпособГруппировкиОбъектов",
		"ГруппировкаПоРазделам");
	Если ЗначениеНастроек <> Неопределено Тогда
		ГруппировкаПоРазделам = ЗначениеНастроек;
	Иначе
		ГруппировкаПоРазделам = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция НавигационнаяСсылкаПоПараметру(Знач Представление, Знач ПараметрМакетыВыгрузки)
	
	Возврат ПолучитьНавигационнуюСсылку(
		Метаданные.РегистрыСведений.ПользовательскиеМакетыПечати,
		Представление,
		ПараметрМакетыВыгрузки);
...
Процедура ДеревоОбъектовМетаданныхЗаполнить()
	...
	ОбъектыМетаданных.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка"));
	ОбъектыМетаданных.Колонки.Добавить("Синоним", Новый ОписаниеТипов("Строка"));
	ОбъектыМетаданных.Колонки.Добавить("Картинка", Новый ОписаниеТипов("Картинка"));
	ОбъектыМетаданных.Колонки.Добавить("Выводить", Новый ОписаниеТипов("Булево"));
	
	ВыводитьКоллекцию = НЕ МакетыВыгрузки;
	
	ИмяМетаданных = НСтр("ru = 'Справочники'");
	НоваяСтрокаКоллекции(
		"Справочники",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеСправочники,
		Истина,
		ОбъектыМетаданных);
	
	ИмяМетаданных = НСтр("ru = 'Документы'");
	НоваяСтрокаКоллекции(
		"Документы",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеДокументы,
		Истина,
		ОбъектыМетаданных);
	
	ИмяМетаданных = НСтр("ru = 'Журналы документов'");
	НоваяСтрокаКоллекции(
		"ЖурналыДокументов",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеЖурналыДокументов,
		ВыводитьКоллекцию,
		ОбъектыМетаданных);
		
	ИмяМетаданных = НСтр("ru = 'Планы видов характеристик'");
	НоваяСтрокаКоллекции(
		"ПланыВидовХарактеристик",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеПланыВидовХарактеристик,
		ВыводитьКоллекцию,
		ОбъектыМетаданных);
		
	ИмяМетаданных = НСтр("ru = 'Планы счетов'");
	НоваяСтрокаКоллекции(
		"ПланыСчетов",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеПланыСчетов,
		ВыводитьКоллекцию,
		ОбъектыМетаданных);
	
	ИмяМетаданных = НСтр("ru = 'Планы видов расчета'");
	НоваяСтрокаКоллекции(
		"ПланыВидовРасчета",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеПланыВидовРасчета,
		ВыводитьКоллекцию,
		ОбъектыМетаданных);
	
	ИмяМетаданных = НСтр("ru = 'Бизнес-процессы'");
	НоваяСтрокаКоллекции(
		"БизнесПроцессы",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеБизнесПроцессы,
		ВыводитьКоллекцию,
		ОбъектыМетаданных);
		
	ИмяМетаданных = НСтр("ru = 'Задачи'");
	НоваяСтрокаКоллекции(
		"Задачи",
		ИмяМетаданных,
		БиблиотекаКартинок.МетаданныеЗадачи,
		ВыводитьКоллекцию,
		ОбъектыМетаданных);
	...
	Если ОбъектыСКомандамиПечати.Количество() > 0 Тогда
		...
		Для Каждого ОбъектМетаданныхПолноеИмя Из ОбъектыСКомандамиПечати Цикл
			
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОбъектМетаданныхПолноеИмя.Значение);
	...
	Для Каждого ОбъектМетаданных Из ОбъектыМетаданных Цикл
		...
		Если НЕ ОбъектМетаданных.Выводить Тогда
			Продолжить;
		КонецЕсли;
		
		Если КоллекцииВыбираемыхОбъектовМетаданных.НайтиПоЗначению(ОбъектМетаданных.Имя) <> Неопределено Тогда
			
			ДобавитьЭлементДереваОбъектовМетаданных(ОбъектМетаданных);
...
&НаСервере
Процедура ДобавитьЭлементДереваОбъектовМетаданных(ПараметрыЭлемента)
	
	НоваяВетка = Макеты.ПолучитьЭлементы().Добавить();
	НоваяВетка.Представление = ПараметрыЭлемента.Имя;
	НоваяВетка.КартинкаГруппы = ПараметрыЭлемента.Картинка;
	НоваяВетка.Идентификатор = ПараметрыЭлемента.Имя;
	НоваяВетка.КартинкаИспользования = -1;
	НоваяВетка.ЭтоГруппа = Истина;
	НоваяВетка.Используется = Истина;
	НоваяВетка.СтрокаПоиска = НРег(НоваяВетка.Представление);
	...
	НоваяВетка.Владелец = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(НоваяВетка.Идентификатор, Ложь);
	НоваяВетка.ДоступноСоздание = Ложь;
	
	МассивКоллекции = Новый Массив; // Массив Из Метаданные
	Для Каждого ОбъектМетаданныхПолноеИмя Из ОбъектыСКомандамиПечати Цикл
		...
		Если ОбъектМетаданных <> Неопределено Тогда
			...
			ИмяБазовогоТипа = ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(ОбъектМетаданных);
			Если ПараметрыЭлемента.Имя = ИмяБазовогоТипа Тогда
				МассивКоллекции.Добавить(ОбъектМетаданных);
			КонецЕсли;
	...
	ВывестиКоллекцию(НоваяВетка, МассивКоллекции);
	
КонецПроцедуры

&НаСервере
Процедура НоваяСтрокаКоллекции(Знач Имя, Знач Синоним, Знач Картинка, Знач ВыводитьКоллекцию, Таблица)
	
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.Имя = Имя;
	НоваяСтрока.Синоним = Синоним;
	НоваяСтрока.Картинка = Картинка;
	НоваяСтрока.Выводить = ВыводитьКоллекцию;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СохранитьНастройкиСпособГруппировкиОбъектов(Знач ГруппировкаПоРазделам)
