...
Процедура ОбновлениеСтатусовМЧД(Доверенности = Неопределено) Экспорт
	...
	Запрос.Текст =  "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                ...
	                |			И МашиночитаемыеДоверенностиСтатусы.ТипТранзакции = ЗНАЧЕНИЕ(Перечисление.ТипыТранзакцийСРеестромМЧД.Регистрация)
	                |			ИЛИ МашиночитаемыеДоверенностиСтатусы.ТехническийСтатус = ЗНАЧЕНИЕ(Перечисление.ТехническиеСтатусыМЧД.Отменена)
	                |			И МашиночитаемыеДоверенностиСтатусы.ТипТранзакции = ЗНАЧЕНИЕ(Перечисление.ТипыТранзакцийСРеестромМЧД.Отмена)
	                |			ТОГДА ЛОЖЬ
	...
	Если Не Результат.Пустой() Тогда
		...
		ДоверенностиДляПроверкиСтатуса.Колонки.Добавить("ХронологическиеДанные");
		...
		Если Авторизация.Ошибка <> Неопределено Тогда
			Для Каждого ДоверенностьДляПроверки Из ДоверенностиДляПроверкиСтатуса Цикл
				...
				ДоверенностьДляПроверки.ДанныеОшибкиЗапросаСтатуса = ДанныеОшибкиЗапросаСтатуса(Авторизация.Ошибка, Неопределено);
		...
		Для Каждого ДоверенностьДляПроверки Из ДоверенностиДляПроверкиСтатуса Цикл
			...
			ДанныеОшибкиЗапросаСтатусаТекущие = ?(ТипЗнч(ДоверенностьДляПроверки.ДанныеОшибкиЗапросаСтатусаТекущие) = Тип("ХранилищеЗначения"), 
				ДоверенностьДляПроверки.ДанныеОшибкиЗапросаСтатусаТекущие.Получить(), Неопределено);
				
			Если ТипЗнч(ДанныеОшибкиЗапросаСтатусаТекущие) = Тип("Структура") Тогда
				ДанныеОшибкиЗапросаСтатусаТекущие = ДанныеОшибкиЗапросаСтатуса(ДанныеОшибкиЗапросаСтатусаТекущие, ДоверенностьДляПроверки.ХронологическиеДанные);
				ДоверенностьДляПроверки.ДанныеОшибкиЗапросаСтатуса = ДанныеОшибкиЗапросаСтатусаТекущие.ДанныеОшибкиЗапросаСтатуса;
				ДоверенностьДляПроверки.ХронологическиеДанные = ДанныеОшибкиЗапросаСтатусаТекущие.ХронологическиеДанные;
			КонецЕсли;
			...
			Если ДоверенностьДляПроверки.ЗапросСтатусаДоверенности Тогда
				...
				Если ДоверенностьДляПроверки.ЗаписыватьВРегистр И ДоверенностьДляПроверки.ДляНалоговыхОрганов Тогда
					ДоверенностьДляПроверки.ХронологическиеДанные = МашиночитаемыеДоверенностиФНССлужебный.ХронологическиеДанныеДоверенностиМЧДРР(
						ДоверенностьДляПроверки.МашиночитаемаяДоверенность,
						ТокенДоступа, НСтр("ru='Не удалось обновить хронологические данные доверенности'"));
				КонецЕсли;
...
Процедура ЗаписатьРезультатОбновленияСтатуса(ДанныеДляЗаписи)
	...
	Попытка
		...
		Для Каждого ДоверенностьДляПроверки Из ДанныеДляЗаписи Цикл
			...
			ДанныеОшибкиЗапросаСтатуса = ДанныеОшибкиЗапросаСтатуса(ДоверенностьДляПроверки.ДанныеОшибкиЗапросаСтатуса, ДоверенностьДляПроверки.ХронологическиеДанные);
			НаборЗаписей[0].ДанныеОшибкиЗапросаСтатуса = Новый ХранилищеЗначения(ДанныеОшибкиЗапросаСтатуса, 
...
Функция ДанныеОшибкиЗапросаСтатуса(ДанныеОшибкиЗапросаСтатуса, ХронологическиеДанные)
	
	Результат = Новый Структура("ДанныеОшибкиЗапросаСтатуса, ХронологическиеДанные");
	
	Если МашиночитаемыеДоверенностиФНССлужебный.СодержитХронологическиеДанные(ДанныеОшибкиЗапросаСтатуса) Тогда
		Результат.ДанныеОшибкиЗапросаСтатуса = ДанныеОшибкиЗапросаСтатуса.ДанныеОшибкиЗапросаСтатуса;
	Иначе
		Результат.ДанныеОшибкиЗапросаСтатуса = ДанныеОшибкиЗапросаСтатуса;
	КонецЕсли;
