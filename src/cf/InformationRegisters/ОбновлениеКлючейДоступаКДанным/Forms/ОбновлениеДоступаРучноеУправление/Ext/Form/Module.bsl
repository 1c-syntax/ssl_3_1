...
Процедура ОбновитьДоступКОбъектуДляВидаПользователей(СсылкаНаОбъект, ПолноеИмя, ИдентификаторТранзакции, ДляВнешнихПользователей, Текст)
	...
	Если ПараметрыОграничения.СЗаписьюВсегдаРазрешенногоКлючаДоступа Тогда
		Если КлючДоступаИсточникаУстарел Тогда
			Текст = Текст + Символы.ПС + НСтр("ru = '1. Установлен всегда разрешенный ключ доступа к объекту.'");
		Иначе
			Текст = Текст + Символы.ПС + НСтр("ru = '1. Обновление не требуется. Всегда разрешенный ключ доступа к объекту уже установлен.'");
		КонецЕсли;
	Иначе
		Если КлючДоступаИсточникаУстарел Тогда
	...
	Если ПараметрыОграничения.СЗаписьюВсегдаРазрешенногоКлючаДоступа Тогда
		Если ЕстьИзмененияПрав Тогда
			Текст = Текст + Символы.ПС
				+ НСтр("ru = '2. Обновлены права на всегда разрешенный ключ доступа
				             |   у групп доступа или пользователей или внешних пользователей.'");
		Иначе
			Текст = Текст + Символы.ПС
				+ НСтр("ru = '2. Обновление не требуется. Права на всегда разрешенный ключ доступа
				             |   у групп доступа, пользователей и внешних пользователей не устарели.'");
		КонецЕсли;
