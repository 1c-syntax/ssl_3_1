...
Процедура ДеревоОбъектовМетаданныхСинхронизироватьПриИзменении(Элемент)
	...
	Если Не ТекущиеДанные.Синхронизировать И ТекущиеДанные.ПредыдущаяСинхронизация Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеВопросаОСинхронизации", ЭтотОбъект);
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для %1 завершить синхронизацию и отключить?'"), 
...
&НаКлиенте
Процедура ПослеВопросаОСинхронизации(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ОткрытьФормуНастроек()
	...
	КонецЕсли;
	ПараметрыЗаписи.Вставить("ЗапрещатьИзменятьОсновныеПараметры", Истина);
