...
Функция ПолучитьТопВариантов(ДатаНачала, ДатаОкончания, Количество, Знач ВерсияПлатформы = Неопределено) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ТОП", Формат(Количество, "ЧГ=0"));
	...
	Если ВерсияПлатформы <> Неопределено Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%УслВерсияПлатформы", "И ДампыПлатформы.ВерсияПлатформы = &ВерсияПлатформы");
		Запрос.УстановитьПараметр("ВерсияПлатформы", ВерсияПлатформыЧисло);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%УслВерсияПлатформы", "");
	КонецЕсли;
