...
Функция КодировкаВерсииФайла(ВерсияСсылка) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КодировкиФайлов.Кодировка КАК Кодировка
		|ИЗ
		|	РегистрСведений.КодировкиФайлов КАК КодировкиФайлов
		|ГДЕ
		|	КодировкиФайлов.Файл = &Файл";

	Запрос.УстановитьПараметр("Файл", ВерсияСсылка);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Выборка.Следующий();
	Результат = Выборка.Кодировка;
...
Процедура ЗаписатьКодировкуВерсииФайла(ВерсияСсылка, Кодировка) Экспорт
	...
	КодировкаВерсииФайла = КодировкаВерсииФайла(ВерсияСсылка);
	Если ЗначениеЗаполнено(КодировкаВерсииФайла) Тогда
		Возврат;
	КонецЕсли;
