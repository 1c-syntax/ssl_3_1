...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		МодульПодключаемыеКомандыКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодключаемыеКомандыКлиент");
		ВыбранныеВладельцы = Новый Массив;
		ВыделенныеСтроки = Элементы.ДеревоОбъектовМетаданных.ВыделенныеСтроки;
		Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
			СтрокаДереваОбъектов = ДеревоОбъектовМетаданных.НайтиПоИдентификатору(ИдентификаторСтроки);
			ДобавитьВладельцаФайла(ВыбранныеВладельцы, СтрокаДереваОбъектов);
		КонецЦикла;
		МодульПодключаемыеКомандыКлиент.НачатьВыполнениеКоманды(ЭтотОбъект, Команда, ВыбранныеВладельцы);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыполнитьКомандуНаСервере(ПараметрыВыполнения)
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		МодульПодключаемыеКоманды = ОбщегоНазначения.ОбщийМодуль("ПодключаемыеКоманды");
		МодульПодключаемыеКоманды.ВыполнитьКоманду(ЭтотОбъект, ПараметрыВыполнения, Элементы.ДеревоОбъектовМетаданных);
	КонецЕсли;
КонецПроцедуры
...
Процедура Подключаемый_ОбновитьКоманды()
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		МодульПодключаемыеКомандыКлиентСервер.ОбновитьКоманды(ЭтотОбъект, Элементы.ДеревоОбъектовМетаданных);
	КонецЕсли;
КонецПроцедуры
...
Процедура РассчитатьСведенияОбУдаляемыхФайлах()
	...
	ПараметрыВыполнения.УточнениеОшибки =
		НСтр("ru = 'Не удалось подсчитать объем удаляемых файлов по причине:'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	УстановитьПривилегированныйРежим(Истина);
	// АПК:453-выкл - №760.4 Допустимо создавать задание без записи для передачи параметров.
	Задание = РегламентныеЗадания.СоздатьРегламентноеЗадание(Метаданные.РегламентныеЗадания.ОчисткаНенужныхФайлов);
	ПараметрыЗадания = Новый Структура("ЭтоЗапускВручную", Истина);
	...
	ПараметрыВыполненияЗадания.ЭтоЗапускВручную = Истина;
...
Процедура НастроитьРежимыОчисткиФайлов()
	...
	ИспользоватьТома = РаботаСФайламиВТомахСлужебный.ХранитьФайлыВТомах();
