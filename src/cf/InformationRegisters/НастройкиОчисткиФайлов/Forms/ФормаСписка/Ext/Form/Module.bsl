...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОписаниеРасчетаИтогов = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "РаботаСФайламиСлужебный.СведенияОбОчищаемыхФайлах");
	...
	НастроитьРежимОчисткиФайлов();
	ОтобразитьИзмененияРежимОчисткиФайлов(ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	Обработчик = Новый ОписаниеОповещения("ПриЗавершенииРасчетаИтогов", ЭтотОбъект);
...
Процедура ОбъемУдаленныхФайлов(Команда)
	ОткрытьФорму("Справочник.ТомаХраненияФайлов.Форма.ФормаСписка");
КонецПроцедуры
...
&НаСервере
Процедура НастроитьРежимОчисткиФайлов()
	Элементы.ОбъемУдаленныхФайлов.Видимость = ИспользоватьТома;
	Элементы.РежимОчисткиФайловОчищатьУдаленные.Видимость = ИспользоватьТома;
	Элементы.РежимОчисткиФайловОчищатьУдаленныеИНенужные.СписокВыбора[0].Представление = ?(ИспользоватьТома,
		НСтр("ru='Очищать удаленные и ненужные'"),
		НСтр("ru='Очищать ненужные'"));
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ОтобразитьИзмененияРежимОчисткиФайлов(Форма)
	
	Если Форма.РежимОчисткиФайлов = ПредопределенноеЗначение("Перечисление.РежимыОчисткиФайлов.НеОчищать") Тогда
		Заголовок = НСтр("ru='Автоматически не очищать файлы'");
		Форма.АвтоматическиОчищатьНенужныеФайлы = Ложь;
	ИначеЕсли Форма.РежимОчисткиФайлов = ПредопределенноеЗначение("Перечисление.РежимыОчисткиФайлов.ОчищатьУдаленные") Тогда
		Заголовок = НСтр("ru='Автоматически очищать удаленные файлы'");
		Форма.АвтоматическиОчищатьНенужныеФайлы = Истина;
	ИначеЕсли Форма.РежимОчисткиФайлов = ПредопределенноеЗначение("Перечисление.РежимыОчисткиФайлов.ОчищатьУдаленныеИНенужные") Тогда
		Заголовок = ?(Форма.ИспользоватьТома, 
						НСтр("ru='Автоматически очищать удаленные и ненужные файлы'"),
						НСтр("ru='Автоматически очищать ненужные файлы'"));
		Форма.АвтоматическиОчищатьНенужныеФайлы = Истина;
	Иначе
		ВызватьИсключение НСтр("ru='Неизвестный режим очистки файлов.'");
	КонецЕсли;
	
	Форма.Элементы.НастройкаОчисткиФайла.Заголовок = Заголовок;
	Форма.Элементы.Расписание.Доступность = Форма.АвтоматическиОчищатьНенужныеФайлы;
	Форма.Элементы.НастроитьРасписание.Доступность = Форма.АвтоматическиОчищатьНенужныеФайлы;
	Форма.Элементы.НастройкаОчисткиФайла.Скрыть();
КонецПроцедуры

&НаКлиенте
Процедура НадписьНастройкаОчисткиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Элементы.НастройкаОчисткиФайла.Показать();
КонецПроцедуры

&НаКлиенте
Процедура ПриЗавершенииРасчетаИтогов(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникла ошибка при подсчете объема удаляемых файлов:
			|%1'"),
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		РезультатЗадания = ДлительныеОперации.ВыполнитьВФоне(РегламентноеЗаданиеМетаданные.ИмяМетода, Новый Структура("РучнойЗапуск", Истина), ПараметрыЗадания);
		Если ЗначениеЗаполнено(ИдентификаторФоновогоЗадания) Тогда
...
Процедура ДобавитьНастройкуОчисткиФайлов()
	...
	Перем Настройка, ПараметрыФормыВыбора, СписокСуществующих, СтрокаДерева, СуществующиеНастройки, ТекстСообщения, ФиксированныеНастройки, ЭлементНастройки;
