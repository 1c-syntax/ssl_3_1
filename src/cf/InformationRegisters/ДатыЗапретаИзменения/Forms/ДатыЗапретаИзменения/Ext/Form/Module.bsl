...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если Не ЕстьПравоПросмотраЗапретаЗагрузкиДанных Тогда
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для просмотра дат запрета загрузки данных прошлых периодов из других приложений.'"),
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru = 'Даты запрета загрузки данных прошлых периодов из других приложений отключены в настройках.'");
		...
		Элементы.ПользователиПолноеПредставление.Заголовок =
			НСтр("ru = 'Приложение: информационная база'");
		
		Элементы.ПользователиКомментарий.Подсказка =
			НСтр("ru = 'Описывает причину отдельного запрета для информационной базы или приложения'");
	...
	Иначе
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru = 'Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках приложения.'");
		Элементы.ПользователиПолноеПредставление.Заголовок = 
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьДатыЗапретаЗагрузки") Тогда
			Если ЕстьПравоРедактированияЗапретаЗагрузкиДанных Тогда
				ПереходКДругимДатамЗапрета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Также можно настроить <a href=""%1"">даты запрета загрузки данных</a> из других приложений.'"),
			...
			ИначеЕсли ЕстьПравоПросмотраЗапретаЗагрузкиДанных Тогда
				ПереходКДругимДатамЗапрета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Также можно посмотреть <a href=""%1"">даты запрета загрузки данных</a> из других приложений.'"),
					"e1cib/command/РегистрСведений.ДатыЗапретаИзменения.Команда.ДатыЗапретаЗагрузкиДанных");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям" И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		Если ЕстьНедоступныеОбъектыПоПользователям Тогда
			...
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для изменения дат запрета.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
...
#Область ОбработчикиСобытийЭлементаПолноеПредставлениеТаблицыФормыПользователи
...
#Область ОбработчикиСобытийЭлементаКомментарийТаблицыФормыПользователи
...
#Область ОбработчикиСобытийЭлементаПолноеПредставлениеТаблицыФормыДатыЗапрета
...
#Область ОбработчикиСобытийЭлементаДатаЗапретаТаблицыФормыДатыЗапрета
...
Процедура УстановкаДатыЗапретаОбработкаВыбораПродолжение(Ответ, ВыбранноеЗначение) Экспорт
	...
	ОтключитьОбработчикОжидания("ОбновитьДанныеПользователяОбработчикОжидания");
...
Функция ПрочитатьДанныеПользователяСРазделами(Знач Пользователь,
	...
	Для Каждого Строка Из ПрочитанныеДатыЗапрета.Строки Цикл
		...
		КонецЦикла;
		Строка.Строки.Сортировать("ПолноеПредставление");
	КонецЦикла;
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		Если Не СправочникВнешниеПользователиДоступен Тогда
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для выбора внешних пользователей.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
...
Процедура ДатыЗапретаУстановитьДоступностьКоманд(Знач ДоступностьКоманд)
	...
	Если СпособУказанияДатыЗапрета = "ПоОбъектам" Тогда
		ДоступностьКоманд = Не Элементы.ДатыЗапрета.ТолькоПросмотр;
	КонецЕсли;
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета загрузки данных из других приложений действуют одинаково для всех пользователей.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru = 'Персональная настройка дат запрета загрузки данных прошлых периодов из других приложений для выбранных пользователей.'");
		КонецЕсли;
	Иначе
		Если УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
...
#Область ВспомогательныеФункцииСтрокИнтерфейсаПользователя
