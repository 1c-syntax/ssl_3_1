...
Процедура ПеренестиДвоичныеДанныеФайловВРегистрСведенийХранилищеФайлов(Выборка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(Выборка.Ссылка);
			// @skip-check query-in-loop - порционная обработка больших объемов данных.
			ЗаписатьДвоичныеДанные(Выборка.Ссылка, ДвоичныеДанные, ПараметрыЗаписиВИБ);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.НастройкиПрограммы") Тогда
		Методы.Вставить("ПеренестиДанные", Истина);
	Иначе
		Методы.Вставить("ПеренестиДанные");
	КонецЕсли;
