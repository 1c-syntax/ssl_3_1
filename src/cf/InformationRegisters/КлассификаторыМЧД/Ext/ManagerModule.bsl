...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.11.52";
		Обработчик.Процедура = "РегистрыСведений.КлассификаторыМЧД.ОчиститьКэшЗагруженныхФайлов";
		Обработчик.РежимВыполнения = "Оперативно";
	КонецЕсли;
...
Функция КлассификаторПолномочий() Экспорт
	...
	Для Каждого Элемент Из ДанныеКлассификатора["elements"] Цикл
		...
		Если СтрНачинаетсяС(КодПолномочия, "1_FNS") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОчиститьКэшЗагруженныхФайлов() Экспорт
	
	Если ДатаИзмененияФайлаКлассификатораПолномочий() < ДатаОбновленияКлассификатораПолномочийВМетаданных() Тогда
		НаборЗаписей = РегистрыСведений.КлассификаторыМЧД.СоздатьНаборЗаписей();
		НаборЗаписей.Записать();
	КонецЕсли;
...
Функция ДатаОбновленияКлассификатораПолномочийВМетаданных()
	
	ТекстовыйДокумент = ПолучитьМакет("Полномочия");
	КлассификаторJSON = ТекстовыйДокумент.ПолучитьТекст();
	ДанныеКлассификатора = ОбщегоНазначения.JSONВЗначение(КлассификаторJSON);
	
	Возврат Дата(ДанныеКлассификатора["lastChangeDate"]);
	
КонецФункции

Функция ДатаИзмененияФайла(ИдентификаторФайла)
	
	ИдентификаторыФайлов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторФайла);
	ДатыИзмененияФайлов = ДатыИзмененияФайлов(ИдентификаторыФайлов);
	
	Возврат ДатыИзмененияФайлов[ИдентификаторФайла];
	
КонецФункции

Функция ДатаИзмененияФайлаКлассификатораПолномочий()
	
	Возврат ДатаИзмененияФайла(ИдентификаторКлассификатораПолномочий());
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчиститьКэшЗагруженныхФайлов");
