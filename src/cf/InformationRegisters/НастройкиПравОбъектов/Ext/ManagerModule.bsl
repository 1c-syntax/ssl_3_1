...
Процедура ОбновитьВспомогательныеДанныеРегистраПоИзменениямКонфигурации() Экспорт
	...
	Если УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
		СтандартныеПодсистемыСервер.УдалитьПараметрРаботыРасширения(ИмяПараметра, Неопределено);
		Возврат;
	КонецЕсли;
...
//                                                 = Булево - аналогично.
//
//  Неопределено - если ограничение доступа на уровне записей отключено в переопределяемом модуле.
//
...
Процедура ОбновитьВспомогательныеДанныеРегистра(ЕстьИзменения = Неопределено) Экспорт
	...
	Если УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
		УправлениеДоступомСлужебный.ОчиститьРегистрСведений(
			Метаданные.РегистрыСведений.НастройкиПравОбъектов.ПолноеИмя(), ЕстьИзменения);
		Возврат;
	КонецЕсли;
...
Функция ВозможныеПраваДляНастройкиПравОбъектов() Экспорт
	...
	Если Кэш.Проверка.Дата + 3 > ТекущаяДатаСеанса
...
Функция ПроверенныеВозможныеПраваСеанса(СвойстваВидовДоступа = Неопределено, ХешСумма = "") Экспорт
	...
	Для Каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Несуществующий владелец прав ""%1"".'"),
	...
	Для Каждого ЭлементСписка Из ТипыВладельцев Цикл
		...
		ПоТипам.Вставить(СвойстваВладельца.ТипСсылки,  ПраваВладельца);
		ПоТипам.Вставить(СвойстваВладельца.ТипОбъекта, ПраваВладельца);
