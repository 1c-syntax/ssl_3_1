...
Процедура ПрограммыПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элементы.Найти("ПрограммыИзменить") <> Неопределено
	   И Элементы.ПрограммыИзменить.Видимость Тогда
		...
		РасширенноеОписаниеПрограммы.РежимИспользования = ПредопределенноеЗначение("Перечисление.РежимыИспользованияПрограммыЭлектроннойПодписи.Настроена");
...
Процедура ПрограммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПрограммыПодробнее
		...
		Если ТекущиеДанные.ЭтоТокен И СтрНайти(ТекущиеДанные.РезультатПроверки, ЭлектроннаяПодписьСлужебныйКлиент.ОшибкаНеНайденТокен()) <> 0 Тогда
			ПараметрыФормы.Вставить("ТекстОшибкиКлиент", ТекущиеДанные.РезультатПроверки + Символы.ПС + ЭлектроннаяПодписьСлужебныйКлиент.РекомендацииДляТокена());
		Иначе
...
Процедура ПослеПроверкиПрограммКриптографии(Результат, Оповещение) Экспорт
	...
	Если Результат.ПроверкаВыполнена Тогда
		...
		Для Каждого Токен Из Результат.Токены Цикл
			...
			НоваяСтрока.Подробнее = ?(ЗначениеЗаполнено(Токен.Ошибка), НСтр("ru = 'Подробнее'") + "...", "");
		...
		Если Не ЕстьПроверяемыеПрограммы Тогда
			Элементы.ГруппаПодсказкаКриптопровайдеры.Видимость = Истина;
			Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = 'Если планируется работа с усиленной квалифицированной электронной подписью, то на компьютере должно быть установлено сертифицированное приложение (средство криптографической защиты информации).
