...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПутиКПрограммамПоУмолчанию = Новый Структура;
	РаботаСПрограммамиНаСервере = ПроверятьПодписиНаСервере Или ПодписыватьНаСервере;
	Если ОбщегоНазначения.ЭтоWindowsКлиент()
		И (Не РаботаСПрограммамиНаСервере
		Или ОбщегоНазначения.ЭтоWindowsСервер()) Тогда
		
		Элементы.ГруппаПрограммыLinuxПутьКПрограмме.Видимость = Ложь;
	Иначе
		
		Если ОбщегоНазначения.ЭтоMacOSКлиент() Тогда
	
			ПутьПоУмолчанию = "/opt/cprocsp/lib/libcapi10.dylib:/opt/cprocsp/lib/libcapi20.dylib:"
				+ "/opt/cprocsp/lib/libcapi10.4.dylib:/opt/cprocsp/lib/libcapi20.4.dylib";
			
			ПутиКПрограммамПоУмолчанию.Вставить("ПутьMac", ПутьПоУмолчанию);
			Элементы.ПрограммыLinuxПутьКПрограмме.СписокВыбора.Добавить(
				"ПутьMac", НСтр("ru = 'Путь для Mac OS'") + ": " + ПутьПоУмолчанию);
			
		КонецЕсли;
		
		Если ОбщегоНазначения.ЭтоLinuxКлиент()
			Или (РаботаСПрограммамиНаСервере
			И ОбщегоНазначения.ЭтоLinuxСервер()) Тогда
		
			ПутьПоУмолчанию = "/opt/cprocsp/lib/amd64/libcapilite.so:"
				+ "/opt/cprocsp/lib/amd64/libcapi10.so:/opt/cprocsp/lib/amd64/libcapi20.so";
			
			ПутиКПрограммамПоУмолчанию.Вставить("ПутьLinux", ПутьПоУмолчанию);
			Элементы.ПрограммыLinuxПутьКПрограмме.СписокВыбора.Добавить(
				"ПутьLinux", НСтр("ru = 'Путь для Linux'") + ": " + ПутьПоУмолчанию);
...
Процедура ПрограммыLinuxПутьКПрограммеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ТекущиеДанные.LinuxПутьКПрограмме = ПутиКПрограммамПоУмолчанию[ВыбранноеЗначение];
	СохранитьПутьКПрограмме();
...
Процедура ПрограммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПрограммыПодробнее
		...
		ПараметрыОшибки.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Результат проверки программы'"));
		ПараметрыОшибки.Вставить("ТекстОшибкиКлиент", ТекущиеДанные.РезультатПроверки);
...
Процедура ПеревыпуститьСертификат(Команда)
	...
	ПараметрыСоздания = Новый Структура;
	ПараметрыСоздания.Вставить("СоздатьЗаявление", Истина);
	ПараметрыСоздания.Вставить("СертификатОснование", Элементы.Сертификаты.ТекущиеДанные.Ссылка);
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Ошибки            = Контекст.ПараметрыВыполнения.ОписаниеОшибок.Ошибки; // см. ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваОшибки
