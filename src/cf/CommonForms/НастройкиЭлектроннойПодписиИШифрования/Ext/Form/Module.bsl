...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СертификатыДоступны = ПравоДоступа("Чтение", Метаданные.Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования);
	
	Элементы.ДекорацияОткрытьМашиночитаемыеДоверенности.Видимость = Ложь;
	...
	Если Параметры.ПоказатьСтраницу = "Сертификаты" И СертификатыДоступны Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСертификаты;
	ИначеЕсли Параметры.ПоказатьСтраницу = "Настройки" Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНастройки;
	ИначеЕсли Параметры.ПоказатьСтраницу = "Программы" Тогда
	...
	Если Не ПравоДоступа("Изменение", Метаданные.Справочники.ПрограммыЭлектроннойПодписиИШифрования) Тогда
		...
		Элементы.Программы.Заголовок =
			НСтр("ru = 'Приложения, предусмотренные администратором, которые можно использовать на компьютере'");
	Иначе
		УстановитьПривилегированныйРежим(Истина);
		ЭтоМногопользовательскаяБаза = ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() > 1;
		УстановитьПривилегированныйРежим(Ложь);
		Если Не ЭтоМногопользовательскаяБаза Тогда
			Элементы.Программы.Заголовок = НСтр("ru = 'Настройки приложений электронной подписи и шифрования, установленных на компьютере'");
		КонецЕсли;
	КонецЕсли;
	
	// Страница Сертификаты
	Если СертификатыДоступны Тогда
		...
		ЕстьПравоНаДобавлениеСертификатов = ЭлектроннаяПодписьСлужебный.ЕстьПравоНаДобавлениеСертификатовВСправочник();
	...
	Элементы.ФормаВопросВПоддержку.Видимость = 
	...
	Если ЭлектроннаяПодпись.ИспользоватьСервисМобильнойПодписи() Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ПодписантыСервисаМобильнойПодписи", "Видимость", Истина);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		ПараметрыРазмещения.КоманднаяПанель = Элементы.СертификатыКоманднаяПанельСписка;
		ПараметрыРазмещения.Источники = Новый ОписаниеТипов("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования");
		ПараметрыРазмещения.ВладелецКоманд = Элементы.Сертификаты;
...
Процедура ПриПовторномОткрытии()
	...
	ОпределитьУстановленныеПрограммы();
	Если СертификатыДоступны И ЗначениеЗаполнено(Параметры.СертификатыПоказать) Тогда
		СертификатыПоказать = Параметры.СертификатыПоказать;
		СертификатыОбновитьОтбор(ЭтотОбъект, СостоянияЗаявленияНеВРаботе);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("Запись_СертификатыКлючейЭлектроннойПодписиИШифрования") И СертификатыДоступны Тогда
	...
	СобытияИзмененияНастроекПрограмм = Новый Массив;
	СобытияИзмененияНастроекПрограмм.Добавить(ВРег("Запись_ПрограммыЭлектроннойПодписиИШифрования"));
	СобытияИзмененияНастроекПрограмм.Добавить(ВРег("Запись_ПутиКПрограммамЭлектроннойПодписиИШифрованияНаСерверахLinux"));
	СобытияИзмененияНастроекПрограмм.Добавить(ВРег("Запись_ЛичныеНастройкиЭлектроннойПодписиИШифрования"));
	
	ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииСобытийИзмененияНастроекПрограмм(СобытияИзмененияНастроекПрограмм);
			
	// При изменении состава или настроек программ.
	Если СобытияИзмененияНастроекПрограмм.Найти(ВРег(ИмяСобытия)) <> Неопределено Тогда
	...
	Если ВРег(Источник) = ВРег("ИспользоватьЭлектронныеПодписи")
		...
		ПодключитьОбработчикОжидания("ПриИзмененииИспользованияПодписанияИлиШифрования", 0.1, Истина);
	КонецЕсли;
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНастройки Тогда
		...
		ПутьКЛогуКомпонентыНаКлиенте = ПерсональныеНастройки.ПутьККаталогуДиагностики.Получить(ИдентификаторКлиента);
...
Процедура ДекорацияПроверкаУстановкиКриптопровайдераОбработкаНавигационнойСсылки(
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеУстановитьБиблиотекиДляТокенов() Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуУстановкиПрограммКриптопровайдеров(ПараметрыОткрытия, ЭтотОбъект,
				Новый ОписаниеОповещения("ПослеВыполненияДействийПодробнее", ЭтотОбъект));
	Иначе
		...
		ОповещениеПослеПроверкиПрограмм = Новый ОписаниеОповещения("ОпределитьУстановленныеПрограммыПослеПроверкиПрограммКриптографии",
			ЭтотОбъект);
...
Процедура ДекорацияОткрытьМашиночитаемыеДоверенностиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	МодульМашиночитаемыеДоверенностиФНССлужебныйКлиент.ОткрытьСписокМЧД(НавигационнаяСсылкаФорматированнойСтроки);
...
&НаКлиенте
Процедура ПутьКЛогуКомпонентыНаКлиентеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	Оповещение = Новый ОписаниеОповещения("ВыбратьКаталогПослеВыбора",
		ЭтотОбъект);
	ФайловаяСистемаКлиент.ВыбратьКаталог(Оповещение, НСтр("ru = 'Выберите каталог файлов диагностики'"), ПутьКЛогуКомпонентыНаКлиенте);
	
КонецПроцедуры

&НаКлиенте
Процедура ПутьКЛогуКомпонентыНаКлиентеПриИзменении(Элемент)
...
Процедура СертификатыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого СтрокаСписка Из Строки Цикл
		Если НЕ СтрокаСписка.Значение.Данные.Свойство("Пользователь") Тогда
			Возврат;
		КонецЕсли;
		Прервать;
	КонецЦикла;
	...
	Запрос = Неопределено;
	Для Каждого СтрокаСписка Из Строки Цикл
		Если ЗначениеЗаполнено(СтрокаСписка.Значение.Данные.Пользователь) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ВыборкаСсылка.НайтиСледующий(СтрокаСписка.Значение.Данные["Ссылка"], "Ссылка") Тогда
			...
			Если КоличествоПользователей = 1 Тогда
				СтрокаСписка.Значение.Данные.Пользователь = ВыборкаПользователь.ПользовательПредставление;
				Продолжить;
			КонецЕсли;
			Пользователь1 = ВыборкаПользователь.ПользовательПредставление;
			ВыборкаПользователь.Следующий();
			Пользователь2 = ВыборкаПользователь.ПользовательПредставление;
			СтрокаСписка.Значение.Данные.Пользователь = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПользователиСертификатаСтрокой(
...
Процедура ПрограммыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Элементы.Программы.ИзменятьСоставСтрок Тогда
		...
		ЭлектроннаяПодписьКлиентЛокализация.ПриДобавленииПриложенияЭлектроннойПодписи(СтандартнаяОбработка);
...
Процедура ПрограммыПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элементы.Найти("ПрограммыИзменить") <> Неопределено
	   И Элементы.ПрограммыИзменить.Видимость
	   И ЗначениеЗаполнено(Элементы.Программы.ТекущиеДанные.Ссылка) Тогда
		...
		РасширенноеОписаниеПрограммы.РежимИспользования = ПредопределенноеЗначение("Перечисление.РежимыИспользованияПрограммыЭлектроннойПодписи.Настроено");
		ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы(РасширенноеОписаниеПрограммы, ЭтотОбъект);
	КонецЕсли;
...
Процедура ПрограммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПрограммыПодробнее
		...
		Если ТекущиеДанные.Действие = "УстановитьБиблиотекиДляТокенов" Тогда
			ПараметрыОткрытия = Новый Структура("ВидТокена, ТолькоУстановкаТокенов");
			ЗаполнитьЗначенияСвойств(ПараметрыОткрытия, ТекущиеДанные.Параметры);
			ПараметрыОткрытия.ТолькоУстановкаТокенов = Истина;
			ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуУстановкиПрограммКриптопровайдеров(ПараметрыОткрытия, ЭтотОбъект,
				Новый ОписаниеОповещения("ПослеВыполненияДействийПодробнее", ЭтотОбъект));
		Иначе

			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Результат проверки приложения'"));
			Если ТекущиеДанные.ЭтоТокен И СтрНайти(ТекущиеДанные.РезультатПроверки,
				ЭлектроннаяПодписьСлужебныйКлиент.ОшибкаНеНайденТокен()) <> 0 Тогда
				ПараметрыФормы.Вставить("ТекстОшибкиКлиент", ТекущиеДанные.РезультатПроверки + Символы.ПС
					+ ЭлектроннаяПодписьСлужебныйКлиент.РекомендацииДляТокена());
			Иначе
	...
	ИначеЕсли Не ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		...
		Если ТекущиеДанные.ЭтоТокен Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.Слот) Тогда
				ОписаниеТокена = ЭлектроннаяПодписьСлужебныйКлиент.НовыеСвойстваТокена();
				ОписаниеТокена.Представление = ТекущиеДанные.Наименование;
				ОписаниеТокена.СерийныйНомер = ТекущиеДанные.ИмяПрограммы;
				ОписаниеТокена.Слот = ТекущиеДанные.Слот;
		...
		Иначе
			...
			ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы(ОписаниеПрограммы, ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьДляПодписанияИШифрованияИзФайлов(Команда)
	...
	ЭлектроннаяПодписьКлиент.УстановитьРасширение(Ложь, 
		Новый ОписаниеОповещения("ДобавитьДляПодписанияИШифрованияИзФайловПослеУстановкиРасширения", ЭтотОбъект));
...
Процедура ВопросВПоддержку(Команда)
	...
	ПараметрыСообщения = Новый Структура("Тема, Сообщение",
		НСтр("ru = 'Проблема при настройке электронной подписи и шифрования'"), Новый Массив);
	
	ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(НСтр("ru = 'Настройки электронной подписи и шифрования'"), ПараметрыСообщения);
...
Процедура ИнформацияДляОтправкиВПоддержку(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(НСтр("ru = 'Настройки электронной подписи и шифрования'"));
...
&НаКлиенте
Процедура ДобавитьДляПодписанияИШифрованияИзФайловПослеУстановкиРасширения(Результат, Контекст) Экспорт
	...
	ПараметрыСоздания = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыДобавленияСертификата();
	ЭлектроннаяПодписьСлужебныйКлиент.ДобавитьСертификатПослеВыбораНазначения("ДляПодписанияШифрованияИРасшифровкиИзФайлов",
		ПараметрыСоздания);
		
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьКаталогПослеВыбора(ВыбранныйКаталог, Контекст) Экспорт
	
	Если Не ЗначениеЗаполнено(ВыбранныйКаталог) Тогда
		ПутьКЛогуКомпонентыНаКлиенте = "";
	Иначе
		ПутьКЛогуКомпонентыНаКлиенте = ВыбранныйКаталог;
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеВыполненияДействийПодробнее(Результат, Контекст) Экспорт
	ЭлектроннаяПодписьСлужебныйКлиент.ОчиститьКэшУстановленныхКриптопровайдеров();
	ОпределитьУстановленныеПрограммы();
КонецПроцедуры
...
Процедура ДобавитьСертификатыВЛичномХранилищеВПараметрДинамическогоСписка()
	...
	Элементы.ГруппаОбновлениеСпискаСертификатов.Видимость = Истина;
...
Процедура ОбновитьСписокСертификатовПослеПолученияСвойствСертификатовНаКлиенте(Результат, Параметры) Экспорт
	...
	Элементы.ГруппаОбновлениеСпискаСертификатов.Видимость = Ложь;
...
Процедура ИзменитьПометкуУдаленияПрограммы(Программа)
	...
	СтандартнаяОбработка = Истина;
	ЭлектроннаяПодписьЛокализация.ПриИзмененииПометкиУдаленияПриложения(Программа, УникальныйИдентификатор, СтандартнаяОбработка);
	...
	Попытка
		...
		Иначе
			Объект.РежимИспользования = Перечисления.РежимыИспользованияПрограммыЭлектроннойПодписи.Настроено;
		КонецЕсли;
		Объект.Записать();
...
Процедура ОбновитьТекущуюВидимостьЭлементов()
	...
	// Страница Сертификаты
	Элементы.СтраницаСертификаты.Видимость = СертификатыДоступны;
	Если СертификатыДоступны Тогда
		...
		Элементы.ДобавитьДляПодписанияИШифрования.Заголовок = СвойстваКомандыДобавленияСертификатов.Заголовок;
		Элементы.ДобавитьДляПодписанияИШифрования.РасширеннаяПодсказка.Заголовок = СвойстваКомандыДобавленияСертификатов.ЗаголовокРасширеннойПодсказки;
		...
		Элементы.ДобавитьДляПодписанияИШифрованияИзФайлов.Заголовок = СвойстваКомандыДобавленияСертификатов.Заголовок;
		Элементы.ДобавитьДляПодписанияИШифрованияИзФайлов.РасширеннаяПодсказка.Заголовок = СвойстваКомандыДобавленияСертификатов.ЗаголовокРасширеннойПодсказки;
	КонецЕсли;
...
Процедура ОпределитьУстановленныеПрограммыПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Если Подключено Тогда
		Элементы.ГруппаПрограммыОбновление.Видимость = Истина;
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыПослеПодключенияРасширения(Подключено, Контекст) Экспорт
...
#Если ВебКлиент Тогда
		...
		ТребуетсяУстановкаДополнительныхКомпонент =
			Ждать ЭлектроннаяПодписьСлужебныйКлиент.ТребуетсяУстановкаДополнительныхКомпонент(Компоненты);
		...
		Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
			...
			ОбработчикПродолжения = Новый ОписаниеОповещения("ОпределитьУстановленныеПрограммыПослеПодключенияРасширения", ЭтотОбъект, Контекст);
			ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы, ,
				УникальныйИдентификатор, , , ОбработчикПродолжения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			Возврат;
		КонецЕсли;
#Иначе
		ПоказатьЗаголовокДляУстановкиРасширения();
...
&НаКлиенте
Процедура ПоказатьЗаголовокДляУстановкиРасширения()
	...
	Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Нажмите здесь</a> для установки расширения для работы с 1С:Предприятием, чтобы работать с электронной подписью.'"),
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст)
	...
	КоличествоПрограмм = Программы.Количество();
	
	Если КоличествоПрограмм <= Контекст.Индекс + 1 Тогда
		// После цикла.
		Элементы.ГруппаПрограммыОбновление.Видимость = Ложь;
	...
	ЭлектроннаяПодписьКлиентСерверЛокализация.ПриОпределенииДоступностиПриложения(ОписаниеПрограммы, Структура);
	Пока ЗначениеЗаполнено(Структура) И КоличествоПрограмм >= Контекст.Индекс + 1 Цикл
		Для Каждого КлючИЗначение Из Структура Цикл
			ОбновитьЗначение(ОписаниеПрограммы[КлючИЗначение.Ключ], КлючИЗначение.Значение);
		КонецЦикла;
	...
	Если ОписаниеПрограммы.ТипРазмещения <> 1 Или АвтоматическиеНастройкиКриптопровайдера(ОписаниеПрограммы) Тогда
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПослеПолученияПутиКПрограмме(ОписаниеПути, Контекст) Экспорт
	...
	Если ОписаниеПрограммы.ПометкаУдаления 
		Или ЭлектроннаяПодписьСлужебныйКлиентСервер.ПрограммаНеИспользуется(ОписаниеПрограммы.РежимИспользования) Тогда
	...
	ИначеЕсли ОписаниеПрограммы.ЭтоВстроенныйКриптопровайдер Тогда
		...
		ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст);
		Возврат;
	ИначеЕсли ОписаниеПрограммы.ЭтоТокен Тогда
		ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст);
		Возврат;
	ИначеЕсли АвтоматическиеНастройкиКриптопровайдера(ОписаниеПрограммы) Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ru = 'Установлено на компьютере.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, Истина);
		ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		...
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
		ТекстОшибки = НСтр("ru = 'Не установлено на компьютере.'") + " " + Ошибка.Описание;
...
Процедура ОбновитьLinuxПутьКПрограмме()
	...
	ПутьПоУмолчанию = "";
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТипЗнч(ТекущиеДанные.Ссылка) <> Тип("СправочникСсылка.ПрограммыЭлектроннойПодписиИШифрования") Тогда
			Элементы.ПрограммыLinuxПутьКПрограмме.Видимость = Ложь;
			Возврат;
		КонецЕсли;
		
		Элементы.ПрограммыLinuxПутьКПрограмме.Видимость = Истина;
		
		Если АвтоматическиеНастройкиКриптопровайдера(ТекущиеДанные) Тогда
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		СтандартнаяОбработка = Истина;
		ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииТокенов(
			Новый ОписаниеОповещения("ПослеПолученияТокеновВМобильномКлиенте", ЭтотОбъект, Оповещение), Ложь, СтандартнаяОбработка);
		Если Не СтандартнаяОбработка Тогда
			Возврат;
		КонецЕсли;
#Иначе
		Элементы.ГруппаПодсказкаКриптопровайдеры.Видимость = Истина;
		Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Нажмите здесь</a>, чтобы увидеть все приложения для работы с электронной подписью, установленные на компьютере.'"),
			"ПроверитьУстановкуПрограммКриптографии");
#КонецЕсли
	ИначеЕсли Результат.ВозможенКонфликт Тогда
		ЭлектроннаяПодписьКлиентЛокализация.ПриПроверкеКонфликтаПрограммКриптографии(ЭтотОбъект, Результат);
	Иначе
	...
	Если Результат.ПроверкаВыполнена Тогда
		...
		Для Каждого Криптопровайдер Из Результат.Программы Цикл
			...
			Если Найдено.Количество() = 0
				Или Найдено[0].Автоопределение = Ложь
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Криптопровайдер,, "Ссылка, ПутьКПрограммеНаСервереАвто, РежимИспользования");
				НоваяСтрока.ТипРазмещения = 1;
				
				Если Не ЗначениеЗаполнено(НоваяСтрока.Ссылка)
					Или ЭлектроннаяПодписьСлужебныйКлиентСервер.ИспользуютсяАвтоматическиеНастройки(НоваяСтрока.РежимИспользования) Тогда
		...
		КонецЦикла;
		ЗаполнитьСписокТокенов(Результат.Токены);
		ЭлектроннаяПодписьКлиентЛокализация.ПриПроверкеУстановленныхПрограммКриптографии(ЭтотОбъект, Результат, ЕстьПроверяемыеПрограммы);
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеПолученияТокеновВМобильномКлиенте(Токены, Оповещение) Экспорт
	Если ЗначениеЗаполнено(Токены) Тогда
		ЗаполнитьСписокТокенов(Токены);
	КонецЕсли;
	Если Оповещение <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(Оповещение);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьСписокТокенов(Токены)
	Для Каждого Токен Из Токены Цикл
		
		Найдено = Программы.НайтиСтроки(Новый Структура("Наименование",Токен.Представление));
		...
		НоваяСтрока.Автоопределение = Истина;
		НоваяСтрока.РежимИспользованияКартинка = -1;
		НоваяСтрока.Наименование = Токен.Представление;
		НоваяСтрока.ЭтоТокен = Истина;
		НоваяСтрока.ИмяПрограммы = Токен.СерийныйНомер;
		НоваяСтрока.Слот = Токен.Слот;
		НоваяСтрока.Действие = "";
		НоваяСтрока.Параметры = Неопределено;
		НоваяСтрока.Подробнее = "";
		НоваяСтрока.РезультатПроверки = ?(ЗначениеЗаполнено(Токен.Ошибка), Токен.Ошибка, НСтр("ru = 'Доступен.'"));
		
		Если ЗначениеЗаполнено(Токен.Ошибка) Тогда
			Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ОшибкаЗагрузкиБиблиотекиТокена(Токен.Ошибка) Тогда
				НоваяСтрока.Подробнее = НСтр("ru = 'Установить'") + "...";
				НоваяСтрока.Действие = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеУстановитьБиблиотекиДляТокенов();
				НоваяСтрока.Параметры = Новый Структура("ВидТокена", Токен.Токен);
			Иначе
				НоваяСтрока.Подробнее = НСтр("ru = 'Подробнее'") + "...";
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьПрограммыИНастройки(ОбновитьПовтИсп = Ложь)
	...
	|	Программы.ЭтоВстроенныйКриптопровайдер,
	|	ЗНАЧЕНИЕ(Перечисление.РежимыИспользованияПрограммыЭлектроннойПодписи.Настроено),
	|	4
	|ИЗ
	|	Справочник.ПрограммыЭлектроннойПодписиИШифрования КАК Программы
	|ГДЕ
	...
	ЭлектроннаяПодписьЛокализация.ПриЗаполненииПриложенийЭлектроннойПодписи(ТаблицаВыборки);
	...
	ТаблицаВыборки.Колонки.Добавить("АвтоопределениеНаСервере", Новый ОписаниеТипов("Булево"));
		...
		Если РезультатКриптопровайдеры.ПроверкаВыполнена Тогда
			...
			Для Каждого Криптопровайдер Из РезультатКриптопровайдеры.Программы Цикл
				...
				Если Найдено.Количество() = 0
					...
					КонецЕсли;
					ЗаполнитьЗначенияСвойств(СтрокаВыборки, Криптопровайдер,,"Ссылка, РежимИспользования");
					...
					СтрокаВыборки.АвтоопределениеНаСервере = Истина;
				КонецЕсли;
	...
	Для каждого СтрокаВыборки Из ТаблицаВыборки Цикл
		...
		ОбновитьЗначение(Строка.АвтоопределениеНаСервере,     СтрокаВыборки.АвтоопределениеНаСервере);
		...
		Если ТипЗнч(Строка.Ссылка) = Тип("СправочникСсылка.ПрограммыЭлектроннойПодписиИШифрования")
			И ЭлектроннаяПодписьСлужебныйКлиентСервер.ИспользуютсяАвтоматическиеНастройки(СтрокаВыборки.РежимИспользования)
			Или Не ЗначениеЗаполнено(Строка.Ссылка) Тогда
			ОбновитьЗначение(Строка.РежимИспользованияКартинка, -1);
		Иначе
			ОбновитьЗначение(Строка.РежимИспользованияКартинка, 0);
		КонецЕсли;
		
		Если СтрокаВыборки.АвтоопределениеНаСервере Тогда // установленный криптопровайдер
		...
		Структура = Новый Структура;
		ЭлектроннаяПодписьКлиентСерверЛокализация.ПриОпределенииДоступностиПриложения(СтрокаВыборки, Структура);
		Если ЗначениеЗаполнено(Структура) Тогда
			Для Каждого КлючИЗначение Из Структура Цикл
				ОбновитьЗначение(Строка[КлючИЗначение.Ключ], КлючИЗначение.Значение);
			КонецЦикла;
		ИначеЕсли Строка.ЭтоВстроенныйКриптопровайдер И Не Строка.ПометкаУдаления Тогда
			ОбновитьЗначение(Строка.РезультатПроверки, НСтр("ru = 'Доступен.'"));
			ОбновитьЗначение(Строка.Установлена, Истина);
		ИначеЕсли (Настройки.ПроверятьЭлектронныеПодписиНаСервере Или Настройки.СоздаватьЭлектронныеПодписиНаСервере) 
			И  АвтоматическиеНастройкиКриптопровайдера(Строка, Истина) Тогда
			ОбновитьЗначение(Строка.РезультатПроверкиНаСервере, НСтр("ru = 'Установлено на сервере.'"));
			ОбновитьЗначение(Строка.УстановленаНаСервере, Истина);
		ИначеЕсли Строка.ТипРазмещения = 1 И (Настройки.ПроверятьЭлектронныеПодписиНаСервере Или Настройки.СоздаватьЭлектронныеПодписиНаСервере) 
			И Не ПрограммаНеИспользуется Тогда
...
Процедура СохранитьНастройки()
	...
	СохраняемыеНастройки.Вставить("ПутьКЛогуКомпонентыНаКлиенте", ПутьКЛогуКомпонентыНаКлиенте);
	
	СохранитьНастройкиНаСервере(СохраняемыеНастройки, ИдентификаторКлиента);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СохранитьНастройкиНаСервере(СохраняемыеНастройки, ИдентификаторКлиента)
	
	ПерсональныеНастройки = ЭлектроннаяПодпись.ПерсональныеНастройки();
	ПутьККаталогуДиагностики = ПерсональныеНастройки.ПутьККаталогуДиагностики;
	ЗаполнитьЗначенияСвойств(ПерсональныеНастройки, СохраняемыеНастройки);
	ПерсональныеНастройки.ПутьККаталогуДиагностики.Вставить(ИдентификаторКлиента,
		СохраняемыеНастройки.ПутьКЛогуКомпонентыНаКлиенте);
...
Процедура СохранитьПутьКПрограмме()
	...
	Если БезПраваСохранениеДанныхПользователя Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'Невозможно сохранить путь к приложению. Отсутствует право сохранения данных, обратитесь к администратору.'"));
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Функция АвтоматическиеНастройкиКриптопровайдера(ОписаниеПрограммы, Серверная = Ложь)
	Автоопределение = ?(Серверная, ОписаниеПрограммы.АвтоопределениеНаСервере, ОписаниеПрограммы.Автоопределение);
	
	Возврат ОписаниеПрограммы.ТипРазмещения = 1 И Автоопределение И (Не ЗначениеЗаполнено(ОписаниеПрограммы.Ссылка) 
		Или ТипЗнч(ОписаниеПрограммы.Ссылка) <> Тип("СправочникСсылка.ПрограммыЭлектроннойПодписиИШифрования")
		Или ЭлектроннаяПодписьСлужебныйКлиентСервер.ИспользуютсяАвтоматическиеНастройки(ОписаниеПрограммы.РежимИспользования));
