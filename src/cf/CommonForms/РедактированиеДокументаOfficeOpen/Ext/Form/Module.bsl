...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.КнопкаПоказатьОригиналВсеДействия.Видимость = Элементы.КнопкаПоказатьОригинал.Видимость;
	Элементы.КнопкаПоказатьОригиналВсеДействия.Доступность = Элементы.КнопкаПоказатьОригинал.Доступность;
	...
	КонструкторФормул.ДобавитьСписокПолейНаФорму(ЭтотОбъект, ПараметрыДобавления);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Если Модифицированность И Не ОтказСохраненияИзменений Тогда
			ТекстПредупреждения = НСтр("ru = 'Макет печатной формы отредактирован в другом приложении. 
				|Нажмите Записать, чтобы не потерять изменения.'");
			Отказ = Истина;
		КонецЕсли;
		Возврат;
	КонецЕсли;

	Если Не ОтказСохраненияИзменений Тогда
	...
	Если Не Отказ И ЗначениеЗаполнено(КлючОбъектаРедактирования) Тогда
...
Процедура СохранитьВФайлПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru = 'Сохранение макета в файл'");
	ПараметрыСохранения.Диалог.Фильтр = НСтр("ru = 'Офисный документ'") + " (*.docx)|*.docx";
	ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, АдресМакетаВоВременномХранилище, ИмяДокумента, ПараметрыСохранения);
...
Функция СодержитПараметры(АдресМакета)
	...
	Если ЗначениеЗаполнено(СтруктураДокумента.ПараметрыТекста) Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ОповеститьОЗаписиОфисногоДокумента()
	...
	ПараметрыОповещения.ПутьКФайлу = ПутьКФайлуМакета;
...
Процедура ПередЗакрытиемОкончание(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПовторноеЗакрытие <> Истина Тогда
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохранить изменения в %1%2?'"), 
		ИмяДокумента, ОписаниеЯзыка);
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(ОписаниеОповещения, Отказ, Ложь, ТекстВопроса);
...
Процедура ОткрытьМакет(ПараметрыОткрытия)
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(ПослеПодключенияРасширения);
	...
	Элементы.ОтобразитьИнструкцию.Пометка = Ложь;
КонецПроцедуры
...
Процедура ПриОтветеНаВопросОПереводеМакета(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность.ПереводТекста") Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриЗавершенииПроверкиНастроекПеревода", ЭтотОбъект, ПараметрыОткрытияМакета);
		МодульПереводТекстаНаДругиеЯзыкиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПереводТекстаНаДругиеЯзыкиКлиент");
		МодульПереводТекстаНаДругиеЯзыкиКлиент.ПроверитьНастройки(ЭтотОбъект, ОписаниеОповещения);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПриЗавершенииПроверкиНастроекПеревода(НастройкаВыполнена, ПараметрыОткрытияМакета) Экспорт
...
Процедура РасширитьСписокПолей()
	...
	ИмяКолонкиПредставление = КонструкторФормулСлужебный.ИмяКолонкиПредставление(ИмяСпискаПолей());
...
Процедура Подключаемый_ДоступныеПоляПередНачаломИзменения(Элемент, Отказ)
	...
	ВыбранноеПоле = КонструкторФормулКлиент.ВыбранноеПолеВСпискеПолей(ЭтотОбъект, ИмяСпискаПолей());
...
Процедура Подключаемый_СписокПолейВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = Элемент.Имя + "Представление" Тогда
		...
		Иначе
			ТекущееЗначение = ТекущееЗначение + КонструкторФормулКлиент.ВыражениеДляВставки(ВыбранноеПоле);
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_СписокПолейАктивизацияСтроки(Элемент)
	...
	УстановитьВHTMLПоле(КонструкторФормулКлиент.ВыражениеДляВставки(Оператор));
...
Процедура Подключаемый_ОператорыНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	ПараметрыПеретаскивания.Значение = КонструкторФормулКлиент.ВыражениеДляВставки(Оператор);
...
Процедура НастроитьПредставлениеКоманд()
	...
	Элементы.ПеревестиВсеДействия.Доступность = Элементы.Перевести.Доступность;
