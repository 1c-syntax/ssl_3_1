...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ОбъектОтбора) Тогда
		ОсновнойОбъект = Параметры.ОбъектОтбора;
	КонецЕсли;
...
Процедура СтрокаПоискаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	НайтиИВыделитьСтроку(Текст);
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаОтчетаПриАктивизации(Элемент)
	...
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбласти Цикл 
		...
		НоваяОбласть = ТаблицаОтчета.Область(ВыделеннаяОбласть.Верх, ,ВыделеннаяОбласть.Низ);
		ВыбранныеСтроки.Добавить(НоваяОбласть);
	КонецЦикла;
	...
	ВыбранныеСтроки = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ВыбранныеСтроки);
	
	Элементы.ТаблицаОтчета.УстановитьВыделенныеОбласти(ВыбранныеСтроки);
...
Процедура ВывестиДляТекущего(Команда)
	...
	ТекущаяОбласть = Элементы.ТаблицаОтчета.ТекущаяОбласть;
	
	Если ТекущаяОбласть = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТекущийОбъект = РасшифровкаПервогоПоляСтроки(ТекущаяОбласть.Верх);
...
Процедура ИзменитьПометкуУдаления(Команда)
	...
	ПредставленияВыбранныхЭлементов = ?(Сценарий.Пометка, Статистика.БезПометкиНаУдаление, Статистика.СПометкойНаУдаление);
	Сценарий.Вставить("ПредставленияВыбранныхЭлементов", ПредставленияВыбранныхЭлементов);
...
&НаКлиенте
Процедура НайтиСледующуюСтроку(Команда)
	НачалоПоиска = Неопределено;
	ВыделенныеОбласти = Элементы.ТаблицаОтчета.ПолучитьВыделенныеОбласти();
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбласти Цикл 
		Если ТипЗнч(ВыделеннаяОбласть) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда
			СледующаяСтрокаВерх = ВыделеннаяОбласть.Верх + 1;
			СледующаяСтрокаНиз = ВыделеннаяОбласть.Низ + 1;
			НачалоПоиска = ТаблицаОтчета.Область(СледующаяСтрокаВерх, 1,СледующаяСтрокаНиз, 1);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	НачалоПоиска = ?(НачалоПоиска <> Неопределено, НачалоПоиска, Неопределено);
	
	НайтиИВыделитьСтроку(СтрокаПоиска, НачалоПоиска);
КонецПроцедуры
...
#Область ПроцедурыВыводаВТабличныйДокумент
...
#Область ПроцедурыПостроенияДереваПодчиненностиДокументов
	...
	Для каждого ТипОбъекта Из ОбъектыПоТипу Цикл
		...
		Для Индекс = 1 По 3 Цикл
			ТекстПредставление = ТекстПредставление + ",
				...
				+ " КАК ДополнительныйРеквизит" + Индекс; // @query-part-2, @query-part-3
		КонецЦикла;
...
&НаСервере
Функция ТекущийОбъектВыводилсяВВетке(Знач Родитель, Знач ТекущийОбъект, Выводился = Ложь)
	...
	Пока Родитель <> Неопределено Цикл
		...
		Для Каждого Элемент Из Родитель.ПолучитьЭлементы() Цикл 
			Если ТекущийОбъект = Элемент.Ссылка Тогда 
				Выводился = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция ИмяРеквизитаДокумента(Знач МетаданныеОбъекта, Знач ИмяРеквизита) 
	...
	ИмяРеквизитаДокумента = СтруктураПодчиненностиПереопределяемый.ИмяРеквизитаДокумента(МетаданныеОбъекта.Имя, ИмяРеквизита); // АПК:222
...
Функция РеквизитыДляПредставления(Знач ПолноеИмяОбъектаМетаданных, Знач ИмяОбъектаМетаданных)
	...
	// Для обратной совместимости.
	Возврат СтруктураПодчиненностиПереопределяемый.МассивРеквизитовОбъектаДляФормированияПредставления(ИмяОбъектаМетаданных); // АПК:222
...
Функция ПредставлениеОбъектаДляВывода(Данные) 
	...
	// Для обратной совместимости.
	Возврат СтруктураПодчиненностиПереопределяемый.ПредставлениеОбъектаДляВыводаВОтчет(Данные); // АПК:222
	
КонецФункции

#КонецОбласти

#Область ПроцедурыИзмененияСостоянияОбъектов
...
Функция ВыбранныеЭлементы()
		...
		Для НомерКолонки = Границы.Лево По Границы.Право Цикл
			...
			Для НомерСтроки = Границы.Верх По Границы.Низ Цикл
				...
				Если Не ЗначениеЗаполнено(Расшифровка) Тогда 
					Расшифровка = РасшифровкаПервогоПоляСтроки(Ячейка.Верх);
				КонецЕсли;
	...
	ВыбранныеЭлементы = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ВыбранныеЭлементы);
...
Процедура ВыполнитьСценарийИзмененияПометкиУдаления(Ответ, Сценарий) Экспорт 
	...
	Ошибки = ИзменитьПометкуУдаленияЭлементов(Сценарий.ПредставленияВыбранныхЭлементов, Сценарий.Пометка);
...
Функция ИзменитьПометкуУдаленияЭлементов(ВыбранныеЭлементы, Пометка)
	...
	ШаблонОшибки = ?(Пометка, 
		НСтр("ru = 'Недостаточно прав на пометку на удаление ""%1"".'"), 
		Нстр("ru = 'Недостаточно прав на снятие пометки на удаление ""%1"".'"));
	
	Ошибки = Новый Массив;
	Для Каждого СвойстваЭлемента Из ВыбранныеЭлементы Цикл
		
		Элемент = СвойстваЭлемента.Ключ;
			
		ДоступноИзменение = Ложь;
		Если Пометка Тогда
			ДоступноИзменение = ПравоДоступа("ИнтерактивнаяПометкаУдаления", Элемент.Метаданные());
		Иначе
			ДоступноИзменение = ПравоДоступа("ИнтерактивноеСнятиеПометкиУдаления", Элемент.Метаданные());
		КонецЕсли;

		Если Не ДоступноИзменение Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
				СвойстваЭлемента.Значение);
...
Процедура ИзменитьПроведениеДокументов(Режим)
	...
	ПоказатьОповещениеПользователя(Оповещение, Ссылка, Пояснение, БиблиотекаКартинок.ДиалогИнформация);
...
Функция ОбработанныеДокументы(ВыбранныеДокументы, Режим, Ошибки)
	...
	ЗапрещенныеКИзменениюПроведенияДокументы = Новый Массив;
	Пока Индекс >= 0 Цикл 
		...
		Иначе
			Если Режим = РежимЗаписиДокумента.Проведение Тогда
				ИнтерактивноеПраво = "ИнтерактивноеПроведение";
			ИначеЕсли Режим = РежимЗаписиДокумента.ОтменаПроведения Тогда
				ИнтерактивноеПраво = "ИнтерактивнаяОтменаПроведения";
			Иначе
				ИнтерактивноеПраво = Неопределено;
			КонецЕсли;
			Если ЗначениеЗаполнено(ИнтерактивноеПраво) И Не ПравоДоступа(ИнтерактивноеПраво, МетаданныеОбъекта) Тогда
				ЗапрещенныеКИзменениюПроведенияДокументы.Добавить(ВыбранныеДокументы[Индекс]);
				ВыбранныеДокументы.Удалить(Индекс);
			КонецЕсли;
		КонецЕсли;
		Индекс = Индекс - 1;
	...
	СвойстваДокументов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ЗапрещенныеКИзменениюПроведенияДокументы, "Представление");
	ШаблонОшибки = ?(Режим = РежимЗаписиДокумента.Проведение, НСтр("ru = 'Недостаточно прав на проведение ""%1"".'"), Нстр("ru = 'Недостаточно прав на отмену проведения ""%1"".'"));
	Для Каждого СвойствоДокумента Из СвойстваДокументов Цикл
		ПредставлениеДокумента = СвойствоДокумента.Значение.Представление;
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПредставлениеДокумента);
		Ошибки.Добавить(ТекстОшибки);
	КонецЦикла;
...
&НаКлиенте
Функция РасшифровкаПервогоПоляСтроки(НомерСтроки)
	...
	МаксимальныйНомерКолонки = 100;
	Пока НомерТекущейКолонки <> МаксимальныйНомерКолонки Цикл
...
&НаКлиенте
Процедура НайтиИВыделитьСтроку(ТекстПоиска, НачалоПоиска = Неопределено)
	
	Если Не ЗначениеЗаполнено(ТекстПоиска) Тогда
		Возврат;
	КонецЕсли;
	
	РезультатПоиска = ТаблицаОтчета.НайтиТекст(ТекстПоиска, НачалоПоиска,,Истина,,,Истина);
	Если РезультатПоиска = Неопределено И НачалоПоиска <> Неопределено Тогда
		РезультатПоиска = ТаблицаОтчета.НайтиТекст(ТекстПоиска,,,Истина,,,Истина);
	КонецЕсли;
	
	Если РезультатПоиска <> Неопределено Тогда
		ВыбранныеСтроки = Новый Массив;
		ВыбранныеСтроки.Добавить(РезультатПоиска);
		Элементы.ТаблицаОтчета.УстановитьВыделенныеОбласти(ВыбранныеСтроки);
		ТекущийЭлемент = Элементы.ТаблицаОтчета;
	Иначе
