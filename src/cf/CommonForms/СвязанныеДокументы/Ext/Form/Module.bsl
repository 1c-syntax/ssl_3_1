...
Функция ОсновнойДокументДоступен()
	...
	|	1
	|ИЗ
	|	" + ОсновнойОбъект.Метаданные().ПолноеИмя() + " КАК Таб
	|ГДЕ
...
//                - СправочникСсылка
//                - ПВХСсылка - ссылка на объект, значения реквизитов которого
//                              получаются запросом.
//
// Возвращаемое значение:
...
&НаСервере
Процедура ВывестиПодчиненныеОбъекты(ТекущийОбъект, ДеревоРодитель, ВыведенныеОбъекты, СлужебныеОбъекты = Неопределено)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НоваяСтрока <> Неопределено
			...
			ВывестиПодчиненныеОбъекты(Выборка.Ссылка, НоваяСтрока, ВыведенныеОбъекты, СлужебныеОбъекты);
			
		ИначеЕсли СлужебныеОбъекты[Выборка.Ссылка] = Неопределено Тогда 
			
			СлужебныеОбъекты.Вставить(Выборка.Ссылка, Истина);
			ВывестиПодчиненныеОбъекты(Выборка.Ссылка, ДеревоРодитель, ВыведенныеОбъекты, СлужебныеОбъекты);
...
Функция ДобавитьСтрокуВДерево(Родитель, Данные, ВыведенныеОбъекты, ЭтоПодчиненный = Ложь)
	...
	Если Не ВыводитьТекущийОбъект(Данные.Ссылка, ВыведенныеОбъекты, ЭтоПодчиненный) Тогда 
		Возврат Неопределено;
	КонецЕсли;
...
&НаСервере
Функция ВыводитьТекущийОбъект(ТекущийОбъект, ВыведенныеОбъекты, ЭтоПодчиненный, Отказ = Ложь)
