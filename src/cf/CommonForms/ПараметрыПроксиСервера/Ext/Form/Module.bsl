	...
	Если ТипЗнч(НастройкаПроксиСервера) = Тип("Соответствие") Тогда
		...
		Если ИспользоватьПрокси И НЕ ИспользоватьСистемныеНастройки Тогда
			...
			Иначе
				...
				ЗначениеПараметра = ДополнительныеПрокси.Получить("http");
				Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
					СерверHTTP = ЗначениеПараметра.Адрес;
					ПортHTTP   = ЗначениеПараметра.Порт;
				КонецЕсли;
				
				ЗначениеПараметра = ДополнительныеПрокси.Получить("https");
				Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
					СерверHTTPS = ЗначениеПараметра.Адрес;
					ПортHTTPS   = ЗначениеПараметра.Порт;
				КонецЕсли;
				
				ЗначениеПараметра = ДополнительныеПрокси.Получить("ftp");
				Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
					СерверFTP = ЗначениеПараметра.Адрес;
					ПортFTP   = ЗначениеПараметра.Порт;
				КонецЕсли;
...
Процедура УстановитьВидимостьДоступность(Форма)
	...
	Форма.Элементы.ДополнительныеПараметрыПроксиСервера.Видимость = Форма.ВариантИспользованияПроксиСервера <> 0;
