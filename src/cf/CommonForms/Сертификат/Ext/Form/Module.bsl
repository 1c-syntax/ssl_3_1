...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступныДополнительныеСвойстваСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата();
	
	Если ДоступныДополнительныеСвойстваСертификата Тогда
		Элементы.ГруппаЛицензияКриптоПро.Видимость = СвойстваСертификата.СодержитВстроеннуюЛицензиюКриптоПро;
		...
		НазначениеКоды = СтрЗаменить(СвойстваСертификата.Назначение, Символы.ПС, ", ");
		Для Каждого Адрес Из СвойстваСертификата.АдресаСписковОтзыва Цикл
	...
	Если ЭлектроннаяПодпись.ОбщиеНастройки().ПроверятьЭлектронныеПодписиНаСервере
		...
		Если Не ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаСпискиОтзыва 
		И СпискиОтзыва.Количество() = 0
		И Не ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
...
Процедура КорневыеСертификатыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуСертификата(ПараметрыФормы);
...
Процедура ЗаполнитьВнутренниеПоляСертификата()
	...
	Элементы.ВнутреннееСодержаниеСвойство.Видимость = Истина;
	...
	Если ГруппаВнутреннихПолей = "Общие" Тогда
		...
		Если ДоступныДополнительныеСвойстваСертификата Тогда
			Если ЗначениеЗаполнено(Сертификат.УниверсальнаяДатаНачалаЗакрытогоКлюча) Тогда
				ДобавитьСвойство(Сертификат, "УниверсальнаяДатаНачалаЗакрытогоКлюча",    НСтр("ru = 'Дата начала закрытого ключа'"));
			КонецЕсли;
			Если ЗначениеЗаполнено(Сертификат.УниверсальнаяДатаОкончанияЗакрытогоКлюча) Тогда
				ДобавитьСвойство(Сертификат, "УниверсальнаяДатаОкончанияЗакрытогоКлюча", НСтр("ru = 'Дата окончания закрытого ключа'"));
			КонецЕсли;
		Иначе
	...
	ИначеЕсли ГруппаВнутреннихПолей = "РасширенныеСвойства" И ДоступныДополнительныеСвойстваСертификата Тогда
		
		ИменаИдентификаторов = Новый СписокЗначений;
		ИменаИдентификаторов.Добавить("2.5.29.31", НСтр("ru = 'Точки распространения списков отзыва'"));
		ИменаИдентификаторов.Добавить("2.5.29.16", НСтр("ru = 'Период использования закрытого ключа'"));
		ИменаИдентификаторов.Добавить("2.5.29.15", НСтр("ru = 'Использование ключа'"));
		ИменаИдентификаторов.Добавить("2.5.29.37", НСтр("ru = 'Улучшенный ключ'"));
		ИменаИдентификаторов.Добавить("1.2.643.100.114", НСтр("ru = 'Тип идентификации при выдаче сертификата'"));
		ИменаИдентификаторов.Добавить("1.2.643.100.112", НСтр("ru = 'Средства электронной подписи и УЦ издателя'"));
		ИменаИдентификаторов.Добавить("2.5.29.32", НСтр("ru = 'Политики сертификата'"));
		ИменаИдентификаторов.Добавить("1.3.6.1.5.5.7.1.1", НСтр("ru = 'Адреса для загрузки сертификатов издателя и OCSP служб'"));
		ИменаИдентификаторов.Добавить("1.2.643.100.111", НСтр("ru = 'Средство электронной подписи владельца'"));
		ИменаИдентификаторов.Добавить("2.5.29.19", НСтр("ru = 'Основные ограничения'"));
		ИменаИдентификаторов.Добавить("1.2.643.2.2.49.2", НСтр("ru = 'Ограниченная лицензия КриптоПро CSP'"));
				
		Коллекция = Сертификат.ДополненияСертификата;
		
		ИменаИИдентификаторы = Новый Соответствие;
		
		Для Каждого ЭлементСписка Из ИменаИдентификаторов Цикл
			Если Коллекция[ЭлементСписка.Значение] <> Неопределено Тогда
...
Процедура ДобавитьСвойство(ЗначенияСвойств, Свойство, Представление, НижнийРегистр = Неопределено)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Число") И Свойство = "2.5.29.15" Тогда
		Значение = ЭлектроннаяПодписьСлужебный.ИспользованиеКлючаСтрокой(Значение);
	ИначеЕсли ТипЗнч(Значение) = Тип("ФиксированныйМассив") Или ТипЗнч(Значение) = Тип("Массив") Тогда
		...
		Для каждого ЭлементМассива Из ФиксированныйМассив Цикл
			Если ТипЗнч(ЭлементМассива) = Тип("Дата") Тогда
				ЭлементМассива = СтрШаблон("%1 UTC", ЭлементМассива);
			КонецЕсли;
