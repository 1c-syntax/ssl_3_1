...
//
&НаКлиенте
Процедура ПроверитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ОбъектыИспользуются Тогда
		Если КоличествоСсылок = 1 Тогда
			ТекстСообщения =
				НСтр("ru = 'Объект уже используется в других местах в приложении.
				           |Не рекомендуется разрешать редактирование из-за риска рассогласования данных.'");
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выбранные объекты (%1) уже используются в других местах в приложении.
				           |Не рекомендуется разрешать редактирование из-за риска рассогласования данных.'"),
				КоличествоСсылок);
		КонецЕсли;
	Иначе
		Если КоличествоСсылок = 1 Тогда
			ТекстСообщения =
				НСтр("ru = 'Объект еще не используется в других местах в приложении.
