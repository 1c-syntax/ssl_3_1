...
Процедура ОформлениеВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Строка.ЭтоПараметрВывода Тогда 
		Если Строка(Строка.Идентификатор) = "TITLE"
			...
			ПоказатьВводСтроки(Обработчик, Строка.Значение, ЗаголовокПредставленияЗаголовкаДляПечати(),, Истина);
		КонецЕсли;
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Заголовок = ПредставлениеЗаголовкаДляПечати(Объект.Значение);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеЗаголовкаДляПечати(Знач ЗаголовокДляПечати)
	
	Если Не ЗначениеЗаполнено(ЗаголовокДляПечати) Тогда 
		ЗаголовокДляПечати = НСтр("ru = '<Отсутствует>'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1: %2'"),
		ЗаголовокПредставленияЗаголовкаДляПечати(),
		ЗаголовокДляПечати);
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ЗаголовокПредставленияЗаголовкаДляПечати()
	
	Возврат НСтр("ru = 'Заголовок для печати'");
...
Процедура ОформлениеВводЗаголовкаЗавершение(Значение, Идентификатор) Экспорт 
	...
	Строка.Заголовок = ПредставлениеЗаголовкаДляПечати(Значение);
...
Функция ПредставлениеПолейГруппировки(ЭлементНастройки, ПометкаУдаления)
	...
	Поля = ЭлементНастройки.ПоляГруппировки; // ПоляГруппировкиКомпоновкиДанных
...
Процедура УстановитьДоступностьКомандСтруктурыВарианта(ИдентификаторСтроки)
	...
	Элементы.СтруктураВарианта_ПереместитьНаУровеньВыше.Доступность = ЕстьРодитель И РодителяМожноПеремещать;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньВыше1.Доступность = ЕстьРодитель И РодителяМожноПеремещать;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньНиже.Доступность = ЕстьПотенциальныеРодители;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньНиже1.Доступность = ЕстьПотенциальныеРодители;
КонецПроцедуры
