...
Процедура ПоказатьСписокВыбора(Строка, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("ЗначенияДляВыбора", ЗначенияДляВыбора(Строка));
...
Функция НайтиПолеПорядка(СортировкаУзлаНастроек, Поле)
	Для Каждого ЭлементНастройки Из СортировкаУзлаНастроек.Элементы Цикл 
		Если ЭлементНастройки.Поле = Поле Тогда 
			Возврат Поле;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьКоллекциюСтруктуры(Знач Узел, Знач ИмяКоллекции, Строки)
	...
	// Добавление заголовка коллекции.
	Если ИмяКоллекции <> "Структура"
		И (ТипЗнч(Коллекция) = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных")
		Или ТипЗнч(Коллекция) = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных")) Тогда 
		Строка = Строки.Добавить();
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	СвойстваЭлемента.ИменаКоллекций = СтрРазделить(ИменаКоллекций, ", ", Ложь);
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	ОтображатьСтруктуруВарианта = (СодержитПользовательскиеЭлементыСтруктуры Или ЭтоРасширенныйРежим)
