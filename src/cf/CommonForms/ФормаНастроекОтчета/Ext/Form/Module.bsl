...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	Если НеобходимоОбновитьНастройки Тогда
		...
		ПараметрыЗаполнения = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета("ПриСозданииНаСервере");
		ПараметрыЗаполнения.ОбновитьНастройкиВарианта = Не РежимИзмененияЭлементаСтруктурыНастроек И РасширенныйРежим = 1;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не РезультатВыбораСформирован Тогда
		...
		Модифицированность = Модифицированность Или ВариантМодифицирован Или ПользовательскиеНастройкиМодифицированы;
		ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(Оповещение, Отказ, ЗавершениеРаботы,
			НСтр("ru = 'Настройки изменены. Сохранить изменения?'"));
	КонецЕсли;
...
Процедура РасширенныйРежимПриИзменении(Элемент)
	...
	ПараметрыОбновления = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета("РасширенныйРежимПриИзменении");
	ПараметрыОбновления.КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
	ПараметрыОбновления.ОбновитьНастройкиВарианта = (РасширенныйРежим = 1);
	ПараметрыОбновления.СброситьПользовательскиеНастройки = (РасширенныйРежим <> 1);
...
Процедура ВыбранныеПоляПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ТекущаяСтрока, Поле)
	...
	Если ИсточникПеретаскиванияНаКлиенте <> Элемент.Имя Или ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыбранныеПоляПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ТекущаяСтрока, Поле)
	...
	Иначе
		ПеретащитьПоляСортировкиВВыбранныеПоля(ПараметрыПеретаскивания.Значение);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтборыПриАктивизацииЯчейки(Элемент)
	...
	ОграничениеТипа = Строка.ТипЗначения;
	Если Не ЭтоПолеСписка И ОтчетыКлиентСервер.ЭтоВидСравненияБезСтрогойТипизации(Строка.ВидСравнения) Тогда
		ОграничениеТипа = ОтчетыКлиентСервер.ИзменитьТипНаОбычнуюСтроку(ОграничениеТипа);
	КонецЕсли;
	
	ПолеЗначения.ОграничениеТипа = ?(ЭтоПолеСписка, Новый ОписаниеТипов("СписокЗначений"), ОграничениеТипа);
	...
	УстановитьПредставлениеЗначения(Строка, Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора);
КонецПроцедуры
...
Процедура ОтборыВидСравненияПриИзменении(Элемент)
	...
	ПолеЗначения.ВыборГруппИЭлементов = ОтчетыКлиентСервер.ЗначениеТипаГруппыИЭлементы(
		Строка.ВыборГруппИЭлементов, Условие);
	
	ЭтоПолеСписка = Строка.ДоступенСписокЗначений 
		Или ОтчетыКлиентСервер.ЭтоВидСравненияСписка(Строка.ВидСравнения);
	Если Не ЭтоПолеСписка И ОтчетыКлиентСервер.ЭтоВидСравненияБезСтрогойТипизации(Строка.ВидСравнения) Тогда
		Если ТипЗнч(Строка.ПравоеЗначение) = Тип("Строка") Тогда
			Строка.ПравоеЗначение = СокрП(Строка.ПравоеЗначение);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтборыЗначениеПриИзменении(Элемент)
	...
	Если НастройкиОтчета.ЗагрузитьНастройкиПриИзмененииПараметров.Найти(ЭлементНастройки.Параметр) <> Неопределено Тогда 
		...
		ПараметрыОбновления = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета();
		ПараметрыОбновления.КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
		ПараметрыОбновленияВариантМодифицирован = ВариантМодифицирован;
		ПараметрыОбновления.ПользовательскиеНастройкиМодифицированы = ПользовательскиеНастройкиМодифицированы;
		ПараметрыОбновления.СброситьПользовательскиеНастройки = Истина;
		ОбновитьФорму(ПараметрыОбновления);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтборыПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ТекущаяСтрока, Поле)
	...
	Если ТекущаяСтрока = Неопределено Или ТипЗнч(ПараметрыПеретаскивания.Значение) <> Тип("Массив") Тогда
		ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Отмена;
		Возврат;
	КонецЕсли;
...
Процедура СтруктураВариантаПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ИдентификаторЦели, Поле)
	// Проверка общих условий.
	Если ИдентификаторЦели = Неопределено Или ТипЗнч(ПараметрыПеретаскивания.Значение) <> Тип("Число") Тогда
		ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Отмена;
		Возврат;
	КонецЕсли;
	// Проверка источника.
...
Процедура СтруктураВариантаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ИдентификаторЦели, Поле)
	...
	ДополнительныеПараметры = ДополнительныеПараметрыПеремещенияЭлементовСтруктурыВарианта();
	ДополнительныеПараметры.Копировать = ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Копирование;
	Результат = ПереместитьЭлементыСтруктурыВарианта(Строка, НовыйРодитель, ДополнительныеПараметры);
...
Процедура СтруктураВарианта_Переместить(Результат, ПараметрыВыполнения) Экспорт
	...
	ДополнительныеПараметры = ДополнительныеПараметрыПеремещенияЭлементовСтруктурыВарианта();
	ДополнительныеПараметры.ПередЧемВставить = СтрокаТаблицыВниз;
	Результат = ПереместитьЭлементыСтруктурыВарианта(СтрокаТаблицыВверх, СтрокаТаблицыВниз.ПолучитьРодителя(), ДополнительныеПараметры);
	...
	КонецЕсли;
	ДополнительныеПараметры = ДополнительныеПараметрыПеремещенияЭлементовСтруктурыВарианта();
	ДополнительныеПараметры.ПередЧемВставить = ПередЧемВставить;
	Результат = ПереместитьЭлементыСтруктурыВарианта(СтрокаТаблицыВниз, СтрокаТаблицыВверх, ДополнительныеПараметры);
...
Процедура НастроитьКолонтитулы(Команда)
	...
	Перем НастройкиКолонтитулов;
	Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("НастройкиКолонтитулов", НастройкиКолонтитулов);
	ОткрытьФорму("ОбщаяФорма.НастройкаКолонтитулов", Новый Структура("Настройки, ЭтоИндивидуальныеНастройки", НастройкиКолонтитулов, Истина),
...
Процедура ИзменитьУсловияОтборовЗавершение(УсловияОтборов, Контекст) Экспорт
	...
	ПараметрыОбновления = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета("ИзменитьУсловияОтборов");
	ПараметрыОбновления.КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
	ПараметрыОбновления.ПользовательскиеНастройкиМодифицированы = Истина;
	ПараметрыОбновления.УсловияОтборов =УсловияОтборов;
	ОбновитьФорму(ПараметрыОбновления);
КонецПроцедуры
...
Процедура Подключаемый_Список_ВставитьИзБуфера(Команда)
	ПутьКСписку = СтрЗаменить(Команда.Имя, ОтчетыКлиентСервер.ИмяКомандыВставитьИзБуфера(), "");
...
Функция УстановитьДанныеСтрокиОтборов(Строка, СвойствоЭлементаСтруктуры, ЭлементНастройки, ОписаниеНастройки = Неопределено)
	...
	Иначе
		ОтчетыКлиентСервер.ПривестиЗначениеКТипу(ЭлементНастройки.ПравоеЗначение,
			ОписаниеНастройки.ТипЗначения, ЭлементНастройки.ВидСравнения);
	КонецЕсли;
	...
	ПривестиЗначениеКВидуСравнения(Строка, ЭлементНастройки);
	УстановитьПредставлениеЗначения(Строка, СвойствоЭлементаСтруктуры.ДоступныеПоляОтбора);
	...
	Если Строка.ДоступныеЗначения <> Неопределено Тогда
		...
		Для Каждого ДоступноеЗначение Из Строка.ДоступныеЗначения Цикл 
			Список.Добавить(
				...
				ДоступноеЗначение.Пометка,
				ДоступноеЗначение.Картинка);
		КонецЦикла;
...
Процедура СдвинутьОтборы(ВНачало = Истина)
	...
	Для Каждого СтрокаСписка Из ПараметрыСдвига.Строки Цикл 
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, СтрокаСписка);
		ЭлементыНастроек = СвойствоЭлементаСтруктуры.Элементы;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПредставлениеЗначения(Строка, ДоступныеПоляОтбора)
	...
	Если ДоступныеЗначения = Неопределено Тогда
		Если ТипЗнч(Значение) = Тип("ПолеКомпоновкиДанных") Тогда
			ДоступноеПоле = ДоступныеПоляОтбора.НайтиПоле(Значение);
			Если ДоступноеПоле <> Неопределено Тогда
				Строка.ПредставлениеЗначения = ДоступноеПоле.Заголовок;
			КонецЕсли;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьСписокВыбора(Строка, СтандартнаяОбработка, Элемент)
	...
	Обработчик = Новый ОписаниеОповещения("ЗавершитьВыборИзСписка", ЭтотОбъект, Строка.ПолучитьИдентификатор());
	ДоступныеТипы = ДоступныеТипыДляВыбора(Строка.ТипЗначения);
	
	Если ОтчетыКлиент.ЭтоВыборОбъектовМетаданных(ДоступныеТипы, ТекущееЗначение, Обработчик) Тогда
		Возврат;
	КонецЕсли;
	
	ЗначенияДляВыбора = ЗначенияДляВыбора(Строка);
	ОграничиватьВыборУказаннымиЗначениями = Строка.ДоступныеЗначения <> Неопределено;
	...
	Иначе
		ОтчетыКлиент.НачатьВыборПользователей(ЭтотОбъект,
			Элемент, ДоступныеТипы, ТекущееЗначение, ПолеЗначения.ПараметрыВыбора, Обработчик,
...
&НаКлиенте
Процедура ПоказатьСписокВыбораПродолжение(Результат, Контекст) Экспорт
	
	ОткрытьФорму("ОбщаяФорма.ВводЗначенийСпискомСФлажками",
		Контекст.ПараметрыОткрытия, ЭтотОбъект,,,, Контекст.Обработчик);
...
Функция ДоступныеТипыДляВыбора(Знач ТипЗначения)
	
	Возврат ОтчетыСервер.РасширенноеОписаниеТипов(ТипЗначения, Истина).ОписаниеТиповДляФормы;
...
&НаКлиенте
Функция ДополнительныеПараметрыПеремещенияЭлементовСтруктурыВарианта()
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ПередЧемВставить", Неопределено);
	ДополнительныеПараметры.Вставить("Индекс", Неопределено);
	ДополнительныеПараметры.Вставить("ИндексЭлементаНастройки", Неопределено);
	ДополнительныеПараметры.Вставить("Копировать", Ложь);
	
	Возврат ДополнительныеПараметры;
КонецФункции

&НаКлиенте
Функция ПереместитьЭлементыСтруктурыВарианта(Знач Строка, Знач НовыйРодитель, Знач ДополнительныеПараметры = Неопределено)
	...
	ПередЧемВставить = ДополнительныеПараметры.ПередЧемВставить;
	Индекс = ДополнительныеПараметры.Индекс;
	ИндексЭлементаНастройки = ДополнительныеПараметры.ИндексЭлементаНастройки;
	Копировать = ДополнительныеПараметры.Копировать;
...
&НаКлиенте
Процедура НачатьЗаполнениеСписка(Элемент, ПараметрыЗаполнения, ВыборПереопределен = Неопределено, ЭтоПодбор = Ложь)
	ТипЗначения = ЭтотОбъект[ПараметрыЗаполнения.ПутьКСписку].ТипЗначения;
	...
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПодбор", Ложь) Тогда
		ТипЗначения = ОписаниеТиповБезПримитивных(ТипЗначения);
	КонецЕсли;
	...
	ТипЗначения = ОтчетыКлиент.ТипЗначенияОграниченныйСвязьюПоТипу(
		Сведения.Настройки, ПользовательскиеНастройки, Сведения.Элемент, Сведения.Описание, ТипЗначения);
	...
	Если ОтчетыКлиент.ВыборПереопределен(ЭтотОбъект, Обработчик, Сведения.Описание,
			ТипЗначения, ОтмеченныеЗначения, ПараметрыВыбора) Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПодбор", Ложь)
		И ОтчетыКлиент.ЭтоВыборОбъектовМетаданных(ТипЗначения, ОтмеченныеЗначения, Обработчик) Тогда
		Возврат;
	КонецЕсли;
	
	Контекст = Новый Структура;
	Контекст.Вставить("Типы", Типы);
	Контекст.Вставить("ПараметрыЗаполнения", ПараметрыЗаполнения);
	Контекст.Вставить("Элемент", Элемент);
	
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПодбор", Ложь) Тогда
		ОтчетыКлиент.НачатьВыборПользователей(ЭтотОбъект,
			Элемент, ТипЗначения, ОтмеченныеЗначения, ПараметрыВыбора, Обработчик,
			Новый ОписаниеОповещения("НачатьЗаполнениеСпискаПродолжение", ЭтотОбъект, Контекст));
	Иначе
		НачатьЗаполнениеСпискаПродолжение(Неопределено, Контекст);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура НачатьЗаполнениеСпискаПродолжение(ВыбранныйЭлемент, Контекст) Экспорт
	
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ПродолжитьЗаполнениеСписка(ВыбранныйЭлемент, Контекст.ПараметрыЗаполнения);
		Возврат;
	КонецЕсли;
	
	Если Контекст.Типы.Количество() = 1 Тогда
		Контекст.ПараметрыЗаполнения.ВыбранныйТип = Контекст.Типы[0];
		ПродолжитьЗаполнениеСписка(-1, Контекст.ПараметрыЗаполнения);
		Возврат;
	КонецЕсли;
	
	ДоступныеТипы = Новый СписокЗначений;
	ДоступныеТипы.ЗагрузитьЗначения(Контекст.Типы);
	
	Обработчик = Новый ОписаниеОповещения("ПродолжитьЗаполнениеСписка", ЭтотОбъект, Контекст.ПараметрыЗаполнения);
	ПоказатьВыборИзМеню(Обработчик, ДоступныеТипы, Контекст.Элемент);
КонецПроцедуры
...
Процедура ПродолжитьЗаполнениеСписка(ВыбранныйЭлемент, ПараметрыЗаполнения) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПутьКФорме) Тогда 
		...
		Если ТипЗнч(Список) = Тип("СписокЗначений")
		   И ЗначениеЗаполнено(Список) Тогда
			ПоследнийЭлемент = Список[Список.Количество()-1];
			Если ПоследнийЭлемент.Значение = Неопределено Тогда
				ОписаниеТипа = Новый ОписаниеТипов(
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыбранныйЭлемент.Значение));
				ПоследнийЭлемент.Значение = ОписаниеТипа.ПривестиЗначение(Неопределено);
			КонецЕсли;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура УдалитьСтроки(Элемент, Отказ)
	...
	Пока Индекс >= 0 Цикл 
		...
		Если ТипЗнч(Строка.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных")
			Или (Элемент.Имя = "Отборы" И (Строка.ЭтоРаздел Или Строка.ЭтоПараметр)) 
			Или (Элемент.Имя = "Оформление" И Строка.ЭтоПараметрВывода) Тогда 
			Продолжить;
		КонецЕсли;
...
Процедура ИзменитьИспользование(ИмяКоллекции, Использование = Истина, ЭлементыКоллекции = Неопределено)
	...
	Для Каждого Элемент Из ЭлементыКоллекции Цикл 
		...
		ИзменитьИспользование(ИмяКоллекции, Использование, Элемент.ПолучитьЭлементы());
	КонецЦикла;
...
Процедура ИзменитьИспользованиеЭлементаНастройки(ИмяКоллекции)
	...
	Если ИмяКоллекции = "Оформление" И  Строка.ЭтоПараметрВывода 
...
Процедура ИзменитьИспользованиеПоля(ИмяКоллекции, Поле, Использование)
	...
	СтрокиТаблицы = ОтчетыКлиентСервер.НайтиСтрокиТаблицы(ЭтотОбъект[ИмяКоллекции], Условие);
	...
	Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
		...
		Если СвойствоЭлементаСтруктуры = Неопределено Тогда
			КлючСвойства = КлючСвойстваЭлементаСтруктурыНастроек(ИмяКоллекции, СтрокаТаблицы);
			СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(
				Отчет.КомпоновщикНастроек, КлючСвойства, ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
		КонецЕсли;
		
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, СтрокаТаблицы);
		Если ЭлементНастройки <> Неопределено Тогда
...
Функция РезультатВыбора(Переформировать)
	...
	РезультатВыбора = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета(ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияФормыНастроек());
	РезультатВыбора.Переформировать = Переформировать;
	РезультатВыбора.СброситьПользовательскиеНастройки = (РасширенныйРежим = 1);
	РезультатВыбора.ФормаНастроекРасширенныйРежим = РасширенныйРежим;
	РезультатВыбора.ФормаНастроекИмяСтраницы = Элементы.СтраницыНастроек.ТекущаяСтраница.Имя;
	РезультатВыбора.КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
	РезультатВыбора.ВариантМодифицирован = ВариантИзменен;
	РезультатВыбора.ПользовательскиеНастройкиМодифицированы =
...
Процедура ПереместитьВИерархии(Контекст)
	...
	ДополнительныеПараметры = ДополнительныеПараметрыПеремещенияЭлементовСтруктурыВарианта();
	ДополнительныеПараметры.Индекс = Индекс;
	ДополнительныеПараметры.ИндексЭлементаНастройки = Индекс;
	Результат = ПереместитьЭлементыСтруктурыВарианта(Контекст.ТекущаяСтрока, НовыйРодитель, ДополнительныеПараметры);
...
#Область КлиентТаблицыПолейУниверсальныеТочкиВхода
...
#Область КлиентТаблицыПолейФункциональнаяЧасть
...
//               - Неопределено
//  ЭлементНастройки - см. ЭлементКоллекцииФормыНастроек
//
// Возвращаемое значение:
...
//
&НаКлиентеНаСервереБезКонтекста
Функция ЭлементНастройки(Знач УзелНастроек, Знач ЭлементНастройки)
	Результат = Неопределено;
	
	Если ЭлементНастройки <> Неопределено
		И ТипЗнч(ЭлементНастройки.Идентификатор) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		
		Результат = УзелНастроек.ПолучитьОбъектПоИдентификатору(ЭлементНастройки.Идентификатор);
	КонецЕсли;
	
	Если ТипЗнч(Результат) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
		Результат = ЭлементНастройки.Настройки;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьФорму(ПараметрыОбновления)
...
Процедура ОбновитьКоллекцииФормыНастроек()
	...
	НесуществующиеПоля = Новый Соответствие;
	НайтиПомеченныеНаУдалениеПоля();
	УстановитьПодсказкуНесуществующихПолей(НесуществующиеПоля);
КонецПроцедуры
...
#Область ОбщиеСвойстваЭлементов
	...
	Если ТипЗнч(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ОтображатьСтраницы", Неопределено)) = Тип("Булево") Тогда
	...
	Элементы.ЗакрытьИСформировать.Видимость = КоличествоДоступныхНастроек.Итог > 0 Или Элементы.Дополнительно.Видимость Или ОтображатьСтраницы;
	Если ЭтоМобильныйКлиент Тогда
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	Если Не ПараметрыЗагрузки.ВариантМодифицирован Тогда
		ПараметрыЗагрузки.ВариантМодифицирован = ВариантМодифицирован;
	КонецЕсли;
	Если Не ПараметрыЗагрузки.ПользовательскиеНастройкиМодифицированы Тогда
	...
	Если ПараметрыЗагрузки.УсловияОтборов <> Неопределено Тогда
		ПользовательскиеНастройки = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки;
		Для Каждого Условие Из ПараметрыЗагрузки.УсловияОтборов Цикл
...
Процедура НайтиПомеченныеНаУдалениеВыбранныеПоля(Настройки, Знач ЭлементСтруктуры = Неопределено, Группа = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		ДобавитьНесуществующееПоле(ЭлементНастройки.Поле, НСтр("ru = 'Выбор'"));
	КонецЦикла;
КонецПроцедуры
...
Процедура НайтиПомеченныеНаУдалениеПоляОтбора(Настройки, Знач ЭлементСтруктуры = Неопределено, Группа = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		ДобавитьНесуществующееПоле(ЭлементНастройки.ЛевоеЗначение, НСтр("ru = 'Отбор'"));
	КонецЦикла;
КонецПроцедуры
...
Процедура НайтиПомеченныеНаУдалениеПоляПорядка(Настройки, Знач ЭлементСтруктуры = Неопределено)
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		ДобавитьНесуществующееПоле(ЭлементНастройки.Поле, НСтр("ru = 'Порядок'"));
	КонецЦикла;
КонецПроцедуры
...
Процедура НайтиПомеченныеНаУдалениеЭлементыУсловногоОформления(Настройки, Знач ЭлементСтруктуры = Неопределено)
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		Для Каждого Элемент Из ЭлементНастройки.Поля.Элементы Цикл 
			...
			ДобавитьНесуществующееПоле(Элемент.Поле, НСтр("ru = 'Условное оформление'"));
		КонецЦикла;
...
Процедура НайтиПомеченныеНаУдалениеЭлементыОтбораУсловногоОформления(Настройки, ЭлементСтруктуры, ЭлементОформления, Группа = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Запись.КлючСвойстваЭлементаСтруктуры = "УсловноеОформление";
		ДобавитьНесуществующееПоле(ЭлементНастройки.ЛевоеЗначение, НСтр("ru = 'Условное оформление'"));
	КонецЦикла;
КонецПроцедуры
...
Процедура НайтиПомеченныеНаУдалениеПоляГруппировки(Настройки, ЭлементСтруктуры)
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		ДобавитьНесуществующееПоле(ЭлементНастройки.Поле, НСтр("ru = 'Поля группировки'"));
	КонецЦикла;
КонецПроцедуры
...
&НаСервере
Процедура ДобавитьНесуществующееПоле(Знач ИмяПоля, Знач ИмяКоллекции)
	
	ИмяПоля = Строка(ИмяПоля);
	ИмяКоллекции = Строка(ИмяКоллекции);
	
	Свойства = НесуществующиеПоля.Получить(ИмяПоля);
	Если Свойства = Неопределено Тогда
		Свойства = Новый Массив;
		НесуществующиеПоля.Вставить(ИмяПоля, Свойства);
	КонецЕсли;
	
	Если Свойства.Найти(ИмяКоллекции) = Неопределено Тогда
		Свойства.Добавить(ИмяКоллекции);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьПодсказкуНесуществующихПолей(НесуществующиеПоля)
	
	ТекстПодсказки =
		НСтр("ru = 'Несуществующие поля могут появиться, если разработчик отчета удалил поле или запретил его вывод.
		           |Использование таких полей может привести к ошибкам при формировании отчета.
		           |Рекомендуется удалить несуществующие поля из настроек отчета или заменить их на существующие.'");
	
	Строки = Новый Массив;
	Для Каждого КлючИЗначение Из НесуществующиеПоля Цикл
		Строки.Добавить("- " + КлючИЗначение.Ключ + " (" + СтрСоединить(КлючИЗначение.Значение, ", ") + ")");
	КонецЦикла;
	
	Элементы.ГруппаЕстьНесуществующиеПоля.РасширеннаяПодсказка.Заголовок = ТекстПодсказки
		+ Символы.ПС + Символы.ПС
		+ НСтр("ru = 'Несуществующие поля:'")
