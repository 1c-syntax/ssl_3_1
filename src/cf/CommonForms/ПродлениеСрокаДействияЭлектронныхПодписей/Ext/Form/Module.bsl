...
Процедура ОпределитьТипПодписейОбъекта()
	...
	ПараметрыПолученияПодписей.ПорядковыйНомер = ?(ЗначениеЗаполнено(ПорядковыйНомер), ПорядковыйНомер, Неопределено);
	
	ПодписиОбъекта = ЭлектроннаяПодпись.ПодписиОбъекта(ПодписанныйОбъект, ПараметрыПолученияПодписей);
	
	ЕстьБазоваяПодпись = Ложь;
	ЕстьПодписьСМеткойВремени = Ложь;
	ЕстьАрхивнаяПодпись = Ложь;
