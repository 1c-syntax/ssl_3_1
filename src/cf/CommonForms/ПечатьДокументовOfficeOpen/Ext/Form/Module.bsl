...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФормВходящие Цикл
		...
		Для Каждого ПечатнаяФорма Из ПечатныеФормыДокумента Цикл
			Если Не ЭтоАдресВременногоХранилища(ПечатнаяФорма.Ключ) Тогда
				Продолжить;
			КонецЕсли;
	...
	КонецЕсли;
	Элементы.СохранитьВПрисоединенныеФайлы.Видимость = СохранениеВПрисоединенныеФайлы;
	Элементы.КнопкаСохранитьВПрисоединенныеФайлыВсеДействия.Видимость = СохранениеВПрисоединенныеФайлы;
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.Сохранить.Заголовок = НСтр("ru = 'На устройство...'");
	...
	УправлениеПечатью.ОбработатьКомандыОтправки(ЭтотОбъект);
...
Процедура ПерейтиКУправлениюМакетами(Команда)
	...
	УправлениеПечатьюКлиент.ПерейтиКМакету(НастройкиПечатныхФорм[0].ПутьКМакету);
...
&НаКлиенте
Процедура СохранитьВПрисоединенныеФайлы(Команда)

	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОбъектыПечати", ОбъектыПечати);
	ПараметрыФормы.Вставить("РасширениеДляРаботыСФайламиПодключено", Ложь);
	ПараметрыФормы.Вставить("ОграничениеФорматовСохранения", "docx");
	ПараметрыФормы.Вставить("Назначение", "ПрисоединенныеФайлы");
...
Процедура НастроитьФорму(КоличествоПечатныхФорм)
	...
	Элементы.ГруппаУправлениеФлажкамиЕще.Видимость = Элементы.ПечатныеФормы.Видимость;
	Элементы.ГруппаУправлениеСтрокамиЕще.Видимость = Элементы.ПечатныеФормы.Видимость;
...
Процедура ОткрытьОфисныйДокументПослеПолученияВременногоКаталога(ИмяКаталога, ДополнительныеПараметры) Экспорт
	...
	ИмяФайлаДляОткрытия = ИмяКаталога + ПолучитьРазделительПути() + ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ДополнительныеПараметры.ИмяФайлаДляОткрытия);
...
Процедура ПодписатьЗаписанныеОбъекты(ЗаписанныеОбъекты, ОповещениеЗавершенияПодписания)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		МодульРаботаСФайламиКлиент.ПодписатьФайл(ЗаписанныеОбъекты, УникальныйИдентификатор, ДополнительныеПараметры, ПараметрыПодписи);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПодписатьФайлы(ФайлыВоВременномХранилище, ПараметрыВыбора)
	...
	ПараметрыВыбора.Вставить("СоответствиеФайловОбъектамПечати", Соответствие);
...
Процедура ЗавершениеОбработкиВыбора(Результат, ПараметрыВыбора) Экспорт	
			...
			Если ВыбранноеЗначение.ВариантСохранения = "СохранитьВПапку" Тогда
				Если ПараметрыВыбора.ВыбранноеЗначение.Подписать Тогда
					ФайлыВоВременномХранилище = ПолучитьФайлыПодписейИПоместитьВАрхив(
						Результат, ВыбранноеЗначение, ПараметрыВыбора.СоответствиеФайловОбъектамПечати);
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.ВыборФорматаВложений")
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			Если ВыбранноеЗначение.Подписать Тогда
				ФайлыВоВременномХранилище = ПолучитьФайлыПодписейИПоместитьВАрхив(Результат, ВыбранноеЗначение, 
					ПараметрыВыбора.СоответствиеФайловОбъектамПечати);
			...
			Иначе
				Вложения = ?(ВыбранноеЗначение.Свойство("СписокВложений"), ВыбранноеЗначение.СписокВложений, Неопределено);
...
&НаКлиенте
Процедура ПродолжитьПечатьПослеУстановкиРасширения1СПредприятия(РасширениеПодключено, ДополнительныеПараметры) Экспорт
...
Процедура ПечатьЗавершение(МассивПечатныхФорм)	
	Обработчик = Новый ОписаниеОповещения("ПродолжитьПечатьПослеУстановкиРасширения1СПредприятия", ЭтотОбъект, МассивПечатныхФорм);
