...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФорматВложений.Видимость = ВыбранныеФорматыСохранения.Количество() > 1;
	...
	Элементы.ГруппаДополнительныеПараметры.Видимость = Не Параметры.ПропуститьВыборФорматаВложений;
	
	ЗаполненоАдресов = 0;
	СтрокаСАдресом = Неопределено;
	Для Каждого Получатель Из Получатели Цикл
		Если НЕ ПустаяСтрока(Получатель.ПредставлениеАдреса) Тогда
			СтрокаСАдресом = Получатель;
			ЗаполненоАдресов = ЗаполненоАдресов + 1;
		КонецЕсли;
	КонецЦикла;
	Если ЗаполненоАдресов = 1 Тогда
		СтрокаСАдресом.Выбран = Истина;
	КонецЕсли;
...
Процедура ФорматВложенийНажатие(Элемент, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораФорматаВложений", ЭтотОбъект);
	ОбщегоНазначенияКлиент.ПоказатьВыборФорматаВложений(ОписаниеОповещения, ВыбранныеНастройкиФормата());
КонецПроцедуры
...
&НаКлиенте
Процедура ПослеВыбораФорматаВложений(НастройкиСохранения, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(НастройкиСохранения) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьВыборФормата(НастройкиСохранения.ФорматыСохранения);
...
Процедура ЗаполнитьТаблицуПолучателейИзМассиваСтруктур(СписокПолучателей)
	...
	Для Каждого Получатель Из СписокПолучателей Цикл
		...
		НовыйПолучатель.ПредставлениеАдреса = НовыйПолучатель.Адрес;
		Если Не ПустаяСтрока(НовыйПолучатель.ВидПочтовогоАдреса) Тогда
			НовыйПолучатель.ПредставлениеАдреса = НовыйПолучатель.ПредставлениеАдреса + " (" + НовыйПолучатель.ВидПочтовогоАдреса + ")";
		КонецЕсли;
	КонецЦикла;
