...
Процедура ВыборРоли(ТекущиеДанные)
	...
	Если ТекущиеДанные.ИспользуетсяСОбъектамиАдресации Тогда
		...
		ПараметрыФормы = БизнесПроцессыИЗадачиКлиент.ПараметрыФормыВыбораРолиИсполнителя(ТекущиеДанные.Ссылка);
		ПараметрыФормы.ВыборОбъектаАдресации = Истина;
	
		БизнесПроцессыИЗадачиКлиент.ОткрытьФормуВыбораРолиИсполнителя(ПараметрыФормы, ЭтотОбъект);
		
	Иначе
		
		ВыбранноеЗначение = Новый Структура("РольИсполнителя, ОсновнойОбъектАдресации, ДополнительныйОбъектАдресации", 
			ТекущиеДанные.Ссылка, Неопределено, Неопределено);
		ОповеститьОВыборе(ВыбранноеЗначение);
