...
Процедура ПриОткрытии(Отказ)
	...
	Если НастройкиПечатныхФорм.Количество() = 0 Тогда
		ДлительнаяОперация = НачатьФормированиеПечатныхФорм(ИменаМакетов);
		Контекст = Новый Структура("ИменаМакетов", ИменаМакетов);
...
Функция НачатьФормированиеПечатныхФорм(ИменаМакетов)
	...
	ПараметрыВыполнения.АдресРезультата = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификаторХранилища);
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	
	СтруктураПараметров = Новый Структура("ИсточникДанных,ПараметрыИсточника,ПараметрыПечати,ИмяМенеджераПечати,ПараметрКоманды");
	Для каждого Параметр Из СтруктураПараметров Цикл
		СтруктураПараметров.Вставить(Параметр.Ключ, ОбщегоНазначения.СкопироватьРекурсивно(Параметры[Параметр.Ключ]));
	КонецЦикла;
	СтруктураПараметров.ПараметрыПечати.ДополнительныеПараметры.Вставить("ПодписьИПечать",	
	...
	ПараметрыВыполнения.ЗапуститьНеВФоне = Не ПараметрыПечатиМожноПередатьВФоновоеЗадание(СтруктураПараметров.ПараметрыПечати);
	
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "УправлениеПечатью.СформироватьПечатныеФормыВФоне", ИменаМакетов, 
		СтруктураПараметров, ПараметрыВывода, ТекущийЯзык, ОбъектыПечати);
КонецФункции
...
Процедура ПриЗавершенииФормированияПечатныхФорм(Результат, Знач Контекст) Экспорт
	Если Результат <> Неопределено Тогда
		...
		ОбъектыПечати = СтруктураРезультата.ОбъектыПечати;
		
		МассивИменПараметров = СтрРазделить("ИмяМенеджераПечати,ИсточникДанных,ПараметрКоманды,ПараметрыИсточника,ПараметрыПечати", ",");
		Для Каждого ИмяПараметра Из МассивИменПараметров Цикл
			Контекст.Вставить(ИмяПараметра, ОбщегоНазначенияКлиент.СкопироватьРекурсивно(Параметры[ИмяПараметра]));
		КонецЦикла;
		
		Контекст.Вставить("ОбъектыПечати", ОбъектыПечати);
		Контекст.Вставить("КоллекцияПечатныхФорм", КоллекцияПечатныхФорм);
		Контекст.Вставить("ПараметрыВывода", СтруктураРезультата.ПараметрыВывода);
		Контекст.Вставить("Сообщения", Результат.Сообщения);
		ОткрытьФорму("ОбщаяФорма.ПечатьДокументов", Контекст, ВладелецФормы, Строка(Новый УникальныйИдентификатор));
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьРезультатФоновогоЗадания(Результат)
	...
	СтруктураРезультата.КоллекцияПечатныхФорм = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаПечатныхФорм);
...
Процедура СохранитьПечатнуюФормуВФайл()
	...
	ПечатьСКД = ФайлыВоВременномХранилище[0].ОбъектПечати <> Неопределено;
...
Процедура ОткрытьОфисныеДокументыПослеПолученияВременногоКаталога(ПолученныйВременныйКаталог, Контекст) Экспорт
	...
	Для Каждого ФайлДляЗаписи Из Контекст.ФайлыВоВременномХранилище Цикл
		...
		Вызов.Добавить(ПолученныйВременныйКаталог+ФайлДляЗаписи.Представление);
		Вызов.Добавить();
		Вызов.Добавить(Ложь);
		Вызовы.Добавить(Вызов);
		
		МассивПередаваемыхФайлов.Добавить(Новый ОписаниеПередаваемогоФайла(ПолученныйВременныйКаталог+ФайлДляЗаписи.Представление, ФайлДляЗаписи.АдресВоВременномХранилище));
	КонецЦикла;
...
&НаСервере
Функция ПараметрыПечатиМожноПередатьВФоновоеЗадание(Знач ПараметрыПечати)
	
	Для Каждого Элемент Из ПараметрыПечати Цикл
		Если ТипЗнч(Элемент.Значение) = Тип("Строка") И ЭтоАдресВременногоХранилища(Элемент.Значение) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
