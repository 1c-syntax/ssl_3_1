...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭтоМобильныйКлиент И ЭтоПечатьКомплекта() Тогда
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.КнопкаСохранитьВсеДействия.Заголовок = НСтр("ru = 'На устройство...'");
	КонецЕсли;
...
&НаКлиенте
Процедура НастройкиПечатныхФормПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Элемент.ТекущаяСтрока <> Неопределено Тогда
		ВыбраннаяСтрокаКомплекта = Элемент.ТекущаяСтрока;
		Элементы.НастройкиПечатныхФорм.ТекущаяСтрока = Элемент.ТекущаяСтрока;
	КонецЕсли;
...
Процедура НастроитьВидимостьЭлементовФормы()
	...
	Элементы.КнопкаПереключитьРедактированиеВсеДействия.Видимость = Элементы.КнопкаРедактирование.Видимость;
	...
	Элементы.КнопкаПоказатьСкрытьНастройкуКомплектаВсеДействия.Видимость = ЭтоПечатьКомплекта;
	...
	Элементы.УправлениеФлажкамиВсеДействия.Видимость = ЭтоПечатьНастраиваемогоКомплекта;
	Элементы.УправлениеСтрокамиВсеДействия.Видимость = ЭтоПечатьНастраиваемогоКомплекта;
...
Процедура УстановитьВидимостьНастройкиКоличестваКопий(Знач Видимость = Неопределено)
	...
	Элементы.УправлениеФлажкамиВсеДействия.Видимость = Видимость И ДоступнаНастройкаКомплекта;
	Элементы.УправлениеСтрокамиВсеДействия.Видимость = Видимость И ДоступнаНастройкаКомплекта;
КонецПроцедуры
...
Процедура УстановитьТекущуюСтраницу()
	...
	Если ЭтоМобильныйКлиент Тогда
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаТекущаяПечатнаяФорма Тогда
			ТекущийЭлемент = Элементы.ТекущаяПечатнаяФорма;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция БезопасноеИмяФайлаДляАрхива(Представление, ИмяВременногоКаталога)
	
	ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(Представление);
	...
	Если ЗначениеЗаполнено(СтруктураИмениФайла.Расширение) Тогда
		ИмяФайла = СтруктураИмениФайла.ИмяБезРасширения + НРег(СтруктураИмениФайла.Расширение);
	КонецЕсли;
	
	ПолноеИмяФайла = ИмяВременногоКаталога + ИмяФайла;
	
	Возврат ФайловаяСистема.УникальноеИмяФайла(ПолноеИмяФайла);
...
Функция ПоместитьФайлыВАрхив(ПечатныеФормыДокументов, ПереданныеНастройки)
	...
	Для Каждого СтруктураФайла Из ПечатныеФормыДокументов Цикл
		...
		ПолноеИмяФайла = БезопасноеИмяФайлаДляАрхива(СтруктураФайла.Представление, ИмяВременногоКаталога);
...
Процедура ПереключитьПометкуКнопкиРедактирование()
	...
	Элементы.КнопкаПереключитьРедактированиеВсеДействия.Пометка = Элементы.КнопкаРедактирование.Пометка;
	Элементы.КнопкаПереключитьРедактированиеВсеДействия.Доступность = Элементы.КнопкаРедактирование.Доступность;
...
Функция НастройкаТекущейПечатнойФормы()
	...
	Если ЭтоМобильныйКлиент И ВыбраннаяСтрокаКомплекта >= 0
	  И Элементы.НастройкиПечатныхФорм.ТекущиеДанные = Неопределено Тогда
		
		НастройкаПечатнойФормы = НастройкиПечатныхФорм.НайтиПоИдентификатору(ВыбраннаяСтрокаКомплекта);
		Если НастройкаПечатнойФормы <> Неопределено Тогда
			Возврат НастройкаПечатнойФормы;
		КонецЕсли;
	КонецЕсли;
