...
#КонецЕсли
	...
	Для Каждого ФайлДляЗаписи Из СписокФайловВоВременномХранилище Цикл
		...
		ДвоичныеДанные.Записать(УникальноеИмяФайла(ИмяКаталога + ФайлДляЗаписи.Представление));
	КонецЦикла;
...
Функция ТабличныеДокументыДляПечати()
	...
	// Записываем сведения о печати формы в регистр состояний оригиналов
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УчетОригиналовПервичныхДокументов") Тогда
		Массив = НастройкиПечатныхФорм.Выгрузить(,"ИмяМакета, Название");
		СписокПечати = Новый СписокЗначений; 
		Для Каждого ПечатнаяФорма Из Массив Цикл
