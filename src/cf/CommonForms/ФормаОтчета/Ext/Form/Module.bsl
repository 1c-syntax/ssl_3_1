...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПраваПользователей.Видимость = Пользователи.ЭтоПолноправныйПользователь() 
		...
		И (ТипЗнч(НастройкиОтчета.ОтчетСсылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		Или ТипЗнч(НастройкиОтчета.ОтчетСсылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений"));
...
Процедура Подключаемый_ПоказатьУровеньГруппировок(Команда)
	...
	УровеньГруппировкиСтрокой = СтрЗаменить(Команда.Имя, Элементы.КонтекстноеМенюОбластиЗаголовкаМенюПоказатьУровеньГруппировок.Имя, "");
...
Процедура РассчитатьПоказателиДинамически()
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиОтчета, "ВыводитьСуммуВыделенныхЯчеек", Истина) Тогда 
...
Процедура ОчиститьМенюУровнейГруппировок()
	...
	Кнопки = Элементы.КонтекстноеМенюОбластиЗаголовкаМенюУровниГруппировок.ПодчиненныеЭлементы;
	...
	Пока Индекс >= 0 Цикл 
		...
		Если Кнопка = Элементы.КонтекстноеМенюОбластиЗаголовкаМенюПоказатьУровеньГруппировок Тогда 
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьМенюУровнейГруппировок(ИндексСтруктурыОтчета)
	...
	Для Каждого Свойства Из СвойстваМеню Цикл 
		...
		Кнопка = Элементы.Добавить(Свойства.ИмяКоманды, Тип("КнопкаФормы"), Элементы.КонтекстноеМенюОбластиЗаголовкаМенюУровниГруппировок);
...
Функция СвойстваМенюУровнейГруппировок(ИндексСтруктурыОтчета)
	...
	Для УровеньГруппировки = 1 По КоличествоУровнейГруппировок Цикл 
		...
		Свойства.ИмяКоманды = Элементы.КонтекстноеМенюОбластиЗаголовкаМенюПоказатьУровеньГруппировок.Имя + УровеньГруппировки;
...
Процедура УстановитьДоступностьМенюУровнейГруппировок()
	...
	МенюЗаполнено = (Элементы.КонтекстноеМенюОбластиЗаголовкаМенюУровниГруппировок.ПодчиненныеЭлементы.Количество() > 1);
	
	Элементы.КонтекстноеМенюОбластиЗаголовкаМенюПоказатьУровеньГруппировок.Видимость = Не МенюЗаполнено;
	Элементы.КонтекстноеМенюОбластиЗаголовкаМенюУровниГруппировок.Видимость = МенюЗаполнено;
...
Процедура ПоказатьВыбранныйУровеньГруппировок(УровеньГруппировки = Неопределено)
	...
	ИмяКнопки = Элементы.КонтекстноеМенюОбластиЗаголовкаМенюПоказатьУровеньГруппировок.Имя + УровеньГруппировки;
...
Процедура ПрименитьНастройкиИзКонтекстногоМеню(Результат)
	...
	Если Не Результат.Переформировать
		И Результат.УчитыватьВремяФормирования Тогда 
