...
Процедура ОбработкаВыбора(Результат, ПодчиненнаяФорма)
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если ИмяФормыИсточника = ВРег("ФормаНастроекОтчета")
				Или ИмяФормыИсточника = ВРег("ФормаНастроек")
				Или ИмяФормыИсточника = ВРег("ФормаВариантаОтчета")
				Или ИмяФормыИсточника = ВРег("ФормаВарианта") Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеВидимостиВариантаОтчетаВПанелиОтчетов"
	        И Параметр = НастройкиОтчета.ПолноеИмя
...
Процедура Подключаемый_ЭлементНастройки_ПриИзменении(Элемент)
	...
	Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
		...
		ФормаПараметры.Вставить("СброситьНастройки", Ложь); 
...
Процедура НастройкиПоУмолчанию(Команда)
	...
	Если ВариантМодифицирован Или ПустаяСтрока(ФормаПараметры.КлючНазначенияИспользования) Тогда
	...
	ФормаПараметры.Вставить("СброситьНастройки", Истина);
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	ФормаПараметры.Вставить("СброситьНастройки", Истина);
	ЗагрузитьВариант(ВариантФормы.КлючВарианта, Ложь);  
...
Процедура Сформировать()
	...
	Если Результат.Свойство("ИмяПараметраХраненияОтказаОтПредупреждения") Тогда
		...
		Если ЗначениеЗаполнено(Результат.ИмяПараметраХраненияОтказаОтПредупреждения) Тогда
			ПараметрКомпоновки = Новый ПараметрКомпоновкиДанных(
				Результат.ИмяПараметраХраненияОтказаОтПредупреждения);
...
Процедура СформироватьПослеПредупреждения(Ответ, Контекст) Экспорт
	...
	ИначеЕсли ТипЗнч(Ответ) = Тип("Структура") Тогда
		Если Ответ.БольшеНеЗадаватьЭтотВопрос
		   И Контекст.НастройкаОтказаОтПредупреждения <> Неопределено Тогда
		...
		Если Ответ.Значение = "Продолжить" Тогда
			...
			Результат = РезультатФормированияОтчета(ФормированиеПриОткрытии,
				НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный, Истина);
		Иначе
			Возврат;
		КонецЕсли;
	Иначе
...
Процедура ПослеФормирования(Результат, ПараметрыФормирования) Экспорт 
	...
	Если Результат = Неопределено Тогда 
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет не сформирован'"), Окно.ПолучитьНавигационнуюСсылку(), Заголовок);
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда 
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет сформирован'"), Окно.ПолучитьНавигационнуюСсылку(), Заголовок);
...
Функция ПараметрыФормированияОтчета(ИмяОтчета, Непосредственно)
	...
	ПараметрыФормированияОтчета.Вставить("ПараметрыИзменены", ПараметрыИзменены);
...
Процедура ПоказатьСписокВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ОтчетыКлиент.ВыборПереопределен(ЭтотОбъект, Обработчик, Сведения.Описание,
			Элемент.ДоступныеТипы, ОтмеченныеЗначения, ПараметрыВыбора) Тогда
		Возврат;
	КонецЕсли;
	
	Если ОтчетыКлиент.ЭтоВыборОбъектовМетаданных(Элемент.ДоступныеТипы, ОтмеченныеЗначения, Обработчик)
		Или ОтчетыКлиент.ЭтоВыборПользователей(ЭтотОбъект, Элемент, Элемент.ДоступныеТипы, ОтмеченныеЗначения, ПараметрыВыбора, Обработчик) Тогда 
		Возврат;
	КонецЕсли;
...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоиска(ПодходящиеЗначения, ПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Пока Выборка.Следующий() Цикл 
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл 
			...
			ВариантыПоиска = Новый Массив;
			ВариантыПоиска.Добавить(СвойстваПоиска);			
			ДополнительныеЗначения.Добавить(ВариантыПоиска, 
				ПредставлениеРезультатПоиска(ПолеПоиска.Значение, СвойстваПоиска.ВидСравнения, 
...
Функция ПолеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"КОГДА ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 СПЕЦСИМВОЛ ""~"" ТОГДА ""%3""",
			Поле.Имя,
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если ТипРеквизита.СодержитТип(Тип("Строка")) Тогда 
			...
			Условие = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 СПЕЦСИМВОЛ ""~""",
				Поле.Имя,
				"""%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(ПараметрыПоиска.СтрокаПоискаНормализованная) + "%""");
...
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, ПолеПоиска, Условие, СтрокаПоиска)
	...
	Возврат Новый ФорматированнаяСтрока(ПолеПоиска, ФрагментУсловия, ФрагментДоСтрокиПоиска,
		ФрагментСтрокиПоиска, ФрагментПослеСтрокиПоиска);
...
Процедура ЗаполнитьМенюУровнейГруппировок(ИндексСтруктурыОтчета)
	...
	Для Каждого Свойства Из СвойстваМеню Цикл 
		...
		// Подменю Еще.
		Кнопка = Элементы.Добавить(Свойства.ИмяКоманды + "Еще", Тип("КнопкаФормы"), 
			Элементы.ГруппаУровниГруппировокЕще);
		Кнопка.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		Кнопка.ИмяКоманды = Свойства.ИмяКоманды;
		Кнопка.Заголовок = Свойства.ПредставлениеУровняГруппировки;
...
Процедура СохранитьОтчетПослеВыбораИмениФайла(Результат, Контекст) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ПолноеИмяФайлаОтчета) Тогда
		Контекст.Вставить("ПолноеИмяФайлаОтчета", ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(Заголовок));
		...
		СписокФорматов.ПоказатьВыборЭлемента(ОписаниеОповещенияОЗакрытии, НСтр("ru='Выберите формат сохранения'"), ФорматПоУмолчанию);
	Иначе
...
Процедура ПослеВыбораФорматаСохранения(ВыбранныйЭлемент, Контекст) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("СохранитьОтчетПослеСохраненияРезультатаОтчета", ЭтотОбъект, ПолноеИмяФайлаОтчета);
...
Функция ДоступныеФорматыСохраненияОтчета(ИндексФорматовСохраненияОтчета)
	...
	Для Каждого ФорматСохранения Из ФорматыСохранения Цикл
		...
		ПредставлениеФорматаСохранения = СтрЗаменить(ФорматСохранения.Представление,
			"." + ФорматСохранения.Расширение, РасширениеФорматаСохранения);
...
Процедура УстановитьВидимостьДоступность()
	...
	Если ДоступноСохранятьТекущийВариант
	...
	Элементы.ДругиеОтчеты.Видимость = НастройкиОтчета.РазрешеноВыбиратьВарианты
		И Не ЗначениеЗаполнено(КонтекстВарианта);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ПоделитьсяНастройками", "Видимость", РазрешеноИспользоватьНастройки
			И (ЭтоПредопределенныйВариант
				Или ВариантыОтчетов.ПолныеПраваНаВарианты()
				Или ЗначениеЗаполнено(НастройкиОтчета.ВариантСсылка)
				  И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкиОтчета.ВариантСсылка,
	...
	// В модели сервиса редактирование СКД недоступна с точки зрения безопасности.	
...
Функция СобытияНастроек()
	...
	НайтиСобытияНастроек(
		Элементы.ОтчетТабличныйДокумент.КонтекстноеМеню,
		УточненияСобытийНастроек(),
		СобытияНастроекИсключения(),
		СобытияНастроек);
	
	СобытияНастроек.Вставить("ФильтроватьИСформировать", НСтр("ru = 'Фильтровать...'"));
	
	СобытияНастроек.Вставить(
		ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияФормыНастроек(),
		НСтр("ru = 'Расширенная настройка'"));
	
	СобытияНастроек.Вставить(
		ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияИзмененияСоставаБыстрыхНастроек(),
...
Процедура НайтиСобытияНастроек(Меню, Уточнения, Исключения, СобытияНастроек)
	...
	Для Каждого Элемент Из Меню.ПодчиненныеЭлементы Цикл 
		...
		ИначеЕсли СобытияНастроек[Элемент.ИмяКоманды] = Неопределено
			...
			ДействиеКоманды = ?(ЗначениеЗаполнено(НайденнаяКоманда.Действие), НайденнаяКоманда.Действие, НайденнаяКоманда.Имя);
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	Если ДоступныеНастройки.Настройки <> Неопределено И НастройкиОтчета.События.ПередЗагрузкойНастроекВКомпоновщик Тогда      
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ФормаПараметры, "СброситьНастройки", Ложь) Тогда
			ДоступныеНастройки.Настройки.ДополнительныеСвойства.Вставить("СброситьНастройки", Истина);	
		КонецЕсли;
		...
		ФормаПараметры.Вставить("СброситьНастройки", Ложь);  
		ДоступныеНастройки.Настройки.ДополнительныеСвойства.Вставить("СброситьНастройки", Ложь);
	КонецЕсли;
...
Процедура ПоказатьОшибкиФормирования(ИнформацияОбОшибке)
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Отчет не сформирован по причине:'") 
...
Функция ТребуетсяСохранитьВариант()
	...
	Если ДополнительныеСвойства.Свойство("ФормаПараметрыОтбор")
	   И ЗначениеЗаполнено(ДополнительныеСвойства.ФормаПараметрыОтбор)
	 Или РежимРасшифровки Тогда
...
Процедура СохранитьНовыйИлиСуществующийВариантОтчетаИПродолжить(Команда)
	...
	Если Не Элементы.СохранитьВариантКак.Видимость Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Невозможно выполнить действие, так как недоступно сохранение вариантов отчетов.'"));
		Возврат;
	КонецЕсли;
	...
	Если Элементы.СохранитьВариант.Видимость Тогда
		...
		ПараметрыВопроса.Заголовок = ЭтотОбъект.ОтчетНаименованиеТекущегоВарианта;
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
			Новый ОписаниеОповещения("Подключаемый_КомандаПослеОтветаНаВопросСохранитьВариант", ЭтотОбъект, Команда),
			НСтр("ru = 'Сохранить вариант отчета?'"),
			Кнопки,
			ПараметрыВопроса);
		Возврат;
	Иначе
		...
		ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.ФормаСохранения",
			ПараметрыФормы, ЭтотОбъект,,,, Новый ОписаниеОповещения(
				"Подключаемый_КомандаПослеСохраненияВарианта", ЭтотОбъект, Команда));
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_КомандаПослеОтветаНаВопросСохранитьВариант(Ответ, Команда) Экспорт
	...
	Если Ответ = Неопределено
	 Или Ответ.Значение <> "СохранитьИПродолжить" Тогда
		Возврат;
	КонецЕсли;
...
Процедура СохранитьВариантОтчета(ПослеДобавленияНового = Ложь)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Вариант отчета сохранен'"), Окно.ПолучитьНавигационнуюСсылку(), Заголовок);
...
Процедура ОбновитьКомандыВыбораВариантов()
	...
	Иначе
		ДоступныеПодсистемы = ВариантыОтчетовПовтИсп.ПодсистемыТекущегоПользователя().Список;
		ПараметрыПоиска.Вставить("Подсистемы", ДоступныеПодсистемы);
...
Процедура ОбновитьИнформациюОВариантеОтчета()
	...
	Если Не ЗначениеЗаполнено(КлючЗамеров) Тогда
		КлючЗамеров = ВариантыОтчетовСлужебный.КлючЗамеров(НастройкиОтчета.ПолноеИмя,
			КлючТекущегоВарианта);
	КонецЕсли;
...
Процедура ОбновитьСочетаниеКлавишКомандПереходаМеждуИзменениямиНастроек()
	...
	Если ТекущиеНастройкиСтека.Количество() > 0 Тогда 
		...
		Если Не СочетаниеКлавишУстановлено
			И ПорядокКомандыТекущихНастроекСтека = 1
	...
	ИначеЕсли СтекНастроек.Количество() > 0
		И КнопкаУстановкиСтандартныхНастроек.Пометка Тогда 
...
Функция СписокДоступныхФорматовСохраненияОтчета(ИндексФорматовСохраненияОтчета)
	...
	Для Каждого ФорматСохранения Из ИндексФорматовСохраненияОтчета Цикл
		...
		ДоступныеФорматы.Добавить(НайденноеОписание.Расширение, НайденноеОписание.Представление, , НайденноеОписание.Картинка);
	КонецЦикла;
