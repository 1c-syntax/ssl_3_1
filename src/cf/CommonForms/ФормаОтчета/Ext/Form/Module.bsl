...
Процедура ОбработкаВыбора(Результат, ПодчиненнаяФорма)
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если СтрСравнить(ИмяФормыИсточника, "ФормаНастроекОтчета") = 0 
				Или СтрСравнить(ИмяФормыИсточника, "ФормаНастроек") = 0
				Или СтрСравнить(ИмяФормыИсточника, "ФормаВариантаОтчета") = 0 
				Или СтрСравнить(ИмяФормыИсточника, "ФормаВарианта") = 0 Тогда
		...
		Если ВариантыОтчетовСлужебныйКлиент.ЭтоСобытиеКонтекстнойНастройки(Результат) 
			И Результат.ИдентификаторВладельца = УникальныйИдентификатор Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = ВариантыОтчетовКлиент.ИмяСобытияИзменениеВарианта() Тогда
		...
		Если ЗначениеЗаполнено(Параметр.КлючВарианта) Тогда
			УстановитьТекущийВариант(Параметр.КлючВарианта);
		Иначе
			ВариантыПанелиКлючТекущегоВарианта = ПустойКлючВарианта();
		КонецЕсли;
	...
	ИначеЕсли ВариантыОтчетовСлужебныйКлиент.ЭтоСобытиеКонтекстнойНастройки(Параметр) 
		И Параметр.ИдентификаторВладельца = УникальныйИдентификатор Тогда
	...
	ИнтеграцияПодсистемБСПКлиент.ПриОбработкеОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник,
		Истина);
	ОтчетыКлиентПереопределяемый.ОбработкаОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник, Истина);
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИнтеграцияПодсистемБСПКлиент.ПриОбработкеДополнительнойРасшифровки(ЭтотОбъект,
		Элемент, Расшифровка, СтандартнаяОбработка);
	
	ОтчетыКлиентПереопределяемый.ОбработкаДополнительнойРасшифровки(ЭтотОбъект,
		Элемент, Расшифровка, СтандартнаяОбработка);

	Если СтандартнаяОбработка Тогда
		Данные = ДанныеЭлементаРасшифровки(Расшифровка);
		ВариантыОтчетовСлужебныйКлиент.ОбработкаДополнительнойРасшифровки(ЭтотОбъект,
			Данные, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ПрименитьНастройкиИПереформироватьОтчет(Результат, ПараметрыВыполнения) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Результат, ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета(), Ложь);

	Если Результат.ИмяСобытия = ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияФормыНастроек() 
...
Процедура НастройкиПоУмолчанию(Команда)
	ПараметрыЗаполнения = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета(
		ОтчетыКлиентСервер.ИмяСобытияНастройкиПоУмолчанию());
...
Процедура ДругиеОтчеты(Команда)
	...
	Для Каждого ВариантОтчета Из ДобавленныеВарианты Цикл
		ВидимыеВарианты.Добавить(ВариантОтчета.Ссылка);
	КонецЦикла;
...
#Область ОбработчикиКомандМодификацииСКД
...
Процедура ВосстановитьСтандартнуюСхему(Команда)
	...
	ПараметрыЗаполнения = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета();
	ПараметрыЗаполнения.КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
	ПараметрыЗаполнения.ПользовательскиеНастройкиМодифицированы = Истина;
	ОбновитьЭлементыФормыНастроек(ПараметрыЗаполнения);
...
#Область ОбработчикиКомандОбменаНастройкамиОсновнымиИПользовательскими
...
#Область ОбработчикиКомандСохраненияРезультатаОтчета
...
Процедура ОбновитьЭлементыФормыНастроек(ПараметрыОбновления)
	...
	Если ПараметрыОбновления.Переформировать Тогда
		ОчиститьСообщения();
		Сформировать();
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьРезультатФормированияОтчета()
	...
	ПараметрыОбновления = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета("ПослеФормирования");
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыОбновления, Результат, Истина);
	ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления);
КонецПроцедуры
...
Процедура ПерейтиКНастройкам(РасширенныйРежим = Неопределено)
	...
	Форма = ОткрытьФорму(НастройкиОтчета.ПолноеИмя + ".ФормаНастроек", ПараметрыФормы, ЭтотОбъект,,,, Обработчик);

	Если ВыполнятьЗамеры И Форма <> Неопределено Тогда
		МодульОценкаПроизводительностиКлиент.ЗавершитьЗамерВремени(ИдентификаторЗамера);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			...
			ПутьКДаннымПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1.%2", ПолеПоиска.Значение, Выборка.ПолеПоУсловиюПредставление);
...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоискаСтроки(ПодходящиеЗначения, ПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Для Каждого ПолеПоиска Из ПоляПоиска Цикл
		...
		Для Каждого ПодходящееЗначение Из ПодходящиеЗначения Цикл
			Если СтрСравнить(ПодходящееЗначение.Представление, ПредставлениеРезультата) = 0 Тогда
...
Функция ТекстЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	|	&ПредставлениеПоляПоУсловию КАК ПолеПоУсловиюПредставление,
	...
	ПоляПоУсловию = ПоляТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска);
	
	ТекстЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&ПолноеИмяОбъекта", Объект.ПолноеИмя());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеПоУсловию", ПоляПоУсловию.Значение);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПредставлениеПоляПоУсловию", ПоляПоУсловию.Представление);
...
//
&НаСервереБезКонтекста
Функция ПоляТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	
	Результат = Новый Структура("Значение, Представление", """""", """""");

	ПоляПоУсловию = Новый Массив;
	ПредставленияПолейПоУсловию = Новый Массив;
	Поля = Объект.ВводПоСтроке; // СписокПолей

	ФрагментЗапроса = "КОГДА ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 СПЕЦСИМВОЛ ""~"" ТОГДА ""%3"""; // @query-part
	СтрокаПоиска = """%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(ПараметрыПоиска.СтрокаПоискаНормализованная) + "%""";
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ФрагментЗапроса, 
			Поле.Имя, СтрокаПоиска, Поле.Имя);
		ПоляПоУсловию.Добавить(ПолеПоУсловию);
		
		ПредставлениеПоляПоУсловию = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ФрагментЗапроса,
			Поле.Имя, СтрокаПоиска, ПредставлениеПоля(Поле.Имя, Объект));
		ПредставленияПолейПоУсловию.Добавить(ПредставлениеПоляПоУсловию);

	КонецЦикла;

	Если ПоляПоУсловию.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;

	ПоляПоУсловию.Вставить(0, "ВЫБОР"); // @query-part
	ПоляПоУсловию.Добавить("ИНАЧЕ """""); // @query-part
	ПоляПоУсловию.Добавить("КОНЕЦ"); // @query-part
	Результат.Значение = СтрСоединить(ПоляПоУсловию, " ");
	
	ПредставленияПолейПоУсловию.Вставить(0, "ВЫБОР"); // @query-part
	ПредставленияПолейПоУсловию.Добавить("ИНАЧЕ """""); // @query-part
	ПредставленияПолейПоУсловию.Добавить("КОНЕЦ"); // @query-part
	Результат.Представление = СтрСоединить(ПредставленияПолейПоУсловию, " ");
...
Процедура ПрименитьЗначениеУниверсальногоПоиска(ВариантыПоиска)
	...
	Если Не ЭтоПользовательскийПоиск Тогда
		...
		РезультатНастройки = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета();
		РезультатНастройки.КомпоновщикНастроекКД = КомпоновщикНастроек;
		РезультатНастройки.ПользовательскиеНастройкиМодифицированы = Истина;
		ПодключитьОбработчикОжидания("ОбновитьЭлементыФормыНастроекОтложенно", 0.1, Истина);
	КонецЕсли;
...
Процедура УстановитьКлючТекущегоВарианта(ОтчетПолноеИмя, ОтчетОбъект)
	...
	Если ТипЗнч(Расшифровка) = Тип("ОписаниеОбработкиРасшифровкиКомпоновкиДанных") Тогда
		ДанныеРасшифровки = ПолучитьИзВременногоХранилища(Расшифровка.Данные);
		Если ТипЗнч(ДанныеРасшифровки) = Тип("ДанныеРасшифровкиКомпоновкиДанных") Тогда
			Настройки = ДанныеРасшифровки.Настройки;
...
&НаСервере
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления)
	ЗагрузитьНастройкиВКомпоновщик(ПараметрыОбновления);

	ОтчетыСервер.ОбновитьЭлементыФормыНастроек(ЭтотОбъект, Элементы.КомпоновщикНастроекПользовательскиеНастройки, 
		ПараметрыОбновления);

	Если ПараметрыОбновления.ИмяСобытия <> "ПослеФормирования" Тогда
		Если ПараметрыОбновления.Переформировать И Не ПроверитьЗаполнение() Тогда
			ПараметрыОбновления.Переформировать = Ложь;
		ИначеЕсли ПараметрыОбновления.Переформировать Тогда
			ОтчетыКлиентСервер.ОтобразитьСостояниеОтчета(
				ЭтотОбъект, НСтр("ru = 'Отчет формируется...'"), БиблиотекаКартинок.ДлительнаяОперация48);
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	Если ПараметрыЗагрузки.ИмяСобытия <> ОтчетыКлиентСервер.ИмяСобытияНастройкиПоУмолчанию() 
		И Не ПараметрыЗагрузки.ВариантМодифицирован Тогда
	...
	Если ПараметрыЗагрузки.СброситьНастройкиВарианта Тогда
	...
	Если ПараметрыЗагрузкиПередУстановкойТекущегоВарианта <> Неопределено Тогда
		ПараметрыЗагрузки.СброситьПользовательскиеНастройки = 
	...
	Если ПараметрыЗагрузки.ИмяСобытия = ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияФормыНастроек() 
		И Не ПустаяСтрока(ПараметрыЗагрузки.ФормаНастроекИмяСтраницы) Тогда
...
#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
	...
	ПараметрыЗаполнения = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета(ОтчетыКлиентСервер.ИмяСобытияНастройкиПоУмолчанию());
	ПараметрыЗаполнения.ПользовательскиеНастройкиМодифицированы = Истина;
	ПараметрыЗаполнения.КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
	ОбновитьЭлементыФормыНастроек(ПараметрыЗаполнения);
#КонецЕсли
...
Процедура УточнитьПризнакАвтоформированияОтчета(ПараметрыОбновления)
	...
	Если Не ПараметрыОбновления.Переформировать Тогда
		...
		ПараметрыОбновления.Переформировать = (ВремяФормированияОтчета <= ДопустимоеВремяДляАвтоформированияОтчета);
	КонецЕсли;
