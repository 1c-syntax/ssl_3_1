...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиОтчета, "ВыводитьСуммуВыделенныхЯчеек", Истина) Тогда
		...
		Элементы.ГруппаВидыПоказателейЕще.Видимость = Ложь;
...
Процедура ПослеФормирования(Результат, ЗагрузитьРезультатФормированияОтчета) Экспорт 
	...
	ИнтеграцияПодсистемБСПКлиент.ПослеФормирования(ЭтотОбъект, Результат.Статус = "Выполнено");
...
Процедура ПоказатьСписокВыбора(Элемент, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("ЗначенияДляВыбора", ЗначенияДляВыбора(Сведения));
	...
	Обработчик = Новый ОписаниеОповещения("ЗавершитьВыборИзСписка", ЭтотОбъект, Сведения.ЭлементПользовательскойНастройки);
	Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
...
&НаКлиенте
Функция ЗначенияДляВыбора(СведенияОЭлементеНастройки)
	ЗначенияДляВыбора = СведенияОЭлементеНастройки.Описание.ДоступныеЗначения;
	Если ЗначенияДляВыбора = Неопределено Тогда 
		ЗначенияДляВыбора = Новый СписокЗначений;
	КонецЕсли;
	
	Идентификатор = СведенияОЭлементеНастройки.ЭлементПользовательскойНастройки.ИдентификаторПользовательскойНастройки;
	...
	ЗначениеОтбора = КэшЗначенийОтборов.Получить(Идентификатор);
	Если ЗначениеОтбора <> Неопределено Тогда 
		ЗначенияДляВыбора = ЗначениеОтбора;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗавершитьВыборИзСписка(Список, ЭлементПользовательскойНастройки) Экспорт
...
Процедура РассчитатьПоказатели(ТекущаяКоманда = "РассчитатьСумму")
	...
	Иначе
		РасчетныеПоказатели = ОбщегоНазначенияСлужебныйКлиентСервер.РасчетныеПоказателиЯчеек(
			ОтчетТабличныйДокумент, ПараметрыРасчета.ВыделенныеОбласти);
	КонецЕсли;
...
Функция РасчетныеПоказателиСервер(ПараметрыРасчета)
	Возврат ОбщегоНазначенияСлужебныйКлиентСервер.РасчетныеПоказателиЯчеек(ОтчетТабличныйДокумент, ПараметрыРасчета.ВыделенныеОбласти);
КонецФункции
...
Процедура ПрименитьПредопределенныеНастройки(Настройки)
	...
	ЭтоПредопределенный = Не НастройкиОтчета.Пользовательский
