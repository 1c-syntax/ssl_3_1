...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	ВосстановитьВыбранныйУровеньГруппировок();
КонецПроцедуры
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	СохранитьВыбранныйУровеньГруппировок();
...
Функция ПодходящиеЗначенияУниверсальногоПоиска(Запрос)
	...
	ИспользуемыеПоляПоискаПоТипам = НастройкиОтчета.ИспользуемыеПоляУниверсальногоПоискаПоТипам;
	
	Если ИспользуемыеПоляПоискаПоТипам.Количество() = 0 Тогда 
		Возврат ПодходящиеЗначения;
	КонецЕсли;
	
	Если ПараметрыУниверсальногоПоискаПрименимыКСсылкам(ПараметрыПоиска) Тогда 
		НайтиПодходящиеЗначенияУниверсальногоПоиска(ПодходящиеЗначения, ИспользуемыеПоляПоискаПоТипам, ПараметрыПоиска);
	КонецЕсли;
	
	НайтиПодходящиеЗначенияУниверсальногоПоискаЧисла(ПодходящиеЗначения, ПараметрыПоиска);
	
	Если ПодходящиеЗначения.Количество() = 0
		И СтрДлина(ПараметрыПоиска.СтрокаПоиска) > 2 Тогда 
		
		Представление = Новый ФорматированнаяСтрока(НСтр("ru = 'Совпадений не найдено'"),, WebЦвета.НейтральноСерый);
		ПодходящиеЗначения.Добавить("", Представление);
...
&НаКлиенте
Процедура НайтиПодходящиеЗначенияУниверсальногоПоискаЧисла(ПодходящиеЗначения, ПараметрыПоиска)
	...
	ИспользуемыеПоляПоискаПоТипам = НастройкиОтчета.ИспользуемыеПоляУниверсальногоПоискаПоТипам;
	ПоляПоиска = ИспользуемыеПоляПоискаПоТипам[Тип("Число")];
	
	ОписаниеЧисла = Новый ОписаниеТипов("Число");
	ЗначениеПоиска = ОписаниеЧисла.ПривестиЗначение(ПараметрыПоиска.СтрокаПоиска);
	
	Для Каждого ПолеПоиска Из ПоляПоиска Цикл 
		
		СвойстваПоиска = СвойстваУниверсальногоПоиска(ПолеПоиска.Ключ, ЗначениеПоиска, ПараметрыПоиска.УправляющиеСимволы);
		...
		ПодходящиеЗначения.Добавить(ВариантыПоиска, ПредставлениеПоиска);
...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоиска(ПодходящиеЗначения, ИспользуемыеПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Для Каждого Элемент Из ИспользуемыеПоляПоискаПоТипам Цикл 
		...
		ТекстЗапроса = ТекстЗапросаПоОбъектуПоиска(ОбъектМетаданных, ПараметрыПоиска.СтрокаПоиска);
	...
	Разделитель = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1ОБЪЕДИНИТЬ ВСЕ%2'"), Символы.ПС, Символы.ПС);
	...
	Пока Выборка.Следующий() Цикл 
		...
		ПоляПоиска = ИспользуемыеПоляПоискаПоТипам[ТипЗнч(Выборка.Ссылка)];
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл 
			...
			Если ВариантыПоиска = Неопределено Тогда 
				...
				ПредставлениеПоиска = ПредставлениеУниверсальногоПоискаСсылки(
					Выборка, СвойстваПоиска.ВидСравнения, ПараметрыПоиска.СтрокаПоиска);
			...
			СвойстваПоиска = СвойстваУниверсальногоПоиска(РеквизитПоля, ПараметрыПоиска.СтрокаПоиска, ПараметрыПоиска.УправляющиеСимволы);
			ПредставлениеПоиска = ПредставлениеЗначенияУниверсальногоПоискаСтроки(
...
&НаСервереБезКонтекста
Функция ТекстЗапросаПоОбъектуПоиска(Объект, СтрокаПоиска)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеПоУсловию", ПолеТекстаЗапросаПоОбъектуПоиска(Объект, СтрокаПоиска));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Условие", УсловиеТекстаЗапросаПоОбъектуПоиска(Объект, СтрокаПоиска));
...
//
&НаСервереБезКонтекста
Функция ПолеТекстаЗапросаПоОбъектуПоиска(Объект, СтрокаПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'КОГДА %1 ПОДОБНО %2 ТОГДА ""%3""'"),
			Поле.Имя,
			"""%" + СтрокаПоиска + "%""",
			Поле.Имя);
	...
	ОписаниеПоляПоУсловию.Вставить(0, НСтр("ru = 'ВЫБОР'"));
	ОписаниеПоляПоУсловию.Добавить(НСтр("ru = 'ИНАЧЕ """"'"));
	ОписаниеПоляПоУсловию.Добавить(НСтр("ru = 'КОНЕЦ'"));
...
//
&НаСервереБезКонтекста
Функция УсловиеТекстаЗапросаПоОбъектуПоиска(Объект, СтрокаПоиска)
...
&НаСервереБезКонтекста
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, Условие, СтрокаПоиска)
	...
	ФрагментСтрокиПоиска = ФрагментСтрокиПоиска(
		Сред(ПредставлениеЗначения, НачалоСтрокиПоиска, СтрДлина(СтрокаПоиска)));
...
Функция ФрагментУсловия(Условие, ОтступДоУсловия = " ", ОтступПослеУсловия = " ")
	...
	ШаблонФрагментаУсловия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1<span style=""color: СкрытыйВариантОтчетаЦвет"">%2</span>%3'"),
		ОтступДоУсловия,
		НРег(Условие),
		ОтступПослеУсловия);
...
Функция ФрагментСтрокиПоиска(СтрокаПоиска)
	...
	ШаблонФрагментаСтрокиПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '<span style=""color: МоиВариантыОтчетовЦвет; font: ВажнаяНадписьШрифт"">%1</span>'"), СтрокаПоиска);
...
&НаСервере
Процедура НайтиПоляУниверсальногоПоискаПоТипам()
	
	ИспользуемыеПоляПоискаПоТипам = Новый Соответствие;
	
	Настройки = Отчет.КомпоновщикНастроек.Настройки;
	
	Коллекции = Новый Массив;
	Коллекции.Добавить(Настройки.ДоступныеПоляОтбора.Элементы);
	...
	Для Каждого ПолеПараметровДанных Из ПоляПараметровДанных Цикл 
		Коллекции.Добавить(ПолеПараметровДанных.Элементы);
	КонецЦикла;
	...
	Для Каждого Коллекция Из Коллекции Цикл 
		...
		Для Каждого ОписаниеПоля Из Коллекция Цикл 
			...
			Для Каждого Тип Из Типы Цикл
				...
				ПоляПоТипу = ИспользуемыеПоляПоискаПоТипам[Тип];
				...
				ИспользуемыеПоляПоискаПоТипам.Вставить(Тип, ПоляПоТипу);
	...
	НастройкиОтчета.ИспользуемыеПоляУниверсальногоПоискаПоТипам = ИспользуемыеПоляПоискаПоТипам;
...
Процедура ИнициализироватьМенюУровнейГруппировок()
	...
	Если ИндексСтруктурыОтчета <> Неопределено
		И ИндексСтруктурыОтчета.Количество() > 0 Тогда 
...
Функция ГруппировкиОтчетаСУчетомПоляСИерархическойГруппировкой(ИндексСтруктурыОтчета, ПоляСИерархическойГруппировкой, КоличествоУровнейГруппировок)
	...
	ГруппировкиОтчета = ИндексСтруктурыОтчета.Скопировать();
...
Процедура ПоказатьВыбранныйУровеньГруппировок(УровеньГруппировки = Неопределено)
	...
	Если УровеньГруппировки = Неопределено Тогда 
		УровеньГруппировки = ВыбранныйУровеньГруппировок;
	КонецЕсли;
...
&НаСервере
Процедура СохранитьВыбранныйУровеньГруппировок()
	...
	ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить(
		КлючОбъектаВыбранногоУровняГруппировок(), "ВыбранныйУровеньГруппировок", ВыбранныйУровеньГруппировок);
	
КонецПроцедуры

&НаСервере
Процедура ВосстановитьВыбранныйУровеньГруппировок()
	...
	СохраненноеЗначение = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(
		КлючОбъектаВыбранногоУровняГруппировок(), "ВыбранныйУровеньГруппировок");
	
	Если ЗначениеЗаполнено(СохраненноеЗначение) Тогда 
		ВыбранныйУровеньГруппировок = СохраненноеЗначение;
	Иначе
		НеопределенныйУровеньГруппировок = 999;
...
&НаСервере
Функция КлючОбъектаВыбранногоУровняГруппировок()
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"%1/%2", НастройкиОтчета.ПолноеИмя, КлючТекущегоВарианта);
...
Процедура ЗагрузитьВариант(КлючВарианта)
	Если Не РежимРасшифровки И Не ВариантМодифицирован Тогда
		КлючОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1/%2/ТекущиеПользовательскиеНастройки'"), НастройкиОтчета.ПолноеИмя, КлючТекущегоВарианта);
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления = Неопределено)
	...
	НайтиПоляУниверсальногоПоискаПоТипам();
	УстановитьВидимостьДоступность();
КонецПроцедуры
