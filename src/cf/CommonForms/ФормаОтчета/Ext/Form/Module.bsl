...
Функция ПараметрыУниверсальногоПоиска(Запрос)
	...
	ПараметрыПоиска.Вставить("ЧислоПоиска", 0);
...
Функция ПолеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'КОГДА ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 ТОГДА ""%3""'"),
			Поле.Имя,
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если ТипРеквизита.СодержитТип(Тип("Строка")) Тогда 
			...
			Условие = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2'"),
				Поле.Имя,
				"""%" + ПараметрыПоиска.СтрокаПоиска + "%""");
		...
		ИначеЕсли ТипРеквизита.СодержитТип(Тип("Число")) Тогда 
			...
			Условие = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'ВЫРАЗИТЬ(%1 КАК ЧИСЛО) = %2'"),
				Поле.Имя,
				ПараметрыПоиска.ЧислоПоиска);
