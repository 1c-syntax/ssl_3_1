...
Функция ПодходящиеЗначенияУниверсальногоПоиска(Запрос)
	...
	Если ВариантМодифицирован
		Или ИспользуемыеПоляПоискаПоТипам.Количество() = 0 Тогда 
		
		НайтиПоляУниверсальногоПоискаПоТипам(Отчет.КомпоновщикНастроек, НастройкиОтчета);
		ИспользуемыеПоляПоискаПоТипам = НастройкиОтчета.ИспользуемыеПоляУниверсальногоПоискаПоТипам;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура НайтиПоляУниверсальногоПоискаПоТипам(КомпоновщикНастроек, НастройкиОтчета)
