...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.ПриОбновлении Тогда
		Элементы.ИнформацияОписание.Заголовок = НСтр("ru = 'Установленные исправления вступят в силу после перезапуска приложения.'");
	...
	Элементы.ПоискИУстановкаОбновлений.Видимость = Не ЭтоВебКлиент
		И ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы");
...
Процедура ОбновитьСписокИсправлений()
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		Исправление = УстановленныеИсправления.Добавить();
		Исправление.Имя = Расширение.Имя;
		Исправление.КонтрольнаяСумма = Base64Строка(Расширение.ХешСумма);
		Исправление.ИдентификаторРасширения = Расширение.УникальныйИдентификатор;
		Исправление.Подключать = Расширение.Активно;
		Исправление.Версия = Расширение.Версия;
		Если СвойстваИсправления = "ОшибкаЧтения" Тогда
			Исправление.Статус = 0;
		ИначеЕсли СвойстваИсправления <> Неопределено Тогда
			Исправление.Статус = 0;
			Исправление.Описание = СвойстваИсправления.Description;
			Исправление.ПрименимоДля = ПрименимостьДляКонфигураций(СвойстваИсправления);
		Иначе
			Описание = ОписаниеИсправлений[Расширение.Имя];
			Исправление.Статус = 1;
			Исправление.Описание = Описание;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПрименимостьДляКонфигураций(СвойстваИсправления)
	
	ИменаКонфигураций = Новый Массив;
	Для Каждого ИмяКонфигурации Из СвойстваИсправления.AppliedFor Цикл
		ИменаКонфигураций.Добавить(ИмяКонфигурации.ConfigurationName);
	КонецЦикла;
	
	Возврат СтрСоединить(ИменаКонфигураций, Символы.ПС);
...
Функция ИдентификаторыПодключенныхИсправлений()
	...
	Для Каждого Исправление Из УстановленныеИсправления Цикл
		Если Не Исправление.Подключать Тогда
			Продолжить;
		КонецЕсли;
		
		Подключенные.Добавить(Исправление.ПолучитьИдентификатор());
	КонецЦикла;
