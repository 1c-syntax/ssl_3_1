...
Функция НайтиКонтакты()
	...
	Возврат Взаимодействия.НайтиКонтакты(СтрокаПоиска, ЭтотОбъект);
...
Функция КонтактыНайденныеПоСтроке()
	...
	Возврат Взаимодействия.ПолнотекстовыйПоискКонтактовПоСтроке(СтрокаПоиска, ЭтотОбъект, Истина);
...
Процедура НайтиПоНачалуНаименования()
	...
	Взаимодействия.НайтиКонтактыСАдресамиПоНаименованию(СтрокаПоиска, ЭтотОбъект);
...
Процедура ДобавитьПолучателя(Адрес, Наименование, Контакт, СписокАдресов = "")
	...
	Для Индекс = 0 По ПолучателиПисьма.Количество() - 1 Цикл
		ПолучательПисьма = ПолучателиПисьма[ПолучателиПисьма.Количество() - Индекс - 1];
		Если ПустаяСтрока(ПолучательПисьма.Адрес) И ПустаяСтрока(ПолучательПисьма.Представление) И Не ЗначениеЗаполнено(ПолучательПисьма.Контакт) Тогда
			ПолучателиПисьма.Удалить(Индекс);
		КонецЕсли;	
	КонецЦикла;	
