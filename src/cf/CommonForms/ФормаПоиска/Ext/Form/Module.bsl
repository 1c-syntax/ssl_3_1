...
Процедура ПриВыполненииПоискаНаСервере(КонтейнерСостояний, Знач СтрокаПоиска, НаправлениеПоиска = "ПерваяЧасть")
...
Функция ПредставлениеОбластиПоиска(КонтейнерСостояний)
	...
	Если ОбластиПоиска.Количество() < 5 Тогда
		...
		Для каждого Область Из ОбластиПоиска Цикл
			...
			ПредставлениеОбластиПоиска = ПредставлениеОбластиПоиска + ОбщегоНазначения.ПредставлениеСписка(ОбъектМетаданных) + ", ";
		КонецЦикла;
