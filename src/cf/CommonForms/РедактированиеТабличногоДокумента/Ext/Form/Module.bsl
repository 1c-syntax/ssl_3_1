...
Процедура УстановитьПримерыЗначений(КоллекцияПолей = Неопределено, ДанныеПечати = Неопределено)
	...
	Для Каждого Элемент Из КоллекцияПолей.ПолучитьЭлементы() Цикл
		...
		Если Не Элемент.Папка И Не Элемент.Таблица Тогда
			...
			Элемент.Значение = ПримерыЗначений[Элемент.ПутьКДанным];
			Элемент.Образец = ПримерыЗначений[Элемент.ПутьКДанным];
			ИмяТаблицыПоля = ИмяТаблицыПоля(Элемент);
			Если ЗначениеЗаполнено(ИмяТаблицыПоля)
				И ПримерыЗначений[ИмяТаблицыПоля] <> Неопределено И ПримерыЗначений[ИмяТаблицыПоля].Количество() > 0 Тогда
				Элемент.Значение = ПримерыЗначений[ИмяТаблицыПоля][1][Элемент.ПутьКДанным];
				Элемент.Образец = ПримерыЗначений[ИмяТаблицыПоля][1][Элемент.ПутьКДанным];
			КонецЕсли;
		КонецЕсли;
		...
		УстановитьПримерыЗначений(Элемент, ДанныеПечати);
	КонецЦикла;
...
&НаСервере
Функция ПодготовитьМакетДляЗаписи(УстановитьКодЯзыка = Истина)
	...
	Для НомерСтроки = 1 По Макет.ВысотаТаблицы Цикл
		Для НомерКолонки = 1 По Макет.ШиринаТаблицы Цикл
			...
			Если ЗначениеЗаполнено(Область.Текст) Тогда
				ЗаменитьПредставленияПараметров(Область.Текст);
			КонецЕсли;
			
		КонецЦикла;
	КонецЦикла;
	
	ЗаменитьПредставленияПараметров(Макет.ВерхнийКолонтитул.ТекстСлева);
	ЗаменитьПредставленияПараметров(Макет.ВерхнийКолонтитул.ТекстВЦентре);
	ЗаменитьПредставленияПараметров(Макет.ВерхнийКолонтитул.ТекстСправа);
	ЗаменитьПредставленияПараметров(Макет.НижнийКолонтитул.ТекстСлева);
	ЗаменитьПредставленияПараметров(Макет.НижнийКолонтитул.ТекстВЦентре);
	ЗаменитьПредставленияПараметров(Макет.НижнийКолонтитул.ТекстСправа);
	...
	Для Каждого Область Из Макет.Области Цикл
		Если ТипЗнч(Область) = Тип("ОбластьЯчеекТабличногоДокумента")
			...
			ЗаменитьПредставленияПараметров(Область.ПараметрРасшифровки);
		КонецЕсли;
	КонецЦикла;
...
// Параметры:
//  Поле - ДанныеФормыЭлементДерева
//
&НаСервере
Функция ИмяТаблицыПоля(Поле)
	...
	Если Родитель.Таблица Тогда
		Возврат Родитель.Имя;
	КонецЕсли;
	
	Возврат ИмяТаблицыПоля(Родитель);
