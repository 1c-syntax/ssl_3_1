...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтборПоВариантамНастроек = Параметры.ОтборПоВариантамНастроек;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеВидимостиПредупрежденийЗацикливания" Тогда
		
		ПослеЗакрытияФормыУправленияЦикличностиОбменов();
...
Процедура СписокПриложенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "СписокПриложенийТранспортКартинка" Тогда
		
		ТекущиеДанные = Элементы.СписокПриложений.ТекущиеДанные;
		
		Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторТранспорта) Тогда
			...
			НастройкиТранспорта = ТранспортСообщенийОбменаВызовСервера.НастройкиТранспорта(
				ТекущиеДанные.УзелИнформационнойБазы, ТекущиеДанные.ИдентификаторТранспорта); 
...
Процедура СоздатьНастройкуСинхронизации(Команда)
	...
	Если ОтборПоВариантамНастроек.Количество() Тогда
		
		ПараметрыФормыНовойСинхронизацииДанных = Новый Структура;
		ПараметрыФормыНовойСинхронизацииДанных.Вставить("ОтборПоВариантамНастроек", ОтборПоВариантамНастроек);
...
Процедура ОбработкаКомандыВыполненияСинхронизации(ТекущиеДанные, ИспользоватьДопОтборы = Ложь)
	...
	ОписаниеСтрокиПриложения.Вставить("ИдентификаторТранспорта",		ТекущиеДанные.ИдентификаторТранспорта);
	ОписаниеСтрокиПриложения.Вставить("ОбъектнаяКонвертацияXDTO",		ТекущиеДанные.ОбъектнаяКонвертацияXDTO);
...
&НаКлиенте
Процедура ОткрытьФормуРазбораРезультатовПообъектногоОбмена(ТекущиеДанные)
	
	Если ТекущиеДанные.КоличествоПредупрежденийПолучение = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("УзелИнформационнойБазы", ТекущиеДанные.УзелИнформационнойБазы);
	
	ОткрытьФорму("Обработка.РаботаСРезультатамиОбмена.Форма.РазборРезультатовПообъектногоОбмена", ПараметрыОткрытия
	             , ЭтотОбъект, ТекущиеДанные.УзелИнформационнойБазы,,,, РежимОткрытияОкнаФормы.Независимый);
...
Процедура ВыполнитьКомандуСПредварительнойПроверкой(КомандаСтрокой)
	...
	КомандыДляПроверкиАутентификации =
		"ВыполнитьСинхронизацию,
		|ВыполнитьСинхронизациюСДополнительнымиОтборами,
		|НачальнаяВыгрузкаДанных";
		
	Если СтрНайти(КомандыДляПроверкиАутентификации, КомандаСтрокой) > 0 Тогда
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Корреспондент", ТекущиеДанные.УзелИнформационнойБазы);
		ДополнительныеПараметры.Вставить("КомандаСтрокой", КомандаСтрокой);
		
		ПараметрыАутентификации = ТранспортСообщенийОбменаКлиент.ПараметрыАутентификации();
		ПараметрыАутентификации.Корреспондент = ТекущиеДанные.УзелИнформационнойБазы;
		ПараметрыАутентификации.ИдентификаторТранспорта = ТекущиеДанные.ИдентификаторТранспорта;
		
		ТребуетсяАутентификация = Ложь;
		ОповещениеОЗакрытии = Новый ОписаниеОповещения("ЗавершениеАутентификации", ЭтотОбъект, ДополнительныеПараметры);
	...
	ИначеЕсли КомандаСтрокой = "ОткрытьРезультатыСинхронизацииСобытияЗагрузки" Тогда
		...
		Если ТекущиеДанные.ОбъектнаяКонвертацияXDTO Тогда
		
			ОткрытьФормуРазбораРезультатовПообъектногоОбмена(ТекущиеДанные);
...
&НаКлиенте
Процедура ЗавершениеАутентификации(ДанныеАутентификации, ДополнительныеПараметры) Экспорт 
	...
	ТранспортСообщенийОбменаВызовСервера.УстановитьПарольСинхронизацииДанных(
		ДополнительныеПараметры.Корреспондент, ДанныеАутентификации);
	
	ВыполнитьКомандуСПредварительнойПроверкой(ДополнительныеПараметры.КомандаСтрокой);
...
Функция НастройкаСинхронизацииЗавершена(СтрокаПриложение, ДанныеДляЗавершенияНастройки = Неопределено)
	...
	ПассивныйРежим = ТранспортСообщенийОбмена.ПараметрТранспорта(СтрокаПриложение.ИдентификаторТранспорта, "ПассивныйРежим");
	...
	ДанныеДляЗавершенияНастройки.Вставить("ПассивноеПодключение",      ПассивныйРежим);
...
Процедура СинхронизацияДанных(ОписаниеСтрокиПриложения)
	...
	// Форма интерактивной синхронизации не актуальна для объектной конвертации, т.к. есть настройка действий
...
Процедура ОткрытьПомощникИнтерактивнойСинхронизации(ДополнительныеПараметры)
	...
	ПараметрыПомощника.Вставить("ОбъектнаяКонвертацияXDTO", ДополнительныеПараметры.ОбъектнаяКонвертацияXDTO);
...
Процедура ПредупреждениеОЗацикливанииОписаниеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "2" Тогда
		
		СкрытьПредупреждениеЗацикливанияОтПользователя();
...
&НаКлиенте
Процедура СкрытьПредупреждениеЗацикливанияОтПользователя()
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеПодтвержденияСкрытияПредупреждения", ЭтотОбъект);
	
	ТекстПредупреждения = НСтр("ru ='Функциональность контроля циклически настроенных синхронизаций расположена в каждой форме настроек синхронизации в меню Еще (см. команду ""Предупреждение цикличности настроек обменов"")'",
		ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	ТекстЗаголовка = НСтр("ru ='Скрытие предупреждения о цикличности настроек'",
		ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	ПоказатьПредупреждение(ОписаниеОповещения, ТекстПредупреждения, 30, ТекстЗаголовка);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеПодтвержденияСкрытияПредупреждения(Результат) Экспорт
	
	Элементы.ИнформационнаяПанельОбнаруженоЗацикливание.Видимость = Ложь;
...
Функция АктуализироватьИнформациюОПроблемахСинхронизацииДанных(УзелСинхронизации)
	...
	Возврат РегистрыСведений.РезультатыОбменаДанными.КоличествоПредупрежденийДетально(УзелСинхронизации, Истина);
...
Функция СостояниеСинхронизацииДанных(СтрокаПриложение)
	...
	Если Не СтрокаПриложение.НастройкаЗавершена Тогда
		...
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(СтрокаПриложение.ТекущееДействиеУзла) Тогда
			Состояние.Представление = СтрокаПриложение.ТекущееДействиеУзла;
			Состояние.Картинка = 4;
		ИначеЕсли СтрокаПриложение.ДатаНачалаПоследнейЗагрузки > СтрокаПриложение.ДатаОкончанияПоследнейЗагрузки Тогда
...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	Элементы.СписокПриложенийЗагрузитьПравилаСинхронизацииДанных.Доступность = Не МодельСервиса И ЕстьНастроенныеОбмены;
	...
	Элементы.ИнформационнаяПанельОбнаруженоЗацикливание.Видимость = ОбнаруженоЗацикливание И НЕ СкрытоПредупреждениеЗацикливания;
	...
	ИначеЕсли ОтключенныеСценарии.Количество() > 1 Тогда
		...
		Шаблон = НСтр("ru = 'Обнаружены <a href = ""%1"">сценарии</a>, которые были отключены из-за ошибок выполнения. Включите сценарий после устранения проблем.'");
		ТекстПредупреждения = СтрШаблон(Шаблон, "СписокСценариев");
	...
	Если Не ЭтоПолноправныйПользователь Тогда
		Элементы.СписокПриложенийПредставлениеДатыПоследнейУспешнойЗагрузки.ГиперссылкаЯчейки = Ложь;
		Элементы.СписокПриложенийПолучениеПредупреждения.ГиперссылкаЯчейки                    = Ложь;
	КонецЕсли;
...
Процедура ОбновитьСписокПриложений(ОбновитьПриложенияМоделиСервиса = Ложь)
	...
	Если ОтборПоВариантамНастроек.Количество() Тогда
		...
		УзлыИнформационнойБазы = ТаблицаМонитора.ВыгрузитьКолонку("УзелИнформационнойБазы");
		
		УзлыСРеквизитомВариантНастройки = Новый Массив;
		
		Для Каждого УзелИнформационнойБазы Из УзлыИнформационнойБазы Цикл
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УзелИнформационнойБазы, "ВариантНастройки") Тогда
				УзлыСРеквизитомВариантНастройки.Добавить(УзелИнформационнойБазы);
			КонецЕсли;
		КонецЦикла;
		
		ВариантыНастроекУзлов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(УзлыСРеквизитомВариантНастройки, "ВариантНастройки");
		
		УдаляемыеСтроки = Новый Массив;
		Для Каждого СтрокаМонитора Из ТаблицаМонитора Цикл
			
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаМонитора.УзелИнформационнойБазы, "ВариантНастройки") Тогда
				ВариантНастройки = ВариантыНастроекУзлов[СтрокаМонитора.УзелИнформационнойБазы];
			Иначе
				ВариантыНастроекОбмена = ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(СтрокаМонитора.ИмяПланаОбмена, "ВариантыНастроекОбмена");
				Если ВариантыНастроекОбмена.Количество() > 0 Тогда
					ВариантНастройки = ВариантыНастроекОбмена[0].ИдентификаторНастройки;
				Иначе
					УдаляемыеСтроки.Добавить(СтрокаМонитора);
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			
			Если ОтборПоВариантамНастроек.НайтиПоЗначению(ВариантНастройки) = Неопределено Тогда
				УдаляемыеСтроки.Добавить(СтрокаМонитора);
			КонецЕсли;
		...
		Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
			ТаблицаМонитора.Удалить(УдаляемаяСтрока);
		КонецЦикла;
	...
	Для Каждого СтрокаПриложение Из СписокПриложений Цикл
		...
		Иначе
			...
			СтрокаПриложение.ЗапускОбменаИзКорреспондента = ТранспортСообщенийОбмена.ПараметрТранспорта(СтрокаПриложение.ИдентификаторТранспорта, "ПассивныйРежим");
		...
		СтрокаПриложение.ТранспортКартинка = ТранспортСообщенийОбмена.ПараметрТранспорта(СтрокаПриложение.ИдентификаторТранспорта, "Картинка");
		...
		Если СтрокаПриложение.НастройкаЗавершена Тогда
			...
			СтрокаПриложение.КоличествоПредупрежденийПолучение = СтруктураЗаголовков.КоличествоПредупрежденийПолучения;
...
&НаСервере
Процедура ПослеЗакрытияФормыУправленияЦикличностиОбменов()
