...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	Элементы.ИнформационнаяПанельТребуетсяПерезагрузка.Видимость = ТребуетсяПерезагрузка И СписокПриложений.Количество() > 0;
	...
	Если ОтключенныеСценарии.Количество() = 1 Тогда
		...
		Шаблон = НСтр("ru = 'Обнаружен сценарий, который были отключен из-за ошибок выполнения: %1'");
		Текст = СтрШаблон(Шаблон, ОтключенныеСценарии[0].Значение);
		Элементы.ПредупреждениеОбОтключенныхСценарияхОписание.Заголовок = Текст;
		
	ИначеЕсли ОтключенныеСценарии.Количество() > 1 Тогда
		
		Шаблон = НСтр("ru = 'Обнаружены сценарии, которые были отключены из-за ошибок выполнения: %1'");
		МассивСценариев = ОтключенныеСценарии.ВыгрузитьЗначения();
		Текст = СтрШаблон(Шаблон, СтрСоединить(МассивСценариев, ", "));
		Элементы.ПредупреждениеОбОтключенныхСценарияхОписание.Заголовок = Текст;
