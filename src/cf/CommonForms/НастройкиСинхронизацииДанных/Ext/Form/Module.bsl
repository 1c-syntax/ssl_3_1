...
Процедура ОбработкаКомандыВыполненияСинхронизации(ТекущиеДанные, ИспользоватьДопОтборы = Ложь)
	...
	Если РезультатыПроверок.ПродолжитьНастройкуНовойСинхронизации Тогда
		...
		Если ТекущиеДанные.ВнешняяСистема
			И РезультатыПроверок.ФоновоеЗаданиеНастройки <> Неопределено Тогда
			
			ОписаниеСтрокиПриложения.Вставить("ОбменДаннымиСВнешнейСистемой", Истина);
			ОткрытьФормуОжиданияОкончанияНастройки(РезультатыПроверок.ФоновоеЗаданиеНастройки, ОписаниеСтрокиПриложения);
...
Функция СерверныеПроверкиПередВыполнениемКоманд(СтрокаПриложение, ДанныеДляЗавершенияНастройки, ВариантПроверки = "")
	...
	ФоновоеЗаданиеНастройки = ФоновоеЗаданиеВариантыНастроекОбменаДаннымиСВнешнимиСистемами(СтрокаПриложение.УзелИнформационнойБазы, УникальныйИдентификатор);
	РезультатыПроверок.Вставить("ФоновоеЗаданиеНастройки", ФоновоеЗаданиеНастройки);
...
Процедура ЗавершитьНастройкуСинхронизацииВДиалоге(ОписаниеСтрокиПриложения, ДанныеДляЗавершенияНастройки)
	...
	Если ОписаниеСтрокиПриложения.ЭтоНачальнаяВыгрузка Тогда
		...
		ТекстЧасть1 = НСтр("ru = 'Перед выгрузкой данных для сопоставления необходимо завершить настройку синхронизации.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	Иначе
		
		ТекстЧасть1 = НСтр("ru = 'Перед запуском синхронизации данных необходимо завершить ее настройку.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
