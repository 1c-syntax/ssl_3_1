...
Процедура ОбновитьКомандуРезультатовСинхронизации()
	...
	СтруктураЗаголовка.Вставить("Заголовок", НСтр("ru = 'Предупреждений нет'"));
	СтруктураЗаголовка.Вставить("Картинка", Новый Картинка);
	
	ЕстьПроблемы = Ложь;
	УзлыОбмена = МассивИспользуемыхУзлов(СписокПриложений);
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫБОР
	|		КОГДА РезультатыОбменаДанными.ПроблемныйОбъект ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ЕстьПроблемыОбменов
	|ИЗ
	|	РегистрСведений.РезультатыОбменаДанными КАК РезультатыОбменаДанными
	|ГДЕ
	|	НЕ РезультатыОбменаДанными.Пропущена
	|	И РезультатыОбменаДанными.УзелИнформационнойБазы В (&УзлыОбмена)
	...
	Если Выборка.Следующий() Тогда
		...
		ЕстьПроблемы = Выборка.ЕстьПроблемыОбменов;
	...
	Если НЕ ЕстьПроблемы Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВерсионированиеОбъектов") Тогда
			...
			ЕстьПроблемы = МодульВерсионированиеОбъектов.ЕстьПроблемыВерсийПриСинхронизацииДанных(УзлыОбмена);
	...
	Если ЕстьПроблемы Тогда
		...
		СтруктураЗаголовка.Вставить("Заголовок", НСтр("ru = 'Предупреждения (есть, требуется ознакомиться)'"));
		СтруктураЗаголовка.Вставить("Картинка", БиблиотекаКартинок.Предупреждение);
