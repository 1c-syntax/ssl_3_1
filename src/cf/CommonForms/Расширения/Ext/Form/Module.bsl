	...
	Если Не ТекущееРасширение.Подключать
	   И ЭтоРасширениеСДанными(ТекущееРасширение.ИдентификаторРасширения) Тогда
		...
		ПользователиСлужебныйКлиент.ПоказатьПредупреждениеБезопасности(Оповещение,
			ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередОтключениемРасширенияСДанными);
	Иначе
...
Процедура ПроверитьВозможностьПримененияВсехРасширений(Команда)
	...
	Если Результат.КоличествоПроблем = 0 Тогда
		...
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(Неопределено,
			НСтр("ru = 'Проверка возможности применения всех расширений пройдена успешно.'"),
...
Процедура УдалитьРасширения(ВыделенныеСтроки)
	...
	ВидПредупреждения = ?(ЕстьРасширениеСДанными(ИдентификаторыРасширений),
		ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередУдалениемРасширенияСДанными,
		ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередУдалениемРасширенияБезДанных);
	
	ПользователиСлужебныйКлиент.ПоказатьПредупреждениеБезопасности(Оповещение,
		ВидПредупреждения, ИдентификаторыРасширений.Количество() > 1);
...
Процедура ЗагрузитьРасширение(Знач ИдентификаторРасширения, МножественныйВыбор = Ложь)
	...
	ПользователиСлужебныйКлиент.ПоказатьПредупреждениеБезопасности(Оповещение,
		ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередДобавлениемРасширений);
...
Процедура ВыключитьПодключениеРасширений()
	...
	ИначеЕсли СинонимыРасширений.СДанными.Количество() > 0 Тогда
		...
		Если СинонимыРасширений.СДанными.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Среди отключаемых расширений есть расширение, хранящее данные в приложении, которые станут недоступными.
		...
		Иначе
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Среди отключаемых расширений есть расширения, хранящие данные в приложении, которые станут недоступными.
				           |Кроме того, некоторые данные самого приложения могут тоже стать недоступными для изменения.
				           |
