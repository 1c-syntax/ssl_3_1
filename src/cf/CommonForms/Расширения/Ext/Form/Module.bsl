...
Процедура СписокРасширенийПодключатьПриИзменении(Элемент)
	...
	ИдентификаторСтроки = ТекущееРасширение.ПолучитьИдентификатор();
	
	Если Не ТекущееРасширение.Подключать
	   И ЭтоРасширениеСДанными(ТекущееРасширение.ИдентификаторРасширения) Тогда
		
		Контекст = Новый Структура;
		Контекст.Вставить("ИдентификаторСтроки", ИдентификаторСтроки);
	...
	Иначе
		СписокРасширенийПодключатьПриИзмененииНаСервере(ИдентификаторСтроки);
	КонецЕсли;
		...
		Если ИдентификаторРасширения <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					ТекстОшибки);
			Иначе
				ОписаниеРасширения = Новый ОписаниеКонфигурации(НовыеДвоичныеДанные);
				Если ПрежнееИмяРасширения = ОписаниеРасширения.Имя Тогда
...
Процедура СписокРасширенийПодключатьПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	Если Расширение <> Неопределено Тогда
		...
		Если Расширение.Активно <> СтрокаСписка.Подключать Тогда
