...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("АдминистрированиеРасширенийКонфигурации", Метаданные) Тогда
		...
		Элементы.ФормаУстановленныеПатчи.Видимость = Ложь;
		Элементы.ПроверитьВозможностьПримененияВсехРасширений.Видимость = Ложь;
		Элементы.ПодменюВключитьРасширения.Видимость = Ложь;
		Элементы.ПодменюПередаватьВПодчиненныеУзлыРИБ.Видимость = Ложь;
		Элементы.ПодменюБезопасныйРежим.Видимость = Ложь;
		Элементы.УстановитьОтветственного.Видимость = Ложь;
		Элементы.СписокРасширенийОтветственный.Видимость = Ложь;
		Элементы.СписокРасширенийЕстьКомментарий.Видимость = Ложь;
		Элементы.СписокРасширенийКомментарий.Видимость = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура СписокРасширенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя="СписокРасширенийЕстьКомментарий" Тогда
		ТекущийЭлемент = Элементы.СписокРасширенийКомментарий;
	КонецЕсли;
...
&НаКлиенте
Процедура СписокРасширенийОтветственныйПриИзменении(Элемент)
	
	ТекущееРасширение = Элементы.СписокРасширений.ТекущиеДанные;
	СохранитьСвойстваРасширений(ТекущееРасширение.ИдентификаторРасширения, СвойстваРасширений(ТекущееРасширение));
	
КонецПроцедуры

&НаКлиенте
Процедура СписокРасширенийКомментарийПриИзменении(Элемент)
	
	ТекущееРасширение = Элементы.СписокРасширений.ТекущиеДанные;
	ТекущееРасширение.ЕстьКомментарий = ЗначениеЗаполнено(ТекущееРасширение.Комментарий);
	СохранитьСвойстваРасширений(ТекущееРасширение.ИдентификаторРасширения, СвойстваРасширений(ТекущееРасширение));
...
&НаКлиенте
Процедура ПроверитьВозможностьПримененияВсехРасширений(Команда)
	
	Результат = РезультатПроверкиВозможностиПримененияВсех();
	
	Если Результат.КоличествоПроблем = 0 Тогда
		ПараметрыФормыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыФормыВопроса.Картинка = БиблиотекаКартинок.ДиалогИнформация;
		ПараметрыФормыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		ПараметрыФормыВопроса.Заголовок = НСтр("ru = 'Результат проверки возможности применения всех расширений'");
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(Неопределено, НСтр("ru = 'Проверка возможности применения всех расширений пройдена успешно.'"),РежимДиалогаВопрос.ОК, ПараметрыФормыВопроса);
	Иначе
		Результат.ИнформацияОПроблемах.Показать(НСтр("ru = 'Результат проверки расширений.'"));
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВключитьРасширения(Команда)
	
	ПоказатьДлительнуюОперацию();
	ПодключитьОбработчикОжидания("ВключитьПодключениеРасширений", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыключитьРасширения(Команда)
	
	ПоказатьДлительнуюОперацию();
	ПодключитьОбработчикОжидания("ВыключитьПодключениеРасширений", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ВключитьПередачуВПодчиненныеУзлыРИБ(Команда)
	
	ПоказатьДлительнуюОперацию();
	ПодключитьОбработчикОжидания("ВключитьПередачуВПодчиненныеУзлыРИБЗавершение", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыключитьПередачуВПодчиненныеУзлыРИБ(Команда)
	
	ПоказатьДлительнуюОперацию();
	ПодключитьОбработчикОжидания("ВыключитьПередачуВПодчиненныеУзлыРИБЗавершение", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ВключитьБезопасныйРежим(Команда)
	
	ПоказатьДлительнуюОперацию();
	ПодключитьОбработчикОжидания("ВключитьБезопасныйРежимЗавершение", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыключитьБезопасныйРежим(Команда)
		
	ПоказатьДлительнуюОперацию();
	ПодключитьОбработчикОжидания("ВыключитьБезопасныйРежимЗавершение", 0.1, Истина);
...
Процедура УстановитьОтветственного(Команда)
	...
	ПараметрыФормы.Вставить("РасширенныйПодбор", Ложь);
	
	Оповещение = Новый ОписаниеОповещения("УстановитьОтветственногоПослеВыбора", ЭтотОбъект);
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	ОбновитьСвойстваРасширенийВСписке();
...
Процедура УдалитьРасширенияНаСервере(ИдентификаторыРасширений)
	...
	УдалитьСвойстваРасширений(ИдентификаторыРасширений);
...
&НаСервере
Процедура УдалитьСвойстваРасширений(ИдентификаторыРасширений)
	
	Для каждого ИдентификаторРасширения Из ИдентификаторыРасширений Цикл
		РегистрыСведений.СвойстваРасширений.УдалитьСвойстваРасширенияПоИдентификатору(ИдентификаторРасширения);
	КонецЦикла;
...
Процедура ЗагрузитьРасширениеЗавершение()
	...
	Попытка
		...
		Если РасширенияДляПовторногоДобавления.Количество() > 0 Тогда
...
Процедура ИзменитьРасширенияНаСервере(ПомещенныеФайлы, ИдентификаторыСтрок,
	...
	РасширенияДляПовторногоДобавления.Очистить();
	...
	Исключение
		...
		ИначеЕсли ЗначениеЗаполнено(ИмяФайлаДобавляемогоРасширения) Тогда
			...
			Если ЭтоОшибкаИсключительнойБлокировкиИнформационнойБазы(КраткоеПредставлениеОшибки) Тогда
				
				ИндексПервогоНедобавленногоРасширения = ПомещенныеФайлы.Найти(ПомещенныйФайл);
				Если ИндексПервогоНедобавленногоРасширения <> Неопределено Тогда
					РасширенияДляПовторногоДобавления.Очистить();
					Для Индекс = ИндексПервогоНедобавленногоРасширения По ПомещенныеФайлы.ВГраница() Цикл
						РасширенияДляПовторногоДобавления.Добавить(ПомещенныеФайлы.Получить(Индекс));
					КонецЦикла;
				КонецЕсли;
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Иначе
				...
				Если ВременноеРасширение = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Расширение с идентификатором %1 не существует в приложении, возможно оно было удалено в другом сеансе.'"),
...
Процедура УстановитьДоступностьКнопокКомандныхПанелей()
	...
	ИначеЕсли Элементы.СтраницыОбновление.ТекущаяСтраница = Элементы.СтраницаСписокРасширений Тогда 
		...
		ПодключитьОбработчикОжидания("УстановитьЗаголовокЭлементаКомментарий", 0.1, Истина);
...
&НаСервереБезКонтекста
Функция РезультатПроверкиВозможностиПримененияВсех()
	
	ИнформацияОПроблемахПрименения = РасширенияКонфигурации.ПроверитьВозможностьПримененияВсех();
	
	Результат = Новый Структура;
	Результат.Вставить("КоличествоПроблем", ИнформацияОПроблемахПрименения.Количество());
	Результат.Вставить("ИнформацияОПроблемах", Новый ТекстовыйДокумент);

	Если ИнформацияОПроблемахПрименения.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	ПроблемныеРасширения = Новый Соответствие;
	
	Для каждого Проблема Из ИнформацияОПроблемахПрименения Цикл
		ИнформацияОПроблеме = ПроблемныеРасширения.Получить(Проблема.Расширение);
		Если ИнформацияОПроблеме = Неопределено Тогда
			Текст = Проблема.Расширение.Синоним;
		Иначе
			Текст = ИнформацияОПроблеме;
		КонецЕсли;
		Текст = Текст + Символы.ПС + Символы.Таб + "(" + Строка(Проблема.Важность) + ")" + " " + Проблема.Описание;
		ПроблемныеРасширения.Вставить(Проблема.Расширение, Текст);
	КонецЦикла;
	
	Для каждого Расширение Из ПроблемныеРасширения Цикл
		Результат.ИнформацияОПроблемах.ДобавитьСтроку(Расширение.Значение);
	КонецЦикла;
...
&НаКлиенте
Процедура ВключитьБезопасныйРежимЗавершение()
	
	ОшибкиИзмененияРасширений = Новый Массив;
	ВключитьВыключитьБезопасныйРежимДляВыделенныхРасширений(Истина, ОшибкиИзмененияРасширений);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыключитьБезопасныйРежимЗавершение()
	
	ОшибкиИзмененияРасширений = Новый Массив;
	ВключитьВыключитьБезопасныйРежимДляВыделенныхРасширений(Ложь, ОшибкиИзмененияРасширений);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаСервере
Процедура ВключитьВыключитьБезопасныйРежимДляВыделенныхРасширений(Значение, ОшибкиИзмененияРасширений)
	
	Для Каждого ИдентификаторСтроки Из Элементы.СписокРасширений.ВыделенныеСтроки Цикл
		СтрокаСписка = СписокРасширений.НайтиПоИдентификатору(ИдентификаторСтроки);
		Если СтрокаСписка = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаСписка.БезопасныйРежимФлаг = Значение;
		...
		Исключение
			СтрокаСписка.БезопасныйРежимФлаг = Не СтрокаСписка.БезопасныйРежимФлаг;
			ОписаниеПроблемы = СтрокаСписка.Синоним + ": " + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ОшибкиИзмененияРасширений.Добавить(ОписаниеПроблемы);
		КонецПопытки;
...
&НаКлиенте
Процедура ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений)
	Если ОшибкиИзмененияРасширений.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИнформацияОбОшибках = "";
	Для Каждого СтрокаОшибка Из ОшибкиИзмененияРасширений Цикл
		ИнформацияОбОшибках = ?(ЗначениеЗаполнено(ИнформацияОбОшибках), ИнформацияОбОшибках + Символы.ПС + Символы.ПС + СтрокаОшибка, СтрокаОшибка);
	КонецЦикла;
	
	ПараметрыФормыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыФормыВопроса.Картинка = БиблиотекаКартинок.ДиалогВосклицание;
	ПараметрыФормыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	ПараметрыФормыВопроса.Заголовок = НСтр("ru = 'Не все расширения удалось перенастроить'");
	СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(Неопределено, ИнформацияОбОшибках,РежимДиалогаВопрос.ОК, ПараметрыФормыВопроса);
	
КонецПроцедуры

&НаКлиенте
Процедура ВключитьПередачуВПодчиненныеУзлыРИБЗавершение()
	
	ОшибкиИзмененияРасширений = Новый Массив;
	ВключитьВыключитьПередачуВПодчиненныеУзлыРИБ(Истина, ОшибкиИзмененияРасширений);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаКлиенте
Процедура ВыключитьПередачуВПодчиненныеУзлыРИБЗавершение()
	
	ОшибкиИзмененияРасширений = Новый Массив;
	ВключитьВыключитьПередачуВПодчиненныеУзлыРИБ(Ложь, ОшибкиИзмененияРасширений);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаСервере
Процедура ВключитьВыключитьПередачуВПодчиненныеУзлыРИБ(Значение, ОшибкиИзмененияРасширений)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.СписокРасширений.ВыделенныеСтроки Цикл
		...
		СтрокаСписка.ПередаватьВПодчиненныеУзлыРИБ = Значение;
		
		Расширение = НайтиРасширение(СтрокаСписка.ИдентификаторРасширения);
		
		Если Расширение = Неопределено
			Или Расширение.ИспользуетсяВРаспределеннойИнформационнойБазе = СтрокаСписка.ПередаватьВПодчиненныеУзлыРИБ Тогда
			Продолжить;
		КонецЕсли;
...
&НаКлиенте
Процедура ВключитьПодключениеРасширений()
	
	ОшибкиИзмененияРасширений = Новый Массив;
	ВключитьВыключитьПодключениеРасширений(Истина, ОшибкиИзмененияРасширений);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаКлиенте
Процедура ВыключитьПодключениеРасширений()
	
	СинонимыРасширений = СинонимыРасширенийВыделенныхРасширенийДляВыключения();
	Если СинонимыРасширений.СДанными.Количество() = 1 И СинонимыРасширений.БезДанных.Количество() = 0 Тогда
		
		ТекущееРасширение = Элементы.СписокРасширений.ТекущиеДанные;
		Если ТекущееРасширение = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ТекущееРасширение.Подключать = Ложь;
		Контекст = Новый Структура;
		Контекст.Вставить("ИдентификаторСтроки", ТекущееРасширение.ПолучитьИдентификатор());
	...
	ИначеЕсли СинонимыРасширений.СДанными.Количество() > 0 Тогда
		
		Если СинонимыРасширений.СДанными.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Среди отключаемых расширений есть расширение, хранящее данные в приложении,
			           |которые станут недоступными. Кроме того, некоторые данные самого приложения
			           |могут тоже стать недоступными для изменения.
			           |
			           |Расширение с данными:
			           | - %1'"),
				СинонимыРасширений.СДанными[0]);
		Иначе
			РасширенияСДаннымиТекст = СтрСоединить(СинонимыРасширений.СДанными, Символы.ПС + " - ");
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Среди отключаемых расширений есть расширения, хранящие данные в приложении,
			           |которые станут недоступными. Кроме того, некоторые данные самого приложения
			           |могут тоже стать недоступными для изменения.
			           |
			           |Расширения с данными:
			           | - %1'"),
				РасширенияСДаннымиТекст);
		КонецЕсли;
			
		ОбработчикЗавершения = Новый ОписаниеОповещения(
			"ВыключитьРасширенияПослеВопроса", ЭтотОбъект);
		
		Кнопки = Новый СписокЗначений;
		Если СинонимыРасширений.БезДанных.Количество() > 0 Тогда
			Кнопки.Добавить("ОтключитьТолькоБезДанных", НСтр("ru = 'Отключить только расширения без данных'"));
			
			Если СинонимыРасширений.БезДанных.Количество() = 1 Тогда
				РасширенияБезДанныхТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Расширение без данных:
			           | - %1'"),
					СинонимыРасширений.БезДанных[0]);
			Иначе
				РасширенияБезДанныхТекст = СтрСоединить(СинонимыРасширений.БезДанных, Символы.ПС + " - ");
				РасширенияБезДанныхТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Расширения без данных:
			           | - %1'"), 
					РасширенияБезДанныхТекст);
			КонецЕсли;
			ТекстВопроса = ТекстВопроса + Символы.ПС + Символы.ПС + РасширенияБезДанныхТекст;
		КонецЕсли;
		Кнопки.Добавить("ОтключитьВсе", НСтр("ru = 'Отключить все'"));
		Кнопки.Добавить("Отмена", НСтр("ru = 'Отмена'"));
		
		ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыВопроса.Заголовок = НСтр("ru = 'Предупреждение'");
		ПараметрыВопроса.Картинка = БиблиотекаКартинок.ДиалогВосклицание;
		ПараметрыВопроса.КнопкаПоУмолчанию = "ОтключитьТолькоБезДанных";
		ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	...
	Иначе
		ОшибкиИзмененияРасширений = Новый Массив;
		ВключитьВыключитьПодключениеРасширений(Ложь, ОшибкиИзмененияРасширений);
		ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
		ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
	КонецЕсли;
...
&НаСервере
Процедура ВключитьВыключитьПодключениеРасширений(Значение, ОшибкиИзмененияРасширений, ОбрабатыватьРасширенияСДанными = Истина)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.СписокРасширений.ВыделенныеСтроки Цикл
		...
		Если Не ОбрабатыватьРасширенияСДанными И ЭтоРасширениеСДанными(СтрокаСписка.ИдентификаторРасширения) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаСписка.Подключать = Значение;
		
	Попытка
		Справочники.ВерсииРасширений.ПереключитьИспользованиеРасширения(СтрокаСписка.ИдентификаторРасширения, Значение);
	Исключение
		СтрокаСписка.Подключать = Не СтрокаСписка.Подключать;
		ОписаниеПроблемы = СтрокаСписка.Синоним + ": " + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ОшибкиИзмененияРасширений.Добавить(ОписаниеПроблемы);
...
&НаСервере
Функция СинонимыРасширенийВыделенныхРасширенийДляВыключения()
	
	СДанными = Новый массив;
	БезДанных = Новый массив;
	
	Для Каждого ИдентификаторСтроки Из Элементы.СписокРасширений.ВыделенныеСтроки Цикл
		СтрокаСписка = СписокРасширений.НайтиПоИдентификатору(ИдентификаторСтроки);
		Если СтрокаСписка = Неопределено И СтрокаСписка.Подключать Тогда
			Продолжить;
		КонецЕсли;
		Если ЭтоРасширениеСДанными(СтрокаСписка.ИдентификаторРасширения) Тогда
			СДанными.Добавить(СтрокаСписка.Синоним);
		Иначе
			БезДанных.Добавить(СтрокаСписка.Синоним);
		КонецЕсли;
	КонецЦикла;
	
	СинонимыРасширений = Новый Структура;
	СинонимыРасширений.Вставить("СДанными", СДанными);
	СинонимыРасширений.Вставить("БезДанных", БезДанных);
	
	Возврат СинонимыРасширений;
	
КонецФункции

&НаКлиенте
Процедура ВыключитьРасширенияПослеВопроса(Результат, Контекст) Экспорт
	
	Если Результат = Неопределено Или Результат.Значение = "Отмена" Тогда
		ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
		Возврат;
	ИначеЕсли Результат.Значение = "ОтключитьТолькоБезДанных" Тогда
		ОбрабатыватьРасширенияСДанными = Ложь;
	ИначеЕсли Результат.Значение = "ОтключитьВсе" Тогда
		ОбрабатыватьРасширенияСДанными = Истина;
	КонецЕсли;
		
	ОшибкиИзмененияРасширений = Новый Массив;
	ВключитьВыключитьПодключениеРасширений(Ложь, ОшибкиИзмененияРасширений, ОбрабатыватьРасширенияСДанными);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаСервереБезКонтекста
Процедура СохранитьСвойстваРасширений(ИдентификаторРасширения, СвойстваРасширений)
	РегистрыСведений.СвойстваРасширений.СохранитьСвойстваРасширений(ИдентификаторРасширения, СвойстваРасширений);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция СвойстваРасширений(ТекущееРасширение)
	
	Свойства = Новый Структура;
	Свойства.Вставить("Ответственный", ТекущееРасширение.Ответственный);
	Свойства.Вставить("Комментарий", ТекущееРасширение.Комментарий);
...
&НаСервере
Процедура ОбновитьСвойстваРасширенийВСписке()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Расширения.ИдентификаторРасширения КАК ИдентификаторРасширения
		|ПОМЕСТИТЬ ИдентификаторыРасширений
		|ИЗ
		|	&СписокРасширений КАК Расширения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СвойстваРасширений.Ответственный КАК Ответственный,
		|	СвойстваРасширений.Комментарий КАК Комментарий,
		|	ИдентификаторыРасширений.ИдентификаторРасширения КАК ИдентификаторРасширения
		|ИЗ
		|	ИдентификаторыРасширений КАК ИдентификаторыРасширений
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СвойстваРасширений КАК СвойстваРасширений
		|		ПО ИдентификаторыРасширений.ИдентификаторРасширения = СвойстваРасширений.ИдентификаторРасширения";
	
	Запрос.УстановитьПараметр("СписокРасширений", СписокРасширений.Выгрузить());
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураДляПоиска.Вставить("ИдентификаторРасширения", Выборка.ИдентификаторРасширения);
		СтрокиРасширений = СписокРасширений.НайтиСтроки(СтруктураДляПоиска);
		ЗаполнитьЗначенияСвойств(СтрокиРасширений[0], Выборка);
		СтрокиРасширений[0].ЕстьКомментарий = ЗначениеЗаполнено(СтрокиРасширений[0].Комментарий);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьЗаголовокЭлементаКомментарий()
	
	Если Элементы.СписокРасширений.ТекущиеДанные = Неопределено Тогда
		 Возврат;
	КонецЕсли;
	
	Элементы.СписокРасширенийКомментарий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Комментарий для %1'"), Элементы.СписокРасширений.ТекущиеДанные.Синоним);
		
КонецПроцедуры

&НаКлиенте
Процедура УстановитьОтветственногоПослеВыбора(Ответственный, ДополнительныеПараметры) Экспорт
	
	Если Ответственный = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьОтветственногоДляВыделенныхРасширений(Ответственный);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтветственногоДляВыделенныхРасширений(Ответственный)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.СписокРасширений.ВыделенныеСтроки Цикл
		...
		Если СтрокаСписка = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СтрокаСписка.Ответственный = Ответственный;
		СохранитьСвойстваРасширений(СтрокаСписка.ИдентификаторРасширения, СвойстваРасширений(СтрокаСписка));
	КонецЦикла;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЭтоОшибкаИсключительнойБлокировкиИнформационнойБазы(ТекстОшибки)
	ТекстОшибкиИсключительнойБлокировки = НСтр("ru='Ошибка исключительной блокировки информационной базы'");
	Возврат (СтрНайти(ТекстОшибки, ТекстОшибкиИсключительнойБлокировки) <> 0);
КонецФункции
...
Процедура ОткрытьФормуЗавершенияРаботыПользователей()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
		...
		ПараметрыФормы = МодульСоединенияИБКлиент.ПараметрыОткрытияФормыОшибкиУстановкиМонопольногоРежима();
		КоличествоКЗагрузке = РасширенияДляПовторногоДобавления.Количество();
		ПараметрыФормы.Заголовок = ?(КоличествоКЗагрузке = 1,
			НСтр("ru = 'Не удалось добавить расширение'"), НСтр("ru = 'Не удалось добавить расширения'"));
		ПараметрыФормы.ТекстСообщенияОбОшибке = ?(КоличествоКЗагрузке = 1, 
			НСтр("ru = 'Не удалось добавить расширение, т.к. с приложением работают пользователи.'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось добавить расширения (%1), т.к. с приложением работают пользователи.'"), КоличествоКЗагрузке));
		ПараметрыФормы.ТекстСообщенияОбОшибкеЗавершенияРаботы = ?(КоличествоКЗагрузке = 1, 
			НСтр("ru = 'Невозможно выполнить добавление расширения, т.к. не удалось завершить работу пользователей:'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно выполнить добавление расширений (%1), т.к. не удалось завершить работу пользователей:'"), КоличествоКЗагрузке));
		ПараметрыФормы.СообщениеПриВходеВПриложение = НСтр("ru = 'Приложение заблокировано для добавления расширения.'");
		ПараметрыФормы.ЗавершатьВсеСеансыКромеТекущего = Истина;
		ПараметрыФормы.ЗавершатьСеансКонфигуратора = Истина;
		ПараметрыФормы.ПериодБлокировки = 60;
...
Процедура ПослеУстановкиМонопольногоРежима(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Ложь Тогда // Монопольный режим установлен.
		ПоказатьДлительнуюОперацию();
		ПодключитьОбработчикОжидания("ПовторноеДобавлениеРасширенияСДанными", 0.1, Истина);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПовторноеДобавлениеРасширенияСДанными()
	
	Если РасширенияДляПовторногоДобавления = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	РасширенияИзменены = Ложь;
	ПодтверждениеЗаменыИмени = Новый Структура("СтароеИмя, НовоеИмя", "", "");
	ИзменитьРасширенияНаСервере(РасширенияДляПовторногоДобавления.ВыгрузитьЗначения(), Неопределено, НеподключенныеРасширения,
