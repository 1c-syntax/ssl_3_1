...
&НаКлиенте
Процедура ВыбратьФайлы(МножественныйВыбор = Ложь)
...
Процедура ВыбратьФайлПослеСозданияМенеджераКриптографии(МенеджерКриптографии, Контекст) Экспорт
	...
	МенеджерКриптографии.НачатьПолучениеСертификатовИзПодписи(Новый ОписаниеОповещения(
		...
		"ВыбратьФайлПослеОшибкиПолученияСертификатовИзПодписи", ЭтотОбъект), Контекст.РезультатДобавленияПодписи.ДанныеПодписи);
...
Функция ДобавитьСтрокуНаСервере(Знач РезультатДобавленияСтроки, ДобавитьНовуюСтроку)
	...
	Если ПараметрыПодписи = Неопределено Тогда
		...
		Попытка
			...
			Сертификаты = МенеджерКриптографии.ПолучитьСертификатыИзПодписи(РезультатДобавленияСтроки.ДанныеПодписи);
