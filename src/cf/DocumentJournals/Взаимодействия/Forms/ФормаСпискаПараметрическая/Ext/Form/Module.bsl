	...
	Если ТипЗнч(Параметры.Отбор) = Тип("Структура") Тогда
		...
		Если Параметры.Отбор.Свойство("Предмет") Тогда
			...
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, 
				ОбщегоНазначения.ПредметСтрокой(ПредметДляОтбора));
			
		ИначеЕсли Параметры.Отбор.Свойство("Контакт") Тогда
			
			Контакт = Параметры.Отбор.Контакт;
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, 
				ОбщегоНазначения.ПредметСтрокой(Контакт));
...
Процедура ДеревоВзаимодействийПередУдалением(Элемент, Отказ)
	...
	ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВопросОПометкеНаУдалениеПослеЗавершения", 
...
//  Массив из ОпределяемыйТип.КонтактВзаимодействия
//
&НаСервере
	...
	Для каждого ЭлементМассиваОписания Из МассивОписанияКонтактов Цикл
		...
		Если ЭлементМассиваОписания.Имя = ИмяТаблицыКонтакта Тогда
			...
			Если НЕ ПустаяСтрока(Связь) Тогда
				...
				|	&ИмяТаблицы КАК СправочникКонтакт
				|ГДЕ
				|	СправочникКонтакт.ИмяРеквизитаСравнения = &Контакт"; 
				
				ИмяРеквизитаСравнения = Прав(Связь, СтрДлина(Связь) - СтрНайти(Связь, "."));
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяРеквизитаСравнения", 
					ИмяРеквизитаСравнения);
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяТаблицы", 
					"Справочник." + Лев(Связь, СтрНайти(Связь, ".") - 1));
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяРеквизитаСравнения", 
					"СправочникКонтакт." + ИмяРеквизитаСравнения);
...
Процедура ВопросОПометкеНаУдалениеПослеЗавершения(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ОбработатьИзменениеПометкиУдаленияВДереве(Элементы.ДеревоВзаимодействий.ВыделенныеСтроки, 
			ДополнительныеПараметры.ЕстьПомеченныеНаУдаление);
	КонецЕсли;
