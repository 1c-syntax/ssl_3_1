...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Закладки, "Владелец", 
	...
	ИмяТекущейПанелиНавигации = Параметры.ИмяТекущейПанелиНавигации;
	Если ЗначениеЗаполнено(Параметры.ТекущаяСтрока) Тогда
		ПодготовитьНастройкиФормыКВыводуТекущейСсылки(Параметры.ТекущаяСтрока);
	КонецЕсли;
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	Если Не ПустаяСтрока(Параметры.ИмяТекущейПанелиНавигации) Тогда
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ИначеЕсли ИмяТекущейПанелиНавигации = "СтраницаСвойства" Тогда
		Если ТаблицаДопРеквизитовСвойств.НайтиСтроки(
			Новый Структура("ДопРеквизитСведение", ТекущееСвойствоПанелиНавигации)).Количество() = 0 Тогда
			ИмяТекущейПанелиНавигации = "СтраницаПредмет";
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПустаяСтрока(ИмяТекущейПанелиНавигации) Или ПустаяСтрока(Статус) Или ПустаяСтрока(ТипВзаимодействия) Тогда
...
#Если МобильныйКлиент Тогда
	...
	Элементы.СписокСтатус.Вид            = ВидГруппыФормы.ГруппаКнопок;
	Элементы.СписокТипВзаимодействия.Вид = ВидГруппыФормы.ГруппаКнопок;
...
Процедура ПапкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для создания папки.'"), КатегорияОшибки.НарушениеПравДоступа);
	КонецЕсли;
...
Процедура СтрокаПоискаПриИзменении(Элемент)
	...
	Иначе 
		...
		Элементы.ПредупреждениеОНеактуальностиИндекса.Видимость = Ложь;
...
Процедура ПеренестиВПапку(Команда)
	...
	Если ИмяТекущегоЭлемента = "Папки" Тогда
		...
		ИначеЕсли ПапкиТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для изменения папок.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
	КонецЕсли;
...
#Область ОбработкаИзмененияБыстрыхОтборов
...
#Область ОбработкаАктивизацииСтрокСписковИПанелиНавигации
...
Процедура ОбработатьАктивизациюСтрокиПанелиНавигации();
	...
	Если Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.Контакт = ТекущееЗначениеПанелиНавигации Тогда
				Возврат;
			Иначе
				ТекущееЗначениеПанелиНавигации = ТекущиеДанные.Контакт; 
			КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПредмет Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.Предмет = ТекущееЗначениеПанелиНавигации Тогда
				Возврат;
			Иначе 
			
				ОчиститьПредпросмотр();

				ТекущееЗначениеПанелиНавигации = ТекущиеДанные.Предмет;
			...
			Если ТекущиеДанные = Неопределено
				Или ТекущиеДанные.Предмет <> ТекущееЗначениеПанелиНавигации Тогда
				
				Элементы.ПредметыПанельНавигации.ТекущаяСтрока = ТекущееЗначениеПанелиНавигации;
...
#Область ПереключениеИЗаполнениеПанелейНавигации
...
Процедура УстановитьЗаголовокВидаПанелиНавигации(ЗначениеОтбора = Неопределено)
	...
	Элементы.ГруппаПанельНавигации.Заголовок = ?(ПустаяСтрока(ЗаголовокПанелиНавигации), 
	                                           НСтр("ru = 'Отбор не установлен'"),
	                                           НСтр("ru = 'Установлен отбор'"));
...
Процедура ПослеЗаполненияПанелиНавигации(УстанавливатьПризнакНеОтрабатыватьАктивизациюПанелиНавигации = Истина)
	...
	ТекущееЗначениеПанелиНавигации = Неопределено;
	...
	Если Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
		...
		ТекущееЗначениеПанелиНавигации = ЗначениеНастроек.ТекущееЗначение;
...
Функция ЭтоПанельСДинамическимСписком(ИмяТекущейПанелиНавигации)
	...
	Возврат ИмяТекущейПанелиНавигации = "СтраницаПредмет" ИЛИ ИмяТекущейПанелиНавигации = "СтраницаКонтакт";
...
Процедура ПроверитьИндексПолнотекстовогоПоиска()
	...
	Если Не ИндексПолнотекстовогоПоискаАктуален Тогда 
		...
		Если ИнформационнаяБазаФайловая Тогда
			...
			ОповещениеПослеОтветаНаВопрос = Новый ОписаниеОповещения("ПроверитьИндексПолнотекстовогоПоискаЗавершение", ЭтотОбъект);
			
			ПоказатьВопрос(ОповещениеПослеОтветаНаВопрос, 
				НСтр("ru = 'Индекс полнотекстового поиска неактуален. Обновить индекс?'"), РежимДиалогаВопрос.ДаНет);
			
		Иначе
			
			Элементы.ПредупреждениеОНеактуальностиИндекса.Видимость = Истина;
