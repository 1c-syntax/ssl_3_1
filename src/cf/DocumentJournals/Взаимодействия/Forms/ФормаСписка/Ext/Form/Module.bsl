...
Функция ДобавитьВЗакладкиСервер(Знач ДанныеДляОбработки, ИмяЭлементаФормы)
	...
	Иначе
		...
		Если ИмяЭлементаФормы = "ПредметыПанельНавигации" Тогда
			...
			ИначеЕсли ДанныеДляОбработки.Значение = "НеУказан" Тогда
...
Процедура ВыполнитьПолнотекстовыйПоиск()
	...
	Если Не Результат.ЕстьОшибка  Тогда
		...
		ТекстОповещения = ?(ТолькоПочта, НСтр("ru = 'Найдено писем - %1.'"), НСтр("ru = 'Найдено взаимодействий - %1.'"));
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОповещения, Строка(Результат.КоличествоЭлементовНайдено));
