...
Процедура ПриОпределениеКомандТипаКонтактнойИнформации(КомандыТипа, Тип) Экспорт
	...
	// Локализация
	 Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		СвойстваКоманды = УправлениеКонтактнойИнформацией.СвойстваКоманды(
				НСтр("ru = 'Адрес на Яндекс.Картах'"),
				НСтр("ru = 'Показывает адрес на картах Яндекс.Карты'"),
				БиблиотекаКартинок.ЯндексКарты,
				"УправлениеКонтактнойИнформациейКлиентЛокализация.ПоказатьАдресНаКартеЯндекс");
		
		КомандыТипа.Вставить("ПоказатьНаКартеЯндекс",  СвойстваКоманды);
	КонецЕсли;
	// Конец Локализация
...
Процедура ПриКонвертированииКонтактнуюИнформациюИзJSONВXML(Знач КонтактнаяИнформацияВJSON, КонтактнаяИнформацияВXML, ОжидаемыйТип = Неопределено) Экспорт
	...
	// Локализация
	КонтактнаяИнформацияВXML = КонтактнаяИнформацияИзJSONВXML(КонтактнаяИнформацияВJSON, ОжидаемыйТип);
	// Конец Локализация
...
Процедура ПриКонвертированииКонтактнойИнформацииИзXML(Знач Текст, КонтактнаяИнформация, Знач ОжидаемыйВид = Неопределено, РезультатПреобразования = Неопределено, НастройкиКонвертации = Неопределено) Экспорт
	...
	// Локализация
	КонтактнаяИнформация = КонтактнаяИнформацияИзXML(Текст, ОжидаемыйВид,РезультатПреобразования, НастройкиКонвертации);
	// Конец Локализация
...
Процедура ПриКонвертированииКонтактнойИнформацииВСтруктуруJSON(КонтактнаяИнформация, КонтактнаяИнформацияJSON, Тип, НастройкиКонвертации) Экспорт
	...
	// Локализация
	КонтактнаяИнформацияJSON = КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Тип, НастройкиКонвертации);
	// Конец Локализация
...
Процедура ПриПриведенииКонтактнойИнформацииВXML(Знач Данные, Результат, СтандартнаяОбработка) Экспорт
	...
	Результат = ПривестиКонтактнуюИнформациюXML(Данные);
	// Конец Локализация
...
Процедура ПриКонвертацииТелефонаВОбъектXDTO(ЗначенияПолей, Результат, Представление = "", ОжидаемыйТип = Неопределено) Экспорт
	...
	// Локализация
	Результат =  ДесериализацияТелефонаФакса(ЗначенияПолей, Представление, ОжидаемыйТип);
	// Конец Локализация
...
Процедура ПриОпределенииТипаКонтактнойИнформации(Знач XMLСтрока, Тип) Экспорт
	// Локализация
	Тип = ЗначениеИзСтрокиXML(XSLT_ТипКонтактнойИнформацииПоСтрокеXML(XMLСтрока));
	// Конец Локализация
КонецПроцедуры
...
Процедура ПриПреобразованииКонтактнойИнформацииXDTOПоПредставлению(Текст, Результат, ОжидаемыйВид) Экспорт
	...
	// Локализация
	Результат = КонтактнаяИнформацияXDTOПоПредставлению(Текст, ОжидаемыйВид);
	// Конец Локализация
...
Процедура ПриПреобразованииКонтактнойИнформацииXDTOВXML(ОбъектXDTOИнформации, КонтактнаяИнформацияВXML) Экспорт
	...
	// Локализация
	КонтактнаяИнформацияВXML = КонтактнаяИнформацияXDTOВXML(ОбъектXDTOИнформации);
	// Конец Локализация
...
Процедура ПриПолученииСтрокиСоставаКонтактнойИнформации(XMLДанные, СтрокаСостава) Экспорт
	...
	// Локализация
	СтрокаСостава = СтрокаСоставаКонтактнойИнформации(XMLДанные);
	// Конец Локализация
...
Процедура ПриЗаполненииРеквизитовТабличнойЧастиДляВебСтраницы(Источник, Результат) Экспорт
	...
	// Локализация
	Результат = ЗаполнитьРеквизитыТабличнойЧастиДляВебСтраницы(Источник);
	// Конец Локализация
...
Процедура ПриОпределенииРазличияКонтактнойИнформации(Знач Данные1, Знач Данные2, Результат) Экспорт
	...
	Результат = ЗначениеИзСтрокиXML( XSLT_ИнтерпретацияРазличияXMLКонтактнойИнформации(
...
Процедура ПриПроверкеСтраныПослеПоискаСтраныПоКлассификатору(СтранаПоКлассификатору) Экспорт
	...
	// Локализация
	Если СтранаПоКлассификатору <> Неопределено 
		И (СтранаПоКлассификатору.Код = "897" Или СтранаПоКлассификатору.Код = "898") Тогда
		СтранаПоКлассификатору = Неопределено;
	КонецЕсли;
	// Конец Локализация
...
Процедура СкорректироватьКонтактнуюИнформацию(Форма, СтрокаКИ, РезультатПреобразования) Экспорт
	...
	Результат = КонтактнаяИнформацияИзXML(СтрокаКИ.ЗначенияПолей, 
	...
	Если РезультатПреобразования.СведенияИсправлены Тогда
		ПриПреобразованииКонтактнойИнформацииXDTOВXML(Результат, СтрокаКИ.ЗначенияПолей);
	КонецЕсли;
...
Функция КонтактнаяИнформацияИзXML(Знач Текст, Знач ОжидаемыйВид = Неопределено, РезультатПреобразования = Неопределено, НастройкиКонвертации = Неопределено) Экспорт
	...
	ИначеЕсли ОжидаемыйТип = ПеречислениеТелефон Тогда
		...
		Иначе
			Результат = ДесериализацияТелефонаФакса(Текст, Представление, ОжидаемыйТип)
		КонецЕсли;
...
Функция ЗаполнитьРеквизитыТабличнойЧастиДляВебСтраницы(Источник)
...
Функция АдресXMLВXDTO(Знач ЗначенияПолей, Знач Представление = "", Знач ОжидаемыйТип = Неопределено) Экспорт
	...
	Если Метаданные.Обработки.Найти("РасширенныйВводКонтактнойИнформации") <> Неопределено Тогда
		...
		Возврат МодульРасширенныйВводКонтактнойИнформации.АдресXMLВXDTO(ЗначенияПолей, Представление, ОжидаемыйТип);
	КонецЕсли;
...
Функция КонтактнаяИнформацияXDTOПоПредставлению(Текст, ОжидаемыйВид)
...
Функция КонвертироватьТелефонФаксИзJSONВXML(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректная контактная информация, ожидается телефон или факс.'");
	КонецЕсли;
...
Функция КонвертироватьПрочуюКонтактнуюИнформациюИзJSONВXML(ЗначенияПолей, Знач Представление = "", ОжидаемыйТип = Неопределено)
	...
	ИначеЕсли ОжидаемыйТип <> Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Некорректная контактная информация, ожидается другой тип.'");
...
Функция КонтактнаяИнформацияИзJSONВXML(Знач КонтактнаяИнформация, ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип = Неопределено Тогда
		...
		Если ТипЗнч(КонтактнаяИнформация) = Тип("Структура") И КонтактнаяИнформация.Свойство("type") Тогда
			...
			ОжидаемыйТип = УправлениеКонтактнойИнформациейСлужебный.ТипКонтактнойИнформацииИзСтроки(КонтактнаяИнформация.type);
			
		ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(КонтактнаяИнформация) Тогда
			
			СтандартнаяОбработка    = Истина;
			КонтактнаяИнформацияXML = Неопределено;
			
			ПриПриведенииКонтактнойИнформацииВXML(КонтактнаяИнформация, КонтактнаяИнформацияXML, СтандартнаяОбработка);
			Если Не СтандартнаяОбработка  И КонтактнаяИнформацияXML <> Неопределено Тогда
...
Функция ПривестиКонтактнуюИнформациюXML(Знач Данные) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректная контактная информация, не определен тип контактной информации.'");
	КонецЕсли;
