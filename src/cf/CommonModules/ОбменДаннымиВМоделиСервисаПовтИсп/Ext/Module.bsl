...
// 
// Возвращаемое значение:
//  Массив из Строка - Массив из имен планов обмена, используемых для синхронизации
//
...
Функция ПолучитьWSПроксиСервисаОбмена() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОбменСообщениями") Тогда
		...
		КонечнаяТочкаМС = МодульРаботаВМоделиСервиса.КонечнаяТочкаМенеджераСервиса();
		
		НастройкиТранспорта = МодульНастройкиТранспортаОбменаСообщениями.НастройкиТранспортаWS(КонечнаяТочкаМС);
...
// Параметры:
//  УзелИнформационнойБазы - ПланОбменаСсылка
//  СтрокаСообщенияОбОшибке - Строка - Значение по умолчанию = "".
// 
// Возвращаемое значение:
//  - Неопределено - Если не удалось создать WSПрокси
//  - WSПрокси 
//
...
// 
// Параметры:
//  ИмяПланаОбмена - Строка -  Имя плана обмена, как задано в конфигураторе
// 
// Возвращаемое значение:
//  Булево 
//
