...
Функция ТолькоСимволыНациональногоАлфавитаВСтроке(Знач ПроверяемаяСтрока, Знач ДополнительныеДопустимыеСимволы = Неопределено) Экспорт
	...
	СимволыНациональногоАлфавита = "abcdefghijklmnopqrstuvwxyz";
	...
	Если ДополнительныеДопустимыеСимволы = Неопределено Тогда
		ДополнительныеДопустимыеСимволы = " " + Символы.Таб + Символы.ПС + Символы.ВК + Символы.ВТаб + Символы.НПП + Символы.ПФ;
	КонецЕсли;
	
	Возврат СтрРазделить(НРег(ПроверяемаяСтрока), СимволыНациональногоАлфавита + ДополнительныеДопустимыеСимволы, Ложь).Количество() = 0;
...
Функция СформироватьСтрокуСимволов(Знач Символ, Знач ДлинаСтроки) Экспорт
	...
	Если ДлинаСтроки <= 0 Тогда
		Возврат "";
	КонецЕсли;
	Возврат СтрСоединить(Новый Массив(ДлинаСтроки + 1), Символ);
...
//
Функция ПреобразоватьЧислоВРимскуюНотацию(Знач АрабскоеЧисло, ИспользоватьКириллицу = Истина) Экспорт
...
//
Функция ПреобразоватьЧислоВАрабскуюНотацию(Знач РимскоеЧисло, ИспользоватьКириллицу = Истина) Экспорт
...
Функция ИзвлечьТекстИзHTML(Знач ИсходныйТекст) Экспорт
	...
	Пока Позиция > 0 Цикл
		ПозицияЗакрывающегоТега = СтрНайти(Текст, "</script>", , Позиция);
	...
	Пока Позиция > 0 Цикл
		ПозицияЗакрывающегоТега = СтрНайти(Текст, "</style>", , Позиция);
		Если ПозицияЗакрывающегоТега = 0 Тогда
			// Не найден закрывающий тег - вырезаем оставшийся текст.
			ПозицияЗакрывающегоТега = СтрДлина(Текст);
		КонецЕсли;
