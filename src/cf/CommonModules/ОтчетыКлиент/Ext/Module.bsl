...
#Область МетодыРаботыССКДИзФормыОтчета
...
Функция ТипЗначенияОграниченныйСвязьюПоТипу(Настройки, ПользовательскиеНастройки, ЭлементНастройки, ОписаниеЭлементаНастройки, ТипЗначения = Неопределено) Экспорт 
	...
	Если ТипЗнч(ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") И ТипЗначения = Новый ОписаниеТипов("Дата")
		И Не ОтчетыКлиентСервер.ЭтоВидСравненияСписка(ЭлементНастройки.ВидСравнения) Тогда
		ТипЗначения = Новый ОписаниеТипов("СтандартнаяДатаНачала");
	КонецЕсли;
	
	Если ТипЗнч(ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") 
		И ОтчетыКлиентСервер.ЭтоВидСравненияБезСтрогойТипизации(ЭлементНастройки.ВидСравнения) Тогда
		ТипЗначения = ОтчетыКлиентСервер.ИзменитьТипНаОбычнуюСтроку(ТипЗначения);
	КонецЕсли;
...
Функция ЭтоВыборОбъектовМетаданных(ДоступныеТипы, Знач ОтмеченныеЗначения, Обработчик) Экспорт 
	...
	Если КоличествоТипов = 1 Тогда
		ЭтоВыборОбъектовМетаданных =
			    ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных"))
			Или ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений"));
		
	ИначеЕсли КоличествоТипов = 2 Тогда
		ЭтоВыборОбъектовМетаданных =
			    ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных"))
			  И ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений"));
	Иначе
		ЭтоВыборОбъектовМетаданных = Ложь;
	КонецЕсли;
	...
	Если ЭтоВыборОбъектовМетаданных Тогда 
		...
		ПараметрыПодбора.ВыбранныеОбъектыМетаданных = ОтмеченныеЗначения;
...
Процедура НачатьВыборПользователей(Форма, ЭлементФормы, ДоступныеТипы, Знач ОтмеченныеЗначения,
	...
	ПрочиеТипы = Новый ОписаниеТипов(ДоступныеТипы,,
	...
	ТипыПользователей = Новый ОписаниеТипов(ДоступныеТипы,, ПрочиеТипы.Типы());
	
	КоличествоТипов = ТипыПользователей.Типы().Количество();
	
	Если КоличествоТипов = 1 Тогда
		ЭтоВыборПользователей =
			    ТипыПользователей.СодержитТип(Тип("СправочникСсылка.Пользователи"))
			Или ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи"));
		
	ИначеЕсли КоличествоТипов = 2 Тогда
		ЭтоВыборПользователей =
			    ТипыПользователей.СодержитТип(Тип("СправочникСсылка.Пользователи"))
			  И ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи"))
			Или ТипыПользователей.СодержитТип(Тип("СправочникСсылка.Пользователи"))
			  И ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ГруппыПользователей"))
			Или ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи"))
			  И ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ГруппыВнешнихПользователей"));
	...
	Если Не ЭтоВыборПользователей Тогда
		ВыполнитьОбработкуОповещения(ОбработчикПродолжения, Неопределено);
		Возврат;
	КонецЕсли;
	...
	ВыбратьТип =
		...
		Или КоличествоТипов = 2
		  И ТипыПользователей.СодержитТип(Тип("СправочникСсылка.Пользователи"))
		  И ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи"));
	
	Контекст = Новый Структура;
	Контекст.Вставить("ТипыПользователей",     ТипыПользователей);
	Контекст.Вставить("ПрочиеТипы",            ПрочиеТипы);
	Контекст.Вставить("ОтмеченныеЗначения",    ОтмеченныеЗначения);
	Контекст.Вставить("ПараметрыВыбора",       ПараметрыВыбора);
	Контекст.Вставить("ОбработчикВыбора",      ОбработчикВыбора);
	Контекст.Вставить("ОбработчикПродолжения", ОбработчикПродолжения);
	
	СписокТиповПользователей = Новый СписокЗначений;
	СписокТиповПользователей.ЗагрузитьЗначения(ПрочиеТипы.Типы());
	
	Если Не ВыбратьТип И ПрочиеТипы.Типы().Количество() = 0 Тогда
	...
	Иначе
		Если ТипыПользователей.СодержитТип(Тип("СправочникСсылка.Пользователи")) Тогда
			СписокТиповПользователей.Добавить(Тип("СправочникСсылка.Пользователи"));
		КонецЕсли;
		Если ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ВнешниеПользователи")) Тогда
			СписокТиповПользователей.Добавить(Тип("СправочникСсылка.ВнешниеПользователи"));
		КонецЕсли;
		Если ПрочиеТипы.Типы().Количество() > 0 Тогда
			Для Каждого ЭлементСписка Из СписокТиповПользователей Цикл
...
Процедура ВыбратьПользователейПослеВыбораТипа(ВыбранныйЭлемент, Контекст) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = Тип("СправочникСсылка.Пользователи") Тогда
		...
		Если Контекст.ТипыПользователей.СодержитТип(Тип("СправочникСсылка.ГруппыПользователей")) Тогда 
		...
		КонецЕсли;
	ИначеЕсли ВыбранныйЭлемент.Значение = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		КонецЕсли;
	Иначе
		ВыполнитьОбработкуОповещения(Контекст.ОбработчикПродолжения, ВыбранныйЭлемент);
		Возврат;
	КонецЕсли;
	
	Список = Новый СписокЗначений;
	Если Контекст.ОтмеченныеЗначения <> Неопределено Тогда
		Список.ЗагрузитьЗначения(Контекст.ОтмеченныеЗначения.ВыгрузитьЗначения());
	КонецЕсли;
	ПроверитьОтмеченныеЗначения(Список, Контекст.ТипыПользователей);
	ВыбранныеПользователи = Список.ВыгрузитьЗначения();
	...
	Обработчик = Новый ОписаниеОповещения("ВыбратьПользователейЗавершение", ЭтотОбъект, Контекст);
	
	ОткрытьФорму(ПолноеИмяФормыВыбора, ПараметрыПодбора, ЭтотОбъект,,,, Обработчик);
...
Процедура ВыбратьПользователейЗавершение(Результат, Контекст) Экспорт
	...
	Если Контекст.ПрочиеТипы.Типы().Количество() > 0 Тогда
		Список = Новый СписокЗначений;
		Список.ЗагрузитьЗначения(Результат);
		Список.ЗаполнитьПометки(Истина);
		
		Для Каждого ЭлементСписка Из Контекст.ОтмеченныеЗначения Цикл
			Если Контекст.ПрочиеТипы.СодержитТип(ТипЗнч(ЭлементСписка.Значение)) Тогда
				ЗаполнитьЗначенияСвойств(Список.Добавить(), ЭлементСписка);
			КонецЕсли;
		КонецЦикла;
		ВыполнитьОбработкуОповещения(Контекст.ОбработчикВыбора, Список);
	Иначе
		ВыполнитьОбработкуОповещения(Контекст.ОбработчикВыбора, Результат);
	КонецЕсли;
