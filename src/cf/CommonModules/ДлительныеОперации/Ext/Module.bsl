...
Функция ВыполнитьФункцию(Знач ПараметрыВыполнения, ИмяФункции, Знач Параметр1 = Неопределено,
	...
	ПроверитьИмяПроцедуры(ИмяФункции,
		"ИмяФункции", "ДлительныеОперации.ВыполнитьФункцию", ПараметрыВыполнения);
	
	ПараметрыВызова = СписокПараметров(Параметр1, Параметр2, Параметр3, Параметр4,
		Параметр5, Параметр6, Параметр7);
	
	ПараметрыВыполнения = ПодготовитьПараметрыВыполнения(ПараметрыВыполнения, "Функция");
...
Функция ВыполнитьПроцедуру(Знач ПараметрыВыполнения = Неопределено, ИмяПроцедуры, Знач Параметр1 = Неопределено,
	...
	ПроверитьИмяПроцедуры(ИмяПроцедуры,
		"ИмяПроцедуры", "ДлительныеОперации.ВыполнитьФункцию", ПараметрыВыполнения);
	
	ПараметрыВызова = СписокПараметров(Параметр1, Параметр2, Параметр3, Параметр4,
		Параметр5, Параметр6, Параметр7);
	ПараметрыВыполнения = ПодготовитьПараметрыВыполнения(ПараметрыВыполнения, "Процедура");
...
Функция ВыполнитьФункциюВНесколькоПотоков(ИмяФункции, ПараметрыВыполнения, НаборПараметровФункции = Неопределено) Экспорт
	...
	ПроверитьИмяПроцедуры(ИмяФункции,
		"ИмяФункции", "ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков", ПараметрыВыполнения);
	
	Возврат ВыполнитьМетодВНесколькоПотоков(ИмяФункции, ПараметрыВыполнения, НаборПараметровФункции, Истина);
...
Функция ВыполнитьПроцедуруВНесколькоПотоков(ИмяПроцедуры, ПараметрыВыполнения, НаборПараметровПроцедуры = Неопределено) Экспорт
	...
	ПроверитьИмяПроцедуры(ИмяПроцедуры,
		"ИмяПроцедуры", "ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков", ПараметрыВыполнения);
	
	Возврат ВыполнитьМетодВНесколькоПотоков(ИмяПроцедуры, ПараметрыВыполнения, НаборПараметровПроцедуры, Ложь);
...
//                                  расширений конфигурации. Имеет приоритет над параметром ЗапуститьНеВФоне.
//     * ВнешнийОтчетОбработка    - см. НовоеОписаниеВнешнегоОтчетаОбработки
//                                - Неопределено - по умолчанию, когда указано имя метода, которое
//                                  не начинается с "ВнешнийОтчет." или "ВнешняяОбработка.", в противном случае
//                                  следует использовать конструктор описания внешнего отчета или обработки.
...
Функция ВыполнитьВФоне(Знач ИмяПроцедуры, Знач ПараметрыПроцедуры, Знач ПараметрыВыполнения) Экспорт
	...
	ПроверитьИмяПроцедуры(ИмяПроцедуры,
		"ИмяПроцедуры", "ДлительныеОперации.ВыполнитьВФоне", ПараметрыВыполнения);
	
	ЗаполнитьПараметрыВыполненияСУчетомОбратнойСовместимости(ПараметрыВыполнения);
	ПроверитьПараметрыВыполнения(ИмяПроцедуры, ПараметрыВыполнения);
	Если ПараметрыВыполнения.СвойстваИсполняющегоПотокаМногопоточнойДлительнойОперации = Неопределено Тогда
		ПроверитьИЗаполнитьАдресРезультата(ПараметрыВыполнения, "ДлительныеОперации.ВыполнитьВФоне");
	КонецЕсли;
	...
	Если ПустаяСтрока(ПараметрыВыполнения.ЭтоФункция) Тогда
	...
	ВыполнитьБезФоновогоЗадания = ЭтоВыполнениеБезФоновогоЗадания(ИмяПроцедуры, ПараметрыВыполнения);
	Если ВыполнитьБезФоновогоЗадания Тогда
		СообщенияПередВызовом = ПолучитьСообщенияПользователю(Истина);
		Попытка
			Если ПараметрыВыполнения.ЭтоФункция = "Функция" Тогда
		...
		Исключение
			...
			Если ЗначениеЗаполнено(ПараметрыВыполнения.УточнениеОшибки) Тогда
		...
		Для Каждого Сообщение Из СообщенияПередВызовом Цикл
			Сообщение.Сообщить();
		КонецЦикла;
		Возврат Результат;
	КонецЕсли;
	
	Если ПараметрыВыполнения.ОжидатьЗавершение <> Неопределено
	   И СтандартныеПодсистемыСервер.ОтключенаЛогикаНачалаРаботыСистемы()
	   И Не СтандартныеПодсистемыСервер.ЭтоСеансФоновогоЗадания() Тогда
	...
	Исключение
		Результат.Статус = "Ошибка";
		ИнформацияОбОшибке = ?(Задание <> Неопределено И Задание.ИнформацияОбОшибке <> Неопределено,
			Задание.ИнформацияОбОшибке, ИнформацияОбОшибке());
		УстановитьСвойстваОшибки(Результат, ИнформацияОбОшибке);
		Если Не ЭтоОшибкаЗаданиеСТакимЗначениемКлючаУжеВыполняется(Задание, Результат, ПараметрыВыполнения) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Ошибка выполнения'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, , , Результат.ПодробноеПредставлениеОшибки);
		КонецЕсли;
		Возврат Результат;
	...
	Если ИмяПроцедуры = ИмяМетодаМногопоточногоПроцесса()
	   И Не ПараметрыВыполнения.ЭтоПерезапускУправляющегоПотока Тогда
...
// Конструктор значения свойства ВнешнийОтчетОбработка см. ПараметрыВыполненияВФоне
// Требуется, когда указано имя метода, которое начинается
// с "ВнешнийОтчет." или "ВнешняяОбработка.".
// Когда используется режим ВыполнитьНеВФоне, а данные не указаны (Неопределено),
// тогда производится попытка создать объект внешнего отчета или обработки по его имени,
// исходя из того, что объект уже подключен в текущем сеансе.
//
// Возвращаемое значение:
//  Структура:
//   * Данные - ДвоичныеДанные - для подключения объекта требуется право
//      интерактивного открытия внешнего отчета или обработки соответственно.
//      Внешний отчет или обработка подключаются с защитой от опасных действий.
//            - СправочникСсылка - ссылка на дополнительный отчет или обработку,
//      добавленную администратором. Требуется подсистема ДополнительныеОтчетыИОбработки.
//      Для подключения объекта не требуется право интерактивного открытия внешнего отчета и
//      внешней обработки, так как подключение объекта выполняется по настройкам,
//      сделанным администратором, а также не требуется защита от опасных действий,
//      так как администратор уже проверил и подтвердил безопасность.
//   * ПолноеИмяФайла - Строка - пустая строка по умолчанию.
//      Если указать полное имя файла, который содержит указанные
//      двоичные данные внешнего отчета или внешней обработки, тогда этот файл
//      будет использован при подключении вместо создания временного файла.
//
Функция НовоеОписаниеВнешнегоОтчетаОбработки() Экспорт
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	ИдентификаторПроцесса = ПараметрыСеанса.ДлительныеОперации.Запущенные.Получить(ИдентификаторЗадания);
	Если ИдентификаторПроцесса <> Неопределено Тогда
		ОтменитьВыполнениеВсехПотоков(ИдентификаторПроцесса);
		ОперацияВыполнена(ИдентификаторЗадания);
	КонецЕсли;
...
Функция ЗаданиеВыполнено(Знач ИдентификаторЗадания, РасширенныйРезультат = Ложь) Экспорт
	...
	Если РасширенныйРезультат Тогда
		...
		Свойства.Вставить("ИнформацияОбОшибке",           Результат.ИнформацияОбОшибке);
	...
	Если Результат.Статус = "Отменено" Тогда
		...
		Исключение
			Если Не РасширенныйРезультат Тогда
				ВызватьИсключение;
			КонецЕсли;
			Свойства.ИнформацияОбОшибке = ИнформацияОбОшибке();
		КонецПопытки;
	...
	ИначеЕсли Задание = Неопределено Тогда
		...
		ДляАдминистратора = НСтр("ru = 'См. также журнал регистрации.'");
		Попытка
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,, ДляАдминистратора, Результат.ИнформацияОбОшибке);
		Исключение
			Если Не РасширенныйРезультат Тогда
				ВызватьИсключение;
			КонецЕсли;
	...
	Иначе
		...
		ДляАдминистратора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка выполнения фонового задания %1 с идентификатором %2.
			           |См. также журнал регистрации.'"),
			Задание.ИмяМетода,
			Строка(ИдентификаторЗадания));
		Попытка
...
Функция ЗапуститьВыполнениеВФоне(Знач ИдентификаторФормы, Знач ИмяЭкспортнойПроцедуры, Знач Параметры,
	...
	ПроверитьИмяПроцедуры(ИмяЭкспортнойПроцедуры,
		"ИмяЭкспортнойПроцедуры", "ДлительныеОперации.ЗапуститьВыполнениеВФоне");
	...
	Если ОбщегоНазначения.РежимОтладки()
	 Или СтандартныеПодсистемыСервер.ОтключенаЛогикаНачалаРаботыСистемы()
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Задание = Неопределено) Экспорт
	...
	Если Задание = Неопределено Тогда
		...
		Если РезультатИзОповещения <> Неопределено Тогда
			...
			УстановитьЗавершениеДляУправляющегоПотока(ИдентификаторЗадания, Результат);
			Возврат Результат;
		КонецЕсли;
		Если ВыполненПерезапускУправляющегоПотока(ИдентификаторЗадания, Задание) Тогда
			Возврат Результат;
		КонецЕсли;
		ТекстОшибки = НСтр("ru = 'Операция не выполнена из-за аварийного завершения фонового задания.'");
		УточнениеДляАдминистратора = НСтр("ru = 'Фоновое задание не существует'") + ": "
			+ Строка(ПоследнийИдентификатор);
		Попытка
			ВызватьИсключение(ТекстОшибки,,, УточнениеДляАдминистратора);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
		КонецПопытки;
		УстановитьСвойстваОшибки(Результат, ИнформацияОбОшибке);
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		Если ПараметрыСеанса.ДлительныеОперации.Отмененные.Найти(ПоследнийИдентификатор) = Неопределено Тогда
			...
			Если Задание.ИнформацияОбОшибке <> Неопределено Тогда
				Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(Задание.ИнформацияОбОшибке,
					НСтр("ru = 'Операция отменена администратором.'"));
				Попытка
					ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, Задание.ИнформацияОбОшибке);
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
				КонецПопытки;
				УстановитьСвойстваОшибки(Результат, ИнформацияОбОшибке);
...
Процедура ВыполнитьПроцедуруМодуляОбъектаОбработки(Параметры, АдресХранилища) Экспорт 
	...
	Если Параметры.ЭтоВнешняяОбработка Тогда
		...
		Иначе
			...
			Если БезопасныйРежим = Ложь И Не ПравоДоступа("Администрирование", Метаданные) Тогда
				БезопасныйРежимПриСоздании = Истина;
			Иначе
				БезопасныйРежимПриСоздании = БезопасныйРежим;
			КонецЕсли;
			Обработка = ВнешниеОбработки.Создать(Параметры.ИмяОбработки, БезопасныйРежимПриСоздании); // Не нарушает безопасность информации, так как внешний код подключается также, как при использовании Файл - Открыть.
		КонецЕсли;
	Иначе
		Обработка = Обработки[Параметры.ИмяОбработки].Создать();
	КонецЕсли;
	
	ПроверитьИмяПроцедуры(Параметры.ИмяМетода,
		"Параметры.ИмяМетода", "ВыполнитьПроцедуруМодуляОбъектаОбработки",, Обработка);
...
Процедура ВыполнитьПроцедуруМодуляОбъектаОтчета(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.ЭтоВнешнийОтчет Тогда
		...
		Иначе
			БезопасныйРежимПриСоздании = БезопасныйРежим;
		КонецЕсли;
		Отчет = ВнешниеОтчеты.Создать(Параметры.ИмяОтчета, БезопасныйРежимПриСоздании); // Не нарушает безопасность информации, так как внешний код подключается также, как при использовании Файл - Открыть.
	Иначе
		Отчет = Отчеты[Параметры.ИмяОтчета].Создать();
	КонецЕсли;
	
	ПроверитьИмяПроцедуры(Параметры.ИмяМетода,
		"Параметры.ИмяМетода", "ВыполнитьПроцедуруМодуляОбъектаОтчета",, Отчет);
...
Процедура УстановкаПараметровСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если ИмяПараметра = "ДлительныеОперации" Тогда
		...
		Свойства.Вставить("Запущенные", Новый ФиксированноеСоответствие(Новый Соответствие));
		...
		Свойства.Вставить("ДатаДобавления", '00010101');
		Свойства.Вставить("ДатаДобавленияМиллисекунды", 0);
		ПараметрыСеанса.ДлительныеОперации = Новый ФиксированнаяСтруктура(Свойства);
...
Процедура УстановитьСвойстваОшибки(Свойства, Ошибка)
	...
	Свойства.ИнформацияОбОшибке = Ошибка;
...
Функция ПараметрыМногопоточнойОперации(ИдентификаторПроцесса, СохраненныеПараметры = Неопределено) 
	...
	Результат = Новый Структура;
	Результат.Вставить("ИдентификаторПроцесса", ИдентификаторПроцесса);
	Результат.Вставить("ИмяМетода",             "");
	Результат.Вставить("ЭтоФункция",            "Процедура");
	Результат.Вставить("ПараметрыВыполнения",   ПараметрыВыполненияФункции(Неопределено));
	Результат.Вставить("ПараметрыМетода",       0);
...
// Только для процедуры ПриПериодическомПолученииДанныхКлиентаНаСервере.
//
// Параметры:
...
//   * Значение - см. НовыйРезультатВыполненияОперации
//  
Функция РезультатПроверкиДлительныхОпераций(Параметры) Экспорт
	...
	Для Каждого ИдентификаторЗадания Из Параметры.ЗаданияДляПроверки Цикл
		...
		РезультатОперации = ОперацияВыполнена(ИдентификаторЗадания);
		РезультатОперации.Сообщения = ОписаниеСообщенийПользователю(РезультатОперации.Сообщения);
		Результат.Вставить(ИдентификаторЗадания, РезультатОперации);
	КонецЦикла;
	...
	Для Каждого ИдентификаторЗадания Из Параметры.ЗаданияДляОтмены Цикл
		...
		РезультатОперации.Статус = "Отменено";
		Результат.Вставить(ИдентификаторЗадания, РезультатОперации);
	КонецЦикла;
...
// Для процедуры РезультатПроверкиДлительныхОпераций, и функции ПропуститьОповещение.
//
// Параметры:
//  Сообщения - ФиксированныйМассив из СообщениеПользователю
// 
// Возвращаемое значение:
//  ФиксированныйМассив из см. ОписаниеСообщенияПользователю
//
Функция ОписаниеСообщенийПользователю(Сообщения)
	...
	Для Каждого Сообщение Из Сообщения Цикл
		Результат.Добавить(ОписаниеСообщенияПользователю(Сообщение));
	КонецЦикла;
...
// Преобразует СообщениеПользователю в структуру.
//
// Параметры:
...
//   * Текст - Строка
//
Функция ОписаниеСообщенияПользователю(Сообщение)
	
	Результат = Новый Структура;
	Результат.Вставить("ИдентификаторНазначения", Сообщение.ИдентификаторНазначения);
	Результат.Вставить("КлючДанных",              Сообщение.КлючДанных);
	Результат.Вставить("Поле",                    Сообщение.Поле);
	Результат.Вставить("ПутьКДанным",             Сообщение.ПутьКДанным);
	Результат.Вставить("Текст",
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(Сообщение.Текст));
...
Функция ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяПроцедуры,
			ПараметрыВыполнения, ПараметрыПроцедуры = Неопределено, БезопасныйРежим = Ложь, ОтправлятьОповещения = Ложь)
	...
	СтекЗапуска = "";
	Если ОбщегоНазначения.ОбщиеПараметрыБазовойФункциональности().ВыводитьПолныйСтекПриОшибкеДлительнойОперации
	 Или ПараметрыКлиента.Получить("ПервыйСерверныйВызовВыполнен") = Ложь Тогда
		Попытка
			ВызватьИсключение НСтр("ru = 'Запуск фонового задания длительной операции:'");
		Исключение
			СтекЗапуска = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
	КонецЕсли;
	...
	ВсеПараметры.Вставить("СтекЗапуска",         СтекЗапуска);
...
// Для функции ВыполнитьВФоне.
Функция ЭтоОшибкаЗаданиеСТакимЗначениемКлючаУжеВыполняется(Задание, Результат, ПараметрыВыполнения)
	
	Если Задание <> Неопределено
	 Или Не ЗначениеЗаполнено(ПараметрыВыполнения.КлючФоновогоЗадания) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(Результат.ИнформацияОбОшибке);
	// АПК:1405-выкл - требуется для быстрого определения причины ошибки
	СтрокаПоиска = НСтр("ru = 'Задание с таким значением ключа уже выполняется';
						|en = 'A job with the same key value is performed already'");
	// АПК:1405-вкл
	Если ЗначениеЗаполнено(СтрокаПоиска) И СтрНачинаетсяС(КраткоеПредставлениеОшибки, СтрокаПоиска) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Отбор = Новый Структура;
	Отбор.Вставить("ИмяМетода", ИмяПроцедурыФоновогоЗаданияДлительнойОперации());
	Отбор.Вставить("Ключ",      ПараметрыВыполнения.КлючФоновогоЗадания);
	Отбор.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
	
	НайденныеЗадания = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	Возврат НайденныеЗадания.Количество() > 1;
...
Процедура ВыполнитьСКонтекстомКлиента(ВсеПараметры) Экспорт
	...
	Если ПараметрыКлиента.Получить("ПервыйСерверныйВызовВыполнен") = Ложь
	   И ЗначениеЗаполнено(ВсеПараметры.СтекЗапуска) Тогда
		
		ПараметрыКлиента = Новый Соответствие(ПараметрыКлиента);
		ПараметрыКлиента.Вставить("СтекЗапускаДлительнойОперации",
			ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(ВсеПараметры.СтекЗапуска));
		ПараметрыКлиента = Новый ФиксированноеСоответствие(ПараметрыКлиента);
	КонецЕсли;
	...
	Исключение
		...
		СтекЗапуска = ВсеПараметры.СтекЗапуска;
		СтекЗапуска = ?(ЗначениеЗаполнено(СтекЗапуска),
			ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтекЗапуска), Неопределено);
		Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке,
			ПараметрыВыполнения.УточнениеОшибки);
		Попытка
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,, СтекЗапуска, ИнформацияОбОшибке);
		Исключение
			УточненнаяИнформацияОбОшибке = ИнформацияОбОшибке();
			УстановитьСвойстваОшибки(Результат, УточненнаяИнформацияОбОшибке);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		...
		МодульУправлениеДоступом.ЗапуститьОбновлениеДоступа();
	КонецЕсли;
...
Процедура ВызватьПроцедуру(ИмяПроцедуры, ПараметрыВызова, ПараметрыВыполнения)
	...
	Если Не ЭтоПроцедураМодуляОбработки Тогда
		ПроверитьИмяПроцедуры(ИмяПроцедуры, "ИмяПроцедуры", "ВызватьПроцедуру");
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяПроцедуры, ПараметрыВызова);
		Возврат;
	КонецЕсли;
	
	ОбработкаОтчетОбъект = ОбработкаОтчетОбъект(ИмяПроцедуры,
		ЧастиИмени, ПараметрыВыполнения, "ИмяПроцедуры", "ВызватьПроцедуру");
	
	ПроверитьИмяПроцедуры(ЧастиИмени[3], "ИмяПроцедуры", "ВызватьПроцедуру",, ОбработкаОтчетОбъект);
	
	ОбщегоНазначения.ВыполнитьМетодОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[3], ПараметрыВызова);
	
КонецПроцедуры

Функция ОбработкаОтчетОбъект(Имя, ЧастиИмени, ПараметрыВыполнения, ИмяПараметраМетода, ИмяМетода)
	...
	Если ЭтоОбработка Или ЭтоОтчет Тогда
		...
		Возврат МенеджерОбъекта[ЧастиИмени[1]].Создать();
	КонецЕсли;
	...
	Если ЭтоВнешняяОбработка Или ЭтоВнешнийОтчет Тогда
		Возврат ВнешняяОбработкаОтчетОбъект(ЭтоВнешнийОтчет, ПараметрыВыполнения, ЧастиИмени[1]);
	КонецЕсли;
	
	ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверный формат параметра %1 (%2) в %3.'"),
		Имя, ИмяПараметраМетода, ИмяМетода),
...
Функция ВнешняяОбработкаОтчетОбъект(ЭтоВнешнийОтчет, ПараметрыВыполнения, ИмяПодключеннойОбработкиОтчета)
	...
	ВнешнийОтчетОбработка = ПараметрыВыполнения.ВнешнийОтчетОбработка;
	
	Если ВнешнийОтчетОбработка = Неопределено Тогда
		...
		КонецЕсли;
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для вызова процедуры внешнего отчета или внешней обработки в фоне
			           |требуется заполнить свойство %1 параметра %2.'"),
			"ВнешнийОтчетОбработка", "ПараметрыВыполнения");
	...
	Если ТипЗнч(ВнешнийОтчетОбработка.Данные) <> Тип("ДвоичныеДанные") Тогда
		МодульДополнительныеОтчетыИОбработки = ОбщегоНазначения.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
		Возврат МодульДополнительныеОтчетыИОбработки.ОбъектВнешнейОбработки(
			ВнешнийОтчетОбработка.Данные);
	КонецЕсли;
	
	ИмяПраваДоступа = ?(ЭтоВнешнийОтчет, "ИнтерактивноеОткрытиеВнешнихОтчетов", "ИнтерактивноеОткрытиеВнешнихОбработок");
	ВыполнитьПроверкуПравДоступа(ИмяПраваДоступа, Метаданные);
	Если БезопасныйРежим = Ложь И Не ПравоДоступа("Администрирование", Метаданные) Тогда
		БезопасныйРежимПриСоздании = Истина;
	Иначе
		БезопасныйРежимПриСоздании = БезопасныйРежим;
	...
	ДвоичныеДанные = ВнешнийОтчетОбработка.Данные;
	ПолноеИмяФайла = ВнешнийОтчетОбработка.ПолноеИмяФайла;
	...
	Иначе
		...
		Если Не СоздатьВременныйФайл Тогда
			СоздатьВременныйФайл = ДвоичныеДанныеФайла <> ВнешнийОтчетОбработка.Данные;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		ОбработкаОтчетОбъект = МенеджерОбъекта.Создать(ПолноеИмяФайла, БезопасныйРежимПриСоздании); // Не нарушает безопасность информации, так как внешний код подключается также, как при использовании Файл - Открыть.
	Исключение
		Если СоздатьВременныйФайл Тогда
...
Процедура ВызватьФункцию(ИмяФункции, ПараметрыПроцедуры, ПараметрыВыполнения)
	...
	ЭтоПроцедураМодуляОбработки = (ЧастиИмени.Количество() = 4) И ВРег(ЧастиИмени[2]) = "МОДУЛЬОБЪЕКТА";
	Если Не ЭтоПроцедураМодуляОбработки Тогда
		ПроверитьИмяПроцедуры(ИмяФункции, "ИмяФункции", "ВызватьФункцию");
		Результат = ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяФункции, ПараметрыПроцедуры, Истина);
		УстановитьРезультатВызоваФункции(Результат, ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	
	ОбработкаОтчетОбъект = ОбработкаОтчетОбъект(ИмяФункции,
		ЧастиИмени, ПараметрыВыполнения, "ИмяФункции", "ВызватьФункцию");
	
	ПроверитьИмяПроцедуры(ЧастиИмени[3], "ИмяФункции", "ВызватьФункцию",, ОбработкаОтчетОбъект);
	
	Результат = ОбщегоНазначения.ВыполнитьМетодОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[3], ПараметрыПроцедуры, Истина);
	УстановитьРезультатВызоваФункции(Результат, ПараметрыВыполнения);
...
Функция ПропуститьОповещение(Данные) Экспорт
	...
	Если Данные.ИмяОповещения <> ИмяОповещения() Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если Данные.Результат.ВидОповещения = "ДлительнаяОперацияЗавершена"
	   И Не СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны()
	   И Не СерверныеОповещения.СистемаВзаимодействийПодключена() Тогда
		
		Возврат Истина;
	КонецЕсли;
	
	Свойства = Новый Структура("ИдентификаторОповещения", Null);
	ЗаполнитьЗначенияСвойств(Свойства, Данные);
	Если Свойства.ИдентификаторОповещения <> Null Тогда
		Данные.Результат.Результат.Сообщения =
			ОписаниеСообщенийПользователю(Данные.Результат.Результат.Сообщения);
	КонецЕсли;
...
Процедура ОтправитьОповещениеКлиенту(ВидОповещения, ПередаваемоеЗначение,
	...
	ЗаписатьСообщенияПользователю = ФоновоеЗадание <> Неопределено И ВидОповещения = "СообщениеПользователю"
		И Не СтандартныеПодсистемыПовтИсп.ЭтоСеансДлительнойОперации();
	...
	Если Не ЗаписатьСообщенияПользователю Тогда
		// Отправка накопленных сообщений, если есть, например,
		// от платформы, которая не использует ОбщегоНазначения.СообщитьПользователю.
	...
	Если ВидОповещения = "СообщениеПользователю" Тогда
		Если ПередаваемоеЗначение = Null Тогда
			Возврат; // Только отправка накопленных сообщений.
		КонецЕсли;
	...
	ДополнительныеПараметрыОтправки.ЗаписатьБезусловно = ВидОповещения = "СообщениеПользователю"
		Или ВидОповещения = "ДлительнаяОперацияЗавершена"
...
Функция ВидыОповещенийДлительныхОпераций() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить(НРег(ИдентификаторВидаОповещения("СообщениеПользователю")));
	Результат.Добавить(НРег(ИдентификаторВидаОповещения("Прогресс")));
	Результат.Добавить(НРег(ИдентификаторВидаОповещения("ДлительнаяОперацияЗавершена")));
...
Процедура ЗаписатьОстаткиСообщенийПользователю(ИдентификаторЗадания)
	...
	Если СтандартныеПодсистемыПовтИсп.ЭтоСеансДлительнойОперации() Тогда
		// Отправка накопленных сообщений.
		ОтправитьОповещениеКлиенту("СообщениеПользователю", Null);
		Возврат;
	КонецЕсли;
...
Функция ВыполнитьФоновоеЗадание(ПараметрыВыполнения, Параметры, Ключ, Наименование)
	...
	// Имя процедуры должно быть, как указано в функции ИмяПроцедурыФоновогоЗаданияДлительнойОперации.
	Если ПараметрыВыполнения.БезРасширений Тогда
		Возврат РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеБезРасширений(
			"ДлительныеОперации.ВыполнитьСКонтекстомКлиента", Параметры, Ключ, Наименование);
...
Процедура ПроверитьИмяПроцедуры(ИмяПроцедуры, ИмяПараметраМетода, ИмяМетода,
			ПараметрыВыполнения = Неопределено, Объект = Неопределено)
	
	ПроверяемоеИмяПроцедуры = ИмяПроцедуры;
	
	Если Объект = Неопределено Тогда
		ЧастиИмени = СтрРазделить(ИмяПроцедуры, ".", Истина);
		
		Если ЧастиИмени.Количество() = 4 И ВРег(ЧастиИмени[2]) = "МОДУЛЬОБЪЕКТА" Тогда
			Параметры = Новый Структура("ВнешнийОтчетОбработка, ЗапуститьНеВФоне");
			Если ТипЗнч(ПараметрыВыполнения) = Тип("Структура") Тогда
				ЗаполнитьЗначенияСвойств(Параметры, ПараметрыВыполнения);
			КонецЕсли;
			Параметры.ЗапуститьНеВФоне = Истина;
			ЗаполнитьПараметрВнешнийОтчетОбработка(Параметры.ВнешнийОтчетОбработка);
			ПроверитьПараметрВнешнийОтчетОбработка(Параметры, ИмяПроцедуры, ИмяМетода);
			Объект = ОбработкаОтчетОбъект(ИмяПроцедуры,
				ЧастиИмени, Параметры, "ИмяПроцедуры", "ВызватьПроцедуру");
			ПроверяемоеИмяПроцедуры = ЧастиИмени[3];
		
		ИначеЕсли ЧастиИмени.Количество() < 2 Или ЧастиИмени.Количество() > 3 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректное значение параметра %1 (%2) в %3.'"),
				ИмяПараметраМетода, ИмяПроцедуры, ИмяМетода);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	КонецЕсли;
	
	Попытка
		СтандартныеПодсистемыСервер.ПроверитьМетодДляВызоваКакПроизвольныйКод(ПроверяемоеИмяПроцедуры, Истина, Объект);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
...
Функция ПодготовитьПараметрыВыполнения(ПереданныйПараметр, ЭтоФункция)
	...
	ИначеЕсли ЭтоФункция = "Функция" Тогда
		...
		ИначеЕсли ТипЗнч(ПереданныйПараметр) = Тип("УникальныйИдентификатор")
		        И ЗначениеЗаполнено(ПереданныйПараметр) Тогда
	...
	Результат.ЭтоФункция = ЭтоФункция;
	Возврат Результат;
	
КонецФункции

Процедура ЗаполнитьПараметрыВыполненияСУчетомОбратнойСовместимости(ПараметрыВыполнения)
	
	НовыеПараметрыВыполнения = ПараметрыВыполненияВФоне();
	...
	ЗаполнитьПараметрВнешнийОтчетОбработка(НовыеПараметрыВыполнения.ВнешнийОтчетОбработка);
	
	ПараметрыВыполнения = НовыеПараметрыВыполнения;
	
КонецПроцедуры

Процедура ЗаполнитьПараметрВнешнийОтчетОбработка(ЗначениеПараметра)
	
	Если ЗначениеПараметра = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Результат = НовоеОписаниеВнешнегоОтчетаОбработки();
	
	Если ТипЗнч(ЗначениеПараметра) = Тип("ДвоичныеДанные") Тогда
		Результат.Данные = ЗначениеПараметра;
		
	ИначеЕсли ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
		ЗаполнитьЗначенияСвойств(Результат, ЗначениеПараметра);
		
		Если Результат.Данные = Неопределено Тогда
			УстаревшиеСвойства = Новый Структура("ДвоичныеДанные");
			ЗаполнитьЗначенияСвойств(УстаревшиеСвойства, ЗначениеПараметра);
			Результат.Данные = УстаревшиеСвойства.ДвоичныеДанные;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьПараметрыВыполнения(ИмяПроцедуры, ПараметрыВыполнения)
...
#КонецЕсли
	...
	Если ПараметрыВыполнения.ЭтоФункция <> "Функция"
	   И ПараметрыВыполнения.ЭтоФункция <> "Процедура"
	   И ПараметрыВыполнения.ЭтоФункция <> "" Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Служебный параметр ""%1"" может принимать значения """", ""%2"", ""%3"" в %4.
				           |А указано значение ""%5"".'"),
				"ЭтоФункция",
				"Функция",
				"Процедура",
				"ДлительныеОперации.ВыполнитьВФоне",
				ПараметрыВыполнения.ЭтоФункция),
			КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	
	ПроверитьПараметрВнешнийОтчетОбработка(ПараметрыВыполнения,
		ИмяПроцедуры, "ДлительныеОперации.ВыполнитьВФоне");
	
КонецПроцедуры

Процедура ПроверитьПараметрВнешнийОтчетОбработка(ПараметрыВыполнения, ИмяПроцедуры, ИмяМетода)
	
	Если ПараметрыВыполнения.ВнешнийОтчетОбработка = Неопределено Тогда
		Если ПараметрыВыполнения.ЗапуститьНеВФоне 
		 Или Не СтрНачинаетсяС(ВРег(ИмяПроцедуры), "ВНЕШНИЙОТЧЕТ.")
		   И Не СтрНачинаетсяС(ВРег(ИмяПроцедуры), "ВНЕШНЯЯОБРАБОТКА.") Тогда
			Возврат;
		КонецЕсли;
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для вызова процедуры внешнего отчета или внешней обработки в фоне
			           |требуется заполнить свойство %1 параметра %2 в %3.'"),
			"ВнешнийОтчетОбработка", "ПараметрыВыполнения", ИмяМетода);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	
	ТипыДанных = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("ДвоичныеДанные"));
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		МодульДополнительныеОтчетыИОбработки = ОбщегоНазначения.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
		ТипыДанных.Добавить(МодульДополнительныеОтчетыИОбработки.ТипДополнительныйОтчетИлиОбработка());
	КонецЕсли;
	
	ОжидаемыеТипыСвойств = Новый Структура;
	ОжидаемыеТипыСвойств.Вставить("Данные", Новый ОписаниеТипов(ТипыДанных));
	ОжидаемыеТипыСвойств.Вставить("ПолноеИмяФайла", Тип("Строка"));
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяМетода,
		"ПараметрыВыполнения.ВнешнийОтчетОбработка",
		ПараметрыВыполнения.ВнешнийОтчетОбработка,
		Тип("Структура"),
		ОжидаемыеТипыСвойств);
	
	Если Не ЗначениеЗаполнено(ПараметрыВыполнения.ВнешнийОтчетОбработка.Данные) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для вызова процедуры внешнего отчета или внешней обработки в фоне
			           |требуется заполнить свойство %1 параметра %2 в %3.'"),
			"ВнешнийОтчетОбработка.Данные", "ПараметрыВыполнения", ИмяМетода);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
Процедура ПроверитьИЗаполнитьАдресРезультата(ПараметрыВыполнения, ИмяФункции)
	...
	Если ПараметрыВыполнения.АдресРезультата = Неопределено Тогда
		...
		Если  ОбщегоНазначения.РежимОтладки()
		   И (Не ЗначениеЗаполнено(ПараметрыВыполнения.ИдентификаторФормы)
		      Или ТипЗнч(ПараметрыВыполнения.ИдентификаторФормы) <> Тип("УникальныйИдентификатор")) Тогда
			
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не указан ни уникальный идентификатор формы в параметре %1,
				...
				"ПараметрыВыполнения.ИдентификаторФормы", "ПараметрыВыполнения.АдресРезультата", ИмяФункции);
			Попытка
				ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
			Исключение
	...
	ИначеЕсли Не ЭтоАдресВременногоХранилища(ПараметрыВыполнения.АдресРезультата) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Указан некорректный адрес временного хранилища в параметре %1
			           |функции %2.'"),
			"ПараметрыВыполнения.АдресРезультата", ИмяФункции);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	
КонецПроцедуры

Функция ЭтоВыполнениеБезФоновогоЗадания(ИмяПроцедуры, ПараметрыВыполнения)
...
#Если ВнешнееСоединение Тогда
	...
	КонецЕсли;
#Иначе
	Если ПараметрыВыполнения.БезРасширений
	 Или ПараметрыВыполнения.СРасширениямиБазыДанных Тогда
		Возврат Ложь;
	КонецЕсли;
#КонецЕсли
	...
	Возврат ОбщегоНазначения.РежимОтладки()
		Или СтандартныеПодсистемыСервер.ОтключенаЛогикаНачалаРаботыСистемы()
		  И Не СтандартныеПодсистемыСервер.ЭтоСеансФоновогоЗадания()
		Или ПараметрыВыполнения.ЗапуститьНеВФоне
		Или Не ПараметрыВыполнения.ЗапуститьВФоне
		  И ЕстьФоновыеЗаданияВФайловойИБ()
...
Функция ОбщиеПараметрыВыполненияВФоне()
	...
	Результат.Вставить("УточнениеОшибки", "");
	Результат.Вставить("ЭтоФункция", ""); // служебный
	Результат.Вставить("ЭтоПерезапускУправляющегоПотока", Ложь); // служебный
	Результат.Вставить("СвойстваИсполняющегоПотокаМногопоточнойДлительнойОперации", Неопределено); // служебный
...
Функция ВыполнитьМногопоточныйПроцесс(ПараметрыОперации) Экспорт
	...
	Пока Истина Цикл
		...
		// @skip-check query-in-loop - Получение данных о потоках текущего процесса.
		...
		Если Не ДинамическоеПолучениеПорций И КоличествоОбработанныхПорций = Неопределено Тогда
			КоличествоОбработанныхПорций = КоличествоПорций - Потоки.Количество();
		КонецЕсли;
		
		Для Каждого Поток Из Потоки Цикл
			
			ЗавершитьДосрочно = ПроконтролироватьВыполнениеПотока(Поток, ПараметрыОперации, ИдентификаторЗаданияПроцесса);
		...
		// @skip-check query-in-loop - На каждой итерации необходимо получать актуальные данные из ИБ.
		ОжидатьЗавершениеВсехПотоков(ИдентификаторПроцесса, ПрерватьВыполнениеЕслиОшибка, ЗавершитьДосрочно);
	...
	РезультатВыполненияПотока = НовыйРезультатВыполненияПотока();
	...
	Для Каждого Поток Из ПотокиПроцесса Цикл
		...
		Результаты.Вставить(Ключ, Новый Структура(Новый ФиксированнаяСтруктура(РезультатВыполненияПотока)));
		Результаты[Ключ].АдресРезультата = ПоместитьВоВременноеХранилище(Поток.Результат.Получить(), Поток.АдресРезультата);
		Результаты[Ключ].ИнформацияОбОшибке = Поток.ХранилищеИнформацииОбОшибке.Получить();
...
Функция ПроконтролироватьВыполнениеПотока(Поток, ПараметрыОперации, ИдентификаторЗаданияПроцесса)
	...
	Если Результат.Статус = СтатусДлительнойОперации().Ошибка И ПрерватьВыполнениеЕслиОшибка Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ВыполнитьПоток(Поток, ПараметрыОперации, ВыполнитьВФоне, ИдентификаторЗаданияПроцесса)
	...
	ПараметрыВыполнения.СвойстваИсполняющегоПотокаМногопоточнойДлительнойОперации = СвойстваПотока;
	...
	ПараметрыВыполнения = ПодготовитьПараметрыВыполнения(ПараметрыВыполнения, ПараметрыОперации.ЭтоФункция);
	ПараметрыВыполнения.ВнешнийОтчетОбработка = ПараметрыОперации.ПараметрыВыполнения.ВнешнийОтчетОбработка;
	
	ПотокЗанят = Ложь;
	НоваяПопыткаВыполнения = Поток.Статус <> СтатусДлительнойОперации().Создано;
	
	Если ПараметрыВыполнения.ЗапуститьНеВФоне = Ложь Тогда
		...
		Попытка
			...
			Если Потоки.Количество() < МаксимумПотоков Тогда
				ВременныйРезультатЗапуска = НовыйРезультатВыполненияПотока();
				...
				ОбновитьСведенияОПотоке(Поток, ВременныйРезультатЗапуска);
				ПотокЗанят = Истина;
			КонецЕсли;
	...
	Если ПараметрыВыполнения.ЗапуститьНеВФоне = Ложь И Не ПотокЗанят Тогда
	...
	Исключение
		...
		Если ПотокЗанят Тогда
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке,
				НСтр("ru = 'Не удалось запустить задание обработки порции по причине:'"));
			Попытка
				ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, ИнформацияОбОшибке);
			Исключение
				УточненнаяИнформацияОбОшибке = ИнформацияОбОшибке();
			КонецПопытки;
			ОшибочныйРезультатЗапуска = НовыйРезультатВыполненияПотока();
			ОшибочныйРезультатЗапуска.Статус = СтатусДлительнойОперации().Ошибка;
			УстановитьСвойстваОшибки(ОшибочныйРезультатЗапуска, УточненнаяИнформацияОбОшибке);
			ОбновитьСведенияОПотоке(Поток, ОшибочныйРезультатЗапуска);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки;
	УстановитьПолноеИмяПрикладнойПроцедуры(ИмяМетодаМногопоточногоПроцесса());
	ОбновитьСведенияОПотоке(Поток, РезультатЗапуска, НоваяПопыткаВыполнения);
	
	Возврат РезультатЗапуска;
	
КонецФункции

Процедура ОбновитьСведенияОПотоке(Поток, РезультатЗапуска, НоваяПопытка = Ложь,
			ПерезапускПроцесса = Ложь, УстановитьЗавершениеАктивного = Ложь)
	...
	Если Не РезультатЗапускаУказан Тогда
		РезультатЗапуска = НовыйРезультатВыполненияПотока(); 
		
		Если ЗначениеЗаполнено(Поток.ИдентификаторЗадания) Тогда
			ПоследнийИдентификатор = ПоследнийИдентификаторЗаданияПотока(Поток);
			Результат = ЗаданиеВыполнено(ПоследнийИдентификатор, Истина);
			РезультатЗапуска.Статус                       = СтатусДлительнойОперации()[Результат.Статус];
			РезультатЗапуска.ИнформацияОбОшибке           = Результат.ИнформацияОбОшибке;
			РезультатЗапуска.КраткоеПредставлениеОшибки   = Результат.КраткоеПредставлениеОшибки;
			РезультатЗапуска.ПодробноеПредставлениеОшибки = Результат.ПодробноеПредставлениеОшибки;
		Иначе
	...
	Попытка
		...
		СтатусОбновлен = Неопределено;
		
		Если НаборЗаписей.Количество() > 0 Тогда
			Запись = НаборЗаписей.Получить(0);
			СтатусОбновлен = Ложь;
			
			Если РезультатЗапускаУказан И Не УстановитьЗавершениеАктивного Тогда
				Если ПерезапускПроцесса Тогда
					Запись.КлючПотока = Новый ХранилищеЗначения(РезультатЗапуска.ИдентификаторЗадания, Новый СжатиеДанных(9));
				Иначе
			...
			Если Не УстановитьЗавершениеАктивного
			 Или Запись.Статус = СтатусДлительнойОперации().Выполняется
			   И ПоследнийИдентификаторЗаданияПотока(Запись) = ПоследнийИдентификаторЗаданияПотока(Поток) Тогда
				...
				Если РезультатЗапуска.Статус = СтатусДлительнойОперации().Ошибка Тогда
					Запись.ПараметрыМетода = Новый ХранилищеЗначения(РезультатЗапуска.ИнформацияОбОшибке, Новый СжатиеДанных(9));
	...
	УстановитьЗавершениеАктивного = СтатусОбновлен;
...
Функция ПервыйИдентификаторЗаданияУправляющегоПотока(ИдентификаторПроцесса)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Управляющий поток многопоточной длительной операции %1 не существует.'"),
		Строка(ИдентификаторПроцесса));
	ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	
КонецФункции

Функция ПоследнийИдентификаторЗаданияПотока(Поток)
...
Процедура УстановитьРезультатПотока(СвойстваПотока, Результат)
	...
	Результат = Новый ХранилищеЗначения(Результат, Новый СжатиеДанных(9));
...
//
Процедура ОжидатьЗавершениеПотока(Поток, Длительность = 1)
...
Функция ЕстьЗавершенныеПотоки(Потоки, ЗавершитьДосрочноЕслиОшибка, ИдентификаторПроцесса)
	...
	Пока Индекс >= 0 Цикл
		...
		Если Результат.Статус = "Выполняется"
		 Или Результат.Статус = "Ошибка"
		   И Результат.Задание = Неопределено
		   И Не ЭтоПотокТекущегоПроцесса Тогда
			Продолжить;
		КонецЕсли;
		
		СтатусОбновлен = Истина;
		ОбновитьСведенияОПотоке(Поток, Неопределено,,, СтатусОбновлен);
		ОбновленныйСтатус = Поток.Статус;
		
		Если СтатусОбновлен <> Неопределено
		   И ОбновленныйСтатус = СтатусДлительнойОперации().Выполняется Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтатусОбновлен <> Неопределено
		   И ОбновленныйСтатус = СтатусДлительнойОперации().Выполнено Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтатусОбновлен = Истина
		   И ОбновленныйСтатус = СтатусДлительнойОперации().Ошибка Тогда
			
			ЗаписатьОшибку(Результат.ПодробноеПредставлениеОшибки);
		КонецЕсли;
	...
	Для Каждого ИдентификаторПроцесса Из ИдентификаторыПроцессов Цикл
		...
		Если УправляющийПоток <> Неопределено
		   И УправляющийПоток.ДатаСоздания + СрокОтмены > ТекущаяДатаСеанса Тогда
			...
			ИдентификаторЗадания = ПоследнийИдентификаторЗаданияПотока(УправляющийПоток);
			Задание = НайтиЗаданиеПоИдентификатору(ИдентификаторЗадания);
			Если Задание = Неопределено
			   И (УправляющийПоток.Статус = СтатусДлительнойОперации().Выполняется
			      Или УправляющийПоток.ДатаСоздания + СрокУстаревания > ТекущаяДатаСеанса
			      Или УправляющийПоток.Статус = СтатусДлительнойОперации().Ошибка
			        И УправляющийПоток.НомерПопытки < КоличествоПопыток())
			 Или Задание <> Неопределено
			   И (    Задание.Состояние = СостояниеФоновогоЗадания.Активно
			      Или Задание.Конец + СрокУстаревания > ТекущаяДатаСеанса
			      Или Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно
			        И УправляющийПоток.НомерПопытки < КоличествоПопыток()) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого ПотокПроцесса Из ПотокиПроцесса Цикл
			ИдентификаторЗадания = ПоследнийИдентификаторЗаданияПотока(ПотокПроцесса);
			Задание = НайтиЗаданиеПоИдентификатору(ИдентификаторЗадания);
			Если Задание <> Неопределено
			   И Задание.Состояние = СостояниеФоновогоЗадания.Активно Тогда
...
//      * НомерПопытки - Число
//      * ХранилищеИнформацииОбОшибке - ХранилищеЗначения
...
Функция ПотокиДлительныхОпераций(ИдентификаторПроцесса = Неопределено)
	...
	|	ДлительныеОперации.ПараметрыМетода КАК ХранилищеИнформацииОбОшибке,
...
//   * ПодробноеПредставлениеОшибки - Строка - устарел.
//
Функция НовыйРезультатВыполненияПотока()
...
Процедура ПодготовитьМногопоточнуюОперациюКЗапуску(Знач ИмяМетода, АдресаРезультатов,
	...
	Для каждого КлючЗначение Из Порции Цикл
		...
		Запись.КлючПотока            = Новый ХранилищеЗначения(КлючЗначение.Ключ, Новый СжатиеДанных(9));
		Запись.АдресРезультата       = АдресаРезультатов[КлючЗначение.Ключ];
		...
		Запись.ПараметрыПотока       = Новый ХранилищеЗначения(КлючЗначение.Значение, Новый СжатиеДанных(9));
	...
	Иначе
		...
		Попытка
			...
			НаборИзОднойЗаписи[0].ПараметрыВыполнения = Новый ХранилищеЗначения(ОбновленныеПараметрыОперации, Новый СжатиеДанных(9));
...
Процедура ПроверитьВозможностьЗапускаМногопоточнойДлительнойОперации(ПараметрыВыполнения, НаборПараметров, ИмяФункции)
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяФункции,
		"ПараметрыВыполнения", ПараметрыВыполнения, Тип("Структура"));
	
	Если НаборПараметров <> Неопределено
	   И ТипЗнч(НаборПараметров) <> Тип("Соответствие")
	   И ТипЗнч(НаборПараметров) <> Тип("Структура") Тогда
		
		ТекстОшибки = НСтр("ru = 'Неверный тип набора параметров многопоточной длительной операции.'");
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
// См. ВыполнитьФункциюВНесколькоПотоков
Функция ВыполнитьМетодВНесколькоПотоков(Знач ИмяМетода, Знач ПараметрыВыполнения, Знач НаборПараметровМетода,
			Знач ЭтоВыполнениеФункции)
	
	НовыеПараметрыВыполнения = ПараметрыВыполненияФункции(Неопределено);
	ЗаполнитьЗначенияСвойств(НовыеПараметрыВыполнения, ПараметрыВыполнения);
	ПараметрыВыполнения = НовыеПараметрыВыполнения;
	
	ИмяФункции = ?(ЭтоВыполнениеФункции, "ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков",
	"ДлительныеОперации.ВыполнитьПроцедуруВНесколькоПотоков");

	ПроверитьВозможностьЗапускаМногопоточнойДлительнойОперации(ПараметрыВыполнения, НаборПараметровМетода, ИмяФункции);
	
	ПроверитьИЗаполнитьАдресРезультата(ПараметрыВыполнения, ИмяФункции);
	...
	Если ТипЗнч(НаборПараметровМетода) = Тип("Соответствие") Тогда
		Для Каждого ПараметрФункции Из НаборПараметровМетода Цикл
			АдресРезультатаПотока = ПоместитьВоВременноеХранилище(Неопределено, Новый УникальныйИдентификатор);
			АдресаРезультатов.Вставить(ПараметрФункции.Ключ, АдресРезультатаПотока);
		КонецЦикла;
		ПараметрыМетода = НаборПараметровМетода.Количество();
	Иначе
		ПараметрыМетода = НаборПараметровМетода; // Структура
		НаборПараметровМетода = Новый Соответствие;
	КонецЕсли;
	...
	ПараметрыМногопоточнойОперации.ИмяМетода = ИмяМетода;
	ПараметрыМногопоточнойОперации.ЭтоФункция = ?(ЭтоВыполнениеФункции, "Функция", "Процедура");
	...
	ПодготовитьМногопоточнуюОперациюКЗапуску(ИмяМетода,
		АдресаРезультатов, ИдентификаторПроцесса, НаборПараметровМетода);
	
	РезультатЗапуска = Новый Структура("Статус, ИдентификаторЗадания, АдресРезультата",
		СтатусДлительнойОперации().Выполняется,, ПараметрыВыполнения.АдресРезультата);
	ЗапланироватьЗапускПотоковДлительныхОпераций(РезультатЗапуска, ПараметрыМногопоточнойОперации);
	
	// Имя процедуры должно быть, как указано в функции ИмяМетодаМногопоточногоПроцесса.
	РезультатЗапуска = ВыполнитьФункцию(ПараметрыВыполнения,
		"ДлительныеОперации.ВыполнитьМногопоточныйПроцесс", ПараметрыМногопоточнойОперации);
	...
	Иначе
		УстановитьОтключениеБезопасногоРежима(Истина);
		УстановитьПривилегированныйРежим(Истина);
		ЗарегистрироватьИдентификаторУправляющегоПотока(РезультатЗапуска.ИдентификаторЗадания, ИдентификаторПроцесса);
		УстановитьПривилегированныйРежим(Ложь);
		УстановитьОтключениеБезопасногоРежима(Ложь);
	КонецЕсли;
...
Процедура ЗарегистрироватьИдентификаторУправляющегоПотока(ИдентификаторЗадания, ИдентификаторПроцесса)
	
	Если ПараметрыСеанса.ДлительныеОперации.Запущенные.Получить(ИдентификаторЗадания) = Неопределено Тогда
		Свойства = Новый Структура(ПараметрыСеанса.ДлительныеОперации);
		Запущенные = Новый Соответствие(Свойства.Запущенные);
		Запущенные.Вставить(ИдентификаторЗадания, ИдентификаторПроцесса);
		Свойства.Запущенные = Новый ФиксированноеСоответствие(Запущенные);
		ПараметрыСеанса.ДлительныеОперации = Новый ФиксированнаяСтруктура(Свойства);
	КонецЕсли;
...
Функция ВыполненПерезапускУправляющегоПотока(ИдентификаторЗадания, Задание)
	...
	Если ИдентификаторПроцесса = Неопределено
	 Или Задание <> Неопределено
	...
	|ГДЕ
	|	ДлительныеОперации.ИдентификаторПроцесса = &ИдентификаторПроцесса
	|	И ДлительныеОперации.ИдентификаторПотока = &ИдентификаторПотока";
	...
	Попытка
		...
		ПараметрыВыполнения.ЭтоПерезапускУправляющегоПотока = Истина;
		
		// Имя процедуры должно быть, как указано в функции ИмяМетодаМногопоточногоПроцесса.
		РезультатЗапуска = ВыполнитьФункцию(ПараметрыВыполнения,
			"ДлительныеОперации.ВыполнитьМногопоточныйПроцесс", ПараметрыОперации);
		
		Если Не ЗначениеЗаполнено(РезультатЗапуска.ИдентификаторЗадания) Тогда
			ВызватьИсключение(НСтр("ru = 'Пустой идентификатор фонового задания.'"), 
				КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
		НовоеЗадание = НайтиЗаданиеПоИдентификатору(РезультатЗапуска.ИдентификаторЗадания);
		Если НовоеЗадание = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Фоновое задание с идентификатором %1 не существует.'"),
		...
		ОбновитьСведенияОПотоке(Поток, РезультатЗапуска, Истина, Истина);
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура УстановитьЗавершениеДляУправляющегоПотока(ИдентификаторЗадания, Результат)
	...
	Запрос.УстановитьПараметр("ИдентификаторПроцесса", ИдентификаторПроцесса);
	Запрос.УстановитьПараметр("ИдентификаторПотока", ПустойИдентификатор);
	Запрос.УстановитьПараметр("Статус", СтатусДлительнойОперации().Выполняется);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	ДлительныеОперации.ИдентификаторПроцесса = &ИдентификаторПроцесса
	|	И ДлительныеОперации.ИдентификаторПотока = &ИдентификаторПотока
	|	И ДлительныеОперации.Статус = &Статус";
	
	Если Не Запрос.Выполнить().Пустой() Тогда
		// Установка статуса Выполнено у основного потока.
		РезультатЗапуска = НовыйРезультатВыполненияПотока();
		РезультатЗапуска.Статус                       = СтатусДлительнойОперации()[Результат.Статус];
		РезультатЗапуска.ИнформацияОбОшибке           = Результат.ИнформацияОбОшибке;
		РезультатЗапуска.КраткоеПредставлениеОшибки   = Результат.КраткоеПредставлениеОшибки;
		РезультатЗапуска.ПодробноеПредставлениеОшибки = Результат.ПодробноеПредставлениеОшибки;
		ОписаниеОсновногоПотока = Новый Структура;
		ОписаниеОсновногоПотока.Вставить("ИдентификаторПроцесса", ИдентификаторПроцесса);
		ОписаниеОсновногоПотока.Вставить("ИдентификаторПотока",   ПустойИдентификатор);
		ОписаниеОсновногоПотока.Вставить("ИдентификаторЗадания",  ИдентификаторЗадания);
		ОписаниеОсновногоПотока.Вставить("КлючПотока",
			Новый ХранилищеЗначения(ПоследнийИдентификатор(ИдентификаторЗадания)));
		
		ОбновитьСведенияОПотоке(ОписаниеОсновногоПотока, РезультатЗапуска,,, Истина);
	КонецЕсли;
	
	// Удаление из запущенных.
	Свойства = Новый Структура(ПараметрыСеанса.ДлительныеОперации);
	Запущенные = Новый Соответствие(Свойства.Запущенные);
	Запущенные.Удалить(ИдентификаторЗадания);
	Свойства.Запущенные = Новый ФиксированноеСоответствие(Запущенные);
	ПараметрыСеанса.ДлительныеОперации = Новый ФиксированнаяСтруктура(Свойства);
...
Процедура ЗапланироватьЗапускПотоковДлительныхОпераций(РезультатЗапуска, ПараметрыОперации)
	...
	Попытка
		...
		Запись.ИдентификаторЗадания  = РезультатЗапуска.ИдентификаторЗадания;
		Запись.Статус                = РезультатЗапуска.Статус;
		Запись.ПараметрыВыполнения   = Новый ХранилищеЗначения(ПараметрыОперации, Новый СжатиеДанных(9));
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьФункцию";
	ИсключениеПроверки.ФрагментСодержания = "Возврат ВыполнитьВФоне(ИмяФункции, ПараметрыВызова, ПараметрыВыполнения)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПроцедуру";
	ИсключениеПроверки.ФрагментСодержания = "Возврат ВыполнитьВФоне(ИмяПроцедуры, ПараметрыВызова, ПараметрыВыполнения)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВызватьПроцедуру";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяПроцедуры, ПараметрыВызова)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВызватьПроцедуру";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[3], ПараметрыВызова)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВызватьФункцию";
	ИсключениеПроверки.ФрагментСодержания = "Результат = ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяФункции, ПараметрыПроцедуры, Истина)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВызватьФункцию";
	ИсключениеПроверки.ФрагментСодержания = "Результат = ОбщегоНазначения.ВыполнитьМетодОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[3], ПараметрыПроцедуры, Истина)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьМногопоточныйПроцесс";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяМетодаПолученияПорций, ПараметрыПолученияПорций)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПоток";
	ИсключениеПроверки.ФрагментСодержания = "РезультатЗапуска = ВыполнитьВФоне(ПараметрыОперации.ИмяМетода, ПараметрыМетода, ПараметрыВыполнения)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПроцедуруМодуляОбъектаОтчета";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(Отчет, Параметры.ИмяМетода, ПараметрыМетода)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПроцедуруМодуляОбъектаОбработки";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(Обработка, Параметры.ИмяМетода, ПараметрыМетода)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ДлительныеОперации";
	ИсключениеПроверки.ИмяПроцедуры       = "ЗапуститьВыполнениеВФоне";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяЭкспортнойПроцедуры, ПараметрыЭкспортнойПроцедуры)";
	...
	Методы.Вставить("ВыполнитьПроцедуруМодуляОбъектаОбработки", Истина);
	Методы.Вставить("ВыполнитьПроцедуруМодуляОбъектаОтчета", Истина);
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ДлительныеОперации", Методы);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьМногопоточныйПроцесс", Истина);
	Методы.Вставить("ВыполнитьПроцедуруМодуляОбъектаОбработки", Истина);
	Методы.Вставить("ВыполнитьПроцедуруМодуляОбъектаОтчета", Истина);
