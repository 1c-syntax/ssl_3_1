...
Процедура ВыполнитьСКонтекстомКлиента(ВсеПараметры) Экспорт
	...
	Исключение
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.22.2009") < 0 Тогда
			СтекОшибки = НСтр("ru = 'Стек ошибки фонового задания:'") + Символы.ПС
				+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			СтекЗапуска = СтекОшибки + ?(ЗначениеЗаполнено(СтекЗапуска),
				Символы.ПС + Символы.ПС + СтекЗапуска, "");
		КонецЕсли;
	...
	Попытка
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			Если РезультатЗапуска.Статус = СтатусДлительнойОперации().Ошибка Тогда
				Запись.ПараметрыМетода = Новый ХранилищеЗначения(РезультатЗапуска.ИнформацияОбОшибке);
