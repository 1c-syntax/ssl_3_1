...
Функция ВыполнитьФункциюВНесколькоПотоков(ИмяФункции, Знач ПараметрыВыполнения, Знач НаборПараметровФункции = Неопределено) Экспорт
	...
	Если НаборПараметровФункции <> Неопределено
	   И ТипЗнч(НаборПараметровФункции) <> Тип("Соответствие")
	   И ТипЗнч(НаборПараметровФункции) <> Тип("Структура") Тогда
	...
	ПараметрыМногопоточнойОперации.Вставить("ИдентификаторПроцесса", Новый УникальныйИдентификатор);
	ПараметрыМногопоточнойОперации.Вставить("ИмяМетода",             ИмяФункции);
	ПараметрыМногопоточнойОперации.Вставить("ДляФункции",            Истина);
	ПараметрыМногопоточнойОперации.Вставить("ПараметрыВыполнения",   ПараметрыВыполнения);
	...
	Возврат ВыполнитьФункцию(ПараметрыВыполнения, ИмяМетодаМногопоточногоПроцесса(), ПараметрыМногопоточнойОперации);
...
Функция ВыполнитьПроцедуруВНесколькоПотоков(ИмяПроцедуры, Знач ПараметрыВыполнения, Знач НаборПараметровПроцедуры = Неопределено) Экспорт
	...
	Если НаборПараметровПроцедуры <> Неопределено
	   И ТипЗнч(НаборПараметровПроцедуры) <> Тип("Соответствие")
	   И ТипЗнч(НаборПараметровПроцедуры) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("ru ='Передан неверный тип набора параметров'");
	КонецЕсли;
	...
	Результат = ВыполнитьФункцию(ПараметрыВыполнения, ИмяМетодаМногопоточногоПроцесса(), ПараметрыМногопоточнойОперации);
...
Функция ВыполнитьМногопоточныйПроцесс(ПараметрыОперации) Экспорт
	...
	ПотокиПроцесса = НовоеОписаниеПотоков();
	...
	ИмяМетодаПолученияПорций = Неопределено;
	КонтекстПолученияИОбработкиПорций = Новый Структура;
	
	Порции = Новый Массив;
	Результаты = Новый Соответствие();
	
	Если ТипЗнч(ПараметрыОперации.ПараметрыМетода) = Тип("Соответствие") Тогда
		Для Каждого КлючЗначение Из ПараметрыОперации.ПараметрыМетода Цикл
			Порции.Добавить(КлючЗначение);
		...
		КонецЦикла;
	Иначе
		ПараметрыОперации.ПараметрыМетода.Свойство("ИмяМетодаПолученияПорций", ИмяМетодаПолученияПорций);
		ПараметрыОперации.ПараметрыМетода.Свойство("Контекст", КонтекстПолученияИОбработкиПорций);
		КонтекстПолученияИОбработкиПорций.Вставить("Процент", 0);
	КонецЕсли;
	
	КоличествоОбработанныхПорций = 0;
	ИндексТекущейПорции = -1;
	...
	Попытка
		...
		Пока Истина Цикл
			ИндексТекущейПорции = ИндексТекущейПорции + 1;
			
			Если ИндексТекущейПорции >= Порции.Количество()
			   И ИмяМетодаПолученияПорций <> Неопределено Тогда
				...
				Для Каждого КлючИЗначение Из НовыеПорции Цикл
					Порции.Добавить(КлючИЗначение);
					Результаты.Вставить(КлючИЗначение.Ключ,
						Новый Структура(Новый ФиксированнаяСтруктура(РезультатДлительнойОперации)));
				КонецЦикла;
			КонецЕсли;
			
			Если ИндексТекущейПорции >= Порции.Количество() Тогда
				Прервать;
			КонецЕсли;
			КлючЗначение = Порции.Получить(ИндексТекущейПорции);
			
			ПорцияОбработана = Ложь;
			Пока Не ПорцияОбработана Цикл
				
				ВыполнитьВФоне = ОжидатьСвободныйПоток(ИдентификаторПроцесса, ПотокиПроцесса,
					Результаты, ПрерватьВыполнениеЕслиОшибка);
				Если ВыполнитьВФоне = Неопределено Тогда
					ЗавершитьДосрочно = Истина;
					Прервать;
				КонецЕсли;
				
				Поток = ДобавитьПоток(ПотокиПроцесса, ИдентификаторПроцесса, КлючЗначение.Ключ, КлючЗначение.Значение, ВыполнитьВФоне);
				
				Если ТипЗнч(Поток) = Тип("СтрокаТаблицыЗначений") Тогда
					Результат = ВыполнитьПоток(ПараметрыОперации, ПотокиПроцесса, Поток, Результаты[КлючЗначение.Ключ], ВыполнитьВФоне);
					
					ЗаполнитьЗначенияСвойств(Результаты[КлючЗначение.Ключ], Результат);
					Если Результат.Статус <> "Выполняется" Тогда
						ОтправитьСообщенияПотока(Результат.ИдентификаторЗадания);
					КонецЕсли;
					
					Если Результат.Статус = "Ошибка" И ПрерватьВыполнениеЕслиОшибка Тогда
						ЗавершитьДосрочно = Истина;
						Прервать;
					КонецЕсли;
			...
			Если ЗавершитьДосрочно Тогда
				...
				ОтменитьВыполнениеВсехПотоков(ПотокиПроцесса, Результаты);
				Прервать;
			КонецЕсли;
			
			Если ИмяМетодаПолученияПорций = Неопределено Тогда
		...
		ОжидатьЗавершениеВсехПотоков(ПотокиПроцесса, Результаты, ИдентификаторПроцесса, ПрерватьВыполнениеЕслиОшибка);
...
Функция ДобавитьПоток(Потоки, ИдентификаторПроцесса, КлючПотока, ПараметрыПотока, ВыполнитьВФоне)
	...
	Попытка
		...
		Если КоличествоРаботающихПотоков() <= ДопустимоеКоличествоПотоков() Тогда
			...
			ЗаполнитьЗначенияСвойств(Запись, Поток, , "ПараметрыМетода,КлючПотока");
...
//   см. ВыполнитьВФоне
//
Функция ВыполнитьПоток(ПараметрыОперации, ПотокиПроцесса, Поток, Результат, ВыполнитьВФоне)
	...
	РезультатЗапуска = ВыполнитьВФоне(ПараметрыОперации.ИмяМетода, Поток.ПараметрыМетода, ПараметрыВыполнения);
...
Функция ОжидатьСвободныйПоток(ИдентификаторПроцесса, ПотокиПроцесса, Результаты, ЗавершитьДосрочноЕслиОшибка)
	...
	Пока Истина Цикл
		...
		ЕстьАктивныйПотокПроцесса = Ложь;
		
		Для Каждого ПотокПроцесса Из ПотокиПроцесса Цикл
			Поток = Потоки.Найти(ПотокПроцесса.ИдентификаторПотока, "ИдентификаторПотока");
			Если Поток = Неопределено Тогда
				Поток = Потоки.Добавить();
			Иначе
				ЕстьАктивныйПотокПроцесса = Истина;
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(Поток, ПотокПроцесса);
		КонецЦикла;
		
		ИдентификаторыПроцессов = Потоки.Скопировать(, "ИдентификаторПроцесса");
		ИдентификаторыПроцессов.Свернуть("ИдентификаторПроцесса");
		КоличествоВсехАктивныхПотоков = Потоки.Количество()
			+ ИдентификаторыПроцессов.Количество() - ?(ЕстьАктивныйПотокПроцесса, 1, 0);
		
		Если КоличествоВсехАктивныхПотоков < МаксимумПотоков Тогда
			Прервать;
		КонецЕсли;
		
		ЕстьЗавершенныеПотоки = ЕстьЗавершенныеПотоки(Потоки,
			Результаты, ЗавершитьДосрочноЕслиОшибка, ИдентификаторПроцесса);
		
		ИндексПотока = ПотокиПроцесса.Количество() - 1;
		Пока ИндексПотока >= 0 Цикл
			ПотокПроцесса = ПотокиПроцесса[ИндексПотока];
			Если Потоки.Найти(ПотокПроцесса.ИдентификаторПотока, "ИдентификаторПотока") = Неопределено Тогда
				ПотокиПроцесса.Удалить(ПотокПроцесса);
			КонецЕсли;
			ИндексПотока = ИндексПотока - 1;
		КонецЦикла;
		
		Если ЗавершитьДосрочноЕслиОшибка И ЕстьЗавершенныеПотоки = Неопределено Тогда
			ВыполнитьВФоне = Неопределено;
			Прервать;
		КонецЕсли;
		...
		Если Не ЕстьАктивныйПотокПроцесса Тогда
...
Процедура ОжидатьЗавершениеВсехПотоков(Потоки, Результаты, ИдентификаторПроцесса, ЗавершитьДосрочноЕслиОшибка)
	...
	Пока Потоки.Количество() > 0 Цикл
		...
		Если ЕстьЗавершенныеПотоки = Неопределено Тогда
			ОтменитьВыполнениеВсехПотоков(Потоки, Результаты);
			Прервать;
		КонецЕсли;
...
Функция ЕстьЗавершенныеПотоки(Потоки, Результаты, ЗавершитьДосрочноЕслиОшибка, ИдентификаторПроцесса)
	...
	Пока Индекс >= 0 Цикл
		...
		КлючПотока = Поток.КлючПотока;
		
		Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
			Попытка
				ЗаданиеВыполнено = ЗаданиеВыполнено(ИдентификаторЗадания);
				
				Если Поток.ИдентификаторПроцесса = ИдентификаторПроцесса
				   И КлючПотока <> Неопределено
				   И ЗаданиеВыполнено = Истина Тогда
			...
			Исключение
				...
				Если Поток.ИдентификаторПроцесса = ИдентификаторПроцесса
				   И КлючПотока <> Неопределено Тогда
...
// 
Процедура ОтменитьВыполнениеВсехПотоков(Потоки, Результаты)
	...
	Для Каждого Поток Из Потоки Цикл
		...
		Если КлючПотока <> Неопределено
		   И Результаты[КлючПотока].Статус <> "Ошибка" Тогда
...
Процедура СохранитьПоток(Поток)
	...
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ДлительныеОперации"); 
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	ЭлементБлокировки.УстановитьЗначение("ИдентификаторПотока", Поток.ИдентификаторПотока);
...
Функция ПотокиДлительныхОпераций(ИдентификаторПроцесса = Неопределено)
	...
	Для Каждого Поток Из Результат Цикл
		...
		ЗаполнитьЗначенияСвойств(Строка, Поток, , "КлючПотока, ПараметрыМетода");
		...
		КонецЕсли;
		Если Поток.КлючПотока <> Неопределено Тогда
			Строка.ПараметрыМетода = Поток.КлючПотока.Получить();
		КонецЕсли;
