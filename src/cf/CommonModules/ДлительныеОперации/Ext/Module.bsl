...
Функция ВыполнитьМногопоточныйПроцесс(ПараметрыОперации) Экспорт
	...
	Пока Истина Цикл
		...
		Для Каждого Поток Из Потоки Цикл
			...
			Результат = ВыполнитьПоток(Поток, ПараметрыОперации, ВыполнитьВФоне);
...
//   см. ВыполнитьВФоне
//
Функция ВыполнитьПоток(Поток, ПараметрыОперации, ВыполнитьВФоне)
...
Процедура ОбновитьСведенияОбПотоке(Поток, РезультатЗапуска = Неопределено)
	...
	Если РезультатЗапуска = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Поток.ИдентификаторЗадания) Тогда
			...
			Если Задание <> Неопределено Тогда
				...
				Если Задание.ИнформацияОбОшибке  <> Неопределено Тогда
					РезультатЗапуска.КраткоеПредставлениеОшибки =
						ОбработкаОшибок.КраткоеПредставлениеОшибки(Задание.ИнформацияОбОшибке);
					РезультатЗапуска.ПодробноеПредставлениеОшибки =
...
Функция ЕстьЗавершенныеПотоки(Потоки, ЗавершитьДосрочноЕслиОшибка, ИдентификаторПроцесса)
	...
	Пока Индекс >= 0 Цикл
		...
		Если ЗавершитьДосрочноЕслиОшибка = Истина
		   И Поток.ИдентификаторПроцесса = ИдентификаторПроцесса Тогда
...
Функция ВыполненПерезапускУправляющегоПотока(ИдентификаторЗадания, Задание)
	...
	Если Задание <> Неопределено
	   И Задание.Состояние <> СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		Возврат Ложь;
	КонецЕсли;
