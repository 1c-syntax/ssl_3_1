...
Функция ВыполнитьФункциюВНесколькоПотоков(ИмяФункции, Знач ПараметрыВыполнения, Знач НаборПараметровФункции = Неопределено) Экспорт
	...
	Возврат ВыполнитьФункцию(ПараметрыВыполненияДочернихПотоков, "ДлительныеОперации.ВыполнитьМногопоточныйПроцесс", ПараметрыМногопоточнойОперации);
...
Функция ВыполнитьПроцедуруВНесколькоПотоков(ИмяПроцедуры, Знач ПараметрыВыполнения, Знач НаборПараметровПроцедуры = Неопределено) Экспорт
	...
	Результат = ВыполнитьФункцию(ПараметрыВыполненияДочернихПотоков, "ДлительныеОперации.ВыполнитьМногопоточныйПроцесс", ПараметрыМногопоточнойОперации);
	...
	Попытка
		...
		Для Каждого КлючЗначение Из Порции Цикл
			...
			Пока Не ПорцияОбработана Цикл
				...
				ПроброситьПрогрессВУправляющийПоток(Поток.ИдентификаторЗадания);
...
Функция ОжидатьСвободныйПоток(ИдентификаторПроцесса, Результаты, ИдентификаторЗадания, ЗавершитьДосрочноЕслиОшибка)
	...
	Пока Потоки.Количество() >= МаксимумПотоков Цикл
		...
		ЕстьЗавершеныеПотоки = ЕстьЗавершенныеПотоки(Потоки, Результаты, ЗавершитьДосрочноЕслиОшибка);
		
		Если ЗавершитьДосрочноЕслиОшибка И ЕстьЗавершеныеПотоки = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		
		Если ЕстьЗавершеныеПотоки Тогда
			Продолжить;
		КонецЕсли;
		
		ПроброситьПрогрессВУправляющийПоток(ИдентификаторЗадания);
...
Процедура ПроброситьПрогрессВУправляющийПоток(Знач ИдентификаторЗадания)
	
	ПрочитанныйПрогресс = ПрочитатьПрогрессИСообщения(ИдентификаторЗадания);
	
	Если ПрочитанныйПрогресс.Прогресс <> Неопределено Тогда
		СообщитьПрогресс(ПрочитанныйПрогресс.Прогресс.Процент, 
			ПрочитанныйПрогресс.Прогресс.Текст);
	КонецЕсли;
...
Процедура ОжидатьЗавершениеВсехПотоков(Результаты, ИдентификаторФормы, ЗавершитьДосрочноЕслиОшибка)
	...
	Пока Потоки.Количество() > 0 Цикл
		...
		Если ЕстьЗавершеныеПотоки = Неопределено Тогда
			ОтменитьВыполнениеВсехПотоков(Результаты, ИдентификаторФормы);
			Прервать;
		КонецЕсли;
		
		Если Не ЕстьЗавершеныеПотоки Тогда
			ОжидатьЗавершениеПотока(Потоки[0]);
		КонецЕсли;
...
Функция ДопустимоеКоличествоПотоков()
	...
	ДопустимоеКоличествоПотоков = Константы.КоличествоПотоковДлительныхОпераций.Получить() - 1;
	
	Возврат ?(ДопустимоеКоличествоПотоков < 1, 1, ДопустимоеКоличествоПотоков)
