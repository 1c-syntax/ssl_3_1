...
Функция ЧастиАдресаТаблицей(Знач Текст)
	...
	Для Каждого Часть Из СловаТекстаТаблицей(Текст, "," + Символы.ПС) Цикл
		...
		Для Каждого ТипАдресныхОбъектов Из ТипыАдресныхОбъектов Цикл
			...
			Если СтрНачинаетсяС(ЗначениеВерхнийРегистр, ТипАдресныхОбъектов.СокращениеВНачале) Тогда
				
				Строка.Наименование = СокрЛП(Сред(Значение, ТипАдресныхОбъектов.Длина + 1));
				Строка.Сокращение   = СокрЛП(Лев(Значение, ТипАдресныхОбъектов.Длина));
				Строка.Значение     = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Строка.Наименование, Строка.Сокращение);
				Прервать;
				
			ИначеЕсли СтрЗаканчиваетсяНа(ЗначениеВерхнийРегистр, ТипАдресныхОбъектов.СокращениеВКонце) Тогда
				
				Строка.Наименование = СокрЛП(Сред(Значение, 1, СтрДлина(Значение) - ТипАдресныхОбъектов.Длина));
				Строка.Сокращение   = СокрЛП(Прав(Значение, ТипАдресныхОбъектов.Длина));
				Строка.Значение     = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Строка.Наименование, Строка.Сокращение);
				Прервать;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда			
			...
			КодРегиона = МодульАдресныйКлассификатор.КодРегионаПоНаименованию(
				УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Строка.Наименование, Строка.Сокращение, Истина));
		...
		Строка.Сокращение   = ЧастиАдресногоОбъекта[0];
...
Функция ТипыАдресныхОбъектов()
	...
	СписокСокращений.Колонки.Добавить("Сокращение",        ОбщегоНазначения.ОписаниеТипаСтрока(50));
	СписокСокращений.Колонки.Добавить("Длина",             ОбщегоНазначения.ОписаниеТипаЧисло(3));
	СписокСокращений.Колонки.Добавить("СокращениеВНачале", ОбщегоНазначения.ОписаниеТипаСтрока(20));
	СписокСокращений.Колонки.Добавить("СокращениеВКонце",  ОбщегоНазначения.ОписаниеТипаСтрока(20));
...
Функция СтрокуJSONВСтруктуру(Значение) Экспорт
	...
	Если Результат.Свойство("area") И УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступныМодулиРаботаСАдресами() Тогда
