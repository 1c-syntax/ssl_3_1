...
Функция ПроверитьАдрес(Адрес, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если Метаданные.Обработки.Найти("РасширенныйВводКонтактнойИнформации") <> Неопределено Тогда
		...
		МодульРасширенныйВводКонтактнойИнформации.ПроверитьАдрес(Адрес, РезультатПроверки, ПараметрыПроверкиАдреса);
	КонецЕсли;
...
Функция КодыСтран() Экспорт
	
	Возврат Справочники.СтраныМира.КодыСтран();
	
КонецФункции

Функция ПустаяСсылкаСправочникаСтраныМира() Экспорт
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.УправлениеКонтактнойИнформацией", Методы);
...
Процедура АвтоподборАдреса(Знач Текст, ДанныеВыбора) Экспорт
	...
	ДополнительныеПараметры.ПрямойПорядок = Истина;
	
	МодульРасширенныйВводКонтактнойИнформации = ОбщегоНазначения.ОбщийМодуль("Обработки.РасширенныйВводКонтактнойИнформации");
	Результат = МодульРасширенныйВводКонтактнойИнформации.СписокАвтоподбораНаселенногоПункта(Текст, ДополнительныеПараметры);
...
Функция ПараметрыАвтоподбораАдреса() Экспорт
	...
	ДополнительныеПараметры.Вставить("ПрямойПорядок", Ложь);
...
Функция ЭтоТипАдрес(ЗначениеТипа)
	Возврат Метаданные.Перечисления.ТипыКонтактнойИнформации.ЗначенияПеречисления.Найти(ЗначениеТипа) = 
		Метаданные.Перечисления.ТипыКонтактнойИнформации.ЗначенияПеречисления.Адрес;
КонецФункции
...
Процедура ИсправитьВидыКонтактнойИнформацииВФоне(Знач ПараметрыПроверки, АдресХранилища = Неопределено) Экспорт
	...
	ПараметрыВыборки = МодульКонтрольВеденияУчета.ПараметрыВыборкиПроблемныхОбъектов();
	...
	Пока ПроблемныеОбъекты.Количество() > 0 Цикл
		...
		НачальныйПроблемныйОбъект = ПроблемныеОбъекты.Получить(ПроблемныеОбъекты.Количество() - 1).ПроблемныйОбъект;
		...
		ПараметрыВыборки.НачальныйПроблемныйОбъект = НачальныйПроблемныйОбъект;
		// @skip-check query-in-loop - Порционная обработка больших объемов данных.
		ПроблемныеОбъекты = МодульКонтрольВеденияУчета.ПроблемныеОбъекты(Проверка, ПараметрыВыборки);
...
Процедура ПроверитьВидыКонтактнойИнформации(Проверка, ПараметрыПроверки) Экспорт
	...
	ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки("КонтактнаяИнформация", 
		НСтр("ru='Виды контактной информации'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Для Каждого СтрокаКлассификатора Из МодульРаботаСАдресами.ТаблицаКлассификатора() Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно записать страну мира %1 (код %2) при обновлении классификатора по причине:
						|%3'"),
...
Функция КонтактнаяИнформацияПоПредставлению(Представление, ОжидаемыйВид, РазбиватьНаПоля = Ложь, РаспознатьАдрес = Истина) Экспорт
	...
	Если ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Возврат СформироватьАдресПоПредставлению(Представление, ОжидаемыйВид, РазбиватьНаПоля, РаспознатьАдрес);
...
Функция СформироватьАдресПоПредставлению(Представление, ОжидаемыйВид, РазбиватьНаПоля = Ложь, РаспознатьАдрес = Истина)
	...
	Если Адрес.Country = НаименованиеОсновнаяСтрана 
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Иначе
			...
			Если ЕстьРаботаСАдресамиКлиентСервер Тогда
				МодульРаботаСАдресами = ОбщегоНазначения.ОбщийМодуль("РаботаСАдресами");
				Если МодульРаботаСАдресами <> Неопределено Тогда
					МодульРаботаСАдресами.ОбновитьПредставлениеАдреса(Адрес, Ложь);
				КонецЕсли;
			Иначе
	...
	Иначе
		...
		Если ТипЗнч(ОжидаемыйВид) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
			ВключатьСтрануВПредставление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОжидаемыйВид, "ВключатьСтрануВПредставление");
		КонецЕсли;
		
		НовоеПредставление = Новый Массив;
		НовоеПредставлениеДляУлицы = Новый Массив;
		ДанныеАнализа.Сортировать("Позиция");
		Для каждого ЧастьАдрес Из ДанныеАнализа Цикл
			Если ЧастьАдрес.Уровень >=0 Тогда
				НовоеПредставлениеДляУлицы.Добавить(ЧастьАдрес.Значение);
			КонецЕсли;
				
			Если Не ВключатьСтрануВПредставление И ЧастьАдрес.Уровень = -2 Тогда
				Продолжить
			КонецЕсли;
		...
		Адрес.street = СтрСоединить(НовоеПредставлениеДляУлицы, ", ");
...
Функция ЧастиАдресаТаблицей(Знач Текст) Экспорт
	...
	МодульАдресныйКлассификатор = Неопределено;
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификатор = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификатор");
	КонецЕсли;
	
	НаборЧастейАдреса = СтрРазделить(СтрЗаменить(Текст, "№", ""), "," + Символы.ПС, Ложь);
	
	Для Каждого Часть Из НаборЧастейАдреса Цикл
		Значение = СокрЛП(Часть);
		...
		Для Каждого ТипАдресныхОбъектов Из ТипыАдресныхОбъектов Цикл
			...
			Если СтрНачинаетсяС(ЗначениеВерхнийРегистр, ТипАдресныхОбъектов.ТипОбъектаВНачале) Тогда
				Длина = ТипАдресныхОбъектов.Длина;
				Строка.Наименование = СокрЛП(Сред(Значение, Длина + 1));
				Строка.ТипОбъекта   = СокрЛП(Лев(Значение, Длина));
			...
			ИначеЕсли СтрЗаканчиваетсяНа(ЗначениеВерхнийРегистр, ТипАдресныхОбъектов.ТипОбъектаВКонце) Тогда
				Длина = ТипАдресныхОбъектов.Длина;
				Строка.Наименование = СокрЛП(Сред(Значение, 1, СтрДлина(Значение) - Длина));
				Строка.ТипОбъекта   = СокрЛП(Прав(Значение, Длина));
				Строка.Значение     = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Строка.Наименование, Строка.ТипОбъекта);
				НайденТип           = Истина;
				Прервать;
		...
		Если Не НайденТип Тогда
			ЧастиАдресногоОбъекта = СтрРазделить(Значение, " ", Ложь);
			Если ЧастиАдресногоОбъекта.Количество() > 0 Тогда
		...
		Если КодРегиона = Неопределено
		   И ЗначениеЗаполнено(Строка.Значение)
		   И МодульАдресныйКлассификатор <> Неопределено Тогда
			КодРегиона = МодульАдресныйКлассификатор.КодРегионаПоНаименованию(Строка.Значение);
...
Процедура ОпределитьСтрануИИндекс(АдресПоПолям) Экспорт
	
	Если Не УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступныМодулиРаботаСАдресами() Тогда
		НаборПолей = АдресПоПолям.ВыгрузитьКолонку("Значение");
		ДанныеОСтране = ОпределитьСтрануМира(НаборПолей);
		Если ЗначениеЗаполнено(ДанныеОСтране.Наименование) Тогда
			Для каждого ЭлементАдреса Из АдресПоПолям Цикл
				Если СтрСравнить(ДанныеОСтране.Наименование, ЭлементАдреса.Значение) = 0 Тогда
	...
	Для каждого ЭлементАдреса Из АдресПоПолям Цикл
		...
		Иначе
			СтранаПоКлассификатору = Классификатор.Найти(ВРег(ЭлементАдреса.Значение), "Наименование");
			УправлениеКонтактнойИнформациейЛокализация.ПриПроверкеСтраныПослеПоискаСтраныПоКлассификатору(СтранаПоКлассификатору);
			Если СтранаПоКлассификатору <> Неопределено Тогда
				ЭлементАдреса.Уровень = -2;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ОпределитьСтрануМира(АдресПоПолям) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СтраныМира.Наименование В(&НаборПолей)";
		
	Запрос.УстановитьПараметр("НаборПолей", АдресПоПолям);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		Возврат Результат;
	КонецЕсли;
	СведенияОСтране = РезультатЗапроса.Выбрать();
	
	Если СведенияОСтране.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(Результат, СведенияОСтране);
		Возврат Результат;
	КонецЕсли;
...
Функция ПредставлениеКонтактнойИнформации(Знач КонтактнаяИнформация, ПеревестиВТранслит = Ложь) Экспорт
	...
	Если ТипЗнч(ПеревестиВТранслит) = Тип("Булево") И ПеревестиВТранслит Тогда
		Возврат СтроковыеФункции.СтрокаЛатиницей(КонтактнаяИнформация.Value);
	КонецЕсли;
...
Функция ПредставлениеАдреса(Знач Адрес, Знач ВидИнформации)
	...
	Если ТипЗнч(Адрес) = Тип("Структура") Тогда
		...
		Если УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступныМодулиРаботаСАдресами() Тогда
			МодульРаботаСАдресами = ОбщегоНазначения.ОбщийМодуль("РаботаСАдресами");
			МодульРаботаСАдресами.ОбновитьПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление);
		Иначе
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Наименование) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заполнено обязательное поле Наименование у вида контактной информации ""%1"".'"),
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		Результат.ЕстьОшибки = Истина;
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заполнено обязательное поле Тип у вида контактной информации ""%1"".'"),
	...
	Если ВидКонтактнойИнформации.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Если НЕ ВидКонтактнойИнформации.ТолькоНациональныйАдрес
			...
			ИЛИ ВидКонтактнойИнформации.СкрыватьНеактуальныеАдреса) Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректно заполнены настройки проверки адреса у вида контактной информации %1.
		...
		Если ВидКонтактнойИнформации.РазрешитьВводНесколькихЗначений
			И ВидКонтактнойИнформации.ХранитьИсториюИзменений Тогда
				Результат.ТекстОшибки = Результат.ТекстОшибки + Разделитель 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Некорректно заполнены настройки адреса у вида контактной информации %1.
...
Процедура ДобавитьКонтактнуюИнформациюДляСсылки(Ссылка, ЗначениеИлиПредставление, ВидКонтактнойИнформации, 
...
//   Замещать - Булево
// 
Процедура ДобавитьКонтактнуюИнформацию(Объект, ЗначениеИлиПредставление, ВидКонтактнойИнформации, Знач Дата, Знач Замещать, РаспознатьАдрес = Истина) Экспорт
	...
	СвойстваВидаКонтактнойИнформации      = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидКонтактнойИнформации, 
		"Тип, ХранитьИсториюИзменений");
	...
	Иначе
		...
		Если ЭтоКонтактнаяИнформацияВXML Тогда
			...
			Значение = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(ЗначениеИлиПредставление, 
				СвойстваВидаКонтактнойИнформации.Тип);
			ОбъектКонтактнойИнформации = JSONВКонтактнуюИнформациюПоПолям(Значение, 
				СвойстваВидаКонтактнойИнформации.Тип);
			Представление = ОбъектКонтактнойИнформации.Value;
		...
		Иначе
			ОбъектКонтактнойИнформации = КонтактнаяИнформацияПоПредставлению(ЗначениеИлиПредставление, 
				СвойстваВидаКонтактнойИнформации.Тип, ,РаспознатьАдрес);
	...
	ЗаполнитьТехническиеПоляКонтактнойИнформации(СтрокаКонтактнойИнформации, ОбъектКонтактнойИнформации, 
		СвойстваВидаКонтактнойИнформации.Тип);
...
Процедура УстановитьКонтактнуюИнформациюОбъекта(Объект, КонтактнаяИнформация, ОбъектМетаданных, Знач Замещать) Экспорт
	...
	Если Замещать Тогда
		...
		Для каждого СтрокаКонтактнойИнформацииОбъекта Из КонтактнаяИнформация Цикл
			...
			СтрокиКонтактнойИнформации = НайтиСтрокиКонтактнойИнформации(СтрокаКонтактнойИнформацииОбъекта.Вид, 
				ДатаОтбора, Объект.КонтактнаяИнформация, Периодическая);
			Для каждого СтрокаТаблицы Из СтрокиКонтактнойИнформации Цикл
	...
	Для каждого СтрокаКонтактнойИнформацииОбъекта Из КонтактнаяИнформация Цикл
		...
		Если Замещать Тогда
			...
			ИдентификаторСтрокиТабличнойЧасти = ?(БезИдентификатораТабличнойЧасти, Неопределено, 
				СтрокаКонтактнойИнформацииОбъекта.ИдентификаторСтрокиТабличнойЧасти);
			Если ЗапрещенВводНесколькихЗначений(СтрокаКонтактнойИнформацииОбъекта.Вид, Объект.КонтактнаяИнформация, 
				СтрокаКонтактнойИнформацииОбъекта.Дата, ХранитьИсториюИзменений, ИдентификаторСтрокиТабличнойЧасти) Тогда
				Продолжить;
			КонецЕсли;
		...
		Иначе
			...
			Если ХранитьИсториюИзменений Тогда
				...
				СтрокиКонтактнойИнформации = Объект.КонтактнаяИнформация.НайтиСтроки(Отбор);
			...
			Если НЕ ХранитьИсториюИзменений
				И ЗапрещенВводНесколькихЗначений(СтрокаКонтактнойИнформацииОбъекта.Вид, Объект.КонтактнаяИнформация, 
					СтрокаКонтактнойИнформацииОбъекта.Дата, ХранитьИсториюИзменений)
				ИЛИ СтрокиКонтактнойИнформации.Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
		...
		ЗаполнитьКонтактнуюИнформациюОбъектаИзСтроки(СтрокаКонтактнойИнформацииОбъекта, ХранитьИсториюИзменений, 
...
Процедура УстановитьКонтактнуюИнформациюОбъектаДляСсылки(Ссылка, Знач КонтактнаяИнформация, ОбъектМетаданных, 
...
Процедура УстановитьКонтактнуюИнформациюОбъектовДляСсылки(ВладелецКонтактнойИнформации, 
	СтрокиКонтактнойИнформацииОбъекта, Знач Замещать) Экспорт
	...
	Попытка
		...
		УстановитьКонтактнуюИнформациюОбъектов(ВладелецКонтактнойИнформации, Объект, 
			СтрокиКонтактнойИнформацииОбъекта, Замещать);
...
Процедура УстановитьКонтактнуюИнформациюОбъектов(ВладелецКонтактнойИнформации, Объект, Знач СтрокиКонтактнойИнформацииОбъекта, Знач Замещать) Экспорт
	...
	Для каждого СтрокаКонтактнойИнформацииОбъекта Из СтрокиКонтактнойИнформацииОбъекта Цикл // СтрокаТаблицыЗначений из см. УправлениеКонтактнойИнформацией.НоваяКонтактнаяИнформация
		...
		ХранитьИсториюИзменений = ВладелецКонтактнойИнформации.Значение["Периодическая"] 
			И СтрокаКонтактнойИнформацииОбъекта.Вид.ХранитьИсториюИзменений;
		
		Если Замещать Тогда
			
			Если ЗапрещенВводНесколькихЗначений(СтрокаКонтактнойИнформацииОбъекта.Вид, Объект.КонтактнаяИнформация, 
				СтрокаКонтактнойИнформацииОбъекта.Дата, ХранитьИсториюИзменений) Тогда
				Продолжить;
			КонецЕсли;
			СтрокаКонтактнойИнформации = Объект.КонтактнаяИнформация.Добавить();
...
Функция ЗапрещенВводНесколькихЗначений(ВидКонтактнойИнформации, КонтактнаяИнформация, Дата, Периодическая, ИдентификаторСтрокиТабличнойЧасти = Неопределено)
	...
	СвойстваВидаКонтактнойИнформации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидКонтактнойИнформации, "РазрешитьВводНесколькихЗначений, ХранитьИсториюИзменений");
	
	Если СвойстваВидаКонтактнойИнформации.РазрешитьВводНесколькихЗначений Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если Периодическая И СвойстваВидаКонтактнойИнформации.ХранитьИсториюИзменений Тогда
...
Процедура ЗаполнитьРеквизитыТабличнойЧастиДляАдреса(СтрокаТабличнойЧасти, Адрес)
	...
	Если Адрес.Свойство("Country") Тогда
		...
		Если Метаданные.Обработки.Найти("РасширенныйВводКонтактнойИнформации") <> Неопределено Тогда
			МодульРасширенныйВводКонтактнойИнформации = ОбщегоНазначения.ОбщийМодуль("Обработки.РасширенныйВводКонтактнойИнформации");
			МодульРасширенныйВводКонтактнойИнформации.ЗаполнитьРасширенныеРеквизитыТабличнойЧастиДляАдреса(Адрес, СтрокаТабличнойЧасти);
		КонецЕсли;
...
// Умолчания
	...
	Иначе
		...
		АдресСтрокой = "";
		УправлениеКонтактнойИнформациейЛокализация.ПриЗаполненииРеквизитовТабличнойЧастиДляВебСтраницы(Источник, АдресСтрокой);
...
Функция ЕстьЗаполненныеСвойстваКонтактнойИнформации(Знач Владелец)
	...
	Если ЭтоТипАдрес(Владелец.Type) 
	   И УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступныМодулиРаботаСАдресами() Тогда
		СписокПолейДляПроверки = Новый Массив();
		СписокПолейДляПроверки.Добавить("Country");
	
		МодульРаботаСАдресамиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("РаботаСАдресамиКлиентСервер");
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокПолейДляПроверки, 
			МодульРаботаСАдресамиКлиентСервер.ИменаУровнейАдреса(Владелец, Истина));
...
Функция ДесериализацияТелефонаФаксаВJSON(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(ЗначенияПолей) Тогда
		...
		УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнойИнформацииИзXML(ЗначенияПолей, Результат, ОжидаемыйТип);
	...
	Если ПустаяСтрока(Данные.Number) Тогда
		Если СтрНачинаетсяС(СокрЛ(Представление), "+") И СтрДлина(Представление) > НачалоГорода Тогда
...
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Знач Тип = Неопределено, НастройкиКонвертации = Неопределено) Экспорт
	...
	Если Тип = Неопределено Тогда
		Если ТипЗнч(КонтактнаяИнформация) <> Тип("Строка") 
			Или (ТипЗнч(КонтактнаяИнформация) = Тип("Строка") 
	...
	Если УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступныМодулиРаботаСАдресами() 
		И Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
	...
	КонтактнаяИнформацияВJSON = Неопределено;
	УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнойИнформацииВСтруктуруJSON(КонтактнаяИнформация, 
		КонтактнаяИнформацияВJSON, Тип, НастройкиКонвертации);
	
	Если КонтактнаяИнформацияВJSON <> Неопределено Тогда
...
Функция СтруктураВСтрокуJSON(Значение) Экспорт
	...
	ЗаписатьJSON(ЗаписьJSON, КонтактнаяИнформацияПоПолям,, "АдаптацияПолейКонтактнойИнформации", 
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьЗначениеПризнакаИспользуется");
	Методы.Вставить("ОбновитьНастройкуДобавочногоНомераТелефона");
	Методы.Вставить("ОбновитьСуществующиеСтраныМира");
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтрольВеденияУчета") Тогда
		Методы.Вставить("ПроверитьВидыКонтактнойИнформации");
	КонецЕсли;
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ИсправлениеУстаревшихАдресов");
	КонецЕсли;
	
	Методы.Вставить("ИсправитьВидыКонтактнойИнформацииВФоне", Истина);
