...
Функция СертификатыПоПорядкуДоКорневого(ДанныеСертификатов) Экспорт
	...
	Для Каждого ДанныеСертификата Из ДанныеСертификатов Цикл
		...
		СертификатыПоСубъектам.Вставить(Сертификат.Субъект.CN, ДанныеСертификата);
	КонецЦикла;
	
	ЭлектроннаяПодписьСлужебныйКлиентСервер.УпорядочитьСертификаты(
		ПоПорядку, ОписаниеСертификатов, СертификатыПоСубъектам);
...
Процедура ПриОпределенииОбработчиковПоставляемыхДанных(Обработчики) Экспорт
	...
	Обработчик.ВидДанных = "ОшибкиКриптографии2";
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Загружать = Дескриптор.DataType = "ОшибкиКриптографии2";
...
Процедура ОбработатьНовыеДанные(Знач Дескриптор, Знач ПутьКФайлу) Экспорт
	...
	Если Дескриптор.DataType = "ОшибкиКриптографии2" Тогда
...
Функция ПроверитьСертификатНаКорректностьПовторно(МенеджерКриптографииДляПроверки, СертификатДляПроверки, 
	...
	Если ТипЗнч(МенеджерКриптографииДляПроверки) = Тип("МенеджерКриптографии") Тогда
		...
		Если ЗначениеЗаполнено(Действия) И Действия.Найти("УстановкаСпискаОтзываСертификатов") <> Неопределено Тогда
...
// Ошибка по классификатору.
// 
// Параметры:
//  ТекстДляПоискаВКлассификаторе - Строка
//  ОшибкаНаСервере - Булево
// 
// Возвращаемое значение:
//  Неопределено, Структура - Ошибка по классификатору:
//   * Ссылка - Строка
//   * Причина - Строка
//   * Решение - Строка
//   * СпособУстранения - Строка
//   * ДействияДляУстранения - Массив, Неопределено
//
...
Функция КлассификаторОшибокКриптографии() Экспорт
	...
	Если ТипЗнч(ДанныеКлассификатора) = Тип("Структура") Тогда
		Попытка
			Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ДанныеКлассификатора.Версия, "2.0.0.8") < 0 Тогда
				Версия = Неопределено;
			Иначе
...
Процедура УстановитьСписокОтзываСертификата(Сертификат, Адреса, АдресВнутренний)
	...
	Если Не ЗначениеЗаполнено(Адреса) Тогда
		ВызватьИсключение НСтр("ru = 'Не указаны адреса списков отзыва.'");
	Иначе
