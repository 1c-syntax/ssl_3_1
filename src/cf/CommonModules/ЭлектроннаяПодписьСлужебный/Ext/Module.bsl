		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если ЭтоМодельСервисаСДоступнымУсовершенствованием Тогда
				...
				Элементы.ТипПодписиКриптографииПоУмолчаниюРасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
						НСтр(
					"ru = 'В приложении в Интернете архивная подпись по умолчанию недоступна, этот тип подписи можно выбрать при подписании <a href=%1>сертификатом</a>, установленным на компьютер с помощью <a href=Программы>приложения электронной подписи</a>.'"), "Сертификаты");
			Иначе
				...
				Элементы.ТипПодписиКриптографииПоУмолчанию1РасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
						НСтр(
					"ru = 'В приложении в Интернете по умолчанию установлен тип подписи Базовая, типы подписи с метками времени можно выбрать при подписании <a href=%1>сертификатом</a>, установленным на компьютер с помощью <a href=Программы>приложения электронной подписи</a>.'"), "Сертификаты");
			КонецЕсли;
...
Функция АдресаСерверовМетокВремениПоУмолчанию() Экспорт
	...
	Если Метаданные.Обработки.Найти("ПрограммыЭлектроннойПодписиИШифрования") <> Неопределено Тогда
		Возврат Обработки["ПрограммыЭлектроннойПодписиИШифрования"].АдресаСерверовМетокВремениПоУмолчанию();
	КонецЕсли;
...
Функция ЗаголовокИнформацииДляПоддержки() Экспорт
	...
	Если ВидимостьСсылкиНаИнструкциюПоТипичнымПроблемамПриРаботеСПрограммами() Тогда
		Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'При возникновении затруднений ознакомьтесь со списком <a href = %1>типичных проблем при работе с приложениями электронной подписи и их решений</a>.
				|
				|В иных случаях <a href = %2>обратитесь в службу поддержки фирмы ""1С""</a>'"),
				"ТипичныеПроблемы", "ТехническаяИнформация");
	Иначе
		Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<a href = %1>Обратитесь в службу поддержки фирмы ""1С""</a>'"),
...
Функция ОтпечаткиСертификатов(ТолькоЛичные, ОписаниеОшибки) Экспорт
	...
	Если ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере() Тогда
		...
		Если МенеджерКриптографии <> Неопределено Тогда
			...
			МассивОтпечатков = ПолучитьОтпечаткиСертификатовВХранилище(
			...
			Если Не ТолькоЛичные Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивОтпечатков,
					ПолучитьОтпечаткиСертификатовВХранилище(
...
Функция ПолучитьОтпечаткиСертификатовВХранилище(МенеджерКриптографии, ТипХранилища, Ошибка)
...
Функция ОшибкаПоКлассификатору(ТекстДляПоискаВКлассификаторе, ОшибкаНаСервере = Ложь, ОшибкаПроверкиПодписи = Ложь) Экспорт
	...
	ОшибкаПоКлассификатору = ЭлектроннаяПодписьСлужебныйПовтИсп.ОшибкаПоКлассификатору(
		ТекстДляПоискаВКлассификаторе, ОшибкаНаСервере, ОшибкаПроверкиПодписи);
...
Функция КлассификаторОшибокКриптографии() Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти("ЭлектроннаяПодписьСлужебныйЛокализация") <> Неопределено Тогда
		...
		Версия = МодульЭлектроннаяПодписьСлужебныйЛокализация.ВерсияКлассификатора();
...
Функция ДополнитьРешениеКлассификатораОшибокПодробностями(ОшибкаПоКлассификатору,
	...
	Результат = Новый Структура("ОшибкаПоКлассификатору, ПараметрыДополненияТекстаРешенияОшибкиКлассификатораНаКлиенте",
		ОшибкаПоКлассификатору, ПараметрыДополненияТекстаРешенияОшибкиКлассификатораНаКлиенте);
...
#Если Не МобильноеПриложениеСервер И Не МобильныйКлиент Тогда
	Если ОшибкаПоКлассификатору.ДействияДляУстранения.Найти(
					"ПроверитьСертификатВЛокальномХранилище") <> Неопределено Тогда

		СистемнаяИнформация = Новый СистемнаяИнформация;
		ВерсияПриложения = СистемнаяИнформация.ВерсияПриложения;
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПриложения, "8.3.24.0") < 0 Тогда
			Если МестоОшибки = "Сервер" Тогда
				Если ЭлектроннаяПодпись.ОбщиеНастройки().ПроверятьЭлектронныеПодписиНаСервере
					...
					СертификатВЛокальномХранилище = СертификатВЛокальномХранилище(
						ДополнительныеДанные.ДанныеСертификата);
					Если СертификатВЛокальномХранилище = Истина Тогда
						Решение = Новый Массив;
						Решение.Добавить(СтроковыеФункции.ФорматированнаяСтрока(
							СертификатВЛокальномХранилищеТекстРешения(МестоОшибки)));
							
						Причина = Новый Массив;
						Причина.Добавить(СтроковыеФункции.ФорматированнаяСтрока(
							СертификатВЛокальномХранилищеТекстПричины(МестоОшибки)));
					КонецЕсли;
				КонецЕсли;
			Иначе
				Результат.ПараметрыДополненияТекстаРешенияОшибкиКлассификатораНаКлиенте.ПроверитьСертификатВЛокальномХранилищеКлиента = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
#КонецЕсли
	...
	Если Решение.Количество() > 0 Тогда
		Результат.ОшибкаПоКлассификатору.Решение = Новый ФорматированнаяСтрока(Решение);
	КонецЕсли;
	
	Если Причина.Количество() > 0 Тогда
		Результат.ОшибкаПоКлассификатору.Причина = Новый ФорматированнаяСтрока(Причина);
	КонецЕсли;
...
Функция СертификатВЛокальномХранилищеТекстРешения(Место)
	
	Если Место = "Сервер" Тогда

		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Удалите сертификат из локального хранилища сервера %1.'"), ИмяКомпьютера());

	КонецЕсли;

	Возврат ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатВЛокальномХранилищеТекстРешения();
...
Функция СертификатВЛокальномХранилищеТекстПричины(Место) Экспорт
	...
	Возврат ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатВЛокальномХранилищеТекстПричины();
...
Функция СертификатВЛокальномХранилище(ДанныеСертификата)
...
#Если Не МобильноеПриложениеСервер И Не МобильныйКлиент Тогда
	...
	МенеджерКриптографии = МенеджерКриптографии("ПолучениеСертификатов");
	Попытка
		ХранилищеЛокальногоКомпьютера = МенеджерКриптографии.ПолучитьХранилищеСертификатов(
			ТипХранилищаСертификатовКриптографии.ПерсональныеСертификаты,
			РасположениеХранилищаСертификатовКриптографии.ДанныеКомпьютера);
		СертификатКриптографии = Новый СертификатКриптографии(ДанныеСертификата);
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронная подпись.Диагностика ошибок'",
			ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
	КонецПопытки;
	
	Если ХранилищеЛокальногоКомпьютера.НайтиПоОтпечатку(СертификатКриптографии.Отпечаток) <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
