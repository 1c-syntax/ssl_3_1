...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		НастройкиПодсистемы.Вставить("ОписаниеКомпоненты",    ЭлектроннаяПодписьСлужебныйПовтИсп.ОписаниеКомпоненты());
...
Функция ОбъектВнешнейКомпонентыXMLDSig()
	...
	ОписаниеКомпоненты = ЭлектроннаяПодписьСлужебныйПовтИсп.ОписаниеКомпоненты();
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось подключить внешнюю компоненту XMLDSig.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура НастроитьКомпоненту(ОбъектКомпоненты)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'При установке свойства СоответствиеOID компоненты XMLDSig произошла ошибка:'")
...
Функция СвойстваКриптопровайдера(МенеджерКриптографии)
	...
	Свойства.Путь = ПутьКПрограмме(ОписаниеПрограммы.Ссылка).ПутьКПрограмме;
...
Функция C14N(ОбъектКомпоненты, КонвертXML, XPath)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода C14N компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция C14N_body(ОбъектКомпоненты, ТекстXML, Алгоритм)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода C14N_body компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция TransformSMEV(ОбъектКомпоненты, ТекстXML)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода TransformSMEV компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РезультатHash(ОбъектКомпоненты, КанонизированныйТекстXMLBody, OIDАлгоритмаХеширования, ТипКриптопровайдера)
	...
	Попытка
		АтрибутDigestValue = ОбъектКомпоненты.Hash(
			...
			ТипКриптопровайдера);
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода Hash компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РезультатSign(ОбъектКомпоненты, КанонизированныйТекстXMLSignedInfo,
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода Sign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РезультатVerifySign(ОбъектКомпоненты, КанонизированныйТекстXMLSignedInfo,
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.VerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода VerifySign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РезультатCMSSign(ОбъектКомпоненты, ДанныеДляПодписания, ПараметрыCMS, СертификатКриптографии, ПарольДоступаКЗакрытомуКлючу)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода CMSSign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РезультатCMSVerifySign(ОбъектКомпоненты, Подпись, Данные, ПараметрыCMS, ТипКриптопровайдера)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка вызова метода CMSVerifySign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
