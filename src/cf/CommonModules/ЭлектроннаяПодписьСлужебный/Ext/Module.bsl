...
Процедура УстановитьПараметрРегламентногоЗаданияПродлениеДостоверностиПодписей(Знач ИмяПараметра, Знач ЗначениеПараметра) Экспорт
...
Процедура ПередДобавлениемКомандОтчетов(КомандыОтчетов, Параметры, СтандартнаяОбработка) Экспорт
	...
	Отчеты.ПродлениеСрокаДействияЭлектронныхПодписей.ПередДобавлениемКомандОтчетов(КомандыОтчетов, Параметры, СтандартнаяОбработка);
...
//  Операция     - Строка - "Усовершенствование", "Подписание", Неопределено - для каких операций формируется список.
//               - Неопределено - (по умолчанию) список значений для формы настроек.
...
Процедура ЗаполнитьСписокТиповПодписейКриптографии(СписокВыбора, Операция = Неопределено, ТипПодписи = Неопределено) Экспорт
	...
	Если (ТипПодписи = Неопределено Или ТипПодписи = Перечисления.ТипыПодписиКриптографии.CAdESBES) И Операция <> "Усовершенствование" Тогда
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.CAdESBES, НСтр("ru='Базовая подпись (CAdES-BES)
			|становится недействительной после окончания срока действия сертификата подписанта (обычно в течение 1 года).'"));
	КонецЕсли;
	Если ТипПодписи = Неопределено
		Или ТипПодписи = Перечисления.ТипыПодписиКриптографии.CAdEST И Операция <> "Усовершенствование"
		Или ТипПодписи = Перечисления.ТипыПодписиКриптографии.CAdESBES Тогда
			СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.CAdEST, НСтр("ru='Подпись с меткой доверенного времени (CAdES-T)
				|действует после окончания срока действия сертификата подписанта.'"));
	КонецЕсли;
	Если Операция = Неопределено Или Не ЭлектроннаяПодпись.ОбщиеНастройки().УсовершенствованиеЧерезСервис Тогда
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.CAdESAv3, НСтр("ru='Архивная подпись (CAdES-A)
...
//
// Параметры:
//  ТипПодписи - Перечисление.ТипыПодписиКриптографии - тип подписи, для которого нужно определить доступность настроек.
//
Функция ДоступностьНастроекУсовершенствованияПодписи(ТипПодписи) Экспорт

	Возврат ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CMS")
			И ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdESBES");
КонецФункции
...
Процедура УсовершенствоватьРегламентнымЗаданием(Выборка, ТипПодписи, ДобавитьАрхивнуюМеткуВремени, ОперацияНачалась) Экспорт
	
	ОшибкиСозданияМенеджераКриптографии = Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если МенеджерКриптографии = Неопределено Тогда
			Если ОшибкиСозданияМенеджераКриптографии.Найти(ПараметрыСоздания.ОписаниеОшибки) Тогда
				ОшибкиСозданияМенеджераКриптографии.Вставить(ПараметрыСоздания.ОписаниеОшибки);
			КонецЕсли;
			СвойстваПодписи = Новый Структура("ПодписанныйОбъект, ПорядковыйНомер, ОшибкаПриАвтоматическомПродлении",
				Выборка.ПодписанныйОбъект, Выборка.ПорядковыйНомер, Истина);
		...
		Если Не Результат.Успех Тогда
			ЗарегистрироватьОшибкуУсовершенствованияПодписиВЖурнале(Результат.ТекстОшибки);
			СвойстваПодписи = Новый Структура("ПодписанныйОбъект, ПорядковыйНомер, ОшибкаПриАвтоматическомПродлении",
				Выборка.ПодписанныйОбъект, Выборка.ПорядковыйНомер, Истина);
			ОбновитьУсовершенствованнуюПодпись(СвойстваПодписи);
			Продолжить;
		КонецЕсли;
		
		Результат.СвойстваПодписи.ПодписанныйОбъект = Выборка.ПодписанныйОбъект;
		Результат.СвойстваПодписи.ПорядковыйНомер = Выборка.ПорядковыйНомер;
		ОбновитьУсовершенствованнуюПодпись(Результат.СвойстваПодписи);
		
	КонецЦикла;
		
	Если ОшибкиСозданияМенеджераКриптографии.Количество() > 0 Тогда
		ИмяСобытия = НСтр("ru = 'Электронная подпись.Ошибка усовершенствования подписи'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		СообщениеЖурналаРегистрации = СтрСоединить(ОшибкиСозданияМенеджераКриптографии, Символы.ПС);
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, Неопределено,
			Неопределено, СообщениеЖурналаРегистрации);
	КонецЕсли;
	
КонецПроцедуры 

// Только для внутреннего использования.
Процедура ЗарегистрироватьУсовершенствованиеПодписиВЖурнале(ПараметрыПодписиКриптографии, ОписаниеОшибки = "") Экспорт
	
	ЭтоОшибкаУсовершенствования = ЗначениеЗаполнено(ОписаниеОшибки);
	СвойстваСертификата = ПараметрыПодписиКриптографии.ОписаниеСертификата;
	
	ДатаПодписи = ?(ЗначениеЗаполнено(ПараметрыПодписиКриптографии.ДатаПодписиИзМетки),
		ПараметрыПодписиКриптографии.ДатаПодписиИзМетки, ПараметрыПодписиКриптографии.НеподтвержденнаяДатаПодписи);
		
	СведенияОПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Дата подписи: %1
			|Сертификат: %2
			|Кем выдан: %3
			|Владелец: %4
	...
	Если ЭтоОшибкаУсовершенствования Тогда   
		СообщениеЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1
			|Ошибка: %2'"), СведенияОПодписи, ОписаниеОшибки);
		ЗарегистрироватьОшибкуУсовершенствованияПодписиВЖурнале(СообщениеЖурналаРегистрации);  
		Возврат;
	КонецЕсли;

	ИмяСобытия = НСтр("ru = 'Электронная подпись.УсовершенствованиеПодписи'",
		ОбщегоНазначения.КодОсновногоЯзыка());
	СообщениеЖурналаРегистрации = СведенияОПодписи;
	ЗаписьЖурналаРегистрации(ИмяСобытия,
		УровеньЖурналаРегистрации.Информация,
		Неопределено,
		Неопределено,
...
// Только для внутреннего использования.
Процедура ЗарегистрироватьОшибкуУсовершенствованияПодписиВЖурнале(ОписаниеОшибки)
...
Процедура ЗаполнитьДополнительныеСвойстваСертификата(Форма)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Форма.Сертификат,
		...
		|ДействителенДо, ДанныеСертификата");
	...
	Если ЕстьПараметрОповеститьОбОкончанииСрокаДействия И Форма.Параметры.ОповеститьОбОкончанииСрокаДействия 
		...
		Форма.ОповеститьОбОкончанииСрокаДействия = Форма.СертификатДействителенДо <= ТекущаяУниверсальнаяДата() + 30*24*60*60
			И РегистрыСведений.ОповещенияПользователейСертификатов.ПрочитатьОтметкуОНапоминании(Форма.Сертификат) <> Истина
...
Процедура ЗаполнитьНастройкиДляУсовершенствованияПодписей() Экспорт
	...
	Константы.ТипПодписиКриптографииПоУмолчанию.Установить(Перечисления.ТипыПодписиКриптографии.CAdESBES);
	Константы.АдресаСерверовМетокВремени.Установить("https://1stdss.1c.ru/TSP/tsp.srf");
...
Функция ЗапросДляПродленияДостоверностиПодписей(Параметры) Экспорт
	...
	Если ПараметрыЗапроса.ТребуетсяУсовершенствоватьПодписи Тогда
		...
		|	И ЭлектронныеПодписи.СрокДействияПоследнейМеткиВремени > &ТекущаяДатаСеанса
		...
		Если УсовершенствоватьДоТипа = Перечисления.ТипыПодписиКриптографии.CAdEST Тогда
			ТипПодписи.Добавить(Перечисления.ТипыПодписиКриптографии.CAdESBES);
		ИначеЕсли УсовершенствоватьДоТипа = Перечисления.ТипыПодписиКриптографии.CAdESAv3 Тогда
			ТипПодписи.Добавить(Перечисления.ТипыПодписиКриптографии.CAdESBES);
			ТипПодписи.Добавить(Перечисления.ТипыПодписиКриптографии.CAdEST);
			ТипПодписи.Добавить(Перечисления.ТипыПодписиКриптографии.CAdESC);
			...
			ТипПодписи.Добавить(Перечисления.ТипыПодписиКриптографии.CAdESXLongType2);
		Иначе
			ТекстЗапроса = "";
		КонецЕсли;
	...
	Если ПараметрыЗапроса.ТребуетсяДобавитьАрхивныеМетки Тогда
		...
		|	РегистрСведений.ЭлектронныеПодписи КАК ЭлектронныеПодписи
		|ГДЕ
		|	ЭлектронныеПодписи.ТипПодписи = ЗНАЧЕНИЕ(Перечисление.ТипыПодписиКриптографии.CAdESAv3)
...
Функция ОшибкаПоКлассификатору(ТекстДляПоискаВКлассификаторе) Экспорт
	...
	Если ТипЗнч(КлассификаторОшибок) <> Тип("Структура")
		Или КлассификаторОшибок.Классификатор.Количество() = 0 Тогда
		
		Возврат Неопределено;
	КонецЕсли;
	
	ВерсияКлассификатора = КлассификаторОшибок.Версия;
	...
	Запрос.УстановитьПараметр("КлассификаторОшибок", КлассификаторОшибок.Классификатор);
	...
	Если ВерсияКлассификатора = Неопределено Тогда
		ПредставлениеОшибки.Причина = ФорматированнаяСтрока(ПредставлениеОшибки.Причина);
		ПредставлениеОшибки.Решение = ФорматированнаяСтрока(ПредставлениеОшибки.Решение);
	Иначе
...
Функция ФорматированнаяСтрока(Строка)
	
	Соответствие = Новый Соответствие;
	Соответствие.Вставить("http:", 5);
	Соответствие.Вставить("https:", 6);
	
	ДлинаСтроки = СтрДлина(Строка);
	ЭтоСсылка = Ложь;
	СтрокаПреобразованная = Новый Массив;
	СтрокаБезСсылок = Новый Массив;
	
	Для Позиция = 1 По ДлинаСтроки Цикл
		
		Если Не ЭтоСсылка Тогда
			Для Каждого КлючИЗначение Из Соответствие Цикл
				Если Позиция + КлючИЗначение.Значение >= ДлинаСтроки Тогда
					Продолжить;
				КонецЕсли;
				Часть = Сред(Строка, Позиция, КлючИЗначение.Значение);
				Если НРег(Часть) = КлючИЗначение.Ключ Тогда
					Если СтрокаБезСсылок.Количество() > 0 Тогда
						СтрокаПреобразованная.Добавить(СтрСоединить(СтрокаБезСсылок));
						СтрокаБезСсылок.Очистить();
					КонецЕсли; 
					Ссылка = Часть;
					ЭтоСсылка = Истина;
					Позиция = Позиция + КлючИЗначение.Значение;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Если ЭтоСсылка Тогда
			...
			Если ТекущийСимвол = "." Тогда
				Если Позиция <> ДлинаСтроки Тогда
					...
					ЭтоСсылка = ЭтоСимволСсылки(СледующийСимвол);
				Иначе
					ЭтоСсылка = Ложь;
				КонецЕсли;
			Иначе
				ЭтоСсылка = ЭтоСимволСсылки(ТекущийСимвол);
			КонецЕсли;
				
			Если Не ЭтоСсылка Тогда
				СтрокаПреобразованная.Добавить(Новый ФорматированнаяСтрока(Ссылка,,,,Ссылка));
				СтрокаБезСсылок.Очистить();
				СтрокаБезСсылок.Добавить(ТекущийСимвол);
				Продолжить;
			КонецЕсли;
				
			Ссылка = Ссылка + ТекущийСимвол;
		Иначе
			СтрокаБезСсылок.Добавить(ТекущийСимвол);
		КонецЕсли;
	...
	Возврат Новый ФорматированнаяСтрока(СтрокаПреобразованная); // АПК:1356 - можно использовать составную форматированную строку,
																// так как массив строк формируется из переданного в функцию текста.
КонецФункции

Функция ЭтоСимволСсылки(Символ)
...
Функция КлассификаторОшибокКриптографии() Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеКлассификатора) Тогда
		Возврат Новый Структура("Классификатор, Версия", ПредставлениеКлассификатораДляХранения());
	КонецЕсли;
	
	ДанныеКлассификатора = ДанныеКлассификатора.Получить();
	
	Если ТипЗнч(ДанныеКлассификатора) = Тип("ТаблицаЗначений") Тогда
		Возврат Новый Структура("Классификатор, Версия", ДанныеКлассификатора);
	КонецЕсли;
...
Функция ОбновитьКлассификатор()
	...
	ЗаписатьДанныеКлассификатора(ДанныеКлассификатора, ДатаПоследнегоИзменения);
...
Процедура ЗаписатьДанныеКлассификатора(ДанныеКлассификатора, ДатаПоследнегоИзменения = Неопределено)
	...
	Если ДанныеКлассификатора <> Неопределено Тогда
		...
		Константы.КлассификаторОшибокКриптографии.Установить(
			Новый ХранилищеЗначения(Новый Структура("Классификатор, Версия",
				ПредставлениеКлассификатора, ВерсияКлассификатора), Новый СжатиеДанных(9)));
...
Функция РасширенныеДанныеДляПечатиСертификата(Данные) Экспорт
	...
	Иначе 
		ВызватьИсключение НСтр("ru = 'Ошибка при чтении расширенных данных сертификата.'");
	КонецЕсли;
