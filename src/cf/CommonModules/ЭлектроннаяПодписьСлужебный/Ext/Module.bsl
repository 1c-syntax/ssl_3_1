...
Процедура ЗаполнитьСписокТиповПодписейКриптографии(СписокВыбора, Операция = Неопределено, ТипПодписи = Неопределено) Экспорт
	...
	Если (ТипПодписи = Неопределено Или ТипПодписи = Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES) И Операция <> "Усовершенствование" Тогда
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Базовая подпись (%1)
	...
	Если РазделениеВключено Тогда
		ЭтоМодельСервисаСДоступнымУсовершенствованием = 
			ЭлектроннаяПодпись.ОбщиеНастройки().ЭтоМодельСервисаСДоступнымУсовершенствованием;
		ДобавитьТипПодписи = ЭтоМодельСервисаСДоступнымУсовершенствованием Или Не Операция = "Настройки";
	Иначе
		Если ТипПодписи = Неопределено 
			Или ТипПодписи = Перечисления.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST
				И Операция <> "Усовершенствование"
			Или ТипПодписи = Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES Тогда
	...
	Если ДобавитьТипПодписи Тогда
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Подпись с меткой доверенного времени (%1)
						|действует после окончания срока действия сертификата подписанта.'"),
					"CAdES-T"));
	КонецЕсли;
	
	Если Операция = Неопределено
		Или Не (РазделениеВключено И Операция = "Настройки")
		Тогда
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.АрхивнаяCAdESAv3,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Архивная подпись (%1)
...
Функция ДоступностьНастроекУсовершенствованияПодписи(ТипПодписи) Экспорт
	...
	Возврат ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS")
...
Процедура ВыполнитьОбработкуДанныхРегламентнымЗаданием(ПараметрыЗапроса, ПараметрыВыполнения) Экспорт
	...
	Пока Истина Цикл
		...
		УсовершенствоватьРегламентнымЗаданием(
			Выборка, ПараметрыВыполнения);
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не Результат.Успех Тогда
			Если СвойстваПодписи = Неопределено Тогда
				СвойстваПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи();
			КонецЕсли;
...
// Только для внутреннего использования.
Функция УсовершенствоватьПодписьВСервисе(Подпись, ПараметрыВыполнения) Экспорт
	
	Результат = Новый Структура("Успех, ТекстОшибки, СвойстваПодписи", Ложь,, ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи());
	...
	Если ПараметрыВыполнения.ТипПодписи <> Перечисления.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно усовершенствовать подпись в сервисе DSS до типа: %1.'"), ПараметрыВыполнения.ТипПодписи);
		Возврат Результат;
	КонецЕсли;
		
	Попытка
		МодульСервисКриптографииDSSASNКлиентСервер = ОбщегоНазначения.ОбщийМодуль("СервисКриптографииDSSASNКлиентСервер");
		СвойстваПодписи = МодульСервисКриптографииDSSASNКлиентСервер.ПолучитьСвойстваПодписи(Подпись); 
		Сертификат = СвойстваПодписи[0].Сертификат;
		СертификатКриптографии = Новый СертификатКриптографии(Сертификат);
		СвойстваСертификата = ЭлектроннаяПодпись.СвойстваСертификата(СертификатКриптографии);
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при чтении данных подписи для усовершенствования в сервисе DSS:
	...
	Если РезультатПроверкиСертификата.Выполнено = Ложь Или РезультатПроверкиСертификата.Результат <> Истина Тогда
		...
		Результат.СвойстваПодписи.СрокДействияПоследнейМеткиВремени = СвойстваСертификата.ДатаОкончания;
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при проверке сертификата, подробности см. в журнале регистрации:
				|%1'"), СведенияОСертификате);
	
		Возврат Результат;
	КонецЕсли;
	
	Если Не СвойстваПодписи[0].Свойство("ТипПодписи") Тогда
		Результат.ТекстОшибки = НСтр("ru='Не удалось определить тип подписи'");
		Возврат Результат;
	КонецЕсли;
	
	ПараметрыПодписи = ПараметрыВыполнения.ПараметрыПодписиCAdEST;
	РезультатЗапроса = МодульСервисКриптографииDSS.УсовершенствоватьПодпись(ПараметрыВыполнения.СлужебнаяУчетнаяЗаписьDSS, Подпись, ПараметрыПодписи);
	Если РезультатЗапроса.Выполнено Тогда
		АлгоритмПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмСформированнойПодписи(РезультатЗапроса.Результат, Ложь, Истина);
		Если Не ЗначениеЗаполнено(АлгоритмПодписи) Тогда
			Результат.ТекстОшибки = НСтр("ru='Сервис DSS вернул некорректный ответ:'") + Символы.ПС + РезультатЗапроса.Результат;
		Иначе
		...
		КонецЕсли;
	Иначе
		Результат.ТекстОшибки = НСтр("ru='Не удалось усовершенствовать подпись в сервисе DSS. Описание ошибки см. в журнале регистрации.'");
	КонецЕсли;
...
Функция НастройкиСлужебнойУчетнойЗаписиДляУсовершенствованияПодписей(ИдентификаторФормы = Неопределено) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			НастройкиСлужебнойУчетнойЗаписи.СлужебнаяУчетнаяЗаписьDSS = ПоместитьВоВременноеХранилище(Результат.НастройкиПользователя, ИдентификаторФормы);
		КонецЕсли;
...
Функция УсовершенствоватьНаСторонеСервера(Параметры) Экспорт
	...
	Результат.Вставить("ТекстОшибки", Ложь);
	Результат.Вставить("СвойстваПодписи", ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи());
	...
	Попытка
		Подпись = ПолучитьИзВременногоХранилища(Параметры.ЭлементДанныхДляСервера.Подпись);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	Если Параметры.ЭтоМодельСервисаСДоступнымУсовершенствованием Тогда
		...
		ПараметрыВыполнения.Вставить("СлужебнаяУчетнаяЗаписьDSS", ПолучитьИзВременногоХранилища(Параметры.СлужебнаяУчетнаяЗаписьDSS));
		...
		Если Результат.Успех Тогда
			Если Параметры.ЭлементДанныхДляСервера.Свойство("ПодписанныйОбъект") Тогда
				...
				ПредставлениеОшибки = ОбновитьУсовершенствованнуюПодпись(
		...
		ПоместитьВоВременноеХранилище(ПараметрыВыполнения.СлужебнаяУчетнаяЗаписьDSS, Параметры.ИдентификаторФормы);
	...
	Иначе
		...
		Иначе
			РезультатЗапроса = ЭлектроннаяПодпись.УсовершенствоватьПодпись(Подпись,
...
Процедура ЗаполнитьНастройкиДляУсовершенствованияПодписей() Экспорт
	...
	МенеджерЗначения.ОбменДанными.Загрузка = Истина;
	МенеджерЗначения.Значение = "https://1stdss.1c.ru/TSP/tsp.srf";
	МенеджерЗначения.Записать();
