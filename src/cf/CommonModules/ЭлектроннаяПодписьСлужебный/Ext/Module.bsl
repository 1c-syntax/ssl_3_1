...
Функция ПараметрыШтампов(ЭлектронныеПодписи, ТекстыОтметок = Неопределено) Экспорт
	...
	Для Каждого Подпись Из ЭлектронныеПодписи Цикл
		...
		Если МодульМашиночитаемыеДоверенностиФНССлужебный <> Неопределено
			И Подпись.Свойство("РезультатПроверкиПодписиПоМЧД")
			И ЗначениеЗаполнено(Подпись.РезультатПроверкиПодписиПоМЧД) Тогда
			
			Если ЭтоАдресВременногоХранилища(Подпись.РезультатПроверкиПодписиПоМЧД) Тогда
				РезультатПроверкиПодписиПоМЧД = ПолучитьИзВременногоХранилища(Подпись.РезультатПроверкиПодписиПоМЧД);
			Иначе
				РезультатПроверкиПодписиПоМЧД = Подпись.РезультатПроверкиПодписиПоМЧД;
			КонецЕсли;
			
			Если ТипЗнч(РезультатПроверкиПодписиПоМЧД) = Тип("Структура") Тогда
				РезультатПроверкиПодписиПоМЧД = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РезультатПроверкиПодписиПоМЧД);
			КонецЕсли;
			
			Если РезультатПроверкиПодписиПоМЧД.Количество() > 0 Тогда
				ДанныеДоверенностиДляПечати = МодульМашиночитаемыеДоверенностиФНССлужебный.ДанныеДоверенностиДляПечати(
					РезультатПроверкиПодписиПоМЧД[0].МашиночитаемаяДоверенность, ПараметрыШтампа.ДатаПодписи);
	...
	Если МодульОрганизацииСервер <> Неопределено И Сертификаты.Количество() > 0 Тогда
		СертификатыСсылки = ЭлектроннаяПодпись.СсылкиНаСертификаты(Сертификаты, Истина);
		ОрганизацииСсылки = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СертификатыСсылки, "Организация");
		Индекс = 0; ИндексПодписи = -1;
		Для каждого ПараметрыШтампа Из ПараметрыШтампов Цикл
			...
			ДополнительныеСведения = МодульОрганизацииСервер.ДополнительныеСведенияОрганизации(Организация,
				"ЭмблемаОрганизацииДляШтампаЭлектроннойПодписи", ЭлектронныеПодписи[ИндексПодписи].ДатаПодписи);
...
Функция ПроверитьСертификат(МенеджерКриптографии, Сертификат, ОписаниеОшибки = Null, НаДату = Неопределено, ДополнительныеПараметрыПроверки = Неопределено) Экспорт
	...
	Если МенеджерКриптографииДляПроверки = Неопределено Тогда
		...
		Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
			ПараметрыСоздания.АлгоритмПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПредставлениеАлгоритмаПодписи(
				СертификатДляПроверки.АлгоритмОткрытогоКлюча, Истина, Ложь);
		Иначе
	...
	Если ДополнительныеПараметрыПроверки <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ДополнительныеПараметры, ДополнительныеПараметрыПроверки);
		
		Если ТипЗнч(ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра) = Тип("Булево") Тогда // Для совместимости
			ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра = ?(
				ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра,
		...
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ЭлектроннаяПодписьСлужебный.ПроверитьСертификат",
			"ДополнительныеПараметрыПроверки.ВыполнятьПроверкуУдостоверяющегоЦентра", ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра,
			Тип("Строка"),, ОжидаемыеЗначения);
	КонецЕсли;
	
	РежимыПроверкиСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.РежимыПроверкиСертификата(
		ЗначениеЗаполнено(НаДату), ДополнительныеПараметры.ИгнорироватьПроверкуВСпискеОтозванныхСертификатов);
	...
	Иначе
		...
		Если ДополнительныеПараметры.УказанаДатаМеткиВремени И ЗначениеЗаполнено(НаДату) Тогда
			УниверсальнаяДатаПроверки = НаДату - РазницаСУниверсальнымВременем();
		Иначе
			УниверсальнаяДатаПроверки = Неопределено;
		КонецЕсли;
		
		РезультатПроверкиНаКорректность = ПроверитьСертификатНаКорректность(МенеджерКриптографииДляПроверки, СертификатДляПроверки, 
			РежимыПроверкиСертификата, ОписаниеОшибки, ВызыватьИсключение, УниверсальнаяДатаПроверки);
	...
	ОшибкаПросрочки = ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатПросрочен(СертификатДляПроверки,
		НаДату, РазницаСУниверсальнымВременем());
	...
	Если ДополнительныеПараметры.ДляПроверкиПодписи Тогда
		...
		СвойстваСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСертификата(
			СертификатДляПроверки, РазницаСУниверсальнымВременем(), ДанныеСертификата);
	...
	Если ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра <> ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат() Тогда
		
		ПараметрыПроверкиУдостоверяющегоЦентра = Новый Структура;
		ПараметрыПроверкиУдостоверяющегоЦентра.Вставить("ЭтоПроверкаПодписи", ДополнительныеПараметры.ДляПроверкиПодписи);
		ПараметрыПроверкиУдостоверяющегоЦентра.Вставить("ПроверятьСертификат", ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра);
		
		Результат = РезультатПроверкиУдостоверяющегоЦентраСертификата(СертификатДляПроверки, НаДату,
			ПараметрыПроверкиУдостоверяющегоЦентра, СвойстваСертификата);
		
		Если Не Результат.Действует Или ЗначениеЗаполнено(ДополнительныеПараметры.Сертификат) И ЗначениеЗаполнено(Результат.Предупреждение.ТекстОшибки)
			...
			Если ЗначениеЗаполнено(ДополнительныеПараметры.Сертификат) 
			И ЗначениеЗаполнено(Результат.Предупреждение.ТекстОшибки)
				И Не ДополнительныеПараметры.ДляПроверкиПодписи Тогда
...
Функция ДополнительныеПараметрыПроверкиСертификата() Экспорт
	...
	ДополнительныеПараметры.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные());
	ДополнительныеПараметры.Вставить("ДляПроверкиПодписи", Ложь);
	ДополнительныеПараметры.Вставить("УказанаДатаМеткиВремени", Ложь);
	ДополнительныеПараметры.Вставить("ИгнорироватьПроверкуВСпискеОтозванныхСертификатов", Ложь);
...
// Параметры:
//  ОбъектСсылка - ОпределяемыйТип.ПодписанныйОбъект
//  МассивОтпечатков - см. ЭлектроннаяПодпись.СертификатыШифрования
//
Процедура ДобавитьСертификатыШифрования(ОбъектСсылка, МассивОтпечатков) Экспорт

	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ЭлектроннаяПодписьСлужебный.ДобавитьСертификатыШифрования", 
		"ОбъектСсылка", ОбъектСсылка,
		ЭлектроннаяПодписьСлужебныйПовтИсп.ТипыВладельцев(Истина));
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
		МодульУправлениеДоступом.ПроверитьИзменениеРазрешено(ОбъектСсылка);
	КонецЕсли;
	...
	Для Каждого ОтпечатокСтруктура Из МассивОтпечатков Цикл
		...
		МенеджерЗаписи.Сертификат = Новый ХранилищеЗначения(ОтпечатокСтруктура.Сертификат, Новый СжатиеДанных(9));
...
// Параметры:
//  Источник - ОпределяемыйТип.ПодписанныйОбъект
//  Приемник - СправочникОбъект - соответствует ОпределяемыйТип.ПодписанныйОбъект
//
Процедура СкопироватьПодписиИСертификатыШифрования(Источник, Приемник) Экспорт

	ИмяФункции = "ЭлектроннаяПодписьСлужебный.СкопироватьПодписиИСертификатыШифрования";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяФункции, "Источник", Источник,
		ЭлектроннаяПодписьСлужебныйПовтИсп.ТипыВладельцев(Истина));
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяФункции, "Приемник", Источник,
		ЭлектроннаяПодписьСлужебныйПовтИсп.ТипыВладельцев(Ложь));
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
		МодульУправлениеДоступом.ПроверитьЧтениеРазрешено(Источник);
		МодульУправлениеДоступом.ПроверитьИзменениеРазрешено(Приемник);
	КонецЕсли;
	...
	Если Источник.ПодписанЭП Тогда
		...
		Приемник.Прочитать();
		Приемник.ПодписанЭП = Истина;
		Приемник.Записать();

		ЭлектронныеПодписиИсходногоФайла = ЭлектроннаяПодпись.ПодписиОбъекта(Источник.Ссылка);
		Для Каждого Запись Из ЭлектронныеПодписиИсходногоФайла Цикл
			МенеджерЗаписи = РегистрыСведений.ЭлектронныеПодписи.СоздатьМенеджерЗаписи();
			ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Запись);
			МенеджерЗаписи.ПодписанныйОбъект = Приемник.Ссылка;
	...
	Если Источник.Зашифрован Тогда
		...
		Приемник.Прочитать();
		Приемник.Зашифрован = Истина;

		СертификатыШифрованияИсходногоФайла = ЭлектроннаяПодпись.СертификатыШифрования(Источник);
		ДобавитьСертификатыШифрования(Приемник.Ссылка, СертификатыШифрованияИсходногоФайла);
	
		// Чтобы прошла запись ранее подписанного объекта.
		Приемник.ДополнительныеСвойства.Вставить("ЗаписьПодписанногоОбъекта", Истина);
		Приемник.Записать();
...
Процедура ОчиститьСертификатыШифрования(ОбъектСсылка) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ЭлектроннаяПодписьСлужебный.ОчиститьСертификатыШифрования", 
		"ОбъектСсылка", ОбъектСсылка,
		ЭлектроннаяПодписьСлужебныйПовтИсп.ТипыВладельцев(Истина));
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
...
// Возвращаемое значение:
//  Массив из Структура:
//   * Данные   - Строка - адрес во временном хранилище, в зависимости от того, как передали в ЭлектроннаяПодписьКлиент.Подписать.ОписаниеДанных.
//              - ДвоичныеДанные 
//   * ИмяФайла - Строка 
...
Функция ПолучитьФайлыИзРезультатаПодписания(РезультатПодписания, ПараметрыПолученияФайлов, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыПолученияФайлов.ВыгрузитьДоверенность 
		И ЗначениеЗаполнено(РезультатПодписания.ВыбраннаяДоверенность) 
		...
		Если МодульМашиночитаемыеДоверенностиФНССлужебный.ЭтоМашиночитаемаяДоверенность(
			РезультатПодписания.ВыбраннаяДоверенность) Тогда

			МодульМашиночитаемыеДоверенностиФНС = ОбщегоНазначения.ОбщийМодуль("МашиночитаемыеДоверенностиФНС");
			ФайлыДоверенности = МодульМашиночитаемыеДоверенностиФНС.ФайлыДоверенности(
			РезультатПодписания.ВыбраннаяДоверенность, Ложь);

			Для Каждого ФайлДоверенности Из ФайлыДоверенности Цикл
				Если ЗначениеЗаполнено(ФайлДоверенности.ОписаниеОшибки) Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось сохранить файл доверенности %1: %2'"),
						РезультатПодписания.ВыбраннаяДоверенность, ФайлДоверенности.ОписаниеОшибки);
				КонецЕсли;

				СтруктураОписанияФайла = Новый Структура("Данные, ИмяФайла, ТипФайла");
				СтруктураОписанияФайла.Данные = ПоместитьВоВременноеХранилище(
					ФайлДоверенности.ДвоичныеДанные, УникальныйИдентификатор);
				СтруктураОписанияФайла.ИмяФайла = ФайлДоверенности.ИмяФайла;
				СтруктураОписанияФайла.ТипФайла = ?(ФайлДоверенности.ТипФайла = "Подпись", "ПодписьДоверенности", "Доверенность");
	...
	СтруктураОписанияФайлаСертификата = Неопределено;
	...
	Для Каждого ПодписанныйФайл Из НаборДанных Цикл
		...
		Если ПараметрыПолученияФайлов.ВыгрузитьСертификат И СтруктураОписанияФайлаСертификата = Неопределено Тогда
			
			ДанныеСертификата = ПоместитьВоВременноеХранилище(СвойстваПодписи.Сертификат, УникальныйИдентификатор);
			КомуВыданСертификат = ?(ЗначениеЗаполнено(СвойстваПодписи.КомуВыданСертификат), СвойстваПодписи.КомуВыданСертификат, НСтр("ru = 'Сертификат'"));
			Если ПараметрыПолученияФайлов.ПереводитьИменаФайловВТранслит Тогда
				КомуВыданСертификат = СтроковыеФункции.СтрокаЛатиницей(КомуВыданСертификат);
			КонецЕсли;

			ИмяФайлаСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.ИмяФайлаСертификата(
				"", КомуВыданСертификат, "cer", Ложь);

			СтруктураОписанияФайлаСертификата = Новый Структура("Данные, ИмяФайла, ТипФайла");
			СтруктураОписанияФайлаСертификата.Данные = ДанныеСертификата;
			СтруктураОписанияФайлаСертификата.ИмяФайла = ИмяФайлаСертификата;
			СтруктураОписанияФайлаСертификата.ТипФайла = "Сертификат";
			
		КонецЕсли;
		
		СтруктураОписанияФайла = Новый Структура("Данные, ИмяФайла, ТипФайла");
		СтруктураОписанияФайла.Данные = ПодписанныйФайл.Данные;
		СтруктураОписанияФайла.ИмяФайла = ПодписанныйФайл.Представление;
		СтруктураОписанияФайла.ТипФайла = "Данные";
		Результат.Добавить(СтруктураОписанияФайла);
		
		ДанныеПодписи = ПоместитьВоВременноеХранилище(СвойстваПодписи.Подпись, УникальныйИдентификатор);
		ИмяФайлаПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ИмяФайлаПодписи(Файл.ИмяБезРасширения,
				КомуВыданСертификат, РасширениеДляФайловПодписи);
		
		СтруктураОписанияФайла = Новый Структура("Данные, ИмяФайла, ТипФайла");
		СтруктураОписанияФайла.Данные = ДанныеПодписи;
		СтруктураОписанияФайла.ИмяФайла = ИмяФайлаПодписи;
		СтруктураОписанияФайла.ТипФайла = "Подпись";
		Результат.Добавить(СтруктураОписанияФайла);
	
	КонецЦикла;
	
	Если СтруктураОписанияФайлаСертификата <> Неопределено Тогда
		Результат.Добавить(СтруктураОписанияФайлаСертификата);
	КонецЕсли;
...
Функция ПараметрыПолученияФайлов() Экспорт
	...
	Структура.Вставить("ПереводитьИменаФайловВТранслит", Ложь);
	Структура.Вставить("ВыгрузитьСертификат", Истина);
	Структура.Вставить("ВыгрузитьДоверенность", Истина);
...
// 
// Параметры:
//  ТипОбъекта - Тип - например, СправочникСсылка.Проекты.
// 
// Возвращаемое значение:
//  Булево
//
...
//
// Параметры:
//  СведенияОбЭлектроннойПодписи - Структура - строка с подписями из массива полученного методом ЭлектроннаяПодпись.ПодписиОбъекта.
...
//
Функция ДанныеПоСертификату(СведенияОбЭлектроннойПодписи, УникальныйИдентификатор) Экспорт
...
Процедура НастроитьФормуОбщихНастроек(Форма, РеквизитПутьКДанным) Экспорт
	...
	Если РеквизитПутьКДанным = "РазрешитьДоступКИнтернетСервисамЭлектроннойПодписи" Или  РеквизитПутьКДанным = ""
	 Или РеквизитПутьКДанным = "РазрешитьДоступКИнтернетСервисам" Тогда
		Форма.КонстантаРазрешитьДоступКИнтернетСервисамЭлектроннойПодписи = ОбщиеНастройки.РазрешенДоступКИнтернетСервисам;
		Элементы.КонстантаРазрешитьДоступКИнтернетСервисамЭлектроннойПодписи.Доступность = РазрешенДоступКИнтернетСервисам;
		
		РазрешенДоступКИнтернетСервисамЗаголовок = Новый Массив;
		РазрешенДоступКИнтернетСервисамЗаголовок.Добавить(НСтр(
			"ru = 'Загружать и устанавливать списки отзыва и свежие рекомендации по устранению проблем из Интернета автоматически.'"));

		// СтандартныеПодсистемы.МашиночитаемыеДоверенности
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.МашиночитаемыеДоверенности") Тогда
			РазрешенДоступКИнтернетСервисамЗаголовок.Добавить(НСтр(
				"ru = 'Загружать и проверять статусы машиночитаемых доверенностей единого формата в Распределенном реестре ФНС.'"));
		КонецЕсли;
		// Конец СтандартныеПодсистемы.МашиночитаемыеДоверенности
		
		Элементы.КонстантаРазрешитьДоступКИнтернетСервисамЭлектроннойПодписиРасширеннаяПодсказка.Заголовок = СтрСоединить(РазрешенДоступКИнтернетСервисамЗаголовок, " ");
		Элементы.ГруппаКомментарийРазрешитьЗагрузкуИзИнтернетаВЭлектроннойПодписи.Видимость = Не РазрешенДоступКИнтернетСервисам;
	...
	ИначеЕсли (РеквизитПутьКДанным = "КонстантаТипПодписиКриптографииПоУмолчанию" Или РеквизитПутьКДанным
		= "КонстантаУсовершенствоватьПодписиАвтоматически") Тогда
	...
	ЭлектроннаяПодписьЛокализация.ПриНастройкеФормыОбщихНастроек(
		Форма, РеквизитПутьКДанным, РазрешенДоступКИнтернетСервисам, ОбщиеНастройки, СтандартнаяОбработка);
...
// 
// Параметры:
//  ФизическиеЛицаДляОтбора - Массив из ОпределяемыйТип.ФизическоеЛицо
// 
// Возвращаемое значение:
...
//
Функция СертификатыФизическихЛиц(ФизическиеЛицаДляОтбора) Экспорт
	...
	Запрос.УстановитьПараметр("МассивФизическихЛиц", ФизическиеЛицаДляОтбора);
...
Функция СертификатыФизическихЛицПользователей(Пользователи) Экспорт
	...
	Если ЭлектроннаяПодпись.ОбщиеНастройки().ФизическоеЛицоИспользуется Тогда
...
Функция АдресаСерверовМетокВремениПоУмолчанию() Экспорт
	...
	Возврат АдресаСерверовМетокВремени;
...
// Заполняет список выбора типов подписей в зависимости от настроек.
//
// Параметры:
...
Процедура ЗаполнитьСписокТиповПодписейКриптографии(СписокВыбора, Операция = Неопределено, ТипПодписи = Неопределено) Экспорт
	...
	Если Операция <> "Усовершенствование"
		...
		ЭлектроннаяПодписьЛокализация.ПриЗаполненииПредставленияТипаПодписи(Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES, ПредставлениеПодписи);
		Если ПредставлениеПодписи = "" Тогда
			...
			"CAdES-BES");
		КонецЕсли;
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES, ПредставлениеПодписи);
	КонецЕсли;
	...
	Если ДобавитьТипПодписи Тогда
		...
		ПредставлениеПодписи = "";
		ЭлектроннаяПодписьЛокализация.ПриЗаполненииПредставленияТипаПодписи(Перечисления.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST, ПредставлениеПодписи);
		Если ПредставлениеПодписи = "" Тогда
			ПредставлениеПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если Операция = Неопределено Или Не (РазделениеВключено И Операция = "Настройки") Тогда
		...
		ПредставлениеПодписи = "";
		ЭлектроннаяПодписьЛокализация.ПриЗаполненииПредставленияТипаПодписи(Перечисления.ТипыПодписиКриптографии.АрхивнаяCAdESAv3, ПредставлениеПодписи);
		Если ПредставлениеПодписи = "" Тогда
			ПредставлениеПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.АрхивнаяCAdESAv3, ПредставлениеПодписи);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Метаданные.Обработки.Найти("ПрограммыЭлектроннойПодписиИШифрования") <> Неопределено Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Обновление сведений о приложениях электронной подписи и шифрования.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает стандартные адреса серверов меток времени.'");
	...
	Обработчик.Комментарий =
		НСтр("ru = 'Устанавливает режим использования для приложений электронной подписи и шифрования.'");
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("a6c42e6b-4824-4f5e-a35d-aa0922b253f3");
	...
	Обработчик.Процедура = "ЭлектроннаяПодписьСлужебный.ВключитьКонстантуРазрешитьДоступКИнтернетСервисам";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения = "Оперативно";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.12.68";
	Обработчик.Процедура = "ЭлектроннаяПодписьСлужебный.ДобавитьРольЧтениеСертификатовКлючейЭлектроннойПодписиИШифрования";
	Обработчик.РежимВыполнения = "Оперативно";
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.2.1.7", СтароеИмя, НовоеИмя, Библиотека);
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.3.1.10", СтароеИмя, НовоеИмя, Библиотека);
	
	СтароеИмя = "Роль.ИспользованиеЭлектроннойПодписиИШифрования";
	НовоеИмя  = "Роль.ДобавлениеИзменениеЭлектронныхПодписейИШифрование";
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.3.3.2", СтароеИмя, НовоеИмя, Библиотека);
...
#Область ОбработчикиСобытийПодсистемыУправлениеДоступом

// См. УправлениеДоступомПереопределяемый.ПриЗаполненииСписковСОграничениемДоступа.
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	
	Списки.Вставить(Метаданные.Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования, Истина);
	
	Если Метаданные.Обработки.Найти("ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата") <> Неопределено Тогда
		ОбработкаЗаявлениеНаВыпускНовогоКвалифицированногоСертификата =
			ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
				"Обработка.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата");
		ОбработкаЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.ПриЗаполненииСписковСОграничениемДоступа(Списки);
	КонецЕсли;
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Описание = Описание + "
		|Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Чтение.Пользователи
		|Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Изменение.Пользователи";
	
	Если Метаданные.Обработки.Найти("ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата") <> Неопределено Тогда
		ОбработкаЗаявлениеНаВыпускНовогоКвалифицированногоСертификата =
			ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
				"Обработка.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата");
		ОбработкаЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание);
	КонецЕсли;
	
	ЭлектроннаяПодписьПереопределяемый.ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание);
	
КонецПроцедуры

#КонецОбласти

#Область ОбработчикиСобытийПодсистемыТекущиеДела
...
#Область ОбработчикиСобытийПодсистемыВариантыОтчетов
...
Процедура ПриНастройкеВариантовОтчетов(Настройки) Экспорт
	...
	МодульВариантыОтчетов.НастроитьОтчетВМодулеМенеджера(Настройки, Метаданные.Отчеты.СертификатыЭлектроннойПодписи);
...
Процедура ПриОтправкеДанных(ЭлементДанных, ОтправкаЭлемента, Подчиненному, СозданиеНачальногоОбраза)
	...
	Если ТипЗнч(ЭлементДанных) = Тип("КонстантаМенеджерЗначения.РазрешитьДоступКИнтернетСервисамЭлектроннойПодписи") Тогда
		ОтправкаЭлемента = ОтправкаЭлементаДанных.Игнорировать;
	КонецЕсли;
	
КонецПроцедуры

Функция АдресаСерверовМетокВремени(Строка) Экспорт
	Возврат СтрРазделить(Строка, ", ;" + Символы.ПС + Символы.Таб + Символы.ВТаб, Ложь);
КонецФункции
...
Функция ПараметрыПодписиКриптографии(КонтейнерПодписи, РазницаСУниверсальнымВременем, ДатаСеанса) Экспорт
	...
	Если СертификатСуществует Тогда
		ПараметрыПодписи.Сертификат = Подпись.СертификатПодписи;
...
Функция СвойстваПодписиЧтениеМенеджеромКриптографии(Подпись, МенеджерКриптографии, ПрочитатьСертификаты) Экспорт
	...
	ПараметрыПодписиКриптографии = ПараметрыПодписиКриптографии(
						КонтейнерПодписи, РазницаСУниверсальнымВременем(), ТекущаяДатаСеанса());
...
Функция СертификатИзДвоичныхДанныхПодписи(Подпись, РезультатВХранилищеЗначения = Ложь) Экспорт
	...
	СвойстваПодписи = СвойстваПодписиИзДвоичныхДанных(Подпись, Истина);
...
//
// Параметры:
//  ДобавитьИзФайлов - Булево - сертификат добавляется из файлов.
//
// Возвращаемое значение:
//  Структура - сведения для установки на форме, содержащей указанные свойства:
//   * Заголовок - Строка - заголовок кнопки добавления сертификата.
//   * ЗаголовокРасширеннойПодсказки - Строка - текст расширенной подсказки кнопки добавления сертификата.
//   * Назначение - Строка - см. ЭлектроннаяПодписьСлужебныйКлиент.ДобавитьСертификатПослеВыбораНазначения
//
Функция СвойстваКомандыДобавленияСертификатов(ДобавитьИзФайлов = Ложь) Экспорт
	
	Результат = Новый Структура("Заголовок, ЗаголовокРасширеннойПодсказки, Назначение");
	
	Если ЭлектроннаяПодпись.ДобавлениеИзменениеЭлектронныхПодписей() Тогда
		
		Если ДобавитьИзФайлов Тогда
			Результат.Заголовок = НСтр("ru = 'Для подписания и шифрования из файлов...'");
			Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для подписания и шифрования из файлов'");
			Результат.Назначение = "ДляПодписанияШифрованияИРасшифровкиИзФайлов";
		Иначе
			Результат.Заголовок = НСтр("ru = 'Для подписания и шифрования из личного хранилища...'");
			Если ИспользоватьСервисОблачнойПодписи() Тогда
				Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для подписания и шифрования из установленных на сервере DSS и на компьютере'");
			ИначеЕсли ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
				Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для подписания и шифрования из установленных в сервисе и на компьютере'");
			Иначе
				Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для подписания и шифрования из установленных на компьютере'");
			КонецЕсли;
			Результат.Назначение = "ДляПодписанияШифрованияИРасшифровки";
		КонецЕсли;
		
	Иначе
		
		Если ДобавитьИзФайлов Тогда
			Результат.Заголовок = НСтр("ru = 'Для шифрования и расшифровки из файлов...'");
			Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для шифрования и расшифровки из файлов'");
			Результат.Назначение = "ДляПодписанияШифрованияИРасшифровкиИзФайлов";
		Иначе
			Результат.Заголовок = НСтр("ru = 'Для шифрования и расшифровки из личного хранилища...'");
			Если ИспользоватьСервисОблачнойПодписи() Тогда
				Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для шифрования и расшифровки из установленных на сервере DSS и на компьютере'");
			ИначеЕсли ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
				Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для шифрования и расшифровки из установленных в сервисе и на компьютере'");
			Иначе
				Результат.ЗаголовокРасширеннойПодсказки = НСтр("ru = 'Добавить для шифрования и расшифровки из установленных на компьютере'");
			КонецЕсли;
			Результат.Назначение = "ДляШифрованияИРасшифровки";
		КонецЕсли;
...
Функция ОшибкаСертификатПомеченКакОтозванный() Экспорт
	...
	ПредставлениеОшибки.Вставить("ТекстОшибки", НСтр("ru='Сертификат помечен как отозванный.'"));
...
// Только для внутреннего использования.
Функция ИспользованиеКлючаСтрокой(Число) Экспорт
	
	Массив = Новый Массив;
	Соответствие = Новый Соответствие;
	Соответствие.Вставить(0, НСтр("ru = 'Цифровая подпись'"));
	Соответствие.Вставить(1, НСтр("ru = 'Неотрекаемость'"));
	Соответствие.Вставить(2, НСтр("ru = 'Шифрование ключей'"));
	Соответствие.Вставить(3, НСтр("ru = 'Шифрование данных'"));
	Соответствие.Вставить(4, НСтр("ru = 'Согласование ключей'"));
	Соответствие.Вставить(5, НСтр("ru = 'Подписывание сертификатов'"));
	Соответствие.Вставить(6, НСтр("ru = 'Подписывание списков отзыва'"));
	Соответствие.Вставить(7, НСтр("ru = 'Только шифрование'"));
	Соответствие.Вставить(8, НСтр("ru = 'Только расшифровка'"));
	
	Для Номер = 0 По 8 Цикл
		Если ПроверитьБит(Число, Номер) Тогда
			Массив.Добавить(Соответствие.Получить(Номер));
		КонецЕсли;
	КонецЦикла;
...
//  ПараметрыМенеджераКриптографии - см. ПараметрыСозданияМенеджераКриптографии.
//
// Возвращаемое значение:
...
Функция МенеджерКриптографии(Операция, ПараметрыСозданияМенеджераКриптографии = Неопределено) Экспорт
	...
	АлгоритмШифрования	= ПараметрыСозданияМенеджераКриптографии.АлгоритмШифрования;
	Автоопределение		= ПараметрыСозданияМенеджераКриптографии.Автоопределение;
	
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("Операция", Операция);
	ДополнительныеПараметры.Вставить("Автоопределение", Автоопределение);
	ДополнительныеПараметры.Вставить("АлгоритмПодписи", АлгоритмПодписи);
	ДополнительныеПараметры.Вставить("АлгоритмШифрования", АлгоритмШифрования);
	
	ИмяКомпьютера = ИмяКомпьютера();
	ОписаниеОшибок = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовоеОписаниеОшибок(ИмяКомпьютера);
	Менеджер = НовыйМенеджерКриптографии(Программа, ОписаниеОшибок.Ошибки, ДополнительныеПараметры);
...
Функция ОтпечаткиСертификатов(ТолькоЛичные, ОписаниеОшибки) Экспорт
	...
	Если ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере() Тогда
		...
		ПараметрыСоздания = ПараметрыСозданияМенеджераКриптографии();
		ПараметрыСоздания.ПоказатьОшибку = ОписаниеОшибки;
		МенеджерКриптографии = МенеджерКриптографии("ПолучениеСертификатов", ПараметрыСоздания);
		
		Ошибка = ПараметрыСоздания.ОписаниеОшибки;
		Если МенеджерКриптографии <> Неопределено Тогда
			
			МассивОтпечатков = ОтпечаткиСертификатовВХранилище(
				МенеджерКриптографии, ТипХранилищаСертификатовКриптографии.ПерсональныеСертификаты, Ошибка);
			
			Если Не ТолькоЛичные Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивОтпечатков,
					ОтпечаткиСертификатовВХранилище(
						МенеджерКриптографии, ТипХранилищаСертификатовКриптографии.СертификатыПолучателей, Ошибка),
					Истина);
			КонецЕсли;
...
Процедура ПередНачаломРедактированияСертификатаКлюча(Ссылка, Сертификат, ПараметрыРеквизитов) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		ПараметрыРеквизитов.Вставить(СтрокаТаблицы.ИмяРеквизита, Параметры);
	КонецЦикла;
...
Функция СведенияОПодписиДляЖурналаРегистрации(ДатаПодписи, СвойстваПодписи, ЭтоОшибкаПодписания = Ложь) Экспорт
	...
	Если НовыеСвойстваПодписи.ОписаниеСертификата <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(СвойстваСертификата, НовыеСвойстваПодписи.ОписаниеСертификата);
		Если Не ЗначениеЗаполнено(СвойстваСертификата.ДействителенДо) Тогда
			СвойстваСертификата.ДействителенДо = СвойстваСертификата.ДатаОкончания;
		КонецЕсли;
	Иначе
		
		Если ТипЗнч(НовыеСвойстваПодписи.Сертификат) = Тип("Строка")
		   И ЭтоАдресВременногоХранилища(НовыеСвойстваПодписи.Сертификат) Тогда
			Сертификат = ПолучитьИзВременногоХранилища(НовыеСвойстваПодписи.Сертификат);
		Иначе
			Сертификат = НовыеСвойстваПодписи.Сертификат;
		КонецЕсли;
		...
		Иначе
			СвойстваСертификата.КомуВыдан = НовыеСвойстваПодписи.КомуВыданСертификат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьСписокСертификатов(Сертификаты, СвойстваСертификатовНаКлиенте, КромеУжеДобавленных,
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		ДоступныТолькоСКлючом = ДополнительныеПараметры.ДоступныТолькоСКлючом;
	Иначе
		ОтборПоОрганизации = Неопределено;
		ВыполнятьНаСервере = Неопределено;
		ДоступныТолькоСКлючом = Ложь;
	КонецЕсли;
		...
		Если МенеджерКриптографии <> Неопределено Тогда
			...
			Попытка
				...
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ДобавитьСвойстваСертификатов(ТаблицаСвойствСертификатов,
					МассивСертификатов, БезОтбора, РазницаСУниверсальнымВременем(), ТекущаяДатаСеанса());
			Исключение
				ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиентСервер.ДобавитьСвойстваСертификатов(ТаблицаСвойствСертификатов,
			МассивСертификатов, БезОтбора, РазницаСУниверсальнымВременем(), ТекущаяДатаСеанса(), ПараметрыДобавленияСвойств);
	...
	ФильтрКромеУжеДобавленных = Не БезОтбора И КромеУжеДобавленных;
	ОбработатьДобавленныеСертификаты(ТаблицаСвойствСертификатов, ФильтрКромеУжеДобавленных, ОтборПоОрганизации, ДоступныТолькоСКлючом);
...
Функция ПроверитьСертификатНаКорректность(МенеджерКриптографииДляПроверки, СертификатДляПроверки, 
	...
	Попытка
		ПроверитьСертификатНаКорректностьНаДату(МенеджерКриптографииДляПроверки, СертификатДляПроверки,
			РежимыПроверкиСертификата, УниверсальнаяДатаПроверки);
		Возврат Истина;
	Исключение
		ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(ОшибкаПоКлассификатору.ДействияДляУстранения) Тогда
		Возврат ПроверитьСертификатНаКорректностьПовторно(МенеджерКриптографииДляПроверки, СертификатДляПроверки,
			...
			ОшибкаПоКлассификатору.ДействияДляУстранения, УниверсальнаяДатаПроверки);
	КонецЕсли;
...
// Для функции ПроверитьСертификат
Процедура ПроверитьСертификатНаКорректностьНаДату(МенеджерКриптографииДляПроверки, СертификатДляПроверки,
	РежимыПроверкиСертификата, УниверсальнаяДатаПроверки)
	
	Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
		//@skip-check bsl-legacy-check-dynamic-feature-access
		МенеджерКриптографииДляПроверки.ПроверитьСертификат(
			СертификатДляПроверки, РежимыПроверкиСертификата, УниверсальнаяДатаПроверки);
	Иначе
...
Функция ПроверитьСертификатНаКорректностьПовторно(МенеджерКриптографииДляПроверки, СертификатДляПроверки, 
	РежимыПроверкиСертификата, ОписаниеОшибки, ВызыватьИсключение, ДействияДляУстранения, УниверсальнаяДатаПроверки)
	
	ТребуетсяУстановкаСпискаОтзыва = Ложь;
	
	Если ТипЗнч(МенеджерКриптографииДляПроверки) = Тип("МенеджерКриптографии") Тогда
		Если ЗначениеЗаполнено(ДействияДляУстранения) И ДействияДляУстранения.Найти("УстановитьСписокОтзываСертификата") <> Неопределено 
			...
			Если ЗначениеЗаполнено(ИмяИздателя) Тогда
				...
				СвойстваСертификата = ЭлектроннаяПодпись.СвойстваСертификата(СертификатДляПроверки);
				
				Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
					АдресСпискаОтзываВнутренний = ЭлектроннаяПодписьСлужебныйКлиентСервер.АдресСпискаОтзываВнутренний(
						ИмяИздателя,
						СвойстваСертификата, 
						ЭлектроннаяПодписьСлужебныйПовтИсп.КаталогиСписковОтзываУЦ());
				Иначе
					АдресСпискаОтзываВнутренний = ЭлектроннаяПодписьСлужебныйКлиентСервер.АдресСпискаОтзываВнутренний(
						ИмяИздателя,
						СертификатДляПроверки.Выгрузить(), 
						ЭлектроннаяПодписьСлужебныйПовтИсп.КаталогиСписковОтзываУЦ());
				КонецЕсли;
	...
	Если ТребуетсяУстановкаСпискаОтзыва Тогда
		...
		СведенияОСертификатеСтрокой = ЭлектроннаяПодписьСлужебныйКлиентСервер.СведенияОСертификатеСтрокой(СвойстваСертификата);
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось установить список отзыва на сервере при проверке сертификата: %1
				|%2.'"),
				
				ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()),
				СведенияОСертификатеСтрокой);
		...
		ЗаписьЖурналаРегистрации(
					...
					УровеньЖурналаРегистрации.Информация, , ,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Установлен список отзыва на сервере при проверке сертификата: %1'"),
						СведенияОСертификатеСтрокой));
...
Процедура ДополнитьСписокСертификатовВЛичномХранилищеНаСервере(СписокСертификатов, Ошибка = "") Экспорт
	...
	ОтпечаткиСертификатовСервиса = ОтпечаткиСертификатовСервиса();
	Для Каждого Отпечаток Из ОтпечаткиСертификатовСервиса Цикл
		СписокСертификатов.Добавить(Отпечаток);
	КонецЦикла;
...
Функция СохраненныеСвойстваСертификата(Отпечаток, Адрес, ПараметрыРеквизитов, ДляШифрования = Ложь) Экспорт
	...
	СохраненныеСвойства.Вставить("ФизическоеЛицо");
...
Процедура НастроитьПояснениеВводаПароля(Форма, ИмяЭлементаВводитьПарольВПрограммеЭлектроннойПодписи = "", ИмяЭлементаПояснениеУсиленногоПароля = "") Экспорт
	...
	Если ЗначениеЗаполнено(ИмяЭлементаВводитьПарольВПрограммеЭлектроннойПодписи) Тогда
		...
		Элемент.Заголовок = НСтр("ru = 'Вводить пароль в приложении электронной подписи'");
		Элемент.Подсказка =
			НСтр("ru = '- Приложение электронной подписи запрашивает пароль и позволяет его сохранить.
...
Процедура ЗарегистрироватьОшибкуУсовершенствованияПодписиВЖурнале(ОписаниеОшибки, СвойстваПодписи = Неопределено) 
	...
	ИмяСобытия = НСтр("ru = 'Электронная подпись.Усовершенствование подписи'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	ЗаписьЖурналаРегистрации(ИмяСобытия,
...
Функция НастройкиСлужебнойУчетнойЗаписиДляУсовершенствованияПодписей(ИдентификаторФормы = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкиПодключения.Ошибка) Тогда
...
Функция СвойстваКриптопровайдера(МенеджерКриптографии)
	...
	Если ЗначениеЗаполнено(ОписаниеПути.ТекстОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить путь к приложению электронной подписи
			           |""%1"" по причине:
			           |%2'"),
...
Процедура ОбработатьДобавленныеСертификаты(ТаблицаСвойствСертификатов, КромеУжеДобавленных,
	ОтборПоОрганизации = Неопределено, ДоступныТолькоСКлючом = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		СертификатСсылка = Выборка.Ссылка;
		...
		Строка = ТаблицаСвойствСертификатов.Найти(Выборка.Отпечаток, "Отпечаток"); // Повторный поиск, могли удалить ранее.
		Если Строка = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		// Оставляем только доступные пользователю сертификаты.
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
			ЧтениеРазрешено = МодульУправлениеДоступом.ЧтениеРазрешено(СертификатСсылка, ТекущийПользователь);
		Иначе
			ЧтениеРазрешено = ПравоДоступа("Чтение", СертификатСсылка.Метаданные());
		КонецЕсли;
		Если ЧтениеРазрешено Тогда // Защита от ошибки в данных (дубли сертификатов).
			Если ДоступныТолькоСКлючом И СертификатСсылка.Пользователи.Количество() <> 0 Тогда
				СтрокаПользовательСертификата = СертификатСсылка.Пользователи.Найти(ТекущийПользователь, "Пользователь");
				Если СтрокаПользовательСертификата <> Неопределено
					И СтрокаПользовательСертификата.БезИспользованияКлюча Тогда
					ТаблицаСвойствСертификатов.Удалить(Строка); // Исключаются сертификаты, недоступные для подписания и расшифровки.
					Продолжить;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСуществующиеСертификатыПользователя(СписокВыбора, ОтпечаткиСертификатовНаКлиенте,
		...
		Если МенеджерКриптографии <> Неопределено Тогда
			...
			Попытка
				...
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ДобавитьОтпечаткиСертификатов(ОтпечаткиСертификатовНаКлиенте,
					МассивСертификатов, РазницаСУниверсальнымВременем(), ТекущаяДатаСеанса);
			Исключение // АПК:280
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиентСервер.ДобавитьОтпечаткиСертификатов(
			ОтпечаткиСертификатовНаКлиенте, МассивСертификатов, РазницаСУниверсальнымВременем(), ТекущаяДатаСеанса);
	КонецЕсли;
	...
	|				И &ОтборБезИспользованияКлюча)";
		
	ТекстЗапроса = ?(ОтборПоОрганизации,
		СтрЗаменить(ТекстЗапроса, "&ОтборПоОрганизации", "Сертификаты.Организация = &Организация"),
		СтрЗаменить(ТекстЗапроса, "&ОтборПоОрганизации", "ИСТИНА"));
	
	ТекстЗапроса = ?(Пользователи.ЭтоПолноправныйПользователь(),
		СтрЗаменить(ТекстЗапроса, "&ОтборБезИспользованияКлюча", "ИСТИНА"),
		СтрЗаменить(ТекстЗапроса, "&ОтборБезИспользованияКлюча", "СертификатыКлючейЭлектроннойПодписиИШифрованияПользователи.БезИспользованияКлюча = ЛОЖЬ"));
...
Функция ОтпечаткиСертификатовВХранилище(МенеджерКриптографии, ТипХранилища, Ошибка)
	...
	Для Каждого Сертификат Из МассивСертификатов Цикл
		Результат.Добавить(Base64Строка(Сертификат.Отпечаток));
	КонецЦикла;
...
Процедура ЗаполнитьДополнительныеСвойстваСертификата(Форма)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "РезультатПроверкиУдостоверяющегоЦентра") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПроверятьСертификат") Тогда
			ПараметрыПроверки = Новый Структура;
			ПараметрыПроверки.Вставить("ЭтоПроверкаПодписи", Ложь);
			ПараметрыПроверки.Вставить("ПроверятьСертификат", Форма.ПроверятьСертификат);
		КонецЕсли;
		Форма.РезультатПроверкиУдостоверяющегоЦентра = РезультатПроверкиУдостоверяющегоЦентраСертификата(Сертификат,,ПараметрыПроверки);
	КонецЕсли;
	...
	Если ЕстьПараметрОповеститьОбОкончанииСрокаДействия И Форма.Параметры.ОповеститьОбОкончанииСрокаДействия 
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "РезультатПроверкиУдостоверяющегоЦентра")
			И Форма.РезультатПроверкиУдостоверяющегоЦентра <> Неопределено
			И Форма.РезультатПроверкиУдостоверяющегоЦентра.Действует
			И (ЗначениеЗаполнено(Форма.РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ТекстОшибки) 
			Или ЗначениеЗаполнено(Форма.РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ДополнительныеСведения)) Тогда
			Форма.ОповеститьОбОкончанииСрокаДействия = Не РегистрыСведений.ОповещенияПользователейСертификатов.ПользовательОповещен(Форма.Сертификат)
...
Функция НовыйМенеджерКриптографии(Программа, Ошибки, ДополнительныеПараметры)
	
	ПрограммыАвто		= Неопределено;
	Операция			= ДополнительныеПараметры.Операция;
	Автоопределение		= ДополнительныеПараметры.Автоопределение;
	АлгоритмПодписи		= ДополнительныеПараметры.АлгоритмПодписи;
	АлгоритмШифрования	= ДополнительныеПараметры.АлгоритмШифрования;
	...
	Для Каждого ОписаниеПрограммы Из ОписанияПрограмм Цикл
		...
		АлгоритмыУстановлены = ЭлектроннаяПодписьСлужебныйКлиентСервер.МенеджерКриптографииАлгоритмыУстановлены(
			ОписаниеПрограммы, Менеджер, Ошибки, АлгоритмШифрования);
...
Процедура ЗаполнитьПараметрыДляСозданияМенеджераКриптографии(Программа, АлгоритмПодписи, ОписаниеОшибки, ТребуетсяЗакрытыйКлюч)
	...
	ИначеЕсли ТипДанных = "ЗашифрованныеДанные" Тогда
		...
		Программа = Неопределено;
		ОписаниеОшибки = НСтр("ru = 'Для определения приложения для расшифровки в процедуру должны быть переданы данные сертификата.'");
		Возврат;
		
	КонецЕсли;
	
	Программа = Неопределено;
	ОписаниеОшибки = НСтр("ru = 'Не удалось определить приложение электронной подписи и шифрования для переданных данных.'");
...
Функция ПрограммаПоСертификату(Знач Сертификат, ТребуетсяЗакрытыйКлюч = Неопределено,  ОбъектКомпоненты = Неопределено) Экспорт
	...
	Если ТребуетсяЗакрытыйКлюч <> Ложь Тогда
		...
		ИначеЕсли ТребуетсяЗакрытыйКлюч = Истина И ТекущийКриптопровайдер.Получить("type") <> 0 Тогда
			Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сертификат связан с приложением %1 с типом %2 на сервере %3, настройте его для использования и выберите в сертификате.'"),
	...
	Если ТребуетсяЗакрытыйКлюч = Истина Тогда
		Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить приложение по закрытому ключу сертификата на сервере %1.'"), ИмяКомпьютера());
		Возврат Результат;
	КонецЕсли;
...
Функция ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Экспорт
	...
	Возврат МодульЭлектроннаяПодписьВМоделиСервиса.ИспользованиеВозможно() И ОбщегоНазначения.РазделениеВключено();
...
Функция ПоставляемыеНастройкиПрограмм() Экспорт
	...
	Настройки.Колонки.Добавить("Неактуальная", Новый ОписаниеТипов("Булево"));
...
// Вызывается при переходе на версию конфигурации 3.1.11.61 
Процедура ВключитьКонстантуРазрешитьДоступКИнтернетСервисам() Экспорт
	
	Константы.РазрешитьДоступКИнтернетСервисамЭлектроннойПодписи.Установить(Истина);
...
Функция КоличествоСертификатовСИстекающимСрокомДействия()
	...
	Если Метаданные.Обработки.Найти("ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата") <> Неопределено Тогда
		ОбработкаЗаявлениеНаВыпускНовогоКвалифицированногоСертификата =
			ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
				"Обработка.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата");
		ОбработкаЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.ДополнитьЗапросПолученияКоличестваСертификатовСИстекающимСрокомДействия(Запрос);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ДополнительноеУсловие", "");
	КонецЕсли;
...
Функция ПолучитьФизическиеЛицаПоПолюСертификатаКомуВыдан(КомуВыдан) Экспорт
	...
	Пока ВыборкаНаименование.Следующий() Цикл
		...
		ФизическиеЛицаДляНаименования = Новый Массив;
		
		Пока Выборка.Следующий() Цикл
			ФизическиеЛицаДляНаименования.Добавить(Выборка.Ссылка);
		КонецЦикла;
		
		ФизическиеЛицаПоФИО.Вставить(ВыборкаНаименование.Наименование, ФизическиеЛицаДляНаименования); 
...
Процедура УстановитьЗаголовокЭлектроннаяПодписьНаСервере(Форма)
	...
	Если НаборКонстант.ИспользоватьЭлектронныеПодписи Тогда
		...
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать приложение электронной подписи на компьютер пользователя для проверки электронных подписей и сертификатов.'");
	Иначе
		ЗаголовокФлажка = НСтр("ru = 'Проверять сертификаты на сервере'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать приложение электронной подписи на компьютер пользователя для проверки сертификатов.'");
	КонецЕсли;
	
	Элементы.ПроверятьЭлектронныеПодписиНаСервере.Заголовок = ЗаголовокФлажка;
	
	Если ИнформационнаяБазаФайловая Тогда
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на компьютер, где работает веб-сервер, подключенный к файловой информационной базе, должно быть установлено хотя бы одно <a href=%1>приложение электронной подписи</a> из списка.'");
	Иначе
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на каждый компьютер, где работает сервер 1С:Предприятия, должно быть установлено хотя бы одно <a href=%1>приложение электронной подписи</a> из списка.'");
	КонецЕсли;
	...
	Элементы.ПроверятьЭлектронныеПодписиНаСервереРасширеннаяПодсказка.Заголовок = 
		СтроковыеФункции.ФорматированнаяСтрока(ПодсказкаФлажка, "Программы");
	
	Если Не НаборКонстант.ИспользоватьЭлектронныеПодписи Тогда
		ЗаголовокФлажка = НСтр("ru = 'Шифровать и расшифровывать на сервере'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать приложение электронной подписи и сертификат на компьютер пользователя для шифрования и расшифровки.'");
		
	ИначеЕсли Не НаборКонстант.ИспользоватьШифрование Тогда
		ЗаголовокФлажка = НСтр("ru = 'Подписывать на сервере'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать приложение электронной подписи и сертификат на компьютер пользователя для подписания.'");
	Иначе
		ЗаголовокФлажка = НСтр("ru = 'Подписывать и шифровать на сервере'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать приложение электронной подписи и сертификат на компьютер пользователя для подписания, шифрования и расшифровки.'");
	КонецЕсли;
		
	Элементы.СоздаватьЭлектронныеПодписиНаСервере.Заголовок = ЗаголовокФлажка;
	
	Если ИнформационнаяБазаФайловая Тогда
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на компьютер, где работает веб-сервер, подключенный к файловой информационной базе, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.'");
	Иначе
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на каждый компьютер, где работает сервер 1С:Предприятия, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.'"); 
	КонецЕсли;
...
Функция КриптографияДоступна() Экспорт
	
	Настройки = ЭлектроннаяПодпись.ОбщиеНастройки();
	
	Возврат Не ОбщегоНазначения.РазделениеВключено()
		И Настройки.ПроверятьЭлектронныеПодписиНаСервере
		Или Настройки.СоздаватьЭлектронныеПодписиНаСервере
		Или ОбщегоНазначения.ИнформационнаяБазаФайловая()
		И Не ОбщегоНазначения.КлиентПодключенЧерезВебСервер();
	
КонецФункции

// Функция - Проверяет наличие прав на добавление сертификатов в справочник.
// 
// Возвращаемое значение:
//   Булево
//
Функция ЕстьПравоНаДобавлениеСертификатовВСправочник() Экспорт
	
	Возврат ПравоДоступа("Добавление", Метаданные.Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования);
...
Функция ОшибкаПоКлассификатору(ТекстДляПоискаВКлассификаторе, ОшибкаНаСервере = Ложь, ОшибкаПроверкиПодписи = Ложь,
		УказанСертификат = Ложь) Экспорт
	
	ОшибкаПоКлассификатору = ЭлектроннаяПодписьСлужебныйПовтИсп.ОшибкаПоКлассификатору(
		ТекстДляПоискаВКлассификаторе, ОшибкаНаСервере, ОшибкаПроверкиПодписи, УказанСертификат);
...
Функция ПредставлениеОшибки() Экспорт
	...
	ПредставлениеОшибки.Вставить("НеверныйХешПодписи", Ложь);
	ПредставлениеОшибки.Вставить("НеизвестныйАлгоритмКриптографии", Ложь);
...
Функция КлассификаторОшибокКриптографии() Экспорт
	...
	Результат = НовыйКлассификаторОшибокКриптографии();
	Если ДанныеКлассификатора = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из ДанныеКлассификатора.Классификатор Цикл
...
Функция НовыйКлассификаторОшибокКриптографии()
	...
	Результат.Колонки.Добавить("НеверныйХешПодписи", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ОшибкаПриПроверкеПодписи", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("НеизвестныйАлгоритмКриптографии", Новый ОписаниеТипов("Булево"));
...
Функция ПредставлениеКлассификатораОшибок(ДанныеКлассификатора) Экспорт
	...
	Для Каждого ИзвестнаяОшибка Из КлассификаторОшибок.Classifier Цикл
		...
		НоваяОшибка.ОшибкаПриПроверкеПодписи     = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ИзвестнаяОшибка, "Sign", Ложь);
		...
		Если ЗначениеЗаполнено(Категория) Тогда
			НоваяОшибка.НеверныйХешПодписи = ОшибкаНеверныйХешПодписи(Категория);
			Если Не НоваяОшибка.НеверныйХешПодписи Тогда
				НоваяОшибка.ТребуетПроверки   = ОшибкаТребуетПерепроверки(Категория);
				Если Не НоваяОшибка.ТребуетПроверки Тогда
					НоваяОшибка.СертификатОтозван = ОшибкаСертификатОтозван(Категория);
					Если Не НоваяОшибка.СертификатОтозван Тогда
						НоваяОшибка.НеизвестныйАлгоритмКриптографии = ОшибкаНеизвестныйАлгоритмКриптографии(Категория);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		НоваяОшибка.ТекстОшибкиНижнийРегистр = НРег(НоваяОшибка.ТекстОшибки);
		
		ТекстОшибкиНаАнглийскомЯзыке = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ИзвестнаяОшибка, "En", Неопределено);
		Если ЗначениеЗаполнено(ТекстОшибкиНаАнглийскомЯзыке) Тогда
			НоваяОшибкаНаАнглийскомЯзыке = Классификатор.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяОшибкаНаАнглийскомЯзыке, НоваяОшибка);
			НоваяОшибкаНаАнглийскомЯзыке.ТекстОшибки = ИзвестнаяОшибка.En;
			НоваяОшибкаНаАнглийскомЯзыке.ТекстОшибкиНижнийРегистр = НРег(НоваяОшибкаНаАнглийскомЯзыке.ТекстОшибки);
		КонецЕсли;
...
Функция ОшибкаНеверныйХешПодписи(Категория)
	
	Если Категория = "hash_value" Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ОшибкаНеизвестныйАлгоритмКриптографии(Категория)
	
	Если Категория = "unknown_algo" Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ДополнитьТехническойИнформациейОСервере(ТехническаяИнформация,
	...
	Иначе
		...
		ТехническаяИнформация = ТехническаяИнформация
			+ Символы.ПС + ЭлектроннаяПодписьСлужебныйВызовСервера.РезультатДиагностикиСерверовМетокВремени();
...
Функция ТехническаяИнформацияОКомпьютере()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		+ СтандартныеПодсистемыСервер.ИнформацияДляПоддержки();
...
Функция ТехническаяИнформацияОПрограммах()
	...
	Если Не УстановленныеПрограммыРезультат.ПроверкаВыполнена Тогда
		ДиагностическаяИнформация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось автоматически определить приложения электронной подписи и шифрования на ""%1"": %2'"),
	...
	Иначе
		...
		Если УстановленныеПрограммыРезультат.Криптопровайдеры.Количество() = 0 Тогда
			ДиагностическаяИнформация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Приложения электронной подписи и шифрования на ""%1"" не установлены.'"),
				ИмяКомпьютера());
		Иначе
			ДиагностическаяИнформация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Автоматически определенные приложения электронной подписи и шифрования на сервере ""%1"":'"), ИмяКомпьютера()) + Символы.ПС;
	...
	ДиагностическаяИнформация = ДиагностическаяИнформация + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Приложения на сервере из справочника ""%1"":'"), ИмяКомпьютера()) + Символы.ПС;
...
Функция ТехническаяИнформацияОНастройкахПрограммВСправочнике(ПроверенныеПутиКМодулямПрограммНаКлиенте)
	...
	ДиагностическаяИнформация = Символы.ПС
		+ НСтр("ru = 'Настройки приложений электронной подписи и шифрования:'") + Символы.ПС;
	
	ИспользуемыеПрограммы = ЭлектроннаяПодписьСлужебныйВызовСервера.ИспользуемыеПрограммы();
	Если ИспользуемыеПрограммы.Количество() = 0 Тогда
		ДиагностическаяИнформация = ДиагностическаяИнформация
			+ НСтр("ru = 'В справочнике нет ни одного приложения электронной подписи и шифрования.'");
		Возврат ДиагностическаяИнформация;
	КонецЕсли;
...
Функция СкачатьФайлСпискаОтзываНаСервере(Знач Адреса, АдресВнутренний = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(АдресВнутренний) Тогда
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ДанныеИзКэша) И ДанныеИзКэша.ДатаПоследнегоИзменения = ДатаПоследнегоИзменения И ДатаПоследнегоИзменения <> Неопределено Тогда
			...
			Иначе
				...
				ПериодДействияСпискаОтзыва = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСпискаОтзыва(РезультатЗагрузки.ДанныеФайла);
...
Процедура СкачатьФайлСпискаОтзыва(Адреса, РезультатЗагрузки, ДанныеИзКэша = Неопределено, ДатаПоследнегоИзменения = Неопределено)
	...
	Для Каждого ТекущийАдрес Из Адреса Цикл
		...
		Если СтрНачинаетсяС(НРег(ТекущийАдрес), "ldap://") Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Результат.Статус Тогда
			...
			Если СтрокаBase64 <> Неопределено Тогда
				Попытка
					...
					РезультатЗагрузки.ДанныеФайла = ДвоичныеДанные;
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					РезультатЗагрузки.Вставить("СообщениеОбОшибке",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не удалось получить данные списка отзыва %1 по причине:
							|%2'"), ТекущийАдрес, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
					ЗаписьЖурналаРегистрации(
						НСтр("ru = 'Электронная подпись.Обновление списков отзыва'",
						ОбщегоНазначения.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка, , ,
...
Процедура ЗаполнитьРезультатЗагрузкиСпискаОтзываИзДанныхКэша(РезультатЗагрузки, ДанныеИзКэша)
	...
	Если Не ЗначениеЗаполнено(ДанныеИзКэша.СписокОтзыва) Тогда
	...
	Если Не ЗначениеЗаполнено(ДанныеИзКэша.ДатаОкончанияДействия) 
		...
		РезультатЗагрузки.ИмяФайла = Неопределено;
		РезультатЗагрузки.ДанныеФайла = Неопределено;
		РезультатЗагрузки.СообщениеОбОшибке = НСтр("ru = 'Список отзыва просрочен. Обратитесь в удостоверяющий центр, выдавший сертификат.'");
		Возврат;
	...
	Если СтрЗаканчиваетсяНа(РезультатЗагрузки.ИмяФайла, ".zip") Тогда
		РезультатЗагрузки.ИмяФайла = СтрЗаменить(РезультатЗагрузки.ИмяФайла, ".zip", ".crl");
	КонецЕсли;
...
Функция ДатаПоследнегоИзмененияФайла(РезультатЗагрузки) Экспорт
	...
	Заголовки = СтандартныеПодсистемыСервер.HTTPЗаголовкиВНижнийРегистр(Заголовки);
...
Функция ДополнитьРешениеКлассификатораОшибокПодробностями(ОшибкаПоКлассификатору,
	ДополнительныеДанные, МестоОшибки = Неопределено) Экспорт

	Если Не ЗначениеЗаполнено(ОшибкаПоКлассификатору.ДействияДляУстранения) Тогда
	...
	Если ОшибкаПоКлассификатору.ДействияДляУстранения.Найти(
		...
		ИздательСертификата = ИздательСертификата(ДополнительныеДанные.ДанныеСертификата);
	...
	Если ОшибкаПоКлассификатору.ДействияДляУстранения.Найти(
					"ДиагностикаСерверовМетокВремениСервер") <> Неопределено Тогда

		Решение.Добавить(СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru='<a href = ""%1"">Проверьте доступность</a> серверов меток времени на сервере %2.'"),
			"ДиагностикаСерверовМетокВремениСервер", ИмяКомпьютера()));
	КонецЕсли;

	Если ОшибкаПоКлассификатору.ДействияДляУстранения.Найти(
					"ДиагностикаСерверовМетокВремениКлиент") <> Неопределено Тогда

		Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
			
			АдресаСерверовМетокВремени = ЭлектроннаяПодпись.ОбщиеНастройки().АдресаСерверовМетокВремени;
			
			Массив = Новый Массив;
			
			Для Каждого АдресСервера Из АдресаСерверовМетокВремени Цикл
				
				СтруктураСсылки = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресСервера);
				Массив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<a href = ""%1"">%1</a>'"), АдресСервера));
				
			КонецЦикла;
			
			Решение.Добавить(СтроковыеФункции.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Проверьте доступность серверов меток времени на компьютере: %1.'"),
					СтрСоединить(Массив, ", "))));
			
		Иначе
			Решение.Добавить(СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru='<a href = ""%1"">Проверьте доступность</a> серверов меток времени на компьютере.'"),
					"ДиагностикаСерверовМетокВремениКлиент"));
		КонецЕсли;

	КонецЕсли;

	Если Решение.Количество() > 0 Тогда
		ОшибкаПоКлассификатору.Решение = Новый ФорматированнаяСтрока(Решение);
	КонецЕсли;
	
	Если Причина.Количество() > 0 Тогда
		ОшибкаПоКлассификатору.Причина = Новый ФорматированнаяСтрока(Причина);
	КонецЕсли;
...
Функция ИздательСертификата(ДанныеСертификата)
...
Процедура УстановитьСписокОтзываСертификата(Сертификат, Адреса = Неопределено, АдресВнутренний = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Адреса) Тогда
		Если ТипЗнч(Адреса) = Тип("Строка") Тогда
			Адреса = СтрРазделить(Адреса, ";");
		КонецЕсли;
	Иначе
		Адреса = Новый Массив;
	КонецЕсли;
	
	АдресаСписковОтзыва = АдресаСписковОтзыва(Сертификат, ОбъектКомпоненты);
	Если ТипЗнч(АдресаСписковОтзыва) = Тип("Строка") И Не ЗначениеЗаполнено(Адреса) Тогда
		ВызватьИсключение АдресаСписковОтзыва;
	КонецЕсли;
	...
	Результат = СкачатьФайлСпискаОтзываНаСервере(Адреса, АдресВнутренний);
	...
	СвойстваСпискаОтзыва = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСпискаОтзыва(ДанныеФайла);
	
	Если СвойстваСпискаОтзыва = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл %1 не является списком отзыва.'"), Результат.АдресФайла);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СвойстваСпискаОтзыва.ДатаОкончания) И СвойстваСпискаОтзыва.ДатаОкончания < ТекущаяУниверсальнаяДата() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Список отзыва %1 просрочен. Обратитесь в удостоверяющий центр, выдавший сертификат.'"), Результат.АдресФайла);
	КонецЕсли;
...
Функция АдресаСписковОтзыва(Сертификат, ОбъектКомпоненты)
...
// Добавляет роль ЧтениеСертификатовКлючейЭлектроннойПодписиИШифрования во все профили, в которые входят
// роли ДобавлениеИзменениеЭлектронныхПодписей и ШифрованиеИРасшифровкаДанных.
//
Процедура ДобавитьРольЧтениеСертификатовКлючейЭлектроннойПодписиИШифрования() Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		Возврат;
	КонецЕсли;
	
	ИмяДобавляемойРоли = Метаданные.Роли.ЧтениеСертификатовКлючейЭлектроннойПодписиИШифрования.Имя;
	ИмяРолиДобавлениеИзменениеЭлектронныхПодписей = Метаданные.Роли.ДобавлениеИзменениеЭлектронныхПодписей.Имя;
	ИмяРолиШифрованиеИРасшифровкаДанных = Метаданные.Роли.ШифрованиеИРасшифровкаДанных.Имя;
	ИмяРолиРасшифровкаДанных = Метаданные.Роли.РасшифровкаДанных.Имя;
	
	НовыеРолиКДобавлениеИзменениеЭлектронныхПодписей = Новый Массив;
	НовыеРолиКДобавлениеИзменениеЭлектронныхПодписей.Добавить(ИмяДобавляемойРоли);
	НовыеРолиКДобавлениеИзменениеЭлектронныхПодписей.Добавить(ИмяРолиДобавлениеИзменениеЭлектронныхПодписей);
	
	НовыеРолиКШифрованиеИРасшифровкаДанных = Новый Массив;
	НовыеРолиКШифрованиеИРасшифровкаДанных.Добавить(ИмяДобавляемойРоли);
	НовыеРолиКШифрованиеИРасшифровкаДанных.Добавить(ИмяРолиШифрованиеИРасшифровкаДанных);
	
	НовыеРолиКРасшифровкаДанных = Новый Массив;
	НовыеРолиКРасшифровкаДанных.Добавить(ИмяДобавляемойРоли);
	НовыеРолиКРасшифровкаДанных.Добавить(ИмяРолиРасшифровкаДанных);
	
	ЗаменяемыеРоли = Новый Соответствие;
	ЗаменяемыеРоли.Вставить(ИмяРолиДобавлениеИзменениеЭлектронныхПодписей, НовыеРолиКДобавлениеИзменениеЭлектронныхПодписей);
	ЗаменяемыеРоли.Вставить(ИмяРолиШифрованиеИРасшифровкаДанных, НовыеРолиКШифрованиеИРасшифровкаДанных);
	ЗаменяемыеРоли.Вставить(ИмяРолиРасшифровкаДанных, НовыеРолиКРасшифровкаДанных);
...
Функция РасширенныеДанныеДляПечатиСертификата(Данные) Экспорт
	...
	Структура = Новый Структура("ТипИдентификации, СКЗИ, Заключение, СКЗИ_УЦ, ЗаключениеУЦ, Подпись, ИспользованиеКлюча");
				...
				Пока АнализДанных.Смещение < СмещениеСледующего Цикл
					...
					КонецЕсли; 
						...
						ИначеЕсли СтрокаБуфера = "2A85036472" Тогда // 1.2.643.100.114
							...
							АнализДанных.Родители.Удалить(0); // OCTET STRING
						ИначеЕсли СтрокаБуфера = "551D0F" Тогда // 2.5.29.15 keyUsage
							ЭлектроннаяПодписьСлужебныйКлиентСервер.ПропуститьБлок(АнализДанных, 0, 1, Ложь); // BOOLEAN
								ЭлектроннаяПодписьСлужебныйКлиентСервер.ПропуститьНачалоБлока(АнализДанных, 0, 4); // OCTET STRING
								РазмерДанных = АнализДанных.Родители[0].РазмерДанных;
								Структура.ИспользованиеКлюча = ПолучитьHexСтрокуИзБуфераДвоичныхДанных(
									АнализДанных.Буфер.Прочитать(АнализДанных.Смещение, РазмерДанных));
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаменитьРольДобавлениеИзменениеЭлектронныхПодписейИШифрование");
	Методы.Вставить("ЗаполнитьНастройкиДляУсовершенствованияПодписей");
	Методы.Вставить("ВключитьКонстантуРазрешитьДоступКИнтернетСервисам");
	Методы.Вставить("ЗаполнитьАдресаСерверовМетокВремени");
	Методы.Вставить("ДобавитьРольЧтениеСертификатовКлючейЭлектроннойПодписиИШифрования");
	Методы.Вставить("УсовершенствоватьНаСторонеСервера", Истина);
	Методы.Вставить("СкачатьФайлСпискаОтзываНаСервере", Истина);
