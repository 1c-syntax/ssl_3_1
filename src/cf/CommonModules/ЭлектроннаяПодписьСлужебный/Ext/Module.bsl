...
Процедура УстановитьСписокОтзываСертификата(Сертификат, Адреса = Неопределено, АдресВнутренний = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваСпискаОтзыва.ДатаСледующейПубликации) И СвойстваСпискаОтзыва.ДатаСледующейПубликации < ТекущаяУниверсальнаяДата() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Дата следующей публикации списка отзыва %1 просрочена. Обратитесь в удостоверяющий центр, выдавший сертификат.'"), Результат.АдресФайла);
	КонецЕсли;
