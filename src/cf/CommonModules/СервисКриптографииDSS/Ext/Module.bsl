...
Функция Зашифровать(ИдентификаторПользователя, 
	...
	Если СписокСертификатов.Количество() > 0 И СтруктураДокумента.ТипДокумента <> "Неопределенно" Тогда
		АдресПодключения	= НастройкиПользователя.Методы.Документы + "/encrypt";
...
Функция СертификатыРасшифрования(ИдентификаторПользователя, ДанныеДокумента, ТипШифрования = "CMS", ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	СтруктураДокумента		= СервисКриптографииDSSКлиентСервер.ПодготовитьДанныеДокумента(ДанныеДокумента);
	АдресПодключения 		= НастройкиПользователя.Методы.Документы + "/decrypt/parse";
...
Функция Расшифровать(
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Документы + "/decrypt";
...
Функция ХешированиеДанных(ИдентификаторПользователя, 
	...
	АдресПодключения 		= НастройкиПользователя.Методы.ХешДокумента;
...
Функция Подписать(ИдентификаторПользователя, 
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Документы;
	ЭтоТранзакция			= СвойствоСтруктуры(ПараметрыОперации, "Транзакция", Ложь);
	ЗначениеПинКода			= ПолучитьЗначениеПароля(ПинКод, НастройкиПользователя);
	...
	Если НЕ ЭтоТранзакция Тогда 
		...
		Для каждого СтрокаКлюча Из СвойстваПодписи Цикл
			...
			ИначеЕсли СтрокаКлюча.Ключ = "DocumentType" ИЛИ СтрокаКлюча.Ключ = "DocumentInfo" Тогда
				Продолжить;
			Иначе
				ПараметрыПодписи.Вставить(СтрокаКлюча.Ключ, СтрокаКлюча.Значение);
			КонецЕсли;	
		КонецЦикла;
...
Функция ПодписатьПакет(ИдентификаторПользователя,
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Документы + "/packagesignature";
	ЭтоТранзакция			= СвойствоСтруктуры(ПараметрыОперации, "Транзакция", Ложь);
	ЗначениеПинКода			= ПолучитьЗначениеПароля(ПинКод, НастройкиПользователя);
...
Функция ПроверитьСертификат(ИдентификаторПользователя, ДанныеСертификата, ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.ПроверкаСертификата;
...
Функция ПолучитьСведенияОПодписантах(ИдентификаторПользователя, ДанныеДокумента, ТипПодписи = "CMS", ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.ИнформацияПодписантах;
	СтруктураДокумента 		= СервисКриптографииDSSКлиентСервер.ПодготовитьДанныеДокумента(ДанныеДокумента);
...
Функция ПроверитьПодпись(ИдентификаторПользователя,
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.ПроверкаПодписи;
	ИдТиповПодписей			= СервисКриптографииDSSКлиентСервер.ПолучитьТипыПодписей();
	СтруктураДокумента 		= СервисКриптографииDSSКлиентСервер.ПодготовитьДанныеДокумента(ДанныеДокумента);
...
Функция АутентификацияПоЛогину(ИдентификаторПользователя, ПараметрыОперации = Неопределено) Экспорт
	...
	АдресПодключения 		= НастройкиПользователя.Методы.Авторизация;
	...
	Соединение 			= СервисКриптографииDSSСлужебный.ПодготовитьПодключение(СвойстваПодключения);
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		КонецЕсли;	
	Иначе
		РезультатЗапроса.Ошибка = РезультатЗапроса.Ошибка;
	КонецЕсли;	
	
	Если СохранитьНастройки Тогда
		СервисКриптографииDSSСлужебный.УстановитьНастройкиПользователя(НастройкиПользователя.Ссылка, НастройкиПользователя, ИсточникНастроек);
	КонецЕсли;
...
Функция АутентификацияПоСертификату(ИдентификаторПользователя, ПараметрыОперации = Неопределено) Экспорт
	...
	ВходящиеПараметры		= ПроверитьПараметрыОперации(ПараметрыОперации);
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ВходящиеПараметры);
	АдресПодключения 		= НастройкиПользователя.Методы.АвторизацияПоСертификату;
	...
	СвойстваПодключения	= СервисКриптографииDSSСлужебный.ПодготовитьСвойстваПодключения(НастройкиПользователя.Подключение, АдресПодключения);
	Соединение 			= СервисКриптографииDSSСлужебный.ПодготовитьПодключение(СвойстваПодключения, СертификатКлиента);
...
Функция ПолучитьАдресВебАутентификации(ИдентификаторПользователя, НеявныйТипРазрешения = Ложь, ПараметрыОперации = Неопределено) Экспорт
	...
	АдресПодключения	= НастройкиПользователя.Методы.Аутентификация;
	СвойстваПодключения	= СервисКриптографииDSSСлужебный.ПодготовитьСвойстваПодключения(НастройкиПользователя.Подключение, АдресПодключения);
...
Функция УстановитьСертификат(ИдентификаторПользователя, ДанныеСертификата, ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Сертификаты;
...
Функция ПолучитьСертификатыСервиса(ИдентификаторПользователя, ИдентификаторСертификата = Неопределено, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторСертификата) Тогда
		АдресПодключения = АдресПодключения + "/" + XMLСтрока(ИдентификаторСертификата);
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Для каждого СтрокаМассива Из МассивРезультата Цикл
			...
			ПарольСертификата	= СервисКриптографииDSSКлиентСервер.ПодготовитьОбъектПароля(
									ПаролиСертификатов[ОтпечатокСертификата], НастройкиПользователя.АвторизованныйПользователь);
...
Функция ВалидацияПинКодаСертификата(ИдентификаторПользователя, ИдентификаторСертификата, ПинКодСертификата = "", ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Сертификаты + "/%1/pin/check";
	АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, XMLСтрока(ИдентификаторСертификата));
	
	ПараметрыЗапроса	= Новый Структура;
	ПараметрыЗапроса.Вставить("Pin", ПолучитьЗначениеПароля(ПинКодСертификата, НастройкиПользователя));
...
Функция СменитьПинКодСертификата(ИдентификаторПользователя, ИдентификаторСертификата, СтарыйПинКод, НовыйПинКод, ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Сертификаты + "/%1/pin";
	АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, XMLСтрока(ИдентификаторСертификата));
	
	ПараметрыЗапроса		= Новый Структура;
	ПараметрыЗапроса.Вставить("OldPin", ПолучитьЗначениеПароля(СтарыйПинКод, НастройкиПользователя));
	ПараметрыЗапроса.Вставить("NewPin", ПолучитьЗначениеПароля(НовыйПинКод, НастройкиПользователя));
...
Функция НазначитьОсновнымСертификат(ИдентификаторПользователя, ИдентификаторСертификата, Установить = Истина, ПараметрыОперации = Неопределено) Экспорт
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Сертификаты + "/%1/default";
	АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, XMLСтрока(ИдентификаторСертификата));
...
Функция ПолучитьСвойстваСертификата(ИдентификаторПользователя, ИдентификаторСертификата, ПараметрыОперации = Неопределено) Экспорт
	...
	АдресПодключения 		= НастройкиПользователя.Методы.Сертификаты + "/%1/content?format=%2";
	АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, XMLСтрока(ИщемСертификат), 2);
	ПараметрыЗапроса		= Новый Структура;
...
Функция СоздатьЗапросНаСертификат(ИдентификаторПользователя,
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.ЗапросНаСертификат;
	ЭтоТранзакция			= СвойствоСтруктуры(ПараметрыОперации, "Транзакция", Ложь);
	ЗначениеПинКода			= ПолучитьЗначениеПароля(ПинКод, НастройкиПользователя);
	...
	ПараметрыЗапроса.Вставить("Parameters", Новый Структура("GroupId", КриптоПровайдер));
...
Функция ПолучитьЗапросНаСертификат(ИдентификаторПользователя, 
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.ЗапросНаСертификат;
	АдресПодключения		= АдресПодключения + "/" + Формат(ИдентификаторЗапроса, "ЧГ=0");
	ПараметрыЗапроса		= Новый Структура;
...
Функция ПолучитьПараметрыПолитики(НастройкиПользователя, ПараметрыОперации = Неопределено) Экспорт
	...
	АдресПодключения 		= НастройкиПользователя.Методы.ПолитикаПодписи;
	СвойстваПодключения		= СервисКриптографииDSSСлужебный.ПодготовитьСвойстваПодключения(НастройкиПользователя.Подключение, АдресПодключения);
...
Функция СоздатьТранзакцию(ИдентификаторПользователя, ДанныеЗапроса, КодОперации, ДанныеДокумента = Неопределено, ПараметрыОперации = Неопределено) Экспорт 
	...
	НастройкиПользователя	= ПолучитьНастройкиПользователя(ИдентификаторПользователя, ПараметрыОперации);
	АдресПодключения 		= НастройкиПользователя.Методы.Транзакции;
	...
	Если РезультатЗапроса.Выполнено Тогда
		РезультатЗапроса.Результат = РезультатЗапроса.Результат;
...
//   см. СервисКриптографииDSSКлиентСервер.ОтветСервисаПоУмолчанию
//
Функция ВыполнитьЗапросКСервису(НастройкиПользователя, АдресПодключения, ПараметрыЗапроса, HTTPМетод, ТипСодержания) Экспорт
...
Функция ПолучитьЗаписиАудита(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		АдресПодключения = НастройкиПользователя.Методы.Аудит_ЗапросОтчета;
...
Функция ЗагрузитьДокумент(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_Загрузить;
...
Функция ЗагрузитьПакетДокументов(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_ЗагрузитьПакет;
		Заголовки				= Новый Соответствие;
...
Функция ИнформацияОДокументеПоИдентификатору(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_Информация;
...
Функция ИнформацияОДокументеПоХешу(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_ИнформацияХеш;
...
Функция ПолучитьОбработанныйДокумент(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_Получить;
		АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, ИдентификаторДокумента);
		Заголовки				= Новый Соответствие;
...
Функция ИзменитьИнформациюОДокументе(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_ИзменитьИнформацию;
		АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, ИдентификаторДокумента);
		Заголовки				= Новый Соответствие;
...
Функция ПривязатьДокумент(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 		= НастройкиПользователя.Методы.Документы_Связанный;
		АдресПодключения		= ЗаполнитьПараметрыСтроки(АдресПодключения, ИдентификаторДокумента);
		Заголовки				= Новый Соответствие;
...
Функция СконвертироватьДокумент(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 	= НастройкиПользователя.Методы.Документы_Конвертация;
		АдресПодключения	= ЗаполнитьПараметрыСтроки(АдресПодключения, ИдентификаторДокумента);
		Заголовки			= Новый Соответствие;
...
Функция ПолучитьИнформациюДокументаКонвертации(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения 	= НастройкиПользователя.Методы.Документы_КонвертацияИнформация;
		АдресПодключения	= ЗаполнитьПараметрыСтроки(АдресПодключения, ИдентификаторДокумента);
		Заголовки			= Новый Соответствие;
		РезультатЗапроса	= ПодключитьВыполнитьМетодСервиса(НастройкиПользователя, АдресПодключения, Неопределено, "GET", "json", Заголовки, ПараметрыОперации);
...
Функция СоздатьЗакрытыйКлюч(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения	= НастройкиПользователя.Методы.ЗакрытыйКлюч + "/generate";
		РезультатЗапроса	= ПодключитьВыполнитьМетодСервиса(НастройкиПользователя, АдресПодключения, ПараметрыЗапроса, "POST", "json", , ПараметрыОперации);
		Если РезультатЗапроса.Выполнено Тогда
...
Функция УстановитьПараметрЗакрытогоКлюча(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		АдресПодключения	= НастройкиПользователя.Методы.ЗакрытыйКлюч + "/%1/params";
...
Функция ПолучитьПараметрЗакрытогоКлюча(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		АдресПодключения	= НастройкиПользователя.Методы.ЗакрытыйКлюч + "/%1/params?param=%2";
...
// Параметры:
//  ИдентификаторПользователя 	- Структура, СправочникСсылка.УчетныеЗаписиDSS - см. функцию НастройкиПользователяПоУмолчанию()
//  СвойствоПодписи				- Структура - изначально заполняется свойствами подписи, подробнее СервисКриптографииDSSКлиентСервер.ПолучитьСвойствоПодписиАсинхронной()
//  ИдентификаторСертификата 	- Число - идентификатор сертификата на сервере DSS
//  ПинКод 						- Строка - пин-код для доступа к закрытому ключу сертификата учетной записи
//  АсинхронныйРежим			- Булево - указывает, что подпись нужно сделать в асинхронном режиме
...
Функция ПодписатьАсинхронно(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		Если СвойствоПодписи <> Неопределено Тогда
			ПараметрыЗапроса = ОбщегоНазначения.СкопироватьРекурсивно(СвойствоПодписи);
			ПараметрыЗапроса.Signature.Вставить("CertificateId", ИдентификаторСертификата);
		...
		АдресПодключения	= НастройкиПользователя.Методы.ПодписьАсинхронная;
		РезультатЗапроса	= ПодключитьВыполнитьМетодСервиса(НастройкиПользователя, АдресПодключения, ПараметрыЗапроса, "POST", "json", , ПараметрыОперации);
		Если РезультатЗапроса.Выполнено Тогда
			РезультатОперации = Новый Структура;
			ОтветСервиса = РезультатЗапроса.Результат.Operation;
			СтатусОперации = ОтветСервиса.Status;
			Если НЕ СтатусОперации = "Error" Тогда
				РезультатОперации.Вставить("ИдентификаторОперации", СвойствоСтруктуры(ОтветСервиса, "ID", ""));
				Если СтатусОперации = "Completed" Тогда
					РезультатОперации.Вставить("Статус", "Завершено");
					МассивДокументов = Новый Массив;
					Для Каждого СтрокаОтвета Из ОтветСервиса.Result.ProcessedDocuments Цикл
				...
				Иначе	
					РезультатОперации.Вставить("Статус", "Выполняется");
				КонецЕсли;	
				РезультатОперации.Вставить("СрокДействия", ДатаUTC(СвойствоСтруктуры(ОтветСервиса, "ExpirationDate", 0)));
				
				РезультатЗапроса.Результат = РезультатОперации;
			Иначе	
				РезультатЗапроса.Выполнено = Ложь;
				РезультатЗапроса.Ошибка = СвойствоСтруктуры(ОтветСервиса, "ErrorDescription", "");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СведенияОбОперации(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		АдресПодключения	= ЗаполнитьПараметрыСтроки(НастройкиПользователя.Методы.Операции, ИдентификаторОперации);
		РезультатЗапроса	= ПодключитьВыполнитьМетодСервиса(НастройкиПользователя, АдресПодключения, Неопределено, "GET", "json", , ПараметрыОперации);
		Если РезультатЗапроса.Выполнено Тогда
			РезультатОперации = Новый Структура;
			ОтветСервиса = РезультатЗапроса.Результат.Operation;
...
// Посылает запрос на выполнение операции, ранее созданной на сервере облачной подписи.
//
// Параметры:
//  ИдентификаторПользователя 	- Структура, СправочникСсылка.УчетныеЗаписиDSS - см. функцию НастройкиПользователяПоУмолчанию()
//  ИдентификаторОперации 		- Строка - идентификатор операции ранее созданной на сервере
...
//   см. СервисКриптографииDSSКлиентСервер.ОтветСервисаПоУмолчанию
//
Функция ВыполнитьОперацию(ИдентификаторПользователя, 
					ИдентификаторОперации,
					ПараметрыОперации = Неопределено) Экспорт
	...
	Если РезультатЗапроса.Выполнено Тогда
		АдресПодключения	= НастройкиПользователя.Методы.Операции + "/start";
		АдресПодключения	= ЗаполнитьПараметрыСтроки(АдресПодключения, ИдентификаторОперации);
		РезультатЗапроса	= ПодключитьВыполнитьМетодСервиса(НастройкиПользователя, АдресПодключения, Неопределено, "POST", "json", , ПараметрыОперации);
	КонецЕсли;
...
Функция ИспользоватьСервисОблачнойПодписи() Экспорт
	...
	Результат = ПолучитьФункциональнуюОпцию("ИспользоватьСервисОблачнойПодписи");
...
Функция НайтиСертификат(ОписаниеСертификата, ПараметрыОперации = Неопределено) Экспорт
	...
	Если НашлиСертификат = Неопределено Тогда
		...
		СервисКриптографииDSSСлужебный.ПолучитьОписаниеОшибки(
				Результат,
				"ОшибкаВыполнения", 
				НСтр("ru = 'Сертификат не обнаружен в сервисе облачной подписи'", СервисКриптографииDSSСлужебный.КодЯзыка()));
	Иначе
		Результат.Вставить("ДанныеСертификата", ПолучитьОбъектСертификата(НашлиСертификат.ДанныеСертификата, Истина));
		Результат.Вставить("УчетнаяЗапись", НашлиСертификат.УчетнаяЗапись);
		Результат.Вставить("Идентификатор", НашлиСертификат.Идентификатор);
		Результат.Вставить("ДатаНачала", НашлиСертификат.ДатаНачала);
		Результат.Вставить("ДатаОкончания", НашлиСертификат.ДатаОкончания);
		Результат.Вставить("Наименование", НашлиСертификат.Наименование);
		Результат.Вставить("Отпечаток", НашлиСертификат.Отпечаток);
		Результат.Вставить("СерийныйНомер", НашлиСертификат.СерийныйНомер);
		Результат.Вставить("ИдентификаторСертификата", НашлиСертификат.ИдентификаторСертификата);
	КонецЕсли;
...
//      ** Ключ						- Строка
//      ** Значение					- Структура - подробнее СервисКриптографииDSSКлиентСервер.СвойстваСертификатаПользователя:
//        *** Содержимое			- Строка
...
Функция ДатаUTC(Знач ЧислоДаты, Добавка = 1970)
	...
	МассивМесяцев = СтрРазделить("31,28,31,30,31,30,31,31,30,31,30,31", ",", Ложь);
	
	ЗначениеГода = Цел(ЧислоДаты / 31436000) + Добавка;
	
	ВисокосныхГодов = Цел((ЗначениеГода - 1969) / 4);
	ВисокосныйГод = (Цел((ЗначениеГода - 1969) / 4) - ВисокосныхГодов) = 0;
	
	КоличествоДней = Цел(ЧислоДаты / 86400);
	КоличествоДней = КоличествоДней - ВисокосныхГодов;
	ДнейТекущегоГода = КоличествоДней % 365;
	...
	Для Счетчик = 0 По 11 Цикл
		ЗначениеМесяца = Счетчик + 1;
		ДнейМесяца = Число(МассивМесяцев[Счетчик]);
		ВсегоДней = ВсегоДней + ДнейМесяца;
		Если ВсегоДней > ДнейТекущегоГода Тогда
			ЗначениеДня = ДнейТекущегоГода - (ВсегоДней - ДнейМесяца) + 1;
			Прервать;
		КонецЕсли;	
	КонецЦикла;
	
	СекундыДня = ЧислоДаты % 86400;
	
	ЗначениеСекунд = СекундыДня % 60;
	ЗначениеЧасов = Цел(СекундыДня / 3600);
	ЗначениеМинут = Цел((СекундыДня - ЗначениеЧасов * 3600) / 60);
	
	Результат = Дата(ЗначениеГода, ЗначениеМесяца, ЗначениеДня, ЗначениеЧасов, ЗначениеМинут, ЗначениеСекунд);
...
//  Строка
//
Функция ПолучитьЗначениеПароля(ОбъектПароля, НастройкиПользователя)
