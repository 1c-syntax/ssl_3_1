...
Функция ПроверитьПодпись(ИдентификаторПользователя,
	...
	Если Не ПодписьПрисоединенная И Не ЗначениеЗаполнено(ИсходныйДокумент) Тогда
		РезультатЗапроса = СервисКриптографииDSSКлиентСервер.ОтветСервисаПоУмолчанию(Ложь);
		
		ТекстОшибки = НСтр("ru = 'Запрошена проверка открепленной CMS подписи, но не передан исходный документ'");
		СервисКриптографииDSSСлужебный.ПолучитьОписаниеОшибки(РезультатЗапроса, "content_required", ТекстОшибки);
	Иначе	
		РезультатЗапроса = ПодключитьВыполнитьМетодСервиса(НастройкиПользователя, АдресПодключения, ПараметрыЗапроса, "POST", "json", , НовыеПараметры);
	КонецЕсли;
...
Функция АутентификацияПоЛогину(ИдентификаторПользователя, ПараметрыОперации = Неопределено) Экспорт
	...
	СекретСервисногоКлиента = СервисКриптографииDSSСлужебный.ПолучитьЗакрытыеДанные(КлючСервисногоКлиента(НастройкиПользователя),
			"СекретСервисногоКлиента", Истина).Значение;
	
	Если НастройкиПользователя.ПервичнаяАутентификация = Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент Тогда
		ПараметрыЗапроса.Вставить("password", "");
		ЗаголовкиЗапроса.Вставить("Authorization", 
			ПолучитьПредставлениеСекрета(НастройкиПользователя.Подключение.СервисныйКлиент, СекретСервисногоКлиента));
	ИначеЕсли ПустаяСтрока(СекретСервисногоКлиента) ИЛИ ПустаяСтрока(НастройкиПользователя.Подключение.СервисныйКлиент) Тогда
	...
	Иначе
		ПараметрыЗапроса.Вставить("client_id", НастройкиПользователя.Подключение.ИдентификаторКлиента);
		ПараметрыЗапроса.Вставить("client_secret", СекретСервисногоКлиента);
		ПараметрыЗапроса.Вставить("password", ПолучитьЗначениеПароля(НастройкиПользователя.Пароль, НастройкиПользователя));
	КонецЕсли;
	...
	РезультатЗапроса	= СервисКриптографииDSSСлужебный.ВыполнитьМетодСервиса(Соединение, СвойстваПодключения.АдресПодключения, ПараметрыЗапроса, "POST", ЗаголовкиЗапроса, , ПараметрыОперации);
...
Функция АутентификацияПоСертификату(ИдентификаторПользователя, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ПустаяСтрока(СекретСервисногоКлиента) ИЛИ ПустаяСтрока(НастройкиПользователя.Подключение.СервисныйКлиент) Тогда
		ПараметрыЗапроса.Вставить("client_id", НастройкиПользователя.Подключение.ИдентификаторКлиента);
	Иначе
		ПараметрыЗапроса.Вставить("client_id", НастройкиПользователя.Подключение.ИдентификаторКлиента);
		ПараметрыЗапроса.Вставить("client_secret", СекретСервисногоКлиента);
	...
	РезультатЗапроса	= СервисКриптографииDSSСлужебный.ВыполнитьМетодСервиса(Соединение, СвойстваПодключения.АдресПодключения, ПараметрыЗапроса, "GET", ЗаголовкиЗапроса, , ПараметрыОперации);
...
Функция ПроверитьМаркерДоступа(НастройкиПользователя, РезультатЗапроса, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ВРег(ДанныеТокена.Логин) = ВРег(НастройкиПользователя.Логин) Тогда
		...
		ТокенАвторизации.ТокенОбновления= СервисКриптографииDSSКлиентСервер.ОбъектПароля(ТокенОбновления, 
											?(ЗначениеЗаполнено(ТокенОбновления), 0, 3));
...
Функция ПолучитьСертификатыСервиса(ИдентификаторПользователя, ИдентификаторСертификата = Неопределено, ПараметрыОперации = Неопределено) Экспорт
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Для каждого СтрокаМассива Из МассивРезультата Цикл
			...
			МобильныйЗакрытыКлюч= СвойствоСтруктуры(СтрокаМассива, "Storage", "") = "Mobile";
			...
			НоваяСтрока.МобильныйЗакрытыйКлюч = МобильныйЗакрытыКлюч;
...
Функция СменитьПинКодСертификата(ИдентификаторПользователя, ИдентификаторСертификата, СтарыйПинКод, НовыйПинКод, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ВерсияПодтверждения = 2 И РезультатЗапроса.Выполнено Тогда
		...
		Если НЕ Выжимка.Выполнено Тогда
			...
			РезультатЗапроса.СтатусОшибки.Представление = Выжимка.Ошибка;
		КонецЕсли;
...
Функция ОбновитьПолитикуПользователя(ИдентификаторПользователя, Принудительно = Ложь, ПараметрыОперации = Неопределено) Экспорт
	...
	ПодтверждатьОперации = НастройкиПользователя.ПервичнаяАутентификация = Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент;
 	...
 	Если НЕ НастройкиПользователя.Политика.Заполнено ИЛИ Принудительно Тогда
		...
		Если РезультатЗапроса.Выполнено Тогда
			СервисКриптографииDSSСлужебный.НастройкиРаботыАнализПолитикиПользователя(НастройкиПользователя.Политика, РезультатЗапроса.Результат, ПодтверждатьОперации);
		КонецЕсли;	
...
Функция ПолучитьОписаниеСервиса(АдресСервера, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ПараметрыОперации = Неопределено Тогда
		СвойстваПодключения.Вставить("ЗащищенноеСоединениеГОСТ", Ложь);
	Иначе
		СвойстваПодключения.Вставить("ЗащищенноеСоединениеГОСТ", ПараметрыОперации.ЗащищенноеСоединениеГОСТ);
	КонецЕсли;
...
Функция СостояниеУчетнойЗаписи(НастройкиПользователя, ПараметрыОперации = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.СтатусОшибки.Представление = НСтр("ru = 'Не удалось сформировать отчет'", СервисКриптографииDSSСлужебный.КодЯзыка());
		Результат.Ошибка = Результат.СтатусОшибки.Представление;
	КонецЕсли;	
...
Функция СоздатьЗакрытыйКлюч(ИдентификаторПользователя, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Если РезультатЗапроса.Выполнено Тогда
			...
			Иначе	
				РезультатЗапроса.Выполнено = Ложь;
				РезультатЗапроса.СтатусОшибки.Представление = СвойствоСтруктуры(РезультатЗапроса.Результат, "ErrorDescription", "");
				РезультатЗапроса.Ошибка = РезультатЗапроса.СтатусОшибки.Представление;
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Если РезультатЗапроса.Выполнено Тогда
			...
			ИначеЕсли НЕ РезультатОперации.ТребуетПодтверждения Тогда
				МассивИдентификаторов = Новый СписокЗначений;
				МассивСодержимого = Новый СписокЗначений;
				Для Каждого СтрокаОтвета Из Выжимка.Действия Цикл
					...
					ИмяДокумента = СвойствоСтруктуры(СтрокаОтвета, "OriginalContentInfo", "");
					Позиция = СтрНайти(ИмяДокумента, " #", НаправлениеПоиска.СКонца);
					Если Позиция > 0 Тогда
						ИмяДокумента = Сред(ИмяДокумента, Позиция + 2);
						ЕстьСортировка = ЕстьСортировка ИЛИ ЗначениеЗаполнено(ИмяДокумента);
					КонецЕсли;
					Если ЗначениеЗаполнено(ИдентификаторДокумента) Тогда
						МассивИдентификаторов.Добавить(ИдентификаторДокумента, ИмяДокумента);
					ИначеЕсли ЗначениеЗаполнено(СодержимоеДокумента) Тогда
						МассивСодержимого.Добавить(СодержимоеДокумента, ИмяДокумента);
					КонецЕсли;
				КонецЦикла;
				
				Если ЕстьСортировка Тогда
					МассивИдентификаторов.СортироватьПоПредставлению(НаправлениеСортировки.Возр);
					МассивСодержимого.СортироватьПоПредставлению(НаправлениеСортировки.Возр);
				КонецЕсли;
				
				РезультатОперации.Вставить("ИдентификаторыДокументов", МассивИдентификаторов.ВыгрузитьЗначения());
				РезультатОперации.Вставить("СодержимоеДокументов", МассивСодержимого.ВыгрузитьЗначения());
			Иначе
...
//    * Подключение					- Структура - данные для подключения к серверу, см. НастройкиРаботыПодключение
//    * Методы						- Структура - данные для вызова операций на сервере, см. НастройкиРаботыТочкиПодключения
...
Функция ПолучитьОписаниеСервисаDSS() Экспорт
	...
	Результат.Вставить("СервисныйКлиент", "");
...
Функция ПодключитьВыполнитьМетодСервиса(
	...
	Для Счетчик = 1 По 2 Цикл
		...
		Если РезультатЗапроса.Выполнено ИЛИ НЕ СервисКриптографииDSSКлиентСервер.ЭтоОшибкаАутентификации(РезультатЗапроса.СтатусОшибки) Тогда
			Прервать;
		КонецЕсли;	
	КонецЦикла;
...
Функция ПроверитьТокенАутентификации(НастройкиПользователя, Принудительно, ПараметрыОперации = Неопределено)
	...
	Если НЕ Авторизован ИЛИ Принудительно Тогда
		...
		Если ВидАутентификации = Перечисления.СпособыАвторизацииDSS.Первичный_УчетныеДанные // основной сценарий
			ИЛИ ВидАутентификации = Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент
...
Функция ПолучитьПредставлениеСекрета(НастройкиПользователя, ПарольКлиента = "")
	...
	Если ТипЗнч(НастройкиПользователя) = Тип("Строка") Тогда
		Владелец = НастройкиПользователя;
	Иначе
		Владелец = НастройкиПользователя.Логин;
	КонецЕсли;
	
	ИсходныеДанные 	= ПолучитьДвоичныеДанныеИзСтроки(Владелец 
						+ ":" 
						+ ПарольКлиента, 
						"UTF-8");
...
// Формирует путь хранения в безопасном хранилище для сервисного клиента.
//
// Параметры:
//  НастройкиПользователя 	- Структура
//
...
//  Строка
//
Функция КлючСервисногоКлиента(НастройкиПользователя) Экспорт
	
	Возврат СтрЗаменить(
		НастройкиПользователя.Методы.СервисИдентификации + НастройкиПользователя.Подключение.СервисныйКлиент, "/", "");
