...
//                         в случае, если получатель один и не указан Заголовок, иначе будет создано групповое обсуждение.
//                      - Строка - ключ группового обсуждения, если не найдено, будет создано. Если найдено, в чат будут
//                           добавлены недостающие получатели, рекомендуется при формировании ключа использовать GUID. 
//                         
//   Заголовок - Строка - заголовок группового обсуждения при создании.
//
// Пример:
...
//
Процедура ОтправитьСообщение(Знач Автор, Знач Получатели, Сообщение, ОбсуждениеКонтекст = Неопределено, Заголовок = Неопределено) Экспорт
	...
	Если Получатели.Количество() > 0 И ТипЗнч(Получатели[0]) = Тип("СправочникСсылка.Пользователи") Тогда
	...
	Если ОбсуждениеКонтекст <> Неопределено 
			И ТипЗнч(ОбсуждениеКонтекст) <> Тип("ИдентификаторОбсужденияСистемыВзаимодействия")
			И ТипЗнч(ОбсуждениеКонтекст) <> Тип("Строка") Тогда
	...
	ИначеЕсли ОбсуждениеКонтекст = Неопределено Или ТипЗнч(ОбсуждениеКонтекст) = Тип("Строка") Тогда
		
		Если Получатели.Количество() = 1 
			И Не ЗначениеЗаполнено(Заголовок)
			И Не ЗначениеЗаполнено(ОбсуждениеКонтекст) Тогда
			Участник = Получатели[0];
			Обсуждение = НеГрупповоеОбсуждениеМеждуПользователями(Автор.Идентификатор, Участник.Идентификатор);
		Иначе
			Обсуждение = ГрупповоеОбсуждениеМеждуПользователями(Автор.Идентификатор, Получатели, Заголовок, Сообщение.Текст, ОбсуждениеКонтекст);
		КонецЕсли;
...
//   * ПанельКнопок - Структура - см. синтакс-помощник СообщениеСистемыВзаимодействия:
//     ** ВидКнопок - Неопределено, ВидКнопкиПанелиКнопокСообщенияСистемыВзаимодействия - Неопределено на платформе
//           ниже 8.3.25.
//     ** РядыКнопок - Массив из Массив Из см. ОписаниеКнопки
//   * Действия - СписокЗначений - устарел, следует использовать ПанельКнопок.
//
...
Функция ОписаниеСообщения(Знач Текст) Экспорт
	...
	Описание.Вставить("ПанельКнопок", Новый Структура("ВидКнопок, РядыКнопок",
...
// Формирует описание кнопки панели кнопок для отправки сообщения через процедуры
// и функции подсистемы Обсуждения.
//
// Параметры:
//   Действие - ДействиеКнопкиПанелиКнопокСообщенияСистемыВзаимодействия - действие кнопки.
//   Текст - Строка - содержит надпись на кнопке.
// 
// Возвращаемое значение:
//   Структура:
//   * Данные - Произвольный
...
//   * Доступность - Булево
//   * ИмяДействия - Строка
//   * Картинка - Неопределено, Картинка
...
//@skip-check doc-comment-type
//
Функция ОписаниеКнопки(Действие, Текст) Экспорт
	Если ТипЗнч(Текст) = Тип("Строка") Тогда
		Текст = Новый ФорматированнаяСтрока(Текст);
	КонецЕсли;
	...
	Описание.Вставить("Доступность", Истина);
	Описание.Вставить("ИмяДействия", "");
	Описание.Вставить("Картинка", Неопределено);
	Описание.Вставить("НавигационнаяСсылка", "");
	Описание.Вставить("Текст", Текст);
	Описание.Вставить("ТекстСообщения", "");
...
Функция ВидКнопкиПанелиКнопокСообщенияСистемыВзаимодействия()
	
	Если ДоступнаПанельКнопок() Тогда
		Возврат ОбщегоНазначения.ВычислитьВБезопасномРежиме("ВидКнопкиПанелиКнопокСообщенияСистемыВзаимодействия.ОбычнаяКнопка");
	Иначе
...
//    ОбсуждениеСистемыВзаимодействия
//
Функция ГрупповоеОбсуждениеМеждуПользователями(Автор, Получатели, Заголовок, ТекстСообщения, КлючОбсуждения = Неопределено)
	Обсуждение = Неопределено;
	
	Если ЗначениеЗаполнено(КлючОбсуждения) Тогда
		Отбор = Новый ОтборОбсужденийСистемыВзаимодействия;
		Отбор.КонтекстноеОбсуждение = Ложь;
		Отбор.Групповое = Истина;
		Отбор.Ключ = КлючОбсуждения;
		НайденныеОбсуждения = СистемаВзаимодействия.ПолучитьОбсуждения(Отбор);
		Если НайденныеОбсуждения.Количество() > 0 Тогда
			Обсуждение = НайденныеОбсуждения[0];
			ПолучателиДляДобавления = Новый Массив;
			Если Не Обсуждение.Участники.Содержит(Автор) Тогда
				ПолучателиДляДобавления.Добавить(Автор);
			КонецЕсли;
			Для каждого Получатель Из Получатели Цикл
				Если ТипЗнч(Получатель) = Тип("ИдентификаторПользователяСистемыВзаимодействия")
					И Не Обсуждение.Участники.Содержит(Получатель) Тогда
					ПолучателиДляДобавления.Добавить(Получатель);
				ИначеЕсли ТипЗнч(Получатель) = Тип("ПользовательСистемыВзаимодействия")
					И Не Обсуждение.Участники.Содержит(Получатель.Идентификатор) Тогда
					ПолучателиДляДобавления.Добавить(Получатель.Идентификатор);
				КонецЕсли;
			КонецЦикла;
			Если ПолучателиДляДобавления.Количество() > 0 Тогда
				ДобавитьПолучателей(Обсуждение.Участники, ПолучателиДляДобавления);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Обсуждение = Неопределено Тогда
		...
		КонецЕсли;
		Обсуждение.Заголовок = ?(ЗначениеЗаполнено(Заголовок), Заголовок, ТекстСообщения);
		Обсуждение.Участники.Добавить(Автор);
...
Функция СообщениеИзОписания(Автор, ИдентификаторОбсуждения, Получатели, Сообщение)
	...
	Если Сообщение.ПанельКнопок.ВидКнопок <> Неопределено
		И ДоступнаПанельКнопок() И Сообщение.ПанельКнопок.РядыКнопок.Количество() > 0 Тогда
		
		СообщениеСВ.ПанельКнопок.ВидКнопок = Сообщение.ПанельКнопок.ВидКнопок;

		Для каждого РядКнопок Из Сообщение.ПанельКнопок.РядыКнопок Цикл
			ТекущийРяд = СообщениеСВ.ПанельКнопок.РядыКнопок.Добавить();
			Для каждого Кнопка Из РядКнопок Цикл
				ТекущаяКнопка = ТекущийРяд.Добавить(Кнопка.Действие, Кнопка.Текст);
				ТекущаяКнопка.Данные = Кнопка.Данные;
				ТекущаяКнопка.Доступность = Кнопка.Доступность;
				ТекущаяКнопка.ИмяДействия = Кнопка.ИмяДействия;
				Если ТипЗнч(Кнопка.Картинка) = Тип("Картинка") Тогда
					ТекущаяКнопка.Картинка = Кнопка.Картинка;
				КонецЕсли;
				ТекущаяКнопка.НавигационнаяСсылка = Кнопка.НавигационнаяСсылка;
				ТекущаяКнопка.ТекстСообщения = Кнопка.ТекстСообщения;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
