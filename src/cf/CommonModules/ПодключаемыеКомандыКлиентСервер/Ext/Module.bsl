...
Функция УсловияВыполняются(Условия, ЗначенияРеквизитов) Экспорт
...
Процедура ОбновитьКомандыИсточника(Знач Форма, Знач Источник, Знач ИмяИсточника = "")
	...
	ПриОбновленииКомандСПометкой(Форма, Источник, ПараметрыКлиента.КомандыСПометкой);
	...
	Для каждого ОписаниеКоманды Из ПараметрыКлиента.КомандыСПометкой Цикл
		...
		Если ЗначениеЗаполнено(ОписаниеКоманды.ЗначениеПометки) Тогда
			Если ТипЗнч(Источник) = Тип("ТаблицаФормы") Тогда
				ПутьДоРеквизитаЗначениеПометки = СтрЗаменить(ВРег(ОписаниеКоманды.ЗначениеПометки), "%ИСТОЧНИК%", Источник.Имя);
			Иначе
				ПутьДоРеквизитаЗначениеПометки = СтрЗаменить(ВРег(ОписаниеКоманды.ЗначениеПометки), "." + "%ИСТОЧНИК%", "");
			КонецЕсли;
			
			ЧастиПутиЗначениеПометки = СтрРазделить(ПутьДоРеквизитаЗначениеПометки, ".");
			ЗначениеПометки = Форма;
			Для Каждого Часть Из ЧастиПутиЗначениеПометки Цикл
				ЗначениеПометки = ЗначениеПометки[Часть];
			КонецЦикла;
			Форма.Элементы[ОписаниеКоманды.ИмяВФорме].Пометка = ЗначениеПометки;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого КраткиеСведенияОПодменю Из ПараметрыКлиента.ПодменюСУсловиямиВидимости Цикл
		...
		ЕстьКомандыНаКоманднойПанели = Ложь;
		...
		Для Каждого Команда Из КраткиеСведенияОПодменю.КомандыСУсловиямиВидимости Цикл
			...
			ТолькоВоВсехДействиях = КомандаЭлемент.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
			ЕстьКомандыНаКоманднойПанели = ЕстьКомандыНаКоманднойПанели Или Не ТолькоВоВсехДействиях;
		КонецЦикла;
		...
		Если Не КраткиеСведенияОПодменю.ЕстьКомандыБезУсловийВидимости Тогда
			...
			Если КомандаЗаглушка <> Неопределено Тогда
				КомандаЗаглушка.Видимость = Не ЕстьВидимыеКоманды И ЕстьКомандыНаКоманднойПанели;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ПриОбновленииКомандСПометкой(Форма, Источник, КомандыСПометкой)
	
	Если КомандыСПометкой.Количество() = 0 Тогда
		Возврат
	КонецЕсли;
	
	ТипИсточника = ТипЗнч(Источник);
	
	СсылкаОбъектаФормы = Неопределено;
	Если ТипИсточника = Тип("ДанныеФормыСтруктура") Тогда
		СсылкаОбъектаФормы = Источник.Ссылка;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СсылкаОбъектаФормы) Тогда
		Результат = ПодключаемыеКомандыВызовСервера.ПометкиОсновныхКоманд(СсылкаОбъектаФормы, КомандыСПометкой);
