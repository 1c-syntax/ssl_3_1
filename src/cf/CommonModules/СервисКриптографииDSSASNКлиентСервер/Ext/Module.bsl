...
//    * ВключитьИсходныеДанные 	- Булево - включает исходные данные в вид HEX, по умолчанию Ложь
...
Функция ЧтениеСтруктурыASNФайла(ДанныеASN, ПараметрыОперации = Неопределено) Экспорт
	...
	ОпределениеASN.Вставить("ВключитьИсходныеДанные", СвойствоСтруктуры(ПараметрыОперации, "ВключитьИсходныеДанные", Ложь));
	...
	Исключение
		ОтработатьВозникновениеОшибки(ОпределениеASN);
		Результат = Неопределено;
	КонецПопытки;	
...
Функция ПолучитьСвойстваСертификата(ДанныеФайла) Экспорт
	...
	Если ТретийТег = "BITSTRING" Тогда
		...
		Результат.ОткрытыйКлюч = Сред(ОткрытыйКлюч.Данные, 7);
	КонецЕсли;	
...
//	
Функция ПолучитьСвойстваПодписи(ДанныеФайла) Экспорт
		...
		Для Каждого СтрокаПодписи Из ДанныеПодписи Цикл
			...
			ИначеЕсли СтрокаПодписи.Тип = "OPTIONAL" Тогда
				...
				Если ХешДокумента <> Неопределено Тогда
					НоваяСтрока.АтрибутыПодписи = "31" + Сред(СтрокаПодписи.Данные, 3); // заменяем тег
	...
	Для Каждого СтрокаМассива Из ВеткаСертификатов Цикл
		...
		Если НоваяСтрока <> Неопределено Тогда
			НоваяСтрока.Сертификат = ПолучитьДвоичныеДанныеИзHexСтроки(СтрокаМассива.Данные);
			НоваяСтрока.ЗначениеОткрытогоКлюча = СвойствоСтруктуры(СертификатПодписанта, "[6].Значение[1].Значение", "");
...
Функция ПолучитьСледующийБайт(ПотокБайтов, ТекущийСчетчик)
...
Функция ПолучитьСледующиеБайты(ПотокБайтов, ТекущийСчетчик, Количество = 0, ПустыеБайты = Ложь)
...
Функция ПолучитьТекущийТег(ПотокБайтов, ТекущийСчетчик)
	
	ТекущийБайт = ПолучитьСледующийБайт(ПотокБайтов, ТекущийСчетчик);
...
Функция ПолучитьТекущуюДлину(ПотокБайтов, ТекущийСчетчик)
	...
	Если ПроверитьБит(ТекущийБайт, 7) Тогда
		...
		ИначеЕсли Количество > ПотокБайтов.Размер Тогда
			Результат = 0;
		Иначе	
			МассивБайтов = ПолучитьСледующиеБайты(ПотокБайтов, ТекущийСчетчик, Количество);
...
Функция Привести_Идентификатор(МассивБайтов)
	...
	Для Счетчик = 1 По МассивЗначений.Количество() Цикл
		МассивЗначений[Счетчик - 1] = Формат(МассивЗначений[Счетчик - 1], "ЧГ=0");
	КонецЦикла;
...
Функция Привести_СтрокаБитов(ОпределениеASN, МассивБайтов, ТипКодирования)
	
	Результат = "";
	
	Если ТипКодирования = 3 Тогда
		Если ОпределениеASN.ИсключитьСодержимое Тогда
			Результат = "";
		Иначе	
	...
	Иначе
		ТекущийСчетчик = 1;
		Пока МассивБайтов.Размер >= ТекущийСчетчик Цикл
			ЗначениеТега = ПолучитьТекущийТег(МассивБайтов, ТекущийСчетчик);
			Результат = Результат + ЗначениеТега.Значение;
		КонецЦикла;
	КонецЕсли;
...
Функция Привести_СтрокаБайтов(ОпределениеASN, МассивБайтов)
	...
	Иначе
		Попытка
			...
			ПрочитатьОчереднуюПорцию(ОпределениеASN, МассивБайтов, 1, Результат, Ложь);
		Исключение
			Удалось = Ложь;
...
Функция ПолучитьТекущееЗначение(ОпределениеASN, ПотокБайтов, ТекущийСчетчик, Длина, Тег, ПустыеБайты = Ложь)
	
	Результат		= Новый Структура("Значение, ИсходныеДанные");
	
	НовыйПоток	 	= ПолучитьСледующиеБайты(ПотокБайтов, ТекущийСчетчик, Длина, ПустыеБайты);
	...
	ИначеЕсли ОпределениеТипа = "OCTETSTRING" Тогда
		Значение = Привести_СтрокаБайтов(ОпределениеASN, НовыйПоток);
	ИначеЕсли ОпределениеТипа = "BITSTRING" Тогда
		Значение = Привести_СтрокаБитов(ОпределениеASN, НовыйПоток, Тег.Блок);
	...
	Если ОпределениеASN.ВключитьИсходныеДанные Тогда
		Результат.ИсходныеДанные = НовыйПоток;
	КонецЕсли;	
...
Функция ПолучитьДанныеВетки(ОпределениеASN, ПотокБайтов, НоваяСтрока)
	
	Результат = Неопределено;
	
	Если ПотокБайтов <> Неопределено И ОпределениеASN.ВключитьИсходныеДанные Тогда
		НоваяСтрока.Данные = ПолучитьHexСтрокуИзБуфераДвоичныхДанных(ПотокБайтов);
	КонецЕсли;
...
Функция ПрочитатьСоставнойТип(ОпределениеASN, ПотокБайтов, ТекущийСчетчик, ДеревоСтруктуры, ТекущаяДлина, Тег, ДлинаЗаголовка)
	...
	Если ОпределениеASN.ВключитьИсходныеДанные Тогда
		Если ДлинаЗаголовка > 0 Тогда
			ДанныеПотокаТега = ПотокБайтов.Прочитать(ТекущийСчетчик - 1 - ДлинаЗаголовка, ДлинаЗаголовка);
		Иначе	
			ДанныеПотокаТега = Новый БуферДвоичныхДанных(0);
		КонецЕсли;
	КонецЕсли;
	
	ТипТега = ПолучитьТипТега(ОпределениеASN, Тег);
	НовыйЭлемент = Новый Массив();
	НоваяСтрока = НоваяВетка(ОпределениеASN, ДеревоСтруктуры, ТипТега, НовыйЭлемент);
	
	Если ТекущаяДлина = -1 Тогда
		Результат = ПрочитатьОчереднуюПорцию(ОпределениеASN, ПотокБайтов, ТекущийСчетчик, НоваяСтрока);
	Иначе
		НовыйПоток	= ПолучитьСледующиеБайты(ПотокБайтов, ТекущийСчетчик, Длина);
		Результат = ПрочитатьОчереднуюПорцию(ОпределениеASN, НовыйПоток, 1, НоваяСтрока);
	КонецЕсли;
	
	Если ОпределениеASN.ВключитьИсходныеДанные Тогда
		
		Если ДлинаЗаголовка > 0 Тогда
			ДанныеПотокаТега = ДанныеПотокаТега.Соединить(Результат);
		КонецЕсли;
		ПолучитьДанныеВетки(ОпределениеASN, ДанныеПотокаТега, НоваяСтрока);
...
Функция ПрочитатьОчереднуюПорцию(ОпределениеASN, ПотокБайтов, ТекущийСчетчик, ДеревоСтруктуры, ДобавлятьЭлемент = Истина) // пытаем разобрать двоичный образ тега
	...
	Пока ПотокБайтов.Размер >= ТекущийСчетчик Цикл
		...
		ЗначениеТега	= ПолучитьТекущийТег(ПотокБайтов, ТекущийСчетчик);
		ТекущаяДлина	= ПолучитьТекущуюДлину(ПотокБайтов, ТекущийСчетчик);
		Длина			= ТекущаяДлина;
		ПустыеБайты		= Ложь;
		Если ТекущаяДлина = -1 Тогда
			Длина = 0;
		...
		Если ПолучитьТип(ОпределениеASN, ЗначениеТега) = "Primitive" Тогда
			...
			ЗначениеТега= ПолучитьТекущееЗначение(ОпределениеASN, ПотокБайтов, ТекущийСчетчик, Длина, ЗначениеТега, ПустыеБайты);
			Значение	= ЗначениеТега.Значение;
			НовыйПоток	= ЗначениеТега.ИсходныеДанные;
		...
		ИначеЕсли ПолучитьТип(ОпределениеASN, ЗначениеТега) = "Constructed" Тогда
			НовыйПоток = ПрочитатьСоставнойТип(ОпределениеASN, ПотокБайтов, ТекущийСчетчик, ДеревоСтруктуры, ТекущаяДлина, ЗначениеТега, ДлинаЗаголовка);
		...
		Если ОпределениеASN.ВключитьИсходныеДанные Тогда
			...
			Если ДлинаЗаголовка > 0 Тогда
				ДанныеПотокаТега = ДанныеПотокаТега.Соединить(ПотокБайтов.Прочитать(НачалоСчетчика - 1, ДлинаЗаголовка));
			КонецЕсли;
			
			ДанныеПотокаТега = ДанныеПотокаТега.Соединить(НовыйПоток);
			
			Если ТекущаяДлина = -1 Тогда
				ДанныеПотокаТега = ДанныеПотокаТега.Соединить(Новый БуферДвоичныхДанных(2));
			КонецЕсли;
			
			Результат = Результат.Соединить(ДанныеПотокаТега);
			
			Если ЗаполнитьВетку <> Неопределено Тогда
				ПолучитьДанныеВетки(ОпределениеASN, ДанныеПотокаТега, ЗаполнитьВетку);
			КонецЕсли;
...
Процедура ОтработатьВозникновениеОшибки(ОпределениеASN)
