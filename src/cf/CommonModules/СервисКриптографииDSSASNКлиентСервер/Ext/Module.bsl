...
// Функция определяет присоединенная или отсоединенная проверяемая электронная подпись.
//
// Параметры:
//  ДанныеФайла - ДвоичныеДанные - содержит криптографические данные
//
...
//  Булево
//
Функция ПодписьПрисоединенная(ДанныеФайла) Экспорт

	Результат = Ложь;
	
	СтруктураФайла = ЧтениеСтруктурыASNФайла(ДанныеФайла, Новый Структура("ИсключитьСодержимое", Истина));
	Если СтруктураФайла = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(НашлиЭлемент.Значение) = Тип("Массив")
		И НашлиЭлемент.Значение.Количество() = 1
		И НашлиЭлемент.Значение[0].Тип = "OCTETSTRING" Тогда
		Присоединенная = Истина;
	КонецЕсли;      
	
	Результат = Присоединенная;
