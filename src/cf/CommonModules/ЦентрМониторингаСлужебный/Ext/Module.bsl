...
Функция HTTPОтветВСтруктуру(Ответ)
	...
	HTTPЗаголовки = ЗаголовкиВНижнийРегистр(Ответ.Заголовки);
	...
	Иначе 
		...
		Если МассивСтрок.Количество() >= 2 Тогда
			...
			Если МассивПодстрок.Количество() >= 2 Тогда
				...
				Исключение
					РезультатЗапроса.Успешно = Ложь;
					РезультатЗапроса.ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				КонецПопытки;
			КонецЕсли;
		Иначе
			РезультатЗапроса.Успешно = Ложь;
	...
	Если Параметры.КодВыполнен Тогда		
		Попытка
			...
			Если ЗначениеЗаполнено(ОбщиеНастройки.ДополнительнаяИнформацияОтчета) Тогда
				...
				ЧтениеJSON = Новый ЧтениеJSON();
				ЧтениеJSON.УстановитьСтроку(ОбщиеНастройки.ДополнительнаяИнформацияОтчета);
	...
	Если Параметры.КодВыполнен Тогда		
		Попытка
			...
			Если ОбщиеНастройки.ВариантОтображенияСообщения = ПеречислениеВариантОтображенияСообщенияОбОшибке[СохраненныеПараметры.ВариантОтображенияСообщенияОбОшибке]
				...
				КодДляВыполнения = "СтрокаСообщения = Новый ФорматированнаяСтрока(НСтр(""ru = 'К сожалению, возникла непредвиденная ситуация'""), Новый Шрифт(,,,,,,140));
