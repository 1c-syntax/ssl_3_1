...
Процедура РассчитатьИтоги() Экспорт
	...
	УстанавливатьМинимальныйПериод = Ложь;
	МинимальныйПериодРассчитанныхИтоговРегистровНакопления = Дата(1,1,1);
	МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии = Дата(1,1,1);
	
	КоличествоМесяцевМинимальногоПериодаРассчитанныхИтогов =
		Константы.КоличествоМесяцевМинимальногоПериодаРассчитанныхИтогов.Получить();
	Если ЗначениеЗаполнено(КоличествоМесяцевМинимальногоПериодаРассчитанныхИтогов) Тогда
		УстанавливатьМинимальныйПериод = Истина;
		
		МинимальныйПериодРассчитанныхИтоговРегистровНакопления =
			НачалоМесяца(ДобавитьМесяц(ДатаСеанса, -(КоличествоМесяцевМинимальногоПериодаРассчитанныхИтогов - 1)));
		МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии =
			НачалоМесяца(ДобавитьМесяц(ДатаСеанса, -(КоличествоМесяцевМинимальногоПериодаРассчитанныхИтогов - 2)));
	КонецЕсли;
	...
	Для Каждого РегистрМетаданные Из Метаданные.РегистрыНакопления Цикл
		...
		МаксимальныйПериодРассчитанныхИтогов = РегистрНакопленияМенеджер.ПолучитьМаксимальныйПериодРассчитанныхИтогов();
		МинимальныйПериодРассчитанныхИтогов = РегистрНакопленияМенеджер.ПолучитьМинимальныйПериодРассчитанныхИтогов();
		Если МаксимальныйПериодРассчитанныхИтогов >= РегистрНакопленияПериод
			И (НЕ УстанавливатьМинимальныйПериод 
				ИЛИ МинимальныйПериодРассчитанныхИтогов = МинимальныйПериодРассчитанныхИтоговРегистровНакопления)Тогда
			Продолжить;
		КонецЕсли;
		Если МаксимальныйПериодРассчитанныхИтогов < РегистрНакопленияПериод
			И (УстанавливатьМинимальныйПериод
				И МинимальныйПериодРассчитанныхИтогов <> МинимальныйПериодРассчитанныхИтоговРегистровНакопления) Тогда
				РегистрНакопленияМенеджер.УстановитьМинимальныйИМаксимальныйПериодыРассчитанныхИтогов(
					МинимальныйПериодРассчитанныхИтоговРегистровНакопления, РегистрНакопленияПериод);
		ИначеЕсли МаксимальныйПериодРассчитанныхИтогов < РегистрНакопленияПериод Тогда
			РегистрНакопленияМенеджер.УстановитьМаксимальныйПериодРассчитанныхИтогов(РегистрНакопленияПериод);
		ИначеЕсли УстанавливатьМинимальныйПериод
			И МинимальныйПериодРассчитанныхИтогов <> МинимальныйПериодРассчитанныхИтоговРегистровНакопления Тогда
			РегистрНакопленияМенеджер.УстановитьМинимальныйПериодРассчитанныхИтогов(МинимальныйПериодРассчитанныхИтоговРегистровНакопления);
		КонецЕсли;
	...
	Для Каждого РегистрМетаданные Из Метаданные.РегистрыБухгалтерии Цикл
		...
		МаксимальныйПериодРассчитанныхИтогов = РегистрБухгалтерииМенеджер.ПолучитьМаксимальныйПериодРассчитанныхИтогов();
		МинимальныйПериодРассчитанныхИтогов = РегистрБухгалтерииМенеджер.ПолучитьМинимальныйПериодРассчитанныхИтогов();
		Если МаксимальныйПериодРассчитанныхИтогов >= РегистрБухгалтерииПериод
			И (НЕ УстанавливатьМинимальныйПериод 
				ИЛИ МинимальныйПериодРассчитанныхИтогов = МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии) Тогда
			Продолжить;
		КонецЕсли;
		Если МаксимальныйПериодРассчитанныхИтогов < РегистрБухгалтерииПериод
			И (УстанавливатьМинимальныйПериод
				И МинимальныйПериодРассчитанныхИтогов <> МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии) Тогда
				РегистрБухгалтерииМенеджер.УстановитьМинимальныйИМаксимальныйПериодыРассчитанныхИтогов(
					МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии, РегистрБухгалтерииПериод);
		ИначеЕсли МаксимальныйПериодРассчитанныхИтогов < РегистрБухгалтерииПериод Тогда
			РегистрБухгалтерииМенеджер.УстановитьМаксимальныйПериодРассчитанныхИтогов(РегистрБухгалтерииПериод);
		ИначеЕсли УстанавливатьМинимальныйПериод
			И МинимальныйПериодРассчитанныхИтогов <> МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии Тогда
			РегистрБухгалтерииМенеджер.УстановитьМинимальныйПериодРассчитанныхИтогов(МинимальныйПериодРассчитанныхИтоговРегистровБухгалтерии);
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru = 'Включает или отключает обновление и перестроение агрегатов по расписанию.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Прототип.Представление = НСтр("ru = 'Оптимизировать работу приложения'");
...
#Область ДляРаботыВФайловомРежиме
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновлениеАгрегатовОбработчикЗадания", Истина);
	Методы.Вставить("ПерестроениеАгрегатовОбработчикЗадания", Истина);
	Методы.Вставить("УстановкаПериодаРассчитанныхИтоговОбработчикЗадания", Истина);
