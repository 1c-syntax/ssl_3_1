...
Функция СведенияОбЯзыках() Экспорт
	...
	Результат.Вставить("КоличествоДополнительныхЯзыков", 2);
...
Функция СуффиксЯзыка(Язык) Экспорт
	...
	Если СтрСравнить(Язык, Константы.ДополнительныйЯзык1.Получить()) = 0 И ИспользуетсяДополнительныйЯзык("Язык1") Тогда
		Возврат "Язык1";
	КонецЕсли;
	
	Если СтрСравнить(Язык, Константы.ДополнительныйЯзык2.Получить()) = 0 И ИспользуетсяДополнительныйЯзык("Язык2") Тогда
		Возврат "Язык2";
	КонецЕсли;
...
Функция ИспользуетсяДополнительныйЯзык(СуффиксЯзыка) Экспорт
	...
	ИмяКонстанты = ?(СуффиксЯзыка <> "Язык1",
		СтрЗаменить("ИспользоватьДополнительныйЯзык1", "Язык1", СуффиксЯзыка),
		"ИспользоватьДополнительныйЯзык1");
	
	Возврат Константы[ИмяКонстанты].Получить() = Истина;
...
Функция ЕстьМультиязычныеРеквизитыВШапкеОбъекта(ОбъектМетаданныхПолноеИмя) Экспорт
	...
	Для каждого Колонка Из РезультатЗапроса.Колонки Цикл
		Если (СтрЗаканчиваетсяНа(Колонка.Имя, МультиязычностьСервер.СуффиксПервогоЯзыка())
		 Или СтрЗаканчиваетсяНа(Колонка.Имя, МультиязычностьСервер.СуффиксВторогоЯзыка()))
