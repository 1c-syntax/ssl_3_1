...
#Область ПолучениеИОтправкаПисем

// Проверяет наличие настроенных учетных записей электронной почты.
//
// Возвращаемое значение:
//  Булево - Истина, если есть хотя бы одна настроенная учетная запись.
//
Функция ЕстьНастроенныеУчетныеЗаписи()
	
	Возврат РаботаСПочтовымиСообщениями.ДоступныеУчетныеЗаписи(Истина, Истина, Истина).Количество() > 0;
...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И НЕ ЕстьНастроенныеУчетныеЗаписи() Тогда
		РегламентныеЗаданияСервер.УстановитьИспользованиеПредопределенногоРегламентногоЗадания(
			Метаданные.РегламентныеЗадания.ПолучениеИОтправкаЭлектронныхПисем, Ложь);
		ТекстСообщения = НСтр("ru = 'Регламентное задание ""Получение и отправка электронной почты"" отключено,
			|так как не настроены учетные записи электронной почты для работы в модели сервиса.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		РегламентныеЗаданияСервер.ОтменитьВыполнениеЗадания(
			Метаданные.РегламентныеЗадания.ПолучениеИОтправкаЭлектронныхПисем, ТекстСообщения);
...
Процедура ОтправитьПисьма(Запрос, ВсеПолученныеПисьма, ПисьмаДляОпределенияПапок, ОтправленоПисем = 0, ЕстьОшибки = Ложь, Интерактивно = Ложь)
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		УчетнаяЗаписьЗаблокирована = Истина;
		...
		Исключение
			СнятьБлокировкуУчетнойЗаписиДляПолучения(УчетнаяЗапись);
			УчетнаяЗаписьЗаблокирована = Ложь;
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонСообщенияОбОшибке,
				УчетнаяЗапись,
				РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(ИнформацияОбОшибке(), ОбщегоНазначения.КодОсновногоЯзыка(), , УчетнаяЗапись));
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщенияОбОшибке);
		...
		Если УчетнаяЗаписьЗаблокирована Тогда
			СнятьБлокировкуУчетнойЗаписиДляПолучения(УчетнаяЗапись);
			УчетнаяЗаписьЗаблокирована = Ложь;
		КонецЕсли;
...
Процедура ПолучитьПисьма(Знач ДанныеУчетнойЗаписи, ЕстьОшибки, Получено, ПолученныеПисьма)
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(
			ИнформацияОбОшибке(), ОбщегоНазначения.КодОсновногоЯзыка(), , ДанныеУчетнойЗаписи.Ссылка);
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Пока ИдентификаторыПисемКЗагрузке.Количество() > (ПолученоПисем + КоличествоОшибокПриЗаписи + КоличествоНеАктуальныхСообщений) Цикл
			...
			Исключение
				...
				ТекстОшибкиДляПользователя = РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(
					ИнформацияОбОшибке(), , Ложь, ДанныеУчетнойЗаписи.Ссылка);
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				РезультатЗаписи = РезультатЗаписиЭлектронногоПисьма(ДанныеУчетнойЗаписи, Сообщение, 
				...
				Если РезультатЗаписи.Успешно Тогда
					...
					ПолученныеПисьма.ВсеПолученныеПисьма.Добавить(РезультатЗаписи.СозданноеПисьмо);
					ПолученныеПисьма.ПолученныеПисьмаПоУчетнойЗаписи.Добавить(РезультатЗаписи.СозданноеПисьмо);
					Если ДобавлятьВМассивПисемДляПолученияПапки Тогда
						ПолученныеПисьма.ПисьмаДляОпределенияПапок.Добавить(РезультатЗаписи.СозданноеПисьмо);
					КонецЕсли; 
	...
	Для каждого ИмяАктивнойПапки Из ИменаАктивныхПапок Цикл
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(ИнформацияОбОшибке(),
					ОбщегоНазначения.КодОсновногоЯзыка(), Ложь, ДанныеУчетнойЗаписи.Ссылка));
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начало получения писем из папки ""%1"" учетной записи ""%2"". Писем к загрузке: %3'"),
		...
		Если ТаблицаИдентификаторов.Количество() > 0 Или ЗаголовкиСПустымИдентификатором.Количество() > 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Получено писем из папки ""%1"" учетной записи ""%2"": %3'"),
...
Функция РезультатЗаписиЭлектронногоПисьма(ДанныеУчетнойЗаписи, Сообщение, ОтветственныйЗаОбработкуПисем,
	ПомещатьПисьмоВПапкуПисьмаОснования, ДобавлятьВМассивПисемДляПолученияПапки, ЭтоИсходящееПисьмо);
	
	Результат = Новый Структура;
	Результат.Вставить("Успешно",         Ложь);
	Результат.Вставить("СозданноеПисьмо", Неопределено);
	...
	Для каждого ЭлементСоответствия Из ВложенияИПодписи Цикл
		...
		Если НайденноеВложение <> Неопределено Тогда
			ЗаписатьВложениеЭлектронногоПисьма(Письмо, НайденноеВложение, ПодписиФайла, КоличествоПустыхИменВоВложениях);
		КонецЕсли;
	...
	Результат.СозданноеПисьмо = Письмо.Ссылка;
...
Функция СтроковоеПолеЗаголовкаИзПисьма(Сообщение, ИмяЗаголовка)
	...
	НеобработанныеИдентификаторы = СтрРазделить(СтрокаИдентификаторы, ">");
	ИдентификаторыКВозврату      = "";
	
	НуженПереводСтроки = Ложь;
	
	Для Каждого НеобработанныйИдентификатор Из НеобработанныеИдентификаторы Цикл
		
		Если ПустаяСтрока(НеобработанныйИдентификатор) Тогда
			
			Продолжить;
			
		Иначе
			
			Идентификатор = СтрЗаменить(НеобработанныйИдентификатор, "<", ""); 
			...
			Если НуженПереводСтроки Тогда
				 ИдентификаторыКВозврату = ИдентификаторыКВозврату + Символы.ПС;
			КонецЕсли;
			
			ИдентификаторыКВозврату = ИдентификаторыКВозврату + Идентификатор;
	...
	Возврат ИдентификаторыКВозврату;
	...
	Если ТекстHTML <> "" Тогда
		...
		Иначе
			...
			Попытка
				...
				Письмо.Текст = ПолучитьПростойТекстИзHTML(ТекстHTML);
			
			Исключение
				
				Письмо.Текст = "";
				ШаблонСообщенияОбОшибке = НСтр("ru = 'При извлечении простого текста из письма %1 возникла ошибка.
					|Текст письма будет доступен только в формате HTML.
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка());
				
				ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонСообщенияОбОшибке, 
					Взаимодействия.ПредставлениеПисьма(Письмо.Тема, Письмо.Дата),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщенияОбОшибке);
...
Функция ИменаАктивныхПапок(Почта)
	...
	Если ИменаАктивныхПапок.Количество() = 0 Или ИменаАктивныхПапок.Найти("INBOX") = Неопределено Тогда
	...
	МассивИгнорируемыхИмен  = ИгнорируемыеИменаПапокПриЗагрузкеПисем();
...
Функция ИгнорируемыеИменаПапокПриЗагрузкеПисем()

	ИменаПапок = Новый Массив;
	ИменаПапок.Добавить("spam");
	ИменаПапок.Добавить("trash");
	ИменаПапок.Добавить("drafts");
	ИменаПапок.Добавить("draftBox");
	ИменаПапок.Добавить("deleted");
	ИменаПапок.Добавить("junk");
	ИменаПапок.Добавить("bulk mail");
...
#Область РаботаСВложенияЭлектронныхПисем
...
// Параметры:
//  ИмяПредопределеннойПапки - Строка
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыПредопределенныхПапокПисем
//
...
Функция ТекстЗапросаПоДоступнымУчетнымЗаписям()
	...
	|	УчетныеЗаписиЭлектроннойПочты.Ссылка КАК УчетнаяЗапись,
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(НастройкиУчетныхЗаписейЭлектроннойПочты.ВПредставлениеВключатьИмяПользователя, ЛОЖЬ)
	|			ТОГДА Пользователи.Наименование
	|		ИНАЧЕ УчетныеЗаписиЭлектроннойПочты.ИмяПользователя 
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтправитьЗагрузитьПочтуПользователя", Истина);
