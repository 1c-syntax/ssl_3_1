...
Функция ОбработатьОшибкуОтправкиПисьма(Знач ПисьмоОбъект, Знач ВыборкаПисьма, Знач ПараметрыПисьма, Знач Интерактивно)
	...
	Если ЭтоПроблемаОтвергнутыхСерверомПочтовыхАдресов И Не ВсеАдресатыПисьмаОтвергнутыСервером Тогда
		ШаблонСообщенияОбОшибке = НСтр("ru = 'Следующие адресаты электронного письма %1 не приняты почтовым сервером:
	...
	Иначе
		ШаблонСообщенияОбОшибке = НСтр("ru = 'Во время отправки электронного письма %1 произошла ошибка
...
Процедура ОтправитьУведомленияОПрочтении(ДляТекущегоПользователя)
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Пока ВыборкаПисьма.Следующий() Цикл
			...
			СсылкиПисем.Добавить(ВыборкаПисьма.Письмо);
		КонецЦикла;
		
		РаботаСПочтовымиСообщениями.ОтправитьПисьма(УчетнаяЗапись, Письма);
		
		Для Каждого Ссылка Из СсылкиПисем Цикл
	...
	Попытка
		...
		Если Результат.ПротоколПодключения = ПротоколИнтернетПочты.IMAP Тогда
			...
			Для Каждого ПочтовыйЯщик Из ПочтовыеЯщики Цикл
				Если НРег(ПочтовыйЯщик) = "отправленные" Или НРег(ПочтовыйЯщик) = "sent" Тогда
...
Процедура ЗагрузитьПочтуПользователя(ПолученоПисем, ДоступноУчетныхЗаписей, ЕстьОшибки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Получено = 0;
		СтруктураМассивовПисем = ПолученныеПисьма();
...
Процедура ПолучитьПочтуПоПроколуIMAP(ДанныеУчетнойЗаписи, Почта, Получено, ПолученныеПисьма)
	...
	ТаблицаПустыхИдентификаторов.Колонки.Добавить("ИдентификаторНаСервере", КвалификаторСтроки150);
