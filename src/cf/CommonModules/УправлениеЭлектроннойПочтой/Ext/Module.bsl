...
Процедура ЗаполнитьДокументЭлектронноеПисьмо(Письмо, Сообщение, ЭтоИсходящееПисьмо)
	...
	ДатаПисьма = Сообщение.ДатаОтправления;
	
	Если Не ЭтоИсходящееПисьмо Тогда
		Письмо.ДатаПолучения    = Сообщение.ДатаПолучения;
		Письмо.ОтправительАдрес = ОтправительАдрес;
		
		Если ДатаПисьма <> Дата(1,1,1) Тогда
			ДатаОтправленияУниверсальная = ДатаПисьма - Сообщение.СмещениеДатыОтправления;
			ДатаПисьма = МестноеВремя(ДатаПисьма - Сообщение.СмещениеДатыОтправления, ЧасовойПоясСеанса());
		КонецЕсли;
	Иначе
	...
	Письмо.Дата = ?(ДатаПисьма = Дата(1,1,1), ТекущаяДатаСеанса(), ДатаПисьма);
