...
Процедура ОтправитьПисьма(Запрос, ВсеПолученныеПисьма, ПисьмаДляОпределенияПапок, ОтправленоПисем = 0, ЕстьОшибки = Ложь, Интерактивно = Ложь)
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Исключение
			...
			ШаблонСообщенияОбОшибке = НСтр("ru = 'Не удалось подключиться к учетной записи %1 по причине:
				|%2'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПолучитьПисьма(Знач ДанныеУчетнойЗаписи, ЕстьОшибки, Получено, ПолученныеПисьма)
	...
	Исключение		
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось подключиться к учетной записи %1 по причине:
				|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
// Возвращаемое значение:
//  Структура:
//   * ВсеПолученныеПисьма - Массив из Документы.ЭлектронноеПисьмоВходящее
//                         - Массив из Документы.ЭлектронноеПисьмоИсходящее
//   * ПисьмаДляОпределенияПапок - Массив из Документы.ЭлектронноеПисьмоВходящее
//                               - Массив из Документы.ЭлектронноеПисьмоИсходящее
//   * ПолученныеПисьмаПоУчетнойЗаписи - Массив из Документы.ЭлектронноеПисьмоВходящее
//                                     - Массив из Документы.ЭлектронноеПисьмоИсходящее
//
...
Функция ЗаписатьЭлектронноеПисьмо(ДанныеУчетнойЗаписи, Сообщение, ОтветственныйЗаОбработкуПисем,
	...
	Попытка
		...
		ПредметИПапка = ЗаполнитьПредметИКонтакты(Письмо, ДанныеУчетнойЗаписи.Ссылка, ЭтоИсходящееПисьмо,
...
Функция ЗаполнитьПредметИКонтакты(Письмо, УчетнаяЗапись, ЭтоИсходящееПисьмо, ПомещатьПисьмоВПапкуПисьмаОснования)
...
Процедура ЗаписатьВложениеЭлектронногоПисьма(Объект, Вложение,МассивПодписей,КоличествоПустыхИменВоВложениях)
	...
	Если ЕстьПодписи Тогда
		...
		Для Каждого ПодписьВложения Из МассивПодписей Цикл
			...
			ОписаниеОшибки = "";
			ДанныеПодписи = МодульЭлектроннаяПодпись.ПрочитатьДанныеПодписи(ПодписьВложения.Данные, ОписаниеОшибки);
			...
			Если Не ПустаяСтрока(ОписаниеОшибки) Тогда
				...
				ОшибкаНеУдалосьПрочитатьДанныеПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ТекстСобытия, ВложениеПисьмаСсылка, ОписаниеОшибки);
