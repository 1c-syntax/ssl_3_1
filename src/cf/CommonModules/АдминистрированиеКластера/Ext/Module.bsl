...
Функция ИдентификаторыКлиентскихПриложений() Экспорт
	Результат = Новый Массив;
	Результат.Добавить("1CV8");               // идентификатор приложения 1С:Предприятие в режиме запуска "Толстый клиент".
	Результат.Добавить("1CV8CDirect");        // идентификатор приложения 1С:Предприятие в режиме запуска "Тонкий клиент" в случае прямого подключения к серверу
	Результат.Добавить("1CV8C");              // идентификатор приложения 1С:Предприятие в режиме запуска "Тонкий клиент".
	Результат.Добавить("WebClient");          // идентификатор приложения 1С:Предприятие в режиме запуска "Веб-клиент".
	Результат.Добавить("Designer");           // идентификатор приложения Конфигуратор.
	Результат.Добавить("COMConnection");      // идентификатор сессии внешнего соединения 1С:Предприятия через COM.
	Результат.Добавить("WSConnection");       // идентификатор сессии Web-сервиса.
	Результат.Добавить("BackgroundJob");      // идентификатор сессии обработки заданий.
	Результат.Добавить("WebServerExtension"); // идентификатор расширения Web-сервера.
	Возврат Результат;
КонецФункции
