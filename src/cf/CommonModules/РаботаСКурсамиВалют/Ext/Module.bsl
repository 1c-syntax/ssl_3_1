...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если ОповещатьОНеобходимостиОбновленияКурсовВалют Тогда
		УстановитьДатуСледующегоОповещения();
	КонецЕсли;
...
Процедура ПриДобавленииСерверныхОповещений(Оповещения) Экспорт
	...
	Если Не ОповещатьОНеобходимостиОбновленияКурсовВалют() Тогда
		Возврат;
	КонецЕсли;
	
	Оповещение = СерверныеОповещения.НовоеСерверноеОповещение(
		"СтандартныеПодсистемы.Валюты.ТребуетсяОбновитьКурсыВалют");
	
	Оповещение.ИмяМодуляОтправки  = "РаботаСКурсамиВалют";
	Оповещение.ИмяМодуляПолучения = "РаботаСКурсамиВалютКлиент";
...
Процедура ПриОтправкеСерверногоОповещения(ИмяОповещения, ВариантыПараметров) Экспорт
	...
	ДатаСледующегоОповещения = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить(
		"СтандартныеПодсистемы.Валюты", "ДатаСледующегоОповещения", '00010101',, "");
	
	Если Не ЗначениеЗаполнено(ДатаСледующегоОповещения) Или ДатаСледующегоОповещения > ТекущаяДатаСеанса() Тогда
		Возврат;
	КонецЕсли;
	
	Если Не КурсыАктуальны() Тогда
		СерверныеОповещения.ОтправитьСерверноеОповещение(ИмяОповещения, "", ВариантыПараметров[0].Адресаты);
	КонецЕсли;
...
Функция ОповещатьОНеобходимостиОбновленияКурсовВалют()
	...
	Если ОбщегоНазначения.РазделениеВключено() Или ОбщегоНазначения.ЭтоАвтономноеРабочееМесто() Тогда
		Возврат Ложь; // В модели сервиса обновляются автоматически.
	ИначеЕсли Не ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КурсыВалют) Тогда
		Возврат Ложь; // Пользователь не может обновлять курсы валют.
	КонецЕсли;
...
Процедура УстановитьДатуСледующегоОповещения()
	
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(
		"СтандартныеПодсистемы.Валюты", "ДатаСледующегоОповещения", КонецДня(ТекущаяДатаСеанса()) + 1 + 7*60*60,, "");
