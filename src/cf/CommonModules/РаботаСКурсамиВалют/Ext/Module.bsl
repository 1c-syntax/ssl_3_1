...
Процедура ПриОтправкеСерверногоОповещения(ИмяОповещения, ВариантыПараметров) Экспорт
	...
	Если ДатаСледующегоОповещения > ТекущаяДатаСеанса Тогда
		Возврат;
	КонецЕсли;
	...
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(
		"СтандартныеПодсистемы.Валюты", "ДатаСледующегоОповещения", КонецДня(ТекущаяДатаСеанса) + 1,, "");
...
// Для процедур ПриДобавленииПараметровРаботыКлиентаПриЗапуске и
