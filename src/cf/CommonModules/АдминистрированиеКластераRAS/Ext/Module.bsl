...
Функция ПреобразоватьКоманднуюСтрокуВМассив(Знач КоманднаяСтрока)
	...
	Для Индекс = 1 По ДлинаСтроки Цикл
		...
		Если (Символ = " " И Не ОткрытыеКавычки) Или Индекс = ДлинаСтроки Тогда
			
			Если Не ПустаяСтрока(НовыйЭлемент) Тогда
				НовыйЭлемент = СокрЛП(СтрЗаменить(НовыйЭлемент, """", ""));
		...
		ИначеЕсли Символ = """" Тогда
			ОткрытыеКавычки = Не ОткрытыеКавычки;
		КонецЕсли;
