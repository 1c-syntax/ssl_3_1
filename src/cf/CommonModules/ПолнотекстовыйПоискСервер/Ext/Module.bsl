...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Подсказка = 
		НСтр("ru = 'Попробуйте выключить полнотекстовый поиск и снова включить.'");
...
Функция РезультатыПолнотекстовогоПоиска(СписокПоиска)
	...
	Для Индекс = 0 По СписокПоиска.Количество() - 1 Цикл
		...
		ИнтеграцияПодсистемБСП.ПриПолученииРезультатовПолнотекстовогоПоиска(МетаданныеОбъекта, Значение, Представление);
...
Процедура ЗаписьЖурнала(УровеньЖурнала, КомментарийСПараметрами, ИнформацияОбОшибке = Неопределено,
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Полнотекстовое индексирование'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИнициализироватьФункциональнуюОпциюПолнотекстовыйПоиск");
	Методы.Вставить("ОбновлениеИндексаППДПоРасписанию", Истина);
	Методы.Вставить("СлияниеИндексаППДПоРасписанию", Истина);
