...
Процедура ОбновитьКэшПервогоПоказа(НосительМакетов)
	...
	Для Каждого Пакет Из ПакетыСтраниц Цикл
		...
		Запись.Состав = Новый ХранилищеЗначения(СоставПакета, Новый СжатиеДанных(9));
	КонецЦикла;
	
	// Метаинформация записывается в регистр под номером 0.
	Запись = НаборЗаписей.Добавить();
	Запись.Номер  = 0;
	Запись.Состав = Новый ХранилищеЗначения(ПакетыСтраниц, Новый СжатиеДанных(9));
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОперативноеОбновлениеОбщихДанных");
