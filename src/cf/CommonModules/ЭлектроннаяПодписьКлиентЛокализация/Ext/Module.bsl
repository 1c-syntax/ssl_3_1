...
// Параметры отправителя в сервис мобильной подписи.
// 
// Параметры:
//  Отправитель - Структура
//
Процедура ПриОпределенииПараметровОтправителяДокументовНаПодпись(Отправитель) Экспорт
	
	// Локализация
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		МодульСервисМобильнойПодписиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиКлиент");
		Отправитель = МодульСервисМобильнойПодписиКлиент.ПараметрыОтправителя();
	КонецЕсли;
	// Конец Локализация
...
// При получении ошибок на клиенте.
// 
// Параметры:
//  ОписаниеОшибок - Структура
//
Процедура ПриПолученииОшибокНаКлиенте(ОписаниеОшибок) Экспорт
	
	// Локализация
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		Если ТипЗнч(ОписаниеОшибок) = Тип("Структура")
		И ОписаниеОшибок.Свойство("Ошибки")
		И ТипЗнч(ОписаниеОшибок.Ошибки) = Тип("Массив")
		И ОписаниеОшибок.Ошибки.Количество() > 0
		И ОписаниеОшибок.Ошибки[0].Описание = ЭлектроннаяПодписьСлужебныйКлиент.ТекстОшибкиУстройствоНеПоддерживается() Тогда
			Если ОписаниеОшибок.Ошибки.Количество() = 1 Тогда
				ОписаниеОшибок.Ошибки[0].Описание = НСтр("ru = 'Рутокен не найден. Подробности см. в журнале регистрации.'");
			Иначе
				ОписаниеОшибок.Ошибки.Удалить(0);
			КонецЕсли;
		КонецЕсли;
#КонецЕсли
	// Конец Локализация
...
Процедура ПриОпределенииФормыНаправленияНаПодписание(ИмяФормы) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		...
		ИмяФормы = МодульСервисМобильнойПодписиСлужебныйКлиент.ИмяФормыОтправкиНаПодписание();
	КонецЕсли;
	// Конец Локализация
	
КонецПроцедуры

// При получении идентификатора мобильной компоненты.
// 
// Параметры:
//  Компонента - Строка
//
Процедура ПриПолученииИдентификатораМобильнойКомпоненты(Компонента) Экспорт
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		...
		Результат.Вставить("Токены", Ждать ТокеныИзКэша());
		Возврат Результат;
#КонецЕсли
		...
		Для Каждого Слот Из СписокСлотов Цикл
			...
			Для Каждого ЗначениеСлота Из ЗначенияСлотов Цикл
				...
				Попытка
					...
					Ждать ОшибкаКомпоненты(ОбъектКомпоненты);
					Если Не ЗначениеЗаполнено(ИнформацияОТокене.Значение) Тогда
						ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Метод %1 вернул пустую информацию'"),
							"PKCS11ПолучитьИнформациюОТокене");
					КонецЕсли;
					Информация = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПрочитатьОтветКомпоненты(
					...
					СвойстваТокена.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 		НСтр("ru = '%1 (серийный номер %2)'"), СвойстваТокена.Модель, ПредставлениеСерийногоНомера(
							ТипУстройства, СвойстваТокена.СерийныйНомер));
				Исключение
					СвойстваТокена.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					СтрокиТокенов.Вставить(СвойстваТокена.Слот, СвойстваТокена);
					Продолжить;
				КонецПопытки;
...
// Добавляет в переданный массив сертификаты с токена.
// 
// Параметры:
//  Токены - Массив, Неопределено
//  МассивСертификатов - Массив
// 
// Возвращаемое значение:
//  Массив Из СертификатКриптографии
//
Асинх Функция ДобавитьСертификатыСТокеновВМассив(Токены, МассивСертификатов) Экспорт
...
// Получает токен по сертификату.
// 
// Параметры:
//  Оповещение - ОписаниеОповещения
//  Сертификат - СертификатКриптографии
//  СтандартнаяОбработка - Булево
//  ПредложитьУстановить - Булево - предложить установить компоненту.
//  Обновить - Булево - обновить список сертификатов на токене в кэше.
// 
// Возвращаемое значение:
//  Обещание - Массив из Структура - токены
//  
Процедура ПриПолученииТокенаПоСертификату(Оповещение, Сертификат, СтандартнаяОбработка, ПредложитьУстановить = Истина, Обновить = Ложь) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	ПолучитьТокенПоСертификату(Оповещение, Сертификат, ПредложитьУстановить, Обновить);
	// Конец Локализация
	
КонецПроцедуры

// Получает сертификат на токене по отпечатку.
// 
// Параметры:
//  Оповещение - ОписаниеОповещения
//  Отпечаток - Строка
...
//  
Процедура ПриПолученииСертификатаПоОтпечаткуНаТокене(Оповещение, Отпечаток, СтандартнаяОбработка, ПредложитьУстановить = Истина, Обновить = Ложь) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	ПолучитьСертификатПоОтпечаткуНаТокене(Оповещение, Отпечаток, ПредложитьУстановить, Обновить);
	// Конец Локализация
	
КонецПроцедуры

// Получает сертификаты на токене.
// 
// Параметры:
//  Оповещение - ОписаниеОповещения - результат выполнения оповещения: Массив Из СертификатКриптографии.
//  Токен - Структура
//  СтандартнаяОбработка - Булево
//  ПредложитьУстановить - Булево - предложить установить компоненту.
//  Обновить - Булево - обновить список сертификатов на токене в кэше.
...
//  
//  
Процедура ПриПолученииСертификатовНаТокене(Оповещение, Токен, СтандартнаяОбработка, ПредложитьУстановить = Истина, Обновить = Ложь) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	ПолучитьСертификатыНаТокене(Оповещение, Токен, ПредложитьУстановить, Обновить);
	// Конец Локализация
	
КонецПроцедуры

// Находит токен для шифрования, поддерживающий алгоритм шифрования.
// 
// Параметры:
//  Оповещение - ОписаниеОповещения - результат выполнения оповещения: структура со свойствами токена.
//  Сертификат - СертификатКриптографии
...
//  СтандартнаяОбработка - Булево
//
Процедура ПриПоискеТокенаДляШифрования(Оповещение, Сертификат, АлгоритмШифрования, СвойстваСертификата = Неопределено,
	СтандартнаяОбработка = Истина) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	НачатьПоискТокенаДляШифрования(Оповещение, Сертификат, АлгоритмШифрования, СвойстваСертификата);
	// Конец Локализация
...
//
Процедура ПриПолученииТокенов(Оповещение, ПредложитьУстановить = Истина, СтандартнаяОбработка = Истина) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	ПолучитьТокены(Оповещение, ПредложитьУстановить);
	// Конец Локализация
	
КонецПроцедуры

// Получает отпечатки сертификатов на токенах.
// 
// Параметры:
//  Оповещение - ОписаниеОповещения
//  ВключаяПросроченные - Булево
//  ПредложитьУстановить - Булево - предложить установить компоненту, если не установлена.
//  СтандартнаяОбработка - Булево - стандартная обработка
//
Процедура ПриПолученииОтпечатковСертификатовНаТокенах(Оповещение, ВключаяПросроченные = Ложь,
	ПредложитьУстановить = Истина, СтандартнаяОбработка = Истина) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	ПолучитьОтпечаткиСертификатовНаТокенах(Оповещение, ВключаяПросроченные, ПредложитьУстановить);
	// Конец Локализация
	
КонецПроцедуры

// Выполняет подписание на токене.
// 
// Параметры:
...
//  СтандартнаяОбработка - Булево 
//
Процедура ПриПодписанииНаТокене(ОповещениеОЗавершении, ПараметрыПодписи, Данные, СтандартнаяОбработка = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыПодписи.ТипПодписи) 
		...
		ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, НедоступноСозданиеПодписиТекст);
		Возврат;
	КонецЕсли;
	
	ПодписатьНаТокене(ОповещениеОЗавершении, ПараметрыПодписи, Данные);
	// Конец Локализация
	
КонецПроцедуры

// Выполняет расшифровку на токене.
// 
// Параметры:
//  ОповещениеОЗавершении - ОписаниеОповещения
...
//  СтандартнаяОбработка - Булево 
//
Процедура ПриРасшифровкеНаТокене(ОповещениеОЗавершении, ПараметрыРасшифровки, Данные, СтандартнаяОбработка = Истина) Экспорт
		
	// Локализация
	СтандартнаяОбработка = Ложь;
	Если Не ЗначениеЗаполнено(ПараметрыРасшифровки.Токен.Пароль) Тогда
		ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, НСтр("ru = 'Укажите пин-код токена.'"));
		Возврат;
	КонецЕсли;
	РасшифроватьНаТокене(ОповещениеОЗавершении, ПараметрыРасшифровки, Данные);
	// Конец Локализация
	
КонецПроцедуры

// Выполняет шифрование на токене.
// 
// Параметры:
//  ОповещениеОЗавершении - ОписаниеОповещения
...
//  СтандартнаяОбработка - Булево 
//
Процедура ПриШифрованииНаТокене(ОповещениеОЗавершении, ПараметрыШифрования, Данные, СтандартнаяОбработка = Истина) Экспорт
		
	// Локализация
	СтандартнаяОбработка = Ложь;
	ЗашифроватьНаТокене(ОповещениеОЗавершении, ПараметрыШифрования, Данные);
	// Конец Локализация
	
КонецПроцедуры

// Выполняет проверку подписи на токене.
// 
// Параметры:
//  ОповещениеОЗавершении - ОписаниеОповещения
//  Данные - ДвоичныеДанные
...
//  СтандартнаяОбработка - Булево
//
Процедура ПриПроверкеПодписиНаТокене(ОповещениеОЗавершении, Данные, Подпись, ПараметрыПроверки, СтандартнаяОбработка = Истина) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	НачатьПоискТокенаДляПроверкиПодписи(ОповещениеОЗавершении,  Данные, Подпись, ПараметрыПроверки);
	// Конец Локализация
...
Процедура ПриПолученииПараметровПодписанта(ПараметрыПодписанта) Экспорт
	...
	// Локализация
	МодульСервисМобильнойПодписиКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиКлиентСервер");
	ПараметрыПодписанта = МодульСервисМобильнойПодписиКлиентСервер.ПараметрыПодписанта();
	// Конец Локализация
	
КонецПроцедуры

// Переопределяет процедуру добавления приложения электронной подписи.
// 
// Параметры: 
//  СтандартнаяОбработка - Булево
//
Процедура ПриДобавленииПриложенияЭлектроннойПодписи(СтандартнаяОбработка) Экспорт
	
	// Локализация
	СтандартнаяОбработка = Ложь;
	ДобавитьПриложениеЭлектроннойПодписи();
	// Конец Локализация
	
КонецПроцедуры

// Дополняет список событий для обновления списка настроек приложений электронной подписи.
// 
// Параметры:
//  События - Массив
//
Процедура ПриПолученииСобытийИзмененияНастроекПрограмм(События) Экспорт
	
	// Локализация
	События.Добавить(ВРег("Запись_НастройкиОтправкиВГосключ"));
	События.Добавить(ВРег("Запись_УчетнойЗаписиDSS"));
	// Конец Локализация
...
Процедура ПодписантПриОткрытии(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	КонецЕсли;
	МодульСервисМобильнойПодписиСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиСлужебныйКлиент");
	МодульСервисМобильнойПодписиСлужебныйКлиент.ПодписантПриОткрытии(Форма, Элемент, СтандартнаяОбработка);
	// Конец Локализация
	
КонецПроцедуры

// При создании подписанта.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  Элемент - ПолеФормы
...
Процедура ПодписантПриОчистке(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	КонецЕсли;
	МодульСервисМобильнойПодписиСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиСлужебныйКлиент");
	МодульСервисМобильнойПодписиСлужебныйКлиент.ПодписантПриОчистке(Форма, Элемент, СтандартнаяОбработка);
	// Конец Локализация
...
#Область ТокеныЛокализация

Процедура ВвестиПинКодТокена(ОписаниеОповещения) Экспорт
	
	ОткрытьФорму("Обработка.ПрограммыЭлектроннойПодписиИШифрования.Форма.ВводПинКода",, ЭтотОбъект, Истина,
...
//
Асинх Функция ПроверитьПодпись(Токен, Данные, Подпись, ПараметрыПроверки) Экспорт
	...
	Исключение
		...
		ТекстОшибки = ТекстОшибки(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), Токен);
...
Процедура ПослеПроверкиПодписи(Результат, Контекст) Экспорт
	...
	ИначеЕсли Контекст.ПараметрыПроверки.ВернутьСертификатыДляПроверки Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат.Сертификаты);
	Иначе
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат.Статус);
	КонецЕсли;
КонецПроцедуры

Асинх Процедура ПодписатьНаТокене(ОповещениеОЗавершении, ПараметрыПодписи, Данные)
	
	Контекст = Новый Структура("ПараметрыПодписи, Данные, Оповещение", ПараметрыПодписи, Данные, ОповещениеОЗавершении);
	
	Если Не ЗначениеЗаполнено(ПараметрыПодписи.Токен.Пароль) Тогда
		ВвестиПинКодТокена(Новый ОписаниеОповещения("ПродолжитьПодписаниеПослеВводаПинКода", ЭтотОбъект, Контекст));
		Возврат;
	КонецЕсли;
	
	ПродолжитьПодписаниеПослеВводаПинКода(ПараметрыПодписи.Токен.Пароль, Контекст);

КонецПроцедуры

Асинх Процедура ПродолжитьПодписаниеПослеВводаПинКода(ПинКод, Контекст) Экспорт
	
	Если Не ЗначениеЗаполнено(ПинКод) Тогда 
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, НСтр("ru = 'Укажите пин-код токена.'"));
		Возврат;
	КонецЕсли;
	
	Контекст.ПараметрыПодписи.Токен.Пароль = ПинКод;
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	Оповещение = Новый ОписаниеОповещения("ПослеОперацииНаТокенеВМобильномКлиенте", ЭтотОбъект, Контекст);
	НачатьПодписьНаТокенеВМобильномКлиенте(Оповещение, Контекст.ПараметрыПодписи, Контекст.Данные);
	Возврат;
#КонецЕсли
	
	Результат = Ждать ПодписьНаТокене(Контекст.ПараметрыПодписи, Контекст.Данные);
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
	
КонецПроцедуры

Асинх Процедура РасшифроватьНаТокене(ОповещениеОЗавершении, ПараметрыРасшифровки, Данные)
	
	Контекст = Новый Структура("ПараметрыРасшифровки, Данные, Оповещение", ПараметрыРасшифровки, Данные, ОповещениеОЗавершении);
	Если Не ЗначениеЗаполнено(ПараметрыРасшифровки.Токен.Пароль) Тогда
		ВвестиПинКодТокена(Новый ОписаниеОповещения("ПродолжитьРасшифровкуПослеВводаПинКода", ЭтотОбъект, Контекст));
		Возврат;
	КонецЕсли;
	
	ПродолжитьРасшифровкуПослеВводаПинКода(ПараметрыРасшифровки.Токен.Пароль, Контекст);

КонецПроцедуры

Асинх Процедура ПродолжитьРасшифровкуПослеВводаПинКода(ПинКод, Контекст) Экспорт
	...
	Контекст.ПараметрыРасшифровки.Токен.Пароль = ПинКод;
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеОперацииНаТокенеВМобильномКлиенте", 
			ЭтотОбъект, Контекст);
		НачатьРасшифровкуНаТокенеВМобильномКлиенте(Оповещение, Контекст.ПараметрыРасшифровки, Контекст.Данные);
		Возврат;
#КонецЕсли
	
	Результат = Ждать РасшифровкаНаТокене(Контекст.ПараметрыРасшифровки, Контекст.Данные);
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
	
КонецПроцедуры

Асинх Процедура ЗашифроватьНаТокене(ОповещениеОЗавершении, ПараметрыШифрования, Данные)
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	Контекст = Новый Структура("ПараметрыРасшифровки, Данные, Оповещение", ПараметрыШифрования, Данные, ОповещениеОЗавершении);
	Оповещение = Новый ОписаниеОповещения("ПослеОперацииНаТокенеВМобильномКлиенте", 
		ЭтотОбъект, Контекст);
	НачатьШифрованиеНаТокенеВМобильномКлиенте(Оповещение, ПараметрыШифрования, Данные);
	Возврат;
#КонецЕсли
	
	Результат = Ждать ШифрованиеНаТокене(ПараметрыШифрования, Данные);
	ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, Результат);

КонецПроцедуры

Асинх Процедура НачатьПоискТокенаДляШифрования(Оповещение, Сертификат, АлгоритмШифрования, СвойстваСертификата)

#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	Токены = Ждать ТокеныИзКэша();
	Если Не ЗначениеЗаполнено(Токены) Тогда
		...
		НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("НайтиТокенДляШифрованияПослеПоискаТокенов", ЭтотОбъект, Контекст));
		Возврат;
	КонецЕсли;
	ТокенДляШифрования = Ждать ПодходящийТокенДляШифрования(Токены, Сертификат, АлгоритмШифрования, СвойстваСертификата);
#Иначе
	ТокенДляШифрования = Ждать ТокенДляШифрования(Сертификат, АлгоритмШифрования, СвойстваСертификата);
#КонецЕсли
	
	ВыполнитьОбработкуОповещения(Оповещение, ТокенДляШифрования);
	
КонецПроцедуры

Асинх Процедура НачатьПоискТокенаДляПроверкиПодписи(Оповещение,  Данные, Подпись, ПараметрыПроверки)
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	...
	Если Не ЗначениеЗаполнено(Токены) Тогда
		...
		НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("ПроверитьПодписьПослеПоискаТокенов", ЭтотОбъект, Контекст));
		Возврат;
	КонецЕсли;
	ТокенДляПроверкиПодписи = ТокенДляПроверкиПодписи(Токены, ПараметрыПроверки, Подпись);
	Если ТипЗнч(ТокенДляПроверкиПодписи) <> Тип("Структура") Тогда
		ВыполнитьОбработкуОповещения(Оповещение, ТокенДляПроверкиПодписи);
		Возврат;
	КонецЕсли;
	
	НачатьПроверкуПодписиНаТокенеВМобильномКлиенте(Оповещение, ТокенДляПроверкиПодписи, Данные, Подпись, ПараметрыПроверки);
	Возврат;
#Иначе
	...
	Результат = Ждать ПроверитьПодпись(ТокенДляПроверкиПодписи, Данные, Подпись, ПараметрыПроверки);
...
Процедура ПроверитьПодписьПослеПоискаТокенов(Токены, Контекст) Экспорт
	...
	ТокенДляПроверкиПодписи = ТокенДляПроверкиПодписи(Токены, Контекст.ПараметрыПроверки, Контекст.Подпись);
	Если ТипЗнч(ТокенДляПроверкиПодписи) <> Тип("Структура") Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, ТокенДляПроверкиПодписи);
		Возврат;
	КонецЕсли;
	
	НачатьПроверкуПодписиНаТокенеВМобильномКлиенте(Контекст.Оповещение, ТокенДляПроверкиПодписи, Контекст.Данные,
		Контекст.Подпись, Контекст.ПараметрыПроверки);
	
КонецПроцедуры

Функция ТокенДляПроверкиПодписи(Токены, ПараметрыПроверки, Подпись)
	...
	Если НайденТокен = Неопределено Тогда
...
//  Обещание - Структура
//
Асинх Функция ТокенДляШифрования(Сертификат, АлгоритмШифрования = Неопределено, СвойстваСертификата = Неопределено)
	...
	Возврат Ждать ПодходящийТокенДляШифрования(Токены, Сертификат, АлгоритмШифрования, СвойстваСертификата);
	
КонецФункции

Асинх Процедура НайтиТокенДляШифрованияПослеПоискаТокенов(Токены, Контекст) Экспорт 
	...
	ПодходящийТокенДляШифрования = Ждать ПодходящийТокенДляШифрования(
		Токены, Контекст.Сертификат, Контекст.АлгоритмШифрования, Контекст.СвойстваСертификата);
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, ПодходящийТокенДляШифрования);
	
КонецПроцедуры

Асинх Функция ПодходящийТокенДляШифрования(Токены, Сертификат, АлгоритмШифрования, СвойстваСертификата)
...
Асинх Процедура ПолучитьТокенПоСертификату(Оповещение, Сертификат, ПредложитьУстановить, Обновить)
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	Токены = Ждать ТокеныИзКэша();
	Если Не ЗначениеЗаполнено(Токены) Или Обновить Тогда
		НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("ПолучитьТокенПоСертификатуПослеПолученияСертификатовНаТокенах", ЭтотОбъект,
				Новый Структура("Оповещение, Сертификат", Оповещение, Сертификат)));
		Возврат;
	КонецЕсли;
#Иначе
	Токены = Ждать СертификатыНаТокенах(ПредложитьУстановить, Обновить);
#КонецЕсли
	...
	ВыполнитьОбработкуОповещения(Оповещение, НайтиТокенПоСертификату(Сертификат, Токены));
	
КонецПроцедуры

Асинх Процедура ПолучитьСертификатПоОтпечаткуНаТокене(Оповещение, Отпечаток, ПредложитьУстановить, Обновить)
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	Токены = Ждать ТокеныИзКэша();
	Если Не ЗначениеЗаполнено(Токены) Или Обновить Тогда
		НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("ПолучитьСертификатПоОтпечаткуПослеПолученияСертификатовНаТокене", ЭтотОбъект,
				Новый Структура("Оповещение, Отпечаток", Оповещение, Отпечаток)));
		Возврат;
	КонецЕсли;
#Иначе
	Токены = Ждать СертификатыНаТокенах(ПредложитьУстановить, Обновить);
#КонецЕсли
	...
	ВыполнитьОбработкуОповещения(Оповещение, Ждать НайтиСертификатПоОтпечаткуНаТокене(Отпечаток, Токены));
	
КонецПроцедуры

Асинх Процедура ПолучитьСертификатыНаТокене(Оповещение, Токен, ПредложитьУстановить, Обновить)

#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		Токены = Ждать ТокеныИзКэша();
		Если Не ЗначениеЗаполнено(Токены) Или Обновить Тогда
			НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("ПолучитьСертификатыНаТокенеПослеПолученияТокена", ЭтотОбъект,
			Новый Структура("Токен, Оповещение", Токен, Оповещение)));
			Возврат;
		КонецЕсли;
		ОповеститьОСертификатахНаТокене(Токены, Токен, Оповещение);
		Возврат;
#Иначе
		Результат = Ждать СертификатыНаТокене(Токен, ПредложитьУстановить, Обновить);
		СертификатыНаТокене = Результат.Сертификаты;
		ВыполнитьОбработкуОповещения(Оповещение, СертификатыНаТокене);
#КонецЕсли

КонецПроцедуры

Процедура ПолучитьСертификатыНаТокенеПослеПолученияТокена(Токены, Контекст) Экспорт
	ОповеститьОСертификатахНаТокене(Токены, Контекст.Токен, Контекст.Оповещение);
КонецПроцедуры

Процедура ОповеститьОСертификатахНаТокене(Токены, Токен, Оповещение)
	СертификатыНаТокене = Новый Массив;
	Для Каждого ТекущийТокен Из Токены Цикл
		Если ТекущийТокен.СерийныйНомер = Токен.СерийныйНомер Тогда
			Для Каждого ТекущийСертификат Из ТекущийТокен.Сертификаты Цикл
				СертификатыНаТокене.Добавить(ТекущийСертификат.Значение);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	ВыполнитьОбработкуОповещения(Оповещение, СертификатыНаТокене);
...
Функция НайтиТокенПоСертификату(Сертификат, Токены)
...
Асинх Функция НайтиСертификатПоОтпечаткуНаТокене(Отпечаток, Токены)
...
Асинх Процедура ПолучитьТокены(Оповещение, ПредложитьУстановить)
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("ПослеПолученияСвойствСертификатовНаТокенах", ЭтотОбъект, Оповещение));
		Возврат;
#КонецЕсли
	
	Токены = Ждать СертификатыНаТокенах(ПредложитьУстановить);
	ВыполнитьОбработкуОповещения(Оповещение, Токены);
	
КонецПроцедуры

Асинх Процедура ПолучитьОтпечаткиСертификатовНаТокенах(Оповещение, ВключаяПросроченные, ПредложитьУстановить)
	
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(
			Новый ОписаниеОповещения("ПолучитьОтпечаткиПослеПолученияСвойствСертификатовНаТокенах", ЭтотОбъект,
				Новый Структура("Оповещение, ВключаяПросроченные", Оповещение, ВключаяПросроченные)));
		Возврат;
#КонецЕсли
	...
	МассивОтпечатков = Ждать ПолучитьОтпечаткиСертификатовНаТокенахАсинх(Токены, ВключаяПросроченные);
	ВыполнитьОбработкуОповещения(Оповещение, МассивОтпечатков);
	
КонецПроцедуры

Асинх Процедура ПолучитьОтпечаткиПослеПолученияСвойствСертификатовНаТокенах(Токены, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Токены) Тогда
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.Оповещение, Новый Массив);
		Возврат;
	КонецЕсли;
	
	МассивОтпечатков = Ждать ПолучитьОтпечаткиСертификатовНаТокенахАсинх(Токены, ДополнительныеПараметры.ВключаяПросроченные);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.Оповещение, МассивОтпечатков);
...
//    Обещание - Массив
//
Асинх Функция ПолучитьОтпечаткиСертификатовНаТокенахАсинх(Токены, ВключаяПросроченные)
...
Процедура ПослеПолученияСвойствСертификатовНаТокенах(Токены, Оповещение) Экспорт
	ВыполнитьОбработкуОповещения(Оповещение, Токены);
КонецПроцедуры
...
//
Асинх Функция ПодписьНаТокене(ПараметрыПодписи, Данные)
...
//  Обещание - ДвоичныеДанные, Строка
//
Асинх Функция ШифрованиеНаТокене(ПараметрыШифрования, Данные)
	...
	СертификатыДляШифрованияСтрокой = Ждать СертификатыДляШифрования(ПараметрыШифрования.Сертификат);
...
Асинх Функция СертификатыДляШифрования(Сертификаты)
	
	Если ТипЗнч(Сертификаты) = Тип("Массив") Тогда
		СертификатыДляШифрования = Сертификаты;
	Иначе
		СертификатыДляШифрования = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Сертификаты);
	КонецЕсли;
	...
	Попытка
		...
		КонецЦикла;
		Возврат СертификатыДляШифрованияСтрокой;
	Исключение
		Возврат ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
//  Обещание - ДвоичныеДанные, Строка
//
Асинх Функция РасшифровкаНаТокене(ПараметрыРасшифровки, Данные)
...
//  Обещание - Массив Из Структура - Токены
//  
Асинх Функция СертификатыНаТокенах(ПредложитьУстановить, Обновить = Ложь)
...
Асинх Функция СертификатыНаТокене(Токен, ОбъектКомпоненты = Неопределено, ПредложитьУстановить = Ложь) Экспорт
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Результат.Ошибка) Тогда
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Получение сертификатов на токене'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
				"Ошибка", Результат.Ошибка,, Истина);
			Возврат Результат;
		КонецЕсли;
	...
	Попытка
		...
		Ждать ОшибкаКомпоненты(ОбъектКомпоненты);
		Если СертификатыНаТокене.Значение = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Метод получения сертификатов вернул Неопределено.'");
		КонецЕсли;
	...
	Исключение
		...
		Если ЭтоОшибкаИнициализацииСлота(ТекстОшибки) Тогда
			Результат.Ошибка = ОшибкаИнициализацииСлота(Токен);
		Иначе
...
#Область ТокеныВМобильномКлиенте

Функция ФормаОперацииНаТокене()
	...
	Форма = ПараметрыПриложения["СтандартныеПодсистемы.ПараметрыЭлектроннойПодписиИШифрования"].Получить("ОперацииНаТокене");
	
	Если Форма = Неопределено Тогда
		Форма = ОткрытьФорму("Обработка.ПрограммыЭлектроннойПодписиИШифрования.Форма.ОперацииНаТокене");
		ПараметрыПриложения["СтандартныеПодсистемы.ПараметрыЭлектроннойПодписиИШифрования"].Вставить("ОперацииНаТокене", Форма);
	КонецЕсли;
...
Функция ПараметрыОперацииНаТокене(Операция)
	Возврат Новый Структура("Операция", Операция);
КонецФункции

Процедура ПолучитьТокенПоСертификатуПослеПолученияСертификатовНаТокенах(Токены, Контекст) Экспорт
	...
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, НайтиТокенПоСертификату(Контекст.Сертификат, Токены));
	
КонецПроцедуры

Асинх Процедура ПолучитьСертификатПоОтпечаткуПослеПолученияСертификатовНаТокене(Токены, Контекст) Экспорт
	...
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ждать НайтиСертификатПоОтпечаткуНаТокене(Контекст.Отпечаток, Токены));
	
КонецПроцедуры

Асинх Функция ТокеныИзКэша()
	
	УстановленныеКриптопровайдеры = Ждать ЭлектроннаяПодписьСлужебныйКлиент.УстановленныеКриптопровайдерыИзКэша(Ложь);
	Возврат УстановленныеКриптопровайдеры.Токены;
	
КонецФункции

Асинх Процедура НачатьПолучениеСертификатовНаТокенахВМобильномКлиенте(ОповещениеОЗавершении)
	
	Токены = Ждать ТокеныИзКэша();
	Если ЗначениеЗаполнено(Токены) Тогда
		ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, Токены);
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = ПараметрыОперацииНаТокене(ПолучениеСертификатов());
	
	Оповещение = Новый ОписаниеОповещения("ПослеПолученияСертификатов", ЭтотОбъект, ОповещениеОЗавершении);
	ПараметрыФормы.Вставить("Оповещение", Оповещение);
	
	Форма = ФормаОперацииНаТокене();
	Оповестить("БСП_ОперацииНаТокене", ПараметрыФормы);
	
КонецПроцедуры

Асинх Процедура ПослеПолученияСертификатов(Токены, Оповещение) Экспорт
	
	Если Не ЗначениеЗаполнено(Токены) Или ТипЗнч(Токены) = Тип("Строка") Тогда
		ВыполнитьОбработкуОповещения(Оповещение, Новый Массив);
		Возврат;
	КонецЕсли;
...
Процедура НачатьПодписьНаТокенеВМобильномКлиенте(Оповещение, ПараметрыПодписи, Данные)
	
	ПараметрыОперации = Новый Структура("ВключатьСертификатВПодпись, Открепленная, ТипПодписи, Токен");
	ЗаполнитьЗначенияСвойств(ПараметрыОперации, ПараметрыПодписи);
	ПараметрыОперации.Вставить("Отпечаток", Base64Строка(ПараметрыПодписи.Сертификат.Отпечаток));
	
	ПараметрыФормы = ПараметрыОперацииНаТокене(Подписание());
...
Процедура НачатьРасшифровкуНаТокенеВМобильномКлиенте(Оповещение, ПараметрыРасшифровки, Данные)
	
	ПараметрыОперации = Новый Структура("Токен");
	ЗаполнитьЗначенияСвойств(ПараметрыОперации, ПараметрыРасшифровки);
	ПараметрыОперации.Вставить("Отпечаток", Base64Строка(ПараметрыРасшифровки.Сертификат.Отпечаток));
	
	ПараметрыФормы = ПараметрыОперацииНаТокене(Расшифровка());
	ПараметрыФормы.Вставить("ПараметрыОперации", ПараметрыОперации);
	ПараметрыФормы.Вставить("Данные", Данные);
	ПараметрыФормы.Вставить("Оповещение", Оповещение);
...
Асинх Процедура НачатьШифрованиеНаТокенеВМобильномКлиенте(Оповещение, ПараметрыШифрования, Данные)
	
	ПараметрыОперации = Новый Структура("Токен, АлгоритмШифрования");
	ЗаполнитьЗначенияСвойств(ПараметрыОперации, ПараметрыШифрования);
	
	Если ЗначениеЗаполнено(ПараметрыОперации.АлгоритмШифрования) Тогда
		АлгоритмШифрования = АлгоритмШифрованияСтрока(ПараметрыОперации.АлгоритмШифрования);

		Если АлгоритмШифрования = АлгоритмНеПоддерживается() Тогда
			ВыполнитьОбработкуОповещения(Оповещение, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'На мобильном устройстве шифрование алгоритмом %1 будет реализовано позднее.'"),
				ПараметрыОперации.АлгоритмШифрования));
			Возврат;
		КонецЕсли;

		ПараметрыОперации.АлгоритмШифрования = АлгоритмШифрования;
	КонецЕсли;
	
	СертификатыДляШифрования = Ждать СертификатыДляШифрования(ПараметрыШифрования.Сертификат);
	ПараметрыОперации.Вставить("Сертификаты", СертификатыДляШифрования);
	
	ПараметрыФормы = ПараметрыОперацииНаТокене(Шифрование());
	ПараметрыФормы.Вставить("ПараметрыОперации", ПараметрыОперации);
	ПараметрыФормы.Вставить("Данные", Данные);
	ПараметрыФормы.Вставить("Оповещение", Оповещение);
...
Асинх Процедура НачатьПроверкуПодписиНаТокенеВМобильномКлиенте(Оповещение, Токен, Данные, Подпись, ПараметрыПроверки)
		
	ПараметрыОперации = Новый Структура("Токен, Данные, Подпись, ОткрепленнаяПодпись");
	ЗаполнитьЗначенияСвойств(ПараметрыОперации, ПараметрыПроверки);
	ПараметрыОперации.Вставить("Подпись", Подпись);
	ПараметрыОперации.Вставить("Токен", Токен);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеПроверкиПодписи", ЭтотОбъект,
		Новый Структура("Оповещение, ПараметрыПроверки", Оповещение, ПараметрыПроверки));
	
	ПараметрыФормы = ПараметрыОперацииНаТокене(ПроверкаПодписи());
	ПараметрыФормы.Вставить("ПараметрыОперации", ПараметрыОперации);
	ПараметрыФормы.Вставить("Данные", Данные);
	ПараметрыФормы.Вставить("Оповещение", ОписаниеОповещения);
...
Процедура ПослеОперацииНаТокенеВМобильномКлиенте(Результат, Контекст) Экспорт
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
КонецПроцедуры

Функция ПолучениеСертификатов() Экспорт
	Возврат "ПолучениеСертификатов";
КонецФункции

Функция Подписание() Экспорт
	Возврат "Подписание";
КонецФункции

Функция Шифрование() Экспорт
	Возврат "Шифрование";
КонецФункции

Функция Расшифровка() Экспорт
	Возврат "Расшифровка";
КонецФункции

Функция ПроверкаПодписи() Экспорт
...
Функция БиблиотекаУжеИнициализирована(ТекстОшибки) Экспорт
	Если СтрНайти(ТекстОшибки, "pkcs11:401") <> 0 Тогда
		Возврат Истина;
	КонецЕсли;
...
Асинх Функция ОшибкаКомпоненты(ОбъектКомпоненты, ВызыватьИсключение = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(Ошибка) Или БиблиотекаУжеИнициализирована(Ошибка) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция МодельТокена(Модель, ТипУстройства = Неопределено) Экспорт
	
	Если СтрНачинаетсяС(НРег(Модель), "rutokenecp") Или СтрНачинаетсяС(НРег(Модель), "rutoken ecp") Тогда
	...
	ИначеЕсли СтрНачинаетсяС(НРег(Модель), "jacartagost") Тогда
		Возврат НСтр("ru = 'JaCarta ГОСТ'");
	ИначеЕсли СтрНачинаетсяС(НРег(Модель), "jacartalaser") Тогда
		Возврат НСтр("ru = 'JaCarta PKI'");
	ИначеЕсли СтрНачинаетсяС(НРег(Модель), "jacartapro") Тогда
		Возврат НСтр("ru = 'JaCarta PRO'");
...
Функция МеханизмПроверкиПодписи(АлгоритмПодписи)
	Если АлгоритмПодписи = "ГОСТ 34.10-2012 256" Тогда
		Возврат "3560050696"; // "CKM_GOSTR3410_WITH_GOSTR3411_12_256"
	ИначеЕсли АлгоритмПодписи = "ГОСТ 34.10-2012 512" Тогда
		Возврат "3560050697"; // "CKM_GOSTR3410_WITH_GOSTR3411_12_512"
	ИначеЕсли АлгоритмПодписи = "ГОСТ 34.10-2001" Тогда
		Возврат "4609"; //"CKM_GOSTR3410"
	ИначеЕсли АлгоритмПодписи = "RSA_SIGN" Тогда
		Возврат "1"; // "CKM_RSA_PKCS";
	КонецЕсли;
	Возврат "";
КонецФункции
...
Функция АлгоритмШифрованияСтрока(АлгоритмШифрования)
	АлгоритмШифрования = ЭлектроннаяПодписьКлиентСерверЛокализация.ПреобразованныйАлгоритмШифрования(АлгоритмШифрования);
	Если АлгоритмШифрования = "1.2.643.7.1.1.5.2.1" Тогда
		Возврат АлгоритмНеПоддерживается();
	ИначеЕсли АлгоритмШифрования = "1.2.643.7.1.1.5.1.1" Тогда
		Возврат АлгоритмНеПоддерживается();
	КонецЕсли;
	Возврат "GOST28147_GCFB";
КонецФункции

Функция АлгоритмНеПоддерживается()
	Возврат "АлгоритмНеПоддерживается";
КонецФункции
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	Если Не ТипЗнч(СписокМеханизмов) = Тип("Массив") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого Элемент Из СписокМеханизмов Цикл
		Если СокрЛП(Формат(Элемент.CK_MECHANISM,"ЧГ=0" )) = Механизм Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
#Иначе
	...
	Для Каждого Элемент Из СписокМеханизмов Цикл
		Если СокрЛП(Формат(Элемент.type,"ЧГ=0" )) = Механизм Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьПриложениеЭлектроннойПодписи()
	
	Оповещение = Новый ОписаниеОповещения("ДобавлениеПрограммыЭлектроннойПодписиПослеВыбора", ЭтотОбъект);
	НоваяФорма = ОткрытьФорму("Обработка.ПрограммыЭлектроннойПодписиИШифрования.Форма.ДобавлениеПриложения",,,,,,
		Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Если НоваяФорма = Неопределено Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы();
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавлениеПрограммыЭлектроннойПодписиПослеВыбора(Результат, Контекст) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	Если Результат = "ОблачнаяПодпись" Тогда
		МодульСервисКриптографииDSSКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSКлиент");
		МодульСервисКриптографииDSSКлиент.ОткрытьСведенияУчетнойЗаписи(Неопределено, Неопределено);
	ИначеЕсли Результат = "НастройкаОтправкиВГосключ" Тогда
		МодульСервисМобильнойПодписиСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиСлужебныйКлиент");
		МодульСервисМобильнойПодписиСлужебныйКлиент.ДобавитьНастройкуОтправкиВГосключ();
	Иначе
		ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы();
	КонецЕсли;
