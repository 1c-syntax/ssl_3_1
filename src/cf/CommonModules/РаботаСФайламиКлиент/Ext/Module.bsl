...
#Если ВебКлиент Тогда
	Если ПараметрыВыполнения.ДанныеФайла.Расширение <> "mxl" Тогда
		Текст = НСтр("ru = 'Необходимо сохранить файл на компьютер, после чего распечатать его при помощи приложения, предназначенного для работы с данным файлом.'");
...
#Если Не МобильныйКлиент Тогда
	...
	ИначеЕсли ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() Тогда
		...
		НачалоПечатиLinux = Новый ОписаниеОповещения("НапечататьИзПриложенияПоИмениФайлаНачалоПечатиLinux", 
			ЭтотОбъект, ДополнительныеПараметры);
		ПроверитьПрограммаLinuxУстановлена("Unoconv", НачалоПечатиLinux);
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Печать файлов данного типа возможна только из приложения для Windows или Linux.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьПрограммаLinuxУстановлена(ИмяПрограммы, Оповещение)
	...
	ФайловаяСистемаКлиент.ЗапуститьПрограмму("dpkg -s '" + Строка(ИмяПрограммы) + "'", ПараметрыЗапускаПрограммы);
...
Процедура НапечататьИзПриложенияПоИмениФайлаНачалоПечатиLinux(Результат, Параметры) Экспорт
	...
	ЗавершениеПечатиLinux = Новый ОписаниеОповещения("НапечататьИзПриложенияПоИмениФайлаЗавершениеПечатиLinux", ЭтотОбъект);
	
	ПараметрыЗапускаПрограммы = ФайловаяСистемаКлиент.ПараметрыЗапускаПрограммы();
	ПараметрыЗапускаПрограммы.Вставить("ДождатьсяЗавершения", Истина);
	ПараметрыЗапускаПрограммы.Вставить("ПолучитьПотокОшибок", Истина);
	ПараметрыЗапускаПрограммы.Вставить("Оповещение", ЗавершениеПечатиLinux);
	
	ФайловаяСистемаКлиент.ЗапуститьПрограмму("unoconv --stdout '" + Параметры.ИмяОткрываемогоФайла + "' |lpr", ПараметрыЗапускаПрограммы);
	
КонецПроцедуры

Процедура НапечататьИзПриложенияПоИмениФайлаЗавершениеПечатиLinux(Результат, Параметры) Экспорт
	...
	Если Результат.КодВозврата <> 0 Тогда
	     ПоказатьПредупреждение(, Результат.ПотокОшибок);
	КонецЕсли;
