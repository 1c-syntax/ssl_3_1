...
Процедура ОткрытьФайл(Знач ДанныеФайла, Знач ДляРедактирования = Ложь) Экспорт
	...
	Если ДанныеФайла.Свойство("АсинхронноеПолучениеФайла") Тогда
		
		РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом(ДанныеФайла);
		
	Иначе

		РаботаСФайламиСлужебныйКлиент.ОткрытьФайлСлужебный(ДанныеФайла, ДляРедактирования);
...
Процедура ДобавитьФайлы(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач Фильтр = "", ГруппаФайлов = Неопределено,
	...
	РаботаСФайламиСлужебныйКлиент.ПоказатьВопросОбУстановкеРасширения1СПредприятия(ОписаниеОповещения);
...
Процедура СохранитьФайлКак(Знач ДанныеФайла, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Иначе
		...
		РаботаСФайламиСлужебныйКлиент.СохранитьФайлКакСлужебный(ДанныеФайла, ОбработчикЗавершения);
...
//
Процедура ОткрытьФормуСпискаФайлов(Знач ВладелецФайлов, Знач ПараметрыФормы = Неопределено, 
	ВладелецФормы = Неопределено, ОповещениеОЗакрытии = Неопределено) Экспорт
	
	Если РаботаСФайламиСлужебныйКлиент.Используется1СДокументооборотДляХраненияФайловОбъекта(ВладелецФайлов) Тогда
		
		// ИнтеграцияС1СДокументооборотом
		РаботаСФайламиСлужебныйКлиент.ОткрытьФормуПрисоединенныеФайлы1СДокументооборот(ВладелецФайлов);
		// Конец ИнтеграцияС1СДокументооборотом
		
	Иначе
		Если ПараметрыФормы = Неопределено Тогда
			ПараметрыФормы = ПараметрыОткрытияФормыСпискаФайлов(ВладелецФайлов);
		КонецЕсли;
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ПрисоединенныеФайлы", ПараметрыФормы, ВладелецФормы, , , , 
			ОповещениеОЗакрытии);
	КонецЕсли;
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено,
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Добавление электронных подписей не доступно в этой версии приложения.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не МодульЭлектроннаяПодписьКлиент.ИспользоватьЭлектронныеПодписи() Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Добавление электронных подписей запрещено в настройках приложения.'"));
		Возврат;
	КонецЕсли;
...
// Выполняет отправку на подписание файла в сервис мобильной подписи.
// При отсутствии подсистемы "Электронная подпись" и подсистемы "Сервис мобильной подписи" будет показано предупреждение
// о невозможности добавления электронной подписи.
//
// Параметры:
//  ПрисоединенныйФайл      - ОпределяемыйТип.ПрисоединенныйФайл - ссылка на элемент справочника с файлом.
...
//                                - Массив из см. РаботаСФайлами.ДанныеФайла
//       * ОбработкаРезультата    - ОписаниеОповещения - при вызове передается значение типа Булево,
//                                  если Истина - файл успешно отправлен на подпись, иначе не отправлен,
//                                  если свойства нет, оповещение не будет вызвано.
//  ПараметрыПодписи         - см. ЭлектроннаяПодписьКлиент.НовыйТипПодписи
//
Процедура ОтправитьФайлНаПодписание(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено,
	ПараметрыПодписи = Неопределено) Экспорт
	...
	РаботаСФайламиСлужебныйКлиент.ОтправитьФайлНаПодписание(ПрисоединенныйФайл,
		ДополнительныеПараметры.ДанныеФайла, ИдентификаторФормы, ОбработкаРезультата, Неопределено, ПараметрыПодписи);
...
Функция ДанныеФайла(Знач ФайлСсылка,
	...
	ДополнительныеПараметры.ДляРедактирования				= ДляРедактирования;

	Возврат РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайла(ФайлСсылка, ДополнительныеПараметры);
...
Процедура ОткрытьФормуНастройкиСканирования() Экспорт
	...
	Если Не РаботаСФайламиСлужебныйКлиент.ДоступноСканирование() Тогда
		ТекстСообщения = НСтр("ru = 'Сканирование доступно в приложении для Microsoft Windows и Linux.'");
...
Процедура ОбъединитьВМногостраничныйФайл(ОповещениеВозврата, ОбъектыДляОбъединения, ПараметрыКонвертацииГрафическогоДокумента) Экспорт
	...
	ИначеЕсли ИспользоватьImageMagick И Не ЗначениеЗаполнено(ПутьКПрограммеКонвертации) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан путь к приложению %1.
...
Процедура ОбработкаОповещения(Форма, ИмяСобытия) Экспорт
	...
	Для НомерЭлемента = 0 По Форма.ПараметрыРаботыСФайлами.ОписанияЭлементовФормы.ВГраница() Цикл
		...
		Если ПозицияКоличестваВЗаголовке = 0 Тогда
			ИсходныйЗаголовок = Гиперссылка.Заголовок;
		Иначе
			ИсходныйЗаголовок = Лев(Гиперссылка.Заголовок, ПозицияКоличестваВЗаголовке - 1);
		КонецЕсли;
		Постфикс = "";
		Если ЗначениеЗаполнено(КоличествоПрисоединенныхФайлов) Тогда
			Постфикс = СтрШаблон(" (%1)", КоличествоПрисоединенныхФайловСтрокой);
		КонецЕсли;
		Гиперссылка.Заголовок = СокрЛП(ИсходныйЗаголовок) + Постфикс;
	КонецЦикла;
...
Процедура ПолеПредпросмотраПеретаскивание(Форма, Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка) Экспорт
	...
	Иначе
		...
		ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(ОповещениеОбУстановке, , Ложь);
	КонецЕсли;
...
// Инициализирует структуру параметров для асинхронного получения присоединенного файла.
//
// Параметры:
//  ВидДействия - Строка - доступный вид действия получения файла. Вид действия зависит от используемого метода
//                         получения данных файла.
//						   Значение по умолчанию - Неопределено.
//  ИмяМетода	- Строка - имя метода общего модуля, используемого для получения данных присоединенного файла.
//						   Значение по умолчанию - Неопределено.
//						   Предусмотрено использование следующих методов получения данных присоединенного файла:
//						   1) РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный.
//								Доступные виды действий:
//								а) "ОткрытьФайл".
//								б) "ОткрытьВерсиюФайла".
//								в) "ОткрытьФайлСОповещением".
//								г) "ВыбратьРежимИРедактироватьФайл".
//						   2) РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный.
//								Доступные виды действий:
//								а) "СохранитьКак".
//						   3) РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный.
//								Доступные виды действий:
//								а) "ОткрытьФайл".
//								б) "СохранитьКак".
//
// Возвращаемое значение:
//   Структура:
//    * АсинхронноеПолучениеФайла	 - Булево - признак асинхронного получения данный файла. Проверяется наличие данного свойства
//									 в структуре параметров.
//                                   Значение по умолчанию - Ложь.
//    * ПрисоединенныйФайл			 - см. РаботаСФайлами.ДвоичныеДанныеФайла.ПрисоединенныйФайл.
//    * ВерсияСсылка				 - СправочникСсылка.ВерсииФайлов - версия файла, по которой формируются данные, если не указан,
//                                   то по текущей версии или файлу.
//						   			 Значение по умолчанию - Неопределено.
//    * ПроверятьНаличиеФайлаВАрхиве - Булево - если передать Ложь, то в методах получения данных присоединенного файла
//                                              не будет
//									 выполняться проверка размещения файла в файловом архиве. Используется в тех случаях, когда 
//									 обращение к присоединенному файлу выполняется из формы объекта метаданных, в которой уже была
//									 выполнена проверка наличия файла в файловом архиве.
//                                   Значение по умолчанию - Истина.
//    * ФайлВАрхиве					 - Булево - используется в ситуации, когда свойство ПроверятьНаличиеФайлаВАрхиве равно Ложь.
//									 В этой ситуации данное свойство содержит результат проверки наличия файла в файловом архиве.
//                                   Значение по умолчанию - Ложь.
//    * ФормаВладелец				 - ФормаКлиентскогоПриложения - форма, из которой выполняется обращение к присоединенному файлу.
//    * ИмяМетодаПолученияФайла		 - Строка - имя серверного метода общего модуля, используемого для получения данных присоединенного
//									 файла.
//									 Значение по умолчанию - "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный".
//    * ПараметрыМетода				 - Структура - перечень параметров метода, указанного в свойстве ИмяМетодаПолученияФайла.
//    * ПараметрыДействия			 - Структура - параметры действия получения данных присоединенного файла. Всегда содержит свойство:
//                   * ВидДействия - Строка - значение параметра функции ВидДействия.
//									 Дополнительные параметры действий добавляются в методе РаботаСФайламиКлиент.ДополнитьПараметрыДействияСФайлом.
//
Функция ПараметрыАсинхронногоПолученияФайла(Знач ВидДействия = Неопределено, Знач ИмяМетода = Неопределено) Экспорт
	
	Если ИмяМетода = Неопределено Тогда
		ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный";
	КонецЕсли;
	
	Если ВидДействия = Неопределено Тогда
		Если ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный" Тогда
			ВидДействия = "СохранитьКак";
		Иначе
			ВидДействия = "ОткрытьФайл";
		КонецЕсли;
	КонецЕсли;
	
	Результат = Новый Структура;
	Результат.Вставить("АсинхронноеПолучениеФайла"		, Ложь);
	Результат.Вставить("ПрисоединенныйФайл");
	Результат.Вставить("ВерсияСсылка"					, Неопределено);
	Результат.Вставить("ПроверятьНаличиеФайлаВАрхиве"	, Истина);
	Результат.Вставить("ФайлВАрхиве"					, Ложь);
	Результат.Вставить("ФормаВладелец");

	Результат.Вставить("ИмяМетодаПолученияФайла", ИмяМетода);
	Результат.Вставить("ПараметрыМетода"		, Новый Структура);	
	Результат.Вставить("ПараметрыДействия"		, Новый Структура);
	
	Результат.ПараметрыДействия.Вставить("ВидДействия", ВидДействия);

	Если ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный" Тогда
		// Возможные варианты действий
		// "ОткрытьФайл"/"ОткрытьВерсиюФайла"/"ОткрытьФайлСОповещением"/"ВыбратьРежимИРедактироватьФайл"
		ПараметрыМетода = Результат.ПараметрыМетода;
		ПараметрыМетода.Вставить("ПрисоединенныйФайл");
		ПараметрыМетода.Вставить("ВерсияФайла");
		ПараметрыМетода.Вставить("ИдентификаторФормы"		, Неопределено);
		ПараметрыМетода.Вставить("РабочийКаталогВладельца"	, Неопределено);
		ПараметрыМетода.Вставить("ПредыдущийАдресФайла"		, Неопределено);
		ПараметрыМетода.Вставить("ПараметрыПолученияФайла"	, Неопределено);

	ИначеЕсли ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный" Тогда
		// Возможные варианты действий
		// "СохранитьКак"
		ПараметрыМетода = Результат.ПараметрыМетода;
		ПараметрыМетода.Вставить("ПрисоединенныйФайл");
	...
	ИначеЕсли ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный" Тогда
		// Возможные варианты действий
		// "ОткрытьФайл"/"СохранитьКак"
		ПараметрыМетода = Результат.ПараметрыМетода;
		ПараметрыМетода.Вставить("ПрисоединенныйФайл");
		ПараметрыМетода.Вставить("ИдентификаторФормы"				, Неопределено);
		ПараметрыМетода.Вставить("ПолучатьСсылкуНаДвоичныеДанные"	, Истина);
		ПараметрыМетода.Вставить("ДляРедактирования"				, Ложь);
		ПараметрыМетода.Вставить("ПараметрыПолученияФайла"			, Неопределено);

	КонецЕсли;

	ДополнитьПараметрыДействияСФайлом(Результат);
...
Процедура НапечататьФайлыВыполнение(ОбработчикРезультата, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.НомерФайла >= ПараметрыВыполнения.ДанныеФайлов.Количество() Тогда
		Возврат;
	КонецЕсли;
	ПараметрыВыполнения.ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляПечати(
		ПараметрыВыполнения.ДанныеФайлов[ПараметрыВыполнения.НомерФайла],
...
#КонецЕсли
	...
	Если РаботаСФайламиСлужебныйКлиент.Расширение1СПредприятияПодключено() Тогда
		Обработчик = Новый ОписаниеОповещения("НапечататьФайлПослеПолученияВерсииВРабочийКаталог", ЭтотОбъект, 
			ПараметрыВыполнения);
		РаботаСФайламиСлужебныйКлиент.ПолучитьФайлВерсииВРабочийКаталог(Обработчик, ПараметрыВыполнения.ДанныеФайла,
			"", ПараметрыВыполнения.УникальныйИдентификатор);
	Иначе
		ПараметрыВыполнения.ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
			ПараметрыВыполнения.ДанныеФайлов[ПараметрыВыполнения.НомерФайла], Неопределено);
...
Процедура ОткрытьФормуНастройкиСканированияЗавершение(РезультатПроверкиИнициализации, ПараметрыВыполнения) Экспорт
	...
	ОповещениеПродолжения = Новый ОписаниеОповещения("ОткрытьФормуНастройкиСканированияПослеВключенияЖурнала", 
		ЭтотОбъект, КомпонентаУстановлена);
	
	РаботаСФайламиСлужебныйКлиент.ВключитьЖурналСканирования(РезультатПроверкиИнициализации.ПодключаемыйМодуль, 
...
Процедура ДоступнаКомандаСканироватьЗавершение(РезультатПроверкиИнициализации, ОповещениеРезультата) Экспорт
	...
	ОповещениеЗавершения = Новый ОписаниеОповещения("ДоступнаКомандаСканироватьПослеВключенияЖурнала", ЭтотОбъект, 
		РезультатПроверкиИнициализации);
...
Процедура ПослеПроверкиНаличияПрограммыКонвертации(РезультатЗапуска, Контекст) Экспорт
	Если СтрНайти(РезультатЗапуска.ПотокВывода, "ImageMagick") = 0  Тогда
		...
		НСтр("ru='Ошибочно указан путь к приложению %1.
		|Объединение многостраничных документов выполняется штатными средствами.
...
Процедура КомандаУправленияПрисоединеннымиФайламиЗавершение(Форма, Команда, ВладелецПрисоединенныхФайлов)
	...
	Если СтрНачинаетсяС(ИмяКоманды, "ОткрытьСписок") Тогда
		...
		Если ИспользоватьДОДляХраненияФайловОбъекта Тогда
			...
			// ИнтеграцияС1СДокументооборотом
			РаботаСФайламиСлужебныйКлиент.ОткрытьФормуПрисоединенныеФайлы1СДокументооборот(
				ВладелецПрисоединенныхФайлов,
				Форма);
			// Конец ИнтеграцияС1СДокументооборотом
...
Процедура ВыполнитьДействиеСФайлом(ПараметрыВыполнения, ОбработчикЗавершения)
	...
	Если ЗначениеЗаполнено(Размещение) Тогда
		...
		Если ПараметрыВыполнения.Действие = "ПросмотретьФайл" Тогда
			...
			ПараметрыПолученияФайла = ПараметрыАсинхронногоПолученияФайла("ОткрытьФайл");
			ПараметрыПолученияФайла.ПрисоединенныйФайл	= Размещение;
			ПараметрыПолученияФайла.ФормаВладелец		= Форма;

			ОткрытьФайл(ПараметрыПолученияФайла);
...
Процедура ДополнитьПараметрыДействияСФайлом(ПараметрыПолученияФайла)

	ИмяМетода			= ПараметрыПолученияФайла.ИмяМетодаПолученияФайла;
	ПараметрыДействия	= ПараметрыПолученияФайла.ПараметрыДействия;
	ВидДействия			= ПараметрыДействия.ВидДействия;

	Если ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный" Тогда
		
		Если ВидДействия = "ОткрытьФайл" Тогда
			
			ПараметрыДействия.Вставить("ОповещатьЕслиЗашифрован", Ложь);
			ПараметрыДействия.Вставить("ФайлРедактируется"		, Ложь);
		ИначеЕсли ВидДействия = "ОткрытьВерсиюФайла" Тогда
			
			ПараметрыДействия.Вставить("УникальныйИдентификатор");
		ИначеЕсли ВидДействия = "ВыбратьРежимИРедактироватьФайл" Тогда

			ПараметрыДействия.Вставить("ФормаВладелец");
			ПараметрыДействия.Вставить("ДоступностьКомандыРедактировать");
		ИначеЕсли ВидДействия = "ОткрытьФайлСОповещением" Тогда
			
		КонецЕсли;
	ИначеЕсли ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный" Тогда
		Если ВидДействия = "ОткрытьФайл" Тогда
			ПараметрыДействия.Вставить("ДляРедактирования"	, Ложь);
		КонецЕсли;
	ИначеЕсли ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный" Тогда
