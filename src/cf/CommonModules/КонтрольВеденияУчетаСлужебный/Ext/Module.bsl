...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если ЗначениеЗаполнено(ИнформацияОПоследнейПроверке.ДатаПоследнейПроверки) Тогда
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подсказка, ИнформацияОПоследнейПроверке.ДатаПоследнейПроверки);
		Иначе
			Подсказка = "";
		КонецЕсли;
		Дело = ТекущиеДела.Добавить();
...
// Учитываются только системные проверки.
//
Функция ИнформацияОПоследнейПроверкеВеденияУчета() Экспорт
...
// Размещает группу индикатора ошибок в форме.
//
// Параметры:
//  ФормаКлиентскогоПриложения     - ФормаКлиентскогоПриложения - Форма объекта на которой нужно разместить группу индикатора.
...
Процедура ВыполнитьПроверкиВФоновомЗадании(Знач ПараметрыПроверки, АдресХранилища = Неопределено) Экспорт
	...
	Проверки = ПараметрыПроверки.Проверки;
	Для Каждого Проверка Из Проверки Цикл
