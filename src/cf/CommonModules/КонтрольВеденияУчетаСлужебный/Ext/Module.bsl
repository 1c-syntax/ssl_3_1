...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Обновляются правила проверки учета в соответствие с изменениями в новой версии приложения.
...
Процедура ПриРегистрацииОбработчиковВыгрузкиДанных(ТаблицаОбработчиков) Экспорт
	...
	Обработчик.ОбъектМетаданных = Метаданные.РегистрыСведений.КлючиДоступаКРегиструРезультатыПроверкиУчета;
	Обработчик.Обработчик = РегистрыСведений.КлючиДоступаКРегиструРезультатыПроверкиУчета;
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ОписаниеПроверок = КонтрольВеденияУчетаСлужебныйПовтИсп.ПроверкиВеденияУчета();
	Для Каждого Проверка Из ОписаниеПроверок.Проверки Цикл
		ИменаПроцедур.Вставить(Проверка.ОбработчикПроверки);
	КонецЦикла;
	
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик проверки ведения учета ""%1"".'");
	
	// Исключения.
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.КонтрольВеденияУчетаСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПроверку";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(СтрокаПроверки.ОбработчикПроверки, ПараметрыОбработчика)";
...
Функция ИнформацияОПоследнейПроверкеВеденияУчета(ГруппаПроверок = Неопределено) Экспорт
	...
	Иначе
		ВремяСПоследнегоЗапуска = (ТекущаяДатаСеанса() - Результат.ДатаПоследнейПроверки) / (60 * 60 * 24 * 30);
...
Функция ПоследняяПроверкаОбъекта(Ссылка) Экспорт
	...
	Запрос.УстановитьПараметр("ПроблемныйОбъект", Ссылка);
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ИгнорироватьПроблему <> Значение Тогда
				...
				Если ОписаниеПроблемы.Свойство("КлючУникальности") Тогда
					НаборЗаписей.Отбор.КлючУникальности.Установить(ОписаниеПроблемы.КлючУникальности);
				КонецЕсли;
...
#Область ИнформацияОРегистрах

Функция СсылочныеПоляРегистраСведений(ОбъектМетаданных)
	ИнформацияОРегистре = ИнформацияОНезависимомРегистре(ОбъектМетаданных).ИнформацияОРегистре;
	Результат = НовоеОписаниеПолей();
	Если ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
		Поле = Результат.Добавить();
		Поле.Имя = "Регистратор";
		Поле.ПредставлениеВОписание = ПредставлениеВОписание(ОбъектМетаданных.СтандартныеРеквизиты.Регистратор);
	КонецЕсли;
	
	Для Каждого ИнформацияОРеквизите Из ИнформацияОРегистре Цикл
		Если ИнформацияОРеквизите.СодержитСсылочныйТип Тогда
			Поле = Результат.Добавить();
			Поле.Имя = ИнформацияОРеквизите.ИмяМетаданных;
			Поле.ПредставлениеВОписание = ИнформацияОРеквизите.ПредставлениеВОписание;
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
КонецФункции

Функция ПредставлениеВОписание(МетаданныеПоля, Постфикс = "")
	Результат = МетаданныеПоля.Синоним;
	Если ЗначениеЗаполнено(Результат) Тогда
		СинонимБезРазделителей = СтрЗаменить(МетаданныеПоля.Синоним, " ", "");
		СинонимБезРазделителей = СтрЗаменить(СинонимБезРазделителей, ":", "");
		СинонимБезРазделителей = СтрЗаменить(СинонимБезРазделителей, "-", "");
		СинонимБезРазделителей = СтрЗаменить(СинонимБезРазделителей, "(", "");
		СинонимБезРазделителей = СтрЗаменить(СинонимБезРазделителей, ")", "");
		СинонимОбразованОтИмени = НРег(СинонимБезРазделителей) = НРег(МетаданныеПоля.Имя);
		Если НЕ СинонимОбразованОтИмени Тогда
			Результат = СтрШаблон("%1 (%2)", МетаданныеПоля.Синоним, МетаданныеПоля.Имя);	
		КонецЕсли;
	Иначе
		Результат = МетаданныеПоля.Имя;
	КонецЕсли;
...
Процедура ДобавитьИнформациюОРеквизитах(ИнформацияОРегистре, МетаЭлементы, ТипМетаданныхВИменительномПадеже)
	Для Каждого МетаданныеПоля Из МетаЭлементы Цикл
		ИнформацияОРеквизите = ИнформацияОРегистре.Добавить();
		ИнформацияОРеквизите.ТипМетаданныхВИменительномПадеже = ТипМетаданныхВИменительномПадеже;
		ИнформацияОРеквизите.ИмяМетаданных = МетаданныеПоля.Имя;
		ИнформацияОРеквизите.СодержитСсылочныйТип = СодержитСсылочныйТип(МетаданныеПоля);
		ИнформацияОРеквизите.ЭтоИзмерение = ТипМетаданныхВИменительномПадеже = "Измерение";
		ИнформацияОРеквизите.ПредставлениеВОписание = ПредставлениеВОписание(МетаданныеПоля);
	КонецЦикла;	
КонецПроцедуры
...
Функция ИнформацияОНезависимомРегистре(ОбъектМетаданных, ПолучитьИзмерения = Истина, ПолучитьРесурсы = Истина, ПолучитьРеквизиты = Истина)
	...
	ИнформацияОРегистре.Колонки.Добавить("СодержитСсылочныйТип", Новый ОписаниеТипов("Булево"));
	ИнформацияОРегистре.Колонки.Добавить("АлиасПоля");
	ИнформацияОРегистре.Колонки.Добавить("ЭтоИзмерение", Новый ОписаниеТипов("Булево"));
	ИнформацияОРегистре.Колонки.Добавить("ПредставлениеВОписание");
	
	Если ПолучитьИзмерения Тогда
		ДобавитьИнформациюОРеквизитах(ИнформацияОРегистре, ОбъектМетаданных.Измерения, "Измерение");
	КонецЕсли;
	
	Если ПолучитьРесурсы Тогда
		ДобавитьИнформациюОРеквизитах(ИнформацияОРегистре, ОбъектМетаданных.Ресурсы, "Ресурс");
	КонецЕсли;
	
	Если ПолучитьРеквизиты Тогда
		ДобавитьИнформациюОРеквизитах(ИнформацияОРегистре, ОбъектМетаданных.Реквизиты, "Реквизит");
	КонецЕсли;
	
	СписокПолейВыборки = Новый Массив();
	Для Каждого ИнформацияОРеквизите Из ИнформацияОРегистре Цикл
		АлиасПоля = СтрШаблон("%1%2Ссылка", ИнформацияОРеквизите.ИмяМетаданных, ИнформацияОРеквизите.ТипМетаданныхВИменительномПадеже);  // @query-part
		ВыражениеВыбора = СтрШаблон("ПсевдонимЗаданнойТаблицы.%1 Как %2", ИнформацияОРеквизите.ИмяМетаданных, АлиасПоля);  // @query-part
		СписокПолейВыборки.Добавить(ВыражениеВыбора);
	КонецЦикла;

	Результат = Новый Структура;
	Результат.Вставить("ИнформацияОРегистре", ИнформацияОРегистре);
	Результат.Вставить("ПоляВыборки", СтрСоединить(СписокПолейВыборки, ","));
	Возврат Результат;
	
КонецФункции

Функция ЗначенияПозицииЗаписи(ВыборкаРегистра, ПоляПозицииЗаписи)
	Результат = Новый Структура;
	Для Каждого Поле Из ПоляПозицииЗаписи Цикл
		ЗначениеПоля = ВыборкаРегистра[Поле.Имя];
		Результат.Вставить(Поле.Имя, ЗначениеПоля);
	КонецЦикла;
	Возврат Результат;
КонецФункции

Функция ПредставлениеПозицииЗаписи(ВыборкаРегистра, ПоляПозицииЗаписи)
	ЧастиПредставления = Новый Массив();
	Для Каждого Поле Из ПоляПозицииЗаписи Цикл
		ЗначениеПоляВПредставление = ВыборкаРегистра[Поле.Имя];
		Если НЕ ЗначениеЗаполнено(ЗначениеПоляВПредставление) Тогда
			ЗначениеПоляВПредставление = НСтр("ru = '<не указан>'");
		КонецЕсли;
		
		ПредставлениеПоля = СтрШаблон("%1 = %2", Поле.ПредставлениеВОписание, ЗначениеПоляВПредставление);
		Если Поле.Имя = "Регистратор" Тогда
			Шаблон = НСтр("ru = '%1 (номер строки = %2)'");
			ПредставлениеПоля = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, 
				ПредставлениеПоля, Формат(ВыборкаРегистра.НомерСтроки, "ЧГ=;"));
		КонецЕсли;		
		ЧастиПредставления.Добавить(ПредставлениеПоля);
	КонецЦикла;
	Результат = СтрСоединить(ЧастиПредставления, Символы.ПС);
	Возврат Результат;
КонецФункции

Функция ПоляПозицииЗаписи(ОбъектМетаданных)
	Результат = НовоеОписаниеПолей();
	
	ЭтоРегистрСведенийБезРегистратора = ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных)
		И ОбъектМетаданных.РежимЗаписи <> Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору;
	Если ЭтоРегистрСведенийБезРегистратора Тогда
		Если ОбъектМетаданных.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
			Поле = Результат.Добавить();
			Поле.Имя = "Период";
			Поле.ПредставлениеВОписание = ПредставлениеВОписание(ОбъектМетаданных.СтандартныеРеквизиты.Период); 
		КонецЕсли;
		Для Каждого МетаИзмерение Из ОбъектМетаданных.Измерения Цикл			
			Поле = Результат.Добавить();
			Поле.Имя = МетаИзмерение.Имя;
			Поле.ПредставлениеВОписание = ПредставлениеВОписание(МетаИзмерение);
		КонецЦикла;
	Иначе
		Поле = Результат.Добавить();
		Поле.Имя = "Регистратор";
...
Функция ПоляОбязательныхРеквизитов(ОбъектМетаданных)
	Результат = НовоеОписаниеПолей();
	КлассыРеквизитов = СтрРазделить("Измерения,Реквизиты,Ресурсы", ",");
	
	ЭтоРегистрБухгалтерииСКорреспонденцией = ОбщегоНазначения.ЭтоРегистрБухгалтерии(ОбъектМетаданных)
		И ОбъектМетаданных.Корреспонденция;
	Для Каждого КлассРеквизитов Из КлассыРеквизитов Цикл
		Для Каждого МетаданныеПоля Из ОбъектМетаданных[КлассРеквизитов] Цикл
			Если МетаданныеПоля.ПроверкаЗаполнения <> ПроверкаЗаполнения.ВыдаватьОшибку Тогда
				Продолжить;
			КонецЕсли;
			
			ЭтоНеБалансовоеПоле = ЭтоРегистрБухгалтерииСКорреспонденцией
				И (КлассРеквизитов = "Измерения" Или КлассРеквизитов = "Ресурсы")
				И НЕ МетаданныеПоля.Балансовый; 
			Если ЭтоНеБалансовоеПоле Тогда
				Поле = Результат.Добавить();
				Поле.Имя = МетаданныеПоля.Имя + "Дт";
				Поле.ПредставлениеВОписание = ПредставлениеВОписание(МетаданныеПоля, "ДТ");
				
				Поле = Результат.Добавить();
				Поле.Имя = МетаданныеПоля.Имя + "Кт";
				Поле.ПредставлениеВОписание = ПредставлениеВОписание(МетаданныеПоля, "КТ");
			Иначе
				Поле = Результат.Добавить();
				Поле.Имя = МетаданныеПоля.Имя;
				Поле.ПредставлениеВОписание = ПредставлениеВОписание(МетаданныеПоля);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция НовоеОписаниеПолей()
	Результат = Новый ТаблицаЗначений();
	Результат.Колонки.Добавить("Имя");
	Результат.Колонки.Добавить("ПредставлениеВОписание");
	Возврат Результат;	
КонецФункции

Функция ВыборкаРегистра(ПолноеИмя, ПараметрыПроверки, ПоляПозицииЗаписи)
	МенеджерРегистра = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
	
	ЕстьПериод = ПоляПозицииЗаписи.Найти("Период", "Имя") <> Неопределено;
	ДатаНачалаПроверки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПроверки, "ДатаНачалаПроверки");
	Если ЕстьПериод И ЗначениеЗаполнено(ДатаНачалаПроверки) Тогда
		Результат = МенеджерРегистра.Выбрать(ДатаНачалаПроверки);
	Иначе 
		Результат = МенеджерРегистра.Выбрать();
	КонецЕсли;	
	Возврат Результат;
КонецФункции
...
Функция ЭтоБитаяСсылка(ПроверяемыеДанные, ПроверенныеСсылки) 
	...
	ТипПроверяемыхДанных = ТипЗнч(ПроверяемыеДанные);
	Если ТипПроверяемыхДанных = Тип("Число") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ТипПроверяемыхДанных = Тип("Булево") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ТипПроверяемыхДанных = Тип("Строка") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ТипПроверяемыхДанных = Тип("Дата") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ТипПроверяемыхДанных = Тип("УникальныйИдентификатор") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ТипПроверяемыхДанных = Тип("ХранилищеЗначения") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТипПроверяемыхДанных = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		Или ТипПроверяемыхДанных = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если БизнесПроцессы.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов().СодержитТип(ТипПроверяемыхДанных) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура НайтиБитыеСсылкиВРегистрахНакопления(ОбъектМетаданных, ПараметрыПроверки, ПроверенныеСсылки)
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			ТекущийНаборЗаписей = МенеджерРегистра.СоздатьНаборЗаписей(); // РегистрНакопленияНаборЗаписей
			...
			Проблема.ДополнительнаяИнформация = Новый ХранилищеЗначения(ДополнительнаяИнформация, Новый СжатиеДанных(9));
...
// Найти битые ссылки в регистрах сведений.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданныхРегистрСведений
//  ПараметрыПроверки - см. КонтрольВеденияУчета.ОписаниеПроблемы 
//  ПроверенныеСсылки - Соответствие - кэш проверяемых ссылок
//
Процедура НайтиБитыеСсылкиВРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, ПроверенныеСсылки)
	СсылочныеПоляРегистраСведений = СсылочныеПоляРегистраСведений(ОбъектМетаданных);
	Если НЕ ЗначениеЗаполнено(СсылочныеПоляРегистраСведений) Тогда
		Возврат;
	КонецЕсли;
	
	ПоляПозицииЗаписи = ПоляПозицииЗаписи(ОбъектМетаданных);
	ПолноеИмя = ОбъектМетаданных.ПолноеИмя();
	ИдентификаторОбъектаМетаданных = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ПолноеИмя);

	ВыборкаРегистра = ВыборкаРегистра(ПолноеИмя, ПараметрыПроверки, ПоляПозицииЗаписи);
	Пока ВыборкаРегистра.Следующий() Цикл
		Для Каждого Поле Из СсылочныеПоляРегистраСведений Цикл
			ЗначениеПоля = ВыборкаРегистра[Поле.Имя];
			Если НЕ ЭтоБитаяСсылка(ЗначениеПоля, ПроверенныеСсылки) Тогда
				Продолжить;
			КонецЕсли;

			Проблема = ОписаниеПроблемы(ИдентификаторОбъектаМетаданных, ПараметрыПроверки); // @skip-check query-in-loop

			ПредставлениеПоляБитойСсылки = СтрШаблон("%1: %2 (%3)", Поле.ПредставлениеВОписание, ЗначениеПоля, ЗначениеПоля.Метаданные().ПолноеИмя());
			ПредставлениеПозицииЗаписи = ПредставлениеПозицииЗаписи(ВыборкаРегистра, ПоляПозицииЗаписи);
			
			ШаблонУточнения = НСтр("ru = 'Нарушена ссылочная целостность поля:
			|%1
			|
			|Позиция записи:
			|%2'");

			Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонУточнения, 
				ПредставлениеПоляБитойСсылки, ПредставлениеПозицииЗаписи);
				
			ЗначенияПозицииЗаписи = ЗначенияПозицииЗаписи(ВыборкаРегистра, ПоляПозицииЗаписи);
			Проблема.ДополнительнаяИнформация = Новый ХранилищеЗначения(ЗначенияПозицииЗаписи, Новый СжатиеДанных(9));

			ЗаписатьПроблему(Проблема, ПараметрыПроверки); // @skip-check query-in-loop
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура НайтиБитыеСсылкиВРегистрахБухгалтерии(ОбъектМетаданных, ПараметрыПроверки, ВидыСубконто, ПроверенныеСсылки)
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			ТекущийНаборЗаписей = МенеджерРегистра.СоздатьНаборЗаписей(); // РегистрБухгалтерииНаборЗаписей
			ТекущийНаборЗаписей.Отбор.Регистратор.Установить(СтрокаРезультата.РегистраторРеквизитСсылка);
			ТекущийНаборЗаписей.Прочитать();
...
Процедура НайтиБитыеСсылкиВРегистрахРасчета(ОбъектМетаданных, ПараметрыПроверки, ПроверенныеСсылки)
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			ТекущийНаборЗаписей = МенеджерРегистра.СоздатьНаборЗаписей(); // РегистрРасчетаНаборЗаписей
			ТекущийНаборЗаписей.Отбор.Регистратор.Установить(СтрокаРезультата.РегистраторРеквизитСсылка);
			ТекущийНаборЗаписей.Прочитать();
...
Функция ВидыСубконто(ОбъектМетаданных)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Таблица.Ссылка КАК ВидСубконто
	|ИЗ
	|	&ПланВидовХарактеристик КАК Таблица");
...
Процедура НайтиНезаполненныеОбязательныеРеквизитыВРегистрах(ОбъектМетаданных, ПараметрыПроверки)
	...
	КонецЕсли;
	ПоляПозицииЗаписи = ПоляПозицииЗаписи(ОбъектМетаданных);
	Если НЕ ЗначениеЗаполнено(ПоляПозицииЗаписи) Тогда
		Возврат;
	КонецЕсли;
	ПоляОбязательныхРеквизитов = ПоляОбязательныхРеквизитов(ОбъектМетаданных);
	Если НЕ ЗначениеЗаполнено(ПоляОбязательныхРеквизитов) Тогда
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаРегистра.Следующий() Цикл
		...
		Для Каждого Поле Из ПоляОбязательныхРеквизитов Цикл
			ЗначениеПоля = ВыборкаРегистра[Поле.Имя];
			Если НЕ ЗначениеЗаполнено(ЗначениеПоля) Тогда
				ПустыеРеквизиты.Добавить(Поле.ПредставлениеВОписание);
			КонецЕсли;
		КонецЦикла;
		...
		Шаблон = НСтр("ru = 'У записи с полями:
			|%1
			|
			|обнаружены незаполненные данные, обязательные к заполнению:
			|%2'");
		Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
			ПредставлениеПозицииЗаписи, СтрСоединить(ПустыеРеквизиты, Символы.ПС));
...
Процедура НайтиДублиПредопределенныхЭлементов(ВидМетаданных, ПараметрыПроверки)
	...
	Для Каждого ОбъектМетаданных Из ВидМетаданных Цикл
		...
		Если Итератор = 100 Тогда
			// Отбираем по 100 таблиц, чтобы избежать избыточно сложного запроса.
			Запрос.Текст = ТекстВременнойТаблицы + ";" + ТекстВыборки + ИтогиВыборки;
			ПромежуточныйЗапрос = Запрос;
			МассивЗапросов.Добавить(ПромежуточныйЗапрос);
	...
	Если Не ЗначениеЗаполнено(ТекстВременнойТаблицы) И МассивЗапросов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция РасшифровкаЯчейки(Ссылка, ПравилоПроверки, ВидПроверки, УточнениеПроблемы, КлючУникальности) Экспорт
...
Функция ОшибкиЗаполненияОбъекта()
	СообщенияПроверки = ПолучитьСообщенияПользователю(Истина);
	Если ЗначениеЗаполнено(СообщенияПроверки) Тогда
		Тексты = Новый Массив();
		Для Каждого СообщениеПроверки Из СообщенияПроверки Цикл
			Тексты.Добавить(СообщениеПроверки.Текст);
		КонецЦикла;
		Результат = СтрСоединить(Тексты, Символы.ПС);
	Иначе
		Результат = НСтр("ru = 'Для подробной информации откройте форму объекта.'");
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура УдалитьРезультатыПредыдущейПроверки(РезультатПоследнейПроверки)
	...
	Для Каждого СтрокаТаблицы Из РезультатПоследнейПроверки Цикл
		Набор = РегистрыСведений.РезультатыПроверкиУчета.СоздатьНаборЗаписей();
		Набор.Отбор.ПроблемныйОбъект.Установить(СтрокаТаблицы.ПроблемныйОбъект);
		Набор.Отбор.ПравилоПроверки.Установить(СтрокаТаблицы.ПравилоПроверки);
		Набор.Отбор.ВидПроверки.Установить(СтрокаТаблицы.ВидПроверки);
		Набор.Отбор.КлючУникальности.Установить(СтрокаТаблицы.КлючУникальности);
		Набор.Отбор.ИгнорироватьПроблему.Установить(СтрокаТаблицы.ИгнорироватьПроблему);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьВспомогательныеДанныеПоИзменениямКонфигурации");
	Методы.Вставить("ПроверитьНезаполненныеОбязательныеРеквизиты");
	Методы.Вставить("ПроверитьНаличиеПредопределенныхУзловПлановОбмена");
	Методы.Вставить("ПроверитьДублированиеПредопределенныхЭлементов");
	Методы.Вставить("ПроверитьОтсутствующиеПредопределенныеЭлементы");
	Методы.Вставить("ПроверитьЦиклическиеСсылки");
	Методы.Вставить("ПроверитьСсылочнуюЦелостность");
	Методы.Вставить("ВыполнитьПроверки", Истина);
	Методы.Вставить("ВыполнитьПроверку", Истина);
	Методы.Вставить("ПроверитьОбъект", Истина);
	Методы.Вставить("ИсправитьПроблемуЦиклическихСсылокВФоновомЗадании", Истина);
