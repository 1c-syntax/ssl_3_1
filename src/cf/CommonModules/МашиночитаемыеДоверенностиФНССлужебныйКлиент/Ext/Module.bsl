...
Процедура ОткрытьСписокМЧД(ИмяФормы,
	Отборы = Неопределено, ОповещениеОЗакрытии = Неопределено, Владелец = Неопределено) Экспорт
	
	ОткрытьФорму(ИмяФормы, Отборы, Владелец, Отборы,,,ОповещениеОЗакрытии);
...
Процедура ПродолжитьВыгрузкуДоверенностиВФайл(РезультатДиалога, Контекст) Экспорт
	...
	ФайлыДоверенностей = МашиночитаемыеДоверенностиФНССлужебныйВызовСервера.ФайлыДоверенностей(
		Контекст.Доверенности, ДляНалоговыхОрганов, Контекст.ПроверятьДоступностьПерсональныхДанных);
	
	Если Не ЗначениеЗаполнено(ФайлыДоверенностей) Тогда
		Возврат;
	КонецЕсли;
	
	ПродолжитьВыгрузкуДоверенностиВФайлПроверкаУстановкиКомпонент(ФайлыДоверенностей);
	
КонецПроцедуры

Асинх Процедура ПродолжитьВыгрузкуДоверенностиВФайлПроверкаУстановкиКомпонент(ФайлыДоверенностей, ПовторныйВызов = Ложь)
	
	Если Не ПовторныйВызов Тогда
		ОповещениеПриОтсутствииКомпонентРаботыСЭП = Новый ОписаниеОповещения("ПродолжитьВыгрузкуДоверенностиВФайлПовторноПослеУстановкиКомпонент",
			ЭтотОбъект, ФайлыДоверенностей);
...
// Повторный вызов процедуры ПродолжитьВыгрузкуДоверенностиВФайлПроверкаУстановкиКомпонент после установки компонент.
Процедура ПродолжитьВыгрузкуДоверенностиВФайлПовторноПослеУстановкиКомпонент(Результат, ФайлыДоверенностей) Экспорт
	
	Если Результат = Истина Тогда
		ПродолжитьВыгрузкуДоверенностиВФайлПроверкаУстановкиКомпонент(ФайлыДоверенностей, Истина);
	КонецЕсли;
...
Функция КонтекстДляОбработкиОшибкиРР() Экспорт
	...
	Контекст.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Сбой в распределенном реестре доверенностей'"));
	Контекст.Вставить("Форма");
	Возврат Контекст;
КонецФункции

Процедура ВыгрузитьДоверенностиВФайлы(Знач Доверенности, Знач ПроверятьДоступностьПерсональныхДанных) Экспорт
	...
	Если ДляНалоговыхОрганов Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьВыгрузкуДоверенностиВФайл", ЭтотОбъект, 
			Новый Структура("Доверенности, ПроверятьДоступностьПерсональныхДанных", Доверенности, ПроверятьДоступностьПерсональныхДанных));
	...
	Иначе
		ПродолжитьВыгрузкуДоверенностиВФайл(Новый Структура("Значение", Ложь), 
			Новый Структура("Доверенности, ПроверятьДоступностьПерсональныхДанных", Доверенности, ПроверятьДоступностьПерсональныхДанных));
	КонецЕсли;
...
Процедура ВыгрузитьДоверенностьВФайл(Знач Доверенность, Знач ПроверятьДоступностьПерсональныхДанных) Экспорт
	Если ТипЗнч(Доверенность) <> Тип("Массив") Тогда
		Доверенность = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Доверенность);
	КонецЕсли;
	ВыгрузитьДоверенностиВФайлы(Доверенность, ПроверятьДоступностьПерсональныхДанных);
КонецПроцедуры 
