...
//      * ВерсииФорматаОбмена                            - Соответствие из КлючИЗначение - номеров поддерживаемых версий
//                                                                  формата и ссылок на общие модули с логикой обмена
//                                                                  через формат. Используется только для обменов через
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Если ПланыОбменаБСП.Количество() > 0 Тогда
		...
		ЕстьЗацикливание = ОбменДаннымиКонтрольЗацикливания.ЕстьЗацикливание();
...
Функция ПолучитьПараметрыИнформационнойБазы_3_0_2_2(Знач ИмяПланаОбмена, Знач КодУзла, СообщениеОбОшибке, 
	...
	ПарпаметрыИБ = ПараметрыИнформационнойБазы(ИмяПланаОбмена, КодУзла, СообщениеОбОшибке, ДополнительныеПараметры);
	
	Возврат ОбщегоНазначения.ЗначениеВСтрокуXML(ПарпаметрыИБ);
