...
Функция ТекстЗапросаПланыОбменаДляМонитора(ПараметрыЗапроса = Неопределено, ИсключитьПланыОбменаАвтономнойРаботы = Истина) Экспорт
	...
	ИмяВрТаблицы = ?(РезультатВоВременнуюТаблицу, "ПОМЕСТИТЬ ПланыОбменаКонфигурации", "");
	ТекстЗапросаРезультат = СтрЗаменить(ТекстЗапросаРезультат, "// ПомещениеВоВременнуюТаблицу", ИмяВрТаблицы);
