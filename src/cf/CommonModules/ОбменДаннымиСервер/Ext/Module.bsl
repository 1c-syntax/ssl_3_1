...
Функция ЭтоАвтономноеРабочееМесто() Экспорт
	...
	ГлавныйУзелЭтойИнформационнойБазы = ГлавныйУзел();
	Возврат ГлавныйУзелЭтойИнформационнойБазы <> Неопределено
		И ОбменДаннымиПовтИсп.ЭтоУзелАвтономногоРабочегоМеста(ГлавныйУзелЭтойИнформационнойБазы);
									...
									Если ЗначениеЗаполнено(ЗначениеОтбора) Тогда
										...
										Иначе
											Если ВРег(СтрЗаменить(ТипЗначенияОтбора, " ", "")) = "УНИКАЛЬНЫЙИДЕНТИФИКАТОР" Тогда
												ОбъектКОбработке.Отбор[ИмяОтбора].Установить(XMLЗначение(Тип("УникальныйИдентификатор"), ЗначениеОтбора));
											Иначе
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект) Экспорт
	...
	Для Каждого СтрокаДвижения Из ТаблицаДвижений Цикл
		...
		Если Не ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, СтрокаДвижения.Имя);
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	...
	Для Каждого СтрокаДвижения Из МассивОбрабатываемыхСтрокТаблицыДвижений Цикл		
		...
		Исключение
			...
			ТекстИсключения = НСтр("ru = 'Операция не выполнена: %1
				|%2'");
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, СтрокаДвижения.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение ТекстИсключения;
		КонецПопытки;
	КонецЦикла;
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ru = 'Синхронизация данных с другими программами'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция ПараметрыКонтекстаПолученияВариантовНастроек(ИмяКорреспондента, ВерсияКорреспондента, КорреспондентВМоделиСервиса)
	Возврат Новый Структура("ИмяКорреспондента, ВерсияКорреспондента, КорреспондентВМоделиСервиса",
				ИмяКорреспондента, ВерсияКорреспондента, КорреспондентВМоделиСервиса);
КонецФункции
...
//   ИмяКорреспондента      - Строка - Имя конфигурации корреспондента.
// Возвращаемое значение:
//   Структура - подробнее см. комментарий к функции ОписаниеВариантаНастройкиОбменаПоУмолчанию.
...
//  ВерсияКорреспондента - Строка - версия конфигурации-корреспондента.
// Возвращаемое значение: Структура.
Функция ПараметрыКонтекстаПолученияОписанияВариантаНастройки(ИмяКорреспондента, ВерсияКорреспондента)
	Возврат Новый Структура("ВерсияКорреспондента, ИмяКорреспондента",
...
Функция ШаблонИмениФайлаСообщения(ТекущийУзелПланаОбмена, УзелИнформационнойБазы, ЭтоИсходящееСообщение, Транслитерация = Ложь, ИспользоватьВиртуальныйКодУзлаПриПолучении = Ложь) Экспорт
	...
	Если Транслитерация Тогда
		ИмяФайлаСообщения = СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(ИмяФайлаСообщения);
	КонецЕсли;
...
Процедура ПроверитьКодыУзлов(РезультатАнализаДанныхКЗагрузке, УзелИнформационнойБазы) Экспорт
	Если НЕ ЭтоПланОбменаXDTO(УзелИнформационнойБазы) Тогда
		Возврат;
	КонецЕсли;
	...
	Если УзелКорреспондентаПерекодирован Тогда
		...
		Если ЗначениеЗаполнено(ПсевдонимПредопределенногоУзла)
			...
			КодПредопределенногоУзла = КодПредопределенногоУзлаПланаОбмена(ИмяПланаОбмена);
			Если СокрЛП(КодПредопределенногоУзла) = РезультатАнализаДанныхКЗагрузке.To Тогда
				ОбменДаннымиСлужебный.УдалитьНаборЗаписейВРегистреСведений(Новый Структура("Корреспондент", УзелИнформационнойБазы),
...
Функция КоличествоПроблемОбменаДанными(УзлыОбмена = Неопределено)
	...
	ПараметрыПоискаПроблем.ТипПроблемы = Новый Массив;
	ПараметрыПоискаПроблем.ТипПроблемы.Добавить(Перечисления.ТипыПроблемОбменаДанными.НезаполненныеРеквизиты);
	ПараметрыПоискаПроблем.ТипПроблемы.Добавить(Перечисления.ТипыПроблемОбменаДанными.НепроведенныйДокумент);
	ПараметрыПоискаПроблем.ТипПроблемы.Добавить(Перечисления.ТипыПроблемОбменаДанными.ОшибкаПроверкиСконвертированногоОбъекта);
...
Процедура ПолучитьСостоянияОбменовДанными(МенеджерВременныхТаблиц)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		|	СостоянияОбменовДанными.ДатаОкончания КАК ДатаНачала,
...
Процедура ОбработчикПриВыгрузкеДанныхБСП(СтандартнаяОбработка,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОбменСообщениями") Тогда
		...
		МодульОбменСообщениямиВнутренний.ПриВыгрузкеДанных(
			СтандартнаяОбработка,
			Получатель,
			ИмяФайлаСообщения,
			ДанныеСообщения,
...
Процедура ОбработчикПриЗагрузкеДанныхБСП(СтандартнаяОбработка,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОбменСообщениями") Тогда
		МодульОбменСообщениямиВнутренний = ОбщегоНазначения.ОбщийМодуль("ОбменСообщениямиВнутренний");
		МодульОбменСообщениямиВнутренний.ПриЗагрузкеДанных(
			СтандартнаяОбработка,
			Отправитель,
			ИмяФайлаСообщения,
			ДанныеСообщения,
...
Процедура ЗаполнитьИдентификаторыСсылокВДереве(СтрокиДерева)
	...
	Для Каждого Строка Из СтрокиДерева Цикл
		Строка.Идентификатор = XMLСтрока(Строка.Ссылка);
...
//                                                                  шаблонов с заполненным настройками планов обмена
...
//      * ОтборыКорреспондента                                  - Структура - Отборы на узле плана обмена, подлежат заполнению
//                                                                          значениями по умолчанию для базы корреспондента.
//      * ЗначенияПоУмолчанию                                   - Структура - Значения по умолчанию на узле плана обмена. 
//      * ЗначенияПоУмолчаниюКорреспондента                     - Структура - Значения по умолчанию на узле для базы корреспондента. 
...
//      * ИмяФормыОтборов                                       - Строка - Имя используемой формы настройки отборов на узле. 
//      * ИмяФормыЗначенийПоУмолчанию                           - Строка - Имя используемой формы значений по умолчанию
//                                                                         на узле.
//      * ИмяФормыОтборовКорреспондента                         - Строка - Имя используемой формы настройки отборов на
//                                                                         узле базы корреспондента.
//      * ИмяФормыЗначенийПоУмолчаниюКорреспондента             - Строка - Имя используемой формы значений по умолчанию
//                                                                         на узле базы корреспондента.
//      * ИмяФормыОбщиеДанныеУзлов                              - Строка - Имя используемой формы настроек узлов. 
//      * ИмяФайлаНастроекДляПриемника                          - Строка - Имя файла по умолчанию для сохранения
...
//      * ДополнительныеДанныеДляКорреспондента                 - Структура - дополнительные данные, которые будут
//                                                                         использованы в базе-корреспонденте при
//                                                                         настройке обмена. Могут быть использованы в обработчике
//                                                                        ПриСозданииНаСервере в форме настройки узла
//                                                                        плана обмена ФормаНастройкиЗначенийПоУмолчаниюБазыКорреспондента
...
//                                                                         информационной базе.
//      * ПояснениеДляНастройкиПараметровУчетаКорреспондента    - Строка - Пояснение о последовательности действий пользователя 
...
//                                                                         базы-корреспондента в помощнике создания обмена
//      * ОтображатьЗначенияПоУмолчаниюНаУзлеБазыКорреспондента - Булево - Признак отображения значений по умолчанию
//                                                                         базы-корреспондента в помощнике создания обмена
//      * ИспользуемыеТранспортыСообщенийОбмена                 - Массив - Перечень используемых транспортов сообщений.
//                                                                         Если не заполнен, будут доступны все
...
//
Функция ОписаниеВариантаНастройкиОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ОписаниеВарианта.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными",        МетаданныеПланаОбмена.Синоним);
	...
	ОписаниеВарианта.Вставить("ЗаголовокУзлаПланаОбмена",                              МетаданныеПланаОбмена.Синоним);
...
Функция ИнтерактивноеИзменениеВыгрузки(Знач УзелИнформационнойБазы, Знач АдресХранилищаФормы, Знач ЕстьСценарийУзла=Неопределено) Экспорт
	...
	Если ЕстьСценарийУзла Тогда
		Если ЕстьАлгоритмМенеджераПланаОбмена("НастроитьИнтерактивнуюВыгрузку",МетаУзел.Имя) Тогда
...
Функция ПредставлениеДополненияВыгрузкиПоСценариюУзла(Знач ДополнениеВыгрузки)
	...
	Если НЕ ЕстьАлгоритмМенеджераПланаОбмена("ПредставлениеОтбораИнтерактивнойВыгрузки",МетаУзел.Имя) Тогда
		Возврат "";
	КонецЕсли;
