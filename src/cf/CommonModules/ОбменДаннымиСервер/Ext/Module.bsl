...
Процедура ФормаУзлаПриСозданииНаСервере(Форма, Отказ) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ВидимостьНастройкиПодключения = Ложь;
		
		Если Не Форма.Объект.Ссылка.Пустая() Тогда
			
			ВидТранспорта = РегистрыСведений.НастройкиТранспортаОбменаДанными.ВидТранспортаСообщенийОбменаПоУмолчанию(Форма.Объект.Ссылка);
			
			ВидимостьНастройкиПодключения = (ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.WS
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			"ФормаОбщаяКомандаНастройкиПодключения",
			"Видимость",
			ВидимостьНастройкиПодключения);
...
//
Функция ПоместитьФайлВХранилищеВСервисе(Прокси, Знач ИмяФайла, Знач УзелИнформационнойБазы, Знач РазмерЧастиКБ = 1024)
...
Процедура ЗагрузитьМенеджерРегистрацииОбменаДанными(Отказ,
										Знач ИмяПланаОбмена,
										Знач ВидПравил)
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Для Каждого ЗаписьПланаОбмена Из Результат Цикл
		...
		Если ОбменДаннымиПовтИсп.ЕстьМакетПланаОбмена(ИмяПланаОбмена, "ПравилаРегистрации") 
		...
		ИначеЕсли Не ОбменДаннымиПовтИсп.ПравилаРегистрацииВМенеджере(ИмяПланаОбмена) Тогда
		...
		Если ОбменДаннымиПовтИсп.ПравилаРегистрацииВМенеджере(ИмяПланаОбмена) Тогда 
			...
			ЗагрузитьМенеджерРегистрацииОбменаДанными(Отказ, ИмяПланаОбмена, Перечисления.ВидыПравилДляОбменаДанными.ПравилаРегистрацииОбъектов);
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = СтруктураРезультат.Заголовок;
...
Процедура ОжиданиеЗавершенияОперации(СтруктураНастроекОбмена, ПараметрыОбмена, Прокси, ДействиеПриОбменеВЭтойИБ = Неопределено)
	...
	Если ПараметрыОбмена.ИнтервалОжиданияНаСервере = 0 Тогда
		...
		Если ДействиеПриОбменеВЭтойИБ <> Неопределено Тогда
			...
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрШаблон(ШаблонСообщения, ДействиеВКорреспондентеСтрокой), СтруктураНастроекОбмена);
	...
	Пока ПараметрыОбмена.ДлительнаяОперация Цикл // Замена рекурсии
		...
		Пауза(ПараметрыОбмена.ИнтервалОжиданияНаСервере);
			...
			Если СтандартнаяОбработка Тогда
				...
				Попытка
					...
					Если ПараметрыОбмена.ДлительнаяОперация Тогда
						...
						ОжиданиеЗавершенияОперации(СтруктураНастроекОбмена, ПараметрыОбмена, Прокси, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		...
		Если СтруктураНастроекОбмена.ИспользоватьПередачуБольшогоОбъемаДанных Тогда
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) И Не Отказ Тогда
				...
				Попытка
					...
					ИдентификаторФайлаУИД = ПоместитьФайлВХранилищеВСервисе(Прокси, ФайлСообщенияОбмена, УзелИнформационнойБазы, 1024);
					...
					Иначе
						...
						Если ПараметрыОбмена.ДлительнаяОперация Тогда
							...
							ОжиданиеЗавершенияОперации(СтруктураНастроекОбмена, ПараметрыОбмена, Прокси, Перечисления.ДействияПриОбмене.ВыгрузкаДанных);
