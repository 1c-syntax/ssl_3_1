...
Функция ОписаниеВариантаНастройкиОбменаПоУмолчанию(ИмяПланаОбмена) Экспорт
	...
	ОписаниеВарианта.Вставить("ОбъектнаяКонвертацияXDTO",                              Ложь);
	ОписаниеВарианта.Вставить("ИнтервалОбновленияСтратегииПоискаПоКлючам",             0);
...
Процедура ЗавершитьНастройкуСинхронизацииДанных(УзелОбмена) Экспорт
	...
	Попытка
		...
		Если ОбменДаннымиПовтИсп.СпособКонвертацииОбъектовXDTO(УзелОбмена) Тогда
			
			ЕстьДанныеБезСопоставления = РегистрыСведений.ОбъектыКЗагрузке.ЕстьДанныеДляРучногоСопоставления(УзелОбмена);
			НомерПринятогоСообщения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "НомерПринятого");
			
			Если (НомерПринятогоСообщения > 0 И ЕстьДанныеБезСопоставления)
				Или НомерПринятогоСообщения = 0 Тогда
				
				// Нельзя завершать настройку при наличии несопоставленных данных 
				//  или если не получено сообщение для сопоставления
				НастройкаЗавершена = Ложь;
...
Функция НастройкаСинхронизацииЗавершена(УзелОбмена) Экспорт
	...
	Иначе
		...
		ОбменДаннымиПереопределяемый.ПриОпределенииПризнакаОЗавершенииНастройкиСинхронизации(СтандартнаяОбработка, НастройкаЗавершена, УзелОбмена);
		
		Если СтандартнаяОбработка Тогда
			УстановитьПривилегированныйРежим(Истина);
			
			НастройкаЗавершена = РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.НастройкаЗавершена(УзелОбмена);
		КонецЕсли;
...
Функция ТаблицаМонитораОбменаДанными(Знач ПланыОбмена, Знач ДополнительныеСвойстваПланаОбмена = "") Экспорт
	...
	ТекстЗапроса = 
		...
		|	ЕСТЬNULL(ОбщиеНастройкиУзловИнформационныхБаз.ОбъектнаяКонвертацияXDTO, ЛОЖЬ) КАК ОбъектнаяКонвертацияXDTO,
		|	ВЫБОР
...
Функция ПараметрыОбмена() Экспорт
	...
	СтруктураПараметров.Вставить("КлючОбрабатывающегоЗадания", "");
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ПроблемыЗагруженныхОбъектовОбрабатываемыеПользователем", "ОбменДаннымиСлужебный.УстановкаПараметровСеанса");
...
Процедура ПриПолученииСпискаШаблонов(ШаблоныЗаданий) Экспорт
	...
	ШаблоныЗаданий.Добавить("ПроверкаКорректностиЗаписейПообъектногоОбмена");
	ШаблоныЗаданий.Добавить("ЗагрузкаДанныхПообъектногоОбменаБезОбрабатывающихЗаданий");
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиПравилОбмена";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПослеЗагрузкиПравилОбмена)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередВыгрузкойДанных";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПослеЗагрузкиПравилОбмена);";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередПолучениемИзмененныхОбъектов";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПередПолучениемИзмененныхОбъектов, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеВыгрузкиДанных";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПослеВыгрузкиДанных, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередВыгрузкойОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПередВыгрузкойОбъекта, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеВыгрузкиОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПослеВыгрузкиОбъекта, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередКонвертациейОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПередКонвертациейОбъекта, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередОтправкойИнформацииОбУдалении";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Конвертация.ИмяОбработчикаПередОтправкойИнформацииОбУдалении, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередЗагрузкойДанных";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Конвертация.ИмяОбработчикаПередЗагрузкойДанных, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиДанных";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Конвертация.ИмяОбработчикаПослеЗагрузкиДанных)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПередЗагрузкойОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Конвертация.ИмяОбработчикаПередЗагрузкойОбъекта, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Конвертация.ИмяОбработчикаПослеЗагрузкиОбъекта, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПриПолученииИнформацииОбУдалении";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Конвертация.ИмяОбработчикаПриПолученииИнформацииОбУдалении, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиПараметров";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, ""Конвертация_ПослеЗагрузкиПараметров"", ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Конвертация_ПослеПолученияИнформацииОбУзлахОбмена";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Конвертация.ИмяОбработчикаПослеПолученияИнформацииОбУзлахОбмена, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПередВыгрузкойОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКО.ИмяОбработчикаПередВыгрузкой, ПараметрыОбработчика)";

	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПриВыгрузкеОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКО.ИмяОбработчикаПриВыгрузке, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПослеВыгрузкиОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКО.ИмяОбработчикаПослеВыгрузки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПослеВыгрузкиОбъектаВФайлОбмена";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКО.ИмяОбработчикаПослеВыгрузкиВФайл, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПередЗагрузкойОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Правило.ИмяОбработчикаПередЗагрузкой, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПриЗагрузкеОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Правило.ИмяОбработчикаПриЗагрузке, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПослеЗагрузкиОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Правило.ИмяОбработчикаПослеЗагрузки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКО_ПоследовательностьПолейПоиска";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, ИмяОбработчика, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКС_ПередВыгрузкойСвойства";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКС.ИмяОбработчикаПередВыгрузкой, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКС_ПриВыгрузкеСвойства";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКС.ИмяОбработчикаПриВыгрузке, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКС_ПослеВыгрузкиСвойства";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКС.ИмяОбработчикаПослеВыгрузки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКГС_ПередОбработкойВыгрузки";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКГС.ИмяОбработчикаПередОбработкойВыгрузки, ПараметрыОбработчика)";

	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКГС_ПередВыгрузкойСвойства";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКГС.ИмяОбработчикаПередВыгрузкой, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКГС_ПриВыгрузкеСвойства";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКГС.ИмяОбработчикаПриВыгрузке, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКГС_ПослеВыгрузкиСвойства";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКГС.ИмяОбработчикаПослеВыгрузки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПКГС_ПослеОбработкиВыгрузки";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, ПКГС.ИмяОбработчикаПослеОбработкиВыгрузки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПВД_ПослеОбработкиПравила";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Правило.ИмяОбработчикаПослеОбработки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПВД_ПередОбработкойПравила";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Правило.ИмяОбработчикаПередОбработкой, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПВД_ПередВыгрузкойОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Правило.ИмяОбработчикаПередВыгрузкой, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПВД_ПослеВыгрузкиОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаВыгрузки, Правило.ИмяОбработчикаПослеВыгрузки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПОД_ПередОбработкойПравила";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Правило.ИмяОбработчикаПередОбработкой, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПОД_ПередУдалениемОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Правило.ИмяОбработчикаПередУдалением, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_ПОД_ПослеОбработкиПравила";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, Правило.ИмяОбработчикаПослеОбработки, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "Обработка.КонвертацияОбъектовИнформационныхБаз.МодульОбъекта";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчик_Параметры_ПослеЗагрузкиПараметра";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодОбъекта(
	|		ОбработкаЗагрузки, ИмяОбработчика, ПараметрыОбработчика)";
	...
	// Исключения.
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность")
	   И Не ОбщегоНазначения.РазделениеВключено() Тогда
		
		Методы = Новый Соответствие;
		Методы.Вставить("ЗаполнитьОбработчикиРазделенныхДанных");
		Методы.Вставить("УстановитьКодыПредопределенныхУзлов");
		Методы.Вставить("ЗаблокироватьКонечныеТочки");
		Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ОбменДаннымиВМоделиСервиса", Методы);
	КонецЕсли;
...
Процедура ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВоВременноеХранилище(Знач ИмяПланаОбмена, Знач КодУзлаИнформационнойБазы, Адрес) Экспорт
	...
	КаталогВременногоХранилищаФайлов = КаталогВременногоХранилищаФайлов();
	ПолноеИмяФайлаСообщенияОбмена = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, Строка(Новый УникальныйИдентификатор) + ".xml");
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	ЭтоУзелОбъектнойКонвертации = ОбменДаннымиПовтИсп.СпособКонвертацииОбъектовXDTO(ПараметрыОбмена.УзелИнформационнойБазы);
	Если НЕ НастройкаСинхронизацииЗавершена(ПараметрыОбмена.УзелИнформационнойБазы) 
		И НЕ ЭтоУзелОбъектнойКонвертации Тогда
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		...
		Если ПараметрыОбмена.ПолноеИмяФайлаСообщенияОбмена = ""
			...
			КаталогВременногоХранилищаФайлов = КаталогВременногоХранилищаФайлов();
			ПараметрыОбмена.ПолноеИмяФайлаСообщенияОбмена = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, 
				Строка(Новый УникальныйИдентификатор) + ".xml");
	...
	Иначе
		...
		Если СтруктураНастроекОбмена.ОбменПоПравиламКонвертацииОбъектов Тогда
			...
			Если ФормироватьСообщениеОбмена Тогда
				КаталогВременногоХранилищаФайлов = КаталогВременногоХранилищаФайлов();
				ИмяФайлаСообщенияОбмена = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, Строка(Новый УникальныйИдентификатор) + ".xml");
			КонецЕсли;
	...
	Иначе
		...
		Если СтруктураНастроекОбмена.ОбменПоПравиламКонвертацииОбъектов Тогда
			...
			Если ОбменДаннымиПовтИсп.ЭтоПланОбменаXDTO(СтруктураНастроекОбмена.ИмяПланаОбмена) Тогда
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтруктураНастроекОбмена.ДополнительныеПараметры, "КлючОбрабатывающегоЗадания") Тогда
					ПараметрыЗагрузки.Вставить("КлючОбрабатывающегоЗадания", СтруктураНастроекОбмена.ДополнительныеПараметры.КлючОбрабатывающегоЗадания);
				КонецЕсли;
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина) Экспорт
	...
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелИнформационнойБазы, "ПометкаУдаления") = Истина Тогда
...
Процедура ВыполнитьИнициализациюОбработкиОбменаПоПравиламКонвертации(СтруктураНастроекОбмена) Экспорт
	...
	Если СтруктураНастроекОбмена.ДополнительныеПараметры.Свойство("ПараметрыОбработкиКонвертации") Тогда
		Для Каждого КлючЗначение Из СтруктураНастроекОбмена.ДополнительныеПараметры.ПараметрыОбработкиКонвертации Цикл
			ОбработкаОбменаДанными.Параметры.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
	КонецЕсли;
...
//  ПассивныйРежимФайловаяИБ - Булево - признак того, что получение файла происходит в файловой ИБ при настройке WS
//                                        соединения в пассивном режиме
//  УдалитьФайлИзХранилища - Булево - признак определяющий удалять ли информацию о файле сообщения обмена из хранилища.
//
// Возвращаемое значение:
//  Строка - имя файла.
//
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, ПассивныйРежимФайловаяИБ = Ложь, УдалитьФайлИзХранилища = Истина) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		МодульОбменДаннымиВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиВМоделиСервиса");
		МодульОбменДаннымиВМоделиСервиса.ПриПолученииФайлаИзХранилища(ИдентификаторФайла, ИмяФайла, УдалитьФайлИзХранилища);
	...
	Если ОбменДаннымиСлужебный.ПутьКФайлуСодержитНедопустимыеСимволы(ИмяФайла) Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимое имя файла.'");
	КонецЕсли;
...
//  УзелОбмена             - ПланОбменаСсылка - узел обмена, для которого создается файл.
//  Действие               - ПеречислениеСсылка.ДействияПриОбмене - действие при обмене (выгрузка или загрузка).
//
// Возвращаемое значение:
//  УникальныйИдентификатор - идентификатор файла.
//
Функция ПоместитьФайлВХранилище(Знач ИмяФайла,
	Знач ИдентификаторФайла = Неопределено,
	Знач УзелОбмена = Неопределено,
	Знач Действие = Неопределено) Экспорт
	...
	СтруктураЗаписи.Вставить("УзелОбмена", УзелОбмена);
	СтруктураЗаписи.Вставить("Действие", Действие);
	
	Если ОбщегоНазначения.РазделениеВключено()
		И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		
		УдалитьСтарыеФайлыСообщенияОбмена(УзелОбмена, Действие);
...
// Очистка старых файлов сообщений обмена для указанного узла и действия.
//
// Параметры:
//  УзелОбмена - ПланОбменаСсылка - узел обмена, для которого нужно удалить старые файлы.
//  Действие   - ПеречислениеСсылка.ДействияПриОбмене - действие при обмене (выгрузка или загрузка).
//
Процедура УдалитьСтарыеФайлыСообщенияОбмена(Знач УзелОбмена, Знач Действие)
	
	Если УзелОбмена = Неопределено Или Действие = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	|ГДЕ
	|	СообщенияОбменаДанными.УзелОбмена = &УзелОбмена
	|	И СообщенияОбменаДанными.Действие = &Действие";
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				// Игнорируем ошибки удаления файлов, так как они могут быть уже удалены
				Продолжить;
			КонецПопытки;
...
Функция ПолучитьИспользуемыеПланыОбмена() Экспорт
	...
	Если Не ПланыОбменаБСП.Количество() Тогда
	...
	Для Каждого ИмяПланаОбмена Из ПланыОбменаБСП Цикл
		...
		+ "ВЫБРАТЬ ПЕРВЫЕ 1"
		+ Символы.Таб + """" + ИмяПланаОбмена + """ КАК ИмяПланаОбмена"
		+ Символы.ПС + "ИЗ"
		+ Символы.Таб + "ПланОбмена." + ИмяПланаОбмена + " КАК ПланОбмена"
		+ Символы.ПС + "ГДЕ"
		+ Символы.Таб + "НЕ ПланОбмена.ЭтотУзел";
	...
	Пока Выборка.Следующий() Цикл
		МассивПлановОбмена.Добавить(Выборка.ИмяПланаОбмена);
	КонецЦикла;
...
// 
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено, КлючОбрабатывающегоЗадания = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		// ИНИЦИАЛИЗАЦИЯ ОБМЕНА ДАННЫМИ
		СтруктураНастроекОбмена = НастройкиОбменаДанными(Выборка.НастройкаВыполненияОбмена, Выборка.НомерСтроки, КлючОбрабатывающегоЗадания);
...
Процедура ВыполнитьОбменДаннымиПоРегламентномуЗаданию(КодСценарияОбмена) Экспорт
	...
	ИдентификаторФоновогоЗадания = РаботаСРезультатамиОбмена.ИдентификаторТекущегоФоновогоЗаданияСтрокой();
	
	Выборка = РезультатЗапроса.Выбрать();
	Если Выборка.Следующий() Тогда
		
		ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Ложь, Выборка.Ссылка, Неопределено, ИдентификаторФоновогоЗадания);
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(УзелИнформационнойБазы, ПараметрыОбмена, Отказ, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ДополнительныеПараметры.Вставить("КлючОбрабатывающегоЗадания", ПараметрыОбмена.КлючОбрабатывающегоЗадания);
...
Процедура ПослеЧтенияСообщенияОбмена(Знач Получатель, Знач СообщениеОбмена, Знач СообщениеПрочитано,
	...
	Если ОбменДаннымиСлужебный.ПутьКФайлуСодержитНедопустимыеСимволы(СообщениеОбмена) Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимое имя файла.'");
	КонецЕсли;
		...
		Если ОбновитьСообщениеВКэше И ЗначениеЗаполнено(СообщениеОбмена) Тогда
			...
			Если ТипСтруктура Или ТипЗнч(СтароеСообщение) = Тип("ДвоичныеДанные") Тогда
				...
				Если ТипСтруктура Тогда
					Если ОбменДаннымиСлужебный.ПутьКФайлуСодержитНедопустимыеСимволы(СтароеСообщение.ПутьКФайлу) Тогда
						ВызватьИсключение НСтр("ru = 'Недопустимое имя файла.'");
					КонецЕсли;
...
//
Функция НастройкиОбменаДанными(НастройкаВыполненияОбмена, НомерСтроки, КлючОбрабатывающегоЗадания)
	...
	СтруктураНастроекОбмена.ДополнительныеПараметры.Вставить("КлючОбрабатывающегоЗадания", КлючОбрабатывающегоЗадания);
	Возврат СтруктураНастроекОбмена;
...
Процедура ПолучитьОбщиеНастройкиУзловИнформационныхБаз(МенеджерВременныхТаблиц)
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ТекстЗапросаРезультат =
			...
			|	ЕСТЬNULL(ОбщиеНастройкиУзловИнформационныхБаз.ОбъектнаяКонвертацияXDTO, """") КАК ОбъектнаяКонвертацияXDTO,
			|	ЕСТЬNULL(ОбщиеНастройкиУзловИнформационныхБаз.СинхронизацияНедоступна, ЛОЖЬ) КАК СинхронизацияНедоступна
	...
	Иначе
		...
		ТекстЗапросаРезультат =
			...
			|	ЛОЖЬ КАК ОбъектнаяКонвертацияXDTO,
...
Процедура СохранитьСообщениеИзАрхиваВКэш(УзелИнформационнойБазы) Экспорт
	...
	Если ЗначениеЗаполнено(Выборка.ПолноеИмяФайла) Тогда
		Если ОбменДаннымиСлужебный.ПутьКФайлуСодержитНедопустимыеСимволы(Выборка.ПолноеИмяФайла) Тогда
			ВызватьИсключение НСтр("ru = 'Недопустимое имя файла.'");
		КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьПрефиксИнформационнойБазы", Истина);
	Методы.Вставить("УстановитьКодыПредопределенныхУзлов");
	Методы.Вставить("ЗаполнитьОбработчикиРазделенныхДанных");
	Методы.Вставить("УстановитьКоличествоЭлементовВТранзакцииЗагрузкиДанныхПоУмолчанию");
	Методы.Вставить("НастроитьАрхивированиеСообщенийАРМ");
	Методы.Вставить("ЗарегистрироватьТолькоСправочникиДляНачальнойВыгрузкиВФоне", Истина);
	Методы.Вставить("ЗарегистрироватьВсеДанныеКромеСправочниковДляНачальнойВыгрузкиВФоне", Истина);
	Методы.Вставить("ИнтерактивноеИзменениеВыгрузки_СформироватьДеревоЗначений", Истина);
	Методы.Вставить("ИнтерактивноеИзменениеВыгрузки_СформироватьТабличныйДокументПользователя", Истина);
	Методы.Вставить("УдалитьРольНастройкаСинхронизацииДанных");
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ВыполнитьОбменДаннымиПоРегламентномуЗаданию"); 
		Методы.Вставить("ВыполнитьОбменДаннымиСВнешнейСистемой");
	КонецЕсли;
