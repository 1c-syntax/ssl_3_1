...
Процедура ПриСозданииНаСервереПользователя(Отказ, Форма, Объект) Экспорт
	...
	Форма.ПредлагатьОбсуждения = Не Отказ И Не ЗначениеЗаполнено(Объект.ИдентификаторПользователяИБ) И ПредлагатьОбсуждения 
	...
	Иначе
		ВключитьПозднее = НСтр("ru = 'Включить обсуждения также можно позднее из настроек приложения.'");
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса("WSS", "1cdialog.com", 443, 
		НСтр("ru = 'Сервис 1С:Диалог для системы взаимодействия (тематические обсуждения, переписка и видеозвонки между пользователями приложения).'"));
...
Функция ДоступныеИнтеграции() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить("Telegram");
	Результат.Добавить("WebChat");
	Результат.Добавить("Webhook");

	ОбсужденияЛокализация.ПриОпределенииДоступныхИнтеграций(Результат);
	Возврат Результат;
...
Функция ПараметрыИнтеграции() Экспорт
	...
	ПараметрыИнтеграции.Вставить("channel", "");
	
	Возврат ПараметрыИнтеграции;
КонецФункции

Функция ТокенИнтеграции(ОписаниеИнтеграции)
	Перем Токен;
...
Функция ТокенДляЗаписи(ИдентификаторИнтеграции, Токен, ТокенИзменен) Экспорт
	Если ТокенИзменен Тогда
		Возврат Токен;
	КонецЕсли;
	
	ОписаниеИнтеграции = СистемаВзаимодействия.ПолучитьИнтеграцию(ИдентификаторИнтеграции);
	Возврат ТокенИнтеграции(ОписаниеИнтеграции);
КонецФункции

Процедура ОформитьПолеТокена(Элемент, Токен, ОписаниеИнтеграции) Экспорт
	Если ЗначениеЗаполнено(ТокенИнтеграции(ОписаниеИнтеграции)) Тогда
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаблокироватьНедействительныхПользователейВСистемеВзаимодействий");
	Методы.Вставить("ПользователиДляБлокировкиВСистемеВзаимодействий");
