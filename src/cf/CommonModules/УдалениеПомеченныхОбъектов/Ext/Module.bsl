...
Функция УдалитьПомеченныеОбъекты(УдаляемыеОбъекты, РежимУдаления = "Стандартный") Экспорт
	ДопустимыеРежимы = УдалениеПомеченныхОбъектовСлужебный.ДопустимыеРежимыУдаления();
	...
	Результат = Новый Структура("ПрепятствующиеУдалению, Удаленные, НеУдаленные");
	
	// Для отмены задания с клиента
	ИдентификаторЗадания = Неопределено;
	Если  УдаляемыеОбъекты.Количество() > 0 
		И ТипЗнч(УдаляемыеОбъекты[УдаляемыеОбъекты.Количество() -1]) = Тип("УникальныйИдентификатор") Тогда
		
		ИдентификаторЗадания =  УдаляемыеОбъекты[УдаляемыеОбъекты.Количество() -1];
		УдаляемыеОбъекты.Удалить(УдаляемыеОбъекты.Количество() -1);
	КонецЕсли;
...
Функция СсылкиНаУдаляемыеОбъекты(Источник) Экспорт
	...
	Пока (РезультатЗапроса.Следующий()) Цикл
		СсылкиНаУдаляемыеОбъекты.Вставить(РезультатЗапроса.Ссылка, РезультатЗапроса.Представление);
	КонецЦикла;
...
Функция СсылкиНаУдаляемыеОбъектыВРеквизитах(Источник, Реквизиты)
	ЗначенияСсылочныхТипов = Новый Массив;
...
Процедура СсылкиНаУдаляемыеОбъектыВСубконто(Знач Значение, ЗначенияСсылочныхТипов)
	Перем ЗначениеСубконто;
...
Функция СсылкиНаУдаляемыеОбъектовВТабличныхЧастях(Источник, ТабличныеЧасти, ИмяКоллекцииРеквизитов = "Реквизиты")
	ЗначенияСсылочныхТипов = Новый Массив;
...
Функция СсылкиНаУдаляемыеОбъектыВНабореЗаписей(НаборЗаписей, Знач Реквизиты)
	ЗначенияСсылочныхТипов = Новый Массив;
...
Функция СсылкаНаУдаляемыеОбъектыВИсточнике(Источник, МетаданныеИсточника)
	ЗначенияСсылочныхТипов = Новый Массив;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(МетаданныеИсточника, "ТабличныеЧасти") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗначенияСсылочныхТипов, СсылкиНаУдаляемыеОбъектовВТабличныхЧастях(
		Источник, МетаданныеИсточника.ТабличныеЧасти));
	КонецЕсли;

	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(МетаданныеИсточника,
		"СтандартныеТабличныеЧасти") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗначенияСсылочныхТипов, СсылкиНаУдаляемыеОбъектовВТабличныхЧастях(
			Источник, МетаданныеИсточника.СтандартныеТабличныеЧасти, "СтандартныеРеквизиты"));
	КонецЕсли;
	
	Возврат ЗначенияСсылочныхТипов;
КонецФункции

Функция СсылкиНаУдаляемыеОбъектыВРегистрах(Источник, МетаданныеИсточника)
	ЗначенияСсылочныхТипов = Новый Массив;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(МетаданныеИсточника, "Ресурсы") Тогда
		...
		КонецЕсли;
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗначенияСсылочныхТипов, СсылкиНаУдаляемыеОбъектыВНабореЗаписей(
			Источник, Реквизиты));
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(МетаданныеИсточника,"СтандартныеРеквизиты") Тогда
		Если ОбщегоНазначения.ЭтоРегистрБухгалтерии(МетаданныеИсточника) Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(МетаданныеИсточника, "Реквизиты") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ЗначенияСсылочныхТипов,
			СсылкиНаУдаляемыеОбъектыВНабореЗаписей(Источник, МетаданныеИсточника.Реквизиты));
	КонецЕсли;
...
Функция СформироватьСтандартныеРеквизитыРегистраБухгалтерии(МетаданныеИсточника)
	...
	Если МетаданныеИсточника.ПланСчетов.МаксКоличествоСубконто Тогда
