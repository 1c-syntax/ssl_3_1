...
Функция ПараметрыДанныхФайла() Экспорт
	...
	ПараметрыДанных.Вставить("ПараметрыПолученияФайла",		   Неопределено);
	Возврат ПараметрыДанных;
	
КонецФункции

// Инициализирует структуру параметров для получения данных файла. См. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляСохранения.
//
// Возвращаемое значение:
//  Структура:
//    * ВерсияФайла             - СправочникСсылка.ВерсииФайлов - версия файла.
//    * ИдентификаторФормы      - УникальныйИдентификатор - уникальный идентификатор формы.
//    * РабочийКаталогВладельца	- Строка - рабочий каталог владельца файла.
//    * ПараметрыПолученияФайла	- см. РаботаСФайламиКлиентСервер.ПараметрыПолученияФайла
//
Функция ДополнительныеПараметрыДанныеФайлаДляСохранения() Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ВерсияФайла"				,Неопределено);
	ДополнительныеПараметры.Вставить("ИдентификаторФормы"		,Неопределено);
	ДополнительныеПараметры.Вставить("РабочийКаталогВладельца"	,Неопределено);
	ДополнительныеПараметры.Вставить("ПараметрыПолученияФайла"	,Неопределено);
...
Функция НастройкиСканированияПользователя() Экспорт
	...
	Результат.Вставить("ПоказыватьДиалогСканера", Ложь);
	Результат.Вставить("ИмяУстройства", "");
	Результат.Вставить("ФорматСканированногоИзображения");
	Результат.Вставить("СохранятьВPDF", Ложь);
	Результат.Вставить("ФорматХраненияМногостраничный");
	Результат.Вставить("Разрешение");
	Результат.Вставить("Цветность");
	Результат.Вставить("Поворот");
	Результат.Вставить("РазмерБумаги");
	Результат.Вставить("ДвустороннееСканирование", Ложь);
	Результат.Вставить("АвтоматическаяПодачаДокументов", Ложь);
	Результат.Вставить("ИспользоватьImageMagickДляПреобразованияВPDF", Ложь);
	Результат.Вставить("КачествоJPG", 0);
	Результат.Вставить("СжатиеTIFF");
	Результат.Вставить("ПутьКПрограммеКонвертации", "");
	Результат.Вставить("КаталогЖурналаСканирования");
	Результат.Вставить("ИспользоватьКаталогЖурналаСканирования", Ложь);
	Возврат Результат;
КонецФункции
...
Функция ПараметрыПолученияФайла() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("АдресВРодительскомСеансе");
	Результат.Вставить("АдресТекущейВерсииВРодительскомСеансе");
...
//  Булево
//
Функция ХранитьФайлыВТомах(СпособХраненияФайлов) Экспорт

	Возврат СпособХраненияФайлов = "ВТомахНаДиске" Или СпособХраненияФайлов = "ВИнформационнойБазеИТомахНаДиске";
