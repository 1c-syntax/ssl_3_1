...
Процедура ПроверитьПроведенностьДокументов(ОписаниеПроцедурыЗавершения, СписокДокументов, Форма = Неопределено) Экспорт
	...
	ОчиститьСообщения();
	СведенияОПроведенииДокументов = ПодключаемыеКомандыКлиент.СведенияОПроведенииДокументов(СписокДокументов);
	ЕстьНепроведенныеДокументы = СведенияОПроведенииДокументов.НепроведенныеДокументы.Количество() > 0;
	Если ЕстьНепроведенныеДокументы Тогда
		ДополнительныеПараметры.Вставить("НепроведенныеДокументы", СведенияОПроведенииДокументов.НепроведенныеДокументы);
		ДополнительныеПараметры.Вставить("ЕстьПравоПроведения", СведенияОПроведенииДокументов.ЕстьПравоПроведения);
...
Функция ОписаниеПараметровПечати() Экспорт
	...
	Результат.Вставить("ЗаменитьОсновнуюПечатнуюФорму");
	Результат.Вставить("ИдентификаторИзКомплекта");
	Результат.Вставить("Идентификатор");
...
#Область ФункцииИнициализацииИЗакрытияСсылок
...
#Область ФункцияОтображенияПечатнойФормыПользователю
...
Процедура ПереключитьЯзык(Форма, Команда) Экспорт
	...
	Параметры.Вставить("КнопкаФормыВсеДействия", Форма.Элементы.Найти(Команда.Имя+"ВсеДействия"));
...
Процедура ПриИзмененииНастроекМакета(Параметры, ОписаниеОповещения) Экспорт
	
	ОткрытьФорму("Справочник.МакетыПечатныхФорм.Форма.НастройкиМакета",
		Параметры,
		ЭтотОбъект,,,,
		ОписаниеОповещения,
...
// Перед формированием печатной формы проверяет необходимость сохранения наименования основной печатной формы,
// показывает диалог для подтверждения. В зависимости от ответа пользователя модифицирует возвращаемый параметр
// "ОписаниеКоманды". После формирования печатной формы "ОписаниеКоманды" необходимо в неизменном виде передать в
// процедуру УправлениеПечатью.ПриВыполненииКомандыПечати для сохранения наименования.
// 
// Параметры:
//  Объекты              - ЛюбаяСсылка
//                       - Массив из ЛюбаяСсылка - объекты печати.
//  ОписаниеКоманды      - Структура
//                       - Массив из Структура - возвращаемый параметр, ключи структуры соответствуют колонкам
//                                               из таблицы см. УправлениеПечатью.СоздатьКоллекциюКомандПечати.
//  ОбработчикЗавершения - ОписаниеОповещения - содержит описание процедуры, которая будет вызвана после
//                         закрытия диалога о подтверждении замены наименования с параметрами:
//                           * Результат               - Структура
//                                                     - Массив из Структура - модифицированный параметр ОписаниеКоманды
//                           * ДополнительныеПараметры - Произвольный - дополнительные параметры оповещения.
//  Пример:
//		Процедура ВызывающийКод()
//			ОбработчикЗавершения = Новый ОписаниеОповещения("ПродолжитьВызывающийКод", ЭтотОбъект, ПараметрыОповещения);
//			УправлениеПечатьюКлиент.ПередНачаломВыполненияКомандыПечати(Объекты, ОписаниеКоманды, ОбработчикЗавершения);
//		КонецПроцедуры
//		Процедура ПродолжитьВызывающийКод(Результат, ДополнительныеПараметры) Экспорт
//			ОписаниеКоманды = Результат;
//		КонецПроцедуры
//
Процедура ПередНачаломВыполненияКомандыПечати(Объекты, ОписаниеКоманды, ОбработчикЗавершения) Экспорт
	
	Если ТипЗнч(Объекты) <> Тип("Массив") Тогда
		МассивОбъектов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объекты);
	Иначе
		МассивОбъектов = Объекты;
	КонецЕсли;
	
	Если ТипЗнч(ОписаниеКоманды) = Тип("Массив") Тогда
		
		КоличествоОсновныхПечатныхФорм = 0;
		Для Каждого Команда Из ОписаниеКоманды Цикл
			Если Команда.ОсновнаяПечатнаяФорма Тогда
				КоличествоОсновныхПечатныхФорм = КоличествоОсновныхПечатныхФорм + 1;
				ИндексЕдинственнойОсновнойПечатнойФормы = ОписаниеКоманды.Найти(Команда);
			КонецЕсли;
		КонецЦикла;
		
		Если КоличествоОсновныхПечатныхФорм > 1 Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("КомандыПечати", ОписаниеКоманды);
			ПараметрыФормы.Вставить("МассивОбъектов", МассивОбъектов);
			
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("МассивОбъектов", МассивОбъектов);
			ДополнительныеПараметры.Вставить("ОписаниеОповещения", ОбработчикЗавершения);
			ДополнительныеПараметры.Вставить("КомандыПечати", ОписаниеКоманды);
			ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
				"ПослеВыбораКомандыОсновнойПечатнойФормы", ЭтотОбъект, ДополнительныеПараметры);
			
			ОткрытьФорму("ОбщаяФорма.ВыборОсновнойПечатнойФормы",
				ПараметрыФормы, , , , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			
		ИначеЕсли КоличествоОсновныхПечатныхФорм = 1 Тогда
			ПередНачаломВыполненияКомандыПечатиПродолжение(МассивОбъектов, ОписаниеКоманды, ОбработчикЗавершения, ИндексЕдинственнойОсновнойПечатнойФормы);
			
		Иначе
			ВыполнитьОбработкуОповещения(ОбработчикЗавершения, ОписаниеКоманды);
			
		КонецЕсли;
		
	Иначе
		КомандаВМассиве = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеКоманды);
		ПередНачаломВыполненияКомандыПечатиПродолжение(МассивОбъектов, КомандаВМассиве, ОбработчикЗавершения)
	КонецЕсли;
	
КонецПроцедуры

// АПК:299-выкл вызывается из ERP.

// Формирует и выводит на экран или на принтер печатные формы по командам печати.
// 
// Параметры:
//  МассивОбъектов        - Массив из ЛюбаяСсылка - объекты печати.
//  Команды               - Массив из Структура - см. СоздатьКомандуПечати.
//  ОбработчикЗавершения  - ОписаниеОповещения - содержит описание процедуры, которая будет вызвана перед формированием
//                                               печатных форм, если пользователь не отказался от печати. Параметры:
//                         * Результат               - Булево - всегда возвращает Истина.
//                         * ДополнительныеПараметры - Произвольный - дополнительные параметры оповещения.
//  НаПринтер             - Булево - если Истина, то вывод печатных форм будет произведен на принтер, иначе - на экран.
//  ФиксированныйКомплект - Булево - если Истина, то количество экземпляров документов в комплекте будет недоступно для изменения.
//
Процедура ВыполнитьКомандыПечати(МассивОбъектов, Команды, ОбработчикЗавершения, НаПринтер = Ложь, ФиксированныйКомплект = Ложь) Экспорт
	...
	ПараметрыОповещения.Вставить("НаПринтер", НаПринтер);
	ПараметрыОповещения.Вставить("ФиксированныйКомплект", ФиксированныйКомплект);
	ПараметрыОповещения.Вставить("ПереопределитьПользовательскиеНастройкиКоличества", ФиксированныйКомплект);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьВыполнениеКомандПечати", ЭтотОбъект, ПараметрыОповещения);
	ПередНачаломВыполненияКомандыПечати(МассивОбъектов, Команды, ОписаниеОповещения);
	
КонецПроцедуры

// см. УправлениеПечатью.НапечататьВФайл()
Процедура НапечататьВФайл(КомандыПечати, СписокОбъектов, НастройкиСохранения, ОбработчикЗавершения) Экспорт
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОбработчикЗавершения", ОбработчикЗавершения);
	ПараметрыОповещения.Вставить("КомандыПечати", КомандыПечати);
	ПараметрыОповещения.Вставить("СписокОбъектов", СписокОбъектов);
	ПараметрыОповещения.Вставить("НастройкиСохранения", НастройкиСохранения);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьПечататьВФайл", ЭтотОбъект, ПараметрыОповещения); 
	ПередНачаломВыполненияКомандыПечати(СписокОбъектов, КомандыПечати, ОписаниеОповещения);
	
КонецПроцедуры

// Конструктор элемента массива для второго параметра процедуры ВыполнитьКомандыПечати.
// 
// Возвращаемое значение:
//  Структура - содержит необходимые поля для выполнения процедуры ВыполнитьКомандыПечати.
//
Функция СоздатьКомандуПечати() Экспорт
	ШаблонКоманды = УправлениеПечатьюВызовСервера.СоздатьКомандуПечати();
...
Процедура ПриПереключенииЯзыка(Ответ, Параметры) Экспорт
	...
	ОбработатьЭлементыПодменюЕще = Параметры.КнопкаФормыВсеДействия <> Неопределено;
	
	Элементы.Язык.Заголовок = Заголовок;
	КнопкаФормы = Параметры.КнопкаФормы;
	КнопкаФормыВсеДействия = Параметры.КнопкаФормыВсеДействия;
	...
	Если ОбработатьЭлементыПодменюЕще Тогда
		Элементы.ЯзыкВсеДействия.Заголовок = Заголовок;
		...
		Если КнопкаФормыВсеДействия.Родитель = МенюЯзыкВсеДействия Тогда
			КнопкаФормыВсеДействия.Пометка = Истина;
		Иначе
			КнопкаФормыВсеДействия.Видимость = Ложь;
		КонецЕсли;
	...
	Если ЭтоФормаРедактирования Тогда
		...
		Если Не МакетСоздан Тогда
			...
			Если ОбработатьЭлементыПодменюЕще Тогда
				...
				Для Каждого КнопкаЯзыка Из ДобавляемыеЯзыкиВсеДействия.ПодчиненныеЭлементы Цикл
					Если ТипЗнч(КнопкаЯзыка) = Тип("КнопкаФормы") Тогда
						Если СтрЗаканчиваетсяНа(КнопкаЯзыка.ИмяКоманды, ПереключаемыйЯзык) Тогда
							КнопкаЯзыка.Видимость = Истина;
							Прервать;
						КонецЕсли;
					КонецЕсли;
...
Процедура ПерейтиКМакету(ПутьКМакету) Экспорт
	...
	ПараметрыОткрытия = Новый Структура("ПутьКМакету", ПутьКМакету);
	ОткрытьФорму("РегистрСведений.ПользовательскиеМакетыПечати.Форма.МакетыПечатныхФорм", ПараметрыОткрытия);
	
КонецПроцедуры

Процедура ПослеВыбораКомандыОсновнойПечатнойФормы(ИндексКоманды, ДополнительныеПараметры) Экспорт
	
	Если ИндексКоманды <> Неопределено Тогда
		
		ВыбраннаяКоманда = ДополнительныеПараметры.КомандыПечати[ИндексКоманды];
		ВыбраннаяКоманда.ЗаменитьОсновнуюПечатнуюФорму = Истина;
		ВыбраннаяКоманда.ОсновнаяПечатнаяФорма = Ложь;
		
		ПередНачаломВыполненияКомандыПечатиПродолжение(
			ДополнительныеПараметры.МассивОбъектов,
			ДополнительныеПараметры.КомандыПечати,
...
Асинх Процедура ПередНачаломВыполненияКомандыПечатиПродолжение(МассивОбъектов, Команды, ОписаниеОповещения, ИндексКоманды = Неопределено)
	
	Если ИндексКоманды = Неопределено Тогда
		ОписаниеКоманды = Команды[0];
	Иначе
		ОписаниеКоманды = Команды[ИндексКоманды];
	КонецЕсли;
	
	Если Не ОписаниеКоманды.ОсновнаяПечатнаяФорма Тогда //это комплект
		
		ОсновныеПечатныеФормыВКомплекте = УправлениеПечатьюВызовСервера.ОсновнаяПечатнаяФормаВКомплекте(
			МассивОбъектов, ОписаниеКоманды);
		
		КоличествоОсновныхВКомплекте = ОсновныеПечатныеФормыВКомплекте.Количество();
		Если КоличествоОсновныхВКомплекте > 0 Тогда
			ОписаниеКоманды.ОсновнаяПечатнаяФорма = Истина;
			ПерваяОсновная = ОсновныеПечатныеФормыВКомплекте[0];
			ОписаниеКоманды.ИдентификаторИзКомплекта = ПерваяОсновная.Идентификатор;
			ОписаниеКоманды.НаименованиеПечатнойФормы = ПерваяОсновная.НаименованиеПечатнойФормы;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ОписаниеКоманды.ОсновнаяПечатнаяФорма
		И Не ОписаниеКоманды.ЗаменитьОсновнуюПечатнуюФорму
		И Не ПользователиКлиент.ЭтоСеансВнешнегоПользователя() Тогда
		
		СохраненныеНаименования = УправлениеПечатьюВызовСервера.СохраненныеНаименования(МассивОбъектов);
		ЕстьСохраненныеНаименования = СохраненныеНаименования.Количество();
		ТекстВопроса = "";
		ЗадаватьВопрос = Ложь;
		
		Для Каждого СохраненноеНаименование Из СохраненныеНаименования Цикл
			Если ЗначениеЗаполнено(СохраненноеНаименование.Идентификатор)
				И СохраненноеНаименование.Идентификатор <> УправлениеПечатьюКлиентСервер.ИдентификаторБезСпецСимволов(ОписаниеКоманды.Идентификатор) Тогда
				ЗадаватьВопрос = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ЗадаватьВопрос Тогда
			Если СохраненныеНаименования.Количество() = 1 Тогда
				ПервоеНаименование = СохраненныеНаименования[0];
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'По документу ранее уже была сформирована основная печатная форма ""%1"", пользователь ""%2"", дата формирования ""%3"". Заменить сохраненную основную печатную форму для документа?'"),
					ПервоеНаименование.НаименованиеПечатнойФормы,
					ПервоеНаименование.Пользователь,
					Формат(ПервоеНаименование.Дата, "ДЛФ=;"));
			Иначе
				ТекстВопроса = НСтр("ru = 'По одному или нескольким из выбранных документов уже печаталась основная форма. Заменить сохраненную основную печатную форму для документов?'");
			КонецЕсли;
		КонецЕсли;
		...
		Если Не ПустаяСтрока(ТекстВопроса) Тогда
			...
			КнопкиВопроса.Добавить("Заменить", НСтр("ru = 'Заменить'"));
			КнопкиВопроса.Добавить("НеЗаменять", НСтр("ru = 'Не заменять'"));
			
			Ответ = Ждать ВопросАсинх(ТекстВопроса, КнопкиВопроса);
			Если Ответ = "Заменить" Тогда
				ОписаниеКоманды.ЗаменитьОсновнуюПечатнуюФорму = Истина;
			ИначеЕсли Ответ = Неопределено Тогда
				Возврат;
			КонецЕсли;
			
		Иначе
			Если Не ЕстьСохраненныеНаименования Тогда
				ОписаниеКоманды.ЗаменитьОсновнуюПечатнуюФорму = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ИндексКоманды = Неопределено Тогда
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Команды[0]);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Команды);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПродолжитьВыполнениеКомандПечати(Команды, ДополнительныеПараметры) Экспорт
	...
	ПечатныеФормы = УправлениеПечатьюВызовСервера.СформироватьПечатныеФормы(МассивОбъектов, Команды);
			
	Если ДополнительныеПараметры.НаПринтер Тогда
		
		ИменаМакетов = Новый СписокЗначений;
		...
		Для Каждого ПечатнаяФорма Из ПечатныеФормы.КоллекцияПечатныхФорм Цикл
			...
			Если (ТипЗнч(ТабличныйДокумент) = Тип("ТабличныйДокумент")) И (ТабличныйДокумент.ВысотаТаблицы <> 0) Тогда
				СписокПечатныхФорм.Добавить(ТабличныйДокумент, ПечатнаяФорма.СинонимМакета);
				ИменаМакетов.Добавить(ПечатнаяФорма.ИмяМакета, ПечатнаяФорма.СинонимМакета);
			КонецЕсли;
		КонецЦикла;
		
		РаспечататьТабличныеДокументы(СписокПечатныхФорм, ПечатныеФормы.ОбъектыПечати);
		
		// СтандартныеПодсистемы.УчетОригиналовПервичныхДокументов
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.УчетОригиналовПервичныхДокументов") Тогда
			МодульУчетОригиналовПервичныхДокументовКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетОригиналовПервичныхДокументовКлиент");
			МодульУчетОригиналовПервичныхДокументовКлиент.ЗаписатьСостоянияОригиналовПослеПечати(ПечатныеФормы.ОбъектыПечати, ИменаМакетов);
		КонецЕсли;
		// Конец СтандартныеПодсистемы.УчетОригиналовПервичныхДокументов
	...
	Иначе
		ПараметрыОткрытия = УправлениеПечатьюСлужебныйКлиент.ПараметрыОткрытияФормыПечати();
		ПараметрыОткрытия.ПараметрКоманды = МассивОбъектов;
		ПараметрыОткрытия.ОбъектыПечати = ПечатныеФормы.ОбъектыПечати;
		ПараметрыОткрытия.КоллекцияПечатныхФорм = ПечатныеФормы.КоллекцияПечатныхФорм;
		ПараметрыОткрытия.ПараметрыВывода = ПечатныеФормы.ПараметрыВывода;
		
		ПараметрыПечати = Новый Структура;
		ПараметрыПечати.Вставить("ФиксированныйКомплект", ДополнительныеПараметры.ФиксированныйКомплект);
		ПараметрыПечати.Вставить("ПереопределитьПользовательскиеНастройкиКоличества", ДополнительныеПараметры.ПереопределитьПользовательскиеНастройкиКоличества);
		ПараметрыОткрытия.ПараметрыПечати = ПараметрыПечати;
...
Процедура ПродолжитьПечататьВФайл(Команды, ДополнительныеПараметры) Экспорт
	
	Результат = УправлениеПечатьюВызовСервера.НапечататьВФайл(
		Команды, ДополнительныеПараметры.СписокОбъектов, ДополнительныеПараметры.НастройкиСохранения);
