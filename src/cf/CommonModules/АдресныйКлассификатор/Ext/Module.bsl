...
Функция КодРегионаПоНаименованию(Название) Экспорт
	...
	ДанныеОРегионе = АдресныйКлассификаторПовтИсп.ДанныеОРегионе(НаименованиеРегиона);
	Если ДанныеОРегионе <> Неопределено Тогда
		КодСубъектаРФ = ДанныеОРегионе.КодСубъектаРФ;
	КонецЕсли;
...
Функция РазрешенДоступВИнтернет() Экспорт
	...
	Возврат Не АдресныйКлассификаторПовтИсп.ИспользоватьТолькоЗагруженныеДанные();
...
//      * ИдентификаторПомещения - Строка - уникальный идентификационный код помещения дома(здания) адресного объекта.
...
// Получает сведения об адресе по коду ОКТМО (8 или 11 цифр).
// При передаче единичного кода ОКТМО (типа Строка или Число) функция возвращает соответствие, 
// где ключом является исходное значение кода.
// Для каждого кода в качестве значения соответствия возвращается структура с адресными сведениями.
// Если код не найден или некорректен, значением будет Неопределено.
//
// Параметры:
//   ОКТМО - Соответствие из КлючИЗначение: 
//             * Ключ - Произвольный - произвольный уникальный идентификатор для связи с результатом;
//             * Значение - Строка, Число - код ОКТМО;
//         - Строка - код общероссийского классификатора территорий муниципальных образований;
//         - Число - код общероссийского классификатора территорий муниципальных образований.
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - Строка, Число - исходный код ОКТМО (если был передан один код строкой или числом) или ключ из входного соответствия.
//  * Значение - Структура:
...
//    ** КодУчасткаИФНСФЛ - Строка - код территориального участка инспекции ФНС, обслуживающей физических лиц.
//    ** КодУчасткаИФНСЮЛ - Строка - код территориального участка инспекции ФНС, обслуживающей юридические лица.
//  - Неопределено - если передан несуществующий ОКТМО.
//
Функция СведенияПоОКТМО(ОКТМО) Экспорт
	
	Если ТипЗнч(ОКТМО) = Тип("Соответствие") Тогда
		СписокОКТМО = Новый Соответствие(Новый ФиксированноеСоответствие(ОКТМО));
	Иначе
		СписокОКТМО = Новый Соответствие;
		СписокОКТМО.Вставить(ОКТМО, ОКТМО);
	КонецЕсли;
	
	Результат = АдресныйКлассификаторСлужебный.СведенияПоСпискуОКТМО(СписокОКТМО);
	
	Возврат Результат;
	
КонецФункции

// Проверяет корректность ОКТМО бюджетополучателей.
// 
...
Функция ПроверитьОКТМОБюджетополучателя(ОКТМОБюджетополучателей) Экспорт
	...
	СписокОКТМО = ?(ТипЗнч(ОКТМОБюджетополучателей) = Тип("Строка"), 
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОКТМОБюджетополучателей), ОКТМОБюджетополучателей);
		
	Возврат АдресныйКлассификаторСлужебный.ПроверитьОКТМОБюджетополучателя(СписокОКТМО);
	 
КонецФункции

// Возвращает номер часовой зоны РФ согласно федеральному закону от 03.06.2011 N 107-ФЗ (ред. от 14.04.2023) "Об
// исчислении времени". Для адресов, в которых отсутствуют идентификаторы ГАР, может потребоваться подключение к
// интернет-сервису поддержки пользователей. В этом случае необходимо наличие стабильного и постоянного соединения с интернетом.
//
// Параметры:
//    Адрес - Строка - адрес во внутреннем формате JSON или XML, соответствующий структуре XDTO-пакета Адрес.
//
// Возвращаемое значение:
//    Число        - номер часовой зоны РФ от 1 до 11.
//    Неопределено - если не удалось определить часовую зону.
//
Функция НомерЧасовойЗоны(Знач Адрес) Экспорт
	
	Если ТипЗнч(Адрес) <> Тип("Строка") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЧасоваяЗонаРФ = Неопределено;
	ПоляАдреса = АдресныйКлассификаторСлужебный.КонтактнаяИнформацияВСтруктуруJSON(Адрес);
	
	ТипАдреса = ПоляАдреса.addressType;
	Если СтрСравнить(ТипАдреса, "ЕАЭС") = 0 Или СтрСравнить(ТипАдреса, "Иностранный") = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЧасовыеЗоны = РегистрыСведений.АдресныеОбъекты.ЧасовыеЗоныСубъектовРФ();
	
	НайденныеСтроки = Новый Массив;
	
	Если ЗначениеЗаполнено(ПоляАдреса.areaId) Тогда
		Фильтр = Новый Структура;
		Фильтр.Вставить("ИдентификаторРегиона", ПоляАдреса.areaId);
		НайденныеСтроки = ЧасовыеЗоны.НайтиСтроки(Фильтр);
	КонецЕсли;
	
	НаименованиеРегиона = ?(ЗначениеЗаполнено(ПоляАдреса.areaValue), ПоляАдреса.areaValue, ПоляАдреса.area);
	Если НайденныеСтроки.Количество() = 0 И ЗначениеЗаполнено(НаименованиеРегиона) Тогда
		КодРегиона = КодРегионаПоНаименованию(НаименованиеРегиона);
		Если ЗначениеЗаполнено(КодРегиона) Тогда
			Фильтр = Новый Структура;
			Фильтр.Вставить("КодСубъектаРФ", КодРегиона);
			НайденныеСтроки = ЧасовыеЗоны.НайтиСтроки(Фильтр);
		КонецЕсли;
	КонецЕсли;
	
	Если НайденныеСтроки.Количество() = 1 Тогда
		ЧасоваяЗонаРФ = НайденныеСтроки[0].ЧасоваяЗонаРФ;
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		Если Не ЗначениеЗаполнено(ПоляАдреса.munDistrictId) И Не ЗначениеЗаполнено(ПоляАдреса.districtId) Тогда
			АдресныйКлассификаторСлужебный.УстановитьИдентификаторыАдреса(ПоляАдреса);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ПоляАдреса.munDistrictId) И Не ЗначениеЗаполнено(ПоляАдреса.districtId) Тогда
			Возврат Неопределено;
		КонецЕсли;
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если (ЗначениеЗаполнено(ПоляАдреса.munDistrictId) И ПоляАдреса.munDistrictId = НайденнаяСтрока.ИдентификаторРайонаМуниципальный) 
				Или ЗначениеЗаполнено(ПоляАдреса.districtId) И ПоляАдреса.districtId = НайденнаяСтрока.ИдентификаторРайонаАдминистративный Тогда
				ЧасоваяЗонаРФ = НайденнаяСтрока.ЧасоваяЗонаРФ;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ЧасоваяЗонаРФ;
...
// Устарела. Возвращает сведения об адресе по коду ОКТМО, который состоит из 8 или 11 цифр.
//
// Параметры:
