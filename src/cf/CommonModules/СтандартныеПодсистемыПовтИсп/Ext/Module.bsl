...
Функция ОписанияПодсистем() Экспорт
		...
		Если ОписанияПодсистем.ПоИменам.Получить(Описание.Имя) <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при подготовке описаний подсистем:
				           ...
				           |указано имя подсистемы ""%3"", которое уже зарегистрировано ранее.'"),
	...
	Если ОписаниеКонфигурацииНайдено Тогда
		...
		Если Описание.Версия <> Метаданные.Версия Тогда
			...
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	ИначеЕсли Метаданные.Имя = "БиблиотекаСтандартныхПодсистем" Тогда
		ТекстОшибки = НСтр("ru = 'Файл поставки 1С:Библиотека стандартных подсистем не предназначен для создания информационных баз по шаблону. 
...
#Область ИспользованиеИдентификаторовОбъектовМетаданныхКонфигурацииИРасширений
...
#Область ПроцедурыИФункцииРаботыСОбменомДанными
...
Функция ЭтоРазделенныйРежимБезРасширенийОбластиДанных() Экспорт
	...
	ЭтоРазделенныйРежимСеансаБезРазделителей = СтандартныеПодсистемыСервер.ЭтоРазделенныйРежимСеансаБезРазделителей();
	
	Если Не ЭтоРазделенныйРежимСеансаБезРазделителей Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ФоновоеЗадание = ПолучитьТекущийСеансИнформационнойБазы().ПолучитьФоновоеЗадание();
	Если ФоновоеЗадание <> Неопределено
	   И ФоновоеЗадание.РазделениеДанных.Количество() > 0 Тогда
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	Возврат ОбщегоНазначения.ОбщиеПараметрыБазовойФункциональности(Ложь);
...
// Свойства видов объектов метаданных.
//
// Возвращаемое значение:
//  ФиксированнаяСтруктура:
//   * ПоКоллекциям - ФиксированноеСоответствие из КлючИЗначение:
//      ** Ключ     - Строка - имя коллекции объектов метаданных, например, "СПРАВОЧНИКИ", "CATALOGS".
//      ** Значение - см. НовыеСвойстваВидаОбъектов
//
//   * ПоВидамОбъектов - ФиксированноеСоответствие из КлючИЗначение:
//      ** Ключ     - Строка - имя вида объектов метаданных, например, "СПРАВОЧНИК", "CATALOG".
//      ** Значение - см. НовыеСвойстваВидаОбъектов
//
Функция СвойстваВидовОбъектовМетаданных() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПоКоллекциям", Новый Соответствие);
	Результат.Вставить("ПоВидамОбъектов", Новый Соответствие);
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"Подсистемы", "Подсистема", "Subsystems", "Subsystem"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОбщиеМодули", "ОбщийМодуль", "CommonModules", "CommonModule"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ПараметрыСеанса", "ПараметрСеанса", "SessionParameters", "SessionParameter"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"Роли", "Роль", "Roles", "Role"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОбщиеРеквизиты", "ОбщийРеквизит", "CommonAttributes", "CommonAttribute"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ПланыОбмена, Истина, Истина, Ложь,
		"ПланыОбмена", "ПланОбмена", "ExchangePlans", "ExchangePlan"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, КритерииОтбора, Истина, Ложь, Ложь,
		"КритерииОтбора", "КритерийОтбора", "FilterCriteria", "FilterCriterion"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ПодпискиНаСобытия", "ПодпискаНаСобытие", "EventSubscriptions", "EventSubscription"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"РегламентныеЗадания", "РегламентноеЗадание", "ScheduledJobs", "ScheduledJob"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"Боты", "Бот", "Bots", "Bot"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ФункциональныеОпции", "ФункциональнаяОпция", "FunctionalOptions", "FunctionalOption"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ПараметрыФункциональныхОпций", "ПараметрФункциональныхОпций", "FunctionalOptionParameters", "FunctionalOptionsParameter"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОпределяемыеТипы", "ОпределяемыйТип", "DefinedTypes", "DefinedType"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ХранилищаНастроек, Истина, Ложь, Ложь,
		"ХранилищаНастроек", "ХранилищеНастроек", "SettingsStorages", "SettingsStorage"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОбщиеКоманды", "ОбщаяКоманда", "CommonCommands", "CommonCommand"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ГруппыКоманд", "ГруппаКоманд", "CommandGroups", "CommandGroup"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОбщиеФормы", "ОбщаяФорма", "CommonForms", "CommonForm"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОбщиеМакеты", "ОбщийМакет", "CommonTemplates", "CommonTemplate"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ОбщиеКартинки", "ОбщаяКартинка", "CommonPictures", "CommonPicture"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ПакетыXDTO", "ПакетXDTO", "XDTOPackages", "XDTOPackage"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"WebСервисы", "WebСервис", "WebServices", "WebService"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"HTTPСервисы", "HTTPСервис", "HTTPServices", "HTTPService"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, WSСсылки, Ложь, Ложь, Ложь,
		"WSСсылки", "WSСсылка", "WSReferences", "WSReference"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, СервисыИнтеграции, Ложь, Ложь, Ложь,
		"СервисыИнтеграции", "СервисИнтеграции", "IntegrationServices", "IntegrationService"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"ЭлементыСтиля", "ЭлементСтиля", "StyleItems", "StyleItem"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"Стили", "Стиль", "Styles", "Style"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Неопределено, Ложь, Ложь, Ложь,
		"Языки", "Язык", "Languages", "Language"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Константы, Истина, Ложь, Ложь,
		"Константы", "Константа", "Constants", "Constant"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Справочники, Истина, Истина, Ложь,
		"Справочники", "Справочник", "Catalogs", "Catalog"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Документы, Истина, Истина, Ложь,
		"Документы", "Документ", "Documents", "Document"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Последовательности, Ложь, Ложь, Ложь,
		"Последовательности", "Последовательность", "Sequences", "Sequence"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ЖурналыДокументов, Истина, Ложь, Ложь,
		"ЖурналыДокументов", "ЖурналДокументов", "DocumentJournals", "DocumentJournal"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Перечисления, Истина, Истина, Ложь,
		"Перечисления", "Перечисление", "Enums", "Enum"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Отчеты, Истина, Ложь, Ложь,
		"Отчеты", "Отчет", "Reports", "Report"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Обработки, Истина, Ложь, Ложь,
		"Обработки", "Обработка", "DataProcessors", "DataProcessor"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ПланыВидовХарактеристик, Истина, Истина, Ложь,
		"ПланыВидовХарактеристик", "ПланВидовХарактеристик", "ChartsOfCharacteristicTypes", "ChartOfCharacteristicTypes"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ПланыСчетов, Истина, Истина, Ложь,
		"ПланыСчетов", "ПланСчетов", "ChartsOfAccounts", "ChartOfAccounts"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ПланыВидовРасчета, Истина, Истина, Ложь,
		"ПланыВидовРасчета", "ПланВидовРасчета", "ChartsOfCalculationTypes", "ChartOfCalculationTypes"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, РегистрыСведений, Истина, Ложь, Истина,
		"РегистрыСведений", "РегистрСведений", "InformationRegisters", "InformationRegister"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, РегистрыНакопления, Истина, Ложь, Истина,
		"РегистрыНакопления", "РегистрНакопления", "AccumulationRegisters", "AccumulationRegister"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, РегистрыБухгалтерии, Истина, Ложь, Истина,
		"РегистрыБухгалтерии", "РегистрБухгалтерии", "AccountingRegisters", "AccountingRegister"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, РегистрыРасчета, Истина, Ложь, Истина,
		"РегистрыРасчета", "РегистрРасчета", "CalculationRegisters", "CalculationRegister"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, БизнесПроцессы, Истина, Истина, Ложь,
		"БизнесПроцессы", "БизнесПроцесс", "BusinessProcesses", "BusinessProcess"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, Задачи, Истина, Истина, Ложь,
		"Задачи", "Задача", "Tasks", "Task"); // @Non-NLS-1 @Non-NLS-2
	
	ДобавитьСвойстваВидаОбъектов(Результат, ВнешниеИсточникиДанных, Ложь, Ложь, Ложь,
		"ВнешниеИсточникиДанных", "ВнешнийИсточникДанных", "ExternalDataSources", "ExternalDataSource"); // @Non-NLS-1 @Non-NLS-2
	
	ФиксированныйРезультат = Новый Структура;
	ФиксированныйРезультат.Вставить("ПоКоллекциям",
		Новый ФиксированноеСоответствие(Результат.ПоКоллекциям));
	ФиксированныйРезультат.Вставить("ПоВидамОбъектов",
		Новый ФиксированноеСоответствие(Результат.ПоВидамОбъектов));
	
	Возврат Новый ФиксированнаяСтруктура(ФиксированныйРезультат);
	
КонецФункции

// АПК:134-выкл - Количество параметров более 7 для компактного описания и производительности.

// Для функции СвойстваВидовОбъектовМетаданных.
//
// Параметры:
//  Результат - Структура:
//   * ПоКоллекциям - Соответствие
//   * ПоВидамОбъектов - Соответствие
//  Менеджер            - Произвольный - менеджер объектов.
//  ЕстьМодульМенеджера - Булево
//  ЕстьСсылка          - Булево
//  ЕстьКлючЗаписи      - Булево
//  ИмяКоллекцииЯзыкРусский      - Строка - например, "Справочники".
//  ИмяВидаОбъектаЯзыкРусский    - Строка - например, "Справочник".
//  ИмяКоллекцииЯзыкАнглийский   - Строка - например, "Catalogs".
//  ИмяВидаОбъектаЯзыкАнглийский - Строка - например, "Catalog".
//
Процедура ДобавитьСвойстваВидаОбъектов(Результат, Менеджер, ЕстьМодульМенеджера, ЕстьСсылка, ЕстьКлючЗаписи,
			ИмяКоллекцииЯзыкРусский, ИмяВидаОбъектаЯзыкРусский, ИмяКоллекцииЯзыкАнглийский, ИмяВидаОбъектаЯзыкАнглийский)
	
	ЕстьОбъект = ЕстьСсылка И ИмяВидаОбъектаЯзыкАнглийский <> "Enum"
		Или ИмяВидаОбъектаЯзыкАнглийский = "Report"
		Или ИмяВидаОбъектаЯзыкАнглийский = "DataProcessor";
	ЕстьНаборЗаписей = ЕстьКлючЗаписи
		Или ИмяВидаОбъектаЯзыкАнглийский = "Sequence";
	
	Свойства = Новый Структура(СтандартныеПодсистемыПовтИсп.НовыеСвойстваВидаОбъектов()); // см. НовыеСвойстваВидаОбъектов
	Свойства.Менеджер            = Менеджер;
	Свойства.ЕстьМодульМенеджера = ЕстьМодульМенеджера;
	Свойства.ЕстьСсылка          = ЕстьСсылка;
	Свойства.ЕстьКлючЗаписи      = ЕстьКлючЗаписи;
	Если Метаданные.ВариантВстроенногоЯзыка = Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		Свойства.ИмяКоллекции         = ИмяКоллекцииЯзыкРусский;
		Свойства.ИмяВидаОбъекта       = ИмяВидаОбъектаЯзыкРусский;
		Свойства.ИмяТипаСсылки        = ?(ЕстьСсылка,       ИмяВидаОбъектаЯзыкРусский + "Ссылка",       ""); // @Non-NLS
		Свойства.ИмяТипаКлючаЗаписи   = ?(ЕстьКлючЗаписи,   ИмяВидаОбъектаЯзыкРусский + "КлючЗаписи",   ""); // @Non-NLS
		Свойства.ИмяТипаОбъекта       = ?(ЕстьОбъект,       ИмяВидаОбъектаЯзыкРусский + "Объект",       ""); // @Non-NLS
		Свойства.ИмяТипаНабораЗаписей = ?(ЕстьНаборЗаписей, ИмяВидаОбъектаЯзыкРусский + "НаборЗаписей", ""); // @Non-NLS
	Иначе
		Свойства.ИмяКоллекции         = ИмяКоллекцииЯзыкАнглийский;
		Свойства.ИмяВидаОбъекта       = ИмяВидаОбъектаЯзыкАнглийский;
		Свойства.ИмяТипаСсылки        = ?(ЕстьСсылка,       ИмяВидаОбъектаЯзыкАнглийский + "Ref", "");
		Свойства.ИмяТипаКлючаЗаписи   = ?(ЕстьКлючЗаписи,   ИмяВидаОбъектаЯзыкАнглийский + "RecordKey", "");
		Свойства.ИмяТипаОбъекта       = ?(ЕстьОбъект,       ИмяВидаОбъектаЯзыкАнглийский + "Object", "");
		Свойства.ИмяТипаНабораЗаписей = ?(ЕстьНаборЗаписей, ИмяВидаОбъектаЯзыкАнглийский + "RecordSet", "");
	КонецЕсли;
	Свойства = Новый ФиксированнаяСтруктура(Свойства);
	
	Результат.ПоКоллекциям.Вставить(ВРег(ИмяКоллекцииЯзыкРусский),    Свойства);
	Результат.ПоКоллекциям.Вставить(ВРег(ИмяКоллекцииЯзыкАнглийский), Свойства);
	
	Результат.ПоВидамОбъектов.Вставить(ВРег(ИмяВидаОбъектаЯзыкРусский),    Свойства);
	Результат.ПоВидамОбъектов.Вставить(ВРег(ИмяВидаОбъектаЯзыкАнглийский), Свойства);
	
КонецПроцедуры

// АПК:134-вкл

// Описание свойств фиксированной структуры.
//
// Возвращаемое значение:
//  ФиксированнаяСтруктура:
//   * ИмяКоллекции   - Строка - например, "Справочники" на варианте встроенного языка конфигурации.
//   * ИмяВидаОбъекта - Строка - например, "Справочник" на варианте встроенного языка конфигурации.
//   * ЕстьСсылка           - Булево
//   * ЕстьКлючЗаписи       - Булево
//   * ИмяТипаСсылки        - Строка
//   * ИмяТипаКлючаЗаписи   - Строка
//   * ИмяТипаОбъекта       - Строка
//   * ИмяТипаНабораЗаписей - Строка
//   * ЕстьМодульМенеджера  - Булево
//   * Менеджер - ПланыОбменаМенеджер
...
//              - Неопределено - менеджер объекта, имеющего модуль менеджера.
//
Функция НовыеСвойстваВидаОбъектов() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ИмяКоллекции",         "");
	Результат.Вставить("ИмяВидаОбъекта",       "");
	Результат.Вставить("ЕстьСсылка",           Ложь);
	Результат.Вставить("ЕстьКлючЗаписи",       Ложь);
	Результат.Вставить("ИмяТипаСсылки",        "");
	Результат.Вставить("ИмяТипаКлючаЗаписи",   "");
	Результат.Вставить("ИмяТипаОбъекта",       "");
	Результат.Вставить("ИмяТипаНабораЗаписей", "");
	Результат.Вставить("ЕстьМодульМенеджера",  Ложь);
...
// Возвращает Истина, если у объекта метаданных, найденного по указанному типу,
// есть модуль объекта, модуль набора записей или модуль менеджера значения.
//
// Параметры:
//  Тип - Строка - ЗначениеВСтрокуВнутр(<Тип>)
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоОбъектСМодулем(СериализованныйТип) Экспорт
	
	Тип = ЗначениеИзСтрокиВнутр(СериализованныйТип);
	...
	Свойства = СвойстваВидовОбъектов.ПоВидамОбъектов.Получить(ВРег(ЧастиИмени[0]));
	Если Свойства = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Свойства.ИмяТипаОбъекта) Тогда
		Возврат Тип(Свойства.ИмяТипаОбъекта + "." + ЧастиИмени[1]) = Тип;
	...
	ИначеЕсли ЗначениеЗаполнено(Свойства.ИмяТипаНабораЗаписей) Тогда
		Если Свойства.ИмяВидаОбъекта = "РегистрРасчета"
		   ...
		   И ОбъектМетаданных.Родитель().Перерасчеты.Найти(ЧастиИмени[3]) <> Неопределено Тогда
			
			Возврат Тип("ПерерасчетНаборЗаписей." + ЧастиИмени[1] + "." + ЧастиИмени[3]) = Тип;
		Иначе 
			Возврат Тип(Свойства.ИмяТипаНабораЗаписей + "." + ЧастиИмени[1]) = Тип;
		КонецЕсли;
		
	ИначеЕсли Свойства.ИмяВидаОбъекта = "Константа" Тогда
		Возврат Тип("КонстантаМенеджерЗначения." + ЧастиИмени[1]) = Тип;
		
	ИначеЕсли Свойства.ИмяВидаОбъекта <> "ВнешнийИсточникДанных"
	      Или ЧастиИмени.Количество() < 4 Тогда
		
		Возврат Ложь;
	КонецЕсли;
	
	МетаданныеРодитель = ОбъектМетаданных.Родитель();
	
	Если ЧастиИмени.Количество() = 4 Тогда
		Если МетаданныеРодитель.Кубы.Содержит(ОбъектМетаданных) Тогда
			Возврат Тип("ВнешнийИсточникДанныхКубНаборЗаписей." + ЧастиИмени[1] + "." + ЧастиИмени[3]) = Тип;
			
		ИначеЕсли МетаданныеРодитель.Таблицы.Содержит(ОбъектМетаданных) Тогда
			Если ОбъектМетаданных.ТипДанныхТаблицы
			   = Метаданные.СвойстваОбъектов.ТипДанныхТаблицыВнешнегоИсточникаДанных.ОбъектныеДанные Тогда
			   
				Возврат Тип("ВнешнийИсточникДанныхТаблицаОбъект." + ЧастиИмени[1] + "." + ЧастиИмени[3]) = Тип;
			Иначе
				Возврат Тип("ВнешнийИсточникДанныхТаблицаНаборЗаписей." + ЧастиИмени[1] + "." + ЧастиИмени[3]) = Тип;
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли ЧастиИмени.Количество() = 6
	        И МетаданныеРодитель.Родитель().Кубы.Содержит(МетаданныеРодитель)
	        И МетаданныеРодитель.ТаблицыИзмерений.Найти(ЧастиИмени[5]) <> Неопределено Тогда
		
		Возврат Тип("ВнешнийИсточникДанныхКубТаблицаИзмеренияОбъект."
			+ ЧастиИмени[1] + "." + ЧастиИмени[3] + "." + ЧастиИмени[5]) = Тип;
	КонецЕсли;
...
// См. СтандартныеПодсистемыСервер.ОписаниеКлючаЗаписи
Функция ОписаниеКлючаЗаписи(ПолноеИмяРегистра) Экспорт
	...
	Для Каждого Колонка Из Колонки Цикл
		...
		КонецЕсли;
		Если Не ЭтоРегистрСведенийСРегистратором
		 Или Не ЭтоРазделитель
		   И Поле <> "Регистратор"
		   И Поле <> "НомерСтроки" Тогда
			Продолжить;
		КонецЕсли;
...
Функция РежимДобавленияНабораЗаписей() Экспорт
	...
	Если Не РежимыЗамещенияДоступны() Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Возврат Вычислить("РежимЗамещения.Добавление"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
...
Функция РежимЗамещенияНабораЗаписей() Экспорт
	...
	// АПК:488-выкл Поддержка новых типов платформы (исполняемый код безопасен)
	Возврат Вычислить("РежимЗамещения.Замещение"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
...
Функция РежимОбновленияНабораЗаписей() Экспорт
	...
	// АПК:488-выкл Поддержка новых типов платформы (исполняемый код безопасен)
	Возврат Вычислить("РежимЗамещения.Обновление"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
...
Функция РежимСлиянияНабораЗаписей() Экспорт
	...
	// АПК:488-выкл Поддержка новых типов платформы (исполняемый код безопасен)
	Возврат Вычислить("РежимЗамещения.Слияние"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
...
Функция РежимУдаленияНабораЗаписей() Экспорт
	...
	// АПК:488-выкл Поддержка новых типов платформы (исполняемый код безопасен)
	Возврат Вычислить("РежимЗамещения.Удаление"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
	
КонецФункции

Функция РежимыЗамещенияДоступны()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.3.27.1719") < 0
	 Или ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.5.1.0") > 0
	   И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.5.1.980") < 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ТипРежимЗамещения() Экспорт
	
	ИмяТипа = "РежимЗамещения";
	Возврат Тип(ИмяТипа);
...
//
Функция МетодыРазрешенныеДляВызоваКакПроизвольныйКод(ИмяМодуля) Экспорт
	
	Модуль = ОбщегоНазначения.ОбщийМодуль(ИмяМодуля);
	
	Возврат СтандартныеПодсистемыСервер.МетодыРазрешенныеДляВызоваКакПроизвольныйКод(Модуль);
