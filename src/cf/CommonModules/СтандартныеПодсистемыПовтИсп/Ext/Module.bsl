...
Функция РежимыЗамещенияДоступны(ВключаяОбновление = Ложь)
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.3.26.0") > 0
	   И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.3.26.1540") < 0
	 Или ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.3.27.0") > 0
	   И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Версия, "8.3.27.1483") < 0 Тогда
		Возврат Ложь;
	КонецЕсли;
