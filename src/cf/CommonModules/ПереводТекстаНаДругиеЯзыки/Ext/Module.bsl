...
Функция ПеревестиТексты(Тексты, ЯзыкПеревода = Неопределено, ИсходныйЯзык = Неопределено) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСервисПереводаТекста") Или Не ОбщегоНазначения.РазрешенДоступКИнтернетСервисам() Тогда
		Возврат НайденныеПереводы;
	КонецЕсли;	
...
Функция ДоступенПереводТекста() Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьСервисПереводаТекста") И ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
...
Функция ДоступнаНастройкаСервисаПереводаТекста() Экспорт
	
	Возврат ПравоДоступа("Просмотр", Метаданные.ОбщиеФормы.НастройкаПереводаТекста);
...
Функция ТребуетсяНастройка() Экспорт
	...
	Если Не Константы.ИспользоватьСервисПереводаТекста.Получить() Тогда
		Возврат Истина;
	КонецЕсли;
	
	МодульСервисаПереводаТекста = МодульСервисаПереводаТекста();
	НастройкаВыполнена = МодульСервисаПереводаТекста <> Неопределено И МодульСервисаПереводаТекста.НастройкаВыполнена();
	
	Возврат Не НастройкаВыполнена;
