...
Функция СформироватьПечатныеФормы(МассивОбъектов, Команды) Экспорт
	...
	Для Каждого КомандаПечати Из Команды Цикл
		ЭтоВнешняяПечатнаяФорма = (КомандаПечати.МенеджерПечати = "СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки");
		
		Если ЭтоВнешняяПечатнаяФорма Тогда
			Источник = КомандаПечати.ДополнительныеПараметры.Ссылка;
			ПечатныеФормы = УправлениеПечатью.СформироватьВнешнююПечатнуюФорму(Источник, КомандаПечати.Идентификатор, МассивОбъектов, ОбъектыПечати);
		Иначе
		...
		Если Не ЭтоВнешняяПечатнаяФорма Тогда
	...
	ПараметрыОтправки.Получатель = ПечатныеФормы.ПараметрыВывода.ПараметрыОтправки.Получатель;
	...
	Результат.ПараметрыВывода = ПараметрыВывода;
