...
//
// Параметры:
//  СобытияДляЖурналаРегистрации - Массив из Структура:
...
Процедура ЗаписатьСобытияВЖурналРегистрации(СобытияДляЖурналаРегистрации) Экспорт
	...
	Если ТипЗнч(СобытияДляЖурналаРегистрации) <> Тип("Массив")
	   И ТипЗнч(СобытияДляЖурналаРегистрации) <> Тип("СписокЗначений") Тогда
		Возврат;
	КонецЕсли;
	...
	СобытияДляЖурнала = ?(ТипЗнч(СобытияДляЖурналаРегистрации) = Тип("Массив"),
		СобытияДляЖурналаРегистрации, СобытияДляЖурналаРегистрации.ВыгрузитьЗначения());
	
	Для Каждого ЗначениеСообщения Из СобытияДляЖурнала Цикл
		...
		КонецЕсли;
		Комментарий = Формат(ДатаСобытия, "ДЛФ=DT") + " " + ЗначениеСообщения.Комментарий;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПрочитатьСобытияЖурналаРегистрации", Истина);
