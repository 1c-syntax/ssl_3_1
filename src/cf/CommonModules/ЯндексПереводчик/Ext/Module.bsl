...
Функция IAMТокен()
	...
	Если Не ЗначениеЗаполнено(ПараметрыТокена.IAMТокен) 
		Или Не ЗначениеЗаполнено(ПараметрыТокена.СрокДействия) Или ПараметрыТокена.СрокДействия <= УниверсальноеВремя(ТекущаяДата()) Тогда // АПК:143
...
Функция НастройкаВыполнена() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	НастройкиАвторизации = НастройкиАвторизации();
	Возврат ЗначениеЗаполнено(НастройкиАвторизации.OAuthТокен) И ЗначениеЗаполнено(НастройкиАвторизации.ИдентификаторКаталога);
