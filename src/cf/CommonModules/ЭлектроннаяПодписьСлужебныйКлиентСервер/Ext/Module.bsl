...
Процедура ДобавитьСвойстваСертификатов(Таблица, МассивСертификатов, БезОтбора,
	...
	Для Каждого ТекущийСертификат Из МассивСертификатов Цикл
		...
		СвойстваСертификата.Вставить("Представление",
			ПредставлениеСертификата(ТекущийСертификат, РазницаСУниверсальнымВременем));
...
Функция СвойстваСертификата(Сертификат, РазницаСУниверсальнымВременем, ДвоичныеДанныеСертификата = Неопределено) Экспорт
	...
	Иначе
		Свойства.Вставить("ДействителенДо", ДатыСертификата.ДатаОкончания);
		Свойства.Вставить("Представление",  ПредставлениеСертификата(Сертификат, РазницаСУниверсальнымВременем));
	КонецЕсли;
...
Функция ДиагностическаяИнформацияОКомпьютере(ДляКлиента = Ложь) Экспорт
	...
	Если Не ПустаяСтрока(ПрограммаПросмотра) Тогда
		ПрограммаПросмотра = Символы.ПС + НСтр("ru = 'Приложение:'") + " " + ПрограммаПросмотра;
	КонецЕсли;
	
	Возврат НСтр("ru = 'Операционная система:'") + " " + СисИнфо.ВерсияОС
		+ Символы.ПС + НСтр("ru = 'Версия:'") + " " + СисИнфо.ВерсияПриложения
...
Функция СертификатВЛокальномХранилищеТекстРешения() Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='<a href = ""%1"">Удалите сертификат</a> из локального хранилища компьютера.'"),
		"УдалитьСертификатИзЛокальногоХранилища");
...
Функция ПараметрыДополненияТекстаРешенияОшибкиКлассификатора() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("ПроверитьСертификатВЛокальномХранилищеКлиента", Ложь);
	Структура.Вставить("ДиагностикаСерверовМетокВремениКлиент", Ложь);
					...
					КонецЕсли; 
						...
						Если РазмерДанных = 3 Тогда
							...
							ИначеЕсли СтрокаБуфера = "551D10" Тогда // 2.5.29.16 privateKeyUsagePeriod
								ЗаполнитьПериодДействияЗакрытогоКлюча(ПрочитанныйБлок(АнализДанных, 0, 4, Истина), Структура, РазницаСУниверсальнымВременем);
							КонецЕсли;
