...
Функция ПредупреждениеПриПроверкеУдостоверяющегоЦентраСертификата() Экспорт
	...
	Предупреждение.Вставить("РазрешитьПодписание", Истина);
...
Функция ТекстОшибкиНеУдалосьОпределитьПрограмму(Ошибка) Экспорт
	...
	Если ЗначениеЗаполнено(Ошибка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось автоматически определить приложение электронной подписи и шифрования:
			|%1'"), Ошибка);
	Иначе
		ТекстОшибки = НСтр("ru = 'Не удалось автоматически определить приложение электронной подписи и шифрования.'");
	КонецЕсли;
...
Процедура ЗаполнитьРезультатПроверкиСертификатНеквалифицированный(ПараметрыПроверки, Результат) Экспорт
	Если ПараметрыПроверки.ПроверятьСертификат = ТолькоКвалифицированные() Тогда
		ТекстОшибки = НСтр("ru='Сертификат неквалифицированный, подпись таким сертификатом не является юридически значимой.'");
		Результат.Действует = Ложь;
		Результат.Предупреждение.ТекстОшибки = ТекстОшибки;
		Результат.Предупреждение.РазрешитьПодписание = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Функция РезультатПоискаКриптопровайдеров(КриптопровайдерыРезультат, ИмяСервера = "") Экспорт
	...
	Если ИмяСервера = "" Тогда
		...
		ИначеЕсли КриптопровайдерыРезультат.ПроверкаВыполнена Тогда
			...
			Иначе
				ТекстОшибки = НСтр("ru = 'На компьютере не установлены приложения электронной подписи и шифрования.'");
			КонецЕсли;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'На компьютере не удалось автоматически определить установленные приложения электронной подписи и шифрования:
	...
	Иначе
		...
		ИначеЕсли КриптопровайдерыРезультат.ПроверкаВыполнена Тогда
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'На сервере %1 не установлены приложения электронной подписи и шифрования.'"), ИмяСервера);
			КонецЕсли;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'На сервере %1 не удалось автоматически определить установленные приложения электронной подписи и шифрования:
...
Функция АдресСпискаОтзываВнутренний(ИмяИздателя, Сертификат, КаталогиСписковОтзываУЦ) Экспорт
	...
	Возврат ЭлектроннаяПодписьКлиентСерверЛокализация.АдресСпискаОтзываВнутренний(ИмяИздателя, Сертификат, КаталогиСписковОтзываУЦ);
...
Функция АктуальныеАлгоритмыПрограмм() Экспорт
	...
	Возврат ЭлектроннаяПодписьКлиентСерверЛокализация.АктуальныеАлгоритмыПрограмм();
...
// Запрещен доступ к интернет сервисам текст сообщения.
// 
// Возвращаемое значение:
//  Строка
//
Функция ЗапрещенДоступКИнтернетСервисамТекстСообщения() Экспорт
	Возврат НСтр("ru = 'Доступ к Интернет-сервисам электронной подписи из приложения запрещен администратором (""Общие настройки"" - ""Электронная подпись"" - ""Разрешить доступ к Интернет-сервисам"").'");
КонецФункции

// Конструктор свойств подписи при подписании.
// 
// Параметры:
...
// См. ЭлектроннаяПодписьКлиент.НовыйТипПодписи
// 
// Возвращаемое значение:
//  Строка
//
Функция ПроверятьКвалифицированные() Экспорт
	Возврат "ПроверятьКвалифицированные";
КонецФункции
...
Функция ТолькоКвалифицированные() Экспорт
	Возврат "ТолькоКвалифицированные";
КонецФункции
...
Функция ОшибкаЗагрузкиБиблиотекиТокена(ТекстОшибки) Экспорт
...
Функция ДействиеУстановитьБиблиотекиДляТокенов() Экспорт
	Возврат "УстановитьБиблиотекиДляТокенов";
КонецФункции

Функция ЭтоЗаменаСертификата(СвойстваНового, СвойстваСтарого) Экспорт
...
Функция ЦепочкаСертификатовИзОтветаКомпоненты(ОтветКомпоненты, ИдентификаторФормы) Экспорт
	...
	Исключение
		...
		Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно получить цепочку сертификатов %1'"),
				ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
...
Функция УстановленныеКриптопровайдерыИзОтветаКомпоненты(ОтветКомпоненты, ПрограммыПоИменамСТипом, 
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно получить свойства криптопровайдеров:
	...
	Для Каждого ТекущийКриптопровайдер Из Криптопровайдеры Цикл
		...
		Если РасширенноеОписаниеПрограммы.Неактуальная
			Или РасширенноеОписаниеПрограммы.ИмяПрограммы = "Microsoft Base Smart Card Crypto Provider" Тогда
			Неактуальные.Добавить(РасширенноеОписаниеПрограммы);
		Иначе
	...
	Для Каждого ТекущийКриптопровайдер Из Неактуальные Цикл
		КриптопровайдерыРезультат.Добавить(ТекущийКриптопровайдер);
	КонецЦикла;
...
Функция ОпределитьПрограмму(СвойстваСертификата,
	...
	Если УстановленныеКриптопровайдеры.Количество() = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Не установлены приложения электронной подписи и шифрования.'");
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ПрограммыПоОткрытомуКлючу = Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено подходящих приложений электронной подписи и шифрования по открытому ключу сертификата %1.'"),
	...
	Если ПрограммаНайдена = Неопределено Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Не предусмотрено использование ни одного приложения
...
Функция МенеджерКриптографииОписанияПрограмм(Программа, Ошибки, Знач ОписанияПрограмм, Знач ПрограммыАвто = Неопределено) Экспорт
	...
	ИначеЕсли Программа <> Неопределено Тогда
		...
		Если Не ПрограммаНайдена Тогда
			МенеджерКриптографииДобавитьОшибку(Ошибки, Программа, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Приложение %1 не предусмотрено для использования (отключено администратором).'"), Программа), Истина);
			Возврат Неопределено;
		КонецЕсли;
...
Функция МенеджерКриптографииСвойстваПрограммы(ОписаниеПрограммы, ЭтоLinux, Ошибки, ЭтоСервер,
	...
	Если Не ЗначениеЗаполнено(ОписаниеПрограммы.ИмяПрограммы) Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
			НСтр("ru = 'Не указано название приложения.'"), Истина);
		Возврат Неопределено;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОписаниеПрограммы.ТипПрограммы) Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
			НСтр("ru = 'Не указан тип приложения.'"), Истина);
		Возврат Неопределено;
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеПути.ПутьКПрограмме) И Не ОписаниеПути.Существует Тогда
			Если ОписаниеПути.Свойство("ТекстОшибки")
			   И ЗначениеЗаполнено(ОписаниеПути.ТекстОшибки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось определить путь к приложению электронной подписи и шифрования по причине:
...
Функция МенеджерКриптографииАлгоритмПодписиПоддерживается(ОписаниеПрограммы, Операция,
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Приложение не поддерживает алгоритм подписи %1.'"),
		СокрЛП(ВозможныеАлгоритмы[0]));
...
Функция МенеджерКриптографииАлгоритмыУстановлены(ОписаниеПрограммы, Менеджер, Ошибки, АлгоритмШифрования = "") Экспорт
	...
	ЭлектроннаяПодписьКлиентСерверЛокализация.ПриУстановкеПараметровМенеджераКриптографии(
		ОписаниеПрограммы, Менеджер, АлгоритмШифрования, АлгоритмыУстановлены);
	...
	Если ПустаяСтрока(АлгоритмШифрования) Тогда
		АлгоритмШифрования = ЭлектроннаяПодписьКлиентСерверЛокализация.ПреобразованныйАлгоритмШифрования(Строка(ОписаниеПрограммы.АлгоритмШифрования));
	Иначе
...
Процедура МенеджерКриптографииПрограммаНеНайдена(ОписаниеПрограммы, Ошибки, ЭтоСервер) Экспорт
	...
	МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
		НСтр("ru = 'Приложение не установлено на компьютере.'"), ЭтоСервер, Истина);
...
Функция МенеджерКриптографииИмяПрограммыСовпадает(ОписаниеПрограммы, ИмяПрограммыПолученное, Ошибки, ЭтоСервер) Экспорт
	...
	Если ИмяПрограммыПолученное <> ОписаниеПрограммы.ИмяПрограммы Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получено другое приложение электронной подписи и шифрования с именем ""%1"".'"), 
			ИмяПрограммыПолученное), ЭтоСервер, Истина);
		Возврат Ложь;
	КонецЕсли;
...
Процедура МенеджерКриптографииЗаполнитьПредставлениеОшибок(ОписаниеОшибок,
	...
	Если ОписаниеОшибок.Ошибки.Количество() = 0 Тогда
		Если Не ЗначениеЗаполнено(АлгоритмПодписи) Тогда
			ТекстОшибки = НСтр("ru = 'Не предусмотрено использование ни одного приложения электронной подписи и шифрования.'");
		Иначе
			ШаблонОшибки = НСтр("ru = 'Не предусмотрено использование ни одного приложения электронной подписи и шифрования
			                          |с алгоритмом подписи %1.'");
...
Функция ЗаписатьСертификатВСправочник(Контекст, Ошибка) Экспорт
	...
	Исключение
		...
		Контекст.ЗаголовокФормы = НСтр("ru = 'Невозможно добавить сертификат'");
...
//  ОписаниеПрограммы - Структура:
//   * Ссылка - СправочникСсылка.ПрограммыЭлектроннойПодписиИШифрования
//            - Строка - представление токена
//  Токен - см. ЭлектроннаяПодписьСлужебныйКлиент.НовыеСвойстваТокена
//        - Неопределено
//  Операция - Строка
//  ТекстОшибки - Строка
//  ЭтоПолноправныйПользователь - Булево
...
Процедура ЗаполнитьОшибкуДобавленияСертификата(ОписаниеОшибок, ОписаниеПрограммы, Операция,
	...
	Если ТипЗнч(ОписаниеПрограммы.Ссылка) = Тип("Строка") Тогда
		СвойстваОшибки.Программа = ОписаниеПрограммы.Ссылка;
		СвойстваОшибки.Токен = ОписаниеПрограммы.Токен;
	Иначе
		СвойстваОшибки.Программа = ОписаниеПрограммы.Ссылка;
	КонецЕсли;
...
#КонецЕсли
	...
	Если ИгнорироватьПроверкуВСпискеОтозванныхСертификатов Тогда
...
Функция СертификатПросрочен(Сертификат, НаДату, РазницаСУниверсальнымВременем) Экспорт
	...
	ДатыСертификата = ДатыСертификата(Сертификат, РазницаСУниверсальнымВременем);
	
	Если ДатыСертификата.ДатаОкончания > НаДату Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'На %1 просрочен сертификат.'"), НаДату);
...
Функция ЗакрытыйКлючПросрочен(СвойстваСертификата, НаДату) Экспорт
	...
	Если СвойстваСертификата.ДатаОкончанияЗакрытогоКлюча > НаДату Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'На %1 просрочен закрытый ключ сертификата.'"), НаДату);
...
Процедура ДобавитьСвойстваСертификатов(Таблица, МассивСертификатов, БезОтбора,
	...
	Если Параметры <> Неопределено Тогда
		...
		КонецЕсли;
		Если Параметры.Свойство("НаТокене") Тогда
			НаТокене = Параметры.НаТокене;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ТекущийСертификат Из МассивСертификатов Цикл
		...
		ДатыСертификата = ДатыСертификата(ТекущийСертификат, РазницаСУниверсальнымВременем);
		
		Если ДатыСертификата.ДействителенДо <= ТекущаяДатаСеанса Тогда
		...
		ИначеЕсли ДатыСертификата.ДействителенДо <= ТекущаяДатаСеанса + 30*24*60*60 Тогда
			СостояниеСертификата = 3;
		КонецЕсли;
		
		СвойстваСертификата = Новый Структура;
		СвойстваСертификата.Вставить("Отпечаток", Отпечаток);
		СвойстваСертификата.Вставить("Представление", ПредставлениеСертификата(
			Новый Структура("Сертификат, ДействителенДо", ТекущийСертификат, ДатыСертификата.ДействителенДо)));
		...
		Иначе
			...
			ИначеЕсли НаТокене Тогда
				СвойстваСертификата.Вставить("НаТокене", Истина);	
			КонецЕсли;
...
Функция ПараметрыПодписиКриптографии(ПараметрыПодписи, Подпись, СертификатСуществует, РазницаСУниверсальнымВременем, ДатаСеанса) Экспорт
	...
	Если ЗначениеЗаполнено(Подпись.НеподтвержденноеВремяПодписи) Тогда
		ПараметрыПодписи.НеподтвержденнаяДатаПодписи = Подпись.НеподтвержденноеВремяПодписи + РазницаСУниверсальнымВременем;
	КонецЕсли;
	...
	Если Подпись.МеткаВремениПодписи <> Неопределено Тогда
		...
		ДатаПодписиИзМетки = Мин(ДатаПодписиИзМетки, Подпись.МеткаВремениПодписи.Дата);
		ПараметрыПодписи.ДатаПоследнейМеткиВремени = Подпись.МеткаВремениПодписи.Дата + РазницаСУниверсальнымВременем;
	КонецЕсли;
	...
	Если Подпись.МеткаВремениДанныхПроверкиПодписи <> Неопределено Тогда
		...
		СрокДействияПоследнейМеткиВремени = СертификатПоследнейМеткиВремени.ДатаОкончания;
		ДатаПодписиИзМетки = Мин(ДатаПодписиИзМетки, Подпись.МеткаВремениДанныхПроверкиПодписи.Дата);
		ПараметрыПодписи.ДатаПоследнейМеткиВремени = Подпись.МеткаВремениДанныхПроверкиПодписи.Дата + РазницаСУниверсальнымВременем;
	КонецЕсли;
	
	Если Подпись.АрхивныеМеткиВремени.Количество() > 0 Тогда
		ИндексПоследнейМетки = 0;
		ДатаПоследнейМетки = Дата(1,1,1);
		ИндексМетки = 0;
		Для Каждого ТекущаяМетка Из Подпись.АрхивныеМеткиВремени Цикл
			Если ТекущаяМетка.Дата > ДатаПоследнейМетки Тогда
				ИндексПоследнейМетки = ИндексМетки;
				ДатаПоследнейМетки = ТекущаяМетка.Дата;
			КонецЕсли;
			ДатаПодписиИзМетки = Мин(ДатаПодписиИзМетки, ТекущаяМетка.Дата);
		...
		СрокДействияПоследнейМеткиВремени = СертификатПоследнейМеткиВремени.ДатаОкончания;
		ПараметрыПодписи.ДатаПоследнейМеткиВремени = Подпись.АрхивныеМеткиВремени[ИндексПоследнейМетки].Дата + РазницаСУниверсальнымВременем;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СрокДействияПоследнейМеткиВремени) Тогда
		ПараметрыПодписи.СрокДействияПоследнейМеткиВремени = СрокДействияПоследнейМеткиВремени + РазницаСУниверсальнымВременем; 
	...
	Если ДатаПодписиИзМетки <> Дата(3999, 12, 31) Тогда
		ПараметрыПодписи.ДатаПодписиИзМетки = ДатаПодписиИзМетки + РазницаСУниверсальнымВременем;
	КонецЕсли;
...
Функция НастройкиСозданияПодписи(ТипПодписи, АдресаСерверовМетокВремени) Экспорт
	...
	Результат.АдресаСерверовМетокВремени = Новый Массив;
	...
	Если ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST") 
		...
		Граница = Результат.АдресаСерверовМетокВремени.ВГраница();
		Для Счетчик = 0 По Граница Цикл
			ТекущийИндекс = Граница - Счетчик;
			СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(Результат.АдресаСерверовМетокВремени[ТекущийИндекс]);
			Если СтруктураURI.ИмяСервера = "" Тогда
				Результат.АдресаСерверовМетокВремени.Удалить(ТекущийИндекс);
			КонецЕсли;
		КонецЦикла;
		
		Если Результат.АдресаСерверовМетокВремени.Количество() = 0 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для создания подписи с типом ""%1"" заполните адреса серверов меток времени.'"), ТипПодписи);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеОшибкиНаСервере.ЗаголовокОшибки)
		   И ЗначениеЗаполнено(ОписаниеОшибкиНаСервере.Описание) Тогда
			...
			ОбщееОписаниеОшибки =
				...
				+ Символы.ПС
				+ НСтр("ru = 'На сервере:'")
				+ " " + ОписаниеОшибкиНаСервере.Описание;
		Иначе
			ОбщееОписаниеОшибки =
				НСтр("ru = 'На сервере:'")
				+ " " + ОписаниеОшибкиНаСервере.ОписаниеОшибки;
		КонецЕсли;
	...
	Иначе
		Если ОписаниеОшибкиНаКлиенте.ЗаголовокОшибки = ОписаниеОшибкиНаСервере.ЗаголовокОшибки
		   И ЗначениеЗаполнено(ОписаниеОшибкиНаКлиенте.ЗаголовокОшибки) Тогда
			...
			ОбщееОписаниеОшибки = ОписаниеОшибкиНаКлиенте.ЗаголовокОшибки + Символы.ПС;
		...
		ОбщееОписаниеОшибки = ОбщееОписаниеОшибки
			...
			+ " " + ТекстОшибкиНаКлиенте
			+ Символы.ПС
			+ НСтр("ru = 'На сервере:'")
			+ " " + ТекстОшибкиНаСервере;
	КонецЕсли;
...
Функция СвойстваПодписиИзДвоичныхДанных(Данные, РазницаСУниверсальнымВременем = Неопределено, ПрочитатьСертификаты = Ложь) Экспорт
	...
	СвойстваПодписи.Вставить("СертификатПодписи", Неопределено);
		...
		ПропуститьНачалоБлока(АнализДанных, 2, 0);
			...
			ПропуститьНачалоБлока(АнализДанных, 0, 16);
				...
				ПропуститьНачалоБлока(АнализДанных, 0, 16);
					...
					РазмерДанных = АнализДанных.Родители[0].РазмерДанных;
					Буфер = АнализДанных.Буфер.Прочитать(АнализДанных.Смещение, РазмерДанных);
					СтрокаБуфера = ПолучитьHexСтрокуИзБуфераДвоичныхДанных(Буфер);
					Если СтрокаБуфера = "2A864886F70D0109100104" Тогда //1.2.840.113549.1.9.16.1.4 tSTInfo
						ПропуститьРодительскийБлок(АнализДанных);
						ПропуститьНачалоБлока(АнализДанных, 2, 0); // [0]
						РазмерДанных = АнализДанных.Родители[0].РазмерДанных;
						Буфер = АнализДанных.Буфер.Прочитать(АнализДанных.Смещение, РазмерДанных); // БуферДвоичныхДанных
						ДатаШтампаВремени = ПрочитатьДатуИзШтампаВремени(Буфер);
						Если ЗначениеЗаполнено(ДатаШтампаВремени) Тогда
							СвойстваПодписи.ДатаШтампаВремени = ДатаШтампаВремени + ?(ЗначениеЗаполнено(РазницаСУниверсальнымВременем),
								РазницаСУниверсальнымВременем, 0);
						КонецЕсли;
						ПропуститьРодительскийБлок(АнализДанных); // [0]
					Иначе
						ПропуститьРодительскийБлок(АнализДанных); // OBJECT IDENTIFIER (contentType).
					КонецЕсли;

				ПропуститьРодительскийБлок(АнализДанных); // SEQUENCE (contentInfo      ContentInfo).
				...
				ПропуститьНачалоБлока(АнализДанных, 0, 17);
					...
					ПропуститьНачалоБлока(АнализДанных, 0, 16);
						...
						// SEQUENCE (issuerAndSerialNumber     IssuerAndSerialNumber).
						ПропуститьНачалоБлока(АнализДанных, 0, 16);
							// SEQUENCE (issuerAndSerialNumber     issuer rdnSequence).
							ПропуститьБлок(АнализДанных, 0, 16);
							// INTEGER (serialNumber CertificateSerialNumber).
							ПропуститьНачалоБлока(АнализДанных, 0, 2);
							РазмерДанных = АнализДанных.Родители[0].РазмерДанных;
							Буфер = АнализДанных.Буфер.Прочитать(АнализДанных.Смещение, РазмерДанных);
							СерийныйНомерСертификатаПодписи = ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер);
							ПропуститьРодительскийБлок(АнализДанных); // INTEGER
							ПропуститьРодительскийБлок(АнализДанных); // SEQUENCE
						// SEQUENCE (digestAlgorithm           DigestAlgorithmIdentifier).
						ПропуститьБлок(АнализДанных, 0, 16);
						// [0]CS    (authenticatedAttributes   [0] IMPLICIT Attributes OPTIONAL).
	...
	Для Каждого Сертификат Из СвойстваПодписи.Сертификаты Цикл
		СертификатКриптографии = Новый СертификатКриптографии(Сертификат);
		Если СертификатКриптографии.СерийныйНомер = СерийныйНомерСертификатаПодписи Тогда
			СвойстваПодписи.СертификатПодписи = Сертификат;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Пока АнализДанных.Смещение < СмещениеСледующего И Не АнализДанных.ЕстьОшибка Цикл
		...
		Если РазмерДанных = 9 Тогда
			...
			ИначеЕсли СтрокаБуфера = "2A864886F70D010905" Тогда // 1.2.840.113549.1.9.5 signingTime
				...
				Если ЗначениеЗаполнено(ДатаПодписания) Тогда
					СвойстваПодписи.ДатаПодписания = ДатаПодписания + ?(ЗначениеЗаполнено(РазницаСУниверсальнымВременем),
						РазницаСУниверсальнымВременем, 0);
				КонецЕсли;
	...
	Пока АнализДанных.Смещение < СмещениеСледующего И Не АнализДанных.ЕстьОшибка Цикл
		...
		Если РазмерДанных = 11 Тогда
			...
			Если СтрокаБуфера = "2A864886F70D010910020E" Тогда // 1.2.840.113549.1.9.16.2.14 timeStampToken
				...
				ПропуститьРодительскийБлок(АнализДанных); // OBJECT IDENTIFIER
				БлокПропущен = Истина;
				ПропуститьНачалоБлока(АнализДанных, 0, 17); // SET
			
				РазмерДанных = АнализДанных.Родители[0].РазмерДанных;
				Буфер = АнализДанных.Буфер.Прочитать(АнализДанных.Смещение, РазмерДанных); // БуферДвоичныхДанных
				ДанныеПодписи = ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер);
				
				СвойстваПодписиИзДвоичныхДанных = СвойстваПодписиИзДвоичныхДанных(ДанныеПодписи, РазницаСУниверсальнымВременем, Ложь);
				СвойстваПодписи.ДатаШтампаВремени = СвойстваПодписиИзДвоичныхДанных.ДатаШтампаВремени;
				
				ПропуститьРодительскийБлок(АнализДанных); // SET
		...
		Если РазмерДанных = 6 Тогда
			...
			Если СтрокаБуфера = "04008D450204" Тогда // 0.4.0.1733.2.4 archiveTimestampV3 attribute
				...
				Если Не ЗначениеЗаполнено(СвойстваПодписи.ДатаШтампаВремени) Тогда
					ПропуститьРодительскийБлок(АнализДанных); // OBJECT IDENTIFIER
					БлокПропущен = Истина;
						ПропуститьНачалоБлока(АнализДанных, 0, 17); // SET
						РазмерДанных = АнализДанных.Родители[0].РазмерДанных;
	...
	Если ПустаяСтрока(Представление) Тогда
		...
		Иначе
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, до %2'"),
				...
				Формат(ДатыСертификата.ДействителенДо, "ДФ=MM.yyyy"));
		КонецЕсли;
	КонецЕсли;
...
// 
// Параметры:
//  Сертификат - СертификатКриптографии - также может быть структура со свойствами как у сертификата криптографии.
...
// * ДатаНачала - Дата
// * ДатаОкончания - Дата
// * ДатаНачалаЗакрытогоКлюча - Дата
// * ДатаОкончанияЗакрытогоКлюча - Дата 
// * ДействителенДо - Дата 
// * ИдентификаторКлючаУдостоверяющегоЦентра - Строка
// * СодержитВстроеннуюЛицензиюКриптоПро - Булево
// * АлгоритмПодписи - Строка
// * АлгоритмОткрытогоКлюча - Строка 
// * АдресаСписковОтзыва - Массив 
// * Назначение - Строка
...
Функция СвойстваСертификата(Сертификат, РазницаСУниверсальнымВременем, ДвоичныеДанныеСертификата = Неопределено) Экспорт
	...
	Свойства.Вставить("ДатаНачалаЗакрытогоКлюча", ДатыСертификата.ДатаНачалаЗакрытогоКлюча);
	Свойства.Вставить("ДатаОкончанияЗакрытогоКлюча", ДатыСертификата.ДатаОкончанияЗакрытогоКлюча);
	Свойства.Вставить("ДействителенДо", ДатыСертификата.ДействителенДо);
	
	Свойства.Вставить("ИдентификаторКлючаУдостоверяющегоЦентра", "");
	Свойства.Вставить("СодержитВстроеннуюЛицензиюКриптоПро", Ложь);
	Свойства.Вставить("АлгоритмПодписи", "");
	Свойства.Вставить("АлгоритмОткрытогоКлюча", "");
	Свойства.Вставить("АдресаСписковОтзыва", Новый Массив);
	
	Если ТипЗнч(Сертификат) = Тип("СертификатКриптографии") И ДоступныДополнительныеСвойстваСертификата() Тогда
		
		Свойства.Вставить("Назначение", ПолучитьНазначениеСертификатаКриптографии(Сертификат));
		Свойства.АлгоритмОткрытогоКлюча = Сертификат.АлгоритмОткрытогоКлюча;
		Свойства.АлгоритмПодписи = Сертификат.АлгоритмПодписи;
		
		ДанныеКлючаУдостоверяющегоЦентра = Сертификат.ДополненияСертификатаКакИсходныеДанные.Получить("2.5.29.35");
		Если ДанныеКлючаУдостоверяющегоЦентра <> Неопределено Тогда
			Свойства.ИдентификаторКлючаУдостоверяющегоЦентра = ИдентификаторКлючаУдостоверяющегоЦентра(ДанныеКлючаУдостоверяющегоЦентра);
		КонецЕсли;
		
		Свойства.СодержитВстроеннуюЛицензиюКриптоПро = Сертификат.ДополненияСертификата.Получить("1.2.643.2.2.49.2") <> Неопределено;
		
		АдресаСписковОтзыва = Сертификат.ДополненияСертификата.Получить("2.5.29.31");
		Если АдресаСписковОтзыва <> Неопределено Тогда
			Свойства.АдресаСписковОтзыва = АдресаСписковОтзыва;
		КонецЕсли;
			
	ИначеЕсли ДвоичныеДанныеСертификата <> Неопределено Тогда
		
		ДополнительныеСвойстваСертификата = ДополнительныеСвойстваСертификата(ДвоичныеДанныеСертификата, РазницаСУниверсальнымВременем);
		
		Если ЗначениеЗаполнено(ДополнительныеСвойстваСертификата.ДатаОкончанияЗакрытогоКлюча) Тогда
			Свойства.ДействителенДо = Мин(ДатыСертификата.ДатаОкончания, ДополнительныеСвойстваСертификата.ДатаОкончанияЗакрытогоКлюча);
		КонецЕсли;
...
Функция ДоступныДополнительныеСвойстваСертификата() Экспорт
#Если МобильныйКлиент Тогда
		Возврат Ложь;
#КонецЕсли
	СистемнаяИнформация = Новый СистемнаяИнформация();
	Версия = ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(СистемнаяИнформация.ВерсияПриложения);
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(Версия, "8.3.27") >= 0;
...
Функция ДатыСертификата(Сертификат, РазницаСУниверсальнымВременем) Экспорт
	...
	ДатыСертификата.Вставить("ДатаНачала",    Сертификат.ДатаНачала    + РазницаСУниверсальнымВременем);
	ДатыСертификата.Вставить("ДатаОкончания", Сертификат.ДатаОкончания + РазницаСУниверсальнымВременем);
	ДатыСертификата.Вставить("ДействителенДо", ДатыСертификата.ДатаОкончания);
	ДатыСертификата.Вставить("ДатаНачалаЗакрытогоКлюча", Дата(1,1,1));
	ДатыСертификата.Вставить("ДатаОкончанияЗакрытогоКлюча", Дата(1,1,1));
	
	Если ДоступныДополнительныеСвойстваСертификата() И ТипЗнч(Сертификат) = Тип("СертификатКриптографии") Тогда
		СвойстваДаты = Новый Структура("УниверсальнаяДатаНачалаЗакрытогоКлюча, УниверсальнаяДатаОкончанияЗакрытогоКлюча");
		ЗаполнитьЗначенияСвойств(СвойстваДаты, Сертификат);
		Если СвойстваДаты.УниверсальнаяДатаНачалаЗакрытогоКлюча <> Неопределено Тогда
			ДатыСертификата.Вставить("ДатаНачалаЗакрытогоКлюча", СвойстваДаты.УниверсальнаяДатаНачалаЗакрытогоКлюча
				+ РазницаСУниверсальнымВременем);
		КонецЕсли;
		Если СвойстваДаты.УниверсальнаяДатаОкончанияЗакрытогоКлюча <> Неопределено Тогда
			ДатыСертификата.Вставить("ДатаОкончанияЗакрытогоКлюча", СвойстваДаты.УниверсальнаяДатаОкончанияЗакрытогоКлюча
				+ РазницаСУниверсальнымВременем);
			ДатыСертификата.Вставить("ДействителенДо", Мин(ДатыСертификата.ДатаОкончания,
				ДатыСертификата.ДатаОкончанияЗакрытогоКлюча));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьНазначение(Сертификат) // "2.5.29.37"
...
// Для функции СвойстваСертификата.
Функция ПолучитьНазначениеСертификатаКриптографии(Сертификат)
	
	Назначения = Сертификат.ДополненияСертификата.Получить("2.5.29.37");
	Если Назначения = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат СтрСоединить(Назначения, Символы.ПС);
...
Функция НовоеРасширенноеОписаниеПрограммы() Экспорт
	...
	Описание.Вставить("Неактуальная", Ложь);
...
Функция ДанныеДляДополненияОшибкиИзКлассификатора(ДополнительныеДанные) Экспорт
	...
	Структура = Новый Структура("ДанныеСертификата, ДанныеПодписи");
				...
				Если Сертификат.Количество() > 0 Тогда
					Если ТипЗнч(Сертификат[0]) = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
						Если ЗначениеЗаполнено(Сертификат[0]) Тогда
							ДанныеСертификата = ЭлектроннаяПодписьСлужебныйВызовСервера.ДанныеСертификата(Сертификат[0]);
						КонецЕсли;
	...
	Структура.ДанныеСертификата = ДанныеСертификата;
	Структура.ДанныеПодписи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеДанные, "ДанныеПодписи", Неопределено);
...
Функция ДиагностикаСерверовМетокВремени(АдресаСерверовМетокВремени, СтрокаПодключения) Экспорт
	...
	Массив = Новый Массив;
	Массив.Добавить(СтрокаПодключения);
	Массив.Добавить(Символы.ПС);
	
	Для Каждого Адрес Из АдресаСерверовМетокВремени Цикл
		
		СообщениеОбОшибке = ДиагностикаСервераМетокВремени(Адрес);
		
		Если ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
			Массив.Добавить(СообщениеОбОшибке);
		Иначе
			Массив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 - доступен.'"), Адрес));
		КонецЕсли;
...
// Функция для диагностики сервера меток времени.
//
// Параметры:
...
//
Функция ДиагностикаСервераМетокВремени(Знач URL, Перенаправления = Неопределено)
...
#Иначе
		...
		Если ПустаяСтрока(Протокол) Тогда
			Протокол = "http"; // Допустимо использование незащищенного соединения для диагностики.
		КонецЕсли;
		...
		Попытка
			...
			Соединение = Новый HTTPСоединение(Сервер, Порт, , , Прокси, 7, ЗащищенноеСоединение);
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось установить HTTP-соединение с сервером %1:%2
					 |по причине:
					 |%3'"), Сервер, Формат(Порт, "ЧГ="), ОбработкаОшибок.КраткоеПредставлениеОшибки(
				ИнформацияОбОшибке()));
		...
		Попытка
			...
			Если ОтветHTTP.КодСостояния = 301 // 301 Moved Permanently
				...
				Иначе 
					...
					Возврат ДиагностикаСервераМетокВремени(НовыйURL, Перенаправления);
			...
			Если ОтветHTTP.КодСостояния < 200 Или ОтветHTTP.КодСостояния >= 300 Тогда
				...
				Если ОтветHTTP.КодСостояния < 200 Или ОтветHTTP.КодСостояния >= 300 И ОтветHTTP.КодСостояния < 400 Тогда

					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Неподдерживаемый ответ сервера (%1)'"), ОтветHTTP.КодСостояния);
				...
				ИначеЕсли ОтветHTTP.КодСостояния >= 400 И ОтветHTTP.КодСостояния < 500 Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при выполнении запроса (%1)'"), ОтветHTTP.КодСостояния);
					ВызватьИсключение ТекстОшибки;

				Иначе

					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка сервера при обработке запроса к ресурсу (%1)'"), ОтветHTTP.КодСостояния);
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось получить файл %1 с сервера %2:%3 по причине:
					 |%4'"), URL, Сервер, Формат(Порт, "ЧГ="), ОбработкаОшибок.КраткоеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			Возврат ТекстОшибки;
...
Функция ТехническаяИнформацияОТокенах(ИнформацияОТокенах, ДляКлиента = Истина) Экспорт
	...
	Токены = Новый Массив;
	Для Каждого Токен Из ИнформацияОТокенах Цикл
		Если ЗначениеЗаполнено(Токен.Ошибка) Тогда
			Если ЗначениеЗаполнено(Токен.Слот) Тогда
				Токены.Добавить(СтрШаблон("%1: %2", Токен.Слот, Токен.Ошибка));
			Иначе
				Токены.Добавить(Токен.Ошибка);
			КонецЕсли;
		Иначе
			Механизмы = СтрЗаменить(Токен.Механизмы, Символы.ПС, "");
			Механизмы = СтрЗаменить(Механизмы, Символы.Таб, "");
			Механизмы = СтрЗаменить(Механизмы, " ", "");
			Токены.Добавить(СтрШаблон("%1: %2", Токен.Слот, Механизмы));
		КонецЕсли;
	КонецЦикла;
	
	Если ДляКлиента Тогда
		Если Токены.Количество() > 0 Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Автоматически определенные токены на компьютере:
			|%1'"), СтрСоединить(Токены, Символы.ПС));
		Иначе
			Возврат НСтр("ru = 'На компьютере не определены подключенные токены.'");
		КонецЕсли;
	Иначе
		Если Токены.Количество() > 0 Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Автоматически определенные токены на сервере:
			|%1'"), СтрСоединить(Токены, Символы.ПС));
		Иначе
			Возврат НСтр("ru = 'На сервере не определены подключенные токены.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоСертификатГОСТ(АлгоритмСертификата) Экспорт
	...
	Возврат СтрНачинаетсяС(АлгоритмСертификата, "1.2.643");
...
Функция АлгоритмПодписиСоответствуетСертификату(ПредставлениеСертификата, АлгоритмСертификата, АлгоритмПодписи, АлгоритмХеширования = Неопределено) Экспорт
	...
	Если Не ЭтоСертификатГОСТ(АлгоритмСертификата) Тогда
		Возврат Истина;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(АлгоритмПодписи) И Не ЗначениеЗаполнено(АлгоритмХеширования) Тогда
		Возврат Истина;
	КонецЕсли;

	НаборыАлгоритмовДляСозданияПодписи = НаборыАлгоритмовДляСозданияПодписи();

	АлгоритмыДляПроверки = Неопределено;
	Для Каждого НаборАлгоритмов Из НаборыАлгоритмовДляСозданияПодписи Цикл
		Если НаборАлгоритмов.ИдентификаторАлгоритмаОткрытогоКлюча = АлгоритмСертификата Тогда
			АлгоритмыДляПроверки = НаборАлгоритмов;
			Прервать;
		КонецЕсли;
	КонецЦикла;

	Если АлгоритмыДляПроверки <> Неопределено Тогда
		
		Если АлгоритмПодписи <> АлгоритмСертификата И АлгоритмыДляПроверки.ИдентификаторАлгоритмаПодписи <> АлгоритмСертификата Тогда
			Если АлгоритмыДляПроверки.ИменаАлгоритмаПодписи.Количество() > 0
				И АлгоритмыДляПроверки.ИменаАлгоритмаПодписи.Найти(ВРЕГ(АлгоритмПодписи)) = Неопределено Тогда
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					ПредставлениеСертификата, АлгоритмПодписи, ПредставлениеАлгоритма(
							АлгоритмыДляПроверки.ИменаАлгоритмаПодписи));

				Возврат Ошибка;
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(АлгоритмХеширования) И (АлгоритмХеширования <> АлгоритмыДляПроверки.ИдентификаторАлгоритмаХеширования) Тогда
			Если АлгоритмыДляПроверки.ИменаАлгоритмаХеширования.Количество() > 0
				И АлгоритмыДляПроверки.ИменаАлгоритмаХеширования.Найти(ВРЕГ(
							АлгоритмХеширования)) = Неопределено Тогда
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр(
					...
					ПредставлениеСертификата, АлгоритмХеширования, ПредставлениеАлгоритма(
								АлгоритмыДляПроверки.ИменаАлгоритмаХеширования));
				Возврат Ошибка;
			КонецЕсли;
		КонецЕсли;
...
Функция ДополнительныеСвойстваСертификата(Данные, РазницаСУниверсальнымВременем = Неопределено) Экспорт
	...
	ПропуститьНачалоБлока(АнализДанных, 0, 16);
		...
		ПропуститьНачалоБлока(АнализДанных, 0, 16);
			...
			Если АнализДанных.ЕстьОшибка Тогда
				Структура.ТекстОшибки = НСтр("ru = 'Некорректные данные сертификата.'");
				Возврат Структура;
			КонецЕсли; 
				// SEQUENCE OF
				ПропуститьНачалоБлока(АнализДанных, 0, 16);
					...
					КонецЕсли; 
						...
						Если РазмерДанных = 3 Тогда
							...
							ИначеЕсли СтрокаБуфера = "551D10" Тогда // 2.5.29.16 privateKeyUsagePeriod
								ЗаполнитьПериодДействияЗакрытогоКлюча(ПрочитанныйБлок(АнализДанных, 0, 4, Истина), Структура,
									?(РазницаСУниверсальнымВременем = Неопределено, 0, РазницаСУниверсальнымВременем));
							КонецЕсли;
...
Процедура ЗаполнитьПериодДействияЗакрытогоКлюча(ДвоичныеДанные, Структура, РазницаСУниверсальнымВременем)
	...
	Если РазницаСУниверсальнымВременем <> Неопределено И ЗначениеЗаполнено(Структура.ДатаНачалаЗакрытогоКлюча) Тогда
		Структура.ДатаНачалаЗакрытогоКлюча = Структура.ДатаНачалаЗакрытогоКлюча + РазницаСУниверсальнымВременем;
	КонецЕсли;
	
	Если РазницаСУниверсальнымВременем <> Неопределено И ЗначениеЗаполнено(Структура.ДатаОкончанияЗакрытогоКлюча) Тогда
		Структура.ДатаОкончанияЗакрытогоКлюча = Структура.ДатаОкончанияЗакрытогоКлюча + РазницаСУниверсальнымВременем;
	КонецЕсли;
...
Процедура ЗаполнитьИдентификаторКлючаУдостоверяющегоЦентра(ДвоичныеДанные, Структура)
	...
	ИдентификаторКлючаУдостоверяющегоЦентра = ИдентификаторКлючаУдостоверяющегоЦентра(АнализДанных);
	Если ИдентификаторКлючаУдостоверяющегоЦентра <> Неопределено Тогда
		Структура.ИдентификаторКлючаУдостоверяющегоЦентра = ИдентификаторКлючаУдостоверяющегоЦентра;
	КонецЕсли; 

КонецПроцедуры

Функция ИдентификаторКлючаУдостоверяющегоЦентра(АнализДанных)
	
	Если ТипЗнч(АнализДанных) = Тип("ДвоичныеДанные") Тогда
		АнализДанных = НовыйАнализДанных(АнализДанных);
	КонецЕсли;
...
Функция АлгоритмПодписи(Данные, ЭтоДанныеСертификата, ВключаяOID = Ложь, ТолькоOID = Ложь, АлгоритмПодписиНеСоответствуетГОСТ = Неопределено)
	...
	Возврат ПредставлениеАлгоритмаПодписи(OIDАлгоритмаПодписи, ЭтоДанныеСертификата, ВключаяOID);
	
КонецФункции

Функция ПредставлениеАлгоритмаПодписи(OIDАлгоритмаПодписи, ЭтоДанныеСертификата, ВключаяOID) Экспорт
	
	ИдентификаторыАлгоритмов = ИдентификаторыАлгоритмовПодписи(ЭтоДанныеСертификата);
	Возврат АлгоритмПоOID(OIDАлгоритмаПодписи, ИдентификаторыАлгоритмов, ВключаяOID);
...
Функция АлгоритмШифрованияЗашифрованногоФайла(Данные) Экспорт
	...
	ИнформацияАлгоритма = Новый Структура("Представление, Идентификатор", "", "");
	
	ДвоичныеДанные = ДвоичныеДанныеИзДанных(Данные,
		"ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмШифрованияЗашифрованногоФайла");
	...
	ПропуститьНачалоБлока(АнализДанных, 0, 16);
		...
		ПропуститьНачалоБлока(АнализДанных, 0, 6);
			...
			ПроверитьДанныеБлока(АнализДанных, "2A864886F70D010703");
			ПропуститьРодительскийБлок(АнализДанных);
		// [0]CS             (content [0] EXPLICIT ANY DEFINED BY contentType OPTIONAL).
		ПропуститьНачалоБлока(АнализДанных, 2, 0);
	...
	OIDАлгоритмаШифрования = ПрочитанныйOID(АнализДанных);
	Если АнализДанных.ЕстьОшибка Тогда
		Возврат ИнформацияАлгоритма;
	КонецЕсли;
	
	ИнформацияАлгоритма.Идентификатор = OIDАлгоритмаШифрования;
	
	ИдентификаторыАлгоритмов = ИдентификаторыАлгоритмовШифрования();
	
	ИмяАлгоритма = ИдентификаторыАлгоритмов.Получить(OIDАлгоритмаШифрования);
	Если ЗначениеЗаполнено(ИмяАлгоритма) Тогда
		ИнформацияАлгоритма.Представление = СтрРазделить(ИмяАлгоритма, ",", Ложь)[0];
	КонецЕсли;
...
Функция СвойстваСпискаОтзыва(Данные) Экспорт
	...
	ДвоичныеДанные = ДвоичныеДанныеИзДанных(
		Данные, "ЭлектроннаяПодписьСлужебный.СвойстваСпискаОтзыва");
	
	АнализДанных = НовыйАнализДанных(ДвоичныеДанные);
	
	Результат = Новый Структура("ДатаНачала, ДатаОкончания, ДатаСледующейПубликации, ИдентификаторКлючаУдостоверяющегоЦентра");
	...
	// SEQUENCE (CertificateList).
		ПропуститьНачалоБлока(АнализДанных, 0, 16);
			...
			ПропуститьНачалоБлока(АнализДанных, 0, 16);
				...
				// UTC TIME (thisUpdate).
				Результат.ДатаНачала = ПрочитатьДатуИзБуфера(АнализДанных.Буфер, АнализДанных.Смещение);
				// UTC TIME (nextUpdate).
				Результат.ДатаОкончания = ПрочитатьДатуИзБуфера(АнализДанных.Буфер, АнализДанных.Смещение + 15);
				
				// SEQUENCE (thisUpdate              Time).
				ПропуститьБлок(АнализДанных, 0, 23);
				// SEQUENCE (nextUpdate              Time).
				ПропуститьБлок(АнализДанных, 0, 23, Ложь);
				// SEQUENCE (revokedCertificates).
				ПропуститьБлок(АнализДанных, 0, 16, Ложь);
				...
				Пока АнализДанных.Смещение < СмещениеСледующего Цикл
					...
					КонецЕсли;
						...
						Если РазмерДанных = 3 Тогда
							...
							ПропуститьРодительскийБлок(АнализДанных); // OBJECT IDENTIFIER
							Если СтрокаБуфера = "551D23" Тогда // 2.5.29.35 authorityKeyIdentifier
								ЗаполнитьИдентификаторКлючаУдостоверяющегоЦентра(ПрочитанныйБлок(АнализДанных, 0, 4, Истина), Результат);
							КонецЕсли;
						ИначеЕсли РазмерДанных = 9 Тогда
							Буфер = АнализДанных.Буфер.Прочитать(АнализДанных.Смещение, РазмерДанных); // БуферДвоичныхДанных
							СтрокаБуфера = ПолучитьHexСтрокуИзБуфераДвоичныхДанных(Буфер);
							ПропуститьРодительскийБлок(АнализДанных); // OBJECT IDENTIFIER
							Если СтрокаБуфера = "2B0601040182371504" Тогда // 1.3.6.1.4.1.311.21.4 crlNextPublish
								Результат.ДатаСледующейПубликации = ПрочитатьДатуИзБуфера(АнализДанных.Буфер, АнализДанных.Смещение + 2);
							КонецЕсли;
						Иначе
							ПропуститьРодительскийБлок(АнализДанных); // OBJECT IDENTIFIER
						КонецЕсли;
...
Функция КонецБлокаНеопределеннойДлины(АнализДанных, ОбщийБлок = Ложь)
	...
	Если ОбщийБлок Тогда
		...
		ЛишниеСимволыВКонце = 0;
		Пока Буфер[Буфер.Размер - ЛишниеСимволыВКонце -1] <> 0 И ЛишниеСимволыВКонце < Буфер.Размер Цикл
			ЛишниеСимволыВКонце = ЛишниеСимволыВКонце + 1;
		КонецЦикла;
		
		Смещение = Буфер.Размер - ЛишниеСимволыВКонце - 2;
...
Функция ИдентификаторыАлгоритмовШифрования()
	...
	Для Каждого Набор Из Наборы Цикл
		ИдентификаторыАлгоритмов.Вставить(Набор.ИдентификаторАлгоритмаШифрования,
			СтрСоединить(Набор.ИменаАлгоритмаШифрования, ", "));
	КонецЦикла;
...
Функция НовыйНаборАлгоритмовШифрования() Экспорт
	...
	Свойства = Новый Структура;
	Свойства.Вставить("ИдентификаторАлгоритмаШифрования", "");
	Свойства.Вставить("ИменаАлгоритмаШифрования", Новый Массив);
	
	Возврат Свойства;
	
КонецФункции

Функция ПредставлениеАлгоритма(ИменаАлгоритмаПодписи)
	ВГраница = ИменаАлгоритмаПодписи.ВГраница();
	Если ВГраница = 0 Тогда
		Возврат ИменаАлгоритмаПодписи[0];
	Иначе
		Имена = "";
		Для н = 1 По ВГраница Цикл
			Имена = Имена + ИменаАлгоритмаПодписи[н] + ?(н = ВГраница, "", ", ");
		КонецЦикла;
		Возврат ИменаАлгоритмаПодписи[0] + " (" + Имена + ")";
	КонецЕсли;
КонецФункции
