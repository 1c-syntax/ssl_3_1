...
Функция МенеджерКриптографииОписанияПрограмм(Программа, Ошибки, Знач ОписанияПрограмм, АлгоритмПодписи = "") Экспорт
	...
	ИначеЕсли Не ПустаяСтрока(АлгоритмПодписи) Тогда
		
		Если СтрНайти(АлгоритмПодписи, ",") > 0 Тогда
			
			ВозможныеАлгоритмы = СтрРазделить(АлгоритмПодписи, ",", Ложь);
			Для СчетчикАлгоритмов = 0 По ВозможныеАлгоритмы.Количество() - 1 Цикл
				ВозможныеАлгоритмы[СчетчикАлгоритмов] = СокрЛП(ВозможныеАлгоритмы[СчетчикАлгоритмов]);
			КонецЦикла;
			
		Иначе
			ВозможныеАлгоритмы = Новый Массив;
			ВозможныеАлгоритмы.Добавить(СокрЛП(АлгоритмПодписи));
		КонецЕсли;
		
		ПрограммыСУказаннымАлгоритмом = Новый Массив;
		Для Каждого ОписаниеПрограммы Из ОписанияПрограмм Цикл
			
			Если ВозможныеАлгоритмы.Найти(ОписаниеПрограммы.АлгоритмПодписи) <> Неопределено Тогда
				ПрограммыСУказаннымАлгоритмом.Добавить(ОписаниеПрограммы);
			КонецЕсли;
			
		КонецЦикла;
		
		Возврат ПрограммыСУказаннымАлгоритмом;
...
Функция АлгоритмПодписи(Данные, ЭтоДанныеСертификата = Ложь)
	...
	Если ТипЗнч(Данные) = Тип("Строка")
		И ЭтоАдресВременногоХранилища(Данные) Тогда
		
		ЧтениеДанных = Новый ЧтениеДанных(
			ПолучитьИзВременногоХранилища(Данные));
	...
	Попытка
		...
		Если ЭтоДанныеСертификата Тогда
			ПропуститьНачалоБлока(ЧтениеДанных, 3);
			ПропуститьБлок(ЧтениеДанных, 2);
			ПропуститьНачалоБлока(ЧтениеДанных, 1);
		Иначе
			
			ПропуститьНачалоБлока(ЧтениеДанных, 1);
			ПропуститьБлок(ЧтениеДанных, 1);
			ПропуститьНачалоБлока(ЧтениеДанных, 2);
			ПропуститьБлок(ЧтениеДанных, 3);
			ПропуститьНачалоБлока(ЧтениеДанных, 3);
			ПропуститьБлок(ЧтениеДанных, 2);
			ПропуститьНачалоБлока(ЧтениеДанных, 1);
		...
		РазмерБлока = ЧтениеДанных.ПрочитатьБайт();
		Идентификатор = Новый Массив;
		
		БайтSID1 = ЧтениеДанных.ПрочитатьБайт();
		...
		Для Индекс = 1 По РазмерБлока - 1 Цикл
			...
			ТекущийБайт = ЧтениеДанных.ПрочитатьБайт();
	...
	Возврат ?(ПредставлениеАлгоритма = Неопределено, "", ПредставлениеАлгоритма);
	
КонецФункции

Процедура ПропуститьНачалоБлока(ЧтениеДанных, КоличествоИтераций)
	...
	Для СчетчикПоследовательностей = 1 По КоличествоИтераций Цикл
		...
		Если РазмерБлока > 128 Тогда
			ЧтениеДанных.ПрочитатьВБуферДвоичныхДанных(РазмерБлока - 128);
		КонецЕсли;
...
Процедура ПропуститьБлок(ЧтениеДанных,КоличествоИтераций)
	...
	Для СчетчикДанных = 1 По КоличествоИтераций Цикл
		...
		Иначе
			...
			Буфер = ЧтениеДанных.ПрочитатьВБуферДвоичныхДанных(РазмерБлока - 128);
			
			ДлинаБлокаДанных = 0;
			РазмерБуфера = Буфер.Размер - 1;
			Для ТекущийБайт = 0 По РазмерБуфера Цикл
				ДлинаБлокаДанных = ДлинаБлокаДанных
					+ Буфер[ТекущийБайт] * Pow(16, 2*(РазмерБуфера - ТекущийБайт));
			КонецЦикла;
				
		КонецЕсли;
		ЧтениеДанных.ПрочитатьВБуферДвоичныхДанных(ДлинаБлокаДанных);
