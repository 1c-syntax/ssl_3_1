			...
			ПропуститьНачалоБлока(АнализДанных, 0, 16);
				...
				ПропуститьНачалоБлока(АнализДанных, 0, 16);
					...
					Если СтрокаБуфера = "2A864886F70D0109100104" Тогда //1.2.840.113549.1.9.16.1.4 tSTInfo
						...
						РазмерДанных = АнализДанных.Родители[1].РазмерДанных - 17;
						
						Буфер = АнализДанных.Буфер.Прочитать(АнализДанных.Смещение + 15, РазмерДанных);
...
Функция ДатыСертификата(Сертификат, РазницаСУниверсальнымВременем) Экспорт
	...
	Если ДоступныДополнительныеСвойстваСертификата() И ТипЗнч(Сертификат) = Тип("СертификатКриптографии") Тогда
		Если Сертификат.УниверсальнаяДатаНачалаЗакрытогоКлюча <> Неопределено Тогда
			ДатыСертификата.Вставить("ДатаНачалаЗакрытогоКлюча", Сертификат.УниверсальнаяДатаНачалаЗакрытогоКлюча
				+ РазницаСУниверсальнымВременем);
		КонецЕсли;
		Если Сертификат.УниверсальнаяДатаОкончанияЗакрытогоКлюча <> Неопределено Тогда
			ДатыСертификата.Вставить("ДатаОкончанияЗакрытогоКлюча", Сертификат.УниверсальнаяДатаОкончанияЗакрытогоКлюча
				+ РазницаСУниверсальнымВременем);
