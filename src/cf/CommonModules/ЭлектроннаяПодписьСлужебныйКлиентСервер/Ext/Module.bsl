...
Функция РезультатПоискаКриптопровайдеров(КриптопровайдерыРезультат, ИмяСервера = "") Экспорт
	...
	Если КриптопровайдерыРезультат = Неопределено Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 не удалось автоматически определить установленные приложения электронной подписи и шифрования.'"), ИмяСервера);
	ИначеЕсли КриптопровайдерыРезультат.ПроверкаВыполнена Тогда
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 не установлены приложения электронной подписи и шифрования.'"), ИмяСервера);
		КонецЕсли;
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 не удалось автоматически определить установленные приложения электронной подписи и шифрования:
