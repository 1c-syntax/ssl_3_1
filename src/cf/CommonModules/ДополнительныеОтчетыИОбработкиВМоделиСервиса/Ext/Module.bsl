...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Результат = Менеджер.Подключить(АдресВоВременномХранилище, ПараметрыПодключения.ИмяОбъекта, БезопасныйРежим, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
	// АПК:553-вкл
	// АПК:556-вкл
	// АПК:552-вкл
...
Процедура ПриСозданииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ЭтоОтчет(Ссылка) Тогда
		Результат = ВнешниеОтчеты.Создать(ИмяОбработки); // Не нарушает безопасность информации, так как создается уже подключенный объект.
	Иначе
		Результат = ВнешниеОбработки.Создать(ИмяОбработки); // Не нарушает безопасность информации, так как создается уже подключенный объект.
	КонецЕсли;
	// АПК:553-вкл
	// АПК:552-вкл
...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, 
	...
	Попытка
		...
		Иначе
			...
			Менеджер.ПараметрыИнсталляции = Новый ХранилищеЗначения(Контекст, Новый СжатиеДанных(9));
			Менеджер.Записать();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("УдалитьПоставляемуюОбработкуИзОбластиДанных");
		Методы.Вставить("УстановитьПоставляемуюОбработкуПриПолучении");
		Методы.Вставить("АктуализацияНастроекИспользуемойОбработки");
	КонецЕсли;
