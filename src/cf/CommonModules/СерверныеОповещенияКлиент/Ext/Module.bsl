...
Процедура ПроверитьПолучитьСерверныеОповещенияСПоказателями(СостояниеПолучения, Показатели);
	...
	Если ПериодическаяОтправкаДанных Тогда
		...
		Если ЗакончилосьВремяОжидания(ИмяПараметра) Тогда
			...
			Если СостояниеПолучения.ИдентификаторЛичногоОбсуждения = Неопределено Тогда
	...
	Если ПериодическаяОтправкаДанных Тогда
		...
		Если РезультатОбщегоВызова.Свойство("РегистрироватьПоказатели") Тогда
			СостояниеПолучения.РегистрироватьПоказатели = РезультатОбщегоВызова.РегистрироватьПоказатели;
		КонецЕсли;
...
Процедура ОбработатьСерверноеОповещениеНаКлиенте(СостояниеПолучения, СерверноеОповещение)
	...
	Если ИмяОповещения = "СтандартныеПодсистемы.БазоваяФункциональность.СерверныеОповещения.РегистрацияПоказателейИзменена" Тогда
		СостояниеПолучения.РегистрироватьПоказатели = (Результат = Истина);
		Возврат;
	ИначеЕсли ИмяОповещения = "СтандартныеПодсистемы.БазоваяФункциональность.СерверныеОповещения.ДоставкаБезСистемыВзаимодействияИзменена" Тогда
