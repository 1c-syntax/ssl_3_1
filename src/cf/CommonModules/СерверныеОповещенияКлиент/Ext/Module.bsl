...
Процедура ПроверитьПолучитьСерверныеОповещенияСПоказателями(СостояниеПолучения, Показатели);
	...
	Интервал = 60;
	ОбсужденияАктивны = СостояниеПолучения.УведомленияКлиентаДоступны
	...
	Если ТипЗнч(РезультатОбщегоВызова) <> Тип("Структура") Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Функция ""%1""
			           |вернула некорректное значение ""%2"" типа ""%3"" вместо типа ""%4"":
			           |- либо функция была изменена расширением;
			           |- либо после очередного обновления конфигурации некорректно обновились кэши метаданных платформы 1С:Предприятия на клиенте или на сервере (тогда их нужно почистить);
			           |- либо при серверном вызове возникла необычная ошибка, которая была некорректно обработана платформой 1С:Предприятия и вместо вызова исключения возвращено значение Неопределено (для регистрации нужна возможность устойчивого воспроизведения).'"),
			"СерверныеОповещенияСлужебныйВызовСервера.НедоставленныеСерверныеОповещенияСеанса",
			Строка(РезультатОбщегоВызова),
			Строка(ТипЗнч(РезультатОбщегоВызова)),
			Строка(Тип("Структура")));
		Попытка
			ВызватьИсключение ТекстОшибки;
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			СерверныеОповещенияСлужебныйВызовСервера.ЗарегистрироватьОшибкуПолученияНедоставленныхСерверныхОповещений(
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
		ПодключитьОбработчикПроверкиПолученияСерверныхОповещений(15);
		Возврат;
	...
	ТекущаяДатаСеанса = ОбщегоНазначенияКлиент.ДатаСеанса();
	Если РезультатОбщегоВызова.Свойство("ДатаПоследнегоОповещения")
	   И РезультатОбщегоВызова.ДатаПоследнегоОповещения <= ТекущаяДатаСеанса + 2 Тогда
	...
	КонецЕсли;
	СостояниеПолучения.ДатаОбновленияСостояния = ТекущаяДатаСеанса;
	СостояниеПолучения.ДатаПоследнегоСерверногоВызова = ТекущаяДатаСеанса;
...
Функция СостояниеПолучения() Экспорт
	...
	СостояниеПолучения.УведомленияКлиентаДоступны =
		СтандартныеПодсистемыКлиентПовтИсп.УведомленияКлиентаДоступны();
...
//   * ОбработчикУведомленийКлиентаПодключен - Булево
...
Функция НовоеСостояниеПолучения()
	...
	Состояние.Вставить("УведомленияКлиентаДоступны", Ложь);
	Состояние.Вставить("ОбработчикУведомленийКлиентаПодключен", Ложь);
...
Процедура ПодключитьОбработчикНовыхСообщений(СостояниеПолучения)
	...
	Если СостояниеПолучения.УведомленияКлиентаДоступны Тогда
		Если СостояниеПолучения.ОбработчикУведомленийКлиентаПодключен Тогда
			Возврат;
		КонецЕсли;
		...
		СостояниеПолучения.ОбработчикУведомленийКлиентаПодключен = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПриПолученииСерверногоОповещения(Данные, СостояниеПолучения)
	...
	ЭтоДатаДобавленияОповещения = Ложь;
	...
	Если ДатаПоследнегоОповещения = Неопределено Тогда
		...
		Если ДатаПоследнегоОповещения = Неопределено Тогда
			...
			ЭтоДатаДобавленияОповещения = Истина;
		КонецЕсли;
	КонецЕсли;
	
	ТекущаяДатаСеанса = ОбщегоНазначенияКлиент.ДатаСеанса();
	Если ДатаПоследнегоОповещения <= ТекущаяДатаСеанса + 2 Тогда
		...
		КонецЕсли;
		Если ЭтоДатаДобавленияОповещения Тогда
			СостояниеПолучения.ДатаОбновленияСостояния = ТекущаяДатаСеанса;
		КонецЕсли;
	КонецЕсли;
...
Функция ОповещениеУжеПолучено(СостояниеПолучения, СерверноеОповещение)
	...
	Если СерверноеОповещение.ДатаДобавления < СостояниеПолучения.ДатаПоследнегоОповещения - 5
	   И (СерверноеОповещение.ОтправленоИзОчереди
	      Или Не СостояниеПолучения.УведомленияКлиентаДоступны) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если ПолученныеОповещения.Количество() > 1000 Тогда
