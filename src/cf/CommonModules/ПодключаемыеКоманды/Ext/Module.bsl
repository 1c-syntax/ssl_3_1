	...
	Для Каждого СведенияОПодменюКоманды Из СведенияОВсехПодменю Цикл
		Для Каждого ИндексКоманды Из СведенияОПодменюКоманды.Команды Цикл
			...
			Команда.ИмяВФорме = ОпределитьИмяКоманды(Форма, ГруппаФормы.Имя, Команда.Идентификатор, 
				СчетчикКомандСАвтогенерируемымИменем, ПрефиксКоманд, ЗанятыеИмена);
			...
			ЗанятыеИмена.Вставить(Команда.ИмяВФорме, Истина);
			...
			Если СведенияОПодменюКоманды.КоличествоКоманд = 1 И ТипЗнч(ГруппаФормы.Родитель) <> Тип("ФормаКлиентскогоПриложения") 
...
Функция ОпределитьИмяКоманды(Форма, ИмяГруппы, ИдентификаторКоманды, СчетчикКомандСАвтогенерируемымИменем, ПрефиксКоманд, ЗанятыеИмена)
	...
	Пока ЗанятыеИмена[ИмяКоманды] = Истина Цикл
		СчетчикКомандСАвтогенерируемымИменем = СчетчикКомандСАвтогенерируемымИменем + 1;
		ИмяКоманды = ИмяГруппы + "_" + ПрефиксКоманд + "_" + Формат(СчетчикКомандСАвтогенерируемымИменем, "ЧН=; ЧГ=");
	КонецЦикла;
	Возврат ИмяКоманды;
