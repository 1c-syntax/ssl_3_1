	...
	Для Каждого ВидКоманд Из ВидыКоманд Цикл
		...
		Для Каждого Команда Из КомандыВида Цикл
			...
			Если ЗначениеЗаполнено(Команда.ЗначениеПометки) И ЭтоПутьДоРеквизита(Команда.ЗначениеПометки) Тогда
				ПодключенныеКоманды.ОписаниеКоманд.Добавить(ОписаниеКомандыНаКлиенте(Команда, КнопкаФормы));
			КонецЕсли;
...
Функция КлючПараметровРазмещения(Знач ПараметрыРазмещения)
	...
	Если ТипЗнч(ПараметрыРазмещения.КоманднаяПанель) = Тип("ГруппаФормы") Тогда
		ПараметрыРазмещения.КоманднаяПанель = ПараметрыРазмещения.КоманднаяПанель.Имя;
	КонецЕсли;
...
Функция ЭтоПутьДоРеквизита(ПутьДоРеквизита)
	Результат = Истина;
...
//
// Возвращаемое значение:
//   см. ПодключаемыеКомандыПереопределяемый.ПриОпределенииКомандПодключенныхКОбъекту.Команды
//
