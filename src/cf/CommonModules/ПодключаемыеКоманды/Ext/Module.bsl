...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Для Каждого ВидКоманд Из ВидыКоманд Цикл
		...
		Для Каждого Команда Из КомандыВида Цикл 
			...
			КонецЕсли;
			КомандаФормы.ИзменяетСохраняемыеДанные = Команда.ИзменяетВыбранныеОбъекты И ПараметрыРазмещения.ЭтоФормаОбъекта;
			...
			ИначеЕсли Команда.Важность <> "СмТакже" Тогда
		...
		Если Не ЭтоКоманднаяПанель Тогда
			...
			Иначе
				...
				Если Элементы.Найти(ИмяКомандыЗаглушки) = Неопределено Тогда
					...
					Если ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(ВерсияПлатформыБезНомераСборки, "8.3.15") >= 0 Тогда
						КнопкаФормы.ПоложениеВКоманднойПанели = ОбщегоНазначения.ВычислитьВБезопасномРежиме("ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ОперативноеОбновлениеОбщихДанных(Отбор)
	...
	Для Каждого ОбъектМетаданныхПоставщика Из Состав Цикл
		...
		Если ТипЗнч(СсылкаПодключенногоОМ) <> Отбор Тогда
			Продолжить;
		КонецЕсли;
