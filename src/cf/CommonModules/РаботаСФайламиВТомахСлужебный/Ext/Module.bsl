...
Процедура ДобавитьФайл(ПрисоединенныйФайл,
	...
	Если ТипХраненияФайла(РасширениеФайла, РазмерФайла) = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		Если Не ЗначениеЗаполнено(СсылкаНаФайл) Тогда
			СсылкаНаФайл = Справочники[ПрисоединенныйФайл.Метаданные().Имя].ПолучитьСсылку();
...
//     * ВладелецФайла          - ОпределяемыйТип.ВладелецПрисоединенныхФайлов, ОпределяемыйТип.ВладелецФайлов,
//                              Неопределено - владелец файла.
//     * Наименование           - Строка - наименование файла.
//     * Расширение             - Строка - расширение файла.
//
Функция СвойстваФайлаВТоме(Файл = Неопределено) Экспорт
	
	ИменаРеквизитов = "Том, ПутьКФайлу, Наименование, Расширение, ВладелецФайла";
	
	СвойстваФайла = Новый Структура(ИменаРеквизитов);
	Если ЗначениеЗаполнено(Файл) Тогда
		
		Если ТипЗнч(Файл) = Тип("СправочникСсылка.ВерсииФайлов") Тогда
			ИменаРеквизитов = СтрЗаменить(ИменаРеквизитов, "ВладелецФайла", "Владелец") + ", НомерВерсии";
		КонецЕсли;
...
//
// Параметры:
//   СвойстваФайла - Структура - См. СвойстваФайлаВТоме.
//
...
Функция ОбъемТома(Том) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(Версии.Размер), 0)
	|ИЗ
...
Процедура ДобавитьФайлыВТомаПриРазмещении(СоответствиеПутейФайлов, ТипХраненияФайла)
	...
	ВерсииФайлов = Запрос.Выполнить().Выгрузить();
...
Процедура ПроверитьЦелостностьФайлов(ТаблицаФайловНаДиске, Том) Экспорт
	...
	Для Каждого СправочникФайлов Из ТипыФайлов Цикл
		...
		Если МетаданныеСправочника.Реквизиты.Найти("ВладелецФайла") <> Неопределено Тогда
			...
			|	СправочникПрисоединенныеФайлы.ВладелецФайла " + ?(ДобавитьПсевдонимПоля, "КАК ВладелецФайла,",",") + "
		...
		ИначеЕсли МетаданныеСправочника.Реквизиты.Найти("РодительскаяВерсия") <> Неопределено Тогда
			...
			|ВЫБРАТЬ
			|	СправочникПрисоединенныеФайлы.Ссылка " + ?(ДобавитьПсевдонимПоля, "КАК Ссылка,",",") + "
			|	СправочникПрисоединенныеФайлы.Владелец.ВладелецФайла " + ?(ДобавитьПсевдонимПоля, "КАК ВладелецФайла,",",") + "
			|	СправочникПрисоединенныеФайлы.Расширение " + ?(ДобавитьПсевдонимПоля, "КАК Расширение,",",") + "
			|	СправочникПрисоединенныеФайлы.Наименование " + ?(ДобавитьПсевдонимПоля, "КАК Наименование,",",") + "
			|	СправочникПрисоединенныеФайлы.Том " + ?(ДобавитьПсевдонимПоля, "КАК Том,",",") + "
