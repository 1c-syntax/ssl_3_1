...
Процедура ЗаполнитьСведенияОФайле(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть, 
	...
	РазмерФайла = ПрисоединенныйФайл.Размер;
	ТипХраненияФайла = ПрисоединенныйФайл.ТипХраненияФайла;
	Если НЕ ЗначениеЗаполнено(ТипХраненияФайла) Тогда
		ТипХраненияФайла = РаботаСФайламиСлужебный.ТипХраненияФайлов(ПрисоединенныйФайл.Размер, ПрисоединенныйФайл.Расширение);
	КонецЕсли;
	...
	Иначе
		...
		ТомХранения = СвободныйТом(РазмерФайла);
...
// Параметры:
//   ПрисоединенныйФайл - ОпределяемыйТип.ПрисоединенныйФайл - ссылка на элемент справочника с файлом.
//   Путь - Строка - полный путь (включая имя файла), куда будет скопирован файл из тома.
//
...
Процедура ПереименоватьФайл(ПрисоединенныйФайл,Знач НовоеИмя,
	...
	Попытка
		...
		НовыйПутьКФайлу = ФайлНаДиске.Путь
			+ РаботаСФайламиСлужебныйКлиентСервер.ПолучитьУникальноеИмяСПутем(ФайлНаДиске.Путь, НовоеИмяФайла);
...
Процедура ОчиститьУдаленныеФайлы() Экспорт
	...
	Для Каждого Том Из ДоступныеТома() Цикл
		ОчиститьУдаленныеФайлыВТоме(Том);
	КонецЦикла;
КонецПроцедуры
...
//  Том - СправочникСсылка.ТомаХраненияФайлов
//
Процедура ОчиститьУдаленныеФайлыВТоме(Том)
	РеквизитыТома = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Том, "ВремяПоследнейОчисткиФайлов, ПометкаУдаления");
	
	Если РеквизитыТома.ПометкаУдаления <> Неопределено И РеквизитыТома.ПометкаУдаления Тогда
		Возврат;
	КонецЕсли;
	
	ВремяПоследнейОчистки = РеквизитыТома.ВремяПоследнейОчисткиФайлов;
	КорневойКаталог = ПолныйПутьТома(Том);
	
	ФайлыДляУдаления = ФайлыДляУдаления(КорневойКаталог, ВремяПоследнейОчистки);
	
	Для Каждого Файл Из УдаленныеФайлы(Том, КорневойКаталог, ФайлыДляУдаления) Цикл
		УдалитьФайл(Файл);
	КонецЦикла;
	...
	Попытка
		...
		Элемент = Блокировка.Добавить(Том.Метаданные().ПолноеИмя());
		Элемент.УстановитьЗначение("Ссылка", Том);
...
//   Массив из Файл
//  
Функция УдаленныеФайлы(Том, КорневойКаталог, ФайлыДляУдаления)
...
// Имя метаданного (80) + дата (6) + подтом для исключения повторения (6 символов) + имя файла (150).
// Итого: 242.
//
Функция ЗапросПроверкиСуществованияФайлов(Знач ТаблицаФайлы)
	...
	|		ПО ПрисоединенныеФайлы.Том = &Том
...
Процедура ПередОбновлениемДанныхФайла(Контекст) Экспорт
	...
	КонтейнерСвойствФайла.ПутьКФайлу = ""; // Всегда будем создавать новую версию на диске, не затирая старую.
...
Процедура ДобавитьФайлыВТомаПриРазмещении(СоответствиеПутейФайлов, ТипХраненияФайла)
	...
	Для каждого СведенияОФайлеПуть Из СоответствиеПутейФайлов Цикл
		...
		Попытка
			...
			Объект = СсылкаНаФайл.ПолучитьОбъект();
...
//
Функция ТипХраненияФайла(Расширение, Размер) Экспорт
