...
//   Булево
//
Функция ЕстьТомаХраненияФайлов(Знач ВидТомаХраненияФайлов = Неопределено, Знач СпособХраненияФайлов = Неопределено) Экспорт
	...
	|	И (&ЛюбойСпособХраненияФайлов
	|			ИЛИ ТомаХраненияФайлов.СпособХраненияФайлов = &СпособХраненияФайлов)";

	Запрос.УстановитьПараметр("ВидТомаХраненияФайлов"		, ВидТомаХраненияФайлов);
	Запрос.УстановитьПараметр("СпособХраненияФайлов"		, СпособХраненияФайлов);
	Запрос.УстановитьПараметр("ЛюбойСпособХраненияФайлов"	, СпособХраненияФайлов = "ВТомахСЛюбымСпособомХранения");
...
//
// Параметры:
//   ПрисоединенныйФайл - ОпределяемыйТип.ПрисоединенныйФайл, Структура - ссылка на элемент справочника с файлом, либо значения 
//                       его свойств в виде структуры. Ключи структуры должны быть идентичны ключам метода см.
//                       СвойстваФайлаВТоме, и содержать ключ "Ссылка".
...
//
Функция ДанныеФайла(Источник, Знач ВызыватьИсключение = Истина, ПолучениеФайлаИзАрхива = Ложь) Экспорт

	Результат = Неопределено;
	Если ТипЗнч(Источник) = Тип("Структура") Тогда
		СвойстваФайла = Источник;
		ФайлСсылка = СвойстваФайла.Ссылка;
	Иначе
		СвойстваФайла = СвойстваФайлаВТоме(Источник);
		ФайлСсылка = Источник;
	КонецЕсли;
	
	Попытка
		ПолноеИмяФайлаВТоме = ПолноеИмяФайлаВТоме(СвойстваФайла);
		Результат = Новый ДвоичныеДанные(ПолноеИмяФайлаВТоме);
	Исключение
		ФайлОбъект = РаботаСФайламиСлужебный.ФайлОбъект(ФайлСсылка);
		РаботаСФайламиСлужебный.СообщитьОбОшибкеФайлНеНайден(ФайлОбъект, ВызыватьИсключение, ПолучениеФайлаИзАрхива);
	КонецПопытки;
...
Процедура ЗаполнитьСведенияОФайле(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть, 
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		РаботаСФайламиСлужебный.ЗаписатьФайлВИнформационнуюБазу(СсылкаНаФайл, ДанныеФайла, ПараметрыЗаписиВИБ);
	...
	Иначе
		...
		Иначе
			ПрисоединенныйФайл.Том = РаботаСФайловымАрхивомСервер.СвободныйТомХраненияПоТипуХраненияФайлов(ПрисоединенныйФайл, 
				ТипХраненияФайла, Перечисления.ВидыХраненияФайлов.ОперационноеХранение);
		КонецЕсли;
		
		Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахТомНеОпределен Тогда
			ПрисоединенныйФайл.ТипХраненияФайла = РаботаСФайловымАрхивомСервер.ТипХраненияПоТомуХраненияФайлов(ПрисоединенныйФайл.Том);
		ИначеЕсли ПрисоединенныйФайл.ТипХраненияФайла <> ТипХраненияФайла Тогда
...
Процедура ПереименоватьФайл(ПрисоединенныйФайл,Знач НовоеИмя,
	...
	Попытка
		...
		ПрисоединенныйФайлОбъект.ДополнительныеСвойства.Вставить("РазрешитьИзменениеПутиФайла", Истина);
...
Функция СвойстваФайлаВТоме(Файл = Неопределено) Экспорт
	...
	Результат.Вставить("Том", Справочники.ТомаХраненияФайлов.ПустаяСсылка());
	Результат.Вставить("ПутьКФайлу", "");
	Результат.Вставить("ВладелецФайла", Неопределено);
	...
	Если ЗначениеЗаполнено(Файл) Тогда
		...
		ЗаполнитьЗначенияСвойств(Результат, РеквизитыФайла);
		 
		Если РеквизитыФайла.Свойство("Владелец") И ЗначениеЗаполнено(РеквизитыФайла.Владелец) Тогда
			Результат.ВладелецФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыФайла.Владелец, "ВладелецФайла");
		КонецЕсли;
		
	КонецЕсли;
	
	Если ТипЗнч(Результат.НомерВерсии) <> Тип("Строка") Тогда
		Результат.НомерВерсии = Строка(Результат.НомерВерсии);
	КонецЕсли;
...
// см. РаботаСФайламиСлужебныйПовтИсп.ПолныйПутьТома
Функция ПолныйПутьТома(Том) Экспорт
	Результат = РаботаСФайламиСлужебныйПовтИсп.ПолныйПутьТома(Том);
	Возврат Результат;
КонецФункции
...
Функция ОбъемТома(Том) Экспорт
	Тома = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Том);
	ОбъемыТомов = ОбъемыТомов(Тома);
	Результат = ОбъемыТомов.Получить(Том);
	Возврат Результат;
КонецФункции
...
//     * Значение - Число - размер тома в байтах
//
Функция ОбъемыТомов(Тома)
	Результат = Новый Соответствие;
	Для Каждого Том Из Тома Цикл
		Результат.Вставить(Том, 0);
	КонецЦикла; 
	...
	МетаданныеСправочниковСФайламиДляРасчетаОбъемаТомов = МетаданныеСправочниковСФайламиДляРасчетаОбъемаТомов();
	Порции = РазбитьНаПорции(МетаданныеСправочниковСФайламиДляРасчетаОбъемаТомов, 250);
	Для Каждого Порция Из Порции Цикл
		//@skip-check query-in-loop
		ПоместитьОбъемТомовПорцииСправочниковСФайлами(Результат, Тома, Порция);
	КонецЦикла;
	Возврат Результат;
КонецФункции

Процедура ПоместитьОбъемТомовПорцииСправочниковСФайлами(ОбъемыТомов, Тома, Порция)
	ТекстЗапросаОбъемаТомовИзПорцииСправочников = ТекстЗапросаОбъемаТомовИзПорцииСправочников(Порция);
	Запрос = Новый Запрос(ТекстЗапросаОбъемаТомовИзПорцииСправочников);
	Запрос.УстановитьПараметр("Тома", Тома);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		ОбъемТома = ОбъемыТомов.Получить(Выборка.Том) + Выборка.Размер;
		ОбъемыТомов.Вставить(Выборка.Том, ОбъемТома);
	КонецЦикла;
КонецПроцедуры

Функция ТекстЗапросаОбъемаТомовИзПорцииСправочников(Порция)
	ЧастиВложенногоЗапроса = Новый Массив;
	...
	Для Каждого МетаданныеСправочника Из Порция Цикл
		ЧастьВложенногоЗапроса = СтрЗаменить(ШаблонВложенногоЗапроса, "&ИмяТаблицы", МетаданныеСправочника.ПолноеИмя());
		ЧастиВложенногоЗапроса.Добавить(ЧастьВложенногоЗапроса);
	КонецЦикла;
	
	ТекстВложенногоЗапроса = СтрСоединить(ЧастиВложенногоЗапроса, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС);
	ТекстВложенногоЗапроса = СтрШаблон("(%1)", ТекстВложенногоЗапроса);
	
	ШаблонЗапроса = "ВЫБРАТЬ
	|	ВложенныйЗапрос.Том КАК Том,
	|	СУММА(ВложенныйЗапрос.Размер) КАК Размер
	|ИЗ
	|	&ТекстВложенногоЗапроса КАК ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Том";
	Результат = СтрЗаменить(ШаблонЗапроса, "&ТекстВложенногоЗапроса", ТекстВложенногоЗапроса);
	Возврат Результат;
КонецФункции

Функция РазбитьНаПорции(ИсходныйМассив, РазмерПорции)
	Результат = Новый Массив();
	РазмерВложеннойПорции = 0;
	Для Каждого Элемент Из ИсходныйМассив Цикл
		Если РазмерВложеннойПорции = 0 Тогда
			Порция = Новый Массив();
			Результат.Добавить(Порция);
		КонецЕсли;
		Порция.Добавить(Элемент);
		РазмерВложеннойПорции = РазмерВложеннойПорции + 1;
		Если РазмерВложеннойПорции = РазмерПорции Тогда
			РазмерВложеннойПорции = 0;
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
...
Функция МетаданныеСправочниковСФайламиДляРасчетаОбъемаТомов()
	Результат = Новый Массив();
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ПрисоединенныйФайл.Тип.Типы() Цикл
		...
		Если МетаданныеСправочника.Реквизиты.Найти("ТекущаяВерсия") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Результат.Добавить(МетаданныеСправочника);
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
// Возвращает признак того, что файлы могут храниться в томах на дисках.
//
// Возвращаемое значение:
//  Булево
//
Функция ХранитьФайлыВТомах() Экспорт
	...
	Возврат РаботаСФайламиКлиентСервер.ХранитьФайлыВТомах(СпособХраненияФайлов);
...
Процедура ЗаписатьДанныеФайлаВТом(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть)
	...
	Попытка
		...
		РаботаСФайламиСлужебный.УстановитьУниверсальноеВремяИзменения(ПутьКФайлу, ПрисоединенныйФайл.ДатаМодификацииУниверсальная);
...
Функция ЗапросПроверкиСуществованияФайловСВосстановленнымиСвязями(Знач ТаблицаФайлы)
	...
	|ИЗ
	|	&Файлы КАК Файлы
	|ИНДЕКСИРОВАТЬ ПО Путь, АльтернативныйПуть");  
	...
	|	Файлы КАК Файлы
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ &СправочникПрисоединенныхФайлов КАК ПрисоединенныеФайлы
	|		ПО НЕ Файлы.ИспользоватьАльтернативныйПуть И ВЫРАЗИТЬ(ПрисоединенныеФайлы.ПутьКФайлу КАК Строка(1024)) = Файлы.Путь
	|ГДЕ
	|	ПрисоединенныеФайлы.Том В (&Тома)
	| 
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Файлы КАК Файлы
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ &СправочникПрисоединенныхФайлов КАК ПрисоединенныеФайлы
	|		ПО Файлы.ИспользоватьАльтернативныйПуть И ВЫРАЗИТЬ(ПрисоединенныеФайлы.ПутьКФайлу КАК Строка(1024)) = Файлы.АльтернативныйПуть
	|ГДЕ
	|	ПрисоединенныеФайлы.Том В (&Тома)
	|";
...
Процедура ПередОбновлениемДанныхФайла(Контекст) Экспорт
	...
	Контекст.ИзменяемыеРеквизиты.Вставить("ТипХраненияФайла", Неопределено);
	Если НЕ Контекст.ЭтоНовый Тогда
		ТипХраненияФайлаПроверяемый = Контекст.ПараметрыДобавленияФайла.ТипХраненияФайла;
		Если ТипХраненияФайлаПроверяемый <> Перечисления.ТипыХраненияФайлов.ВоВнешнемХранилищеДвоичныхДанных
				И ТипХраненияФайлаПроверяемый <> Перечисления.ТипыХраненияФайлов.ВоВстроенномХранилищеДвоичныхДанных Тогда
	...
	УстановитьОтключениеБезопасногоРежима(Истина);
	ДобавитьФайл(КонтейнерСвойствФайла, Контекст.ДанныеФайла,,,КонтейнерСвойствФайла.Том);
	...
	Контекст.ИзменяемыеРеквизиты.ТипХраненияФайла = КонтейнерСвойствФайла.ТипХраненияФайла;
КонецПроцедуры
...
Процедура ДобавитьФайлыВТомаПриРазмещении(СоответствиеПутейФайлов, ТипХраненияФайла)
	...
	Для каждого СведенияОФайлеПуть Из СоответствиеПутейФайлов Цикл
		...
		Попытка
			...
			РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(Объект, ДвоичныеДанные, Объект.Размер, Объект.Расширение);

			Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(Объект.ТипХраненияФайла) Тогда
				...
				ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(Объект);
				РаботаСФайламиСлужебный.ЗаписатьФайлВИнформационнуюБазу(Объект.Ссылка, ДвоичныеДанные, ПараметрыЗаписиВИБ);
			...
			Иначе
				...
				ДобавитьФайл(Объект, ПолныйПутьНовый,,,Объект.Том);
...
Функция ДоступныеТома(ПараметрыПроверки = Неопределено) Экспорт
	...
	|	Справочник.ТомаХраненияФайлов КАК ТомаХраненияФайлов
	|ГДЕ
	|	ТомаХраненияФайлов.ВидТомаХраненияФайлов = ЗНАЧЕНИЕ(Перечисление.ВидыХраненияФайлов.ОперационноеХранение)
...
//   СпособХраненияФайлов - Строка, Неопределено - доступные значения см. в комментарии константы
//                                                 "СпособХраненияФайлов". Значение по умолчанию - Неопределено.
//
// Возвращаемое значение:
...
//
Функция ТипХраненияФайла(Знач РазмерФайла, Знач РасширениеФайла, Знач СпособХраненияФайлов = Неопределено) Экспорт

	Если СпособХраненияФайлов = Неопределено Тогда
		ТипХранения = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске;
	Иначе
		ТипХранения = РаботаСФайловымАрхивомСервер.ТипХраненияФайлаПоСпособуХранения(СпособХраненияФайлов);
	КонецЕсли;
...
//   ВидТомаХраненияФайлов	- ПеречислениеСсылка.ВидыХраненияФайлов - вид хранения тома. Значение по умолчанию - Неопределено
//   СпособХраненияФайлов	- ПеречислениеСсылка.СпособыХраненияФайлов, Строка
//
// Возвращаемое значение:
//   СправочникСсылка.ТомаХраненияФайлов
//
Функция СвободныйТом(ПрисоединенныйФайл, Знач ВидТомаХраненияФайлов = Неопределено, Знач СпособХраненияФайлов = Неопределено) Экспорт
	Перем Результат;
	...
	РезультатЗапроса = Запрос.Выполнить();
	ОписанияТомов = РезультатЗапроса.Выгрузить();
	Если ЗначениеЗаполнено(ОписанияТомов) Тогда
		ОбъемыТомов = Неопределено;
		
		Для каждого ОписаниеТома Из ОписанияТомов Цикл
			Том = ОписаниеТома.Ссылка;
			Если ОписаниеТома.МаксимальныйРазмер = 0 Тогда
				Результат = Том;
			Иначе
				Если ОбъемыТомов = Неопределено Тогда
					СписокТомов = ОписанияТомов.ВыгрузитьКолонку("Ссылка");
					//@skip-check query-in-loop
					ОбъемыТомов = ОбъемыТомов(СписокТомов);
				КонецЕсли;
				
				МаксимальныйРазмерТомаВБайтах = ОписаниеТома.МаксимальныйРазмер * 1024 * 1024;
				Если ОбъемыТомов[Том] + ПрисоединенныйФайл.Размер <= МаксимальныйРазмерТомаВБайтах Тогда
					Результат = Том;
				КонецЕсли;
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			ШаблонИсключения = НСтр("ru = 'Не удалось добавить файл ""%1.%2"": в томах хранения файлов недостаточно места.
					|Обратитесь к администратору.'");
			
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонИсключения, 
				ПрисоединенныйФайл.Наименование, ПрисоединенныйФайл.Расширение);
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	Иначе
		ШаблонИсключения = НСтр("ru = 'Не удалось добавить файл ""%1.%2"": не настроено ни одного тома для хранения файлов.
				|Обратитесь к администратору.'");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьПутьТомаLinux");
	Методы.Вставить("ЗаполнитьНастройкиХраненияФайлов");
	Методы.Вставить("УстановитьСпособФормированияПутиТома");
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтрольВеденияУчета") Тогда
		Методы.Вставить("ПроверкаСсылокНаНесуществующиеФайлыВТоме");
	КонецЕсли;
