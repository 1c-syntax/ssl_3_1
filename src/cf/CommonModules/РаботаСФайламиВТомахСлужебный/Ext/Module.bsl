...
// Возвращает соответствие томов и суммарных размеров всех файлов в томе в байтах.
//
// Параметры:
//   Тома - Массив из СправочникСсылка.ТомаХраненияФайлов - том, размер которого необходимо посчитать.
//
// Возвращаемое значение:
//   Соответствие - размер томов .
//
Функция ОбъемыТомов(Тома)
	...
	ТекстЗапросаПрисоединенныхФайлов = 
	"ВЫБРАТЬ
	|	Версии.Том КАК Том,
	...
	Для Каждого Тип Из ТипыПрисоединенныхФайлов Цикл
		...
		ТекстЗапросаПрисоединенныхФайлов = ТекстЗапросаПрисоединенныхФайлов + "
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|" + СтрЗаменить(ТекстЗапросаПоСправочнику, "&ИмяСправочника",
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТекстЗапросаПрисоединенныхФайлов", ТекстЗапросаПрисоединенныхФайлов);
	
	Запрос.Параметры.Вставить("Тома", Тома);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Результат.Вставить(Выборка.Том, Выборка.РазмерФайлов);
	КонецЦикла;
...
Функция ПредставлениеСтатусаПроверки(Знач СтатусПроверки) Экспорт
	...
	ИначеЕсли СтатусПроверки = "ЛишнийФайлВТоме" Тогда 
		Возврат НСтр("ru = 'Лишние файлы (есть на диске, но сведения о них отсутствуют)'");
	ИначеЕсли СтатусПроверки = "НетФайлаВТоме" Тогда
		Возврат НСтр("ru = 'Отсутствуют файлы в томе на диске'");
	КонецЕсли;
КонецФункции
...
Функция СвободныйТом(РазмерФайла)
	...
	МассивТомов = Результат.Выгрузить().ВыгрузитьКолонку("Ссылка"); 
	...
	Пока ВыборкаТомов.Следующий() Цикл
		...
		Иначе
			Если ОбъемыТомов = Неопределено Тогда
				...
				ОбъемыТомов = ОбъемыТомов(МассивТомов);
			КонецЕсли;
			ОбъемТома = ОбъемыТомов[ВыборкаТомов.Ссылка];
