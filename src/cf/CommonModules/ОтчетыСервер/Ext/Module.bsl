...
Функция ОтчетОбъект(Идентификатор) Экспорт
	...
	ИначеЕсли Вид = "ВНЕШНИЙОТЧЕТ" Тогда
		Возврат ВнешниеОтчеты.Создать(Имя); // АПК:552 Для внешних отчетов, не подключенных к подсистеме "Дополнительные отчеты и обработки". Не нарушает безопасность информации, так как создается уже подключенный объект.
	Иначе
