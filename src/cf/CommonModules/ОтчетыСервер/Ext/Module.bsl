...
Процедура ОбновитьЭлементыФормыНастроек(Форма, УзелИерархииЭлементов, ПараметрыОбновления = Неопределено) Экспорт 
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда 
		...
		ВосстановитьЗначенияОтборов(Форма);
		Возврат;
	КонецЕсли;
...
Процедура СброситьПользовательскиеНастройки(ДоступныеНастройки, ПараметрыЗагрузки) Экспорт 
	...
	Если Не ПараметрыЗагрузки.СброситьПользовательскиеНастройки Тогда 
		Возврат;
	КонецЕсли;
...
Функция ТребуетсяСброситьПредопределенныеПараметрыВывода(ПараметрыЗагрузки) Экспорт 
	...
	Возврат ПараметрыЗагрузки.ИмяСобытия = ОтчетыКлиентСервер.ИмяСобытияНастройкиПоУмолчанию();
...
Процедура ВосстановитьЗначенияОтборов(Форма) 
	...
	Для Каждого ЭлементКэша Из КэшЗначенийОтборов Цикл 
		...
		ЗначениеСписка = Форма.Элементы.Найти(ИмяСписка + "Значение");
		Если Список.Количество() > 0 И ЗначениеСписка <> Неопределено
			И ЗначениеСписка.ПараметрыВыбора.Количество() > 0 Тогда 
			Продолжить;
		КонецЕсли;
...
Функция СвойстваЭлементаПользовательскихНастроек(КомпоновщикНастроек, ЭлементПользовательскойНастройки, ЭлементНастройки, ОписаниеНастройки)
	...
	Свойства.Тип = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЭлементаНастройки);
...
Функция ОтчетОбъект(Идентификатор) Экспорт
	...
	ИначеЕсли Вид = "ВНЕШНИЙОТЧЕТ" Тогда
		Возврат ВнешниеОтчеты.Создать(Имя); // АПК:552 Для внешних отчетов, не подключенных к подсистеме "Дополнительные отчеты и обработки". Вызов безопасен, так как проверки безопасности для внешнего отчета выполнены ранее при подключении.
	Иначе
...
#Область ОбновлениеЭлементовФормыПользовательскихНастроек
...
Процедура ПередОбновлениемЭлементовФормыНастроек(Форма, ПараметрыОбновления)
	...
	КомпоновщикНастроек = Форма.Отчет.КомпоновщикНастроек;
	Если ПараметрыОбновления.ИмяСобытия = "НастройкиПоУмолчанию" Тогда 
...
#Область УстановкаСвойствЭлементовЗначение
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл 
		...
		Иначе // Поле ввода.
			...
			ОграничениеТипа = РасширенноеОписаниеТипа.ОписаниеТиповДляФормы;
			
			СвойстваПоля = СвойстваЭлементов.Поля.Найти(Элемент.ИндексНастройки, "ИндексНастройки");
			Если СвойстваПоля <> Неопределено И СвойстваПоля.ЭлементНастройки <> Неопределено Тогда
				ЭлементНастройки = СвойстваПоля.ЭлементНастройки;
			...
			Поле.ДоступныеТипы = ОграничениеТипа;
			Поле.ОграничениеТипа = ОграничениеТипа;
...
Процедура ДобавитьКомандыСписка(Форма, ЭлементНастройки, ЭлементыНастроек, ИмяСписка)
	...
	Если ТипЗнч(ЭлементНастройки.ТипЗначения) <> Тип("ОписаниеТипов")
	 Или Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗагрузкаДанныхИзФайла") Тогда 
		Возврат;
	КонецЕсли;
	
	ЕстьСсылочныйТип = Ложь;
	Для Каждого ТекущийТип Из ЭлементНастройки.ТипЗначения.Типы() Цикл
	...
	Если ЕстьСсылочныйТип Тогда
		...
		ИмяКоманды = ИмяСписка + ОтчетыКлиентСервер.ИмяКомандыВставитьИзБуфера();
...
#Область СохранениеСостоянияФормы
...
Процедура УстановитьДоступныеЗначенияНастроек(Форма, Отчет, КомпоновщикНастроек, ЭлементПользовательскойНастройки, 
	ЭлементОсновнойНастройки, ОписаниеНастройки)
