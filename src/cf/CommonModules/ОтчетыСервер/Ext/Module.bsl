...
//        - URL - см. синтакс-помощник: ИсточникДоступныхНастроекКомпоновкиДанных.
//  Отчет - ОтчетОбъект
//        - Неопределено - отчет, чей компоновщик инициализируется.
//  ВариантОтчета - СправочникСсылка.ВариантыОтчета
...
//   * Поля - см. ПоляЭлементовФормыНастроек
//
...
Функция ИдентификаторыСвойствНастроек(ДополнительныеСвойства)
	...
	Если ИдентификаторыСвойств.Количество() = 0 Тогда 
		Возврат ОбщегоНазначения.СкопироватьРекурсивно(ИдентификаторыСвойствПоУмолчанию);
	КонецЕсли;
...
Процедура ДобавитьЭлементПериода(Элементы, Группа, ШаблонИмени, Свойство, ЗаголовокЭлементаНастройки)
	...
	ШаблонЗаголовка = НСтр("ru = '%1 (дата %2)'");
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонЗаголовка,
		ЗаголовокЭлементаНастройки,
		?(СтрЗаканчиваетсяНа(Свойство, "Начала"), НСтр("ru = 'начала'"), НСтр("ru = 'окончания'")));
