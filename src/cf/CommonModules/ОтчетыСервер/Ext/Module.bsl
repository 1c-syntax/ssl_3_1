...
Процедура ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов, ВидыСтилизованныхЭлементов)
	ЭтоОсновнаяФорма = (Форма.ТипФормыОтчета = ТипФормыОтчета.Основная);
	...
	УстановитьСвойстваЭлементовФормыНастроек(Форма, ЭлементыНастроек, СвойстваЭлементов, ЭтоОсновнаяФорма);
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл 
		...
		Если Группа = Неопределено Тогда 
			Группа = ГруппаЭлементовФормыНастроек(Элементы, УзелИерархииЭлементов, ИмяГруппы, ЭтоОсновнаяФорма);
		...
		ПодготовитьЭлементыФормыНастроекКРаспределению(
			ЭлементыНастроекГруппы, СвойстваГруппы.Группировка, ЭтоОсновнаяФорма, ДополнительныеСвойства);
		
		РаспределитьЭлементыФормыНастроек(Форма.Элементы, Группа, ЭлементыНастроекГруппы, ЭтоОсновнаяФорма);
	КонецЦикла;
	...
	ДобавитьОтступКолонок(Элементы, ЭлементыНастроек, УзелИерархииЭлементов, ЭтоОсновнаяФорма);
КонецПроцедуры

Процедура ДобавитьОтступКолонок(Элементы, ЭлементыНастроек, УзелИерархииЭлементов, ЭтоОсновнаяФорма)
	...
	Для НомерКолонки = 2 По КоличествоКолонок Цикл 
		...
		ГруппаПолейКолонки = ГруппаЭлементовФормыНастроек(Элементы, УзелИерархииЭлементов, ИмяГруппы, ЭтоОсновнаяФорма);
...
//    * Поля - ТаблицаЗначений
//
Процедура УстановитьСвойстваЭлементовФормыНастроек(Форма, ЭлементыНастроек, СвойстваЭлементов, ЭтоОсновнаяФорма)
	КомпоновщикНастроек = Форма.Отчет.КомпоновщикНастроек;
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл 
		...
		Если ТипЗнч(Поле) = Тип("ДекорацияФормы") Тогда 
			...
			Если Не ЭтоОсновнаяФорма
				Или Элемент.ЭтоЗначениеКакФлажок Тогда 
		...
		Иначе // Поле ввода.
			...
			Если РасширенноеОписаниеТипа.КоличествоТипов = 1 Тогда 
				Если РасширенноеОписаниеТипа.СодержитТипЧисло Тогда 
					Поле.КнопкаВыбора = (Поле.СписокВыбора.Количество() = 0);
...
Процедура РаспределитьЭлементыФормыНастроек(Элементы, Знач Группа, ЭлементыНастроек, ЭтоОсновнаяФорма)
	КоличествоКолонок = 0;
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл 
		...
		Если Колонка = Неопределено Тогда 
			Колонка = ГруппаЭлементовФормыНастроек(Элементы, Группа, ИмяКолонки, ЭтоОсновнаяФорма);
		...
		Если ТолькоПоляВвода Тогда 
			РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Колонка, ЭлементыНастроек, НомерКолонки, ЭтоОсновнаяФорма);
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Признаки Из ПризнакиЭлементов Цикл 
			...
			Родитель = ИерархияЭлементовФормыНастроек(Элементы, Колонка, Признаки, НомерСтроки, НомерКолонки, ЭтоОсновнаяФорма);
			
			РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Родитель, ЭлементыНастроек, НомерКолонки, ЭтоОсновнаяФорма, Признаки.НомерГруппы);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

Процедура РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Родитель, ЭлементыНастроек, НомерКолонки, ЭтоОсновнаяФорма, НомерГруппы = Неопределено)
	...
	Для Каждого СвойствоНастройки Из СвойстваНастроек Цикл 
		...
		Группа = ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяГруппы, ЭтоОсновнаяФорма);
...
Функция ИерархияЭлементовФормыНастроек(Элементы, Родитель, Признаки, НомерСтроки, НомерКолонки, ЭтоОсновнаяФорма)
	ИмяСтроки = Родитель.Имя + "Строка" + НомерСтроки;
	Строка = ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяСтроки, ЭтоОсновнаяФорма);
	...
	Колонка = ГруппаЭлементовФормыНастроек(Элементы, Строка, ИмяКолонки, ЭтоОсновнаяФорма);
	Колонка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяГруппы, ЭтоОсновнаяФорма = Ложь)
...
Функция ЗаголовокЭлементаПериода(Свойство, ЗаголовокЭлементаНастройки)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		?(СтрЗаканчиваетсяНа(НРег(Свойство), "начала"), НСтр("ru = 'начала'"), НСтр("ru = 'окончания'")));
...
Процедура ДобавитьКомандуВыбораПериода(Форма, Группа, ШаблонИмени, Период, ЭтоСтандартноеПредставление)
	...
	ГруппаКнопки = ГруппаЭлементовФормыНастроек(Форма.Элементы, Группа, ИмяГруппыКнопки);
