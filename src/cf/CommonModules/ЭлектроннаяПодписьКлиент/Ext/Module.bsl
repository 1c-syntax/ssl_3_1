...
//    * ОтборДоверенностей   - Структура - (необязательный) - параметры отбора для поиска подходящей доверенности
//                             * Организация - Строка - (необязательный) - ИНН для отбора доверителя в списке доверенностей.
//                                           - ОпределяемыйТип.Организация - содержит ссылку на организацию доверителя,
//                                              по ИНН которой будет установлен отбор в списке доверенностей.
//                             * Полномочия - Массив из Строка - (необязательный) - коды полномочий для поиска
//                                                               подходящей доверенности.
//                             * КлючОтбора - Строка - (необязательный) - для сохранения выбранной доверенности по
//                                 сертификату в настройках пользователя и автоматической подстановки при последующем 
//                                 подписании в данном сценарии. Например, "ДляКадровогоЭДО". Рекомендуется использовать,
//                                 если не заполнены коды полномочий для отбора.
...
Процедура Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено, ПараметрыПодписи = Неопределено) Экспорт
	...
	СерверныеПараметры.Вставить("ОтборДоверенностей");
...
//   * РазрешитьОтправкуНаПодписание - разрешить отправку на подписание в сервис мобильной подписи.
//   * Отправитель - Неопределено - см. ЭлектроннаяПодписьКлиентЛокализация.ПриОпределенииПараметровОтправителяДокументовНаПодпись
//   * ПакетнаяОтправкаНаПодписание - Неопределено, Булево - если Неопределено, будет возможность выбора в форме.
...
Функция НовыйТипПодписи(ТипПодписи = Неопределено) Экспорт
	...
	Структура.Вставить("РазрешитьОтправкуНаПодписание", Ложь);
	Структура.Вставить("ПакетнаяОтправкаНаПодписание", Неопределено);
...
// Конструктор параметров для добавления подписанта.
// 
// Возвращаемое значение:
//  Структура
//
Функция ПараметрыПодписанта() Экспорт
	...
	ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииПараметровПодписанта(ПараметрыПодписанта);
...
//  ОписаниеДанных - Структура:
//    * Операция             - Строка - заголовок формы подписания данных, например Подписание файла.
//    * ЗаголовокДанных      - Строка - заголовок набора данных, например Файл.
//    * Подписант            - см. ПараметрыПодписанта
//                           - ЛюбаяСсылка - ссылка на справочник подписантов.
//    * ОтборПодписантов - Структура:
//                                    * Ключ - Строка - Имя поля отбора в списке подписантов: "Организация", "ИНН".
//                                    * Значение - ЛюбаяСсылка, Строка - значение отбора.
//    * ОтборДоверенностей   - Структура - (необязательный) - параметры отбора для поиска подходящей доверенности
//                             * Организация - Строка - (необязательный) - ИНН для отбора доверителя в списке доверенностей.
//                                           - ОпределяемыйТип.Организация - содержит ссылку на организацию доверителя,
//                                              по ИНН которой будет установлен отбор в списке доверенностей.
...
//                           ошибка, выполнение прекращается без отображения формы операции или с закрытием этой формы,
//                           если она была открыта.
//    * ПакетнаяОтправка    - Булево - (необязательный) отправлять по одному документу или пакетом. По умолчанию Истина.
...
//                             ** Значение      - ЛюбаяСсылка
//                                              - ОписаниеОповещения - для открытия.
//                             ** Представление - Строка - представление в форме отправки на подписание.
//                             ** ИмяФайла      - Строка - (необязательный) имя файла, отображается в мобильном приложении,
//                              заполнять, если оно отличается от представления. 
//                              Имя файла с расширением длиной не более 250 символов и без дублей
//                              в наборе данных в случае пакетной отправки. Символы, не соответствующие регулярному выражению 
//                               [^\.\(\)\w,\s-] будут заменены на "_", символ № на N.
//    * ПредставлениеНабора - Строка - представления нескольких элементов набора данных, например, "Файлы (%1)".
//                          В это представление в параметр %1 заполняется количество элементов.
//                          По гиперссылке можно открыть список.
//                          Если в наборе данных 1 элемент, тогда используется значение
//                          в свойстве Представление свойства НаборДанных, если не указано, тогда
//                          представление вычисляется по значению свойства Объект элемента набора данных.
//    * СписокПредставлений - Массив - массив со значениями, как у свойства Представление, которые
//                          сможет открыть пользователь.
//
//  Форма - ФормаКлиентскогоПриложения - форма, из которой нужно получить уникальный идентификатор,
//                                который будет использоваться при блокировке объекта.
//        - УникальныйИдентификатор - уникальный идентификатор, который будет
...
//     В результат передается входной параметр ОписаниеДанных, который в успешном случае дополняется свойствами:
//     # Успех - Булево - Истина, если все прошло успешно. Если Успех = Ложь, то частичное завершение
//               определяется по наличию свойства ИдентификаторДокумента. Если есть, то шаг выполнен.
//     # Отказ - Булево - Истина, если пользователь отменил операцию интерактивно.
//     # ПользовательНажалКнопкуПодписать - Булево - если Истина, то пользователь хотя бы раз
//               нажал на кнопку Подписать. Используется для сценариев, когда для продолжения бизнес-процесса
//               достаточно простой подписи (намерения установить подпись), а установка квалифицированной подписи
//               является дополнением, которое можно реализовать позднее, если возникли технические проблемы.
//     # ВыбранныйПодписант - Структура - содержит свойства подписанта.
//     # ВыбраннаяДоверенность - ЛюбаяСсылка
//     # НаборДанных
//      ## ИдентификаторДокумента - УникальныйИдентификатор - идентификатор, который присвоен документу для запроса
//      результатов подписания.
//                       
//  ПараметрыПодписи - см. НовыйТипПодписи
//
Процедура ОтправитьНаПодписание(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено, ПараметрыПодписи = Неопределено) Экспорт
	
	ИмяФормы = "";
	ЭлектроннаяПодписьКлиентЛокализация.ПриОпределенииФормыНаправленияНаПодписание(ИмяФормы);
	
	Если Не ЗначениеЗаполнено(ИмяФормы) Тогда
		ВызватьИсключение НСтр("ru = 'Отправка на подписание не поддерживается в приложении.'");
	КонецЕсли;
	...
	СерверныеПараметры.Вставить("ПоказатьКомментарий", Ложь);
	СерверныеПараметры.Вставить("Подписант");
	СерверныеПараметры.Вставить("ОтборПодписантов");
	СерверныеПараметры.Вставить("ОтборДоверенностей");
	
	ЗаполнитьЗначенияСвойств(СерверныеПараметры, ОписаниеДанных);
	
	СерверныеПараметры.Вставить("ПараметрыПодписи", ПараметрыПодписи);
	
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьНовуюФорму("ОтправитьНаПодписание",
		КлиентскиеПараметры, СерверныеПараметры, ОбработкаЗавершения);
...
Процедура ДобавитьПодписьИзФайла(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	Контекст.Вставить("ПроверитьМенеджерКриптографииНаКлиенте", Ложь);
...
Процедура ПроверитьСертификат(Оповещение, Сертификат, МенеджерКриптографии = Неопределено, НаДату = Неопределено, ПараметрыПроверки = Неопределено) Экспорт
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПроверитьСертификат(Оповещение, Сертификат, МенеджерКриптографии, НаДату, ПараметрыПроверки);
...
Процедура ОткрытьНастройкиЭлектроннойПодписиИШифрования(Страница = "Сертификаты") Экспорт
	...
	ПараметрыФормы.Вставить("ПоказатьСтраницу", Страница);
...
Процедура ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, ФормаВладелец = Неопределено, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, ФормаВладелец, ОповещениеОЗавершении);
...
Функция ПараметрыФормыРасширенногоПредставленияОшибки() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("ЗаголовокПредупреждения", "");
	Структура.Вставить("ИнформацияДляПоддержки", "");
	Структура.Вставить("ТекстОшибкиКлиент", "");
	Структура.Вставить("ТекстОшибкиСервер", "");
	Структура.Вставить("ДополнительныеДанные", Новый Структура);
	Структура.Вставить("ПоказатьТребуетсяПомощь", Ложь);
	Структура.Вставить("ПоказатьПереходКНастройкеПрограмм", Ложь);
	Структура.Вставить("ПоказатьУстановкуРасширения", Ложь);
	Структура.Вставить("ПоказатьИнструкцию", Ложь);
	Структура.Вставить("ТекстДополнительнойСсылки", "");
	Структура.Вставить("ОбработчикДополнительнойСсылки", "");
	Структура.Вставить("ПараметрОбработчикаДополнительнойСсылки");
...
Функция СвойстваСертификата(Сертификат) Экспорт
	...
	Возврат ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСертификата(Сертификат,
		ЭлектроннаяПодписьСлужебныйКлиент.РазницаСУниверсальнымВременем(), Неопределено);
...
//            - Неопределено - использовать параметры по умолчанию.
//
// Возвращаемое значение:
//  Строка
//
...
Функция ПараметрыЗаписиСертификата() Экспорт
	...
	ДополнительныеПараметры.Вставить("ФизическоеЛицо", Неопределено);
...
Функция ПараметрыДобавленияСертификата() Экспорт
	...
	ПараметрыДобавления.Вставить("НаТокене", Ложь);
...
Процедура ДобавитьСертификат(ОбработчикЗавершения = Неопределено, ПараметрыДобавления = Неопределено) Экспорт
	...
	Если ПараметрыИнтерактивногоВыбора.СоздатьЗаявление Тогда
		ПараметрыИнтерактивногоВыбора.Вставить("НаТокене", ПараметрыДобавления.НаТокене);
	КонецЕсли;
...
Процедура ПолучитьОтпечаткиСертификатов(Оповещение, ТолькоЛичные, ПараметрыПолучения = Истина) Экспорт
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПолучитьОтпечаткиСертификатов(Оповещение, ТолькоЛичные, ПараметрыПолучения);
...
Функция ПараметрыПолученияОтпечатковСертификатов(Клиентские = Истина, Серверные = Истина, Сервис = Истина) Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Клиентские", Клиентские);
	Структура.Вставить("Серверные", Серверные);
	Структура.Вставить("Сервис", Сервис);
	Структура.Вставить("ВозвращатьИсточникПолучения", Истина);
	Возврат Структура;
...
//      см. Обработка.ПрограммыЭлектроннойПодписиИШифрования.УстановкаПриложенийДляРаботыСЭлектроннойПодписью,
...
Процедура ОповеститьОбОкончанииСрокаДействияСертификата(Сертификат) Экспорт
	...
	Если ЗначениеЗаполнено(Результат.СсылкаНаСертификат) И Не Результат.Оповещен Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Необходима замена сертификата'"), ДействиеПриНажатии, Сертификат,
			БиблиотекаКартинок.ДиалогВосклицание, СтатусОповещенияПользователя.Важное, Сертификат);
...
Процедура ВвестиОбоснованиеДостоверностиПодписи(ВладелецФормы, Оповещение) Экспорт
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеВводаОбоснованияДостоверностиПодписи", ЭлектроннаяПодписьСлужебныйКлиент, Оповещение);
	ОткрытьФорму("РегистрСведений.ЭлектронныеПодписи.Форма.ОбоснованиеДостоверностиПодписи",,ВладелецФормы,Истина,,,
...
Процедура ОткрытьПодпись(ТекущиеДанные) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.АдресПодписи) Тогда
		
		// СтандартныеПодсистемы.СервисМобильнойПодписи
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи")
			И ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторПодписи) Тогда
			МодульСервисМобильнойПодписиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиКлиент");
			МодульСервисМобильнойПодписиКлиент.ОткрытьПодпись(ТекущиеДанные);
		КонецЕсли;
...
Процедура СохранитьПодпись(АдресПодписи, ИмяФайлаПодписи = "") Экспорт
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьПодпись(АдресПодписи, ИмяФайлаПодписи);
