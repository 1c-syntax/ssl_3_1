...
//                          - Строка
//                          - Структура - (необязательный), если не указан, тогда
...
//             * Расшифровка         - Булево
//                                   - Неопределено - так же, как указано выше.
//             * <Имя дополнительной проверки> - Булево
//                                             - Неопределено - так же, как указано выше.
...
// Формирует структуру свойств для уточнения данных конверта SOAP и алгоритмов
// подписания и хеширования.
//
// Не требуется заполнять алгоритмы при использовании сертификатов
// с алгоритмами открытого ключа ГОСТ 94, ГОСТ 2001, ГОСТ 2012/256 и ГОСТ 2012/512.
// 
// Из сертификата извлекается OIDАлгоритмаОткрытогоКлюча и по нему из переданной таблицы
// определяются идентификаторы и XML-имена алгоритмов подписания и хеширования.
//
// Возвращаемое значение:
//  Структура:
...
//                         Смотри также функцию ДоступностьСозданияЗаявления общего модуля ЭлектроннаяПодпись.
//
// Пример:
