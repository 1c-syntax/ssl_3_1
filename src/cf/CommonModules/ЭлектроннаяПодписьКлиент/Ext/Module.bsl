...
//             * <Имя дополнительной проверки>Ошибка - Строка - заполняется текстом ошибки, если проверка имеет значение
//             Ложь.
//
...
Процедура ОткрытьИнструкциюПоТипичнымПроблемамПриРаботеСПрограммами(ИмяРаздела = "") Экспорт
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://its.1c.ru/db/metod81#content:5784:hdoc"
...
//                             + Символы.ПС + "urn:ietf:params:xml:ns:cpxmlsec:algorithms:gostr34102012-gostr34112012-256" + ...
...
Функция ПараметрыЗаписиСертификата() Экспорт
	...
	ДополнительныеПараметры.Вставить("УсиленнаяЗащитаЗакрытогоКлюча", Ложь);
...
//                                ОписаниеОповещения.
//   Сертификат              - ДвоичныеДанные, Строка - двоичные данные сертификата электронной подписи или
...
//                           - см. ЭлектроннаяПодписьКлиент.ПараметрыЗаписиСертификата.
//
Процедура ЗаписатьСертификатВСправочник(ОбработчикЗавершения,
	...
	Контекст.Вставить("ЗаголовокОшибки", ?(Контекст.ДляШифрования = Истина,
		НСтр("ru = 'Не удалось пройти проверку шифрования по причине:'"),
		НСтр("ru = 'Не удалось пройти проверку подписания по причине:'")));
	Контекст.Вставить("ЗаголовокФормы", ?(Контекст.ДляШифрования = Истина,
		НСтр("ru = 'Проверка шифрования и расшифровки'"),
		НСтр("ru = 'Проверка установки электронной подписи'")));
	...
	Если ОбщиеНастройки().ПроверятьЭлектронныеПодписиНаСервере Тогда
		...
		Контекст.Вставить("ОшибкаНаСервере",
			Новый Структура("ОписаниеОшибки, Ошибки", "", Новый Массив));
...
Процедура ДобавитьСертификат(ОбработчикЗавершения = Неопределено, ПараметрыДобавления = Неопределено) Экспорт
	...
	ПараметрыИнтерактивногоВыбора = Новый Структура("ВЛичныйСписок, Организация");
