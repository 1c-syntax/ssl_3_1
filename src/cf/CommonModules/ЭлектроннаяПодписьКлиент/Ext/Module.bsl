...
//                       
//  ТипПодписи - Перечисление.ТипыПодписиКриптографии - тип подписи, которую требуется создать, если не
//                      указан, используется тип подписи по умолчанию из настроек. Если версия платформы не поддерживает
//                      усовершенствованную подпись, параметр игнорируется, подпись будет создана в формате CAdES. 
//
Процедура Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено, ТипПодписи = Неопределено) Экспорт
	...
	КлиентскиеПараметры.Вставить("ТипПодписи", ТипПодписи);
...
//                           тот же или ниже, тогда никаких действий произведено не будет.
...
//                          ## ТипПодписи - ПеречислениеСсылка.ТипыЭлектроннойПодписи - тип подписи до усовершенствования.
//                          ## СрокДействияПоследнейМеткиВремени - Дата - срок действия до усовершенствования.
//                          ## СвойстваПодписи = см. ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи -
//                          свойства усовершенствованной подписи (только изменившиеся).
//                          ## ТекстОшибки = Строка - текст ошибки при попытке усовершенствования. Если произошла ошибка,
