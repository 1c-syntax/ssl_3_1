...
#Область ФункцииКонструкторыРазрешений
...
#Область ФункцииКонструкторыЗапросовНаИспользованиеВнешнихРесурсов
...
Функция ИспользуютсяПрофилиБезопасности() Экспорт
	Возврат Не ОбщегоНазначения.ИнформационнаяБазаФайловая()
		И ПолучитьФункциональнуюОпцию("ИспользуютсяПрофилиБезопасности");
КонецФункции
...
Функция ПрофильБезопасностиИнформационнойБазы(ПроверкаИспользования = Ложь) Экспорт
	...
	Если ПроверкаИспользования И Не ИспользуютсяПрофилиБезопасности() Тогда
		Возврат "";
	КонецЕсли;
	...
	Если ТипЗнч(ПрофильБезопасности) <> Тип("Строка")
	 Или Не ЗначениеЗаполнено(ПрофильБезопасности) Тогда
		Возврат "";
	КонецЕсли;
