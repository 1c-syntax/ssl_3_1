...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстПредупреждения = НСтр("ru = 'Имеются занятые для редактирования файлы, изменения в которых не помещены в приложение.'");
...
Процедура ПоказатьОшибкуСканирования(Форма, Заголовок, ПодробноеПредставлениеОшибки, РежимТребуетсяПомощь = Ложь) Экспорт
	...
	ПараметрыОткрытияФормы.Вставить("ПоказыватьДиалогСканера", Форма.ПоказыватьДиалогСканера);
	ПараметрыОткрытияФормы.Вставить("РежимТребуетсяПомощь", РежимТребуетсяПомощь);
	ПараметрыОткрытияФормы.Вставить("Разрешение", Форма.РазрешениеПеречисление);
	...
	ОткрытьФорму("Обработка.Сканирование.Форма.ОшибкаСканирования", ПараметрыОткрытияФормы, Форма,,,, 
		ПослеЗакрытияФормыОшибки);
	
КонецПроцедуры

Функция БылаОшибкаСканирования() Экспорт
	
	ПараметрыЗаданияСканирования = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить(
...
#Если Не ВебКлиент Тогда	
	...
	Текст =  НСтр("ru = 'Информация о компьютере:'") + Символы.ПС + СтандартныеПодсистемыКлиент.ИнформацияДляПоддержки();
