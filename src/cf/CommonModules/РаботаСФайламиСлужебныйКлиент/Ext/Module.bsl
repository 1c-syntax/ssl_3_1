...
Процедура ОткрытьФайлСлужебный(Знач ДанныеФайла, Знач ДляРедактирования = Ложь) Экспорт
	
	Если ДляРедактирования Тогда
		РедактироватьФайл(Неопределено, ДанныеФайла);
	Иначе
		ОткрытьФайлСОповещением(Неопределено, ДанныеФайла, , ДляРедактирования); 
	КонецЕсли;
	
КонецПроцедуры

Процедура СохранитьФайлКакСлужебный(Знач ДанныеФайла, ОбработчикЗавершения = Неопределено) Экспорт
	
	Оповещение = Новый ОписаниеОповещения("СохранитьФайлКакПослеСохранения",
		РаботаСФайламиСлужебныйКлиент, ОбработчикЗавершения);
	
	СохранитьКак(Оповещение, ДанныеФайла, Неопределено);
	
КонецПроцедуры	

Процедура ДействиеСФайлом(ПараметрыДействияСФайлом) Экспорт
	
	СтандартнаяОбработка = Истина;
	РаботаСФайламиКлиентПереопределяемый.ПриВыполненииДействияСФайлом(ПараметрыДействияСФайлом, СтандартнаяОбработка);

	Если НЕ СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
	
	ПрисоединенныйФайл	= ПараметрыДействияСФайлом.ПрисоединенныйФайл;
	ВерсияФайла			= ПараметрыДействияСФайлом.ВерсияСсылка;
	ФормаВладелец		= ПараметрыДействияСФайлом.ФормаВладелец;
	ИмяМетода			= ПараметрыДействияСФайлом.ИмяМетодаПолученияФайла;	

	Если ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный"
			Или ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный" Тогда
		ИмяПроцедурыОповещения = "ЗавершитьДействиеСФайлом";
	ИначеЕсли ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный" Тогда
		ИмяПроцедурыОповещения	= "ЗавершитьДействиеСФайломУправлениеЭлектроннойПочтой";
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Используется непредусмотренный метод получения файла: %1'"), ИмяМетода));
	КонецЕсли;

	ПараметрыДлительнойОперации = ПараметрыДлительнойОперации(ПараметрыДействияСФайлом);
	
	ДлительнаяОперация = РаботаСФайламиСлужебныйВызовСервера.НачатьДействиеСФайлом(ПараметрыДлительнойОперации, ИмяМетода);

	ОписаниеОповещения = Новый ОписаниеОповещения(ИмяПроцедурыОповещения, РаботаСФайламиСлужебныйКлиент, ПараметрыДействияСФайлом.ПараметрыДействия);
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ФормаВладелец);
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Получение файла'");
...
//
Процедура ПоказатьПредупреждениеОНеобходимостиРасширения1СПредприятия(ОбработчикРезультата, ПредставлениеКоманды = "") Экспорт
	Если Не КлиентПоддерживаетСинхронныеВызовы() Тогда
		ТекстПредупреждения = НСтр("ru = 'Действие ""%1"" доступно только в клиентском приложении.'");
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Для выполнения команды ""%1"" установите расширение для работы с 1С:Предприятием.'");
	КонецЕсли;
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Путь = ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекЗагрузить("ИмяПапкиВыгрузки", "ИмяПапкиВыгрузки");
...
//
//  УдалятьНекорректныеСимволы - Булево - если указать Истина, то удаляет некорректные символы
//                                        из проверяемого имени файла.
//
Процедура КорректноеИмяФайла(ИмяФайла, УдалятьНекорректныеСимволы = Ложь) Экспорт
	
	НедопустимыеСимволы = ОбщегоНазначенияКлиентСервер.НайтиНедопустимыеСимволыВИмениФайла(ИмяФайла);
	Результат = НедопустимыеСимволы.Количество() = 0;
	...
	Если Не Результат Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ОбщегоНазначенияКлиентСервер.ПолучитьНедопустимыеСимволыВИмениФайла());
	КонецЕсли;
...
Функция ВыбратьПутьККаталогуДанныхПользователя() Экспорт
	...
	ИмяКаталога = "";
	Если Расширение1СПредприятияПодключено() Тогда
...
//
Функция Расширение1СПредприятияПодключено() Экспорт
	Возврат ?(КлиентПоддерживаетСинхронныеВызовы(), ПодключитьРасширениеРаботыСФайлами(), Ложь);
КонецФункции

Процедура ПоказатьВопросОбУстановкеРасширения1СПредприятия(ОписаниеОповещения) Экспорт
	Если Не КлиентПоддерживаетСинхронныеВызовы() Тогда
...
Процедура ОтправитьФайлыПоПочте(МассивФайлов, ИдентификаторФормы, ПараметрыОтправки, ЭтоФайл = Ложь) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтправитьФайлыПоПочтеНастройкаУчетнойЗаписиПредложена", 
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтеграцияС1СДокументооборотом") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБИД, "3.0.2.4") >= 0 Тогда
			...
			Если Форма = Неопределено Тогда
				Форма = Новый Структура("ИспользоватьПрисоединенныеФайлы1СДокументооборота", Истина);
			КонецЕсли;
...
Процедура ОткрытьФормуПрисоединенныеФайлы1СДокументооборот(ВладелецПрисоединенныхФайлов, Форма = Неопределено,
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБИД, "3.0.2.7") >= 0 Тогда
		Параметры = МодульИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыОткрытияСпискаФайлов(Форма);
		Если ПараметрыВыполненияКоманды <> Неопределено Тогда
			Параметры.Уникальность = ПараметрыВыполненияКоманды.Уникальность;
			Параметры.Окно = ПараметрыВыполненияКоманды.Окно;
		КонецЕсли;
...
#Область ПроцедурыИФункцииРаботыСКриптографией
...
Процедура ПроверитьПодписи(Форма, СсылкаНаДвоичныеДанные, ВыделенныеСтроки = Неопределено, ДанныеФайла = Неопределено) Экспорт
	...
	ОписаниеДанных.Вставить("Объект",                ДанныеФайла.Ссылка);
...
Процедура ПроверитьПодписиПослеПроверкиСтроки(РезультатПроверкиПодписи, ДополнительныеПараметры) Экспорт
	...
	РезультатПроверки = ДанныеСтроки.РезультатПроверки;
	...
	ЗаполнитьЗначенияСвойств(РезультатПроверки, РезультатПроверкиПодписи);
	РезультатПроверки.ДополнительныеАтрибутыПровереныВручную = Ложь;
	РезультатПроверки.АвторРучнойПроверкиДополнительныхАтрибутов = Неопределено;
	РезультатПроверки.ОбоснованиеРучнойПроверкиДополнительныхАтрибутов = "";
	ДанныеСтроки.РезультатПроверки = РезультатПроверки;
	
	Если СтрокаПодписи.Свойство("ОписаниеОшибки") Тогда
	...
	Иначе
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			МодульЭлектроннаяПодписьКлиентСервер.ЗаполнитьСтатусПодписи(ДанныеСтроки, ОбщегоНазначенияКлиент.ДатаСеанса());
		КонецЕсли;
	КонецЕсли;
...
#Область ОбщиеПроцедурыИФункцииРаботыСФайламиОперационнойСистемы
...
Процедура ОткрытьВерсиюФайла(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ПоказатьВопросОбУстановкеРасширения1СПредприятия(Обработчик);
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстПредупреждения = НСтр("ru = 'Рекомендуется посмотреть занятые для редактирования файлы, изменения которых не помещены в приложение.'");
...
Процедура ОбработкаВыбораТабличногоДокумента(ФормаОтчета, Элемент, Область, СтандартнаяОбработка) Экспорт
	...
	Если ФормаОтчета.НастройкиОтчета.ПолноеИмя <> "Отчет.ПроверкаЦелостностиТома" Тогда
		Возврат;
	КонецЕсли;
	
	Если Область.Расшифровка = "ПроверкаЦелостностиТома.ВосстановитьФайлы" Тогда
		СтандартнаяОбработка = Ложь;
		Том = ФормаОтчета.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Том").Значение;
		Задание = РаботаСФайламиСлужебныйВызовСервера.ЗапуститьВосстановлениеФайлов(Том, ФормаОтчета.УникальныйИдентификатор);

		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ФормаОтчета",  ФормаОтчета);
		
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеВосстановленияФайлов", ЭтотОбъект, ДополнительныеПараметры);

		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ФормаОтчета);
		ПараметрыОжидания.Заголовок = НСтр("ru='Выполняется восстановление сведений о файлах'");
	...
	Если Файл.Существует() Тогда
		...
		Исключение
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Работа с файлами.Ошибка установки времени изменения'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Функция РасширениеДляЗашифрованныхФайлов()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		МодульЭлектроннаяПодписьКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектроннаяПодписьКлиент");
		Возврат МодульЭлектроннаяПодписьКлиент.ПерсональныеНастройки().РасширениеДляЗашифрованныхФайлов;
	Иначе
		Возврат "p7m";
	КонецЕсли;
...
Процедура ПроверитьПодписиЦиклНачало(ДополнительныеПараметры)
	...
	Если Не ЗначениеЗаполнено(ДополнительныеПараметры.СтрокаПодписи.АдресПодписи) 
		И ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		
		МодульСервисМобильнойПодписиСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисМобильнойПодписиСлужебныйКлиент");
		МодульСервисМобильнойПодписиСлужебныйКлиент.ПроверитьОжидаемуюПодпись(
			Новый ОписаниеОповещения("ПроверитьПодписиПослеПроверкиСтроки", ЭтотОбъект, ДополнительныеПараметры),
			ДополнительныеПараметры.Данные, ДополнительныеПараметры.СтрокаПодписи.ИдентификаторПодписи, ПараметрыПроверкиПодписи);
		Возврат;
...
Процедура ДействиеПриОткрытииФайлаВРабочемКаталоге(ОбработчикРезультата, ИмяФайлаСПутем, ДанныеФайла)
		...
		Если Параметры.РазмерВХранилищеФайлов <> 0
		   И Параметры.РазмерВХранилищеФайлов <> Параметры.РазмерВРабочемКаталоге Тогда
			...
			Параметры.Вставить("Сообщение",
				НСтр("ru = 'Размер файла в рабочем каталоге на компьютере и его копии в приложении отличается.
				           |
				           |В некоторых случаях размер файла меняется, хотя его содержимое не затрагивается.
				           |Например, после повторного сохранения или обновлении служебной информации.
				           |
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			Параметры.Вставить("Заголовок", НСтр("ru = 'В приложении более новая версия файла'"));
			Параметры.Вставить("Сообщение",
				НСтр("ru = 'Файл в приложении, отмеченный как занятый для редактирования,
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Иначе
			...
			Параметры.Вставить(
				"Сообщение",
				НСтр("ru = 'Копия файла в рабочем каталоге на компьютере имеет более позднюю дату изменения (новее), чем в приложении. Возможно, эта копия была отредактирована.
				           |
...
Процедура ВывестиНапоминаниеПриРедактировании(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если НЕ Расширение1СПредприятияПодключено() Тогда
			...
			ПараметрыНапоминания.Вставить("Картинка", БиблиотекаКартинок.ДиалогИнформация);
...
Функция ДанныеСтрокиПодписи()
	...
	МодульЭлектроннаяПодписьКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектроннаяПодписьКлиентСервер");
	Возврат МодульЭлектроннаяПодписьКлиентСервер.РезультатПроверкиПодписиВФорме();
...
Процедура ПоместитьВыбранныеФайлыВХранилище(Знач ВыбранныеФайлы,
	...
	Для Каждого ПолноеИмяФайла Из ВыбранныеФайлы Цикл
		...
		Если НЕ ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, ИдентификаторФормы) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось поместить в приложение файл ""%1"".'"),
					Файл.ПолноеИмя) );
			Продолжить;
		КонецЕсли;
		...
		ИмяИРасширениеФайла = ИмяИРасширениеФайла(Файл);
		РасширениеФайла = ИмяИРасширениеФайла.РасширениеФайла;
		ИмяФайлаБезРасширения = ИмяИРасширениеФайла.ИмяФайлаБезРасширения;
...
Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы,
	...
	РазмерВМб = РаботаСФайламиСлужебныйКлиентСервер.ПредставлениеРазмераФайла(Файл.Размер() / (1024 * 1024));
	...
	Иначе
		Если ТекущаяПозиция = Неопределено Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				СохраняемоеИмяФайла,
				РазмерВМб);
			ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение файла'"), , ТекстПояснения, БиблиотекаКартинок.ДиалогИнформация);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	РасширениеПодключено = Расширение1СПредприятияПодключено();
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Файл освобожден'"),
		ДанныеФайла.НавигационнаяСсылка, ДанныеФайла.ПолноеНаименованиеВерсии, БиблиотекаКартинок.ДиалогИнформация);
...
Функция ОшибкаСозданияНовогоФайла(ИнформацияОбОшибке)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Невозможно добавить файл в приложение.
		           |%1'"),
...
#Область ЗавершениеРедактированияФайлаИПомещениеЕгоНаСервер
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
		...
		Если ПараметрыВыполнения.ПрименитьКоВсем = Ложь Тогда
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось поместить в приложение файл
					           |""%1"" (%2)
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Если ДанныеФайла.ХранитьВерсии Тогда
			...
			ОткрытьФорму("Обработка.РаботаСФайлами.Форма.СохранениеВерсииФайла", ВозвратФайла, , , , , Обработчик);
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(ПараметрыВыполнения)
	...
	Иначе
		...
		Исключение
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось поместить файл с компьютера в приложение по причине:
				|""%1"".
				|
				|Повторить операцию?'"),
				ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Попытка
		ВерсияОбновлена = РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияИОсвободитьФайл(ПараметрыВыполнения.ДанныеФайла, 
			СведенияОФайле, НеМенятьЗаписьВРабочемКаталоге, ПараметрыВыполнения.ПолныйПутьКФайлу, 
			РабочийКаталогПользователя(), ПараметрыВыполнения.ИдентификаторФормы);
		Если ЭтоАдресВременногоХранилища(СведенияОФайле.АдресВременногоХранилищаТекста) Тогда
...
Процедура ЗакончитьРедактированиеБезРасширения(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Если ПараметрыВыполнения.ХранитьВерсии Тогда
			ПараметрыВыполнения.СоздатьНовуюВерсию = Истина;
			СоздатьНовуюВерсиюДоступность = ПараметрыВыполнения.АвторТекущейВерсии = ПараметрыВыполнения.Редактирует;
		Иначе
			ПараметрыВыполнения.СоздатьНовуюВерсию = Ложь;
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.СохранениеВерсииФайла", СтруктураПараметров, , , , , Обработчик);
...
#Область ВыборФайлаИСозданиеИзНегоНовойВерсии
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы,
	...
	Иначе
		ПутьВыбора = ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекЗагрузить("НастройкиПрограммы", "ПапкаДляОбновитьИзФайла");
	КонецЕсли;
	...
	Если ПустаяСтрока(ДанныеФайла.РабочийКаталогВладельца) Тогда
		Если ПутьВыбораПрежний <> ПутьВыбора Тогда
			ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить("НастройкиПрограммы", "ПапкаДляОбновитьИзФайла",  ПутьВыбора);
		КонецЕсли;
	КонецЕсли;
...
#Область ПометкаФайлаКакЗанятогоДляРедактирования
...
#Область ПометкаФайловКакЗанятыхДляРедактирования
...
#Область ОткрытиеФайлаДляРедактированияПоСсылке
...
Процедура РедактироватьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.Вставить("ДанныеФайла", Неопределено);
	ПараметрыВыполнения.Вставить("РасширениеПодключено", Расширение1СПредприятияПодключено());
...
#Область ОткрытиеФайлаДляРедактирования
...
Процедура ОткрытьВерсиюФайлаПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если Расширение1СПредприятияПодключено() Тогда
		...
		ПолучитьФайлВерсииВРабочийКаталог(
			...
			ПараметрыВыполнения.УникальныйИдентификатор);
	Иначе
		Если ПараметрыВыполнения.ДанныеФайла.Свойство("СсылкаНаДвоичныеДанныеФайла") 
				И ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.СсылкаНаДвоичныеДанныеФайла) Тогда
			Адрес = ПараметрыВыполнения.ДанныеФайла.СсылкаНаДвоичныеДанныеФайла;
		Иначе
...
#Область ОсвобождениеФайловБезОбновления
...
#Область ОсвобождениеФайлаБезОбновления
...
Процедура СохранитьИзмененияФайлаБезРасширенияПослеШифрованияФайла(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Результат = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИСохранитьИзмененияФайла(ПараметрыВыполнения.ОбъектСсылка, 
		СведенияОФайле, "", ПараметрыВыполнения.ПолныйПутьКФайлу, Ложь, ПараметрыВыполнения.ИдентификаторФормы);
	ПараметрыВыполнения.ДанныеФайла = Результат.ДанныеФайла;
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Новая версия сохранена'"),
...
#Область ДействиеСФайлом

// Обработчик оповещения, вызываемый из РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом для выполнения операций,
//  завершающих действие с файлом.
//	Вызывается для методов РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный и РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный.
//
// Параметры:
//  РезультатВыполнения		- Структура - см. ДлительныеОперацииКлиент.НовыйРезультатДлительнойОперации()
//  ДополнительныеПараметры	- Произвольный - произвольные данные, переданные в описании оповещения.
//
Процедура ЗавершитьДействиеСФайлом(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	ДанныеФайла = ПолучитьИзВременногоХранилища(РезультатВыполнения.АдресРезультата);

	АктуализироватьСпискиПрисоединенныхФайлов(ДанныеФайла);

	Если ДополнительныеПараметры.ВидДействия = "ОткрытьФайл" Тогда
	
		Если ДополнительныеПараметры.ОповещатьЕслиЗашифрован И ДанныеФайла.Зашифрован Тогда
			// Файл может быть изменен в другом сеансе.
			ОповеститьОбИзменении(ДанныеФайла.Ссылка);
			Возврат;		
		КонецЕсли;

		ОткрытьФайлСлужебный(ДанныеФайла, ДополнительныеПараметры.ФайлРедактируется);
		
	ИначеЕсли ДополнительныеПараметры.ВидДействия = "ОткрытьВерсиюФайла" Тогда

		ОткрытьВерсиюФайла(Неопределено, ДанныеФайла, ДополнительныеПараметры.УникальныйИдентификатор);

	ИначеЕсли ДополнительныеПараметры.ВидДействия = "ОткрытьФайлСОповещением" Тогда

		ОткрытьФайлСОповещением(Неопределено, ДанныеФайла);

	ИначеЕсли ДополнительныеПараметры.ВидДействия = "ВыбратьРежимИРедактироватьФайл" Тогда

		ПараметрыОбработчика = Новый Структура;
		ПараметрыОбработчика.Вставить("ДанныеФайла", ДанныеФайла);
		Обработчик = Новый ОписаниеОповещения("СписокВыборПослеВыбораРежимаРедактирования", ДополнительныеПараметры.ФормаВладелец, ПараметрыОбработчика);
		
		ВыбратьРежимИРедактироватьФайл(Обработчик, ДанныеФайла, ДополнительныеПараметры.ДоступностьКомандыРедактировать);
		
	ИначеЕсли ДополнительныеПараметры.ВидДействия = "СохранитьКак" Тогда

		СохранитьКак(Неопределено, ДанныеФайла, ДополнительныеПараметры.УникальныйИдентификатор);
...
// Обработчик оповещения, вызываемый из РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом для выполнения операций,
//  завершающих действие с файлом.
//	Вызывается для метода РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный.
//
// Параметры:
//  РезультатВыполнения		- Структура - см. ДлительныеОперацииКлиент.НовыйРезультатДлительнойОперации()
//  ДополнительныеПараметры	- Произвольный - произвольные данные, переданные в описании оповещения.
//
Процедура ЗавершитьДействиеСФайломУправлениеЭлектроннойПочтой(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ВидДействия = "ОткрытьФайл" Тогда
		...
		ОткрытьФайлСлужебный(ДанныеФайла, ДополнительныеПараметры.ДляРедактирования);

	ИначеЕсли ДополнительныеПараметры.ВидДействия = "СохранитьКак" Тогда

		СохранитьФайлКакСлужебный(ДанныеФайла);

	КонецЕсли;

КонецПроцедуры

Функция ПараметрыДлительнойОперации(ПараметрыДействияСФайлом)

	ПрисоединенныйФайл		= ПараметрыДействияСФайлом.ПрисоединенныйФайл;
	ВерсияФайла				= ПараметрыДействияСФайлом.ВерсияСсылка;
	ИдентификаторВладельца	= ПараметрыДействияСФайлом.ФормаВладелец.УникальныйИдентификатор;	
	ИмяМетода = ПараметрыДействияСФайлом.ИмяМетодаПолученияФайла;
	...
	Если ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный"
		...
		Результат.Вставить("ПараметрыМетода", ПараметрыДействияСФайлом.ПараметрыМетода);

		ПараметрыМетода = Результат.ПараметрыМетода;
		ПараметрыМетода.ПрисоединенныйФайл			= ?(ЗначениеЗаполнено(ПрисоединенныйФайл), ПрисоединенныйФайл, Неопределено);
		ПараметрыМетода.ВерсияФайла					= ?(ЗначениеЗаполнено(ВерсияФайла), ВерсияФайла, Неопределено);
		ПараметрыМетода.ИдентификаторФормы			= ИдентификаторВладельца;
		ПараметрыМетода.ПараметрыПолученияФайла		= РаботаСФайламиКлиентСервер.ПараметрыПолученияФайла();

		ПараметрыПолученияФайла = ПараметрыМетода.ПараметрыПолученияФайла;
		
		ПараметрыПолученияФайла.АдресВРодительскомСеансе				= ПоместитьВоВременноеХранилище(Неопределено, ИдентификаторВладельца);
		ПараметрыПолученияФайла.АдресТекущейВерсииВРодительскомСеансе	= ПоместитьВоВременноеХранилище(Неопределено, ИдентификаторВладельца);
		ЗаполнитьЗначенияСвойств(ПараметрыПолученияФайла, ПараметрыДействияСФайлом);
	...
	ИначеЕсли ИмяМетода = "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный" Тогда
		...
		ПараметрыМетода = Результат.ПараметрыМетода;
		ПараметрыМетода.ПрисоединенныйФайл		= ПрисоединенныйФайл;
		ПараметрыМетода.ИдентификаторФормы		= ИдентификаторВладельца;
		ПараметрыМетода.ПараметрыПолученияФайла = РаботаСФайламиКлиентСервер.ПараметрыПолученияФайла();
		
		ПараметрыДействия = ПараметрыДействияСФайлом.ПараметрыДействия;
		Если ПараметрыДействия.ВидДействия = "ОткрытьФайл" Тогда
			ПараметрыМетода.ДляРедактирования = ПараметрыДействия.ДляРедактирования;
		КонецЕсли;
...
#Область УдалениеФайлаПередУдалениемСнимаетсяАтрибутТолькоДляЧтения
...
#Область ПолучениеФайлаИзХранилищаВРабочийКаталог
...
Процедура ПолучитьФайлВерсииВЛокальныйКэшФайлов(ОбработчикРезультата, ДанныеФайла, НаЧтение,
	...
	Если ПараметрыВыполнения.ПолноеИмяФайла = "" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не удалось получить файл из приложения в рабочий каталог на компьютере.'"));
...
#Область ПолучениеФайлаИзПрограммыНаКомпьютер
...
Процедура ПолучитьФайлВерсииВРабочийКаталог(ОбработчикРезультата, ДанныеФайла, ПолноеИмяФайла,
	...
	Иначе
		...
		ПолучитьФайлВерсииВРабочийКаталогПапки(ОбработчикРезультата, ДанныеФайла, ПолноеИмяФайла, 
			ДанныеФайла.НаЧтение, ИдентификаторФормы, ДополнительныеПараметры);
	КонецЕсли;
...
Функция РабочийКаталогДоступен(РабочийКаталогВладельца, Владелец)
	...
	Попытка
		...
		Если НЕ СведенияОКаталоге.Существует() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Каталог папки файлов %1 не существует.'"), 
				Владелец);
		КонецЕсли;
	...
	Исключение
		// Нет прав на создание каталога, или такой путь отсутствует,
		// тогда установка настроек по умолчанию.
		ТекстСообщения = НСтр("ru='Рабочий каталог %1 для папки файлов %2 не существует или нет права на запись. Восстановлены настройки по умолчанию.
			|Технические подробности:
			|%3'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, 
			РабочийКаталогВладельца, Владелец, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru='Работа с файлами'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Предупреждение", ТекстСообщения, ОбщегоНазначенияКлиент.ДатаСеанса(), Истина);
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Контекст.ДанныеФайла) <> Тип("Структура")
		...
		ДанныеФайлаДополнительныеПараметры = РаботаСФайламиКлиентСервер.ПараметрыДанныхФайла();
		ДанныеФайлаДополнительныеПараметры.ИдентификаторФормы	= Контекст.ИдентификаторФормы;
		ДанныеФайлаДополнительныеПараметры.ДляРедактирования	= Контекст.ДляРедактирования;
		
		Контекст.Вставить("ДанныеФайла", РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайла(
			Контекст.ПрисоединенныйФайл, ДанныеФайлаДополнительныеПараметры));
	КонецЕсли;
	
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru = 'Не удалось получить файл на компьютер из приложения по причине:'") + Символы.ПС);
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Контекст.ДанныеФайла) <> Тип("Структура") Тогда
		...
		ДанныеФайлаДополнительныеПараметры = РаботаСФайламиКлиентСервер.ПараметрыДанныхФайла();
		ДанныеФайлаДополнительныеПараметры.ИдентификаторФормы				= Контекст.ИдентификаторФормы;
		ДанныеФайлаДополнительныеПараметры.ПолучатьСсылкуНаДвоичныеДанные	= Ложь;
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru = 'Не удалось поместить файл с компьютера в приложение по причине:'") + Символы.ПС);
...
#Если ВебКлиент Тогда
	Если НЕ Расширение1СПредприятияПодключено() Тогда
		ПоказатьПредупреждениеОНеобходимостиРасширения1СПредприятия(ОбработчикРезультата);
		Возврат;
	КонецЕсли;
#КонецЕсли
...
#Область УдалениеФайлаСКомпьютераИИзРегистраСведений
...
#Область ОчисткаРабочегоКаталога
...
#Область ПолучениеФайлаССервераИРегистрацияВЛокальномКэше
...
Процедура ПолучитьФайлВерсииВРабочийКаталогПапкиПродолжение(ПараметрыВыполнения)
	...
	СведенияОФайле = РаботаСФайламиСлужебныйВызовСервера.СведенияОФайлахВРабочемКаталоге(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыВыполнения.ПолноеИмяФайла));
	СведенияОФайле = СведенияОФайле[ПараметрыВыполнения.ПолноеИмяФайла];
	ПараметрыВыполнения.Вставить("ФайлЕстьВРегистре", СведенияОФайле.ФайлЕстьВРегистре);
	Версия            = СведенияОФайле.Файл;
	Владелец          = СведенияОФайле.Владелец;
	ВРегистреНаЧтение = СведенияОФайле.ВРегистреНаЧтение;
	ВРегистреПапка    = СведенияОФайле.ВРегистреПапка;
...
#Область ПолучениеФайлаССервераИРегистрацияВРабочемКаталоге
...
#Область ПроверкаМаксимальнойДлиныРабочегоКаталогаСЗаменойИПереносомФайлов
...
#Область КопированиеСодержимогоИзОдногоКаталогаВДругой
...
#Область ПереносСодержимогоРабочегоКаталогаВНовый
...
#Область УдалениеМассиваПутейПереданныхПапокИФайлов
...
#Область ИмпортФайловСПроверкойРазмера
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.УдалитьФайлыПослеЗагрузки = Истина Тогда
...
Процедура УдалитьФайлыПослеДобавления(МассивСтруктурВсехФайлов, МассивВсехПапок)
	...
	Для Каждого Элемент Из МассивСтруктурВсехФайлов Цикл
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось удалить файл после добавления в приложение:
					|""%1"". 
					|Возможно он занят другим приложением.
					|
					|%2'"),
...
#Область СохранениеФайлаНаКомпьютер
...
//   УникальныйИдентификатор - УникальныйИдентификатор - идентификатор формы..
//
Процедура СохранитьКак(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор)
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Если ПутьВыбораПрежний <> ПараметрыВыполнения.ПутьВыбора Тогда
		ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить("НастройкиПрограммы", "ПапкаДляСохранитьКак", ПараметрыВыполнения.ПутьВыбора);
	КонецЕсли;
...
#Область ПоказываетНапоминаниеПередПомещениемФайлаЕслиОноНастроено
...
#Область ПоказываетИнформациюОТомЧтоФайлНеБылИзменен
...
#Область ЗагружаетОтредактированныйФайлВПрограммуСнимаетБлокировкуИРассылаетОповещение
...
#Область ЗахватываетФайлОткрываетДиалогРедактированияИРассылаетОповещение
...
#Область ЗахватываетФайлИлиНесколькоФайловИРассылаетОповещение
...
#Область СнимаетБлокировкуФайлаИРассылаетОповещение
...
#Область ОткрываетФайлИРассылаетОповещение
...
Процедура ОткрытьФайлСОповещением(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено, ДляРедактирования = Истина) Экспорт
	...
	Если НЕ ПараметрыВыполнения.ДанныеФайла.Свойство("Владелец") Или Не ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Владелец) Тогда
		...
		ПоказатьВопросОбУстановкеРасширения1СПредприятия(ОписаниеОповещения);
		Возврат;
	КонецЕсли;
...
#Область ЗагружаетФайлВПрограммуИРассылаетОповещение
...
#Область ПоказываетДиалогВыбораФайлаЗагружаетВыбранныйФайлВПрограммуКакВерсиюИРассылаетОповещение
...
#Область ПроцедурыИФункцииДляРаботыСКриптографиейИзФормыФайла
...
Процедура ПрочитатьСертификатыПодписейЦиклНачало(Контекст)
	...
	Если Не ЗначениеЗаполнено(Контекст.СтрокаТаблицы.АдресПодписи) Тогда
		ПрочитатьСертификатыПодписейЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьФайлНаПодписание(ПрисоединенныйФайл, ДанныеФайла, ИдентификаторФормы,
	...
	ОбработчикПродолжения = Новый ОписаниеОповещения("ПослеОтправкиНаПодписание", ЭтотОбъект, ПараметрыВыполнения);
	...
	Если ПодписьМассиваФайлов  Тогда
		...
		Если ПрисоединенныйФайл.Количество() > 1 Тогда
			ОписаниеДанных.Вставить("Операция",            НСтр("ru = 'Отправка файлов на подписание'"));
			ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ru = 'Файлы'"));
			ОписаниеДанных.Вставить("ПредставлениеНабора", "Файлы (%1)");
		Иначе
			ОписаниеДанных.Вставить("Операция",        НСтр("ru = 'Отправка файла на подписание'"));
			ОписаниеДанных.Вставить("ЗаголовокДанных", НСтр("ru = 'Файл'"));
		КонецЕсли;
		...
		Для Каждого Файл Из ПрисоединенныйФайл Цикл
			...
			Представление.Вставить("Значение", Файл);
			Представление.Вставить("ИмяФайла", ПараметрыВыполнения.ДанныеФайла[ИндексФайла].ИмяФайла);
			Представление.Вставить("Представление", Представление.ИмяФайла);
			
			ОписаниеДанныхФайла.Вставить("Представление", Представление);
	...
	Иначе
		...
		Представление = Новый Структура;
		Представление.Вставить("Значение", ПрисоединенныйФайл);
		Представление.Вставить("ИмяФайла", ПараметрыВыполнения.ДанныеФайла.ИмяФайла);
		Представление.Вставить("Представление", Представление.ИмяФайла);

		ОписаниеДанныхФайла.Вставить("Представление", Представление);
		ОписаниеДанныхФайла.Вставить("Данные", ПараметрыВыполнения.ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
		Если ОбработчикПриПолученииПодписи = Неопределено Тогда
			ОписаниеДанныхФайла.Вставить("Объект", ПрисоединенныйФайл);
		Иначе
			ОписаниеДанныхФайла.Вставить("Объект", ОбработчикПриПолученииПодписи);
		КонецЕсли;
	...
	КонецЕсли;
	МодульЭлектроннаяПодписьКлиент.ОтправитьНаПодписание(ОписаниеДанных, ИдентификаторФормы, ОбработчикПродолжения, ПараметрыПодписания);
	
КонецПроцедуры

// Завершение процедуры ОтправитьФайлНаПодписание.
Процедура ПослеОтправкиНаПодписание(ОписаниеДанных, ПараметрыВыполнения) Экспорт
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ДанныеФайла, ИдентификаторФормы,
	...
	Если ПодписьМассиваФайлов И ПрисоединенныйФайл.Количество() > 1 Тогда
		...
		Для Каждого Файл Из ПрисоединенныйФайл Цикл
			...
			ОписаниеДанныхФайла = Новый Структура;
			ОписаниеДанныхФайла.Вставить("Представление", ПараметрыВыполнения.ДанныеФайла[ИндексФайла].ИмяФайла);
			ОписаниеДанныхФайла.Вставить("Данные", ПараметрыВыполнения.ДанныеФайла[ИндексФайла].СсылкаНаДвоичныеДанныеФайла);
	...
	Иначе
		...
		Если ПодписьМассиваФайлов Тогда
			ОписаниеДанных.Вставить("Представление", ПараметрыВыполнения.ДанныеФайла[0].ИмяФайла);
			...
			КонецЕсли;
		Иначе
			ОписаниеДанных.Вставить("Представление", ПараметрыВыполнения.ДанныеФайла.ИмяФайла);
	...
	Если ТипЗнч(ДанныеФайла) = Тип("Массив") Тогда
		...
		Для Каждого Файл Из ДанныеФайла Цикл
			Если ЗначениеЗаполнено(Файл.Редактирует) Тогда
				Предупреждения.Добавить(РаботаСФайламиСлужебныйКлиентСервер.СообщениеОНедопустимостиПодписанияЗанятогоФайла(Файл.Ссылка));
				Продолжить;
			КонецЕсли;
			
			Если Файл.Зашифрован Тогда
				Предупреждения.Добавить(РаботаСФайламиСлужебныйКлиентСервер.СообщениеОНедопустимостиПодписанияЗашифрованногоФайла(Файл.Ссылка));
				Продолжить;
			КонецЕсли;
		КонецЦикла;
...
#Область ЗагружаетСтруктуруФайловИКаталоговСКомпьютераВПриложение
...
//    * ВыбранныеФайлы            - СписокЗначений - импортируемые объекты Файл с компьютера или устройства.
//    * Индикатор                 - Число - число от 0 до 100 - прогресс выполнения.
//    * Комментарий               - Строка - комментарий.
//    * ХранитьВерсии             - Булево - хранить версии.
//    * УдалитьФайлыПослеЗагрузки - Булево - удалять файлы ВыбранныеФайлы после завершения загрузки.
//    * Рекурсивно                - Булево - рекурсивно обходить подкаталоги.
//    * ИдентификаторФормы        - УникальныйИдентификатор - идентификатор формы.
...
Функция ПараметрыИмпортаФайлов() Экспорт
	...
	ПараметрыВыполнения.Вставить("УдалитьФайлыПослеЗагрузки");
...
//    * Комментарий               - Строка - комментарий.
//    * ХранитьВерсии             - Булево - хранить версии.
//    * УдалитьФайлыПослеЗагрузки - Булево - удалять файлы ВыбранныеФайлы после завершения импорта.
//    * Рекурсивно                - Булево - рекурсивно обходить подкаталоги.
...
Процедура ИмпортФайловРекурсивноБезДиалогов(Знач Владелец, Знач ВыбранныеФайлы, Знач ПараметрыВыполнения, Знач ЗадаватьВопросПапкаУжеСуществует)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Попытка
			...
			// Создаем элемент справочника Файлы.
			ИмпортироватьФайл(ВыбранныйФайл, Владелец, ПараметрыВыполнения, АдресВременногоХранилищаФайла, 
				АдресВременногоХранилищаТекста);
			Если ЭтоАдресВременногоХранилища(АдресВременногоХранилищаТекста) Тогда
				УдалитьИзВременногоХранилища(АдресВременногоХранилищаТекста);
			КонецЕсли;
...
// ПараметрыВыполнения - см. ИмпортФайловРекурсивно.ПараметрыВыполнения
//
Процедура ИмпортироватьФайл(Знач ВыбранныйФайл, Знач Владелец, Знач ПараметрыВыполнения, 
...
#КонецЕсли
	...
	Если НРег(ДанныеФайла.Расширение) = НРег("mxl") Тогда
		...
		ПараметрыФормы.ИмяДокумента = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(
			ДанныеФайла.ПолноеНаименованиеВерсии, ДанныеФайла.Расширение);
		ПараметрыФормы.ПутьКФайлу = ИмяОткрываемогоФайла;
		Если Не ДанныеФайла.НаЧтение Тогда
			ПараметрыФормы.Вставить("ПрисоединенныйФайл", ДанныеФайла.Ссылка);
		КонецЕсли;
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеВопрос",      НСтр("ru = 'Завершить работу с приложением?'"));
...
//  ТаблицаФайлов - Массив из см. ДанныеФайловВРабочемКаталоге - файлы к удалению.
//
Процедура ОбходФайловТаблица(Знач Путь, Знач МассивФайлов, Знач ТаблицаФайлов)
	
#Если Не ВебКлиент Тогда

	ИмяКаталога = РабочийКаталогПользователя();
	АнализируемыеФайлы = Новый Массив;
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		...
		АнализируемыеФайлы.Добавить(ОтносительныйПуть);
	КонецЦикла;
	
	СведенияОФайлах = РаботаСФайламиСлужебныйВызовСервера.СведенияОФайлахВРабочемКаталоге(АнализируемыеФайлы);
	Для каждого СведенияОФайлеКлюч Из СведенияОФайлах Цикл
		ОтносительныйПуть = СведенияОФайлеКлюч.Ключ;
		СведенияОФайле = СведенияОФайлеКлюч.Значение;
		...
		ДатаПомещения = ?(СведенияОФайле.ФайлЕстьВРегистре, СведенияОФайле.ДатаПомещения, Дата('00010101'));
		
		// Если не занят текущим пользователем, можно удалить.
		Если Не СведенияОФайле.ФайлЕстьВРегистре 
			Или СведенияОФайле.ФайлЕстьВРегистре И Не СведенияОФайле.РедактируетТекущийПользователь Тогда
			...
			Исключение
				Запись.Размер = 0;
			КонецПопытки;
			Запись.Версия = СведенияОФайле.Файл;
...
Процедура СравнитьТабличныеДокументы(ПутьКФайлу1, ПутьКФайлу2, ЗаголовокЛевый, ЗаголовокПравый)
	...
	ПараметрыОткрытияФормы = СтандартныеПодсистемыКлиент.ПараметрыСравненияТабличныхДокументов();
	ПараметрыОткрытияФормы.ЗаголовокЛевый = ЗаголовокЛевый;
	ПараметрыОткрытияФормы.ЗаголовокПравый = ЗаголовокПравый;
	ПараметрыОткрытияФормы.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Сравнение %1 с %2'"), ЗаголовокЛевый, ЗаголовокПравый);
	СтандартныеПодсистемыКлиент.ПоказатьСравнениеТабличныхДокументов(ПомещенныеФайлы[0].Хранение, 
		ПомещенныеФайлы[1].Хранение, ПараметрыОткрытияФормы);
...
Процедура ОткрытьФормуПеретаскиванияИзвне(ПапкаДляДобавления, МассивИменФайлов) Экспорт
	...
	ОткрытьФорму("Справочник.Файлы.Форма.ПеретаскиваниеФайлов", ПараметрыФормы);
	
КонецПроцедуры

#Область СлужебныеПроцедурыИФункцииДляАсинхронныхМетодов
...
#Область ПроцедурыИФункцияДляРаботыСоСканером
...
Процедура ПроинициализироватьКомпоненту(ОповещениеВозврата, ПредложитьУстановить = Ложь) Экспорт
	...
	Если Не ДоступноСканирование() Тогда 
		...
		РезультатПодключенияКомпоненты.ОписаниеОшибки = НСтр("ru = 'Сканирование доступно только при работе в Microsoft Windows или Linux.'");
...
#Иначе
			...
			Или ОбщегоНазначенияКлиент.ТипПлатформыКлиента() = ТипПлатформы.Windows_x86_64
			Или ОбщегоНазначенияКлиент.ТипПлатформыКлиента() = ТипПлатформы.Linux_x86 
			Или ОбщегоНазначенияКлиент.ТипПлатформыКлиента() = ТипПлатформы.Linux_x86_64;
#КонецЕсли
	Иначе
...
#Область ВспомогательныеПроцедурыДляПоддержкиАсинхроннойМодели
...
#Область ОбработкаСобытийОбщихФормРаботыСФайлами
...
//   ДанныеФайла - см. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла
//
Процедура ВыбратьРежимИРедактироватьФайл(ОбработчикРезультата, ДанныеФайла, ДоступностьКомандыРедактировать)
	...
	Если Не ЗначениеЗаполнено(ДанныеФайла.Редактирует)
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.КакОткрытьФайл", , , , , , Обработчик, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		Возврат;
	КонецЕсли;
...
#Область ПечатьТабличногоИлиОфисногоДокументаСоШтампомЭлектроннойПодписи
...
Процедура НапечататьФайлСоШтампом(Ссылка, УникальныйИдентификатор) Экспорт
	...
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайла(Ссылка, ДополнительныеПараметры);
	Если Не ДанныеФайла.ПодписанЭП Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Печать документа со штампом электронной подписи недоступна.'"));
		Возврат;
	КонецЕсли;
	 
	Если ДанныеФайла.Зашифрован Тогда
		МассивСертификатовШифрования = ДанныеФайла.МассивСертификатовШифрования;
		ПолучитьРасшифрованныеДанныеДляПечати(ДанныеФайла, УникальныйИдентификатор);
	Иначе
		ПечатьФайлаСоШтампом(ДанныеФайла);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПолучитьРасшифрованныеДанныеДляПечати(ДанныеФайла, УникальныйИдентификатор)
	
	ОбработчикПродолжения = Новый ОписаниеОповещения("ПечатьФайлаСоШтампомПослеРасшифровки", ЭтотОбъект,
		Новый Структура("ДанныеФайла, УникальныйИдентификатор", ДанныеФайла, УникальныйИдентификатор));
		
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		ВыполнитьОбработкуОповещения(ОбработчикПродолжения, Ложь);
		Возврат;
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("СертификатыШифрования",
		ПоместитьВоВременноеХранилище(ДанныеФайла.МассивСертификатовШифрования, УникальныйИдентификатор));
	ОписаниеДанных.Вставить("СообщитьОЗавершении", Ложь);
...
Процедура ПечатьФайлаСоШтампомПослеРасшифровки(ОписаниеДанных, Контекст) Экспорт
	...
	Контекст.ДанныеФайла.СсылкаНаДвоичныеДанныеФайла = АдресФайла;
	ПечатьФайлаСоШтампом(Контекст.ДанныеФайла);
	
КонецПроцедуры

Процедура ПечатьФайлаСоШтампом(ДанныеФайла)
	
	Документ = РаботаСФайламиСлужебныйВызовСервера.ДокументСоШтампом(ДанныеФайла);
...
Функция ПолучитьФайлВРабочийКаталог(Знач АдресДвоичныхДанныхФайла,
	...
	Исключение
		СообщениеОбОшибке = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщениеОбОшибке = НСтр("ru = 'Не удалось создать каталог на компьютере:'") + " " + СообщениеОбОшибке;
		ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке);
		Возврат Ложь;
	КонецПопытки;
...
//   ДанныеФайла - см. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла
//
Процедура ПоказатьДиалогНужноПолучитьФайлССервера(ОбработчикРезультата, Знач ИмяФайлаСПутем, Знач ДанныеФайла, 
	Знач ДляРедактирования)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределено", 
		ЭтотОбъект, Параметры);
...
Функция ПодключенныеУстройства(Форма, ПодключаемыйМодуль)
	...
	Исключение   
		...
		ПоказатьОшибкуСканирования(Форма, НСтр("ru='Не удалось получить список устройств'"), ПредставлениеОшибки);
	КонецПопытки;
...
Функция НастройкаСканера(Форма, ПодключаемыйМодуль, ИмяУстройства, ИмяНастройки) Экспорт
	...
	Исключение
		...
		ПоказатьОшибкуСканирования(Форма, НСтр("ru='Не удалось получить настройки устройства.'"), ПредставлениеОшибки);
		Возврат -1;
	КонецПопытки;
...
Функция ЕстьУстройства(Форма, ПодключаемыйМодуль, ПоказыватьОшибку = Истина) Экспорт
	...
	Исключение
		...
		Если ПоказыватьОшибку Тогда
			ПоказатьОшибкуСканирования(Форма, НСтр("ru='Не удалось проверить наличие устройств'"), ПредставлениеОшибки);
		КонецЕсли;
		ЕстьУстройства = Ложь;
	КонецПопытки;
...
//  ПараметрыСканирования - см. РаботаСФайламиСлужебныйКлиентСервер.ПараметрыСканирования
//
Процедура НачатьСканирование(Форма, ПодключаемыйМодуль, ПараметрыСканирования) Экспорт
	
	ПараметрСжатие = ?(ВРег(ФорматКартинки) = "JPG", ПараметрыСканирования.КачествоJPG, 
		ПараметрыСканирования.СжатиеTIFF);
	
	ЗначенияПараметров = Новый Массив;
	ЗначенияПараметров.Добавить("ПоказыватьДиалог" + ": " + ПараметрыСканирования.ПоказыватьДиалог);
	ЗначенияПараметров.Добавить("ВыбранноеУстройство" + ": " + ПараметрыСканирования.ВыбранноеУстройство);
	ЗначенияПараметров.Добавить("ФорматКартинки" + ": " + ПараметрыСканирования.ФорматКартинки);
	ЗначенияПараметров.Добавить("Разрешение" + ": " + ПараметрыСканирования.Разрешение);
	ЗначенияПараметров.Добавить("Цветность" + ": " + ПараметрыСканирования.Цветность);
	ЗначенияПараметров.Добавить("Поворот" + ": " + ПараметрыСканирования.Поворот);
	ЗначенияПараметров.Добавить("РазмерБумаги" + ": " + ПараметрыСканирования.РазмерБумаги);
	ЗначенияПараметров.Добавить("ПараметрСжатие" + ": " + ПараметрСжатие);
	ЗначенияПараметров.Добавить("ДвустороннееСканирование" + ": " + ПараметрыСканирования.ДвустороннееСканирование);
	ЗначенияПараметров.Добавить("АвтоматическаяПодачаДокументов" + ": " + ПараметрыСканирования.АвтоматическаяПодачаДокументов);

	ТекстСобытия = "НачатьСканирование" + "(" + СтрСоединить(ЗначенияПараметров, ", ") + ")";
	...
	Попытка
		ПодключаемыйМодуль.НачатьСканирование(
			...
			ПараметрыСканирования.АвтоматическаяПодачаДокументов);
	...
	Исключение
		...
		ПоказатьОшибкуСканирования(Форма, НСтр("ru='Не удалось отсканировать документ'"), ПредставлениеОшибки);
	КонецПопытки;
...
Процедура ВключитьЖурналСканирования(ПодключаемыйМодуль, ОповещениеПродолжения, Знач Перезапуск = Ложь) Экспорт 
	ИдентификаторКлиента = ИдентификаторКлиента();
	ПараметрыЖурналаСканирования = РаботаСФайламиСлужебныйВызовСервера.ПараметрыЖурналаСканирования(ИдентификаторКлиента);
	
	Если Не Перезапуск Тогда
		Перезапуск = ПараметрыЖурналаСканирования.ДатаНачалаЖурналаСканирования = Дата(1,1,1);
	КонецЕсли;
	
	ИмяЛогФайла = ПараметрыЖурналаСканирования.ИмяЛогФайла;
	...
	Если Перезапуск И ИмяЛогФайла <> Неопределено Тогда
		...
		КаталогФайла = Новый Файл(ФайлЛога.Путь);
	...
	КаталогЖурналаСканирования = ПараметрыЖурналаСканирования.КаталогЖурналаСканирования;
	ИспользоватьКаталогЖурналаСканирования  = ПараметрыЖурналаСканирования.ИспользоватьКаталогЖурналаСканирования;
	
	Контекст = Новый Структура;
	Контекст.Вставить("ПодключаемыйМодуль", ПодключаемыйМодуль);
	Контекст.Вставить("ОповещениеПродолжения", ОповещениеПродолжения);
	
	ОповещениеПослеСозданияВременногоКаталогаДляПодключенияЛогФайлов = Новый ОписаниеОповещения(
	...
	Если ИспользоватьКаталогЖурналаСканирования И КаталогЖурналаСканирования <> Неопределено Тогда
		...
		ОповещениеРезультатаПроверки = Новый ОписаниеОповещения("ПослеПроверкиДоступностиКаталогаЖурналаСканирования", 
			ЭтотОбъект, Контекст);	
	...
	ИначеЕсли Перезапуск Тогда
		...
		ОповещениеРезультатаПроверки = Новый ОписаниеОповещения("ПослеПроверкиДоступностиКаталогаЛогФайла", 
			ЭтотОбъект, Контекст);
		ФайлЛога = Новый Файл(ИмяЛогФайла);
...
Процедура ПослеПроверкиДоступностиКаталогаЖурналаСканирования(Результат, ВнешнийКонтекст) Экспорт
	Если Не Результат.Успешно Тогда
		РаботаСФайламиСлужебныйВызовСервера.СброситьПараметрыКаталогаЖурналаСканирования(ВнешнийКонтекст.ИдентификаторКлиента);
		ЗаписатьЖурналСканирования("ФайлКомпоненты", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Каталог журнала сканирования недоступен %1. Использование каталога отменено.'"), 
			ВнешнийКонтекст.КаталогЖурналаСканирования), Истина);
		...
		Оповестить("ИзмененыНастройкиСканирования", НастройкиСканирования); 
...
Процедура ОповещениеПослеСозданияВременногоКаталогаДляПодключенияЛогФайлов(КаталогЖурналаСканирования, Контекст) Экспорт
	...
	РаботаСФайламиСлужебныйВызовСервера.УстановитьПараметрыНачалаЖурналаСканирования(ИмяЛогФайла);
...
Процедура ПоказатьОшибкуСканирования(Форма, Заголовок, ПодробноеПредставлениеОшибки)
	
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПредставлениеОшибки") Тогда
		Возврат;
	КонецЕсли;
	
	Форма.ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1: %2'"),
		Заголовок,
		ПодробноеПредставлениеОшибки);
	
	ПоказатьПредупреждение(, Заголовок);
	
КонецПроцедуры

Функция БылаОшибкаСканирования() Экспорт
	
	ПараметрыЗаданияСканирования = ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекЗагрузить(
		"КомпонентаСканирования", "ПараметрыЗаданияСканирования", Неопределено);
	Возврат ПараметрыЗаданияСканирования <> Неопределено;
	
КонецФункции

Процедура УдалитьОшибкуСканирования(ПодключаемыйМодуль, ОповещениеЗавершения = Неопределено, 
	ОчиститьЖурналКомпоненты = Истина) Экспорт
	
	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить("КомпонентаСканирования", 
		"ПараметрыЗаданияСканирования", Неопределено);
	ВключитьЖурналСканирования(ПодключаемыйМодуль, ОповещениеЗавершения, ОчиститьЖурналКомпоненты);
...
Процедура ПолучитьТехническуюИнформацию(ПодробноеПредставлениеОшибки, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Если Не ДиалогОткрытияФайла.Выбрать() Тогда
		Возврат;
	КонецЕсли;
	ИменаФайлов = ДиалогОткрытияФайла.ВыбранныеФайлы;
	...
	Контекст.Вставить("ОповещениеПослеПолученияТехническойИнформации", ОповещениеПриЗавершении);
	Контекст.Вставить("ИмяФайла", ИменаФайлов[0]);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеСозданияВременногоКаталогаДляЛогФайлов", ЭтотОбъект, Контекст);
...
#Если Не ВебКлиент Тогда	
	...
	ЗаписьZIP = Новый ЗаписьZipФайла(Контекст.ИмяФайла);
	ИмяЛогФайла = ТехническаяИнформация.ИмяЛогФайла;
	Если ИмяЛогФайла = Неопределено Тогда
		ЗаписатьЖурналСканирования("ФайлКомпоненты", НСтр("ru='Не задано имя файла журнала компоненты сканирования.'"), 
			Истина);
	Иначе
	...
	ИнформацияДляПоддержки = СтандартныеПодсистемыКлиент.ИнформацияДляПоддержки();
	Текст =  НСтр("ru = 'Информация о компьютере:'") + Символы.ПС + ИнформацияДляПоддержки;
...
#Если Не ВебКлиент Тогда
	...
	Если РезультатИнициализации.Подключено Тогда
		...
		Исключение
			ЗаписатьЖурналСканирования("Версия", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
				Истина);
		КонецПопытки;
	Иначе
		Возврат;
	...
	ТекстСводнойИнформации = ТекстСводнойИнформации + НСтр("ru = 'Режим работы информационной базы'")+ " - "
		+ РежимРаботы;
	...
	КонецЦикла;
	ОповещениеЗавершения = Новый ОписаниеОповещения("СводнаяИнформацияПослеПолученияКомпонентыИУдаленияОшибкиСканирования", 
		ЭтотОбъект, Контекст);
...
Процедура УдалитьДанные(ОбработчикЗавершения, ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	Если ТипЗнч(ФайлИлиВерсия) <> Тип("СправочникСсылка.ВерсииФайлов") Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удалить файл %1 без возможности восстановления?'"),
...
Процедура УдалитьДанныеФайловПослеОтветаНаВопрос(Ответ, Контекст) Экспорт
	...
	Результат = Новый Структура("Файлы,ИндексУдаляемогоФайла,ОбработчикЗавершения,ТекстПредупреждения", 
		Новый Массив, 0);
		...
		Для Каждого Предупреждение Из Предупреждения Цикл
			ТекстПредупреждения = ?(ТекстПредупреждения = "", "", Символы.ПС)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1: %2'"), 
					Предупреждение.Ключ, Предупреждение.Значение);
		КонецЦикла;
...
Процедура ПослеВосстановленияФайлов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ПодробностиВыполнения = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		Если ПодробностиВыполнения.Обработано < ПодробностиВыполнения.Всего Тогда
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Нстр("ru='Восстановлено %1 из %2 поврежденных сведений о файлах. Подробности см. в журнале регистрации.'"),
				ПодробностиВыполнения.Обработано, ПодробностиВыполнения.Всего);
			ПоказатьОповещениеПользователя(НСтр("ru = 'Восстановление сведений о файлах'"),,
				Сообщение, БиблиотекаКартинок.ДиалогВосклицание, СтатусОповещенияПользователя.Важное);
		ИначеЕсли ПодробностиВыполнения.Всего > 0 Тогда
			ПоказатьОповещениеПользователя(НСтр("ru = 'Восстановление сведений о файлах'"),,
				Нстр("ru='Сведения о файлах восстановлены. Переформирование отчета...'"));
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Нет поврежденных файлов для восстановления.'"));
			Возврат;
		КонецЕсли;
		
		ДополнительныеПараметры.ФормаОтчета.СкомпоноватьРезультат();
...
Функция ИмяИРасширениеФайла(Файл)
#Если МобильныйКлиент Тогда
...
#КонецЕсли
	Результат = Новый Структура;
	Результат.Вставить("РасширениеФайла", РасширениеФайла);
	Результат.Вставить("ИмяФайлаБезРасширения", ИмяФайлаБезРасширения);
	
	Возврат Результат;
КонецФункции

// Формирует техническую информацию для поддержки.
//
// Параметры:
//  ОписаниеОшибки       - Строка - основное описание ошибки.
//  ОбработчикЗавершения - ОписаниеОповещения, Неопределено - обработчик завершения, который вызывается после
//                                                            формирования технической информации. В Результате
//                                                            возвращается Структура с полями:
//                                                              * ТехническаяИнформация - Строка - техническая информация.
//                                                              * ДополнительныеФайлы - Массив из см. ОбращенияВТехническуюПоддержкуСлужебныйКлиент.ДанныеДополнительногоФайла.
//
Процедура СформироватьИнформациюДляПоддержки(ОписаниеОшибки, ОбработчикЗавершения = Неопределено) Экспорт
...
#Если Не ВебКлиент Тогда
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПродолжитьФормированиеИнформацииДляПоддержки",
		ЭтотОбъект,
...
Процедура ПродолжитьФормированиеИнформацииДляПоддержки(Результат, ДополнительныеПараметры) Экспорт
	
	РезультатФормирования = Новый Структура;
	РезультатФормирования.Вставить("ТехнологическаяИнформация", "");
	РезультатФормирования.Вставить("ДополнительныеФайлы", Новый Массив);
	
	ИнформацияДляПоддержки = Новый Массив;
	ИнформацияДляПоддержки.Добавить(СтандартныеПодсистемыКлиент.ИнформацияДляПоддержки());
	ИнформацияДляПоддержки.Добавить(НСтр("ru = 'Техническая информация'"));
	ИнформацияДляПоддержки.Добавить(ДополнительныеПараметры.ОписаниеОшибки);
	ИнформацияДляПоддержки.Добавить("");
	...
	Если Результат.Подключено Тогда
		...
		Попытка
			...
			ИнформацияДляПоддержки.Добавить(ИнформацияОКомпоненте);
			ИнформацияДляПоддержки.Добавить("");
			
		Исключение
			
			ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписатьЖурналСканирования(НСтр("ru = 'Версия компоненты'"), ПодробноеПредставлениеОшибки, Истина);
			
		КонецПопытки;
		
	КонецЕсли;
	
	РежимРаботы = РежимРаботыПриложенияДляПоддержки();
	ИнформацияДляПоддержки.Добавить(РежимРаботы);
	
	ТехническаяИнформация = РаботаСФайламиСлужебныйВызовСервера.ТехническаяИнформация();
	ИнформацияДляПоддержки.Добавить(ТехническаяИнформация.ТехническаяИнформацияОВерсияхПодсистемИРасширениях);
	
	РезультатФормирования.ТехнологическаяИнформация = СтрСоединить(ИнформацияДляПоддержки, Символы.ПС);
	
	ДанныеФайлаЛогирования = ДанныеФайлаЛогированияКомпонентыДляПоддержки();
	
	Если ДанныеФайлаЛогирования <> Неопределено Тогда
		РезультатФормирования.ДополнительныеФайлы.Добавить(ДанныеФайлаЛогирования);
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикЗавершения, РезультатФормирования);
	УдалитьОшибкуСканирования(ПодключаемыйМодуль);
	
КонецПроцедуры

Функция ДанныеФайлаЛогированияКомпонентыДляПоддержки()
	
	Результат = Неопределено;
	
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку") Тогда
		Возврат Результат;
	КонецЕсли;
	
	ПараметрыЖурналаСканирования = РаботаСФайламиСлужебныйВызовСервера.ПараметрыЖурналаСканирования(
		ИдентификаторКлиента());
	
	ИмяФайлаЛогирования = ПараметрыЖурналаСканирования.ИмяЛогФайла;
	ФайлЛогирования = Новый Файл(ИмяФайлаЛогирования);
	
	Если ФайлЛогирования.Существует() Тогда
		
		АдресХраненияЛогов = ПоместитьВоВременноеХранилище(
			Новый ДвоичныеДанные(ИмяФайлаЛогирования),
			Новый УникальныйИдентификатор);
		
		// Служба технической поддержки поддерживает вложения формата "txt".
		...
		Результат = МодульОбращенияВТехническуюПоддержкуСлужебныйКлиент.ДанныеДополнительногоФайла(
			АдресХраненияЛогов,
			НСтр("ru = 'ImageScan.txt'"));
	...
	Иначе
		...
		ЗаписатьЖурналСканирования(НСтр("ru = 'Файл компоненты'"), Комментарий, Истина);
...
Функция РежимРаботыПриложенияДляПоддержки()
	
	Результат = "";
	
	Если ОбщегоНазначенияКлиент.ИнформационнаяБазаФайловая() Тогда
		Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда
			Результат = НСтр("ru = 'Режим работы информационной базы: Файловый через веб-сервер'");
		Иначе
			Результат = НСтр("ru = 'Режим работы информационной базы: Файловый'");
		КонецЕсли;
	Иначе
		Результат = НСтр("ru = 'Режим работы информационной базы: Клиент-серверный'");
	КонецЕсли;
...
Процедура АктуализироватьСпискиПрисоединенныхФайлов(ДанныеФайла)

	Если Не ДанныеФайла.Свойство("ОбновлятьСпискиФайловНаФормах") Тогда
		Возврат;
	КонецЕсли;

	Если ДанныеФайла.ОбновлятьСпискиФайловНаФормах Тогда
		ПараметрыОповещенияЗаписиФайла = ПараметрыОповещенияЗаписиФайла("ФайлыИзмененыПриРаботеСАрхивом");
		
		Если ТипЗнч(ДанныеФайла.Ссылка) = Тип("СправочникСсылка.Файлы") И ЗначениеЗаполнено(ДанныеФайла.Версия) Тогда
			ФайлСсылка = ДанныеФайла.Версия;
		Иначе
			ФайлСсылка = ДанныеФайла.Ссылка;
		КонецЕсли;
