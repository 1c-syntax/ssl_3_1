...
Процедура ОчиститьРабочийКаталогЗапуск(ПараметрыВыполнения)
	...
	ОбработчикЗавершения = Новый ОписаниеОповещения("ОчиститьРабочийКаталогОбработчикДиалогаВЦикле", ЭтотОбъект);
	ЗарегистрироватьОбработчикЗавершения(ПараметрыВыполнения, ОбработчикЗавершения);
...
Процедура ОчиститьРабочийКаталогОбработчикЦикла(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.НомерШага = 1 Тогда
		Если Не ПараметрыВыполнения.ДаДляВсех
			...
			ПоказатьВопрос(ОбработчикЗавершения(ПараметрыВыполнения), ТекстВопроса, Кнопки);
			Возврат;
		КонецЕсли;
...
Процедура ПеренестиФайлВПриложенныеФайлы(ФайлСсылка, ВладелецФайла)
	...
	Если Результат = "Скопировать" Тогда
		...
		Оповестить("Запись_Файл", Новый Структура("Владелец, Файл, ЭтоНовый", ВладелецФайла, СозданФайл, Истина), СозданФайл);
