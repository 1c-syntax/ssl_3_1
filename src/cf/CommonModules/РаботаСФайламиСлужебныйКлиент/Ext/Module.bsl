...
Процедура ПроверитьПодписиПослеПроверкиСтроки(Результат, ДополнительныеПараметры) Экспорт
	...
	РаботаСФайламиСлужебныйКлиентСервер.ЗаполнитьСтатусПодписи(СтрокаПодписи, ОбщегоНазначенияКлиент.ДатаСеанса());
...
#Иначе
	...
	Если ПараметрыСканирования = Неопределено Тогда
		ПараметрыСканирования = РаботаСФайламиСлужебныйВызовСервера.ПараметрыСканирования(Истина, ИдентификаторКлиента);
	КонецЕсли;
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Для каждого Элемент Из Коллекция Цикл
		...
		ДанныеСтроки.Вставить("ТипПодписи",          СтрокаПодписи.ТипПодписи);
		ДанныеСтроки.Вставить("СрокДействияПоследнейМеткиВремени",
			СтрокаПодписи.СрокДействияПоследнейМеткиВремени);
...
Процедура ПроинициализироватьКомпоненту(ОповещениеВозврата, ПредложитьУстановить = Ложь) Экспорт
	...
	ОбщегоНазначенияКлиент.ПодключитьКомпонентуИзМакета(ОповещениеВозврата, "AddInNativeExtension", "ОбщийМакет.КомпонентаTWAIN", ПараметрыПодключения);
...
Функция ПолучитьУстройства(ПодключаемыйМодуль) Экспорт
	
	Массив = Новый Массив;
	
	СтрокаУстройств = ПодключаемыйМодуль.ПолучитьУстройства();
...
//
Функция ПолучитьНастройку(ПодключаемыйМодуль, ИмяУстройства, ИмяНастройки) Экспорт
	...
	Попытка
		Возврат ПодключаемыйМодуль.ПолучитьНастройку(ИмяУстройства, ИмяНастройки);
	Исключение
		Возврат -1;
	КонецПопытки;
...
Функция ГотовностьКСканированию(ПодключаемыйМодуль) Экспорт
	Если ПодключаемыйМодуль.ЕстьУстройства() Тогда
		Попытка
			ПодключаемыйМодуль.ПолучитьУстройства();
			Возврат Истина;
		Исключение
			Возврат Ложь;
		КонецПопытки;
