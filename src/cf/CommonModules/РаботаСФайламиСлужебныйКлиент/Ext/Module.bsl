...
Процедура УдалитьДанные(ОбработчикЗавершения, ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Удалить'") + " "
		+ ?(ЭтоВерсияФайла, НСтр("ru = 'версию файла'"), НСтр("ru = 'файл'"))
		+ " " + НСтр("ru = 'без возможности восстановления?'");
	ТекстЗаголовка = НСтр("ru = 'Удаление'") + " "
		+ ?(ЭтоВерсияФайла, НСтр("ru = 'версии файла'"), НСтр("ru = 'файла'"));
...
Процедура УдалитьДанныеПослеОтветаНаВопрос(Ответ, Контекст) Экспорт
	...
	ИначеЕсли Не ПустаяСтрока(Результат.ТекстПредупреждения) Тогда
		ПоказатьПредупреждение(Контекст.ОбработчикЗавершения, Результат.ТекстПредупреждения);
	ИначеЕсли Контекст.ОбработчикЗавершения <> Неопределено Тогда
...
Процедура ПослеУдаленияФайлаИзКеша(РезультатУдаления, Контекст) Экспорт
	...
	Если Не ПустаяСтрока(Контекст.ТекстПредупреждения) Тогда
		ПоказатьПредупреждение(Контекст.ОбработчикЗавершения, Контекст.ТекстПредупреждения);
	ИначеЕсли Контекст.ОбработчикЗавершения <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОбработчикЗавершения);
	Иначе
		Оповестить("Запись_Файл", Новый Структура);
