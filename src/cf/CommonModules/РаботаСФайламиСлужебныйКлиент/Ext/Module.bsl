...
//       * ФормаВладелец - ФормаКлиентскогоПриложения из которой вызвано создание файла.
...
//
// Параметры:
//  ОбработчикРезультата - ОписаниеОповещения, Неопределено - Описание процедуры, принимающей
...
// Параметры:
//   РасширениеУстановлено - Булево - .
//   ПараметрыВыполнения   - Структура - с полями:
...
Процедура ОчиститьРабочийКаталогДляОткрываемогоФайла(ПараметрыВыполнения)
	...
	Для каждого Элемент Из ПараметрыВыполнения.ТаблицаФайлов Цикл
		...
		ФайлНаДиске.УстановитьТолькоЧтение(Ложь);
		УдалитьФайл(ПараметрыВыполнения, ПолныйПуть, Ложь);
...
// Параметры:
//   ТекущиеПараметрыВыполнения - Структура - где:
//     * ПараметрыВыполнения - Структура - где:
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Попытка
		...
		ИначеЕсли СпособСравненияВерсийФайлов = "OpenOfficeOrgWriter" Тогда 
			...
			ИнстансDispatcher = ОбъектServiceManager.createInstance("com.sun.star.frame.DispatchHelper");
			...
			ИнстансDesktop.loadComponentFromURL(ПреобразоватьВURL(ПутьКФайлу2), "_blank", 0, ПараметрыДокумента);
			...
			ИнстансDispatcher.executeDispatch(ТекущееОкно, ".uno:ShowTrackedChanges", "", 0, ПараметрыСравнения);
			...
			ИнстансDispatcher.executeDispatch(ТекущееОкно, ".uno:CompareDocuments", "", 0, ПараметрыВызова);
...
//
// Параметры:
//   Контекст - Структура - где:
//     * РезультатДействий - Структура - где:
//       ** Результаты - Массив -.
//
...
Процедура НачатьУдалениеФайлаИзКеша(Контекст)
	...
	УдалитьФайлИзРабочегоКаталога(Оповещение, Контекст.Файлы[Контекст.ИндексУдаляемогоФайла], Истина, Ложь);
