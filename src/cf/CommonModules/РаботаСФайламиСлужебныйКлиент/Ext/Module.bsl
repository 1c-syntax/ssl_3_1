...
#Если МобильныйКлиент Тогда
	...
	Если ТипРезультата = ТипРезультатаКонвертацииДвоичныеДанные() Тогда
	...
	ИначеЕсли ТипРезультата = ТипРезультатаКонвертацииИмяФайла() Тогда
...
// Возвращаемое значение:
//  Строка - обозначение типа результата конвертации - конвертация в файл на клиенте.
//
...
// Возвращаемое значение:
//  Строка - обозначение типа результата конвертации - конвертация в двоичные данные
//
...
// Возвращаемое значение:
//  Строка - обозначение типа результата конвертации - сохранение результата в присоединенный файл
//
	...
	Если ОписаниеДанных.Успех Тогда
		Если ТипЗнч(ПараметрыВыполнения.ПрисоединенныйФайл) = Тип("Массив") Тогда
			...
			Для Каждого Файл Из ПараметрыВыполнения.ПрисоединенныйФайл Цикл
				...
				Оповестить("Запись_Файл", Новый Структура, Файл);
			КонецЦикла;
...
Функция ПолучитьУстройства(Форма, ПодключаемыйМодуль) Экспорт
	...
	Массив = Новый Массив;
	СтрокаУстройств = ПодключенныеУстройства(Форма, ПодключаемыйМодуль);
...
Функция ПодключенныеУстройства(Форма, ПодключаемыйМодуль)
	...
	Попытка
		ПодключенныеУстройства = ПодключаемыйМодуль.ПолучитьУстройства();
		ЗаписатьЖурналСканирования("ПолучитьУстройства.Результат", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ПолучитьУстройства() = %1", ПодключенныеУстройства));
	Исключение   
...
Функция ЕстьУстройства(Форма, ПодключаемыйМодуль) Экспорт
...
Процедура ВключитьЛогированиеКомпоненты(ПодключаемыйМодуль, Знач Перезапуск = Ложь) Экспорт 
	...
	Если Перезапуск И ИмяЛогФайла <> Неопределено Тогда
		ПодключаемыйМодуль.LogFilePath = "";
	...
	ОповещениеПослеСозданияВременногоКаталогаДляПодключенияЛогФайлов = Новый ОписаниеОповещения("ОповещениеПослеСозданияВременногоКаталогаДляПодключенияЛогФайлов", ЭтотОбъект, Контекст);
...
Процедура ПолучитьТехническуюИнформацию(ПодробноеПредставлениеОшибки, ОповещениеПослеПолученияТехническойИнформации = Неопределено) Экспорт
...
#Если Не ВебКлиент Тогда	
	...
	ИмяЛогФайла = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("КомпонентаСканирования", "ИмяЛогФайла");
	...
	ФайлЖурнала = КаталогВременныхФайлов + "ЖурналРегистрации.xml";
	АдресЛогов = РаботаСФайламиСлужебныйВызовСервера.ЖурналСканированияДляТехподдержки();
	ДанныеЛогов = ПолучитьИзВременногоХранилища(АдресЛогов);
	ДанныеЛогов.Записать(ФайлЖурнала);
	ФайлыДляУдаления.Добавить(ФайлЖурнала);
	ЗаписьZIP.Добавить(ФайлЖурнала); 
	
	КонтекстИсходящий = Новый Структура;
	Контекст.Вставить("ПослеФормированияСводнойИнформации", 
		Новый ОписаниеОповещения("ПослеФормированияСводнойИнформации", ЭтотОбъект, ЗаписьZIP));
	Контекст.Вставить("ФайлыДляУдаления", ФайлыДляУдаления);
	Контекст.Вставить("ПодробноеПредставлениеОшибки", ПодробноеПредставлениеОшибки);
	Контекст.Вставить("ОповещениеПослеПолученияТехническойИнформации", ОповещениеПослеПолученияТехническойИнформации);
	Контекст.Вставить("КаталогВременныхФайлов", КаталогВременныхФайлов);
	
	СводнаяИнформация(Контекст);
#КонецЕсли
КонецПроцедуры

Процедура СводнаяИнформация(Контекст)
	...
	Контекст.Вставить("СводнаяИнформация", Текст);
...
Процедура СводнаяИнформацияПослеПолученияКомпоненты(РезультатИнициализации, Контекст) Экспорт
	СводнаяИнформация = Контекст.СводнаяИнформация;
	...
	СводнаяИнформация = СводнаяИнформация + Символы.ПС + ИнформацияОКомпоненте
	...
	СводнаяИнформация = СводнаяИнформация + НСтр("ru = 'Режим работы информационной базы'")+ " - " 
		+ РежимРаботы + Символы.ПС + РаботаСФайламиСлужебныйВызовСервера.ТехническаяИнформацияОВерсияхПодсистемИРасширениях() + Символы.ПС;
	
	ИмяФайлаСводнойИнформации = Контекст.КаталогВременныхФайлов + "СводнаяИнформация.txt";
	ПолучитьДвоичныеДанныеИзСтроки(СводнаяИнформация).Записать(ИмяФайлаСводнойИнформации);
	...
	ВыполнитьОбработкуОповещения(Контекст.ПослеФормированияСводнойИнформации, КонтекстЗавершения);
	
КонецПроцедуры

Процедура ПослеФормированияСводнойИнформации(Контекст, ЗаписьZIP) Экспорт
	ФайлыДляУдаления = Контекст.ФайлыДляУдаления;
	ЗаписьZIP.Добавить(Контекст.ИмяФайлаСводнойИнформации); 
	ФайлыДляУдаления.Добавить(Контекст.ИмяФайлаСводнойИнформации);
