...
Процедура ПриВключенииОтложенногоОбновления(Знач Использование) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОчередьЗаданий") Тогда
		...
		Шаблон = МодульОчередьЗаданий.ШаблонПоИмени("ОтложенноеОбновлениеИБ");
		...
		МодульОчередьЗаданий.ИзменитьЗадание(Задания[0].Идентификатор, ПараметрыЗадания);
...
Процедура ЗапланироватьОбновлениеОбластейДанных(Знач БлокироватьОбласти = Истина, Знач СообщениеБлокировки = "")
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ПараметрыЗадания.Вставить("ВыполнятьЧерезРегламентноеЗадание", ЕстьРасширенияИзменяющиеСтруктуру);
			ПараметрыЗадания.Вставить("ОжидатьЗавершенияВыполненияРегламентногоЗадания", ЕстьРасширенияИзменяющиеСтруктуру);
