...
Процедура СформироватьПланОбновленияОбластиДанных(ИдентификаторБиблиотеки, ВсеОбработчики, 
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Попытка
			...
			Если СохранятьИзмененияПланаОбновления() Тогда
				ПредыдущийПланОбновления = МенеджерЗаписи.ПланОбновления.Получить();
				ТекущиеЗвездочныеОбработчики = Новый Массив;
				Если ПредыдущийПланОбновления <> Неопределено Тогда
					ПараметрыОтбора = Новый Структура;
					ПараметрыОтбора.Вставить("ВерсияРегистрации", "*");
					ТекущиеЗвездочныеОбработчики = ПредыдущийПланОбновления.План.Строки.НайтиСтроки(ПараметрыОтбора, Истина);
				КонецЕсли;
				
				Для Каждого Версия Из ОписаниеПлана.План.Строки Цикл
					Если Версия.Версия <> "*" Тогда
						Продолжить;
					КонецЕсли;
					
					Для Каждого Обработчик Из ТекущиеЗвездочныеОбработчики Цикл
						НайденныйОбработчик = Версия.Строки.Найти(Обработчик.Процедура, "Процедура");
						Если НайденныйОбработчик = Неопределено Тогда
							ЗаполнитьЗначенияСвойств(Версия.Строки.Добавить(), Обработчик);
						КонецЕсли;
					КонецЦикла;
				КонецЦикла;
			КонецЕсли;
			
			МенеджерЗаписи.ПланОбновления = Новый ХранилищеЗначения(ОписаниеПлана, Новый СжатиеДанных(9));
...
Процедура ЗаполнитьПризнакЭтоОсновнаяКонфигурация(ПредыдущееИмяКонфигурации) Экспорт
	...
	ЭлементБлокировки.УстановитьЗначение("ИмяПодсистемы", ПредыдущееИмяКонфигурации);
...
Процедура УдалитьЗаданияОбновленияОбластей() Экспорт
...
Процедура ПриОтметкеРегистрацииОтложенныхОбработчиковОбновления(ИмяПодсистемы, Значение, СтандартнаяОбработка) Экспорт
	...
	Если ИмяПодсистемы <> Неопределено Тогда
		ЭлементБлокировки.УстановитьЗначение("ИмяПодсистемы", ИмяПодсистемы);
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ВерсииПодсистем") Тогда
		...
		Если СозданиеНачальногоОбраза Тогда
			...
			Для Каждого СтрокаТаблицы Из Результат Цикл
				ВерсииПодсистем.Вставить(СтрокаТаблицы.ИмяПодсистемы, СтрокаТаблицы.Версия);
			КонецЦикла;
...
Процедура ПослеОбновленияИнформационнойБазы(Знач ПредыдущаяВерсия, Знач ТекущаяВерсия,
	...
	Если Не МонопольныйРежим() Тогда
		...
		ИмяЗадания = ОбъектМетаданных.Имя;
		// АПК:280-выкл обработка исключения не требуется
		Попытка
			ФоновыеЗадания.Выполнить(Метаданные.РегламентныеЗадания[ИмяЗадания].ИмяМетода,, Задание.Ключ, Задание.Наименование);
		Исключение
...
Процедура ЗапланироватьОбновлениеОбластейДанных()
	...
	Для Каждого СтрокаТаблицы Из ВерсииКонфигурацииВОбластях Цикл
		ВерсииОбластей.Вставить(СтрокаТаблицы.ОбластьДанных, СтрокаТаблицы.Версия);
	КонецЦикла;
...
// и снимает блокировку сеансов в области, в случае если она была установлена ранее.
//
Процедура ВыполнитьОбновлениеТекущейОбластиДанных(АктивированныеРасширения = Неопределено) Экспорт
	...
	Для Каждого Идентификатор Из АктивированныеРасширения Цикл
		...
		Расширения = РасширенияКонфигурации.Получить(Новый Структура("УникальныйИдентификатор", Идентификатор), 
...
Функция СохранятьИзмененияПланаОбновления()
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИмяПодсистемы", Метаданные.Имя);
	Запрос.Текст =
		"ВЫБРАТЬ
	...
	ПланОбновления = Результат[0].ПланОбновления.Получить();
	Если ПланОбновления = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ПланОбновления.ВерсияНа = Метаданные.Версия Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция РезультатБлокировкиОбластиДанных(КлючЗаписи, НомерПопытки)
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Обновление области данных не выполнено. Запись версий области данных заблокирована.'"));
	КонецПопытки;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ВыполнитьОбновлениеТекущейОбластиДанных");
	КонецЕсли;
