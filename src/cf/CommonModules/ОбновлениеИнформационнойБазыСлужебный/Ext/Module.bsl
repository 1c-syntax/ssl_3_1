...
Функция ПредопределенныеДанныеОбъекта(Знач МетаданныеОбъекта, МенеджерОбъекта, ЛокализуемыеРеквизитыОбъекта) Экспорт
	...
	Языки = МультиязычностьСервер.ЯзыкиКонфигурации();
...
// Заполнение предопределенных элементов при первом запуске
//
Процедура НачальноеЗаполнениеПредопределенныхДанных(ОбновитьМультиязычныеСтроки = Ложь) Экспорт
	...
	Для Каждого МетаданныеОбъекта Из ОбъектыСНачальнымЗаполнением Цикл
		...
		ЛокализуемыеРеквизитыОбъекта = МультиязычностьСервер.МультиязычныеРеквизитыОбъекта(МетаданныеОбъекта);
...
Функция ВыполнитьДействияПриОбновленииИнформационнойБазы(ПараметрыОбновления, ДополнительныеПараметры)
...
Процедура ОбновитьСписокВыполняемыхОбработчиковОбновления(ИтерацииОбновления, ПервыйОбменВРИБ = Ложь)
...
Функция ОбработчикиДляОтложеннойРегистрацииДанных(БезФильтра = Ложь) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("РежимВыполнения", Перечисления.РежимыВыполненияОтложенныхОбработчиков.Параллельно);
	...
	Запрос.Текст =
		...
		|	РегистрСведений.ОбработчикиОбновления КАК ОбработчикиОбновления
		|ГДЕ
		|	ОбработчикиОбновления.Версия <> &Версия
		|	И (ОбработчикиОбновления.РежимВыполненияОтложенногоОбработчика = &РежимВыполнения
		|		ИЛИ &БезФильтра)";
	Обработчики = Запрос.Выполнить().Выгрузить();
...
Процедура ДобавитьОтложенныеОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям, ГруппаОбновления, ТекстОшибок)
	...
	Для Каждого СтрокаВерсия Из ОбработчикиПоВерсиям Цикл
		...
		Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
			...
			Если ТекущийРежимВыполнения = "Параллельно" И Обработчик.Версия <> "*" Тогда
...
Процедура ДобавитьНезавершенныеОтложенныеОбработчики(ИтерацияОбновления, ИмяБиблиотеки,
	...
	Для Каждого Обработчик Из НезавершенныеОбработчикиБиблиотеки Цикл
		...
		СтрокаВерсия = Обработчики.Найти(Обработчик.Версия, "Версия");
		Если СтрокаВерсия = Неопределено Тогда
			СтрокаВерсия = Обработчики.Добавить();
			СтрокаВерсия.Версия = Обработчик.Версия;
		КонецЕсли;
		
		НовыйОбработчик = СтрокаВерсия.Строки.Добавить();
		ЗаполнитьЗначенияСвойств(НовыйОбработчик, Обработчик, , "РежимВыполнения");
...
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры, ОбъектМетаданных = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для каждого ОбъектМетаданныхСЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		ЛокализуемыеРеквизитыОбъекта = МультиязычностьСервер.МультиязычныеРеквизитыОбъекта(ОбъектМетаданныхСЭлементами);
		...
		МультиязычностьСервер.СформироватьИменаМультиязычныхРеквизитов(ИменаРеквизитовОбъекта, ЛокализуемыеРеквизитыОбъекта);
		...
		ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонаЗапроса, СтрСоединить(ИменаРеквизитовОбъекта, ", " + Символы.ПС), ОбъектМетаданныхСЭлементами.ПолноеИмя());
		...
		Пока ДанныеЗапроса.Следующий() Цикл
			...
			Если ПоставляемыеСведения <> Неопределено Тогда
				...
				ИначеЕсли ПараметрыРегистрации.РежимОбновления = "МультиязычныеСтроки" Тогда
					...
					Если МультиязычностьСервер.МультиязычныеСтрокиРеквизитовИзменены(ПоставляемыеСведения, ДанныеЗапроса, ЛокализуемыеРеквизитыОбъекта, ПараметрыРегистрации) Тогда
						ОбъектыКОбработке.Добавить(ДанныеЗапроса.Ссылка);
					КонецЕсли;
...
Процедура ЗаполнитьЭлементыНачальнымиДанными(Параметры, ОбъектМетаданных, ОбновитьМультиязычныеСтроки = Ложь) Экспорт
	...
	Если ОбновитьМультиязычныеСтроки Тогда
		Результат = МультиязычностьСервер.ОбновитьМультиязычныеСтрокиПредопределенныхЭлементов(СсылкиНаОбъекты, ОбъектМетаданных);
	Иначе
...
Функция ДанныеСодержатОтличия(ДанныеЗапроса, ИмяРеквизитОбъекта, ПоставляемыеСведения, ЛокализуемыеРеквизитыОбъекта, ПараметрыРегистрации)
	...
	Для каждого ИмяРеквизитаОбъекта Из ИмяРеквизитОбъекта Цикл
		...
		ИмяРеквизитаБезСуффиксаЯзык= МультиязычностьСервер.ИмяРеквизитаБезСуффиксаЯзык(ИмяРеквизитаОбъекта);
		...
		Иначе
			...
			Если МультиязычностьСервер.МультиязычныеСтрокиРеквизитаИзменены(ПоставляемыеСведения, ДанныеЗапроса,
...
Процедура ДобавитьКолонкуТаблицыПредопределенныхДанных(ПредопределенныеДанные, Реквизит, ЛокализуемыеРеквизиты, Языки)
	...
	Для каждого Язык Из Языки Цикл
		ПредопределенныеДанные.Колонки.Добавить(Реквизит.Имя + "_" + Язык, Реквизит.Тип, МультиязычностьСервер.ПризнакЛокализуемогоРеквизита());
	КонецЦикла;
...
Функция КоличествоОбработчиковНаТекущуюВерсию(ИтерацииОбновления, РежимВыполненияОтложенногоОбновления)
	...
	УчитыватьПроцедурыРегистрации = Истина;
	Если РежимВыполненияОтложенногоОбновления = "Отложенно" Тогда
		...
		Если КлиентСервер И Коробка Тогда
			...
			УчитыватьПроцедурыРегистрации = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|
		|ВЫБРАТЬ
		|	КОЛИЧЕСТВО(ОбработчикиОбновленияОбщихДанных.ИмяОбработчика) КАК Количество
		|ИЗ
	...
	КоличествоОбработчиков = Результат[0].Количество + Результат[1].Количество;
	Если РежимВыполненияОтложенногоОбновления = "Монопольно" Тогда
		КоличествоОбработчиков = КоличествоОбработчиков + Результат[2].Количество;
	КонецЕсли;
	Если УчитыватьПроцедурыРегистрации Тогда
		КоличествоОбработчиков = КоличествоОбработчиков + Результат[3].Количество;
	КонецЕсли;
...
Процедура ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика,
	...
	ЗавершитьПроцедуруОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления);
