...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Для Каждого Обработчик Из ОбработчикиПерехода Цикл
		// Обработчики перехода с другой программы.
		Если ЗначениеЗаполнено(Обработчик.Процедура) Тогда
...
// Формирует набор параметров, необходимых для заполнения объекта предопределенными данными.
//
// Параметры:
//   МетаданныеОбъекта - ОбъектМетаданных - объект метаданных, для которого формируются параметры.
//
// Возвращаемое значение:
//   Структура:
//     * НастройкиПредопределенныхЭлементов - Структура - настройки предопределенных элементов объекта.
//     * ПредопределенныеДанные - ТаблицаЗначений - таблица предопределенных данных объекта.
//     * РеквизитыИсключения - Строка - имя реквизита, исключаемого из заполнения
//     * ПоддерживаетсяИерархия - Булево - признак поддержки иерархии объектом.
//     * ТабличныеЧасти - Массив из Строка - имена табличных частей, входящих в предопределенные данные.
//     * ЛокализуемыеРеквизитыОбъекта - Соответствие из КлючИЗначение - локализуемые реквизиты объекта.
//     * МультиязычныеСтрокиВРеквизитах - Булево - признак наличия мультиязычных строк в реквизитах объекта.
//     * ОбъектСодержитТЧПредставления - Булево - признак наличия у объекта табличной части Представления.
//     * ТЧПредставленияИспользуетсяДляГрупп - Булево - признак использования табличной части Представления для групп.
...
//                       - ЗадачаМенеджер
//                       - ПланОбменаМенеджер -  менеджер объекта, полученный по полному имени метаданных.
//     * РеквизитыСЭлементами - Массив из Строка - имена реквизитов, используемых только для элементов
//
			...
			Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ОбъектМетаданных) Или ЭтоПеречисление Тогда
				...
				Если ИмяПоля = Неопределено Тогда
					...
					Если ЭтоБизнесПроцесс Тогда
						ТипТочекМаршрута = Тип("ТочкаМаршрутаБизнесПроцессаСсылка." + ОбъектМетаданных.Имя);
				...
				ИначеЕсли ВРег(ИмяПоля) = ВРег("Точки") Тогда
					ТипТочекМаршрута = Тип("ТочкаМаршрутаБизнесПроцессаСсылка." + ОбъектМетаданных.Имя);
					УдаляемыеТипы.Вставить(ТипТочекМаршрута, ПолноеИмя + ".Точки");
				Иначе // Точка маршрута бизнес-процесса.
...
Функция ИменаТаблицПоТипам()
	...
	Для Каждого МетаданныеБизнесПроцесса Из Метаданные.БизнесПроцессы Цикл
		...
		ТипТочекМаршрута = Тип("ТочкаМаршрутаБизнесПроцессаСсылка." + МетаданныеБизнесПроцесса.Имя);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	КонецЕсли;
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ОчисткаУстаревшихДанных");
	КонецЕсли;
