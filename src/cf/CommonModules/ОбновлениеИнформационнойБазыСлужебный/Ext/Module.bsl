...
Функция ОбработчикиОбновленияВИнтервале(Знач ИсходнаяТаблицаОбработчиков, Знач ВерсияОт, Знач ВерсияДо,
	...
	КонецЕсли;
	Схема.Параметры.Найти("ИдентификаторОбработчикаОчисткиУстаревшихДанных").Значение =
		ИдентификаторОбработчикаОчисткиУстаревшихДанных();
...
// Для внутреннего использования.
//
Функция ПараметрыРаботыПрограммыПередОбновлениемИнформационнойБазыОбновлены() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить(
		"ПараметрыРаботыПрограммыПередОбновлениемИнформационнойБазыОбновлены") <> Неопределено;
...
Функция СтатусНевыполненныхОбработчиков(ПриОбновлении = Ложь) Экспорт
	...
	Если СтатусыОбработчиков.Найти(Перечисления.СтатусыОбработчиковОбновления.НеВыполнялся) <> Неопределено
		Или СтатусыОбработчиков.Найти(Перечисления.СтатусыОбработчиковОбновления.Выполняется) <> Неопределено Тогда
...
Процедура ВыполнитьОтложенноеОбновлениеСейчас(ПараметрыОбновления = Неопределено) Экспорт
	...
	ОбновлениеСкриптом = СтрНайти(ПараметрыЗапуска, "ВыполнитьОбновлениеИЗавершитьРаботу") > 0;
	...
	ВыполнитьДействияПослеОтложенногоОбновленияИнформационнойБазы(СинхронноеОбновление, ОбновлениеСкриптом);
...
Функция ТребуетсяПроверитьЛегальностьПолученияОбновления() Экспорт
	...
	МодульЛегальностьПолученияОбновлений = ОбщегоНазначения.ОбщийМодуль("Обработки.ЛегальностьПолученияОбновлений");
	Результат = МодульЛегальностьПолученияОбновлений.ТребуетсяПроверитьЛегальностьПолученияОбновления();
	...
	Если ЕстьПраваНаОбновлениеИнформационнойБазы(УчитыватьПривилегированныйРежим,
		...
		Если ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
			...
			Если Не Результат.МожноВыполнятьОбновление Тогда
				Сообщение = НСтр("ru = 'Вход в приложение временно невозможен в связи с обновлением на новую версию.
...
Процедура ЗаписатьСведенияОбОбновленииИнформационнойБазы(Знач СведенияОбОбновлении) Экспорт
	...
	МенеджерКонстанты.Значение = Новый ХранилищеЗначения(НовоеЗначение, Новый СжатиеДанных(9));
...
Процедура ПеререгистрироватьДанныеДляОтложенногоОбновления() Экспорт
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Попытка
			...
			ВыполнитьПроцедуру(Обработчик.ПроцедураЗаполненияДанныхОбновления, ПараметрыОбработчикаСтруктура, "РегистрацияДанных");
			Если Обработчик.Многопоточный Тогда
...
Процедура ЗаписатьПараметрыОтложенногоОбработчикаОбновления(Идентификатор, Параметры) Экспорт
	...
	УстановитьСвойствоОбработчика(Идентификатор,
		...
		Новый ХранилищеЗначения(СтатистикаВыполнения, Новый СжатиеДанных(9)));
КонецПроцедуры
		...
		Если ПозицияПараметра > 0 Тогда
			...
			Исключение
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Укажите параметр запуска приложения ""%1"" в формате
						|""%1=Х"", где ""Х"" - максимальное количество потоков обновления.'"),
					"ЧислоПотоковОбновления");
...
Процедура ОбновитьСписокВыполняемыхОбработчиковОбновления(ИтерацииОбновления, ПервыйОбменВРИБ = Ложь, ТипыОбработчиков = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ТекстОшибок) Тогда
			ВызватьИсключение(ТекстОшибок, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОчисткаУстаревшихДанных;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Пользователи.ЭтоПолноправныйПользователь(, Истина)
		...
		ВерсияПроверкиПатчей = Сведения.ВерсияПроверкиПатчей;
		Если ВерсияПроверкиПатчей = Неопределено
			Или ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Метаданные.Версия, ВерсияПроверкиПатчей) > 0 Тогда
			МодульОбновлениеКонфигурации = ОбщегоНазначения.ОбщийМодуль("ОбновлениеКонфигурации");
			Результат = МодульОбновлениеКонфигурации.ИсправленияИзменены();
			Сведения.ВерсияПроверкиПатчей = Метаданные.Версия;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = СтандартныеПодсистемыСервер.ВерсияБиблиотеки(); // Не влияет на выбор для выполнения.
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыСлужебный.ОчиститьУстаревшиеДанныеПолностью";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Идентификатор = ИдентификаторОбработчикаОчисткиУстаревшихДанных();
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ОбновлениеИнформационнойБазыСлужебный.ПустаяПроцедураРегистрации";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Некритичный;
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.ОбработчикиОбновления";
	Обработчик.ИзменяемыеОбъекты = "РегистрСведений.ОбработчикиОбновления";
	Обработчик.Комментарий =
		НСтр("ru = 'Очищает устаревшие данные, чтобы переход на следующую версию при реструктуризации был без ошибки ""Записи регистра стали неуникальными"".'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru = 'Обновление приложения не завершено'");
...
Процедура ПриПолученииПервогоСообщенияОбменаРИБПослеОбновления() Экспорт
	...
	ЗаполнитьНомерОчередиОбработчиков(ИтерацииОбновления);
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Для Каждого Обработчик Из Обработчики Цикл
		// Оперативные и монопольные обработчики.
		Если Обработчик.РежимВыполнения <> "Отложенно"
			И ЗначениеЗаполнено(Обработчик.Процедура) Тогда
			ИменаПроцедур.Вставить(Обработчик.Процедура);
			
			Продолжить;
		КонецЕсли;
		
		// Отложенные обработчики.
		Если ЗначениеЗаполнено(Обработчик.Процедура)
			И Не СтрЗаканчиваетсяНа(Обработчик.Процедура, СтандартноеИмяПроцедурыОтложенногоОбработчика()) Тогда
			ИменаПроцедур.Вставить(Обработчик.Процедура);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Обработчик.ПроцедураЗаполненияДанныхОбновления)
			И Не СтрЗаканчиваетсяНа(Обработчик.ПроцедураЗаполненияДанныхОбновления, СтандартноеИмяПроцедурыРегистрации()) Тогда
			ИменаПроцедур.Вставить(Обработчик.ПроцедураЗаполненияДанныхОбновления);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Обработчик.ПроцедураПроверки)
			И Обработчик.ПроцедураПроверки <> СтандартноеИмяПроцедурыПроверки() Тогда
			ИменаПроцедур.Вставить(Обработчик.ПроцедураПроверки);
		КонецЕсли;
	КонецЦикла;
	
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик обновления информационной базы ""%1"".'");
	
	// Исключения.
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПроцедуру";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ПолноеИмяПроцедуры, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры))";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ПерейтиСДругойПрограммы";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(Обработчик.Процедура, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ПерейтиСДругойПрограммы";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(Обработчик.Процедура)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчикОбновления";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(Обработчик.Процедура, ПараметрыОбработчика)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ОтменитьВыполнениеВсехПотоков";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПотока.ПриОтменеПотока, ПараметрыВызова)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ЗавершитьПотокиВыполнившиеФЗ";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПотока.ПроцедураЗавершения, ПараметрыВызова)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ЗавершитьПотокиВыполнившиеФЗ";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПотока.ПриАварийномЗавершении, ПараметрыВызова)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьПоток";
	ИсключениеПроверки.ФрагментСодержания = "РезультатЗапуска = ДлительныеОперации.ВыполнитьВФоне(ОписаниеПотока.Процедура,
	                                        |			Поток.ПараметрыПроцедуры,
	                                        |			ПараметрыВыполнения)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбновлениеИнформационнойБазы";
	ИсключениеПроверки.ИмяПроцедуры       = "ОбъектОбработан";
	ИсключениеПроверки.ФрагментСодержания = "Обработан = ОбщегоНазначения.ВыполнитьМетодКонфигурации(СвойстваОбработчика.ПроцедураПроверки,
	                                        |					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МетаданныеИОтбор), Истина)";
	
	Методы = Новый Соответствие;
	Методы.Вставить("ПриАварийномЗавершенииПотокаОтложенногоОбработчика");
	Методы.Вставить("ПриОтменеПотокаОтложенногоОбработчика");
	Методы.Вставить("ПриАварийномЗавершенииПотокаПоискаПорцииДляОбновления");
	Методы.Вставить("ПриОтменеПотокаПоискаПорцииДляОбновления");
	Методы.Вставить("ЗаполнитьДанныеОтложенногоОбработчика", Истина);
	Методы.Вставить("ЗавершитьРегистрациюДанныхОтложенногоОбновления", Истина);
	Методы.Вставить("ВыполнитьОтложенныйОбработчик", Истина);
	Методы.Вставить("ЗавершитьВыполнениеОтложенногоОбработчика", Истина);
	Методы.Вставить("НайтиПорциюДляОбновления", Истина);
	Методы.Вставить("ЗавершитьПоискПорцииДляОбновления", Истина);
	Методы.Вставить("УстаревшиеДанныеПриЗапросеПорцийВФоне", Истина);
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ОбновлениеИнформационнойБазыСлужебный", Методы);
...
Функция ОбъектыСНачальнымЗаполнением() Экспорт
	...
	Возврат ОбновлениеИнформационнойБазыСлужебныйПовтИсп.ОбъектыСНачальнымЗаполнением();
...
Процедура НачальноеЗаполнениеПредопределенныхДанных(ОбновитьМультиязычныеСтроки = Ложь,
	...
	ПроверятьНеобходимостьВыполнения = ДополнительныеПараметры <> Неопределено И ТипЗнч(ДополнительныеПараметры) = Тип(
		"Структура") И ДополнительныеПараметры.Свойство("НовыеОбъекты");
	...
	ПараметрыОбновления.ОбновитьТолькоМультиязычныеСтроки = ОбновитьМультиязычныеСтроки;
	
	Для Каждого ОбъектСНачальнымЗаполнением Из ОбъектыСНачальнымЗаполнением Цикл

		МетаданныеОбъекта          = ОбъектСНачальнымЗаполнением.Ключ;
		ПолноеИмяОбъектаМетаданных = ОбъектСНачальнымЗаполнением.Значение;
		
		Если ПроверятьНеобходимостьВыполнения 
		   И ДополнительныеПараметры.НовыеОбъекты.Найти(ПолноеИмяОбъектаМетаданных) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		// @skip-check query-in-loop - механизм обновления
		ОбновитьПредопределенныеЭлементыОбъекта(МетаданныеОбъекта,ПараметрыОбновления);
...
Процедура ОбновитьПредопределенныеЭлементыОбъекта(МетаданныеОбъекта, ПараметрыОбновления) Экспорт
	...
	Контекст = НаборПараметровДляЗаполненияОбъекта(МетаданныеОбъекта);
	
	МенеджерОбъекта                   = Контекст.МенеджерОбъекта;
	ПредопределенныеДанные            = Контекст.ПредопределенныеДанные;
	НастройкиЗаполнения               = Контекст.НастройкиПредопределенныхЭлементов;
	ПолноеИмяОбъектаМетаданных        = МетаданныеОбъекта.ПолноеИмя();
	ОбновитьТолькоМультиязычныеСтроки = ПараметрыОбновления.ОбновитьТолькоМультиязычныеСтроки;
	Элементы                          = ПараметрыОбновления.Элементы;
	ТолькоОпределенныеЭлементы        = Элементы.Количество() > 0;
	
	ЗаполняемыеРеквизиты               = Новый Соответствие;
	Для Каждого ЗаполняемыйРеквизит Из СтрРазделить(ПараметрыОбновления.Реквизиты, ",", Ложь) Цикл
		ЗаполняемыеРеквизиты.Вставить(СокрЛП(ЗаполняемыйРеквизит), Истина);
	КонецЦикла;
	
	ИмяКлючевогоРеквизита = НастройкиЗаполнения.ПереопределяемыеНастройки.ИмяКлючевогоРеквизита;
	
	СуществующиеЭлементы = СуществующиеПоставляемыеЭлементы(ПредопределенныеДанные, НастройкиЗаполнения,
	МенеджерОбъекта, МетаданныеОбъекта);
	...
	Если СтрНачинаетсяС(ПолноеИмяОбъектаМетаданных, "ПланВидовХарактеристик") Тогда
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ПредопределенныеДанные Цикл
			...
			Если СсылкаНаОбъект <> Неопределено Тогда
				...
				Если ТолькоОпределенныеЭлементы И Элементы.Найти(СсылкаНаОбъект) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				БлокировкаДанных = Новый БлокировкаДанных;
				ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(ПолноеИмяОбъектаМетаданных);
			...
			ОтредактированныеРеквизиты = ОтредактированныеРеквизиты(ЗаполняемыйЭлемент, ПолноеИмяОбъектаМетаданных);
			
			Если Не ОбновитьТолькоМультиязычныеСтроки Тогда
				...
				Если ПоддерживаетсяИерархия И ЗначениеЗаполнено(СтрокаТаблицы.Родитель) Тогда
					...
					Если ИмяКлючевогоРеквизита = "Ссылка" Тогда
						Если ТипЗнч(СтрокаТаблицы.Родитель) = Тип("Строка") Тогда
							ЗаполняемыйЭлемент.Родитель = МенеджерОбъекта.ПолучитьСсылку(
							Новый УникальныйИдентификатор(СтрокаТаблицы.Родитель));
				...
				ТекущиеПоляИсключения =?(ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа,
				РеквизитыИсключенияСЭлементами, РеквизитыИсключения);
				ПоляИсключенияОбъекта = Новый Соответствие(Новый ФиксированноеСоответствие(ТекущиеПоляИсключения));
				Для Каждого ОтредактированныйРеквизит Из ОтредактированныеРеквизиты Цикл
					ПоляИсключенияОбъекта.Вставить(ОтредактированныйРеквизит, Истина);
				КонецЦикла;
				
				Если СсылкаНаОбъект <> Неопределено Тогда
					ДанныеЗаполнения = ОпределитьДанныеЗаполнения(ПредопределенныеДанные, СтрокаТаблицы, ПоляИсключенияОбъекта, ЗаполняемыеРеквизиты);
				Иначе
					ДанныеЗаполнения = ОпределитьДанныеЗаполнения(ПредопределенныеДанные, СтрокаТаблицы, ПоляИсключенияОбъекта);
				КонецЕсли;
			...
			Если Контекст.ЛокализуемыеРеквизитыОбъекта.Количество() > 0
				И Контекст.ЛокализуемыеРеквизитыОбъекта["Наименование"] <> Неопределено Тогда
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
					МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
					МодульМультиязычностьСервер.НачальноеЗаполнениеПредопределенныхДанныхЛокализуемыеРеквизиты(
					ЗаполняемыйЭлемент, СтрокаТаблицы, ПоляИсключенияОбъекта, Контекст);
				ИначеЕсли Контекст.ОбъектСодержитТЧПредставления Тогда
					НачальноеЗаполнениеТЧПредставления(ЗаполняемыйЭлемент, СтрокаТаблицы, Контекст);
				КонецЕсли;
			ИначеЕсли ОбновитьТолькоМультиязычныеСтроки И ПредопределенныеДанные.Колонки.Найти("Наименование") <> Неопределено
				И ЗначениеЗаполнено(СтрокаТаблицы["Наименование"]) Тогда
				ЗаполняемыйЭлемент["Наименование"] = СтрокаТаблицы["Наименование"];
			КонецЕсли;
			
			Если Не ОбновитьТолькоМультиязычныеСтроки
				И НастройкиЗаполнения.ПереопределяемыеНастройки.ПриНачальномЗаполненииЭлемента Тогда
				
				МенеджерОбъекта.ПриНачальномЗаполненииЭлемента(ЗаполняемыйЭлемент, СтрокаТаблицы,
				НастройкиЗаполнения.ПереопределяемыеНастройки.ДополнительныеПараметры);
				ОбновлениеИнформационнойБазыПереопределяемый.ПриНачальномЗаполненииЭлемента(
				ПолноеИмяОбъектаМетаданных, ЗаполняемыйЭлемент, СтрокаТаблицы,
				НастройкиЗаполнения.ПереопределяемыеНастройки.ДополнительныеПараметры);
...
Процедура НачальноеЗаполнениеТЧПредставления(ЗаполняемыйЭлемент, ДанныеЗаполнения, ПараметрыЗаполнения) Экспорт
	...
	Для каждого КодЯзыка Из Языки Цикл
		...
		ПредставленияОбъекта = ЗаполняемыйЭлемент.Представления; // ТабличнаяЧасть 
		СтрокаТаблицы = ПредставленияОбъекта.Найти(КодЯзыка, "КодЯзыка");
		Если СтрокаТаблицы  = Неопределено Тогда
			СтрокаТаблицы          = ПредставленияОбъекта.Добавить();
			СтрокаТаблицы.КодЯзыка = КодЯзыка;
		КонецЕсли;
		
		Для каждого ИмяЛокализуемогоРеквизита Из ЛокализуемыеРеквизитыОбъекта Цикл
			Значение = ДанныеЗаполнения[ИмяЛокализуемогоРеквизита.Ключ + "_" + КодЯзыка];
			СтрокаТаблицы[ИмяЛокализуемогоРеквизита.Ключ] = ?(ЗначениеЗаполнено(Значение), Значение, ДанныеЗаполнения[ИмяЛокализуемогоРеквизита.Ключ]);
		КонецЦикла;
...
Функция НаборПараметровДляЗаполненияОбъекта(МетаданныеОбъекта) Экспорт
	...
	ОбъектыСНачальнымЗаполнением = ОбъектыСНачальнымЗаполнением();
	ЕстьПредопределенныеДанные   =  ОбъектыСНачальнымЗаполнением.Получить(МетаданныеОбъекта) <> Неопределено;
...
Процедура ОпределитьИзмененныеРеквизитыПредопределенныхЭлементов(Объект) Экспорт
	...
	ПолноеИмяОбъектаМетаданных = ОбъектыСНачальнымЗаполнением.Получить(ОбъектМетаданных);

	Если Не ЗначениеЗаполнено(ПолноеИмяОбъектаМетаданных) Тогда
		Возврат;
	КонецЕсли;
	
	СодержитОтредактированныеПредопределенныеРеквизиты = СодержитОтредактированныеПредопределенныеРеквизиты(ПолноеИмяОбъектаМетаданных);
	Если Не СодержитОтредактированныеПредопределенныеРеквизиты Тогда
		Возврат;
	КонецЕсли;

	НаборДанныхДляЗаполнения = НаборПараметровДляЗаполненияОбъекта(
		ОбъектМетаданных);
		
	Если НаборДанныхДляЗаполнения.НастройкиПредопределенныхЭлементов.ЕстьКолонкаИмяПредопределенныхДанных Тогда
		ЗначениеКлючевогоРеквизита = Объект["ИмяПредопределенныхДанных"];
	КонецЕсли;
	
	Если ПустаяСтрока(ЗначениеКлючевогоРеквизита) Тогда
		ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизита(
			НаборДанныхДляЗаполнения.НастройкиПредопределенныхЭлементов);
		ЗначениеКлючевогоРеквизита = Объект[ИмяКлючевогоРеквизита];
	Иначе
	...
	СтрокаТаблицы = НаборДанныхДляЗаполнения.ПредопределенныеДанные.Найти(ЗначениеКлючевогоРеквизита,
	...
	РеквизитыОбъекта = ДоступныеДляИзмененияРеквизитыОбъекта(ОбъектМетаданных, ИмяКлючевогоРеквизита);
	ОтредактированныеПредопределенныеРеквизиты = СтрРазделить(Объект.ОтредактированныеПредопределенныеРеквизиты, ",",
		Ложь);
	
	Для Каждого КолонкаТаблицы Из НаборДанныхДляЗаполнения.ПредопределенныеДанные.Колонки Цикл
		ИмяРеквизита = КолонкаТаблицы.Имя;

		Если РеквизитыОбъекта[ИмяРеквизита] = Неопределено Тогда
			Продолжить;
		КонецЕсли;

		МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмяОбъектаМетаданных);

		Если НаборДанныхДляЗаполнения.ЛокализуемыеРеквизитыОбъекта[ИмяРеквизита] <> Неопределено Тогда
			
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
				МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
				МодульМультиязычностьСервер.ОпределитьИзмененныеМультиязычныеЭлементы(Объект,
					СтрокаТаблицы, ИмяРеквизита, ОтредактированныеПредопределенныеРеквизиты, НаборДанныхДляЗаполнения);
			КонецЕсли;
		...
		Иначе
			Если ИмяРеквизита = "Родитель" Тогда
				...
				Если ИмяКлючевогоРеквизита = "Ссылка" Тогда
					Если ТипЗнч(СтрокаТаблицы.Родитель) = Тип("Строка") Тогда
						ЗначениеРеквизита = МенеджерОбъекта.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрокаТаблицы.Родитель));
					ИначеЕсли ТипЗнч(СтрокаТаблицы.Родитель) = Тип("УникальныйИдентификатор") Тогда
						ЗначениеРеквизита = МенеджерОбъекта.ПолучитьСсылку(СтрокаТаблицы.Родитель);
					Иначе
						ЗначениеРеквизита = СтрокаТаблицы.Родитель;
					КонецЕсли;
				ИначеЕсли ТипЗнч(СтрокаТаблицы.Родитель) = Тип("Строка") Тогда
					
					Если ИмяКлючевогоРеквизита = "ИмяПредопределенныхДанных" Тогда
						ЗначениеРеквизита = МенеджерОбъекта[СтрокаТаблицы.Родитель];
					Иначе
						ЗначениеРеквизита = МенеджерОбъекта.НайтиПоРеквизиту(ИмяКлючевогоРеквизита, СтрокаТаблицы.Родитель);
					КонецЕсли;
			...
			Иначе
				ЗначениеРеквизита = СтрокаТаблицы[ИмяРеквизита];
			КонецЕсли;
				
			Если Объект[ИмяРеквизита] <> ЗначениеРеквизита Тогда
				Если ОтредактированныеПредопределенныеРеквизиты.Найти(КолонкаТаблицы.Имя) = Неопределено Тогда
					ОтредактированныеПредопределенныеРеквизиты.Добавить(КолонкаТаблицы.Имя);
				КонецЕсли;
			КонецЕсли;
	...
	КонецЦикла;
	Объект.ОтредактированныеПредопределенныеРеквизиты = СтрСоединить(ОтредактированныеПредопределенныеРеквизиты, ",");
...
Функция СодержитОтредактированныеПредопределенныеРеквизиты(ПолноеИмя) Экспорт
	...
	Результат = Ложь;
	Если Метаданные.ОбщиеРеквизиты.Найти("ОтредактированныеПредопределенныеРеквизиты") = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ОбъектМетаданных    = Метаданные.НайтиПоПолномуИмени(ПолноеИмя);
	СведенияОбРеквизите = Метаданные.ОбщиеРеквизиты.ОтредактированныеПредопределенныеРеквизиты.Состав.Найти(ОбъектМетаданных);

	Если СведенияОбРеквизите <> Неопределено Тогда
		Если СведенияОбРеквизите.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать Тогда
			Результат = Истина;
		ИначеЕсли СведенияОбРеквизите.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Авто
			И Метаданные.ОбщиеРеквизиты.ОтредактированныеПредопределенныеРеквизиты.АвтоИспользование = Метаданные.СвойстваОбъектов.АвтоИспользованиеОбщегоРеквизита.Использовать Тогда
			Результат = Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ДоступныеДляУстановкиИсправления() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы") Тогда
		...
		Если Результат.Исправления <> Неопределено Тогда
...
Функция ЗагрузитьИУстановитьИсправления() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы") Тогда
		МодульПолучениеОбновленийПрограммы = ОбщегоНазначения.ОбщийМодуль("ПолучениеОбновленийПрограммы");
		Результат = МодульПолучениеОбновленийПрограммы.ЗагрузитьИУстановитьИсправления();
	КонецЕсли;
...
Функция ОпределитьДанныеЗаполнения(Знач ПредопределенныеДанные, Знач СтрокаТаблицы, Знач ПоляИсключения, Знач ЗаполняемыеРеквизиты = Неопределено)

	ТолькоЗаполняемыеРеквизиты = ?(ТипЗнч(ЗаполняемыеРеквизиты) = Тип("Соответствие"), ЗаполняемыеРеквизиты.Количество() > 0, Ложь);
	...
	Для Каждого Колонка Из ПредопределенныеДанные.Колонки Цикл
		...
		Если ТолькоЗаполняемыеРеквизиты И ЗаполняемыеРеквизиты[ИмяПоля]= Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Значение) = Тип("Булево") Тогда
			Заполнено = Значение;
			ОбновитьБулево = Истина;
		Иначе
			Заполнено = ЗначениеЗаполнено(Значение);
		КонецЕсли;
		
		Если Заполнено Или ОбновитьБулево Тогда
...
Процедура УстановитьПараметрыРаботыПрограммыПередОбновлениемИнформационнойБазыОбновлены() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ТекущиеПараметры = Новый Соответствие(ПараметрыСеанса.ПараметрыКлиентаНаСервере);
	ТекущиеПараметры.Вставить("ПараметрыРаботыПрограммыПередОбновлениемИнформационнойБазыОбновлены", Истина);
	ПараметрыСеанса.ПараметрыКлиентаНаСервере = Новый ФиксированноеСоответствие(ТекущиеПараметры);
...
Функция ДействияПередОбновлениемИнформационнойБазы(ПараметрыОбновления)
	...
	Если Не ПараметрыОбновления.ПриЗапускеКлиентскогоПриложения Тогда
		...
		Исключение
			...
			ВызватьИсключение;
		КонецПопытки;
		УстановитьПараметрыРаботыПрограммыПередОбновлениемИнформационнойБазыОбновлены();
	КонецЕсли;
	...
	Если НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		Сообщение = НСтр("ru = 'Недостаточно прав для обновления приложения на новую версию.'");
		ЗаписатьОшибку(Сообщение);
		ВызватьИсключение(Сообщение, КатегорияОшибки.НарушениеПравДоступа);
	КонецЕсли;
...
Процедура ВыполнитьДействияПриОбновленииИнформационнойБазы(ПараметрыОбновления, ДополнительныеПараметры)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса");
		МодульОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.УдалитьЗаданияОбновленияОбластей();
	КонецЕсли;
	...
	Сообщение = НСтр("ru = 'Для обновления приложения на новую версию будут выполнены обработчики: %1'");
...
Процедура ВыполнитьДействияПослеОтложенногоОбновленияИнформационнойБазы(СинхронноеОбновление = Ложь, ОбновлениеСкриптом = Ложь)
	
	Если Не ОтложенноеОбновлениеЗавершено() Тогда
		Если ОбновлениеСкриптом И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
			МодульОбновлениеКонфигурации = ОбщегоНазначения.ОбщийМодуль("ОбновлениеКонфигурации");
			ПрерватьОбновление = МодульОбновлениеКонфигурации.ДляТекущейВерсииНеобходимоУспешноеЗавершениеОбработчиков();
			Если ПрерватьОбновление Тогда
				ВызватьИсключение НСтр("ru = 'Отложенное обновление завершилось с ошибками. Подробнее см. в предыдущих записях журнала регистрации.'");
			КонецЕсли;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьНомерОчередиОбработчиков(ИтерацииОбновления)
	
	Обработки.ОписаниеОбработчиковОбновления.ЗаполнитьНомерОчереди(ИтерацииОбновления);
	
	ОбработчикОчисткиУстаревшихДанных = Неопределено;
	МаксимальнаяОчередь = 0;
	Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
		ТекущиеОчереди = ИтерацияОбновления.Обработчики.Скопировать(, "ОчередьОтложеннойОбработки");
		ТекущиеОчереди.Сортировать("ОчередьОтложеннойОбработки УБЫВ");
		Если ЗначениеЗаполнено(ТекущиеОчереди) Тогда
			ТекущаяОчередь = ТекущиеОчереди[0].ОчередьОтложеннойОбработки;
			МаксимальнаяОчередь = ?(МаксимальнаяОчередь < ТекущаяОчередь,
				ТекущаяОчередь, МаксимальнаяОчередь);
		КонецЕсли;
		Если ИтерацияОбновления.Подсистема <> "СтандартныеПодсистемы" Тогда
			Продолжить;
		КонецЕсли;
		ОбработчикОчисткиУстаревшихДанных = ИтерацияОбновления.Обработчики.Найти(
			ИдентификаторОбработчикаОчисткиУстаревшихДанных(), "Идентификатор");
	КонецЦикла;
	
	Если ОбработчикОчисткиУстаревшихДанных <> Неопределено Тогда
		ОбработчикОчисткиУстаревшихДанных.ОчередьОтложеннойОбработки = МаксимальнаяОчередь + 1;
	КонецЕсли;
...
Функция ИдентификаторОбработчикаОчисткиУстаревшихДанных()
	Возврат Новый УникальныйИдентификатор("724850c1-7de4-4d18-8d99-33d0c6b46afa");
КонецФункции

Функция ЭтоОбработчикОчисткиУстаревшихДанных(Обработчик) Экспорт
	Возврат Обработчик.Идентификатор = ИдентификаторОбработчикаОчисткиУстаревшихДанных();
КонецФункции
...
Процедура ВыполнитьОбновлениеИнформационнойБазыВФоне(ПараметрыОбновленияИБ, АдресХранилища) Экспорт
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		РезультатОбновления.Вставить("ИнформацияОбОшибке", ИнформацияОбОшибке);
	...
	Иначе
		...
		Если ОписаниеПредыдущейПодсистемы = Неопределено Тогда
			...
			Для Каждого СтрокаТаблицы Из Результат Цикл
				...
				НаборЗаписей.Отбор.ИмяОбработчика.Установить(СтрокаТаблицы.ИмяОбработчика);
				НаборЗаписей.Прочитать();
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Исключение
			...
			ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При вызове обработчика перехода с другого приложения
				           |""%1""
				           |произошла ошибка:
				           |""%2"".'"),
				ИмяОбработчика,
...
Процедура ВыполнитьПроцедуру(ПолноеИмяПроцедуры, Параметры, Действие)
	
	Если Действие = "РегистрацияДанных" Тогда
		ПредопределенноеИмяПроцедуры = СтандартноеИмяПроцедурыРегистрации();
	ИначеЕсли Действие = "ОтложеннаяОбработка" Тогда
		ПредопределенноеИмяПроцедуры = СтандартноеИмяПроцедурыОтложенногоОбработчика();
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Указано недопустимое значение параметра ""%1"": %2'"),
			"Действие", Действие);
	КонецЕсли;
	
	ИмяПроцедурыЧастями = СтрРазделить(ПолноеИмяПроцедуры, ".");
	Если ИмяПроцедурыЧастями.Получить(ИмяПроцедурыЧастями.ВГраница()) <> ПредопределенноеИмяПроцедуры Тогда
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(ПолноеИмяПроцедуры, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры));
		Возврат;
	КонецЕсли;
	
	ИмяПроцедурыЧастями.Удалить(ИмяПроцедурыЧастями.ВГраница());
	Менеджер = ОбщегоНазначения.ОбщийМодуль(СтрСоединить(ИмяПроцедурыЧастями, "."));
	Если Действие = "РегистрацияДанных" Тогда
		Менеджер.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры);
	Иначе
		Менеджер.ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры);
	КонецЕсли;
	
КонецПроцедуры

Функция СтандартноеИмяПроцедурыРегистрации()
	
	Возврат "ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	
КонецФункции

Функция СтандартноеИмяПроцедурыОтложенногоОбработчика()
	
	Возврат "ОбработатьДанныеДляПереходаНаНовуюВерсию";
	
КонецФункции

Функция СтандартноеИмяПроцедурыПроверки() Экспорт
	
	Возврат "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	
КонецФункции

#Область ПротоколированиеХодаОбновления
...
#Область МеханизмОтложенногоОбновления
...
Процедура ВыполнитьОтложенноеОбновление() Экспорт
	...
	ВременноОтключитьОбновление = Ложь;
	Попытка
		Если ФорсироватьОбновление(СведенияОбОбновлении) Тогда
			...
			Пока ОбработчикиВыполнялись Цикл
				...
				ИначеЕсли Поток = "ПрерватьВыполнение" Тогда
					ВременноОтключитьОбновление = Истина;
					ОжидатьЗавершениеВсехПотоков(Группы); // @skip-check query-in-loop - многопоточное обновление.
					Прервать;
				КонецЕсли;
	...
	Если ВременноОтключитьОбновление Тогда
		// Выполняется только в режиме сервиса, когда все обработчики текущего порядка выполнены.
		// Включается автоматически при переключении порядка.
...
Процедура ОчиститьПрогрессОбработкиЗаИнтервалыПредыдущегоДня()
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		...
		МенеджерЗаписи.ИмяОбработчика = СтрокаТаблицы.ИмяОбработчика;
		МенеджерЗаписи.ИнтервалЧас = СтрокаТаблицы.ИнтервалЧас;
		МенеджерЗаписи.КлючЗаписи = СтрокаТаблицы.КлючЗаписи;
...
Функция НайтиОбработчикОбновления(КонтекстОбработчика, ПараметрыОбновления = Неопределено)
	...
	КонтекстОбработчика.СобиратьДлительностьВыполнения = СобиратьДлительностьВыполненияОбработчиков();
...
Процедура ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков, Знач ИмяБиблиотеки, ТекстОшибок)
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Последовательно"
		И Обработчик.Многопоточный Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У отложенного обработчика ""%1"" включено недопустимое свойство ""Многопоточный"",
...
Процедура ПроверитьУникальностьИдентификаторовОтложенныхОбработчиков(ИтерацииОбновления)
	...
	ТекстСообщения = НСтр("ru = 'У некоторых отложенных обработчиков обновления совпадают уникальные идентификаторы:'");
...
Процедура ДобавитьОтложенныеОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям, ГруппаОбновления, ТекстОшибок, ТекущийРежимВыполнения = "")
	...
	Для Каждого СтрокаВерсия Из ОбработчикиПоВерсиям Цикл
		...
		Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
			ПроверитьСвойстваОтложенногоОбработчика(Обработчик, ТекущийРежимВыполнения, ИмяБиблиотеки, ТекстОшибок);
			Если ТекущийРежимВыполнения = "Параллельно" Тогда
...
Функция ОбработчикДляВыполнения(Обработчики, ГруппыОбработчиковИЗависимость, СведенияОбОбновлении)
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Если ЭтоОбработчикОчисткиУстаревшихДанных(Обработчик)
		   И Обработчики.Количество() > 1 Тогда
			Продолжить;
		КонецЕсли;
...
#Область МеханизмМногопоточногоОбновления
	...
	Если Не Описание.ВыполняетсяПоискПорции Тогда
		...
		Попытка
			УстановитьСвойствоОбработчика(ИмяОбработчика,
				...
				Новый ХранилищеЗначения(Описание, Новый СжатиеДанных(9)));
			СохранитьПотокОбновления(Поток);
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
...
Процедура ВыполнитьОтложенныйОбработчик(КонтекстОбработчика, АдресРезультата) Экспорт
	...
	Попытка
		...
		ВыполнитьПроцедуру(КонтекстОбработчика.ИмяОбработчика, КонтекстОбработчика.Параметры, "ОтложеннаяОбработка");
...
Процедура РассчитатьВремяВыполненияПроцедурыОбработчика(КонтекстОбработчика, ИмяОбработчика)
	...
	Если Не КонтекстОбработчика.СобиратьДлительностьВыполнения Тогда
		Возврат;
	КонецЕсли;
...
Функция НовыйКонтекстОбработчика()
	...
	КонтекстОбработчика.Вставить("СобиратьДлительностьВыполнения", Ложь);
...
#Область МеханизмРаботыСПотоками
...
Процедура СохранитьПотокОбновления(Поток)
	...
	Запись.ПараметрыПроцедуры = Новый ХранилищеЗначения(Поток.ПараметрыПроцедуры, Новый СжатиеДанных(9));
	Запись.ПараметрыПроцедурыЗавершения = Новый ХранилищеЗначения(Поток.ПараметрыПроцедурыЗавершения, Новый СжатиеДанных(9));
...
Процедура ВыполнитьПоток(Группы, Поток, ИдентификаторФормы = Неопределено)
	...
	Если Не ПустаяСтрока(ОписаниеПотока.Процедура) И Поток.ПараметрыПроцедуры <> Неопределено Тогда
		...
		ИначеЕсли Статус <> "Выполняется" И Статус <> "Выполнено" Тогда
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(РезультатЗапуска.ИнформацияОбОшибке);
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, РезультатЗапуска.ИнформацияОбОшибке);
		КонецЕсли;
	КонецЕсли;
	...
	Пока Индекс >= 0 Цикл
		...
		Если ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
			...
			Исключение
				...
				Если ПустаяСтрока(ОписаниеПотока.ПриАварийномЗавершении) Тогда
					ВызватьИсключение;
				КонецЕсли;
...
#Область МеханизмМногопоточногоВыполненияОбработчикаОбновления
...
Процедура НайтиПорциюДляОбновления(ПараметрыПоиска, АдресРезультата) Экспорт
	...
	ПроверитьПараметрыВыборки(ПараметрыВыборки, ПараметрыПоиска.КонтекстОбработчика.ИмяОбработчика);
	...
	УпорядочиваниеПоВнешнимТаблицам = ЕстьУпорядочиваниеПоВнешнимТаблицам(ПоляУпорядочивания);
	
	Если УпорядочиваниеПоВнешнимТаблицам Тогда
		Если ОбщегоНазначения.РежимОтладки() Тогда
			ТекстСообщения = НСтр(
				"ru = 'Поля упорядочивания обработчиков обновлений не могут содержать обращения через точку.'");
			ВызватьИсключение(ТекстСообщения, КатегорияОшибки.ОшибкаКонфигурации);
		Иначе
			ТекстСообщения = НСтр(
				"ru = 'Не рекомендуется использовать обращение через точку в полях упорядочивания обработчиков обновлений.'");
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыВыборки.ОптимизироватьВыборкуПоСтраницам = Не УпорядочиваниеПоВнешнимТаблицам;
	...
	Если Не ПоискСНачала(ПараметрыПоиска) И Не ОбработкаПорции(ПараметрыПоиска) Тогда
...
//
Процедура ПроверитьПараметрыВыборки(ПараметрыВыборки, ПроцедураЗаполненияДанныхОбновления)
	...
	Если Не ИзвестныйСпособ Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Укажите способ выборки %1 в процедуре регистрации данных к обновлению %2.
			|Указан неизвестный способ выборки ""%3"".'"),
			"Параметры.ПараметрыВыборки.СпособВыборки", ПроцедураЗаполненияДанныхОбновления, СпособВыборки);
		ВызватьИсключение(ТекстСообщения, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	...
	Если Не ТаблицыУказаны Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Укажите обрабатываемые таблицы в процедуре регистрации данных к обновлению %1.
			|Указывается в ""%2"" и/или ""%3"".'"),
			ПроцедураЗаполненияДанныхОбновления, "Параметры.ПараметрыВыборки.ПолныеИменаОбъектов", 
...
Функция ВыбратьДанныеПорции(ПараметрыВыборки, Очередь, СсылочныйОбъект, ТабличныйОбъект)
	...
	СпособВыборки = ПараметрыВыборки.СпособВыборки;
	ПараметрыВыборки.Вставить("АвтоматическаяВыборкаПорции", Истина);
...
// Определить, обрабатывается порция данных или страница данных.
//
// Параметры:
//  ПараметрыПоиска - см. НовыеПараметрыПоискаПорции
//
// Возвращаемое значение:
//  Булево - Истина, если обрабатывается порция данных, иначе идет обработка страницы.
//
Функция ОбработкаПорции(ПараметрыПоиска)
	
	Возврат ПараметрыПоиска.ПоследняяВыбраннаяЗапись = Неопределено
...
// Определить, обрабатывается ли первая страница данных.
//
// Параметры:
//  ПараметрыПоиска - см. НовыеПараметрыПоискаПорции
//
// Возвращаемое значение:
//  Булево - Истина, если обрабатывается первая страница данных, иначе идет обработка последующей страницы
//           или порции.
//
Функция ПоискСНачала(ПараметрыПоиска)
...
Процедура ЗавершитьПоискПорцииДляОбновления(ПараметрыПоиска, АдресРезультата) Экспорт
	...
	УстановитьСвойствоОбработчика(ОбработчикОбновления.ИмяОбработчика,
		"ОбрабатываемыеДанные",
		Новый ХранилищеЗначения(ОписаниеПроцедурыЗаполнения, Новый СжатиеДанных(9)));
...
#Область НачальноеЗаполнениеИнформационнойБазы
...
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры, ОбъектМетаданных = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		ОбъектыСПредопределеннымиЭлементами = Новый Соответствие();
		ОбъектыСПредопределеннымиЭлементами.Вставить(ОбъектМетаданных, ОбъектМетаданных.ПолноеИмя());
	Иначе
	...
	Для каждого ОбъектСПредопределеннымиЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		ОбъектМетаданныхСЭлементами = ОбъектСПредопределеннымиЭлементами.Ключ;
		ПолноеИмяОбъектаМетаданных  = ОбъектСПредопределеннымиЭлементами.Значение;
...
Процедура ЗаполнитьРеквизитыНачальнымиДанными(ЗаполняемыйЭлемент, Контекст, НастройкиЗаполнения)
	
	ИмяКлючевогоРеквизита = ИмяКлючевогоРеквизита(Контекст.НастройкиПредопределенныхЭлементов);
	
	Если ИмяКлючевогоРеквизита = "Ссылка" Тогда
		Если ЗаполняемыйЭлемент.ЭтоНовый() Тогда
			ЗначениеКлючаОбъекта=ЗаполняемыйЭлемент.ПолучитьСсылкуНового();
		Иначе
			ЗначениеКлючаОбъекта=ЗаполняемыйЭлемент.Ссылка;
		КонецЕсли;
	Иначе
		...
		Если Не ЗначениеЗаполнено(ЗначениеКлючаОбъекта) Тогда
			...
			Если Не Контекст.НастройкиПредопределенныхЭлементов.ЕстьКолонкаИмяПредопределенныхДанных Тогда 
				Возврат;
			КонецЕсли;
	...
	СтрокаТаблицы = Контекст.ПредопределенныеДанные.Найти(ЗначениеКлючаОбъекта, ИмяКлючевогоРеквизита);
	Если СтрокаТаблицы = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	КромеРеквизитов = Контекст.РеквизитыИсключения;
	
	Если Контекст.ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа Тогда
		
		КромеРеквизитов = Контекст.РеквизитыИсключения + ?(ПустаяСтрока(
			Контекст.РеквизитыИсключения), "", ",") + СтрСоединить(
			Контекст.РеквизитыСЭлементами, ",");
	КонецЕсли;
	...
	Если Не НастройкиЗаполнения.ОбновитьТолькоМультиязычныеСтроки Тогда
		...
		Если Контекст.ПоддерживаетсяИерархия И ЗначениеЗаполнено(СтрокаТаблицы.Родитель) Тогда
			...
			ИначеЕсли ТипЗнч(СтрокаТаблицы.Родитель) = Тип("Строка") Тогда 
				...
				СуществующиеЭлементы = СуществующиеПоставляемыеЭлементы(Контекст.ПредопределенныеДанные,
					Контекст.НастройкиПредопределенныхЭлементов, МенеджерОбъекта, МетаданныеОбъекта);
				ЗаполняемыйЭлемент.Родитель = СуществующиеЭлементы[СтрокаТаблицы.Родитель];
		...
		Если НЕ (Контекст.ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа) Тогда
			Для Каждого ИмяТабличнойЧасти Из Контекст.ТабличныеЧасти Цикл
				ЗаполняемыйЭлемент[ИмяТабличнойЧасти].Загрузить(СтрокаТаблицы[ИмяТабличнойЧасти]);
			КонецЦикла;
		КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		ПоляИсключения = СтрРазделитьСокрЛП(КромеРеквизитов, ",", Ложь);
		МодульМультиязычностьСервер.ЗаполнитьЭлементыМультиязычнымиНачальнымиДанными(ЗаполняемыйЭлемент, 
			СтрокаТаблицы, Контекст, ПоляИсключения);
	КонецЕсли;
	
	Если Контекст.НастройкиПредопределенныхЭлементов.ПереопределяемыеНастройки.ПриНачальномЗаполненииЭлемента Тогда
		МенеджерОбъекта.ПриНачальномЗаполненииЭлемента(ЗаполняемыйЭлемент, СтрокаТаблицы, Контекст.НастройкиПредопределенныхЭлементов.ПереопределяемыеНастройки.ДополнительныеПараметры);
	КонецЕсли;
...
Функция ОбновитьЭлементыПредопределенныхЭлементов(СсылкиНаОбъекты, МетаданныеОбъекта, НастройкиЗаполнения)
	...
	СодержитОтредактированныеПредопределенныеРеквизиты = СодержитОтредактированныеПредопределенныеРеквизиты(ПолноеИмя);
	...
	Пока СсылкиНаОбъекты.Следующий() Цикл
		...
		Попытка
			...
			БлокировкаДанных = Новый БлокировкаДанных;
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(ПолноеИмя);
			...
			Если ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа Тогда
				КромеРеквизитов = РеквизитыИсключения + ?(ПустаяСтрока(РеквизитыИсключения), "", ",") + СтрСоединить(ПараметрыДляЗаполненияОбъекта.РеквизитыСЭлементами, ",");
			КонецЕсли;
			...
			Иначе
				...
				Если СодержитОтредактированныеПредопределенныеРеквизиты Тогда
					ОтредактированныеРеквизиты = ОтредактированныеРеквизиты(ЗаполняемыйЭлемент, ПолноеИмя);
					
					Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
						МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
						МодульМультиязычностьСервер.УдалитьМультиязычныеРеквизиты(ОтредактированныеРеквизиты);
					КонецЕсли;
					
					ОтредактированныеРеквизитыСтрокой = СтрСоединить(ОтредактированныеРеквизиты, ",");
				
					Если ЗначениеЗаполнено(ОтредактированныеРеквизитыСтрокой) Тогда
						КромеРеквизитов = ?(ЗначениеЗаполнено(КромеРеквизитов), "," , "") + ОтредактированныеРеквизитыСтрокой;
					КонецЕсли;
				КонецЕсли;
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
				МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
				МодульМультиязычностьСервер.ЗаполнитьЭлементыМультиязычнымиНачальнымиДанными(ЗаполняемыйЭлемент,
					СтрокаТаблицы, ПараметрыДляЗаполненияОбъекта, ОтредактированныеРеквизиты(ЗаполняемыйЭлемент, ПолноеИмя));
			КонецЕсли;
...
Функция СуществующиеПоставляемыеЭлементы(ПредопределенныеДанные, НастройкиПредопределенныхЭлементов, Знач МенеджерОбъекта, МетаданныеОбъекта)
	...
	Если СтрСравнить(ИмяКлючевогоРеквизита, "ИмяПредопределенныхДанных") <> 0 Тогда
		...
		Для Каждого ПредопределенныйЭлемент Из ПредопределенныеДанные Цикл
			...
			Если ЕстьКолонкаИмяПредопределенныхДанных И ЗначениеЗаполнено(
				ПредопределенныйЭлемент.ИмяПредопределенныхДанных) И ИменаПредопределенных.Найти(
				ПредопределенныйЭлемент.ИмяПредопределенныхДанных) <> Неопределено Тогда
				Результат.Вставить(ПредопределенныйЭлемент.ИмяПредопределенныхДанных,
					МенеджерОбъекта[ПредопределенныйЭлемент.ИмяПредопределенныхДанных]);
					Продолжить;
			КонецЕсли;
...
Процедура ОчисткаУстаревшихДанных() Экспорт
	...
	Иначе
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Очистка устаревших данных выполняется
...
Процедура НастроитьЗаданиеОчисткаУстаревшихДанныхБезПопытки(Включить)
	...
	Количество = ?(ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных(), 0, 1);
	Если НайденныеЗадания.Количество() > Количество Тогда
		Для Каждого НайденноеЗадание Из НайденныеЗадания Цикл
			Если НайденноеЗадание = НайденныеЗадания[0] И Количество > 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ОчиститьУстаревшиеДанныеПолностью(Параметры) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Очистка устаревших данных (отложенно)'");
	...
	НаборПараметровПроцедуры.Вставить("Контекст",
		Новый Структура("ЧиститьУдаляемые, ОтложенноеОбновление", Ложь, Истина));
...
Процедура УстаревшиеДанныеПриЗапросеПорцийВФоне(НовыеПорции, Контекст) Экспорт
	...
	Если Не Контекст.Свойство("ОчищаемыеТаблицы") Тогда
		Если Не Контекст.Свойство("ЭтоРегламентноеЗадание")
		   И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
	...
	Если Контекст.Свойство("Процент")
	   И Не Контекст.Свойство("ОтложенноеОбновление") Тогда
...
Процедура УстановитьПорядокОтложенногоОбновления() Экспорт
	...
	ПрогрессОперативногоОбновления = ОбновлениеИнформационнойБазы.ПрогрессОбновленияОбластейДанных("Оперативное");
	ПрогрессОбновления = ОбновлениеИнформационнойБазы.ПрогрессОбновленияОбластейДанных("Отложенное");
	...
	Если ЗначениеЗаполнено(ПрогрессОперативногоОбновления.Проблемы) Тогда
		// Области, где оперативное обновление завершилось с ошибкой, не останавливают отложенное обновление.
		ПрогрессОбновления.ОбластиОжидает = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
			ПрогрессОбновления.ОбластиОжидает, ПрогрессОперативногоОбновления.ОбластиСПроблемами);
		ПрогрессОбновления.Ожидают = ПрогрессОбновления.ОбластиОжидает.Количество();
	КонецЕсли;
	
	ЕстьОжидающие = ПрогрессОбновления.Ожидают <> 0;
	...
	Запрос.УстановитьПараметр("ПредельноеЧислоПопыток", 3);
	Запрос.УстановитьПараметр("Порядок", ПорядокОбрабатываемыхДанных);
	Запрос.УстановитьПараметр("Статусы", Статусы);
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОбработчикиОбновления.ОбластьДанныхВспомогательныеДанные КАК Область
		|ИЗ
		|	РегистрСведений.ОбработчикиОбновления КАК ОбработчикиОбновления
		|ГДЕ
		|	ОбработчикиОбновления.РежимВыполнения = &РежимВыполнения
		...
		|	И ОбработчикиОбновления.ЧислоПопыток <= &ПредельноеЧислоПопыток
	...
	НеобновленныеОбласти = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Область");
	
	ОтключитьЗадание = Ложь;
	ПорядокИзменен   = Ложь;
	Если НеобновленныеОбласти.Количество() = 0 И Не ЕстьОжидающие Тогда
	...
	Если ПорядокИзменен Тогда
		ПриВключенииОтложенногоОбновления(Истина);
	Иначе
		// Проверка, что задания для необновленных областей включены.
		ОтборЗаданий = Новый Структура;
		ОтборЗаданий.Вставить("Метаданные", Метаданные.РегламентныеЗадания.ОтложенноеОбновлениеИБ);
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ЗаданияОчереди", Задания);
		Запрос.УстановитьПараметр("НеобновленныеОбласти", НеобновленныеОбласти);
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ЗаданияОчереди.Идентификатор КАК Идентификатор,
			|	ЗаданияОчереди.ОбластьДанных КАК ОбластьДанных
			|ПОМЕСТИТЬ ВТЗаданияОчереди
			|ИЗ
			|	&ЗаданияОчереди КАК ЗаданияОчереди
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	НезавершенныеЗаданияОчереди.Идентификатор КАК Идентификатор
			|ИЗ
			|	ВТЗаданияОчереди КАК НезавершенныеЗаданияОчереди
			|ГДЕ
			|	НезавершенныеЗаданияОчереди.ОбластьДанных В(&НеобновленныеОбласти)";
		ЗаданияДляВключения = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Идентификатор");
		Для Каждого Задание Из ЗаданияДляВключения Цикл
	...
	Если ТранзакцияАктивнаНаНачалоВыполнения Тогда
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
				...
				ВызватьИсключение(Комментарий, КатегорияОшибки.ОшибкаКонфигурации);
			КонецПопытки;
		Иначе
			ШаблонКомментария = НСтр("ru = 'Ошибка выполнения обработчика обновления %1:
...
Процедура ПроверитьСвойстваОбработчиков(ИтерацияОбновления)
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		ВызватьИсключение(ОписаниеОшибки, КатегорияОшибки.ОшибкаКонфигурации);
...
Функция ОтметкаНачалаОбновления()
	...
	Иначе
		СеансыИнформационнойБазы = Неопределено;
	КонецЕсли;
	...
	Попытка
		...
		Если Не СеансыСовпадают Тогда
			СеансОбновленияАктивен = СеансАктивен(СохраненныеПараметры, СеансыИнформационнойБазы);
...
Функция СеансАктивен(ОписаниеСеанса, СеансыИнформационнойБазы)
	Если ОписаниеСеанса = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если СеансыИнформационнойБазы = Неопределено Тогда
		СеансыИнформационнойБазы = ПолучитьСеансыИнформационнойБазы();
	КонецЕсли;
...
Функция ЗначениеПеречисленияПоИмени(ИмяЗначения, МетаданныеПеречисления) Экспорт
	...
	Если Результат = Неопределено Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ИмяЗначения, МетаданныеПеречисления, СтрСоединить(ДоступныеЗначения, ", "));
		ВызватьИсключение(ТекстСообщения, КатегорияОшибки.ОшибкаКонфигурации); 
	КонецЕсли;
...
Функция РазделыОписанияИзменений() Экспорт
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		...
		Для каждого Область Из Макет.Области Цикл
			...
			Версия = ""
				+ РазрядыВерсииСтроками[0] + "."
				+ РазрядыВерсииСтроками[1] + "."
				+ РазрядыВерсииСтроками[2] + "."
				+ РазрядыВерсииСтроками[3];
...
#Область ВспомогательныеПроцедурыИФункцииОтложенногоОбновления
	...
	Если ТипЗнч(ОбработчикОбновления) = Тип("СтрокаТаблицыЗначений") Тогда
		...
		Попытка
			...
			Если ОбработчикОбновления.Многопоточный Тогда
				...
				ПроверитьПараметрыВыборки(ПараметрыВыборки, ОбработчикОбновления.ПроцедураЗаполненияДанныхОбновления);
				ПараметрыВыборки.МаксимумВыборки = ОбновлениеИнформационнойБазы.МаксимальноеКоличествоЗаписейВВыборке();
				ПараметрыПоиска = НовыеПараметрыПоискаПорции();
...
Процедура ЗавершитьВыполнениеОтложенногоОбработчикаОбновления(КонтекстОбработчика)
	...
	ИначеЕсли ОбработчикОбновления.Статус = Перечисления.СтатусыОбработчиковОбновления.Выполняется Тогда
		...
		Если ОбработчикОбновления.Приоритет = "ЖелательноБыстрее" Тогда
			...
			УстанавливаемыеСвойства.Вставить("СтатистикаВыполнения", Новый ХранилищеЗначения(СтатистикаВыполнения, Новый СжатиеДанных(9)));
		КонецЕсли;
...
Функция НовыеСведенияОбОбновлении(СтарыеСведения = Неопределено)
	...
	СведенияОбОбновлении.Вставить("ВерсияПроверкиПатчей");
	СведенияОбОбновлении.Вставить("ЗависимостьГруппОбработчиков", Новый Соответствие);
...
Процедура ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика,
	...
	Попытка
		...
		МаксимальноеКоличествоЗапусков        = ЧислоПотоковОбновления * 100000;
		...
		Свойства.Вставить("СтатистикаВыполнения", Новый ХранилищеЗначения(СтатистикаВыполнения, Новый СжатиеДанных(9)));
		УстановитьСвойстваОбработчика(ОбработчикОбновления.ИмяОбработчика, Свойства);
		
		ОбрабатываемыеДанные = ОбработчикОбновления.ОбрабатываемыеДанные.Получить();
		Если ОбработчикОбновления.Многопоточный Тогда
			ПроверитьПараметрыВыборки(ОбрабатываемыеДанные.ПараметрыВыборки, ОбработчикОбновления.ПроцедураЗаполненияДанныхОбновления);
		КонецЕсли;
	...
	Попытка
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим И Не КонтекстОбработчика.ПропуститьПроверкуОбработанныхДанных Тогда
			...
			Если ЗначениеЗаполнено(ОбработанныеТаблицыРегистраторов) Тогда
				...
				УстанавливаемыеСвойства.Вставить("ОбрабатываемыеДанные", Новый ХранилищеЗначения(ОписаниеПроцедурыЗаполнения, Новый СжатиеДанных(9)));
...
Процедура ЗаполнитьБлокируемыеОбъекты(СтрокаВерсия, СведенияОБлокируемыхОбъектах)
	...
	Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
		...
		Если ЗначениеЗаполнено(ПроцедураПроверки) И ЗначениеЗаполнено(БлокируемыеОбъекты) Тогда
			Если Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Некритичный Тогда
				// Некритичные обработчики работу с объектом не должны блокировать.
				Продолжить;
			КонецЕсли;
		...
		ИначеЕсли ЗначениеЗаполнено(БлокируемыеОбъекты) И Не ЗначениеЗаполнено(ПроцедураПроверки) Тогда
			ТекстИсключения =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У отложенного обработчика обновления ""%1""
					|заполнен список блокируемых объектов, но не задано свойство ""%2"".'"),
					Обработчик.ИмяОбработчика, "ПроцедураПроверки");
			ВызватьИсключение(ТекстИсключения, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеОбрабатываемыхДанных, АдресРезультата) Экспорт
	...
	Попытка
		НачалоРегистрации = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ВыполнитьПроцедуру(ОписаниеОбрабатываемыхДанных.ПроцедураЗаполнения, ПараметрыОтметкиКОбработке, "РегистрацияДанных");
		КонецРегистрации  = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Исключение
...
Процедура УстановитьПараметрыОбработчикаОбновления(ОбработчикОбновления, Отложенно = Ложь, Параллельно = Ложь)
	...
	ПараметрыОбработчикаОбновления.Многопоточный = ОбработчикОбновления.Многопоточный;
	Если Параллельно Тогда
		ОбрабатываемыеДанные = ОбработчикОбновления.ОбрабатываемыеДанные.Получить();
...
Функция НовыеПараметрыОбработчикаОбновления() Экспорт
	...
	ПараметрыОбработчикаОбновления.Вставить("Многопоточный", Ложь);
...
Функция ВидСравненияСтрокой(Знач УсловиеСравнения, ИмяОбработчика)
	...
	ИначеЕсли УсловиеСравнения = ВидСравнения.НеРавно Тогда
		УсловиеСравнения = "<>";
	Иначе
		ТекстОшибки = НСтр("ru = 'Указан недопустимый вид сравнения ""%1"" для фильтра актуальных данных
...
Функция МинимальнаяОчередьОтложеннойОбработки()
	...
	Выполняемые.Добавить(Перечисления.ПорядокОбработчиковОбновления.Критичный);
	Если ТекущийПорядок = Перечисления.ПорядокОбработчиковОбновления.Обычный Тогда
		Выполняемые.Добавить(Перечисления.ПорядокОбработчиковОбновления.Обычный);
	ИначеЕсли ТекущийПорядок = Перечисления.ПорядокОбработчиковОбновления.Некритичный Тогда
		Выполняемые.Добавить(Перечисления.ПорядокОбработчиковОбновления.Обычный);
		Выполняемые.Добавить(Перечисления.ПорядокОбработчиковОбновления.Некритичный);
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("РежимВыполнения", Перечисления.РежимыВыполненияОбработчиков.Отложенно);
	Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыОбработчиковОбновления.Выполнен);
	Запрос.УстановитьПараметр("Выполняемые", Выполняемые);
	Запрос.УстановитьПараметр("РежимВыполненияОтложенногоОбработчика", Перечисления.РежимыВыполненияОтложенныхОбработчиков.Параллельно);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ОбработчикиОбновления.ОчередьОтложеннойОбработки КАК Очередь,
...
Процедура ДобавитьИнформациюОбОшибкеВОбработчике(ИмяОбработчика) Экспорт
	...
	Попытка
		...
		МенеджерЗаписи.СтатистикаВыполнения = Новый ХранилищеЗначения(СтатистикаВыполнения, Новый СжатиеДанных(9));
...
Функция ЧистаяДлительностьВыполненияОбработчика(ОбработчикОбновления, СтатистикаВыполнения)
	...
	Если ОбработчикОбновления.Многопоточный
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДлительностей Цикл
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.Начало)
				Или Не ЗначениеЗаполнено(СтрокаТаблицы.Конец) Тогда
				Продолжить;
			КонецЕсли;
			Если НачалоТекущегоИнтервала = Неопределено Тогда
				НачалоТекущегоИнтервала = СтрокаТаблицы.Начало;
				КонецТекущегоИнтервала = СтрокаТаблицы.Конец;
				Продолжить;
			КонецЕсли;
			
			Если СтрокаТаблицы.Конец < КонецТекущегоИнтервала Тогда
				Продолжить;
			КонецЕсли;
			
			Если СтрокаТаблицы.Начало < КонецТекущегоИнтервала Тогда
				Если СтрокаТаблицы.Конец > КонецТекущегоИнтервала Тогда
					КонецТекущегоИнтервала = СтрокаТаблицы.Конец;
				КонецЕсли;
				Продолжить;
...
Функция СобиратьДлительностьВыполненияОбработчиков()
	
	ПараметрЗапускаКлиента = СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере().Получить("ПараметрЗапуска");
	Возврат СтрНайти(НРег(ПараметрЗапускаКлиента), НРег("СобиратьДлительностьВыполненияОбработчиков")) > 0;
	
КонецФункции

// Механизм начального заполнения предопределенных элементов

Функция ОтредактированныеРеквизиты(ЗаполняемыйЭлемент, ПолноеИмяОбъектаМетаданных)
	
	ОтредактированныеРеквизиты = Новый Массив;
	
	Если СодержитОтредактированныеПредопределенныеРеквизиты(ПолноеИмяОбъектаМетаданных) Тогда
		ОтредактированныеРеквизиты = СтрРазделить(ЗаполняемыйЭлемент.ОтредактированныеПредопределенныеРеквизиты, ",", Ложь);
	КонецЕсли;
	
	Возврат ОтредактированныеРеквизиты;
	
КонецФункции

Функция ДоступныеДляИзмененияРеквизитыОбъекта(ОбъектМетаданных, ИмяКлючевогоРеквизита)

	РеквизитыОбъекта = Новый Соответствие;

	РеквизитыИсключение = Новый Соответствие;
	РеквизитыИсключение.Вставить("Ссылка", Истина);
	РеквизитыИсключение.Вставить("ПометкаУдаления", Истина);
	РеквизитыИсключение.Вставить("ЭтоГруппа", Истина);
	РеквизитыИсключение.Вставить("Предопределенный", Истина);
	РеквизитыИсключение.Вставить("ИмяПредопределенныхДанных", Истина);
	РеквизитыИсключение.Вставить("ИмяКлючевогоРеквизита", Истина);

	Для Каждого Реквизит Из ОбъектМетаданных.Реквизиты Цикл
		РеквизитыОбъекта.Вставить(Реквизит.Имя, Истина);
	КонецЦикла;

	Для Каждого Реквизит Из ОбъектМетаданных.СтандартныеРеквизиты Цикл
		Если РеквизитыИсключение[Реквизит.Имя] = Неопределено Тогда
			РеквизитыОбъекта.Вставить(Реквизит.Имя, Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановкаПараметровСеанса");
	Методы.Вставить("ЗаписатьПодтверждениеЛегальностиПолученияОбновлений");
	Методы.Вставить("ПустаяПроцедураРегистрации");
	Методы.Вставить("НачальноеЗаполнениеПредопределенныхДанных");
	Методы.Вставить("УстановитьКлючРегламентногоЗадания");
	Методы.Вставить("ОчиститьУстаревшиеДанныеПолностью");
	Методы.Вставить("ПриАварийномЗавершенииПотокаОтложенногоОбработчика");
	Методы.Вставить("ПриОтменеПотокаОтложенногоОбработчика");
	Методы.Вставить("ПриАварийномЗавершенииПотокаПоискаПорцииДляОбновления");
	Методы.Вставить("ПриОтменеПотокаПоискаПорцииДляОбновления");
	Методы.Вставить("ОбработчикПроверкиВеденияУчета");
	...
	Методы.Вставить("ЗавершитьПоискПорцииДляОбновления", Истина);
	Методы.Вставить("УстаревшиеДанныеПриЗапросеПорцийВФоне", Истина);
	Методы.Вставить("УстаревшиеДанныеПриОчисткеПорцииВФоне", Истина);
	Методы.Вставить("ОбновлениеПодОграниченнымиПравами", Истина);
	Методы.Вставить("ЗапускРегистрацииДанныхОтложенногоОбновления", Истина);
	Методы.Вставить("СформироватьСписокУстаревшихДанныхВФоне", Истина);
	Методы.Вставить("ОчиститьУстаревшиеДанныеВФоне", Истина);
	Методы.Вставить("ВыполнитьОбновлениеИнформационнойБазыВФоне", Истина);
	Методы.Вставить("ЗагрузитьИУстановитьИсправления", Истина);
