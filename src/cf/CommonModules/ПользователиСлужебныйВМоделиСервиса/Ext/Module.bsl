...
//
//  МинимальныеДействия - Неопределено - возвращаемое значение,
//   содержащее минимальные типа НовыеДействияСПользователемСервиса,
//   в том числе, если возникает ошибка взаимодействия с сервисом.
//  
// Возвращаемое значение:
//   см. НовыеДействияСПользователемСервиса
//
Функция ПолучитьДействияСПользователемСервиса(Знач Пользователь = Неопределено,
			МинимальныеДействия = Неопределено) Экспорт
	
	МинимальныеДействия = НовыеДействияСПользователемСервиса();
...
Процедура СообщитьИзмененЗапускПриложения(Пользователь, ПользовательИБ, СтарыйПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность")
	 Или Не ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОбменСообщениями") Тогда
		Возврат;
	КонецЕсли;
	
	ВозможенЗапуск = ВозможенЗапускПриложения(ПользовательИБ);
	Если СтарыйПользовательИБ <> Неопределено
	   И ВозможенЗапуск = ВозможенЗапускПриложения(СтарыйПользовательИБ)
	...
	ИнфоПользователя.Вставить("СохраняемоеЗначениеПароля", ПользовательИБ.СохраняемоеЗначениеПароля);
	ИнфоПользователя.Вставить("Язык", ПолучитьКодЯзыка(ПользовательИБ.Язык));
	ИнфоПользователя.Вставить("ПолныеПрава", ВозможенЗапуск И ПользовательИБ.Роли.Содержит(Метаданные.Роли.ПолныеПрава));
	ИнфоПользователя.Вставить("ВходВПрограммуРазрешен", Пользователи.ВходВПрограммуРазрешен(ПользовательИБ));
	
	НачатьТранзакцию();
	Попытка
		МодульОбменСообщениями.ОтправитьСообщениеСейчас("UserHandler/LaunchSwitch", ОбщегоНазначения.ЗначениеВСтрокуXML(
...
Процедура ПриЗавершенииОбработкиПользователяИБ(ПользовательОбъект, ПараметрыОбработки, ОбновлятьРоли) Экспорт
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи")
	   ...
	   И ОписаниеПользователяИБ.Свойство("Действие")
...
#Область ОбработчикиСобытийПодсистемыБСПРаботаВМоделиСервиса
...
Процедура ОбновитьИдентификаторыПользователейИБ(Знач СоответствиеИдентификаторов)
	...
	|	Пользователи.ИдентификаторПользователяИБ КАК ИдентификаторПользователяСервиса
	|ИЗ
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			НовыйИдентификаторПользователяИБ = СоответствиеИдентификаторов[Выборка.ИдентификаторПользователяИБ];
			СоответствиеИдентификаторов.Удалить(Выборка.ИдентификаторПользователяИБ);
			Если Выборка.ИдентификаторПользователяИБ = ПустойИдентификатор
			   И Выборка.ИдентификаторПользователяСервиса = ПустойИдентификатор
			   И Не ЗначениеЗаполнено(НовыйИдентификаторПользователяИБ) Тогда
				Продолжить;
			КонецЕсли;
			ПользовательОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ПользовательОбъект.ИдентификаторПользователяСервиса = Неопределено;
			ПользовательОбъект.ИдентификаторПользователяИБ = НовыйИдентификаторПользователяИБ;
