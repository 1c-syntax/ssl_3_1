...
Процедура ПоказатьПанельОтчетов(ПутьКПодсистеме, ПараметрыВыполненияКоманды) Экспорт
	ФормаПараметры = Новый Структура("ПутьКПодсистеме, ИскатьВоВсехРазделах", ПутьКПодсистеме, Истина);
...
Процедура ОбновитьОткрытыеФормы(Знач КлючВарианта = "", Знач Источник = Неопределено) Экспорт
	...
	ПараметрыОбновленияФормы.КлючВарианта = КлючВарианта;
	Оповестить(ИмяСобытияИзменениеВарианта(), ПараметрыОбновленияФормы, Источник);
...
Функция ИмяСобытияИзменениеВарианта() Экспорт
	Возврат "Изменение_ВариантыОтчетов";
КонецФункции
...
Процедура ОформитьПользователейВариантаОтчета(Форма, СбрасыватьПризнакИспользования = Истина) Экспорт 
	...
	Для Каждого СтрокаТаблицы Из Форма.ПользователиВарианта Цикл 
		Если СтрокаТаблицы.Значение = ВариантАвтор И ВариантАвтор <> Неопределено Тогда
			ПризнакАвтора = "[ЭтоАвторВариантаОтчета]";
		Иначе
			ПризнакАвтора = "";
			КоличествоПомеченных = КоличествоПомеченных + Булево(СтрокаТаблицы.Пометка);
		КонецЕсли;
		Если СтрокаТаблицы.Представление <> ПризнакАвтора Тогда
			СтрокаТаблицы.Представление = ПризнакАвтора;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьВариантОтчетаИзФайла(ОписаниеФайла, СвойстваВариантаОтчетаОснования) 
	...
	Если СвойстваВариантаОтчетаОснования.ИмяОтчета = СвойстваВариантаОтчета.ИмяОтчета Тогда 
		...
		ОбновитьОткрытыеФормы(СвойстваВариантаОтчета.КлючВарианта);
