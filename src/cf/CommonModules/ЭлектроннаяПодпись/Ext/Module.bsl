...
Функция ПодписиОбъекта(Объект, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ДополнительныеПараметры.ВозвращатьСертификатыИзПодписей И ЗначениеЗаполнено(СвойстваПодписи.Подпись) Тогда
			СвойстваПодписи.Сертификат = ЭлектроннаяПодписьСлужебный.СертификатИзДвоичныхДанныхПодписи(СвойстваПодписи.Подпись);
			Если СвойстваПодписи.Сертификат = Неопределено И ЭлектроннаяПодписьСлужебный.КриптографияДоступна() Тогда
...
Функция ШтампВизуализацииЭлектроннойПодписи(Знач Подпись, Знач ДатаПодписи = Неопределено, 
	...
	Иначе
		Подпись.ДатаПодписи = ?(ДатаПодписи = Неопределено, Подпись.ДатаПодписи, ДатаПодписи);
	КонецЕсли;
...
Функция ШтампыВизуализацииЭлектронныхПодписей(Знач ПодписанныйФайл, Знач ЭлектронныеПодписи = Неопределено,
	...
	Для Каждого Подпись Из ЭлектронныеПодписи Цикл
		...
		ПараметрыШтампа.Вставить("ДатаПодписи", Подпись.ДатаПодписи);
	...
	Индекс = 0;
	Для Каждого ПараметрыШтампа Из ПараметрыШтампов Цикл
		ИмяМакета = ?(Не ПустаяСтрока(ПараметрыШтампа.Доверенность), "ШтампРасширенный", "Штамп");
		Штамп = Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования.ПолучитьМакет(ИмяМакета);
	...
	Если ТипЗнч(ОписаниеШтампов) = Тип("Массив") Тогда
		...
		Для Каждого Штамп Из ОписаниеШтампов Цикл
			...
			Иначе
				...
				Для Каждого ОчереднаяОбласть Из Документ.Области Цикл
					ИмяОбласти = ОчереднаяОбласть.Имя; 
					Если СтрНачинаетсяС(ИмяОбласти, "Документ") И СтрНайти(ИмяОбласти, "Устарела") = 0 Тогда
						ОчереднаяОбласть.Имя = ИмяОбласти+"_Устарела_"+ИндексШтампа;
						Документ.Область().Имя = ИмяОбласти;
						Прервать;
					КонецЕсли;
				КонецЦикла;
	...
	Для Каждого СвойстваПодписи Из МассивСвойствПодписи Цикл
		...
		Иначе
			СообщениеЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отправлен на подписание: %1'"), НоваяЗапись.КомуВыданСертификат);
		КонецЕсли;
