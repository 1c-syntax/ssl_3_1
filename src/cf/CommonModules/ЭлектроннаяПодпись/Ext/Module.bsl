...
//
// Возвращаемое значение:
//  Массив - массив из ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи 
//
...
//  СвойстваПодписи - Строка - адрес временного хранилища, содержащий описанную ниже структуру.
//                  - Структура - см. ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи.
//  ОбновитьПоПорядковомуНомеру - Булево - подпись обновляется по порядковому номеру, т.к. двоичные данные
//                                 подписи изменились при усовершенствовании.
//
...
Процедура ДобавитьШтампыВТабличныйДокумент(Документ, ОписаниеШтампов, Размеры = Неопределено) Экспорт
	...
	Если ТипЗнч(ОписаниеШтампов) = Тип("Массив") Тогда
		...
		НомерШтампаВСтроке = 0;
		Если Документ.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт Тогда
			КоличествоШтамповПоШирине = Мин(3, ОписаниеШтампов.Количество());
		Иначе
			КоличествоШтамповПоШирине = Мин(2, ОписаниеШтампов.Количество());
		КонецЕсли;
		...
		Для Каждого Штамп Из ОписаниеШтампов Цикл
			...
			Иначе
				...
				Если НомерШтампаВСтроке = 1 Или НомерШтампаВСтроке > КоличествоШтамповПоШирине Тогда
					...
					ШиринаШтамповВСтроке = КоличествоШтамповПоШирине * Штамп.ШиринаТаблицы;
					Документ.Область(ВысотаНачало, ШиринаНачало, ВысотаКонец, ШиринаШтамповВСтроке).Разъединить();
					...
					Если Документ.ШиринаТаблицы > ШиринаШтамповВСтроке Тогда
						Документ.Область(ВысотаНачало, ШиринаШтамповВСтроке + 1, ВысотаКонец, Документ.ШиринаТаблицы).ШиринаКолонки = 0;
					КонецЕсли;
				КонецЕсли;
...
//   
Функция УсовершенствоватьПодпись(Подпись, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь, МенеджерКриптографии = Неопределено) Экспорт
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось прочитать данные подписи: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(ПараметрыПодписиКриптографии.СрокДействияПоследнейМеткиВремени)
	...
	Если РезультатПроверкиСертификата Тогда
		Если ЗначениеЗаполнено(ТипПодписи) И ЭлектроннаяПодписьСлужебныйКлиентСервер.ПодлежитУсовершенствованию(
			...
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось усовершенствовать подпись: %1'"), КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Возврат Результат;
			КонецПопытки;
		ИначеЕсли ДобавитьАрхивнуюМеткуВремени И (ПараметрыПодписиКриптографии.ТипПодписи
			...
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось добавить архивную метку времени в подпись: %1'"), КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Возврат Результат;
			КонецПопытки;
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось прочитать данные усовершенствованной подписи: %1'"), КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;

	ПараметрыПодписиКриптографии = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыПодписиКриптографии(
...
Функция УсовершенствоватьПодписьОбъекта(ПодписанныйОбъект, ПорядковыйНомер, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь,
			ИдентификаторФормы = Неопределено, МенеджерКриптографии = Неопределено) Экспорт
	...
	Результат = УсовершенствоватьПодпись(Подпись, ТипПодписи, ДобавитьАрхивнуюМеткуВремени, МенеджерКриптографии);
...
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, ОписаниеОшибки = Null, НаДату = Неопределено) Экспорт
	...
	ИначеЕсли МенеджерКриптографииДляПроверки = Неопределено
		...
		СвойстваПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваПодписиИзДвоичныхДанных(
			ПодписьДляПроверки, ЭлектроннаяПодписьСлужебный.ДобавкаВремени());
	...
	Иначе
		...
		Если ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись Тогда
			...
			Попытка
				КонтейнерПодписи = МенеджерКриптографииДляПроверки.ПолучитьКонтейнерПодписейКриптографии(ПодписьДляПроверки);
			Исключение
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При чтении данных подписи: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				Если ВызыватьИсключение Тогда
					ВызватьИсключение;
				КонецЕсли;
				Возврат Ложь;
			...
			ДатаПодписания = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДатаПодписиИзПараметровПодписи(ПараметрыПодписиКриптографии);
	...
	Возврат ЭлектроннаяПодписьСлужебный.ПроверитьСертификат(
		МенеджерКриптографииДляПроверки, Сертификат, ОписаниеОшибки, ДатаПодписания, ДополнительныеПараметры);
...
Функция ДоступностьСозданияЗаявления() Экспорт
	...
	ДоступностьСозданияЗаявления.Вставить("ДляРуководителейЮридическихЛиц", ЗаявлениеДоступно И МосковскоеВремя < '20220101');
	ДоступностьСозданияЗаявления.Вставить("ДляСотрудниковЮридическихЛиц", ЗаявлениеДоступно И МосковскоеВремя < '20230901');
	ДоступностьСозданияЗаявления.Вставить("ДляИндивидуальныхПредпринимателей", ЗаявлениеДоступно И МосковскоеВремя < '20220101');
...
//
Функция ПрочитатьДанныеПодписи(Подпись, ОписаниеОшибки = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваПодписиИзДвоичныхДанных.ДатаПодписания) Тогда
		Результат.ДатаПодписи = СвойстваПодписиИзДвоичныхДанных.ДатаПодписания;
	Иначе
		Результат.ДатаПодписи = ТекущаяДатаСеанса();
	КонецЕсли;
	
	МенеджерКриптографии = МенеджерКриптографии("ПолучениеСертификатов", ПоказатьОшибку, ОписаниеОшибки);
	Если МенеджерКриптографии = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Если Сертификаты.Количество() > 0 Тогда
		...
		Иначе
			...
			Попытка
				ДвоичныеДанныеСертификата = ЭлектроннаяПодписьСлужебныйВызовСервера.СертификатыПоПорядкуДоКорневого(
		...
		Результат.Отпечаток = Base64Строка(Сертификат.Отпечаток);
		Результат.КомуВыданСертификат = ПредставлениеСубъекта(Сертификат);
		Результат.Сертификат = ДвоичныеДанныеСертификата;
