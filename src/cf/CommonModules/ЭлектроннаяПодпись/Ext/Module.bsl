...
//
// Параметры:
//  Объект - ЛюбаяСсылка - ссылка на подписанный объект.
...
// 
// Параметры:
//  Объект - ЛюбаяСсылка - по ссылке будет получен объект, заблокирован, изменен, записан.
//                    Объект должен иметь реквизит ПодписанЭП.
//         - СправочникОбъект - объект будет изменен без блокировки и без записи.
//         - ДокументОбъект
	...
	Если МенеджерКриптографииДляПроверки = Неопределено Тогда
		...
		ИначеЕсли ЭтоXMLDSig Тогда
			ДанныеСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатИзКонвертаSOAP(
				ИсходныеДанныеДляПроверки.КонвертSOAP, Ложь);
			ПараметрыСоздания.АлгоритмПодписи =
...
//       * ПутиКПрограммамЭлектроннойПодписиИШифрования - Соответствие:
...
//     * ОписанияПрограмм - Массив из см. ОписаниеПрограмм - сведения о поддерживаемых программах криптографии.                                                 
//
// См. также:
//   ОбщаяФорма.НастройкиЭлектроннойПодписиИШифрования - место определения данных параметров и
//   их текстовые описания.
