...
Функция СсылкиНаСертификаты(Знач Сертификаты, Знач ВозвращатьНесуществующие = Ложь) Экспорт
	...
	Для каждого Сертификат Из Сертификаты Цикл
		Отпечатки.Добавить(ОтпечатокСертификата(Сертификат));
	КонецЦикла;
	Запрос = Новый Запрос;
	...
	Если ВозвращатьНесуществующие Тогда
		...
		Для каждого Отпечаток Из Отпечатки Цикл
			...
			Сертификат = ОтпечаткиСертификатов.НайтиСтроки(Отбор);
			Результат.Добавить(?(Сертификат.Количество() > 0, Сертификат[0].Ссылка, 
...
Функция ШтампыВизуализацииЭлектронныхПодписей(Знач ПодписанныйФайл, Знач ЭлектронныеПодписи = Неопределено,
	...
	Для Каждого Подпись Из ЭлектронныеПодписи Цикл
		...
		Если Подпись.Сертификат = Неопределено Тогда
			ТекстОшибки = ?(Подпись.ИмяФайлаПодписи = "",
				НСтр("ru = 'Сертификат подписанта отсутствует в подписи и не найден в локальном хранилище.'"),
		...
		ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПериодДействияШаблон,
		...
		Если МодульРаботаСФайламиПереопределяемый <> Неопределено Тогда
