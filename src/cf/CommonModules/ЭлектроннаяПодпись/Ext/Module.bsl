...
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, ОписаниеОшибки = Null, НаДату = Неопределено) Экспорт
	...
	ИначеЕсли МенеджерКриптографииДляПроверки = Неопределено
		...
		СвойстваПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваПодписиИзДвоичныхДанных(ПодписьДляПроверки);
...
Процедура ДобавитьСтрокуПодписи(ОбъектДанных, СвойстваПодписи, СообщениеЖурналаРегистрации)
	...
	ДатаПодписиИзПараметровПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДатаПодписиИзПараметровПодписи(
		СвойстваПодписи);
		
	Если ЗначениеЗаполнено(ДатаПодписиИзПараметровПодписи) Тогда
		ДатаПодписи = ДатаПодписиИзПараметровПодписи;
	Иначе
