...
Функция ОшибкаПоКлассификатору(ТекстДляПоискаВКлассификаторе, ОшибкаНаСервере = Ложь) Экспорт
	...
	Возврат ЭлектроннаяПодписьСлужебный.ОшибкаПоКлассификатору(ТекстДляПоискаВКлассификаторе, ОшибкаНаСервере);
...
// Выполняет проверку установленных программ
// 
// Параметры:
		...
		ИначеЕсли ТипЗнч(Контекст.ПроверяемыеПрограммы) = Тип("ДвоичныеДанные")
			...
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Данные для поиска программы криптографии не являются сертификатом или подписью.'"), );
			КонецЕсли;
...
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, ОписаниеОшибки = Null, НаДату = Неопределено) Экспорт
	...
	Иначе
		...
		Если ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись Тогда
			...
			Исключение
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При чтении данных подписи: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				Если ВызыватьИсключение Тогда
					ВызватьИсключение;
				КонецЕсли;
				Возврат Ложь;
			...
			ДатаПодписания = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДатаДляПроверкиПодписи(ПараметрыПодписиКриптографии);
...
Процедура УдалитьСтрокиПодписи(ПодписанныйОбъект, ПорядковыеНомера, СообщениеЖурналаРегистрации)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЕстьПрава = ЭтоПолноправныйПользователь Или МенеджерЗаписи.УстановившийПодпись
