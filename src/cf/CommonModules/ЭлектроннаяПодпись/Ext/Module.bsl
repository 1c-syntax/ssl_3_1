...
Функция УсовершенствоватьПодпись(Подпись, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь, МенеджерКриптографии = Неопределено) Экспорт
	...
	Если ОписаниеОшибки = "" Тогда
		РезультатПроверкиСертификата = ПроверитьСертификат(МенеджерКриптографии, ПараметрыПодписиКриптографии.СертификатПоследнейМеткиВремени, ОписаниеОшибки);
		Если РезультатПроверкиСертификата Тогда
			Если ЗначениеЗаполнено(ТипПодписи)
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
				КонецПопытки;
			ИначеЕсли ДобавитьАрхивнуюМеткуВремени И (ПараметрыПодписиКриптографии.ТипПодписи = Перечисления.ТипыПодписиКриптографии.АрхивнаяCAdESAv3
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При проверке сертификата: %1'"), ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
	
	Если ИнформацияОбОшибке <> Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При усовершенствовании подписи: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецЕсли;
	...
	Если УсовершенствованиеПроизошло Тогда
		...
		Иначе 
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При проверке сертификата после усовершенствования: %1
...
Функция УсовершенствоватьПодписьОбъекта(ПодписанныйОбъект, ПорядковыйНомер, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь,
	...
	СвойстваПодписи = УстановленныеПодписи(ПодписанныйОбъект, ПорядковыйНомер)[0];
