...
Функция ДоступнаУсовершенствованнаяПодпись() Экспорт
	...
	Возврат ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись;
...
//   - Структура - см. ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи
//
...
//   * СвойстваПодписи - см. ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи
//   
Функция УсовершенствоватьПодпись(Подпись, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь, МенеджерКриптографии = Неопределено) Экспорт
	...
	ПараметрыПодписиКриптографии = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыПодписиКриптографии(Подпись,
		МенеджерКриптографии, ЭлектроннаяПодписьСлужебный.ДобавкаВремени(), ТекущаяДатаСеанса());
		
	УсовершенствованиеПроизошло = Ложь;
	Если Не ПараметрыПодписиКриптографии.СрокДействияПоследнейМеткиВремени < ТекущаяДатаСеанса() Тогда
		Если ЗначениеЗаполнено(ТипПодписи)
			И ЭлектроннаяПодписьСлужебныйКлиентСервер.ПодлежитУсовершенствованию(ПараметрыПодписиКриптографии.ТипПодписи, ТипПодписи) Тогда
			Попытка
				...
				Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
					УсовершенствованиеПроизошло = Истина;
				КонецЕсли;
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				
			КонецПопытки;
		ИначеЕсли ДобавитьАрхивнуюМеткуВремени И (ПараметрыПодписиКриптографии.ТипПодписи = Перечисления.ТипыПодписиКриптографии.CAdESAv3
			Или ПараметрыПодписиКриптографии.ТипПодписи = Перечисления.ТипыПодписиКриптографии.CAdESAv2) Тогда
			Попытка
	...
	СвойстваПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи();
	Если УсовершенствованиеПроизошло Тогда
		СвойстваПодписи.Подпись = ДвоичныеДанныеРезультата;
		ПараметрыПодписиКриптографии = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыПодписиКриптографии(ДвоичныеДанныеРезультата,
			МенеджерКриптографии, ЭлектроннаяПодписьСлужебный.ДобавкаВремени(), ТекущаяДатаСеанса());
	КонецЕсли;
...
Функция УсовершенствоватьПодписьОбъекта(ПодписанныйОбъект, ПорядковыйНомер, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь,
	...
	СвойстваПодписи = УстановленныеПодписи(ПодписанныйОбъект, ПорядковыйНомер);
...
Функция ДоступностьСозданияЗаявления() Экспорт
	...
	ДоступностьСозданияЗаявления.Вставить("ДляСотрудниковЮридическихЛиц", ЗаявлениеДоступно И МосковскоеВремя < '20230101');
...
Процедура ПродлениеДостоверностиПодписей() Экспорт
	...
	Если Не ДоступнаУсовершенствованнаяПодпись() Тогда
		Возврат;
	КонецЕсли;
	...
	Если (Не ТребуетсяУсовершенствоватьПодписи
		Или (ТипПодписиКриптографииПоУмолчанию <> Перечисления.ТипыПодписиКриптографии.CAdEST
		И ТипПодписиКриптографииПоУмолчанию <> Перечисления.ТипыПодписиКриптографии.CAdESAv3)) И Не ТребуетсяДобавитьАрхивныеМетки Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыЗапроса.Вставить("ОшибкиПриАвтоматическойОбработке", Ложь);
	...
	Пока Истина Цикл
		...
		Результат = ЭлектроннаяПодписьСлужебный.УсовершенствоватьРегламентнымЗаданием(
			Выборка, ТипПодписиКриптографииПоУмолчанию, ТребуетсяДобавитьАрхивныеМетки, ОперацияНачалась);
		Если Не ОперацияНачалась Тогда
			// Возможно, на сервере не установлен менеджер криптографии.
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьСтрокуПодписи(ОбъектДанных, СвойстваПодписи, СообщениеЖурналаРегистрации)
	...
	НоваяЗапись.ТипПодписи = СвойстваПодписи.ТипПодписи;
	НоваяЗапись.СрокДействияПоследнейМеткиВремени = СвойстваПодписи.СрокДействияПоследнейМеткиВремени;
