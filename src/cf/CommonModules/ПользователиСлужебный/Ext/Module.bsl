...
// Возвращает указанного служебного пользователя информационной базы, создает его в случае отсутствия.
// Функция может вызвать исключение, которое необходимо обработать.
//
// Параметры:
...
//
Функция СлужебныйПользовательИБ(ИмяПользователя) Экспорт
	...
	Если ЗаписатьПользователяИБ Тогда
		...
		Если ДоступныНастройки8_3_26() Тогда
			...
			ПользовательИБ.СохраняемоеЗначениеПароля =
				Вычислить("ВычислитьСохраняемоеЗначениеПароляПользователя(НовыйПароль)");
			// АПК:488-вкл
		Иначе
			ПользовательИБ.СохраняемоеЗначениеПароля =
				Пользователи.СохраняемоеЗначениеСтрокиПароля(НовыйПароль);
		КонецЕсли;
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СсылкаСуществующего) Тогда
