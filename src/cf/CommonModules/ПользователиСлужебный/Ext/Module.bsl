...
//  ПользовательСсылка - СправочникСсылка.Пользователи - пользователь, которого необходимо
//                       добавить в группу.
//
// Возвращаемое значение:
...
Функция СписокПользователейДляОбновления(ПользователиИБСПочтой, РезультатЗапроса)
	...
	Для Каждого ПользователяДляОбновления Из РезультатЗапроса Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для пользователя %1 не может быть установлена почта %2 для восстановления пароля, т.к. она используется для пользователя %3(%4)'"),
			ПользователяДляОбновления.Ссылка, СведенияОПользователе.АдресЭлектроннойПочты, СведенияОПользователе.Имя, СведенияОПользователе.ПолноеИмя);
			
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.Справочники.Пользователи, ПользователяДляОбновления.Ссылка, ТекстСообщения);
