...
//
// Параметры:
//  Объект       - СправочникОбъект.Пользователи, СправочникОбъект.ВнешниеПользователи - 
...
//
// Параметры:
//  Обработчики - ТаблицаЗначений - где:
//    * Версия - Строка - 
//    * Процедура - Строка - 
...
//  НайденныйВнешнийПользователь - Неопределено, СправочникСсылка.ВнешниеПользователи - 
...
Функция ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации,
	...
	НачатьТранзакцию();
	Попытка
		ОбъектаАвторизации = ОбъектаАвторизации(СсылкаНаОбъектАвторизации, СсылкаНаТекущегоВнешнегоПользователя);
		
		Если ОбъектаАвторизации.Используется Тогда
			НайденныйВнешнийПользователь = ОбъектаАвторизации.Ссылка;
		КонецЕсли;
	...
	Возврат ОбъектаАвторизации.Используется;
...
//
Функция ОбъектаАвторизации(СсылкаНаОбъектАвторизации, СсылкаНаТекущегоВнешнегоПользователя)
	...
	ОбъектаАвторизации = Новый Структура;
	ОбъектаАвторизации.Вставить("Используется", Выборка.Следующий());
	ОбъектаАвторизации.Вставить("Ссылка", Выборка.Ссылка);
	
	Возврат ОбъектаАвторизации;
...
Процедура ЗаполнитьНазначениеГруппВнешнихПользователей() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ГруппаОбъект = Выборка.Ссылка.ПолучитьОбъект(); // СправочникСсылка.ГруппыВнешнихПользователей
...
// 
// Параметры:
//  СведенияОПользователе - Структура - строковое и ссылочное представление пользователя, где:
//       * ПользовательСсылка  - СправочникСсылка.Пользователи - пользователь, у которого нужно получить настройки.
//       * ИмяПользователяИнформационнойБазы - Строка - пользователь информационной базы, у которого нужно получить настройки.
//  Настройки - Структура - прочие пользовательские настройки, где:
//       * Ключ     - Строка - строковый идентификатор настройки.
...
//   * Ссылка - СправочникСсылка.ГруппыВнешнихПользователей, СправочникСсылка.ГруппыПользователей -  ссылка на элемент справочника.
//   * Родитель - СправочникСсылка.ГруппыВнешнихПользователей, СправочникСсылка.ГруппыПользователей - ссылка на родителя
...
//
// Параметры:
//  ПользовательОбъект - СправочникОбъект.Пользователи, СправочникОбъект.ВнешниеПользователи - Описание
//  ПараметрыОбработки - Структура - где:
...
//
// Параметры:
//  Параметры - Строка - где:
//    * ОсновнойПараметр - Неопределено, Булево, Строка, Массив, Соответствие - 
//    * Форма - ФормаКлиентскогоПриложения - 
//    * КоллекцияРолей - ТаблицаЗначений - где:
//        ** Роль - Строка - 
...
// Параметры:
//  Параметры - Структура - 
//  Коллекция - ДанныеФормыЭлементДерева - где:
//    * Пометка - Булево - 
//    * Имя - Строка -
//    * Синоним - Строка - 
//  Добавить - Булево - 
