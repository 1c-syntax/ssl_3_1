...
Функция ПодсказкаПотребоватьСменуПароляПриВходе(ДляВнешнихПользователей) Экспорт
	...
	Подсказка.Добавить(НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.'"));
	Подсказка.Добавить(Символы.ПС);
	
	Если Не ЭтоПолноправныйПользователь Тогда
		Подсказка.Добавить(НСтр("ru = 'За подробностями обратитесь к администратору.'"));
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
	
	Подсказка.Добавить(НСтр("ru = 'См.'"));
	Подсказка.Добавить(" ");
	
	Если Не ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода Тогда
		Подсказка.Добавить(НСтр("ru = 'Параметры информационной базы в конфигураторе в меню Администрирование.'"));
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
	
	Если ДляВнешнихПользователей Тогда
		Ссылка = НСтр("ru = 'Настройки входа внешних пользователей'");
	Иначе
		Ссылка = НСтр("ru = 'Настройки входа пользователей'");
	КонецЕсли;
	
	Подсказка.Добавить(Новый ФорматированнаяСтрока(Ссылка,,,, "НастройкиВходаПользователей"));
	
	Если Метаданные.Подсистемы.Найти("Администрирование") <> Неопределено Тогда
		Подсказка.Добавить(" ");
		Подсказка.Добавить(НСтр("ru = 'в разделе Администрирование,
...
// Только для внутреннего использования.
Процедура ПослеИзмененияПользователяИлиГруппыПользователейВФорме() Экспорт
	
	ИнтеграцияПодсистемБСП.ПослеИзмененияПользователяИлиГруппыВФорме();
...
Процедура ОбновитьРолиВнешнихПользователей(Знач МассивВнешнихПользователей = Неопределено) Экспорт
	...
	Попытка
		...
		// АПК:96-выкл: допустимо, так как строки не должны повторятся и объем данных небольшой (от единиц до тысяч).
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		// АПК:96-вкл: допустимо, так как строки не должны повторятся и объем данных небольшой (от единиц до тысяч).
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ПарольЗадан Или СоздатьНовогоПользователяИБ И Не ХешПароляЗадан Тогда
