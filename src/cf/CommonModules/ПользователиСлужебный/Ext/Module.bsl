...
Процедура УстановитьНастройкиВосстановленияПароля(НастройкиВосстановленияПароля) Экспорт
...
// Возвращает настройки восстановления пароля из информационной базы
// 
// Возвращаемое значение:
//  Структура:
...
//
Функция НастройкиВосстановленияПароля() Экспорт
...
Функция СпособВосстановленияПароляПользователяИнформационнойБазы(СпособВосстановленияПароля) Экспорт
...
// Возвращаемое значение:
//   см. НастройкиВосстановленияПароля
//
Функция НовыеНастройкиВосстановленияПароля() Экспорт
	
	Если ПользователиСлужебныйПовтИсп.ВерсияПредприятияПоддерживаетВосстановлениеПаролей() Тогда
		НастройкиВосстановленияПароля = Вычислить("Новый НастройкиВосстановленияПароля");
		Возврат НастройкиВосстановленияПароля;
	КонецЕсли;
...
Функция ОписаниеПользователя(Пользователь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Результат.ИдентификаторПользователяИБ) Тогда
		ПользовательСсылка = ?(ТипЗнч(Пользователь) = Тип("СправочникОбъект.Пользователи"),
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры, Отказ, ЭтоВызовПередНачаломРаботыСистемы) Экспорт
	...
	Результат = АвторизоватьТекущегоПользователяПриВходе(РегистрироватьВЖурнале);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.6.108";
		Обработчик.РежимВыполнения = "Оперативно";
		Обработчик.Процедура = "Константы.УдалитьНастройкиВходаПользователей.ПеренестиНастройки";
		Обработчик.Комментарий = НСтр("ru = 'Перенос настроек входа пользователей'");
...
// См. ПользователиСлужебныйПовтИсп.Настройки
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() И УровеньДоступа.НастройкиДляВхода Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Попытка
		...
		ИначеЕсли Не (ВызовИзФормыСменаПароля И Параметры.ТолькоПроверить) Тогда
			...
			Иначе
				...
				КонецЕсли;
				ОбновитьСведенияОбИспользованииПароля(СведенияОПользователе, СохраняемоеЗначениеПароля);
				НаборЗаписей.Записать();
			КонецЕсли;
		КонецЕсли;
...
Функция ПодсказкаПотребоватьСменуПароляПриВходе(ДляВнешнихПользователей) Экспорт
	...
	Если ДляВнешнихПользователей Тогда
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           |См. <a href = ""%1"">Настройки входа внешних пользователей</a> в разделе Администрирование,
				           |пункт Настройки пользователей и прав.'");
		Иначе
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           |См. <a href = ""%1"">Настройки входа внешних пользователей</a>.'");
		КонецЕсли;
	Иначе
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           |См. <a href = ""%1"">Настройки входа пользователей</a> в разделе Администрирование,
				           |пункт Настройки пользователей и прав.'");
		Иначе
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           |См. <a href = ""%1"">Настройки входа пользователей</a>.'");
		КонецЕсли;
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		СтроковыеФункции.ФорматированнаяСтрока(Подсказка),
		"НастройкиВходаПользователей");
	
КонецФункции

// Добавляет пароль в список используемых,
// если в настройках для входа заполнено свойство ЗапретитьПовторениеПароляСредиПоследних
// 
// Параметры:
//  Пользователь - СправочникСсылка.Пользователи
//               - СправочникСсылка.ВнешниеПользователи - пользователь для которого добавляется пароль
//  СохраняемоеЗначениеПароля - Строка - хеш пароля
//
Процедура ДобавитьИспользованныйПароль(Пользователь, СохраняемоеЗначениеПароля) Экспорт
	...
	Попытка
		...
		СведенияОПользователе.Пользователь = Пользователь;
		СведенияОПользователе.Прочитать();
		
		Если Не СведенияОПользователе.Выбран() Тогда
			СведенияОПользователе.Пользователь = Пользователь;
		КонецЕсли;
...
// Обновляет данные использования пароля в записи сведений о пользователе
// 
// Параметры:
//  СведенияОПользователе - РегистрСведенийМенеджерЗаписи.СведенияОПользователях
//              - РегистрСведенийЗапись.СведенияОПользователях - запись регистра сведений
//  СохраняемоеЗначениеПароля - Строка - хеш пароля
//  НастройкиВхода - см. ПользователиСлужебный.НастройкиВхода
//
Процедура ОбновитьСведенияОбИспользованииПароля(СведенияОПользователе, СохраняемоеЗначениеПароля)
	
	Если ТипЗнч(СведенияОПользователе.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		НастройкиВхода = ПользователиСлужебныйПовтИсп.Настройки().ВнешниеПользователи;
	Иначе
		НастройкиВхода = ПользователиСлужебныйПовтИсп.Настройки().Пользователи;
	КонецЕсли;
	...
	Если Пользователи.ОбщиеНастройкиВходаИспользуются()
	...
	ДатаНачалаИспользованияПароля = ?(ОбщегоНазначения.РазделениеВключено(),
		НачалоДня(ТекущаяДатаСеанса()),
		Неопределено);
	
	СведенияОПользователе.ДатаНачалаИспользованияПароля = ДатаНачалаИспользованияПароля;
...
//
// Параметры:
//  ОписаниеПользователя   - СправочникОбъект.Пользователи
//                         - СправочникОбъект.ВнешниеПользователи
...
Функция ДоступнаНастройкаВосстановленияПароля(УровеньДоступа) Экспорт
	...
	Возврат ПользователиСлужебныйПовтИсп.ВерсияПредприятияПоддерживаетВосстановлениеПаролей();
...
Функция ПользователиДляВключенияВосстановленияПароля() Экспорт
	...
	ПользователиИБСПочтой = ПользователиCЭлектроннойПочтойДляВосстановления();
...
Функция ОбновитьПочтуДляВосстановленияПароля(ПользовательСсылка, ОбъектАвторизации = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПользовательСсылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ПользователиCЭлектроннойПочтойДляВосстановления()
...
Функция ПоляВосстановленияПаролей() Экспорт 
...
Процедура ПеренестиНастройкиДлиныИСложностиПаролейКонфигуратора() Экспорт
	...
	ИмяКонстанты = ?(Метаданные.Константы.Найти("УдалитьНастройкиВходаПользователей") = Неопределено,
		"НастройкиВходаПользователей",
		"УдалитьНастройкиВходаПользователей");
	
	Блокировка = Новый БлокировкаДанных;
	Блокировка.Добавить("Константа." + ИмяКонстанты);
	...
	Попытка
		...
		Константы[ИмяКонстанты].Установить(Новый ХранилищеЗначения(НастройкиВхода));
...
Процедура ОбновитьВнешнегоПользователя(СсылкаНаОбъектАвторизации, ПометкаУдаленияОбъектаАвторизации)
	...
	Попытка
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Если ПользователиСлужебныйПовтИсп.ВерсияПредприятияПоддерживаетВосстановлениеПаролей()
...
// Параметры:
//  Пароль - Строка
//  МинимальнаяДлинаПароля - Число
//  СложныйПароль - Булево
//  ИмяДляВхода - Строка
//
// Возвращаемое значение:
//  Строка
//  Неопределено
//
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, Знач МинимальнаяДлинаПароля, СложныйПароль, ИмяДляВхода) Экспорт
	...
	Если СложныйПароль И СтрНайти(ВРег(Пароль), ВРег(ИмяДляВхода)) Тогда
		Возврат НСтр("ru = 'Пароль содержит Имя (для входа).'")
			+ Символы.ПС + Символы.ПС
			+ ПодсказкаДляНовогоПароля();
	КонецЕсли;
...
Процедура ОтключитьНеактивныхИПросроченныхПользователей(ТолькоДляАвторизованногоПользователя = Ложь,
	...
	Если Не Настройки.ОбщиеНастройкиВхода
	 Или Не ЗначениеЗаполнено(Настройки.Пользователи.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода)
	   И Не ЗначениеЗаполнено(Настройки.ВнешниеПользователи.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода) Тогда
		Возврат;
	КонецЕсли;
