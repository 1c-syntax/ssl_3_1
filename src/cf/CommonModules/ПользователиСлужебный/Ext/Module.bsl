...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(НайденныйПользователь) Тогда
		...
		Если ПриНачалеРаботыСистемы
		   И РолиАдминистратораДоступны()
		   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			
			МодульУправлениеДоступомСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступомСлужебный");
			МодульУправлениеДоступомСлужебный.ПриСозданииИлиАвторизацииАдминистратора(НайденныйПользователь,
...
//      ** Значение - Строка - представление роли.
//   * Таблица - ХранилищеЗначения из ТаблицаЗначений:
//      ** Имя - Строка - имя роли.
//   * Список - ХранилищеЗначения из СписокЗначений:
...
Процедура ОбновитьНазначениеПриСозданииНаСервере(Форма, ДобавлятьПользователей = Истина, ТолькоВнешниеПользователи = Ложь) Экспорт
	...
	Если Не ВнешниеПользователи.ИспользоватьВнешнихПользователей()
	   И (    Назначение.Количество() = 1
	        И Назначение[0].ТипПользователей = Справочники.Пользователи.ПустаяСсылка()
	      Или Назначение.Количество() = 0) Тогда
...
//                           - Null - пакетное обновление ролей пользователей ИБ.
//
Процедура ЗаписатьПользователяИнформационнойБазы(ПользовательИБ, ЭтоВнешнийПользователь = Ложь,
	...
	ЭтоОбновлениеСправочниковПользователейИлиРолей = ПользовательИБ.УникальныйИдентификатор
		= ПараметрыСеанса.ОбновлениеСправочниковПользователей.Получить("ИдентификаторПользователяИБ")
		Или СообщатьВМенеджерСервиса = Null;
	...
	СообщатьВМенеджерСервиса = СообщатьВМенеджерСервиса = Истина
	...
	Если СообщатьВМенеджерСервиса
	   И Не ЭтоОбновлениеСправочниковПользователейИлиРолей
	...
	Если Не ЭтоОбновлениеСправочниковПользователейИлиРолей Тогда
...
Функция РегистрироватьИзмененияПравДоступа() Экспорт
	...
	Возврат ПользователиСлужебныйПовтИсп.РегистрироватьИзмененияПравДоступа()
		И ВключенаЗаписьИнформацииВЖурнал();
	
КонецФункции

// Проверяет, что включена запись событий в журнал регистрации уровня Информация.
//
// Возвращаемое значение:
//  Булево
//
Функция ВключенаЗаписьИнформацииВЖурнал() Экспорт
	
	Возврат ПолучитьИспользованиеЖурналаРегистрации().Найти(УровеньЖурналаРегистрации.Информация) <> Неопределено;
...
#Область РаботаКомпоновщикомНастроек
...
#Область ДополнительнаяФункциональностьДляОбменаДанными
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Методы.Вставить("ДобавитьАдминистраторамПравоОткрытияВнешнихОтчетовИОбработок");
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ПользователиСлужебный", Методы);
...
#Область ПроцедурыИФункцииРаботыСНастройкамиВходаПользователей
...
Процедура ЗаполнитьОбщиеНастройкиИзОбщейПолитикиПаролей(Настройки)
	...
	Настройки.СрокБездействияДоЗавершенияСеанса =
		Окр(ПолучитьВремяЗавершенияСеансаПриБездействии_8_3_26() / 60);
	
	Настройки.ПредупреждатьОЗавершенииСеансаПриБездействии =
		Окр(ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26() / 60);
	
	НастройкиСохранения =
		ДополнительныеНастройкиАутентификации_ПолучитьНастройкиАвтоматическогоСохраненияАутентификации_8_3_26();
	...
	НастройкиПроверки =
		ДополнительныеНастройкиАутентификации_ПолучитьНастройкиПроверкиРаскрытияПароля_8_3_26();
...
Процедура ОбновитьОбщуюПолитикуПаролей(Настройки) Экспорт
	...
	Если ПолучитьВремяЗавершенияСеансаПриБездействии_8_3_26()
	  <> Настройки.СрокБездействияДоЗавершенияСеанса * 60
	   И ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26()
	  <> Настройки.ПредупреждатьОЗавершенииСеансаПриБездействии * 60 Тогда
		
		УстановитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26(0);
	КонецЕсли;
	
	Если ПолучитьВремяЗавершенияСеансаПриБездействии_8_3_26()
	  <> Настройки.СрокБездействияДоЗавершенияСеанса * 60 Тогда
		
		УстановитьВремяЗавершенияСеансаПриБездействии_8_3_26(
			Настройки.СрокБездействияДоЗавершенияСеанса * 60);
	КонецЕсли;
	
	Если ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26()
	  <> Настройки.ПредупреждатьОЗавершенииСеансаПриБездействии * 60 Тогда
		
		УстановитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26(
	...
	Если Записать Тогда
		ДополнительныеНастройкиАутентификации_УстановитьНастройкиАвтоматическогоСохраненияАутентификации_8_3_26(
			НастройкиСохранения);
	КонецЕсли;
	...
	Если Записать Тогда
		ДополнительныеНастройкиАутентификации_УстановитьНастройкиПроверкиРаскрытияПароля_8_3_26(
			НастройкиПроверки);
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиИзПолитикиПаролейПользователей(Настройки)
	...
	Если ДоступныНастройки8_3_26() Тогда
		Настройки.ПарольДолженОтсутствоватьВСпискеЗапрещенных
			= ПолучитьПроверкуРаскрытияПаролейПользователей_8_3_26();
...
Процедура ОбновитьПолитикуПаролейПользователей(Настройки) Экспорт
	...
	Если ДоступныНастройки8_3_26() Тогда
		Если ПолучитьПроверкуРаскрытияПаролейПользователей_8_3_26()
		  <> Настройки.ПарольДолженОтсутствоватьВСпискеЗапрещенных Тогда
			
			УстановитьПроверкуРаскрытияПаролейПользователей_8_3_26(
				Настройки.ПарольДолженОтсутствоватьВСпискеЗапрещенных);
		КонецЕсли;
		...
		Если ПолучитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации_8_3_26()
		  <> НовоеДействие Тогда
			
			УстановитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации_8_3_26(НовоеДействие);
		КонецЕсли;
	КонецЕсли;
...
Функция ТекущееДействиеПриВходеЕслиТребованиеНеВыполнено(ПолитикаПаролей = Неопределено)
	...
	ТекущееДействие = ?(ПолитикаПаролей = Неопределено,
		ПолучитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации_8_3_26(),
		ПолитикаПаролей.ДействиеПриНесоответствииПаролейТребованиямПриАутентификации);
	
	Если ТекущееДействие = ДействиеПриНесоответствииПароляТребованиямПриАутентификации_ПотребоватьСменуПароля_8_3_26() Тогда
		Возврат "ПотребоватьСменуПароля";
		
	ИначеЕсли ТекущееДействие = ДействиеПриНесоответствииПароляТребованиямПриАутентификации_ПредложитьСменуПароля_8_3_26() Тогда
...
Функция ЗначениеДействияПриВходеЕслиТребованиеНеВыполнено(ИмяДействия)
	...
	Если ИмяДействия = "ПотребоватьСменуПароля" Тогда
		Возврат ДействиеПриНесоответствииПароляТребованиямПриАутентификации_ПотребоватьСменуПароля_8_3_26();
		
	ИначеЕсли ИмяДействия = "ПредложитьСменуПароля" Тогда
		Возврат ДействиеПриНесоответствииПароляТребованиямПриАутентификации_ПредложитьСменуПароля_8_3_26();
	КонецЕсли;
	
	Возврат ДействиеПриНесоответствииПароляТребованиямПриАутентификации_Нет_8_3_26();
	
КонецФункции

#Область ПроцедурыИФункцииПлатформы_8_3_26

Функция ПолучитьВремяЗавершенияСеансаПриБездействии_8_3_26()
	...
	Возврат Вычислить("ПолучитьВремяЗавершенияСеансаПриБездействии()"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26()
	...
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии()"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ДополнительныеНастройкиАутентификации_ПолучитьНастройкиАвтоматическогоСохраненияАутентификации_8_3_26()
	...
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ДополнительныеНастройкиАутентификации.ПолучитьНастройкиАвтоматическогоСохраненияАутентификации()"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ДополнительныеНастройкиАутентификации_ПолучитьНастройкиПроверкиРаскрытияПароля_8_3_26()
	...
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ДополнительныеНастройкиАутентификации.ПолучитьНастройкиПроверкиРаскрытияПароля()"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ПолучитьПроверкуРаскрытияПаролейПользователей_8_3_26()
	...
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ПолучитьПроверкуРаскрытияПаролейПользователей()"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ПолучитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации_8_3_26()
	...
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ПолучитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации()"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ДействиеПриНесоответствииПароляТребованиямПриАутентификации_ПотребоватьСменуПароля_8_3_26()
	
	УстановитьБезопасныйРежим(Истина);
	
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ДействиеПриНесоответствииПароляТребованиямПриАутентификации.ПотребоватьСменуПароля"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ДействиеПриНесоответствииПароляТребованиямПриАутентификации_ПредложитьСменуПароля_8_3_26()
	
	УстановитьБезопасныйРежим(Истина);
	
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ДействиеПриНесоответствииПароляТребованиямПриАутентификации.ПредложитьСменуПароля"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ДействиеПриНесоответствииПароляТребованиямПриАутентификации_Нет_8_3_26()
	
	УстановитьБезопасныйРежим(Истина);
	
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ДействиеПриНесоответствииПароляТребованиямПриАутентификации.Нет"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ПроверитьСоответствиеПароляПользователяСохраняемомуЗначению_8_3_26(Пароль, ПользовательИБ) Экспорт
	
	УстановитьБезопасныйРежим(Истина);
	
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ПроверитьСоответствиеПароляПользователяСохраняемомуЗначению(Пароль, ПользовательИБ)"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Функция ВычислитьСохраняемоеЗначениеПароляПользователя_8_3_26(Пароль) Экспорт
	...
	// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Возврат Вычислить("ВычислитьСохраняемоеЗначениеПароляПользователя(Пароль)"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:488-вкл
	
КонецФункции

Процедура УстановитьВремяПредупрежденияОЗавершенииСеансаПриБездействии_8_3_26(Таймаут)
	...
	Выполнить("УстановитьВремяПредупрежденияОЗавершенииСеансаПриБездействии(Таймаут)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:487-вкл
	
КонецПроцедуры

Процедура УстановитьВремяЗавершенияСеансаПриБездействии_8_3_26(Таймаут)
	...
	// АПК:487-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Выполнить("УстановитьВремяЗавершенияСеансаПриБездействии(Таймаут)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:487-вкл
	
КонецПроцедуры

Процедура ДополнительныеНастройкиАутентификации_УстановитьНастройкиАвтоматическогоСохраненияАутентификации_8_3_26(Настройки)
	...
	// АПК:487-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Выполнить("ДополнительныеНастройкиАутентификации.УстановитьНастройкиАвтоматическогоСохраненияАутентификации(Настройки)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:487-вкл
	
КонецПроцедуры

Процедура ДополнительныеНастройкиАутентификации_УстановитьНастройкиПроверкиРаскрытияПароля_8_3_26(Настройки)
	...
	// АПК:487-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Выполнить("ДополнительныеНастройкиАутентификации.УстановитьНастройкиПроверкиРаскрытияПароля(Настройки)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:487-вкл
	
КонецПроцедуры

Процедура УстановитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации_8_3_26(Действие)
	...
	// АПК:487-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Выполнить("УстановитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации(Действие)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:487-вкл
	
КонецПроцедуры

Процедура УстановитьПроверкуРаскрытияПаролейПользователей_8_3_26(ПроверкаРаскрытия)
	...
	// АПК:487-выкл Поддержка новых методов платформы (исполняемый код безопасен)
	Выполнить("УстановитьПроверкуРаскрытияПаролейПользователей(ПроверкаРаскрытия)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
	// АПК:487-вкл
...
Процедура УстановитьРеквизитПоказыватьВСпискеВыбораУВсехПользователейИБ(Показывать) Экспорт
	...
	|		ИЛИ Пользователи.Недействителен КАК Скрытый
	|ИЗ
	|	Справочник.Пользователи КАК Пользователи
	|ГДЕ
	...
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		НайденнаяСтрока = Идентификаторы.Найти(ПользовательИБ.УникальныйИдентификатор, "Идентификатор");
		Если Не Показывать
		 Или НайденнаяСтрока = Неопределено
		 Или НайденнаяСтрока.Скрытый Тогда
		...
		Если ПользовательИБ.ПоказыватьВСпискеВыбора <> ПоказыватьВСпискеВыбора Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				ИзмененныеПользователи.Добавить(НайденнаяСтрока.Ссылка);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	РегистрыСведений.СведенияОПользователях.ОбновитьДанныеРегистра(ИзмененныеПользователи);
	
КонецПроцедуры

#КонецОбласти

#Область ПроцедурыИФункцииРаботыСПаролем
...
#Область ПроцедурыИФункцииРаботыСПользователями
...
Функция СтарыйПарольСовпадаетССохраненным(Пароль, ИдентификаторПользователяИБ) Экспорт
	...
	Если ДоступныНастройки8_3_26() Тогда
		// АПК:488-выкл Поддержка новых методов платформы (исполняемый код безопасен)
		Результат = ПроверитьСоответствиеПароляПользователяСохраняемомуЗначению_8_3_26(Пароль, ПользовательИБ);
		// АПК:488-вкл
	Иначе
...
Процедура ПользовательОбъектПередЗаписью(Объект, ПараметрыОбработки) Экспорт
	...
	Если Не Объект.ОбменДанными.Загрузка
	 ...
	 Или ЗаписатьИзмененияВЖурнал
	   И ВключенаЗаписьИнформацииВЖурнал() Тогда
...
Функция НовыеИзмененияСоставовГрупп() Экспорт
	...
	Если РегистрироватьИзмененияПравДоступа() Тогда
...
#Область ПроцедурыИФункцииРаботыСВнешнимиПользователями
...
#Область РаботаСНастройкамиПользователяИнформационнойБазы
...
#Область ПроцедурыИФункцииПеремещенияПользователейМеждуГруппами
...
#Область ВосстановлениеПаролейПользователей
...
Процедура ОбновитьПредопределенныеВидыКонтактнойИнформацииПользователей() Экспорт
	...
	ПараметрыВида = МодульУправлениеКонтактнойИнформацией.ПараметрыАдресаЭлектроннойПочты();
	...
	ПараметрыВида = МодульУправлениеКонтактнойИнформацией.ПараметрыТелефона();
...
#Область РаботаСНастройкамиПользователей
...
#Область ПрочиеПользовательскиеНастройки
...
Процедура СоздатьТекущегоПользователяВСправочнике(СведенияОПользователе)
	...
	Попытка
		...
		Если СведенияОПользователе.СсылкаНового = Неопределено Тогда
			ПользовательПоИдентификаторуСуществует(
				СведенияОПользователе.ИдентификаторПользователяИБ,, Пользователь);
...
Процедура СкопироватьНастройкиПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если Не ПараметрыОбработки.Свойство("ЗначениеКопирования")
	 Или Не ПараметрыОбработки.НовыйПользовательИБСуществует
...
#Область ПроцедурыИспользуемыеПриОбменеДаннымиРИБ
...
#Область ПроцедурыОбработкиНастроекВхода
...
#Область ДляПроцедурыНачатьОбработкуПользователяИБ
...
#Область ДляПроцедурыЗавершитьОбработкуПользователяИБ
...
Процедура ПроверитьИзмененияРеквизитовПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если СтарыйПользователь.ПометкаУдаления = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно пометить на удаление пользователя ""%1"", т.к. ему разрешен вход в приложение.'"),
			ПользовательОбъект.Ссылка);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если СтарыйПользователь.Недействителен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно пометить недействительным пользователя ""%1"", т.к. ему разрешен вход в приложение.'"),
			ПользовательОбъект.Ссылка);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если СтарыйПользователь.Подготовлен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно пометить подготовленным пользователя ""%1"", т.к. ему разрешен вход в приложение.'"),
			ПользовательОбъект.Ссылка);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
#Область ДляПроцедурыОбработатьИнтерфейсРолей
...
Процедура ЗаполнитьРоли(Параметры)
	...
	Свойства = Новый Структура("БезОбновленияДереваРолей", Ложь);
	ЗаполнитьЗначенияСвойств(Свойства, Параметры);
	Если Свойства.БезОбновленияДереваРолей Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьИерархиюГруппПользователей");
	Методы.Вставить("ОбновитьПредопределенныеВидыКонтактнойИнформацииПользователей");
	Методы.Вставить("ДобавитьАдминистраторамПравоОткрытияВнешнихОтчетовИОбработок");
	Методы.Вставить("ПереименоватьКлючХраненияРешенияПоОткрытиюВнешнихОтчетовИОбработок");
	Методы.Вставить("ПеренестиНастройкиДлиныИСложностиПаролейКонфигуратора");
	Методы.Вставить("ЗаполнитьСпискиНастроек", Истина);
	Методы.Вставить("ЗаполнитьПочтуДляВосстановленияПароляУПользователейВФоне", Истина);
