...
//  Строка
//
Функция ИмяСобытияИзменениеИзменениеУчастниковГруппПользователейДляЖурналаРегистрации() Экспорт
...
//  Строка
//
Функция ИмяСобытияИзменениеИзменениеУчастниковГруппВнешнихПользователейДляЖурналаРегистрации() Экспорт
...
//                         * Значение - Неопределено
//
//   ДляРегистрации - Неопределено - регистрация не требуется.
//                  - ТаблицаЗначений:
...
//                                            - СправочникСсылка.ВнешниеПользователи
//                      * Используется        - Булево
//                      * ВидИзменения        - Строка - одна из строк "Добавлен", "Удален", "ИспользованиеИзменено".
//
Функция НовыеИзмененияСоставовГрупп() Экспорт
	...
	Если ПользователиСлужебныйПовтИсп.РегистрироватьИзмененияПравДоступа() Тогда
		...
		ДляРегистрации = Новый ТаблицаЗначений;
		ДляРегистрации.Колонки.Добавить("ГруппаПользователей", Измерения.ГруппаПользователей.Тип);
		ДляРегистрации.Колонки.Добавить("Пользователь",        Измерения.Пользователь.Тип);
		ДляРегистрации.Колонки.Добавить("Используется",        Новый ОписаниеТипов("Булево"));
		ДляРегистрации.Колонки.Добавить("ВидИзменения",        Новый ОписаниеТипов("Строка"));
		ДляРегистрации.Индексы.Добавить("Пользователь, ГруппаПользователей");
		Результат.ДляРегистрации = ДляРегистрации;
	КонецЕсли;
...
Процедура ПослеОбновленияСоставовГруппПользователей(ИзмененияСоставов, ЕстьИзменения = Ложь) Экспорт
	...
	ЕстьИзменения = Истина;
	ЗарегистрироватьИзмененияСоставовГрупп(ИзмененияСоставов);
...
Процедура ДобавитьИзменениеСостава(ДляРегистрации, Выборка, Удален = Ложь)
	...
	Если Удален Тогда
		Строка.ВидИзменения = "Удален";
		Строка.Используется = Ложь;
		
	ИначеЕсли Выборка.ИспользуетсяБыло = Null Тогда
		Строка.ВидИзменения = "Добавлен";
		Строка.Используется = Выборка.Используется;
	Иначе
		Строка.ВидИзменения = "ИспользованиеИзменено";
		Строка.Используется = Выборка.Используется;
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗарегистрироватьИзмененияСоставовГрупп(ИзмененияСоставов)
	
	ДляРегистрации = ИзмененияСоставов.ДляРегистрации;
	Если Не ЗначениеЗаполнено(ДляРегистрации) Тогда
		Возврат;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИзмененияСоставов", ДляРегистрации);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Свойства.Вставить("СсылкаГруппы", ЗначениеВСтрокуВнутр(Выборка.ГруппаПользователей));
	...
	Пока Выборка.Следующий() Цикл
		...
		Свойства.Вставить("СсылкаПользователя", ЗначениеВСтрокуВнутр(Выборка.Пользователь));
		Данные.ПредставлениеПользователей.Добавить(Свойства);
	КонецЦикла;
	
	Если ТипЗнч(ДляРегистрации[0].Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		ИмяСобытия = ИмяСобытияИзменениеИзменениеУчастниковГруппВнешнихПользователейДляЖурналаРегистрации();
	Иначе
		ИмяСобытия = ИмяСобытияИзменениеИзменениеУчастниковГруппПользователейДляЖурналаРегистрации();
	КонецЕсли;
...
Функция КлючиРолейСокращены(СтарыеКлючиРолейСтрокой, НовыеКлючиРолейСтрокой)
	...
	Если СтарыеКлючи.Количество() > НовыеКлючи.Количество() Тогда
		Возврат Истина;
	КонецЕсли;
	
	Для Каждого КлючРоли Из СтарыеКлючи Цикл
		Если НовыеКлючи.Найти(КлючРоли) = Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция РолиСокращены(СтарыйПользовательИБ, НовыйПользовательИБ)
	...
	Для Каждого Роль Из СтарыйПользовательИБ.Роли Цикл
		Если Не НовыйПользовательИБ.Роли.Содержит(Роль) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
