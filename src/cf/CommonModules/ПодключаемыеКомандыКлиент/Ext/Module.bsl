...
Процедура ПослеЗаписи(Форма, Объект, ПараметрыЗаписи) Экспорт
	...
	Если ТипЗнч(ПараметрыЗаписи) = Тип("Структура") И ПараметрыЗаписи.Свойство("ПараметрыВыполненияПодключаемойКоманды") Тогда
		...
		ИмяПараметра = "СтандартныеПодсистемы.ПодключаемыеКоманды.ПараметрыВыполненияПодключаемойКоманды";
		ПараметрыПриложения[ИмяПараметра] = Контекст;
		ПодключитьОбработчикОжидания("ПродолжитьВыполнениеПодключаемойКомандыПослеЗаписиВФорме", 0.1, Истина);
	КонецЕсли;
...
Функция СведенияОПроведенииДокументов(Знач Документы) Экспорт
	...
	Возврат ПодключаемыеКомандыВызовСервера.СведенияОПроведенииДокументов(Документы);
...
Процедура ПродолжитьВыполнениеКоманды(ПараметрыВыполнения)
	...
	ПараметрыВыполнения.МассивСсылок = ?(ТипЗнч(Источник) = Тип("ДанныеФормыСтруктура"),
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Источник.Ссылка),
		ВыбранныеОбъекты(Источник, ОписаниеКоманды));
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		МодульУправлениеПечатьюСлужебныйКлиент.ПередВыполнениемКоманды(ПараметрКоманды, ПараметрыВыполнения);
	КонецЕсли;
	
	// Запись в форме объекта.
	Если ПараметрыВыполнения.ТребуетсяЗапись Тогда
		ПараметрыВыполнения.ТребуетсяЗапись = Ложь;
		ПараметрыВыполнения.ТребуетсяПроведение = ПараметрыВыполнения.ТребуетсяПроведение И Источник.Свойство("Проведен");
...
Процедура ПродолжитьВыполнениеКомандыПослеПодтверждениеПроведения(Ответ, Контекст) Экспорт
	...
	ОчиститьСообщения();
	ДанныеОНепроведенныхДокументах = ОбщегоНазначенияКлиент.ПровестиДокументы(Контекст.НепроведенныеДокументы);
...
Процедура ПродолжитьВыполнениеКомандыПослеУстановкиРасширенияРаботы1СПредприятия(РасширениеРаботыСФайламиПодключено, Контекст) Экспорт
...
Процедура ПослеЗаписиПродолжение() Экспорт
	...
	ИмяПараметра = "СтандартныеПодсистемы.ПодключаемыеКоманды.ПараметрыВыполненияПодключаемойКоманды";
	Контекст = ПараметрыПриложения[ИмяПараметра];
