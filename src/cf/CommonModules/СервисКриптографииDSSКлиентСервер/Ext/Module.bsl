...
Функция ПодготовитьОбъектПароля(ЗначениеПароля, АвторизованныйПользователь, Признак = 1, ИсточникНастроек = Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Признак) Тогда
		Результат = ОбъектПароля(ЗакрытьСекретПользователя(ЗначениеПароля, АвторизованныйПользователь, Признак), 5, ИсточникНастроек);
...
Функция ПолучитьПредставлениеСпособовАвторизации(ЗначениеВыбора) Экспорт
	...
	ВариантМобильногоПриложения = СтрШаблон(ВариантМобильногоПриложения, "myDSS 2.0");
