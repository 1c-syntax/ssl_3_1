...
//    * МобильныйЗакрытыйКлюч 	- Булево - закрытый ключ храниться на мобильном устройстве
//
Функция СвойстваСертификатаПользователя() Экспорт
	...
	Результат.Вставить("МобильныйЗакрытыйКлюч", Ложь);
...
Функция ПолучитьЗначениеПароля(ОбъектПароля, АвторизованныйПользователь) Экспорт
	...
	Если ТипЗнч(ОбъектПароля) = Тип("Структура") Тогда
		...
		Если ОбъектПароля.Состояние = 0 И АвторизованныйПользователь = Неопределено Тогда
			Результат = ОбъектПароля.Значение;
		ИначеЕсли ОбъектПароля.Состояние = 0 Тогда
...
Функция УчетнаяЗаписьАвторизована(НастройкиПользователя, ДатаСеанса) Экспорт
	...
	Если НастройкиПользователя.Политика.Заполнено Тогда
		...
		ИначеЕсли НЕ ТребуетсяВводКонфиденциальныхДанных(НастройкиПользователя.ТокенАвторизации.ТокенОбновления) Тогда
			Результат = Истина;
		ИначеЕсли НастройкиПользователя.ЗапоминатьПароли 
...
Функция ПодготовитьПакетДляПодписи(МассивДокументов) Экспорт
	...
	Результат = Новый Массив;
	НесколькоФайлов = МассивДокументов.Количество() > 1;
	
	Для Счетчик = 1 По МассивДокументов.Количество() Цикл
		СтрокаМассива = МассивДокументов[Счетчик - 1];
		...
		Если НесколькоФайлов Тогда
			НоваяСтрока.ContentInfo = НоваяСтрока.ContentInfo + " #" + Формат(Счетчик, "ЧЦ=3; ЧВН=; ЧГ=0");
		КонецЕсли;
...
//    * Ошибка			- Строка - содержит форматированный текст ошибки, если поле Выполнено = Ложь, устаревшее
//    * КодОшибки		- Строка - содержит код ошибки, устаревшее
...
//    * ИнформацияОбОшибке - ИнформацияОбОшибке - состояние ошибки, например возникшей в фоновой задаче
//
...
Функция ЭтоОшибкаПинКода(ТекстОшибки) Экспорт
	...
	Если ТипЗнч(ТекстОшибки) = Тип("Структура") Тогда
		Содержимое = ТекстОшибки.КодОшибки;
	Иначе
		Содержимое = ПолучитьКодОшибки(ТекстОшибки);
	КонецЕсли;

	Результат = Содержимое = "210009";
	Возврат Результат;
...
Функция ЭтоОшибкаАутентификации(ОписаниеОшибки) Экспорт
	...
	Если ТипЗнч(ОписаниеОшибки) = Тип("Структура") Тогда
		Содержимое = ОписаниеОшибки.КодОшибки;
	Иначе
		Содержимое = ПолучитьКодОшибки(ОписаниеОшибки);
	КонецЕсли;

	Если Сред(Содержимое, 1, 2) = "20"
		ИЛИ Сред(Содержимое, 1, 6) = "000003" Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ЭтоОшибкаОтказа(ТекстОшибки) Экспорт
	...
	Результат = Содержимое = "210000"
				ИЛИ Содержимое = "210001"
				ИЛИ Содержимое = "300045";
...
Процедура РазобратьРезультатЗапроса(РезультатЗапроса, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Если ПолучитьПолеСтруктуры(АнализОтвета, "IsError", Истина) Тогда
			...
			КодОшибки = РезультатЗапроса.СтатусОшибки.КодОшибки;
		...
		ИначеЕсли ПолучитьПолеСтруктуры(АнализОтвета, "IsFinal", Истина) Тогда
			...
			Иначе
				НовыйЭтап = "Ошибка";
				КодОшибки = РезультатВыполнения.СтатусОшибки.КодОшибки;
				ТекстОшибки = РезультатВыполнения.СтатусОшибки.Представление;
	...
	Иначе
		НовыйЭтап 	= "Ошибка";
		ТекстОшибки = РезультатЗапроса.СтатусОшибки.Представление;
...
Функция ДоступныйРеестрОшибок(Знач ИдентификаторОшибки)
	...
	ИначеЕсли ИдентификаторОшибки = НРег("УстановкаРасширенияСФайлами") Тогда
		Результат = ОписаниеОшибки_УстановкаРасширения1СПредприятия();
...
Функция ОписаниеОшибки_ОшибкаВыполнения()
	...
	Результат = ОбщееОписаниеОшибки("100003", 
			НСтр("ru = 'Не удалось подключиться к сервису DSS.
			|1. Возможно сервер DSS сейчас недоступен, проверьте настройки сети или обратитесь к администратору сервера DSS.
			|2. Проверьте настройки учетной записи сервиса DSS в приложении.
			|3. Проверьте настройки для TLS подключения (на сервере 1С:Предприятия должны быть установлены корневые сертификаты,
			|   с помощью которых проверяется сертификат сервиса DSS).
			|4. Для защищенного соединения ГОСТ на сервере 1С:Предприятия должен быть установлен криптопровайдер КриптоПро CSP 
			|   (см. https://www.cryptopro.ru/products/csp/tls).'"));
	Возврат Результат;
...
//   см. ОбщееОписаниеОшибки
//
Функция ОписаниеОшибки_УстановкаРасширения1СПредприятия()
	
	Результат = ОбщееОписаниеОшибки("300046", 
			НСтр("ru = 'Не удалось установить расширение для работы с 1С:Предприятием.'"));
...
Функция ОписаниеОшибки_НеПоддерживаемаяВерсия()
	...
	Результат = ОбщееОписаниеОшибки("300052", 
			НСтр("ru = 'Для выполнения действия требуется более новая версия сервера DSS.'"));
...
Функция ОписаниеОшибки_ОбработкаОтсутствиеНастроек()
	...
	Результат = ОбщееОписаниеОшибки("300073", 
			НСтр("ru = 'Для операций проверки подписи или сертификата не заданы настройки подключения.'"));
...
// Получает реестр типов вторичной авторизации типа для аутентификационного испытания. Варианты методов:
// 	otpviasms (SMS),
//	mobile (myDSS),
// 	airkey (AirKeyDSS),
// 	oath,
...
// Разбор аутентификационного испытания в процессе процедуры подтверждения
...
// Разбор первого этапа аутентификационного испытания - выбор способа подтверждения
// и заполнения данных цикла подтверждения.
//
// Параметры:
//  ДанныеОтвета - Структура - значение аутентификационного испытания.
...
// Для рекурсивного формирования представления структуры.
...
// Проверяет строковое значение, что оно целое число.
//
// Параметры:
...
// Проверяет имя поля на соответствие имени переменной.
//
// Параметры:
//  ПроверяемоеЗначение - Строка
//
...
// Определяет ключ для сохранения и получения настроек из параметра сеанса.
//
// Параметры:
//  НастройкиПользователя 	- см. СервисКриптографииDSS.НастройкиПользователяПоУмолчанию
//
