...
//    * Признак		- Число - алгоритм закрытия 
//
Функция ОбъектПароля(ЗначениеПароля, СостояниеПароля = 0, Признак = 1) Экспорт
...
//
Функция ПодготовитьОбъектПароля(ЗначениеПароля, АвторизованныйПользователь, Признак = 1) Экспорт
	...
	Если ТипЗнч(ЗначениеПароля) = Тип("Структура") Тогда
		...
		Если ЗначениеПароля.Состояние = 0 И ЗначениеЗаполнено(ЗначениеПароля.Значение) Тогда
			Результат = ОбъектПароля(ЗакрытьСекретПользователя(ЗначениеПароля.Значение, АвторизованныйПользователь, Признак), 1);
		КонецЕсли;	
		
	ИначеЕсли ЗначениеПароля = Неопределено Тогда
		Результат = ОбъектПароля(ЗначениеПароля, 3);
		
	ИначеЕсли НЕ ЗначениеЗаполнено(ЗначениеПароля) Тогда
		Результат = ОбъектПароля(ЗначениеПароля, 4);
		
	ИначеЕсли ЗначениеЗаполнено(Признак) Тогда
		Результат = ОбъектПароля(ЗакрытьСекретПользователя(ЗначениеПароля, АвторизованныйПользователь, Признак), 1);
		
	Иначе
		Результат = ОбъектПароля(ЗначениеПароля, 0);
...
Функция ПодготовитьПакетДляПодписи(МассивДокументов) Экспорт
	...
	Для каждого СтрокаМассива Из МассивДокументов Цикл
		...
		ТипДокумента = ПолучитьТипДанныхДокумента(СтрокаМассива.Содержание);
		
		НоваяСтрока = Новый Структура();
		НоваяСтрока.Вставить("Content", СтрокаМассива.Содержание);
		НоваяСтрока.Вставить("Name", СтрокаМассива.ИмяДокумента);
		НоваяСтрока.Вставить("Type", СтрокаМассива.ТипДокумента);
		НоваяСтрока.Вставить("OriginalContent", СтрокаМассива.ИсходноеСодержание);
		НоваяСтрока.Вставить("ВернутьКак", ТипДокумента);
...
Функция ПолучитьСвойствоПодписиАсинхронной(СвойстваПодписи, ИдентификаторыДокументов, ИдентификаторПрофиля = Неопределено, ИдентификаторыСвязанныхДокументов = Неопределено) Экспорт
	...
	СвойстваПодписи.Удалить("SignatureType");
	...
	Если ТипЗнч(ИдентификаторыДокументов) = Тип("Строка") Тогда
		МассивДокументов.Добавить(ИдентификаторыДокументов);
	Иначе
		МассивДокументов = ИдентификаторыДокументов;
	КонецЕсли;
	
	ДанныеДокументов = Новый Массив;
	Для Каждого СтрокаМассива Из МассивДокументов Цикл
		НоваяСтрока = Новый Структура("RefId", СтрокаМассива);
		ДанныеДокументов.Добавить(НоваяСтрока);
	КонецЦикла;
	
	МассивДокументов = Новый Массив;
	Если ТипЗнч(ИдентификаторыСвязанныхДокументов) = Тип("Строка") Тогда
		МассивДокументов.Добавить(ИдентификаторыСвязанныхДокументов);
	ИначеЕсли ИдентификаторыСвязанныхДокументов <> Неопределено Тогда
		МассивДокументов = ИдентификаторыСвязанныхДокументов;
	КонецЕсли;
...
Функция ПолучитьТипДанныхДокумента(ДанныеДокумента) Экспорт
	...
	Если ТипЗнч(ДанныеДокумента) = Тип("Массив") 
		И ДанныеДокумента.Количество() > 0 Тогда
		ЭлементДанных = ДанныеДокумента[0];
	Иначе
		ЭлементДанных = ДанныеДокумента;
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Сервис криптографии облачной подписи'");
...
Процедура РазобратьРезультатЗапроса(РезультатЗапроса, 
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		ИначеЕсли ДанныеПодтверждения.ЭтапЦикла = "Выбор" Тогда
			...
			Если ДанныеПодтверждения.ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение")
				ИЛИ ДанныеПодтверждения.ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_AirKeyLite") Тогда
				НовыйЭтап	= "Ожидание";
			Иначе	
				НовыйЭтап	= "ВводКода";
			КонецЕсли;
		...
		ИначеЕсли ДанныеПодтверждения.ЭтапЦикла = "Начало" Тогда
			ПолучитьМетодыВторичнойАвторизации(АнализОтвета, ДанныеПодтверждения, ВторичнаяАвторизация);
			...
			ИначеЕсли ДанныеПодтверждения.СписокМетодов.Количество() = 1 Тогда
				ПолучитьДанныеВторичнойАвторизации(АнализОтвета, ДанныеПодтверждения, ДатаСеанса);
				Если ДанныеПодтверждения.ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение")
					ИЛИ ДанныеПодтверждения.ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_AirKeyLite") Тогда
					НовыйЭтап	= "Ожидание";
				Иначе	
					НовыйЭтап	= "ВводКода";
				КонецЕсли;
			ИначеЕсли ЗначениеЗаполнено(ВторичнаяАвторизация) Тогда
				НовыйЭтап	= "Выбор";
			Иначе	
				НовыйЭтап	= "Выбор";
			КонецЕсли;	
...
Функция ПодготовитьПараметрыОтправкиЗапроса(НастройкиПользователя, ДанныеПодтверждения) Экспорт
	...
	Если ДанныеПодтверждения.ЭтапЦикла = "Начало" Тогда
		...
		Если ДанныеПодтверждения.ТипПодтверждения = "Транзакция" Тогда
			ПараметрыЗапроса.Вставить("TransactionTokenId", ДанныеПодтверждения.Идентификатор);
		ИначеЕсли ДанныеПодтверждения.ТипПодтверждения = "ПроизвольнаяОперация" Тогда
			Если ЗначениеЗаполнено(ДанныеПодтверждения.Идентификатор) Тогда
				ПараметрыЗапроса.Вставить("TransactionTokenId", ДанныеПодтверждения.Идентификатор);
			Иначе	
	...
	ИначеЕсли ДанныеПодтверждения.ЭтапЦикла = "Выбор" Тогда
		...
		ОписаниеВыбора.Вставить("RefID", ТипВторичнойАвторизации(ДанныеПодтверждения.ВторичнаяАвторизация));
		НовыйЭлемент.ChoiceSelected.Добавить(ОписаниеВыбора);
		
		МассивИспытаний = Новый Массив;
		МассивИспытаний.Добавить(НовыйЭлемент);
		...
		ПараметрыЗапроса.ChallengeResponse.Вставить("ChoiceChallengeResponse", МассивИспытаний);
	...
	ИначеЕсли ДанныеПодтверждения.ЭтапЦикла = "ВводКода" Тогда
		...
		ПараметрыЗапроса.Вставить("Resource", НастройкиПользователя.Подключение.ИдентификаторПодписи);
		ПараметрыЗапроса.Вставить("ChallengeResponse", Новый Структура);
		ПараметрыЗапроса.ChallengeResponse.Вставить("TextChallengeResponse", МассивИспытаний);
...
Функция ОписаниеОшибки_ОшибкаВыполнения()
	...
	Результат = ОбщееОписаниеОшибки("100003", 
			...
			|3. Проверьте настройки и права сервера 1С на подключение к сервису облачной подписи.
...
// Служебная функция для разбора типа аутентификационного испытания в процессе процедуры подтверждения
// варианты методов.
// 	otpviasms (SMS).
//	mobile (myDSS).
// 	airkey (AirKeyDSS).
// 	oath.
// 	simauth.
// 	otpviaemail (Электронная почта).
// 	mtmo.
// 
// Параметры:
//  МетодАутентификации - Строка - значение аутентификационного испытания
//	
// Возвращаемое значение:
//   Структура:
//    * Ключ - ПеречислениеСсылка.СпособыАвторизацииDSS
//    * Значение - Строка - представление метода
//
Функция ТипВторичнойАвторизации(МетодАутентификации)
	
	ВсеТипыПодтверждения = ТипыВторичнойАвторизации();
	Результат = ВсеТипыПодтверждения[МетодАутентификации];
...
Процедура ПолучитьДанныеВторичнойАвторизации(ДанныеОтвета, ДанныеПодтверждения, ДатаСеанса)
	...
	ДанныеПодтверждения.ВторичнаяАвторизация = ТипВторичнойАвторизации(ВторичныйМетод);
...
//
Процедура ПолучитьМетодыВторичнойАвторизации(ДанныеОтвета, ДанныеПодтверждения, ВторичнаяАвторизация)
	...
	Если НесколькоМетодов <> Неопределено Тогда
		ДанныеПодтверждения.Идентификатор = ПолучитьПолеСтруктуры(ДанныеОтвета, "Challenge.ChoiceChallenge[0].RefID", "");
		...
		Для каждого СтрокаМассива Из НесколькоМетодов Цикл
			ВторичныйМетод = ВсеТипыПодтверждения[ПолучитьПолеСтруктуры(СтрокаМассива, "RefID", "")];
			Если ДанныеПодтверждения.ДоступныеМетоды.Найти(ВторичныйМетод) <> Неопределено Тогда
	...
	Иначе
		ВторичныйМетод = ПолучитьПолеСтруктуры(ДанныеОтвета, "Challenge.TextChallenge[0].AuthnMethod", "");
		ВторичныйМетод = ТипВторичнойАвторизации(ВторичныйМетод);
		ДанныеПодтверждения.Идентификатор = ПолучитьПолеСтруктуры(ДанныеОтвета, "Challenge.TextChallenge[0].RefID", "");
		ДанныеПодтверждения.ОписаниеПодтверждения = ПолучитьПолеСтруктуры(ДанныеОтвета, "Challenge.TextChallenge[0].Label", "");
		Если ДанныеПодтверждения.ДоступныеМетоды.Найти(ВторичныйМетод) <> Неопределено Тогда
			ДанныеПодтверждения.СписокМетодов.Добавить(ВторичныйМетод);
	...
	Если ДанныеПодтверждения.СписокМетодов.Найти(ВторичнаяАвторизация) = Неопределено Тогда
