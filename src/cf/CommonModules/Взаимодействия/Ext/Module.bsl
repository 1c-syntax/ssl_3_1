...
// Возвращает список контактов предмета по заданному типу контактной информации.
// 
// Параметры:
//  Предмет - ОпределяемыйТип.ПредметВзаимодействия - предмет, для которого необходимо получить контакты.
//  ТипыКонтактнойИнформации - Массив из ПеречислениеСсылка.ТипыКонтактнойИнформации - типы контактной информации, 
//                             которые необходимо получить вместе с контактами.
// 
// Возвращаемое значение:
//  Массив из Структура:
...
Функция СоздатьПисьмо(Сообщение, УчетнаяЗапись, ОтправитьСразу = Истина) Экспорт
	...
	Исключение
		...
		ШаблонТекстаСообщения = НСтр("ru = 'Не удалось отправить письмо по причине:
				|%1'");
		ТекстОшибки = РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(ИнформацияОбОшибке, ОбщегоНазначения.КодОсновногоЯзыка(), , УчетнаяЗапись);
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка,, Письмо,
	...
	Если Не Письмо.УдалятьПослеОтправки Тогда
		...
		Исключение
			...
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ШаблонТекстаСообщения = НСтр("ru = 'Не удалось сохранить письмо в приложении после успешной отправки по причине:
				|%1'");
...
// Для вызова из обработчика ПриЗаписи объекта ПровайдерSMS.
//
// Параметры:
//  Значение - ПеречислениеСсылка.ПровайдерыSMS
//
Процедура ПриЗаписиПровайдераSMS(Значение) Экспорт
	
	Если ЗначениеЗаполнено(Значение) И ОбщегоНазначения.РазделениеВключено() Тогда
		РегламентныеЗаданияСервер.УстановитьИспользованиеПредопределенногоРегламентногоЗадания(
			Метаданные.РегламентныеЗадания.ОтправкаSMS, Истина);
		РегламентныеЗаданияСервер.УстановитьИспользованиеПредопределенногоРегламентногоЗадания(
			Метаданные.РегламентныеЗадания.ОбновлениеСтатусовДоставкиSMS, Истина);
	КонецЕсли;
	
КонецПроцедуры

// Для вызова из обработчика ПриЗаписи объекта УчетнаяЗаписьЭлектроннойПочты.
// 
// Параметры:
//  Объект - СправочникОбъект.УчетныеЗаписиЭлектроннойПочты
//
Процедура ПриЗаписиУчетнойЗаписиЭлектроннойПочты(Объект) Экспорт
	
	Если Не Объект.ПометкаУдаления И (Объект.ИспользоватьДляПолучения Или Объект.ИспользоватьДляОтправки)
	  И Не Объект.ЭтоГруппа И ОбщегоНазначения.РазделениеВключено() Тогда
		РегламентныеЗаданияСервер.УстановитьИспользованиеПредопределенногоРегламентногоЗадания(
			Метаданные.РегламентныеЗадания.ПолучениеИОтправкаЭлектронныхПисем, Истина);
	КонецЕсли;
...
#Область ВспомогательныеПроцедурыИФункцииОтвечающиеЗаПоискКонтактов
...
#Область ОсновныеПроцедурыИФункцииПоискаКонтактов
...
Функция ТекстЗапросаНаПолучениеИнформацииОКонтактах(ВключатьEmail)
	...
	ТекстЗапроса = СтрСоединить(ТекстыЗапроса, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС)
		+ "
		|УПОРЯДОЧИТЬ ПО
		|	Наименование"; // @query-part-1, @query-part-2
...
Функция КонтактыПоДоменномуАдресу(Знач ИмяДомена)
	...
	ТекстЗапроса = СтрСоединить(ТекстыЗапроса, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС) + "
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	АдресЭП"; // @query-part-1, @query-part-2
...
Функция ПолнотекстовыйПоискКонтактовПоСтроке(Знач СтрокаПоиска, НайденныеКонтакты, Знач ДляАдреснойКниги = Ложь) Экспорт
	...
	Исключение
		Возврат НСтр("ru = 'Не удалось выполнить поиск, попробуйте изменить выражение поиска.
		              |Например, для поиска по части строки необходимо добавлять *. Если ввести ""докум"", то слово ""документ"" найдено не будет, необходимо ввести в строку поиска ""докум*"".
		              |Подробно о поисковых выражениях полнотекстового поиска см. справку.'");
	КонецПопытки;
...
#Область ПроцедурыИФункцииПолученияДанныхКонтактовВзаимодействийИПредметовВзаимодействий
...
#Область ПроцедурыИФункцииРаботыСВзаимодействиями
...
Процедура ОбработатьОснованияПисьма(СтрокаИдентификаторов)
	
	СтрокаИдентификаторов = СтрЗаменить(СтрокаИдентификаторов, " ", Символы.ПС);
	
	СтрокиДоОбработки = СтрРазделить(СтрокаИдентификаторов, Символы.ПС, Ложь);
	СтрокиПослеОбработки = Новый Массив;
	
	Для Каждого СтрокаОснований Из СтрокиДоОбработки Цикл
		
		Если Не СтрНачинаетсяС(СтрокаОснований, "<") Тогда
			СтрокаОснований = "<" + СтрокаОснований;
		КонецЕсли;
		
		Если Не СтрЗаканчиваетсяНа(СтрокаОснований, ">") Тогда
			СтрокаОснований = СтрокаОснований + ">";
		КонецЕсли;
		
		СтрокиПослеОбработки.Добавить(СтрокаОснований);
		
	КонецЦикла;
	
	СтрокаИдентификаторов = СтрСоединить(СтрокиПослеОбработки, " ");
...
Функция ПараметрыПисьмаДляОтправки(Объект) Экспорт
	...
	Если Не ПустаяСтрока(Объект.ИдентификаторОснования) Тогда
		ОбработатьОснованияПисьма(Объект.ИдентификаторОснования);
	...
	Если Не ПустаяСтрока(Объект.ИдентификаторыОснований) Тогда
		ОбработатьОснованияПисьма(Объект.ИдентификаторыОснований);
	...
	Пока ВыборкаВложения.Следующий() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
			ЭлектронныеПодписиВладельца = МодульЭлектроннаяПодпись.ПодписиОбъекта(ВыборкаВложения.Ссылка);
			НомерСтроки = 1;
...
Процедура ДобавитьВложенияПисьмаВПочтовоеСообщение(Сообщение, ВыборкаВложения)
	...
	Пока ВыборкаВложения.Следующий() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
			ПодписиВложения = МодульЭлектроннаяПодпись.ПодписиОбъекта(ВыборкаВложения.Ссылка);
			НомерСтроки = 1;
...
#Область РаботаСЭлементамиИРеквизитамиФормСписковИДокументов
...
//    ** ПредметСписок - КомандаФормы
//    ** Предмет       - КомандаФормы
//  Параметры - Структура:
//    * ТолькоПочта - Булево
//
Процедура ИнициализироватьФормуСпискаВзаимодействий(Форма, Параметры) Экспорт

	Если Параметры.ТолькоПочта Тогда
...
#Область РаботаСПредметамиВзаимодействий
...
Функция ОбработатьТекстHTML(Письмо, ОтключитьВнешниеРесурсы = Истина, ЕстьВнешниеРесурсы = Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(ТекстHTML) Тогда
		...
		Если ТаблицаФайлов.Количество() Тогда
			КоличествоЧастейHtml = СтрЧислоВхождений(ТекстHTML, "<html");
			Если КоличествоЧастейHtml > 1 Тогда
				ЧастиHtml = Новый Массив;
				НомерЧастиHtml = 1;
				Пока НомерЧастиHtml <= КоличествоЧастейHtml Цикл
					ЧастьHtml = СодержимоеТегаHTML(ТекстHTML, "html", Истина, НомерЧастиHtml);
					ЧастиHtml.Добавить(ЧастьHtml);
					НомерЧастиHtml = НомерЧастиHtml + 1;
				КонецЦикла;
				ТекстHTML = СтрСоединить(ЧастиHtml, Символы.ПС + "<br>");
			Иначе
...
Процедура ПроверитьСтатусыДоставкиSMS(СообщениеSMS, Модифицированность) Экспорт
	...
	ЕстьИзменения = Ложь;
	Адресаты = Запрос.Выполнить().Выгрузить();
	ИдентификаторыСообщений = Адресаты.ВыгрузитьКолонку("ИдентификаторСообщения");
	СтатусыДоставки = ОтправкаSMS.СтатусыДоставки(ИдентификаторыСообщений);
	
	Для Каждого Адресат Из Адресаты Цикл
		
		СостояниеСообщения = СостояниеСообщенияSMSСогласноСтатусуДоставки(СтатусыДоставки[Адресат.ИдентификаторСообщения]);
		
		Если СостояниеСообщения <> Адресат.СостояниеСообщения Тогда
			СообщениеSMS.Адресаты[Адресат.НомерСтроки - 1].СостояниеСообщения = СостояниеСообщения;
...
Функция СообщенияSMSДляОтправки()
...
Процедура ОтправитьSMS() Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			РегламентныеЗаданияСервер.УстановитьИспользованиеПредопределенногоРегламентногоЗадания(
				Метаданные.РегламентныеЗадания.ОтправкаSMS, Ложь);
			ТекстСообщения = НСтр("ru = 'Регламентное задание ""Отправка SMS"" отключено,
				|так как не настроен провайдер SMS.'", ОбщегоНазначения.КодОсновногоЯзыка());
			РегламентныеЗаданияСервер.ОтменитьВыполнениеЗадания(
				Метаданные.РегламентныеЗадания.ОтправкаSMS, ТекстСообщения);
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
	...
	Результат = СообщенияSMSДляОтправки();
	Если Результат.Пустой() Тогда
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Попытка
			РезультатОтправки = ОтправкаSMS.ОтправитьSMS(НомераДляОтправки, ТекстСообщения, Неопределено,
				ОтправлятьВТранслите);
		Исключение
		...
		Если ЗначениеЗаполнено(РезультатОтправки.ОписаниеОшибки) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось отправить %1 по причине: 
				|%2'"),
				ВыборкаДокументы.Ссылка, РезультатОтправки.ОписаниеОшибки);
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка, МетаданныеДокумента, ВыборкаДокументы.Ссылка,
				ТекстСообщения);
		КонецЕсли;
...
Функция СообщенияSMSДляПроверкиСтатусов()
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			РегламентныеЗаданияСервер.УстановитьИспользованиеПредопределенногоРегламентногоЗадания(
				Метаданные.РегламентныеЗадания.ОбновлениеСтатусовДоставкиSMS, Ложь);
			ТекстСообщения = НСтр("ru = 'Регламентное задание ""Обновление статусов доставки SMS"" отключено,
				|так как не настроен провайдер SMS.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			РегламентныеЗаданияСервер.ОтменитьВыполнениеЗадания(
				Метаданные.РегламентныеЗадания.ОбновлениеСтатусовДоставкиSMS, ТекстСообщения);
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Предупреждение, , , ТекстСообщения);
		КонецЕсли;
	...
	Результат = СообщенияSMSДляПроверкиСтатусов();
	Если Результат.Пустой() Тогда
		Возврат;
	КонецЕсли;
	...
	ВыборкаДокументы = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаДокументы.Следующий() Цикл
		ОбрабатываемыеДокументы[ВыборкаДокументы.Ссылка] = Новый Массив;
		ВыборкаИдентификаторы = ВыборкаДокументы.Выбрать();
		Пока ВыборкаИдентификаторы.Следующий() Цикл
			Адресаты = Новый Структура;
			Адресаты.Вставить("НомерСтроки");
			Адресаты.Вставить("ИдентификаторСообщения");
			Адресаты.Вставить("СостояниеСообщения");
			ЗаполнитьЗначенияСвойств(Адресаты, ВыборкаИдентификаторы);
			ОбрабатываемыеДокументы[ВыборкаДокументы.Ссылка].Добавить(Адресаты);
			ИдентификаторыСообщений.Добавить(Адресаты.ИдентификаторСообщения);
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого ОбрабатываемыйДокумент Из ОбрабатываемыеДокументы Цикл
		Ссылка = ОбрабатываемыйДокумент.Ключ;
		Адресаты = ОбрабатываемыйДокумент.Значение;
		
		ТаблицаИзменившихсяСтатусов.Очистить();
		ПредставлениеДокумента = ОбщегоНазначения.ПредметСтрокой(Ссылка);
		...
		Попытка
			...
			Для Каждого Адресат Из Адресаты Цикл
				...
				Если СостояниеСообщения <> Адресат.СостояниеСообщения Тогда
					...
					НоваяСтрока.НомерСтроки        = Адресат.НомерСтроки;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ПредставлениеДокумента, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение, МетаданныеДокумента, Ссылка,
				ТекстСообщения);
...
Процедура ОтразитьРезультатыОтправкиSMSВДокументе(ДокументОбъект, РезультатОтправки)
	...
	ОтправленныеСообщения = Новый Соответствие;
	Для Каждого ОтправленноеСообщение Из РезультатОтправки.ОтправленныеСообщения Цикл
		ОтправленныеСообщения[ОтправленноеСообщение.НомерПолучателя] = ОтправленноеСообщение.ИдентификаторСообщения;
	КонецЦикла;
	
	Для Каждого Адресат Из ДокументОбъект.Адресаты Цикл
		Адресат.ИдентификаторСообщения = ОтправленныеСообщения[Адресат.НомерДляОтправки];
		Если ЗначениеЗаполнено(Адресат.ИдентификаторСообщения) Тогда
			Адресат.СостояниеСообщения = Перечисления.СостоянияСообщенияSMS.ОтправляетсяПровайдером;
		Иначе
			Адресат.СостояниеСообщения = Перечисления.СостоянияСообщенияSMS.НеДоставлено;
		КонецЕсли;
	КонецЦикла;
...
#Область РаботаСЭлементамиИРеквизитамиФормВзаимодействий
...
#Область РаботаСОбъектомДокументHTML
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтключитьПодсистемуВМоделиСервиса");
