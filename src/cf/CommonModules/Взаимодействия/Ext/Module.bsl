...
// 
// Параметры:
//  Объекты - Массив - содержит объекты, которые не требуется синхронизировать.
//
...
// 
// Параметры:
//  ЧитаемыеОбъекты - Массив - объекты, читаемые при выполнении обработчика.
//
...
// 
// Параметры:
//  ИзменяемыеОбъекты - Массив - объекты, читаемые при выполнении обработчика.
//
		...
		Для каждого ЭлементМассиваОписания Из МассивОписанийКонтактов Цикл
			Если ЭлементМассиваОписания.Имя = ИмяМетаданныхКонтакта Тогда
				...
				Если ЭлементМассиваОписания.ЕстьВладелец Тогда
					ТаблицаФормы = ТаблицаФормыПоИмени(Форма, "Таблица_" + ЭлементМассиваОписания.ИмяВладельца);
...
//                     - ДокументСсылка - предмет взаимодействия.
...
Функция ИнтернетПочтовоеСообщениеИзВходящегоПисьма(Письмо)
	...
	ДобавитьПолучателейВПочтовоеСообщениеПоВыборке(ОбъектИнтернетПочтовоеСообщение, РезультатЗапросаПолучатели.Выбрать(), "Получатели");
	ДобавитьПолучателейВПочтовоеСообщениеПоВыборке(ОбъектИнтернетПочтовоеСообщение, РезультатЗапросаКопии.Выбрать(), "Копии");
	ДобавитьПолучателейВПочтовоеСообщениеПоВыборке(ОбъектИнтернетПочтовоеСообщение, РезультатЗапросаОбратныйАдрес.Выбрать(), "ОбратныйАдрес");
	ДобавитьПолучателейВПочтовоеСообщениеПоВыборке(ОбъектИнтернетПочтовоеСообщение, РезультатЗапросаАдресаУведомленияОПрочтении.Выбрать(), "АдресаУведомленияОПрочтении");
...
Процедура ДобавитьПолучателяВПочтовоеСообщение(Сообщение, ИмяТаблицы, Адрес, Представление)
	...
	ТаблицаАдресов                   = Сообщение[ИмяТаблицы]; // ИнтернетПочтовыеАдреса
	ПолучательПисьма                 = ТаблицаАдресов.Добавить(Адрес);
...
// 	  * ИменаДобавленныхТаблиц - СписокЗначений
//
...
Функция ТекстПредупрежденияОПриостановкеОтправкиПисем() Экспорт
	...
	Если ВключеноРегламентноеЗаданиеПолученияИОтправкиПисем() Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
			Возврат СтроковыеФункции.ФорматированнаяСтрока(НСтр(
				"ru = 'Регламентное задание <b>Получение и отправка писем</b> не было выполнено по расписанию. <a href = ""ПерейтиКНастройкеРегламентныхЗаданий"">Подробнее</a>'"));
		Иначе
			Возврат СтроковыеФункции.ФорматированнаяСтрока(НСтр(
	...
	Иначе
		Возврат СтроковыеФункции.ФорматированнаяСтрока(НСтр(
			"ru = 'Регламентное задание <b>Получение и отправка писем</b> отключено, доставка писем не выполняется. <a href =  ""ВключитьПолучениеИОтправкуПисем"">Включить</a>'"));
	КонецЕсли;
