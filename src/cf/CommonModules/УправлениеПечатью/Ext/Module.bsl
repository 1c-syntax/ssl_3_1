...
Процедура ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Ссылка) Экспорт
	...
	Для Каждого ОбластьПечати Из ТабличныйДокумент.Области Цикл
		Если СтрНачинаетсяС(ОбластьПечати.Имя, "ШтампЭП") И СтрНайти(ОбластьПечати.Имя, "_Документ_") = 0 Тогда
			ОбластьПечати.Имя = ОбластьПечати.Имя + "_" + ИмяОбласти;
		КонецЕсли;
	КонецЦикла;
	...
	Если СтандартнаяОбработка Тогда
		...
		Иначе
			...
			Если НастройкиСписка.АвтоматическоеЗаполнение Тогда
				...
				КонецЕсли;
			Иначе
				ДобавляемыеКомандыПечати = КомандыПечатиОбъекта(ОбъектМетаданных);
...
Функция СоздатьКоллекциюКомандПечати() Экспорт
	...
	Результат.Колонки.Добавить("ОсновнаяПечатнаяФорма", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("НаименованиеПечатнойФормы", Новый ОписаниеТипов("Строка"));
	...
	Результат.Колонки.Добавить("КомандаПоУмолчанию", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ЗаменитьОсновнуюПечатнуюФорму", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ИдентификаторИзКомплекта", Новый ОписаниеТипов("Строка"));
...
Функция НапечататьВФайл(КомандыПечати, СписокОбъектов, НастройкиСохранения) Экспорт
	...
	ОписаниеКоманды = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СоздатьКоллекциюКомандПечати().Добавить());
	...
	Для Каждого Команда Из СписокКоманд Цикл
		КомандаПечати = ОбщегоНазначения.СкопироватьРекурсивно(ОписаниеКоманды);
...
Функция НастройкиПечатиОбъекта(МенеджерОбъекта) Экспорт
	...
	НастройкиПечати = УправлениеПечатьюПовтИсп.НастройкиПечати(); 
	
	Если НастройкиПечати.ОбъектыПечати.Получить(МенеджерОбъекта) <> Неопределено Тогда
	...
	ОбъектыСКомандамиПечати = УправлениеПечатьюПовтИсп.ОбъектыСКомандамиПечати();
	
	Если ОбъектыСКомандамиПечати.Получить(МенеджерОбъекта) <> Неопределено Тогда
...
Функция НаименованияСформированныхОсновныхПечатныхФорм(Ссылки) Экспорт
	...
	НаименованияПечатныхФорм = РегистрыСведений.ОсновныеПечатныеФормыОбъектов.НаименованияПечатныхФорм(Ссылки);
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат НаименованияПечатныхФорм;
...
Функция НаименованиеСформированнойОсновнойПечатнойФормы(Ссылка) Экспорт
	...
	Результат = НаименованияСформированныхОсновныхПечатныхФорм(Ссылки);
...
Процедура ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, Знач ПереходНаСледующуюСтроку = Ложь) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = СообщениеОбОшибке + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно вывести в печатную форму область ""%1"" из макета.'"),
...
//  ЭлектронныеПодписи - см. ЭлектроннаяПодпись.ПодписиОбъекта
//  
Процедура ДобавитьШтампыВОфисныйДокумент(АдресДокумента, ЭлектронныеПодписи) Экспорт
	...
	МакетШтампа = ПолучитьОбщийМакет("ШтампЭлектроннойПодписиOfficeOpen");
	...
	УправлениеПечатьюСлужебный.НайтиУзлыПоСодержимому(СтруктураДокумента.ДеревоДокумента, "w:bookmarkStart", НайденныеУзлы, ПараметрыПоискаУзлов);
	
	Если НайденныеУзлы.Количество() > 0 Тогда
		УзелЗакладкиШтампа = НайденныеУзлы[НайденныеУзлы.ВГраница()];
		...
		РодительУзлаШтампа.Строки.Удалить(ИндексРазмещения);
	Иначе
		РодительУзлаШтампа = НайтиСтрокуПоИмениТега(СтруктураДокумента.ДеревоДокумента.Строки, "w:body");
		ИндексРазмещения = РодительУзлаШтампа.Строки.Количество();
	КонецЕсли;
	...
	ЗначенияДляЗаполнения.Вставить("[ТекстШтампа]", "");

	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		МодульЭлектроннаяПодписьСлужебный = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодписьСлужебный");
		ПараметрыШтампов = МодульЭлектроннаяПодписьСлужебный.ПараметрыШтампов(ЭлектронныеПодписи);
	Иначе
		ПараметрыШтампов = Новый Массив;
		Для Каждого ДанныеПодписи Из ЭлектронныеПодписи Цикл
			СертификатКриптографии = Новый СертификатКриптографии(ДанныеПодписи.Сертификат);
			
			ПоляТекстаШтампа = Новый Массив;
			ПоляТекстаШтампа.Добавить(НСтр("ru = 'Сертификат'") + ": " + СертификатКриптографии.СерийныйНомер);
			ПоляТекстаШтампа.Добавить(НСтр("ru = 'Владелец'") + ": " + ДанныеПодписи.КомуВыданСертификат);
			...
			ПоляТекстаШтампа.Добавить(НСтр("ru = 'Действителен'") + " " + ПериодДействия);
			
			ПараметрыШтампов.Добавить(Новый Структура("ТекстШтампа", СтрСоединить(ПоляТекстаШтампа, Символы.ПС)));
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ПараметрыШтампа Из ПараметрыШтампов Цикл
		Если ДобавитьОтступ Тогда
			УзелОтступ = Новый Структура;
			УзелОтступ.Вставить("Атрибуты", Новый Структура);
			УзелОтступ.Вставить("Строки", Новый Массив);
			УзелОтступ.Вставить("ИмяТега", "w:t");
			УзелОтступ.Вставить("Текст", "");
			УзелОтступ.Вставить("ПолныйТекст", "");
			УправлениеПечатьюСлужебный.ДобавитьУзелОписанияРаздела(РодительУзлаШтампа, ИндексРазмещения, УзелОтступ);
			ИндексРазмещения = ИндексРазмещения + 1;
		Иначе
			ДобавитьОтступ = Истина;
		КонецЕсли;
		
		ЗначенияДляЗаполнения.Вставить("[ТекстШтампа]", ПараметрыШтампа.ТекстШтампа);
	...
	ПолучитьПечатнуюФорму(ДеревоМакета, АдресДокумента);
...
Процедура УстановитьПараметры(ДеревоМакета, ЗначенияПараметров, ПараметрыЗаполненияОбласти) Экспорт
	...
	Если ЗаполнятьКолонтитулы Тогда
		Для Каждого Колонтитул Из СтруктураДокумента.Колонтитулы Цикл
			УстановитьПараметрыВДереве(ЗначенияПараметров, Колонтитул.Значение, ДеревоМакета, ДобавлятьСвязи, Колонтитул.Ключ);
		КонецЦикла;
	КонецЕсли;
		
	УстановитьПараметрыВДереве(ЗначенияПараметров, ДеревоДокумента, ДеревоМакета, ДобавлятьСвязи);
...
Функция ИсточникиКомандПечати() Экспорт
	...
	ИнтеграцияПодсистемБСП.ПриОпределенииОбъектовСКомандамиПечати(Настройки.ОбъектыПечати); // АПК:222 Вызов устаревшей процедуры для обратной совместимости.
	УправлениеПечатьюПереопределяемый.ПриОпределенииОбъектовСКомандамиПечати(Настройки.ОбъектыПечати); // АПК:222 Вызов устаревшей процедуры для обратной совместимости.
	
	Результат = Новый Массив;
	Для Каждого ОбъектМенеджер Из Настройки.ОбъектыПечати Цикл
...
Функция СформироватьПечатныеФормыВФоне(ПараметрыФоновойПечати) Экспорт
	...
	Иначе
		...
		ПараметрыПечати.Свойство("ОсновнаяПечатнаяФорма", ОсновнаяПечатнаяФорма);
		Если ОсновнаяПечатнаяФорма <> Неопределено Тогда
			ПриВыполненииКомандыПечати(ПараметрыФоновойПечати.ПараметрКоманды, ПараметрыПечати, Результат);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	КомандыПечати = КомандыПечатиФормы(ИмяФормы, СписокОбъектов);
	УстанавливатьПометки = (КомандыПечати.Количество() > 1);
	
	Если ПоказыватьПечатьПоУмолчанию(ИмяФормы, СписокОбъектов) Тогда;
		КомандаПечатиПоУмолчанию = КомандыПечати.Добавить();
		КомандаПечатиПоУмолчанию.МенеджерПечати = "УправлениеПечатью";
		КомандаПечатиПоУмолчанию.Идентификатор = "ПечатьПоУмолчанию";
		КомандаПечатиПоУмолчанию.Представление = НСтр("ru = 'Печать по умолчанию'");
		КомандаПечатиПоУмолчанию.КомандаПоУмолчанию = Истина;
		КомандаПечатиПоУмолчанию.Картинка = БиблиотекаКартинок.Печать;
		КомандаПечатиПоУмолчанию.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечатиПоУмолчанию.УсловияВидимости = Новый Массив(1);
	КонецЕсли;
	...
	|ОсновнаяПечатнаяФорма,НаименованиеПечатнойФормы,ЗаменитьОсновнуюПечатнуюФорму,ИдентификаторИзКомплекта";
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		...
		КонецЕсли;
		Если КомандаПечати.ОсновнаяПечатнаяФорма Тогда
			Команда.Важность = "Важное";
			Если НастройкиФормы.ЭтоФормаОбъекта И УстанавливатьПометки Тогда
				ИдентификаторДляПометки = УправлениеПечатьюКлиентСервер.ИдентификаторБезСпецСимволов(КомандаПечати.Идентификатор);
				Команда.ЗначениеПометки = "ПараметрыУправленияПечатью.%ИСТОЧНИК%.ЗначениеПометки" + ИдентификаторДляПометки;
			КонецЕсли;
		КонецЕсли;
		Если КомандаПечати.КомандаПоУмолчанию Тогда
	...
	Если СписокОбъектов = Неопределено Тогда
		...
		Если ОбъектМетаданных <> Неопределено Тогда
			СписокОбъектов = ОбъектМетаданных.РегистрируемыеДокументы;
		КонецЕсли;
...
Функция КоллекцииПолейИсточниковДанных(ИсточникиДанных) Экспорт
	...
	ТипыИсточников = Новый Массив; // Массив из Тип
	Для Каждого ОписаниеОбъекта Из ОписанияОбъектов Цикл
		...
		Для Каждого СхемаКомпоновкиДанных Из СхемыКомпоновкиДанныхИсточникаПолей(ИмяОбъектаМетаданных, Ложь) Цикл
			Результат.Добавить(КонструкторФормул.КоллекцияПолей(СхемаКомпоновкиДанных.Значение), СхемаКомпоновкиДанных.Представление);
		КонецЦикла;
		
		Тип = СтандартныеПодсистемыСервер.ТипСсылкиИлиКлючаЗаписиОбъектаМетаданных(ОписаниеОбъекта.Значение);
		Если ТипыИсточников.Найти(Тип) = Неопределено Тогда
			ТипыИсточников.Добавить(Тип);
		КонецЕсли;
		
	КонецЦикла;

	Результат.Добавить(КоллекцияПолейОбщиеРеквизиты(ТипыИсточников), "ОбщиеРеквизиты");
...
Функция СписокОператоров(ДополнительныеПоля = Неопределено) Экспорт
	...
	Иначе
		...
		ИмяМетода =  Префикс + "ЕслиПустое";
		ДобавитьОператорВГруппу(Группа, ИмяМетода, НСтр("ru = 'Если пустое'"),, Истина);
		ИмяМетода =  Префикс + "ЧислоПрописьюПоПараметрам";
		ДобавитьОператорВГруппу(Группа, ИмяМетода, НСтр("ru = 'Число прописью'"), ТипСтрока, Истина);
...
Функция ДанныеПечати(Объекты, Поля, КодЯзыка) Экспорт
	...
	ТипыПолейПараметров = Новый Соответствие;
	...
	Для Каждого ПутьКДанным Из Поля Цикл
		Пока ЗначениеЗаполнено(ПутьКДанным) Цикл
			Если ТребуемыеПоля[ПутьКДанным] = Неопределено Тогда
				...
				ДобавитьОписаниеПоля(ОписаниеПолей, ОписаниеПоля);
				...
				Если ЗначениеЗаполнено(КодЯзыкаБезРегиона) И Не СтрЗаканчиваетсяНа(ПутьКДанным, "." + КодЯзыкаБезРегиона)
					...
					КонецЕсли;
				КонецЕсли;
				ИмяПоляТаблицы = ИмяПоляВТаблицу(ПутьКДанным);
				ТипыПолейПараметров[ИмяПоляТаблицы] = ОписаниеПоля.Тип;
			КонецЕсли;
	...
	ДанныеПечати["ТипыПолейПараметров"] = ТипыПолейПараметров;
...
//   * ОсновнаяПечатнаяФорма - Булево
//   * НаименованиеПечатнойФормы - Строка
//   * МакетДляВыгрузкиОбъекта - Булево
//   * ФорматСохраненияВыгрузки - ПеречислениеСсылка.ФорматыВыгрузкиОбъектов
// 
Функция ОписаниеМакета() Экспорт
	...
	Результат.Вставить("ОсновнаяПечатнаяФорма");
	Результат.Вставить("НаименованиеПечатнойФормы");
	Результат.Вставить("МакетДляВыгрузкиОбъекта", Ложь);
	Результат.Вставить("ФорматСохраненияВыгрузки", Перечисления.ФорматыВыгрузкиОбъектов.ПустаяСсылка());
...
Функция СформироватьОфисныйДокумент(Макет, МассивОбъектов, ОбъектыПечати, КодЯзыка, ПараметрыПечати) Экспорт
	...
	УправлениеПечатьюСлужебный.ИсключитьВложенныеТаблицы(ИменаТабличныхЧастейОбъекта);
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Для ИндексОбласти = 0 По СтруктураДокумента.Области.Количество()-1 Цикл
			...
			Если ЗначениеЗаполнено(ТекущаяОбласть.Коллекция) Тогда
				...
				Для Каждого ЭлементКоллекции Из МассивКоллекции Цикл
					...
					ОбработатьВложенныеОбласти(СтруктураОбластиСтроки, ТекущаяОбласть, ЭлементКоллекции, ДанныеОбъекта, НастройкиФорматаПолей, КодЯзыка);
		...
		УправлениеПечатьюСлужебный.ВосстановитьПолныйТекст(ДеревоДляВывода, 
			ДеревоМакетаДляЗаполнения.СтруктураДокумента.Гиперссылки);
		
		УдалитьАбзацыСОчищеннымТекстом(ДеревоДляВывода,
			ДеревоМакетаДляЗаполнения.СтруктураДокумента.ДеревоДокумента);
...
Процедура ПриСозданииНаСервере(Форма, ТаблицаПодключаемыхКоманд, ЭтоФормаОбъекта) Экспорт
	
	СоздатьРеквизитХраненияНастроек(Форма, ТаблицаПодключаемыхКоманд);
	
КонецПроцедуры

Процедура ПриОбновленииПометкиОсновныхКоманд(Ссылка, КомандыСПометкой, Результат) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	СохраненныйИдентификатор = РегистрыСведений.ОсновныеПечатныеФормыОбъектов.ИдентификаторКоманды(Ссылка);
	УстановитьПривилегированныйРежим(Ложь);
	
	ПараметрыУправленияПечатью = Новый Структура;
	
	Для Каждого Команда Из КомандыСПометкой Цикл
		Идентификатор = УправлениеПечатьюКлиентСервер.ИдентификаторБезСпецСимволов(Команда.Идентификатор);
		
		Если ПустаяСтрока(СохраненныйИдентификатор) Тогда
			ПараметрыУправленияПечатью.Вставить("ЗначениеПометки" + Идентификатор, Ложь);
		Иначе
			ЗначениеПометки = (Идентификатор = СохраненныйИдентификатор);
			ПараметрыУправленияПечатью.Вставить("ЗначениеПометки" + Идентификатор, ЗначениеПометки);
		КонецЕсли;
	КонецЦикла;
	
	Результат.Вставить("ПараметрыУправленияПечатью", ПараметрыУправленияПечатью);
	
КонецПроцедуры

Процедура ПриВыводеКоманд(Форма, ПодключенныеКоманды) Экспорт
	
	ПараметрыУправленияПечатью = Форма.ПолучитьРеквизиты().Найти("ПараметрыУправленияПечатью");
	Если ПараметрыУправленияПечатью <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПодключенныеКоманды, Форма.ПараметрыУправленияПечатью);
	КонецЕсли;
	
КонецПроцедуры

// Записывает данные основной печатной формы, которая была сформирована по объекту
//
// Параметры:
//  Объекты               - ЛюбаяСсылка
//                        - Массив из ЛюбаяСсылка - документы, для которых необходимо сохранить наименование.
//  ОписаниеКоманды       - Структура - строка из таблицы см. УправлениеПечатью.СоздатьКоллекциюКомандПечати.
//  КоллекцияПечатныхФорм - ТаблицаЗначений - см. УправлениеПечатьюПереопределяемый.ПриПечати.КоллекцияПечатныхФорм.
//                          Параметр передается, если печать сформирована в табличный документ.
//
Процедура ПриВыполненииКомандыПечати(Объекты, ОписаниеКоманды, КоллекцияПечатныхФорм = Неопределено) Экспорт
	
	Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ОписаниеКоманды.ОсновнаяПечатнаяФорма Или Не ОписаниеКоманды.ЗаменитьОсновнуюПечатнуюФорму Тогда
		Возврат;
	КонецЕсли;
	
	Идентификатор = ОписаниеКоманды.ИдентификаторИзКомплекта;
	Если ПустаяСтрока(Идентификатор) Тогда
		Идентификатор = ОписаниеКоманды.Идентификатор;
	КонецЕсли;
	...
	Иначе
		ПечатнаяФорма = КоллекцияПечатныхФорм.Найти(Идентификатор, "ИмяМакета");
		Если ПечатнаяФорма <> Неопределено Тогда
			ДокументСформирован = ПечатнаяФорма.ТабличныйДокумент.ВысотаТаблицы > 0;
		КонецЕсли;
	КонецЕсли;
	...
	РегистрыСведений.ОсновныеПечатныеФормыОбъектов.СохранитьНаименованиеОсновнойПечатнойФормы(
		Объекты, ОписаниеКоманды.НаименованиеПечатнойФормы, Идентификатор);
	
	Если ТипЗнч(Объекты) <> Тип("Массив") Тогда
		Массив = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объекты);
	Иначе
		Массив = Объекты;
	КонецЕсли;
	
	Для Каждого Объект Из Массив Цикл
		РегистрыСведений.ОсновныеПечатныеФормыКонтрагентов.ЗаписатьИдентификаторПечатнойФормы(Объект, Идентификатор);
	КонецЦикла;
	
КонецПроцедуры

Функция ЗаменитьВСтроке(Знач Строка, ПараметрыЗамены) Экспорт
...
Функция ЗаменитьВФорматированнойСтроке(Строка, ПараметрыЗамены) Экспорт
...
Функция ЗначенияПараметров(Параметры, ДанныеПечати, НастройкиФорматаПолей, КодЯзыка) Экспорт
...
Функция ВычислитьВыражение(Знач ИсходноеВыражение, ДанныеПечати, НастройкиФорматаПолей, КодЯзыка, ПрименятьФорматирование = Неопределено) Экспорт
	...
	Для Каждого ОписаниеЭлемента Из ЭлементыФормулы.ОперандыИФункции Цикл
		...
		Если Не ЭтоФункция Тогда
			Если СтрНайти(ВРег("И,ИЛИ,НЕ,ИСТИНА,ЛОЖЬ"), ВРег(Операнд)) Тогда
				Продолжить;
			КонецЕсли;
			
			Если ВРег("СимволПС") = ВРег(Операнд) Тогда
				
				Операнд = Символы.ПС;
				ЭлементыФормулы.ВсеЭлементы[ОписаниеЭлемента.Ключ] = "Символы.ПС";
				Продолжить;
...
Функция ОчиститьКвадратныеСкобки(Строка) Экспорт
...
//  ТабличныйДокумент, ДвоичныеДанные - макет.
//
Функция ПредставлениеМакета(ПутьКМакету, КодЯзыка) Экспорт
...
Функция МакетСуществует(ПутьКМакету) Экспорт
...
Функция ОбластиМакета(Макет, ДанныеПечати) Экспорт
...
Функция ПоляМакета(Макет) Экспорт
...
Процедура ОпределитьВидимостьКомандПечатиПоФункциональнымОпциям(КомандыПечати, Форма = Неопределено) Экспорт
...
Процедура ИсправитьПризнакПроверкаПроведенияПередПечатью(КомандыПечати, ОбъектМетаданных) Экспорт
	
	Если Не Метаданные.Документы.Содержит(ОбъектМетаданных)
		Или ОбъектМетаданных.Проведение = Метаданные.СвойстваОбъектов.Проведение.Запретить Тогда
...
// Параметры:
//  ПутьКДанным - Строка
//  ИмяТаблицы - Неопределено, Строка - имя таблицы
// 
// Возвращаемое значение:
//  Строка - имя поля в таблицу
//
Функция ИмяПоляВТаблицу(ПутьКДанным, ИмяТаблицы = Неопределено) Экспорт
	
	Если ИмяТаблицы = Неопределено Тогда
		Возврат СтрЗаменить(ПутьКДанным, ".", "_"); 
	Иначе
		Возврат СтрЗаменить(ПутьКДанным, ИмяТаблицы + ".", "");
	КонецЕсли;
	...
	Для Каждого Рисунок Из ТабличныйДокумент.Рисунки Цикл
		...
		Если Позиция > 0 Тогда
			...
			Если ЗначениеЗаполнено(Рисунок.ПараметрРасшифровки) Тогда
				ИмяКартинки = Рисунок.ПараметрРасшифровки;
			Иначе
				ИмяКартинки = Лев(Рисунок.Имя, Позиция - 1);
			КонецЕсли;
			
			Картинка = Неопределено;
			Если ТипЗнч(КомплектПодписейИПечатей) = Тип("Соответствие")
				Или ТипЗнч(КомплектПодписейИПечатей) = Тип("Структура") И КомплектПодписейИПечатей.Свойство(ИмяКартинки) Тогда
				Картинка = КомплектПодписейИПечатей[ИмяКартинки];
			КонецЕсли;
...
Процедура УстановитьНастройкиКомандПечати(КомандыПечати, Владелец)
	...
	ОтключенныеКоманды = РегистрыСведений.НастройкиКомандПечати.ОтключенныеКоманды(Владелец);
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		...
		Если ОтключенныеКоманды[КомандаПечати.УникальныйИдентификатор] <> Неопределено Тогда
...
Функция КомандыПечатиОбъекта(ОбъектМетаданных, СПечатнымиФормами = Истина) Экспорт
	...
	ОсновныеКомандыПечати = КомандыПечати.НайтиСтроки(Новый Структура("ОсновнаяПечатнаяФорма", Истина));
	Для Каждого КомандаПечати Из ОсновныеКомандыПечати Цикл
		Если Не ЗначениеЗаполнено(КомандаПечати.НаименованиеПечатнойФормы) Тогда
			КомандаПечати.НаименованиеПечатнойФормы = КомандаПечати.Представление;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьВнешнююПечатнуюФорму(ДополнительнаяОбработкаСсылка, Идентификатор, СписокОбъектов, ОбъектыПечати = Неопределено) Экспорт
...
Процедура ВыполнитьКомандуПечатиВФайл(КомандаПечати, НастройкиСохранения, СписокОбъектов, Результат)
	...
	ПриВыполненииКомандыПечати(СписокОбъектов, КомандаПечати, ДанныеПечати.КоллекцияПечатныхФорм);
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		...
		Если НастройкиСохранения.ПодписьИПечать Тогда
			ДанныеПечатиРисунковТабличногоДокумента = ПодписиИПечатиТабличногоДокумента(
				ОбъектыПечати, ПечатнаяФорма.ТабличныйДокумент, ОбщегоНазначения.КодОсновногоЯзыка());
			...
			ДобавитьПодписьИПечать(ПечатнаяФорма.ТабличныйДокумент, ДанныеПечатиРисунковТабличногоДокумента);
		Иначе
			УбратьПодписьИПечать(ПечатнаяФорма.ТабличныйДокумент);
		КонецЕсли;
...
Процедура ЗаполнитьДанныеПечати(ДанныеПечати, ОписаниеПолей, ИерархияПолей, Объекты, КодЯзыка)
	...
	Для Каждого КоллекцияПолей Из ИерархияПолей Цикл
		...
		Для Каждого Поле Из ПодчиненныеПоля Цикл
			ОписанияПоПодчиненномуПолю = ОписаниеПолей.НайтиСтроки(Новый Структура("Поле", Поле.Ключ));
			Для Каждого ОписаниеПоля Из ОписанияПоПодчиненномуПолю Цикл
				...
				Если ПоляСхемыКомпоновкиДанных.Найти(ОписаниеПоля.ПутьКДанным) = Неопределено Тогда
		...
		ОписанияПустыхИсточниковДанных = ОписанияИсточниковДанных.Скопировать();
			...
			Если ИсточникПолейВТабличнойЧасти Тогда
				...
				Для НомерСтроки = 1 По ДанныеПечати[Объект][ПутьКДаннымТабличнойЧасти].Количество() Цикл
					...
					Иначе
						...
						ДанныеИсточника.Вставить("Объект", Объект);
						ДанныеИсточника.Вставить("ЗначениеИсточникаПолей", ЗначениеИсточникаПолей);
						ДанныеИсточника.Вставить("ПутьКДаннымИсточникаПолей", ПутьКДаннымИсточникаПолей);
						ДанныеИсточника.Вставить("ПутьКДаннымВладельцаИсточника", ПутьКДаннымВладельцаИсточника);
						ДобавитьОписанияПустыхИсточников(
							ОписанияПустыхИсточниковДанных,
							ДанныеПечати,
							СхемыКомпоновкиДанных,
							Поля,
							ДанныеИсточника);
		...
		ПустыеЗначенияДобавлены = Ложь;
		
		Для Каждого Элемент Из Поля Цикл
			ИдентификаторСхемыКомпоновкиДанных = СхемыКомпоновкиДанных[Элемент.Ключ];
			
			АктуализироватьОписанияИсточниковДанных(
				ПустыеЗначенияДобавлены,
				ОписанияИсточниковДанных,
				ОписанияПустыхИсточниковДанных,
			...
			Для Каждого СсылкиПоТипу Из СсылкиПоТипам(ИсточникиДанных) Цикл
				ТипИсточников = СсылкиПоТипу.Ключ;
				ИсточникиДанных = СсылкиПоТипу.Значение;
				
				ПроверятьТипИсточника = Ложь;
				ЧастиСтроки = СтрРазделить(ИдентификаторСхемыКомпоновкиДанных, ".");
				Если ЧастиСтроки.Количество() > 1 Тогда
					ИмяОбъектаМетаданных = ЧастиСтроки[0] + "." +  ЧастиСтроки[1];
					ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяОбъектаМетаданных);
					ПроверятьТипИсточника = ОбъектМетаданных <> Неопределено;
				КонецЕсли;
				
				ИмяТипаИсточника = "";
				Если ПроверятьТипИсточника Тогда
					ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипИсточников);
					ИмяТипаИсточника = ОбъектМетаданных.ПолноеИмя();
				КонецЕсли;
				
				Если ПроверятьТипИсточника И ЗначениеЗаполнено(ИмяТипаИсточника)
					И Не СтрНачинаетсяС(ИдентификаторСхемыКомпоновкиДанных, ИмяТипаИсточника + ".")
					И ИдентификаторСхемыКомпоновкиДанных <> ИмяТипаИсточника Тогда
					Продолжить;
				КонецЕсли;
				...
				ДанныеПечати["ИменаТабличныхЧастейОбъекта"] = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДанныеПечати["ИменаТабличныхЧастейОбъекта"]);
...
Процедура АктуализироватьОписанияИсточниковДанных(ПустыеЗначенияДобавлены, ОписанияИсточниковДанных, Знач ОписанияПустыхИсточниковДанных, Знач ИдентификаторСхемыКомпоновкиДанных)
	
	Если ИдентификаторСхемыКомпоновкиДанных = "ДанныеПредопределенныеЗначения" Тогда
		
		Если НЕ ПустыеЗначенияДобавлены Тогда
			
			Для Каждого ОписаниеИсточника Из ОписанияПустыхИсточниковДанных Цикл
				
				ПустыеЗначенияДобавлены = Истина;
				ЗаполнитьЗначенияСвойств(ОписанияИсточниковДанных.Добавить(), ОписаниеИсточника);
	...
	Иначе
		...
		КоличествоЭлементовКоллекции = ОписанияИсточниковДанных.Количество();
		Для ОбратныйИндекс = 1 По КоличествоЭлементовКоллекции Цикл
			
			Индекс = КоличествоЭлементовКоллекции - ОбратныйИндекс;
			СтрокаТаблицы = ОписанияИсточниковДанных[Индекс];
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Значение) Тогда
				ОписанияИсточниковДанных.Удалить(Индекс);
			КонецЕсли;
...
Процедура ДобавитьОписанияПустыхИсточников(ОписанияПустыхИсточниковДанных, Знач ДанныеПечати, Знач СхемыКомпоновкиДанных, Знач Поля, ДанныеИсточника)
	
	Объект = ДанныеИсточника.Объект;
	ЗначениеИсточникаПолей = ДанныеИсточника.ЗначениеИсточникаПолей;
	ПутьКДаннымИсточникаПолей = ДанныеИсточника.ПутьКДаннымИсточникаПолей;
	ПутьКДаннымВладельцаИсточника = ДанныеИсточника.ПутьКДаннымВладельцаИсточника;
	...
	Для Каждого Элемент Из Поля Цикл
		...
		Если ИдентификаторСхемыКомпоновкиДанных = "ДанныеПредопределенныеЗначения" Тогда
			...
			ОписаниеИсточника = ОписанияПустыхИсточниковДанных.Добавить();
			Если ЗначениеЗаполнено(ПутьКДаннымВладельцаИсточника) Тогда
				ОписаниеИсточника.Владелец = ДанныеПечати[Объект][ПутьКДаннымВладельцаИсточника];
			ИначеЕсли ПутьКДаннымИсточникаПолей <> "Ссылка" Тогда
				ОписаниеИсточника.Владелец = Объект;
...
Функция ДанныеИсточников(Параметры)
	...
	ДоступныеЗначенияПолей = РезультатКомпоновки.ДоступныеЗначенияПолей;
	...
	Для Каждого ЭлементРасшифровки Из ДанныеРасшифровки.Элементы Цикл
		...
		Если Таблицы.Найти(ИмяТаблицы) <> Неопределено Тогда
			...
			ЗначениеПоля = ЗначениеПоля(ИмяПоля, 
				ПараметрыРасшифровки.СписокПолей[ПараметрыРасшифровки.ИмяПоляРасшифровки], ДоступныеЗначенияПолей);
				
			СтрокаТаблицы.Вставить(ИмяПоля, ЗначениеПоля);
		Иначе
			ИмяПоля = ПараметрыРасшифровки.ИмяПоляРасшифровки;
			...
			Если ИмяПоля = "Номер" И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрефиксацияОбъектов") Тогда
				...
				ДанныеПечати.Вставить(ИмяПоля, ЗначениеПоля(ИмяПоля, Значение, ДоступныеЗначенияПолей));
			Иначе			
				ДанныеПечати.Вставить(ИмяПоля, ЗначениеПоля(ИмяПоля, ПараметрыРасшифровки.СписокПолей[ИмяПоля], ДоступныеЗначенияПолей));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ЗначениеПоля(Знач ИмяПоля, Знач Значение, Знач ДоступныеЗначенияПолей)
	
	Если ТипЗнч(Значение) = Тип("Число") И ТипЗнч(ДоступныеЗначенияПолей[ИмяПоля]) = Тип("СписокЗначений") Тогда
		НайденноеЗначение = ДоступныеЗначенияПолей[ИмяПоля].НайтиПоЗначению(Значение);
...
Функция СкомпоноватьДанные(Параметры)
	...
	Если ИдентификаторСхемыКомпоновкиДанных = "ДанныеПредопределенныеЗначения" Тогда
		...
		Для Каждого ТекущееПоле Из СхемаКомпоновкиДанных.НаборыДанных[0].Поля Цикл
			
			Если ТипЗнч(ТекущееПоле) = Тип("ПолеНабораДанныхСхемыКомпоновкиДанных")
			   И ТекущееПоле.Поле <> "Ссылка"
			   И ЗначениеЗаполнено(ТекущееПоле.ТипЗначения) Тогда
				
				Типы = ТекущееПоле.ТипЗначения.Типы();
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивТипов, Типы);
				
			КонецЕсли;
			
		КонецЦикла;
		
		ДополнительныеПараметры.Вставить("ТипИсточникаПолей", Новый ОписаниеТипов(МассивТипов));
		
	КонецЕсли;
	
	Если ИдентификаторСхемыКомпоновкиДанных = "ОбщиеРеквизиты" Тогда
		
		ПоляКомпоновки = Новый Массив; // Массив из Строка
		Для Каждого ТекущееПоле Из СхемаКомпоновкиДанных.НаборыДанных[0].Поля Цикл
			
			Если ТипЗнч(ТекущееПоле) = Тип("ПолеНабораДанныхСхемыКомпоновкиДанных")
			   И ТекущееПоле.Поле <> "Ссылка" Тогда
				
				ПоляКомпоновки.Добавить(ТекущееПоле.Поле);
				
			КонецЕсли;
			
		КонецЦикла;
		
		ДополнительныеПараметры.Вставить("ПоляКомпоновкиОбщихРеквизитов", ПоляКомпоновки);
	...
	ДоступныеЗначенияПолей = Новый Соответствие;
	...
	Для Каждого ПутьКДанным Из ПоляМакета Цикл
		...
		ДоступныеЗначенияПоля = ДоступныеЗначенияПоля(ПутьКДанным, НастройкиКомпоновщика);
		Если ДоступныеЗначенияПоля <> Неопределено Тогда
			ДоступныеЗначенияПолей.Вставить(ПутьКДанным, ДоступныеЗначенияПоля);
		КонецЕсли;
	КонецЦикла;
	...
	Результат.Вставить("ДоступныеЗначенияПолей", ДоступныеЗначенияПолей);
	
	Возврат	Результат;
	
КонецФункции

Функция ДоступныеЗначенияПоля(ПутьКДанным, НастройкиКомпоновщика)

	ДоступноеПоле = НастройкиКомпоновщика.Отбор.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ПутьКДанным));
	
	Если ДоступноеПоле <> Неопределено Тогда
		ЗначенияСвойств = Новый Структура("ДоступныеЗначения");
		ЗаполнитьЗначенияСвойств(ЗначенияСвойств, ДоступноеПоле);
		
		Если ТипЗнч(ЗначенияСвойств.ДоступныеЗначения) = Тип("СписокЗначений") Тогда
			Возврат ОбщегоНазначения.СкопироватьРекурсивно(ЗначенияСвойств.ДоступныеЗначения);
		КонецЕсли;
	КонецЕсли;
...
Функция ФункцииСРазделениемПараметров()
	...
	ФункцииСРазделениемПараметров.Добавить(ИмяМодуляПечатиСРазделителем + "МаксимумПоКолонке");
	ФункцииСРазделениемПараметров.Добавить(ИмяМодуляПечатиСРазделителем + "МинимумПоКолонке");
	ФункцииСРазделениемПараметров.Добавить(ИмяМодуляПечатиСРазделителем + "СреднееПоКолонке");
...
Функция ИмяТаблицыВОбластиМакета(Макет, Область, Таблицы)
	...
	Для НомерСтроки = Область.Верх По Область.Низ Цикл
		Для НомерСтолбца = 1 По Макет.ШиринаТаблицы Цикл
			...
			ПараметрыВТексте = НайтиПараметрыВТексте(ОбластьЯчейки.Текст);
			
			Для Каждого Параметр Из ПараметрыВТексте Цикл
				Выражение = ОчиститьКвадратныеСкобки(Параметр);
				ЭлементыФормулы =КонструкторФормулСлужебный.ЭлементыФормулы(Выражение);
				
				НайденаФункцияИтогов = Ложь;
				
				Для Каждого Элемент Из ЭлементыФормулы.ВсеЭлементы Цикл
					Если СтрНачинаетсяС(Элемент, ИмяМодуляПечати() + РазделительКоманды())
						И ФункцииСРазделениемПараметров.Найти(Элемент) <> Неопределено Тогда
						НайденаФункцияИтогов = Истина;
						Продолжить;
					КонецЕсли;
					
					Для Каждого Таблица Из Таблицы Цикл
						СтрокаПоиска = Таблица + ".";
						Если СтрНачинаетсяС(Элемент, СтрокаПоиска) Тогда
							Если НайденаФункцияИтогов Тогда
								НайденаФункцияИтогов = Ложь;
							Иначе
								Возврат Таблица;
							КонецЕсли;
...
Функция ЕслиПустое(ПроверяемоеЗначение, ЗначениеЗамены) Экспорт // АПК:299 - обработчик функций конструктора формул
	
	ПроизводитьЗамену = Ложь;
	Если ТипЗнч(ПроверяемоеЗначение) = Тип("Строка") Тогда
		ПроизводитьЗамену = ПустаяСтрока(ПроверяемоеЗначение);
	Иначе
		ПроизводитьЗамену = НЕ ЗначениеЗаполнено(ПроверяемоеЗначение);
	КонецЕсли;
	
	Если ПроизводитьЗамену Тогда
		Возврат ЗначениеЗамены;
	Иначе
...
Функция ЧислоПрописьюПоПараметрам(Число, ФорматнаяСтрока = Неопределено, ИсточникПараметровПрописи = Неопределено) Экспорт // АПК:299 - обработчик функций конструктора формул
	
	Возврат ЧислоПрописью(Число, ФорматнаяСтрока, ИсточникПараметровПрописи);
...
Функция ОписаниеСписковПолей(ИсточникиДанныхПечати, Операторы = Неопределено, ИсточникДанных = Неопределено) Экспорт
	...
	Для Каждого Элемент Из ИсточникиДанныхПечати Цикл
		...
		ИсточникДоступныхПолей.Корневой = Истина;
	КонецЦикла;
...
Функция СхемыКомпоновкиДанныхИсточникаПолей(ИмяИсточникаПолей, ДобавитьОбщиеРеквизиты = Ложь, ТипИсточникаПолей = Неопределено)
	...
	Если ДобавитьОбщиеРеквизиты Тогда
		...
		СхемаКомпоновкиДанных = ПолучитьОбщийМакет("ДанныеПечатиОбщиеРеквизиты");
		Если ОбъектМетаданных <> Неопределено Тогда
			
			ТипОбъекта = СтандартныеПодсистемыСервер.ТипСсылкиИлиКлючаЗаписиОбъектаМетаданных(ОбъектМетаданных);
			ТипыИсточников = Новый Массив; // Массив из Тип
			ТипыИсточников.Добавить(ТипОбъекта);
			ДополнитьСхемуКомпоновкиОбщихПолей(ТипыИсточников, СхемаКомпоновкиДанных);
			
		КонецЕсли;
		
		ИсточникиПолей.Добавить(СхемаКомпоновкиДанных, "ОбщиеРеквизиты");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Валюты") Тогда
		...
		Если МодульРаботаСКурсамиВалют.ЭтоТипДенежнойСуммы(ТипИсточникаПолей) Тогда
			МодульРаботаСКурсамиВалют.ПодключитьИсточникДанныхПечатиЧислоПрописью(ИсточникиПолей, ИмяИсточникаПолей);
		КонецЕсли;
	КонецЕсли;
	
	ПодключитьПредопределенныеЗначения(ИсточникиПолей, ОбъектМетаданных, ТипИсточникаПолей);
...
Процедура ПриПодготовкеДанныхПечати(Объекты, ВнешниеНаборыДанных, ИдентификаторСхемыКомпоновкиДанных, КодЯзыка, ДополнительныеПараметры)
	...
	Если ИдентификаторСхемыКомпоновкиДанных = "ОбщиеРеквизиты" Тогда
		ВнешниеНаборыДанных.Вставить("Данные", ОбщиеРеквизитыПечатныхФорм(Объекты, ДополнительныеПараметры));
		Возврат;
	КонецЕсли;
	...
	Если ИдентификаторСхемыКомпоновкиДанных = "ДанныеПредопределенныеЗначения" Тогда
		ВнешниеНаборыДанных.Вставить("Данные", ДанныеПечатиПредопределенныеЗначения(Объекты, ДополнительныеПараметры));
		Возврат;
	КонецЕсли;
...
Функция ОбщиеРеквизитыПечатныхФорм(Объекты, ДополнительныеПараметры)
	...
	МассивИменИсточников = Новый Массив; // Массив из Тип
	Для Каждого ТекущийОбъект Из Объекты Цикл
		
		ТипОбъекта = ТипЗнч(ТекущийОбъект);
		Если МассивИменИсточников.Найти(ТипОбъекта) = Неопределено Тогда
			
			ТаблицаОбщихПолей = НоваяТаблицаОбщихПолейДляПечати();
			УправлениеПечатьюПереопределяемый.ПриЗаполненииСпискаОбщихПолей(ТипОбъекта, ТаблицаОбщихПолей);
			Для Каждого СтрокаТаблицы Из ТаблицаОбщихПолей Цикл
				
				Идентификатор = СтрокаТаблицы.Идентификатор;
				Идентификатор =
					СтандартныеПодсистемыСервер.ПреобразоватьСтрокуВДопустимоеНаименованиеКолонки(Идентификатор);
				Если НЕ ОбщиеРеквизиты.Свойство(Идентификатор) Тогда
					ОбщиеРеквизиты.Вставить(Идентификатор, СтрокаТаблицы.Значение);
				КонецЕсли;
				
			КонецЦикла;
			МассивИменИсточников.Добавить(ТипОбъекта);
			
		КонецЕсли;
		
	КонецЦикла;
	
	ДополнитьОбщиеРеквизитыПоПолямКомпоновки(ДополнительныеПараметры, ОбщиеРеквизиты);
...
Процедура ДополнитьОбщиеРеквизитыПоПолямКомпоновки(ДополнительныеПараметры, ОбщиеРеквизиты)
	
	Если ДополнительныеПараметры.Свойство("ПоляКомпоновкиОбщихРеквизитов") Тогда
		
		ПоляКомпоновкиОбщихРеквизитов = ДополнительныеПараметры.ПоляКомпоновкиОбщихРеквизитов;
		Для Каждого ИмяПоля Из ПоляКомпоновкиОбщихРеквизитов Цикл
			
			Если НЕ ОбщиеРеквизиты.Свойство(ИмяПоля) Тогда
				ОбщиеРеквизиты.Вставить(ИмяПоля, "");
			КонецЕсли;
...
Процедура ДобавитьКомандыПечати(КомандыПечати, ОбъектМетаданных)
	...
	|	И НЕ МакетыПечатныхФорм.МакетДляВыгрузкиОбъекта
...
Функция ПолучитьДанныеОбласти(ДанныеПечати, ОбъектПечати, Область, ДеревоМакета, КодЯзыка, ПараметрыПечати)
	...
	Если СоответствиеСтрок <> Неопределено Тогда
		Для НомерСтрокиТабличнойЧасти = 1 По СоответствиеСтрок.Количество() Цикл
			...
			СоответствиеСтроки = ОбщегоНазначения.СкопироватьРекурсивно(ИсточникДанных);
...
// 
// Параметры:
//  ЗначенияПараметров - см. УстановитьПараметры.ЗначенияПараметров
...
//  ИмяКолонтитула - Строка - имя колонтитула для установки значения в колонтитул документа
//
Процедура УстановитьПараметрыВДереве(Знач ЗначенияПараметров, ДеревоXML, ДеревоМакета = Неопределено, ДобавлятьСвязи = Истина, ИмяКолонтитула = "")
	
	ТаблицаСортировки = Новый ТаблицаЗначений();
	ТаблицаСортировки.Колонки.Добавить("Ключ");
	ТаблицаСортировки.Колонки.Добавить("Значение");
	ТаблицаСортировки.Колонки.Добавить("Длина");
	Для Каждого СоответствиеЗамены Из ЗначенияПараметров Цикл
		НоваяСтрока = ТаблицаСортировки.Добавить();
	...
	Для Каждого СоответствиеЗамены Из ТаблицаСортировки Цикл
		...
		Для Каждого Узел Из МассивУзлов Цикл
			Если Узел.Строки.Количество() = 0 ИЛИ Узел.ИмяТега = "w:hyperlink" Тогда
				УправлениеПечатьюСлужебный.УстановитьЗначениеВДокумент(Узел, СоответствиеЗамены, ДеревоМакета, ДобавлятьСвязи, ИмяКолонтитула);
			Иначе
...
Функция КоллекцияПолейОбщиеРеквизиты(ТипыИсточников)
	
	СхемаКомпоновкиДанных = ПолучитьОбщийМакет("ДанныеПечатиОбщиеРеквизиты");
	ДополнитьСхемуКомпоновкиОбщихПолей(ТипыИсточников, СхемаКомпоновкиДанных);
	Возврат КонструкторФормул.КоллекцияПолей(СхемаКомпоновкиДанных);

КонецФункции

Процедура ДополнитьСхемуКомпоновкиОбщихПолей(ТипыИсточников, СхемаКомпоновкиДанных)
	
	ТаблицаОбщихПолей = НоваяТаблицаОбщихПолейДляПечати();
	
	Для Каждого ТипОбъекта Из ТипыИсточников Цикл
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОбщихПолей Цикл
			...
			Идентификатор = СтрокаТаблицы.Идентификатор;
			Идентификатор = СтандартныеПодсистемыСервер.ПреобразоватьСтрокуВДопустимоеНаименованиеКолонки(Идентификатор);
			ПутьКДанным = "ОбщиеРеквизиты." + Идентификатор;
			
			Поля = СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Поля; // ПоляНабораДанныхСхемыКомпоновкиДанных
			Если Поля.Найти(ПутьКДанным) = Неопределено Тогда
				
				НовоеПоле = Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
				НовоеПоле.Поле = Идентификатор;
				НовоеПоле.ПутьКДанным = "ОбщиеРеквизиты." + Идентификатор;
				НовоеПоле.Заголовок = СтрокаТаблицы.Представление;
				
				Тип = ТипЗнч(СтрокаТаблицы.Значение);
				МассивТипов = Новый Массив; // Массив из Тип
...
Процедура ОчиститьПоследнийИспользованныйМакет()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекУдалить("Печать", "ПоследнийИспользованныйМакет", ПользователиИнформационнойБазы.ТекущийПользователь().Имя);
	
КонецПроцедуры

Процедура ОбработатьВложенныеОбласти(СтруктураОбластиСтроки, ТекущаяОбласть, ЭлементКоллекции, ДанныеОбъекта, НастройкиФорматаПолей, КодЯзыка);
	Если ТекущаяОбласть.ВложенныеОбласти <> Неопределено Тогда
		ЗначенияУсловий = Новый Соответствие;
		ДанныеПечатиСтрок = Новый Соответствие;
		Для Каждого ДанныеПечатиСтроки Из ЭлементКоллекции Цикл
			ДанныеПечатиСтрок.Вставить(ОчиститьКвадратныеСкобки(ДанныеПечатиСтроки.Ключ), ДанныеПечатиСтроки.Значение);
		КонецЦикла;
		
		УстановитьБезопасныйРежим(Истина);
		Для Каждого УсловиеОбласти Из ТекущаяОбласть.ВложенныеОбласти.ВыгрузитьКолонку("УсловиеОбласти") Цикл
			Если ЗначенияУсловий[УсловиеОбласти] = Неопределено Тогда
				ВыводитьОбласть = ВычислитьВыражение(УсловиеОбласти, ДанныеПечатиСтрок, НастройкиФорматаПолей, КодЯзыка, Ложь);
				ЗначенияУсловий.Вставить(УсловиеОбласти, ВыводитьОбласть);
			КонецЕсли;
		КонецЦикла;
		УстановитьБезопасныйРежим(Ложь);
		
		УзлыДляУдаления = Новый Массив;
		Для Каждого ВложеннаяОбласть Из ТекущаяОбласть.ВложенныеОбласти Цикл
			Если ЗначенияУсловий[ВложеннаяОбласть.УсловиеОбласти] = Ложь Тогда
				УзелОбласти = СтруктураОбластиСтроки.ДеревоОбласти.Строки.Найти(ВложеннаяОбласть.Индекс, "Индекс", Истина);
				Если УзелОбласти <> Неопределено Тогда
					УзелОбласти.Родитель.Строки.Удалить(УзелОбласти);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ЕдинственнаяКнопкаОтправки(ПодчиненныеЭлементы)
	
	Если ПодчиненныеЭлементы.Количество() = 1 
		И ТипЗнч(ПодчиненныеЭлементы[0]) = Тип("КнопкаФормы") Тогда
			Возврат ПодчиненныеЭлементы[0];
	КонецЕсли;
...
Процедура ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ИнтеграцияПодсистемБСП.ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
	УправлениеПечатьюПереопределяемый.ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
	
	ОбработатьКомандыОтправки(Форма);
	
КонецПроцедуры

Процедура ОбработатьКомандыОтправки(Форма) Экспорт
			
	Элементы = Форма.Элементы;
	КнопкаОтправки = ЕдинственнаяКнопкаОтправки(Элементы.ПодменюОтправить.ПодчиненныеЭлементы);
	Если КнопкаОтправки <> Неопределено Тогда
		Элементы.Переместить(КнопкаОтправки, Элементы.ПодменюОтправить.Родитель, Элементы.ПодменюОтправить);
	КонецЕсли;
	
	КнопкаОтправки = ЕдинственнаяКнопкаОтправки(Элементы.ПодменюОтправитьВсеДействия.ПодчиненныеЭлементы);
	Если КнопкаОтправки <> Неопределено Тогда
		Элементы.Переместить(КнопкаОтправки, Элементы.ПодменюОтправитьВсеДействия.Родитель, Элементы.ПодменюОтправитьВсеДействия);
	КонецЕсли;
	
КонецПроцедуры

// Группирует ссылки по одинаковым идентификаторам команд печати
Функция ПараметрыВыполненияДляПечатиПоУмолчанию(Ссылки) Экспорт
	
	// поиск по объектам
	СсылкиПоИдентификаторам = РегистрыСведений.ОсновныеПечатныеФормыОбъектов.ИдентификаторыКоманд(Ссылки);
	
	СсылкиСИдентификаторами = Новый Массив;
	Для Каждого ИдентификаторСсылки Из СсылкиПоИдентификаторам Цикл
		Для Каждого Ссылка Из ИдентификаторСсылки.Значение Цикл
			Если Ссылки.Найти(Ссылка) <> Неопределено Тогда
				СсылкиСИдентификаторами.Добавить(Ссылка);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	// поиск оставшихся по контрагентам
	СсылкиБезИдентификаторов = ОбщегоНазначенияКлиентСервер.РазностьМассивов(Ссылки, СсылкиСИдентификаторами);
	ИдентификаторыБезОбъектов = Новый Массив;
	
	Если ЗначениеЗаполнено(СсылкиБезИдентификаторов) Тогда
		Для Каждого Ссылка Из СсылкиБезИдентификаторов Цикл
			КлючевыеРеквизиты = РегистрыСведений.ОсновныеПечатныеФормыКонтрагентов.КлючевыеРеквизиты();
			УправлениеПечатьюПереопределяемый.ПриОпределенииКлючевыхРеквизитовОсновныхПечатныхФорм(Ссылка, КлючевыеРеквизиты);
			
			Идентификатор = РегистрыСведений.ОсновныеПечатныеФормыКонтрагентов.ИдентификаторКоманды(
				Ссылка, КлючевыеРеквизиты.Организация, КлючевыеРеквизиты.Получатель);
			
			Если Не ПустаяСтрока(Идентификатор) Тогда
				Если СсылкиПоИдентификаторам.Свойство(Идентификатор) Тогда // АПК:1416 структура является хранилищем списка вместо массива
					СсылкиПоИдентификаторам[Идентификатор].Добавить(Ссылка);
				Иначе
					Массив = Новый Массив;
					Массив.Добавить(Ссылка);
					СсылкиПоИдентификаторам.Вставить(Идентификатор, Массив);
					ИдентификаторыБезОбъектов.Добавить(Идентификатор);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого Ссылка Из Ссылки Цикл
		МетаданныеМассив.Добавить(Ссылка.Метаданные());
	КонецЦикла;
	МетаданныеМассив = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МетаданныеМассив);
	
	ВсеКомандыПечати = СоздатьКоллекциюКомандПечати();
	ВсеКомандыПечати.Колонки.Добавить("ИдентификаторБезСпецСимволов");
	ВсеКомандыПечати.Колонки.Добавить("Метаданные");
	
	Для Каждого МетаданныеСсылки Из МетаданныеМассив Цикл
		КомандыПечатиОбъекта = КомандыПечатиОбъекта(МетаданныеСсылки); // @skip-check query-in-loop - Малый цикл
		
		Для Каждого КомандаПечати Из КомандыПечатиОбъекта Цикл
			НоваяКоманда = ВсеКомандыПечати.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяКоманда, КомандаПечати);
			НоваяКоманда.ИдентификаторБезСпецСимволов = УправлениеПечатьюКлиентСервер.ИдентификаторБезСпецСимволов(КомандаПечати.Идентификатор);
			НоваяКоманда.Метаданные = МетаданныеСсылки;
		КонецЦикла;
	КонецЦикла;
	
	ЗначенияРеквизитовСУсловиямиВидимости = ЗначенияРеквизитовСУсловиямиВидимости(ВсеКомандыПечати, Ссылки); 
	
	НовыеПараметрыВыполнения = Новый Массив;
	
	// основной позитивный сценарий
	ПоляОписанияДляЗамены = "Идентификатор,МенеджерПечати,Обработчик,ОбъектыПечати,СразуНаПринтер,ФорматСохранения,
		|ФиксированныйКомплект,ДополнительныеПараметры,ОсновнаяПечатнаяФорма,ЗаменитьОсновнуюПечатнуюФорму,
		|НаименованиеПечатнойФормы,КомандаПоУмолчанию,ИдентификаторИзКомплекта,ПроверкаПроведенияПередПечатью";
	Для Каждого ИдентификаторСсылки Из СсылкиПоИдентификаторам Цикл
		КомандаПечати = ВсеКомандыПечати.Найти(ИдентификаторСсылки.Ключ, "ИдентификаторБезСпецСимволов");
		...
		Для Каждого Ссылка Из ИдентификаторСсылки.Значение Цикл
			Если ПодключаемыеКомандыКлиентСервер.УсловияВыполняются(КомандаПечати.УсловияВидимости, ЗначенияРеквизитовСУсловиямиВидимости[Ссылка]) Тогда
		...
		ПараметрыВыполнения = Новый Структура("ОбъектыПечати,МассивСсылок,ОписаниеКоманды");
		ПараметрыВыполнения.ОбъектыПечати = ОбъектыПечати;
		ПараметрыВыполнения.МассивСсылок = ОбъектыПечати;
		
		НовоеОписание = Новый Структура(ПоляОписанияДляЗамены);
		ЗаполнитьЗначенияСвойств(НовоеОписание, ОписаниеКоманды);
		ЗаменитьОсновнуюПечатнуюФорму = ИдентификаторыБезОбъектов.Найти(ИдентификаторСсылки.Ключ) <> Неопределено;
		НовоеОписание.ЗаменитьОсновнуюПечатнуюФорму = ЗаменитьОсновнуюПечатнуюФорму;
		НовоеОписание.КомандаПоУмолчанию = Истина;
		
		ПараметрыВыполнения.ОписаниеКоманды = НовоеОписание;
		
		НовыеПараметрыВыполнения.Добавить(ПараметрыВыполнения);
	КонецЦикла;
	
	ЕстьПараметрыВыполнения = Булево(НовыеПараметрыВыполнения.Количество());
	
	// печать в первый раз нужно предоставить выбор команды
	Если Не ЕстьПараметрыВыполнения И Ссылки.Количество() = 1 Тогда
		ОписанияКоманд = Новый Структура;
		ОсновныеКоманды = ВсеКомандыПечати.НайтиСтроки(Новый Структура("ОсновнаяПечатнаяФорма", Истина));
		
		КомандыДляВыбора = Новый СписокЗначений();
		Если Не Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			Для Каждого ОписаниеКоманды Из ОсновныеКоманды Цикл
				Идентификатор = УправлениеПечатьюКлиентСервер.ИдентификаторБезСпецСимволов(ОписаниеКоманды.Идентификатор);
				
				Если ЗначениеЗаполнено(ЗначенияРеквизитовСУсловиямиВидимости)
					И Не ПодключаемыеКомандыКлиентСервер.УсловияВыполняются(ОписаниеКоманды.УсловияВидимости, ЗначенияРеквизитовСУсловиямиВидимости[Ссылки[0]]) Тогда
					Продолжить;
				КонецЕсли;
				
				КомандыДляВыбора.Добавить(Идентификатор, ОписаниеКоманды.Представление);
				
				НовоеОписание = Новый Структура(ПоляОписанияДляЗамены);
				ЗаполнитьЗначенияСвойств(НовоеОписание, ОписаниеКоманды);
				НовоеОписание.ЗаменитьОсновнуюПечатнуюФорму = Истина;
				ОписанияКоманд.Вставить(Идентификатор, НовоеОписание);
			КонецЦикла;
		КонецЕсли;
		
		ПараметрыВыполнения = Новый Структура;
		ПараметрыВыполнения.Вставить("КомандыДляВыбора", КомандыДляВыбора);
		ПараметрыВыполнения.Вставить("ОписанияКоманд", ОписанияКоманд);
		
		НовыеПараметрыВыполнения.Добавить(ПараметрыВыполнения);
	КонецЕсли;
	
	Результат = Новый Структура;
	Результат.Вставить("ЕстьПараметрыВыполнения", ЕстьПараметрыВыполнения);
	Результат.Вставить("НовыеПараметрыВыполнения", НовыеПараметрыВыполнения);
	
	Возврат Результат;
	
КонецФункции

Функция ЗначенияРеквизитовСУсловиямиВидимости(ВсеКомандыПечати, Ссылки)
	
	ИнформацияОМетаданных = Новый Соответствие;
	
	Для Каждого Ссылка Из Ссылки Цикл
		МетаданныеСсылки = Ссылка.Метаданные();
		Информация = ИнформацияОМетаданных[МетаданныеСсылки];
		
		Если Информация = Неопределено Тогда
			ИнформацияОМетаданных.Вставить(МетаданныеСсылки, Новый Структура("Реквизиты,Ссылки", Новый Массив, Новый Массив));
		КонецЕсли;
		
		ИнформацияОМетаданных[МетаданныеСсылки].Ссылки.Добавить(Ссылка);
	КонецЦикла;
	
	Для Каждого КомандаПечати Из ВсеКомандыПечати Цикл
		
		Реквизиты = ИнформацияОМетаданных[КомандаПечати.Метаданные].Реквизиты;
		СтандартныеРеквизиты = КомандаПечати.Метаданные.СтандартныеРеквизиты;
		
		Для Каждого Условие Из КомандаПечати.УсловияВидимости Цикл
			РеквизитСуществует = КомандаПечати.Метаданные.Реквизиты.Найти(Условие.Реквизит) <> Неопределено
				Или ОбщегоНазначения.ЭтоСтандартныйРеквизит(СтандартныеРеквизиты, Условие.Реквизит);
			
			Если Реквизиты.Найти(Условие.Реквизит) = Неопределено И РеквизитСуществует Тогда
				Реквизиты.Добавить(Условие.Реквизит);
			КонецЕсли;
		КонецЦикла;
	...
	Результат = Новый Соответствие;
	Для Каждого КлючЗначение Из ИнформацияОМетаданных Цикл
		
		РеквизитыСУсловиями = КлючЗначение.Значение.Реквизиты;
		Если Не ЗначениеЗаполнено(РеквизитыСУсловиями) Тогда
			Продолжить;
		КонецЕсли;
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(КлючЗначение.Значение.Ссылки, РеквизитыСУсловиями);
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(Результат, ЗначенияРеквизитов, Истина);
	КонецЦикла;
...
Процедура СоздатьРеквизитХраненияНастроек(Форма, ТаблицаПодключаемыхКоманд)
	
	ИмяРеквизита = "ПараметрыУправленияПечатью";
	ЗначенияСвойств = Новый Структура(ИмяРеквизита, Null);
	ЗаполнитьЗначенияСвойств(ЗначенияСвойств, Форма);
	
	Результат = ЗначенияСвойств.ПараметрыУправленияПечатью;
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		...
		Отбор = Новый Структура("Вид, Важность", "Печать", "Важное");
		ОсновныеКоманды = ТаблицаПодключаемыхКоманд.НайтиСтроки(Отбор);
		
		Результат = Новый Структура;
		Для Каждого КомандаСПометкой Из ОсновныеКоманды Цикл
			КлючПометки = "ЗначениеПометки" + УправлениеПечатьюКлиентСервер.ИдентификаторБезСпецСимволов(КомандаСПометкой.Идентификатор);
			Результат.Вставить(КлючПометки, Ложь);
		КонецЦикла;
		
		Форма.ПараметрыУправленияПечатью = Результат;
	КонецЕсли;
	
КонецПроцедуры

Функция ВходитВСоставОбъектовСОсновнымиПечатнымиФормами(Тип) Экспорт
	
	Результат = Ложь;
	
	ТипПараметра = ТипЗнч(Тип);
	ТипПолучатель = Метаданные.ОпределяемыеТипы.ОбъектСОсновнымиПечатнымиФормами.Тип;
	
	Если ТипПараметра = Тип("Тип") Тогда
		
		Результат = ТипПолучатель.СодержитТип(Тип);
		
	ИначеЕсли ТипПараметра = Тип("Строка") Тогда
		
		Для Каждого Элемент Из ТипПолучатель.Типы() Цикл
			Если Элемент = Тип("Строка") Тогда // рудимент остается после встраивания, нужно пропустить
				Продолжить;
			КонецЕсли;
			Если ОбщегоНазначения.СтроковоеПредставлениеТипа(Элемент) = Тип Тогда
				Результат = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция ОсновнаяПечатнаяФормаВКомплекте(МассивОбъектов, КомандаПечати) Экспорт
	
	Результат = Новый Массив;
	
	Если ТипЗнч(МассивОбъектов) <> Тип("Массив") Тогда
		Объекты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МассивОбъектов);
	Иначе
		Объекты = МассивОбъектов;
	КонецЕсли;
	
	МассивМетаданных = Новый Массив;
	Для Каждого Элемент Из Объекты Цикл
		МассивМетаданных.Добавить(Элемент.Метаданные());
	КонецЦикла;
	МассивМетаданных = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивМетаданных);
	
	Команды = СоздатьКоллекциюКомандПечати();
	ЗаполнитьКомандыПечатиДляСпискаОбъектов(МассивМетаданных, Команды);
	
	ИдентификаторыКомплекта = СтрРазделить(КомандаПечати.Идентификатор, ",");
	Для Каждого Элемент Из ИдентификаторыКомплекта Цикл
		
		Если СтрНайти(Элемент, ".") Тогда
			ЧастиСтроки = СтрРазделить(Элемент, ".");
			Идентификатор = ЧастиСтроки[ЧастиСтроки.ВГраница()];
		Иначе
		...
		Отбор.Вставить("ОсновнаяПечатнаяФорма", Истина);
		
		КомандыПоиск = Команды.НайтиСтроки(Отбор);
		Для Каждого Команда Из КомандыПоиск Цикл
			Структура = Новый Структура;
			Структура.Вставить("НаименованиеПечатнойФормы", Команда.НаименованиеПечатнойФормы);
			Структура.Вставить("Идентификатор", Идентификатор);
			Структура.Вставить("ОсновнаяПечатнаяФорма", Истина);
...
Функция ПоказыватьПечатьПоУмолчанию(ИмяФормы, СписокОбъектов)
	...
	Если ОбъектМетаданных <> Неопределено И Не Метаданные.ОбщиеФормы.Содержит(ОбъектМетаданных) Тогда
		ОбъектМетаданных = ОбъектМетаданных.Родитель();
	КонецЕсли;
	Если ОбъектМетаданных <> Неопределено И ОбщегоНазначения.ЭтоЖурналДокументов(ОбъектМетаданных) Тогда
		КоллекцияКПроверке = ОбъектМетаданных.РегистрируемыеДокументы
	Иначе
		КоллекцияКПроверке = СписокОбъектов;
	КонецЕсли;
	Если КоллекцияКПроверке <> Неопределено Тогда
		Для Каждого ЭлементКоллекции Из КоллекцияКПроверке Цикл
			ПолноеИмяОбъектаМетаданных = ЭлементКоллекции.ПолноеИмя();
			ЧастиИмениОбъектаМетаданных = СтрРазделить(ПолноеИмяОбъектаМетаданных, ".");
			ПолноеИмяТипаСсылки = ЧастиИмениОбъектаМетаданных[0] + "Ссылка." + ЧастиИмениОбъектаМетаданных[1];
			
			Если ВходитВСоставОбъектовСОсновнымиПечатнымиФормами(ПолноеИмяТипаСсылки) Тогда
				Результат = Истина;
				Прервать
			КонецЕсли;
		КонецЦикла;
...
Процедура УдалитьАбзацыСОчищеннымТекстом(ДеревоПечатнойФормы, ДеревоМакета)
	
	АбзацыМакета = АбзацыДокумента(ДеревоМакета);
	ТекстыАбзацевМакета = Новый Соответствие;
	
	Для Каждого АбзацМакета Из АбзацыМакета Цикл
		ТекстыАбзацевМакета.Вставить(АбзацМакета.Индекс, АбзацМакета.ПолныйТекст);
	КонецЦикла;
	
	АбзацыПечатнойФормы = АбзацыДокумента(ДеревоПечатнойФормы);
	УдаляемыеАбзацы = Новый Массив;
	
	Для Каждого АбзацПечатнойФормы Из АбзацыПечатнойФормы Цикл
		Если АбзацПечатнойФормы.ПолныйТекст = "" Тогда
			ТекстАбзацаМакета = ТекстыАбзацевМакета[АбзацПечатнойФормы.Индекс];
			Если ТекстАбзацаМакета <> "" И Не АбзацСодержитКартинку(АбзацПечатнойФормы)
				И Не АбзацСодержитМестоДляШтампа(АбзацПечатнойФормы) Тогда
				УдаляемыеАбзацы.Добавить(АбзацПечатнойФормы);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Абзац Из УдаляемыеАбзацы Цикл
		Абзац.Родитель.Строки.Удалить(Абзац);
	КонецЦикла;
		
КонецПроцедуры

Функция АбзацыДокумента(ДеревоДокумента)
	
	Результат = Новый Массив;
	
	Для Каждого СекцияДокумент Из ДеревоДокумента.Строки Цикл
		Если СекцияДокумент.ИмяТега <> "w:document" Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого СекцияСодержимое Из СекцияДокумент.Строки Цикл
			Если СекцияСодержимое.ИмяТега <> "w:body" Тогда
				Продолжить;
			КонецЕсли;
			
			Для Каждого СекцияАбзац Из СекцияСодержимое.Строки Цикл
				Если СекцияАбзац.ИмяТега <> "w:p" Тогда
					Продолжить;
				КонецЕсли;
				
				Результат.Добавить(СекцияАбзац);
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Функция АбзацСодержитКартинку(Знач Абзац)

	Для Каждого Элемент Из Абзац.Строки Цикл
		Если ФрагментСодержитКартинку(Элемент) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ФрагментСодержитКартинку(Знач Фрагмент)

	Для Каждого Элемент Из Фрагмент.Строки Цикл
		Если Элемент.ИмяТега = "w:drawing" Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция АбзацСодержитМестоДляШтампа(Знач Абзац)
	
	НайденныеСтроки = НайтиСтрокиПоАтрибуту(Абзац.Строки, "w:name", "V8DSStamp");
	Возврат НайденныеСтроки.Количество() > 0;
	
КонецФункции

Функция НайтиСтрокиПоАтрибуту(Знач Строки, Знач ИмяАтрибута, Знач ЗначениеАтрибута)
	
	Результат = Новый Массив;
	
	Для Каждого Строка Из Строки Цикл
		Если Строка.Атрибуты[ИмяАтрибута] = ЗначениеАтрибута Тогда
			Результат.Добавить(Строка);
		КонецЕсли;
		
		НайденныеСтроки = НайтиСтрокиПоАтрибуту(Строка.Строки, ИмяАтрибута, ЗначениеАтрибута);
...
Функция НайтиСтрокуПоИмениТега(Знач Строки, Знач ИмяТега)
	
	Для Каждого Строка Из Строки Цикл
		Если Строка.ИмяТега = ИмяТега Тогда
			Возврат Строка;
		КонецЕсли;
		
		НайденнаяСтрока = НайтиСтрокуПоИмениТега(Строка.Строки, ИмяТега);
...
Процедура ПодключитьПредопределенныеЗначения(ИсточникиПолей, ОбъектМетаданных, ТипИсточникаПолей)
	
	МассивОбъектовМетаданных = Новый Массив; // Массив из ОбъектМетаданных
	Если ОбъектМетаданных <> Неопределено Тогда
		
		Если ОбъектСОбрабатываемымиПредопределеннымиЗначениями(ОбъектМетаданных) Тогда
			МассивОбъектовМетаданных.Добавить(ОбъектМетаданных);
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(ТипИсточникаПолей) = Тип("ОписаниеТипов") Тогда
		
		МассивТипов = Новый Массив; // Массив из Тип
		Для Каждого Тип Из ТипИсточникаПолей.Типы() Цикл
			
			Если МассивТипов.Найти(Тип) = Неопределено
			   И ОбъектСОбрабатываемымиПредопределеннымиЗначениями(Тип)  Тогда
				
				МассивТипов.Добавить(Тип);
				МассивОбъектовМетаданных.Добавить(Метаданные.НайтиПоТипу(Тип));
	...
	Если МассивОбъектовМетаданных.Количество() > 0 Тогда
		
		СхемаПредопределенныеЗначения = СхемаПредопределенныеЗначения(МассивОбъектовМетаданных);
		Если СхемаПредопределенныеЗначения <> Неопределено Тогда
			ИсточникиПолей.Добавить(СхемаПредопределенныеЗначения, "ДанныеПредопределенныеЗначения");
		КонецЕсли;
...
Функция СхемаПредопределенныеЗначения(МассивОбъектовМетаданных)
	
	СписокПолей = ДеревоПолейДанныхПечати();
	ТаблицаИсточниковСПредопределеннымиЗначениями = НоваяТаблицаПредопределенныхЗначенийИсточников();
	ТаблицаИсточниковПеречислений = НоваяТаблицаПредопределенныхЗначенийИсточников();
	...
	Для Каждого ОбъектМетаданных Из МассивОбъектовМетаданных Цикл
		...
		Если ВидОбъекта = "Справочник"
		 ...
		 ИЛИ ВидОбъекта = "ПланСчетов" Тогда
			НоваяСтрока = ТаблицаИсточниковСПредопределеннымиЗначениями.Добавить();
		Иначе
			НоваяСтрока = ТаблицаИсточниковПеречислений.Добавить();
		КонецЕсли;
		
		ИмяИсточника = ОбъектМетаданных.Имя;
		НоваяСтрока.ИмяИсточника = ИмяИсточника;
		НоваяСтрока.ВидИсточника = ВидОбъекта;
		НоваяСтрока.ИдентификаторИсточника = ВидОбъекта + "_" + ИмяИсточника;
		НоваяСтрока.СинонимИсточника = ОбъектМетаданных.Синоним;
		
	КонецЦикла;
	
	ТаблицаПредопределенныхЗначенийИсточников =
		ПредопределенныеЗначенияИсточников(ТаблицаИсточниковСПредопределеннымиЗначениями, ТаблицаИсточниковПеречислений);
	
	Если ТаблицаПредопределенныхЗначенийИсточников.Количество() > 0 Тогда
		...
		Группа.Идентификатор = "ГруппаПредопределенныеЗначения";
		Группа.Представление = НСтр("ru = 'Предопределенные значения'");
		Группа.Порядок = 1;
		Группа.Папка = Истина;
		
		СчетчикПорядок = 0;
		
		Для Каждого ИсточникПредопределенногоЗначения Из ТаблицаПредопределенныхЗначенийИсточников Цикл
			
			СчетчикПорядок = СчетчикПорядок + 1;
			
			ГруппаТипа = Группа.Строки.Добавить();
			ГруппаТипа.Идентификатор = ИсточникПредопределенногоЗначения.ИдентификаторИсточника;
			ГруппаТипа.Представление = ИсточникПредопределенногоЗначения.СинонимИсточника;
			ГруппаТипа.Порядок = СчетчикПорядок;
		ГруппаТипа.Папка = Истина;
			
			ПредопределенныеЗначения = ИсточникПредопределенногоЗначения.ПредопределенныеЗначения;
			
			Счетчик = 0;
			Для Каждого ТекущееЗначение Из ПредопределенныеЗначения Цикл
				
				Счетчик = Счетчик + 1;
				МассивТипов = Новый Массив; // Массив из Тип
				МассивТипов.Добавить(ТипЗнч(ТекущееЗначение.Значение));
				
				Поле = ГруппаТипа.Строки.Добавить();
				Поле.Идентификатор = ТекущееЗначение.ИдентификаторЗначения;
				Поле.Представление = ТекущееЗначение.Представление;
...
Функция ДанныеПечатиПредопределенныеЗначения(Объекты, ДополнительныеПараметры)
	
	ДанныеПечати = Новый ТаблицаЗначений();
	ДанныеПечати.Колонки.Добавить("Ссылка");
	ТипИсточникаПолей = ДополнительныеПараметры.ТипИсточникаПолей;
	МассивТипов = Новый Массив; // Массив из Тип
	...
	ЗаполнитьТаблицыИсточников(
		МассивТипов,
		ТаблицаИсточниковСПредопределеннымиЗначениями,
		ТаблицаИсточниковПеречислений);
	
	ТаблицаПредопределенныхЗначенийИсточников = ПредопределенныеЗначенияИсточников(
		ТаблицаИсточниковСПредопределеннымиЗначениями,
		ТаблицаИсточниковПеречислений);
	
	ДополнитьДанныеПечатиПредопределеннымиЗначениями(ДанныеПечати, ТаблицаПредопределенныхЗначенийИсточников);
	...
	Для Каждого Объект Из Объекты Цикл
		...
		Для Каждого ИсточникПредопределенногоЗначения Из ТаблицаПредопределенныхЗначенийИсточников Цикл
			...
			ПредопределенныеЗначения = ИсточникПредопределенногоЗначения.ПредопределенныеЗначения;
			Для Каждого ТекущееЗначение Из ПредопределенныеЗначения Цикл
				НоваяСтрока[ТекущееЗначение.ИдентификаторЗначения] = ТекущееЗначение.Значение;
			КонецЦикла;
...
Процедура ДополнитьДанныеПечатиПредопределеннымиЗначениями(ДанныеПечати, ТаблицаПредопределенныхЗначенийИсточников)
	...
	Для Каждого ИсточникПредопределенногоЗначения Из ТаблицаПредопределенныхЗначенийИсточников Цикл
		...
		Для Каждого ТекущееЗначение Из ПредопределенныеЗначения Цикл
			...
			Если ДанныеПечати.Колонки.Найти(ТекущееЗначение.ИдентификаторЗначения) = Неопределено Тогда
				ДанныеПечати.Колонки.Добавить(ТекущееЗначение.ИдентификаторЗначения);
			КонецЕсли;
...
Процедура ЗаполнитьТаблицыИсточников(МассивТипов, ТаблицаИсточниковСПредопределеннымиЗначениями, ТаблицаИсточниковПеречислений)
...
Функция ОбъектСОбрабатываемымиПредопределеннымиЗначениями(ОбъектПроверки)
	
	Результат = Ложь;
	Если ТипЗнч(ОбъектПроверки) = Тип("ОбъектМетаданных") Тогда
		
		Результат = (ОбщегоНазначения.ЭтоСправочник(ОбъектПроверки)
			ИЛИ ОбщегоНазначения.ЭтоПланВидовХарактеристик(ОбъектПроверки)
			ИЛИ ОбщегоНазначения.ЭтоПеречисление(ОбъектПроверки)
			ИЛИ ОбщегоНазначения.ЭтоПланВидовРасчета(ОбъектПроверки)
			ИЛИ ОбщегоНазначения.ЭтоПланСчетов(ОбъектПроверки));
		
	Иначе
		
		Результат = (Справочники.ТипВсеСсылки().СодержитТип(ОбъектПроверки)
			ИЛИ ПланыВидовХарактеристик.ТипВсеСсылки().СодержитТип(ОбъектПроверки)
			ИЛИ Перечисления.ТипВсеСсылки().СодержитТип(ОбъектПроверки)
			ИЛИ ПланыВидовРасчета.ТипВсеСсылки().СодержитТип(ОбъектПроверки)
			ИЛИ ПланыСчетов.ТипВсеСсылки().СодержитТип(ОбъектПроверки))
...
Функция ЗначенияПеречисления(ВидОбъекта, ИмяИсточника)
	
	ТаблицаПредопределенныхЗначений = НоваяТаблицаПредопределенныхЗначений();
	
	Счетчик = 0;
	Для Каждого ЗначениеКоллекции Из Метаданные.Перечисления[ИмяИсточника].ЗначенияПеречисления Цикл
		
		НоваяСтрока = ТаблицаПредопределенныхЗначений.Добавить();
		ИдентификаторЗначения = ИдентификаторЗначенияПоПолномуПути(
			ВидОбъекта,
			ИмяИсточника,
			ЗначениеКоллекции.Имя);
		НоваяСтрока.ИдентификаторЗначения = ИдентификаторЗначения;
		НоваяСтрока.Представление = ЗначениеКоллекции.Синоним;
		НоваяСтрока.Значение = Перечисления[ИмяИсточника][Счетчик];
		Счетчик = Счетчик + 1;
		
	КонецЦикла;
	
	Возврат ТаблицаПредопределенныхЗначений;
	
КонецФункции

Функция ПредопределенныеЗначения(ТаблицаИсточников)
	
	ТаблицаПредопределенныхЗначенийИсточников = НоваяТаблицаПредопределенныхЗначенийИсточников();
	
	Если ТаблицаИсточников.Количество() = 0 Тогда
		
		Возврат ТаблицаПредопределенныхЗначенийИсточников;
		
	КонецЕсли;
	
	ТекстЗапросаШаблон =
	"ВЫБРАТЬ
	|	&НомерИдентификатораИсточника КАК НомерИдентификатораИсточника,
	|	ИсточникДанных.Ссылка КАК Значение,
	|	ИсточникДанных.Представление КАК Представление,
	|	ИсточникДанных.ИмяПредопределенныхДанных КАК ИдентификаторЗначения
	|ИЗ
	|	&ПолноеИмяИсточника КАК ИсточникДанных
	|ГДЕ
	...
	|	Представление
	|ИТОГИ ПО
	|	НомерИдентификатораИсточника";
	
	МассивЗапросов = Новый Массив;
	СчетчикСтрок = 0;
	СоответствиеНомерИдентификатораИсточника = Новый Соответствие;
	Для Каждого ТекущийИсточник Из ТаблицаИсточников Цикл
		
		СчетчикСтрок = СчетчикСтрок + 1;
		ИдентификаторИсточника = ТекущийИсточник.ИдентификаторИсточника;
		СоответствиеНомерИдентификатораИсточника.Вставить(СчетчикСтрок, ИдентификаторИсточника);
		ПолноеИмяИсточника = ТекущийИсточник.ВидИсточника + "." + ТекущийИсточник.ИмяИсточника;
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапросаШаблон, "&ПолноеИмяИсточника", ПолноеИмяИсточника);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НомерИдентификатораИсточника", СчетчикСтрок);
	...
	Для Счетчик = 0 По Граница Цикл
		...
		ВыборкаПоИсточнику = Результат[Счетчик].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Если ВыборкаПоИсточнику.Следующий() Тогда
			
			Выборка = ВыборкаПоИсточнику.Выбрать();
			Если Выборка.Количество() > 0 Тогда
				
				НоваяСтрока = ТаблицаПредопределенныхЗначенийИсточников.Добавить();
				ИдентификаторИсточника =
					СоответствиеНомерИдентификатораИсточника[ВыборкаПоИсточнику.НомерИдентификатораИсточника];
				СтрокаТаблицыИсточников = ТаблицаИсточников.Найти(ИдентификаторИсточника, "ИдентификаторИсточника");
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицыИсточников);
				НоваяСтрока.ПредопределенныеЗначения = ТаблицаПредопределенныхЗначений(Выборка, НоваяСтрока);
...
Функция ТаблицаПредопределенныхЗначений(Выборка, НоваяСтрокаИсточника)
	
	ПредопределенныеЗначения = НоваяТаблицаПредопределенныхЗначений();
	Пока Выборка.Следующий() Цикл
		
		НоваяСтрокаЗначений = ПредопределенныеЗначения.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаЗначений, НоваяСтрокаИсточника);
		ЗаполнитьЗначенияСвойств(НоваяСтрокаЗначений, Выборка);
		ИдентификаторЗначения = ИдентификаторЗначенияПоПолномуПути(
			НоваяСтрокаИсточника.ВидИсточника,
			НоваяСтрокаИсточника.ИмяИсточника,
			НоваяСтрокаЗначений.ИдентификаторЗначения);
			
		НоваяСтрокаЗначений.ИдентификаторИсточника = ИдентификаторЗначения;
...
Функция ПредопределенныеЗначенияИсточников(ТаблицаИсточниковСПредопределеннымиЗначениями, ТаблицаИсточниковПеречислений)
	
	ТаблицаПредопределенныхЗначенийИсточников = ПредопределенныеЗначения(ТаблицаИсточниковСПредопределеннымиЗначениями);
	
	Для Каждого ТекущийИсточник Из ТаблицаИсточниковПеречислений Цикл
		
		ИмяИсточника = ТекущийИсточник.ИмяИсточника;
		ВидИсточника = ТекущийИсточник.ВидИсточника;
		
		НоваяСтрока = ТаблицаПредопределенныхЗначенийИсточников.Добавить();
		НоваяСтрока.ИмяИсточника = ВидИсточника;
		НоваяСтрока.ИмяИсточника = ИмяИсточника;
		НоваяСтрока.СинонимИсточника = ТекущийИсточник.СинонимИсточника;
		НоваяСтрока.ИдентификаторИсточника = ВидИсточника + "_" + ИмяИсточника;
		НоваяСтрока.ПредопределенныеЗначения = ЗначенияПеречисления(ВидИсточника, ИмяИсточника);
...
Функция ИдентификаторЗначенияПоПолномуПути(Знач ВидОбъекта = "", Знач ИмяИсточника = "", Знач ИдентификаторЗначения = "")
	
	МассивИдентификаторов = Новый Массив; // Массив из Строка
	Если НЕ ПустаяСтрока(ВидОбъекта) Тогда
		МассивИдентификаторов.Добавить(ВидОбъекта);
	КонецЕсли;
	Если  НЕ ПустаяСтрока(ИмяИсточника) Тогда
		МассивИдентификаторов.Добавить(ИмяИсточника);
	КонецЕсли;
	Если  НЕ ПустаяСтрока(ИдентификаторЗначения) Тогда
		
		ИдентификаторЗначения =
			СтандартныеПодсистемыСервер.ПреобразоватьСтрокуВДопустимоеНаименованиеКолонки(ИдентификаторЗначения);
		МассивИдентификаторов.Добавить(ИдентификаторЗначения);
		
	КонецЕсли;
	Идентификатор = СтрСоединить(МассивИдентификаторов, "_");
	
	Возврат Идентификатор;
	
КонецФункции

Функция НоваяТаблицаПредопределенныхЗначенийИсточников()
	
	ОписаниеТиповСтрока = Новый ОписаниеТипов("Строка");
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("ИмяИсточника", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("ВидИсточника", ОписаниеТиповСтрока); 
	ТаблицаЗначений.Колонки.Добавить("СинонимИсточника", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("ИдентификаторИсточника", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("ПредопределенныеЗначения", Новый ОписаниеТипов("ТаблицаЗначений"));
	
	Возврат ТаблицаЗначений;
	
КонецФункции

Функция НоваяТаблицаПредопределенныхЗначений()
	
	ОписаниеТиповСтрока = Новый ОписаниеТипов("Строка");
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("ИдентификаторИсточника", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("ИдентификаторЗначения", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("Представление", ОписаниеТиповСтрока);
...
// Возвращаемое значение:
//  ТаблицаЗначений - таблица общих полей для печати:
//   * Идентификатор - Строка - имя поля.
//   * Представление - Строка - заголовок поля.
//   * Значение - Произвольный - значение общего поля.
//
Функция НоваяТаблицаОбщихПолейДляПечати()
	
	ОписаниеТиповСтрока = Новый ОписаниеТипов("Строка");
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("Идентификатор", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("Представление", ОписаниеТиповСтрока);
	ТаблицаЗначений.Колонки.Добавить("Значение");
...
Процедура ДобавитьОписаниеПоля(ОписаниеПолей, ОписаниеПоля)
	
	ЗаполнитьЗначенияСвойств(ОписаниеПолей.Добавить(), ОписаниеПоля);
	
	Для Каждого ОписаниеСхемы Из ОписаниеПоля.ДоступныеСхемы Цикл
		Если ОписаниеСхемы.СхемаКомпоновкиДанных = ОписаниеПоля.СхемаКомпоновкиДанных Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаОписания = ОписаниеПолей.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаОписания, ОписаниеПоля, ,
			"СхемаКомпоновкиДанных, ИдентификаторСхемыКомпоновкиДанных");
		СтрокаОписания.СхемаКомпоновкиДанных = ОписаниеСхемы.СхемаКомпоновкиДанных;
		СтрокаОписания.ИдентификаторСхемыКомпоновкиДанных =
			ОписаниеСхемы.ИдентификаторСхемыКомпоновкиДанных;
	КонецЦикла;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДобавитьРольРедактированиеПечатныхФормВПрофилиСБазовымиПравами");
	Методы.Вставить("СформироватьПечатныеФормыВФоне", Истина);
