...
//   * ПриДобавленииКомандПечати - Булево, Неопределено - при установке значения Истина будет вызвана процедура модуля
//                                                        менеджера объекта ДобавитьКомандыПечати,
...
// Удаляет из дерева значений не используемые атрибуты.
// 
// Параметры:
//  Дерево - см. УправлениеПечатьюСлужебный.ПрочитатьXMLВДерево
//
...
Процедура УстановитьПараметры(ДеревоМакета, СоответствиеЗамен, ПараметрыЗаполненияОбласти) Экспорт
	...
	Если СобратьСтроки Тогда
		СобратьСтрокиУсловныхОбластей(ДеревоДокумента, СтруктураДокумента.Гиперссылки);
	КонецЕсли;
...
Функция СформироватьПечатныеФормыВФоне(ИменаМакетов, Параметры, ПараметрыВывода, ТекущийЯзык, ОбъектыПечати) Экспорт
	...
	Иначе
		...
		ПечатныеФормы = СформироватьПечатныеФормы(Параметры.ИмяМенеджераПечати, ИменаМакетов,
	...
	Если ТипЗнч(Параметры.ПараметрыПечати) = Тип("Структура") И Параметры.ПараметрыПечати.Свойство("ФорматСохранения")
		...
		НайденныйФормат = НастройкиФорматовСохраненияТабличногоДокумента().Найти(ТипФайлаТабличногоДокумента[Параметры.ПараметрыПечати.ФорматСохранения], "ТипФайлаТабличногоДокумента");
	...
	РезультатФормирования = Новый Структура("КоллекцияПечатныхФорм, ПараметрыФоновогоЗадания,ОфисныеДокументы,ОбъектыПечати,ПараметрыВывода");
	РезультатФормирования.КоллекцияПечатныхФорм = Результат;
	РезультатФормирования.ПараметрыВывода = ПараметрыВывода;
	РезультатФормирования.ПараметрыФоновогоЗадания = Параметры;
...
Функция СписокОператоров(ДополнительныеПоля = Неопределено) Экспорт
	...
	ДобавитьОператорВГруппу(Группа, "УправлениеПечатью_СтрокаЛатиницей", НСтр("ru = 'Строка латиницей'"), Новый ОписаниеТипов("Строка"), Истина);
...
Функция СформироватьОфисныйДокумент(Макет, МассивОбъектов, ОбъектыПечати, КодЯзыка, ПараметрыПечати) Экспорт
	...
	СобратьСтрокиУсловныхОбластей(СтруктураДокумента.ДеревоДокумента, СтруктураДокумента.Гиперссылки);
...
Процедура СобратьСтрокиУсловныхОбластей(ДеревоXML, Гиперссылки)
	...
	МассивФрагментовТекста = СтрРазделить(ПолныйТекст, "{",);
	Для Каждого ФрагментТекста Из МассивФрагментовТекста Цикл
		ПозицияИмениТега = СтрНайти(ФрагментТекста, ИмяТега);
		Если ПозицияИмениТега И ПозицияИмениТега < 3 Тогда
			МассивФрагментовСтрок = СтрРазделить(ФрагментТекста, "}");
									
			НоваяСтрока = ТаблицаСортировки.Добавить();
			НоваяСтрока.Ключ 		= "{"+МассивФрагментовСтрок[0]+"}";
	...
	Для Каждого СоответствиеЗамены Из ТаблицаСортировки Цикл
		...
		КонецЦикла;
	КонецЦикла;
	УправлениеПечатьюСлужебный.ВосстановитьПолныйТекст(ДеревоXML, Гиперссылки);
КонецПроцедуры
