...
Процедура ПроверитьРазметкуТабличногоДокументаПоОбъектамПечати(ТабличныйДокумент, ОбъектыПечати, Знач МенеджерПечати, Знач Идентификатор)
	...
	ТекстОшибкиРазметки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		...
		|Используйте процедуру УправлениеПечатью.ЗадатьОбластьПечатиДокумента()
...
Функция ВычислитьВыражение(Знач ИсходноеВыражение, ДанныеПечати, НастройкиФорматаПолей, КодЯзыка)
	...
	ЭлементыФормулы = КонструкторФормулСлужебный.ЭлементыФормулы(Выражение);
	Параметры = Новый Массив;
	ПрименятьФорматирование = ЭлементыФормулы.Операнды.Количество() = 1 И ЭлементыФормулы.Операнды[0] = Выражение;
	Для Каждого Операнд Из ЭлементыФормулы.Операнды Цикл
		КоллекцияДанных = ДанныеПечати;
		
		Если ЭлементыФормулы.Функции.Найти(Операнд) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Формат) И ЭлементыФормулы.Операнды.Количество() = 1 Тогда
				Если ЗначениеЗаполнено(КодЯзыка) Тогда
			...
			Выражение = СтрЗаменить(Выражение, Операнд, "Параметры[" + Параметры.ВГраница() + "]");
		КонецЕсли;
	КонецЦикла;
