...
Процедура ЗаписатьМакет(ИмяОбъектаМетаданныхМакета, АдресМакетаВоВременномХранилище, КодЯзыка = Неопределено) Экспорт
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоLinuxСервер() И СтрДлина(Результат) > 120 Тогда
		ПараметрыДляВставки.ПредставлениеОбъекта = Лев(ПараметрыДляВставки.ПредставлениеОбъекта,
			СтрДлина(ПараметрыДляВставки.ПредставлениеОбъекта) - (СтрДлина(Результат) - 120));
		Результат = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(Шаблон, ПараметрыДляВставки);
	КонецЕсли;
