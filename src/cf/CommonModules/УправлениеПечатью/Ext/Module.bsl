...
Процедура ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Ссылка) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|ожидалось: ЛюбаяСсылка, передано значение: ""%1"" (тип %2)'"), Ссылка, ТипЗнч(Ссылка));
	ОбщегоНазначенияКлиентСервер.Проверить(ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Ссылка)), ТекстСообщения, "УправлениеПечатью.ЗадатьОбластьПечатиДокумента");
...
// или ГенерацияШтрихкодаВызовСервера.ИзображениеШтрихкода из библиотеки 
...
Функция ПечатныеФормыПоОбъектам(ПечатнаяФорма, ОбъектыПечати) Экспорт
	...
	Для Каждого ОбъектПечати Из ОбъектыПечати Цикл
		...
		Результат.Добавить(Новый Структура("Ключ, Значение", ОбъектПечати.Значение, ТабличныйДокумент));
	КонецЦикла;
