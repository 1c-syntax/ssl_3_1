...
Процедура ДобавитьШтампыВОфисныйДокумент(АдресДокумента, ЭлектронныеПодписи) Экспорт
	...
	Для Каждого ДанныеПодписи Из ЭлектронныеПодписи Цикл
		...
		УправлениеПечатьюСлужебный.ДобавитьУзелОписанияРаздела(УзелРазмещенияШтампа, ИндексРазмещения, УзелОтступ);
	КонецЦикла;
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	УстанавливатьПометки = УстанавливатьПометки(КомандыПечати);
...
Процедура ОбработатьКомандыОтправки(Форма)
...
Функция УстанавливатьПометки(КомандыПечати)
	
	Результат = Истина;
	
	Если КомандыПечати.Количество() = 2 Тогда
		КомандаПечатьПоУмолчанию = КомандыПечати.Найти("ПечатьПоУмолчанию", "Идентификатор");
		Если КомандаПечатьПоУмолчанию <> Неопределено Тогда
			Результат = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьАбзацыСОчищеннымТекстом(ДеревоПечатнойФормы, ДеревоМакета)
	...
	Для Каждого АбзацПечатнойФормы Из АбзацыПечатнойФормы Цикл
		Если АбзацПечатнойФормы.ПолныйТекст = "" Тогда
			...
			Если ТекстАбзацаМакета <> "" И Не АбзацСодержитКартинку(АбзацПечатнойФормы) Тогда
