...
Функция ИнициализироватьПечатнуюФорму(Знач УдалитьТипДокумента, Знач УдалитьНастройкиСтраницыМакета = Неопределено, Макет = Неопределено) Экспорт
	...
	Если Макет = Неопределено Тогда
		ВызватьИсключение "ru = 'Необходимо указать значение параметра ""Макет""'";
	КонецЕсли;
...
Функция ИмяФайлаОфисногоДокумента(Знач ИмяФайла, Знач ПереводитьВТранслит = Ложь) Экспорт
	...
	ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ИмяФайла);
	Если Не СтрЗаканчиваетсяНа(ИмяФайла, ".odt") И Не СтрЗаканчиваетсяНа(ИмяФайла, ".docx") Тогда
...
Функция ИменаМакета(Знач ИмяМакета)
	...
	Если СтрНайти(ИмяМакета, "ПФ_DOC_") > 0 
		...
		Если КодЯзыка <> Метаданные.ОсновнойЯзык.КодЯзыка Тогда
			Результат.Добавить(ИмяМакета + "_" + Метаданные.ОсновнойЯзык.КодЯзыка);
		КонецЕсли;
