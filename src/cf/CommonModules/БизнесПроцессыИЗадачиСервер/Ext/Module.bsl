...
Функция ФормаВыполненияЗадачи(Знач ЗадачаСсылка) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("БизнесПроцессыИЗадачи.ФормаВыполненияЗадачи", 
...
Процедура ВыполнитьЗадачу(ЗадачаСсылка, ДействиеПоУмолчанию = Ложь) Экспорт
	...
	НачатьТранзакцию();
	Попытка
		ЗаблокироватьЗадачи(ЗадачаСсылка);
...
Функция ПеренаправитьЗадачи(Знач ПеренаправляемыеЗадачи, Знач ИнфоОПеренаправлении, Знач ТолькоПроверка = Ложь,
	...
	Если ТипЗнч(ИнфоОПеренаправлении) = Тип("Структура") 
	  И (ТипЗнч(ИнфоОПеренаправлении.Исполнитель) = Тип("СправочникСсылка.Пользователи")
	Или ТипЗнч(ИнфоОПеренаправлении.Исполнитель) = Тип("СправочникСсылка.ВнешниеПользователи")) Тогда
		ОписаниеПользователя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИнфоОПеренаправлении.Исполнитель, "Недействителен,ПометкаУдаления");
		
		Если ОписаниеПользователя.Недействителен 
		 Или ОписаниеПользователя.ПометкаУдаления Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Исполнителем задачи выбран недействительный пользователь.'"));
			Возврат Ложь; 
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Для Каждого Задача Из СведенияОЗадачах Цикл
			...
			ПодчиненныеБизнесПроцессы = ВыбратьБизнесПроцессыВедущейЗадачи(Задача.Ключ, Истина).Выбрать();
			УстановитьПривилегированныйРежим(Ложь);
			...
			УстановитьПривилегированныйРежим(Истина);
			//@skip-check query-in-loop - пообъектная запись больших объемов данных.
			ПодчиненныеБизнесПроцессы = БизнесПроцессыГлавнойЗадачи(Задача.Ключ, Истина);
			УстановитьПривилегированныйРежим(Ложь);
...
#Область КомандыРаботыСБизнесПроцессами
...
Процедура СделатьАктивнымБизнесПроцессы(БизнесПроцессы) Экспорт
	...
	НачатьТранзакцию();
	Попытка
		ЗаблокироватьБизнесПроцессы(БизнесПроцессы);
...
Процедура СделатьАктивнымБизнесПроцесс(БизнесПроцесс) Экспорт
	...
	НачатьТранзакцию();
	Попытка
		ЗаблокироватьБизнесПроцессы(БизнесПроцесс);
...
Процедура ПринятьЗадачиКИсполнению(Задачи) Экспорт
	...
	Попытка
		...
		ВыполненныеЗадачи = СтатусыВыполненияЗадачПоИсполнителю(Задачи, Пользователи.ТекущийПользователь());
		...
		Для каждого Задача Из Задачи Цикл
			...
			ИнформацияОЗадаче = ВыполненныеЗадачи.Найти(Задача, "Задача");
			Если ИнформацияОЗадаче = Неопределено Или ИнформацияОЗадаче.Выполнена Тогда
				Продолжить;
			КонецЕсли;
...
#Область ОтложенныйСтартБизнесПроцессов
...
Процедура ПриЗаполненииВидовРегистрируемыхСсылок(ВидыСсылок) Экспорт
	...
	ВидСсылок.Имя = "ИзмененныеГруппыИсполнителейЗадач";
	ВидСсылок.ДопустимыеТипы = Новый ОписаниеТипов("СправочникСсылка.ГруппыИсполнителейЗадач");
	ВидСсылок.ИмяПараметраРаботыРасширений =
		"СтандартныеПодсистемы.БизнесПроцессыИЗадачи.ИзмененныеГруппыИсполнителейЗадачИзменениеПриЗагрузке";
...
Процедура ПриПолученииДанныхОтГлавного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	ПриПолученииДанных(ЭлементДанных, ПолучениеЭлемента);
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	...
	ИсключенияПоискаСсылок.Добавить(Метаданные.Справочники.ГруппыИсполнителейЗадач.ПолноеИмя());
...
Процедура ПриДобавленииКомандСозданияНаОсновании(Объект, КомандыСозданияНаОсновании, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Если МодульРаботаСФайламиСлужебный.ЭтоСправочникФайлы(Объект) Тогда
			БизнесПроцессы.Задание.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
		КонецЕсли;
	КонецЕсли;
...
#Область МониторингИКонтрольИсполнения
...
Функция ИсполнителиЗадач(Задачи)
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		ИсполнителиЗадачи = Результат.ПоЗадачам[ВыборкаЗапроса.ЗадачаИсполнителя];
		Если ИсполнителиЗадачи = Неопределено Тогда
			ИсполнителиЗадачи = Новый Массив;
			Результат.ПоЗадачам[ВыборкаЗапроса.ЗадачаИсполнителя] = ИсполнителиЗадачи;
		КонецЕсли;
...
Функция СписокПросроченныхЗадач()
	...
	ВыборкаБизнесПроцессов = ВыбратьБизнесПроцессыВедущихЗадач(ПросроченныеЗадачи.ВыгрузитьКолонку("Ссылка"));
	Индекс = ПросроченныеЗадачи.Количество() - 1;
	Отбор = Новый Структура("ВедущаяЗадача"); 
	Пока Индекс >= 0 Цикл
		...
		Если ЗначениеЗаполнено(ПросроченнаяЗадача.Исполнитель) Тогда
			Продолжить;
		КонецЕсли;
		ВыборкаБизнесПроцессов.Сбросить();
		Отбор.ВедущаяЗадача = ПросроченнаяЗадача.Ссылка; 
		Если ВыборкаБизнесПроцессов.НайтиСледующий(Отбор) Тогда
...
Функция ПисьмаОПросроченныхЗадачах(ПросроченныеЗадачи)
	...
	Для Каждого ПросроченнаяЗадача Из ПросроченныеЗадачи Цикл
		...
		ДобавитьПолучателяПисьма(ПолучателиПисем, ПросроченнаяЗадача.Автор, ПросроченнаяЗадачаСсылка, НСтр("ru = 'Автор'"));
		Если ЗначениеЗаполнено(ПросроченнаяЗадача.Исполнитель) Тогда
			ДобавитьПолучателяПисьма(ПолучателиПисем, ПросроченнаяЗадача.Исполнитель, ПросроченнаяЗадачаСсылка, НСтр("ru = 'Исполнитель'"));
		Иначе
			...
			Для Каждого Координатор Из Координаторы Цикл
				ДобавитьПолучателяПисьма(ПолучателиПисем, Координатор, ПросроченнаяЗадачаСсылка, НСтр("ru = 'Координатор'"));
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	ОписаниеПользователей = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ПолучателиПисем, "Недействителен,ПометкаУдаления");
	
	Позиция = ПолучателиПисем.ВГраница();
	Пока Позиция >= 0 Цикл
		ОписаниеПользователя = ОписаниеПользователей[ПолучателиПисем[Позиция]];
		Если ОписаниеПользователя = Неопределено
			Или ОписаниеПользователя.Недействителен
			Или ОписаниеПользователя.ПометкаУдаления Тогда
			ПолучателиПисем.Удалить(Позиция);
		КонецЕсли;
		Позиция = Позиция - 1;
	КонецЦикла;
...
Процедура ДобавитьПолучателяПисьма(ПолучателиПисем, ПолучательПисьма, СсылкаНаОбъект, Детали = "")
	
	Если Не ЗначениеЗаполнено(ПолучательПисьма) Тогда
		ТекстЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru ='У задачи %1 не заполнен %2. Задача не может быть обработана.
			|Необходимо заполнить обязательные реквизиты или удалить задачу.'"),
					СсылкаНаОбъект, Детали);
		...
		ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка,МетаданныеОбъекта, СсылкаНаОбъект, ТекстЖурнала);
		Возврат;
		
	КонецЕсли;
	
	ПолучателиПисем.Добавить(ПолучательПисьма);
...
Процедура ОтправитьУведомлениеОПросроченноеЗадаче(Письмо)
	...
	МодульРаботаСПочтовымиСообщениямиСлужебныйКлиентСервер = ОбщегоНазначения.ОбщийМодуль(
	...
	Исключение
		...
		Если Не МодульРаботаСПочтовымиСообщениямиСлужебныйКлиентСервер.ЭтоОшибкаРаботыИнтернетПочты(
...
// Параметры:
//  Задача - Массив из ЗадачаСсылка - проверяемые задачи.
//  Исполнитель - СправочникСсылка.Пользователи, 
//                СправочникСсылка.ВнешниеПользователи - исполнитель для проверки.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Задача - ЗадачаСсылка - задача
//   * Выполнена - статус выполнения, если пользователь является исполнителем или входит в роль исполнителей.
//
Функция СтатусыВыполненияЗадачПоИсполнителю(Задачи, Исполнитель)
	...
	|		ИЛИ ИсполнителиЗадач.Исполнитель = &Исполнитель)";
...
Функция ВыбратьБизнесПроцессыВедущейЗадачи(ЗадачаСсылка, ДляИзменения = Ложь)
...
Функция ВыбратьБизнесПроцессыВедущихЗадач(Задачи, ДляИзменения = Ложь)
	
	ШаблонЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	Таблица.Ссылка КАК Ссылка,
		|	Таблица.ВедущаяЗадача КАК ВедущаяЗадача
		|ИЗ
		|	&ИмяТаблицы КАК Таблица
		|ГДЕ
		|	Таблица.ВедущаяЗадача В (&ВедущиеЗадачи)";
	ТекстыЗапросов = Новый Массив;
	Для Каждого ТипБизнесПроцесса Из Метаданные.ОпределяемыеТипы.БизнесПроцесс.Тип.Типы() Цикл
		...
		Если ДляИзменения Тогда
			Блокировка = Новый БлокировкаДанных;
			Для каждого ЗадачаСсылка Из Задачи Цикл
				ЭлементБлокировки = Блокировка.Добавить(МетаданныеБизнесПроцесса.ПолноеИмя());
				ЭлементБлокировки.УстановитьЗначение("ВедущаяЗадача", ЗадачаСсылка);
			КонецЦикла;
			Блокировка.Заблокировать();
	...
	Запрос = Новый Запрос(СтрСоединить(ТекстыЗапросов, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС));
	Запрос.УстановитьПараметр("ВедущиеЗадачи", Задачи);
...
Процедура ПриИзмененииСостоянияЗадач(Задачи, СтароеСостояние, НовоеСостояние)
	...
	ШаблонЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВложенныеБизнесПроцессы.Ссылка КАК Ссылка
		|ИЗ
		|	#БизнесПроцессы КАК ВложенныеБизнесПроцессы
		|ГДЕ
		|  ВложенныеБизнесПроцессы.ВедущаяЗадача В (&Задачи)
		|  И ВложенныеБизнесПроцессы.ПометкаУдаления = ЛОЖЬ
		|	И ВложенныеБизнесПроцессы.Завершен = ЛОЖЬ";
	...
	ШаблонЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПодчиненныеБизнесПроцессы.Ссылка КАК Ссылка
		|ИЗ
		|	#БизнесПроцессы КАК ПодчиненныеБизнесПроцессы
		|ГДЕ
		|   ПодчиненныеБизнесПроцессы.ГлавнаяЗадача В (&Задачи)
		|   И ПодчиненныеБизнесПроцессы.ПометкаУдаления = ЛОЖЬ
		| 	И ПодчиненныеБизнесПроцессы.Завершен = ЛОЖЬ";
	ТекстыЗапроса = Новый Массив;
...
Процедура ПриПолученииДанных(ЭлементДанных, ПолучениеЭлемента)
	...
	Если ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ИсполнителиЗадач") Тогда
		ЗарегистрироватьИзменениеИсполнителейЗадачПриПолученииДанных(ЭлементДанных);
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры ПриПолученииДанных.
Процедура ЗарегистрироватьИзменениеИсполнителейЗадачПриПолученииДанных(ЭлементДанных)
	...
	Группы = ЭлементДанных.ИзмененныеГруппыИсполнителейЗадач();
	Если Не ЗначениеЗаполнено(Группы) Тогда
		Возврат;
	КонецЕсли;
	
	ПользователиСлужебный.ЗарегистрироватьСсылки("ИзмененныеГруппыИсполнителейЗадач", Группы);
...
Процедура ОбновитьВспомогательныеДанныеЭлементовИзмененныхПриПолученииДанных()
	...
	ИмяВидаСсылок = "ИзмененныеГруппыИсполнителейЗадач";
	
	Группы = ПользователиСлужебный.ЗарегистрированныеСсылки(ИмяВидаСсылок);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		...
		МодульУправлениеДоступомСлужебный.ОбновитьПользователейГруппИсполнителей(Группы);
	КонецЕсли;
...
Функция СистемнаяУчетнаяЗаписьНастроена(ОписаниеОшибки)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		ОписаниеОшибки = НСтр("ru = 'Отправка почты не предусмотрена в приложении.'");
	Иначе
		МодульРаботаСПочтовымиСообщениями = ОбщегоНазначения.ОбщийМодуль("РаботаСПочтовымиСообщениями");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьОтветственногоЗаКонтрольИсполнения");
	Методы.Вставить("ОбновитьИспользованиеРегламентныхЗаданий");
	Методы.Вставить("ЗаполнитьНаименованиеПредопределенногоЭлементаВсеОбъектыАдресации");
