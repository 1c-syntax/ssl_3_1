...
Функция ОсвободитьФайлы(Знач Файлы) Экспорт
	...
	Для Каждого ПрисоединенныйФайл Из Файлы Цикл
		...
		ДанныеФайла = ДанныеФайла(ПрисоединенныйФайл);
...
Функция ДанныеФайловДляПодписания(Знач Файлы, ИдентификаторФормы = Неопределено) Экспорт
	...
	Если ТипЗнч(Файлы) <> Тип("Массив") Тогда
		Возврат ДанныеФайла(Файлы,,ИдентификаторФормы);
	КонецЕсли;
	
	ДанныеФайлов = Новый Массив;
	Для Каждого Файл Из Файлы Цикл
		ДанныеФайлов.Добавить(ДанныеФайла(Файл,,ИдентификаторФормы));
	КонецЦикла;
...
Функция КоличествоПрисоединенныхФайлов(ВладелецФайлов, ВозвращатьДанныеФайлов = Ложь, РеквизитРазмещения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСправочникаХранения) Тогда
		...
		Если ВозвращатьДанныеФайлов И КоличествоФайлов > 0 Тогда
			...
			Иначе
				ФайлыВладельца.ДанныеФайла = ДанныеФайла(ВладелецФайлов[РеквизитРазмещения], , , Ложь);
			КонецЕсли;
		КонецЕсли;
...
Функция ДанныеФайлаИНавигационнаяСсылкаВсехВерсийФайла(ФайлСсылка, ИдентификаторФормы) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеФайла = ДанныеФайла(ФайлСсылка, ВерсияСсылка, , Ложь);
