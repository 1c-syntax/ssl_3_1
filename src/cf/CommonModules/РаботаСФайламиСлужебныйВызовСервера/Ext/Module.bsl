...
//
Функция ЗанятьФайл(ДанныеФайла, СтрокаОшибки = "", 
	Знач УникальныйИдентификатор = Неопределено, Знач Пользователь = Неопределено) Экспорт
...
Функция ОбновитьОбъектФайла(ФайлСсылка, СведенияОФайле, ВерсияСсылка = Неопределено, 
	...
	ТипХраненияФайла = РаботаСФайламиСлужебный.ТипХраненияФайла(Расширение, СведенияОФайле.Размер);
...
Функция ЗанятьФайлДляРедактирования(ФайлСсылка, УникальныйИдентификатор = Неопределено, 
	...
	Если Не ЗначениеЗаполнено(Результат.ДанныеФайла.Редактирует) 
		И Не ЗанятьФайл(Результат.ДанныеФайла, Результат.ТекстСообщения, УникальныйИдентификатор) Тогда
		Возврат Результат;
	КонецЕсли;
...
//     * Путь                         - Неопределено
//     * Размер                       - Неопределено
//     * ДатаПомещенияВРабочийКаталог - Неопределено
//     * НаЧтение                     - Неопределено
//
...
Процедура СохранитьРабочийКаталогПапкиИЗаменитьПутиВРегистре(СсылкаПапки,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураЗаписи.Файл = Выборка.Файл;
		СтруктураЗаписи.Путь = НовыйПуть;
		СтруктураЗаписи.Размер = Выборка.Размер;
		СтруктураЗаписи.ДатаПомещенияВРабочийКаталог = Выборка.ДатаПомещенияВРабочийКаталог;
		СтруктураЗаписи.НаЧтение = Выборка.НаЧтение;
	...
	Для Каждого СтруктураЗаписи Из СписокДляЗамены Цикл
		...
		СтруктураДляЗаписиВРегистр = СтруктураЗаписи;// см. СтруктураЗаписи
		ЗаписатьСтруктуруЗаписиВРегистр(
			СтруктураДляЗаписиВРегистр.Файл,
			СтруктураДляЗаписиВРегистр.Путь,
			СтруктураДляЗаписиВРегистр.Размер,
			СтруктураДляЗаписиВРегистр.ДатаПомещенияВРабочийКаталог,
			СтруктураДляЗаписиВРегистр.НаЧтение,
...
Процедура ОбновитьРазмерФайлаИВерсии(ДанныеФайла, РазмерФайла, УникальныйИдентификатор) Экспорт
	...
	Попытка
		...
		Блокировка.Заблокировать();
		ЗаблокироватьДанныеДляРедактирования(ДанныеФайла.Версия, , УникальныйИдентификатор);
		...
		РазблокироватьДанныеДляРедактирования(ДанныеФайла.Версия, УникальныйИдентификатор);
		РазблокироватьДанныеДляРедактирования(ДанныеФайла.Ссылка, УникальныйИдентификатор);
