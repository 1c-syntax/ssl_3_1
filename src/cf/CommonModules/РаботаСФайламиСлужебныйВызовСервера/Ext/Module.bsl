...
Функция РезультатУдаленияФайлов(ФайлыИлиВерсии, УникальныйИдентификатор) Экспорт
	...
	Для Каждого ФайлИлиВерсия Из ФайлыИлиВерсии Цикл
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлИлиВерсия, "Автор") <> АвторизованныйПользователь Тогда
			...
			Результат.ТекстПредупреждения = НСтр("ru = 'Только автор может удалить версию файла.'");
...
Процедура ПроверитьПодписи(ИсходныеДанные, ДанныеСтрок, ПодписанныйОбъект) Экспорт
	...
	Для Каждого СтрокаПодписи Из ДанныеСтрок Цикл
		...
		ОписаниеОшибки = "";
		РезультатПроверки = МодульЭлектроннаяПодпись.ПроверитьПодпись(Неопределено, ИсходныеДанные, СтрокаПодписи.АдресПодписи,
			ОписаниеОшибки, СтрокаПодписи.ДатаПодписи, РезультатПроверкиПодписи);
		...
		Если ЗначениеЗаполнено(СтрокаПодписи.РезультатПроверкиПодписиПоМЧД) И ОбщегоНазначения.ПодсистемаСуществует(
			...
			Для Каждого Результат Из РезультатПроверкиДоверенностей Цикл
				РезультатПроверкиПодписиПоМЧД = МодульМашиночитаемыеДоверенностиФНССлужебный.РезультатПроверкиПодписиПоМЧД(
					Результат.МашиночитаемаяДоверенность, ПодписанныйОбъект,
					РезультатПроверкиПодписи.Сертификат, СтрокаПодписи.ДатаПодписи);
...
Функция ПроверитьПодписиПоМЧД(Подписи, ПодписанныйОбъект) Экспорт
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		РезультатПроверки = Новый Структура("Индекс, РезультатПроверкиПодписиПоМЧД", Подпись.Индекс, НовыйРезультатПроверкиДоверенностей);
		МодульМашиночитаемыеДоверенностиФНССлужебный.ДобавитьИнформациюМЧД(РезультатПроверки, Подпись.РезультатПроверкиПодписиПоМЧД);
...
Функция ПараметрыЛогирования(ИдентификаторКлиента) Экспорт
	Результат = Новый Структура;
	Результат.Вставить("ДатаНачалаЛогированияСканирования", 
		ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("КомпонентаСканирования", "ДатаНачалаЛогированияСканирования", 
		Дата(1,1,1)));
	...
	Результат.Вставить("КаталогЖурналаСканирования",
		ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиСканирования/КаталогЖурналаСканирования", 
		ИдентификаторКлиента, Неопределено));
	Результат.Вставить("ИспользоватьКаталогЖурналаСканирования",
		ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиСканирования/ИспользоватьКаталогЖурналаСканирования", 
...
//   * ИмяЛогФайла - Строка - имя логфайла, используемое компонентой сканирования
//
Функция ТехническаяИнформация() Экспорт
	...
	Результат.Вставить("ИмяЛогФайла",
		ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("КомпонентаСканирования", "ИмяЛогФайла"));
	Возврат Результат;
КонецФункции
...
Процедура УстановитьПараметрыНачалаЛогирования(ИмяЛогФайла) Экспорт
	...
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("КомпонентаСканирования", "ДатаНачалаЛогированияСканирования", 
		ТекущаяДатаСеанса());
КонецПроцедуры
