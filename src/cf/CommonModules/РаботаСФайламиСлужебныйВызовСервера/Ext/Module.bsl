...
Функция ДанныеФайлаИДвоичныеДанные(ФайлИлиВерсияСсылка, АдресПодписи = Неопределено, ИдентификаторФормы = Неопределено) Экспорт
	...
	Иначе
		ВерсияСсылка = ФайлИлиВерсияСсылка;
		ДанныеФайла = ДанныеФайла(ФайлИлиВерсияСсылка.Владелец, ВерсияСсылка, ПараметрыДанныхФайла);
	КонецЕсли;
...
Функция СоздатьФайлСВерсией(ВладелецФайла, СведенияОФайле) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(СведенияОФайле.Кодировка) Тогда
			СведенияОФайле.Кодировка = РаботаСФайламиСлужебныйКлиентСервер.ОпределитьКодировкуДвоичныхДанных(СведенияОФайле.АдресВременногоХранилищаФайла, СведенияОФайле.РасширениеБезТочки);
		КонецЕсли;
...
Функция ЗанятьФайл(ДанныеФайла, СтрокаОшибки = "", ДополнительныеПараметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(ДополнительныеПараметры.ДополнительныеСвойства) = Тип("Структура") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ФайлОбъект.ДополнительныеСвойства, ДополнительныеПараметры.ДополнительныеСвойства, Ложь);
		КонецЕсли;
...
Функция КоличествоПрисоединенныхФайлов(ВладелецФайлов, ВозвращатьДанныеФайлов = Ложь, РеквизитРазмещения = Неопределено) Экспорт
...
Функция КонвертироватьНастройкиСканирования(ВходящиеНастройки) Экспорт
	НастройкиПеречислениями = Новый Структура("Разрешение, Цветность, Поворот, РазмерБумаги, СжатиеTIFF");
	Результат = Новый Структура("Разрешение, Цветность, Поворот, РазмерБумаги, СжатиеTIFF");
	
	ЗаполнитьЗначенияСвойств(НастройкиПеречислениями, ВходящиеНастройки);
	
	Результат.Разрешение = -1;
	Если НастройкиПеречислениями.Разрешение = Перечисления.РазрешенияСканированногоИзображения.dpi200 Тогда
		Результат.Разрешение = 200; 
	ИначеЕсли НастройкиПеречислениями.Разрешение  = Перечисления.РазрешенияСканированногоИзображения.dpi300 Тогда
		Результат.Разрешение = 300;
	ИначеЕсли НастройкиПеречислениями.Разрешение  = Перечисления.РазрешенияСканированногоИзображения.dpi600 Тогда
		Результат.Разрешение = 600;
	ИначеЕсли НастройкиПеречислениями.Разрешение  = Перечисления.РазрешенияСканированногоИзображения.dpi1200 Тогда
		Результат.Разрешение = 1200;
	КонецЕсли;
	
	Результат.Цветность = -1;
	Если НастройкиПеречислениями.Цветность = Перечисления.ЦветностиИзображения.Монохромное Тогда
		Результат.Цветность = 0;
	ИначеЕсли НастройкиПеречислениями.Цветность = Перечисления.ЦветностиИзображения.ГрадацииСерого Тогда
		Результат.Цветность = 1;
	ИначеЕсли НастройкиПеречислениями.Цветность = Перечисления.ЦветностиИзображения.Цветное Тогда
		Результат.Цветность = 2;
	КонецЕсли;
	
	Результат.Поворот = 0;
	Если НастройкиПеречислениями.Поворот = Перечисления.СпособыПоворотаИзображения.НетПоворота Тогда
		Результат.Поворот = 0;
	ИначеЕсли НастройкиПеречислениями.Поворот  = Перечисления.СпособыПоворотаИзображения.ВправоНа90 Тогда
		Результат.Поворот = 90;
	ИначеЕсли НастройкиПеречислениями.Поворот  = Перечисления.СпособыПоворотаИзображения.ВправоНа180 Тогда
		Результат.Поворот = 180;
	ИначеЕсли НастройкиПеречислениями.Поворот  = Перечисления.СпособыПоворотаИзображения.ВлевоНа90 Тогда
		Результат.Поворот = 270;
	КонецЕсли;
	
	Результат.РазмерБумаги = 0;
	Если НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.НеЗадано Тогда
		Результат.РазмерБумаги = 0;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.A3 Тогда
		Результат.РазмерБумаги = 11;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.A4 Тогда
		Результат.РазмерБумаги = 1;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.A5 Тогда
		Результат.РазмерБумаги = 5;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.B4 Тогда
		Результат.РазмерБумаги = 6;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.B5 Тогда
		Результат.РазмерБумаги = 2;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.B6 Тогда
		Результат.РазмерБумаги = 7;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.C4 Тогда
		Результат.РазмерБумаги = 14;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.C5 Тогда
		Результат.РазмерБумаги = 15;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.C6 Тогда
		Результат.РазмерБумаги = 16;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.USLetter Тогда
		Результат.РазмерБумаги = 3;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.USLegal Тогда
		Результат.РазмерБумаги = 4;
	ИначеЕсли НастройкиПеречислениями.РазмерБумаги = Перечисления.РазмерыБумаги.USExecutive Тогда
		Результат.РазмерБумаги = 10;
	КонецЕсли;
	
	Результат.СжатиеTIFF = 6; // БезСжатия
	Если НастройкиПеречислениями.СжатиеTIFF = Перечисления.ВариантыСжатияTIFF.LZW Тогда
		Результат.СжатиеTIFF = 2;
	ИначеЕсли НастройкиПеречислениями.СжатиеTIFF = Перечисления.ВариантыСжатияTIFF.RLE Тогда
		Результат.СжатиеTIFF = 5;
	ИначеЕсли НастройкиПеречислениями.СжатиеTIFF = Перечисления.ВариантыСжатияTIFF.БезСжатия Тогда
		Результат.СжатиеTIFF = 6;
	ИначеЕсли НастройкиПеречислениями.СжатиеTIFF = Перечисления.ВариантыСжатияTIFF.CCITT3 Тогда
		Результат.СжатиеTIFF = 3;
	ИначеЕсли НастройкиПеречислениями.СжатиеTIFF = Перечисления.ВариантыСжатияTIFF.CCITT4 Тогда
...
Функция ОбновитьОбъектФайла(ФайлСсылка, СведенияОФайле, ВерсияСсылка = Неопределено, 
	...
	Попытка
		...
		Если СведенияОФайле.ХранитьВерсии Тогда
			РаботаСФайламиСлужебный.ОбновитьВерсиюВФайле(ФайлСсылка, Версия.Ссылка, СведенияОФайле.АдресВременногоХранилищаТекста, УникальныйИдентификаторФормы);
		Иначе
			РаботаСФайламиСлужебный.ОбновитьТекстВФайле(ФайлСсылка, СведенияОФайле.АдресВременногоХранилищаТекста, УникальныйИдентификаторФормы);
		КонецЕсли;
...
Функция ЗанятьФайлДляРедактирования(ФайлСсылка, УникальныйИдентификатор = Неопределено, 
	...
	Результат.ДанныеФайла = ДанныеФайлаДляОткрытия(ФайлСсылка, ВерсияСсылка, УникальныйИдентификатор, РабочийКаталогВладельца);
	...
	Если Не ЗначениеЗаполнено(Результат.ДанныеФайла.Редактирует) 
		И Не ЗанятьФайл(Результат.ДанныеФайла, Результат.ТекстСообщения, ПараметрыЗанятияФайла) Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция ДанныеФайлаИРабочийКаталог(ФайлИлиВерсияСсылка, РабочийКаталогВладельца = Неопределено) Экспорт
	...
	Иначе
		...
		ВозможностьХранитьВерсии = ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия", ФайлИлиВерсияСсылка.Метаданные());
	КонецЕсли;
	
	ТекущаяВерсия = ?(Не ВозможностьХранитьВерсии, Неопределено, 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлСсылка, "ТекущаяВерсия"));
	...
	Если РабочийКаталогВладельца <> "" Тогда
		...
		Если ВерсияСсылка <> Неопределено Тогда
			ПолноеИмяФайлаВРабочемКаталоге = ПолноеИмяФайлаВРабочемКаталоге(ВерсияСсылка, ИмяКаталога, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
		ИначеЕсли ВозможностьХранитьВерсии И ТекущаяВерсия <> Неопределено Тогда
			ПолноеИмяФайлаВРабочемКаталоге = ПолноеИмяФайлаВРабочемКаталоге(ТекущаяВерсия, ИмяКаталога, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
		Иначе
			ПолноеИмяФайлаВРабочемКаталоге = ПолноеИмяФайлаВРабочемКаталоге(ФайлСсылка, ИмяКаталога, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
		КонецЕсли;
...
Процедура СохранитьРабочийКаталогПапкиИЗаменитьПутиВРегистре(СсылкаПапки, РабочийКаталогПапки, 
	...
	Запрос.УстановитьПараметр("Путь", 
		ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(ИмяКаталогаПрежнееЗначение) + "%");
	УстановитьПривилегированныйРежим(Истина);
	...
	Для Каждого ЭлементСписка Из СписокДляЗамены Цикл
		...
		ЗаписатьСтруктуруЗаписиВРегистр(
			ФайлВРабочемКаталоге.Файл,
			ФайлВРабочемКаталоге.Путь,
			ФайлВРабочемКаталоге.Размер,
			ФайлВРабочемКаталоге.ДатаПомещенияВРабочийКаталог,
			ФайлВРабочемКаталоге.НаЧтение,
...
Функция ДанныеФайлаИНавигационнаяСсылкаВсехВерсийФайла(ФайлСсылка, ИдентификаторФормы) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДанныеФайла.ПометкаУдаления = Ложь
			И Не ПрисоединенныйФайлНаходитсяНаДиске(ВерсияСсылка) Тогда
...
Функция РезультатУдаленияДанных(ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлИлиВерсия, "Автор") <> АвторизованныйПользователь Тогда
		...
		Результат.ТекстПредупреждения = ?(ЭтоВерсияФайла,
			НСтр("ru = 'Только автор может удалить версию файла.'"),
...
Процедура УдалитьДанныеФайла(ФайлИлиВерсия, УникальныйИдентификатор, Результат, АвторизованныйПользователь)
	...
	Если ВерсияДругогоАвтора <> Неопределено Тогда
		РаботаСФайламиСлужебный.ОбновитьВерсиюВФайле(
			ФайлИлиВерсия, ВерсияДругогоАвтора, Неопределено, УникальныйИдентификатор);
	Иначе
		Результат.Файлы.Добавить(ФайлИлиВерсия);
		УдалитьДанные(ФайлИлиВерсия, УникальныйИдентификатор);
	КонецЕсли;
...
Функция ПараметрыДобавленияИзображения(ВладелецФайлов, РеквизитРазмещения = Неопределено) Экспорт
	...
	ПараметрыДобавления.Вставить("ФайлыВладельца" , КоличествоПрисоединенныхФайлов(ВладелецФайлов, Истина, РеквизитРазмещения));
...
///////////////////////////////////////////////////////////////////////////////////
// Печать офисного документа со штампом электронной подписи.

Функция ОфисныйДокументСоШтампом(СсылкаНаФайл, Ссылка) Экспорт
	...
	МодульУправлениеПечатью.ДобавитьШтампыВОфисныйДокумент(ДанныеФайла.СсылкаНаДвоичныеДанныеФайла, ЭлектронныеПодписи);
...
//   Булево
//
Функция ПрисоединенныйФайлНаходитсяНаДиске(ПрисоединенныйФайл)
