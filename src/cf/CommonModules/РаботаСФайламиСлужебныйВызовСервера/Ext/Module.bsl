...
// Возвращает различные сведения о файле и версии.
// 
// Параметры:
...
Функция ОбновитьОбъектФайла(ФайлСсылка, СведенияОФайле, ВерсияСсылка = Неопределено, 
	...
	ТипХраненияФайлов = РаботаСФайламиСлужебный.ТипХраненияФайлов(Расширение, СведенияОФайле.Размер);
	
	МенеджерФайла = РаботаСФайламиСлужебный.МенеджерФайловПоТипу(ТипХраненияФайлов);
...
// Возвращаемое значение:
//   Структура:
//    * ДанныеПолучены - Булево  - успешно ли выполнена операция получения данных;
//    * ДанныеФайла    - Структура - структура с данными файла.
...
Функция ДанныеФайлаИРабочийКаталог(ФайлИлиВерсияСсылка, РабочийКаталогВладельца = Неопределено) Экспорт
	...
	Если РабочийКаталогВладельца <> "" Тогда
		...
		ИначеЕсли ВозможностьХранитьВерсии И ТекущаяВерсия <> Неопределено Тогда
			ПолноеИмяФайлаВРабочемКаталоге = ПолучитьПолноеИмяФайлаИзРегистра(ТекущаяВерсия, ИмяКаталога, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
		Иначе
