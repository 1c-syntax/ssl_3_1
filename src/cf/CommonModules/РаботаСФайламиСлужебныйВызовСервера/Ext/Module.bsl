...
Функция СоздатьФайлСВерсией(ВладелецФайла, СведенияОФайле) Экспорт
	...
	Если СведенияОФайле.Зашифрован = Неопределено Тогда
		...
		СведенияОФайле.Зашифрован = НРег(СведенияОФайле.РасширениеБезТочки) = НРег(РаботаСФайлами.РасширениеДляЗашифрованныхФайлов());
		
		Если СведенияОФайле.Зашифрован Тогда
			ДляНезашифрованногоФайла = Новый Файл(СведенияОФайле.ИмяБезРасширения);
			СведенияОФайле.РасширениеБезТочки = ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(ДляНезашифрованногоФайла.Расширение);
			СведенияОФайле.ИмяБезРасширения = ДляНезашифрованногоФайла.ИмяБезРасширения;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Если Не СведенияОФайле.Зашифрован И Не ЗначениеЗаполнено(СведенияОФайле.Кодировка) Тогда
...
//   * ОбновлятьСпискиФайловНаФормах - Булево - признак необходимости обновления списков файлов в открытых формах, для
//                                              актуализации данных.
//
Функция ДанныеФайла(Знач ФайлСсылка, Знач ВерсияСсылка = Неопределено, Знач ПараметрыДанныхФайла = Неопределено, Знач ПолучениеФайлаИзАрхива = Ложь) Экспорт
	...
	ДанныеФайла.Вставить("ИмяФайла", ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(
	...
	Если ВерсияСсылка = Неопределено Тогда
		Если ПолучатьСсылкуНаДвоичныеДанные Тогда
			ДвоичныеДанныеФайла = РаботаСФайламиСлужебный.ДвоичныеДанныеФайла(ТекущаяВерсияФайла, ВызыватьИсключение, ПолучениеФайлаИзАрхива);
			
			Если ПолучениеФайлаИзАрхива Тогда
				ДвоичныеДанныеФайла = РаботаСФайловымАрхивомСервер.ДвоичныеДанныеФайлаИзвлеченногоИзZipАрхива(ДвоичныеДанныеФайла, ФайлСсылка);
			КонецЕсли;
			
			СсылкаНаДвоичныеДанныеФайла = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, ИдентификаторФормы);
		Иначе
	...
	Иначе
		Если ПолучатьСсылкуНаДвоичныеДанные Тогда
			ДвоичныеДанныеФайла = РаботаСФайламиСлужебный.ДвоичныеДанныеФайла(ВерсияСсылка, ВызыватьИсключение, ПолучениеФайлаИзАрхива);

			Если ПолучениеФайлаИзАрхива Тогда
				ДвоичныеДанныеФайла = РаботаСФайловымАрхивомСервер.ДвоичныеДанныеФайлаИзвлеченногоИзZipАрхива(ДвоичныеДанныеФайла, ВерсияСсылка);
			КонецЕсли;			
	...
	ДанныеФайла.Вставить("ОбновлятьСпискиФайловНаФормах", Ложь);
...
// и как значение параметра ДанныеФайла в других процедурах и функциях. см. РаботаСФайлами.ДанныеФайла.
//
// Параметры: 
...
//  см. РаботаСФайлами.ДанныеФайла.
//
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл, Знач ДополнительныеПараметры = Неопределено) Экспорт	
...
Функция ДанныеФайлаДляПечати(Знач ПрисоединенныйФайл, Знач ИдентификаторФормы = Неопределено) Экспорт
	...
	ДанныеФайла = ПолучитьДанныеФайла(ПрисоединенныйФайл, ДополнительныеПараметры);
...
//   ПараметрыПолученияФайла - см. РаботаСФайламиКлиентСервер.ПараметрыПолученияФайла
//
// Возвращаемое значение:
//   см. ДанныеФайла
//
Функция ДанныеФайлаДляОткрытия(ФайлСсылка, ВерсияСсылка, ИдентификаторФормы = Неопределено,
	РабочийКаталогВладельца = Неопределено, ПредыдущийАдресФайла = Неопределено, ПараметрыПолученияФайла = Неопределено) Экспорт
	...
	Если ТипЗнч(ПараметрыПолученияФайла) = Тип("Структура") Тогда
		...
		ПараметрыДанныхФайла.ИдентификаторФормы = ПараметрыПолученияФайла.АдресВРодительскомСеансе;
	...
	Если ПроверятьНаличиеФайлаВАрхиве Тогда
		ПолучениеФайлаИзАрхива = РаботаСФайламиСлужебный.ФайлНаходитсяВФайловомАрхиве(ФайлСсылка, ВерсияСсылка);
	КонецЕсли;
	
	ДанныеФайла = ДанныеФайла(ФайлСсылка, ВерсияСсылка, ПараметрыДанныхФайла, ПолучениеФайлаИзАрхива);
	...
	Если ДанныеФайла.Версия <> Неопределено И СведенияОВерсии.ТипХраненияФайла
		...
		ДвоичныеДанные = РаботаСФайламиВТомахСлужебный.ДанныеФайла(ДанныеФайла.Версия,,ПолучениеФайлаИзАрхива);
		Если ПолучениеФайлаИзАрхива Тогда
			ДвоичныеДанные = РаботаСФайловымАрхивомСервер.ДвоичныеДанныеФайлаИзвлеченногоИзZipАрхива(ДвоичныеДанные, ДанныеФайла.Версия);
		КонецЕсли;
		
		Если ТипЗнч(ПараметрыПолученияФайла) = Тип("Структура") Тогда
			ИдентификаторФормы = ПараметрыПолученияФайла.АдресТекущейВерсииВРодительскомСеансе;
		КонецЕсли;
...
Функция ЗапуститьВосстановлениеФайлов(Том, УникальныйИдентификаторФормы) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = '""Восстановление файлов""'");
	ПараметрыВыполнения.КлючФоновогоЗадания = "ВосстановлениеФайлов";
	
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Отчеты.ПроверкаЦелостностиТома.ВосстановитьФайлы", Том);
КонецФункции

Функция НачатьДействиеСФайлом(Знач ПараметрыДлительнойОперации, Знач ИмяФункции) Экспорт
	
	ИдентификаторВладельца = ПараметрыДлительнойОперации.ИдентификаторВладельца;
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторВладельца);
	
	Если ИмяФункции = "РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный" Тогда
		ДлительнаяОперация	= ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
			"РаботаСФайламиСлужебный.ДанныеФайлаДляОткрытияАсинхронный", ПараметрыДлительнойОперации);
	ИначеЕсли ИмяФункции = "РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный" Тогда
		ДлительнаяОперация	= ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
			"РаботаСФайламиСлужебный.ДанныеФайлаДляСохраненияАсинхронный", ПараметрыДлительнойОперации);
	ИначеЕсли ИмяФункции = "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный" Тогда
		ДлительнаяОперация	= ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
			"РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный", ПараметрыДлительнойОперации);
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Непредусмотренный метод получения фала: %1'"), ИмяФункции));
	КонецЕсли;
...
// Получает коллекцию, содержащую значения реквизитов "Наименование" и "Расширение" присоединенного файла.
//
// Параметры:
//  ПрисоединенныйФайл	- ОпределяемыйТип.ПрисоединенныйФайл - ссылка на элемент справочника с файлом.
//
// Возвращаемое значение:
//  Структура:
//   * Расширение - Строка - расширение присоединенного файла. см. реквизит ПрисоединенныйФайл.Расширение.
//   * ИмяФайла - Строка - имя присоединенного файла с расширением. см. реквизиты ПрисоединенныйФайл.Наименование и ПрисоединенныйФайл.Расширение.
//
Функция ИмяФайлаИРасширение(Знач ПрисоединенныйФайл) Экспорт

	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайл, "Наименование,Расширение");

	Результат = Новый Структура;
	Результат.Вставить("Расширение"	, ЗначенияРеквизитов.Расширение);
	Результат.Вставить("ИмяФайла"	, ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ЗначенияРеквизитов.Наименование, ЗначенияРеквизитов.Расширение));
...
Функция СоздатьФайл(Знач Владелец, Знач СведенияОФайле)
	...
	Файл.Наименование = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(СведенияОФайле.ИмяБезРасширения);
	...
	Файл.ИндексКартинки = РаботаСФайламиСлужебныйКлиентСервер.ИндексПиктограммыФайла(Неопределено);
	Файл.ХранитьВерсии = СведенияОФайле.ХранитьВерсии;
	Файл.Расширение = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(СведенияОФайле.РасширениеБезТочки);
	Файл.Размер = СведенияОФайле.Размер;
	Файл.ДатаМодификацииУниверсальная = СведенияОФайле.ВремяИзмененияУниверсальное;
	Если Не ЗначениеЗаполнено(Файл.ДатаМодификацииУниверсальная)
		Или Файл.ДатаМодификацииУниверсальная > ТекущаяУниверсальнаяДата() Тогда
		Файл.ДатаМодификацииУниверсальная = ТекущаяУниверсальнаяДата();
	КонецЕсли;
	Файл.Зашифрован = СведенияОФайле.Зашифрован;

	РаботаСФайламиСлужебный.ЗаполнитьИзвлеченныйТекст(Файл, СведенияОФайле.АдресВременногоХранилищаТекста);
...
Функция ОбновитьОбъектФайла(ФайлСсылка, СведенияОФайле, ВерсияСсылка = Неопределено,
	...
	Попытка	
		...
		РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(Версия, ДвоичныеДанные, СведенияОФайле.Размер, Расширение);		
		...
		Иначе // хранение в томе
			...
			РаботаСФайламиВТомахСлужебный.ЗаполнитьСведенияОФайле(Версия, ДвоичныеДанные,,,Версия.Том);
		КонецЕсли;
		...
		Иначе
			РаботаСФайламиСлужебный.ЗаполнитьИзвлеченныйТекст(Версия, СведенияОФайле.АдресВременногоХранилищаТекста);
		КонецЕсли;
		...
		Если Версия.Размер = 0 И РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ТипХраненияФайла) Тогда
			ДвоичныеДанныеФайла = ХранилищеФайла.Получить(); // ДвоичныеДанные
			Версия.Размер = ДвоичныеДанныеФайла.Размер();
		КонецЕсли;
...
Функция СкопироватьФайл(ИсходныйФайл, ВладелецНовогоФайла)
	...
	НовыйФайл.ТекстХранилище = Новый ХранилищеЗначения(ИсходныйФайл.ТекстХранилище.Получить(), Новый СжатиеДанных(9));
	НовыйФайл.ФайлХранилище  = Новый ХранилищеЗначения(ИсходныйФайл.ФайлХранилище.Получить(), Новый СжатиеДанных(9));

	ДвоичныеДанные = РаботаСФайлами.ДвоичныеДанныеФайла(ИсходныйФайл);
	ДвоичныеДанныеВХранилищеЗначения = Новый ХранилищеЗначения(ДвоичныеДанные, Новый СжатиеДанных(9));	

	РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(НовыйФайл, ДвоичныеДанные, НовыйФайл.Размер, НовыйФайл.Расширение);

	Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(НовыйФайл.ТипХраненияФайла) Тогда
		ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(НовыйФайл);
		
		УстановитьПривилегированныйРежим(Истина);
		РегистрыСведений.ХранилищеФайлов.ЗаписатьДвоичныеДанные(КопияФайлаСсылка, ДвоичныеДанные, ПараметрыЗаписиВИБ);
		УстановитьПривилегированныйРежим(Ложь);
	Иначе

		РаботаСФайламиВТомахСлужебный.ДобавитьФайл(НовыйФайл, ДвоичныеДанные,,,НовыйФайл.Том);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Если ДоступнаЭлектроннаяПодпись Тогда
			МодульЭлектроннаяПодписьСлужебный.СкопироватьПодписиИСертификатыШифрования(ИсходныйФайл, НовыйФайл);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьДанныеФайлаИСохранитьИзмененияФайла(ФайлСсылка, СведенияОФайле, ОтносительныйПутьКФайлу,
	...
	Если Не ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда
		ВызватьИсключение НСтр("ru = 'Файл не занят для редактирования.'");
	КонецЕсли;
...
Функция РабочийКаталогПапки(СсылкаПапки) Экспорт
	...
	Отбор.Вставить("Папка", СсылкаПапки);
	Отбор.Вставить("Пользователь", Пользователи.АвторизованныйПользователь());
	
	ЗначенияРесурсов = РегистрыСведений.РабочиеКаталогиФайлов.Получить(Отбор);
	РабочийКаталог = ЗначенияРесурсов.Путь;
...
Функция СведенияОФайлахВРабочемКаталоге(Знач ИменаФайлов) Экспорт
	...
	Попытка
		...
		|	ФайлыВРабочемКаталоге.Путь КАК ПутьФайла
		|ИЗ
		|	РегистрСведений.ФайлыВРабочемКаталоге КАК ФайлыВРабочемКаталоге
		|ГДЕ
		|	ФайлыВРабочемКаталоге.Путь В (&ИменаФайлов)
		...
		Пока Выборка.Следующий() Цикл
			...
			СведенияОФайле = СведенияОФайлеВРабочемКаталоге();
			ЗаполнитьЗначенияСвойств(СведенияОФайле, Выборка);
			СведенияОФайле.ФайлЕстьВРегистре = Истина;

			// @skip-check query-in-loop - обращения к разным таблицам из составного типа
			ВладелецФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Выборка.Владелец, "ВладелецФайла,Редактирует");
			СведенияОФайле.ВРегистреПапка = ВладелецФайла.ВладелецФайла;
			СведенияОФайле.РедактируетТекущийПользователь = ВладелецФайла.Редактирует = АвторизованныйПользователь;
			Результат[Выборка.ПутьФайла] = СведенияОФайле;
	...
	Для каждого ИмяФайла Из ИменаФайлов Цикл
		Если Результат[ИмяФайла] = Неопределено Тогда
			Результат[ИмяФайла] = СведенияОФайлеВРабочемКаталоге();
		КонецЕсли;
	КонецЦикла;
...
Функция СведенияОФайлеВРабочемКаталоге()
...
Процедура СортироватьМассивСтруктур(МассивСтруктур) Экспорт
	...
	Для Каждого СведенияОФайле Из МассивСтруктур Цикл
		НоваяСтрока = ТаблицаФайлов.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СведенияОФайле, "Путь, Размер, Версия, ДатаПомещенияВРабочийКаталог");
	КонецЦикла;
	...
	Для Каждого СведенияОФайле Из ТаблицаФайлов Цикл
		Запись = Новый Структура;
		Запись.Вставить("Путь", СведенияОФайле.Путь);
		Запись.Вставить("Размер", СведенияОФайле.Размер);
		Запись.Вставить("Версия", СведенияОФайле.Версия);
		Запись.Вставить("ДатаПомещенияВРабочийКаталог", СведенияОФайле.ДатаПомещенияВРабочийКаталог);
		Результат.Добавить(Запись);
	КонецЦикла;

	МассивСтруктур = Результат;
...
//
// Параметры:
//  ФайлИлиВерсияСсылка     - СправочникСсылка.Файлы - файла.
//  ДополнительныеПараметры	- см. РаботаСФайламиКлиентСервер.ДополнительныеПараметрыДанныеФайлаДляСохранения.
//
// Возвращаемое значение:
//   Структура - структура с данными файла.
//
Функция ДанныеФайлаДляСохранения(Знач ФайлСсылка, Знач ДополнительныеПараметры = Неопределено) Экспорт

	Если ДополнительныеПараметры = Неопределено Тогда
		ДополнительныеПараметры = РаботаСФайламиКлиентСервер.ДополнительныеПараметрыДанныеФайлаДляСохранения();
	КонецЕсли;
	
	ДанныеФайла = ДанныеФайлаДляОткрытия(ФайлСсылка, 
											ДополнительныеПараметры.ВерсияФайла, 
											ДополнительныеПараметры.ИдентификаторФормы, 
											ДополнительныеПараметры.РабочийКаталогВладельца,
											,
											ДополнительныеПараметры.ПараметрыПолученияФайла);
...
Процедура ДобавитьПодписьКФайлу(ФайлСсылка, СвойстваПодписи, ИдентификаторФормы) Экспорт
	...
	Если ЗначениеЗаполнено(Редактирует) Тогда
		ВызватьИсключение РаботаСФайламиСлужебныйКлиентСервер.СообщениеОНедопустимостиПодписанияЗанятогоФайла(
			ФайлСсылка);
	КонецЕсли;

	Зашифрован = СтруктураРеквизитов.Зашифрован;
	Если Зашифрован Тогда
		СтрокаИсключения = РаботаСФайламиСлужебныйКлиентСервер.СообщениеОНедопустимостиПодписанияЗашифрованногоФайла(
			ФайлСсылка);
...
Функция РезультатУдаленияФайлов(ФайлыИлиВерсии, УникальныйИдентификатор) Экспорт
	...
	АвторыФайлов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ФайлыИлиВерсии, "Автор");
	...
	Для Каждого ФайлИлиВерсия Из ФайлыИлиВерсии Цикл
		...
		Если АвторыФайлов[ФайлИлиВерсия] = АвторизованныйПользователь Тогда
...
Процедура УдалитьДанные(ФайлИлиВерсия, УникальныйИдентификатор)
	...
	Попытка
		...
		Иначе
			УстановитьПривилегированныйРежим(Истина);
			РегистрыСведений.ХранилищеФайлов.УдалитьДвоичныеДанные(ФайлИлиВерсия);
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;	
...
Процедура ПроверитьПодписи(ИсходныеДанные, ДанныеСтрок, ПодписанныйОбъект) Экспорт
	...
	Для Каждого СтрокаПодписи Из ДанныеСтрок Цикл
		...
		Если СтрокаПодписи.ОписаниеОшибки <> Неопределено Тогда
			СтрокаПодписи.ОписаниеОшибки    = ОписаниеОшибки; // Для совместимости.
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(СтрокаПодписи.РезультатПроверки, РезультатПроверкиПодписи);
		СтрокаПодписи.РезультатПроверки.ДополнительныеАтрибутыПровереныВручную = Ложь;
		СтрокаПодписи.РезультатПроверки.АвторРучнойПроверкиДополнительныхАтрибутов = Неопределено;
		СтрокаПодписи.РезультатПроверки.ОбоснованиеРучнойПроверкиДополнительныхАтрибутов = "";
		...
		Если ЗначениеЗаполнено(СтрокаПодписи.РезультатПроверкиПодписиПоМЧД) И ОбщегоНазначения.ПодсистемаСуществует(
			...
			Для Каждого Результат Из РезультатПроверкиДоверенностей Цикл
				Если Не ЗначениеЗаполнено(РезультатПроверкиПодписи.Сертификат) Тогда
				...
				КонецЕсли;
			КонецЦикла;
			СтрокаПодписи.РезультатПроверкиПодписиПоМЧД = ПоместитьВоВременноеХранилище(
		...
		Если СтрокаПодписи.ОписаниеОшибки <> Неопределено Тогда
			РаботаСФайламиСлужебныйКлиентСервер.ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекущаяДатаСеанса()); // Для совместимости.
		КонецЕсли;
		МодульЭлектроннаяПодписьКлиентСервер.ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекущаяДатаСеанса());
...
Функция ПроверитьПодписиПоМЧД(Подписи, ПодписанныйОбъект) Экспорт
	...
	МодульЭлектроннаяПодписьСлужебный = ОбщегоНазначения.ОбщийМодуль(
			"ЭлектроннаяПодписьСлужебный");
			
	Возврат МодульЭлектроннаяПодписьСлужебный.ПроверитьПодписиПоМЧД(Подписи, ПодписанныйОбъект);
...
Функция ДокументСоШтампом(Знач ДанныеФайла) Экспорт
	...
	Если Расширение = "mxl" Тогда
		...
		Штампы = МодульЭлектроннаяПодпись.ШтампыВизуализацииЭлектронныхПодписей(ДанныеФайла.Ссылка);
	...
	ИначеЕсли Расширение = "docx" Тогда
		...
		ЭлектронныеПодписи = МодульЭлектроннаяПодпись.ПодписиОбъекта(ДанныеФайла.Ссылка, ПараметрыПолученияПодписей);
...
Функция ПрисоединенныйФайлНаходитсяНаДиске(Знач ПрисоединенныйФайл)
	...
	Иначе
		Возврат РаботаСФайламиВТомахСлужебный.ПрисоединенныйФайлНаходитсяВТоме(ПрисоединенныйФайл);
	КонецЕсли;
КонецФункции
...
Функция ПараметрыЖурналаСканирования(ИдентификаторКлиента) Экспорт
	Результат = Новый Структура;
	Результат.Вставить("ДатаНачалаЖурналаСканирования", ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"КомпонентаСканирования", "ДатаНачалаЖурналаСканирования", Дата(1, 1, 1)));
...
Процедура СброситьПараметрыКаталогаЖурналаСканирования(ИдентификаторКлиента) Экспорт
	...
	МассивСтруктур.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("КаталогЖурналаСканирования",
		"", ИдентификаторКлиента));
	МассивСтруктур.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ИспользоватьКаталогЖурналаСканирования",
...
Функция ТехническаяИнформация() Экспорт
	...
	Результат.Вставить("ИмяЛогФайла", ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"КомпонентаСканирования", "ИмяЛогФайла"));
	Возврат Результат;
КонецФункции

Процедура УстановитьПараметрыНачалаЖурналаСканирования(ИмяЛогФайла) Экспорт
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("КомпонентаСканирования", "ИмяЛогФайла", ИмяЛогФайла);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("КомпонентаСканирования",
		"ДатаНачалаЖурналаСканирования", ТекущаяДатаСеанса());
КонецПроцедуры

Процедура ОчиститьРабочийКаталог(Знач СсылкаПапки) Экспорт
