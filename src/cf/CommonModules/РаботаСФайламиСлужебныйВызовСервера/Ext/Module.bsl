...
//     * Ссылка - ОпределяемыйТип.ПрисоединенныйФайл - ссылка на файл.
//     * Версия - СправочникСсылка.ВерсииФайлов - ссылка на версию файла.
...
Функция ДанныеФайлаДляОткрытия(ФайлСсылка, ВерсияСсылка, ИдентификаторФормы = Неопределено,
	...
	Если ДанныеФайла.Версия <> Неопределено
		...
		ДанныеФайлаТом = СведенияОВерсии.Том;
		ДанныеФайлаПутьКФайлу = СведенияОВерсии.ПутьКФайлу;
...
Функция ОбновитьОбъектФайла(ФайлСсылка, СведенияОФайле, ВерсияСсылка = Неопределено, 
	...
	Иначе // хранение на диске
		РаботаСФайламиВТомахСлужебный.УдалитьФайл(РаботаСФайламиВТомахСлужебный.ПолноеИмяФайлаВТоме(СвойстваВерсии));
		РаботаСФайламиВТомахСлужебный.ДобавитьФайл(Версия, ДвоичныеДанные, , Истина);
	КонецЕсли;
...
Функция СохранитьИзмененияИОсвободитьФайл(ДанныеФайла, СведенияОФайле,
	...
	Попытка
		...
		Если СведенияОФайле.Кодировка <> Неопределено Тогда
			Если Не ЗначениеЗаполнено(РегистрыСведений.КодировкиФайлов.КодировкаВерсииФайла(ДанныеФайла.ТекущаяВерсия)) Тогда
				РегистрыСведений.КодировкиФайлов.ЗаписатьКодировкуВерсииФайла(ДанныеФайла.ТекущаяВерсия, СведенияОФайле.Кодировка);
			КонецЕсли;
		КонецЕсли;
...
Функция СохранитьИзмененияФайла(ФайлСсылка, СведенияОФайле, 
	...
	Попытка
		...
		Если СведенияОФайле.Кодировка <> Неопределено Тогда
			Если Не ЗначениеЗаполнено(РегистрыСведений.КодировкиФайлов.КодировкаВерсииФайла(ТекущаяВерсия)) Тогда
				РегистрыСведений.КодировкиФайлов.ЗаписатьКодировкуВерсииФайла(ТекущаяВерсия, СведенияОФайле.Кодировка);
			КонецЕсли;
		КонецЕсли;
...
//   ФайлУжеРедактируетТекущийПользователь - Булево - файл уже был захвачен для редактирования текущим пользователем.
//
Функция ПолучитьДанныеФайлаДляОткрытияИЗанятьФайл(ФайлСсылка,
	...
	Результат.ФайлУжеРедактируетТекущийПользователь = ДанныеФайла.ФайлРедактируетТекущийПользователь;
...
Процедура УдалитьДанныеФайла(ФайлИлиВерсия, УникальныйИдентификатор, Результат, АвторизованныйПользователь)
	...
	Если ВерсияДругогоАвтора <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Не удалось удалить файл, т.к. в него вносили изменения другие авторы.
			|Были удалены только Ваши версии файла.'");
