...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.СервисКриптографииDSSСлужебный";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьВФоне";
	ИсключениеПроверки.ФрагментСодержания = "Возврат ДлительныеОперации.ВыполнитьВФоне(ИмяПроцедуры, ПараметрыПроцедуры, ПараметрыВыполнения)";
	
	Методы = Новый Соответствие;
	Методы.Вставить("ОбновитьКэшСертификатов");
	Методы.Вставить("АутентификацияПоСертификату", Истина);
	Методы.Вставить("АутентификацияПоЛогину", Истина);
	Методы.Вставить("Зашифровать", Истина);
	Методы.Вставить("СертификатыРасшифрования", Истина);
	Методы.Вставить("ХешированиеДанных", Истина);
	Методы.Вставить("СоздатьТранзакцию", Истина);
	Методы.Вставить("Расшифровать", Истина);
	Методы.Вставить("Подписать", Истина);
	Методы.Вставить("ПодписатьПакет", Истина);
	Методы.Вставить("УсовершенствоватьПодпись", Истина);
	Методы.Вставить("ВалидацияПинКодаСертификата", Истина);
	Методы.Вставить("ПолучитьДанныеСертификата", Истина);
	Методы.Вставить("ПолучитьСвойстваСертификата", Истина);
	Методы.Вставить("ОбновитьДанныеСертификатов", Истина);
	Методы.Вставить("СоздатьЗапросНаСертификат", Истина);
	Методы.Вставить("СписокЗапросовНаСертификат", Истина);
	Методы.Вставить("ПолучитьЗапросНаСертификат", Истина);
	Методы.Вставить("УдалитьЗапросНаСертификат", Истина);
	Методы.Вставить("УстановитьСертификат", Истина);
	Методы.Вставить("НазначитьОсновнымСертификат", Истина);
	Методы.Вставить("СменитьПинКодСертификата", Истина);
	Методы.Вставить("ПроверитьСертификат", Истина);
	Методы.Вставить("ПолучитьСведенияОПодписантах", Истина);
	Методы.Вставить("ПроверитьПодпись", Истина);
	Методы.Вставить("ОбновитьПолитикуПользователя", Истина);
	Методы.Вставить("ПолучитьОписаниеСервиса", Истина);
	Методы.Вставить("ВыполнитьЗапросКСервису", Истина);
	Методы.Вставить("НайтиСертификат", Истина);
	Методы.Вставить("СостояниеУчетнойЗаписи", Истина);
	Методы.Вставить("ПодготовитьТранзакцию", Истина);
	Методы.Вставить("ПолучитьЗаписиАудита", Истина);
	Методы.Вставить("ЗагрузитьДокумент", Истина);
	Методы.Вставить("ЗагрузитьПакетДокументов", Истина);
	Методы.Вставить("ИнформацияОДокументеПоИдентификатору", Истина);
	Методы.Вставить("ИнформацияОДокументеПоХешу", Истина);
	Методы.Вставить("ПолучитьОбработанныйДокумент", Истина);
	Методы.Вставить("ИзменитьИнформациюОДокументе", Истина);
	Методы.Вставить("ПривязатьДокумент", Истина);
	Методы.Вставить("ПолучитьИнформациюДокументаКонвертации", Истина);
	Методы.Вставить("УдалитьДокумент", Истина);
	Методы.Вставить("СоздатьЗакрытыйКлюч", Истина);
	Методы.Вставить("УстановитьПараметрЗакрытогоКлюча", Истина);
	Методы.Вставить("ПолучитьПараметрЗакрытогоКлюча", Истина);
	Методы.Вставить("ПодписатьАсинхронно", Истина);
	Методы.Вставить("ПолучитьОбработанныеДокументы", Истина);
	Методы.Вставить("ЗашифроватьАсинхронно", Истина);
	Методы.Вставить("РасшифроватьАсинхронно", Истина);
	Методы.Вставить("СертификатыРасшифрованияАсинхронно", Истина);
	Методы.Вставить("СведенияОбОперации", Истина);
	Методы.Вставить("ПолучитьТокенПоКодуАвторизации", Истина);
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.СервисКриптографииDSSСлужебный", Методы);
	
	Методы = Новый Соответствие;
	Методы.Вставить("ОбменССерверомАктивироватьУчетнуюЗапись", Истина);
	Методы.Вставить("ОбменССерверомАктивироватьКлючПриложенияУчетнойЗаписи", Истина);
	Методы.Вставить("ОбменССерверомОтправитьКодАктивацииКлюча", Истина);
	Методы.Вставить("ПолучитьТикетИнтернетПоддержкиПользователя", Истина);
	Методы.Вставить("ОтправитьЗаявлениеНаПортал", Истина);
	Методы.Вставить("ОбменССерверомСписокСерверовОблачнойПодписи", Истина);
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("Обработка.УправлениеПодключениемDSS", Методы);
...
Процедура ПриОпределенииОбработчиковПоставляемыхДанных(Обработчики) Экспорт
	...
	Обработчик.ВидДанных = ИмяНастроекСлужебнойУчетнойЗаписи();
	Обработчик.КодОбработчика = ИмяНастроекСлужебнойУчетнойЗаписи();
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Загружать = Дескриптор.DataType = ИмяНастроекСлужебнойУчетнойЗаписи();
...
Процедура ОбработатьНовыеДанные(Знач Дескриптор, Знач ПутьКФайлу) Экспорт
	...
	Если Дескриптор.DataType = ИмяНастроекСлужебнойУчетнойЗаписи() Тогда
		ДанныеНастройки = Новый ДвоичныеДанные(ПутьКФайлу);
		УстановитьПривилегированныйРежим(Истина);
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
				ИмяНастроекСлужебнойУчетнойЗаписи(),
				ПолучитьСтрокуИзДвоичныхДанных(ДанныеНастройки), "Настройки");
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
//    * Наименование 				- Строка
//    * АдресСервера 				- Строка
//    * ЗащищенноеСоединениеГОСТ - Булево
...
Функция ПолучитьВсеСерверы(ОтборЗапроса = Неопределено) Экспорт
	...
	|	ЭкземплярыСервераDSS.ЗащищенноеСоединениеГОСТ КАК ЗащищенноеСоединениеГОСТ,
	...
	|	ЭкземплярыСервераDSS.СервисныйКлиент КАК СервисныйКлиент,
	|	ЭкземплярыСервераDSS.ВнутреннийИдентификатор КАК ВнутреннийИдентификатор,
...
//             			- Структура - см. СервисКриптографииDSSКлиентСервер.СтатусОшибки
//
// Возвращаемое значение:
...
Функция ПолучитьОписаниеОшибки(РезультатОперации, ТипОшибки, ТекстОшибки = "") Экспорт
	...
	Иначе
		...
		Если ТипЗнч(ТекстОшибки) = Тип("Структура") Тогда
			Содержимое = ТекстОшибки.Представление;
		Иначе
			Содержимое = ТекстОшибки;
		КонецЕсли;
		...
		Если РезультатОперации <> Неопределено Тогда
			...
			РезультатОперации.СтатусОшибки.ИсходныйТекст = Содержимое;
		КонецЕсли;
...
Процедура ВыполнитьЗапросКСервису(ПараметрыВызова, АдресРезультата) Экспорт
	...
	НастройкиПользователя = ПараметрыЗапроса.НастройкиПользователя;
	
	СекретныйКлиент = ПолучитьПолеСтруктуры(ПараметрыЗапроса.ПараметрыЗапроса, "ClientID");
	Если ЗначениеЗаполнено(СекретныйКлиент)
		И НастройкиПользователя.ПервичнаяАутентификация <> Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент Тогда
		СекретСервисногоКлиента = ПолучитьЗакрытыеДанные(
			СервисКриптографииDSS.КлючСервисногоКлиента(НастройкиПользователя), 
			"СекретСервисногоКлиента", Истина).Значение;
		Если ЗначениеЗаполнено(СекретСервисногоКлиента) Тогда
			ПараметрыЗапроса.ПараметрыЗапроса.Вставить("ClientSecret", СекретСервисногоКлиента); 
		КонецЕсли;
	КонецЕсли;
...
//  ЭтоСервисныйКлиент		- Булево
//
Процедура НастройкиРаботыАнализПолитикиПользователя(ПолитикаПользователя, ДанныеПолитики, ЭтоСервисныйКлиент = Ложь) Экспорт
	...
	Для каждого СтрокаМассива Из ПоляДействий Цикл
		...
		Если КлючПолитики <> Неопределено И СписокДействий.Свойство(КлючПолитики, КлючПоиска) Тогда
			ПолитикаПользователя.Действия[КлючПоиска[0]].Подтверждать = ЗначениеПолитики ИЛИ ЭтоСервисныйКлиент;
...
//  ОбщиеНастройки			- Булево - настройки для всех пользователей
//
// Возвращаемое значение:
//  Строка
//
Функция ПолучитьЗакрытыеДанные(НастройкиПользователя, ИмяЗначения = Неопределено, ОбщиеНастройки = Ложь) Экспорт
	
	Результат			= "";
	КлючЗначения		= КлючЗаписиКонфиденциальныхДанных(НастройкиПользователя, ОбщиеНастройки);
	...
	Если ПолучитьПолеСтруктуры(ЗначениеПоУмолчанию.СекретСервисногоКлиента, "Состояние") = 5 
		И ТекущиеНастройки.Параметры[ЗначениеПоУмолчанию.СекретСервисногоКлиента.Значение] = Неопределено Тогда
		ЗначениеПоУмолчанию.СекретСервисногоКлиента = "";
	КонецЕсли;
	...
	Если ТипЗнч(ЗначениеПоУмолчанию.СекретСервисногоКлиента) = Тип("Строка") Тогда
		Если ЗначениеЗаполнено(ЗначениеПоУмолчанию.СекретСервисногоКлиента) Тогда
			ЗначениеПоУмолчанию.СекретСервисногоКлиента = СервисКриптографииDSSКлиентСервер.ОбъектПароля(ЗначениеПоУмолчанию.СекретСервисногоКлиента, 0);
		Иначе
			ЗначениеПоУмолчанию.СекретСервисногоКлиента = СервисКриптографииDSSКлиентСервер.ОбъектПароля("", 3);
		КонецЕсли;
	КонецЕсли;
...
//  ОбщиеНастройки			- Булево - настройки для всех пользователей
//
Процедура СохранитьЗакрытыеДанные(НастройкиПользователя, Значение, ИмяЗначения = Неопределено, ОбщиеНастройки = Ложь) Экспорт
	
	КлючЗначения = КлючЗаписиКонфиденциальныхДанных(НастройкиПользователя, ОбщиеНастройки);
	НовоеЗначение = Значение;
	
	Если ЗначениеЗаполнено(ИмяЗначения) Тогда
		ВсеДанные = ПолучитьЗакрытыеДанные(НастройкиПользователя, , ОбщиеНастройки);
...
Функция ДоступныеСпособыПервичнойАвторизации() Экспорт
	...
	Результат.Добавить(Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент);
...
Функция ПодготовитьПодключение(СвойстваПодключения, СертификатКлиента = Неопределено, ПараметрыОперации = Неопределено) Экспорт
	...
	Попытка
		...
		Если СертификатКлиента = Неопределено 
			...
			ТипСоединения = ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(Неопределено, СертификатыУЦ,
				?(СвойстваПодключения.ЗащищенноеСоединениеГОСТ, "КриптоПро", "OpenSSL"));
	...
	Исключение
		...
		ПолучитьОписаниеОшибки(Результат, "Подключение");
		Результат.СтатусОшибки.ПолныйТекст = ПротоколОшибкиПриВызове(Результат.СтатусОшибки.Представление,
								"Подключение", 
								"",
								ИнформацияОбОшибке(),
								ПараметрыОперации);
...
Функция ВыполнитьМетодСервиса(Соединение, 
	...
	Исключение
		...
		ОтветМетода.СтатусОшибки.ПолныйТекст = ПротоколОшибкиПриВызове(ОтветМетода.СтатусОшибки.Представление,
								"ВыполнениеМетода", 
								ПараметрыМетода,
								ИнформацияОбОшибке(),
								ПараметрыОперации);
...
Функция ПротоколОшибкиПриВызове(РезультатВызова, ВидОперации, ВходящийКонтекст, КонтекстОшибки = Неопределено, ПараметрыОперации = Неопределено) Экспорт
	...
	ИначеЕсли НЕ РезультатВызова.Выполнено Тогда
		НоваяОшибка = РезультатВызова.СтатусОшибки.Представление;
	КонецЕсли;	
...
Функция ПолучитьРазделыОтвета(ТипКонтента, ДанныеОтвета)
	...
	РазделительРаздела = Символы.ВК + Символы.ПС;
	...
	Пока Истина Цикл
		...
		ДанныеРаздела = ЧтениеРаздела.Прочитать().ПолучитьБуферДвоичныхДанных();
		РазмерДанных = ДанныеРаздела.Размер;
		Если РазмерДанных > 2 
			И ПолучитьСтрокуИзБуфераДвоичныхДанных(ДанныеРаздела.ПолучитьСрез(РазмерДанных - 2)) = РазделительРаздела Тогда
			ДанныеРаздела = ДанныеРаздела.ПолучитьСрез(0, РазмерДанных - 2);
		ИначеЕсли РазмерДанных = 2 
			И ПолучитьСтрокуИзБуфераДвоичныхДанных(ДанныеРаздела.ПолучитьСрез(0)) = РазделительРаздела Тогда
			ДанныеРаздела = Новый БуферДвоичныхДанных(0);
		КонецЕсли;
		
		НоваяСтрока.Содержимое = ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(ДанныеРаздела);
...
Функция ОписаниеОшибки_ПроверкаМаркерДоступа()
	...
	Результат = ОбщееОписаниеОшибки("300043", 
			...
			|Потребуется повторная аутентификация, может потребоваться перезапустить приложение.'"));
...
Функция ПакетнаяОперацияПользователя(НастройкиПользователя, ПараметрыВыполнения, ВходящиеПараметры)
	...
	ПодтверждатьПринудительно = ПолучитьПолеСтруктуры(ВходящиеПараметры, "ПодтверждатьПринудительно", Ложь) 
								ИЛИ ДанныеПодтверждения.ПараметрыВариантаВстроенного.ПодтверждатьПринудительно;
	...
	ЭтоМассивДокументов = ТипЗнч(ДанныеДокумента) = Тип("Массив");
	...
	Если РезультатЗапроса.Выполнено 
		...
		ДанныеПодтверждения.Сертификат = ТекущийСертификат;
		ПодтверждатьПринудительно = ПодтверждатьПринудительно ИЛИ ТекущийСертификат.МобильныйЗакрытыйКлюч;
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Если ТипОперации = "Подписание" Тогда
			...
			Если ЭтоМассивДокументов И ВерсияПодтверждения = 1 Тогда
	...
	Если РезультатЗапроса.Выполнено
		...
		НастройкиПользователя = ПроверитьМаркерОбновления(РезультатЗапроса, НастройкиПользователя, МаркерОбновлен);
		ВходящиеПараметры.Вставить("ПодтверждатьПринудительно", Истина);
		...
		ИначеЕсли ТипОперации = "Подписание" И ВерсияПодтверждения = 2 Тогда
			Если ТекущийСертификат.МобильныйЗакрытыйКлюч Тогда
				СписокДокументов = СервисКриптографииDSSКлиентСервер.ЗначениеВМассиве(ДанныеДокумента);
				ПараметрыДокументов = Новый Массив;
				Для Счетчик = 0 По СписокДокументов.Количество() - 1 Цикл
					СтрокаМассива = СписокДокументов[Счетчик];
					ИмяДокумента = ПолучитьПолеСтруктуры(СтрокаМассива, "ИмяДокумента", "Подпись");
					ТипДокумента = ПолучитьПолеСтруктуры(СтрокаМассива, "ТипДокумента", "bin");
					НоваяСтрока = СервисКриптографииDSSКлиентСервер.ПолучитьСвойстваДокумента(ИмяДокумента, ТипДокумента, Истина, "");
					РезультатЗапроса = СервисКриптографииDSS.ЗагрузитьДокумент(НастройкиПользователя, СтрокаМассива.Содержание, НоваяСтрока, ВходящиеПараметры);
					Если РезультатЗапроса.Выполнено Тогда
						СписокДокументов[Счетчик] = РезультатЗапроса.Результат;
					Иначе
						Прервать;
					КонецЕсли;
				КонецЦикла;
				...
				Если РезультатЗапроса.Выполнено Тогда
					Если НЕ ЭтоМассивДокументов Тогда
						ДанныеДокумента = СписокДокументов[Счетчик];
					Иначе
						Для Счетчик = 0 По СписокДокументов.Количество() - 1 Цикл
							ДанныеДокумента[Счетчик] = СписокДокументов[Счетчик];
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Если РезультатЗапроса.Выполнено Тогда
				РезультатЗапроса = СервисКриптографииDSS.ПодписатьАсинхронно(НастройкиПользователя, ДанныеДокумента, СвойствоПодписи, ТекущийСертификат.Идентификатор, ПинКод, ВходящиеПараметры);
		...
		ИначеЕсли ТипОперации = "Расшифрование" И ВерсияПодтверждения = 2 Тогда
			РезультатЗапроса = СервисКриптографииDSS.РасшифроватьАсинхронно(НастройкиПользователя, ДанныеДокумента, ТекущийСертификат.Идентификатор, "CMS", ПинКод, ВходящиеПараметры);
			ТипРезультата = ПолучитьПолеСтруктуры(РезультатЗапроса, "ТипРезультата");
			ИдентификаторТранзакции = ПолучитьПолеСтруктуры(РезультатЗапроса, "Результат.ИдентификаторОперации", "");
		Иначе	
...
Функция ПодготовитьТранзакциюПодтверждения(НастройкиПользователя, ТипОперации, ПараметрыОперации = Неопределено) Экспорт
	...
	Иначе
		...
		ДанныеПодтверждения.Ошибка = РезультатЗапроса.СтатусОшибки.Представление;
		ДанныеПодтверждения.КодОшибки = РезультатЗапроса.СтатусОшибки.КодОшибки;
	КонецЕсли;	
...
Функция ОтправитьЗапросОбъектаПодтверждения(НастройкиПользователя, ДанныеПодтверждения, ДатаСеанса = Неопределено, ПараметрыОперации = Неопределено)
	...
	СекретныйКлиент = ПолучитьПолеСтруктуры(ПараметрыЗапроса, "ClientID");
	Если ЗначениеЗаполнено(СекретныйКлиент)
		И НастройкиПользователя.ПервичнаяАутентификация <> Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент Тогда
		СекретСервисногоКлиента = ПолучитьЗакрытыеДанные(
			СервисКриптографииDSS.КлючСервисногоКлиента(НастройкиПользователя), 
			"СекретСервисногоКлиента", Истина).Значение;
		Если ЗначениеЗаполнено(СекретСервисногоКлиента) Тогда
			ПараметрыЗапроса.Вставить("ClientSecret", СекретСервисногоКлиента); 
		КонецЕсли;
	КонецЕсли;
...
Функция КонвертацияТипАвторизации(СпособАвторизации)
	...
	ОтборСпособов.Вставить(Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент, 11);
...
//    * ЗащищенноеСоединениеГОСТ - Булево - использовать защищенное соединение ГОСТ
...
Функция НастройкиРаботыПодключение(ВыборкаПользователей)
	...
	Если ТипЗнч(ВыборкаПользователей) <> Тип("Структура") Или ТипЗнч(ВыборкаПользователей) = Тип("Структура")
		И ВыборкаПользователей.Свойство("ЗащищенноеСоединениеГОСТ") Тогда
		ДанныеПодключения.Вставить("ЗащищенноеСоединениеГОСТ", ВыборкаПользователей.ЗащищенноеСоединениеГОСТ);
	Иначе
		ДанныеПодключения.Вставить("ЗащищенноеСоединениеГОСТ", Ложь);
	КонецЕсли;
	...
	ДанныеПодключения.Вставить("СервисныйКлиент", ВыборкаПользователей.СервисныйКлиент);
...
Функция НастройкиРаботыДанныеПользователя(ВыборкаПользователей)
	...
	НастройкиРаботыАнализПолитикиПользователя(ДанныеПользователя.Политика,
		Неопределено, ДанныеПользователя.ПервичнаяАутентификация = Перечисления.СпособыАвторизацииDSS.Первичный_СервисныйКлиент);
...
//  ОбщиеНастройки			- Булево - общие настройки пользователей
//
// Возвращаемое значение:
//  Строка - полный путь поиска в регистре БезопасноеХранилище, включает в себя данные об учетной записи и пользователе 1С
//
Функция КлючЗаписиКонфиденциальныхДанных(НастройкиПользователя, ОбщиеНастройки = Ложь)
	
	Если ОбщиеНастройки Тогда
		КлючЗначения = "ЭП_DSS." + НастройкиПользователя;
	ИначеЕсли ТипЗнч(НастройкиПользователя) = Тип("Строка")
...
Функция КонфиденциальныеДанныеПользователя()
	...
	Результат.Вставить("СекретСервисногоКлиента", ЗакрытьКонфиденциальноеЗначение(""));
	
	Возврат Результат;
	
КонецФункции

// Служит для получения содержимого сертификата из кэш сертификатов
//	
// Параметры:
...
Функция ПолучитьПредставлениеСпособовАвторизации(ЗначениеВыбора) Экспорт
	...
	Варианты.Вставить(ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Первичный_СервисныйКлиент"),
			НСтр("ru = 'Сервисный клиент'"));
...
Функция СертификатыПодключенияСервиса1СDSS()
	...
	|ZWRlcmF0aW9uIGJlZm9yZSAyMDQ0DQo9PT09PT09PT09PT09PT09PT09PT09PT09
	|PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT09PT0NCg0K
	|LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tDQpNSUlGNnpDQ0JaaWdBd0lCQWdJ
	|UUdNTk45VGE1L2VJcGVlVmNTQWcyVURBS0JnZ3FoUU1IQVFFREFqQ0NBVHN4DQpJ
	|VEFmQmdrcWhraUc5dzBCQ1FFV0VtUnBkRUJrYVdkcGRHRnNMbWR2ZGk1eWRURUxN
	|QWtHQTFVRUJoTUNVbFV4DQpHREFXQmdOVkJBZ01EemMzSU5DYzBMN1JnZEM2MExM
	|UXNERVpNQmNHQTFVRUJ3d1EwTE11SU5DYzBMN1JnZEM2DQowTExRc0RGVE1GRUdB
	|MVVFQ1F4SzBKL1JnTkMxMFlIUXZkQzEwTDNSZ2RDNjBMRFJqeURRdmRDdzBMSFF0
	|ZEdBDQowTFhRdHRDOTBMRFJqeXdnMExUUXZ0QzhJREV3TENEUmdkR0MwWURRdnRD
	|MTBMM1F1TkMxSURJeEpqQWtCZ05WDQpCQW9NSGRDYzBMalF2ZEdHMExqUmhOR0Ew
