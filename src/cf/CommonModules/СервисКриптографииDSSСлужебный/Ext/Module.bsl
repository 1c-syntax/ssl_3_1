...
//
// Параметры:
//   ТаблицаПараметров	- ТаблицаЗначений - таблица описания параметров, описание состав колонок 
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		...
		МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "ИспользоватьСервисОблачнойПодписи");
...
Процедура ПриОпределенииПоддерживаемыхВерсийПрограммныхИнтерфейсов(СтруктураПоддерживаемыхВерсий) Экспорт
	...
	СтруктураПоддерживаемыхВерсий.Вставить("ОблачнаяПодпись", МассивВерсий);	
...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.Константы.ИспользоватьСервисОблачнойПодписи);
...
Функция СоздатьСерверОблачнойПодписи(НастройкиСервиса) Экспорт
	...
	Иначе
		ЗаписатьОшибкуВЖурналРегистрации(НСтр("ru = 'Некорректно заполнены настройки для сервера облачной подписи'", КодЯзыка), НастройкиСервиса);
		Возврат Результат;
	...
	Если ЗначениеЗаполнено(ЕстьЭкземпляр) Тогда
		...
		Для каждого СтрокаКлюча Из НастройкиСервиса Цикл
			Если ОписаниеОбъекта.Найти(СтрокаКлюча.Ключ) <> Неопределено Тогда
				ЕстьИзменения = СтрокаКлюча.Значение <> ОбъектСервера[СтрокаКлюча.Ключ];
			КонецЕсли;	
		КонецЦикла;	
	Иначе
		...
		Если ОбъектСервера.ПроверитьЗаполнение() Тогда
			...
			Исключение
				...
				ПротоколОшибкиПриВызове(
					НСтр("ru = 'Не выполнена проверка перед записью экземпляра сервера облачной подписи'", КодЯзыка), 
...
Функция СоздатьУчетнуюЗаписьОблачнойПодписи(НастройкиУчетнойЗаписи) Экспорт
	...
	Если ЗначениеЗаполнено(ЕстьЭкземпляр) Тогда
		...
		Для каждого СтрокаКлюча Из НастройкиУчетнойЗаписи Цикл
			Если ОписаниеОбъекта.Найти(СтрокаКлюча.Ключ) <> Неопределено Тогда
				ЕстьИзменения = СтрокаКлюча.Значение <> ОбъектУчетнойЗаписи[СтрокаКлюча.Ключ];
			КонецЕсли;	
		КонецЦикла;	
	Иначе
	...
	Если ЕстьИзменения Тогда
		...
		Если ОбъектУчетнойЗаписи.ПроверитьЗаполнение() Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ПротоколОшибкиПриВызове(
					НСтр("ru = 'Не выполнена проверка перед записью учетной записи облачной подписи'", КодЯзыка), 
...
Процедура ПодписатьАсинхронно(ПараметрыВызова, АдресРезультата) Экспорт
	...
	РезультатЗапроса = СервисКриптографииDSS.ПодписатьАсинхронно(
					...
					ВсеПараметры.ИдентификаторСертификата,
					ПинКод,
					ВсеПараметры.АсинхронныйРежим,
					ВсеПараметры.ОбратныйВызов,
					ВсеПараметры.ПараметрыОперации);
...
// Описание и параметры вызова см. СервисКриптографииDSS.ВыполнитьОперацию()
Процедура ВыполнитьОперацию(ПараметрыВызова, АдресРезультата) Экспорт
	
	ВсеПараметры = ПодготовитьПеременныеВызова(
					"ИдентификаторОперации,
					|НастройкиПользователя", ПараметрыВызова);
					
	РезультатЗапроса = СервисКриптографииDSS.ВыполнитьОперацию(
					ВсеПараметры.НастройкиПользователя,
					ВсеПараметры.ИдентификаторОперации,
					ВсеПараметры.ПараметрыОперации);
...
Функция ВыполнитьВФоне(ИмяПроцедуры, ПараметрыПроцедуры) Экспорт
	...
	Если СинхронныйРежим = Неопределено Тогда
		СинхронныйРежим = НЕ ОбщегоНазначения.ЭтоВебКлиент();
...
Функция ПроверитьОшибкуВыполненияМетода(ОтветСервиса, РезультатВыполнения)
	...
	Если ИщемОшибку = 4 ИЛИ ИщемОшибку = 5 Тогда
		...
		Если СтрНайти(ТипКонтента, "application/json") > 0
			...
			Ответ_Ошибка	= ПолучитьПолеСтруктуры(ТелоОтвета, "error", "");
			Ответ_Сообщение	= ПолучитьПолеСтруктуры(ТелоОтвета, "message", "");
			Если ПустаяСтрока(Ответ_Сообщение) Тогда
				Ответ_Сообщение	= ПолучитьПолеСтруктуры(ТелоОтвета, "MessageDetail", "");
			КонецЕсли;
			Если ПустаяСтрока(Ответ_Сообщение) Тогда
				Ответ_Сообщение	= ПолучитьПолеСтруктуры(ТелоОтвета, "error_description", "");
			КонецЕсли;
			
			Если ПустаяСтрока(Ответ_Ошибка) Тогда
				Ответ_Ошибка = Ответ_Сообщение;
			КонецЕсли;
		...
		ИначеЕсли ОтветСервиса.КодСостояния = 500 Тогда
			ПолучитьОписаниеОшибки(РезультатВыполнения, "HTTP500", Ответ_Сообщение);
			
		Иначе
			ПолучитьОписаниеОшибки(РезультатВыполнения, Ответ_Ошибка, Ответ_Сообщение);
...
Функция ВыполнитьМетодСервиса(Соединение, 
	...
	Исключение
		...
		ПолучитьОписаниеОшибки(ОтветМетода, "ВыполнениеМетода");
...
Функция ОписаниеОшибки_HTTP401()
	...
	НСтр("ru = 'Пользователь не авторизован.
			...
			|3. Указанный указан неверный пароль при авторизации с подтверждением.'"));
...
Функция ОписаниеОшибки_unauthorized_client()
	...
	Результат = ОбщееОписаниеОшибки("200007", 
			НСтр("ru = 'OAuth-клиент использует незарегистрированный сценарий аутентификации (Flow)
			|или был передан некорректный redirect_uri.'"));
...
Функция ОписаниеОшибки_invalid_client()
	...
	Результат = ОбщееОписаниеОшибки("200008", 
			НСтр("ru = 'OAuth-клиент не зарегистрирован или неверно указан clientID.'"));
...
Функция ОписаниеОшибки_HTTP403()
	...
	Результат = ОбщееОписаниеОшибки("200011", 
			НСтр("ru = 'Доступ запрещен. Возможные причины ошибки:
				...
				|4. Сертификат авторизации не зарегистрирован на сервере облачной подписи.
...
Функция ОписаниеОшибки_СертификатБезУчетнойЗаписи()
	...
	Результат = ОбщееОписаниеОшибки("300048", 
			НСтр("ru = 'Выбранный сертификат является облачным, но в параметрах не указана учетная запись облачной подписи'"));
...
Функция ОписаниеОшибки_ЧужойСертификат()
	...
	Результат = ОбщееОписаниеОшибки("300049", 
			...
			|1. Сертификат привязан к другой учетной записи облачной подписи.
			|2. Нет подтверждения наличия сертификата на сервере облачной подписи.'"));
...
Функция ОписаниеОшибки_ПодсистемаОтключена()
	...
	Результат = ОбщееОписаниеОшибки("300050", 
			НСтр("ru = 'Отключена подсистема ""Облачная подпись"".
...
Функция ПакетнаяОперацияПользователя(НастройкиПользователя, ПараметрыВыполнения, ВходящиеПараметры)
	...
	Если ДанныеПодтверждения = Неопределено Тогда
		ДанныеПодтверждения = СервисКриптографииDSSПодтверждениеСервер.ДанныеВторичнойАвторизацииПоУмолчанию();
	КонецЕсли;
	...
	Если РезультатЗапроса.Выполнено Тогда
		...
		Если ТипОперации = "Подписание" Тогда
			...
			ПодписьCMS = СервисКриптографииDSSКлиентСервер.ПолучитьСвойствоПодписиCMS(Истина, Ложь);
			СервисКриптографииDSSКлиентСервер.ПолучитьИнформациюДокументаДляПодписи(ПодписьCMS, ОписаниеДокумента, ТипДокумента);
			...
			КонецЕсли;	
			СвойствоТранзакции	= СервисКриптографииDSSКлиентСервер.СвойстваТранзакцииПодписания(ПодписьCMS, ТекущийСертификат.Идентификатор);
	...
	Если ЕстьТранзакция
		...
		ИдентификаторТранзакции = РезультатЗапроса.Результат;
...
Функция ПодготовитьТранзакциюПодтверждения(НастройкиПользователя, ТипОперации, ПараметрыОперации = Неопределено) Экспорт
	...
	Если РезультатЗапроса.МаркерОбновлен Тогда
		...
		УстановитьНастройкиПользователя(НастройкиПользователя.Ссылка, НастройкиПользователя, ИсточникНастроек);
	КонецЕсли;
...
Функция ПолучитьРезультатПодтверждения(НастройкиПользователя, ИдентификаторТранзакции, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ДанныеПодтверждения.ЭтапЦикла = "Окончание" И ДанныеПодтверждения.ТипПодтверждения = "Транзакция" Тогда
		...
		Если ДанныеПодтверждения.ТипОперации = "Подписание" И ЭтоПакет Тогда
			РезультатЗапроса = СервисКриптографииDSS.ПодписатьПакет(
...
Функция СохранитьЗакрытыеДанныеПользователя(НастройкиПользователя, ИмяПараметра, ЗначениеПараметра, ПараметрыОперации = Неопределено) Экспорт
	...
	Если Успешно Тогда
		...
		УстановитьНастройкиПользователя(НастройкиПользователя.Ссылка, НастройкиПользователя, ПараметрыОперации);
	КонецЕсли;
...
Функция ОтправитьЗапросОбъектаПодтверждения(НастройкиПользователя, ДанныеПодтверждения, ДатаСеанса = Неопределено, ПараметрыОперации = Неопределено)
	...
	АдресПодключения = НастройкиПользователя.Методы.ДвухфакторнаяАвторизация;
...
Функция ПрочитатьОбъектПодтверждения(НастройкиПользователя, ИдентификаторТранзакции, ПараметрыОперации = Неопределено)
	...
	Если Результат = Неопределено Тогда
		Результат = СервисКриптографииDSSПодтверждениеСервер.ДанныеВторичнойАвторизацииПоУмолчанию();
		Результат.ЭтапЦикла = "Ошибка";
		ПолучитьОписаниеОшибки(Результат, "invalid_transaction");
	ИначеЕсли (Результат.ИдентификаторТранзакции <> ИдентификаторТранзакции 
...
Функция НастройкиРаботыТочкиПодключения(ВыборкаПользователей = Неопределено)
	...
	ИДВерсии = СервисКриптографииDSSКлиентСервер.ИдентификаторВерсииСервера(ВыборкаПользователей.ВерсияАПИ);
	...
	СервисПользователей	= ВыборкаПользователей.СервисИдентификации;
	СервисДокументов	= "";
	СервисАудита		= "";
	...
	// раздел: авторизация
	Методы.Вставить("Аутентификация", СервисИдентификации + "/oauth/authorize");					// страница для интерактивного ввода пароля пользователя
	Методы.Вставить("АвторизацияПоСертификату", СервисИдентификации + "/oauth/authorize/certificate");// аутентификация по SSL сертификату пользователя
	Методы.Вставить("Авторизация", СервисИдентификации + "/oauth/token");						// запрос токена по коду подтверждения, по паролю и прочее
	Методы.Вставить("ДвухфакторнаяАвторизация", СервисИдентификации + "/confirmation");			// двухфакторная аутентификация
	
	// раздел: управление пользователями
	Методы.Вставить("ИнформацияПользователя", СервисПользователей + "/ums/user");				// информация о пользователю по GUID
	Методы.Вставить("СписокПользователей", СервисПользователей + "/ums/users");					// работы со списком пользователей (поиск и пр.)
	Методы.Вставить("СредстваАутентификации", СервисПользователей + "/ums/authntokens");		// список средств аутентификации
	Методы.Вставить("ПолитикаУправлениеПользователями", СервисПользователей + "/ums/policy");	// политика управления пользователями
	Методы.Вставить("ПолитикаГруппыПользователей", СервисПользователей + "/ums/groupPolicy");	// политика группы пользователей
	
	Если ИДВерсии > 1 Тогда
		
		// раздел: обработка документов
		Методы.Вставить("Документы_Загрузить", СервисДокументов + "/api/documents");			// загрузить один документ на сервер, удалить документ
		Методы.Вставить("Документы_ЗагрузитьПакет", СервисДокументов + "/api/documents/pack");	// загрузить пакет документов на сервер
		Методы.Вставить("Документы_Информация", СервисДокументов + "/api/documents/%1/info");	// информация о документе по идентификатору
		Методы.Вставить("Документы_ИнформацияХеш", СервисДокументов + "/api/documents/hash/%1/info");	// информация о документе по хешу
		Методы.Вставить("Документы_ИзменитьИнформацию", СервисДокументов + "/api/documents/%1");// изменить информацию о документе по идентификатору
		Методы.Вставить("Документы_Получить", СервисДокументов + "/api/documents/%1/content");	// получить документ с сервера по идентификатору
		Методы.Вставить("Документы_Связанный", СервисДокументов + "/api/documents/%1/link");	// привязать документ на сервере по идентификаторам и получить информацию о нем
		Методы.Вставить("Документы_Конвертация", СервисДокументов + "/api/documents/%1/converted_content"); // конвертация документа, получение конвертации документа
		Методы.Вставить("Документы_КонвертацияИнформация", СервисДокументов + "/rest/api/documents/%1/converted_content/info"); // информация о конвертации документа
		
		// раздел: сервис аудита
		Методы.Вставить("Аудит_ЗапросОтчета", СервисАудита + "/api/audit");				// запрос отчета
		Методы.Вставить("Аудит_ОтчетСервера", СервисАудита + "/api/reports/create");	// запрос отчета о КриптоПро
					  
		// раздел: сведения об операциях
		Методы.Вставить("ОперацииПользователя", СервисИдентификации + "/operations");	// сведения об операциях пользователя
		
		// раздел: закрытый ключ
		Методы.Вставить("ЗакрытыйКлюч", СервисПодписи + "/rest/api/keys");				// операции с закрытым ключом
		
		// раздел: асинхронные операции
		Методы.Вставить("ПодписьАсинхронная", СервисПодписи + "/rest/api/v2/signature");// операции подписания
		Методы.Вставить("Операции", СервисПодписи + "/api/operations/%1");			// управление операциями
		
	КонецЕсли;
	
	// раздел: работа с данными
	Методы.Вставить("ПолитикаПодписи", СервисПодписи + "/rest/api/policy");						// политика подписи
	Методы.Вставить("ЗапросНаСертификат", СервисПодписи + "/rest/api/requests");				// создание запроса на сертификат
	Методы.Вставить("Транзакции", СервисПодписи + "/rest/api/transactions");					// создание транзакций для обработки
	Методы.Вставить("ХешДокумента", СервисПодписи + "/rest/api/documents/hash");				// вычисление хеша для документа
	Методы.Вставить("Сертификаты", СервисПодписи + "/rest/api/certificates");					// операции с сертификатами
	Методы.Вставить("Документы", СервисПодписи + "/rest/api/documents");						// операции с документами
	
	// раздел: работа сервиса проверки
	Если ЗначениеЗаполнено(СервисПроверки) Тогда
		Методы.Вставить("ПроверкаСертификата", СервисПроверки + "/rest/api/certificates");		// проверка сертификата
		Методы.Вставить("ПолитикаПроверки", СервисПроверки + "/rest/api/policy");				// политика проверки
		Методы.Вставить("ПроверкаПодписи", СервисПроверки + "/rest/api/signatures");			// проверка подписи
		Методы.Вставить("ИнформацияПодписантах", СервисПроверки + "/rest/api/signatures/signersInfo");// информация о подписанте
	Иначе
		Методы.Вставить("ПроверкаСертификата", "");
		Методы.Вставить("ПолитикаПроверки", "");
		Методы.Вставить("ПроверкаПодписи", "");
		Методы.Вставить("ИнформацияПодписантах", "");
	КонецЕсли;	
...
Процедура ИзменитьРежимИспользоватьСервисОблачнойПодписи(НовоеЗначение)
	...
	Если СервисКриптографииDSS.ИспользоватьСервисОблачнойПодписи() <> НовоеЗначение Тогда
		УстановитьПривилегированныйРежим(Истина);
		Константы.ИспользоватьСервисОблачнойПодписи.Установить(НовоеЗначение);
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;	
...
Функция ЗакрытьКонфиденциальноеЗначение(ТекущееЗначение)
	...
	Если ТипЗнч(ТекущееЗначение) = Тип("Структура") Тогда
		Если ТекущееЗначение.Состояние <> 3 Тогда
