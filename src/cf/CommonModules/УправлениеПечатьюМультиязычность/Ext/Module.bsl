...
Процедура ЗаполнитьПодменюЯзык(Форма, ТекущийЯзык = Неопределено, Знач Отбор = Неопределено) Экспорт
	...
	РазместитьВПодменюЕще = Элементы.Найти("ЯзыкВсеДействия") <> Неопределено;
	...
	Если Не ЭтоМобильныйКлиент Тогда
		...
		Если РазместитьВПодменюЕще Тогда
			Элементы.ЯзыкВсеДействия.Заголовок = Элементы.Язык.Заголовок;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого КодЛокализации Из ДоступныеЯзыки Цикл
		...
		Если РазместитьВПодменюЕще И Не ЭтоМобильныйКлиент Тогда
			КнопкаФормы = Элементы.Добавить(Команда.Имя+"ВсеДействия", Тип("КнопкаФормы"), Элементы.ЯзыкВсеДействия);
			КнопкаФормы.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
			КнопкаФормы.Пометка = КодЯзыка = ТекущийЯзык;
			КнопкаФормы.ИмяКоманды = Команда.Имя;
			КнопкаФормы.Заголовок = ПредставлениеЯзыка;
		...
		Если ЭтоФормаРедактирования Тогда
			...
			Если РазместитьВПодменюЕще И Не ЭтоМобильныйКлиент Тогда
				КнопкаФормы = Элементы.Добавить(Команда.Имя+"ВсеДействия", Тип("КнопкаФормы"), Элементы.ДобавляемыеЯзыкиВсеДействия);
				КнопкаФормы.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
				КнопкаФормы.ИмяКоманды = Команда.Имя;
				КнопкаФормы.Заголовок = НСтр("ru = 'Добавить:'") + " " + ПредставлениеЯзыка;
				КнопкаФормы.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
...
Функция ДоступенАвтоматическийПеревод(КодЯзыка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность.ПереводТекста") Тогда
		...
		Возврат ЭтоДополнительныйЯзыкПечатныхФорм(КодЯзыка)
			И (МодульПереводТекстаНаДругиеЯзыки.ДоступенПереводТекста()
			Или МодульПереводТекстаНаДругиеЯзыки.ДоступнаНастройкаСервисаПереводаТекста());
	КонецЕсли;
