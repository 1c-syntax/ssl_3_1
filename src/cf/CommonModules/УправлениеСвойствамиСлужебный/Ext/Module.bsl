...
//  ВидСвойства  - Строка - "ДополнительныеРеквизиты" или "ДополнительныеСведения" или "Метки".
//
...
Функция СписокСвойствДляВидаОбъектов(ВидОбъектов, Знач ВидСвойств) Экспорт
	...
	ТекстЗапроса = 
		...
		|	И ТаблицаСвойств.Свойство.ВидСвойства В (&ВидыСвойств)";
...
Процедура КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам) Экспорт
	...
	Если МетаданныеСправочника.ТабличныеЧасти.Найти("ДополнительныеРеквизиты") <> Неопределено Тогда
		...
		ЗначенияСвойств = ДополнительныеЗначенияСвойств(Свойства);
		СоответствиеЗначений = Новый Соответствие;
		Для Каждого ДополнительноеЗначение Из ЗначенияСвойств Цикл
			Значение = СоответствиеЗначений[ДополнительноеЗначение.Свойство];
			Если Значение = Неопределено Тогда
				СоответствиеЗначений.Вставить(ДополнительноеЗначение.Свойство, 
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДополнительноеЗначение.Ссылка));
			Иначе
				СоответствиеЗначений[ДополнительноеЗначение.Свойство].Добавить(ДополнительноеЗначение.Ссылка);
			КонецЕсли;
		КонецЦикла;
...
Процедура ПриПодготовкеДанныхОбъекта(Объект, ДополнительныеРеквизиты) Экспорт 
	...
	Если Объект.ДополнительныеСвойства.Свойство("ЗаписываемыеДополнительныеСведения") Тогда
		ЗначенияСвойств = Объект.ДополнительныеСвойства.ЗаписываемыеДополнительныеСведения;
	Иначе
		...
		Если ПолучатьДопРеквизиты Или ПолучатьДопСведения Тогда
			ЗначенияСвойств = УправлениеСвойствами.ЗначенияСвойств(Объект.Ссылка, ПолучатьДопРеквизиты, ПолучатьДопСведения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриПолученииРезультатовПолнотекстовогоПоиска(МетаданныеОбъекта, Значение, Представление) Экспорт
	...
	// а не форма записи регистра сведений.
...
//   ВидСвойства - ПеречислениеСсылка.ВидыСвойств
//
Функция ЗначенияСвойств(ДополнительныеСвойстваОбъекта, Наборы, ВидСвойства) Экспорт
...
Процедура ПередУдалениемСсылочногоОбъекта(Объект, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		...
		РеквизитОбъект = СтрокаТаблицы.Ссылка.ПолучитьОбъект();// ПланВидовХарактеристикОбъект.ДополнительныеРеквизитыИСведения
...
Процедура ПроверитьОбновитьСоставСвойствГруппы(Группа) Экспорт
	...
	Если Не Обновить Тогда
		Индекс = 0;
		Для каждого ДополнительныйРеквизит Из ГруппаОбъект.ДополнительныеРеквизиты Цикл
			Если ДополнительныйРеквизит.Свойство <> ДополнительныеРеквизитыГруппы[Индекс].Свойство Тогда
				Обновить = Истина;
			КонецЕсли;
			Индекс = Индекс + 1;
		КонецЦикла;
	...
	Если Не Обновить Тогда
		Индекс = 0;
		Для каждого ДополнительноеСведение Из ГруппаОбъект.ДополнительныеСведения Цикл
			Если ДополнительноеСведение.Свойство <> ДополнительныеСведенияГруппы[Индекс].Свойство Тогда
				Обновить = Истина;
			КонецЕсли;
			Индекс = Индекс + 1;
		КонецЦикла;
...
Функция ДополнительныеЗначенияСвойства(Свойство) Экспорт
	...
	ИмяПроцедуры = "УправлениеСвойствамиСлужебный.ДополнительныеЗначенияСвойства";
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяПроцедуры, "Свойство", Свойство, 
		Тип("ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения"));
	
	ЗначенияСвойств = ДополнительныеЗначенияСвойств(Свойство);
	Результат       = ЗначенияСвойств.ВыгрузитьКолонку("Ссылка");
...
Функция НаименованияНаборовСвойств(ПредопределенныеДанные = Неопределено) Экспорт
...
Процедура ОбновитьСписокСвойствТекущегоНабора(Форма, Набор, ВидСвойств, ТекущаяДоступность = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ЭтоОсновнойЯзык", ОбщегоНазначения.ЭтоОсновнойЯзык());
	Запрос.УстановитьПараметр("КодЯзыка", ТекущийЯзык().КодЯзыка);
	Запрос.УстановитьПараметр("ВидСвойства", ВидСвойств);
	...
	Если Не Форма.ПоказатьНеиспользуемыеРеквизиты Тогда
		...
		|			ТОГДА 12
		|		КОГДА Свойства.ВидСвойства = ЗНАЧЕНИЕ(Перечисление.ВидыСвойств.Метки)
		|			ТОГДА Свойства.ЦветСвойства.Порядок + 1
		|		ИНАЧЕ 11
		|	КОНЕЦ КАК НомерКартинки,
		...
		|	И СвойстваНаборов.Свойство.ВидСвойства В (ЗНАЧЕНИЕ(Перечисление.ВидыСвойств.ПустаяСсылка), &ВидСвойства)
	...
	Иначе
		...
		|	И Свойства.ВидСвойства В (ЗНАЧЕНИЕ(Перечисление.ВидыСвойств.ПустаяСсылка), &ВидСвойства)
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		ТекущийСписокЗначений = СоответствиеЗначенийРеквизитов[СтрокаТаблицы.Владелец];
		Если ТекущийСписокЗначений = Неопределено Тогда
			Индекс = 1;
			СоответствиеЗначенийРеквизитов.Вставить(СтрокаТаблицы.Владелец, СтрокаТаблицы.Наименование);
		Иначе
			Индекс = Индекс + 1;
			Если Индекс > 4 Тогда
				Продолжить;
			...
			Иначе
				ТекущийСписокЗначений = ТекущийСписокЗначений + ", " + СтрокаТаблицы.Наименование;
			КонецЕсли;
			СоответствиеЗначенийРеквизитов.Вставить(СтрокаТаблицы.Владелец, ТекущийСписокЗначений);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		РеквизитыСоЗначениями.Вставить(СтрокаТаблицы.Владелец, Истина);
	КонецЦикла;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СоздатьПредопределенныеНаборыСвойств");
