...
Функция ИнтернетПочтовыйПрофиль(УчетнаяЗапись, Знач ДляПолучения = Ложь) Экспорт
	...
	ИначеЕсли ДляПолучения Тогда
		ТребуемыеСвойства.Добавить("АдресСервераPOP3");
		ТребуемыеСвойства.Добавить("ПортPOP3");
		ТребуемыеСвойства.Добавить("ИспользоватьSSLPOP3");
		ТребуемыеСвойства.Добавить("Пользователь");
	...
	Если ЗначениеЗаполнено(ТребуемыеСвойства) Тогда
		ЗаполнитьЗначенияСвойств(Результат, Выборка, СтрСоединить(ТребуемыеСвойства, ","));
	КонецЕсли;
	...
	ПреобразуемыеЗначения = Новый Массив;
	ПреобразуемыеЗначения.Добавить("АдресСервераPOP3");
	ПреобразуемыеЗначения.Добавить("АдресСервераIMAP");
	ПреобразуемыеЗначения.Добавить("АдресСервераSMTP");
	ПреобразуемыеЗначения.Добавить("Пользователь");
	ПреобразуемыеЗначения.Добавить("ПользовательIMAP");
	ПреобразуемыеЗначения.Добавить("ПользовательSMTP");
	
	Для Каждого ИмяСвойства Из ПреобразуемыеЗначения Цикл
		Результат[ИмяСвойства] = СтрокаВPunycode(Результат[ИмяСвойства]);
	КонецЦикла;
...
Процедура СохранитьНастройкиУчетнойЗаписиДляВосстановленияПароля(Настройки) Экспорт
	...
	Константы.УчетнаяЗаписьДляВосстановленияПароля.Установить(Новый ХранилищеЗначения(СведенияОбУчетнойЗаписи, Новый СжатиеДанных(9)));
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.3.3.11", СтароеИмя, НовоеИмя, Библиотека);
	
	СтароеИмя = "Роль.ЧтениеУчетныхЗаписейЭлектроннойПочты";
	НовоеИмя  = "Роль.ДобавлениеИзменениеУчетныхЗаписейЭлектроннойПочты";
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.4.1.1", СтароеИмя, НовоеИмя, Библиотека);
...
Процедура ПриДобавленииСерверныхОповещений(Оповещения) Экспорт
	...
	ИмяОповещения = РаботаСПочтовымиСообщениямиСлужебныйКлиентСервер.ИмяСерверногоОповещения();
	
	Оповещение = СерверныеОповещения.НовоеСерверноеОповещение(ИмяОповещения);
	Оповещение.ИмяМодуляПолучения = "РаботаСПочтовымиСообщениямиСлужебныйКлиент";
...
// Определяет доступность авторизации на почтовом сервисе по данным учетной записи
// и настройкам авторизации в информационной базе.
//
// Параметры:
...
//  Булево
//
Функция ДоступнаАвторизацияПочтовогоСервиса(АдресЭлектроннойПочты, ИмяПочтовогоСервиса = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(ПочтовыйДомен) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	
	ЕстьЛичныеНастройкиАвторизации = Справочники.НастройкиАвторизацииИнтернетСервисов.ЕстьНастройкиАвторизации(
		ПочтовыйДомен,
		ИмяПочтовогоСервиса);
	
	ЕстьОбщиеНастройкиАвторизации = Справочники.ОбщиеНастройкиАвторизацииИнтернетСервисов.ЕстьНастройкиАвторизации(
		ПочтовыйДомен,
		ИмяПочтовогоСервиса);
	
	УстановитьПривилегированныйРежим(Ложь);
	
	Результат = ЕстьЛичныеНастройкиАвторизации Или ЕстьОбщиеНастройкиАвторизации;
...
Функция ЗагрузитьСообщения(Знач УчетнаяЗаписьИлиСоединение, Знач ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Если УчетнаяЗапись = Неопределено Тогда
		...
		НастройкиТранспорта.Вставить("ОставлятьКопииСообщенийНаСервере", Истина);
		НастройкиТранспорта.Вставить("ПериодХраненияСообщенийНаСервере", 0);
	Иначе
	...
	Если УчетнаяЗапись <> Неопределено И РежимТестирования Тогда	
...
Функция НастройкиПодсистемы() Экспорт
	...
	Настройки.Вставить("ИспользоватьПротоколPOP3", Истина);
...
Функция ОтправитьПисьма(УчетнаяЗаписьИлиСоединение, Письма, ТекстИсключения = Неопределено) Экспорт
	...
	Если УчетнаяЗапись <> Неопределено Тогда
		...
		Исключение
			...
			ТекстОшибки = РасширенноеПредставлениеОшибки(ИнформацияОбОшибке, ОбщегоНазначения.КодОсновногоЯзыка(), , УчетнаяЗапись);
...
// Возвращает параметры для получения пояснения по ошибке.
//
// Возвращаемое значение:
//  Структура:
//    * ТекстОшибки               - Строка - подробный текст с ошибкой.
//    * КодЯзыка                  - Строка - язык интерфейса, выбранный для текущего пользователя.
//    * Контекст                  - Строка - см. КонтекстДляПояснения.
//    * ИменаСерверов             - Строка - см. ИменаСерверовДляПояснения.
//
Функция ПараметрыПояснения() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТекстОшибки", "");
	Результат.Вставить("КодЯзыка", ТекущийЯзык().КодЯзыка);
	Результат.Вставить("Контекст", "");
	Результат.Вставить("ИменаСерверов", "");
	
	Возврат Результат;
	
КонецФункции

// Возвращает контекст, для которого требуется получить пояснения по ошибке.
//
// Возвращаемое значение:
//  Структура:
//    * Ключ - Строка
//    * Значение - Строка
//
Функция КонтекстДляПояснения() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("АвтоматическаяНастройка", "AutomaticSetting");
	Результат.Вставить("РучнаяНастройка", "ManualSetting");
	Результат.Вставить("ОтправкаПисьма", "SendingEmail");
	
	Возврат Результат;
	
КонецФункции

// Возвращает имена серверов почты, через запятую, для которых требуется получить пояснения по ошибке.
//
// Параметры:
...
//  Строка
//
Функция ИменаСерверовДляПояснения(СерверВходящейПочты, СерверИсходящейПочты) Экспорт
	
	Если ПустаяСтрока(СерверВходящейПочты) И ПустаяСтрока(СерверИсходящейПочты) Тогда
		Возврат "";
	КонецЕсли;
	
	Результат = Новый Массив;
	Результат.Добавить(СерверВходящейПочты);
	Результат.Добавить(СерверИсходящейПочты);
	
	Возврат СтрСоединить(Результат, ", ");
	
КонецФункции

// Возвращает пояснения по ошибке.
//
// Параметры:
//  ПараметрыПояснения - Структура - см. ПараметрыПояснения.
//
// Возвращаемое значение:
//  Структура:
//    * ВозможныеПричины - Массив
//    * СпособыУстранения - Массив
//
Функция ПоясненияПоОшибке(ПараметрыПояснения) Экспорт
	
	ТекстОшибки = ПараметрыПояснения.ТекстОшибки;
	КодЯзыка = ПараметрыПояснения.КодЯзыка;
	...
	Для Каждого ОписаниеОшибки Из ОписанияОшибок Цикл
		...
		Для Каждого ШаблонТекста Из ШаблоныТекста Цикл
			...
			Для Каждого ВозможнаяПричина Из СтрРазделить(ЗначениеСвойства("Reason", ОписаниеОшибки, КодЯзыка), Символы.ПС, Ложь) Цикл
				ВозможныеПричины.Добавить(ВозможнаяПричина);
			КонецЦикла;
			
			Для Каждого Элемент Из ОписаниеОшибки["HowToFix"] Цикл
				Если Не СпособУстраненияПрименим(Элемент, ПараметрыПояснения) Тогда
					Продолжить;
				КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ВозможныеПричины) Тогда
		...
		ВозможныеПричины.Добавить(НСтр("ru = 'Неполадки на почтовом сервере.'"));
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(СпособыУстранения) Тогда
		...
		Если ЭтоАвтоматическаяНастройка(ПараметрыПояснения) Тогда
		...
		СпособыУстранения.Добавить(НСтр("ru = 'Повторите действие еще раз через некоторое время.'"));
...
Функция ЭтоАвтоматическаяНастройка(ПараметрыПояснения)
	
	Возврат ПараметрыПояснения.Контекст = КонтекстДляПояснения().АвтоматическаяНастройка;
	
КонецФункции

Функция СпособУстраненияПрименим(СпособУстранения, ПараметрыПояснения)
	
	Результат = Истина;
	РасширенныеТеги = СпособУстранения["TagsV2"];
	
	Если РасширенныеТеги = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ПроверитьСпособУстраненияПоКонтексту(Результат, РасширенныеТеги, ПараметрыПояснения);
	ПроверитьСпособУстраненияПоПровайдеру(Результат, РасширенныеТеги, ПараметрыПояснения);
	
	Возврат Результат;
	
КонецФункции

Процедура ПроверитьСпособУстраненияПоКонтексту(Результат, РасширенныеТеги, ПараметрыПояснения)
	
	Контексты = РасширенныеТеги.Получить("context");
	
	Если Контексты = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТекущийКонтекст = ПараметрыПояснения.Контекст;
	
	Для Каждого Контекст Из Контексты Цикл
		
		Если Контекст = "ClientServerMode" Тогда
			Результат = Результат И Не ОбщегоНазначения.ИнформационнаяБазаФайловая();
		Иначе
			Результат = Результат И ТекущийКонтекст = Контекст;
		КонецЕсли;
...
Процедура ПроверитьСпособУстраненияПоПровайдеру(Результат, РасширенныеТеги, ПараметрыПояснения)
	
	Провайдеры = РасширенныеТеги.Получить("provider");
	
	Если Провайдеры = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИменаСерверов = ПараметрыПояснения.ИменаСерверов;
	
	Для Каждого Провайдер Из Провайдеры Цикл
		Результат = Результат И СтрНайти(ИменаСерверов, Провайдер) > 0;
	КонецЦикла;
	
КонецПроцедуры

Функция РасширенноеПредставлениеОшибки(ИнформацияОбОшибке, КодЯзыка,
	ВключитьПодробноеПредставлениеОшибки = Истина, УчетнаяЗапись = Неопределено) Экспорт
	...
	Если ТипЗнч(УчетнаяЗапись) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
		РеквизитыУчетнойЗаписи = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись, "СерверВходящейПочты, СерверИсходящейПочты");
		СерверВходящейПочты = РеквизитыУчетнойЗаписи.СерверВходящейПочты;
		СерверИсходящейПочты = РеквизитыУчетнойЗаписи.СерверИсходящейПочты;
	КонецЕсли;
	
	ПараметрыПояснения = ПараметрыПояснения();
	ПараметрыПояснения.ТекстОшибки = КраткоеПредставлениеОшибки;
	ПараметрыПояснения.КодЯзыка = КодЯзыка;
	ПараметрыПояснения.Контекст = КонтекстДляПояснения().ОтправкаПисьма;
	ПараметрыПояснения.ИменаСерверов = ИменаСерверовДляПояснения(СерверВходящейПочты, СерверИсходящейПочты);
	
	ПоясненияПоОшибке = ПоясненияПоОшибке(ПараметрыПояснения);
...
Функция ВыполнитьЗапрос(АдресСервера, АдресРесурса, ПараметрыЗапроса, ПоместитьПараметрыВТелоЗапроса = Истина) Экспорт
	...
	Если Не ОбщегоНазначения.РазрешенДоступКИнтернетСервисам() Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияАвторизацияПоПротоколуOAuth(),
			УровеньЖурналаРегистрации.Ошибка, , , ОбщегоНазначения.ЗапрещенДоступКИнтернетСервисамТекстСообщения());
		Возврат Результат;
	КонецЕсли;
...
Функция ПодключениеКПочте(Знач УчетнаяЗапись, Знач ДляПолучения = Ложь) Экспорт
	...
	Если ДляПолучения И ЗначениеЗаполнено(ИнтернетПочтовыйПрофиль.АдресСервераIMAP)
		Или Не ДляПолучения И ИспользоватьIMAPПриОтправкеПисем(ИнтернетПочтовыйПрофиль) Тогда
		Протокол = ПротоколИнтернетПочты.IMAP;
	КонецЕсли;
	
	ИнтернетПочта = Новый ИнтернетПочта;
	ПодключитьсяКИнтернетПочте(ИнтернетПочта, ИнтернетПочтовыйПрофиль, Протокол);
	
	Возврат ИнтернетПочта;
	
КонецФункции

Процедура ПодключитьсяКИнтернетПочте(ИнтернетПочта, Профиль, Протокол)
	...
	Исключение
		...
		ВызватьИсключение(УточнениеИсключения.Текст, УточнениеИсключения.Категория, КодОшибки, , ИнформацияОбОшибке);
...
Функция АдресаСерверовDNS() Экспорт
	...
	РаботаСПочтовымиСообщениямиЛокализация.ПриПолученииАдресовСерверовDNS(Результат, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.Добавить("8.8.8.8"); // dns.google - Не нарушает безопасность информации, так как ip-адрес публичный и его недоступность не блокирует работу приложения.
	Результат.Добавить("8.8.4.4"); // dns.google - Не нарушает безопасность информации, так как ip-адрес публичный и его недоступность не блокирует работу приложения.
...
Функция АдресФайлаСНастройками() Экспорт
	...
	АдресФайла = "https://downloads.1c.eu/content/common/settings/mailservers.json";
...
Функция АдресФайлаСОписаниемОшибок()
	...
	АдресФайла = "https://downloads.1c.eu/content/common/settings/v2/mailerrors.json";
...
Функция АдресВнешнегоРесурса()
	...
	АдресВнешнегоРесурса = "downloads.1c.eu";
...
Функция ОткрытаяАвторизацияПочтовогоСервисаОпубликована() Экспорт
	
	Опубликована = Ложь;
	СтандартнаяОбработка = Истина;
	
	ИнтеграцияПодсистемБСП.ПриОпределенииПубликацииОткрытойАвторизацииПочтовогоСервиса(Опубликована, СтандартнаяОбработка);
	...
	Если Не ЗначениеЗаполнено(АдресПубликацииИнформационнойБазыВИнтернете) Тогда
		Возврат Опубликована;
	КонецЕсли;
	
	Если Не ДоступнаПроверкаПубликацииВстроенногоСервиса() Тогда
		Возврат Опубликована;
	КонецЕсли;
	
	СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресПубликацииИнформационнойБазыВИнтернете);
	
	АдресСервера = СтруктураURI.Хост;
	АдресРесурса = "/" + СтруктураURI.ПутьНаСервере + "/hs/oauth2_mail/ping";
	...
	Попытка
		...
		Соединение = Новый HTTPСоединение(АдресСервера, Порт, , , Прокси, 5, ЗащищенноеСоединение);
		
		Ответ = Соединение.Получить(Запрос);
		Опубликована = Ответ.КодСостояния = 200;
		
	Исключение
		
		ШаблонКомментария = НСтр("ru = 'Не удалось проверить публикацию HTTP-сервиса ""Открытая авторизация почтового сервиса"".
			|
			|Подробное описание:
			|%1'");
		...
		ЗаписьЖурналаРегистрации(
			ИмяСобытияАвторизацияПоПротоколуOAuth(),
...
Функция ДоступнаПроверкаПубликацииВстроенногоСервиса()
	
	Возврат Не (ОбщегоНазначения.ИнформационнаяБазаФайловая()
		И (ОбщегоНазначения.ЭтоВебКлиент() Или ОбщегоНазначения.КлиентПодключенЧерезВебСервер()));
	
КонецФункции

Функция АдресОткрытойАвторизацииПочтовогоСервиса() Экспорт
	
	Адрес = ОбщегоНазначения.АдресПубликацииИнформационнойБазыВИнтернете() + "/hs/oauth2_mail/callback";
	ИнтеграцияПодсистемБСП.ПриПолученииАдресаОткрытойАвторизацииПочтовогоСервиса(Адрес);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ПолучениеСтатусовЭлектронныхПисем");
	КонецЕсли;
