		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Возникла непредвиденная ситуация при запуске приложения.
				           |
				           |Техническая информация о проблеме:
				           |Обработчики событий платформы %1 и %2 не поддерживают %3,
				           |поэтому реализованы в БСП (смотри общий модуль %4).
				           |Недопустимый вызов %5 при запуске приложения.
				           |Пока не завершена асинхронная процедура %6,
				           |следует вызывать %7.
