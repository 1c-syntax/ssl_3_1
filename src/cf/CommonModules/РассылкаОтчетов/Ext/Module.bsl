	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			...
			РасширенноеПредставлениеОшибки = РаботаСПочтовымиСообщениями.РасширенноеПредставлениеОшибки(
				ИнформацияОбОшибке, ОбщегоНазначения.КодОсновногоЯзыка(), Ложь, ПараметрыДоставки.УчетнаяЗапись);
			...
			Если НЕ ИспользуетсяПочтовыйКлиент() И ПолучитьФункциональнуюОпцию("ХранитьИсториюРассылкиОтчетов")
			   И ТипЗнч(ПараметрыЖурнала.Данные) = Тип("СправочникСсылка.РассылкиОтчетов") Тогда
				...
				АдресаПолучателя = ПараметрыДоставки.ПараметрыПисьма[КлючАдресаДоставки];
				Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"%1 %2", ТекстСообщения, РасширенноеПредставлениеОшибки);
				ЗафиксироватьНеудачныйРезультатРассылки(ПараметрыДоставки, ПараметрыЖурнала,
					ПараметрыДоставки.Получатель, АдресаПолучателя, Комментарий);
			КонецЕсли;
		КонецПопытки;
...
Процедура ЗафиксироватьНеудачныйРезультатРассылки(ПараметрыДоставки, ПараметрыЖурнала, Получатель, 
	...
	Для Каждого Кому Из АдресаПолучателя Цикл
		...
		Если ТипЗнч(РезультатОтправки) = Тип("Строка") Тогда
			ПоляИстории.Комментарий = РезультатОтправки;
		Иначе
...
Функция ПодготовитьПисьмо(ПараметрыДоставки, ПараметрыПисьма)
	...
	Для Каждого Картинка Из ПараметрыДоставки.Картинки Цикл
		Идентификатор = СтроковыеФункции.СтрокаЛатиницей(Картинка.Ключ);
		Если СтрНайти(ПараметрыПисьма.Тело, "cid:" + Картинка.Ключ) > 0 Тогда
			ПараметрыПисьма.Тело = СтрЗаменить(ПараметрыПисьма.Тело, "cid:" + Картинка.Ключ, "cid:" + Идентификатор);
		Иначе
			ПараметрыПисьма.Тело = СтрЗаменить(ПараметрыПисьма.Тело, Картинка.Ключ, "cid:" + Идентификатор);
		КонецЕсли;
		
		ФайлКартинка = Картинка.Значение.ПолучитьДвоичныеДанные();
		АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ФайлКартинка);
		
		ДанныеВложения = Новый Структура;
		ДанныеВложения.Вставить("Представление", Картинка.Ключ);
		ДанныеВложения.Вставить("АдресВоВременномХранилище", АдресВоВременномХранилище);
	...
	Для Каждого Вложение Из ПараметрыПисьма.Вложения Цикл
		ФайлВложение = Новый ДвоичныеДанные(Вложение.Значение);
		АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ФайлВложение);
		
		ДанныеВложения = Новый Структура;
		ДанныеВложения.Вставить("Представление", Вложение.Ключ);
		ДанныеВложения.Вставить("АдресВоВременномХранилище", АдресВоВременномХранилище);
		
		МассивВложений.Добавить(ДанныеВложения);
	КонецЦикла;
	
	ПараметрыПисьма.Вложения = МассивВложений;
	...
	Исключение
		...
		Если ПолучитьФункциональнуюОпцию("ХранитьИсториюРассылкиОтчетов") И Не ИспользуетсяПочтовыйКлиент() И ТипЗнч(
			...
			Для Каждого СтрокаПолучатель Из ПараметрыДоставки.Получатели Цикл
				...
				ЗафиксироватьНеудачныйРезультатРассылки(ПараметрыДоставки, ПараметрыЖурнала, СтрокаПолучатель.Ключ,
					АдресаПолучателя, Комментарий);
			КонецЦикла;
		КонецЕсли;
	КонецПопытки;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьКоличествоМесяцевХраненияИсторииРассылкиОтчетов");
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ОчиститьУстаревшиеЗаписиИсторииРассылкиОтчетов");
		Методы.Вставить("ВыполнитьРассылкуПоРасписанию");
	КонецЕсли;
	
	Методы.Вставить("РезультатФормированияПорцииОтчетов", Истина);
	Методы.Вставить("ВыполнитьОчисткуИсторииРассылкиОтчетовВФоновомЗадании", Истина);
	Методы.Вставить("ВыполнитьРассылкиВФоновомЗадании", Истина);
	Методы.Вставить("ВыполнитьРассылкиSMSСПаролямиАрхивовРассылкиОтчетовВФоновомЗадании", Истина);
