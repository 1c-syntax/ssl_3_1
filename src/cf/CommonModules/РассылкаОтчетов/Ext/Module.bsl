...
Функция СформироватьСписокПолучателейРассылки(Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	Иначе
		...
		|ГДЕ
		|	Контакты.Вид = &ВидПочтовогоАдресаПолучателей
		|	И Получатели.Ссылка В ИЕРАРХИИ
		|		(ВЫБРАТЬ
		|			Получатель
		|		ИЗ
		|			ТаблицаПолучателей
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		ПолноеИмяФайла = ПолноеИмяФайлаПоШаблону(
			КаталогПолучателя, СтрокаОтчет.Ключ, ПараметрыФормата, ПараметрыДоставки.ТранслитерироватьИменаФайлов);
...
Функция ПолноеИмяФайлаПоШаблону(Каталог, НаименованиеОтчета, Формат, Транслитерировать)
	...
	Возврат Каталог + ПривестиИмяФайла(ИмяФайла, Транслитерировать);
