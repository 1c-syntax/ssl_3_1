...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.ПараметрыПисьма.Важность = ?(ЗначениеЗаполнено(РассылкаОбъект.ВажностьПисьма),
			ВажностьИнтернетПочтовогоСообщенияИзСтроки(РассылкаОбъект.ВажностьПисьма),
			ВажностьИнтернетПочтовогоСообщения.Обычная);
		
		Если Рассылка.ПисьмоВФорматеHTML Тогда
			КартинкиПисьмаВФорматеHTML = Рассылка.КартинкиПисьмаВФорматеHTML.Получить();
...
Функция ПараметрыДоставки() Экспорт
	...
	ПараметрыДоставки.КаталогВременныхФайлов = ФайловаяСистема.ОбщийКаталогВременныхФайлов("RP" + Новый УникальныйИдентификатор);
...
Функция ВыполнитьФормированиеОтчетов(Отчеты, ПараметрыДоставки, НаименованиеРассылки, ПараметрыЖурнала) 
	...
	Иначе
		Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() И Не ОбщегоНазначения.РазделениеВключено() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение, НСтр(
...
Процедура ДобавитьВСписокДелУстановитьКаталогВременныхФайлов(ТекущиеДела)
	...
	Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта(Метаданные.Справочники.РассылкиОтчетов.ПолноеИмя());
	Для Каждого Раздел Из Разделы Цикл
		ИдентификаторРассылкаОтчетов = "РассылкаОтчетов";
		Дело = ТекущиеДела.Добавить();
		Дело.Идентификатор  = ИмяДела + СтрЗаменить(Раздел.ПолноеИмя(), ".", "");
...
Функция ВажностьИнтернетПочтовогоСообщенияИзСтроки(Важность)
	
	Если СтрСравнить(Важность, РассылкаОтчетовКлиентСервер.ВысокаяВажностьИнтернетПочтовогоСообщения()) = 0 Тогда
		Возврат ВажностьИнтернетПочтовогоСообщения.Высокая;
	ИначеЕсли СтрСравнить(Важность, РассылкаОтчетовКлиентСервер.НизкаяВажностьИнтернетПочтовогоСообщения()) = 0 Тогда
