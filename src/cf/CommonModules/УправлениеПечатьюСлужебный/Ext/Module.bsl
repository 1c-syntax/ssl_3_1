...
Функция ИнициализироватьДокументСКД()
	...
	Результат.Вставить("СвязиКолонтитулов",      Новый Соответствие);
...
Функция ИнициализироватьДокумент() Экспорт
	...
	Результат.Вставить("ТаблицаСвязейКонтента",  ТаблицаСвязейКонтента());
	Результат.Вставить("КаталогКартинок",        "");
	Результат.Вставить("РасширенияКартинок",     Новый Массив);
	Результат.Вставить("ИдентификаторДокумента", "");
...
Функция ТаблицаСвязейКонтента()
	
	ОписаниеЧисла  = Новый ОписаниеТипов("Число");
	ОписаниеСтроки = Новый ОписаниеТипов("Строка");
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("ИмяРесурса",   ОписаниеСтроки);
	Результат.Колонки.Добавить("ИдРесурса",    ОписаниеСтроки);
	Результат.Колонки.Добавить("НомерРесурса", ОписаниеЧисла);
...
Функция СтруктураОбластиИзМакета(ДеревоМакета, Области, ИндексОбласти, Гиперссылки)
	...
	Для Каждого УзелГиперссылки Из МассивУзловГиперссылок Цикл
		ИдентификаторВнешнейГиперссылки = УзелГиперссылки.Атрибуты["r:id"];
		Если ИдентификаторВнешнейГиперссылки <> Неопределено Тогда
			ТекстГиперссылки = Гиперссылки[ИдентификаторВнешнейГиперссылки];
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивПараметров,
				УправлениеПечатью.НайтиПараметрыВТексте(ТекстГиперссылки));
		КонецЕсли;
	КонецЦикла;
...
Функция СобратьФайлОфисногоДокумента(ДеревоМакета, Кодировка = "UTF-8")
	...
	Для Каждого Колонтитул Из СтруктураДокумента.Колонтитулы Цикл
		...
		УзелСвязейКолонтитула = СтруктураДокумента.СвязиКолонтитулов.Получить(Колонтитул.Ключ);
		Если УзелСвязейКолонтитула <> Неопределено И УзелСвязейКолонтитула.Строки.Количество()>0 Тогда
			ИмяФайлаСвязейКолонтитула = ДеревоМакета.ИмяКаталога + УстановитьРазделительПути("\word\_rels\") + Колонтитул.Ключ + ".xml.rels";
			ЗаписьXML = Новый ЗаписьXML;
			ЗаписьXML = ИнициализироватьЗаписьXML("", ИмяФайлаСвязейКолонтитула);
			ПоместитьДеревоВЗаписьXML(ЗаписьXML, УзелСвязейКолонтитула);
			ЗаписьXML.Закрыть();
		КонецЕсли;
...
Процедура УстановитьЗначениеВДокумент(Узел, СоответствиеЗамены, ДеревоМакета, ДобавлятьСвязи, ИмяКолонтитула = "") Экспорт
	СтруктураДокумента = ДеревоМакета.СтруктураДокумента;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ИмяКолонтитула) Тогда
			ВнестиСвязьКартинкиСКолонтитуломСКД(ДеревоМакета, ИмяКолонтитула, СтруктураКартинка);
		КонецЕсли;
...
Функция СвязьСуществуетПоИдентификатору(УзелСвязей, Идентификатор)
	
	Для Каждого СтрокаСвязи Из УзелСвязей.Строки Цикл
		ЗначениеАтрибута = СтрокаСвязи.Атрибуты.Получить("Id");
		Если ЗначениеАтрибута = Идентификатор Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ВнестиСвязьКартинкиСКолонтитуломСКД(ДеревоМакета, ИмяКолонтитула, СтруктураКартинка)
	
	ТипКартинка = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image";
	
	СвязиКолонтитулов = ДеревоМакета.СтруктураДокумента.СвязиКолонтитулов;
	УзелСвязейКолонтитула = СвязиКолонтитулов.Получить(ИмяКолонтитула);
	Если УзелСвязейКолонтитула = Неопределено Тогда
		УзелСвязейКолонтитула = ДеревоДокумента();
		КорневойУзел = УзелСвязейКолонтитула.Строки.Добавить();
		КорневойУзел.ИмяТега = "Relationships";
		КорневойУзел.Атрибуты.Вставить("xmlns", "http://schemas.openxmlformats.org/package/2006/relationships");
		СвязиКолонтитулов.Вставить(ИмяКолонтитула, УзелСвязейКолонтитула);
	КонецЕсли;
	
	ИдентификаторКартинки = СтруктураКартинка.rId;
	УзелСвязей = УзелСвязейКолонтитула.Строки[0];
	Если НЕ СвязьСуществуетПоИдентификатору(УзелСвязей, ИдентификаторКартинки) Тогда
		...
		ИмяКартинки  = СтруктураКартинка.ИмяКартинки + "." + СтруктураКартинка.РасширениеКартинки;
		ИмяРесурса   = "media/" + ИмяКартинки;
...
Функция СобратьФайлДокументаDOCX(ПечатнаяФорма)
	...
	Для Каждого Раздел Из ПечатнаяФорма.СтруктураДокумента.Разделы Цикл
		...
		Для Каждого ЭлементКолонтитула Из Раздел.Значение.Колонтитулы Цикл
			...
			Если Не ПустаяСтрока(Колонтитул.СвязиКонтента) Тогда
				ИмяФайлаСвязейКолонтитула = ПечатнаяФорма.ИмяКаталога + УстановитьРазделительПути("\word\_rels\") + Колонтитул.ИмяВнутр + ".xml.rels";
				ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаСвязейКолонтитула, КодировкаТекста.UTF8);
				ЗаписьТекста.Записать(Колонтитул.СвязиКонтента);
...
Процедура ЗаполнитьПараметрыОбласти(ПечатнаяФорма, Область, ДанныеОбъекта)
	...
	ОткрытыСвойстваСтроки = Истина;
	УстановленАтрибутСохраненияПробелов = Ложь;
	
	ШиринаТаблицы       = 0;
	ШиринаКлеткиТаблицы = 0;
	ВысотаСтрокиТаблицы = 0;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ОткрытаТаблица И ЧтениеНачалаСвойствСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML) Тогда
			ОткрытыСвойстваСтроки = Истина;
		КонецЕсли;
		
		Если ОткрытыСвойстваСтроки И ЧтениеВысотыСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML) Тогда
			УстановитьВысотуСтроки(ЧтениеXML, ВысотаСтрокиТаблицы);
		КонецЕсли;
		
		Если ЧтениеКонцаСвойствСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML) Тогда
			ОткрытыСвойстваСтроки = Ложь;
		КонецЕсли;
		
		Если ЧтениеКонцаСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML) Тогда
			ВысотаСтрокиТаблицы = 0;
		КонецЕсли;
		...
		Если ОбработатьТекст И ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
			...
			ИначеЕсли ВыводКартинки Тогда
				...
				Если ВысотаСтрокиТаблицы <> 0 Тогда
					МаксимальнаяВысотаКартинки = ВысотаСтрокиТаблицы * 914400 / РазрешениеОсновногоЭкрана / 20;
					Если ВысотаКартинки > МаксимальнаяВысотаКартинки Тогда
						КоэффициентМасштаба = МаксимальнаяВысотаКартинки / ВысотаКартинки;
						ШиринаКартинки = ШиринаКартинки * КоэффициентМасштаба;
						ВысотаКартинки = МаксимальнаяВысотаКартинки;
					КонецЕсли;
				КонецЕсли;
				...
				Если Область.Имя = "ВерхнийКолонтитул" ИЛИ Область.Имя = "НижнийКолонтитул" Тогда
					ВнестиСвязьКартинкиСКолонтитулом(Область, СтруктураКартинка);
				КонецЕсли;
...
Процедура УстановитьВысотуСтроки(ЧтениеXML, ВысотаСтроки)
	
	ТипВысоты = ЧтениеXML.ПолучитьАтрибут("w:hRule");
	
	ВысотаСтроки = 0;
	Если ТипВысоты = "exact" Тогда
		ЗначениеВысоты = ЧтениеXML.ПолучитьАтрибут("w:val");
		Если ЗначениеЗаполнено(ЗначениеВысоты) Тогда
			ВысотаСтроки = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ЗначениеВысоты);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВнестиСвязьКартинкиСКолонтитулом(Колонтитул, СтруктураКартинка)
	
	ТипКартинка = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image";
	Если ПустаяСтрока(Колонтитул.СвязиКонтента) Тогда
		Колонтитул.СвязиКонтента = "<?xml version=""1.0"" encoding=""UTF-8"" standalone=""yes""?>"
			+ "<Relationships xmlns=""http://schemas.openxmlformats.org/package/2006/relationships""></Relationships>";
	КонецЕсли;
	
	ЧтениеXML = ИнициализироватьЧтениеXML(Колонтитул.СвязиКонтента);
	ЗаписьXML = ИнициализироватьЗаписьXML("");
	
	ИмяРесурса = "media/" + СтруктураКартинка.ИмяКартинки + "." + СтруктураКартинка.РасширениеКартинки;
	ИдРесурса = СтруктураКартинка.rId;
	...
	ЧтениеXML.Закрыть();
	Колонтитул.СвязиКонтента = ЗаписьXML.Закрыть();
...
Функция ЧтениеНачалаСвойствСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML)
	
	Если Не ПустаяСтрока(СтруктураРазбораXML.БлокирующийПоток) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.Имя = "w:trPr";
	
КонецФункции

Функция ЧтениеВысотыСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML)
	
	Если Не ПустаяСтрока(СтруктураРазбораXML.БлокирующийПоток) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.Имя = "w:trHeight";
	
КонецФункции

Функция ЧтениеКонцаСвойствСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML)
	
	Если Не ПустаяСтрока(СтруктураРазбораXML.БлокирующийПоток) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента И ЧтениеXML.Имя = "w:trPr";
	
КонецФункции

Функция ЧтениеКонцаСтрокиТаблицы(СтруктураРазбораXML, ЧтениеXML)
	
	Если Не ПустаяСтрока(СтруктураРазбораXML.БлокирующийПоток) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента И ЧтениеXML.Имя = "w:tr";
