...
Функция МакетИзДвоичныхДанных(ДвоичныеДанныеМакета) Экспорт
	...
	Если Расширение <> "docx" Тогда
		ТекстОшибки = НСтр("ru = 'Макет шаблона офисного документа имеет не верный формат.'");
...
Процедура КонвертироватьПараметры(ДеревоДокумента) Экспорт
	...
	Для Каждого МассивОбласти Из МассивОбластей Цикл
		УзелНачала = МассивОбласти[0];  // СтрокаДереваЗначений: см. УправлениеПечатьюСлужебный.ПрочитатьXMLВДерево
		...
		УзелОкончания = МассивОбласти[МассивОбласти.ВГраница()]; // СтрокаДереваЗначений: см. УправлениеПечатьюСлужебный.ПрочитатьXMLВДерево
...
Функция ОбъединитьОфисныеДокументы(ОфисныеДокументы, АдресЕдиногоДокумента)
	...
	Для каждого ОфисныйДокумент Из ОфисныеДокументы Цикл
		...
		Иначе                           
			...
			УзелDocumentНовогоДокумента = СтруктураДокумента.ДеревоДокумента.Строки[0]; // СтрокаТаблицыЗначений: см. УправлениеПечатьюСлужебный.ПрочитатьXMLВДерево
...
Процедура НайтиОбласти(СтруктураДокумента, ИменаТабличныхЧастейОбъекта) Экспорт
	...
	Для Каждого ИндексНайденнойОбластиНачало Из ИндексыНайденныхОбластейНачало Цикл
		...
		Если ОбластьОкончания = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Не удалось найти окончание условной области'");
		КонецЕсли;
		
		УсловиеОбласти = ПолучитьУсловиеОбласти(ОбластьНачала);
		
		ОбластьКоллекции = Неопределено;
		ОкончанияУсловныхОбластей = Новый Массив;
		Для Каждого Область Из Области Цикл
			Если Область.Индекс < ИндексНайденнойОбластиОкончание И ИндексНайденнойОбластиНачало < Область.Индекс Тогда
				...
				Если ЭтоРодительОкончанияУсловия Тогда
					Если ОбластьКоллекции <> Неопределено Тогда
						УзелСледующейОбласти = НайтиГраницуСледующейОбласти(ОбластьКоллекции.УзелДереваДокумента, Области);
						Если УзелСледующейОбласти <> Неопределено И УзелСледующейОбласти.Индекс < ИндексНайденнойОбластиОкончание Тогда
							СтруктураОкончания = Новый Структура("УзелДереваДокумента, УсловиеОбласти");
							СтруктураОкончания.УзелДереваДокумента = УзелСледующейОбласти;
							СтруктураОкончания.УсловиеОбласти = УсловиеОбласти;
							ОкончанияУсловныхОбластей.Добавить(СтруктураОкончания);
						КонецЕсли;
					КонецЕсли;
					
					УзелСледующейОбласти = НайтиГраницуСледующейОбласти(ОбластьОкончания, Области);
		...
		Для Каждого СтруктураОкончания Из ОкончанияУсловныхОбластей Цикл
			НоваяОбласть = Области.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяОбласть, СтруктураОкончания);
			НоваяОбласть.Индекс = НоваяОбласть.УзелДереваДокумента.Индекс;
		КонецЦикла;
...
// Параметры:
//  СтрокаДерева - СтрокаДереваЗначений: см. ДеревоДокумента
//  Ключ - Строка
...
// Возвращаемое значение:
// - СтрокаДереваЗначений: см. ДеревоДокумента
// - Неопределено
//
...
Функция НайтиГраницуСледующейОбласти(Строка, Области)
	...
	Если ПерейтиКРодителю Тогда
		...
		Возврат НайтиГраницуСледующейОбласти(Родитель, Области);
...
Функция ПолучитьУсловиеОбласти(Область)
...
Процедура УстановитьЗначениеВДокумент(Узел, СоответствиеЗамены, ДеревоМакета, ДобавлятьСвязи) Экспорт
	...
	ИначеЕсли ДеревоМакета = Неопределено Или ТипЗнч(СоответствиеЗамены.Значение) <> Тип("Структура") Тогда
		...
		Иначе
			Узел.Текст = СтрЗаменить(Узел.Текст, СоответствиеЗамены.Ключ, СоответствиеЗамены.Значение);
		КонецЕсли;
...
Процедура ДобавитьОбластьСУсловием(Области, Область)
	УзелСледующейОбласти = НайтиГраницуСледующейОбласти(Область, Области);
	...
	НайденнаяОбласть.УсловиеОбласти = ПолучитьУсловиеОбласти(Область);
КонецПроцедуры
...
// Параметры:
//  УзелПоиска - СтрокаДереваЗначений: см. ДеревоДокумента
...
// Параметры:
//  УзелТаблицы - СтрокаДереваЗначений: см. ДеревоДокумента
//  УзелТекста - СтрокаДереваЗначений: см. ДеревоДокумента
//
