...
Процедура СоздатьНовоеПисьмоВложенияПодготовлены(ВложенияПодготовлены, ПараметрыОтправки, ВыполненВыборПолучателей = Ложь)
	...
	Если ПараметрыОтправки.Получатель = Неопределено Или (ПараметрыОтправки.Получатель <> Неопределено 
		И (ПараметрыОтправки.Получатель.Количество() <= 1 Или Не ПараметрыОтправки.ИнтерактивныйВыборПолучателей)) Тогда
		ПослеВыбораПолучателей(ПараметрыФормы, ПараметрыОтправки);
	ИначеЕсли ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
