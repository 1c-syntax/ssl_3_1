...
// Обработчик уточнения подключаемой команды.
//
// Параметры:
...
//
Процедура ПередВыполнениемКоманды(МассивСсылок, ПараметрыВыполнения) Экспорт
	
	Если ПараметрыВыполнения.ОписаниеКоманды.Обработчик <> "УправлениеПечатьюСлужебныйКлиент.ОбработчикКоманды"
	 Или ПараметрыВыполнения.ОписаниеКоманды.ДополнительныеПараметры.МенеджерПечати <> "СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки"
	 Или ТипЗнч(МассивСсылок) <> Тип("Массив")
	 Или Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		Возврат;
	КонецЕсли;
	
	МодульДополнительныеОтчетыИОбработкиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ДополнительныеОтчетыИОбработкиКлиент");
	МодульДополнительныеОтчетыИОбработкиКлиент.ПередВыполнениемКомандыПечати(МассивСсылок, ПараметрыВыполнения);
...
Процедура ОбработчикКоманды(Знач МассивСсылок, Знач ПараметрыВыполнения) Экспорт
	...
	Если ОписаниеКоманды.КомандаПоУмолчанию Тогда
		...
		Если ПараметрыПечатиПоУмолчанию.ЕстьПараметрыВыполнения Тогда
			...
			КонецЦикла;
		Иначе
			НовыеПараметрыВыполнения = ПараметрыПечатиПоУмолчанию.НовыеПараметрыВыполнения;
			Если НовыеПараметрыВыполнения.Количество() = 0 Тогда
				ПоказатьПредупреждение( , НСтр("ru = 'Действие не может быть выполнено для выбранных элементов.'"));
				Возврат;
			КонецЕсли;
			
			НовыеПараметры = НовыеПараметрыВыполнения[0];
			...
			ИначеЕсли СписокКоманд.Количество() = 0 Тогда
...
Процедура ВыполнитьПодключаемуюКомандуПечатиЗавершение(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры)
	...
	Если Не ПустаяСтрока(ОписаниеКоманды.Обработчик) Тогда
		...
		ЧастиИмени = СтрРазделить(ИмяОбработчика, ".");
		Свойства = Новый Структура;
		Для Каждого ЧастьИмени Из ЧастиИмени Цикл
			Попытка
				Свойства.Вставить(ЧастьИмени);
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректное имя процедуры печати ""%1"".'"), ИмяОбработчика);
		...
		Результат = Вычислить(ИмяОбработчика + "(ПараметрыПечати)"); // Не нарушает безопасность информации, так как вычисляется только функция модуля.
