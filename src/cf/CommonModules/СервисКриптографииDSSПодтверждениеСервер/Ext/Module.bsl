...
//    * Оформление					- Структура - кэш стилей, служебное
//
...
Функция ПроверитьВыбранныйСертификат(СертификатСсылка) Экспорт
	...
	Если НЕ СервисКриптографииDSSСлужебный.ПроверитьПраво("СертификатыПользователяDSS")
		ИЛИ ТипЗнч(РеквизитыСертификата.Программа) <> СервисКриптографииDSSКлиентСервер.ПолучитьТипОблачнойПодписи() Тогда
		Возврат Результат;
	КонецЕсли;
...
Процедура ДополнитьДанныеПодтверждения(ДанныеПодтверждения)
	...
	ДанныеПодтверждения.ПараметрыВариантаВстроенного.Вставить("ПодтверждатьПринудительно", Ложь);
