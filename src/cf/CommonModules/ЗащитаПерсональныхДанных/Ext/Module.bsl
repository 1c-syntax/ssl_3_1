...
Процедура ПриЗаписиФормыНастройкиРегистрацииСобытий(Форма) Экспорт
	...
	ДеревоОбластей = ДанныеФормыВЗначение(Форма[ИмяРеквизитаДеревоОбластей()], Тип("ДеревоЗначений")); // ДеревоЗначений
...
//         - ДокументОбъект
//         - РегистрСведенийНаборЗаписей - проверяемый объект. Также
...
Процедура ПеренестиЗащищаемыеОбластиПерсональныхДанных(ПараметрыОбновления) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.УдалитьОбластиПерсональныхДанных");
...
Функция ТекстЗапросаСубъектыДляСкрытияПерсональныхДанных(ВоВременнуюТаблицу = Ложь)
	...
	Для Каждого КлючИЗначение Из МетаданныеПоТипамСубъекта Цикл
		...
		|ИЗ
		|	&МетаданныеСубъекта КАК СправочникСубъекта
		...
		ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "&МетаданныеСубъекта",
			"Справочник." + МетаданныеСубъекта.Имя);
		ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, ",&ПомещениеВоВременнуюТаблицу",
			?(ВоВременнуюТаблицу И ОсновнаяТаблица, "ПОМЕСТИТЬ ВТСубъектыДляСкрытия", ""));
		ТекстЗапроса = ТекстЗапроса + ?(ПустаяСтрока(ТекстЗапроса), "", " ОБЪЕДИНИТЬ ВСЕ") + ФрагментЗапроса;
...
// Параметры:
//  Субъекты - Массив
//           - ОпределяемыйТип.СубъектПерсональныхДанных - ссылка или массив ссылок на субъекты.
//
// Возвращаемое значение:
...
// Параметры:
//  Объект - СправочникОбъект
//         - ДокументОбъект - объект со ссылкой на субъект в табличной части.
...
//      * Значение - см. НастройкиСкрытияСубъектовПоУмолчанию.
//
...
//  Субъекты - Массив
//           - ОпределяемыйТип.СубъектПерсональныхДанных - ссылка или массив ссылок на субъекты.
//  НастройкиСкрытия - см. НастройкиСкрытияСубъектовПоУмолчанию.
//
...
//     * СрокДействияСогласия - Дата
//     * Основание - ДокументСсылка.СогласиеНаОбработкуПерсональныхДанных
//                 - ДокументСсылка.ОтзывСогласияНаОбработкуПерсональныхДанных
//     * ДнейДоСкрытия - Число
//
Функция СведенияОСубъектах()
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("Субъект");
	Результат.Колонки.Добавить("СрокДействияСогласия");
	Результат.Колонки.Добавить("Основание");
	Результат.Колонки.Добавить("ДнейДоСкрытия");
	Возврат Результат;
...
//           - см. СведенияОСубъектах
//
Процедура ОбновитьНастройкиСкрытияСубъектов(Субъекты)
	...
	Если ТипЗнч(Субъекты) = Тип("Массив") Тогда
		СведенияОСубъектах = СведенияОСубъектах();
		СведенияОСубъектах.ЗагрузитьКолонку(Субъекты, "Субъект");
		СведенияОСубъектах.Индексы.Добавить("Субъект");
		
		СубъектыБезСрокаДействияСогласия = Субъекты;
	Иначе
		СведенияОСубъектах = Субъекты;
		СубъектыБезСрокаДействияСогласия = Новый Массив;
		Для каждого Субъект Из Субъекты Цикл
			Если Субъект.СрокДействияСогласия = Неопределено Тогда
				СубъектыБезСрокаДействияСогласия.Добавить(Субъект.Субъект);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	 
	Если СубъектыБезСрокаДействияСогласия.Количество() > 0 Тогда
		Запрос = ЗапросДанныеДействующихСогласий(СубъектыБезСрокаДействияСогласия);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Субъект = СведенияОСубъектах.Найти(Выборка.Субъект, "Субъект");
			Если Субъект <> Неопределено Тогда
				Субъект.СрокДействияСогласия = Выборка.СрокДействия;
				Субъект.Основание = Выборка.ДокументОснование;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Для каждого Субъект Из СведенияОСубъектах Цикл
		
		Если Субъект.ДнейДоСкрытия = Неопределено Тогда
			Субъект.ДнейДоСкрытия = ДнейДоСкрытияПерсональныхДанныхСубъектов();
		КонецЕсли;
		
		НастройкиСкрытия = НастройкиСкрытияСубъектовПоУмолчанию();
		Если ЗначениеЗаполнено(Субъект.СрокДействияСогласия) Тогда
			НастройкиСкрытия.ДатаСкрытия = Субъект.СрокДействияСогласия + Субъект.ДнейДоСкрытия * 86400;
		ИначеЕсли Субъект.СрокДействияСогласия = Дата(1, 1, 1) Тогда
			НастройкиСкрытия.ДатаСкрытия = Субъект.СрокДействияСогласия;
		КонецЕсли;
		
		НастройкиСкрытия.Основание = Субъект.Основание;
		ЗаписатьНастройкиСкрытияСубъектов(Субъект.Субъект, НастройкиСкрытия);
...
Процедура РассчитатьДатуСкрытияПерсональныхДанныхПоОснованиям(Основания)
	...
	ПрежниеСубъекты = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Субъект");
	ОбновитьНастройкиСкрытияСубъектов(ПрежниеСубъекты);
	...
	КонецЦикла;
	ОбновитьНастройкиСкрытияСубъектов(Субъекты);
...
Процедура ОчиститьСрокСкрытияПерсональныхДанныхСубъектов()
	...
	Попытка
		Блокировка = Новый БлокировкаДанных();
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СубъектыДляСкрытияПерсональныхДанных");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
		Блокировка.Заблокировать();
...
Процедура РассчитатьДатуСкрытияПДнСубъектовДавшихСогласие(ДнейДоСкрытияПДн)
	...
	Пока Выборка.Следующий() Цикл
		...
		Субъект = СведенияОСубъектах.Добавить();
		Субъект.Субъект = Выборка.Субъект;
		Субъект.СрокДействияСогласия = Выборка.СрокДействия;
		Субъект.Основание = Выборка.ДокументОснование;
		Субъект.ДнейДоСкрытия = ДнейДоСкрытияПДн;
		
	КонецЦикла;
		
	ОбновитьНастройкиСкрытияСубъектов(СведенияОСубъектах);
...
Процедура ОбновитьДатыСкрытияПерсональныхДанныхСубъектов(Субъекты) Экспорт
	...
	СубъектыКОбновлению = Новый Массив;
	НастройкиСубъектов = ПрочитатьНастройкиСкрытияСубъектов(Субъекты);
	Для Каждого КлючИЗначение Из НастройкиСубъектов Цикл
		...
		Если НастройкиСубъекта.Состояние = Перечисления.СостоянияСубъектовДляСкрытия.СкрытиеОтменено
			...
			Продолжить;
		КонецЕсли;
		СубъектыКОбновлению.Добавить(Субъект);
	КонецЦикла;
	ОбновитьНастройкиСкрытияСубъектов(СубъектыКОбновлению);
