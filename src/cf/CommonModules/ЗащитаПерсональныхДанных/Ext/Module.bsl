...
#Область ПроцедурыИФункцииДляВстраиванияПодсистемыВФормыОбъектовКонфигурации
...
Процедура ПриПолученииДанныхНаСервере(Настройки, Строки) Экспорт
	...
	Субъекты = СубъектыСИстекшимСрокомХранения(Строки.ПолучитьКлючи(), ТекущаяДата);
	...
	Для каждого Субъект Из Субъекты Цикл
		...
		Если СтрокаСписка <> Неопределено И СтрокаСписка.Данные.Свойство("ОтсутствуетСогласие") Тогда
...
Процедура ПриЗаписиФормыНастройкиРегистрацииСобытий(Форма) Экспорт
	...
	ДеревоКатегорий = Форма.РеквизитФормыВЗначение(ИмяРеквизитаДеревоКатегорий()); // ДеревоЗначений
...
Функция СубъектыСИстекшимСрокомХранения(Субъекты, ДатаАктуальности = Неопределено) Экспорт
	...
	Возврат РегистрыСведений.СрокиХраненияПерсональныхДанных.СубъектыСИстекшимСрокомХранения(Субъекты, ДатаАктуальности);
...
Процедура ПриОпределенииНастроекРегистрацииСобытийДоступаКДанным(Настройки) Экспорт
	...
	КатегорииДанных = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ИмяОбласти");
	
	// Таблица сведений о персональных данных.
	ТаблицаСведений = СведенияОПерсональныхДанных();
	
	Для Каждого СтрокаСведений Из ТаблицаСведений Цикл
		Если КатегорииДанных.Найти(СтрокаСведений.ОбластьДанных) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		// Составление описания использования события.
		...
		Для Индекс = 0 По ПоляРегистрации.ВГраница() Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Настройка = Новый ОписаниеИспользованияСобытияДоступЖурналаРегистрации(СтрокаСведений.Объект);
		Настройка.ПоляДоступа = СтрРазделить(СтрокаСведений.ПоляДоступа, ",");
		Настройка.ПоляРегистрации = ПоляРегистрации;
...
Процедура УстановитьПрефиксИнформационнойБазыИОрганизации(Источник, СтандартнаяОбработка, Префикс) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрефиксацияОбъектов") Тогда
		МодульПрефиксацияОбъектовСобытия = ОбщегоНазначения.ОбщийМодуль("ПрефиксацияОбъектовСобытия");
		МодульПрефиксацияОбъектовСобытия.УстановитьПрефиксИнформационнойБазыИОрганизации(Источник, СтандартнаяОбработка,
			Префикс);
	КонецЕсли;
...
Функция ОрганизацияПоУмолчанию() Экспорт
	...
	Организация = ЗащитаПерсональныхДанныхПереопределяемый.ОрганизацияПоУмолчанию(); // АПК:222 Для обратной совместимости.
	...
	ПолноеИмя = МетаданныеОрганизации.ПолноеИмя();
	...
	Организация = МодульОрганизации.ОрганизацияПоУмолчанию();
...
Процедура НастроитьПоказСубъектовСоСкрытымиПДн(Форма, Список)
	...
	УстановитьОтборСписка(Список, НоваяКнопка, 
		ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(Форма.ИмяФормы, "ПоказыватьСоСкрытымиПДн", Ложь));
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьДанныеОбновленияРегистровДляУничтоженияПерсональныхДанных");
	Методы.Вставить("УстановитьРасписаниеРегламентногоЗаданияРасчетСроковХраненияПерсональныхДанных");
	Методы.Вставить("ПеренестиЗащищаемыеОбластиПерсональныхДанных");
	Методы.Вставить("ЗаполнитьДанныеОбновленияРасписанияРегламентногоЗаданияРасчетСроковХраненияПерсональныхДанных");
	Методы.Вставить("ЗаполнитьРегистрыДляУничтоженияПерсональныхДанных");
	Методы.Вставить("ЗаполнитьДанныеОбновленияОбластейПерсональныхДанных");
	Методы.Вставить("ОбновитьСрокХраненияПерсональныхДанныхСубъекта", Истина);
	Методы.Вставить("ОбновитьСрокиХраненияПерсональныхДанных", Истина);
	Методы.Вставить("ПрочитатьСобытияДоступаКПерсональнымДанным", Истина);
	Методы.Вставить("ИзменитьНастройкиУничтоженияПерсональныхДанных", Истина);
	Методы.Вставить("РассчитатьСрокиХраненияПерсональныхДанных", Истина);
