...
//  ТекстОшибки - Строка
//
Процедура ЗарегистрироватьОшибкуПолученияНедоставленныхСерверныхОповещений(Знач ТекстОшибки) Экспорт
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Серверные оповещения.Ошибка получения недоставленных серверных оповещений'",
...
Процедура ЗаписатьПоказателиПроизводительности(Знач Комментарий) Экспорт
	...
	ИмяСобытия = НСтр("ru = 'Серверные оповещения.Показатели производительности'",
		ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса") Тогда
		...
		МодульОбщегоНазначенияБТС.ЗаписьТехнологическогоЖурнала(ИмяСобытия, Комментарий);
	КонецЕсли;
