...
Функция ВариантОтчета(Отчет, КлючВарианта) Экспорт
	...
	Если ТипЗнч(Отчет) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		...
		|		ПО ВариантыОтчетов.ПредопределенныйВариант = ОтчетыРасширений.Вариант
		|ГДЕ
...
#Область ПоддержкаПереопределяемыхМодулей
...
#Область ДляВызоваИзОбработчиковОбновленияПрикладнойКонфигурации
...
Процедура ЗагрузитьПользовательские(ПользовательскиеВарианты = Неопределено) Экспорт
	...
	Пока ОписаниеВарианта.Следующий() Цикл 
		...
		Иначе
			ХранилищеВарианта.Настройки = Новый ХранилищеЗначения(ОписаниеВарианта.Настройки, Новый СжатиеДанных(9));
		КонецЕсли;
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			...
			Если Выборка.Следующий() Тогда
				ВызватьИсключение(НСтр("ru = 'Недостаточно прав для просмотра.'"), КатегорияОшибки.НарушениеПравДоступа);
			Иначе
...
#Область ПрочиеПроцедурыСлужебногоПрограммногоИнтерфейса
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет ""%1"" не существует в приложении, он будет значиться как внешний.'"),
					ОтчетПолноеИмя));
...
Функция ПредопределенныеВариантыОтчетов(ТипОтчетов = "Внутренний", ПодключенныеКХранилищу = Истина) Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл 
		...
		Если ПодключенныеКХранилищу
			...
			Продолжить;
		КонецЕсли;
		ОписаниеОбъектовМетаданных.Добавить(ОтчетМетаданные);
	КонецЦикла;
	
	ИдентификаторыОтчетов = ОбщегоНазначения.ИдентификаторыОбъектовМетаданных(ОписаниеОбъектовМетаданных);
	
	Для Каждого ОтчетМетаданные Из ОписаниеОбъектовМетаданных Цикл
		
		ПолноеИмяОтчета = ОтчетМетаданные.ПолноеИмя();
		ОтчетСсылка = ИдентификаторыОтчетов.Получить(ПолноеИмяОтчета);
		...
		Если ЕстьПодключаемыеКоманды И СоставПодключаемыхОтчетовИОбработок.Содержит(ОтчетМетаданные) Тогда
			НастройкиПоставщика = МодульПодключаемыеКоманды.НастройкиПодключаемогоОбъекта(ПолноеИмяОтчета);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = НСтр("ru = 'Обновление индекса поиска отчетов, предусмотренных в приложении.'");
	КонецЕсли;
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.1.0.2", 
		"Роль.ЧтениеВариантовОтчетов", "Роль.ИспользованиеВариантовОтчетов", "СтандартныеПодсистемы");
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.3.3.3", 
		"Роль.ИспользованиеВариантовОтчетов", "Роль.ДобавлениеИзменениеЛичныхВариантовОтчетов", "СтандартныеПодсистемы");
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	КомандыОтчетов.Колонки.Добавить("Неконтекстный", Новый ОписаниеТипов("Булево"));
	...
	Для Каждого КомандаОтчетов Из КомандыОтчетов Цикл
		...
		Команда.ПараметрыФормы.Вставить("Неконтекстный", КомандаОтчетов.Неконтекстный);
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из приложения'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
	 Или ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		...
		Если ТипЗнч(Результат.Метаданные) <> Тип("ОбъектМетаданных") Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отчет ""%1"" не существует в приложении.'"),
				Результат.Имя);
			Возврат Результат;
		КонецЕсли;
...
#Область ОбновлениеИнформационнойБазыНачальноеЗаполнениеИАктуализацияСправочников
	...
	Попытка
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			...
			Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков Цикл
				...
				КодЯзыка = МодульМультиязычностьСервер.КодДополнительногоЯзыкаИнформационнойБазы(ПорядковыйНомерЯзыка);
				
				Если КодЯзыка = ТекущийЯзык().КодЯзыка Тогда
					ВариантОбъект["Наименование" + ИмяСуффиксаЯзыка ] = ?(ЗначениеЗаполнено(ОписаниеВарианта.Наименование), 
						ОписаниеВарианта.Наименование, ВариантОбъект.Наименование);
					ВариантОбъект["Описание" + ИмяСуффиксаЯзыка ] = ?(ЗначениеЗаполнено(ОписаниеВарианта.Описание),
						ОписаниеВарианта.Описание, ВариантОбъект.Описание);
				КонецЕсли; 
...
Процедура АктуализироватьВариантыОтчетовПоПредопределенным(Режим, Результат)
	...
	|	&ПредопределенныеВариантыОтчетовНаименование,
	...
	|	ПредопределенныеВариантыОтчетов.Назначение КАК Назначение,
	|	ПредопределенныеВариантыОтчетов.Родитель КАК Родитель
	|ПОМЕСТИТЬ втПредопределенные
	|ИЗ
	|	&ТаблицаПредопределенныеВариантыОтчетов КАК ПредопределенныеВариантыОтчетов
	|ГДЕ
	|	ПредопределенныеВариантыОтчетов.ПометкаУдаления = ЛОЖЬ
	|;
	|
	...
	|	&втПредопределенныеНаименование,
	|	втПредопределенные.Отчет КАК СсылкаОтчета,
	...
	Если Режим = "РазделенныеДанныеКонфигурации" Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТаблицаПредопределенныеВариантыОтчетов", "Справочник.ПредопределенныеВариантыОтчетов");
	ИначеЕсли Режим = "РазделенныеДанныеРасширений" Тогда
		Запрос.УстановитьПараметр("ТипОтчета", Перечисления.ТипыОтчетов.Расширение);
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТаблицаПредопределенныеВариантыОтчетов", "Справочник.ПредопределенныеВариантыОтчетовРасширений");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		Если ЗначениеЗаполнено(СуффиксЯзыка) Тогда
			...
			НаборПредопределенныеВариантыОтчетов = Новый Массив;
			НаборВариантыОтчетов                 = Новый Массив;
			НаборВТПредопределенные              = Новый Массив;
			НаборВТВариантыОтчетов               = Новый Массив;
			
			НаборПредопределенныеВариантыОтчетов.Добавить("ПредопределенныеВариантыОтчетов.Наименование КАК Наименование");
			НаборВариантыОтчетов.Добавить("ВариантыОтчетов.Наименование КАК Наименование");
			НаборВТПредопределенные.Добавить("втПредопределенные.Наименование КАК Наименование");
			НаборВТВариантыОтчетов.Добавить("втВариантыОтчетов.Наименование КАК РеквизитНаименование");
			
			Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков Цикл
				ИмяСуффиксаЯзыка  = МодульМультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
				ИмяРеквизита    = "Наименование" + ИмяСуффиксаЯзыка ;
				
				НаборПредопределенныеВариантыОтчетов.Добавить(
					СтрШаблон("ПредопределенныеВариантыОтчетов.%1 КАК %1", ИмяРеквизита));
				НаборВариантыОтчетов.Добавить(СтрШаблон("ВариантыОтчетов.%1 КАК %1", ИмяРеквизита));
				НаборВТПредопределенные.Добавить(СтрШаблон("втПредопределенные.%1 КАК %1", ИмяРеквизита));
				НаборВТВариантыОтчетов.Добавить(СтрШаблон("втВариантыОтчетов.%1 КАК Реквизит%1", ИмяРеквизита));
				
				ИзменяемыеРеквизиты.Вставить(ИмяРеквизита);
				СоответствиеРеквизитов.Вставить(ИмяРеквизита, "Реквизит" + ИмяРеквизита);
				
			КонецЦикла;
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПредопределенныеВариантыОтчетовНаименование", 
				СтрСоединить(НаборПредопределенныеВариантыОтчетов, "," + Символы.ПС));
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВариантыОтчетовНаименование", 
				СтрСоединить(НаборВариантыОтчетов, "," + Символы.ПС));
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&втПредопределенныеНаименование",
				СтрСоединить(НаборВТПредопределенные, "," + Символы.ПС));
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&втВариантыОтчетовНаименование",
				СтрСоединить(НаборВТВариантыОтчетов, "," + Символы.ПС));
	...
	СводнаяТаблицаПредопределенных.Колонки.СсылкаОтчета.Имя = "Отчет";
...
// Запись параметров вариантов отчетов (кэш метаданных для ускорения работы):
//
// Параметры:
...
#Область ОбновлениеПредставленийНаДругихЯзыках
...
Функция СлужебныйПользователь(Знач КодЯзыка)
	...
	Свойства.Язык = ЯзыкПоКоду(КодЯзыка).Имя;
	
	СсылкаНового = Справочники.Пользователи.ПолучитьСсылку(
		Новый УникальныйИдентификатор("ad74f108-c37d-11ef-8882-b06ebfbf08c7"));
	
	Попытка
		ПользовательИБ = Пользователи.СлужебныйПользовательИБ(Свойства, СсылкаНового);
	Исключение
		ЗаписьЖурналаРегистрации(
...
#Область РаботаСДеревомПодсистемИзФорм
...
#Область РаботаСоСпискомВариантаОтчета
...
#Область ФормированиеПредставленийПолейПараметровИОтборовДляПоиска
...
Функция ДопустимоУстановитьКонтекст(Форма) Экспорт
	...
	Возврат (Форма.ФормаПараметры.НачальныйКлючПредопределенногоВарианта = Форма.НастройкиОтчета.КлючПредопределенногоВарианта)
		Или (Форма.ФормаПараметры.НачальныйКлючВарианта = Форма.КлючТекущегоВарианта);
...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	ИсходнаяТаблица.Колонки.ОтчетСсылка.Имя = "Отчет";
...
Функция ТекстЗапросаОтчетовСУточненнымиОтборами()
	...
	ТекстЗапросаПредварительный = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ВариантыОтчетов.Отчет КАК ОтчетСсылка,
		|	ВариантыОтчетов.КлючВарианта КАК КлючВарианта,
	...
	|	ВариантыОтчетов.ОтчетСсылка КАК ОтчетСсылка,
	|	ВЫБОР
	|		КОГДА &ПолучатьИтоговуюТаблицу
	|			ТОГДА ВариантыОтчетов.ОтчетСсылка.Имя
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ИмяОтчета,
	|	ВариантыОтчетов.КлючВарианта КАК КлючВарианта,
	|	ВариантыОтчетов.ТипОтчета КАК ТипОтчета,
...
Функция ТекстЗапросаДоступныхВариантовОтчетов()
	...
	Если СуффиксТекущегоЯзыка <> Неопределено Тогда
		...
		ТекстЗапросаПредварительный = "
			...
			|ГДЕ
			|	НЕ ВариантыОтчетов.ПометкаУдаления
			|	И &УсловиеПоНазначениюВариантовОтчетов
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(втВарианты.ОтчетСсылка, втПредопределенные.ОтчетСсылка) КАК ОтчетСсылка,
	...
	|	втВсеВариантыСПодсистемами.ОтчетСсылка,
	...
	|			ТОГДА втВсеВарианты.ОтчетСсылка.Имя
	|		КОГДА втВсеВарианты.ТипОтчета = ЗНАЧЕНИЕ(Перечисление.ТипыОтчетов.Дополнительный)
	|			ТОГДА """"
	|		ИНАЧЕ ПОДСТРОКА(ВЫРАЗИТЬ(втВсеВарианты.ОтчетСсылка КАК СТРОКА(150)), 14, 137)
	|	КОНЕЦ КАК ИмяОтчета,
	|	ЕСТЬNULL(втВсеВарианты.Наименование, """") КАК Наименование,
	|	втВсеВарианты.Описание КАК Описание,
	|	втВсеВарианты.Автор КАК Автор,
	|	втВсеВарианты.ОтчетСсылка КАК ОтчетСсылка,
...
#Область ОпределениеИспользуемыхТаблиц
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр("ru = 'Отчет может содержать некорректные данные, так как не завершен переход на новую версию приложения. Если отчет долгое время недоступен, обратитесь к администратору.'");
КонецФункции
...
#Область ВспомогательныеДляСлужебногоПрограммногоИнтерфейса
	...
	Для Каждого Отчет Из СсылкиОтчетов Цикл
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" отключен в настройках приложения>'"),
					ИмяОтчета);
			Иначе
...
Функция ОбновитьВариантОтчетаПоОписанию(Знач ОписаниеВариантаОтчета, Знач ВариантОтчетаОснование = Неопределено)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ВариантОтчета.Ссылка) Тогда 
			...
			Объект.Настройки = Новый ХранилищеЗначения(ОписаниеВариантаОтчета.Настройки, Новый СжатиеДанных(9));
...
Функция ПользовательскийВариантОтчета(Отчет, КлючВарианта, Автор)
	...
	|		ПО ОтчетыРасширений.Вариант = ВариантыОтчетов.ПредопределенныйВариант
	|		И ОтчетыРасширений.Отчет = ВариантыОтчетов.Отчет
	|		И ОтчетыРасширений.КлючВарианта = ВариантыОтчетов.КлючВарианта
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(&Отчет) = ТИП(Справочник.ИдентификаторыОбъектовРасширений)
	|	И ВариантыОтчетов.Отчет = &Отчет
	|	И ВариантыОтчетов.КлючВарианта = &КлючВарианта
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьИндексПоискаПользовательскихВариантовОтчетов");
	Методы.Вставить("ОперативноеОбновлениеОбщихДанныхКонфигурации");
	Методы.Вставить("ОперативноеОбновлениеСлужебногоПользователя");
	Методы.Вставить("ОбновитьИндексПоискаПредопределенныхВариантовОтчетов");
	Методы.Вставить("ОперативноеОбновлениеРазделенныхДанныхКонфигурации");
	Методы.Вставить("НайтиВариантыОтчетовДляВывода", Истина);
	Методы.Вставить("СформироватьОтчетВФоне", Истина);
	Методы.Вставить("ЗаполнитьПредставленияПредопределенныхВариантовОтчетов", Истина);
