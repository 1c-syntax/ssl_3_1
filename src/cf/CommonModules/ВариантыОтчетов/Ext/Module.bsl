...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			КлючВарианта, МетаданныеОтчета.Имя);
	КонецЕсли;
...
//
// Параметры:
//  ПолноеИмяОтчета - Строка - см. ОбъектМетаданныхОтчет.ПолноеИмя().
...
//
// Параметры:
//  Обработчики - ТаблицаЗначений - см. ОбновлениеИнформационнойБазыБСП.ПриДобавленииОбработчиковОбновления, где:
...
//
// Параметры:
//   ОписаниеВарианта - СтрокаТаблицыЗначений - свойства варианта отчета.
//   ОписаниеОтчета - СтрокаТаблицыЗначений - свойства отчета, где:
//       * Метаданные - ОбъектМетаданныхОтчет - описание метаданных отчета.
//
...
//   Результат - Структура - см. РезультатОбновленияОбщихДанных.
//
...
//   ОписаниеВарианта - СтрокаТаблицыЗначений - свойства варианта отчета, где:
//       * Включен - Булево - если Ложь, то вариант отчета не регистрируется в подсистеме.
//   ВариантИзБазы - СтрокаТаблицыЗначений - свойства основного варианта отчета (см.
//       КоллекцияПредопределенныхВариантовОтчетов), где:
//       * Отчет - СправочникСсылка.ИдентификаторыОбъектовРасширений,
//                 СправочникСсылка.ДополнительныеОтчетыИОбработки,
//                 СправочникСсылка.ИдентификаторыОбъектовМетаданных,
...
Процедура АктуализироватьВариантыОтчетовПоПредопределенным(Режим, Результат)
	...
	Для Каждого СтрокаТаблицы Из Найденные Цикл
		...
		Если Не СтрокаТаблицы.УстановитьПометкуУдаления
			...
			ОсновнойВариант = Результат.ОсновныеВарианты.Добавить(); // см. КоллекцияОсновныхВариантовОтчетов
			ОсновнойВариант.Отчет   = СтрокаТаблицы.Отчет;
			ОсновнойВариант.Вариант = СтрокаТаблицы.РеквизитСсылка; // СправочникСсылка.ВариантыОтчетов
		КонецЕсли;
	КонецЦикла;
...
//   Результат - Структура - см. РезультатОбновленияОбщихДанных, где:
//       * ТаблицаЗамеров - ТаблицаЗначений - см. ТаблицаЗамеров.
//
...
//       * Размещение - ТабличнаяЧасть - список подсистем, в которых отображается отчет.
...
//       * Ссылка - СправочникСсылка.ВариантыОтчетов - ссылка на вариант отчета.
//   Результат - Структура - см. НайтиВариантыОтчетов
//
...
Функция ПоУмолчаниюВсеПодключеныКХранилищу()
	МетаданныеХранилища = Метаданные.ХранилищеВариантовОтчетов;
...
//       * ОсновныеВарианты - ТаблицаЗначений - см. КоллекцияОсновныхВариантовОтчетов.
//
...
//       * ТаблицаЗамеров - ТаблицаЗначение - см. ТаблицаЗамеров.
