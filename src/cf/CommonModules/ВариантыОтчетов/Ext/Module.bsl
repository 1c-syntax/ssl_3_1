...
//                            - УникальныйИдентификатор - идентификатор формы, из которой подключается отчет.
//
// Возвращаемое значение:
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
	...
	Если НадоПрочитатьСсылкуОтчета Или НадоПрочитатьНастройки Тогда
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			КонецЕсли;
		КонецЕсли;
		ИменаСвойств = "КлючВарианта, Родитель, Родитель.КлючВарианта" + ?(НадоПрочитатьСсылкуОтчета, ", Отчет", "") + ?(НадоПрочитатьНастройки, ", Настройки", "");
		...
		Если ЗначениеЗаполнено(СвойстваВарианта.Родитель) Тогда
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось прочитать схему отчета %1:
						|%2'"), ОтчетМетаданные.Имя, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				...
				Исключение
					Если ОбщегоНазначения.РазделениеВключено() Тогда
						ШаблонТекстаОшибки = НСтр("ru = 'Не удалось прочитать список вариантов отчета %1 в неразделенном сеансе,
							...
							|См. подробнее на ИТС: https://its.1c.ru/bmk/bsp_reports_service_model
							|%2'");
						
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							ШаблонТекстаОшибки, ОтчетМетаданные.Имя, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					Иначе
...
Функция ОбщиеНастройкиПанели() Экспорт
	...
	Если ОбщиеНастройки = Неопределено Тогда
		ОбщиеНастройки = Новый Структура("ПоказыватьПодсказки, ИскатьВоВсехРазделах");
...
Функция ПараметрыКлиента() Экспорт
	...
	ПараметрыКлиента.Вставить("ПрефиксЗамеров", "");
	Если ПараметрыКлиента.ВыполнятьЗамеры Тогда
		УстановитьПривилегированныйРежим(Истина);
		ПараметрыКлиента.ПрефиксЗамеров = СтрЗаменить(ПараметрыСеанса["КомментарийЗамераВремени"], ";", "; ");
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура ВывестиПризнакУведомленияПользователейВариантаОтчета(Признак) Экспорт 
	...
	Признак.Видимость = МодульОбсуждения.СистемаВзаимодействийПодключена();
...
Функция СохраняемыеПараметрыФормыОтчета(ПараметрыФормы) Экспорт
	...
	Параметры.Вставить("Расшифровка", Неопределено);
...
Функция ПодготовитьУсловиеПоискаПоСтроке(Знач Запрос, ТекстЗапроса, Знач ЕстьСтрокаПоиска, Знач ПараметрыПоиска, Знач ПравоЧтенияАвторов)
	...
	Если ЕстьСтрокаПоиска Тогда
		...
		ТекстыПоиска.Добавить("ВариантыОтчетов.ТекущиеНаименованиеВарианта"); 
		...
		ТекстыПоиска.Добавить("ВариантыОтчетов.ТекущиеОписаниеВарианта"); 
...
Функция СвойстваВариантовОтчетовИзФайлов(ОписаниеФайлов) Экспорт 
	...
	Для Каждого ОписаниеФайла Из ОписаниеФайлов Цикл 
		...
		ВариантОтчетаОснование = ВариантОтчетаОснование(
			ОписаниеВариантаОтчета.ИмяОтчета, ОписаниеВариантаОтчета.КлючВарианта);
...
//    * Ключ - Строка
//    * Значение - ПользовательскиеНастройкиКомпоновкиДанных
//    * Ссылка - см. ВариантОтчетаПоОписанию
//
...
//
// Возвращаемое значение:
//   см. ОписаниеНастроекВариантаОтчетаПоУмолчанию
//
...
//  - Неопределено
//
Функция ВариантОтчетаОснование(ИмяОтчета, КлючВарианта)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда 
		...
		ОписаниеИмениОтчета = СтрРазделить(ИмяОтчета, ".");
		ИмяОбъекта = ?(ОписаниеИмениОтчета.Количество() > 1, ОписаниеИмениОтчета[1], ИмяОтчета);
		
		МодульДополнительныеОтчетыИОбработки = ОбщегоНазначения.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
		ТекстЗапросаДополнительныхОтчетов = МодульДополнительныеОтчетыИОбработки.ТекстЗапросаДополнительныхОтчетовПоИмениОбъекта();
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫБРАТЬ НЕОПРЕДЕЛЕНО", ТекстЗапросаДополнительныхОтчетов);
