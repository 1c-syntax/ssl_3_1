...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Иначе
		ОписаниеОтчета = ОписаниеОтчета(Настройки, Отчет); // см. ОписаниеОтчета
	КонецЕсли;
...
Функция ПредопределенныеВариантыОтчетов(ТипОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОписаниеОтчета Из Результат.НайтиСтроки(Новый Структура("ЭтоВариант", Ложь)) Цикл
		...
		Если ЕстьОсновойВариант Тогда
			...
			НайденныеОписания = Результат.НайтиСтроки(ПоискОписания); // см. КоллекцияПредопределенныхВариантовОтчетов
			Описание = НайденныеОписания.Получить(0);
...
Процедура ЗаполнитьОписаниеСтрокиВарианта(ОписаниеВарианта, ОписаниеОтчета)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда 
		ОписаниеВарианта.КлючЗамеров = ОбщегоНазначения.СократитьСтрокуКонтрольнойСуммой(
			"Отчет." + ОписаниеОтчета.Метаданные.Имя + "." + ОписаниеВарианта.КлючВарианта, 135);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьОчищенныеНаименования(СвойстваВариантовОтчетов, КэшСвойствВариантовОтчетов)
	НайденныеСвойства = СвойстваВариантовОтчетов.НайтиСтроки(Новый Структура("Наименование", ""));
...
Функция АктуализироватьВариантыОтчетов(Режим)
	...
	// 3. Настроить доступность (видимость) вариантов отчетов в панели отчетов (по умолчанию).
	РегистрыСведений.НастройкиВариантовОтчетов.УстановитьДоступностьВариантовОтчетовПоУмолчанию();
...
//   ИзмененныеПодсистемы - Массив из СтрокаДереваЗначений - Массив из строк дерева значений, содержащий измененные
...
Процедура УстановитьУсловноеОформлениеСпискаПользователейВариантаОтчета(Форма) Экспорт 
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Форма.Элементы.ПользователиВариантаЗначение.Имя);
...
Процедура ОпределитьПоведениеСпискаПользователейВариантаОтчета(Форма) Экспорт 
	...
	Элементы.ПользователиВариантаГруппаПодобрать.Видимость = Форма.ИспользоватьГруппыПользователей
	...
	Если Элементы.ПользователиВариантаГруппаПодобрать.Видимость Тогда 
		
		Элементы.ПользователиВариантаГруппаПодобрать.Доступность = НастройкаДоступностиРазрешена;
		Элементы.ПользователиВариантаПодобратьПользователейГруппы.Доступность = НастройкаДоступностиРазрешена;
		Элементы.ПользователиВариантаПодобратьГруппыВнешнихПользователей.Доступность = НастройкаДоступностиРазрешена;
...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		|	РазмещениеВариантовОтчетов.Ссылка КАК Ссылка,
		|	РазмещениеВариантовОтчетов.Подсистема,
		|	ЕСТЬNULL(ПредставленияПодсистем.Представление, """") КАК НаименованиеПодсистемы
...
Функция ОписаниеНастроекВариантаОтчета(ИмяКаталога)
	...
	#Область КонструкторОписанияНастроек
	
	ОписаниеНастроек = Новый Структура("ИмяОтчета, Настройки,
		|КлючВарианта, ПредставлениеВарианта,
		|КлючТекущихПользовательскихНастроек, ПредставлениеТекущихПользовательскихНастроек,
		|ПользовательскиеНастройки, ХранилищеПользовательскихНастроек,
		|ОписаниеОшибки");
	
	ОписаниеНастроек.ПользовательскиеНастройки = Новый СписокЗначений;
	ОписаниеНастроек.ХранилищеПользовательскихНастроек = Новый Соответствие;
...
Функция ВариантОтчетаПоОписанию(ОписаниеВариантаОтчета, ВариантОтчетаОснование)
	...
	Попытка
		Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ОписаниеВариантаОтчета.ИмяОтчета);
	Исключение
		ВызватьИсключение НСтр("ru = 'Отчет отсутствует в программе'");
	КонецПопытки;
	...
	Попытка
		...
		ВариантОтчета = ПользовательскийВариантОтчета(Отчет, ОписаниеВариантаОтчета.КлючВарианта, Автор);
		...
		Иначе
			...
			Наименование = ДоступноеНаименованиеВариантаОтчета(Отчет, ОписаниеВариантаОтчета.ПредставлениеВарианта);
			...
			ДанныеЗаполнения.Вставить("Отчет", Отчет);
...
Процедура УстановитьПредставлениеПользовательскихНастроек(ШаблонОписанияНастроек, ТекущийПользователь)
	...
	Если ПользовательскиеНастройки <> Неопределено
		...
		ШаблонОписанияНастроек.Представление = ПользовательскиеНастройки[0].Наименование;
		Возврат;
...
//                  из которых требуется извлечь информацию о наименованиях полей.
//                  Имена перечисляются через запятую.
//                  К сожалению, в макетах отсутствует информация о связях полей и их представлениях
//                  (которая есть в СКД), поэтому для более точной работы механизма поиска рекомендуется
//                  заполнять НаименованияПолей, а не ИменаМакетов.
//       * СистемнаяИнформация - Структура - другая служебная информация.
//       * Тип - Строка - перечень идентификаторов типов.
//       * ЭтоВариант - Булево - признак того, что описание отчета относится к варианту отчета.
//       * ФункциональныеОпции - Массив - коллекция идентификаторов функциональных опций, где:
//             ** Строка - идентификатор функциональной опции.
