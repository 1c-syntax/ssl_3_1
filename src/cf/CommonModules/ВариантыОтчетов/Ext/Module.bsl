...
Процедура ЗапланироватьЗаполнениеПредставлений(Знач КодыЯзыков = Неопределено, Знач ИндексЯзыка = 0)
	...
	ЗаполнениеПредставлений = РегламентныеЗадания.СоздатьРегламентноеЗадание(
		Метаданные.РегламентныеЗадания.ОбновлениеПредопределенныхВариантовОтчетов);
	ЗаполнениеПредставлений.ИмяПользователя = СлужебныйПользователь(КодЯзыка);
...
Функция СлужебныйПользователь(Знач КодЯзыка)
	Результат = ПользователиИнформационнойБазы.НайтиПоИмени("СлужебныйПользовательДляОбновленияПредставлений");
	Если Результат = Неопределено Тогда
		Результат = ПользователиИнформационнойБазы.СоздатьПользователя();
		Результат.Имя = "СлужебныйПользовательДляОбновленияПредставлений";
		Результат.ЗапрещеноИзменятьПароль = Истина;
		Результат.ПоказыватьВСпискеВыбора = Ложь;
	КонецЕсли;	
	Результат.Язык = ЛокализацияСервер.ЯзыкПоКоду(КодЯзыка);
	Результат.Записать();
КонецФункции
