...
Функция ИспользуемыеТаблицыОтчета(Отчет) Экспорт
	...
	НастройкиОтчета = НастройкиФормыОтчета(Идентификатор, Неопределено, ОтчетОбъект);
				...
				Исключение
					Если ОбщегоНазначения.РазделениеВключено() Тогда
						ШаблонТекстаОшибки = НСтр("ru = 'Не удалось прочитать список вариантов отчета %1.
							|Возможно настройки содержат ссылки на разделенные предопределенные объекты.
...
//                 - СправочникСсылка.ИдентификаторыОбъектовРасширений
//    * Безопасный - Булево
//    * ПрочитатьФлажокФормироватьСразуИзПользовательскихНастроек - Булево
//    * Печать - Структура:
//        ** ПолеСверху - Число
//        ** ПолеСлева  - Число
//        ** ПолеСнизу  - Число
