...
//    * Помещения - Массив из см. ПоляПомещения
...
//    * ИдентификаторПомещения - УникальныйИдентификатор - идентификационный код помещения (квартиры, офиса) дома.
...
Функция ПоляАдреса() Экспорт
	...
	Результат = УправлениеКонтактнойИнформациейКлиентСервер.ПоляАдреса();
	...
	Результат.Вставить("ИдентификаторПомещения"         , Неопределено);
	
	Результат.Вставить("УровниМуниципальногоАдреса"                 , Новый Массив());
	Результат.Вставить("УровниАдминистративноТерриториальногоАдреса", Новый Массив());
...
// Возвращает структуру полей помещений адреса для программного формирования адреса.
//
// Возвращаемое значение:
//   Структура:
...
//
Функция ПоляПомещения() Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("ТипПомещения", "");
...
// Проверяет, соответствует ли переданный тип адреса муниципальному делению.
// Функция сравнивает входную строку с каноническим представлением типа адреса,
// возвращаемым функцией МуниципальныйАдрес().
//
// Параметры:
//   ТипАдреса - Строка - тип адресного деления для проверки.
//
// Возвращаемое значение:
//   Булево - если Истина, то тип адреса является муниципальным.
//
...
// Проверяет, соответствует ли переданный тип адреса административно‑территориальному делению.
// Функция сравнивает входную строку с каноническим представлением типа адреса,
// возвращаемым функцией АдминистративноТерриториальныйАдрес().
//
// Параметры:
//   ТипАдреса - Строка - тип адресного деления для проверки.
//
// Возвращаемое значение:
//   Булево - если Истина, то тип адреса является административно‑территориальным.
//
...
// Возвращает каноническую строку, обозначающую административно‑территориальный тип адреса.
// Используется для унификации сравнений и идентификации типа адреса в других функциях,
// например, в функции РаботаСАдресами.ПредставлениеАдресногоЭлементаПоУровню -
// для корректного получения представления адресного элемента с учетом типа деления.
//
// Возвращаемое значение:
//   Строка - каноническое представление типа адреса: "Административно-территориальный".
//
Функция АдминистративноТерриториальныйАдрес() Экспорт
	Возврат "Административно-территориальный";
КонецФункции

// Возвращает каноническую строку, обозначающую муниципальный тип адреса.
// Используется для унификации сравнений и идентификации типа адреса в других функциях,
// например, в функции РаботаСАдресами.ПредставлениеАдресногоЭлементаПоУровню -
// для корректного получения представления адресного элемента с учетом типа деления.
//
// Возвращаемое значение:
//   Строка - каноническое представление типа адреса: "Муниципальный".
//
...
Функция ПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление, ТипАдреса = Неопределено, ПредставлениеИзПолей = Ложь) Экспорт
	...
	Если ТипЗнч(Адрес) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("ru = 'Для создания представления адреса был передан недопустимый тип данных.'");
	КонецЕсли;
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(ТипАдреса) И Не ПредставлениеИзПолей Тогда
	...
	СокращатьТипы = ?(УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(ТипАдреса) И ПредставлениеИзПолей, Ложь, Истина);
	...
	Если Адрес.Свойство("houseNumber") И НЕ ПустаяСтрока(Адрес.HouseNumber) Тогда
		...
		СписокЗаполненныхУровней.Добавить(СоединитьНомерИТипЗдания(Адрес.HouseNumber, Адрес.HouseType, СокращатьТипы));
		
	ИначеЕсли Адрес.Свойство("stead") И Не ПустаяСтрока(Адрес.stead) Тогда
		
		СписокЗаполненныхУровней.Добавить(
			СоединитьНомерИТипЗдания(Адрес.stead, НаименованиеЗемельногоУчастка(), СокращатьТипы));
	...
	Если Адрес.Свойство("buildings") И Адрес.Buildings.Количество() > 0 Тогда
		...
		Для каждого Строение Из Адрес.Buildings Цикл
			Если ЗначениеЗаполнено(Строение.Number) Тогда
				...
				СписокЗаполненныхУровней.Добавить(СоединитьНомерИТипЗдания(Строение.Number, Строение.Type, СокращатьТипы));
...
//     * apartmentId - идентификатор помещения.
...
Функция СопоставлениеНаименованиеУровнюАдреса(ИмяУровня) Экспорт
	...
	Уровни.Вставить("HOUSE",       9);
	
	Возврат Уровни[ВРег(ИмяУровня)];
	
КонецФункции

Функция ИдентификаторАдреса(Адрес, ВключатьУлицу, ВключатьУровеньДома = Ложь) Экспорт
	
	ИдентификаторАдреса = Неопределено;
	
	Уровни = ИменаУровнейАдресаПоАдресу(Адрес, ВключатьУлицу, ВключатьУровеньДома);
...
Функция ИменаУровнейАдреса(ТипАдреса, ВключатьУровеньУлицы, ВключатьУровеньДома = Ложь) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоИностранныйАдресИлиЕАЭС(ТипАдреса) Тогда
...
Функция ТипЗемельногоУчастка() Экспорт
	Возврат "з/у";
КонецФункции

Функция СоединитьНомерИТипЗдания(Знач Номер, Знач ТипЗданияИлиПомещения, СокращатьТипы = Истина) Экспорт
	
	Если ПустаяСтрока(ТипЗданияИлиПомещения) Тогда
		Возврат Номер;
	КонецЕсли;
	
	Если СокращатьТипы Тогда
	...
	Иначе		
		Возврат ТипЗданияИлиПомещения + " " + Номер;
	КонецЕсли;
...
//    *  УАдресаЕстьУлицы - Булево
//
Функция ВозвращаемыеДанныеОбАдресе() Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("НаселенныйПунктДетально", Новый Структура);
	Результат.Вставить("УАдресаЕстьУлицы",        Ложь); 
	
	Возврат Результат;
	
КонецФункции

Функция НаименованиеРодительскогоОбъекта(Адрес, Родитель) Экспорт
	
	ИменаУровней = ИменаУровнейАдреса("Все", Истина);
	
	НаименованиеРодительскогоОбъекта = "";
	Для Каждого ИмяУровня Из ИменаУровней Цикл
		Если СтрСравнить(Адрес[ИмяУровня + "Id"], Родитель) = 0 Тогда
			ЭтоРегион = СтрСравнить(ИмяУровня, "area") = 0;
			НаименованиеРодительскогоОбъекта = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
				Адрес[ИмяУровня], Адрес[ИмяУровня+ "Type"], ЭтоРегион); 
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Возврат НаименованиеРодительскогоОбъекта;
