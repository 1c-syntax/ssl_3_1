...
//      * УровниАдминистративноТерриториальногоАдреса - Массив - поля для формирования административно-территориального адреса.
//		                                                           Подробнее смотри описание свойства УровниМуниципальногоАдреса.
...
Функция СведенияОбАдресеПриказМинфинаРФ171н(Адрес, ДополнительныеПараметры = Неопределено) Экспорт
	...
	УровниАдреса = ПорядокУровнейПоУмолчанию();
...
Функция ПорядокУровнейПоУмолчанию(ТолькоНаселенныйПункт = Ложь)
	...
	УровниАдреса.Добавить("НаселенныйПункт");
	УровниАдреса.Добавить("Территория");
	Если Не ТолькоНаселенныйПункт Тогда
		УровниАдреса.Добавить("Улица");
	КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "КОДРЕГИОНА" Тогда
			...
			Если ПустаяСтрока(Результат.area) Тогда
				НаименованиеИТип = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(
					РегионКода(ЭлементСписка.Значение));
...
Функция СведенияОбАдресахВВидеСтруктуры(Адреса, ДополнительныеПараметры)
	...
	Для каждого СтрокаАдреса Из ТаблицаАдресов Цикл
		...
		Если ЭтоНациональныйАдрес Тогда
			АдресПоПолям.КодРегиона = КодРегиона(АдресИзJSON.areaValue, АдресИзJSON.area, АдресИзJSON.areaType);
		КонецЕсли;
		...
		АдресПоПолям.КодМуниципальногоРайона = КодМуниципальногоРайона(АдресИзJSON.munDistrictType, АдресИзJSON.oktmo,
			ЭтоГородФедеральногоЗначения(СокрЛП(АдресИзJSON.area + " " + АдресИзJSON.areaType)));
		...
		АдресПоПолям.УровниАдминистративноТерриториальногоАдреса = ПорядокУровнейАдреса(АдресПоПолям, АдресИзJSON, РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес());
		АдресПоПолям.УровниМуниципальногоАдреса =  ПорядокУровнейАдреса(АдресПоПолям, АдресИзJSON, РаботаСАдресамиКлиентСервер.МуниципальныйАдрес());
...
Функция ПорядокУровнейАдреса(АдресПоПолям, АдресИзJSON, ТипАдреса)
		...
		Для Каждого ИмяУровня Из АдресИзJSON[ИмяПоляУровней] Цикл
			Если АдресИзJSON.Свойство(ИмяУровня) И ЗначениеЗаполнено(АдресИзJSON[ИмяУровня]) Тогда
				...
				Если ЗначениеЗаполнено(ИмяУровняПредставление) Тогда
					ИмяУровней.Добавить(ИмяУровняПредставление);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		ИмяУровней = ОпределитьПорядокУровнейДляПредставления(ПараметрыПредставленияАдреса, ТипАдреса);
		
	КонецЕсли;
	
	Для каждого ИмяУровня Из ИмяУровней Цикл
		Если ЗначениеЗаполнено(АдресПоПолям[ИмяУровня]) Тогда
			Результат.Добавить(ИмяУровня);
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(АдресПоПолям.Здание.Номер) Тогда
		Результат.Добавить("Здание");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(АдресПоПолям.Корпуса) Тогда
		Результат.Добавить("Корпуса");
		КонецЕсли;
		
	Если ЗначениеЗаполнено(АдресПоПолям.Помещения) Тогда
		Результат.Добавить("Помещения");
	КонецЕсли;
			
	Если ЗначениеЗаполнено(АдресПоПолям.НомерЗемельногоУчастка) Тогда
...
Функция КодМуниципальногоРайона(Знач ТипМуниципальногоРайона, ОКТМО, ЭтоГородФедеральногоЗначения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		КодМуниципальногоРайона = МодульАдресныйКлассификаторСлужебный.КодМуниципальногоРайона(ТипМуниципальногоРайона, 
			ОКТМО, ЭтоГородФедеральногоЗначения);
	КонецЕсли;
...
//
// Параметры:
//   ПолноеНаименование            - Строка - полное официальное наименование региона.
//   НаименованиеАдресногоЭлемента - Строка - наименование адресного элемента региона, например "Тверская".
//   ТипАдресногоЭлемента          - Строка - вид адресного элемента региона.
//
// Возвращаемое значение:
//   Строка  - код региона из двух цифр; пустая строка, если код региона по наименованию определить не удалось.
//   Неопределено - если нет подсистемы адресного классификатора.
//
Функция КодРегиона(Знач ПолноеНаименование, НаименованиеАдресногоЭлемента = "", ТипАдресногоЭлемента = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(Код) И ЗначениеЗаполнено(НаименованиеАдресногоЭлемента) Тогда
		Код = МодульАдресныйКлассификатор.КодРегионаПоНаименованию(НаименованиеАдресногоЭлемента);
		Если Не ЗначениеЗаполнено(Код) И ЗначениеЗаполнено(ТипАдресногоЭлемента) Тогда
			Код = МодульАдресныйКлассификатор.КодРегионаПоНаименованию(НаименованиеАдресногоЭлемента + " " + ТипАдресногоЭлемента);
		КонецЕсли;
	КонецЕсли;
