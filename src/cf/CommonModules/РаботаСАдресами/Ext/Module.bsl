...
Функция ПредставлениеАдреса(АдресПоПолям, ТипАдреса, ПараметрыПредставления = Неопределено) Экспорт
	...
	Для Каждого ИмяПоля Из ПорядокПолей Цикл
		...
		Если СтрСравнить(ИмяПоля, "Дом") = 0 Тогда
			...
			ПоляПредставления.Добавить(
				УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(АдресПоПолям.Здание, "Номер", "ТипЗдания", Истина));
			
			Для Каждого Корпус Из АдресПоПолям.Корпуса Цикл
				ПоляПредставления.Добавить(
					УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(Корпус, "Номер", "ТипКорпуса", Истина));
			КонецЦикла;
			
		ИначеЕсли СтрСравнить(ИмяПоля, "Помещения") = 0 Тогда
			Для Каждого Помещение Из АдресПоПолям.Помещения Цикл
				ПоляПредставления.Добавить(
					УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(Помещение, "Номер", "ТипПомещения", Истина));
			КонецЦикла;
...
Функция РазложитьПредставлениеАдресаПоПолям(Представление, ТекущийАдрес = Неопределено) Экспорт
	...
	Для Каждого ЧастьАдрес Из НераспределенныеДанные Цикл
		...
		Если ЗначениеЗаполнено(ЧастьАдрес.ТипОбъекта) Тогда
			...
			Если ВариантыДанныхДом.Найти(ТРег(ЧастьАдрес.ТипОбъекта)) <> Неопределено Тогда
				...
				ИначеЕсли ВариантыДанныхСтроение.Найти(ТРег(ЧастьАдрес.ТипОбъекта)) <> Неопределено Тогда
