...
// Проверяет адреса на соответствие ФИАС и для устаревших адресов возвращает актуальные варианты.
...
//       * АдресКорректный - Булево - признак, что адрес соответствует ФИАС. Устаревшие адреса считаются некорректными.
...
Процедура ЗаполнитьМаскиНомераТелефона(СписокВыбора) Экспорт
	...
	СписокВыбора.Добавить("+7(999) 999-99-99", СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='+7(999) 999-99-99, <span style=""color: ПоясняющийТекст""> (Пример: +7(910) 123-23-23)</span>'")));
	СписокВыбора.Добавить("##(99999) 999-99-99", СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='##(99999) 999-99-99, <span style=""color: ПоясняющийТекст""> (Пример: 8(4822) 12-34-56)</span>'")));
	СписокВыбора.Добавить("(999) 999-9999", СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='999999999, <span style=""color: ПоясняющийТекст""> (Пример: 9001234567)</span>'")));
...
Процедура УстановитьСостояниеРегламентногоЗадания(Знач Статус) Экспорт
	...
	ИдентификаторЗадания = РегламентныеЗаданияСервер.УникальныйИдентификатор(Метаданные.РегламентныеЗадания.ИсправлениеУстаревшихАдресов);
...
Процедура ИсправитьУстаревшиеАдресаПослеПроверкиАвто(Адреса, СоответствиеАдресаИСсылки)
	...
	Для каждого СтрокаТаблицы Из ГруппировкаПоОбъекту Цикл
		...
		Попытка
			...
			Для каждого СтрокаВидаКИ Из СтрокаТаблицы.Значение Цикл
