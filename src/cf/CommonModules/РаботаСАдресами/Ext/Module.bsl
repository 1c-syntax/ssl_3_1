...
// Возвращает официальное наименование субъекта РФ для адреса или пустую строку, если субъект не определен.
// Если переданная строка не содержит информации об адресе, то будет вызвано исключение.
//
// Параметры:
//    Адрес - Строка - строка в формате JSON или XML соответствующая XDTO пакету Адрес.
//          - Структура - адрес с разбивкой по полям.
//
...
Функция РегионАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	Если ЗначениеЗаполнено(Адрес.areaValue) Тогда
		ОфициальноеНаименование = Адрес.areaValue;
	Иначе 
		ОфициальноеНаименование = ОфициальноеНаименованиеРегиона(Адрес.area);
	КонецЕсли;
	
	Возврат ОфициальноеНаименование;
	
КонецФункции

// Возвращает строковое представление наименования города.
// Для города федерального значения возвращается наименование самого города.
//
// Если переданная строка не содержит корректной информации об адресе, то функция вызывает исключение.
//
// Параметры:
//   Адрес - Строка -  адрес в формате JSON или XML, соответствующий XDTO-пакету "Адрес".
//         - Структура - адрес с разбивкой по полям.
//
// Возвращаемое значение:
//   Строка - наименование города (например, "г Москва", "г Сочи")
//            или пустая строка, если город не определен.
//
Функция ГородАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	Если ТипЗнч(Адрес) = Тип("Строка") Тогда
		...
		Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Адрес) Тогда
			Адрес = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(
				Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
		КонецЕсли;
		
		Адрес = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
			Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
	...
	ИмяРегиона = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Адрес.area, Адрес.areaType);
	Если ЭтоГородФедеральногоЗначения(ИмяРегиона, Адрес.areaValue) Тогда
		
		Если ЗначениеЗаполнено(Адрес.area) Тогда
			Город = ИмяРегиона;
		ИначеЕсли ЗначениеЗаполнено(Адрес.areaValue) Тогда
			Город = Адрес.areaValue;
		Иначе
			Город = ОфициальноеНаименованиеРегиона(Адрес.area);
		КонецЕсли;
		
	ИначеЕсли СтрСравнить(Адрес.AddressType, УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес()) = 0
			И ПустаяСтрока(Адрес.City) Тогда
		Город = ИмяРегиона;
	Иначе
		Город = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Адрес.City, Адрес.CityType);
	КонецЕсли;
...
// Возвращает сведения о муниципальных адресах в виде отдельных полей адреса.
// 
// Параметры:
//  Адреса - Соответствие из КлючИЗначение:
//   * Ключ - Произвольный - идентификатор адреса
//   * Значение - Строка - адрес в формате JSON
//  ДополнительныеПараметры - см. НовыйДополнительныеПараметрыДеленийАдресов
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//   * Ключ - Произвольный - идентификатор адреса
//   * Значение - см. СведенияОМуниципальномАдресе
//
Функция СведенияОМуниципальныхАдресах(Адреса, ДополнительныеПараметры = Неопределено) Экспорт
	Возврат СведенияОАдресахОбщая(Адреса, ДополнительныеПараметры, РаботаСАдресамиКлиентСервер.МуниципальныйАдрес());
КонецФункции

// Возвращает сведения о административно территориальных адресах в виде отдельных полей адреса.
// 
// Параметры:
//  Адреса - Соответствие из КлючИЗначение:
//   * Ключ - Произвольный - идентификатор адреса
...
//  Соответствие из КлючИЗначение:
//   * Ключ - Произвольный - идентификатор адреса
//   * Значение - см. СведенияОбАдминистративноТерриториальномАдресе
//
Функция СведенияОбАдминистративноТерриториальныхАдресах(Адреса, ДополнительныеПараметры = Неопределено) Экспорт
	Возврат СведенияОАдресахОбщая(Адреса, ДополнительныеПараметры, РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес());
	
КонецФункции

// Возвращает сведения о муниципальном адресе в виде отдельных полей адреса.
// При отсутствии данных возвращается пустая строка.
// 
// Параметры:
//  Адрес - Строка - Адрес в формате JSON
//  ДополнительныеПараметры - см. НовыйДополнительныеПараметрыДеленийАдресов
//
// Возвращаемое значение:
//   Структура - сведения о муниципальном адресе:
//    * Представление    - Строка - текстовое представление адреса по муниципальному делению.
//    * Индекс           - Строка - почтовый индекс.
//    * Комментарий      - Строка - дополнительный комментарий к адресу.
//    * ИдентификаторАдресногоЭлемента - Строка - уникальный идентификатор последнего адресного элемента в адресе.
//                         Например, для адреса "г Москва, вн.тер.г. городской округ Троицк, г Троицк, ул 3-я Вишневая,
//                         д. 1" это будет идентификатор улицы "3-я Вишневая".
//    * Регион           - Структура - сведения о регионе (субъект РФ):
//      ** Представление - Строка - полное наименование региона с типом ("Московская область").
//      ** Наименование  - Строка - название региона без типа ("Московская").
//      ** Тип           - Строка - тип региона ("область", "край", "республика").
//      ** Идентификатор - Строка - уникальный идентификатор региона (GUID).
//      ** Код           - Строка - код региона РФ ("77").
//    * МуниципальныйРайон - Структура - сведения о муниципальном районе (округе):
//      ** Представление - Строка - полное наименование муниципального района ("Ленинский муниципальный район").
//      ** Наименование  - Строка - название района без типа ("Ленинский").
//      ** Тип           - Строка - тип муниципального образования ("муниципальный район").
//      ** Идентификатор - Строка - уникальный идентификатор муниципального района. 
//      ** Код           - Строка - код типа муниципального образования: 1 - муниципальный район; 2 - городской округ; 
//                                  3 - внутригородская территория; 4 - муниципальный округ; 5 - федеральная территория.
//    * Поселение        - Структура - сведения о поселении:
//      ** Представление - Строка - полное наименование поселения ("Сосновское сельское поселение").
//      ** Наименование  - Строка - название поселения без типа ("Сосновское").
//      ** Тип           - Строка - тип поселения ("сельское поселение").
//      ** Идентификатор - Строка - уникальный идентификатор поселения.
//      ** Код           - Строка - код типа поселения: 1 - городское поселение; 2 - сельское поселение; 
//                                  3 - межселенная территория; 4 - внутригородской район городского округа.
//    * НаселенныйПункт  - Структура - сведения о населенном пункте:
//      ** Представление - Строка - полное наименование населенного пункта ("деревня Восход").
//      ** Наименование  - Строка - название населенного пункта ("Восход").
//      ** Тип           - Строка - тип населенного пункта ("деревня").
//      ** Идентификатор - Строка - уникальный идентификатор населенного пункта.
//    * Территория       - Структура - сведения о территории (элемент планировочной структуры):
//      ** Представление - Строка - полное наименование территории.
//      ** Наименование  - Строка - название территории.
//      ** Тип           - Строка - тип территории.
//      ** Идентификатор - Строка - уникальный идентификатор территории.
//    * Улица            - Структура - сведения об улице (элемент улично-дорожной сети):
//      ** Представление - Строка - полное наименование улицы ("улица Ленина").
//      ** Наименование  - Строка - название улицы ("Ленина").
//      ** Тип           - Строка - тип улицы ("улица").
//      ** Идентификатор - Строка - уникальный идентификатор улицы.
//    * Здание           - Структура:
//      ** Представление - Строка - представление здания ("дом 1").
//      ** Идентификатор - Строка - идентификатор здания.
//      ** Здание - Массив из Структура:
//        *** Наименование  - Строка - номер дома ("1").
//        *** Тип           - Строка - тип здания ("дом").
//    * Помещения        - Структура:
//      ** Представление - Строка - представление помещения
//      ** Идентификатор - Строка - идентификатор помещения.
//      ** Помещение - Массив из Структура:
//        *** Наименование  - Строка - номер помещения ("1").
//        *** Тип           - Строка - тип помещения ("квартира").
//    * ЗемельныйУчасток - Структура - информация о земельном участке:
//      ** Представление - Строка - представление земельного участка
//      ** Наименование  - Строка - номер земельного участка.
//      ** Тип           - Строка - тип земельного участка (всегда "з/у").
//      ** Идентификатор - Строка - идентификатор участка.
//
Функция СведенияОМуниципальномАдресе(Адрес, ДополнительныеПараметры = Неопределено) Экспорт
	
	Адреса = Новый Соответствие();
	Адреса.Вставить(1, Адрес);
	Результаты = СведенияОМуниципальныхАдресах(Адреса, ДополнительныеПараметры);
	Возврат Результаты[1];
	
КонецФункции

// Возвращает сведения об административно-территориальном адресе в виде отдельных полей адреса.
// 
// Параметры:
//  Адрес - Строка - Адрес в формате JSON.
//  ДополнительныеПараметры - см. НовыйДополнительныеПараметрыДеленийАдресов
...
//    * Комментарий      - Строка - дополнительный комментарий к адресу.
//    * ИдентификаторАдресногоЭлемента - Строка - уникальный идентификатор последнего адресного элемента в адресе.
//                         Например, для адреса "город Москва, город Троицк, улица 3-я Вишневая, дом 1"
//                         это будет идентификатор улицы "3-я Вишневая".
//    * Регион           - Структура - сведения о регионе (субъект РФ):
//      ** Представление - Строка - полное наименование региона с типом ("Московская область").
//      ** Наименование  - Строка - название региона без типа ("Московская").
//      ** Тип           - Строка - тип региона ("область", "край", "республика").
//      ** Идентификатор - Строка - уникальный идентификатор региона (GUID).
//      ** Код           - Строка - код региона РФ ("77").
//    * Район - Структура - сведения о административном районе:
//      ** Представление - Строка - полное наименование административного района.  Например, "Ленинский район".
//      ** Наименование  - Строка - название административного района без типа. Например, "Ленинский".
//      ** Тип           - Строка - тип административного района. Например, "р-н" или "район".
//      ** Идентификатор - Строка - уникальный идентификатор района.
//    * Город            - Структура - сведения о городе:
//      ** Представление - Строка - полное наименование города. Например, "г Торжок".
//      ** Наименование  - Строка - название города без типа. Например, "Торжок".
//      ** Тип           - Строка - тип города. Например, "г".
//      ** Идентификатор - Строка - уникальный идентификатор города.
//    * НаселенныйПункт  - Структура - сведения о населенном пункте:
//      ** Представление - Строка - полное наименование населенного пункта. Например, "деревня Восход".
//      ** Наименование  - Строка - название населенного пункта. Например, "Восход".
//      ** Тип           - Строка - тип населенного пункта. Например, "деревня".
//      ** Идентификатор - Строка - уникальный идентификатор населенного пункта.
//    * Территория       - Структура - сведения о территории (элемент планировочной структуры):
//      ** Представление - Строка - полное наименование территории.
//      ** Наименование  - Строка - название территории.
...
//      ** Идентификатор - Строка - уникальный идентификатор территории.
//    * Улица            - Структура - сведения об улице (элемент улично-дорожной сети):
//      ** Представление - Строка - полное наименование улицы. Например, "ул. Ленина".
//      ** Наименование  - Строка - название улицы. Например, "Ленина".
//      ** Тип           - Строка - тип улицы. Например, "ул.".
//      ** Идентификатор - Строка - уникальный идентификатор улицы.
//    * Здания           - Структура - информация о здание (строение), сооружение:
//      ** Представление - Строка - представление здания
//      ** Идентификатор - Строка - идентификатор здания.
//      ** Здание - Массив из Структура:
//        *** Наименование  - Строка - номер дома. Например, "1".
//        *** Тип           - Строка - тип здания. Например, "дом".
//    * Помещения        - Структура: - информация о помещения:
//      ** Представление - Строка - представление помещения
//      ** Идентификатор - Строка - идентификатор помещения.
//      ** Помещение - Массив из Структура:
//        *** Наименование  - Строка - номер помещения, Например, "1".
//        *** Тип           - Строка - тип помещения. Например, "квартира".
//    * ЗемельныйУчасток - Структура - информация о земельном участке:
//      ** Представление - Строка - представление земельного участка
//      ** Наименование  - Строка - номер земельного участка.
//      ** Тип           - Строка - тип земельного участка (всегда "з/у").
//      ** Идентификатор - Строка - идентификатор участка.
//
Функция СведенияОбАдминистративноТерриториальномАдресе(Адрес, ДополнительныеПараметры = Неопределено) Экспорт
	
	Адреса = Новый Соответствие();
	Адреса.Вставить(1, Адрес);
	Результаты = СведенияОбАдминистративноТерриториальныхАдресах(Адреса, ДополнительныеПараметры);
	Возврат Результаты[1];
	
КонецФункции

// Возвращает структуру с дополнительными параметрами формирования представлений делений адресов.
//
//
// Возвращаемое значение:
//   Структура:
//     * ВариантНаписанияТипа - Строка -способ отображения типа адресного элемента:
//          "Полный"            - полное написание (например, "город");
//          "Сокращенный"       - сокращенное написание (например, "г.");
//          "СокращенныйПоПриказуМинфина171н" - сокращенное написание, соответствующее
//                                                требованиям приказа Минфина РФ № 171н
//                                                для типов адресных элементов, домов, зданий и помещений;
//          "" (пустая строка)  - по умолчанию, преобразование типа не выполняется.
//     * ВключатьСтрануВПредставление  - Булево -
//         если Истина, то в строковом представлении адреса будет указана страна;
//         если Ложь, страна исключается из представления.
//     * ВключатьИндексВПредставление - Булево -
//         если Истина, то в строковом представлении адреса будет включен почтовый индекс;
//         если Ложь, индекс не включается в представление.
//
Функция НовыйДополнительныеПараметрыДеленийАдресов() Экспорт
	
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("ВариантНаписанияТипа",            "");
	ДополнительныеПараметры.Вставить("ВключатьСтрануВПредставление",    Ложь);
	ДополнительныеПараметры.Вставить("ВключатьИндексПредставление",     Истина);
	
	Возврат ДополнительныеПараметры; 
	
КонецФункции

// Возвращает представление адресного элемента по правилам ГАР.
// Для региона при доступности адресного классификатора всегда возвращается официальное наименование.
// 
// Параметры:
//  Наименование         - Строка - название адресного объекта ("Московская", "Ленинская", "Сосновское").
//  Тип                  - Строка - тип адресного объекта ("область", "д.", "ш").
//  ИмяУровня            - Строка - (необязательный) уровень адресного элемента ("Регион", "Город", Улица" и др.) 
//                                  Если не указан, то будет использован первый найденный вариант.
//  ВариантНаписанияТипа - Строка - Определяет формат написания типа адресного элемента (кроме Региона). Варианты значений,
//                                   Полный - полное написание (например, «область»);
//                                   Сокращенный - сокращенное написание (например, «обл.»);
//                                   СокращенныйПоПриказуМинфина171н - сокращенное написание , где типы адресных элементов,
//                                                  типы домов, зданий и помещений соответствуют приказу Минфина РФ N 171н.
//                                   "" - По умолчанию, пустая строка (преобразование не выполняется).
// 
// Возвращаемое значение:
//  Строка - полное представление адресного элемента:
//           "Московская область", "Ленинский муниципальный район", "Московская обл." (при сокращении).
// 
// Пример:
//  Регион = ПредставлениеАдресногоЭлемента("Кабардино-Балкарская", "респ", Истина); // "Кабардино-Балкарская Республика"
//  Район  = ПредставлениеАдресногоЭлемента("Ленинский", "муниципальный район");     // "муниципальный район Ленинский"
//  Регион = ПредставлениеАдресногоЭлемента("Приморск", "г.");                       // "г. Приморск"
//  Улица  = ПредставлениеАдресногоЭлемента("Сосновское", "ш",, "Полный");  // "шоссе Сосновское"
//
Функция ПредставлениеАдресногоЭлемента(Наименование, Тип, ИмяУровня = "", ВариантНаписанияТипа = "") Экспорт
	
	Результат = "";
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		Результат = МодульАдресныйКлассификаторСлужебный.ПредставлениеАдресногоЭлемента(Наименование, 
					Тип, ИмяУровня, ВариантНаписанияТипа);
	КонецЕсли;
	
	Если ПустаяСтрока(Результат) Тогда
		Результат = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(Наименование, Тип);
	КонецЕсли;
...
// Возвращает представление адресного элемента из адреса по указанному уровню и типу деления.
// Для регионов автоматически возвращает официальное наименование из ГАР.
// 
// Параметры:
//  Адрес                - Структура - адрес в формате JSON.
//  ИмяУровня      - Строка - уровень адресного объекта по ГАР: "Регион", "МуниципальныйРайон", "Поселение", 
//                                            "Город", "НаселенныйПункт", "Территория", "Улица".
//  ТипАдреса            - Строка - тип адресного деления: 
//                                  РаботаСАдресамиКлиентСервер.МуниципальныйАдрес() или 
//                                  РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес().
//  ВариантНаписанияТипа - Строка - вариант сокращения типа адресного элемента или объекта:
//                                  Полный - полное написание (например, «область»);
//                                  Сокращенный - сокращенное написание (например, «обл.»);
//                                  СокращенныйПоПриказуМинфина171н - сокращенное написание , где типы адресных элементов,
//                                  типы домов, зданий и помещений соответствуют приказу Минфина РФ N 171н.
...
// 
// Возвращаемое значение:
//  Строка - полное представление адресного элемента или пустая строка:
//           "Кемеровская область - Кузбасс", "улица Широкая".
//           Если передан несуществующий уровень, то будет возвращена пустая строка.
// 
// Пример:
//  Регион    = ПредставлениеАдресногоЭлементаПоУровню(Адрес, "Регион");            // "Московская область"
//  Район     = ПредставлениеАдресногоЭлементаПоУровню(Адрес, "МуниципальныйРайон", 
//                 РаботаСАдресамиКлиентСервер.МуниципальныйАдрес());               // "Ленинский муниципальный район"
//  Улица     = ПредставлениеАдресногоЭлементаПоУровню(Адрес, "Улица");             // "ул Широкая"
//
Функция ПредставлениеАдресногоЭлементаПоУровню(Адрес, ИмяУровня, ТипАдреса, ВариантНаписанияТипа = "") Экспорт
	
	Уровни =  ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИмяУровня);
	Результат = ПредставлениеАдресныхЭлементовПоУровням(Адрес, Уровни, ТипАдреса, ВариантНаписанияТипа);
	Возврат Результат[ИмяУровня];
...
// Параметры:
//  Адрес                - Структура - адрес в формате JSON.
//  ИменаУровней         - Массив из Строка - уровни адресного объектов по ГАР: "Регион", "МуниципальныйРайон", "Поселение", 
//                                            "Город", "НаселенныйПункт", "Территория", "Улица".
//  ТипАдреса            - Строка - тип адресного деления: 
//                                  РаботаСАдресамиКлиентСервер.МуниципальныйАдрес() или 
//                                  РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес().
...
// Возвращаемое значение:
//   Соответствие из КлючИЗначение:
//     * Ключ - Строка - элементы из ИмяУровней, 
//     * Значение - Строка - строковые представления адресных элементов. 
//                           Для уровней, которые отсутствуют или не могут быть представлены, возвращается пустая строка.
//
Функция ПредставлениеАдресныхЭлементовПоУровням(Адрес, ИменаУровней, ТипАдреса, ВариантНаписанияТипа = "") Экспорт
	
	Результаты = Новый Соответствие();
	УровниАдреса = СоответствиеНаименованиеУровнейПолямJSON();
	
	АдресИзJSON = ?(ТипЗнч(Адрес) = Тип("Строка"), 
		УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес),
		Адрес);
	
	ИмяУровней = ПорядокУровнейАдреса(АдресИзJSON, ТипАдреса);
	
	Для Каждого УровеньЭлемента Из ИменаУровней Цикл
		ИмяУровня = ?(ЗначениеЗаполнено(УровниАдреса[УровеньЭлемента]),
			УровниАдреса[УровеньЭлемента], "");
		
		Если Не ЗначениеЗаполнено(ИмяУровня) ИЛИ ИмяУровней.Найти(УровеньЭлемента) = Неопределено Тогда
			Результаты.Вставить(УровеньЭлемента, "");
			Продолжить;
		КонецЕсли;
		
		Если УровеньЭлемента = "Здание" Тогда
			ЧастиПредставления = Новый Массив();
			
			Если ЗначениеЗаполнено(АдресИзJSON.houseNumber) Тогда
				Часть = ЧастьПредставленияЗданияИПомещения(АдресИзJSON.houseNumber, АдресИзJSON.houseType, 
					ВариантНаписанияТипа, УровеньЭлемента);
				ЧастиПредставления.Добавить(Часть);
			КонецЕсли;
			
			Для Каждого Строение Из АдресИзJSON.buildings Цикл
				Часть = ЧастьПредставленияЗданияИПомещения(Строение.number, Строение.type,
					ВариантНаписанияТипа, УровеньЭлемента);
				ЧастиПредставления.Добавить(Часть);
			КонецЦикла;
			
			Результаты.Вставить(УровеньЭлемента, СтрСоединить(ЧастиПредставления, ", "));
			Продолжить;
			
		ИначеЕсли УровеньЭлемента = "Помещение" Тогда
			ЧастиПредставления = Новый Массив();
			
			Для Каждого Помещение Из АдресИзJSON.apartments Цикл
				Часть = ЧастьПредставленияЗданияИПомещения(Помещение.number, Помещение.type,
					ВариантНаписанияТипа, УровеньЭлемента);
				ЧастиПредставления.Добавить(Часть);
			КонецЦикла;
		...
		Результат = ПредставлениеАдресногоЭлемента(АдресИзJSON[ИмяУровня], АдресИзJSON[ИмяУровня + "Type"], 
			УровеньЭлемента, ВариантНаписанияТипа);
		Результаты.Вставить(УровеньЭлемента, Результат);
...
//                                     Массив может содержать до 1000 элементов.
...
//        * Помещения - Массив из см. РаботаСАдресамиКлиентСервер.ПоляПомещения - перечень помещений адреса.
//        * НомерЗемельногоУчастка - Строка - текстовое представление номера земельного участка (только для адресов РФ).
//        * УровниМуниципальногоАдреса - Массив - поля, используемые для формирования муниципального адреса. 
//                                                Порядок элементов в массиве определяет последовательность отображения полей. 
//                                                Имена уровней соответствуют названиям свойств, которые возвращает эта функция.
//        * УровниАдминистративноТерриториальногоАдреса - Массив - поля для формирования
//                                                                 административно-территориального адреса. Подробнее
//                                                                 смотри описание свойства УровниМуниципальногоАдреса.
...
//        * ИдентификаторПомещения        - УникальныйИдентификатор - идентификационный код помещения дома адресного объекта.
//                                        Поле отсутствует, если дополнительный параметр КодыАдреса равен Ложь.
//                                        Пустая строка если значение отсутствует.
...
// Если в качестве адреса передана пустая строка, функция вызывает конструктор для создания адреса с пустыми полями 
// (аналогично поведению функции СведенияОбАдресе). В этом случае у возвращаемого значения 
// будут отсутствовать поля: КодМуниципальногоРайона, КодПоселения и др.
//
// Параметры:
//  Адрес - Строка - адрес во внутреннем формате JSON или в XML, соответствующем XDTO-пакету Адрес.
...
Функция СведенияОбАдресеПриказМинфинаРФ171н(Адрес, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(Адрес)
	 Или УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Адрес) 
	 Или ТипЗнч(Адрес) = Тип("Структура") Тогда
		АдресПоПолям = СведенияОбАдресе(Адрес, Параметры);
	Иначе
		Возврат СведенияОбАдресе(Неопределено, Параметры);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		ПереченьЭлементов = МодульАдресныйКлассификаторСлужебный.АдресныеСокращенияПоПриказуМинфинаРФ171н();
	...
	УровниАдреса = ПорядокУровнейАдресныхЭлементовПоУмолчанию();
...
Функция ПоляАдресаВJSON(ПоляАдреса) Экспорт
	...
	НаборПолейАдреса = СведенияОбАдресе(Неопределено);
	ЗаполнитьЗначенияСвойств(НаборПолейАдреса, ПоляАдреса);
	
	ТипУникальныйИдентификатор = Тип("УникальныйИдентификатор");
	
	Результат.AddressType = НаборПолейАдреса.ТипАдреса;
	
	Если НаборПолейАдреса.ТипАдреса <> РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес() Тогда
		Результат.Value = НаборПолейАдреса.Представление;
	Иначе
		Результат.Value = НаборПолейАдреса.МуниципальноеПредставление;
	КонецЕсли;
	
	Результат.Comment     = НаборПолейАдреса.Комментарий;
	
	Результат.Country     = НаборПолейАдреса.Страна;
	Результат.CountryCode = НаборПолейАдреса.КодСтраны;
	
	Результат.ZIPcode     = НаборПолейАдреса.Индекс;
	
	Результат.areaCode    = НаборПолейАдреса.КодРегиона;
	
	Результат.area     = НаборПолейАдреса.Регион;
	Результат.areaType = НаборПолейАдреса.РегионСокращение;
	
	Результат.City     = НаборПолейАдреса.Город;
	Результат.CityType = НаборПолейАдреса.ГородСокращение;
	
	Результат.Street     = НаборПолейАдреса.Улица;
	Результат.StreetType = НаборПолейАдреса.УлицаСокращение;
	
	Результат.District     = НаборПолейАдреса.Район;
	Результат.DistrictType = НаборПолейАдреса.РайонСокращение;
	
	Результат.MunDistrict     = НаборПолейАдреса.МуниципальныйРайон;
	Результат.MunDistrictType = НаборПолейАдреса.МуниципальныйРайонСокращение;
	
	Результат.Settlement     = НаборПолейАдреса.Поселение;
	Результат.SettlementType = НаборПолейАдреса.ПоселениеСокращение;
	
	Результат.CityDistrict     = НаборПолейАдреса.ВнутригородскойРайон;
	Результат.CityDistrictType = НаборПолейАдреса.ВнутригородскойРайонСокращение;
	
	Результат.Locality     = НаборПолейАдреса.НаселенныйПункт;
	Результат.LocalityType = НаборПолейАдреса.НаселенныйПунктСокращение;
	
	Результат.Territory     = НаборПолейАдреса.Территория;
	Результат.TerritoryType = НаборПолейАдреса.ТерриторияСокращение;
	
	Результат.Stead = НаборПолейАдреса.НомерЗемельногоУчастка;
	
	Результат.HouseType   = НаборПолейАдреса.Здание.ТипЗдания;
	Результат.HouseNumber = НаборПолейАдреса.Здание.Номер;
	
	Если НаборПолейАдреса.Свойство("Идентификаторы") Тогда
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.РегионИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.РегионИдентификатор) Тогда
			Результат.areaID = Формат(НаборПолейАдреса.Идентификаторы.РегионИдентификатор, "ЧРГ=' '; ЧГ=0");
			Результат.ID = Результат.areaID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.РайонИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.РайонИдентификатор) Тогда
			Результат.DistrictID = Строка(НаборПолейАдреса.Идентификаторы.РайонИдентификатор);
			Результат.ID         = Результат.DistrictID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.ГородИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.ГородИдентификатор) Тогда
			Результат.CityID = Строка(НаборПолейАдреса.Идентификаторы.ГородИдентификатор);
			Результат.ID     = Результат.CityID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.МуниципальныйРайонИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.МуниципальныйРайонИдентификатор) Тогда
			Результат.MunDistrictID = Строка(НаборПолейАдреса.Идентификаторы.МуниципальныйРайонИдентификатор);
			Результат.ID            = Результат.MunDistrictID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.ПоселениеИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.ПоселениеИдентификатор) Тогда
			Результат.SettlementID = Строка(НаборПолейАдреса.Идентификаторы.ПоселениеИдентификатор);
			Результат.ID           = Результат.SettlementID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.ВнутригородскойРайонИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.ВнутригородскойРайонИдентификатор) Тогда
			Результат.CityDistrictID = Строка(НаборПолейАдреса.Идентификаторы.ВнутригородскойРайонИдентификатор);
			Результат.ID             = Результат.CityDistrictID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.НаселенныйПунктИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.НаселенныйПунктИдентификатор) Тогда
			Результат.LocalityID = Строка(НаборПолейАдреса.Идентификаторы.НаселенныйПунктИдентификатор);
			Результат.ID         = Результат.LocalityID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.ТерриторияИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.ТерриторияИдентификатор) Тогда
			Результат.TerritoryID = Строка(НаборПолейАдреса.Идентификаторы.ТерриторияИдентификатор);
			Результат.ID          = Результат.TerritoryID;
		КонецЕсли;
		
		Если ТипЗнч(НаборПолейАдреса.Идентификаторы.УлицаИдентификатор) = ТипУникальныйИдентификатор
			И ЗначениеЗаполнено(НаборПолейАдреса.Идентификаторы.УлицаИдентификатор) Тогда
			Результат.StreetID = Строка(НаборПолейАдреса.Идентификаторы.УлицаИдентификатор);
			Результат.ID       = Результат.StreetID;
		КонецЕсли;
	КонецЕсли;
	
	Если НаборПолейАдреса.Свойство("ИдентификаторЗемельногоУчастка")
		И ТипЗнч(НаборПолейАдреса.ИдентификаторЗемельногоУчастка) = ТипУникальныйИдентификатор
		И ЗначениеЗаполнено(НаборПолейАдреса.ИдентификаторЗемельногоУчастка) Тогда
		Результат.steadId = Строка(НаборПолейАдреса.ИдентификаторЗемельногоУчастка);
	КонецЕсли;
	
	Если НаборПолейАдреса.Свойство("ИдентификаторДома")
		И ТипЗнч(НаборПолейАдреса.ИдентификаторДома) = ТипУникальныйИдентификатор
		И ЗначениеЗаполнено(НаборПолейАдреса.ИдентификаторДома) Тогда
		Результат.HouseID = Строка(НаборПолейАдреса.ИдентификаторДома);
	КонецЕсли;
	
	Если НаборПолейАдреса.Свойство("ИдентификаторАдресногоОбъекта")
		И ТипЗнч(НаборПолейАдреса.ИдентификаторАдресногоОбъекта) = ТипУникальныйИдентификатор
		И ЗначениеЗаполнено(НаборПолейАдреса.ИдентификаторАдресногоОбъекта) Тогда
		Результат.ID = Строка(НаборПолейАдреса.ИдентификаторАдресногоОбъекта);
	КонецЕсли;
	
	Для Каждого ТекущийКорпус Из НаборПолейАдреса.Корпуса Цикл
	...
	Для Каждого ТекущееПомещение Из НаборПолейАдреса.Помещения Цикл
	...
	Если НаборПолейАдреса.Свойство("КодыКЛАДР") Тогда
		Если Не ПустаяСтрока(НаборПолейАдреса.КодыКЛАДР.Улица) Тогда
			Результат.CodeKLADR = НаборПолейАдреса.КодыКЛАДР.Улица;
		ИначеЕсли Не ПустаяСтрока(НаборПолейАдреса.КодыКЛАДР.НаселенныйПункт) Тогда
			Результат.CodeKLADR = НаборПолейАдреса.КодыКЛАДР.НаселенныйПункт;
		ИначеЕсли Не ПустаяСтрока(НаборПолейАдреса.КодыКЛАДР.Город) Тогда
			Результат.CodeKLADR = НаборПолейАдреса.КодыКЛАДР.Город;
		ИначеЕсли Не ПустаяСтрока(НаборПолейАдреса.КодыКЛАДР.Район) Тогда
			Результат.CodeKLADR = НаборПолейАдреса.КодыКЛАДР.Район;
		ИначеЕсли Не ПустаяСтрока(НаборПолейАдреса.КодыКЛАДР.Регион) Тогда
			Результат.CodeKLADR = НаборПолейАдреса.КодыКЛАДР.Регион;
		КонецЕсли;
	КонецЕсли;
	
	Если НаборПолейАдреса.Свойство("ДополнительныеКоды") Тогда
		Результат.OKTMO          = НаборПолейАдреса.ДополнительныеКоды.ОКТМО;
		Результат.OKATO          = НаборПолейАдреса.ДополнительныеКоды.ОКАТО;
		Результат.IFNSFLCode     = НаборПолейАдреса.ДополнительныеКоды.КодИФНСФЛ;
		Результат.IFNSULCode     = НаборПолейАдреса.ДополнительныеКоды.КодИФНСЮЛ;
		Результат.IFNSFLAreaCode = НаборПолейАдреса.ДополнительныеКоды.КодУчасткаИФНСФЛ;
		Результат.IFNSULAreaCode = НаборПолейАдреса.ДополнительныеКоды.КодУчасткаИФНСЮЛ;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Результат.Value) Тогда
		ОбновитьПредставлениеАдреса(Результат, Ложь);
	КонецЕсли;
...
Функция АдресПоИдентификатору(ИдентификаторАдреса, ДополнительнаяИнформацияАдреса = Неопределено) Экспорт
	...
	РасчетноеПредставление = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(Адрес, Ложь);
	...
	УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнуюИнформациюИзJSONВXML(Адрес, Результат, Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Рекомендация                          = НСтр("ru = 'В зависимости от ситуации следует выбрать один из вариантов:
		...
		|
		|Если адресный классификатор загружен в приложение, то: 
...
Функция ДополнительныеПравилаПреобразования() Экспорт
	...
	Если ВсеРегионы <> Неопределено Тогда
		Для Каждого Регион Из ВсеРегионы Цикл
			КодыРегионов.ДобавитьСтроку("<data:item data:code=""" + Формат(Регион.КодСубъектаРФ, "ЧН=; ЧГ=") + """>" 
				+ Регион.Представление + "</data:item>");
		КонецЦикла;
	КонецЕсли;
...
// Возвращает форматированное представление адреса.
// Вызовы функции необходимо выполнять из общего модуля УправлениеКонтактнойИнформацией
//
// Функция формирует строковое представление адреса в соответствии 
// с указанным порядком полей. Поддерживает различные форматы вывода,
// включая обратный порядок (от номера дома к региону) и сокращенные варианты.
//
// Параметры:
//   Адрес - Строка - адрес в формате JSON.
//         - Структура - перечень полей адреса, где ключ соответствует 
//             именам полей из функции СведенияОбАдресе. Например: Город.
//   ТипАдреса - Строка - тип адреса
//   ПараметрыПредставления - см. НовыеПараметрыПредставленияАдреса
//                          - Неопределено - список параметров по умолчанию
// Возвращаемое значение:
//   Строка - сформированное представление адреса. Если переданный адрес некорректен, будет возвращена пустая строка.
//
Функция ПредставлениеАдреса(АдресПоПолям, ТипАдреса, ПараметрыПредставления = Неопределено) Экспорт
	
	ПараметрыФормированияАдреса = УправлениеКонтактнойИнформацией.НовыеПараметрыПредставленияАдреса();
	Если ПараметрыПредставления <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыФормированияАдреса, ПараметрыПредставления);
	КонецЕсли;

	ПостфиксТипаАдреса = ПараметрыПредставления.ПостфиксТипаАдреса;
	Если ПустаяСтрока(ПостфиксТипаАдреса) Тогда
		ПостфиксТипаАдреса = "ТипКраткий";
	КонецЕсли;
	
	КраткоеНаименованиеТипов = ?(ПостфиксТипаАдреса = "ТипКраткий", Истина, Ложь);
	
	ПорядокПолейИзАдреса = Новый Массив();
	Если РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса) И АдресПоПолям.Свойство("УровниМуниципальногоАдреса") Тогда
		ПорядокПолейИзАдреса = АдресПоПолям.УровниМуниципальногоАдреса;
	ИначеЕсли  АдресПоПолям.Свойство("УровниАдминистративноТерриториальногоАдреса") Тогда
		ПорядокПолейИзАдреса = АдресПоПолям.УровниАдминистративноТерриториальногоАдреса;
	КонецЕсли;
	
	ПорядокПолей = ОпределитьПорядокУровнейДляПредставления(ПараметрыПредставления, ТипАдреса);
	
	ПоляПредставления = Новый Массив;
	
	Для Каждого ИмяПоля Из ПорядокПолей Цикл
		
		Если СтрСравнить(ИмяПоля, "Дом") = 0 Тогда
			
			Если Не ЗначениеЗаполнено(АдресПоПолям.НомерЗемельногоУчастка) Тогда
					
				ПоляПредставления.Добавить(РаботаСАдресамиКлиентСервер.СоединитьНомерИТипЗдания(
					АдресПоПолям.Здание.Номер, АдресПоПолям.Здание.ТипЗдания, КраткоеНаименованиеТипов));
			
				Для Каждого Корпус Из АдресПоПолям.Корпуса Цикл
					ПоляПредставления.Добавить(РаботаСАдресамиКлиентСервер.СоединитьНомерИТипЗдания(Корпус.Номер, Корпус.ТипКорпуса, КраткоеНаименованиеТипов)); 
				КонецЦикла;
			Иначе
				ПоляПредставления.Добавить(РаботаСАдресамиКлиентСервер.СоединитьНомерИТипЗдания(
					АдресПоПолям.НомерЗемельногоУчастка, РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка(), КраткоеНаименованиеТипов));
			КонецЕсли;
			
		ИначеЕсли СтрСравнить(ИмяПоля, "Помещения") = 0 Тогда
			
			Для Каждого Помещение Из АдресПоПолям.Помещения Цикл
				ПоляПредставления.Добавить(РаботаСАдресамиКлиентСервер.СоединитьНомерИТипЗдания(
					Помещение.Номер, Помещение.ТипПомещения, КраткоеНаименованиеТипов));
			КонецЦикла;
				
		ИначеЕсли СтрСравнить(ИмяПоля, "Индекс") = 0 Или СтрСравнить(ИмяПоля, "Страна") = 0 Тогда
			ПоляПредставления.Добавить(АдресПоПолям[ИмяПоля]);
		ИначеЕсли АдресПоПолям.Свойство(ИмяПоля) И ЗначениеЗаполнено(АдресПоПолям[ИмяПоля]) Тогда
			
			Если СтрСравнить(ИмяПоля, "Регион") <> 0 Тогда
				Если ПорядокПолейИзАдреса.Количество() > 0 И ПорядокПолейИзАдреса.Найти(ИмяПоля) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ЗначениеОбъекта = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(АдресПоПолям, ИмяПоля, ПостфиксТипаАдреса, Истина);
			Иначе
			
				ЗначениеОбъекта = "";
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
					СведенияОРегионе = МодульАдресныйКлассификаторСлужебный.СведенияОРегионе(АдресПоПолям[ИмяПоля], АдресПоПолям[ИмяПоля + ПостфиксТипаАдреса]);
					ЗначениеОбъекта  = СведенияОРегионе.ПолноеНаименование;
				КонецЕсли;
				
				Если ПустаяСтрока(ЗначениеОбъекта) Тогда
					ЗначениеОбъекта = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(АдресПоПолям, ИмяПоля, ПостфиксТипаАдреса);
				КонецЕсли;
			КонецЕсли;
			ПоляПредставления.Добавить(ЗначениеОбъекта);
			
		КонецЕсли;
	КонецЦикла;
	
	Возврат СтрСоединить(ПоляПредставления, ", ");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИсправитьУстаревшиеАдресаВФоне",       Истина);
	Методы.Вставить("ИсправитьНекорректныеАдресаВФоне",     Истина);
	Методы.Вставить("ИсправитьТипЗданияЛитерВАдресахВФоне", Истина);
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтрольВеденияУчета") Тогда
		Методы.Вставить("ПроверитьАдресаНаТипЗданияЛитер");
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
			Методы.Вставить("ВыявитьУстаревшиеАдреса");
			Методы.Вставить("ПроверитьАдресаНаСоответствиеАдресномуКлассификатору");
		КонецЕсли;
	КонецЕсли;
...
// Вспомогательные функции сведений об адресе

Функция ЧастьПредставленияЗданияИПомещения(Номер, Тип, ВариантНаписанияТипа, ИмяУровня)
	
	ДанныеОТипе = ТипСогласноВариантуНаписания(
		Тип, ИмяУровня, ВариантНаписанияТипа);
	НаименованиеТипа = НРег(ДанныеОТипе.НаименованиеТипа);
	Часть = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
		Номер, НаименованиеТипа, ДанныеОТипе.ТипВКонец);
	Возврат Часть;
	
КонецФункции

Функция ТипСогласноВариантуНаписания(ТипАдресногоЭлемента, ИмяУровня, ВариантНаписанияТипа) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		Результат = МодульАдресныйКлассификаторСлужебный.ТипСогласноВариантуНаписания(ТипАдресногоЭлемента, ИмяУровня, ВариантНаписанияТипа);
	Иначе
		Результат = ТипАдресногоЭлемента;
	КонецЕсли;
...
Функция СведенияОАдресахОбщая(Адреса, ДополнительныеПараметры, ТипАдреса)
	
	ПараметрыАдреса = НовыйДополнительныеПараметрыДеленийАдресов();
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыАдреса, ДополнительныеПараметры);
	КонецЕсли;
	
	Результаты              = Новый Соответствие();
	СоответствиеУровней     = СоответствиеНаименованиеУровнейПолямJSON();
	УровниАдресныхЭлементов = ПорядокУровнейАдресныхЭлементовПоУмолчанию();
	ВариантНаписанияТипа    = ПараметрыАдреса.ВариантНаписанияТипа;
	
	Для Каждого Адрес Из Адреса Цикл
		АдресJSON = Адрес.Значение;
		
		АдресИзJSON = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(АдресJSON, Перечисления.ТипыКонтактнойИнформации.Адрес);
		ИмяУровней  = ПорядокУровнейАдреса(АдресИзJSON, ТипАдреса);
		
		Результат = ?(ТипАдреса = РаботаСАдресамиКлиентСервер.МуниципальныйАдрес(), 
		              НовыйМуниципальныйАдрес(), 
		              НовыйАдминистративноТерриториальныйАдрес());
		Результат.Комментарий = АдресИзJSON.comment;
		Результат.Индекс      = АдресИзJSON.ZIPcode;
		
		Если ЗначениеЗаполнено(АдресИзJSON.areaValue) Тогда
			НаименованиеРегиона = АдресИзJSON.areaValue;
		Иначе
			НаименованиеРегиона = ОфициальноеНаименованиеРегиона(АдресИзJSON.area);
		КонецЕсли;
		
		НаборПолейДляПредставление     = Новый Массив();
		ИдентификаторАдресногоЭлемента = "";
		
		Если ПараметрыАдреса.ВключатьСтрануВПредставление Тогда
			НаборПолейДляПредставление.Добавить(СокрЛП(АдресИзJSON.country));
		КонецЕсли;
		
		Если ПараметрыАдреса.ВключатьИндексПредставление Тогда
			НаборПолейДляПредставление.Добавить(Результат.Индекс);
		КонецЕсли;
		
		Для Каждого ИмяУровня Из ИмяУровней Цикл
			
			Если ИмяУровня = "Здание" Тогда
				ОбработатьЗданияАдреса(АдресИзJSON, Результат, НаборПолейДляПредставление, ВариантНаписанияТипа);
				Продолжить;
			ИначеЕсли ИмяУровня = "Помещение" Тогда
				ОбработатьПомещенияАдреса(АдресИзJSON, Результат, НаборПолейДляПредставление, ВариантНаписанияТипа);
				Продолжить;
			КонецЕсли;
			
			УровеньАдреса = Результат[ИмяУровня];
			
			Если Не ЗначениеЗаполнено(СоответствиеУровней[ИмяУровня]) Тогда
				Продолжить;
			КонецЕсли;
			
			ИмяУровняВJSON = СоответствиеУровней[ИмяУровня];
			Если Не ЗначениеЗаполнено(АдресИзJSON[ИмяУровняВJSON]) Тогда
				Продолжить;
			КонецЕсли;
			
			ЗначениеТипа = ?(СтрСравнить(ИмяУровняВJSON, "stead") <> 0, АдресИзJSON[ИмяУровняВJSON + "Type"], 
				РаботаСАдресамиКлиентСервер.ТипЗемельногоУчастка());
			ДанныеОТипе = ТипСогласноВариантуНаписания(ЗначениеТипа, ИмяУровня, ВариантНаписанияТипа);
			
			УровеньАдреса.Наименование  = АдресИзJSON[ИмяУровняВJSON];
			УровеньАдреса.Тип           = НРег(ДанныеОТипе.НаименованиеТипа);
			УровеньАдреса.Идентификатор = Строка(АдресИзJSON[ИмяУровняВJSON + "Id"]);
			
			Если ЗначениеЗаполнено(УровеньАдреса.Идентификатор) И УровниАдресныхЭлементов.Найти(ИмяУровня) <> Неопределено Тогда
				ИдентификаторАдресногоЭлемента = УровеньАдреса.Идентификатор;
			КонецЕсли;
			
			Если ИмяУровня = "Регион" Тогда
				Представление = НаименованиеРегиона;
			Иначе
				Представление = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
					УровеньАдреса.Наименование, УровеньАдреса.Тип, ДанныеОТипе.ТипВКонец);
			КонецЕсли;
			
			УстановитьКодУровня(УровеньАдреса, ИмяУровня, АдресИзJSON, НаименованиеРегиона);
			
			УровеньАдреса.Представление = Представление;
			
			Если ЗначениеЗаполнено(Представление) Тогда
				НаборПолейДляПредставление.Добавить(Представление);
			КонецЕсли;
		КонецЦикла;
		
		Результат.ИдентификаторАдресногоЭлемента = ИдентификаторАдресногоЭлемента;
		Результат.Представление = СтрСоединить(НаборПолейДляПредставление, ", ");
		Результаты[Адрес.Ключ]  = Результат;
	КонецЦикла;
...
// Установка кода для уровня адреса
Процедура УстановитьКодУровня(УровеньАдреса, ИмяУровня, АдресИзJSON, НаименованиеРегиона)
    Код = Неопределено;

    Если ИмяУровня = "Регион" Тогда
        Код = АдресИзJSON.areaCode;
        Если ПустаяСтрока(Код) Тогда
            Код = КодРегиона(НаименованиеРегиона);
        КонецЕсли;
    ИначеЕсли ИмяУровня = "МуниципальныйРайон" Тогда
        Код = КодМуниципальногоРайона(АдресИзJSON.munDistrictType, АдресИзJSON.oktmo,
            ЭтоГородФедеральногоЗначения(НаименованиеРегиона));
    ИначеЕсли ИмяУровня = "Поселение" Тогда
        Код = КодПоселения(АдресИзJSON.settlementType);
    КонецЕсли;

    Если Код <> Неопределено Тогда
        УровеньАдреса.Код = Код;
    КонецЕсли;
КонецПроцедуры

Процедура ОбработатьЗданияАдреса(АдресИзJSON, Результат, НаборПолейДляПредставление, ВариантНаписанияТипа)
	
	Если Не ЗначениеЗаполнено(АдресИзJSON.houseNumber) И АдресИзJSON.buildings.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	УровеньАдреса = Результат.Здание;
	ЧастиПредставления = Новый Массив();
	
	Если ЗначениеЗаполнено(АдресИзJSON.houseNumber) Тогда
		ДанныеОТипе = ТипСогласноВариантуНаписания(
			АдресИзJSON.houseType, "Здание", ВариантНаписанияТипа);
		
		ОписаниеЗдания = НовыйНаименованиеАдресногоОбъектаИТипа();
		ОписаниеЗдания.Наименование = АдресИзJSON.houseNumber;
		ОписаниеЗдания.Тип          = НРег(ДанныеОТипе.НаименованиеТипа);
		УровеньАдреса.ЭлементыЗдания.Добавить(ОписаниеЗдания);
		
		Часть = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
			ОписаниеЗдания.Наименование, ОписаниеЗдания.Тип, ДанныеОТипе.ТипВКонец);
		ЧастиПредставления.Добавить(Часть);
	КонецЕсли;
	
	Для Каждого Строение Из АдресИзJSON.buildings Цикл
		ДанныеОТипе = ТипСогласноВариантуНаписания(
			Строение.type, "Здание", ВариантНаписанияТипа);
		
		ОписаниеЗдания = НовыйНаименованиеАдресногоОбъектаИТипа();
		ОписаниеЗдания.Наименование = Строение.number;
		ОписаниеЗдания.Тип          = НРег(ДанныеОТипе.НаименованиеТипа);
		УровеньАдреса.ЭлементыЗдания.Добавить(ОписаниеЗдания);
		
		Часть = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(ОписаниеЗдания.Наименование, 
		                                  ОписаниеЗдания.Тип, 
		                                  ДанныеОТипе.ТипВКонец);
		ЧастиПредставления.Добавить(Часть);
	КонецЦикла;
	
	УровеньАдреса.Представление = СтрСоединить(ЧастиПредставления, ", ");
	УровеньАдреса.Идентификатор = Строка(АдресИзJSON.houseId);
	
	Если ЗначениеЗаполнено(УровеньАдреса.Представление) Тогда
		НаборПолейДляПредставление.Добавить(УровеньАдреса.Представление);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработатьПомещенияАдреса(АдресИзJSON, Результат, НаборПолейДляПредставление, ВариантНаписанияТипа)
	
	Если АдресИзJSON.apartments.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	УровеньАдреса = Результат.Помещения;
	ЧастиПредставления = Новый Массив();
	
	Для Каждого Помещение Из АдресИзJSON.apartments Цикл
		ДанныеОТипе = ТипСогласноВариантуНаписания(
			Помещение.type, "Помещения", ВариантНаписанияТипа);
		
		ОписаниеПомещения = НовыйНаименованиеАдресногоОбъектаИТипа();
		ОписаниеПомещения.Наименование = Помещение.number;
		ОписаниеПомещения.Тип          = НРег(ДанныеОТипе.НаименованиеТипа);
		УровеньАдреса.ЭлементыПомещения.Добавить(ОписаниеПомещения);
		
		Часть = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
			ОписаниеПомещения.Наименование, ОписаниеПомещения.Тип, ДанныеОТипе.ТипВКонец);
		ЧастиПредставления.Добавить(Часть);
	КонецЦикла;
	
	УровеньАдреса.Идентификатор = Строка(АдресИзJSON.apartmentId);
	УровеньАдреса.Представление = СтрСоединить(ЧастиПредставления, ", ");
...
// Порядок уровней по умолчанию.
// 
// Параметры:
//  ПараметрыПредставленияАдреса - см. НовыеПараметрыПредставленияАдреса
// 
// Возвращаемое значение:
//  Массив - порядок уровней по умолчанию
//
Функция ОпределитьПорядокУровнейДляПредставления(ПараметрыПредставленияАдреса, ТипАдреса)
	
	УровниАдреса = Новый Массив;
		
	Если ПараметрыПредставленияАдреса.СписокПолей.Количество() > 0 Тогда
		УровниАдреса = Новый Массив(Новый ФиксированныйМассив(ПараметрыПредставленияАдреса.СписокПолей));
	ИначеЕсли ПараметрыПредставленияАдреса.ПочтовыйАдрес Тогда
		УровниАдреса.Добавить("Улица");
		УровниАдреса.Добавить("Дом");
		УровниАдреса.Добавить("Помещения");
		...
		УровниАдреса.Добавить("Страна");
		УровниАдреса.Добавить("Индекс");
	Иначе
		УровниАдреса.Добавить("Страна");
		УровниАдреса.Добавить("Индекс");
		УровниАдреса.Добавить("Регион");
	...
	Если Не ПараметрыПредставленияАдреса.ВключатьСтрану Тогда
		
		Позиция = УровниАдреса.Найти("Страна");
		Если Позиция <> Неопределено Тогда 
			УровниАдреса.Удалить(УровниАдреса.Найти("Страна"));
		КонецЕсли;
		
	КонецЕсли;
	
	Если Не ПараметрыПредставленияАдреса.ВключатьПочтовыйИндекс Тогда
		
		Позиция = УровниАдреса.Найти("Индекс");
		Если Позиция <> Неопределено Тогда 
			УровниАдреса.Удалить(УровниАдреса.Найти("Индекс"));
		КонецЕсли;
		
	КонецЕсли;
	
	Если ПараметрыПредставленияАдреса.ТолькоНаселенныйПункт Тогда
		ВключатьУлицу            = Ложь;
		ВключатьЗданияИПомещения = Ложь;
	Иначе
		ВключатьУлицу            = ПараметрыПредставленияАдреса.ВключатьУлицу;
		ВключатьЗданияИПомещения = ПараметрыПредставленияАдреса.ВключатьЗданияИПомещения;
	КонецЕсли;
	
	Если Не ВключатьУлицу Тогда
		Позиция = УровниАдреса.Найти("Улица");
		Если Позиция <> Неопределено Тогда 
			УровниАдреса.Удалить(УровниАдреса.Найти("Улица"));
		КонецЕсли;
	КонецЕсли;
	
	Если Не ВключатьЗданияИПомещения Тогда
		Позиция = УровниАдреса.Найти("Дом");
		Если Позиция <> Неопределено Тогда 
			УровниАдреса.Удалить(УровниАдреса.Найти("Дом"));
		КонецЕсли;
		
		Позиция = УровниАдреса.Найти("Помещения");
		Если Позиция <> Неопределено Тогда 
			УровниАдреса.Удалить(УровниАдреса.Найти("Помещения"));
		КонецЕсли;
	КонецЕсли;
...
Функция ПорядокУровнейАдресныхЭлементовПоУмолчанию()
	...
	Для Каждого ЭлементСписка Из ЗначенияПолей Цикл
		...
		ИначеЕсли ИмяПоля = "AREACODE" Тогда
			Если ПустаяСтрока(НациональныйАдрес.СубъектРФ) Тогда
				НациональныйАдрес.СубъектРФ = РегионПоКода(ЭлементСписка.Значение);
			КонецЕсли;
...
Функция ПреобразоватьКлючЗначениеВСтруктуру(Знач ЗначенияПолей, Знач Представление)
	...
	НазваниеОсновнойСтраны  = ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РаботаСАдресамиКлиентСервер.ОсновнаяСтрана(), 
		"Наименование"));
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		ИначеЕсли ИмяПоля = "КОДРЕГИОНА" Тогда
			...
			Если ПустаяСтрока(Результат.area) Тогда
				НаименованиеИТип = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(
					РегионПоКода(ЭлементСписка.Значение));
...
Функция ПодготовитьАдресДляВвода(Данные) Экспорт
	...
	Если Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоИностранныйАдресИлиЕАЭС(Данные.addressType)
	   И Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(Данные.addressType) Тогда
...
//   Неопределено - если подсистема адресного классификатора отсутствует.
// 
Функция ВсеРегионы()
...
//      Неопределено - если нет подсистемы Адресный классификатор.
// 
...
Функция ОфициальноеНаименованиеРегиона(Знач Наименование)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		ДанныеОРегионе = МодульАдресныйКлассификаторПовтИсп.ДанныеОРегионе(Наименование);
...
Функция АдресСодержитУлицы(ИдентификаторНаселенногоПункта, ТипАдреса) Экспорт
	
	СодержитУлицы = Ложь;
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		СодержитУлицы = МодульАдресныйКлассификаторСлужебный.АдресныйОбъектСодержитУлицы(ИдентификаторНаселенногоПункта,
			РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса));
	КонецЕсли;
	
	Возврат СодержитУлицы;
...
Функция СведенияОбАдресахВВидеСтруктуры(Адреса, ДополнительныеПараметры)
	...
	Для каждого СтрокаАдреса Из ТаблицаАдресов Цикл
		...
		АдресИзJSON = СтрокаАдреса.АдресJSON; // см. РаботаСАдресамиКлиентСервер.ОписаниеНовойКонтактнойИнформации
		
		АдресПоПолям = ОбщегоНазначения.СкопироватьРекурсивно(ПодготовленныеПоляАдреса); // см. РаботаСАдресамиКлиентСервер.ПоляАдреса
		...
		АдресПоПолям.КодРегиона = ?(ЭтоНациональныйАдрес, КодРегиона(СокрЛП(АдресИзJSON.area)), "");
		...
		Для каждого ЧастьАдреса Из СоответствиеУровней Цикл
			...
			Если Параметры.КодыАдреса Или Параметры.КодыКЛАДР Тогда
				АдресПоПолям.Идентификаторы.Вставить(ЧастьАдреса.Значение.Имя + "Идентификатор", ИдентификаторТекущегоУровня);
				АдресПоПолям.Идентификаторы.Вставить(ЧастьАдреса.Значение.Имя, ИдентификаторТекущегоУровня);
			КонецЕсли;
		...
		// Коды мун. уровней
		ИмяРегиона = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
			АдресИзJSON.area, АдресИзJSON.areaType, Истина);
		АдресПоПолям.КодМуниципальногоРайона = КодМуниципальногоРайона(АдресИзJSON.munDistrictType, АдресИзJSON.oktmo,
			ЭтоГородФедеральногоЗначения(ИмяРегиона, АдресИзJSON.areaValue));
		...
		Если Параметры.КодыАдреса Тогда
			...
			АдресПоПолям.ИдентификаторПомещения = ?(ЗначениеЗаполнено(АдресИзJSON.apartmentId), АдресИзJSON.apartmentId, "");
		КонецЕсли;
		...
		Для каждого Помещение Из АдресИзJSON.Apartments Цикл
			ПоляПомещения = РаботаСАдресамиКлиентСервер.ПоляПомещения();
			ПоляПомещения.ТипПомещения = Помещение.Type;
			ПоляПомещения.Номер = Помещение.Number;
			АдресПоПолям.Помещения.Добавить(ПоляПомещения);
		КонецЦикла;
		...
		Если ЭтоНациональныйАдрес Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
				...
				Если Параметры.ВозвращатьКоды Тогда
					...
					КонецЕсли;
					Если Параметры.КодыАдреса И ДополнительныеКоды.КодыАдреса.Свойство("ИдентификаторПомещения") Тогда
						АдресПоПолям.ИдентификаторПомещения = ДополнительныеКоды.КодыАдреса.ИдентификаторПомещения;
					КонецЕсли;
				КонецЕсли;
			Иначе
		...
		АдресПоПолям.УровниАдминистративноТерриториальногоАдреса = ПорядокУровнейАдреса(АдресИзJSON, РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес());
		АдресПоПолям.УровниМуниципальногоАдреса =  ПорядокУровнейАдреса(АдресИзJSON, РаботаСАдресамиКлиентСервер.МуниципальныйАдрес());
...
Функция ПорядокУровнейАдреса(АдресИзJSON, ТипАдреса)
	
	Результат = Новый Массив();
	
	ИмяУровней = ИменаУровнейАдресаJSON(АдресИзJSON, ТипАдреса);
	СоответствиеУровней = СоответствиеИменУровней();
	
	Для каждого ИмяУровня Из ИмяУровней Цикл
		Если ЗначениеЗаполнено(АдресИзJSON[ИмяУровня]) Тогда
			ИмяУровняПредставление = СоответствиеУровней[ИмяУровня];
			Если ЗначениеЗаполнено(ИмяУровняПредставление) Тогда
				Результат.Добавить(ИмяУровняПредставление);
			КонецЕсли;
			
		КонецЕсли;
	КонецЦикла;
	
	Если (ЗначениеЗаполнено(АдресИзJSON.houseNumber) 
		 Или АдресИзJSON.buildings.Количество() > 0)
		   И Результат.Найти("Здание") = Неопределено Тогда
				Результат.Добавить("Здание");
		КонецЕсли;
		
	Если АдресИзJSON.apartments.Количество() > 0 И Результат.Найти("Помещение") = Неопределено Тогда
		Результат.Добавить("Помещение");
	КонецЕсли;
			
	Если ЗначениеЗаполнено(АдресИзJSON.stead) И Результат.Найти("ЗемельныйУчасток") = Неопределено Тогда
		Результат.Добавить("ЗемельныйУчасток");
	КонецЕсли;
...
Функция ИменаУровнейАдресаJSON(АдресИзJSON, ТипАдреса)

	ИмяПоляУровней = ?(РаботаСАдресамиКлиентСервер.ЭтоАдминистративноТерриториальныйАдрес(ТипАдреса), "admLevels", "munLevels");
	
	ИмяУровней = Новый Массив();
	
	Если АдресИзJSON[ИмяПоляУровней].Количество() > 0 Тогда
		
		Для Каждого ИмяУровня Из АдресИзJSON[ИмяПоляУровней] Цикл
			Если АдресИзJSON.Свойство(ИмяУровня) И ЗначениеЗаполнено(АдресИзJSON[ИмяУровня]) Тогда
				ИмяУровней.Добавить(ИмяУровня);
			КонецЕсли;
		КонецЦикла;
		
	Иначе
			ИмяУровней.Добавить("area");
			
			Если РаботаСАдресамиКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса) Тогда
				ИмяУровней.Добавить("munDistrict");
				Если ЗначениеЗаполнено(АдресИзJSON.city) И ПустаяСтрока(АдресИзJSON.settlement) Тогда
					ИмяУровней.Добавить("city");
				КонецЕсли;
				ИмяУровней.Добавить("settlement");
			Иначе
				ИмяУровней.Добавить("district");
				ИмяУровней.Добавить("city");
			КонецЕсли;
			
			ИмяУровней.Добавить("locality");
			ИмяУровней.Добавить("territory");
			ИмяУровней.Добавить("street");
	КонецЕсли;
...
Функция СоответствиеУровней()
	...
	СоответствиеУровней.Вставить("district",     Новый Структура("Имя, Уровень", "Район", 2));
	СоответствиеУровней.Вставить("munDistrict",  Новый Структура("Имя, Уровень", "МуниципальныйРайон", 3));
	СоответствиеУровней.Вставить("city",         Новый Структура("Имя, Уровень", "Город", 5));
	СоответствиеУровней.Вставить("settlement",   Новый Структура("Имя, Уровень", "Поселение", 4));
	СоответствиеУровней.Вставить("cityDistrict", Новый Структура("Имя, Уровень", "ВнутригородскойРайон", 0));
	СоответствиеУровней.Вставить("locality",     Новый Структура("Имя, Уровень", "НаселенныйПункт", 6));
	СоответствиеУровней.Вставить("territory",    Новый Структура("Имя, Уровень", "Территория", 7));
	СоответствиеУровней.Вставить("street",       Новый Структура("Имя, Уровень", "Улица", 8));
...
// Возвращаемое значение:
// Соответствие из Структура:
// * Ключ - Строка - представление уровня в JSON
// * Значение - Строка - пользовательское представление уровня
//
Функция СоответствиеИменУровней()
	
	СоответствиеУровней = Новый Соответствие;
	СоответствиеУровней.Вставить("area",         "Регион");
	СоответствиеУровней.Вставить("district",     "Район");
	СоответствиеУровней.Вставить("munDistrict",  "МуниципальныйРайон");
	СоответствиеУровней.Вставить("city",         "Город");
	СоответствиеУровней.Вставить("settlement",   "Поселение");
	СоответствиеУровней.Вставить("cityDistrict", "ВнутригородскойРайон"); // устаревшие свойство
	СоответствиеУровней.Вставить("locality",     "НаселенныйПункт");
	СоответствиеУровней.Вставить("territory",    "Территория");
	СоответствиеУровней.Вставить("street",       "Улица");
	СоответствиеУровней.Вставить("stead",        "ЗемельныйУчасток");
	СоответствиеУровней.Вставить("buildings",    "Здание");
	СоответствиеУровней.Вставить("apartments",   "Помещение");
...
// * Значение - Строка - пользовательское представление уровня
//
Функция СоответствиеНаименованиеУровнейПолямJSON()
	
	СоответствиеУровней = Новый Соответствие;
	СоответствиеУровней.Вставить("Регион",               "area");
	СоответствиеУровней.Вставить("Район",                "district");
	СоответствиеУровней.Вставить("МуниципальныйРайон",   "munDistrict");
	СоответствиеУровней.Вставить("Город",                "city");
	СоответствиеУровней.Вставить("Поселение",            "settlement");
	СоответствиеУровней.Вставить("ВнутригородскойРайон", "cityDistrict"); // устаревшие свойство
	СоответствиеУровней.Вставить("НаселенныйПункт",      "locality");
	СоответствиеУровней.Вставить("Территория",           "territory");
	СоответствиеУровней.Вставить("Улица",                "street");
	СоответствиеУровней.Вставить("ЗемельныйУчасток",     "stead");
	СоответствиеУровней.Вставить("Здание",               "buildings");
	СоответствиеУровней.Вставить("Помещение",            "apartments");
...
Функция КодМуниципальногоРайона(Знач ТипМуниципальногоРайона, ОКТМО, ЭтоГородФедеральногоЗначения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		КодМуниципальногоРайона = МодульАдресныйКлассификаторСлужебный.КодМуниципальногоРайона(ТипМуниципальногоРайона, ОКТМО, ЭтоГородФедеральногоЗначения);
	КонецЕсли;
	...
	Если СтрДлина(ОКТМОСтрокой) > 3 Тогда
		...
		Если СтрНачинаетсяС(ОКТМОСтрокой, "03731000") Тогда
...
Функция КодПоселения(Знач ТипПоселения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		Возврат МодульАдресныйКлассификаторСлужебный.КодПоселения(ТипПоселения);
	КонецЕсли;
...
Функция СтруктураАдресаВСтруктуруJSON(Знач КонтактнаяИнформация)
	...
	СоответствиеПолей.Вставить("ИдентификаторПомещения",         "ApartmentId");	
	СоответствиеПолей.Вставить("Дом",                            "houseNumber");
	СоответствиеПолей.Вставить("ТипДома",                        "houseType");
	СоответствиеПолей.Вставить("НомерЗемельногоУчастка",         "stead");
...
Функция ЭтоГородФедеральногоЗначения(Город, ОфициальноеНаименование = "") Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		Возврат МодульАдресныйКлассификаторСлужебный.ЭтоГородФедеральногоЗначения(Город, ОфициальноеНаименование);
...
Функция ТаблицаКлассификатора() Экспорт
	...
	Результат = РаботаСАдресамиПовтИсп.СтраныМира();
	СтраныМира = Результат.СтраныМира.Скопировать();
	Возврат СтраныМира;
...
Функция РазложитьПредставлениеАдресаПоПолям(Представление, ТекущийАдрес = Неопределено) Экспорт
	...
	Адрес = РаботаСАдресамиКлиентСервер.ОписаниеНовойКонтактнойИнформации(Перечисления.ТипыКонтактнойИнформации.Адрес);
		
	ДанныеАнализа = УправлениеКонтактнойИнформациейСлужебный.ЧастиАдресаТаблицей(Представление);
	
	РазборНаОсновеТекущегоАдреса = Ложь;
	Если ТекущийАдрес <> Неопределено И ПустаяСтрока(ТекущийАдрес.city) И ПустаяСтрока(ТекущийАдрес.area) Тогда
		РазборНаОсновеТекущегоАдреса = Истина
	КонецЕсли;
		
	УправлениеКонтактнойИнформациейСлужебный.ОпределитьСтрануИИндекс(ДанныеАнализа);
	...
	Если СтрокаСтраны <> Неопределено Тогда
		...
		ДанныеАнализа.Удалить(СтрокаСтраны);
	КонецЕсли;
	
	СтрокаИндекс = ДанныеАнализа.Найти(-1, "Уровень");
	Если СтрокаИндекс <> Неопределено Тогда
		Адрес.ZipCode = СокрЛП(СтрокаИндекс.Значение);
		ДанныеАнализа.Удалить(СтрокаИндекс);
	КонецЕсли;	
		
	Если РазборНаОсновеТекущегоАдреса Тогда
		НераспределенныеДанные = Новый Массив;
		РазложитьЧастиАдресаПоПолямНаОсновеТекущегоАдреса(ДанныеАнализа, Адрес, ТекущийАдрес, НераспределенныеДанные)
	Иначе
		НераспределенныеДанные = ДанныеАнализа;
	КонецЕсли;
	
	АдресныеЕдиницы = СписокАдресныхЕдиницССокращениями();
	
	ИменаУровнейАдресаПоКоду = ИменаУровнейАдресаПоКоду();
	ВариантыДанныхДом = РаботаСАдресамиПовтИсп.ВариантыДанныхДом().ВариантыТипа;
	ВариантыДанныхСтроение = РаботаСАдресамиПовтИсп.ВариантыДанныхСтроение().ВариантыТипа;
	СокращенияНаименованийДомов = СокращенияНаименованийДомов();
	
	РазборНаОсновеТекущегоАдреса = НераспределенныеДанные.Количество() <> ДанныеАнализа.Количество();
	
	Для Каждого ЧастьАдрес Из НераспределенныеДанные Цикл
		
		Если Не ЗначениеЗаполнено(Адрес.ZipCode) И СтрДлина(ЧастьАдрес.Значение) = 6
			И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЧастьАдрес.Значение) Тогда
			Адрес.ZipCode = ЧастьАдрес.Значение;
			Продолжить;
		КонецЕсли;
		
		ЗаполненУровеньУлицы = ЗначениеЗаполнено(Адрес.street);
		ПоискПоАдреснымЕдиницам = Не ЗаполненУровеньУлицы Или РазборНаОсновеТекущегоАдреса;
		
		Если ЗначениеЗаполнено(ЧастьАдрес.ТипОбъекта) Тогда
			
			ЗначениеУстановлено = Ложь;
			Если ПоискПоАдреснымЕдиницам И Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЧастьАдрес.Наименование) Тогда
				Для Каждого АдреснаяЕдиница Из АдресныеЕдиницы Цикл
					Если СтрСравнить(ЧастьАдрес.ТипОбъекта, АдреснаяЕдиница.Сокращение) = 0 
						Или СтрСравнить(ЧастьАдрес.ТипОбъекта, АдреснаяЕдиница.Значение) = 0 Тогда
						
						НомерУровня = АдреснаяЕдиница.НомерУровня;
						Если АдреснаяЕдиница.Значение = "Улица" Тогда
							НомерУровня = 8;
						КонецЕсли;
						
						ИмяУровня = ИменаУровнейАдресаПоКоду.Получить(НомерУровня);
						Если ИмяУровня <> Неопределено Тогда
							Если Адрес.Свойство(ИмяУровня) И ПустаяСтрока(Адрес[ИмяУровня])Тогда
								Адрес[ИмяУровня] = ЧастьАдрес.Наименование;
								Если НомерУровня = 8 Тогда
									ЗаполненУровеньУлицы = Истина;
								КонецЕсли;
								Если Адрес.Свойство(ИмяУровня + "Type") Тогда
									Адрес[ИмяУровня + "Type"] = ЧастьАдрес.ТипОбъекта;
								КонецЕсли;
				...
				КонецЦикла;
			КонецЕсли;
			Если ЗначениеУстановлено Тогда
				Продолжить;
			КонецЕсли;
			
			ТипОбъектаДляПоискаПоСокращениям = ?(Прав(ЧастьАдрес.ТипОбъекта, 1) = ".", ВРег(ЧастьАдрес.ТипОбъекта), ВРег(ЧастьАдрес.ТипОбъекта) + ".");
			Если ВариантыДанныхДом.Найти(ТРег(ЧастьАдрес.ТипОбъекта)) <> Неопределено
			   Или СокращенияНаименованийДомов.Получить(ТипОбъектаДляПоискаПоСокращениям) <> Неопределено Тогда
				Если ПустаяСтрока(Адрес.HouseNumber) И ПустаяСтрока(Адрес.stead) Тогда
					Адрес.HouseType = ЧастьАдрес.ТипОбъекта;
					Адрес.HouseNumber = ЧастьАдрес.Наименование;
				ИначеЕсли ВариантыДанныхСтроение.Найти(ТРег(ЧастьАдрес.ТипОбъекта)) <> Неопределено
				   Или СокращенияНаименованийДомов.Получить(ТипОбъектаДляПоискаПоСокращениям) <> Неопределено Тогда
					Адрес.buildings.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
						ЧастьАдрес.ТипОбъекта, ЧастьАдрес.Наименование));
				Иначе
					Помещение = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
						ЧастьАдрес.ТипОбъекта, ЧастьАдрес.Наименование);
					Адрес.Apartments.Добавить(Помещение);
				КонецЕсли;
				Продолжить;
			ИначеЕсли СтрНайти(ЧастьАдрес.Значение, РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка()) <> 0 Тогда
				ЗемельныйУчасток = СокрЛП(СтрЗаменить(ЧастьАдрес.Значение, РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка(), ""));
				Если ПустаяСтрока(Адрес.stead) И ПустаяСтрока(Адрес.HouseNumber) Тогда 
					Адрес.stead = ЗемельныйУчасток;
				Иначе
					Помещение = УправлениеКонтактнойИнформациейКлиентСервер.ЗначениеСтроенияИлиПомещения(
						РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка(), ЗемельныйУчасток);
					Адрес.Apartments.Добавить(Помещение);
				КонецЕсли;
				Продолжить;
			ИначеЕсли ВариантыДанныхСтроение.Найти(ТРег(ЧастьАдрес.ТипОбъекта)) <> Неопределено
		...
		Если ПоискПоАдреснымЕдиницам И (ПустаяСтрока(ЧастьАдрес.Наименование) 
		   Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЧастьАдрес.Наименование)) Тогда
			Если ЗаполнитьПоляГорода(Адрес, ЧастьАдрес, ЗаполненУровеньУлицы) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Адрес.street = ?(ЗначениеЗаполнено(Адрес.street), Адрес.street + " " + ЧастьАдрес.Значение, ЧастьАдрес.Значение);
...
Процедура РазложитьЧастиАдресаПоПолямНаОсновеТекущегоАдреса(ДанныеАнализа, Адрес, ТекущийАдрес, НераспределенныеДанные)
	
	Для Каждого ЧастьАдрес Из ДанныеАнализа Цикл
		ЗначениеУстановлено = Ложь;

		Если ЗначениеЗаполнено(ЧастьАдрес.ТипОбъекта) Тогда
			Если СтрНайти(ЧастьАдрес.Значение, РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка()) <> 0 Тогда
				Адрес.stead = СокрЛП(СтрЗаменить(ЧастьАдрес.Значение,
					РаботаСАдресамиКлиентСервер.НаименованиеЗемельногоУчастка(), ""));
				Продолжить;
			КонецЕсли;
			Если СтрСравнить(ТекущийАдрес.HouseType, ЧастьАдрес.ТипОбъекта) = 0 Тогда
				Адрес.HouseType = ЧастьАдрес.ТипОбъекта;
				Адрес.HouseNumber = ЧастьАдрес.Наименование;
				Продолжить;
			КонецЕсли;

			Для Каждого ЭлементАдреса Из ТекущийАдрес Цикл
				Если СтрСравнить(ЭлементАдреса.Значение, ЧастьАдрес.ТипОбъекта) = 0 И СтрНайти(ЭлементАдреса.Ключ,
					"Type") <> 0 Тогда
					Адрес[ЭлементАдреса.Ключ] = ЧастьАдрес.ТипОбъекта;
					ИмяПоля = СтрЗаменить(ЭлементАдреса.Ключ, "Type", "");
					Если Адрес.Свойство(ИмяПоля) Тогда
						Адрес[ИмяПоля] = ЧастьАдрес.Наименование;
						ЗначениеУстановлено = Истина;
						Прервать;
					КонецЕсли;
				КонецЕсли;
			...
			Если ТипЗнч(ТекущийАдрес.buildings) = Тип("Массив") Тогда
				Для Каждого Строение Из ТекущийАдрес.buildings Цикл
					Если СтрСравнить(Строение.Type, ЧастьАдрес.ТипОбъекта) = 0 Тогда
						Строение.number = ЧастьАдрес.Наименование;
						ЗначениеУстановлено = Истина;
			...
			Если ТипЗнч(ТекущийАдрес.Apartments) = Тип("Массив") Тогда
				Для Каждого Помещение Из ТекущийАдрес.Apartments Цикл
					Если СтрСравнить(Помещение.Type, ЧастьАдрес.ТипОбъекта) = 0 Тогда
						Помещение.number = ЧастьАдрес.Наименование;
						ЗначениеУстановлено = Истина;
						Адрес.Apartments.Добавить(Помещение);
						Прервать;
					КонецЕсли;
		...
		Иначе
			Для Каждого ЭлементАдреса Из ТекущийАдрес Цикл
				Если ЗначениеЗаполнено(ЧастьАдрес.Наименование) Тогда
					Если СтрСравнить(ЭлементАдреса.Значение, ЧастьАдрес.Наименование) = 0 Тогда
						Адрес[ЭлементАдреса.Ключ] = ЧастьАдрес.Наименование;
						ЗначениеУстановлено = Истина;
						Прервать;
					КонецЕсли;
				Иначе
					Если СтрСравнить(ЭлементАдреса.Значение, ЧастьАдрес.Значение) = 0 Тогда
						Адрес[ЭлементАдреса.Ключ] = ЧастьАдрес.Значение;
						ЗначениеУстановлено = Истина;
						Прервать;
					КонецЕсли;
				КонецЕсли;
			...
			КонецЕсли;
		КонецЕсли;
		НераспределенныеДанные.Добавить(ЧастьАдрес);
	КонецЦикла;
	
КонецПроцедуры

Функция ЗаполнитьПоляГорода(Адрес, ЧастьАдрес, ЗаполненУровеньУлицы)
	
	Если ПустаяСтрока(Адрес.area) Тогда
		Адрес.area = ЧастьАдрес.Значение;
		Возврат Истина;
	ИначеЕсли ПустаяСтрока(Адрес.district) Тогда
		Адрес.district = ЧастьАдрес.Значение;
		Возврат Истина;
	ИначеЕсли ПустаяСтрока(Адрес.city) Тогда
		Адрес.city = ЧастьАдрес.Значение;
		Возврат Истина;
	ИначеЕсли ПустаяСтрока(Адрес.locality) Тогда
		Адрес.locality = ЧастьАдрес.Значение;
		Возврат Истина;
	ИначеЕсли ПустаяСтрока(Адрес.street) Тогда
		Адрес.street = ЧастьАдрес.Значение;
		ЗаполненУровеньУлицы = Истина;
		Возврат Истина;
	КонецЕсли;
...
Процедура ОбновитьПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление) Экспорт
	...
	Представление = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление);
	Адрес.Value = Представление;
	
КонецПроцедуры

Процедура ДобавитьУровеньУлицыДляАдреса(Адрес) Экспорт
	
	Если ЗначениеЗаполнено(Адрес.street) Тогда
		Если ТипЗнч(Адрес.munLevels) = Тип("Массив") 
		   И Адрес.munLevels.Найти("street") = Неопределено Тогда
			Адрес.munLevels.Добавить("street");
		КонецЕсли;
		
		Если ТипЗнч(Адрес.admLevels) = Тип("Массив") 
		   И Адрес.admLevels.Найти("street") = Неопределено Тогда
...
Функция СписокАдресныхЕдиницССокращениями()
	
	СписокАдресныхЕдиниц = Новый ТаблицаЗначений;
	СписокАдресныхЕдиниц.Колонки.Добавить("Сокращение", ОбщегоНазначения.ОписаниеТипаСтрока(20));
	СписокАдресныхЕдиниц.Колонки.Добавить("НомерУровня", ОбщегоНазначения.ОписаниеТипаЧисло(1));
	СписокАдресныхЕдиниц.Колонки.Добавить("Значение", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторПовтИсп = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторПовтИсп");
		СписокСокращений = МодульАдресныйКлассификаторПовтИсп.СписокСокращений();
	Иначе
		Возврат СписокАдресныхЕдиниц;
	КонецЕсли;
	
	Для Каждого АдреснаяЕдиница Из СписокСокращений Цикл
		НомерУровня = Лев(АдреснаяЕдиница.Ключ, 1);
		НомерУровня = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(НомерУровня);
		Если НомерУровня = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Сокращение = СтрЗаменить(АдреснаяЕдиница.Ключ, НомерУровня, "");
		
		ЕдиницаАдреса = СписокАдресныхЕдиниц.Добавить();
		ЕдиницаАдреса.Значение = АдреснаяЕдиница.Значение;
		ЕдиницаАдреса.Сокращение = Сокращение;
		ЕдиницаАдреса.НомерУровня = НомерУровня;
	КонецЦикла;
	
	СписокАдресныхЕдиниц.Сортировать("НомерУровня");
	
	Возврат СписокАдресныхЕдиниц;
	
КонецФункции

Функция СокращенияНаименованийДомов()
	
	Результат = Новый Соответствие;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторПовтИсп = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторПовтИсп");
		Результат = МодульАдресныйКлассификаторПовтИсп.СокращенияНаименованийДомов();
	КонецЕсли;
...
Функция НовыйМуниципальныйАдрес()
	
	Результат = НовыйАдресОбщаяЧасть();
	
	Результат.Вставить("МуниципальныйРайон", НовыйАдресныйЭлементСПолемКод());
	Результат.Вставить("Поселение", НовыйАдресныйЭлементСПолемКод());
	
	Возврат Результат;
	
КонецФункции

Функция НовыйАдминистративноТерриториальныйАдрес()
	
	Результат = НовыйАдресОбщаяЧасть();
	
	Результат.Вставить("Район", НовыйАдресныйЭлемент());
	Результат.Вставить("Город", НовыйАдресныйЭлемент());
	
	Возврат Результат;
	
КонецФункции

Функция НовыйАдресОбщаяЧасть()
	
	Результат = Новый Структура;
	Результат.Вставить("Представление", "");
	Результат.Вставить("Индекс",        "");
	Результат.Вставить("Комментарий",  "");
	Результат.Вставить("ИдентификаторАдресногоЭлемента", "");
	
	Результат.Вставить("Регион",           НовыйАдресныйЭлементСПолемКод());
	
	Результат.Вставить("НаселенныйПункт",  НовыйАдресныйЭлемент());
	Результат.Вставить("Территория",       НовыйАдресныйЭлемент());
	Результат.Вставить("Улица",            НовыйАдресныйЭлемент());
	Результат.Вставить("Здание",           НовыйЗдания());
	Результат.Вставить("Помещения",        НовыйПомещения());
	Результат.Вставить("ЗемельныйУчасток", НовыйЗемельныйУчасток());
	
	Возврат Результат;
	
КонецФункции

Функция НовыйАдресныйЭлемент()
...
Функция НовыйАдресныйЭлементСПолемКод()
	
	Результат = НовыйАдресныйЭлемент();
...
// Возвращаемое значение:
//  Структура:
// * Представление - Строка - полное наименование, которое включают все помещения
// * Идентификатор - Строка - уникальный идентификатор
// * ЭлементыЗдания - Массив из см. НовыйНаименованиеИТип
//
Функция НовыйЗдания()
	...
	Результат.Вставить("Представление", "");
	Результат.Вставить("Идентификатор", "");
	Результат.Вставить("ЭлементыЗдания", Новый Массив);
...
// * Тип - Строка
//
Функция НовыйНаименованиеАдресногоОбъектаИТипа()
...
// * Представление - Строка - полное наименование, которое включают все помещения
// * Идентификатор - Строка - уникальный идентификатор
// * ЭлементыПомещения - Массив из см. НовыйНаименованиеИТип
//
Функция НовыйПомещения()
	
	Помещения = Новый Структура;
	Помещения.Вставить("Представление", "");
	Помещения.Вставить("Идентификатор", "");
	Помещения.Вставить("ЭлементыПомещения", Новый Массив);
