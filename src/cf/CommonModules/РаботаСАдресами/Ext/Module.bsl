...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Причины                      = НСтр("ru = 'Переподчинение адресных объектов (улиц, населенных пунктов), их переименование или упразднение влечет за собой внесение изменений в 
		|Федеральную информационную адресную систему (ФИАС). После этого ранее введенные адреса перестают проходить проверку на соответствие ФИАС.'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы          = "КонтактнаяИнформация";
	Проверка.Наименование                 = НСтр("ru = 'Проверка адресов на соответствие ФИАС.'");
	...
	Проверка.Рекомендация                 = НСтр("ru = 'В зависимости от ситуации следует выбрать один из вариантов:
		|•	Убедиться, что адрес введен корректно, проверив  адрес на сайте ФИАС: https://fias.nalog.ru/ExtendedSearch
		|О некорректных или неполных адресных сведениях на сайте ФИАС рекомендуется сообщать в органы местного самоуправления или в службу технической  поддержки online-сервисов ФНС России.
		...
		|Ссылка для быстрого перехода e1cib/command/РегистрСведений.АдресныеОбъекты.Команда.ЗагрузитьАдресныйКлассификатор
		|
		|Если работа ведется в распределенной информационной базе (РИБ), то исправления следует вносить только в главном узле.
		|Затем выполнить синхронизацию с подчиненными узлами.'");
	
	Проверка.Идентификатор                = "РаботаСАдресами.ПроверитьАдресаНаСоответствиеФИАС";
	Проверка.ОбработчикПроверки           = "РаботаСАдресами.ПроверитьАдресаНаСоответствиеФИАС";
	Проверка.КонтекстПроверокВеденияУчета = "_КонтактнаяИнформация";
	Проверка.Отключена                    = Истина;
...
// 
Процедура ПриДобавленииКлассификаторов(Классификаторы, ОписаниеКлассификатора) Экспорт
	
	ОписаниеКлассификатора.Идентификатор              = ИдентификаторКлассификатора();
	ОписаниеКлассификатора.Наименование               = НСтр("ru = 'Общероссийский классификатор стран мира'");
	ОписаниеКлассификатора.ОбновлятьАвтоматически     = Истина;
	ОписаниеКлассификатора.ОбщиеДанные                = Истина;
	ОписаниеКлассификатора.СохранятьФайлВКэш          = Истина;
	ОписаниеКлассификатора.ОбработкаРазделенныхДанных = Ложь;
	
	Классификаторы.Добавить(ОписаниеКлассификатора);
			...
			Для каждого СтрокаКонтактнойИнформации Из НайденныеСтроки Цикл
				АдресБылИзменен = Ложь;
					...
					Если АдресБылИзменен Тогда
						АдресВJSON = УправлениеКонтактнойИнформациейСлужебный.СтруктураВСтрокуJSON(АдресПоПолям);
						
						НовоеПредставление = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(АдресПоПолям, ВключатьСтрануВПредставление);
						ТекстКомментария = НСтр("ru='Тип здания ""Литер"" устарел и был заменен на ""Литера"".'");
						Если ЗначениеЗаполнено(АдресПоПолям.comment) Тогда
							ТекстКомментария = АдресПоПолям.comment + Символы.ПС + ТекстКомментария;
						КонецЕсли;
						УправлениеКонтактнойИнформацией.УстановитьКомментарийКонтактнойИнформации(АдресВJSON, ТекстКомментария); 
						
						СтрокаКонтактнойИнформации.Значение      = АдресВJSON;
						СтрокаКонтактнойИнформации.Представление = НовоеПредставление;
						СтрокаКонтактнойИнформации.ЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(АдресВJSON);
...
Процедура ВыявитьУстаревшиеАдреса(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока СтрокаСАдресом.Следующий() Цикл
		...
		Если Счетчик = 100 Тогда
			Результат = МодульАдресныйКлассификаторСлужебный.АнализАдресовПоКлассификатору(ПорцияАдресов);
...
Функция ИсправитьПорциюУстаревшихАдресовКонтактнойИнформации(Знач ПроблемныеОбъекты, Проверка, ВидПроверки)
	...
	РезультатАнализа = МодульАдресныйКлассификаторСлужебный.АнализАдресовПоКлассификатору(АдресаДляПроверки);
			...
			Для каждого СтрокаВидаКИ Из СтрокиВидаКИ Цикл
				...
				Для каждого АдресОбъекта Из АдресаОбъекта Цикл
					Если СтрСравнить(АдресОбъекта.Представление, СтрокаВидаКИ.Представление) = 0 Тогда
						СведенияОбАдресе = РезультатАнализа[АдресОбъекта.Счетчик];
						
						Если СведенияОбАдресе.Варианты.Количество() > 0 Тогда
							АдресПоПолям = СведенияОбАдресе.Варианты[0];
...
Процедура ОбработкаУстаревшегоАдресаПослеПроверки(Адреса, СоответствиеАдресаИСсылки, ПараметрыПроверки, Проверки)
	...
	Для каждого КлючЗначение Из Адреса Цикл
		...
		Если ПроверенныйАдрес.АдресКорректный ИЛИ КлючЗначение.Значение.Варианты.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
...
// Проверка адресов на соответствие ФИАС.
//
// Параметры:
//   Проверка            - СправочникСсылка.ПравилаПроверкиУчета - исполняемая проверка;
//   ПараметрыПроверки   - см. КонтрольВеденияУчета.ОписаниеПроблемы.ПараметрыПроверки
//
Процедура ПроверитьАдресаНаСоответствиеФИАС(Проверка, ПараметрыПроверки) Экспорт
...
Процедура ОбработкаАдресаПослеПроверки(ПроверенныеАдреса, СоответствиеАдресаИСсылки, Проверка, ПараметрыПроверки, Проверки)
	...
	Для каждого КлючЗначение  Из ПроверенныеАдреса Цикл
		...
		Если ПроверенныйАдрес.Ошибки.Количество() > 0 Тогда
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 ""%2"" не соответствует ФИАС, по причине: %3'"), Строка(ОписаниеАдреса.Вид), ОписаниеАдреса.Представление, ПроверенныйАдрес.Ошибки[0].Текст);
		Иначе
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 ""%2"" не соответствует ФИАС'"), Строка(ОписаниеАдреса.Вид), ОписаниеАдреса.Представление);
		КонецЕсли;
