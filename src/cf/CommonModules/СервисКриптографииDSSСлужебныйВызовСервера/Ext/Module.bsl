...
Процедура НачальныеНастройкиФормы(ИмяФормы) Экспорт
	...
	Если СтрНайти(ИмяФормы, ".") = 0 Тогда
		...
		ПутьОкна = "Обработка.УправлениеПодключениемDSS.Форма." + ИмяФормы + "/НастройкиОкна";
	Иначе
		ПутьФормы = ИмяФормы + "/НастройкиФормы";
		ПутьОкна = ИмяФормы + "/НастройкиОкна";
	КонецЕсли;
...
// В модуле СервисКриптографииDSSСлужебный.ПодготовитьДанныеДокумента() в таком случае они снова помещаются во временное 
// хранилище.
//
...
// -см. СервисКриптографииDSS.ОбновитьДанныеСертификатов()
//
// Возвращаемое значение:
//   см. ДлительныеОперации.ВыполнитьВФоне
//
...
Функция СменитьПинКодСертификата(НастройкиПользователя, ИдентификаторСертификата, СтарыйПинКод, НовыйПинКод, ПараметрыОперации = Неопределено) Экспорт
	...
	ПараметрыВызова.Вставить("ИдентификаторСертификата", ИдентификаторСертификата);
	ПараметрыВызова.Вставить("СтарыйПинКод", СтарыйПинКод);
	ПараметрыВызова.Вставить("НовыйПинКод", НовыйПинКод);
	ПараметрыВызова.Вставить("ПараметрыОперации", ПараметрыОперации);
	ПараметрыВызова.Вставить("НастройкиПользователя", ПодготовитьНастройкиПользователя(НастройкиПользователя, ПараметрыОперации));
	ПараметрыВызова.Вставить("АдресРезультата", АдресРезультата);
...
// см. СервисКриптографииDSS.ПодписатьАсинхронно()
//
// Возвращаемое значение:
//   см. ДлительныеОперации.ВыполнитьВФоне
//
Функция ПодписатьАсинхронно(НастройкиПользователя, СвойствоПодписи, ИдентификаторСертификата, ПинКод, АсинхронныйРежим, ОбратныйВызов, ПараметрыОперации = Неопределено) Экспорт
	...
	ПараметрыВызова.Вставить("ИдентификаторСертификата", ИдентификаторСертификата);
	ПараметрыВызова.Вставить("ПинКод", ПинКод);
	ПараметрыВызова.Вставить("АсинхронныйРежим", АсинхронныйРежим);
	ПараметрыВызова.Вставить("ОбратныйВызов", ОбратныйВызов);
	ПараметрыВызова.Вставить("НастройкиПользователя", ПодготовитьНастройкиПользователя(НастройкиПользователя, ПараметрыОперации));
	ПараметрыВызова.Вставить("ПараметрыОперации", ПараметрыОперации);
	ПараметрыВызова.Вставить("АдресРезультата", АдресРезультата);
...
// см. СервисКриптографииDSS.ВыполнитьОперацию()
//
// Возвращаемое значение:
//   см. ДлительныеОперации.ВыполнитьВФоне
//
Функция ВыполнитьОперацию(НастройкиПользователя, ИдентификаторОперации, ПараметрыОперации = Неопределено) Экспорт
	...
	Возврат СервисКриптографииDSSСлужебный.ВыполнитьВФоне("СервисКриптографииDSSСлужебный.ВыполнитьОперацию", ПараметрыВызова);
...
Процедура ОбновитьНастройкиПользователя(НастройкиПользователя, НовыеНастройки, ПараметрыОперации = Неопределено) Экспорт
	...
	Если НастройкиПользователя.Ссылка = НовыеНастройки.Ссылка Тогда
		...
		ИсточникНастроек = СервисКриптографииDSSСлужебный.ПолучитьИсточникНастроек(ПараметрыОперации);
		СервисКриптографииDSSСлужебный.УстановитьНастройкиПользователя(НовыеНастройки.Ссылка, ПодготовленныеНастройки, ИсточникНастроек);
...
// см. СервисКриптографииDSSСлужебный.ПолучитьСертификатПоСерийномуНомеру().
//
// Возвращаемое значение:
...
// см. СервисКриптографииDSSСлужебный.ПолучитьИнформациюУдостоверяющихЦентров()
//
// Возвращаемое значение:
//  Структура - см. подробнее СервисКриптографииDSSСлужебный.ПолучитьИнформациюУдостоверяющихЦентров.
//
...
// см. СервисКриптографииDSSСлужебный.СохранитьДанныеБезопасногоХранилища()
//
...
// см. СервисКриптографииDSSСлужебный.ПроверитьМаркерДоступа()
//
// Возвращаемое значение:
...
Функция ПроверитьМаркерДоступа(НастройкиПользователя, РезультатЗапроса, ПараметрыОперации = Неопределено) Экспорт
	...
	ИсточникНастроек = СервисКриптографииDSSСлужебный.ПолучитьИсточникНастроек(ПараметрыОперации);
	СервисКриптографииDSSСлужебный.УстановитьНастройкиПользователя(ПодготовленныеНастройки.Ссылка, ПодготовленныеНастройки, ИсточникНастроек);
...
//
Функция ДанныеВторичнойАвторизацииПоУмолчанию() Экспорт
	
	Возврат СервисКриптографииDSSПодтверждениеСервер.ДанныеВторичнойАвторизацииПоУмолчанию();
