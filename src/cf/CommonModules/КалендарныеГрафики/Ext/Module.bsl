...
Функция ПараметрыПолученияБлижайшихРабочихДат(ПроизводственныйКалендарь = Неопределено) Экспорт
	...
	Параметры.Вставить("ПолучатьПредшествующие", Ложь);
	Параметры.Вставить("УчитыватьНерабочиеПериоды", Истина);
	Параметры.Вставить("НерабочиеПериоды", Неопределено);
	Параметры.Вставить("ВызыватьИсключение", Истина);
	Параметры.Вставить("ПолучатьДатыЕслиКалендарьНеЗаполнен", Ложь);
...
Функция БлижайшиеРабочиеДаты(ПроизводственныйКалендарь, НачальныеДаты, ПараметрыПолучения = Неопределено) Экспорт
	...
	Выборка = ВыборкаБлижайшихРабочихДат(ПроизводственныйКалендарь, НачальныеДаты, ПараметрыПолучения);
	Если Не ЗначениеЗаполнено(Выборка) Тогда
		Возврат ДатыРабочихДней;
	КонецЕсли;
	
	ДанныеКалендаряПоУмолчанию = Справочники.ПроизводственныеКалендари.НовыеДанныеПроизводственныхКалендарей();
	
	Пока Выборка.Следующий() Цикл
		
		Если ЗначениеЗаполнено(Выборка.БлижайшаяДата) Тогда
			БлижайшаяДата = Выборка.БлижайшаяДата;
		ИначеЕсли ПараметрыПолучения.ПолучатьДатыЕслиКалендарьНеЗаполнен Тогда
			БлижайшаяДата = БлижайшаяРабочаяДатаИзДанныхКалендаряПоУмолчанию(Выборка.Дата, ДанныеКалендаряПоУмолчанию,
				ПараметрыПолучения.ПолучатьПредшествующие);
		Иначе
			БлижайшаяДата = Неопределено;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(БлижайшаяДата) Тогда
			ДатыРабочихДней.Вставить(Выборка.Дата, БлижайшаяДата);
		ИначеЕсли ПараметрыПолучения.ВызыватьИсключение Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ДанныеКлассификатора() Экспорт
	...
	ПроизводственныеКалендари = Новый Структура;
	ПроизводственныеКалендари.Вставить("ИмяТаблицы", "");
	ПроизводственныеКалендари.Вставить("Данные",     Новый ТаблицаЗначений());
	
	ДанныеПроизводственныхКалендарей = Новый Структура(Новый ФиксированнаяСтруктура(ПроизводственныеКалендари));
	ПериодыНерабочихДней             = Новый Структура(Новый ФиксированнаяСтруктура(ПроизводственныеКалендари));

	ДанныеКлассификатора.Вставить("ДанныеПроизводственныхКалендарей", ДанныеПроизводственныхКалендарей);	
	ДанныеКлассификатора.Вставить("ПроизводственныеКалендари",        ПроизводственныеКалендари);
	ДанныеКлассификатора.Вставить("ПериодыНерабочихДней",             ПериодыНерабочихДней);
...
#Область ОбработчикиОбновленияИБИспользуемыеВДругихПодсистемах
...
//   * БлижайшаяДата - Дата
//
Функция ВыборкаБлижайшихРабочихДат(ПроизводственныйКалендарь, НачальныеДаты, ПараметрыПолучения)
	
	ТекстЗапроса = ТекстЗапросаНачальныхДат(НачальныеДаты);
	...
	Если ПараметрыПолучения.ПолучатьПредшествующие Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДатыКалендаря.Дата >= НачальныеДаты.Дата",
			"ДатыКалендаря.Дата <= НачальныеДаты.Дата");
	КонецЕсли;
...
//  Строка
//
Функция ТекстЗапросаНачальныхДат(НачальныеДаты)
...
// Определяет для переданной даты ближайший к ней рабочий день по данным основного производственного календаря РФ,
// заполненного по умолчанию. Если НачальнаяДата является рабочей датой, то она же и возвращается.
// Используется в случаях, когда производственный календарь еще не заполнен.
// 
// Параметры:
//  НачальнаяДата - Дата
//  ДанныеКалендаряПоУмолчанию - см. Справочники.ПроизводственныеКалендари.НовыеДанныеПроизводственныхКалендарей
//  ПолучатьПредшествующую - Булево - способ получения ближайшей даты:
//	 если Истина - определяется рабочая дата, предшествующая НачальнаяДата, 
//	 если Ложь - получается дата не ранее НачальнаяДата.
// 
// Возвращаемое значение:
//  Дата, Неопределено
//
Функция БлижайшаяРабочаяДатаИзДанныхКалендаряПоУмолчанию(НачальнаяДата, ДанныеКалендаряПоУмолчанию = Неопределено,
	ПолучатьПредшествующую = Ложь)
	
	Если ДанныеКалендаряПоУмолчанию = Неопределено Тогда
		ДанныеКалендаряПоУмолчанию = Справочники.ПроизводственныеКалендари.НовыеДанныеПроизводственныхКалендарей();
	КонецЕсли;
	
	Год = Год(НачальнаяДата);
	Если ДанныеКалендаряПоУмолчанию.Найти(Год, "Год") = Неопределено Тогда
		ДополнитьДанныеКалендаряПоУмолчанию(ДанныеКалендаряПоУмолчанию, Год, ПолучатьПредшествующую);
	КонецЕсли;
	
	СтрокаТаблицы = ДанныеКалендаряПоУмолчанию.Найти(НачальнаяДата, "Дата");
	Если СтрокаТаблицы = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если СтрокаТаблицы.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Рабочий
		Или СтрокаТаблицы.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Предпраздничный Тогда
		Возврат СтрокаТаблицы.Дата;
	КонецЕсли;
	
	Индекс = ДанныеКалендаряПоУмолчанию.Индекс(СтрокаТаблицы);
	ДобавляемоеЗначение = ?(ПолучатьПредшествующую, -1, 1);
	Пока Истина Цикл
		Индекс = Индекс + 1;
		Если Индекс >= ДанныеКалендаряПоУмолчанию.Количество() Тогда
			Если Индекс > 10000 Тогда
				Прервать;
			КонецЕсли;
			Год = Год + ДобавляемоеЗначение;
			ДополнитьДанныеКалендаряПоУмолчанию(ДанныеКалендаряПоУмолчанию, Год, ПолучатьПредшествующую);
		КонецЕсли;
		СтрокаТаблицы = ДанныеКалендаряПоУмолчанию.Получить(Индекс);
		Если СтрокаТаблицы.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Рабочий
			Или СтрокаТаблицы.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Предпраздничный Тогда
			Возврат СтрокаТаблицы.Дата;
		КонецЕсли;
...
//
Процедура ДополнитьДанныеКалендаряПоУмолчанию(ДанныеКалендаряПоУмолчанию, Год, УпорядочитьПоУбыванию)

	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(
		Справочники.ПроизводственныеКалендари.РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию("РФ", Год),
		ДанныеКалендаряПоУмолчанию);
		
	Если УпорядочитьПоУбыванию Тогда
		ДанныеКалендаряПоУмолчанию.Сортировать("Дата Убыв");
	Иначе
		ДанныеКалендаряПоУмолчанию.Сортировать("Дата Возр");
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьИспользованиеНесколькихПроизводственныхКалендарей");
	Методы.Вставить("ОбновитьДанныеПроизводственныхКалендарей");
	Методы.Вставить("ЗаполнитьДанныеОбновленияДанныхЗависимыхОтПроизводственныхКалендарей");
	Методы.Вставить("ОбновитьДанныеЗависимыеОтПроизводственныхКалендарей");
	Методы.Вставить("СброситьВерсиюКлассификатора");
	Методы.Вставить("ОбновитьПроизводственныеКалендари");
	Методы.Вставить("ИсправитьДанныеЗависимыхКалендарей");
	Методы.Вставить("ОбновитьДанныеЗависимыхПроизводственныхКалендарей");
