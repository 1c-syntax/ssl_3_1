	...
	Если ЗначениеЗаполнено(ЭтоКоличествоКоллизий) Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
				...
				СтрокаОтбора = "Выразить(ВерсииОбъектов.Комментарий КАК Строка(1000)) ПОДОБНО &Комментарий 
								|ИЛИ Выразить(ВерсииОбъектов.ПредупреждениеСинхронизации КАК Строка(1000)) ПОДОБНО &Комментарий";
				Запрос.УстановитьПараметр("Комментарий", "%" + СокрЛП(Лев(СтрокаПоиска, 998)) + "%");
...
Функция АктуализироватьИнформациюОПроблемахВерсийСинхронизацииДанных(УзлыСинхронизации) Экспорт
	...
	|	Количество(1) КАК КоличествоПредупрежденийВерсий
	|ИЗ
	|	РегистрСведений.ВерсииОбъектов КАК ВерсииОбъектов
	|ГДЕ
...
Процедура ТекстЗапросаСпискаПредупрежденияВерсий(СписокТекстЗапроса) Экспорт
...
//
// Параметры:
//  Ссылка      - ЛюбаяСсылка - ссылка версионируемый объект;
...
Процедура ВывестиТабличныеЧастиПоРазобранномуОбъекту(ТЧОтчета, ВерсияОбъекта, НомерСтрокиВывода, СсылкаНаОбъект)
	...
	Для Каждого СтрокаТабличнаяЧасть Из ВерсияОбъекта.ТабличныеЧасти Цикл
		...
		Секция = ТЧОтчета.ПолучитьОбласть("R" + XMLСтрока(НомерСтрокиВывода));
