...
//  Отправитель - ПланОбменаОбъект, ПланОбменаСсылка - план обмена БСП.
//
Процедура СоздатьВерсиюОбъектаПоОбменуДанными(Объект, СведенияОВерсииОбъекта, СсылкаСуществует, Отправитель) Экспорт
	...
	Если Не ЗначениеЗаполнено(СведенияОВерсииОбъекта.АвторВерсии) Тогда
		...
		Если Отправитель <> Неопределено
			И ЗначениеЗаполнено(Отправитель.Ссылка) Тогда
			
			СведенияОВерсииОбъекта.АвторВерсии = Отправитель.Ссылка;
	...
	РегистрыСведений.ВерсииОбъектов.УдалитьПредыдущиеПредупрежденияОбменов(СведенияОВерсииОбъекта);
...
Процедура ПриПолученииСпискаШаблонов(ШаблоныЗаданий) Экспорт
	...
	ШаблоныЗаданий.Добавить(Метаданные.РегламентныеЗадания.ОчисткаУстаревшихВерсийОбъектов.Имя);
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		ИмяФункциональнойОпции = "ИспользоватьСинхронизациюДанных";
		
		Настройка = Настройки.Добавить();
		Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОчисткаУстаревшихВерсийОбъектов;
		Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции[ИмяФункциональнойОпции];
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Версия = "3.1.10.55";
		Обработчик.Процедура = "ВерсионированиеОбъектов.УстановитьНастройкиРегламентногоЗаданияОчисткиУстаревшихВерсийОбъектов";
		Обработчик.РежимВыполнения = "Оперативно";
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьНастройкиРегламентногоЗаданияОчисткиУстаревшихВерсийОбъектов() Экспорт
	
	ИмяФункциональнойОпции = "ИспользоватьСинхронизациюДанных";
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") 
		И ПолучитьФункциональнуюОпцию(ИмяФункциональнойОпции) Тогда
		ВключитьАвтоматическуюОчисткуУстаревшихВерсийОбъектов();
	КонецЕсли;
...
Процедура ЗаписатьДанныеТекущейВерсии(Ссылка, ОбменДаннымиЗагрузка = Ложь, РежимЗаписиПроведения = Ложь)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если Не ОбъектВерсионируется(Ссылка, РежимЗаписиПроведения) Тогда
		Возврат;
	КонецЕсли;
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Выбранная версия объекта отсутствует в приложении.'");
	КонецЕсли;
...
Процедура ОчиститьУстаревшиеВерсииОбъектов() Экспорт
	...
	|ИЗ
	|	РегистрСведений.ВерсииОбъектов КАК ВерсииОбъектов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТипыОбъектов КАК ТипыОбъектов
	|		ПО ТИПЗНАЧЕНИЯ(ВерсииОбъектов.Объект) = ТИПЗНАЧЕНИЯ(ТипыОбъектов.Ссылка)
	|ГДЕ
	|	ВерсииОбъектов.ЕстьДанныеВерсии
	|	И ВерсииОбъектов.ДатаВерсии < &ГраницаУдаления";
	
	ОписанияПараметров = ОписанияПараметровЗапросаПоВерсиямОбъектов();
	
	Запрос = Новый Запрос;
	
	ТекстыЗапросовПоТиповОбъектов = Новый Массив;
	ПодготовитьПараметрыЗапроса(Запрос, ШаблонЗапросаТиповОбъектов, ТекстыЗапросовПоТиповОбъектов, ОписанияПараметров);

	ТекстыЗапросовВерсий = Новый Массив;
	ПодготовитьПараметрыЗапроса(Запрос, ШаблонЗапросаВерсий, ТекстыЗапросовВерсий, ОписанияПараметров);
	
	Если Не ЗначениеЗаполнено(ТекстыЗапросовПоТиповОбъектов) Тогда
		Возврат;
	КонецЕсли;
	
	Запрос.Текст = СтрСоединить(ТекстыЗапросовПоТиповОбъектов, Символы.ПС + Символы.ПС) // @Query-part-1
		+ Символы.ПС + Символы.ПС
		+ СтрСоединить(ТекстыЗапросовВерсий, Символы.ПС + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС); // @Query-part-1
...
Функция ГраницыУдаленияОбъектов()
	...
	Результат.Колонки.Добавить("ТипыВерсийОбменДанными", Новый ОписаниеТипов("Булево"));
	...
	Пока ВыборкаСроковХранения.Следующий() Цикл
		...
		Пока ВыборкаОбъектов.Следующий() Цикл
			СписокТипов.Добавить(ТипЗнч(ВыборкаОбъектов.ЗначениеПустойСсылки));
		КонецЦикла;
	...
	ВсеТипы = Метаданные.РегистрыСведений.ВерсииОбъектов.Измерения.Объект.Тип.Типы();
	ВерсионируемыеТипы = Метаданные.ОпределяемыеТипы.ВерсионируемыеДанные.Тип.Типы();
	
	// Другие объекты, по которым отсутствует настройка хранения версий (например, по обмену данными).
	// Срок хранения фиксирован - 1 месяц.
	
	НеверсионируемыеТипы = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
		ВсеТипы, ВерсионируемыеТипы);
	
	СписокТипов = Новый Массив;
	Для Каждого Тип Из НеверсионируемыеТипы Цикл
		СписокТипов.Добавить(Тип);
	КонецЦикла;

	СоответствиеГраницыТипамОбъектов = Результат.Добавить();
	СоответствиеГраницыТипамОбъектов.ГраницаУдаления = ГраницаУдаления(Перечисления.СрокиХраненияВерсий.ЗаПоследнийМесяц);
	СоответствиеГраницыТипамОбъектов.СписокТипов = СписокТипов;
	
	// Версии объектов, не принятых по обмену.
	// Срок хранения фиксирован - 1 месяц.
	
	СписокТипов = Новый Массив;
	Для Каждого Тип Из ВерсионируемыеТипы Цикл
		СписокТипов.Добавить(Тип);
	КонецЦикла;
	...
	СоответствиеГраницыТипамОбъектов.ГраницаУдаления = ГраницаУдаления(Перечисления.СрокиХраненияВерсий.ЗаПоследнийМесяц);
	СоответствиеГраницыТипамОбъектов.СписокТипов = СписокТипов;
	СоответствиеГраницыТипамОбъектов.ТипыВерсийОбменДанными = Истина;
...
Функция ИнформацияОбУстаревшихВерсиях() Экспорт
	...
	|ПОМЕСТИТЬ ИтогиПоПериодам
	|ИЗ
	|	РегистрСведений.ВерсииОбъектов КАК ВерсииОбъектов
	|ГДЕ
	|	ВерсииОбъектов.ЕстьДанныеВерсии
	|	И ВерсииОбъектов.ДатаВерсии < &ГраницаУдаления
	|	И ТИПЗНАЧЕНИЯ(ВерсииОбъектов.Объект) В (&СписокТипов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ИтогиПоПериодам.РазмерДанных) КАК РазмерДанных,
	|	СУММА(ИтогиПоПериодам.КоличествоВерсий) КАК КоличествоВерсий
	|ИЗ
	|	ИтогиПоПериодам";
	...
	ТекстыЗапросов = СтрРазделить(ШаблонЗапроса, ";", Ложь);
	ТекстЗапросаПоПериоду = ТекстыЗапросов[0];
	ТекстИтоговогоЗапроса = ТекстыЗапросов[1];
	ТекстыЗапросов.Очистить();
	
	ПодготовитьПараметрыЗапроса(Запрос, ТекстЗапросаПоПериоду, ТекстыЗапросов);
	
	Результат = Новый Структура;
	Результат.Вставить("КоличествоВерсий", 0);
	Результат.Вставить("РазмерДанных", 0);
	...
	Запрос.Текст = СтрСоединить(ТекстыЗапросов, Символы.ПС + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС) // @Query-part-1
		+ Символы.ПС + ";" + ТекстИтоговогоЗапроса;
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Результат.КоличествоВерсий = Выборка.КоличествоВерсий;
		Результат.РазмерДанных = Выборка.РазмерДанных;
		Результат.Вставить("РазмерДанныхСтрокой", РазмерДанныхСтрокой(Результат.РазмерДанных));
	КонецЕсли;
...
Процедура ПодготовитьПараметрыЗапроса(Запрос, ШаблонЗапроса, ТекстыЗапросов, ОписанияПараметров = Неопределено)
	
	Если ОписанияПараметров = Неопределено Тогда
		ОписанияПараметров = ОписанияПараметровЗапросаПоВерсиямОбъектов();
	КонецЕсли;
	
	Для Каждого ОписаниеПараметров Из ОписанияПараметров Цикл
		ТекстЗапроса = СокрП(ШаблонЗапроса);
		
		Если ТекстыЗапросов.Количество() > 0 Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ИтогиПоПериодам", ""); // @Query-part-1
		КонецЕсли;
		
		Для Каждого ПереименованиеПараметра Из ОписаниеПараметров.ПереименованияПараметров Цикл
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&" + ПереименованиеПараметра.Ключ, "&" + ПереименованиеПараметра.Значение);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ" + " "+ ПереименованиеПараметра.Ключ, "ПОМЕСТИТЬ" + " " + ПереименованиеПараметра.Значение);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВНУТРЕННЕЕ СОЕДИНЕНИЕ" + " "+ ПереименованиеПараметра.Ключ, "ВНУТРЕННЕЕ СОЕДИНЕНИЕ" + " " + ПереименованиеПараметра.Значение);
		КонецЦикла;
		
		Для Каждого Параметр Из ОписаниеПараметров.ЗначенияПараметров Цикл
			Запрос.УстановитьПараметр(Параметр.Ключ, Параметр.Значение);
		КонецЦикла;
		
		СтрокиЗапроса = СтрРазделить(ТекстЗапроса, Символы.ПС);
		Если Не СтрЗаканчиваетсяНа(СтрокиЗапроса[СтрокиЗапроса.ВГраница()], ";") Тогда
			СтрокиЗапроса.Добавить();
			
			Если ОписаниеПараметров.ЗначенияПараметров.Свойство("ТипыВерсий") Тогда
				СтрокиЗапроса[СтрокиЗапроса.ВГраница()] = Символы.Таб + "И ВерсииОбъектов.ТипВерсииОбъекта В(&ТипыВерсий)"; // @Query-part-1
			Иначе
				СтрокиЗапроса[СтрокиЗапроса.ВГраница()] = Символы.Таб + "И ВерсииОбъектов.ТипВерсииОбъекта = Значение(Перечисление.ТипыВерсийОбъекта.ИзмененоПользователем)"; // @Query-part-1
			КонецЕсли;
...
Функция ОписанияПараметровЗапросаПоВерсиямОбъектов()
	...
	Для Индекс = 0 По ГраницыУдаленияОбъектов.Количество() - 1 Цикл
		...
		Если Не ЗначениеЗаполнено(ГраницыУдаленияОбъектов[Индекс].СписокТипов)
			Или Не ЗначениеЗаполнено(ГраницыУдаленияОбъектов[Индекс].ГраницаУдаления) Тогда
			Продолжить;
		КонецЕсли;
		
		ТипыОбъектов = Новый ТаблицаЗначений;
		ТипыОбъектов.Колонки.Добавить("Ссылка", Новый ОписаниеТипов(ГраницыУдаленияОбъектов[Индекс].СписокТипов));
		
		Для Каждого Тип Из ГраницыУдаленияОбъектов[Индекс].СписокТипов Цикл
			МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(Метаданные.НайтиПоТипу(Тип).ПолноеИмя());
			ПустаяСсылка = МенеджерОбъекта.ПустаяСсылка();
			ТипыОбъектов.Добавить().Ссылка = ПустаяСсылка;
		КонецЦикла;
		
		ПереименованияПараметров = Новый Соответствие;
		ПереименованияПараметров["ГраницаУдаления"] = "ГраницаУдаления" + ИндексСтрокой;
		ПереименованияПараметров["СписокТипов"] = "СписокТипов" + ИндексСтрокой;
		ПереименованияПараметров["ТипыОбъектов"] = "ТипыОбъектов" + ИндексСтрокой;

		ЗначенияПараметров = Новый Структура;
		ЗначенияПараметров.Вставить("ГраницаУдаления" + ИндексСтрокой, ГраницыУдаленияОбъектов[Индекс].ГраницаУдаления);
		ЗначенияПараметров.Вставить("СписокТипов" + ИндексСтрокой, ГраницыУдаленияОбъектов[Индекс].СписокТипов);
		ЗначенияПараметров.Вставить("ТипыОбъектов" + ИндексСтрокой, ТипыОбъектов);
		
		Если ГраницыУдаленияОбъектов[Индекс].ТипыВерсийОбменДанными Тогда
			ДополнительноеУсловие = Символы.Таб + "И ВерсииОбъектов.ТипВерсииОбъекта В(&ТипыВерсий)"; // @Query-part-1
			
			ТипыВерсий = Новый Массив;
			ТипыВерсий.Добавить(Перечисления.ТипыВерсийОбъекта.НепринятыеДанныеПоКоллизии);
			ТипыВерсий.Добавить(Перечисления.ТипыВерсийОбъекта.НепринятыйПоДатеЗапретаОбъектНеСуществуетВБазе);
			ТипыВерсий.Добавить(Перечисления.ТипыВерсийОбъекта.НепринятыйПоДатеЗапретаОбъектСуществуетВБазе);
			ТипыВерсий.Добавить(Перечисления.ТипыВерсийОбъекта.ПринятыеДанныеПоКоллизии);

			ЗначенияПараметров.Вставить("ТипыВерсий", ТипыВерсий);
		Иначе
			ДополнительноеУсловие = Символы.Таб 
				+ "И ВерсииОбъектов.ТипВерсииОбъекта = Значение(Перечисление.ТипыВерсийОбъекта.ИзмененоПользователем)"; // @Query-part-1
		КонецЕсли;
		
		ОписаниеПараметров = Новый Структура;
		ОписаниеПараметров.Вставить("Индекс", ИндексСтрокой);
		ОписаниеПараметров.Вставить("ДополнительноеУсловие", ДополнительноеУсловие);
		ОписаниеПараметров.Вставить("ЗначенияПараметров", ЗначенияПараметров);
		ОписаниеПараметров.Вставить("ПереименованияПараметров", ПереименованияПараметров);
		
		ОписанияПараметров.Добавить(ОписаниеПараметров);
	КонецЦикла;
	
	Возврат ОписанияПараметров;
...
#Область ФункцииДляПолученияОтчетаПоОбъекту
...
Функция МетаданныеРеквизита(МетаданныеОбъекта, ИмяРеквизита)
	...
	Результат = МетаданныеОбъекта.ТабличныеЧасти.Найти(ИмяРеквизита);
	Если Результат <> Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВывестиРеквизитыПоРазобранномуОбъекту(Результат, ВерсияОбъекта, СсылкаНаОбъект)
	...
	ВывестиТекстВОтчет(Результат, Секция, "R1C2", НСтр("ru = 'Реквизиты'"), ШрифтыСтиля.КрупныйШрифтТекста);
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	Попытка
		...
		Если Ссылка.ПолучитьОбъект() <> Неопределено Тогда
			РежимЗаписиПроведение = РежимЗаписи = РежимЗаписиДокумента.Проведение 
				Или РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения;
			ЗаписатьДанныеТекущейВерсии(Ссылка, , РежимЗаписиПроведение);
		КонецЕсли;
		...
		ЗаписатьВерсиюОбъекта(Объект, РежимЗаписи);
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ВыполняетсяЗаписьОбъекта(Ложь);
...
Процедура ВключитьАвтоматическуюОчисткуУстаревшихВерсийОбъектов()
	
	УстановитьПараметрРегламентногоЗадания("Использование", Истина);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьНастройкиРегламентногоЗаданияОчисткиУстаревшихВерсийОбъектов");
	Методы.Вставить("ИнформацияОбУстаревшихВерсиях", Истина);
