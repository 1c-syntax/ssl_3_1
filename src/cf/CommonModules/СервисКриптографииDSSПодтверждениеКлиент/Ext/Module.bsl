...
Процедура ОтправкаЗапроса(ОповещениеСледующее, НастройкиПользователя, ДанныеПодтверждения, ПараметрыОперации = Неопределено) Экспорт
	...
	АдресПодключения = СервисКриптографииDSSКлиентСервер.ПолучитьПолеСтруктуры(НастройкиПользователя.Методы, "ДвухфакторнаяАвторизация", "");
...
Процедура ОбработатьРезультатЗапроса(ОповещениеСледующее, 
	...
	ИначеЕсли ДанныеПодтверждения.ЭтапЦикла = "Выбор" Тогда
		Если ДанныеПодтверждения.ДоступныеМетоды.Количество() = 1 Тогда
...
Процедура ФильтроватьСписокСпособов(КонтекстФормы, ИмяЭлемента = "ПодтверждениеСпособПодтверждения") Экспорт
	...
	ЕстьОфлайн = ДанныеПодтверждения.ГрафическийКод <> Неопределено;
	...
	Если СписокМетодов.Количество() = 0 Тогда
		Для каждого СтрокаКлюча Из ДанныеПодтверждения.ДоступныеМетоды Цикл
			СписокВыбора.Добавить(СтрокаКлюча, ПолучитьПредставлениеСпособа(СтрокаКлюча, ДанныеПодтверждения, НастройкиПользователя));
		КонецЦикла;
	Иначе	
		Для каждого СтрокаКлюча Из СписокМетодов Цикл
			Если ДанныеПодтверждения.ДоступныеМетоды.Найти(СтрокаКлюча) <> Неопределено Тогда
				СписокВыбора.Добавить(СтрокаКлюча, ПолучитьПредставлениеСпособа(СтрокаКлюча, ДанныеПодтверждения, НастройкиПользователя));
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;	
	...
	Если ЕстьОфлайн Тогда
		СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение"), 
...
Процедура ПриИзмененииСпособаПодтверждения(КонтекстФормы) Экспорт
	...
	Если СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
	...
	ИначеЕсли СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение")
...
Функция ПодтверждениеПриИзменении(КонтекстФормы, ПолеФормы, ОписаниеДанных, ЗначениеПароля) Экспорт
	...
	ИначеЕсли ИмяЭлемента = "ПодтверждениеСпособПодтверждения" Тогда
		...
		ИначеЕсли КонтекстФормы.СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
			КонтекстФормы.ДанныеПодтверждения.ВторичнаяАвторизация = КонтекстФормы.СпособПодтверждения;
		...
		ИначеЕсли КонтекстФормы.СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение")
			И ДанныеПодтверждения.ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
			КонтекстФормы.ДанныеПодтверждения.ВторичнаяАвторизация = КонтекстФормы.СпособПодтверждения;
...
Процедура ПодтверждениеОбработкаОжидания(КонтекстФормы, ОписаниеДанных, ЗначениеПароля) Экспорт
	...
	ПроверитьСервер = ДанныеПодтверждения.ЭтапЦикла = "Ожидание" 
				...
				И ДанныеПодтверждения.ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение"));
...
//
// Параметры:
//  ВторичнаяАвторизация	- ПеречислениеСсылка - выбранный способ подтверждения
//  ДанныеПодтверждения		- Структура - описывает данные цикла подтверждения
//  НастройкиПользователя	- см. СервисКриптографииDSS.НастройкиПользователяПоУмолчанию
//	
// Возвращаемое значение:
...
Функция ПолучитьПредставлениеСпособа(ВторичнаяАвторизация, ДанныеПодтверждения, НастройкиПользователя)
	...
	Если ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_СМС") Тогда
		Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(СодержаниеПредставленияТелефона(НастройкиПользователя, КодЯзыка));
		
	ИначеЕсли ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ЭлектроннаяПочта") Тогда
		Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(СодержаниеПредставленияПочты(НастройкиПользователя, КодЯзыка));
		
	ИначеЕсли ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение") Тогда
		Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(НСтр("ru='Мобильное приложение <span style=""font: ВыделенныйЗаголовокПодтвержденияШрифт"">КриптоПро myDSS</span>'", КодЯзыка));
		
	ИначеЕсли ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_AirKeyLite") Тогда
		Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(НСтр("ru='Мобильное приложение <span style=""font: ВыделенныйЗаголовокПодтвержденияШрифт"">AirKey DSS</span>'", КодЯзыка)); 
		
	ИначеЕсли ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
		Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(НСтр("ru = 'Офлайн-подтверждение через приложение'", КодЯзыка));
...
Процедура ПодтверждениеОбработатьЭтапЦикла(КонтекстФормы, ОписаниеДанных, ЗначениеПароля, ОжиданиеОтветаПользователя = Ложь)
	...
	ИначеЕсли ДанныеПодтверждения.ЭтапЦикла = "ВводКода" И РазрешенЗапросСервера Тогда
		Если ВторичнаяАвторизация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
			Разница = ДатаСеанса - КонтекстФормы.ДанныеПодтверждения.СрокОпроса;
			Если Разница > ДанныеПодтверждения.ПериодОпроса Тогда
				КонтекстФормы.ОжидатьВыполнения = Истина;
				КонтекстФормы.ДанныеПодтверждения.СрокОпроса = ДатаСеанса;
...
Функция КлючДействияОперации(ТипОперации, НастройкиПользователя, ДанныеПодтверждения)
	...
	Иначе	
		Если ТипОперации = "Подписание" Тогда
			Если ДанныеПодтверждения.ПараметрыВариантаВстроенного.ПакетнаяОперация Тогда
	...
	Если РезультатВызова.Выполнено Тогда
		Если РезультатВызова.Свойство("ДанныеПодтверждения") Тогда
			...
			Если КонтекстФормы.СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
				КонтекстФормы.СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение");
			КонецЕсли;
			ПодтверждениеОбработатьЭтапЦикла(КонтекстФормы, ВходящийКонтекст.ОписаниеДанных, ВходящийКонтекст.ЗначениеПароля);
		КонецЕсли;
...
Процедура СформироватьПодсказкуСпособовВыбора(КонтекстФормы, ИмяЭлемента = "ПодтверждениеСпособПодтверждения")
	...
	Если СписокВыбора.Количество() = 0 Тогда
		...
		+ НСтр("ru = 'Пользователю не назначен ни один способ подтверждения.'", КодЯзыка);
	КонецЕсли;
	
	ПерваяЧасть = НСтр("ru = 'Можно сохранить настройки подтверждения в'", КодЯзыка);
...
Процедура УправлениеЭлементамиДляПодписания(ЭлементыФормы, НеНачалось, НуженПинКод, ПроверенПинКод = Ложь, СтандартныйВид = Ложь)
	...
	ЭлементыФормы.ГруппаПароль.Видимость = НуженПинКод ИЛИ СтандартныйВид;
	ЭлементыФормы.Пароль.ТолькоПросмотр = ПроверенПинКод;
	Если СтандартныйВид Тогда
		ЭлементыФормы.Подписать.КнопкаПоУмолчанию = Истина;
		ЭлементыФормы.Пароль.ТолькоПросмотр = Ложь;
	Иначе
		ЭлементыФормы.Страницы.Видимость = Ложь;
	КонецЕсли;	
	ЭлементыФормы.Подписать.Видимость = НеНачалось ИЛИ СтандартныйВид;
	ЭлементыФормы.Пароль.Заголовок = ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Пин-код'", КодЯзыка), НСтр("ru = 'Пароль'", КодЯзыка));
	ЭлементыФормы.ЗапомнитьПароль.Заголовок = ?(НуженПинКод И НЕ СтандартныйВид, 
								НСтр("ru = 'Запомнить пин-код'", КодЯзыка),
								НСтр("ru = 'Запомнить пароль'", КодЯзыка));
	ЭлементыФормы.ГруппаПодписание.ОтображатьЗаголовок = СтандартныйВид;
...
Процедура УправлениеЭлементамиДляРасшифрования(ЭлементыФормы, НеНачалось, НуженПинКод, ПроверенПинКод = Ложь, СтандартныйВид = Ложь)
	...
	ЭлементыФормы.Пароль.ТолькоПросмотр = ПроверенПинКод;
	Если СтандартныйВид Тогда
		ЭлементыФормы.ФормаРасшифровать.КнопкаПоУмолчанию = Истина;
		ЭлементыФормы.Пароль.ТолькоПросмотр = Ложь;
	Иначе
		ЭлементыФормы.Страницы.Видимость = Ложь;
	КонецЕсли;	
	ЭлементыФормы.ФормаРасшифровать.Видимость = НеНачалось ИЛИ СтандартныйВид;
	ЭлементыФормы.Пароль.Заголовок = ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Пин-код'", КодЯзыка), НСтр("ru = 'Пароль'", КодЯзыка));
	ЭлементыФормы.ЗапомнитьПароль.Заголовок = ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Запомнить пин-код'", КодЯзыка), НСтр("ru = 'Запомнить пароль'", КодЯзыка));
	ЭлементыФормы.ГруппаРасшифровка.ОтображатьЗаголовок = СтандартныйВид;
...
Процедура УправлениеЭлементамиДляПроверки(ЭлементыФормы, НеНачалось, НуженПинКод, ПроверенПинКод = Ложь, СтандартныйВид = Ложь)
	...
	ЭлементыФормы.ГруппаВводаПароля.Видимость = НуженПинКод ИЛИ СтандартныйВид;
	ЭлементыФормы.Пароль.ТолькоПросмотр = ПроверенПинКод;
	Если СтандартныйВид Тогда
		ЭлементыФормы.ФормаПроверить.КнопкаПоУмолчанию = Истина;
		ЭлементыФормы.Пароль.ТолькоПросмотр = Ложь;
	Иначе
		ЭлементыФормы.Страницы.Видимость = Ложь;
	КонецЕсли;
	ЭлементыФормы.ГруппаВводаПароля.ОтображатьЗаголовок = СтандартныйВид;
	ЭлементыФормы.ФормаПроверить.Видимость = НеНачалось ИЛИ СтандартныйВид;
	ЭлементыФормы.Пароль.Заголовок = ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Пин-код'", КодЯзыка), НСтр("ru = 'Пароль'", КодЯзыка));
	ЭлементыФормы.ЗапомнитьПароль.Заголовок = ?(НуженПинКод И НЕ СтандартныйВид, 
								НСтр("ru = 'Запомнить пин-код'", КодЯзыка),
								НСтр("ru = 'Запомнить пароль'", КодЯзыка));
...
Процедура УправлениеЭлементамиОбщее(КонтекстФормы, ОткудаВызов)
	...
	Если НЕ ТребуетсяПодтверждение ИЛИ НЕ ИспользованиеВозможно Тогда
		ВариантВидимости = 0;
	ИначеЕсли СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_AirKeyLite")
		ИЛИ СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение") Тогда
		ВариантВидимости = 3;
	ИначеЕсли СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_СМС")
		ИЛИ СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ЭлектроннаяПочта") Тогда
		ВариантВидимости = 1;
	ИначеЕсли СпособПодтверждения = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
		ВариантВидимости = 2;
	Иначе
		ВариантВидимости = 4;
	КонецЕсли;
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеРазделитель3", "Видимость", ИспользованиеВозможно И (ВариантВидимости ИЛИ БылаАвторизация));
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ГруппаПодтвержденияКод", "Видимость", НЕ НеНачалось И (ЕстьАвторизация ИЛИ ОперацияАутентификации) И (ВариантВидимости = 1 ИЛИ ВариантВидимости = 2) И ВариантВидимости);
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеПовторить", "Видимость", НЕ НеНачалось И ВариантВидимости = 1 И ВариантВидимости);
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ГруппаПодтвержденияОфлайн", "Видимость", НЕ НеНачалось И (ЕстьАвторизация ИЛИ ОперацияАутентификации) И ВариантВидимости = 2 И ВариантВидимости);
	...
	КонецЕсли;	
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ГруппаПодтвержденияОписание", "Видимость", НЕ НеНачалось И (ЕстьАвторизация ИЛИ ОперацияАутентификации) И ВариантВидимости);
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ГруппаПодтвержденияОнлайн", "Видимость", НЕ НеНачалось И (ЕстьАвторизация ИЛИ ОперацияАутентификации) И ВариантВидимости = 3 И ВариантВидимости);
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеСпособПодтвержденияОшибка", "Видимость", ВариантВидимости = 4 И ВариантВидимости);
