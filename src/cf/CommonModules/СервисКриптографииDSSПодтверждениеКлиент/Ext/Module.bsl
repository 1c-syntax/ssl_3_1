...
Процедура ПодтверждениеОбработкаОжидания(КонтекстФормы, ОписаниеДанных, ЗначениеПароля) Экспорт
	...
	Подтверждать = КлючДействияОперации(ТипОперации, НастройкиПользователя, ДанныеПодтверждения);
...
Процедура ПодтверждениеПриОткрытии(КонтекстФормы, Отказ = Ложь, ПарольУстановлен = Ложь, ОписаниеДанных = Неопределено) Экспорт
	...
	Если НЕ Отказ Тогда
		...
		Если ЗначениеЗаполнено(ДанныеПодтверждения) Тогда
			...
			ДанныеПодтверждения.ПараметрыВариантаВстроенного.ПодтверждатьПринудительно = СервисКриптографииDSSКлиентСервер.ПолучитьПолеСтруктуры(ОписаниеДанных, "ПринудительноПодтверждатьОблачнуюОперацию", Ложь);
		КонецЕсли;
...
Функция ОблачнаяПодписьТребуетПодтверждения(КонтекстФормы, УчетАвторизации = Ложь) Экспорт
	...
	Если ЭтоОперацияОблачнойПодписи(КонтекстФормы) Тогда
		...
		Если УчетАвторизации Тогда
			Результат = Авторизован И Подтверждать;
		Иначе	
			Результат = ТребуетПинКод ИЛИ НЕ Авторизован ИЛИ Подтверждать;
		КонецЕсли;	
...
Функция ПроверкаПередВыполнениемОперации(КонтекстФормы, ЗначениеПароля) Экспорт
	...
	Если ЭтоОперацияОблачнойПодписи(КонтекстФормы) Тогда
		...
		Если Подтверждать И НЕ ВыбранСпособ Тогда
...
Процедура ВыполнитьНачальнуюОперациюСервиса(
	...
	ПараметрыОперации.Вставить("ОтобразитьОшибку", ОписаниеДанных = Неопределено);
...
Процедура ВыполнитьНачальнуюОперациюСервисаПослеАвторизации(РезультатВызова, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		ПоказатьОшибкуФормыПодтверждения(КонтекстФормы, 
					...
					РезультатВызова.СтатусОшибки.Представление,
					ВходящийКонтекст.ОписаниеДанных,
					ВходящийКонтекст.ЗначениеПароля,
					ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ВходящийКонтекст, "ОтобразитьОшибку", Ложь));
	КонецЕсли;	
...
Функция КлючДействияОперации(ТипОперации, НастройкиПользователя, ДанныеПодтверждения)
	...
	Иначе	
		Если ТипОперации = "Подписание" Тогда
			Если ДанныеПодтверждения.ВерсияПодтверждения = 2 Тогда
				...
				Обязательно = ДанныеПодтверждения.Сертификат.МобильныйЗакрытыйКлюч;
	...
	Возврат Обязательно
		ИЛИ КлючДействия.Подтверждать
		ИЛИ ПолучитьПолеСтруктуры(ДанныеПодтверждения, "ПараметрыВариантаВстроенного.ПодтверждатьПринудительно", Ложь);
...
Процедура ВыполнитьНачальнуюОперациюСервисаПослеВыполнения(РезультатВыполнения, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		ПоказатьОшибкуФормыПодтверждения(КонтекстФормы,
							ДанныеПодтверждения,
							РезультатВыполнения.СтатусОшибки.Представление,
							ВходящийКонтекст.ОписаниеДанных,
							ВходящийКонтекст.ЗначениеПароля,
							ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ВходящийКонтекст, "ОтобразитьОшибку", Ложь));
...
Процедура ПоказатьОшибкуФормыПодтверждения(КонтекстФормы, 
	...
	Если НЕ ОтобразитьОшибку Тогда
		РезультатВызова = СервисКриптографииDSSКлиент.ОтветСервисаПоУмолчанию(Ложь);
		РезультатВызова.СтатусОшибки.Представление = ?(ПустаяСтрока(СодержаниеОшибки), ДанныеПодтверждения.Ошибка, СодержаниеОшибки);
		РезультатВызова.Ошибка = РезультатВызова.СтатусОшибки.Представление;
		УправлениеЭлементамиОбщее(КонтекстФормы, 9);
		ВыполнитьОповещениеФормы(КонтекстФормы, РезультатВызова, "ПодтверждениеВыполнитьОсновнуюОперацию");
		
	ИначеЕсли ЗначениеЗаполнено(СодержаниеОшибки) Тогда
...
Процедура УправлениеЭлементамиОбщее(КонтекстФормы, ОткудаВызов)
	...
	ПарольНеНужен			= НастройкиПользователя.ПервичнаяАутентификация = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Первичный_СервисныйКлиент");
					
	ТребуетсяПодтверждение	= КлючДействияОперации(ФормаИсточник, НастройкиПользователя, ДанныеПодтверждения);
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ГруппаПодтверждениеПарольЗапомнить", "Видимость", НЕ ОтдельнаяАвторизация И БылаАвторизация И НЕ ПарольНеНужен, ИменаЭлементов);
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеПароль", "Видимость", НЕ ПарольНеНужен, ИменаЭлементов);
