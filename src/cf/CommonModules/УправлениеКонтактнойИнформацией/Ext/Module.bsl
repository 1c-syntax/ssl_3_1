...
#Область ДобавлениеИзменениеИПолучениеКонтактнойИнформации
...
//
Процедура ДобавитьКонтактнуюИнформацию(СсылкаИлиОбъект, ЗначениеИлиПредставление, ВидКонтактнойИнформации, 
	Дата = Неопределено, Замещать = Истина, РаспознатьАдрес = Истина) Экспорт

	Если ПустаяСтрока(ЗначениеИлиПредставление) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СсылкаИлиОбъект)) Тогда
		УправлениеКонтактнойИнформациейСлужебный.ДобавитьКонтактнуюИнформациюДляСсылки(СсылкаИлиОбъект,
			ЗначениеИлиПредставление, ВидКонтактнойИнформации, Дата, Замещать, РаспознатьАдрес);
		Возврат;
	КонецЕсли;
	
	УправлениеКонтактнойИнформациейСлужебный.ДобавитьКонтактнуюИнформацию(СсылкаИлиОбъект, ЗначениеИлиПредставление, 
		ВидКонтактнойИнформации, Дата, Замещать, РаспознатьАдрес);
...
Функция КонтактнаяИнформацияВXML(Знач ЗначенияПолей, Знач Представление = "", Знач ОжидаемыйВид = Неопределено) Экспорт
	...
	Данные = Новый Структура("ЗначенияПолей, Представление, ВидКонтактнойИнформации",
...
#Область РаботаСКонтактнойИнформациейИзДругихПодсистем
...
Функция ТипКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	Если УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступенМодульЛокализации() Тогда
		...
		Тип = Неопределено;
		УправлениеКонтактнойИнформациейЛокализация.ПриОпределенииТипаКонтактнойИнформации(КонтактнаяИнформация, Тип);
...
Функция ПредставлениеКонтактнойИнформации(Знач КонтактнаяИнформация, Знач ПеревестиВТранслит = Ложь) Экспорт
	...
	Возврат УправлениеКонтактнойИнформациейСлужебный.ПредставлениеКонтактнойИнформации(КонтактнаяИнформация, ПеревестиВТранслит);
...
Функция ДанныеСтраныМира(Знач КодСтраны = Неопределено, Знач Наименование = Неопределено) Экспорт
	...
	Если КодСтраны <> Неопределено Тогда
		ШаблонКод = "(Код = %1 ИЛИ Код = %2)";
		УсловиеПоиска.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКод,
			КонтрольКавычекВСтроке(НормализованныйКод), КонтрольКавычекВСтроке(КодСтраны)));
...
#Область ОбработчикиСобытийФормыИМодуляОбъектаВызываемыеПриВнедренииПодсистемы
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Форма, Объект, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из УправлениеКонтактнойИнформациейКлиентСервер.ОписаниеКонтактнойИнформацииНаФорме(Форма) Цикл
		...
		ИмяРеквизита  = СтрокаТаблицы.ИмяРеквизита;
		СвойствоВидаИнформации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидИнформации, 
			"ОбязательноеЗаполнение, ВидРедактирования");
		...
		Если СтрокаТаблицы.ЭтоРеквизитТабличнойЧасти Тогда
			...
			Для Каждого СтрокаТабличнойЧастиФормы Из ТабличнаяЧастьФормы Цикл
				...
				Поле = "Объект." + ИмяТабличнойЧасти + "[" 
					+ XMLСтрока((СтрокаТабличнойЧастиФормы.НомерСтроки - 1)) + "]." + ИмяРеквизита;
				...
				Если ОбязательноеЗаполнение И ПустаяСтрока(Представление) И Не ВидИнформации.ПометкаУдаления Тогда
					...
					ОбщегоНазначения.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""%1"" не заполнено.'"), 
							ВидИнформации.Наименование),,Поле);
					ТекущийУровеньОшибок = 2;
					
				Иначе
					
					Значение = СтрокаТабличнойЧастиФормы[ИмяРеквизита + "Значение"];
					ТекущийУровеньОшибок = ПроверитьЗаполнениеКонтактнойИнформации(Представление, Значение, 
						ВидИнформации, ТипИнформации, ИмяРеквизита, , Поле);
		...
		Иначе
			...
			Если ВидИнформации <> ПредыдущийВид И ОбязательноеЗаполнение И ПустаяСтрока(Представление)
				...
				ОбщегоНазначения.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""%1"" не заполнено.'"), 
						ВидИнформации.Наименование),,, ИмяРеквизита);
				ТекущийУровеньОшибок = 2;
...
Процедура ПередЗаписьюНаСервере(Форма, Объект, Отказ = Ложь) Экспорт
	...
	ОпределитьПараметрыКонтактнойИнформацииПоВладельцу(Форма, Объект, ПараметрыКонтактнойИнформации, 
		ЭтоПараметрыОсновногоОбъекта, СкрываемыеВиды);
...
#Область ВспомогательныеФункцииИКонструкторы
...
#Область ПроверкаИСведенияОбАдресе

// Возвращает тип адреса (адресного деления) из строки‑адреса в формате JSON.
//
// Не рекомендуется сравнивать тип адреса напрямую со строкой. 
// Следует использовать специализированные функции общих модулей 
// УправлениеКонтактнойИнформациейКлиентСервер и РаботаСАдресамиКлиентСервер 
// (если они присутствуют в конфигурации). 
// Подробнее см. функции АдресВСвободнойФорме, ИностранныйАдрес, ЕАЭС из общего модуля 
...
//
// Возвращаемое значение:
//   Строка - тип адреса, например: "Иностранный", "ЕАЭС", "ВСвободнойФорме" и др.
//            Если тип адреса не удалось определить, возвращается пустая строка.
//
Функция ТипАдреса(Адрес) Экспорт
	...
	Если ТипЗнч(Адрес) = Тип("Строка") Тогда
		...
		АдресИзJSON = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
		
		Возврат АдресИзJSON.addressType;
...
Функция СведенияОбАдресе(Адрес = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Адрес = Неопределено Тогда
		Возврат УправлениеКонтактнойИнформациейКлиентСервер.ПоляАдреса();
	КонецЕсли;
	
	Адреса = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Адрес);
	Результат = СведенияОбАдресах(Адреса, ДополнительныеПараметры);
...
Функция СведенияОбАдресах(Адреса, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Возврат СведенияОбАдресахВВидеПолей(Адреса, ДополнительныеПараметры);
КонецФункции
...
// Конструктор параметров, описывающий правила формирования представления адреса.
// 
// Возвращаемое значение:
//  Структура - новые параметры представления адреса:
//   * СписокПолей - Массив - поля, которые будут включены в представление (например: ["Город", "Улица", "Дом"]).
//                            В случае отсутствия данных будет применен стандартный набор и  порядок полей.
//   * ТолькоНаселенныйПункт - Булево - если Истина, то представление должно быть сформировано только для населенного
//                                      пункта (без улицы, дома и помещений). По умолчанию: Ложь.
//   * ПочтовыйАдрес - Булево - если Истина, то представление будет сформировано в порядке, соответствующем
//                              почтовому адресу: улица, дом, квартира, населенный пункт, страна, индекс. По умолчанию:
//                              Ложь.
//   * ВключатьПочтовыйИндекс - Булево - флаг, указывающий на необходимость включения почтового индекса в представление
//                                       адреса. По умолчанию: Истина.
//   * ВключатьСтрану - Булево -  флаг, указывающий на необходимость включения страны в представление адреса. По
//                                умолчанию: Ложь.
//   * ВключатьУлицу- Булево - флаг, указывающий на необходимость включения улицы в представление адреса. По
//                                умолчанию: Истина.
//   * ВключатьЗданияИПомещения - Булево - флаг, указывающий на необходимость включения зданий и помещений в
//                                 представление адреса. По умолчанию: Истина.
//   * ПостфиксТипаАдреса - Строка - содержит суффикс, добавляемый к имени поля для указания типа адресного элемента.
//                                  Используется для формирования составных наименований полей адреса.
//                                  Например, "Улица" и "УлицаТип", где "Международное" и "ш.".
//                                  Если не задано, используется вариант "ТипКраткий". 
//                                  Если требуется отображать полные названия адресных объектов и домов, 
//                                  используйте, например, вариант "ТипПолный".
// 
Функция НовыеПараметрыПредставленияАдреса() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("СписокПолей",              Новый Массив);
	Результат.Вставить("ТолькоНаселенныйПункт",    Ложь);
	Результат.Вставить("ПочтовыйАдрес",            Ложь);
	Результат.Вставить("ВключатьПочтовыйИндекс",   Истина);
	Результат.Вставить("ВключатьСтрану",           Ложь);
	Результат.Вставить("ПостфиксТипаАдреса",       "");
	Результат.Вставить("ВключатьУлицу",            Истина);
	Результат.Вставить("ВключатьЗданияИПомещения", Истина);
	Возврат Результат;
...
Функция ПредставлениеАдреса(Адрес, Знач ПараметрыПредставленияАдреса = Неопределено) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(Адрес) Тогда
		АдресПоПолям = СведенияОбАдресе(Адрес);
		ТипАдреса = АдресПоПолям.ТипАдреса;
	ИначеЕсли ТипЗнч(Адрес) = Тип("Структура") Тогда
		АдресПоПолям = Новый Структура(Новый ФиксированнаяСтруктура(Адрес));
		Если АдресПоПолям.Свойство("ТипАдреса") Тогда
			ТипАдреса = АдресПоПолям.ТипАдреса;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(АдресПоПолям) Тогда
		Возврат "";
	КонецЕсли;
	
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(ТипАдреса)  Тогда
		Если АдресПоПолям.Свойство("Представление") Тогда
			Возврат АдресПоПолям["Представление"];
		Иначе
			Возврат "";
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыПредставления =  НовыеПараметрыПредставленияАдреса();
	Если ЗначениеЗаполнено(ПараметрыПредставленияАдреса) Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыПредставления, ПараметрыПредставленияАдреса);
	КонецЕсли;
	
	Если Не УправлениеКонтактнойИнформациейКлиентСервер.ЭтоИностранныйАдресИлиЕАЭС(ТипАдреса) Тогда
	
		Если УправлениеКонтактнойИнформациейСлужебныйПовтИсп.ДоступныМодулиРаботаСАдресами() Тогда
			МодульРаботаСАдресами = ОбщегоНазначения.ОбщийМодуль("РаботаСАдресами");
			Возврат МодульРаботаСАдресами.ПредставлениеАдреса(АдресПоПолям, ТипАдреса, ПараметрыПредставления);
		КонецЕсли;
	
	КонецЕсли;
	
	ПорядокПолей = ОпределитьПорядокУровнейДляПредставления(ПараметрыПредставления);
	
	Для Каждого ИмяПоля Из ПорядокПолей Цикл
		
		Если ИмяПоля = "Дом" Тогда
			
			ПоляПредставления.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(АдресПоПолям.Здание, "Номер", "ТипЗдания", Истина));
			
			Для Каждого Корпус Из АдресПоПолям.Корпуса Цикл
				ПоляПредставления.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(Корпус, "Номер", "ТипЗдания", Истина));
			КонецЦикла;
			
		ИначеЕсли ИмяПоля = "Помещения" Тогда
			Для Каждого Помещение Из АдресПоПолям.Помещения Цикл
				ПоляПредставления.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(Помещение, "Номер", "ТипПомещения", Истина));
			КонецЦикла;
			
		ИначеЕсли АдресПоПолям.Свойство(ИмяПоля) И ЗначениеЗаполнено(АдресПоПолям[ИмяПоля]) Тогда
			ПоляПредставления.Добавить(УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеАдресногоОбъекта(АдресПоПолям, ИмяПоля));
		КонецЕсли;
	КонецЦикла;
	
	Представление = СтрСоединить(ПоляПредставления, ", ");
...
Функция ПараметрыГруппыВидаКонтактнойИнформации(ГруппаКонтактнойИнформации = Неопределено) Экспорт
	...
	Если ТипЗнч(ГруппаКонтактнойИнформации ) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
		Значения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ГруппаКонтактнойИнформации, 
			"ИмяПредопределенногоВида, ИмяПредопределенныхДанных, Родитель, Наименование, Используется");
		Результат.Имя = ?(ЗначениеЗаполнено(Значения.ИмяПредопределенногоВида), Значения.ИмяПредопределенногоВида, 
			Значения.ИмяПредопределенныхДанных);
...
Функция ПараметрыАдреса() Экспорт
	...
	Возврат ПараметрыВидаКонтактнойИнформации(Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Функция ПараметрыАдресаЭлектроннойПочты() Экспорт
	...
	Возврат ПараметрыВидаКонтактнойИнформации(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
...
Функция ПараметрыТелефона() Экспорт
	...
	Возврат ПараметрыВидаКонтактнойИнформации(Перечисления.ТипыКонтактнойИнформации.Телефон);
...
Процедура ЗаписатьКонтактнуюИнформацию(Объект, Знач Значение, ВидИнформации, ТипИнформации, ИдентификаторСтроки = 0, Дата = Неопределено) Экспорт
	...
	УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнуюИнформациюИзJSONВXML(ОбъектКИ, НоваяСтрока.ЗначенияПолей, ТипИнформации);
...
Функция КонтактнаяИнформацияXMLПоПредставлению(Представление, ОжидаемыйВид) Экспорт
	...
	УправлениеКонтактнойИнформациейЛокализация.ПриПреобразованииКонтактнойИнформацииXDTOПоПредставлению(Представление, XDTOОбъект, ОжидаемыйВид);
	Если XDTOОбъект <> Неопределено Тогда
		УправлениеКонтактнойИнформациейЛокализация.ПриПреобразованииКонтактнойИнформацииXDTOВXML(XDTOОбъект, Результат);
	КонецЕсли;
...
Функция ТипАдресЭлектроннойПочты() Экспорт
...
Процедура ИзменитьПочтуДляВосстановленияПароля(ПользовательОбъект, НовыйАдрес, СтарыйАдрес) Экспорт
	...
	ВладелецКонтактнойИнформации = ?( ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.ВнешниеПользователи"),
		ПользовательОбъект.ОбъектАвторизации, ПользовательОбъект);
	
	ЭлектроннаяПочтаПользователя = КонтактнаяИнформацияОбъекта(ВладелецКонтактнойИнформации, 
		Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ТекущаяДатаСеанса(), Ложь);
	
	СтрокаСНовымАдресом = ЭлектроннаяПочтаПользователя.Найти(НовыйАдрес, "Представление");
	Если СтрокаСНовымАдресом <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаСАдресом = ЭлектроннаяПочтаПользователя.Найти(СтарыйАдрес, "Представление");
	Если СтрокаСАдресом <> Неопределено Тогда
		ЗаполнитьСтрокуТаблицыКонтактнойИнформацииПоПредставлению(СтрокаСАдресом, НовыйАдрес);
		УстановитьКонтактнуюИнформациюОбъекта(ВладелецКонтактнойИнформации, 
			ЭлектроннаяПочтаПользователя, Истина);
		Возврат;
	КонецЕсли;
	
	ВидыКонтактнойИнформацииОбъекта = ВидыКонтактнойИнформацииОбъекта(ВладелецКонтактнойИнформации,
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи") Тогда 
		ВидКонтактнойИнформации = ВидКонтактнойИнформацииПоИмени("EmailПользователя");
		Если ВидКонтактнойИнформации <> Неопределено Тогда
			ПараметрыВидаКИ = ПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации);
			Если ПараметрыВидаКИ.РазрешитьВводНесколькихЗначений Тогда
				ДобавитьКонтактнуюИнформацию(ВладелецКонтактнойИнформации,
					НовыйАдрес, ВидКонтактнойИнформации, ТекущаяДатаСеанса(), Ложь);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ВидКонтактнойИнформации = Неопределено;
	Для Каждого ВидКонтактнойИнформацииОбъекта Из ВидыКонтактнойИнформацииОбъекта Цикл
		Если ВидКонтактнойИнформацииОбъекта.РазрешитьВводНесколькихЗначений Тогда
			ВидКонтактнойИнформации = ВидКонтактнойИнформацииОбъекта.Ссылка;
			Прервать;
		КонецЕсли;
		СтрокаСАдресом = ЭлектроннаяПочтаПользователя.Найти(ВидКонтактнойИнформацииОбъекта.Ссылка, "Вид");
		Если СтрокаСАдресом = Неопределено Тогда
			ВидКонтактнойИнформации = ВидКонтактнойИнформацииОбъекта.Ссылка;
			Прервать;
		КонецЕсли;
	...
	Если ВидКонтактнойИнформации <> Неопределено Тогда
		ДобавитьКонтактнуюИнформацию(ВладелецКонтактнойИнформации,
			НовыйАдрес, ВидКонтактнойИнформации, ТекущаяДатаСеанса(), Ложь);
	ИначеЕсли ЭлектроннаяПочтаПользователя.Количество() > 0 Тогда
		ЗаполнитьСтрокуТаблицыКонтактнойИнформацииПоПредставлению(ЭлектроннаяПочтаПользователя[0], НовыйАдрес);
		УстановитьКонтактнуюИнформациюОбъекта(ВладелецКонтактнойИнформации, ЭлектроннаяПочтаПользователя, Истина);
	КонецЕсли;
...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	Набор.Имя = "Справочник_СтраныМира";
	Набор.Идентификатор = Новый УникальныйИдентификатор("38186b5b-6c32-40fa-b894-8eadad18bf62");
КонецПроцедуры
...
//
Функция СведенияОбАдресахВВидеПолей(Адреса, ДополнительныеПараметры)
	...
	Для ИндексАдреса = 0 По Адреса.ВГраница() Цикл
		...
		Адрес = Адреса.Получить(ИндексАдреса);
		
		АдресИзJSON = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(Адрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
		ПоляАдреса = УправлениеКонтактнойИнформациейКлиентСервер.ПоляАдреса();
		
		ПоляАдреса.ТипАдреса   = АдресИзJSON.addressType;
		ПоляАдреса.Комментарий = АдресИзJSON.comment;
		ПоляАдреса.Страна      = АдресИзJSON.country;
		ПоляАдреса.КодСтраны   = АдресИзJSON.countryCode;
		ПоляАдреса.Регион      = АдресИзJSON.area;
		ПоляАдреса.Индекс      = АдресИзJSON.ZIPcode;
		ПоляАдреса.Город       = АдресИзJSON.city;
		ПоляАдреса.Улица       = АдресИзJSON.street;
		
		ПоляАдреса.ДополнительнаяИнформация = АдресИзJSON.houseNumber;
		
		Если ДополнительныеПараметры.Свойство("БезПредставлений") 
		   И ДополнительныеПараметры.БезПредставлений Тогда
			ПоляАдреса.Удалить("Представление");
		Иначе
			ПоляАдреса.Представление = АдресИзJSON.value;
		КонецЕсли;
		
		Результат.Добавить(ПоляАдреса);
...
//
Функция ОпределитьПорядокУровнейДляПредставления(ПараметрыПредставленияАдреса)
...
#Область ИнициализацияЭлементовНаФормеОбъектВладельцаКонтактнойИнформации
...
Процедура СоздатьРеквизитыКонтактнойИнформации(Знач Форма, Знач ДобавляемыеРеквизиты, Знач ИмяОбъекта, Знач РазмещеныНаФорме,
	...
	Для Каждого ОбъектКонтактнойИнформации Из КонтактнаяИнформация Цикл
		...
		Если ОбъектКонтактнойИнформации.ПометкаУдаления И ПустаяСтрока(ОбъектКонтактнойИнформации.Значение)
		   И ПустаяСтрока(ОбъектКонтактнойИнформации.ЗначенияПолей) Тогда
			Продолжить;
		КонецЕсли;
		...
		Иначе
			...
			Если СозданныйРеквизит = Неопределено Тогда
				...
				КонецЕсли;
			Иначе
				ИмяВида = ?(ЗначениеЗаполнено(ОбъектКонтактнойИнформации.ИмяПредопределенныхДанных),
					ОбъектКонтактнойИнформации.ИмяПредопределенныхДанных, ОбъектКонтактнойИнформации.ИмяПредопределенногоВида);
				ОбъектКонтактнойИнформации.ИмяРеквизита = "КонтактнаяИнформацияПоле" + ИмяВида;
				Если ЕстьПолеКомментарий Тогда
					ОбъектКонтактнойИнформации.ИмяРеквизитаКомментарий = "КомментарийКонтактнаяИнформацияПоле" + ИмяВида;
				КонецЕсли;
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь, 
	КоличествоАдресов = Неопределено, СледующаяСтрока = Неопределено)
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		Если ТипЗнч(ДобавляемыйВид)= Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
			ИнформацияОВидеКИ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДобавляемыйВид, 
				"Тип, Наименование, ВидРедактирования, ВидПоляДругое, ВводитьНомерПоМаске, МаскаНомераТелефона");
		Иначе
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизита, ОписаниеТипаРеквизита,, 
			ИнформацияОВидеКИ.Наименование, Истина));
		Если ЕстьПолеКомментарий Тогда
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизитаКомментарий, 
				ОписаниеТипаРеквизита,, ИнформацияОВидеКИ.Наименование, Истина));
		КонецЕсли;
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() И ПараметрыКонтактнойИнформации.ОтображатьИконки Тогда
		ГруппаСтрокиЗаголовок = Группа("ГруппаЗаголовок" + ИмяРеквизита, Форма, ДобавляемыйВидНаименование, 
			ИмяЭлементаДляРазмещения, "ГруппаЗначенияКИ" + ИмяЭлементаДляРазмещения, 6);
		...
		ГруппаСтрокиЗаголовокКартинка = Группа("ГруппаЗаголовокКартинка" + ИмяРеквизита, Форма, 
			ДобавляемыйВидНаименование, ИмяЭлементаДляРазмещения, "ГруппаЗаголовок" + ИмяРеквизита);
		...
		ДекорацияЗаголовок = Форма.Элементы.Добавить("Заголовок" + ИмяРеквизита, Тип("ДекорацияФормы"), 
			ГруппаСтрокиЗаголовокКартинка);
	...
	Если ЕстьПолеКомментарий Тогда
		...
		ШиринаПоляКомментарий = ?(ПараметрыКонтактнойИнформации.ГруппаДляРазмещенияОграниченаШириной, 
			ПараметрыКонтактнойИнформации.ШиринаПоляКомментарий, 30);
...
Функция СоздатьПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,
	ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	ИначеЕсли ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница И ОбработкаНавигационнойСсылки Тогда
		...
		Если ТипЗнч(ИнформацияОВидеКИ) <> Тип("Структура") 
			И УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(ИнформацияОВидеКИ.Значение) Тогда
			КонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
				ИнформацияОВидеКИ.Значение, Перечисления.ТипыКонтактнойИнформации.ВебСтраница);
			АдресСайта    = КонтактнаяИнформация.value;
			Представление = ?(КонтактнаяИнформация.Свойство("name") И ЗначениеЗаполнено(КонтактнаяИнформация.name), 
				КонтактнаяИнформация.name, ИнформацияОВидеКИ.Представление);
		Иначе
			АдресСайта = "";
	...
	Иначе
		...
		Если ИнформацияОВидеКИ.ВидРедактирования = "Диалог" 
			И ИнформацияОВидеКИ.Тип <> Перечисления.ТипыКонтактнойИнформации.ВебСтраница Тогда
...
Процедура ПодготовитьСтатическийЭлемент(Форма, СтрокаКИ, СозданныеЭлементы, СозданныйЭлемент, ОтображатьИконки, 
	ИмяЭлементаДляРазмещения)
	...
	Если Форма.Элементы.Найти(ИмяЭлементаГруппаСтроки) <> Неопределено Тогда
		...
		Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			...
			Если ОтображатьИконки Тогда	
				ГруппаСтрокиЗаголовок = Группа("ГруппаЗаголовок" + СтрокаКИ.ИмяРеквизита, Форма, 
					ГруппаСтроки.Заголовок, ИмяЭлементаДляРазмещения, "ГруппаЗначенияКИ"+ИмяЭлементаДляРазмещения, 6);
				ГруппаСтрокиЗаголовок.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
				ГруппаСтрокиЗаголовок.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение; 
				
				ГруппаСтрокиЗаголовокКартинка = Группа("ГруппаЗаголовокКартинка" + СтрокаКИ.ИмяРеквизита, Форма, 
					ГруппаСтроки.Заголовок, ИмяЭлементаДляРазмещения, "ГруппаЗаголовок" + СтрокаКИ.ИмяРеквизита);
				...
				Иначе
					ЭлементКартинка = Форма.Элементы.Добавить(ИмяЭлементаКартинка, Тип("ДекорацияФормы"), 
						ГруппаСтрокиЗаголовокКартинка);
				...
				Иначе
				ДекорацияЗаголовок = Форма.Элементы.Добавить(ИмяЭлементаЗаголовок, Тип("ДекорацияФормы"), 
					ГруппаСтрокиЗаголовокКартинка);
	...
	ИначеЕсли ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон
		...
		Если ИнформацияОВидеКИ.ВводитьНомерПоМаске Тогда 
			НомерТелефонаСоответствуетМаске = УправлениеКонтактнойИнформациейСлужебный.НомерТелефонаСоответствуетМаске(
				Форма[ИмяРеквизита], ИнформацияОВидеКИ.МаскаНомераТелефона);	
...
Процедура ДобавитьРеквизитВОписание(Форма, СтрокаКонтактнойИнформации, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ,
	...
	Если ЗаполнятьЗначениеРеквизита И Не ЭтоРеквизитТабличнойЧасти Тогда
		...
		Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(СтрокаКонтактнойИнформации.Тип, ОбработкаНавигационнойСсылки) Тогда
			...
			РеквизитыДляЗаполнения.Вставить("Комментарий" + СтрокаКонтактнойИнформации.ИмяРеквизита, 
				СтрокаКонтактнойИнформации.Комментарий); 
	...
	Если ЭтоНовыйВидКИ И ДанныеВидаКонтактнойИнформации.РазрешитьВводНесколькихЗначений 
		И Не ЭтоРеквизитТабличнойЧасти И Не ДанныеВидаКонтактнойИнформации.ПометкаУдаления Тогда
		...
		Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			ПараметрыКонтактнойИнформации.СписокДобавляемыхЭлементов.Добавить(ДанныеВидаКонтактнойИнформации, 
				Строка(Вид));
		Иначе
			КартинкаТипа = КартинкаТипаКонтактнойИнформации(ДанныеВидаКонтактнойИнформации.Тип);
			ПараметрыКонтактнойИнформации.СписокДобавляемыхЭлементов.Добавить(ДанныеВидаКонтактнойИнформации, 
				Строка(Вид),,КартинкаТипа);
		КонецЕсли;
	КонецЕсли;
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(РезультатЗапроса.ИмяТабличнойЧасти) Тогда
			ИмяТабличнойЧасти = Сред(РезультатЗапроса.ИмяТабличнойЧасти, 
				СтрНайти(РезультатЗапроса.ИмяТабличнойЧасти, ИмяОбъекта) + СтрДлина(ИмяОбъекта));
		Иначе
			ИмяТабличнойЧасти = "";
		КонецЕсли;
...
#Область ЗаполнениеДополнительныхРеквизитовТабличнойЧастиКонтактнаяИнформация
...
Процедура УстановитьНаименованиеВидаКонтактнойИнформации(Знач Объект, Знач Наименование)
	...
	Если ПредопределенныеДанныеОбъекта = Неопределено Или ПустаяСтрока(ИмяКлючевогоРеквизита) Тогда
		ПредопределенныеДанныеОбъекта = ПараметрыЗаполнения.ПредопределенныеДанные.Найти(ЗначениеКлючаОбъекта, 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		ИнформацияОЯзыках = МодульМультиязычностьСервер.СведенияОЯзыках();
		
		Для ПорядковыйНомерЯзыка = 1 По ИнформацияОЯзыках.КоличествоДополнительныхЯзыков Цикл
			
			ПредставлениеНаДополнительномЯзыке = "";
			ИмяСуффиксаЯзыка = МодульМультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
			Если ПредопределенныеДанныеОбъекта <> Неопределено Тогда
				
				Если ЗначениеЗаполнено(ИнформацияОЯзыках[ИмяСуффиксаЯзыка]) Тогда
					ПредставлениеНаДополнительномЯзыке = ПредопределенныеДанныеОбъекта["Наименование" + "_" + ИнформацияОЯзыках[ИмяСуффиксаЯзыка]];
				КонецЕсли;
				
			КонецЕсли;
			
			Если ПустаяСтрока(ПредставлениеНаДополнительномЯзыке) Тогда
				
				ПредставлениеНаДополнительномЯзыке = НаименованияНаРазныхЯзыках.Получить(ИнформацияОЯзыках[ИмяСуффиксаЯзыка]);
				Если ТипЗнч(ПредставлениеНаДополнительномЯзыке) = Тип("Соответствие") Тогда
					ПредставлениеНаДополнительномЯзыке = ПредставлениеНаДополнительномЯзыке.Получить(ИмяПредопределенногоВида);
				КонецЕсли;
				
			КонецЕсли;
			
			Объект["Наименование" + ИмяСуффиксаЯзыка] = ?(ЗначениеЗаполнено(ПредставлениеНаДополнительномЯзыке), 
				ПредставлениеНаДополнительномЯзыке, Объект.Наименование);
	...
	Для Каждого СтрокаКИ Из КонтактнаяИнформация Цикл
		Если ПустаяСтрока(СтрокаКИ.Значение) Тогда
			Если ЗначениеЗаполнено(СтрокаКИ.ЗначенияПолей) Тогда
				...
				КонтактнаяИнформацияПоПолям = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(
					СтрокаКИ.ЗначенияПолей, СтрокаКИ.Тип, НастройкиКонвертации);
...
Процедура СоздатьЭлементыТабличнойЧасти(Знач Форма, Знач ИмяОбъекта, ИмяЭлементаДляРазмещения, 
	...
	Для Каждого СтрокаТабличнойЧасти Из СтрокаКонтактнойИнформации.Строки Цикл
		...
		Если ВидКонтактнойИнформацииТЧ <> ПредыдущийВидТЧ Тогда
			...
			ДобавитьРеквизитВОписание(Форма, СтрокаТабличнойЧасти, ДанныеВидовКонтактнойИнформацииТЧ, Ложь, 
				Истина,, ИмяЭлементаДляРазмещения);
...
Функция ПроверитьЗаполнениеКонтактнойИнформации(Представление, Значение, ВидИнформации, ТипИнформации,
	...
	Если ПустаяСтрока(Значение) Тогда
		...
		Если ВидРедактирования = "Диалог" 
			И СтрСравнить(Представление, УправлениеКонтактнойИнформациейКлиентСервер.ТекстПустогоАдресаВВидеГиперссылки()) = 0 Тогда
			Возврат 0;
		КонецЕсли;
		
		КонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияПоПредставлению(
			Представление, ВидИнформации);
...
Процедура СкорректироватьКонтактнуюИнформацию(Форма, СтрокаКИ)
	...
	Если ПустаяСтрока(СтрокаКИ.Значение) Тогда
		...
		УправлениеКонтактнойИнформациейЛокализация.СкорректироватьКонтактнуюИнформацию(Форма, СтрокаКИ, РезультатПреобразования);
...
Функция ОшибкиЗаполненияАдреса(Источник, ВидИнформации, ИмяРеквизита = "", ПолеРеквизита = "")
	...
	Если Метаданные.Обработки.Найти("РасширенныйВводКонтактнойИнформации") <> Неопределено Тогда
		...
		СписокОшибок = МодульРасширенныйВводКонтактнойИнформации.ОшибкиЗаполненияАдреса(Источник, ВидИнформации);
	...
	Если ДанныеЗаполнения.Свойство("КонтактнаяИнформация", КонтактнаяИнформация) 
		...
		Если КолонкиТаблицы.Найти("ИдентификаторСтрокиТабличнойЧасти") = Неопределено Тогда
			...
			Для Каждого СтрокаКИ Из КонтактнаяИнформация Цикл
				...
				НоваяСтрокаКИ.ЗначенияПолей = КонтактнаяИнформацияВXML(СтрокаКИ.ЗначенияПолей, 
					СтрокаКИ.Представление, СтрокаКИ.Вид);
			КонецЦикла;
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, 
	...
	Если ПустоеЗначение Тогда
		...
		Если Не ПустоеПредставление И ПустыеЗначенияПолей Тогда
			...
			Если ЕстьЗначенияПолей Тогда
				УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнуюИнформациюИзJSONВXML(
					СтрокаКонтактнойИнформации.Значение, СтрокаКонтактнойИнформации.ЗначенияПолей, СтрокаКонтактнойИнформации.Тип);
			КонецЕсли;
		...
		Для Каждого Адрес Из НайденныеСтроки Цикл
			...
			Иначе
				...
				Если СтрокаКИ.МеждународныйФорматАдреса Тогда
					ДанныеАдреса.Адрес = КонтактнаяИнформацияПоПредставлению(ПредставлениеАдреса, Адрес.Тип);
				КонецЕсли;
...
Процедура ПеренестиЗаписьКонтактнойИнформацииИзФормыВТаблицу(КонтактнаяИнформация, СтрокаТаблицы, Знач Значение, 
	...
	УправлениеКонтактнойИнформациейЛокализация.ПриКонвертированииКонтактнуюИнформациюИзJSONВXML(ОбъектКИ, СтрокаКонтактнойИнформации.ЗначенияПолей, СтрокаТаблицы.Тип);
...
Процедура ЗаполнитьСтрокуТаблицыКонтактнойИнформацииПоПредставлению(СтрокаКонтактнойИнформации, Представление)
	СтрокаКонтактнойИнформации.Представление = Представление;
	СтрокаКонтактнойИнформации.Значение      = КонтактнаяИнформацияВJSON(Представление, 
		СтрокаКонтактнойИнформации.Вид);
...
Функция СформироватьТекстЗапроса(Знач ЕстьКолонкаИдентификаторСтрокиТабличнойЧасти, 
	...
	Если СуффиксТекущегоЯзыка <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) Тогда
			МодульМультиязычностьСервер = ОбщегоНазначения.ОбщийМодуль("МультиязычностьСервер");
			МодульМультиязычностьСервер.ИзменитьПолеЗапросаПодТекущийЯзык(ТекстЗапроса, 
				"ВидыКонтактнойИнформации.Наименование КАК Наименование");
		КонецЕсли;
...
Процедура УстановитьДействияДляСтатическихЭлементов(Форма, СтрокаКИ, ИмяЭлементаДляРазмещения)
	...
	ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница И ОбработкаНавигационнойСсылки 
		И Элемент.Вид = ВидПоляФормы.ПолеНадписи Тогда
	...
	Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(Тип, ОбработкаНавигационнойСсылки) Тогда
		...
		Если Форма.Элементы.Найти(ИмяЭлементаКомментарий) <> Неопределено Тогда
			Если Не ЗначениеЗаполнено(Форма.Элементы[ИмяЭлементаКомментарий].ПолучитьДействие("ПриИзменении")) Тогда
				Форма.Элементы[ИмяЭлементаКомментарий].УстановитьДействие("ПриИзменении", 
					"Подключаемый_КонтактнаяИнформацияПриИзменении");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьКонтактнуюИнформациюДляСписковОтложенно", Истина);
