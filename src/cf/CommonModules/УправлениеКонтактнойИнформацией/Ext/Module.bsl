	...
	Если Результат.Свойство("Перечитать") И Результат.Свойство("ИмяЭлементаДляРазмещения") Тогда
		...
		Для Каждого Строка Из ОписаниеКонтактнойИнформации Цикл
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				КонецЕсли;
				Строка.ДействуетС = НайденнаяСтрока.ДействуетС;
				...
				КонецЕсли;
			КонецЕсли;
			Если Строка.ХранитьИсториюИзменений Тогда
				Если ВидыКИСИсторией.Найти(Строка.Вид) = Неопределено Тогда
					ВидыКИСИсторией.Добавить(Строка.Вид);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
