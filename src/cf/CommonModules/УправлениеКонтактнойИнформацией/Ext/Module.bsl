...
Процедура ПередЗаписьюНаСервере(Форма, Объект, Отказ = Ложь) Экспорт
	...
	Если Объект.Ссылка.Пустая() И ТипЗнч(Объект) <> Тип("ДанныеФормыСтруктура") Тогда
		...
		Если ЭтоПараметрыОсновногоОбъекта Тогда
			НоваяСсылка = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Объект.Ссылка).ПолучитьСсылку();
...
Функция УстановитьСвойстваГруппыВидаКонтактнойИнформации(Параметры) Экспорт
	...
	Если ПустаяСтрока(Параметры.Наименование) Тогда
		
		Наименования = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.НаименованияВидовКонтактнойИнформации();
		Для каждого Язык Из Метаданные.Языки Цикл
			Представление = Наименования[Язык.КодЯзыка][СокрЛП(Объект.ИмяПредопределенногоВида)];
...
//                                                    Если Ложь, то в форме вида контактной информации свойства доступны только для просмотра.
//                                                    Значение по умолчанию - Ложь.
//     * РедактированиеТолькоВДиалоге - Булево - Если Истина, то форме выводится гиперссылка с представлением контактной информации 
//                                               по нажатию на которую, открывается форма соответствующего типа КИ.
//                                               Свойство применимо только для контактной информации с типом: Адрес, Телефон, Факс, ВебСтраница.
//                                               Значение по умолчанию - Ложь.
//     * ХранитьИсториюИзменений     - Булево - Признак возможности хранения истории контактной информации.
//                                              Хранение истории допускается, если признак РедактированиеТолькоВДиалоге равен Истина.
//                                              Свойство применимо если табличная часть КонтактнаяИнформация содержит реквизит ДействуетС.
//                                              Значение по умолчанию - Ложь.
...
//                                            Для вида КИ с типом Другое значение по умолчанию - "ОднострочноеШирокое", иначе пустая строка.
//     * НастройкиПроверки - Структура, Неопределено    - Настройки проверки вида контактной информации. 
//                                                        Состав полей зависит от типа контактной информации.
//         Для типа Адрес - Структура, содержащая поля:
//                * ТолькоНациональныйАдрес      - Булево - если Истина, то разрешается ввод только национальных адресов.
//                                                          Изменение страны у адреса не допускается.
//                * ПроверятьКорректность        - Булево - если Истина, то для национальных адресов доступен ввод только адресов с разбивкой 
//                                                          по полям проходящих проверку по ФИАС. Допускается ввод адресов других стран
//                                                          в свободной форме, если свойство ТолькоНациональныйАдрес = Ложь.
//                                                          Значение по умолчанию - Ложь.
//                * ВключатьСтрануВПредставление - Булево - если Истина, то Наименование страны всегда добавляется в представление адреса,
//                                                           даже когда другие поля адреса пустые.
//                                                          Значение по умолчанию - Ложь
...
//                * ТелефонCДобавочнымНомером        - Булево - если Истина, то в форме ввода телефона доступен ввод добавочного номера.
//                                                          Значение по умолчанию - Истина.
	...
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ИначеЕсли СтрНачинаетсяС(ВыборкаДетальныеЗаписи.ИмяПредопределенногоВида, "Документ") Тогда
				ИмяОбъекта = Сред(ВыборкаДетальныеЗаписи.ИмяПредопределенногоВида, 9);
...
Процедура СнятьПризнакПредопределенныйДляВидовКонтактнойИнформации() Экспорт
	...
	Для каждого Вид Из ВидыКонтактнойИнформации Цикл
		...
		ИмяПредопределенногоВида = Сред(Вид.ИмяПредопределенныхДанных, 8);
		...
		Если Не ОбъектВидКИ.ЭтоГруппа Тогда
			ОбъектВидКИ.ИмяГруппы = Сред(Вид.Группа, 8);
		КонецЕсли;
...
//                                              Свойство применимо если табличная часть КонтактнаяИнформация содержит реквизит ДействуетС.
//                                              Значение по умолчанию - Ложь.
//     * ОбязательноеЗаполнение       - Булево - Если Истина, то требуется обязательный ввод значения в поле контактной информации. Значение по умолчанию - Ложь.
//     * РазрешитьВводНесколькихЗначений - Булево - Признак возможности ввода нескольких значений для данного вида. Значение по умолчанию - Ложь.
//     * ЗапретитьРедактированиеПользователем - Булево - Признак недоступности редактирования пользователем вида контактной информации. Значение по умолчанию - Ложь.
//     * МеждународныйФорматАдреса - Булево - Признак международного формата адреса. 
//                                   Если Истина, то ввода всех адресов только в международном формате.
//                                   Значение по умолчанию - Ложь.
//     * ВидПоляДругое             - Строка - Определяет внешний вид поля с типом Другое на форме. Варианты: МногострочноеШирокое, ОднострочноеШирокое, ОднострочноеУзкое.
//                                            Свойство применимо только для контактной информации с типом: Другое.
//                                            Для вида КИ с типом Другое значение по умолчанию - "ОднострочноеШирокое", иначе пустая строка.
//     * НастройкиПроверки - Структура, Неопределено    - Настройки проверки вида контактной информации. Состав полей зависит от типа контактной информации.
//         Для типа Адрес - Структура, содержащая поля:
//                * ТолькоНациональныйАдрес      - Булево - если Истина, то разрешается ввод только национальных адресов. Изменение страны у адреса не допускается.
//                * ПроверятьКорректность        - Булево - если Истина, то для национальных адресов доступен ввод только адресов с разбивкой по полям проходящих проверку по ФИАС.
//                                                          Допускается ввод адресов других стран в свободной форме, если свойство ТолькоНациональныйАдрес = Ложь.
//                                                          Значение по умолчанию - Ложь.
//                * ВключатьСтрануВПредставление - Булево - если Истина, то Наименование страны всегда добавляется в представление адреса, даже когда другие поля адреса пустые.
//                                                          Значение по умолчанию - Ложь
//                * УказыватьОКТМО              - Булево - признак возможности ручного ввода кода ОКМТО в форме ввода адреса.
//            Для типа АдресЭлектроннойПочты - Структура, содержащая поля:
//                * ПроверятьКорректность        - Булево - если Истина, то запрещается пользователю ввод 
