...
#Область ПроцедурыИФункцииПоВыполнениюПроверокВедениюУчетаИПолучениюИхРезультатов
...
Процедура ВыполнитьПроверкиВКонтексте(КонтекстПроверокВеденияУчета) Экспорт
	...
	РезультатВыполнения = ДлительныеОперации.ВыполнитьПроцедуруВНесколькоПотоков(
		"КонтрольВеденияУчета.ВыполнитьПроверку",
		ПараметрыВыполнения,
		ПараметрыМетода);
	
	Если РезультатВыполнения.Статус <> "Выполнено" Тогда
		Если РезультатВыполнения.Статус = "Ошибка" Тогда
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(РезультатВыполнения.ИнформацияОбОшибке);
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, РезультатВыполнения.ИнформацияОбОшибке);
...
Функция КоличествоПроблемПоПравилуПроверки(ПравилоПроверки) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("КонтрольВеденияУчета.КоличествоПроблемПоПравилуПроверки", 
		"ПравилоПроверки", ПравилоПроверки, Тип("СправочникСсылка.ПравилаПроверкиУчета"));
...
Функция ПроблемныеОбъекты(Знач ПравилоПроверки, Знач Параметры = Неопределено) Экспорт
	...
	Если Параметры = Неопределено Тогда
		Параметры = ПараметрыВыборкиПроблемныхОбъектов();
	КонецЕсли;
	Если Параметры.РазмерПорции <> 1000 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "1000", Формат(Параметры.РазмерПорции, "ЧГ=0"));
	КонецЕсли;
	Если ПустаяСтрока(Параметры.ДополнительныеУсловия) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныеУсловия", "Истина");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныеУсловия", Параметры.ДополнительныеУсловия);
		Для каждого ДополнительноеУсловие Из Параметры.ДополнительныеПараметры Цикл
			Запрос.УстановитьПараметр(ДополнительноеУсловие.Ключ, ДополнительноеУсловие.Значение);
		КонецЦикла;
	КонецЕсли;
	
	Если Параметры.НачальныйПроблемныйОбъект = Неопределено Тогда
		Параметры.НачальныйПроблемныйОбъект = "";
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ПравилоПроверки",  ПравилоПроверки);
	Запрос.УстановитьПараметр("ПроблемныйОбъект", Параметры.НачальныйПроблемныйОбъект);
...
Функция ПараметрыВыборкиПроблемныхОбъектов() Экспорт
	...
	Результат.Вставить("НачальныйПроблемныйОбъект", Неопределено);
	Результат.Вставить("РазмерПорции", 1000);
	Результат.Вставить("ДополнительныеУсловия", "");
...
#Область ПроцедурыИФункцииДляРегистрацииПроблемВеденияУчета
