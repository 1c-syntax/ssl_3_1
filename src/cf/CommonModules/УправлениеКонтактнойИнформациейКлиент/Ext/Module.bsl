...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач ПараметрыПисьма = Неопределено,
	...
	Если ТипЗнч(ПараметрыПисьма) = Тип("Строка") Тогда
		Представление = ПараметрыПисьма;
		ПараметрыПисьма = ПараметрыПисьмаSMS();
		ПараметрыПисьма.Представление = Представление;
		ПараметрыПисьма.ОжидаемыйВид = УдалитьОжидаемыйВид;
		ПараметрыПисьма.ИсточникКонтактнойИнформации = УдалитьИсточникКонтактнойИнформации;
		ПараметрыПисьма.ИмяРеквизита = УдалитьИмяРеквизита;
	ИначеЕсли ПараметрыПисьма = Неопределено Тогда
		ПараметрыПисьма = ПараметрыПисьмаSMS();
	КонецЕсли;
	
	АдресПочты = "";
	Если ЗначениеЗаполнено(ПараметрыПисьма.Представление) 
		И ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(ПараметрыПисьма.Представление, Истина) Тогда
	...
	Если ПустаяСтрока(АдресПочты) Тогда
		...
		ОписаниеКонтактнойИнформации = УправлениеКонтактнойИнформациейКлиентСервер.ОписаниеКонтактнойИнформации(
			ЗначенияПолей, ПараметрыПисьма.Представление, ПараметрыПисьма.ОжидаемыйВид);
		...
		ИначеЕсли ТипЗнч(АдресПочты) <> Тип("Строка") Или ТипИнформации = Неопределено Тогда
			ТекстОшибки =  НСтр("ru = 'Невозможно отправить письмо, т.к. значение не является адресом электронной почты.'");
		...
		Иначе
			ПочтовыеАдреса = ОбщегоНазначенияКлиентСервер.АдресаЭлектроннойПочтыИзСтроки(АдресПочты);
			ТекстыОшибок = Новый Массив;
			Для каждого ПочтовыйАдрес Из ПочтовыеАдреса Цикл  
				Если ЗначениеЗаполнено(ПочтовыйАдрес.ОписаниеОшибки) Тогда
					ТекстыОшибок.Добавить(ПочтовыйАдрес.ОписаниеОшибки);
				КонецЕсли;
			КонецЦикла;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			Если ЗначениеЗаполнено(ПараметрыПисьма.ИмяРеквизита) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки,, ПараметрыПисьма.ИмяРеквизита);
			Иначе
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		...
		Получатель.Добавить(Новый Структура("Адрес, Представление, ИсточникКонтактнойИнформации", 
			АдресПочты, СтрЗаменить(Строка(ПараметрыПисьма.ИсточникКонтактнойИнформации), ",", ""), 
				ПараметрыПисьма.ИсточникКонтактнойИнформации));
...
Процедура СоздатьSMS(Знач ЗначенияПолей, Знач ПараметрыSMS = Неопределено,
	...
	Если ТипЗнч(ПараметрыSMS) = Тип("Строка") Тогда
		Представление = ПараметрыSMS;
		ПараметрыSMS = ПараметрыПисьмаSMS();
		ПараметрыSMS.Представление = Представление;
		ПараметрыSMS.ОжидаемыйВид = УдалитьОжидаемыйВид;
		ПараметрыSMS.ИсточникКонтактнойИнформации = УдалитьИсточникКонтактнойИнформации;
	ИначеЕсли ПараметрыSMS = Неопределено Тогда
		ПараметрыSMS = ПараметрыПисьмаSMS();
	КонецЕсли;
	
	НомерПолучателя = "";
	Если ПустаяСтрока(ПараметрыSMS.Представление) Тогда
		
		КонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.ПривестиКонтактнуюИнформациюXML(
			Новый Структура("ЗначенияПолей, Представление, ВидКонтактнойИнформации", 
				ЗначенияПолей, ПараметрыSMS.Представление, ПараметрыSMS.ОжидаемыйВид));
		
		ТекстОшибки = "";
		ТипИнформации = КонтактнаяИнформация.ТипКонтактнойИнформации;
		Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно отправить SMS, т.к. значение не является номером телефона (""%1"").'"), 
				ТипИнформации);
		ИначеЕсли ЗначенияПолей = "" И ПустаяСтрока(ПараметрыSMS.Представление) Тогда
			ТекстОшибки = НСтр("ru = 'Для отправки SMS введите номер телефона.'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			Если ЗначениеЗаполнено(ПараметрыSMS.ИмяРеквизита) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки,, ПараметрыSMS.ИмяРеквизита);
			Иначе
				ПоказатьПредупреждение(, ТекстОшибки );
			КонецЕсли;
			Возврат
	...
	Если ПустаяСтрока(НомерПолучателя) Тогда
		НомерПолучателя = СокрЛП(ПараметрыSMS.Представление);
	КонецЕсли;
...
#КонецЕсли
	...
	СведенияОПолучателе.Вставить("Представление",                Строка(ПараметрыSMS.ИсточникКонтактнойИнформации));
	СведенияОПолучателе.Вставить("ИсточникКонтактнойИнформации", ПараметрыSMS.ИсточникКонтактнойИнформации);
...
Функция ПараметрыПисьмаSMS() Экспорт
	...
	Результат.Вставить("ОжидаемыйВид", Неопределено); 
...
Процедура ПерейтиПоВебСсылке(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	ТипВебСтраница = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.ВебСтраница");
	Если ОжидаемыйВид = Неопределено Тогда
		ОжидаемыйВид = ТипВебСтраница;
	КонецЕсли;
	
	Если ПустаяСтрока(ЗначенияПолей) И Не ПустаяСтрока(Представление) И ОжидаемыйВид = ТипВебСтраница Тогда
		АдресСсылки = Представление;
	Иначе
		КонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.ПривестиКонтактнуюИнформациюXML(
			Новый Структура("ЗначенияПолей, Представление, ВидКонтактнойИнформации", ЗначенияПолей, Представление,
			ОжидаемыйВид));
		ТипИнформации = КонтактнаяИнформация.ТипКонтактнойИнформации;
		...
		Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.ВебСтраница") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Нельзя открыть ссылку по контактной информации с типом ""%1""'"), ТипИнформации);
		КонецЕсли;
	...
	Если ТипЗнч(АдресСсылки) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ru = 'Невозможно перейти по ссылке, неверный тип контактной информации.'");
	КонецЕсли;
...
Процедура ПоказатьАдресНаКарте(Адрес, ИмяКартографическогоСервиса) Экспорт
	...
	Иначе
		Ресурс = "";
		УправлениеКонтактнойИнформациейКлиентЛокализация.ПриОпределенииURLКартографическогоСервиса(ИмяКартографическогоСервиса, Ресурс);
		Если ЗначениеЗаполнено(Ресурс) Тогда
			СтрокаЗапуска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Ресурс, АдресКодированный)
		КонецЕсли;
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.Пользователи.Форма.ФормаЭлемента" Тогда
		Оповещение = Новый ОписаниеОповещения("КонтактнаяИнформацияНачалоВыбораЗавершение",
			ДополнительныеПараметры.Форма);
		ТекущийРезультат = Новый Структура;
		ТекущийРезультат.Вставить("Элемент", Элемент);
		ТекущийРезультат.Вставить("НовоеЗначение", ТекстПредставления);
		ТекущийРезультат.Вставить("Отказ", Ложь);
		ВыполнитьОбработкуОповещения(Оповещение, ТекущийРезультат);
		Если ТекущийРезультат.Отказ Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
//     * Вид           - СправочникСсылка.ВидыКонтактнойИнформации
//
Функция ПараметрКонтактнаяИнформацияДляВыполненияКоманд(Представление, Значение, Тип, Вид) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(ИмяКоманды, "МенюПодменюАдрес") И ТипКонтактнойИнформации = ПредопределенноеЗначение(
		...
		Если СтрокаПотребитель.Свойство("МеждународныйФорматАдреса") И СтрокаПотребитель.МеждународныйФорматАдреса Тогда
			...
			СтрокаПотребитель.Значение = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияПоПредставлению(
				ПредставлениеАдреса, ТипКонтактнойИнформации, Комментарий);
...
Процедура ПриИзмененииКонтактнойИнформации(Форма, Элемент, ЭтоТабличнаяЧасть, ОбновитьФорму, АсинхронныйВызов)
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.Маска) И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяЭлемента) Тогда
		Текст = Форма[ИмяЭлемента];
	Иначе
	...
	ДанныеСтроки.Значение = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияПоПредставлению(Текст, ДанныеСтроки.Вид, ДанныеСтроки.Комментарий);
...
#Если НЕ ВебКлиент Тогда
		Если ПустаяСтрока(ПрограммаТелефонииУстановлена("skype")) Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Для совершения звонка по Skype установите приложение.'"));
			Возврат;
		КонецЕсли;
#КонецЕсли
...
//
Функция ДополнительныеПараметрыДляВыполненияКоманд(ВладелецКонтактнойИнформации, Форма, ИмяРеквизита = "", АсинхронныйВызов = Ложь) Экспорт
...
Процедура ПередСозданиемSMS(КонтактнаяИнформация, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	ПараметрыSMS = ПараметрыПисьмаSMS();
	ПараметрыSMS.Представление = КонтактнаяИнформация.Представление;
	ПараметрыSMS.ОжидаемыйВид = КонтактнаяИнформация.Тип;
	ПараметрыSMS.ИсточникКонтактнойИнформации = ДополнительныеПараметры.ВладелецКонтактнойИнформации;
	СоздатьSMS("", ПараметрыSMS);
...
Процедура ПередСозданиемЭлектронногоПисьма(КонтактнаяИнформация, ДополнительныеПараметры) Экспорт
	...
	СоздатьЭлектронноеПисьмо("", ПараметрыПисьма);

КонецПроцедуры

Процедура ПоказатьАдресНаКартеGoogle(КонтактнаяИнформация, ДополнительныеПараметры) Экспорт
	
	ПоказатьАдресНаКарте(КонтактнаяИнформация.Представление, "GoogleMaps");
...
Процедура ПередВыполнениемКомандыИзРасширеннойПодсказкиАдреса(Форма, Элемент, ИмяРеквизита, ИмяКоманды, АсинхронныйВызов)
	...
	Если СтрСравнить(ИмяКоманды, "ПоказатьНаКарте") = 0 Тогда
		Список = Новый СписокЗначений;
		
		Для каждого КомандаДляВывода Из КомандыДляВывода Цикл
				
				ВидДействия = УправлениеКонтактнойИнформациейКлиентСервер.ВидДействияКомандыТипаКонтактнойИнформации(
					КомандаДляВывода.Значение.Действие);
				
				Если ПустаяСтрока(ВидДействия) Тогда
					УправлениеКонтактнойИнформациейКлиентСерверЛокализация.ПриОпределенияДействияКомандыТипаКонтактнойИнформации(
						КомандаДляВывода.Значение.Действие, ВидДействия);
				КонецЕсли;
					
				Если СтрСравнить(ВидДействия, "ПоказатьНаКарте") = 0 Тогда
					Список.Добавить(КомандаДляВывода.Значение.Действие, КомандаДляВывода.Значение.Заголовок,,
						КомандаДляВывода.Значение.Картинка);
				КонецЕсли;
		КонецЦикла;
		
		Если Список.Количество() = 1 Тогда
			ВыполнитьКомандуКонтактнойИнформации(Список.Получить(0).Значение, Параметры);
			Возврат;
		КонецЕсли;
