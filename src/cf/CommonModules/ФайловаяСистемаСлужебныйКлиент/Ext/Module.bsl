...
Процедура ПоказатьПомещениеФайлаПриПодключенииРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
	...
	Исключение
		...
		Если СтрНайти(ОписаниеОшибки, "32(0x00000020)") Тогда 
			ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Завершите работу с файлом в другом приложении.'"),, 
				НСтр("ru = 'Файл открыт в другом приложении'"));
				Возврат;
		КонецЕсли;
...
Процедура ПоказатьПолучениеФайловПриПодключенииРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
...
#Если МобильныйКлиент Тогда
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораКаталог", ЭтотОбъект, Контекст);
...
Процедура ПослеВыбораКаталог(ПолученныйКаталог, Контекст) Экспорт
	
	Если ПолученныйКаталог = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИмяКаталога = ПолученныйКаталог.Получить(0);
	...
	НачатьПолучениеФайловССервера(ОповещениеОЗавершении, Контекст.ПолучаемыеФайлы, ИмяКаталога, ПараметрыПолученияАрхиваФайлов);
...
Процедура ОткрытьФайлВПрограммеПросмотра(ПутьКФайлу, Знач Оповещение = Неопределено,
	...
	Оповещение = Новый ОписаниеОповещения(
		"ОткрытьФайлВПрограммеПросмотраПослеПроверкиРасширения1СПредприятия", ЭтотОбъект, Контекст);
...
// Продолжение процедуры ФайловаяСистемаКлиент.ОткрытьФайл.
Процедура ОткрытьФайлВПрограммеПросмотраПослеПроверкиРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
...
Процедура ОткрытьПроводникПослеПроверкиРасширенияРаботы1СПредприятия(РасширениеПодключено, Контекст) Экспорт
...
Процедура ОткрытьНавигационнуюСсылкуПослеПроверкиРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
...
Процедура ЗапуститьПрограммуПослеПроверкиРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Не установлено расширение для работы с 1С:Предприятием, запуск приложения невозможен.'");
...
// Возвращает кодировку стандартных поток вывода и ошибок, используемую в клиентской ОС.
//
// Возвращаемое значение:
//  КодировкаТекста
//
Функция КодировкаСтандартныхПотоков()
	...
	Возврат ?(ОбщегоНазначенияКлиент.ЭтоWindowsКлиент(), "CP866", "UTF-8");
...
Процедура ЗапуститьПрограммуПослеЗапускаПриложения(КодВозврата, Контекст) Экспорт 
	...
	Если Контекст.ДождатьсяЗавершения И КодВозврата = Неопределено Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Контекст.СтрокаКоманды,
			Контекст.ТекущийКаталог,
			КодВозврата,
			Контекст.ДождатьсяЗавершения);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Стандартные подсистемы'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Ошибка", ОписаниеОшибки);
	Иначе
		Результат.ПриложениеЗапущено = Истина;
	КонецЕсли;
	
	Результат.КодВозврата = ?(КодВозврата <> Неопределено, КодВозврата, -1);
...
#Если ВебКлиент Тогда
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось запустить приложение
		           |%1
		           |по причине:
		           |Запуск приложений с повышением привилегий недоступен в веб-клиенте.'"),
		Контекст.СтрокаКоманды);
	ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
#ИначеЕсли МобильныйКлиент Тогда
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось запустить приложение
		           |%1
		           |по причине:
		           |Запуск приложений с повышением привилегий недоступен в мобильном клиенте.'"),
		Контекст.СтрокаКоманды);
	ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
#Иначе
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось запустить приложение
			           |%1
			           |по причине:
			           |Запуск приложений с повышением привилегий доступен только в Windows и Linux.'"),
			Контекст.СтрокаКоманды);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура ЗапуститьПрограммуСНаивысшимиПравамиWindows(Контекст)
	...
	СтрокаКоманды = ОбщегоНазначенияСлужебныйКлиентСервер.СтрокаЗапускаКомандыWindows(СтрокаКоманды, 
		ТекущийКаталог, ДождатьсяЗавершения, КодировкаИсполнения);
...
Процедура ВыбратьКаталогПриПодключенииРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
...
//
Процедура ПоказатьДиалогВыбораПриПодключенииРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
...
#Область РасширениеДляРаботы1СПредприятие

Процедура НачатьПодключениеРасширения1СПредприятия(Подключено, Контекст) Экспорт
	...
	// В веб-клиенте в macOS расширение доступно только в браузере Chrome.
	Если ОбщегоНазначенияКлиент.ЭтоMacOSКлиент() 
			И НЕ ДоступноРасширение1СПредприятия() Тогда
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
		ПараметрыПриложения.Вставить(ИмяПараметра, ПредлагатьУстановкуРасширения1СПредприятия());
	КонецЕсли;
	
	ПредлагатьУстановкуРасширения = ПараметрыПриложения[ИмяПараметра] Или ПервоеОбращениеЗаСеанс;
	Если Контекст.ВозможноПродолжениеБезУстановки И Не ПредлагатьУстановкуРасширения Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОписаниеОповещенияЗавершение);
		Возврат;
	КонецЕсли;
	...
	ОткрытьФорму("ОбщаяФорма.ВопросОбУстановкеРасширения1СПредприятия", ПараметрыФормы,,,,,  
		Контекст.ОписаниеОповещенияЗавершение);
		
КонецПроцедуры

Процедура НачатьПодключениеРасширения1СПредприятияПриОтветеНаВопросОбУстановке(Действие, ОповещениеОЗакрытии) Экспорт
...
Функция ПредлагатьУстановкуРасширения1СПредприятия()
	
	СистемнаяИнформация = Новый СистемнаяИнформация();
	ИдентификаторКлиента = СистемнаяИнформация.ИдентификаторКлиента;
	Возврат ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиПрограммы/ПредлагатьУстановкуРасширенияРаботыСФайлами", ИдентификаторКлиента, Истина);
	
КонецФункции

Функция ДоступноРасширение1СПредприятия()
...
//   * Расширение - Строка
//
Процедура СоздатьВременныйКаталогПослеПроверкиРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		СоздатьВременныйКаталогОповеститьОбОшибке(НСтр("ru = 'Не удалось установить расширение для работы с 1С:Предприятием.'"), 
			Контекст);
	КонецЕсли;
...
Процедура СоздатьВременныйКаталогПослеПолученияВременногоКаталога(ИмяКаталогаВременныхФайлов, Контекст) Экспорт 
	...
	НачатьСозданиеКаталога(Оповещение,
		ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ИмяКаталогаВременныхФайлов) + ИмяКаталога);
