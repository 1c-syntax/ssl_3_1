...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	Если Параметры.РежимОбновления = 0 Тогда
		УстановитьОбновлениеСейчасНачало(Форма, Параметры, ПараметрыАдминистрирования);
		Возврат;
	КонецЕсли;
	...
	Если Параметры.РежимОбновления = 1 Тогда // При завершении работы
	...
	Если ПустаяСтрока(КаталогСкрипта) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			"Предупреждение", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Журнал обновления см. в каталоге временных файлов %1.(цифры).'"), "%temp%\1Cv8Update"),, Истина);
...
Процедура ПередНачаломРаботыСистемы(Параметры) Экспорт
	Если СтрНайти(ПараметрЗапуска, "ОтключитьИсправленияИЗавершитьРаботу") > 0 Тогда
		ОбновлениеКонфигурацииВызовСервера.ОтключитьИсправленияИзСкрипта();
		ПрекратитьРаботуСистемы();
	КонецЕсли;
КонецПроцедуры
...
Функция ВерсииТребующиеУспешногоОбновления(ПереданныеФайлыОбновления)
	ФайлыОбновления = Новый Массив;
	Для Каждого ФайлОбновления Из ПереданныеФайлыОбновления Цикл
		Сведения = Новый Структура;
		Сведения.Вставить("ДвоичныеДанные", Новый ДвоичныеДанные(ФайлОбновления.ПолноеИмяФайлаОбновления));
		Сведения.Вставить("Обязательная", ФайлОбновления.ВыполнитьОбработчикиОбновления);
		ФайлыОбновления.Добавить(Сведения);
	КонецЦикла;
	
	Возврат ОбновлениеКонфигурацииВызовСервера.ВерсииТребующиеУспешногоОбновления(ФайлыОбновления);
КонецФункции
...
Функция ИменаФайловОбновления(Параметры, ТолькоПервыйФайл = Ложь)
	...
	Если ПараметрыПриложения.Получить(ИмяПараметра) <> Неопределено Тогда
		...
		ИначеЕсли ТолькоПервыйФайл = Ложь Тогда
			Если ПараметрыПриложения[ИмяПараметра].Свойство("ФайлыОбновления") Тогда
				Параметры.Вставить("ФайлыОбновления", ПараметрыПриложения[ИмяПараметра].ФайлыОбновления);
			КонецЕсли;
	...
	Результат = Новый Структура;
	Результат.Вставить("ИмяПервогоФайла", ИмяПервогоФайла);
	Результат.Вставить("ИменаФайлов", ИменаФайловОбновления);
	Если Параметры.Свойство("ФайлыОбновления") Тогда
		Результат.Вставить("ФайлыОбновления", Параметры.ФайлыОбновления);
	КонецЕсли;
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	ИнтервалУведомленияПользователей = 0;
	Если Параметры.Свойство("ИнтервалУведомленияПользователей") Тогда
		ИнтервалУведомленияПользователей = Параметры.ИнтервалУведомленияПользователей;
	КонецЕсли;
	...
	ВставитьПараметрСкрипта("ИнтервалУведомленияПользователей"  , ИнтервалУведомленияПользователей           , Ложь,   ОбластьПараметров);
	...
	Если Параметры.Свойство("ФайлыОбновления") Тогда
		Параметры.Вставить("ВерсииТребующиеУспешногоОбновления", ВерсииТребующиеУспешногоОбновления(Параметры.ФайлыОбновления));
	Иначе
		Параметры.Вставить("ВерсииТребующиеУспешногоОбновления", Новый Массив);
	КонецЕсли;
...
Функция ИмяПараметраОбновитьСейчас()
	
	Возврат "СтандартныеПодсистемы.ОбновлениеКонфигурации.ОбновитьСейчас";
	
КонецФункции

Функция ИмяПараметраПропуститьЗавершениеРаботы()
	
	Возврат "СтандартныеПодсистемы.ПропуститьЗавершениеРаботыСистемыПослеОбработкиПредупреждений";
	
КонецФункции

// Возвращаемое значение:
//  Структура:
//   * Форма - см. УстановитьОбновление.Форма
//   * Параметры - см. УстановитьОбновление.Параметры
//   * ПараметрыАдминистрирования - см. УстановитьОбновление.ПараметрыАдминистрирования
//
Функция НовыеПараметрыОбновитьСейчас(Форма, Параметры, ПараметрыАдминистрирования)
	
	Результат = Новый Структура;
	Результат.Вставить("Форма", Форма);
	Результат.Вставить("Параметры", Параметры);
	Результат.Вставить("ПараметрыАдминистрирования", ПараметрыАдминистрирования);
	
	Возврат Результат;
	
КонецФункции

Процедура УстановитьОбновлениеСейчасНачало(Форма, Параметры, ПараметрыАдминистрирования)
	
	ПараметрыПриложения.Вставить(ИмяПараметраОбновитьСейчас(),
		НовыеПараметрыОбновитьСейчас(Форма, Параметры, ПараметрыАдминистрирования));
	
	ПодключитьОбработчикОжидания("ОбработчикОжиданияУстановитьОбновлениеСейчас", 0.1, Истина);
	ПараметрыПриложения.Вставить(ИмяПараметраПропуститьЗавершениеРаботы(), Истина);
	
КонецПроцедуры

Процедура ОбработчикОжиданияУстановитьОбновлениеСейчасПродолжение() Экспорт
	
	ПараметрыПриложения.Удалить(ИмяПараметраПропуститьЗавершениеРаботы());
	
	ПараметрыОбновитьСейчас = ПараметрыПриложения[ИмяПараметраОбновитьСейчас()]; // см. НовыеПараметрыОбновитьСейчас
	Если ПараметрыОбновитьСейчас = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Форма = ПараметрыОбновитьСейчас.Форма;
	...
	ПараметрыПриложения.Удалить(ИмяПараметраОбновитьСейчас());
	ОтключитьОбработчикОжидания("ОбработчикОжиданияУстановитьОбновлениеСейчас");
	
	ЗапуститьСкриптОбновления(ПараметрыОбновитьСейчас.Параметры,
		ПараметрыОбновитьСейчас.ПараметрыАдминистрирования);
...
Процедура ЗапуститьСкриптОбновленияЗавершение(Параметры, ПараметрыАдминистрирования)
	...
	ВерсииТребующиеУспешногоОбновления = Новый Массив;
	Если Параметры.Свойство("ВерсииТребующиеУспешногоОбновления") Тогда
		ВерсииТребующиеУспешногоОбновления = Параметры.ВерсииТребующиеУспешногоОбновления;
	КонецЕсли;
	
	ПараметрыОбновления = ОбновлениеКонфигурацииВызовСервера.ПараметрыОбновления();
	ПараметрыОбновления.ИмяАдминистратораОбновления = ИмяПользователя();
	ПараметрыОбновления.ОбновлениеЗапланировано = Истина;
	ПараметрыОбновления.ОбновлениеВыполнено = Ложь;
	ПараметрыОбновления.РезультатОбновленияКонфигурации = Ложь;
	ПараметрыОбновления.ИмяГлавногоФайлаСкрипта = ИмяГлавногоФайлаСкрипта;
	ПараметрыОбновления.ВерсииТребующиеУспешногоОбновления = ВерсииТребующиеУспешногоОбновления;
	ОбновлениеКонфигурацииВызовСервера.ЗаписатьСтатусОбновления(ПараметрыОбновления,
