...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ИспользоватьНапоминания = НастройкиНапоминаний().ИспользоватьНапоминания;
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.3.3.11", 
		"Роль.ИспользованиеНапоминаний", "Роль.ДобавлениеИзменениеНапоминаний", "СтандартныеПодсистемы");
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если ПравоДоступа("Редактирование", Метаданные.РегистрыСведений.НапоминанияПользователя) Тогда
		Команда = Команды.Добавить();
		Команда.Вид = "Органайзер";
		Команда.Представление = НСтр("ru = 'Напомнить...'");
		Команда.ФункциональныеОпции = "ИспользоватьНапоминанияПользователя";
		Команда.Картинка = БиблиотекаКартинок.Напоминание;
		Команда.ТипПараметра = Метаданные.ОпределяемыеТипы.ПредметНапоминания.Тип;
		Команда.РежимЗаписи = "НеЗаписывать";
		Команда.Порядок = 50;
		Команда.Обработчик = "НапоминанияПользователяСлужебныйКлиент.Напомнить"; 
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Параметры.Вставить("ИспользуютсяНапоминанияПользователя", НапоминанияПользователя.ИспользуютсяНапоминанияПользователя());
	Параметры.Вставить("ОтображатьНапоминанияВЦентреОповещений", ОтображатьНапоминанияВЦентреОповещений());
...
Процедура ИзменитьТекстНапоминанийПоПредмету(Предмет, Идентификатор, НовыйТекст) Экспорт
	...
	ЭлементБлокировки.УстановитьЗначение("Источник", Предмет);
...
Функция НастройкиПодсистемы() Экспорт
	...
	Настройки.Вставить("Расписания", СтандартныеРасписанияДляНапоминания());
...
Функция СтандартныеРасписанияДляНапоминания()
...
//   ПредыдущаяДата - Дата 
//   ИскатьТолькоБудущиеДаты - Булево
// 
// Возвращаемое значение:
//   Неопределено, Дата
//
Функция БлижайшаяДатаСобытияПоРасписанию(Расписание, ПредыдущаяДата = '000101010000', ИскатьТолькоБудущиеДаты = Истина) Экспорт
	...
	Календарь = КалендарьНаБудущийПериод(365*4+1, ДатаОтсчета, Расписание.ДатаНачала, Расписание.ПериодПовтораДней, Расписание.ПериодНедель);
	...
	Если Выборка.Следующий() Тогда
		...
		БлижайшееВремя = БлижайшееВремяИзРасписания(Расписание, ВремяОтсчета);
		Если БлижайшееВремя <> Неопределено Тогда
			Результат = БлижайшаяДата + (БлижайшееВремя - '00010101');
		Иначе
			Если Выборка.Следующий() Тогда
				Время = БлижайшееВремяИзРасписания(Расписание);
...
Функция КалендарьНаБудущийПериод(КоличествоДнейКалендаря, ДатаОтсчета, Знач ДатаНачалаПериодичности = Неопределено, 
...
Функция БлижайшееВремяИзРасписания(Расписание, Знач ВремяОтсчета = '000101010000')
...
Функция ГрафикСобытия(Расписание, НачалоПериода, КонецПериода)
	...
	Календарь = КалендарьНаБудущийПериод((НачалоДня(КонецПериода) - НачалоДня(НачалоПериода)) / (60*60*24) + 1, 
	...
	Иначе
		...
		Попытка
			...
			Если Запрос.Выполнить().Пустой() Тогда
				// Подключаем новое напоминание.
				Запись = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(Запись, ПараметрыНапоминания);
				НаборЗаписей.Записать();
			Иначе
				// Удаляем заменяемое напоминание.
				НаборЗаписей.Прочитать();
				НаборЗаписей.Очистить();
				НаборЗаписей.Записать();
				
				// Добавляем напоминание с новым временем события.
				НовоеВремяСобытия = НовоеВремяСобытия(
					ПараметрыНапоминания.Пользователь, ПараметрыНапоминания.Источник);
				
				НаборЗаписей.Отбор.ВремяСобытия.Установить(НовоеВремяСобытия);
				Запись = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(Запись, ПараметрыНапоминания);
				Запись.ВремяСобытия = НовоеВремяСобытия;
				НаборЗаписей.Записать();
			КонецЕсли;
			ЗафиксироватьТранзакцию();
		Исключение
		...
		Если ПодключатьПоРасписанию И Напоминание <> Неопределено Тогда
			...
			Если Расписание <> Неопределено Тогда
				Если Расписание.ПериодПовтораДней > 0 Тогда
					СледующаяДатаПоРасписанию = БлижайшаяДатаСобытияПоРасписанию(Расписание, ПараметрыНапоминания.ВремяСобытия + 1);
				КонецЕсли;
...
Функция ПодключитьПроизвольноеНапоминание(Текст, ВремяСобытия, ИнтервалДоСобытия = 0, Предмет = Неопределено, Идентификатор = Неопределено) Экспорт
	...
	ПараметрыНапоминания.Вставить("НавигационнаяСсылка", НавигационнаяСсылкаНапоминания(ПараметрыНапоминания));
...
Функция СоздатьНапоминание(ПараметрыНапоминания)
	...
	Если Напоминание.Расписание <> Неопределено Тогда
		Напоминание.ВремяСобытия = БлижайшаяДатаСобытияПоРасписанию(Напоминание.Расписание);
...
Функция СписокТекущихНапоминанийПользователя() Экспорт
	...
	Напоминания.Колонки.Добавить("НавигационнаяСсылка", Новый ОписаниеТипов("Строка"));
	
	Для Каждого Напоминание Из Напоминания Цикл
		Напоминание.НавигационнаяСсылка = НавигационнаяСсылкаНапоминания(Напоминание);
	КонецЦикла;
	
	Результат = ОбщегоНазначения.ТаблицаЗначенийВМассив(Напоминания);
...
Функция НавигационнаяСсылкаНапоминания(Напоминание) Экспорт
	Если НапоминанияПользователяКлиентСервер.ЭтоНавигационнаяСсылкаСообщения(Напоминание.Идентификатор) Тогда
		НавигационнаяСсылка = Напоминание.Идентификатор;
	ИначеЕсли ЗначениеЗаполнено(Напоминание.Источник) Тогда
		НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(Напоминание.Источник);
	Иначе
		КлючЗаписи = Новый Структура("Пользователь, ВремяСобытия, Источник");
		ЗаполнитьЗначенияСвойств(КлючЗаписи, Напоминание);
		КлючНапоминания = РегистрыСведений.НапоминанияПользователя.СоздатьКлючЗаписи(КлючЗаписи);
		НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(КлючНапоминания);
	КонецЕсли;
	Возврат НавигационнаяСсылка;
КонецФункции

Функция ОтображатьНапоминанияВЦентреОповещений() Экспорт
	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ОбщиеНастройкиПользователя", 
		"ОтображатьНапоминанияВЦентреОповещений", Ложь);
КонецФункции

Функция НовоеВремяСобытия(Пользователь, Источник)
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДОБАВИТЬКДАТЕ(МИНИМУМ(ДатыСлева.ВремяСобытия), СЕКУНДА, 1) КАК НовоеВремяСобытия
		|ИЗ
		|	Даты КАК ДатыСлева
		|		ЛЕВОЕ СОЕДИНЕНИЕ Даты КАК ДатыСправа
		|		ПО (ДОБАВИТЬКДАТЕ(ДатыСлева.ВремяСобытия, СЕКУНДА, 1) = ДатыСправа.ВремяСобытия)
		|ГДЕ
		|	ДатыСправа.ВремяСобытия ЕСТЬ NULL";
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	НовоеВремяСобытия = Выборка.НовоеВремяСобытия;
	
	Возврат НовоеВремяСобытия;
