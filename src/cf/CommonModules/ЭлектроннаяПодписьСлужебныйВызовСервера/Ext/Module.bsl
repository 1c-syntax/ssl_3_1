		...
		Если Параметры.Операция = "Подписание" Тогда
			...
			Попытка
				Если ЭлектроннаяПодпись.ДоступнаУсовершенствованнаяПодпись() Тогда
					ДвоичныеДанныеРезультата = МенеджерКриптографии.Подписать(Данные, СертификатКриптографии, Параметры.ТипПодписи);
				Иначе
...
// Только для внутреннего использования.
Функция УсовершенствоватьНаСторонеСервера(Знач Параметры, АдресРезультата, ОперацияНачалась, ОшибкаНаСервере) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОшибкаНаСервере.Текст = ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных("ПродлениеСрокаДействияПодписи")
			+ Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		Возврат Ложь;
	КонецПопытки;
	
	Параметры.Вставить("АлгоритмПодписи", ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмСформированнойПодписи(Подпись));
	
	ПараметрыСоздания.АлгоритмПодписи = Параметры.АлгоритмПодписи;
	МенеджерКриптографии = ЭлектроннаяПодписьСлужебный.МенеджерКриптографии("ПродлениеСрокаДействияПодписи", ПараметрыСоздания);
	
	Если МенеджерКриптографии = Неопределено Тогда
		ОшибкаНаСервере.Текст = ПараметрыСоздания.ОписаниеОшибки;
		ОшибкаНаСервере.ОшибкаПриСозданииМенеджераКриптографии = Истина;
		Возврат Ложь;
	КонецЕсли;
	
	Если Параметры.Свойство("ПодписанныйОбъект") Тогда
		Результат = ЭлектроннаяПодпись.УсовершенствоватьПодпись(Подпись,
			Параметры.ТипПодписи,
			Параметры.ДобавитьАрхивнуюМеткуВремени, МенеджерКриптографии);
	Иначе
		Результат = ЭлектроннаяПодпись.УсовершенствоватьПодписьОбъекта(Параметры.ЭлементДанныхДляСервера.ПодписанныйОбъект,
			Параметры.ЭлементДанныхДляСервера.ПорядковыйНомер,
			Подпись,
			Параметры.ТипПодписи,
			Параметры.ДобавитьАрхивнуюМеткуВремени, МенеджерКриптографии);
	КонецЕсли;
	...
	ОперацияНачалась = Истина; 
	Параметры.Вставить("СвойстваПодписи", Результат.СвойстваПодписи);
	
	АдресРезультата = ПоместитьВоВременноеХранилище(Параметры, Параметры.ИдентификаторФормы);
...
// Только для внутреннего использования.
Процедура ЗарегистрироватьУсовершенствованиеПодписиВЖурнале(ЭлементДанных) Экспорт
	
	ЭлектроннаяПодписьСлужебный.ЗарегистрироватьУсовершенствованиеПодписиВЖурнале(ЭлементДанных);
