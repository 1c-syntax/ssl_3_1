...
Функция ДиагностикаСоединения(URL, ЗаписыватьОшибку = Истина, ПроверятьДоставкуПакетов = Истина, ТекстОшибки = "") Экспорт
	...
	Если Не ПустаяСтрока(ТекстОшибки)
		...
		Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(
			"ru = 'Проверьте путь сертификации сертификата к %1 на компьютере <%2>. При необходимости предоставьте доступ к сведениям центра сертификации или установите корневой и промежуточные сертификаты. Если сертификат самоподписанный, добавьте его в доверенные.'"),
			СтруктураСсылки.ИмяСервера, ИмяКомпьютера()));
	...
	Иначе
		...
		ПолучениеФайловИзИнтернетаЛокализация.ПриПолученииАдресаСервераКонтрольнойПроверки(АдресСервераКонтрольнойПроверки);
