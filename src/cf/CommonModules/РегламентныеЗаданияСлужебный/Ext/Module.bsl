...
Функция ВыполнитьРегламентноеЗаданиеВручную(ИсточникЗадания) Экспорт
	...
	Результат = ПараметрыВыполненияРегламентногоЗадания();
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		Результат.ПроцедураУжеВыполняется = Истина;
	...
	Иначе
		...
		ПараметрыВыполненияЗадания.ПоискАктивного = Истина;
		
		ДатаВЧасовомПоясеСервера = ТекущаяДата(); // АПК:143 для сравнения с датой запуска фонового задания
		...
		Результат.ПроцедураУжеВыполняется = Результат.МоментЗапуска < ДатаВЧасовомПоясеСервера;
		Результат.ПредставлениеФоновогоЗадания = ФоновоеЗадание.Наименование;
	КонецЕсли;
	
	Результат.ЗапускВыполнен = НЕ Результат.ПроцедураУжеВыполняется;
