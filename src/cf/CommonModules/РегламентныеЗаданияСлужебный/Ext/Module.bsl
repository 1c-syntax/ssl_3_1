...
//        По умолчанию - Ложь.
//    * ОбращаетсяКВнешнимРесурсам - Булево - Истина, если регламентное задание работает
//        с Интернет-сервисами, но у него не указано свойство РаботаетСВнешнимиРесурсами = Истина.
//        По умолчанию = РаботаетСВнешнимиРесурсами.
//
Функция РегламентныеЗаданияЗависимыеОтФункциональныхОпций() Экспорт
	...
	Зависимости.Колонки.Добавить("ОбращаетсяКВнешнимРесурсам", Новый ОписаниеТипов("Булево"));
...
Функция СвойстваФоновыхЗаданий(Отбор = Неопределено) Экспорт
	...
	ПолучитьПоследнее = ЗначениеЗаполнено(Отбор) И Отбор.Свойство("ПолучитьПоследнееФоновоеЗаданиеРегламентногоЗадания");
...
// 
// Параметры:
//  ИсточникЗадания     - РегламентноеЗадание
//                      - Строка - уникального идентификатора РегламентногоЗадания.
//
// Возвращаемое значение:
//  см. НовыеПараметрыВыполненияРегламентногоЗадания
//
Функция ВыполнитьРегламентноеЗаданиеВручную(ИсточникЗадания) Экспорт
	ВызватьИсключениеЕслиНетПраваАдминистрирования();
	УстановитьПривилегированныйРежим(Истина);
	
	Результат = НовыеПараметрыВыполненияРегламентногоЗадания();
	
	Задание = РегламентныеЗаданияСервер.ПолучитьРегламентноеЗадание(ИсточникЗадания);
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		Результат.МоментЗапуска  = СвойстваПоследнегоФоновогоЗадания.Начало;
		Если ЗначениеЗаполнено(СвойстваПоследнегоФоновогоЗадания.Наименование) Тогда
			Результат.ПредставлениеФоновогоЗадания = СвойстваПоследнегоФоновогоЗадания.Наименование;
		Иначе
			Результат.ПредставлениеФоновогоЗадания = ПредставлениеРегламентногоЗадания(Задание);
		КонецЕсли;
	Иначе
		ПараметрыВыполненияЗадания = РегламентныеЗаданияСервер.НовыеПараметрыВыполненияЗадания();
		ПараметрыВыполненияЗадания.Ключ = Строка(Задание.УникальныйИдентификатор);
		ПараметрыВыполненияЗадания.Наименование = ПредставлениеРегламентногоЗадания(Задание);
		ФоновоеЗадание = РегламентныеЗаданияСервер.ВыполнитьЗадание(Задание, ПараметрыВыполненияЗадания);
		Результат.ИдентификаторФоновогоЗадания = Строка(ФоновоеЗадание.УникальныйИдентификатор);
		Результат.МоментЗапуска = ФоновоеЗадание.Начало;
		Результат.ЗапускВыполнен = Истина;
	КонецЕсли;
	
	Результат.ПроцедураУжеВыполняется = НЕ Результат.ЗапускВыполнен;
	Возврат Результат;
...
Процедура ПриОпределенииИспользованияРегламентныхЗаданий(ТаблицаИспользования) Экспорт
	...
	Для Каждого ОтключаемоеЗадание Из ЗаданияОтключаемыеМоделиСервиса Цикл
		РегламентноеЗадание = ОтключаемоеЗадание.РегламентноеЗадание; // ОбъектМетаданныхРегламентноеЗадание
		Если ТаблицаИспользования.Найти(РегламентноеЗадание.Имя, "РегламентноеЗадание") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = ТаблицаИспользования.Добавить();
		НоваяСтрока.РегламентноеЗадание = РегламентноеЗадание.Имя;
...
Процедура УстановитьСвойстваРегламентногоЗадания(Приемник, ЗаданиеИсточник)
	...
	СвойстваПоследнегоФоновогоЗадания = ЗаданиеИсточник.ПоследнееЗадание;
...
#Область ПроцедурыИФункцииДляРаботыСРегламентнымиЗаданиями

// Новые параметры выполнения регламентного задания.
// 
// Возвращаемое значение:
//  Структура - Новые параметры выполнения регламентного задания:
//    * МоментЗапуска - Неопределено
//                    - Дата - для файловой ИБ устанавливает переданный момент, как момент запуска метода регламентного задания.
...
//    * ПредставлениеФоновогоЗадания - Строка
//    * ПроцедураУжеВыполняется - Булево 
//    * ЗапускВыполнен - Булево
// 
Функция НовыеПараметрыВыполненияРегламентногоЗадания() 
	...
	Результат.Вставить("ПроцедураУжеВыполняется", Ложь);
	Результат.Вставить("ЗапускВыполнен", Ложь);
	Возврат Результат;
...
Функция ПредставлениеРегламентногоЗадания(Знач Задание) Экспорт
	...
	Если РегламентноеЗадание <> Неопределено Тогда
		Если РегламентноеЗадание.Предопределенное Тогда
			Представление = РегламентноеЗадание.Метаданные.Представление();
		Иначе
...
// Возвращает сообщения и описание информации об ошибке из последнего фонового задания 
// указанного регламентного задания.
//
// Параметры:
//  Задание      - РегламентноеЗадание
//               - Строка - уникальный идентификатор регламентного задания строкой.
//
// Возвращаемое значение:
//  Строка
//
...
#Область ПроцедурыИФункцииДляРаботыСФоновымиЗаданиями
...
Процедура ДобавитьСвойстваФоновыхЗаданий(Знач МассивФоновыхЗаданий, Знач ТаблицаСвойствФоновыхЗаданий)
	...
	Пока Индекс >= 0 Цикл
		...
		КонецЕсли;
		Строка.ИдентификаторРегламентногоЗадания = ?(РегламентноеЗадание = Неопределено,
			"", РегламентноеЗадание.УникальныйИдентификатор);
		
		Строка.ОписаниеИнформацииОбОшибке = ?(ФоновоеЗадание.ИнформацияОбОшибке = Неопределено,
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СформироватьТаблицуРегламентныхЗаданий", Истина);
	Методы.Вставить("ЗаполнитьТаблицуСвойствФоновыхЗаданийВФоне", Истина);
