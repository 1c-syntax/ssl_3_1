...
Процедура ПриОпределенииИспользованияРегламентныхЗаданий(ТаблицаИспользования) Экспорт
	...
	Для Каждого ВключаемоеЗадание Из ЗаданияВключаемыеВМоделиСервиса Цикл
		РегламентноеЗадание = ВключаемоеЗадание.РегламентноеЗадание; // ОбъектМетаданныхРегламентноеЗадание
		Если ТаблицаИспользования.Найти(РегламентноеЗадание.Имя, "РегламентноеЗадание") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
//     * Таблица - ТаблицаЗначений - с колонками:
...
Процедура СформироватьТаблицуРегламентныхЗаданий(Параметры, АдресХранилища) Экспорт
	...
	Если ИдентификаторРегламентногоЗадания = Неопределено Тогда
		...
		Для Каждого Задание Из ТекущиеЗадания Цикл
			...
			Если Индекс >= Таблица.Количество() Или Таблица.Получить(Индекс).Идентификатор <> Идентификатор Тогда
...
// 	ПоследнееФоновоеЗадание - ФоновоеЗадание, Неопределено - 
// Возвращаемое значение:
