...
//     * ДополнительныеПараметры - Произвольный - значение, которое было указано при создании объекта
//                               ОписаниеОповещения.
//   СохраняемыеФайлы     - Массив из ОписаниеПереданногоФайла - содержит объекты типа ОписаниеПереданногоФайла.
//   ПараметрыСохранения  - см. ФайловаяСистемаКлиент.ПараметрыСохраненияФайла
//
// Пример:
...
Процедура ЗапуститьПрограмму(Знач КомандаЗапуска, Знач ПараметрыЗапускаПрограммы = Неопределено) Экспорт
	...
	СредаИсполнения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗапускаПрограммы, "СредаИсполнения", "Windows");
	СтрокаКоманды = ОбщегоНазначенияСлужебныйКлиентСервер.БезопаснаяСтрокаКоманды(КомандаЗапуска, СредаИсполнения);
