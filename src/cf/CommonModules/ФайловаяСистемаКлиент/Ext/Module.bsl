...
Процедура СохранитьФайл(ОбработчикЗавершения, РасположениеФайла, ИмяФайла = "", 
	...
	ДанныеФайла = Новый ОписаниеПередаваемогоФайла(ИмяФайла, РасположениеФайла);
...
//
Процедура ОткрытьФайл(РасположениеФайла, ОбработчикЗавершения = Неопределено, ИмяФайла = "",
	...
	Если Не ЭтоАдресВременногоХранилища(РасположениеФайла)
		И Не ОбщегоНазначенияСлужебныйКлиент.ЭтоНавигационнаяСсылка(РасположениеФайла) Тогда
...
Процедура ОткрытьПроводник(ПутьККаталогуИлиФайлу) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения(
		"ОткрытьПроводникПослеПроверкиРасширенияРаботы1СПредприятия", ФайловаяСистемаСлужебныйКлиент, Контекст);
		
	ТекстПредложения = НСтр("ru = 'Для открытия папки установите расширение для работы с 1С:Предприятием.'");
	ПодключитьРасширениеДляРаботы1СПредприятия(Оповещение, ТекстПредложения, Ложь);
...
#КонецЕсли
	...
	Иначе 
		...
		Оповещение = Новый ОписаниеОповещения(
			"ОткрытьНавигационнуюСсылкуПослеПроверкиРасширения1СПредприятия", ФайловаяСистемаСлужебныйКлиент, Контекст);
...
#КонецЕсли
	...
	Оповещение = Новый ОписаниеОповещения("ЗапуститьПрограммуПослеПроверкиРасширения1СПредприятия", 
		ФайловаяСистемаСлужебныйКлиент, Контекст);
	ПодключитьРасширениеДляРаботы1СПредприятия(Оповещение, 
...
#Если Не МобильныйКлиент Тогда
	...
	ИначеЕсли ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() Тогда
		...
		СистемныеСвойства  = Новый СистемнаяИнформация();
		Если СтрНайти(ВРег(СистемныеСвойства.ВерсияОС), "UBUNTU") > 0 Или СтрНайти(ВРег(СистемныеСвойства.ВерсияОС), "ASTRA") > 0 Тогда
...
Процедура ВыбратьКаталог(ОбработчикЗавершения, Заголовок = "", Каталог = "") Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ВыбратьКаталогПриПодключенииРасширения1СПредприятия", ФайловаяСистемаСлужебныйКлиент, Контекст);
	ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещения);
...
Процедура ПоказатьДиалогВыбора(ОбработчикЗавершения, Диалог) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПоказатьДиалогВыбораПриПодключенииРасширения1СПредприятия", ФайловаяСистемаСлужебныйКлиент, Контекст);
	ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещения);
...
Процедура СоздатьВременныйКаталог(Знач Оповещение, Расширение = "") Экспорт 
	...
	Оповещение = Новый ОписаниеОповещения("СоздатьВременныйКаталогПослеПроверкиРасширения1СПредприятия",
		ФайловаяСистемаСлужебныйКлиент, Контекст);
	ПодключитьРасширениеДляРаботы1СПредприятия(Оповещение, 
...
//    КонецЕсли;
//
Процедура ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещенияОЗакрытии, ТекстПредложения = "",
	ВозможноПродолжениеБезУстановки = Истина) Экспорт
	
	ОписаниеОповещенияЗавершение = Новый ОписаниеОповещения(
		"НачатьПодключениеРасширения1СПредприятияПриОтветеНаВопросОбУстановке", ФайловаяСистемаСлужебныйКлиент,
...
#КонецЕсли
	...
	Оповещение = Новый ОписаниеОповещения(
		"НачатьПодключениеРасширения1СПредприятия", ФайловаяСистемаСлужебныйКлиент, Контекст);
...
Процедура ПоказатьПомещениеФайла(ОбработчикЗавершения, ПараметрыПомещения)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПоказатьПомещениеФайлаПриПодключенииРасширения1СПредприятия", ФайловаяСистемаСлужебныйКлиент, ПараметрыПомещения);
	ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещения, ПараметрыПомещения.ТекстПредложения);
...
Процедура ПоказатьПолучениеФайлов(ОбработчикЗавершения, СохраняемыеФайлы, ПараметрыПолучения)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПоказатьПолучениеФайловПриПодключенииРасширения1СПредприятия", ФайловаяСистемаСлужебныйКлиент, ПараметрыПолучения);
	ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещения, ПараметрыПолучения.ТекстПредложения);
...
Процедура НапечататьИзПриложенияПоИмениФайлаLinux(Результат, Параметры) Экспорт
	...
	Если Результат = Ложь Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'Для печати документа требуется установить на компьютере пакет <a href=""%1"">Unoconv</a>.
			|
...
Процедура НапечататьИзПриложенияПоИмениФайлаLinuxЗавершение(Результат, Параметры) Экспорт
	...
	// АПК:566-выкл Выполняется только для Linux
	Файл = Новый Файл(Параметры.ФайлКонвертируемыхДанных);
	Если Файл.Существует() Тогда
		УдалитьФайлы(Параметры.ФайлКонвертируемыхДанных);
	КонецЕсли;
	// АПК:566-вкл
	
	Если Не Результат.ПриложениеЗапущено Или ЗначениеЗаполнено(Результат.ОписаниеОшибки) 
		Или ЗначениеЗаполнено(Результат.ПотокОшибок) Тогда

		ОписаниеОшибки = СокрЛП(?(ЗначениеЗаполнено(Результат.ОписаниеОшибки), 
			Результат.ОписаниеОшибки + Символы.ПС, "") + Результат.ПотокОшибок);
		
		Рекомендация = ""; 
		Если СтрНайти(НРег(ОписаниеОшибки), "no default destination") > 0 Тогда
			Рекомендация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Задайте принтер по умолчанию:
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось распечатать документ по причине:
				|%1'"),
			ОписаниеОшибки, Результат.КодВозврата);
		Если Результат.КодВозврата <> 0 Тогда
			ОписаниеОшибки = ОписаниеОшибки 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(" (%1)", Результат.КодВозврата);
		КонецЕсли;
		Если Не ПустаяСтрока(Рекомендация) Тогда
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + Рекомендация;
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru = 'Стандартные подсистемы'", 
...
#КонецЕсли
	Иначе
		ПолучитьИмяВременногоФайлаОповеститьОбОшибке(НСтр("ru = 'Не удалось установить расширение для работы с 1С:Предприятием.'"), 
			Контекст);
	КонецЕсли;
