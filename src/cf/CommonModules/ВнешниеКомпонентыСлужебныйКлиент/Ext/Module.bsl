...
Процедура ПроверитьДоступностьКомпоненты(Оповещение, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(Контекст.Версия) Тогда
		Контекст.Версия = Информация.Реквизиты.Версия;
	КонецЕсли;
	...
	Иначе
		...
		БудетПодключенаКомпонентаИзМакета = Ложь;
		Если ИскатьКомпонентуПоследнейВерсии Тогда
			ЗаменитьНаАктуальнуюКомпонентуИзСправочника(Результат, Контекст.Идентификатор, Информация.Местоположение, БудетПодключенаКомпонентаИзМакета);
		КонецЕсли;
		
		Если БудетПодключенаКомпонентаИзМакета
			Или Не Информация.ЗаполненыЦелевыеПлатформы
			Или ТекущийКлиентПоддерживаетсяКомпонентой(Информация.Реквизиты.ЦелевыеПлатформы) Тогда
...
Функция ОшибкаСовместимостиКомпонентыИзМакета(Местоположение) Экспорт
	...
	ИнформацияОВнешнейКомпоненте = ВнешниеКомпонентыСлужебныйВызовСервера.ИнформацияОВнешнейКомпонентеИзМакета(Местоположение);
		 
	Если ИнформацияОВнешнейКомпоненте <> Неопределено 
		И Не ТекущийКлиентПоддерживаетсяКомпонентой(ИнформацияОВнешнейКомпоненте.Реквизиты.ЦелевыеПлатформы) Тогда
			Возврат ОписаниеОшибкиСовместимостиКомпоненты();
	КонецЕсли;
...
Процедура ПроверитьСовместимостьКомпонентыИзМакета(Оповещение, КонтекстПодключенияКомпоненты) Экспорт
	...
	ИнформацияОВнешнейКомпоненте = ВнешниеКомпонентыСлужебныйВызовСервера.ИнформацияОВнешнейКомпонентеИзМакета(
		КонтекстПодключенияКомпоненты.Местоположение);
	...
	Если ИнформацияОВнешнейКомпоненте <> Неопределено 
		...
		Контекст.ОписаниеОшибки = ОписаниеОшибкиСовместимостиКомпоненты();
		
		Если КонтекстПодключенияКомпоненты.ПредложитьУстановить Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОтображенияИнформацииОСовместимости", ЭтотОбъект, Контекст);
			
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ТекстПояснения", КонтекстПодключенияКомпоненты.ТекстПояснения);
			ПараметрыФормы.Вставить("ПоддерживаемыеКлиенты", ИнформацияОВнешнейКомпоненте.Реквизиты.ЦелевыеПлатформы);
			ПараметрыФормы.Вставить("ПослеОшибкиПодключения", Истина);
	...
	ПослеОтображенияИнформацииОСовместимости(Неопределено, Контекст);
...
Процедура ПослеОтображенияИнформацииОСовместимости(Результат, Контекст) Экспорт
	...
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Контекст.ОписаниеОшибки);
	
КонецПроцедуры

Функция ОписаниеОшибкиСовместимостиКомпоненты()
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не предусмотрена работа внешней компоненты в клиентском приложении %1. Обратитесь к разработчику внешней компоненты'"),
			ОбщегоНазначенияСлужебныйКлиент.ВидПриложения());
КонецФункции
...
Процедура ЗаменитьНаАктуальнуюКомпонентуИзСправочника(Результат, Идентификатор, Местоположение, БудетПодключенаКомпонентаИзМакета)
	...
	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрЗаменить(Результат.Версия, ".", "")) Тогда
		...
		Если ЧастиВерсии.Количество() = 4 И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Результат.Версия,
			Результат.КомпонентаПоследнейВерсии.Версия) <= 0 Тогда
				БудетПодключенаКомпонентаИзМакета = Истина;
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьКомпонентыСПортала(Оповещение, ОбновляемыеКомпоненты) Экспорт
	...
	ВнешниеКомпонентыКлиентЛокализация.ПриОбновленииКомпонентСПортала(ОповещениеФормы, ОбновляемыеКомпоненты);
