...
Процедура ВыполнитьОбменДаннымиПоСценарию(КодСценарияОбмена) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не Выборка.ИспользоватьРегламентноеЗадание И Выборка.ОтключенАвтоматически Тогда
			Возврат;
		КонецЕсли;
		
		Если ВыполнитьСценарийОбменаРегламентнымЗаданием(Сценарий) Тогда
...
Процедура ВыполнитьОбменДаннымиВручную(Узел, ПараметрыОбмена, ДополнениеВыгрузки = Неопределено) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина); // В разделенной ИБ, внутренняя публикация, не полные права
...
Процедура ОтменитьОчередьЗадач(Узел, Сценарий, ИдентификаторОбмена, ОбластьДанных) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация");
			ЭлементБлокировкиДанных.УстановитьЗначение("ДатаСоздания", Выборка.ДатаСоздания);
			ЭлементБлокировкиДанных.УстановитьЗначение("УзелИнформационнойБазы", Выборка.УзелИнформационнойБазы);
			ЭлементБлокировкиДанных.УстановитьЗначение("НомерЗадачи", Выборка.НомерЗадачи);
			ЭлементБлокировкиДанных.УстановитьЗначение("ИдентификаторОбмена", Выборка.ИдентификаторОбмена);
			ЭлементБлокировкиДанных.УстановитьЗначение("ИдентификаторЗадачи", Выборка.ИдентификаторЗадачи);
		...
		Исключение
			...
			Событие = НСтр("ru = 'Обмен данными.Отмена очереди задач'", ОбщегоНазначения.КодОсновногоЯзыка());

			ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка,
				Метаданные.РегистрыСведений.ЗадачиОбменаДаннымиВнутренняяПубликация, , СообщениеОбОшибке);
		КонецПопытки;
...
Процедура ОтметитьВыполнениеЗадачи(Задача, Ошибка) Экспорт
	...
	Если ЗначениеЗаполнено(ТекЗадача) Тогда
		...
		Попытка
			БлокировкаДанных = Новый БлокировкаДанных;
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация");
			ЭлементБлокировкиДанных.УстановитьЗначение("ДатаСоздания", ТекЗадача.ДатаСоздания);
			ЭлементБлокировкиДанных.УстановитьЗначение("УзелИнформационнойБазы", ТекЗадача.УзелИнформационнойБазы);
			ЭлементБлокировкиДанных.УстановитьЗначение("НомерЗадачи", ТекЗадача.НомерЗадачи);
			ЭлементБлокировкиДанных.УстановитьЗначение("ИдентификаторОбмена", ТекЗадача.ИдентификаторОбмена);
			ЭлементБлокировкиДанных.УстановитьЗначение("ИдентификаторЗадачи", ТекЗадача.ИдентификаторЗадачи);
			ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Исключительный;
			БлокировкаДанных.Заблокировать();
		...
		Исключение
			...
			Событие = НСтр("ru = 'Обмен данными.Отметка выполнения задачи'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов(ИмяПланаОбмена, КодУзлаИнформационнойБазы, ИдентификаторЗадачи) Экспорт
	...
	ОбменДаннымиСервер.ПоместитьФайлВХранилище(ИмяФайлаДляПомещенияВХранилище,
		ИдентификаторЗадачи,
		ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзлаИнформационнойБазы),
...
Процедура ВыполнитьЗагрузкуДляУзлаИнформационнойБазыИзСервисаПередачиФайлов(ИмяПланаОбмена, КодУзлаИнформационнойБазы, ИдентификаторЗадачи, ИдентификаторФайла) Экспорт
	...
	ИмяФайлаСообщения = ОбменДаннымиСервер.ПолучитьФайлИзХранилища(ИдентификаторФайла,, Ложь);
	...
	Исключение
		...
		ПредставлениеОшибки = ПризнакОтменыПовторногоВыполненияЗадания();
		ПредставлениеОшибки = ПредставлениеОшибки + ":" + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПризнакОтменыПовторногоВыполненияЗадания() Экспорт
	
	Возврат НСтр("ru = '#ОтменитьПовторноеВыполнениеЗадачи#'");
...
Процедура УдалитьУстаревшиеЗадачи(Сценарий = Неопределено, РучнойОбмен = Ложь)
	...
	Попытка
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	Задачи.ИдентификаторОбмена
		|
		|УПОРЯДОЧИТЬ ПО
		|	МАКСИМУМ(Задачи.ДатаСоздания) УБЫВ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если Выборка.Следующий() Тогда
		...
		Если Состояние = "Запланировано" Или Состояние = "Выполняется" Тогда
			...
			Исключение
				...
				Событие = НСтр("ru = 'Обмен данными.Запись'", ОбщегоНазначения.КодОсновногоЯзыка());
				
				ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка, Метаданные.РегистрыСведений.ЗадачиОбменаДаннымиВнутренняяПубликация, , СообщениеОбОшибке);
			КонецПопытки;
...
Процедура ВыполнитьЗадачу(Задача, ПараметрыОбмена, Отказ) Экспорт
	...
	ИначеЕсли Действие = Перечисления.ДействияПриОбменеВнутренняяПубликация.ЗагрузкаДанных Тогда
		...
		УдалитьЗадачиПриВозникновенииОшибки(Отказ, Задача);
...
Процедура ВыполнитьЗадачуВыгрузкаДанных(Задача, Прокси, СтруктураНастроекОбмена, ОбластьДанных, Отказ, Ошибка)
	...
	ВременныйКаталог = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, "{" + Строка(Новый УникальныйИдентификатор) + "}");
...
Процедура УстановитьФункциональныеОпцииДляУзла(Форма)
	...
	ИспользоватьСценарии = (ИдентификаторТранспорта = "SM" И НастройкиТранспорта.ВнутренняяПубликация)
		...
		Или ИдентификаторТранспорта = "HTTP";
	...
	Попытка
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ОбщиеНастройкиУзловИнформационныхБаз");
		ЭлементБлокировкиДанных.УстановитьЗначение("УзелИнформационнойБазы", Узел);
		ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Исключительный;
		БлокировкаДанных.Заблокировать();
	...
	Исключение
		...
		Событие = НСтр("ru = 'Обмен данными.Установка функциональных опций'", ОбщегоНазначения.КодОсновногоЯзыка());

		ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка,
			Метаданные.РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз, , СообщениеОбОшибке);
	КонецПопытки;
...
Процедура УдалитьЗадачиПриВозникновенииОшибки(Отказ, Задача)
	
	Если Отказ Тогда
		ОтборЗаданий = Новый Структура;
		ОтборЗаданий.Вставить("Ключ", Задача.ИдентификаторЗадачи);
		
		МодульОчередьЗаданий = ОбщегоНазначения.ОбщийМодуль("ОчередьЗаданий");
		
		НайденныеЗадания = МодульОчередьЗаданий.ПолучитьЗадания(ОтборЗаданий);
		
		Если НайденныеЗадания.Количество() Тогда
			ОбластьДанных   = ПараметрыСеанса["ОбластьДанныхЗначение"];
			НайденнаяЗадача = НайденныеЗадания[0];
			
			МодульОчередьЗаданий.УдалитьЗадание(НайденнаяЗадача.Идентификатор);
			
			МодульОбменДаннымиВнутренняяПубликация = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиВнутренняяПубликация");
			МодульОбменДаннымиВнутренняяПубликация.ОтменитьОчередьЗадач(НайденнаяЗадача.Идентификатор, Неопределено, Строка(Задача.ИдентификаторОбмена), ОбластьДанных);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Функция ВыполнитьСценарийОбменаРегламентнымЗаданием(Сценарий)
	...
	|ГДЕ
	|	НастройкиОбмена.Ссылка = &Ссылка
	|	И НастройкиОбмена.ИдентификаторТранспорта ПОДОБНО ""HTTP""
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	СценарииОбменовДаннымиНастройкиОбмена.ИдентификаторТранспорта ПОДОБНО ""WS""
	|	И СценарииОбменовДаннымиНастройкиОбмена.Ссылка = &Ссылка";
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ВыполнитьОчередьЗадач");
		Методы.Вставить("ВыполнитьОбменДаннымиПоСценарию");
		Методы.Вставить("ВыполнитьЗагрузкуДляУзлаИнформационнойБазыИзСервисаПередачиФайлов");
		Методы.Вставить("ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов");
	КонецЕсли;
