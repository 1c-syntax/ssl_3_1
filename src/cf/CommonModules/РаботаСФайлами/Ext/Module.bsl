...
Функция ДвоичныеДанныеФайла(Знач ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Результат = РаботаСФайламиСлужебный.ДвоичныеДанныеФайла(ПрисоединенныйФайл, ВызыватьИсключение);
	Возврат Результат;
...
Функция ДвоичныеДанныеФайлов(Знач ПрисоединенныеФайлы, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Для Индекс = 1 По ПрисоединенныеФайлы.Количество() - 1 Цикл
		Если ТипЗнч(ПрисоединенныеФайлы[Индекс]) <> ТипФайла Тогда
			ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Все присоединенные файлы в параметре %1 должны быть одного типа в %2.'"),
				"ПрисоединенныеФайлы", "РаботаСФайлами.ДвоичныеДанныеФайлов"),
				КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	КонецЦикла;

	МетаданныеОбъектаФайла = ПрисоединенныеФайлы[0].Метаданные();
	ПолноеИмя = МетаданныеОбъектаФайла.ПолноеИмя();
	...
	РеквизитыРазрешенныхПрисоединенныхФайлов = РеквизитыРазрешенныхПрисоединенныхФайлов(ЕстьРеквизитТекущаяВерсия, ПолноеИмя, ПрисоединенныеФайлы);
	РазрешенныеПрисоединенныеФайлы = Новый Массив;
	Для Каждого КлючИЗначение Из РеквизитыРазрешенныхПрисоединенныхФайлов Цикл
		РазрешенныеПрисоединенныеФайлы.Добавить(КлючИЗначение.Ключ);
	КонецЦикла;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаДвоичныеДанныеФайлов(ЕстьРеквизитТекущаяВерсия, ПолноеИмя);
	Запрос.УстановитьПараметр("РазрешенныеПрисоединенныеФайлы", РазрешенныеПрисоединенныеФайлы);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Результат = Новый Соответствие;
	Пока Выборка.Следующий() Цикл
		РеквизитыФайла = РеквизитыРазрешенныхПрисоединенныхФайлов.Получить(Выборка.Ссылка);
		
		ДвоичныеДанные = Неопределено;
		Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(РеквизитыФайла.ТипХраненияФайла) Тогда			
			Если Выборка.ХранилищеФайла <> Null Тогда
				ДвоичныеДанные = Выборка.ХранилищеФайла.Получить();
			КонецЕсли;
		Иначе
			ДвоичныеДанные = РаботаСФайламиВТомахСлужебный.ДанныеФайла(РеквизитыФайла, ВызыватьИсключение);
		КонецЕсли;
...
Функция ДанныеФайла(Знач ПрисоединенныйФайл, Знач ДополнительныеПараметры = Неопределено,
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		ПараметрыПолученияФайла = ?(ДополнительныеПараметры.Свойство("ПараметрыПолученияФайла"), ДополнительныеПараметры.ПараметрыПолученияФайла, Неопределено);
	...
	Иначе
		...
		ПараметрыПолученияФайла = Неопределено;
	КонецЕсли;
	...
	Если Не ВызыватьИсключение Тогда
		...
		Иначе
			ЧтениеРазрешено = ПравоДоступа("Чтение", МетаданныеОбъектаФайла);
		КонецЕсли;
	...
	Если ФайлОбъект = Неопределено Тогда
		Если ВызыватьИсключение Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Строка(ПрисоединенныйФайл), МетаданныеОбъектаФайла.Представление());
	...
	Если ПолучатьСсылкуНаДвоичныеДанные Тогда
		...
		Если ИспользуетсяВерсионированиеФайлов Тогда
			ПрисоединенныйФайлТекущаяВерсия = ПрисоединенныйФайл.ТекущаяВерсия;
		Иначе
			ПрисоединенныйФайлТекущаяВерсия = ПрисоединенныйФайл;
		КонецЕсли;
		
		ПроверятьНаличиеФайлаВАрхиве = Истина;
		ПолучениеФайлаИзАрхива = Ложь;
		
		Если ТипЗнч(ПараметрыПолученияФайла) = Тип("Структура") Тогда
			ПроверятьНаличиеФайлаВАрхиве = ПараметрыПолученияФайла.ПроверятьНаличиеФайлаВАрхиве;

			Если Не ПроверятьНаличиеФайлаВАрхиве Тогда
				ПолучениеФайлаИзАрхива = ПараметрыПолученияФайла.ФайлВАрхиве;
			КонецЕсли;
		КонецЕсли;

		Если ПроверятьНаличиеФайлаВАрхиве Тогда 
			ПолучениеФайлаИзАрхива = РаботаСФайламиСлужебный.ФайлНаходитсяВФайловомАрхиве(ПрисоединенныйФайл, ПрисоединенныйФайлТекущаяВерсия);
		КонецЕсли;
		
		ДвоичныеДанные = РаботаСФайламиСлужебный.ДвоичныеДанныеФайла(ПрисоединенныйФайлТекущаяВерсия, ВызыватьИсключение, ПолучениеФайлаИзАрхива);
		
		Если ПолучениеФайлаИзАрхива Тогда
			ДвоичныеДанные = РаботаСФайловымАрхивомСервер.ДвоичныеДанныеФайлаИзвлеченногоИзZipАрхива(ДвоичныеДанные, ПрисоединенныйФайлТекущаяВерсия);
		КонецЕсли;

		АдресДвоичныхДанныхВоВременномХранилище = "";
		Если ТипЗнч(ПараметрыПолученияФайла) = Тип("Структура") И ЗначениеЗаполнено(ПараметрыПолученияФайла.АдресВРодительскомСеансе) Тогда
			АдресДвоичныхДанныхВоВременномХранилище = ПараметрыПолученияФайла.АдресВРодительскомСеансе;
		ИначеЕсли ТипЗнч(ИдентификаторФормы) = Тип("УникальныйИдентификатор") Тогда
			АдресДвоичныхДанныхВоВременномХранилище = ИдентификаторФормы;
		КонецЕсли;

		Если ЗначениеЗаполнено(АдресДвоичныхДанныхВоВременномХранилище) Тогда
			СсылкаНаДвоичныеДанныеФайла = ПоместитьВоВременноеХранилище(ДвоичныеДанные, АдресДвоичныхДанныхВоВременномХранилище);
		Иначе
	...
	ФайлРедактируетТекущийПользователь = ?(Результат.ФайлРедактируется, ФайлОбъект.Редактирует = Пользователи.АвторизованныйПользователь(), Ложь); 
	Результат.Вставить("ФайлРедактируетТекущийПользователь", ФайлРедактируетТекущийПользователь);
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Служебный", МетаданныеОбъектаФайла) Тогда
...
Процедура ОбновитьФайл(Знач ПрисоединенныйФайл, Знач ИнформацияОФайле) Экспорт
	...
	Попытка
		...
		ИмяБезРасширения = "";
		Если ИнформацияОФайле.Свойство("ИмяБезРасширения", ИмяБезРасширения) 
			И ЗначениеЗаполнено(ИнформацияОФайле.ИмяБезРасширения) Тогда
			ИмяБезРасширения   = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ИмяБезРасширения);
			ФайлОбъект.Наименование = ИмяБезРасширения;
		КонецЕсли;
	
		ФайлОбъект.ДатаМодификацииУниверсальная = ТекущаяУниверсальнаяДата();
		Если ИнформацияОФайле.Свойство("ДатаМодификацииУниверсальная")
			И ЗначениеЗаполнено(ИнформацияОФайле.ДатаМодификацииУниверсальная)
			И ИнформацияОФайле.ДатаМодификацииУниверсальная < ФайлОбъект.ДатаМодификацииУниверсальная Тогда
			ФайлОбъект.ДатаМодификацииУниверсальная = ИнформацияОФайле.ДатаМодификацииУниверсальная;
		КонецЕсли;
		
		Если ИнформацияОФайле.Свойство("Редактирует") Тогда
			ФайлОбъект.Редактирует = ИнформацияОФайле.Редактирует;
		КонецЕсли;
		
		РасширениеФайла = "";
		Если ИнформацияОФайле.Свойство("Расширение", РасширениеФайла) Тогда
			РасширениеФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(РасширениеФайла);
			ФайлОбъект.Расширение = РасширениеФайла;
		КонецЕсли;
		...
		РаботаСФайламиСлужебный.ЗаполнитьИзвлеченныйТекст(ФайлОбъект, 
			ИнформацияОФайле.АдресВременногоХранилищаТекста, ДвоичныеДанные);
		РаботаСФайламиСлужебный.ОбновитьДвоичныеДанныеФайлаНаСервере(ФайлОбъект, ДвоичныеДанные);
...
Функция ДобавитьФайлСДиска(ВладелецФайлов, ПутьКФайлуНаДиске) Экспорт
	...
	Иначе
		...
		Если ОбщегоНазначения.ЭтоWindowsСервер() Тогда
			...
			АдресВременногоХранилищаТекста = Новый ХранилищеЗначения(Текст, Новый СжатиеДанных(9));
		КонецЕсли;
	КонецЕсли;
	...
	Результат = РаботаСФайламиСлужебныйВызовСервера.СоздатьФайлСВерсией(ВладелецФайлов, СведенияОФайле);
...
Функция ДобавитьФайл(ПараметрыФайла, Знач АдресФайлаВоВременномХранилище, 
	...
	РаботаСФайламиСлужебный.ЗаполнитьИзвлеченныйТекст(ПрисоединенныйФайл, АдресВременногоХранилищаТекста,
		ДвоичныеДанные);
	...
	РаботаСФайловымАрхивомСервер.ЗаполнитьСвойстваФайлаПоСуществующемуЭлементуХранилищаДвоичныхДанных(ПрисоединенныйФайл, ДвоичныеДанные);

	Контекст = РаботаСФайламиСлужебный.КонтекстОбновленияФайла(ПрисоединенныйФайл, АдресФайлаВоВременномХранилище, 
		НоваяСсылкаНаФайл, ПрисоединенныйФайл.ТипХраненияФайла);
	...
	Попытка
		...
		Если ХранитьВерсии Тогда
			...
			СведенияОФайле.АдресВременногоХранилищаФайла   = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				Контекст, "ПутьКФайлу", АдресФайлаВоВременномХранилище);
			СведенияОФайле.АдресВременногоХранилищаТекста  = АдресВременногоХранилищаТекста;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Добавление присоединенного файла'", 
...
Функция ПеренестиФайлыМеждуСправочникамиХранения(Знач ВладелецФайлов, Знач Источник = Неопределено,
	...
	Если Источник = Неопределено Или Приемник = Неопределено Тогда
		ИменаСправочников = РаботаСФайламиСлужебный.ИменаСправочниковХраненияФайлов(ВладелецФайлов, Истина);
		Для каждого ИмяСправочника Из ИменаСправочников Цикл
			Если Источник = Неопределено И Не ИмяСправочника.Значение Тогда
				Источник = Метаданные.Справочники[ИмяСправочника.Ключ].ПолноеИмя();
			КонецЕсли;
			
			Если Приемник = Неопределено И ИмяСправочника.Значение Тогда
				Приемник = Метаданные.Справочники[ИмяСправочника.Ключ].ПолноеИмя();
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если Источник = Приемник Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС
				+ НСтр("ru = 'Справочник-источник присоединенных файлов ""%1"" совпадает 
			             |со справочником-приемником в %2.'"),
				Источник, "РаботаСФайлами.ПеренестиФайлыМеждуСправочникамиХранения"),
			КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	
	ИмяПриемника = Метаданные.НайтиПоПолномуИмени(Приемник).Имя;
	
	Результат = Новый Соответствие;
	ИмяСправочника = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(
	ВладелецФайлов, ИмяПриемника, ЗаголовокОшибки);
	
	ФайлыИсточник = РаботаСФайламиСлужебный.ПолучитьВсеПодчиненныеФайлы(ВладелецФайлов, Источник);
	
	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);
	МенеджерПрисоединенныхФайлов = Справочники[ИмяСправочника];
	
	ДоступноВерсионирование = Метаданные.Справочники.Файлы.ПолноеИмя() = Источник;
	...
	Для Каждого ФайлИсточник Из ФайлыИсточник Цикл
		...
		Попытка
			...
			БлокировкаДанных = Новый БлокировкаДанных;
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Источник);
			...
			Если ФайлИсточникОбъект <> Неопределено И Не ФайлИсточникОбъект.ПометкаУдаления Тогда
				...
				Если ДоступноВерсионирование Тогда
					...
					Если СсылкаКорректная Тогда
						...
						СсылкаНаПрисоединенныйФайл = СоздатьПрисоединенныйФайлНаОснованииФайла(ВладелецФайлов, 
							МенеджерПрисоединенныхФайлов, ФайлИсточникОбъект, ТекущаяВерсияОбъект);
				...
				Иначе
					// @skip-check query-in-loop - по-объектная запись файлов в транзакции
					СсылкаНаПрисоединенныйФайл = СоздатьПрисоединенныйФайлНаОснованииФайла(ВладелецФайлов, 
						МенеджерПрисоединенныхФайлов, ФайлИсточникОбъект);
...
Процедура ПриСозданииНаСервере(Форма, ДобавляемыеЭлементы = Неопределено, НастройкиРаботыСФайламиВФорме = Неопределено) Экспорт
	...
	Для Индекс = 0 По ДобавляемыеЭлементы.ВГраница() Цикл
		...
		ПараметрыЭлементаФормы.Вставить("ЗаголовокГруппы",    ЗаголовокГруппы);
		...
		Если ДобавляемыйЭлемент.Свойство("ПутьКДанным") Тогда
			...
			//@skip-check query-in-loop - ввод поля файла
...
Функция ПолучитьНастройкиСканированияПользователя(ИдентификаторКлиента) Экспорт
	...
	Результат.ПоказыватьДиалогСканера = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/ПоказыватьДиалогСканера", 
		ИдентификаторКлиента, Истина);
	
	Результат.ИмяУстройства = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/ИмяУстройства", 
		ИдентификаторКлиента, "");
	
	Результат.ФорматСканированногоИзображения = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
	...
	Если Результат.ФорматСканированногоИзображения = Перечисления.ФорматыСканированногоИзображения.ПустаяСсылка() Тогда
		Результат.ФорматСканированногоИзображения = Перечисления.ФорматыСканированногоИзображения.PNG;	
	КонецЕсли;
	
	Результат.СохранятьВPDF = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/СохранятьВPDF", 
		ИдентификаторКлиента, Ложь);
	
	Результат.ФорматХраненияМногостраничный = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
	...
	Результат.Разрешение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/Разрешение", 
		ИдентификаторКлиента);
	
	Результат.Цветность = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/Цветность", 
		ИдентификаторКлиента);
	
	Результат.Поворот = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/Поворот", 
		ИдентификаторКлиента);
	
	Результат.РазмерБумаги = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/РазмерБумаги", 
		ИдентификаторКлиента);
	
	Результат.ДвустороннееСканирование = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/ДвустороннееСканирование", 
		ИдентификаторКлиента, Ложь);
	
	Результат.АвтоматическаяПодачаДокументов = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/АвтоматическаяПодачаДокументов", 
		ИдентификаторКлиента, Ложь);

	Результат.ИспользоватьImageMagickДляПреобразованияВPDF =  ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/ИспользоватьImageMagickДляПреобразованияВPDF", 
		ИдентификаторКлиента, Ложь);
		
	Результат.КачествоJPG = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/КачествоJPG", 
		ИдентификаторКлиента, 100);
	
	Результат.СжатиеTIFF = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/СжатиеTIFF", 
		ИдентификаторКлиента, Перечисления.ВариантыСжатияTIFF.БезСжатия);
	
	Результат.ПутьКПрограммеКонвертации = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/ПутьКПрограммеКонвертации", 
		ИдентификаторКлиента, ""); // ImageMagick
		
	Результат.КаталогЖурналаСканирования = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/КаталогЖурналаСканирования", 
		ИдентификаторКлиента, ""); 
		
	Результат.ИспользоватьКаталогЖурналаСканирования = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
...
Процедура СохранитьНастройкиСканированияПользователя(НастройкиСканированияПользователя, ИдентификаторКлиента) Экспорт
	...
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ПоказыватьДиалогСканера",
		НастройкиСканированияПользователя.ПоказыватьДиалогСканера, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ИмяУстройства",
		НастройкиСканированияПользователя.ИмяУстройства, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ФорматСканированногоИзображения",
		НастройкиСканированияПользователя.ФорматСканированногоИзображения, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("СохранятьВPDF",
		НастройкиСканированияПользователя.СохранятьВPDF, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ФорматХраненияМногостраничный",
		НастройкиСканированияПользователя.ФорматХраненияМногостраничный, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("Разрешение",
		НастройкиСканированияПользователя.Разрешение, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("Цветность",
		НастройкиСканированияПользователя.Цветность, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("Поворот",
		НастройкиСканированияПользователя.Поворот, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("РазмерБумаги",
		НастройкиСканированияПользователя.РазмерБумаги, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ДвустороннееСканирование",
		НастройкиСканированияПользователя.ДвустороннееСканирование, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("АвтоматическаяПодачаДокументов",
		НастройкиСканированияПользователя.АвтоматическаяПодачаДокументов, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ИспользоватьImageMagickДляПреобразованияВPDF",
		НастройкиСканированияПользователя.ИспользоватьImageMagickДляПреобразованияВPDF, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("КачествоJPG",
		НастройкиСканированияПользователя.КачествоJPG, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("СжатиеTIFF",
		НастройкиСканированияПользователя.СжатиеTIFF, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ПутьКПрограммеКонвертации",
		НастройкиСканированияПользователя.ПутьКПрограммеКонвертации, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("КаталогЖурналаСканирования",
		НастройкиСканированияПользователя.КаталогЖурналаСканирования, ИдентификаторКлиента));
	Результат.Добавить(РаботаСФайламиСлужебный.НастройкаСканирования("ИспользоватьКаталогЖурналаСканирования",
		НастройкиСканированияПользователя.ИспользоватьКаталогЖурналаСканирования, ИдентификаторКлиента));
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранитьМассив(Результат, Истина);
КонецПроцедуры
...
//
Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений(Параметры) Экспорт  // АПК:530 Обрабатывает таблицы присоединенных файлов, а не данные других подсистем.
...
Процедура ИзменитьСправочникХраненияФайлов(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Для Каждого ФайлИсточник Из ФайлыИсточник Цикл
		...
		Попытка
			...
			Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ПрисоединенныйФайл.ТипХраненияФайла) Тогда
				...
				РегистрыСведений.ХранилищеФайлов.ЗаписатьДвоичныеДанные(СсылкаНового, ХранилищеФайла.Получить(), ПараметрыЗаписиВИБ);
			КонецЕсли;
...
Функция СконвертироватьФайлыВПрисоединенные(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Возврат ПеренестиФайлыМеждуСправочникамиХранения(ВладелецФайлов, Метаданные.Справочники.Файлы.ПолноеИмя(), ИмяСправочника);
...
// Создает соответствие с данными выборки по результату запроса в виде структур
//
// Параметры:
// 	РезультатЗапроса - РезультатЗапроса
// 	ИмяПоляКлюча     - Строка - имя поля выборки, для получения ключа соответствия
// 	ВсегдаСтруктура  - Булево - для ситуаций когда в выборке только два поля определяет как возвращать
// 	                           значение ключа. Если Истина, в значение ключа помещается структура с одним
// 	                           элементом, иначе само значение поля.
// 	ПолеКлючаВНижнийРегистр   - Булево - имеет смысл при передаче строковых полей ключа
//
// Возвращаемое значение:
// 	Соответствие Из КлючИЗначение:
// 	 * Ключ     - Произвольный - значение поля выборки по параметру "ИмяПоляКлюча"
// 	 * Значение - Произвольный - при более двух выходных полях всегда структура, при двух выходных полях - в зависимости
// 	                             от флага "ВсегдаСтруктура"
//
Функция РезультатЗапросаВСоответствие(РезультатЗапроса, ИмяПоляКлюча, ВсегдаСтруктура = Ложь, ПолеКлючаВНижнийРегистр = Ложь)
	Результат = Новый Соответствие;
	СписокПолей = Новый Массив();
	Для Каждого КолонкаЗапроса Из РезультатЗапроса.Колонки Цикл
		Если КолонкаЗапроса.Имя <> ИмяПоляКлюча Тогда
			СписокПолей.Добавить(КолонкаЗапроса.Имя);
		КонецЕсли;
	КонецЦикла;
	ЗначениеКакСтруктура = ВсегдаСтруктура ИЛИ СписокПолей.Количество() > 1;
	ИменаПолей = СтрСоединить(СписокПолей, ",");
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		Ключ = Выборка[ИмяПоляКлюча];
		Если ПолеКлючаВНижнийРегистр Тогда
			Ключ = НРег(Ключ);
		КонецЕсли;
		Если ЗначениеКакСтруктура Тогда
			Значение = Новый Структура(ИменаПолей);
			ЗаполнитьЗначенияСвойств(Значение, Выборка);
		Иначе
			Значение = Выборка[ИменаПолей];
		КонецЕсли;
...
//
Функция СоздатьПрисоединенныйФайлНаОснованииФайла(Знач ВладелецФайлов, Знач МенеджерПрисоединенныхФайлов, 
	Знач ФайлИсточникОбъект, Знач ТекущаяВерсияОбъект = Неопределено)
	
	СсылкаНового = МенеджерПрисоединенныхФайлов.ПолучитьСсылку(); // ОпределяемыйТип.ПрисоединенныйФайл
	ПрисоединенныйФайл = МенеджерПрисоединенныхФайлов.СоздатьЭлемент(); // ОпределяемыйТип.ПрисоединенныйФайлОбъект
	ПрисоединенныйФайл.УстановитьСсылкуНового(СсылкаНового);
	
	ИспользуетсяВерсионирование = Истина;
	
	Если ТекущаяВерсияОбъект = Неопределено Тогда
		ИспользуетсяВерсионирование = Ложь;
		ТекущаяВерсияОбъект = Новый Структура;
		ТекущаяВерсияОбъект.Вставить("ДатаМодификацииУниверсальная", ФайлИсточникОбъект.ДатаМодификацииУниверсальная);
		ТекущаяВерсияОбъект.Вставить("Наименование", ФайлИсточникОбъект.Наименование);
		ТекущаяВерсияОбъект.Вставить("Автор", ФайлИсточникОбъект.Автор);
		ТекущаяВерсияОбъект.Вставить("ДатаСоздания", ФайлИсточникОбъект.ДатаСоздания);
		ТекущаяВерсияОбъект.Вставить("Размер", ФайлИсточникОбъект.Размер);
		ТекущаяВерсияОбъект.Вставить("Расширение", ФайлИсточникОбъект.Расширение);
		ТекущаяВерсияОбъект.Вставить("ТипХраненияФайла", ФайлИсточникОбъект.ТипХраненияФайла);
		ТекущаяВерсияОбъект.Вставить("Том", ФайлИсточникОбъект.Том);
		ТекущаяВерсияОбъект.Вставить("ПутьКФайлу", ФайлИсточникОбъект.ПутьКФайлу);
		ТекущаяВерсияОбъект.Вставить("ТекущаяВерсия", Неопределено);
		ТекущаяВерсияОбъект.Вставить("УдалитьЭлектронныеПодписи", ФайлИсточникОбъект.УдалитьЭлектронныеПодписи.Выгрузить());
		ТекущаяВерсияОбъект.Вставить("Ссылка", ФайлИсточникОбъект.Ссылка);
	КонецЕсли;
	...
	Если Не ИспользуетсяВерсионирование Или ЗначениеЗаполнено(ФайлИсточникОбъект.ТекущаяВерсия) Тогда
		Для Каждого ЭПСтрока Из ТекущаяВерсияОбъект.УдалитьЭлектронныеПодписи Цикл
			НоваяСтрока = ПрисоединенныйФайл.УдалитьЭлектронныеПодписи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ЭПСтрока);
		КонецЦикла;
...
//
Функция ХранилищеФайлаИзИнформационнойБазы(ФайлСсылка, ПолучениеФайлаИзАрхива = Ложь) Экспорт
	
	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);
	
	ТипХраненияФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлСсылка, "ТипХраненияФайла");
	
	Если Не ЗначениеЗаполнено(ТипХраненияФайла) Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Не удалось определить тип хранения файла: %1 в функции: %2'"), 
								Строка(ФайлСсылка), "РаботаСФайлами.ХранилищеФайлаИзИнформационнойБазы"),
								КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьТекстЗапросаДляПолученияДвоичныхДанныхФайла(ТипХраненияФайла, ПолучениеФайлаИзАрхива);
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьДвоичныеДанныеФайлов.ДвоичныеДанныеФайла КАК ДвоичныеДанныеФайла
	|ИЗ
	|	РегистрСведений.УдалитьДвоичныеДанныеФайлов КАК УдалитьДвоичныеДанныеФайлов
	|ГДЕ
	|	УдалитьДвоичныеДанныеФайлов.Файл = &ФайлСсылка";
	Выборка = Запрос.Выполнить().Выбрать();
...
// Возвращает текст запроса для получения двоичных данных файла
//
Функция ПолучитьТекстЗапросаДляПолученияДвоичныхДанныхФайла(ТипХраненияФайла, ПолучениеФайлаИзАрхива = Ложь)
	...
	Иначе
		Результат =
		"ВЫБРАТЬ
		|	&ДвоичныеДанные КАК ДвоичныеДанныеФайла
		|ИЗ
		|	РегистрСведений.ХранилищеФайлов КАК ХранилищеФайлов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.МестаХраненияДвоичныхДанных КАК МестаХраненияДвоичныхДанных
		|		ПО ХранилищеФайлов.ХранилищеДвоичныхДанных = МестаХраненияДвоичныхДанных.ХранилищеДвоичныхДанных
		|ГДЕ
		|	ХранилищеФайлов.Файл = &ФайлСсылка";		
		
		ИмяРеквизитаХраненияДвоичныхДанных = РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ОпределитьРеквизитХраненияДвоичныхДанныхПоТипуХраненияФайлов(ТипХраненияФайла, ПолучениеФайлаИзАрхива);

		Результат = СтрЗаменить(Результат, "&ДвоичныеДанные", "МестаХраненияДвоичныхДанных." + ИмяРеквизитаХраненияДвоичныхДанных);
...
#Область ВыполнитьДействияПередЗаписьюПрисоединенногоФайла
...
Процедура ВыполнитьДействияПередЗаписьюПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("СправочникОбъект.ВерсииФайлов") Тогда
		Если Не Источник.ЭтоНовый() И Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ПрежниеЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Источник.Ссылка, "Автор, ПутьКФайлу");
			ПроверитьИзменениеАвтораФайлаПередЗаписью(ПрежниеЗначения, Источник);
			ПроверитьИзменениеПутиКФайлуПередЗаписью(ПрежниеЗначения, Источник);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
		МодульУправлениеДоступом.УстановитьБлокировкуПередЗаписьюВФайловойИБ(, Истина);
	КонецЕсли;
	
	ПроверитьНедопустимыеСимволыВИмениФайла(Источник.Наименование);
	...
	Иначе
		ПрежниеЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Источник.Ссылка,
			"ПометкаУдаления, Автор, Редактирует, Изменил, ПодписанЭП, Зашифрован, ПутьКФайлу");
		ПроверитьИзменениеАвтораФайлаПередЗаписью(ПрежниеЗначения, Источник);
		ПроверитьИзменениеПометкиУдаленияПередЗаписью(ПрежниеЗначения, Источник);
		ПроверитьЗаписьПодписанногоОбъектаПередЗаписью(ПрежниеЗначения, Источник);
		ПроверитьИзменениеПутиКФайлуПередЗаписью(ПрежниеЗначения, Источник);
		ОбновитьТекущуюВерсиюПередЗаписьюПрисоединенногоФайла(Источник);
	КонецЕсли;
	
	ПроверитьВладельцаФайлаПередЗаписью(Источник);
...
Процедура ПроверитьНедопустимыеСимволыВИмениФайла(ИмяФайла)
	НедопустимыеСимволы = ОбщегоНазначенияКлиентСервер.НайтиНедопустимыеСимволыВИмениФайла(ИмяФайла);
	Если НЕ ЗначениеЗаполнено(НедопустимыеСимволы) Тогда
		Возврат;
	КонецЕсли;
	ШаблонИсключения = НСтр("ru = 'В имени файла не должно быть следующих символов: %1'");
	ВсеНедопустимые = ОбщегоНазначенияКлиентСервер.ПолучитьНедопустимыеСимволыВИмениФайла();
	ТекстИсключения =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонИсключения, ВсеНедопустимые);
	ВызватьИсключение ТекстИсключения;
КонецПроцедуры

Процедура ПроверитьИзменениеПометкиУдаленияПередЗаписью(ПрежниеЗначения, Источник)
...
Процедура ПроверитьЗаписьПодписанногоОбъектаПередЗаписью(ПрежниеЗначения, Источник)
	...
	СсылкаПодписан    = ПрежниеЗначения.ПодписанЭП;
	СсылкаЗашифрован  = ПрежниеЗначения.Зашифрован;
	СсылкаЗанят       = ЗначениеЗаполнено(ПрежниеЗначения.Редактирует);
...
Процедура ПроверитьИзменениеПутиКФайлуПередЗаписью(ПрежниеЗначения, Источник)
	РазрешитьИзменениеПутиФайла = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "РазрешитьИзменениеПутиФайла"); 
	Если РазрешитьИзменениеПутиФайла = Истина Тогда
		Возврат;
	КонецЕсли;
	Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(Источник.ТипХраненияФайла) Тогда
		Возврат;
	КонецЕсли;
	ИзмененПуть = Источник.ПутьКФайлу <> ПрежниеЗначения.ПутьКФайлу;
	Если ИзмененПуть Тогда
		ВызватьИсключение(НСтр("ru = 'Запрещено изменять путь файла.'"), КатегорияОшибки.ОшибкаХранимыхДанных);
	КонецЕсли;
КонецПроцедуры

Процедура ОбновитьТекущуюВерсиюПередЗаписьюПрисоединенногоФайла(Источник)
	Если Источник.ЭтоГруппа Тогда
		Возврат;
	КонецЕсли;
	ЕстьВозможностьХранитьВерсии = ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия", Источник.Метаданные());
	ПроверитьВерсию = ЕстьВозможностьХранитьВерсии И ЗначениеЗаполнено(Источник.ТекущаяВерсия);
	Если НЕ ПроверитьВерсию Тогда
		Возврат;
	КонецЕсли;
	
	РеквизитыТекущейВерсии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Источник.ТекущаяВерсия, "Наименование");
	ОбновитьВерсию = РеквизитыТекущейВерсии.Наименование <> Источник.Наименование;
	Если НЕ ОбновитьВерсию Тогда
		Возврат;
	КонецЕсли;
	
	БлокировкаДанных = Новый БлокировкаДанных;
	ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Источник.ТекущаяВерсия.Метаданные().ПолноеИмя());
	ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", Источник.ТекущаяВерсия);
	БлокировкаДанных.Заблокировать();
	
	ОбъектВерсии = Источник.ТекущаяВерсия.ПолучитьОбъект();
	...
	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);
	ОбъектВерсии.Наименование = Источник.Наименование;
	// Чтобы не сработала подписка СкопироватьРеквизитыВерсииФайловВФайл.
	ОбъектВерсии.ДополнительныеСвойства.Вставить("ПереименованиеФайла", Истина);
	ОбъектВерсии.Записать();
	УстановитьПривилегированныйРежим(Ложь);
	УстановитьОтключениеБезопасногоРежима(Ложь);
КонецПроцедуры

Процедура ПроверитьИзменениеАвтораФайлаПередЗаписью(Знач ПрежниеЗначения, Знач Источник)
	...
	Если Источник.Редактирует <> ПрежниеЗначения.Редактирует
		И (ЭтоНедопустимыйАвторПередЗаписью(Источник.Редактирует, ТекущийПользователь) 
			Или ЭтоНедопустимыйАвторПередЗаписью(ПрежниеЗначения.Редактирует, ТекущийПользователь)) Тогда
	...
	Если Источник.Изменил <> ПрежниеЗначения.Изменил
		И (ЭтоНедопустимыйАвторПередЗаписью(Источник.Изменил, ТекущийПользователь) 
			Или ЭтоНедопустимыйАвторПередЗаписью(ПрежниеЗначения.Изменил, ТекущийПользователь)) Тогда
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для редактирования файла.'"), КатегорияОшибки.НарушениеПравДоступа);
	КонецЕсли;

КонецПроцедуры

Процедура ПроверитьВладельцаФайлаПередЗаписью(Источник)
...
// Для определения возможности изменения автора - можно изменять только с пустого значения на себя и наоборот.
// 
// Параметры:
//  АвторСсылка - Произвольный
//  ТекущийПользователь - см. Пользователи.АвторизованныйПользователь
// 
// Возвращаемое значение:
//  Булево
//
Функция ЭтоНедопустимыйАвторПередЗаписью(АвторСсылка, ТекущийПользователь)
	Результат = ЗначениеЗаполнено(АвторСсылка) 
...
#Область ДвоичныеДанныеФайлов

Функция ТекстЗапросаДвоичныеДанныеФайлов(ЕстьРеквизитТекущаяВерсия, ПолноеИмя)
	ДедупликацияВыполнена = РаботаСФайламиСлужебныйПовтИсп.ДедупликацияВыполнена();
	Если ЕстьРеквизитТекущаяВерсия Тогда
		Если ДедупликацияВыполнена Тогда
			Результат = ШаблонЗапросаДвоичныхДанныхВерсииДедупликацияВыполнена();
			
		Иначе
			Результат = ШаблонЗапросаДвоичныхДанныхВерсииДедупликацияНеВыполнена();
			
		КонецЕсли;
	Иначе
		Если ДедупликацияВыполнена Тогда
			Результат = ШаблонЗапросаДвоичныхДанныхБезВерсийДедупликацияВыполнена();

		Иначе
			Результат = ШаблонЗапросаДвоичныхДанныхБезВерсийДедупликацияНеВыполнена();

		КонецЕсли;
	КонецЕсли;
	Результат = СтрЗаменить(Результат, "&ТаблицаПрисоединенныеФайлы", ПолноеИмя);
	Возврат Результат;
КонецФункции

Функция ШаблонЗапросаДвоичныхДанныхВерсииДедупликацияВыполнена()
	Возврат "ВЫБРАТЬ
	|	ПрисоединенныеФайлы.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ПрисоединенныеФайлы.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВИнформационнойБазе)
	|			ТОГДА ХранилищеФайлов.ХранилищеДвоичныхДанных.ДвоичныеДанные
	|		КОГДА
	|			ПрисоединенныеФайлы.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВоВнешнемХранилищеДвоичныхДанных)
	|			ТОГДА ЕСТЬNULL(МестаХраненияДвоичныхДанных.ДвоичныеДанныеВОперационномВнешнемХранилище, НЕОПРЕДЕЛЕНО)
	|		КОГДА
	|			ПрисоединенныеФайлы.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВоВстроенномХранилищеДвоичныхДанных)
	|			ТОГДА ЕСТЬNULL(МестаХраненияДвоичныхДанных.ДвоичныеДанныеВОперационномВстроенномХранилище, НЕОПРЕДЕЛЕНО)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ХранилищеФайла
	|ИЗ
	|	&ТаблицаПрисоединенныеФайлы КАК ПрисоединенныеФайлы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ХранилищеФайлов КАК ХранилищеФайлов
	|		ПО ПрисоединенныеФайлы.Ссылка = ХранилищеФайлов.Файл
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестаХраненияДвоичныхДанных КАК МестаХраненияДвоичныхДанных
	|		ПО ХранилищеФайлов.ХранилищеДвоичныхДанных = МестаХраненияДвоичныхДанных.ХранилищеДвоичныхДанных
	|ГДЕ
	|	ПрисоединенныеФайлы.Ссылка В (&РазрешенныеПрисоединенныеФайлы)
	...
	|	&ТаблицаПрисоединенныеФайлы КАК ПрисоединенныеФайлы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ХранилищеФайлов КАК ХранилищеФайлов
	|		ПО ПрисоединенныеФайлы.ТекущаяВерсия = ХранилищеФайлов.Файл
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестаХраненияДвоичныхДанных КАК МестаХраненияДвоичныхДанных
	|		ПО ХранилищеФайлов.ХранилищеДвоичныхДанных = МестаХраненияДвоичныхДанных.ХранилищеДвоичныхДанных
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВерсииФайлов КАК ВерсииФайлов
	|		ПО ПрисоединенныеФайлы.ТекущаяВерсия = ВерсииФайлов.Ссылка
	|ГДЕ
	|	ПрисоединенныеФайлы.Ссылка В (&РазрешенныеПрисоединенныеФайлы)";	
КонецФункции

Функция ШаблонЗапросаДвоичныхДанныхВерсииДедупликацияНеВыполнена()
	Возврат "ВЫБРАТЬ
	|	ПрисоединенныеФайлы.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ХранилищеФайлов.Файл ЕСТЬ NULL
	|			ТОГДА УдалитьДвоичныеДанныеФайлов.ДвоичныеДанныеФайла
	|		ИНАЧЕ ВЫБОР
	|			КОГДА ПрисоединенныеФайлы.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВИнформационнойБазе)
	|				ТОГДА ХранилищеФайлов.ХранилищеДвоичныхДанных.ДвоичныеДанные
	|			КОГДА
	...
	|		ПО ПрисоединенныеФайлы.Ссылка = УдалитьДвоичныеДанныеФайлов.Файл
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестаХраненияДвоичныхДанных КАК МестаХраненияДвоичныхДанных
	|		ПО ХранилищеФайлов.ХранилищеДвоичныхДанных = МестаХраненияДвоичныхДанных.ХранилищеДвоичныхДанных
	|ГДЕ
	|	ПрисоединенныеФайлы.Ссылка В (&РазрешенныеПрисоединенныеФайлы)
	...
	|		ПО ПрисоединенныеФайлы.ТекущаяВерсия = ХранилищеФайлов.Файл
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УдалитьДвоичныеДанныеФайлов КАК УдалитьДвоичныеДанныеФайлов
	|		ПО ПрисоединенныеФайлы.ТекущаяВерсия = УдалитьДвоичныеДанныеФайлов.Файл
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестаХраненияДвоичныхДанных КАК МестаХраненияДвоичныхДанных
	|		ПО ХранилищеФайлов.ХранилищеДвоичныхДанных = МестаХраненияДвоичныхДанных.ХранилищеДвоичныхДанных
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВерсииФайлов КАК ВерсииФайлов
	|		ПО ПрисоединенныеФайлы.ТекущаяВерсия = ВерсииФайлов.Ссылка
...
Функция ШаблонЗапросаДвоичныхДанныхБезВерсийДедупликацияВыполнена()
	Возврат "ВЫБРАТЬ
	|	ПрисоединенныеФайлы.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ПрисоединенныеФайлы.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВИнформационнойБазе)
...
Функция ШаблонЗапросаДвоичныхДанныхБезВерсийДедупликацияНеВыполнена()
	Возврат "ВЫБРАТЬ
	|	ПрисоединенныеФайлы.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ХранилищеФайлов.Файл ЕСТЬ NULL
...
Функция РеквизитыРазрешенныхПрисоединенныхФайлов(ЕстьРеквизитТекущаяВерсия, ПолноеИмя, ПрисоединенныеФайлы)
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПрисоединенныеФайлы.Ссылка КАК Ссылка,
		|	ПрисоединенныеФайлы.Ссылка КАК СсылкаКлюч,
	...
	Если ЕстьРеквизитТекущаяВерсия Тогда
		...
		|	ВерсииФайлов.Владелец КАК ВладелецФайла,
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаПрисоединенныеФайлы", ПолноеИмя);
	Запрос.УстановитьПараметр("ПрисоединенныеФайлы", ПрисоединенныеФайлы);
	РезультатЗапроса = Запрос.Выполнить();
	
	Результат = РезультатЗапросаВСоответствие(РезультатЗапроса, "СсылкаКлюч");
	Возврат Результат;
КонецФункции
