			...
			Если ОбщегоНазначения.ЭтоWindowsСервер() Тогда 
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключиться с компьютера %1 к кластеру серверов по причине:
						|%2
						|
						|Если в тексте ошибки указано, что ""класс не зарегистрирован"" или не совпадает версия компоненты ""comcntr"" с требуемой,
...
// Устанавливает соединения с рабочими процессами.
//
// Параметры:
//   COMСоединитель - COMОбъект, com-объект V8*.ComConnector.
//   СоединениеСАгентомСервера - COMОбъект - com-объект, реализующий интерфейс IV8AgentConnection.
...
//
// Возвращаемое значение: 
//   Массив Из COMОбъект - com-объекты, реализующие интерфейс IV8ServerConnection.
//
...
//
// Параметры:
//   СоединенияСРабочимиПроцессами - Массив Из COMОбъект - com-объекты, реализующие интерфейс IV8ServerConnection.
//   Кластер - COMОбъект - com-объект, реализующий интерфейс IClusterInfo.
//   ИмяВКластере - Строка - имя информационной базы в кластере серверов.
