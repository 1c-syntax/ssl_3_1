...
Процедура СоздатьКлючевыеОперации(КлючевыеОперации) Экспорт
	...
	Таблица.Колонки.Добавить("ИмяКлючевойОперации", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(1000)));
...
Процедура ИзменитьКлючевыеОперации(КлючевыеОперации) Экспорт
	...
	Таблица = Новый ТаблицаЗначений;
	Таблица.Колонки.Добавить("ИмяКлючевойОперацииСтарое", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(1000)));
	Таблица.Колонки.Добавить("ИмяКлючевойОперацииНовое", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(1000)));
	Таблица.Колонки.Добавить("ЦелевоеВремя", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15,2)));
...
Процедура ЗагрузитьФайлОценкиПроизводительности(ИмяФайла, АдресХранилища) Экспорт
	...
	Попытка
		...
		Для Каждого Файл Из НайтиФайлы(КаталогФайлов, "*.XML") Цикл
			...
			ЗагрузитьФайлОценкиПроизводительностиApdexExport(ЧтениеXML, СуществующиеКлючевыеОперации, КлючевыеОперацииДляЗаписи, СырыеЗамерыДляЗаписи);  // @skip-check query-in-loop - порционная обработка данных.
			ЧтениеXML.Закрыть();
		КонецЦикла;
		УдалитьФайлы(ФайлХранилища);
		УдалитьФайлы(КаталогФайлов);
