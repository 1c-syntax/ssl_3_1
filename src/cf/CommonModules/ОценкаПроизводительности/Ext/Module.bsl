	...
	Попытка
		...
		Для Каждого КлючеваяОперация Из КлючевыеОперации Цикл
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				КлючеваяОперацияОбъект.ВыполненаСОшибкой = КлючеваяОперация.Признак;
...
Функция РазложитьСтрокуПоСловам(Знач Строка)
	...
	Для ПозицияСимвола = 1 По СтрДлина(Строка) Цикл
		...
		Если ТекСимвол = ВРег(ТекСимвол) 
			И (ОценкаПроизводительностиКлиентСервер.ТолькоСимволыНациональногоАлфавитаВСтроке(ТекСимвол)
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЭкспортОценкиПроизводительности", Истина);
	Методы.Вставить("ОчиститьРегистрыЗамерыВремени", Истина);
