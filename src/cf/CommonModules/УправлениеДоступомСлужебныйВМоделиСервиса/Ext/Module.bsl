...
Процедура ОтправитьСообщениеИзмененияГруппыДоступа(ОбъектГруппы, 
	...
	Если Не ОбъектГруппы.ЭтоГруппа Тогда
		...
		Если УпрощенныйРежим 
			...
			СписокПользователей = УчастникиПоПрофилю(Профиль, РодительПерсональныхГрупп);			
...
Процедура ОтправитьГруппыДоступаВМенеджерСервиса(ОтправитьБыстрымСообщением = Истина) Экспорт
	...
	Запрос.УстановитьПараметр("РодительПерсональныхГруппДоступа", 
		Справочники.ГруппыДоступа.РодительПерсональныхГруппДоступа(Истина));
...
Функция УчастникиПоПрофилю(Профиль, РодительПерсональныхГрупп)
	...
	|	И (ГруппыДоступа.Ссылка.Родитель = &РодительПерсональныхГрупп
	|		ИЛИ &ЭтоПрофильАдминистратор)");
	Запрос.УстановитьПараметр("Профиль", Профиль);
	Запрос.УстановитьПараметр("РодительПерсональныхГрупп", РодительПерсональныхГрупп);
	Запрос.УстановитьПараметр("ЭтоПрофильАдминистратор", Профиль = УправлениеДоступом.ПрофильАдминистратор());
...
Функция ТекстЗапросаВыборкиГруппДоступа()
	...
	Иначе
		Возврат "ВЫБРАТЬ
			...
			|	Справочник.ГруппыДоступа КАК ГруппыДоступа
			|ГДЕ
			|	ГруппыДоступа.Родитель <> &РодительПерсональныхГруппДоступа
			|
			|УПОРЯДОЧИТЬ ПО
			|	Группа ИЕРАРХИЯ";
	КонецЕсли;
