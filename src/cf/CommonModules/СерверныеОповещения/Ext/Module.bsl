...
Процедура ОтправитьСерверноеОповещениеСИдентификаторомГруппы(ИмяОповещения, Результат, Адресаты,
	...
	Если ЗапуститьДоставкуСОтсрочкой Тогда
		Если ЗначениеЗаполнено(ДополнительныеПараметры.СобытиеЖурналаПриОтсрочкеДоставки) Тогда
			ЗаписьЖурналаРегистрации(ДополнительныеПараметры.СобытиеЖурналаПриОтсрочкеДоставки,
				УровеньЖурналаРегистрации.Информация,,, ДополнительныеПараметры.КомментарийЖурналаПриОтсрочкеДоставки);
		КонецЕсли;
		ЗапуститьДоставкуСерверныхОповещенийСОтсрочкой();
	КонецЕсли;
...
Функция НедоставленныеСерверныеОповещенияСеанса(Знач Параметры) Экспорт
	...
	Если ПериодическаяОтправкаДанных Тогда
		...
		МоментНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ИмяПараметра = "СтандартныеПодсистемы.БазоваяФункциональность.СерверныеОповещения.ОбновлениеАктивностиСеанса";
		ОбновлениеАктивностиСеанса = ДополнительныеПараметры.Получить(ИмяПараметра) <> Неопределено;
		Попытка
			Если ОбновлениеАктивностиСеанса И Не СеансАдминистратораСервиса Тогда
				ОбновитьАктивностьСеанса();
			КонецЕсли;
		Исключение
			ОбработатьОшибку(ИнформацияОбОшибке());
		КонецПопытки;
		ДобавитьОсновнойПоказатель(Показатели, МоментНачала,
			"СерверныеОповещения.ОбновитьАктивностьСеанса");
		
		Если ОбновлениеАктивностиСеанса Тогда
			МоментНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
			Результат.Вставить("РегистрироватьПоказатели", РегистрироватьПоказателиСерверныхОповещений());
			ДобавитьОсновнойПоказатель(Показатели, МоментНачала,
				"СерверныеОповещения.РегистрироватьПоказателиСерверныхОповещений");
		КонецЕсли;
		...
		Если Не СеансАдминистратораСервиса Тогда
			...
			Если ОбновлениеАктивностиСеанса
			 Или ЗаполнитьИдентификаторыОбсуждений Тогда
			...
			КонецЕсли;
		ИначеЕсли ОбновлениеАктивностиСеанса Тогда
			МоментНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
			Попытка
...
Функция РегистрироватьПоказателиСерверныхОповещений() Экспорт
...
Процедура ПриИзмененииКонстантыРегистрироватьПоказателиСерверныхОповещений(Регистрировать) Экспорт
	...
	ОтправитьСерверноеОповещение(
		"СтандартныеПодсистемы.БазоваяФункциональность.СерверныеОповещения.РегистрацияПоказателейИзменена",
		Регистрировать, Неопределено, Истина);
	
КонецПроцедуры

Процедура ОбновитьАктивностьСеанса()
	
	УстановитьПривилегированныйРежим(Истина);
	
	КлючСеанса = КлючСеанса();
	НаборЗаписей = СлужебныйНаборЗаписей(РегистрыСведений.ПериодическиеСерверныеОповещения);
	НаборЗаписей.Отбор.КлючСеанса.Установить(КлючСеанса);
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ПериодическиеСерверныеОповещения");
	ЭлементБлокировки.УстановитьЗначение("КлючСеанса", КлючСеанса);
	...
	Попытка
		...
		Если НаборЗаписей.Количество() = 1 Тогда
			НаборЗаписей[0].ДатаАктивности = ТекущаяДатаСеанса();
			НаборЗаписей.Записать();
		КонецЕсли;
...
Функция ВсеСеансыСпят()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Граница", ТекущаяДатаСеанса() - 25*60);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	ПериодическиеСерверныеОповещения.ДатаАктивности > &Граница";
...
// Для процедуры ОтправитьСерверноеОповещениеСИдентификаторомГруппы.
Процедура ЗапуститьДоставкуСерверныхОповещенийСОтсрочкой()
...
Функция ПараметрыСерверныхОповещенийЭтогоСеанса() Экспорт
	...
	Если ЗначениеЗаполнено(ПериодическиеОповещения) Тогда
		...
		НоваяЗапись.ДатаАктивности = НоваяЗапись.ДатаДобавления;
		НаборЗаписей.Записать();
...
Процедура ПриИзмененииКонстантыДоставлятьСерверныеОповещенияБезСистемыВзаимодействия(ДоставлятьБезСВ) Экспорт
	...
	ОтправитьСерверноеОповещение(
		"СтандартныеПодсистемы.БазоваяФункциональность.СерверныеОповещения.ДоставкаБезСистемыВзаимодействияИзменена",
