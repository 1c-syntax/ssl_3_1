...
Функция СлужебныйПользовательИБ()
	...
	ИначеЕсли Не ПользовательИБ.Роли.Содержит(РольДляЗапуска) Тогда
	...
	Если Запрос.Выполнить().Пустой() Тогда
		...
		ЭлементБлокировки.УстановитьЗначение("Ссылка",
			Справочники.Пользователи.ПолучитьСсылку(ИдентификаторПользователяИБ));
		НачатьТранзакцию();
		Попытка
			Блокировка.Заблокировать();
			Если Запрос.Выполнить().Пустой() Тогда
...
Функция СистемаВзаимодействийПодключена() Экспорт
	...
	Если Константы.ДоставлятьСерверныеОповещенияБезСистемыВзаимодействия.Получить() Тогда
		ПоследняяПроверка.Подключена = Ложь;
	КонецЕсли;
