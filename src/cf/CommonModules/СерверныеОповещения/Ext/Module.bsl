...
//                         - Массив из Строка - если в массиве есть "*", тогда для всех сеансов пользователя.
//
...
//    
//    * ЗаписатьБезусловно - Булево - если Истина, тогда оповещение будет записано в регистр,
//                                    даже когда уведомления клиента доступны, например,
//                                    для получения сообщений из оповещений.
//
Функция ДополнительныеПараметрыОтправки() Экспорт
	...
	Результат.Вставить("ЗаписатьБезусловно", Ложь);
...
Процедура ОтправитьСерверноеОповещениеСИдентификаторомГруппы(ИмяОповещения, Результат, Адресаты,
	...
	ДатаДобавленияМиллисекунды = Миллисекунды(ДатаДобавления);
	...
	НоваяЗапись.СодержимоеОповещения = Новый ХранилищеЗначения(СодержимоеОповещения, Новый СжатиеДанных(9));
	...
	УведомленияКлиентаДоступны = СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны();
	...
	ИначеЕсли ОтправитьСразу
	        И (УведомленияКлиентаДоступны
	           Или ТекущийПользовательЗарегистрированВСистемеВзаимодействия()) Тогда
	...
	Если ЗначениеЗаполнено(НоваяЗапись.ОтсрочкаЗаписиВСистемуВзаимодействия)
	   И Не УведомленияКлиентаДоступны
	...
	Если Не ДополнительныеПараметры.Заменить
	   И УведомленияКлиентаДоступны
	   И Не ДополнительныеПараметры.ЗаписатьБезусловно
	   И ЗначениеЗаполнено(НоваяЗапись.ДатаЗаписиВСистемуВзаимодействия) Тогда
...
Процедура УдалитьПоследнееНедоставленноеОповещение(ИдентификаторГруппы, ВидОповещенияВГруппе,
	...
	Иначе
		...
		Если ОтсрочкаДоставки > Выборка.ОтсрочкаЗаписиВСистемуВзаимодействия Тогда
			ОтсрочкаДоставки = Выборка.ОтсрочкаЗаписиВСистемуВзаимодействия;
		КонецЕсли;
	КонецЕсли;
...
Функция ОтправитьСообщениеСразу(ИдентификаторОповещения, ДатаДобавления, СодержимоеОповещения)
	...
	Если СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны() Тогда
		Возврат ОтправитьУведомление(Данные);
	КонецЕсли;
...
Функция Миллисекунды(ДатаДобавления = Неопределено)
	
	ДатаВМиллисекундах = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	Результат = ДатаВМиллисекундах - Цел(ДатаВМиллисекундах/1000)*1000;
	
	Если ДатаДобавления = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Если ДатаДобавления < Свойства.ДатаДобавления
	   И Свойства.ДатаДобавления - ДатаДобавления < 5 Тогда
		
		ДатаДобавления = Свойства.ДатаДобавления;
		Результат = Свойства.ДатаДобавленияМиллисекунды;
	КонецЕсли;
	
	Если ДатаДобавления = Свойства.ДатаДобавления
	   И Результат <= Свойства.ДатаДобавленияМиллисекунды Тогда
		
		Результат = Свойства.ДатаДобавленияМиллисекунды + 1;
		Если Результат > 999 Тогда
			Результат = 0;
			ДатаДобавления = ДатаДобавления + 1;
		КонецЕсли;
	КонецЕсли;
	
	Свойства.ДатаДобавления = ДатаДобавления;
	Свойства.ДатаДобавленияМиллисекунды = Результат;
...
Функция НедоставленныеСерверныеОповещенияСеанса(Знач Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДатаДобавленияОповещения = Выборка.ДатаДобавления;
		Если ДатаДобавленияОповещения > ТекущаяДатаСеанса Тогда
			ДатаДобавленияОповещения = НоваяДатаПоследнегоОповещения;
		КонецЕсли;
		Если ЗначениеЗаполнено(Содержимое.ИмяОповещения) Тогда
			...
			Данные.ДатаДобавления          = ДатаДобавленияОповещения;
			Если Не ДлительныеОперации.ПропуститьОповещение(Данные) Тогда
				НовыеСерверныеОповещения.Добавить(Данные);
			КонецЕсли;
		КонецЕсли;
		НоваяДатаПоследнегоОповещения = ДатаДобавленияОповещения;
	КонецЦикла;
...
Функция РегистрироватьПоказателиСерверныхОповещений()
	...
	Возврат Константы.РегистрироватьПоказателиСерверныхОповещений.Получить() = Истина;
...
Процедура ПодготовитьСерверныеОповещения(СостояниеОтправки, МинимальныйПериодПоПользователям = Неопределено)
	...
	АктивныеСеансыПоКлючам = Новый Соответствие;
	Сеансы = ПолучитьСеансыИнформационнойБазы();
	Для Каждого Сеанс Из Сеансы Цикл
		АктивныеСеансыПоКлючам.Вставить(КлючСеанса(Сеанс), Сеанс);
	КонецЦикла;
	
	ПериодическиеОповещения = ПериодическиеСерверныеОповещения(МинимальныйПериодПоПользователям,
		АктивныеСеансыПоКлючам);
	...
	Для Каждого КлючИЗначение Из ПериодическиеОповещения Цикл
		...
		Если ТипЗнч(ДатаПроверки) = Тип("Дата")
		   И ТекущаяДатаСеанса >= ДатаПроверки
		...
		Если МодульОтправки = ПользователиСлужебный Тогда
			ПараметрыПроцедуры = Новый Структура;
			ПараметрыПроцедуры.Вставить("ВариантыПараметров", КлючИЗначение.Значение.ВариантыПараметров);
			ПараметрыПроцедуры.Вставить("АктивныеСеансыПоКлючам", АктивныеСеансыПоКлючам);
		Иначе
			ПараметрыПроцедуры = КлючИЗначение.Значение.ВариантыПараметров;
		КонецЕсли;
		Попытка
			МодульОтправки.ПриОтправкеСерверногоОповещения(ИмяОповещения, ПараметрыПроцедуры);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
//                           Если равен Неопределено, тогда свойства нет.
//
...
Функция СохраняемоеСерверноеОповещение(Оповещение)
	...
	Если Оповещение.Параметры <> Неопределено Тогда
...
Процедура ОтправитьПодготовленныеСерверныеОповещения(СостояниеОтправки, МинимальныйПериодПоПользователям)
	...
	УведомленияКлиентаДоступны = СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны();
	ОтправкаДоступна = УведомленияКлиентаДоступны;
	
	Если Не УведомленияКлиентаДоступны
	...
	ИдентификаторыПоследнихОповещений = Новый Массив;
	
	Выборка = НовыеСерверныеОповещения(?(ЗначениеЗаполнено(СостояниеОтправки.ДатаОповещенияСОшибкой),
		СостояниеОтправки.ДатаОповещенияСОшибкой, СостояниеОтправки.ДатаПоследнегоОповещения), Ложь);
	...
	Контекст.Вставить("ДатаОповещенияСОшибкой", '00010101');
	...
	ТекущаяДатаСеанса = ТекущаяДатаСеанса();
	КорректируемыеСерверныеОповещения = Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		...
		ДатаДобавленияОповещения = Выборка.ДатаДобавления;
		Если ДатаДобавленияОповещения > ТекущаяДатаСеанса Тогда
			Если ДатаДобавленияОповещения > ТекущаяДатаСеанса + 60 Тогда
				КорректируемыеСерверныеОповещения.Добавить(Выборка.ИдентификаторОповещения);
			КонецЕсли;
			ДатаДобавленияОповещения = ДатаПоследнегоОповещения;
		КонецЕсли;
		Если ОтправкаДоступна
		   И ЗначениеЗаполнено(Содержимое.ИмяОповещения)
		   И Выборка.Отправлять Тогда
			...
			Если ТипЗнч(Содержимое.Адресаты) <> Тип("Соответствие") Тогда
				Если Не СообщениеУжеОтправлено(СостояниеОтправки, Выборка, "ВсеПользователи") Тогда
					...
					СообщениеОтправлено = ?(УведомленияКлиентаДоступны, ОтправитьУведомление(Данные),
						ОтправитьСообщение(Данные, ИдентификаторОбщегоОбсуждения));
					...
					Иначе
						...
						КонецЕсли;
						Если Не ЗначениеЗаполнено(Контекст.ДатаОповещенияСОшибкой) Тогда
							Контекст.ДатаОповещенияСОшибкой = Данные.ДатаДобавления;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			Иначе
			...
			КонецЕсли;
		КонецЕсли;
		Если ДатаПоследнегоОповещения <> ДатаДобавленияОповещения Тогда
			ИдентификаторыПоследнихОповещений = Новый Массив;
		КонецЕсли;
		ДатаПоследнегоОповещения = ДатаДобавленияОповещения;
		Если Не ЗначениеЗаполнено(Контекст.ДатаОповещенияСОшибкой) Тогда
			ИдентификаторыПоследнихОповещений.Добавить(Выборка.ИдентификаторОповещения);
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(Контекст.ДатаОповещенияСОшибкой)
	   И Контекст.ДатаОповещенияСОшибкой < ДатаПоследнегоОповещения - МаксимальныйПериодПовторенияДоставки() Тогда
		
		Контекст.ДатаОповещенияСОшибкой = ДатаПоследнегоОповещения - МаксимальныйПериодПовторенияДоставки();
	КонецЕсли;
	
	СостояниеОтправки.ДатаОповещенияСОшибкой   = Контекст.ДатаОповещенияСОшибкой;
	СостояниеОтправки.ДатаПоследнегоОповещения = ДатаПоследнегоОповещения;
	СостояниеОтправки.ИдентификаторыПоследнихОповещений = ИдентификаторыПоследнихОповещений;
	...
	ОбновитьСостояниеОтправки(СостояниеОтправки,
		...
		|ИдентификаторыПоследнихОповещений,
		|ДатаПоследнейОчисткиСообщений,
		|ДатыОповещенийСОшибкамиПоПользователям,
		|ДатыУспешнойОтправкиОповещенийПоПользователям");
	
	Если ЗначениеЗаполнено(КорректируемыеСерверныеОповещения) Тогда
		ИсправитьДатуДобавленияОповещений(КорректируемыеСерверныеОповещения, ДатаПоследнегоОповещения);
	КонецЕсли;
...
Процедура ОтправитьАдресноеСообщение(Данные, Адресаты, Контекст)
	...
	Если УведомленияКлиентаДоступны Или Адресаты.Количество() > 20 Тогда
		Данные.Адресаты = Адресаты;
		УстановитьДатыОповещенийСОшибками(Данные, Контекст.ДатыОповещенийСОшибкамиПоПользователям);
		СообщениеОтправлено = ?(УведомленияКлиентаДоступны, ОтправитьУведомление(Данные),
			ОтправитьСообщение(Данные, Контекст.ИдентификаторОбщегоОбсуждения));
...
Процедура ИсправитьДатуДобавленияОповещений(ИдентификаторыОповещений, ДатаПоследнегоОповещения)
	
	ДатаДобавленияМиллисекунды = 999;
	Индекс = ИдентификаторыОповещений.Количество();
	Пока Индекс > 0 Цикл
		Индекс = Индекс - 1;
		ИдентификаторОповещения = ИдентификаторыОповещений[Индекс];
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОтправленныеСерверныеОповещения");
		ЭлементБлокировки.УстановитьЗначение("ИдентификаторОповещения", ИдентификаторОповещения);
		...
		Попытка
			...
			Если НаборЗаписей.Количество() = 1 Тогда
				НаборЗаписей[0].ДатаДобавления = ДатаПоследнегоОповещения;
				НаборЗаписей[0].ДатаДобавленияМиллисекунды = ДатаДобавленияМиллисекунды;
				НаборЗаписей.Записать();
			КонецЕсли;
		...
		Исключение
			...
			ВызватьИсключение;
		КонецПопытки;
		ДатаДобавленияМиллисекунды = ДатаДобавленияМиллисекунды - 1;
	КонецЦикла;
...
Функция СообщениеУжеОтправлено(СостояниеОтправки, Выборка, ИдентификаторПользователяИБ)
	...
	ДатаОповещенияСОшибкой =
		СостояниеОтправки.ДатыОповещенийСОшибкамиПоПользователям.Получить(ИдентификаторПользователяИБ);
	...
	Возврат Выборка.ДатаДобавления < ДатаОповещенияСОшибкой
		Или СостояниеОтправки.ИдентификаторыПоследнихОповещений.Найти(Выборка.ИдентификаторОповещения) <> Неопределено;
...
Функция НовыеСерверныеОповещения(ДатаПоследнегоОповещения, ТолькоДляТекущегоПользователя = Истина)
	...
	ПолеОтправлять = "ИСТИНА";
	УсловиеОтбора = ?(ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных(), "ИСТИНА",
			"ОтправленныеСерверныеОповещения.ОбластьДанныхВспомогательныеДанные = 0");
	
	Если ТолькоДляТекущегоПользователя Тогда
	...
	Если СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны() Тогда
		Если ТолькоДляТекущегоПользователя Тогда
			УсловиеОтбора = УсловиеОтбора + "
			|	И НЕ ОтправленныеСерверныеОповещения.ВидОповещенияВГруппе В (&ВидыОповещенийДлительныхОпераций)";
		Иначе
			ПолеОтправлять = "НЕ ОтправленныеСерверныеОповещения.ВидОповещенияВГруппе В (&ВидыОповещенийДлительныхОпераций)";
		КонецЕсли;
		Запрос.УстановитьПараметр("ВидыОповещенийДлительныхОпераций",
			ДлительныеОперации.ВидыОповещенийДлительныхОпераций());
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Отправлять", ПолеОтправлять);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Отбор", УсловиеОтбора);
...
//
Функция ПериодическиеСерверныеОповещения(МинимальныйПериодПоПользователям, АктивныеСеансыПоКлючам)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если АктивныеСеансыПоКлючам.Получить(Выборка.КлючСеанса) = Неопределено Тогда
...
//   * ИдентификаторыПоследнихОповещений - Массив из Строка - идентификаторы последних
//       оповещений с одинаковой датой последнего оповещения.
...
Функция СостояниеОтправкиСерверныхОповещений()
	...
	Состояние.Вставить("ИдентификаторыПоследнихОповещений", Новый Массив);
	...
	ТекущаяДатаСеанса = ТекущаяДатаСеанса();
	Если Состояние.ДатаПоследнейПроверки > ТекущаяДатаСеанса Тогда
		Состояние.ДатаПоследнейПроверки = ТекущаяДатаСеанса;
	КонецЕсли;
	Если Состояние.ДатаПоследнегоОповещения > ТекущаяДатаСеанса Тогда
		Состояние.ДатаПоследнегоОповещения = ТекущаяДатаСеанса;
	КонецЕсли;
	Если Состояние.ДатаОповещенияСОшибкой > ТекущаяДатаСеанса Тогда
		Состояние.ДатаОповещенияСОшибкой = ТекущаяДатаСеанса;
	КонецЕсли;
...
Процедура ОбновитьСостояниеОтправки(НовоеСостояниеОтправки, ИменаСвойств)
	...
	Попытка
		...
		Если Записать Тогда
			...
			МенеджерЗначения.Значение = Новый ХранилищеЗначения(СостояниеОтправки, Новый СжатиеДанных(9));
...
Процедура ЗапуститьДоставкуСерверныхОповещенийСОтсрочкой(Запущено = Ложь)
	...
	Если Не СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны()
	   И Не ТекущийПользовательЗарегистрированВСистемеВзаимодействия()
	...
	// Имя процедуры должно быть, как указано в функции ИмяМетодаЗаданияДоставкиСерверныхОповещенийСОтсрочкой.
	ФоновыеЗадания.Выполнить("СерверныеОповещения.ДоставкаСерверныхОповещенийСОтсрочкой",,, НаименованиеЗадания);
...
Процедура ДоставкаСерверныхОповещенийСОтсрочкой() Экспорт
	...
	Если ДоставкаСерверныхОповещенийСОтсрочкойУжеВыполняется(ТекущееФоновоеЗадание)
	 Или Не СерверныеОповещенияСлужебныйПовтИсп.УведомленияКлиентаДоступны()
	   И Не ТекущийПользовательЗарегистрированВСистемеВзаимодействия() Тогда
		Возврат;
	КонецЕсли;
...
Функция ПараметрыСерверныхОповещенийЭтогоСеанса() Экспорт
	...
	Если ЗначениеЗаполнено(ПериодическиеОповещения) Тогда
		...
		НоваяЗапись.ИдентификаторПользователяИБ =
			ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор;
		НоваяЗапись.Оповещения = Новый ХранилищеЗначения(СохраняемыеПериодическиеОповещения(ПериодическиеОповещения), Новый СжатиеДанных(9));
...
Процедура УстановитьИспользованиеЗаданияОтправкаСерверныхОповещенийКлиентамБезПопытки(Знач Использование)
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		Использование = Ложь;
		ОтборНеразделенных = Новый Структура("Метаданные, Использование", МетаданныеЗадания, Истина);
		// АПК:453-выкл - №760.3 Допустимо получить некорректно созданное задание для отключения.
		Задания = РегламентныеЗадания.ПолучитьРегламентныеЗадания(ОтборНеразделенных);
		// АПК:453-вкл
		Для Каждого Задание Из Задания Цикл
			РегламентныеЗаданияСервер.УстановитьИспользованиеРегламентногоЗадания(Задание, Ложь);
		КонецЦикла;
		Отбор.Вставить("ОбластьДанных", 0);
	КонецЕсли;
	Отбор.Вставить("Использование", Не Использование);
...
Функция СлужебныйПользовательИБ()
	...
	СсылкаНового = Справочники.Пользователи.ПолучитьСсылку(
		Новый УникальныйИдентификатор("ea3b6bcf-d6d8-11ee-886a-b06ebfbf08c7"));
	
	Возврат Пользователи.СлужебныйПользовательИБ(Свойства, СсылкаНового);
...
//
// Возвращаемое значение:
//  Булево - Истина, если уведомление успешно принято для отправки.
//
Функция ОтправитьУведомление(Данные)
	
	Если ЗначениеЗаполнено(Данные.Адресаты) Тогда
		...
		Для Каждого КлючИЗначение Из Данные.Адресаты Цикл
			Если ТипЗнч(КлючИЗначение.Значение) <> Тип("Массив") Тогда
				...
				Прервать;
			КонецЕсли;
			Для Каждого КлючСеанса Из КлючИЗначение.Значение Цикл
				ЧастиКлючаСеанса = СтрРазделить(КлючСеанса, " ", Ложь);
				НомерСеансаСтрокой = ?(ЧастиКлючаСеанса.Количество() > 1,
					ЧастиКлючаСеанса.Получить(ЧастиКлючаСеанса.ВГраница()), "0");
				НомерСеанса = ОписаниеЧисла.ПривестиЗначение(НомерСеансаСтрокой);
				...
				КонецЕсли;
			КонецЦикла;
			Если НомераСеансов = Неопределено Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	Иначе
	...
	КлючУведомления = СерверныеОповещенияСлужебныйКлиентСервер.КлючУведомленийСерверныхОповещений();
	ОтправитьУведомлениеКлиента(КлючУведомления, Данные, НомераСеансов);
	
	Возврат Истина;
	
КонецФункции

// Позволяет подключиться расширением для журналирования отправляемых уведомлений.
Процедура ОтправитьУведомлениеКлиента(КлючУведомления, Данные, НомераСеансов)
	МенеджерУведомленийКлиента().ОтправитьУведомление(КлючУведомления, Данные, НомераСеансов);
КонецПроцедуры
...
Процедура ОчиститьУстаревшиеСообщения(СостояниеОтправки)
	...
	Если СостояниеОтправки.ДатаПоследнейОчисткиСообщений + Период > НоваяДата Тогда
		Если СостояниеОтправки.ДатаПоследнейОчисткиСообщений > НоваяДата Тогда
...
Функция МенеджерУведомленийКлиента()
	...
	Возврат Вычислить("УведомленияКлиента"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство глобального контекста платформы.
	// АПК:488-вкл
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ОтправкаСерверныхОповещенийКлиентам");
	КонецЕсли;
