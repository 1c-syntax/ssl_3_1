...
Процедура ПриПолученииНастройкиИспользованияМобильногоСервиса(Используется) Экспорт
	...
	Используется = МодульСервисМобильнойПодписи.ИспользоватьСервисМобильнойПодписи() 
		Или МодульСервисМобильнойПодписи.ИспользоватьГосключ();
	// Конец Локализация
...
// Помечает на удаление настройку для подписания в сервисе.
// 
// Параметры:
//  Программа - СправочникСсылка
//  УникальныйИдентификатор - УникальныйИдентификатор
//  СтандартнаяОбработка - Булево
//
Процедура ПриИзмененииПометкиУдаленияПриложения(Программа, УникальныйИдентификатор, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		...
		МодульСервисМобильнойПодписиСлужебный.ИзменитьПометкуУдаленияНастройки(Программа, УникальныйИдентификатор, СтандартнаяОбработка);
	КонецЕсли;
...
// При заполнении приложений электронной подписи.
// 
// Параметры:
//  ТаблицаПриложений - ТаблицаЗначений
//
Процедура ПриЗаполненииПриложенийЭлектроннойПодписи(ТаблицаПриложений) Экспорт
	
	// Локализация
	Если ЭлектроннаяПодписьСлужебный.ИспользоватьСервисОблачнойПодписи() Тогда
		МодульСервисКриптографииDSSСлужебный = ОбщегоНазначения.ОбщийМодуль("СервисКриптографииDSSСлужебный");
		МодульСервисКриптографииDSSСлужебный.ДополнитьВыборкуПрограмм(ТаблицаПриложений);
	КонецЕсли;
	
	Если ЭлектроннаяПодпись.ИспользоватьСервисМобильнойПодписи() Тогда
		МодульСервисМобильнойПодписиСлужебный = ОбщегоНазначения.ОбщийМодуль("СервисМобильнойПодписиСлужебный");
		МодульСервисМобильнойПодписиСлужебный.ДополнитьВыборкуПриложений(ТаблицаПриложений);
	КонецЕсли;
	// Конец Локализация
...
Процедура ПриЗаполненииМашиночитаемыхДоверенностей(Форма, ПодписанныйОбъект = Неопределено) Экспорт
	...
	Для Каждого ТекущаяСтрока Из Подписи Цикл
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.МашиночитаемаяДоверенность) И ОтборЗаполнен Тогда
...
Процедура ПриПолученииОтпечатковОблачногоСервиса(МассивОтпечатков) Экспорт
	...
	// Локализация
	МодульСервисКриптографииDSSСлужебный = ОбщегоНазначения.ОбщийМодуль("СервисКриптографииDSSСлужебный");
	МассивСертификатов = МодульСервисКриптографииDSSСлужебный.ПолучитьВсеСертификаты(Ложь);

	Для Каждого Сертификат Из МассивСертификатов Цикл
		Если ЗначениеЗаполнено(Сертификат.Отпечаток) Тогда
			ДанныеОтпечатка = Base64Строка(ПолучитьДвоичныеДанныеИзHexСтроки(Сертификат.Отпечаток));
			Если ЗначениеЗаполнено(ДанныеОтпечатка) Тогда
				МассивОтпечатков.Добавить(ДанныеОтпечатка);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	// Конец Локализация
...
Процедура ПриДобавленииСвойствСертификатовОблачногоСервиса(ТаблицаСвойствСертификатов, БезОтбора) Экспорт
	...
	Сертификаты = МодульСервисКриптографииDSSСлужебный.ПолучитьВсеСертификаты(Истина);
	...
	Для каждого СтрокаТаблицы Из Сертификаты Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСертификата) Тогда
			МассивСертификатов.Добавить(Новый СертификатКриптографии(ДанныеСертификата));
		КонецЕсли;
	КонецЦикла;
...
// При настройке формы общих настроек.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  РеквизитПутьКДанным - Строка
//  РазрешенДоступКИнтернетСервисам - Булево
//  ОбщиеНастройки - см. ЭлектроннаяПодпись.ОбщиеНастройки
//  СтандартнаяОбработка - Булево
//
Процедура ПриНастройкеФормыОбщихНастроек(Форма, РеквизитПутьКДанным, РазрешенДоступКИнтернетСервисам, ОбщиеНастройки, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьГосключ"
			Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьСервисМобильнойПодписи" Или РеквизитПутьКДанным = ""
			Или РеквизитПутьКДанным = "РазрешитьДоступКИнтернетСервисамЭлектроннойПодписи"
			...
			ИспользоватьГосключ = МодульСервисМобильнойПодписи.ИспользоватьГосключ();
			Элементы.НастройкиОтправкиВГосключ.Видимость = ИспользоватьГосключ;
			Элементы.ПодписантыСервисаМобильнойПодписи.Видимость = ИспользоватьГосключ Или МодульСервисМобильнойПодписи.ИспользоватьСервисМобильнойПодписи();
			
			Элементы.ИспользоватьСервисМобильнойПодписи.Видимость = Ложь;
		КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодписьСервисаDSS") Тогда
		...
		Если (РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЭлектронныеПодписи" Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьШифрование"
			...
			Если ОбщегоНазначения.РазделениеВключено() Тогда
				...
				ЭлектроннаяПодписьСлужебный.ЗаполнитьСписокТиповПодписейКриптографии(
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ОбновлениеСостоянияЗаявленияНаВыпускСертификата");
	КонецЕсли;
