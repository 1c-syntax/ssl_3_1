...
Функция ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, Знач Источник, Знач ТипXDTO, 
	...
	ВложенныеРасширения = Новый Соответствие;
	ВложенныеРасширения.Вставить("РасширенияКлючевыхСвойств", Неопределено);
	ВложенныеРасширения.Вставить("РасширенияТабличныхЧастей", Неопределено);
	
	ДополнитьСвойстваПакетаИзРасширений(СвойстваИсточника, КомпонентыОбмена, Расширения, ВложенныеРасширения);
	...
	Для Каждого Свойство Из СвойстваИсточника Цикл
		...
		Если ТипЗнч(ЗначениеСвойства) = Тип("Структура") Тогда
			...
			ЗначениеСвойства.Свойство("Расширения", Расширения);
		...
		ИначеЕсли ТипЗнч(Свойство.Тип) = Тип("ТипОбъектаXDTO") Тогда
			...
			ИначеЕсли ЭтоТаблицаОбъекта(Свойство) Тогда
				ТипСвойства = "Таблица";
			ИначеЕсли Свойство.Имя = КлассКлючевыеСвойстваФормата()
				Или СтрНайти(Свойство.Тип.Имя, КлассКлючевыеСвойстваФормата()) > 0 Тогда
				ТипСвойства = КлассКлючевыеСвойстваФормата();
			Иначе
				Если СвойствоНайдено Тогда
					Если ТипЗнч(ЗначениеСвойства) = Тип("Структура") 
						И (ЗначениеСвойства.Свойство("Значение")
							Или СтрНайти(Свойство.Тип, КлассОбщиеСвойства()) > 0) Тогда
		...
		Попытка
			...
			Иначе
				...
				Если ТипСвойства = КлассКлючевыеСвойстваФормата() Тогда
					...
					РасширениеКлючевогоСвойства = УстановитьРасширениеКлючевыхСвойств(ВложенныеРасширения, Свойство.Тип.Имя);
					ЗначениеXDTO = ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, ЗначениеСвойства, Свойство.Тип, СсылкиИзОбъекта, , РасширениеКлючевогоСвойства);
				...
				ИначеЕсли ТипСвойства = "Таблица" Тогда
					...
					РасширениеТабличнойЧасти = УстановитьРасширениеСтрокиТабличнойЧасти(ВложенныеРасширения, Свойство.Имя);
					
					Для Каждого ИсточникСтрока Из ЗначениеСвойства Цикл
						
						ПриемникСтрока = ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, ИсточникСтрока, ТипТаблицы, СсылкиИзОбъекта, Ложь, РасширениеТабличнойЧасти);
...
Функция ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, Режим = "ОтправкаПолучение", УзелОбмена = Неопределено) Экспорт
	...
	НастройкиПланаОбмена = ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(
		ИмяПланаОбмена,
		"ВерсииФорматаОбмена, РасширенияФорматаОбмена");
		
	РежимОтправка  = СтрНайти(Режим, "Отправка") > 0;
	РежимПолучение = СтрНайти(Режим, "Получение") > 0;
	
	Для Каждого Версия Из НастройкиПланаОбмена.ВерсииФорматаОбмена Цикл
		
		РасширенияВерсии = Новый Массив;
		Для Каждого Расширение Из НастройкиПланаОбмена.РасширенияФорматаОбмена Цикл
			Если Расширение.Значение = Версия.Ключ Тогда
...
Процедура ЗаполнитьКешПубличныхИдентификаторов(КомпонентыОбмена, ИмяФайлаОбмена, ЧтениеXML)
	...
	Разыменователь = Новый РазыменовательПространствИменDOM("ab",
		"http://v8.1c.ru/edi/edi_stnd/EnterpriseData/" + КомпонентыОбмена.ВерсияФорматаОбмена);
	Выражение = "//ab:Ссылка";
...
Функция УстановитьРасширениеСтрокиТабличнойЧасти(ВложенныеРасширения, ИмяТабличнойЧасти)
	
	Если ТипЗнч(ВложенныеРасширения) = Тип("Соответствие") Тогда
		
		РасширенияТабличныхЧастей = ВложенныеРасширения.Получить("РасширенияТабличныхЧастей");
		Если РасширенияТабличныхЧастей <> Неопределено Тогда
			
			Возврат РасширенияТабличныхЧастей.Получить(ИмяТабличнойЧасти);
...
Функция УстановитьРасширениеКлючевыхСвойств(ВложенныеРасширения, ИмяКлючевогоСвойства)
	
	Если ТипЗнч(ВложенныеРасширения) = Тип("Соответствие") Тогда
		
		РасширенияКлючевыхСвойств = ВложенныеРасширения.Получить("РасширенияКлючевыхСвойств");
		Если РасширенияКлючевыхСвойств <> Неопределено Тогда
			
			Возврат РасширенияКлючевыхСвойств.Получить(ИмяКлючевогоСвойства);
...
Процедура ДополнитьСвойстваПакетаИзРасширений(СвойстваИсточника, КомпонентыОбмена, Расширения, ВложенныеРасширения)
	...
	// КомпонентыОбмена.РасширенияФормата - расширения доступные в текущем формате обмена;
	// Расширения - расширения инициализированное в правиле обмена.
	Для Каждого Расширение Из Расширения Цикл
		
		Если КомпонентыОбмена.РасширенияФормата.Получить(Расширение.Ключ) = Неопределено Тогда
			
			Продолжить;
			
		КонецЕсли;
		
		РасширенныйТипXDTO = Неопределено;
		НужныеСвойстваОтсутствуют = Истина;
		Если Расширение.Значение.Свойство("ТипXDTO", РасширенныйТипXDTO) Тогда
			
			НужныеСвойстваОтсутствуют = Ложь;
			
		ИначеЕсли Расширение.Значение.Свойство("ТипСоставногоСвойства", РасширенныйТипXDTO) Тогда
			
			НужныеСвойстваОтсутствуют = Ложь;
			
		КонецЕсли;
		
		Если НужныеСвойстваОтсутствуют
			ИЛИ РасширенныйТипXDTO = Неопределено Тогда
		...
		Для Каждого Свойство Из РасширенныйТипXDTO.Свойства Цикл
			...
			Если Свойство.URIПространстваИмен <> Расширение.Ключ Тогда
				
				Если ЭтоТаблицаОбъекта(Свойство) Тогда
					
					РасширенияТабличныхЧастейПоОсновнымРасширениям(Свойство, Расширение, ВложенныеРасширения["РасширенияТабличныхЧастей"]);
					
				ИначеЕсли Свойство.Имя = КлассКлючевыеСвойстваФормата()
					Или СтрНайти(Свойство.Тип.Имя, КлассКлючевыеСвойстваФормата()) > 0 Тогда
					
					РасширенияКлючевыхСвойствПоОсновнымРасширениям(Свойство, Расширение, ВложенныеРасширения["РасширенияКлючевыхСвойств"]);
...
Процедура РасширенияКлючевыхСвойствПоОсновнымРасширениям(Свойство, Расширение, РасширенияКлючевыхСвойств)
	
	РасширенныеКлючевыеСвойстваТипXDTO = ФабрикаXDTO.Тип(Расширение.Ключ, Свойство.Тип.Имя);
	Если РасширенныеКлючевыеСвойстваТипXDTO = Неопределено Тогда
	...
	Если РасширенияКлючевыхСвойств = Неопределено Тогда
		...
		РасширенияКлючевыхСвойств = Новый Соответствие;
		
	КонецЕсли;
	
	СтруктураРасширенияПравилаКонвертацииОбъекта = Новый Структура;
	СтруктураРасширенияПравилаКонвертацииОбъекта.Вставить("ТипXDTO", РасширенныеКлючевыеСвойстваТипXDTO);
	СтруктураРасширенияПравилаКонвертацииОбъекта.Вставить("ТипКлючевыхСвойствОбъектаXDTO");
	СтруктураРасширенияПравилаКонвертацииОбъекта.Вставить("ТипДанных");
	
	ОписаниеРасширенияКлючевыхСвойств = Новый Соответствие;
	ОписаниеРасширенияКлючевыхСвойств.Вставить(Расширение.Ключ, СтруктураРасширенияПравилаКонвертацииОбъекта);
	
	РасширенияКлючевыхСвойств.Вставить(Свойство.Тип.Имя, ОписаниеРасширенияКлючевыхСвойств);
	
КонецПроцедуры

Процедура РасширенияТабличныхЧастейПоОсновнымРасширениям(Свойство, Расширение, РасширенияТабличныхЧастей)
	
	РасширеннаяТабличнаяЧастьТипXDTO = ФабрикаXDTO.Тип(Расширение.Ключ, Свойство.Тип.Свойства[0].Тип.Имя);
	Если РасширеннаяТабличнаяЧастьТипXDTO = Неопределено Тогда
	...
	Если РасширенияТабличныхЧастей = Неопределено Тогда
		...
		РасширенияТабличныхЧастей = Новый Соответствие;
		
	КонецЕсли;
	
	СтруктураРасширенияПравилаКонвертацииОбъекта = Новый Структура;
	СтруктураРасширенияПравилаКонвертацииОбъекта.Вставить("ТипXDTO", РасширеннаяТабличнаяЧастьТипXDTO);
	СтруктураРасширенияПравилаКонвертацииОбъекта.Вставить("ТипКлючевыхСвойствОбъектаXDTO");
	СтруктураРасширенияПравилаКонвертацииОбъекта.Вставить("ТипДанных");
	
	ОписаниеРасширенияТабличнойЧасти = Новый Соответствие;
	ОписаниеРасширенияТабличнойЧасти.Вставить(Расширение.Ключ, СтруктураРасширенияПравилаКонвертацииОбъекта);
	
	РасширенияТабличныхЧастей.Вставить(Свойство.Имя, ОписаниеРасширенияТабличнойЧасти);
