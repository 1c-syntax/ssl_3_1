...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	ДополнитьСвойстваПакетаИзРасширений(СвойстваИсточника, КомпонентыОбмена, Расширения);
	...
	Для Каждого Свойство Из СвойстваИсточника Цикл
		...
		ИначеЕсли ТипЗнч(Свойство.Тип) = Тип("ТипОбъектаXDTO") Тогда
			...
			ИначеЕсли ЭтоТаблицаОбъекта(Свойство) Тогда
				ТипСвойства = "Таблица";
			ИначеЕсли Свойство.Имя = КлассКлючевыеСвойства()
				Или СтрНайти(Свойство.Тип.Имя, КлассКлючевыеСвойства()) > 0 Тогда
			...
			Иначе
				...
				ИначеЕсли ТипСвойства = "Таблица" Тогда
					...
					Для Каждого ИсточникСтрока Из ЗначениеСвойства Цикл
						...
						ПриемникСтрока = ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, ИсточникСтрока, ТипТаблицы, СсылкиИзОбъекта, , ?(ЭтоБазоваяСхема, Неопределено, Расширения));
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Иначе
				ИмяПланаОбмена = ПодтверждениеXDTO.ExchangePlan;
...
Функция ОбъектXDTOВСтруктуру(ОбъектXDTO, КомпонентыОбмена) Экспорт
	...
	Если Приемник.Свойство(КлассКлючевыеСвойства(), КлючевыеСвойстваКопия)
		И ТипЗнч(КлючевыеСвойстваКопия) = Тип("Структура")
		И КлючевыеСвойстваКопия.Свойство(КлассСсылка(), СсылкаКопия)
		Тогда
		
		Приемник.Вставить(КлассСсылка(), СсылкаКопия);
...
Функция ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, Режим = "ОтправкаПолучение", УзелОбмена = Неопределено) Экспорт
	...
	Для Каждого Версия Из НастройкиПланаОбмена.ВерсииФорматаОбмена Цикл
		...
		Для Каждого Расширение Из НастройкиПланаОбмена.РасширенияФорматаОбмена Цикл
			Если Расширение.Значение = Версия Тогда
...
Функция СхемаРасширенияФормата(УзелОбмена) Экспорт
	
	МенеджерЗаписи = РегистрыСведений.НастройкиОбменаДаннымиXDTO.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.УзелИнформационнойБазы = УзелОбмена;
	МенеджерЗаписи.Прочитать();
	
	Возврат МенеджерЗаписи.РасширениеФормата;
...
Функция ТаблицаПравилОбработкиДанных(КомпонентыОбмена) Экспорт
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если КомпонентыОбмена.НаправлениеОбмена = "Получение" Тогда
			...
			КлючевыеСвойства = ТипXDTO.Свойства.Получить(КлассКлючевыеСвойства());
...
Функция ТаблицаПравилКонвертации(КомпонентыОбмена)
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если ЭтоБазоваяСхема И ЕстьРасширения Тогда
			...
			Для Каждого Расширение Из ПравилоКонвертации.Расширения Цикл
				ТипXDTO = ФабрикаXDTO.Тип(Расширение.Ключ, ПравилоКонвертации.ОбъектФормата);
				Если ТипXDTO = Неопределено Тогда
					РасширенияКУдалению.Добавить(Расширение.Ключ);
				Иначе
					Расширение.Значение.ТипXDTO = ТипXDTO;
				КонецЕсли;
			КонецЦикла;
		...
		Если ЗначениеЗаполнено(ПравилоКонвертации.ОбъектФормата) Тогда
			КлючевыеСвойства = ПравилоКонвертации.ТипXDTO.Свойства.Получить(КлассКлючевыеСвойства());
			Если КлючевыеСвойства <> Неопределено Тогда
			...
			Если ЭтоБазоваяСхема И ЕстьРасширения Тогда
				Для Каждого Расширение Из ПравилоКонвертации.Расширения Цикл
					КлючевыеСвойства = Расширение.Значение.ТипXDTO.Свойства.Получить(КлассКлючевыеСвойства());
					
					Если КлючевыеСвойства <> Неопределено Тогда
						
						ТипКлючевыхСвойствОбъектаXDTO = КлючевыеСвойства.Тип;
						Расширение.Значение.ТипКлючевыхСвойствОбъектаXDTO = ТипКлючевыхСвойствОбъектаXDTO;
...
Процедура ЗаписатьЗаголовокСообщенияОбмена(ФайлОбмена, ПараметрыЗаголовка) Экспорт
	...
	Если ПараметрыЗаголовка.ЭтоОбменЧерезПланОбмена Тогда
		...
		Иначе
			ПодтверждениеXDTO.ExchangePlan = ПараметрыЗаголовка.ИмяПланаОбмена;
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник,
	...
	ТипЗначенияXDTO = ?(ТипСвойства = Неопределено, ТипЗначенияСвойстваXDTO(КомпонентыОбмена, Свойство, ЗначениеXDTO), ТипСвойства);
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ЗначениеXDTO) = Тип("ОбъектXDTO") Тогда
			...
			ИначеЕсли КлассЗначенияXDTO = КлассКлючевыеСвойства() Тогда
...
Функция ПрочитатьЗначениеXDTOСложногоТипа(ЗначениеXDTO, СложныйТип, ТипСвойства = Неопределено)
	...
	СтруктураXDTO.Вставить("ЭтоСсылка", СложныйТип = КлассСсылка());
...
// Если свойства относится к пространству имен расширения - тип у него не будет определен
...
Функция ТипЗначенияСвойстваXDTO(Знач КомпонентыОбмена, Знач СвойствоXDTO, ЗначениеXDTO)
	...
	ТипОбъектаXDTO = СвойствоXDTO.ОбъектВладелец.Тип();
	Если ЭтоБазоваяСхема(КомпонентыОбмена, ТипОбъектаXDTO.URIПространстваИмен) Тогда
		РасширениеОбъекта = ПКОПоТипуОбъектаXDTO(КомпонентыОбмена, ТипОбъектаXDTO).Расширения.Получить(ПространствоИменСвойства);
		Если РасширениеОбъекта <> Неопределено Тогда
			Результат = РасширениеОбъекта.ТипXDTO.Свойства.Получить(СвойствоXDTO.Имя).Тип;
		КонецЕсли;
...
Процедура КонвертироватьКлючевоеСвойство(Приемник, Знач КомпонентыОбмена, Знач ИмяСвойства, Знач ТипЗначенияXDTO, Знач ЗначениеXDTO)
	...
	Значение.Вставить("ТипЗначения", СтрЗаменить(ТипЗначенияXDTO.Имя, КлассКлючевыеСвойства(), ""));
...
Процедура РасширитьСвойстваТаблицыОбъекта(ТаблицаОбъекта, Знач ИмяОбъекта, Знач ИмяТаблицыОбъекта, Знач РасширенияФормата)
	...
	Для Каждого Расширение Из РасширенияФормата Цикл
		ТипОбъекта = ФабрикаXDTO.Тип(Расширение.Ключ, ИмяОбъекта);
		Если ТипОбъекта = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ТабличнаяЧасть = ТипОбъекта.Свойства.Получить(ИмяТаблицыОбъекта);
		Если ТабличнаяЧасть = Неопределено Или Не ЭтоТаблицаОбъекта(ТабличнаяЧасть) Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого Колонка Из ТабличнаяЧасть.Тип.Свойства[0].Тип.Свойства Цикл
			Если СтрНайти(Колонка.Тип.Имя, КлассКлючевыеСвойства()) > 0 Тогда
				Для Каждого ПодКолонка Из Колонка.Тип.Свойства Цикл
					Если ТаблицаОбъекта.Колонки.Найти(ПодКолонка.Имя) <> Неопределено Тогда
						Продолжить;
					КонецЕсли;
					
					ТаблицаОбъекта.Колонки.Добавить(ПодКолонка.Имя);
				КонецЦикла;
			Иначе
				Если ТаблицаОбъекта.Колонки.Найти(Колонка.Имя) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ТаблицаОбъекта.Колонки.Добавить(Колонка.Имя);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПКОПоТипуОбъектаXDTO(КомпонентыОбмена, ТипОбъектаXDTO, ВозвращатьПустоеЗначение = Ложь)
	
	ПравилоКонвертации = КомпонентыОбмена.ПравилаКонвертацииОбъектов.Найти(ТипОбъектаXDTO, "ТипXDTO");
	Если ПравилоКонвертации = Неопределено Тогда
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено ПКО для типа объекта XDTO.
					|Тип объекта XDTO: %1
					|Описание ошибки: %2'"),
...
Процедура ЗаполнитьСтруктуруНастроекXDTOКорреспондента(СтруктураНастроек,
	...
	Если Header.AvailableExtensions <> Неопределено Тогда
...
Процедура ДополнитьСвойстваПакетаИзРасширений(СвойстваИсточника, КомпонентыОбмена, Расширения)
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		НужныеСвойстваОтсутствуют = НЕ Расширение.Значение.Свойство("ТипXDTO", РасширенныйТипXDTO)
			И НЕ Расширение.Значение.Свойство("ТипСоставногоСвойства", РасширенныйТипXDTO);
