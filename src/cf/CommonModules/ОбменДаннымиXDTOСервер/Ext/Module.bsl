...
//  СтрокаТаблицыЗначений - СтрокаТаблицыЗначений правил конвертации.
//
...
Функция ЗаписатьВПротоколВыполнения(КомпонентыОбмена,
	...
	Если ПустаяСтрока(КомпонентыОбмена.СтрокаСообщенияОбОшибке) Тогда
		КомпонентыОбмена.СтрокаСообщенияОбОшибке = КраткоеПредставлениеОшибки;
	Иначе
		КомпонентыОбмена.СтрокаСообщенияОбОшибке = КомпонентыОбмена.СтрокаСообщенияОбОшибке + "
		|" + КраткоеПредставлениеОшибки; 
	КонецЕсли;
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Попытка
		Если НомерЭтапа = 1 Тогда
			...
			Если ПКС.ИспользуетсяАлгоритмКонвертации
				И Не ЗначениеЗаполнено(ПравилоКонвертацииСвойства) Тогда
				// Конвертация будет выполняться по алгоритму.
				Возврат;
			КонецЕсли;
