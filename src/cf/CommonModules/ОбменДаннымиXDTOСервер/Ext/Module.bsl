...
//     * ТолькоНастройкиXDTO - Булево - признак того, что будет выполнена загрузка только первоначальных настроек синхронизации.
//     * ИспользоватьКвитирование - Булево - признак использования квитирования для удаления регистрации изменений.
//     * ПриоритетыКонвертации - см. КонвертацияОбъектноСервер.ПриоритетыКонвертацииВерсииФормата.
...
Функция ИнициализироватьКомпонентыОбмена(НаправлениеОбмена) Экспорт
	...
	Если НаправлениеОбмена = "Отправка" Тогда
		...
		КомпонентыОбмена.Вставить("ФильтрВыборкиИзменений");
	...
	Иначе
		...
		КомпонентыОбмена.Вставить("ПриоритетыКонвертации",                         Неопределено);
		КомпонентыОбмена.Вставить("ОбъектнаяКонвертацияXDTO",                      Ложь);
		КомпонентыОбмена.Вставить("ПодобранныеПубличныеИдентификаторы",            Новый Соответствие);
		КомпонентыОбмена.Вставить("КлючОбрабатывающегоЗадания",                    "");
		КомпонентыОбмена.Вставить("ИмяФайлаСообщенияОбмена",                       "");
		КомпонентыОбмена.Вставить("ЧтениеЭлементаПутемКопированияФрагментаПотока", Ложь);
	...
	КомпонентыОбмена.Вставить("ИмяВременногоКаталогаЗамеров", "");
	...
	КомпонентыОбмена.Вставить("КешДанныхОбъектовДляРегистрацииВОбменах", Новый Соответствие);
...
//  ДляПолученияПоддерживаемыхОбъектов - Булево - Признак того, необходимо получить только заголовки правил
//
Процедура ИнициализироватьТаблицыПравилОбмена(КомпонентыОбмена, ДляПолученияПоддерживаемыхОбъектов = Ложь) Экспорт
	
	ИнициализироватьТаблицыПравилОбменаРасширенный(КомпонентыОбмена, , ДляПолученияПоддерживаемыхОбъектов);
...
Функция ЗаписатьВПротоколВыполнения(КомпонентыОбмена,
	...
	ИначеЕсли ТипЗнч(КодОшибки) = Тип("Структура") Тогда
		...
		КодОшибки.Свойство("СсылкаНаОбъект", СсылкаНаОбъект);
...
// Проверяет возможность выгрузки ссылки в соответствии с правилами регистрации
// и настройками отправки данных на узле обмена, и регистрирует изменения на узле обмена.
// Инициализирует дополнительные параметры конвертации данных,
// передаваемые в метод ОбменДаннымиСервер.ВыполнитьДействиеОбмена для выполнения выборочной выгрузки.
// Возвращает список ссылок, которые будут выгружены.
//
// Параметры:
//  УзелОбмена - ПланОбменаСсылка - узел обмена, по которому выгружаются данные.
//  СписокСсылок - Массив из ЛюбаяСсылка - список ссылок выгружаемых объектов.
//  ДополнительныеПараметры - Структура - исходящий параметр. Зарезервировано для служебного использования.
//
// Возвращаемое значение:
//  Массив из ЛюбаяСсылка - список выгружаемых ссылок.
// 
Функция ИнициализироватьВыборочнуюВыгрузкуДанных(УзелОбмена, СписокСсылок, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Ссылка Из СписокСсылок Цикл
		Если ПланыОбмена.ИзменениеЗарегистрировано(УзелОбмена, Ссылка) Тогда
			ВыгружаемыеСсылки.Добавить(Ссылка);
		Иначе
			ОбъектДляВыгрузки = Ссылка.ПолучитьОбъект();
			
			ОбменДаннымиСобытия.ВыполнитьПравилаРегистрацииДляОбъекта(ОбъектДляВыгрузки, ИмяПланаОбмена);
	...
	ПараметрыОбработкиКонвертации = Новый Структура;
	ПараметрыОбработкиКонвертации.Вставить("ИспользоватьКвитирование", Ложь);
	ПараметрыОбработкиКонвертации.Вставить("ФильтрВыборкиИзменений", ОбщегоНазначения.СкопироватьРекурсивно(ВыгружаемыеСсылки));
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ПараметрыОбработкиКонвертации", ПараметрыОбработкиКонвертации);
	
	Возврат ВыгружаемыеСсылки;
	
КонецФункции

// Выполняет действия, связанные с успешной доставкой корреспонденту сообщения обмена:
//  - добавляет в регистр публичных идентификаторов записи по выгруженным объектам;
//  - (опционально) удаляет регистрацию изменений выгруженных объектов.
// Метод предназначен для использования только в сценарии выборочной выгрузки данных.
// Параметры:
//  УзелОбмена - ПланОбменаСсылка - узел обмена, по которому выгружаются данные.
//  ДополнительныеПараметры - Структура - зарезервировано для служебного использования.
//                            В данный параметр передается структура, полученная при подготовке выборочной
//                            выгрузки данных методом ОбменДаннымиXDTOСервер.ИнициализироватьВыборочнуюВыгрузкуДанных.
//
Процедура ПодтвердитьОтправкуСообщенияВыборочнойВыгрузкиДанных(УзелОбмена, ДополнительныеПараметры) Экспорт
	
	КомпонентыОбмена = Новый Структура;
	КомпонентыОбмена.Вставить("УзелКорреспондента", УзелОбмена);
	КомпонентыОбмена.Вставить("НомерСообщенияПолученногоКорреспондентом", 0);
	
	КомпонентыОбмена.Вставить("ИспользоватьКвитирование");
	КомпонентыОбмена.Вставить("ФильтрВыборкиИзменений");
	ЗаполнитьЗначенияСвойств(КомпонентыОбмена, ДополнительныеПараметры.ПараметрыОбработкиКонвертации);
	
	ОбработатьПолучениеПодтвержденияОтКорреспондента(КомпонентыОбмена, Ложь);
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	УзелДляОбмена = КомпонентыОбмена.УзелКорреспондента; // ПланОбменаСсылка
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если КомпонентыОбмена.ПропускатьОбъектыСОшибкамиПроверкиПоСхеме Тогда
			ИсключаемыеТипыДляРегистрации = ИнициализироватьМассивИсключаемыхТиповДляРегистрации();
			Для Каждого ОбъектСсылка Из КомпонентыОбмена.НеВыгруженныеОбъекты Цикл
				Если ИсключаемыеТипыДляРегистрации.Найти(ТипЗнч(ОбъектСсылка)) = Неопределено Тогда
...
Процедура ВыгрузкаОбъектаВыборки(КомпонентыОбмена, Объект, ПравилоОбработки = Неопределено) Экспорт
	...
	ИспользованиеПКО = ПравилаКонвертацииИзПравилаОбработки(ПравилоОбработки);
	
	ПриОбработкеПОД(КомпонентыОбмена, ПравилоОбработки, Объект, ИспользованиеПКО, ПрерватьОбработку);
	...
	Если Не ПрерватьОбработку Тогда
		...
		Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
			...
			ДобавитьИдентификаторВПубличные(КомпонентыОбмена, Объект, ОбъектСсылочногоТипа, ПравилоКонвертации.ОбъектФормата);
			ВыгрузитьОбъектыПоСсылке(КомпонентыОбмена, СсылкиИзОбъекта);
			ОтразитьВыгруженныеОбъекты(КомпонентыОбмена, ОбъектСсылочногоТипа, Объект);
			...
			Иначе
				...
				ИначеЕсли ТипСвойства = "ОбычноеСвойство" Тогда
					...
					Если ЭтоСсылкаXDTO(СвойствоТипЗначения) Тогда // Конвертация ссылки
						...
						МассивТиповНеВыгружаемыхПоСсылке = Новый Массив;
						МассивТиповНеВыгружаемыхПоСсылке.Добавить(Тип("Структура"));
						МассивТиповНеВыгружаемыхПоСсылке.Добавить(Тип("УникальныйИдентификатор"));
						
						Если МассивТиповНеВыгружаемыхПоСсылке.Найти(ТипЗнч(ЗначениеСвойства)) = Неопределено
							И СсылкиИзОбъекта.Найти(ЗначениеСвойства) = Неопределено
							Тогда
...
Процедура ВыгрузитьСвойство(КомпонентыОбмена, ДанныеИБ, ПолучательСвойства, ПКС, СтекВыгрузки, ЭтапВыгрузки = 1) Экспорт
	...
	Если ЭтапВыгрузки = 1 Тогда
		Если ПКС.СвойствоКонфигурации = КлассФорматаСсылкаВнешняяНавигационная() Тогда
			ЗначениеСвойства = СсылкаВнешняяНавигационнаяИзОбъектаВыгрузки(ДанныеИБ);
		ИначеЕсли ЗначениеЗаполнено(ПКС.СвойствоКонфигурации) Тогда
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Иначе
			ЗаписьСообщения.НомерСообщения = 0;
			ЗаписьСообщения.НомерПринятого = 0;
		КонецЕсли;
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если Не КомпонентыОбмена.ОбменДаннымиСВнешнейСистемой Тогда
			...
			ПараметрыЗаголовка.ИмяПланаОбменаКорреспондента = 
				ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбменаКорреспондента(КомпонентыОбмена.УзелКорреспондента);
		...
		КомпонентыОбмена.КешПубличныхИдентификаторов = Новый Соответствие;
...
Процедура ТипСвойстваПоИмениИЗначениюСвойства(ТипXDTO, СвойствоНайдено, Свойство, ЗначениеСвойства, ТипСвойства)
	...
	ИначеЕсли ТипЗнч(Свойство.Тип) = Тип("ТипОбъектаXDTO") Тогда
		...
		ИначеЕсли СтрНайти(Свойство.Тип.Имя, КлассКлючевыеСвойстваФормата()) > 0 Тогда
...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, Знач ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если ДанныеИБ = Неопределено Тогда
		...
		ИначеЕсли ВариантИдентификации = "ПоПолямПоиска"
			...
			// посредством произвольного алгоритма, описанного в менеджере обмена.
	...
	Если ЗаписыватьОбъектВИБ Тогда
		...
		ЗапомнитьОбъектДляОтложенногоЗаполнения(ДанныеДляЗаписиВИБ, ПравилоКонвертации, КомпонентыОбмена, Действие);
...
Процедура ПроизвестиЧтениеДанных(КомпонентыОбмена, ТаблицыДляЗагрузки = Неопределено) Экспорт
	Перем ОбъектнаяКонвертацияXDTO;
	
	КомпонентыОбмена.Свойство("ОбъектнаяКонвертацияXDTO", ОбъектнаяКонвертацияXDTO);
	...
	Если ОбъектнаяКонвертацияXDTO = Ложь
...
Процедура ПроизвестиЧтениеДанныхВРежимеАнализа(КомпонентыОбмена, ПараметрыАнализа = Неопределено) Экспорт
	...
	Если НЕ КонвертацияОбъектноСервер.ЭтоОбъектнаяКонвертация(КомпонентыОбмена) Тогда
		
		// Имеет смысл только для пакетной конвертации,
		// т.к. в объектной модели удаление обрабатывается непосредственно при выборке прочитанных данных (без накопления)
		ПрименитьУдалениеОбъектов(КомпонентыОбмена, Результаты.МассивОбъектовКУдалению, Результаты.МассивЗагруженныхОбъектов);
	КонецЕсли;
...
//  КомпонентыОбмена - Структура - содержит все правила и параметры обмена.
//  ИмяФайлаОбмена - Строка - имя файла обмена.
//  ЭтоПовторноеОткрытие - Булево - признак того что файл открывается повторно и не требуется инициализировать прочие 
//                                  данные структуры КомпонентыОбмена.
//
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена, ЭтоПовторноеОткрытие = Ложь) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Попытка
			...
			КомпонентыОбмена.ИмяФайлаСообщенияОбмена = ИмяФайлаОбмена;
		...
		Если Не ЭтоПовторноеОткрытие Тогда
		...
		Если Не ЭтоПовторноеОткрытие Тогда
			...
			Если ЭтоОбменЧерезПланОбмена Тогда
				...
				Если КомпонентыОбмена.ОбъектнаяКонвертацияXDTO Тогда
					
					КомпонентыОбмена.ТолькоНастройкиXDTO = (URIФормата = ФорматПланаОбмена);
				...
				Если Не КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
					...
					Если КомпонентыОбмена.ИспользоватьКвитирование Тогда
						...
						ОбработатьПолучениеСообщения(КомпонентыОбмена);
...
Функция ОбъектXDTOВСтруктуру(ОбъектXDTO, КомпонентыОбмена) Экспорт
	...
	Если Приемник.Свойство(КлассКлючевыеСвойстваФормата(), КлючевыеСвойстваКопия)
		...
		Если КлючевыеСвойстваКопия.Свойство(КлассСсылкаФормата(), СсылкаКопия) Тогда
			
			Приемник.Вставить(КлассСсылкаФормата(), СсылкаКопия);
			
		КонецЕсли;
		
		НавигационнаяСсылкаКопия = Неопределено;
		Если КлючевыеСвойстваКопия.Свойство(КлассФорматаСсылкаВнешняяНавигационная(), НавигационнаяСсылкаКопия) Тогда
			
			Приемник.Вставить(КлассФорматаСсылкаВнешняяНавигационная(), НавигационнаяСсылкаКопия);
	...
	ОбменДаннымиОценкаПроизводительности.ЗавершитьЗамер(ВремяНачала,Событие, Приемник, КомпонентыОбмена,
		ОбменДаннымиОценкаПроизводительности.ТипСобытияБиблиотека());
...
Функция ОтменитьПроведениеОбъектаВИБ(Объект, Отправитель, КомпонентыОбмена = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонТекста = НСтр("ru ='При отмене проведения документа:
			...
			|Подробное описание ошибки см. в журнале регистрации'", ОбщегоНазначения.КодОсновногоЯзыка());
		Кратко = СтрШаблон(ШаблонТекста, Строка(Объект), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Подробно = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		
		ОписаниеОшибки = Новый Структура;
		ОписаниеОшибки.Вставить("Уровень", УровеньЖурналаРегистрации.Ошибка);
		ОписаниеОшибки.Вставить("КраткоеПредставлениеОшибки", Кратко);
		ОписаниеОшибки.Вставить("ПодробноеПредставлениеОшибки", Подробно);
	
		ЗаписатьВПротоколВыполнения(КомпонентыОбмена, ОписаниеОшибки, , Ложь);
		ЗаписатьОшибкуОбработкиОбъектаПриОтправке(Объект, КомпонентыОбмена.УзелКорреспондента, Кратко, ТипПроблемы);
...
Функция ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, Режим = "ОтправкаПолучение", УзелОбмена = Неопределено) Экспорт
	...
	ПравилаОтправкиМенеджеровОбмена = Новый Соответствие;
	ПравилаПолученияМенеджеровОбмена = Новый Соответствие;
	...
	Для Каждого Версия Из ВерсииФорматаОбмена Цикл
		...
		Для Каждого СхемаРасширения Из РасширенияВерсии Цикл
			...
			Если РежимОтправка Тогда
				...
				ТаблицыПравил = ПравилаОтправкиМенеджеровОбмена[КомпонентыОбмена.МенеджерОбмена];
				ИнициализироватьТаблицыПравилОбменаРасширенный(КомпонентыОбмена, ТаблицыПравил, Истина);
				ПравилаОтправкиМенеджеровОбмена[КомпонентыОбмена.МенеджерОбмена] = ТаблицыПравил;
			...
			Если РежимПолучение Тогда
				...
				ТаблицыПравил = ПравилаПолученияМенеджеровОбмена[КомпонентыОбмена.МенеджерОбмена];
				ИнициализироватьТаблицыПравилОбменаРасширенный(КомпонентыОбмена, ТаблицыПравил, Истина);
				ПравилаПолученияМенеджеровОбмена[КомпонентыОбмена.МенеджерОбмена] = ТаблицыПравил;
...
Функция ПропускатьОбъектыСОшибкамиПроверкиПоСхеме(УзелИнформационнойБазы, НовоеЗначение = Неопределено) Экспорт
	...
	Иначе
		...
		Попытка
			...
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.НастройкиОбменаДаннымиXDTO");
			ЭлементБлокировкиДанных.УстановитьЗначение("УзелИнформационнойБазы", УзелИнформационнойБазы);
		...
		Исключение
			...
			Событие = НСтр("ru = 'Обмен данными через универсальный формат.Выгрузка.Запись настройки обмена'", ОбщегоНазначения.КодОсновногоЯзыка());
			
			ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка, Метаданные.РегистрыСведений.НастройкиОбменаДаннымиXDTO, , СообщениеОбОшибке);
...
// Возвращает имя свойства "ЛюбаяСсылка" пакета XDTO EnterpriseData.
//
// Возвращаемое значение:
//   Строка - "ЛюбаяСсылка".
//
Функция КлассФорматаЛюбаяСсылка() Экспорт
	...
	Возврат "ЛюбаяСсылка"; // @Non-NLS
	
КонецФункции

// Возвращает имя свойства "УдалениеОбъекта" пакета XDTO EnterpriseData.
//
// Возвращаемое значение:
//   Строка - "УдалениеОбъекта".
//
Функция КлассФорматаУдалениеОбъекта() Экспорт
	
	Возврат "УдалениеОбъекта"; // @Non-NLS
...
// Создает коллекцию правил обмена версии формата обмена
//
// Параметры:
...
//                               -  ТаблицаЗначений
// 
Функция КоллекцияПравилКонвертации(ВерсияФорматаМенеджераОбмена) Экспорт
	...
	ПравилаКонвертации.Колонки.Добавить("ПриПолученииЗапросаВыгрузкиОбъекта",    Новый ОписаниеТипов("Строка"));
	ПравилаКонвертации.Колонки.Добавить("ПриУдаленииОбъектаИБ",                  Новый ОписаниеТипов("Строка"));
	ПравилаКонвертации.Колонки.Добавить("ПередЗаписьюПолученныхДанных",          Новый ОписаниеТипов("Строка"));
	ПравилаКонвертации.Колонки.Добавить("ПриОбработкеСсылокБлокирующихКонвертацию", Новый ОписаниеТипов("Строка"));
	ПравилаКонвертации.Колонки.Добавить("ПослеЗагрузкиВсехДанных",               Новый ОписаниеТипов("Строка"));
	ПравилаКонвертации.Колонки.Добавить("ПослеКонвертацииОбъекта",               Новый ОписаниеТипов("Строка"));
	...
	ПравилаКонвертации.Колонки.Добавить("РазрешитьСоздаватьОбъектИзСтруктуры",   Новый ОписаниеТипов("Булево"));
	ПравилаКонвертации.Колонки.Добавить("ИгнорироватьПроблемыПриЗаписи",         Новый ОписаниеТипов("Булево"));
...
// Инициализирует таблицу значений поддерживаемых объектов формата.
// 
// Параметры:
//  ТаблицаОбъекты - ТаблицаЗначений - Пустая таблица значений для инициализации колонок
//  Режим - Строка - Направление обмена строкой. Например "ОтправкаПолучение"
//  ИндексироватьКолонки - Булево - Признак того, требуется ли добавлять индексы колонок. По умолчанию = Истина
//
Процедура ИнициализироватьТаблицуПоддерживаемыхОбъектовФормата(ТаблицаОбъекты, Режим, ИндексироватьКолонки = Истина) Экспорт
	...
	Если ИндексироватьКолонки Тогда
...
Процедура ПрименитьУдалениеОбъектов(КомпонентыОбмена, МассивОбъектовКУдалению, МассивЗагруженныхОбъектов) Экспорт
	...
	Для Каждого ЭлементКУдалению Из МассивОбъектовКУдалению Цикл
		...
		Если КомпонентыОбмена.РежимЗагрузкиДанныхВИнформационнуюБазу Тогда
			...
			Отказ = Ложь;
			ПриУдаленииОбъекта(КомпонентыОбмена, Объект, Отказ);
...
Процедура ПровестиГенерациюКодаНомераПриНеобходимости(Объект) Экспорт
...
Процедура СнятьПометкуУдаленияСПредопределенногоЭлемента(Объект, ТипОбъекта, КомпонентыОбмена) Экспорт
...
// 
// Возвращаемое значение:
//  ОбщийМодуль - Менеджер обмена версии формата
//
...
// 
// Параметры:
//  Получатель - ПланОбменаСсылка - ссылка на узел плана обмена, в который производится выгрузка данных.
// 
// Возвращаемое значение:
//  Строка
//
...
#Область ФормализацияКлассовФорматаED

Функция КлассОбщиеСвойства() Экспорт
	
	Возврат "ОбщиеСвойства"; // @Non-NLS
	
КонецФункции

Функция КлассСоставныеСвойства() Экспорт
	
	Возврат "СоставныеСвойства"; // @Non-NLS
	
КонецФункции

Функция КлассПроизвольныеДанные() Экспорт
	
	Возврат "ПроизвольныеДанные"; // @Non-NLS
	
КонецФункции

Функция КлассТаблицаОбъекта() Экспорт
	
	Возврат "ТаблицаОбъекта"; // @Non-NLS
	
КонецФункции

Функция ПостфиксСвойстваОтбор() Экспорт
	
	Возврат "Отбор"; // @Non-NLS
...
// Возвращает имя свойства "Ссылка" пакета XDTO EnterpriseData.
//
// Возвращаемое значение:
//   Строка - "Ссылка".
//
Функция КлассСсылкаФормата() Экспорт
	
	Возврат "Ссылка"; // @Non-NLS
	
КонецФункции

// Возвращает имя свойства "ЗапросНаВыгрузку" пакета XDTO EnterpriseData.
//
// Возвращаемое значение:
//   Строка - "ЗапросНаВыгрузку".
//
Функция КлассФорматаЗапросНаВыгрузку() Экспорт
	
	Возврат "ЗапросНаВыгрузку"; // @Non-NLS
	
КонецФункции

// Возвращает имя свойства "СсылкаНаОбъект" пакета XDTO EnterpriseData.
//
// Возвращаемое значение:
...
Функция КлассФорматаСсылкаНаОбъект() Экспорт
	...
	Возврат "СсылкаНаОбъект"; // @Non-NLS
	
КонецФункции

// Возвращает имя свойства "СсылкаВнешняяНавигационная" пакета XDTO EnterpriseData.
//
// Возвращаемое значение:
//   Строка - "СсылкаВнешняяНавигационная".
//
Функция КлассФорматаСсылкаВнешняяНавигационная() Экспорт
	
	Возврат "СсылкаВнешняяНавигационная"; // @Non-NLS
...
//
// Параметры:
//  Ссылка - ЛюбаяСсылка - ссылка на объект ИБ, для которого необходимо получить массив узлов.
...
// Возвращаемое значение:
//  Массив из ПланОбменаСсылка - массив узлов плана обмена, для которых установлен признак «Выгружать при
//  необходимости», изначально пуст.
//
Функция МассивУзловДляРегистрацииВыгружатьПриНеобходимости(Ссылка, ИмяПланаОбмена, ИмяРеквизитаФлага) Экспорт
	...
	Запрос.УстановитьПараметр("ПометкаУдаления", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ПометкаУдаления"));
...
// 
Функция ТаблицаПравилОбработкиДанных(КомпонентыОбмена, ПравилаОбработкиДанныхМенеджераОбмена = Неопределено) Экспорт
	
	XMLСхема = КомпонентыОбмена.XMLСхема;
	
	Если ПравилаОбработкиДанныхМенеджераОбмена = Неопределено Тогда
		// Инициализация таблицы правил обработки данных.
	    ПравилаОбработкиДанных = КоллекцияПравилОбработкиДанных();
		...
		ПравилаОбработкиДанныхМенеджераОбмена = ПравилаОбработкиДанных.Скопировать();
	Иначе
		ПравилаОбработкиДанных = ПравилаОбработкиДанныхМенеджераОбмена.Скопировать();
	КонецЕсли;
...
Функция СообщениеОбменаВНастройкиПодключения(ТекстXML) Экспорт
	...
	СтруктураНастроекXDTO = СтруктураНастроекXDTO();

	ИмяПланаОбмена = ЗаголовокСообщенияXDTO.Confirmation.ExchangePlan;
	УзелОбмена = ПланыОбмена[ИмяПланаОбмена].ЭтотУзел();
	ЗаполнитьСтруктуруНастроекXDTOКорреспондента(СтруктураНастроекXDTO, ЗаголовокСообщенияXDTO, , УзелОбмена);
	...
	СтруктураНастроекXDTO.Вставить("ИдентификаторОтправителя", ЗаголовокСообщенияXDTO.Confirmation.From);
	...
	НастройкиПодключения.КодНовогоУзлаВторойБазы = СтруктураНастроекXDTO.ИдентификаторОтправителя;
	НастройкиПодключения.ВерсияФорматаОбмена = 
		МаксимальнаяОбщаяВерсияФормата(ИмяПланаОбмена, СтруктураНастроекXDTO.ПоддерживаемыеВерсии);
	НастройкиПодключения.ПоддерживаемыеОбъектыФорматаКорреспондента = Новый ХранилищеЗначения(СтруктураНастроекXDTO.ПоддерживаемыеОбъекты, Новый СжатиеДанных(9));
	НастройкиПодключения.ФорматОбмена = СтруктураНастроекXDTO.Формат;
	НастройкиПодключения.ИдентификаторТранспорта = "ПассивныйРежим";
		
	Возврат НастройкиПодключения;
	
КонецФункции

Функция НастройкиПодключенияВСообщениеОбмена(НастройкиПодключения) Экспорт
	...
	ПараметрыЗаголовка.ПоддерживаемыеОбъекты = ПоддерживаемыеОбъектыФормата(
	...
	ЗаписатьЗаголовокСообщенияОбмена(ЗаписьXML, ПараметрыЗаголовка);
	
	Возврат ЗаписьXML.Закрыть();

КонецФункции

Функция ТаблицаПравилКонвертации(КомпонентыОбмена, ПравилаКонвертацииОбъектовМенеджераОбмена = Неопределено, ДляПолученияПоддерживаемыхОбъектов = Ложь) Экспорт
	
	Если ПравилаКонвертацииОбъектовМенеджераОбмена = Неопределено Тогда
		// Инициализация таблицы правил конвертации.
		ПравилаКонвертации = КоллекцияПравилКонвертации(КомпонентыОбмена.ВерсияФорматаМенеджераОбмена);
		
		ЗаполнитьПравилаКонвертацииОбъектовИзМодуляОбмена(КомпонентыОбмена, ПравилаКонвертации, ДляПолученияПоддерживаемыхОбъектов);
		
		ПравилаКонвертацииОбъектовМенеджераОбмена = ПравилаКонвертации.Скопировать();
	Иначе
		ПравилаКонвертации = ПравилаКонвертацииОбъектовМенеджераОбмена.Скопировать();
	КонецЕсли;
	...
	ПравилаКонвертации.Колонки.Добавить("ЕстьОбработчикПриОбработкеСсылокБлокирующихКонвертацию", Новый ОписаниеТипов("Булево"));
	ПравилаКонвертации.Колонки.Добавить("ЕстьОбработчикПослеЗагрузкиВсехДанных",      Новый ОписаниеТипов("Булево"));
	ПравилаКонвертации.Колонки.Добавить("ЕстьОбработчикПослеКонвертацииОбъекта",      Новый ОписаниеТипов("Булево"));
	ПравилаКонвертации.Колонки.Добавить("ЕстьОбработчикАлгоритмПоиска",               Новый ОписаниеТипов("Булево"));
	ПравилаКонвертации.Колонки.Добавить("ЕстьОбработчикПриПолученииЗапросаВыгрузкиОбъекта", Новый ОписаниеТипов("Булево"));
	ПравилаКонвертации.Колонки.Добавить("ЕстьОбработчикПриУдаленииОбъектаИБ",         Новый ОписаниеТипов("Булево"));
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если ДляПолученияПоддерживаемыхОбъектов Тогда
			Продолжить;
		КонецЕсли;
		...
		Если КомпонентыОбмена.НаправлениеОбмена = "Получение" Тогда
			...
			ПравилоКонвертации.ЕстьОбработчикПослеКонвертацииОбъекта      = Не ПустаяСтрока(ПравилоКонвертации.ПослеКонвертацииОбъекта);
			ПравилоКонвертации.ЕстьОбработчикАлгоритмПоиска               = Не ПустаяСтрока(ПравилоКонвертации.АлгоритмПоиска);
			ПравилоКонвертации.ЕстьОбработчикПриПолученииЗапросаВыгрузкиОбъекта = Не ПустаяСтрока(ПравилоКонвертации.ПриПолученииЗапросаВыгрузкиОбъекта);
			ПравилоКонвертации.ЕстьОбработчикПриУдаленииОбъектаИБ = Не ПустаяСтрока(ПравилоКонвертации.ПриУдаленииОбъектаИБ);
			ПравилоКонвертации.ЕстьОбработчикПриОбработкеСсылокБлокирующихКонвертацию = Не ПустаяСтрока(ПравилоКонвертации.ПриОбработкеСсылокБлокирующихКонвертацию);
...
Процедура ПолучитьЗначениеВложенныхСвойств(ИсточникСвойства, ВложенныеСвойства, ЗначениеСвойства) Экспорт
...
Функция РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Экспорт
...
Функция ПОДПоТипуСсылкиXDTO(КомпонентыОбмена, ТипСсылкиXDTO, ВозвращатьПустоеЗначение = Ложь) Экспорт
...
Функция ПОДПоТипуОбъектаXDTO(КомпонентыОбмена, ТипОбъектаXDTO, ВозвращатьПустоеЗначение = Ложь) Экспорт
...
//          - СправочникОбъект, ДокументОбъект - объект метаданных;
...
//
Функция ПредставлениеОбъектаДляПротокола(Объект, МетаданныеДляПредставленияСтруктуры = Неопределено) Экспорт
	...
	// Для корректного перевода кода на английский язык
	Ссылка = Неопределено;
	...
	ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1, %2 (%3)",
...
// Заполняет переданную коллекцию правил обмена из модуля менеджера
//
// Параметры:
//  КомпонентыОбмена - см. ИнициализироватьКомпонентыОбмена
//  КоллекцияПравил - см. КоллекцияПравилКонвертации
//  ТолькоЗаголовки - Булево - Признак того, что требуется получить только заголовки объектов
// Возвращаемое значение:
//  ТаблицаЗначений - см.КоллекцияПравилКонвертации - заполненная коллекция правил обмена из модуля менеджера.
//
Функция ТаблицаПравилКонвертацииОбъектовИзМодуляОбмена(КомпонентыОбмена, КоллекцияПравил, ТолькоЗаголовки = Ложь) Экспорт
	
	ЗаполнитьПравилаКонвертацииОбъектовИзМодуляОбмена(КомпонентыОбмена, КоллекцияПравил, ТолькоЗаголовки);
	
	Возврат КоллекцияПравил;
...
Функция ТаблицаПравилКонвертацииПредопределенныхДанных(КомпонентыОбмена)
	...
	ЗаполнитьПравилКонвертацииПредопределенныхДанныхИзМодуляОбмена(КомпонентыОбмена, ПравилаКонвертации);
...
Процедура ЗаполнитьПравилаКонвертацииОбъектовИзМодуляОбмена(КомпонентыОбмена, КоллекцияПравил, ТолькоЗаголовки = Ложь)
	
	Если КомпонентыОбмена.ВерсияФорматаМенеджераОбмена = "3" Тогда
		
		КомпонентыОбмена.МенеджерОбмена.ЗаполнитьПравилаКонвертацииОбъектов(КомпонентыОбмена, КоллекцияПравил, ТолькоЗаголовки);
		
	Иначе
		
		КомпонентыОбмена.МенеджерОбмена.ЗаполнитьПравилаКонвертацииОбъектов(КомпонентыОбмена.НаправлениеОбмена, КоллекцияПравил);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьПравилКонвертацииПредопределенныхДанныхИзМодуляОбмена(КомпонентыОбмена, КоллекцияПравил)
	
	Если КомпонентыОбмена.ВерсияФорматаМенеджераОбмена = "3" Тогда
		
		КомпонентыОбмена.МенеджерОбмена.ЗаполнитьПравилаКонвертацииПредопределенныхДанных(КомпонентыОбмена, КоллекцияПравил);
		
	Иначе
		
		КомпонентыОбмена.МенеджерОбмена.ЗаполнитьПравилаКонвертацииПредопределенныхДанных(КомпонентыОбмена.НаправлениеОбмена, КоллекцияПравил);
		
	КонецЕсли;
	
КонецПроцедуры

Функция КоллекцияПравилОбработкиДанных()
...
Процедура ИнициализироватьТаблицыПравилОбменаРасширенный(КомпонентыОбмена, ПравилаМенеджераОбмена = Неопределено, ДляПолученияПоддерживаемыхОбъектов = Ложь)
	...
	Если ПравилаМенеджераОбмена = Неопределено Тогда
		ПравилаМенеджераОбмена = Новый Структура;
		ПравилаМенеджераОбмена.Вставить("ПравилаОбработкиДанных");
		ПравилаМенеджераОбмена.Вставить("ПравилаКонвертацииОбъектов");
	КонецЕсли;
	
	// Инициализация таблиц правил обмена.
	КомпонентыОбмена.Вставить("ПравилаОбработкиДанных",
		ТаблицаПравилОбработкиДанных(КомпонентыОбмена, ПравилаМенеджераОбмена.ПравилаОбработкиДанных));
	КомпонентыОбмена.Вставить("ПравилаКонвертацииОбъектов",
		ТаблицаПравилКонвертации(КомпонентыОбмена, ПравилаМенеджераОбмена.ПравилаКонвертацииОбъектов, ДляПолученияПоддерживаемыхОбъектов));
...
Процедура ТипXDTOПравилаОбработкиИзРасширенийПакетаXDTO(КомпонентыОбмена, ПОД, ТипXDTO)
	...
	Для каждого ОписаниеРасширения Из КомпонентыОбмена.РасширенияФормата Цикл
		...
		Если ТипРасширенияXDTO <> Неопределено Тогда
			...
			// Установили принадлежность к расширению схемы XDTO
...
Процедура ОтметитьКлючевыеСвойстваПравилаКонвертации(КомпонентыОбмена, ПравилоКонвертации)
	...
	ДополнитьКлючевыеСвойстваСсылками(КомпонентыОбмена, ПравилоКонвертации, ТипКлючевыхСвойствОбъектаXDTO);
	
КонецПроцедуры

Процедура ДополнитьКлючевыеСвойстваСсылками(КомпонентыОбмена, ПравилоКонвертации, ТипКлючевыхСвойствОбъектаXDTO)
	...
	ПКСДляСсылки = ПравилоКонвертации.Свойства.Добавить();
	...
	СвойствоСсылкаВнешняяXDTO = ТипКлючевыхСвойствОбъектаXDTO.Свойства.Получить(КлассФорматаСсылкаВнешняяНавигационная());
	Если СвойствоСсылкаВнешняяXDTO <> Неопределено Тогда
		
		// Каждую ссылку сопроводим навигационной ссылкой
		ПКСДляСсылкиВнешней = ПравилоКонвертации.Свойства.Добавить();
		ПКСДляСсылкиВнешней.СвойствоКонфигурации = КлассФорматаСсылкаВнешняяНавигационная();
		ПКСДляСсылкиВнешней.СвойствоФормата = КлассФорматаСсылкаВнешняяНавигационная();
		ПКСДляСсылкиВнешней.ОбработкаКлючевогоСвойства = Истина;
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ПередКонвертацией(КомпонентыОбмена)
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(ШаблонОписанияОшибки, КомпонентыОбмена.НаправлениеОбмена, "ПередКонвертацией", ПодробноеПредставление);
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(КомпонентыОбмена, НомерСообщения)
	...
	ВыполнитьВыгрузкуЗапросовПовторнойОтправкиДанных(КомпонентыОбмена);
...
Процедура ВыполнитьВыгрузкуЗапросовПовторнойОтправкиДанных(КомпонентыОбмена)
	
	КонвертацияОбъектноСервер.ЗапросыНаВыгрузкуДанныхВXML(КомпонентыОбмена);
	
КонецПроцедуры

Процедура ВыполнитьВыгрузкуЗарегистрированныхДанныхСПакетнойРегистрацией(
	КомпонентыОбмена, НомерСообщения, НачальнаяВыгрузкаДанных, УзелДляОбменаОбъект, СписокТипов)
	
	УзелДляОбмена = УзелДляОбменаОбъект.Ссылка;
	...
	Для Каждого ТипОбъекта Из СписокТипов Цикл
		...
		Запрос.Текст = 
			...
			|	ВЫБОР
			|		КОГДА ТаблицаИзменений.Ссылка.Ссылка ЕСТЬ NULL
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК ЭтоУдалениеОбъекта
			|ПОМЕСТИТЬ ВременнаяТаблица
			|ИЗ
			|	#ТаблицаИзменений КАК ТаблицаИзменений
			|ГДЕ
			|	ТаблицаИзменений.Узел = &Узел
			|	И (НЕ &ИспользоватьКвитирование
			|			ИЛИ ТаблицаИзменений.НомерСообщения <= &НомерСообщения)
			|	И (НЕ &ИспользоватьФильтрВыборкиИзменений
			|			ИЛИ ТаблицаИзменений.Ссылка В (&ФильтрВыборкиИзменений))
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|ГДЕ
			|	ВременнаяТаблица.ЭтоУдалениеОбъекта
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	ВременнаяТаблица КАК ВременнаяТаблица
			|ГДЕ
			|	НЕ ВременнаяТаблица.ЭтоУдалениеОбъекта";
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ТаблицаИзменений", ТипОбъекта.ПолноеИмя() + ".Изменения"); //@Query-part-2
		
		Запрос.УстановитьПараметр("Узел", УзелДляОбмена);
		// Параметры квитирования.
		Запрос.УстановитьПараметр("ИспользоватьКвитирование", КомпонентыОбмена.ИспользоватьКвитирование);
		Запрос.УстановитьПараметр("НомерСообщения", НомерСообщения);
		// Параметры ограничения выборки изменений.
		Запрос.УстановитьПараметр("ИспользоватьФильтрВыборкиИзменений", КомпонентыОбмена.ФильтрВыборкиИзменений <> Неопределено);
		Запрос.УстановитьПараметр("ФильтрВыборкиИзменений", КомпонентыОбмена.ФильтрВыборкиИзменений);
		...
		Пока Выборка.Следующий() Цикл
			...
			ВыгрузитьУдаление(КомпонентыОбмена, Выборка.Ссылка);
		...
		МассивДанных = РезультатЗапроса[2].Выгрузить().ВыгрузитьКолонку("Ссылка");
		...
		ОбменДаннымиСобытия.ВыполнитьПакетнуюРегистрациюДляУзла(УзелДляОбмена, МассивДанных, ПараметрыПР);
			
		ОбменДаннымиОценкаПроизводительности.ЗавершитьЗамер(
			ВремяНачала, "ПакетнаяРегистрацияОбъектов", ТипОбъекта.ПолноеИмя(), КомпонентыОбмена,
			ОбменДаннымиОценкаПроизводительности.ТипСобытияПрикладное());
		...
		Для Каждого Ссылка Из ПараметрыПР.СсылкиПоФильтруПакетнойРегистрации Цикл
			...
			Попытка
				...
				ВыгрузкаОбъектаВыборки(КомпонентыОбмена, Объект);
				
			Исключение
		
				Инфо = ИнформацияОбОшибке();
				ПредставлениеДанных = ПредставлениеОбъектаДляПротокола(Объект);
		...
		Если ПараметрыПР.ЕстьПРО_БезПакетнойРегистрации
			И ПараметрыПР.СсылкиНеПоФильтруПакетнойРегистрации.Количество() > 0 Тогда
		...
		ИначеЕсли НачальнаяВыгрузкаДанных
			И НЕ ПараметрыПР.ЕстьПРО_БезПакетнойРегистрации
			И ПараметрыПР.СсылкиНеПоФильтруПакетнойРегистрации.Количество() > 0 Тогда
			
			Для Каждого Ссылка Из ПараметрыПР.СсылкиНеПоФильтруПакетнойРегистрации Цикл
				
				Объект = Ссылка.ПолучитьОбъект();
				ПланыОбмена.УдалитьРегистрациюИзменений(УзелДляОбменаОбъект.Ссылка, Объект);
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанныхБезПакетнойРегистрации(
	КомпонентыОбмена, НомерСообщения, НачальнаяВыгрузкаДанных, УзелДляОбменаОбъект, СписокТипов)
...
Процедура ВыгрузкаЗарегистрированногоОбъекта(КомпонентыОбмена, НачальнаяВыгрузкаДанных, УзелДляОбменаОбъект, Объект)
	
	Попытка
		Если ТипЗнч(Объект) = Тип("УдалениеОбъекта") Тогда
			ВыгрузитьУдаление(КомпонентыОбмена, Объект.Ссылка);
		Иначе
			ОтправкаЭлемента = ОтправкаЭлементаДанных.Авто;
			ОбменДаннымиСобытия.ПриОтправкеДанныхКорреспонденту(Объект, ОтправкаЭлемента, НачальнаяВыгрузкаДанных, УзелДляОбменаОбъект, Ложь);
			
			// Удаление регистра отсылаем в виде пустого набора записей.
			Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Удалить
				И ОбщегоНазначения.ЭтоРегистр(Объект.Метаданные()) Тогда
				ОтправкаЭлемента = ОтправкаЭлементаДанных.Авто;
			КонецЕсли;
...
Процедура ВыгрузитьОбъектыПоСсылке(КомпонентыОбмена, СсылкиИзОбъекта)
	...
	Для Каждого ЗначениеСсылки Из СсылкиИзОбъекта Цикл
		...
		Если КомпонентыОбмена.ВыгруженныеОбъекты.Найти(ЗначениеСсылки) <> Неопределено
			ИЛИ КомпонентыОбмена.ВыгруженныеПоСсылкеОбъекты.Найти(ЗначениеСсылки) <> Неопределено
			ИЛИ КомпонентыОбмена.КешДанныхОбъектовДляРегистрацииВОбменах.Получить(ЗначениеСсылки) <> Неопределено
			ИЛИ РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах.ОбъектЕстьВРегистре(ЗначениеСсылки, УзелКорреспондента)
			Тогда
		...
		СтруктураЗаписи.Ссылка = ЗначениеСсылки;
		Если РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(СтруктураЗаписи) = ИСТИНА Тогда
			
			// Если приложения обменивались текущей ссылкой, то будет запись в ПИСО,
			// следовательно, можно не делать полную выгрузку объекта, т.к. корреспондент должен распознать ссылку.
			Продолжить;
			
		КонецЕсли;
		
		ОбъектМетаданных = ЗначениеСсылки.Метаданные();
		ИмяОбъекта = ОбъектМетаданных.ПолноеИмя();
		Если ОбменДаннымиПовтИсп.ПользовательскийСоставПланаОбмена(УзелКорреспондента).Получить(ИмяОбъекта) = Неопределено Тогда
			
			// Можно проверить наличие правила регистрации, но правильнее проверить состав плана обмена,потому что
			// если правило регистрации есть, а в состав плана обмена этот объект метаданных не включен.
			// То, даже если мы не упадем в ошибку при регистрации, то все равно остается плохой сценарий,
			// при котором сообщение с выгруженным полным объектом по какой-то причине не загружено,
			// а повторная отправка будет выбирать только те объекты, которые зарегистрированы, т.к. считает,
			// что полный экземпляр объекта уже есть в приемнике (см. ДанныеОбъектовДляРегистрацииВОбменах.ОбъектЕстьВРегистре).
			Продолжить;
		...
		// Вместо ВыгружатьОбъектПоНеобходимости(КомпонентыОбмена, ЗначениеСсылки) проверяем регистрацию
		ПравилоОбработки = ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных, Истина);
		Если ПравилоОбработки = Неопределено Тогда
		...
		ВыгрузкаОбъектаВыборки(КомпонентыОбмена, ОбъектВыгружаемыйПоСсылке, ПравилоОбработки);
		
		// Кэшируем сведения
		РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах.ДобавитьОбъектВФильтрРазрешенныхОбъектов(ЗначениеСсылки, УзелКорреспондента);
		КомпонентыОбмена.ВыгруженныеПоСсылкеОбъекты.Добавить(ЗначениеСсылки);
		КомпонентыОбмена.КешДанныхОбъектовДляРегистрацииВОбменах.Вставить(ЗначениеСсылки);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ОтразитьВыгруженныеОбъекты(КомпонентыОбмена, ОбъектСсылочногоТипа, Объект)
	...
	УзелКорреспондента = КомпонентыОбмена.УзелКорреспондента;
	Если ОбъектСсылочногоТипа <> Истина Тогда
		
		Возврат;
		
	КонецЕсли;
	
	ОбъектнаяСсылка = Объект.Ссылка;
	Если КомпонентыОбмена.КешДанныхОбъектовДляРегистрацииВОбменах.Получить(ОбъектнаяСсылка) <> Неопределено Тогда
	...
	Если НЕ Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
		...
		Если РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах.ОбъектЕстьВРегистре(ОбъектнаяСсылка, УзелКорреспондента) <> Истина Тогда
			
			РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах.ДобавитьОбъектВФильтрРазрешенныхОбъектов(ОбъектнаяСсылка, УзелКорреспондента);
			
		КонецЕсли;
		
	КонецЕсли;
	
	// Кэш, чтобы далее не делать запрос по большому регистру "ДанныеОбъектовДляРегистрацииВОбменах"
	КомпонентыОбмена.КешДанныхОбъектовДляРегистрацииВОбменах.Вставить(ОбъектнаяСсылка);
...
Функция КонвертироватьСсылкуВXDTO(КомпонентыОбмена, ЗначениеСсылки, ТипСсылкиXDTO)
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		УИДОбъектаXDTO = КомпонентыОбмена.КешПубличныхИдентификаторов.Получить(ЗначениеСсылки);
		
		Если УИДОбъектаXDTO = Неопределено Тогда 
			
			ОбъектФормата = "";
			УИДОбъектаXDTO = РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ПубличныйИдентификаторПоСсылкеОбъекта(
				КомпонентыОбмена.УзелКорреспондента, ЗначениеСсылки, ОбъектФормата);
			
			КомпонентыОбмена.КешПубличныхИдентификаторов.Вставить(ЗначениеСсылки, УИДОбъектаXDTO);
...
// См. КонвертацияОбъектноСервер.СсылкаВнешняяНавигационнаяИзОбъектаВыгрузки().
//
Функция СсылкаВнешняяНавигационнаяИзОбъектаВыгрузки(ДанныеИБ)
	
	Возврат КонвертацияОбъектноСервер.СсылкаВнешняяНавигационнаяИзОбъектаВыгрузки(ДанныеИБ);
...
Функция ПараметрыЗаголовкаСообщенияОбмена() Экспорт
	...
	ПараметрыЗаголовка.Вставить("ИмяПланаОбменаКорреспондента",   "");
...
// 
Процедура ЗаписатьЗаголовокСообщенияОбмена(ФайлОбмена, ПараметрыЗаголовка)
...
Процедура ДобавитьИдентификаторВПубличные(КомпонентыОбмена, Объект, ОбъектСсылочногоТипа, ОбъектФормата)
	
	Если НЕ ОбъектСсылочногоТипа Тогда
	...
	СтруктураЗаписи = Новый Структура;
	СтруктураЗаписи.Вставить("Ссылка", Объект.Ссылка);
	СтруктураЗаписи.Вставить("УзелИнформационнойБазы", КомпонентыОбмена.УзелКорреспондента);
	СтруктураЗаписи.Вставить("Идентификатор", Объект.Ссылка.УникальныйИдентификатор());
	СтруктураЗаписи.Вставить("ОбъектФормата", ОбъектФормата);
	
	Если (КомпонентыОбмена.ИспользоватьКешПубличныхИдентификаторов = ИСТИНА
			И НЕ ЗаписьЕстьВКешеПубличныхИдентификаторов(СтруктураЗаписи, КомпонентыОбмена))
		ИЛИ НЕ РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(СтруктураЗаписи)
		Тогда
	...
	Если КомпонентыОбмена.ИспользоватьКешПубличныхИдентификаторов Тогда
...
//   КомпонентыОбмена - см. ОбменДаннымиXDTOСервер.ИнициализироватьКомпонентыОбмена
//   Действие - Строка - действие, которое выполняется в СтруктураОбъектаXDTOВДанныеИБ. Может приобретать значение 
//                       "КонвертироватьИЗаписать" или "ПолучитьСсылку".
//
Процедура ЗапомнитьОбъектДляОтложенногоЗаполнения(ДанныеДляЗаписиВИБ, ПравилоКонвертации, КомпонентыОбмена, Действие)
	
	Если ПравилоКонвертации.ЕстьОбработчикПослеЗагрузкиВсехДанных Тогда
		
		ОтборСтрок = Новый Структура("СсылкаНаОбъект", ДанныеДляЗаписиВИБ.Ссылка);
		НайденныеСтроки = КомпонентыОбмена.ЗагруженныеОбъекты.НайтиСтроки(ОтборСтрок);
		...
		ИначеЕсли Действие = "КонвертироватьИЗаписать" Тогда
			...
			Для каждого СтрокаТаблицы Из НайденныеСтроки Цикл
				...
				СтрокаТаблицы.Объект = ДанныеДляЗаписиВИБ;
...
Процедура ПрочитатьСообщениеОбмена(КомпонентыОбмена, Результаты, ТаблицыДляЗагрузки = Неопределено, РежимАнализа = Ложь)
	...
	Если ОбъектнаяКонвертацияXDTO = Истина Тогда
		...
		// При объектной конвертации любой обработчик события вызываем после того,
		// как файл будет прочитан, фактический перенес в ИБ
		РегистрыСведений.ПрочитанныеДанныеСообщенийОбмена.СообщениеОбменаВРегистрИБ(КомпонентыОбмена);
		
		ОбменДаннымиОценкаПроизводительности.ЗавершитьЗамер(
			ВремяНачала, "СообщениеОбменаВРСПрочитанныеДанные", "", КомпонентыОбмена, 
			ОбменДаннымиОценкаПроизводительности.ТипСобытияБиблиотека());
			
		КонвертацияОбъектноСервер.КонвертироватьЗачитанныеДанные(КомпонентыОбмена);
		
	Иначе
		
		// Обработчик глобального события "ПередКонвертацией" (описывает разработчик в КД)
		ПередКонвертацией(КомпонентыОбмена);
		ОбработатьДанныеСообщенияЛинейно(КомпонентыОбмена, Результаты, ТаблицыДляЗагрузки, РежимАнализа);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработатьДанныеСообщенияЛинейно(КомпонентыОбмена, Результаты, ТаблицыДляЗагрузки, РежимАнализа)
	...
	Пока КомпонентыОбмена.ФайлОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		ИначеЕсли ТипОбъектаXDTO.Имя = "ЗапросНаВыгрузку" Тогда
			
			Если РежимАнализа <> Истина Тогда
				
				ПрочитатьЗапросНаВыгрузку(КомпонентыОбмена, ОбъектXDTO);
				Продолжить;
		...
		ПрерватьОбработку = Ложь;
		ИспользованиеПКО = ПравилаКонвертацииИзПравилаОбработки(ПравилоОбработки);
		ПриОбработкеПОД(КомпонентыОбмена, ПравилоОбработки, ДанныеXDTO, ИспользованиеПКО, ПрерватьОбработку);
			...
			Иначе
				...
				Если КомпонентыОбмена.РежимЗагрузкиДанныхВИнформационнуюБазу
					...
					Попытка
						...
						РежимЗагрузки = ?(КомпонентыОбмена.РежимЗагрузкиДанныхВИнформационнуюБазу, "КонвертироватьИЗаписать", "Конвертировать");
						ДанныеДляЗаписиВИБ = СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, ПравилоКонвертации, РежимЗагрузки);
...
Функция НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, КомпонентыОбмена, ТипЗначенияОбъектаИБ)
	...
	Для Каждого Строка Из Результат Цикл
		...
		Иначе
			...
			Если НЕ ОбщегоНазначения.СсылкаСуществует(Строка.Ссылка) Тогда
	...
	Если НайденнаяСсылка <> Неопределено
		И Не ОбщегоНазначения.СсылкаСуществует(НайденнаяСсылка)
...
Процедура ОбработатьПолучениеСообщения(КомпонентыОбмена)
	...
	Иначе
		ОбработатьПолучениеПодтвержденияОтКорреспондента(КомпонентыОбмена);
...
#Область ЗапросПовторнойВыгрузки

Процедура ПрочитатьЗапросНаВыгрузку(КомпонентыОбмена, ОбъектXDTO)
	
	КонвертацияОбъектноСервер.ЗапросыНаВыгрузкуВДанныеИБ(КомпонентыОбмена, ОбъектXDTO);
...
//
Процедура ПрочитатьУдаление(КомпонентыОбмена, ОбъектXDTO, МассивОбъектовКУдалению, ТаблицыДляЗагрузки = Неопределено) Экспорт
	...
	// Если передан отбор ПКО, то определим его актуальность,
	// далее отработаем только по переданному ПКО.
	ИспользоватьОтборПоИмениПКО = Ложь;
	Если КомпонентыОбмена.Свойство("ИсторияКонвертацииОбъекта")
		И ЗначениеЗаполнено(КомпонентыОбмена.ИсторияКонвертацииОбъекта.УказанноеИмяПКО) Тогда
		
		ИспользоватьОтборПоИмениПКО = (МассивИменПКО.Найти(КомпонентыОбмена.ИсторияКонвертацииОбъекта.УказанноеИмяПКО) <> Неопределено);
		
	КонецЕсли;
	
	Для Каждого ПравилоКонвертацииИмя Из МассивИменПКО Цикл
		
		Если ИспользоватьОтборПоИмениПКО
			И КомпонентыОбмена.ИсторияКонвертацииОбъекта.УказанноеИмяПКО <> ПравилоКонвертацииИмя Тогда
...
Процедура УдалитьОбъект(Объект, УдалитьНепосредственно, КомпонентыОбмена)
	...
	Если УдалитьНепосредственно Тогда
		УдалитьОбъектСЗаписямиЗависимыхРегистровСведений(Объект, КомпонентыОбмена);
	Иначе
...
Процедура УдалитьОбъектСЗаписямиЗависимыхРегистровСведений(Объект, КомпонентыОбмена)
	
	НачатьТранзакцию();
	
	Попытка
	
		Для Каждого ЭлементКоллекции Из ЗависимыеРегистрыСведенийОбъекта() Цикл 
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений." + ЭлементКоллекции.Ключ);
			ЭлементБлокировки.УстановитьЗначение(ЭлементКоллекции.Значение, Объект.Ссылка);
			Блокировка.Заблокировать();
			
			НаборЗаписей = РегистрыСведений[ЭлементКоллекции.Ключ].СоздатьНаборЗаписей();
			НаборЗаписей.Отбор[ЭлементКоллекции.Значение].Установить(Объект.Ссылка, Истина);
			НаборЗаписей.ОбменДанными.Загрузка = Объект.ОбменДанными.Загрузка;
	...
	Исключение
		...
		УзелИнформационнойБазы = КомпонентыОбмена.УзелКорреспондента;
		НаправлениеОбмена = КомпонентыОбмена.НаправлениеОбмена;
		
		Событие = ОбменДаннымиСервер.КлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы,
			НаправлениеОбмена);
...
Функция ЗависимыеРегистрыСведенийОбъекта()

	ЗависимыеРегистрыСведений = Новый Структура;
	ЗависимыеРегистрыСведений.Вставить("ПубличныеИдентификаторыСинхронизируемыхОбъектов", "Ссылка");
	ЗависимыеРегистрыСведений.Вставить("ДанныеОбъектовДляРегистрацииВОбменах", "Ссылка");
	ЗависимыеРегистрыСведений.Вставить("ОбъектыНезарегистрированныеПриЗацикливании", "Объект");

	Возврат ЗависимыеРегистрыСведений;
...
//              - Неопределено - узел плана обмена.
//
Процедура ЗаполнитьСтруктуруНастроекXDTOКорреспондента(
	СтруктураНастроек, Header, ФорматСодержитВерсию = Истина, УзелОбмена = Неопределено)
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных, ВозвращатьПустоеЗначение = Ложь)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		Иначе
			...
			ШаблонИсключения = НСтр("ru = 'Не найдено ПОД для объекта метаданных.
				|Объект метаданных: %1.'");
			
			ВызватьИсключение СтрШаблон(ШаблонИсключения, Строка(ОбъектМетаданных.ПолноеИмя()));
...
Функция ПравилаКонвертацииИзПравилаОбработки(ПравилоОбработки)
...
//
Процедура ПриОбработкеПОД(КомпонентыОбмена, ПравилоОбработки, Знач ОбъектОбработки, ИспользованиеПКО, Отказ = Ложь) Экспорт
...
//  ИмяОбработчика   - Строка - имя процедуры-обработчика в модуле менеджера.
//
Процедура ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена, Знач ИмяОбработчика) Экспорт
...
//                       ДанныеИБ.
//
Процедура ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КомпонентыОбмена, ИмяОбработчика, КонвертацияСвойств) Экспорт
...
Процедура ПриУдаленииОбъекта(КомпонентыОбмена, Объект, Отказ)
	
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена
		И ОбменДаннымиСобытия.ЗагрузкаЗапрещена(Объект, КомпонентыОбмена.УзелКорреспондентаОбъект) Тогда
		
		// Если изменение объекта ограничено датой запрета, то продолжим цикл
		Отказ = Истина;
		Возврат;
		
	КонецЕсли;
	
	// Попытка на случай отсутствия обработчика в менеджере обмена данными
	Попытка
		КомпонентыОбмена.МенеджерОбмена.ПередОбработкойУдаляемогоОбъекта(КомпонентыОбмена, Объект);
	Исключение
	...
	Если Метаданные.Документы.Содержит(МетаданныеОбъекта) Тогда
		...
		Если Объект.Проведен Тогда
			...
			ЕстьРезультат = ОтменитьПроведениеОбъектаВИБ(Объект, КомпонентыОбмена.УзелКорреспондента, КомпонентыОбмена);
			Если Не ЕстьРезультат Тогда
				
				// Если при удалении движений возникли ошибки
				Если КомпонентыОбмена.Свойство("ИсторияКонвертацииОбъекта") Тогда
					
					ШаблонТекста = НСтр("ru ='Удаление документа <%1> не выполнено из-за ошибки отмены проведения.'", ОбщегоНазначения.КодОсновногоЯзыка());
					ТекстОшибки = СтрШаблон(ШаблонТекста, Объект);
					
					КонвертацияОбъектноРезультатыСлужебный.ФиксироватьОшибкуОбработкиУдаленияОбъекта(КомпонентыОбмена, Объект, ТекстОшибки);
		...
		ИначеЕсли МетаданныеОбъекта.Проведение = Метаданные.СвойстваОбъектов.Проведение.Запретить Тогда
...
Функция ПредставлениеКоллекцииСвойствДляПротокола(КоллекцияСвойств)
	...
	Если КоллекцияСвойств.Свойство("Наименование", ЗначениеСвойства)
		И ЗначениеЗаполнено(ЗначениеСвойства) Тогда // @Non-NLS
	...
	Если КоллекцияСвойств.Свойство("Номер", ЗначениеСвойства)
		...
		Если КоллекцияСвойств.Свойство("Дата", ЗначениеСвойстваДата)
			И ЗначениеЗаполнено(ЗначениеСвойстваДата) Тогда // @Non-NLS
...
Процедура ЗаписатьПриНеобходимостиПубличныйИдентификатор(ДанныеДляЗаписиВИБ, ПолученныеДанныеСсылка,
		УИДСтрокой, ПравилоКонвертации, КомпонентыОбмена) Экспорт
	...
	Если ДанныеДляЗаписиВИБ <> Неопределено Тогда
		...
		Если ИспользоватьКешПубличныхИдентификаторов
			И ЗаписьЕстьВКешеПубличныхИдентификаторов(СтруктураЗаписи, КомпонентыОбмена) Тогда
			
			Возврат;
			
		Иначе //НЕ ИспользоватьКешПубличныхИдентификаторов
			
			Если РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(СтруктураЗаписи) Тогда
				...
				РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаполнитьОбъектФорматТекущейЗаписи(СтруктураЗаписи);
				Возврат;
...
Процедура ОбработатьПолучениеПодтвержденияОтКорреспондента(КомпонентыОбмена, КешироватьПубличныеИдентификаторы = Истина)
	
	ДополнительныеПараметрыКонвертации = Новый Структура("ФильтрВыборкиИзменений");
	ЗаполнитьЗначенияСвойств(ДополнительныеПараметрыКонвертации, КомпонентыОбмена);
	
	ДобавитьВыгруженныеОбъектыВРегистрПубличныеИдентификаторы(
		КомпонентыОбмена, КешироватьПубличныеИдентификаторы, ДополнительныеПараметрыКонвертации.ФильтрВыборкиИзменений);
...
Процедура ДобавитьВыгруженныеОбъектыВРегистрПубличныеИдентификаторы(КомпонентыОбмена, КешироватьПубличныеИдентификаторы, ФильтрВыборкиИзменений)
	...
	Если НаборЗапросовИзменений.Количество() > 0 Тогда
		...
		|		ПО (ПубличныеИдентификаторы.УзелИнформационнойБазы = ТаблицаИзменений.Узел)
		|			И (ПубличныеИдентификаторы.Ссылка = ТаблицаИзменений.Ссылка)
		|ГДЕ
		...
		// Параметры ограничения выборки изменений.
		Запрос.УстановитьПараметр("ИспользоватьФильтрВыборкиИзменений", ФильтрВыборкиИзменений <> Неопределено);
		Запрос.УстановитьПараметр("ФильтрВыборкиИзменений", ФильтрВыборкиИзменений);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если КешироватьПубличныеИдентификаторы Тогда
				ДобавитьЗаписьВКешПубличныхИдентификаторов(СтруктураЗаписи, КомпонентыОбмена);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДополнитьСписокОбъектовКУдалению(КомпонентыОбмена, ПравилоКонвертации, УникальныйИдентификатор, МассивОбъектовКУдалению)
	
	СсылкаКУдалению = СсылкаОбъектаПоУИДОбъектаXDTO(УникальныйИдентификатор,
		ПравилоКонвертации.ТипДанных, КомпонентыОбмена);
	
	ПравилоДляГруппыСправочника = (ПравилоКонвертации.ЭтоСправочник Или ПравилоКонвертации.ЭтоПланВидовХарактеристик)
		И ПравилоКонвертации.ПравилоДляГруппыСправочника;
		
	Если ПравилоДляГруппыСправочника И (Не СсылкаКУдалению.Пустая()
		И ОбщегоНазначения.СсылкаСуществует(СсылкаКУдалению)) Тогда
		
		ЭтоГруппа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаКУдалению, "ЭтоГруппа");
	
		Если Не ЭтоГруппа Тогда
			СсылкаКУдалению = ПравилоКонвертации.МенеджерОбъекта.ПолучитьСсылку();
		КонецЕсли;
	КонецЕсли;
...
Функция ИнициализироватьМассивИсключаемыхТиповДляРегистрации()
	
	ИсключаемыеТипыДляРегистрации = Новый Массив;	
	ИсключаемыеТипыДляРегистрации.Добавить(Тип("Структура"));
	
	Возврат ИсключаемыеТипыДляРегистрации;
...
Функция ТранслироватьИмя(ИсходноеИмя, НаправлениеПеревода, ДоступныеИмена = Неопределено)
	
	Если Метаданные.ВариантВстроенногоЯзыка = Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		Возврат ИсходноеИмя;
	КонецЕсли;
	
	НовоеИмя = ОбменДаннымиТрансляцияФорматаПовтИсп.ТранслироватьИмя(ИсходноеИмя, НаправлениеПеревода);
...
Процедура ТранслироватьПредопределенныеДанные(ПравилаКонвертации)
	...
	Для Каждого Строка Из ПравилаКонвертации Цикл
		...
		Строка.ТипXDTO = ТранслироватьИмя(Строка.ТипXDTO, "ru");
...
Процедура РасширенияФорматаВКомпонентыОбмена(КомпонентыОбмена) Экспорт
	...
	Для Каждого РасширениеФорматаОбмена Из ДоступныеРасширенияФормата Цикл
		...
		ВключитьПространствоИмен(КомпонентыОбмена, РасширениеФорматаОбмена.Ключ, РасширениеФорматаОбмена.Значение);
