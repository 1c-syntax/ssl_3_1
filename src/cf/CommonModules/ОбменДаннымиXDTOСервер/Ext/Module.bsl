...
//  ПравилоОбработки - СтрокаТаблицыЗначений - Строка таблицы значений правил обработки данных, 
...
Процедура ВыгрузкаОбъектаВыборки(КомпонентыОбмена, Объект, ПравилоОбработки = Неопределено) Экспорт
	...
	Для Каждого ТекущийПКО Из ПравилоОбработки.ИспользуемыеПКО Цикл
		ИспользованиеПКО.Вставить(ТекущийПКО, Истина);
	КонецЦикла;
	...
	Если Не ПрерватьОбработку Тогда
		...
		Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
			...
			Если Не ПропуститьОбработку Тогда
				...
				Контекст.Вставить("ПредставлениеОбъекта", ПредставлениеОбъектаДляПротокола(Объект));
...
Процедура ЗаписатьЗаголовокСообщенияОбмена(ФайлОбмена, ПараметрыЗаголовка) Экспорт
	...
	Иначе
		Header.AvailableVersion.Добавить(ПараметрыЗаголовка.ВерсияФорматаОбмена);
	КонецЕсли;
	...
	Для Каждого Свойство Из ТипОбъектаXDTO.Свойства Цикл
		Если Не ОбъектXDTO.Установлено(Свойство) Тогда
			Если Свойство.НижняяГраница = 1
				...
				СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
				СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + " - " + НСтр("ru = 'не заполнено обязательное поле.'");
		...
		Иначе
			...
			Если ТипЗнч(ЗначениеСвойстваXDTO) = Тип("ЗначениеXDTO") Тогда
				...
				Исключение
					СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
					СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + " - " + НСтр("ru = 'значение поля не соответствует заданным ограничениям.'");
					СтекОшибок.Добавить(СообщениеОбОшибке);
				КонецПопытки;
			ИначеЕсли ЭтоСписокXDTO Тогда
				...
				Для Каждого ЭлементСпискаXDTO Из ЗначениеСвойстваXDTO Цикл
					...
					Если НовыйСтекОшибок.Количество() > 0 Тогда
						СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
						СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + "[" + XMLСтрока(Сч) + "]";
						СтекОшибок.Добавить(СообщениеОбОшибке);
			...
			Иначе
				...
				Если НовыйСтекОшибок.Количество() > 0 Тогда
					СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
					СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя;
					СтекОшибок.Добавить(СообщениеОбОшибке);
					Для Каждого НоваяОшибка Из НовыйСтекОшибок Цикл
						СтекОшибок.Добавить(НоваяОшибка);
					КонецЦикла;
		...
		Попытка
			...
			Если Объект.ПроверитьЗаполнение() Тогда
				...
				Если Объект.ДополнительныеСвойства.Свойство("СведенияОВерсииОбъекта", СведенияОВерсииОбъекта) Тогда
					ОбменДаннымиСобытия.ПриСозданииВерсииОбъекта(Объект, СведенияОВерсииОбъекта, Истина, КомпонентыОбмена.УзелКорреспондента);
				КонецЕсли;
				Объект.Записать();
...
Процедура ОбновитьПрефиксКорреспондента(КомпонентыОбмена) Экспорт
...
Процедура ОбновитьНастройкиXDTOКорреспондента(КомпонентыОбмена) Экспорт
...
Функция ПредставлениеОбъектаДляПротокола(Объект)
		...
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 №%2 от %3'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, %2'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
	...
	ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления, ТипОбъекта, ПредставлениеОбъекта, НавигационнаяСсылка);
