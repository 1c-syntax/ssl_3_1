...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из СвойстваИсточника Цикл
		...
		Если ТипЗнч(ЗначениеСвойства) = Тип("Структура") Тогда
			...
			// Для строк табличной части расширения добавлены в строку,
			// поэтому получение Соответствия с расширениями определено ниже
			...
			Иначе
				...
				ИначеЕсли ТипСвойства = "Таблица" Тогда
					...
					Если ТипЗнч(ЗначениеСвойства) = Тип("ТаблицаЗначений") Тогда
						...
						ЕстьРасширенияXDTO = ЗначениеСвойства.Колонки.Найти("Расширения") <> Неопределено;
						
					ИначеЕсли ТипЗнч(ЗначениеСвойства) = Тип("Массив")
						И ЗначениеСвойства.Количество() > 0 Тогда
						
						ЕстьРасширенияXDTO = Ложь;
						
						ЭлементМассива = ЗначениеСвойства[0];
						Если ТипЗнч(ЭлементМассива) = Тип("Структура") Тогда
							
							ЕстьРасширенияXDTO = ЭлементМассива.Свойство("Расширения");
					...
					Для Каждого ИсточникСтрока Из ЗначениеСвойства Цикл
						...
						Если ЕстьРасширенияXDTO Тогда
							
							РасширенияСтроки = ИсточникСтрока.Расширения;
							
						Иначе
							
							РасширенияСтроки = Неопределено;
							
						КонецЕсли;
						
						ПриемникСтрока = ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, ИсточникСтрока, ТипТаблицы, СсылкиИзОбъекта, Ложь, РасширенияСтроки);
...
Процедура ТипXDTOПравилаОбработкиИзРасширенийПакетаXDTO(КомпонентыОбмена, ПОД, ТипXDTO)
	...
	Для каждого ОписаниеРасширения Из КомпонентыОбмена.РасширенияФормата Цикл
		...
		ТипXDTO = ФабрикаXDTO.Тип(ПространствоИменРасширения, ПОД.ОбъектВыборкиФормат);
		Если ТипXDTO <> Неопределено Тогда
...
Процедура КонвертироватьТаблицуОбъекта(Приемник, Знач КомпонентыОбмена, Знач ИмяСвойства, Знач СвойствоXDTO, Знач ТипЗначенияXDTO, Знач ЗначениеXDTO)
	...
	Если ТипЗнч(ТабличнаяЧастьXDTO) <> Тип("СписокXDTO") Тогда
		...
		МассивОбъектоXDTO = Новый Массив(1);
		МассивОбъектоXDTO[0] = ТабличнаяЧастьXDTO;
		
		ТабличнаяЧастьXDTO = МассивОбъектоXDTO;
	...
	Для Каждого СтрокаXDTO Из ТабличнаяЧастьXDTO Цикл
		...
		Для Каждого СвойствоСтрокиТЧ Из СтрокаXDTO.Свойства() Цикл
			...
			Иначе
				...
				РасширенноеСвойствоСтрокиТЧ = СтрокаТипЗначенияXDTO.Свойства.Получить(СвойствоСтрокиТЧ.Имя);
				...
				СтрокаТипСвойства = РасширенноеСвойствоСтрокиТЧ.Тип;
...
Процедура ПрочитатьСообщениеОбмена(КомпонентыОбмена, Результаты, ТаблицыДляЗагрузки = Неопределено, РежимАнализа = Ложь)
	...
	Пока КомпонентыОбмена.ФайлОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Если ТипОбъектаXDTO = Неопределено Тогда
			...
			// которое отсутствует в приемнике, например, 
			// объект в источнике добавлен расширением пакета XDTO и
			// это расширение отсутствует в приемнике.
			Продолжить;
