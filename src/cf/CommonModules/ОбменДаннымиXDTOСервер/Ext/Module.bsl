...
Процедура ИнициализироватьТаблицыПравилОбмена(КомпонентыОбмена) Экспорт
	...
	ИнициализироватьТаблицыПравилОбменаРасширенный(КомпонентыОбмена);
...
Функция ТаблицаПравилКонвертации(КомпонентыОбмена, ПравилаКонвертацииОбъектовМенеджераОбмена = Неопределено, ДляПолученияПоддерживаемыхОбъектов = Ложь)
...
Процедура ВыгрузитьОбъектыПоСсылке(КомпонентыОбмена, СсылкиИзОбъекта)
	...
	Если КомпонентыОбмена.ФильтрВыборкиИзменений <> Неопределено Тогда
		Для Каждого ЗначениеСсылки Из СсылкиИзОбъекта Цикл
			...
			ОбъектЗарегистрирован = ПланыОбмена.ИзменениеЗарегистрировано(КомпонентыОбмена.УзелКорреспондента, ЗначениеСсылки);
			
			Если ВыгружатьОбъектПоНеобходимости(КомпонентыОбмена, ЗначениеСсылки) Тогда
				ОбъектВыгружалсяРанее = РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах.ОбъектЕстьВРегистре(
					ЗначениеСсылки, КомпонентыОбмена.УзелКорреспондента);
					
				Если ОбъектЗарегистрирован
					Или Не ОбъектВыгружалсяРанее Тогда
					Если ОбщегоНазначения.СсылкаСуществует(ЗначениеСсылки) Тогда
						...
						Если Не ОбъектВыгружалсяРанее Тогда
				...
				КонецЕсли;
			Иначе
				Если ОбъектЗарегистрирован Тогда
					Если ОбщегоНазначения.СсылкаСуществует(ЗначениеСсылки) Тогда
						ОбъектВыгружаемыйПоСсылке = ЗначениеСсылки.ПолучитьОбъект();
						ВыгрузкаОбъектаВыборки(КомпонентыОбмена, ОбъектВыгружаемыйПоСсылке);
						КомпонентыОбмена.ВыгруженныеПоСсылкеОбъекты.Добавить(ЗначениеСсылки);
	...
	Для Каждого ЗначениеСсылки Из СсылкиИзОбъекта Цикл
		...
		Если КомпонентыОбмена.ВыгруженныеОбъекты.Найти(ЗначениеСсылки) = Неопределено
			...
			ОбъектЕстьВРегистре = КомпонентыОбмена.КешДанныхОбъектовДляРегистрацииВОбменах.Получить(ЗначениеСсылки);
			
			Если ОбъектЕстьВРегистре = Неопределено Тогда
				...
				Если Не РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах.ОбъектЕстьВРегистре(
					...
					Если ОбъектВыгружаемыйПоСсылке <> Неопределено Тогда
						...
						КомпонентыОбмена.КешДанныхОбъектовДляРегистрацииВОбменах.Вставить(ЗначениеСсылки, Истина);
...
// Методы этой области заимствованы из будущих проектов для поддержки обратной совместимости.
...
Функция ПравилКонвертацииЗапросаНаВыгрузкуБазовые(КомпонентыОбменаОтправка, ПространствоИменОбъектовФормата)
	...
	КомпонентыОбменаОтправка.МенеджерОбмена =
		МенеджерОбменаВерсииФормата(ВерсияФорматаОбмена, КомпонентыОбменаОтправка.УзелКорреспондента);
	
	Возврат ТаблицаПравилКонвертации(КомпонентыОбменаОтправка);
...
Функция ПравилКонвертацииЗапросаНаВыгрузкуРасширенные(КомпонентыОбменаОтправка, ПространствоИменОбъектовФормата)
	...
	Для каждого ОписаниеРасширенияФормата Из РасширенияФормата Цикл
		...
		ВсеПравилаКонвертацииОтправка = ТаблицаПравилКонвертации(КомпонентыОбменаОтправка);
...
Процедура ЗарегистрироватьСсылкиЗапросаПоНавигации(КомпонентыОбмена, ОтсутствующиеСсылки)
	...
	Для каждого ОписаниеСсылкиЗапроса Из ОтсутствующиеСсылки Цикл
		...
		Если ПодобраннаяОбъектнаяСсылка.ПолучитьОбъект() <> Неопределено
			И ОбъектыПовторнойВыгрузки.Найти(ПодобраннаяОбъектнаяСсылка) = Неопределено Тогда
...
Процедура ЗарегистрироватьСсылкиЗапросаПоПространствуИмен(КомпонентыОбмена, ОтсутствующиеСсылки, ПространствоИменОбъектовФормата)
	...
	Для каждого ОписаниеСсылкиЗапроса Из СсылкиЗапросаНаВыгрузку Цикл
		...
		Если НЕ ОписаниеСсылкиЗапроса.СсылкаНайденаИЗарегистрированаКОтправке Тогда
			...
			ФиксироватьПредупреждениеЗапросаНаВыгрузкуСсылкаНеНайдена(КомпонентыОбмена, ОписаниеСсылкиЗапроса);
...
Процедура НайтиСсылкуНаОтправкуПоКоллекцииПравил(КомпонентыОбмена, ПравилаКонвертации, ОписаниеСсылкиЗапроса)
	...
	НайденныеПКО = ПравилаКонвертации.НайтиСтроки(ОтборСтрокПКО);
	Для каждого ПравилоКонвертации Из НайденныеПКО Цикл
		...
		// В этой версии конвертации нет события "ПриПолученииЗапросаВыгрузкиОбъекта"
...
Процедура ПриОтправкеДанныхКорреспонденту(УзелКорреспондента, ОтправкаЭлемента, ПодобраннаяОбъектнаяСсылка)
	...
	ОбменДаннымиСобытия.ПриОтправкеДанныхКорреспонденту(ПодобраннаяОбъектнаяСсылка, ОтправкаЭлемента, 
		ЭтоНачальнаяВыгрузкаДанных, УзелКорреспондента, ЭтоАнализДанных);
...
Процедура РегистрироватьПодобранныеСсылки(КомпонентыОбмена, ОбъектыПовторнойВыгрузки, ОписаниеСсылкиЗапроса)
	...
	Для каждого ПодобраннаяОбъектнаяСсылка Из ОбъектыПовторнойВыгрузки Цикл
		...
		МенеджерРегистраДанныеОбъектов = РегистрыСведений.ДанныеОбъектовДляРегистрацииВОбменах;
		Если Не МенеджерРегистраДанныеОбъектов.ОбъектЕстьВРегистре(ПодобраннаяОбъектнаяСсылка, УзелКорреспондента) Тогда
			
			ДобавитьОбъектВФильтрРазрешенныхОбъектов(ПодобраннаяОбъектнаяСсылка, УзелКорреспондента);
		...
		Иначе
			...
			ФиксироватьПредупреждениеЗапросаНаВыгрузкуСсылкаНеПроходитФильтр(
...
Процедура ФиксироватьПредупреждениеЗапросаНаВыгрузкуСсылкаНеПроходитФильтр(КомпонентыОбмена, ОписаниеСсылкиЗапроса, ПодобраннаяОбъектнаяСсылка)
	
	ШаблонТекста = НСтр("ru = 'Объект %1 не будет повторно выгружен, т.к. не соответствует условиям фильтра.'");
	
	ПредставлениеСсылки = ПредставлениеОбъектаДляПротокола(ПодобраннаяОбъектнаяСсылка);
	...
	ЗаписатьВПротоколВыполнения(КомпонентыОбмена, ОписаниеПредупреждения, Неопределено, Ложь);
	
КонецПроцедуры

Процедура ФиксироватьПредупреждениеЗапросаНаВыгрузкуСсылкаНеНайдена(КомпонентыОбмена, ОписаниеСсылкиЗапроса)
