...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Для Каждого ПодключенныйОбъект Из ПодключенныеОбъекты Цикл
		...
		Если Источник <> Неопределено Тогда
			...
			Команда.ЗначениеПометки = "Форма.ПараметрыУдаленияПомеченных[""%Источник%""].ЗначениеПометки";
			Команда.Порядок = 20;
...
// Параметры:
//   ИмяПараметра           - см. СтандартныеПодсистемыСервер.УстановкаПараметровСеанса.ИменаПараметровСеанса
//   УстановленныеПараметры - см. СтандартныеПодсистемыСервер.УстановкаПараметровСеанса.ИменаПараметровСеанса
//
...
Процедура УдалениеПомеченныхПоРасписанию() Экспорт
	...
	УдалениеПомеченныхОбъектов.УдалитьПомеченныеОбъекты(УдаляемыеОбъекты);
...
Процедура ПроверкаАктивностиСеансаУдаленияОбъектов() Экспорт
	...
	ВремяЖизниСеанса = 3 * 60*60;
	ВремяСнятияБлокировки = ОбщегоНазначения.ТекущаяДатаПользователя() - ВремяЖизниСеанса;
	...
	|ГДЕ
	|	УдаляемыеОбъекты.ВремяБлокировки <= &ВремяСнятияБлокировки";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ru='Не удалось отключить проверку удаляемых объектов по причине:'") + ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры
...
Функция ВыполнитьОбработкуПричинНеудаления(ТаблицаДействий) Экспорт
	...
	Для Каждого Действие Из ТаблицаДействий Цикл
		Если Не ЗначениеЗаполнено(Действие.Источник.ВерсияДанных) 
			ИЛИ Не ЗначениеЗаполнено(Действие.ОбнаруженныйСсылка.ВерсияДанных) Тогда
...
// 	АдресРезультатаПредыдущегоШага - Строка
...
Функция УдалитьПомеченныеОбъекты(Знач ИсточникУдаляемыхОбъектов, РежимУдаления, НастройкиДополнительныхРеквизитов,
	...
	УдаляемыеОбъекты.Добавить(ИдентификаторЗадания);
	РезультатУдаления = УдалениеПомеченныхОбъектов.УдалитьПомеченныеОбъекты(УдаляемыеОбъекты, РежимУдаления);
...
Процедура УстановитьБлокировкуИспользованияУдаляемыхОбъектов(Пакет, Идентификатор) Экспорт
	...
	ВремяНачалаУдаления = ОбщегоНазначения.ТекущаяДатаПользователя();
...
Функция ОбработатьОчередьПометокНаУдаление(ОчередьПометкиКУдалению)
	...
	Для Каждого Элемент Из ОчередьПометкиКУдалению Цикл
		...
		Исключение
			...
			Описание = Результат.Добавить();
			Описание.УдаляемыйСсылка = Элемент;
			Описание.ТекстОшибки = ОписаниеОшибки();
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗапретитьИспользованиеУдаляемыхОбъектов(Источник, Отказ)
	...
	Если СсылкиНаУдаляемыеОбъекты.Количество() > 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Выполняется удаление ""%1"". 
							   |Замените ""%1"" на непомеченные на удаление аналоги.'");
		Для Каждого УдаляемаяСсылка Из СсылкиНаУдаляемыеОбъекты Цикл
			ОбщегоНазначения.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, УдаляемаяСсылка.Значение),
				УдаляемаяСсылка.Ключ, , , Отказ);
		КонецЦикла;
	КонецЕсли;
...
Функция ДеревоПомеченныхНаУдалениеБезУдаленных(ПомеченныеНаУдаление, Удаленные)
	...
	Для Каждого Элемент Из УдаляемыеОбъекты Цикл
		Если Элемент.Родитель <> Неопределено И Удаленные.Найти(Элемент.УдаляемыйСсылка) <> Неопределено Тогда
...
Процедура ДобавитьСтрокуСвязейНеудаленных(ТаблицаСвязиНеУдаленных, Причина, КешИнформацииОТипах)
	...
	ИначеЕсли Причина.МестоИспользования = Неопределено Тогда
		...
		СтрокаПрепятствующегоУдалению.Представление = Причина.Метаданные.Синоним + " (" + НСтр(
				"ru = 'Константа'") + ")";
		Вид = "КОНСТАНТА";
	Иначе
