...
Процедура УстановитьРасписаниеУдаленияПомеченных() Экспорт
	...
	РегламентныеЗаданияСервер.УстановитьПараметрыРегламентногоЗадания(Метаданные.РегламентныеЗадания.УдалениеПомеченныхОбъектов, 
		ПараметрыЗадания);
...
Функция ЭтоПростойТип(Тип) Экспорт
	Возврат (Тип = Неопределено Или Тип = Тип("Строка") Или Тип = Тип("Число") Или Тип = Тип("Булево") 
		Или Тип = Тип("Дата"));
КонецФункции
...
Процедура КонтрольУдаленияПомеченныхОбъектов() Экспорт
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.КонтрольУдаленияПомеченныхОбъектов);
	...
	Пока РезультатЗапроса.Количество() > 0 Цикл
		...
		Для Каждого ВыборкаДетальныеЗаписи Из РезультатЗапроса Цикл
			...
			Если СеансУдаленияОбработан = Неопределено Тогда
				ЕстьСеансУдаления = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ВыборкаДетальныеЗаписи.ИдентификаторСеанса) <> Неопределено;
...
Функция ОбработатьОчередьПометокНаУдаление(ОчередьПометкиКУдалению)
	...
	Если ПредставленияОшибок.Количество() > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Удаление помеченных'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , ,
			НСтр("ru='Не удалось установить пометку удаления для объектов:'") + Символы.ПС 
				+ СтрСоединить(ТекстОшибки, Символы.ПС + Символы.ПС));
...
Процедура ЗапретитьИспользованиеУдаляемыхОбъектов(Источник, Отказ)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Удаление помеченных'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, , ,
				НСтр("ru='Не удалось выполнить контроль удаляемых объектов:'") + ОбработкаОшибок.ПодробноеПредставлениеОшибки(Ошибка));
	КонецПопытки;
...
Функция ДанныеФормыИзРезультатаУдаления(ДеревоУдаляемых, РезультатУдаления, НастройкиДополнительныхРеквизитов, 
	...
	ДеревоПомеченныхНаУдаление = ДеревоФормыВУниверсальноеДерево(ДеревоУдаляемых, 
		ЧислоДополнительныхРеквизитов(НастройкиДополнительныхРеквизитов));
...
Функция ДеревоПомеченныхНаУдалениеБезУдаленных(ПомеченныеНаУдаление, Удаленные)
	...
	Для Каждого Элемент Из УдаляемыеОбъекты Цикл
		Если Элемент.Родитель <> Неопределено И (Удаленные.Найти(Элемент.УдаляемыйСсылка) <> Неопределено
				ИЛИ НЕ Элемент.ЭтоОписаниеОбъектаМетаданных И Элемент.Технический) Тогда
...
Процедура ДобавитьСтрокуСвязейНеудаленных(ТаблицаСвязиНеУдаленных, Причина, ИнформацияОТипах, Реквизиты)
	...
	СтрокаТаблицы.ОбнаруженныйСсылка = ?(СтрокаТаблицы.ЭтоОшибка,
		Причина.ОписаниеОшибки, Причина.МестоИспользования);
	...
	ИначеЕсли Причина.МестоИспользования = Неопределено Тогда
		...
		СтрокаТаблицы.Представление = ОбщегоНазначения.ПредставлениеОбъекта(Причина.Метаданные)
			+ " (" + НСтр("ru = 'Константа'") + ")";
		Вид = "КОНСТАНТА";
	Иначе
	...
	СтрокаТаблицы.НомерКартинки = ?(НомерКартинки <> 0, НомерКартинки, 
		НомерКартинки(СтрокаТаблицы.ОбнаруженныйСсылка, СтрокаТаблицы.СсылочногоТипа,
...
Функция ЭтоТехническийОбъект(Знач ПолноеИмяОбъекта)
	...
	Возврат ПолноеИмяОбъекта = ВРег("Справочник.ИдентификаторыОбъектовМетаданных")
