...
Процедура КонтрольУдаленияПомеченныхОбъектов() Экспорт
	...
	НачалоКонтроляУдаленияПомеченных = ТекущаяДатаСеанса();
	...
	ТекстЗапроса =
		...
		|УПОРЯДОЧИТЬ ПО
		|	ВремяБлокировки";
	УсловиеПоВремени = "УдаляемыеОбъекты.ВремяБлокировки > &ВремяБлокировки";
	...
	ОбработанныеСеансы = Новый Массив;
	Пока РезультатЗапроса.Количество() > 0 Цикл
		
		Для Каждого ВыборкаДетальныеЗаписи Из РезультатЗапроса Цикл
			Если ОбработанныеСеансы.Найти(ВыборкаДетальныеЗаписи.ИдентификаторСеанса) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Попытка
				Если ВыборкаДетальныеЗаписи.ВремяБлокировки <= ВремяСнятияБлокировки
					...
					ОбработанныеСеансы.Добавить(ВыборкаДетальныеЗаписи.ИдентификаторСеанса);
				КонецЕсли;
			Исключение
		...
		Если ВыборкаДетальныеЗаписи.ВремяБлокировки > НачалоКонтроляУдаленияПомеченных Тогда
			// Если параллельно работает удаление помеченных и дошли до новых записей, то прерываем выполнение.
			Прервать;
		КонецЕсли;
...
Функция ДеревоПомеченныхНаУдаление(УдаляемыеОбъекты, Настройки, Отмеченные)
	...
	Для Каждого УдаляемыйСсылка Из УдаляемыеОбъекты Цикл
		...
		УдаляемыйИнформация = ИнформацияОТипе(ТипУдаляемого, КешТиповУдаляемыхОбъектов);
