...
Функция ВыполнитьОбработкуПричинНеудаления(ТаблицаДействий) Экспорт
	...
	ВерсииДанных = СтандартныеПодсистемыСервер.ЗначенияРеквизитовОбъектовЕслиСуществуют(
		ТаблицаДействий.ВыгрузитьКолонку("Источник"), "ВерсияДанных");
		
	Для Каждого Действие Из ТаблицаДействий Цикл

		Если Действие.Действие <> "ЗаменитьСсылку" И (Не ЗначениеЗаполнено(ВерсииДанных[Действие.Источник])
...
Функция УдалитьПомеченныеОбъекты(Знач ИсточникУдаляемыхОбъектов, РежимУдаления, НастройкиДополнительныхРеквизитов,
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(УдаляемыеОбъекты, 
...
//
// Параметры:
//   Пакет - Массив из Структура - содержит удаляемые ссылки (УдаляемыйСсылка).
//   ИдентификаторСеанса - УникальныйИдентификатор - идентификатор сеанса, в котором выполняется удаление помеченных.
//
Процедура УстановитьБлокировкуИспользованияУдаляемыхОбъектов(Пакет, ИдентификаторСеанса) Экспорт
	...
	Попытка
		...
		КонецЕсли;
		Если НЕ ЗаблокироватьУдаляемыеОбъекты(ИдентификаторСеанса) Тогда
			ТекстСообщения = НСтр("ru = 'Удаление помеченных объектов уже выполняется. Пожалуйста, подождите...'");
			ИмяСобытия = НСтр("ru = 'Удаление помеченных'",
...
// Устанавливает управляемую блокировку на регистр сведений УдаляемыеОбъекты по измерению ИдентификаторСеанса.
//
// Параметры:
//   ИдентификаторСеанса - УникальныйИдентификатор - идентификатор сеанса, в котором выполняется удаление помеченных.
//
...
//            Ложь, если другой сеанс уже удерживает блокировку.
//
Функция ЗаблокироватьУдаляемыеОбъекты(ИдентификаторСеанса)
...
Функция ДеревоПомеченныхНаУдаление(УдаляемыеОбъекты, Настройки, Отмеченные)
	...
	ДополнительныеРеквизитыУдаляемыхОбъектов = СтандартныеПодсистемыСервер.ЗначенияРеквизитовОбъектовЕслиСуществуют(УдаляемыеОбъекты, "Дата");
	...
	Для Каждого УдаляемыйСсылка Из УдаляемыеОбъекты Цикл
		...
		ЗначенияРеквизитов = ДополнительныеРеквизитыУдаляемыхОбъектов.Получить(УдаляемыйСсылка);
		УзелУдаляемого.Дата = ?(ЗначенияРеквизитов = Неопределено, '00010101', ЗначенияРеквизитов.Дата);
	...
	ДеревоЗначений.Колонки.Удалить(ДеревоЗначений.Колонки.Количество);
		ДеревоЗначений.Строки.Сортировать("Дата, Представление", Истина);
...
Функция ДополнитьДеревоДополнительнымиРеквизитами(ДеревоЗначений, Настройки)
	...
	Для Каждого ТипМетаданных Из ДеревоЗначений.Строки Цикл
		...
		ДополнительныеРеквизиты = ЗначенияДополнительныхРеквизитов(ПомеченныеНаУдаление,
			Настройки.НайтиСтроки(Новый Структура("Метаданные", ВРег(ТипМетаданных.УдаляемыйСсылка))));
...
Функция ЗначенияДополнительныхРеквизитов(ПомеченныеНаУдаление, НастройкиДополнительныхРеквизитов)
	...
	Для Каждого Настройка Из НастройкиДополнительныхРеквизитов Цикл
		ДополнительныеРеквизиты.Добавить(Настройка.Реквизит);
	КонецЦикла;
...
Функция ОшибкиПредыдущегоШага(РезультатПредыдущегоШага)
	...
	Для Каждого Элемент Из РезультатПредыдущегоШага Цикл
		...
		ОшибкаПредыдущегоШага = Результат.Добавить();
		Если Не Элемент.ТребуетсяУдаление Тогда
			ОшибкаПредыдущегоШага.УдаляемыйСсылка = Элемент.УдаляемыйСсылка;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВключитьУдалениеПомеченныхОбъектов");
	Методы.Вставить("УстановитьРасписаниеУдаленияПомеченных");
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("УдалениеПомеченныхПоРасписанию");
	КонецЕсли;
	
	Методы.Вставить("ПомеченныеНаУдаление", Истина);
	Методы.Вставить("ВыполнитьОбработкуПричинНеудаления", Истина);
	Методы.Вставить("УдалитьПомеченныеОбъекты", Истина);
	Методы.Вставить("КонтрольУдаленияПомеченныхОбъектов", Истина);
