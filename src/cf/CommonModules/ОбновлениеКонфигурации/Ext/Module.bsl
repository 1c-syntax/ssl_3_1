...
Функция ПараметрыУстановкиИсправлений() Экспорт
	...
	ПараметрыУстановкиИсправлений = Новый Структура;
	
	ПараметрыУстановкиИсправлений.Вставить("ВФоне", Ложь);
	ПараметрыУстановкиИсправлений.Вставить("ОбновитьПараметрыРаботыРасширений", Истина);
	ПараметрыУстановкиИсправлений.Вставить("ПроверятьПрименимостьПоМанифесту", Ложь);
	ПараметрыУстановкиИсправлений.Вставить("ИспользуетсяВРаспределеннойИнформационнойБазе", Истина);
	
	Возврат ПараметрыУстановкиИсправлений;
...
#Область РаботаСИсправлениямиПатчами
...
Функция УстановкаИУдалениеИсправлений(Исправления, Знач ПараметрыУстановкиИсправлений = Неопределено, УдалитьОбновитьПараметрыРаботыРасширений = Истина, УдалитьПроверятьПрименимостьПоМанифесту = Ложь) Экспорт
	...
	Если ПараметрыУстановкиИсправлений = Неопределено Тогда
		ПараметрыУстановкиИсправлений = ПараметрыУстановкиИсправлений();
	ИначеЕсли ТипЗнч(ПараметрыУстановкиИсправлений) = Тип("Булево") Тогда
		ВФоне = ПараметрыУстановкиИсправлений;
		ПараметрыУстановкиИсправлений = ПараметрыУстановкиИсправлений();
		ПараметрыУстановкиИсправлений.ВФоне = ВФоне;
		ПараметрыУстановкиИсправлений.ОбновитьПараметрыРаботыРасширений = УдалитьОбновитьПараметрыРаботыРасширений;
		ПараметрыУстановкиИсправлений.ПроверятьПрименимостьПоМанифесту = УдалитьПроверятьПрименимостьПоМанифесту;
	КонецЕсли;
			...
			Попытка
				...
				Если ТипЗнч(Патч) = Тип("Структура") Тогда
					Если СтрЗаканчиваетсяНа(Патч.Имя, ".zip") Тогда
						ИсправлениеИзАрхива = ИзвлечьИсправлениеИзАрхива(Патч.Хранение, ПараметрыУстановкиИсправлений.ПроверятьПрименимостьПоМанифесту);
						Если ИсправлениеИзАрхива.Свойство("АрхивыПатчей") Тогда
							...
							Результат = УстановкаИУдалениеИсправлений(ИсправленияИзАрхива, ПараметрыУстановкиИсправлений);
					...
					КонецЕсли;
				Иначе
					ИсправлениеИзАрхива = ИзвлечьИсправлениеИзАрхива(Патч, ПараметрыУстановкиИсправлений.ПроверятьПрименимостьПоМанифесту);
					ИмяИсправления = ИсправлениеИзАрхива.ИмяПатча;
					Если ЗначениеЗаполнено(ИсправлениеИзАрхива.ТекстОшибки) Тогда
						ВызватьИсключение ИсправлениеИзАрхива.ТекстОшибки;
					КонецЕсли;
				...
				Расширение.ИспользуетсяВРаспределеннойИнформационнойБазе = ПараметрыУстановкиИсправлений.ИспользуетсяВРаспределеннойИнформационнойБазе;
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И (ПараметрыУстановкиИсправлений.ВФоне Или ЭтоФоновоеЗадание) Тогда
	...
	Если ПараметрыУстановкиИсправлений.ОбновитьПараметрыРаботыРасширений Тогда
...
Функция ДляТекущейВерсииНеобходимоУспешноеЗавершениеОбработчиков() Экспорт
	...
	ТекущийСтатус = СтатусОбновленияКонфигурации();
	Если ТекущийСтатус = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не ТекущийСтатус.Свойство("ВерсииТребующиеУспешногоОбновления") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ТекущийСтатус.ВерсииТребующиеУспешногоОбновления.Найти(Метаданные.Версия) <> Неопределено;
...
Процедура ЗавершитьОбновление(Знач РезультатОбновления, Знач ЭлектроннаяПочта, Знач ИмяАдминистратораОбновления, Знач КаталогСкрипта = Неопределено) Экспорт
	...
	Если Не ЕстьПраваНаУстановкуОбновления() Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для завершения обновления приложения.'");
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,,ТекстСообщения);
	...
	ПараметрыОбновления = ОбновлениеКонфигурацииВызовСервера.ПараметрыОбновления();
	ПараметрыОбновления.ИмяАдминистратораОбновления = ИмяАдминистратораОбновления;
	ПараметрыОбновления.ОбновлениеЗапланировано = Ложь;
	ПараметрыОбновления.ОбновлениеВыполнено = Истина;
	ПараметрыОбновления.РезультатОбновленияКонфигурации = РезультатОбновления;
	ПараметрыОбновления.КаталогСкрипта = КаталогСкрипта;
	ЗаписатьСтатусОбновления(ПараметрыОбновления);
...
Процедура ОбновитьИсправленияИзСкрипта(НовыеИсправления, УдаляемыеИсправления) Экспорт
	...
	Если Не ЕстьПраваНаУстановкуОбновления() Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для обновления исправлений (патчей).'");
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
		ВызватьИсключение(ТекстСообщения, КатегорияОшибки.НарушениеПравДоступа);
	КонецЕсли;
	...
	ПараметрыУстановкиИсправлений = ПараметрыУстановкиИсправлений();
	ПараметрыУстановкиИсправлений.ОбновитьПараметрыРаботыРасширений = Ложь;
	Результат = УстановкаИУдалениеИсправлений(Исправления, ПараметрыУстановкиИсправлений);
...
Процедура ВключитьПрименимыеИсправления() Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Начато включение исправлений после запуска обновления приложения.'");
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	...
	Для Каждого Подсистема Из ОписанияПодсистем.ПоИменам Цикл
		БиблиотекиКонфигурации.Вставить(Подсистема.Ключ, Подсистема.Значение.Версия);
	КонецЦикла;
	ХранилищеМанифестов = Константы.СвойстваИсправленийПередОбновлениемИБ.Получить();
	МанифестыИсправлений = ХранилищеМанифестов.Получить();
	
	ВсеРасширения = РасширенияКонфигурации.Получить();
	Для Каждого Расширение Из ВсеРасширения Цикл
		Если Не ЭтоИсправление(Расширение) Или Расширение.Активно Тогда
			Продолжить;
		КонецЕсли;
		
		Попытка
			ВключитьИсправление = ИсправлениеПрименимоДляТекущейВерсииКонфигурации(
				БиблиотекиКонфигурации, МанифестыИсправлений, Расширение.Имя);
			
			Если ВключитьИсправление Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось включить исправление ""%1"" по причине:
			|
			|%2'"), Расширение.Имя, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Исправления.Включение'", ОбщегоНазначения.КодОсновногоЯзыка())
			, УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		КонецПопытки;
	КонецЦикла;
	
	ТекстСообщения = НСтр("ru = 'Включение исправлений завершено.'");
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	
	ОчиститьСвойстваИсправленийПередОбновлениемИБ();
...
Процедура ЗаписатьСтатусОбновления(СтатусОбновления, СообщенияДляЖурналаРегистрации = Неопределено) Экспорт
	...
	СтатусОбновления.Вставить("РезультатУстановкиПатчей", Неопределено);
	...
	Если СтарыйСтатус <> Неопределено
		...
		СтатусОбновления.РезультатУстановкиПатчей = СтарыйСтатус.РезультатУстановкиПатчей;
	КонецЕсли;
...
Процедура ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, Знач АдресНазначения, Знач УспешноеОбновление)
	...
	Тема = ? (УспешноеОбновление, НСтр("ru = 'Успешное обновление ""%1"", версия %2'"), 
		НСтр("ru = 'Ошибка обновления ""%1"", версия %2'"));
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Тема, Метаданные.КраткаяИнформация, Метаданные.Версия);
	
	Подробности = ?(УспешноеОбновление, НСтр("ru = 'Обновление приложения завершено успешно.'"), 
		НСтр("ru = 'Не удалось обновить приложение, подробности см. в журнале регистрации.'"));
...
Процедура УстановитьСтатусОбновленияКонфигурации(Знач Статус)
	...
	Константы.СтатусОбновленияКонфигурации.Установить(Новый ХранилищеЗначения(Статус, Новый СжатиеДанных(9)));
...
Процедура ЗаписатьМанифестыИсправленийКОтключению() Экспорт
	
	МанифестыИсправлений = Новый Соответствие;
	ВсеРасширения = РасширенияКонфигурации.Получить();
	Для Каждого Расширение Из ВсеРасширения Цикл
		...
		Если Метаданные.ОбщиеМакеты.Найти(ИмяИсправления) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		МанифестИсправления = ПолучитьОбщийМакет(ИмяИсправления);
		МанифестыИсправлений.Вставить(ИмяИсправления, МанифестИсправления);
	КонецЦикла;

	Константы.СвойстваИсправленийПередОбновлениемИБ.Установить(
		Новый ХранилищеЗначения(МанифестыИсправлений, Новый СжатиеДанных(9)));
	
КонецПроцедуры

Функция ИсправлениеПрименимоДляТекущейВерсииКонфигурации(БиблиотекиКонфигурации, МанифестыИсправлений, ИмяИсправления)
	
	ИсправлениеПрименимо = Ложь;
	
	Если ТипЗнч(МанифестыИсправлений) <> Тип("Соответствие") Тогда
		Возврат ИсправлениеПрименимо;
	КонецЕсли;
	
	МанифестИсправления = МанифестыИсправлений.Получить(ИмяИсправления);
	
	Если МанифестИсправления = Неопределено Тогда
		Возврат ИсправлениеПрименимо;
	КонецЕсли;
	
	СтрокаXML = МанифестИсправления.ПолучитьТекст();
	...
	ТекстСообщения = НСтр("ru = 'Проверка исправления ""%1"".'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ИмяИсправления);
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	
	Для Каждого ИнформацияОПрименимости Из СвойстваИсправления.AppliedFor Цикл
		ВерсияБиблиотекиКонфигурации = БиблиотекиКонфигурации.Получить(
		ИнформацияОПрименимости.ConfigurationName);
		Если ВерсияБиблиотекиКонфигурации = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОчиститьСвойстваИсправленийПередОбновлениемИБ()
	
	Константы.СвойстваИсправленийПередОбновлениемИБ.Установить(Неопределено);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОписанияНовыхПатчей", Истина);
