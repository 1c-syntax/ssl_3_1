			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При установке исправления %1 произошла ошибка:
					           |
					           |%2'"), ИмяИсправления, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ДоступнаУстановкаПатчейВБазовойВерсии() Экспорт
	
	ДоступнаУстановкаПатчей = Истина;
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(ВерсияПлатформыБезНомераСборки, "8.3.15") < 0
		...
		Или (ВерсияПлатформыБезНомераСборки = "8.3.16"
			И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.16.1359") < 0)
		Или (ВерсияПлатформыБезНомераСборки = "8.3.17"
			И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.17.1386") < 0) Тогда
		ДоступнаУстановкаПатчей = Ложь;
	КонецЕсли;
	
	Возврат ДоступнаУстановкаПатчей;
	
КонецФункции

Функция ОписанияНовыхПатчей(АдресХранилища, ТекущиеИсправления) Экспорт
	...
	Для Каждого Исправление Из ТекущиеИсправления Цикл
		...
		Для НомерСтроки = 1 По Макет.КоличествоСтрок() Цикл
			...
			Если Не СтрНачинаетсяС(СокрЛП(Строка), "<Description>") Тогда
				Продолжить;
			КонецЕсли;
			ОписаниеПатча = СтрЗаменить(СтрЗаменить(СокрЛП(Строка), "<Description>", ""), "</Description>", "");
