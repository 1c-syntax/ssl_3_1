...
Процедура ПоказатьПраваРолейПользователя(Роли, Пользователь) Экспорт
	
	Отбор = Новый Структура("НачальныйОтбор", Новый Структура("Роль", Роли));
	КлючВарианта = "ПраваРолей";
	
	КлючНазначенияИспользования = КлючВарианта;
	УточнитьКлючНазначенияИспользования(КлючНазначенияИспользования,
		Новый Структура("Пользователь", Пользователь), "Пользователь");
	СократитьКлючНазначенияИспользования(КлючНазначенияИспользования);
	...
	ОткрытьФорму("Отчет.ПраваРолей.Форма", ПараметрыОтчета, ЭтотОбъект);
...
// Для форм ПраваДоступа, ПраваДоступаУпрощенно.
//
// Параметры:
//  РазличияРолей - см. УправлениеДоступомСлужебный.РазличияНастроенногоСоставаРолейСРолямиПользователяИБ
//
Процедура ПоказатьРазличияНастроенногоСоставаРолейСРолямиПользователяИБ(РазличияРолей) Экспорт
	
	Если Не ЗначениеЗаполнено(РазличияРолей) Тогда
		Возврат;
	КонецЕсли;
	
	СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(Неопределено, РазличияРолей);
