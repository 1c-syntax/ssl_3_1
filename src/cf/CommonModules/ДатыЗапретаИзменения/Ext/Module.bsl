...
Функция ИзменениеЗапрещено(ДанныеИлиПолноеИмя, ИдентификаторДанных = Неопределено,
	...
	ИсточникиДанныхТаблиц = ДатыЗапретаИзмененияСлужебный.ИсточникиДанныхДляПроверкиЗапретаИзменения();
	ИсточникиДанных = ИсточникиДанныхТаблиц.Получить(ОбъектМетаданных.ПолноеИмя());
	Если ИсточникиДанных = Неопределено Тогда
		Возврат Ложь; // Для текущего типа объекта не определены запреты по датам.
	КонецЕсли;
	ЭтоРегистр = ИсточникиДанных.ЭтоРегистр;
	...
	Если ТипЗнч(ДанныеИлиПолноеИмя) = Тип("Строка") Тогда
		...
		ИначеЕсли ЭтоРегистр Тогда
			Источник = ИдентификаторДанных; // Набор новых записей для получения существующих записей.
			Замещение = ИдентификаторДанных.ДополнительныеСвойства.Замещение;
			Если Замещение = Ложь
			 Или ОбщегоНазначения.ЭтоДобавлениеНабораЗаписей(Замещение) Тогда
				Возврат Ложь;
			КонецЕсли;
		Иначе
	...
	Если ЭтоРегистр Тогда
		...
		ЗаполнитьЗначенияСвойств(Режим, ДанныеИлиПолноеИмя.ДополнительныеСвойства);
	КонецЕсли;
	...
	ИначеЕсли ВерсияОбъекта = "НоваяВерсия" Тогда
		...
		Если ЭтоРегистр И ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Режим.Замещение) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
