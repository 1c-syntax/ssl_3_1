...
Функция ИзменениеЗапрещено(Данные, ИдентификаторДанных, ОписаниеОшибки, УзелПроверкиЗапретаЗагрузки) Экспорт
	...
	Если ИдентификаторДанных <> Неопределено Тогда
		...
		ЗапретНайден = НайденЗапретИзмененияДанных(ДанныеИДаты,
...
//
// Параметры:
//  ДанныеДляПроверки - см. ДатыЗапретаИзменения.НайденЗапретИзменения.ДанныеДляПроверки
//                    - Структура:
...
//
//  ПараметрыСообщенияОЗапрете - см. ДатыЗапретаИзменения.НайденЗапретИзмененияДанных.ПараметрыСообщенияОЗапрете
//
...
Функция ЗначениеПоля(ЗначенияПолей, Поле)
	...
	Для Каждого ПолеПути Из ПоляПути Цикл
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекущаяТаблица", МетаданныеСсылки.ПолноеИмя());
...
//    * НачалоДня - Дата
//    * СвойстваРазделов - ФиксированнаяСтруктура:
...
Функция СообщениеОЗапретах(Запреты, ПараметрыСообщенияОЗапрете, СвойстваРазделов, ПоискЗапретовЗагрузки, СтруктурноеОписание)
	...
	Для Каждого Запрет Из Запреты Цикл
		...
		КонецЕсли;
		Если ДобавленныеТексты.Получить(Текст) = Неопределено Тогда
...
Функция СообщениеОЗапрете(Знач Запрет, Знач ПоискЗапретовЗагрузки, Знач СвойстваРазделов, Знач ПустойРаздел)
	...
	Если Не ПоискЗапретовЗагрузки Тогда
		Если      Не ПоРазделу И Не ПоОбъекту И ДляНеуказанного Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет изменения данных по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляНеуказанного Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет изменения данных по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляНеуказанного Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет изменения данных по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляНеуказанного Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет изменения данных по %5'");
		
		ИначеЕсли Не ПоРазделу И Не ПоОбъекту И ДляПользователя Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет изменения данных для пользователя ""%4"" по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляПользователя Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет изменения данных для пользователя ""%4"" по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляПользователя Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет изменения данных для пользователя ""%4"" по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляПользователя Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет изменения данных для пользователя ""%4"" по %5'");
		
		ИначеЕсли Не ПоРазделу И Не ПоОбъекту И ДляГруппыПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет изменения данных для группы пользователей ""%4"" по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляГруппыПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет изменения данных для группы пользователей ""%4"" по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляГруппыПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет изменения данных для группы пользователей ""%4"" по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляГруппыПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет изменения данных для группы пользователей ""%4"" по %5'");
		
		ИначеЕсли Не ПоРазделу И Не ПоОбъекту И ДляВсехПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет изменения данных для всех пользователей по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляВсехПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет изменения данных для всех пользователей по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляВсехПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет изменения данных для всех пользователей по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляВсехПользователей Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет изменения данных для всех пользователей по %5'");
		КонецЕсли;
	Иначе
		Если      Не ПоРазделу И Не ПоОбъекту И ДляИнформационнойБазы Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет загрузки данных для информационной базы ""%4"" по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляИнформационнойБазы Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет загрузки данных для информационной базы ""%4"" по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляИнформационнойБазы Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет загрузки данных для информационной базы ""%4"" по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляИнформационнойБазы Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет загрузки данных для информационной базы ""%4"" по %5'");
		
		ИначеЕсли Не ПоРазделу И Не ПоОбъекту И ДляВсехИБПлана Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет загрузки данных для всех информационных баз ""%6"" по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляВсехИБПлана Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет загрузки данных для всех информационных баз ""%6"" по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляВсехИБПлана Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет загрузки данных для всех информационных баз ""%6"" по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляВсехИБПлана Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет загрузки данных для всех информационных баз ""%6"" по %5'");
		
		ИначеЕсли Не ПоРазделу И Не ПоОбъекту И ДляВсехИБ Тогда
			Текст = НСтр("ru = 'Дате %1 соответствует запрет загрузки данных для всех информационных баз по %5'");
		ИначеЕсли    ПоРазделу И Не ПоОбъекту И ДляВсехИБ Тогда
			Текст = НСтр("ru = 'Дате %1 по разделу ""%2"" соответствует запрет загрузки данных для всех информационных баз по %5'");
		ИначеЕсли Не ПоРазделу И    ПоОбъекту И ДляВсехИБ Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" соответствует запрет загрузки данных для всех информационных баз по %5'");
		ИначеЕсли    ПоРазделу И    ПоОбъекту И ДляВсехИБ Тогда
			Текст = НСтр("ru = 'Дате %1 по объекту ""%3"" раздела ""%2"" соответствует запрет загрузки данных для всех информационных баз по %5'");
		КонецЕсли;
	КонецЕсли;
