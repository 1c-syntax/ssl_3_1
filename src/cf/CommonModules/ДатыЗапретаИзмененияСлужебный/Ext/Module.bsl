...
//
//  Замещение       - РежимЗамещения, Булево - режим замещения записей (только для регистров).
//
...
Функция ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(Источник, ИсточникРегистр, Замещение, Удаление, 
	...
	ИначеЕсли ИсточникРегистр
	        И (Замещение = Истина
	           Или ОбщегоНазначения.ЭтоЗамещениеНабораЗаписей(Замещение)
	           Или ОбщегоНазначения.ЭтоОбновлениеНабораЗаписей(Замещение)
	           Или ОбщегоНазначения.ЭтоСлияниеНабораЗаписей(Замещение)
	           Или ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Замещение)) Тогда
		
		ПолноеИмя = Источник.Метаданные().ПолноеИмя();
		УточненныйИсточник = ?(ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Замещение),
			ПолноеИмя, Источник);
		УточненныйИдентификаторДанных = ?(Замещение = Истина, Источник.Отбор, Источник);
		
		Если ИзменениеЗапрещено(
				?(ВерсияОбъекта <> "СтараяВерсия", УточненныйИсточник, ПолноеИмя),
				?(ВерсияОбъекта <> "НоваяВерсия",  УточненныйИдентификаторДанных, Неопределено),
				Результат.ОписаниеОшибки,
				УзелПроверкиЗапретаЗагрузки) Тогда
	...
	Иначе
		...
		// ИЛИ    ИсточникРегистр И (Замещение = Ложь Или ОбщегоНазначения.ЭтоДобавлениеНабораЗаписей(Замещение))
...
Функция ИзменениеЗапрещено(Данные, ИдентификаторДанных, ОписаниеОшибки, УзелПроверкиЗапретаЗагрузки) Экспорт
	...
	Если ИдентификаторДанных <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ДанныеДляПроверки) Тогда
...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки, ПараметрыСообщенияОЗапрете,
	...
	Для Каждого СтрокаТаблицы Из ДанныеДляПроверки Цикл
		...
		Если СтрокаТаблицы.Раздел = Неопределено Тогда
			СтрокаТаблицы.Раздел = ПустойРаздел;
		КонецЕсли;
		
		СвойстваРаздела = СвойстваРазделов.Разделы.Получить(СтрокаТаблицы.Раздел); // см. ПланыВидовХарактеристик.РазделыДатЗапретаИзменения.СвойстваРаздела
		Если СвойстваРаздела = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ДанныеДляПроверки",
				СтрокаТаблицы.Раздел);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		
		Если СвойстваРазделов.БезРазделовИОбъектов Тогда
			СтрокаТаблицы.Раздел = ПустойРаздел;
			СтрокаТаблицы.Объект = ПустойРаздел;
		Иначе
			Если ЗначениеЗаполнено(СвойстваРазделов.ЕдинственныйРаздел) Тогда
				СтрокаТаблицы.Раздел = СвойстваРазделов.ЕдинственныйРаздел;
			Иначе
				СтрокаТаблицы.Раздел = СвойстваРаздела.Ссылка;
			КонецЕсли;
			
			Если СвойстваРазделов.ВсеРазделыБезОбъектов
			 Или Не ЗначениеЗаполнено(СтрокаТаблицы.Объект) Тогда
				
				СтрокаТаблицы.Объект = СтрокаТаблицы.Раздел;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого РазделИОбъект Из РазделыИОбъекты Цикл
		...
		Для Каждого СтрокаТаблицы Из ДанныеДляПроверки.НайтиСтроки(Отбор) Цикл
			ТекущаяДата = НачалоДня(СтрокаТаблицы.Дата);
...
Функция ТекстОшибкиДатыЗапретаЗагрузкиНеВнедрены() Экспорт
	...
	Возврат НСтр("ru = 'Даты запрета загрузки данных прошлых периодов из других приложений
...
Функция РассчитанныеДатыЗапретаИзменения() Экспорт
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		СтрокаТаблицы.ДатаЗапрета = ДатаЗапретаПоОписанию(СтрокаТаблицы.ОписаниеДатыЗапрета,
			СтрокаТаблицы.ДатаЗапрета , НачалоДня);
	КонецЦикла;
...
Функция ДанныеДляПроверкиИзБазыДанных(Данные, ИдентификаторДанных, ДействующиеДаты, УзелПроверкиЗапретаЗагрузки)
	...
	Если ИсточникиДанных.ЭтоРегистр Тогда
		ОбщегоНазначения.ОтборЗаписейНабораИзБазыДанных(ИдентификаторДанных,, Запрос);
	Иначе
...
Функция ДанныеДляПроверкиИзОбъекта(Данные, ДействующиеДаты, УзелПроверкиЗапретаЗагрузки)
	...
	Если ИсточникиДанных.ЭтоРегистр Тогда
		...
		Если ЗначенияПолей.Колонки.Найти("Регистратор") <> Неопределено
		   ...
		   И Данные.Отбор.Регистратор.Использование
		...
		КонецЕсли;
		Для Каждого СтрокаТаблицы Из ЗначенияПолей Цикл
			Для Каждого ИсточникДанных Из ИсточникиДанных.Состав Цикл
				// @skip-check query-in-loop - Цикл для исключения разыменования составного типа (стандарт 654)
				ДобавитьСтрокуДанных(СтрокаТаблицы, СтрокаТаблицы, ИсточникДанных, ДанныеДляПроверки, БезОбъекта);
			КонецЦикла;
		КонецЦикла;
	Иначе
		Для Каждого ИсточникДанных Из ИсточникиДанных.Состав Цикл
			...
			ИначеЕсли ИсточникДанных.ПолеДаты.ТабличнаяЧасть = ИсточникДанных.ПолеОбъекта.ТабличнаяЧасть Тогда
				...
				Для Каждого СтрокаТаблицы Из Значения Цикл
					// @skip-check query-in-loop - Цикл для исключения разыменования составного типа (стандарт 654)
					ДобавитьСтрокуДанных(СтрокаТаблицы, СтрокаТаблицы, ИсточникДанных, ДанныеДляПроверки, БезОбъекта);
				КонецЦикла;
			Иначе
...
Функция ЗначениеПараметраСеансаДействующиеДатыЗапретаИзменения() Экспорт
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Для Каждого СтрокаТаблицы Из Выгрузка.Строки Цикл
			ГруппыПользователей.Вставить(СтрокаТаблицы.Пользователь,
				Новый ФиксированныйМассив(СтрокаТаблицы.Строки.ВыгрузитьКолонку("ГруппаПользователей")));
		КонецЦикла;
...
Функция СообщениеОЗапретах(Запреты, ПараметрыСообщенияОЗапрете, СвойстваРазделов, ПоискЗапретовЗагрузки, СтруктурноеОписание)
	...
	ЭтоРегистр = Ложь;
	Текст = ПредставлениеДанных(ПараметрыСообщенияОЗапрете.Данные, ЭтоРегистр);
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Иначе
			...
			ИначеЕсли ЭтоРегистр И ЭтоУдалениеНабораДанных(ПараметрыСообщенияОЗапрете.Данные) Тогда
...
Функция ПредставлениеДанных(Данные, ЭтоРегистр)
...
Функция ЭтоУдалениеНабораДанных(Данные)
	...
	Иначе
		Замещение = Данные.ДополнительныеСвойства.Замещение;
		Если ОбщегоНазначения.ЭтоЗамещениеНабораЗаписей(Замещение) Тогда
			Возврат Данные.Количество() = 0;
		ИначеЕсли ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Замещение) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапросДатЗапретаИзменения()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗначенияКонстант.ВерсияДатЗапретаИзменения КАК ВерсияДатЗапретаИзменения,
	|	ЗначенияКонстант.ИспользоватьДатыЗапретаЗагрузки КАК ИспользоватьДатыЗапретаЗагрузки,
	|	ЗначенияКонстант.ИспользоватьДатыЗапретаИзменения КАК ИспользоватьДатыЗапретаИзменения
	|ИЗ
	|	Константы КАК ЗначенияКонстант
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция ТекущиеИсточникиДанныхДляПроверкиЗапретаИзменения(СвойстваРазделов)
	...
	Для Каждого СтрокаТаблицы Из Таблицы Цикл
		...
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(СтрокаТаблицы.Таблица);
		Если ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		ЭтоРегистр = СтандартныеПодсистемыСервер.ЭтоТаблицаРегистра(СтрокаТаблицы.Таблица);
		ИсточникиТаблицы.Вставить("ЭтоРегистр", ЭтоРегистр);
		
		ИсточникиДанныхТаблицы = ИсточникиДанных.НайтиСтроки(Новый Структура("Таблица", СтрокаТаблицы.Таблица));
		...
		Для Каждого ИсточникДанныхТаблицы Из ИсточникиДанныхТаблицы Цикл
			СвойстваРаздела = СвойстваРазделов.Разделы.Получить(ИсточникДанныхТаблицы.Раздел);
			Если СвойстваРаздела = Неопределено Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для проверки запрета изменения в источнике данных указан несуществующий
					           |раздел ""%1"" для таблицы ""%2""
					           |(см. %3).'"),
					ИсточникДанныхТаблицы.Раздел,
					ИсточникДанныхТаблицы.Таблица,
					"ДатыЗапретаИзмененияПереопределяемый.ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
			
			Источник = ОписаниеИсточникаДанных();
			Источник.Раздел = ИсточникДанныхТаблицы.Раздел;
			Источник.ПолеДаты    = ПолеТаблицы(ИсточникДанныхТаблицы, "ПолеДаты",    ОбъектМетаданных, ЭтоРегистр);
			Источник.ПолеОбъекта = ПолеТаблицы(ИсточникДанныхТаблицы, "ПолеОбъекта", ОбъектМетаданных, ЭтоРегистр);
...
Процедура ДобавитьНастройки(НаборЗаписей, Настройки, АдресатыНастроек, ГруппыПользователей, НастройкиГрупп)
	...
	Для Каждого АдресатНастроек Из АдресатыНастроек Цикл
		...
		Иначе
			...
			Для Каждого СтрокаТаблицы Из ГруппыПользователей.НайтиСтроки(Отбор) Цикл
				Отбор = Новый Структура("Пользователь", СтрокаТаблицы.ГруппаПользователей);
		...
		Для Каждого ДополнительнаяНастройка Из ДополнительныеНастройки Цикл
			Отбор = Новый Структура("Пользователь, Раздел, Объект",
				АдресатНастроек.Пользователь, ДополнительнаяНастройка.Раздел, ДополнительнаяНастройка.Объект);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьРазделыДатЗапретаИзменения");
	Методы.Вставить("ДополнитьНастройкиУказанныхАдресатов");
	Методы.Вставить("ЗаменитьРазделыДатЗапретаНаНовые");
	Методы.Вставить("ОчиститьПредопределенныеЭлементыРазделовДатЗапрета");
	Методы.Вставить("УстановитьНачальнуюДатуЗапретаИзменения");
	Методы.Вставить("УстановкаПараметровСеанса");
