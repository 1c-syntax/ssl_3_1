...
// Запускает фоновое задание по регламентному заданию или выполняет его метод не в фоне.
// Пользователь для запуска всегда текущий (платформа не дает возможности его указать).
//
// Параметры:
//  Идентификатор - ОбъектМетаданных - метаданные регламентного задания.
//                - Строка - имя метаданных регламентного задания.
//                - РегламентноеЗадание - экземпляр регламентного задания, когда нужно
//                    выполнить с параметрами (можно передать созданный незаписанный объект).
//
//  ПараметрыВыполнения - см. НовыеПараметрыВыполненияЗадания
//                      - Неопределено - использовать значения по умолчанию.
//
// Возвращаемое значение:
//  ФоновоеЗадание - запущенное задание.
//  Неопределено - если указано выполнить не в фоне.
//
Функция ВыполнитьЗадание(Идентификатор, Знач ПараметрыВыполнения = Неопределено) Экспорт
	
	РегламентноеЗадание = Неопределено;
	
	Если ТипЗнч(Идентификатор) = Тип("РегламентноеЗадание") Тогда
		РегламентноеЗадание = Идентификатор;
		МетаданныеЗадания = Идентификатор.Метаданные;
		
	ИначеЕсли ТипЗнч(Идентификатор) = Тип("ОбъектМетаданных") Тогда
		Если Не Метаданные.РегламентныеЗадания.Содержит(Идентификатор) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недопустимое значение параметра %1 в %2.
				           |Ожидался объект метаданных регламентного задания, а передан другой объект метаданных.'"),
				"Идентификатор",
				"РегламентныеЗаданияСервер.ВыполнитьЗадание");
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
		МетаданныеЗадания = Идентификатор;
		
	ИначеЕсли ТипЗнч(Идентификатор) = Тип("Строка") Тогда
		МетаданныеЗадания = Метаданные.РегламентныеЗадания.Найти(Идентификатор);
		Если МетаданныеЗадания = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недопустимое значение параметра %1 в %2.
				           |Регламентное задание ""%3"" не существует в метаданных.'"),
				"Идентификатор",
				"РегламентныеЗаданияСервер.ВыполнитьЗадание",
				Идентификатор);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	Иначе
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("РегламентныеЗаданияСервер.ВыполнитьЗадание",
			"Идентификатор", Идентификатор, Новый ОписаниеТипов("ОбъектМетаданных,Строка,РегламентноеЗадание"));
	КонецЕсли;
	
	Параметры = ?(РегламентноеЗадание <> Неопределено, РегламентноеЗадание.Параметры, Неопределено);
	
	ПараметрыВыполнения = ?(ПараметрыВыполнения = Неопределено,
		НовыеПараметрыВыполненияЗадания(), ПараметрыВыполнения);
	
	Если Не ПараметрыВыполнения.ВФоне Тогда
		ПараметрыСтрока = "";
		...
		КонецЕсли;
		Выполнить(Метаданные.РегламентныеЗадания[МетаданныеЗадания.Имя].ИмяМетода + "(" + ПараметрыСтрока + ")"); // АПК:487 Исполняемый код безопасен. Не нарушает безопасность информации, так как выполняется только процедура, указанная в метаданных регламентного задания.
		Возврат Неопределено;
	КонецЕсли;
	
	Отбор = Неопределено;
	Если ПараметрыВыполнения.ПоискАктивного = Истина Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("ИмяМетода", МетаданныеЗадания.ИмяМетода);
		Отбор.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
		
	ИначеЕсли ТипЗнч(ПараметрыВыполнения.ПоискАктивного) = Тип("Структура") Тогда
		Отбор = ПараметрыВыполнения.ПоискАктивного;
		
	ИначеЕсли ПараметрыВыполнения.ПоискАктивного <> Ложь Тогда
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("РегламентныеЗаданияСервер.ВыполнитьЗадание",
			"ПараметрыВыполнения", ПараметрыВыполнения, Тип("Структура"),
			Новый Структура("ПоискАктивного", Новый ОписаниеТипов("Булево,Структура")));
	КонецЕсли;
	...
	Ключ = ПараметрыВыполнения.Ключ;
	Если Ключ = Неопределено Тогда
		Если РегламентноеЗадание <> Неопределено И ЗначениеЗаполнено(РегламентноеЗадание.Ключ) Тогда
			Ключ = РегламентноеЗадание.Ключ;
		ИначеЕсли ЗначениеЗаполнено(МетаданныеЗадания.Ключ) Тогда
			Ключ = МетаданныеЗадания.Ключ;
		КонецЕсли;
	КонецЕсли;
	
	Наименование = ПараметрыВыполнения.Наименование;
	Если Наименование = Неопределено Тогда
		Если РегламентноеЗадание <> Неопределено Тогда
			Наименование = РегламентноеЗадание.Наименование;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Наименование) Тогда
			Наименование = МетаданныеЗадания.Представление();
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Наименование) Тогда
			Наименование = МетаданныеЗадания.Имя;
		КонецЕсли;
	КонецЕсли;
	
	Если ТипЗнч(ПараметрыВыполнения.ЭтоЗапускВручную) = Тип("Булево") Тогда
		Если РегламентноеЗадание <> Неопределено Тогда
			УстановитьПривилегированныйРежим(Истина);
			Если РегламентныеЗадания.НайтиПоУникальномуИдентификатору(РегламентноеЗадание.УникальныйИдентификатор) <> Неопределено Тогда
				Наименование = Наименование + " " + Строка(РегламентноеЗадание.УникальныйИдентификатор);
			КонецЕсли;
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
		Если ПараметрыВыполнения.ЭтоЗапускВручную Тогда
			Шаблон = НСтр("ru = 'Запуск вручную: %1 (из сеанса %2 от %3)'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			Шаблон = НСтр("ru = 'Автозапуск: %1 (из сеанса %2 от %3)'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
		ТекущийСеанс = ПолучитьТекущийСеансИнформационнойБазы();
		Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
			Наименование,
	...
	Если ПараметрыВыполнения.СРасширениямиБазыДанных = Истина Тогда
		ФоновоеЗадание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(
			Метаданные.РегламентныеЗадания[МетаданныеЗадания.Имя].ИмяМетода,
...
// Конструктор параметров для функции ВыполнитьЗадание.
//
// Возвращаемое значение:
//  Структура:
//    * ВФоне - Булево - по умолчанию Истина.
//                Если Ложь, тогда метод регламентного задания вызывается в текущем сеансе,
//                остальные свойства (ниже) не учитываются.
//
//    * ЭтоЗапускВручную - Булево - по умолчанию Истина (для уточнения наименования задания).
//                           Истина - если запуск выполняется пользователем по кнопке из формы, тогда уточнение
//                             "Запуск вручную: <параметр Наименование> (из сеанса <номер> от <дата>)".
//                           Ложь - если запуск выполняется программно при каком-то событии (в ходе работы
//                             задания изменились метаданные или после записи объекта требуется отработать
//                             быструю часть обновления прав на зависимые объекты), тогда уточнение
//                             "Автозапуск: <параметр Наименование> (из сеанса <номер> от <дата>)".
//                        - Неопределено - использовать значение свойства Наименование без уточнения.
//
//    * ПоискАктивного - Булево - по умолчанию Ложь. Запустить, если не выполняется, иначе вернуть активное.
//                         Ложь   - не искать активное задание, сразу запустить и вернуть запущенное.
//                         Истина - искать по имени метода и состоянию Активно, если найдено, вернуть первое.
//                     - Структура - как параметр Отбор у метода платформы ФоновыеЗадания.ПолучитьФоновыеЗадания.
//
//    * Ключ         - Неопределено - значение по умолчанию. Взять ключ из регламентного задания,
//                       если указано, в противном случае взять ключ из метаданных регламентного задания.
//                   - Строка - также как у метода платформы ФоновыеЗадания.Выполнить.
//
//    * Наименование - Неопределено - по умолчанию. Взять наименование из регламентного задания,
//                       если заполнено, в противном случае взять представление его метаданных.
//                   - Строка - также как у метода платформы ФоновыеЗадания.Выполнить,
//                       но с учетом значения свойства ЭтоЗапускВручную.
//
//    * СРасширениямиБазыДанных - Булево - по умолчанию Истина. Определяет способ выполнения.
//                                  Истина - РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных
//                                  Ложь   - РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеБезРасширений
//                              - Неопределено - ФоновыеЗадания.Выполнить
// 
Функция НовыеПараметрыВыполненияЗадания() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ВФоне",                   Истина);
	Результат.Вставить("ЭтоЗапускВручную",        Истина);
	Результат.Вставить("ПоискАктивного",          Ложь);
	Результат.Вставить("Ключ",                    Неопределено);
	Результат.Вставить("Наименование",            Неопределено);
	Результат.Вставить("СРасширениямиБазыДанных", Истина);
...
#Область ПроцедурыИФункцииБезПоддержкиЗаданийОчередиВМоделиСервиса
...
Процедура УстановитьИспользованиеРегламентныхЗаданийПоФункциональнымОпциям(ВключатьЗадания = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
		...
		МодульРегламентныеЗаданияСлужебный.УстановитьИспользованиеРегламентныхЗаданийПоФункциональнымОпциям(ВключатьЗадания);
	КонецЕсли;
...
Функция УточненныйСписокЗаданий(СписокЗаданий)
	...
	Для Каждого Задание Из КопияСписка Цикл
		Задание.УникальныйИдентификатор = Задание.Идентификатор;
	КонецЦикла;
...
// Параметры:
//  РегламентноеЗадание - ОбъектМетаданныхРегламентноеЗадание
//
// Возвращаемое значение:
//  см. Пользователи.СлужебныйПользовательИБ
//
Функция СлужебныйПользовательИБ(РегламентноеЗадание)
	...
	СсылкаНового = Справочники.Пользователи.ПолучитьСсылку(
		Новый УникальныйИдентификатор("91f28855-c379-11ef-8882-b06ebfbf08c7"));
	
	Попытка
		СлужебныйПользовательИБ = Пользователи.СлужебныйПользовательИБ(Свойства, СсылкаНового);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось создать служебного пользователя ""%1""
			           |для установки в регламентное задание ""%2"" по причине:'"),
			РегламентноеЗадание.Представление(),
	...
	Возврат СлужебныйПользовательИБ;
	
КонецФункции

Функция ИмяСлужебногоПользователя()
	
	Возврат "СлужебныйПользовательДляЗапускаРегламентныхЗаданий";
