...
// Устанавливает служебного пользователя без прав для запуска регламентного задания.
// 
// Параметры:
//  РегламентноеЗадание - ОбъектМетаданныхРегламентноеЗадание - метаданные регламентного задания.
//
Процедура УстановитьСлужебногоПользователяРегламентногоЗадания(Знач РегламентноеЗадание = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяПользователя()) И ИмяПользователя() <> ИмяСлужебногоПользователя() Тогда
		Возврат;
	КонецЕсли;
	
	СлужебныйПользователь = СлужебныйПользователь(ИмяСлужебногоПользователя());
	
	Если СлужебныйПользователь <> Неопределено Тогда
		ПараметрыЗадания = Новый Структура;
		ПараметрыЗадания.Вставить("ИмяПользователя", СлужебныйПользователь.Имя);
		УстановитьПараметрыРегламентногоЗадания(РегламентноеЗадание, ПараметрыЗадания);
...
Функция СлужебныйПользователь(ИмяПользователя)
	
	Попытка 
		СлужебныйПользователь = ПользователиСлужебный.СлужебныйПользовательИБ(ИмяПользователя);
	Исключение
		СлужебныйПользователь = Неопределено;
		ИмяСобытия = НСтр("ru = 'Регламентные задания.Запись служебного пользователя'", ОбщегоНазначения.КодОсновногоЯзыка());
