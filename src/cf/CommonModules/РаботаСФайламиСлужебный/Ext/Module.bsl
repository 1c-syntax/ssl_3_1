...
Функция ТипХраненияФайла(Знач РазмерФайла, Знач РасширениеФайла) Экспорт
	...
	Если РаботаСФайламиВТомахСлужебный.ХранитьФайлыВТомахНаДиске()
		И РаботаСФайламиВТомахСлужебный.ЕстьТомаХраненияФайлов() Тогда
		Возврат РаботаСФайламиВТомахСлужебный.ТипХраненияФайла(РасширениеФайла, РазмерФайла);
	Иначе
		Возврат Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе;
	КонецЕсли;
	...
	Если ПоискВТоме Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось открыть файл:
				...
				|%2
				|
				|Данные файла были удалены. Возможно, файл очищен как ненужный или удален антивирусной программой.'"),
			ИмяФайла,
			ВладелецФайла);
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось открыть файл:
				...
				|%2
				|
				|Данные файла были удалены. Возможно, файл очищен как ненужный.'"),
			ИмяФайла,
			ВладелецФайла);
	КонецЕсли;
