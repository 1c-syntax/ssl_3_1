...
Процедура ЗаписатьИнформациюОШифровании(ФайлСсылка, Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор, 
	ИмяРабочегоКаталога, МассивФайловВРабочемКаталогеДляУдаления, МассивОтпечатков, СведенияОФайле = Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого ДанныеДляЗаписиНаСервере Из МассивДанныхДляЗанесенияВБазу Цикл
			...
			Если ЗначениеЗаполнено(ИмяРабочегоКаталога) Тогда
			...
			Если СведенияОФайле = Неопределено Тогда
				...
				СведенияОФайле.ИмяБезРасширения = ВерсияСсылка.Наименование;
...
Функция ИменаСправочниковХраненияВерсийФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь)
	...
	Ошибки = Новый Массив;
	Ошибки.Добавить(НСтр("ru = 'Ошибка при определении имен справочников для хранения версий файлов.'"));
	ОсновнойСправочникУказан = Ложь;
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ВерсииПрисоединенныхФайлов") И Не КлючИЗначение.Ключ ="ВерсииФайлов" Тогда
			...
			Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У владельца файлов типа ""%1""
					|указано имя справочника ""%2""
					|без обязательного постфикса ""%3"".'"),
				Строка(ТипВладельцаФайлов), Строка(КлючИЗначение.Ключ), "ВерсииПрисоединенныхФайлов"));
...
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, НастраиватьОбъектФормы = Ложь) Экспорт
	...
	Контекст.ДатаМодификации = МестноеВремя(Контекст.ЭтотОбъект.Объект.ДатаМодификацииУниверсальная);
...
Функция ПодготовитьФайлКУдалению(ПрисоединенныйФайл)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписатьВЖурналСобытийОчисткиФайлов(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
// Параметры:
//  ОписаниеПрисоединенногоФайла - СтрокаДереваЗначения из см. ВыбратьНенужныеФайлы
//
...
// Параметры:
//  НастройкаОчистки - СтрокаТаблицыЗначений: см. РегистрыСведений.НастройкиОчисткиФайлов.ТекущиеНастройкиОчистки
...
Функция ПолучитьUID1C(АдресФайлаHRef, ПараметрыСинхронизации)
	...
	НайденныеEtag = ВычислитьXPath("//*[local-name()='propstat'][contains(./*[local-name()='status'],'200 OK')]/*[local-name()='prop']/*[local-name()='UID1C']",
...
Функция ПолучитьEtag(АдресФайлаHRef, ПараметрыСинхронизации)
	...
	КонтекстXml = ОпределитьКонтекстXML(ПараметрыСинхронизации.Ответ.ПолучитьТелоКакСтроку());
	НайденныеEtag = ВычислитьXPath("//*[local-name()='propstat'][contains(./*[local-name()='status'],'200 OK')]/*[local-name()='prop']/*[local-name()='getetag']",
		КонтекстXml).ПолучитьСледующий();
	Если НайденныеEtag <> Неопределено Тогда
		Возврат НайденныеEtag.ТекстовоеСодержимое;
	КонецЕсли;
	...
	Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
		...
		Иначе
			...
			Если ПараметрыФайла.Зашифрован <> Зашифрован Тогда
				...
				ЗаписатьИнформациюОШифровании(СсылкаСуществующегоФайла, Зашифрован, МассивДанныхДляЗанесенияВБазу, Неопределено,
					"", МассивФайловВРабочемКаталогеДляУдаления, МассивОтпечатков, СведенияОФайле);
			Иначе
...
Процедура ПрочитатьПараметрыКаталога(РезультатПроверки, HttpАдрес, СтруктураОбмена)
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			НайденныйPropstat = ВычислитьXPath("./*[local-name()='propstat'][contains(./*[local-name()='status'],'200 OK')]/*[local-name()='prop']", 
...
Процедура СкачатьДеревоФайловРекурсивно(ТекущиеСтрокиДереваФайлов, HttpАдрес, ПараметрыСинхронизации, Отказ=Ложь)
	...
	Исключение
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 
...
Процедура ЗагрузитьПодписиФайла(ФайлСсылка, Подписи, ПараметрыСинхронизации)
	...
	Для Каждого ПараметрыФайла Из Подписи Цикл 
		...
		Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
			...
			ОписаниеОшибки = "";
			ДанныеПодписи = МодульЭлектроннаяПодпись.ПрочитатьДанныеПодписи(Подпись, ОписаниеОшибки);
			
			Если Не ПустаяСтрока(ОписаниеОшибки) И ПустаяСтрока(ОшибкаНеУдалосьПрочитатьДанныеПодписи) Тогда
				...
				ОшибкаНеУдалосьПрочитатьДанныеПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ТекстСобытия, ИмяФайла, ОписаниеОшибки);
			КонецЕсли;
...
Функция СоздатьФайлВОблачномСервисе(Знач АдресСервера, Знач ПараметрыСинхронизации, Знач СтрокаТаблицы, Знач ТаблицаФайлов)
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") И СтрокаТаблицы.ПодписанЭП Тогда
			...
			Для Каждого Подпись Из ЭлектронныеПодписи Цикл
				...
				ToEtagПодписи = ВызватьМетодPUT(ToHrefПодписи, Подпись.Подпись, ПараметрыСинхронизации, Истина);
	...
	ИначеЕсли Не СтрокаТаблицы.ЭтоПапка И Не ОбъектЯвляетсяГруппой Тогда
		...
		КонецЕсли;
		РаботаСФайламиСлужебныйВызовСервера.ЗанятьФайл(ДанныеФайла, , , ПараметрыСинхронизации.АвторФайлов, ДополнительныеСвойства);
	КонецЕсли;
			...
			Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
				...
				Исключение
					...
					ТекстСобытия = НСтр("ru = 'Объект не был удален в облачном сервисе %1 по причине:
						|%2'");
					ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ТекстСобытия, СтрокаТаблицы.НаименованиеСервер, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())),
						ПараметрыСинхронизации.УчетнаяЗапись, УровеньЖурналаРегистрации.Ошибка);
				КонецПопытки;
