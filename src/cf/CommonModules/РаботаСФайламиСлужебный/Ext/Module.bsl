...
//  СведенияОФайлеДляЗаписи - см. РаботаСФайламиКлиентСервер.СведенияОФайле, Неопределено - по умолчанию Неопределено,
//                  структура используется только при синхронизации с облаком.
//
Процедура ЗаписатьИнформациюОШифровании(ФайлСсылка, Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор, 
	ИмяРабочегоКаталога, МассивФайловВРабочемКаталогеДляУдаления, МассивОтпечатков, СведенияОФайлеДляЗаписи = Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого ДанныеДляЗаписиНаСервере Из МассивДанныхДляЗанесенияВБазу Цикл
			...
			Если СведенияОФайлеДляЗаписи = Неопределено Тогда
			...
			Иначе
				СведенияОФайле = СведенияОФайлеДляЗаписи;
			КонецЕсли;
