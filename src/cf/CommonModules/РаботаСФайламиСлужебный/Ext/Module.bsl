...
Процедура ЗаполнитьДополнительныеДанныеФайла(Результат, ПрисоединенныйФайл, ВерсияФайла = Неопределено) Экспорт
	...
	СправочникПоддерживаетВозможностьХранитьВерсии = ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия", 
		Метаданные.НайтиПоТипу(ТипЗнч(ПрисоединенныйФайл)));
...
Функция ДвоичныеДанныеФайла(Знач ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина, ПолучениеФайлаИзАрхива = Ложь) Экспорт
	...
	Если ФайлОбъект.ПометкаУдаления Тогда
		СообщитьОбОшибкеФайлНеНайден(ФайлОбъект, ВызыватьИсключение);
		Возврат Неопределено;
	КонецЕсли;
	
	Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ФайлОбъект.ТипХраненияФайла) Тогда

		Результат = РаботаСФайлами.ХранилищеФайлаИзИнформационнойБазы(ФайлОбъект.Ссылка, ПолучениеФайлаИзАрхива);
	...
	Иначе
		Возврат РаботаСФайламиВТомахСлужебный.ДанныеФайла(ПрисоединенныйФайл, ВызыватьИсключение, ПолучениеФайлаИзАрхива);
	КонецЕсли;
...
Функция ФайлОбъект(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВладелецФайла", МетаданныеОбъектаФайла) Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия", МетаданныеОбъектаФайла) Тогда
			ТекущаяВерсия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрисоединенныйФайл, "ТекущаяВерсия"); 
			ЕстьТекущаяВерсия = ЗначениеЗаполнено(ТекущаяВерсия) И ОбщегоНазначения.СсылкаСуществует(ТекущаяВерсия); 
		КонецЕсли;	
