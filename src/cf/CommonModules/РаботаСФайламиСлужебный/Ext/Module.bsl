...
Процедура ПометитьНаУдалениеВерсииФайла(Знач ФайлСсылка, Знач ВерсияИсключение) Экспорт
	...
	ВыборкаВерсий = ВерсииФайла(ФайлСсылка, ВерсияИсключение, ПолноеИмяСправочникаВерсий);
...
//
// Параметры:
//   ВозможныеПрава - см. УправлениеДоступомПереопределяемый.ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов.ВозможныеПрава
//
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.ИзменяемыеОбъекты    = "РегистрСведений.СведенияОФайлах";
...
Процедура ВыполнитьОчисткуДанныхОВерсии(ФайлСсылка)
	...
	Попытка
		...
		Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			РаботаСФайламиВТомахСлужебный.УдалитьДанныеФайла(ФайлОбъект, ТекстКомментария);
		Иначе
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка)
	...
	Попытка
		...
		ФайлОбъект = ФайлСсылка.ПолучитьОбъект(); // ОпределяемыйТип.ПрисоединенныйФайлОбъект
		
		ТекстОписания = НСтр("ru = 'Файл удален при очистке ненужных файлов.'")
			+ " " + Формат(ТекущаяДатаСеанса(),"ДЛФ=D") + Символы.ПС;
		
		Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			РаботаСФайламиВТомахСлужебный.УдалитьДанныеФайла(ФайлОбъект, ТекстОписания);
		Иначе
			...
			Если ЭлементДанных.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
				...
				Иначе
					...
					НовыйПутьФайла = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(
						...
						ЭлементДанных.Ссылка.УникальныйИдентификатор());
...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента, Отправитель = Неопределено)
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если Не ЭлементДанных.ЭтоНовый()
			И ЭлементДанных.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			
			СвойстваФайла = РаботаСФайламиВТомахСлужебный.СвойстваФайлаВТоме();
			ЗаполнитьЗначенияСвойств(СвойстваФайла, ЭлементДанных);
...
//
Функция ВерсииФайла(ФайлСсылка, ВерсияИсключение, ПолноеИмяСправочникаВерсий)
...
//
// Параметры:
//  ПрисоединенныйФайл - ОпределяемыйТип.ПрисоединенныйФайл, ОпределяемыйТип.ПрисоединенныйФайлОбъект - ссылка или 
...
//  ИдентификаторФормы - УникальныйИдентификатор, Неопределено - уникальный идентификатор формы.
//
// Возвращаемое значение:
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Если Не ЭтоФайлВБазе Тогда
		РаботаСФайламиВТомахСлужебный.ОбновитьФайл(ФайлОбъект, ДвоичныеДанные);
	КонецЕсли;
...
Функция СоздатьВерсию(ФайлСсылка, СведенияОФайле) Экспорт
	...
	Если СведенияОФайле.СсылкаНаВерсиюИсточник <> Неопределено Тогда // создание Файла из шаблона
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске
			...
			Если Не СведенияОФайле.СсылкаНаВерсиюИсточник.Том.Пустая() Тогда
				ДвоичныеДанные = РаботаСФайламиВТомахСлужебный.ДанныеФайла(СведенияОФайле.СсылкаНаВерсиюИсточник);
			КонецЕсли;
	...
	Иначе // Создание объекта Файл на основе выбранного файла с диска.
		...
		Если ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			РаботаСФайламиВТомахСлужебный.ДобавитьФайл(Версия, ДвоичныеДанные);
		КонецЕсли;
...
// Параметры:
//   НастройкаОчистки - СтрокаТаблицыЗначений - колонки:
//     * ВладелецФайла - ОпределяемыйТип.ПрисоединенныйФайл - .
...
Процедура ПередУдалениемПрисоединенногоФайлаСервер(Знач Ссылка,
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске
		И Не Том.Пустая() Тогда
		
		СвойстваФайла = РаботаСФайламиВТомахСлужебный.СвойстваФайлаВТоме(Ссылка);
		РаботаСФайламиВТомахСлужебный.УдалитьФайл(
			РаботаСФайламиВТомахСлужебный.ПолноеИмяФайлаВТоме(СвойстваФайла));
	КонецЕсли;
