...
Процедура ПриДобавленииТиповИсключаемыхИзВозможныхДублей(ИсключаемыеТипы) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеТипы, 
		Метаданные.ОпределяемыеТипы.ПрисоединенныйФайл.Тип.Типы());
	ИсключаемыеТипы.Добавить(Тип("СправочникСсылка.ХранилищеДвоичныхДанных"));

КонецПроцедуры

Функция СправочникВерсийФайлов() Экспорт
	Возврат Тип("СправочникСсылка.ВерсииФайлов"); 
КонецФункции

// Определяет, является ли переданный объект справочником Файлы. 
// Возвращает Истину для объектов типа: метаданные, ссылка или объект.
// 
// Параметры:
//  ОбъектПроверки - ОбъектМетаданных
//                 - СправочникСсылка
//                 - СправочникОбъект
// 
// Возвращаемое значение:
//  Булево - если Истина, то это справочник файлы
// 
Функция ЭтоСправочникФайлы(ОбъектПроверки) Экспорт
	
	Если ТипЗнч(ОбъектПроверки) = Тип("ОбъектМетаданных") И ОбъектПроверки = Метаданные.Справочники.Файлы Тогда
		Возврат Истина;
	ИначеЕсли ТипЗнч(ОбъектПроверки) = Тип("СправочникСсылка.Файлы") Тогда
		Возврат Истина;
	ИначеЕсли ТипЗнч(ОбъектПроверки) = Тип("СправочникОбъект.Файлы") Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗагрузитьФайл(Знач ОбъектФайла, Знач ПутьКФайлу) Экспорт
	...
	РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(ОбъектФайла, ДвоичныеДанные, ОбъектФайла.Размер, ОбъектФайла.Расширение);

	Если ОбъектФайла.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		РаботаСФайламиВТомахСлужебный.ДобавитьФайл(ОбъектФайла, ДвоичныеДанные, , Истина, ОбъектФайла.Том);
	Иначе
...
Процедура ЗаполнитьДополнительныеДанныеФайла(Результат, ПрисоединенныйФайл, ВерсияФайла = Неопределено) Экспорт
	...
	МетаданныеОбъектаФайла = ПрисоединенныйФайл.Метаданные(); 
	...
	ИначеЕсли ЕстьВозможностьХранитьВерсии И ЗначениеЗаполнено(ПрисоединенныйФайл.ТекущаяВерсия) Тогда
	...
	Результат.Вставить("НавигационнаяСсылкаТекущейВерсии", РегистрыСведений.ХранилищеФайлов.НавигационнаяСсылкаФайла(ТекущаяВерсияФайла));
	...
	Если ЗначениеЗаполнено(ТекущаяВерсияФайла) Тогда
		ПолноеИмяФайлаВРабочемКаталоге = РаботаСФайламиСлужебныйВызовСервера.ПолноеИмяФайлаВРабочемКаталоге(
			ТекущаяВерсияФайла, ИмяКаталога, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
...
Процедура ДобавитьНастройкиРаботыСФайлами(ОбщиеНастройки, ПерсональныеНастройки) Экспорт
	...
	ПерсональныеНастройки.Вставить("ПоказыватьКолонкуРазмер", ПоказыватьКолонкуРазмер());
...
Функция ПолучитьВсеПодчиненныеФайлы(Знач ВладелецФайла, Знач ТаблицаХраненияФайлов = Неопределено) Экспорт
	...
	Если ТаблицаХраненияФайлов = Неопределено Тогда
		ТаблицаХраненияФайлов = Метаданные.Справочники.Файлы.ПолноеИмя();
	КонецЕсли;
	...
	|ИЗ
	|	&ТаблицаХраненияФайлов КАК Файлы
	|ГДЕ
	|	Файлы.ВладелецФайла = &ВладелецФайла";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаХраненияФайлов", ТаблицаХраненияФайлов);
...
Функция МенеджерФайловПоТипу(ТипХраненияФайла) Экспорт
	...
	Иначе
		Возврат РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный;
	КонецЕсли;
КонецФункции
...
//
Функция КонтекстОбновленияФайла(ПрисоединенныйФайл, ДанныеФайла, СсылкаНаФайл = Неопределено, 
	ТипХраненияФайла = Неопределено) Экспорт
	...
	Если ТипЗнч(Контекст.ПрисоединенныйФайл) = СправочникВерсийФайлов() Тогда
...
Процедура ПриОбновленииДанныхФайла(Контекст, ПрисоединенныйФайл) Экспорт
	ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(ПрисоединенныйФайл);
	ЗаписатьФайлВИнформационнуюБазу(ПрисоединенныйФайл, Контекст.ДанныеФайла, ПараметрыЗаписиВИБ);
КонецПроцедуры
...
//  Булево
//
Функция ФайлНаходитсяВФайловомАрхиве(ПрисоединенныйФайл, ВерсияСсылка = Неопределено) Экспорт

	ПроверяемыйФайл = ПрисоединенныйФайл;

	Если Не ЗначениеЗаполнено(ВерсияСсылка) И ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия",
		Метаданные.НайтиПоТипу(ТипЗнч(ПрисоединенныйФайл))) Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяВерсия) Тогда
			ПроверяемыйФайл = ТекущаяВерсия;
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(ВерсияСсылка) Тогда
		ПроверяемыйФайл = ВерсияСсылка;
	КонецЕсли;

	Результат = РаботаСФайловымАрхивомСервер.ФайлПеренесенВФайловыйАрхив(ПроверяемыйФайл);
...
//
//  ПолучениеФайлаИзАрхива - Булево - получение данных файла из архивного хранилища - дискового тома или тома внешнего
...
//	ДанныеФайла = РаботаСФайламиСлужебный.ДвоичныеДанныеФайла(Файл, Ложь);
//	Если ДанныеФайла <> Неопределено Тогда
//		ДанныеФайла.Записать(ПутьКФайлу);
//	КонецЕсли;
//
Функция ДвоичныеДанныеФайла(Знач ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина, ПолучениеФайлаИзАрхива = Ложь) Экспорт
	
	ИмяФункции = "РаботаСФайлами.ДвоичныеДанныеФайла";
	ОписаниеТипа = Метаданные.ОпределяемыеТипы.ПрисоединенныйФайл.Тип;
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяФункции, "ПрисоединенныйФайл", ПрисоединенныйФайл, ОписаниеТипа);

	Результат = Неопределено;

	ФайлОбъект = ФайлОбъект(ПрисоединенныйФайл);
	Если ФайлОбъект = Неопределено Тогда
		Если ВызыватьИсключение Тогда
			ШаблонПроверки = НСтр("ru = 'Недопустимое значение параметра %1'");
			СообщениеПроверки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПроверки, "ПрисоединенныйФайл");
			ОбщегоНазначенияКлиентСервер.Проверить(Ложь, СообщениеПроверки, ИмяФункции);
		КонецЕсли;

	ИначеЕсли ФайлОбъект.ЭтоГруппа Тогда
		 Если ВызыватьИсключение Тогда
			ШаблонПроверки = НСтр("ru = 'Недопустимое значение параметра %1 (папка файлов ""%2"")'");
			ПредставлениеФайла = ОбщегоНазначения.ПредметСтрокой(ПрисоединенныйФайл);
			СообщениеПроверки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПроверки, "ПрисоединенныйФайл", ПредставлениеФайла);
			ОбщегоНазначенияКлиентСервер.Проверить(Ложь, СообщениеПроверки, ИмяФункции);
		 КонецЕсли;
	Иначе
		УстановитьОтключениеБезопасногоРежима(Истина);
		...
		Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ФайлОбъект.ТипХраненияФайла) Тогда
			ХранилищеФайлаИзИнформационнойБазы = РаботаСФайлами.ХранилищеФайлаИзИнформационнойБазы(ФайлОбъект.Ссылка, ПолучениеФайлаИзАрхива);
			Если ТипЗнч(ХранилищеФайлаИзИнформационнойБазы) = Тип("ХранилищеЗначения") Тогда
				ЗначениеХранилища = ХранилищеФайлаИзИнформационнойБазы.Получить();
				ТипЗначенияХранилища = ТипЗнч(ЗначениеХранилища);

				Если ТипЗначенияХранилища = Тип("ДвоичныеДанные") Тогда
					Результат = ЗначениеХранилища;

				ИначеЕсли ТипЗначенияХранилища = Тип("Картинка") Тогда
					Результат = ЗначениеХранилища.ПолучитьДвоичныеДанные();

				КонецЕсли;
			КонецЕсли;

			Если Результат = Неопределено Тогда
				СообщитьОбОшибкеФайлНеНайден(ФайлОбъект, ВызыватьИсключение, ПолучениеФайлаИзАрхива);
			КонецЕсли;

		Иначе
			Результат = РаботаСФайламиВТомахСлужебный.ДанныеФайла(ФайлОбъект, ВызыватьИсключение, ПолучениеФайлаИзАрхива);
...
// Метод длительной операции, выполняемой в функции РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом.
//	Получает данные для дальнейшего открытия. Использует метод РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия.
//
// Параметры:
//  ПараметрыДлительнойОперации - см. ПараметрыДлительнойОперации
//
// Возвращаемое значение:
//   Структура - см. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла
//
Функция ДанныеФайлаДляОткрытияАсинхронный(ПараметрыДлительнойОперации) Экспорт
	
	ПараметрыФункции = ПараметрыДлительнойОперации.ПараметрыМетода;
	
	Попытка
	
		ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(ПараметрыФункции.ПрисоединенныйФайл, 
																					ПараметрыФункции.ВерсияФайла, 
																					ПараметрыФункции.ИдентификаторФормы,
																					ПараметрыФункции.РабочийКаталогВладельца,
																					ПараметрыФункции.ПредыдущийАдресФайла,
																					ПараметрыФункции.ПараметрыПолученияФайла);
																					
	Исключение
		Если РаботаСФайловымАрхивомСервер.ИспользоватьФайловыйАрхив() Тогда

			ПараметрыФункции.ПараметрыПолученияФайла.ПроверятьНаличиеФайлаВАрхиве = Истина;
			...
			ДанныеФайла.Вставить("ОбновлятьСпискиФайловНаФормах", Истина);
...
// Метод длительной операции, выполняемой в функции РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом.
//  Получает данные для открытия и сохранения файла. Использует метод РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайла.
//
// Параметры:
//  ПараметрыДлительнойОперации - см. ПараметрыДлительнойОперации
//
...
//   Структура - см. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла
//
Функция ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный(ПараметрыДлительнойОперации) Экспорт
	
	ПараметрыФункции = ПараметрыДлительнойОперации.ПараметрыМетода;
	
	ДополнительныеПараметры = РаботаСФайламиКлиентСервер.ПараметрыДанныхФайла();
	ЗаполнитьЗначенияСвойств(ДополнительныеПараметры, ПараметрыФункции);
	
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайла(ПараметрыФункции.ПрисоединенныйФайл, ДополнительныеПараметры);
...
// Метод длительной операции, выполняемой в функции РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом.
//  Получает данные для сохранения файла на диск. Использует метод РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляСохранения.
//
// Параметры:
//  ПараметрыДлительнойОперации - см. ПараметрыДлительнойОперации
//
...
//   Структура - см. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла
//
Функция ДанныеФайлаДляСохраненияАсинхронный(ПараметрыДлительнойОперации) Экспорт
	
	ПараметрыФункции = ПараметрыДлительнойОперации.ПараметрыМетода;

	Попытка

		ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляСохранения(ПараметрыФункции.ПрисоединенныйФайл, ПараметрыФункции);
...
// Список метаданных справочников присоединенных файлов.
//
// Возвращаемое значение:
...
Функция СправочникиФайлов() Экспорт
	...
	Для Каждого Тип Из ТипыФайлов Цикл
		...
		Результат.Добавить(МетаданныеСправочникаФайлов);
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
Функция ОбъектыХраненияФайловИнформационнойБазе() Экспорт
	...
	Результат = Новый Массив();
	Результат.Добавить(Метаданные.РегистрыСведений.ХранилищеФайлов);
	Результат.Добавить(Метаданные.Справочники.ХранилищеДвоичныхДанных);
	Возврат Результат;
...
Функция ИменаСправочниковХраненияФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь) Экспорт
	...
	Ошибки = Новый Массив;
	Ошибки.Добавить(НСтр("ru = 'Невозможно определить имена справочников для хранения файлов.'"));
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение(СтрСоединить(Ошибки, Символы.ПС + Символы.ПС),  КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
Процедура СкопироватьПрисоединенныеФайлы(Знач Источник, Знач Получатель) Экспорт
	...
	Для Каждого КопируемыйФайл Из КопируемыеФайлы Цикл
		...
		Попытка
			...
			РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(КопияФайла, ДвоичныеДанные, КопияФайла.Размер, КопияФайла.Расширение);

			Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(КопияФайла.ТипХраненияФайла) Тогда
				ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(КопияФайла);
				ЗаписатьФайлВИнформационнуюБазу(КопияФайлаСсылка, ДвоичныеДанные, ПараметрыЗаписиВИБ);
			Иначе
				РаботаСФайламиВТомахСлужебный.ДобавитьФайл(КопияФайла, ДвоичныеДанные,,,КопияФайла.Том);
			КонецЕсли;

			КопияФайла.Записать();

			Если ДоступнаЭлектроннаяПодпись Тогда
				ПодписиОбъекта = МодульЭлектроннаяПодпись.ПодписиОбъекта(КопируемыйФайл);
				МодульЭлектроннаяПодпись.ДобавитьПодпись(КопияФайла.Ссылка, ПодписиОбъекта);				
...
Функция ФайлОбъект(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВладелецФайла", МетаданныеОбъектаФайла) Тогда
		...
		КонецЕсли;
		ЕстьРеквизитТекущаяВерсия = ОбщегоНазначения.ЕстьРеквизитОбъекта("ТекущаяВерсия", МетаданныеОбъектаФайла);
		Если ЕстьРеквизитТекущаяВерсия Тогда
			ИменаРеквизитов = ИменаРеквизитов + "," + "ТекущаяВерсия";
		КонецЕсли;
		РеквизитыПрисоединенного = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайл, ИменаРеквизитов);

		ИспользоватьРеквизитыВерсии = ЕстьРеквизитТекущаяВерсия
			И ЗначениеЗаполнено(РеквизитыПрисоединенного.ТекущаяВерсия)
			И ОбщегоНазначения.СсылкаСуществует(РеквизитыПрисоединенного.ТекущаяВерсия);

		Если ИспользоватьРеквизитыВерсии Тогда
			ИменаРеквизитов = "Ссылка,ТипХраненияФайла,Наименование,Расширение,Том,ПутьКФайлу,ПометкаУдаления,НомерВерсии";
			Если МетаданныеОбъектаФайла.Иерархический Тогда
				ИменаРеквизитов = ИменаРеквизитов + "," + "ЭтоГруппа";
			КонецЕсли;
			Результат = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыПрисоединенного.ТекущаяВерсия, ИменаРеквизитов);
			Результат.Вставить("ВладелецФайла", РеквизитыПрисоединенного.ВладелецФайла);

		Иначе
			РеквизитыПрисоединенного.Удалить("ТекущаяВерсия");

			Результат = РеквизитыПрисоединенного;
		КонецЕсли;

	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("РодительскаяВерсия", МетаданныеОбъектаФайла) Тогда
		Результат = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайл,
			"Ссылка,ТипХраненияФайла,Наименование,Расширение,Том,ПутьКФайлу,Владелец,ПометкаУдаления");

		ВладелецВерсии = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Результат.Владелец, "ВладелецФайла");
		Результат.Вставить("ВладелецФайла", ВладелецВерсии);
		Результат.Удалить("Владелец");
	КонецЕсли;
	Если Не МетаданныеОбъектаФайла.Иерархический Тогда
		Результат.Вставить("ЭтоГруппа", Ложь);
	КонецЕсли;
	НомерВерсии = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "НомерВерсии");
	...
	Для Каждого Справочник Из МетаданныеСправочники Цикл
		Если Справочник.Реквизиты.Найти("ВладелецФайла") <> Неопределено Тогда
			...
			Если ЕстьВозможностьХранитьВерсии Тогда
				ТипРеквизита = Справочник.Реквизиты.ТекущаяВерсия.Тип.Типы()[0];
				ПолноеИмяРеквизита = Метаданные.НайтиПоТипу(ТипРеквизита).ПолноеИмя();
				ПолноеИмяСправочникаВерсийФайлов = ПолноеИмяРеквизита;
...
Функция ТипХраненияФайлов() Экспорт
	...
	Если РаботаСФайламиВТомахСлужебный.ХранитьФайлыВТомах()
...
//  РазмерФайла - Число
//  РасширениеФайла - Строка 
//  ДоступныХранилищаДвоичныхДанных - Булево, Неопределено.
//  СпособХраненияФайлов - Строка, Неопределено - см. значения для "Константа.СпособХраненияФайлов". 
//						   Значение по умолчанию - Неопределено.
//
// Возвращаемое значение:
//   ПеречислениеСсылка.ТипыХраненияФайлов
//
Функция ТипХраненияФайла(Знач РазмерФайла, Знач РасширениеФайла, ДоступныХранилищаДвоичныхДанных = Неопределено, СпособХраненияФайлов = Неопределено) Экспорт

	Если ДоступныХранилищаДвоичныхДанных = Неопределено Тогда
	...
	Если ДоступныХранилищаДвоичныхДанных Тогда
		...
		Если СпособХраненияФайлов = Неопределено Тогда
			СпособХраненияФайлов = РаботаСФайловымАрхивомСервер.СпособХраненияФайлов();
		КонецЕсли;

		Если РаботаСФайловымАрхивомКлиентСервер.СпособХраненияИспользуетТома(СпособХраненияФайлов)
			И РаботаСФайламиВТомахСлужебный.ЕстьТомаХраненияФайлов(, РаботаСФайловымАрхивомСервер.СпособХраненияДляПодбораТомовХранения(СпособХраненияФайлов)) Тогда
			Возврат РаботаСФайламиВТомахСлужебный.ТипХраненияФайла(РазмерФайла, РасширениеФайла, СпособХраненияФайлов);
		Иначе
			Возврат Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе;
		КонецЕсли;
	Иначе
...
//
// Параметры:
//  ТипХраненияФайла	- ПеречислениеСсылка.ТипыХраненияФайлов, Неопределено. 
//						  Значение по умолчанию - Неопределено.
//
// Возвращаемое значение:
//  Булево
//
Функция ЕстьФайлыВТомах(Знач ТипХраненияФайла = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ТипХраненияФайла", ТипХраненияФайла);
...
Функция ЭтоСправочникФайловИлиВерсийФайлов(ПолноеИмя) Экспорт
	...
	Если СтрЗаканчиваетсяНа(ВРег(ЧастиИмени[1]), ВРег(СуффиксСправочникаПрисоединенныеФайлы()))
...
Процедура КриптографияПриСозданииФормыНаСервере(Форма, ЭтоФормаСписка = Истина, ТолькоКартинкаСтрок = Ложь) Экспорт
	...
	Подписание = Ложь;
	ОтправкаНаПодписание = Ложь;
	Шифрование = Ложь;
	ПросмотрЗашифрованных = Ложь;
	ЗаголовокОтправкиНаПодписание = "";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Если МодульЭлектроннаяПодписьСлужебный.ИнтерактивноеИспользованиеЭлектронныхПодписейИШифрования() Тогда
			МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
			МодульЭлектроннаяПодписьЛокализация = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодписьЛокализация");
			...
			ОтправкаНаПодписание  = МодульЭлектроннаяПодпись.ИспользоватьСервисМобильнойПодписи();
			МодульЭлектроннаяПодписьЛокализация.ПриПолученииЗаголовкаКомандыОтправитьНаПодписание(ЗаголовокОтправкиНаПодписание);
		КонецЕсли;
	...
	Если ЭтоФормаСписка Тогда
		Если ОбщегоНазначения.ЭтоСправочник(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Форма.Список.ОсновнаяТаблица)) Тогда
			...
			ДоступнаЭлектроннаяПодпись = ДоступнаЭлектроннаяПодпись(ТипЗнч(ТаблицаФайлов.ПустаяСсылка()));
		Иначе
			ДоступнаЭлектроннаяПодпись = Истина;
		КонецЕсли;
	Иначе
		ДоступнаЭлектроннаяПодпись = ДоступнаЭлектроннаяПодпись(ТипЗнч(Форма.Объект.Ссылка));
	КонецЕсли;
	
	Если Не ТолькоКартинкаСтрок Тогда
		
		Элементы.ФормаГруппаКомандЭлектроннаяПодписьИШифрование.Видимость = ДоступнаЭлектроннаяПодпись;
		
		Элементы.ФормаПодписать.Видимость = Подписание;
		УстановитьВидимостьКомандыОтправкаНаПодписание(Элементы, "ФормаОтправитьНаПодписание", ОтправкаНаПодписание, ЗаголовокОтправкиНаПодписание);
		...
		Если ЭтоФормаСписка Тогда
			...
			УстановитьВидимостьКомандыОтправкаНаПодписание(Элементы, "СписокКонтекстноеМенюОтправитьНаПодписание", ОтправкаНаПодписание, ЗаголовокОтправкиНаПодписание);
			...
			Элементы.СписокКонтекстноеМенюГруппаКомандЭлектроннаяПодписьИШифрование.Видимость = ДоступнаЭлектроннаяПодпись;
			
		Иначе
			Элементы.ЭлектронныеПодписиПродлитьДействиеПодписей.Видимость = Подписание;
			Элементы.ЭлектронныеПодписи.ИзменятьСоставСтрок = Подписание;
			УстановитьВидимостьКомандыОтправкаНаПодписание(Элементы, "ЭлектронныеПодписиОтправитьНаПодписание", ОтправкаНаПодписание, ЗаголовокОтправкиНаПодписание);
			Элементы.ЭлектронныеПодписиПодписать.Видимость = Подписание;
			Элементы.ЭлектронныеПодписиУдалить.Видимость = Подписание;
			Элементы.ГруппаЭлектронныеПодписи.Видимость = ДоступнаЭлектроннаяПодпись;
			Элементы.ГруппаСертификатыШифрования.Видимость = ПросмотрЗашифрованных И ДоступнаЭлектроннаяПодпись;
			Элементы.ФормаСохранитьВместеСПодписью.Видимость = Форма.ЭлектронныеПодписи.Количество() <> 0;
		КонецЕсли;
	КонецЕсли;
	
	Если ЭтоФормаСписка Тогда
		Элементы.СписокНомерКартинкиПодписанЗашифрован.Видимость = ДоступнаЭлектроннаяПодпись;
	КонецЕсли;

	Если Не ДоступнаЭлектроннаяПодпись Тогда
		Возврат;
	КонецЕсли;
...
//
Процедура ПеренестиРезультатыПроверкиПодписей(ПодписиВФормеФайла, ПодписанныйФайл) Экспорт
	...
	ПодписиВОбъекте = МодульЭлектроннаяПодпись.ПодписиОбъекта(ПодписанныйФайл, ПараметрыПолученияПодписей);
	
	ПодписиВФорме = Новый Массив;
	Для Каждого ТекущаяПодпись Из ПодписиВФормеФайла Цикл
		Если Не ЗначениеЗаполнено(ТекущаяПодпись.АдресПодписи) Тогда
			Продолжить;
		КонецЕсли;
		ПодписиВФорме.Добавить(ТекущаяПодпись);
	КонецЦикла;
	...
	Свойства.Вставить("РезультатПроверки", Null);
	...
	КонецЕсли;
	Если Свойства.РезультатПроверки = Null Тогда
		ЕстьРезультатПроверки = Ложь;
		Свойства.Удалить("РезультатПроверки");
	Иначе
		ЕстьРезультатПроверки = Истина;
	КонецЕсли;
	
	Свойства.Вставить("Отпечаток");
	Свойства.Вставить("КомуВыданСертификат");
	
	Для Каждого Подпись Из ПодписиВФорме Цикл
		
		СтрокаПодписи = Неопределено; ИндексПодписи = -1;
		Для Каждого ПодписьВОбъекте Из ПодписиВОбъекте Цикл
			ИндексПодписи = ИндексПодписи + 1;
			Если Подпись.Свойство("ИдентификаторПодписи") Тогда
				Если Подпись.ИдентификаторПодписи = ПодписьВОбъекте.ИдентификаторПодписи Тогда
					СтрокаПодписи = ПодписьВОбъекте;
					Прервать;
				Иначе
					Продолжить;
				КонецЕсли;
			ИначеЕсли Подпись.ПорядковыйНомер = ПодписьВОбъекте.ПорядковыйНомер Тогда
				Если Подпись.ДатаПодписи         <> ПодписьВОбъекте.ДатаПодписи
					Или Подпись.Комментарий         <> ПодписьВОбъекте.Комментарий
					Или Подпись.КомуВыданСертификат <> ПодписьВОбъекте.КомуВыданСертификат И ЗначениеЗаполнено(ПодписьВОбъекте.КомуВыданСертификат)
					Или Подпись.Отпечаток           <> ПодписьВОбъекте.Отпечаток И ЗначениеЗаполнено(ПодписьВОбъекте.Отпечаток)
					Или Подпись.УстановившийПодпись <> ПодписьВОбъекте.УстановившийПодпись Тогда
					Возврат; // Если объект был изменен, результаты проверки не переносятся.
				КонецЕсли;
				СтрокаПодписи = ПодписьВОбъекте;
				Прервать;
		...
		ЗаполнитьЗначенияСвойств(Свойства, ПодписьВОбъекте);
		ЕстьИзменения = Ложь;
		Для Каждого КлючИЗначение Из Свойства Цикл
			...
			Если КлючИЗначение.Ключ = "РезультатПроверкиПодписиПоМЧД" Тогда
				...
				Если ЗначениеЗаполнено(Подпись[КлючИЗначение.Ключ]) Тогда
					НовыйРезультат = ПолучитьИзВременногоХранилища(Подпись[КлючИЗначение.Ключ]);
				КонецЕсли;
			...
			Если Подпись[КлючИЗначение.Ключ] <> Свойства[КлючИЗначение.Ключ] Тогда
				ЕстьИзменения = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если Не ЕстьИзменения Тогда
			ПодписиВОбъекте.Удалить(ИндексПодписи);
			Продолжить; // Не нужно устанавливать модифицированность, если результаты проверки совпадают.
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(Свойства, Подпись);
		ЗаполнитьЗначенияСвойств(ПодписьВОбъекте, Свойства);
		Если ЕстьРезультатПроверки И ЗначениеЗаполнено(Подпись.РезультатПроверки) Тогда
			ЗаполнитьЗначенияСвойств(ПодписьВОбъекте, Подпись.РезультатПроверки);
		КонецЕсли;
		МодульЭлектроннаяПодпись.ОбновитьПодпись(ПодписанныйФайл, ПодписьВОбъекте);
		ПодписиВОбъекте.Удалить(ИндексПодписи);
	КонецЦикла;
...
Процедура ЗаписатьИнформациюОШифровании(ФайлСсылка, ПараметрыЗаписиИнформацииОШифровании) Экспорт
	...
	Попытка
		...
		Для Каждого ДанныеДляЗаписиНаСервере Из ПараметрыЗаписиИнформацииОШифровании.МассивДанныхДляЗанесенияВБазу Цикл
			...
			Если ТипЗнч(ДанныеДляЗаписиНаСервере.ВерсияСсылка) <> СправочникВерсийФайлов() Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			Если ПараметрыЗаписиИнформацииОШифровании.Зашифровать Тогда
				МодульЭлектроннаяПодписьСлужебный.ДобавитьСертификатыШифрования(ФайлСсылка, 
					ПараметрыЗаписиИнформацииОШифровании.МассивОтпечатков);
			Иначе
				МодульЭлектроннаяПодписьСлужебный.ОчиститьСертификатыШифрования(ФайлСсылка);
			КонецЕсли;
		КонецЕсли;
		
		ЗаполнитьИзвлеченныйТекст(ФайлОбъект, АдресВременногоХранилищаТекстаТекущейВерсии);
...
Процедура ЗаписатьРезультатИзвлеченияТекста(ФайлИлиВерсияСсылка, РезультатИзвлечения,
	...
	Попытка
		...
		ЗаполнитьИзвлеченныйТекст(ФайлИлиВерсияОбъект, АдресВременногоХранилищаТекста);
...
Процедура ОчиститьНенужныеФайлы(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	ЭтоЗапускВручную = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоЗапускВручную", Ложь);
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Зависимость = Настройки.Добавить();
	Зависимость.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ПереносФайловМеждуОперационнымХранилищемИФайловымАрхивом;
	Зависимость.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьФайловыйАрхив;
	Зависимость.РаботаетСВнешнимиРесурсами = Истина;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление универсальной даты, типа хранения файлов, а также удаление избыточные двоичных данных в справочнике Файлы.'");
	...
	Обработчик.ЧитаемыеОбъекты      = "Справочник.Файлы,Справочник.ВерсииФайлов";
	...
	ПриоритетВыполнения = Обработчик.ПриоритетыВыполнения.Добавить();
	ПриоритетВыполнения.Процедура = "РегистрыСведений.ХранилищеФайлов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	ПриоритетВыполнения.Порядок = "До";

	ПриоритетВыполнения = Обработчик.ПриоритетыВыполнения.Добавить();
	ПриоритетВыполнения.Процедура = "Справочники.Файлы.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	ПриоритетВыполнения.Порядок = "Любой";
		
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "РегистрыСведений.ХранилищеФайлов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "3.1.3.260";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("84e58943-94fe-4f92-99b3-91be534d3754");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "РегистрыСведений.ХранилищеФайлов.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	ЧитаемыеОбъекты.Добавить(Метаданные.Справочники.Файлы.ПолноеИмя());
	ЧитаемыеОбъекты.Добавить(Метаданные.РегистрыСведений.УдалитьДвоичныеДанныеФайлов.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(ЧитаемыеОбъекты, ",");
	
	ИзменяемыеОбъекты = Новый Массив;
	ИзменяемыеОбъекты.Добавить(Метаданные.Справочники.Файлы.ПолноеИмя());
	ИзменяемыеОбъекты.Добавить(Метаданные.РегистрыСведений.УдалитьДвоичныеДанныеФайлов.ПолноеИмя());
	ИзменяемыеОбъекты.Добавить(Метаданные.РегистрыСведений.ХранилищеФайлов.ПолноеИмя());
	ИзменяемыеОбъекты.Добавить(Метаданные.Справочники.ВерсииФайлов.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(ИзменяемыеОбъекты, ",");
	
	БлокируемыеОбъекты = Новый Массив;
	БлокируемыеОбъекты.Добавить(Метаданные.РегистрыСведений.УдалитьДвоичныеДанныеФайлов.ПолноеИмя());
	БлокируемыеОбъекты.Добавить(Метаданные.РегистрыСведений.ХранилищеФайлов.ПолноеИмя());
	БлокируемыеОбъекты.Добавить(Метаданные.Справочники.ВерсииФайлов.ПолноеИмя());
	БлокируемыеОбъекты.Добавить(Метаданные.Справочники.Файлы.ПолноеИмя());
	...
	ПриоритетВыполнения = Обработчик.ПриоритетыВыполнения.Добавить();
	ПриоритетВыполнения.Процедура = "Справочники.Файлы.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	ПриоритетВыполнения.Порядок = "После";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "РегистрыСведений.НаличиеФайлов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "3.1.10.301";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("fb2fba94-f4a1-408a-9274-d5c44e5a42a1");
	...
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Обновление признака наличия присоединенного файла без учета служебных файлов.'");
	Обработчик.ЧитаемыеОбъекты = ИменаСправочниковСоСлужебнымиФайлами();
	Обработчик.ИзменяемыеОбъекты = Метаданные.РегистрыСведений.НаличиеФайлов.ПолноеИмя();
    Обработчик.Многопоточный = Истина;
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.4.1.1", 
		"Роль.РаботаСПапкамиФайлов", "Роль.ДобавлениеИзменениеПапокИФайлов", "СтандартныеПодсистемы");
...
Функция ТекстЗапросаДляОчисткиФайлов(ВладелецФайла, Настройка, МассивИсключений, ЭлементИсключение)
	...
	Если ЕстьВозможностьХранитьВерсии И Настройка.Действие = Перечисления.ВариантыОчисткиФайлов.ОчиститьВерсии Тогда
		...
		|	И ВерсииФайлов.РодительскаяВерсия <> ЗНАЧЕНИЕ(Справочник.ВерсииФайлов.ПустаяСсылка)"; // @query-part-1, @query-part-2
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЭтоНеГруппа", 
		?(МетаданныеОбъектаФайлов.Иерархический, "НЕ Файлы.ЭтоГруппа", "ИСТИНА")); // @query-part-2, @query-part-3
...
Функция ДвоичныеДанныеИзСведенийОФайле(СведенияОФайле) Экспорт
	...
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректный тип хранилища файлов %1 в %2.'"),
				СведенияОФайле.АдресВременногоХранилищаФайла, 
				"РаботаСФайламиСлужебный.ДвоичныеДанныеИзСведенийОФайле"), 
...
Функция СведенияОбОчищаемыхФайлах() Экспорт
	...
	ЛишниеФайлы = РаботаСФайламиВТомахСлужебный.ЛишниеФайлыВТоме();
...
Процедура ПриЗаписиПрисоединенногоФайлаСервер(ВладелецФайлов, Источник) Экспорт
	...
	Если Не Источник.ЭтоГруппа Тогда
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Метаданные.РегистрыСведений.СведенияОФайлах.ПолноеИмя());
		ЭлементБлокировкиДанных.УстановитьЗначение("Файл", Источник.Ссылка);
	КонецЕсли;
	...
	Попытка
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Служебный", Источник.Метаданные())
		   И ТипЗнч(Источник.Служебный) = Тип("Булево") Тогда
			ЭтоСлужебныйФайл = Источник.Служебный;
		КонецЕсли;
		
		ЕстьФайлы = Не Источник.ПометкаУдаления И Не ЭтоСлужебныйФайл Или ЕстьФайлыУВладельца(ВладелецФайлов);
...
Процедура ПриОтправкеФайла(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза = Ложь, Получатель = Неопределено)
	...
	Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Удалить
		...
		Возврат; // Стандартную обработку не переопределяем.
	КонецЕсли;
...
Процедура ПриОтправкеФайлаСозданиеНачальногоОбраза(ЭлементДанных, ОтправкаЭлемента, Получатель);
	...
	Если ТипФайла = Тип("РегистрСведенийНаборЗаписей.ХранилищеФайлов")
		Или ТипФайла = Тип("СправочникОбъект.ХранилищеДвоичныхДанных") Тогда
...
Процедура ПриОтправкеФайлаСообщениеОбмена(ЭлементДанных, ОтправкаЭлемента, Получатель);
	...
	Если ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ХранилищеФайлов")
		Или ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.ХранилищеДвоичныхДанных") Тогда
...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента, Отправитель = Неопределено)
	...
	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.Файлы") Тогда
		...
		ОбработатьПолученныеФайлы = НЕ ЭлементДанных.ХранитьВерсии;
		Если Не ОбработатьПолученныеФайлы Тогда
	...
	Если Не ЭлементДанных.ЭтоНовый() Тогда
		ТипХраненияПредыдущейВерсииФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементДанных.Ссылка,
			"ТипХраненияФайла");
		...
		Иначе
			УстановитьПривилегированныйРежим(Истина);
			РегистрыСведений.ХранилищеФайлов.УдалитьДвоичныеДанные(ЭлементДанных.Ссылка);
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
	КонецЕсли;
	
	РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(ЭлементДанных, ДвоичныеДанные, ЭлементДанных.Размер, ЭлементДанных.Расширение);
	НовыйТипХраненияФайла = ЭлементДанных.ТипХраненияФайла;
	...
	Если НовыйТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		...
		Если ДвоичныеДанные = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось добавить файл ни в один из томов, т.к. он не существует.
					...
					|%1.'"),
					ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ЭлементДанных.Наименование, 
		...
		Иначе
			РаботаСФайламиВТомахСлужебный.ДобавитьФайл(ЭлементДанных, ДвоичныеДанные, , Истина, ЭлементДанных.Том);
		КонецЕсли;
...
//  ПараметрыЗаписиВИБ - см. РаботаСФайловымАрхивомСервер.ПараметрыЗаписиВИнформационнуюБазу
//
Процедура ЗаписатьФайлВИнформационнуюБазу(Знач ПрисоединенныйФайл, Знач ДвоичныеДанные, ПараметрыЗаписиВИБ) Экспорт

	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);

	РегистрыСведений.ХранилищеФайлов.ЗаписатьДвоичныеДанные(ПрисоединенныйФайл, ДвоичныеДанные, ПараметрыЗаписиВИБ);
...
// Параметры:
//  СсылкаПапки - СправочникСсылка.ПапкиФайлов
//
Процедура ОчиститьРабочийКаталог(СсылкаПапки) Экспорт
	...
	|ГДЕ
	|	ПапкиФайлов.Ссылка В ИЕРАРХИИ(&Ссылка)";
	...
	Попытка
		...
		Если ПакетныйРежим Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				УдаляемаяЗапись.Папка = Выборка.Ссылка;
				УдаляемаяЗапись.Пользователь = ТекущийПользователь;
			КонецЦикла;
			НаборЗаписей.Записать(РежимУдаления);
		Иначе	
			Пока Выборка.Следующий() Цикл
				НаборЗаписей = РегистрыСведений.РабочиеКаталогиФайлов.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.Папка.Установить(Выборка.Ссылка);
...
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("ru = 'У владельца файлов ""%1"" типа ""%2"" нет справочников для хранения файлов.'"),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС
			+ НСтр("ru = 'У владельца файлов ""%1"" типа ""%2"" нет справочника ""%3"" для хранения файлов.'"),
				Строка(ВладелецФайлов),
				Строка(ТипЗнч(ВладелецФайлов)),
	...
	ШаблонПричиныОшибки = НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
	...
	ТекстОшибки = ЗаголовокОшибки + Символы.ПС + ПричинаОшибки + Символы.ПС + ОкончаниеОшибки;
	ВызватьИсключение(СокрЛП(ТекстОшибки), КатегорияОшибки.ОшибкаКонфигурации);
...
Функция ИменаСправочниковСоСлужебнымиФайлами() 

	ОбъектыСФайлами = Метаданные.РегистрыСведений.НаличиеФайлов.Измерения.ОбъектСФайлами.Тип.Типы();
	
	Результат = Новый Массив;
	Результат.Добавить(Метаданные.РегистрыСведений.НаличиеФайлов.ПолноеИмя());
	Для Каждого ОбъектСФайлами Из ОбъектыСФайлами Цикл
		ИменаСправочников = ИменаСправочниковХраненияФайлов(ОбъектСФайлами, Истина);
				
		Для Каждого КлючИЗначение Из ИменаСправочников Цикл
			
			ТекущийСправочник = Метаданные.Справочники[КлючИЗначение.Ключ];
			Если ЕстьРеквизитСлужебный(КлючИЗначение.Ключ) = Ложь Тогда
				Продолжить;
			КонецЕсли;
			ПолноеИмяСправочника = ТекущийСправочник.ПолноеИмя();
			Если Результат.Найти(ПолноеИмяСправочника) = Неопределено Тогда
				Результат.Добавить(ПолноеИмяСправочника);
	       	КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
//   Строка
//
Функция ОшибкаФайлНеНайденВХранилищеФайлов(ФайлОбъект, ПолучениеФайлаИзАрхива = Ложь)
	
	ИмяФайла = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ФайлОбъект.Наименование, ФайлОбъект.Расширение);
	Если Не ПолучениеФайлаИзАрхива Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Если ФайлОбъект.ПометкаУдаления = Истина Тогда
		...
		ИначеЕсли ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ НСтр("ru = 'Данные файла недоступны, т.к. возможно файл был очищен как ненужный или удален антивирусом.'");
		Иначе
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл ""%1"" находится в файловом архиве.
				|Данные файла недоступны.
				|
				|%2'"),
			ИмяФайла, РаботаСФайловымАрхивомСервер.ТекстИнформированияПользователяОНедоступностиФайлаВАрхиве());			
	КонецЕсли;
...
//
Процедура СообщитьОбОшибкеФайлНеНайден(ФайлОбъект, ВызыватьИсключение, ПолучениеФайлаИзАрхива = Ложь) Экспорт
	
	СообщениеОбОшибке = ОшибкаФайлНеНайденВХранилищеФайлов(ФайлОбъект, ПолучениеФайлаИзАрхива);
...
Функция ПолучитьНовыйНомерДляСканирования(ВладелецФайла) Экспорт
	...
	СтруктураОтбора.Вставить("Владелец", ВладелецФайла);
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Владелец", ВладелецФайла);
		Блокировка.Заблокировать();
		...
		НаборЗаписей.Отбор.Владелец.Установить(ВладелецФайла);
		
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Владелец = ВладелецФайла;
...
Процедура УстановитьУниверсальноеВремяИзменения(ПутьКФайлу, НовоеВремяИзменения) Экспорт
	...
	Если Файл.Существует() Тогда
		...
		Исключение
			ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Работа с файлами.Ошибка установки времени изменения'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьДанныеФайлаВРегистрПриОбмене(Источник)
	...
	Если Источник.ДополнительныеСвойства.Свойство("ДвоичныеДанныеФайла", ДвоичныеДанныеФайла) Тогда
		ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(Источник);
		РегистрыСведений.ХранилищеФайлов.ЗаписатьДвоичныеДанные(Источник.Ссылка, ДвоичныеДанныеФайла, ПараметрыЗаписиВИБ);
...
Процедура ОбработатьОтправкуФайлаПоТипуХранения(ЭлементДанных)
	...
	Попытка
		...
		ЭлементДанных.ФайлХранилище = Новый ХранилищеЗначения(
			ПолучитьИзВременногоХранилища(АдресВоВременномХранилище), Новый СжатиеДанных(9));
	Исключение
	...
	ЭлементДанных.ТипХраненияФайла = РаботаСФайловымАрхивомСервер.ТипХраненияПоТомуХраненияФайлов(ЭлементДанных.Том);
	Если ЭлементДанных.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		ЭлементДанных.Том = Справочники.ТомаХраненияФайлов.ПустаяСсылка();
	КонецЕсли;
	ЭлементДанных.ПутьКФайлу = "";
...
// Возвращает отфильтрованный список присоединенных файлов по принадлежности владельцу.
//
// Параметры:
//   Файлы          - Массив из ОпределяемыйТип.ПрисоединенныйФайл - проверяемые файлы.
//   ВладелецФайлов - ОпределяемыйТип.ВладелецПрисоединенныхФайлов - ссылка на объект-владелец файлов (документ,
//                                                                   справочник и т.д.).
//
// Возвращаемое значение:
//  Массив из ОпределяемыйТип.ПрисоединенныйФайл - файлы из параметра Файлы, 
//                  у которых реквизит ВладелецФайла совпадает с параметром ВладелецФайлов.
//                  Пустой массив, если нет подходящих файлов.
//
Функция ОтобратьПрисоединенныеФайлыВладельца(Файлы, ВладелецФайлов) Экспорт
	...
	Если Файлы.Количество() < 50 Тогда
		Запрос = ОтобратьФайлыВладельцаНебольшогоСписка(Файлы, ВладелецФайлов, ИменаСправочников);
	Иначе
		Запрос = ОтобратьФайлыВладельцаБольшогоСписка(Файлы, ВладелецФайлов, ИменаСправочников);
	КонецЕсли;
...
Функция ОтобратьФайлыВладельцаБольшогоСписка(Файлы, ВладелецФайлов, ИменаСправочников)
	
	ТипыФайлов    = Новый Массив;
	НаборЗапросов = Новый Массив;
	
	ШаблонЗапроса = "ВЫБРАТЬ
	|    ВТ_Файлы.Ссылка КАК Ссылка
	|ИЗ
	|    ВТ_Файлы КАК ВТ_Файлы
	|        ВНУТРЕННЕЕ СОЕДИНЕНИЕ &ИмяСправочника КАК ПрисоединенныеФайлы
	|        ПО ВТ_Файлы.Ссылка = ПрисоединенныеФайлы.Ссылка
	|            И ПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайлов";
	
	НужноДобавитьРазрешенные = Истина;
	Для Каждого КлючИЗначение Из ИменаСправочников Цикл
		ТекстЗапроса = СтрЗаменить(ШаблонЗапроса, "&ИмяСправочника", "Справочник." + КлючИЗначение.Ключ);
		
		Если НужноДобавитьРазрешенные Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВЫБРАТЬ", "ВЫБРАТЬ РАЗРЕШЕННЫЕ"); // @Query-part-1, @Query-part-2
			НужноДобавитьРазрешенные = Ложь;
		КонецЕсли;
		
		НаборЗапросов.Добавить(ТекстЗапроса);
		ТипыФайлов.Добавить(Тип("СправочникСсылка." + КлючИЗначение.Ключ));
	КонецЦикла;
	...
	|ПОМЕСТИТЬ ВТ_Файлы
	|ИЗ
	|    &ТаблицаФайлов КАК Файлы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|
	|" + СтрСоединить(НаборЗапросов, ОбщегоНазначения.ТекстОбъединитьВсе());
	
	ТипКолонки = Новый ОписаниеТипов(ТипыФайлов);
	
	СписокФайлов = Новый ТаблицаЗначений;
	СписокФайлов.Колонки.Добавить("Ссылка", ТипКолонки);
	...
	Запрос = Новый Запрос(ТекстЗапросов);
	Запрос.УстановитьПараметр("ВладелецФайлов", ВладелецФайлов);
	Запрос.УстановитьПараметр("ТаблицаФайлов",  СписокФайлов);
	Возврат Запрос;
	
КонецФункции

Функция ОтобратьФайлыВладельцаНебольшогоСписка(Файлы, ВладелецФайлов, ИменаСправочников)
	...
	ШаблонЗапроса = "ВЫБРАТЬ
		...
		|    &ИмяСправочника КАК ПрисоединенныеФайлы
		|ГДЕ
		|    ПрисоединенныеФайлы.Ссылка В(&Файлы)
		|    И ПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайлов";
	...
	ТекстЗапросов = СтрСоединить(НаборЗапросов, ОбщегоНазначения.ТекстОбъединитьВсе());
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	 Знач АдресФайлаВоВременномХранилищеДвоичныеДанные, Знач РеквизитыЗначения = Неопределено) Экспорт
...
Процедура ЗаполнитьИзвлеченныйТекст(ФайлОбъект, АдресВременногоХранилищаТекста, ДвоичныеДанные = Неопределено) Экспорт
	
	ИспользованиеПолнотекстовогоПоиска = (ФайлОбъект.Метаданные().ПолнотекстовыйПоиск 
		= Метаданные.СвойстваОбъектов.ИспользованиеПолнотекстовогоПоиска.Использовать);
	
	РеквизитыФайла = Новый Структура("Зашифрован", Ложь);
	ЗаполнитьЗначенияСвойств(РеквизитыФайла, ФайлОбъект);
	
	Если ИспользованиеПолнотекстовогоПоиска И Не РеквизитыФайла.Зашифрован Тогда
		...
		ИначеЕсли Не ПустаяСтрока(АдресВременногоХранилищаТекста) Тогда
			РезультатИзвлеченияТекста = ИзвлечьТекст(АдресВременногоХранилищаТекста, ДвоичныеДанные,
...
Функция СоздатьВерсию(ФайлСсылка, СведенияОФайле, Контекст = Неопределено) Экспорт
	...
	Версия.ИндексКартинки = РаботаСФайламиСлужебныйКлиентСервер.ИндексПиктограммыФайла(СведенияОФайле.РасширениеБезТочки);
	...
	ДвоичныеДанныеИлиПуть	= Неопределено;

	Если РаботаСФайловымАрхивомСервер.ДоступныХранилищаДвоичныхДанных() Тогда
		Если СведенияОФайле.СсылкаНаВерсиюИсточник <> Неопределено Тогда // создание Файла из шаблона
			...
			Если ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда

				ДвоичныеДанныеИлиПуть = СведенияОФайле.АдресВременногоХранилищаФайла.Получить();

			ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
	...
	РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(Версия, ДвоичныеДанныеИлиПуть, Версия.Размер, Версия.Расширение);

	ТипХраненияФайлов = Версия.ТипХраненияФайла;

	Если СведенияОФайле.СсылкаНаВерсиюИсточник <> Неопределено Тогда // создание Файла из шаблона
		
		ТипХраненияФайловШаблона = СведенияОФайле.СсылкаНаВерсиюИсточник.ТипХраненияФайла;
		Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ТипХраненияФайловШаблона)
			И РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ТипХраненияФайлов) Тогда

			// И шаблон, и новый Файл - в базе или в томах хранилищ двоичных данных
			// При создании Файла из шаблона хранилище значения копируется напрямую.
			ДвоичныеДанныеИлиПуть = СведенияОФайле.АдресВременногоХранилищаФайла.Получить();
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске
			...
			Если Не СведенияОФайле.СсылкаНаВерсиюИсточник.Том.Пустая() Тогда
				...
				РаботаСФайламиВТомахСлужебный.ДобавитьФайл(Версия, ПолныйПутьФайлаШаблона,,,Версия.Том);
			КонецЕсли;
			
		ИначеЕсли РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ТипХраненияФайловШаблона)
			И ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда

			// Шаблон в базе или томах хранилищ двоичных данных, новый файл в томе.
			// В этом случае в АдресВременногоХранилищаФайла находится ХранилищеЗначения с файлом.
			РаботаСФайламиВТомахСлужебный.ДобавитьФайл(Версия, СведенияОФайле.АдресВременногоХранилищаФайла.Получить(),,,Версия.Том);

		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске			
			И РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ТипХраненияФайлов) Тогда

			// Шаблон в томе, новый файл - в базе или томах хранилищ двоичных данных.
			Если Не СведенияОФайле.СсылкаНаВерсиюИсточник.Том.Пустая() Тогда
				ДвоичныеДанныеИлиПуть = РаботаСФайламиВТомахСлужебный.ДанныеФайла(СведенияОФайле.СсылкаНаВерсиюИсточник);
			КонецЕсли;
	...
	Иначе // Создание объекта Файл на основе выбранного файла с компьютера.
		...
		Если ЭтоАдресВременногоХранилища(СведенияОФайле.АдресВременногоХранилищаФайла) Тогда
			...
			Если ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
				...
				Иначе
					РаботаСФайламиВТомахСлужебный.ДобавитьФайл(Версия, ДвоичныеДанныеИлиПуть,,,Версия.Том);
				КонецЕсли;
			КонецЕсли;
	...
	ЗаполнитьИзвлеченныйТекст(Версия, СведенияОФайле.АдресВременногоХранилищаТекста);

	Версия.Заполнить(Неопределено);
	Версия.Записать();

	Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(Версия.ТипХраненияФайла) Тогда
		ПараметрыЗаписиВИБ = РаботаСФайловымАрхивомСервер.ЗаполненныеПараметрыЗаписиВИнформационнуюБазу(Версия);
		ЗаписатьФайлВИнформационнуюБазу(Версия.Ссылка, ДвоичныеДанныеИлиПуть, ПараметрыЗаписиВИБ);
	КонецЕсли;
...
Функция ОбновитьВерсиюФайла(ФайлСсылка,
	...
	Если СведенияОФайле.ХранитьВерсии Тогда
		...
		ЗаголовокОшибки = НСтр("ru = 'Невозможно записать версию присоединенного файла.'");
		...
		ИмяСправочникаХранилищаВерсийФайлов = ИмяСправочникаХраненияВерсийФайлов(
			ТипЗнч(ЗначенияРеквизитовФайла.ВладелецФайла), "", ЗаголовокОшибки);
	...
	РаботаСФайловымАрхивомСервер.ЗаполнитьПараметрыХраненияФайла(Версия, ДвоичныеДанные, СведенияОФайле.Размер, СведенияОФайле.РасширениеБезТочки);
	ТипХраненияФайла = Версия.ТипХраненияФайла;
...
Процедура ОбновитьВерсиюВФайле(ФайлСсылка,
	...
	Попытка
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
			МодульУправлениеДоступом.УстановитьБлокировкуПередЗаписьюВФайловойИБ();
		КонецЕсли;
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(МетаданныеСправочника.ПолноеИмя());
		ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", ФайлСсылка);
		...
		ЗаполнитьИзвлеченныйТекст(ФайлОбъект, АдресВременногоХранилищаТекста);
	...
	Если Размер > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Размер файла ""%1"" (%2 Мб)
			           |превышает максимально допустимый размер %3 Мб.'"),
			Имя,
...
Процедура УстановитьВидимостьКомандыОтправкаНаПодписание(Элементы, Команда, Видимость, Заголовок)
	Если Элементы.Найти(Команда) <> Неопределено Тогда
		Элементы[Команда].Видимость = Видимость;
		Если Видимость И ЗначениеЗаполнено(Заголовок) Тогда
			Элементы[Команда].Заголовок = Заголовок;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, НастраиватьОбъектФормы = Ложь) Экспорт
	...
	Элементы.ФормаОтправить.Видимость = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями");
...
Процедура ЗаполнитьСписокПодписей(Контекст, Знач Источник = Неопределено) Экспорт
	...
	Для Каждого ЭлектроннаяПодписьФайла Из ЭлектронныеПодписи Цикл
		...
		РезультатПроверкиПодписиВФорме = МодульЭлектроннаяПодписьКлиентСервер.РезультатПроверкиПодписиВФорме();
		...
		Если НоваяСтрока.Свойство("РезультатПроверкиКраткий") Тогда
			МодульЭлектроннаяПодписьКлиентСервер.ЗаполнитьСтатусПодписи(НоваяСтрока, ТекущаяДатаСеанса());
		Иначе
			РаботаСФайламиСлужебныйКлиентСервер.ЗаполнитьСтатусПодписи(НоваяСтрока, ТекущаяДатаСеанса());
		КонецЕсли;
		
		Если ЭлектроннаяПодписьФайла.Сертификат <> Неопределено Тогда 
			НоваяСтрока.АдресСертификата = ПоместитьВоВременноеХранилище(
				ЭлектроннаяПодписьФайла.Сертификат, Контекст.УникальныйИдентификатор);
		КонецЕсли;
...
Функция СписокЭлектронныхПодписей(Источник, УникальныйИдентификатор)
	...
	Если Источник.ПодписанЭП Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			ЭлектронныеПодписи = МодульЭлектроннаяПодпись.ПодписиОбъекта(Источник.Ссылка, ПараметрыПолученияПодписей);
...
Функция НастройкиФайлов() Экспорт
	...
	НастройкиФайлов.Вставить("ДоступенФайловыйАрхив",     Истина);
...
Процедура ОчиститьДанныеНенужныхФайлов(НенужныеФайлы)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УдалениеПомеченныхОбъектов") Тогда
		...
		Если Не РезультатУдаления.Успешно Тогда
			ЗаписатьВЖурналСобытийОчисткиФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не все ненужные файлы были удалены автоматически (%1), так как они используются в других местах, либо по другим причинам.
...
Функция УдалитьНенужныйФайл(ПрисоединенныйФайл)
	...
	Попытка
		...
		Если ПрисоединенныйФайлОбъект = Неопределено Тогда // Уже удален
			...
			Возврат Ложь;
		КонецЕсли;	
		Если ТипЗнч(ПрисоединенныйФайл) <> СправочникВерсийФайлов()
...
Функция ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
		...
		Иначе
			...
			РеквизитыФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаСуществующегоФайла, 
				"ХранитьВерсии, ТекущаяВерсия");
			...
			Иначе
				РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияФайла(СсылкаСуществующегоФайла, 
					СведенияОФайле, Истина, "", "", Ложь);
			КонецЕсли;
		...
		ЗапомнитьСерверныеДанныеСсылки(НовыйФайл, АдресФайла, ИдентификаторEtag, ЭтоФайл, ОбъектВладелец, Ложь, 
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Не удалось загрузить файл ""%1"" из облачного сервиса по причине:'") + " " 
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			Если НайденныйHref = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Облачный сервис файлов вернул некорректный ответ без HREF в %1.'");
...
Процедура ЗагрузитьПодписиФайла(ФайлСсылка, Подписи, ПараметрыСинхронизации)
	...
	МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
	МодульЭлектроннаяПодписьКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодписьКлиентСервер");
	ПодписиВФайле = МодульЭлектроннаяПодпись.ПодписиОбъекта(ФайлСсылка);
			...
			Иначе
				...
				ИначеЕсли (ИзмененВБазе ИЛИ ИзмененНаСервере) И НЕ (УдаленВБазе ИЛИ УдаленНаСервере) Тогда
					...
					Если ИзмененНаСервере И СтрокаТаблицы.ПодписанЭП Тогда
						ВызватьИсключение НСтр("ru = 'Не предусмотрена загрузка из облака измененных файлов, которые подписаны в приложении.
...
Функция СоздатьФайлВОблачномСервисе(Знач АдресСервера, Знач ПараметрыСинхронизации, Знач СтрокаТаблицы, Знач ТаблицаФайлов)
	...
	СтрокаТаблицы.ToHref       = ЗакончитьБезСлеша(АдресСервера) 
		+ НачатьСлешем(ЗакончитьБезСлеша(РассчитатьHref(СтрокаТаблицы,ТаблицаФайлов)));
	
	Если ОбщегоНазначения.ОбъектЯвляетсяГруппой(СтрокаТаблицы.ФайлСсылка) Или СтрокаТаблицы.ЭтоПапка Тогда
	...
	Иначе
		...
		СтрокаТаблицы.ToEtag = ВызватьМетодPUT(СтрокаТаблицы.ToHref, СтрокаТаблицы.ФайлСсылка, 
			ПараметрыСинхронизации, СтрокаТаблицы.ЭтоФайл);
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись")
			И СтрокаТаблицы.ПодписанЭП Тогда
			
			МодульЭлектроннаяПодпись = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодпись");
			ЭлектронныеПодписи = МодульЭлектроннаяПодпись.ПодписиОбъекта(СтрокаТаблицы.ФайлСсылка);
			НомерПодписи = 0;
...
Функция ВыбратьДанныеПоПравилам(УчетнаяЗапись, Синхронизировать = ИСТИНА)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		Если (Синхронизировать И СтрокаТаблицы.Синхронизировать > 0)
			Или (Не Синхронизировать И СтрокаТаблицы.Синхронизировать = 0) Тогда
			Прервать;
		КонецЕсли;
		
		УдаляемыеСтроки.Добавить(СтрокаТаблицы);
		Если Не Синхронизировать
			И НесинхронизируемыеВладельцы.Найти(СтрокаТаблицы.Родитель) = Неопределено Тогда
			НесинхронизируемыеВладельцы.Добавить(СтрокаТаблицы.Родитель);
		КонецЕсли;
...
Функция СведенияОСинхронизации(Знач ВладелецФайла) Экспорт
	...
	Пока Таблица.Количество() > 0  Цикл
		...
		Результат.Вставить("АдресПапкиВОблачномСервисе", АдресВОблачномСервисе(Результат.Сервис, Результат.Href));
		Возврат Результат;
	КонецЦикла;
...
Функция АдресВОблачномСервисе(Сервис, Href)
	
	АдресОбъекта = Href;
	
	Если Не ПустаяСтрока(Сервис) Тогда
		Если Сервис = "https://dav.box.com/dav" Тогда
...
Функция КоличествоВерсийСНеизвлеченнымТекстом() Экспорт
	...
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ПрисоединенныйФайл.Тип.Типы() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			Если Тип = СправочникВерсийФайлов() Тогда
				ТекстЗапроса = ТекстЗапроса + "
...
Функция ТекстЗапросаДляФайловСНеизвлеченнымТекстом(ИмяСправочника, ЧислоФайловВВыборке,
	...
	СтрокаЗамены = ?(ПолучитьВсеФайлы, "ВЫБРАТЬ", "ВЫБРАТЬ ПЕРВЫЕ " + Формат(ЧислоФайловВВыборке, "ЧГ=; ЧН=")); // @query-part-1, @query-part-2
...
Функция ИмяФайлаСДвоичнымиДанными(ОбъектСсылка) 
	...
	Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ОбъектСсылка.ТипХраненияФайла) Тогда
...
// 
Функция ИзвлечьТекст(Знач АдресВременногоХранилищаТекста, Знач ДвоичныеДанные = Неопределено, 
	Знач Расширение = Неопределено) Экспорт
	...
	БезопасноеРасширение = РасширениеДляИзвлеченияТекстаКорректное(Расширение);
	
	Если Не БезопасноеРасширение Тогда
		Результат.СтатусИзвлеченияТекста = Перечисления.СтатусыИзвлеченияТекстаФайлов.НеИзвлечен;
		Результат.ТекстХранилище = Новый ХранилищеЗначения("");
		
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииИзвлеченияТекста(),
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку
				(НСтр("ru = 'Попытка извлечения текста из файла с недопустимым расширением: %1'"), Расширение));
...
Функция РасширениеДляИзвлеченияТекстаКорректное(Расширение)
	
	Если Расширение = Неопределено Или ПустаяСтрока(Расширение) Тогда
		Возврат Истина;
	КонецЕсли;
	
	НедопустимыеСимволы = ОбщегоНазначенияКлиентСервер.НайтиНедопустимыеСимволыВИмениФайла(Расширение);
	Если НедопустимыеСимволы.Количество() > 0 
	 Или СтрДлина(Расширение) > 20 Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Попытка
		...
		Если ФайлОбъект <> Неопределено Тогда
			...
			Иначе
				...
				Если Не РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ФайлОбъект.ТипХраненияФайла) Тогда
					ИмяФайлаСДвоичнымиДанными = "";
				КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииИзвлеченияТекста()
	
	Возврат НСтр("ru = 'Файлы.Извлечение текста'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если Не ТекущаяВерсия.Ссылка.Пустая() Тогда
		...
		Если РеквизитыФайла.Размер <> ТекущаяВерсия.Размер 
			...
			Попытка
				...
				СкопироватьДополнительноеСвойство(Источник, ФайлОбъект, "ЗаписьПодписанногоОбъекта");
				ФайлОбъект.ДополнительныеСвойства.Вставить("РазрешитьИзменениеПутиФайла", Истина);
...
Процедура СкопироватьДополнительноеСвойство(Источник, Приемник, ИмяСвойства)
	ЗначениеСвойства = Неопределено;
	Если Источник.ДополнительныеСвойства.Свойство(ИмяСвойства, ЗначениеСвойства) Тогда
		Приемник.ДополнительныеСвойства.Вставить(ИмяСвойства, ЗначениеСвойства);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередУдалениемПрисоединенногоФайлаСервер(Знач Ссылка,
	...
	Если РаботаСФайламиВХранилищеДвоичныхДанныхСлужебный.ТипХраненияНеИспользуетДиски(ТипХраненияФайла) Тогда
		РегистрыСведений.ХранилищеФайлов.УдалитьДвоичныеДанные(Ссылка);
	КонецЕсли;
...
Функция ЕстьФайлыУВладельца(Знач ВладелецФайлов, Знач ФайлИсключение = Неопределено) Экспорт
	...
	Если ФайлИсключение <> Неопределено Тогда
		УсловияИсключений = "ПрисоединенныеФайлы.Ссылка <> &ФайлИсключение";
	...
	Для каждого КлючИЗначение Из ИменаСправочниковХраненияФайлов(ВладелецФайлов) Цикл
		...
		ТекущееУсловияИсключений = УсловияИсключений;
		Если ЕстьРеквизитСлужебный(КлючИЗначение.Ключ) Тогда
			ТекущееУсловияИсключений = ТекущееУсловияИсключений + ?(ЗначениеЗаполнено(ТекущееУсловияИсключений)," И ","") + "ПрисоединенныеФайлы.Служебный = ЛОЖЬ";
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ТекущееУсловияИсключений) Тогда
			ТекущееУсловияИсключений = "ИСТИНА";
		КонецЕсли;
		ТекущийТекстЗапроса = СтрЗаменить(ТекущийТекстЗапроса, "&УсловияИсключений", ТекущееУсловияИсключений); // @Query-part-1
		ТекстыЗапроса.Добавить(СтрЗаменить(ТекущийТекстЗапроса, "&ИмяСправочника", "Справочник." + КлючИЗначение.Ключ));
	КонецЦикла;
...
Процедура ОбновитьСписокЗапрещенныхРасширений() Экспорт
	...
	Если ОбновитьСписокЗапрещенныхРасширений Тогда
		СписокЗапрещенныхРасширенийВБазе = СокрЛП(СтрСоединить(МассивЗапрещенныхРасширений, " "));
...
Процедура ОбновитьСписокЗапрещенныхРасширенийВОбластиДанных() Экспорт
	...
	СписокЗапрещенныхРасширений = СписокЗапрещенныхРасширений();
	...
	Для Каждого Расширение Из СписокЗапрещенныхРасширений Цикл
	...
	Если ОбновитьСписокЗапрещенныхРасширенийОбластиДанных Тогда
		СписокЗапрещенныхРасширенийОбластиДанных = СокрЛП(СтрСоединить(МассивЗапрещенныхРасширенийОбластиДанных, " "));
...
Процедура ОбновитьСписокРасширенийТекстовыхФайлов() Экспорт
	...
	Если ОбновитьСписокРасширений Тогда
		Константы.СписокРасширенийТекстовыхФайлов.Установить(СокрЛП(СтрСоединить(РасширенияТекстовыхФайлов, " ")));
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("КоличествоВерсийСНеизвлеченнымТекстом", Истина);
	Методы.Вставить("СведенияОбОчищаемыхФайлах", Истина);
	Методы.Вставить("СоздатьСерверныйНачальныйОбразНаСервере", Истина);
	Методы.Вставить("СоздатьФайловыйНачальныйОбразНаСервере", Истина);
	Методы.Вставить("ОсвободитьЗахваченныеФайлыФон", Истина);
	Методы.Вставить("ДанныеФайлаДляОткрытияАсинхронный", Истина);
	Методы.Вставить("ДанныеФайлаДляСохраненияАсинхронный", Истина);
	Методы.Вставить("ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный", Истина);
	
	Методы.Вставить("ОбновитьСписокЗапрещенныхРасширений");
	Методы.Вставить("ОбновитьСписокРасширенийТекстовыхФайлов");
	Методы.Вставить("ОбновитьСписокЗапрещенныхРасширенийВОбластиДанных");
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("РегламентнаяСинхронизацияФайловWebdav");
		Методы.Вставить("ИзвлечьТекстИзФайлов");
		Методы.Вставить("ОчиститьНенужныеФайлы");
	КонецЕсли;
