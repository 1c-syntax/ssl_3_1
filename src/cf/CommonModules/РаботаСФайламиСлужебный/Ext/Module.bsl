...
Функция ТекстЗапросаДляОчисткиФайлов(ВладелецФайла, Настройка, МассивИсключений, ЭлементИсключение) Экспорт
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка)
	...
	Попытка
		...
		Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			СвойствоФайла = РаботаСФайламиВТомахСлужебный.СвойстваФайлаВТоме(ФайлОбъект.Ссылка);
			ПолноеИмяФайлаНаДиске = РаботаСФайламиВТомахСлужебный.ПолноеИмяФайлаВТоме(СвойствоФайла);
			ИмяВременногоФайла = ПолучитьИмяВременногоФайла(ФайлОбъект.Расширение);
			
			КопироватьФайл(ПолноеИмяФайлаНаДиске, ИмяВременногоФайла);
			РаботаСФайламиВТомахСлужебный.УдалитьДанныеФайла(ФайлОбъект, ТекстОписания);
			ОписаниеВременногоФайла = Новый Файл(ИмяВременногоФайла);
			ОписаниеВременногоФайла.УстановитьТолькоЧтение(Ложь);
	...
	Исключение
		...
		Если Не ПустаяСтрока(ПолноеИмяФайлаНаДиске) Тогда
			
			// Вернем файл на прежнее место в случае ошибки.
			ПереместитьФайл(ИмяВременногоФайла, ПолноеИмяФайлаНаДиске);
...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента, Отправитель = Неопределено)
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		НовыйТипХраненияФайла = ТипХраненияФайлов();
		Если НовыйТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			НовыйТипХраненияФайла = РаботаСФайламиВТомахСлужебный.ТипХраненияФайла(ЭлементДанных.Расширение, ЭлементДанных.Размер);
		КонецЕсли;
...
Функция ВызватьМетодGET(АдресФайлаHRef, ИдентификаторEtag, ПараметрыСинхронизации, ДатаМодификацииФайла = Неопределено, ДлинаФайла = Неопределено)
	...
	// АПК:216-выкл АПИ внешнего сервиса
