...
Процедура ПриЗаписиПрисоединенногоФайлаСервер(ВладелецФайлов, Источник) Экспорт
	...
	Попытка
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Служебный",Источник.Метаданные()) Тогда
...
Процедура ОбновитьФайл(ИнформацияОФайле, ПрисоединенныйФайл) Экспорт
	...
	ИмяБезРасширения = "";
	Если ИнформацияОФайле.Свойство("ИмяБезРасширения", ИмяБезРасширения) И ЗначениеЗаполнено(ИмяБезРасширения) Тогда
...
Функция ЕстьФайлыУВладельца(Знач ВладелецФайлов, Знач ФайлИсключение = Неопределено) Экспорт
	...
	Для каждого КлючИЗначение Из ИменаСправочниковХраненияФайлов(ВладелецФайлов) Цикл
		...
		КонецЕсли;
		ТекущийТекстЗапроса = СтрЗаменить(ТекущийТекстЗапроса, "&УсловияИсключений", ТекущееУсловияИсключений); // @Query-part-2
