...
Процедура ПередНачаломРаботыСистемы(Знач ОповещениеЗавершения = Неопределено) Экспорт
	...
	Если ОтключенаЛогикаНачалаРаботыСистемы() Тогда
		Попытка
			// Проверка права и заполнение параметров работы клиента на сервере.
...
Процедура ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	Если Не Параметры.ДинамическиеИзмененияКонфигурации.ОповещатьПользователя
		Или Не Параметры.КонфигурацияБазыДанныхИзмененаДинамически Тогда
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Установлено обновление программы'"), "e1cib/app/ОбщаяФорма.КонтрольДинамическогоОбновления",
		СообщениеПользователю, БиблиотекаКартинок.Предупреждение32, СтатусОповещенияПользователя.Важное,
