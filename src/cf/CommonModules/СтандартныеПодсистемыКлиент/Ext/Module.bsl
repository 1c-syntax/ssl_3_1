...
Функция ПараметрыВопросаПользователю() Экспорт
	...
	Параметры.Вставить("Картинка", БиблиотекаКартинок.ДиалогВопрос);
...
Процедура ВыбратьОбъектыМетаданных(ПараметрыФормы, ОписаниеОповещенияОЗакрытии = Неопределено) Экспорт
	ОткрытьФорму("ОбщаяФорма.ВыборОбъектовМетаданных", ПараметрыФормы,,,,, ОписаниеОповещенияОЗакрытии);
КонецПроцедуры
...
Процедура ПоказатьРедакторТабличногоДокумента(Знач ТабличныйДокумент, Знач ПараметрыФормы = Неопределено, 
	...
	Если ПараметрыФормы = Неопределено Тогда
		ПараметрыФормы = ПараметрыРедактораТабличногоДокумента();
	КонецЕсли;
	ПараметрыФормы.ТабличныйДокумент = ТабличныйДокумент;
	
	ОткрытьФорму("ОбщаяФорма.РедактированиеТабличногоДокумента", ПараметрыФормы, Владелец,,,
...
Функция ПараметрыРедактораТабличногоДокумента() Экспорт
	...
	Результат.Вставить("ИмяДокумента", "");
...
Процедура ПоказатьСравнениеТабличныхДокументов(ТабличныйДокументЛевый, ТабличныйДокументПравый, Параметры) Экспорт
	...
	Если ТабличныйДокументЛевый <> Неопределено Тогда
		ПараметрыФормы = ПараметрыСравненияТабличныхДокументов();
		...
		ПараметрыФормы.АдресТабличныхДокументов = ПоместитьВоВременноеХранилище(СравниваемыеДокументы, Неопределено);
	Иначе
		ПараметрыФормы = Параметры;
	КонецЕсли;
	ОткрытьФорму("ОбщаяФорма.СравнениеТабличныхДокументов", ПараметрыФормы);
...
Функция ПараметрыСравненияТабличныхДокументов() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("АдресТабличныхДокументов", "");
	Результат.Вставить("Заголовок", "");
	Результат.Вставить("ЗаголовокЛевый", "");
	Результат.Вставить("ЗаголовокПравый", "");
	Возврат Результат;
...
Процедура НастроитьВидимостьИЗаголовокСсылкиОтправкиОтчетаОбОшибке(Элемент, ИнформацияОбОшибке, ОшибкаТребуетЗавершениеРаботы = Ложь) Экспорт
	...
	Настройки = ПараметрКлиента("НастройкиОтправкиИнформацииОбОшибке");
	КатегорияДляПользователя = ОбработкаОшибок.КатегорияОшибкиДляПользователя(ИнформацияОбОшибке);
	
	Элемент.Видимость =
		    Не ОшибкаТребуетЗавершениеРаботы И КатегорияДляПользователя = КатегорияОшибки.ПрочаяОшибка
		Или Не ОшибкаТребуетЗавершениеРаботы И КатегорияДляПользователя = КатегорияОшибки.ОшибкаКонфигурации
		Или    ОшибкаТребуетЗавершениеРаботы И КатегорияДляПользователя <> КатегорияОшибки.ОшибкаСеанса;
	
	Если Настройки.РежимОтправки = РежимОтправкиИнформацииОбОшибке.Отправлять Тогда
		Элемент.Заголовок = НСтр("ru = 'Отчет об ошибке будет отправлен автоматически.
			|Настроить отчет...'");
	Иначе
		Элемент.Заголовок = НСтр("ru = 'Сформировать отчет об ошибке'");
	КонецЕсли;
...
Процедура ПоказатьОтчетОбОшибке(ОтчетДляОтправки) Экспорт
	...
	Если ЗначениеЗаполнено(Настройки.АдресОтправки)
	   И Настройки.РежимОтправки <> РежимОтправкиИнформацииОбОшибке.НеОтправлять Тогда
		
		ОтчетДляОтправки.Отправить(Истина);
	Иначе
		ОтчетДляОтправки.Записать(, Истина);
	КонецЕсли;
...
Процедура ОтправитьОтчетОбОшибке(ОтчетДляОтправки, ИнформацияОбОшибке, ОшибкаТребуетЗавершениеРаботы = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Настройки.АдресОтправки)
	 Или Настройки.РежимОтправки <> РежимОтправкиИнформацииОбОшибке.Отправлять Тогда
		Возврат;
	КонецЕсли;
	
	КатегорияДляПользователя = ОбработкаОшибок.КатегорияОшибкиДляПользователя(ИнформацияОбОшибке);
	
	Если ОшибкаТребуетЗавершениеРаботы И КатегорияДляПользователя <> КатегорияОшибки.ОшибкаСеанса
	 Или Не ОшибкаТребуетЗавершениеРаботы
	   И (    КатегорияДляПользователя = КатегорияОшибки.ПрочаяОшибка
	      Или КатегорияДляПользователя = КатегорияОшибки.ОшибкаКонфигурации) Тогда
		
		ОтчетДляОтправки.Отправить(Ложь);
	КонецЕсли;
...
Процедура ПередЗавершениемРаботыСистемы(Отказ = Ложь, ТекстПредупреждения = "") Экспорт
	...
	Если Предупреждения.Количество() = 0 Тогда
		Если Не ПараметрКлиента("ЗапрашиватьПодтверждениеПриЗавершенииПрограммы") Тогда
			Возврат;
		КонецЕсли;
		ТекстПредупреждения = НСтр("ru = 'Завершить работу с приложением?'");
		Отказ = Истина;
	Иначе
		Отказ = Истина;
		...
		КонецЕсли;
		МассивПредупреждений.Добавить(Символы.ПС);
		МассивПредупреждений.Добавить(НСтр("ru = 'Для этого выберите ""Продолжить работу"" и затем нажмите на всплывающее оповещение.'"));
...
Процедура ПриГлобальномПоиске(СтрокаПоиска, ПланПоиска) Экспорт
	...
	ИнтеграцияПодсистемБСПКлиент.ПриГлобальномПоиске(СтрокаПоиска, ПланПоиска);
...
Процедура ПриВыбореРезультатаГлобальногоПоиска(ЭлементРезультата, СтандартнаяОбработка) Экспорт
	...
	ИнтеграцияПодсистемБСПКлиент.ПриВыбореРезультатаГлобальногоПоиска(
		ЭлементРезультата,
		СтандартнаяОбработка);
	ОбщегоНазначенияКлиентПереопределяемый.ПриВыбореРезультатаГлобальногоПоиска(
		ЭлементРезультата,
		СтандартнаяОбработка);
...
Процедура ПриВыбореДействияРезультатаГлобальногоПоиска(ЭлементРезультата, Действие) Экспорт
	...
	ИнтеграцияПодсистемБСПКлиент.ПриВыбореДействияРезультатаГлобальногоПоиска(
		ЭлементРезультата,
		Действие);
	ОбщегоНазначенияКлиентПереопределяемый.ПриВыбореДействияРезультатаГлобальногоПоиска(
		ЭлементРезультата,
		Действие);
...
Процедура ОбработкаПереходаПоНавигационнойСсылке(ДанныеПереходаПоНавигационнойСсылке,
	...
	ИнтеграцияПодсистемБСПКлиент.ОбработкаПереходаПоНавигационнойСсылке(
		ДанныеПереходаПоНавигационнойСсылке,
		СтандартнаяОбработка);
	ОбщегоНазначенияКлиентПереопределяемый.ОбработкаПереходаПоНавигационнойСсылке(
		ДанныеПереходаПоНавигационнойСсылке,
		СтандартнаяОбработка);
...
Процедура ЗаполнитьПараметрыКлиента(ПараметрыКлиента) Экспорт
	...
	Если ТипЗнч(ПараметрыПриложения[ИмяПараметра]) <> Тип("Структура") Тогда
		...
		ПараметрыПриложения[ИмяПараметра].Вставить("КоличествоДополнительныхЯзыков");
		ПараметрыПриложения[ИмяПараметра].Вставить("НастройкиОтправкиИнформацииОбОшибке");
		ПараметрыПриложения[ИмяПараметра].Вставить("ЭлементыСтиля");
	КонецЕсли;
...
// См. СтандартныеПодсистемыСервер.ОтключенаЛогикаНачалаРаботыСистемы
...
Функция ЭлементыСтиля() Экспорт
	...
	ПараметрЭлементыСтиля = ПараметрКлиента("ЭлементыСтиля");
	Если ТипЗнч(ПараметрЭлементыСтиля) = Тип("Структура") Тогда
		Возврат ПараметрЭлементыСтиля;
	КонецЕсли;
	
	ЭлементыСтиля = Новый Структура;
	
	Для Каждого ЭлементСтиля Из ПараметрЭлементыСтиля Цикл
...
#КонецЕсли
	...
	УстановитьПараметрКлиента("ЭлементыСтиля", ЭлементыСтиля);
...
Функция НовыйПараметрОповещенияЗаписиТабличногоДокумента() Экспорт
	
	ПараметрыОповещения = Новый Структура;
	
	ПараметрыОповещения.Вставить("ПутьКФайлу", "");
	ПараметрыОповещения.Вставить("ИмяОбъектаМетаданныхМакета", "");
	ПараметрыОповещения.Вставить("КодЯзыка", "");
	ПараметрыОповещения.Вставить("Представление", "");
	ПараметрыОповещения.Вставить("ИсточникиДанных", Новый Массив());
	ПараметрыОповещения.Вставить("ОсновнаяПечатнаяФорма", Ложь);
	ПараметрыОповещения.Вставить("НаименованиеПечатнойФормы", "");
	ПараметрыОповещения.Вставить(
		"ФорматСохраненияВыгрузки",
		ПредопределенноеЗначение("Перечисление.ФорматыВыгрузкиОбъектов.ПустаяСсылка"));
	ПараметрыОповещения.Вставить("МакетДляВыгрузкиОбъекта", Ложь);
...
//
// Системная информация содержит описание технических характеристик клиентского компьютера.
// Серверный аналог: см. СтандартныеПодсистемыСервер.ИнформацияДляПоддержки.
//
// Пример:
...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Если ЗапускПрограммыЗавершен() Тогда
		ПараметрыКонфигурации = ПараметрыРаботыКлиента();
	Иначе
		ПараметрыКонфигурации = ПараметрыРаботыКлиентаПриЗапуске();
	КонецЕсли;
	
	ПараметрНедоступен = НСтр("ru = 'недоступно'");
	
	// АПК:1417-выкл Динамический состав свойств.
	ИнформацияДляПоддержки.НазваниеПрограммы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
	...
	// АПК:1417-выкл Динамический состав свойств.
	ИнформацияДляПоддержки.ВерсияПрограммы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыКонфигурации,
		"ВерсияКонфигурации",
		ПараметрНедоступен);
	// АПК:1417-вкл
	
	ИнформацияДляПоддержки.ВерсияБСП = СтандартныеПодсистемыВызовСервера.ВерсияБиблиотеки();
	...
	ИнформацияДляПоддержки.ЭтоПолноправныйПользователь = ПользователиКлиент.ЭтоПолноправныйПользователь();
	ИнформацияДляПоддержки.ЭтоУчебнаяПлатформа = ЭтоУчебнаяПлатформа();
	
	// АПК:1417-выкл Динамический состав свойств.
	НастройкиОбновления = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ПараметрыКонфигурации,
		"НастройкиОбновления");
	// АПК:1417-вкл
	
	// АПК:1417-выкл Динамический состав свойств.
	ИнформацияДляПоддержки.КонфигурацияИзменена = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		НастройкиОбновления,
...
#Область ВыводРезультатаВыполнения
...
Процедура ДействияПередНачаломРаботыСистемы(ОповещениеЗавершения)
	...
	Параметры.Вставить("ВызванныеПроцедуры", Новый Массив);
	...
	Если ОтключенаЛогикаНачалаРаботыСистемы() Тогда
		Попытка
			...
			Если СвойстваКлиента.Свойство("ОшибкаНетПраваОтключитьЛогикуНачалаРаботыСистемы") Тогда
				УстановитьПоследнююПроцедуру(Параметры, "ПользователиСлужебныйКлиент",
					"УстановитьИнтерактивнуюОбработкуПриОшибкеНедостаточноПравДляВходаВПрограмму*");
	...
	УстановитьПоследнююПроцедуру(Параметры, "ОбщегоНазначенияКлиент", "ПодсистемаСуществует*");
	Попытка
...
Процедура ДействияПередНачаломРаботыСистемыВИнтеграционнойПроцедуре(Неопределен, Контекст) Экспорт
	...
	Попытка
		...
		УстановитьПоследнююПроцедуру(Параметры, "ИнтеграцияПодсистемБСПКлиент", "ПередНачаломРаботыСистемы*");
	...
	Пока Истина Цикл
		...
		Попытка
			Если ТипЗнч(ОписаниеМодуля) <> Тип("Структура") Тогда
				...
				УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы*");
				ТекущийМодуль.ПередНачаломРаботыСистемы(Параметры);
				УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы");
			Иначе
				ТекущийМодуль = ОписаниеМодуля.Модуль;
				Если ОписаниеМодуля.Номер = 2 Тогда
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы2*");
					ТекущийМодуль.ПередНачаломРаботыСистемы2(Параметры);
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы2");
				ИначеЕсли ОписаниеМодуля.Номер = 3 Тогда
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы3*");
					ТекущийМодуль.ПередНачаломРаботыСистемы3(Параметры);
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы3");
				ИначеЕсли ОписаниеМодуля.Номер = 4 Тогда
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы4*");
					ТекущийМодуль.ПередНачаломРаботыСистемы4(Параметры);
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы4");
				ИначеЕсли ОписаниеМодуля.Номер = 5 Тогда
					УстановитьПоследнююПроцедуру(Параметры, ТекущийМодуль, "ПередНачаломРаботыСистемы5*");
...
Процедура ДействияПередНачаломРаботыСистемыВПереопределяемойПроцедуре(Неопределен, Контекст)
	...
	Если ОбщегоНазначенияКлиент.ДоступноИспользованиеРазделенныхДанных() Тогда
		Попытка
			Параметры.Вставить("Модули", Новый Массив);
			УстановитьПоследнююПроцедуру(Параметры, "ОбщегоНазначенияКлиентПереопределяемый",
				"ПередНачаломРаботыСистемы*");
...
Процедура ДействияПриНачалеРаботыСистемы(ОповещениеЗавершения, НепрерывноеВыполнение)
	...
	Если Не ЗапускПрограммыЗавершен() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ВызванныеПроцедурыПередНачаломРаботыСистемы());
		Попытка
...
Процедура ДействияПриНачалеРаботыСистемыПослеВсехПроцедур(Неопределен, Контекст)
	...
	Параметры.Вставить("ОбработкаПродолжения", Новый ОписаниеОповещения(
		"ДействияПриНачалеРаботыСистемыПередЗавершением", ЭтотОбъект));
...
// Только для внутреннего использования. Продолжение процедуры ПриНачалеРаботыСистемы.
Процедура ДействияПриНачалеРаботыСистемыПередЗавершением(Неопределен, Контекст) Экспорт
	...
	ПараметрыПриЗапускеПрограммы = ПараметрыПриложения["СтандартныеПодсистемы.ПараметрыПриЗапускеПрограммы"];
	ПараметрыПриЗапускеПрограммы.Вставить("ПроцедураПриНачалеРаботыСистемыЗавершена");
	Попытка
...
#Область ПрочиеПроцедурыИФункцииДляЗапускаИЗавершенияРаботыПрограммы
...
Процедура ПроверитьВерсиюПлатформыПриЗапуске(Параметры, Контекст) Экспорт
	...
	Если СтрНайти(ПараметрЗапуска, "ВыполнитьОбновлениеИЗавершитьРаботу") > 0
		...
		ТекстСообщения = НСтр("ru = 'Обновление приложения на новую версию невозможно.
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Текущая, Минимальная) < 0 Тогда
		Если ПользователиКлиент.ЭтоПолноправныйПользователь(Истина) Тогда
			ТекстСообщения =
				НСтр("ru = 'Вход в приложение невозможен.
				           |Предварительно обновите версию платформы 1С:Предприятие.'");
		Иначе
			ТекстСообщения =
				НСтр("ru = 'Вход в приложение невозможен.
	...
	Иначе
		Если ПользователиКлиент.ЭтоПолноправныйПользователь(Истина) Тогда
			ТекстСообщения =
				НСтр("ru = 'Рекомендуется завершить работу приложения и обновить версию платформы 1С:Предприятия.
		...
		Иначе
			ТекстСообщения = 
				НСтр("ru = 'Рекомендуется завершить работу приложения и обратиться к администратору для обновления версии платформы 1С:Предприятия.
				         |Новая версия платформы содержит исправления ошибок, которые позволят приложению работать более стабильно.
				         |Вы также можете продолжить работу на текущей версии.
				         |Минимально необходимая версия платформы %1.'");
		КонецЕсли;
...
Процедура ИнтерактивнаяОбработкаВосстановленияСвязиСГлавнымУзлом(Параметры, Контекст) Экспорт
	...
	Если ПараметрыКлиента.ВосстановитьСвязьСГлавнымУзлом = Ложь Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru = 'Вход в приложение временно невозможен до восстановления связи с главным узлом.
...
Процедура ИнтерактивнаяОбработкаНачальныхРегиональныхНастроекИБ(Параметры, Контекст) Экспорт
	...
	Если ПараметрыКлиента.ВыбратьНачальныеРегиональныеНастройкиИБ = Ложь Тогда
		...
		ПоказатьПредупреждение(
			ОповещениеБезРезультата(Параметры.ОбработкаПродолжения),
			НСтр("ru = 'Вход в приложение невозможен до установки начальных региональных настроек.
			           |Обратитесь к администратору за подробностями.'"),
			15);
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		ПараметрыОткрытия  = Новый Структура("Источник", "НачальноеЗаполнение");
		МодульМультиязычностьКлиент.ОткрытьФормуРегиональныхНастроек(ОписаниеОповещения, ПараметрыОткрытия);
...
#Если Не ВебКлиент Тогда
		Отказ = Истина;
#КонецЕсли
		Если Не ПараметрыПриЗапускеПрограммы.Свойство("ПроцедураПриНачалеРаботыСистемыЗавершена") Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьПоследнююПроцедуру(Параметры, ИмяМодуля = "", ИмяПроцедуры = "", ОписаниеОповещения = Неопределено)
	...
	ОписаниеПроцедуры = Новый Структура("Модуль, Имя");
	
	Если ОписаниеОповещения = Неопределено Тогда
		ОписаниеПроцедуры.Модуль = ИмяМодуля;
		ОписаниеПроцедуры.Имя    = ИмяПроцедуры;
	Иначе
		ОписаниеПроцедуры.Модуль = ОписаниеОповещения.Модуль;
		ОписаниеПроцедуры.Имя    = ОписаниеОповещения.ИмяПроцедуры + ИмяПроцедуры;
	КонецЕсли;
	
	ПоследнееОписаниеПроцедуры = ?(ЗначениеЗаполнено(Параметры.ВызванныеПроцедуры),
		Параметры.ВызванныеПроцедуры.Получить(0), Неопределено);
	
	Если ПоследнееОписаниеПроцедуры <> Неопределено
	   И ПоследнееОписаниеПроцедуры.Модуль = ОписаниеПроцедуры.Модуль
	   И ПоследнееОписаниеПроцедуры.Имя    = ОписаниеПроцедуры.Имя + "*" Тогда
		
		ПоследнееОписаниеПроцедуры.Имя = ОписаниеПроцедуры.Имя;
	Иначе
		Параметры.ВызванныеПроцедуры.Вставить(0, ОписаниеПроцедуры);
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры ДействияПриНачалеРаботыСистемы.
Функция ВызванныеПроцедурыПередНачаломРаботыСистемы() Экспорт
	...
	ИменаКлиентскихМодулей = Неопределено;
	
	Строки = Новый Массив;
	Для Каждого ОписаниеПроцедуры Из Параметры.ВызванныеПроцедуры Цикл
		Если ТипЗнч(ОписаниеПроцедуры.Модуль) = Тип("ОбщийМодуль") Тогда
			Если ИменаКлиентскихМодулей = Неопределено Тогда
			...
			Для Каждого ИмяКлиентскогоМодуля Из ИменаКлиентскихМодулей Цикл
				...
				КонецПопытки;
				Если ТекущийМодуль = ОписаниеПроцедуры.Модуль Тогда
					ИмяМодуля = ИмяКлиентскогоМодуля;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли ТипЗнч(ОписаниеПроцедуры.Модуль) = Тип("ФормаКлиентскогоПриложения") Тогда
			ИмяМодуля = ОписаниеПроцедуры.Модуль.ИмяФормы;
		Иначе
			ИмяМодуля = Строка(ОписаниеПроцедуры.Модуль);
		КонецЕсли;
		Строки.Добавить("- " + Строка(ИмяМодуля) + "." + ОписаниеПроцедуры.Имя);
	КонецЦикла;
...
Процедура ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	Если Событие = "Запуск" И ПрекратитьРаботу Тогда
		ТекстПредупреждения = НСтр("ru = 'Запуск приложения невозможен:'")
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	ПоказатьОповещениеПользователя(Заголовок, 
		...
		Текст, БиблиотекаКартинок.ДиалогВосклицание, СтатусОповещенияПользователя.Важное);
КонецПроцедуры
...
Процедура ОповещениеТекущегоПользователяОПредстоящемПерезапуске(СекундДоПерезапуска) Экспорт
	...
	ПоказатьОповещениеПользователя(
		ТекстЗаголовка,
		"e1cib/app/ОбщаяФорма.КонтрольДинамическогоОбновления",
		ТекстСообщения, БиблиотекаКартинок.ДиалогВосклицание,
		СтатусОповещенияПользователя.Важное,
