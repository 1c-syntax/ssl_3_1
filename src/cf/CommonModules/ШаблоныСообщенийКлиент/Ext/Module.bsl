...
Процедура ОтправитьПисьмо(Ссылка, Параметры) Экспорт
	ДополнительныеПараметры = Новый Структура("ИмяФормыИсточникаСообщения", "");
	ДополнительныеПараметры.ИмяФормыИсточникаСообщения = Параметры.Форма.ИмяФормы;
	
	СформироватьСообщение(Ссылка, "Письмо",,, ДополнительныеПараметры);
КонецПроцедуры

Процедура ОтправитьSMS(Ссылка, Параметры) Экспорт
	ДополнительныеПараметры = Новый Структура("ИмяФормыИсточникаСообщения", "");
	ДополнительныеПараметры.ИмяФормыИсточникаСообщения = Параметры.Форма.ИмяФормы;
	
	СформироватьСообщение(Ссылка, "СообщениеSMS",,, ДополнительныеПараметры);
КонецПроцедуры
