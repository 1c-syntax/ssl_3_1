...
Функция ПараметрыОткрытияФормыОшибкиУстановкиМонопольногоРежима() Экспорт
	...
	ПараметрыОткрытияФормы.Вставить("Заголовок", "");
	ПараметрыОткрытияФормы.Вставить("ТекстСообщенияОбОшибке", "");
	ПараметрыОткрытияФормы.Вставить("ТекстСообщенияОбОшибкеЗавершенияРаботы", "");
	ПараметрыОткрытияФормы.Вставить("ЗавершатьСеансКонфигуратора", Ложь);
	ПараметрыОткрытияФормы.Вставить("ЗавершатьВсеСеансыКромеТекущего", Ложь);
	ПараметрыОткрытияФормы.Вставить("СообщениеПриВходеВПриложение", "");
	ПараметрыОткрытияФормы.Вставить("ПериодБлокировки", 300);
...
Процедура КонтрольРежимаЗавершенияРаботыПользователей(ТекущийРежим)
	...
	ИнтервалЗакрытьСЗапросом         = 60 + ?(РазделениеВключено, ИнтервалПрекратитьВМоделиСервиса, ИнтервалПрекратить);
	ИнтервалПредупреждения           = ?(ИнтервалПредупреждения - 60 > ИнтервалЗакрытьСЗапросом,
		ИнтервалПредупреждения, ИнтервалЗакрытьСЗапросом + 60);
	...
	Шаблон = НСтр("ru = 'Рекомендуется завершить текущую работу и сохранить все свои данные. Работа приложения будет завершена %1 в %2. 
		|%3'");
...
Процедура ЗавершитьРаботуПользователей(ТекущийРежим)
	...
	Если ТекущийМомент < ВремяНачалаБлокировки Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Завершение работы пользователей'"), 
			ОповещениеНажатия, ТекстСообщения, БиблиотекаКартинок.ДиалогИнформация);
		Возврат;
	КонецЕсли;
...
Процедура ЗавершитьРаботуЭтогоСеанса(ВыводитьВопрос = Истина)
	...
	ТекстСообщения = НСтр("ru = 'Работа пользователей с приложением запрещена. Завершить работу этого сеанса?'");
...
Процедура ПоказатьПредупреждениеПриЗавершенииРаботы(ТекстСообщения, ВремяНачалаБлокировки)
	...
	Если Не ПараметрыИнформирования.ОповещениеПоказано Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Работа приложения будет завершена'"),, ТекстСообщения,, 
	...
	Если ПараметрыИнформирования.ПредупреждениеПоказано Тогда
		Возврат;
	КонецЕсли;
	
	ПоказатьПредупреждение(, ТекстСообщения, 30);
	
	ПараметрыИнформирования.ПредупреждениеПоказано = Истина;
...
//   * ПредупреждениеПоказано - Булево
//   * ВопросПоказан - Булево
//
Функция ПараметрыИнформирования(ВремяНачалаБлокировки)
	...
	Если Свойства = Неопределено Или Свойства.ВремяНачалаБлокировки <> ВремяНачалаБлокировки Тогда
		...
		Свойства.Вставить("ПредупреждениеПоказано", Ложь);
		Свойства.Вставить("ВопросПоказан", Ложь);
...
Процедура ЗадатьВопросПриЗавершенииРаботы(ТекстСообщения, ВремяНачалаБлокировки)
	...
	Если ПараметрыИнформирования.ВопросПоказан Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыИнформирования.ВопросПоказан = Истина;
...
Функция ОбработатьПараметрыЗапуска(Знач ПараметрыЗапуска)
	...
	// Обработка параметров запуска ЗапретитьРаботуПользователей и РазрешитьРаботуПользователей.
	...
	Если КлючСодержитсяВПараметрахЗапуска(ПараметрыЗапуска, ИмяПараметраРазрешитьРаботуПользователей) Тогда
		...
		Если Не ОбщегоНазначенияКлиент.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр запуска %1 не отработан. Не выполнен вход в область данных.'"),
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выполнен запуск с параметром %1. Работа приложения будет завершена.'"),
				ИмяПараметраРазрешитьРаботуПользователей);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),,
