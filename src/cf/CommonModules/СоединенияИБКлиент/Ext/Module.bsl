...
Процедура ПередПериодическойОтправкойДанныхКлиентаНаСервер(Параметры) Экспорт
	...
	ИмяПараметраПриложения = "СтандартныеПодсистемы.ПараметрыЗавершенияРаботыПользователей";
	ПараметрыКлиента = ПараметрыПриложения[ИмяПараметраПриложения];
	Если ПараметрыКлиента = Неопределено Тогда
		ПараметрыКлиента = Новый Структура;
		ПараметрыПриложения.Вставить(ИмяПараметраПриложения, ПараметрыКлиента);
	КонецЕсли;
	ТекущаяДатаСеанса = ОбщегоНазначенияКлиент.ДатаСеанса();
	Если Не ПараметрыКлиента.Свойство("ДатаПоследнейПериодическойПроверки") Тогда
		ПараметрыКлиента.Вставить("ДатаПоследнейПериодическойПроверки", '00010101');
	КонецЕсли;
	
	Если ПараметрыКлиента.ДатаПоследнейПериодическойПроверки + 60 > ТекущаяДатаСеанса Тогда
		Возврат;
	КонецЕсли;
	ПараметрыКлиента.ДатаПоследнейПериодическойПроверки = ТекущаяДатаСеанса;
	...
	ИначеЕсли ВключенКонтрольЗавершенияРаботыПользователей() Тогда
		Параметры.Вставить(ИмяПараметра, Ложь);
	КонецЕсли;
...
Процедура УстановитьКонтрольЗавершенияРаботыПользователей(Значение = Истина)
	...
	ПараметрыПриложения["СтандартныеПодсистемы.ПараметрыЗавершенияРаботыПользователей"].Вставить("ВыполнятьКонтрольЗавершенияРаботыПользователей", Значение);
...
Функция ВключенКонтрольЗавершенияРаботыПользователей()
	...
	Возврат ТипЗнч(ПараметрыЗавершенияРаботыПользователей) = Тип("Структура")
		И ПараметрыЗавершенияРаботыПользователей.Свойство("ВыполнятьКонтрольЗавершенияРаботыПользователей")
		И ПараметрыЗавершенияРаботыПользователей.ВыполнятьКонтрольЗавершенияРаботыПользователей;
...
Процедура УстановитьПроцедураЗавершитьРаботуПользователейУжеВыполняется(Значение = Истина)
	...
	ПараметрыПриложения["СтандартныеПодсистемы.ПараметрыЗавершенияРаботыПользователей"].Вставить("ПроцедураЗавершитьРаботуПользователейУжеВыполняется", Значение);
...
Функция ПроцедураЗавершитьРаботуПользователейУжеВыполняется()
	...
	Возврат ТипЗнч(ПараметрыЗавершенияРаботыПользователей) = Тип("Структура")
		И ПараметрыЗавершенияРаботыПользователей.Свойство("ПроцедураЗавершитьРаботуПользователейУжеВыполняется")
		И ПараметрыЗавершенияРаботыПользователей.ПроцедураЗавершитьРаботуПользователейУжеВыполняется;
...
Функция ПараметрыИнформирования(ВремяНачалаБлокировки)
	...
	Иначе
		Свойства.ОповещениеПоказано = Истина;
	КонецЕсли;
