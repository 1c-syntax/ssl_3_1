...
Процедура ПродолжитьУстановкуСпискаОтзываПослеСкачивания(РезультатВыполнения, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваСпискаОтзыва.ДатаСледующейПубликации) И СвойстваСпискаОтзыва.ДатаСледующейПубликации < ОбщегоНазначенияКлиент.ДатаУниверсальная() Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Дата следующей публикации списка отзыва %1 просрочена. Обратитесь в удостоверяющий центр, выдавший сертификат.'"), Результат.АдресФайла), Результат, Истина);
		Возврат;
	КонецЕсли;
