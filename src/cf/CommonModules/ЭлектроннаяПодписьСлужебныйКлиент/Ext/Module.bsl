...
Процедура ОткрытьСертификатЗавершение(Контекст)
	...
	ПараметрыФормы.Вставить("ОшибкаНаКлиенте",  Контекст.ОшибкаНаКлиенте);
...
Процедура ВыполнитьНаСторонеЦиклПослеПолученияДанных(Результат, Контекст) Экспорт
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig Тогда
			...
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					НСтр("ru = 'Внешняя компонента XMLDSig может использоваться только для подписания.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
				...
				Если Результат.ПараметрыCMS.ВключениеСертификатовВПодпись <> РежимВключенияСертификатовКриптографии.ВключатьСертификатСубъекта Тогда
					Ошибка = Новый Структура("ОписаниеОшибки",
						НСтр("ru = 'Подписание облачным сертификатом недоступно.
						           |Необходимо выбрать другой сертификат из установленных на компьютере.'"));
					ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
					Возврат;
				КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеОперацииНаКлиенте(ДвоичныеДанные, Контекст) Экспорт
	...
	ИначеЕсли Контекст.Свойство("МенеджерКриптографии") И Контекст.МенеджерКриптографии = "ОблачнаяПодпись" Тогда
		...
		Если Не ДвоичныеДанные.Выполнено Тогда
			...
			Иначе	
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ДвоичныеДанные.Ошибка);
				ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
			КонецЕсли;	
			Возврат;
		КонецЕсли;
...
Процедура НачатьПроверкуПодписиПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст,
		?(Контекст.ТипПодписи = "CMS",
			НСтр("ru = 'Для проверки подписи CMS необходима установка компоненты XMLDSig'"),
			НСтр("ru = 'Для проверки подписи XML необходима установка компоненты XMLDSig'")));
...
Процедура ПослеПодключенияКомпонентыПослеОшибкиУстановкиСоответствиеOID(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ЗавершитьОперациюСОшибкой(Контекст,
		НСтр("ru = 'При установке свойства СоответствиеOID компоненты XMLDSig произошла ошибка:'")
...
Процедура НачатьПодписание_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст,
		НСтр("ru = 'Для подписания XML необходима установка компоненты XMLDSig'"));
...
Процедура НачатьПодписаниеCMS_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст,
		НСтр("ru = 'Для подписания CMS необходима установка компоненты XMLDSig'"));
...
Процедура НачатьПодписаниеCMS(Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода CMSSign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура НачатьПроверкуПодписиCMS(Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода CMSVerifySign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеКанонизации_ПодписываемыйТег(Результат, Контекст) Экспорт
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода Hash компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеКанонизации_SignedInfo(Результат, Контекст) Экспорт
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода Sign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Проверка_ПослеКанонизации_SignedInfo(Результат, Контекст) Экспорт
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода VerifySign компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура C14N(Оповещение, ОбъектКомпоненты, КонвертXML, XPath)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода C14N компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура КанонизироватьТекстXMLЦиклНачало(Контекст)
	...
	ИначеЕсли Алгоритм.Вид = "c14n"
		...
		Если Алгоритм.Вид = "c14n" Тогда
			...
			Исключение
				ЗавершитьОперациюСОшибкой(Контекст,
					НСтр("ru = 'Ошибка вызова метода C14N_body компоненты XMLDSig.'")
					+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		Иначе
			Оповещение = Новый ОписаниеОповещения(
			...
			Исключение
				ЗавершитьОперациюСОшибкой(Контекст,
					НСтр("ru = 'Ошибка вызова метода TransformSMEV компоненты XMLDSig.'")
					+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЕсли;
	Иначе
...
Процедура НачатьПолучениеТекстаОшибки(НачалоОписанияТекстаОшибки, Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода GetLastError компоненты XMLDSig.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
		...
		Если НашлиСертификат.Идентификатор = -1 Тогда
			Ошибка = Новый Структура("ОписаниеОшибки",
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция) + Символы.ПС
					+ НСтр("ru = 'Сертификат отсутствует в сервисе ""Облачная подпись"" (возможно удален).'"));
			Контекст.Вставить("ОперацияНачалась", Ложь);
			ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьОперациюОблачнойПодписи(Оповещение, Контекст, Данные, ПараметрыОперации)
	...
	Если Контекст.Операция = "Подписание" Тогда
		...
		Если ДанныеПодтверждения <> Неопределено Тогда
			ИдентификаторТранзакции = ДанныеПодтверждения.ИдентификаторТранзакции;
	...
	ИначеЕсли Контекст.Операция = "Расшифровка" Тогда
		...
		Если ДанныеПодтверждения <> Неопределено Тогда
			ТипРезультата = ДанныеПодтверждения.ПараметрыВариантаВстроенного.ТипРезультата;
			Если ЗначениеЗаполнено(ДанныеПодтверждения.ИдентификаторТранзакции)
				И ИндексДанных > 0 Тогда
				ВыполнитьСразу = Ложь;
			КонецЕсли;
			ПараметрыОперации.Вставить("ИдентификаторТранзакции", ДанныеПодтверждения.ИдентификаторТранзакции);
		КонецЕсли;
...
Процедура ПроверитьСертификатПослеПроверкиОблачнаяПодпись(РезультатВызова, Контекст) Экспорт
	...
	Если Не РезультатВызова.Выполнено Тогда
		Контекст.ОписаниеОшибкиНаСервере = ТекстОшибкиОблачнаяПодписьНастройкиОперации(РезультатВызова.Ошибка);
		ПроверитьСертификатПослеНеуспешнойПроверки(Контекст);
		Возврат;
	КонецЕсли;
	
	Если Не РезультатВызова.Результат Тогда
		Контекст.ОписаниеОшибкиНаСервере = ТекстОшибкиОблачнаяПодписьСертификатНедействителен();
		ПроверитьСертификатПослеНеуспешнойПроверки(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписиОблачнаяПодпись(РезультатВызова, Контекст) Экспорт
	...
	Если Не РезультатВызова.Выполнено Тогда
		ПроверитьПодписьНаКлиентеПослеОшибкиПроверкиПодписи(
					ТекстОшибкиОблачнаяПодписьНастройкиОперации(РезультатВызова.Ошибка), Ложь, Контекст);
		Возврат;
	КонецЕсли;
	
	Если Не РезультатВызова.Результат Тогда
		ПроверитьПодписьНаКлиентеПослеОшибкиПроверкиПодписи(
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ТекстОшибкиСервисаПодписьНедействительна(РезультатВызова.Ошибка),
				Ложь, Контекст);
		Возврат;
	КонецЕсли;
...
Функция ТекстОшибкиОблачнаяПодписьСертификатНедействителен()
	
	Возврат НСтр("ru = 'Сервис облачной подписи сообщил, что сертификат недействителен.'");
	
КонецФункции

Функция ТекстОшибкиОблачнаяПодписьНастройкиОперации(ТекстОшибки)
	
	Результат = НСтр("ru = 'Не удалось выполнить операцию на сервисе облачной подписи.'");
...
Процедура ПослеСбораТехническойИнформации(ИнформацияОПрограммах, Контекст) Экспорт
