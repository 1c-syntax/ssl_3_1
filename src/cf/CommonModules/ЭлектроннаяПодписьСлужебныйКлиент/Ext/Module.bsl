...
Процедура СохранитьЗапросНаСертификат(Оповещение, ЗапросНаСертификат, ИмяФайлаБезРасширения = "") Экспорт
	...
	Контекст = Новый Структура("Оповещение, АдресСертификата", Оповещение, Неопределено);
	
	Если ТипЗнч(ЗапросНаСертификат) = Тип("ДвоичныеДанные") Тогда
		Контекст.АдресСертификата = ПоместитьВоВременноеХранилище(ЗапросНаСертификат, Новый УникальныйИдентификатор);
		Контекст.Вставить("ОчищатьХранилище", Истина);
	ИначеЕсли ТипЗнч(ЗапросНаСертификат) = Тип("Строка")
		И ЭтоАдресВременногоХранилища(ЗапросНаСертификат) Тогда
		
		Контекст.АдресСертификата = ЗапросНаСертификат;
	Иначе
	...
	ОповещениеОСохранении = Новый ОписаниеОповещения("СохранитьЗапросНаСертификатПослеПолученияФайлов", ЭтотОбъект, Контекст);
	...
	ФайловаяСистемаКлиент.СохранитьФайл(ОповещениеОСохранении, Контекст.АдресСертификата, ИмяФайла, ПараметрыСохранения);
