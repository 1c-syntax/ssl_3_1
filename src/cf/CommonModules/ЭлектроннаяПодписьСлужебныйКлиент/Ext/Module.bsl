...
Асинх Процедура ДиагностическаяИнформацияОПрограммахОпределенныхАвтоматически(Оповещение)
	...
	Если ИспользуемыеПрограммыРезультат.Криптопровайдеры.Количество() = 0 Тогда
		ВыполнитьОбработкуОповещения(Оповещение, НСтр("ru = 'На компьютере нет приложений электронной подписи и шифрования.'"));
		Возврат;
	КонецЕсли;
	
	Заголовок = Символы.ПС + НСтр("ru = 'Автоматически определенные приложения электронной подписи и шифрования на компьютере:'") + Символы.ПС;
...
Асинх Процедура УдалитьСертификатИзЛокальногоХранилищаПродолжение(МенеджерКриптографии, ДанныеСертификата) Экспорт
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Сертификат удален из личного хранилища компьютера. Выполните проверку заново.'"));
