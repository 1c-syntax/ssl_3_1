...
Процедура ПроверитьСертификатПослеНеуспешнойПроверки(Контекст)
	...
	Иначе
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			Новый Структура("ОписаниеОшибкиНаКлиенте, ОписаниеОшибкиНаСервере", 
				Контекст.ОписаниеОшибкиНаКлиенте, Контекст.ОписаниеОшибкиНаСервере));
	КонецЕсли;
			...
			ИначеЕсли Контекст.Операция = "Подписание" Тогда
				...
				Иначе
					Если ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись Тогда
						НастройкиПодписи = ПараметрыПодписи(Контекст.ОписаниеДанных.ТипПодписи);
						...
						КонецЕсли;
						МенеджерКриптографии.НачатьПодписывание(Оповещение, Данные, Контекст.СертификатКриптографии, НастройкиПодписи.ТипПодписи);
					Иначе
...
Процедура ВыполнитьНаСторонеЦиклПослеОперацииНаКлиенте(ДвоичныеДанные, Контекст) Экспорт
	...
	Если Контекст.Операция = "Подписание" Тогда 
		...
		Иначе 
			СвойстваПодписи = ПолучитьСвойстваПодписиПослеПодписания(ДвоичныеДанные, Контекст);
			ВыполнитьНаСторонеЦиклПослеПодписания(СвойстваПодписи, Контекст);
		КонецЕсли;
	ИначеЕсли Контекст.Операция = "Шифрование" Тогда
...
Функция ПолучитьСвойстваПодписиПослеПодписания(ДвоичныеДанные, Контекст)
	
	Если ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись
		И ТипЗнч(Контекст.МенеджерКриптографии) = Тип("МенеджерКриптографии") Тогда
		...
		ПараметрыПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыПодписиКриптографии(
			ДвоичныеДанные, Контекст.МенеджерКриптографии, ДобавкаВремени, ДатаСеанса);
	Иначе
...
Функция ПолучитьСвойстваПодписиПослеУсовершенствования(ДвоичныеДанные, Контекст)
	...
	СвойстваПодписи.Вставить("Подпись",             ДвоичныеДанные);
	СвойстваПодписи.Вставить("ДатаПодписи",         ПараметрыПодписи.ДатаПодписиИзМетки);
	СвойстваПодписи.Вставить("ДатаПроверкиПодписи", ПараметрыПодписи.ДатаПоследнейМеткиВремени);
...
Процедура УсовершенствоватьНаСторонеЦиклНачалоПродолжение(Результат, Контекст) Экспорт
	...
	Если ЭлементДанных.Свойство("НовыеСвойстваПодписи") Тогда
		УсовершенствоватьНаСторонеЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если ТипЗнч(Подпись) = Тип("ДвоичныеДанные") Тогда
			Подпись = ПоместитьВоВременноеХранилище(Подпись, Контекст.ИдентификаторФормы);
		КонецЕсли;
	КонецЕсли;
	...
	ЭлементДанныхДляСервера = Новый Структура;
	ЭлементДанныхДляСервера.Вставить("Подпись", ЭлементДанных);
	...
	Если Контекст.ЭлементДанных.Свойство("ПодписанныйОбъект") Тогда
		Если Контекст.ЭлементДанных.Свойство("ВерсияОбъекта") Тогда
			ЭлементДанныхДляСервера.Свойство("ВерсияОбъекта", Контекст.ЭлементДанных.ВерсияОбъекта);
		КонецЕсли;
	КонецЕсли;
	
	Успех = ЭлектроннаяПодписьСлужебныйВызовСервера.УсовершенствоватьНаСторонеСервера(ПараметрыДляСервера,
		АдресРезультата, Контекст.ОперацияНачалась, ОшибкаНаСервере);
	
	Если Не Успех Тогда
		УсовершенствоватьНаСторонеПослеОшибки(ОшибкаНаСервере, Контекст);
	Иначе
		ВыполнитьНаСторонеЦиклПослеУсовершенствования(АдресРезультата, Контекст);
	КонецЕсли;
...
Процедура УсовершенствоватьНаСторонеПослеСозданияМенеджераКриптографии(МенеджерКриптографии, Контекст) Экспорт
	...
	ПараметрыПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыПодписиКриптографии(
		Подпись, МенеджерКриптографии, ДобавкаВремени, ДатаСеанса);
		...
		ВыполнитьОбработкуОповещения(Оповещение, ПараметрыПодписи);
		Возврат;
	КонецЕсли;
	
	Если ПараметрыПодписи.ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdESAv3")
...
Процедура УсовершенствоватьНаСторонеЦиклПослеОперацииНаКлиенте(ДвоичныеДанные, Контекст) Экспорт
	...
	Контекст.ОперацияНачалась = Истина;
	СвойстваПодписи = ПолучитьСвойстваПодписиПослеУсовершенствования(ДвоичныеДанные, Контекст);
...
Процедура ВыполнитьНаСторонеЦиклПослеУсовершенствования(СвойстваПодписи, Контекст)
	...
	ЭлементДанных = Контекст.ЭлементыДанных[Контекст.Индекс];
	НовыеСвойстваПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваПодписи();
	...
	Если Контекст.ЭлементДанных.Свойство("НеТребуетсяПродление") Тогда 
		Если ЭлементДанных.ТипПодписи <> СвойстваПодписи.ТипПодписи Тогда
			...
			ЭлементДанных.Вставить("Ошибка", НСтр("ru='Продление не требуется или не может быть осуществлено'"));
		КонецЕсли;
	...
	Если Контекст.НаСторонеКлиента И ОбновитьПодпись Тогда
...
Процедура УсовершенствоватьПослеВыполненияНаСторонеСервера(Результат, Контекст) Экспорт
	...
	Иначе
		Если Результат <> Неопределено Тогда
			...
			Если Контекст.ВыполнятьНаСервере = Истина Тогда
				РезультатВыполнения = Новый Структура("Ошибка, СвойстваПодписей, ТекстОшибки",
					Истина, Результат.ЭлементыДанных, Результат.ТекстОшибки);
				УсовершенствоватьПослеВыполненияНаСторонеКлиента(РезультатВыполнения, Контекст);
				Возврат;
			КонецЕсли;
...
Функция ПараметрыПодписи(ТипПодписи)
	...
	Результат.ТипПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ТипПодписиКриптографии(ТипПодписи);
	
	Если ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdEST") 
		Или ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdESC")
		Или ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdESXLongType2")
		Или ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdESAv3") Тогда 
		...
		Если АдресаСерверовМетокВремени.Количество() = 0 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для подписания в формате %1 заполните адреса серверов штампов времени.'"), ТипПодписи);
		КонецЕсли;
		Результат.АдресаСерверовМетокВремени = АдресаСерверовМетокВремени;
		
	ИначеЕсли ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CAdESBES")
		И ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.CMS") Тогда
		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Тип подписи криптографии %1 не поддерживается.'"), ТипПодписи);
	КонецЕсли;
