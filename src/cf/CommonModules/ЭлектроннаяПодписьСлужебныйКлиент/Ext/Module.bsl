...
Процедура ПроверитьПодпись(Оповещение, ИсходныеДанные, Подпись, МенеджерКриптографии = Неопределено, НаДату = Неопределено, ПоказатьОшибку = Истина) Экспорт
	...
	ИначеЕсли ТипЗнч(Контекст.ИсходныеДанные) = Тип("Структура")
		...
		ДанныеСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатИзКонвертаSOAP(
			Контекст.ИсходныеДанные.КонвертSOAP, Ложь);
		Контекст.Вставить("АлгоритмПодписи",
...
Процедура ПроверитьПодписьПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
		...
		ПроверитьПодписьВМоделиСервиса(Контекст);
	ИначеЕсли ЭлектроннаяПодписьСлужебныйВызовСервера.НастроенСервисОблачнойПодписи() Тогда
		// Проверка подписи в сервисе облачной подписи.
		ПроверитьПодписьОблачнаяПодпись(Контекст);
	Иначе
...
Процедура НачатьИнициализациюСертификатаКриптографииДляПроверкиПодписиXMLDSig(ОповещенияПриЗавершении, КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии)
	...
	СертификатКриптографииBase64 = ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатИзКонвертаSOAP(КонвертSOAP);
...
Процедура ПослеПодключенияКомпонентыНастроитьКомпоненту(Контекст)
	...
	Контекст.ОбъектКомпоненты.НачатьУстановкуСоответствиеOID(Новый ОписаниеОповещения(
...
Процедура НачатьПроверкуПодписиCMS(Контекст)
	...
	Попытка
		...
		Контекст.ОбъектКомпоненты.НачатьВызовCMSVerifySign(
			ОписаниеОповещения,
			Контекст.Подпись,
			ПараметрыCMSSign.Открепленная,
			Контекст.Данные,
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru = 'Ошибка вызова метода CMSVerifySign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru = 'Подпись неверна (SignatureValue некорректно).'"));
...
Процедура Проверка_ПослеВыполненияHash_ПодписываемыйТег(HashValue, Параметры, Контекст) Экспорт
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru = 'Подпись неверна (SignatureValue корректно, отличается DigestValue).'"));
...
Процедура ПроверитьСертификатПослеПроверкиОблачнаяПодпись(РезультатВызова, Контекст) Экспорт
	...
	Если Не РезультатВызова.Выполнено Тогда
		Контекст.ОписаниеОшибкиНаСервере = КраткоеПредставлениеОшибки(РезультатВызова.Ошибка);
		Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			ПроверитьСертификатВМоделиСервиса(Контекст);
		Иначе	
			ПроверитьСертификатПослеНеуспешнойПроверки(Контекст);
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписиОблачнаяПодпись(РезультатВызова, Контекст) Экспорт
	...
	Если Не РезультатВызова.Выполнено Тогда
		Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			ПроверитьПодписьВМоделиСервиса(Контекст);
		Иначе	
			ПроверитьПодписьНаКлиентеПослеОшибкиПроверкиПодписи(РезультатВызова.Ошибка, Ложь, Контекст);
		КонецЕсли;	
		Возврат;
	КонецЕсли;
