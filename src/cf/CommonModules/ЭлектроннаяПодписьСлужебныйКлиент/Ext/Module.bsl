...
Процедура НайтиУстановленныеПрограммыЦиклНачало(Контекст)
	...
	Если Контекст.Программы.Количество() <= Контекст.Индекс + 1 Тогда
		...
		Если Контекст.ПроверятьНаСервере Тогда
				Контекст.ПараметрыВыполнения.Вставить("Менеджер", Неопределено);
				Контекст.ПараметрыВыполнения.Вставить("Оповещение", Неопределено);
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Оповещение) Экспорт
	...
	Если Оповещение <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(Оповещение, ЕстьПолученныеФайлы);
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиенте(Контекст)
	...
	Иначе
		Если Контекст.МенеджерКриптографии = "СервисКриптографии" Тогда
			ПроверитьСертификатВМоделиСервиса(Контекст.МенеджерКриптографии, Контекст);
		Иначе
...
Процедура СохранитьДанныеСертификатаВместеСПодписьюЦиклВнутреннийЦиклНачало(Контекст)
	...
	ИначеЕсли Контекст.Подключено Тогда
		Файл = Новый Файл;
		Файл.НачатьИнициализацию(Новый ОписаниеОповещения(
				"СохранитьДанныеСертификатаВместеСПодписьюЦиклВнутреннийЦиклПослеИнициализацииФайла", ЭтотОбъект, Контекст),
			Контекст.ПолноеИмяФайлаСертификата);
	Иначе
...
// Продолжение процедуры ЭлектроннаяПодписьКлиент.СохранитьДанныеВместеСПодписью.
Процедура СохранитьДанныеСертификатаВместеСПодписьюЦиклВнутреннийЦиклПослеИнициализацииФайла(Файл, Контекст) Экспорт
		...
		Иначе
			...
			Если Контекст.Операция = "Подписание" Тогда
				Если МенеджерКриптографии = "СервисКриптографии" Тогда
					...
					МодульСервисКриптографииКлиент.Подписать(Оповещение, Данные, СертификатДляПодписи);
				Иначе
			...
			ИначеЕсли Контекст.Операция = "Шифрование" Тогда
				Если МенеджерКриптографии = "СервисКриптографии" Тогда
					МодульСервисКриптографииКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииКлиент");
					МодульСервисКриптографииКлиент.Зашифровать(Оповещение, Данные, Контекст.СертификатыШифрования);
				Иначе
			...
			Иначе
				Если МенеджерКриптографии = "СервисКриптографии" Тогда
					МодульСервисКриптографииКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииКлиент");
					МодульСервисКриптографииКлиент.Расшифровать(Оповещение, Данные);
				Иначе
...
Процедура ПослеУстановкиСвойстваЗапретитьПользовательскийИнтерфейс(Контекст) Экспорт
	...
	Если Контекст.Режим = "РежимПроверка" Тогда
		НачатьПроверкуПодписиСообщенияSOAP(Контекст);
		
	ИначеЕсли Контекст.Режим = "РежимПодписание" Тогда
		НачатьПодписаниеСообщенияSOAP(Контекст);
	Иначе
		ЗавершитьОперациюСОшибкой(
			Контекст,
...
Процедура НачатьПодписаниеСообщенияSOAP(Контекст)
...
Процедура НачатьПроверкуПодписиСообщенияSOAP(Контекст)
...
// Продолжение процедуры ПроверитьСертификат.
Процедура ПроверитьСертификатВМоделиСервиса(Результат, Контекст)
...
// Продолжение процедуры ВыполнитьНаСтороне.
Процедура ВыполнитьНаСторонеПослеВыгрузкиСертификатаВМоделиСервиса(ВыгруженныеДанные, Контекст) Экспорт
	
	Контекст.Вставить("СвойстваСертификата", ЭлектроннаяПодписьКлиент.СвойстваСертификата(
		ВыгруженныеДанные.Сертификат));
	Контекст.СвойстваСертификата.Вставить("ДвоичныеДанные", ВыгруженныеДанные.Сертификат.Сертификат);
