		...
		Если Не ОбщегоНазначенияКлиент.РазделениеВключено()
			...
			Если СвойстваПодписи.ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES")
				...
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: Для проверки подписей этого типа требуется версия платформы 1С:Предприятие выше 8.3.21.1302.'"),
...
Процедура ПроверитьПодписьНаКлиентеПослеПолученияКонтейнераПодписи(КонтейнерПодписи, Контекст) Экспорт
	...
	ДатаПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДатаДляПроверкиПодписи(ПараметрыПодписи);
...
Асинх Функция ОбъектВнешнейКомпонентыExtraCryptoAPI(ПредложитьУстановить = Истина)
...
Асинх Функция ПрограммаПоСертификату(Сертификат,
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		Попытка
			ОбъектКомпоненты = Ждать ОбъектВнешнейКомпонентыExtraCryptoAPI(ПредложитьУстановить);
		Исключение
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайла(ПараметрыСоздания, МножественныйВыбор = Ложь) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru = 'Выберите файл(ы) сертификата (только для шифрования)'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзКаталога(ПараметрыСоздания)
	...
	ТекстПредложения =  НСтр("ru = 'Для поиска файлов в каталоге требуется расширение для работы с файлами'");
...
Процедура ВыполнитьНаСторонеЦиклПослеОшибкиПолученияКонтейнераПодписиНаКлиенте(ИнформацияОбОшибке, СтандартнаяОбработка, 
	...
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|Результат подписи: %2'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), ДанныеПодписи);
...
Процедура Проверка_ПослеВыполненияHash_ПодписываемыйТег(HashValue, Параметры, Контекст) Экспорт
	...
	Проверка_ПослеВыполненияHash_ПодписываемыйТегПослеПолученияДатыПодписания(Неопределено, Контекст);
...
Асинх Процедура УстановитьСписокОтзываПослеСозданияВременногоКаталога(ИмяВременногоКаталога, Контекст) Экспорт
	...
	ИмяХранилища = "CA";
	РезультатУстановки = Ждать Контекст.ОбъектКомпоненты.ImportCRuАсинх(Контекст.ИмяФайлаСпискаОтзыва, ИмяХранилища);
...
Процедура УстановитьVipNet(Форма, ОповещениеОЗавершении, ВходящийКонтекст) Экспорт
	...
	Если СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86
		...
		Если СтрНайти(СистемнаяИнформация.ВерсияОС, "Windows 10") <> 0
			Или СтрНайти(СистемнаяИнформация.ВерсияОС, "Windows Server 2016") <> 0 Тогда
				
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Работа %1 на операционной системе %2 не поддерживается. Подробности на сайте %3.'"),
				ЭлектроннаяПодписьКлиентСерверЛокализация.ИмяПрограммыVipNet(), СистемнаяИнформация.ВерсияОС,
				"www.infotecs.ru");
...
Процедура УстановитьКриптопровайдерПослеПолученияПодтвержденияНаУстановку(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат = Истина Тогда
		ЗапуститьПриложениеНаКлиенте(
			ВходящийКонтекст.ВременныйКаталог + "Setup.bat", ВходящийКонтекст.ВременныйКаталог,
