...
Процедура ДополнительнаяПроверкаСертификата(СертификатКриптографии, ПроверкаНаСервере, Контекст)
	...
	Если Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра Тогда
		...
		Если ЗначениеЗаполнено(РезультатПроверкиУдостоверяющегоЦентраСертификата.Предупреждение.ТекстОшибки) И Не Контекст.ДляПроверкиПодписи Тогда
			СсылкаНаСертификат = Неопределено;
			Если Не ЭлектроннаяПодписьСлужебныйВызовСервера.ПользовательОповещенОСертификате(СертификатКриптографии.Отпечаток, СсылкаНаСертификат)
				И СсылкаНаСертификат <> Неопределено Тогда
				...
				ПоказатьОповещениеПользователя(
					НСтр("ru = 'Необходима замена сертификата'"), ДействиеПриНажатии,
					СсылкаНаСертификат,
			...
			ИначеЕсли Контекст.Операция = "Подписание" Тогда
				...
				Иначе
					Если ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись Тогда
						НастройкиПодписи = НастройкиПодписания(Контекст.ОписаниеДанных.ТипПодписи);
	...
	Если Контекст.Операция = "Подписание" Тогда 
		...
		Иначе
			Если Контекст.Свойство("МенеджерКриптографии") И ТипЗнч(Контекст.МенеджерКриптографии) = Тип("МенеджерКриптографии") 
				...
				Возврат;
			ИначеЕсли Контекст.Свойство("МенеджерКриптографии") И Контекст.МенеджерКриптографии = "ОблачнаяПодпись"
				И ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись Тогда
				ПараметрыПодписиКриптографии = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеПараметрыПодписиКриптографии();
				ПараметрыПодписиКриптографии.ТипПодписи = Контекст.ОписаниеДанных.ТипПодписи;
			Иначе
...
Функция НастройкиПодписания(ТипПодписи)
...
Процедура ВыполнитьНаСторонеЦиклПослеПодписанияПакета(МассивПодписей, Контекст)
	...
	Если Контекст.Свойство("МенеджерКриптографии") И Контекст.МенеджерКриптографии = "ОблачнаяПодпись"
		И ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись Тогда
		ПараметрыПодписиКриптографии = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеПараметрыПодписиКриптографии();
		ПараметрыПодписиКриптографии.ТипПодписи = Контекст.ОписаниеДанных.ТипПодписи;
	Иначе
