...
Процедура ОткрытьТокен(Токен, ВладелецФормы, ОбработкаЗавершения = Неопределено) Экспорт
	...
	ФормаПередачаПараметров().ОткрытьНовуюФорму("Токен",
...
Асинх Функция РезультатПроверкиУдостоверяющегоЦентраСертификата(СертификатКриптографии,  НаДату = Неопределено, ПараметрыПроверки = Неопределено, СвойстваСертификата = Неопределено) Экспорт
	...
	КонтекстПроверки.РазницаСУниверсальнымВременем = РазницаСУниверсальнымВременем();
...
Процедура СообщитьОбОшибкеДобавленияСертификата(КонтекстДобавления)
	...
	Если ЗначениеЗаполнено(ОшибкаНаКлиенте) И ОшибкаНаКлиенте.Ошибки.Количество() > 0
		...
		ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыОшибки, ЭтотОбъект);
...
Процедура ПодписатьОблачнаяПодпись(Оповещение, Контекст, Данные, ПараметрыОперации, ТипРезультата = Неопределено)
	...
	ИначеЕсли ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись И ЗначениеЗаполнено(Контекст.ОписаниеДанных.ТипПодписи) Тогда
...
Процедура ДополнитьРешениеКлассификатораОшибокПодробностями(
	...
	Если ПараметрыДополненияТекстаРешенияОшибкиКлассификатора.ПроверитьСертификатВЛокальномХранилищеКлиента
		И ЗначениеЗаполнено(ДанныеДляДополнения.ДанныеСертификата) Тогда
		
		Контекст = Новый Структура;
		Контекст.Вставить("ОповещениеОЗавершении", Оповещение);
		Контекст.Вставить("ОшибкаИзКлассификатора", ОшибкаИзКлассификатора);
		Контекст.Вставить("ДанныеСертификата", ДанныеДляДополнения.ДанныеСертификата);
		Контекст.Вставить("ПараметрыДополненияТекстаРешенияОшибкиКлассификатора", ПараметрыДополненияТекстаРешенияОшибкиКлассификатора);
		ЭлектроннаяПодписьКлиент.СоздатьМенеджерКриптографии(
			Новый ОписаниеОповещения("ПослеСозданияМенеджераКриптографииДляДополненияКлассификатораОшибок", ЭтотОбъект, Контекст),
			"ПолучениеСертификатов");
	КонецЕсли;
	
КонецПроцедуры

Асинх Процедура ПослеСозданияМенеджераКриптографииДляДополненияКлассификатораОшибок(МенеджерКриптографии, Контекст) Экспорт
	
	Если ТипЗнч(МенеджерКриптографии) <> Тип("МенеджерКриптографии") Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОповещениеОЗавершении, Контекст.ОшибкаИзКлассификатора);
		Возврат;
	КонецЕсли;
	
#Если Не МобильноеПриложениеКлиент И Не МобильныйКлиент Тогда
		
	ДанныеСертификата = Контекст.ДанныеСертификата;
	
	Попытка
		СертификатКриптографии = Новый СертификатКриптографии;
		Ждать СертификатКриптографии.ИнициализироватьАсинх(ДанныеСертификата);

		ХранилищеЛокальногоКомпьютера = Ждать МенеджерКриптографии.ПолучитьХранилищеСертификатовАсинх(
			ТипХранилищаСертификатовКриптографии.ПерсональныеСертификаты,
			РасположениеХранилищаСертификатовКриптографии.ДанныеКомпьютера);
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись.Диагностика ошибок'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Ошибка", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВыполнитьОбработкуОповещения(Контекст.ОповещениеОЗавершении, Контекст.ОшибкаИзКлассификатора);
		Возврат;
	КонецПопытки;
	
	Если Ждать ХранилищеЛокальногоКомпьютера.НайтиПоОтпечаткуАсинх(СертификатКриптографии.Отпечаток) = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОповещениеОЗавершении, Контекст.ОшибкаИзКлассификатора);
		Возврат;
	КонецЕсли;
	
	Контекст.ОшибкаИзКлассификатора.Решение = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
		ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатВЛокальномХранилищеТекстРешения());
	Контекст.ОшибкаИзКлассификатора.Причина = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
		ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатВЛокальномХранилищеТекстПричины());
...
Процедура УдалитьСертификатИзЛокальногоХранилища(ДанныеСертификата)
	
	ЭлектроннаяПодписьКлиент.СоздатьМенеджерКриптографии(
			Новый ОписаниеОповещения("УдалитьСертификатИзЛокальногоХранилищаПродолжение", ЭтотОбъект, ДанныеСертификата),
			"ПолучениеСертификатов");
	
КонецПроцедуры

Асинх Процедура УдалитьСертификатИзЛокальногоХранилищаПродолжение(МенеджерКриптографии, ДанныеСертификата) Экспорт
	...
	СертификатКриптографии = Новый СертификатКриптографии;
	Ждать СертификатКриптографии.ИнициализироватьАсинх(ПолучитьИзВременногоХранилища(ДанныеСертификата));
	...
	Если Ждать ХранилищеЛокальногоКомпьютера.НайтиПоОтпечаткуАсинх(СертификатКриптографии.Отпечаток) = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Сертификат не найден в личном хранилище компьютера. Выполните проверку заново.'"));
		Возврат;
	КонецЕсли;
	
	ТекстОшибки = "";
	Попытка 
		Ждать ХранилищеЛокальногоКомпьютера.УдалитьАсинх(СертификатКриптографии);
	Исключение
		ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		ПараметрыОшибки = Новый Структура;
		ПараметрыОшибки.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Удаление сертификата из локального хранилища'"));
		ПараметрыОшибки.Вставить("ТекстОшибкиКлиент", ТекстОшибки);
	...
	ПоказатьПредупреждение(
		ОповещениеПослеОбработкиНавигационнойСсылкиКлассификатора("УдалитьСертификатИзЛокальногоХранилища", ДанныеСертификата),
		НСтр("ru = 'Сертификат удален из личного хранилища компьютера.'"));
...
Асинх Процедура УстановитьСписокОтзываСертификатаПослеПодключенияКомпоненты(ОбъектКомпоненты, Контекст) Экспорт
	...
	Если ТипЗнч(АдресаСписковОтзыва) = Тип("Строка") Тогда
	...
	СвойстваСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДополнительныеСвойстваСертификата(ДвоичныеДанныеСертификата);
	Контекст.Вставить("ИдентификаторКлючаУдостоверяющегоЦентра", СвойстваСертификата.ИдентификаторКлючаУдостоверяющегоЦентра);
...
Асинх Функция УстановленныеКриптопровайдерыИзКэша(ПредложитьУстановить = Ложь)
	...
	Иначе
		...
		Результат.Вставить("Токены", Новый Массив(РезультатИзКэша.Токены));
...
Процедура ЗаписатьВКэшУстановленныеКриптопровайдеры(Результат)
	...
	РезультатДляКэша.Вставить("Токены", Новый ФиксированныйМассив(Результат.Токены));
...
Процедура УстановитьКриптоПро(Форма, ОповещениеОЗавершении, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
			...
			|Вы можете загрузить дистрибутив с %2 и установить приложение самостоятельно.'"),
...
Процедура ОбработатьНавигационнуюСсылкуКлассификатора(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, ДополнительныеДанные = Неопределено) Экспорт
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "УдалитьСертификатИзЛокальногоХранилища" Тогда
		
		Если ЗначениеЗаполнено(Параметры.ДанныеСертификата) Тогда
			УдалитьСертификатИзЛокальногоХранилища(Параметры.ДанныеСертификата);
		КонецЕсли;	
...
Процедура ПослеОбработкиНавигационнойСсылкиКлассификатора(Результат, Контекст) Экспорт
	...
	Если НавигационнаяСсылка = "УстановитьСертификат"
		...
		Или НавигационнаяСсылка = "УстановитьСписокОтзываСертификата" Тогда
			...
			ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, ПараметрыФормы);
...
Функция РезультатПроверкиПодписи(Результат, РезультатСтруктура = Неопределено, ТребуетсяПроверка = Неопределено, НеверныйХеш = Неопределено)
	...
	Иначе
		...
		РезультатСтруктура.ПодписьМатематическиВерна = Не РезультатСтруктура.ТребуетсяПроверка;
