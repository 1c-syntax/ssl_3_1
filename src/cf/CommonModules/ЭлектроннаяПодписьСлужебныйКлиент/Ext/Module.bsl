...
Процедура ПолучитьОтпечаткиСертификатовПослеВыполнения(Результат, Контекст) Экспорт
	...
	Если Контекст.ПараметрыПолучения.Серверные Тогда
		
		ОписаниеОшибки = "";
		ОтпечаткиНаСервере = ЭлектроннаяПодписьСлужебныйВызовСервера.ОтпечаткиСертификатов(Контекст.ТолькоЛичные, ОписаниеОшибки, Контекст.ПараметрыПолучения.Сервис);
	...
	ИначеЕсли Контекст.ПараметрыПолучения.Сервис Тогда
...
Асинх Процедура ПроверитьПодписьДобавитьСвойстваПодписиВРезультат(Подпись, Результат, Контекст, ОшибкаМатематическойПроверки = Неопределено)
	...
	Если ТипЗнч(Контекст.РезультатПроверки) = Тип("Структура") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) =  Тип("Строка") Тогда
			...
			Если ОшибкаПоКлассификатору <> Неопределено Тогда
				...
				НеверныйХэш = ОшибкаПоКлассификатору.НеверныйХэшПодписи;
			КонецЕсли;
		КонецЕсли;
	...
	Если НеверныйХэш = Неопределено Тогда
		НеверныйХэш = ОшибкаМатематическойПроверки = Истина;
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(Контекст.Оповещение,
		РезультатПроверкиПодписи(Результат, Контекст.РезультатПроверки, ТребуетсяПроверка, НеверныйХэш));
...
//
Функция РезультатПроверкиПодписи(Результат, РезультатСтруктура = Неопределено, ТребуетсяПроверка = Неопределено, НеверныйХэш = Неопределено)
