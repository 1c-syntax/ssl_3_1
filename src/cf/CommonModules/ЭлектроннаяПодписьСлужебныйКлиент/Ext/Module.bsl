...
Процедура УстановитьСписокОтзываСертификата(ПараметрыУстановкиСпискаОтзыва) Экспорт
	...
	Если Не ЭлектроннаяПодписьКлиент.ОбщиеНастройки().РазрешенДоступКИнтернетСервисам Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(
			...
			ПараметрыУстановкиСпискаОтзыва);
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьОшибкуОбращенияКПрограмме(ЗаголовокФормы, ЗаголовокОшибки, ОшибкаНаКлиенте, ОшибкаНаСервере,
	...
	ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииОшибокНаКлиенте(ОшибкаНаКлиенте);
...
Процедура ДиагностикаСерверовМетокВремени(Место = Неопределено) Экспорт
	...
	РезультатДиагностики = Новый Массив;

	АдресаСерверовМетокВремени = ЭлектроннаяПодписьКлиент.ОбщиеНастройки().АдресаСерверовМетокВремени;

	Если АдресаСерверовМетокВремени.Количество() = 0 Тогда
		РезультатДиагностики.Добавить(НСтр(
			"ru = 'Адреса серверов меток времени не заполнены в настройках электронной подписи.'"));
	Иначе
		Если Место = "Клиент" Или Место = Неопределено Тогда
			РезультатДиагностики.Добавить(РезультатДиагностикиСерверовМетокВремени());
		КонецЕсли;

		Если Место = "Сервер" Или Место = Неопределено
			И ЭлектроннаяПодписьКлиент.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
			Если РезультатДиагностики.Количество() > 0 Тогда
				РезультатДиагностики.Добавить(Символы.ПС);
			КонецЕсли;
			РезультатДиагностики.Добавить(
				ЭлектроннаяПодписьСлужебныйВызовСервера.РезультатДиагностикиСерверовМетокВремени());
		КонецЕсли;
	КонецЕсли;
	
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.УстановитьТекст(СтрСоединить(РезультатДиагностики, Символы.ПС));
	ТекстовыйДокумент.Показать(НСтр("ru = 'Диагностика доступности серверов меток времени'"),);
...
Процедура ОткрытьФормуСертификата(ПараметрыФормы, ФормаВладелец = Неопределено, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.Сертификат", ПараметрыФормы, ФормаВладелец,,,, ОповещениеОЗавершении, ?(ФормаВладелец
		= Неопределено, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца));
...
Процедура ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, ФормаВладелец = Неопределено, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.РасширенноеПредставлениеОшибки", ПараметрыФормы, ФормаВладелец,,,, ОповещениеОЗавершении, ?(ФормаВладелец
		= Неопределено, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца))
...
Процедура ОткрытьФормуПрограммы(ОписаниеПрограммы = Неопределено, ФормаВладелец = Неопределено, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	РежимОткрытияОкна = ?(ФормаВладелец = Неопределено, Неопределено,
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
	Если ОписаниеПрограммы = Неопределено Тогда
		ОткрытьФорму("Справочник.ПрограммыЭлектроннойПодписиИШифрования.ФормаОбъекта", , ФормаВладелец,,,,
			ОповещениеОЗавершении, РежимОткрытияОкна);
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ОписаниеПрограммы) = Тип("Строка") Тогда
		ОткрытьФорму("Справочник.ПрограммыЭлектроннойПодписиИШифрования.ФормаОбъекта", Новый Структура("ИдентификаторПоставляемойНастройки", ОписаниеПрограммы) , ФормаВладелец,,,,
			ОповещениеОЗавершении, РежимОткрытияОкна);
		Возврат;
	КонецЕсли;
	
	ПараметрыПрограммы = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовоеРасширенноеОписаниеПрограммы();
	ЗаполнитьЗначенияСвойств(ПараметрыПрограммы, ОписаниеПрограммы);

	Если ЗначениеЗаполнено(ПараметрыПрограммы.Ссылка) Тогда
		ПоказатьЗначение(ОповещениеОЗавершении, ПараметрыПрограммы.Ссылка);
	ИначеЕсли ПараметрыПрограммы.Автоопределение = Ложь Тогда
		ОткрытьФорму("Справочник.ПрограммыЭлектроннойПодписиИШифрования.ФормаОбъекта", Новый Структура("Программа",
			ПараметрыПрограммы), ФормаВладелец,,,,ОповещениеОЗавершении, РежимОткрытияОкна);
	Иначе
		ОткрытьФорму("Справочник.ПрограммыЭлектроннойПодписиИШифрования.Форма.ПриложениеОпределенноеАвтоматически",
			Новый Структура("Программа", ПараметрыПрограммы), ФормаВладелец,,,, ОповещениеОЗавершении, РежимОткрытияОкна);
	КонецЕсли;
...
//
Процедура ОткрытьФормуУстановкиПрограммКриптопровайдеров(Параметры = Неопределено, Владелец = Неопределено, Оповещение = Неопределено) Экспорт
	
	СтандартнаяОбработка = Истина;
	ЭлектроннаяПодписьКлиентЛокализация.ПриОткрытииФормыУстановкиПрограммКриптопровайдеров(
		Параметры, Владелец, Оповещение, СтандартнаяОбработка);
	
	Если СтандартнаяОбработка Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Действие недоступно в приложении'"));
	КонецЕсли;
...
Функция ОшибкаПоКлассификатору(ТекстОшибки, ОшибкаНаСервере = Ложь, ОшибкаПроверкиПодписи = Ложь,
	...
	Возврат ЭлектроннаяПодписьСлужебныйКлиентПовтИсп.ОшибкаПоКлассификатору(
		ТекстОшибки, ОшибкаНаСервере, ОшибкаПроверкиПодписи, УказанСертификат);
	
КонецФункции

Функция ТекстОшибкиУстройствоНеПоддерживается() Экспорт
...
Процедура ОткрытьТокен(Токен, ВладелецФормы, ОбработкаЗавершения = Неопределено) Экспорт
	...
	СерверныеПараметры.Вставить("Токен", Токен);
	ОткрытьФорму("Справочник.ПрограммыЭлектроннойПодписиИШифрования.Форма.Токен",
		СерверныеПараметры, ВладелецФормы, ОбработкаЗавершения);
	
КонецПроцедуры

Функция РекомендацииДляТокена() Экспорт
	
	Возврат НСтр("ru = 'Убедитесь, что токен правильно вставлен в USB-порт компьютера и на нем горит световой индикатор, обновите список токенов и повторите действие.
...
Функция ОшибкаНеНайденТокен() Экспорт
	...
	Возврат НСтр("ru = 'Не подключен токен'");
	
КонецФункции

Функция ПараметрыПодписиНаТокене(Токен, Сертификат, ТипПодписи) Экспорт
	
	Структура = Новый Структура;
	Структура.Вставить("Токен", Токен);
	Структура.Вставить("Сертификат", Сертификат);
	Структура.Вставить("ТипПодписи", ТипПодписи);
	Структура.Вставить("Открепленная", Истина);
	Структура.Вставить("ОбъектКомпоненты", Неопределено);
	Структура.Вставить("ВключатьСертификатВПодпись", Истина);
	
	Возврат Структура;
	
КонецФункции

Функция ПараметрыПроверкиПодписиНаТокене(АлгоритмПодписи = Неопределено)
	
	Структура = Новый Структура;
	Структура.Вставить("АлгоритмПодписи", АлгоритмПодписи);
	Структура.Вставить("ОбъектКомпоненты", Неопределено);
	Структура.Вставить("ВернутьСертификатыДляПроверки", Ложь);
	Структура.Вставить("ОткрепленнаяПодпись", Истина);

	Возврат Структура;
	
КонецФункции

Процедура ДобавитьСертификатСТокенаПослеПолученияПодписи(Результат, Контекст) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаполнитьОшибкуДобавленияСертификата(
			Контекст.ОшибкаНаКлиенте,
			Новый Структура("Ссылка, Токен", Контекст.Токен.Представление, Контекст.Токен),
			"Подписание",
			Результат,
	...
	Контекст.Вставить("ОписаниеПрограммы", Новый Структура("Ссылка", Неопределено));
	ДобавитьСертификатЦиклПослеПодписания(Результат, Контекст);

КонецПроцедуры

Процедура ДобавитьСертификатСТокенаПослеРасшифровки(Результат, Контекст) Экспорт
...
Процедура ДобавитьСертификатСТокенаПослеШифрования(Результат, Контекст) Экспорт
	...
	ПараметрыРасшифровкиНаТокене = ПараметрыРасшифровкиНаТокене(Контекст.Токен, Контекст.СертификатКриптографии);
	Оповещение = Новый ОписаниеОповещения("ДобавитьСертификатСТокенаПослеРасшифровки", ЭтотОбъект, 
			Контекст);
			
	СтандартнаяОбработка = Истина;
	ЭлектроннаяПодписьКлиентЛокализация.ПриРасшифровкеНаТокене(Оповещение, ПараметрыРасшифровкиНаТокене, Результат, СтандартнаяОбработка);
	Если СтандартнаяОбработка Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОбработчикЗавершения, Неопределено);
		СообщитьОбОшибкеДобавленияСертификата(Контекст);
	КонецЕсли;
...
Функция ПараметрыШифрованияНаТокене(Токен, Сертификат, АлгоритмШифрования = Неопределено) Экспорт
	
	Структура = Новый Структура;
	Структура.Вставить("Токен", Токен);
	Структура.Вставить("Сертификат", Сертификат);
	Структура.Вставить("АлгоритмШифрования", АлгоритмШифрования);
	Структура.Вставить("ОбъектКомпоненты", Неопределено);
...
Функция ПараметрыРасшифровкиНаТокене(Токен, Сертификат) Экспорт
...
Процедура ПолеПароляНачалоВыбора(Форма, ВнутренниеДанные, СвойстваПароля, СтандартнаяОбработка, ДополнительныеПараметры = Неопределено) Экспорт
	...
	КонецЕсли;
	ДополнительныеПараметры.Вставить("ПриИзмененииВидимостиПароля", Истина);
	
	ОбработатьПарольВФорме(Форма,
		ВнутренниеДанные, СвойстваПароля, ДополнительныеПараметры, Элемент.ТекстРедактирования);
	
КонецПроцедуры

Функция ИспользуетсяИнтерактивныйРежимКриптографии(МенеджерКриптографии) Экспорт
	
	Если ТипЗнч(МенеджерКриптографии) = Тип("Структура") И Не МенеджерКриптографии.Свойство("ИспользованиеИнтерактивногоРежима") Тогда
		Возврат Ложь;
	КонецЕсли;
...
Асинх Процедура ПроверитьПутьКПрограмме(ОбработкаЗавершения, ПутьКПрограмме)
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПроверитьПутьКПрограммеПослеПодключенияРасширения1СПредприятия", ЭтотОбъект, Контекст);
	
	ВыполняетсяУстановкаДополнительныхКомпонент = Ждать ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(Оповещение);
...
Процедура ПроверитьПутьКПрограммеПослеПодключенияРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеПрограммы.РезультатПроверкиНаКлиенте =
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ТекстОшибкиПоискаПрограммы(
				НСтр("ru = 'Не установлено на компьютере.'"), Результат);
	КонецЕсли;
...
Процедура ОткрытьСертификатПродолжение(Контекст)
	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		...
		ВызватьИсключение(ТекстСообщения, КатегорияОшибки.ОшибкаКонфигурации);
...
Процедура ОткрытьСертификатЗавершение(Контекст)
	...
	ОткрытьФормуСертификата(ПараметрыФормы);
...
//
Асинх Процедура СохранитьСертификат(Оповещение, Сертификат, ИмяФайлаБезРасширения = "", ПовторныйВызов = Ложь) Экспорт
	
	Если Не ПовторныйВызов Тогда
		ПараметрыПовторногоВызова = Новый Структура("Оповещение, Сертификат, ИмяФайлаБезРасширения",
			Оповещение, Сертификат, ИмяФайлаБезРасширения);
		ОповещениеПриОтсутствииКомпонентРаботыСЭП = Новый ОписаниеОповещения(
			"СохранитьСертификатПовторноПослеУстановкиКомпонент", ЭтотОбъект, ПараметрыПовторногоВызова);
		ВыполняетсяУстановкаДополнительныхКомпонент = Ждать ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(
...
// Повторный вызов процедуры СохранитьСертификат после установки компонент.
Процедура СохранитьСертификатПовторноПослеУстановкиКомпонент(Результат, ПараметрыПовторногоВызова) Экспорт
	
	Если Результат = Истина Тогда
		СохранитьСертификат(ПараметрыПовторногоВызова.Оповещение, ПараметрыПовторногоВызова.Сертификат,
			ПараметрыПовторногоВызова.ИмяФайлаБезРасширения , Истина)
	КонецЕсли;
...
Асинх Процедура СохранитьПодпись(АдресПодписи, ИмяФайлаПодписи = "", ПовторныйВызов = Ложь) Экспорт
	
	Если Не ПовторныйВызов Тогда
		ПараметрыПовторногоВызова = Новый Структура("АдресПодписи, ИмяФайлаПодписи", АдресПодписи, ИмяФайлаПодписи);
		ОповещениеПриОтсутствииКомпонентРаботыСЭП = Новый ОписаниеОповещения(
			"СохранитьПодписьПовторноПослеУстановкиКомпонент", ЭтотОбъект, ПараметрыПовторногоВызова);
		ВыполняетсяУстановкаДополнительныхКомпонент = Ждать ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(
			ОповещениеПриОтсутствииКомпонентРаботыСЭП);
		Если ВыполняетсяУстановкаДополнительныхКомпонент Тогда
			Возврат;
	...
	ФайловаяСистемаКлиент.СохранитьФайл(Оповещение, АдресПодписи, ИмяФайлаПодписи, ПараметрыСохранения);
	
КонецПроцедуры

// Повторный вызов процедуры СохранитьПодпись после установки компонент.
Процедура СохранитьПодписьПовторноПослеУстановкиКомпонент(Результат, ПараметрыПовторногоВызова) Экспорт
	
	Если Результат = Истина Тогда
		СохранитьПодпись(ПараметрыПовторногоВызова.АдресПодписи, ПараметрыПовторногоВызова.ИмяФайлаПодписи, Истина);
	КонецЕсли;
...
//   ТолькоВЛичномХранилище - Булево - если Истина, тогда искать в личном хранилище и на токенах, иначе везде.
...
Процедура ПолучитьСертификатПоОтпечаткуПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		Контекст.Ошибки.Добавить(?(ТипЗнч(Результат) = Тип("Строка"), Результат, Результат.ОписаниеОшибки));
			
		СтандартнаяОбработка = Истина;
		ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииСертификатаПоОтпечаткуНаТокене(
			Новый ОписаниеОповещения("ПослеПолученияСертификатаПоОтпечаткуНаТокене", ЭтотОбъект, Контекст),
			Контекст.Отпечаток, СтандартнаяОбработка, Истина, Истина);

		Если Не СтандартнаяОбработка Тогда
			Возврат;
		КонецЕсли;
	
		ПолучитьСертификатПоОтпечаткуЗавершение(Неопределено, СтрСоединить(Контекст.Ошибки, Символы.ПС), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ПолучитьСертификатПоОтпечаткуПослеПоискаМенеджеромКриптографии(Неопределено, ПредставлениеОшибки, Контекст);
		Возврат;
	КонецПопытки;
...
Процедура ПолучитьСертификатПоОтпечаткуПослеПоиска(Сертификат, Контекст) Экспорт
	...
	ПолучитьСертификатПоОтпечаткуПослеПоискаМенеджеромКриптографии(Сертификат, "", Контекст);
	
КонецПроцедуры

// Продолжение процедуры ПолучитьСертификатПоОтпечатку.
Процедура ПолучитьСертификатПоОтпечаткуПослеПоискаМенеджеромКриптографии(Сертификат, ПредставлениеОшибки, Контекст)
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		Контекст.Ошибки.Добавить(ПредставлениеОшибки);
	КонецЕсли;
	
	Если Контекст.Свойство("Программа") И Контекст.Программа <> Неопределено Тогда
		ПолучитьСертификатПоОтпечаткуЗавершение(Неопределено, СтрСоединить(Контекст.Ошибки, Символы.ПС), Контекст);
		Возврат;
	КонецЕсли;
...
// Продолжение процедуры ПолучитьСертификатПоОтпечатку.
Процедура ПослеПолученияСертификатаПоОтпечаткуНаТокене(Сертификат, Контекст) Экспорт

	Если Сертификат <> Неопределено Тогда
		ПолучитьСертификатПоОтпечаткуЗавершение(Сертификат, СтрСоединить(Контекст.Ошибки, Символы.ПС), Контекст);
	Иначе
		ПолучитьСертификатПоОтпечаткуЗавершение(Неопределено, СтрСоединить(Контекст.Ошибки, Символы.ПС), Контекст);
	КонецЕсли;
...
Процедура ПолучитьОтпечаткиСертификатов(Оповещение, ТолькоЛичные, ПараметрыПолучения = Истина) Экспорт
	...
	Если ТипЗнч(ПараметрыПолучения) = Тип("Булево") Тогда
		ПараметрыПолучения = ЭлектроннаяПодписьКлиент.ПараметрыПолученияОтпечатковСертификатов(ПараметрыПолучения, Неопределено, Неопределено);
		ПараметрыПолучения.ВозвращатьИсточникПолучения = Ложь;
	ИначеЕсли ПараметрыПолучения = Неопределено Тогда
		ПараметрыПолучения = ЭлектроннаяПодписьКлиент.ПараметрыПолученияОтпечатковСертификатов();
	КонецЕсли;
	
	Контекст = Новый Структура;
	Контекст.Вставить("Оповещение",         Оповещение);
	Контекст.Вставить("ТолькоЛичные",       ТолькоЛичные);
	Контекст.Вставить("ПараметрыПолучения", ПараметрыПолучения);
	
	ПолучитьСвойстваСертификатовНаКлиенте(Новый ОписаниеОповещения(
			"ПолучитьОтпечаткиСертификатовПослеВыполнения", ЭтотОбъект, Контекст),
		ТолькоЛичные, Ложь, Истина, ПараметрыПолучения.Клиентские = Истина);
...
Процедура ПолучитьОтпечаткиСертификатовПослеВыполнения(Результат, Контекст) Экспорт
	...
	РезультатПолученияОтпечатков = Новый Структура;
	РезультатПолученияОтпечатков.Вставить("ОшибкаПолученияСертификатовНаКлиенте");
	РезультатПолученияОтпечатков.Вставить("ОшибкаПолученияСертификатовНаСервере");
	
	Если Результат <> Неопределено Тогда
		Если ЗначениеЗаполнено(Результат.ОшибкаПолученияСертификатовНаКлиенте) Тогда
			РезультатПолученияОтпечатков.ОшибкаПолученияСертификатовНаКлиенте = Результат.ОшибкаПолученияСертификатовНаКлиенте;
		Иначе
			Для Каждого КлючИЗначение Из Результат.СвойстваСертификатовНаКлиенте Цикл
				Отпечатки.Вставить(КлючИЗначение.Ключ, ?(Контекст.ПараметрыПолучения.ВозвращатьИсточникПолучения = Истина, "Клиент", КлючИЗначение.Значение))
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	
	Если Контекст.ПараметрыПолучения.Серверные <> Неопределено Тогда
		
		Если Контекст.ПараметрыПолучения.Серверные = Истина Тогда
			ОписаниеОшибки = Null
		Иначе
			ОписаниеОшибки = "";
		КонецЕсли;
		
		ОтпечаткиНаСервере = ЭлектроннаяПодписьСлужебныйВызовСервера.ОтпечаткиСертификатов(Контекст.ТолькоЛичные, ОписаниеОшибки, Контекст.ПараметрыПолучения.Сервис <> Неопределено);
		Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			РезультатПолученияОтпечатков.ОшибкаПолученияСертификатовНаСервере = ОписаниеОшибки;
		КонецЕсли;
		
		Для Каждого Элемент Из ОтпечаткиНаСервере Цикл
			Если Отпечатки[Элемент] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Отпечатки.Вставить(Элемент.Ключ, Элемент.Значение);
		КонецЦикла;
		
	ИначеЕсли Контекст.ПараметрыПолучения.Сервис <> Неопределено Тогда
		
		ОтпечаткиВСервисе = ЭлектроннаяПодписьСлужебныйВызовСервера.ОтпечаткиСертификатовСервиса();
		Для Каждого Элемент Из ОтпечаткиВСервисе Цикл
			Если Отпечатки[Элемент] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Отпечатки.Вставить(Элемент, "Сервис")
		КонецЦикла;
		
	КонецЕсли;
		
	Если ЗначениеЗаполнено(Результат.ОшибкаПолученияСертификатовНаКлиенте) И Не Контекст.ПараметрыПолучения.ВозвращатьИсточникПолучения Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат.ОшибкаПолученияСертификатовНаКлиенте);
		Возврат;
	КонецЕсли;
	
	Если Контекст.ПараметрыПолучения.ВозвращатьИсточникПолучения Тогда
		РезультатПолученияОтпечатков.Вставить("Отпечатки", Отпечатки);
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, РезультатПолученияОтпечатков);
	Иначе
...
Процедура ПроверитьПодпись(Оповещение, ИсходныеДанные, Подпись, МенеджерКриптографии = Неопределено, НаДату = Неопределено, ПараметрыПроверки = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ПараметрыПроверки) = Тип("Структура") Тогда
		...
		КонецЕсли;
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ЭлектроннаяПодписьСлужебныйКлиент.ПроверитьПодпись",
			"ПараметрыПроверки.ПроверятьСертификат", ПараметрыДляПроверки.ПроверятьСертификат, Тип("Строка"),, ОжидаемыеЗначения);
	КонецЕсли;
	...
	Если ПараметрыДляПроверки.РезультатВВидеСтруктуры Тогда
		...
		Контекст.РезультатПроверки.ПараметрыПроверкиСертификата = ПараметрыДляПроверки.ПроверятьСертификат;
		Контекст.Вставить("ПроверятьСертификат", ПараметрыДляПроверки.ПроверятьСертификат);
	Иначе
		Контекст.Вставить("РезультатПроверки", Неопределено);
		Контекст.Вставить("ПроверятьСертификат", ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные());
	КонецЕсли;
	...
	Если Не ЭлектроннаяПодписьКлиент.ПроверятьЭлектронныеПодписиНаСервере() Тогда
		...
		Если МенеджерКриптографии = Неопределено Тогда
			...
			ИначеЕсли ПроверятьСертификатыОблачнойПодписью() Тогда
				ПроверитьПодписьОблачнаяПодпись(Контекст);
			Иначе
				ПроверитьПодписьСредствамиТокена(Контекст);
			КонецЕсли;
...
Асинх Процедура ПроверитьПодписьСредствамиТокена(Контекст) Экспорт
	
	ПараметрыПроверки = ПараметрыПроверкиПодписиНаТокене(Контекст.АлгоритмПодписи);
	
	СтандартнаяОбработка = Истина;
	ЭлектроннаяПодписьКлиентЛокализация.ПриПроверкеПодписиНаТокене(
		Новый ОписаниеОповещения("ПослеПроверкиПодписиНаТокене", ЭтотОбъект, Контекст),
		Контекст.ИсходныеДанные, Контекст.Подпись, ПараметрыПроверки, СтандартнаяОбработка);
		
	Если СтандартнаяОбработка Тогда
		ПослеПроверкиПодписиНаТокене(Неопределено, Контекст);
	КонецЕсли;
	
КонецПроцедуры

Асинх Процедура ПослеПроверкиПодписиНаТокене(РезультатМатематическойПроверки, Контекст) Экспорт
	
	Если РезультатМатематическойПроверки = Неопределено Тогда
		ПроверитьПодписьДобавитьСвойстваПодписиВРезультат(Контекст.Подпись, Неопределено, Контекст);
	Иначе
		
		Если РезультатМатематическойПроверки = Ложь Тогда
			НеверныйХеш = Истина;
			ТребуетсяПроверка = Ложь;
			Результат = НСтр("ru = 'Неверный хеш.'");
		ИначеЕсли РезультатМатематическойПроверки = Истина Тогда
			Если Контекст.ПроверятьСертификат = ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат() Тогда
				ТребуетсяПроверка = Ложь;
				Результат = Истина;
			Иначе
				ТребуетсяПроверка = Истина;
				НеверныйХеш = Ложь;
				Результат = НСтр("ru = 'Проверка сертификата средствами токена будет доступна в следующих версиях приложения.'");
			КонецЕсли;
		Иначе
			ТребуетсяПроверка = Истина;
			Результат = РезультатМатематическойПроверки;
		КонецЕсли;
		
		Если ТипЗнч(Контекст.РезультатПроверки) = Тип("Структура") Тогда
			СвойстваПодписи = Ждать СвойстваПодписиИзДвоичныхДанных(Контекст.Подпись, Истина);
			ЗаполнитьЗначенияСвойств(Контекст.РезультатПроверки, СвойстваПодписи);
		КонецЕсли;
			
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			РезультатПроверкиПодписи(Результат, Контекст.РезультатПроверки, ТребуетсяПроверка, НеверныйХеш));
		
	КонецЕсли;
	
КонецПроцедуры

Асинх Процедура ПроверитьПодписьДобавитьСвойстваПодписиВРезультат(Подпись, Результат, Контекст, ОшибкаМатематическойПроверки = Неопределено)
	...
	Если НеверныйХеш = Неопределено Тогда
		НеверныйХеш = ОшибкаМатематическойПроверки = Истина;
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписиXMLDSig(Данные, Контекст) Экспорт
	...
	Если Контекст.ПроверятьСертификат = ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат() Тогда
		ПроверитьПодписьПослеПроверкиСертификатаПодписи(Истина, Контекст);
	Иначе
		...
		ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра = Контекст.ПроверятьСертификат;

		Оповещение = Новый ОписаниеОповещения("ПроверитьПодписьПослеПроверкиСертификатаПодписи", ЭтотОбъект, Контекст);

		ПроверитьСертификат(Оповещение, Данные.Сертификат, МенеджерКриптографии, Данные.ДатаПодписания,
...
Процедура ПроверитьПодписьНаКлиентеПослеОшибкиПроверкиПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ОшибкаМатематическойПроверки = Истина;
	Иначе
		ТекстОшибки = Строка(ИнформацияОбОшибке);
		ОшибкаМатематическойПроверки = Неопределено;
	КонецЕсли;
	...
	ПроверитьПодписьДобавитьСвойстваПодписиВРезультат(Подпись, ТекстОшибки, Контекст, ОшибкаМатематическойПроверки);
...
Асинх Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если ТипЗнч(Контекст.МенеджерКриптографии) = Тип("МенеджерКриптографии") Тогда
		...
		Если Не ПустаяСтрока(ОшибкаПроверкиПодписи) Тогда
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, РезультатПроверкиПодписи(
					ОшибкаПроверкиПодписи, Контекст.РезультатПроверки,, Ложь));
			Возврат;
		КонецЕсли;
	...
	Иначе // после проверки в облачных сервисах.
		...
		Если СвойстваПодписи.Сертификат = Неопределено Тогда
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, РезультатПроверкиПодписи(
				НСтр("ru = 'Сертификат не существует в данных подписи.'"), Контекст.РезультатПроверки,, Ложь));
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		ДополнительныеПараметры.ДляПроверкиПодписи = Истина;
		ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра = Контекст.ПроверятьСертификат;
		Если СвойстваПодписи.ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.АрхивнаяCAdESAv3") Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПроверитьПодписьПослеПроверкиСертификатаПодписи", ЭтотОбъект, Контекст);
		ПроверитьСертификат(Оповещение, Контекст.Сертификат, Контекст.МенеджерКриптографии, ДатаПодписания,
			ДополнительныеПараметры);
	КонецЕсли;
...
Процедура ПроверитьПодписьПослеПроверкиСертификатаПодписи(РезультатПроверки, Контекст) Экспорт
	...
	ВыполнитьОбработкуОповещения(Контекст.Оповещение,
		РезультатПроверкиПодписи(Результат, Контекст.РезультатПроверки,,Ложь));
...
//
//   НаДату               - Дата - проверить срок действия сертификата на указанную дату.
...
Процедура ПроверитьСертификат(Оповещение, Сертификат, МенеджерКриптографии = Неопределено, НаДату = Неопределено,
	...
	ДополнительныеПараметрыПроверкиСертификата = ДополнительныеПараметрыПроверкиСертификата();
	Если ДополнительныеПараметры <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ДополнительныеПараметрыПроверкиСертификата, ДополнительныеПараметры);
		
		Если ТипЗнч(ДополнительныеПараметрыПроверкиСертификата.ВыполнятьПроверкуУдостоверяющегоЦентра) = Тип("Булево") Тогда // Для совместимости
			ДополнительныеПараметрыПроверкиСертификата.ВыполнятьПроверкуУдостоверяющегоЦентра = ?(
				ДополнительныеПараметрыПроверкиСертификата.ВыполнятьПроверкуУдостоверяющегоЦентра,
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные(), 
		...
		ОжидаемыеЗначения.Добавить(ЭлектроннаяПодписьСлужебныйКлиентСервер.ТолькоКвалифицированные());
		ОжидаемыеЗначения.Добавить(ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат());
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ЭлектроннаяПодписьСлужебныйКлиент.ПроверитьСертификат",
			"ДополнительныеПараметры.ВыполнятьПроверкуУдостоверяющегоЦентра", ДополнительныеПараметрыПроверкиСертификата.ВыполнятьПроверкуУдостоверяющегоЦентра,
			Тип("Строка"),, ОжидаемыеЗначения);
			
		Контекст.ПоказатьОшибку                    = ДополнительныеПараметрыПроверкиСертификата.ПоказатьОшибку;
		Контекст.ОбъединитьОшибкиДанныхСертификата = ДополнительныеПараметрыПроверкиСертификата.ОбъединитьОшибкиДанныхСертификата;
		Контекст.ПроверитьВСервисеПослеОшибки      = ДополнительныеПараметрыПроверкиСертификата.ПроверитьВСервисеПослеОшибки;
		Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра = ДополнительныеПараметрыПроверкиСертификата.ВыполнятьПроверкуУдостоверяющегоЦентра;
		Контекст.ДляПроверкиПодписи                = ДополнительныеПараметрыПроверкиСертификата.ДляПроверкиПодписи;
		Контекст.ИгнорироватьПроверкуВСпискеОтозванныхСертификатов = ДополнительныеПараметрыПроверкиСертификата.ИгнорироватьПроверкуВСпискеОтозванныхСертификатов;
		Контекст.УказанаДатаМеткиВремени = ДополнительныеПараметрыПроверкиСертификата.УказанаДатаМеткиВремени;
...
//   * УказанаДатаМеткиВремени - Булево 
...
//   * ВыполнятьПроверкуУдостоверяющегоЦентра - Строка
//   * ИгнорироватьПроверкуВСпискеОтозванныхСертификатов - Булево
//
Функция КонтекстПроверкиСертификата()
	...
	Контекст.Вставить("УказанаДатаМеткиВремени", Ложь);
	...
	Контекст.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные());
	...
	Контекст.Вставить("ИгнорироватьПроверкуВСпискеОтозванныхСертификатов", Ложь);
...
//   * УказанаДатаМеткиВремени - см. ЭлектроннаяПодписьКлиент.ПараметрыПроверкиСертификата.УказанаДатаМеткиВремени
//
Функция ДополнительныеПараметрыПроверкиСертификата() Экспорт
	...
	Структура.Вставить("УказанаДатаМеткиВремени", Ложь);
	Структура.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные());
	Структура.Вставить("ИгнорироватьПроверкуВСпискеОтозванныхСертификатов", Ложь);
...
Процедура ПроверитьСертификатПослеВыгрузкиСертификата(Сертификат, Контекст) Экспорт
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ИгнорироватьПроверкуВСпискеОтозванныхСертификатов", Контекст.ИгнорироватьПроверкуВСпискеОтозванныхСертификатов);
	...
	ДополнительныеПараметры.Вставить("УказанаДатаМеткиВремени", Контекст.УказанаДатаМеткиВремени);
	ДополнительныеПараметры.Вставить("Предупреждение");
	ДополнительныеПараметры.Вставить("Сертификат", АдресСертификата);
	...
	Если ЭлектроннаяПодписьСлужебныйВызовСервера.ПроверитьСертификат(АдресСертификата,
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.Предупреждение) И Не Контекст.ДляПроверкиПодписи
			...
			ПоказатьОповещениеПользователя(
				...
				БиблиотекаКартинок.ДиалогВосклицание, СтатусОповещенияПользователя.Важное, ДополнительныеПараметры.Сертификат);
...
Процедура ПроверитьСертификатНаКлиенте(Контекст)
	...
	Если ТипЗнч(СертификатДляПроверки) = Тип("ДвоичныеДанные") Тогда
		...
		ЭлектроннаяПодписьКлиент.УстановитьРасширение(Ложь, Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для продолжения установите расширение для работы с 1С:Предприятием.'"));
...
Процедура ПроверитьСертификатНаКлиентеПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Иначе
		ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ТекстОшибкиНеУстановленоРасширение(), Ложь, Контекст);
	КонецЕсли;
...
Процедура ПроверитьСертификатПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Контекст.Вставить("РежимыПроверкиСертификата",
		ЭлектроннаяПодписьСлужебныйКлиентСервер.РежимыПроверкиСертификата(
			ЗначениеЗаполнено(Контекст.НаДату), Контекст.ИгнорироватьПроверкуВСпискеОтозванныхСертификатов));
...
Процедура ПроверитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		Иначе
			Если ТипЗнч(Результат) = Тип("Структура") Тогда
				Контекст.ОписаниеОшибкиНаКлиенте = Результат.ОписаниеОшибки;
			Иначе
				Контекст.ОписаниеОшибкиНаКлиенте = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось получить менеджер криптографии для алгоритма %1: %2.'"),
					Контекст.АлгоритмПодписи, Результат);
			КонецЕсли;
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроверитьСертификатНаКлиентеПослеПроверки", ЭтотОбъект, Контекст,
		"ПроверитьСертификатНаКлиентеПослеОшибкиПроверки", ЭтотОбъект);

	Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата()
		И Контекст.УказанаДатаМеткиВремени Тогда
		//@skip-check bsl-legacy-check-dynamic-feature-access
		Контекст.МенеджерКриптографии.НачатьПроверкуСертификата(ОписаниеОповещения, Контекст.СертификатКриптографии,
			Контекст.РежимыПроверкиСертификата, Контекст.НаДату - РазницаСУниверсальнымВременем());
	Иначе
		Контекст.МенеджерКриптографии.НачатьПроверкуСертификата(ОписаниеОповещения, Контекст.СертификатКриптографии,
			Контекст.РежимыПроверкиСертификата);
	КонецЕсли;
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		Если ОшибкаПоКлассификатору <> Неопределено И ЗначениеЗаполнено(ОшибкаПоКлассификатору.ДействияДляУстранения) Тогда
			...
			Если Действие <> Неопределено Тогда
				...
				Если Действие = "УстановитьСписокОтзываСертификата"
					И ЭлектроннаяПодписьКлиент.ОбщиеНастройки().РазрешенДоступКИнтернетСервисам Тогда
					...
					Если ЗначениеЗаполнено(ИмяИздателя) Тогда
						...
						Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
							СвойстваСертификата = ЭлектроннаяПодписьКлиент.СвойстваСертификата(Контекст.СертификатКриптографии);
							АдресСпискаОтзываВнутренний = ЭлектроннаяПодписьСлужебныйКлиентСервер.АдресСпискаОтзываВнутренний(
								ИмяИздателя, СвойстваСертификата,
								ЭлектроннаяПодписьСлужебныйКлиентПовтИсп.КаталогиСписковОтзываУЦ());
						Иначе
							АдресСпискаОтзываВнутренний = ЭлектроннаяПодписьСлужебныйКлиентСервер.АдресСпискаОтзываВнутренний(
	...
	Если Контекст.ДействияДляУстраненияОшибок.Получить("ДействиеПриПовторнойПроверке") = "УстановитьСписокОтзываСертификата" Тогда
		...
		Если Не Результат.УстановкаВыполнена Тогда
			...
			ТекстОшибкиУстановкиСпискаОтзыва = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось установить список отзыва при проверке сертификата: %1
				|%2.'"), Результат.Сообщение, СвойстваСертификатаСтрокой(Контекст.СертификатКриптографии));
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Обновление списков отзыва'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
				"Ошибка", ТекстОшибкиУстановкиСпискаОтзыва,, Истина);
		...
		Иначе
			...
			ТекстСообщенияОбУстановкеСпискаОтзыва = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установлен список отзыва при проверке сертификата: %1.'"), 
				СвойстваСертификатаСтрокой(Контекст.СертификатКриптографии));
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Обновление списков отзыва'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
				"Информация", ТекстСообщенияОбУстановкеСпискаОтзыва,, Истина);
...
Асинх Процедура ПроверитьСертификатНаКлиентеПослеПроверки(Контекст) Экспорт
	...
	ОшибкаПросрочки = ЭлектроннаяПодписьСлужебныйКлиентСервер.СертификатПросрочен(
		Контекст.СертификатКриптографии, Контекст.НаДату, РазницаСУниверсальнымВременем());
...
Асинх Процедура ДополнительнаяПроверкаСертификата(СертификатКриптографии, ПроверкаНаСервере, Контекст)
	...
	Если Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра <> ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат() Тогда
		
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ЭтоПроверкаПодписи", Контекст.ДляПроверкиПодписи);
		ПараметрыПроверки.Вставить("ПроверятьСертификат", Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра);
		
		РезультатПроверкиУдостоверяющегоЦентраСертификата = Ждать РезультатПроверкиУдостоверяющегоЦентраСертификата(
			СертификатКриптографии, Контекст.НаДату, ПараметрыПроверки, Контекст.СвойстваСертификата);
		...
		Если Не РезультатПроверкиУдостоверяющегоЦентраСертификата.Действует Или ЗначениеЗаполнено(
			...
			Если Не РезультатПроверкиУдостоверяющегоЦентраСертификата.Действует 
				И (Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра = ЭлектроннаяПодписьСлужебныйКлиентСервер.ТолькоКвалифицированные()
					Или ПользовательскиеНастройкиСертификата.ПодписаниеРазрешено <> Истина) Тогда
			...
			Если ЗначениеЗаполнено(РезультатПроверкиУдостоверяющегоЦентраСертификата.Предупреждение.ТекстОшибки)
				...
				Если Не ПользовательскиеНастройкиСертификата.Оповещен И СсылкаНаСертификат <> Неопределено Тогда
					...
					НСтр("ru = 'Необходима замена сертификата'"), ДействиеПриНажатии, СсылкаНаСертификат,
						БиблиотекаКартинок.ДиалогВосклицание, СтатусОповещенияПользователя.Важное, СсылкаНаСертификат);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Асинх Функция РезультатПроверкиУдостоверяющегоЦентраСертификата(СертификатКриптографии,  НаДату = Неопределено, ПараметрыПроверки = Неопределено, СвойстваСертификата = Неопределено) Экспорт
	
	Результат = ЭлектроннаяПодписьСлужебныйКлиентСервер.РезультатПроверкиУдостоверяющегоЦентраПоУмолчанию();
	Результат = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПриЗаполненииРезультатаПроверкиУдостоверяющегоЦентра(
...
Функция ПараметрыСозданияМенеджераКриптографии() Экспорт
	...
	ПараметрыСозданияМенеджераКриптографии.Вставить("АлгоритмШифрования", "");
...
Процедура СоздатьМенеджерКриптографии(Оповещение, Операция, ПараметрыСозданияМенеджераКриптографии = Неопределено) Экспорт
	...
	ЗаполнитьЗначенияСвойств(Контекст, ПараметрыСозданияМенеджераКриптографии);
...
Функция КонтекстСоздатьМенеджерКриптографии()
	...
	Контекст.Вставить("АлгоритмПодписи");
	Контекст.Вставить("АлгоритмШифрования");
...
Асинх Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("ru = 'Требуется приложение электронной подписи и шифрования'");
	...
	Если Не Подключено Тогда
		ТекстОбщейОшибки = НСтр("ru = 'Установите в браузере расширение для работы с 1С:Предприятием.'");
		СвойстваОшибки = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваОшибки();
	...
	Если ЗначениеЗаполнено(ТекстОбщейОшибки) Тогда
		...
		Если Контекст.ПоказатьОшибку = Истина И ТекстОбщейОшибки <> ТекстОшибкиУстройствоНеПоддерживается() Тогда
...
Асинх Функция ЗаполнитьПараметрыДляСозданияМенеджераКриптографии(Контекст, ТребуетсяЗакрытыйКлюч)
	...
	Исключение
		...
		Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить приложение электронной подписи и шифрования для переданных данных: %1'"),
	...
	ИначеЕсли ТипДанных = "ЗашифрованныеДанные" Тогда
		...
		Результат.Контекст.Программа = Неопределено;
		Результат.Ошибка = НСтр("ru = 'Для определения приложения для расшифровки в процедуру должны быть переданы данные сертификата.'");
		Возврат Результат;
		
	КонецЕсли;
	
	Результат.Контекст.Программа = Неопределено;
	Результат.Ошибка = НСтр("ru = 'Не удалось определить приложение электронной подписи и шифрования для переданных данных.'");
...
Процедура СоздатьМенеджерКриптографииЦиклПослеИнициализации(Неопределен, Контекст) Экспорт
	...
	АлгоритмШифрования = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст, "АлгоритмШифрования", "");
...
Асинх Функция ОбъектВнешнейКомпонентыExtraCryptoAPI(ПредложитьУстановить = Истина, ТекстПояснения = Неопределено) Экспорт
	...
	Если Результат.Подключено Тогда
		...
		ЗаполнитьПутьКЛогуКомпоненты(ОбъектКомпоненты, ОписаниеКомпоненты.ИмяОбъекта);
...
Асинх Процедура ЗаполнитьПутьКЛогуКомпоненты(ОбъектКомпоненты, ИмяФайла)
	ПутьККаталогуДиагностики = ЭлектроннаяПодписьКлиент.ПерсональныеНастройки().ПутьККаталогуДиагностики;
	СистемнаяИнформация = Новый СистемнаяИнформация();
	ПутьКЛогуКомпонентыНаКлиенте = ПутьККаталогуДиагностики.Получить(СистемнаяИнформация.ИдентификаторКлиента);
	Если ЗначениеЗаполнено(ПутьКЛогуКомпонентыНаКлиенте) Тогда
		Ждать ОбъектКомпоненты.УстановитьЛогАсинх(ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(
				ПутьКЛогуКомпонентыНаКлиенте) + ИмяФайла + ".log");
	КонецЕсли;
КонецПроцедуры
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		Результат.Ошибка = ТекстОшибкиУстройствоНеПоддерживается();
		Возврат Результат;
#КонецЕсли
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для получения данных о приложении для работы с сертификатом электронной подписи требуется установка внешней компоненты %1.'"),
			"ExtraCryptoAPI");
	...
	Если ТребуетсяЗакрытыйКлюч <> Ложь Тогда
		...
		Если РасширенноеОписаниеПрограммы <> Неопределено Тогда
			...
			Возврат Результат;
		ИначеЕсли ТребуетсяЗакрытыйКлюч = Истина И ЗначениеЗаполнено(ТекущийКриптопровайдер) И ТекущийКриптопровайдер.Получить("type") <> 0 Тогда
			Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сертификат связан с приложением %1 с типом %2, настройте его в справочнике для дальнейшего использования.'"),
...
Функция ТекстОшибкиНеУдалосьОпределитьПрограммуПоЗакрытомуКлючуСертификата()
	...
	Возврат НСтр("ru = 'Не удалось определить приложение электронной подписи и шифрования по закрытому ключу сертификата.'");
...
//           - Неопределено - в случае возникновения известной ошибки.
//
Асинх Функция КонтейнерыПоСертификату(Сертификат, ОбъектКомпоненты = Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(СвойстваСертификатаРезультат.Ошибка) Тогда
		ОшибкаПоКлассификатору = ЭлектроннаяПодписьСлужебныйВызовСервера.ОшибкаПоКлассификатору(СвойстваСертификатаРезультат.Ошибка);
		Если ОшибкаПоКлассификатору = Неопределено Тогда
			ВызватьИсключение СвойстваСертификатаРезультат.Ошибка;
		Иначе
			ПараметрыОшибки = Новый Структура;
			ПараметрыОшибки.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Не удалось получить контейнеры по сертификату'"));
			ПараметрыОшибки.Вставить("ТекстОшибкиКлиент", СвойстваСертификатаРезультат.Ошибка);
			ПараметрыОшибки.Вставить("ПоказатьТребуетсяПомощь", Истина);
			ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыОшибки);
...
Функция ПараметрыДобавленияСертификата() Экспорт
	...
	Структура.Вставить("НаТокене", Ложь);
...
Процедура ДобавитьСертификатПослеВыбораНазначения(Назначение, ПараметрыСоздания) Экспорт
	...
	Если Назначение = "ЗаявлениеНаВыпускСертификата" Тогда
		...
		ПараметрыФормы.Вставить("НаТокене", ПараметрыСоздания.НаТокене);
	...
	Если Назначение = "ДляПодписанияШифрованияИРасшифровкиИзФайлов" Тогда
		ПараметрыФормы.Вставить("ОбработчикЗавершения", ПараметрыСоздания.ОбработчикЗавершения);
		...
		ПараметрыФормы.Вставить("Организация", ПараметрыСоздания.Организация);
		ПараметрыФормы.Вставить("ОтображатьКомандыДобавления", Ложь);
		ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайла(ПараметрыСоздания);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьСертификатПослеПолученияСвойствСертификатовНаКлиенте(Результат, Контекст) Экспорт
	...
	Если Не Форма.Открыта() И Не ПустаяСтрока(Форма.ТекстПредупрежденияПриОткрытии) Тогда
		ПоказатьПредупреждение(, Форма.ТекстПредупрежденияПриОткрытии);
	КонецЕсли;
...
Процедура ПолучитьСвойстваСертификатовНаКлиенте(Оповещение, Личные, БезОтбора, ТолькоОтпечатки = Ложь, ПоказатьОшибку = Неопределено) Экспорт
	...
	Контекст.Вставить("Результат",       Результат);
	Контекст.Вставить("МассивСертификатов", Новый Массив);
...
// Продолжение процедуры ПолучитьСвойстваСертификатовНаКлиенте.
Асинх Процедура ПолучитьСвойстваСертификатовНаКлиентеПослеПолученияВсех(Контекст)
	
	Подключено = Ждать ПодключитьРасширениеРаботыСКриптографиейАсинх();
	Если Не Подключено Тогда
		ПолучитьСвойстваСертификатовНаКлиентеВернутьРезультат(Контекст);
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ПослеПолученияСвойствСертификатовНаТокенах", ЭтотОбъект, Контекст);
	СтандартнаяОбработка = Истина;
	ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииТокенов(Оповещение, Истина, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
	ПолучитьСвойстваСертификатовНаКлиентеВернутьРезультат(Контекст);
...
Асинх Процедура ПослеПолученияСвойствСертификатовНаТокенах(Токены, Контекст) Экспорт
	
	Контекст.МассивСертификатов = Ждать ЭлектроннаяПодписьКлиентЛокализация.ДобавитьСертификатыСТокеновВМассив(Токены, Контекст.МассивСертификатов);
	ПолучитьСвойстваСертификатовНаКлиентеВернутьРезультат(Контекст);
	
КонецПроцедуры

Процедура ПолучитьСвойстваСертификатовНаКлиентеВернутьРезультат(Контекст)
	
	Если Контекст.МассивСертификатов.Количество() = 0 Тогда
...
// Только для внутреннего назначения.
Асинх Процедура ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайла(ПараметрыСоздания, ПовторныйВызов = Ложь)
	
	Если Не ПовторныйВызов Тогда
		ОповещениеПриОтсутствииКомпонентРаботыСЭП = Новый ОписаниеОповещения(
			"ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайлаПовторноПослеУстановкиКомпонент", ЭтотОбъект, ПараметрыСоздания);
		ВыполняетсяУстановкаДополнительныхКомпонент = Ждать ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(
			ОповещениеПриОтсутствииКомпонентРаботыСЭП);
		Если ВыполняетсяУстановкаДополнительныхКомпонент Тогда
			Возврат;
	...
	Оповещение = Новый ОписаниеОповещения("ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайлаПослеПомещенияФайлов",
...
// Повторный вызов процедуры ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайла после установки компонент.
Процедура ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайлаПовторноПослеУстановкиКомпонент(Результат, ПараметрыСоздания) Экспорт
	
	Если Результат = Истина Тогда
		ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайла(ПараметрыСоздания, Истина);
	КонецЕсли;
	
КонецПроцедуры

// Продолжение процедуры ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайла.
Процедура ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайлаПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	
	Если Не ЗначениеЗаполнено(ПомещенныеФайлы) Тогда
		Возврат;
	КонецЕсли;
	
	АдресСертификата = ПомещенныеФайлы.Хранение;
	
	ДанныеСертификата = ПолучитьИзВременногоХранилища(АдресСертификата);
	ЭтоСертификат = ЭлектроннаяПодписьСлужебныйВызовСервера.ФайлЯвляетсяСертификатом(ДанныеСертификата);
	Если Не ЭтоСертификат Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Файл сертификата должен быть в формате DER X.509.'"));
		Возврат;
	КонецЕсли;
	
	ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайлаПослеПомещенияФайла(АдресСертификата, Контекст);
	
КонецПроцедуры

// Продолжение процедуры ДобавитьСертификатТолькоДляШифрованияИзФайла.
Асинх Процедура ДобавитьСертификатДляПодписанияИлиРасшифровкиИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("АдресСертификата", Адрес);
	
	ОбработчикЗавершения = ?(Контекст.Свойство("ОбработчикЗавершения"), Контекст.ОбработчикЗавершения, Неопределено);
	
	ПараметрыУстановкиСертификата = ПараметрыУстановкиСертификата(Адрес);
	ПараметрыУстановкиСертификата.ОповещениеОЗавершении = ?(ОбработчикЗавершения = Неопределено,
		Новый ОписаниеОповещения("ПослеДобавленияСертификатаИзФайлов", ЭтотОбъект, ПараметрыОповещения),
		Контекст.ОбработчикЗавершения);
		
	Контейнеры = Ждать КонтейнерыПоСертификату(Адрес);
	
	Если Контейнеры = Неопределено Тогда
		Возврат;
	ИначеЕсли Контейнеры.Количество() = 0 Тогда
		ПараметрыФормы = ЭлектроннаяПодписьКлиент.ПараметрыФормыРасширенногоПредставленияОшибки();
		ПараметрыФормы.ЗаголовокПредупреждения = НСтр("ru = 'Не удалось установить сертификат'");
		ПараметрыФормы.ТекстОшибкиКлиент = НСтр("ru = 'Не найден контейнер ключа электронной подписи.'");
		ПараметрыФормы.ДополнительныеДанные.Вставить("Сертификат", Адрес);
		ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, , ПараметрыУстановкиСертификата.ОповещениеОЗавершении);
	Иначе
		ПараметрыУстановкиСертификата.Хранилище = Новый Структура("Значение, Представление", "MY", НСтр("ru = 'Личное хранилище сертификатов'"));
		ПараметрыУстановкиСертификата.СвойстваКонтейнера = Контейнеры[0];
		УстановитьСертификатПослеВыбораВариантаУстановки(ПараметрыУстановкиСертификата);
	КонецЕсли;
	
КонецПроцедуры

// Только для внутреннего использования.
Процедура ПослеДобавленияСертификатаИзФайлов(Результат, Параметры) Экспорт
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("АдресСертификата", Результат.Сертификат);
	ВыборСертификатаДляПодписанияИлиРасшифровки(ПараметрыФормы, ЭтотОбъект);
	
КонецПроцедуры

// Только для внутреннего назначения.
Асинх Процедура ДобавитьСертификатТолькоДляШифрованияИзФайла(ПараметрыСоздания, МножественныйВыбор = Ложь, ПовторныйВызов = Ложь) Экспорт
	
	Если Не ПовторныйВызов Тогда
		ПараметрыПовторногоВызова = Новый Структура("ПараметрыСоздания, МножественныйВыбор",
			ПараметрыСоздания, МножественныйВыбор);
		ОповещениеПриОтсутствииКомпонентРаботыСЭП = Новый ОписаниеОповещения(
			"ДобавитьСертификатТолькоДляШифрованияИзФайлаПовторноПослеУстановкиКомпонент", ЭтотОбъект, ПараметрыПовторногоВызова);
		ВыполняетсяУстановкаДополнительныхКомпонент = Ждать ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(
			ОповещениеПриОтсутствииКомпонентРаботыСЭП);
		Если ВыполняетсяУстановкаДополнительныхКомпонент Тогда
			Возврат;
...
// Повторный вызов процедуры ДобавитьСертификатТолькоДляШифрованияИзФайла после установки компонент.
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПовторноПослеУстановкиКомпонент(Результат, ПараметрыПовторногоВызова) Экспорт
	
	Если Результат = Истина Тогда
		ДобавитьСертификатТолькоДляШифрованияИзФайла(
			ПараметрыПовторногоВызова.ПараметрыСоздания, ПараметрыПовторногоВызова.МножественныйВыбор, Истина);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Файл сертификата должен быть в формате DER X.509.'"));
		Возврат;
	КонецЕсли;
	
	Если Не Форма.Открыта() Тогда
		Если ПустаяСтрока(Форма.ТекстПредупрежденияПриОткрытии) Тогда
			...
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПредупрежденияОСуществующем",
					ЭтотОбъект, Форма.СертификатОтпечаток),
...
Процедура ДобавитьСертификатыТолькоДляШифрованияИзФайлаПослеПомещенияФайлов(ПомещенныеФайлы, Контекст)
	...
	ОбработчикЗавершения = ?(Контекст.Свойство("ОбработчикЗавершения"), Контекст.ОбработчикЗавершения, Неопределено);
	Форма = ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.ДобавлениеСертификатовДляШифрования",
		ПараметрыФормы, , , , , ОбработчикЗавершения);
	
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'В выбранном каталоге отсутствуют сертификаты для добавления.'"));
		Возврат;
	КонецЕсли;
...
// Только для внутреннего назначения.
Асинх Процедура ДобавитьСертификатТолькоДляШифрованияИзКаталога(ПараметрыСоздания)
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(Оповещение, ТекстПредложения, Ложь);
...
Асинх Процедура ПоказатьОшибкуОбращенияКПрограммеПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	
	Контекст.ПараметрыФормы.Вставить("РасширениеПодключено", Подключено);
	
#Если ВебКлиент Тогда
	Если Не Подключено Тогда
		Компоненты = НовыйКомпонентыРаботыСКриптографией();
		Компоненты.РасширениеРаботыСКриптографиейУстановлено = Ложь;
		
		ТребуетсяУстановкаДополнительныхКомпонент = Ждать ТребуетсяУстановкаДополнительныхКомпонент(Компоненты);
		
		Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("Компоненты", Компоненты);
			ПараметрыФормы.Вставить("ИмяТребуемойКомпоненты", "РасширениеРаботыСКриптографией");
			ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы,
				, , , , Контекст.ОбработкаПродолжения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			Возврат;
		КонецЕсли;
	КонецЕсли;
#КонецЕсли
...
Процедура ОткрытьНовуюФорму(ВидФормы, КлиентскиеПараметры, СерверныеПараметры, ОбработкаЗавершения) Экспорт
	...
	Если ЭлектроннаяПодписьКлиент.СоздаватьЭлектронныеПодписиНаСервере()
	   И СерверныеПараметры.Свойство("ВыполнятьНаСервере")
	   И СерверныеПараметры.ВыполнятьНаСервере = Истина Или ВидФормы = "ОтправитьНаПодписание" Тогда
...
Процедура ПроверитьСертификатСправочника(Сертификат, ДополнительныеПараметры) Экспорт
	...
	СерверныеПараметры.Вставить("ТипПодписи");
	СерверныеПараметры.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные());
	СерверныеПараметры.Вставить("ИгнорироватьПроверкуВСпискеОтозванныхСертификатов", Ложь);
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		Если ДополнительныеПараметры.Свойство("КонтекстОперации")
			...
			Если ДополнительныеПараметры.КонтекстОперации.ИмяФормы = НачалоИмениФормы + "ПодписаниеДанных" Тогда
				ДополнительныеПараметры.Вставить("ТипПодписи", ДополнительныеПараметры.КонтекстОперации.ТипПодписи);
				ДополнительныеПараметры.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", ДополнительныеПараметры.КонтекстОперации.ПроверятьСертификат);
			КонецЕсли;
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Требуется приложение электронной подписи и шифрования'"),
...
Процедура СохранитьДанныеВместеСПодписьюПослеВыбораПодписей(КоллекцияПодписей, Контекст) Экспорт
	...
	Контекст.Вставить("СохранятьСертификатВместеСПодписью",
		ЭлектроннаяПодписьКлиент.ПерсональныеНастройки().СохранятьСертификатВместеСПодписью);
		
	Контекст.Вставить("КоллекцияПодписей", КоллекцияПодписей);
		
	МашиночитаемыеДоверенности = Новый Массив;
	ЭлектроннаяПодписьКлиентЛокализация.ПриОпределенииФайловМашиночитаемыхДоверенностей(МашиночитаемыеДоверенности,
		Контекст.КоллекцияПодписей);
	
	Контекст.Вставить("МашиночитаемыеДоверенности", МашиночитаемыеДоверенности);
	Контекст.Вставить("ВыгруженныеСертификаты", Новый Соответствие);
...
// Продолжение процедуры ЭлектроннаяПодписьКлиент.СохранитьДанныеВместеСПодписью.
Асинх Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
...
Процедура СохранитьДанныеВместеСПодписьюЦиклПослеСохраненияПодписи(Результат, Контекст) Экспорт
	...
	Если Контекст.СохранятьСертификатВместеСПодписью И Контекст.ВыгруженныеСертификаты.Получить(
		Контекст.ОписаниеПодписи.Отпечаток) = Неопределено Тогда
		Контекст.Вставить("Счетчик", 1);
...
Процедура СохранитьДанныеСертификатаВместеСПодписьюЦиклПослеВнутреннегоЦикла(Контекст)
	...
	Контекст.ВыгруженныеСертификаты.Вставить(Контекст.ОписаниеПодписи.Отпечаток, Истина);
...
Процедура СохранитьДанныеВместеСПодписьюПослеЦикла(Контекст)
	...
	Контекст.Вставить("ИндексДоверенности", -1);
	СохранитьМашиночитаемыеДоверенностиЦиклНачало(Контекст);
	
КонецПроцедуры

// Продолжение процедуры ЭлектроннаяПодписьКлиент.СохранитьДанныеВместеСПодписью.
Процедура СохранитьМашиночитаемыеДоверенностиЦиклНачало(Контекст)
	
	Если Контекст.МашиночитаемыеДоверенности.Количество() <= Контекст.ИндексДоверенности + 1 Тогда
		СохранитьДанныеВместеСПодписьюПослеЦиклаСохраненияМашиночитаемыхДоверенностей(Контекст);
		Возврат;
	КонецЕсли;
	
	Контекст.ИндексДоверенности = Контекст.ИндексДоверенности + 1;
	ВыгружаемаяДоверенность = Контекст.МашиночитаемыеДоверенности[Контекст.ИндексДоверенности];
	
	Если Контекст.Подключено Тогда
		Описание = Новый ОписаниеПередаваемогоФайла(ВыгружаемаяДоверенность.Имя, ВыгружаемаяДоверенность.Хранение);
		Контекст.ПолучаемыеФайлы.Добавить(Описание);
		СохранитьМашиночитаемыеДоверенностиЦиклНачало(Контекст);
	Иначе
		// Сохранение файла из программы на компьютер.
		ФайловаяСистемаКлиент.СохранитьФайл(
			Новый ОписаниеОповещения("СохранитьДанныеВместеСПодписьюПослеЦиклаСохраненияМашиночитаемойДоверенности", ЭтотОбъект, Контекст),
			 ВыгружаемаяДоверенность.Хранение, ВыгружаемаяДоверенность.Имя);
	КонецЕсли;
...
// Продолжение процедуры ЭлектроннаяПодписьКлиент.СохранитьДанныеВместеСПодписью.
Процедура СохранитьДанныеВместеСПодписьюПослеЦиклаСохраненияМашиночитаемыхДоверенностей(Контекст)
...
// Продолжение процедуры УстановитьРасширение.
Асинх Процедура УстановитьРасширениеПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
...
Процедура УстановитьРасширениеПослеОтвета(Ответ, Контекст) Экспорт
	...
	Если Ответ = Истина Или Ответ = КодВозвратаДиалога.Да Тогда
...
#Область ОбщиеПроцедурыИФункцииУправляемыхФорм
...
Процедура ПродолжитьОткрытиеНачало(Оповещение, Форма, КлиентскиеПараметры, Шифрование = Ложь, Расшифровка = Ложь) Экспорт
	...
	Контекст.Вставить("Операция", ?(Расшифровка, "Расшифровка", "Подписание"));
...
Асинх Процедура ПродолжитьОткрытиеНачалоПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") И Не ИспользоватьЭлектроннуюПодписьВМоделиСервиса()
		И Не ИспользоватьСервисОблачнойПодписи() Тогда

		Если Контекст.Операция = "Подписание" Или Контекст.Операция = "Расшифровка" Тогда
			
			Оповещение = Новый ОписаниеОповещения("ПродолжитьОткрытиеПослеПолученияОтпечатковНаТокенах", ЭтотОбъект, 
				Новый Структура("Результат, Контекст", Результат, Контекст));
			СтандартнаяОбработка = Истина;
			ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииОтпечатковСертификатовНаТокенах(Оповещение, Истина, Ложь,
				СтандартнаяОбработка);
			Если Не СтандартнаяОбработка Тогда
				Возврат;
			ИначеЕсли Контекст.Операция = "Подписание" И ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ИспользоватьСервисМобильнойПодписи Тогда
				ВыполнитьОбработкуОповещения(Контекст.Оповещение, Истина);
				Возврат;
			КонецЕсли;
		КонецЕсли;
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Требуется приложение электронной подписи и шифрования'"), "", Результат,
...
// Продолжение процедуры ПродолжитьОткрытиеНачало.
Процедура ПродолжитьОткрытиеПослеПолученияОтпечатковНаТокенах(ОтпечаткиСертификатовНаТокенах, ДополнительныеПараметры) Экспорт
	
	Контекст = ДополнительныеПараметры.Контекст;
	
	Если ОтпечаткиСертификатовНаТокенах.Количество() > 0 Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Истина);
		Возврат;
	КонецЕсли;
	
	ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru = 'Требуется приложение электронной подписи и шифрования'"), "", ДополнительныеПараметры.Результат,
		Контекст.ОшибкаНаСервере);
...
Процедура ПолучитьОтпечаткиСертификатовНаКлиентеПослеСозданияМенеджераКриптографии(МенеджерКриптографии, Контекст) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПослеПолученияОтпечатковНаТокенах", ЭтотОбъект, 
		Новый Структура("Контекст, МенеджерКриптографии", Контекст, МенеджерКриптографии));
	СтандартнаяОбработка = Истина;
	ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииОтпечатковСертификатовНаТокенах(
		Оповещение, Контекст.ВключаяПросроченные, Ложь, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
	ВыполнитьОбработкуОповещения(Оповещение, Новый Массив);
	
КонецПроцедуры

Процедура ПослеПолученияОтпечатковНаТокенах(Массив, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(ДополнительныеПараметры.МенеджерКриптографии) <> Тип("МенеджерКриптографии") Тогда
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.Контекст.Оповещение, Массив);
		Возврат;
	КонецЕсли;
	
	ДополнительныеПараметры.Контекст.Вставить("МассивОтпечатков", Массив);
	
	ДополнительныеПараметры.МенеджерКриптографии.НачатьПолучениеХранилищаСертификатов(
		Новый ОписаниеОповещения(
			"ПолучитьОтпечаткиСертификатовНаКлиентеПослеПолученияХранилища", ЭтотОбъект, ДополнительныеПараметры.Контекст),
		ТипХранилищаСертификатовКриптографии.ПерсональныеСертификаты);
...
Процедура ПолучитьОтпечаткиСертификатовНаКлиентеПослеПолученияВсех(МассивСертификатов, Контекст) Экспорт
	...
	ОтпечаткиСертификатовНаКлиенте = Контекст.МассивОтпечатков;
...
Процедура ОбработатьПарольВФорме(Форма, ВнутренниеДанные, СвойстваПароля, ДополнительныеПараметры = Неопределено, НовыйПароль = Null) Экспорт
	...
	Если ТипЗнч(СвойстваПароля) <> Тип("Структура") Тогда
		...
		СвойстваПароля.Вставить("ВидимостьПароля", Ложь);
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.Свойство("ПриЧтенииСертификатовТокена") Тогда
	...
	Если Не ДополнительныеПараметры.Свойство("ПриИзмененииВидимостиПароля") Тогда
		ДополнительныеПараметры.Вставить("ПриИзмененииВидимостиПароля", Ложь);
	КонецЕсли;
	...
	Элементы = Форма.Элементы;
	СвойстваПароля.Вставить("РежимПароля", Элементы.Пароль.РежимПароля);
	...
	ИначеЕсли ДополнительныеПараметры.ВводитьПарольВПрограммеЭлектроннойПодписи Тогда
		...
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПояснениеУсиленногоПароля;
		Элементы.ПояснениеУсиленногоПароля.Подсказка =
			НСтр("ru = 'Для выбранного сертификата указано ""Вводить пароль в приложении электронной подписи"".'");
...
Асинх Процедура ВыполнитьНаСтороне(Оповещение, Операция, СторонаВыполнения, ПараметрыВыполнения) Экспорт
	...
	АлгоритмШифрования = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст.ОписаниеДанных, "АлгоритмШифрования", "");
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					ДанныеСертификата = ПолучитьИзВременногоХранилища(ПараметрыВыполнения.СертификатАдрес);
					
					СертификатКриптографии = Новый СертификатКриптографии;
					Ждать СертификатКриптографии.ИнициализироватьАсинх(ДанныеСертификата);
					Контекст.Вставить("СертификатКриптографии", СертификатКриптографии);
					Контекст.Вставить("СвойстваСертификата", Ждать СвойстваСертификата(СертификатКриптографии));
					Контекст.СвойстваСертификата.Вставить("ДвоичныеДанные", ДанныеСертификата);
					
					Если Операция = "Шифрование" Тогда
						
						Оповещение = Новый ОписаниеОповещения("ПослеПоискаТокенаДляШифрования", ЭтотОбъект, 
							Новый Структура("Контекст, АлгоритмШифрования, ОписаниеОшибки",
							Контекст, АлгоритмШифрования, ПрограммаПоСертификатуРезультат.Ошибка));
						СтандартнаяОбработка = Истина;
						ЭлектроннаяПодписьКлиентЛокализация.ПриПоискеТокенаДляШифрования(Оповещение, СертификатКриптографии, АлгоритмШифрования,
							Контекст.СвойстваСертификата, СтандартнаяОбработка);
						Если Не СтандартнаяОбработка Тогда
							Возврат;
						КонецЕсли;
					...
					Иначе
						...
						Оповещение = Новый ОписаниеОповещения("ПослеПоискаТокенаПоСертификату", ЭтотОбъект, 
								Новый Структура("Контекст, АлгоритмШифрования, ОписаниеОшибки", 
								Контекст, АлгоритмШифрования, ПрограммаПоСертификатуРезультат.Ошибка));
						СтандартнаяОбработка = Истина;
						ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииТокенаПоСертификату(
							Оповещение, Контекст.СертификатКриптографии, СтандартнаяОбработка, Истина);
						Если Не СтандартнаяОбработка Тогда
							Возврат;
						КонецЕсли;
...
Асинх Процедура ПослеПоискаТокенаДляШифрования(Токен, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Токен) Тогда
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Не найден токен для шифрования данных.'"));
#Иначе
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ДополнительныеПараметры.ОписаниеОшибки);
#КонецЕсли
		ОшибкаНаКлиенте.Вставить("Инструкция", Истина);
		ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, ДополнительныеПараметры.Контекст);
		Возврат;
	КонецЕсли;
	
	ДополнительныеПараметры.Контекст.Вставить("МенеджерКриптографии", Токен);
	ДополнительныеПараметры.Контекст.Вставить("АлгоритмШифрования", ДополнительныеПараметры.АлгоритмШифрования);
	ВыполнитьНаСторонеПодготовкаСертификатовЦиклНачало(ДополнительныеПараметры.Контекст);
		
КонецПроцедуры


Асинх Процедура ПослеПоискаТокенаПоСертификату(Токен, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Токен) Тогда
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Не найден токен с указанным сертификатом.'"));
#Иначе
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ДополнительныеПараметры.ОписаниеОшибки);
#КонецЕсли
	...
	ДополнительныеПараметры.Контекст.Вставить("МенеджерКриптографии", Токен);
	Токен.Вставить("Пароль", ДополнительныеПараметры.Контекст.ЗначениеПароля);
	ДополнительныеПараметры.Контекст.Удалить("ЗначениеПароля");
	ВыполнитьНаСторонеЦиклЗапуск(ДополнительныеПараметры.Контекст);
...
Процедура ВыполнитьНаСторонеПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.Операция = "Шифрование" Тогда

			Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст, "ИспользоватьОблачнуюПодпись", Истина) = Истина
				И ШифроватьДанныеОблачнойПодписью() Тогда
				Контекст.Вставить("МенеджерКриптографии", "ОблачнаяПодпись");
				ВыполнитьНаСторонеОблачнойПодписи(Контекст);
			ИначеЕсли ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("Токен") Тогда
				Контекст.Вставить("МенеджерКриптографии", Результат);
...
//   Контекст - Структура
//
Асинх Процедура ВыполнитьНаСторонеПодготовкаСертификатовЦиклНачало(Контекст)
	...
	Для Каждого СертификатШифрования Из СвойстваСертификатов Цикл
		СертификатКриптографии = Новый СертификатКриптографии;
		Ждать СертификатКриптографии.ИнициализироватьАсинх(СертификатШифрования.Сертификат);
...
Асинх Процедура ВыполнитьНаСторонеПослеВыгрузкиСертификата(ВыгруженныеДанные, Контекст) Экспорт
	...
	Если Контекст.Операция = "Подписание" И ТипЗнч(Контекст.МенеджерКриптографии) = Тип("МенеджерКриптографии")
		И ЗначениеЗаполнено(Контекст.МенеджерКриптографии.АлгоритмПодписи) Тогда

		Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
			АлгоритмСертификата = Контекст.СвойстваСертификата.АлгоритмОткрытогоКлюча;
		Иначе
			АлгоритмСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмПодписиСертификата(
				ВыгруженныеДанные, Ложь, Истина);
		КонецЕсли;

		Результат = ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмПодписиСоответствуетСертификату(
			Контекст.СвойстваСертификата.Представление, АлгоритмСертификата,
			Контекст.МенеджерКриптографии.АлгоритмПодписи);

		Если Результат <> Истина Тогда

			ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", Результат);
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, Новый Структура("Ошибка", ОшибкаНаКлиенте));
			Возврат;
...
Процедура ВыполнитьНаСторонеЦиклПослеПолученияДанных(Результат, Контекст) Экспорт
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig  И ТипЗнч(Контекст.МенеджерКриптографии) = Тип("Структура") Тогда
			Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'Создать подпись XMLDSig с помощью механизмов токена можно будет в следующих версиях приложения.'"));
			ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
			Возврат;
		КонецЕсли;
		...
		ИначеЕсли ЭтоCMS Тогда
			...
			Если МенеджерКриптографии = "СервисКриптографии" Тогда
				Если Результат.ПараметрыCMS.ВключениеСертификатовВПодпись <> РежимВключенияСертификатовКриптографии.ВключатьСертификатСубъекта Тогда
					Ошибка = Новый Структура("ОписаниеОшибки",
						НСтр("ru = 'Подписание сертификатом с приложением встроенного криптопровайдера недоступно.
			...
			ИначеЕсли МенеджерКриптографии = "ОблачнаяПодпись" Тогда
			...
			ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
				
				ПараметрыПодписи = ПараметрыПодписиНаТокене(МенеджерКриптографии, Контекст.СертификатКриптографии,
							Контекст.ОписаниеДанных.ТипПодписи);
				ПараметрыПодписи.Открепленная = Результат.ПараметрыCMS.Открепленная;
				СтандартнаяОбработка = Истина;
				ОповещениеПослеОперацииНаТокене = Новый ОписаниеОповещения("ПослеПодписанияНаТокене",
					ЭтотОбъект, Контекст);
					
				ЭлектроннаяПодписьКлиентЛокализация.ПриПодписанииНаТокене(ОповещениеПослеОперацииНаТокене,
					ПараметрыПодписи, Данные, СтандартнаяОбработка);
				Если Не СтандартнаяОбработка Тогда
					Возврат;
				КонецЕсли;
				ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, 
							НСтр("ru = 'Не найдена процедура подписания для токена.'"));
			Иначе
				ОповещениеУспех = Новый ОписаниеОповещения(
					"ВыполнитьНаСторонеЦиклПослеОперацииНаКлиентеXMLDSig", ЭтотОбъект, Контекст);
		...
		Иначе
			...
			ИначеЕсли Контекст.Операция = "Подписание" Тогда
				...
				Иначе
					...
					Если ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
			...
			ИначеЕсли Контекст.Операция = "Шифрование" Тогда
				...
				ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
					...
					ПараметрыШифрования = ПараметрыШифрованияНаТокене(МенеджерКриптографии,
						Контекст.СертификатыШифрования, Контекст.АлгоритмШифрования);

					СтандартнаяОбработка = Истина;
					ОповещениеПослеОперацииНаТокене = Новый ОписаниеОповещения("ПослеШифрованияНаТокене", ЭтотОбъект, Контекст);
					ЭлектроннаяПодписьКлиентЛокализация.ПриШифрованииНаТокене(ОповещениеПослеОперацииНаТокене,
						ПараметрыШифрования, Данные, СтандартнаяОбработка);
					Если Не СтандартнаяОбработка Тогда
						Возврат;
					КонецЕсли;
					
					ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, 
						НСтр("ru = 'Не найдена процедура шифрования для токена.'"));
			...
			Иначе
				...
				ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
					...
					ПараметрыРасшифровки = ПараметрыРасшифровкиНаТокене(МенеджерКриптографии, 
						Контекст.СертификатКриптографии);
					ОповещениеПослеОперацииНаТокене = Новый ОписаниеОповещения("ПослеРасшифровкиНаТокене",
							ЭтотОбъект, Контекст);
					СтандартнаяОбработка = Истина;
					ЭлектроннаяПодписьКлиентЛокализация.ПриРасшифровкеНаТокене(ОповещениеПослеОперацииНаТокене,
						ПараметрыРасшифровки, Данные, СтандартнаяОбработка);
					Если Не СтандартнаяОбработка Тогда
						Возврат;
					КонецЕсли;
					
					ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, 
						НСтр("ru = 'Не найдена процедура расшифровки для токена.'"));
	...
	ИначеЕсли Контекст.Операция = "Шифрование" Тогда
		...
		АлгоритмШифрования = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст.ОписаниеДанных, "АлгоритмШифрования", "");
		ПараметрыДляСервера.Вставить("АлгоритмШифрования", АлгоритмШифрования);
...
Процедура ПослеПодписанияНаТокене(Подпись,Контекст) Экспорт
		
	Если ТипЗнч(Подпись) = Тип("Строка") Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", Подпись);
		ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
		Возврат;
	КонецЕсли;
	
	ВыполнитьНаСторонеЦиклПослеОперацииНаКлиенте(Подпись, Контекст);
	
КонецПроцедуры

Процедура ПослеШифрованияНаТокене(ЗашифрованныеДанные, Контекст) Экспорт

	Если ТипЗнч(ЗашифрованныеДанные) = Тип("Строка") Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ЗашифрованныеДанные);
		ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
		Возврат;
	КонецЕсли;
	
	ВыполнитьНаСторонеЦиклПослеОперацииНаКлиенте(ЗашифрованныеДанные, Контекст);
	
КонецПроцедуры

Процедура ПослеРасшифровкиНаТокене(РасшифрованныеДанные, Контекст) Экспорт
	
	Если ТипЗнч(РасшифрованныеДанные) = Тип("Строка") Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", РасшифрованныеДанные);
		ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(РасшифрованныеДанные) Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Получены пустые расшифрованные данные.'"));
		ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
		Возврат;
	КонецЕсли;
	
	ВыполнитьНаСторонеЦиклПослеОперацииНаКлиенте(РасшифрованныеДанные, Контекст);
...
Асинх Процедура ВыполнитьНаСторонеЦиклПослеОперацииНаКлиенте(ДвоичныеДанные, Контекст) Экспорт
	...
	Если Контекст.Свойство("МенеджерКриптографии") И Контекст.МенеджерКриптографии = "СервисКриптографии" Тогда
		...
		Если Не ДвоичныеДанные.Выполнено Тогда
			ТекстОшибки = ТекстОшибкиОблачнаяПодпись(Контекст.Операция, ДвоичныеДанные.ИнформацияОбОшибке.Описание, Истина, Истина);
			ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ТекстОшибки);
			ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеПодписания(СвойстваПодписи, Контекст)
	...
	ПодписанныйОбъект = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЭлементДанных, "Объект", Неопределено);
				
	// СтандартныеПодсистемы.МашиночитаемыеДоверенности
	Если Контекст.НаСторонеКлиента И ЗначениеЗаполнено(Контекст.ОписаниеДанных.РезультатПроверкиМЧДДляПодписания)
		...
		РезультатПроверкиПодписиПоМЧД = МодульМашиночитаемыеДоверенностиФНССлужебныйКлиент.РезультатПроверкиПодписиПоМЧД(
			Контекст.ОписаниеДанных.ВыбраннаяДоверенность, 
			?(ТипЗнч(ПодписанныйОбъект) = Тип("ОписаниеОповещения"), Неопределено, ПодписанныйОбъект),
	...
	Если ТипЗнч(ПодписанныйОбъект) <> Тип("ОписаниеОповещения") Тогда
		Если Контекст.НаСторонеКлиента Тогда
			ВерсияОбъекта = Неопределено;
			ЭлементДанных.Свойство("ВерсияОбъекта", ВерсияОбъекта);
			СвойстваПодписи.Вставить("ИдентификаторПодписи", Новый УникальныйИдентификатор);
			...
			Если ЗначениеЗаполнено(Контекст.ОписаниеДанных.РезультатПроверкиМЧДДляПодписания)
				...
				МодульМашиночитаемыеДоверенностиФНССлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
					"МашиночитаемыеДоверенностиФНССлужебныйКлиент");
				РезультатПроверкиПодписиПоМЧД = МодульМашиночитаемыеДоверенностиФНССлужебныйКлиент.РезультатПроверкиПодписиПоМЧД(
					Контекст.ОписаниеДанных.ВыбраннаяДоверенность, ПодписанныйОбъект,
					СвойстваПодписи.Сертификат, Неопределено, Контекст.ОписаниеДанных.РезультатПроверкиМЧДДляПодписания);
				Если ЗначениеЗаполнено(РезультатПроверкиПодписиПоМЧД) Тогда
					СвойстваПодписи.Вставить("РезультатПроверкиПодписиПоМЧД", РезультатПроверкиПодписиПоМЧД);
				КонецЕсли;
			...
			ПредставлениеОшибки = ЭлектроннаяПодписьСлужебныйВызовСервера.ДобавитьПодпись(
				ПодписанныйОбъект, СвойстваПодписи, Контекст.ИдентификаторФормы, ВерсияОбъекта);
			Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ОповеститьОбИзменении(ПодписанныйОбъект);
		ВыполнитьНаСторонеЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
	...
	Попытка
		ВыполнитьОбработкуОповещения(ПодписанныйОбъект, ПараметрыВыполнения);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
...
Процедура ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст)
	...
	Если Ошибка <> Неопределено Тогда
		...
		Если Контекст.Операция = "Расшифровка" Тогда
			ОбработатьОшибкуРасшифровки(Ошибка, Контекст);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьДанныеИзОписанияДанныхПродолжение(Результат, Контекст) Экспорт
	...
	Иначе
		...
		Иначе // Путь к файлу
			...
			ПолучитьДанныеИзОписанияДанныхПродолжениеЗагрузкаИзФайла(Данные, Контекст);
...
Асинх Процедура ПолучитьДанныеИзОписанияДанныхПродолжениеЗагрузкаИзФайла(Данные, Контекст, ПовторныйВызов = Ложь)
	
	Если Не ПовторныйВызов Тогда
		ПараметрыПовторногоВызова = Новый Структура("Данные, Контекст", Данные, Контекст);
		ОповещениеПриОтсутствииКомпонентРаботыСЭП = Новый ОписаниеОповещения(
			"ПолучитьДанныеИзОписанияДанныхПродолжениеЗагрузкаИзФайлаПовторноПослеУстановкиКомпонент", ЭтотОбъект, ПараметрыПовторногоВызова);
		ВыполняетсяУстановкаДополнительныхКомпонент = Ждать ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(
			ОповещениеПриОтсутствииКомпонентРаботыСЭП);
		Если ВыполняетсяУстановкаДополнительныхКомпонент Тогда
			Возврат;
...
Процедура ПолучитьДанныеИзОписанияДанныхПродолжениеЗагрузкаИзФайлаПовторноПослеУстановкиКомпонент(Результат, ПараметрыПовторногоВызова) Экспорт
	
	Если Результат = Истина Тогда
		ПолучитьДанныеИзОписанияДанныхПродолжениеЗагрузкаИзФайла(ПараметрыПовторногоВызова.Данные,
			ПараметрыПовторногоВызова.Контекст, Истина);
	КонецЕсли;
	...
	Если ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ЭтоМодельСервисаСДоступнымУсовершенствованием 
		...
		Если ЗначениеЗаполнено(НастройкиПодключенияСервиса.Ошибка) Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Невозможно усовершенствовать электронную подпись с помощью приложения в Интернете:
				|%1
...
Процедура ПродолжитьУсовершенствованиеПослеОтветаНаВопрос(РезультатВопроса, Контекст) Экспорт
	...
	Иначе
		...
		Результат.Вставить("ТекстОшибки", НСтр("ru='Невозможно усовершенствовать электронную подпись с помощью приложения в Интернете'"));
...
Процедура УсовершенствоватьНаСтороне(Оповещение, СторонаВыполнения, ПараметрыВыполнения)
	...
	Контекст.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра",
		ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат());
	ЗаполнитьЗначенияСвойств(Контекст, ПараметрыВыполнения);
...
Процедура УсовершенствоватьНаСервере(ЭлементДанных, Контекст)
	...
	ПараметрыДляСервера.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра);
	...
	Если ПараметрыДляСервера.ЭтоМодельСервисаСДоступнымУсовершенствованием Тогда
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПродолжитьПослеОперацииУсовершенствованияНаСервере", ЭтотОбъект, Контекст);
...
Процедура ПродолжитьПослеОперацииУсовершенствованияНаСервере(РезультатВыполнения, Контекст) Экспорт
	...
	Если РезультатВыполнения = Неопределено Тогда
		...
		ОшибкаНаСервере.Текст = НСтр("ru='Усовершенствование подписи фоновым заданием не было завершено штатно'");
	...
	Если РезультатВыполнения.Статус = "Ошибка" Тогда
		ОшибкаНаСервере = ОписаниеОшибкиПриУсовершенствовании(Ложь);
		ОшибкаНаСервере.Текст = ОбработкаОшибок.КраткоеПредставлениеОшибки(РезультатВыполнения.ИнформацияОбОшибке);
		УсовершенствоватьНаСторонеПослеОшибки(ОшибкаНаСервере, Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		ОшибкаНаСервере = ОписаниеОшибкиПриУсовершенствовании(Ложь);
		ОшибкаНаСервере.Текст = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		УсовершенствоватьНаСторонеПослеОшибки(ОшибкаНаСервере, Контекст);
		Возврат;
	КонецПопытки;
...
//   * СвойстваПодписи
//   * АлгоритмПодписи
//   * ОшибкаПриСозданииМенеджераКриптографии
//   * ОперацияНачалась
//  Контекст - Структура
//
Процедура ПродолжитьПослеУсовершенствованияНаСервере(Результат, Контекст)
	...
	Если Не Результат.Успех Тогда
		...
		Если Результат.ОшибкаПриСозданииМенеджераКриптографии = Истина Тогда
			ОшибкаНаСервере.ОшибкаСозданияМенеджераКриптографии = Истина;
		КонецЕсли;
		УсовершенствоватьНаСторонеПослеОшибки(ОшибкаНаСервере, Контекст);
	Иначе
...
Асинх Процедура УсовершенствоватьНаСторонеПослеПолученияКонтейнераПодписи(КонтейнерПодписи, Контекст) Экспорт
	...
	Если ПараметрыПодписи.СертификатПоследнейМеткиВремени <> Неопределено Тогда
		...
		ПараметрыПроверки = ДополнительныеПараметрыПроверкиСертификата();
		ПараметрыПроверки.ВыполнятьПроверкуУдостоверяющегоЦентра = Контекст.ВыполнятьПроверкуУдостоверяющегоЦентра;
		ПроверитьСертификат(Оповещение, ПараметрыПодписи.СертификатПоследнейМеткиВремени, Контекст.МенеджерКриптографии,,ПараметрыПроверки);
	Иначе
...
Асинх Процедура ВыполнитьПослеПолученияКонтейнераУсовершенствованнойПодписи(КонтейнерПодписи, Контекст) Экспорт
	...
	Иначе
		...
		СвойстваПодписи.Вставить("ОписаниеСертификата", ПараметрыПодписи.ОписаниеСертификата);
		СвойстваПодписи.Вставить("ДатаПодписи",
			?(ЗначениеЗаполнено(ПараметрыПодписи.НеподтвержденнаяДатаПодписи),
			ПараметрыПодписи.НеподтвержденнаяДатаПодписи, ПараметрыПодписи.ДатаПодписиИзМетки));
...
Процедура ПодключитьКомпоненту(Контекст, ТекстПояснения)
	...
	ОписаниеКомпоненты = ЭлектроннаяПодписьСлужебныйКлиентСервер.ОписаниеКомпоненты();
	Контекст.Вставить("ИмяОбъектаКомпоненты", ОписаниеКомпоненты.ИмяОбъекта);
...
Процедура ПослеПодключенияКомпонентыНастроитьКомпоненту(Контекст)
	...
	ЗаполнитьПутьКЛогуКомпоненты(Контекст.ОбъектКомпоненты, Контекст.ИмяОбъектаКомпоненты);
...
Процедура ПослеПодключенияКомпонентыПослеУстановкиСоответствиеOID(Контекст) Экспорт
	...
	Если ТипЗнч(Контекст.МенеджерКриптографии) = Тип("Структура") Тогда
...
Процедура ОбработатьПароль(ВнутренниеДанные, РеквизитПароль, СвойстваПароля,
	...
	Если ДополнительныеПараметры.ПриИзмененииВидимостиПароля Тогда
		Если Не РеквизитПароль = "****************" И Не СвойстваПароля.РежимПароля Тогда
			СвойстваПароля.Значение = НовыйПароль;
		КонецЕсли;
		ОбновитьРеквизитПароль(РеквизитПароль, СвойстваПароля);
		Возврат;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ПриИзмененииРеквизитаПароль Тогда
		Если РеквизитПароль = "****************" И СвойстваПароля.РежимПароля Тогда
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьРеквизитПароль(РеквизитПароль, СвойстваПароля)
	РеквизитПароль = ?(СвойстваПароля.Значение <> "",
			?(СвойстваПароля.РежимПароля, "****************", СвойстваПароля.Значение),
			?(СвойстваПароля.РежимПароля, "", РеквизитПароль));
КонецПроцедуры
...
Процедура ВыполнитьНаСторонеВМоделиСервиса(Результат, Контекст)
	...
	Если Контекст.Операция = "Подписание" Тогда
		...
		Если ЗначениеЗаполнено(Контекст.ОписаниеДанных.ТипПодписи) Тогда
...
Функция ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ТехнологияСервиса.ЭлектроннаяПодписьВМоделиСервиса") Тогда
		...
		Возврат МодульЭлектроннаяПодписьВМоделиСервисаКлиент.ИспользованиеВозможно() И ОбщегоНазначенияКлиент.РазделениеВключено();
	КонецЕсли;
...
Функция ИспользованиеИнтерактивногоРежимаКриптографииИспользовать()
	...
	Возврат Вычислить("ИспользованиеИнтерактивногоРежимаКриптографии.Использовать"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только свойство системного перечисления платформы.
...
Процедура ДобавитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии")
		...
		Контекст.Вставить("СертификатКриптографии");
		...
		ДобавитьСертификатСТокена(Контекст);
		Возврат;
...
// Продолжение процедуры ЭлектроннаяПодписьКлиент.ДобавитьСертификат.
Асинх Процедура ДобавитьСертификатСТокена(Контекст)
	...
	Если Контекст.СертификатКриптографии = Неопределено Тогда
		СертификатКриптографии = Новый СертификатКриптографии;
		Ждать СертификатКриптографии.ИнициализироватьАсинх(Контекст.ДанныеСертификата);
		Контекст.Вставить("СертификатКриптографии", СертификатКриптографии);
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ДобавитьСертификатСТокенаПослеПолученияТокена", ЭтотОбъект, Контекст);
...
// Продолжение процедуры ЭлектроннаяПодписьКлиент.ДобавитьСертификат.
Процедура ДобавитьСертификатСТокенаПослеПолученияТокена(Токен, Контекст) Экспорт
	...
	Токен.Вставить("Пароль", Контекст.ПарольСертификата);
	Контекст.Вставить("Токен", Токен);
	
	СтандартнаяОбработка = Истина;
	Если Контекст.ДляШифрования = Истина Тогда
		ПараметрыШифрованияНаТокене = ПараметрыШифрованияНаТокене(Токен, Контекст.СертификатКриптографии);
		ЭлектроннаяПодписьКлиентЛокализация.ПриШифрованииНаТокене(Новый ОписаниеОповещения("ДобавитьСертификатСТокенаПослеШифрования", ЭтотОбъект, 
			Контекст), ПараметрыШифрованияНаТокене, Контекст.ДанныеСертификата, СтандартнаяОбработка);
	Иначе
		ПараметрыПодписи = ПараметрыПодписиНаТокене(Токен, 
			Контекст.СертификатКриптографии, ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES"));
		ЭлектроннаяПодписьКлиентЛокализация.ПриПодписанииНаТокене(Новый ОписаниеОповещения("ДобавитьСертификатСТокенаПослеПолученияПодписи", ЭтотОбъект, 
			Контекст), ПараметрыПодписи, Контекст.ДанныеСертификата, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ДобавитьСертификатЦиклПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Результат.Ошибки.Количество() > 0 Тогда
			Если Не (ЗначениеЗаполнено(Контекст.АлгоритмПодписи)
			...
			ИначеЕсли ЗначениеЗаполнено(Контекст.АлгоритмПодписи) 
				И Результат.Ошибки[0].НетАлгоритма 
				И Контекст.ОписанияПрограмм.Количество() = 1 Тогда
				
				Результат.Ошибки[0].ЗаголовокОшибки = Контекст.ЗаголовокОшибкиПрограммы;
				
				ВозможныеАлгоритмы = СтрРазделить(Контекст.АлгоритмПодписи, ",", Ложь);

				Результат.Ошибки[0].Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат связан с не соответствующим ему приложением %1, переустановите сертификат в операционной системе, связав его с приложением электронной подписи с алгоритмом ""%2"".'"),
					Контекст.ОписаниеПрограммы.ИмяПрограммы,
					ВозможныеАлгоритмы[0]);
				Контекст.ОшибкаНаКлиенте.Ошибки.Добавить(Результат.Ошибки[0]);
			КонецЕсли;
		КонецЕсли;
...
Процедура СообщитьОбОшибкеДобавленияСертификата(КонтекстДобавления)
	...
	Если ЗначениеЗаполнено(ОшибкаНаКлиенте) И ОшибкаНаКлиенте.Ошибки.Количество() > 0
		...
		ЕстьОшибкиТокенов = Ложь;
		Для Каждого Ошибка Из ОшибкаНаКлиенте.Ошибки Цикл
			Если ЗначениеЗаполнено(Ошибка.Токен) Тогда
				ЕстьОшибкиТокенов = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;

		Если ЕстьОшибкиТокенов Тогда
			ПоказатьОшибкуОбращенияКПрограмме(КонтекстДобавления.ЗаголовокФормы, "", ОшибкаНаКлиенте, ОшибкаНаСервере,
				ДополнительныеПараметры);
			Возврат;
		КонецЕсли;
...
// Вызывается после добавления в справочник сертификатов электронной подписи.
//
// Параметры:
//  Сертификат								 - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - ссылка на добавленный сертификат
//  ПараметрыОповещенияПриЗаписиСертификата	 - см. ПараметрыОповещенияПриЗаписиСертификата
//
Процедура ПослеДобавленияВСправочникСертификатовЭлектроннойПодписи(Сертификат, ПараметрыОповещенияПриЗаписиСертификата) Экспорт
	
	ОповеститьОбИзменении(Сертификат);
	Оповестить("Запись_СертификатыКлючейЭлектроннойПодписиИШифрования", ПараметрыОповещенияПриЗаписиСертификата, Сертификат);
	Если ПараметрыОповещенияПриЗаписиСертификата.ЭтоНовый Тогда
		ДобавленныеСертификаты = ЭлектроннаяПодписьСлужебныйВызовСервера.ДобавленныеСертификаты(Сертификат);
		ЭлектроннаяПодписьКлиентПереопределяемый.ПослеДобавленияВСправочникСертификатовЭлектроннойПодписи(
			Новый Структура("Сертификаты", ДобавленныеСертификаты));
	КонецЕсли;
...
Процедура ПодписатьОблачнаяПодпись(Оповещение, Контекст, Данные, ПараметрыОперации, ТипРезультата = Неопределено)
	...
	Если ТипЗнч(Данные) = Тип("Структура") 
		И Данные.Свойство("ПараметрыCMS") Тогда
		
		ПараметрыCMS = Данные.ПараметрыCMS;
		Если ПараметрыCMS.ТипПодписи = "CAdES-BES" Тогда
			ТипПодписи = "BES";
		Иначе
			Ошибка = Новый Структура("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недоступно подписание типом подписи %1 с помощью учетной записи DSS.'"), ПараметрыCMS.ТипПодписи));
			ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
			Возврат;
		КонецЕсли;
		СвойствоПодписи = МодульСервисКриптографииDSSКлиентСервер.ПолучитьСвойствоПодписиCAdES(ТипПодписи, ПараметрыCMS.Открепленная, Ложь, "Подпись");
		Данные = Данные.Данные;
		Если ТипЗнч(Данные) = Тип("Строка") Тогда
			Данные = ПолучитьДвоичныеДанныеИзСтроки(Данные);
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(Контекст.ОписаниеДанных.ТипПодписи) Тогда
		...
		ИначеЕсли Контекст.ОписаниеДанных.ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST") Тогда
			СвойствоПодписи = МодульСервисКриптографииDSSКлиентСервер.ПолучитьСвойствоПодписиCAdES("T", Истина, Ложь, "Подпись");
...
Асинх Процедура ВыполнитьНаСторонеЦиклПослеПодписанияПакета(МассивПодписей, Контекст)
	...
	Для Счетчик = 0 По МассивПодписей.Количество() - 1 Цикл
		...
		Иначе // сделать отдельную ветку для возврата в цикл по счетчику
			...
			Контекст.ОписаниеДанных.Вставить("ТекущийЭлементНабораДанных", ЭлементДанных);

			ПараметрыВыполнения = Новый Структура;
			ПараметрыВыполнения.Вставить("ОписаниеДанных", Контекст.ОписаниеДанных);
			ПараметрыВыполнения.Вставить("Оповещение", Новый ОписаниеОповещения(
				"ВыполнитьНаСторонеЦиклПослеЗаписиПакетнойПодписи", ЭтотОбъект, Контекст));
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиПакетнойПодписи(Результат, Контекст) Экспорт
	...
	// Заглушка для пакетного режима, не требуется выполнять цикл выполнения операций.
	Контекст.Индекс = 0;
...
Процедура ОбработатьОшибкуПоКлассификатору(ТекстОшибки, ТребуетПроверки, Контекст, ОшибкаПоКлассификатору = Неопределено)
	...
	Если ОшибкаПоКлассификатору = Неопределено И ЗначениеЗаполнено(ТекстОшибки) Тогда
...
Функция ТекстОшибкиОблачнаяПодпись(Операция, РезультатВызова = Неопределено, ЭтоОшибкаВыполнения = Ложь, ЭтоВстроенныйКриптопровайдер = Ложь) Экспорт
	...
	Если ЭтоВстроенныйКриптопровайдер Тогда
		
		Если Не ОбщегоНазначенияКлиент.РазделениеВключено() Тогда
			ТекстРекомендации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отключите константу ""%1"".'"), НСтр("ru = 'Использовать электронную подпись в модели сервиса'"));
		Иначе
			ТекстРекомендации = РезультатВызова;
		КонецЕсли;
		
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Встроенный криптопровайдер: %1 %2'"), ТекстОшибки, ТекстРекомендации);
	Иначе
...
// Или инициализирует переход на сайт службы поддержки, куда будет отправлена технической информация.
//
// Параметры:
//  Причина				 - Строка - причина сбора технической информации.
//  ПараметрыСообщения	 - Неопределено - если Неопределено, то будет выполнено скачивание файла технической информации.
//                    	 - Структура - если Структура, то будет инициализирован переход на сайт службы поддержки, куда будет отправлена технической информация:
//                * Тема    - Строка - заголовок сообщения.
//                * Сообщение - Строка - содержание сообщения
//  ОбработчикЗавершения - Неопределено,
...
//                     	 - Структура - дополнительные данные, которые необходимо поместить в архив:
//                * Имя    - Строка - имя файла с расширением.
//                * Данные - ДвоичныеДанные
//  					 - Массив - массив из указанных выше структур.
//
Процедура СформироватьТехническуюИнформацию(Причина,
	...
	Контекст = Новый Структура;
	Контекст.Вставить("ВыгрузитьАрхив", ПараметрыСообщения = Неопределено);
	Если ПараметрыСообщения <> Неопределено Тогда
		Контекст.Вставить("Тема", ПараметрыСообщения.Тема);
		Контекст.Вставить("Сообщение");
		Если ЗначениеЗаполнено(ПараметрыСообщения.Сообщение) Тогда
			Контекст.Сообщение = ПараметрыСообщения.Сообщение;
		Иначе
			Контекст.Сообщение = ТекстОбращенияВТехническуюПоддержку();
		КонецЕсли;
	КонецЕсли;
	
	Если ДополнительныеФайлы = Неопределено Тогда
		Файлы = Новый Массив;
	ИначеЕсли ТипЗнч(ДополнительныеФайлы) = Тип("Структура") Тогда
		Файлы = Новый Массив;
		Файлы.Добавить(ДополнительныеФайлы);
	Иначе
		Файлы = ДополнительныеФайлы;
	КонецЕсли;
	
	Контекст.Вставить("ДополнительныеФайлы", Файлы);
	Контекст.Вставить("ОбработчикЗавершения", ОбработчикЗавершения);
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку") Тогда
		СформироватьТехническуюИнформациюСоСнимкомЭкрана(Контекст);
		Возврат;
	КонецЕсли;
	// Конец СтандартныеПодсистемы.ОбращенияВТехническуюПоддержку
	
	Оповещение = Новый ОписаниеОповещения("ПослеСбораТехническойИнформацииОКомпоненте", ЭтотОбъект, Контекст);
	ПолучитьВерсиюКомпоненты(Оповещение);
	
КонецПроцедуры

Процедура СформироватьТехническуюИнформациюСоСнимкомЭкрана(Контекст)
	...
	Если Не МодульОбращенияВТехническуюПоддержкуСлужебныйКлиент.ПриложениеДляСнимкаЭкранаДоступно() Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеСбораТехническойИнформацииОКомпоненте", ЭтотОбъект, Контекст);
		ПолучитьВерсиюКомпоненты(Оповещение);
		Возврат;
	КонецЕсли;
	
	СообщенияВСлужбуТехническойПоддержкиДоступны =
		МодульОбращенияВТехническуюПоддержкуСлужебныйКлиент.СообщенияВСлужбуТехническойПоддержкиДоступны();
	
	// Служба технической поддержки не поддерживает вложения формата "png" или "jpg".
	Если Не Контекст.ВыгрузитьАрхив И СообщенияВСлужбуТехническойПоддержкиДоступны Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеСбораТехническойИнформацииОКомпоненте", ЭтотОбъект, Контекст);
		ПолучитьВерсиюКомпоненты(Оповещение);
		Возврат;
	КонецЕсли;
	...
	ОбработчикЗавершения = Новый ОписаниеОповещения(
		"ПродолжитьФормированиеТехническойИнформацииСоСнимкомЭкрана",
		ЭтотОбъект,
		Контекст);
	
	МодульОбращенияВТехническуюПоддержкуСлужебныйКлиент.ЗапроситьСозданиеСнимкаЭкрана(ОбработчикЗавершения);
	
КонецПроцедуры

Процедура ПродолжитьФормированиеТехническойИнформацииСоСнимкомЭкрана(Результат, Контекст) Экспорт
	
	Если Результат <> Неопределено Тогда
		ДанныеФайла = Новый Структура("Имя, Данные", Результат.ПолноеИмяФайла, Результат.АдресФайла);
		Контекст.ДополнительныеФайлы.Добавить(ДанныеФайла);
	КонецЕсли;
...
Функция ДиагностическаяИнформацияОКомпьютере()
	...
	Возврат НСтр("ru = 'Клиент:'") + Символы.ПС
		+ СтандартныеПодсистемыКлиент.ИнформацияДляПоддержки();
...
Асинх Процедура ДиагностическаяИнформацияОПрограммахОпределенныхАвтоматически(Оповещение)
	...
	Если Не ИспользуемыеПрограммыРезультат.ПроверкаВыполнена Тогда
		ВыполнитьОбработкуОповещения(Оповещение, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось автоматически определить приложение электронной подписи и шифрования на компьютере: %1'"),
			ИспользуемыеПрограммыРезультат.Ошибка));
		Возврат;
	КонецЕсли;
	
	Токены = ЭлектроннаяПодписьСлужебныйКлиентСервер.ТехническаяИнформацияОТокенах(ИспользуемыеПрограммыРезультат.Токены, Истина);
	
	Если ИспользуемыеПрограммыРезультат.Криптопровайдеры.Количество() = 0 Тогда
		ВыполнитьОбработкуОповещения(Оповещение, Токены + Символы.ПС + Символы.ПС + НСтр("ru = 'На компьютере нет приложений электронной подписи и шифрования.'"));
		Возврат;
	КонецЕсли;
	
	Заголовок = Символы.ПС + Токены + Символы.ПС + Символы.ПС + НСтр("ru = 'Автоматически определенные приложения электронной подписи и шифрования на компьютере:'") + Символы.ПС;
...
Процедура ДиагностическаяИнформацияОПрограммах(Оповещение)
	...
	Если ИспользуемыеПрограммы.Количество() = 0 Тогда
		...
		Возврат;
	КонецЕсли;
	Заголовок = Символы.ПС + НСтр("ru = 'Приложения на клиенте из справочника:'") + Символы.ПС;
...
Асинх Процедура ПослеСбораТехническойИнформацииОПрограммахОпределенныхАвтоматически(ИнформацияОПрограммах, Контекст) Экспорт

	Контекст.СопроводительныйТекст = Контекст.СопроводительныйТекст + Символы.ПС + ИнформацияОПрограммах + Символы.ПС;
	Оповещение = Новый ОписаниеОповещения("ПослеСбораТехническойИнформацииОПрограммах", ЭтотОбъект, Контекст);
	...
	ДиагностическаяИнформацияОПрограммах(Оповещение);
	
КонецПроцедуры

Асинх Процедура ПослеСбораТехническойИнформацииОПрограммах(ИнформацияОПрограммах, Контекст) Экспорт
	
	Контекст.СопроводительныйТекст = Контекст.СопроводительныйТекст + ИнформацияОПрограммах + Символы.ПС
		+ РезультатДиагностикиСерверовМетокВремени() + Символы.ПС;
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	КаталогЛогаКомпоненты = ЭлектроннаяПодписьКлиент.ПерсональныеНастройки().ПутьККаталогуДиагностики.Получить(
		СистемнаяИнформация.ИдентификаторКлиента);
		
	Если ЗначениеЗаполнено(КаталогЛогаКомпоненты) Тогда
		
		ЛогКомпоненты = "";
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		ИмяОбъекта = "";
		ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииИдентификатораМобильнойКомпоненты(ИмяОбъекта);
		Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
			ЛогКомпоненты = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(КаталогЛогаКомпоненты)
				+ ИмяОбъекта +".log";
		КонецЕсли;
#Иначе
		ЛогКомпоненты = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(КаталогЛогаКомпоненты)
			+ ЭлектроннаяПодписьСлужебныйКлиентСервер.ОписаниеКомпоненты().ИмяОбъекта + ".log";
#КонецЕсли
		
		Если ЗначениеЗаполнено(ЛогКомпоненты) Тогда
			Файл = Новый Файл(ЛогКомпоненты);
			Если Ждать Файл.СуществуетАсинх() Тогда
				ДвоичныеДанныеФайла = Ждать СоздатьДвоичныеДанныеИзФайлаАсинх(ЛогКомпоненты);
				Если ЗначениеЗаполнено(ДвоичныеДанныеФайла) Тогда
					Контекст.ДополнительныеФайлы.Добавить(
						Новый Структура("Имя, Данные", Файл.ИмяБезРасширения + ".txt", ДвоичныеДанныеФайла));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если Контекст.ВыгрузитьАрхив Тогда
		Контекст.Вставить("АдресАрхива", ЭлектроннаяПодписьСлужебныйВызовСервера.АдресАрхиваТехническойИнформации(
			Контекст.ДополнительныеФайлы, Контекст.СопроводительныйТекст, ПроверенныеПутиКМодулямПрограмм()));
		ФайловаяСистемаКлиент.СохранитьФайл(
			Новый ОписаниеОповещения("СформироватьТехническуюИнформациюПослеСохраненияФайла", ЭтотОбъект, Контекст),
			Контекст.АдресАрхива, "service_info.zip");
		Возврат;
	КонецЕсли;
	
	АдресаФайловТехническойИнформации = ЭлектроннаяПодписьСлужебныйВызовСервера.АдресаФайловТехническойИнформации(
		Контекст.СопроводительныйТекст, ПроверенныеПутиКМодулямПрограмм());
	
	Контекст.ДополнительныеФайлы.Добавить(
		Новый Структура("Имя, Данные", ИмяФайлаТехническойИнформации() + ".txt", АдресаФайловТехническойИнформации.ТехническаяИнформация));
	Контекст.ДополнительныеФайлы.Добавить(
		Новый Структура("Имя, Данные", ИмяФайлаЖурналаРегистрации() + ".xml", АдресаФайловТехническойИнформации.ЖурналРегистрации));
	
	ПараметрыОтправкиВТехподдержку = ПараметрыОтправкиВТехподдержку();
	ПараметрыОтправкиВТехподдержку.Вложения = Контекст.ДополнительныеФайлы;
	ПараметрыОтправкиВТехподдержку.Тема = Контекст.Тема;
	ПараметрыОтправкиВТехподдержку.Сообщение = Контекст.Сообщение;
	ПараметрыОтправкиВТехподдержку.ОбработчикЗавершения = Контекст.ОбработчикЗавершения;
	
	ОтправитьСообщениеВТехподдержку(ПараметрыОтправкиВТехподдержку);
	
КонецПроцедуры

Функция ИмяФайлаТехническойИнформации()
	Возврат НСтр("ru = 'Настройки электронной подписи'");
КонецФункции

Функция ИмяФайлаЖурналаРегистрации()
...
Асинх Процедура ПолучитьВерсиюПослеПодключенияКомпоненты(Результат, Оповещение) Экспорт
...
#Если ВебКлиент Тогда
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
			Компоненты = НовыйКомпонентыРаботыСКриптографией();
			Компоненты.КомпонентаExtraCryptoAPIУстановлена = Ложь;
			
			ТребуетсяУстановкаДополнительныхКомпонент = Ждать ТребуетсяУстановкаДополнительныхКомпонент(Компоненты);
			Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
				ПараметрыФормы = Новый Структура;
				ПараметрыФормы.Вставить("Компоненты", Компоненты);
				ПараметрыФормы.Вставить("ИмяТребуемойКомпоненты", "КомпонентаExtraCryptoAPI");
				ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы,
...
Функция ТекстОшибкиНеУстановленоРасширение()
	...
	Возврат НСтр("ru = 'Не установлено расширение для работы с 1С:Предприятием.'");
...
Асинх Функция ПолучитьСвойстваСертификатаАсинх(Сертификат, ОбъектКомпоненты) Экспорт
	...
	Исключение
		...
		Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно получить расширенные свойства сертификата: %1'"),
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
...
Асинх Функция СертификатBase64Строка(Сертификат)
	...
	ИначеЕсли ТипЗнч(Сертификат) = Тип("ДвоичныеДанные") Тогда
		СертификатКриптографии = Новый СертификатКриптографии;
		Ждать СертификатКриптографии.ИнициализироватьАсинх(Сертификат);
		Сертификат = Ждать СертификатКриптографии.ВыгрузитьАсинх();
	КонецЕсли;
...
Асинх Функция СертификатИзСтроки(СертификатСтрокой) Экспорт
	
	Сертификат = СтрЗаменить(СертификатСтрокой, "-----BEGIN CERTIFICATE-----", "");
	...
	Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
		Возврат НСтр("ru='Не удалось получить сертификат'");
	КонецЕсли;
...
Асинх Процедура УстановитьСписокОтзываСертификатаПослеПодключенияКомпоненты(ОбъектКомпоненты, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(Контекст.Адреса) Тогда
		Если ТипЗнч(Контекст.Адреса) = Тип("Строка") Тогда
			Адреса = СтрРазделить(Контекст.Адреса, ";");
		КонецЕсли;
	Иначе
		Адреса = Новый Массив;
	КонецЕсли;
	...
	Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
		...
		ИначеЕсли ТипЗнч(Контекст.Сертификат) = Тип("ДвоичныеДанные") Тогда
			...
			Ждать Сертификат.ИнициализироватьАсинх(Контекст.Сертификат);
		ИначеЕсли ЭтоАдресВременногоХранилища(Контекст.Сертификат) Тогда
			ДвоичныеДанныеСертификата = ПолучитьИзВременногоХранилища(Контекст.Сертификат);
			Сертификат = Новый СертификатКриптографии;
			Ждать Сертификат.ИнициализироватьАсинх(ДвоичныеДанныеСертификата);
		Иначе
			ДвоичныеДанныеСертификата = Base64Значение(Контекст.Сертификат);
			Сертификат = Новый СертификатКриптографии;
			Ждать Сертификат.ИнициализироватьАсинх(ДвоичныеДанныеСертификата);
		КонецЕсли;
		...
		АдресаСписковОтзыва = СвойстваСертификата.АдресаСписковОтзыва;
		Если АдресаСписковОтзыва.Количество() = 0 И Не ЗначениеЗаполнено(Адреса) Тогда
	...
	Иначе
		...
		Если ТипЗнч(Контекст.Сертификат) = Тип("ДвоичныеДанные") Тогда
			ДвоичныеДанныеСертификата = Контекст.Сертификат;
		ИначеЕсли ТипЗнч(Контекст.Сертификат) = Тип("СертификатКриптографии") Тогда
			ДвоичныеДанныеСертификата = Ждать Контекст.Сертификат.ВыгрузитьАсинх();
		ИначеЕсли ЭтоАдресВременногоХранилища(Контекст.Сертификат) Тогда
			ДвоичныеДанныеСертификата = ПолучитьИзВременногоХранилища(Контекст.Сертификат);
		Иначе
			ДвоичныеДанныеСертификата = Base64Значение(Контекст.Сертификат);
		...
		СвойстваСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДополнительныеСвойстваСертификата(
			ДвоичныеДанныеСертификата);

		АдресаСписковОтзыва = Ждать АдресаСписковОтзыва(ДвоичныеДанныеСертификата, ОбъектКомпоненты);
		Если ТипЗнч(АдресаСписковОтзыва) = Тип("Строка") И Не ЗначениеЗаполнено(Адреса) Тогда
			ОбработатьОшибкуУстановкиСпискаОтзыва(АдресаСписковОтзыва, Контекст);
			Возврат;
		КонецЕсли;

	КонецЕсли;
		
	Для Каждого Адрес Из АдресаСписковОтзыва Цикл
		Если Адреса.Найти(Адрес) = Неопределено Тогда
			Адреса.Добавить(Адрес);
		КонецЕсли;
	КонецЦикла;

	Параметры = Новый Структура("АдресРесурса, НаименованиеОперации, АдресВнутренний");
	Параметры.АдресРесурса = Адреса;
	...
	Контекст.Вставить("ИдентификаторКлючаУдостоверяющегоЦентра",
		СвойстваСертификата.ИдентификаторКлючаУдостоверяющегоЦентра);
...
Асинх Функция АдресаСписковОтзыва(ДвоичныеДанныеСертификата, ОбъектКомпоненты)
	
	СвойстваСертификатаРасширенные = Ждать ПолучитьСвойстваСертификатаАсинх(ДвоичныеДанныеСертификата, ОбъектКомпоненты);

	Если ЗначениеЗаполнено(СвойстваСертификатаРасширенные.Ошибка) Тогда
		Возврат СвойстваСертификатаРасширенные.Ошибка;
	КонецЕсли;
	...
	Если АдресаСписковОтзыва.Количество() = 0 Тогда
		...
		Возврат НСтр("ru = 'В сертификате не указаны адреса списков отзыва.'");
	КонецЕсли;
	
	Возврат АдресаСписковОтзыва;
...
Процедура ПродолжитьУстановкуСпискаОтзываПослеСкачивания(РезультатВыполнения, Контекст) Экспорт
	...
	Если РезультатВыполнения = Неопределено Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(НСтр("ru = 'Загрузка списка отзыва не была завершена штатно'"), Контекст);
		Возврат;
	КонецЕсли;
	
	Если РезультатВыполнения.Статус = "Ошибка" Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(РезультатВыполнения.ИнформацияОбОшибке, Контекст);
		Возврат;
	КонецЕсли;
	...
	Попытка
		Результат = ПолучитьИзВременногоХранилища(РезультатВыполнения.АдресРезультата);
	Исключение
		ОбработатьОшибкуУстановкиСпискаОтзыва(ИнформацияОбОшибке(), Контекст);
		Возврат;
	КонецПопытки;
	...
	СвойстваСпискаОтзыва = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСпискаОтзыва(Результат.ДанныеФайла);
	
	Если СвойстваСпискаОтзыва = Неопределено Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл %1 не является списком отзыва.'"), Результат.АдресФайла), Результат, Истина);
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СвойстваСпискаОтзыва.ДатаОкончания) И СвойстваСпискаОтзыва.ДатаОкончания < ОбщегоНазначенияКлиент.ДатаУниверсальная() Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Список отзыва %1 просрочен. Обратитесь в удостоверяющий центр, выдавший сертификат.'"), Результат.АдресФайла), Результат, Истина);
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Результат.ИдентификаторКлючаУдостоверяющегоЦентра)
		И ЗначениеЗаполнено(СвойстваСпискаОтзыва.ИдентификаторКлючаУдостоверяющегоЦентра)
		И Результат.ИдентификаторКлючаУдостоверяющегоЦентра <> СвойстваСпискаОтзыва.ИдентификаторКлючаУдостоверяющегоЦентра Тогда
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не совпадают идентификаторы ключа удостоверяющего центра полученного списка отзыва (%1) и сертификата (%2).
			|Обратитесь в удостоверяющий центр, выдавший сертификат.'"),
				СвойстваСпискаОтзыва.ИдентификаторКлючаУдостоверяющегоЦентра, Результат.ИдентификаторКлючаУдостоверяющегоЦентра);
		ОбработатьОшибкуУстановкиСпискаОтзыва(Ошибка, Результат, Истина);
		Возврат;
	КонецЕсли;
...
Асинх Процедура УстановитьСписокОтзываПослеСозданияВременногоКаталога(ИмяВременногоКаталога, Контекст) Экспорт
	...
	Исключение
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать список отзыва в файл ""%1"": %2. Проверьте наличие свободного места, права доступа и настройки антивируса.'"),
			ИмяФайлаСпискаОтзыва, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбработатьОшибкуУстановкиСпискаОтзыва(Ошибка, Контекст);
		Возврат;
	КонецПопытки;
	
	Файл = Новый Файл(ИмяФайлаСпискаОтзыва);
	Если Ждать Файл.СуществуетАсинх() Тогда
		РазмерФайла = Ждать Файл.РазмерАсинх();
	Иначе
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать список отзыва в файл ""%1"". Проверьте наличие свободного места, права доступа и настройки антивируса.'"),
	...
	Если РезультатУстановки.Значение <> Истина Тогда
		...
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить список отзыва в хранилище ""%1"" из файла ""%2"" (%3 байт) по причине:
				 |%4'"), ИмяХранилища, ИмяФайлаСпискаОтзыва, РазмерФайла, Ошибка);
		ОбработатьОшибкуУстановкиСпискаОтзыва(Ошибка, Контекст);
		Возврат;
	Иначе
...
Процедура ОбработатьОшибкуУстановкиСпискаОтзыва(Ошибка, Контекст, ОшибкаИздателя = Ложь)
	...
	Если ТипЗнч(Ошибка) = Тип(ИнформацияОбОшибке()) Тогда
		Если Контекст.ОповещениеОЗавершении = Неопределено Тогда 
			...
			Возврат;
		КонецЕсли;
		Ошибка = ОбработкаОшибок.КраткоеПредставлениеОшибки(Ошибка);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресРесурса) Тогда
		Если ОшибкаИздателя Тогда
			
			Ошибка = Ошибка + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адрес списка отзыва, указанный в сертификате: %1'"), СтрСоединить(АдресРесурса, Символы.ПС));
	...
	Иначе
		...
		ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, Контекст.Форма);
...
Процедура УстановитьСертификатПослеПодключенияКомпоненты(ОбъектКомпоненты, ПараметрыУстановкиСертификата) Экспорт
	...
	Если ПараметрыУстановкиСертификата.ВариантыУстановки = Неопределено
		...
		ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.УстановкаСертификата",
			ПараметрыФормы, ПараметрыУстановкиСертификата.Форма,,,,
...
Асинх Процедура УстановитьСертификатПослеВыбораВариантаУстановки(ПараметрыУстановкиСертификата, ОбъектКомпоненты = Неопределено) Экспорт
	...
	Результат = РезультатУстановкиСертификата();
	Результат.УстановкаВыполнена = Истина;
	Результат.Сертификат = ПараметрыУстановкиСертификата.Сертификат;
...
Функция РезультатУстановкиСертификата()
	...
	Возврат Новый Структура("УстановкаВыполнена, Сообщение, Сертификат", Ложь, "", "");
...
Асинх Процедура ПолучитьУстановленныеКриптопровайдерыПослеПодключенияКомпоненты(ОбъектКомпоненты, Контекст) Экспорт
	...
	Если КриптопровайдерыРезультат.ПроверкаВыполнена Тогда
		...
		Результат.Токены = КриптопровайдерыРезультат.Токены;
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		Результат.Вставить("ПроверкаВыполнена", Истина);
...
#КонецЕсли
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для получения списка приложений электронной подписи и шифрования, установленных на компьютере, требуется установить компоненту %1.'"),
			"ExtraCryptoAPI");
	...
	РезультатТокены = Ждать ЭлектроннаяПодписьКлиентЛокализация.УстановленныеТокены(ОбъектКомпоненты, ПредложитьУстановить);
	Результат.Токены = РезультатТокены.Токены;
...
Функция НовыеСвойстваТокена() Экспорт
	...
	СвойстваТокена = Новый Структура;
	СвойстваТокена.Вставить("Представление", НСтр("ru = 'Токен'"));
	СвойстваТокена.Вставить("Модель", "");
	СвойстваТокена.Вставить("СерийныйНомер");
	СвойстваТокена.Вставить("Слот");
	СвойстваТокена.Вставить("Токен");
	СвойстваТокена.Вставить("Механизмы");
	СвойстваТокена.Вставить("Ошибка");
	СвойстваТокена.Вставить("Сертификаты");
	СвойстваТокена.Вставить("Библиотеки", Новый Соответствие);
	
	Возврат СвойстваТокена;
	
КонецФункции

// Возвращает результат проверки криптопровайдеров из кэша с обновлением в 20 минут.
Асинх Функция УстановленныеКриптопровайдерыИзКэша(ПредложитьУстановить = Ложь) Экспорт
...
#КонецЕсли
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено 
		Или Не ЭтоМобильныйКлиент И ПараметрыПриложения[ИмяПараметра].ВремяПроверки + ДобавкаВремени < ТекущаяДата() // АПК:143 Должна использоваться ТекущаяДата().
		Или Не ЭтоМобильныйКлиент И ПараметрыПриложения[ИмяПараметра].Ошибка = ТекстОшибкиНеУстановленаКомпонента() И ПредложитьУстановить Тогда
	...
	Иначе
		...
		МассивКриптопровайдеры = Новый Массив;
		Для Каждого Криптопровайдер Из РезультатИзКэша.Криптопровайдеры Цикл
			МассивКриптопровайдеры.Добавить(Новый Структура(Криптопровайдер));
		КонецЦикла;
		
		МассивТокены = Новый Массив;
		Для Каждого Токен Из РезультатИзКэша.Токены Цикл
			МассивТокены.Добавить(Новый Структура(Токен));
		КонецЦикла;
		
		Результат = Новый Структура;
		Результат.Вставить("ПроверкаВыполнена", РезультатИзКэша.ПроверкаВыполнена);
		Результат.Вставить("Криптопровайдеры", МассивКриптопровайдеры);
		Результат.Вставить("Токены", МассивТокены);
...
Процедура ЗаписатьВКэшУстановленныеКриптопровайдеры(Результат) Экспорт
	...
	Для Каждого Криптопровайдер Из Результат.Криптопровайдеры Цикл
		МассивКриптопровайдеры.Добавить(Новый ФиксированнаяСтруктура(Криптопровайдер));
	КонецЦикла;
	
	РезультатДляКэша.Вставить("Криптопровайдеры", Новый ФиксированныйМассив(МассивКриптопровайдеры));
	
	МассивТокенов = Новый Массив;
	Для Каждого Токен Из Результат.Токены Цикл
		МассивТокенов.Добавить(Новый ФиксированнаяСтруктура(Токен));
	КонецЦикла;
	
	РезультатДляКэша.Вставить("Токены", Новый ФиксированныйМассив(МассивТокенов));
...
// Продолжение процедуры ПроверитьУстановкуПрограммКриптографии
Асинх Процедура ПроверитьУстановкуПрограммКриптографииПослеПолученияУстановленных(Результат, Контекст) Экспорт
	...
	РезультатПроверки.Вставить("Токены", Результат.Токены);
	...
	СтандартнаяОбработка = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПриПродолженииПроверкиУстановленныхПрограммКриптографии(
		Контекст, РезультатПроверки, ЕстьПроверяемыеПрограммы);
...
#Область ОтправкаВТехническуюПоддержку

Функция ПараметрыОтправкиВТехподдержку()
	
	ПараметрыОтправкиВТехподдержку = Новый Структура;
	ПараметрыОтправкиВТехподдержку.Вставить("Вложения", Новый Массив);
	ПараметрыОтправкиВТехподдержку.Вставить("Тема");
	ПараметрыОтправкиВТехподдержку.Вставить("Сообщение");
	ПараметрыОтправкиВТехподдержку.Вставить("ОбработчикЗавершения");
	ПараметрыОтправкиВТехподдержку.Вставить("Получатель", ПолучательОбращенияВТехническуюПоддержку());
	
	Возврат ПараметрыОтправкиВТехподдержку;
	
КонецФункции

Процедура ОтправитьСообщениеВТехподдержку(ПараметрыОтправкиВТехподдержку)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		МодульСообщенияВСлужбуТехническойПоддержкиКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("СообщенияВСлужбуТехническойПоддержкиКлиентСервер");
			...
			ДанныеСообщения.Тема = ПараметрыОтправкиВТехподдержку.Тема;
			ДанныеСообщения.Получатель = ПараметрыОтправкиВТехподдержку.Получатель;
			ДанныеСообщения.Сообщение = ПараметрыОтправкиВТехподдержку.Сообщение;
			
			Вложения = Новый Массив;
			Для Каждого Вложение Из ПараметрыОтправкиВТехподдержку.Вложения Цикл
				Если ТипЗнч(Вложение.Данные) = Тип("ДвоичныеДанные") Тогда
					Вложение.Данные = ПоместитьВоВременноеХранилище(Вложение.Данные, Новый УникальныйИдентификатор);
				КонецЕсли;
				Вложения.Добавить(Новый Структура("Представление, Данные, ВидДанных", Вложение.Имя, Вложение.Данные, "Адрес"));
			КонецЦикла;
			
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеПроверкиОтправкиВПоддержку", ЭтотОбъект, ПараметрыОтправкиВТехподдержку);
			МодульСообщенияВСлужбуТехническойПоддержкиКлиент.ОтправитьСообщение(ДанныеСообщения, Вложения, Неопределено, ОповещениеОЗавершении);
		Возврат;
	КонецЕсли;
	
	ПослеПроверкиОтправкиВПоддержку(Неопределено, ПараметрыОтправкиВТехподдержку);
...
Процедура ПослеПроверкиОтправкиВПоддержку(Результат, Контекст) Экспорт
	...
	Если Результат <> Неопределено И ЗначениеЗаполнено(Результат.КодОшибки) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Отправка сообщения в техподдержку'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
				"Ошибка", Результат.КодОшибки + " " + Результат.СообщениеОбОшибке,, Истина);
	ИначеЕсли Результат <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеПроверкиУчетнойЗаписиПочты", ЭтотОбъект, Контекст);
		МодульРаботаСПочтовымиСообщениямиКлиент.ПроверитьНаличиеУчетнойЗаписиДляОтправкиПочты(ОповещениеОЗавершении); 
		Возврат;
	КонецЕсли;

	ПослеПроверкиУчетнойЗаписиПочты(Неопределено, Контекст);
...
Функция ТекстОбращенияВТехническуюПоддержку() Экспорт
	Возврат НСтр("ru = '<Опишите возникшую проблему, приложите скриншоты ошибки и версии приложения электронной подписи и шифрования (криптопровайдера).>'");
КонецФункции

Функция ТекстОбращенияВТехническуюПоддержкуПриПроверкеСертификата() Экспорт
	Возврат НСтр("ru = '<Опишите возникшую проблему, приложите скриншоты ошибки и версии приложения электронной подписи и шифрования (криптопровайдера), а также
		|результаты тестирования сертификата средствами криптопровайдера.>'");
КонецФункции

Функция ПолучательОбращенияВТехническуюПоддержку()
	Получатель = "v8";
	ЭлектроннаяПодписьКлиентЛокализация.ПриОпределенииПолучателяОбращенияВТехническуюПоддержку(Получатель);
	Возврат Получатель;
КонецФункции

Функция АдресПолучателяОбращенияВТехническуюПоддержку()
	Возврат "v8@1c.ru";
КонецФункции
...
Процедура ПослеПроверкиУчетнойЗаписиПочты(Результат, Контекст) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ПараметрыОтправкиПисьма.Тема = Контекст.Тема;
		ПараметрыОтправкиПисьма.Текст = ШаблонТекстаСообщения(Контекст.Сообщение);
		ПараметрыОтправкиПисьма.Получатель = АдресПолучателяОбращенияВТехническуюПоддержку();
		
		ОбработчикЗавершения = Новый ОписаниеОповещения("ПослеОтправкиПисьмаВПоддержку", ЭтотОбъект,
			Контекст);
		
		Вложения = Новый Массив;
		Для Каждого Вложение Из Контекст.Вложения Цикл
			Если ТипЗнч(Вложение.Данные) = Тип("ДвоичныеДанные") Тогда
				Вложение.Данные = ПоместитьВоВременноеХранилище(Вложение.Данные, Новый УникальныйИдентификатор);
			КонецЕсли;
			Вложения.Добавить(Новый Структура("Представление, АдресВоВременномХранилище", Вложение.Имя, Вложение.Данные));
		КонецЦикла;
...
Процедура ПослеОтправкиПисьмаВПоддержку(Результат, Контекст) Экспорт
	...
	Для Каждого Вложение Из Контекст.Вложения Цикл
		УдалитьИзВременногоХранилища(Вложение.Данные);
	КонецЦикла;
...
Процедура ОбработатьНавигационнуюСсылкуКлассификатора(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, ДополнительныеДанные = Неопределено) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ПродолжитьОбработкуНавигационнойСсылкиКлассификатора(Элемент, НавигационнаяСсылкаФорматированнойСтроки, ДополнительныеДанные);
	
КонецПроцедуры

Асинх Процедура ПродолжитьОбработкуНавигационнойСсылкиКлассификатора(Элемент, НавигационнаяСсылкаФорматированнойСтроки, ДополнительныеДанные) 
	...
	ПродолжитьОбработку = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПриОбработкеНавигационнойСсылкиКлассификатора(Элемент, 
		НавигационнаяСсылкаФорматированнойСтроки, Параметры);
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "УстановитьСписокОтзываСертификата" Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ДанныеСертификата) Тогда
			...
			ПараметрыУстановкиСпискаОтзыва.ОповещениеОЗавершении = ОповещениеПослеОбработкиНавигационнойСсылкиКлассификатора(
				НавигационнаяСсылкаФорматированнойСтроки, Параметры.ДанныеСертификата);
			УстановитьСписокОтзываСертификата(ПараметрыУстановкиСпискаОтзыва);
		КонецЕсли;
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "УстановитьКорневойСертификат" Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ДанныеСертификата) Тогда
			...
			ПараметрыУстановкиСертификата.ОповещениеОЗавершении = ОповещениеПослеОбработкиНавигационнойСсылкиКлассификатора(
				НавигационнаяСсылкаФорматированнойСтроки, Параметры.ДанныеСертификата);
			УстановитьКорневойСертификат(ПараметрыУстановкиСертификата);
		КонецЕсли;
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ДиагностикаСерверовМетокВремениСервер" Тогда
		
		ДиагностикаСерверовМетокВремени("Сервер");
	
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ДиагностикаСерверовМетокВремениКлиент" Тогда
		
		ДиагностикаСерверовМетокВремени("Клиент");
		
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьНастройкиЭлектроннойПодписи" Тогда
	...
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеУстановитьБиблиотекиДляТокенов()) Тогда
		
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ВидТокена", Прав(НавигационнаяСсылкаФорматированнойСтроки,
			СтрДлина(НавигационнаяСсылкаФорматированнойСтроки) - СтрДлина(ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеУстановитьБиблиотекиДляТокенов())));
			ПараметрыОткрытия.Вставить("ТолькоУстановкаТокенов", Истина);
		ОткрытьФормуУстановкиПрограммКриптопровайдеров(ПараметрыОткрытия);
	...
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "e1cib") Тогда 
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(НавигационнаяСсылкаФорматированнойСтроки);
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Действие будет доступно в следующих обновлениях приложения'"));
	КонецЕсли;

КонецПроцедуры

Функция ОповещениеПослеОбработкиНавигационнойСсылкиКлассификатора(НавигационнаяСсылка, ПредметОповещения)
	
	Возврат Новый ОписаниеОповещения("ПослеОбработкиНавигационнойСсылкиКлассификатора", ЭтотОбъект,
		Новый Структура("НавигационнаяСсылка, ПредметОповещения", НавигационнаяСсылка, ПредметОповещения));
...
Процедура ПослеОбработкиНавигационнойСсылкиКлассификатора(Результат, Контекст) Экспорт
	...
	Если НавигационнаяСсылка = "УстановитьСертификат"
		Или НавигационнаяСсылка = "УстановитьСертификатВКонтейнер"
		Или НавигационнаяСсылка = "УстановитьКорневойСертификат"
		Или НавигационнаяСсылка = "УстановитьСписокОтзываСертификата" Тогда
		
		Если Результат.УстановкаВыполнена <> Истина Тогда
				
			ПараметрыФормы = Новый Структура;
			Если НавигационнаяСсылка = "УстановитьСписокОтзываСертификата" Тогда
				ПараметрыФормы.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Не удалось установить список отзыва'"));
			Иначе
				ПараметрыФормы.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Не удалось установить сертификат'"));
			КонецЕсли;
			...
			ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы);

			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Оповестить("БСП_ВыполнитьПроверкуСертификата", Контекст.ПредметОповещения);
...
Функция ДополнительныеДанныеДляКлассификатораОшибок() Экспорт
	...
	ДополнительныеДанные.Вставить("ДанныеПодписи");
...
Процедура УстановитьФизическоеЛицо(Форма, КомуВыдан, ФизическоеЛицо)
	...
	ФизическиеЛицаДляФИО = Результат.ФизическиеЛица.Получить(КомуВыдан);
	...
	Если ФизическиеЛицаДляФИО = Неопределено Тогда
	...
	Если ФизическиеЛицаДляФИО.Количество() = 1 Тогда
		Если ЭтоФормаЭлемента Тогда
			Форма.Объект.ФизическоеЛицо = ФизическиеЛицаДляФИО[0];
		Иначе
			Форма.СертификатФизическоеЛицо = ФизическиеЛицаДляФИО[0];
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Отбор.ПравоеЗначение = ФизическиеЛицаДляФИО;
...
Асинх Функция СвойстваПодписиЧтениеМенеджеромКриптографии(Подпись, МенеджерКриптографии, ПрочитатьСертификаты) Экспорт
	...
	ПараметрыПодписиКриптографии = Ждать ПараметрыПодписиКриптографии(
		КонтейнерПодписи, РазницаСУниверсальнымВременем(), ОбщегоНазначенияКлиент.ДатаСеанса());
...
Асинх Функция СвойстваПодписиИзДвоичныхДанных(Подпись, ПрочитатьСертификаты)
	...
	Попытка
		СвойстваПодписиИзДвоичныхДанных = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваПодписиИзДвоичныхДанных(
			Подпись, РазницаСУниверсальнымВременем(), ПрочитатьСертификаты);
	...
	Если СвойстваПодписиИзДвоичныхДанных.Сертификаты.Количество() > 0 Тогда
		...
		Если СвойстваПодписиИзДвоичныхДанных.Сертификаты.Количество() > 1 Тогда
			Попытка
				...
				Результат.Сертификат = ?(ЗначениеЗаполнено(СвойстваПодписиИзДвоичныхДанных.СертификатПодписи),
					СвойстваПодписиИзДвоичныхДанных.СертификатПодписи, Результат.Сертификаты[0]);
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Иначе
			Результат.Сертификат = ?(ЗначениеЗаполнено(СвойстваПодписиИзДвоичныхДанных.СертификатПодписи),
				СвойстваПодписиИзДвоичныхДанных.СертификатПодписи, СвойстваПодписиИзДвоичныхДанных.Сертификаты[0]);
		КонецЕсли;
...
//
Функция РезультатПроверкиПодписи(Результат, РезультатСтруктура = Неопределено, ТребуетсяПроверка = Неопределено, НеверныйХеш = Неопределено)
...
Процедура ПослеВводаОбоснованияДостоверностиПодписи(Результат, Оповещение) Экспорт
	...
	РезультатПроверки.Вставить("АвторРучнойПроверкиДополнительныхАтрибутов", ПользователиКлиент.ТекущийПользователь());
	РезультатПроверки.Вставить("ОбоснованиеРучнойПроверкиДополнительныхАтрибутов", Результат);
	РезультатПроверки.Вставить("ДополнительныеАтрибутыПровереныВручную", Истина);
	РезультатПроверки.Вставить("ДатаПроверки", ОбщегоНазначенияКлиент.ДатаСеанса());
	РезультатПроверки.Вставить("ПодписьВерна", Истина);
	РезультатПроверки.Вставить("ТребуетсяПроверка", Ложь);
	
	ВыполнитьОбработкуОповещения(Оповещение, РезультатПроверки);
...
//
Асинх Функция ПараметрыПодписиКриптографии(КонтейнерПодписи, РазницаСУниверсальнымВременем, ДатаСеанса) Экспорт
	...
	Возврат ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыПодписиКриптографии(ПараметрыПодписи, Подпись, СертификатСуществует, РазницаСУниверсальнымВременем, ДатаСеанса);
	
КонецФункции

Функция РезультатДиагностикиСерверовМетокВремени()

	АдресаСерверовМетокВремени = ЭлектроннаяПодписьКлиент.ОбщиеНастройки().АдресаСерверовМетокВремени;

	Если АдресаСерверовМетокВремени.Количество() = 0 Тогда

		Возврат НСтр("ru = 'Адреса серверов меток времени не заполнены в настройках электронной подписи.'");
...
#Если ВебКлиент Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			...
			|%1'"), СтрСоединить(АдресаСерверовМетокВремени, Символы.ПС));
	
#Иначе
		СтрокаПодключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подключение к серверам меток времени на компьютере <%1>.'"), ИмяКомпьютера());
		Возврат ЭлектроннаяПодписьСлужебныйКлиентСервер.ДиагностикаСерверовМетокВремени(АдресаСерверовМетокВремени, СтрокаПодключения);
#КонецЕсли
	КонецЕсли;
	
КонецФункции

Процедура ОбработатьОшибкуРасшифровки(Ошибка, Контекст)
	
	ТекстОшибки = Ошибка.ОписаниеОшибки;
	ОшибкаПоКлассификатору = ЭлектроннаяПодписьСлужебныйВызовСервера.ОшибкаПоКлассификатору(ТекстОшибки);
	Если ОшибкаПоКлассификатору = Неопределено Или Не ОшибкаПоКлассификатору.НеизвестныйАлгоритмКриптографии Тогда
		// Обработка не требуется
		Возврат;
	КонецЕсли;
	
	НерасшифрованныеДанные = Контекст.ЭлементДанных.Данные;
	Программа = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст, "Программа", Неопределено);
	ПредставлениеПрограммы = ?(ЗначениеЗаполнено(Программа), Программа.Представление, "");
	
	ИнформацияАлгоритмаШифрования = ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмШифрованияЗашифрованногоФайла(НерасшифрованныеДанные);
	ДополнительныеДанныеПроверок = ДополнительныеПараметрыОшибкиРасшифровки(ИнформацияАлгоритмаШифрования, ПредставлениеПрограммы);
	
	Ошибка.Вставить("ДополнительныеДанныеПроверокНаКлиенте", ДополнительныеДанныеПроверок);
	Ошибка.Вставить("НастройкаПрограмм", Истина);
	Если Не ПустаяСтрока(ИнформацияАлгоритмаШифрования.Идентификатор) Тогда
		Ошибка.ОписаниеОшибки = СокрЛ(ТекстОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'OID алгоритма: %1'"), ИнформацияАлгоритмаШифрования.Идентификатор));
	КонецЕсли;
		
КонецПроцедуры

Функция ДополнительныеПараметрыОшибкиРасшифровки(ИнформацияАлгоритмаШифрования, ПредставлениеПрограммы)
	
	Результат = Новый Структура("ДополнительныеДанныеПроверокНаКлиенте");
	ПредставлениеОшибки = Новый Структура("Причина, Решение");
	ИдентификаторАлгоритма = ИнформацияАлгоритмаШифрования.Идентификатор;
	ПредставлениеАлгоритма = ?(ЗначениеЗаполнено(ИнформацияАлгоритмаШифрования.Представление),
								ИнформацияАлгоритмаШифрования.Представление,
								ИнформацияАлгоритмаШифрования.Идентификатор);
	
	ТекстРешения = "";
	Если СтрНайти(ИдентификаторАлгоритма, "1.2.643.7.1.1.5.2") <> 0
		Или СтрНайти(ИдентификаторАлгоритма, "1.2.643.7.1.1.5.1") <> 0 Тогда
		ТекстРешения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '• Установите приложение электронной подписи, которое расшифрует документы, зашифрованные алгоритмом ""%1"", например КриптоПро CSP версии 5.0.12000 и выше или Сигнал-КОМ CSP.'"),
			ПредставлениеАлгоритма);
	Иначе
		ТекстРешения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '• Установите приложение электронной подписи, которое расшифрует документы, зашифрованные алгоритмом%1.'"),
			" """ + ПредставлениеАлгоритма + """");
	КонецЕсли;
		
	ПредставлениеОшибки.Причина = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '• Приложение электронной подписи %1 не работает с данным алгоритмом шифрования.
		|• Приложение электронной подписи установлено некорректно.'"),
		ПредставлениеПрограммы);
	ПредставлениеОшибки.Решение = ТекстРешения + НСтр("ru = '• Удалите все приложения электронной подписи и установите только одно из них, так как установка нескольких приложений электронной подписи на одном компьютере может вызвать эту ошибку.'");
	
	Результат.ДополнительныеДанныеПроверокНаКлиенте = ПредставлениеОшибки;
...
//  Структура
//
Функция НовыйКомпонентыРаботыСКриптографией() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("КриптопровайдерУстановлен", Истина);
	Результат.Вставить("РасширениеРаботыС1СПредприятиемУстановлено", Истина);
	Результат.Вставить("РасширениеРаботыСКриптографиейУстановлено", Истина);
	Результат.Вставить("КомпонентаExtraCryptoAPIУстановлена", Истина);
...
//
// Параметры:
//  Компоненты - см. НовыйКомпонентыРаботыСКриптографией
// 
// Возвращаемое значение:
//  Булево
//
Асинх Функция ТребуетсяУстановкаДополнительныхКомпонент(Компоненты) Экспорт
	
	Результат = Ложь;
	
	// Проверка установки расширения для работы с 1С:Предприятием
	Если Компоненты.РасширениеРаботыС1СПредприятиемУстановлено Тогда
		Компоненты.РасширениеРаботыС1СПредприятиемУстановлено = Ждать ПодключитьРасширениеРаботыСФайламиАсинх();
		Результат = Результат Или Не Компоненты.РасширениеРаботыС1СПредприятиемУстановлено;
	КонецЕсли;
	
	// Проверка установки расширения для работы с криптографией
	Если Компоненты.РасширениеРаботыСКриптографиейУстановлено Тогда
		Компоненты.РасширениеРаботыСКриптографиейУстановлено = Ждать ПодключитьРасширениеРаботыСКриптографиейАсинх();
		Результат = Результат Или Не Компоненты.РасширениеРаботыСКриптографиейУстановлено;
	КонецЕсли;
	
	// Проверка установки расширения для работы с компонентой ExtraCryptoAPI
	Если Компоненты.КомпонентаExtraCryptoAPIУстановлена Тогда
		...
		РезультатПодключенияКомпоненты = Ждать ОбщегоНазначенияКлиент.ПодключитьКомпонентуИзМакетаАсинх(
			ОписаниеКомпоненты.ИмяОбъекта,
			ОписаниеКомпоненты.ПолноеИмяМакета,
			ПараметрыПодключения);
		Компоненты.КомпонентаExtraCryptoAPIУстановлена = РезультатПодключенияКомпоненты.Подключено;
		Результат = Результат Или Не Компоненты.КомпонентаExtraCryptoAPIУстановлена;
...
// Только для внутреннего использования.
//  Проверяет в веб-клиенте установку дополнительных компонент для работы с электронной подписью, сначала - расширение
//  работы с 1С:Предприятием. Если Расширение работы с 1С:Предприятием не установлено, то производится проверка
//  установки других компонент. Если не установлены хотя бы две компоненты, то открывает форму
//  НачалоРаботыСЭлектроннойПодписью для установки неустановленных компонент. Если не установлен только один компонент,
//  все компоненты установлены или контекст вызова не из веб-клиента - продолжается алгоритм вызывающего кода.
//
// Параметры:
//  Оповещение - ОписаниеОповещения, Неопределено - оповещение закрытия формы НачалоРаботыСЭлектроннойПодписью.
// 
// Возвращаемое значение:
//  Булево - Если требуется установка дополнительных компонент, возвращается Истина.
//
Асинх Функция ВыполнитьУстановкуДополнительныхКомпонентПослеПроверкиРасширенияРаботыС1СПредприятием(Оповещение = Неопределено) Экспорт
	
#Если ВебКлиент Тогда
	РасширениеРаботыС1СПредприятиемУстановлено = Ждать ПодключитьРасширениеРаботыСФайламиАсинх();
	Если Не РасширениеРаботыС1СПредприятиемУстановлено Тогда
		ТребуетсяУстановка = Истина;
		ТребуетсяУстановкаДополнительныхКомпонент = Ложь;
		Компоненты = НовыйКомпонентыРаботыСКриптографией();
		Компоненты.РасширениеРаботыС1СПредприятиемУстановлено = Ложь;
		...
		Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("Компоненты", Компоненты);
			ПараметрыФормы.Вставить("ИмяТребуемойКомпоненты", "РасширениеРаботыС1СПредприятием");
			ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы, ЭтотОбъект, 
				, , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеПроверкиУстановкиПрограммКриптографии(Результат, Компоненты) Экспорт
	
	Если Результат.ПроверкаВыполнена Тогда
		
		Компоненты.КриптопровайдерУстановлен = Результат.Токены.Количество() > 0;
		
		Если Не Компоненты.КриптопровайдерУстановлен Тогда
			Для Каждого Криптопровайдер Из Результат.Программы Цикл
				Если ЗначениеЗаполнено(Криптопровайдер.Программа) И Не Компоненты.КриптопровайдерУстановлен Тогда
					Компоненты.КриптопровайдерУстановлен = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
