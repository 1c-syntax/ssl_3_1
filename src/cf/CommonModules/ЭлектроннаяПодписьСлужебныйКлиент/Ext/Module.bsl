...
Асинх Функция ПолучитьТокенПоСертификату(Сертификат) Экспорт
	
	Отпечаток = Base64Строка(Сертификат.Отпечаток);
	
	Токены = Ждать ЭлектроннаяПодписьКлиентЛокализация.СертификатыНаТокенах(Истина);
	Для Каждого Токен Из Токены Цикл
		Если Токен.Сертификаты = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ДанныеСертификата = Токен.Сертификаты.Получить(Отпечаток);
...
Асинх Функция ПодписьНаТокене(ПараметрыПодписи, Данные) Экспорт
	...
	Возврат Ждать ЭлектроннаяПодписьКлиентЛокализация.ПодписьНаТокене(ПараметрыПодписи, Данные);
...
Процедура ПолучитьСертификатПоОтпечаткуПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПолучитьСертификатПоОтпечаткуНаТокенах(Контекст);
		Возврат;
	КонецЕсли;
...
// Продолжение процедуры ПолучитьСертификатПоОтпечатку.
Асинх Процедура ПолучитьСертификатПоОтпечаткуНаТокенах(Контекст)
	...
	Для Каждого Токен Из Токены Цикл
		Если Токен.Сертификаты = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ДанныеСертификата = Токен.Сертификаты.Получить(Контекст.Отпечаток);
		Если ДанныеСертификата <> Неопределено Тогда
			СертификатКриптографии = Новый СертификатКриптографии;
			Ждать СертификатКриптографии.ИнициализироватьАсинх(ДанныеСертификата);
			ПолучитьСертификатПоОтпечаткуЗавершение(СертификатКриптографии, "", Контекст);
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Асинх Процедура ПроверитьПодписьСредствамиТокена(Контекст) Экспорт
	...
	РезультатМатематическойПроверки = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПроверитьПодпись(
		Контекст.ИсходныеДанные, Контекст.Подпись, ПараметрыПроверки);
...
Асинх Процедура ПолучитьСвойстваСертификатовНаКлиентеПослеПолученияВсех(Контекст)
	...
	Контекст = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПолучитьСвойстваСертификатовНаТокенах(Контекст, Ложь);
...
Асинх Процедура ПродолжитьОткрытиеНачалоПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") И Не ИспользоватьЭлектроннуюПодписьВМоделиСервиса()
		...
		Если Контекст.Операция = "Подписание" Или Контекст.Операция = "Расшифровка" Тогда
			ОтпечаткиСертификатовНаТокенах = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПолучитьОтпечаткиСертификатовНаТокенах(Истина, Ложь);
...
Асинх Процедура ПолучитьОтпечаткиСертификатовНаКлиентеПослеСозданияМенеджераКриптографии(МенеджерКриптографии, Контекст) Экспорт
	
	Массив = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПолучитьОтпечаткиСертификатовНаТокенах(Ложь, Контекст.ВключаяПросроченные);
	
	Если ТипЗнч(МенеджерКриптографии) <> Тип("МенеджерКриптографии") Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Массив);
		Возврат;
	КонецЕсли;
	
	Контекст.Вставить("МассивОтпечатков", Массив);
			...
			Иначе
				...
				Иначе
					...
					Если Операция = "Шифрование" Тогда
						Токен = Ждать ЭлектроннаяПодписьКлиентЛокализация.ТокенДляШифрования(
							Контекст.СертификатКриптографии, АлгоритмШифрования, Контекст.СвойстваСертификата);
						Если Токен <> Неопределено Тогда
							Контекст.Вставить("МенеджерКриптографии", Токен);
							Контекст.Вставить("АлгоритмШифрования", АлгоритмШифрования);
							ВыполнитьНаСторонеПодготовкаСертификатовЦиклНачало(Контекст);
							Возврат;
						КонецЕсли;
					Иначе
						Токен = Ждать ПолучитьТокенПоСертификату(Контекст.СертификатКриптографии);
						Если Токен <> Неопределено Тогда
							Контекст.Вставить("МенеджерКриптографии", Токен);
							Токен.Вставить("Пароль", Контекст.ЗначениеПароля);
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		ИначеЕсли ЭтоCMS Тогда
			...
			ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
				СоздатьПодписьНаТокене(Контекст, Результат.ПараметрыCMS.Открепленная);
			Иначе
				ОповещениеУспех = Новый ОписаниеОповещения(
					"ВыполнитьНаСторонеЦиклПослеОперацииНаКлиентеXMLDSig", ЭтотОбъект, Контекст);
		...
		Иначе
			...
			ИначеЕсли Контекст.Операция = "Подписание" Тогда
				...
				Иначе
					...
					Если ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
						ПодписатьНаТокене(МенеджерКриптографии, Оповещение, Данные, Контекст);
					Иначе
			...
			ИначеЕсли Контекст.Операция = "Шифрование" Тогда
				...
				ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
					ЗашифроватьНаТокене(МенеджерКриптографии, Оповещение, Данные, Контекст);
				Иначе
			...
			Иначе
				...
				ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
					РасшифроватьНаТокене(МенеджерКриптографии, Оповещение, Данные, Контекст);
				Иначе
...
Асинх Процедура ПодписатьНаТокене(Токен, Оповещение, Данные, Контекст)
	
	ПараметрыПодписи = ПараметрыПодписиНаТокене(Токен, Контекст.СертификатКриптографии, Контекст.ОписаниеДанных.ТипПодписи);
	Подпись = Ждать ЭлектроннаяПодписьКлиентЛокализация.ПодписьНаТокене(ПараметрыПодписи, Данные);
...
Асинх Процедура ЗашифроватьНаТокене(Токен, Оповещение, Данные, Контекст)
	
	ПараметрыШифрования = ПараметрыШифрованияНаТокене(Токен, Контекст.СертификатыШифрования, Контекст.АлгоритмШифрования);
	ЗашифрованныеДанные = Ждать ЭлектроннаяПодписьКлиентЛокализация.ШифрованиеНаТокене(ПараметрыШифрования, Данные);
...
Асинх Процедура РасшифроватьНаТокене(Токен, Оповещение, Данные, Контекст)
	
	ПараметрыРасшифровки = ПараметрыРасшифровкиНаТокене(Токен, Контекст.СертификатКриптографии);
	РасшифрованныеДанные = Ждать ЭлектроннаяПодписьКлиентЛокализация.РасшифровкаНаТокене(ПараметрыРасшифровки, Данные);
...
Асинх Процедура СоздатьПодписьНаТокене(Контекст, Открепленная)

	ПараметрыПодписи = ПараметрыПодписиНаТокене(Контекст.Токен, 
		Контекст.СертификатКриптографии, Контекст.ОписаниеДанных.ТипПодписи);
		
	ПараметрыПодписи.Открепленная = Открепленная;
	Подпись = Ждать ПодписьНаТокене(ПараметрыПодписи, Контекст.ЭлементДанных.Данные.Данные);
	
	Если ТипЗнч(Подпись) = Тип("Строка") Тогда
		ВыполнитьНаСторонеЦиклПослеОшибкиОперацииНаКлиентеXMLDSig(Подпись, Контекст);
		Возврат;
	КонецЕсли;
		
	ВыполнитьНаСторонеЦиклПослеОперацииНаКлиентеXMLDSig(Подпись, Контекст);
...
Асинх Процедура ДобавитьСертификатСТокена(Контекст)
	...
	Если Контекст.ДляШифрования = Истина Тогда
		...
		Результат = Ждать ЭлектроннаяПодписьКлиентЛокализация.ШифрованиеНаТокене(ПараметрыШифрованияНаТокене, Контекст.ДанныеСертификата);
	
		Если ТипЗнч(Результат) = Тип("Строка") Тогда
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаполнитьОшибкуДобавленияСертификата(
				Контекст.ОшибкаНаКлиенте,
				Новый Структура("Ссылка, Токен", Токен.Представление, Токен),
				"Шифрование",
				Результат,
		...
		ПараметрыРасшифровкиНаТокене = ПараметрыРасшифровкиНаТокене(Токен, Контекст.СертификатКриптографии);
		Результат = Ждать ЭлектроннаяПодписьКлиентЛокализация.РасшифровкаНаТокене(ПараметрыРасшифровкиНаТокене, Результат);
		Если ТипЗнч(Результат) = Тип("Строка") Тогда
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаполнитьОшибкуДобавленияСертификата(
				Контекст.ОшибкаНаКлиенте,
				Новый Структура("Ссылка, Токен", Токен.Представление, Токен),
	...
	Иначе
		...
		Результат = Ждать ПодписьНаТокене(ПараметрыПодписи, Контекст.ДанныеСертификата);
...
Асинх Функция УстановленныеКриптопровайдерыИзКэша(ПредложитьУстановить = Ложь) Экспорт
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено 
		Или ПараметрыПриложения[ИмяПараметра].ВремяПроверки + 1200 < ТекущаяДата() // АПК:143 Должна использоваться ТекущаяДата().
