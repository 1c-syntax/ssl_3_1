...
Процедура ПроверитьСертификатНаКлиенте(Контекст)
	...
	Иначе
		...
		Контекст.Вставить("АлгоритмПодписи",
			ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмПодписиСертификата(Контекст.Сертификат));
...
Процедура ПродолжитьПроверкуСертификатаНаКлиенте(Контекст)
	...
	Если Контекст.МенеджерКриптографии = Неопределено Тогда
		...
		ПараметрыСоздания = ПараметрыСозданияМенеджераКриптографии();
		ПараметрыСоздания.ПоказатьОшибку = Контекст.ПоказатьОшибку
...
Процедура ПолучитьДанныеИзОписанияДанныхПродолжение(Результат, Контекст) Экспорт
	...
	Иначе
		...
		ИначеЕсли ЭтоCMS Тогда
			...
			Если Данные.ПараметрыCMS.ВключениеСертификатовВПодпись = РежимВключенияСертификатовКриптографии.НеВключать Тогда
				Данные.ПараметрыCMS.ВключениеСертификатовВПодпись = "НеВключать";
			ИначеЕсли Данные.ПараметрыCMS.ВключениеСертификатовВПодпись = РежимВключенияСертификатовКриптографии.ВключатьСертификатСубъекта Тогда
				Данные.ПараметрыCMS.ВключениеСертификатовВПодпись = "ВключатьСертификатСубъекта";
			Иначе
				Данные.ПараметрыCMS.ВключениеСертификатовВПодпись = "ВключатьПолнуюЦепочку";
			КонецЕсли;
...
Процедура ДобавитьСертификатПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	ОписанияПрограмм = ЭлектроннаяПодписьСлужебныйКлиентСервер.МенеджерКриптографииОписанияПрограмм(
		Неопределено, Контекст.ОшибкаНаКлиенте.Ошибки, ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ОписанияПрограмм,
		Контекст.АлгоритмПодписи);
...
Процедура ДобавитьСертификатЦиклПослеОшибкиШифрования(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ТекстТекущейОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|%2'"),
		Контекст.ОписаниеПрограммы.Наименование,
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Контекст.ОшибкаНаКлиенте.ОписаниеОшибки = Контекст.ОшибкаНаКлиенте.ОписаниеОшибки
		+ Символы.ПС + Символы.ПС + ТекстТекущейОшибки;
...
//   ОбработчикЗавершения - ОписаниеОповещения, Неопределено - содержит описание процедуры,
...
//                                ОписаниеОповещения.
//   ДополнительныеФайлы  - Структура, Массив - дополнительные данные, которые необходимо поместить
//                        в архив, или массив таких данных. Поля структуры:
//      * Имя    - Строка - имя файла с расширением.
//      * Данные - ДвоичныеДанные, Строка - данные файла или адрес данных во временном хранилище.
//
Процедура СформироватьТехническуюИнформацию(Причина,
	ОбработчикЗавершения = Неопределено,
	ДополнительныеФайлы = Неопределено) Экспорт
	
	СопроводительныйТекст = НСтр("ru = 'Причина'")
		+ ":" + Символы.ПС + Причина + Символы.ПС + НСтр("ru = 'Информация о клиентском компьютере:'")
		+ Символы.ПС + ДиагностическаяИнформацияОКомпьютере()
		+ Символы.ПС;
...
Функция ДиагностическаяИнформацияОПрограммах(Оповещение)
	
	Заголовок = НСтр("ru = 'Программы на клиенте:'") + Символы.ПС;
...
Функция ИнформацияОПрограммахЦиклНачало(Контекст)
...
Процедура ПослеСбораТехническойИнформации(ИнформацияОПрограммах, Контекст) Экспорт
	...
	ФайловаяСистемаКлиент.СохранитьФайл(Новый ОписаниеОповещения(
		"СформироватьТехническуюИнформациюПослеСохраненияФайла", ЭтотОбъект, Контекст.ОбработчикЗавершения),
		ЭлектроннаяПодписьСлужебныйВызовСервера.АдресАрхиваТехническойИнформации(
		Контекст.СопроводительныйТекст, Контекст.ДополнительныеФайлы), "service_info.zip");
	
КонецПроцедуры

Процедура СформироватьТехническуюИнформациюПослеСохраненияФайла(СохраненныеФайлы, ОповещениеОЗавершении) Экспорт
	
	Если ОповещениеОЗавершении <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, СохраненныеФайлы <> Неопределено);
	КонецЕсли;
