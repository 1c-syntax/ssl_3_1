...
//
// Параметры:
//  Объект - Произвольный - объект, для которого выполняется правило регистрации.
	...
	Если Источник.ДополнительныеСвойства.Свойство("ЗарегистрированныеКВыгрузкеОбъекты", ЗарегистрированныеКВыгрузкеОбъекты) Тогда
		...
		Для Каждого ЭлементНабор Из ЗарегистрированныеКВыгрузкеОбъекты Цикл
			...
			Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементНабор.Ключ) Тогда
				...
				Пока ЭлементНабор.Значение.Количество() > 0 Цикл
					
					Объект = ЭлементНабор.Значение[0].ПолучитьОбъект();
					...
					КонецЕсли;
					Объект = Неопределено;
					ЭлементНабор.Значение.Удалить(0);
			...
			Иначе
				...
				Пока ЭлементНабор.Значение.Количество() > 0 Цикл
					...
					Для Каждого ЭлементОтбора Из ЭлементНабор.Значение.Колонки Цикл
						Объект.Отбор[ЭлементОтбора.Имя].Установить(ЭлементНабор.Значение[0][ЭлементОтбора.Имя]);
					КонецЦикла;
					Объект.Прочитать();
...
Процедура СохранитьРазрешенныеКВыгрузкеОбъекты(УзелОбъект)
	...
	Для Каждого ЭлементСостава Из СоставПланаОбмена Цикл
		...
		Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеЭлемента) Тогда
			...
			ЗарегистрированныеДанные[МетаданныеЭлемента] = Новый Массив;
			...
			Для Каждого СтрокаДанные Из НаборДанных Цикл
				...
				СсылкаНаОбъект = СтрокаДанные.Ссылка;
				
				Если ВыгрузкаОбъектаРазрешена(УзелОбъект.Ссылка, СсылкаНаОбъект.ПолучитьОбъект()) Тогда
					ЗарегистрированныеДанные[МетаданныеЭлемента].Добавить(СсылкаНаОбъект);
				КонецЕсли;
		...
		Иначе // Регистр или последовательность.
			...
			ЗарегистрированныеДанные[МетаданныеЭлемента] = Новый ТаблицаЗначений;
			...
			Для Каждого ЭлементОтбора Из НаборЗаписей.Отбор Цикл
				Если НаборДанных.Колонки.Найти(ЭлементОтбора.Имя) <> Неопределено Тогда
					ЗарегистрированныеДанные[МетаданныеЭлемента].Колонки.Добавить(ЭлементОтбора.Имя);
				КонецЕсли;
			КонецЦикла;
			...
			Для Каждого СтрокаДанные Из НаборДанных Цикл
				...
				НаборЗаписей = МенеджерОбъекта.СоздатьНаборЗаписей();
				Для Каждого ЭлементОтбора Из ЗарегистрированныеДанные[МетаданныеЭлемента].Колонки Цикл
					НаборЗаписей.Отбор[ЭлементОтбора.Имя].Установить(СтрокаДанные[ЭлементОтбора.Имя]);
				КонецЦикла;
				
				НаборЗаписей.Прочитать();
				
				Если ВыгрузкаОбъектаРазрешена(УзелОбъект.Ссылка, НаборЗаписей) Тогда
					СтруктураОтбора = ЗарегистрированныеДанные[МетаданныеЭлемента].Добавить();
