...
//            Если Истина - объект метаданных имеет признак авторегистрации "Разрешена" в составе плана обмена;
//            иначе, если Ложь   - объект метаданных имеет признак авторегистрации "Запрещена" в составе плана обмена
...
//
//  УзелПланаОбмена     - ПланОбменаОбъект - узел,
...
//                 - СправочникОбъект
//                 - ДокументОбъект
//                 - РегистрСведенийНаборЗаписей - элемент данных.
...
//
// Параметры:
//  Объект - СправочникОбъект, ДокументОбъект - записываемый объект ИБ.
//  СведенияОВерсииОбъекта - Структура:
//    * АвторВерсии - СправочникСсылка.Пользователи, ПланОбменаСсылка - источник версии.
...
//  Отправитель - ПланОбменаОбъект - план обмена БСД.
//
...
Процедура ОчиститьСсылкиНаУзелИнформационнойБазы(Источник, Отказ) Экспорт
	...
	ТранспортСообщенийОбмена.УдалитьВсеНастройкиТранспорта(Источник.Ссылка);
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ОчередьЗаданий")
			...
			Если ЗначениеЗаполнено(Источник.Код) Тогда
				...
				Отбор.Вставить("Ключ",          Источник.Код);
			
				ТаблицаЗадания = МодульОчередьЗаданий.ПолучитьЗадания(Отбор);
				Если ТаблицаЗадания <> Неопределено Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ОбменДаннымиВМоделиСервиса") Тогда
			...
			ПоУзлуЗапланированОбмен = МодульОбменДаннымиВнутренняяПубликация.ПоУзлуЗапланированОбмен(
				Источник.Ссылка, СценарииОбменовДанными,ИдентификаторОбмена);
			
			Если ПоУзлуЗапланированОбмен Тогда
				МодульОбменДаннымиВнутренняяПубликация.ОтменитьОчередьЗадач(
					Источник.Ссылка, СценарииОбменовДанными, ИдентификаторОбмена, ОбластьДанных);
			КонецЕсли;	
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
	...
	НеобязательныеПараметры.Вставить("ЗамещениеРасширенное", Неопределено);
	...
	Если ДополнительныеПараметры <> Неопределено Тогда
		...
		АдаптироватьЗначениеЗамещенияНеобязательныхПараметровРегистрации(НеобязательныеПараметры);
	...
	ЗамещениеРасширенное = НеобязательныеПараметры.ЗамещениеРасширенное;
	Если ЗначениеЗаполнено(ЗамещениеРасширенное) И ТипЗнч(ЗамещениеРасширенное) <> Тип("Булево") Тогда
		Объект.ДополнительныеСвойства.Вставить("ЗамещениеРасширенное", ЗамещениеРасширенное);
	КонецЕсли;
	...
	Попытка
		...
		Если Не ОбъектМодифицирован Тогда
			...
			Если ОбменДаннымиПовтИсп.АвтоРегистрацияРазрешена(ИмяПланаОбмена, ОбъектМетаданных.ПолноеИмя()) Тогда
				...
				СократитьПолучателей(Объект, ОбменДаннымиПовтИсп.УзлыПланаОбмена(ИмяПланаОбмена));
...
// Метод выполняет проверку значения режима замещения в необязательных параметрах.
// Если таковой указан как системное перечисление "РежимЗамещения", то выполняет его приведение к типу Булево.
Процедура АдаптироватьЗначениеЗамещенияНеобязательныхПараметровРегистрации(НеобязательныеПараметры)
	
	ВерсияБСП = СтандартныеПодсистемыСервер.ВерсияБиблиотеки();
	
	ЭтаВерсияНижеБСП_3_1_11 = (ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.11.0") <= 0);
	Если ЭтаВерсияНижеБСП_3_1_11
		ИЛИ НеобязательныеПараметры.Замещение = Неопределено
		ИЛИ ТипЗнч(НеобязательныеПараметры.Замещение) = Тип("Булево")
		Тогда
	...
	ЭтоРасширенныйРежимЗамещения = МодульОбщегоНазначения.ЭтоЗамещениеНабораЗаписей(НеобязательныеПараметры.Замещение)
		ИЛИ МодульОбщегоНазначения.ЭтоОбновлениеНабораЗаписей(НеобязательныеПараметры.Замещение)
		ИЛИ МодульОбщегоНазначения.ЭтоСлияниеНабораЗаписей(НеобязательныеПараметры.Замещение)
		ИЛИ МодульОбщегоНазначения.ЭтоУдалениеНабораЗаписей(НеобязательныеПараметры.Замещение);
	
	Если ЭтоРасширенныйРежимЗамещения = Истина Тогда
		
		НеобязательныеПараметры.ЗамещениеРасширенное = НеобязательныеПараметры.Замещение;
		НеобязательныеПараметры.Замещение = Истина;
		Возврат;
		
	КонецЕсли;
	
	НеобязательныеПараметры.Замещение = Ложь;
...
Функция ОпределитьМассивУзловПоЗначениямСвойств(ЗначенияСвойств, ПРО, Знач ИмяПланаОбмена, Объект, Знач Выгрузка, ДополнительныеСвойства = Неопределено)
	...
	Если ИспользоватьКэш Тогда
		...
		ЗначенияСвойствСтрока = ОбщегоНазначения.ЗначениеВСтрокуXML(ЗначенияСвойств);
		Возврат ОбменДаннымиПовтИсп.МассивУзловПоЗначениямСвойств(
			ЗначенияСвойствСтрока, ТекстЗапроса, ИмяПланаОбмена, ПРО.ИмяРеквизитаФлага, Выгрузка);
...
Функция ОпределитьМассивУзловПоЗначениямСвойствДополнительный(ЗначенияСвойств, ПРО, Знач ИмяПланаОбмена, Объект, ДополнительныеСвойства = Неопределено)
	...
	Если ИспользоватьКэш Тогда
		...
		ЗначенияСвойствСтрока = ОбщегоНазначения.ЗначениеВСтрокуXML(ЗначенияСвойств);
		Возврат ОбменДаннымиПовтИсп.МассивУзловПоЗначениямСвойств(
			ЗначенияСвойствСтрока, ТекстЗапроса, ИмяПланаОбмена, ПРО.ИмяРеквизитаФлага);
...
// 
// Параметры:
//  ЗначенияСвойств - Структура - Значения свойств объекта
//  ТекстЗапроса - Строка
//  ИмяПланаОбмена - Строка - Имя плана обмена как оно задано в конфигураторе
...
// 
// Возвращаемое значение:
//  Массив Из ПланОбменаСсылка - Массив узлов по значениям свойств
//
...
// 
// Параметры: 
//  ОбъектМетаданных - ОбъектМетаданных - Объект метаданных, предусматривающий набор записей
// 
// Возвращаемое значение:
//  РегистрСведенийНаборЗаписей, РегистрРасчетаНаборЗаписей, РегистрБухгалтерииНаборЗаписей -
//  - РегистрРасчетаНаборЗаписей, ПоследовательностьНаборЗаписей - набор записей.
//
	...
	Если ПРО.ЕстьОбработчикПередОбработкой Тогда
		...
		Попытка
			...
			Иначе
				...
				ПрофильБезопасностиИБ = МодульСтандартныеПодсистемыСервер.ПрофильБезопасностиДляВыполнитьВычислить();
				Если ЗначениеЗаполнено(ПрофильБезопасностиИБ) Тогда
					УстановитьБезопасныйРежим(ПрофильБезопасностиИБ);
				КонецЕсли;
...
Процедура ПроверитьУстранениеПроблемыДокументаОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	РегистрыСведений.ОбъектыКЗагрузке.ОбработатьЗаписьПроблемыЗагруженногоОбъектаПриЕгоЗаписи(Источник);
...
Функция НаборЗаписей(Знач Данные)
	...
	ЗамещениеРасширенное = Неопределено;
	Если Данные.ДополнительныеСвойства.Свойство("ЗамещениеРасширенное", ЗамещениеРасширенное) Тогда
		Если ОбщегоНазначения.ЭтоСлияниеНабораЗаписей(ЗамещениеРасширенное)
			ИЛИ ОбщегоНазначения.ЭтоОбновлениеНабораЗаписей(ЗамещениеРасширенное)
			ИЛИ ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(ЗамещениеРасширенное) Тогда
			
			НаборЗаписей.РасширенныеРежимыЗамещения = Данные.РасширенныеРежимыЗамещения;
			НаборЗаписей.ДополнительныеСвойства.Вставить("ЗамещениеРасширенное", ЗамещениеРасширенное);
			ДанныеНабораИзИБ = ОбщегоНазначения.ЗаписиНабораИзБазыДанных(Данные, ЗамещениеРасширенное);
			НаборЗаписей.Загрузить(ДанныеНабораИзИБ);
			
			Возврат НаборЗаписей;
			
		ИначеЕсли ОбщегоНазначения.ЭтоДобавлениеНабораЗаписей(ЗамещениеРасширенное) Тогда
			НаборЗаписей.РасширенныеРежимыЗамещения = Данные.РасширенныеРежимыЗамещения;
			НаборЗаписей.ДополнительныеСвойства.Вставить("ЗамещениеРасширенное", ЗамещениеРасширенное);
...
Процедура ИсключитьРегистрациюИзЗацикленныхУзлов(МассивУзловРезультат, Объект, ИмяПланаОбмена)
	...
	Если Не(Объект.ОбменДанными.Загрузка
		Или Объект.ДополнительныеСвойства.Свойство("ОтложеннаяЗапись")
		Или Объект.ДополнительныеСвойства.Свойство("ОтложенноеПроведение")) Тогда
