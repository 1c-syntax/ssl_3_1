...
//   * Пароль                    - Строка
//                               - Неопределено - пароль, используемый при стандартной аутентификации.
//   * СохраняемоеЗначениеПароля - Строка
//                               - Неопределено - хранимые значения пароля (хеш).
...
//                               - Неопределено - имя основного интерфейса пользователя информационной базы
//                                         (из коллекции Метаданные.Интерфейсы).
//   * РежимЗапуска              - Строка
//                               - Неопределено - "Авто", "ОбычноеПриложение" или "УправляемоеПриложение".
//   * Язык                      - Строка
//                               - Неопределено - имя языка из коллекции Метаданные.Языки.
...
Процедура УстановитьСвойстваПользователяИБ(Знач ИмяИлиИдентификатор, Знач ОбновляемыеСвойства,
	...
	Для Каждого КлючИЗначение Из НовыеСвойства Цикл
		...
		ИмяДляВхода = ?(ОбновляемыеСвойства.Свойство("Имя"), ОбновляемыеСвойства.Имя, НовыеСвойства.Имя);
...
Процедура УстановитьНастройкиВхода(СохраняемыеНастройки, ДляВнешнихПользователей = Ложь) Экспорт
	...
	Попытка
		...
		Настройки = ПользователиСлужебный.НастройкиВхода();
		
		Если ДляВнешнихПользователей Тогда
			НастройкиВхода = Настройки.ВнешниеПользователи;
		Иначе
			НастройкиВхода = Настройки.Пользователи;
		КонецЕсли;
		
		Для Каждого СохраняемаяНастройка Из СохраняемыеНастройки Цикл
			
			Если Не НастройкиВхода.Свойство(СохраняемаяНастройка.Ключ)
				Или ТипЗнч(НастройкиВхода[СохраняемаяНастройка.Ключ]) <> ТипЗнч(СохраняемаяНастройка.Значение) Тогда
				Продолжить;
			КонецЕсли;
			
			НастройкиВхода[СохраняемаяНастройка.Ключ] = СохраняемаяНастройка.Значение;
			
		КонецЦикла;
		
		Если Не ЗначениеЗаполнено(НастройкиВхода.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода) Тогда
			НастройкиВхода.ПросрочкаРаботыВПрограммеДатаВключения = Дата(1, 1, 1);
		ИначеЕсли Не ЗначениеЗаполнено(НастройкиВхода.ПросрочкаРаботыВПрограммеДатаВключения) Тогда
			НастройкиВхода.ПросрочкаРаботыВПрограммеДатаВключения = НачалоДня(ТекущаяДатаСеанса());
		КонецЕсли;
		
		УстановитьПривилегированныйРежим(Истина);
		
		Константы.НастройкиВходаПользователей.Установить(Новый ХранилищеЗначения(Настройки));
		...
		Если Не РазделениеВключено Тогда
			...
			Если ЗначениеЗаполнено(Настройки.Пользователи.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода)
...
// Возвращаемое значение:
//  Структура:
//   * ПарольДолженОтвечатьТребованиямСложности - Булево - должен ли пароль отвечать требованиям сложности:
...
//   * МаксимальныйСрокДействияПароля - Число - максимальный срок действия пароля.
//   * МинимальныйСрокДействияПароля - Число - минимальный срок действия пароля.
//   * ЗапретитьПовторениеПароляСредиПоследних - Число - количество последних паролей,
//   													 которые запрещено использовать повторно
//   * ПросрочкаРаботыВПрограммеДоЗапрещенияВхода - Число - количество дней не активности пользователя,
// 														    после чего будет запрещен вход
//   * ПросрочкаРаботыВПрограммеДатаВключения - Дата - дата включения просрочки работы в программе
//
...
Процедура ДобавитьИспользованныйПароль(Пользователь, СохраняемоеЗначениеПароля) Экспорт
	...
	ПользователиСлужебный.ДобавитьИспользованныйПароль(Пользователь, СохраняемоеЗначениеПароля);
