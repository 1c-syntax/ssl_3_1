...
//                                              если ссылка пустая, тогда найден только пользователь ИБ.
...
//		ПарольСовпадает = Вычислить("ПроверитьСоответствиеПароляПользователяСохраняемомуЗначению(Пароль, ПользовательИБ)"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов метода платформы.
//		// АПК:488-вкл
...
// (для облегчения запоминания, параметр ЗапоминающийсяПароль = Истина).
//
// Параметры:
//  СвойстваПароля - см. СвойстваПароля
//                 - Число - устарел (наименьшая длина)
//  УдалитьСложный         - Булево - устарел, следует использовать СвойстваПароля, с 8.3.22 всегда Истина.
...
Функция СоздатьПароль(Знач СвойстваПароля = 7, УдалитьСложный = Ложь, УдалитьУчестьНастройки = "ДляПользователей") Экспорт
	...
	Возврат ГенераторСлучайныхПаролей.СлучайныйПароль(МинимальнаяДлинаПароля);
...
//     * Сложный - Булево - устарел, с 8.3.22 всегда Истина.
