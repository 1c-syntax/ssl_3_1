...
Функция ЗапуститьПрограмму(Знач КомандаЗапуска, ПараметрыЗапускаПрограммы = Неопределено) Экспорт 
	...
	СтрокаКоманды = ОбщегоНазначенияСлужебный.БезопаснаяСтрокаКоманды(КомандаЗапуска);
	...
	Если КодировкаПотоков = "CP866" И Не ЗначениеЗаполнено(КодировкаИсполнения) Тогда
		КодировкаИсполнения = КодировкаПотоков;
	КонецЕсли;
