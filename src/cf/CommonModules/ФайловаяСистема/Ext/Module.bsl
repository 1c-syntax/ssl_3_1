...
Функция ОбщийКаталогВременныхФайлов(ВложенныйКаталог = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И НЕ ОбщегоНазначения.РежимОтладки() Тогда
		...
		Результат = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПолучитьИмяВременногоФайла(ВложенныйКаталог));
	...
	Иначе
		...
		Если ВложенныйКаталог <> Неопределено Тогда
			Результат = Результат + ПолучитьРазделительПутиСервера() + ВложенныйКаталог;
			СоздатьКаталог(Результат);
		КонецЕсли;
