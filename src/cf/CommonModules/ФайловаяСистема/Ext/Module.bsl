...
// Функция возвращает путь к общему каталогу временных файлов.
//
// Путь к каталогу определяется в общих настройках администрирования.
// Использование этого пути при работе конфигурации с использованием кластера серверов обеспечивает гарантированное 
// общее место хранения файлов.
...
//
Функция КаталогОбщегоВременногоХранилищаФайлов(ПодКаталог = Неопределено) Экспорт
	
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И НЕ ОбщегоНазначения.РежимОтладки() Тогда
		
		Результат = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПолучитьИмяВременногоФайла(ПодКаталог));
	...
	Иначе
		...
		Если ПодКаталог <> Неопределено Тогда
			Результат = Результат + ПолучитьРазделительПутиСервера() + ПодКаталог;
			СоздатьКаталог(Результат);
		КонецЕсли;
