...
Функция ТолькоСимволыНациональногоАлфавитаВСтроке(Знач ПроверяемаяСтрока, Знач ДополнительныеДопустимыеСимволы = Неопределено) Экспорт
	...
	// Локализация
	СимволыНациональногоАлфавита = СимволыНациональногоАлфавита + "абвгдеёжзийклмнопрстуфхцчшщъыьэюя"; // АПК:163 буква ё разрешена в данном контексте.
	// Конец Локализация
	
	Если ДополнительныеДопустимыеСимволы = Неопределено Тогда
		ДопустимыеСимволы = " " + Символы.Таб + Символы.ПС + Символы.ВК + Символы.ВТаб + Символы.НПП + Символы.ПФ;
	КонецЕсли;
