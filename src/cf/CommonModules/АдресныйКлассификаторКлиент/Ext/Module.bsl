...
Процедура ОткрытьСведенияОбАдресномКлассификаторе(Параметры, ОбработчикЗавершения = Неопределено) Экспорт
	
	ОткрытьФорму(
		"РегистрСведений.АдресныеОбъекты.Форма.СведенияОбАдресномКлассификаторе", Параметры, , , , , ОбработчикЗавершения);
...
Процедура АнализДоступностиФайловКлассификатораВКаталоге(ОписаниеЗавершения, КодыРегионов, Каталог, ПараметрыЗагрузки) Экспорт
	...
	ДополнительныеПараметры.ЗагружатьПомещения = ПараметрыЗагрузки.ЗагружатьПомещения;
...
Функция ПараметрыЗагрузки()
	...
	ДополнительныеПараметры.Вставить("ЗагружатьПомещения", Ложь);
...
Процедура АнализДоступностиФайловКлассификатораВКаталогеЗавершение(ИндексРегиона, ДополнительныеПараметры) Экспорт
	...
	Если ИндексРегиона <= ДополнительныеПараметры.Результат.КодыРегионов.Количество() - 1 Тогда
		...
		Если ДополнительныеПараметры.ЗагружатьПомещения Тогда
			ИмяФайла = Формат(КодРегиона.Значение, "ЧЦ=2; ЧН=; ЧВН=; ЧГ=")+ "apt" + ".ZIP";
			МаскаФайла = МаскаФайла(ИмяФайла);
		Иначе
			ИмяФайла = Формат(КодРегиона.Значение, "ЧЦ=2; ЧН=; ЧВН=; ЧГ=") + ".ZIP";
			МаскаФайла = МаскаФайла(Формат(КодРегиона.Значение, "ЧЦ=2; ЧН=; ЧВН=; ЧГ=")+ "???" + ".ZIP");
		КонецЕсли;
		...
		НачатьПоискФайлов(ОписаниеОповещения, ДополнительныеПараметры.РабочийКаталог, МаскаФайла);
	...
	Иначе // окончание цикла
		...
		Для каждого ОтсутствующийФайл Из ДополнительныеПараметры.ОтсутствующиеФайлы Цикл
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(НСтр("ru = 'Для региона ""%1"" отсутствует файл адресного классификатора ""%2"".
			|Загрузите актуальную версию с сайта <a href=https://its.1c.ru/download/gar>https://its.1c.ru/download/gar</a>'"), Представление, ОтсутствующийФайл.Значение);
...
Процедура АнализДоступностиФайловКлассификатораВКаталогеПослеПоискаФайлов(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураФайла.Существует Тогда
		...
		ДополнительныеПараметры.ИмяФайла = СтруктураФайла.Имя;
	Иначе
