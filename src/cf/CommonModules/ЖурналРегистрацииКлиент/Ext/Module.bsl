...
// АПК:142-выкл Необязательных параметров более 3 для обратной совместимости

// Записывает сообщение в журнал регистрации.
// Если параметр ЗаписатьСобытия = Истина, то запись выполняется сразу (произойдет вызов сервера).
...
//
//  Параметры: 
//   ИмяСобытия          - Строка - имя события записи журнала регистрации.
//   ПредставлениеУровня - Строка - имя значения коллекции УровеньЖурналаРегистрации
//                                  "Информация", "Ошибка", "Предупреждение", "Примечание".
//   Комментарий         - Строка - комментарий события записи журнала регистрации.
//   ДатаСобытия         - Дата   - дата события, которая вставляется в начало комментария
//                                  (если не заполнена, тогда используется текущая дата сеанса).
//   ЗаписатьСобытия     - Булево - выполнить запись всех ранее накопленных сообщений
//                                  в журнал регистрации (произойдет вызов сервера).
//
// Пример:
...
Процедура ДобавитьСообщениеДляЖурналаРегистрации(Знач ИмяСобытия, Знач ПредставлениеУровня = "Информация", 
	Знач Комментарий = "", Знач ДатаСобытия = '00010101', Знач ЗаписатьСобытия = Ложь) Экспорт
	...
	Если СтрНайти(",Информация,Ошибка,Предупреждение,Примечание,", "," + ПредставлениеУровня + ",") = 0 Тогда
		ПредставлениеУровня = ""; // По умолчанию для метода ЗаписьЖурналаРегистрации.
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДатаСобытия) Тогда
		Если ТипЗнч(ПараметрыПриложения) = Тип("Соответствие")
		   И ПараметрыПриложения["СтандартныеПодсистемы.ПараметрыКлиента"] <> Неопределено Тогда
	...
	СтруктураСообщения.Вставить("ИмяСобытия",
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(ИмяСобытия, " "));
	...
	Сообщения = ПараметрыПриложения[ИмяПараметра]; // см. ЖурналРегистрации.ЗаписатьСобытияВЖурналРегистрации
