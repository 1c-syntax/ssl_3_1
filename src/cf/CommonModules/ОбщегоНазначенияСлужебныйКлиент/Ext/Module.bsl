...
#КонецЕсли
	...
	СокращеннаяСтрока = СократитьСтрокуСИпользованиемХеш(ИмяБезРасширения, ИспользуемоеКоличествоСимволов);
...
Функция ВычислитьХешСтрокиПоАлгоритмуMD5(Знач Строка)
	...
	ЧтениеДанных = Новый ЧтениеДанных(ПоследнийБлок);
	ЧтениеДанных.Пропустить(55);
	
	Если ЧтениеДанных.ПрочитатьЦелое64() <> 0 Тогда
	...
	Для НомерБлока = 0 По МассивБлоковИзСтроки.Количество() - 1 Цикл 
		...
		Если Блок = 0 Тогда
			...
			КонецЦикла;
		Иначе
			ЧтениеДанных = Новый ЧтениеДанных(Блок);
			Для НомерСлова = 0 По 15 Цикл
				Если ЧтениеДанных.ЧтениеЗавершено Тогда
					X[НомерСлова] = 0;
				Иначе
					Слово = ЧтениеДанных.ПрочитатьЦелое32();
...
Функция СократитьСтрокуСИпользованиемХеш(Строка, ИспользуемоеКоличествоСимволов)
...
Функция ЧислоВДвоичныеДанные(Число)   
	Поток = Новый ПотокВПамяти(4);
	ЗаписьДанных = Новый ЗаписьДанных(Поток);
	ЗаписьДанных.ЗаписатьЦелое32(Число);
