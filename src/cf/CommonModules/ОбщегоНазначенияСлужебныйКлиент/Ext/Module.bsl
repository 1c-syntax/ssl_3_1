...
Функция КонтекстПодключенияКомпоненты() Экспорт
	...
	Контекст.Вставить("ПоказыватьВопросУстановки", Истина);
...
Асинх Функция ПодключитьКомпонентуАсинх(Контекст) Экспорт
	...
	Если Не Контекст.ВыполненПоискНовойВерсии Тогда
		...
		Если Контекст.Кэшировать Тогда
			...
			СвойстваПодключаемогоМодуля = ПолучитьОбъектКомпонентыИзКэша(Контекст.Местоположение);

			Если СвойстваПодключаемогоМодуля <> Неопределено Тогда
				Результат = РезультатПодключенияКомпоненты();
				Результат.Подключено = Истина;
				Результат.ПодключаемыйМодуль = СвойстваПодключаемогоМодуля.ПодключаемыйМодуль;
				Результат.Местоположение = СвойстваПодключаемогоМодуля.Местоположение;
				Результат.СимволическоеИмя = СвойстваПодключаемогоМодуля.Имя;
				Возврат Результат;
			КонецЕсли;
...
Процедура ПодключитьКомпоненту(Контекст) Экспорт
	...
	Если Не Контекст.ВыполненПоискНовойВерсии Тогда
		...
		Если Контекст.Кэшировать Тогда
			...
			Если СвойстваПодключаемогоМодуля <> Неопределено Тогда
				ПодключитьКомпонентуОповеститьОПодключении(СвойстваПодключаемогоМодуля, Контекст);
				Возврат;
			КонецЕсли;
...
Функция РезультатПодключенияКомпоненты() Экспорт
	...
	Результат.Вставить("СимволическоеИмя", "");
...
//      * ПоказыватьВопросУстановки - Булево
//
Процедура УстановитьКомпоненту(Контекст) Экспорт
	...
	Если СимволическоеИмя = Неопределено Тогда
		...
		Если Контекст.ПоказыватьВопросУстановки Тогда
			Оповещение = Новый ОписаниеОповещения(
		...
		Иначе
			УстановитьКомпонентуПослеОтветаНаВопросОбУстановке(КодВозвратаДиалога.Да, Контекст);
		КонецЕсли;
...
Процедура СократитьИмяФайла(ИмяФайла) Экспорт
	...
	ХешСумма = ВычислитьХешСтрокиПоАлгоритмуMD5(ОстатокСтроки);
...
Процедура ПодключитьКомпонентуПослеПопыткиПодключения(Подключено, Контекст) Экспорт 
	...
	Если Подключено Тогда 
		...
		Если Контекст.Кэшировать Тогда 
			ЗаписатьОбъектКомпонентыВКэш(ИсходноеМестоположение, ПодключаемыйМодуль, Контекст.Местоположение, Контекст.СимволическоеИмя);
		КонецЕсли;
		
		ПодключитьКомпонентуОповеститьОПодключении(Новый Структура("ПодключаемыйМодуль, Местоположение, Имя",
			ПодключаемыйМодуль, Контекст.Местоположение, Контекст.СимволическоеИмя), Контекст);
	...
	Иначе 
		...
		Иначе
			...
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ВнешниеКомпоненты") Тогда
				Оповещение = Новый ОписаниеОповещения("ПослеПроверкиСовместимостиКомпонентыИзМакета", ЭтотОбъект, Контекст);
				МодульВнешниеКомпонентыСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ВнешниеКомпонентыСлужебныйКлиент");
				МодульВнешниеКомпонентыСлужебныйКлиент.ПроверитьСовместимостьКомпонентыИзМакета(Оповещение, Контекст);
				Возврат;
			КонецЕсли;
			
			ПослеПроверкиСовместимостиКомпонентыИзМакета("", Контекст);
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"".
						 ...
						 |%3
						 |Метод %4 вернул Ложь.'"), Контекст.Идентификатор, Результат,
			Контекст.Местоположение, "НачатьПодключениеВнешнейКомпоненты");
	Иначе
		ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"".
...
Процедура ПодключитьКомпонентуНачатьУстановку(Контекст)
	...
	КонтекстУстановки = КонтекстПодключенияКомпоненты();
	ЗаполнитьЗначенияСвойств(КонтекстУстановки, Контекст);
	КонтекстУстановки.Оповещение = Оповещение;
...
Асинх Функция НовыйОбъектКомпонентыАсинх(Контекст)
	...
	Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда 
		...
		Попытка                         
			ИдентификаторКомпоненты = "AddIn." + Контекст.СимволическоеИмя + "." + Контекст.Идентификатор;
#Если ВебКлиент Тогда
			ПодключаемыйМодуль = Ждать СоздатьОбъектВнешнейКомпонентыАсинх(ИдентификаторКомпоненты);
#Иначе 
			ПодключаемыйМодуль = Новый(ИдентификаторКомпоненты);
#КонецЕсли
	...
	Иначе 
		...
		Для каждого ИдентификаторОбъекта Из Контекст.ИдентификаторыСозданияОбъектов Цикл 
			...
			Попытка              
				ИдентификаторКомпоненты = "AddIn." + Контекст.СимволическоеИмя + "." + ИдентификаторОбъекта;
#Если ВебКлиент Тогда
				ПодключаемыйМодуль = Ждать СоздатьОбъектВнешнейКомпонентыАсинх(ИдентификаторКомпоненты);
#Иначе 
				ПодключаемыйМодуль = Новый(ИдентификаторКомпоненты);
#КонецЕсли
	...
	ОтправитьСтатистикуПодключенияКомпоненты(Контекст.Идентификатор, Контекст.Местоположение);
...
Процедура ОтправитьСтатистикуПодключенияКомпоненты(Идентификатор, Местоположение)
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЦентрМониторинга") Тогда
		Возврат;
	КонецЕсли;
	
	МодульЦентрМониторингаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЦентрМониторингаКлиент");
	
	МодульЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики(
		"СтандартныеПодсистемы.ВнешниеКомпоненты." + Идентификатор + ".НаКлиенте." + ?(СтрНачинаетсяС(
		Местоположение, "e1cib"), "Справочник", "Макет"), 1);
...
Асинх Функция ПодключитьКомпонентуПослеПопыткиПодключенияАсинх(Подключено, Контекст)
	...
	Если Подключено Тогда 
		...
		Попытка
			ПодключаемыйМодуль = Ждать НовыйОбъектКомпонентыАсинх(Контекст);
		Исключение
			// Текст ошибки уже скомпонован в НовыйОбъектКомпонентыАсинх, требуется только оповестить.
			ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Возврат ОшибкаПодключенияКомпоненты(ТекстОшибки);
		КонецПопытки;

		Если Контекст.Кэшировать Тогда
			ЗаписатьОбъектКомпонентыВКэш(
				ИсходноеМестоположение, ПодключаемыйМодуль, Контекст.Местоположение, Контекст.СимволическоеИмя);
		КонецЕсли;
		
		Результат = РезультатПодключенияКомпоненты();
		Результат.Подключено = Истина;
		Результат.ПодключаемыйМодуль = ПодключаемыйМодуль;
		Результат.Местоположение = Контекст.Местоположение;
		Результат.СимволическоеИмя = Контекст.СимволическоеИмя;
		Возврат Результат;
...
Функция ПолучитьОбъектКомпонентыИзКэша(КлючОбъекта)
	...
	Если ТипЗнч(КэшированныеОбъекты) = Тип("ФиксированноеСоответствие") Тогда
		СвойстваПодключаемогоМодуля = КэшированныеОбъекты.Получить(КлючОбъекта);
		Если СвойстваПодключаемогоМодуля <> Неопределено Тогда
			Если ПроверитьПодключениеВнешнейКомпоненты(СвойстваПодключаемогоМодуля.Местоположение,
				СвойстваПодключаемогоМодуля.Имя) Тогда
					Возврат СвойстваПодключаемогоМодуля;
			Иначе
				ОчиститьОбъектКомпонентыВКэше(КлючОбъекта);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьОбъектКомпонентыВКэш(КлючОбъекта, ПодключаемыйМодуль, Местоположение, Имя)
	...
	СвойстваПодключаемогоМодуля = Новый Структура("ПодключаемыйМодуль, Местоположение, Имя",
		ПодключаемыйМодуль, Местоположение, Имя);
	Соответствие.Вставить(КлючОбъекта, СвойстваПодключаемогоМодуля);
...
Процедура ОчиститьОбъектКомпонентыВКэше(КлючОбъекта)
	
	КэшированныеОбъекты = ПараметрыПриложения["СтандартныеПодсистемы.ВнешниеКомпоненты.Объекты"];
	
	Если ТипЗнч(КэшированныеОбъекты) = Тип("ФиксированноеСоответствие")
		И КэшированныеОбъекты.Получить(КлючОбъекта) <> Неопределено Тогда
		
		Соответствие = Новый Соответствие;
		Для каждого Элемент Из КэшированныеОбъекты Цикл
			Если Элемент.Ключ = КлючОбъекта Тогда
				Продолжить;
			КонецЕсли;
			Соответствие.Вставить(Элемент.Ключ, Элемент.Значение);
		КонецЦикла;
	...
	Если ТипЗнч(КэшированныеСимволическоеИмена) = Тип("ФиксированноеСоответствие")
		И КэшированныеСимволическоеИмена.Получить(КлючОбъекта) <> Неопределено Тогда
...
Процедура ЗарегистрироватьCOMСоединительПриПроверкеРегистрации(Результат, Контекст) Экспорт
	...
	Если ПриложениеЗапущено Тогда
		...
		Если ВыполнитьПерезагрузкуСеанса Тогда
			...
			ТекстВопроса = 
				НСтр("ru = 'Для завершения перерегистрации компоненты comcntr перезапустите приложение.
