...
// 
// Возвращаемое значение:
//  Строка - имя плана обмена как объекта метаданного.
//
...
// Возвращает признак использования кеша при поиске ссылок в регистре сведений 
...
//
//  Возвращаемое значение:
//    Булево - ИСТИНА, если используется кеш, иначе ЛОЖЬ.
//             Если значение ЛОЖЬ, то используются запросы к РС ПубличныеИдентификаторыСинхронизируемыхОбъектов.
// 
...
Функция СпособКонвертацииОбъектовXDTO(Знач УзелКорреспондента) Экспорт
	...
	Возврат РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.СпособОбъектнойКонвертацииXDTO(УзелКорреспондента);
...
//
// Возвращаемое значение:
//   Массив Из Строка - Массив номеров версий, поддерживаемых интерфейсом корреспондента.
//
...
// 
// Возвращаемое значение:
//  ПланОбменаСсылка - предопределенный узел плана обмена.
//
...
// 
// Возвращаемое значение:
//  Массив Из ПланОбменаСсылка - массив всех узлов для заданного плана обмена кроме предопределенного узла.
//
...
// 
// Параметры:
//  ВерсияФорматаМенеджераОбмена - Строка - Номер версии менеджера обмена
//  ВерсияФормата - Строка - Версия формата EnterpriseData
//  ТолькоПоддерживаемыеОбъектыФормата - Булево - Признак того, что требуется получить только заголовки объектов
// 
// Возвращаемое значение:
// см. ОбменДаннымиXDTOСервер.ТаблицаПравилКонвертацииОбъектовИзМодуляОбмена
//
Функция ТаблицаПравилКонвертации(ВерсияФорматаМенеджераОбмена, ВерсияФормата, ТолькоПоддерживаемыеОбъектыФормата = Ложь) Экспорт
	
	КоллекцияПравилКонвертации = ОбменДаннымиXDTOСервер.КоллекцияПравилКонвертации(ВерсияФорматаМенеджераОбмена);
	
	КомпонентыОбмена = ОбменДаннымиXDTOСервер.ИнициализироватьКомпонентыОбмена("Получение");
	
	КомпонентыОбмена.МенеджерОбмена = ОбменДаннымиXDTOСервер.МенеджерОбменаВерсииФормата(ВерсияФормата);
	КомпонентыОбмена.ВерсияФорматаМенеджераОбмена = ВерсияФорматаМенеджераОбмена;
	КомпонентыОбмена.ВерсияФорматаОбмена = ВерсияФормата;
	
	КоллекцияПравилКонвертации = ОбменДаннымиXDTOСервер.ТаблицаПравилКонвертацииОбъектовИзМодуляОбмена(КомпонентыОбмена, КоллекцияПравилКонвертации, ТолькоПоддерживаемыеОбъектыФормата);
	
	Возврат КоллекцияПравилКонвертации;
	
КонецФункции

Функция ДоступныеДляПолученияОбъектыМетаданных(УзелПланаОбмена) Экспорт
	
	Возврат РегистрыСведений.ДействияПриПолученииДанныхОбмена.ДоступныеДляПолученияОбъектыМетаданных(УзелПланаОбмена);
	
КонецФункции 

Функция ПоддерживаемыеОбъектыФормата(УзелПланаОбмена) Экспорт
	
	Возврат РегистрыСведений.ДействияПриПолученииДанныхОбмена.ПоддерживаемыеОбъектыФормата(УзелПланаОбмена);
...
// 
// Возвращаемое значение:
//   см. ОбменДаннымиСервер.ПолучитьИспользуемыеПланыОбмена
//
...
Функция ПользовательскийСоставПланаОбмена(Знач Получатель) Экспорт
	...
	ОтправкаВыключена = Ложь; // Учитываем ниже при выборе значения по-умолчанию
	
	Если РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.СпособОбъектнойКонвертацииXDTO(Получатель) Тогда
		
		ОтправкаВыключена = Истина;
		
		Для Каждого СвойствоОтправки Из СвойстваПолучателя Цикл
			
			Если СвойствоОтправки.Значение <> Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать Тогда
				
				ОтправкаВыключена = Ложь;
				Прервать;
	...
	Для Каждого Элемент Из Метаданные.ПланыОбмена[ИмяПланаОбмена].Состав Цикл
		...
		Если ПравилаОбъекта.Количество() = 0 Тогда // Правила регистрации не заданы.
			...
			Если ОтправкаВыключена Тогда
				
				РежимВыгрузки = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать;
...
//
// Параметры:
//  ИмяОбъекта - Строка -  полное имя объекта метаданных, для которого требуется получить режим выгрузки;
...
// 
// Возвращаемое значение:
//   см. ПравилаДляПланаОбменаИзКонфигурации.
//
...
// 
// Возвращаемое значение:
//  СписокЗначений Из Строка - список планов обмена конфигурации.
//
...
// Параметры:
//  УзелИнформационнойБазы - ПланОбменаСсылка - узел плана обмена, 
//                           для которого требуется получить значение функции.
// 
// Возвращаемое значение:
//  Строка 
//
...
// 
// Возвращаемое значение:
//  Структура - Табличные части плана обмена:
//    * ТаблицыОбщие - Массив из Строка
//    * ТаблицыЭтойБазы - Массив из Строка
//    * ВсеТаблицыЭтойБазы - Массив из Строка
//
...
// 
// Возвращаемое значение:
//   см. ОбменДаннымиСервер.ДеревоМетаданныхКонфигурации
//
...
//  УзелИнформационнойБазы - ПланОбменаСсылка - узел плана обмена, 
//                           для которого требуется получить значение функции.
//  ИмяФайлаСообщенияОбмена - Строка
// 
// Возвращаемое значение:
...
// 
// Параметры:
//  ЗначенияСвойств - Структура - Структура, содержащая значения свойств объекта из правил
//  ТекстЗапроса - Строка - Текст запроса из правил
//  ИмяПланаОбмена - Строка - Имя плана обмена как оно задано в конфигураторе
//  ИмяРеквизитаФлага - Строка
//  Выгрузка - Булево
// 
// Возвращаемое значение:
//   см. ОбменДаннымиСобытия.МассивУзловПоЗначениямСвойств
//
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, ТекстЗапроса, ИмяПланаОбмена, ИмяРеквизитаФлага, Знач Выгрузка = Ложь) Экспорт
	...
	Если ТипЗнч(ЗначенияСвойств) = Тип("Строка") Тогда
		ЗначенияСвойствСтруктура = ОбщегоНазначения.ЗначениеИзСтрокиXML(ЗначенияСвойств);
	Иначе
		ЗначенияСвойствСтруктура = ЗначенияСвойств;
	КонецЕсли;
	
	Возврат ОбменДаннымиСобытия.МассивУзловПоЗначениямСвойств(
		ЗначенияСвойствСтруктура, ТекстЗапроса, ИмяПланаОбмена, ИмяРеквизитаФлага, Выгрузка);
...
// 
// Возвращаемое значение:
//  Массив из Тип - Все ссылочные типы конфигурации
//
...
Функция WSПрокси(ПараметрыПодключения, СообщениеОбОшибке = "", СообщениеПользователю = "") Экспорт
	...
	Возврат ОбменДаннымиВебСервис.WSПрокси(ПараметрыПодключения, СообщениеОбОшибке, СообщениеПользователю);
...
// 
// Возвращаемое значение:
//  СписокЗначений Из Строка - список макетов типовых правил для обмена данными.
//
