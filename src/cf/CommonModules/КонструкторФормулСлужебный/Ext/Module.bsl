...
Функция ЭлементыФормулы(Знач Формула) Экспорт
	...
	Возврат КонструкторФормулКлиентСервер.ЭлементыФормулы(Формула);
...
Функция ПроверитьФормулу(Форма, ПредставлениеФормулы) Экспорт
	...
	НайденныеОперанды = Новый Соответствие;
	...
	Для Каждого Элемент Из Форма.ПодключенныеСпискиПолей Цикл
		...
		Для Каждого ПредставлениеОперанда Из ЭлементыФормулы.Операнды Цикл
			...
			Если Реквизит <> Неопределено  Тогда
				...
				КонецЕсли;
				НайденныеОперанды.Вставить(ПредставлениеОперанда, Операнд);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого Операнд Из ЭлементыФормулы.Операнды Цикл
		Позиция = СтрНайти(Выражение, Операнд);
		Если Позиция = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Если НайденныеОперанды[Операнд] = Неопределено Тогда
...
Процедура ЗаполнитьСписокДоступныхРеквизитов(Знач ТекущийРеквизит, ИсточникиДоступныхПолей, Знач Отбор = "", 
	...
	Для Каждого КоллекцияДоступныхПолей Из КоллекцииДоступныхПолей Цикл
		Для Каждого ОписаниеПоля Из КоллекцияДоступныхПолей.Элементы Цикл
			...
			Если ОписаниеПоля.Элементы.Количество() > 0 И Реквизит.Имя <> "Ссылка" Тогда
...
Функция ФормулаИзПредставления(Форма, ПредставлениеФормулы) Экспорт
	...
	ЗаменитьЭлементыФормулы(Выражение, ЭлементыФормулы, НайденныеОперанды);
	Возврат Выражение;
	...
	Для Каждого Элемент Из Форма.ПодключенныеСпискиПолей Цикл
		...
		Для Каждого Операнд Из ЭлементыФормулы.Операнды Цикл
			...
			Если Реквизит <> Неопределено Тогда
				...
				КонецЕсли;
				НайденныеОперанды.Вставить(Операнд, ПредставлениеОперанда);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПредставлениеВыражения(Знач Выражение, СпискиПолей) Экспорт
	...
	Результат = Выражение;
	НайденныеПоля  = Новый Соответствие;
		...
		Для Каждого Операнд Из ЭлементыФормулы.Операнды Цикл
			...
			Если Реквизит <> Неопределено Тогда
				...
				Если НастройкиСписка.СкобкиПредставлений Тогда
					ПредставлениеОперанда = ОбернутьВКвадратныеСкобки(Реквизит.ПредставлениеПутиКДанным);
				КонецЕсли;
				НайденныеПоля.Вставить(Операнд, ПредставлениеОперанда);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;

	ЗаменитьЭлементыФормулы(Результат, ЭлементыФормулы, НайденныеПоля);
	Возврат Результат;
...
Процедура ЗаменитьЭлементыФормулы(Выражение, ЭлементыФормулы, Значения) Экспорт
	...
	Для Каждого Операнд Из ЭлементыФормулы.Операнды Цикл
		...
		Если Позиция > 1 Тогда
			ЧастиСтроки.Добавить(Лев(Выражение, Позиция - 1));
		КонецЕсли;
		
		Значение = Значения[Операнд];
		Если Значение = Неопределено Тогда
			Если ЭлементыФормулы.Функции.Найти(Операнд) <> Неопределено
		...
		Выражение = Сред(Выражение, Позиция + СтрДлина(Операнд));
	КонецЦикла;
	
	ЧастиСтроки.Добавить(Выражение);
	Выражение = СтрСоединить(ЧастиСтроки);
