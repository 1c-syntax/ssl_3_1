...
Процедура ОбработчикКонструктораФормул(Форма, Параметр, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли КлючОперации = "ОбработатьСообщениеПоиска" Тогда	
...
Функция ЗапуститьПоискВСпискеПолей(СтруктураФормы) Экспорт
	...
	СтруктураЭлементаФормы = Новый Структура("Видимость", НЕ ОтборУстановлен);
	Элементы.Вставить(ИмяСпискаПолей + "Представление", СтруктураЭлементаФормы);
	
	СтруктураЭлементаФормы = Новый Структура("Видимость", ОтборУстановлен);
	Элементы.Вставить(ИмяСпискаПолей + "ПредставлениеПутиКДанным", СтруктураЭлементаФормы);
	
	СтруктураЭлементаФормы = Новый Структура("Отображение", ?(ОтборУстановлен, ОтображениеТаблицы.Список, ОтображениеТаблицы.Дерево));
	Элементы.Вставить(ИмяСпискаПолей, СтруктураЭлементаФормы);
	...
	Иначе
		...
		Если МассивНайденныхЭлементов.Количество()  <> 0 Тогда
			...
			Для Каждого НайденныйЭлемент Из МассивНайденныхЭлементов Цикл
				ИдентификаторыСтрок.Добавить(НайденныйЭлемент.Идентификатор);
			КонецЦикла;
			
			СокращенноеДерево = СократитьДерево(ДеревоПоиска, ИдентификаторыСтрок);
			Результат = Новый Структура("ДеревоЭлементов, НайденныеЭлементы", СокращенноеДерево, ИдентификаторыСтрок);
			Возврат Результат;
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗапуститьФоновыйПоискВСпискеПолей(Форма)
	...
	ДлинаСтрокиОтбора = СтрДлина(СтрокаОтбора);
	ИмяСписка = ИмяРеквизитаСпискаПолей(ИмяТекущегоРеквизитаСтрокиПоиска);
	ТаблицаТекущегоСписка = ДеревоДоступныхРеквизитов(Форма, ИмяСписка);
	...
	ПодключенныйСписокПолей = ПодключенныеСпискиПолейСтрокиПоиска[0];
	
	СообщениеСтрокиОжидания = НСтр("ru = 'Продолжите ввод...'");
	СтрокиДерева = ТаблицаТекущегоСписка.Строки;		// КоллекцияСтрокДереваЗначений
	
	СтрокиПоОтбору = СтрокиДерева.НайтиСтроки(Новый Структура("СоответствуетОтбору", Истина), Истина);
	Для Каждого СтрокаПоОтбору Из СтрокиПоОтбору Цикл
		СтрокаПоОтбору.СоответствуетОтбору = Ложь;
	КонецЦикла;
	
	СтрокиПоОтбору = СтрокиДерева.НайтиСтроки(Новый Структура("ПодчиненныйЭлементСоответствуетОтбору", Истина), Истина);
	Для Каждого СтрокаПоОтбору Из СтрокиПоОтбору Цикл
		СтрокаПоОтбору.ПодчиненныйЭлементСоответствуетОтбору = Ложь;
	КонецЦикла;
	
	СтрокаОжидания = СтрокиДерева.Найти(СообщениеСтрокиОжидания, "Заголовок", Ложь); // СтрокаДереваЗначений: см. ДеревоДоступныхРеквизитов
	
	Если ДлинаСтрокиОтбора <> 0 И ДлинаСтрокиОтбора < ПодключенныйСписокПолей.КоличествоСимволовДляНачалаПоиска Тогда
		СтрокаОтбора = "";
		Форма.Элементы[ИмяСписка].Доступность = Ложь;
		Если СтрокаОжидания = Неопределено Тогда
			СтрокаОжидания = СтрокиДерева.Добавить();
		КонецЕсли;
		СтрокаОжидания.Заголовок = СообщениеСтрокиОжидания;
		СтрокаОжидания.ПредставлениеПутиКДанным = СообщениеСтрокиОжидания;
		СтрокаОжидания.СоответствуетОтбору = Истина;
		Форма.Элементы[ИмяСписка].Отображение = ОтображениеТаблицы.Дерево;
		ЗначениеВДанныеФормы(ТаблицаТекущегоСписка, Форма[ИмяСписка]);
   		Возврат Неопределено;
	Иначе
		Если СтрокаОжидания <> Неопределено Тогда
			СтрокиДерева.Удалить(СтрокаОжидания);
		КонецЕсли;
		Форма.Элементы[ИмяСписка].Доступность = Истина;
		ЗначениеВДанныеФормы(ТаблицаТекущегоСписка, Форма[ИмяСписка]);
	КонецЕсли;
	...
	Для Каждого ПодключенногоСпискаПолей Из ПодключенныеСпискиПолейСтрокиПоиска Цикл
		...
		УбратьПризнакОтбора(ДеревоПолей);
...
Процедура УстановитьОтбор(Знач Форма, Знач ИмяСписка, Знач Отбор,
	...
	Пока Истина Цикл	
		...
		Пока Истина Цикл
			...
			Если ЭтоФоновоеЗадание Тогда
				...
				Если МассивНайденныхЭлементов.Количество()  <> 0 Тогда
					...
					СокращенноеДерево = СократитьДерево(КоллекцияРеквизитов, ИдентификаторыСтрок);
					Результат = Новый Структура("ДеревоЭлементов, НайденныеЭлементы", СокращенноеДерево, ИдентификаторыСтрок);
					ОбщегоНазначения.СообщитьПользователю(ОбщегоНазначения.ЗначениеВСтрокуXML(Результат));
				КонецЕсли;
			КонецЕсли;
		...
		Уровень = Уровень + 1;
		Если Уровень > МаксимальныйУровеньПоиска Тогда
			Прервать;
		КонецЕсли;
...
Функция СократитьДерево(Дерево, ИдентификаторыСтрок)
	Результат = Дерево.Скопировать();
	СтрокиДляОтправки = Новый Соответствие;
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		СтрокаДерева = Результат.Строки.Найти(ИдентификаторСтроки, "Идентификатор", Истина);
		СтрокиРодителей(Результат, СтрокаДерева, СтрокиДляОтправки);
	КонецЦикла;
	
	УдалитьСтрокиНеВСписке(Результат, СтрокиДляОтправки);
	Возврат Результат;
КонецФункции

Процедура УдалитьСтрокиНеВСписке(Дерево, Фильтр)
	МассивСтрокДляУдаленияПодчиненных = Новый Массив;
	Для Каждого Строка Из Дерево.Строки Цикл
		Если Фильтр[Строка] = Истина Тогда
			УдалитьСтрокиНеВСписке(Строка, Фильтр)
		Иначе
			МассивСтрокДляУдаленияПодчиненных.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаУдаления Из МассивСтрокДляУдаленияПодчиненных Цикл
		СтрокаУдаления.Строки.Очистить();
	КонецЦикла;
КонецПроцедуры

Процедура СтрокиРодителей(Дерево, СтрокаДерева, СтрокиДляОтправки)
	СтрокиДляОтправки.Вставить(СтрокаДерева, Истина);
	Если СтрокаДерева.Родитель <> Неопределено Тогда
		СтрокиРодителей(Дерево, СтрокаДерева.Родитель, СтрокиДляОтправки);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатПоиска(Форма, АдресРезультата, ИдентификаторЗадания)
	...
	ДеревоПолей = Форма.РеквизитФормыВЗначение(ИмяСписка);
	СтрокаДереваПоОтбору = Неопределено;
			
	ДеревоЭлементов = РезультатПоиска.ДеревоЭлементов;
	НайденныеЭлементы = РезультатПоиска.НайденныеЭлементы;
	Для Каждого НайденныйЭлемент Из НайденныеЭлементы Цикл
		СтрокаДляДобавления = ДеревоЭлементов.Строки.Найти(НайденныйЭлемент, "Идентификатор", Истина);
		
		ДобавляемыйЭлемент = ДобавитьЭлементПоПутиКДанным(ДеревоПолей, СтрокаДляДобавления);
		ДобавляемыйЭлемент.СоответствуетОтбору = Истина;
		СтрокаДереваПоОтбору = ?(СтрокаДереваПоОтбору = Неопределено, ДобавляемыйЭлемент, СтрокаДереваПоОтбору); 
		Если ДобавляемыйЭлемент.Родитель <> Неопределено Тогда
			ДобавляемыйЭлемент.Родитель.ПодчиненныйЭлементСоответствуетОтбору = Истина;
		КонецЕсли;
	КонецЦикла;
			
	Форма.ЗначениеВРеквизитФормы(ДеревоПолей, ИмяСписка);
	
	СтрокаКоллекции = СтрокаКоллекцииПоСтрокеДерева(Форма[ИмяСписка], СтрокаДереваПоОтбору, ДеревоПолей);
	Форма.Элементы[ИмяСписка].ТекущаяСтрока = СтрокаКоллекции.ПолучитьИдентификатор();
...
Процедура ОбработатьСообщенияПоиска(Форма, Сообщения, ИдентификаторЗадания)
	...
	ИдентификаторТекущейСтроки = Форма.Элементы[ИмяСписка].ТекущаяСтрока;
	Если ИдентификаторТекущейСтроки <> Неопределено Тогда
		СтрокаКоллекции = Форма[ИмяСписка].НайтиПоИдентификатору(ИдентификаторТекущейСтроки);
		ТекущаяСтрокаДерева = ДеревоПолей.Строки.Найти(СтрокаКоллекции.ПутьКДанным, "ПутьКДанным", Истина);
		ТекущаяСтрокаДерева = ?(СтрокаКоллекции.СоответствуетОтбору, ТекущаяСтрокаДерева, Неопределено);
	КонецЕсли;
	
	ПоступилиНовыеРезультатыПоиска = Ложь;
	...
	Для каждого Сообщение Из Сообщения Цикл
		...
		Если Результат.Свойство("Элементы") Тогда
			Для Каждого Элемент Из Результат.Элементы Цикл
				ЗаполнитьЗначенияСвойств(Форма.Элементы[Элемент.Ключ], Элемент.Значение);
			КонецЦикла;
		КонецЕсли;
		
		Если Результат.Свойство("НайденныеЭлементы") Тогда
			
			ДеревоЭлементов = Результат.ДеревоЭлементов;
			...
			Форма.ЗначениеВРеквизитФормы(ДеревоПолей, ИмяСписка);
			ПоступилиНовыеРезультатыПоиска = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Если ПоступилиНовыеРезультатыПоиска Тогда
		Если ТекущаяСтрокаДерева = Неопределено Тогда
			СтрокаКоллекции = СтрокаКоллекцииПоСтрокеДерева(Форма[ИмяСписка], СтрокаДереваПоОтбору, ДеревоПолей);
		Иначе
			СтрокаКоллекции = СтрокаКоллекцииПоСтрокеДерева(Форма[ИмяСписка], ТекущаяСтрокаДерева, ДеревоПолей);
		КонецЕсли;
		Форма.Элементы[ИмяСписка].ТекущаяСтрока = СтрокаКоллекции.ПолучитьИдентификатор();
	КонецЕсли;
...
//  ТекущаяСтрокаДерева - СтрокаДереваЗначений
//  ДеревоПолей - ДеревоЗначений
// 
Функция СтрокаКоллекцииПоСтрокеДерева(ДеревоФормы, ТекущаяСтрокаДерева, ДеревоПолей)
	РодительТекущейСтроки = ТекущаяСтрокаДерева.Родитель;
	Если РодительТекущейСтроки <> Неопределено Тогда
		Результат = СтрокаКоллекцииПоСтрокеДерева(ДеревоФормы, РодительТекущейСтроки, ДеревоПолей);
		ИндексСтрокиДерева = РодительТекущейСтроки.Строки.Индекс(ТекущаяСтрокаДерева);
		Возврат Результат.ПолучитьЭлементы().Получить(ИндексСтрокиДерева);
	Иначе
		ИндексСтрокиДерева = ДеревоПолей.Строки.Индекс(ТекущаяСтрокаДерева);
		Возврат ДеревоФормы.ПолучитьЭлементы().Получить(ИндексСтрокиДерева);
	КонецЕсли;
...
// Параметры:
//  ДеревоПолей - ДеревоЗначений
//  СтрокаДляДобавления - СтрокаДереваЗначений
// 
// Возвращаемое значение:
//  
Функция ДобавитьЭлементПоПутиКДанным(ДеревоПолей, СтрокаДляДобавления)
	НайденнаяСтрока = ДеревоПолей.Строки.Найти(СтрокаДляДобавления.ПутьКДанным, "ПутьКДанным", Истина);
	Если НайденнаяСтрока <> Неопределено Тогда
		Возврат НайденнаяСтрока;
	Иначе
		СтрокаРодителя = ДобавитьЭлементПоПутиКДанным(ДеревоПолей, СтрокаДляДобавления.Родитель);
		СтрокиТекущегоУровня = СтрокаРодителя.Строки;
		СтрокиТекущегоУровня.Очистить();
		КоллекцияСтрокУровня = СтрокаДляДобавления.Родитель.Строки;
		Для Каждого СтрокаУровня Из КоллекцияСтрокУровня Цикл
			НоваяСтрокаДереваПолей = СтрокиТекущегоУровня.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаДереваПолей, СтрокаУровня,,"Родитель");
			
			ЕстьПодчиненные = СтрокаУровня.Папка ИЛИ СтрокаУровня.Таблица;
	
			Если Не ЕстьПодчиненные Тогда
				Для Каждого Тип Из СтрокаУровня.Тип.Типы() Цикл
					ЕстьПодчиненные = ЕстьПодчиненные ИЛИ ОбщегоНазначения.ЭтоСсылка(Тип);
					Если ЕстьПодчиненные Тогда
						Прервать;
					КонецЕсли; 
				КонецЦикла;
			...
			Если ЕстьПодчиненные Тогда
				НоваяСтрокаДереваПолей.Строки.Добавить();
			КонецЕсли;

		КонецЦикла;
		
		Возврат СтрокиТекущегоУровня.Найти(СтрокаДляДобавления.ПутьКДанным, "ПутьКДанным");
		
	КонецЕсли;
	
КонецФункции

Функция ПолучитьМассивИменПоляКомпоновки(ПолеКомпоновки)
	Представление = Строка(ПолеКомпоновки);
	Результат = Новый Массив;
	МассивСлов = СтрРазделить(Представление, "[]");
	Если МассивСлов.Количество() = 1 Тогда
		Состав = СтрРазделить(Представление, ".", Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, Состав, Ложь);
	Иначе
		Для Каждого Слово Из МассивСлов Цикл
			Состав = СтрРазделить(Слово, ".", Истина);
			Если Состав[0] = "" Тогда
				Состав.Удалить(0);
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, Состав, Ложь);
			ИначеЕсли Состав[Состав.ВГраница()] = "" Тогда
				Состав.Удалить(Состав.ВГраница());
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, Состав, Ложь);
			Иначе
...
Процедура УстановитьНайденныйЭлементДерева(Форма, СписокПолей, ИмяСпискаПолей, МассивПутиКДанным, НайденныйЭлемент, Уровень = 0)
	
	МассивПутиКДаннымВр = Новый Массив(Новый ФиксированныйМассив(МассивПутиКДанным));
	
	Пока МассивПутиКДаннымВр.ВГраница() > Уровень Цикл
		МассивПутиКДаннымВр.Удалить(МассивПутиКДаннымВр.ВГраница());
	КонецЦикла;
	
	ИскомыйПутьКДанным = СтрСоединить(МассивПутиКДаннымВр, ".");
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ИмяСписка", ИмяСпискаПолей); 
	
	Для Каждого Элемент Из СписокПолей Цикл
		ПутьКДанным = Элемент.ПутьКДанным;
		КоличествоСлов = СтрРазделить(ПутьКДанным, ".", Ложь);
		Если КоличествоСлов.Количество() < Уровень + 1 Тогда
			ПутьКДанным = СтрСоединить(ПолучитьМассивИменПоляКомпоновки(Элемент.Поле), ".");
		КонецЕсли;
		
		Если ПутьКДанным = ИскомыйПутьКДанным Тогда
			Если МассивПутиКДанным.ВГраница() = Уровень Тогда
				Элемент.СоответствуетОтбору = Истина;
				Элемент.ПредставлениеПутиКДанным = НайденныйЭлемент.ПредставлениеПутиКДанным;
				Прервать;
			Иначе
				Элемент.ПодчиненныйЭлементСоответствуетОтбору = Истина;
				ПодчиненныеЭлементы = Элемент.ПолучитьЭлементы();
				Если ПодчиненныеЭлементы.Количество() < 2 Тогда
					ИдентификаторЭлемента = Элемент.ПолучитьИдентификатор();
					
					ПараметрыЗаполнения.Вставить("ИдентификаторСтроки", ИдентификаторЭлемента);
					ЗаполнитьСписокДоступныхПолей(Форма, ПараметрыЗаполнения);
				КонецЕсли;
				
				УстановитьНайденныйЭлементДерева(Форма, Элемент.ПолучитьЭлементы(), ИмяСпискаПолей, МассивПутиКДанным, НайденныйЭлемент, Уровень+1)
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция СписокОператоров(ГруппыОператоров = Неопределено) Экспорт
	...
	Если ГруппыОператоров = Неопределено Тогда
		...
		|ЧисловыеФункции, СтроковыеФункции, ФункцииДляТабличныхЧастей, ПрочиеФункции";
	...
	Для Каждого Элемент Из Новый Структура(ГруппыОператоров) Цикл
		...
		ИначеЕсли ИмяГруппы = "ФункцииДляТабличныхЧастей" Тогда
...
Процедура ДобавитьГруппуОператоровЛогическиеОперации(СписокОператоров)
	...
	ДобавитьОператорВГруппу(Группа, "ВЫБОР", НСтр("ru = 'ВЫБОР'"), , , Истина);
...
Процедура ДобавитьГруппуОператоровФункцииДляТабличныхЧастей(СписокОператоров)
	...
	ДобавитьОператорВГруппу(Группа, "УправлениеПечатью_СуммаПоКолонке", НСтр("ru = 'Сумма по колонке'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, "УправлениеПечатью_КоличествоСтрок", НСтр("ru = 'Количество строк'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, "УправлениеПечатью_МаксимумПоКолонке", НСтр("ru = 'Максимум по колонке'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, "УправлениеПечатью_МинимумПоКолонке", НСтр("ru = 'Минимум по колонке'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, "УправлениеПечатью_СреднееПоКолонке", НСтр("ru = 'Среднее по колонке'"), Тип, Истина);
КонецПроцедуры
