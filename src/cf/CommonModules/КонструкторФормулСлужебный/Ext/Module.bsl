...
Функция ОписаниеПоля(ПутьКДанным, СпискиПолей) Экспорт
	...
	ДоступныеСхемыПоля = Новый Массив;
	...
	Для Каждого НастройкиСписка Из СпискиПолей Цикл
		...
		Если КоллекцияПолей = Неопределено Тогда
			...
			КоллекцияПолей = НастройкиСписка.КоллекцияПолей;
			ПараметрыЗаполненияСпискаДоступныхРеквизитов = ПараметрыЗаполненияСпискаДоступныхРеквизитов(КоллекцияПолей);
			ПараметрыЗаполненияСпискаДоступныхРеквизитов.НастройкиСписка = НастройкиСписка;
			ЗаполнитьСписокДоступныхРеквизитов(ПараметрыЗаполненияСпискаДоступныхРеквизитов, ИсточникиДоступныхПолей);
		КонецЕсли;
			...
			Если Родитель <> Неопределено Тогда
				Для Каждого ИсточникДоступныхПолей Из ИсточникиДоступныхПолей(ИсточникиДоступныхПолей, НастройкиСписка, Родитель) Цикл
					...
					Если Поле <> Неопределено Тогда
						...
						ДобавитьДоступнуюСхемуПоля(ДоступныеСхемыПоля, ИсточникДоступныхПолей);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Результат.Вставить("ДоступныеСхемы", ДоступныеСхемыПоля);
...
Функция ЭлементыФормулы(Знач Формула) Экспорт
	...
	ОткрывающихСкобок = 0;
	ЭтоСтрокаВДвойныхКавычках = Ложь;
	ЭтоСтрокаВОдинарныхКавычках = Ложь;
	...
	Для Индекс = 1 По СтрДлина(Формула) Цикл
		...
		Если ОткрывающихСкобок = 0 И Символ = """" И Не ЭтоСтрокаВОдинарныхКавычках Тогда
			ЭтоСтрокаВДвойныхКавычках = Не ЭтоСтрокаВДвойныхКавычках;
			ВсеЭлементы.Добавить(Символ);
			Продолжить;
		КонецЕсли;

		Если ОткрывающихСкобок = 0 И Символ = "'" И Не ЭтоСтрокаВДвойныхКавычках Тогда
			ЭтоСтрокаВОдинарныхКавычках = Не ЭтоСтрокаВОдинарныхКавычках;
			ВсеЭлементы.Добавить(Символ);
			Продолжить;
		КонецЕсли;
		
		Если ЭтоСтрокаВДвойныхКавычках Или ЭтоСтрокаВОдинарныхКавычках Тогда
			ВсеЭлементы[ВсеЭлементы.ВГраница()] = ВсеЭлементы[ВсеЭлементы.ВГраница()] + Символ;
			Продолжить;
		КонецЕсли;
...
Функция ЗапуститьПоискВСпискеПолей(СтруктураФормы) Экспорт
	...
	Если ОтборУстановлен Тогда
		ТаблицаКеша = Новый ТаблицаЗначений;
		ТаблицаКеша.Колонки.Добавить("ИмяСпискаПолей", Новый ОписаниеТипов("Строка"));
		ТаблицаКеша.Колонки.Добавить("ТипПоля", Новый ОписаниеТипов("ОписаниеТипов"));
		ТаблицаКеша.Колонки.Добавить("ДоступныеПоля", Новый ОписаниеТипов("ТаблицаЗначений"));
		СтруктураФормы.Вставить("КэшДляСКД", ТаблицаКеша);
		РезультатПоиска = УстановитьОтбор(СтруктураФормы, ИмяСпискаПолей, Отбор, СтруктураФормы[ИмяСпискаПолей]);
	КонецЕсли;
	
	Если РезультатПоиска.НайденныеЭлементы.Количество() > 0 Тогда
...
Функция ЗапуститьФоновыйПоискВСпискеПолей(Форма)
	...
	РезультатОтмены = ОтменитьЗаданиеФоновогоПоиска(Форма);
	ПараметрыСпискаПоиска = ПараметрыСпискаПоиска(Форма);
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, 
		"КонструкторФормулСлужебный.ЗапуститьПоискВСпискеПолей", ПараметрыСпискаПоиска);

	РезультатОтмены.ФоновыеЗаданияПоиска.Вставить(РезультатОтмены.ИмяСтрокиПоиска, 
		ДлительнаяОперация.ИдентификаторЗадания);
	Если ЭтоАдресВременногоХранилища(Форма.АдресОписанияДлительнойОперации) Тогда
		ПоместитьВоВременноеХранилище(РезультатОтмены.ФоновыеЗаданияПоиска, Форма.АдресОписанияДлительнойОперации);
	Иначе
		Форма.АдресОписанияДлительнойОперации = ПоместитьВоВременноеХранилище(РезультатОтмены.ФоновыеЗаданияПоиска, 
...
Функция ОтменитьЗаданиеФоновогоПоиска(Форма)
	
	Если ЭтоАдресВременногоХранилища(Форма.АдресОписанияДлительнойОперации) Тогда
		ФоновыеЗаданияПоиска = ПолучитьИзВременногоХранилища(Форма.АдресОписанияДлительнойОперации);
	Иначе
		ФоновыеЗаданияПоиска = Новый Соответствие();
	КонецЕсли;
	
	ИмяСтрокиПоиска = Форма.ИмяТекущейСтрокиПоиска;
	
	Если ФоновыеЗаданияПоиска[ИмяСтрокиПоиска] <> Неопределено Тогда
		ИдентификаторЗадания = ФоновыеЗаданияПоиска[ИмяСтрокиПоиска];
		ДлительныеОперации.ОтменитьВыполнениеЗадания(ИдентификаторЗадания);
		ДождатьсяЗавершенияЗадания = ОбщегоНазначения.ИнформационнаяБазаФайловая();
		Пока ДождатьсяЗавершенияЗадания Цикл
	...
	Возврат Новый Структура("ФоновыеЗаданияПоиска, ИмяСтрокиПоиска", ФоновыеЗаданияПоиска, ИмяСтрокиПоиска);
КонецФункции

Функция ПараметрыСпискаПоиска(Форма)
	...
	ПараметрыСпискаПоиска = Новый Структура;
	ПараметрыСпискаПоиска.Вставить("ИмяСписка", ИмяСписка);
	ПараметрыСпискаПоиска.Вставить("ПодключенныеСпискиПолей", ПодключенныеСпискиПолейСтрокиПоиска);
	...
	Для Каждого ПодключенногоСпискаПолей Из ПодключенныеСпискиПолейСтрокиПоиска Цикл
		...
		ПараметрыСпискаПоиска.Вставить(ИмяРеквизитаСтрокиПоиска, Форма[ИмяРеквизитаСтрокиПоиска]);
		...
		ПараметрыСпискаПоиска.Вставить(ПодключенногоСпискаПолей.ИмяСпискаПолей, ДеревоПолей);
		
		ТаблицаИсточников = Форма.РеквизитФормыВЗначение(ПодключенногоСпискаПолей.ИмяСпискаИсточников);
		...
		ПараметрыСпискаПоиска.Вставить(ПодключенногоСпискаПолей.ИмяСпискаИсточников, СтрокаТаблицыИсточников);
	КонецЦикла;
	Возврат ПараметрыСпискаПоиска;
КонецФункции
...
Функция ДеревоДоступныхРеквизитов(Знач Форма, Знач ИмяСписка)
	ДеревоТекущегоСписка = Форма.РеквизитФормыВЗначение(ИмяСписка);// ДеревоЗначений
...
Процедура ДобавитьСписокПолейНаФорму(Форма, Параметры) Экспорт
	...
	Если АдресОписанияДлительнойОперации = Неопределено Тогда
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("КэшДляСКД", Новый ОписаниеТипов("ТаблицаЗначений")));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяСпискаПолей", Новый ОписаниеТипов("Строка"), "КэшДляСКД"));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ТипПоля", Новый ОписаниеТипов("ОписаниеТипов"), "КэшДляСКД"));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ДоступныеПоля", Новый ОписаниеТипов("ТаблицаЗначений"), "КэшДляСКД"));
		Для Каждого ОписаниеРеквизита Из РеквизитыПодключаемогоСписка() Цикл
			ИмяРеквизита = ОписаниеРеквизита.Ключ;
			ТипРеквизита = ОписаниеРеквизита.Значение;
			ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизита, ТипРеквизита, "КэшДляСКД.ДоступныеПоля"));
		КонецЦикла;
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ЕстьПодчиненные", Новый ОписаниеТипов("Булево"), "КэшДляСКД.ДоступныеПоля"));
	КонецЕсли;
	...
	ПараметрыЗаполненияСпискаДоступныхРеквизитов = ПараметрыЗаполненияСпискаДоступныхРеквизитов(ДеревоПолей);
	ПараметрыЗаполненияСпискаДоступныхРеквизитов.УникальныйИдентификаторФормы = Форма.УникальныйИдентификатор;
	ПараметрыЗаполненияСпискаДоступныхРеквизитов.НастройкиСписка = ПодключенныйСписок;
	ЗаполнитьСписокДоступныхРеквизитов(ПараметрыЗаполненияСпискаДоступныхРеквизитов, ТаблицаИсточников);
...
Процедура ОчиститьСтрокуПоиска(Форма, ИмяЭлемента)
	...
	СтрокаПоиска = Форма.Элементы[СтрЗаменить(ИмяЭлемента, "Очистка", "")];
	Форма.ТекущийЭлемент = СтрокаПоиска;
	Форма[СтрокаПоиска.Имя] = "";
	ОтменитьЗаданиеФоновогоПоиска(Форма);
	
КонецПроцедуры

Процедура ОбновитьКоллекцииПолей(Форма, КоллекцииПолей, ИмяСпискаПолей = "ДоступныеПоля") Экспорт
	ОбновитьИсточникиДоступныхПолей(Форма, КоллекцииПолей, ИмяСпискаПолей);
	...
	ПараметрыЗаполненияСпискаДоступныхРеквизитов = ПараметрыЗаполненияСпискаДоступныхРеквизитов(Форма[ИмяСпискаПолей]);
	ПараметрыЗаполненияСпискаДоступныхРеквизитов.НастройкиСписка = НастройкиСписка;
...
Процедура ОбновитьИсточникиДоступныхПолей(Форма, КоллекцииПолей, ИмяСпискаПолей)
	ИсточникиДоступныхПолей = СписокИсточниковДоступныхПолей(Форма, ИмяСпискаПолей); // ДанныеФормыКоллекция
...
Функция ПараметрыЗаполненияСпискаДоступныхРеквизитов(ТекущийРеквизит)
	Параметры = Новый Структура;
	Параметры.Вставить("ИмяСпискаПолей", "");
	Параметры.Вставить("УникальныйИдентификаторФормы", Неопределено);
	Параметры.Вставить("НастройкиСписка", Неопределено);
	Параметры.Вставить("ТекущийРеквизит", ТекущийРеквизит);
	Возврат Параметры;
КонецФункции

Процедура ЗаполнитьСписокДоступныхРеквизитов(Параметры, ИсточникиДоступныхПолей, КэшСКД = Неопределено)
...
Функция ДоступныеРеквизиты(КоллекцииДоступныхПолей)
	...
	ДоступныеРеквизиты.Колонки.Добавить("КоличествоТочекВПоле", Новый ОписаниеТипов("Число"));
	...
	Для Каждого КоллекцияДоступныхПолей Из КоллекцииДоступныхПолей Цикл
		Для Каждого ОписаниеПоля Из КоллекцияДоступныхПолей.Элементы Цикл
			...
			Реквизит.КоличествоТочекВПоле = СтрЧислоВхождений(Реквизит.Поле, ".");
	...
	ПроверитьУникальностьПредставленийРеквизитов(ДоступныеРеквизиты);
	
	ДоступныеРеквизиты.Сортировать("Порядок, Заголовок, КоличествоТочекВПоле");
	Возврат ДоступныеРеквизиты;
	
КонецФункции

Процедура ПроверитьУникальностьПредставленийРеквизитов(ДоступныеРеквизиты)
	
	ЗаголовкиРеквизитов = Новый Соответствие;
	
	Для Каждого ДоступныйРеквизит Из ДоступныеРеквизиты Цикл
		Если ЗаголовкиРеквизитов[ДоступныйРеквизит.Заголовок] = Неопределено Тогда
			ЗаголовкиРеквизитов[ДоступныйРеквизит.Заголовок] = 1;
		Иначе
			ЗаголовкиРеквизитов[ДоступныйРеквизит.Заголовок] = ЗаголовкиРеквизитов[ДоступныйРеквизит.Заголовок] + 1;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ЗаголовокРеквизита Из ЗаголовкиРеквизитов Цикл
		Если ЗаголовокРеквизита.Значение > 1 Тогда
			ДублирующиесяРеквизиты = ДоступныеРеквизиты.НайтиСтроки(Новый Структура("Заголовок", ЗаголовокРеквизита.Ключ));
			Счетчик = 1;
			Для Каждого Реквизит Из ДублирующиесяРеквизиты Цикл
				Реквизит.Заголовок = Реквизит.Заголовок + " #" + Счетчик;
				Если ЗначениеЗаполнено(Реквизит.ПредставлениеПутиКДанным) Тогда
					Реквизит.ПредставлениеПутиКДанным = Реквизит.Заголовок;
				КонецЕсли;
				Счетчик = Счетчик + 1;
			КонецЦикла;
...
Функция ИсточникиДоступныхПолей(ИсточникиДоступныхПолей, НастройкиСписка, Реквизит = Неопределено, УникальныйИдентификаторФормы = Неопределено)
	...
	Если Реквизит = Неопределено Тогда
		Отбор = Новый Структура("Корневой", Истина);
	...
	Если Родитель = Неопределено Тогда
		Отбор = Новый Структура("Корневой", Истина);
		Для Каждого ИсточникДоступныхПолей Из ИсточникиДоступныхПолей.НайтиСтроки(Отбор) Цикл
			Если ЗначениеЗаполнено(ИсточникДоступныхПолей.ИсточникДанных) Тогда
				ИсточникДанных = ИсточникДоступныхПолей.ИсточникДанных + "." + Реквизит.Имя;
...
Процедура ЗаполнитьСписокДоступныхПолей(Форма, ПараметрыЗаполнения) Экспорт
	...
	КоллекцииПолей = ПараметрыЗаполнения.КоллекцииПолей;
	Если ЗначениеЗаполнено(КоллекцииПолей) Тогда
		ОбновитьИсточникиДоступныхПолей(Форма, КоллекцииПолей, ИмяСписка);
	КонецЕсли;
...
Процедура ВыполнитьПоискВСпискеПолей(Форма) Экспорт
	...
	Если ЗначениеЗаполнено(Отбор) Тогда
		КореньРезультатовПоиска = КонструкторФормулКлиентСервер.СтрокаРезультатовПоиска(Форма[ИмяСпискаПолей], Ложь);
		Если КореньРезультатовПоиска <> Неопределено Тогда
			Форма[ИмяСпискаПолей].ПолучитьЭлементы().Удалить(КореньРезультатовПоиска);
		КонецЕсли;
		
		ПараметрыСпискаПоиска = ПараметрыСпискаПоиска(Форма);
		РезультатПоиска = ЗапуститьПоискВСпискеПолей(ПараметрыСпискаПоиска);
		Если РезультатПоиска <> Неопределено Тогда
			КореньРезультатовПоиска = КонструкторФормулКлиентСервер.СтрокаРезультатовПоиска(Форма[ИмяСпискаПолей]);
			КореньРезультатовПоиска.Заголовок = Отбор;
			Для Каждого НайденныйЭлемент Из РезультатПоиска.НайденныеЭлементы Цикл
				СтрокаРезультатовПоиска = КореньРезультатовПоиска.ПолучитьЭлементы().Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРезультатовПоиска, НайденныйЭлемент);
			КонецЦикла;
			КонструкторФормулКлиентСервер.СортироватьПоКолонке(КореньРезультатовПоиска, "Вес");
		КонецЕсли;
	Иначе
		КореньРезультатовПоиска = КонструкторФормулКлиентСервер.СтрокаРезультатовПоиска(Форма[ИмяСпискаПолей], Ложь);
		Если КореньРезультатовПоиска <> Неопределено Тогда
	...
	Форма.Элементы[ИмяСпискаПолей].Отображение = ?(ОтборУстановлен, ОтображениеТаблицы.Список, 
...
Функция УстановитьОтбор(Знач Форма, Знач ИмяСписка, Знач Отбор,
	...
	КоличествоВПорции = 5;
	МаксимальноеКоличествоРезультатов = 25;
	МаксимальныйУровеньПоиска = 3;
	
	ДостигнутоМаксимальноеКоличествоРезультатов = Ложь;
	ОтборСУчетомУровней = СтрРазделить(Отбор, ".", Ложь);
	ПоискСУчетомУровней = ОтборСУчетомУровней.Количество() > 1;
	Если ПоискСУчетомУровней Тогда
		МаксимальныйУровеньПоиска = ОтборСУчетомУровней.Количество();
	КонецЕсли;
	Отбор = ?(СтрЗаканчиваетсяНа(Отбор, "."), Сред(Отбор, 1, СтрДлина(Отбор)-1), Отбор);
	
	СтруктураОтбора = Новый Структура("СоответствуетОтбору", Истина);
	Уровень = 1;
	КоличествоНайденных = 0;
	ПараметрыПолученияКоллекции = Новый Структура;
	ПараметрыПолученияКоллекции.Вставить("ИмяСписка", ИмяСписка);
	ПараметрыПолученияКоллекции.Вставить("Форма", Форма);
	ПараметрыПолученияКоллекции.Вставить("ПоискСУчетомУровней", ПоискСУчетомУровней);
	...
	Пока Истина Цикл	
		...
		Пока Истина Цикл
			ПорцияПолейТекущегоУровня = ПолучитьПорциюПолейЗаданногоУровня(ПараметрыПолученияКоллекции, КоллекцияРеквизитов, Уровень, ИспользованныеУзлы);
			Если ПорцияПолейТекущегоУровня = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Если ПоискСУчетомУровней Тогда
				УстановитьПризнакОтбора(КоличествоНайденных, МаксимальноеКоличествоРезультатов, ПорцияПолейТекущегоУровня, ОтборСУчетомУровней[Уровень-1], 
					ПоискСУчетомУровней, ОтборСУчетомУровней.Количество() = Уровень);
			Иначе
				...
				Если МассивНайденныхЭлементов.Количество() <> 0 Тогда
					Для Каждого НайденныйЭлемент Из МассивНайденныхЭлементов Цикл
						...
						Если СтрокиПорции.Количество() = КоличествоВПорции Тогда
							ОтправитьПорциюНайденных(СтрокиПорции);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		...
		Если Уровень >= 2 И ЭтоФоновоеЗадание И СтрокиПорции.Количество() > 0 Тогда
			ОтправитьПорциюНайденных(СтрокиПорции);
		КонецЕсли;
	...
	Если Не ЭтоФоновоеЗадание Тогда
		МассивНайденныхЭлементов = КоллекцияРеквизитов.Строки.НайтиСтроки(СтруктураОтбора, Истина);
		Если МассивНайденныхЭлементов.Количество() <> 0 Тогда
			Для Каждого НайденныйЭлемент Из МассивНайденныхЭлементов Цикл
				СтруктураСтроки = СтрокуДереваВСтруктуру(НайденныйЭлемент, КоллекцияРеквизитов);
				ВсеНайденныеСтроки.Добавить(СтруктураСтроки);
	...
	Результат = Новый Структура("НайденныеЭлементы", ВсеНайденныеСтроки);
	Возврат Результат;
	
КонецФункции

Процедура ОтправитьПорциюНайденных(НайденныеСтроки)
...
Процедура УстановитьПризнакОтбора(КоличествоНайденных, Знач МаксимальноеКоличествоРезультатов, Знач КоллекцияРеквизитов,
	Знач Отбор, ПоискСУчетомУровней = Ложь, ЦелевойУровень = Ложь)
	
	Для Каждого Реквизит Из ПодчиненныеЭлементы(КоллекцияРеквизитов) Цикл
		Если КоличествоНайденных = МаксимальноеКоличествоРезультатов Тогда
			Возврат;
		КонецЕсли;
		Если (ПоискСУчетомУровней И Не РодительРелевантен(Реквизит))
			ИЛИ (Не ПоискСУчетомУровней И РодительскийРеквизитСоответствуетОтбору(Реквизит)) Тогда
			Реквизит.СоответствуетОтбору = Ложь;
		Иначе
			РезультатПоиска = НайтиТекстВСтроке(Реквизит, Отбор, ПоискСУчетомУровней);
			Если Не ПоискСУчетомУровней Или ЦелевойУровень Тогда
				Реквизит.СоответствуетОтбору = РезультатПоиска.СоответствуетОтбору;
			КонецЕсли;
			
			Если РезультатПоиска.СоответствуетОтбору Тогда
				КоличествоНайденных = КоличествоНайденных + 1;
				Реквизит.ПредставлениеПутиКДанным = РезультатПоиска.ФорматированнаяСтрока;
				Реквизит.Вес = РезультатПоиска.Вес;
				...
				Если РодительРеквизита <> Неопределено Тогда
					...
					Если ПоискСУчетомУровней Тогда
						РодительРеквизита.Вес = РодительРеквизита.Вес + Реквизит.Вес;
						Если РодительРеквизита.СоответствуетОтбору Тогда
							КоличествоНайденных = КоличествоНайденных - 1;
						КонецЕсли;
						РодительРеквизита.СоответствуетОтбору = Ложь;
					Иначе
						РодительРеквизита.Вес = Макс(РодительРеквизита.Вес, Реквизит.Вес);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьПорциюПолейЗаданногоУровня(ПараметрыПолученияКоллекции, КоллекцияРеквизитов, Уровень, ИспользованныеУзлы)
	...
	Возврат ПолучитьКоллекциюУровня(ПараметрыПолученияКоллекции, КоллекцияРеквизитов, Уровень, ИспользованныеУзлы);
КонецФункции    
...
Функция ПолучитьКоллекциюУровня(ПараметрыПолученияКоллекции, КоллекцияРеквизитов, Уровень, ИспользованныеУзлы, ТекущийУровень = 1, ИндексРодителя = "")
	...
	Для Каждого Реквизит Из ЭлементыКоллекции Цикл
		...
		Если ИспользованныеУзлы.Получить(ИдентификаторРеквизита) = Неопределено Тогда
			...
			Если Реквизит.ПутьКДанным = "<СтрокаРезультатовПоиска>" Или СтрНайти(Реквизит.ПутьКДанным, ".Удалить") > 0 Тогда
				ИспользованныеУзлы.Вставить(ИдентификаторРеквизита, Истина);
				Продолжить;
			КонецЕсли;
			...
			Если Уровень = ТекущийУровень + 1 Тогда
				Если (Не Реквизит.СоответствуетОтбору И Не ПараметрыПолученияКоллекции.ПоискСУчетомУровней) 
					Или (Реквизит.Вес > 0 И ПараметрыПолученияКоллекции.ПоискСУчетомУровней) Тогда
					РазвернутьРеквизит(ИдентификаторРеквизита, ПараметрыПолученияКоллекции.ИмяСписка, ПараметрыПолученияКоллекции.Форма);
				КонецЕсли;
			КонецЕсли;
			
			Коллекция = ПолучитьКоллекциюУровня(ПараметрыПолученияКоллекции, Реквизит, Уровень, ИспользованныеУзлы, ТекущийУровень + 1, ИдентификаторРеквизита);
...
Функция РодительРелевантен(Реквизит)
	
	Родитель = Родитель(Реквизит);
	
	Если Родитель <> Неопределено Тогда
		Возврат Родитель.Вес <> 0;
	КонецЕсли;
...
Процедура РазвернутьРеквизит(ИдентификаторСтроки, ИмяСписка, Форма)
	...
	КэшДляСКД = ?(ТипЗнч(Форма) = Тип("Структура"), Форма.КэшДляСКД, Форма.РеквизитФормыВЗначение("КэшДляСКД"));
	
	ПараметрыЗаполненияСпискаДоступныхРеквизитов = ПараметрыЗаполненияСпискаДоступныхРеквизитов(ТекущиеДанные);
	ПараметрыЗаполненияСпискаДоступныхРеквизитов.ИмяСпискаПолей = ИмяСписка;
	ПараметрыЗаполненияСпискаДоступныхРеквизитов.НастройкиСписка = НастройкиСписка;
	ЗаполнитьСписокДоступныхРеквизитов(ПараметрыЗаполненияСпискаДоступныхРеквизитов, ИсточникиДоступныхПолей, КэшДляСКД);
	
	Если ТипЗнч(Форма) = Тип("Структура") Тогда
		Форма.КэшДляСКД = КэшДляСКД;
	Иначе
		Форма.ЗначениеВРеквизитФормы(КэшДляСКД, "КэшДляСКД");
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление(Форма, ИмяСпискаПолей)
	...
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяСпискаПолей + ".ПредставлениеПутиКДанным");
...
Функция НайтиРеквизит(Форма, ИмяСписка, ПутьКДанным, КоллекцияРеквизитов, ИскатьПоПредставлению)
	...
	Для Каждого Реквизит Из КоллекцияРеквизитов Цикл
		Если НРег(Реквизит[ИмяПоляПоиска]) = НРег(ПутьКДанным) Тогда
			Возврат Реквизит;
		КонецЕсли;
		Если Реквизит.Папка Или СтрНачинаетсяС(НРег(ПутьКДанным), НРег(Реквизит[ИмяПоляПоиска]) + ".") Тогда
			Владельцы.Добавить(Реквизит);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Владелец Из Владельцы Цикл
		РазвернутьРеквизит(Владелец.ПолучитьИдентификатор(), ИмяСписка, Форма);
		НайденныйРеквизит = НайтиРеквизит(Форма, ИмяСписка, ПутьКДанным, Владелец.ПолучитьЭлементы(), ИскатьПоПредставлению);
...
Функция РеквизитыПодключаемогоСписка()
	...
	Результат.Вставить("Вес", Новый ОписаниеТипов("Число"));
...
Функция РеквизитыСпискаИсточниковДоступныхПолей()
	...
	Результат.Вставить("Корневой", Новый ОписаниеТипов("Булево"));
...
Процедура ДобавитьГруппуОператоровСтроковыеФункции(СписокОператоров)
	...
	ДобавитьОператорВГруппу(Группа, "Лев", НСтр("ru = 'Строка слева'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, "Прав", НСтр("ru = 'Строка справа'"), Тип, Истина);
	...
	ДобавитьОператорВГруппу(Группа, "Сред", НСтр("ru = 'Часть строки'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, "СимволПС", НСтр("ru = 'С новой строки'"));
...
Процедура ДобавитьГруппуОператоровПрочиеФункции(СписокОператоров)
	...
	ДобавитьОператорВГруппу(Группа, "?", НСтр("ru = 'Условие'"), ТипБулево, Истина);
	ДобавитьОператорВГруппу(Группа, "ЗначениеЗаполнено", НСтр("ru = 'Значение заполнено'"), ТипБулево, Истина);
	ДобавитьОператорВГруппу(Группа, "Формат", НСтр("ru = 'Формат'"), ТипСтрока, Истина);
...
Процедура ДобавитьГруппуОператоровЛогическиеОперации(СписокОператоров)
	...
	Оператор.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 %2 ... %3 ...'"),
		ПредставлениеОператора(НСтр("ru = 'ВЫБОР'")),
		ПредставлениеОператора(НСтр("ru = 'КОГДА'")),
		ПредставлениеОператора(НСтр("ru = 'ТОГДА'")));
	Оператор.Картинка = БиблиотекаКартинок.Пустая;
	Оператор.ВыражениеДляВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1
		|	%2 <%6> %3 <%6>
		|	%4 <%6>
		|%5'"),
...
Процедура УбратьПризнакОтбора(Коллекция)
	Для Каждого СтрокаКоллекции Из ПодчиненныеЭлементы(Коллекция) Цикл
...
Функция НайтиТекстВСтроке(Строка, Текст, ПоискСУчетомУровней)
	Возврат КонструкторФормулКлиентСервер.НайтиТекстВСтроке(Строка, Текст, 
		ШрифтыСтиля.ВажнаяНадписьШрифт, ЦветаСтиля.РезультатУспехЦвет, ПоискСУчетомУровней);
КонецФункции
	
Процедура ДобавитьДоступнуюСхемуПоля(ДоступныеСхемыПоля, ИсточникДоступныхПолей)
	
	Для Каждого ОписаниеСхемы Из ДоступныеСхемыПоля Цикл
		Если ОписаниеСхемы.ИсточникДанных = ИсточникДоступныхПолей.ИсточникДанных Тогда
			ОписаниеСхемы.СхемаКомпоновкиДанных = ИсточникДоступныхПолей.СхемаКомпоновкиДанных;
			ОписаниеСхемы.ИдентификаторСхемыКомпоновкиДанных = ИсточникДоступныхПолей.ИдентификаторСхемыКомпоновкиДанных;
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	ОписаниеСхемы = Новый Структура;
	ОписаниеСхемы.Вставить("ИсточникДанных", ИсточникДоступныхПолей.ИсточникДанных);
	ОписаниеСхемы.Вставить("СхемаКомпоновкиДанных", ИсточникДоступныхПолей.СхемаКомпоновкиДанных);
	ОписаниеСхемы.Вставить("ИдентификаторСхемыКомпоновкиДанных",
		ИсточникДоступныхПолей.ИдентификаторСхемыКомпоновкиДанных);
	ДоступныеСхемыПоля.Добавить(ОписаниеСхемы);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗапуститьПоискВСпискеПолей", Истина);
