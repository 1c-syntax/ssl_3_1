...
Процедура АутентификацияПоЛогинуПослеВыполнения(РезультатВызова, ВходящийКонтекст) Экспорт
	...
	Если СервисКриптографииDSSКлиентСервер.ЭтоОшибкаАутентификации(РезультатВыполнения.СтатусОшибки)
		...
		ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
		ОтобразитьОшибкуПользователю(ОповещениеСледующее, РезультатВыполнения.СтатусОшибки, , ВходящийКонтекст.ПараметрыОперации);
	Иначе
	...
	Если РезультатВызова.Выполнено Тогда
		...
		Если Принудительно Тогда
			...
			ИначеЕсли ВидАутентификации = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Первичный_УчетныеДанные") Тогда
				ВызватьДиалог = ПустойПароль;
			ИначеЕсли ВидАутентификации = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Первичный_СервисныйКлиент") Тогда
				ВызватьДиалог = Ложь;
			КонецЕсли;	
		Иначе	
...
Процедура ПодтверждениеПроизвольнойОперацииСертификат(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаАутентификации(РезультатВыполнения.СтатусОшибки) Тогда
		ОбъектОповещения	= Новый ОписаниеОповещения("ПодтверждениеПроизвольнойОперацииНачало", ЭтотОбъект);
		ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
...
Процедура ПодтверждениеПроизвольнойОперацииПослеВыполнения(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли НеОбрабатыватьОшибкиСервиса(ВходящийКонтекст) Тогда
		ПодтверждениеПроизвольнойОперацииРезультат(РезультатВыполнения, ВходящийКонтекст);
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаПинКода(РезультатВыполнения.СтатусОшибки) Тогда // неверный пин-код
...
Процедура РасшифроватьСоздатьТранзакцию(ДлительнаяОперация, ВходящийКонтекст)
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Иначе
			ПодтверждатьОперацию = КлючДействия.Подтверждать ИЛИ ВходящийКонтекст.ПодтверждатьПринудительно ИЛИ ОписаниеСертификата.МобильныйЗакрытыйКлюч;
		...
		Иначе
			...
			Для Каждого СтрокаМассива Из СписокУчетныхЗаписей Цикл
				Если МассивОшибок.Количество() = 0 Тогда
					МассивОшибок.Добавить(СтрокаМассива.Результат.СтатусОшибки.Представление);
				ИначеЕсли СтрокаМассива.Результат.СтатусОшибки.КодОшибки = "100002" Тогда
					МассивОшибок.Вставить(0, СтрокаМассива.Результат.СтатусОшибки.Представление);
				ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаОтказа(СтрокаМассива.Результат.СтатусОшибки) Тогда
					МассивОшибок.Вставить(1, СтрокаМассива.Результат.СтатусОшибки.Представление);
				Иначе
					МассивОшибок.Добавить(СтрокаМассива.Результат.СтатусОшибки.Представление);
				КонецЕсли;	
			КонецЦикла;
			...
			Если МассивОшибок.Количество() = 0 И РезультатВызова <> Неопределено Тогда
				ПолучитьОписаниеОшибки(РезультатСОшибкой, "РасшифровкаВЦикле", РезультатВызова.СтатусОшибки.Представление);
			Иначе	
				РезультатСОшибкой.СтатусОшибки.Представление = СтрСоединить(МассивОшибок, Символы.ПС);
				РезультатСОшибкой.Ошибка = РезультатСОшибкой.СтатусОшибки.Представление;
			КонецЕсли;
...
Процедура ПодписатьСоздатьТранзакцию(РезультатВыполнения, ВходящийКонтекст)
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		ОписаниеСертификата = ВходящийКонтекст.ОписаниеСертификата;
		СвойствоПодписи = ВходящийКонтекст.СвойствоПодписи;
		
		Если ВерсияПодтверждения = 2 И НЕ ЗначениеЗаполнено(ВходящийКонтекст.ИдентификаторТранзакции) Тогда
			ВходящийКонтекст.Вставить("ПодтверждатьОперацию", ПодтверждатьОперацию);
			ОбъектОповещения 	= Новый ОписаниеОповещения("ПодписатьАсинхронноПослеВыгрузкиДокументов", ЭтотОбъект);
			СледующееОповещение = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
			
			Если ОписаниеСертификата.МобильныйЗакрытыйКлюч И ВходящийКонтекст.ПакетнаяПодпись Тогда
				ПараметрыДокументов = Новый Массив;
				Для Каждого СтрокаМассива Из ВходящийКонтекст.ДанныеДокумента Цикл
					ИмяДокумента = СвойствоСтруктуры(СтрокаМассива, "Name", "Подпись");
					ТипДокумента = СвойствоСтруктуры(СтрокаМассива, "Type", "bin");
					НоваяСтрока = СервисКриптографииDSSКлиентСервер.ПолучитьСвойстваДокумента(ИмяДокумента, ТипДокумента, Истина, "");
					НоваяСтрока.Вставить("Content", Base64Строка(СтрокаМассива.Содержание));
					ПараметрыДокументов.Добавить(НоваяСтрока);
				КонецЦикла;
				
				ОжидатьЗавершенияВыполненияВФоне(
					СледующееОповещение, 
					СервисКриптографииDSSСлужебныйВызовСервера.ЗагрузитьПакетДокументов(
							НастройкиПользователя,
							ПараметрыДокументов,
							ВходящийКонтекст.ПараметрыОперации),
					ВходящийКонтекст.ПараметрыОперации);
			ИначеЕсли ОписаниеСертификата.МобильныйЗакрытыйКлюч Тогда
				ИмяДокумента = СвойствоСтруктуры(СвойствоПодписи, "DocumentInfo", "Подпись");
				ТипДокумента = СвойствоСтруктуры(СвойствоПодписи, "DocumentType", "bin");
				ПараметрыДокументов = СервисКриптографииDSSКлиентСервер.ПолучитьСвойстваДокумента(ИмяДокумента, ТипДокумента, Истина, "");
				
				ОжидатьЗавершенияВыполненияВФоне(
					СледующееОповещение, 
					СервисКриптографииDSSСлужебныйВызовСервера.ЗагрузитьДокумент(
							НастройкиПользователя,
							ВходящийКонтекст.ДанныеДокумента,
							ПараметрыДокументов, 
							ВходящийКонтекст.ПараметрыОперации),
					ВходящийКонтекст.ПараметрыОперации);
			Иначе
				ПодписатьАсинхронноПослеВыгрузкиДокументов(Неопределено, ВходящийКонтекст);
			КонецЕсли;
...
// Продолжение процедуры Подписать.
Процедура ПодписатьАсинхронноПослеВыгрузкиДокументов(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли РезультатВыполнения <> Неопределено Тогда
		Если РезультатВыполнения.Выполнено Тогда
			Если ВходящийКонтекст.ПакетнаяПодпись Тогда
				Для Счетчик = 0 По РезультатВыполнения.Результат.Количество() - 1 Цикл
					ВходящийКонтекст.ДанныеДокумента[Счетчик] = РезультатВыполнения.Результат[Счетчик];
				КонецЦикла;
			Иначе
				ВходящийКонтекст.ДанныеДокумента = РезультатВыполнения.Результат;
			КонецЕсли;
		ИначеЕсли НеОбрабатыватьОшибкиСервиса(ВходящийКонтекст) Тогда
			ПодписатьРезультат(РезультатВыполнения, ВходящийКонтекст);
			Возврат;
	...
	Если ПодтверждатьОперацию ИЛИ ОписаниеСертификата.МобильныйЗакрытыйКлюч Тогда
...
Процедура ПодтвердитьОперациюПослеВыполнения(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если ДанныеПодтверждения.ТипПодтверждения = "Аутентификация"
		И СервисКриптографииDSSКлиентСервер.ЭтоОшибкаАутентификации(РезультатВыполнения.СтатусОшибки) Тогда
...
Процедура ПинКодСертификатаПослеПроверки(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаПинКода(РезультатВыполнения.СтатусОшибки) Тогда
...
Процедура СоздатьЗапросНаСертификатОшибка(РезультатВыполнения, ВходящийКонтекст)
	...
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаАутентификации(РезультатВыполнения.СтатусОшибки) Тогда
		...
		ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
		ПроверкаАутентификацииПользователя(ОповещениеСледующее, ВходящийКонтекст, Истина);
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаНеобработанныеЗапросы(РезультатВыполнения.СтатусОшибки) Тогда
...
Процедура ПроверитьПодпись(НастройкиПользователя, 
	...
	Попытка
		ПодписьПрисоединенная = СервисКриптографииDSSASNКлиентСервер.ПодписьПрисоединенная(ДанныеДокумента);
	Исключение
		ПодписьПрисоединенная = Неопределено;	
	КонецПопытки;	
		
	Если ПодписьПрисоединенная <> Неопределено И ПодписьПрисоединенная Тогда
		ПараметрыЦикла.ИсходныйДокумент = Неопределено;	
	КонецЕсли;
...
Процедура СохранитьДанныеВФайлПроверитьРасширение(РезультатВыполнения, ВходящийКонтекст)
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(ОповещениеСледующее, ТекстСообщения, Ложь);
	...
	Если РезультатВыполнения.Выполнено 
		...
		Иначе
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
				МодульЭлектроннаяПодписьСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
						"ЭлектроннаяПодписьСлужебныйКлиент");
				МодульЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы();
			КонецЕсли;
		КонецЕсли;	
	КонецЕсли;			
...
Функция ПолучитьРезультатВыполненияВФоне(ДлительнаяОперация, ВходящийКонтекст = Неопределено) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		Ответ.СтатусОшибки.Представление = НСтр("ru = 'Работа сервиса DSS завершилась с ошибкой.'", КодЯзыка());
	...
	Иначе
		Ответ.СтатусОшибки.Представление = НСтр("ru = 'Вызов API сервиса DSS завершился с ошибкой при выполнении.'", КодЯзыка()) 
						+ ДлительнаяОперация.КраткоеПредставлениеОшибки;
		Ответ.СтатусОшибки.ИнформацияОбОшибке = ДлительнаяОперация.ИнформацияОбОшибке; 
	КонецЕсли;
...
Процедура ОшибкаПриРаботеСервиса(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	ОтветВызова.СтатусОшибки.ИнформацияОбОшибке = ИнформацияОбОшибке;
	
	Если ЗначениеЗаполнено(КодОшибки) Тогда
		ОтветВызова.Ошибка = ТекстПользователя;
		ОтветВызова.СтатусОшибки.Представление = ТекстПользователя;
	Иначе
...
Функция ПодготовитьПараметрыОперации(ПоказатьОшибку = Истина, ТекущаяСтруктура = Неопределено) Экспорт
	...
	Результат.Вставить("ЗащищенноеСоединениеГОСТ", Ложь);
...
Процедура АктивироватьИУстановитьСертификатРасшифровать(ОтветСервиса, ВходящийКонтекст) Экспорт
	...
	Если НЕ ОтветСервиса.Выполнено Тогда
		...
		ПолучитьОписаниеОшибки(РезультатВыполнения,
							ВходящийКонтекст.ТипОшибки,
							ОтветСервиса.СтатусОшибки.Представление);
		АктивироватьИУстановитьСертификатРезультат(РезультатВыполнения, ВходящийКонтекст);
...
Процедура АктивироватьИУстановитьСертификатПослеРасшифровки(ОтветСервиса, ВходящийКонтекст) Экспорт
	...
	Если ОтветСервиса.Выполнено Тогда
		...
		КонецЕсли;	
	Иначе
		ТекстОшибки = ОтветСервиса.СтатусОшибки.Представление;
	КонецЕсли;
...
Процедура ОткрытьФормуСертификата(НастройкиПользователя, 
	...
	Если Не ОткрытьФормуВзаимодействия("ФормаСвойствоСертификата", ПараметрыФормы, ПараметрыОперации,
		ОповещениеОЗавершении, Истина) Тогда
		Если ФормаВладелец = Неопределено Тогда
			Если ПоказатьСведениеСервиса Тогда
				ОткрытьФорму("Обработка.УправлениеПодключениемDSS.Форма.СвойствоСертификата", ПараметрыФормы,,,,,
					ОбъектОповещенияОбработкиФормы(ОповещениеОЗавершении));
			Иначе
				Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
					МодульЭлектроннаяПодписьСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
						"ЭлектроннаяПодписьСлужебныйКлиент");
					МодульЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуСертификата(ПараметрыФормы,,
						ОбъектОповещенияОбработкиФормы(ОповещениеОЗавершении));
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ПоказатьСведениеСервиса Тогда
				ОткрытьФорму("Обработка.УправлениеПодключениемDSS.Форма.СвойствоСертификата", ПараметрыФормы,
					ФормаВладелец,,,, ОбъектОповещенияОбработкиФормы(ОповещениеОЗавершении),
					РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			Иначе
				Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
					МодульЭлектроннаяПодписьСлужебныйКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
						"ЭлектроннаяПодписьСлужебныйКлиент");
					МодульЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуСертификата(ПараметрыФормы, ФормаВладелец,
						ОбъектОповещенияОбработкиФормы(ОповещениеОЗавершении));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОтобразитьОшибкуПользователю(ОповещениеСледующее, 
	...
	Если ТипЗнч(ТекстОшибки) = Тип("Структура") Тогда
		ПараметрыФормы.Вставить("ТекстСообщения", ТекстОшибки.Представление);
	Иначе
...
Процедура ВернутьРезультатВызова(ВходящийКонтекст, РезультатВызова, ПодготовленныйРезультат = Неопределено)
	...
	Если ПоказатьОшибку 
		...
		И ЗначениеЗаполнено(РезультатВызова.СтатусОшибки.Представление) 
		И НЕ СервисКриптографииDSSКлиентСервер.ЭтоОшибкаОтказа(РезультатВызова.СтатусОшибки) Тогда
		ВывестиОшибку(ОповещениеОЗавершении, РезультатВызова.СтатусОшибки, , ПодготовленныйРезультат);
	ИначеЕсли НЕ РезультатВызова.Выполнено
		И РезультатВызова.СтатусОшибки.ИнформацияОбОшибке <> Неопределено Тогда
		СтандартныеПодсистемыКлиент.ВывестиИнформациюОбОшибке(РезультатВызова.СтатусОшибки.ИнформацияОбОшибке);
