...
Процедура АутентификацияПоЛогину(НастройкиПользователя, ОповещениеОЗавершении, ПараметрыОперации = Неопределено) Экспорт
	...
	Если ИспользованиеОблачнойПодписиВозможно(ПараметрыВызова) Тогда
		...
		Если НЕ ТребуетсяВводКонфиденциальныхДанных(ПарольПользователя)
			И ПараметрыВызова.РежимСохранения <> "Обязательно" Тогда
...
Процедура ПодтверждениеПроизвольнойОперации(НастройкиПользователя, ОповещениеОЗавершении, СодержаниеОперации = "", СпособПодтверждения = Неопределено, ПараметрыОперации = Неопределено) Экспорт
	...
	ПараметрыЦикла.Вставить("ШаблонПодтверждения", НастройкиПользователя.ШаблонПодтверждения);
	ПараметрыЦикла.Вставить("КодОперации", КлючДействия.КодОперации);
...
Процедура ПодтверждениеПроизвольнойОперацииСоздатьТранзакцию(РезультатВыполнения, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		СвойствоПодписи = Новый Структура();
		СвойствоПодписи.Вставить("SignatureType", "CMS");
		СвойствоПодписи.Вставить("Hash", "False");
		СвойствоПодписи.Вставить("DocumentInfo", ВходящийКонтекст.СодержаниеОперации);
		СвойствоПодписи.Вставить("DocumentType", "txt");
		СвойствоПодписи.Вставить("CertificateID", ВходящийКонтекст.Сертификат.Идентификатор);
...
Процедура ПодтверждениеПроизвольнойОперацииОжидать(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		РезультатВыполнения.Свойство("Результат", ИдТранзакции);
		
		ПараметрыПодтверждения = Новый Структура();
		ПараметрыПодтверждения.Вставить("ИдентификаторТранзакции", ИдТранзакции);
...
Процедура ЗашифроватьФоновымЗаданием(РезультатВыполнения, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		ОжидатьЗавершенияВыполненияВФоне(
			ОповещениеСледующее, 
			СервисКриптографииDSSСлужебныйВызовСервера.Зашифровать(
					ВходящийКонтекст.НастройкиПользователя,
...
Процедура РасшифроватьПолучитьСертификаты(РезультатВызова, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Если ВходящийКонтекст.Сертификат = Неопределено Тогда
			...
			ОжидатьЗавершенияВыполненияВФоне(
				ОповещениеСледующее, 
				СервисКриптографииDSSСлужебныйВызовСервера.СертификатыРасшифрования(
						ВходящийКонтекст.НастройкиПользователя,
						ВходящийКонтекст.ДанныеДокумента, 
						ВходящийКонтекст.ТипШифрования, 
						ВходящийКонтекст.ПараметрыОперации),
...
Процедура РасшифроватьСоздатьТранзакцию(ДлительнаяОперация, ВходящийКонтекст)
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Если ПодтверждатьОперацию Тогда
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Если ЭтоПакет Тогда
			ОбработкаОперации = СервисКриптографииDSSСлужебныйВызовСервера.ПодписатьПакет(
					...
					ВходящийКонтекст.ОписаниеСертификата.ПинКод, 
					ВходящийКонтекст.ПараметрыОперации);
...
Процедура ПодтвердитьОперацию(НастройкиПользователя, ОповещениеОЗавершении, ПараметрыПодтверждения, ПараметрыОперации = Неопределено)
	...
	ДанныеПодтверждения = СервисКриптографииDSSСлужебныйВызовСервера.ДанныеВторичнойАвторизацииПоУмолчанию();
	
	ДанныеПодтверждения.Идентификатор = СвойствоСтруктуры(ПараметрыПодтверждения, "ИдентификаторТранзакции", "");
	...
	ДанныеПодтверждения.ИдентификаторТранзакции = ДанныеПодтверждения.Идентификатор;
	...
	Если РезультатВызова.Выполнено Тогда
		...
		Если ТекущийСертификат.ТребуетсяПинКод Тогда
			...
			ПроверитьПинКодСертификата(ВходящийКонтекст.НастройкиПользователя, 
										...
										ВходящийКонтекст.ПараметрыОперации)
		Иначе
			СменитьПинКодСертификатаСоздатьТранзакцию(РезультатВызова, ВходящийКонтекст);
		КонецЕсли;	
	Иначе
...
// 1. подготовка параметров цикла
// 2. проверка аутентификации пользователя
// 3. запуск фонового задания для проверки сертификата
...
Процедура ПроверитьСертификат(НастройкиПользователя, ОповещениеОЗавершении, ДанныеСертификата, ПараметрыОперации = Неопределено) Экспорт
	...
	ОбъектОповещения = Новый ОписаниеОповещения("ПроверитьСертификатФоновымЗаданием", ЭтотОбъект);
	ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ПараметрыЦикла);
...
Процедура ПроверитьСертификатФоновымЗаданием(РезультатВыполнения, ВходящийКонтекст) Экспорт
...
// 1. подготовка параметров цикла
// 2. проверка аутентификации пользователя, вложенный процесс
// 3. запуск фонового задания для проверки подписи
// 4. проверка результата выполнения
// 5. возврат результат успеха.
//
Процедура ПроверитьПодпись(НастройкиПользователя, 
	...
	ОбъектОповещения = Новый ОписаниеОповещения("ПроверитьПодписьФоновымЗаданием", ЭтотОбъект);
	ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ПараметрыЦикла);
...
Процедура ПроверитьПодписьФоновымЗаданием(РезультатВыполнения, ВходящийКонтекст) Экспорт
...
#Область ПодписатьАсинхронно

// Цикл для подписи документа в асинхронном режиме
//
// Шаги цикла:
// 1. подготовка параметров цикла
// 2. проверка аутентификации пользователя
// 3. проверка и определение сертификата
// 4. проверка пин-кода к сертификату
// 5. запуск фонового задания
// 6. проверка успешности окончания операции
// 7. сохранение нового токена для сеанса, возврат результата операции.
//
Процедура ПодписатьАсинхронно(
			НастройкиПользователя, 
			ОповещениеОЗавершении, 
			СвойствоПодписи,
			Сертификат,
	...
	ПараметрыЦикла = Новый Структура();
	ПараметрыЦикла.Вставить("ВидОперации", НСтр("ru = 'Асинхронная подпись'", КодЯзыка()));
	ПараметрыЦикла.Вставить("ОповещениеОЗавершении", ОповещениеОЗавершении);
	ПараметрыЦикла.Вставить("НастройкиПользователя", НастройкиПользователя);
	ПараметрыЦикла.Вставить("СвойствоПодписи", СвойствоПодписи);
	ПараметрыЦикла.Вставить("Сертификат", Сертификат);
	ПараметрыЦикла.Вставить("ПинКод", СвойствоСтруктуры(ПараметрыОперации, "ПинКод"));
	ПараметрыЦикла.Вставить("АсинхронныйРежим", СвойствоСтруктуры(ПараметрыОперации, "АсинхронныйРежим", Ложь));
	ПараметрыЦикла.Вставить("ОбратныйВызов", СвойствоСтруктуры(ПараметрыОперации, "ОбратныйВызов", ""));
	ПараметрыЦикла.Вставить("ПараметрыОперации", ПроверитьПараметрыОперации(ПараметрыОперации));
	
	ОбъектОповещения = Новый ОписаниеОповещения("ПодписатьАсинхронноПолучитьСертификат", ЭтотОбъект);
	ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ПараметрыЦикла);
	ПроверкаАутентификацииПользователя(ОповещениеСледующее, ПараметрыЦикла, Ложь);
			
КонецПроцедуры	

// Продолжение процедуры ПодписатьАсинхронно.
Процедура ПодписатьАсинхронноПолучитьСертификат(РезультатВыполнения, ВходящийКонтекст) Экспорт
	
	Если РезультатВыполнения.Выполнено Тогда
		НастройкиПользователя = РезультатВыполнения.НастройкиПользователя;
		ВходящийКонтекст.НастройкиПользователя = НастройкиПользователя;
		
		СвойствоПодписи = ВходящийКонтекст.СвойствоПодписи.Signature;
		Если НастройкиПользователя.Политика.ТипыПодписей.Найти(СвойствоПодписи.Type) = Неопределено Тогда
			РезультатВыполнения = ОтветСервисаПоУмолчанию(Ложь);
			ПолучитьОписаниеОшибки(РезультатВыполнения,
								"ТипПодписи", 
								НСтр("ru = 'Тип подписи:'", КодЯзыка()) + " "
								+ СвойствоПодписи.Type);
			ПодписатьАсинхронноРезультат(РезультатВыполнения, ВходящийКонтекст);
			
		Иначе
			ТекущийСертификат = СервисКриптографииDSSКлиентСервер.НайтиСертификат(НастройкиПользователя, ВходящийКонтекст.Сертификат);
			
			Если НЕ ЗначениеЗаполнено(ТекущийСертификат.Отпечаток) Тогда
				РезультатОтвета = ОтветСервисаПоУмолчанию(Ложь);
				ПолучитьОписаниеОшибки(РезультатОтвета,
									"certificate_not_found", 
									НСтр("ru = 'При асинхронном подписании'", КодЯзыка()));
				ПодписатьАсинхронноРезультат(РезультатОтвета, ВходящийКонтекст);
				
			ИначеЕсли ВходящийКонтекст.ПинКод <> Неопределено Тогда
				ТекущийСертификат.ПинКод = СервисКриптографииDSSКлиентСервер.ПодготовитьОбъектПароля(ВходящийКонтекст.ПинКод, НастройкиПользователя.АвторизованныйПользователь);
				РезультатВыполнения = ОтветСервисаПоУмолчанию();
				ВходящийКонтекст.Вставить("ОписаниеСертификата", ТекущийСертификат);
				ВходящийКонтекст.Вставить("ПредставлениеСертификата", ТекущийСертификат);
				ПодписатьАсинхронноПослеВводаПинКода(РезультатВыполнения, ВходящийКонтекст);
				
			Иначе	
				ВходящийКонтекст.Вставить("ОписаниеСертификата", ТекущийСертификат);
				ВходящийКонтекст.Вставить("ПредставлениеСертификата", ТекущийСертификат);
				ОбъектОповещения = Новый ОписаниеОповещения("ПодписатьАсинхронноПослеВводаПинКода", ЭтотОбъект);
				ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
				ПинКодСертификата(ВходящийКонтекст.НастройкиПользователя, ОповещениеСледующее, ТекущийСертификат.Идентификатор, ВходящийКонтекст.ПараметрыОперации);
...
// Продолжение процедуры ПодписатьАсинхронно.
Процедура ПодписатьАсинхронноПослеВводаПинКода(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		Если РезультатВыполнения.Результат.ОбновилиПинКод Тогда
			ВходящийКонтекст.НастройкиПользователя = РезультатВыполнения.Результат.НастройкиПользователя;
		КонецЕсли;	
		ПодписатьАсинхронноФоновымЗаданием(РезультатВыполнения, ВходящийКонтекст);
	Иначе
		ПодписатьАсинхронноРезультат(РезультатВыполнения, ВходящийКонтекст);
	КонецЕсли;	
...
//    * ОписаниеСертификата - см. СервисКриптографииDSSКлиентСервер.СвойстваСертификатаПользователя
//
Процедура ПодписатьАсинхронноФоновымЗаданием(РезультатВыполнения, ВходящийКонтекст) Экспорт
	
	Если РезультатВыполнения.Выполнено Тогда
		ОбъектОповещения = Новый ОписаниеОповещения("ПодписатьАсинхронноПослеВыполнения", ЭтотОбъект);
		ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
		НачатьЗамерВремени(ВходящийКонтекст);
		
		ДействияНаСервере = СервисКриптографииDSSСлужебныйВызовСервера.ПодписатьАсинхронно(
					ВходящийКонтекст.НастройкиПользователя,
					ВходящийКонтекст.СвойствоПодписи, 
					ВходящийКонтекст.ОписаниеСертификата.Идентификатор, 
					ВходящийКонтекст.ОписаниеСертификата.ПинКод, 
					ВходящийКонтекст.АсинхронныйРежим, 
					ВходящийКонтекст.ПараметрыОперации);
...
// Продолжение процедуры ПодписатьАсинхронно.
Процедура ПодписатьАсинхронноПослеВыполнения(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли НеОбрабатыватьОшибкиСервиса(ВходящийКонтекст) Тогда
		ПодписатьАсинхронноРезультат(РезультатВыполнения, ВходящийКонтекст);
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаАутентификации(РезультатВыполнения.Ошибка) Тогда
		ОбъектОповещения = Новый ОписаниеОповещения("ПодписатьАсинхронноФоновымЗаданием", ЭтотОбъект);
		ОповещениеСледующее = ОбработчикОповещения(ОбъектОповещения, ВходящийКонтекст);
		ПроверкаАутентификацииПользователя(ОповещениеСледующее, ВходящийКонтекст, Истина);
	Иначе
		ПодписатьАсинхронноРезультат(РезультатВыполнения, ВходящийКонтекст);
...
// Продолжение процедуры ПодписатьАсинхронно.
Процедура ПодписатьАсинхронноРезультат(РезультатВызова, ВходящийКонтекст)
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Если ВыполняемоеДействие = "СведенияОбОперации" Тогда
			ДействияНаСервере = СервисКриптографииDSSСлужебныйВызовСервера.СведенияОбОперации(
						...
						ВходящийКонтекст.ПараметрыОперации);
		ИначеЕсли ВыполняемоеДействие = "ВыполнитьОперацию" Тогда
			ДействияНаСервере = СервисКриптографииDSSСлужебныйВызовСервера.ВыполнитьОперацию(
						ВходящийКонтекст.НастройкиПользователя,
						ВходящийКонтекст.ИдентификаторОперации, 
						ВходящийКонтекст.ПараметрыОперации);
		КонецЕсли;
...
// 3. возврат результат успеха.
//
Процедура ВывестиИнформацию(ОповещениеОЗавершении, ТекстСообщения, ТаймаАут = Неопределено, Заголовок = "", ПараметрыОперации = Неопределено) Экспорт
	...
	ПараметрыЦикла.Вставить("ТаймАут", ТаймаАут);
...
// 3. возврат результат успеха.
//
Процедура ВывестиВопрос(ОповещениеОЗавершении, ТекстСообщения, СписокКоманд, ТаймаАут = Неопределено, Заголовок = "", ПараметрыОперации = Неопределено) Экспорт
...
Функция ПолучитьРезультатВыполненияВФоне(ДлительнаяОперация, ВходящийКонтекст = Неопределено) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		Ответ.Ошибка = НСтр("ru = 'Работа сервиса облачной криптографии завершилась с ошибкой.'", КодЯзыка());
	...
	Иначе
		Ответ.Ошибка = НСтр("ru = 'Вызов API сервиса облачной криптографии завершился с ошибкой при выполнении.'", КодЯзыка()) 
...
Процедура АктивироватьИУстановитьСертификатПослеПроверкиАвторизация(ОтветСервиса, ВходящийКонтекст) Экспорт
	...
	Если НЕ ОтветСервиса.Выполнено Тогда
		...
		ПолучитьОписаниеОшибки(РезультатВыполнения,
							...
							НСтр("ru = 'Ошибка авторизации при установке сертификата в контейнер ключа и хранилище сертификатов облачной подписи'", КодЯзыка()));
...
Процедура АктивироватьИУстановитьСертификатПодтверждениеПользователя(ВходящийКонтекст)
	...
	Если ВходящийКонтекст.ДействиеАктивироватьПароль И ВходящийКонтекст.ДействиеАктивироватьКлючПриложения Тогда
		...
		ОписаниеДействия = НСтр("ru = 'Необходимо завершить настройки для подключения к серверу облачной подписи'")
							...
							+ " - " + НСтр("ru = 'подключитесь к серверу облачной подписи с новым паролем'") + ПС
	...
	ИначеЕсли ВходящийКонтекст.ДействиеАктивироватьПароль Тогда
		...
		ОписаниеДействия = НСтр("ru = 'Необходимо завершить настройки для подключения к серверу облачной подписи.'") + ПС 
							+ " - " + НСтр("ru = 'запомните или сохраните полученный пароль для подключения'") + ПС
							+ " - " + НСтр("ru = 'подтвердите получение пароля'") + ПС
							+ " - " + НСтр("ru = 'с помощью мобильного приложения выполните операцию контрольной подписи'") + ПС
							+ " - " + НСтр("ru = 'подключитесь к серверу облачной подписи с новым паролем'");
	...
	Если ЗначениеЗаполнено(ОписаниеДействия) Тогда
		ПолучитьПодтверждениеПользователя(ОповещениеСледующее,
					НСтр("ru = 'Параметры подключения к облачной подписи'"),
					ОписаниеДействия,
					ПараметрыДляПечати, 
					ЗаголовокПодтверждения,
					ПараметрыОперации);
...
Процедура АктивироватьИУстановитьСертификатПослеАктивированияПароля(ОтветСервиса, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		ПараметрыОперации.Вставить("ПояснениеОперации", НСтр("ru = 'Авторизуйтесь на сервере облачной подписи с помощью пароля, полученного ранее.'"));
...
Процедура АктивироватьИУстановитьСертификатПроверитьСертификат(РезультатВыполнения, ВходящийКонтекст)
	...
	Иначе
		...
		ПолучитьОписаниеОшибки(РезультатВыполнения,
							ВходящийКонтекст.ТипОшибки,
							НСтр("ru = 'Обновление списка сертификатов из хранилища сертификатов облачной подписи'", КодЯзыка()));
		АктивироватьИУстановитьСертификатРезультат(РезультатВыполнения, ВходящийКонтекст); 
...
Процедура ВосстановитьУчетнуюЗаписьПослеПодтверждения(РезультатВыполнения, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Иначе
			...
			ЗаголовокОшибки = НСтр("ru = 'Не удалось создать учетную запись облачной подписи'");
...
Функция ПодготовитьПараметрыФормы(ПараметрыФормы) Экспорт
	...
	Если ПараметрыФормы <> Неопределено Тогда
		Для каждого СтрокаКлюча Из Результат Цикл
			...
			Если ТипЗнч(СтрокаКлюча.Значение) = Тип("ФормаКлиентскогоПриложения") Тогда
