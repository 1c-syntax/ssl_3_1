...
//   * Объект - ЗадачаОбъект 
//            - ЗадачаОбъект.ЗадачаИсполнителя - задача.
...
Процедура ПринятьЗадачуКИсполнению(Форма, ТекущийПользователь) Экспорт
	...
	Исполнитель                     = Форма.Объект.Исполнитель;
	...
	Если Не ЗначениеЗаполнено(Исполнитель) Тогда
		...
		ПринятьЗадачуКИсполнениюПродолжение(Форма, Истина);
	ИначеЕсли Исполнитель = ТекущийПользователь Тогда
		ПринятьЗадачуКИсполнениюПродолжение(Форма);
	Иначе
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ТекущийПользователь", ТекущийПользователь);
		ДополнительныеПараметры.Вставить("Форма",               Форма);
		
		ОповещениеВопрос = Новый ОписаниеОповещения("ПослеВопросаПринятьЗадачуКИсполнению", ЭтотОбъект, ДополнительныеПараметры);
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru ='Принять задачу в работу со сменой исполнителя?
			|Новый исполнитель: %1 (вместо %2)'"), ТекущийПользователь, Исполнитель);
		
		КнопкиДиалога = Новый СписокЗначений();
		КнопкиДиалога.Добавить(КодВозвратаДиалога.Да,     НСтр("ru = 'Да'"));
		КнопкиДиалога.Добавить(КодВозвратаДиалога.Нет,    НСтр("ru = 'Оставить текущего'"));
		КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Отмена'"));
		
		ПоказатьВопрос(ОповещениеВопрос, ТекстВопроса, КнопкиДиалога);
	КонецЕсли;
	...
	Если ИмяСобытия = "Запись_ЗадачаИсполнителя" Тогда
		Если НЕ Форма.Модифицированность 
			...
			Иначе
				Форма.Прочитать();
			КонецЕсли;
		ИначеЕсли Параметр.Свойство("ПринятьЗадачуКИсполнению") И Параметр.ПринятьЗадачуКИсполнению = "Обновить" Тогда
			Форма.Прочитать();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеВопросаПринятьЗадачуКИсполнению(Результат, ДополнительныеПараметры) Экспорт
	
	ТребуетсяОбновлениеФормы = Ложь;
	Если Результат = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	ИначеЕсли Результат = КодВозвратаДиалога.Да Тогда
		ДополнительныеПараметры.Форма.Объект.Исполнитель = ДополнительныеПараметры.ТекущийПользователь;
		ТребуетсяОбновлениеФормы = Истина;
	КонецЕсли;
	
	ПринятьЗадачуКИсполнениюПродолжение(ДополнительныеПараметры.Форма, ТребуетсяОбновлениеФормы);
...
//  Форма               - ФормаКлиентскогоПриложения
//                      - РасширениеУправляемойФормыДляОбъектов - форма задачи, где:
//   * Объект - ЗадачаОбъект.ЗадачаИсполнителя 
//            - ЗадачаОбъект - задача.
//  ТекущийПользователь - СправочникСсылка.ВнешниеПользователи
//                      - СправочникСсылка.Пользователи - ссылка на текущего
//                                                        пользователя приложения.
//
Процедура ПринятьЗадачуКИсполнениюПродолжение(Форма, ТребуетсяОбновлениеФормы = Ложь)
	
	ПринятьЗадачуКИсполнению = ?(ТребуетсяОбновлениеФормы, "Обновить", "");
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ПринятьЗадачуКИсполнению", ПринятьЗадачуКИсполнению);
