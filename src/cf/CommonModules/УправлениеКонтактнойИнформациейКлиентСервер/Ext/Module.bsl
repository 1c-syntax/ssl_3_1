...
Функция РасширеннаяПодсказкаАдреса(КомандыДляВывода, ПредставлениеАдреса, Комментарий) Экспорт
	...
	Для каждого КомандаДляВывода Из КомандыДляВывода Цикл
		ВидДействия = ВидДействияКомандыТипаКонтактнойИнформации(КомандаДляВывода.Значение.Действие);
		Если СтрСравнить(ВидДействия, "ПоказатьНаКарте") = 0 Тогда
			Если ПустаяСтрока(ПоказатьНаКарте) Тогда
				ПоказатьНаКарте = Новый ФорматированнаяСтрока(КомандаДляВывода.Значение.Заголовок,, WebЦвета.Серый,, ВидДействия);
			Иначе
				ПоказатьНаКарте = Новый ФорматированнаяСтрока(НСтр("ru = 'На карте'"),, WebЦвета.Серый,, ВидДействия);
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ВидДействияКомандыТипаКонтактнойИнформации(Действие) Экспорт
	
	Если Действие = "УправлениеКонтактнойИнформациейКлиент.ПоказатьАдресНаКартеGoogle" Тогда
		ВидДействия = "ПоказатьНаКарте";
	Иначе
		ВидДействия = "";
