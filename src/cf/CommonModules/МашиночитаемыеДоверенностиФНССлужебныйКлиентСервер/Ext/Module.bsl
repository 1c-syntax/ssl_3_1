...
Функция ЭтоПолномочиеФНС(КодПолномочия) Экспорт
	Возврат СтрНачинаетсяС(КодПолномочия, "1_FNS")
			Или СтрНачинаетсяС(КодПолномочия, "2_FNS")
...
Функция НазначениеДоверенностиДляПроверкиРеквизитов() Экспорт
	
	НазначениеДоверенности = Новый Структура;
	НазначениеДоверенности.Вставить("ДляНалоговыхОрганов", Ложь);
	НазначениеДоверенности.Вставить("ДляГосударственныхУслуг", Ложь);
	НазначениеДоверенности.Вставить("Нотариальная", Ложь);
	НазначениеДоверенности.Вставить("ТипУчастника", Неопределено);
	
	Возврат НазначениеДоверенности;
...
// 
// Параметры:
//  Назначение - см. НазначениеДоверенностиДляПроверкиРеквизитов
...
//  СписокЗначений
//	
Функция РеквизитыОрганизацииСписок(Назначение = Неопределено, ДляЗаполнения = Истина, ВидОрганизации = Неопределено) Экспорт
	
	Если Назначение = Неопределено Тогда
		Назначение = НазначениеДоверенностиДляПроверкиРеквизитов();
	КонецЕсли;
	ЭтоДоверитель = Назначение.ТипУчастника = ПредопределенноеЗначение("Перечисление.ТипыУчастниковМЧД.Доверитель");
	
	// Пометка списка Истина означает, что поле необязательное
	...
	Реквизиты.Добавить("ИНН",                              НСтр("ru = 'ИНН'"), ВидОрганизации = "ИностраннаяОрганизация" И ЭтоДоверитель);
	...
	Если ДляЗаполнения Или ВидОрганизации <> "ИндивидуальныйПредприниматель" Тогда
		Реквизиты.Добавить("КПП", НСтр("ru = 'КПП'"), ВидОрганизации = "ИностраннаяОрганизация" И ЭтоДоверитель);
	КонецЕсли;
	...
	Если ДляЗаполнения Или ВидОрганизации = "ИностраннаяОрганизация" Тогда
		Реквизиты.Добавить("НомерЗаписиОбАккредитации",               НСтр("ru = 'Номер записи об аккредитации'"), ЭтоДоверитель);
		Реквизиты.Добавить("СтранаРегистрации",                       НСтр("ru = 'Страна регистрации (инкорпорации)'"), Не ЭтоДоверитель);
		Реквизиты.Добавить("СтранаРегистрацииКод",                    НСтр("ru = 'Код страны регистрации (инкорпорации)'"), Не ЭтоДоверитель);
		Реквизиты.Добавить("РегистрационныйНомерВСтранеРегистрации",  НСтр("ru = 'Регистрационный номер в стране регистрации (инкорпорации)'"), Не ЭтоДоверитель);
		Реквизиты.Добавить("НаименованиеРегистрирующегоОргана",       НСтр("ru = 'Наименование регистрирующего органа'"), Истина);
		Реквизиты.Добавить("КодНалогоплательщикаВСтранеРегистрации",  НСтр("ru = 'Код налогоплательщика в стране регистрации (инкорпорации) или аналог'"), Истина);
		...
		Реквизиты.Добавить("ЮридическийАдресВСтранеРегистрации",      НСтр("ru = 'Адрес юридического лица на территории государства, в котором оно зарегистрировано'"), Не ЭтоДоверитель);
	...
	Если ДляЗаполнения И ВидОрганизации = "ИндивидуальныйПредприниматель" Тогда
		Реквизиты.НайтиПоЗначению("НаименованиеПолное").Пометка = Не Назначение.ДляГосударственныхУслуг;
...
//  СписокЗначений
//
Функция РеквизитыФизическогоЛицаСписок(Назначение = Неопределено, ДляЗаполнения = Истина, ВидФизическогоЛица = Неопределено) Экспорт
		
	Если Назначение = Неопределено Тогда
		Назначение = НазначениеДоверенностиДляПроверкиРеквизитов();
	КонецЕсли;
	
	НеобязательныйРеквизит = Назначение.ТипУчастника = ПредопределенноеЗначение("Перечисление.ТипыУчастниковМЧД.Доверитель") 
		Или МашиночитаемыеДоверенностиФНССлужебныйВызовСервера.РеквизитыПредставителяСталиНеобязательными();
	...
	Реквизиты.Добавить("Пол",                      НСтр("ru = 'Пол'"), Истина);
	Если ДляЗаполнения Или ВидФизическогоЛица <> "ИндивидуальныйПредприниматель" Тогда
		Реквизиты.Добавить("ИННФЛ", НСтр("ru = 'ИНН'"));
	КонецЕсли;
	Реквизиты.Добавить("СтраховойНомерПФР",        НСтр("ru = 'СНИЛС'"));
	Реквизиты.Добавить("АдресРегистрации",         НСтр("ru = 'Адрес регистрации'"), Истина);
	...
	Реквизиты.Добавить("ДокументВид",              НСтр("ru = 'Вид документа'"), НеобязательныйРеквизит);
	Реквизиты.Добавить("ДокументНомер",            НСтр("ru = 'Серия и номер'"), НеобязательныйРеквизит);
	Реквизиты.Добавить("ДокументКемВыдан",         НСтр("ru = 'Кем выдан'"), НеобязательныйРеквизит);
	Реквизиты.Добавить("ДокументДатаВыдачи",       НСтр("ru = 'Дата выдачи'"), НеобязательныйРеквизит);
	Реквизиты.Добавить("ДокументКодПодразделения", НСтр("ru = 'Код подразделения'"), НеобязательныйРеквизит);
	Реквизиты.Добавить("ДокументСрокДействия",     НСтр("ru = 'Срок действия'"), Истина);
	Реквизиты.Добавить("Гражданство",              НСтр("ru = 'Гражданство'"), Не Назначение.ДляНалоговыхОрганов);
...
Функция ТипУчастникаМЧД(ИмяЭлементаОрганизация) Экспорт
	
	Если ИмяЭлементаОрганизация = "Представитель" Или ИмяЭлементаОрганизация = "ЛицоПредставителя" Тогда
		ТипУчастникаМЧД = ПредопределенноеЗначение("Перечисление.ТипыУчастниковМЧД.Представитель");
	Иначе
		ТипУчастникаМЧД = ПредопределенноеЗначение("Перечисление.ТипыУчастниковМЧД.Доверитель");
	КонецЕсли;
	
	Возврат ТипУчастникаМЧД;
	
КонецФункции

Функция РеквизитыФизическогоЛица(ДляЗаполнения, ВидУчастника) Экспорт
	
	Реквизиты = РеквизитыФизическогоЛицаСписок(Неопределено, ДляЗаполнения, ВидУчастника);
...
Функция РеквизитыОрганизации(ДляЗаполнения, ВидУчастника) Экспорт
	...
	Реквизиты = РеквизитыОрганизацииСписок(Неопределено, ДляЗаполнения, ВидУчастника);
	Возврат СтруктураРеквизитовИзСписка(Реквизиты);
...
Функция РезультатПроверкиДоверенности(Результат) Экспорт
	...
	Если Не РезультатПроверкиДоверенности.Верна Или СтатусВернаУстановленВручную Тогда
		...
		ИначеЕсли Не ЗначениеЗаполнено(Результат.ДатаПроверкиИзДанныхДоверенности) Тогда
			...
			ОписаниеОшибки = НовыеСвойстваОшибки();
			ОписаниеОшибки.Описание = НСтр("ru='Статус доверенности - требуется проверка'");
			ОписаниеОшибок.Ошибки.Добавить(ОписаниеОшибки);
			Ошибки.Добавить(ОписаниеОшибки.Описание);
		Иначе
			
			ОписаниеОшибки = НовыеСвойстваОшибки();
			ОписаниеОшибки.Описание = НСтр("ru='Статус доверенности - не верна'");
			ОписаниеОшибок.Ошибки.Добавить(ОписаниеОшибки);
			Ошибки.Добавить(ОписаниеОшибки.Описание);
