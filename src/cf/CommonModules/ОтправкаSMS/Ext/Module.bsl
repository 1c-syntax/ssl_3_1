...
Функция ОтправитьSMS(НомераПолучателей, Знач Текст, ИмяОтправителя = Неопределено, ПеревестиВТранслит = Ложь) Экспорт
	...
	Если Не ОбщегоНазначения.РазрешенДоступКИнтернетСервисам() Тогда
		Результат.ОписаниеОшибки = ОбщегоНазначения.ЗапрещенДоступКИнтернетСервисамТекстСообщения();
		Возврат Результат;
	КонецЕсли;
		...
		Если Результат.ОтправленныеСообщения.Количество() > 0 Тогда
			...
			Для Индекс = 0 По Результат.ОтправленныеСообщения.Количество() - 1 Цикл
				ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
					"ОтправкаSMSПереопределяемый.ОтправитьSMS",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Результат.ОтправленныеСообщения[%1]", 
						Формат(Индекс, "ЧН=; ЧГ=0")),
...
Функция СтатусДоставки(Знач ИдентификаторСообщения) Экспорт
	...
	Возврат СтатусыДоставки(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторСообщения))[ИдентификаторСообщения];
...
Функция ДоступнаОтправкаSMS() Экспорт
	...
	Возврат ОтправкаSMSПовтИсп.ДоступнаОтправкаSMS() И ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
...
//
// Параметры:
//  ИдентификаторыСообщений - Массив из Строка - идентификаторы, присвоенные SMS при отправке;
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//   * Ключ - Строка - идентификатор SMS;
//   * Значение - Строка - статус доставки сообщения, который вернул поставщик услуг:
//           "НеОтправлялось" - сообщение еще не было обработано поставщиком услуг (в очереди);
//           "Отправляется"   - сообщение стоит в очереди на отправку у провайдера;
//           "Отправлено"     - сообщение отправлено, ожидается подтверждение о доставке;
//           "НеОтправлено"   - сообщение не отправлено (недостаточно средств на счете, перегружена сеть оператора);
...
//           "Ошибка"         - не удалось получить статус у поставщика услуг (статус неизвестен).
//
Функция СтатусыДоставки(Знач ИдентификаторыСообщений) Экспорт
	
	ПроверитьПрава();
	
	ОбщегоНазначения.РазрешенДоступКИнтернетСервисам(Истина);
	
	СтатусыДоставки = ОтправкаSMSПовтИсп.СтатусыДоставки(СтрСоединить(ИдентификаторыСообщений, ","));
	
	Возврат Новый Соответствие(СтатусыДоставки);
...
Функция НастройкиПровайдераПоУмолчанию()
	...
	Результат.Вставить("СтатусыДоставкиОднимЗапросом", Ложь);
