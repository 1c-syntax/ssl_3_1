...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон = Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		НазначенияШаблона = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ПараметрыОтправки.Шаблон, "ПредназначенДляSMS, ПредназначенДляЭлектронныхПисем");
		Если НазначенияШаблона.ПредназначенДляSMS Тогда
			ПараметрыОтправки.ДополнительныеПараметры.Вставить("ВидСообщения", "СообщениеSMS");
		ИначеЕсли НазначенияШаблона.ПредназначенДляЭлектронныхПисем Тогда
			ПараметрыОтправки.ДополнительныеПараметры.Вставить("ВидСообщения", "ЭлектроннаяПочта");
		Иначе
			ПараметрыОтправки.ДополнительныеПараметры.Вставить("ВидСообщения", "Произвольный");
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" И СведенияОШаблоне <> Неопределено Тогда
		...
		ДобавитьВыбранныеМакетыВыгрузкиВоВложения(ПараметрыОтправки, СведенияОШаблоне, Сообщение.Вложения, ПараметрыШаблона);
	...
	Если ПараметрыШаблона.ДобавлятьПрисоединенныеФайлы Тогда
			...
			Для Каждого ФайлВладельца Из ФайлыВладельца Цикл
				...
				ПараметрыДанныхФайла = МодульРаботаСФайламиКлиентСервер.ПараметрыДанныхФайла();
				ПараметрыДанныхФайла.ВызыватьИсключение = Ложь;
				ДанныеФайла = МодульРаботаСФайлами.ДанныеФайла(ФайлВладельца, ПараметрыДанныхФайла);
	...
	Если ПараметрыШаблона.ТипШаблона <> "Произвольный" Тогда
...
Функция СформироватьСообщениеИОтправить(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон.ПредназначенДляSMS Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
			...
			Если Не МодульОтправкаSMS.ДоступнаОтправкаSMS() Тогда
			...
			РезультатОтправкаSMS = МодульОтправкаSMS.ОтправитьSMS(НомераПолучателей, Сообщение.Текст, 
				Сообщение.ДополнительныеПараметры.Отправитель, Сообщение.ДополнительныеПараметры.ПеревестиВТранслит);
	...
	ИначеЕсли ПараметрыОтправки.Шаблон.ПредназначенДляЭлектронныхПисем Тогда
		Если Сообщение.Получатель.Количество() = 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			УчетнаяЗапись = ?( ПараметрыОтправки.ДополнительныеПараметры.УчетнаяЗапись <> Неопределено,
				ПараметрыОтправки.ДополнительныеПараметры.УчетнаяЗапись, МодульРаботаСПочтовымиСообщениями.СистемнаяУчетнаяЗапись());
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия") Тогда
				...
				Если МодульВзаимодействия.ИспользуетсяПочтовыйКлиент() Тогда
					...
					ПараметрыПисьма.ДополнительныеПараметры.Комментарий = КомментарийПоНаименованиюШаблона(
						Сообщение.ДополнительныеПараметры.Наименование);
					
					РезультатОтправки = МодульВзаимодействия.СоздатьПисьмо(ПараметрыПисьма, УчетнаяЗапись, 
						ПараметрыОтправки.ДополнительныеПараметры.ОтправитьСразу);
...
Функция ЕстьДоступныеШаблоны(ТипШаблона, Предмет = Неопределено) Экспорт
	...
	Если Не ЕстьПравоЧтенияШаблоновСообщений() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "ШаблоныСообщенийСлужебный.ОбработатьДанныеДляПереходаНаПодключаемыеКоманды";
	Обработчик.Версия = "3.1.10.99";
	Обработчик.РежимВыполнения = "Оперативно";
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	ДоступныеВарианты = ДоступныеВариантыОтправкиШаблоновСообщений();
	
	Если ДоступныеВарианты.Почта Тогда
		Команда = Команды.Добавить();
		Команда.Вид = "Отправить";
		Команда.Представление = НСтр("ru = 'Почта'");
		Команда.Картинка = БиблиотекаКартинок.ОтправитьЭлектронноеПисьмо;
		Команда.РежимЗаписи = "НеЗаписывать";
		Команда.Порядок = 40;
		Команда.ТипПараметра = Метаданные.ОпределяемыеТипы.ПредметШаблонаСообщения.Тип;
		Команда.Обработчик = "ШаблоныСообщенийКлиент.ОтправитьПисьмо";
		Команда.МножественныйВыбор = Ложь;
	КонецЕсли;
	
	Если ДоступныеВарианты.SMS Тогда
		Команда = Команды.Добавить();
		Команда.Вид = "Отправить";
		Команда.Представление = НСтр("ru = 'SMS'");
		Команда.РежимЗаписи = "НеЗаписывать";
		Команда.Порядок = 50;
		Команда.ТипПараметра = Метаданные.ОпределяемыеТипы.ПредметШаблонаСообщения.Тип;
		Команда.Обработчик = "ШаблоныСообщенийКлиент.ОтправитьSMS";
		Команда.МножественныйВыбор = Ложь;
	КонецЕсли;
...
Процедура ПриОпределенииСоставаНастроекПодключаемыхОбъектов(НастройкиПрограммногоИнтерфейса) Экспорт
	...
	Настройка.Ключ          = "ДобавитьКомандыОтправки";
...
// Возвращает сведения о доступных вариантах отправки шаблонов сообщений.
// 
// Возвращаемое значение:
//  ФиксированнаяСтруктура:
//   * SMS - Булево - если Истина, то можно формировать SMS сообщение по шаблонам.
//   * Почта - Булево - если Истина, то можно создавать письма по шаблонам.
//
Функция ДоступныеВариантыОтправкиШаблоновСообщений() Экспорт
	
	Возврат ШаблоныСообщенийСлужебныйПовтИсп.ДоступныеВариантыОтправкиШаблоновСообщений();
...
Процедура ОпределитьСписокРеквизитовИВложений(СведенияОШаблоне, ПараметрыШаблона)
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.ПолноеИмяТипаНазначения) Тогда
		...
		РеквизитыОбъектаНазначения = РеквизитыОбъектаНазначения(СведенияОШаблоне.Реквизиты, 
			ПараметрыШаблона.ПолноеИмяТипаНазначения, ПараметрыШаблона.Назначение);
		...
		Если МетаданныеОбъект <> Неопределено Тогда
			...
			ОпределитьСписокМакетовВыгрузки(МетаданныеОбъект, СведенияОШаблоне);
		...
		ШаблоныСообщенийПереопределяемый.ПриПодготовкеШаблонаСообщения(РеквизитыОбъектаНазначения, СведенияОШаблоне.Вложения, 
			ПараметрыШаблона.ПолноеИмяТипаНазначения, ПараметрыШаблона);
...
Функция СформироватьСообщениеВнешнейОбработкой(ПараметрыШаблона, СведенияОШаблоне, ПараметрыОтправки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		ИначеЕсли ТипЗнч(Сообщение) = Тип("Структура") И ПараметрыОтправки.ДополнительныеПараметры.ВидСообщения = "Произвольный" Тогда
			
			СформированноеСообщение.Текст = Сообщение.ТекстПроизвольногоСообщения;
...
Функция СообщениеБезШаблона(ПараметрыОтправки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыОтправки.ДополнительныеПараметры.ПечатныеФормы) Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВыгрузкаОбъектовВФайлы")
			   И ЗначениеЗаполнено(ПараметрыОтправки.ДополнительныеПараметры.МакетыВыгрузки) Тогда
				
				МодульВыгрузкаОбъектовВФайлы = ОбщегоНазначения.ОбщийМодуль("ВыгрузкаОбъектовВФайлы");
				МакетыВыгрузки = ПараметрыОтправки.ДополнительныеПараметры.МакетыВыгрузки;
				Файлы = МодульВыгрузкаОбъектовВФайлы.СохранитьПоФорматуВФайл(
					МакетыВыгрузки,
					СписокОбъектов,
					НастройкиСохранения);
				Для Каждого Файл Из Файлы Цикл
...
Функция ПодготовитьЗапросДляПолученияСпискаШаблонов(ТипШаблона, Предмет = Неопределено, ВладелецШаблона = Неопределено, ВыводитьОбщиеШаблон = Истина) Экспорт
	...
	ПредназначенДляSMS = ТипШаблона = "SMS";
	ПредназначенДляЭлектронныхПисем = ТипШаблона = "Письмо";
	...
	|				КОНЕЦ
	|		КОГДА ШаблоныСообщений.ПредназначенДляSMS
	|			ТОГДА ШаблоныСообщений.ТекстШаблонаSMS
	|		ИНАЧЕ ШаблоныСообщений.ТекстШаблонаПроизвольный
...
Процедура ОпределитьСписокМакетовВыгрузки(МетаданныеОбъект, Знач ПараметрыШаблона, ИмяПараметра = "")
	...
	Если ИсточникиКомандПечати.Найти(МетаданныеОбъект) <> Неопределено Тогда
		...
		КомандыВыгрузкиОбъекта =
			МодульВыгрузкаОбъектовВФайлы.КомандыВыгрузкиОбъектаДоступныеДляВложений(МетаданныеОбъект);
		СоответствиеФорматаВыгрузкиРасширению =
			МодульВыгрузкаОбъектовВФайлы.СоответствиеФорматаВыгрузкиФорматуСохранения();
		ПроверкаНаДубли = Новый Соответствие;
		
		Для Каждого Вложение Из КомандыВыгрузкиОбъекта Цикл
			
			Если ДобавлятьКомандуВыгрузкиВоВложение(Вложение, ПроверкаНаДубли) Тогда
				
				ФорматСохраненияОбъекта = Вложение.ДополнительныеПараметры.ФорматСохранения;
				
				НоваяСтрока = ПараметрыШаблона.Вложения.Добавить();
				НоваяСтрока.Имя = Вложение.ИдентификаторМакета;
				НоваяСтрока.Идентификатор = Вложение.Идентификатор;
				НоваяСтрока.Представление = Вложение.Представление;
				НоваяСтрока.МенеджерПечати = Вложение.МенеджерПечати;
				НоваяСтрока.ТипФайла = СоответствиеФорматаВыгрузкиРасширению[ФорматСохраненияОбъекта];
				НоваяСтрока.Статус = "МакетВыгрузки";
				НоваяСтрока.ИмяПараметра = ИмяПараметра;
				НоваяСтрока.ПараметрыПечати = Вложение.ДополнительныеПараметры;
				ПроверкаНаДубли.Вставить(Вложение.УникальныйИдентификатор, Истина);
...
Функция ДобавлятьКомандуВыгрузкиВоВложение(Вложение, ПроверкаНаДубли)
	
	Возврат НЕ Вложение.Отключена
		И СтрНайти(Вложение.Идентификатор, ",") = 0
		И НЕ ПустаяСтрока(Вложение.МенеджерПечати)
		И НЕ Вложение.СкрытаФункциональнымиОпциями
		И ПроверкаНаДубли[Вложение.УникальныйИдентификатор] = Неопределено
		И ЗначениеЗаполнено(Вложение.ФорматСохранения);
...
// Параметры:
//  ПараметрыОтправки - см. ШаблоныСообщений.СформироватьПараметрыОтправки
//  СведенияОШаблоне - см. КонструкторСведенийОШаблоне
//  Вложения - Соответствие из КлючИЗначение:
//   * Ключ - Строка 
//   * Значение - Строка 
//  ПараметрыШаблона - см. ШаблоныСообщенийКлиентСервер.ОписаниеПараметровШаблона
//
...
//  ПараметрыШаблона - см. ШаблоныСообщенийКлиентСервер.ОписаниеПараметровШаблона
//
Процедура ДобавитьВыбранныеМакетыВыгрузкиВоВложения(ПараметрыОтправки, СведенияОШаблоне, Вложения, ПараметрыШаблона)
	
	Если СведенияОШаблоне.Вложения.Количество() = 0 
	...
	КомандыВыгрузки = Новый Массив;  // Массив Из Структура
	
	Для Каждого ВложениеМакетВыгрузки Из СведенияОШаблоне.Вложения Цикл
		
		ИмяПараметраСМакетомВыгрузкиВШаблоне = ПараметрыШаблона.ВыбранныеВложения[ВложениеМакетВыгрузки.Идентификатор];
		
		Если ВложениеМакетВыгрузки.Статус = "МакетВыгрузки" И ИмяПараметраСМакетомВыгрузкиВШаблоне <> Неопределено Тогда
			
			ИмяМенеджераПечати = ВложениеМакетВыгрузки.МенеджерПечати;
			ПараметрыПечати = ВложениеМакетВыгрузки.ПараметрыПечати;
			ИменаМакетов = ВложениеМакетВыгрузки.Идентификатор;
			
			КомандаВыгрузки = Новый Структура;
			КомандаВыгрузки.Вставить("Идентификатор", ИменаМакетов);
			КомандаВыгрузки.Вставить("МенеджерПечати", ИмяМенеджераПечати);
			КомандаВыгрузки.Вставить("ДополнительныеПараметры", ПараметрыПечати);
	...
	Если КомандыВыгрузки.Количество() > 0 Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Ошибка при создании файла выгрузки. По причине:
								   |%1'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ПредставлениеОшибки);
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(),
		...
		Для Каждого Файл Из Файлы Цикл
			...
			АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(
				Файл.ДвоичныеДанные,
				ПараметрыОтправки.УникальныйИдентификатор);
			Вложения.Вставить(Файл.ИмяФайла, АдресВоВременномХранилище);
...
Функция ПараметрыШаблона(Шаблон) Экспорт
	...
	ИначеЕсли ТипЗнч(Шаблон) = Тип("СправочникСсылка.ШаблоныСообщений") Тогда
		...
		Если СведенияОШаблоне.Количество() > 0 Тогда
			...
			Результат.ТипШаблона                 = ТипШаблона(СтрокаСведенийОШаблоне);
			...
			ИначеЕсли Результат.ТипШаблона = "Письмо" Тогда
...
Процедура УстановитьПараметрыШаблона(Шаблон, Результат)
	...
	Результат.ТипШаблона                  = ТипШаблона(Шаблон);
	...
	ИначеЕсли Результат.ТипШаблона = "Письмо" Тогда
		Если Шаблон.ТипТекстаПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
			Результат.Текст = Шаблон.ТекстШаблонаПисьмаHTML;
		Иначе
			Результат.Текст = Шаблон.ТекстШаблонаПисьма;
		КонецЕсли;
	Иначе
		Результат.Текст = Шаблон.ТекстШаблонаПроизвольный;
	КонецЕсли;
...
Функция ТипШаблона(Шаблон)
	
	Результат = "Произвольный";
	
	Если Шаблон.ПредназначенДляSMS Тогда
		Результат = "SMS";
	ИначеЕсли Шаблон.ПредназначенДляЭлектронныхПисем Тогда
		Результат = "Письмо";
	КонецЕсли;
...
Функция ПараметрыИзТекстаСообщения(ПараметрыШаблона) Экспорт
	...
	ИначеЕсли ПараметрыШаблона.ТипШаблона = "SMS" 
	      Или ПараметрыШаблона.ТипШаблона = "Произвольный" Тогда
...
Функция ЕстьПравоЧтенияШаблоновСообщений()
	
	Возврат ПравоДоступа("Чтение", Метаданные.Справочники.ШаблоныСообщений);
	
КонецФункции

Процедура ОбработатьДанныеДляПереходаНаПодключаемыеКоманды() Экспорт
		
	ЕстьПодключаемыеКоманды = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды");

	Блокировка = Новый БлокировкаДанных;
	Блокировка.Добавить("Константа.ИспользоватьОтправкуSMSВШаблонахСообщений");
	Блокировка.Добавить("Константа.ИспользоватьЭлектроннуюПочтуВШаблонахСообщений");
	
	НачатьТранзакцию();
	Попытка
		Блокировка.Заблокировать();
		
		Объект = Метаданные.Константы.ИспользоватьОтправкуSMSВШаблонахСообщений;
		ПредставлениеОбъекта = Объект.Представление();
		Константы.ИспользоватьОтправкуSMSВШаблонахСообщений.Установить(Не ЕстьПодключаемыеКоманды);
		
		Объект = Метаданные.Константы.ИспользоватьЭлектроннуюПочтуВШаблонахСообщений;
		ПредставлениеОбъекта = Объект.Представление();
		Константы.ИспользоватьЭлектроннуюПочтуВШаблонахСообщений.Установить(Не ЕстьПодключаемыеКоманды);
		
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
				
		ОбновлениеИнформационнойБазы.ЗаписатьОшибкуВЖурналРегистрации(Объект, ПредставлениеОбъекта, ИнформацияОбОшибке());
	КонецПопытки;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДобавитьРольДобавлениеИзменениеЛичныхШаблоновВПрофилиСБазовымиПравами");
	Методы.Вставить("ОбработатьДанныеДляПереходаНаПодключаемыеКоманды");
