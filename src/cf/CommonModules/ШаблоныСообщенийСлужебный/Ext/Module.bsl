...
Функция ПреобразоватьТекстШаблона( Знач Текст, Знач СписокВсехПараметров, ВариантКонвертации) Экспорт
	...
	Для каждого ПараметрВШаблоне Из ПараметрыВШаблоне Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(ОписаниеПараметра.Формат) И ИмяРеквизитаПолучателя = "ПолноеПредставление" Тогда
				НайденныеСтроки[0].Формат = ОписаниеПараметра.Формат;
			КонецЕсли;
			
			Если ПустаяСтрока(НайденныеСтроки[0].Формат) Тогда
				ПараметрыВШаблоне[ПараметрВШаблоне.Ключ] = НайденныеСтроки[0][ИмяРеквизитаПолучателя];
			Иначе
				ПараметрыВШаблоне[ПараметрВШаблоне.Ключ] = НайденныеСтроки[0][ИмяРеквизитаПолучателя] + "{" + НайденныеСтроки[0].Формат + "}";
			КонецЕсли;
...
//   Сведения - см. ШаблоныСообщенийКлиентСервер.СтрокаВложений
//   ИмяФайла - Строка
//   Размер - Число 
