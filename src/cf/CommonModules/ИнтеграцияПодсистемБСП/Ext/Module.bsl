...
Функция СобытияБСП() Экспорт
	...
	События.Вставить("ПриПолученииОбщихНастроекАвторизацииИнтернетСервиса", Ложь);
	События.Вставить("ПриПолученииАдресаОткрытойАвторизацииПочтовогоСервиса", Ложь);
	События.Вставить("ПриОпределенииПубликацииОткрытойАвторизацииПочтовогоСервиса", Ложь);
...
Процедура РегистрацияИнтерфейсовПринимаемыхСообщений(МассивОбработчиков) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.БазоваяФункциональностьВМоделиСервиса") Тогда
		МодульСообщенияОткрытаяАвторизацияПочтовогоСервисаИнтерфейсСлужебный = ОбщегоНазначения.ОбщийМодуль("СообщенияОткрытаяАвторизацияПочтовогоСервисаИнтерфейсСлужебный");
		МодульСообщенияОткрытаяАвторизацияПочтовогоСервисаИнтерфейсСлужебный.РегистрацияИнтерфейсовПринимаемыхСообщений(МассивОбработчиков);
	КонецЕсли;
...
Процедура ПриПолученииСпискаШаблонов(ШаблоныЗаданий) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		МодульОбменДаннымиСервер.ПриПолученииСпискаШаблонов(ШаблоныЗаданий);
	КонецЕсли;
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		МодульУправлениеКонтактнойИнформациейСлужебный.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		МодульОбменДаннымиСервер = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСервер");
		МодульОбменДаннымиСервер.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	КонецЕсли;
	...
	ПользователиСлужебный.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		МодульУправлениеДоступомСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступомСлужебный");
		МодульУправлениеДоступомСлужебный.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ГенерацияШтрихкода") Тогда
		...
		МодульГенерацияШтрихкода.ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений);
	КонецЕсли;
...
// Вызывается при получении общих настроек авторизации интернет-сервиса.
// Позволяет задать идентификатор/пароль приложения, адреса перенаправления и др.
//
// Параметры:
//  ИмяИнтернетСервиса   - Строка - имя почтового сервиса
//  ВладелецДанных       - Строка - домен владельца данных
//  НастройкиАвторизации - см. Справочники.ОбщиеНастройкиАвторизацииИнтернетСервисов.НастройкиАвторизацииИнтернетСервиса
//
Процедура ПриПолученииОбщихНастроекАвторизацииИнтернетСервиса(ИмяИнтернетСервиса, ВладелецДанных, НастройкиАвторизации) Экспорт
	
	Если ИнтеграцияПодсистемБСППовтИсп.ПодпискиБТС().ПриПолученииОбщихНастроекАвторизацииИнтернетСервиса Тогда
		МодульИнтеграцияПодсистемБТС = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияПодсистемБТС");
		МодульИнтеграцияПодсистемБТС.ПриПолученииОбщихНастроекАвторизацииИнтернетСервиса(ИмяИнтернетСервиса, ВладелецДанных, НастройкиАвторизации);
	КонецЕсли;
	
КонецПроцедуры

// Позволяет переопределить адрес HTTP-сервиса открытой авторизации почтового сервиса.
// По умолчанию: <АдресПубликацииИБ>/hs/oauth2_mail/callback
//
// Параметры:
//  Адрес - Строка - адрес перенаправления
//
Процедура ПриПолученииАдресаОткрытойАвторизацииПочтовогоСервиса(Адрес) Экспорт
	
	Если ИнтеграцияПодсистемБСППовтИсп.ПодпискиБТС().ПриПолученииАдресаОткрытойАвторизацииПочтовогоСервиса Тогда
		МодульИнтеграцияПодсистемБТС = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияПодсистемБТС");
		МодульИнтеграцияПодсистемБТС.ПриПолученииАдресаОткрытойАвторизацииПочтовогоСервиса(Адрес);
	КонецЕсли;
	
КонецПроцедуры

// Вызывается при определении публикации HTTP-сервиса открытой авторизации почтового сервиса.
// Позволяет задать признак публикации без стандартной проверки доступности сервиса.
//
// Параметры:
//  Опубликована         - Булево - признак публикации HTTP-сервиса.
//                                  Истина - сервис опубликован и доступен;
//                                  Ложь - сервис не опубликован.
//  СтандартнаяОбработка - Булево - если установить Ложь, стандартная проверка публикации
//                                  выполняться не будет, используется значение параметра Опубликована.
//
Процедура ПриОпределенииПубликацииОткрытойАвторизацииПочтовогоСервиса(Опубликована, СтандартнаяОбработка) Экспорт
	
	Если ИнтеграцияПодсистемБСППовтИсп.ПодпискиБТС().ПриОпределенииПубликацииОткрытойАвторизацииПочтовогоСервиса Тогда
		МодульИнтеграцияПодсистемБТС = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияПодсистемБТС");
		МодульИнтеграцияПодсистемБТС.ПриОпределенииПубликацииОткрытойАвторизацииПочтовогоСервиса(Опубликована, СтандартнаяОбработка);
	КонецЕсли;
