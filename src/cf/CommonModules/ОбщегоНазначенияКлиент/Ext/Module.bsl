...
Функция ПроверитьПроведенностьДокументов(Знач Документы) Экспорт
	...
	Возврат ОбщегоНазначенияВызовСервера.ПроверитьПроведенностьДокументов(Документы); // АПК:222 вызываемая функция исключена из программного интерфейса.
...
Функция ПровестиДокументы(Документы) Экспорт
	...
	Возврат ОбщегоНазначенияВызовСервера.ПровестиДокументы(Документы);
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректное имя общего модуля ""%1"".'"), Имя);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецПопытки;
	
	Модуль = Вычислить(Имя); // Не нарушает безопасность информации, так как вычисляется только имя переменной.
...
#КонецЕсли
	...
	Возврат ТипПлатформыКлиента = ТипПлатформы.Linux_x86
		Или ТипПлатформыКлиента = ТипПлатформы.Linux_x86_64
		Или ТипПлатформыКлиента = ТипПлатформы["Linux_ARM64"]
...
Процедура ПоказатьВыборФорматаВложений(ОписаниеОповещения, НастройкиФормата = Неопределено, Владелец = Неопределено) Экспорт
	Если НастройкиФормата <> Неопределено Тогда
		...
		ПараметрыФормы.Вставить("УпаковатьВАрхив", НастройкиФормата.УпаковатьВАрхив);
		ПараметрыФормы.Вставить("ФорматыСохранения", НастройкиФормата.ФорматыСохранения);
		ПараметрыФормы.Вставить("ПереводитьИменаФайловВТранслит", НастройкиФормата.ПереводитьИменаФайловВТранслит);
		ПараметрыФормы.Вставить("Подписать", НастройкиФормата.Подписать);
	Иначе
...
#Область ПроцедурыИФункцииДляПодключенияИУстановкиВнешнихКомпонентИзМакетовКонфигурации
...
Функция ПараметрыПодключенияКомпоненты() Экспорт
	...
	Параметры.Вставить("ПоказыватьВопросУстановки", Истина);
...
Процедура УстановитьКомпонентуИзМакета(Оповещение, ПолноеИмяМакета, ПараметрыУстановки = Неопределено) Экспорт
	...
	Контекст.Оповещение = Оповещение;
	Контекст.Местоположение = ПолноеИмяМакета;
	Контекст.ТекстПояснения = Параметры.ТекстПояснения;
	Контекст.Идентификатор = ПолноеИмяМакета;
...
//                             ** Ключ - Строка - идентификатор внешней компоненты;
//                             ** Значение - ОбъектВнешнейКомпоненты - экземпляр объекта.
//    * Местоположение     - Строка - имя макета или навигационная ссылка компоненты из справочника, если она более
//                                    старшей версии, из которых была подключена компонента.
//    * СимволическоеИмя   - Строка - символическое имя, с которым была подключена компонента.
//         По местоположению и символическому имени из прикладного кода можно проверять подключение компоненты методом платформы
//         ПроверитьПодключениеВнешнейКомпоненты(Местоположение, Имя)
//    * ОписаниеОшибки     - Строка - краткое описание ошибки. При отмене пользователем пустая строка.
//
...
Функция ХранилищеСистемныхНастроекЗагрузить(КлючОбъекта, КлючНастроек, ЗначениеПоУмолчанию = Неопределено, 
	...
	Возврат ОбщегоНазначенияВызовСервера.ХранилищеСистемныхНастроекЗагрузить(
		КлючОбъекта,
		КлючНастроек,
		ЗначениеПоУмолчанию,
		ОписаниеНастроек,
...
Функция ХранилищеНастроекДанныхФормЗагрузить(КлючОбъекта, КлючНастроек, ЗначениеПоУмолчанию = Неопределено,
	...
	Возврат ОбщегоНазначенияВызовСервера.ХранилищеНастроекДанныхФормЗагрузить(
		КлючОбъекта,
		КлючНастроек,
		ЗначениеПоУмолчанию,
		ОписаниеНастроек,
...
#Если ТолстыйКлиентОбычноеПриложение Тогда
	// ПерейтиПоНавигационнойСсылке недоступен в толстом клиенте обычного приложения.
	Оповещение = Новый ОписаниеОповещения;
...
Процедура ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ОписаниеОповещенияОЗакрытии, 
		ТекстПредложения = "", ВозможноПродолжениеБезУстановки = Истина) Экспорт
	
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(ОписаниеОповещенияОЗакрытии, 
