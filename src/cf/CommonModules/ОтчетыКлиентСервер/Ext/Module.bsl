		...
		ИначеЕсли ТипЗнч(ЭлементАнализируемый) = Тип("ОтборКомпоновкиДанных")
			...
			Если ЗначениеЗаполнено(ЭлементАнализируемый.ИдентификаторПользовательскойНастройки) Тогда 
				...
				НайденныеЭлементы = Настройки.ПолучитьОсновныеНастройкиПоИдентификаторуПользовательскойНастройки(
					ЭлементАнализируемый.ИдентификаторПользовательскойНастройки);
				
				Если НайденныеЭлементы.Количество() > 0 Тогда 
					ЭлементыНастроекОтобранные(Настройки, Отбор, НайденныеЭлементы[0].Элементы, Результат);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция НайтиДоступноеПолеКД(НастройкиКД, ПолеКД)
	...
	Если ТипЗнч(НастройкиКД) = Тип("ГруппировкаКомпоновкиДанных") Тогда 
		ДоступнаяНастройка = НастройкиКД.Родитель.ДоступныеПоляОтбора.НайтиПоле(ПолеКД);
	Иначе
...
Функция ЗначениеПараметраВыбора(Настройки, ПользовательскиеНастройки, Поле, РежимИзмененияВарианта)
	...
	НайтиЗначенияПоляЭлементовОтбора(Поле, Настройки.Отбор.Элементы, ПользовательскиеНастройки, Значение, РежимИзмененияВарианта);
	...
	Для Каждого Элемент Из ЭлементыОтбора Цикл 
		...
		Иначе
			Если Элемент.ЛевоеЗначение = Поле
				...
				Значение = Элемент.ПравоеЗначение;
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
