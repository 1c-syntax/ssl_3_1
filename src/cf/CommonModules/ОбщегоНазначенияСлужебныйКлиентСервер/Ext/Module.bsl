...
//  Строка
//
Функция БезопаснаяСтрокаКоманды(КомандаЗапуска, Знач ЭтоАдминистратор = Ложь) Экспорт
	
	Если ТипЗнч(КомандаЗапуска) = Тип("Строка") Тогда
		ПроверитьСодержитНебезопасныеДействия(КомандаЗапуска, ЭтоАдминистратор);
		Возврат КомандаЗапуска;
	КонецЕсли;
	
	Если ТипЗнч(КомандаЗапуска) <> Тип("Массив") Тогда
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Для Каждого Элемент Из КомандаЗапуска Цикл
		ПроверитьСодержитНебезопасныеДействия(Элемент, ЭтоАдминистратор);
	КонецЦикла;
#Иначе
	ПроверитьСодержитНебезопасныеДействия(КомандаЗапуска[0], ЭтоАдминистратор);
#КонецЕсли
	
	Возврат МассивВСтрокуКоманды(КомандаЗапуска);
...
#Если МобильныйКлиент Тогда
	...
	СтрокаСообщенияОбОшибке = НСтр("ru = 'Подключение к приложению недоступно в мобильном клиенте.'");
...
Процедура ПроверитьСодержитНебезопасныеДействия(Знач КомандаЗапуска, ЭтоАдминистратор)
	
	ЕстьНебезопасныеДействия = СтрНайти(КомандаЗапуска, "${") <> 0
		Или СтрНайти(КомандаЗапуска, "$(") <> 0
		Или СтрНайти(КомандаЗапуска, "`") <> 0
		Или СтрНайти(КомандаЗапуска, "|") <> 0
		Или (СтрНайти(КомандаЗапуска, ";") <> 0 И Не ЭтоАдминистратор)
		Или СтрНайти(КомандаЗапуска, "&") <> 0;
	
	Если ЕстьНебезопасныеДействия Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось запустить приложение. Недопустимая строка команды:
			           |%1
			           |
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РасчетныеПоказателиЯчеек", Истина);
