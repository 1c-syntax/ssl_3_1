...
// Для вызова с клиента следует использовать ОбщегоНазначенияСлужебныйКлиент.БезопаснаяСтрокаКоманды,
// для вызова с сервера - ОбщегоНазначенияСлужебный.БезопаснаяСтрокаКоманды.
// 
// Параметры:
//  КомандаЗапуска - Строка
//  СредаИсполнения - Строка
// 
// Возвращаемое значение:
//  Строка
//
Функция БезопаснаяСтрокаКоманды(КомандаЗапуска, СредаИсполнения = "Windows") Экспорт
	
	Результат = "";
	
	Если ТипЗнч(КомандаЗапуска) = Тип("Строка") Тогда 
		
		Если СредаИсполнения = "Windows" И СодержитНебезопасныеДействия(КомандаЗапуска) Тогда 
	...
	ИначеЕсли ТипЗнч(КомандаЗапуска) = Тип("Массив") Тогда
		...
		Если КомандаЗапуска.Количество() > 0 Тогда
			...
			Если СредаИсполнения = "Windows" И СодержитНебезопасныеДействия(КомандаЗапуска[0]) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось запустить программу
				           |по причине:
