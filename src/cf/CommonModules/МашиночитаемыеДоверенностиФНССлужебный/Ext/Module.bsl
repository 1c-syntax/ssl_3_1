...
Функция РезультатПроверкиДанныхДоверенности(ДанныеДоверенности, ПроверятьВРеестреФНС = Неопределено, ИдентификаторФормы = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеДоверенности.НомерДоверенности)
		...
		РезультатПроверкиВРеестреФНС = ПолучитьЧастичныеДанныеДоверенностиМЧДРР(
...
Функция ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета,  ДополнительныеФайлы, ЗаголовокОшибки = "", ТекстОшибки = "") Экспорт
	Результат = Новый Структура;
...
Функция АвторизоватьсяНаСервереМЧДРР(ЦельАвторизации = "") Экспорт
	...
	Исключение
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, , ДополнительныеФайлы, ЗаголовокОшибки, ПредставлениеОшибки);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецПопытки;
	...
	Исключение
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, ЗаголовокОшибки, ПредставлениеОшибки);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецПопытки;
	...
	Если НЕ ЗначениеЗаполнено(Результат.ТокенДоступа) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, ЗаголовокОшибки, 
...
Функция ПолучитьНомерМЧДРР(ТокенДоступа = "") Экспорт
	...
	Исключение
		...
		ПараметрыФайловДляРасследования.Ответ = ОтветHTTP;
		ДополнительныеФайлы = ФайлыДляРасследования(ПараметрыФайловДляРасследования);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, , ДополнительныеФайлы, ЗаголовокОшибки, 
			ПредставлениеОшибки);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
...
Функция ПолучитьЧастичныеДанныеДоверенностиМЧДРР(НомерДоверенности, ТокенДоступа = "", ЦельПолучения = "") Экспорт
...
Функция ПолучитьПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ТокенДоступа = "", ЦельПолучения = "") Экспорт
...
Функция ЗагрузитьДоверенностьИзРеестра(НомерДоверенности, ИННДоверителя, ТокенДоступа = "") Экспорт
	...
	РезультатПолучения = ПолучитьПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ТокенДоступа, ЗаголовокОшибки);
