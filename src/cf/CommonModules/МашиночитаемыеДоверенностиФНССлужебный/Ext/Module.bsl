...
Функция ДанныеДоверенностиДляПечати(Доверенность, НаДату = Неопределено) Экспорт
	...
	Результат.ПредставлениеДоверенности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
			|%5действует с %2 по %3,
			|%5выдана от %4'"), РеквизитыДоверенности.НомерДоверенности, Формат(
		РеквизитыДоверенности.ДатаВыдачи, "ДЛФ=D"), Формат(РеквизитыДоверенности.ДатаОкончания, "ДЛФ=D"), Доверитель, Символы.Таб);
		
	Возврат Результат;
			
КонецФункции

Процедура ДобавитьПараметрыВыбораДоверенности(Форма, ОтборДоверенностей) Экспорт
	
	ПараметрыВыбораМассив = Новый Массив;
	ПараметрыВыбораМассив.Добавить(Новый ПараметрВыбора("ТолькоДействующие", Истина));

	Если ТипЗнч(ОтборДоверенностей) = Тип("Структура") Тогда
		СтруктураОтбора = Новый Структура("Организация, Полномочия");
		ЗаполнитьЗначенияСвойств(СтруктураОтбора, ОтборДоверенностей);
		
		Если ТипЗнч(СтруктураОтбора.Организация) = Тип("Строка") Тогда
			ПараметрыВыбораМассив.Добавить(Новый ПараметрВыбора("Доверитель", СтруктураОтбора.Организация));
		ИначеЕсли СтруктураОтбора.Организация <> Неопределено И ОписаниеТиповОрганизации().СодержитТип(ТипЗнч(
			СтруктураОтбора.Организация)) Тогда
			РеквизитыУчастника = РеквизитыУчастника(Неопределено, Неопределено, "Организация",
				СтруктураОтбора.Организация, Ложь);
			ПараметрыВыбораМассив.Добавить(Новый ПараметрВыбора("Доверитель", РеквизитыУчастника.ИНН));
		КонецЕсли;

		Если ТипЗнч(СтруктураОтбора.Полномочия) = Тип("Массив") И СтруктураОтбора.Полномочия.Количество() > 0 Тогда
			СписокЗначений = Новый СписокЗначений;
			Для Каждого ЭлементМассива Из СтруктураОтбора.Полномочия Цикл
				СписокЗначений.Добавить(ЭлементМассива);
			КонецЦикла;
			ПараметрыВыбораМассив.Добавить(Новый ПараметрВыбора("Полномочия", СписокЗначений));
		КонецЕсли;
		
		Форма.ОтборДоверенностей = ОтборДоверенностей;
	КонецЕсли;
	
	Форма.Элементы.МашиночитаемаяДоверенность.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораМассив);
	
КонецПроцедуры

Процедура ПодобратьДоверенностьВФормеПодписания(Форма, ДоверенностьНастройка) Экспорт
	
	ОтборДляДоверенностейПоСертификату = Новый Структура;
	
	Если Форма.ИмяФормы = "Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.ПодписаниеДанных"
		И Форма.ТипПодписанта = "ПодписатьСертификатом" Тогда
		СертификатКриптографии = Новый СертификатКриптографии(ПолучитьИзВременногоХранилища(Форма.СертификатАдрес));
		ОтборДляДоверенностейПоСертификату = МашиночитаемыеДоверенностиФНС.ОтборДляДоверенностейПоСертификату(
			СертификатКриптографии, "Представитель");
	// СтандартныеПодсистемы.СервисМобильнойПодписи
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		МодульСервисМобильнойПодписиСлужебный = ОбщегоНазначения.ОбщийМодуль("СервисМобильнойПодписиСлужебный");
		ОтборДляДоверенностейПоСертификату = МодульСервисМобильнойПодписиСлужебный.ОтборДляДоверенностейПоПодписанту(Форма.Подписант);
	// Конец СтандартныеПодсистемы.СервисМобильнойПодписи 
	КонецЕсли;
	
	Элементы = Форма.Элементы;
	
	Если ТипЗнч(ДоверенностьНастройка) = Тип("Структура") И ЗначениеЗаполнено(ДоверенностьНастройка.Доверенность) Тогда
		Элементы.МашиночитаемаяДоверенность.СписокВыбора.Добавить(ДоверенностьНастройка.Доверенность,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (Персональная настройка)'"), ДоверенностьНастройка.Доверенность));
		Форма.МашиночитаемаяДоверенность = ДоверенностьНастройка.Доверенность;
	КонецЕсли;
		
	ОтборДоверенностей = Форма.ОтборДоверенностей;

	Если ТипЗнч(ОтборДоверенностей) = Тип("Структура") Тогда
		СтруктураОтбора = Новый Структура("Организация, Полномочия");
		ЗаполнитьЗначенияСвойств(СтруктураОтбора, ОтборДоверенностей);
		Если ТипЗнч(СтруктураОтбора.Организация) = Тип("Строка") Тогда
			ОтборДляДоверенностейПоСертификату.Вставить("ДоверительИНН", СтруктураОтбора.Организация);
		ИначеЕсли СтруктураОтбора.Организация <> Неопределено И ОписаниеТиповОрганизации().СодержитТип(ТипЗнч(
			СтруктураОтбора.Организация)) Тогда
			РеквизитыУчастника = РеквизитыУчастника(Неопределено, Неопределено, "Организация",
				СтруктураОтбора.Организация, Ложь);
			ОтборДляДоверенностейПоСертификату.Вставить("ДоверительИНН", РеквизитыУчастника.ИНН);
		КонецЕсли;

		Если ТипЗнч(СтруктураОтбора.Полномочия) = Тип("Массив") И СтруктураОтбора.Полномочия.Количество() > 0 Тогда
			ОтборДляДоверенностейПоСертификату.Вставить("КодПолномочия", СтруктураОтбора.Полномочия);
		КонецЕсли;
	КонецЕсли;
		
	Если ОтборДляДоверенностейПоСертификату.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Элемент Из ОтборДляДоверенностейПоСертификату Цикл
	...
	Доверенности = МашиночитаемыеДоверенностиФНС.ДоверенностиСОтбором(ОтборДляДоверенностейПоСертификату, ВыбранныеПоля);
	
	Если Доверенности.Количество() > 0 Тогда
		Для Каждого Строка Из Доверенности Цикл
			Найдено = Элементы.МашиночитаемаяДоверенность.СписокВыбора.НайтиПоЗначению(
				Строка.МашиночитаемаяДоверенность);
			Если Найдено = Неопределено Тогда
				Элементы.МашиночитаемаяДоверенность.СписокВыбора.Добавить(Строка.МашиночитаемаяДоверенность, СтрШаблон(
				"%1, %2", Строка.Наименование, Формат(Строка.ДатаВыдачи, "ДЛФ=D")));
			КонецЕсли;
		КонецЦикла;
		Форма.МашиночитаемаяДоверенность = Элементы.МашиночитаемаяДоверенность.СписокВыбора[0].Значение;
	КонецЕсли;
	
КонецПроцедуры

Функция НастройкаПоДоверенностиДляОтбора(Форма) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.ПодписаниеДанных"
		...
		Иначе
			Подписант = Форма.Сертификат;
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(Форма.Подписант) Тогда
			Подписант = Неопределено;
		Иначе
			Подписант = Форма.Подписант;
		КонецЕсли;
	КонецЕсли;

	НастройкаПоДоверенности = НастройкаДоверенностиДляПодписанта(Подписант);

	Если НастройкаПоДоверенности = Неопределено Тогда
		Возврат НастройкаПоДоверенности;
	КонецЕсли;

	Возврат НастройкаПоДоверенности.Получить(КлючНастроекПоДоверенности(Форма.ОтборДоверенностей));
	
КонецФункции

Процедура ДобавитьНастройкуПоДоверенности(Форма, НастройкиДляСохранения, НастройкаПодписант) Экспорт
	
	НастройкаПоДоверенности = НастройкаДоверенностиДляПодписанта(НастройкаПодписант);
	Если НастройкаПоДоверенности = Неопределено Тогда
		НастройкаПоДоверенности = Новый Соответствие;
	КонецЕсли;
	НастройкаПоДоверенности.Вставить(КлючНастроекПоДоверенности(Форма.ОтборДоверенностей), 
		Новый Структура("ПоДоверенности, Доверенность", Форма.ПоДоверенности, 
			?(Форма.ПоДоверенности, Форма.МашиночитаемаяДоверенность, Неопределено)));
	НастройкиДляСохранения.Добавить(Новый Структура("Объект, Настройка, Значение", НастройкаПодписант,
		"ПоДоверенности", НастройкаПоДоверенности));
...
Функция НастройкаДоверенностиДляПодписанта(Подписант)
	...
	Если ТипЗнч(НастройкаПоДоверенности) = Тип("Булево") Тогда
		НоваяНастройкаПоДоверенности = Новый Соответствие;
		НоваяНастройкаПоДоверенности.Вставить(Неопределено, 
			Новый Структура("ПоДоверенности, Доверенность", НастройкаПоДоверенности));
		Возврат НоваяНастройкаПоДоверенности;
	КонецЕсли;
...
Функция КлючНастроекПоДоверенности(ОтборДоверенностей)
	
	Если ТипЗнч(ОтборДоверенностей) <> Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	СтруктураОтбора = Новый Структура("Организация, Полномочия, КлючОтбора");
	ЗаполнитьЗначенияСвойств(СтруктураОтбора, ОтборДоверенностей);
	...
	Для Каждого КлючИЗначение Из СтруктураОтбора Цикл
		...
		ИначеЕсли КлючИЗначение.Ключ = "Полномочия" И ТипЗнч(КлючИЗначение.Значение) = Тип("Массив") Тогда
			Значение = СтрСоединить(КлючИЗначение.Значение, ",");
		ИначеЕсли КлючИЗначение.Ключ = "Организация" 
			И ОписаниеТиповОрганизации().СодержитТип(ТипЗнч(КлючИЗначение.Значение)) Тогда
			Значение = Строка(КлючИЗначение.Значение.УникальныйИдентификатор());
		Иначе
			Продолжить;
		КонецЕсли;
		
		КлючОтбора.Вставить(КлючИЗначение.Ключ, Значение);
	КонецЦикла;
...
Процедура ПроверитьЗаполнениеРеквизита(Форма, Реквизит, Отказ, ЭтоИП, ЭтоФЛ)
	...
	ИначеЕсли ИмяРеквизита = "ЮридическийАдрес"
		...
		ИначеЕсли Форма.ИмяФормы <> "Справочник.МашиночитаемыеДоверенности.Форма.СведенияОбОрганизации" Тогда
			ПродолжитьПриОшибке = ТекстСообщения = НСтр("ru = 'Не указан номер дома'");
		КонецЕсли;
...
Функция ОшибкиПроверкиАдреса(Знач АдресЗначение)
	...
	Если АдресСтруктура.РезультатПроверкиАдреса <> "Успех" И АдресСтруктура.Свойство("ТипАдреса") Тогда
		Если АдресСтруктура.ТипАдреса = МодульРаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес()
		...
		ИначеЕсли Не ЗначениеЗаполнено(АдресСтруктура.КодРегиона) Тогда
			Возврат  НСтр("ru = 'Не заполнен Код региона'");
		КонецЕсли; 
	КонецЕсли;
...
Функция СоединениеССерверомИнтернета(URL, ПараметрыСоединения = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЗащищенноеСоединение = Неопределено И (ЗначениеЗаполнено(СтруктураURI.Логин) Или ЗначениеЗаполнено(СтруктураURI.Пароль)) Тогда
			ВызватьИсключение НСтр("ru = 'Недопустима передача логина и пароля без установки защищенного соединения.'");
		КонецЕсли;
...
Процедура ЗагрузитьКвитанцииИзРеестра(Доверенность, ОшибкаПриЗагрузке, УникальныйИдентификатор) Экспорт

	Отбор = Новый Структура("МашиночитаемаяДоверенность", Доверенность);
	Отбор.Вставить("Верна", Неопределено);
	Отбор.Вставить("Подписана", Неопределено);
	Отбор.Вставить("Статус", Неопределено);
	
	РеквизитыДоверенности = МашиночитаемыеДоверенностиФНС.ДоверенностиСОтбором(Отбор,
		"ПредставительИНН, ПредставительИННФЛ, ДоверительИНН, ДоверительИННФЛ, НомерДоверенности", Дата(1, 1, 1));

	Если РеквизитыДоверенности.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Не удалось получить реквизиты доверенности для загрузки. Загрузите доверенность из реестра ФНС вручную'");
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(НСтр("ru='Не удалось загрузить файлы доверенности'"), ТекстОшибки);
		ОшибкаПриЗагрузке = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат;
	КонецЕсли;

	СтрокаДоверенности = РеквизитыДоверенности[0];
	ИННДоверителя = ?(ЗначениеЗаполнено(СтрокаДоверенности.ДоверительИНН), СтрокаДоверенности.ДоверительИНН,
		СтрокаДоверенности.ДоверительИННФЛ);
	ИННПредставителя = ?(ЗначениеЗаполнено(СтрокаДоверенности.ПредставительИНН), СтрокаДоверенности.ПредставительИНН,
		СтрокаДоверенности.ПредставительИННФЛ);

	ПолныеДанныеДоверенностиМЧДРР = ПолныеДанныеДоверенностиМЧДРР(
		СтрокаДоверенности.НомерДоверенности, ИННДоверителя, ИННПредставителя,, НСтр(
		"ru='Не удалось загрузить файлы доверенности'"));

	Если ПолныеДанныеДоверенностиМЧДРР.Ошибка <> Неопределено Тогда
		ОшибкаПриЗагрузке = ПолныеДанныеДоверенностиМЧДРР.Ошибка;
		Возврат;
	КонецЕсли;
	...
	ОписаниеФайла.Вставить("Хранение", ПоместитьВоВременноеХранилище(ДвоичныеДанныеАрхива, УникальныйИдентификатор));

	ЗагруженныеФайлы = Новый Массив;
	ЗагруженныеФайлы.Добавить(ОписаниеФайла);
	ФайлыДоверенностейИПодписей = Справочники.МашиночитаемыеДоверенности.ФайлыДоверенностейИПодписей(ЗагруженныеФайлы,
		УникальныйИдентификатор);
	Если ФайлыДоверенностейИПодписей.ДругиеФайлы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ФайлыДоверенности = Квитанции(Доверенность).Выгрузить();

		Для Каждого КлючИЗначение Из ФайлыДоверенностейИПодписей.ДругиеФайлы Цикл
			ЗагруженныйФайл = КлючИЗначение.Ключ;
			ИмяФайла = ЗагруженныйФайл.ИмяФайла;
			АдресФайла = ЗагруженныйФайл.Хранение;
			
			Файл = Новый Файл(ИмяФайла);
			
			Найдено = ФайлыДоверенности.НайтиСтроки(Новый Структура("Наименование, Расширение",
				Файл.ИмяБезРасширения, СтрЗаменить(Файл.Расширение, ".", "")));
				
			Если Найдено.Количество() = 0 Тогда
				ПрисоединенныйФайл = Справочники.МашиночитаемыеДоверенности.ДобавитьПрисоединенныйФайл(Доверенность,
					ИмяФайла, АдресФайла);
			Иначе
				ПрисоединенныйФайл = Найдено[0].Ссылка;
				РаботаСФайлами.ОбновитьФайл(ПрисоединенныйФайл, 
					Новый Структура("АдресФайлаВоВременномХранилище, АдресВременногоХранилищаТекста", АдресФайла));
				Если Найдено[0].ПометкаУдаления Тогда
					ПрисоединенныйФайлОбъект = ПрисоединенныйФайл.ПолучитьОбъект();
					ПрисоединенныйФайлОбъект.ПометкаУдаления = Ложь;
					ПрисоединенныйФайлОбъект.Записать();
				КонецЕсли;
			КонецЕсли;

			ПодписиФайла = ФайлыДоверенностейИПодписей.ПодписиФайлов[ЗагруженныйФайл];
			Если ПодписиФайла = Неопределено Тогда
				Продолжить;
			КонецЕсли;

			УстановленныеПодписи = ЭлектроннаяПодпись.ПодписиОбъекта(ПрисоединенныйФайл);

			Если УстановленныеПодписи.Количество() = 0 Тогда
				ПодписиФайлаДляДобавления = ПодписиФайла;
			Иначе
				ПодписиФайлаДляДобавления = Новый Массив;
				Для Каждого Подпись Из ПодписиФайла Цикл
					ДанныеПодписи = ПолучитьИзВременногоХранилища(Подпись.Хранение);
					ПодписьВКодировкеDER = ЭлектроннаяПодпись.ПодписьВКодировкеDER(ДанныеПодписи);

					ПодписьДобавлена = Ложь;
					Для Каждого УстановленнаяПодпись Из УстановленныеПодписи Цикл
						Если ПодписьВКодировкеDER = УстановленнаяПодпись.Подпись Тогда
							ПодписьДобавлена = Истина;
							Прервать;
						КонецЕсли;
					КонецЦикла;
					Если Не ПодписьДобавлена Тогда
						ПодписиФайлаДляДобавления.Добавить(Подпись);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;

			Если ПодписиФайлаДляДобавления.Количество() > 0 Тогда
				Справочники.МашиночитаемыеДоверенности.ДобавитьПодписи(ПрисоединенныйФайл, ПодписиФайлаДляДобавления,
					Доверенность);
			КонецЕсли;
...
Функция Квитанции(Доверенность, ПометкаУдаления = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МашиночитаемыеДоверенностиПрисоединенныеФайлы.Ссылка КАК Ссылка,
	|	МашиночитаемыеДоверенностиПрисоединенныеФайлы.Представление КАК Представление,
	|	МашиночитаемыеДоверенностиПрисоединенныеФайлы.Расширение КАК Расширение,
	|	МашиночитаемыеДоверенностиПрисоединенныеФайлы.Наименование КАК Наименование,
	|	МашиночитаемыеДоверенностиПрисоединенныеФайлы.ПометкаУдаления КАК ПометкаУдаления,
	|	МашиночитаемыеДоверенностиПрисоединенныеФайлы.ИндексКартинки КАК ИндексКартинки
	|ИЗ
	|	Справочник.МашиночитаемыеДоверенности КАК МашиночитаемыеДоверенности
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МашиночитаемыеДоверенностиПрисоединенныеФайлы КАК МашиночитаемыеДоверенностиПрисоединенныеФайлы
	|		ПО МашиночитаемыеДоверенности.Ссылка = МашиночитаемыеДоверенностиПрисоединенныеФайлы.ВладелецФайла
	|			И (МашиночитаемыеДоверенностиПрисоединенныеФайлы.Ссылка <> МашиночитаемыеДоверенности.ФайлДоверенности)
	|ГДЕ
	|	МашиночитаемыеДоверенности.Ссылка = &Доверенность
	|	И &ПометкаУдаления";

	Запрос.УстановитьПараметр("Доверенность", Доверенность);
	Запрос.УстановитьПараметр("ПометкаУдаления", ПометкаУдаления);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПометкаУдаления", ?(ПометкаУдаления = Неопределено, "ИСТИНА",
		"МашиночитаемыеДоверенностиПрисоединенныеФайлы.ПометкаУдаления = &ПометкаУдаления"));
...
Функция ПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ИННПредставителя, ТокенДоступа = "", ЦельПолучения = "")
	...
	Если Не ЗначениеЗаполнено(Результат.СтатусПолучения) Тогда
		ТекстОшибки = НСтр("ru='Распределенный реестр ФНС России не предоставил полных данных доверенности и не сообщил о статусе запроса данных.'");
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Распределенный реестр ФНС России не предоставил полных данных доверенности, статус запроса данных: %1 (%2).'"),
			Результат.СтатусПолучения, СтатусТранзакцииПеречислением(Результат.СтатусПолучения));
	КонецЕсли;
	
	ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, ТекстОшибки);
	...
	Если ЗначениеЗаполнено(Результат.СтатусПолучения) Тогда
		Результат.Ошибка.Решение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru='Повторите действие позднее или скачайте архив доверенности с <a href=%1>сайта распределенного реестра ФНС России</a> и загрузите из файла с помощью команды в <a href=%2>списке машиночитаемых доверенностей</a>.
			|При необходимости обратитесь <a href=%3>службу поддержки ФНС России</a>.'"),
			"https://m4d.nalog.gov.ru/emchd/get-info",
			"e1cib/list/Справочник.МашиночитаемыеДоверенности",
			"https://www.nalog.gov.ru/rn77/service/service_feedback/?service=101");
	КонецЕсли;
...
Функция ФайлОтменыДоверенности(Доверенность, ПричинаОтмены) Экспорт
	...
	Если ДанныеДоверенности.Документ.Довер <> Неопределено Тогда
		...
		ИначеЕсли ДанныеДоверенности.Документ.Довер.СвДоверит[0].ТипДоверит = "3" Тогда
			...
			Иначе
				ЗаписьXML.ЗаписатьАтрибут("НаимИП", ФИОСтрокой(СведенияОДоверителе.СведФЛ.ФИО));
			КонецЕсли;
...
Функция ФИОСтрокой(ФИО) Экспорт
	...
	Если ЗначениеЗаполнено(Отчество) Тогда
		ФИОСтрокой = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 %2 %3'"),
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьПараметрСеансаПараметрыАвторизацииВРаспределенномРеестре");
	Методы.Вставить("ОбновитьИПрочитатьСтатусДоверенностей", Истина);
