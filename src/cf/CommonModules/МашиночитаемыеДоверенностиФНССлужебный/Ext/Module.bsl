...
Функция ЗагрузитьДоверенностьИзРеестра(НомерДоверенности, ИННДоверителя, ТокенДоступа = "") Экспорт
	...
	ПолныеДанныеДоверенностиМЧДРР = ПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ТокенДоступа, ЗаголовокОшибки);
	РезультатЗагрузкиИзРеестра.Ошибка = ПолныеДанныеДоверенностиМЧДРР.Ошибка;
	
	Если ЗначениеЗаполнено(ПолныеДанныеДоверенностиМЧДРР.Ошибка) Тогда
		Возврат РезультатЗагрузкиИзРеестра;
	КонецЕсли;
	
	Поток = ПолныеДанныеДоверенностиМЧДРР.ДанныеАрхива.ОткрытьПотокДляЧтения();
	...
	РезультатЗагрузкиВИнформационнуюБазу =
		Справочники.МашиночитаемыеДоверенности.ЗагрузитьДоверенностиВИнформационнуюБазу(ФайлыДляЗагрузки);
	
	Если ЗначениеЗаполнено(РезультатЗагрузкиВИнформационнуюБазу.ТекстОшибки) Тогда
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, РезультатЗагрузкиВИнформационнуюБазу.ТекстОшибки);
	...
	РегистрыСведений.МашиночитаемыеДоверенностиСтатусы.ОбновлениеСтатусовМЧД(РезультатЗагрузкиВИнформационнуюБазу.Доверенности);
	РезультатЗагрузкиИзРеестра.Доверенность = РезультатЗагрузкиВИнформационнуюБазу.Доверенности[0];
	РезультатЗагрузкиИзРеестра.ЦепочкаДоверенностей.Добавить(РезультатЗагрузкиИзРеестра.Доверенность);
	
	ПредыдущаяДоверенность = РезультатЗагрузкиВИнформационнуюБазу.ПредыдущиеДоверенности[РезультатЗагрузкиИзРеестра.Доверенность];
	
	Если ПредыдущаяДоверенность <> Неопределено Тогда
		НомерДоверенностиОснования = ПредыдущаяДоверенность.НомерДоверенности;
		ИННДоверителяДоверенностиОснования = ПредыдущаяДоверенность.ИННДоверителя;
		РезультатЗагрузкиОснования = ЗагрузитьДоверенностьИзРеестра(НомерДоверенностиОснования, ИННДоверителяДоверенностиОснования);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РезультатЗагрузкиИзРеестра.ЦепочкаДоверенностей, РезультатЗагрузкиОснования.ЦепочкаДоверенностей);
		
		Доверенность = Справочники.МашиночитаемыеДоверенности.НайтиДоверенность(НомерДоверенностиОснования);
		
		Если Доверенность <> Неопределено Тогда
			ДоверенностьОбъект = РезультатЗагрузкиИзРеестра.Доверенность.ПолучитьОбъект(); // СправочникОбъект.МашиночитаемыеДоверенности
			ДоверенностьОбъект.РодительскаяДоверенность = Доверенность;
			
			НачатьТранзакцию();
			Попытка
				Блокировка = Новый БлокировкаДанных;
				ЭлементБлокировки = Блокировка.Добавить("Справочник.МашиночитаемыеДоверенности");
				ЭлементБлокировки.УстановитьЗначение("Ссылка", РезультатЗагрузкиИзРеестра.Доверенность);
...
Функция ПолучитьСтруктуруОтвета(ОтветHTTP)
	...
	ИначеЕсли ОтветHTTP.Заголовки["Content-Type"] = "application/xml;charset=UTF-8" Тогда
		СтруктураОтвета = Новый Структура("title", ОтветHTTP.ПолучитьТелоКакСтроку());
	Иначе
