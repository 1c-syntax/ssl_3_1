...
Процедура ЗаполнитьПротоколПроверки(Форма, ГруппаПротоколПроверки, Знач РезультатПроверкиПодписиПоМЧД) Экспорт
	...
	Для Каждого Результат Из МассивРезультатов Цикл // см. МашиночитаемыеДоверенностиФНССлужебныйКлиентСервер.РезультатПроверкиПодписиПоМЧД
		...
		Для Каждого ПолеВывода Из ОчередностьВывода Цикл
			...
			Если ЗначениеЗаполнено(Проверка.ТекстПроверки) И Не Результат.СтатусВернаУстановленВручную Тогда
				Элемент = Форма.Элементы.Добавить("ТекстПроверки" + ПолеВывода + Индекс, Тип("ДекорацияФормы"), ГруппаПроверкаЗаголовок);
			...
			Иначе
				...
				Элемент.Вид = ВидДекорацииФормы.Надпись;
				Элемент.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(Проверка.ЗаголовокПроверки);
				Элемент.АвтоМаксимальнаяШирина = Ложь;
			КонецЕсли;
...
Процедура ДобавитьМашиночитаемуюДоверенностьПодписи(ПодписанныйОбъект, ИдентификаторПодписи, РезультатПроверкиПодписиПоМЧД) Экспорт
	...
	Для Каждого Результат Из МассивРезультатов Цикл
		...
		Если ЗначениеЗаполнено(РезультатПроверкиПодписиПоМЧД.ПротоколПроверки) Тогда
			НоваяЗапись.ПротоколПроверки = Новый ХранилищеЗначения(РезультатПроверкиПодписиПоМЧД.ПротоколПроверки);
		КонецЕсли;
...
Функция РезультатПроверкиПодписиПоМЧД(Доверенность, ПодписанныйОбъект, Сертификат, НаДату = Неопределено, Знач ГотовыеПроверки = Неопределено) Экспорт
	...
	РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Доверенность,
		"ФайлДоверенности, РегистрироватьВРеестре, НомерДоверенности, Статус, Ссылка, Верна, СтатусВернаУстановленВручную, УстановившийСтатусВерна, Полномочия, Ограничения, СовместныеПолномочия");
	РеквизитыДоверенности.Вставить("ПодписанныйОбъект", ПодписанныйОбъект);
		
	Возврат РезультатПроверкиПодписиПоДаннымМЧД(РеквизитыДоверенности, Сертификат, НаДату, ГотовыеПроверки);
...
Функция РезультатПроверкиДоверенности(Доверенность, ПроверятьВРеестреФНС = Неопределено, ИдентификаторФормы = Неопределено) Экспорт
	...
	РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Доверенность,
		"ФайлДоверенности, РегистрироватьВРеестре, НомерДоверенности, Статус, Ссылка, Верна, СтатусВернаУстановленВручную, УстановившийСтатусВерна");

	Результат = РезультатПроверкиДанныхДоверенности(РеквизитыДоверенности, ПроверятьВРеестреФНС, ИдентификаторФормы);
...
Функция ПолучитьЧастичныеДанныеДоверенностиМЧДРР(НомерДоверенности, ТокенДоступа = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат.ХешДоверенности) И НЕ ЗначениеЗаполнено(Результат.НомерДоверенности) Тогда
		...
		ПараметрыФайловДляРасследования.СоединениеHTTP = СоединениеHTTP;
		ДополнительныеФайлы = ФайлыДляРасследования(ПараметрыФайловДляРасследования);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета , ДополнительныеФайлы, НСтр("ru='Не удается получить частичные данные доверенности'"), 
...
Функция ОтменитьМЧДРР(ИмяФайлаВыгрузки, ДанныеИлиАдресВыгрузки, ДанныеИлиАдресПодписи, Доверенность, ТокенДоступа = "") Экспорт
	...
	Исключение
		...
		ПараметрыФайловДляРасследования.СоединениеHTTP = СоединениеHTTP;
		ДополнительныеФайлы = ФайлыДляРасследования(ПараметрыФайловДляРасследования);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, , ДополнительныеФайлы, НСтр("ru='Не удалось разобрать ответ при отмене доверенности на сервере МЧД'"), ПредставлениеОшибки);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецПопытки;
	...
	Если НЕ ЗначениеЗаполнено(Результат.ИдентификаторТранзакции) Тогда
		...
		ПараметрыФайловДляРасследования.СоединениеHTTP = СоединениеHTTP;
		ДополнительныеФайлы = ФайлыДляРасследования(ПараметрыФайловДляРасследования);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, НСтр("ru='Сервер МЧД не вернул номер транзакции отмены'"), ПредставлениеОшибки);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецЕсли;    
...
Функция ПолучитьФайлОтменыДоверенности(Доверенность, ПричинаОтмены) Экспорт
	...
	Результат.Вставить("ИмяФайла", "revoke.xml");
