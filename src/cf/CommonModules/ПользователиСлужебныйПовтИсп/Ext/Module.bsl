...
Функция ВсеРоли() Экспорт
	...
	Для каждого Роль Из Метаданные.Роли Цикл
		ИмяРоли = Роль.Имя;
		ПредставлениеРоли = Роль.Представление();
		
		Массив.Добавить(ИмяРоли);
		Соответствие.Вставить(ИмяРоли, ПредставлениеРоли);
		Таблица.Добавить().Имя = ИмяРоли;
		Список.Добавить(ИмяРоли, ПредставлениеРоли);
	КонецЦикла;
	...
	ВсеРоли.Вставить("Таблица",      Новый ХранилищеЗначения(Таблица, Новый СжатиеДанных(9)));
	ВсеРоли.Вставить("Список",       Новый ХранилищеЗначения(Список, Новый СжатиеДанных(9)));
...
Функция ДеревоРолей(ПоПодсистемам = Истина, Назначение = "ДляПользователей") Экспорт
	...
	Возврат Новый ХранилищеЗначения(Дерево, Новый СжатиеДанных(9));
...
Функция РолиРасширений() Экспорт
	...
	Для Каждого Роль Из Метаданные.Роли Цикл
		Если Роль.РасширениеКонфигурации() = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Результат.Вставить(Роль.Имя, Истина);
	КонецЦикла;
