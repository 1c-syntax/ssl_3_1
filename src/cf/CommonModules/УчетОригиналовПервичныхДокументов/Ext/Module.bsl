...
Процедура ПриСозданииНаСервере_ФормаСписка(Форма, Список, Расположение = Неопределено) Экспорт
	...
	РеквизитСписокСостояние.Заголовок = НСтр("ru='Состояние оригинал получен'");
...
Процедура ПриПолученииДанныхНаСервере(СтрокиСписка) Экспорт
	...
	Для Каждого СтрокаСписка Из СтрокиСписка Цикл
		...
		Если СтрокаСписка.Оформление.Получить("СостояниеОригиналаПервичногоДокумента") = Неопределено 
			Или СтрокаСписка.Оформление.Получить("СостояниеОригиналПолучен") = Неопределено Тогда
			Возврат;
		КонецЕсли;
		Прервать;
	КонецЦикла;
	...
	Для Каждого СтрокаСписка Из СтрокиСписка Цикл
		СтрокаСписка = СтрокиСписка[СтрокаСписка.Ключ];
		Выборка.Сбросить();
		Если Выборка.НайтиСледующий(СтрокаСписка.Данные["Ссылка"], "Ссылка") Тогда 
			СтрокаСписка.Данные["СостояниеОригиналаПервичногоДокумента"] = Выборка.СостояниеОригиналаПервичногоДокумента;
			СтрокаСписка.Оформление["СостояниеОригиналаПервичногоДокумента"].УстановитьЗначениеПараметра("ЦветТекста", 
				ЦветаСтиля.ГиперссылкаЦвет); 
			СтрокаСписка.Данные["СостояниеОригиналПолучен"] = Выборка.СостояниеОригиналПолучен;
		Иначе
			СтрокаСписка.Данные["СостояниеОригиналаПервичногоДокумента"] = НСтр("ru = '<Неизвестно>'");
			СтрокаСписка.Оформление["СостояниеОригиналаПервичногоДокумента"].УстановитьЗначениеПараметра("ЦветТекста", 
