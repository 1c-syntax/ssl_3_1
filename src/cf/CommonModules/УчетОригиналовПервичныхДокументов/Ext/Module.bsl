...
Функция УстановитьНовоеСостояниеОригиналаМассив(Знач ДанныеЗаписи, Знач ИмяСостояния)
	...
	Для Каждого Запись Из ДанныеЗаписи Цикл
		...
		Если ТабличнаяЧасть <> "" Тогда
			ИзмененыСостояния = ЗаписатьСостояниеОригиналаДокументаПоСотрудникам(НаборЗаписей, Запись.Ссылка, ТабличнаяЧасть, ИмяСостояния);
		Иначе
			ИзмененыСостояния = ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(НаборЗаписей, Запись.Ссылка, ИмяСостояния);
		КонецЕсли;
...
Функция УстановитьНовоеСостояниеОригиналаДокумента(Знач Документ, Знач ИмяСостояния)
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Если ТабличнаяЧасть <> "" Тогда
			ИзмененыСостояния = ЗаписатьСостояниеОригиналаДокументаПоСотрудникам(НаборЗаписей, Документ, ТабличнаяЧасть, ИмяСостояния);
		Иначе
			ИзмененыСостояния = ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(НаборЗаписей, Документ, ИмяСостояния);
		КонецЕсли;
	КонецЕсли; 
