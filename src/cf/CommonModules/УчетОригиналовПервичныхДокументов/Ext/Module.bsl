...
//               - см. УстановитьНовоеСостояниеОригиналаМассивСтруктура.ДанныеЗаписи.
...
Процедура ЗаполнитьСписокВыбораСостоянийОригинала(Форма, СостоянияОригиналов)
	...
	ПредставлениеСостояния = Новый Структура;
	
	Для каждого Состояние Из СостоянияОригиналов Цикл
		ПредставлениеСостояния.Вставить("Состояние",Состояние.Наименование);

		Представление = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru='[Состояние]'"), ПредставлениеСостояния);
		Если Состояние.Ссылка = Справочники.СостоянияОригиналовПервичныхДокументов.ОригиналПолучен Тогда 
			СписокВыбораСостоянийОригинала.Добавить(Состояние.Наименование,Представление,,БиблиотекаКартинок.СостояниеОригиналаПервичногоДокументаОригиналПолучен);
		ИначеЕсли Состояние.Ссылка = Справочники.СостоянияОригиналовПервичныхДокументов.ФормаНапечатана Тогда
			СписокВыбораСостоянийОригинала.Добавить(Состояние.Наименование,Представление,,БиблиотекаКартинок.СостояниеОригиналаПервичногоДокументаОригиналНеПолучен);
		Иначе
			СписокВыбораСостоянийОригинала.Добавить(Состояние.Наименование,Представление);
		КонецЕсли;
...
Функция КлючЗаписиОбщегоСостояния(ДокументСсылка) Экспорт
	...
	Для Каждого Ключ Из Выборка Цикл
		ПередаваемыеПараметры = Новый Структура("Владелец, ПервичныйДокумент, ОбщееСостояние, ФормаИзвне");
