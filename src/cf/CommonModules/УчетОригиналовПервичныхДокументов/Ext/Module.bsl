...
Процедура ПриСозданииНаСервере_ФормаДокумента(Форма, Расположение = Неопределено) Экспорт
	...
	Если Настройки.ВыводитьКнопкиНаФормеДокумента Тогда 
		Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
			ВывестиНаФормуКомандыСостоянияОригинала(Форма, Неопределено, ИспользуемыеСостояния());
		КонецЕсли;
		НастроитьКнопкиНаФормеДокумента(Форма);
	Иначе
		НастроитьГиперссылкуНаФормеДокумента(Форма, Расположение);
	КонецЕсли;
...
Функция УстановитьНовоеСостояниеОригинала(Знач ОбъектыЗаписи, Знач СостояниеОригинала) Экспорт
	...
	Если ТипЗнч(ОбъектыЗаписи) = Тип("Массив") Тогда 
		Для Каждого Объект Из ОбъектыЗаписи Цикл
			Ссылка = Объект.Ссылка;
			Если ЭтоОбъектУчета(Ссылка) Тогда
				МассивСсылок.Добавить(Ссылка);
				МассивЗаписи.Добавить(Объект);
			КонецЕсли;
		КонецЦикла;
	Иначе
		МассивСсылок.Добавить(ОбъектыЗаписи);
		МассивЗаписи = ОбъектыЗаписи;
	КонецЕсли;
	...
	Если ТипЗнч(ОбъектыЗаписи) = Тип("Массив") Тогда
		 Результат = УстановитьНовоеСостояниеОригиналаМассив(МассивЗаписи, СостояниеОригинала);
	Иначе
		 Результат = УстановитьНовоеСостояниеОригиналаДокумента(МассивЗаписи, СостояниеОригинала);
	КонецЕсли;
...
Функция СведенияОСостоянииОригиналаПоСсылке(Документ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		СведенияОСостоянии.Вставить("Ссылка", Документ);
...
Процедура ЗаписатьСостоянияОригиналовПослеПечати(ОбъектыПечати, СписокПечати, Записано = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетОригиналовПервичныхДокументов") И Не Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
		ПриОпределенииСпискаПечатныхФорм(ОбъектыПечати, СписокПечати);
		Если СписокПечати.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		ЗаписатьСостоянияОригиналовДокументаПослеПечатиФормы(ОбъектыПечати, СписокПечати, Записано);
	КонецЕсли;
...
Процедура ВывестиНаФормуКомандыСостоянияОригинала(Форма, Список, СостоянияОригиналов) Экспорт
	...
	Родитель = ?(ТипЗнч(Список) = Тип("ТаблицаФормы"), Список.КоманднаяПанель, Форма.КоманднаяПанель);
	Картинка = ?(ТипЗнч(Список) = Тип("ТаблицаФормы"), БиблиотекаКартинок.УстановитьСостояниеОригиналаПервичногоДокумента,
	...
	Если Элементы.Найти("ПодменюУстановитьНастроитьСостояниеОригинала") = Неопределено Тогда
		ПодменюУстановитьНастроитьСостояниеОригинала = Элементы.Добавить("ПодменюУстановитьНастроитьСостояниеОригинала", Тип("ГруппаФормы"), Родитель);
		...
		ПодменюУстановитьНастроитьСостояниеОригинала.Картинка  = Картинка;
	...
	Если Элементы.Найти("ГруппаУстановитьОригиналПолучен") = Неопределено И Не ТипЗнч(Список) = Тип("Булево") Тогда
	...
	Для Каждого Команда Из ГруппаУстановитьСостояниеОригинала.ПодчиненныеЭлементы Цикл
		МассивИменКоманд.Добавить(Команда.ИмяКоманды);
	КонецЦикла;
	
	Для Каждого Команда Из МассивИменКоманд Цикл
		НайденнаяКоманда = Форма.Команды.Найти(Команда);
		НайденнаяКнопка = Форма.Элементы.Найти(Команда);
	...
	// Добавляет кнопку для уточнения состояния оригинала по печатным формам.
	Если ТипЗнч(Список) = Тип("Булево") Тогда
		ИмяКоманды = "УточнитьПоПечатнымФормам";
		ИмяКнопки = НСтр("ru = 'Уточнить по печатным формам...'");
		...
		Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
			...
			КнопкаНастроитьСостояния = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"),ГруппаУстановитьСостояниеОригинала);
			КнопкаНастроитьСостояния.Заголовок = ИмяКнопки;
			КнопкаНастроитьСостояния.ИмяКоманды = ИмяКоманды;
			КнопкаНастроитьСостояния.Картинка = БиблиотекаКартинок.УточнитьСостояниеОригиналаПервичногоДокументаПоПечатнымФормам;
		КонецЕсли; 
		
	КонецЕсли;
	
	// Добавляет кнопку для перехода к настройкам состояний в подменю командой панели списка "Установить состояние".
	Если ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Справочники.СостоянияОригиналовПервичныхДокументов) Тогда
	...
	ИмяКоманды = "УстановкаСостоянияОригиналПолучен";
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено И Не ТипЗнч(Список) = Тип("Булево") Тогда
		КомандаФормы  = Форма.Команды.Добавить(ИмяКоманды);
		КомандаФормы.Действие = "Подключаемый_УстановитьСостояниеОригинала";
...
Функция ПредставлениеГиперссылкиСостояния(Документ) Экспорт
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		Возврат ТекущиеСостояниеОригинала;
	КонецЕсли;
	
	ТекущиеСостояниеОригинала = СведенияОСостоянииОригиналаПоСсылке(Документ);
...
Процедура ЗаписатьСостоянияОригиналовДокументаПослеПечатиФормы(ОбъектыПечати, ПечатныеФормы, Записано = Ложь) Экспорт
	...
	Попытка
		...
		Для Каждого Документ Из ОбъектыПечати Цикл
			Если ЭтоОбъектУчета(Документ.Значение) Тогда 
		...
		Для Каждого Документ Из ОбъектыПечати Цикл
			Если ЭтоОбъектУчета(Документ.Значение) Тогда 
				ТЧ = ТабличнаяЧастьССотрудниками(Документ.Значение);
				Если ТЧ <> "" Тогда
...
Процедура ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(Документ, ПечатнаяФорма, Представление, Состояние, 
	...
	Если ЗначениеЗаполнено(Сотрудник) Тогда
		...
		Если ПредставлениеСотрудник = 0 Тогда
			ЗаписьСостоянияОригинала.ПервичныйДокументПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 %2'"), Представление, ФИО);
		Иначе
...
Процедура ЗаписатьОбщееСостояниеОригиналаДокумента(Документ, Состояние) Экспорт
	...
	Если ПроверкаЗаписьСостоянияОригинала.Количество() Тогда
		...
		Для Каждого Запись Из ПроверкаЗаписьСостоянияОригинала Цикл
			...
			Иначе
				ЗаписьСостоянияОригинала.АвторИзменения = Пользователи.ТекущийПользователь();
			КонецЕсли;
			Если Запись.Состояние = СостояниеОригинала Тогда
				Продолжить;
			КонецЕсли;
			СостояниеФормОдинаково = Ложь;
		КонецЦикла;
		Если СостояниеФормОдинаково Тогда
	...
	ИнтеграцияПодсистемБСП.ПриИзмененииОбщегоСостоянияОригинала(Документ, СостояниеОригинала);
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	ИнтеграцияПодсистемБСП.ПриОпределенииОбъектовСКомандамиУчетаОригиналов(ОбъектыСУчетомОригиналовПервичныхДокументов);
	...
	Если Не ВыводитьКоманды И Не (НастройкиФормы.ЭтоФормаОбъекта И Настройки.ВыводитьКнопкиНаФормеДокумента) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Состояние Из СостоянияОригиналов Цикл
		...
		Команда.ДополнительныеПараметры.Вставить("СсылкаНаСостояние", Состояние.Ссылка); 
		
		Порядок = Порядок + 1;
	КонецЦикла;
	
	// Команда для уточнения состояния оригинала по печатным формам	
	Команда = Команды.Добавить();
	Команда.Вид = "УстановкаСостоянияОригинала";
	Команда.Порядок = Порядок; 
	Команда.Идентификатор = "УточнитьПоПечатнымФормам";
	Команда.Представление = НСтр("ru = 'Уточнить по печатным формам...'");
	Команда.Картинка = БиблиотекаКартинок.УточнитьСостояниеОригиналаПервичногоДокументаПоПечатнымФормам;
	...
	Команда.Идентификатор = "УстановкаСостоянияОригиналПолучен";
	Команда.Представление = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = 'Установить состояние ""[Наименование]""'"),
	...
	Команда.ФункциональныеОпции = "ИспользоватьУчетОригиналовПервичныхДокументов";
	Команда.Обработчик = "УчетОригиналовПервичныхДокументовКлиент.Подключаемый_УстановитьСостояниеОригинала";	
	Команда.ДополнительныеПараметры.Вставить("СсылкаНаСостояние", Справочники.СостоянияОригиналовПервичныхДокументов.ОригиналПолучен);
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьУчетОригиналовПервичныхДокументов") 
		Или Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияОригиналовПервичныхДокументов) Тогда
		Возврат;
	КонецЕсли;	
	...
	Параметры.Вставить("УчетОригиналовПервичныхДокументов", Новый ФиксированнаяСтруктура(
		НастройкиПодсистемы));
...
Функция НастройкиПодсистемы() Экспорт
	...
	Настройки.Вставить("ВыводитьКнопкиНаФормеДокумента", Ложь);
	Настройки.Вставить("ОтображатьПодсказкуВФормеИзмененияСостояний", Истина);
	Настройки.Вставить("ОткрыватьВыпадающиеМенюПоГиперссылке", Истина);
	
	ИнтеграцияПодсистемБСП.ПриОпределенииНастроекУчетаОригиналов(Настройки);
...
Процедура НастроитьГиперссылкуНаФормеДокумента(Форма, Расположение)
...
Процедура НастроитьКнопкиНаФормеДокумента(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда
		Сведения = СведенияОСостоянииОригиналаПоСсылке(Форма.Объект.Ссылка);
		Если Не Сведения.Количество() = 0 Тогда 
			ТекущиеСостояниеОригинала = Сведения.СостояниеОригиналаПервичногоДокумента;
			Картинка = ?(ТекущиеСостояниеОригинала = Справочники.СостоянияОригиналовПервичныхДокументов.ОригиналПолучен,
...
//                 * ОбщееСостояние    - Булево - Истина, если текущее состояние является общим.
//                 * Ссылка 		   - ОпределяемыйТип.ОбъектСУчетомОригиналовПервичныхДокументов - документ, для 
//																		которого необходимо изменить состояние оригинала.
//                 * СостояниеОригиналаПервичногоДокумента - СправочникСсылка.СостоянияОригиналовПервичныхДокументов -
//                                                           текущие состояние оригинала первичного документа.
//                 * ПервичныйДокумент - Строка - идентификатор первичного документа. 
//                                                Задается, если данное состояние не является общим.
...
//
Функция УстановитьНовоеСостояниеОригиналаМассив(Знач ОбъектыЗаписи, Знач СостояниеОригинала)
	...
	Попытка
		...
		Для Каждого Запись Из ОбъектыЗаписи Цикл
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СостоянияОригиналовПервичныхДокументов");
		...
		Для Каждого Запись Из ОбъектыЗаписи Цикл
			...
			Если Запись.СостояниеОригиналаПервичногоДокумента = СостояниеОригинала Тогда
				Продолжить;
			КонецЕсли;
			...
			Если Запись.ОбщееСостояние Тогда
				...
				Если НаборЗаписей.Количество() = 0 Тогда
					Изменено = Истина;
					ЗаписатьОбщееСостояниеОригиналаДокумента(Запись.Ссылка, СостояниеОригинала);
					Продолжить;
				КонецЕсли;
					
				Для Каждого ПрошлаяЗапись Из НаборЗаписей Цикл
					Если ПрошлаяЗапись.Состояние = СостояниеОригинала Тогда
						Продолжить;
					КонецЕсли; 
					Изменено = Истина;
					...
					ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(Запись.Ссылка,
						ПрошлаяЗапись.ПервичныйДокумент, ПрошлаяЗапись.ПервичныйДокументПредставление, СостояниеОригинала,
			...
			Если ТабличнаяЧасть <> "" Тогда
				ИзмененыСостояния = ЗаписатьСостояниеОригиналаПоСотрудникам(НаборЗаписей, Запись.Ссылка, 
					ТабличнаяЧасть, СостояниеОригинала);
			Иначе
				ИзмененыСостояния = ЗаписатьСостояниеОригиналаПоПечатнымФормам(НаборЗаписей, Запись.Ссылка, 
					СостояниеОригинала);
			КонецЕсли;
...
// Параметры:
//   Документ - ОпределяемыйТип.ОбъектСУчетомОригиналовПервичныхДокументов - документ, для которого необходимо изменить 
//																			 состояние оригинала.
//   СостояниеОригинала 	- СправочникСсылка.СостоянияОригиналовПервичныхДокументов - ссылка на устанавливаемое состояние.
//
// Возвращаемое значение:
//   Булево - Истина, если состояние оригинала документа не повторяется и было записано.
//
Функция УстановитьНовоеСостояниеОригиналаДокумента(Знач Документ, Знач СостояниеОригинала)
	...
	Попытка
		...
		Если НаборЗаписей.Количество() И НаборЗаписей[0].Состояние = СостояниеОригинала Тогда
			ЗафиксироватьТранзакцию();
			Возврат Ложь;
		КонецЕсли;
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			Если ТабличнаяЧасть <> "" Тогда
				ЗаписатьСостояниеОригиналаПоСотрудникам(НаборЗаписей, Документ, ТабличнаяЧасть, СостояниеОригинала);
			Иначе
				ЗаписатьСостояниеОригиналаПоПечатнымФормам(НаборЗаписей, Документ, СостояниеОригинала);
			КонецЕсли;
		КонецЕсли;
		
		ЗаписатьОбщееСостояниеОригиналаДокумента(Документ, 
			СостояниеОригинала);
...
Функция ЗаписатьСостояниеОригиналаПоПечатнымФормам(НаборЗаписей, Документ, СостояниеОригинала)
	...
	Для Каждого ПрошлаяЗапись Из НаборЗаписей Цикл
		...
		КонецЕсли;
		Изменено = Истина;
		ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(Документ, ПрошлаяЗапись.ПервичныйДокумент, 
			ПрошлаяЗапись.ПервичныйДокументПредставление, СостояниеОригинала, ПрошлаяЗапись.ФормаИзвне);
	КонецЦикла;
	Возврат Изменено;
	
КонецФункции

Функция ЗаписатьСостояниеОригиналаПоСотрудникам(НаборЗаписей, Документ, ТабличнаяЧасть, СостояниеОригинала)
	...
	Для Каждого Сотрудник Из Документ[ТабличнаяЧасть] Цикл
		...
		Для Каждого ПрошлаяЗапись Из НаборЗаписей Цикл
			...
			Изменено = Истина;
			ЗаписатьСостояниеОригиналаДокументаПоПечатнымФормам(Документ, ПрошлаяЗапись.ПервичныйДокумент, 
				ПрошлаяЗапись.ПервичныйДокументПредставление, СостояниеОригинала, ПрошлаяЗапись.ФормаИзвне, Сотрудник.Сотрудник);
		КонецЦикла;
	КонецЦикла;
	Возврат Изменено;
...
//
// Параметры:
//  ДокументСсылка - ОпределяемыйТип.ОбъектСУчетомОригиналовПервичныхДокументов - ссылка на документ, который необходимо
//                                                                                проверить.
//
// Возвращаемое значение:
//   Булево
...
Функция ТабличнаяЧастьССотрудниками(ДокументСсылка) Экспорт
	...
	Результат = "";
	ИнтеграцияПодсистемБСП.ПриОпределенииМногосотрудниковыхДокументов(СписокОбъектов);
...
Функция СсылкаПоШтрихкодуТабличногоДокумента(Штрихкод, Менеджеры = Неопределено) 
	...
	Иначе
		...
		Для Каждого ПустаяСсылка Из Менеджеры Цикл
			...
			Иначе
				ТекстИсключения = НСтр("ru = 'Неизвестный тип штрихкода ""%Тип%"".'");
...
//
// Параметры:
//  ДокументСсылка - ОпределяемыйТип.ОбъектСУчетомОригиналовПервичныхДокументов - ссылка на документ, у которого 
//																			необходимо проверить состояния печатных форм.
//  СостояниеОригинала 	- СправочникСсылка.СостоянияОригиналовПервичныхДокументов - ссылка на проверяемое состояние.
//
// Возвращаемое значение:
//   Булево - Истина, если все печатные формы документа имеют одинаковое состояние.
//
Функция СостояниеПечатныхФормОдинаково(ДокументСсылка, СостояниеОригинала) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	СостоянияОригиналовПервичныхДокументов.Владелец = &Ссылка
	               |	И СостоянияОригиналовПервичныхДокументов.Состояние <> &СостояниеОригинала
	               |	И НЕ СостоянияОригиналовПервичныхДокументов.ОбщееСостояние";
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("СостояниеОригинала", СостояниеОригинала);
...
Функция ТаблицаУчетаОригиналов()
	...
	ИнтеграцияПодсистемБСП.ПриЗаполненииТаблицыУчетаОригиналов(Таблица);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьСостоянияОригиналаПервичногоДокумента");
