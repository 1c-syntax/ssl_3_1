...
Процедура ОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении = Неопределено, 
	...
	Если ДлительнаяОперация.Статус <> "Выполняется" Тогда
		...
		ОбработатьНакопленныеСерверныеОповещения(РасширенныеПараметры.ИдентификаторЗадания, Ложь);
		...
		Иначе
			Операция = Новый Структура(Новый ФиксированнаяСтруктура(ДлительнаяОперация));
			Операция.Вставить("Прогресс");
			Операция.Вставить("ФоновоеЗаданиеЗавершено");
			ОбработатьРезультатАктивнойОперации(РасширенныеПараметры, Операция);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		РасширенныеПараметры.Вставить("ПолученоОповещениеОЗавершении", Ложь);
		...
		ОбработатьНакопленныеСерверныеОповещения(РасширенныеПараметры.ИдентификаторЗадания);
	КонецЕсли;
...
//
//   * Сообщения - ФиксированныйМассив из СообщениеПользователю - сообщения,
//                   сформированные в процедуре-обработчике длительной операции.
...
//  ОбсужденияАктивны - Булево - для доставки сообщений используется Система взаимодействия.
...
Процедура ПриЗакрытииФормыДлительнаяОперация(Результат, Контекст) Экспорт
	...
	Если Контекст.ОповещениеОЗавершении = Неопределено
	 Или Контекст.Результат <> Неопределено
	   И Контекст.Результат.Статус = "Выполняется" Тогда
...
Функция ПараметрыПроверкиДлительныхОпераций(ОбсужденияАктивны, Интервал)
	...
	ОстальныеАктивные = Новый Массив;
	...
	Для Каждого ДлительнаяОперация Из АктивныеДлительныеОперации Цикл
		...
		Иначе
			ДатаКонтроля = ДлительнаяОперация.Контроль
				+ ?(Не ОбсужденияАктивны
					Или ДлительнаяОперация.ПолученоОповещениеОЗавершении
					Или ДлительнаяОперация.ТекущийИнтервал > ИнтервалКонтроляОбсуждений,
			...
			ИначеЕсли ОбсужденияАктивны Тогда
				ОстальныеАктивные.Добавить(ДлительнаяОперация);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ДлительнаяОперация Из ОстальныеАктивные Цикл
		КонтролируемыеОперации.Вставить(ДлительнаяОперация.ИдентификаторЗадания, ДлительнаяОперация);
		ЗаданияДляПроверки.Добавить(ДлительнаяОперация.ИдентификаторЗадания);
	КонецЦикла;
...
// Для процедур ПриПолученииСерверногоОповещения, ПослеПериодическогоПолученияДанныхКлиентаНаСервере и
// ОбработатьНакопленныеСерверныеОповещения.
//
Процедура ОбработатьРезультатОперации(АктивныеДлительныеОперации, Операция, Результат)
	...
	Если ЗначениеЗаполнено(Результат.Сообщения) Тогда
		...
		Для Каждого ОписаниеСообщения Из Результат.Сообщения Цикл
			Если ТипЗнч(ОписаниеСообщения) = Тип("СообщениеПользователю") Тогда
				Сообщение = ОписаниеСообщения;
			Иначе
				Сообщение = Новый СообщениеПользователю;
				ЗаполнитьЗначенияСвойств(Сообщение, ОписаниеСообщения);
			КонецЕсли;
...
Процедура УточнитьИнтервалОбработчикаОжидания(Интервал, АктивныеДлительныеОперации, ОбсужденияАктивны)
	...
	Для Каждого Операция Из АктивныеДлительныеОперации Цикл
		Если Не ОбсужденияАктивны Или Операция.Значение.ПолученоОповещениеОЗавершении Тогда
...
Функция ИнтервалКонтроляОбсуждений()
	...
	Возврат ?(СерверныеОповещенияКлиент.СостояниеПолучения().УведомленияКлиентаДоступны, 180, 30);
...
Процедура ПриПолученииСерверногоОповещения(ИмяОповещения, Результат) Экспорт
	...
	Если Операция = Неопределено Тогда
		БудущиеОперации = ТекущиеДлительныеОперации.БудущиеОперации;
		ОповещенияОперации = БудущиеОперации.Получить(Результат.ИдентификаторЗадания);
		Если ОповещенияОперации = Неопределено Тогда
			ОповещенияОперации = Новый Структура;
			ОповещенияОперации.Вставить("Результаты", Новый Массив);
			ОповещенияОперации.Вставить("ДатаПоследнегоДобавления", '00010101');
			БудущиеОперации.Вставить(Результат.ИдентификаторЗадания, ОповещенияОперации);
		КонецЕсли;
		ОповещенияОперации.Результаты.Добавить(Результат.Результат);
		ОповещенияОперации.ДатаПоследнегоДобавления = ОбщегоНазначенияКлиент.ДатаУниверсальная();
		Возврат;
	ИначеЕсли ДлительнаяОперацияОтменена(Операция) Тогда
		Возврат;
	КонецЕсли;
...
// Для процедуры ОжидатьЗавершение.
Процедура ОбработатьНакопленныеСерверныеОповещения(ИдентификаторЗадания, Выполняется = Истина)
	
	ТекущиеДлительныеОперации = АктивныеДлительныеОперации();
	БудущиеОперации = ТекущиеДлительныеОперации.БудущиеОперации;
	ОповещенияОперации = БудущиеОперации.Получить(ИдентификаторЗадания);
	БудущиеОперации.Удалить(ИдентификаторЗадания);
	
	Если Выполняется Тогда
		АктивныеДлительныеОперации = ТекущиеДлительныеОперации.Список;
		Операция = АктивныеДлительныеОперации.Получить(ИдентификаторЗадания);
		Если Операция <> Неопределено И ОповещенияОперации <> Неопределено Тогда
			Для Каждого Результат Из ОповещенияОперации.Результаты Цикл
				ОбработатьРезультатОперации(АктивныеДлительныеОперации, Операция, Результат);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	ПросроченныеБудущиеОперации = Новый Массив;
	Для Каждого КлючИЗначение Из БудущиеОперации Цикл
		Если КлючИЗначение.Значение.ДатаПоследнегоДобавления + 15 < ДатаУниверсальная Тогда
			ПросроченныеБудущиеОперации.Добавить(КлючИЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
	Для Каждого ПросроченнаяБудущаяОперация Из ПросроченныеБудущиеОперации Цикл
		БудущиеОперации.Удалить(ПросроченнаяБудущаяОперация);
	КонецЦикла;
	
КонецПроцедуры

// Параметры:
//  РасширенныеПараметры - Структура:
//   * ФормаВладелец          - ФормаКлиентскогоПриложения
//                            - Неопределено
//   * Заголовок              - Строка
...
//   * ПолученоОповещениеОЗавершении - Булево
//
//  ДлительнаяОперация - см. ДлительныеОперации.НовыйРезультатВыполненияОперации
//
Функция ОбработатьРезультатАктивнойОперации(РасширенныеПараметры, ДлительнаяОперация)
	
	Если ДлительнаяОперация.Статус <> "Отменено" Тогда
		Если РасширенныеПараметры.ОповещениеОПрогрессеВыполнения <> Неопределено Тогда
			Состояние = НовоеСостояниеДлительнойОперации();
			Состояние.Статус    = ДлительнаяОперация.Статус;
			Состояние.Прогресс  = ДлительнаяОперация.Прогресс;
			Состояние.Сообщения = ДлительнаяОперация.Сообщения;
			Состояние.ИдентификаторЗадания = РасширенныеПараметры.ИдентификаторЗадания;
			Попытка
				ВыполнитьОбработкуОповещения(РасширенныеПараметры.ОповещениеОПрогрессеВыполнения, Состояние);
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При вызове оповещения о прогрессе длительной операции
					           |%1 возникла ошибка:
					           |%2'"),
					Строка(РасширенныеПараметры.ИдентификаторЗадания),
				...
				ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
					...
					ТекстОшибки);
			КонецПопытки;
		ИначеЕсли ДлительнаяОперация.Сообщения <> Неопределено Тогда
			Для Каждого Сообщение Из ДлительнаяОперация.Сообщения Цикл
				РасширенныеПараметры.НакопленныеСообщения.Добавить(Сообщение);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДлительнаяОперация.Статус <> "Выполняется" Тогда
		Если ДлительнаяОперация.Статус <> "Выполнено"
		 Или ДлительнаяОперация.Свойство("ФоновоеЗаданиеЗавершено")
		 Или Не (  РасширенныеПараметры.Свойство("АдресРезультата")
		         И ЗначениеЗаполнено(РасширенныеПараметры.АдресРезультата)
		       Или РасширенныеПараметры.Свойство("АдресДополнительногоРезультата")
		         И ЗначениеЗаполнено(РасширенныеПараметры.АдресДополнительногоРезультата))
		 ...
		 Или ДлительныеОперацииВызовСервера.ФоновоеЗаданиеЗавершено(РасширенныеПараметры.ИдентификаторЗадания) Тогда
		 
			ЗавершитьДлительнуюОперацию(РасширенныеПараметры, ДлительнаяОперация);
			Возврат Истина;
			
		ИначеЕсли ДлительнаяОперация.Статус = "Выполнено" Тогда
			РасширенныеПараметры.ТекущийИнтервал = 1;
			РасширенныеПараметры.ПолученоОповещениеОЗавершении = Истина;
			СерверныеОповещенияКлиент.ПодключитьОбработчикПроверкиПолученияСерверныхОповещений();
		КонецЕсли;
	КонецЕсли;
	
	ИнтервалОжидания = РасширенныеПараметры.ТекущийИнтервал;
	Если РасширенныеПараметры.Интервал = 0
	   И ДлительнаяОперация.Свойство("КонтрольДлительныхОперацийБезСистемыВзаимодействия") Тогда
		ИнтервалОжидания = ИнтервалОжидания * 1.4;
		Если ИнтервалОжидания > 15 Тогда
			ИнтервалОжидания = 15;
		КонецЕсли;
		РасширенныеПараметры.ТекущийИнтервал = ИнтервалОжидания;
	КонецЕсли;
	РасширенныеПараметры.Контроль = ТекущаяДата() + ИнтервалОжидания; // АПК:143 Дата сеанса не используется для проверки интервалов
	Возврат Ложь;
...
Процедура ЗавершитьДлительнуюОперацию(РасширенныеПараметры, ДлительнаяОперация)
	
	Если РасширенныеПараметры.ОповещениеОЗавершении = Неопределено Тогда
		Если ДлительнаяОперация.Статус = "Выполнено" Тогда
			ПоказатьОповещение(РасширенныеПараметры.ОповещениеПользователя, РасширенныеПараметры.ФормаВладелец);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если РасширенныеПараметры.Свойство("АдресРезультата") Тогда
			Результат.АдресРезультата = РасширенныеПараметры.АдресРезультата;
		КонецЕсли;
		Если РасширенныеПараметры.Свойство("АдресДополнительногоРезультата") Тогда
			Результат.АдресДополнительногоРезультата = РасширенныеПараметры.АдресДополнительногоРезультата;
		КонецЕсли;
		Результат.Вставить("ИнформацияОбОшибке",           ДлительнаяОперация.ИнформацияОбОшибке);
		...
		Результат.Вставить("Сообщения", Новый ФиксированныйМассив(РасширенныеПараметры.НакопленныеСообщения));
		Результат.ИдентификаторЗадания = РасширенныеПараметры.ИдентификаторЗадания;
	КонецЕсли;
	
	ОповеститьОЗавершенииДлительнойОперации(РасширенныеПараметры.ОповещениеОЗавершении,
		Результат, РасширенныеПараметры.ИдентификаторЗадания);
...
//       ** Ключ - УникальныйИдентификатор - идентификатор фонового задания.
//       ** Значение - см. ОбработатьРезультатАктивнойОперации.ДлительнаяОперация
//    * БудущиеОперации - Соответствие из КлючИЗначение:
//       ** Ключ - УникальныйИдентификатор - идентификатор фонового задания.
//       ** Значение - см. НовыеОповещенияБудущейОперации
//
Функция АктивныеДлительныеОперации()
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
		...
		Операции.Вставить("БудущиеОперации", Новый Соответствие);
...
Процедура ПоказатьОповещение(ОповещениеПользователя, ВладелецФормы)
	...
	Если ТипЗнч(Оповещение.Важное) = Тип("Булево") Тогда
		СтатусОповещения = ?(Оповещение.Важное,
			СтатусОповещенияПользователя.Важное, СтатусОповещенияПользователя.Информация);
	КонецЕсли;
	
	Текст = ?(Оповещение.Текст <> Неопределено, Оповещение.Текст, НСтр("ru = 'Действие выполнено'"));
	
	ВывестиОповещениеПользователю(Текст, НавигационнаяСсылкаОповещения, ПояснениеОповещения,
		Оповещение.Картинка, СтатусОповещения, Неопределено, ВладелецФормы);
	
КонецПроцедуры

// Для подключения расширением для тестирования.
Процедура ВывестиОповещениеПользователю(Текст, ДействиеПриНажатии, Пояснение,
			Картинка, СтатусОповещения, КлючУникальности, ВладелецФормы)
	
	ПоказатьОповещениеПользователя(Текст, ДействиеПриНажатии, Пояснение,
		Картинка, СтатусОповещения, КлючУникальности);
