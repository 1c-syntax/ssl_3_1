...
#Область ПроцедурыИФункцииДляСтандартнойОбработкиСвойств
...
Процедура ПриПолученииДанныхНаСервере(Настройки, Строки, ИмяВладельца = Неопределено) Экспорт
	...
	РеквизитыМеток = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Метки, "ЦветСвойства, ПометкаУдаления");
	
	Для каждого СтрокаСписка Из Строки Цикл
		КлючСтроки = СтрокаСписка.Ключ;
		...
		Для каждого Метка Из Метки Цикл
			...
			Если ДанныеСтроки.Данные.Свойство(ИмяСвойства) Тогда
				ДанныеСтроки.Данные[ИмяСвойства] =
					Перечисления.ЦветаСвойств.Индекс(РеквизитыМетки.ЦветСвойства) + 1;
...
#Область ПроцедурыИФункцииДляПрограммнойРаботыСоСвойствами
...
Функция ДобавитьСвойство(Знач Владелец, Знач Параметры, Знач ВидСвойства = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Владелец) = Тип("Строка") Тогда
		Если СтрНайти(Владелец, ".") = 0 Тогда
			ИмяПредопределенного = Владелец; // Это набор свойств.
		Иначе
	...
	ПараметрыДобавленияСвойства = ПараметрыДобавленияСвойства();
	ЗаполнитьЗначенияСвойств(ПараметрыДобавленияСвойства, Параметры);
	
	НезаполненныеПараметры = Новый Массив;
	Если Не ЗначениеЗаполнено(ПараметрыДобавленияСвойства.Наименование) Тогда
		НезаполненныеПараметры.Добавить("Наименование");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыДобавленияСвойства.ТипЗначения) Тогда
		Если ЗначениеЗаполнено(ПараметрыДобавленияСвойства.Тип) Тогда
			ПараметрыДобавленияСвойства.ТипЗначения = ПараметрыДобавленияСвойства.Тип;
		Иначе
			НезаполненныеПараметры.Добавить("ТипЗначения");
		КонецЕсли;
	КонецЕсли;
	
	Если ТипЗнч(ВидСвойства) = Тип("Булево") Или ВидСвойства = Неопределено Тогда
		ВидСвойства = ?(ВидСвойства = Истина, Перечисления.ВидыСвойств.ДополнительныеСведения,
			Перечисления.ВидыСвойств.ДополнительныеРеквизиты); // Для обратной совместимости
	ИначеЕсли ВидСвойства = Перечисления.ВидыСвойств.Метки 
		И (Не ЗначениеЗаполнено(ПараметрыДобавленияСвойства.ЦветСвойства)) Тогда
		НезаполненныеПараметры.Добавить("ЦветСвойства");
	КонецЕсли;
	ЭтоСведение = (ВидСвойства = Перечисления.ВидыСвойств.ДополнительныеСведения);
	
	Если НезаполненныеПараметры.Количество() > 0 Тогда
		НезаполненныеПараметры = СтрСоединить(НезаполненныеПараметры, ", ");
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заданы обязательные свойства в параметре %1 при вызове %2:
				|%3.'"), "Параметры", "УправлениеСвойствами.ДобавитьСвойство", НезаполненныеПараметры);
	...
	Результат = УправлениеСвойствамиСлужебный.НаименованиеУжеИспользуется(ПустаяСсылка, НаборСвойств, ПараметрыДобавленияСвойства.Наименование);
	Если ЗначениеЗаполнено(Результат) Тогда
		ВызватьИсключение Результат;
	КонецЕсли;
	
	// 2. Проверка, что имя уже используется.
	Если ЗначениеЗаполнено(ПараметрыДобавленияСвойства.Имя) Тогда
		Результат = УправлениеСвойствамиСлужебный.ИмяУжеИспользуется(ПараметрыДобавленияСвойства.Имя, ПустаяСсылка);
		Если ЗначениеЗаполнено(Результат) Тогда
			ВызватьИсключение Результат;
		КонецЕсли;
	Иначе
		Имя = "";
		ЗаголовокОбъекта = ПараметрыДобавленияСвойства.Наименование;
		...
		ПараметрыДобавленияСвойства.Вставить("Имя", Имя);
	КонецЕсли;
	
	// 3. Проверка, что идентификатор для формул уже используется.
	Если ЗначениеЗаполнено(ПараметрыДобавленияСвойства.ИдентификаторДляФормул) Тогда
		Результат = УправлениеСвойствамиСлужебный.ИдентификаторДляФормулУжеИспользуется(ПараметрыДобавленияСвойства.ИдентификаторДляФормул, ПустаяСсылка);
		Если ЗначениеЗаполнено(Результат) Тогда
			ВызватьИсключение Результат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ЗаполнитьЗначенияСвойств(НовоеСвойство, ПараметрыДобавленияСвойства);
		
		НовоеСвойство.ВидСвойства = ВидСвойства;
		НовоеСвойство.Заголовок                 = ПараметрыДобавленияСвойства.Наименование;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			...
			СведенияОЯзыках = МодульМультиязычностьСервер.СведенияОЯзыках();
			Для Каждого СведенияОЯзыке Из СведенияОЯзыках.Используется Цикл
				НовоеСвойство["Заголовок" + СведенияОЯзыке.Ключ] = ПараметрыДобавленияСвойства.Наименование;
			КонецЦикла;
		КонецЕсли;
...
Функция ДобавитьЗначениеСвойства(Знач Владелец, Параметры, Иерархия = Ложь) Экспорт
	...
	ОписаниеТипаЗначенийИерархия = Новый ОписаниеТипов("СправочникСсылка.ЗначенияСвойствОбъектовИерархия");
	Если ТипРеквизита = ОписаниеТипаЗначенийИерархия Тогда
...
Функция ПараметрыДобавленияСвойства() Экспорт
	...
	Параметры.Вставить("ЦветСвойства");
...
#Область ПроцедурыИФункцииДляНестандартнойОбработкиДополнительныхСвойств
...
//           - ПланСчетовОбъект
//  Свойства - Соответствие из КлючИЗначение:
...
Процедура УстановитьСвойстваУОбъекта(Владелец, Свойства) Экспорт
	...
	СсылкиСвойств = Новый Массив;
	Для каждого СвойствоКлючЗначение Из Свойства Цикл
		СсылкиСвойств.Добавить(СвойствоКлючЗначение.Ключ);
	КонецЦикла;
	
	ТипыСвойствСведение = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СсылкиСвойств, "ЭтоДополнительноеСведение");
	Для каждого СвойствоКлючЗначение Из Свойства Цикл
		Если ТипыСвойствСведение[СвойствоКлючЗначение.Ключ] = Истина Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 в процедуре %2. Допустима установка только дополнительных реквизитов.'"),
			"Свойства",
		...
		Если ТипЗнч(СвойствоКлючЗначение.Значение) = Тип("Строка")
			...
			НоваяСтрока.ТекстоваяСтрока = СвойствоКлючЗначение.Значение;
		КонецЕсли;
		
		НоваяСтрока.Свойство = СвойствоКлючЗначение.Ключ;
		НоваяСтрока.Значение = СвойствоКлючЗначение.Значение
		
	КонецЦикла;
	
	МассивДопРеквизитовВладельца = СвойстваОбъекта(Владелец, Истина, Ложь);
	Для Каждого ДопРеквизит Из ТаблицаДопРеквизитов Цикл
		Если МассивДопРеквизитовВладельца.Найти(ДопРеквизит.Свойство) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		МассивСтрок = Владелец.ДополнительныеРеквизиты.НайтиСтроки(Новый Структура("Свойство", ДопРеквизит.Свойство));
		Если МассивСтрок.Количество() > 0 Тогда
			СтрокаСвойства = МассивСтрок[0];
			ЗаполнитьЗначенияСвойств(СтрокаСвойства, ДопРеквизит, "Свойство,Значение,ТекстоваяСтрока");
		Иначе
			СтрокаСвойства = Владелец.ДополнительныеРеквизиты.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаСвойства, ДопРеквизит, "Свойство,Значение,ТекстоваяСтрока");
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьМеткиОбъекта(Форма, Объект = Неопределено, ПроизвольныйОбъект = Ложь) Экспорт
	...
	РеквизитыМеток = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Метки,
		"Имя, ЦветСвойства, Наименование, ПометкаУдаления, Подсказка");
	Если ВариантОтображенияМеток = Перечисления.ВариантыОтображенияМеток.Надпись Тогда
		Для Индекс = 0 По КоличествоМеток - 1 Цикл
			...
			НовыйЭлемент.ЦветФона = ЭлементСтиляПоЦвету(Метка.ЦветСвойства);
	...
	Иначе
		Для Индекс = 0 По КоличествоМеток - 1 Цикл
			...
			Если Не ПроизвольныйОбъект Тогда
				НовыйЭлемент.Гиперссылка = Истина;
			КонецЕсли;
			НовыйЭлемент.Картинка = КартинкаМеткиПоЦвету(Метка.ЦветСвойства);
			НовыйЭлемент.УстановитьДействие("Нажатие", "Подключаемый_СвойстваВыполнитьКоманду");
...
Функция СвойстваПоВидуДополнительныхРеквизитов(Свойства, ВидСвойства) Экспорт
	...
	ВидыСвойствОбъектов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(СписокСвойств, "ВидСвойства");
	Если ВидСвойства = Перечисления.ВидыСвойств.ДополнительныеРеквизиты Тогда
		Для Каждого Свойство Из СписокСвойств Цикл
			ВидСвойстваОбъекта = ВидыСвойствОбъектов.Получить(Свойство);
			Если Не ЗначениеЗаполнено(ВидСвойстваОбъекта) Тогда
				Продолжить;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ВидСвойстваОбъекта.ВидСвойства)
				Или ВидСвойстваОбъекта.ВидСвойства = ВидСвойства Тогда
	...
	ИначеЕсли ВидСвойства = Перечисления.ВидыСвойств.Метки Тогда
		Для Каждого Свойство Из СписокСвойств Цикл
			...
			Если Не ЗначениеЗаполнено(ВидСвойстваОбъекта) Тогда
				Продолжить;
			КонецЕсли;
			Если ВидСвойстваОбъекта.ВидСвойства = ВидСвойства Тогда
				СвойстваПоВиду.Добавить(Свойство);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьНастройкиФормСДополнительнымиРеквизитами() Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПредопределенных Цикл
		Попытка
			...
			ИмяНабора = Сред(СтрокаТаблицы.ИмяПредопределенныхДанных, ДлинаПрефикса + 1, 
				СтрДлина(СтрокаТаблицы.ИмяПредопределенныхДанных) - ДлинаПрефикса);
			
			ИдентификаторСсылки = Строка(СтрокаТаблицы.Ссылка.УникальныйИдентификатор());
...
Процедура ЗаполнитьЛегендуМеток(Форма, Объект)
	...
	РеквизитыМеток = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Метки,
		"Ссылка, Имя, Наименование, ЦветСвойства, ПометкаУдаления");
	Если ОтборМеток Тогда
		Реквизиты = Новый Массив;
		Для Каждого Метка Из Метки Цикл
			РеквизитыМетки = РеквизитыМеток.Получить(Метка);
...
Функция КартинкаМеткиПоЦвету(ЦветСвойства)
	
	Если ЦветСвойства = Перечисления.ЦветаСвойств.Голубой Тогда
	...
	ИначеЕсли ЦветСвойства = Перечисления.ЦветаСвойств.Лаймовый Тогда
...
Функция ЭлементСтиляПоЦвету(ЦветСвойства)
