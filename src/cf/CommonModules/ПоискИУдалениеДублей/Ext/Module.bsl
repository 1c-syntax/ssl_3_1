...
Функция НайтиДублиЭлемента(Знач ОбластьПоиска, Знач ЭталонныйОбъект, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из Результат.НайтиСтроки(Новый Структура("Родитель", Неопределено)) Цикл
		Результат.Удалить(Элемент);
	КонецЦикла;
	ПустаяСсылка = МенеджерОбластиПоиска.ПустаяСсылка();
	Для Каждого Элемент Из Результат.НайтиСтроки(Новый Структура("Ссылка", ПустаяСсылка)) Цикл
		Результат.Удалить(Элемент);
	КонецЦикла;
...
Процедура ДополнитьДублиСвязаннымиПодчиненнымиОбъектами(ПарыЗамен, ПараметрыЗамены) Экспорт
	...
	Для каждого ДубльОригинал Из ПарыЗамен Цикл
		ОтметитьИспользуемыеСвязи(ДубльОригинал.Ключ, СвязиПодчиненныхОбъектов);
		ДобавитьВТаблицыЗамены(ДубльОригинал, ТаблицыЗамен, СвязиПодчиненныхОбъектов);
	КонецЦикла;
	...
	ТаблицыНайденныхПодчиненных = Новый Соответствие;
	...
	Пока Позиция <= ИспользуемыеСвязи.Количество() - 1 Цикл
		...
		ТаблицыНайденныхПодчиненных.Вставить(ОписаниеПодчиненногоОбъекта.Ключ, ЧастиПакета.Количество() * 3 - 3);
	...
	ПодчиненныеКДублям = ПараметрыЗамены.ПодчиненныеКДублям;
	Для Каждого РезультатЗапроса Из ТаблицыНайденныхПодчиненных Цикл
		
		ОписаниеПодчиненногоОбъекта = ОписаниеПодчиненныхОбъектов[РезультатЗапроса.Ключ];
		ТаблицаПодчиненных = Результат[РезультатЗапроса.Значение].Выгрузить();
		
		Для Каждого КлючевойРеквизит Из ОписаниеПодчиненногоОбъекта.КлючевыеРеквизиты Цикл
			Для Каждого СтрокаСПодчиненным Из ТаблицаПодчиненных Цикл
				ЗначениеКлючевогоРеквизита = СтрокаСПодчиненным[КлючевойРеквизит];
				ПоискДубляВПарах = ПарыЗамен[ЗначениеКлючевогоРеквизита];
				Если ПоискДубляВПарах <> Неопределено Тогда
					ПодчиненныеКДублям.Вставить(СтрокаСПодчиненным.Ссылка, ЗначениеКлючевогоРеквизита);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Функция НайтиПохожиеСтроки(ИсходнаяСтрока, СтрокаПоиска, Разделитель = "~", ПараметрыПоиска = Неопределено) Экспорт
	...
	Возврат СтрЗаменить(ИндексыСтрок, Символ(8239), "");
...
Функция ПараметрыПоискаПохожихСтрок(ПодключитьКомпоненту = Истина) Экспорт
	...
	Результат.Вставить("ПроцентСовпаденияСтрок", 90);
	Результат.Вставить("ПроцентСовпаденияНебольшихСтрок", 80);
	Результат.Вставить("ДлинаНебольшихСтрок", 30);
	Результат.Вставить("СловаИсключения", Новый Массив);
	
	НечеткийПоиск = Неопределено; 
	Если ПодключитьКомпоненту Тогда
		УстановитьОтключениеБезопасногоРежима(Истина);
		НечеткийПоиск = ОбщегоНазначения.ПодключитьКомпонентуИзМакета("FuzzyStringMatchExtension", 
			"ОбщийМакет.КомпонентаПоискаСтрок");
		...
		КонецЕсли;
	КонецЕсли;
	Результат.Вставить("КомпонентаПоиска", НечеткийПоиск);
...
//         куда будет помещен результат выполнения ОбщегоНазначения.ЗаменитьСсылки().
//
...
Функция ТекстЗапросаОбъектовДляЗамены(ОписаниеПодчиненногоОбъекта, Связи)
	...
	Для каждого КлючевойРеквизит Из ОписаниеПодчиненногоОбъекта.КлючевыеРеквизиты Цикл
		...
		Значение = "ОБЪЕДИНИТЬ"
			+ СтрЗаменить(" ВЫБРАТЬ
							...
							|		Таб.Дубль
							|	ИЗ
							|		ТаблицаЗамен КАК Таб)", "&КлючевойРеквизит","Таб." + КлючевойРеквизит); //  @query-part-1
		...
		Если ИзменяемыеРеквизитыМассив.Найти(КлючевойРеквизит) <> Неопределено Тогда
			...
			Значение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ВЫБОР КОГДА Замена%1.Дубль IS NULL ТОГДА Таб.%1 ИНАЧЕ Замена%1.Оригинал КОНЕЦ КАК %1", КлючевойРеквизит);
			...
			Значение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(" ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаЗамен КАК Замена%1
								|ПО Таб.%1 = Замена%1.Дубль", КлючевойРеквизит);
	...
	ВставитьВТекстЗапроса(ТекстЗапроса, "&ЗначениеИзменяемыеРеквизиты", ?(ПустаяСтрока(ЗначениеИзменяемыеРеквизиты), "НЕОПРЕДЕЛЕНО", ЗначениеИзменяемыеРеквизиты));
...
Процедура ДобавитьВТаблицыЗамены(ДубльОригинал, ТаблицыЗамен, СвязиПодчиненныхОбъектов) 

	МетаданныеКлючевогоРеквизита = ДубльОригинал.Ключ.Метаданные();
	...
	Если ТаблицаЗамен = Неопределено Тогда
		Типы = Новый Массив;
		Типы.Добавить(ТипЗнч(ДубльОригинал.Значение));
	...
	СтрокаЗамены.Оригинал = ДубльОригинал.Значение;
	СтрокаЗамены.Дубль = ДубльОригинал.Ключ;
...
Процедура ВыполнитьПоискПоПрикладнымПравилам(Знач ИспользуемыеСвязи, Знач НеподобранныеЗамены, Знач ОписаниеПодчиненногоОбъекта, Знач ПарыЗамен)
	...
	Для Индекс = 0 По НеподобранныеЗамены.Количество() - 1 Цикл
		НеподобранныеЗамены[Индекс] = НовыйПараметрыЗамены(НеподобранныеЗамены[Индекс],
			ОписаниеПодчиненногоОбъекта, ИспользуемыеСвязи);	
	КонецЦикла;
	
	Модуль = ОбщегоНазначения.ОбщийМодуль(ОписаниеПодчиненногоОбъекта.ИмяМодуляМетодаПоиска);
	Модуль.ПриПоискеЗаменыСсылок(ПарыЗамен, НеподобранныеЗамены);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаменитьСсылки", Истина);
	Методы.Вставить("ОпределитьМестаИспользования", Истина);
