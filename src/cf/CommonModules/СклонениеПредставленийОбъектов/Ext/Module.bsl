	...
	Попытка
		...
		Если ЗаписьИзФормы Или НаборЗаписейСклонения.Количество() = 0 Тогда
			...
			Если Таблица.НайтиСтроки(Отбор).Количество() = 0 Тогда
				Запись = ?(НаборЗаписейСклонения.Количество() = 0,
					НаборЗаписейСклонения.Добавить(), НаборЗаписейСклонения[0]);
...
Процедура ЗаполнитьСклоненияСтрокИзРегистра(Представления)
	...
	// АПК:1377-выкл Измерение Объект состоит из небольшого количества типов и каждый тип требуется проверить.
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеСклонения", Истина);
