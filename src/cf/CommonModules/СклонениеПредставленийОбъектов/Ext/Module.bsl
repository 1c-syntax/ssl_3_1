...
Процедура ЗаписатьВРегистрСклонения(Представление, Объект, Склонения, ЗаписьИзФормы = Ложь)
	...
	Если Не ЗначениеЗаполнено(Объект)
	 Или Не Метаданные.ОпределяемыеТипы.ОбъектСклонения.Тип.СодержитТип(ТипЗнч(Объект)) Тогда
		Возврат;
	КонецЕсли;
...
Функция СклоненияИзРегистра(Представление, Объект = Неопределено)
	...
	Если Объект <> Неопределено
	   И Метаданные.ОпределяемыеТипы.ОбъектСклонения.Тип.СодержитТип(ТипЗнч(Объект)) Тогда
		...
		|УПОРЯДОЧИТЬ ПО
		|	ВЫБОР
		|		КОГДА СклоненияПредставленийОбъектов.Объект = &Объект
		|			ТОГДА 0
		|		КОГДА ТИПЗНАЧЕНИЯ(СклоненияПредставленийОбъектов.Объект) = ТИПЗНАЧЕНИЯ(&Объект)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ";
	Иначе
		...
		|УПОРЯДОЧИТЬ ПО
		|	ВЫБОР
		|		КОГДА СклоненияПредставленийОбъектов.Объект = НЕОПРЕДЕЛЕНО
		|			ТОГДА 2
		|		КОГДА СклоненияПредставленийОбъектов.Объект.Ссылка ЕСТЬ NULL
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ";
	КонецЕсли;
