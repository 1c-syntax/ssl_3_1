...
Процедура ОткрытьВложение(Ссылка, Форма, ДляРедактирования = Ложь) Экспорт
	...
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ИмяФайлаИРасширение(Ссылка);
	
	Если Форма.ЗапрещенныеРасширения.НайтиПоЗначению(ДанныеФайла.Расширение) <> Неопределено Тогда

		ДополнительныеПараметры = Новый Структура("ДляРедактирования", ДляРедактирования);
		ДополнительныеПараметры.Вставить("ПрисоединенныйФайл", Ссылка);
	...
	ОткрытьФайлВложения(Ссылка, Форма, ДляРедактирования);
...
Процедура ОткрытьФайлПослеПодтверждения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И Результат = "Продолжить" Тогда
		...
		ОткрытьФайлВложения(ДополнительныеПараметры.ПрисоединенныйФайл, 
								ДополнительныеПараметры.Форма, 
								ДополнительныеПараметры.ДляРедактирования);
	КонецЕсли;

КонецПроцедуры

Процедура ОткрытьФайлВложения(Ссылка, Форма, ДляРедактирования)
	
	ПараметрыПолученияФайла = РаботаСФайламиКлиент.ПараметрыАсинхронногоПолученияФайла("ОткрытьФайл", "РаботаСФайламиСлужебный.ДанныеФайлаУправлениеЭлектроннойПочтойАсинхронный");
	ПараметрыПолученияФайла.ПрисоединенныйФайл	= Ссылка;
	ПараметрыПолученияФайла.ФормаВладелец 		= Форма;

	ПараметрыПолученияФайла.ПараметрыДействия.ДляРедактирования = ДляРедактирования;
