...
Функция АдресныеСокращения() Экспорт
	...
	АдресныеСокращения.Сортировать("Уровень, Актуально Убыв");
...
//   * Наименование - Строка
//   * ТипОбъекта - Строка
//   * ОфициальноеНаименование - Строка
//   * КодСубъектаРФ - Число
//
Функция ДанныеОРегионе(Наименование) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Наименование", "");
	Результат.Вставить("ТипОбъекта", "");
	Результат.Вставить("ОфициальноеНаименование", "");
	Результат.Вставить("КодСубъектаРФ", Неопределено);
	
	СписокРегионов = СписокРегионов();
	Для Каждого СтрокаРегиона Из СписокРегионов Цикл
		
		Если СтрСравнить(СтрокаРегиона.Наименование, Наименование) = 0 
		 Или СтрСравнить(СтрокаРегиона.ОфициальноеНаименование, Наименование) = 0 Тогда
			ЗаполнитьЗначенияСвойств(Результат, СтрокаРегиона);
			Возврат Результат;
		КонецЕсли;
	...
	Для Каждого СтрокаРегиона Из Классификатор Цикл
		
		Если СтрСравнить(СтрокаРегиона.Наименование, Наименование) = 0 
		 Или СтрСравнить(СтрокаРегиона.ПолноеНаименование, Наименование) = 0 Тогда
			ЗаполнитьЗначенияСвойств(Результат, СтрокаРегиона);
			Результат.ОфициальноеНаименование = СтрокаРегиона.ПолноеНаименование;
			Возврат Результат;
		КонецЕсли;
...
Функция УровниАдресныхСведений() Экспорт
	...
	Иначе
		АдресныеСокращения = РегистрыСведений.АдресныеОбъекты.АдресныеСокращения();
		АдресныеСокращения.Свернуть("Уровень");
		Уровни = АдресныеСокращения.ВыгрузитьКолонку("Уровень");
	КонецЕсли;
...
Функция СведенияОРегионе(Знач Наименование, ТипОбъекта = "") Экспорт
	...
	РезультатЗапроса = АдресныйКлассификаторСлужебный.АктуальныеДанныеОРегионе(Наименование);
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Если РезультатЗапроса.Пустой() Тогда
			...
			ДанныеОРегионе = ДанныеОРегионе(Наименование);
			КодСубъектаРФ = ?(ДанныеОРегионе <> Неопределено, ДанныеОРегионе.КодСубъектаРФ, Неопределено);
...
Функция СписокРегионов()
	...
	|	СлужебныеАдресныеСведения.Значение КАК ОфициальноеНаименование,
	|	АдресныеОбъекты.КодСубъектаРФ КАК КодСубъектаРФ
	|ИЗ
	|	РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СлужебныеАдресныеСведения КАК СлужебныеАдресныеСведения
	|		ПО (АдресныеОбъекты.КодСубъектаРФ = (ВЫРАЗИТЬ(СлужебныеАдресныеСведения.Идентификатор КАК ЧИСЛО(2, 0))))
	|ГДЕ
	|	АдресныеОбъекты.Уровень = 1
	|	И СлужебныеАдресныеСведения.Тип = ""СубъектыРФ""";
