...
//
// Параметры:
//   Данные         - СправочникОбъект, ДокументОбъект - Объект для получения значений реквизитов и других свойств.
...
//
// Параметры:
//   СоответствиеИменПсевдонимам - Соответствие - имена методов и соответствующие им псевдонимы.
...
//
// Параметры:
//   Типы - Массив - объекты метаданных, исключаемые из выгрузки и загрузки.
//
...
Процедура ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДанными) Экспорт
	...
	Если СтрокаСценария.НомерИнформационнойБазы = 1 Тогда
		...
		Если Не ОбменДаннымиСервер.НастройкаСинхронизацииЗавершена(УзелИнформационнойБазы)
			Или ОбменДаннымиСервер.ПолученоСообщениеСДаннымиДляСопоставления(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неизвестное действие ""%1"" при обмене данными между областями данных.'"),
	...
	ИначеЕсли СтрокаСценария.НомерИнформационнойБазы = 2 Тогда
		...
		Если ВерсииКорреспондента.Найти("2.4.5.1") <> Неопределено Тогда
			...
			WSПрокси = ОбменДаннымиВМоделиСервисаПовтИсп.ПолучитьWSПроксиКорреспондента_2_4_5_1(УзелИнформационнойБазы);
			ПараметрСценарийОбменаДанными = СериализаторXDTO.ЗаписатьXDTO(СценарийОбменаДанными);
			
		ИначеЕсли ВерсииКорреспондента.Найти("2.1.6.1") <> Неопределено Тогда
			
			WSПрокси = ОбменДаннымиВМоделиСервисаПовтИсп.ПолучитьWSПроксиКорреспондента_2_1_6_1(УзелИнформационнойБазы);
			ПараметрСценарийОбменаДанными = СериализаторXDTO.ЗаписатьXDTO(СценарийОбменаДанными);
		...
		Иначе
			...
			ПараметрСценарийОбменаДанными = ЗначениеВСтрокуВнутр(СценарийОбменаДанными);
		...
		WSПрокси.StartExchangeExecutionInSecondDataBase(ИндексСтрокиСценария, ПараметрСценарийОбменаДанными);
