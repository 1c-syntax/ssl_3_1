...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	МодульВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки(Типы,
		Метаданные.Справочники.НастройкиТранспортаСообщенийОбмена, МодульВыгрузкаЗагрузкаДанных.ДействиеСоСсылкамиНеВыгружатьОбъект());
	...
	Типы.Добавить(Метаданные.РегистрыСведений.УдалитьНастройкиТранспортаОбменаДанными);
	...
	Типы.Добавить(Метаданные.РегистрыСведений.КоличествоБлокируемыхОбъектовКЗагрузке);
	Типы.Добавить(Метаданные.РегистрыСведений.ОбъектыКЗагрузке);
	Типы.Добавить(Метаданные.РегистрыСведений.ОбъектыБлокирующиеЗагрузку);
	Типы.Добавить(Метаданные.РегистрыСведений.ПрочитанныеДанныеСообщенийОбмена);
	Типы.Добавить(Метаданные.РегистрыСведений.ДействияПриПолученииДанныхОбмена);
...
//
Процедура ПриПолученииФайлаИзХранилища(Знач ИдентификаторФайла, ИмяФайла, УдалитьФайлИзХранилища = Истина) Экспорт
...
Процедура ЗаблокироватьКонечныеТочки() Экспорт
	...
	МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
	КонечнаяТочкаМенеджераСервиса = МодульРаботаВМоделиСервиса.КонечнаяТочкаМенеджераСервиса();
	...
	Попытка
		...
		Запрос.УстановитьПараметр("КонечнаяТочкаМенеджераСервиса", КонечнаяТочкаМенеджераСервиса);
...
Процедура ПриОтправкеДанныхПодчиненному(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза, Получатель) Экспорт
	...
	Если Получатель = Неопределено 
		ИЛИ ОтправкаЭлемента = ОтправкаЭлементаДанных.Удалить
		ИЛИ ОтправкаЭлемента = ОтправкаЭлементаДанных.Игнорировать
		ИЛИ ТипЗнч(ЭлементДанных) = Тип("УдалениеОбъекта")
		Тогда
	...
	// Для объектов разделенных вспомогательным разделителем при выгрузке из области
	// данных для локального АРМ необходимо очистить сведения о разделители.
	// Раннее это делали только при создании начального образа, 
	// т.к. не было объектов, которые участвуют в регулярном обмене.
	
	Если ОбщегоНазначения.РазделениеВключено()
		И АвтономнаяРаботаСлужебный.ЭтоУзелАвтономногоРабочегоМеста(Получатель.Ссылка) Тогда
		
		Если НЕ Получатель.ДополнительныеСвойства.Свойство("СвойстваМетаданных") Тогда
			
			// Добавлено для регулярного обмена, потому что при создании начального образа добавляется из
			// ПомощникСозданияАвтономногоРабочегоМеста.ЗаписатьПакетУстановкиВоВременноеХранилище()
			Получатель.ДополнительныеСвойства.Вставить("СвойстваМетаданных", Новый Соответствие);
		...
		Если СвойстваМетаданных.ЭтоРазделенныйОбъектМетаданных Тогда
			...
			Если СозданиеНачальногоОбраза Тогда
				...
				// При создании начального образа разделенные объекты не попадают в имидж (1cv8.1CD)
				// и будут выгружены отдельно.
...
Процедура ПолучитьСостоянияОбменовДанными(МенеджерВременныхТаблиц) Экспорт
	...
	|	МАКСИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания) КАК ДатаСоздания
	|ПОМЕСТИТЬ НезавершенныеЗадачи
	|ИЗ
	|	РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|ГДЕ
	|	ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаУспешно = ЛОЖЬ
	|	И ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаСОшибкой = ЛОЖЬ
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	|	МАКСИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания) КАК ДатаСоздания,
	|	МИНИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.НомерЗадачи) КАК НомерЗадачи
	|ПОМЕСТИТЬ НезавершенныеЗадачиНомераЗадач
	|ИЗ
	|	НезавершенныеЗадачи КАК ВременнаяТаблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|		ПО ВременнаяТаблица.УзелИнформационнойБазы = ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы
	|			И ВременнаяТаблица.ДатаСоздания = ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания
	|ГДЕ
	|	ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаУспешно = ЛОЖЬ
	|	И ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаСОшибкой = ЛОЖЬ
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВложенныйЗапрос.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	|	МАКСИМУМ(ВложенныйЗапрос.ДатаСоздания) КАК ДатаСоздания
	|ПОМЕСТИТЬ СгруппированныеПоследниеВыполненныеЗадачи
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	...
	|		РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|	ГДЕ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаУспешно = ИСТИНА
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы,
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания
	|	ИЗ
	|		РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|	ГДЕ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаСОшибкой = ИСТИНА) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.УзелИнформационнойБазы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВложенныйЗапрос.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	|	МАКСИМУМ(ВложенныйЗапрос.ДатаСоздания) КАК ДатаСоздания,
	|	МАКСИМУМ(ВложенныйЗапрос.НомерЗадачи) КАК НомерЗадачи
	|ПОМЕСТИТЬ ПоследниеВыполненныеЗадачиНомераЗадач
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	|		МАКСИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания) КАК ДатаСоздания,
	|		МАКСИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.НомерЗадачи) КАК НомерЗадачи
	|	ИЗ
	|		СгруппированныеПоследниеВыполненныеЗадачи КАК СгруппированныеПоследниеВыполненныеЗадачи
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|			ПО СгруппированныеПоследниеВыполненныеЗадачи.УзелИнформационнойБазы = ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы
	|				И СгруппированныеПоследниеВыполненныеЗадачи.ДатаСоздания = ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания
	|	ГДЕ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаУспешно = ИСТИНА
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|	ВЫБРАТЬ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы,
	|		МАКСИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания),
	|		МАКСИМУМ(ЗадачиОбменаДаннымиВнутренняяПубликация.НомерЗадачи)
	|	ИЗ
	|		СгруппированныеПоследниеВыполненныеЗадачи КАК СгруппированныеПоследниеВыполненныеЗадачи
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|			ПО СгруппированныеПоследниеВыполненныеЗадачи.УзелИнформационнойБазы = ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы
	|				И СгруппированныеПоследниеВыполненныеЗадачи.ДатаСоздания = ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания
	|	ГДЕ
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.ЗавершенаСОшибкой = ИСТИНА
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.УзелИнформационнойБазы
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НезавершенныеЗадачиНомераЗадач.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	|	НезавершенныеЗадачиНомераЗадач.ДатаСоздания КАК ДатаСоздания,
	|	НезавершенныеЗадачиНомераЗадач.НомерЗадачи КАК НомерЗадачи
	|ПОМЕСТИТЬ НевыполненныеЗадачи
	|ИЗ
	|	НезавершенныеЗадачиНомераЗадач КАК НезавершенныеЗадачиНомераЗадач
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследниеВыполненныеЗадачиНомераЗадач КАК ПоследниеВыполненныеЗадачиНомераЗадач
	|		ПО НезавершенныеЗадачиНомераЗадач.УзелИнформационнойБазы = ПоследниеВыполненныеЗадачиНомераЗадач.УзелИнформационнойБазы
	|			И НезавершенныеЗадачиНомераЗадач.ДатаСоздания <= ПоследниеВыполненныеЗадачиНомераЗадач.ДатаСоздания
	|			И НезавершенныеЗадачиНомераЗадач.НомерЗадачи < ПоследниеВыполненныеЗадачиНомераЗадач.НомерЗадачи
	|ГДЕ
	|	ПоследниеВыполненныеЗадачиНомераЗадач.УзелИнформационнойБазы ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НевыполненныеЗадачи.УзелИнформационнойБазы КАК УзелИнформационнойБазы,
	|	ЗадачиОбменаДаннымиВнутренняяПубликация.Действие КАК Действие
	|ПОМЕСТИТЬ ТекущиеЗадачиУзлов
	|ИЗ
	|	НевыполненныеЗадачи КАК НевыполненныеЗадачи
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ЗадачиОбменаДаннымиВнутренняяПубликация
	|		ПО НевыполненныеЗадачи.УзелИнформационнойБазы = ЗадачиОбменаДаннымиВнутренняяПубликация.УзелИнформационнойБазы
	|			И НевыполненныеЗадачи.ДатаСоздания = ЗадачиОбменаДаннымиВнутренняяПубликация.ДатаСоздания
	|			И НевыполненныеЗадачи.НомерЗадачи = ЗадачиОбменаДаннымиВнутренняяПубликация.НомерЗадачи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ НезавершенныеЗадачи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ НезавершенныеЗадачиНомераЗадач
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ СгруппированныеПоследниеВыполненныеЗадачи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ПоследниеВыполненныеЗадачиНомераЗадач
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ НевыполненныеЗадачи");
...
// 
// Возвращаемое значение:
//  УникальныйИдентификатор - идентификатор новой сессии
//
...
Процедура ОбновитьНастройкиТранспортаОбластиДанных(Параметры) Экспорт
	...
	ИначеЕсли НастройкиТранспорта.ВидТранспортаСообщенийОбменаПоУмолчанию = Перечисления.ВидыТранспортаСообщенийОбмена.FTP Тогда
		...
		НастройкиFTP = ТранспортСообщенийОбмена.FTPНастройкиСоединения();
		...
		FTPСоединение = ТранспортСообщенийОбмена.FTPСоединение(НастройкиFTP);
		
		АбсолютныйКаталогОбменаИнформацией = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(
			НастройкиТранспорта.FTPПуть,
			ОтносительныйКаталогОбменаИнформацией);

		Если Не ТранспортСообщенийОбмена.FTPКаталогСуществует(АбсолютныйКаталогОбменаИнформацией, ОтносительныйКаталогОбменаИнформацией, FTPСоединение) Тогда
...
// 
// Возвращаемое значение:
//  Строка - Событие журнала регистрации строкой
//
...
Процедура ВыполнитьВыгрузкуДанных(Отказ, Знач Корреспондент) Экспорт
	...
	ОбменДаннымиСервер.ВыполнитьОбменДаннымиДляУзлаИнформационнойБазы(
		Корреспондент, ПараметрыОбмена, Отказ);
...
Процедура ВыполнитьЗагрузкуДанных(Отказ, Знач Корреспондент, СообщениеДляСопоставленияДанных = Ложь) Экспорт
	...
	ПараметрыОбмена.ИдентификаторТранспорта = "SM";
	ПараметрыОбмена.СообщениеДляСопоставленияДанных = СообщениеДляСопоставленияДанных;
	...
	Если НастройкиТранспорта <> Неопределено Тогда
		...
		ИначеЕсли НастройкиТранспорта.ВидТранспортаСообщенийОбменаПоУмолчанию = Перечисления.ВидыТранспортаСообщенийОбмена.FTP Тогда
			...
			Попытка
				...
				Если ТранспортСообщенийОбмена.FTPКаталогСуществует(
					НастройкиТранспорта.FTPПуть,
					НастройкиТранспорта.ОтносительныйКаталогОбменаИнформацией, 
					FTPСоединение) Тогда
...
Процедура СоздатьНастройкуОбмена_3_0_1_1(НастройкиПодключения,
	...
	Попытка
		...
		НастройкиТранспорта.Вставить("КонечнаяТочкаКорреспондента", НастройкиПодключения.КонечнаяТочкаКорреспондента);
		НастройкиТранспорта.Вставить("НаименованиеКорреспондента", НастройкиПодключения.НаименованиеКорреспондента);
		НастройкиТранспорта.Вставить("ОбластьДанныхКорреспондента", НастройкиПодключения.ОбластьДанныхКорреспондента);
		НастройкиТранспорта.Вставить("ВнутренняяПубликация", Ложь);
				
		ТранспортСообщенийОбмена.СохранитьНастройкиТранспорта(Корреспондент, "SM", НастройкиТранспорта, Истина);
...
Функция ТребуемаяВерсияПлатформы() Экспорт
	...
	ВерсияПлатформы = СтандартныеПодсистемыСервер.ВерсияРежимаСовместимости();
	ВерсияПлатформы = СтрРазделить(ВерсияПлатформы, ".");
...
// 
// Параметры:
//  ПланыОбменаМетода - Массив из Строка - массив имен планов обмена, как в конфигурации
//  ДополнительныеСвойстваПланаОбмена - Строка - Дополнительные свойства плана обмена
//  ТолькоНеуспешные - Булево
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица монитора обмена данными
//
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе");
		Методы.Вставить("ВыполнитьДействиеСценарияОбменаДаннымиВоВторойИнформационнойБазе");
		Методы.Вставить("ВыполнитьОбменДанными");
		Методы.Вставить("УстановитьПризнакИзмененияДанных");
