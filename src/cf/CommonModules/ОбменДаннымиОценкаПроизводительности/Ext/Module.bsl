...
Процедура Инициализировать(КомпонентыОбмена, Анализ = Ложь) Экспорт
	...
	Если КомпонентыОбмена.УзелКорреспондентаОбъект = Неопределено Тогда
		...
		Если Не КомпонентыОбмена.УзелКорреспондента = Неопределено Тогда
	...
	Наименование = СтрШаблон(ШаблонНаименования,
		...
		КомпонентыОбмена.УзелКорреспондентаОбъект.Код);
	...
	ИмяВременногоКаталогаЗамеров = 
		ОбменДаннымиПовтИсп.КаталогВременногоХранилищаФайлов() + Строка(Новый УникальныйИдентификатор);
		
	СоздатьКаталог(ИмяВременногоКаталогаЗамеров);
		
	ИмяВременногоФайлаЗамеров = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(
		ИмяВременногоКаталогаЗамеров, "log.txt"); //АПК:441 временный файл будет удален при подведении итогов
		
	КомпонентыОбмена.ИмяВременногоКаталогаЗамеров = ИмяВременногоКаталогаЗамеров;
	...
	ЗаголовокЛога = НСтр("ru = 'Начало замера; Время выполнения; Тип события; Событие; Комментарий'");
...
Процедура Завершить(КомпонентыОбмена) Экспорт
	...
	ИмяВременногоКаталогаЗамеров = КомпонентыОбмена.ИмяВременногоКаталогаЗамеров;
	ИмяВременногоФайлаИтогов = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(ИмяВременногоКаталогаЗамеров, "total.txt");
	ИмяВременногоФайлаАрхива = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(ИмяВременногоКаталогаЗамеров, "archive.zip");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗамеров Цикл
		...
		Количество = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(Количество, 7, " ", "Слева");
	...
	УдалитьФайлы(ИмяВременногоКаталогаЗамеров);
