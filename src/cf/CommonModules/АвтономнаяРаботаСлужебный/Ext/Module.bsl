...
//  Булево - Признак, требуется ли показывать вопрос о долгой синхронизации
//
...
// 
// Возвращаемое значение:
//  Строка - Значение константы АдресДляВосстановленияПароляУчетнойЗаписи
//
...
// 
// Возвращаемое значение:
//  Булево - Истина, если в приложении содержится один план обмена автономной работы
//
...
// 
// Возвращаемое значение:
//  - Неопределено - см. ОбменДаннымиСервер.ГлавныйУзел
//  - ПланОбменаСсылка
//
...
// 
// Возвращаемое значение:
//  Строка - Имя объекта метаданных ПланОбмена, который является автономным рабочим местом
//  
...
// 
// Параметры:
//  АвтономноеРабочееМесто - ПланОбменаСсылка - ссылка на узел плана обмена,
// 
// Возвращаемое значение:
//  - Неопределено - Если не заполнена дата последней синхронизации по переданному узлу
//  - Дата
//
...
// 
// Параметры:
//  ПоследнийПрефикс - Строка
// 
// Возвращаемое значение:
//  Строка - Префикс автономного рабочего места
//
...
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Монитор автономных рабочих мест
//
...
// 
// Возвращаемое значение:
//  Строка - Событие журнала регистрации создание автономного рабочего места
...
// 
// Параметры:
//  ИмяМакета - Строка - Имя макета обработки ПомощникСозданияАвтономногоРабочегоМеста
// 
// Возвращаемое значение:
//  Строка 
//
...
// 
// Параметры:
//  ЗагрузкаДанных - Булево - Значение по умолчанию - Ложь.
//
...
Процедура ОтключитьАвтоматическуюСинхронизациюДанныхСПриложениемВИнтернете(Источник) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Параметры.Корреспондент = Источник.Корреспондент;
		Параметры.ИдентификаторТранспорта = Источник.ИдентификаторТранспорта;
		
		Транспорт = ТранспортСообщенийОбмена.Инициализация(Параметры, Ложь);
		
		Если Транспорт.ТребуетсяАутентификация() Тогда
...
//    *ДниНедели - Массив из Число
//    *ПериодПовтораВТечениеДня - Число - 60 минут.
...
// Возвращаемое значение:
//  Структура:
// * УзелИнформационнойБазы - см. ПриложениеВСервисе 
// * АдресДляВосстановленияПароляУчетнойЗаписи - см. АдресДляВосстановленияПароляУчетнойЗаписи
// * ЗакрытьПриУспешнойСинхронизации - Булево - По умолчанию - Истина.
//
...
// 
// Параметры:
//  Интервал - Число - По умолчанию - 1 час.
// 
// Возвращаемое значение:
//  Булево 
// 
...
Процедура ЗаписатьЭлементДанныхНачальногоОбраза(ЭлементДанных, СвойстваМетаданных, Получатель) Экспорт 
	...
	Исключение
		...
		Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеЭлемента) Тогда
			Шаблон = НСтр("ru = 'Навигационная ссылка: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
			Ссылка = СтрШаблон(Шаблон, ПолучитьНавигационнуюСсылку(ЭлементДанных.Ссылка));
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеЭлемента) Тогда
			Шаблон = НСтр("ru = 'Отбор: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
			Ссылка = СтрШаблон(Шаблон, ЭлементДанных.Отбор);
		Иначе
			Ссылка = "";
		КонецЕсли;
		
		Шаблон = НСтр(
			"ru = 'Ошибка при записи элемента ""%1"" при создании начального образа
		     |%2 
		     |Подробности см. в журнале регистрации.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
			
		ТекстИсключения = СтрШаблон(Шаблон, ЭлементДанных, Ссылка);
...
Процедура ЗакрытьЗаписьДанныхНачальногоОбраза(Получатель, НачальныйОбразСформирован = Ложь) Экспорт 
	...
	ИначеЕсли ПараметрыВыгрузки.ИспользоватьОптимизированнуюЗапись Тогда
		...
		ИначеЕсли ПараметрыВыгрузки.ЗаписаноЭлементовПослеПроверкиРазмераФайла >= ПараметрыВыгрузки.КоличествоЭлементовПроверкиРазмераФайла Тогда
			...
			Если ОбменДаннымиСлужебный.ПутьКФайлуСодержитНедопустимыеСимволы(ПараметрыВыгрузки.ИмяФайлаДанных) Тогда
...
Процедура ЗагрузитьПараметрыИзНачальногоОбраза()
	...
	Попытка
		...
		ЗаблокироватьДанныеДляРедактирования(ГлавныйУзелСсылка);
		...
		СсылкаУзелАвтономногоРабочегоМеста = ПланыОбмена[ПланОбменаАвтономнойРаботы()].ЭтотУзел();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить(ОбщегоНазначения.ИмяТаблицыПоСсылке(СсылкаУзелАвтономногоРабочегоМеста));
		ЭлементБлокировки.УстановитьЗначение("Ссылка", СсылкаУзелАвтономногоРабочегоМеста);
		Блокировка.Заблокировать();
		
		ЗаблокироватьДанныеДляРедактирования(СсылкаУзелАвтономногоРабочегоМеста);
		
		УзелАвтономногоРабочегоМеста = СсылкаУзелАвтономногоРабочегоМеста.ПолучитьОбъект();
	...
	Попытка
		...
		Корреспондент = ПриложениеВСервисе();
		НастройкиТранспорта = Новый Структура();
		НастройкиТранспорта.Вставить("АдресВебСервиса", Параметры.URL);
		ТранспортСообщенийОбмена.СохранитьНастройкиТранспорта(Корреспондент, "WS", НастройкиТранспорта, Истина);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СоздатьНачальныйОбразАвтономногоРабочегоМеста", Истина);
	Методы.Вставить("УдалитьАвтономноеРабочееМесто", Истина);
