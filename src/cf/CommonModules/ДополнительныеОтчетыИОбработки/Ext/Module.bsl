...
#КонецЕсли
	...
	Если РежимПодключения = Неопределено Тогда
		...
		ИмяОбработки = Менеджер.Подключить(АдресВоВременномХранилище, , БезопасныйРежимПервогоПодключения, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
		ВнешнийОбъект = Менеджер.Создать(ИмяОбработки); // Не нарушает безопасность информации, так как создается уже подключенный объект.
		...
		Если (Не СведенияОВнешнейОбработке.БезопасныйРежим Или ЕстьРазрешения)
			...
			ИмяОбработки = Менеджер.Подключить(АдресВоВременномХранилище, , БезопасныйРежим, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
			
			РежимПодключения = БезопасныйРежим;
			СохранитьРежимПодключенияОтчетаОбработки(Ссылка, ДвоичныеДанные, БезопасныйРежим);
		КонецЕсли;
	Иначе
		ИмяОбработки = Менеджер.Подключить(АдресВоВременномХранилище, , РежимПодключения, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
	КонецЕсли;
...
Функция ОбъектВнешнейОбработки(Ссылка) Экспорт
	...
	Возврат Менеджер.Создать(ИмяОбработки); // Не нарушает безопасность информации, так как создается уже подключенный объект.
...
Процедура СохранитьНастройки(Ссылка, Настройки) Экспорт
	...
	Попытка
		...
		ДополнительнаяОбработкаОбъект = Ссылка.ПолучитьОбъект();
		ДополнительнаяОбработкаОбъект.ХранилищеНастроек = Новый ХранилищеЗначения(Настройки, Новый СжатиеДанных(9));
...
//                      СправочникСсылка.ИдентификаторыОбъектовРасширений - ссылка объекта метаданных.
...
// Возвращает таблицу команд дополнительных отчетов или обработок.
//
// Параметры:
...
//   ЭтоФормаОбъекта - Булево - тип форм, в которых размещены контекстные дополнительные отчеты и обработки.
...
//   ТолькоВключенные - Булево - Истина, если нужно вернуть только включенные пользователем в настройках.
//                               Ложь, если нужно вернуть все.
//
// Возвращаемое значение:
//   ТаблицаЗначений:
...
//
Функция ДополнительныеКоманды(Знач ВидОбработок, Знач Размещение, Знач ЭтоФормаОбъекта = Неопределено, 
	Знач ТипыКоманд = Неопределено, Знач ТолькоВключенные = Истина) Экспорт
	
	Запрос = Новый Запрос;
	
	Если ТипЗнч(Размещение) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		ИЛИ ТипЗнч(Размещение) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		СсылкаРодителяИлиРаздела = Размещение;
	Иначе
		СсылкаРодителяИлиРаздела = ?(ЗначениеЗаполнено(Размещение),
			ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Размещение),
			Неопределено);
	КонецЕсли;
...
// Возвращает таблицу команд загрузки данных из файла.
//
// Возвращаемое значение:
//   см. ДополнительныеКоманды
//
Функция ДополнительныеКомандыЗагрузкиДанныхИзФайла() Экспорт

	Возврат ДополнительныеКоманды(Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительнаяОбработка,
		Неопределено, Ложь, Перечисления.СпособыВызоваДополнительныхОбработок.ЗагрузкаДанныхИзФайла);

КонецФункции

// Возвращает таблицу команд печати.
//
// Параметры:
//   Размещение - СправочникСсылка.ИдентификаторыОбъектовМетаданных
//              - Строка - ссылка или полное имя объекта метаданных, к которому привязаны 
//       искомые дополнительные отчеты и обработки.
//   ЭтоФормаОбъекта - Булево - тип форм, в которых размещены контекстные дополнительные отчеты и обработки.
//       Истина - только отчеты и обработки, привязанные к формам объектов.
//       Ложь - только отчеты и обработки, привязанные к формам списков.
//
...
//   см. ДополнительныеКоманды
//
Функция ДополнительныеКомандыПечати(Знач Размещение, Знач ЭтоФормаОбъекта = Неопределено) Экспорт

	Возврат ДополнительныеКоманды(Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма, Размещение, 
		ЭтоФормаОбъекта);
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.3.3.3", 
		"Роль.ИспользованиеДополнительныхОтчетовИОбработок", "Роль.ЧтениеДополнительныхОтчетовИОбработок", "СтандартныеПодсистемы");
...
Процедура ПриУдаленииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") 
		Или Настройки.ИдентификаторНастройки <> "НастройкаБыстрогоДоступа" Тогда
		Возврат;
	КонецЕсли;
	...
	Блокировка = Новый БлокировкаДанных();
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ПользовательскиеНастройкиДоступаКОбработкам");
	ЭлементБлокировки.УстановитьЗначение("Пользователь", СведенияОПользователе.ПользовательСсылка);
	...
	Попытка
		...
		Если ПакетныйРежим Тогда
			УдаляемыеЗаписи = РегистрыСведений.ПользовательскиеНастройкиДоступаКОбработкам.СоздатьНаборЗаписей();
			Для Каждого ЗначениеНастройки Из Настройки.ЗначениеНастройки Цикл
				УдаляемаяЗапись = УдаляемыеЗаписи.Добавить();
				УдаляемаяЗапись.ДополнительныйОтчетИлиОбработка  = ЗначениеНастройки.Значение;
				УдаляемаяЗапись.ИдентификаторКоманды             = ЗначениеНастройки.Представление;
				УдаляемаяЗапись.Пользователь                     = СведенияОПользователе.ПользовательСсылка;
			КонецЦикла;
			УдаляемыеЗаписи.Записать(РежимУдаления);
		Иначе
			Для Каждого ЗначениеНастройки Из Настройки.ЗначениеНастройки Цикл
				УдаляемыеЗаписи = РегистрыСведений.ПользовательскиеНастройкиДоступаКОбработкам.СоздатьНаборЗаписей();
				УдаляемыеЗаписи.Отбор.ДополнительныйОтчетИлиОбработка.Установить(ЗначениеНастройки.Значение);
				УдаляемыеЗаписи.Отбор.ИдентификаторКоманды.Установить(ЗначениеНастройки.Представление);
				УдаляемыеЗаписи.Отбор.Пользователь.Установить(СведенияОПользователе.ПользовательСсылка);
...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент ""%1"" не подключен, потому что ""Дополнительные отчеты и обработки"" отключены в настройках приложения.'"),
			"'" + Строка(Ссылка) + "'");
		Возврат;
	КонецЕсли;
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Попытка
			...
			ПараметрыОтчета.Объект = ВнешниеОтчеты.Создать(ПараметрыОтчета.Имя); // Не нарушает безопасность информации, так как создается уже подключенный объект.
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение НСтр("ru = '""Дополнительные отчеты и обработки"" отключена в настройках приложения.'");
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках приложения.'"),
...
Процедура ПриПолученииКомандПечати(КомандыПечати, ИмяОбъекта) Экспорт
	...
	ТаблицаКоманд = ДополнительныеКоманды(Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма, ИмяОбъекта);
...
Процедура ПриПолученииСпискаВнешнихПечатныхФорм(ВнешниеПечатныеФормы, ПолноеИмяОбъектаМетаданных) Экспорт
	...
	ТаблицаКоманд = ДополнительныеКоманды(Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма, 
...
Процедура ПриПолученииВнешнейПечатнойФормы(Идентификатор, ПолноеИмяОбъектаМетаданных, ВнешняяПечатнаяФормаСсылка) Экспорт
	...
	ТаблицаКоманд = ДополнительныеКоманды(Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма, ПолноеИмяОбъектаМетаданных);
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	Методы.Вставить("ОбработчикКомандыЗаполнения");
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ДополнительныеОтчетыИОбработки", Методы);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(
		...
		Методы = Новый Соответствие;
		Методы.Вставить("ЗаблокироватьДополнительныеОтчетыИОбработкиДляОбновления");
		Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить(
			"ОбщийМодуль.ДополнительныеОтчетыИОбработкиВМоделиСервиса", Методы);
	КонецЕсли;
...
Функция ТипДополнительныйОтчетИлиОбработка() Экспорт
...
Функция ПредставлениеОбъектаМетаданных(Объект) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		КонецЕсли;
	Иначе
		ОбъектМетаданных = ?(ТипЗнч(Объект) = Тип("ОбъектМетаданных"), Объект, Метаданные.Подсистемы.Найти(Объект));
	КонецЕсли;
	Возврат ОбщегоНазначения.ПредставлениеСписка(ОбъектМетаданных);
...
#Область ПроцедурыИспользуемыеПриОбменеДанными
...
#Область СопоставлениеЭлементовСправочникаОбъектамМетаданныхКонфигурации
...
#Область ВыгрузкаОтчетовИОбработокКонфигурацииВФайлыВнешнихОтчетовИОбработок
...
#Область ПакетнаяРегистрацияВнешнихОтчетовИОбработокВСправочнике
...
Процедура ЗарегистрироватьОтчетыИОбработки(ОтчетыИОбработки)
	...
	Для Каждого СтрокаТаблицы Из ОтчетыИОбработки Цикл
		...
		ИмяОбъекта = Менеджер.Подключить(АдресДанных, , Истина, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
		ВнешнийОбъект = Менеджер.Создать(ИмяОбъекта); // Не нарушает безопасность информации, так как создается уже подключенный объект.
		...
		СправочникОбъект.ХранилищеОбработки = Новый ХранилищеЗначения(СтрокаТаблицы.ДвоичныеДанные, Новый СжатиеДанных(9));
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Для Каждого Разрешение Из РегистрационныеДанные.Разрешения Цикл
		...
		КонецЦикла;
		СтрокаТЧ.Параметры = Новый ХранилищеЗначения(Параметры, Новый СжатиеДанных(9));
...
#Иначе
		РезультатРегистрации.ИмяОбъекта =
			СокрЛП(Менеджер.Подключить(ПараметрыРегистрации.АдресДанныхОбработки, , Истина, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений())); // Не нарушает безопасность информации, так как подключается только внешний код, подтвержденный администратором.
#КонецЕсли
		
		// Получение сведений о внешней обработке.
		ВнешнийОбъект = Менеджер.Создать(РезультатРегистрации.ИмяОбъекта); // Не нарушает безопасность информации, так как создается уже подключенный объект.
...
#КонецЕсли
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		Иначе
			...
			|Возможно, она не подходит для этой версии приложения.'");
		КонецЕсли;
...
//   ** ОбъектНазначения - СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений
//
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ВыполнитьОбработкуПоРегламентномуЗаданию");
	КонецЕсли;
	
	Методы.Вставить("ВыполнитьКоманду", Истина);
