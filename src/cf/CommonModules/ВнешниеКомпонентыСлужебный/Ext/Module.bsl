...
Функция ПредставлениеКомпоненты(Идентификатор, Версия) Экспорт
	...
	Если ЗначениеЗаполнено(Версия) Тогда
		ПредставлениеКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (версии %2)'"), Идентификатор, Версия);
	Иначе
		ПредставлениеКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (последней версии)'"), Идентификатор);
	КонецЕсли;
		...
		Если Не СтрНачинаетсяС(Местоположение, "e1cib/data/Справочник.ВнешниеКомпоненты.ХранилищеКомпоненты") Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ВнешниеКомпонентыВМоделиСервиса") Тогда
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключить компоненту ""%1"" по причине:
...
Функция ОшибкаСовместимостиКомпонентыИзМакета(Местоположение) Экспорт
	...
	ИнформацияОВнешнейКомпоненте = ИнформацияОВнешнейКомпонентеИзМакета(Местоположение);
		 
	Если ИнформацияОВнешнейКомпоненте <> Неопределено 
		И Не ОперационнаяСистемаПоддерживаетсяКомпонентой(ИнформацияОВнешнейКомпоненте.Реквизиты.ЦелевыеПлатформы) Тогда
		Возврат ОписаниеОшибкиСовместимости();
	КонецЕсли;
...
//
Процедура ЗагрузитьКомпонентуИзДвоичныхДанных(Параметры, ВыполнятьРазборИнфоФайла = Истина, 
	ИспользуемыеВнешниеКомпоненты = Неопределено) Экспорт
	
	Если ТипЗнч(Параметры.Данные) = Тип("Строка") Тогда
		Если ПустаяСтрока(Параметры.Данные) Или Не ЭтоАдресВременногоХранилища(Параметры.Данные) Тогда
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не указан адрес временного хранилища в параметре %1 при вызове %2.'"),
				"Параметры.Данные", "ЗагрузитьКомпонентуИзДвоичныхДанных");
	...
	Если ТипЗнч(ДвоичныеДанные) <> Тип("ДвоичныеДанные") Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Данные во временном хранилище в параметре %1 не являются двоичными данными при вызове %2.'"),
			"Параметры.Данные", "ЗагрузитьКомпонентуИзДвоичныхДанных");
		ВызватьИсключение(ТекстИсключения, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Идентификатор) Тогда
		ТекстИсключения = НСтр("ru = 'Не указан идентификатор компоненты, необходимо ввести его вручную.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Попытка
		...
		Объект.Наименование = ?(ЗначениеЗаполнено(Параметры.Наименование), Параметры.Наименование, 
			Информация.Реквизиты.Наименование);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "3.1.10.107";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("25e8efe8-37d5-47b4-a3c6-7e5277161b95");
...
// Возвращает внешние компоненты, используемые в конфигурации.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Идентификатор - Строка
...
//    "ДляОбновления" - компоненты только из справочника с установленным признаком загрузки из Интернета.
...
Функция ДанныеВнешнихКомпонент(Вариант = "ДляОбновления") Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 (%2) при вызове %3.'"), 
			"Вариант", Вариант, "ВнешниеКомпонентыСлужебный.ДанныеВнешнихКомпонент");
	КонецЕсли;
...
Процедура ДобавитьВерсииКомпонент(СписокВерсий, Выборка)
	...
	Пока Выборка.Следующий() Цикл
		СписокВерсий.Добавить(НРег(Выборка.Ссылка.УникальныйИдентификатор()) + " " + Выборка.ВерсияДанных);
	КонецЦикла;
...
//
Функция ПроверитьПодключениеКомпоненты(Знач Идентификатор, Знач Версия = Неопределено,
	...
	Информация = ВнешниеКомпонентыСлужебныйВызовСервера.ИнформацияОСохраненнойКомпоненте(Идентификатор, Версия, 
		ПараметрыПодключения.ПолноеИмяМакета);
	...
	ИначеЕсли Информация.Состояние = "НеНайдена" Тогда
		...
		Возврат Результат;
	ИначеЕсли Информация.ЗаполненыЦелевыеПлатформы 
		И Не ОперационнаяСистемаПоддерживаетсяКомпонентой(Информация.Реквизиты.ЦелевыеПлатформы) Тогда
		Результат.ОписаниеОшибки = ОписаниеОшибкиСовместимости();
		Возврат Результат;
	КонецЕсли;
...
Функция ОписаниеОшибкиСовместимости()
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ИмяТипаПлатформы = ОбщегоНазначенияКлиентСервер.ИмяТипаПлатформы(СистемнаяИнформация.ТипПлатформы);
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не предусмотрена работа компоненты в операционной системе %1.'"), ИмяТипаПлатформы);
	
КонецФункции

Функция ИнформацияОВнешнейКомпонентеИзМакета(Местоположение) Экспорт
	
	Если Не ОбщегоНазначения.МакетСуществует(Местоположение) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	МестоположениеМакетаРазделенное = СтрРазделить(Местоположение, ".");
	ИмяМакета = МестоположениеМакетаРазделенное.Получить(МестоположениеМакетаРазделенное.ВГраница());
	
	Если МестоположениеМакетаРазделенное.Количество() = 2 Тогда
		ДвоичныеДанныеКомпоненты = ПолучитьОбщийМакет(ИмяМакета);
	Иначе
		МестоположениеМакетаРазделенное.Удалить(МестоположениеМакетаРазделенное.ВГраница());
		МестоположениеМакетаРазделенное.Удалить(МестоположениеМакетаРазделенное.ВГраница());
		МенеджерОбъектаПоПолномуИмени =  ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
			СтрСоединить(МестоположениеМакетаРазделенное, "."));
		ДвоичныеДанныеКомпоненты = МенеджерОбъектаПоПолномуИмени.ПолучитьМакет(ИмяМакета);
	КонецЕсли;
	
	ИнформацияОКомпонентеИзФайла = ИнформацияОКомпонентеИзФайла(ДвоичныеДанныеКомпоненты);
	Если Не ИнформацияОКомпонентеИзФайла.Разобрано Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ИнформацияОКомпонентеИзФайла;
...
//    * ПоследняяВерсияКомпонентыИзМакета - см. СтандартныеПодсистемыПовтИсп.ПоследняяВерсияКомпонентыИзМакета
//
Функция ИнформацияОСохраненнойКомпоненте(Идентификатор, Версия = Неопределено, 
	ПутьКМакетуДляПоискаПоследнейВерсии = Неопределено) Экспорт
	...
	Если РеквизитыОбъекта.ЦелевыеПлатформы = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка совместимости компоненты'",
			ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение, , Результат.Ссылка, 
			ТекстПредупреждения);
...
Процедура ЗаполнитьРеквизитыПоManifestXML(ИмяФайлаManifestXML, Реквизиты)
	...
	Если ЧтениеXML.Имя <> "bundle" Или ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ЧтениеXML.Закрыть();
		Возврат;
	КонецЕсли;
	
	Пока ЧтениеXML.Прочитать() Цикл
		Если ЧтениеXML.Имя <> "component" Или ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ОперационнаяСистема = "linux" И ТипКомпоненты = "plugin" Тогда
			...
			Если АрхитектураПлатформы = "i386" И (ПрограммаПросмотра = "yandexbrowser" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_x86_ЯндексБраузер = Истина;
			КонецЕсли;
			
			Если АрхитектураПлатформы = "i386" И (ПрограммаПросмотра = "chrome" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_x86_Chrome = Истина;
			КонецЕсли;

			Если АрхитектураПлатформы = "x86_64" И (ПрограммаПросмотра = "yandexbrowser" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_x86_64_ЯндексБраузер = Истина;
			КонецЕсли;

			Если АрхитектураПлатформы = "x86_64" И (ПрограммаПросмотра = "chrome" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_x86_64_Chrome = Истина;
			КонецЕсли;
			
			Если АрхитектураПлатформы = "arm64" И (ПрограммаПросмотра = "yandexbrowser" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_ARM64_ЯндексБраузер = Истина;
			КонецЕсли;

			Если АрхитектураПлатформы = "arm64" И (ПрограммаПросмотра = "chrome" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_ARM64_Chrome = Истина;
			КонецЕсли;
			
			Если АрхитектураПлатформы = "e2k" И (ПрограммаПросмотра = "yandexbrowser" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.Linux_E2K_ЯндексБраузер = Истина;
			КонецЕсли;

			Если АрхитектураПлатформы = "e2k" И (ПрограммаПросмотра = "chrome" 
				Или ПрограммаПросмотра = "anychromiumbased") Тогда
		...
		Если ОперационнаяСистема = "macos" И ТипКомпоненты = "plugin" Тогда
			...
			Если  (АрхитектураПлатформы = "x86_64" Или АрхитектураПлатформы = "universal") 
			...
			Если (АрхитектураПлатформы = "x86_64" Или АрхитектураПлатформы = "universal") 
				И (ПрограммаПросмотра = "chrome" Или ПрограммаПросмотра = "anychromiumbased") Тогда
				Реквизиты.MacOS_x86_64_Chrome = Истина;
			КонецЕсли;
			
			Если (АрхитектураПлатформы = "x86_64" Или АрхитектураПлатформы = "universal") 
				И (ПрограммаПросмотра = "yandexbrowser" Или ПрограммаПросмотра = "anychromiumbased") Тогда
...
Процедура НоваяКомпонентаСПортала(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатОперации.КодОшибки) Тогда
		ТекстИсключения = ?(Пользователи.ЭтоПолноправныйПользователь(), РезультатОперации.ИнформацияОбОшибке, 
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("НоваяКомпонентаСПортала", Истина);
	Методы.Вставить("ОбновитьКомпонентыСПортала", Истина);
