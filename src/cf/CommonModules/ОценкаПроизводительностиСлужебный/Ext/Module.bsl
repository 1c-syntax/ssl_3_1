...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ВыполнятьЗамерыПроизводительности;
	Настройка.РаботаетСВнешнимиРесурсами = Истина;
	Настройка.Параметризуется = Истина;
	
	Настройка = Настройки.Добавить();
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОчисткаЗамеровВремени;
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ВыполнятьЗамерыПроизводительности;
	Настройка.РаботаетСВнешнимиРесурсами = Истина;
КонецПроцедуры
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ЕстьБазоваяФункциональность() Тогда
		// АПК:488-выкл ОбщегоНазначения.ВычислитьВБезопасномРежиме не используется, так нет технической возможности.
		УстановитьБезопасныйРежим(Истина);
		МодульОбщегоНазначения = Вычислить("ОбщегоНазначения"); // Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только имя общего модуля.
		УстановитьБезопасныйРежим(Ложь);
		// АПК:488-вкл
		Модуль = МодульОбщегоНазначения.ОбщийМодуль(Имя);
	Иначе
		Если Метаданные.ОбщиеМодули.Найти(Имя) <> Неопределено Тогда
			// АПК:488-выкл ОбщегоНазначения.ВычислитьВБезопасномРежиме не используется, так как нет БСП.
			УстановитьБезопасныйРежим(Истина);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьЗначенияКонстанты_3_1_3_38");
