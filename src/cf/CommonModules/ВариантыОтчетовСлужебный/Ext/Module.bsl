...
Функция ПредставлениеПолейГруппировки(Группировка, ПометкаУдаления = Ложь) Экспорт 
	...
	Для Каждого Элемент Из Поля.Элементы Цикл 
		...
		Если ТипЗнч(Элемент) = Тип("АвтоПолеГруппировкиКомпоновкиДанных") Тогда 
...
Функция ИндексСтруктурыОтчета(Форма) Экспорт
	...
	Если ИндексСтруктурыОтчета <> Неопределено И Не Форма.ВариантМодифицирован Тогда 
		Возврат ИндексСтруктурыОтчета;
	КонецЕсли;
	
	СвойстваРезультата = НастройкиОтчета.СвойстваРезультата;
	
	КомпоновщикНастроекОтчета = Форма.Отчет.КомпоновщикНастроек;
	КомпоновщикНастроекОтчета.РазвернутьАвтоПоля();
	
	ИтоговыеНастройки = КомпоновщикНастроекОтчета.ПолучитьНастройки();
	...
	ИндексСтруктурыОтчета = ИндексСтруктурыОтчетаБезКонтекста(КомпоновщикНастроекОтчета, ИтоговыеНастройки);
...
Функция ПоляСортировки(Группировка, Настройки, ПоляСортировки = Неопределено, Замещать = Истина)
	...
	Для Каждого Элемент Из ЭлементыСортировки Цикл 
		...
		Если Замещать Или ПоляСортировки[Элемент.Поле] = Неопределено Тогда 
...
Процедура ИндексироватьПоляГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки,
	...
	Для Каждого Элемент Из ПоляГруппировки.Элементы Цикл 
		...
		Если Не ВариантыОтчетовСлужебныйКлиентСервер.ПолеСодержитсяВГруппировкеОтчета(Группировка.Выбор, Элемент.Поле)
...
Процедура ИндексироватьВыбранныеПоляНастроекОтчета(ИндексСтруктурыОтчета, ИндексГруппировки, Настройки,
	...
	Для Каждого Элемент Из Поля.Элементы Цикл 
		...
		ИначеЕсли РазрешеноИспользоватьПолеВГруппировке(Группировка, Элемент.Поле)
