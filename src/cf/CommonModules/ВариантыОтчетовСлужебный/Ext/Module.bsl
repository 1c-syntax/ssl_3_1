...
Процедура ИнициализироватьЗаголовкиОтчета(Форма) Экспорт 
	...
	Для НомерСтроки = 1 По РезультатОтчета.ВысотаТаблицы Цикл 
		...
		Для НомерКолонки = 1 По РезультатОтчета.ШиринаТаблицы Цикл 
			...
			Если НЕ ОбщегоНазначения.ЭтоМобильныйКлиент() ИЛИ ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияРежимаСовместимости, "8.3.19.0") >= 0 
			   И Форма.ПолучитьТекущийРежимОтображенияРезультата() = РежимОтображенияРезультатаОтчета.Обычный Тогда
...
//  ПоляСортировки - Соответствие из КлючЗначение:
