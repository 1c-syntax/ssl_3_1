...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОбновлениеПредопределенныхВариантовОтчетов;
...
Функция ИндексСтруктурыОтчета(Форма) Экспорт
	...
	Если ИндексСтруктурыОтчета = Неопределено
		...
		ОпределитьДоступныеДействияПолейОтчета(Настройки, ИндексСтруктурыОтчета);
...
Процедура ОпределитьДоступныеДействияПолейОтчета(Настройки, ИндексСтруктурыОтчета)
	...
	Для Каждого ПорядокРаздела Из ПорядокРазделов Цикл 
		...
		ОпределитьДоступныеДействияПолейРазделаОтчета(Настройки, ИндексСтруктурыОтчета, ПорядокРаздела);
		ОпределитьДоступныеДействияПолейРазделаОтчета(Настройки, ИндексСтруктурыОтчета, ПорядокРаздела, Истина);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ОпределитьДоступныеДействияПолейРазделаОтчета(Настройки, ИндексСтруктурыОтчета, ПорядокРаздела, ЭтоРесурс = Ложь)
	...
	Для Каждого ПорядокГруппировки Из ПорядокГруппировок Цикл 
		...
		Для Каждого ПолеГруппировки Из ПоляГруппировки Цикл 
			...
			ЕстьДоступноеПолеГруппировкиВыше = Ложь;
			Если ПоляГруппировкиВыше <> Неопределено Тогда
				Для Каждого ПолеГруппировкиВыше Из ПоляГруппировкиВыше Цикл
					Если Настройки.ДоступныеПоляВыбора.Элементы.Найти(ПолеГруппировкиВыше.Поле) <> Неопределено
					   Или Настройки.ДоступныеПоляГруппировок.Элементы.Найти(ПолеГруппировкиВыше.Поле) <> Неопределено Тогда
						ЕстьДоступноеПолеГруппировкиВыше = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			...
			Индекс.ПереместитьПолеВыше = Не ЭтоПолеГруппировкиКолонки
				...
				И ЕстьДоступноеПолеГруппировкиВыше;
...
Функция ДанныеЭлементаРасшифровки(Форма, Расшифровка) Экспорт 
	...
	Иначе
		ТипЭлементаРасшифровки = ВариантыОтчетовСлужебныйКлиентСервер.ТипРеквизит();
		Если ТипЗнч(Родитель) = Тип("ЭлементРасшифровкиКомпоновкиДанныхГруппировка") Тогда
			Для Каждого Группировка Из Данные.Настройки.Структура Цикл
				Если СтрСравнить(Родитель.Группировка, Группировка.Идентификатор) = 0 И ТипЗнч(Группировка) = Тип("ГруппировкаКомпоновкиДанных") Тогда
					РасшифровкаДоступна = Группировка.ПоляГруппировки.Элементы.Количество() > 0;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	ДанныеЭлементаРасшифровки.Вставить("РасшифровкаДоступна", РасшифровкаДоступна);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьЗначенияФильтра", Истина);
