...
Процедура УстановитьНастройкиРезервногоКопирования(Знач Настройки, Знач Пользователь = Неопределено) Экспорт
	...
	Если Пользователь <> Неопределено Тогда
		...
		Константы.ПараметрыРезервногоКопирования.Установить(Новый ХранилищеЗначения(ПараметрыКопирования, Новый СжатиеДанных(9)));
	КонецЕсли;
КонецПроцедуры
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Иначе
			...
			Если НастройкиРезервногоКопирования.РезервноеКопированиеНастроено Тогда
				ТекущаяНастройка = НСтр("ru = 'Резервное копирование не выполняется (организовано сторонними приложениями).'");
			КонецЕсли;
...
//
// Возвращаемое значение:
//   см. НовыеНастройкиРезервногоКопирования
//
Функция ПараметрыРезервногоКопирования() Экспорт
	...
	НовыеПараметры = НовыеНастройкиРезервногоКопирования();
	Если Параметры = Неопределено Тогда
		УстановитьНастройкиРезервногоКопирования(НовыеПараметры);
	Иначе
...
Функция ПодготовитьОбщиеПараметрыСкрипта(Знач ПараметрыСкрипта) Экспорт
	...
	ПараметрыТекста["[ПутьКCOMСоединителю]"] = ПодготовитьТекст(ПараметрыСкрипта.ПутьКCOMСоединителю);
...
Функция СообщенияСкриптов()
	...
	Сообщения["[СообщениеОтказПереносаФайлаБазыВоВременныйКаталог]"] = НСтр("ru = 'Файл базы не был перенесен во временный каталог (возможно в приложении есть активные сеансы): {0}, {1}.'");
