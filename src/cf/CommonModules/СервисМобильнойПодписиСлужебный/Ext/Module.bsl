...
Процедура ПроверкаПараметровПодписанта(ПараметрыПодписанта, ДляДобавления = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыПодписанта.ВидПодписанта) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ПараметрыПодписанта.ИННФЛ) И ЗначениеЗаполнено(ПараметрыПодписанта.ОГРН) Тогда
			ПараметрыПодписанта.ВидПодписанта = Перечисления.ВидыПодписантовМобильногоСервиса.ИндивидуальныйПредприниматель;
		Иначе
			ПараметрыПодписанта.ВидПодписанта = Перечисления.ВидыПодписантовМобильногоСервиса.ФизическоеЛицо;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьСтатусПодписанияВГосключе(ИдентификаторЗадачи, Настройка, ТокенДоступа = "", Ошибка = Null)
	...
	ЗаголовокОшибки = НСтр("ru='Не удалось получить информацию о подписании документов в Госключе'");
	...
	Если ОтветHTTP.КодСостояния = 200 Тогда
		...
		Если Не ПустаяСтрока(ОшибкаИзСервиса) Тогда
			Если ОшибкаЗаявлениеОжидаетОтправки(ОшибкаИзСервиса) Тогда
				ЗаголовокОшибки = НСтр("ru='Ожидается отправка документов на подпись в Госключ'");
			...
			Иначе
				ЗаголовокОшибки = НСтр("ru='Отправка документов на подпись была отменена'");
				ЗаписьЖурналаРегистрации(ВидОперации, УровеньЖурналаРегистрации.Ошибка,,,
				ЗаголовокОшибки + Символы.ПС + ОшибкаИзСервиса + Символы.ПС + ОтветHTTP.ПолучитьТелоКакСтроку());
				Если Ошибка <> Null Тогда
					Ошибка = ОшибкаМобильногоСервиса(ЗаголовокОшибки, ОшибкаИзСервиса, ИдентификаторЗапроса, СоединениеHTTP, ЗапросHTTP, ОтветHTTP);
					ЗаписатьРезультатПодписанияВРегистрСтатусов(ИдентификаторЗадачи, Ошибка, Перечисления.СервисМобильнойПодписиСтатусы.Отклонено);
					Возврат;
				КонецЕсли;
				ВызватьИсключение ОшибкаИзСервиса;
			КонецЕсли;
		КонецЕсли;
...
Функция ОшибкаЗаявлениеОжидаетОтправки(Ошибка)
	Возврат СтрНачинаетсяС(Ошибка, "Дождитесь отправки заявления"); // АПК:1297 - сообщение из сервиса.
КонецФункции
...
Процедура ОбработатьОшибкуСервиса(СтруктураОтвета)
	...
	СообщениеОбОшибке = "";
	Ошибки = Новый Массив;
	Если ТипЗнч(СтруктураОтвета.error) = Тип("Массив") Тогда
		...
		КонецЦикла;
	Иначе
		Ошибки.Добавить(ЗаписатьЗначениеJSON(СтруктураОтвета.error));
	КонецЕсли;
...
Функция ИдентификаторСхемыЗапросаВГосключ(ПараметрыПодписанта)
...
Функция ЗапросНаОтправкуДокумента(ПараметрыПодписанта, ПараметрыОтправителя) Экспорт
	...
	ИдентификаторСхемыЗапросаВГосключ = ИдентификаторСхемыЗапросаВГосключ(ПараметрыПодписанта);
	
	Запрос = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ИдентификаторСхемыЗапросаВГосключ, "SignRequestType"));
	...
	Если ЗначениеЗаполнено(ПараметрыПодписанта.НомерЗаписиОбАккредитации) Тогда
		ПараметрыОрганизации = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ИдентификаторСхемыЗапросаВГосключ, "ForeignOrgType"));
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ПараметрыПодписанта.ОГРН) Тогда
			ПараметрыОрганизации = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ИдентификаторСхемыЗапросаВГосключ, "RFOrgType"));
	...
	Запрос.SignExpiration = МосковскоеВремя();
	Атрибут = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ИдентификаторСхемыЗапросаВГосключ, "AttributeType"));
	...
	ЗапросНаОтправкуДокументаXML = СтрЗаменить(ЗапросНаОтправкуДокументаXML, """" + ИдентификаторСхемыЗапросаВГосключ + """",
...
// Для функции ЗапросНаОтправкуДокумента.
Функция МосковскоеВремя()
	ВремяОжидания = 24;
	МосковскоеВремя = ТекущаяУниверсальнаяДата() + (ВремяОжидания + 3) * 3600;
	Возврат ЗаписатьДатуJSON(МосковскоеВремя, ФорматДатыJSON.ISO, ВариантЗаписиДатыJSON.ЛокальнаяДатаСоСмещением);
КонецФункции
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		Методы.Вставить("ПолучениеРезультатовИзСервисаМобильнойПодписи");
	КонецЕсли;
