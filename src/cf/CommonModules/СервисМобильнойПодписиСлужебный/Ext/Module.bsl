...
// Заголовок команды отправки на подписание.
// 
...
// При получении типов подписания.
// 
// Параметры:
...
// Удалить ожидаемые подписи объекта.
// 
// Параметры:
//  ПодписанныйОбъект - ЛюбаяСсылка
...
Процедура ПроверкаПараметровПодписанта(ПараметрыПодписанта)
	...
	Если СтрДлина(НомерТелефона) <> 11 И Не СтрНачинаетсяС(НомерТелефона, "7") Тогда
...
Функция ОтправитьНаПодписаниеВСервис(НаборДанных, Подписант, Параметры = Неопределено, РеквизитыПодписанта = Неопределено, ТокенДоступа = "")
	...
	ПараметрыПодписания = Новый Структура("ТипПодписи, ИдентификаторФормы, Доверенность, Комментарий");
	...
	Если РазмерЗапроса > 1024 * 1024 * 100 Тогда
		Результат.Ошибка = ОшибкаМобильногоСервиса(ЗаголовокОшибки,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Размер отправляемых файлов в одном пакете превышает 100 Мб: %1 байт'"),
				РазмерЗапроса), "");
		Возврат Результат;
	КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(СтруктураОтвета)
	...
	Для Каждого КлючИЗначение Из СтруктураОтвета.error Цикл
		...
		ИначеЕсли КлючИЗначение.Ключ = "non_field_errors" Тогда
			Ошибки.Добавить(КлючИЗначение.Значение);
		Иначе
			Ошибки.Добавить(КлючИЗначение.Ключ + ": " + Строка(КлючИЗначение.Значение));
		КонецЕсли;
	КонецЦикла;
