...
Процедура ПроверкаПараметровПодписанта(ПараметрыПодписанта) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыПодписанта.ИННФЛ) И Не ЗначениеЗаполнено(ПараметрыПодписанта.ИНН) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнен ИНН'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыПодписанта.ИНН) И Не ЗначениеЗаполнено(ПараметрыПодписанта.ОГРН) Тогда
...
Функция ДобавитьПодписантаВСправочник(Знач ПараметрыПодписанта) Экспорт
	...
	|	Справочник.ПодписантыСервисаМобильнойПодписи КАК ПодписантыСервисаМобильнойПодписи
	|ГДЕ
	|	ПодписантыСервисаМобильнойПодписи.НомерТелефона = &НомерТелефона
	|	И ПодписантыСервисаМобильнойПодписи.ИННФЛ = &ИННФЛ
	|	И ПодписантыСервисаМобильнойПодписи.ИНН = &ИНН
	|	И ПодписантыСервисаМобильнойПодписи.ОГРН = &ОГРН";
	...
	Если ЗначениеЗаполнено(ПараметрыПодписанта.ИНН) Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ПодписантыСервисаМобильнойПодписи.ИННФЛ = &ИННФЛ", "");
	Иначе
		Запрос.УстановитьПараметр("ИННФЛ", ПараметрыПодписанта.ИННФЛ);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ПодписантыСервисаМобильнойПодписи.ИНН = &ИНН", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ПодписантыСервисаМобильнойПодписи.ОГРН = &ОГРН", "");
	КонецЕсли;
