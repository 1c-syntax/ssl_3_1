...
Процедура ПриОтправкеСерверногоОповещения(ИмяОповещения, ВариантыПараметров) Экспорт
	...
	Если ИмяОповещения = "СтандартныеПодсистемы.БазоваяФункциональность.ИзмененаКонфигурацияИлиРасширения" Тогда
		ПриОтправкеСерверногоОповещенияИзмененаКонфигурацияИлиРасширения(ИмяОповещения, ВариантыПараметров);
		
	ИначеЕсли ИмяОповещения = "СтандартныеПодсистемы.БазоваяФункциональность.ФункциональныеОпцииИзменены" Тогда
...
Функция ЭтоОшибкаТребованияПерезапускаСеанса(ИнформацияОбОшибке) Экспорт
	...
	Возврат ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке")
	      ...
	      И СтрНачинаетсяС(КраткоеПредставлениеОшибки(ИнформацияОбОшибке), ТекстОшибки);
...
Функция ЭтоПредопределенныеДанные(Знач Элемент, ИмяРеквизита = "", ЗначениеРеквизита = "") Экспорт
	...
	// Для определения предопределенных элементов в подсистемах Свойства и КонтактнаяИнформация
	// зачитываются реквизиты ИмяПредопределенногоНабора и ИмяПредопределенногоВида.
	ЗначенияРеквизитов = Новый Структура("ИмяПредопределенногоНабора,ИмяПредопределенногоВида,ИмяПредопределенныхДанных", 
		"", "", "");
	...
	Если (ЗначенияРеквизитов.ИмяПредопределенныхДанных = ""
		...
		И ЗначенияРеквизитов.ИмяПредопределенногоВида = "") Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура СброситьРазмерыИПоложениеОкна(Форма) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(РежимСовместимости, "8.3.18.0") < 0 Тогда
		КлючПоискаРусский = "НастройкиОкна"; // @Non-NLS
...
Процедура ПриДобавленииСерверныхОповещений(Оповещения) Экспорт
	...
	Оповещение = СерверныеОповещения.НовоеСерверноеОповещение(
		"СтандартныеПодсистемы.БазоваяФункциональность.ИзмененаКонфигурацияИлиРасширения");
	
	Оповещение.ИмяМодуляОтправки  = "СтандартныеПодсистемыСервер";
	Оповещение.ИмяМодуляПолучения = "СтандартныеПодсистемыКлиент";
	Оповещение.Параметры = ПараметрыПроверкиОповещенияИзмененаКонфигурацияИлиРасширения();
...
Процедура ПослеЗагрузкиДанных(Контейнер) Экспорт
	...
	РегистрыСведений.ПараметрыРаботыВерсийРасширений.ЗапуститьЗаполнениеПараметровРаботыРасширений(Истина);
...
//  УточняющаяСсылка - Строка
//  
// Возвращаемое значение:
//  Строка 
//
Функция ТекстСообщенияПриДинамическомОбновлении(ДинамическиеИзмененияКонфигурации, УточняющаяСсылка = "") Экспорт
	...
	Если ДинамическиеИзмененияКонфигурации.Исправления <> Неопределено Тогда
		Если ДинамическиеИзмененияКонфигурации.Исправления.Добавлено > 0
			...
			ТекстСообщенияПатчи = НСтр("ru = 'Установлены новые исправления (патчи): %1, удалены: %2.'") + УточняющаяСсылка;
		ИначеЕсли ДинамическиеИзмененияКонфигурации.Исправления.Добавлено = 1 Тогда
			ТекстСообщенияПатчи = НСтр("ru = 'Установлено новое исправление (патч).'") + УточняющаяСсылка;
		ИначеЕсли ДинамическиеИзмененияКонфигурации.Исправления.Добавлено > 0 Тогда
			ТекстСообщенияПатчи = НСтр("ru = 'Установлены новые исправления (патчи): %1.'") + УточняющаяСсылка;
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	ИначеЕсли (ЗначениеРеквизита <> "" И СтарыеСвойства[ИмяРеквизита] = ""
...
Процедура УстановитьКлючСохраненияПоложенияОкнаФормы(Форма, Ключ, УстановитьНастройки)
	...
	// Русский вариант.
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(РежимСовместимости, "8.3.18.0") < 0 Тогда
		ТипыНастроек.Добавить("/НастройкиОкна"); // @Non-NLS
		ТипыНастроек.Добавить("/Такси/НастройкиОкна"); // @Non-NLS
	Иначе
...
//   * УстановленныеРасширенияПриЗапуске - см. Справочники.ВерсииРасширений.УстановленныеРасширенияПриЗапуске
//
Функция ПараметрыПроверкиОповещенияИзмененаКонфигурацияИлиРасширения()
	
	Результат = Новый Структура;
	Результат.Вставить("ВерсияКонфигурации", Метаданные.Версия);
	Результат.Вставить("УстановленныеРасширенияПриЗапуске",
		Справочники.ВерсииРасширений.УстановленныеРасширенияПриЗапуске());
...
Процедура ПриОтправкеСерверногоОповещенияИзмененаКонфигурацияИлиРасширения(ИмяОповещения, ВариантыПараметров)
	
	Для Каждого ВариантПараметров Из ВариантыПараметров Цикл
		ВариантПараметров = ВариантПараметров; // Структура
		КонфигурацияБазыДанныхИзмененаДинамически = ВариантПараметров.Параметры.ВерсияКонфигурации <> Метаданные.Версия;
		ДинамическиеИзменения = Справочники.ВерсииРасширений.ДинамическиИзмененныеРасширения(
			ВариантПараметров.Параметры.УстановленныеРасширенияПриЗапуске);
		...
		СообщениеПользователю = ТекстСообщенияПриДинамическомОбновлении(ДинамическиеИзменения);
		СерверныеОповещения.ОтправитьСерверноеОповещение(ИмяОповещения, СообщениеПользователю, Адресаты);
	КонецЦикла;
...
Процедура ДобавитьОбъектыФункциональнойОпции(Объекты, ФункциональнаяОпция)
	...
	Для Каждого ЭлементСостава Из ФункциональнаяОпция.Состав Цикл
		...
		Если Объекты.Получить(Объект) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ЧастиИмени = СтрРазделить(Объект.ПолноеИмя(), ".", Ложь);
