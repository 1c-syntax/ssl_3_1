...
Функция ЭтоБазоваяВерсияКонфигурации() Экспорт
	...
	ЭтоБазоваяВерсияКонфигурации = СтрНайти(ВРег(Метаданные.Имя), "БАЗОВАЯ") > 0;
...
// Возвращает настройки, используемые при статическом поиске имен процедур и функций,
// которые указываются в параметрах процедур и функций, описанных в комментарии
// к процедуре ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод.
// Используется в отчете ПроверкаВнедренийБСП.erf, чтобы учесть случаи, когда
// подсистемы допускают передачу полных имен обработчиков в качестве параметров,
// например, обработчиков установки параметров сеанса, обработчиков обновления
// информационной базы и др.
// Смотри также ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод.
//
// Возвращаемое значение:
//  см. ОбщегоНазначенияПереопределяемый.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод.Настройки
//
Функция НастройкиПроверкиМетодовВызываемыхКакПроизвольныйКод() Экспорт
	
	ОписаниеОбработчиков = Новый ТаблицаЗначений;
	ОписаниеОбработчиков.Колонки.Добавить("ИменаПроцедур",   Новый ОписаниеТипов("Соответствие"));
	ОписаниеОбработчиков.Колонки.Добавить("ЗаголовокОшибки", Новый ОписаниеТипов("Строка"));
	
	ИсключенияПроверки = Новый ТаблицаЗначений;
	ИсключенияПроверки.Колонки.Добавить("ПолноеИмяОбъекта",   Новый ОписаниеТипов("Строка"));
	ИсключенияПроверки.Колонки.Добавить("ИмяПроцедуры",       Новый ОписаниеТипов("Строка"));
	ИсключенияПроверки.Колонки.Добавить("ФрагментСодержания", Новый ОписаниеТипов("Строка"));
	
	Настройки = Новый Структура;
	Настройки.Вставить("ОписаниеОбработчиков", ОписаниеОбработчиков);
	Настройки.Вставить("ИсключенияПроверки",   ИсключенияПроверки);
	Настройки.Вставить("ОшибкаКогдаУказанныйМетодНеВызывается", Истина);
	Настройки.Вставить("ИсключенияКогдаУказанныйМетодНеВызывается", Новый Соответствие);
	
	ИнтеграцияПодсистемБСП.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	Попытка
		ОбщегоНазначенияПереопределяемый.ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ПроцедураУказана = СтрНайти(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке),
			"(" + "ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод" + ")") = 0;
		Если ПроцедураУказана Тогда
			ВызватьИсключение;
		КонецЕсли;
	КонецПопытки;
...
// Заполняет имена процедур и функций модуля, которые можно вызывать как по имени,
// как произвольный код, или запускать в фоне.
// Процедуры и функции могут вызываться по имени, как произвольный код, из
// • ОбщегоНазначения.ВыполнитьМетодКонфигурации,
// • ОбщегоНазначения.ВыполнитьМетодОбъекта.
// Процедуры и функции могут запускаться в фоне из
// • ДлительныеОперации.ВыполнитьВФоне,
// • ДлительныеОперации.ВыполнитьФункцию,
// • ДлительныеОперации.ВыполнитьПроцедуру,
// • ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков,
...
//
// Параметры:
//  Методы - Соответствие из КлючИЗначение:
//   * Ключ - Строка - имя процедуры или функции в модуле.
//   * Значение - Неопределено, Булево - если Истина, тогда можно запускать в фоне.
//
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	
	Методы.Вставить("ВключитьКонстантуДоставлятьСерверныеОповещенияБезСистемыВзаимодействия");
	Методы.Вставить("ПометитьЗаписиКэшаВерсийНеактуальными");
	Методы.Вставить("УстановитьРасписаниеЗапускаСервисовИнтеграции");
	Методы.Вставить("ВключитьКонстантуРазрешитьДоступКИнтернетСервисам");
	Методы.Вставить("ПеренестиБезопасноеХранилищеДанныхОбластейДанных");
	Методы.Вставить("УстановитьКонстантуНеИспользоватьРазделениеПоОбластямДанных");
	Методы.Вставить("СравнитьТаблицы", Истина);
...
// Возвращает Истина, если при запуске указан параметр ОтключитьЛогикуНачалаРаботыСистемы.
//
// Возвращаемое значение:
//  Булево
//
Функция ОтключенаЛогикаНачалаРаботыСистемы() Экспорт
	...
	ПараметрЗапускаПриложения = ПараметрыКлиентаНаСервере(Ложь).Получить("ПараметрЗапуска");
	
	Возврат СтрНайти(ПараметрЗапускаПриложения, "ОтключитьЛогикуНачалаРаботыСистемы") > 0;
...
Функция ПараметрыКлиентаНаСервере(ВызыватьИсключение = Истина) Экспорт
	...
	Иначе
		КомментарийДляЖурналаРегистрацииБезСтекаВызовов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое обращение к неинициализированным параметрам клиента на сервере.
			           |Вероятно, вызов выполнен после некорректной очистки параметров сеанса без использования %2.'",
			     ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Попытка
		ВызватьИсключение(КомментарийДляЖурналаРегистрацииБезСтекаВызовов,
			КатегорияОшибки.ОшибкаКонфигурации,, ПараметрыКлиента.Получить("СтекЗапускаДлительнойОперации"));
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	КонецПопытки;
...
//                   если старое и новое значения параметра не совпадают.
//
Процедура УдалитьПараметрРаботыПрограммы(ИмяПараметра, ЕстьИзменения = Ложь) Экспорт
	
	РегистрыСведений.ПараметрыРаботыПрограммы.УдалитьПараметрРаботыПрограммы(ИмяПараметра, ЕстьИзменения);
...
Процедура УстановитьПараметрРаботыРасширения(ИмяПараметра, Значение, БезУчетаВерсииРасширений = Ложь) Экспорт
	...
	РегистрыСведений.ПараметрыРаботыВерсийРасширений.УстановитьПараметрРаботыРасширения(ИмяПараметра,
		Значение, БезУчетаВерсииРасширений);
	
КонецПроцедуры

// Удаляет строку с параметром для текущей версии расширений.
// Перед вызовом требуется установить привилегированный режим.
//
// Параметры:
//  ИмяПараметра - Строка - не более 128 символов. Например,
...
//
//  БезУчетаВерсииРасширений - Булево
//                           - Неопределено - удалить все строки с именем параметра.
//
//  ЕстьИзменения  - Булево - возвращаемое значение. Устанавливается Истина,
//                   если старое и новое значения параметра не совпадают.
//
Процедура УдалитьПараметрРаботыРасширения(ИмяПараметра, БезУчетаВерсииРасширений = Ложь, ЕстьИзменения = Ложь) Экспорт
	
	РегистрыСведений.ПараметрыРаботыВерсийРасширений.УдалитьПараметрРаботыРасширения(ИмяПараметра,
		БезУчетаВерсииРасширений, ЕстьИзменения);
...
Функция ЭтоСеансФоновогоЗадания() Экспорт
	...
	ТекущийСеанс = ПолучитьТекущийСеансИнформационнойБазы();
	Возврат ТекущийСеанс.ИмяПриложения = "BackgroundJob";
...
// Возвращает признак того, что конфигурация работает в режиме PaaS.
//
Функция ЭтоИнформационнаяБазаВМоделиСервиса() Экспорт
	...
	МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
	ВерсияБиблиотеки           = МодульТехнологияСервиса.ВерсияБиблиотеки();
	
	ВерсияЧастями = СтрРазделить(ВерсияБиблиотеки, ".");
	ВерсияЧастями.Удалить(ВерсияЧастями.ВГраница());
	НомерВерсии = СтрСоединить(ВерсияЧастями, ".");
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(НомерВерсии, "2.0.15") = 0
			И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБиблиотеки, "2.0.15.33") >= 0
		Или ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(НомерВерсии, "2.1.1") >= 0
			И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБиблиотеки, "2.1.1.39") >= 0 Тогда
		Возврат МодульРаботаВМоделиСервиса.ЭтоИнформационнаяБазаВМоделиСервиса();
	КонецЕсли;
...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.РегистрыСведений.ДлительныеОперации);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.12.95";
		Обработчик.Процедура = "СтандартныеПодсистемыСервер.УстановитьРасписаниеЗапускаСервисовИнтеграции";
		Обработчик.РежимВыполнения = "Оперативно";
	КонецЕсли;
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	// Обработчики установки параметров сеанса.
	ИменаПроцедур = Новый Соответствие;
	
	Обработчики = Новый Соответствие;
	ИнтеграцияПодсистемБСП.ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики);
	Для Каждого КлючИЗначение Из Обработчики Цикл
		ИменаПроцедур.Вставить(КлючИЗначение.Значение);
	КонецЦикла;
	...
	ОписаниеОбработчика = Настройки.ОписаниеОбработчиков.Добавить();
	ОписаниеОбработчика.ИменаПроцедур = ИменаПроцедур;
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик установки параметров сеанса ""%1"".'");
	
	// Исключения.
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбщегоНазначения";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьФоновоеЗадание";
	ИсключениеПроверки.ФрагментСодержания =
	...
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбщегоНазначения";
	ИсключениеПроверки.ИмяПроцедуры       = "ВызватьФункциюКонфигурации";
	ИсключениеПроверки.ФрагментСодержания = "Возврат ВыполнитьМетодКонфигурации(ИмяМетода, Параметры, Истина)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.ОбщегоНазначения";
	ИсключениеПроверки.ИмяПроцедуры       = "ВызватьФункциюОбъекта";
	ИсключениеПроверки.ФрагментСодержания = "Возврат ВыполнитьМетодОбъекта(Объект, ИмяМетода, Параметры, Истина)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.СтандартныеПодсистемыСервер";
	ИсключениеПроверки.ИмяПроцедуры       = "ВыполнитьОбработчикиУстановкиПараметровСеанса";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(Обработчик, ПараметрыОбработчика)";
	
	Методы = Новый Соответствие;
	Методы.Вставить("ЕстьСсылкиНаОбъект", Истина);
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.ОбщегоНазначения", Методы);
	
	Методы = Новый Соответствие;
	Методы.Вставить("ПеренестиБезопасноеХранилищеДанныхОбластейДанных");
	Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить("ОбщийМодуль.СтандартныеПодсистемыСервер", Методы);
...
Процедура УстановитьРасписаниеЗапускаСервисовИнтеграции() Экспорт
	...
	РегламентныеЗаданияСервер.УстановитьПараметрыРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.ОбработкаСервисовИнтеграции, ПараметрыЗадания);
...
// Обработчик события см. ПодпискаНаСобытие.ОбработкаПредопределенныхЭлементовПередЗаписью
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник)
	...
	Если ВладелецЗаполнен(Источник) Тогда
		ЭтоПредопределенныйЭлемент = ЗначениеЗаполнено(СтарыеСвойства.ИмяПредопределенныхДанных)
			Или (ЗначениеЗаполнено(ИмяРеквизита) И ЗначениеЗаполнено(СтарыеСвойства[ИмяРеквизита]));

		Запретить = ЭтоПредопределенныйЭлемент 
			И СтарыеСвойства.ПометкаУдаления <> Истина 
			И ЗначениеПометкиУдаления(Источник.Владелец) <> Истина;
	...
	ЭтоУстановкаИмениПредопределенного = ЗначениеЗаполнено(ЗначениеРеквизита) И Не ЗначениеЗаполнено(СтарыеСвойства[ИмяРеквизита])
		Или НЕ ЗначениеЗаполнено(СтарыеСвойства.ИмяПредопределенныхДанных);
	ЭтоУстановкаИмениПредопределенногоВПомеченныйЭлемент = ЭтоУстановкаИмениПредопределенного И СтарыеСвойства.ПометкаУдаления = Истина;
	Если ЭтоУстановкаИмениПредопределенногоВПомеченныйЭлемент Тогда	
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
// Обработчик события см. ПодпискаНаСобытие.ЗапретитьУдалениеПредопределенныхЭлементов
Процедура ЗапретитьУдалениеПредопределенныхЭлементовПередУдалением(Источник, Отказ) Экспорт
	...
	Если ВладелецЗаполнен(Источник) Тогда
		Если ЗначениеПометкиУдаления(Источник.Владелец) <> Ложь Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция МинимальнаяВерсияПлатформыДляИспользования() Экспорт
	...
	Версии.Добавить("8.3.27", "8.3.27.1688");
...
Функция ВозможноВыполнениеОбработчиковБезУстановкиБезопасногоРежима()
	...
	Возврат Вычислить("ДоступенПереходВПривилегированныйРежим()"); // АПК:488 - Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только вызов функции модуля конфигурации.
...
Функция ВладелецЗаполнен(Источник)
	ЗначенияРеквизитов = Новый Структура("Владелец");
	ЗаполнитьЗначенияСвойств(ЗначенияРеквизитов, Источник);
	Результат = ЗначениеЗаполнено(ЗначенияРеквизитов.Владелец);
	Возврат Результат;	
КонецФункции

Функция ЗначениеПометкиУдаления(ПроизвольнаяСсылка)
	Результат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроизвольнаяСсылка, "ПометкаУдаления");
	Возврат Результат;
КонецФункции
...
// Параметры:
//  ПолноеИмяМетода - Строка
//  ВФоне - Булево
//  Объект - Произвольный
//  Текст - Произвольный - если указан не Null, тогда установить текст ошибки и не вызывать исключение.
//
Процедура ПроверитьМетодДляВызоваКакПроизвольныйКод(ПолноеИмяМетода, ВФоне = Ложь, Объект = Неопределено, Текст = Null) Экспорт
	
	Если Не ПроверятьВызовыМетодовКакПроизвольныйКод() И Текст = Null Тогда
		Возврат;
	КонецЕсли;
	
	Если Объект <> Неопределено Тогда
		Результат = МетодыРазрешенныеДляВызоваКакПроизвольныйКод(Объект);
		ИмяМетода = ПолноеИмяМетода;
	Иначе
		ЧастиИмени = СтрРазделить(ПолноеИмяМетода, ".", Истина);
		
		Если ЧастиИмени.Количество() = 3 Тогда
			ИмяМодуля = ЧастиИмени[0] + "." + ЧастиИмени[1];
			ИмяМетода = ЧастиИмени[2];
		Иначе
		...
		Попытка
			Результат = СтандартныеПодсистемыПовтИсп.МетодыРазрешенныеДляВызоваКакПроизвольныйКод(ИмяМодуля);
		Исключение
			Если Текст <> Null И ОбщегоНазначенияКлиентСервер.ЭтоИсключениеСКодомОшибки(ИнформацияОбОшибке(),
					"СтандартныеПодсистемы.БазоваяФункциональность.ЭтоОшибкаПолученияОбщегоМодуляПоИмени
					|СтандартныеПодсистемы.БазоваяФункциональность.ЭтоОшибкаПолученияМодуляМенеджераПоИмени") Тогда
				Возврат;
			КонецЕсли;
			ВызватьИсключение;
		КонецПопытки;
	...
	Если ТипЗнч(Результат) = Тип("ФиксированноеСоответствие") Тогда
		ИспользованиеМетода = Результат.Получить(ИмяМетода);
		Если ВФоне И ИспользованиеМетода = Истина
		 Или Не ВФоне И ИспользованиеМетода <> Неопределено Тогда
			Возврат;
		КонецЕсли;
		ПроцедураОпределена = Истина;
	Иначе
		ПроцедураОпределена = Ложь;
	КонецЕсли;
	
	ВызовЗапрещен = Текст <> Null Или ЗапрещенВызовЛюбыхМетодовКакПроизвольныйКод(ПроцедураОпределена);
	
	Если Не ВызовЗапрещен И Не ПроцедураОпределена Тогда
		УровеньЖурнала = УровеньРегистрацииВызововНеуказанныхМетодовКакПроизвольныйКод();
	...
	Если Объект <> Неопределено Тогда
		ИмяМодуля = Объект.Метаданные().ПолноеИмя() + "." + "МодульОбъекта";
	КонецЕсли;
	
	Если ВФоне И ПроцедураОпределена Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для запуска в фоне в процедуре ""%2"" модуля ""%3"".'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	ИначеЕсли Не ВФоне И ПроцедураОпределена Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для вызова, как произвольный код, в процедуре ""%2"" модуля ""%3"".'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	ИначеЕсли ВФоне И Не ПроцедураОпределена Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для запуска в фоне в процедуре ""%2"" модуля ""%3"" (процедура не определена).'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для вызова, как произвольный код, в процедуре ""%2"" модуля ""%3"" (процедура не определена).'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	КонецЕсли;
	
	Если Текст <> Null Тогда
		Текст = ТекстОшибки;
		Возврат;
	КонецЕсли;
	
	Если ВызовЗапрещен Тогда
	...
	ИмяСобытия = НСтр("ru = 'Безопасность.Метод не разрешен для вызова, как произвольный код'",
		ОбщегоНазначения.КодОсновногоЯзыка());
	
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурнала,, ИмяМодуля + "." + ИмяМетода,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	
КонецПроцедуры

// Параметры:
//  Модуль - Произвольный - ссылка на общий модуль, модуль менеджера, объект.
//
// Возвращаемое значение:
//  - ФиксированноеСоответствие
//  - Неопределено
//
Функция МетодыРазрешенныеДляВызоваКакПроизвольныйКод(Модуль) Экспорт
	
	Методы = Новый Соответствие;
	Попытка
		Модуль.ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ПроцедураНеУказана = СтрНайти(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке),
			"(" + "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод" + ")") > 0;
		Если ПроцедураНеУказана Тогда
	...
	Результат = Новый Соответствие;
	Для Каждого КлючИЗначение Из Методы Цикл
		Результат.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение = Истина);
	КонецЦикла;
...
// Включает проверку вызовов методов в режиме 1С:Предприятие и в отчете ПроверкаВнедренияБСП.
// Можно переопределить с помощью расширения для обнаружения мест, требующих настройки.
//
// Возвращаемое значение:
//  Булево
//
Функция ПроверятьВызовыМетодовКакПроизвольныйКод() Экспорт
	Возврат Ложь;
КонецФункции

// Позволяет блокировать вызов методов, которые не разрешены для вызова, как произвольный код.
// Требуется указать Истина в функции ПроверятьВызовыМетодовКакПроизвольныйКод.
//
// Параметры:
//  ПроцедураОпределена - Булево - если Ложь, тогда в модуле еще не определена
//    процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод, что позволяет
//    включить запрет только тогда, когда процедура определена в модуле, а не безусловно.
//
// Возвращаемое значение:
//  Булево
//
Функция ЗапрещенВызовЛюбыхМетодовКакПроизвольныйКод(ПроцедураОпределена)
	Возврат Ложь;
КонецФункции

// Возвращает уровень журнала регистрации, если требуется регистрировать вызываемые методы,
// когда не определена процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод.
// Можно переопределить с помощью расширения для обнаружения случаев (путем автоматизированного
// тестирования), которые нельзя обнаружить статическим анализом кода с помощью
// отчета ПроверкаВнедренияБСП.erf (смотри ПроверятьВызовыМетодовКакПроизвольныйКод).
//
// Возвращаемое значение:
//  - УровеньЖурналаРегистрации
//  - Неопределено
//
Функция УровеньРегистрацииВызововНеуказанныхМетодовКакПроизвольныйКод()
