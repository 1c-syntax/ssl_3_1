...
Процедура ЗаполнитьЭлементыНачальнымиДанными(ОбновитьМультиязычныеСтроки = Ложь) Экспорт
	...
	Для Каждого МетаданныеОбъекта Из ОбъектыСНачальнымЗаполнением Цикл
		...
		ЛокализуемыеРеквизитыОбъекта = МультиязычностьСервер.НаименованияЛокализуемыхРеквизитыОбъекта(МетаданныеОбъекта);
...
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры) Экспорт
	...
	Язык1 = МультиязычностьСервер.КодПервогоДополнительногоЯзыкаИнформационнойБазы();
	Язык2 = МультиязычностьСервер.КодВторогоДополнительногоЯзыкаИнформационнойБазы();
	...
	Для каждого ОбъектМетаданных Из ОбъектыСПредопределеннымиЭлементами Цикл
		
		ЛокализуемыеРеквизитыОбъекта = МультиязычностьСервер.НаименованияЛокализуемыхРеквизитыОбъекта(ОбъектМетаданных);
		Если ЛокализуемыеРеквизитыОбъекта.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонаЗапроса, СтрСоединить(ИмяРеквизитОбъекта, ", " + Символы.ПС), ОбъектМетаданных.ПолноеИмя());
		...
		ПредопределенныеДанные = ПредопределенныеДанныеОбъекта(ОбъектМетаданных, МенеджерОбъекта, ЛокализуемыеРеквизитыОбъекта);
		НастройкиПредопределенныхЭлементов = НастройкиПредопределенныхЭлементов(МенеджерОбъекта, ПредопределенныеДанные);
		...
		Пока ДанныеЗапроса.Следующий() Цикл
			...
			Если Элемент <> Неопределено Тогда
				Для каждого ОписаниеРеквизита Из ЛокализуемыеРеквизитыОбъекта Цикл
					...
					Если СтрСравнить(Элемент[МультиязычностьСервер.ИмяЛокализуемогоРеквизита(ИмяРеквизита, ОсновнойЯзык)], ДанныеЗапроса[ИмяРеквизита]) <> 0 Тогда
						ОбъектыКОбработке.Добавить(ДанныеЗапроса.Ссылка);
						Продолжить;
					КонецЕсли;
						
					Если ЗначениеЗаполнено(Язык1)
						И СтрСравнить(Элемент[МультиязычностьСервер.ИмяЛокализуемогоРеквизита(ИмяРеквизита, Язык1)], ДанныеЗапроса[ИмяРеквизита + "Язык1"]) <> 0 Тогда
							ОбъектыКОбработке.Добавить(ДанныеЗапроса.Ссылка);
							Продолжить;
					КонецЕсли;
					
					Если ЗначениеЗаполнено(Язык2)
						И СтрСравнить(Элемент[МультиязычностьСервер.ИмяЛокализуемогоРеквизита(ИмяРеквизита, Язык2)], ДанныеЗапроса[ИмяРеквизита + "Язык2"]) <> 0 Тогда
							ОбъектыКОбработке.Добавить(ДанныеЗапроса.Ссылка);
					КонецЕсли;
...
Процедура ОбновитьПредставленияПредопределенныхЭлементов(Параметры) Экспорт
	...
	СсылкаНаОбъект = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, ОбъектМетаданных.ПолноеИмя());
	
	Результат = ОбновитьПредставленияПредопределенныхЭлементовОбъекта(СсылкаНаОбъект, ОбъектМетаданных);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "СтандартныеПодсистемыСервер.ЗаполнитьЭлементыНачальнымиДанными";
...
Функция ОбновитьПредставленияПредопределенныхЭлементовОбъекта(СсылкаНаОбъект, ОбъектМетаданных)
	...
	ОсновнойЯзык = ОбщегоНазначения.КодОсновногоЯзыка();
	КодЯзыка1 = МультиязычностьСервер.КодПервогоДополнительногоЯзыкаИнформационнойБазы();
	КодЯзыка2 = МультиязычностьСервер.КодВторогоДополнительногоЯзыкаИнформационнойБазы();
	...
	Пока СсылкаНаОбъект.Следующий() Цикл
		...
		Попытка
			...
			Элемент = ПредопределенныеДанные.Найти(ИмяПредопределенныхДанных, "ИмяПредопределенныхДанных");
			Если Элемент <> Неопределено Тогда
				Объект = СсылкаНаОбъект.Ссылка.ПолучитьОбъект();
				
				Для каждого ЛокализуемыйРеквизитОбъекта Из ЛокализуемыеРеквизитыОбъекта Цикл
					Имя = ЛокализуемыйРеквизитОбъекта.Ключ;
					Объект[Имя] = Элемент[МультиязычностьСервер.ИмяЛокализуемогоРеквизита(Имя, ОсновнойЯзык)];
					
					Если МультиязычностьСервер.ИспользуетсяПервыйДополнительныйЯзык() Тогда
						Объект[Имя + "Язык1"] = Элемент[МультиязычностьСервер.ИмяЛокализуемогоРеквизита(Имя, КодЯзыка1)];
					КонецЕсли;
					Если МультиязычностьСервер.ИспользуетсяВторойДополнительныйЯзык() Тогда
						Объект[Имя + "Язык2"] = Элемент[МультиязычностьСервер.ИмяЛокализуемогоРеквизита(Имя, КодЯзыка2)];
					КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				СсылкаНаОбъект.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			ОбъектМетаданных, СсылкаНаОбъект.Ссылка, ТекстСообщения);
