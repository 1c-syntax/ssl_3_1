...
Функция ПроверитьИЗарегистрироватьCOMСоединитель(Знач СтруктураНастроек, Оповещение = Неопределено) Экспорт
	
	Если НЕ ОбщегоНазначенияКлиент.ИнформационнаяБазаФайловая() Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если НЕ ОбменДаннымиВызовСервера.ПроверитьИЗарегистрироватьCOMСоединитель(СтруктураНастроек) Тогда
