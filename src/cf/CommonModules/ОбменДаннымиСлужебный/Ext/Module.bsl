...
Процедура УстановкаПараметровСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	// Параметры, которые актуальны для разделенного и локального приложения.
	//
	...
	ПараметрыСеанса.ПроблемыЗагруженныхОбъектовОбрабатываемыеПользователем = Новый ХранилищеЗначения(Новый Массив);
	УстановленныеПараметры.Добавить("ПроблемыЗагруженныхОбъектовОбрабатываемыеПользователем");
...
Функция ЗагрузитьЧастьФайла(ИдентификаторФайла, НомерЗагружаемойЧастиФайла, ЗагружаемаяЧастьФайла, СообщениеОбОшибке) Экспорт
	...
	Если ПутьКФайлуСодержитНедопустимыеСимволы(ИдентификаторФайла) Тогда
...
Функция ВыгрузитьЧастьФайла(ИдентификаторФайла, НомерВыгружаемойЧастиФайла, СообщениеОбОшибке) Экспорт
	...
	Если ПутьКФайлуСодержитНедопустимыеСимволы(ИдентификаторФайла) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Недопустимое имя файла.'");
...
Процедура ПриОпределенииУзлаКорреспондентаКомпонентыОбмена(КомпонентыОбмена, УзелКорреспондента, НаправлениеОбмена = "") Экспорт
	...
	КомпонентыОбмена.УзелКорреспондента = УзелКорреспондента;
	
	Если НаправлениеОбмена = "Получение" Тогда
		
		ИмяУзелКорреспондента = ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелКорреспондента);
		КомпонентыОбмена.ИспользоватьКешПубличныхИдентификаторов = ОбменДаннымиПовтИсп.ИспользоватьКешПубличныхИдентификаторов(ИмяУзелКорреспондента);
		КомпонентыОбмена.ОбъектнаяКонвертацияXDTO = ОбменДаннымиПовтИсп.СпособКонвертацииОбъектовXDTO(УзелКорреспондента);
...
// 
// Возвращаемое значение:
//  Булево - Истина, если путь к файлу содержит недопустимые символы (например, ".."),
//			 Ложь - если путь не содержит недопустимых символов.
//
Функция ПутьКФайлуСодержитНедопустимыеСимволы(ПутьКФайлу) Экспорт
	
	Если СтрНайти(ПутьКФайлу, "..") > 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Недопустимое имя файла.'"));
		Возврат Истина;
	КонецЕсли;
...
// 
Процедура ДобавитьЗаписьВРегистрСведений(СтруктураЗаписи, Знач ИмяРегистра, Загрузка = Ложь, ДополнительныеСвойства = Неопределено) Экспорт
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПодготовитьДанныеДляВыгрузкиИзИнформационнойБазы", Истина);
	Методы.Вставить("ЗагрузитьДанныеXDTOВИнформационнуюБазу", Истина);
