	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			Для Каждого ЭлементОтбора Из Данные.Отбор Цикл
				ЗначениеОтбора = Новый Структура("Значение", ЭлементОтбора.Значение);
				ПроверитьЗаменитьСсылкуПредопределенногоВРеквизитеОбъекта(ЗначениеОтбора, "Значение", ТаблицаПредопределенных);
				ЭлементОтбора.Установить(ЗначениеОтбора.Значение);
			КонецЦикла;
...
Функция СоздатьНаборЗаписейРегистраСведений(СтруктураЗаписи, ИмяРегистра) Экспорт
	...
	Для Каждого Измерение Из МетаданныеРегистра.Измерения Цикл
		...
		Если СтруктураЗаписи.Свойство(Измерение.Имя, ЗначениеСвойства) Тогда
			
			НаборЗаписей.Отбор[Измерение.Имя].Установить(ЗначениеСвойства);
...
//   Данные - Произвольный - объект данных.
//   МассивКоллекций - Массив из КоллекцияОбъектовМетаданных - набор коллекций реквизитов.
//   ТаблицаПредопределенных - ТаблицаЗначений - см. ОбменДаннымиСлужебный.ТаблицаПредопределенныхДанных
//
