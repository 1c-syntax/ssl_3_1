...
Функция ЭтоСобытиеКонтекстнойНастройки(Событие) Экспорт 
	...
	Если ТипЗнч(Событие) <> Тип("Структура") Или Не Событие.Свойство("Действие") Тогда
		Возврат Ложь;
	КонецЕсли;

	События = СобытияКонтекстнойНастройки();
	Возврат События[Событие.Действие] = Истина;
...
Процедура ВставитьПолеВГруппировкуРаздела(Группировка, Группировка2, ВыбранноеПоле, ТекущееПоле, Действие, РолиПолей)
	...
	Иначе
		...
		Если ИндексПоля = -1 И НайденноеТекущееПоле.Родитель <> Неопределено Тогда
			ИндексПоля = Поля.Элементы.Индекс(НайденноеТекущееПоле.Родитель);
		КонецЕсли;
...
Процедура СкопироватьПоля(Группировка, Группировка2, ИмяКоллекции)
	...
	ЭлементыГруппировки = ?(ЗначениеЗаполнено(ИмяКоллекции), Группировка[ИмяКоллекции].Элементы, Группировка.Элементы);
	ЭлементыГруппировки2 = ?(ЗначениеЗаполнено(ИмяКоллекции), Группировка2[ИмяКоллекции].Элементы, Группировка2.Элементы);
	
	ЭлементыГруппировки.Очистить();
	Для Каждого ТекущееПоле2 Из ЭлементыГруппировки2 Цикл
		НовоеПоле = ЭлементыГруппировки.Добавить(ТипЗнч(ТекущееПоле2));
		ЗаполнитьЗначенияСвойств(НовоеПоле, ТекущееПоле2);
		Если ТипЗнч(ТекущееПоле2) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			СкопироватьПоля(НовоеПоле, ТекущееПоле2, "");
		КонецЕсли;
	КонецЦикла;
...
Процедура Фильтровать(Форма, ВидСравнения, СвойстваЗаголовка, ДанныеРасшифровки)
	...
	Если Настройки.Структура.Количество() > 1 Тогда
		Если Фильтр = Неопределено
		   Или Фильтр <> Неопределено И Фильтр.ВидСравнения <> ВидСравнения Или Фильтр.ПравоеЗначение <> ДанныеРасшифровки.Значение Тогда
			Фильтры = ВариантыОтчетовСлужебныйКлиентСервер.ФильтрыРазделаОтчета(Настройки, СвойстваЗаголовка, ЭтоГруппировка);
			Фильтр = ВариантыОтчетовСлужебныйКлиентСервер.ФильтрРазделаОтчета(Фильтры, СвойстваЗаголовка.Поле);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеВыбораПоля(ВыбранноеПоле, ДополнительныеПараметры) Экспорт 
	...
	ИначеЕсли СтрСравнить(Действие, "ВставитьГруппировкуВыше") = 0 Или СтрСравнить(Действие, "ВставитьГруппировкуНиже") = 0 Тогда
...
Процедура ПереместитьРесурсыВКонец(Поля)
	...
	СобратьВсеРесурсыСУчетомВложенности(Ресурсы, Поля.ДоступныеПоляВыбора.Элементы);
...
Процедура СобратьВсеРесурсыСУчетомВложенности(Ресурсы, Элементы)
	Для Каждого ДоступноеПоле Из Элементы Цикл
		...
		ИначеЕсли ДоступноеПоле.Папка Тогда
			СобратьВсеРесурсыСУчетомВложенности(Ресурсы, ДоступноеПоле.Элементы)
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ОткрытьФормуОтчета(Форма, ПараметрыОткрытия)
	...
	Если ПараметрыОткрытия.Данные <> Неопределено 
	   И ПараметрыОткрытия.Данные.Тип = ВариантыОтчетовСлужебныйКлиентСервер.ТипРеквизит() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Действие недоступно. Нельзя расшифровывать детальные записи.'"));
		Возврат;
	КонецЕсли;
