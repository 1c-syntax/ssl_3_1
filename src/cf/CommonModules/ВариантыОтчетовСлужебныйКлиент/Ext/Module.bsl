...
Процедура ПриАктивизацииРезультатаОтчета(Форма, Элемент) Экспорт 
	...
	ЭтоРасшифровка = (ТипЗнч(Область.Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных"));
	...
	Если Не ЭтоРасшифровка
		И (Элементы.КонтекстноеМенюОбластиЗаголовка.Видимость
		Или Элементы.КонтекстноеМенюОбластиДанных.Видимость) Тогда 
		
		Элементы.КонтекстноеМенюОбластиОбщееРедактирование.Вид = ВидГруппыФормы.Подменю;
	Иначе
		Элементы.КонтекстноеМенюОбластиОбщееРедактирование.Вид = ВидГруппыФормы.ГруппаКнопок;
	КонецЕсли;
...
Процедура СброситьСортировку(Настройки, СвойстваЗаголовка, ЭлементыСтруктуры = Неопределено)
	...
	Если ЭлементыСтруктуры = Неопределено Тогда 
		...
		УдалитьЭлементыСортировки(Настройки.Порядок.Элементы);
		
		Если СвойстваЗаголовка.КоличествоРазделов = 1 Тогда 
			
			СброситьСортировку(Настройки, СвойстваЗаголовка, Настройки.Структура);
		...
		Иначе
			...
			Если ТипЗнч(Раздел) = Тип("ГруппировкаКомпоновкиДанных") Тогда 
				...
				УдалитьЭлементыСортировки(Раздел.Порядок.Элементы);
				СброситьСортировку(Настройки, СвойстваЗаголовка, Раздел.Структура);
			
			ИначеЕсли ТипЗнч(Раздел) = Тип("ТаблицаКомпоновкиДанных") Тогда 
				
				СброситьСортировку(Настройки, СвойстваЗаголовка, Раздел.Строки);
				СброситьСортировку(Настройки, СвойстваЗаголовка, Раздел.Колонки);
...
Процедура УдалитьЭлементыСортировки(ЭлементыСортировки)
...
Процедура ДобавитьПараметрыРазмераПоляОтчета(Форма, СвойстваЗаголовка, Размер, Ориентация = "Высота")
	...
	Если Ориентация = "Ширина"
		И СвойстваЗаголовка.КоличествоРазделов = 1 Тогда 
...
Процедура ПослеИзмененияЭлементаОформленияГруппировкиОтчета(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ДополнительныеПараметры.КоличествоРазделов = 1 Тогда 
...
Процедура НайтиПоляГруппировки(Настройки, Группировка, ДобавляемыеПоля)
	...
	Для Каждого Элемент Из ПоляГруппировки.Элементы Цикл 
		...
		Если ТипЗнч(Элемент) = Тип("ПолеГруппировкиКомпоновкиДанных")
			...
			ВыбранноеПолеГруппировки = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(Группировка.Выбор, Элемент.Поле);
			ВыбранноеПолеНастроек = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(Настройки.Выбор, Элемент.Поле);
...
Процедура НайтиВыбранныеПоляНастроек(КомпоновщикНастроек, Группировка, Узел, Поля, ДобавляемыеПоля)
	...
	Для Каждого Элемент Из Поля.Элементы Цикл 
		...
		ИначеЕсли ТипЗнч(Элемент) <> Тип("АвтоВыбранноеПолеКомпоновкиДанных")
			...
			ПолеОтчета = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(Группировка.Выбор, Элемент.Поле);
...
Процедура ПереместитьПолеВертикально(Форма, Команда)
	...
	Если ДоступноОбновлениеПолейРаздела(Группировка, Команда) Тогда 
		ОбновитьПоляРаздела(КомпоновщикНастроек, Настройки, Раздел);
	КонецЕсли;
	
	ПолеГруппировки = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(Группировка.ПоляГруппировки, СвойстваЗаголовка.Поле);
	ВыбранноеПоле = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(Группировка.Выбор, СвойстваЗаголовка.Поле);
...
Функция ДоступноОбновлениеПолейРаздела(Группировка, Команда)
	...
	ЭтоАвтоПоле = КоличествоВыбранныхПолей = 1
		И ТипЗнч(ВыбранныеПоля[0]) = Тип("АвтоВыбранноеПолеКомпоновкиДанных");
	
	Возврат ДействиеКоманды(Команда) <> "ПереместитьПолеВыше"
		Или КоличествоПолейГруппировки = 1
		Или КоличествоВыбранныхПолей > 0 И Не ЭтоАвтоПоле;
...
Функция ДействиеДоступно(Форма, Команда = Неопределено)
...
Процедура ДобавитьФормулу(Настройки, КоллекцияПолей, ОписаниеФормулы) Экспорт 
	...
	УстановитьСвойстваФормулы(Формула, ОписаниеФормулы);
...
Процедура ИзменитьФормулу(Форма, Настройки, ПутьКДанным, ИмяКоллекции = "ДоступныеПоляВыбора", ОбработатьВФорме = Ложь) Экспорт 
	...
	Модуль = ?(ОбработатьВФорме, Форма, ЭтотОбъект);
	Обработчик = Новый ОписаниеОповещения("ПослеИзмененияФормулы", Модуль, Формула);
...
Процедура УстановитьСвойстваФормулы(Формула, ОписаниеФормулы) 
