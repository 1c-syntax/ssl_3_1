...
Процедура ОбработкаДополнительнойРасшифровки(Форма, Данные, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт 
	...
	Иначе
		...
		ДополнительноеМеню = КонтекстноеМенюОбластиДанных(Данные.ДоступныеВидыСравнения);
	КонецЕсли;
...
Процедура ПоказатьРасширеннуюНастройкуФильтра(Форма, СвойстваЗаголовка, ПравоеЗначение = Неопределено) Экспорт 
	...
	ПараметрыФормы.Вставить("НастройкиОтчета", НастройкиОтчета);
	ПараметрыФормы.Вставить("СвойстваЗаголовка", СвойстваЗаголовка);
	ПараметрыФормы.Вставить("ПравоеЗначение", ПравоеЗначение);
...
Процедура СкрытьГруппировку(Настройки, Группировка, Поле)
	...
	Группировки = Группировка.Родитель.Структура;
...
Процедура Фильтровать(Форма, ВидСравнения, СвойстваЗаголовка, ДанныеРасшифровки)
	...
	Если ТипЗнч(ВидСравнения) = Тип("Строка") Тогда 
		...
		ПоказатьРасширеннуюНастройкуФильтра(Форма, СвойстваЗаголовка, ДанныеРасшифровки.Значение);
		Возврат;
...
Процедура НайтиПоляГруппировки(Настройки, Группировка, ДобавляемыеПоля)
	...
	Для Каждого Элемент Из Группировка.ПоляГруппировки.Элементы Цикл 
		...
		Если ТипЗнч(Элемент) = Тип("ПолеГруппировкиКомпоновкиДанных")
			...
			Если ВыбранноеПолеГруппировки = Неопределено
				...
				ДобавитьПолеГруппировки(ДобавляемыеПоля, Группировка.ПоляГруппировки, Элемент);
...
Функция КонтекстноеМенюОбластиЗаголовка()
	...
	// Вставить
	КонтекстноеМеню.Добавить("ВставитьПолеСправа", НСтр("ru = 'Вставить поле справа...'"),, БиблиотекаКартинок.ВставитьКолонкуСправа);
	КонтекстноеМеню.Добавить("ВставитьГруппировкуНиже", НСтр("ru = 'Вставить группировку ниже...'"),, БиблиотекаКартинок.ВставитьГруппировкуНиже);
	
	// Переместить
	КонтекстноеМеню.Добавить("ПереместитьПолеВлево", НСтр("ru = 'Переместить влево'"),, БиблиотекаКартинок.ПереместитьКолонкуВлево);
	КонтекстноеМеню.Добавить("ПереместитьПолеВправо", НСтр("ru = 'Переместить вправо'"),, БиблиотекаКартинок.ПереместитьКолонкуВправо);
	КонтекстноеМеню.Добавить("ПереместитьПолеВыше", НСтр("ru = 'Переместить выше'"),, БиблиотекаКартинок.ПереместитьГруппировкуВыше);
	КонтекстноеМеню.Добавить("ПереместитьПолеНиже", НСтр("ru = 'Переместить ниже'"),, БиблиотекаКартинок.ПереместитьГруппировкуНиже);
	
	//
	КонтекстноеМеню.Добавить("СкрытьПоле", НСтр("ru = 'Скрыть'"));
	КонтекстноеМеню.Добавить("ПереименоватьПоле", НСтр("ru = 'Переименовать...'"));
...
Функция КонтекстноеМенюОбластиДанных(ДоступныеВидыСравнения)
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт 
	...
	Если ТипЗнч(ВыполненноеДействие) = Тип("ВидСравненияКомпоновкиДанных")
		Или ТипЗнч(ВыполненноеДействие) = Тип("Строка") И ВыполненноеДействие = "ФильтроватьЕще" Тогда 
...
Функция КомпоновщикНастроекОтчета(Форма)
	...
	Если СтрЗаканчиваетсяНа(Форма.ИмяФормы, "Форма")
		...
		Возврат Форма.Отчет.КомпоновщикНастроек;
	КонецЕсли;
...
Функция ИспользуемыеНастройки(Форма, ИдентификаторНастроек = Неопределено)
	...
	Если СтрЗаканчиваетсяНа(Форма.ИмяФормы, "Форма")
		...
		ИспользуемыеНастройки = СвойстваРезультата.ИтоговыеНастройки;
