...
Процедура Сортировать(Форма, Команда, СвойстваЗаголовка = Неопределено) Экспорт 
	...
	ОповеститьОЗавершенииКонтекстнойНастройки(Форма, ДействиеКоманды(Команда), Неопределено, Ложь);
...
#Если ВебКлиент Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.20.1859") > 0 Тогда
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт 
	...
	Если ВыполненноеДействие =  "РасшифроватьПоДетальнымЗаписям" Тогда 
		...
		ПодготовитьОтборОбработчикаРасшифровки(ДополнительныеПараметры);
		ПодготовитьПрименяемыеНастройкиОбработчикаРасшифровки(ВыполненноеДействие, ДополнительныеПараметры);
...
Процедура ПодготовитьОтборОбработчикаРасшифровки(ДополнительныеПараметры)
	...
	Для Каждого Элемент Из Данные.Отбор Цикл 
		...
		СвойстваОтбора = СвойстваЭлементаОтбораОбработчикаРасшифровки(Данные.Настройки.Отбор, Элемент);
		Отбор.Вставить(Элемент.Ключ, СвойстваОтбора);
		
	КонецЦикла;
	
	ДополнительныеПараметры.Вставить("Отбор", Отбор);
	
	ДополнитьОтборОбработчикаРасшифровки(
		ДополнительныеПараметры.Отбор,
		ДополнительныеПараметры.Данные.Настройки.Структура);
	
КонецПроцедуры

Процедура ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементыСтруктуры)
	...
	Для Каждого ЭлементСтруктуры Из ЭлементыСтруктуры Цикл 
		...
		Если ТипЗнч(ЭлементСтруктуры) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда 
			...
			ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементСтруктуры.Настройки.Структура);
		
		ИначеЕсли ТипЗнч(ЭлементСтруктуры) = Тип("ТаблицаКомпоновкиДанных") Тогда 
			
			ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементСтруктуры.Строки);
		...
		ИначеЕсли ТипЗнч(ЭлементСтруктуры) = Тип("ГруппировкаКомпоновкиДанных")
			...
			Для Каждого ЭлементОтбора Из ЭлементСтруктуры.Отбор.Элементы Цикл 
				ДобавитьЭлементОтбораОбработчикаРасшифровки(Отбор, ЭлементОтбора);
			КонецЦикла;
			
			ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементСтруктуры.Структура);
...
Процедура ДобавитьЭлементОтбораОбработчикаРасшифровки(Отбор, ЭлементОтбора)
	...
	ПутьКДаннымПоля = Строка(ЭлементОтбора.ЛевоеЗначение);
	
	Если Отбор.Свойство(ПутьКДаннымПоля) Тогда 
		Возврат;
	КонецЕсли;
	
	СвойстваЭлементаОтбора = ВариантыОтчетовСлужебныйКлиентСервер.СвойстваОтбораОбработчикаРасшифровкиПоДетальнымЗаписям();
	ЗаполнитьЗначенияСвойств(СвойстваЭлементаОтбора, ЭлементОтбора,
		"ВидСравнения, РежимОтображения, Представление, ИдентификаторПользовательскойНастройки");
	
	Если Не ЗначениеЗаполнено(СвойстваЭлементаОтбора.ИдентификаторПользовательскойНастройки) Тогда 
		
		СвойстваЭлементаОтбора.ИдентификаторПользовательскойНастройки = Новый УникальныйИдентификатор;
		СвойстваЭлементаОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
		
	КонецЕсли;
	
	СвойстваЭлементаОтбора.Значение = ЭлементОтбора.ПравоеЗначение;
	Отбор.Вставить(ПутьКДаннымПоля, СвойстваЭлементаОтбора);
	
КонецПроцедуры


Функция СвойстваЭлементаОтбораОбработчикаРасшифровки(Отбор, ОсновныеСвойства)
	
	ЗначениеЭлементаОтбора = ОсновныеСвойства.Значение;
	
	Если ТипЗнч(ЗначениеЭлементаОтбора) = Тип("СписокЗначений")
		И ЗначениеЭлементаОтбора.Количество() = 1 Тогда 
		
		ЗначениеЭлементаОтбора = ЗначениеЭлементаОтбора[0].Значение;
	КонецЕсли;
	
	СвойстваЭлементаОтбора = ВариантыОтчетовСлужебныйКлиентСервер.СвойстваОтбораОбработчикаРасшифровкиПоДетальнымЗаписям();
	СвойстваЭлементаОтбора.Значение = ЗначениеЭлементаОтбора;
	
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Отбор, ОсновныеСвойства.Ключ);
	
	Если ЭлементыОтбора.Количество() > 0 Тогда 
		ЗаполнитьЗначенияСвойств(СвойстваЭлементаОтбора, ЭлементыОтбора[0], "РежимОтображения, Представление, ИдентификаторПользовательскойНастройки");
	КонецЕсли;
...
Процедура ПодготовитьПрименяемыеНастройкиОбработчикаРасшифровки(ВыполненноеДействие, ДополнительныеПараметры)
	
	Настройки = ДополнительныеПараметры.Данные.Настройки;
	...
	ДополнительныеСвойства.Вставить("ДействиеОбработкиРасшифровки", ВыполненноеДействие);
	ДополнительныеСвойства.Вставить("СвойстваЗаголовка", ДополнительныеПараметры.СвойстваОбласти.СвойстваЗаголовка);
...
Процедура ОповеститьОЗавершенииКонтекстнойНастройки(Форма, Действие, ТекущееПоле = Неопределено, РазвернутьАвтоПоля = Истина)
