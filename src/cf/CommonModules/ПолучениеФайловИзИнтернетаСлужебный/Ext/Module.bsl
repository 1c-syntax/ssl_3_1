...
Процедура СохранитьНастройкиПроксиНаСервере1СПредприятие(Знач Настройки) Экспорт
	...
	Константы.НастройкаПроксиСервера.Установить(Новый ХранилищеЗначения(Настройки, Новый СжатиеДанных(9)));
...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
	...
	Если НастройкаСоединения.ПроверятьДоступКИнтернетСервисам И Не ОбщегоНазначения.РазрешенДоступКИнтернетСервисам() Тогда
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить файл %1 по причине:
					|%2'"), URL, ОбщегоНазначения.ЗапрещенДоступКИнтернетСервисамТекстСообщения());
...
Функция ПроверитьДоступностьСервера(АдресСервера) Экспорт
	...
	Если Не ДоменноеИмяКорректное(АдресСервера) Тогда
		Результат.ЖурналДиагностики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверный адрес сервера: %1'"), АдресСервера);
		Возврат Результат;
	КонецЕсли;
...
Функция ЖурналТрассировкиМаршрутаСервера(АдресСервера) Экспорт
	...
	Если Не ДоменноеИмяКорректное(АдресСервера) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Трассировка маршрута к удаленному серверу %1 не выполнена:
				|Неверный адрес сервера.'"), АдресСервера);
		Возврат ТекстОшибки;
	КонецЕсли;
...
Функция ДоменноеИмяКорректное(ДоменноеИмя)
