		...
		Попытка
			...
			Если ОтветHTTP.КодСостояния = 301 // 301 Moved Permanently
				...
				Иначе 
					...
					НовыйURL = ЗаголовкиВНижнийРегистр(ОтветHTTP.Заголовки)["location"];
			...
			Если ОтветHTTP.КодСостояния < 200 Или ОтветHTTP.КодСостояния >= 300 Тогда
				...
				Если ОтветHTTP.КодСостояния = 304 Тогда
					...
					HTTPЗаголовки = ЗаголовкиВНижнийРегистр(ЗапросHTTP.Заголовки);
...
Функция РазмерФайлаHTTP(СоединениеHTTP, Знач ПутьНаСервере, Знач Заголовки = Неопределено)
	...
	Исключение
		Возврат 0;
	КонецПопытки;
	РазмерСтрокой = ЗаголовкиВНижнийРегистр(ПолученныеЗаголовки.Заголовки)["content-length"];
