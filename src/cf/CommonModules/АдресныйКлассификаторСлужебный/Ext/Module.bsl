...
Функция СписокДомов(ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска, ПорцияПриПоиске = 20) Экспорт
	...
	ВариантыДомов.Сортировать("ПредставлениеДляСортировки");
...
Функция ОдноуровневыйАвтоподборНаселенногоПункта(ТипаАдреса, Уровень, Порция = 20)
	...
	Если Порция > 0 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%Порция%", "ПЕРВЫЕ " + XMLСтрока(Порция));
	Иначе
...
Функция ПервичнаяПроверкаАдресов(Знач Адреса)
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		НаименованиеСубъектаРФ = СокрЛП(Адрес.Area + " " + Адрес.AreaType);
...
Процедура ИзвлечьСписокДомов(СтрокаСоСпискомДомов, СтрокаПоиска, ВариантыДомов, Запись, ТочноеСовпадение)
	...
	Для каждого Дом Из МассивДомов Цикл
		...
		ОписаниеДома.Дом              = СокрЛП(Сред(НомерДома[0], 2));
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Строение) Тогда
		...
		НазваниеСтроения = ТРег(НазваниеСтроения);
	...
	Если ВариантыДомов.Найти(Строка(ТекстПроДомФормат), "Представление") <> Неопределено Тогда
		Возврат; // Дубль дома
	КонецЕсли;
	...
	ИнформацияОДоме.ПредставлениеДляСортировки = СокрЛП(ОписаниеДома.НазваниеВладения) + НомераДляСортировки(ОписаниеДома.Дом)
	 + ?(ЗначениеЗаполнено(ОписаниеДома.Корпус), "корпус", "") + НомераДляСортировки(ОписаниеДома.Корпус)
	 + СокрЛП(НазваниеСтроения) + НомераДляСортировки(ОписаниеДома.Строение);
...
Функция НомераДляСортировки(ИсходнаяСтрока)
	...
	ТипЧисло  = ОбщегоНазначения.ОписаниеТипаЧисло(6);
	Результат = ТипЧисло.ПривестиЗначение(ИтоговоеЧисло);
	
	Если Результат = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат Формат(Результат ,"ЧЦ=6; ЧН=; ЧВН=; ЧГ=0" );
...
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * ПредставлениеДляСортировки - Строка
//   * Дом - Строка
//   * Корпус - Строка
//   * Строение - Строка
...
Функция КонструкторСпискаДомов() Экспорт
	...
	ВариантыДомов.Колонки.Добавить("ПредставлениеДляСортировки",       ОбщегоНазначения.ОписаниеТипаСтрока(250));
	...
	ВариантыДомов.Колонки.Добавить("Значение", Новый ОписаниеТипов("Структура"));
	ВариантыДомов.Колонки.Добавить("Представление", ОбщегоНазначения.ОписаниеТипаСтрока(250));
	ВариантыДомов.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("УникальныйИдентификатор"));
	
	ВариантыДомов.Индексы.Добавить("ПредставлениеДляСортировки");
	ВариантыДомов.Индексы.Добавить("Представление");
