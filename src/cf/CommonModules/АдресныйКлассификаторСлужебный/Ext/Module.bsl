...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, КешПредставлений, Знач Текст, ДополнительныеПараметры, ЗагруженныеАдресныеСведения)
	...
	Для Каждого СтрокаАдреса Из РезультатЗапроса.Выгрузить() Цикл
		...
		Для Позиция = 0 По 5 Цикл
			Наименование = СтрокаАдреса["Наименование" + Строка(Позиция)];
			Если ПустаяСтрока(Наименование) Тогда
				Прервать;
			КонецЕсли;
			Если СтрокаАдреса["Уровень" + Строка(Позиция)] = 1 Тогда
				НаборПредставление.Добавить(Наименование + " " + СтрокаАдреса["Сокращение" + Строка(Позиция)]);
			Иначе
				НаборПредставление.Добавить(СтрокаАдреса["Сокращение" + Строка(Позиция)] + " " + Наименование);
			КонецЕсли;
