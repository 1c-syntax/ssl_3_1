	...
	Для Каждого СведенияОЗагрузкеСубъектаРФ Из СведенияОЗагрузкеСубъектовРФ Цикл
		...
		Если ТипЗнч(Значение) = Тип("Структура") И Значение.ИспользоватьЗагруженные Тогда
			СведенияОРегионах.Добавить(
				...
				НСтр("ru = 'Загружен'") + " - " + Строка(Значение.ДатаЗагрузки));
		КонецЕсли;
		...
		Для Каждого АдресИзВыборки Из Выборка Цикл
			...
			Для Позиция = 0 По 5 Цикл
				Если СведенияОбАдресномОбъекте.Родитель = АдресИзВыборки["Идентификатор" + Строка(Позиция)] Тогда
					...
					Прервать;
				ИначеЕсли СтрНайти(ВРег(СведенияОбАдресномОбъекте.Представление), ВРег(АдресИзВыборки["Наименование"
				   + Строка(Позиция)])) = 0 Тогда
					МуниципальныйАдресНайден = Ложь;
					Прервать;
				Иначе
					МуниципальныйАдрес = АдресИзВыборки;
...
Функция ПроверитьОКМТОБюджетополучателя(ОКТМОБюджетополучателей) Экспорт
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("АдресныйКлассификаторСлужебный.ПроверитьОКМТОБюджетополучателя",
...
Функция ОбработатьПорциюОКТМОБюджетополучателей(Результат, Порция)
	...
	Если РезультатПроверки.КодСостояния = 200 Тогда
		...
		Если ТипЗнч(ДанныеПроверки) = Тип("Соответствие") Тогда
			...
			Для каждого ДанныеОКТМО Из НаборОКТМО Цикл
				...
				ЗначениеОКМТО = ДанныеОКТМО.Значение;
				ДанныеПоОКТМО = ДанныеПоОКТМО();
				
				Если ЗначениеОКМТО["validateOktmoStatus"] = "VERIFIED_OKTMO" Тогда
					ДанныеПоОКТМО.Успешно = Истина;
				КонецЕсли;
				Если ЗначениеЗаполнено(ЗначениеОКМТО["validateMessage"]) Тогда
					ДанныеПоОКТМО.ТекстОшибки = ЗначениеОКМТО["validateMessage"];
				КонецЕсли;
				Если ЗначениеЗаполнено(ЗначениеОКМТО["oktmoMunValue"]) Тогда
					ДанныеПоОКТМО.МуниципальныеОбразования = ЗначениеОКМТО["oktmoMunValue"];
				КонецЕсли;
	...
	Иначе
		...
		Результат.Отказ = Истина;
		ТекстОшибки     = НСтр("ru = 'При проверке ОКМТО бюджетополучателя через веб-сервис возникли проблемы.'");
...
Процедура ОбработатьОшибкуОбращенияКВебСервисуПриПолученииОКТМО(Результат)
	...
	Результат.ПредставлениеОшибкиДляПользователя = НСтр("ru = 'При проверке ОКМТО бюджетополучателя через веб-сервис возникли проблемы.'");
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,,
