...
Функция ИзвлечьФайлыВКаталог(ОписаниеФайлов)
	...
	Для Каждого ОписаниеФайла Из ОписаниеФайлов Цикл
		...
		Если ВРег(Прав(ПолноеИмяФайла, 4)) = ".ZIP" Тогда
			...
			ФайлыСтарогоАдресногоКлассификатора = НайтиФайлы(КаталогФайлов, "*EXTRAINFO.FI");
			ФайлыНовогоАдресногоКлассификатора = НайтиФайлы(КаталогФайлов, "*EXTRAACT.FI");
			
			Если ФайлыНовогоАдресногоКлассификатора.Количество() = 0 Тогда
				
				Если ФайлыСтарогоАдресногоКлассификатора.Количество() > 0 Тогда
					
					ТекстПричины = НСтр("ru = 'Не допускается загрузка адресных сведений без поддержки муниципальных адресов.'");
					
				Иначе
					
					ТекстПричины = НСтр("ru = 'Файл не содержит адресных сведений'");
...
Функция КонтактнаяИнформацияВФорматеКлючЗначениеВJSON(Адрес)
	...
	ВызватьИсключение НСтр("ru = 'Сведения контактной информации были повреждены. Для их восстановления обратитесь к техническому специалисту.'");
