...
Процедура УстановитьМуниципальныеСведения(Адрес, ОКТМО, ИдентификаторАдреса = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(ИдентификаторАдреса) Тогда
			
			Результат = АктуальныеАдресныеСведения(Новый Структура("Идентификатор", ИдентификаторАдреса));
	...
	Если Не ЭтоГородФедеральногоЗначения(ПолноеНаименованиеРегиона(Адрес.Area, Адрес.AreaType)) Тогда
		Возврат;
	КонецЕсли;
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ТипАдреса, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Попытка
			...
			ДополнительныеПараметрыПоиска = "";
			Если ДополнительныеПараметры.Свойство("Позиция") Тогда
				Позиция = Формат(ДополнительныеПараметры.Позиция,"ЧГ=0");
				ДополнительныеПараметрыПоиска = ДополнительныеПараметрыПоиска + "&from=" + Позиция;
			КонецЕсли;
			
			Если ДополнительныеПараметры.Свойство("СтрокаПоиска")
				 И ЗначениеЗаполнено(ДополнительныеПараметры.СтрокаПоиска) Тогда
					ДополнительныеПараметрыПоиска = ДополнительныеПараметрыПоиска + "&text=" + ДополнительныеПараметры.СтрокаПоиска;
			КонецЕсли;
			
			Если Уровень > 1 Тогда
				ШаблонаЗапроса = "select?level=%1&base=0&limit=%2&parentId=%3" + ДополнительныеПараметрыПоиска;
			Иначе
				ШаблонаЗапроса = "select?level=%1&base=0&limit=%2" + ДополнительныеПараметрыПоиска;
			КонецЕсли;
		...
		Попытка
			...
			Для каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
				Если ЗначениеЗаполнено(СтрокаДанных[ИмяПоляИдентификатор]) Тогда
					Если ДанныеВыбора.Данные.НайтиПоЗначению(СтрокаДанных[ИмяПоляИдентификатор]) = Неопределено Тогда
...
Функция СписокДомов(ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска, ПорцияПриПоиске = 20) Экспорт
	...
	Если ВариантыДомов.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		НаименованиеЗданийИСтроений = АдресныйКлассификаторПовтИсп.НаименованиеВладенийИСтроений();
...
Функция АктуальныеАдресныеСведения(СведенияОбАдресномОбъекте) Экспорт
	...
	Если СведенияОбАдресномОбъекте.Свойство("ЗагруженныеДанные") И ТипЗнч(СведенияОбАдресномОбъекте.ЗагруженныеДанные) = Тип("Булево") Тогда
	...
	Если ЗагруженныеДанные = Истина Тогда
		...
		Результат.ИсключатьГородИзМуниципальногоАдреса = ИсключатьГородИзМуниципальногоАдреса(Результат.Данные);
	КонецЕсли;
...
//  Булево
//
Функция ИсключатьГородИзМуниципальногоАдреса(Адрес) Экспорт
	
	Если АдресПустой(Адрес) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	УровеньРодительскогоОбъекта = УровеньМуниципальногоРодительскогоОбъектаПоАдресу(Адрес);
	Если УровеньРодительскогоОбъекта = 31 ИЛИ УровеньРодительскогоОбъекта = 41 Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПроверкаАдресов(Знач НаборАдресов, ПроверятьУстареваниеАдресов = Ложь) Экспорт
	...
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] И АдресаЗагруженныеДанные.Количество() > 0 Тогда
		РезультатыЗагруженныеДанные = ПроверитьАдресаПоКлассификатору(АдресаЗагруженныеДанные, ПроверятьУстареваниеАдресов);
...
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Представление = "") Экспорт
	...
	Иначе
		...
		СубъектРФ = НаименованиеИСокращение(АдаптацияНазванияРегиона(АдресРФ.СубъектРФ));
...
Функция РезультатПроверкиАдресов(НаборАдресов, ПроверятьУстареваниеАдресов = Ложь) Экспорт
	...
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] И АдресаЗагруженныеДанные.Количество() > 0 Тогда
		РезультатЗагруженныеДанные = ПроверитьАдресаПоКлассификатору(АдресаЗагруженныеДанные, ПроверятьУстареваниеАдресов);
	КонецЕсли;
...
Функция КраткоеНаписаниеМуниципальныхРайонов(Наименование, Сокращение) Экспорт
	...
	ВырезаемаяПодстрока = МуниципальныеРайоны.Получить(ВРег(Сокращение));
...
Процедура ПолныеНаименованияСокращений(СоответствиеСокращений) Экспорт
	...
	Для Каждого ОписаниеСокращения Из СоответствиеСокращений Цикл
		...
		ПолноеНаименование = Строка(СписокСокращений.Получить(Строка(ОписаниеСокращения.Ключ) + ОписаниеСокращения.Значение));
...
Функция СведенияОРегионе(НаименованиеРегиона) Экспорт
	...
	Результат = АдресныйКлассификаторПовтИсп.СведенияОРегионе(АдаптацияНазванияРегиона(НаименованиеРегиона));
	Возврат Результат;
...
Функция СведенияОЗагрузкеСубъектовРФ() Экспорт
	...
	|	Классификатор.Сокращение КАК Сокращение,
	...
	ЗагруженныеСведения.Колонки.Добавить("Представление", ОбщегоНазначения.ОписаниеТипаСтрока(250));
	...
	// Формируем наименования регионов по правилам их написания. 
	Для Каждого Строка Из ЗагруженныеСведения Цикл
		Строка.Представление = СоединитьНаименованиеИТипАдресногоОбъекта(Строка.Наименование, Строка.Сокращение, Истина);
...
Функция АдаптацияНазванияРегиона(Название) Экспорт
	...
	Если СтрСравнить(Название, "Кемеровская область - Кузбасс обл") = 0
		 ...
		 Или СтрСравнить(Название, "Кемеровская область")  = 0 Тогда
			Возврат ДействующиеНаименованиеКемеровскойОбласти();
	КонецЕсли;
	// АПК: 1297-вкл
	
	Возврат СокрЛП(Название);
...
// Параметры:
//  ЧастиАдреса - см. УправлениеКонтактнойИнформациейСлужебный.ЧастиАдреса
// Возвращаемое значение:
//  Структура:
//   * Country - Строка
//   * ID - Строка
...
//   * DistrictID - Строка
//   * MunDistrict - Строка
//   * MunDistrictType - Строка
//   * MunDistrictID - Строка
//   * CityID - Строка
//   * Settlement - Строка
//   * SettlementType - Строка
//   * SettlementID - Строка
//   * CityDistrict - Строка
//   * CityDistrictType - Строка
//   * CityDistrictID - Строка
//   * Territory - Строка
//   * TerritoryType - Строка
//   * TerritoryID - Строка
//   * Locality - Строка
//   * LocalityType - Строка
//   * LocalityID - Строка
...
//   * IFNSFLCode - Строка
//   * IFNSULCode - Строка
//   * IFNSFLAreaCode - Строка
//   * IFNSULAreaCode - Строка
//
Функция РаспознатьАдресЗагруженныеДанные(ЧастиАдреса)
	...
	Для каждого ЭлементАдреса Из ДомаЗдания Цикл
			...
			Тип = ВРег(СокрЛП(СтрЗаменить(ЭлементАдреса.Сокращение, "№", "")));
			Значение = ЭлементАдреса.Наименование;
			
			Если КодыДомовВладенийИСтроений.Дома[Тип] <> Неопределено Тогда
				СведенияОДоме.ТипДома = ТРег(ЭлементАдреса.Сокращение);
				СведенияОДоме.НомерДома = Значение;
			ИначеЕсли КодыДомовВладенийИСтроений.Строения[Тип] <> Неопределено Тогда
				Строения.Добавить(ЗначениеСтроенияИлиПомещения(ТРег(ЭлементАдреса.Сокращение), Значение));
				Если КодыДомовВладенийИСтроений.Строения[Тип] = -1 Тогда
					СведенияОДоме.НомерКорпуса = Значение;
				Иначе
					СведенияОДоме.ТипСтроения   = ТРег(ЭлементАдреса.Сокращение);
					СведенияОДоме.НомерСтроения = Значение;
				КонецЕсли;
			Иначе
				Квартиры.Добавить(ЗначениеСтроенияИлиПомещения(ТРег(ЭлементАдреса.Сокращение), Значение));
			КонецЕсли;
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ ПЕРВЫЕ 10
		|	АдресныеОбъекты0.Наименование + "" "" + АдресныеОбъекты0.Сокращение КАК Представление,
		|	АдресныеОбъекты.Идентификатор КАК Идентификатор,
		...
		|ИЗ
		|	РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыМуниципальный
		|	
		|ГДЕ
		|	АдресныеОбъекты.Наименование = &ЧастьАдреса И АдресныеОбъекты.Сокращение = &ЧастьАдресаСокращение
		|
		|УПОРЯДОЧИТЬ ПО
		...
		|ИЗ
		|	РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыАдминистративный
		|	
		|ГДЕ
		|	АдресныеОбъекты.Наименование = &ЧастьАдреса И АдресныеОбъекты.Сокращение = &ЧастьАдресаСокращение
		|
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
		"АдресныеОбъекты0.Наименование + "" "" + АдресныеОбъекты0.Сокращение КАК Представление,", 
		"АдресныеОбъекты0.Наименование + "" "" + АдресныеОбъекты0.Сокращение + "+ СтрСоединить(Выбор, " + ") + " КАК Представление,");

	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
		"АдресныеОбъекты.Идентификатор КАК Идентификатор,", 
		"АдресныеОбъекты" + Строка(СопоставленныеЧастиАдреса.Количество() - 1) + ".Идентификатор КАК Идентификатор,");
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
		"РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыМуниципальный", 
		"РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты0 " + Символы.ПС + СтрСоединить(СоединениеМуниципальный, Символы.ПС));
			
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
		"РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыАдминистративный", 
		"РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты0 " + Символы.ПС + СтрСоединить(Соединение, Символы.ПС));
	...
	ВариантыАдресов = ДополнительныеСведенияНайденныхАдресов(НайденныеАдреса);
...
// Параметры:
//  НайденныеАдреса - РезультатЗапроса
//
// Возвращаемое значение:
//  ТаблицаЗначений:
...
//   * ДополнительныеАдресныеСведения - Строка
//   * ПочтовыйИндекс - Строка
//   * Уровень - Число
//
Функция ДополнительныеСведенияНайденныхАдресов(НайденныеАдреса)
	
	Возврат НайденныеАдреса.Выгрузить();
...
Процедура УстановитьИдентификаторыАдресаПоЗагруженнымДанным(Адрес)
	...
	Для каждого УровеньАдреса Из ИменаУровнейАдреса Цикл
		...
		Если ЗначениеЗаполнено(Адрес[УровеньАдреса]) Тогда
			...
			Запрос.УстановитьПараметр("Сокращение"   + XMLСтрока(Счетчик), Адрес[УровеньАдреса + "Type"]);
	...
	Для ИндексСчетчик = Счетчик По 5 Цикл
		Запрос.УстановитьПараметр("Наименование" + XMLСтрока(ИндексСчетчик), "");
		Запрос.УстановитьПараметр("Сокращение" + XMLСтрока(ИндексСчетчик), "");
	КонецЦикла;
...
Процедура УстановитьИдентификаторыАдресовПоЗагруженнымДанным(Адреса)
	...
	Запрос.УстановитьПараметр("Наборы", ДанныеДляПроверкиАдресов.АдресаПоПолям);
	Запрос.УстановитьПараметр("КодыРегионов", КодыРегионов);
	
	Запрос.Текст = ТекстЗапросаУстановкиИдентификаторовАдресов();

	РезультатЗапроса        = Запрос.ВыполнитьПакет();
	РезультатЗапросаАдреса  = РезультатЗапроса[8];;
	РезультатЗапросаДома    = РезультатЗапроса[9];
	...
	Пока ПолученныйАдрес.Следующий() Цикл 
		...
		Адрес = Адреса[ПолученныйАдрес.Ключ];
		КакиеУровниПроверять = ДанныеДляПроверкиАдресов.ПроверяемыеУровни[ПолученныйАдрес.Ключ];
		...
		Если ЭтоГородФедеральногоЗначения(ПолноеНаименованиеРегиона(Адрес.Area, Адрес.AreaType)) = Истина
...
Процедура ЗаполнитьАктуальныеАдресныеСведенияВнутр(Результат, Значение)
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	ЕСТЬNULL(ДополнительныеСведения.ПочтовыйИндекс, """"),
		|	ЕСТЬNULL(ДополнительныеСведения.ОКТМО, """"),
		|	ЕСТЬNULL(ДополнительныеСведения.OKATO, """"),
		|	ЕСТЬNULL(ДополнительныеСведения.КодУчасткаИФНСЮЛ, """"),
		|	ЕСТЬNULL(ДополнительныеСведения.КодУчасткаИФНСФЛ, """"),
		|	ЕСТЬNULL(ДополнительныеСведения.КодИФНСЮЛ, """"),
		|	ЕСТЬNULL(ДополнительныеСведения.КодИФНСФЛ, """")
		|ИЗ
...
Функция АктуальныеАдресДляИсторического(Адрес) 
	...
	Запрос.Текст = 
		...
		|	 АдресныеОбъектыУровень0.Идентификатор КАК Идентификатор0,
		|	 АдресныеОбъектыУровень0.Наименование КАК Наименование0,
		|	 АдресныеОбъектыУровень0.Сокращение КАК Сокращение0,
		|	 АдресныеОбъектыУровень0.Уровень КАК Уровень0,
		|	 АдресныеОбъектыУровень1.Идентификатор КАК Идентификатор1,
		|	 АдресныеОбъектыУровень1.Наименование КАК Наименование1,
		|	 АдресныеОбъектыУровень1.Сокращение КАК Сокращение1,
		|	 АдресныеОбъектыУровень1.Уровень КАК Уровень1,
		|	 АдресныеОбъектыУровень2.Идентификатор КАК Идентификатор2,
		|	 АдресныеОбъектыУровень2.Наименование КАК Наименование2,
		|	 АдресныеОбъектыУровень2.Сокращение КАК Сокращение2,
		|	 АдресныеОбъектыУровень2.Уровень КАК Уровень2,
		|	 АдресныеОбъектыУровень3.Идентификатор КАК Идентификатор3,
		|	 АдресныеОбъектыУровень3.Наименование КАК Наименование3,
		|	 АдресныеОбъектыУровень3.Сокращение КАК Сокращение3,
		|	 АдресныеОбъектыУровень3.Уровень КАК Уровень3,
		|	 АдресныеОбъектыУровень4.Идентификатор  КАК Идентификатор4,
		|	 АдресныеОбъектыУровень4.Наименование КАК Наименование4,
		|	 АдресныеОбъектыУровень4.Сокращение КАК Сокращение4,
		|	 АдресныеОбъектыУровень4.Уровень КАК Уровень4,
		|	 АдресныеОбъектыУровень5.Идентификатор  КАК Идентификатор5,
		|	 АдресныеОбъектыУровень5.Наименование КАК Наименование5,
		|	 АдресныеОбъектыУровень5.Сокращение КАК Сокращение5,
	...
	ТекстОтбор   = ?(Счетчик > 2, " И НЕ ИсторияАдресныхОбъектов1.Наименование ЕСТЬ NULL", "");
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрСведений.ИсторияАдресныхОбъектов КАК ИсторияАдресныхОбъектов0", 
		"РегистрСведений.ИсторияАдресныхОбъектов КАК ИсторияАдресныхОбъектов0
		| " + ТекстСоединения);
		
	Если ЗначениеЗаполнено(ТекстОтбор) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ИсторияАдресныхОбъектов0.Сокращение = &Сокращение0",
			"И ИсторияАдресныхОбъектов0.Сокращение = &Сокращение0
			|" + ТекстОтбор);
	КонецЕсли;
	...
	Пока Результат.Следующий() Цикл
		...
		Для Каждого ИмяУровня Из ИменаУровней Цикл
			НовыйАдрес[ИмяУровня.Значение]          = "";
			НовыйАдрес[ИмяУровня.Значение + "Type"] = "";
			НовыйАдрес[ИмяУровня.Значение + "Id"]   = "";
		КонецЦикла;
		
		Для Индекс = 0 По 5 Цикл
			Уровень = Результат["Уровень" + Индекс];
			Если ЗначениеЗаполнено(Уровень) Тогда
				НовыйАдрес[ИменаУровней[Уровень]]          = Результат["Наименование"  + Индекс];
				НовыйАдрес[ИменаУровней[Уровень] + "Type"] = Результат["Сокращение"    + Индекс];
				НовыйАдрес[ИменаУровней[Уровень] + "Id"]   = Результат["Идентификатор" + Индекс];
			Иначе
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Варианты.Добавить(НовыйАдрес);
	...
	РезультатПоиска.Варианты = Варианты;
	РезультатПоиска.Представление = Представление;  
...
//  СведенияОРегионе - см. АдресныйКлассификаторСлужебный.НаименованиеИСокращение
// 
Процедура ПроверкаНаУстаревшийРегион(Результат, СведенияОРегионе) Экспорт
...
//  Неопределено
//
Функция АктуальныеДанныеОРегионе(Знач СведенияОРегионе) Экспорт
	
	Запрос = Новый Запрос;
	
	Если ЗначениеЗаполнено(СведенияОРегионе.Сокращение) Тогда
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, КешПредставлений, Знач Текст, ДополнительныеПараметры, ЗагруженныеАдресныеСведения)
	...
	Для Каждого СтрокаАдреса Из РезультатЗапроса.Выгрузить() Цикл
		...
		Для Позиция = 0 По 5 Цикл
			...
			НаборПредставление.Добавить(СоединитьНаименованиеИТипАдресногоОбъекта(Наименование, 
				СтрокаАдреса["Сокращение" + ПозицияСтрокой], СтрокаАдреса["Уровень" + ПозицияСтрокой] = 1))
			
		КонецЦикла;
		
		Представление = СтрСоединить(НаборПредставление, ", ");
		
		Если СтрокаАдреса.Уровень > 1 И ЗначениеЗаполнено(СтрокаАдреса.ПочтовыйИндекс) Тогда
		...
		Если СтрокаАдреса.Уровень = 1 Тогда
		...
		Сведения = СведенияОПодбираемомАдресе(Представление, СтрокаАдреса.Идентификатор, СтрокаАдреса.Муниципальный, Истина,, ПредлагатьЗагрузкуКлассификатора);
...
Функция ТаблицаПриоритетовПриАвтоподборе(ТипАдреса)
	...
	Приоритеты.Колонки.Добавить("Уровень", ОбщегоНазначения.ОписаниеТипаЧисло(2));
	Приоритеты.Колонки.Добавить("Порядок", ОбщегоНазначения.ОписаниеТипаЧисло(2));
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 4; // Город
	НоваяКолонка.Порядок = 1;
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 1; // Регион
	НоваяКолонка.Порядок = 2;
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 31;
	НоваяКолонка.Порядок = 2;
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 41;
	НоваяКолонка.Порядок = 3;
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 3; // Район 
	НоваяКолонка.Порядок = 3;

	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 6;
	НоваяКолонка.Порядок = 4;
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 65;
	НоваяКолонка.Порядок = 5;
	
	НоваяКолонка = Приоритеты.Добавить();
	НоваяКолонка.Уровень = 5;
...
Функция ОдноуровневыйАвтоподборНаселенногоПункта(ТипаАдреса, Уровень)
	...
	Если Уровень > 1 Тогда
		...
		Представление = "АдресныеОбъекты.Сокращение + "" "" + АдресныеОбъекты.Наименование";
	Иначе
		ОтборПоИдентификатору = "ИСТИНА";
		Представление = "АдресныеОбъекты.Наименование + "" "" + АдресныеОбъекты.Сокращение";
	КонецЕсли;
...
Функция КаскадныйСписокАвтоподбораНаселенногоПункта(ТипАдреса)
	...
	|ИНДЕКСИРОВАТЬ ПО
	|	Уровень,
	|	Порядок" + ОбщегоНазначения.РазделительПакетаЗапросов();
	
	Если ЭтоАдминистративноТерриториальныйАдрес(ТипАдреса) Тогда
		Возврат ТекстЗапроса + ПодборАдминистративноТерриториальныеАдреса();
	ИначеЕсли ЭтоМуниципальныйАдрес(ТипАдреса) Тогда
		Возврат ТекстЗапроса + ПодборМуниципальногоАдреса();
	КонецЕсли;
	
	ТекстЗапроса = ТекстЗапроса +
	"ВЫБРАТЬ ПЕРВЫЕ 10
	|	АдресныеОбъекты.Наименование КАК Наименование0,
	|	АдресныеОбъекты.Сокращение КАК Сокращение0,
	|	АдресныеОбъекты.Уровень КАК Уровень0,
	|	АдресныеОбъектыУровень1.Наименование КАК Наименование1,
	|	АдресныеОбъектыУровень1.Сокращение КАК Сокращение1,
	|	АдресныеОбъектыУровень1.Уровень КАК Уровень1,
	...
	|	АдресныеОбъекты.Уровень КАК Уровень,
	|	ДополнительныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс,
	|	ЕСТЬNULL(Приоритеты.Порядок, 0) КАК Порядок
	|ПОМЕСТИТЬ МуниципальныеОбъекты
	|ИЗ
	|	РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыУровень1
	...
	|	ДополнительныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс,
	|	ЕСТЬNULL(Приоритеты.Порядок, 0) КАК Порядок
	|ПОМЕСТИТЬ АдминистративноТерриториальныйОбъекты
	|ИЗ
	|	РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыУровень1
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МуниципальноеДеление.Наименование0 КАК Наименование0,
	|	МуниципальноеДеление.Сокращение0 КАК Сокращение0,
	|	МуниципальноеДеление.Уровень0 КАК Уровень0,
	|	МуниципальноеДеление.Наименование1 КАК Наименование1,
	|	МуниципальноеДеление.Сокращение1 КАК Сокращение1,
	|	МуниципальноеДеление.Уровень1 КАК Уровень1,
	|	МуниципальноеДеление.Наименование2 КАК Наименование2,
	|	МуниципальноеДеление.Сокращение2 КАК Сокращение2,
	|	МуниципальноеДеление.Уровень2 КАК Уровень2,
	|	МуниципальноеДеление.Наименование3 КАК Наименование3,
	|	МуниципальноеДеление.Сокращение3 КАК Сокращение3,
	|	МуниципальноеДеление.Уровень3 КАК Уровень3,
	|	МуниципальноеДеление.Наименование4 КАК Наименование4,
	|	МуниципальноеДеление.Сокращение4 КАК Сокращение4,
	|	МуниципальноеДеление.Уровень4 КАК Уровень4,
	|	МуниципальноеДеление.Наименование5 КАК Наименование5,
	|	МуниципальноеДеление.Сокращение5 КАК Сокращение5,
	|	МуниципальноеДеление.Уровень5 КАК Уровень5,
	|	МуниципальноеДеление.Идентификатор КАК Идентификатор,
	|	ИСТИНА КАК Муниципальный,
	|	МуниципальноеДеление.Уровень КАК Уровень,
	|	МуниципальноеДеление.ПочтовыйИндекс КАК ПочтовыйИндекс,
	|	МуниципальноеДеление.Порядок КАК Порядок
	|ИЗ
	|	МуниципальныеОбъекты КАК МуниципальноеДеление
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	АдминистративноТерриториальноеДеление.Наименование0,
	|	АдминистративноТерриториальноеДеление.Сокращение0,
	|	АдминистративноТерриториальноеДеление.Уровень0,
	|	АдминистративноТерриториальноеДеление.Наименование1,
	|	АдминистративноТерриториальноеДеление.Сокращение1,
	|	АдминистративноТерриториальноеДеление.Уровень1,
	|	АдминистративноТерриториальноеДеление.Наименование2,
	|	АдминистративноТерриториальноеДеление.Сокращение2,
	|	АдминистративноТерриториальноеДеление.Уровень2,
	|	АдминистративноТерриториальноеДеление.Наименование3,
	|	АдминистративноТерриториальноеДеление.Сокращение3,
	|	АдминистративноТерриториальноеДеление.Уровень3,
	|	АдминистративноТерриториальноеДеление.Наименование4,
	|	АдминистративноТерриториальноеДеление.Сокращение4,
	|	АдминистративноТерриториальноеДеление.Уровень4,
	|	АдминистративноТерриториальноеДеление.Наименование5,
	|	АдминистративноТерриториальноеДеление.Сокращение5,
	|	АдминистративноТерриториальноеДеление.Уровень5,
	|	АдминистративноТерриториальноеДеление.Идентификатор,
	|	ЛОЖЬ,
	|	АдминистративноТерриториальноеДеление.Уровень,
	|	АдминистративноТерриториальноеДеление.ПочтовыйИндекс,
	|	АдминистративноТерриториальноеДеление.Порядок
	|ИЗ
	|	АдминистративноТерриториальныйОбъекты КАК АдминистративноТерриториальноеДеление
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок";
...
Функция ПодборАдминистративноТерриториальныеАдреса()
	
	ТекстЗапрос = "ВЫБРАТЬ ПЕРВЫЕ 20
	|	АдресныеОбъекты.Наименование КАК Наименование0,
	|	АдресныеОбъекты.Сокращение КАК Сокращение0,
	|	АдресныеОбъекты.Уровень КАК Уровень0,
	|	АдресныеОбъектыУровень1.Наименование КАК Наименование1,
...
Функция ПодборМуниципальногоАдреса()
...
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораВнутр(Результат, Знач ПочтовыйИндекс, ДополнительныеПараметры)
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ТипАдреса) Тогда
		...
		ВключатьАдминистративноТерриториальныеАдреса = ЭтоАдминистративноТерриториальныйАдрес(ДополнительныеПараметры.ТипАдреса);
...
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораВнутр(Результат, Родитель, Уровень, ТипАдреса, ДополнительныеПараметры)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	АдресныеОбъекты.Наименование, АдресныеОбъекты.Сокращение";
	...
	Если Уровень > 1 Тогда
		...
		Если ЭтоМуниципальныйАдрес(ТипАдреса) Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоИдентификатору", "АдресныеОбъекты.МуниципальныйРодительскийИдентификатор = &Идентификатор");
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоИдентификатору", "АдресныеОбъекты.РодительскийИдентификатор = &Идентификатор");
		КонецЕсли;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Представление", "АдресныеОбъекты.Сокращение + "" "" + АдресныеОбъекты.Наименование");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоИдентификатору", "ИСТИНА");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Представление", "АдресныеОбъекты.Наименование + "" "" + АдресныеОбъекты.Сокращение");
	КонецЕсли;
	...
	Для Каждого Строка Из РезультатЗапроса.Выгрузить() Цикл
		...
		Если Уровень = 1 Тогда
			Если НаименованиеРегионаБезТипа(Строка.Наименование) Тогда
				Строка.Представление = Строка.Наименование;
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(Строка.ПочтовыйИндекс) Тогда
...
Функция ИзвлечьФайлыВКаталог(ОписаниеФайлов, Знач КаталогФайлов = Неопределено) Экспорт
	...
	Для Каждого ОписаниеФайла Из ОписаниеФайлов Цикл
		...
		Если ФайлАрхива.Существует() И ВРег(Прав(ПолноеИмяФайла, 4)) = ".ZIP" Тогда
			...
			ЕстьФайлВерсииСАдминистративнымиДанными = Ложь;
			ЕстьФайлВерсииСМуниципальнымиДанными    = Ложь;
			
			Для Каждого ЭлементАрхива Из Архив.Элементы Цикл
				
				Если СтрНайти(ЭлементАрхива.Имя, "EXTRAACT.FI") > 0 Тогда
					ЕстьФайлВерсииСМуниципальнымиДанными = Истина;
					Прервать;
				КонецЕсли;
				
				Если СтрНайти(ЭлементАрхива.Имя, "EXTRAINFO.FI") > 0 Тогда
					ЕстьФайлВерсииСАдминистративнымиДанными = Истина;
				КонецЕсли;
			...
			Если Не ЕстьФайлВерсииСМуниципальнымиДанными Тогда
				...
				ТекстПричины = ?(ЕстьФайлВерсииСАдминистративнымиДанными,
					НСтр("ru = 'Не допускается загрузка адресных сведений без поддержки муниципальных адресов.'"), 
					НСтр("ru = 'Файл не содержит адресных сведений'"));
				
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
				,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось загрузить адресные сведения из файла: %1
				|по причине: %2
				|Используйте адресные сведения, расположенные по адресу https://its.1c.ru/download/fias2'"), ПолноеИмяФайла, ТекстПричины));
				ВызватьИсключение ТекстПричины;
				
			КонецЕсли;
			
			Архив.ИзвлечьВсе(КаталогФайлов + ФайлАрхива.ИмяБезРасширения, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
			Архив.Закрыть();
...
Процедура СписокИзвлеченныхФайлов(КаталогФайлов)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Список извлеченных файлов адресных сведений региона: %1'"), 
				СтрСоединить(СписокФайлов, ",")));
КонецПроцедуры
...
Функция НоваяТаблицаАдресовСРазбивкойНаПоля()
	...
	АдресаПоПолям.Колонки.Добавить("Сокращение0", Строка20);
	АдресаПоПолям.Колонки.Добавить("Наименование1", Строка250);
	АдресаПоПолям.Колонки.Добавить("Сокращение1", Строка20);
	АдресаПоПолям.Колонки.Добавить("Наименование2", Строка250);
	АдресаПоПолям.Колонки.Добавить("Сокращение2", Строка20);
	АдресаПоПолям.Колонки.Добавить("Наименование3", Строка250);
	АдресаПоПолям.Колонки.Добавить("Сокращение3", Строка20);
	АдресаПоПолям.Колонки.Добавить("Наименование4", Строка250);
	АдресаПоПолям.Колонки.Добавить("Сокращение4", Строка20);
	АдресаПоПолям.Колонки.Добавить("Наименование5", Строка250);
	АдресаПоПолям.Колонки.Добавить("Сокращение5", Строка20);
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		Если ЭтоКонтактнаяИнформацияВXML(Адрес) Тогда
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(Результат.Ошибки, "СубъектРФ",
				НСтр("ru = 'Адрес не может быть проверен, так как он в старом формате'") );
		...
		Если СтрСравнить(Адрес.country, ОсновнаяСтрана()) <> 0  Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(Результат.Ошибки, "СубъектРФ",
				НСтр("ru = 'Адрес не может быть проверен, так как он иностранный'") );
			Результат.АдресПроверен   = Истина;
			Результат.АдресКорректный = Истина;
			Продолжить;
		КонецЕсли;
		...
		Если ПустаяСтрока(Адрес.Area) Тогда
			// Нет субъекта
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(Результат.Ошибки, "СубъектРФ", НСтр("ru = 'Не указан регион адреса'"));
			Результат.АдресПроверен   = Истина;
			Продолжить;
		КонецЕсли;
		
		НаименованиеСубъектаРФ = СоединитьНаименованиеИТипАдресногоОбъекта(Адрес.Area, Адрес.AreaType, Истина);
		РегионЗагруженВАдресныеСведения = СведенияОРегионе(НаименованиеСубъектаРФ);
		
		Если РегионЗагруженВАдресныеСведения.Устарел Тогда
			АктуализироватьДанныеОРегионе(Адрес, РегионЗагруженВАдресныеСведения.КодСубъектаРФ, Результат);
		ИначеЕсли РегионЗагруженВАдресныеСведения.КодСубъектаРФ = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(Результат.Ошибки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указан несуществующий регион ""%1""'"), НаименованиеСубъектаРФ));
			Результат.АдресПроверен   = Истина;
		ИначеЕсли Не РегионЗагруженВАдресныеСведения.Загружен Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(Результат.Ошибки, "СубъектРФ",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствуют адресные сведения по региону: %1'"),
				НаименованиеСубъектаРФ));
			Результат.АдресПроверен   = Ложь;
			Результат.АдресКорректный = Истина;
			Продолжить;
		Иначе
			Если Сведения.КодыРегионов.Найти(РегионЗагруженВАдресныеСведения.КодСубъектаРФ) = Неопределено Тогда
				Сведения.КодыРегионов.Добавить(РегионЗагруженВАдресныеСведения.КодСубъектаРФ);
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьАдресаПоЗагруженнымДанным(Адреса, ПроверятьУстареваниеАдресов = Ложь)
	...
	РезультатЗапроса       = Запрос.ВыполнитьПакет();
	ВариантыАдресов        = РезультатЗапроса[8];
	Дома                   = РезультатЗапроса[9];
	ДополнительныеСведения = РезультатЗапроса[11];
	Участки                = РезультатЗапроса[12];
	
	АнализАдресныхОбъектов(Результаты, Адреса, ДанныеДляПроверкиАдресов.ПроверяемыеУровни, ВариантыАдресов, ПроверятьУстареваниеАдресов);
	АнализДомовИДополнительныхСведений(Результаты, Адреса, Дома, Участки, ДополнительныеСведения);
...
Функция ПроверитьАдресаПоКлассификатору(Адреса, ПроверятьУстареваниеАдресов = Ложь)
	...
	АдресГородовФедеральногоЗначения = Новый Соответствие;
	Для Каждого ЭлементПроверки Из Адреса Цикл
		Если Результаты[ЭлементПроверки.Ключ].АдресПроверен = Ложь
			И Результаты[ЭлементПроверки.Ключ].Ошибки.Количество() > 0
			И ЭтоГородФедеральногоЗначения(ПолноеНаименованиеРегиона(ЭлементПроверки.Значение.Area, ЭлементПроверки.Значение.AreaType))
			И ЭтоМуниципальныйАдрес(ЭлементПроверки.Значение.addressType) Тогда
			
				Адрес = ОбщегоНазначения.СкопироватьРекурсивно(ЭлементПроверки.Значение);
				Адрес.munDistrict     = "";
				Адрес.munDistrictType = "";
				Адрес.settlement      = "";
				Адрес.settlementType  = "";
				
				АдресГородовФедеральногоЗначения.Вставить(ЭлементПроверки.Ключ, Адрес);
		КонецЕсли;
	КонецЦикла;

	Если АдресГородовФедеральногоЗначения.Количество() > 0 Тогда
		РезультатыПовторнойПроверки = ПроверитьАдресаПоЗагруженнымДанным(АдресГородовФедеральногоЗначения, ПроверятьУстареваниеАдресов);
		Для каждого РезультатПовторнойПроверки Из РезультатыПовторнойПроверки Цикл
			Результаты[РезультатПовторнойПроверки.Ключ] = РезультатПовторнойПроверки.Значение;
		КонецЦикла;
	КонецЕсли;
...
Функция СформироватьДанныеДляПроверкиАдресов(Знач Адреса, Знач Результаты)
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		ИсключатьГород = ИсключатьГородИзМуниципальногоАдреса(Адрес);
		...
		ИменаУровнейАдреса = ИменаУровнейАдреса(Адрес.AddressType, Истина, ИсключатьГород);
		...
		Для каждого УровеньАдреса Из ИменаУровнейАдреса Цикл
			Если Адрес.Свойство(УровеньАдреса) 
				И ЗначениеЗаполнено(Адрес[УровеньАдреса])
				И Счетчик < 6 Тогда
				...
				Если СтрСравнить(УровеньАдреса, "MunDistrict") = 0 Тогда
					Наименование = КраткоеНаписаниеМуниципальныхРайонов(Наименование, Сокращение);
				КонецЕсли;
				
				Если СтрСравнить(УровеньАдреса, "Settlement") = 0 Тогда
					Наименование = КраткоеНаписаниеПоселений(Наименование, Сокращение);
				КонецЕсли;
				
				АдресПоПолям["Наименование" + XMLСтрока(Счетчик)] = Наименование;
				АдресПоПолям["Сокращение"   + XMLСтрока(Счетчик)] = Сокращение;
				ПроверяемыеУровниАдреса.Вставить(УровеньАдреса, Счетчик);
				Счетчик = Счетчик + 1;
...
Функция ТекстЗапросаПроверкиАдресов()
	...
	ТекстЗапроса = "ВЫБРАТЬ
		|	Наборы.Ключ КАК Ключ,
		|	Наборы.ЭтоМуниципальныйАдрес КАК ЭтоМуниципальныйАдрес,
		|	Наборы.Наименование0 КАК Наименование0,
		|	Наборы.Сокращение0 КАК Сокращение0,
		|	Наборы.Наименование1 КАК Наименование1,
		|	Наборы.Сокращение1 КАК Сокращение1,
		|	Наборы.Наименование2 КАК Наименование2,
		|	Наборы.Сокращение2 КАК Сокращение2,
		|	Наборы.Наименование3 КАК Наименование3,
		|	Наборы.Сокращение3 КАК Сокращение3,
		|	Наборы.Наименование4 КАК Наименование4,
		|	Наборы.Сокращение4 КАК Сокращение4,
		|	Наборы.Наименование5 КАК Наименование5,
		|	Наборы.Сокращение5 КАК Сокращение5
		|ПОМЕСТИТЬ Набор
		|ИЗ
		|	&Наборы КАК Наборы
		|
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Набор.Ключ КАК Ключ,
		|	Набор.ЭтоМуниципальныйАдрес КАК ЭтоМуниципальныйАдрес,
		|	АдресныеОбъекты.Наименование КАК Наименование,
		|	АдресныеОбъекты.Сокращение КАК Сокращение,
		|	АдресныеОбъекты.Уровень КАК Уровень,
		|	АдресныеОбъекты.КодСубъектаРФ КАК КодСубъектаРФ,
		...
		|	Набор КАК Набор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (АдресныеОбъекты.Наименование = Набор.Наименование0)
		|			И (АдресныеОбъекты.Сокращение = Набор.Сокращение0)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
		|		ПО (АдресныеОбъекты.Идентификатор = СвязиАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	АдресныеОбъекты.Уровень = 1
		...
		|	Набор КАК Набор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (АдресныеОбъекты.Наименование = Набор.Наименование1)
		|			И (АдресныеОбъекты.Сокращение = Набор.Сокращение1)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
		|		ПО (АдресныеОбъекты.Идентификатор = СвязиАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	АдресныеОбъекты.КодСубъектаРФ В(&КодыРегионов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	Набор КАК Набор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (АдресныеОбъекты.Наименование = Набор.Наименование2)
		|			И (АдресныеОбъекты.Сокращение = Набор.Сокращение2)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
		|		ПО (АдресныеОбъекты.Идентификатор = СвязиАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	НЕ АдресныеОбъекты.Идентификатор ЕСТЬ NULL
		...
		|	Набор КАК Набор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (АдресныеОбъекты.Наименование = Набор.Наименование3)
		|			И (АдресныеОбъекты.Сокращение = Набор.Сокращение3)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
		|		ПО (АдресныеОбъекты.Идентификатор = СвязиАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	НЕ АдресныеОбъекты.Идентификатор ЕСТЬ NULL
		...
		|	Набор КАК Набор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (АдресныеОбъекты.Наименование = Набор.Наименование4)
		|			И (АдресныеОбъекты.Сокращение = Набор.Сокращение4)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
		|		ПО (АдресныеОбъекты.Идентификатор = СвязиАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	НЕ АдресныеОбъекты.Идентификатор ЕСТЬ NULL
		...
		|	Набор КАК Набор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (АдресныеОбъекты.Наименование = Набор.Наименование5)
		|			И (АдресныеОбъекты.Сокращение = Набор.Сокращение5)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
		|		ПО (АдресныеОбъекты.Идентификатор = СвязиАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	НЕ АдресныеОбъекты.Идентификатор ЕСТЬ NULL
		...
		|	АдресныеОбъектыУровень0.Ключ КАК Ключ,
		|	АдресныеОбъектыУровень0.Наименование КАК НаименованиеУровень0,
		|	АдресныеОбъектыУровень0.Сокращение КАК СокращениеУровень0,
		|	АдресныеОбъектыУровень0.Идентификатор КАК ИдентификаторУровень0,
		|	АдресныеОбъектыУровень0.ДополнительныеСведения КАК ДополнительныеСведения0,
		...
		|	АдресныеОбъектыУровень5.Идентификатор КАК ИдентификаторУровень5,
		|	АдресныеОбъектыУровень5.ДополнительныеСведения КАК ДополнительныеСведения5,
		|	ЕСТЬNULL(АдресныеОбъектыУровень5.Идентификатор, ЕСТЬNULL(АдресныеОбъектыУровень4.Идентификатор, ЕСТЬNULL(АдресныеОбъектыУровень3.Идентификатор, ЕСТЬNULL(АдресныеОбъектыУровень2.Идентификатор, АдресныеОбъектыУровень1.Идентификатор)))) КАК Идентификатор
		|ПОМЕСТИТЬ АдресныеОбъекты
		|ИЗ
		|	АдресныеОбъектыУровень0 КАК АдресныеОбъектыУровень0
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень1 КАК АдресныеОбъектыУровень1
		|		ПО (АдресныеОбъектыУровень0.Ключ = АдресныеОбъектыУровень1.Ключ)
		|			И (АдресныеОбъектыУровень0.Идентификатор = АдресныеОбъектыУровень1.МуниципальныйРодительскийИдентификатор
		|				ИЛИ АдресныеОбъектыУровень0.Идентификатор = АдресныеОбъектыУровень1.СвязиМуниципальныйРодительскийИдентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень2 КАК АдресныеОбъектыУровень2
		|		ПО (АдресныеОбъектыУровень1.Ключ = АдресныеОбъектыУровень2.Ключ)
		|			И (АдресныеОбъектыУровень1.Идентификатор = АдресныеОбъектыУровень2.МуниципальныйРодительскийИдентификатор
		|				ИЛИ АдресныеОбъектыУровень1.Идентификатор = АдресныеОбъектыУровень2.СвязиМуниципальныйРодительскийИдентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень3 КАК АдресныеОбъектыУровень3
		|		ПО (АдресныеОбъектыУровень2.Ключ = АдресныеОбъектыУровень3.Ключ)
		|			И (АдресныеОбъектыУровень2.Идентификатор = АдресныеОбъектыУровень3.МуниципальныйРодительскийИдентификатор
		|				ИЛИ АдресныеОбъектыУровень2.Идентификатор = АдресныеОбъектыУровень3.СвязиМуниципальныйРодительскийИдентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень4 КАК АдресныеОбъектыУровень4
		|		ПО (АдресныеОбъектыУровень3.Ключ = АдресныеОбъектыУровень4.Ключ)
		|			И (АдресныеОбъектыУровень3.Идентификатор = АдресныеОбъектыУровень4.МуниципальныйРодительскийИдентификатор
		|				ИЛИ АдресныеОбъектыУровень3.Идентификатор = АдресныеОбъектыУровень4.СвязиМуниципальныйРодительскийИдентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень5 КАК АдресныеОбъектыУровень5
		|		ПО (АдресныеОбъектыУровень4.Ключ = АдресныеОбъектыУровень5.Ключ)
		|			И (АдресныеОбъектыУровень4.Идентификатор = АдресныеОбъектыУровень5.МуниципальныйРодительскийИдентификатор
		|				ИЛИ АдресныеОбъектыУровень4.Идентификатор = АдресныеОбъектыУровень5.СвязиМуниципальныйРодительскийИдентификатор)
		|ГДЕ АдресныеОбъектыУровень0.ЭтоМуниципальныйАдрес = ИСТИНА
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 1000
		|	АдресныеОбъектыУровень0.Ключ,
		|	АдресныеОбъектыУровень0.Наименование,
		|	АдресныеОбъектыУровень0.Сокращение,
		|	АдресныеОбъектыУровень0.Идентификатор,
		|	АдресныеОбъектыУровень0.ДополнительныеСведения,
		|	АдресныеОбъектыУровень1.Наименование,
		|	АдресныеОбъектыУровень1.Сокращение,
		|	АдресныеОбъектыУровень1.Идентификатор,
		|	АдресныеОбъектыУровень1.ДополнительныеСведения,
		|	АдресныеОбъектыУровень2.Наименование,
		|	АдресныеОбъектыУровень2.Сокращение,
		|	АдресныеОбъектыУровень2.Идентификатор,
		|	АдресныеОбъектыУровень2.ДополнительныеСведения,
		|	АдресныеОбъектыУровень3.Наименование,
		|	АдресныеОбъектыУровень3.Сокращение,
		|	АдресныеОбъектыУровень3.Идентификатор,
		|	АдресныеОбъектыУровень3.ДополнительныеСведения,
		|	АдресныеОбъектыУровень4.Наименование,
		|	АдресныеОбъектыУровень4.Сокращение,
		|	АдресныеОбъектыУровень4.Идентификатор,
		|	АдресныеОбъектыУровень4.ДополнительныеСведения,
		|	АдресныеОбъектыУровень5.Наименование,
		|	АдресныеОбъектыУровень5.Сокращение,
		|	АдресныеОбъектыУровень5.Идентификатор,
		|	АдресныеОбъектыУровень5.ДополнительныеСведения,
		|	ЕСТЬNULL(АдресныеОбъектыУровень5.Идентификатор, ЕСТЬNULL(АдресныеОбъектыУровень4.Идентификатор, ЕСТЬNULL(АдресныеОбъектыУровень3.Идентификатор, ЕСТЬNULL(АдресныеОбъектыУровень2.Идентификатор, АдресныеОбъектыУровень1.Идентификатор))))
		|ИЗ
		|	АдресныеОбъектыУровень0 КАК АдресныеОбъектыУровень0
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень1 КАК АдресныеОбъектыУровень1
		|		ПО (АдресныеОбъектыУровень0.Ключ = АдресныеОбъектыУровень1.Ключ)
		|			И АдресныеОбъектыУровень0.Идентификатор = АдресныеОбъектыУровень1.РодительскийИдентификатор
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень2 КАК АдресныеОбъектыУровень2
		|		ПО (АдресныеОбъектыУровень1.Ключ = АдресныеОбъектыУровень2.Ключ)
		|			И АдресныеОбъектыУровень1.Идентификатор = АдресныеОбъектыУровень2.РодительскийИдентификатор
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень3 КАК АдресныеОбъектыУровень3
		|		ПО (АдресныеОбъектыУровень2.Ключ = АдресныеОбъектыУровень3.Ключ)
		|			И АдресныеОбъектыУровень2.Идентификатор = АдресныеОбъектыУровень3.РодительскийИдентификатор
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень4 КАК АдресныеОбъектыУровень4
		|		ПО (АдресныеОбъектыУровень3.Ключ = АдресныеОбъектыУровень4.Ключ)
		|			И АдресныеОбъектыУровень3.Идентификатор = АдресныеОбъектыУровень4.РодительскийИдентификатор
		|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъектыУровень5 КАК АдресныеОбъектыУровень5
		|		ПО (АдресныеОбъектыУровень4.Ключ = АдресныеОбъектыУровень5.Ключ)
		|			И АдресныеОбъектыУровень4.Идентификатор = АдресныеОбъектыУровень5.РодительскийИдентификатор
		|ГДЕ АдресныеОбъектыУровень0.ЭтоМуниципальныйАдрес = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	АдресныеОбъекты.Ключ КАК Ключ,
		|	АдресныеОбъекты.НаименованиеУровень0 КАК НаименованиеУровень0,
		|	АдресныеОбъекты.СокращениеУровень0 КАК СокращениеУровень0,
		|	АдресныеОбъекты.ИдентификаторУровень0 КАК ИдентификаторУровень0,
		|	АдресныеОбъекты.ДополнительныеСведения0 КАК ДополнительныеСведения0,
		|	АдресныеОбъекты.НаименованиеУровень1 КАК НаименованиеУровень1,
		|	АдресныеОбъекты.СокращениеУровень1 КАК СокращениеУровень1,
		|	АдресныеОбъекты.ИдентификаторУровень1 КАК ИдентификаторУровень1,
		|	АдресныеОбъекты.ДополнительныеСведения1 КАК ДополнительныеСведения1,
		|	АдресныеОбъекты.НаименованиеУровень2 КАК НаименованиеУровень2,
		|	АдресныеОбъекты.СокращениеУровень2 КАК СокращениеУровень2,
		|	АдресныеОбъекты.ИдентификаторУровень2 КАК ИдентификаторУровень2,
		|	АдресныеОбъекты.ДополнительныеСведения2 КАК ДополнительныеСведения2,
		|	АдресныеОбъекты.НаименованиеУровень3 КАК НаименованиеУровень3,
		|	АдресныеОбъекты.СокращениеУровень3 КАК СокращениеУровень3,
		|	АдресныеОбъекты.ИдентификаторУровень3 КАК ИдентификаторУровень3,
		|	АдресныеОбъекты.ДополнительныеСведения3 КАК ДополнительныеСведения3,
		|	АдресныеОбъекты.НаименованиеУровень4 КАК НаименованиеУровень4,
		|	АдресныеОбъекты.СокращениеУровень4 КАК СокращениеУровень4,
		|	АдресныеОбъекты.ИдентификаторУровень4 КАК ИдентификаторУровень4,
		|	АдресныеОбъекты.ДополнительныеСведения4 КАК ДополнительныеСведения4,
		|	АдресныеОбъекты.НаименованиеУровень5 КАК НаименованиеУровень5,
		|	АдресныеОбъекты.СокращениеУровень5 КАК СокращениеУровень5,
		|	АдресныеОбъекты.ИдентификаторУровень5 КАК ИдентификаторУровень5,
		|	АдресныеОбъекты.ДополнительныеСведения5 КАК ДополнительныеСведения5,
		|	АдресныеОбъекты.Идентификатор КАК Идентификатор
		|ИЗ
		|	АдресныеОбъекты КАК АдресныеОбъекты
		|;
		...
		|	ДомаЗданияСтроения.ДополнительныеАдресныеСведения КАК ДополнительныеАдресныеСведения,
		|	ДомаЗданияСтроения.Строения КАК Строения,
		|	ДомаАдресныхОбъектов.Идентификатор КАК Идентификатор,
		|	ДомаАдресныхОбъектов.Ключ КАК Ключ,
		|	ДополнительныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс
		|ИЗ
		|	АдресныеОбъекты КАК ДомаАдресныхОбъектов
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДомаЗданияСтроения КАК ДомаЗданияСтроения
		|		ПО (ДомаЗданияСтроения.АдресныйОбъект = ДомаАдресныхОбъектов.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеАдресныеСведения КАК ДополнительныеСведения
		|		ПО (ДополнительныеСведения.Идентификатор = ДомаЗданияСтроения.ДополнительныеАдресныеСведения)
		|ГДЕ
		|	НЕ ДомаЗданияСтроения.Строения ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	АдресныеОбъектыИсточник.Ключ КАК Ключ,
		|	АдресныеОбъекты.Идентификатор КАК Идентификатор,
		|	АдресныеОбъекты.КодСубъектаРФ КАК КодСубъектаРФ,
		|	АдресныеОбъекты.РодительскийИдентификатор КАК РодительскийИдентификатор,
		|	АдресныеОбъекты.МуниципальныйРодительскийИдентификатор КАК МуниципальныйРодительскийИдентификатор,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КодыАдресногоОбъекта.Ключ КАК Ключ,
		|	КодыАдресногоОбъекта.Идентификатор КАК Идентификатор,
		|	ДополнительныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс,
		|	ДополнительныеСведения.ОКТМО КАК ОКТМО
		|ИЗ
		|	АдресныйОбъект КАК КодыАдресногоОбъекта
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеАдресныеСведения КАК ДополнительныеСведения
		|		ПО (ДополнительныеСведения.Идентификатор = КодыАдресногоОбъекта.ДополнительныеАдресныеСведения)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1000
		|	ЗемельныеУчастки.АдресныйОбъект КАК АдресныйОбъект,
		|	ЗемельныеУчастки.КодСубъектаРФ КАК КодСубъектаРФ,
		|	ЗемельныеУчастки.ДополнительныеАдресныеСведения КАК ДополнительныеАдресныеСведения,
		...
		|		ПО (ЗемельныеУчастки.АдресныйОбъект = ДомаАдресныхОбъектов.Идентификатор)
		|ГДЕ
		|	НЕ ЗемельныеУчастки.Участки ЕСТЬ NULL";
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаУстановкиИдентификаторовАдресов()
...
Процедура АктуализироватьДанныеОРегионе(Знач Адрес, Знач КодСубъектаРФ, Знач Результат)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НовыйАдрес.areaType = ВыборкаДетальныеЗаписи.Сокращение;
	...
	Если ПустаяСтрока(НовоеНаименование) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Регион ""%1"" устарел'"),
			СтароеНаименование);
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Регион ""%1"" был переименован в ""%2""'"),
...
Процедура АнализАдресныхОбъектов(Результаты, Адреса, Знач СписокПроверяемыхУровней, ВариантыАдресов, ПроверятьУстареваниеАдресов)
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		НайденныеАдреса         = ТаблицаАдресов.НайтиСтроки(Новый Структура("Ключ", КлючЗначение.Ключ));
		КакиеУровниПроверять    = СписокПроверяемыхУровней[КлючЗначение.Ключ];
		СписокНекорректныхАдресов = Новый Массив;
		Адрес                   = КлючЗначение.Значение;
		
		ПроверитьКорректностьАдресов(СписокНекорректныхАдресов, КакиеУровниПроверять, Адрес, НайденныеАдреса, ОшибкиПроверки);
		
		Если СписокНекорректныхАдресов.Количество() = 0 Тогда
			
			Результат.АдресКорректный = Ложь;
			Результат.АдресПроверен = ?(ЭтоГородФедеральногоЗначения(ПолноеНаименованиеРегиона(Адрес.Area, Адрес.AreaType)), Ложь, Истина); // Для городов федерального значения выполняется дополнительная проверка.
...
Процедура ПроверитьКорректностьАдресов(СписокНекорректныхАдресов, Знач КакиеУровниПроверять, Адрес, Знач НайденныеАдреса, ОшибкиПроверки)
	...
	Для каждого АдресСтрока Из НайденныеАдреса Цикл
			...
			Если КакиеУровниПроверять["Distict"] <> Неопределено И АдресСтрока["Наименование" + КакиеУровниПроверять["Distict"]] = NULL Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Район ""%1"" отсутствует в адресном классификаторе'"), 
			...
			Если НЕ ДанныеУровняКорректные("Area", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Субъект РФ'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Субъект РФ ""%1"" отсутствует в адресном классификаторе'"),
			...
			Если НЕ ДанныеУровняКорректные("MunDistrict", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Муниципальный район'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Муниципальный район ""%1"" отсутствует в адресном классификаторе'"),
			...
			Если НЕ ДанныеУровняКорректные("Settlement", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Поселение'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поселение ""%1"" отсутствует в адресном классификаторе'"),
			...
			Если НЕ ДанныеУровняКорректные("City", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Город'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Город ""%1"" отсутствует в адресном классификаторе'"),
			...
			Если НЕ ДанныеУровняКорректные("Territory",  АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Территория'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Территория ""%1"" отсутствует в адресном классификаторе'"),
			...
			Если НЕ ДанныеУровняКорректные("Street",  АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Улица'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Улица ""%1"" отсутствует в адресном классификаторе'"),
...
Процедура АнализДомовИДополнительныхСведений(Результаты, Знач Адреса, Знач Дома, Знач Участки, Знач ДополнительныеСведения)
	
	ТаблицаДомов = Дома.Выгрузить();
	ТаблицаДополнительныеСведения = ДополнительныеСведения.Выгрузить();
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		ДомКорректный = Дома.Пустой(); // Домов нет, любой дом считаем корректным.
		ПочтовыйИндекс = 0;
		...
		НайденныеДома = ТаблицаДомов.НайтиСтроки(Новый Структура("Ключ", КлючЗначение.Ключ));
		...
		Если НЕ ДомКорректный Тогда
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 отсутствует в адресном классификаторе%2'"),
		...
		НайденныеДополнительныеСведения = ТаблицаДополнительныеСведения.НайтиСтроки(Новый Структура("Ключ", КлючЗначение.Ключ));
		
		// Проверка индекса 
		Если Не ЗначениеЗаполнено(ПочтовыйИндекс) И НайденныеДополнительныеСведения.Количество() > 0 Тогда
			ДополнительныеАдресныеСведения = НайденныеДополнительныеСведения[0]; // Нет индекса на уровне домов.
...
Процедура ПроверитьУчастки(Знач Участок, Знач ИндексАдреса, Знач ОшибкиПроверки, Знач Результат, Знач ТаблицаУчастков)
	...
	// Проверка участка
	НайденныеУчастки = ТаблицаУчастков.НайтиСтроки(Новый Структура("Ключ", ИндексАдреса));
	Для каждого ИнформацияОУчастках Из НайденныеУчастки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСоСпискомУчастков) Тогда
			...
			НаборУчастков = СтрРазделить(СтрокаСоСпискомУчастков, Символы.Таб, Ложь);
	...
	ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дом",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Земельный участок %1 отсутствует в адресном классификаторе'"),
		Участок));
...
Функция СведенияПоСпискуОКМТО(СписокОКТМО) Экспорт
	...
	Для каждого ЗаписьОКТМО Из СписокОКТМО Цикл
		...
		Если СтрНачинаетсяС(ЗаписьОКТМО.Значение, "118")
			Или СтрНачинаетсяС(ЗаписьОКТМО.Значение, "718")
...
Функция СведенияПоСпискуОКМТОЗагруженныеДанные(ТаблицаОКТМО, Результат)
	...
	|	АдресныеОбъекты.Уровень КАК Уровень0,
	|	АдресныеОбъекты.КодКЛАДР КАК КодКЛАДР,
	|	АдресныеОбъекты.МуниципальныйРодительскийИдентификатор КАК РодительскийИдентификаторСвязи
	|ПОМЕСТИТЬ АдресныеОбъекты0
	|ИЗ
	|	СписокОКТМО КАК СписокОКТМО
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеАдресныеСведения КАК ДополнительныеСведения
	...
	|	АдресныеОбъектыИтог.ИдентификаторУровень2 КАК ИдентификаторУровень2,
	|	АдресныеОбъектыИтог.Уровень2 КАК Уровень2,
	|	АдресныеОбъектыИтог.НаименованиеУровень1 КАК НаименованиеУровень1,
	|	АдресныеОбъектыИтог.СокращениеУровень1 КАК СокращениеУровень1,
	|	АдресныеОбъектыИтог.ИдентификаторУровень1 КАК ИдентификаторУровень1,
	|	АдресныеОбъектыИтог.Уровень1 КАК Уровень1,
	|	АдресныеОбъектыИтог.НаименованиеУровень0 КАК НаименованиеУровень0,
	|	АдресныеОбъектыИтог.СокращениеУровень0 КАК СокращениеУровень0,
	|	АдресныеОбъектыИтог.ИдентификаторУровень0 КАК ИдентификаторУровень0,
	|	АдресныеОбъектыИтог.Уровень0 КАК Уровень0,
	|	АдресныеОбъекты0.OKATO КАК OKATO,
	|	АдресныеОбъекты0.ОКТМО КАК ОКТМО,
	|	АдресныеОбъекты0.КодУчасткаИФНСЮЛ КАК КодУчасткаИФНСЮЛ,
	|	АдресныеОбъекты0.КодУчасткаИФНСФЛ КАК КодУчасткаИФНСФЛ,
	|	АдресныеОбъекты0.КодИФНСЮЛ КАК КодИФНСЮЛ,
	|	АдресныеОбъекты0.КодИФНСФЛ КАК КодИФНСФЛ,
	|	АдресныеОбъекты0.КодКЛАДР КАК КодКЛАДР,
	|	АдресныеОбъекты0.ПочтовыйИндекс КАК ПочтовыйИндекс,
	|	АдресныеОбъекты0.КодСубъектаРФ КАК КодРегиона
	|ИЗ
	|	АдресныеОбъекты4 КАК АдресныеОбъектыИтог
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ПО АдресныеОбъектыИтог.РодительскийИдентификаторСвязи = АдресныеОбъекты.Идентификатор
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиАдресныхОбъектов КАК СвязиАдресныхОбъектов
	|		ПО АдресныеОбъектыИтог.РодительскийИдентификаторСвязи = СвязиАдресныхОбъектов.Идентификатор
	|		ЛЕВОЕ СОЕДИНЕНИЕ АдресныеОбъекты0 КАК АдресныеОбъекты0
	|		ПО АдресныеОбъектыИтог.КлючЗаписиОКТМО = АдресныеОбъекты0.КлючЗаписиОКТМО";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для НомерСегмента = 0 По 5 Цикл
			...
			Если ЗначениеЗаполнено(Уровень) Тогда
				...
				СведенияПоОКТМО[ИмяПоля] = ВыборкаДетальныеЗаписи["НаименованиеУровень" + НомерСегментаСтрокой];
				СведенияПоОКТМО[ИмяПоля + "ТипКраткий"] = ВыборкаДетальныеЗаписи["СокращениеУровень" + НомерСегментаСтрокой];
				СведенияПоОКТМО[ИмяПоля + "Идентификатор"] = ВыборкаДетальныеЗаписи["ИдентификаторУровень" + НомерСегментаСтрокой];
				ПолныеНаименованияСокращений = СписокСокращений.Получить(Строка(Уровень) + СведенияПоОКТМО[ИмяПоля + "ТипКраткий"]);
		...
		СведенияПоОКТМО.КодМуниципальногоРайона = КодМуниципальногоРайона(СведенияПоОКТМО["МуниципальныйРайонТипКраткий"],
			СведенияПоОКТМО.ОКТМО, ЭтоГородФедеральногоЗначения(НаименованиеРегиона));
...
Процедура СведенияПоСпискуОКМТОВебСервис(СписокОКТМО, Результат)
	...
	Для каждого ЗаписьОКТМО Из СписокОКТМО Цикл
		...
		Для каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
			...
			СведенияПоОКТМО.OKATO = СокрЛП(СтрокаДанных.okato);
			СведенияПоОКТМО.ОКТМО = СокрЛП(СтрокаДанных.oktmo);
			СведенияПоОКТМО.КодИФНСФЛ = СокрЛП(СтрокаДанных.ifnsFlCode);
			СведенияПоОКТМО.КодИФНСЮЛ = СокрЛП(СтрокаДанных.ifnsUlCode);
			СведенияПоОКТМО.КодКЛАДР  = СокрЛП(СтрокаДанных.codeKLADR);
			СведенияПоОКТМО.КодУчасткаИФНСФЛ = СокрЛП(СтрокаДанных.ifnsFlAreaCode);
			СведенияПоОКТМО.КодУчасткаИФНСЮЛ = СокрЛП(СтрокаДанных.ifnsUlAreaCode);
			СведенияПоОКТМО.ПочтовыйИндекс   = СокрЛП(СтрокаДанных.ZIPcode);
			
			СведенияПоОКТМО.Регион = СокрЛП(СтрокаДанных.area);
			СведенияПоОКТМО.РегионИдентификатор = ?(ПустаяСтрока(СтрокаДанных.areaId),
				Неопределено, Новый УникальныйИдентификатор(СокрЛП(СтрокаДанных.areaId)));
			СведенияПоОКТМО.РегионТипКраткий = СокрЛП(СтрокаДанных.areaType);
			СведенияПоОКТМО.РегионТипПолный = СокрЛП(СписокСокращений.Получить("1" + СведенияПоОКТМО.РегионТипКраткий));
			НаименованиеРегиона = ПолноеНаименованиеРегиона(СведенияПоОКТМО.Регион, СведенияПоОКТМО.РегионТипКраткий);
			СведенияПоОКТМО.КодРегиона = АдресныйКлассификатор.КодРегионаПоНаименованию(НаименованиеРегиона);
			
			СведенияПоОКТМО.МуниципальныйРайон = СокрЛП(СтрокаДанных.munDistrict);
			СведенияПоОКТМО.МуниципальныйРайонИдентификатор = ?(ПустаяСтрока(СтрокаДанных.munDistrictId),
				Неопределено, Новый УникальныйИдентификатор(СокрЛП(СтрокаДанных.munDistrictId)));
			СведенияПоОКТМО.МуниципальныйРайонТипКраткий = СокрЛП(СтрокаДанных.munDistrictType);
			СведенияПоОКТМО.МуниципальныйРайонТипПолный = СокрЛП(СписокСокращений.Получить("31" + СведенияПоОКТМО.МуниципальныйРайонТипКраткий));
			СведенияПоОКТМО.КодМуниципальногоРайона = КодМуниципальногоРайона(СведенияПоОКТМО.МуниципальныйРайонТипКраткий,
				СведенияПоОКТМО.ОКТМО, ЭтоГородФедеральногоЗначения(НаименованиеРегиона));
				
			СведенияПоОКТМО.НаселенныйПункт = СокрЛП(СтрокаДанных.locality);
			СведенияПоОКТМО.НаселенныйПунктИдентификатор = ?(ПустаяСтрока(СтрокаДанных.localityId),
				Неопределено, Новый УникальныйИдентификатор(СокрЛП(СтрокаДанных.localityId)));
			СведенияПоОКТМО.НаселенныйПунктТипКраткий = СокрЛП(СтрокаДанных.localityType);
			СведенияПоОКТМО.НаселенныйПунктТипПолный = СокрЛП(СписокСокращений.Получить("6" + СведенияПоОКТМО.НаселенныйПунктТипКраткий));
			
			СведенияПоОКТМО.Поселение = СокрЛП(СтрокаДанных.settlement);
			СведенияПоОКТМО.ПоселениеИдентификатор = ?(ПустаяСтрока(СтрокаДанных.settlementId),
				Неопределено, Новый УникальныйИдентификатор(СокрЛП(СтрокаДанных.settlementId)));
			СведенияПоОКТМО.ПоселениеТипКраткий = СокрЛП(СтрокаДанных.settlementType);
			СведенияПоОКТМО.ПоселениеТипПолный = СокрЛП(СписокСокращений.Получить("41" + СведенияПоОКТМО.ПоселениеТипКраткий));
			СведенияПоОКТМО.КодПоселения = КодПоселения(СведенияПоОКТМО.ПоселениеТипКраткий);
			
			СведенияПоОКТМО.Территория = СокрЛП(СтрокаДанных.territory);
			СведенияПоОКТМО.ТерриторияИдентификатор = ?(ПустаяСтрока(СтрокаДанных.territoryId),
				Неопределено, Новый УникальныйИдентификатор(СокрЛП(СтрокаДанных.territoryId)));
			СведенияПоОКТМО.ТерриторияТипКраткий = СокрЛП(СтрокаДанных.territoryType);
			СведенияПоОКТМО.ТерриторияТипПолный = СокрЛП(СписокСокращений.Получить("65" + СведенияПоОКТМО.ТерриторияТипКраткий));
...
Функция ИменаУровнейАдресаПоКодуУровня()
	...
	Уровни.Вставить(31, "МуниципальныйРайон");
	Уровни.Вставить(41, "Поселение");
	Уровни.Вставить(3, "МуниципальныйРайон");
	Уровни.Вставить(4, "НаселенныйПункт");
	Уровни.Вставить(5, "Территория");
	Уровни.Вставить(6, "НаселенныйПункт");
	Уровни.Вставить(65, "Территория");
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(ВариантыАдреса, КешПредставлений, Текст, Родитель, Количество, 
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ТипАдреса) Тогда
		...
		ДобавлятьАдминистративноТерриториальныеАдреса = ЭтоАдминистративноТерриториальныйАдрес(ДополнительныеПараметры.ТипАдреса);
	КонецЕсли;
	...
	Для каждого Данные Из РезультатВебСервис.Данные Цикл
		...
		Если ДобавлятьМуниципальныеАдреса
			И ЗначениеЗаполнено(Данные.munId)
			И ЗначениеЗаполнено(ПредставлениеМуниципальное) Тогда
		...
		Если ДобавлятьАдминистративноТерриториальныеАдреса
			И ЗначениеЗаполнено(ПредставлениеТерриториальное)
			И (Не ЗначениеЗаполнено(Данные.munId)
...
Функция ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Знач АдресаДляПроверки, ЭтоУстановкаИдентификаторов = Истина)
	...
	Для каждого КлючЗначение Из АдресаДляПроверки Цикл
		...
		АдресДляПроверки.Вставить("typeToVerify", ?(ЭтоМуниципальныйАдрес(Адрес.addressType), "MUNICIPAL", "ADMINISTRATIVE"));
	...
	Для Каждого Проверка Из ДанныеПроверки Цикл
		...
		Если ЭтоУстановкаИдентификаторов Или РезультатПроверки.АдресУстарел Тогда
			Если ЭлементПроверки.Свойство("value") И ЗначениеЗаполнено(ЭлементПроверки.value) Тогда
				Вариант = ОбщегоНазначения.СкопироватьРекурсивно(ЭлементПроверки.value);
				ШаблонКонтактнойИнформации = ОписаниеНовойКонтактнойИнформации();
				Для каждого СвойствоСтруктуры Из Вариант Цикл
				     Если ШаблонКонтактнойИнформации.Свойство(СвойствоСтруктуры.Ключ) И СвойствоСтруктуры.Значение = Неопределено Тогда
...
Процедура ОбработатьОбщепринятыеСокращения(ДоступныеУровни)
	...
	ОбщепринятыеСокращения.Вставить("ОФ.", "Офис");
	ОбщепринятыеСокращения.Вставить("ПОМЕЩ", "Квартира");
	ОбщепринятыеСокращения.Вставить("ПОМЕЩ.", "Квартира");
	...
	Для каждого НераспознаннаяЧастьАдреса Из НераспознанныеСтроки Цикл
		Если (ВРЕГ(НераспознаннаяЧастьАдреса.Наименование) = "МОСКВА" 
			...
			И ПустаяСтрока(НераспознаннаяЧастьАдреса.Сокращение) Тогда
				...
				НераспознаннаяЧастьАдреса.Значение = СоединитьНаименованиеИТипАдресногоОбъекта(НераспознаннаяЧастьАдреса.Наименование,
					НераспознаннаяЧастьАдреса.Сокращение, Истина);
		Иначе
			
			Сокращение = ОбщепринятыеСокращения[ВРЕГ(НераспознаннаяЧастьАдреса.Сокращение)];
			Если Сокращение <> Неопределено Тогда
				НераспознаннаяЧастьАдреса.Сокращение = Сокращение;
			КонецЕсли;
...
//
Функция НаименованиеИСокращение(Название, ЭтоРегион = Ложь) Экспорт
		
	Результат = НовыйНаименованиеИСокращение();
	...
	Если СтрЗаканчиваетсяНа(ТекстВерхнийРегистр, "ТЕР. СНТ")
		...
		Результат.Сокращение = Прав(Название, 8);
...
//   * Сокращение   - Строка
//
Функция НовыйНаименованиеИСокращение()
	
	Результат = Новый Структура;
	Результат.Вставить("Наименование", "");
	Результат.Вставить("Сокращение",   "");
	Возврат Результат;
...
Функция ИменаУровнейАдреса(ТипАдреса, ВключатьУлицу, ИсключатьГород = Ложь)
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Дом) Тогда
		...
		Иначе
			ТекстПроДом = СтроковыеФункции.ФорматированнаяСтрока("%1 <span style='color:%2; font:%3'>%4</span>%5",
				ОписаниеДома.НазваниеВладения, "РезультатУспехЦвет", "ВажнаяНадписьШрифт", СтрокаПоиска, Сред(ОписаниеДома.Дом, СтрДлина(СтрокаПоиска) + 1));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		...
		Иначе
			ТекстПроСтроения = СтроковыеФункции.ФорматированнаяСтрока("корпус <span style='color:%1; font:%2'>%3</span>%4",
				"РезультатУспехЦвет", "ВажнаяНадписьШрифт", СтрокаПоиска, Сред(ОписаниеДома.Корпус, СтрДлина(СтрокаПоиска) + 1));
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Строение) Тогда
		...
		Если  ЗначениеЗаполнено(СтрокаПоиска) И СтрНачинаетсяС(ОписаниеДома.Строение, СтрокаПоиска) Тогда
			НайденныйФрагментСтроения = СтроковыеФункции.ФорматированнаяСтрока("%1 <span style='color:%2; font:%3'>%4</span>%5",
				Разделитель + НазваниеСтроения, "РезультатУспехЦвет", "ВажнаяНадписьШрифт", СтрокаПоиска, Сред(ОписаниеДома.Строение, СтрДлина(СтрокаПоиска) + 1));
		Иначе
...
// Обратная совместимость
//
Функция ИсправлениеСокращений(Сокращение)
	
	// АПК: 1297-выкл Данные адресного классификатора, не локализуются
	Если СтрСравнить(Сокращение, "г. п.") = 0 Тогда
		Возврат "г.п.";
	КонецЕсли;
	
	Если СтрСравнить(Сокращение, "вн.тер") = 0 
		Или СтрСравнить(Сокращение, "вн.тер.") = 0  Тогда
		Возврат "вн.тер.г.";
	КонецЕсли;
	
	Если СтрСравнить(Сокращение, "мун.окр.") = 0
		Или СтрСравнить(Сокращение, "мун. окр.") = 0 Тогда
		Возврат "м.о.";
	КонецЕсли;
	// АПК: 1297-вкл
...
Процедура ЗакрытьФайлЗагрузкиСубъектаРФ(ОписаниеФайла)
	...
	Если ОписаниеФайла.УдалитьПутьПриЗакрытии Тогда
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,,, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удаление файла региона %1'"), ОписаниеФайла.Путь));
...
Функция КонтактнаяИнформацияВФорматеКлючЗначениеВJSON(Адрес)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Переданный адрес не может быть преобразован в функции %1.'"), "АдресныйКлассификаторСлужебный.КодыАдреса");
...
Процедура ЗагрузитьДанныеКлассификатораАдресовИзФайла(СубъектРФ, ОписаниеФайлов, ПараметрыЗагрузки) Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,,, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка региона %1 из %2'"), СубъектРФ, КаталогФайлов));
	...
	Попытка
		...
		ДлительныеОперации.СообщитьПрогресс(0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загружается %1'"), НаименованиеРегиона));
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
					...
					|по причине: %3'"), СубъектРФ, НаименованиеРегиона, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
				ВызватьИсключение;
			КонецПопытки;
...
Процедура ЗагрузитьКлассификаторАдресовПоставляемыеДанные(КодыСубъектовРФ, ОписаниеФайлов, ПараметрыЗагрузки)
	...
	Иначе
		...
		Если ТипЗнч(КодыСубъектовРФ) = Тип("Массив") И КодыСубъектовРФ.Количество() > 0 Тогда
			КаталогОбщихАдресныхСведений = КаталогФайлов + Формат(КодыСубъектовРФ[0], "ЧЦ=2; ЧВН=") + ПолучитьРазделительПутиСервера();
		Иначе
			Возврат;
		КонецЕсли;
	КонецЕсли; 
	...
	Попытка
		...
		Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
			...
			Попытка
				...
				КаталогФайловРегиона = КаталогФайлов + Формат(СубъектРФ, "ЧЦ=2; ЧВН=") + ПолучитьРазделительПутиСервера();
...
Процедура ЗагрузитьАдресныеОбъекты(КодСубъектаРФ, КаталогФайлов, ЗагружатьОднимНабором)
	...
	Связи = СвязиАдресныхОбъектов(КодСубъектаРФ, КаталогФайлов, ЗагружатьОднимНабором);
	...
	Попытка
		...
		Пока ТекущаяЗапись.Прочитать() Цикл
			...
			ПреобразоватьЗаписьДляКраткойЗаписиАдресов(Запись, Связи);
...
Функция СвязиАдресныхОбъектов(КодСубъектаРФ, КаталогФайлов, ЗагружатьОднимНабором)
	
	ТаблицаСвязейАдресныхОбъектов = Новый ТаблицаЗначений;
	ТаблицаСвязейАдресныхОбъектов.Колонки.Добавить("МуниципальныйРодительскийИдентификатор", Новый ОписаниеТипов("УникальныйИдентификатор"));
	ТаблицаСвязейАдресныхОбъектов.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("УникальныйИдентификатор"));
	
	ТаблицаСвязейАдресныхОбъектов.Индексы.Добавить("МуниципальныйРодительскийИдентификатор");
	ТаблицаСвязейАдресныхОбъектов.Индексы.Добавить("Идентификатор");
	
	ФайлЗагрузки = ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, "LINK", "Links");
	
	Если ФайлЗагрузки = Неопределено Тогда
		Возврат ТаблицаСвязейАдресныхОбъектов;
	КонецЕсли;
	...
	Попытка
		...
		Пока ТекущаяЗапись.Прочитать() Цикл
			...
			СвязьАдресныхОбъектов = ТаблицаСвязейАдресныхОбъектов.Добавить();
			
			СвязьАдресныхОбъектов.Идентификатор = УникальныйИдентификаторИзСтроки64(ТекущаяЗапись.ЗначениеАтрибута("AOGUID"));
			СвязьАдресныхОбъектов.МуниципальныйРодительскийИдентификатор = УникальныйИдентификаторИзСтроки64(ТекущаяЗапись.ЗначениеАтрибута("MUNPARENTGUID"));
		КонецЦикла;
...
Процедура УдалитьФайлыРегиона(КаталогФайлов)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,,, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удаление файла региона: %1'"), КаталогФайлов));
...
Процедура ПреобразоватьЗаписьДляКраткойЗаписиАдресов(Запись, Связи)
	
	Если Запись.Уровень = 31 Тогда
		Запись.Сокращение =ИсправлениеСокращений(Запись.Сокращение);
		Запись.Наименование = КраткоеНаписаниеМуниципальныхРайонов(Запись.Наименование, Запись.Сокращение);
	КонецЕсли;
	
	Если Запись.Уровень = 41 Тогда
		Запись.Сокращение =ИсправлениеСокращений(Запись.Сокращение);
		Запись.Наименование = КраткоеНаписаниеПоселений(Запись.Наименование, Запись.Сокращение);
	КонецЕсли;
	
	// Городской округ - город - населенный пункт
	Связь = Связи.Найти(Запись.Идентификатор, "Идентификатор");
	Если Связь <> Неопределено Тогда
		Запись.МуниципальныйРодительскийИдентификатор = Связь.МуниципальныйРодительскийИдентификатор;
	КонецЕсли;
...
Функция УровеньМуниципальногоРодительскогоОбъектаПоАдресу(Адрес)
	
	УровеньРодительскогоОбъекта = 0;
	
	ИдентификаторОбъекта = Неопределено;
	Если ЗначениеЗаполнено(Адрес.localityId) Тогда
		ИдентификаторОбъекта = Адрес.localityId;
	ИначеЕсли ЗначениеЗаполнено(Адрес.territoryId) Тогда
		ИдентификаторОбъекта = Адрес.territoryId;
	КонецЕсли;
	
	Если ИдентификаторОбъекта <> Неопределено Тогда
		УровеньРодительскогоОбъекта  = УровеньМуниципальногоРодительскогоОбъекта(ИдентификаторОбъекта);
	КонецЕсли;
	
	Возврат УровеньРодительскогоОбъекта;
	
КонецФункции

Функция УровеньМуниципальногоРодительскогоОбъекта(Идентификатор)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	РодительскийАдресныйОбъект.Уровень КАК УровеньРодительскогоОбъекта
	|ИЗ
	|	РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК РодительскийАдресныйОбъект
	|		ПО АдресныйОбъект.МуниципальныйРодительскийИдентификатор = РодительскийАдресныйОбъект.Идентификатор
	|ГДЕ
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			Возврат ВыборкаДетальныеЗаписи.УровеньРодительскогоОбъекта;
		КонецЕсли;
