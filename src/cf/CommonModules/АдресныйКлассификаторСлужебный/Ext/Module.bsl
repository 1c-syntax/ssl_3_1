...
Функция РаспознатьАдресЗагруженныеДанные(ЧастиАдреса, КодРегиона)
	...
	ОбработатьОбщепринятыеСокращенияИУдалитьТочкуВКонецТипа(ЧастиАдреса);
	Адрес = ОписаниеНовойКонтактнойИнформации();
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ПО (ДанныеАдреса.Наименование = АдресныеОбъекты.Наименование)
	|			И (ДанныеАдреса.ТипОбъекта = АдресныеОбъекты.ТипОбъекта ИЛИ ДанныеАдреса.ТипОбъекта + ""."" = АдресныеОбъекты.ТипОбъекта )
	|ГДЕ
	...
	МаксимальноеКоличествоЧастейАдреса = 5;
	КоличествоЧастейАдреса = ?(СопоставленныеЧастиАдреса.Количество() > МаксимальноеКоличествоЧастейАдреса, 
		МаксимальноеКоличествоЧастейАдреса, СопоставленныеЧастиАдреса.Количество() - 1);
	Позиция                            = 0;
	
	Пока Позиция <= КоличествоЧастейАдреса Цикл
...
Функция ПодборУлицЗагруженныеДанные(ПараметрыАвтоподбора, ТекстПоиска = Неопределено)
	...
	Если ПустаяСтрока(ТекстПоиска) Тогда
		ТекстЗапроса = ТекстЗапросаПодбораВсехУлиц();
	Иначе
		ТекстЗапроса = ТекстЗапросаПодбораУлицМуниципальнаяИерархия();
	КонецЕсли;
	
	Если ПараметрыАвтоподбора.ТипАдреса = АдминистративноТерриториальныйАдрес() Тогда
...
Процедура ОбработатьОбщепринятыеСокращенияИУдалитьТочкуВКонецТипа(АдресПоЧастям)
