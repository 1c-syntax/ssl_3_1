...
Процедура ОбработатьОбщепринятыеСокращенияИУдалитьТочкуВКонецТипа(АдресПоЧастям)
	...
	ОбщепринятыеСокращения = ОбщепринятыеСокращенияЗданийИПомещений();
...
Функция ОбщепринятыеСокращенияЗданийИПомещений()
...
Функция СервисКлассификатора1С(ВремяОжидания = 20) Экспорт
	...
	URLПорталаПоддержки = "https://portal.1c.ru/";
	СохраненныйТекущийURLПорталаПоддержки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("АдресныйКлассификатор", "URLПорталаПоддержки");
	Если ТипЗнч(СохраненныйТекущийURLПорталаПоддержки) = Тип("Строка") И ЗначениеЗаполнено(СохраненныйТекущийURLПорталаПоддержки) Тогда
		URLПорталаПоддержки = СохраненныйТекущийURLПорталаПоддержки;
	КонецЕсли;
