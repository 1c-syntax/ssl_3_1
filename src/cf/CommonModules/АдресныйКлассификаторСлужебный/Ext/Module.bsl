...
// Возвращаемое значение:
//     Структура:
//       * Отказ                        - Булево - поставщик недоступен.
...
Функция ВерсияПоставщикаДанных() Экспорт
	...
	Если Не ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки() Тогда
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			КраткоеПредставлениеОшибки = НСтр("ru = 'Сервис временно недоступен. Отсутствует подключение к интернет-поддержке пользователей.'");
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,,,
				КраткоеПредставлениеОшибки);
		Иначе
			КраткоеПредставлениеОшибки = 
				НСтр("ru = 'Для автоподбора и проверки адресов необходимо подключиться к интернет-поддержке пользователей или загрузить адресные сведения.'");
		КонецЕсли;
		Результат.КраткоеПредставлениеОшибки   = КраткоеПредставлениеОшибки;
		Результат.ПодробноеПредставлениеОшибки = КраткоеПредставлениеОшибки;
...
//     ДополнительныеПараметры - Структура - см. ПараметрыПоискаАдресовПоПочтовомуИндексу
//
// Возвращаемое значение:
...
//
Функция АдресаПоПочтовомуИндексу(Индекс, ДополнительныеПараметры) Экспорт

	Результат = Новый Структура;
	Результат.Вставить("Данные", ТаблицаДанныхДляВыбораПоПочтовомуИндексу());
	Результат.Вставить("ОбщаяЧастьПредставления", "");
	Результат.Вставить("ЭтоДанныеСВебСервиса", Ложь);
	
	СтруктураОписанияОшибкиПоставщика(Результат);

	Если Не ДополнительныеПараметры.ПоискТолькоВВебСервисе Тогда
	...
	Если Результат.Данные.Количество() = 0 И Не Результат.Отказ Тогда
		...
		Попытка
			ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораСервис1С(Результат, Индекс, ДополнительныеПараметры.ПодстрокаПоиска);
			Результат.ЭтоДанныеСВебСервиса = Истина;
		Исключение
			СтруктураОписанияОшибкиПоставщика(Результат, ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации( СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				Результат.ПодробноеПредставлениеОшибки);
...
// Возвращает настройки по умолчанию для поиска адресов по почтовому индексу классификатора.
//
// Возвращаемое значение:
//   Структура - описание настроек поиска. Содержит поля:
//     * ТипАдреса              - Строка - тип используемого классификатора.
//     * ПодстрокаПоиска        - Строка - для уточнения поиска адресов.
//     * ПоискТолькоВВебСервисе - Булево - если Истина, то поиск будет происходить только по данным из веб сервиса.
//
Функция ПараметрыПоискаАдресовПоПочтовомуИндексу() Экспорт
	
	Настройки = Новый Структура;
	Настройки.Вставить("ТипАдреса", "");
	Настройки.Вставить("ПодстрокаПоиска", "");
	Настройки.Вставить("ПоискТолькоВВебСервисе", Ложь);
...
Процедура УстановитьИдентификаторыАдресов(Адреса, Источник = "") Экспорт
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		КодРегиона = АдресныйКлассификатор.КодРегионаПоНаименованию(Адрес.Area);
...
Процедура УстановитьМуниципальныеСведения(Адрес, ОКТМО, ИдентификаторАдреса = Неопределено, ЗагруженныеДанные = Истина) Экспорт
	...
	ПолученныеСведенияУровней = Новый Соответствие();
	
	Для Каждого ЗаписьУровня Из РезультатЗапроса.Выгрузить() Цикл
		
		ИмяУровня = СловарьИменПолей[ЗаписьУровня.Уровень];
		Если ПолученныеСведенияУровней.Получить(ИмяУровня) = Неопределено Тогда
			СведенияАдреса = Новый Структура;
			СведенияАдреса.Вставить("Наименование", ЗаписьУровня.Наименование);
			СведенияАдреса.Вставить("ТипОбъекта", ЗаписьУровня.ТипОбъекта);
			СведенияАдреса.Вставить("Идентификатор", ЗаписьУровня.Идентификатор);
			ПолученныеСведенияУровней.Вставить(ИмяУровня, СведенияАдреса);
		КонецЕсли;
		
		Если СтрСравнить(Адрес[ИмяУровня], ЗаписьУровня.Наименование) = 0 
		 И  СтрСравнить(Адрес[ИмяУровня + "Type"] , ЗаписьУровня.ТипОбъекта) = 0 Тогда
		 	ПолученныеСведенияУровней[ИмяУровня]["Наименование"] = "";
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого СведенияУровня Из ПолученныеСведенияУровней Цикл
		Если ЗначениеЗаполнено(СведенияУровня.Значение.Наименование) Тогда
			ИмяУровня = СведенияУровня.Ключ;
			Адрес[ИмяУровня]          = СведенияУровня.Значение.Наименование;
			Адрес[ИмяУровня + "Type"] = СведенияУровня.Значение.ТипОбъекта;
			Адрес[ИмяУровня + "Id"]   = СведенияУровня.Значение.Идентификатор;
		КонецЕсли;
	КонецЦикла;
...
Функция РаспознатьАдрес(ЧастиАдреса, Представление, ВСтруктуру = Ложь) Экспорт
	...
	Если ЗагруженныеАдресныеСведения.Получить("ИспользоватьЗагруженные") = Истина 
	   ...
	   И ЗагруженныеАдресныеСведения.Получить(КодРегиона).ИспользоватьЗагруженные Тогда
		...
		Адрес = РаспознатьАдресЗагруженныеДанные(ЧастиАдреса, КодРегиона, Представление);
		Если Адрес <> Неопределено И СтрСравнить(Адрес.addressType, АдресВСвободнойФорме()) <> 0 Тогда
			Возврат Адрес;
		КонецЕсли;
...
// Возвращает список помещений по идентификатору адресного объекта, поиск по подобию.
//
// Параметры:
//     ИдентификаторАдресногоОбъекта - УникальныйИдентификатор - родительский объект.
//     СтрокаПоиска                  - Строка                  - текст, отбора в списке домов.
//     ПорцияПриПоиске               - Число - количество получаемых вариантов за один запрос.
//
// Возвращаемое значение:
//   см. КонструкторСпискаПомещений
//
Функция СписокПомещений(ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска, ПорцияПриПоиске = 20) Экспорт
	
	ВариантыПомещений = Неопределено;
	...
	ЗагруженныеАдресныеСведения = АдресныйКлассификаторПовтИсп.СведенияОЗагрузкеСубъектовРФ();
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] Тогда
		ВариантыПомещений = СписокПомещенийИзЗагруженныхСведений(ИдентификаторАдресногоОбъекта, ТекстПоиска, ТочноеСовпадение);
	КонецЕсли;
	
	Если ТипЗнч(ВариантыПомещений) <> Тип("ТаблицаЗначений") Тогда
		ВариантыПомещений = КонструкторСпискаПомещений();
	ИначеЕсли ВариантыПомещений.Количество() > 0 Тогда
		ВариантыПомещений.Сортировать("ТипПомещения, ПомещениеЧислом");
		Возврат ВариантыПомещений;
	КонецЕсли;
	...
	ТекстGETЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("selectApartment?parentId=%1&limit=%2",
		Строка(ИдентификаторАдресногоОбъекта), Формат(ПорцияПриПоиске, "ЧГ=0"));
	...
	Для Каждого СведенияОПомещении Из РезультатВебСервис.Данные Цикл
		
		ОписаниеПомещения = ОписаниеПомещения();
		
		ОписаниеПомещения.НомерПомещения         = СведенияОПомещении.number;
		ОписаниеПомещения.ТипПомещения           = СведенияОПомещении.type;
		ОписаниеПомещения.ПомещениеЧислом        = ТипЧисло.ПривестиЗначение(СведенияОПомещении.number);
		ОписаниеПомещения.ИдентификаторПомещения = СведенияОПомещении.id;
		ОписаниеПомещения.Представление          = СведенияОПомещении.value;

		ИнформацияОПомещении = ВариантыПомещений.Добавить();
		ЗаполнитьЗначенияСвойств(ИнформацияОПомещении, ОписаниеПомещения);
		ИнформацияОПомещении.Значение = ОписаниеПомещения;
	КонецЦикла;

	Если ВариантыПомещений.Количество() > 0 Тогда
		ВариантыПомещений.Сортировать("ТипПомещения, ПомещениеЧислом");
	КонецЕсли;
...
//    * ЗагруженныеДанные - Булево - признак, что источник адресных сведений загруженные в приложение данные.
...
Функция РезультатПроверкиАдресовПоКлассификатору(Адреса) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДляСортировки Цикл
		...
		РезультатПроверки = РезультатЗагруженныеДанные[СтрокаТаблицы.ПорядковыйНомер];
		Если РезультатПроверки = Неопределено Тогда
			РезультатПроверки = РезультатВебСервис[СтрокаТаблицы.ПорядковыйНомер];
			Если РезультатПроверки = Неопределено Тогда
				РезультатПроверки = Новый Структура;
				РезультатПроверки.Вставить("id", СтрокаТаблицы.ПорядковыйНомер);
...
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Представление = "") Экспорт
	...
	ИначеЕсли ТипЗнч(АдресРФ) = Тип("Строка") Тогда
		...
		Если СтрЧислоВхождений(АдресРФ, ",") = 9 Тогда
			...
			Результат.areaValue = ОфициальноеНаименованиеРегиона(Результат.Area);
	...
	Иначе
		...
		Если ЗначениеГородаОтличаетсяОтНаселенногоПункта(АдресРФ.Город, АдресРФ.НаселПункт) Тогда
...
Функция ПредставлениеАдресногоЭлемента(Знач Наименование, Знач ТипАдресногоЭлемента, ИмяУровня = "", ВариантНаписанияТипа = "") Экспорт
	
	Если ПустаяСтрока(ТипАдресногоЭлемента) Тогда
		Возврат Наименование;
	КонецЕсли;
	
	Если СтрСравнить(ИмяУровня, "Регион") = 0 Или ПустаяСтрока(ИмяУровня) Тогда
		
		ДанныеОРегионе = АдресныйКлассификаторПовтИсп.ДанныеОРегионе(Наименование); 
		Если ДанныеОРегионе <> Неопределено Тогда
			Возврат ДанныеОРегионе.ОфициальноеНаименование;
		КонецЕсли;
		
	КонецЕсли;
	
	Результат = ТипСогласноВариантуНаписания(ТипАдресногоЭлемента, ИмяУровня, ВариантНаписанияТипа);
	
	Возврат СоединитьНаименованиеИТипАдресногоОбъекта(Наименование, НРег(Результат.НаименованиеТипа), Результат.ТипВКонец);
...
Функция АдресныеСокращенияПоПриказуМинфинаРФ171н() Экспорт

	Возврат АдресныйКлассификаторПовтИсп.АдресныеСокращенияПоПриказуМинфинаРФ171н();

КонецФункции

Функция ТипСогласноВариантуНаписания(Знач ТипАдресногоЭлемента, ИмяУровня, Знач ВариантНаписанияТипа) Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("ТипВКонец",        Ложь);
	Результат.Вставить("НаименованиеТипа", "");
	
	КодУровня = ?(ЗначениеЗаполнено(ИмяУровня), КодУровняПоИмениУровня()[ВРег(ИмяУровня)], Неопределено);
	
	НаименованиеТипа = ТипАдресногоЭлемента;
	Если ВариантНаписанияТипа = "Сокращенный" Тогда
		Вариант = АдресноеСокращение(НаименованиеТипа, "Наименование", КодУровня);
		
		Если Вариант <> Неопределено Тогда
			НаименованиеТипа = Вариант.Сокращение;
		Иначе
			Вариант = АдресноеСокращениеПоВсемПолям(НаименованиеТипа, КодУровня);
		КонецЕсли;
		
		Если Вариант <> Неопределено И Вариант.ТипВКонце = 1 Тогда
			Результат.ТипВКонец = Истина;
		КонецЕсли;
	ИначеЕсли ВариантНаписанияТипа = "СокращенныйПоПриказуМинфина171н"  Тогда
		Вариант = АдресноеСокращение(НаименованиеТипа, "Наименование", КодУровня);
		
		Если Вариант <> Неопределено Тогда
			НаименованиеТипа = Вариант.СокращениеПриказаМинфинаРФ171н;
		Иначе
			
			Вариант = АдресноеСокращение(НаименованиеТипа, "Сокращение", КодУровня);
			Если Вариант <> Неопределено Тогда
				НаименованиеТипа = Вариант.СокращениеПриказаМинфинаРФ171н;
			Иначе
				Вариант = АдресноеСокращение(НаименованиеТипа, "СокращениеПриказаМинфинаРФ171н", КодУровня);
			КонецЕсли;
	...
	ИначеЕсли ВариантНаписанияТипа = "Полный" Тогда
		
		Вариант = АдресноеСокращение(НаименованиеТипа, "СокращениеПриказаМинфинаРФ171н", КодУровня);
		Если Вариант <> Неопределено Тогда
			НаименованиеТипа = Вариант.Наименование;
		Иначе
	...
	Результат.НаименованиеТипа = НаименованиеТипа;
...
Функция КодМуниципальногоРайона(Знач ТипМуниципальногоРайона, ОКТМО, ЭтоГородФедеральногоЗначения) Экспорт
	
	Если ЗначениеЗаполнено(ТипМуниципальногоРайона) Тогда
		
		ТипМуниципальногоРайонаОбработанный = СтрЗаменить(ВРег(ТипМуниципальногоРайона), " ", "");
		ТипМуниципальногоРайонаОбработанный = СтрЗаменить(ТипМуниципальногоРайонаОбработанный, ".", "");
		ТипМуниципальногоРайонаОбработанный = СтрЗаменить(ТипМуниципальногоРайонаОбработанный, "-", "");
		
		КодыМуниципальныхРайонов = АдресныйКлассификаторПовтИсп.КодыМуниципальныхРайонов();
		КодМуниципальногоРайона = КодыМуниципальныхРайонов.Получить(ТипМуниципальногоРайонаОбработанный);
	...
	Если СтрДлина(ОКТМОСтрокой) > 3 Тогда
		...
		Если СтрНачинаетсяС(ОКТМО, "03731000") Тогда
					КодМуниципальногоРайона = "5";
		ИначеЕсли КодВторойСтупениКлассификации = "3"
			  Или ЭтоГородФедеральногоЗначения 
			    И КодВторойСтупениКлассификации = "9" Тогда
		...
		ИначеЕсли КодВторойСтупениКлассификации = "8" Тогда
			ВтораяСтупеньКлассификации = Сред(ОКТМОСтрокой, 3, 3);
			ТипЧисло = ОбщегоНазначения.ОписаниеТипаЧисло(3);
			Код = ТипЧисло.ПривестиЗначение(ВтораяСтупеньКлассификации);
			Если Код >= 810 И Код <= 829 Тогда
				КодМуниципальногоРайона = "1";
			ИначеЕсли Код >= 830 И Код <= 849 Тогда
				КодМуниципальногоРайона = "4";
			ИначеЕсли Код >= 850 И Код <= 898 Тогда
				КодМуниципальногоРайона = "2";
			КонецЕсли;
		КонецЕсли;
...
Функция КодПоселения(Знач ТипПоселения) Экспорт
	
	Если Не ЗначениеЗаполнено(ТипПоселения) Тогда
		Возврат "";
	КонецЕсли;
	
	ТипПоселенияОбработанный = СтрЗаменить(ВРег(ТипПоселения), " ", "");
	ТипПоселенияОбработанный = СтрЗаменить(ТипПоселенияОбработанный, ".", "");
	ТипПоселенияОбработанный = СтрЗаменить(ТипПоселенияОбработанный, "-", "");
	
	КодыПоселений = АдресныйКлассификаторПовтИсп.КодыПоселений();
	КодПоселения = КодыПоселений.Получить(ТипПоселенияОбработанный);
	
	Если ТипПоселения = Неопределено Тогда
		Возврат "";
	КонецЕсли;
...
Функция КодыАдреса(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Если ТипЗнч(АдресСтруктура) = Тип("Структура") Тогда
		ИдентификаторАдресногоОбъекта = ИдентификаторАдресногоОбъекта(АдресСтруктура);
	КонецЕсли;
...
Функция КодыАдресаИКодыКЛАДР(Знач Адрес, Знач ИдентификаторАдресногоОбъекта = "", Источник = Неопределено) Экспорт
	...
	Результат.КодыАдреса.ИдентификаторПомещения = Адрес.ApartmentID;
	...
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] И Источник <> "Сервис1С" Тогда
		...
		Если Не ЗначениеЗаполнено(Результат.КодыАдреса.ИдентификаторПомещения) И ЗначениеЗаполнено(Адрес.houseId) И Адрес.Apartments.Количество() = 1 Тогда
			
			СведенияОПомещении = Адрес.Apartments[0];
			ПредставлениеПомещения = СведенияОПомещении.type + " " + СведенияОПомещении.number;
			
			ВариантыПомещений = СписокПомещенийИзЗагруженныхСведений(Адрес.houseId, ПредставлениеПомещения, Истина);
			Если ВариантыПомещений <> Неопределено И ВариантыПомещений.Количество() = 1 Тогда
				ВариантПомещения = ВариантыПомещений[0];
				Результат.КодыАдреса.ИдентификаторПомещения = ?(ЗначениеЗаполнено(ВариантПомещения.ИдентификаторПомещения),
					Новый УникальныйИдентификатор(ВариантПомещения.ИдентификаторПомещения), "");
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗагруженныеАдресныеСведения["КлассификаторДоступен"] Или Источник = "Сервис1С" Тогда
		...
		Если Не РезультатВебСервис.Отказ И РезультатВебСервис.Данные.Количество() > 0 Тогда
			...
			АдресКорректный = АдресПослеПроверки.АдресКорректный Или АдресПослеПроверки.АдресЧастичноКорректный;
			Если Не АдресКорректный Или АдресПослеПроверки.Варианты.Количество() = 0 Тогда
				Возврат Результат;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(Результат.КодыАдреса.ИдентификаторПомещения) И Сведения.Apartments.Количество() = 1 Тогда
				СведенияОПомещении = Сведения.Apartments[0];
				Результат.КодыАдреса.ИдентификаторПомещения = СведенияОПомещении.id;
			КонецЕсли;
			...
			// Получение кодов КЛАДР, если их нет на уровне дома
			Если ПустаяСтрока(Сведения.codeKLADR) Тогда
				АдресБезДома = ОбщегоНазначения.СкопироватьРекурсивно(Адрес);
				АдресБезДома.apartments = Новый Массив;
				АдресБезДома.buildings = Новый Массив;
				АдресБезДома.houseId = "";
				АдресБезДома.houseNumber = "";
				АдресБезДома.houseType = "";
				ИндексЭлемента = АдресБезДома.munLevels.Найти("house");
				Если ИндексЭлемента <> Неопределено Тогда
					АдресБезДома.munLevels.Удалить(ИндексЭлемента);
				КонецЕсли;
				ИндексЭлемента = АдресБезДома.admLevels.Найти("house");
				Если ИндексЭлемента <> Неопределено Тогда
					АдресБезДома.admLevels.Удалить(ИндексЭлемента);
				КонецЕсли;

				Адреса = Новый Соответствие;
				Адреса.Вставить(1, АдресБезДома);
...
Функция ЭтоГородФедеральногоЗначения(Город, ОфициальноеНаименование = "") Экспорт
	...
	Если ЗначениеЗаполнено(ОфициальноеНаименование) Тогда
		Результат = НазванияГородовФедеральногоЗначения.Получить(ВРег(ОфициальноеНаименование));
	КонецЕсли;
	
	Если Результат = Неопределено Тогда
		Результат = НазванияГородовФедеральногоЗначения.Получить(ВРег(Город));
	КонецЕсли;
...
// Возвращает признак, указывающий на наличие улиц в адресном объекте.
// 
// Параметры:
//  Идентификатор - УникальныйИдентификатор - идентификатор объекта, для которого проверяется наличие улиц.
//  ЭтоМуниципальныйАдрес - Булево - признак, что адрес муниципальный.
// 
// Возвращаемое значение:
//  Булево - Истина, если адресный объект содержит улицы.
//
Функция АдресныйОбъектСодержитУлицы(Идентификатор, ЭтоМуниципальныйАдрес) Экспорт
	...
	Запрос.Текст ="ВЫБРАТЬ ПЕРВЫЕ 1
		|	ЕСТЬNULL(АдресныйОбъект5.Уровень, ЕСТЬNULL(АдресныйОбъект4.Уровень, ЕСТЬNULL(АдресныйОбъект3.Уровень, ЕСТЬNULL(АдресныйОбъект2.Уровень, АдресныйОбъект1.Уровень)))) = &Уровень КАК Поле1
		|ИЗ
		|	РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект0
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия1
		|		ПО (Иерархия1.РодительскийИдентификатор = АдресныйОбъект0.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия2
		|		ПО (Иерархия2.РодительскийИдентификатор = Иерархия1.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия3
		|		ПО (Иерархия3.РодительскийИдентификатор = Иерархия2.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия4
		|		ПО (Иерархия4.РодительскийИдентификатор = Иерархия3.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия5
		|		ПО (Иерархия5.РодительскийИдентификатор = Иерархия4.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект1
		|		ПО (Иерархия1.Идентификатор = АдресныйОбъект1.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект2
		|		ПО (Иерархия2.Идентификатор = АдресныйОбъект2.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект3
		|		ПО (Иерархия3.Идентификатор = АдресныйОбъект3.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект4
		|		ПО (Иерархия4.Идентификатор = АдресныйОбъект4.Идентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект5
		|		ПО (Иерархия5.Идентификатор = АдресныйОбъект5.Идентификатор)
		|ГДЕ
		|	АдресныйОбъект0.Идентификатор = &Идентификатор
		|	И ЕСТЬNULL(АдресныйОбъект5.Уровень, ЕСТЬNULL(АдресныйОбъект4.Уровень, ЕСТЬNULL(АдресныйОбъект3.Уровень, ЕСТЬNULL(АдресныйОбъект2.Уровень, АдресныйОбъект1.Уровень)))) = &Уровень";

	Если Не ЭтоМуниципальныйАдрес Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, Метаданные.РегистрыСведений.МуниципальнаяИерархия.Имя, Метаданные.РегистрыСведений.АдминистративнаяИерархия.Имя);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("Уровень", 8);
...
// Возвращает данные классификатора выбора по улицам и территориям.
//
// Параметры:
//    СтрокаПоиска - Строка - текст, по которому подбираются варианты улиц.
//    ПараметрыАвтоподбора - Структура:
//     * Идентификатор  - УникальныйИдентификатор - адресный объект.
//     * ТипАдреса - Строка -Варианты: Муниципальный, Административно-территориальный.
//
// Возвращаемое значение:
//     Структура:
...
//       * КраткоеПредставлениеОшибки   - Строка - описание ошибки, если поставщик недоступен. Неопределено, если Отказ
//                                                 = Ложь.
//       * Данные                       - СписокЗначений - содержит данные для выбора:
//          ** Значение       - УникальныйИдентификатор - идентификатор адресного объекта.
//          ** Представление  - Строка - представление адреса.
//
Функция ПодборУлиц(ПараметрыАвтоподбора, СтрокаПоиска = Неопределено) Экспорт
	...
	Если ЗагруженныеАдресныеСведения.Получить("ИспользоватьЗагруженные") Или Уровень = 1 Тогда
		РезультатПодбораПоЗагруженнымДанным = ПодборУлицЗагруженныеДанные(ПараметрыАвтоподбора, СтрокаПоиска);
		ЗаполнитьЗначенияСвойств(ДанныеАдресныхОбъектов, РезультатПодбораПоЗагруженнымДанным);
	КонецЕсли;
	
	Если ДанныеАдресныхОбъектов.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		
		Родитель  = ПараметрыАвтоподбора.Идентификатор;
		// Веб сервис 1С, может быть на обслуживании.
		Попытка
			...
			ДобавленныеИдентификаторыАдресногоОбъекта = Новый Соответствие();
			...
			Для Каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
				...
				Если ПустаяСтрока(Представление)
					Или ДобавленныеИдентификаторыАдресногоОбъекта.Получить(Идентификатор) = Истина Тогда
						Продолжить;
				КонецЕсли;
				...
				ДобавленныеИдентификаторыАдресногоОбъекта.Вставить(АдресныйОбъект.Идентификатор, Истина);
...
// Возвращает признак, указывающий на присутствие в данном адресном объекте дочерних элементов.
// 
// Параметры:
//  Идентификатор - УникальныйИдентификатор - идентификатор объекта, для которого проверяется наличие улиц.
//  ЭтоМуниципальныйАдрес - Булево - признак, что адрес муниципальный.
// 
// Возвращаемое значение:
//  Массив - набор наименований дочерних уровней с адресными объектами.
// 
Функция ДочерниеУровниСАдреснымиОбъектами(Идентификатор, ЭтоМуниципальныйАдрес) Экспорт
	...
	Запрос.Текст ="ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ИЗ
		|	РегистрСведений.МуниципальнаяИерархия КАК МуниципальнаяИерархия
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО (МуниципальнаяИерархия.Идентификатор = АдресныеОбъекты.Идентификатор)
		|ГДЕ
		|	МуниципальнаяИерархия.РодительскийИдентификатор = &Идентификатор  
		|	И НЕ АдресныеОбъекты.Уровень ЕСТЬ NULL
		|
		|УПОРЯДОЧИТЬ ПО
		|	Уровень";
	Иначе 
		...
		|	АдресныеОбъекты.Уровень КАК Уровень
		|ИЗ
		|	РегистрСведений.АдминистративнаяИерархия КАК МуниципальнаяИерархия
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО МуниципальнаяИерархия.Идентификатор = АдресныеОбъекты.Идентификатор
		|ГДЕ
		|	МуниципальнаяИерархия.РодительскийИдентификатор = &Идентификатор  
	...
	СписокУровней = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Уровень");

	ИменаУровнейАдресаПоКоду = ИменаУровнейАдресаПоКоду();
	Для Каждого НомерУровня Из СписокУровней Цикл
		Результат.Добавить(ИменаУровнейАдресаПоКоду[НомерУровня]);
	КонецЦикла;
...
// Возвращает информацию для поддержки.
//
// Параметры:
//  ПредставлениеАдреса - Строка - представление адреса.
//  СведенияОбАдресе - см. РаботаСАдресами.СведенияОбАдресе.
//
// Возвращаемое значение:
//  Строка - информация для поддержки.
//
Функция ИнформацияДляПоддержки(Знач ПредставлениеАдреса, Знач СведенияОбАдресе) Экспорт
	...
	Если ТипЗнч(СведенияОбАдресе) = Тип("Структура") Тогда
		...
		Результат.Добавить(НСтр("ru = 'Коды и идентификаторы адреса:'"));
		
		Для Каждого Элемент Из СведенияОбАдресе Цикл
				...
				Для Каждого ЭлементВложенный Из Элемент.Значение Цикл
					Если СтрЗаканчиваетсяНа(ЭлементВложенный.Ключ, "Идентификатор")
						Или ПустаяСтрока(ЭлементВложенный.Значение) Тогда
							Продолжить;
					КонецЕсли;
					Результат.Добавить(ЭлементВложенный.Ключ + ": " + ЭлементВложенный.Значение);
				КонецЦикла;
			Иначе
	...
	Если Не ИспользоватьЗагруженные Тогда
		Результат.Добавить(НСтр("ru = 'В адресном классификаторе отсутствуют загруженные адресные сведения.'"));
	Иначе
		Результат.Добавить(НСтр("ru = 'Автоподбор адреса выполняется по загруженным данным для регионов:'"));
	КонецЕсли;
	...
	Для Каждого СведенияОЗагрузкеСубъектаРФ Из СведенияОЗагрузкеСубъектовРФ Цикл
		...
		Если ТипЗнч(Значение) = Тип("Структура") И Значение.ИспользоватьЗагруженные Тогда
			СведенияОРегионах.Добавить(
				Формат(СведенияОЗагрузкеСубъектаРФ.Ключ,"ЧЦ=2; ЧВН="),
				НСтр("ru = 'Загружено'") + " - " + Строка(Значение.ДатаЗагрузки));
		КонецЕсли;
	...
	Для Каждого Регион Из СведенияОРегионах Цикл
		...
		СтрокаОтчета = Регион.Значение + ", " + ИмяРегиона + ": " + Регион.Представление;
		Результат.Добавить(СтрокаОтчета);
	КонецЦикла;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		...
		Если ИПППодключена Тогда
			Результат.Добавить(НСтр("ru = 'Подключен к Интернет-поддержке пользователей: Да.'"));
		Иначе
			Результат.Добавить(НСтр("ru = 'Подключен к Интернет-поддержке пользователей: Нет.'"));
		КонецЕсли;
	...
	Если ИПППодключена Тогда
		...
		Если Не АдресныйКлассификаторПовтИсп.ИспользоватьТолькоЗагруженныеДанные() Тогда
			...
			СсылкаНаВебСервис = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = ':%1/%2'"),
				ИнформацияОСервисе.Порт,
			...
			Если КлассификаторДоступен Тогда
				...
				|URL подключения: %1'");
				Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, СсылкаНаВебСервис));
				
			Иначе
				
				Результат.Добавить(НСтр("ru = 'Веб-сервис недоступен.'"));
				
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
					
					МодульПолучениеФайловИзИнтернета = ОбщегоНазначения.ОбщийМодуль("ПолучениеФайловИзИнтернета");
					РезультатДиагностики = МодульПолучениеФайловИзИнтернета.ДиагностикаСоединения(СсылкаНаВебСервис,,Ложь);
					Результат.Добавить(РезультатДиагностики.ЖурналДиагностики);
					
				КонецЕсли;
				
			КонецЕсли;
		Иначе
			Результат.Добавить(НСтр("ru = 'Использование веб-сервиса отключено.'"));
		КонецЕсли;
	...
	ОбщаяИнформацияДляПоддержки = СтандартныеПодсистемыСервер.ИнформацияДляПоддержки();
	Результат.Добавить(ОбщаяИнформацияДляПоддержки);
	
	Результат.Добавить("");
	Результат.Добавить(НСтр("ru = 'Отчет создан'") + ": " + ТекущаяДатаСеанса());
...
Функция ИспользоватьВебСервис() Экспорт
	...
	Возврат СтрСравнить(Источник, "Приложение") <> 0 И ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
...
//   * Родитель - УникальныйИдентификатор
// 
Функция СведенияОбАдресномОбъекта(ИдентификаторАдреса) Экспорт
	...
	СведенияОбАдресе.Вставить("Представление",     "");
	СведенияОбАдресе.Вставить("Родитель",          Неопределено);
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Описание.Идентификатор              = ИдентификаторЧасовыеЗоныСубъектовРФ();
	Описание.Наименование               = НСтр("ru = 'Часовые зоны субъектов РФ'");
...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан, ДополнительныеПараметры) Экспорт
	...
	Если Идентификатор <> ИдентификаторЧасовыеЗоныСубъектовРФ() Тогда
		Возврат;
	КонецЕсли;
...
Функция АдресныеСокращения()
	...
	Возврат АдресныйКлассификаторПовтИсп.АдресныеСокращения();

КонецФункции

// Возвращает адресное сокращение по заданному критерию.
//
// Параметры:
//  ЗначениеПоиска - Строка - искомое значение (например, "обл." или "область").
//  ИмяПоля - Строка - имя поля для сравнения (например, "Сокращение" или "ПолноеНаименование").
//  Уровень - Строка - : уровень адресного элемента для фильтрации (например, "Регион", "Район").
//
// Возвращаемое значение:
//  Структура - найденная строка таблицы сокращений 
//  Неопределено - если не найдено.
//
Функция АдресноеСокращение(ЗначениеПоиска, ИмяПоля, Уровень = Неопределено)

	ВсеСокращения = АдресныйКлассификаторПовтИсп.АдресныеСокращения();

	Если ЗначениеЗаполнено(Уровень) Тогда
		Отбор = Новый Структура("Уровень", Уровень);
		СокращенияДляПоиска = ВсеСокращения.НайтиСтроки(Отбор);
	Иначе
		СокращенияДляПоиска = ВсеСокращения;
	КонецЕсли;

	Для Каждого Строка Из СокращенияДляПоиска Цикл
		Если СтрСравнить(Строка[ИмяПоля], ЗначениеПоиска) = 0 Тогда
			Возврат Строка;
		КонецЕсли;
	КонецЦикла;
...
// Возвращает адресное сокращение по значению во всех поисковых полях таблицы сокращений.
//
// Параметры:
//  ЗначениеПоиска - Строка - искомое значение (например, "обл.", "область", "г.").
//  Уровень - Строка - : уровень адресного элемента для фильтрации
//    (например, "Регион", "Район", "Город"). Если не указан - поиск по всем уровням.
//
// Возвращаемое значение:
//  Структура - найденная строка таблицы сокращений 
//  Неопределено - если не найдено.
//
Функция АдресноеСокращениеПоВсемПолям(ЗначениеПоиска, Уровень = Неопределено)
	...
	ПоляДляПоиска.Добавить("Сокращение");
	ПоляДляПоиска.Добавить("СокращениеПриказаМинфинаРФ171н");

	Для Каждого Строка Из СокращенияДляПоиска Цикл
		Для Каждого ИмяПоля Из ПоляДляПоиска Цикл
			Если СтрСравнить(Строка[ИмяПоля], ЗначениеПоиска) = 0 Тогда
				Возврат Строка;
			КонецЕсли;
		КонецЦикла;
...
Функция ЗначениеГородаОтличаетсяОтНаселенногоПункта(Город, НаселенныйПункт)
	Возврат СтрСравнить(Город, НаселенныйПункт) <> 0;
КонецФункции

Функция ОфициальноеНаименованиеРегиона(Знач Наименование)
...
Функция РаспознатьАдресЗагруженныеДанные(ИсходныеЧастиАдреса, КодРегиона, Представление)

	ЧастиАдреса = ОбработатьОбщепринятыеСокращенияИУдалитьТочкуВКонецТипа(ИсходныеЧастиАдреса);
	...
	|	ДанныеАдреса.Наименование КАК Наименование,
	|	ДанныеАдреса.ТипОбъекта КАК ТипОбъекта,
	|	ДанныеАдреса.ТипОбъекта2 КАК ТипОбъекта2
	|ПОМЕСТИТЬ ДанныеАдреса
	|ИЗ
	|	&ДанныеАдреса КАК ДанныеАдреса
	|ГДЕ
	...
	|	АдресныеОбъекты.Наименование КАК Наименование,
	|	АдресныеОбъекты.ТипОбъекта КАК ТипОбъекта,
	|	ДанныеАдреса.ТипОбъекта КАК ИсходныйТипОбъекта,
	|	АдресныеОбъекты.Идентификатор КАК Идентификатор
	|ПОМЕСТИТЬ НайденныеОбъекты
	|ИЗ
	|	ДанныеАдреса КАК ДанныеАдреса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ПО (ДанныеАдреса.Наименование = АдресныеОбъекты.Наименование)
	|			И (ДанныеАдреса.ТипОбъекта = АдресныеОбъекты.ТипОбъекта
	|				ИЛИ ДанныеАдреса.ТипОбъекта + ""."" = АдресныеОбъекты.ТипОбъекта
	|				ИЛИ ДанныеАдреса.ТипОбъекта2 = АдресныеОбъекты.ТипОбъекта
	|				ИЛИ ДанныеАдреса.ТипОбъекта2 + ""."" = АдресныеОбъекты.ТипОбъекта)
	|ГДЕ
	...
	|	ДанныеАдреса.Наименование КАК Наименование,
	|	ДанныеАдреса.ТипОбъекта КАК ТипОбъекта, 
	|	ДанныеАдреса.ИсходныйТипОбъекта КАК ИсходныйТипОбъекта,
	|	ДанныеАдреса.Идентификатор КАК Идентификатор,
	|	АдминистративнаяИерархия.РодительскийИдентификатор КАК АдминистративныйРодительскийИдентификатор,
	|	МуниципальнаяИерархия.РодительскийИдентификатор КАК МуниципальныйРодительскийИдентификатор
	|ИЗ
	|	НайденныеОбъекты КАК ДанныеАдреса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдминистративнаяИерархия КАК АдминистративнаяИерархия
	|		ПО (АдминистративнаяИерархия.Идентификатор = ДанныеАдреса.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК МуниципальнаяИерархия
	|		ПО (МуниципальнаяИерархия.Идентификатор = ДанныеАдреса.Идентификатор)
	|ГДЕ
	|	(АдминистративнаяИерархия.КодСубъектаРФ = &КодСубъектаРФ ИЛИ МуниципальнаяИерархия.КодСубъектаРФ = &КодСубъектаРФ)
	|	И НЕ ДанныеАдреса.Идентификатор ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДанныеАдреса.Уровень УБЫВ, ДанныеАдреса.Позиция УБЫВ
	|";
	...
	Муниципальный = Истина;
	АдресПоНайденнымДанным = ОпределитьАдресПоНайденнымДанным(СопоставленныеЧастиАдреса, Муниципальный, Представление);
	Если АдресПоНайденнымДанным.Адрес = Неопределено Тогда
		Муниципальный = Ложь;
		АдресПоНайденнымДанным = ОпределитьАдресПоНайденнымДанным(СопоставленныеЧастиАдреса, Муниципальный, Представление);
	КонецЕсли;
	...
	Для Каждого ЭлементАдреса Из АдресПоНайденнымДанным.ЦепочкаАдреса Цикл
		
		ОтборПоУровню = Новый Структура;
		ОтборПоУровню.Вставить("Наименование", ЭлементАдреса.Наименование);
		ОтборПоУровню.Вставить("ТипОбъекта", ЭлементАдреса.ИсходныйТипОбъекта);
		
		СтрокиПоУровню = ЧастиАдреса.НайтиСтроки(ОтборПоУровню);
		
		Для Каждого СтрокаУровня Из СтрокиПоУровню Цикл
			СтрокаУровня.Уровень = ЭлементАдреса.Уровень;
		КонецЦикла;
		
	КонецЦикла;
	
	АдресДанные = АдресПоНайденнымДанным.Адрес;
	
	// Все нераспознанное считаем домами.
	...
	Для Каждого ЭлементАдреса Из ЧастиАдреса Цикл
		Если ЭлементАдреса.Уровень <> 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЭтоТипЗемельныйУчасток(ЭлементАдреса) Тогда
	...
	ИдентификаторАдресногоЭлемента = ИдентификаторАдресногоОбъекта(Адрес);
	...
	Если ЗначениеЗаполнено(СведенияОЗемельномУчастке) Тогда 
		ДанныеОбУчастке = КодыПоИдентификаторуАдресногоОбъектаЗемельныйУчасток(ИдентификаторАдресногоЭлемента, СведенияОЗемельномУчастке);
	...
	ОписаниеДома = КодыПоИдентификаторуАдресногоОбъектаДома(ИдентификаторАдресногоЭлемента, СведенияОДоме);
...
Функция ОпределитьАдресПоНайденнымДанным(Знач СопоставленныеЧастиАдреса, Муниципальный, Представление)
	
	РезультатОпределения = Новый Структура;
	РезультатОпределения.Вставить("Адрес",         Неопределено);
	РезультатОпределения.Вставить("ЦепочкаАдреса", Неопределено);
	
	ВерхнийУровеньАдреса = СопоставленныеЧастиАдреса.Получить(0);
	
	ОтборПоУровню = Новый Структура;
	ОтборПоУровню.Вставить("Наименование", ВерхнийУровеньАдреса.Наименование);
	ОтборПоУровню.Вставить("ТипОбъекта",   ВерхнийУровеньАдреса.ТипОбъекта);
	СтрокиУровня = СопоставленныеЧастиАдреса.НайтиСтроки(ОтборПоУровню);
	
	ИмяПоляРодительскогоИдентификатора = ?(Муниципальный, 
		"МуниципальныйРодительскийИдентификатор", "АдминистративныйРодительскийИдентификатор");
	
	СведенияОбАдресе = Неопределено;
	
	Для каждого СтрокаУровня Из СтрокиУровня Цикл
		
		ЦепочкаРодителей = Новый Структура;
		ЦепочкаРодителей.Вставить("КореньНайден", Ложь);
		ЦепочкаРодителей.Вставить("Уровни",       Новый Массив);
		
		ЦепочкаРодителей.Уровни.Добавить(СтрокаУровня);
		
		ЦепочкаРодителей = ПутьДоКорняНайден(СопоставленныеЧастиАдреса,
			СтрокаУровня[ИмяПоляРодительскогоИдентификатора], ИмяПоляРодительскогоИдентификатора, ЦепочкаРодителей);
			
		Если ЦепочкаРодителей.КореньНайден Тогда
			
			СведенияОбАдресе = СведенияОбАдресномОбъекта(СтрокаУровня.Идентификатор);
			СведенияОбАдресе.ЗагруженныеДанные = Истина;
			СведенияОбАдресе.Муниципальный = Муниципальный;
			СведенияОбАдресе.Представление = Представление;
			РезультатОпределения.ЦепочкаАдреса = ЦепочкаРодителей.Уровни;
			Прервать;
	...
	РезультатОпределения.Адрес = АктуальныеАдресныеСведения(СведенияОбАдресе);
	
	Возврат РезультатОпределения;
	
КонецФункции

Функция ПутьДоКорняНайден(СопоставленныеЧастиАдреса, Идентификатор, ИмяПоляРодительскогоИдентификатора, ЦепочкаРодителей)
	
	НайденнаяСтрока = СопоставленныеЧастиАдреса.Найти(Идентификатор, "Идентификатор");
	Если НайденнаяСтрока = Неопределено Тогда
		ЦепочкаРодителей.КореньНайден = Ложь;
		Возврат ЦепочкаРодителей;
	КонецЕсли;
	
	Если НайденнаяСтрока.Уровень = 1 Тогда
		ЦепочкаРодителей.Уровни.Добавить(НайденнаяСтрока);
		ЦепочкаРодителей.КореньНайден = Истина;
		Возврат ЦепочкаРодителей;
	КонецЕсли;
	
	ЦепочкаРодителей.Уровни.Добавить(НайденнаяСтрока);
	
	Возврат ПутьДоКорняНайден(СопоставленныеЧастиАдреса, НайденнаяСтрока[ИмяПоляРодительскогоИдентификатора], ИмяПоляРодительскогоИдентификатора, ЦепочкаРодителей);
...
Функция СопоставленныеЧастиАдреса(Знач РезультатЗапроса)
	...
	СопоставленныеЧастиАдреса = РезультатЗапроса[2].Выгрузить();
		...
		Если Результат.Данные.Количество() > 0 Тогда
			...
			Для Каждого АдресДляПроверки Из АдресаДляПроверки Цикл
				...
				Если АдресИзСервиса.Варианты.Количество() > 0 Тогда
					...
					Если Адрес.Свойство("apartmentId") И ВариантИзСервиса.Свойство("Apartments") 
					   И ВариантИзСервиса.Apartments.Количество() = 1 Тогда
						СведенияОПомещении = ВариантИзСервиса.Apartments[0];
						Адрес.apartmentId = СведенияОПомещении.id;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
...
Процедура УстановитьИдентификаторыАдресовПоЗагруженнымДанным(Адреса)
	...
	ВариантыПомещений = Новый СписокЗначений();
	
	Дома = РезультатЗапросаДома.Выгрузить();
	...
	Пока ПолученныйАдрес.Следующий() Цикл
		...
		Результат = НайтиСтрокуСДомом(Дома, СведенияОДоме);
		Если Результат <> Неопределено Тогда
			Адрес.HouseID = Результат.ИдентификаторДома;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Адрес.HouseID) И Адрес.apartments.Количество() = 1 Тогда
			ПредставлениеПомещения = Адрес.apartments[0].type + " " +Адрес.apartments[0].number;
			ВариантыПомещений.Добавить(Адрес.HouseID, ПредставлениеПомещения, Истина);
		КонецЕсли;
	
	КонецЦикла;
	
	Если ВариантыПомещений.Количество() > 0 Тогда
		ИдентификаторыПомещений = ПомещенияАдресныхОбъектовИзЗагруженныхСведений(ВариантыПомещений);
		
		Для Каждого КлючИЗначение Из Адреса Цикл
			Адрес = КлючИЗначение.Значение;
			СопоставленныеПомещения = ИдентификаторыПомещений.Получить(Адрес.HouseID);
			Если СопоставленныеПомещения <> Неопределено Тогда
				Для Каждого ВариантПомещения Из СопоставленныеПомещения Цикл
					Если СтрСравнить(ВариантПомещения.ТипПомещения, Адрес.apartments[0].type) = 0 Тогда
						Адрес.apartmentId = ?(ЗначениеЗаполнено(ВариантПомещения.ИдентификаторПомещения),
							Новый УникальныйИдентификатор(ВариантПомещения.ИдентификаторПомещения), "");
						Продолжить;
					КонецЕсли;
				КонецЦикла;
...
Процедура ЗаполнитьАдресныеСведенияИзЗагруженныхДанных(Результат, СведенияОбАдресномОбъекте)
	...
	Если Выборка.Количество() > 2 И ЗначениеЗаполнено(СведенияОбАдресномОбъекте.Представление)Тогда
		
		Адреса = Выборка.СкопироватьКолонки();
		
		МуниципальныйАдресНайден = Ложь;
		Для Каждого АдресИзВыборки Из Выборка Цикл
			
			Если АдресИзВыборки.ТипАдреса = АдминистративноТерриториальныйАдрес() Тогда 
				СтрокаАдреса = Адреса.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаАдреса, АдресИзВыборки);
				Продолжить;
			КонецЕсли;
			
			Если МуниципальныйАдресНайден Тогда
				Продолжить;
			КонецЕсли;
			
			Для Позиция = 0 По 5 Цикл
				Если СведенияОбАдресномОбъекте.Родитель = АдресИзВыборки["Идентификатор" + Строка(Позиция)] Тогда
					МуниципальныйАдрес = АдресИзВыборки;
					МуниципальныйАдресНайден = Истина;
					Прервать;
				ИначеЕсли СтрНайти(ВРег(СведенияОбАдресномОбъекте.Представление), 
							ВРег(АдресИзВыборки["Наименование" + Строка(Позиция)])) = 0 Тогда
					МуниципальныйАдресНайден = Ложь;
					Прервать;
				Иначе
					МуниципальныйАдрес = АдресИзВыборки;
		...
		СтрокаАдреса = Адреса.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаАдреса, МуниципальныйАдрес);
		Выборка = Адреса;
	КонецЕсли;
	...
	Для Каждого АдресИзВыборки Из Выборка Цикл
		...
		ИначеЕсли АдресИзВыборки.ТипАдреса = АдминистративноТерриториальныйАдрес() Тогда
...
Функция ПодборУлицЗагруженныеДанные(ПараметрыАвтоподбора, ТекстПоиска = Неопределено)
	
	Идентификатор  = ПараметрыАвтоподбора.Идентификатор;
	...
	ТекстЗапроса = ?(ПустаяСтрока(ТекстПоиска), ТекстЗапросаПодбораВсехУлиц(),  ТекстЗапросаПодбораУлицМуниципальнаяИерархия());
	
	Если ЭтоАдминистративноТерриториальныйАдрес(ПараметрыАвтоподбора.ТипАдреса)  Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, Метаданные.РегистрыСведений.МуниципальнаяИерархия.Имя,
			Метаданные.РегистрыСведений.АдминистративнаяИерархия.Имя);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстПоиска) Тогда
		...
		Запрос.УстановитьПараметр("Уровень", 7); // Территория или Улица
	Иначе
		Запрос.УстановитьПараметр("Уровень", УровеньУлицы); // Только улицы
	КонецЕсли;
	Запрос.УстановитьПараметр("Идентификатор", Новый УникальныйИдентификатор(Идентификатор));
	...
	ИсключенияПоУровням = Новый Структура();
	Для Каждого Уровень Из АдресныйКлассификаторПовтИсп.УровниАдресныхСведений() Цикл
		ИсключенияПоУровням.Вставить("Уровень" + Строка(Уровень), NULL);
	КонецЦикла;
	
	Для Каждого АдресИзВыборки Из Выборка Цикл
		УровниВАдресе = Новый Соответствие;

		Для Позиция = 0 По 5 Цикл

			НаименованиеОбъекта = СоединитьНаименованиеИТипАдресногоОбъекта(АдресИзВыборки["Наименование" 
				+ Строка(Позиция)], АдресИзВыборки["ТипОбъекта" + Строка(Позиция)]);

			Если ПустаяСтрока(НаименованиеОбъекта) Или ПустаяСтрока(АдресИзВыборки["Уровень" + Строка(Позиция)])
				Или АдресИзВыборки["Уровень" + Строка(Позиция)] = УровеньУлицы Тогда
				Продолжить;
			КонецЕсли;

			УровеньАдреса = "Уровень" + АдресИзВыборки["Уровень" + Строка(Позиция)];
			УровниВАдресе.Вставить(АдресИзВыборки["Уровень" + Строка(Позиция)], Истина);

			Если ИсключенияПоУровням[УровеньАдреса] = Null Тогда
				ИсключенияПоУровням[УровеньАдреса] = НаименованиеОбъекта;
			ИначеЕсли ИсключенияПоУровням[УровеньАдреса] <> Неопределено Тогда
				Если СтрСравнить(ИсключенияПоУровням[УровеньАдреса], НаименованиеОбъекта) <> 0 Тогда
					ИсключенияПоУровням[УровеньАдреса] = Неопределено;
				КонецЕсли;
			КонецЕсли;
			
		КонецЦикла;

		Для Позиция = 1 По 7 Цикл
			УровеньАдреса = "Уровень" + Строка(Позиция);
			Если УровниВАдресе[Позиция] = Неопределено И ИсключенияПоУровням[УровеньАдреса] <> Null Тогда
				ИсключенияПоУровням[УровеньАдреса] = Неопределено;
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого АдресИзВыборки Из Выборка Цикл
		...
		Для Позиция = 0 По 5 Цикл
			...
			УровеньАдреса = "Уровень" + АдресИзВыборки["Уровень" + Строка(Позиция)];
			Если ПустаяСтрока(АдресИзВыборки["Уровень" + Строка(Позиция)])
			 Или (АдресИзВыборки["Уровень" + Строка(Позиция)] <> 8 И ИсключенияПоУровням[УровеньАдреса] <> Неопределено) Тогда
				Продолжить;
			КонецЕсли;
			
			НаборПредставление.Добавить(
				СоединитьНаименованиеИТипАдресногоОбъекта(АдресИзВыборки["Наименование" + Строка(Позиция)], 
				АдресИзВыборки["ТипОбъекта" + Строка(Позиция)]));
			
		КонецЦикла;
		
		Если НаборПредставление.Количество() > 0 И ЗначениеЗаполнено(АдресИзВыборки.Идентификатор) Тогда
			
			АдресныйОбъект = СведенияОбПодбираемомАдресномОбъекте();
			АдресныйОбъект.ЗагруженныеДанные = Истина;
			АдресныйОбъект.Наименование = СтрСоединить(НаборПредставление, ", ");
			АдресныйОбъект.Идентификатор = АдресИзВыборки.Идентификатор;
			АдресныйОбъект.Уровень = АдресИзВыборки.Уровень0;
			Результат.Данные.Добавить(АдресныйОбъект, СтрСоединить(НаборПредставление, ", "));
...
Функция ТекстЗапросаПодбораУлицМуниципальнаяИерархия()
	
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 20
	|	ЕСТЬNULL(АдресныйОбъект5.Идентификатор, ЕСТЬNULL(АдресныйОбъект4.Идентификатор, ЕСТЬNULL(АдресныйОбъект3.Идентификатор, ЕСТЬNULL(АдресныйОбъект2.Идентификатор, АдресныйОбъект1.Идентификатор)))) КАК Идентификатор
	|ПОМЕСТИТЬ ИдентификаторыДочернихОбъектов
	|ИЗ
	|	РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия1
	|		ПО (Иерархия1.РодительскийИдентификатор = АдресныйОбъект0.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект1
	|		ПО (Иерархия1.Идентификатор = АдресныйОбъект1.Идентификатор)
	|			И (АдресныйОбъект1.Наименование ПОДОБНО &НачалоФразы СПЕЦСИМВОЛ ""~"" 
	|				ИЛИ АдресныйОбъект1.Наименование ПОДОБНО &НачалоФразыСПробелом СПЕЦСИМВОЛ ""~"")
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия2
	|		ПО (Иерархия2.РодительскийИдентификатор = Иерархия1.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект2
	|		ПО (Иерархия2.Идентификатор = АдресныйОбъект2.Идентификатор)
	|			И (АдресныйОбъект2.Наименование ПОДОБНО &НачалоФразы СПЕЦСИМВОЛ ""~"" 
	|				ИЛИ АдресныйОбъект2.Наименование ПОДОБНО &НачалоФразыСПробелом СПЕЦСИМВОЛ ""~"")
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия3
	|		ПО (Иерархия3.РодительскийИдентификатор = Иерархия2.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект3
	|		ПО (Иерархия3.Идентификатор = АдресныйОбъект3.Идентификатор)
	|			И (АдресныйОбъект3.Наименование ПОДОБНО &НачалоФразы СПЕЦСИМВОЛ ""~"" 
	|				ИЛИ АдресныйОбъект3.Наименование ПОДОБНО &НачалоФразыСПробелом СПЕЦСИМВОЛ ""~"")
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия4
	|		ПО (Иерархия4.РодительскийИдентификатор = Иерархия3.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект4
	|		ПО (Иерархия4.Идентификатор = АдресныйОбъект4.Идентификатор)
	|			И (АдресныйОбъект4.Наименование ПОДОБНО &НачалоФразы СПЕЦСИМВОЛ ""~"" 
	|				ИЛИ АдресныйОбъект4.Наименование ПОДОБНО &НачалоФразыСПробелом СПЕЦСИМВОЛ ""~"")
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия5
	|		ПО (Иерархия5.РодительскийИдентификатор = Иерархия4.Идентификатор )
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект5
	|		ПО (Иерархия5.Идентификатор = АдресныйОбъект5.Идентификатор)
	|			И (АдресныйОбъект5.Наименование ПОДОБНО &НачалоФразы СПЕЦСИМВОЛ ""~"" 
	|				ИЛИ АдресныйОбъект5.Наименование ПОДОБНО &НачалоФразыСПробелом СПЕЦСИМВОЛ ""~"")
	|ГДЕ
	|	АдресныйОбъект0.Идентификатор = &Идентификатор И 
	|	НЕ ЕСТЬNULL(АдресныйОбъект5.Идентификатор, ЕСТЬNULL(АдресныйОбъект4.Идентификатор, ЕСТЬNULL(АдресныйОбъект3.Идентификатор, ЕСТЬNULL(АдресныйОбъект2.Идентификатор, АдресныйОбъект1.Идентификатор)))) ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 20
	|	АдресныйОбъект0.Идентификатор КАК Идентификатор,
	|	АдресныйОбъект0.Наименование КАК Наименование0,
	|	АдресныйОбъект0.ТипОбъекта КАК ТипОбъекта0,
	|	АдресныйОбъект0.Уровень КАК Уровень0,
	...
	|ИЗ
	|	ИдентификаторыДочернихОбъектов КАК ИдентификаторыОбъектов
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект0
	|		ПО (ИдентификаторыОбъектов.Идентификатор = АдресныйОбъект0.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия1
	|		ПО (Иерархия1.Идентификатор = АдресныйОбъект0.Идентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия2
	|		ПО (Иерархия2.Идентификатор = Иерархия1.РодительскийИдентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия3
	|		ПО (Иерархия3.Идентификатор = Иерархия2.РодительскийИдентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия4
	|		ПО (Иерархия4.Идентификатор = Иерархия3.РодительскийИдентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия5
	|		ПО (Иерархия5.Идентификатор = Иерархия4.РодительскийИдентификатор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект1
	...
	|ГДЕ
	|		АдресныйОбъект0.Уровень >= &Уровень
	|УПОРЯДОЧИТЬ ПО
	|	Наименование0, ТипОбъекта0";
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаПодбораВсехУлиц()

	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(АдресныйОбъект5.Идентификатор, ЕСТЬNULL(АдресныйОбъект4.Идентификатор,
	|		ЕСТЬNULL(АдресныйОбъект3.Идентификатор, ЕСТЬNULL(АдресныйОбъект2.Идентификатор,
	|		АдресныйОбъект1.Идентификатор)))) КАК Идентификатор
	|ПОМЕСТИТЬ ИдентификаторыДочернихОбъектов
	|ИЗ
	|	РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МуниципальнаяИерархия КАК Иерархия1
	...
	|ГДЕ
	|	АдресныйОбъект0.Идентификатор = &Идентификатор
	|	И НЕ ЕСТЬNULL(АдресныйОбъект5.Идентификатор, ЕСТЬNULL(АдресныйОбъект4.Идентификатор,
	|		ЕСТЬNULL(АдресныйОбъект3.Идентификатор, ЕСТЬNULL(АдресныйОбъект2.Идентификатор,
	|		АдресныйОбъект1.Идентификатор)))) ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция ЭтоТипЗемельныйУчасток(ЭлементАдреса)
	
	// АПК: 1297-выкл Данные адресного классификатора, не локализуются
	ТипОбъекта = ВРег(СокрЛП(ЭлементАдреса.ТипОбъекта));
	Возврат СтрСравнить(ТипОбъекта, "З/У") = 0 Или СтрНачинаетсяС(ВРег(ЭлементАдреса.Значение), ВРег("Земельный участок"));
	// АПК: 1297-вкл
...
Функция ПредставлениеУровняАдреса(Адрес, ИмяУровня) Экспорт
	...
	Если СтрСравнить(ИмяУровня, "munDistrict") = 0 Тогда 
		...
		Если СтрНачинаетсяС(ВРег(Адрес[ИмяУровня]), ВРег("Муниципальный район"))
		 Или СтрНачинаетсяС(ВРег(Адрес[ИмяУровня]), ВРег("Муниципальные районы")) Тогда
	...
	Если СтрСравнить(ИмяУровня, "settlement") = 0 Тогда
		Если СтрНачинаетсяС(ВРег(Адрес[ИмяУровня]), ВРег("Межселенные территории"))
		 Или СтрНачинаетсяС(ВРег(Адрес[ИмяУровня]), ВРег("Межселенная территория"))
		 Или СтрНачинаетсяС(ВРег(Адрес[ИмяУровня]), ВРег("Эвенкийское")) Тогда
			Возврат Адрес[ИмяУровня];
		КонецЕсли;
	КонецЕсли;
	// АПК: 1297-вкл
...
Функция КодыПоИдентификаторуАдресногоОбъектаДома(ИдентификаторАдресногоОбъекта, СведенияОДоме)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДомаЗданияСтроения.Строения КАК Строения,
	|	ДополнительныеСведения.ПочтовыйИндекс КАК Индекс,
	...
	Иначе
		...
		Коды = КодыПоИдентификаторуАдресногоОбъекта(ИдентификаторАдресногоОбъекта);
...
Функция КодыПоИдентификаторуАдресногоОбъектаЗемельныйУчасток(ИдентификаторАдресногоОбъекта, СведенияОЗемельномУчастке)
	...
	ОписаниеУчастка = НайтиСтрокуСЗемельнымУчастком(РезультатЗапроса, СведенияОЗемельномУчастке);
	
	Если ОписаниеУчастка <> Неопределено Тогда
		
		Возврат ОписаниеУчастка;
		
	Иначе
		// Коды с точностью до дома не найдены. Поэтому используем коды адресного объекта
		Коды = КодыПоИдентификаторуАдресногоОбъекта(ИдентификаторАдресногоОбъекта);
		Коды.Вставить("ИдентификаторУчастка", "");
...
Функция КодыПоИдентификаторуАдресногоОбъекта(ИдентификаторАдресногоОбъекта)
			...
			Для Каждого СтрокаДом Из МассивДомов Цикл
				...
				Если СтрСравнить(ДомВАдресе.НомерДома, ОписаниеДома.НомерДома) = 0 И СтрСравнить(
					...
					Если СтрСравнить(ДомВАдресе.ДополнительныйНомерДома1, ОписаниеДома.ДополнительныйНомерДома1) = 0
						...
						ОписаниеДома.Индекс = ИнформацияОДомах.Индекс;
...
Функция КаскадныйСписокАвтоподбораНаселенногоПункта(ТипАдреса)
	...
	|	АдресныеОбъекты.КодСубъектаРФ КАК КодСубъектаРФ,
	|	ДополнительныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс
	|ПОМЕСТИТЬ АдресныеОбъектыНачалоФразы
	|ИЗ
	|	Приоритеты КАК Приоритеты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	...
	|	АдресныеОбъекты.КодСубъектаРФ КАК КодСубъектаРФ,
	|	ДополнительныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс
	|ПОМЕСТИТЬ АдресныеОбъектыНачалоФразыСПробелом
	|ИЗ
	|	Приоритеты КАК Приоритеты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
	|		ПО Приоритеты.Уровень = АдресныеОбъекты.Уровень
	...
	|ГДЕ
	|	АдресныеОбъекты.Уровень < 8
	|	И АдресныеОбъекты.Наименование ПОДОБНО &НачалоФразыСПробелом СПЕЦСИМВОЛ ""~""
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АдресныеОбъектыНачалоФразы.Порядок КАК Порядок,
	|	АдресныеОбъектыНачалоФразы.ТипОбъекта КАК ТипОбъекта,
	|	АдресныеОбъектыНачалоФразы.Наименование КАК Наименование,
	|	АдресныеОбъектыНачалоФразы.Идентификатор КАК Идентификатор,
	|	АдресныеОбъектыНачалоФразы.Уровень КАК Уровень,
	|	АдресныеОбъектыНачалоФразы.КодСубъектаРФ КАК КодСубъектаРФ,
	|	АдресныеОбъектыНачалоФразы.ПочтовыйИндекс КАК ПочтовыйИндекс
	|ПОМЕСТИТЬ АдресныйОбъект
	|ИЗ
	|	АдресныеОбъектыНачалоФразы КАК АдресныеОбъектыНачалоФразы
	|  
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	АдресныеОбъектыНачалоФразыСПробелом.Порядок,
	|	АдресныеОбъектыНачалоФразыСПробелом.ТипОбъекта,
	|	АдресныеОбъектыНачалоФразыСПробелом.Наименование,
	|	АдресныеОбъектыНачалоФразыСПробелом.Идентификатор,
	|	АдресныеОбъектыНачалоФразыСПробелом.Уровень,
	|	АдресныеОбъектыНачалоФразыСПробелом.КодСубъектаРФ,
	|	АдресныеОбъектыНачалоФразыСПробелом.ПочтовыйИндекс
	|ИЗ
	|	АдресныеОбъектыНачалоФразыСПробелом КАК АдресныеОбъектыНачалоФразыСПробелом
	|
...
Процедура ЗаполнитьАдресаДляВыбораПоЗагруженнымДанным(Результат, Родитель, Уровень, ТипАдреса, ДополнительныеПараметры)
	...
	Если ЭтоУровеньУлицы(Уровень) Тогда
		ЗаполнитьУлицыАдресаДляВыбораПоЗагруженнымДанным(Результат, Родитель, ТипАдреса);
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из РезультатЗапроса.Выгрузить() Цикл
		...
		Если Не ЭтоУровеньРегиона(Уровень) И ЗначениеЗаполнено(СтрокаТаблицы.ПочтовыйИндекс) Тогда
			СтрокаТаблицы.Представление = СтрокаТаблицы.Представление + " (" + Формат(СтрокаТаблицы.ПочтовыйИндекс, "ЧГ=0") + ")";
		КонецЕсли;
...
Процедура ЗаполнитьУлицыАдресаДляВыбораПоЗагруженнымДанным(Результат, Родитель, ТипАдреса)

	ПараметраПодбора = Новый Структура();
	ПараметраПодбора.Вставить("Идентификатор", Родитель);
	ПараметраПодбора.Вставить("ТипАдреса",     ТипАдреса);
	РезультатПодбора = ПодборУлиц(ПараметраПодбора);
	
	Если РезультатПодбора.Отказ Тогда
		Возврат;
	КонецЕсли;
	
	Данные = Результат.Данные;
	ЭтоМуниципальный = ЭтоМуниципальныйАдрес(ТипАдреса);
	Для Каждого СтрокаТаблицы Из РезультатПодбора.Данные Цикл
	
		СтрокаДанных = Данные.Добавить();
		СтрокаДанных.Идентификатор    = СтрокаТаблицы.Значение.Идентификатор;
		СтрокаДанных.Представление    = СтрокаТаблицы.Представление;
		СтрокаДанных.РегионЗагружен   = СтрокаТаблицы.Значение.ЗагруженныеДанные;
		СтрокаДанных.Уровень          = СтрокаТаблицы.Значение.Уровень;
		СтрокаДанных.ЭтоМуниципальный = ЭтоМуниципальный;
...
Функция ИзвлечьФайлыВКаталог(ОписаниеФайлов, Знач КаталогФайлов = Неопределено, ПроверятьНаличиеФайлаСПомещениями = Истина) Экспорт
	...
	Если ТипЗнч(ОписаниеФайлов) <> Тип("Массив") Тогда
		...
		ХранилищеПараметров = Новый ХранилищеЗначения(ПараметрыАдресногоКлассификатора, Новый СжатиеДанных(9));
	...
	Для Каждого ОписаниеФайла Из ОписаниеФайлов Цикл
		...
		Если ФайлАрхива.Существует() И ВРег(Прав(ПолноеИмяФайла, 4)) = ".ZIP" Тогда
			...
			Для Каждого ЭлементАрхива Из Архив.Элементы Цикл
				...
				Если ПроверятьНаличиеФайлаСПомещениями И СтрНайти(ЭлементАрхива.Имя, "APARTMENT.FI") > 0 Тогда
					ЕстьФайлСПомещениями = Истина;
				КонецЕсли;
				
			КонецЦикла;
			
			Если ПроверятьНаличиеФайлаСПомещениями И Не ЕстьФайлСПомещениями Тогда
				Архив.Закрыть();
				
				Результат.ПроизошлаОшибка = Истина;
				Результат.ТекстОшибки = СтроковыеФункции.ФорматированнаяСтрока(
					НСтр("ru = 'Не удалось выполнить загрузку адресных сведений - отсутствует обязательный файл с помещениями.
					|Скачайте актуальную версию файла адресного классификатора с сайта
					| <a href=%1>%1</a>'"), "https://its.1c.ru/download/gar");
				
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось загрузить адресные сведения из файла: %1
						|по причине: %2'"),
					ПолноеИмяФайла, Результат.ТекстОшибки));
			...
			ИмяКаталога = ФайлАрхива.ИмяБезРасширения;
			Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ФайлАрхива.ИмяБезРасширения) Тогда
				ИмяКаталога = Лев(ИмяКаталога, 2);
			КонецЕсли;
			Архив.ИзвлечьВсе(КаталогФайлов + ИмяКаталога,
	...
	Результат.КаталогФайлов = КаталогФайлов;
	Возврат Результат;
...
//    * АдресЧастичноКорректный - Булево
//    * Варианты - Массив
//    * Ошибки - Массив
//
Функция РезультатПроверкиАдреса() Экспорт
	...
	Результат.Вставить("АдресЧастичноКорректный", Ложь);
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		Для Каждого УровеньАдреса Из ИменаУровнейАдреса Цикл
			Если Адрес.Свойство(УровеньАдреса) И ЗначениеЗаполнено(Адрес[УровеньАдреса]) И НомерПозиции < 6 Тогда
				...
				Наименование = Строка(Адрес[УровеньАдреса]);
				ТипОбъекта   = Строка(Адрес[УровеньАдреса + "Type"]);
				...
				Если ЗначениеЗаполнено(ТипОбъекта) Тогда
					ТипОбъекта = СтрЗаменитьПоРегулярномуВыражению(ТипОбъекта, "\.$", "");
				КонецЕсли;
...
Функция ТекстЗапросаПроверкиАдресов()
	...
	|			И (АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта0 ИЛИ АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта0 + ""."")
	|			И (АдресныеОбъекты.Уровень = 1)
	...
	|			И (АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта1 ИЛИ АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта1 + ""."")
	|			И (АдресныеОбъекты.КодСубъектаРФ В (&КодыРегионов))
	...
	|			И (АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта2 ИЛИ АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта2 + ""."")
	|			И (АдресныеОбъекты.КодСубъектаРФ В (&КодыРегионов))
	...
	|			И (АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта3 ИЛИ АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта3 + ""."")
	|			И (АдресныеОбъекты.КодСубъектаРФ В (&КодыРегионов))
	...
	|			И (АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта4 ИЛИ АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта4 + ""."")
	|			И (АдресныеОбъекты.КодСубъектаРФ В (&КодыРегионов))
	...
	|			И (АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта5 ИЛИ АдресныеОбъекты.ТипОбъекта = АдресаПоПолям.ТипОбъекта5 + ""."")
	|			И (АдресныеОбъекты.КодСубъектаРФ В (&КодыРегионов))
	...
	|	Сведения.ПочтовыйИндекс КАК Индекс,
						...
						Если СтрСравнить(ДомВАдресе.НаименованиеДома, ОписаниеДома.НаименованиеДома) = 0 Тогда
							...
							Если СтрСравнить(ДомВАдресе.ДополнительныйНомерДома1,
							   ...
							   И СтрСравнить(ДомВАдресе.НаименованиеДополнительногоДома2,
								...
								ПочтовыйИндекс = ИнформацияОДомах.Индекс;
								Прервать;
							Иначе
...
Функция СведенияПоСпискуОКТМО(СписокОКТМО) Экспорт
	...
	ТипыКлючаЗаписиОКТМО = Новый ОписаниеТипов(ДопустимыеТипы);
	
	ТаблицаОКТМО = Новый ТаблицаЗначений;
	ТаблицаОКТМО.Колонки.Добавить("ОКТМО", ТипЧисло);
	ТаблицаОКТМО.Колонки.Добавить("КлючЗаписиОКТМО", ТипыКлючаЗаписиОКТМО);
	...
	Для Каждого ЗаписьОКТМО Из СписокОКТМО Цикл
		...
		Если ТипЗнч(ЗаписьОКТМО.Значение) = Тип("Число") Тогда
			ОКТМОСтрокой = Формат(ЗаписьОКТМО.Значение, "ЧГ=0;");
			Если СтрДлина(ОКТМОСтрокой) = 7 Или СтрДлина(ОКТМОСтрокой) = 10 Тогда
				ОКТМОСтрокой = "0" + ОКТМОСтрокой;
			КонецЕсли;
		Иначе
			ОКТМОСтрокой = Строка(ЗаписьОКТМО.Значение);
		КонецЕсли;
		
		Если СтрНачинаетсяС(ОКТМОСтрокой, "118") Или СтрНачинаетсяС(ОКТМОСтрокой, "718")
			Или СтрНачинаетсяС(ОКТМОСтрокой, "719") Тогда
			КодОКТМОРегиона = Лев(ОКТМОСтрокой, 3);
		Иначе
			КодОКТМОРегиона = Лев(ОКТМОСтрокой, 2);
		КонецЕсли;
	...
	Если ТаблицаОКТМО.Количество() > 0 Тогда
		СведенияПоСпискуОКТМОЗагруженныеДанные(ТаблицаОКТМО, Результат);
	КонецЕсли;
	
	Если ТаблицаОКТМОВебСервис.Количество() > 0 Тогда
		СведенияПоСпискуОКТМОВебСервис(ТаблицаОКТМОВебСервис, Результат);
	КонецЕсли;
...
Функция СведенияПоСпискуОКТМОЗагруженныеДанные(ТаблицаОКТМО, Результат)
...
Процедура СведенияПоСпискуОКТМОВебСервис(СписокОКТМО, Результат)
...
Функция ПроверитьОКТМОБюджетополучателя(ОКТМОБюджетополучателей) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("АдресныйКлассификаторСлужебный.ПроверитьОКТМОБюджетополучателя",
		"ОКТМОБюджетополучателей", ОКТМОБюджетополучателей, Тип("Массив"));
		
	РазмерПорции = 100;
	Результат    = НовыйРезультатИзВебСервиса();
	
	Порция = Новый Массив;

	Для Позиция = 0 По ОКТМОБюджетополучателей.ВГраница() Цикл
		
		ОКТМОБюджетополучателя = СокрЛП(ОКТМОБюджетополучателей.Получить(Позиция));
		Если СтрДлина(ОКТМОБюджетополучателя) = 8 Тогда
			
			Если Результат.Данные[ОКТМОБюджетополучателя] = Неопределено Тогда
				Порция.Добавить(ОКТМОБюджетополучателя);
			КонецЕсли;
		Иначе
			
			ДанныеПоОКТМО             = ДанныеПоОКТМО();
			ДанныеПоОКТМО.ТекстОшибки = НСтр("ru = 'Недопустимая длина кода ОКТМО бюджетополучателя. Требуется 8 цифр.'");
			Результат.Данные.Вставить(ОКТМОБюджетополучателя, ДанныеПоОКТМО);
			
		КонецЕсли;
		
		Если (Порция.Количество() = РазмерПорции 
		 Или Позиция = ОКТМОБюджетополучателей.ВГраница()) 
		   И Порция.Количество() > 0 Тогда
				ОбработатьПорциюОКТМОБюджетополучателей(Результат, Порция);
...
Функция ДанныеПоОКТМО()
	
	ДанныеПоОКТМО = Новый Структура();
	ДанныеПоОКТМО.Вставить("Успешно",                  Ложь);
	ДанныеПоОКТМО.Вставить("ТекстОшибки",              "");
	ДанныеПоОКТМО.Вставить("МуниципальныеОбразования", Новый Массив);
	
	Возврат ДанныеПоОКТМО;
	
КонецФункции

Функция ОбработатьПорциюОКТМОБюджетополучателей(Результат, Порция)
	
	ИдентификаторЗапроса = Строка(Новый УникальныйИдентификатор);
	ДанныеДляЗапроса = Новый Структура("oktmoList", Порция);
	
	ОписаниеСервиса = АдресныйКлассификаторПовтИсп.СервисКлассификатора1С();
	Сервис = ОписаниеСервиса.СервисКлассификатора;
	
	ПрефиксВерсииЗапроса = ПрефиксВерсииЗапросаСервисаОКТМО();
	HTTPЗапрос = Новый HTTPЗапрос(ПрефиксВерсииЗапроса + "validateOktmoList");
	...
	Исключение
		...
		ОбработатьОшибкуОбращенияКВебСервисуПриПолученииОКТМО(Результат);
		Возврат Результат;
	КонецПопытки;
	
	Если ТребуетсяОбновитьТикетАвторизации(РезультатПроверки.КодСостояния) Тогда
		
		Сервис = ОбновитьОписаниеСервисаКлассификатора(ОписаниеСервиса);
		
		Попытка
			Ответ = Сервис.ОтправитьДляОбработки(HTTPЗапрос);
		Исключение
	...
	Если РезультатПроверки.КодСостояния = 200 Тогда
		...
		ДанныеПроверки = СтрокуJSONВСоответствие(ПолученныеДанные);
		Если ТипЗнч(ДанныеПроверки) = Тип("Соответствие") Тогда
		
			НаборОКТМО = ДанныеПроверки["validateOktmoData"];
			Для каждого ДанныеОКТМО Из НаборОКТМО Цикл
				
				ЗначениеОКТМО = ДанныеОКТМО.Значение;
				ДанныеПоОКТМО = ДанныеПоОКТМО();
				
				Если ЗначениеОКТМО["validateOktmoStatus"] = "VERIFIED_OKTMO" Тогда
					ДанныеПоОКТМО.Успешно = Истина;
				КонецЕсли;
				Если ЗначениеЗаполнено(ЗначениеОКТМО["validateMessage"]) Тогда
					ДанныеПоОКТМО.ТекстОшибки = ЗначениеОКТМО["validateMessage"];
				КонецЕсли;
				Если ЗначениеЗаполнено(ЗначениеОКТМО["oktmoMunValue"]) Тогда
					ДанныеПоОКТМО.МуниципальныеОбразования = ЗначениеОКТМО["oktmoMunValue"];
				КонецЕсли;
				
				Результат.Данные.Вставить(ДанныеОКТМО.Ключ, ДанныеПоОКТМО)
			КонецЦикла;
	...
	Иначе
		...
		Результат.ПредставлениеОшибкиДляПользователя  = НСтр("ru = 'Проверка ОКТМО бюджетополучателя недоступна по причине:
						...
						|  антивирус или другие приложения;
						|- веб-сервис отключен или на техническом обслуживании.'");
		
		Результат.Отказ = Истина;
		ТекстОшибки     = НСтр("ru = 'При проверке ОКТМО бюджетополучателя через веб-сервис возникли проблемы.'");
	
		ЗаписатьОшибкуЗапросаКВебСервисуВЖурналРегистрации(ПолученныеДанные, ТекстОшибки, HTTPЗапрос.АдресРесурса
...
Процедура ОбработатьОшибкуОбращенияКВебСервисуПриПолученииОКТМО(Результат)
	
	Результат.Отказ              = Истина;
	Результат.ИнформацияОбОшибке = ИнформацияОбОшибке();
	Результат.ПредставлениеОшибкиДляПользователя = НСтр("ru = 'При проверке ОКТМО бюджетополучателя через веб-сервис возникли проблемы.'");
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(Результат.ИнформацияОбОшибке));
		
КонецПроцедуры

Функция ОбновитьОписаниеСервисаКлассификатора(ОписаниеСервиса)
	
	ОписаниеСервиса.СервисКлассификатора = СервисКлассификатора1С();
	Сервис = ОписаниеСервиса.СервисКлассификатора;
	Возврат Сервис;
	
КонецФункции

Функция ТребуетсяОбновитьТикетАвторизации(КодСостояния)
	
	Возврат КодСостояния = 401
		Или КодСостояния = 409
		Или КодСостояния = 419;
...
Функция ИменаУровнейАдресаПоКодуУровня()
	...
	Уровни.Вставить(8,  "Улица");
	Уровни.Вставить(9,  "ЗемельныйУчасток");
	Уровни.Вставить(10, "Здание");
	Уровни.Вставить(11, "Помещение");
	Уровни.Вставить(12, "МашиноМесто");
	
	Возврат Уровни;
	
КонецФункции

Функция КодУровняПоИмениУровня()
	
	Уровни = Новый Соответствие;
	
	Уровни.Вставить(ВРег("Регион"), 1);
	Уровни.Вставить(ВРег("Район"), 2);
	Уровни.Вставить(ВРег("МуниципальныйРайон"), 3);
	Уровни.Вставить(ВРег("Поселение"), 4);
	Уровни.Вставить(ВРег("Город"), 5);
	Уровни.Вставить(ВРег("НаселенныйПункт"), 6);
	Уровни.Вставить(ВРег("Территория"), 7);
	Уровни.Вставить(ВРег("Улица"), 8);
	Уровни.Вставить(ВРег("ЗемельныйУчасток"), 9);
	Уровни.Вставить(ВРег("Здание"), 10);
	Уровни.Вставить(ВРег("Помещение"), 11);
	Уровни.Вставить(ВРег("МашиноМесто"), 17);
...
// 
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораСервис1С(Результат, Индекс, ПодстрокаПоиска)
	
	ТекстGETЗапроса = "selectByCode?code=%ZipCode%&text=%Text%&codeType=POSTAL_CODE";

	ТекстGETЗапроса = СтрЗаменить(ТекстGETЗапроса, "%ZipCode%", Формат(Индекс, "ЧГ=0"));
	ТекстGETЗапроса = СтрЗаменить(ТекстGETЗапроса, "%Text%", ПодстрокаПоиска);
...
Процедура ЗаполнитьВерсиюПоставщикаДанныхСервис1С(Результат)
	...
	РезультатВебСервис = ВыполнитьЗапросЧерезВебСервис("ping", 5);

	Если РезультатВебСервис.Отказ Тогда
		Результат.Данные = Ложь;
		Результат.Отказ = Истина;
		Если ЗначениеЗаполнено(РезультатВебСервис.ПредставлениеОшибкиДляПользователя) Тогда
			Результат.КраткоеПредставлениеОшибки = РезультатВебСервис.ПредставлениеОшибкиДляПользователя;
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(ВариантыАдреса, КешПредставлений, Текст, Родитель, Количество,
	...
	Если Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(Текст) Тогда
		...
		РезультатВебСервис = ВыполнитьЗапросЧерезВебСервис(ТекстGETЗапроса);
	Иначе
		РезультатВебСервис = АдресныеСведенияСервис1СПоКоду(Текст, "ID");
	КонецЕсли;
	...
	Для Каждого Данные Из РезультатВебСервис.Данные Цикл
		...
		Если ВариантыАдреса.Количество() >= 20 Тогда
			Возврат;
		КонецЕсли;
		ПредставлениеТерриториальное = ?(ДополнительныеПараметры.ПрямойПорядок, Данные.value, Данные.valueReversed);
		ПредставлениеМуниципальное   = ?(ДополнительныеПараметры.ПрямойПорядок, Данные.munValue, Данные.munValueReversed);
...
Функция СведенияОПодбираемомАдресе(Знач Представление, Знач Идентификатор, Знач Муниципальный, Знач ЗагруженныеДанные,
	...
	Сведения.Вставить("Родитель", Неопределено);
	Возврат Сведения;
...
Процедура ЗаполнитьАктуальныеАдресныеСведенияСервис1С(Результат, СведенияОбАдресномОбъекте, ЭтоМуниципальныйАдрес = Истина)
	...
	РезультатВебСервис = АдресныеСведенияСервис1СПоКоду(Идентификатор, "ID");
	...
	Иначе
		...
		Для Каждого АдресВебСервиса Из РезультатВебСервис.Данные Цикл
			Если СведенияОбАдресномОбъекте.Родитель = АдресВебСервиса.munDistrictId Тогда
				ВыбранныйАдрес = АдресВебСервиса;
				Прервать;
			ИначеЕсли АдресВебСервиса.munValueReversed = СведенияОбАдресномОбъекте.Представление Тогда
				ВыбранныйАдрес = АдресВебСервиса;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция АдресныеСведенияСервис1СПоКоду(Знач Идентификатор, Знач ИмяКода)
...
Функция ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Знач АдресаДляПроверки,
	...
	ОписаниеСервиса = АдресныйКлассификаторПовтИсп.СервисКлассификатора1С(20);
	Сервис = ОписаниеСервиса.СервисКлассификатора;
	...
	Иначе
		...
		ЗаписатьОшибкуЗапросаКВебСервисуВЖурналРегистрации(ПолученныеДанные, Результат.ПодробноеПредставлениеОшибки, HTTPЗапрос.АдресРесурса
			+ " X-1C-Request-UID:" + ИдентификаторЗапроса, РезультатПроверки.КодСостояния);
	...
	Для Каждого Проверка Из ДанныеПроверки Цикл
		...
		Если ЭлементПроверки.Свойство("status") Тогда
			...
			РезультатПроверки.АдресЧастичноКорректный = (СтрСравнить(ЭлементПроверки.status, "PARTIALLY_VERIFIED") = 0);
...
//   * Данные - Соответствие
//   * Отказ - Булево
//   * ПредставлениеОшибкиДляПользователя - Строка
//   * ИнформацияОбОшибке - ИнформацияОбОшибке
//                        - Неопределено
//
Функция НовыйРезультатИзВебСервиса()

	Результат = Новый Структура;
	Результат.Вставить("Данные", Новый Соответствие);
	Результат.Вставить("Отказ", Ложь);
	Результат.Вставить("ПредставлениеОшибкиДляПользователя", "");
...
Функция КонструкторКодовАдреса()
	...
	Коды.Вставить("ИдентификаторДома",      "");
	Коды.Вставить("ИдентификаторПомещения", "");
	Коды.Вставить("ОКАТО",                  "");
	Коды.Вставить("ОКТМО",                  "");
...
//       * КраткоеПредставлениеОшибки   - Строка - описание ошибки, если поставщик недоступен. Неопределено, если Отказ
//                                                 = Ложь.
//       * Данные                       - Неопределено, Произвольный - данные получение из веб-сервиса.
//
...
Процедура ЗаполнитьПоляВСоответствииСУровнемАдреса(Адрес, Выборка, Суффикс,
	...
	Если ЗначениеЗаполнено(Выборка["Наименование" + Суффикс]) Тогда
		...
		Если Муниципальный Тогда
			ПереопределитьМуниципальныйУровеньГородаКакНаселенныйПунктПоГАР(ИмяУровня, Выборка, Суффикс);
		КонецЕсли;
...
Процедура ПереопределитьМуниципальныйУровеньГородаКакНаселенныйПунктПоГАР(ИмяУровня, Выборка, Суффикс)

	ТипОбъекта = ВРег(Выборка["ТипОбъекта" + Суффикс]);

	Если СтрСравнить(ИмяУровня, "city") = 0 
	 Или (СтрСравнить(ИмяУровня,"district") = 0 И (ТипОбъекта = "Г" Или ТипОбъекта = "Г.")) Тогда
		ИмяУровня = "locality";
	КонецЕсли;
...
Функция ОбработатьОбщепринятыеСокращенияИУдалитьТочкуВКонецТипа(ИсходнаяТаблица)
	
	АдресПоЧастям = ИсходнаяТаблица.Скопировать();
	АдресПоЧастям.Колонки.Добавить("ТипОбъекта2", ОбщегоНазначения.ОписаниеТипаСтрока(128));
	
	ОбщепринятыеСокращения = ОбщепринятыеСокращенияАдресныхЭлементовИОбъектов();
	ОбщепринятыеСокращенияЭлементов = ОбщепринятыеСокращенияАдресныхЭлементов();
	
	Фильтр = Новый Структура("Уровень", 0);
	НераспознанныеСтроки = АдресПоЧастям.НайтиСтроки(Фильтр);
	
	Для Каждого ЧастьАдреса Из НераспознанныеСтроки Цикл
		
		ЧастьАдреса.ТипОбъекта = СтрЗаменитьПоРегулярномуВыражению(ЧастьАдреса.ТипОбъекта, "\.$", "");
		ТипВРег = ВРЕГ(СокрЛП(ЧастьАдреса.ТипОбъекта));
		
		// Города без типа → определяем автоматически
		Если (ВРЕГ(ЧастьАдреса.Наименование) = "МОСКВА" 
			ИЛИ ВРЕГ(ЧастьАдреса.Наименование) = "САНКТ-ПЕТЕРБУРГ")
			И ПустаяСтрока(ТипВРег) Тогда
			
			ЧастьАдреса.ТипОбъекта = "г";
			ЧастьАдреса.Значение = СоединитьНаименованиеИТипАдресногоОбъекта(
				ЧастьАдреса.Наименование, "г", Истина);
			Продолжить;
		КонецЕсли;
		
		Если ПустаяСтрока(ТипВРег) Тогда
			Продолжить;
		КонецЕсли;
		
		Сокращение = ОбщепринятыеСокращенияЭлементов[ТипВРег];
		Если Сокращение <> Неопределено Тогда
			ЧастьАдреса.ТипОбъекта2 = Сокращение;
		КонецЕсли;
		
		// Замена сокращения
		ПолноеИмя = ОбщепринятыеСокращения[ТипВРег];
		Если ПолноеИмя <> Неопределено Тогда
			ЧастьАдреса.ТипОбъекта = ПолноеИмя;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат АдресПоЧастям;
	
КонецФункции

Функция ОбщепринятыеСокращенияАдресныхЭлементовИОбъектов()
	
	СтатическиеСокращения = Новый Соответствие;
	
	// Адресные элементы
	СтатическиеСокращения.Вставить("ЛИНИЯ",  "Лн.");
	
	// Дома, здания, помещения, офисы, квартиры
	СтатическиеСокращения.Вставить("КОРП",   "Корпус");
	СтатическиеСокращения.Вставить("К",      "Корпус");
	СтатическиеСокращения.Вставить("КВ",     "Квартира");
	СтатическиеСокращения.Вставить("КВАРТ",  "Квартира");
	СтатическиеСокращения.Вставить("ОФ",     "Офис");
	СтатическиеСокращения.Вставить("ПОМЕЩ",  "Помещение");
	СтатическиеСокращения.Вставить("ПОМ",    "Помещение");
	СтатическиеСокращения.Вставить("Д",      "Дом");
	СтатическиеСокращения.Вставить("СТР",    "Строение");
	СтатическиеСокращения.Вставить("СТРОЕН", "Строение");
	СтатическиеСокращения.Вставить("ЛИТЕР",  "Литера");
	
	Возврат СтатическиеСокращения;
	
КонецФункции

Функция ОбщепринятыеСокращенияАдресныхЭлементов()
	
	СтатическиеСокращения = Новый Соответствие;
	
	СтатическиеСокращения = СокращенияАдресныхОбъектов();
	СтатическиеСокращения.Вставить("ЛИНИЯ",  "Лн.");
	СтатическиеСокращения.Вставить("ПРОЕЗД", "пр-д");
...
Функция ИдентификаторЧасовыеЗоныСубъектовРФ() Экспорт
	Возврат "TimeZones";
КонецФункции
...
Функция ПрефиксВерсииЗапроса()
	Возврат "rest/v5/";
КонецФункции

Функция ПрефиксВерсииЗапросаСервисаОКТМО()
	Возврат "rest/oktmo/v1/";
КонецФункции
...
// Извлекает список помещений полученного из регистра сведения описания и добавляет их в список значений.
//
// Параметры:
//  СтрокаСоСпискомПомещений - Строка - описание списка помещений из регистра сведения.
//  СтрокаПоиска             - Строка - строка отбора.
//  ВариантыПомещений        - СписокЗначений - список помещений.
//  ТочноеСовпадение         - Булево - флаг точного совпадения.
//
Процедура ИзвлечьСписокПомещений(СтрокаСоСпискомПомещений, СтрокаПоиска, ВариантыПомещений, ТочноеСовпадение)
	
	МассивПомещений = СтрРазделить(СтрокаСоСпискомПомещений, Символы.ПС, Ложь);
	
	ТипЧисло = Новый ОписаниеТипов("Число");
	НаименованиеПомещений = АдресныйКлассификаторПовтИсп.НаименованияПомещений();
	
	ПодборВариантовПомещений = ЗначениеЗаполнено(СтрокаПоиска);
	СтрокаПоискаВРег = ?(ПодборВариантовПомещений И Не ТочноеСовпадение, ВРег(СтрокаПоиска), "");
	
	Для Каждого Помещение Из МассивПомещений Цикл
	
		ДанныеПомещения = Сред(Помещение, 25);
		ТипИНомерПомещения = СтрРазделить(ДанныеПомещения, Символы.НПП);
		НомерПомещения = ТипИНомерПомещения[1];
		
		ТипПомещения = ТипЧисло.ПривестиЗначение(ТипИНомерПомещения[0]);
		ТипПомещенияПредставление = НаименованиеПомещений.Получить(ТипПомещения);
		ПредставлениеПомещения = ТипПомещенияПредставление + " " + НомерПомещения;
		
		ДобавитьВСписок = Ложь;
		
		Если Не ПодборВариантовПомещений Тогда
			ДобавитьВСписок = Истина;
		ИначеЕсли ТочноеСовпадение Тогда
			ДобавитьВСписок = (СтрСравнить(ПредставлениеПомещения, СтрокаПоиска) = 0);
		Иначе
			ДобавитьВСписок = СтрНайти(ВРег(ПредставлениеПомещения), СтрокаПоискаВРег) > 0;
		КонецЕсли;
		
		Если ДобавитьВСписок Тогда
			
			ОписаниеПомещения = ОписаниеПомещения();
			ОписаниеПомещения.ИдентификаторПомещения = УникальныйИдентификаторИзСтроки64(Лев(Помещение, 24));
	
			ОписаниеПомещения.ТипПомещения    = ТипПомещенияПредставление;
			ОписаниеПомещения.НомерПомещения  = НомерПомещения;
			ОписаниеПомещения.ПомещениеЧислом = ТипЧисло.ПривестиЗначение(НомерПомещения);
			ОписаниеПомещения.Представление   = ПредставлениеПомещения;
			
			ПомещениеДанные = ВариантыПомещений.Добавить();
			ЗаполнитьЗначенияСвойств(ПомещениеДанные, ОписаниеПомещения);
			ПомещениеДанные.Значение = ОписаниеПомещения;
		КонецЕсли;
...
// Возвращаемое значение:
//  Структура:
//    * ПомещениеЧислом - Число
//    * НомерПомещения - Строка
//    * ТипПомещения - Строка
//    * Представление - Строка
//    * ИдентификаторПомещения - Неопределено, УникальныйИдентификатор
//
Функция ОписаниеПомещения() Экспорт

	ОписаниеПомещения = Новый Структура();

	ОписаниеПомещения.Вставить("ПомещениеЧислом",        0);
	ОписаниеПомещения.Вставить("НомерПомещения",         "");
	ОписаниеПомещения.Вставить("ТипПомещения",           "");
	ОписаниеПомещения.Вставить("Представление",          "");
	ОписаниеПомещения.Вставить("ИдентификаторПомещения", Неопределено);

	Возврат ОписаниеПомещения;
...
//     Строка - путь к файлу описания данных.
//
Функция АдресВебСервисаКонтактнойИнформации()
...
Функция ЗаполнитьКодыАдреса(Результат, ИдентификаторАдресногоОбъекта, СведенияОДоме)
	...
	Коды = КодыПоИдентификаторуАдресногоОбъектаДома(ИдентификаторАдресногоОбъекта, СведенияОДоме);
	Если Коды <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Коды);

		Для Каждого КлючЗначение Из Результат Цикл
			Если СтрНайти(КлючЗначение.Ключ, "Идентификатор") = 0 Тогда
...
Функция СервисКлассификатора1С(ВремяОжидания = 20) Экспорт
	...
	URLПорталаПоддержки = URLСтраницыПорталаПоддержки();
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Сервис интернет-поддержки пользователей не подключен.'");
	КонецЕсли;
	РезультатПолученияТикета = МодульИнтернетПоддержкаПользователей.ТикетАутентификацииНаПорталеПоддержки(URLПорталаПоддержки);
	...
	Если ЗначениеЗаполнено(РезультатПолученияТикета.КодОшибки) Тогда
		...
		ПарольПользователя = "";
	Иначе
		ИмяПользователя = "AUTH_TOKEN";
		ПарольПользователя = РезультатПолученияТикета.Тикет;
	КонецЕсли;
	
	СтруктураURIВебСервиса = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресВебСервисаКонтактнойИнформации());
	...
	Исключение
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
			...
			МодульПолучениеФайловИзИнтернета = ОбщегоНазначения.ОбщийМодуль("ПолучениеФайловИзИнтернета");
			РезультатДиагностики = МодульПолучениеФайловИзИнтернета.ДиагностикаСоединения(СсылкаНаРесурс,,Ложь);
...
Функция URLСтраницыПорталаПоддержки()
	Возврат "https://portal.1c.ru/";
КонецФункции
...
Функция ВыполнитьЗапросЧерезВебСервис(СтрокаЗапроса, ВремяОжидания = 60)
	...
	ОписаниеСервиса = АдресныйКлассификаторПовтИсп.СервисКлассификатора1С(ВремяОжидания);
	Сервис = ОписаниеСервиса.СервисКлассификатора;
	...
	Если ТребуетсяОбновитьТикетАвторизации(Ответ.КодСостояния) Тогда
	...
	Если Результат.КодСостояния = 200 Тогда
		...
		Если ПустаяСтрока(ПолученныеДанные) Тогда
			Результат.Отказ = Истина;
			Результат.ПредставлениеОшибкиДляПользователя = ТекстОшибкиПриРаботеССервисомДляПользователя();
			Результат.ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Веб-сервис адресов успешно выполнил запрос ""%1"", но вернул пустой результат.'"), СтрокаЗапроса);
			ЗаписатьОшибкуЗапросаКВебСервисуВЖурналРегистрации(ПолученныеДанные, Результат.ПодробноеПредставлениеОшибки, ТекстURL, Ответ.КодСостояния);
			Возврат Результат;
		КонецЕсли;
...
Функция ТекстОшибкиПриРаботеССервисомДляПользователя()
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ТекстОшибки = НСтр("ru = 'Сервис для подбора и проверки адресов временно недоступен.'");
	Иначе
		ТекстОшибки = НСтр("ru = 'Сервис для подбора и проверки адресов временно недоступен по причине:
			|- неполадки у интернет-провайдера;
			|- подключение к серверу блокирует межсетевой экран, антивирус или другие приложения;
...
Процедура ОбработатьОшибкуОбращенияКВебСервису(Результат, Знач Ответ, ИнформацияОбОшибке, ТекстURL)
	...
	Если ПустаяСтрока(ПредставлениеОшибки) Тогда
		ПредставлениеОшибки = НСтр("ru = 'Веб-сервис для работы с адресами недоступен по причине:
						|- не подключена интернет-поддержка пользователей;
						|- неполадки у интернет-провайдера;
						|- подключение к серверу блокирует межсетевой экран, 
						|  антивирус или другие приложения;
	...
	ЗаписатьОшибкуЗапросаКВебСервисуВЖурналРегистрации(ОтветОтСервера, Результат.ПодробноеПредставлениеОшибки, ТекстURL, Ответ.КодСостояния);
...
Процедура ЗаписатьОшибкуЗапросаКВебСервисуВЖурналРегистрации(ОтветОтСервераСтрокой, ПодробноеПредставлениеОшибки, ТекстURL, КодСостояния)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстОтветаСервера,
		ПодробноеПредставлениеОшибки, ТекстURL, КодСостояния, ОтветОтСервераСтрокой);
	...
	Если (СтрСравнить(Свойство, "buildings") = 0
	   ...
	   И ТипЗнч(Значение) = Тип("Массив") Тогда
		...
		Для Каждого ЗначениеМассива Из Значение Цикл
			Если ЗначениеМассива.Свойство("type") Тогда
				ЗначениеМассива.type = ТРег(ЗначениеМассива.type);
			Иначе
				ЗначениеМассива.Вставить("type", "");
			КонецЕсли; 
		КонецЦикла;
...
Функция СтрокуJSONВСоответствие(Значение)
...
Функция СписокПомещенийИзЗагруженныхСведений(Знач ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска,
	Знач ТочноеСовпадение)
	
	Помещения = Новый СписокЗначений();
	Помещения.Добавить(ИдентификаторАдресногоОбъекта, СтрокаПоиска, ТочноеСовпадение);
	
	Результат = ПомещенияАдресныхОбъектовИзЗагруженныхСведений(Помещения);
	
	Возврат Результат.Получить(ИдентификаторАдресногоОбъекта);
	
КонецФункции

Функция ПомещенияАдресныхОбъектовИзЗагруженныхСведений(Помещения)
	
	Результат = Новый Соответствие();
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	КвартирыОфисыПомещения.Помещения КАК Помещения,
	|	КвартирыОфисыПомещения.АдресныйОбъект КАК ИдентификаторАдресногоОбъекта
	|ИЗ
	|	РегистрСведений.КвартирыОфисыПомещения КАК КвартирыОфисыПомещения
	|ГДЕ
	|	КвартирыОфисыПомещения.АдресныйОбъект В (&АдресныйОбъект)";

	Запрос.УстановитьПараметр("АдресныйОбъект", Помещения.ВыгрузитьЗначения());
	Запись = Запрос.Выполнить().Выбрать();

	Пока Запись.Следующий() Цикл
		ВариантыПомещений = КонструкторСпискаПомещений();
		
		Если ЗначениеЗаполнено(Запись.Помещения) Тогда
			Описание = Запись.Помещения.Получить();
			Если ЗначениеЗаполнено(Описание) Тогда
				СведенияОПомещении = Помещения.НайтиПоЗначению(Запись.ИдентификаторАдресногоОбъекта);
				Если СведенияОПомещении <> Неопределено Тогда
					ИзвлечьСписокПомещений(Описание, СведенияОПомещении.Представление, ВариантыПомещений, СведенияОПомещении.Пометка);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Результат.Вставить(Запись.ИдентификаторАдресногоОбъекта, ВариантыПомещений);
	КонецЦикла;
...
//   * Значение - Структура
//   * Представление - Строка
//   * ИдентификаторПомещения - Строка
//
Функция КонструкторСпискаПомещений() Экспорт

	ВариантыПомещений = Новый ТаблицаЗначений;
	ВариантыПомещений.Колонки.Добавить("ТипПомещения", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	ВариантыПомещений.Колонки.Добавить("ПомещениеЧислом", ОбщегоНазначения.ОписаниеТипаЧисло(15));
	ВариантыПомещений.Колонки.Добавить("НомерПомещения", ОбщегоНазначения.ОписаниеТипаСтрока(30));
	ВариантыПомещений.Колонки.Добавить("Значение");
	ВариантыПомещений.Колонки.Добавить("Представление", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	ВариантыПомещений.Колонки.Добавить("ИдентификаторПомещения", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	
	ВариантыПомещений.Индексы.Добавить("ТипПомещения, ПомещениеЧислом");
...
Функция ЗданияИПомещенияАдреса(XDTOАдрес, НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение <> Неопределено Тогда
		...
		Если НовоеЗначение.Свойство("Здания") Тогда
			Для Каждого СтрокаТаблицы Из НовоеЗначение.Здания Цикл
				ВставитьЗданиеПомещение(XDTOАдрес, СтрокаТаблицы.Тип, СтрокаТаблицы.Значение);
			КонецЦикла;
		КонецЕсли;
		Если НовоеЗначение.Свойство("Помещения") Тогда
			Для Каждого СтрокаТаблицы Из НовоеЗначение.Помещения Цикл
				ВставитьЗданиеПомещение(XDTOАдрес, СтрокаТаблицы.Тип, СтрокаТаблицы.Значение);
			КонецЦикла;
		КонецЕсли;
...
Функция НазванияГородовФедеральногоЗначения()
	...
	Для Каждого Город Из СписокГородов Цикл
		...
		Результат.Вставить(СоединитьНаименованиеИТипАдресногоОбъекта(Город, "Г", Ложь), Истина);
		Результат.Вставить(СоединитьНаименованиеИТипАдресногоОбъекта("ГОРОД", Город, Ложь), Истина);
		Результат.Вставить(СоединитьНаименованиеИТипАдресногоОбъекта(Город, "Г.", Истина), Истина);
		Результат.Вставить(СоединитьНаименованиеИТипАдресногоОбъекта(Город, "Г.", Ложь), Истина);
...
Процедура ЗагрузкаРегионаАдресногоКлассификатораССайта(ОписаниеСубъектаРФ, ПараметрыЗагрузки) Экспорт
	...
	Расширение = ".ZIP";
	Если ПараметрыЗагрузки.ЗагружатьПомещения Тогда
		Позиция = СтрНайти(ВРег(ОписаниеРегиона.Адрес), Расширение);
		Если Позиция > 0 Тогда
			ОписаниеРегиона.Адрес = Лев(ОписаниеРегиона.Адрес, Позиция -1) + "apt" + Прав(ОписаниеРегиона.Адрес, СтрДлина(Расширение));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьДанныеКлассификатораАдресовИзФайла(СубъектРФ, ОписаниеФайлов, ПараметрыЗагрузки) Экспорт
	...
	Иначе
		Результат = ИзвлечьФайлыВКаталог(ОписаниеФайлов, ПараметрыЗагрузки.КаталогФайлов, ПараметрыЗагрузки.ЗагружатьПомещения);
		Если Результат.ПроизошлаОшибка Тогда
			ВызватьИсключение Результат.ТекстОшибки;
		КонецЕсли;
		КаталогФайлов = Результат.КаталогФайлов;
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторАдресовПоставляемыеДанные(КодыСубъектовРФ, ОписаниеФайлов, ПараметрыЗагрузки)
	...
	Иначе
		Результат = ИзвлечьФайлыВКаталог(ОписаниеФайлов,, ПараметрыЗагрузки.ЗагружатьПомещения);
		Если Результат.ПроизошлаОшибка Тогда
			ВызватьИсключение Результат.ТекстОшибки;
		КонецЕсли;
		КаталогФайлов = Результат.КаталогФайлов;
...
Функция ПараметрыЗагрузкиКлассификатораАдресов() Экспорт
	...
	Результат.Вставить("КаталогФайлов", "");
	Результат.Вставить("ЗагружатьПомещения", Ложь);
...
Процедура ЗагрузитьАдресныеСведенияПоРегиону(Знач СубъектРФ, Знач КаталогФайлов, ПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки.ЗагружатьПомещения Тогда
		ЗагрузитьПомещения(СубъектРФ, КаталогФайлов, ЗагружатьОднимНабором);
	КонецЕсли;
...
Процедура ЗагрузитьПомещения(КодСубъектаРФ, КаталогФайлов, ЗагружатьОднимНабором)

	Сжатие = Новый СжатиеДанных(9);
	ФайлЗагрузки = ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, "APARTMENT", "Apartments");

	ТекущаяЗапись = ФайлЗагрузки.ФайлЧтения;

	Набор = РегистрыСведений.КвартирыОфисыПомещения.СоздатьНаборЗаписей();
	Набор.Отбор.КодСубъектаРФ.Установить(КодСубъектаРФ);
	...
	Попытка
		...
		Пока ТекущаяЗапись.Прочитать() Цикл
			...
			Запись.АдресныйОбъект = УникальныйИдентификаторИзСтроки64(
				ТекущаяЗапись.ЗначениеАтрибута("PARENTOBJECTGUID"));
			Запись.Помещения = Новый ХранилищеЗначения(ТекущаяЗапись.ЗначениеАтрибута("APARTMENTS"), Сжатие);
...
Процедура ОчиститьПомещения(КодСубъектаРФ)
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьНачальноеЗаполнение");
	Методы.Вставить("ДоступныеВерсииАдресныхСведений", Истина);
	Методы.Вставить("ЗагрузкаРегионаАдресногоКлассификатораССайта", Истина);
	Методы.Вставить("ЗагрузитьДанныеКлассификатораАдресовИзФайла", Истина);
	Методы.Вставить("ФоновоеЗаданиеОчисткиКлассификатораАдресов", Истина);
