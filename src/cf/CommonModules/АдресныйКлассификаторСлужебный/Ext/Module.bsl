...
Функция СписокЗемельныхУчастков(ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска, ПорцияПриПоиске = 20) Экспорт
	...
	Если ВариантыЗемельныхУчастков.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		ТекстGETЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("selectStead?parentId=%1&limit=%2",
			Строка(ИдентификаторАдресногоОбъекта), Формат(ПорцияПриПоиске, "ЧГ=0"));
		Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
			ТекстGETЗапроса = ТекстGETЗапроса + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("&text=%1",
				СокрЛП(СтрокаПоиска));
		...
		Для каждого СведенияОДоме Из РезультатВебСервис.Данные Цикл
			...
			ДобавитьИнформациюОДоме(ВариантыЗемельныхУчастков, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска);
		КонецЦикла;
...
Процедура УстановитьИдентификаторыАдресаПоЗагруженнымДанным(Адрес)
	...
	|			И (АдресныеОбъектыУровень1.Наименование = &Наименование1)
	...
	|			И (АдресныеОбъектыУровень2.Наименование = &Наименование2)
	...
	|			И (АдресныеОбъектыУровень3.Наименование = &Наименование3)
	...
	|			И (АдресныеОбъектыУровень4.Наименование = &Наименование4)
	...
	|			И (АдресныеОбъектыУровень5.Наименование = &Наименование5)
...
Процедура ЗаполнитьАктуальныеАдресныеСведенияВнутр(Результат, Значение)
	...
	Индекс = 5;
	Пока Индекс >= 0 Цикл
		ЗаполнитьПоляВСоответствииСУровнемАдреса(Результат, Выборка[1], Индекс, СловарьИменПолей);
		Индекс = Индекс - 1;
	КонецЦикла;
	
	Индекс = 5;
	Пока Индекс >= 0 Цикл
		ЗаполнитьПоляВСоответствииСУровнемАдреса(Результат, Выборка[0], Индекс, СловарьИменПолей);
		Индекс = Индекс - 1;
	КонецЦикла;
...
Процедура ОповеститьОПрогрессе(Знач НаименованиеРегиона, Знач СубъектРФ, Знач ОсталосьДляЗагрузки)
	...
	Перем ИнформацияОЗагрузке, ТекстОсталосьЗагрузить, ШаблонСообщения;
	
	ШаблонСообщения = НСтр("ru = 'Загрузка региона ""%1 - %2"" %3'") + "...";
	
	ТекстОсталосьЗагрузить = ?(ОсталосьДляЗагрузки > 0,
		" (" + НСтр("ru = 'осталось'") + " " + Формат(ОсталосьДляЗагрузки, "ЧН=") + ")",
		"");
...
Процедура ЗаполнитьПоляВСоответствииСУровнемАдреса(Адрес, Выборка, Суффикс, СловарьИменПолей)
...
Функция ЗаполнитьКодыАдреса(Результат, ИдентификаторАдресногоОбъекта, СведенияОДоме)
	...
	Если Коды <> Неопределено Тогда
		...
		Результат.Вставить("ИдентификаторДома", Коды.ИдентификаторДома);
		Возврат Истина;
	КонецЕсли;
...
Процедура ИзвлечьСписокЗемельныхУчастков(СтрокаСоСпискомУчастков, СтрокаПоиска, ВариантыЗемельныхУчастков, Запись, ТочноеСовпадение)
	...
	Для каждого Участок Из МассивУчастков Цикл
		...
		ОписаниеУчастка.Номер         = НомерУчастка[0];
		...
		Если ПодборВариантовЗемельныхУчастков Тогда
			...
			ИначеЕсли СтрНачинаетсяС(ОписаниеУчастка.Номер, СтрокаПоиска) Тогда
				ЗемельныйУчасток = ВариантыЗемельныхУчастков.Добавить();
				ЗемельныйУчасток.Представление = НомерУчастка[0];
				ЗемельныйУчасток.Значение = ОписаниеУчастка;
			КонецЕсли;
