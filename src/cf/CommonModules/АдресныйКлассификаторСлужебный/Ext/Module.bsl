...
//
Функция АнализАдресовПоКлассификатору(Знач Адреса) Экспорт
	...
	Возврат РезультатПроверкиАдресов(АдресаJSON);
...
//               *** КодУчасткаИФНСЮЛ - Число - Данные ФНС.
//
Функция РезультатПроверкиАдресов(НаборАдресов) Экспорт
	...
	Если Результат.Отказ = Ложь Тогда
		...
		Если Результат.Данные.Количество() > 0 Тогда
			...
			Для каждого АдресДляПроверки Из АдресаДляПроверки Цикл
				...
				АдресИзСервиса = Результат.Данные[АдресДляПроверки.Ключ];
...
Функция ПервичнаяПроверкаАдресов(Знач Адреса)
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		Результат = Сведения.Адреса[КлючЗначение.Ключ];
...
Функция РезультатПроверкиАдреса()
...
Функция ПроверитьАдресаПоЗагруженнымДанным(Адреса)
...
Функция ПроверитьАдресаПоКлассификатору(Адреса)
	...
	Для Каждого ЭлементПроверки Из Адреса Цикл
		Если Результаты[ЭлементПроверки.Ключ].АдресПроверен = Ложь
			...
			И ЭтоГородФедеральногоЗначения(ЭлементПроверки.Значение)
	...
	Если АдресГородовФедеральногоЗначения.Количество() > 0 Тогда
		РезультатыПовторнойПроверки = ПроверитьАдресаПоЗагруженнымДанным(АдресГородовФедеральногоЗначения);
	...
	Для Каждого КлючЗначение Из Адреса Цикл 
		...
		Для каждого УровеньАдреса Из ИменаУровнейАдреса Цикл
			Если Адрес.Свойство(УровеньАдреса) 
				...
				АдресПоПолям["Наименование" + Строка(Счетчик)] = Наименование;
				АдресПоПолям["Сокращение"   + Строка(Счетчик)] = Сокращение;
...
Функция ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Знач АдресаДляПроверки)
	...
	Исключение
		РезультатПроверкиАдреса = РезультатПроверкиАдреса();
