...
//     * Ссылка - ЛюбаяСсылка
//  ИмяСписка - Строка - имя динамического списка на форме. По умолчанию, "Список".
//
Процедура ПриСозданииНаСервере(Форма, Объект = Неопределено, ИмяСписка = "Список") Экспорт
	...
	Если Объект = Неопределено Тогда
		Если ТипФормы = "ОсновнаяФормаСписка" Или ТипФормы = "ОсновнаяФормаВыбора" Тогда
			ИзменениеТекстаЗапросаСпискаДляТекущегоЯзыка(Форма, ИмяСписка);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
// Регистрация предопределенных элементов объекта для отложенного обработчика.
// Процедура используется только для отложенных обработчиков обновления.
//
// Параметры:
//  Параметры  -см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке
//  ОбъектМетаданных - ОбъектМетаданных - если указан, то будет зарегистрированы предопределенные
//                     только этого объекта.
//                     Иначе будут зарегистрированы предопределенные элементы у объектов, описанных в
...
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры, ОбъектМетаданных = Неопределено) Экспорт
	...
	Для каждого ОбъектМетаданныхСЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		Пока ДанныеЗапроса.Следующий() Цикл
			Элемент = НастройкиПредопределенныхЭлементов.ДополнительныеПараметры.ПредопределенныеДанные.Найти(ДанныеЗапроса.ИмяПредопределенныхДанных, "ИмяПредопределенныхДанных");
