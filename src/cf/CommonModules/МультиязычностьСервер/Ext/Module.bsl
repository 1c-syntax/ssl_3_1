...
Процедура ПередЗаписьюНаСервере(ТекущийОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) И МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Тогда
	...
	Если ИменаЛокализуемыхРеквизитов.Количество() > 0 Тогда
		СкопироватьМультиязычныеЗначенияВРеквизитыШапки(ТекущийОбъект, ИменаЛокализуемыхРеквизитов, КодПервогоДополнительногоЯзыкаИнформационнойБазы());
		СкопироватьМультиязычныеЗначенияВРеквизитыШапки(ТекущийОбъект, ИменаЛокализуемыхРеквизитов, КодВторогоДополнительногоЯзыкаИнформационнойБазы());
	КонецЕсли;
...
Функция ОбъектСодержитТЧПредставления(СсылкаИлиПолноеИмяМетаданных) Экспорт
	Возврат МультиязычностьПовтИсп.ОбъектСодержитТЧПредставления(СсылкаИлиПолноеИмяМетаданных);
КонецФункции
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбъектыСТЧПредставления = ОбъектыСТЧПредставления();
	...
	Если ОбъектыСТЧПредставления.Количество() > 0 Тогда
		...
		|ИЗ
		|	#Таблица КАК СправочникСТЧПредставления";
		...
		ОбъектыСТабличнойЧастьюПредставлениеКОбработке = РезультатыЗапроса.ВыгрузитьКолонку("Ссылка");
	
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбъектыКОбработке, ОбъектыСТабличнойЧастьюПредставлениеКОбработке);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбъектыСТЧПредставления = ОбъектыСТЧПредставления();
	Для Каждого ИмяОбъектСТЧПредставления Из ОбъектыСТЧПредставления Цикл
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ОбъектыДляОбработки = ОбновлениеИнформационнойБазыСлужебный.ОбъектыСНачальнымЗаполнениемСтрокой();
	
	ОбъектыСтрокой = СтрСоединить(ОбъектыСТЧПредставления(), ",");
	Если ЗначениеЗаполнено(ОбъектыСтрокой) Тогда
		ОбъектыДляОбработки = ОбъектыДляОбработки + "," + ОбъектыСтрокой;
	КонецЕсли;
...
Процедура УстановитьЗначенияРеквизитов(Объект, Значения, Знач КодЯзыка = Неопределено) Экспорт
	...
	МультиязычныеСтрокиВРеквизитах = МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных);
	МультиязычныеРеквизиты = МультиязычныеРеквизитыОбъекта(ОбъектМетаданных);
	...
	Для Каждого ЗначениеРеквизита Из Значения Цикл
		...
		Для Каждого Элемент Из ЗначенияРеквизита Цикл
			...
			Если МультиязычныеРеквизиты[ИмяРеквизита] = Истина Тогда
				Если МультиязычныеСтрокиВРеквизитах Тогда
					СуффиксЯзыка = "";
					Если КодОсновногоЯзыка() <> Язык Тогда
...
Функция НаименованияЛокализуемыхРеквизитовОбъекта(ОбъектМетаданных, Префикс = "") Экспорт
	...
	ИначеЕсли ОбъектСодержитТЧПредставления(ОбъектМетаданных.ПолноеИмя()) Тогда
...
//  Массив из Строка
//
Функция ОбъектыСТЧПредставления()
...
Процедура ОбработатьСсылочныеОбъекты(ПолноеИмя, ДанныеОбъекта, ДанныеДляИзмененияМультиязычныхРеквизитов, ПараметрыЗаполнения)
	...
	ШаблонПолейЗапроса = "ДанныеОбъекта.%1%2 КАК %1%2"; // @Qurey-part1
