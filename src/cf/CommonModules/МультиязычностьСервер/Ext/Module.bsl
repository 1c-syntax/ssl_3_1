...
Функция СуффиксЯзыка(КодЯзыка) Экспорт
	...
	Возврат МультиязычностьПовтИсп.СуффиксЯзыка(КодЯзыка);
...
// 
// Параметры:
//  СуффиксЯзыка - Строка - суффикс дополнительного языка. Например, "Язык1"
// 
// Возвращаемое значение:
//  Булево - если, Истина, то дополнительный язык включен в приложении и используется. 
//
Функция ИспользуетсяДополнительныйЯзык(СуффиксЯзыка) Экспорт

	Возврат МультиязычностьПовтИсп.ИспользуетсяДополнительныйЯзык(СуффиксЯзыка);
...
Функция ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы() Экспорт
	...
	Если ДанныеИзмененияМультиязычныхРеквизитов <> Неопределено Тогда
		...
		Если (ДанныеИзмененияМультиязычныхРеквизитов.ПервыйДополнительныйЯзыкИзменен
		   И ТекущийЯзык().КодЯзыка = КодПервогоДополнительногоЯзыкаИнформационнойБазы())
		 Или (ДанныеИзмененияМультиязычныхРеквизитов.ВторойДополнительныйЯзыкИзменен
		   И ТекущийЯзык().КодЯзыка = КодВторогоДополнительногоЯзыкаИнформационнойБазы()) Тогда
				Возврат Истина;
		КонецЕсли;
...
Функция ИспользуетсяПервыйДополнительныйЯзык() Экспорт
	...
	Возврат ИспользуетсяДополнительныйЯзык("Язык1");
	
КонецФункции

Функция ИспользуетсяВторойДополнительныйЯзык() Экспорт
	
	Возврат ИспользуетсяДополнительныйЯзык("Язык2");
	...
	Если ЛокализуемыеРеквизитыОбъекта.Количество() > 0 Тогда
		Если МультиязычныеСтрокиВРеквизитах Тогда
			...
			Для Каждого ИмяЛокализуемогоРеквизита Из ЛокализуемыеРеквизитыОбъекта Цикл
				...
				Для Позиция = 1 По СведенияОбЯзыках.КоличествоДополнительныхЯзыков Цикл
					
					СуффиксЯзыка = "Язык" + Строка(Позиция);
					ИмяРеквизита = ИмяЛокализуемогоРеквизита.Ключ + СуффиксЯзыка;
					
					Если ПоляИсключения[ИмяРеквизита] <> Неопределено
						Или (Не ИспользуетсяДополнительныйЯзык(СуффиксЯзыка) 
...
Функция НаименованияЛокализуемыхРеквизитовОбъекта(ОбъектМетаданных, Префикс = "") Экспорт
	...
	Если МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Тогда
		...
		Для каждого Колонка Из РезультатЗапроса.Колонки Цикл
			Если (СтрЗаканчиваетсяНа(Колонка.Имя, "Язык1")
			 Или  СтрЗаканчиваетсяНа(Колонка.Имя, "Язык2"))
			   И Не СтрНачинаетсяС(Колонка.Имя, "Удалить") Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Попытка
					...
					Для Каждого МультиязычныйРеквизит Из МультиязычныеРеквизиты Цикл
						...
						ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ДополнительныйЯзык1СтароеЗначение] = ИзменяемыйОбъект[МультиязычныйРеквизит
							+ СуффиксПервогоЯзыка()];
						ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ДополнительныйЯзык2СтароеЗначение] = ИзменяемыйОбъект[МультиязычныйРеквизит
							+ СуффиксВторогоЯзыка()];
						...
						Если ОтредактированныеРеквизиты <> Неопределено Тогда
							...
							Если ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит) <> Неопределено Тогда
								...
								Замены.Вставить(Позиция, МультиязычныйРеквизит + СуффиксЯзыка(ЗначенияЯзыковыхКонстант.ОсновнойЯзыкСтароеЗначение));
							КонецЕсли;
							Если ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит + СуффиксПервогоЯзыка()) <> Неопределено Тогда
								Позиция = ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит + СуффиксПервогоЯзыка());
								Замены.Вставить(Позиция, МультиязычныйРеквизит + СуффиксЯзыка(ЗначенияЯзыковыхКонстант.ДополнительныйЯзык1СтароеЗначение));
							КонецЕсли;
							Если ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит + СуффиксВторогоЯзыка()) <> Неопределено Тогда 
								Позиция = ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит + СуффиксВторогоЯзыка());
								Замены.Вставить(Позиция, МультиязычныйРеквизит + СуффиксЯзыка(ЗначенияЯзыковыхКонстант.ДополнительныйЯзык2СтароеЗначение));
							КонецЕсли;
