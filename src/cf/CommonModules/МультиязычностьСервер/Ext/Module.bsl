...
Процедура ПриСозданииНаСервере(Форма, Объект = Неопределено, ИмяОбъекта = Неопределено) Экспорт
	...
	Если Форма.Параметры.Свойство("ЗначенияЗаполнения") 
	   И ЗначениеЗаполнено(Форма.Параметры.ЗначенияЗаполнения) Тогда
		ПриЧтенииНаСервере(Форма, Объект, ИмяОбъекта);
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект, ИмяОбъекта = Неопределено) Экспорт
	...
	Если ТипЗнч(ИмяОбъекта) <> Тип("Строка") Тогда
		ИмяОбъекта = ?(ОбщегоНазначения.ЭтоРегистр(ОбъектМетаданных), "Запись", "Объект");
	КонецЕсли;

	ПроверкаСвойства = Новый Структура(ИмяОбъекта, Неопределено);
	ЗаполнитьЗначенияСвойств(ПроверкаСвойства, Форма, ИмяОбъекта);
	Если ПроверкаСвойства[ИмяОбъекта] <> Неопределено Тогда
		ПриЧтенииПредставленийНаСервере(Форма[ИмяОбъекта]);
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(ТекущийОбъект) Экспорт
	...
	ИменаМультиязычныхРеквизитов       = НаименованияЛокализуемыхРеквизитовОбъекта(ОбъектМетаданных);
		...
		Если Не ОбщегоНазначения.ЭтоРегистр(ОбъектМетаданных) И ТекущийОбъект.ЭтоНовый()  Тогда
			...
			Для Каждого Реквизит Из ИменаМультиязычныхРеквизитовВШапке Цикл
				...
				Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков() Цикл
					...
					Если ПустаяСтрока(ТекущийОбъект[Реквизит.Ключ + ИмяСуффиксаЯзыка]) Тогда
						ТекущийОбъект[Реквизит.Ключ + ИмяСуффиксаЯзыка] = ЗначениеРеквизита;
					КонецЕсли;
				КонецЦикла;
	...
	Если Не ОбъектСодержитТЧПредставления(ПолноеИмя) Тогда
		Возврат;
	КонецЕсли;
	
	Реквизиты = МультиязычностьПовтИсп.МультиязычныеРеквизитыТабличнойЧасти(ПолноеИмя);
	...
	ЗаполнитьМультиязычныеРеквизитыТабличнойЧастиПредставления(ТекущийОбъект, Реквизиты, ИменаМультиязычныхРеквизитовВШапке);
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		Для каждого ИмяРеквизита Из Реквизиты Цикл  
			Если ИменаМультиязычныхРеквизитовВШапке[ИмяРеквизита] = Неопределено
			   ...
			   И ПустаяСтрока(СуффиксТекущегоЯзыка)) 
			   И ТекущийОбъект[ИмяРеквизита] <> Null Тогда
...
Процедура ПриЧтенииПредставленийНаСервере(Объект) Экспорт
	...
	Для каждого ИмяРеквизита Из Реквизиты Цикл
		...
		Если ИменаЛокализуемыхРеквизитов.Получить(ИмяРеквизита) <> Неопределено Тогда
			Представление[ИмяРеквизита] = Объект[ИмяРеквизита + СуффиксТекущегоЯзыка];
		Иначе
...
Функция СуффиксЯзыка(КодЯзыка) Экспорт
	...
	Возврат МультиязычностьПовтИсп.СуффиксЯзыкаПоКодуЯзыка(КодЯзыка);
...
Функция ИменаРеквизитовСУчетомКодаЯзыка(ИменаРеквизитов, КодЯзыка = "") Экспорт
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		Результат.Вставить(СокрЛ(Реквизит), СокрЛП(Реквизит) + СуффиксЯзыка);
	КонецЦикла;
...
Функция КоличествоДополнительныхЯзыков() Экспорт
	Возврат МультиязычностьПовтИсп.КоличествоДополнительныхЯзыков();
КонецФункции
...
Функция СведенияОЯзыках() Экспорт
	Возврат МультиязычностьПовтИсп.СведенияОЯзыках();
КонецФункции
...
Функция ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы() Экспорт
	...
	ДанныеИзмененияМультиязычныхРеквизитов = ДанныеДляИзмененияМультиязычныхРеквизитов();
	Если ДанныеИзмененияМультиязычныхРеквизитов <> Неопределено Тогда
		Если ДанныеИзмененияМультиязычныхРеквизитов.ОсновнойЯзыкИзменен
		 Или Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
			Возврат Истина;
		КонецЕсли;
		
		Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков() Цикл
			ИмяКонстантыЯзыка = ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка);
			СведенияОбЯзыке = ДанныеИзмененияМультиязычныхРеквизитов.Изменения[ИмяКонстантыЯзыка];
			
			Если СведенияОбЯзыке.Значение
			   И ТекущийЯзык().КодЯзыка = КодДополнительногоЯзыкаИнформационнойБазы(ПорядковыйНомерЯзыка) Тогда
			   Возврат Истина;
			КонецЕсли;
...
Функция ИмяРеквизитаБезСуффиксаЯзык(Знач ИмяРеквизита) Экспорт
	...
	Шаблон = МультиязычностьКлиентСервер.СуффиксЯзыка() + "\d+";
	
	РезультатВыражения = СтрНайтиПоРегулярномуВыражению(ИмяРеквизита, Шаблон, НаправлениеПоиска.СКонца,, Истина);
	Если РезультатВыражения.НачальнаяПозиция > 0 Тогда
		Возврат Лев(ИмяРеквизита, РезультатВыражения.НачальнаяПозиция - 1);
	КонецЕсли;
...
Функция СведенияОРеквизите(Знач ИмяРеквизита) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Мультиязычный", Ложь);
	Результат.Вставить("ИмяРеквизитаБезСуффиксаЯзык", ИмяРеквизита);
	Результат.Вставить("Удаленный", СтрНачинаетсяС(ИмяРеквизита, "Удалить"));
	
	Шаблон = МультиязычностьКлиентСервер.СуффиксЯзыка() + "\d+";
	РезультатВыражения = СтрНайтиПоРегулярномуВыражению(ИмяРеквизита, Шаблон, НаправлениеПоиска.СКонца,, Истина);
	Если РезультатВыражения.НачальнаяПозиция > 0 Тогда
		Результат.Мультиязычный = Истина;
		Результат.ИмяРеквизитаБезСуффиксаЯзык = Лев(ИмяРеквизита, РезультатВыражения.НачальнаяПозиция - 1);
	КонецЕсли;
...
Процедура ОпределитьИзмененныеМультиязычныеЭлементы(НовоеЗначение, СтрокаТаблицы, ИмяЛокализуемогоРеквизита, ОтредактированныеРеквизиты, НаборДанныхДляЗаполнения) Экспорт

	ЛокализуемыеРеквизитыОбъекта = НаборДанныхДляЗаполнения.ЛокализуемыеРеквизитыОбъекта;
	...
	СведенияОбЯзыках       = СведенияОЯзыках();
	ПоддерживаетсяИерархия = НаборДанныхДляЗаполнения.ПоддерживаетсяИерархия;

	Если НаборДанныхДляЗаполнения.МультиязычныеСтрокиВРеквизитах Тогда
		
		Для НомерЯзыка = 0 По СведенияОбЯзыках.КоличествоДополнительныхЯзыков Цикл

			КодЯзыка = СведенияОбЯзыках["Язык" + Строка(НомерЯзыка)];

			ИмяЛокализуемогоРеквизитаВШапке = ?(НомерЯзыка = 0, ИмяЛокализуемогоРеквизита, ИмяЛокализуемогоРеквизита
				+ "Язык" + Строка(НомерЯзыка));
			Если ЗначениеЗаполнено(КодЯзыка) Тогда
				Если НовоеЗначение[ИмяЛокализуемогоРеквизитаВШапке] <> СтрокаТаблицы[ИмяЛокализуемогоРеквизита + "_"
					+ КодЯзыка] Тогда
					ОтредактированныеРеквизиты.Добавить(ИмяЛокализуемогоРеквизитаВШапке);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли Не (ПоддерживаетсяИерархия И НовоеЗначение.ЭтоГруппа) Тогда
		
		Для Каждого КодЯзыка Из Языки Цикл
			ТаблицаПредставлений = НовоеЗначение.Представления; // ТаблицаЗначений
			...
			Для Каждого ИмяЛокализуемогоРеквизита Из ЛокализуемыеРеквизитыОбъекта Цикл
				...
				НовоеПредставление[ИмяЛокализуемогоРеквизита.Ключ] = ?(ЗначениеЗаполнено(Значение), Значение,
					СтрокаТаблицы[ИмяЛокализуемогоРеквизита.Ключ]);
			КонецЦикла;
		КонецЦикла;
...
Процедура УдалитьМультиязычныеРеквизиты(ОтредактированныеРеквизиты) Экспорт
	
	СведенияОбЯзыках = СведенияОЯзыках();
	
	Позиция = ОтредактированныеРеквизиты.Количество() - 1;
	
	Пока Позиция >= 0 Цикл
	
		Для НомерЯзыка = 1 По СведенияОбЯзыках.КоличествоДополнительныхЯзыков Цикл
			Если СтрЗаканчиваетсяНа(ОтредактированныеРеквизиты[Позиция], "Язык" + НомерЯзыка) Тогда
				ОтредактированныеРеквизиты.Удалить(Позиция);
				Прервать;
			КонецЕсли;
		КонецЦикла;
			...
			Для Каждого ИсточникЗапроса Из ОператорЗапроса.Источники Цикл
				Если ТипЗнч(ИсточникЗапроса.Источник) <> Тип("ВложенныйЗапросСхемыЗапроса")
					...
					Для каждого ОписаниеРеквизита Из ЛокализуемыеРеквизиты Цикл
						...
						ИмяОсновногоРеквизита = ИмяРеквизитаБезСуффиксаЯзык(ОписаниеРеквизита.Ключ);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого ОбъектСПредопределеннымиЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		Если ОбрабатываемыеОбъекты.Найти(ОбъектСПредопределеннымиЭлементами.Значение) = Неопределено Тогда
			ОбрабатываемыеОбъекты.Добавить(ОбъектСПредопределеннымиЭлементами.Значение);
		КонецЕсли;
	...
	Для каждого ОбъектСПредопределеннымиЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		Выборка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь,
			ОбъектСПредопределеннымиЭлементами.Значение);
		Если Выборка.Количество() > 0 Тогда
		
			// @skip-check query-in-loop - Порционная обработка данных в обработчике обновления
			ЗаполнитьПустыеМультиязычныеРеквизитыЗначениемОсновногоЯзыка(Выборка, ОбъектСПредопределеннымиЭлементами.Ключ,
				ИтогоОбработано);
...
#Область НачальноеЗаполнениеЭлементовДанными
		...
		Для каждого ЛокализуемыйРеквизитОбъекта Из НастройкиОбновления.ЛокализуемыеРеквизитыОбъекта Цикл
			...
			Для Каждого ЯзыкИспользуется Из СведенияОбЯзыках.Используется Цикл
				Если ЯзыкИспользуется.Значение Тогда
					...
					Объект[Имя + ЯзыкИспользуется.Ключ] = Элемент[ИмяЛокализуемогоРеквизита(Имя, КодЯзыка)];
				КонецЕсли;
			КонецЦикла;
...
Функция НастройкиОбновлениеПредопределенныхДанных(ОбъектМетаданных) Экспорт
	...
	СведенияОбЯзыках = СведенияОЯзыках();
	Для ПорядковыйНомерЯзыка = 1 По СведенияОбЯзыках.КоличествоДополнительныхЯзыков Цикл
		ИмяСуффикса = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
		Результат.Вставить(ИмяСуффикса, СведенияОбЯзыках[ИмяСуффикса]);
	КонецЦикла;
...
Функция МультиязычныеСтрокиРеквизитаИзменены(ПоставляемыеСведения, ДанныеЗапроса, ИмяРеквизита, ПараметрыРегистрации) Экспорт
	...
	СведенияОбЯзыках = МультиязычностьПовтИсп.СведенияОЯзыках();
	...
	Для Каждого ЯзыкИспользуется Из СведенияОбЯзыках.Используется Цикл
		...
		Если ЯзыкИспользуется.Значение
			И СтрСравнить(ПоставляемыеСведения[ИмяЛокализуемогоРеквизита(ИмяРеквизита, СведенияОбЯзыках[ЯзыкИспользуется.Ключ])], ДанныеЗапроса[ИмяРеквизита + ЯзыкИспользуется.Ключ]) <> 0  Тогда
			
			Если ПустаяСтрока(ПоставляемыеСведения[ИмяЛокализуемогоРеквизита(ИмяРеквизита, СведенияОбЯзыках[ЯзыкИспользуется.Ключ])]) Тогда
				Если Не ПараметрыРегистрации.ПропускатьПустые Тогда
					Возврат Истина;
				КонецЕсли;
			Иначе
...
Процедура СформироватьИменаМультиязычныхРеквизитов(Знач ИменаРеквизитовОбъекта, Знач ЛокализуемыеРеквизитыОбъекта) Экспорт
	...
	Для каждого РеквизитОбъекта Из ЛокализуемыеРеквизитыОбъекта Цикл
		...
		Для Каждого ЯзыкИспользуется Из СведенияОбЯзыках.Используется Цикл
			Если ЯзыкИспользуется.Значение Тогда
				ИменаРеквизитовОбъекта.Добавить(РеквизитОбъекта.Ключ + ЯзыкИспользуется.Ключ);
			КонецЕсли;
		КонецЦикла;
...
Процедура ПриПолученииПараметровЗаполненияПредопределенныхДанных(ПараметрыЗаполнения) Экспорт
	...
	ПараметрыЗаполнения.Вставить("СведенияОбЯзыках", СведенияОЯзыках());
	
КонецПроцедуры

Процедура НачальноеЗаполнениеПредопределенныхДанныхЛокализуемыеРеквизиты(ЗаполняемыйЭлемент, СтрокаТаблицы, 
	ПоляИсключения, Контекст) Экспорт
	
	СведенияОбЯзыках = СведенияОЯзыках();
	
	ПоддерживаетсяИерархия = Контекст.ПоддерживаетсяИерархия;
	
	ЛокализуемыеРеквизитыОбъекта = Контекст.ЛокализуемыеРеквизитыОбъекта;
	МультиязычныеСтрокиВРеквизитах = Контекст.МультиязычныеСтрокиВРеквизитах;
	...
	Если ЛокализуемыеРеквизитыОбъекта.Количество() > 0 Тогда
		Если МультиязычныеСтрокиВРеквизитах Тогда
			...
			Для Каждого ИмяЛокализуемогоРеквизита Из ЛокализуемыеРеквизитыОбъекта Цикл
				...
				ЗначениеОсновнойЯзык = СтрокаТаблицы[ИмяЛокализуемогоРеквизита.Ключ + "_"
				+ СведенияОбЯзыках.ОсновнойЯзык];
				
				Если ПустаяСтрока(ЗначениеОсновнойЯзык) Тогда
					ЗначениеОсновнойЯзык = СтрокаТаблицы[ИмяЛокализуемогоРеквизита.Ключ];
				КонецЕсли;
				
				Если ПоляИсключения[ИмяЛокализуемогоРеквизита.Ключ] = Неопределено Тогда
					ЗаполняемыйЭлемент[ИмяЛокализуемогоРеквизита.Ключ] = ЗначениеОсновнойЯзык;
				КонецЕсли;
				
				Для ПорядковыйНомерЯзыка = 1 По СведенияОбЯзыках.КоличествоДополнительныхЯзыков Цикл
					
					СуффиксЯзыка = "Язык" + Строка(ПорядковыйНомерЯзыка);
					ИмяРеквизита = ИмяЛокализуемогоРеквизита.Ключ + СуффиксЯзыка;
					Если ПоляИсключения[ИмяРеквизита] <> Неопределено
						Или (Не ИспользуетсяДополнительныйЯзык(ПорядковыйНомерЯзыка) 
						И ЗначениеЗаполнено(ЗаполняемыйЭлемент[ИмяРеквизита])) Тогда
						Продолжить;
					КонецЕсли;
					
					Если ЗначениеЗаполнено(СведенияОбЯзыках[СуффиксЯзыка]) Тогда
						ЗначениеЯзык = СтрокаТаблицы[ИмяЛокализуемогоРеквизита.Ключ + "_" + СведенияОбЯзыках[СуффиксЯзыка]];
					КонецЕсли;
					
					ЗаполняемыйЭлемент[ИмяРеквизита] = ?(ПустаяСтрока(ЗначениеЯзык), ЗначениеОсновнойЯзык, ЗначениеЯзык);
		...
		Если Контекст.ОбъектСодержитТЧПредставления И Не (ПоддерживаетсяИерархия
			И ЗаполняемыйЭлемент.ЭтоГруппа) Тогда
			ОбновлениеИнформационнойБазыСлужебный.НачальноеЗаполнениеТЧПредставления(ЗаполняемыйЭлемент, СтрокаТаблицы,
			Контекст);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьЭлементыМультиязычнымиНачальнымиДанными(ЗаполняемыйЭлемент, СтрокаТаблицы, Контекст, ПоляИсключения) Экспорт
	...
	Если МультиязычныеСтрокиВРеквизитах Тогда 
		...
		НачальноеЗаполнениеПредопределенныхДанныхЛокализуемыеРеквизиты(ЗаполняемыйЭлемент, СтрокаТаблицы, МассивВСоответствие(ПоляИсключения), Контекст);
...
Функция КодДополнительногоЯзыкаИнформационнойБазы(ПорядковыйНомерЯзыка) Экспорт
	
	Если Не ИспользуетсяДополнительныйЯзык(ПорядковыйНомерЯзыка) Тогда
		Возврат "";
	КонецЕсли;
	
	КодЯзыка = Константы[ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка)].Получить();
...
Функция ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка) Экспорт
	Возврат "ДополнительныйЯзык" + Формат(ПорядковыйНомерЯзыка,"ЧГ=0");
КонецФункции

Функция ИмяФункциональнойОпции(ПорядковыйНомерЯзыка) Экспорт
	Возврат "ИспользоватьДополнительныйЯзык" + Формат(ПорядковыйНомерЯзыка,"ЧГ=0");
КонецФункции

// Заполняет мультиязычный реквизит в обработке начального заполнения при преобразовании кода заполнения в таблицу. 
//
Процедура ЗаполнитьМультиязычныйРеквизитНачальногоЗаполнения(Элемент, ИмяРеквизита, ЛокализуемаяСтрока, КодыЯзыков) Экспорт
	...
	ОбщиеЯзыковыеПараметры.Вставить("ОсновнойЯзык", ОсновнойЯзык);
	...
	Для Каждого КодЯзыка Из КодыЯзыков Цикл
		...
		ИначеЕсли КодЯзыка = ОбщиеЯзыковыеПараметры.ДополнительныйЯзык1 Тогда
			ИмяЛокализуемогоРеквизита = ИмяРеквизита + "Язык1";
		ИначеЕсли КодЯзыка = ОбщиеЯзыковыеПараметры.ДополнительныйЯзык2 Тогда
			ИмяЛокализуемогоРеквизита = ИмяРеквизита + "Язык2";
		КонецЕсли;
...
//
Функция ОписаниеСтарыхИНовыхНастроекЯзыков(КоличествоДополнительныхЯзыков) Экспорт
	
	ОписаниеКонстант = Новый Структура;

	НовоеИСтароеЗначение = Новый Структура;
	НовоеИСтароеЗначение.Вставить("НовоеЗначение", "");
	НовоеИСтароеЗначение.Вставить("СтароеЗначение", "");
	
	ОписаниеКонстант.Вставить("ОсновнойЯзык", НовоеИСтароеЗначение);
	...
	Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков() Цикл
		...
		ОписаниеКонстант.Вставить(ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка), НовоеИСтароеЗначение);
	КонецЦикла;
...
Функция НаименованияЛокализуемыхРеквизитовОбъекта(ОбъектМетаданных, Префикс = "") Экспорт
	...
	Если МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Тогда
		...
		Для каждого Колонка Из РезультатЗапроса.Колонки Цикл
			СведенияОРеквизите = СведенияОРеквизите(Колонка.Имя);
			Если СведенияОРеквизите.Мультиязычный И Не СведенияОРеквизите.Удаленный Тогда
				СписокРеквизитовОбъекта.Вставить(Префикс + СведенияОРеквизите.ИмяРеквизитаБезСуффиксаЯзык, Ложь);
			КонецЕсли;
		КонецЦикла;
	...
	Если ОбъектСодержитТЧПредставления(ПолноеИмя) Тогда
		
		РеквизитыТабличнойЧастиПредставления = МультиязычностьПовтИсп.МультиязычныеРеквизитыТабличнойЧасти(ПолноеИмя);
		Для каждого ИмяРеквизита Из РеквизитыТабличнойЧастиПредставления Цикл
			СписокРеквизитовОбъекта.Вставить(Префикс + ИмяРеквизита, Истина);
		КонецЦикла;
...
Функция ИменаОбъектовСМультиязычнымиРеквизитами() Экспорт
	...
	Для каждого Элемент Из Метаданные.ФункциональныеОпции["ИспользоватьДополнительныйЯзык1"].Состав Цикл
		...
		ИмяРеквизита = ИмяРеквизитаБезСуффиксаЯзык(Реквизит.Имя);
	...
	Если МультиязычныеСтрокиВРеквизитах(ОбъектМетаданные) Тогда
		...
		Для Каждого Реквизит Из СписокМультиязычныхРеквизитов Цикл
			...
			Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков() Цикл
				...
				Объект[Реквизит.Ключ + СуффиксЯзыка] = "";
			КонецЦикла;
			...
			Если ТекущийОбъект <> Неопределено Тогда
				...
				Для Каждого Реквизит Из ИменаЛокализуемыхРеквизитов Цикл
					...
					Для ПорядковыйНомерЯзыка =1 По КоличествоДополнительныхЯзыков() Цикл
						...
						СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
						Если ПустаяСтрока(ТекущийОбъект[Реквизит.Ключ + СуффиксЯзыка]) Тогда
							ТекущийОбъект[Реквизит.Ключ + СуффиксЯзыка] = ЗначениеРеквизита;
						КонецЕсли;
...
Процедура ЗаполнитьМультиязычныеРеквизитыТабличнойЧастиПредставления(ТекущийОбъект, Знач Реквизиты, ИменаМультиязычныхРеквизитовВШапке)
	...
	Для Каждого ЯзыкПечатнойФормы Из ДоступныеЯзыки Цикл
		
		Если ТекущийЯзык().КодЯзыка = ЯзыкПечатнойФормы Тогда
			Продолжить;
		КонецЕсли;
		
		Отбор = Новый Структура();
		Отбор.Вставить("КодЯзыка", ЯзыкПечатнойФормы);
		НайденныеСтроки = Представления.НайтиСтроки(Отбор);
		...
		Иначе
			Представление = Представления.Добавить();
			Представление.КодЯзыка = ЯзыкПечатнойФормы;
		КонецЕсли;
		
		Для каждого ИмяРеквизита Из Реквизиты Цикл
			Если ПустаяСтрока(Представление[ИмяРеквизита]) Тогда
				ЗначениеНаТекущемЯзыке = ?(ИменаМультиязычныхРеквизитовВШапке[ИмяРеквизита] <> Неопределено,
					ТекущийОбъект[ИмяРеквизита + СуффиксТекущегоЯзыка], ТекущийОбъект[ИмяРеквизита]) ;
				Представление[ИмяРеквизита] = ЗначениеНаТекущемЯзыке;
			КонецЕсли;
...
Процедура ЗаполнитьИПеренестиСтрокиИзТЧПредставленияВРеквизиты(Ссылка, ПараметрыЗаполнения, ОбъектовОбработано, ПроблемныхОбъектов)
	...
	Попытка
		...
		Для Каждого КодЯзыка Из Языки Цикл
			...
			МультиязычныеРеквизиты = МультиязычностьПовтИсп.МультиязычныеРеквизитыТабличнойЧасти(ПолноеИмя);
			Для каждого ИмяРеквизита Из МультиязычныеРеквизиты Цикл
				...
				Если ПустаяСтрока(СтрокаТаблицы[ИмяРеквизита]) Тогда
					...
					Если ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
						...
						Для ПорядковыйНомерЯзыка =1 По КоличествоДополнительныхЯзыков() Цикл
							Если СтрСравнить(КодДополнительногоЯзыкаИнформационнойБазы(ПорядковыйНомерЯзыка), КодЯзыка) = 0 Тогда
								СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
								Объект[ИмяРеквизита + СуффиксЯзыка] = ДанныеЗаполнения;
								Прервать;
							КонецЕсли;
						КонецЦикла;
				...
				Иначе
					...
					Для ПорядковыйНомерЯзыка =1 По КоличествоДополнительныхЯзыков() Цикл
						...
						СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
						Если СтрСравнить(КодДополнительногоЯзыкаИнформационнойБазы(ПорядковыйНомерЯзыка), КодЯзыка) = 0
						   И ПустаяСтрока(Объект[ИмяРеквизита + СуффиксЯзыка]) Тогда
							Объект[ИмяРеквизита + СуффиксЯзыка] = СтрокаТаблицы[ИмяРеквизита];
							Прервать;
						КонецЕсли;
...
Функция ОбъектыСНачальнымЗаполнениемСтрокой(СписокОбъектов)
	...
	Для Каждого ОбъектСПредопределеннымиЭлементами Из ОбъектыСНачальнымЗаполнением Цикл
		ПолноеИмя = ОбъектСПредопределеннымиЭлементами.Значение;
...
Функция ИспользуетсяПервыйДополнительныйЯзык()
	
	Возврат ИспользуетсяДополнительныйЯзык(1);
	
КонецФункции

Функция ИспользуетсяВторойДополнительныйЯзык()
	
	Возврат ИспользуетсяДополнительныйЯзык(2);
...
Функция СписокОбъектовКОбработкеДляПереходаНаНовуюВерсию(ВариантДанных)
	...
	ДополнительныеЯзыкиИспользуются = Ложь;

	Для ПорядковыйНомерЯзыка =1 По КоличествоДополнительныхЯзыков() Цикл
		
		Если ЗначениеЗаполнено(КодДополнительногоЯзыкаИнформационнойБазы(ПорядковыйНомерЯзыка)) Тогда
			ДополнительныеЯзыкиИспользуются = Истина;
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ДополнительныеЯзыкиИспользуются Тогда
		...
		Для Каждого ОбъектСПредопределеннымиЭлементами Из ОбъектыСПредопределеннымиЭлементами Цикл
			...
			ТекстЗапросаПредварительный = СтрЗаменить(ШаблонЗапроса, "&Таблица", ПолноеИмяОбъектаМетаданных);
			ТекстЗапросаПредварительный = СтрЗаменить(ТекстЗапросаПредварительный, "&УсловиеОтбора", СтрСоединить(
				Отборы, " ИЛИ "));
	...
	Если ОбъектыСТЧПредставления.Количество() > 0 Тогда
		...
		Для Каждого ИмяОбъектаМетаданных Из ОбъектыСТЧПредставления Цикл
			...
			МультиязычныеРеквизиты = МультиязычностьПовтИсп.МультиязычныеРеквизитыТабличнойЧасти(ИмяОбъектаМетаданных);
			...
			Для Каждого МультиязычныйРеквизит Из МультиязычныеРеквизиты Цикл
				...
				УсловияОтбора.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонГде, МультиязычныйРеквизит));
			КонецЦикла;
		...
		Для Каждого ЯзыкИспользуется Из СведенияОбЯзыках.Используется Цикл
			Если ЯзыкИспользуется.Значение Тогда
				ТекстЗапроса2 = СтрЗаменить(ТекстЗапроса, "#СуффиксЯзыка", ЯзыкИспользуется.Ключ);
				ТекстЗапроса2 = СтрЗаменить(ТекстЗапроса2, "&КодЯзыка", СведенияОбЯзыках[ЯзыкИспользуется.Ключ]);
				НаборЗапросов.Добавить(ТекстЗапроса2);
			КонецЕсли;
		КонецЦикла;
		
		Запрос.Текст = СтрСоединить(НаборЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
...
//       *** ЯзыковыеПоля - Массив
//   * Изменения - Структура:
//      ** ДополнительныйЯзык1 - Булево
//      ** ДополнительныйЯзык2 - Булево
//	
Функция ДанныеДляИзмененияМультиязычныхРеквизитов() Экспорт
	...
	ДанныеДляИзмененияМультиязычныхРеквизитов.Вставить("ОсновнойЯзыкИзменен", 
		СтрСравнить(НастройкиИзмененияЯзыков.ОсновнойЯзык.НовоеЗначение, НастройкиИзмененияЯзыков.ОсновнойЯзык.СтароеЗначение) <> 0);
		
	Изменения = Новый Структура;
	Для ПорядковыйНомер = 1 По КоличествоДополнительныхЯзыков() Цикл
		ИмяКонстанты = ИмяКонстантыЯзыка(ПорядковыйНомер);
		Изменения.Вставить(ИмяКонстанты,
			СтрСравнить(НастройкиИзмененияЯзыков[ИмяКонстанты].НовоеЗначение, НастройкиИзмененияЯзыков[ИмяКонстанты].СтароеЗначение) <> 0);
	КонецЦикла;
	ДанныеДляИзмененияМультиязычныхРеквизитов.Вставить("Изменения", Изменения);
...
Процедура ИзменитьЯзыкВМультиязычныхРеквизитахКонфигурации(Параметры, Адрес) Экспорт
	...
	Для Каждого ДанныеОбъекта Из ДанныеДляИзмененияМультиязычныхРеквизитов.Объекты Цикл
		...
		Если ОбъектыСНачальнымЗаполнением.Получить(МетаданныеОбъекта) <> Неопределено Тогда

			Контекст = ОбновлениеИнформационнойБазыСлужебный.НаборПараметровДляЗаполненияОбъекта(
				МетаданныеОбъекта);
			
		КонецЕсли;
		
		Процент = Окр(ОбработаноОбъектов * 100 / ОбъектыСНачальнымЗаполнением.Количество()) + 1;
		ДлительныеОперации.СообщитьПрогресс(?(Процент > 99, 98, Процент), МетаданныеОбъекта.Представление());
		...
		Если СтрНачинаетсяС(ПолноеИмя, "Справочник")
		 ...
		 Или СтрНачинаетсяС(ПолноеИмя, "ПланВидовРасчета") Тогда
			
			// @skip-check query-in-loop - Порционная обработка больших объемов данных.
			ОбработатьСсылочныеОбъекты(ПолноеИмя, ДанныеОбъекта.Значение, ДанныеДляИзмененияМультиязычныхРеквизитов,
				Контекст);
		...
		ИначеЕсли СтрНачинаетсяС(ПолноеИмя, "РегистрСведений") Тогда
			...
			Если ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
				// @skip-check query-in-loop - Порционная обработка больших объемов данных.
				ИзменитьЯзыкВПодчиненныхРегистрахСведений(ОбъектМетаданных, ДанныеОбъекта.Значение,
					ДанныеДляИзмененияМультиязычныхРеквизитов, Контекст);
			Иначе
				Периодический = ОбъектМетаданных.ПериодичностьРегистраСведений
					<> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический;
				// @skip-check query-in-loop - Порционная обработка больших объемов данных.
				ИзменитьЯзыкВНезависимыхРегистрахСведений(ОбъектМетаданных, ДанныеОбъекта.Значение,
					ДанныеДляИзмененияМультиязычныхРеквизитов, Контекст, Периодический);
			КонецЕсли;
		Иначе
			ОбъектыДляУдаления.Добавить(ПолноеИмя);
		КонецЕсли;
		
		ОбработаноОбъектов = ОбработаноОбъектов + 1;
	КонецЦикла;
	
	Если ОбъектыДляУдаления.Количество() > 0 Тогда
		Для Каждого ПолноеИмяОбъектаДляУдаления Из ОбъектыДляУдаления Цикл
			ДанныеДляИзмененияМультиязычныхРеквизитов.Объекты.Удалить(ПолноеИмяОбъектаДляУдаления);
		КонецЦикла;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		ДлительныеОперации.СообщитьПрогресс(99 , НСтр("ru = 'Варианты отчетов'"));
	...
	Пока ЕстьПорцияДанных Цикл
		...
		Для Каждого МультиязычныйРеквизит Из МультиязычныеРеквизиты Цикл
			...
			Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков Цикл
				СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
				ПоляЗапроса.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонПолейЗапроса, МультиязычныйРеквизит, СуффиксЯзыка));
			КонецЦикла;
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			СодержитОтредактированныеПредопределенныеРеквизиты = 
				ОбновлениеИнформационнойБазыСлужебный.СодержитОтредактированныеПредопределенныеРеквизиты(ПолноеИмя);
			
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				
				ПредставлениеИзменяемогоОбъекта = Строка(ВыборкаДетальныеЗаписи.Ссылка);
				...
				Попытка
					...
					ОтредактированныеРеквизиты = Неопределено;
					Если СодержитОтредактированныеПредопределенныеРеквизиты И ЗначениеЗаполнено(ИзменяемыйОбъект.ОтредактированныеПредопределенныеРеквизиты) Тогда
						ОтредактированныеРеквизиты = СтрРазделить(ИзменяемыйОбъект.ОтредактированныеПредопределенныеРеквизиты, ",", Ложь);
					КонецЕсли;
					...
					Для Каждого МультиязычныйРеквизит Из МультиязычныеРеквизиты Цикл
						...
						ЗначенияРеквизита = Новый Соответствие;
						ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение] =  ИзменяемыйОбъект[МультиязычныйРеквизит];
						
						Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков Цикл
							ИмяКонстантыЯзыка = ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка);
							СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
							
							ЗначенияРеквизита[ЗначенияЯзыковыхКонстант[ИмяКонстантыЯзыка].СтароеЗначение] = ИзменяемыйОбъект[МультиязычныйРеквизит
								+ СуффиксЯзыка];
						КонецЦикла;
						
						Если ОтредактированныеРеквизиты <> Неопределено Тогда
							
							Замены = Новый Соответствие;
							Если ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит) <> Неопределено Тогда
								Позиция = ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит);
								Замены.Вставить(Позиция, МультиязычныйРеквизит + СуффиксЯзыка(ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение));
							КонецЕсли;
							
							Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков Цикл
								
								СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
								Если ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит + СуффиксЯзыка) <> Неопределено Тогда
									Позиция = ОтредактированныеРеквизиты.Найти(МультиязычныйРеквизит + СуффиксЯзыка);
									ИмяКонстантыЯзыка = ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка);
									Замены.Вставить(Позиция, МультиязычныйРеквизит + СуффиксЯзыка(ЗначенияЯзыковыхКонстант[ИмяКонстантыЯзыка].СтароеЗначение));
								КонецЕсли; 
							
							КонецЦикла;
							
							Для Каждого Замена Из Замены Цикл 
								ОтредактированныеРеквизиты[Замена.Ключ] = Замена.Значение;
							КонецЦикла;
						...
						Если ДанныеДляИзмененияМультиязычныхРеквизитов.ОсновнойЯзыкИзменен Тогда
							НовоеЗначение = ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ОсновнойЯзык.НовоеЗначение];
							ЗаполнитьРеквизитИТЧПредставления(ПараметрыЗаполнения, МультиязычныйРеквизит,
								ИзменяемыйОбъект, НовоеЗначение, ЗначениеПоУмолчанию,
								ЗначенияЯзыковыхКонстант.ОсновнойЯзык.НовоеЗначение);
								
							Если ОбъектСодержитТЧПредставления Тогда
								Отбор = Новый Структура("КодЯзыка", ЗначенияЯзыковыхКонстант.ОсновнойЯзык.НовоеЗначение);
								...
								Если ЗначениеЗаполнено(ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение) 
								   И РеквизитПрисутствуетВТабличнойЧасти Тогда
									...
									Отбор = Новый Структура("КодЯзыка", ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение);
									НайденныеСтроки = ИзменяемыйОбъект.Представления.НайтиСтроки(Отбор);
									Если НайденныеСтроки.Количество() > 0 Тогда
										СтрокаДанных = НайденныеСтроки[0];
									Иначе
										СтрокаДанных= ИзменяемыйОбъект.Представления.Добавить();
										СтрокаДанных.КодЯзыка= ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение;
									КонецЕсли;
									СтрокаДанных[МультиязычныйРеквизит] = ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение];
								КонецЕсли;
						...
						Для каждого ДополнительныйЯзыкИзменен Из ДанныеДляИзмененияМультиязычныхРеквизитов.Изменения Цикл
							
							Если ДополнительныйЯзыкИзменен.Значение Тогда
								ЗначенияЯзыка = ДанныеДляИзмененияМультиязычныхРеквизитов.НастройкиИзмененияЯзыков[ДополнительныйЯзыкИзменен.Ключ];
								
								НовоеЗначение = ЗначенияРеквизита[ЗначенияЯзыка.НовоеЗначение];
								ЗаполнитьРеквизитИТЧПредставления(ПараметрыЗаполнения, МультиязычныйРеквизит, ИзменяемыйОбъект, НовоеЗначение, ЗначениеПоУмолчанию,
									ЗначенияЯзыковыхКонстант[ДополнительныйЯзыкИзменен.Ключ].НовоеЗначение);
						...
						Если НоваяСтрокаТЧПредставления <> Неопределено Тогда
							НоваяСтрокаТЧПредставления[МультиязычныйРеквизит] = ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение];
						КонецЕсли;
						
					КонецЦикла;
					
					Если ОтредактированныеРеквизиты <> Неопределено Тогда
						ИзменяемыйОбъект.ОтредактированныеПредопределенныеРеквизиты = СтрСоединить(ОтредактированныеРеквизиты, ",");
					КонецЕсли;
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, 
						МетаданныеОбъекта, ПредставлениеИзменяемогоОбъекта,
...
Процедура ЗаполнитьРеквизитИТЧПредставления(ПараметрыЗаполнения, МультиязычныйРеквизит, ИзменяемыйОбъект,
	НовоеЗначение, ЗначениеПоУмолчанию, КодЯзыка = "")
	
	ЗначениеЗаполнения = "";
	ЕстьПараметрыЗаполнения  = ПараметрыЗаполнения.Количество() <> 0;
	ОбъектСодержитТЧПредставления = ?(ЕстьПараметрыЗаполнения, ПараметрыЗаполнения.ОбъектСодержитТЧПредставления,Ложь);
...
Процедура ЗаписатьДанныеДляИзмененияМультиязычныхРеквизитов(ДанныеДляИзмененияМультиязычныхРеквизитов)
	...
	Иначе
		УпакованноеЗначение = Новый ХранилищеЗначения(ДанныеДляИзмененияМультиязычныхРеквизитов, Новый СжатиеДанных(9));
	КонецЕсли;
...
Процедура ИзменитьЯзыкВПодчиненныхРегистрахСведений(ОбъектМетаданных, ДанныеОбъекта, ДанныеДляИзмененияМультиязычныхРеквизитов, ПараметрыЗаполнения, Периодический = Ложь)
	...
	СведенияОЯзыках = СведенияОЯзыках();
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			НачатьТранзакцию();
				Попытка
					...
					Для Каждого ИзменяемаяЗапись Из ТекущийНаборЗаписей Цикл
						...
						Для Каждого МультиязычныйРеквизит Из МультиязычныеРеквизиты Цикл
							...
							// Сохраняем старые значения
							ЗначенияРеквизита = Новый Соответствие;
							ЗначенияРеквизита[ЗначенияЯзыковыхКонстант.ОсновнойЯзык.СтароеЗначение] =  ИзменяемаяЗапись[МультиязычныйРеквизит]; 
							Для ПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков() Цикл
								ИмяКонстантыЯзыка = ИмяКонстантыЯзыка(ПорядковыйНомерЯзыка);
								ИмяСуффиксаЯзыка   = МультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
								ЗначенияРеквизита[ЗначенияЯзыковыхКонстант[ИмяКонстантыЯзыка].СтароеЗначение] = ИзменяемаяЗапись[МультиязычныйРеквизит + ИмяСуффиксаЯзыка];
							КонецЦикла;
							...
							Для Каждого ДополнительныйЯзыкИзменен Из ДанныеДляИзмененияМультиязычныхРеквизитов.Изменения Цикл
								Если ДополнительныйЯзыкИзменен.Значение Тогда
									...
									НовоеЗначение = ЗначенияРеквизита[ЗначенияЯзыковыхКонстант[ДополнительныйЯзыкИзменен.Ключ].НовоеЗначение];
									ЗаполнитьРеквизитИТЧПредставления(ПараметрыЗаполнения, МультиязычныйРеквизит, ИзменяемаяЗапись, НовоеЗначение, ЗначениеПоУмолчанию,
										ЗначенияЯзыковыхКонстант[ДополнительныйЯзыкИзменен.Ключ].НовоеЗначение);
								КонецЕсли;
							КонецЦикла;
				...
				Исключение
					...
					МетаданныеОбъекта = Метаданные.НайтиПоПолномуИмени(ПолноеИмя);
					ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
						МетаданныеОбъекта,, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ОписаниеОшибки));
...
Функция ОбъектыСНачальнымЗаполнением(ВариантДанных)
	...
	ОбъектыСПредопределеннымиЭлементамиПодВариантДанных = Новый Соответствие(ОбъектыСПредопределеннымиЭлементами);
	
	Для Каждого ОбъектСПредопределеннымЭлементом Из ОбъектыСПредопределеннымиЭлементами Цикл
		
		ЭтоРазделенныйОбъектМетаданных = МодульРаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(
			ОбъектСПредопределеннымЭлементом.Ключ);
		Если ВариантДанных = "Общие" И ЭтоРазделенныйОбъектМетаданных Тогда
			ОбъектыСПредопределеннымиЭлементамиПодВариантДанных.Удалить(ОбъектСПредопределеннымЭлементом.Ключ);
	...
	Возврат ОбъектыСПредопределеннымиЭлементамиПодВариантДанных;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Мультиязычность'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИзменитьЯзыкВМультиязычныхРеквизитахКонфигурации", Истина);
