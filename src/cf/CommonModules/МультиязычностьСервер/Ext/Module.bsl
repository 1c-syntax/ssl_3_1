...
Процедура ОбработатьОбщиеДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	Для каждого ОбъектыКОбработке Из ОбъектыКОбработке Цикл
		
		Если ОбъектМетаданных = Неопределено Или ТипЗнч(ОбъектМетаданных) <> ТипЗнч(ОбъектыКОбработке) Тогда
			ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнч(ОбъектыКОбработке));
			ПараметрыЗаполнения = ОбновлениеИнформационнойБазыСлужебный.НаборПараметровДляЗаполненияОбъекта(ОбъектМетаданных);
		КонецЕсли;
		
		Если ПараметрыЗаполнения <> Неопределено Тогда
			ЗаполнитьИПеренестиСтрокиИзТЧПредставленияВРеквизиты(ОбъектыКОбработке, ПараметрыЗаполнения, ОбъектовОбработано, ПроблемныхОбъектов);
		КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого ИмяОбъектСТЧПредставления Из ОбъектыСТЧПредставления Цикл
		...
		ОбрабатываемыеОбъекты.Добавить(ИмяОбъектСТЧПредставления);
			...
			Если ОбъектМетаданных.Иерархический 
				И ОбъектМетаданных.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Тогда
					...
					Если ТабличнаяЧастьОбъекта.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента Тогда
						...
						|ГДЕ СправочникСТЧПредставления.ЭтоГруппа <> ИСТИНА";
