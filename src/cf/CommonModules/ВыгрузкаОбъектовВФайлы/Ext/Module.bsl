...
Функция ТаблицаОбластей(Макет)
	...
	АктуализироватьОбластиУсловий(ТаблицаОбластей);
...
Процедура АктуализироватьОбластиУсловий(ТаблицаОбластей)
	
	ТаблицаОбластей.Сортировать("Верх, Низ Убыв, Приоритет");
	
	Для Каждого Область Из ТаблицаОбластей Цикл
		
		Если НЕ Область.ЭтоОбластьУсловияВывода Тогда
			Продолжить;
		КонецЕсли;
		
		Отбор = Новый Структура;
		Отбор.Вставить("ЭтоОбластьУсловияВывода", Истина);
		Отбор.Вставить("ПараметрРасшифровки", Область.ПараметрРасшифровки);
		НайденныеСтроки = ТаблицаОбластей.НайтиСтроки(Отбор);
		Для Каждого ТекущаяСтрока Из НайденныеСтроки Цикл
			
			Если Область.Низ <= ТекущаяСтрока.Низ
			   И Область.Верх > ТекущаяСтрока.Верх Тогда
				
				Область.ЭтоОбластьУсловияВывода = Ложь;
				Область.Приоритет = 0;
				Прервать;
