...
Процедура ПерезапуститьОтложенноеОбновление(Отбор = Неопределено) Экспорт
	...
	Попытка
		...
		ВременныеОчереди = Запрос.Выполнить().Выгрузить();
	...
	Если ТипЗнч(Отбор) = Тип("Массив") Тогда
		// При передаче отбора по подсистемам незавершенные обработчики добавляются заново.
...
// Регистрирует проблему с данными, выявленную при выполнении обработчика обновления в подсистеме
...
Процедура УдалитьРегистрациюИзмененийПодчиненногоРегистра(Узел, Регистраторы, ПолноеИмяОбъекта)
	...
	Для Каждого Регистратор Из МассивЭлементов(Регистраторы) Цикл
		...
		ЗаписатьПрогрессВыполненияОбработчика(Набор, Узел);
...
Процедура УдалитьРегистрациюИзмененийНезависимогоРегистра(Узел, Записи, ПолноеИмяОбъекта)
	...
	Для Каждого Запись Из Записи Цикл
		...
		ЗаписатьПрогрессВыполненияОбработчика(Набор, Узел, МетаданныеРегистра);
		УдалитьРегистрациюИзмененийЭлементаДанных(Набор, ПараметрыУдаления);
	КонецЦикла;
...
Процедура ЗаписатьПрогрессВыполненияОбработчика(Данные, Узел, МетаданныеОбъекта = Неопределено)
	...
	МенеджерЗаписи.ИмяОбработчика = ПараметрыОбработчикаОбновления.ИмяОбработчика;
	МенеджерЗаписи.КлючЗаписи = ПараметрыОбработчикаОбновления.КлючЗаписиПрогрессаОбновления;
	МенеджерЗаписи.ИнтервалЧас = ИнтервалЧас;
	МенеджерЗаписи.Прочитать();
	Если Не МенеджерЗаписи.Выбран() Тогда
		МенеджерЗаписи.ИмяОбработчика = ПараметрыОбработчикаОбновления.ИмяОбработчика;
		МенеджерЗаписи.КлючЗаписи = ПараметрыОбработчикаОбновления.КлючЗаписиПрогрессаОбновления;
		МенеджерЗаписи.ИнтервалЧас = ИнтервалЧас;
		МенеджерЗаписи.ОбъектовОбработано = ОбъектовОбработано;
	Иначе
		МенеджерЗаписи.ОбъектовОбработано = МенеджерЗаписи.ОбъектовОбработано + ОбъектовОбработано;
	КонецЕсли;
