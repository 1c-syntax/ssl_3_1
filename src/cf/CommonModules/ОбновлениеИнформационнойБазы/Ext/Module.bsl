...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОсновныеПараметры.Свойство("ПараметрыВыборки")
		...
		Если Несуществующие.Количество() <> 0 Тогда
			ТекстИсключения = НСтр("ru = 'В свойстве ПараметрыВыборки процедуры заполнения данных отложенного обработчика указаны несуществующие объекты:
				|%1.'");
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, СтрСоединить(Несуществующие, ", "));
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
