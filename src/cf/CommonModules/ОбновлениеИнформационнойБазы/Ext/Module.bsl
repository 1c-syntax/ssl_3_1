...
//  Данные - Ссылка
//         - Массив
//         - НаборДанных - данные, по которым нужно зарегистрировать изменения.
...
//  Данные            - Ссылка
//                    - Массив
//                    - НаборЗаписей - данные, по которым нужно зарегистрировать изменения.
//                    - ТаблицаЗначений - значения измерений независимого регистра сведений. Требования:
...
Функция СоздатьВременнуюТаблицуСсылокДляОбработки(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ДополнительныеПараметры.ВыбиратьПорциями Тогда
		ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса, "ВЫБРАТЬ ПЕРВЫЕ 10000", "ВЫБРАТЬ");
	КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл
		...
		Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) Тогда
			...
			|			ИСТИНА
			|		ИЗ
			|			#ТаблицаОбъекта КАК #ИмяОбъекта
			|		ГДЕ
			|			#ИмяОбъекта.Ссылка = ТаблицаИзменений.Ссылка)";
...
Функция ВыполнитьОбновлениеИнформационнойБазы(ВыполнятьОтложенныеОбработчики = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		МодульТехнологияСервисаБазоваяФункциональность = ОбщегоНазначения.ОбщийМодуль("ТехнологияСервиса");
		
		ВерсияБиблиотекиБТС = МодульТехнологияСервисаБазоваяФункциональность.ВерсияБиблиотеки();
		ТребуетсяУстановкаРасширения = ОбновлениеИнформационнойБазыСлужебный.ВесВерсии("2.0.0.0") < ОбновлениеИнформационнойБазыСлужебный.ВесВерсии(ВерсияБиблиотекиБТС);
		Если ТребуетсяУстановкаРасширения Тогда
			// АПК:278-выкл Выполнение обработчиков обновления с установленным расширением БТС при включенном разделении.
			// БТС не предоставляет программного интерфейса для корректного вызова обработчиков.
...
//                             Доступные значения: "Оперативное", "Отложенное", "ОбщиеДанные".
//
// Возвращаемое значение:
//  Структура:
//     * Обновлено - Массив - обновленные области.
//                 - Булево - признак окончания обновления для режима ОбщиеДанные.
//     * Выполняется - Массив из Число - области, где обработчики выполняются.
//                   - Булево - признак выполнения обновления для режима ОбщиеДанные.
//     * Ожидают - Массив из Число - области, в которых отложенное обновление еще не началось
//               - Булево - признак окончания обновления для режима выполнения ОбщиеДанные.
//     * Проблемы - Массив из Число - области, где обновление завершилось с ошибкой.
//                - Булево - признак, что обновление завершилось с ошибкой для режима ОбщиеДанные.
//     * ОбластиСПроблемами - Массив из Число - только для режимов "Оперативное" и "Отложенное".
//     * ОбластиВыполняется - Массив из Число - только для режимов "Оперативное" и "Отложенное".
//
Функция ПрогрессОбновленияОбластейДанных(РежимОбновления) Экспорт
	
	РежимОбновленияДляЗапроса = Новый Массив;
	Если РежимОбновления = "Отложенное" Тогда
		РежимОбновленияДляЗапроса.Добавить(Перечисления.РежимыВыполненияОбработчиков.Отложенно);
	ИначеЕсли РежимОбновления = "Оперативное" Или РежимОбновления = "ОбщиеДанные" Тогда
		РежимОбновленияДляЗапроса.Добавить(Перечисления.РежимыВыполненияОбработчиков.Оперативно);
		РежимОбновленияДляЗапроса.Добавить(Перечисления.РежимыВыполненияОбработчиков.Монопольно);
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректное значение параметра ""РежимОбновления"".
			|Доступные значения: ""Оперативное"", ""Отложенное"", ""ОбщиеДанные""'");
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СтатусВыполняется", Перечисления.СтатусыОбработчиковОбновления.Выполняется);
	Запрос.УстановитьПараметр("СтатусОшибка", Перечисления.СтатусыОбработчиковОбновления.Ошибка);
	Запрос.УстановитьПараметр("СтатусНеВыполнялся", Перечисления.СтатусыОбработчиковОбновления.НеВыполнялся);
	Запрос.УстановитьПараметр("СтатусВыполнен", Перечисления.СтатусыОбработчиковОбновления.Выполнен);
	Запрос.УстановитьПараметр("РежимВыполнения", РежимОбновленияДляЗапроса);
	Запрос.УстановитьПараметр("ОбластиОбновленныеНаВерсию", ОбластиОбновленныеНаВерсию);
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОбработчикиОбновления.ОбластьДанныхВспомогательныеДанные КАК ОбластьДанных,
		|	КОЛИЧЕСТВО(ВЫБОР
		|			КОГДА ОбработчикиОбновления.Статус = &СтатусВыполняется
		|				ТОГДА ОбработчикиОбновления.Статус
		|		КОНЕЦ) КАК Выполняется,
		|	КОЛИЧЕСТВО(ВЫБОР
		|			КОГДА ОбработчикиОбновления.Статус = &СтатусОшибка
		|				ТОГДА ОбработчикиОбновления.Статус
		|		КОНЕЦ) КАК Ошибки,
		|	КОЛИЧЕСТВО(ВЫБОР
		|			КОГДА ОбработчикиОбновления.Статус = &СтатусНеВыполнялся
		|				ТОГДА ОбработчикиОбновления.Статус
		|		КОНЕЦ) КАК НеВыполнялись,
		|	КОЛИЧЕСТВО(ВЫБОР
		|			КОГДА ОбработчикиОбновления.Статус = &СтатусВыполнен
		|				ТОГДА ОбработчикиОбновления.Статус
		|		КОНЕЦ) КАК Выполнены
		|ПОМЕСТИТЬ СтатистикаОбновленияПоОбластям
		|ИЗ
		|	РегистрСведений.ОбработчикиОбновления КАК ОбработчикиОбновления
		|ГДЕ
		|	ОбработчикиОбновления.РежимВыполнения В(&РежимВыполнения)";
	
	Если РежимОбновления <> "ОбщиеДанные" Тогда
		Запрос.Текст = Запрос.Текст + Символы.ПС + Символы.ПС
			+ "СГРУППИРОВАТЬ ПО
			|	ОбработчикиОбновления.ОбластьДанныхВспомогательныеДанные"
	КонецЕсли;
	...
	ТекстЗапроса =
		...
		|ГДЕ
		|	СтатистикаОбновленияПоОбластям.Выполнены <> 0
		|	И СтатистикаОбновленияПоОбластям.Выполняется = 0
		|	И СтатистикаОбновленияПоОбластям.Ошибки = 0
		|	И СтатистикаОбновленияПоОбластям.НеВыполнялись = 0
		|	И СтатистикаОбновленияПоОбластям.ОбластьДанных В(&ОбластиОбновленныеНаВерсию)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СтатистикаОбновленияПоОбластям КАК СтатистикаОбновленияПоОбластям
		|ГДЕ
		|	СтатистикаОбновленияПоОбластям.Выполняется <> 0
		|	И СтатистикаОбновленияПоОбластям.Ошибки = 0
		|	И СтатистикаОбновленияПоОбластям.ОбластьДанных В(&ОбластиОбновленныеНаВерсию)
		|;
		|
		...
		|	СтатистикаОбновленияПоОбластям КАК СтатистикаОбновленияПоОбластям
		|ГДЕ
		|	СтатистикаОбновленияПоОбластям.Ошибки <> 0
		|	И СтатистикаОбновленияПоОбластям.ОбластьДанных В(&ОбластиОбновленныеНаВерсию)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|				И СтатистикаОбновленияПоОбластям.Ошибки = 0
		|				И СтатистикаОбновленияПоОбластям.НеВыполнялись <> 0
		|			ИЛИ НЕ СтатистикаОбновленияПоОбластям.ОбластьДанных В (&ОбластиОбновленныеНаВерсию))";
	Запрос.Текст = Запрос.Текст + Символы.ПС + ";" + Символы.ПС + ТекстЗапроса;
	Если РежимОбновления = "ОбщиеДанные" Тогда
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст,
			"РегистрСведений.ОбработчикиОбновления",
			"РегистрСведений.ОбработчикиОбновленияОбщихДанных");
	...
	ПрогрессОбновления = Новый Структура;
	Если РежимОбновления = "ОбщиеДанные" Тогда
		ПрогрессОбновления.Вставить("Обновлено", РезультатЗапроса[1].Выгрузить().Количество() > 0);
		ПрогрессОбновления.Вставить("Проблемы", РезультатЗапроса[3].Выгрузить().Количество() > 0);
		ПрогрессОбновления.Вставить("Выполняется", ВыполняетсяОбновлениеИнформационнойБазы());
		Ожидает = Не ПрогрессОбновления.Обновлено
			И Не ПрогрессОбновления.Проблем
			И Не ПрогрессОбновления.Выполняется
			И НеобходимоОбновлениеИнформационнойБазы();
		ПрогрессОбновления.Вставить("Ожидают", Ожидает);
	Иначе
		Выполняются = РезультатЗапроса[2].Выгрузить();
		Проблемы = РезультатЗапроса[3].Выгрузить();
		
		ПрогрессОбновления.Вставить("Обновлено", РезультатЗапроса[1].Выгрузить().Количество());
		ПрогрессОбновления.Вставить("Выполняется", Выполняются.Количество());
		ПрогрессОбновления.Вставить("ОбластиВыполняется", Выполняются.ВыгрузитьКолонку("ОбластьДанных"));
		ПрогрессОбновления.Вставить("Проблемы", Проблемы.Количество());
		ПрогрессОбновления.Вставить("ОбластиСПроблемами", Проблемы.ВыгрузитьКолонку("ОбластьДанных"));
		ПрогрессОбновления.Вставить("Ожидают", РезультатЗапроса[4].Выгрузить().Количество());
	КонецЕсли;
...
// В режиме сервиса, если не задан отбор по области данных, что возвращаются
...
Функция ОбработчикиОбновления(Отбор = Неопределено) Экспорт
	...
	УсловиеЗапроса = "";
	Если Отбор.Свойство("РежимВыполнения") Тогда
		УсловиеЗапроса = "ОбработчикиОбновления.РежимВыполнения = &РежимВыполнения";
		Запрос.УстановитьПараметр("РежимВыполнения", Перечисления.РежимыВыполненияОбработчиков[Отбор.РежимВыполнения]);
	КонецЕсли;
	
	Если Отбор.Свойство("Статус") Тогда
		Если ЗначениеЗаполнено(УсловиеЗапроса) Тогда
			УсловиеЗапроса = УсловиеЗапроса + Символы.ПС + " И " + "ОбработчикиОбновления.Статус = &Статус";
		Иначе
			УсловиеЗапроса = "ОбработчикиОбновления.Статус = &Статус";
		КонецЕсли;
		Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыОбработчиковОбновления[Отбор.Статус]);
	КонецЕсли;
	
	Если Отбор.Свойство("ОбластьДанных") Тогда
		Если ЗначениеЗаполнено(УсловиеЗапроса) Тогда
			УсловиеЗапроса = УсловиеЗапроса + Символы.ПС + " И " + "ОбработчикиОбновления.ОбластьДанныхВспомогательныеДанные = &ОбластьДанных";
		Иначе
			УсловиеЗапроса = "ОбработчикиОбновления.ОбластьДанныхВспомогательныеДанные = &ОбластьДанных";
		КонецЕсли;
		Запрос.УстановитьПараметр("ОбластьДанных", Отбор.ОбластьДанных);
	КонецЕсли;
...
Функция ОбновляемыеОбъекты() Экспорт
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Для Каждого ИзменяемыйОбъект Из ИзменяемыеОбъектыЧастями Цикл
			...
			КонецЕсли;
			ОбновляемыеОбъекты[ИзменяемыйОбъект].Добавить(Обработчик.ИмяОбработчика);
		КонецЦикла;
	КонецЦикла;
...
//  ВременныеТаблицы - Соответствие - где в ключе объект метаданных, а в значении имя соответствующей ему
//                     временной таблицы.
//
...
//
// Возвращаемое значение:
//  Соответствие, Произвольный - новое или найденное по ключу значение.
//
...
Функция ТекстЗапросаВТЗаблокированныхПоТЧ(РеквизитыТЧ, ПолноеИмяОбъекта, ПолноеИмяРегистра, ВременныеТаблицы)
	...
	ШаблонЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаИзменений.%1 КАК Ссылка
		|ПОМЕСТИТЬ ЗаблокированныеПоТЧ
		|ИЗ
		|	%2.Изменения КАК ТаблицаИзменений
		|ГДЕ
		|	%3
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка";
	ШаблонУсловия =
		...
		|			ИСТИНА
		|		ИЗ
		|			%1.%2 КАК ТЧДокумента%2
		|		ГДЕ
		|			ТаблицаИзменений.%3 = ТЧДокумента%2.Ссылка
		|			И (%4))";
	ШаблонУсловияПоЗаблокированным =
		...
		|					ГДЕ
		|						%2)";
	ШаблонУсловияПоРеквизиту = "ТЧДокумента%1.%2 = %3.Ссылка";
	РазделительУсловий =
		"
		|	ИЛИ ";
	...
	Для Каждого ОписаниеТабличнойЧасти Из РеквизитыТЧ Цикл
		...
		Для Каждого ОписаниеМетаданных Из МетаданныеРеквизитов Цикл
			...
			УсловиеПоЗаблокированным = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонУсловияПоЗаблокированным,
				ТаблицаЗаблокированных,
...
Функция ТекстУсловияЗаблокированныхПоТЧ(ПолноеИмяОбъекта, ПолноеИмяРегистра)
	...
	Поле = ?(ПолноеИмяРегистра <> Неопределено, "Регистратор", "Ссылка");
	ШаблонУсловия =
		"НЕ ИСТИНА В (
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			ИСТИНА
		...
		|		ГДЕ
		|			ТаблицаИзменений.%1 = ЗаблокированныеПоТЧ.Ссылка)";
