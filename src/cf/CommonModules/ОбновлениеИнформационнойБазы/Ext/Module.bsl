...
#Область ПроцедурыИФункцииДляИспользованияВОбработчикахОбновления
...
Процедура ЗаписатьНаборЗаписей(Знач НаборЗаписей, Замещать = Истина, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено,
	...
	ОбновлениеИнформационнойБазыПереопределяемый.ПослеЗаписиДанных(НаборЗаписей);
...
#Область ПроцедурыИФункцииДляПроверкиДоступностиОбъектаПриВыполненииОтложенногоОбновления
...
Процедура ПроверитьОбъектОбработан(Данные, Форма = Неопределено, ИмяОтложенногоОбработчика = "", ИмяПроцедурыПрограммногоИнтерфейса = "") Экспорт
	...
	Если Не ЭтоВызовИзОбработчикаОбновления() Тогда
		...
		Если Форма.Команды.Найти("ОбновлениеВерсииИБ_ОбъектЗаблокирован") = Неопределено Тогда
...
Функция ОбъектОбработан(Данные) Экспорт
	...
	Для Каждого Обработчик Из ОбработчикиОбъекта Цикл
		...
		ИначеЕсли ТипЗнч(Данные) = Тип("Строка") Тогда
			Обработан = Ложь;
		Иначе
			Если СвойстваОбработчика.ПроцедураПроверки = ОбновлениеИнформационнойБазыСлужебный.СтандартноеИмяПроцедурыПроверки() Тогда
				//@skip-check query-in-loop - проверка должна выполняться для каждого обработчика отдельно.
				Обработан = ДанныеОбновленыНаНовуюВерсиюПрограммы(МетаданныеИОтбор);
			Иначе
				Обработан = ОбщегоНазначения.ВыполнитьМетодКонфигурации(СвойстваОбработчика.ПроцедураПроверки,
	...
	Если НезавершенныеОбработчики.Количество() > 0 Тогда
		...
		ЧастиИсключения.Добавить(НСтр("ru = 'Действия с объектом временно запрещены, так как не завершен переход на новую версию приложения.
...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Данные) = Тип("Массив") И Данные.Количество() = 0
		Или ТипЗнч(Данные) = Тип("ТаблицаЗначений") И Данные.Количество() = 0
		Или ТипЗнч(Данные) = Тип("РезультатЗапроса") И Данные.Пустой() Тогда
		Возврат;
	КонецЕсли;
	...
	Иначе
		Если ТипЗнч(Данные) = Тип("Массив") Или ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Данные))
			ИЛИ (ТипЗнч(Данные) = Тип("РезультатЗапроса") И Данные.Колонки.Найти("Ссылка") <> Неопределено) Тогда
...
Функция ДополнительныеПараметрыВыборкиДанныхДляМногопоточнойОбработки() Экспорт
	...
	ДополнительныеПараметры.Вставить("АвтоматическаяВыборкаПорции", Ложь);
...
Функция ВыбратьРегистраторыРегистраДляОбработки(Очередь, ПолноеИмяДокумента, ПолноеИмяРегистра, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПроверитьПараметрыМногопоточности(ДополнительныеПараметры, ПолноеИмяРегистра); // АПК:1036 Орфография не нарушена
...
Функция ВыбратьСсылкиДляОбработки(Очередь, ПолноеИмяОбъекта, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПроверитьПараметрыМногопоточности(ДополнительныеПараметры, ПолноеИмяОбъекта); // АПК:1036 Орфография не нарушена
...
Функция ВыбратьИзмеренияНезависимогоРегистраСведенийДляОбработки(Очередь, ПолноеИмяОбъекта, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ФильтрАктуальныхДанных = ОбновлениеИнформационнойБазыСлужебный.ФильтрАктуальныхДанных(ПараметрыОбработчикаОбновления, 
		"ТаблицаИзменений");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеАктуальныеДанные", ФильтрАктуальныхДанных.Условие);
	Если ФильтрАктуальныхДанных.ЕстьФильтр Тогда
		ТекстЗапросаПроверки =
	...
	ДобавитьПроверкуБлокировкиДополнительныхИсточниковДляНезависимогоРегистра(Очередь,
		ТекстЗапроса, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры);	
...
//   * ИмяВременнойТаблицы - Строка - имя созданной временной таблицы.
//
Функция СоздатьВременнуюТаблицуИзмеренийНезависимогоРегистраСведенийДляОбработки(Очередь, ПолноеИмяОбъекта, 
	МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'Неправильный тип параметра ""%1"" при вызове функции %2.'");
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, 
...
Функция ОбработкаДанныхЗавершена(Очередь, ПолноеИмяМетаданныеОбъекта = "", Отбор = Неопределено) Экспорт
	...
	Если ПолноеИмяМетаданныеОбъекта = "" Тогда
		МассивИменМетаданных = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
			ПараметрыСеанса.ПараметрыОбработчикаОбновления.ИзменяемыеОбъекты, ",", , Истина);
		Если МассивИменМетаданных.Количество() = 1 Тогда
			ПолноеИмяМетаданныеОбъекта = ПараметрыСеанса.ПараметрыОбработчикаОбновления.ИзменяемыеОбъекты;
		ИначеЕсли МассивИменМетаданных.Количество() > 1 Тогда
			ТекстСообщения = НСтр("ru = 'Необходимо указать параметр %1 при вызове функции %2.'");
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, 
				"ПолноеИмяМетаданныеОбъекта", "ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена");
			ВызватьИсключение(ТекстСообщения, КатегорияОшибки.ОшибкаКонфигурации);
		Иначе
			ТекстСообщения = НСтр("ru = 'Необходимо указать свойство %1 обработчика обновления.'");
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, "ИзменяемыеОбъекты");
			ВызватьИсключение(ТекстСообщения, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	КонецЕсли;
...
//                                      Важно: Свойство игнорируется для обработчиков с порядком Некритичный, такие
//                                      обработчики не должны блокировать работу пользователей с объектом.
...
Функция ДанныеДляОбновленияВМногопоточномОбработчике(Параметры) Экспорт
	...
	ОписаниеТипаСсылки = Неопределено;
	НесколькоНаборовДанных = Ложь;
	Для Каждого ЭлементНабора Из НаборДанных Цикл
		ТаблицаДанных = ЭлементНабора.Данные;
		КолонкаСсылка = ТаблицаДанных.Колонки.Найти("Ссылка");
		Если КолонкаСсылка = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ОписаниеТипаСсылки = Неопределено Тогда
			ОписаниеТипаСсылки = КолонкаСсылка.ТипЗначения;
			Продолжить;
		КонецЕсли;
		
		НесколькоНаборовДанных = Истина;
		ОписаниеТипаСсылки = Новый ОписаниеТипов(ОписаниеТипаСсылки, КолонкаСсылка.ТипЗначения.Типы());
	КонецЦикла;
	
	Если НаборДанных.Количество() > 0 Тогда
		Данные = НаборДанных[0].Данные.Скопировать();
		КолонкаСсылка = Данные.Колонки.Найти("Ссылка");
		Если КолонкаСсылка <> Неопределено И НесколькоНаборовДанных Тогда
			МассивСсылок = Данные.ВыгрузитьКолонку("Ссылка");
			Данные.Колонки.Удалить(КолонкаСсылка);
			Данные.Колонки.Добавить("Ссылка", ОписаниеТипаСсылки);
			Данные.ЗагрузитьКолонку(МассивСсылок, "Ссылка");
		КонецЕсли;
...
Функция ПрогрессОбновленияОбластейДанных(РежимОбновления) Экспорт
	...
	|			ИНАЧЕ &ПорядокСостоянияВыполняется
...
// Возвращает Истина, если доступен просмотр изменений в новой версии приложения.
//
// Возвращаемое значение:
//  Булево
//
Функция ДоступенПросмотрОписанияИзмененийПрограммы() Экспорт
	
	Возврат ПравоДоступа("Просмотр", Метаданные.ОбщиеФормы.ОписаниеИзмененийПрограммы);
...
//
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры, ОбъектМетаданных = Неопределено, 
	ДополнительныеПараметры = Неопределено) Экспорт
	
	ОбновлениеИнформационнойБазыСлужебный.ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры, ОбъектМетаданных, 
...
// Обновляет предопределенные элементы указанного объекта метаданных по правилам, заданным
// в обработчике начального заполнения (ПриНачальномЗаполненииЭлементов) модуля менеджера объекта.
//
// Согласно стандарту Использование предопределенных элементов, в подчиненных узлах РИБ обновление не выполняется,
// так как предопределенные элементы там не создаются автоматически и не обновляются при изменении метаданных;
// они должны быть получены из главного узла вместе с изменениями конфигурации.
//
// Параметры:
//  ОбъектМетаданных     - ОбъектМетаданных - справочник или план видов характеристик, для которого
//                         требуется обновить предопределенные элементы.
//  ПараметрыОбновления  - см. ПараметрыОбновленияПредопределенныхЭлементов
//                       - Неопределено - обновить все предопределенные элементы.
//
Процедура ОбновитьПредопределенныеЭлементы(ОбъектМетаданных, ПараметрыОбновления = Неопределено) Экспорт
	...
	ПараметрыОбновленияПредопределенныхЭлементов = ?(ПараметрыОбновления = Неопределено,
		ПараметрыОбновленияПредопределенныхЭлементов(), ПараметрыОбновления);
	ОбновлениеИнформационнойБазыСлужебный.ОбновитьПредопределенныеЭлементыОбъекта(ОбъектМетаданных, ПараметрыОбновленияПредопределенныхЭлементов);
...
Процедура УстановитьНедостающиеОтборыВНаборе(Набор, МетаданныеНабора, УстанавливаемыеОтборы)
	Для Каждого Измерение Из МетаданныеНабора.Измерения Цикл
		...
		Если ТипЗнч(УстанавливаемыеОтборы) = Тип("ТаблицаЗначений")
			ИЛИ ТипЗнч(УстанавливаемыеОтборы) = Тип("РезультатЗапроса") Тогда
	...
	Если МетаданныеНабора.ОсновнойОтборПоПериоду Тогда
		...
		Если ТипЗнч(УстанавливаемыеОтборы) = Тип("ТаблицаЗначений") 
			ИЛИ ТипЗнч(УстанавливаемыеОтборы) = Тип("ВыборкаИзРезультатаЗапроса") Тогда
...
Процедура ЗарегистрироватьИзмененияОбъекта(Параметры, Узел, Ссылки, ВидДанных, ПолноеИмяОбъекта = "")
	...
	Если ТипЗнч(Ссылки) = Тип("РезультатЗапроса") Тогда
		
		Выборка = Ссылки.Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.Ссылка.Пустая() Тогда
				Продолжить;
			КонецЕсли;
			
			ЗарегистрироватьИзмененияЭлементаДанных(Выборка.Ссылка, ПараметрыРегистрации);
		КонецЦикла;
...
// Зарегистрировать изменения независимого регистра по таблице значений или результату запроса
// где колонки - это измерения, а строки - это регистрируемые записи.
//
// Параметры:
//  Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке.
//  Узел - ПланОбменаСсылка.ОбновлениеИнформационнойБазы.
//  Записи - ТаблицаЗначений или РезультатЗапроса.
//  ВидДанных - Строка.
//  ПолноеИмяОбъекта - Строка.
//
Процедура ЗарегистрироватьИзмененияНезависимогоРегистра(Параметры, Узел, Записи, ВидДанных, ПолноеИмяОбъекта)
	...
	Если ТипЗнч(Записи) = Тип("ТаблицаЗначений") Тогда
		
		Для Каждого Запись Из Записи Цикл
			ЗарегистрироватьИзмененияНабораНезависимогоРегистраСведений(ПараметрыРегистрации, 
				Узел, 
				МетаданныеРегистра, 
				Запись, 
				Записи);
	...
	ИначеЕсли ТипЗнч(Записи) = Тип("РезультатЗапроса") Тогда
		
		ВыборкаЗаписи = Записи.Выбрать();
...
// Зарегистрировать изменения набор независимого регистра по таблице значений или результату запроса
// где колонки - это измерения, а строки - это регистрируемые записи.
//
// Параметры:
//  Параметры - см. НовыеПараметрыРегистрации.
//  Узел - ПланОбменаСсылка.ОбновлениеИнформационнойБазы.
//  МетаданныеРегистра - ОбъектМетаданных независимого регистра.
//  Запись - СтрокаТаблицыЗначений или ВыборкаИзРезультатаЗапроса.
//  Записи - ТаблицаЗначений или РезультатЗапроса.
//
Процедура ЗарегистрироватьИзмененияНабораНезависимогоРегистраСведений(Параметры, Узел, МетаданныеРегистра, Запись, Записи)
	
	Набор = ПолучитьПереиспользуемыйНабор(Параметры.ПереиспользуемыеНаборы);
	...
	ЗарегистрироватьИзмененияЭлементаДанных(Набор, Параметры);
...
//   * ИспользуемыеПоляУпорядочивания - Соответствие - кэш полей, которые уже использованы для упорядочивания.
...
Функция ВыбратьДанныеПостранично(Запрос, ПараметрыПостроения)
	...
	Иначе
		ВыбратьДиапазон = ПерваяЗапись <> Неопределено Или ПоследняяЗапись <> Неопределено;
...
Процедура УстановитьУсловияВыборкиПоСтраницам(Запрос, Условия, Таблица, Параметры, Первые, НомерУсловия = Неопределено)
	...
	ВыбратьДиапазон = ПерваяЗапись <> Неопределено
	              Или ПоследняяЗапись <> Неопределено;
	...
	Для ИндексСтроки = ИндексНачала По ИндексОкончания Цикл
		...
		Для ИндексКолонки = 0 По КоличествоКолонок - 1 Цикл
			...
			Если Не ПустаяСтрока(Оператор) Тогда
				...
				Если ЭтоИмяКолонкиКонцаДиапазона(Колонка.Имя) Тогда
...
// Добавить одно условие через И в существующий ряд условий.
//
// Параметры:
//  Условия - СтрокаТаблицыЗначений из см. НовыеУсловияПостраничнойВыборки
//  ПоляВыборки - Массив - выбираемые запросом поля.
//  Количество - Число - количество полей выборки с начала, которые нужно использовать для формировании условий.
//  Оператор - Строка - оператор, который добавляется в последнее условие.
//  ДляДиапазона - Булево - Ложь, если нужно задать условие начала диапазона, Истина, для задания конца диапазона.
//
// Возвращаемое значение:
//  СтрокаТаблицыЗначений из см. НовыеУсловияПостраничнойВыборки
//
Функция ДобавитьУсловиеВКонец(Условия, ПоляВыборки, Количество, Оператор, ДляДиапазона = Ложь)
	
	ИмяКолонкиНачало = ИмяКолонкиИзПоляВыборки(ПоляВыборки[Количество - 1]);
	ИмяКолонкиКонец = ИмяКолонкиДиапазона(ИмяКолонкиНачало);
	Условия[?(ДляДиапазона, ИмяКолонкиКонец, ИмяКолонкиНачало)] = Оператор;
	
	Возврат Условия;
	
КонецФункции

// Добавить ряд условий для границы диапазона (вида =, =, >=).
//
// Параметры:
//  ВсеУсловия - см. НовыеУсловияПостраничнойВыборки
//  ПоляВыборки - Массив - выбираемые запросом поля.
//  Количество - Число - количество полей выборки с начала, которые нужно использовать для формировании условий.
//  Оператор - Строка - оператор, который добавляется в последнее условие.
//  ДляДиапазона - Булево - Ложь, если нужно задать условие начала диапазона, Истина, для задания конца диапазона.
...
//  СтрокаТаблицыЗначений из см. НовыеУсловияПостраничнойВыборки
//
Функция ДобавитьУсловия(ВсеУсловия, ПоляВыборки, Количество, Оператор, ДляДиапазона = Ложь)
	
	Условия = ВсеУсловия.Добавить();
	
	Для НомерКолонки = 1 По Количество - 1 Цикл
		ИмяКолонкиНачало = ИмяКолонкиИзПоляВыборки(ПоляВыборки[НомерКолонки - 1]);
		ИмяКолонкиКонец = ИмяКолонкиДиапазона(ИмяКолонкиНачало);
		Условия[?(ДляДиапазона, ИмяКолонкиКонец, ИмяКолонкиНачало)] = "=";
	КонецЦикла;
	
	ДобавитьУсловиеВКонец(Условия, ПоляВыборки, Количество, Оператор, ДляДиапазона);
...
Функция УсловияДляДиапазонаСтраницы(ПоляВыборки, Параметры, Направления)
	...
	Если ПерваяЗапись = Неопределено И ПоследняяЗапись = Неопределено Тогда
		Возврат ВсеУсловия;
	КонецЕсли;
	
	Если ПерваяЗапись = ПоследняяЗапись Тогда
		ДобавитьУсловия(ВсеУсловия, ПоляВыборки, КоличествоПолей, "=");
	КонецЕсли;
	
	Пока КоличествоПолей > 0 Цикл
		Направление = Направления[КоличествоПолей - 1];
		ТекущиеКолонкиРавны = ЗаписиРавны(ПерваяЗапись, ПоследняяЗапись, КоличествоПолей);
		ПредыдущиеКолонкиРавны = ЗаписиРавны(ПерваяЗапись, ПоследняяЗапись, КоличествоПолей - 1);
		
		Если ПерваяЗапись <> Неопределено Тогда
			Если КоличествоПолей = ВсегоПолей Тогда // Все колонки начала диапазона.
				Оператор = ОператорБольшеИлиРавно(Направление);
				Условия = ДобавитьУсловия(ВсеУсловия, ПоляВыборки, КоличествоПолей, Оператор);
				
				Если ПредыдущиеКолонкиРавны Тогда
					Оператор = ОператорМеньшеИлиРавно(Направление);
					ДобавитьУсловиеВКонец(Условия, ПоляВыборки, КоличествоПолей, Оператор, Истина);
				КонецЕсли;
			Иначе // Часть колонок начала диапазона.
				Если Не ТекущиеКолонкиРавны Тогда
					Оператор = ОператорБольше(Направление);
					Условия = ДобавитьУсловия(ВсеУсловия, ПоляВыборки, КоличествоПолей, Оператор);
					
					Если ПредыдущиеКолонкиРавны Тогда
						Оператор = ОператорМеньше(Направление);
						ДобавитьУсловиеВКонец(Условия, ПоляВыборки, КоличествоПолей, Оператор, Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Если ПоследняяЗапись <> Неопределено Тогда
			Если КоличествоПолей = ВсегоПолей Тогда // Все колонки конца диапазона.
				Если Не ПредыдущиеКолонкиРавны Тогда
					Оператор = ОператорМеньшеИлиРавно(Направление);
					Условия = ДобавитьУсловия(ВсеУсловия, ПоляВыборки, КоличествоПолей, Оператор, Истина);
				КонецЕсли;
			Иначе // Часть колонок конца диапазона.
				Если Не ТекущиеКолонкиРавны И Не ПредыдущиеКолонкиРавны Тогда
					Оператор = ОператорМеньше(Направление);
					Условия = ДобавитьУсловия(ВсеУсловия, ПоляВыборки, КоличествоПолей, Оператор, Истина);
				КонецЕсли;
			КонецЕсли;
...
Функция ЗаписиРавны(ПерваяЗапись, ПоследняяЗапись, КоличествоПолей)
	...
	Если ПерваяЗапись = Неопределено Или ПоследняяЗапись = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ОбъектРегистрируетсяНаПланеОбмена(Данные)
	...
	Кэш = ОбновлениеИнформационнойБазыСлужебныйПовтИсп.КешПроверкиЗарегистрированныхОбъектов();
	ВходитВСостав = Кэш.Получить(ТипОбъекта);
	...
	Кэш.Вставить(ТипОбъекта, ВходитВСостав);
...
Процедура ЗаписатьПрогрессВыполненияОбработчика(Данные, Узел, МетаданныеОбъекта = Неопределено)
	...
	Иначе
		...
		Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицыИзменений", ПолноеИмя + ".Изменения"); // @query-part-2
...
Процедура ПроверитьПараметрыМногопоточности(ДополнительныеПараметры, ИмяОбъекта) // АПК:1036 Орфография не нарушена
	
	Если ПараметрыСеанса.ПараметрыОбработчикаОбновления.Многопоточный
		И (ДополнительныеПараметры = Неопределено
		ИЛИ Не (ДополнительныеПараметры.Свойство("АвтоматическаяВыборкаПорции")
		И ДополнительныеПараметры.АвтоматическаяВыборкаПорции)) Тогда
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для многопоточного обработчика %1 необходимо вызывать метод %2 модуля %3.'"),
			ИмяОбъекта,
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПерезапуститьОтложенноеОбновление", Истина);
