...
Функция ОтправитьПолучитьПочтуПользователяВФоне(УникальныйИдентификатор) Экспорт
	...
	Если Взаимодействия.ФоновоеЗаданиеПолучениеОтправкаПочтыВыполняется() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Интерактивная отправка и получение почты не выполнено, так как в данный момент выполняется периодическая отправка и получение писем.'"));
		Возврат Неопределено;
	КонецЕсли;
