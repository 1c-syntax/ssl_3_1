...
// Устанавливает признак разрешения отправки документов на подпись в Госключ.
// 
// Параметры:
//  Включить - Булево
//
Процедура НастроитьРаботуСГосключом(Включить) Экспорт
	
	Если Константы.ИспользоватьГосключ.Получить() <> Включить Тогда
		Константы.ИспользоватьГосключ.Установить(Включить);
	КонецЕсли;
...
//  Булево
//
Функция ИспользоватьГосключ() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьГосключ") И ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
...
// 
// Возвращаемое значение:
//  см. СервисМобильнойПодписиКлиентСервер.ПараметрыПодписания
//
Функция ПараметрыПодписанта(Подписант) Экспорт
	...
	Если ЗначениеЗаполнено(Подписант) Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Подписант,
			"Ссылка, ВидПодписанта, ИНН, СНИЛС, НомерТелефона, ИННФЛ, ОГРН, Имя, Отчество, Фамилия, Должность, Организация, НаименованиеОрганизации, ФизическоеЛицо, Наименование, Добавил, МояПодпись, ГосключУНЭП, ГосключУКЭП, НомерЗаписиОбАккредитации");
		ЗаполнитьЗначенияСвойств(ПараметрыПодписанта, Реквизиты);

		Если Не Пользователи.ЭтоПолноправныйПользователь() И Реквизиты.Добавил <> Пользователи.ТекущийПользователь() Тогда
			ПараметрыПодписанта.НомерТелефона = "";
		КонецЕсли;
	КонецЕсли;
