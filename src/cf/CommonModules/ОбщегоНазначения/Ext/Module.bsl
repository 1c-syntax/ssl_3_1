...
Процедура ПриНачалеВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено) Экспорт
	...
	Если ИнформационнаяБазаФайловая()
		И ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными")
		И Не РазделениеВключено()
		И ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
		
		МодульОбменДаннымиСервер = ОбщийМодуль("ОбменДаннымиСервер");
		ВыполняетсяЗагрузкаДанныхРИБ = МодульОбменДаннымиСервер.ВыполняетсяЗагрузкаДанныхРИБ();
		
		Если ВыполняетсяЗагрузкаДанныхРИБ Тогда
			
			Текст = НСтр("ru = 'Регламентное задание недоступно до окончания загрузки данных из главного узла.
                          |Выполнение прервано.'");
			РегламентныеЗаданияСервер.ОтменитьВыполнениеЗадания(РегламентноеЗадание, Текст);
...
Процедура ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, ТаблицаПовторногоПоиска)
	...
	Для Каждого Строка Из ТаблицаПовторногоПоиска Цикл
		...
		Фильтр.ОбъектОшибки = Строка.Данные;
		Фильтр.Ссылка       = Строка.Ссылка;
		Если Результат.Ошибки.НайтиСтроки(Фильтр).Количество() > 0 Тогда
			Продолжить; // По данной проблеме уже записана ошибка.
		КонецЕсли;

		ЗарегистрироватьОшибкуЗамены(Результат, Строка.Ссылка, 
			ОписаниеОшибкиЗамены("ДанныеИзменены", Строка.Данные, ПредметСтрокой(Строка.Данные),
