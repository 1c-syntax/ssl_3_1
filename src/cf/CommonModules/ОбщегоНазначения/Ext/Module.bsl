...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач ПараметрыЗамены = Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого Дубль Из Дубли Цикл
			...
			ДлительныеОперации.СообщитьПрогресс(Сч
				,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Замена дублей...обработано (%1 из %2)'"), Сч, КоличествоДублей));
			Сч = Сч + 1;
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.УдалениеПомеченныхОбъектов") 
		...
		УдалениеПомеченныхМенеджер = ОбщийМодуль("УдалениеПомеченныхОбъектов");
		
		ДлительныеОперации.СообщитьПрогресс(0, НСтр("ru='Удаление дублей...'"));
		РезультатУдаления = УдалениеПомеченныхМенеджер.УдалитьПомеченныеОбъекты(Результат.ОчередьКНепосредственномуУдалению);
...
Функция ЗадачиПроцесса(БизнесПроцесс, ТипЗадач)
	...
	Запрос = Новый Запрос(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ВЫБРАТЬ Таб.Ссылка ИЗ %1 КАК Таб ГДЕ Таб.БизнесПроцесс = &БизнесПроцесс",ТипЗадач));
...
Процедура ОтправитьСтатистикуЗаменыСсылок(Статистика)
	...
	ЦентрМониторингаМенеджер = ОбщийМодуль("ЦентрМониторинга");
