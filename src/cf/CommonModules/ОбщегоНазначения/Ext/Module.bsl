...
//                      Если значение на указанном языке не заполнено, возвращается значение на языке конфигурации.
//
			...
			Если ЗначениеЗаполнено(СуффиксЯзыка) Тогда
				...
				Для Позиция = 0 По НаборРеквизитов.ВГраница() Цикл
					...
					Если МультиязычныеРеквизиты[ИмяРеквизита] <> Неопределено Тогда
						...
						НаборРеквизитов[Позиция] = СтрШаблон("ВЫБОР КОГДА %1 = """" ТОГДА %2 ИНАЧЕ %1 КОНЕЦ КАК %2", ИмяССуффиксом, ИмяРеквизита);
					КонецЕсли;
				КонецЦикла;
			...
			Если ЗначениеЗаполнено(КодЯзыка) И КодЯзыка <> КодОсновногоЯзыка()
				...
				Для Каждого Реквизит Из СтрРазделить(Реквизиты, ",") Цикл
					Если МультиязычныеРеквизиты[Реквизит] = Истина Тогда
						ПоляТаблиц.Добавить("ЕСТЬNULL(ТаблицаПредставления." + Реквизит + ", ПсевдонимЗаданнойТаблицы." + Реквизит + ") КАК " + Реквизит);
					Иначе
