...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	// Обновление минимальной и рекомендуемой версии платформы по данным сервиса
	Если ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБИП, "2.7.1.35") > 0 Тогда
			...
			// Попытка получить данные версий платформы из кеша и переопределить их
			Попытка
				...
				Если Не ПустаяСтрока(ИнформацияОВерсияхПлатформы.МинимальнаяВерсияПлатформы)
					Или Не ПустаяСтрока(ИнформацияОВерсияхПлатформы.РекомендуемаяВерсияПлатформы) Тогда
					
					Минимальная   = НомерСборкиПоТекущейВерсииПлатформы(
						ИнформацияОВерсияхПлатформы.МинимальнаяВерсияПлатформы);
					Рекомендуемая = НомерСборкиПоТекущейВерсииПлатформы(
						ИнформацияОВерсияхПлатформы.РекомендуемаяВерсияПлатформы);
					...
					Если МинимальнаяИРекомендуемаяВерсияПлатформыЗаполненаНеПравильно(Минимальная, Рекомендуемая) Тогда
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							...
							Минимальная,
							НомерСборкиПоТекущейВерсииПлатформы(
						...
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'Базовая функциональность'", КодОсновногоЯзыка()),
...
Функция МинимальнаяВерсияПлатформы() Экспорт // АПК:581 - экспортная, так как используется при тестировании.
	...
	Возврат "8.3.21.1775; 8.3.22.1923";
...
//
// Возвращаемое значение:
//  Булево - Истина, если минимальная и рекомендуемая версия платформы заполнена не правильно.
//
Функция МинимальнаяИРекомендуемаяВерсияПлатформыЗаполненаНеПравильно(Минимальная, Рекомендуемая)
	...
	// Минимальная версия платформы не должна быть меньше минимальной версии платформы необходимой для правильной работы
	// БСП.
	МинимальнаяБСП = НомерСборкиПоТекущейВерсииПлатформы(
		МинимальнаяВерсияПлатформы());
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(МинимальнаяБСП, Минимальная) > 0 Тогда
		Возврат Истина;
	КонецЕсли;
