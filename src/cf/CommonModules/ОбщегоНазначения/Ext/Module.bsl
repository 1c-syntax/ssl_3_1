...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	МинимальнаяВерсияПоБСП = НомерСборкиПоТекущейВерсииПлатформы(ОбщиеПараметры.МинимальнаяВерсияПлатформы);
	
	ОбщегоНазначенияПереопределяемый.ПриОпределенииОбщихПараметровБазовойФункциональности(ОбщиеПараметры);
	
	МинимальнаяВерсияПослеПереопределения   = НомерСборкиПоТекущейВерсииПлатформы(ОбщиеПараметры.МинимальнаяВерсияПлатформы);
	РекомендуемаяВерсияПослеПереопределения = НомерСборкиПоТекущейВерсииПлатформы(ОбщиеПараметры.РекомендуемаяВерсияПлатформы);
	// Проверка, что после переопределения минимальная версия не ниже установленной в БСП.
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(МинимальнаяВерсияПослеПереопределения, МинимальнаяВерсияПоБСП) < 0 Тогда
		ОбщиеПараметры.МинимальнаяВерсияПлатформы = МинимальнаяВерсияПоБСП;
	Иначе
		ОбщиеПараметры.МинимальнаяВерсияПлатформы = МинимальнаяВерсияПослеПереопределения;
	КонецЕсли;
	
	// Проверка, что после переопределения рекомендуемая версия не ниже минимальной в БСП.
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(РекомендуемаяВерсияПослеПереопределения, МинимальнаяВерсияПоБСП) < 0 Тогда
		ОбщиеПараметры.РекомендуемаяВерсияПлатформы = МинимальнаяВерсияПоБСП;
	Иначе
		ОбщиеПараметры.РекомендуемаяВерсияПлатформы = РекомендуемаяВерсияПослеПереопределения;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Минимальная)
		...
		ТекстСообщения = НСтр("ru = 'Минимальная версия платформы ""1С:Предприятие"" указана выше рекомендуемой в %1.
			|Минимальная версия %2, рекомендуемая версия %3.'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстСообщения, "ОбщегоНазначенияПереопределяемый.ПриОпределенииОбщихПараметровБазовойФункциональности",
...
//  ИсключенияПоискаСсылок	 - Соответствие
// 
// Возвращаемое значение:
//   Булево
//
...
Процедура ПроизвестиЗаменуВКонстанте(Результат, Знач МестоИспользования, Знач ПараметрыЗаписи)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗаменыСсылок(), УровеньЖурналаРегистрации.Ошибка,
			МетаданныеКонстанты,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция НомерСборкиПоТекущейВерсииПлатформы(НомераСборокСтрокой) Экспорт
...
Функция МинимальнаяВерсияПлатформы() Экспорт // АПК:581 - экспортная, так как используется при тестировании.
	...
	Возврат "8.3.17.2733; 8.3.18.1902; 8.3.19.1665; 8.3.20.2076; 8.3.21.1508; 8.3.22.1603";
