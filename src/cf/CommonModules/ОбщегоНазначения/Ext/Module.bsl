...
Функция ОперативнаяПамятьДоступнаяКлиентскомуПриложению() Экспорт
	...
	ДоступныйОбъем = СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере(Ложь).Получить("ОперативнаяПамять");
...
Процедура СохранитьПерсональныеНастройки(Настройки) Экспорт
	...
	Если Настройки.Свойство("НапоминатьОбУстановкеРасширенияРаботыСФайлами") Тогда
		Если ЭтоВебКлиент() Тогда
			ИдентификаторКлиента = СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере(Ложь).Получить("ИдентификаторКлиента");
...
Функция МинимальнаяВерсияПлатформы() Экспорт // АПК:581 - экспортная, так как используется при тестировании.
	...
	Возврат "8.3.17.2760; 8.3.18.1957; 8.3.19.1723; 8.3.20.2180; 8.3.21.1622; 8.3.22.1704";
...
Функция ЭтоВерсияЗащищенногоПрограммногоКомплекса(Версия)
	...
	Версии = Новый Массив;
	Версии.Добавить("8.3.17.2102");
	Версии.Добавить("8.3.17.2316");
	Версии.Добавить("8.3.17.2598");
	Версии.Добавить("8.3.17.2714");
	Версии.Добавить("8.3.19.1405");
...
Процедура УточнитьВерсиюПлатформы(ОбщиеПараметры)
	...
	Если Не ЗначениеЗаполнено(НоваяСборка) Тогда
		НоваяСборка = НоваяСборка(ОбщиеПараметры.МинимальнаяВерсияПлатформы);
		Если ЗначениеЗаполнено(НоваяСборка)
		   И Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ОбщиеПараметры.РекомендуемаяВерсияПлатформы, НоваяСборка) < 0 Тогда
				ОбщиеПараметры.РекомендуемаяВерсияПлатформы = НоваяСборка;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ОбщиеПараметры.МинимальнаяВерсияПлатформы, НоваяСборка) < 0 Тогда
		ОбщиеПараметры.РекомендуемаяВерсияПлатформы = НоваяСборка;
		ОбщиеПараметры.МинимальнаяВерсияПлатформы = НоваяСборка;
		ОбщиеПараметры.МинимальноНеобходимаяВерсияПлатформы = НоваяСборка;
		ОбщиеПараметры.РаботаВПрограммеЗапрещена = Истина;
