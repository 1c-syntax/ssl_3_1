...
Функция ПодключитьКомпонентуИзМакета(Знач Идентификатор, Знач ПолноеИмяМакета, Знач Изолированно = Null) Экспорт
	...
	Если Изолированно = Null Тогда
		Изолированно = ТипПодключенияКомпонентыПоУмолчанию();
	КонецЕсли;
...
#Если Не ВебКлиент И Не МобильныйКлиент И Не МобильноеПриложениеСервер Тогда
	...
	Если СтрНачинаетсяС(ВерсияПриложения, "8.3.24") И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПриложения, "8.3.24.1267") >= 0
	 Или СтрНачинаетсяС(ВерсияПриложения, "8.3.23") И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПриложения, "8.3.23.1947") >= 0
	 Или СтрНачинаетсяС(ВерсияПриложения, "8.3.22") И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПриложения, "8.3.22.2322") >= 0
	 Или СтрНачинаетсяС(ВерсияПриложения, "8.3.21") И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПриложения, "8.3.21.1930") >= 0 Тогда
		Возврат Неопределено;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроизвестиЗаменуВНаборе(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения)
	...
	ОписаниеНабора = ОписаниеКлючаЗаписи(МетаданныеРегистра);
...
Функция СистемнаяИнформацияДляЖурналаРегистрации()
	...
	Возврат Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Платформа 1С:Предприятие: %1
		           |Тип платформы: %2
		           |Версия ОС: %3
		           |Процессор: %4
...
Функция ЭтоВерсияЗащищенногоПрограммногоКомплекса(Версия)
	...
	Версии = СтандартныеПодсистемыСервер.ВерсииЗащищенногоПрограммногоКомплекса();
...
Процедура ПроверитьАлгоритм(Знач Алгоритм)
	...
	Если ТекстыОшибок.Количество() > 0 Тогда
		ВызватьИсключение СтрСоединить(ТекстыОшибок, Символы.ПС + Символы.ПС);
	КонецЕсли;
КонецПроцедуры
