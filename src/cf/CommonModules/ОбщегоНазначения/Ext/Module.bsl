...
//   
//   ПараметрыЗамены - Структура - см. ОбщегоНазначения.ПараметрыЗаменыСсылок.
//
// Возвращаемое значение:
...
Функция СоздатьWSПрокси(ПереданныеПараметры) Экспорт
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'WSПрокси'", КодЛокализацииИнформационнойБазы()),
...
// либо очищает переданное значение, если в нем передана несуществующая ссылка. 
//
// Параметры:
...
Функция ОчиститьНесуществующиеСсылки(Значение)
	...
	ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(Тип)
		...
		ИначеЕсли ЗначениеРеквизитаОбъекта(Значение, "Ссылка") = Неопределено Тогда
...
#Область Прочее_ПодключитьКомпонентуИзМакета
