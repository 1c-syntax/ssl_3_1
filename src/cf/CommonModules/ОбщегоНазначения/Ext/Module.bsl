...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь, Знач КодЯзыка = Неопределено) Экспорт
	...
	Для каждого КлючИЗначение Из СтруктураПолей Цикл
		...
		Если МультиязычныеРеквизиты[ИмяПоля] = Ложь Тогда
		...
		Если ЗначениеЗаполнено(КодЯзыка) Тогда
			...
			Если ЗначениеЗаполнено(СуффиксЯзыка) Тогда
				...
				Для Позиция = 0 По НаборРеквизитов.ВГраница() Цикл
					...
					Если МультиязычныеРеквизиты[ИмяРеквизита] = Ложь Тогда
...
Функция ПроверитьПроведенностьДокументов(Знач Документы) Экспорт
	...
	ТекстОбъединитьВсе = ТекстОбъединитьВсе();
...
Функция ПровестиДокументы(Документы) Экспорт
	...
	Для Каждого ДокументСсылка Из Документы Цикл
		...
		Если Не ВыполненоУспешно Тогда
			НепроведенныеДокументы.Добавить(Новый Структура("Ссылка,ОписаниеОшибки", 
				ДокументСсылка, ПредставлениеОшибки));
		КонецЕсли;
...
//       * ДубльРодитель - ЛюбаяСсылка - ссылка на дубль для группировки ошибок подчиненных объектов.
//                                       Если нет связанных подчиненных, заполняется значением из колонки Ссылка.
...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач ПараметрыЗамены = Неопределено) Экспорт
	...
	ОшибкиЗамены.Колонки.Добавить("ДубльРодитель");
	...
	ОшибкиЗамены.Колонки.Добавить("ИнформацияОбОшибке");
	...
	ПараметрыВыполнения.Вставить("ПодчиненныеКДублям", Новый Соответствие());
	...
	Попытка
		...
		ТекстПрогресса = НСтр("ru = 'Замена дублей...'");
		ДополнительныеПараметрыПрогресса = Новый Структура;
		ДополнительныеПараметрыПрогресса.Вставить("КоличествоОбработанных", 0);
		...
		Для Каждого Дубль Из Дубли Цикл
			...
			ДополнительныеПараметрыПрогресса.КоличествоОбработанных = 
				ДополнительныеПараметрыПрогресса.КоличествоОбработанных + 1;
			
			ДобавитьВСтатистикуЗаменыСсылок(Статистика, Дубль, Результат.ЕстьОшибки);
			
			Если ДополнительныеПараметрыПрогресса.КоличествоОбработанных >9 Тогда
				ДлительныеОперации.СообщитьПрогресс(ДополнительныеПараметрыПрогресса.КоличествоОбработанных,
					ТекстПрогресса, ДополнительныеПараметрыПрогресса);
				ДополнительныеПараметрыПрогресса.КоличествоОбработанных = 0;
			КонецЕсли;
			
		КонецЦикла;
		
		Если ДополнительныеПараметрыПрогресса.КоличествоОбработанных > 0 Тогда
			ДлительныеОперации.СообщитьПрогресс(ДополнительныеПараметрыПрогресса.КоличествоОбработанных, 
				ТекстПрогресса, ДополнительныеПараметрыПрогресса);
		КонецЕсли;
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.УдалениеПомеченныхОбъектов") 
		...
		ДополнительныеПараметрыПрогресса = Новый Структура;
		ДлительныеОперации.СообщитьПрогресс(0, НСтр("ru = 'Удаление дублей...'"), ДополнительныеПараметрыПрогресса);
		
		РезультатУдаления = МодульУдалениеПомеченныхОбъектов.УдалитьПомеченныеОбъекты(Результат.ОчередьКНепосредственномуУдалению);
		ЗарегистрироватьОшибкиУдаления(Результат, РезультатУдаления.ПрепятствующиеУдалению, ПараметрыВыполнения);
...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "", ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИсключенияПоискаРегистровПодчиненныхРегистратору = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДополнительныеПараметры, "ИсключенияПоискаРегистровПодчиненныхРегистратору", Неопределено);
	УстановитьПривилегированныйРежим(Истина);
	МестаИспользования = НайтиПоСсылкам(НаборСсылок, , , ИсключенияПоискаРегистровПодчиненныхРегистратору); // см. МестаИспользования.
	УстановитьПривилегированныйРежим(Ложь);
	...
	ДополнительныеИсключенияПоискаСсылок = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДополнительныеПараметры, "ДополнительныеИсключенияПоискаСсылок", Новый Соответствие);
	Для каждого МетаданныеРеквизитыИсключения Из ДополнительныеИсключенияПоискаСсылок Цикл
		...
		Если ЗначениеИсключения = Неопределено Тогда
			ИсключенияПоискаСсылок.Вставить(МетаданныеРеквизитыИсключения.Ключ, МетаданныеРеквизитыИсключения.Значение);
		ИначеЕсли ТипЗнч(ЗначениеИсключения) = Тип("Массив") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗначениеИсключения, МетаданныеРеквизитыИсключения.Значение);
		КонецЕсли;
	КонецЦикла;
...
//                                                                 объектов метаданных.
//   * ИсключенияПоискаРегистровПодчиненныхРегистратору - Массив из ОбъектМетаданных - если в см. ОбщегоНазначенияПереопределяемый.ПриДобавленииИсключенийПоискаСсылок
//          указаны регистры подчиненные регистратору, то их необходимо добавить.
//
Функция ПараметрыПоискаМестИспользования() Экспорт
	...
	ПараметрыПоиска.Вставить("ИсключенияПоискаРегистровПодчиненныхРегистратору", Новый Массив);
...
Функция ИсключенияПоискаСсылок() Экспорт
	...
	Для Каждого ИсключениеПоиска Из ИсключенияПоиска Цикл
		...
		Иначе
			...
			//     "Документ.ЗаказПокупателя.ТабличнаяЧасть.СчетаНаОплату.Реквизит.Счет",
			//     "ПланВидовРасчета.ОсновныеНачисления.СтандартнаяТабличнаяЧасть.БазовыеВидыРасчета.СтандартныйРеквизит.ВидРасчета".
...
//		МодульОбработкаПолнотекстовыйПоиск = ОбщегоНазначения.ОбщийМодуль("Обработки.ПолнотекстовыйПоиск");
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти(Имя) <> Неопределено Тогда
		...
		Модуль = Вычислить(Имя); // Не нарушает безопасность информации, так как вычисляется ссылка на общий модуль по проверенному имени.
		// АПК:488-вкл
	ИначеЕсли СтрЧислоВхождений(Имя, ".") > 0 Тогда
		Возврат МенеджерОбъектаПоИмени(Имя, Истина, Истина, Истина, "ОбщегоНазначения.ОбщийМодуль", "Имя");
	Иначе
		Модуль = Неопределено;
	КонецЕсли;
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			КатегорияОшибки.ОшибкаКонфигурации,
			"СтандартныеПодсистемы.БазоваяФункциональность.ЭтоОшибкаПолученияОбщегоМодуляПоИмени");
	КонецЕсли;
...
Функция ЭтоLinuxСервер() Экспорт
	...
	Возврат СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Linux_x86
		...
		Или СистемнаяИнформация.ТипПлатформы = ТипПлатформы["Linux_ARM64"]
...
Функция ИнформационнаяБазаФайловая(Знач СтрокаСоединенияИнформационнойБазы = "") Экспорт
	...
	Если ПустаяСтрока(СтрокаСоединенияИнформационнойБазы) Тогда
		Возврат СтандартныеПодсистемыПовтИсп.ИнформационнаяБазаФайловая();
	КонецЕсли;
...
Функция АдресПубликацииИнформационнойБазыВИнтернете() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Результат = Константы.АдресПубликацииИнформационнойБазыВИнтернете.Получить();
	Если РазделениеВключено() И ДоступноИспользованиеРазделенныхДанных() Тогда 
		Если ПустаяСтрока(Результат)
			И ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность")
			И ПодсистемаСуществует("ТехнологияСервиса.ВнешнийПрограммныйИнтерфейс") Тогда

			МодульРаботаВМоделиСервиса = ОбщийМодуль("РаботаВМоделиСервиса");
			МодульПрограммныйИнтерфейсСервиса = ОбщийМодуль("ПрограммныйИнтерфейсСервиса");
			РазделительСеанса = МодульРаботаВМоделиСервиса.ЗначениеРазделителяСеанса();
			Попытка
				Результат = МодульПрограммныйИнтерфейсСервиса.СвойстваПриложения(РазделительСеанса).АдресПриложения;
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Адрес публикации приложения'", КодОсновногоЯзыка()), // АПК:154 Не считаем ошибкой проблемы недоступности Менеджера сервиса. 
					УровеньЖурналаРегистрации.Предупреждение,,, 
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат "";
			КонецПопытки;
			Константы.АдресПубликацииИнформационнойБазыВИнтернете.Установить(Результат);
		КонецЕсли;
		Возврат Результат;
	КонецЕсли;	
	Возврат Результат;
...
Функция АдресПубликацииИнформационнойБазыВЛокальнойСети() Экспорт
	...
	Если РазделениеВключено() И ДоступноИспользованиеРазделенныхДанных() Тогда 
		Возврат АдресПубликацииИнформационнойБазыВИнтернете();
	КонецЕсли;
...
#Область ФункцииДляРаботыСДатамиСУчетомЧасовогоПоясаСеанса
...
#Область ФункцииДляРаботыСНастройкойРабочейДаты
...
Функция ПрочитатьXMLВТаблицу(Знач XML) Экспорт
	...
	Если Не Чтение.Прочитать() Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось загрузить данные из XML-файла, т.к. он пустой.'");
	ИначеЕсли Чтение.Имя <> "Items" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось загрузить данные из XML-файла: отсутствует обязательный тег %1.'"),
			"Items");
	КонецЕсли;
	...
	Пока Чтение.Прочитать() Цикл
		...
		ИначеЕсли Чтение.Имя <> "Item" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось загрузить данные из XML-файла: в теге %1 отсутствует обязательный тег %2.'"),
				"Items", "Item");
		КонецЕсли;
...
#Область ФункцииОпределенияТиповОбъектовМетаданных
...
// Принадлежность определяется по базовому типу, то есть для формы справочника будет Истина,
// для формы регистра будет Ложь. Если объект метаданных не найден по типу, тогда будет Ложь.
//
// Параметры:
//  ОписаниеОбъекта - ОбъектМетаданных, Тип
// 
// Возвращаемое значение:
//   Булево - Истина, если базовый тип объекта ссылочного типа.
//
Функция ЭтоОбъектСсылочногоТипа(ОписаниеОбъекта) Экспорт
	
	Если ТипЗнч(ОписаниеОбъекта) = Тип("Тип") Тогда
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ОписаниеОбъекта);
		Если ОбъектМетаданных = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
	Иначе
		ОбъектМетаданных = ОписаниеОбъекта;
	КонецЕсли;
	
	ИмяВидаОбъектов = СтрРазделить(ОбъектМетаданных.ПолноеИмя(), ".")[0];
	ПоВидамОбъектов = СтандартныеПодсистемыПовтИсп.СвойстваВидовОбъектовМетаданных().ПоВидамОбъектов;
	Свойства = ПоВидамОбъектов.Получить(ВРег(ИмяВидаОбъектов));
	
	Если Свойства.ИмяКоллекции = "Перечисления" Тогда // Обратная совместимость.
		Возврат Ложь;
	КонецЕсли;
	
	Возврат Свойства <> Неопределено И Свойства.ЕстьСсылка;
	
КонецФункции

#КонецОбласти

#Область ПроцедурыИФункцииДляРаботыСТипамиОбъектамиМетаданныхИИхСтроковымиПредставлениями
...
// Возвращает имя базового типа по началу полного имени объекта метаданных.
//
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - объект метаданных, для которого необходимо определить базовый тип.
// 
// Возвращаемое значение:
//  Строка - имя базового типа указанного объекта метаданных, например,
//           "ОбщиеМодули", ..., "Языки", ..., "Документы", ..., "РегистрыСведений", ...
//           Для перерасчетов регистров расчета возвращается "Перерасчеты".
//
// Пример:
//  ИмяБазовогоТипа = ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(Метаданные.Справочники.Номенклатура);
//
Функция ИмяБазовогоТипаПоОбъектуМетаданных(ОбъектМетаданных) Экспорт
	
	ЧастиИмени = СтрРазделить(ОбъектМетаданных.ПолноеИмя(), ".");
	ПоВидамОбъектов = СтандартныеПодсистемыПовтИсп.СвойстваВидовОбъектовМетаданных().ПоВидамОбъектов;
	Свойства = ПоВидамОбъектов.Получить(ВРег(ЧастиИмени[0]));
	
	Если Свойства = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Если Свойства.ИмяВидаОбъекта = "РегистрРасчета"
	   И ЧастиИмени.Количество() = 4
	   И ВРег(ЧастиИмени[2]) = "ПЕРЕРАСЧЕТ" Тогда
		Возврат "Перерасчеты";
	КонецЕсли;
	
	Возврат Свойства.ИмяКоллекции;
	
КонецФункции

// Возвращает менеджер объекта по началу полному имени объекта метаданных.
// Вызывает исключение при неверном начале полного имени или
// отсутствии объектов в конфигурации из начала полного имени,
// определяющего менеджер объекта.
// Например, для реквизитов или форм справочника используется только
// имя справочника и возвращается менеджер справочника,
// а для перерасчета используется имя регистра расчета, имя перерасчета и
// возвращается менеджер перерасчета.
//
// Параметры:
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Возврат МенеджерОбъектаПоИмени(ПолноеИмя, Истина, Ложь, Ложь,
		"ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени", "ПолноеИмя", Ложь);
	
КонецФункции

// Возвращает менеджер объекта по ссылке на объект.
// См. МенеджерОбъектаПоПолномуИмени.
//
// Параметры:
//  Ссылка - ЛюбаяСсылка - ссылка на объект, менеджер которого требуется получить.
//
// Возвращаемое значение:
//  - ПланОбменаМенеджер
//  - СправочникМенеджер
...
Функция МенеджерОбъектаПоСсылке(Ссылка) Экспорт
	...
	ЧастиИмени = СтрРазделить(Ссылка.Метаданные().ПолноеИмя(), ".");
	ПоВидамОбъектов = СтандартныеПодсистемыПовтИсп.СвойстваВидовОбъектовМетаданных().ПоВидамОбъектов;
	Свойства = ПоВидамОбъектов.Получить(ВРег(ЧастиИмени[0]));
	
	Если Свойства <> Неопределено И Свойства.ЕстьСсылка Тогда
		Возврат Свойства.Менеджер[ЧастиИмени[1]];
	КонецЕсли;
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4.'"),
		Ссылка, ТипЗнч(Ссылка), "Ссылка", "ОбщегоНазначения.МенеджерОбъектаПоСсылке");
	ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
...
Функция ВидОбъектаПоТипу(ТипОбъекта) Экспорт
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ОбщегоНазначения.ВидОбъектаПоТипу",
			"ТипОбъекта", ТипОбъекта, Тип("Тип"));
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Не существует объекта метаданных для указанного типа.'"),
			ТипОбъекта, ТипЗнч(ТипОбъекта), "ТипОбъекта", "ОбщегоНазначения.ВидОбъектаПоТипу");
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	...
	Если Свойства = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Не определен вид объектов метаданных для указанного типа.'"),
			ТипОбъекта, ТипЗнч(ТипОбъекта), "ТипОбъекта", "ОбщегоНазначения.ВидОбъектаПоТипу");
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	
	Возврат Свойства.ИмяВидаОбъекта;
...
Функция ОбъектМетаданныхПоПолномуИмени(ПолноеИмя) Экспорт
	...
	ИмяВидаОбъектов = Лев(ПолноеИмя, ПозицияТочки - 1);
	
	ПоВидамОбъектов = СтандартныеПодсистемыПовтИсп.СвойстваВидовОбъектовМетаданных().ПоВидамОбъектов;
	Свойства = ПоВидамОбъектов.Получить(ВРег(ИмяВидаОбъектов));
	
	Если Свойства <> Неопределено Тогда
		Если Свойства.ИмяКоллекции <> "Подсистемы" Тогда
			ИмяОбъекта = Сред(ПолноеИмя, ПозицияТочки + 1);
			ОбъектМетаданных = Метаданные[Свойства.ИмяКоллекции].Найти(ИмяОбъекта);
		Иначе
...
//
Процедура ДобавитьПереименование(Переименования, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, 
	ИдентификаторБиблиотеки = "") Экспорт
	
	Справочники.ИдентификаторыОбъектовМетаданных.ДобавитьПереименование(Переименования,
		ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки);
	
КонецПроцедуры

// Возвращает строковое представление типа, например, "СправочникСсылка.ИмяОбъекта", "Форма клиентского приложения".
// Тип приводится к строке напрямую, кроме простых типов, ссылочных типов,
// типов ключа записи регистров, типов объекта и типов набора записей.
//
// Параметры:
//  Тип - Тип - для которого надо получить представление.
//
// Возвращаемое значение:
//  Строка - "СправочникСсылка.ИмяОбъекта", "РегистрСведенийКлючЗаписи.ИмяОбъекта",
//           "СправочникОбъект.ИмяОбъекта", "РегистрСведенийНаборЗаписей.ИмяОбъекта", ...
//
Функция СтроковоеПредставлениеТипа(Тип) Экспорт
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		...
		Свойства = ПоВидамОбъектов.Получить(ВРег(ЧастиИмени[0]));
		Если Свойства <> Неопределено Тогда
			Если Свойства.ИмяВидаОбъекта = "БизнесПроцесс"
			   И БизнесПроцессы.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов().СодержитТип(Тип) Тогда
				ИмяТипа = "ТочкаМаршрутаБизнесПроцессаСсылка";
			ИначеЕсли Свойства.ЕстьСсылка Тогда
				ИмяТипа = Свойства.ИмяТипаСсылки;
			ИначеЕсли Свойства.ЕстьКлючЗаписи Тогда
				ИмяТипа = Свойства.ИмяТипаКлючаЗаписи;
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяТипа) Тогда
				Результат = ИмяТипа + "." + ЧастиИмени[1];
				Если Тип(Результат) = Тип Тогда
					Возврат Результат;
				КонецЕсли;
			КонецЕсли;
			ИмяТипа = Неопределено;
			Если ЗначениеЗаполнено(Свойства.ИмяТипаОбъекта) Тогда
				ИмяТипа = Свойства.ИмяТипаОбъекта;
			ИначеЕсли ЗначениеЗаполнено(Свойства.ИмяТипаНабораЗаписей) Тогда
				ИмяТипа = Свойства.ИмяТипаНабораЗаписей;
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяТипа) Тогда
				Результат = ИмяТипа + "." + ЧастиИмени[1];
				Если Тип(Результат) = Тип Тогда
...
Функция ОтборЗаписейНабораИзБазыДанных(НаборЗаписей, Замещение = Неопределено, Запрос = Неопределено,
	...
	Иначе
		...
		Если ТипЗнч(Режим.Замещение) <> Тип("Булево")
		   ...
		   И Не ЭтоУдалениеНабораЗаписей(Режим.Замещение) Тогда
			...
			Если СтандартныеПодсистемыПовтИсп.РежимДобавленияНабораЗаписей() <> Неопределено Тогда
				ОжидаемыеТипы.Добавить(СтандартныеПодсистемыПовтИсп.ТипРежимЗамещения());
			КонецЕсли;
	...
	Если ЭтоОбновлениеНабораЗаписей(Режим.Замещение)
	 ...
	 Или ЭтоУдалениеНабораЗаписей(Режим.Замещение) Тогда
		...
		ОписаниеКлючаЗаписи = СтандартныеПодсистемыСервер.ОписаниеКлючаЗаписи(
		...
		Если ЗначениеЗаполнено(СписокПолей) Тогда
			...
			КонецЕсли;
			Условие = СтрШаблон("(%1) В (&" + ИмяПараметраТаблицы + ")",
				Псевдоним + СтрЗаменить(СписокПолей, ",", ", " + Псевдоним)); // @query-part-1
		Иначе
			Условие = "ИСТИНА";
		КонецЕсли;
		
	ИначеЕсли Режим.Замещение = Истина
	      Или ЭтоЗамещениеНабораЗаписей(Режим.Замещение) Тогда
...
Функция ЗаписиНабораИзБазыДанных(НаборЗаписей, Замещение = Неопределено, СписокПолей = "") Экспорт
	...
	ПолноеИмяРегистра = НаборЗаписей.Метаданные().ПолноеИмя();
	СвернутьРезультат = Ложь;
	
	Если ЗначениеЗаполнено(СписокПолей) Тогда
		ОписаниеКлючаЗаписи = СтандартныеПодсистемыСервер.ОписаниеКлючаЗаписи(ПолноеИмяРегистра);
		Поля = СтрРазделить(ВРег(СписокПолей), ",");
		Для Каждого Поле Из ОписаниеКлючаЗаписи.ОписаниеПолей Цикл
			Если Поля.Найти(ВРег(Поле.Имя)) = Неопределено Тогда
				СвернутьРезультат = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	Иначе
	...
	Если Замещение = Ложь Или ЭтоДобавлениеНабораЗаписей(Замещение) Тогда
		Возврат НаборЗаписей.Выгрузить(Новый Массив,
			?(ЗначениеЗаполнено(СписокПолей), СписокПолей, Неопределено));
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Таблица", ПолноеИмяРегистра);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ВыбираемыеПоля", СписокПолей);
	
	Результат = Запрос.Выполнить().Выгрузить();
	
	Если СвернутьРезультат Тогда
		Результат.Свернуть(СписокПолей);
	КонецЕсли;
...
Функция РазрешенДоступКИнтернетСервисам(ПоказатьОшибку = Ложь) Экспорт
	...
	ДоступРазрешен = ПолучитьФункциональнуюОпцию("РазрешитьДоступКИнтернетСервисам");

	Если Не ДоступРазрешен Тогда
		ВерсияМетаданных = Метаданные.Версия;
		Если Не ПустаяСтрока(ВерсияМетаданных) И ВерсияМетаданных <> ОбновлениеИнформационнойБазыСлужебный.ВерсияИБ(
			Метаданные.Имя) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;

	Если Не ДоступРазрешен И ПоказатьОшибку Тогда
		ВызватьИсключение ЗапрещенДоступКИнтернетСервисамТекстСообщения();
	КонецЕсли;
...
Функция ЗапрещенДоступКИнтернетСервисамТекстСообщения() Экспорт
	...
	Возврат НСтр("ru = 'Доступ к Интернет-сервисам в приложении запрещен администратором (""Интернет-поддержка и сервисы"" - ""Доступ к Интернет-сервисам"").'");
...
Процедура НастроитьРаботуСИнтернетСервисами(Разрешить) Экспорт
	...
	Если Не Разрешить И РазделениеВключено() Тогда
		Возврат;
	КонецЕсли;
	
	Константы.РазрешитьДоступКИнтернетСервисам.Установить(Разрешить);
	
	Если Разрешить И ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда

		МодульБлокировкаРаботыСВнешнимиРесурсами = ОбщийМодуль("БлокировкаРаботыСВнешнимиРесурсами");
		МодульБлокировкаРаботыСВнешнимиРесурсами.ПриВключенииДоступаКИнтернетСервисам();
...
Функция ПараметрыПодключенияWSПрокси() Экспорт
	...
	Результат.Вставить("ПроверятьДоставкуПакетовПриОшибке", Истина);
	Результат.Вставить("ПроверятьДоступКИнтернетСервисам", Истина);
	Возврат Результат;
КонецФункции
...
Функция СоздатьWSПрокси(Знач ПараметрыПодключенияWSПрокси) Экспорт
	...
	Если ПараметрыПодключения.ПроверятьДоступКИнтернетСервисам Тогда
		РазрешенДоступКИнтернетСервисам(Истина);
	КонецЕсли;
		...
		Исключение
			...
			Если ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
				...
				РезультатДиагностики = МодульПолучениеФайловИзИнтернета.ДиагностикаСоединения(АдресТочкиПодключения,,
					ПараметрыПодключения.ПроверятьДоставкуПакетовПриОшибке);
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1
...
#Область ВерсионированиеПрограммныхИнтерфейсов
...
Функция ПолучитьВерсииИнтерфейса(Знач Адрес, Знач Пользователь, Знач Пароль = Неопределено, 
	...
	ПараметрыПодключения.Вставить("ПроверятьДоставкуПакетовПриОшибке", ПроверятьДоставкуПакетовПриОшибке);
	
	Если Не ПараметрыПодключения.Свойство("URL") 
		Или Не ЗначениеЗаполнено(ПараметрыПодключения.URL) Тогда
		
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение параметра %1 функции %2. Не задан URL сервиса.'"), 
			"ПараметрыПодключения", "ОбщегоНазначения.ПолучитьВерсииИнтерфейса"), КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
Функция ВерсииИнтерфейсов() Экспорт
	...
	ИнтеграцияПодсистемБСП.ПриОпределенииПоддерживаемыхВерсийПрограммныхИнтерфейсов(Результат);
	ОбщегоНазначенияПереопределяемый.ПриОпределенииПоддерживаемыхВерсийПрограммныхИнтерфейсов(Результат);
...
Процедура ЗаписатьДанныеВБезопасноеХранилище(Владелец, Данные, Ключ = "Пароль") Экспорт
	...
	Если ЭтоОбластьДанных Тогда
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.БезопасноеХранилищеДанныхОбластейДанных");
	Иначе
		БезопасноеХранилищеДанных = РегистрыСведений.БезопасноеХранилищеДанных.СоздатьМенеджерЗаписи();
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.БезопасноеХранилищеДанных");
	КонецЕсли;
	...
	Попытка
		...
		Если Данные <> Неопределено Тогда
			...
			Иначе
				...
				БезопасноеХранилищеДанных.Владелец = Владелец;
			КонецЕсли;
			ДанныеДляХранилищеЗначения = Новый ХранилищеЗначения(ДанныеДляСохранения, Новый СжатиеДанных(9));
...
//	КонецПроцедуры
//
Функция ПрочитатьДанныеВладельцевИзБезопасногоХранилища(
		Владельцы,
		Ключи = "Пароль",
		ОбщиеДанные = Неопределено,
		ОбластьДанных = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ОбластьДанных) Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(ОбластьДанных) = Тип("Число"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 в %2.
			|параметр должен содержать строку; передано значение: %3 (тип %4).'"),
			"ОбластьДанных", "ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище",
				ОбластьДанных,
				ТипЗнч(ОбластьДанных)));
	КонецЕсли;
	
	Возврат ДанныеИзБезопасногоХранилища(Владельцы, Ключи, ОбщиеДанные, ОбластьДанных);
...
Функция ПрочитатьДанныеИзБезопасногоХранилища(
	...
	ДанныеВладельца = ПрочитатьДанныеВладельцевИзБезопасногоХранилища(Владельцы, Ключи, ОбщиеДанные, ОбластьДанных);
	Возврат ДанныеВладельца[Владелец];
...
Процедура УдалитьДанныеИзБезопасногоХранилища(Знач Владелец, Знач Ключи = Неопределено) Экспорт
	...
	Для каждого ВладелецДанных Из Владельцы Цикл
		ЭлементБлокировки.УстановитьЗначение("Владелец", ВладелецДанных);
	КонецЦикла;
				...
				Если Ключи <> Неопределено И ТипЗнч(ДанныеДляСохранения) = Тип("Структура") Тогда
					...
					Если БезопасноеХранилищеДанных.Выбран() И СписокКлючей.Количество() > 0 Тогда
						Для каждого КлючДляУдаления Из СписокКлючей Цикл
							...
							КонецЕсли;
						КонецЦикла;
						БезопасноеХранилищеДанных.Данные = Новый ХранилищеЗначения(ДанныеДляСохранения, Новый СжатиеДанных(9));
						БезопасноеХранилищеДанных.Записать();
						Продолжить;
					КонецЕсли;
				КонецЕсли;
...
// Запускает фоновое задание с проверкой вызываемой процедуры.
//
// Параметры:
//  ИмяМетода - Строка - также как у метода платформы ФоновыеЗадания.Выполнить.
//  ПараметрыВыполнения - см. НовыеПараметрыВыполненияФоновогоЗадания
//                      - Неопределено - использовать значения по умолчанию.
//
// Возвращаемое значение:
//  ФоновоеЗадание - также как у метода платформы ФоновыеЗадания.Выполнить.
//
Функция ВыполнитьФоновоеЗадание(Знач ИмяМетода, Знач ПараметрыВыполнения = Неопределено) Экспорт
	
	Если ПараметрыВыполнения = Неопределено Тогда
		ПараметрыВыполнения = НовыеПараметрыВыполненияФоновогоЗадания();
	КонецЕсли;
	
	Попытка
		СтандартныеПодсистемыСервер.ПроверитьМетодДляВызоваКакПроизвольныйКод(ИмяМетода, Истина);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильное значение параметра %1 (передано значение: ""%2"") в %3:'"),
			"ИмяМетода", ИмяМетода, "ОбщегоНазначения.ВыполнитьФоновоеЗадание");
	...
	Если ПараметрыВыполнения.СРасширениямиБазыДанных = Истина Тогда
		ФоновоеЗадание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных(ИмяМетода,
			ПараметрыВыполнения.Параметры, ПараметрыВыполнения.Ключ, ПараметрыВыполнения.Наименование);
		
	ИначеЕсли ПараметрыВыполнения.СРасширениямиБазыДанных = Ложь Тогда
		ФоновоеЗадание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеБезРасширений(ИмяМетода,
			ПараметрыВыполнения.Параметры, ПараметрыВыполнения.Ключ, ПараметрыВыполнения.Наименование);
	Иначе
...
// Конструктор параметров для функции ВыполнитьФоновоеЗадание.
//
// Возвращаемое значение:
//  Структура:
//    * Параметры    - Неопределено - значение по умолчанию.
//                   - Массив - также как у метода платформы ФоновыеЗадания.Выполнить.
//
//    * Ключ         - Неопределено - значение по умолчанию.
//                   - Строка - также как у метода платформы ФоновыеЗадания.Выполнить.
//
//    * Наименование - Неопределено - значение по умолчанию.
//                   - Строка - также как у метода платформы ФоновыеЗадания.Выполнить.
//
//    * СРасширениямиБазыДанных - Неопределено    - ФоновыеЗадания.Выполнить
//                              - Булево - Истина - РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных
//                                         Ложь   - РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеБезРасширений
//
Функция НовыеПараметрыВыполненияФоновогоЗадания() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Параметры");
	Результат.Вставить("Ключ");
	Результат.Вставить("Наименование");
	Результат.Вставить("СРасширениямиБазыДанных");
...
// Выполнить экспортную процедуру или функцию по имени с уровнем привилегий конфигурации.
...
//
// Параметры:
//  ИмяМетода  - Строка - имя экспортной процедуры или функции в формате
//                       <имя объекта>.<имя процедуры или функции>, где <имя объекта> - это
//                       общий модуль или модуль менеджера объекта.
//  Параметры  - Массив - параметры передаются в процедуру или функцию <ИмяМетода>
//                        в порядке расположения элементов массива.
//  ЭтоФункция - Булево - указать Истина, если вызывается функция.
//
// Возвращаемое значение:
//  Произвольный - если это функция, иначе Неопределено.
//
// Пример:
...
//
Функция ВыполнитьМетодКонфигурации(Знач ИмяМетода, Знач Параметры = Неопределено, ЭтоФункция = Ложь) Экспорт
	...
	СтрокаВызова = СтрокаВызоваМетодаКакПроизвольногоКода(ИмяМетода, Параметры);
	Если ЭтоФункция Тогда
		Возврат Вычислить(СтрокаВызова); // АПК:488 Исполняемый код безопасен.
	КонецЕсли;
	Выполнить(СтрокаВызова); // АПК:487 Исполняемый код безопасен.
	
КонецФункции

// Выполнить экспортную процедуру или функцию объекта по имени.
// При включении профилей безопасности для вызова оператора Выполнить() используется
// переход в безопасный режим с профилем безопасности, используемым для информационной базы
// (если выше по стеку не был установлен другой безопасный режим).
//
// Параметры:
//  ОбъектМетода - ПланОбменаОбъект
...
//  Произвольный - если это функция, иначе Неопределено.
//
Функция ВыполнитьМетодОбъекта(Знач ОбъектМетода, Знач ИмяМетода, Знач Параметры = Неопределено, ЭтоФункция = Ложь) Экспорт
	...
	СтрокаВызова = СтрокаВызоваМетодаКакПроизвольногоКода(ИмяМетода, Параметры, ОбъектМетода);
	Если ЭтоФункция Тогда
		Возврат Вычислить(СтрокаВызова); // АПК:488 Исполняемый код безопасен.
	КонецЕсли;
	Выполнить(СтрокаВызова); // АПК:487 Исполняемый код безопасен.
...
//  
//  3) Вызов процедур и функций БСП:
...
Функция ВычислитьВБезопасномРежиме(Знач Выражение, Знач Параметры = Неопределено,
	...
	ПроверитьАлгоритм(Выражение, ПараметрыПроверкиАлгоритма);
	...
	Попытка
		Если ТранзакцияАктивна() Тогда
			Результат = Вычислить(Выражение); // АПК:488 Код выполняется в безопасном режиме.
		Иначе
			НачатьТранзакцию(); // АПК:326 принудительная отмена транзакции
			Попытка
				Результат = Вычислить(Выражение); // АПК:488 Код выполняется в безопасном режиме.
...
// Возвращает строку для вызова указанного метода с параметрами с помощью Выполнить/Вычислить.
// См. ВыполнитьМетодКонфигурации и см. ВыполнитьМетодОбъекта.
//
// Проверяет, что переданное имя метода является именем экспортной процедуры или функции
// конфигурации или объекта и не содержит произвольного алгоритма на встроенном языке 1С:Предприятия.
// В случае если переданная строка не соответствует требованиям, вызывается исключение.
//
// Параметры:
//  ИмяМетода    - см. ВыполнитьМетодОбъекта.ИмяМетода
//               - см. ВыполнитьМетодКонфигурации.ИмяМетода
//  Параметры    - см. ВыполнитьМетодОбъекта.Параметры
//               - см. ВыполнитьМетодКонфигурации.Параметры
//  ОбъектМетода - см. ВыполнитьМетодОбъекта.ОбъектМетода
//               - Неопределено - строка для вызова метода конфигурации.
//
// Возвращаемое значение:
//  Строка - если параметр Объект указан, тогда строка в формате
//           "ОбъектМетода.ИмяМетода(Параметры[0],...)",
//           если параметр Объект не указан, тогда строка в формате
//           "ИмяМетода(Параметры[0],...)".
//
Функция СтрокаВызоваМетодаКакПроизвольногоКода(Знач ИмяМетода, Знач Параметры = Неопределено,
			Знач ОбъектМетода = Неопределено) Экспорт
	
	Если ОбъектМетода = Неопределено Тогда
		...
		Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ИмяМетода", ИмяМетода, "ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода");
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
		...
		Если ЧастиИмени.Количество() = 2 И Метаданные.ОбщиеМодули.Найти(ИмяОбъекта) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ИмяМетода", ИмяМетода, "ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода", ИмяОбъекта);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
		Если ЧастиИмени.Количество() = 3 Тогда
			ПолноеИмяОбъекта = ЧастиИмени[0] + "." + ЧастиИмени[1];
			Попытка
				Менеджер = МенеджерОбъектаПоИмени(ПолноеИмяОбъекта, Истина, Истина, Истина,
					"ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода", "ИмяМетода");
			Исключение
			...
			Если Менеджер = Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					"ИмяМетода", ИмяМетода, "ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода", ПолноеИмяОбъекта);
				ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
			КонецЕсли;
		КонецЕсли;
		ИмяМетодаОбъекта = ЧастиИмени[ЧастиИмени.ВГраница()];
	Иначе
		ИмяМетодаОбъекта = ИмяМетода;
		СериализованныйТипОбъекта = ЗначениеВСтрокуВнутр(ТипЗнч(ОбъектМетода));
		Если Не СтандартныеПодсистемыПовтИсп.ЭтоОбъектСМодулем(СериализованныйТипОбъекта)
		   И Не ЭтоВнешнийОтчетИлиВнешняяОбработка(ОбъектМетода) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неправильный формат параметра %1 (передано значение типа: ""%2"") в %3:
				           |Указан недопустимый тип объекта для выполнения метода.'"),
				"ОбъектМетода",
				СтроковоеПредставлениеТипа(ТипЗнч(ОбъектМетода)),
				"ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода");
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильный формат параметра %1 (передано значение: ""%2"") в %3:
			           |Имя ""%4"" не соответствует требованиям образования имен процедур и функций.'"),
			"ИмяМетода", ИмяМетода, "ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода", ИмяМетодаОбъекта);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецПопытки;
	
	Попытка
		СтандартныеПодсистемыСервер.ПроверитьМетодДляВызоваКакПроизвольныйКод(ИмяМетода,, ОбъектМетода);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильное значение параметра %1 (передано значение: ""%2"") в %3:'"),
	...
	Если Параметры <> Неопределено Тогда
		Если ТипЗнч(Параметры) <> Тип("Массив") Тогда
			...
			ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ОбщегоНазначения.СтрокаВызоваМетодаКакПроизвольногоКода",
				"Параметры", Параметры, ДопустимыеТипы);
		КонецЕсли;
	...
	Если ОбъектМетода = Неопределено Тогда
		Возврат ИмяМетода + "(" + ПараметрыСтрока + ")";
	КонецЕсли;
	
	Возврат "ОбъектМетода." + ИмяМетода + "(" + ПараметрыСтрока + ")";
...
Функция ПараметрыПроверкиАлгоритма() Экспорт
	...
	ПараметрыПроверки.Вставить("НедопустимыеМетоды", Новый Массив);
	ПараметрыПроверки.Вставить("ЗапрещенныеСвойстваГлобальногоКонтекста", Новый Массив);
	ПараметрыПроверки.Вставить("ЗапрещенныеМодули", Новый Массив);
...
Процедура ПриНачалеВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено) Экспорт
	...
	Если РегистрыСведений.ПараметрыРаботыПрограммы.НеобходимоОбновление() Тогда
		Текст = НСтр("ru = 'Вход в приложение временно невозможен в связи с обновлением на новую версию.
	...
	Если Не РазделениеВключено()
	   ...
	   И ЗначениеЗаполнено(Константы.ГлавныйУзел.Получить()) Тогда
		...
		Текст = НСтр("ru = 'Вход в приложение временно невозможен до восстановления связи с главным узлом.
	...
	Если СтандартныеПодсистемыСервер.ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы() Тогда
		Текст = НСтр("ru = 'Регламентное задание недоступно до установки начальных региональных настроек приложения.
			                |Выполнение прервано.'");
		РегламентныеЗаданияСервер.ОтменитьВыполнениеЗадания(РегламентноеЗадание, Текст);
		ВызватьИсключение Текст;
	КонецЕсли;
...
Процедура ЗаполнитьКоллекциюЭлементовДереваДанныхФормы(КоллекцияЭлементовДерева, ДеревоЗначений) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДеревоЗначений.Строки Цикл
		...
		Если СтрокаТаблицы.Строки.Количество() > 0 Тогда
			ЗаполнитьКоллекциюЭлементовДереваДанныхФормы(ЭлементДерева.ПолучитьЭлементы(), СтрокаТаблицы);
		КонецЕсли;
...
Функция ЭтоДобавлениеНабораЗаписей(Режим) Экспорт
	...
	Значение = СтандартныеПодсистемыПовтИсп.РежимДобавленияНабораЗаписей();
	Возврат Значение <> Неопределено И Режим = Значение;
...
Функция ЭтоЗамещениеНабораЗаписей(Режим) Экспорт
	...
	Значение = СтандартныеПодсистемыПовтИсп.РежимЗамещенияНабораЗаписей();
	Возврат Значение <> Неопределено И Режим = Значение;
...
Функция ЭтоОбновлениеНабораЗаписей(Режим) Экспорт
	...
	Значение = СтандартныеПодсистемыПовтИсп.РежимОбновленияНабораЗаписей();
	Возврат Значение <> Неопределено И Режим = Значение;
...
Функция ЭтоСлияниеНабораЗаписей(Режим) Экспорт
	...
	Значение = СтандартныеПодсистемыПовтИсп.РежимСлиянияНабораЗаписей();
	Возврат Значение <> Неопределено И Режим = Значение;
...
Функция ЭтоУдалениеНабораЗаписей(Режим) Экспорт
	...
	Значение = СтандартныеПодсистемыПовтИсп.РежимУдаленияНабораЗаписей();
	Возврат Значение <> Неопределено И Режим = Значение;
...
Функция РежимОбновленияНабораЗаписей() Экспорт
	...
	Возврат СтандартныеПодсистемыПовтИсп.РежимОбновленияНабораЗаписей();
...
Функция РежимСлиянияНабораЗаписей() Экспорт
	...
	Возврат СтандартныеПодсистемыПовтИсп.РежимСлиянияНабораЗаписей();
...
Функция РежимУдаленияНабораЗаписей() Экспорт
	...
	Возврат СтандартныеПодсистемыПовтИсп.РежимУдаленияНабораЗаписей();
...
Функция ОбъектПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение параметра %1 в функции %2. Некорректное полное имя отчета или обработки ""%3"".'"), 
			"ПолноеИмя", "ОбщегоНазначения.ОбъектПоПолномуИмени", ПолноеИмя),
			КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение параметра %1 в функции %2. ""%3"" не является отчетом или обработкой.'"), 
			"ПолноеИмя", "ОбщегоНазначения.ОбъектПоПолномуИмени", ПолноеИмя),
			КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
КонецФункции
...
Функция ПодключитьКомпонентуПоИдентификатору(Знач Идентификатор, Знач Местоположение, Знач Изолированно = Неопределено) Экспорт
...
#КонецЕсли
	...
	Исключение
		...
		Результат.ОписаниеОшибки = ЗаголовокОшибки + Символы.ПС
			+ ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		
		КомментарийДляЖурнала = ЗаголовокОшибки + Символы.ПС
			+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)
			+ СистемнаяИнформацияДляЖурналаРегистрации();
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Подключение внешней компоненты на сервере'", КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, КомментарийДляЖурнала);
		Возврат Результат;
	КонецПопытки;
	
	Если Не РезультатПодключения Тогда
		
		ОшибкаСовместимостиКомпонентыИзМакета = ОшибкаСовместимостиКомпонентыИзМакета(Местоположение);
		
		Если ЗначениеЗаполнено(ОшибкаСовместимостиКомпонентыИзМакета) Тогда
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на сервере по причине:
					 |%2.
					 |Техническая информация:
					 |%3
					 |Метод %4 вернул Ложь.'"), Идентификатор, ОшибкаСовместимостиКомпонентыИзМакета, Местоположение,
		...
		Иначе
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на сервере.
					 |Техническая информация:
					 |%2
					 |Метод %3 вернул Ложь.'"), Идентификатор, Местоположение, "ПодключитьВнешнююКомпоненту");
		КонецЕсли;
		
		Попытка
			ВызватьИсключение НСтр("ru = 'Стек вызовов:'")
		Исключение
			КомментарийДляЖурнала = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС
				+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
				+ СистемнаяИнформацияДляЖурналаРегистрации();
		КонецПопытки;
	...
	Если ПодключаемыйМодуль = Неопределено Тогда
		...
		КомментарийДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось создать объект внешней компоненты ""%1"", подключенной на сервере, по причине:
			           |%2'"),
			Идентификатор, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)
				+ СистемнаяИнформацияДляЖурналаРегистрации());
	...
	ОтправитьСтатистикуПодключенияКомпоненты(Идентификатор, Местоположение);
	Возврат Результат;
...
Функция ДоменноеИмяКорректное(ДоменноеИмя) Экспорт
	
	Если СтрДлина(ДоменноеИмя) > 255 Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Выражение = "(?:xn--[a-z0-9-]{1,59})(?:\.xn--[a-z0-9-]{1,59})*"
		+ "|(?!^\d+$)(?:(?!-)[a-z0-9-]{1,63}(?<!-))(?:\.(?!-)[a-z0-9-]{1,63}(?<!-))*";
	
	Возврат СтрПодобнаПоРегулярномуВыражению(ДоменноеИмя, Выражение, Истина);
	
КонецФункции

// Проверяет произвольный алгоритм на встроенном языке 1С:Предприятия, на наличие в нем опасных конструкций,
// позволяющих получить неограниченный доступ к базе.
//
// Параметры:
//  Алгоритм - Строка - алгоритм на встроенном языке 1С:Предприятия, который требуется проверить.
//  ПараметрыПроверки - см. ОбщегоНазначения.ПараметрыПроверкиАлгоритма
//                    - Неопределено
//
Процедура ПроверитьАлгоритм(Знач Алгоритм, ПараметрыПроверки) Экспорт
	
	ТекстыОшибок = Новый Массив;
	
	Если ПараметрыПроверки = Неопределено Тогда
		ПараметрыПроверки = ПараметрыПроверкиАлгоритма();
	КонецЕсли;
	
	Алгоритм = ВРег(Алгоритм);
	// Исключение метода объекта Запрос.
	Алгоритм = СтрЗаменить(СтрЗаменить(Алгоритм, ВРег("Выполнить()"), ""), ВРег("Execute()"), "");
	// Проверка наличия в алгоритме недопустимых методов.
	НедопустимыеМетоды = НедопустимыеМетоды();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НедопустимыеМетоды, ПараметрыПроверки.НедопустимыеМетоды, Истина);
	НайденныеВызовы = НайденныеВызовы(Алгоритм, НедопустимыеМетоды, "(");
	Если НайденныеВызовы.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Вызовы недопустимых методов:
			|%1'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрСоединить(НайденныеВызовы, Символы.ПС));
		
		ТекстыОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	
	// Проверка обращений к общим модулям и свойствам глобального контекста.
	ЗапрещенныеСвойстваГлобальногоКонтекста = ЗапрещенныеСвойстваГлобальногоКонтекста();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ЗапрещенныеСвойстваГлобальногоКонтекста, ПараметрыПроверки.ЗапрещенныеСвойстваГлобальногоКонтекста, Истина);
	НайденныеВызовы = НайденныеВызовы(Алгоритм, ЗапрещенныеСвойстваГлобальногоКонтекста, ".");
	НайденныеВызовыАльтернативные = НайденныеВызовы(Алгоритм, ЗапрещенныеСвойстваГлобальногоКонтекста, "[");
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НайденныеВызовы, НайденныеВызовыАльтернативные, Истина);
	Если НайденныеВызовы.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Вызовы недопустимых свойств глобального контекста:
			|%1'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрСоединить(НайденныеВызовы, Символы.ПС));
	...
	ЗапрещенныеМодули = ЗапрещенныеМодули();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗапрещенныеМодули, ПараметрыПроверки.ЗапрещенныеМодули, Истина);
	НайденныеВызовы = НайденныеВызовы(Алгоритм, ЗапрещенныеМодули, ".");
	Если НайденныеВызовы.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Вызовы недопустимых модулей:
			|%1'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрСоединить(НайденныеВызовы, Символы.ПС));
	...
	Если ТекстыОшибок.Количество() > 0 Тогда
		ВызватьИсключение (СтрСоединить(ТекстыОшибок, Символы.ПС + Символы.ПС), КатегорияОшибки.ОшибкаВнешнегоИсточникаДанных);
	КонецЕсли;
	...
	Для Каждого МестоИспользования Из Результат.МестаИспользования Цикл
		...
		Иначе
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Замена ссылок в ""%1"" недоступна.'"), Информация.ПолноеИмя);
...
Процедура ЗаменитьСсылкиКороткимиТранзакциями(Результат, Знач ПараметрыВыполнения, Знач Дубль, Знач ТаблицаПоиска)
	...
	Иначе
		...
		ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, ТаблицаПовторногоПоиска, ПараметрыВыполнения);
	КонецЕсли;
...
Процедура ПроизвестиЗаменуВКонстанте(Результат, Знач МестоИспользования, Знач ПараметрыЗаписи)
	...
	Попытка
		...
		Исключение
			...
			УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заменить в константе ""%1"", т.к. данные редактируются в другом сеансе работы с приложением. 
				|Повторите попытку замены.'"), 
				ПредставлениеДанных);
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке, УточнениеОшибки);
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, ИнформацияОбОшибке);
		КонецПопытки;
		...
		Для Каждого СтрокаТаблицы Из ОбрабатываемыеСтроки Цикл
			Если МенеджерКонстанты.Значение = СтрокаТаблицы.Ссылка Тогда
				МенеджерКонстанты.Значение = СтрокаТаблицы.ЦелеваяСсылка;
		...
		Исключение
			...
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке,
				НСтр("ru = 'Не удалось заменить по причине:'"));
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, ИнформацияОбОшибке);
		КонецПопытки;
	...
	Исключение
		...
		Если СостояниеОперации = "ОшибкаЗаписи" Тогда
			Для Каждого СтрокаТаблицы Из ОбрабатываемыеСтроки Цикл
				ЗарегистрироватьОшибкуЗамены(Результат, СтрокаТаблицы.Ссылка,
					ОписаниеОшибкиЗамены("ОшибкаЗаписи", Данные, ПредставлениеДанных, ИнформацияОбОшибке()),
					ПараметрыЗаписи.ПодчиненныеКДублям);
			КонецЦикла;
		Иначе
			ЗарегистрироватьОшибкуЗамены(Результат, СтрокаТаблицы.Ссылка,
				ОписаниеОшибкиЗамены(СостояниеОперации, Данные, ПредставлениеДанных, ИнформацияОбОшибке()),
				ПараметрыЗаписи.ПодчиненныеКДублям);
		КонецЕсли;
	КонецПопытки;
...
Процедура ПроизвестиЗаменуВОбъекте(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения)
	...
	Попытка
		...
		Исключение
			СостояниеОперации = "ОшибкаБлокировки";
			УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заменить в ""%1"", т.к. данные редактируются в другом сеансе работы с приложением. 
				|Повторите попытку замены.'"), 
				ПредставлениеДанных);
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке, УточнениеОшибки);
	...
	Исключение
		...
		Ошибка = ОписаниеОшибкиЗамены(СостояниеОперации, Данные, ПредставлениеДанных, ИнформацияОбОшибке());
		Если СостояниеОперации = "ОшибкаЗаписи" Тогда
 			Для Каждого СтрокаТаблицы Из ОбрабатываемыеСтроки Цикл
				ЗарегистрироватьОшибкуЗамены(Результат, СтрокаТаблицы.Ссылка, Ошибка,
					ПараметрыВыполнения.ПодчиненныеКДублям);
			КонецЦикла;
		Иначе
...
Процедура ПроизвестиЗаменуВНаборе(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения)
	...
	ОписаниеНабора = ОписаниеНабораЗаписей(МетаданныеРегистра);
	НаборЗаписей = ОписаниеНабора.НаборЗаписей; // РегистрСведенийНаборЗаписей
	
	ПарыЗамен = Новый Соответствие;
	Для Каждого СтрокаТаблицы Из ОбрабатываемыеСтроки Цикл
		ПарыЗамен.Вставить(СтрокаТаблицы.Ссылка, СтрокаТаблицы.ЦелеваяСсылка);
	КонецЦикла;
	...
	Попытка
		...
		Для Каждого КлючЗначение Из ОписаниеНабора.СписокИзмерений Цикл
			...
			Для Каждого СтрокаТаблицы Из ОбрабатываемыеСтроки Цикл
				ТекущаяСсылка = СтрокаТаблицы.Ссылка;
		...
		Исключение
			СостояниеОперации = "ОшибкаБлокировки";
			УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заменить в регистре ""%1"", т.к. данные редактируются в другом сеансе работы с приложением. 
				|Повторите попытку замены.'"), 
				ПредставлениеДанных);
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке, УточнениеОшибки);
	...
	Исключение
		...
		Информация = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗаменыСсылок(), УровеньЖурналаРегистрации.Ошибка,
			МетаданныеРегистра,, ОбработкаОшибок.ПодробноеПредставлениеОшибки(Информация));
		Ошибка = ОписаниеОшибкиЗамены(СостояниеОперации, Данные, ПредставлениеДанных, ИнформацияОбОшибке());
		Если СостояниеОперации = "ОшибкаЗаписи" Тогда
			Для Каждого СтрокаТаблицы Из ОбрабатываемыеСтроки Цикл
				ЗарегистрироватьОшибкуЗамены(Результат, СтрокаТаблицы.Ссылка, Ошибка,
...
Процедура ПроизвестиЗаменуВРегистреСведений(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения)
	...
	Попытка
		...
		Если НаборЗаписейДубля.Количество() = 0 Тогда 
			ОтменитьТранзакцию();
			Возврат;
		КонецЕсли;
...
Функция ИзмененныеОбъектыПриЗаменеВОбъекте(ПараметрыВыполнения, МестоИспользования, ОбрабатываемыеСтроки)
	...
	ОписаниеДвижений            = ОписаниеДвижений(МестоИспользования.Метаданные, ПараметрыВыполнения);
...
Функция УстановитьПометкуУдаления(Результат, Знач УдаляемаяСсылка, Знач ВсеМестаИспользования, Знач ПараметрыВыполнения, 
	...
	Если МестаИспользования.Количество() > 0 Тогда
		ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, МестаИспользования, ПараметрыВыполнения);
	...
	Успешно = Истина;
	Попытка 
		ЗаписатьОбъектСПерехватомСообщений(Объект, "ПометкаУдаления", Неопределено, ПараметрыВыполнения);
		Результат.ОчередьКНепосредственномуУдалению.Добавить(Объект.Ссылка);
	Исключение
		Успешно = Ложь;
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке,
			НСтр("ru = 'Элемент не был помечен на удаление по причине:'"));
		Попытка
			ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, ИнформацияОбОшибке);
		Исключение
			ОписаниеОшибки = ОписаниеОшибкиЗамены("ОшибкаУдаления", УдаляемаяСсылка, ПредставлениеСсылки, ИнформацияОбОшибке());
			ЗарегистрироватьОшибкуЗамены(Результат, УдаляемаяСсылка, ОписаниеОшибки, ПараметрыВыполнения.ПодчиненныеКДублям);
			Если ЕстьВнешняяТранзакция Тогда
				ВызватьИсключение;
			КонецЕсли;	
		КонецПопытки;
...
Процедура ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, ТаблицаПовторногоПоиска, ПараметрыВыполнения)
	
	Фильтр = Новый Структура("Ссылка, ОбъектОшибки");
	Для Каждого СтрокаТаблицы Из ТаблицаПовторногоПоиска Цикл
		Тест = Новый Структура("ВспомогательныеДанные", Ложь);
		ЗаполнитьЗначенияСвойств(Тест, СтрокаТаблицы);
		Если Тест.ВспомогательныеДанные Тогда
			Продолжить;
		КонецЕсли;
		
		Фильтр.ОбъектОшибки = СтрокаТаблицы.Данные;
		Фильтр.Ссылка       = СтрокаТаблицы.Ссылка;
		Если Результат.Ошибки.НайтиСтроки(Фильтр).Количество() > 0 Тогда
			Продолжить; // По данной проблеме уже записана ошибка.
		КонецЕсли;
		
		ЗарегистрироватьОшибкуЗамены(Результат, СтрокаТаблицы.Ссылка,
			ОписаниеОшибкиЗамены("ДанныеИзменены", СтрокаТаблицы.Данные, ПредметСтрокой(СтрокаТаблицы.Данные),
...
// Параметры:
//   ОбъектМетаданных - ОбъектМетаданных
//   ПараметрыВыполнения - см. ПараметрыПоискаМестИспользования
// 
// Возвращаемое значение:
//  Массив из Структура:
...
//
Функция ОписаниеДвижений(Знач ОбъектМетаданных, ПараметрыВыполнения = Неопределено)
	...
	Исключения = ИсключенияПоискаСсылок();
	
	Для Каждого Движение Из ОбъектМетаданных.Движения Цикл
		
		Если Исключения[Движение] = "*" Тогда
			Если ПараметрыВыполнения <> Неопределено И ПараметрыВыполнения.УдалятьНепосредственно Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
					ПараметрыВыполнения.ПараметрыПоискаМестИспользования.ИсключенияПоискаРегистровПодчиненныхРегистратору,
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Движение), Истина);
			КонецЕсли;
...
Функция ОписаниеНабораЗаписей(Знач МетаданныеТаблицы)
...
Процедура ЗаменитьВКоллекцииСтрок(ВидКоллекции, ИмяКоллекции, Объект, Коллекция, Знач СписокПолей, Знач ПарыЗамен)
	...
	Для Каждого СтрокаТаблицы Из ИзмененнаяКоллекция Цикл
		
		Для Каждого КлючЗначение Из СписокПолей Цикл
			ИмяРеквизита = КлючЗначение.Ключ;
			ЦелеваяСсылка = ПарыЗамен[ СтрокаТаблицы[ИмяРеквизита] ];
			Если ЦелеваяСсылка <> Неопределено Тогда
				ЗарегистрироватьФактЗамены(Объект, СтрокаТаблицы[ИмяРеквизита], ЦелеваяСсылка, ВидКоллекции, ИмяКоллекции, 
					ИзмененнаяКоллекция.Индекс(СтрокаТаблицы), ИмяРеквизита);
				СтрокаТаблицы[ИмяРеквизита] = ЦелеваяСсылка;
...
Процедура ЗагрузитьИзмененныйНаборВРегистрБухгалтерии(НаборЗаписей, ИзмененнаяКоллекция, ИменаИзмененныхРеквизитов)
	...
	НеизмененныеРесурсы = Новый Соответствие;
	ИзмененныеРесурсы = Новый Соответствие;
	...
	Для Каждого Ресурс Из МетаданныеРегистра.Ресурсы Цикл
		...
		Если Ресурс.Балансовый Или Не МетаданныеРегистра.Корреспонденция Тогда
			ИменаРесурсов.Добавить(Ресурс.Имя);
		Иначе
			ИменаРесурсов.Добавить(Ресурс.Имя + "Дт");
			ИменаРесурсов.Добавить(Ресурс.Имя + "Кт");
		КонецЕсли;
		
		Для Каждого ИмяРесурса Из ИменаРесурсов Цикл
			Если ИменаИзмененныхРеквизитов.Найти(ИмяРесурса) = Неопределено Тогда
				НеизмененныеРесурсы.Вставить(ИмяРесурса, НаборЗаписей.ВыгрузитьКолонку(ИмяРесурса));
			Иначе
				ИзмененныеРесурсы.Вставить(ИмяРесурса, НаборЗаписей.ВыгрузитьКолонку(ИмяРесурса));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Для Сч = 0 По НаборЗаписей.Количество() - 1 Цикл
		...
		Для Каждого ИмяРесурсаЗначения Из ИзмененныеРесурсы Цикл
			Если НаборЗаписей[Сч][ИмяРесурсаЗначения.Ключ] = NULL Тогда
				ИмяРесурсаЗначения.Значение[Сч] = NULL;
			Иначе
				ИмяРесурсаЗначения.Значение[Сч] = ИзмененнаяКоллекция[Сч][ИмяРесурсаЗначения.Ключ];
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого ИзмеренияЗначенияВКолонке Из НеизмененныеРесурсы Цикл
		НаборЗаписей.ЗагрузитьКолонку(ИзмеренияЗначенияВКолонке.Значение, ИзмеренияЗначенияВКолонке.Ключ);
	КонецЦикла;
	
	Для Каждого ИзмеренияЗначенияВКолонке Из ИзмененныеРесурсы Цикл
		НаборЗаписей.ЗагрузитьКолонку(ИзмеренияЗначенияВКолонке.Значение, ИзмеренияЗначенияВКолонке.Ключ);
	КонецЦикла;

КонецПроцедуры

Процедура ЗаписатьОбъектСПерехватомСообщений(Знач Объект, Знач Действие, Знач РежимЗаписи, Знач ПараметрыЗаписи)
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Уточнение = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(ИнформацияОбОшибке);
		Уточнение.Текст = Уточнение.Текст + Символы.ПС + СокрЛП(ТекстСообщений);
		ВызватьИсключение(Уточнение.Текст, Уточнение.Категория,,, ИнформацияОбОшибке);
	КонецПопытки;
...
Процедура ЗаписатьОбъект(Знач Объект, Знач ПараметрыЗаписи)
	...
	Если ЭтоДокумент(МетаданныеОбъекта) Тогда
		ЗаписатьОбъектСПерехватомСообщений(Объект, "Запись", РежимЗаписиДокумента.Запись, ПараметрыЗаписи);
		Возврат;
	КонецЕсли;
	...
	Если СвойстваОбъекта.Иерархический Или СвойстваОбъекта.ВидыСубконто <> Неопределено Тогда 
		...
		Если Объект.Родитель = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно записать ""%1"", т.к. его родительским элементом не может быть он сам.'"),
				ПредметСтрокой(Объект));
			КонецЕсли;
	...
	Если СвойстваОбъекта.Владельцы.Количество() > 1 И Объект.Владелец = Объект.Ссылка Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно записать ""%1"", т.к. его владельцем не может быть он сам.'"),
			ПредметСтрокой(Объект));
	КонецЕсли;
	...
	ЗаписатьОбъектСПерехватомСообщений(Объект, "Запись", Неопределено, ПараметрыЗаписи);
КонецПроцедуры
...
//   ПодчиненныеКДублям - Соответствие, Неопределено
//
Процедура ЗарегистрироватьОшибкуЗамены(Результат, Знач Ссылка, Знач ОписаниеОшибки, ПодчиненныеКДублям = Неопределено)
	...
	Строка.ИнформацияОбОшибке         = ?(ТипЗнч(ОписаниеОшибки.ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке"), 
		ОписаниеОшибки.ИнформацияОбОшибке, Неопределено);
	Строка.ТекстОшибки                = ?(ТипЗнч(ОписаниеОшибки.ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке"),
		ОбработкаОшибок.КраткоеПредставлениеОшибки(ОписаниеОшибки.ИнформацияОбОшибке), ОписаниеОшибки.ИнформацияОбОшибке);
	Строка.ТипОшибки                  = ОписаниеОшибки.ТипОшибки;
	
	ДубльРодитель = Ссылка;
	Если ПодчиненныеКДублям <> Неопределено И ПодчиненныеКДублям[Ссылка] <> Неопределено Тогда
		ДубльРодитель = ПодчиненныеКДублям[Ссылка];
	КонецЕсли;
	Строка.ДубльРодитель = ДубльРодитель;
...
//
Функция ОписаниеОшибкиЗамены(Знач ТипОшибки, Знач ОбъектОшибки, Знач ПредставлениеОбъектаОшибки, Знач ИнформацияОбОшибке)
...
Процедура ЗарегистрироватьОшибкиУдаления(Результат, ПрепятствующиеУдалению, ПараметрыВыполнения)

	ПредставленияСсылок = ПредметыСтрокой(ПрепятствующиеУдалению.ВыгрузитьКолонку("МестоИспользования"));
	Для каждого ПрепятствующийУдалению Из ПрепятствующиеУдалению Цикл
		ТекстОшибки = НСтр("ru = 'Элемент не удален, т.к. на него есть ссылки.'");
		ОписаниеОшибки = ОписаниеОшибкиЗамены("ОшибкаУдаления", ПрепятствующийУдалению.МестоИспользования,
			ПредставленияСсылок[ПрепятствующийУдалению.МестоИспользования], ТекстОшибки);
		ЗарегистрироватьОшибкуЗамены(Результат, ПрепятствующийУдалению.УдаляемыйСсылка, ОписаниеОшибки,
			ПараметрыВыполнения.ПодчиненныеКДублям);
	КонецЦикла;
...
Функция СформироватьДубли(ПараметрыВыполнения, ПараметрыЗамены, ПарыЗамен, Результат)
	...
	Если ПараметрыВыполнения.УчитыватьПрикладныеПравила И ПодсистемаСуществует("СтандартныеПодсистемы.ПоискИУдалениеДублей") Тогда
		...
		Для Каждого КлючЗначение Из Ошибки Цикл
			...
			ЗарегистрироватьОшибкуЗамены(Результат, Дубль, Причина, ПараметрыВыполнения.ПодчиненныеКДублям);
		КонецЦикла;
	КонецЕсли;
	Возврат Дубли;
...
Функция СравниваемыеКолонки(Знач КоллекцияСтрок, Знач ИменаКолонок, Знач ИсключаяКолонки)
	...
	Если ПустаяСтрока(ИменаКолонок) Тогда
		...
		Иначе	
			ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверное значение параметра %1 (тип %2) в %3. Укажите имена полей, по которым производится сравнение.'"),
				"КоллекцияСтрок1", ТипКоллекции, "ОбщегоНазначения.КоллекцииИдентичны"),
				КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
	Иначе
...
Процедура ПроверкаФиксированностиДанных(Данные, ДанныеВЗначенииФиксированныхТипов = Ложь)
	...
	ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверное значение параметра %1 в функции %2. Данные типа %3 не могут быть зафиксированы.'"),
		"Данные", "ОбщегоНазначения.ФиксированныеДанные", Строка(ТипДанных)),
		КатегорияОшибки.ОшибкаКонфигурации);
...
Функция ДанныеИзБезопасногоХранилища(Владельцы, Ключи, ОбщиеДанные, ОбластьДанных)
	
	ИмяБезопасногоХранилищаДанных = "РегистрСведений.БезопасноеХранилищеДанных";
	ЗапросОбласти = (РазделениеВключено()
		И ОбщиеДанные <> Истина
		И (ДоступноИспользованиеРазделенныхДанных()
			 Или ЗначениеЗаполнено(ОбластьДанных)));
	Если ЗапросОбласти Тогда
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"#ИмяБезопасногоХранилищаДанных",
		ИмяБезопасногоХранилищаДанных);
	ТекстЗапроса = ?((ЗначениеЗаполнено(ОбластьДанных) И ЗапросОбласти),
		СтрЗаменить(
			ТекстЗапроса,
			"&Условие",
			"БезопасноеХранилищеДанных.Владелец В (&Владельцы)
			|И БезопасноеХранилищеДанных.ОбластьДанныхВспомогательныеДанные = &ОбластьДанных"),
		СтрЗаменить(
			ТекстЗапроса,
			"&Условие",
			"БезопасноеХранилищеДанных.Владелец В (&Владельцы)"));
...
//
// Параметры:
//  Имя - Строка - имя например, "Справочники.Организации", "Справочник.Организации", "Справочники", "Справочник".
//  ЭтоПолноеИмя - Неопределено, Булево
//  ЭтоИмяМенеджера - Неопределено, Булево
//  ТребуетсяМодульМенеджера - Булево
//  ИмяВызывающейПроцедуры - Строка
//  ИмяПараметра - Строка
//  МенеджерПоТочномуИмени - Булево - если Ложь,
//    тогда для вложенных объектов (форм, реквизитов, ...)
//    возвращается менеджер основного объекта (по первым двум частям имени через точку),
//    при этом для перерасчетов возвращается менеджер перерасчета (по первым четырем частям имени через точку).
//
// Возвращаемое значение:
//  - см. МенеджерОбъектаПоПолномуИмени
//  - ПланыОбменаМенеджер
...
//  - ВнешнийИсточникДанныхКубыМенеджер
//  - ВнешнийИсточникДанныхКубТаблицыИзмеренийМенеджер
//
Функция МенеджерОбъектаПоИмени(Имя, ЭтоПолноеИмя, ЭтоИмяМенеджера, ТребуетсяМодульМенеджера,
			ИмяВызывающейПроцедуры, ИмяПараметра, МенеджерПоТочномуИмени = Истина)
	
	ЧастиИмени = СтрРазделить(Имя, ".");
	ПерваяЧастьИмени = ЧастиИмени[0];
	
	СвойстваВидовОбъектов = СтандартныеПодсистемыПовтИсп.СвойстваВидовОбъектовМетаданных();
	Свойства = СвойстваВидовОбъектов.ПоКоллекциям.Получить(ВРег(ПерваяЧастьИмени));
	НайденоПоКоллекциям = Истина;
	Если Свойства = Неопределено Тогда
		Свойства = СвойстваВидовОбъектов.ПоВидамОбъектов.Получить(ВРег(ПерваяЧастьИмени));
		НайденоПоКоллекциям = Ложь;
	КонецЕсли;
	КодОшибки = "СтандартныеПодсистемы.БазоваяФункциональность.ЭтоОшибкаПолученияМодуляМенеджераПоИмени";
	
	Если Свойства = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Не существует объектов ""%5"".'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ПерваяЧастьИмени);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	КонецЕсли;
	Если ЭтоИмяМенеджера = Истина И Не НайденоПоКоллекциям Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Указано ""%5"" - начало имени объекта метаданных, а ожидается начало имени менеджера объекта.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ПерваяЧастьИмени);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	ИначеЕсли ЭтоИмяМенеджера = Ложь И НайденоПоКоллекциям Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Указано ""%5"" - начало имени менеджера объекта, а ожидается начало имени объекта метаданных.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ПерваяЧастьИмени);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	КонецЕсли;
	
	Менеджер = Свойства.Менеджер;
	Если Менеджер = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. У объектов ""%5"" нет менеджера.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ПерваяЧастьИмени);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	КонецЕсли;
	
	ЕстьМодульМенеджера = Свойства.ЕстьМодульМенеджера;
	КоллекцияМетаданных = Метаданные[Свойства.ИмяКоллекции];
	Индекс = 0;
	
	Если ЧастиИмени.Количество() > 2
	   И (    Свойства.ИмяВидаОбъекта = "РегистрРасчета"
	      Или Свойства.ИмяВидаОбъекта = "ВнешнийИсточникДанных") Тогда
		
		Если Свойства.ИмяВидаОбъекта = "РегистрРасчета" Тогда
			УстановитьМенеджерВложенногоОбъекта("ПЕРЕРАСЧЕТЫ", "ПЕРЕРАСЧЕТ", Ложь,
				Индекс, Менеджер, ЕстьМодульМенеджера, КоллекцияМетаданных, Имя, ЧастиИмени,
				НайденоПоКоллекциям, ИмяВызывающейПроцедуры, ИмяПараметра, КодОшибки);
		Иначе
			УстановитьМенеджерВложенногоОбъекта("КУБЫ", "КУБ", Истина,
				Индекс, Менеджер, ЕстьМодульМенеджера, КоллекцияМетаданных, Имя, ЧастиИмени,
				НайденоПоКоллекциям, ИмяВызывающейПроцедуры, ИмяПараметра, КодОшибки);
			Если Индекс = 0 Тогда
				УстановитьМенеджерВложенногоОбъекта("ТАБЛИЦЫ", "ТАБЛИЦА", Истина,
					Индекс, Менеджер, ЕстьМодульМенеджера, КоллекцияМетаданных, Имя, ЧастиИмени,
					НайденоПоКоллекциям, ИмяВызывающейПроцедуры, ИмяПараметра, КодОшибки);
			ИначеЕсли ЧастиИмени.Количество() > 4 Тогда
				УстановитьМенеджерВложенногоОбъекта("ТАБЛИЦЫИЗМЕРЕНИЙ", "ТАБЛИЦАИЗМЕРЕНИЯ", Истина,
					Индекс, Менеджер, ЕстьМодульМенеджера, КоллекцияМетаданных, Имя, ЧастиИмени,
					НайденоПоКоллекциям, ИмяВызывающейПроцедуры, ИмяПараметра, КодОшибки);
			КонецЕсли;
		КонецЕсли;
	...
	Если ЧастиИмени.Количество() = Индекс + 1 Тогда
		Если Не ЭтоПолноеИмя Тогда
			Возврат Менеджер;
		КонецЕсли;
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Ожидается полное имя объекта.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	КонецЕсли;
	
	Если ТребуетсяМодульМенеджера И Не ЕстьМодульМенеджера Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. У объектов ""%5"" нет модулей менеджера.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ЧастиИмени[Индекс]);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	КонецЕсли;
	
	ИмяОбъекта = ЧастиИмени[Индекс + 1];
	...
	Исключение
		Если КоллекцияМетаданных.Найти(ИмяОбъекта) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Не существует объект ""%5"".'"),
				Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ИмяОбъекта);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки;
...
// АПК:134-выкл - Количество параметров более 7 для компактного описания и производительности (0-2 вызовов за раз).

// Для функции МенеджерОбъектаПоИмени.
Процедура УстановитьМенеджерВложенногоОбъекта(ИмяКоллекции, ИмяВидаОбъекта, ЕстьМодуль,
			Индекс, Менеджер, ЕстьМодульМенеджера, КоллекцияМетаданных, Имя, ЧастиИмени,
			НайденоПоКоллекциям, ИмяВызывающейПроцедуры, ИмяПараметра, КодОшибки)
	
	ПерваяЧастьВложенногоИмени = ЧастиИмени[Индекс + 2];
	
	Если ВРег(ПерваяЧастьВложенногоИмени) <> ИмяВидаОбъекта
	   И ВРег(ПерваяЧастьВложенногоИмени) <> ИмяКоллекции Тогда
		Возврат;
	КонецЕсли;
	
	Если ВРег(ПерваяЧастьВложенногоИмени) = ИмяВидаОбъекта И НайденоПоКоллекциям Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Указано ""%5"" - часть имени объекта метаданных, а ожидается часть имени менеджера объекта.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ПерваяЧастьВложенногоИмени);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
		
	ИначеЕсли ВРег(ПерваяЧастьВложенногоИмени) = ИмяКоллекции И Не НайденоПоКоллекциям Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Указано ""%5"" - часть имени менеджера объекта, а ожидается часть имени объекта метаданных.'"),
			Имя, ТипЗнч(Имя), ИмяПараметра, ИмяВызывающейПроцедуры, ПерваяЧастьВложенногоИмени);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации, КодОшибки);
	КонецЕсли;
	...
	Попытка
		Менеджер = Менеджер[ИмяОбъекта][ИмяКоллекции];
	Исключение
		Если КоллекцияМетаданных.Найти(ИмяОбъекта) = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверное значение ""%1"" (тип %2) параметра %3 в %4. Не существует объект ""%5"".'"),
	...
	ЕстьМодульМенеджера = ЕстьМодуль;
	КоллекцияМетаданных = КоллекцияМетаданных[ИмяОбъекта][ИмяКоллекции];
	Индекс              = Индекс + 2;
...
Функция ЭтоВнешнийОтчетИлиВнешняяОбработка(ОбъектМетода)
	
	Попытка
		ПолноеИмя = ОбъектМетода.Метаданные().ПолноеИмя();
	Исключение
		Возврат Ложь;
	КонецПопытки;
	...
	ИмяВидаОбъекта = ЧастиИмени[0];
	
	Возврат ИмяВидаОбъекта = "ВнешнийОтчет"     // @Non-NLS
	    Или ИмяВидаОбъекта = "ВнешняяОбработка" // @Non-NLS
	    Или ИмяВидаОбъекта = "ExternalReport"
	    Или ИмяВидаОбъекта = "ExternalDataProcessor";
	
КонецФункции

// Следует использовать ВыполнитьМетодКонфигурации.
//
// Параметры:
//  ИмяМетода - см. ВыполнитьМетодКонфигурации.ИмяМетода
//  Параметры - см. ВыполнитьМетодКонфигурации.Параметры
//
// Возвращаемое значение:
//  см. ВыполнитьМетодКонфигурации
//
Функция ВызватьФункциюКонфигурации(ИмяМетода, Параметры = Неопределено) Экспорт
	Возврат ВыполнитьМетодКонфигурации(ИмяМетода, Параметры, Истина);
КонецФункции

// Следует использовать ВыполнитьМетодОбъекта.
//
// Параметры:
//  Объект    - см. ВыполнитьМетодОбъекта.Объект
//  ИмяМетода - см. ВыполнитьМетодОбъекта.ИмяМетода
//  Параметры - см. ВыполнитьМетодОбъекта.Параметры
//
// Возвращаемое значение:
//  см. ВыполнитьМетодОбъекта
//
Функция ВызватьФункциюОбъекта(Объект, ИмяМетода, Параметры = Неопределено) Экспорт
	Возврат ВыполнитьМетодОбъекта(Объект, ИмяМетода, Параметры, Истина);
КонецФункции
...
Функция ОшибкаСовместимостиКомпонентыИзМакета(Местоположение)
	...
	Если Не ПодсистемаСуществует("СтандартныеПодсистемы.ВнешниеКомпоненты") Тогда
		Возврат "";
	КонецЕсли;
	
	МодульВнешниеКомпонентыСлужебный = ОбщийМодуль("ВнешниеКомпонентыСлужебный");
	
	Возврат МодульВнешниеКомпонентыСлужебный.ОшибкаСовместимостиКомпонентыИзМакета(
		Местоположение);
	
КонецФункции

Функция СистемнаяИнформацияДляЖурналаРегистрации()
	
	Возврат СтандартныеПодсистемыСервер.ИнформацияДляПоддержки();
	
КонецФункции

Процедура ОтправитьСтатистикуПодключенияКомпоненты(Идентификатор, Местоположение)
	...
	МодульЦентрМониторинга = ОбщийМодуль("ЦентрМониторинга");
	МодульЦентрМониторинга.ЗаписатьОперациюБизнесСтатистики(
		"СтандартныеПодсистемы.ВнешниеКомпоненты." + Идентификатор + ".НаСервере." + ?(СтрНачинаетсяС(Местоположение, "e1cib"), "Справочник", "Макет"), 1);
...
Функция МинимальнаяВерсияПлатформы() Экспорт // АПК:581 - экспортная, так как используется при тестировании.
	...
	МинимальныеВерсииПлатформы = СтандартныеПодсистемыСервер.МинимальнаяВерсияПлатформыДляИспользования();
	НайденнаяВерсия = МинимальныеВерсииПлатформы.НайтиПоЗначению(ВерсияРежимаСовместимости);
	
	Если НайденнаяВерсия = Неопределено Тогда
		НайденнаяВерсия = МинимальныеВерсииПлатформы[МинимальныеВерсииПлатформы.Количество() - 1];
	КонецЕсли;
	
	Возврат НайденнаяВерсия.Представление;
...
Функция ЭтоВерсияЗащищенногоПрограммногоКомплекса(Версия)
	...
	СистемнаяИнформация = Новый СистемнаяИнформация();
	Возврат ЗначениеЗаполнено(СистемнаяИнформация.ВариантПриложения);
...
Процедура УточнитьВерсиюПлатформы(ОбщиеПараметры)
	...
	СистемнаяИнформация = Новый СистемнаяИнформация;
	НоваяСборка = СтандартныеПодсистемыСервер.НомерВерсииПлатформыВместоОтозванной(СистемнаяИнформация.ВерсияПриложения);
	Если Не ЗначениеЗаполнено(НоваяСборка) Тогда
		НоваяРекомендуемаяСборка = СтандартныеПодсистемыСервер.НомерВерсииПлатформыВместоОтозванной(ОбщиеПараметры.МинимальнаяВерсияПлатформы);
...
Процедура УточнитьМинимальнуюВерсиюПлатформыИзСервиса(ОбщиеПараметры)
	...
	МинимальнаяИзСервиса = "";
	РекомендуемаяИзСервиса = "";

	СтандартныеПодсистемыСерверЛокализация.ПриОпределенииМинимальнойИРекомендуемойВерсийПлатформы(
		МинимальнаяИзСервиса, РекомендуемаяИзСервиса);
	
	Если Не ЗначениеЗаполнено(МинимальнаяИзСервиса) И Не ЗначениеЗаполнено(РекомендуемаяИзСервиса) Тогда
		Возврат;
	КонецЕсли;
	
	Минимальная = НомерСборкиПоТекущейВерсииПлатформы(МинимальнаяИзСервиса);
	Рекомендуемая = НомерСборкиПоТекущейВерсииПлатформы(РекомендуемаяИзСервиса);
	
	Если МинимальнаяИРекомендуемаяВерсияПлатформыЗаполненаНеПравильно(Минимальная, Рекомендуемая) Тогда
		СтандартныеПодсистемыСерверЛокализация.ПриНеверномЗаполненииМинимальнойИРекомендуемойВерсийПлатформы(
			Минимальная, Рекомендуемая, НомерСборкиПоТекущейВерсииПлатформы(МинимальнаяВерсияПлатформы()), ОбщиеПараметры);
		Возврат;
	КонецЕсли;
...
#Область ПроверкаАлгоритмов

Функция НайденныеВызовы(Алгоритм, СтрокиПоиска, Окончание = Неопределено)
	
	НайденныеВызовы  = Новый Массив;
	ПоискОбращенияЧерезТочку = Ложь;
	ПоискТаблицы = Ложь;
	Для Каждого Элемент Из СтрокиПоиска Цикл
		
		Если ТипЗнч(Элемент) = Тип("Структура") Тогда
			ПоискОбращенияЧерезТочку = Истина;
			СтрокаПоиска = Элемент.Коллекция;
		Иначе
			СтрокаПоиска = Элемент;
		КонецЕсли;
		
		ПозицияВызова  = СтрНайти(Алгоритм, ВРег(СтрокаПоиска));
		НомерВхождения = 1;
		Пока ПозицияВызова > 0 Цикл
			Если ПоискОбращенияЧерезТочку Тогда
				Окончание = ".";
				ПоискТаблицы = Ложь;
			КонецЕсли;
			
			ЧислоСимволов  = СтрДлина(Окончание);
			ПозицияСимвола = ПозицияВызова + СтрДлина(СтрокаПоиска);
			
			Пока ПозицияСимвола + ЧислоСимволов < СтрДлина(Алгоритм) Цикл
				ЗначениеСимвола = СокрЛП(Сред(Алгоритм, ПозицияСимвола, ЧислоСимволов));
				Если ВРег(СтрокаПоиска) = ВРег("Выполнить")
					Или ВРег(СтрокаПоиска) = ВРег("Execute") Тогда
					
					Если Не ЗначениеЗаполнено(ЗначениеСимвола) Тогда
						НайденныеВызовы.Добавить(СтрокаПоиска);
						Прервать;
					КонецЕсли;
				КонецЕсли;
				
				Если СтрДлина(ЗначениеСимвола) < СтрДлина(Окончание) Тогда
					ЧислоСимволов = ЧислоСимволов + 1;
				ИначеЕсли ПоискОбращенияЧерезТочку
					И ЗначениеСимвола = Окончание
					И Не ПоискТаблицы Тогда
					ПозицияСимвола = ПозицияСимвола + ЧислоСимволов;
					Окончание      = ВРег(Элемент.Таблица);
					ЧислоСимволов  = СтрДлина(Окончание);
					ПоискТаблицы = Истина;
				ИначеЕсли ЗначениеСимвола = Окончание Тогда
					Если ПоискОбращенияЧерезТочку Тогда
						НайденныеВызовы.Добавить(Элемент.Коллекция + "." + Элемент.Таблица);
					Иначе
						НайденныеВызовы.Добавить(СтрокаПоиска);
					КонецЕсли;
					Прервать;
				ИначеЕсли ЗначениеСимвола = "/" Тогда
					НайденныеВызовы.Добавить(СтрокаПоиска);
					Прервать;
				ИначеЕсли ЗначениеЗаполнено(СокрЛП(ЗначениеСимвола)) Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			НомерВхождения = НомерВхождения + 1;
			ПозицияВызова  = СтрНайти(Алгоритм, ВРег(СтрокаПоиска), , , НомерВхождения);
		КонецЦикла;
	КонецЦикла;
	
	Возврат НайденныеВызовы;
	
КонецФункции

Функция НедопустимыеМетоды()
	Массив = Новый Массив;
	Массив.Добавить("Выполнить"); // @Non-NLS
	Массив.Добавить("Вычислить"); // @Non-NLS
	Массив.Добавить("Подключить"); // @Non-NLS
	Массив.Добавить("ЗафиксироватьТранзакцию"); // @Non-NLS
	Массив.Добавить("ОтменитьТранзакцию"); // @Non-NLS
	Массив.Добавить("НачатьТранзакцию"); // @Non-NLS
	Массив.Добавить("СократитьЖурналРегистрации"); // @Non-NLS
	Массив.Добавить("УстановитьИспользованиеСобытияЖурналаРегистрации"); // @Non-NLS
	Массив.Добавить("ЗапуститьПриложение"); // @Non-NLS
	Массив.Добавить("УстановитьМаксимальныйСрокДействияПаролейПользователей"); // @Non-NLS
	Массив.Добавить("УстановитьМинимальнуюДлинуПаролейПользователей"); // @Non-NLS
	Массив.Добавить("УстановитьМинимальныйСрокДействияПаролейПользователей"); // @Non-NLS
	Массив.Добавить("УстановитьОграничениеПовторенияПаролейПользователейСредиПоследних"); // @Non-NLS
	Массив.Добавить("УстановитьПроверкуСложностиПаролейПользователей"); // @Non-NLS
	Массив.Добавить("УстановитьНастройкиВосстановленияПароля"); // @Non-NLS
	Массив.Добавить("Base64Значение"); // @Non-NLS
	Массив.Добавить("ЗначениеИзСтрокиВнутр"); // @Non-NLS
	Массив.Добавить("XMLЗначение"); // @Non-NLS
	
	Массив.Добавить("Execute");
	Массив.Добавить("Eval");
	Массив.Добавить("Connect");
	Массив.Добавить("CommitTransaction");
	Массив.Добавить("RollbackTransaction");
	Массив.Добавить("BeginTransaction");
	Массив.Добавить("TruncateEventLog");
	Массив.Добавить("SetEventLogEventUse");
	Массив.Добавить("RunApp");
	Массив.Добавить("SetUserPasswordMaxEffectivePeriod");
	Массив.Добавить("SetUserPasswordMinLength");
	Массив.Добавить("SetUserPasswordMinEffectivePeriod");
	Массив.Добавить("SetUserPasswordReuseLimit");
	Массив.Добавить("SetUserPasswordStrengthCheck");
	Массив.Добавить("SetPasswordRecoverySettings");
	Массив.Добавить("Base64Value");
	Массив.Добавить("ValueFromStringInternal");
	Массив.Добавить("XMLValue");
	
	// Процедуры и функции БСП
	Массив.Добавить("ВыполнитьМетодОбъекта");
	Массив.Добавить("ВыполнитьМетодКонфигурации");
	Массив.Добавить("ВыполнитьФоновоеЗадание");
	Массив.Добавить("ОбщийМодуль");
	Массив.Добавить("ЗначениеИзСтрокиXML");
	
	Возврат Массив;
КонецФункции

Функция ЗапрещенныеСвойстваГлобальногоКонтекста()
	
	Массив = Новый Массив;
	Массив.Добавить("ПользователиИнформационнойБазы"); // @Non-NLS
	Массив.Добавить("ДополнительныеНастройкиАутентификации"); // @Non-NLS
	Массив.Добавить("ПараметрыСеанса"); // @Non-NLS
	Массив.Добавить("ФоновыеЗадания"); // @Non-NLS
	Массив.Добавить("РасширенияКонфигурации"); // @Non-NLS
	Массив.Добавить("InfoBaseUsers");
	Массив.Добавить("AdditionalAuthenticationSettings");
	Массив.Добавить("SessionParameters");
	Массив.Добавить("BackgroundJobs");
	Массив.Добавить("ConfigurationExtensions");
	
	Возврат Массив;
	
КонецФункции

Функция ЗапрещенныеМодули()
	
	Массив = Новый Массив;
	Массив.Добавить("ДлительныеОперации");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЕстьСсылкиНаОбъект", Истина);
