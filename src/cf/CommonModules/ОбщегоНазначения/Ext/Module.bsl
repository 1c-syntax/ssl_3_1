...
//
// Параметры:
//  ОписаниеОбъекта - ОбъектМетаданных, Тип - объект, для которого необходимо определить принадлежность к заданному типу.
// 
// Возвращаемое значение:
...
Функция ВидОбъектаПоТипу(ТипОбъекта) Экспорт
	...
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверный тип значения параметра %1 функции %2: %3.'"), "ТипОбъекта",
			"ОбщегоНазначения.ВидОбъектаПоТипу", Строка(ТипОбъекта)), КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
Функция СерверныйМодульМенеджера(Имя)
	...
	Если Не ОбъектНайден Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			КатегорияОшибки.ОшибкаКонфигурации,
			"СтандартныеПодсистемы.БазоваяФункциональность.ЭтоОшибкаПолученияМодуляМенеджераПоИмени");
	КонецЕсли;
	
	// АПК:488-выкл ВычислитьВБезопасномРежиме не используется, чтобы избежать вызова ОбщийМодуль рекурсивно.
	УстановитьБезопасныйРежим(Истина);
	Модуль = Вычислить(Имя); // Не нарушает безопасность информации, так как вычисляется ссылка на модуль менеджера по проверенному имени.
	// АПК:488-вкл
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверное значение параметра %1 в %2. Менеджер для объекта ""%3"" не существует.'"), Имя),
		"Имя", "ОбщегоНазначения.МенеджерОбъектаПоИмени", КатегорияОшибки.ОшибкаКонфигурации);
