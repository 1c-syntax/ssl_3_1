...
// Тип приводится к строке напрямую, кроме простых типов, ссылочных типов и типов ключей регистров.
//
// Параметры:
...
Функция СтроковоеПредставлениеТипа(Тип) Экспорт
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		...
		Если Свойства <> Неопределено Тогда
			...
			ИначеЕсли Свойства.ЕстьСсылка Или Свойства.ЕстьКлючЗаписи Тогда
				ИмяТипа = Свойства.ИмяТипа;
			КонецЕсли;
			
			Возврат ИмяТипа + "." + ЧастиИмени[1];
...
// Выполнить экспортную процедуру или функцию объекта встроенного языка по имени.
// При включении профилей безопасности для вызова оператора Выполнить() используется
// переход в безопасный режим с профилем безопасности, используемым для информационной базы
// (если выше по стеку не был установлен другой безопасный режим).
//
// Параметры:
//  ОбъектМетода - Произвольный - объект встроенного языка 1С:Предприятия, содержащий методы (например, ОбработкаОбъект).
