...
#Область СозданиеЗапросовРазрешений
...
Функция КонтрольныеСуммыФайловКомплектаВнешнейКомпоненты(Знач ИмяМакета) Экспорт
	...
	Для Каждого ОписаниеКомпоненты Из ОписаниеКомплекта.component Цикл
		...
		СвойстваКомпоненты = Новый Структура("os, type");
		ЗаполнитьЗначенияСвойств(СвойстваКомпоненты, ОписаниеКомпоненты);
		ОС = НРег(СвойстваКомпоненты.os);
		
		Если (ОС = "windows" Или ОС = "linux") И (СвойстваКомпоненты.type = "native" Или СвойстваКомпоненты.type = "com") Тогда
...
Функция СсылкаИзРегистраРазрешений(Знач Тип, Знач Идентификатор) Экспорт
	...
	Возврат ?(ПустаяСтрока(Идентификатор), Менеджер.ПустаяСсылка(), Менеджер.ПолучитьСсылку(Идентификатор));
...
#Область НастройкаРазрешенийНаИспользованиеВнешнихРесурсов

Процедура ВыполнитьОбработкуЗапросов(Знач ИдентификаторыЗапросов, Знач АдресВременногоХранилища,
			Знач АдресВременногоХранилищаСостояния, Знач ДобавитьОчисткуЗапросовПередПрименением = Ложь) Экспорт
	
	Обработки.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.ВыполнитьОбработкуЗапросов(
		ИдентификаторыЗапросов,
		АдресВременногоХранилища,
		АдресВременногоХранилищаСостояния,
		ДобавитьОчисткуЗапросовПередПрименением);
	
КонецПроцедуры

Процедура ВыполнитьОбработкуЗапросовОбновления(Знач АдресВременногоХранилища,
			Знач АдресВременногоХранилищаСостояния) Экспорт
	
	Обработки.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.ВыполнитьОбработкуЗапросовОбновления(
		АдресВременногоХранилища, АдресВременногоХранилищаСостояния);
	
КонецПроцедуры

Процедура ВыполнитьОбработкуЗапросовОтключения(Знач АдресВременногоХранилища,
			Знач АдресВременногоХранилищаСостояния) Экспорт
	
	Обработки.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.ВыполнитьОбработкуЗапросовОтключения(
		АдресВременногоХранилища, АдресВременногоХранилищаСостояния);
	
КонецПроцедуры

Процедура ВыполнитьОбработкуЗапросовВосстановления(Знач АдресВременногоХранилища,
			Знач АдресВременногоХранилищаСостояния) Экспорт
	
	Обработки.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.ВыполнитьОбработкуЗапросовВосстановления(
		АдресВременногоХранилища, АдресВременногоХранилищаСостояния);
