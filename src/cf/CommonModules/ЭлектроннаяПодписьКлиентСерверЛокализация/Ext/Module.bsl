...
Функция РезультатПроверкиУдостоверяющегоЦентраСертификата(Сертификат, КонтекстПроверки) Экспорт
	...
	Если Не РезультатПроверкиУдостоверяющегоЦентра.Действует Тогда
		...
		Если СтрНачинаетсяС(КонтекстПроверки.ЗначенияПоиска, "1024001434049")
			И ЗначениеЗаполнено(ДатаПрекращенияАккредитацииУЦ)
			И ДатаПрекращенияАккредитацииУЦ - КонтекстПроверки.РазницаСУниверсальнымВременем = Дата(2024, 7, 20, 21, 0, 0) Тогда
