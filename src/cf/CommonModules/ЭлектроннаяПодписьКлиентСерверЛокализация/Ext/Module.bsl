...
// При определении доступности приложения.
// 
// Параметры:
//  Параметры - ВыборкаИзРезультатаЗапроса, Структура
//  Структура - Структура
//
Процедура ПриОпределенииДоступностиПриложения(Параметры, Структура) Экспорт
	
	// Локализация
	ТипРазмещения = Параметры.ТипРазмещения + ?(Параметры.ПометкаУдаления, 4, 0);
	Если Параметры.ИмяПрограммы = "Госключ" И Не Параметры.ПометкаУдаления
		Или ЭлектроннаяПодписьСлужебныйКлиентСервер.РазмещениеСертификата(Параметры.ТипРазмещения) = "ОблачнаяПодпись"
		И Не Параметры.ПометкаУдаления Или ТипРазмещения = 5 Тогда
		Структура.Вставить("РезультатПроверки", НСтр("ru = 'Доступен.'"));
		Структура.Вставить("Установлена", Истина);
	КонецЕсли;
	// Конец Локализация
