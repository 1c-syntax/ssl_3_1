...
Процедура ПриОпределенииДоступностиПриложения(Параметры, Структура) Экспорт
	...
	// Локализация
	Если Параметры.ИмяПрограммы = "Госключ" 
		Или ЭлектроннаяПодписьСлужебныйКлиентСервер.РазмещениеСертификата(Параметры.ТипРазмещения) = "ОблачнаяПодпись" Тогда
		Если Параметры.ПометкаУдаления Тогда
			Структура.Вставить("РезультатПроверки", "");
			Структура.Вставить("Установлена", "");
		Иначе
