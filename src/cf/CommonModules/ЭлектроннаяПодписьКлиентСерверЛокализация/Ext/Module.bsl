...
Функция ДанныеУдостоверяющегоЦентра(ПоляПоиска, АккредитованныеУдостоверяющиеЦентры) Экспорт
	...
	Для Каждого ПолеПоиска Из МассивПолейПоиска Цикл
		...
		ПодготовленноеПолеПоиска = ПодготовитьПолеПоиска(ПолеПоиска);
		
		Если НайденПериодДействия = Неопределено Тогда
			НайденПериодДействия = АккредитованныеУдостоверяющиеЦентры.ПериодыДействия.Получить(ПодготовленноеПолеПоиска);
		КонецЕсли;
		
		Если НайденаДатаОкончанияДействия = Неопределено Тогда
			НайденаДатаОкончанияДействия = АккредитованныеУдостоверяющиеЦентры.ДатыОкончанияДействия.Получить(ПодготовленноеПолеПоиска);
		КонецЕсли;
		
		Если Не Государственный И АккредитованныеУдостоверяющиеЦентры.ГосударственныеУЦ.Получить(ПодготовленноеПолеПоиска) <> Неопределено Тогда
	...
	Результат.Вставить("ДатаПроверкиОбновления", АккредитованныеУдостоверяющиеЦентры.ДатаПроверкиОбновления);
	
	Возврат Результат;
	
КонецФункции
	
Функция ПодготовитьПолеПоиска(Знач ПолеПоиска) Экспорт
	
	ПолеПоиска = ВРег(ПолеПоиска);
	ПолеПоиска = СтрЗаменить(ПолеПоиска, """", "");
	ПолеПоиска = СтрЗаменить(ПолеПоиска, "«", "");
	ПолеПоиска = СтрЗаменить(ПолеПоиска, "»", "");
	ПолеПоиска = СтрЗаменить(ПолеПоиска, "“", "");
	ПолеПоиска = СтрЗаменить(ПолеПоиска, "”", "");
...
Функция ДанныеДляПроверкиУдостоверяющегоЦентра(Сертификат) Экспорт
	...
	Результат = Новый Структура("ПоляПоиска, НаименованиеУдостоверяющегоЦентра");
	...
	Если СвойстваИздателя.Свойство("ОГРН") И ЗначениеЗаполнено(СвойстваИздателя.ОГРН) Тогда
		ПоляПоиска.Добавить(СвойстваИздателя.ОГРН);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СвойстваИздателя.ОбщееИмя) Тогда
		ПоляПоиска.Добавить(СвойстваИздателя.ОбщееИмя);
	КонецЕсли;
	
	Если ПоляПоиска.Количество() > 0 Тогда
		Результат.ПоляПоиска = СтрСоединить(ПоляПоиска, ",");
	КонецЕсли;
...
Функция РезультатПроверкиУдостоверяющегоЦентраСертификата(Сертификат, КонтекстПроверки) Экспорт
	...
	ДатаПрекращенияДействия = ДатаПрекращенияДействияСертификатовВыданныхКоммерческимиУЦ();
	...
	Если Не РезультатПроверкиУдостоверяющегоЦентра.Действует Тогда
		...
		РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сертификат не действует с %1, так как аккредитация удостоверяющего центра %2, выдавшего сертификат, прекращена.'"),
				Формат(ДатаПрекращенияАккредитацииУЦ, "ДЛФ=D"), КонтекстПроверки.НаименованиеУдостоверяющегоЦентра);
		...
		Если Не КонтекстПроверки.ЭтоПроверкаПодписи Тогда
			...
			Иначе
				...
				КонецЕсли;
		Иначе
			РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ТекстОшибки = 
				РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ТекстОшибки +" "+ РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.Причина;
			РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.Причина = Неопределено;
		КонецЕсли;
	...
	Если Не ДанныеУдостоверяющегоЦентра.Государственный И Не ЭтоФизическоеЛицо И (НаДату >= ДатаПрекращенияДействия 
		...
		Если НаДату < ДатаПрекращенияДействия Тогда
			ТекстОшибки = НСтр("ru = 'После %1 нельзя будет подписывать документы этим сертификатом, так как не будут действовать сертификаты, выданные коммерческими аккредитованными удостоверяющими центрами %2.'");
			ДатаПредупреждения = ДатаПрекращенияДействия;
		Иначе
			ТекстОшибки = НСтр("ru = 'Сертификат не действует c %1, так как с этой даты не действуют сертификаты, выданные коммерческими аккредитованными удостоверяющими центрами %2.'");
		КонецЕсли;
		
		РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки,
			Формат(ДатаПрекращенияДействия, "ДЛФ=D"),
			?(ЭтоИндивидуальныйПредприниматель, НСтр("ru = 'индивидуальным предпринимателям'"), НСтр("ru = 'юридическим лицам и их сотрудникам'")));
	...
	Если ЗначениеЗаполнено(ДанныеУдостоверяющегоЦентра.ДатаОкончанияДействия)
		И (ДатаПредупреждения = Неопределено Или ДанныеУдостоверяющегоЦентра.ДатаОкончанияДействия < ДатаПредупреждения) Тогда
			
		ДатаПредупреждения = ДанныеУдостоверяющегоЦентра.ДатаОкончанияДействия;
		...
		РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Формат(ДанныеУдостоверяющегоЦентра.ДатаОкончанияДействия, "ДЛФ=D"), КонтекстПроверки.НаименованиеУдостоверяющегоЦентра);
