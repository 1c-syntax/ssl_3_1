...
Процедура ОтключитьОбновлениеКлючейДоступа(Отключить, ПланироватьОбновление = Истина) Экспорт
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()
	 Или Не УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не Отключить И ПланироватьОбновление Тогда
		...
		Если ИзмененныеСписки.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из ИзмененныеСписки Цикл
				Списки.Добавить(Метаданные.НайтиПоТипу(КлючИЗначение.Ключ).ПолноеИмя());
			КонецЦикла;
