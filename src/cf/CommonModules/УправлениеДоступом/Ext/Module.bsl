...
#Область ПроцедурыИФункцииИспользуемыеДляПроверкиПрав
...
Функция ПраваДоступаКДанным(ОписаниеДанных, ДляВнешнихПользователей = Ложь, СоставПользователей = Неопределено) Экспорт
	...
	Возврат УправлениеДоступомСлужебный.ПраваДоступаКДанным(ОписаниеДанных,
...
#Область ПроцедурыДляВключенияИИсключенияПользователяВПрофилеГруппДоступа
...
#Область ПроцедурыИФункцииДляПолученияОбщихНастроекПодсистемы
...
#Область ПроцедурыИФункцииИспользуемыеДляНастройкиИнтерфейсаУправляемыхФорм
...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Свойства = Новый Структура("Ссылка", Null);
	ЗаполнитьЗначенияСвойств(Свойства, ТекущийОбъект);
	
	Если Свойства.Ссылка <> Null
	   И Не ЗначениеЗаполнено(Свойства.Ссылка)
	   И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ТипЗнч(ТекущийОбъект)) Тогда
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуру %1 следует вызывать только из обработчика формы %2,
			           |то есть для существующего объекта с непустой ссылкой.'"),
			"УправлениеДоступом.ПриЧтенииНаСервере",
			"ПриЧтенииНаСервере");
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
//
// Параметры:
//  Идентификаторы - Массив из СправочникСсылка.ИдентификаторыОбъектовМетаданных
...
#Область ПроцедурыИФункцииДляРаботыСНаборамиЗначенийДоступа
...
#Область ПроцедурыИФункцииИспользуемыеВПереопределяемомМодуле
...
Функция НовоеОписаниеПрофиляГруппДоступа() Экспорт
	...
	НовоеОписание.Вставить("ЭтоПапка",        Ложь);
...
Функция НовоеОписаниеПапкиПрофилейГруппДоступа() Экспорт
	...
	НовоеОписание.Вставить("Идентификатор", "");
	НовоеОписание.Вставить("Наименование",  "");
	НовоеОписание.Вставить("ЭтоПапка",      Истина);
...
Процедура ЗаменитьРолиВПрофилях(ЗаменяемыеРоли) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"УправлениеДоступом.ЗаменитьРолиВПрофилях");
...
Функция ПрофильАдминистратор() Экспорт
	...
	Возврат УправлениеДоступомСлужебныйПовтИсп.ПрофильАдминистратор();
...
Функция ГруппаДоступаАдминистраторы() Экспорт
	...
	Возврат УправлениеДоступомСлужебныйПовтИсп.ГруппаДоступаАдминистраторы();
...
Процедура ЗаменитьПраваВНастройкахПравОбъектов(ТаблицаПереименований) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"УправлениеДоступом.ЗаменитьПраваВНастройкахПравОбъектов");
...
#Область ПроцедурыИФункцииИспользуемыеДляОбновленияСлужебныхДанных
...
Процедура ОтключитьОбновлениеКлючейДоступа(Отключить, ПланироватьОбновление = Истина) Экспорт
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()
	 Или УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
		Возврат;
	КонецЕсли;
	
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"УправлениеДоступом.ОтключитьОбновлениеКлючейДоступа");
...
Процедура ДобавитьОбработчикОбновленияДляВключенияУниверсальногоОграничения(Версия, Обработчики,
			УстарелТолькоНачальноеЗаполнение = Ложь, УстарелБезУчетаРИБ = Ложь) Экспорт
	...
	Если УстарелТолькоНачальноеЗаполнение Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗапланироватьОбновлениеДоступа(Таблица) Экспорт
	...
	УправлениеДоступомСлужебный.ЗапланироватьОбновлениеДоступа(Таблица);
	
КонецПроцедуры

// Запустить обновление доступа, если требуется. Только вне транзакции.
// Обновление запускается регламентным задания ОбновлениеДоступаНаУровнеЗаписей,
// которое программно включается при добавлении задания в регистр сведений
// ОбновлениеКлючейДоступаКДанным или ОбновлениеКлючейДоступаПользователей.
// Однако, если меняются данные в таблице, которая используются в ограничении доступа
// другой таблицы, тогда записываются точечные задания, которые может потребоваться
// начать отрабатывать более оперативно, не дожидаясь запуска регламентного задания.
// При интерактивном изменении оперативность достигается вызовом этой процедуры
// из обработчика события формы ПослеЗаписиНаСервере, а при программном изменении,
// кроме длительных операций, такой вызов нужно делать явно, если требуется.
//
...
// ТехнологияСервиса.ПользователиСервиса

// Устанавливает права пользователя в соответствии с переданными группами доступа 
// (профилями групп доступа в случае упрощенного режима) и группами пользователей
// 
// Параметры:
//  Пользователь - СправочникСсылка.Пользователи
//  ГруппыДоступа - Массив из СправочникСсылка.ГруппыДоступа, СправочникСсылка.ПрофилиГруппДоступа
//  ГруппыПользователей - Массив из СправочникСсылка.ГруппыПользователей
//
Процедура УстановитьПраваПользователя(Пользователь, ГруппыДоступа, ГруппыПользователей) Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.УправлениеДоступомВМоделиСервиса") Тогда
		Возврат;
	КонецЕсли;
	...
	МодульУправлениеДоступомСлужебныйВМоделиСервиса.УстановитьПраваПользователя(
		Пользователь, 
		ГруппыДоступа, 
		ГруппыПользователей);
...
// Обновляет профиль групп доступа по шаблону из Менеджера сервиса
// 
// Параметры:
//  ИмяШаблона - Строка
//  Комментарий - Строка
//  ИдентификаторШаблона - УникальныйИдентификатор
//  РолиШаблона - Массив из Строка - полные имена объектов метаданных ролей 
...
Процедура ОбновитьПрофильГруппДоступаПоШаблону(ИмяШаблона, Комментарий, ИдентификаторШаблона, РолиШаблона, ЭтоОтключение) Экспорт
	...
	МодульУправлениеДоступомСлужебныйВМоделиСервиса.ОбновитьПрофильГруппДоступаПоШаблону(
		ИмяШаблона, 
		Комментарий, 
...
// Отправляет группы доступа в Менеджер сервиса при переходе на новую версию.
Процедура ОтправитьГруппыДоступаВМенеджерСервисаПриПереходеНаНовуюВерсию() Экспорт
	...
	МодульУправлениеДоступомСлужебныйВМоделиСервиса = 
		ОбщегоНазначения.ОбщийМодуль("УправлениеДоступомСлужебныйВМоделиСервиса");
	МодульУправлениеДоступомСлужебныйВМоделиСервиса.ОтправитьГруппыДоступаВМенеджерСервисаПриПереходеНаНовуюВерсию();
КонецПроцедуры

// Конец ТехнологияСервиса.ПользователиСервиса

// Требуется в файловой ИБ для предотвращения взаимоблокировок связанных с работой файловой СУБД,
// в случаях, когда выполняется чтение с ограничением (RLS) в транзакции или перед записью
// до срабатывания подписки на событие ПроверитьДоступПередЗаписью.
// Рекомендуется устанавливать при фактическом начале транзакции, а не во вложенных транзакциях.
//
// Существуют следующие особенности работы файловой СУБД.
// 1. При чтении в транзакции файловая СУБД автоматически ставит разделяемые блокировки на все таблицы,
//    используемые в запросе, в том числе таблицы от ограничения доступа (RLS), добавляемые на уровне SDBL.
// 2. При установке разделяемой блокировки в сеансе 1 нельзя поднять ее до исключительной,
//    если в другом сеансе выполняется ожидание установки исключительной блокировки.
//
// Параметры:
//  ПередЗаписьюНовогоОбъекта - Булево - указать Истина, если где-то в общей транзакции
//    записывается новый ссылочный объект с ограничением доступа (RLS).
//  ПередЗаписьюНастройкиДоступа - Булево - указать Истина, если где-то в общей транзакции
//    меняется объект так, что это влияет на права доступа
//    а) добавляется новое значение доступа (например, организация, контрагент, ...);
//    б) меняется группа значений доступа у значения доступа (например, группа доступа контрагентов у контрагента);
//    в) меняется другая настройкой доступа (например, новая папка файлов или меняется родитель папки файлов);
//    г) меняется константа, которая включает/отключает вид доступа (например, ИспользоватьВнешнихПользователей).
//
Процедура УстановитьБлокировкуПередЗаписьюВФайловойИБ(ПередЗаписьюНовогоОбъекта = Ложь,
			ПередЗаписьюНастройкиДоступа = Ложь) Экспорт
	
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Возврат;
	КонецЕсли;
	
	УправлениеДоступомСлужебный.УстановитьБлокировкуПередЗаписьюВФайловойИБ(
		ПередЗаписьюНовогоОбъекта, ПередЗаписьюНастройкиДоступа);
...
Процедура ВключитьОтключитьПрофильПользователя(Пользователь, Профиль, Включить, Источник = Неопределено) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"УправлениеДоступом.ВключитьОтключитьПрофильПользователя");
