...
// АПК:142-выкл - 5 необязательных параметров для обратной совместимости.
...
//
//  ДополнительныеПараметры - Структура - см. ПараметрыПриСозданииФормыЗначенияДоступа.
//
...
Процедура НастроитьОтборыДинамическогоСписка(Список, ОписаниеОтборов) Экспорт
	...
	Если Не ОграничиватьДоступНаУровнеЗаписей()
	 Или УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально(, Истина)
	 Или Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаменитьПраваВНастройкахПравОбъектов(ТаблицаПереименований) Экспорт
	...
	// АПК:96-выкл: допустимо, так как строки не должны повторятся и это разовая операция в обработчике обновления.
	Запрос = Новый Запрос;
	...
	|ИЗ
	|	НастройкиПрав КАК НастройкиПрав";
	// АПК:96-вкл: допустимо, так как строки не должны повторятся и это разовая операция в обработчике обновления.
	...
	Если Не Отключить И ПланироватьОбновление Тогда
		...
		Если ИзмененныеСписки.Количество() > 0 Тогда
			Если УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
				...
				КонецЦикла;
				УправлениеДоступомСлужебный.ДобавитьЗависимыеСписки(Списки, ДобавленныеСписки);
...
//      * Ошибки      - Массив - с элементами описания отдельных ошибок:
//          ** Значение - Структура - со свойствами:
...
//
//   * ДополнительныеТаблицы - Массив - с элементами:
//       ** Значение - Структура - со свойствами:
