...
Функция ОтправитьПисьмо(УчетнаяЗаписьИлиСоединение, Письмо) Экспорт
	...
	Возврат РаботаСПочтовымиСообщениямиСлужебный.ОтправитьПисьмо(УчетнаяЗаписьИлиСоединение, Письмо);
...
Функция ОтправитьПисьма(УчетнаяЗаписьИлиСоединение, Письма, ТекстОшибки = Неопределено) Экспорт
	...
	Возврат РаботаСПочтовымиСообщениямиСлужебный.ОтправитьПисьма(УчетнаяЗаписьИлиСоединение, Письма, ТекстОшибки);
...
Функция ЗагрузитьПочтовыеСообщения(Знач УчетнаяЗаписьИлиСоединение, Знач ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Результат = РаботаСПочтовымиСообщениямиСлужебный.ЗагрузитьСообщения(УчетнаяЗаписьИлиСоединение, ПараметрыЗагрузки);
	Возврат Результат;
...
Функция УчетнаяЗаписьНастроена(УчетнаяЗапись, Знач ДляОтправки = Неопределено, Знач ДляПолучения = Неопределено) Экспорт
	...
	Параметры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись,
	...
	Если ПустаяСтрока(Параметры.АдресЭлектроннойПочты) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДляОтправки И Не Параметры.ИспользоватьДляОтправки Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДляПолучения И Не Параметры.ИспользоватьДляПолучения Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДляПолучения И ПустаяСтрока(Параметры.СерверВходящейПочты) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДляОтправки И ПустаяСтрока(Параметры.СерверИсходящейПочты) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДляОтправки И Параметры.ПротоколВходящейПочты = "IMAP" И ПустаяСтрока(Параметры.СерверВходящейПочты) Тогда
		Возврат Ложь;
	КонецЕсли;
...
//   УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - учетная запись электронной почты, у которой
//                                                                    возникла ошибка.
//
// Возвращаемое значение:
//  Структура:
...
//
Функция ПоясненияПоОшибке(ТекстОшибки, УчетнаяЗапись = Неопределено) Экспорт
	...
	ПараметрыПояснения.ТекстОшибки = ТекстОшибки;
	ПараметрыПояснения.Контекст = РаботаСПочтовымиСообщениямиСлужебный.КонтекстДляПояснения().ОтправкаПисьма;
...
//  УчетнаяЗапись                        - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - учетная запись электронной
//      почты, у которой возникла ошибка.
//
// Возвращаемое значение:
//  Строка
//
Функция РасширенноеПредставлениеОшибки(ИнформацияОбОшибке, КодЯзыка,
	...
	Возврат РаботаСПочтовымиСообщениямиСлужебный.РасширенноеПредставлениеОшибки(
		ИнформацияОбОшибке, КодЯзыка, ВключитьПодробноеПредставлениеОшибки, УчетнаяЗапись);
...
Функция ПодключениеКПочте(Знач УчетнаяЗапись, Знач ДляПолучения = Ложь) Экспорт
	...
	Возврат РаботаСПочтовымиСообщениямиСлужебный.ПодключениеКПочте(УчетнаяЗапись, ДляПолучения);
