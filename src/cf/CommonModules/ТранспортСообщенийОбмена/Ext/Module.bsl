...
//   но только данные аутентификации, например: Пароль, ИмяПользователя
//
...
//   * ИмяПланаОбмена - Строка 
//   * ИмяПланаОбменаКорреспондента - Строка
//   * ИдентификаторТранспорта - Строка - доступный идентификатор транспорта.
//   Доступные идентификаторы см. ДоступныеТипыТранспорта
//   * НастройкиТранспорта - Структура - состав повторяет реквизиты обработки транспорта,
//   * ДанныеАутентификации - Структура - состав повторяет реквизиты обработки транспорта, 
//   но только данные аутентификации, например: Пароль, ИмяПользователя
//  СоздатьКаталогВременныхФайлов - Булево - если Истина, то создается каталог временных файлов
// 
// Возвращаемое значение:
...
//    При инициализации создается ВременныйКаталог, задается временное имя файла СообщениеОбмена, 
//    а так же шаблоны имен файлов (ШаблоныИменДляПолученияСообщения) для получения
//  
Функция Инициализация(ПараметрыИнициализации, СоздатьКаталогВременныхФайлов = Истина) Экспорт
	...
	Если СоздатьКаталогВременныхФайлов Тогда
...
// Уничтожение обработки транспорта, удаление временного каталога обмена 
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Структура - Настройки транспорта (реквизиты обработки транспорта)
//
...
// 
// Возвращаемое значение:
//  Структура - Настройки транспорта по умолчанию (реквизиты обработки транспорта)
//
...
// Структура проверки полученных параметров транспорта.
// 
// Возвращаемое значение:
//  Структура - Структура проверки полученных параметров транспорта:
//   * ПодключениеУстановлено - Булево - проверка подключения выполнена успешно.
//   * ПодключениеРазрешено - Булево - есть разрешение использовать перечисленный вид транспорта.
//   * ВерсииИнтерфейса - Неопределено, 
//                        Строка - для версионируемых видов транспорта, например, ВЕБ- или HTTP- сервисы.
//   * СообщениеОбОшибке - Строка - текст ошибки подключения.
//   * ПараметрыКорреспондентаПолучены - Булево - получен ответ с параметрами корреспондента.
//   * ПараметрыКорреспондента - Неопределено,
//                               Строка - строка XML с параметрами корреспондента.
//   * ИмяПланаОбменаКорреспондента - Строка - имя плана обмена в корреспонденте.
//   * ЭтотУзелЕстьВКорреспонденте - Булево - признак того, что в корреспонденте нет настройки с этой ИБ. 
//   * УзелКорреспондентаЕстьВЭтойБазе - Булево - признак того, что в этой ИБ нет настройки с корреспондентом.
//   * УзелДляУдаления - Неопределено,
//                       ПланОбменаСсылка - узел обмена, который планируется удалить.
//
...
// 
// Возвращаемое значение:
//  Строка - идентификатор транспорта
//  
...
#Область МенеджерыТиповыхОбработокТранспорта
...
Процедура ПроверитьИЗаполнитьНастройкиПодключенияXML(
	...
	// При пассивном режиме, в значении свойства "ПоддерживаемыеОбъектыФормата" настроек подключения
	//  содержатся поддерживаемые объекты формата корреспондента
	//  по этой причине, необходимо присвоить значение свойству "ПоддерживаемыеОбъектыФорматаКорреспондента".
	
	Если НастройкиПодключения.ИдентификаторТранспорта = "ПассивныйРежим"
		И НастройкиПодключения.ПоддерживаемыеОбъектыФормата <> Неопределено
		И НастройкиПодключения.ПоддерживаемыеОбъектыФорматаКорреспондента = Неопределено Тогда
		
		НастройкиПодключения.ПоддерживаемыеОбъектыФорматаКорреспондента = НастройкиПодключения.ПоддерживаемыеОбъектыФормата;
		
		НастройкиПодключения.ПоддерживаемыеОбъектыФормата = Неопределено;
...
// Проверка того, что все обязательные реквизиты настройки транспорта заполнены.
//
// Параметры:
//  Настройка - РезультатЗапроса, Структура - значения реквизитов настройки транспорта.
//  ИдентификаторНастройки - Строка - идентификатор транспорта.
//
// Возвращаемое значение:
//   Булево - Истина, если все обязательные реквизиты настройки заполнены.
//
Функция ЗаполненыОбязательныеРеквизитыНастройкиТранспорта(Знач Настройка, Знач ИдентификаторНастройки) Экспорт
	
	НастройкаТранспорта = НастройкаТранспортаВСтруктуру(Настройка);

	МенеджерТранспорта = МенеджерТранспортаПоИдентификатору(ИдентификаторНастройки);
	
	НастройкаЗаполнена = ОбязательныеРеквизитыНастройкиЗаполнены(НастройкаТранспорта, МенеджерТранспорта);
	
	ТранспортСообщенийОбменаПереопределяемый.ПриПроверкиЗаполненияОбязательныхРеквизитовНастройкиТранспорта(НастройкаТранспорта, МенеджерТранспорта, НастройкаЗаполнена);
...
Функция ТребуетсяАутентификация(ПараметрыАутентификации, ИмяФормы = "") Экспорт
	...
	Транспорт = Инициализация(ПараметрыАутентификации, Ложь);
...
Функция ТаблицаИзМассива_ПоддерживаемыеОбъектыФормата(Массив, ВерсииФорматаОбмена) Экспорт
	...
	ОбменДаннымиXDTOСервер.ИнициализироватьТаблицуПоддерживаемыхОбъектовФормата(Результат, "ОтправкаПолучение");
...
//
// Параметры:
//  Настройка - Структура - значения реквизитов настройки транспорта.
...
//   Булево - Истина, если все обязательные реквизиты настройки заполнены.
//
Функция ОбязательныеРеквизитыНастройкиЗаполнены(Настройка, МенеджерТранспорта)
	...
	Если МенеджерТранспорта = МенеджерТранспортаCOM() Тогда
		НастройкаЗаполнена = ОбязательныеРеквизитыНастройкиТранспортаCOMЗаполнены(Настройка);
	Иначе
		ОбязательныеРеквизиты = ОбязательныеДляЗаполненияРеквизитыТранспортов();
		
		ОбязательныеРеквизитыТранспорта = ОбязательныеРеквизиты.Получить(МенеджерТранспорта);
		
		Если ОбязательныеРеквизитыТранспорта <> Неопределено Тогда
			Для Каждого РеквизитНастройки Из ОбязательныеРеквизитыТранспорта Цикл
				Если Настройка.Свойство(РеквизитНастройки) Тогда
					Если Не ЗначениеЗаполнено(Настройка[РеквизитНастройки]) Тогда
...
// Проверка того, что все обязательные реквизиты настройки транспорта COM заполнены.
//
// Параметры:
//  Настройка - Структура - значения реквизитов настройки транспорта.
//
...
//   Булево - Истина, если все обязательные реквизиты настройки заполнены.
//
Функция ОбязательныеРеквизитыНастройкиТранспортаCOMЗаполнены(Настройка)
	
	НастройкаЗаполнена = Ложь;
	
	Если Настройка.ВариантРаботыИнформационнойБазы Тогда
		Если ЗначениеЗаполнено(Настройка.ИмяСервера1СПредприятия) И ЗначениеЗаполнено(Настройка.ИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			НастройкаЗаполнена = Истина;
		КонецЕсли;
	Иначе
		НастройкаЗаполнена = ЗначениеЗаполнено(Настройка.КаталогИнформационнойБазы);
	КонецЕсли;
...
// Возвращает соответствие, которое содержит в себе обязательные реквизиты настройки для заполнения
//
// Возвращаемое значение:
//  Соответствие - 
//   * Ключ - Произвольный - менеджер транспорта см. в функции ТранспортСообщенийОбмена.МенеджерыТранспортаПоИдентификаторам;
//   * Значение - Массив - содержит имена реквизитов обязательных для заполнения;
//
Функция ОбязательныеДляЗаполненияРеквизитыТранспортов()
	
	ОбязательныеДляЗаполненияРеквизиты = Новый Соответствие;
	
	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("УчетнаяЗапись");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаEMAIL(), РеквизитыНастройки);
	
	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("КаналОтправки");
	РеквизитыНастройки.Добавить("КаналПолучения");
	РеквизитыНастройки.Добавить("КодПолучателя");
	РеквизитыНастройки.Добавить("СервисИнтеграции");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаESB1C(), РеквизитыНастройки);
	
	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("КаталогОбменаИнформацией");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаFILE(), РеквизитыНастройки);

	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("Путь"); 
	РеквизитыНастройки.Добавить("Пользователь");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаFTP(), РеквизитыНастройки); 
	
	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("ClientID");
	РеквизитыНастройки.Добавить("ClientSecret");
	РеквизитыНастройки.Добавить("AccessToken");
	РеквизитыНастройки.Добавить("RefreshToken");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаЯндексДиск(), РеквизитыНастройки);
	...
	РеквизитыНастройки.Добавить("AccessToken");
	РеквизитыНастройки.Добавить("RefreshToken");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаGoogleDrive(), РеквизитыНастройки);
	
	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("АдресВебСервиса");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаWS(), РеквизитыНастройки);
	
	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("АдресВебСервиса");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаHTTP(), РеквизитыНастройки);

	РеквизитыНастройки = Новый Массив;
	РеквизитыНастройки.Добавить("НаименованиеКорреспондента");
	РеквизитыНастройки.Добавить("КонечнаяТочкаКорреспондента");
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаSM(), РеквизитыНастройки);
	
	РеквизитыНастройки = Новый Массив;
	ОбязательныеДляЗаполненияРеквизиты.Вставить(МенеджерТранспортаПассивныйРежим(), РеквизитыНастройки);
	
	Возврат ОбязательныеДляЗаполненияРеквизиты;
	
КонецФункции

// Преобразует настройку в структуру.
// 
// Параметры:
//  Настройка - РезультатЗапроса, Структура - значения реквизитов настройки транспорта. 
//
// Возвращаемое значение:
//  Структура - содержит значения реквизитов настройки.
//
Функция НастройкаТранспортаВСтруктуру(Настройка)
	
	Если ТипЗнч(Настройка) = Тип("РезультатЗапроса") Тогда
		Результат = Новый Структура;
		
		Выборка = Настройка.Выбрать();
		
		Пока Выборка.Следующий() Цикл
			Результат.Вставить(Выборка.Настройка, Выборка.Значение);
		КонецЦикла;
	Иначе
		Результат = Настройка;
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверитьАутентификацию", Истина);
