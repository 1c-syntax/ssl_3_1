...
Функция АдресаЭлектроннойПочтыИзСтроки(Знач СписокАдресов) Экспорт
	...
	Для Каждого Строка Из СтрРазделить(СписокАдресов, ";", Ложь) Цикл
		ЧастьПредставления = "";
		Для Каждого АдресСПредставлением Из СтрРазделить(Строка, ",", Ложь) Цикл
			Если Не ЗначениеЗаполнено(АдресСПредставлением) Тогда
				ЧастьПредставления = ЧастьПредставления + АдресСПредставлением;
				Продолжить;
			КонецЕсли;
			
			ЧастиСтроки = СтрРазделить(СокрП(АдресСПредставлением), " ", Ложь);
			Адрес = ЧастиСтроки[ЧастиСтроки.ВГраница()];
			Псевдоним = "";
			ОписаниеОшибки = "";
			...
			Если СтрНайти(Адрес, "@") Или СтрНайти(Адрес, "<") Или СтрНайти(Адрес, ">") Тогда
				...
				Псевдоним = СокрЛП(ЧастьПредставления + СтрСоединить(ЧастиСтроки, " "));
				ЧастьПредставления = "";
		...
		Если ЗначениеЗаполнено(СтрЗаменить(ЧастьПредставления, ",", "")) Тогда
			Псевдоним = ЧастьПредставления;
			Адрес = "";
			ОписаниеОшибки = НСтр("ru = 'Строка адреса не соответствует формату ""Имя получателя <Адрес@Почты>""'");
			СтруктураАдреса = Новый Структура("Псевдоним, Адрес, ОписаниеОшибки", Псевдоним, Адрес, ОписаниеОшибки);
			Результат.Добавить(СтруктураАдреса);
		КонецЕсли;
	КонецЦикла;
...
Функция РасчетныеПоказателиЯчеек(Знач ТабличныйДокумент, Знач ПолеТабличногоДокумента) Экспорт 
	...
	ПараметрыРасчета = ОбщегоНазначенияСлужебныйКлиентСервер.ПараметрыРасчетаПоказателейЯчеек(ПолеТабличногоДокумента);
