...
Функция ЭтоИсключениеСКодомОшибки(ИнформацияОбОшибке, КодыОшибок) Экспорт
	...
	Коды = СтрРазделить(КодыОшибок, Символы.ПС, Ложь);
	ТекущаяПричина = ИнформацияОбОшибке;
	
	Пока ТекущаяПричина <> Неопределено Цикл
		Если Коды.Найти(ТекущаяПричина.Код) <> Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
		ТекущаяПричина = ТекущаяПричина.Причина;
	КонецЦикла;
...
//   ОжидаемыеТипы - ОписаниеТипов - тип Неопределено исключается из состава указанных.
//                 - Тип
//                 - Массив
//                 - ФиксированныйМассив
//                 - Соответствие
...
Функция ПолучитьПолноеИмяФайла(Знач ИмяКаталога, Знач ИмяФайла) Экспорт
	...
	Если (Прав(ИмяКаталога, 1) <> "\") И (Прав(ИмяКаталога, 1) <> "/") Тогда
		РазделительПути = ?(СтрНайти(ИмяКаталога, "\") = 0, "/", "\");
	КонецЕсли;
	
	Возврат ИмяКаталога + РазделительПути + ИмяФайла;
...
Функция АдресаЭлектроннойПочтыИзСтроки(Знач СписокАдресов) Экспорт
	...
	Для Каждого АдресЭлектроннойПочты Из СтрРазделить(СписокАдресов, ";", Ложь) Цикл
		ЧастиПредставления = Новый Массив;
		Для Каждого АдресСПредставлением Из СтрРазделить(СокрЛП(АдресЭлектроннойПочты), ",", Ложь) Цикл
...
Функция АдресЭлектроннойПочтыСоответствуетТребованиям(Знач Адрес, РазрешитьЛокальныеАдреса = Ложь) Экспорт
	...
	// Проверяем наличие спецсимволов в начале и в конце имени и домена.
	Если ЕстьСимволыВНачалеВКонце(ЛокальноеИмя, ".") Или ЕстьСимволыВНачалеВКонце(Домен, СпецСимволы) Тогда
		Возврат Ложь;
	КонецЕсли;
...
//  Точность            - Число  - точность округления при распределении. Не влияет на погрешность - итог по суммам
//                                 распределения всегда равен распределяемой сумме.
//
// Возвращаемое значение:
//  - Массив Из Число - массив размерностью равный массиву коэффициентов, содержит суммы в соответствии с весом коэффициента 
//                      (из массива коэффициентов). 
//  - Неопределено    - В случае, если распределить невозможно: кол-во коэффициентов = 0, есть коэффициенты с разным знаком
//                      или суммарный вес коэффициентов = 0.
//
// Пример:
//
...
Функция РаспределитьСуммуПропорциональноКоэффициентам(Знач РаспределяемаяСумма, Знач Коэффициенты, Знач Точность = 2) Экспорт
	...
	ЕстьОтрицательныеКоэффициенты = Ложь;
	ЕстьПоложительныеКоэффициенты = Ложь;
	
	Размерность = КоэффициентыАбсолютные.Количество();
	Для Индекс = 0 По Размерность - 1 Цикл
		...
		Если Коэффициент < 0 Тогда 
			...
			ЕстьОтрицательныеКоэффициенты = Истина;
		ИначеЕсли Коэффициент > 0 Тогда 
			ЕстьПоложительныеКоэффициенты = Истина;
		КонецЕсли;
		
		Если ЕстьОтрицательныеКоэффициенты И ЕстьПоложительныеКоэффициенты Тогда
			// Недопустимо значение параметра Коэффициенты
	...
	Если ЗначениеЗаполнено(СуммарнаяПогрешность) Тогда  
		РаспределяетсяЦелоеЧислоСНулевойТочностью = Точность = 0 И РаспределяемаяСумма = ЦЕЛ(РаспределяемаяСумма);
		РаспределятьПропорционально = РаспределяетсяЦелоеЧислоСНулевойТочностью Или СуммарнаяПогрешность < 0;
		Если РаспределятьПропорционально Тогда
			ИндексыВПорядкеВеса = ИндексыВПорядкеВеса(КоэффициентыАбсолютные);
			ЗначениеПогрешности = 1 / Pow(10, Точность) * ?(СуммарнаяПогрешность < 0, -1, 1);
			
			КоличествоЭлементовПогрешности = ЦЕЛ(СуммарнаяПогрешность / ЗначениеПогрешности);
			ИндексВеса = -1;
			Для НомерЭлементаПогрешности = 1 По КоличествоЭлементовПогрешности Цикл
				ИндексВеса = ИндексВеса + 1;
				Если ИндексВеса = Размерность Тогда
					ИндексВеса = 0;
				КонецЕсли;
				Индекс = ИндексыВПорядкеВеса[ИндексВеса];
...
Функция НовоеЗащищенноеСоединение(Знач СертификатКлиента = Неопределено, Знач СертификатыУдостоверяющихЦентров = Неопределено, Знач ВидСоединения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидСоединения) Тогда
		...
		ПроверитьПараметр("ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение", "ВидСоединения", ВидСоединения, Тип("Строка"),, ОжидаемыеЗначения);
	КонецЕсли;
...
#Иначе
	Если СертификатыУдостоверяющихЦентров = Неопределено Тогда
		ВерсииПлатформыДляИспользованияСертификатов = "8.3.24.1446";
	
		СистемнаяИнформация = Новый СистемнаяИнформация;
		ТекущийНомерВерсии = ВерсияКонфигурацииБезНомераСборки(СистемнаяИнформация.ВерсияПриложения);
		ИспользоватьСертификатыУдостоверяющихЦентров = Истина;
	...
	Если ВидСоединения = "КриптоПро" Тогда
		...
		// АПК:547-выкл инструкция препроцессора допустима, так как в выполняемом коде нет обращения к модулям.
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		УстановитьБезопасныйРежим(Истина);
#КонецЕсли
		// АПК:487-выкл Поддержка новых методов платформы (исполняемый код безопасен)
		Выполнить("ЗащищенноеСоединениеКриптоПро = Новый ЗащищенноеСоединениеКриптоПро(СертификатКлиента, СертификатыУдостоверяющихЦентров, РежимПроверкиОтзываTLSСертификатаСервера.Строгий)"); // Не нарушает безопасность информации, так как выполняется фиксированная строка, содержащая только вызов метода платформы.
		// АПК:487-вкл
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
...
// АПК:222-выкл устаревший программный интерфейс может вызывать устаревшие процедуры и функции.
...
#Если Не МобильныйКлиент Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Не удалось подключиться к другому приложению: %1'");
		...
		Исключение
			Информация = ИнформацияОбОшибке();
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Не удалось подключиться к приложению: %1'");
...
#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Запуск приложения недоступен в веб-клиенте.'");
#Иначе
...
// Создает массив индексов исходного массива, отсортированных по убыванию значений исходного массива.
// Т.е. первое значение это индекс элемента с максимальным значением, второе - с максимальным значением из оставшихся и т.д.
// 
// Параметры:
...
//  Массив из Число
//
Функция ИндексыВПорядкеВеса(Знач Массив)
	Результат = Новый Массив();
	
	Сортировка = Новый СписокЗначений;
	Сортировка.ЗагрузитьЗначения(Массив);
	Сортировка.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
	
	Для каждого ЭлемСписка Из Сортировка Цикл
		Индекс = Массив.Найти(ЭлемСписка.Значение);
...
// АПК:222-выкл код сохранен для обратной совместимости, используется в устаревшем программном интерфейсе.
...
Функция ПроверитьДоступностьСервера(АдресСервера)
	...
	Иначе
		Журнал.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удаленный сервер %1 недоступен:'"), 
			АдресСервера));
	КонецЕсли;
