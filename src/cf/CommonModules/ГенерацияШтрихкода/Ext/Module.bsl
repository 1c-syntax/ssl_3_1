...
#КонецЕсли
	...
	Если Метаданные.Подсистемы.Найти("ПоддержкаОборудования") = Неопределено Тогда
		// Вызов БСП
		Возврат Вычислить("ОбщегоНазначения"); // АПК:488 Код выполняется в безопасном режиме.
		// Конец Вызов БСП
	Иначе
		Возврат Вычислить("ОбщегоНазначенияБПО"); // АПК:488 Код выполняется в безопасном режиме.
	КонецЕсли;
