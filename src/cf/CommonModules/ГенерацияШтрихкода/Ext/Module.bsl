...
//      25 - Aztec
//      26 - QR в формате универсального платежного кода.
...
//   * ОтображениеУниверсальногоПлатежногоКода - Число - отображение QR универсального платежного кода (когда ТипКода = 26).
//      Возможные значения: 0 - Основная(градиент), 1 - Монохромная, 2 - Монохромная с цветным логотипом
//   * ЛоготипРазмерПроцентОтШК - Число - процент от генерированного QR для вписывания логотипа.
//   * ФорматВыходногоИзображения - Число - формат выходного изображения
//      Возможные значения: 0 - PNG, 1 - SVG
//
Функция ПараметрыГенерацииШтрихкода() Экспорт
	...
	ПараметрыШтрихкода.Вставить("ИмяШрифта"); 
	...
	ПараметрыШтрихкода.Вставить("ОтображениеУниверсальногоПлатежногоКода", 0);
	ПараметрыШтрихкода.Вставить("НовыйВызовКомпоненты", Истина);
	ПараметрыШтрихкода.Вставить("ФорматВыходногоИзображения", 0);
...
#КонецЕсли
	...
	// Если в системе установлен шрифт по умолчанию.
	// Если шрифт не найден, будет использован шрифт "Default" встроенный в ВК генерации штрихкода.
	Если ВнешняяКомпонента.НайтиШрифт(ШрифтПоУмолчанию()) Тогда
		// Выбираем его как шрифт для формирования картинки.
		ВнешняяКомпонента.Шрифт = ШрифтПоУмолчанию();
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты(
		"ОбщийМакет.КомпонентаПечатиШтрихкодов", НСтр("ru = 'Печать штрихкодов.'")));
...
// Возвращает имя шрифта по умолчанию используемое для генерации надписей в штрихкоде.
//
// Возвращаемое значение: 
//   Строка - Имя шрифта
//
Функция ШрифтПоУмолчанию()
...
Функция ПодготовитьИзображениеШтрихкода(ВнешняяКомпонента, ПараметрыШтрихкода)
	...
	ЗаписьXML.ЗаписатьТекст(Формат(ШиринаШтрихкода, "ЧГ=0"));
	ЗаписьXML.ЗаписатьКонецЭлемента();
	...
	ЗаписьXML.ЗаписатьТекст(Формат(ВысотаШтрихкода, "ЧГ=0"));
	ЗаписьXML.ЗаписатьКонецЭлемента();
	...
	ЗаписьXML.ЗаписатьКонецЭлемента();
	// Имя шрифта 
	Если ПараметрыШтрихкода.Свойство("ИмяШрифта") И НЕ ПустаяСтрока(ПараметрыШтрихкода.ИмяШрифта) Тогда
		ЗаписьXML.ЗаписатьНачалоЭлемента("Font");
		ЗаписьXML.ЗаписатьТекст(XMLСтрока(ПараметрыШтрихкода.ИмяШрифта));
		ЗаписьXML.ЗаписатьКонецЭлемента();
	КонецЕсли;
	...
	Если ПараметрыШтрихкода.Свойство("Инвертирован") Тогда
		...
		ЗаписьXML.ЗаписатьКонецЭлемента();
	КонецЕсли;          
	// Формат выходного изображения, 0 - PNG, 1 - SVG
	Если ПараметрыШтрихкода.Свойство("ФорматВыходногоИзображения") Тогда
		ЗаписьXML.ЗаписатьНачалоЭлемента("OutputImageFormat");
		ЗаписьXML.ЗаписатьТекст(XMLСтрока(ПараметрыШтрихкода.ФорматВыходногоИзображения));
		ЗаписьXML.ЗаписатьКонецЭлемента();
	КонецЕсли;          
	// Тип штрихкода - QR
	Если ПараметрыШтрихкода.ТипКода = 16 Или ПараметрыШтрихкода.ТипКода = 26 Тогда 
	...
	// Тип штрихкода - QR YPC
	Если ПараметрыШтрихкода.ТипКода = 26 Тогда 
		ЗаписьXML.ЗаписатьНачалоЭлемента("FormatQRUPC");
		ЗаписьXML.ЗаписатьТекст(XMLСтрока(Число(ПараметрыШтрихкода.ОтображениеУниверсальногоПлатежногоКода)));
		ЗаписьXML.ЗаписатьКонецЭлемента();
	КонецЕсли;
...
#Если НЕ МобильноеПриложениеСервер Тогда
			МодульОбщегоНазначения = МодульОбщегоНазначения();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Используется старая версия компоненты генерации штрихкода'", 
				МодульОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
#КонецЕсли
	...
	Если ПараметрыШтрихкода.Свойство("ФорматВыходногоИзображения") Тогда      
		Попытка
			ВнешняяКомпонента.ФорматВыходногоИзображения = ПараметрыШтрихкода.ФорматВыходногоИзображения;
		Исключение  
#Если НЕ МобильноеПриложениеСервер Тогда
			МодульОбщегоНазначения = МодульОбщегоНазначения();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Используется старая версия компоненты генерации штрихкода'",
...
#КонецЕсли
	...
	Если ПараметрыШтрихкода.ТипКода = 26 И ПараметрыШтрихкода.Свойство("ОтображениеУниверсальногоПлатежногоКода") Тогда
		Попытка
			ВнешняяКомпонента.ФорматQRУПК = ПараметрыШтрихкода.ОтображениеУниверсальногоПлатежногоКода;
		Исключение  
#Если НЕ МобильноеПриложениеСервер Тогда
			МодульОбщегоНазначения = МодульОбщегоНазначения();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Используется старая версия компоненты генерации штрихкода'",
...
#КонецЕсли
	...
	Если ПараметрыШтрихкода.Свойство("РазмерШрифта") И (ПараметрыШтрихкода.ОтображатьТекст)
		И (ПараметрыШтрихкода.РазмерШрифта > 0) И (ВнешняяКомпонента.РазмерШрифта <> ПараметрыШтрихкода.РазмерШрифта) Тогда
			ВнешняяКомпонента.РазмерШрифта = ПараметрыШтрихкода.РазмерШрифта;
	КонецЕсли;
	
	// Имя шрифта 
	Если ПараметрыШтрихкода.Свойство("ИмяШрифта") И (ПараметрыШтрихкода.ОтображатьТекст) Тогда
		ИмяШрифта = ?(ПустаяСтрока(ПараметрыШтрихкода.ИмяШрифта), ШрифтПоУмолчанию(), ПараметрыШтрихкода.ИмяШрифта);
		Если ВнешняяКомпонента.Шрифт <> ИмяШрифта Тогда
			Если ВнешняяКомпонента.НайтиШрифт(ИмяШрифта) Тогда
				ВнешняяКомпонента.Шрифт = ИмяШрифта;
			Иначе
				ВнешняяКомпонента.Шрифт = ШрифтПоУмолчанию();
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыШтрихкода.ТипКода = 16 Или ПараметрыШтрихкода.ТипКода = 26 Тогда // QR
...
#КонецЕсли
	Если Метаданные.Подсистемы.Найти("ПоддержкаОборудования") = Неопределено Тогда
		// Вызов БСП
		Возврат Вычислить("ОбщегоНазначения"); // АПК:488 Код выполняется в безопасном режиме. Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только имя общего модуля.
		// Конец Вызов БСП
	Иначе
		Возврат Вычислить("ОбщегоНазначенияБПО"); // АПК:488 Код выполняется в безопасном режиме. Не нарушает безопасность информации, так как вычисляется фиксированная строка, содержащая только имя общего модуля.
	КонецЕсли;
