	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОбновитьРеквизиты Тогда
			...
			Попытка
				...
				ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.КонтурСинхронизации");
				ЭлементБлокировкиДанных.УстановитьЗначение("УзелИнформационнойБазы", Выборка.УзелИнформационнойБазы);
				ЭлементБлокировкиДанных.УстановитьЗначение("КодУзла", Выборка.КодУзла);
				ЭлементБлокировкиДанных.УстановитьЗначение("КодУзлаКорреспондента", Выборка.КодУзлаКорреспондента);
			...
			Исключение
				...
				Событие = НСтр("ru = 'Контроль зацикливания.Обновление реквизитов записи контура синхронизации'", ОбщегоНазначения.КодОсновногоЯзыка());

				ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка,
					Метаданные.РегистрыСведений.КонтурСинхронизации, , СообщениеОбОшибке);
			КонецПопытки; 
		...
		ИначеЕсли Выборка.УдалитьУзелИзКонтура Тогда
			...
			Исключение
				...
				Событие = НСтр("ru = 'Контроль зацикливания.Удаление записи контура синхронизации'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ИначеЕсли Выборка.УстановитьПоследнееОбновление Тогда
			...
			Исключение
				...
				Событие = НСтр("ru = 'Контроль зацикливания.Очистка устаревшей версии'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура СкрытьПоказатьПредупреждениеЗацикливанияОтПользователя(Направление) Экспорт
	
	ОписаниеНастроек = НСтр("ru = 'Скрыть подсказку о цикличных настройках синхронизации.'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	КлючОбъекта = "СкрытьПоказатьПредупреждениеЗацикливания";
	КлючНастроек = "СкрытьПоказатьПредупреждениеЗацикливания";
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъекта, КлючНастроек, Направление);
	
КонецПроцедуры

Функция СкрытоПредупреждениеЗацикливанияОтПользователя() Экспорт
	
	КлючОбъекта = "СкрытьПоказатьПредупреждениеЗацикливания";
	КлючНастроек = "СкрытьПоказатьПредупреждениеЗацикливания";
	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, КлючНастроек, Ложь);
	
КонецФункции

Функция ЕстьЗацикливание(УчитыватьОтключениеКонтроля = Истина) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ОтключитьКонтрольКонтураСинхронизации
	|ПОМЕСТИТЬ КонтрольКонтураОтключен
	|ИЗ
	|	РегистрСведений.ОбщиеНастройкиУзловИнформационныхБаз КАК ОбщиеНастройкиУзловИнформационныхБаз
	|ГДЕ
	|	ОбщиеНастройкиУзловИнформационныхБаз.ОтключитьКонтрольКонтураСинхронизации = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьЗацикливание,
	|	КонтрольКонтураОтключен.ОтключитьКонтрольКонтураСинхронизации КАК ОтключитьКонтрольКонтураСинхронизации
	|ИЗ
	|	РегистрСведений.КонтурСинхронизации КАК КонтурСинхронизации
	|		ЛЕВОЕ СОЕДИНЕНИЕ КонтрольКонтураОтключен КАК КонтрольКонтураОтключен
	|		ПО (КонтрольКонтураОтключен.ОтключитьКонтрольКонтураСинхронизации = ИСТИНА)
	|ГДЕ
	...
	// Проверим отключение контроля
	Если УчитыватьОтключениеКонтроля Тогда
		
		ВыборкаРезультата = Результат.Выбрать();
		ВыборкаРезультата.Следующий();
		Если ВыборкаРезультата.ОтключитьКонтрольКонтураСинхронизации = Истина Тогда
			
			Возврат Ложь; // Контроль отключен
...
Функция УзелЗациклен(УзелИнформационнойБазы, УчитыватьОтключениеКонтроля = Истина) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("УзелИнформационнойБазы", УзелИнформационнойБазы);
	Запрос.УстановитьПараметр("КодУзла", УзелИнформационнойБазы.Код);
	
	// Допускаем секцию ИЛИ в левом соединении, т.к. количество узлов исчисляется десятками, максимум сотнями
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ОбщиеНастройкиУзловИнформационныхБаз.ОтключитьКонтрольКонтураСинхронизации КАК ОтключитьКонтрольКонтураСинхронизации,
	...
	|ИЗ
	|	РегистрСведений.КонтурСинхронизации КАК КонтурСинхронизации,
	|	КонтрольКонтураОтключен КАК КонтрольКонтураОтключен
	|ГДЕ
	|	КонтурСинхронизации.Зацикливание
...
Функция ПроверитьЗацикливаниеРекурсивно(КонтурСинхронизации, НачальныйУзел, ТекущийУзел = "", Знач Родитель = "", Знач ЦепочкаУзлов = Неопределено) 
	...
	Если ЦепочкаУзлов.НайтиПоЗначению(ТекущийУзел) <> Неопределено Тогда // Уже анализировали переданный узел
		Возврат ЦепочкаУзлов;
	КонецЕсли;
	
	Если ЦепочкаУзлов.НайтиПоЗначению(НачальныйУзел) <> Неопределено Тогда // Определено зацикливание
		Возврат ЦепочкаУзлов;
	КонецЕсли;
	
	Если ПустаяСтрока(ТекущийУзел) Тогда
		ТекущийУзел = НачальныйУзел;
	Иначе
		ЦепочкаУзлов.Добавить(ТекущийУзел); // Добавляем все узлы, кроме начального
	КонецЕсли;
		...
		Если УзлыКонтура.Количество() > 0 
			...
			Для Каждого КоррУзелКонтура Из УзлыКонтура[0].Строки Цикл
				...
				Попытка
					БлокировкаДанных = Новый БлокировкаДанных;
					ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.КонтурСинхронизации");
					ЭлементБлокировкиДанных.УстановитьЗначение("УзелИнформационнойБазы", Неопределено);
					ЭлементБлокировкиДанных.УстановитьЗначение("КодУзла", КоррУзелКонтура.КодУзла);
					ЭлементБлокировкиДанных.УстановитьЗначение("КодУзлаКорреспондента", КоррУзелКонтура.КодУзлаКорреспондента);
					ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Исключительный;
					БлокировкаДанных.Заблокировать();
				...
				Исключение
					...
					Событие = НСтр("ru = 'Контроль зацикливания.Обновление контура синхронизации'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаменитьПрефиксы()
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Событие = НСтр("ru = 'Контроль зацикливания.Замена префиксов'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УстановитьПризнакЗацикливания(СписокУзлов = Неопределено)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Событие = НСтр("ru = 'Контроль зацикливания.Изменение признака зацикливание'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура СнятьПризнакЗацикливания()
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Событие = НСтр("ru = 'Контроль зацикливания.Изменение признака зацикливания'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УдалитьКоррУзел(КодУзла, КодУзлаКорреспондента)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Событие = НСтр("ru = 'Контроль зацикливания.Удаление записи'", ОбщегоНазначения.КодОсновногоЯзыка());
