...
Процедура СкрытьЛишниеПоляДанных(Вариант, НастройкиКД, ПользовательскиеНастройкиКД)
	...
	Если Вариант = "ПраваПользователяНаТаблицу" Тогда
		Если Не ВариантСОграничениямиДоступа()
		 Или НастройкиПравПоТаблицеВОтборе(НастройкиКД, ПользовательскиеНастройкиКД) <> Неопределено Тогда
			НастройкиПрав = "*";
		КонецЕсли;
...
Функция ТекстЗапросаОбщий()
	...
	Если УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"МАКСИМУМ(ПраваРолей.ПравоЧтениеБезОграничения)",     // @query-part-1
			"МАКСИМУМ(ПраваРолей.ПравоЧтение)");                  // @query-part-1
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"МАКСИМУМ(ПраваРолей.ПравоИзменениеБезОграничения)",  // @query-part-1
			"МАКСИМУМ(ПраваРолей.ПравоИзменение)");               // @query-part-1
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"МАКСИМУМ(ПраваРолей.ПравоДобавлениеБезОграничения)", // @query-part-1
			"МАКСИМУМ(ПраваРолей.ПравоДобавление)");              // @query-part-1
	КонецЕсли;
