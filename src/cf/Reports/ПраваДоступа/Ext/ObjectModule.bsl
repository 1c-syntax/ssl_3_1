...
Функция ВыбратьСведенияОПравахДоступа(Знач ВозможныеПрава, Знач ВыводПравГруппы, Знач ПользовательИлиГруппа)
	...
	|			ПО (ГруппыДоступаПользователя.Профиль = ПрофилиПользователя.Профиль)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВидыОграниченийПрав КАК ВидыОграниченийПрав
	|			ПО (ВидыОграниченийПрав.Таблица = ПраваРолей.ОбъектМетаданных)
	|				И (ТИПЗНАЧЕНИЯ(ВидыОграниченийПрав.ВидДоступа) = ТИП(Перечисление.ДополнительныеЗначенияДоступа))
	|				И (ВидыОграниченийПрав.Право = ""Чтение"")) КАК ПраваРолейПрофилей
	|ИТОГИ
	...
	|			ПО ПраваРолей.Роль = РолиПрофилей.Роль
	|				И (ПраваРолей.ПравоИзменение
	|					ИЛИ ПраваРолей.ПравоДобавление)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ГруппыДоступаПользователя КАК ГруппыДоступаПользователя
	|			ПО (ГруппыДоступаПользователя.Профиль = ПрофилиПользователя.Профиль)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВидыОграниченийПрав КАК ВидыОграниченийПрав
	|			ПО (ВидыОграниченийПрав.Таблица = ПраваРолей.ОбъектМетаданных)
	|				И (ТИПЗНАЧЕНИЯ(ВидыОграниченийПрав.ВидДоступа) = ТИП(Перечисление.ДополнительныеЗначенияДоступа))
	|				И (ВидыОграниченийПрав.Право = ""Изменение"")) КАК ПраваРолейПрофилей
	|ИТОГИ
	|	МАКСИМУМ(ПорядокВидаОбъектов),
	|	МАКСИМУМ(Таблица),
	|	МАКСИМУМ(ИдентификаторОбъектаМетаданных),
...
Процедура ВывестиДоступныеДляРедактирования(Знач ВозможныеПрава, Знач Макет, Знач РезультатЗапроса,
	...
	Для Каждого ОписаниеВидаОбъектов Из ПраваОбъектов.Строки Цикл
		...
		ДобавлениеИспользуется = Не СтандартныеПодсистемыСервер.ЭтоТаблицаРегистра(ОписаниеВидаОбъектов.Таблица);
...
Функция ШаблонПредставленияВидаДоступа(ОписаниеВидаДоступа, ВладельцыНастроекПрав)
	...
	Если ОписаниеВидаДоступа.ВсеРазрешены = Неопределено Тогда
		ШаблонПредставленияВидаДоступа = "%1";
	
	ИначеЕсли ОписаниеВидаДоступа.Строки.Количество() = 0 Тогда
		...
		ИначеЕсли ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка()
			 Или ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				
				ШаблонПредставленияВидаДоступа = НСтр("ru = '%1 (без запрещенных, авторизованный пользователь и его группы всегда разрешены)'");
			Иначе
		...
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка()
			 Или ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				...
				ШаблонПредставленияВидаДоступа = НСтр("ru = '%1 (без разрешенных, авторизованный пользователь и его группы всегда разрешены)'");
			Иначе
		...
		Если ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка()
			 Или ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				...
				ШаблонПредставленияВидаДоступа = НСтр("ru = '%1 (запрещенные, авторизованный пользователь и его группы всегда разрешены):'");
			Иначе
		...
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка()
			 Или ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				...
				ШаблонПредставленияВидаДоступа = НСтр("ru = '%1 (разрешенные, авторизованный пользователь и его группы всегда разрешены):'");
			Иначе
