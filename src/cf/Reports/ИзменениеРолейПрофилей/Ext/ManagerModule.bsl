...
Процедура ПередДобавлениемКомандОтчетов(КомандыОтчетов, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов")
	 ...
	 Или Не ПравоДоступа("Просмотр", Метаданные.Отчеты.ИзменениеРолейПрофилей)
	...
	Если Параметры.ИмяФормы <> ПараметрыДляОтчетов.ПолноеИмяФормыСпискаПрофилей
	   И Параметры.ИмяФормы <> ПараметрыДляОтчетов.ПолноеИмяФормыЭлементаПрофилей Тогда
		Возврат;
	КонецЕсли;
	
	Команда = КомандыОтчетов.Добавить();
	Команда.Представление = НСтр("ru = 'Изменение ролей профилей'");
	Команда.Менеджер = "Отчет.ИзменениеРолейПрофилей";
...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание =
		НСтр("ru = 'Выводит изменения ролей профилей с учетом изменения состава ролей в метаданных за указанный период по событиям журнала регистрации.'");
