...
Процедура ПередФормированиемОтчета(ФормаОтчета, ДополнительныеПараметры) Экспорт
	...
	Для Счетчик = 1 По 10000 Цикл
		Миллисекунд = 100 * 2 / 10 * 1000 / 50;
	КонецЦикла;
	Миллисекунд = ТекущаяУниверсальнаяДатаВМиллисекундах() - МоментНачала;
	ТактоваяЧастота = Окр(150 / Миллисекунд, 1);
	ТактоваяЧастота = ?(ТактоваяЧастота < 1, 1, ТактоваяЧастота);
	Коэффициент = Окр(4.2 / ТактоваяЧастота, 1);
	
	ГигабайтДо = Окр(КоличествоЯчеек / 1500000 + КоличествоСтрок / 6000, 1) + 0.4;
	МинутДо = Окр(КоличествоЯчеек / 1000000 * (1.15 + (КоличествоЯчеек / 65000000)
		* (КоличествоЯчеек / 65000000)) * Коэффициент, 1) + 0.2;
