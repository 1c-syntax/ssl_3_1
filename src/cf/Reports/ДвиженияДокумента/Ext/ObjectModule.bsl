...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, Настройки, ПользовательскиеНастройки) Экспорт
	...
	КлючРодительскогоВарианта = КлючРодительскогоВарианта(Контекст, КлючВарианта, Настройки);
	...
	Если КлючРодительскогоВарианта = "Основной" Тогда
		ПодготовитьГоризонтальныйВариант(ПараметрыОтчета.ДокументВладелец, Настройки, ЭтоПредопределенныйВариант);
	ИначеЕсли КлючРодительскогоВарианта = "Дополнительный" Тогда
		ПодготовитьВертикальныйВариант(ПараметрыОтчета.ДокументВладелец, Настройки, ЭтоПредопределенныйВариант);
	КонецЕсли;
...
Процедура ПередЗагрузкойВариантаНаСервере(Форма, Настройки) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.КонтекстВарианта) Тогда 
		Настройки.ДополнительныеСвойства.Вставить("ИсходныйДокументВладелец", ДокументВладелец);
	КонецЕсли;
	...
	Для Каждого СвойстваРегистра Из СвойстваРегистров Цикл 
		...
		Для Индекс = 0 По ЭтажностьСтроки - 1 Цикл
			...
			Для Каждого ГруппаПолей Из ГруппыВыбранныхПолей Цикл
				...
				Заголовок = "";
				Если ГруппаПолей.Заголовок = НСтр("ru = 'Стандартные реквизиты'") Тогда
					Заголовок = НСтр("ru = 'Стандартный реквизит'");
				ИначеЕсли ГруппаПолей.Заголовок = НСтр("ru = 'Измерения'") Тогда
					Заголовок = НСтр("ru = 'Измерение'");
				ИначеЕсли ГруппаПолей.Заголовок = НСтр("ru = 'Ресурсы'") Тогда
					Заголовок = НСтр("ru = 'Ресурс'");
				ИначеЕсли ГруппаПолей.Заголовок = НСтр("ru = 'Реквизиты'") Тогда
					Заголовок = НСтр("ru = 'Реквизит'");
				ИначеЕсли ГруппаПолей.Заголовок = НСтр("ru = 'Данные расчета'") Тогда
