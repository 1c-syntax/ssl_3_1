...
Функция ТекстЗапросаПоМетаданным(ПараметрыОтчета)
	...
	Если СтрНачинаетсяС(ОтборИсточника, "({") Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Где &ОтборИсточника", "");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяИсточника", ИмяИсточника);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Где &ОтборИсточника", ОтборИсточника);
	КонецЕсли;
...
Процедура ДобавитьИтогиРегистра(Знач ПараметрыОтчета, Знач СхемаКомпоновкиДанных)
	...
	Если ПараметрыОтчета.ИмяТаблицы = "ОстаткиИОбороты" 
		...
		ДобавитьПоляПериодаВНаборДанных(СхемаКомпоновкиДанных.НаборыДанных[0], ЕстьПолеРегистратор);
	КонецЕсли;
	...
	Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
		...
		Для НомерСубконто = 1 По КоличествоСубконто Цикл
			...
			ПолеСубконто.Роль.ИгнорироватьЗначенияNULL = Ложь;
		КонецЦикла;
...
//  ЕстьПолеРегистратор - Булево
//
// Возвращаемое значение:
//  СписокЗначений
//
Функция ДобавитьПоляПериодаВНаборДанных(НаборДанных, ЕстьПолеРегистратор)
	...
	ПапкаПолейНабораДанных.Заголовок   = НСтр("ru = 'Периоды'");
	...
	Для каждого Период Из СписокПериодов Цикл
		...
		ПолеНабораДанных.Роль.НомерПериода = ?(ЕстьПолеРегистратор, СписокПериодов.Индекс(Период) + 2, СписокПериодов.Индекс(Период));
