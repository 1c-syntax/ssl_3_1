...
Процедура УстановитьФиксированныйПараметр(Идентификатор, Параметры, Настройки, ПользовательскиеНастройки, ДоступныеЗначения = Неопределено)
	...
	Если НЕ СброситьНастройки И ТипЗнч(ПользовательскиеНастройки) = Тип("ПользовательскиеНастройкиКомпоновкиДанных")
	...
	Если Идентификатор = "ТипОбъектаМетаданных"
		...
		ОбъектМетаданных = МетаданныеИсточникаДанных(Параметры.ИсточникДанных);
...
Процедура УстановитьСтандартныеНастройки(Отчет, ФиксированныеПараметры, Настройки, ПользовательскиеНастройки) Экспорт 
	...
	СброситьНастройки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Настройки.ДополнительныеСвойства, "СброситьНастройки", Ложь);
	
	Если ОтчетИнициализирован И НЕ СброситьНастройки Тогда 
		Возврат;
	КонецЕсли;
...
Функция МетаданныеИсточникаДанных(ИсточникДанных)
	Попытка
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(ИсточникДанных);
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отчет не может быть сформирован по причине: %1'"),
...
Процедура ВывестиКоличествоПодчиненныхЗаписейГруппировки(Группировка, Выводить)
	...
	Если ПоляГруппировки.Количество() > 0 Тогда 
		...
		ВыбранныПоля = Группировка.Выбор;
		ПолеКоличестваЗаписей = ПолеКоличестваПодчиненныхЗаписей(ВыбранныПоля, ВыбранныПоля.Элементы);
...
Процедура ВывестиКоличествоПодчиненныхЗаписейТаблицы(Таблица, Выводить)
	...
	ВыбранныПоля = КолонкаКоличестваЗаписей.Выбор;
	ПолеКоличестваЗаписей = ПолеКоличестваПодчиненныхЗаписей(ВыбранныПоля, ВыбранныПоля.Элементы);
	ПолеКоличестваЗаписей.Использование = Выводить;
...
Функция ПолеКоличестваПодчиненныхЗаписей(ВыбранныПоля, Элементы)
	...
	Для Каждого Элемент Из Элементы Цикл 
		...
		ИначеЕсли ТипЗнч(Элемент) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда 
			...
			Если Элемент.Использование Тогда 
				ПолеКоличестваПодчиненныхЗаписей(ВыбранныПоля, Элемент.Элементы);
			КонецЕсли;
	...
	Если ПолеКоличестваЗаписей = Неопределено Тогда 
		...
		ПолеКоличестваЗаписей = ВыбранныПоля.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
