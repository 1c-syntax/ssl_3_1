...
Процедура УстановитьСтандартныйЗаголовокОтчета(Контекст, Настройки, ФиксированныеПараметры, ДоступныеЗначения) Экспорт 
	...
	Если ЗначенияПараметров.Период <> Неопределено
		И ТипЗнч(ЗначенияПараметров.Период) = Тип("СтандартныйПериод") Тогда 
		
		Период = ЗначенияПараметров.Период; // СтандартныйПериод
		ПредставлениеПериода = СтроковыеФункции.ПредставлениеПериодаВТексте(Период.ДатаНачала, Период.ДатаОкончания);
...
Функция ВариантыИспользованияОформления()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СтандартныеПодсистемыСервер.ВерсияРежимаСовместимости(), "8.3.16.0") < 0 Тогда 
