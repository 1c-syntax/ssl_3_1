...
Процедура УстановитьСтандартныйЗаголовокОтчета(Контекст, Настройки, ФиксированныеПараметры, ДоступныеЗначения) Экспорт 
	...
	Если ЗначенияПараметров.Период <> Неопределено Тогда 
		...
		Период = ЗначенияПараметров.Период;
...
Процедура СкрытьКоличествоЗаписейВДетальныхЗаписях(ДетальныеЗаписи)
	...
	ВариантыИспользования = Новый Структура("ВЗаголовке, ВЗаголовкеОбщегоИтога,
		|ВЗаголовкеПолей, ВЗаголовкеПолейРесурсов, ВЗаголовкеПолейРесурсовОбщегоИтога,
		|ВОбщемИтоге, ВОтборе, ВПараметрах");
	
	Для Каждого Элемент Из ВариантыИспользования Цикл 
		
		ВариантИспользования = "Использовать" + Элемент.Ключ;
