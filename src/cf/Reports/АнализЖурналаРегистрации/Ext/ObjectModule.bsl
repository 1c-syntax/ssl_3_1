...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если ВариантОтчета <> "АнализАктивностиПользователей"
	   И ВариантОтчета <> "АнализАктивностиПодразделений"
	   И ВариантОтчета <> "АктивностьПользователя" Тогда
		
		СхемаКомпоновкиДанных.Параметры.Приложение.ОграничениеИспользования = Истина;
	Иначе
		ИменаПриложений = Отчеты.АнализЖурналаРегистрации.ИменаПриложенийАктивностиПользователей();
		Приложения = Новый СписокЗначений;
		Для Каждого ИмяПриложения Из ИменаПриложений Цикл
			Приложения.Добавить(ИмяПриложения, ПредставлениеПриложения(ИмяПриложения));
		КонецЦикла;
		СхемаКомпоновкиДанных.Параметры.Приложение.УстановитьДоступныеЗначения(Приложения);
		ПолеПриложение = СхемаКомпоновкиДанных.НаборыДанных.АнализАктивностиПользователей.Поля.Найти("Приложение");
		ПолеПриложение.УстановитьДоступныеЗначения(Приложения);
	КонецЕсли;
...
Функция ПараметрыОтчетаАктивностьПользователя(НастройкиОтчета)
	...
	ПараметрПриложение = НастройкиОтчета.ПараметрыДанных.Элементы.Найти("Приложение");
	Если Не ПараметрПриложение.Использование Тогда
		ИменаПриложений = Отчеты.АнализЖурналаРегистрации.ИменаПриложенийАктивностиПользователей();
	ИначеЕсли Не ЗначениеЗаполнено(ПараметрПриложение.Значение) Тогда
		ИменаПриложений = Новый Массив;
	ИначеЕсли ТипЗнч(ПараметрПриложение.Значение) = Тип("СписокЗначений") Тогда
		ИменаПриложений = ПараметрПриложение.Значение.ВыгрузитьЗначения();
	Иначе
		ИменаПриложений = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрПриложение.Значение);
	КонецЕсли;
	...
	ПараметрыОтчета.Вставить("ИменаПриложений", ИменаПриложений);
