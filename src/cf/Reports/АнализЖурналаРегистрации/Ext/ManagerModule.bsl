...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	ПодсистемаАдминистрирования = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(
...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПодразделений" Тогда
		ЗаполнитьПользователейДляАнализаИзПодразделения(ПользовательФильтр, Подразделение);
	Иначе
...
Процедура ЗаполнитьПользователейДляАнализаИзПодразделения(ПользовательФильтр, ЗначениеОтбора)
	...
	Если ТипЗнч(ЗначениеОтбора) = Тип("СписокЗначений") Тогда
		ВыбранныеПодразделения = ЗначениеОтбора.ВыгрузитьЗначения();
	Иначе
		ВыбранныеПодразделения = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЗначениеОтбора);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПользовательИБ <> Неопределено Тогда
			ПользовательФильтр.Добавить(ПользовательИБ.Имя);
		КонецЕсли;
	КонецЦикла;
...
Функция ПользовательДляОтбора(ПользовательСсылка) Экспорт
