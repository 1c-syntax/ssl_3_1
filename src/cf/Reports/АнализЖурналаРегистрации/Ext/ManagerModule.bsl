...
//    * ИменаПриложений     - см. ИменаПриложенийАктивностиПользователей
//
...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	ИсходныеДанные.Колонки.Добавить("Приложение");
	...
	ИменаВсехПриложений = ИменаПриложенийАктивностиПользователей();
	Для Каждого ИмяПриложения Из ПараметрыОтчета.ИменаПриложений Цикл
		Если ИменаВсехПриложений.Найти(ИмяПриложения) <> Неопределено Тогда
			ИменаПриложений.Добавить(ИмяПриложения);
		КонецЕсли;
	КонецЦикла;
	...
	ОтборЖурналаРегистрации.Вставить("ИмяПриложения", ИменаПриложений);
	...
	Сеансы.Колонки.Добавить("Приложение");
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл
		...
		Если СеансДобавлен Тогда
			Сеанс.Приложение = СтрокаДанныхЖурналаРегистрации.ИмяПриложения;
		...
		Если СтрокаИсходныхДанных <> Неопределено Тогда
			...
			СтрокаИсходныхДанных.Приложение = СтрокаДанныхЖурналаРегистрации.ИмяПриложения;
	...
	Для Каждого Сеанс Из Сеансы Цикл
		...
		Пока Продолжать Цикл
			...
			СтрокаИсходныхДанных.Приложение = Сеанс.Приложение;
	...
	Если ВариантОтчета = "АнализАктивностиПользователей"
	 Или ВариантОтчета = "АнализАктивностиПодразделений" Тогда
		...
		ДанныеЖурналаРегистрации = ИнформацияПоОшибкамЖурналаРегистрации(ДатаНачала,
			ДатаОкончания, СмещениеВремениСервера, ПользовательФильтр, ИменаПриложений);
...
// Список приложений, используемых при получении информации по активности пользователей.
//
// Возвращаемое значение:
//  Массив из Строка - имя приложения, например, 1CV8C.
//
Функция ИменаПриложенийАктивностиПользователей() Экспорт
	
	Приложения = Новый Массив;
	Приложения.Добавить("1CV8C");
	Приложения.Добавить("WebClient");
	Приложения.Добавить("1CV8");
	Приложения.Добавить("BackgroundJob");
	Приложения.Добавить("MobileClient");
	Приложения.Добавить("HTTPServiceConnection");
	Приложения.Добавить("WSConnection");
	Приложения.Добавить("ODataConnection");
	Приложения.Добавить("COMConnection");
...
Функция ИнформацияПоОшибкамЖурналаРегистрации(ДатаНачала, ДатаОкончания,
			СмещениеВремениСервера, ПользовательФильтр = Неопределено, ИменаПриложений = Неопределено)
	...
	КонецЕсли;
	Если ИменаПриложений <> Неопределено Тогда
