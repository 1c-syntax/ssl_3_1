...
Функция СообщениеОСтатусеЗадания(Форма)
	...
	ИначеЕсли Форма.Отложен Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Задание будет запущено <a href=""%1"">%2</a>'"),
				"ОткрытьНастройкуОтложенногоСтарта", ДатаОтложенногоСтартаСтрокой);
	КонецЕсли;
