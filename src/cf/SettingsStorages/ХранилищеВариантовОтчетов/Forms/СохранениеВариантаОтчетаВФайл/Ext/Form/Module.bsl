...
Процедура ПриОткрытии(Отказ)
	...
	Обработчик = Новый ОписаниеОповещения("ПослеУстановкиРасширения1СПредприятия", ЭтотОбъект);
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(Обработчик, ТекстПредложения());
...
Процедура ПользовательскиеНастройкиПриИзменении(Элемент)
	...
	ТекущиеПользовательскиеНастройки = ПерсональныеНастройки.НайтиПоЗначению(
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПерсональныеНастройки.Представление");
...
Процедура ЗаполнитьОписаниеВариантовОтчетов()
	...
	Если Параметры.ВыбранныеВариантыОтчетов <> Неопределено Тогда 
...
Процедура ПрочитатьПользовательскиеНастройки()
	...
	Пока Выборка.Следующий() Цикл 
		...
		ПерсональныеНастройки.Добавить(Выборка.КлючНастроек, Выборка.Представление);
		ЗаполнитьЗначенияСвойств(ХранилищеПользовательскихНастроек.Добавить(), Выборка);
		
	КонецЦикла;
	
	ТекущиеПользовательскиеНастройки = ПерсональныеНастройки.НайтиПоЗначению(ОписаниеВариантаОтчета.КлючПользовательскихНастроек);
	Если ТекущиеПользовательскиеНастройки = Неопределено Тогда 
		Элементы.КоманднаяПанельПерсональныеНастройки.Видимость = Ложь;
		Элементы.ПерсональныеНастройки.ТолькоПросмотр = Истина;
		Элементы.ПерсональныеНастройки.Доступность = Ложь;
		ПерсональныеНастройки.Добавить("", НСтр("ru='У отчета нет персональных настроек.'"));
		Возврат;
	КонецЕсли;
	...
	Индекс = ПерсональныеНастройки.Индекс(ТекущиеПользовательскиеНастройки);
	Если Индекс > 0 Тогда 
		ПерсональныеНастройки.Сдвинуть(ТекущиеПользовательскиеНастройки, -Индекс);
	КонецЕсли;
...
Процедура УпаковатьНастройкиВариантаОтчета(ОписаниеВариантаОтчета)
	...
	Для Каждого ЭлементСписка Из ПерсональныеНастройки Цикл 
...
&НаКлиенте
Процедура ПослеУстановкиРасширения1СПредприятия(РасширениеПодключено, ДополнительныеПараметры) Экспорт
