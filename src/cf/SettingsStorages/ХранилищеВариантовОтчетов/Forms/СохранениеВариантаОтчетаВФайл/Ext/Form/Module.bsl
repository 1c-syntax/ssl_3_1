...
Процедура ПриОткрытии(Отказ)
	...
	ТекстПредложения = НСтр("ru = 'Для сохранения варианта отчета в файл необходимо
...
Процедура Сохранить(Команда)
	...
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресХранилищаАрхива);
...
Процедура ЗаполнитьОписаниеВариантаОтчета()
	...
	ОписаниеВариантаОтчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ВариантОтчета, "Отчет, КлючВарианта, Представление, Настройки");
...
Процедура ИмяФайлаПослеВыбораКаталога(Каталог, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(Каталог) Тогда 
		ИмяФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(Каталог) + "ReportOptions.zip";
	КонецЕсли;
...
&НаКлиенте
Процедура УпаковатьНастройкиВариантаОтчетаЗавершение(ДополнительныеПараметры) Экспорт 
...
&НаКлиенте
Процедура ПослеУстановкиРасширенияРаботыСФайлами(Знач РасширениеПодключено, ДополнительныеПараметры) Экспорт
	
	РасширениеПодключено = (РасширениеПодключено = Истина);
	
	Элементы.Сохранить.Доступность = РасширениеПодключено;
	Элементы.ИмяФайла.ТолькоПросмотр = Не РасширениеПодключено;
	
	Если Не РасширениеПодключено
		Или ЗначениеЗаполнено(ИмяФайла) Тогда
...
Процедура ПослеПолученияКаталогаДокументов(ИмяКаталогаДокументов, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(ИмяКаталогаДокументов) Тогда 
		ИмяФайла = ИмяКаталогаДокументов + "ReportOptions.zip";
	КонецЕсли;
