...
&НаСервере
Процедура ЗаполнитьСписокДоступныхПолей(Знач ПараметрыЗаполнения)
	ПараметрыЗаполнения.КоллекцииПолей = КоллекцииПолей();
...
Процедура АктивизироватьДоступноеПоле()
	...
	Для Каждого СтрокаСписка Из ДанныеДоступныхПолей Цикл 
		Если СтрокаСписка.Поле = Поле Тогда 
			ПолеДоступныхПолей.ТекущаяСтрока = СтрокаСписка.ПолучитьИдентификатор();
		КонецЕсли;
	КонецЦикла;
