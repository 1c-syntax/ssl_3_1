...
Процедура ЗавершитьЗаполнениеЗначений(РезультатЗаполнения = Неопределено)
	...
	Если РезультатЗаполнения <> Неопределено Тогда 
		...
		Если РезультатЗаполнения.Свойство("Значения") Тогда 
			ОбщегоНазначенияКлиентСервер.ЗаполнитьКоллекциюСвойств(РезультатЗаполнения.Значения, Значения);
		КонецЕсли;
...
Процедура ОбработатьОшибкуЗаполненияЗначений(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки)
	...
	Комменатий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, ПодробноеПредставлениеОшибки);
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Настройка поля отчета.Заполнение фильтра по значению'"),
