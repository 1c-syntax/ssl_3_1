...
Функция НачатьЗаполнениеЗначений(ИдентификаторЗадания)
	...
	ПараметрыМетода.УточнениеОшибки = НСтр("ru = 'Не удалось заполнить фильтр по значению по причине:'");
...
Процедура ПослеЗаполненияЗначений(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		...
		Элементы.ОписаниеОшибкиЗаполнения.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонКомментария, Результат.КраткоеПредставлениеОшибки);
...
Функция ТипГруппировкиПоля()
	...
	ГруппировкаКомпоновкиДанных = КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(СвойстваЗаголовка.ИдентификаторГруппировки);
	
	Если ТипЗнч(ГруппировкаКомпоновкиДанных) <> Тип("ГруппировкаКомпоновкиДанных")
		И ТипЗнч(ГруппировкаКомпоновкиДанных) <> Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда 
		
		Возврат ТипГруппировкиПоля;
	КонецЕсли;
	
	ПолеГруппировки = ВариантыОтчетовСлужебныйКлиентСервер.ПолеОтчета(ГруппировкаКомпоновкиДанных.ПоляГруппировки, СвойстваЗаголовка.Поле);
