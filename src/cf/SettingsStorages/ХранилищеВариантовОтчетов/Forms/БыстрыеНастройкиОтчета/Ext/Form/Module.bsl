...
Процедура Применить(Команда)
	...
	Результат = ОтчетыКлиентСервер.ПараметрыОбновленияФормыОтчета(
		ВариантыОтчетовСлужебныйКлиентСервер.ИмяСобытияИзмененияСоставаБыстрыхНастроек());
	Результат.КомпоновщикНастроекКД = КомпоновщикНастроек;
	Результат.ВариантМодифицирован = ВариантМодифицирован;
	Результат.СброситьПользовательскиеНастройки = ВариантМодифицирован;
	Результат.ПользовательскиеНастройкиМодифицированы = ВариантМодифицирован;
...
Функция ЗаголовокНастройки(ЭлементНастроек, ОписаниеНастройки, ТипНастройки, ЭлементПользовательскихНастроек)
	...
	Если ТипЗнч(ЗначениеНастройки) = Тип("ПолеКомпоновкиДанных") Тогда
		ДоступноеПоле = КомпоновщикНастроек.Настройки.Выбор.ДоступныеПоляВыбора.НайтиПоле(ЗначениеНастройки);
		Если ДоступноеПоле <> Неопределено Тогда
			ЗначениеНастройки = ДоступноеПоле.Заголовок;
		КонецЕсли;
	КонецЕсли;
