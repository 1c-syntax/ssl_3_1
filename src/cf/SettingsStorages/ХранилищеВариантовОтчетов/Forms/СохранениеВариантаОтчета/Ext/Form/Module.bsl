...
Процедура ВыполнитьПакет(Результат, Пакет) Экспорт
	...
	Если Пакет.ЗакрытьПослеЗаписи = Истина Тогда
		ВариантыОтчетовКлиент.ОбновитьОткрытыеФормы(, ВариантСсылка);
...
Процедура ПроверитьИЗаписатьВариантОтчета(Пакет)
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ПрототипСсылка) = Тип("СправочникСсылка.ВариантыОтчетов") И Не ПрототипСсылка.Пустая() Тогда
			ЭлементБлокировки = Блокировка.Добавить(ПрототипСсылка.Метаданные().ПолноеИмя());
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ПрототипСсылка);
...
Процедура ЗаполнитьПредставления(Вариант)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
			...
			ШаблонНаименование  = "ВЫБОР
			|		КОГДА НЕ ИзКонфигурации.%1 ЕСТЬ NULL
			|			ТОГДА ИзКонфигурации.%1
			|		КОГДА НЕ ИзРасширений.%1 ЕСТЬ NULL
			|			ТОГДА ИзРасширений.%1
			|		ИНАЧЕ Пользовательские.%1
			|	КОНЕЦ КАК %1";
			
			ШаблонОписание  = "ВЫБОР
			|		КОГДА ПОДСТРОКА(Пользовательские.%1, 1, 1) <> """"
			|			ТОГДА Пользовательские.%1
			|		КОГДА НЕ ИзКонфигурации.%1 ЕСТЬ NULL
			|			ТОГДА ИзКонфигурации.%1
			|		КОГДА НЕ ИзРасширений.%1 ЕСТЬ NULL
			|			ТОГДА ИзРасширений.%1
			...
			Для ПорядковыйНомерЯзыка = 1 По МодульМультиязычностьСервер.КоличествоДополнительныхЯзыков() Цикл
				
				СуффиксЯзыка = МодульМультиязычностьКлиентСервер.СуффиксЯзыка(ПорядковыйНомерЯзыка);
				ИмяРеквизитаНаименование = "Наименование" + СуффиксЯзыка;
				ИмяРеквизитаОписание     = "Описание" + СуффиксЯзыка;
				Набор.Добавить(СтрШаблон(ШаблонНаименование, ИмяРеквизитаНаименование));
				Набор.Добавить(СтрШаблон(ШаблонОписание, ИмяРеквизитаОписание));
			КонецЦикла;
			
			ТекстЗапросаМультиязычныеРеквизиты = СтрСоединить(Набор, "," + Символы.ПС);
