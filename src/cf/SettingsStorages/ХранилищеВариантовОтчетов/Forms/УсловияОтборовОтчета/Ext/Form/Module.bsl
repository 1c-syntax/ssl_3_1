...
Процедура ВыбратьИЗакрыть()
	...
	Для Каждого Элемент Из Отборы.ПолучитьЭлементы() Цикл
		Если Элемент.ИсходныйВидСравнения <> Элемент.ВидСравнения Тогда
			УсловияОтборов.Вставить(Элемент.Идентификатор, Элемент.ВидСравнения);
		КонецЕсли;
	КонецЦикла;
