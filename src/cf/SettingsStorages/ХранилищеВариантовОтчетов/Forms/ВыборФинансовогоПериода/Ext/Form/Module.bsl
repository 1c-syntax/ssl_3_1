...
Процедура УстановитьКартинкуПереходаКСтандартномуПериоду()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СтандартныеПодсистемыСервер.ВерсияРежимаСовместимости(), "8.3.17.0") >= 0 Тогда
		КартинкаПереходаКСтандартномуПериоду = БиблиотекаКартинок["Календарь"];
	Иначе  
		КартинкаПереходаКСтандартномуПериоду = БиблиотекаКартинок["УстановитьИнтервал"];
	КонецЕсли;
