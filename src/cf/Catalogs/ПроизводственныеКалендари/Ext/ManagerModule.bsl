...
Функция ДанныеПроизводственныхКалендарейИзXML(Знач ДанныеXML, ТаблицаКалендарей, КодыКалендарей = Неопределено, ФормироватьПолныйНабор = Истина) Экспорт
	...
	ГодыКалендарей.Сортировать("Calendar, Year");
	...
	Для Каждого Сочетание Из ГодыКалендарей Цикл
		...
		Если КодБазовогоКалендаря <> Неопределено Тогда
			...
			Для Каждого СтрокаКлассификатора Из СтрокиДанныхКалендаря Цикл
				...
				ПараметрыОтбора = Новый Структура("КодПроизводственногоКалендаря,Дата");
				ПараметрыОтбора.КодПроизводственногоКалендаря = СтрокаКлассификатора.Calendar;
				ПараметрыОтбора.Дата = Дата(СтрокаКлассификатора.Date);
...
//  ДанныеПроизводственногоКалендаря - см. Справочники.ПроизводственныеКалендари.ДанныеПроизводственногоКалендаря
//
...
//   * ДатаПереноса - Дата 
//
Функция НовыеДанныеПроизводственныхКалендарей() Экспорт
	
	ТаблицаДанных = Новый ТаблицаЗначений;
	ТаблицаДанных.Колонки.Добавить("КодПроизводственногоКалендаря", Новый ОписаниеТипов("Строка", ,
		Новый КвалификаторыСтроки(2)));
...
#Область ПечатнаяФормаПроизводственногоКалендаря
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьПроизводственныеКалендариПоУмолчаниюДлительнаяОперация", Истина);
