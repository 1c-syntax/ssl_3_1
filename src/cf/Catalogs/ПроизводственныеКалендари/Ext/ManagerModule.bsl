...
Функция ДанныеПроизводственныхКалендарейИзФайлаКлассификатора(КодыКалендарей = Неопределено)
...
Функция БазовыйКалендарь(ПроизводственныйКалендарь) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПроизводственныйКалендарь", ПроизводственныйКалендарь);
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(Базовый.Ссылка, Текущий.Ссылка) КАК Ссылка,
		|	ЕСТЬNULL(Базовый.Код, Текущий.Код) КАК Код,
		|	ЕСТЬNULL(Базовый.Наименование, Текущий.Наименование) КАК Наименование
		|ИЗ
		|	Справочник.ПроизводственныеКалендари КАК Текущий
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПроизводственныеКалендари КАК Базовый
		|		ПО Текущий.БазовыйКалендарь = Базовый.Ссылка
		|ГДЕ
		|	Текущий.Ссылка = &ПроизводственныйКалендарь";
