...
Процедура ЗашифроватьПослеШифрованияНаКлиенте(Результат, ПараметрыВыполнения) Экспорт
	...
	Оповестить("Запись_Файл", Новый Структура, Объект.Ссылка);
...
Процедура ОсвободитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ОсвободитьФайл();
	ОповеститьОбИзменении(Объект.Ссылка);
	Оповестить("Запись_Файл", Новый Структура("Событие", "РедактированиеОтменено"), Объект.Ссылка);
...
Процедура ПечатьСоШтампом(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка)
		Или Записать() Тогда
		ДокументСоШтампом = РаботаСФайламиСлужебныйВызовСервера.ТабличныйДокументСоШтампом(Объект.Ссылка, Объект.Ссылка);
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Оповестить("Запись_Файл", Новый Структура("ЭтоНовый", ФайлБылСоздан), Объект.Ссылка);
