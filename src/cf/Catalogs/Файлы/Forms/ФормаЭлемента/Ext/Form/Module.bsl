...
Процедура УдалитьИзСпискаПодписейИЗаписатьФайл()
	...
	Для Каждого НомерВыделеннойСтроки Из Элементы.ЭлектронныеПодписи.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(УдаляемаяСтрока.ПорядковыйНомер) Тогда
			ИндексыСтрок.Добавить(УдаляемаяСтрока.ПорядковыйНомер);
		Иначе
			ИндексыСтрок.Добавить(УдаляемаяСтрока.ИдентификаторПодписи);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПеренестиРезультатыПроверкиПодписей()
	РаботаСФайламиСлужебный.ПеренестиРезультатыПроверкиПодписей(ЭлектронныеПодписи, Объект.Ссылка);
КонецПроцедуры
