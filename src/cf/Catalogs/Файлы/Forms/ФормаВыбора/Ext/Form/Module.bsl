...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВладелецФайла) = Тип("СправочникСсылка.ПапкиФайлов") Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Параметры.ТекущаяСтрока) И ВладелецФайла.Пустая() Тогда
			 НовоеЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ТекущаяСтрока, "ВладелецФайла", Истина);
			 Если НовоеЗначение <> Неопределено Тогда
			 	ВладелецФайла = НовоеЗначение;
			 КонецЕсли;
		КонецЕсли;	
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" Тогда
		...
		Если Параметр.ЭтоНовый = Истина Тогда
			Если Параметр.Владелец = Элементы.Папки.ТекущаяСтрока Тогда
...
Процедура ОпределитьВозможностьДобавлениеШаблоновФайлов()
	...
	Иначе
		ЕстьПравоДобавлениеФайлов = ПравоДоступа("Добавление", Метаданные.Справочники.Файлы) 
			И ПравоДоступа("Чтение", Метаданные.Справочники.ПапкиФайлов);
	КонецЕсли;
