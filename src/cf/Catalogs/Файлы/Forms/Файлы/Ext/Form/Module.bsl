...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ДоступностьКомандыРедактировать = Элементы.ФормаРедактировать.Видимость И Элементы.ФормаРедактировать.Доступность;
	...
	ПараметрыДействия.ДоступностьКомандыРедактировать = ДоступностьКомандыРедактировать;
...
Процедура Подписать(Команда)
	...
	РаботаСФайламиКлиент.ПодписатьФайл(Элементы.Список.ВыделенныеСтроки, УникальныйИдентификатор, ДополнительныеПараметры,
		ПараметрыПодписания);
...
Процедура ОтправитьНаПодписание(Команда)
	...
	РаботаСФайламиКлиент.ОтправитьФайлНаПодписание(Элементы.Список.ВыделенныеСтроки, УникальныйИдентификатор, ДополнительныеПараметры,
		ПараметрыПодписания);
...
Процедура Отправить(Команда)
	...
	ПриОтправкеФайловПочтой(ПараметрыОтправки, Элементы.Список.ВыделенныеСтроки, ТекущиеДанные.Ссылка, 
		УникальныйИдентификатор);
...
Процедура УстановитьДоступностьКомандПриИзмененииПапки()
	...
	ЕстьТекущаяСтрока = ЗначениеЗаполнено(Элементы.Папки.ТекущаяСтрока);
	Если ЕстьТекущаяСтрока Тогда
		Если Элементы.Папки.ТекущаяСтрока <> ТекущаяПапка Тогда
			...
			НастройкиПравПапки = НастройкиПравПапки(Элементы.Папки.ТекущаяСтрока);
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, НастройкиПравПапки);
		КонецЕсли;
	КонецЕсли;
	
	УстановитьДоступностьКомандПапокПриИзмененииПапки();
	УстановитьДоступностьКомандСпискаПриИзмененииПапки();
...
Функция ЭлементФормыСуществует(Форма, ИмяЭлемента)
	Результат = Форма.Элементы.Найти(ИмяЭлемента) <> Неопределено;
	Возврат Результат;
КонецФункции

&НаКлиенте
Процедура УстановитьДоступностьКомандПапокПриИзмененииПапки()
	ЕстьТекущаяСтрока = ЗначениеЗаполнено(Элементы.Папки.ТекущаяСтрока);
	РазрешеноРедактирование = ЕстьТекущаяСтрока И Не ФайлыРедактируютсяВОблачномСервисе;
	РазрешеноДобавление = РазрешеноРедактирование И ДобавлениеФайлов;
	
	Элементы.ФормаИмпортПапки.Доступность = РазрешеноДобавление;	
	Элементы.ПапкиКонтекстноеМенюИмпортПапки.Доступность = Элементы.ФормаИмпортПапки.Доступность;
	
	Элементы.ПапкиКонтекстноеМенюУровеньВверх.Доступность = ЕстьТекущаяСтрока;
	Элементы.ПапкиКонтекстноеМенюЭкспортПапки.Доступность = ЕстьТекущаяСтрока;
	Элементы.ПапкиКонтекстноеМенюНастроитьСинхронизацию.Доступность = ЕстьТекущаяСтрока;

	Если ЭлементФормыСуществует(ЭтотОбъект, "ПапкиКонтекстноеМенюСоздать") Тогда
		Элементы.ФормаСоздатьПапку.Доступность = ИзменениеПапок И РазрешеноРедактирование;
		Элементы.ПапкиКонтекстноеМенюСоздать.Доступность = Элементы.ФормаСоздатьПапку.Доступность;
	КонецЕсли;
	Если ЭлементФормыСуществует(ЭтотОбъект, "ПапкиКонтекстноеМенюУстановитьПометкуУдаления") Тогда
		Элементы.ПапкиКонтекстноеМенюУстановитьПометкуУдаления.Доступность = ИзменениеПапок;
	КонецЕсли;
	Если ЭлементФормыСуществует(ЭтотОбъект, "ПапкиКонтекстноеМенюПеренестиЭлемент") Тогда
		Элементы.ПапкиКонтекстноеМенюПеренестиЭлемент.Доступность = ИзменениеПапок;
	КонецЕсли;
	Если ЭлементФормыСуществует(ЭтотОбъект, "ПапкиКонтекстноеМенюСкопировать") Тогда
		Элементы.ПапкиКонтекстноеМенюСкопировать.Доступность = РазрешеноДобавление;
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьДоступностьКомандСпискаПриИзмененииПапки()
	ЕстьТекущаяСтрока = ЗначениеЗаполнено(Элементы.Папки.ТекущаяСтрока);
	РазрешеноРедактирование = ЕстьТекущаяСтрока И Не ФайлыРедактируютсяВОблачномСервисе;
	РазрешеноДобавление = РазрешеноРедактирование И ДобавлениеФайлов;
	
	Элементы.ФормаПодменюСоздать.Доступность = РазрешеноДобавление;
	Элементы.ФормаСоздатьИзФайла.Доступность = РазрешеноДобавление;
	Элементы.ФормаСоздатьИзШаблона.Доступность = РазрешеноДобавление;
	Элементы.ФормаСоздатьСоСканера.Доступность = РазрешеноДобавление;
	Элементы.ФормаПеренестиВПапку.Доступность = РазрешеноРедактирование;
	Элементы.ФормаОсвободить.Доступность = РазрешеноРедактирование;
	Элементы.ФормаИмпортФайлов.Доступность = РазрешеноДобавление;
	
	Элементы.СписокКонтекстноеМенюПеренестиВПапку.Доступность = Элементы.ФормаПеренестиВПапку.Доступность;
	Элементы.СписокКонтекстноеМенюОсвободить.Доступность = Элементы.ФормаОсвободить.Доступность;
	Элементы.СписокКонтекстноеМенюИмпортФайлов.Доступность = Элементы.ФормаИмпортФайлов.Доступность;

	Если ЭлементФормыСуществует(ЭтотОбъект, "ФормаСкопировать") Тогда
		Элементы.ФормаСкопировать.Доступность = РазрешеноДобавление;
		Элементы.СписокКонтекстноеМенюСкопировать.Доступность = Элементы.ФормаСкопировать.Доступность;
	КонецЕсли;

	Если ЭлементФормыСуществует(ЭтотОбъект, "СписокКонтекстноеМенюСоздать") Тогда
		Элементы.СписокКонтекстноеМенюСоздать.Доступность = РазрешеноДобавление;
	КонецЕсли;
	
	Если ЭлементФормыСуществует(ЭтотОбъект, "ФормаУстановитьПометкуУдаления") Тогда
		Элементы.ФормаУстановитьПометкуУдаления.Доступность = РазрешеноРедактирование И ПометкаУдаленияФайлов;
		Элементы.СписокКонтекстноеМенюУстановитьПометкуУдаления.Доступность = Элементы.ФормаУстановитьПометкуУдаления.Доступность;
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьВидимостьПоясненияСинхронизацииФайлов()
	...
	Если Элементы.Папки.ТекущиеДанные = Неопределено Или Элементы.Папки.ТекущаяСтрока.Пустая() Тогда
		Элементы.ГруппаПояснениеОблачногоСервиса.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	...
	Если Элементы.Папки.ТекущиеДанные.ПапкаСинхронизируется Тогда
		...
		Иначе
			...
			Элементы.ДекорацияПояснение.Заголовок = 
				НСтр("ru = 'Работа с файлами ведется в облачном сервисе.'");
...
Процедура УстановитьДоступностьФайловыхКоманд(Результат = Неопределено, ПараметрыВыполнения = Неопределено) Экспорт
	...
	Если ТекущиеДанные <> Неопределено И Элементы.Список.ТекущаяСтрока <> Неопределено
		...
		УстановитьДоступностьКоманд(ТекущиеДанные, Элементы.Список.ВыделенныеСтроки.Количество() > 1);
	Иначе
...
Процедура УстановитьДоступностьКоманд(Знач ДанныеКоманд, Знач ВыделеноНесколькоСтрок)
	...
	Элементы.ФормаЗакончитьРедактирование.Доступность                 = ИзменениеФайлов И РедактируетТекущийПользователь И Не ФайлыРедактируютсяВОблачномСервисе И Не ФайлПеремещенВАрхив;
	Элементы.СписокКонтекстноеМенюЗакончитьРедактирование.Доступность = ИзменениеФайлов И РедактируетТекущийПользователь И Не ФайлыРедактируютсяВОблачномСервисе И Не ФайлПеремещенВАрхив;
	
	Элементы.ФормаСохранитьИзменения.Доступность                 = ИзменениеФайлов И РедактируетТекущийПользователь И Не ФайлПеремещенВАрхив;
	Элементы.СписокКонтекстноеМенюСохранитьИзменения.Доступность = ИзменениеФайлов И РедактируетТекущийПользователь И Не ФайлПеремещенВАрхив;
	
	Элементы.ФормаОсвободить.Доступность                 = ИзменениеФайлов И ЗначениеЗаполнено(Редактирует) И Не ФайлыРедактируютсяВОблачномСервисе И Не ФайлПеремещенВАрхив;
	Элементы.СписокКонтекстноеМенюОсвободить.Доступность = ИзменениеФайлов И ЗначениеЗаполнено(Редактирует) И Не ФайлыРедактируютсяВОблачномСервисе И Не ФайлПеремещенВАрхив;
	
	Элементы.ФормаЗанять.Доступность                 = ИзменениеФайлов И Не ЗначениеЗаполнено(Редактирует) И НЕ ПодписанЭП И НЕ Служебный И Не ФайлыРедактируютсяВОблачномСервисе И Не ФайлПеремещенВАрхив;
	Элементы.СписокКонтекстноеМенюЗанять.Доступность = ИзменениеФайлов И Не ЗначениеЗаполнено(Редактирует) И НЕ ПодписанЭП И НЕ Служебный И Не ФайлыРедактируютсяВОблачномСервисе И Не ФайлПеремещенВАрхив;
	
	Элементы.ФормаРедактировать.Доступность                 = ИзменениеФайлов И Не ПодписанЭП И Не РедактируетДругой И Не Служебный И Не ФайлПеремещенВАрхив;
	Элементы.СписокКонтекстноеМенюРедактировать.Доступность = ИзменениеФайлов И Не ПодписанЭП И Не РедактируетДругой И Не Служебный И Не ФайлПеремещенВАрхив;
