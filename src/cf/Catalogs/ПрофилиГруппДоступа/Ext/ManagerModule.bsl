	...
	Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
		...
		Если ПоследниеИзменения <> Неопределено Тогда
			Для Каждого ЧастьИзменений Из ПоследниеИзменения Цикл
				Если ТипЗнч(ЧастьИзменений) <> Тип("ФиксированнаяСтруктура") Тогда
					Продолжить;
				КонецЕсли;
				ТекущаяЧастьИзменений = Новый Структура("Удаленные");
				ЗаполнитьЗначенияСвойств(ТекущаяЧастьИзменений, ЧастьИзменений);
				Если ТипЗнч(ТекущаяЧастьИзменений.Удаленные) = Тип("ФиксированныйМассив") Тогда
					Для Каждого Удаленный Из ТекущаяЧастьИзменений.Удаленные Цикл
...
Процедура ПриИзмененииЯзыкаИнформационнойБазы(ИзменениеЯзыков) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"Справочники.ПрофилиГруппДоступа.ПриИзмененииЯзыкаИнформационнойБазы");
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		Наименование = ОписаниеПрофиля.Наименование;
...
Функция ПодготовленныеСтандартныеРолиРасширенийСеанса(ХешСумма) Экспорт
	...
	ДобавитьЭлементВерсии(ОписаниеВерсии, "ОбщиеПрава",
		РолиСИдентификаторами(ОбщиеПрава));
	ДобавитьЭлементВерсии(ОписаниеВерсии, "БазовыеПрава",
		РолиСИдентификаторами(БазовыеПрава));
	ДобавитьЭлементВерсии(ОписаниеВерсии, "БазовыеПраваВнешнихПользователей",
		РолиСИдентификаторами(БазовыеПраваВнешнихПользователей));
	ДобавитьЭлементВерсии(ОписаниеВерсии, "АдминистраторСистемы",
		РолиСИдентификаторами(АдминистраторСистемы));
	ДобавитьЭлементВерсии(ОписаниеВерсии, "ПолныеПрава",
		РолиСИдентификаторами(ПолныеПрава));
	ДобавитьЭлементВерсии(ОписаниеВерсии, "ДополнительныеРолиАдминистратора",
		РолиСИдентификаторами(ДополнительныеРолиАдминистратора));
...
//  СтарыеЗначения - Структура
//  ОбъектМетаданныхПоИдентификатору - см. ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору
//
Процедура ЗарегистрироватьИзменениеРолейПрофилей(Объект, СтарыеЗначения,
			ОбъектМетаданныхПоИдентификатору = Неопределено) Экспорт
	
	Данные = Новый Структура;
	Данные.Вставить("ВерсияСтруктурыДанных", 1);
	Данные.Вставить("ИзменениеРолей", Новый Массив);
	Данные.Вставить("ПредставлениеРолей", Новый Массив);
	Данные.Вставить("ПредставлениеПрофилей", Новый Массив);
	
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.ПрофилиГруппДоступа") Тогда
		АктивностьПрофиляИзменена = Объект.ПометкаУдаления <> СтарыеЗначения.ПометкаУдаления;
		
		ИзменениеРолей = Объект.Роли.Выгрузить();
		ИзменениеРолей.Свернуть("Роль");
		ИзменениеРолей.Колонки.Добавить("ВидИзменения", Новый ОписаниеТипов("Строка"));
		ИзменениеРолей.ЗаполнитьЗначения("Добавлено", "ВидИзменения");
		
		Если ТипЗнч(СтарыеЗначения.Роли) = Тип("РезультатЗапроса")
		   И Не СтарыеЗначения.Роли.Пустой() Тогда
			...
			Пока Индекс > 0 Цикл
				Индекс = Индекс - 1;
				НоваяРоль = ИзменениеРолей.Получить(Индекс);
				СтараяРоль = СтарыеРоли.Найти(НоваяРоль.Роль, "Роль");
				Если СтараяРоль = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если АктивностьПрофиляИзменена Тогда
					НоваяРоль.ВидИзменения = "Изменено";
				Иначе
					ИзменениеРолей.Удалить(НоваяРоль);
				КонецЕсли;
				СтарыеРоли.Удалить(СтараяРоль);
			КонецЦикла;
			Для Каждого СтараяРоль Из СтарыеРоли Цикл
				НоваяСтрока = ИзменениеРолей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтараяРоль);
				НоваяСтрока.ВидИзменения = "Удалено";
			КонецЦикла;
		КонецЕсли;
		
		Если Не АктивностьПрофиляИзменена
		   И Не ЗначениеЗаполнено(ИзменениеРолей) Тогда
			Возврат;
		КонецЕсли;
		
		СериализованнаяСсылкаПрофиля = СериализованнаяСсылка(Объект.Ссылка);
		ИдентификаторыРолей = ИзменениеРолей.ВыгрузитьКолонку("Роль");
		СвойстваРолей = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ИдентификаторыРолей,
			"ПометкаУдаления, Имя, Синоним");
		
		МетаданныеРолей = ОбщегоНазначения.ОбъектыМетаданныхПоИдентификаторам(ИдентификаторыРолей, Ложь);
		
		Для Каждого Строка Из ИзменениеРолей Цикл
			Если Не ЗначениеЗаполнено(Строка.Роль) Тогда
				Продолжить;
			КонецЕсли;
			СериализованнаяСсылкаРоли = СериализованнаяСсылка(Строка.Роль);
			
			Свойства = Новый Структура;
			Свойства.Вставить("Профиль", СериализованнаяСсылкаПрофиля);
			Свойства.Вставить("Роль", СериализованнаяСсылкаРоли);
			Свойства.Вставить("ВидИзменения", Строка.ВидИзменения);
			Данные.ИзменениеРолей.Добавить(Свойства);
			
			МетаданныеРоли = МетаданныеРолей.Получить(Строка.Роль);
			СвойстваРоли = СвойстваРолей.Получить(Строка.Роль);
			ИмяРоли     = Неопределено;
			СинонимРоли = Неопределено;
			
			Если ТипЗнч(МетаданныеРоли) = Тип("ОбъектМетаданных") Тогда
				ИмяРоли     = МетаданныеРоли.Имя;
				СинонимРоли = МетаданныеРоли.Синоним;
			ИначеЕсли СвойстваРоли <> Неопределено Тогда
				ИмяРоли     = СвойстваРоли.Имя;
				СинонимРоли = СвойстваРоли.Синоним;
			КонецЕсли;
			ПометкаУдаленияРоли = ?(СвойстваРоли = Неопределено,
				Неопределено, СвойстваРоли.ПометкаУдаления);
			
			Свойства = Новый Структура;
			Свойства.Вставить("Роль", СериализованнаяСсылкаРоли);
			Свойства.Вставить("ПометкаУдаления", ПометкаУдаленияРоли);
			Свойства.Вставить("ЕстьВМетаданных", ТипЗнч(МетаданныеРоли) = Тип("ОбъектМетаданных"));
			Свойства.Вставить("Имя", ИмяРоли);
			Свойства.Вставить("Синоним", СинонимРоли);
			Свойства.Вставить("СтарыеЗначенияСвойств", Новый Структура);
			Данные.ПредставлениеРолей.Добавить(Свойства);
		КонецЦикла;
		...
		КонецЕсли;
		Данные.ПредставлениеПрофилей.Добавить(Свойства);
	Иначе
		Если Объект.ПометкаУдаления = СтарыеЗначения.ПометкаУдаления Тогда
			Возврат;
		КонецЕсли;
		...
		"ВЫБРАТЬ
		|	ПрофилиГруппДоступа.Ссылка КАК Ссылка,
		|	ПрофилиГруппДоступа.ПометкаУдаления КАК ПометкаУдаления,
		|	ПрофилиГруппДоступа.Представление КАК Представление
		|ИЗ
		|	Справочник.ПрофилиГруппДоступа КАК ПрофилиГруппДоступа
		|ГДЕ
		|	ПрофилиГруппДоступа.Ссылка В
		|			(ВЫБРАТЬ
		|				РолиПрофилей.Ссылка
		|			ИЗ
		|				Справочник.ПрофилиГруппДоступа.Роли КАК РолиПрофилей
		|			ГДЕ
		|				РолиПрофилей.Роль = &Роль)";
		...
		СериализованнаяСсылкаРоли = СериализованнаяСсылка(Объект.Ссылка);
		МетаданныеРоли = ОбъектМетаданныхПоИдентификатору;
		ИмяРоли = ?(ТипЗнч(МетаданныеРоли) = Тип("ОбъектМетаданных"), МетаданныеРоли.Имя, Объект.Имя);
		СинонимРоли = ?(ТипЗнч(МетаданныеРоли) = Тип("ОбъектМетаданных"), МетаданныеРоли.Синоним, Объект.Синоним);
		...
		Если Объект.ПометкаУдаления <> СтарыеЗначения.ПометкаУдаления Тогда
			Свойства.СтарыеЗначенияСвойств.Вставить("ПометкаУдаления", СтарыеЗначения.ПометкаУдаления);
		КонецЕсли;
		Если Объект.Имя <> СтарыеЗначения.Имя Тогда
			Свойства.СтарыеЗначенияСвойств.Вставить("Имя",
				?(ТипЗнч(СтарыеЗначения.Имя) = Тип("Строка"), СтарыеЗначения.Имя, ""));
		КонецЕсли;
		Если Объект.Синоним <> СтарыеЗначения.Синоним Тогда
			Свойства.СтарыеЗначенияСвойств.Вставить("Синоним",
				?(ТипЗнч(СтарыеЗначения.Синоним) = Тип("Строка"), СтарыеЗначения.Синоним, ""));
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			СериализованнаяСсылкаПрофиля = СериализованнаяСсылка(Выборка.Ссылка);
			
			Свойства = Новый Структура;
			Свойства.Вставить("Профиль", СериализованнаяСсылкаПрофиля);
			Свойства.Вставить("Роль", СериализованнаяСсылкаРоли);
			Свойства.Вставить("ВидИзменения", "Изменено");
			Данные.ИзменениеРолей.Добавить(Свойства);
	...
	ИмяСобытия = УправлениеДоступомСлужебный.ИмяСобытияИзменениеРолейПрофилейДляЖурналаРегистрации();
	ДанныеДляЖурнала = ОбщегоНазначения.ЗначениеВСтрокуXML(Данные);
...
#Область ПроцедурыИФункцииДляПоддержкиОбменаДаннымиВРИБ
...
Процедура ВосстановитьСоставРолейРасширений(ЭлементДанных) Экспорт
	...
	ПользователиСлужебный.ПроверитьБезопасныйРежимОтключен(
		"Справочники.ПрофилиГруппДоступа.ВосстановитьСоставРолейРасширений");
...
Процедура ЗарегистрироватьИзменениеПриПолученииДанных(ЭлементДанных) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.ПрофилиГруппДоступа") Тогда
		ЗаполнитьСтандартныеРолиРасширений(ЭлементДанных.Роли);
	КонецЕсли;
	...
	ПользователиСлужебный.ЗарегистрироватьСсылки("ПрофилиГруппДоступа", Профиль);
	
	Если УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
		ПользователиСлужебный.ЗарегистрироватьСсылки("ПрофилиГруппДоступаРоли", ИзмененныеРоли);
	КонецЕсли;
...
Процедура ОбработатьИзменениеЗарегистрированноеПриПолученииДанных() Экспорт
	...
	ИзмененныеПрофили = ПользователиСлужебный.ЗарегистрированныеСсылки("ПрофилиГруппДоступа");
	...
	ПользователиСлужебный.ЗарегистрироватьСсылки("ПрофилиГруппДоступа", Null);
	
	Если Не УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
		Возврат;
	КонецЕсли;
	
	ИзмененныеРоли = ПользователиСлужебный.ЗарегистрированныеСсылки("ПрофилиГруппДоступаРоли");
	Если ИзмененныеРоли.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ПользователиСлужебный.ЗарегистрироватьСсылки("ПрофилиГруппДоступаРоли", Null);
...
Функция ПроверенныеПоставляемыеПрофилиСеанса(СвойстваВидовДоступа = Неопределено, ХешСумма = "") Экспорт
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		ЭтоГруппа = ОписаниеПрофиля.ЭтоПапка;
		...
		Если УправлениеДоступомСлужебный.ОтключеноОграничениеНаУровнеЗаписей() Тогда
			СвойстваПрофиля.ВидыДоступа     = Новый ФиксированноеСоответствие(Новый Соответствие);
			СвойстваПрофиля.ЗначенияДоступа = Новый ФиксированныйМассив(Новый Массив);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Если ИменаПапокПрофилей.Получить(ВРег(ОписаниеПрофиля.Родитель)) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ШаблонОшибки = ?(ОписаниеПрофиля.ЭтоПапка,
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		Если Не ОписаниеПрофиля.ЭтоПапка Тогда
			Продолжить;
		КонецЕсли;
	...
	РазделениеВключено = ОбщегоНазначения.РазделениеВключено();
	ИдентификаторыВсехРолей = ИдентификаторыВсехРолей();
	...
	Для Каждого ОписаниеПрофиля Из СписокПрофилей Цикл
		...
		Если СвойстваПрофиля.ЭтоГруппа Тогда
			Продолжить;
		КонецЕсли;
		ДобавитьСвойстваВерсии(ОписаниеВерсии, СвойстваПрофиля, "Назначение");
		РолиСИдентификаторами = РолиСИдентификаторами(СвойстваПрофиля.Роли, ИдентификаторыВсехРолей);
		ДобавитьСвойстваВерсии(ОписаниеВерсии, Новый Структура("Роли", РолиСИдентификаторами) , "Роли");
...
// Для функции ПоставляемыеПрофили.
Функция ИдентификаторыВсехРолей()
	
	Результат = Новый Соответствие;
	
	Для Каждого Роль Из Метаданные.Роли Цикл
		Результат.Вставить(Роль.Имя,
			Справочники.ИдентификаторыОбъектовМетаданных.КлючОбъектаМетаданныхРоль(Роль));
	КонецЦикла;
...
// Для функции ПоставляемыеПрофили.
Функция РолиСИдентификаторами(ИменаРолей, ИдентификаторыВсехРолей = Неопределено)
	
	Результат = Новый Массив;
	
	Для Каждого ИмяРоли Из ИменаРолей Цикл
		Если ИдентификаторыВсехРолей = Неопределено Тогда
			ИдентификаторРоли = Справочники.ИдентификаторыОбъектовМетаданных.КлючОбъектаМетаданныхРоль(
				Метаданные.Роли[ИмяРоли]);
		Иначе
			ИдентификаторРоли = ИдентификаторыВсехРолей.Получить(ИмяРоли);
		КонецЕсли;
		Результат.Добавить(ИмяРоли + " " + Строка(ИдентификаторРоли));
	КонецЦикла;
...
Функция ПояснениеПоставляемыхПрофилей() Экспорт
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		Если ОписаниеПрофиля.ЭтоПапка Тогда
			ПредставлениеВидовДоступа.Вставить(ОписаниеПрофиля.Идентификатор, "");
		Иначе
