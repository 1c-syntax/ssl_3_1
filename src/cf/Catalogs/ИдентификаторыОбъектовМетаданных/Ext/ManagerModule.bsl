...
Процедура УстановкаПараметровСеанса(ИменаПараметровСеанса, УстановленныеПараметры) Экспорт
	...
	Результат.Вставить("ОбновлениеВыполняется", Ложь);
	   ...
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не может использоваться
			           ...
			           |
			           |Для восстановления связи с главным узлом запустите 1С:Предприятие и нажмите кнопку Восстановить 
					   |или программно установите главный узел, указанный в константе Главный узел.
			           |
			           |Для подтверждения отмены связи с главным узлом 
					   |запустите 1С:Предприятие и нажмите кнопку Отключить 
					   |или программно очистите константу Главный узел.'"),
	...
	Попытка
		...
		Для каждого Объект Из Объекты Цикл
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(СвойстваЗагружаемого.Ссылка) Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Невозможно загрузить идентификаторы объектов метаданных из главного узла.
						           |Для нового элемента не указана ссылка нового:
						           |""%1"".'"),
						Объект.ПолноеИмя);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
			...
			Если Не ЭтоКоллекция(СвойстваЗагружаемого.Ссылка) Тогда
				СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу = ОбъектМетаданныхПоКлючу(
					КлючОбъектаМетаданныхИзХранилища(Объект.КлючОбъектаМетаданных, Объект), "", "", "");
		...
		Для каждого СвойстваЗагружаемого Из ТаблицаЗагружаемых Цикл
			...
			Если ПолныеИменаЗагружаемых.Получить(НРег(Объект.ПолноеИмя)) <> Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно загрузить идентификаторы объектов метаданных из главного узла.
					           |У двух элементов совпадает полное имя:
					           |""%1"".'"),
					Объект.ПолноеИмя);
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
			...
			КлючОбъектаМетаданных = КлючОбъектаМетаданныхИзХранилища(Объект.КлючОбъектаМетаданных, Объект);
			Если ТипЗнч(КлючОбъектаМетаданных) = Тип("Тип")
			   И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
				...
				Если КлючиЗагружаемых.Получить(КлючОбъектаМетаданных) <> Неопределено Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Невозможно загрузить идентификаторы объектов метаданных из главного узла.
						           |У двух элементов совпадает ключ объекта метаданных:
						           |""%1"".'"),
				...
				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу <> СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Невозможно загрузить идентификаторы объектов метаданных из главного узла.
						           |Ключ объекта метаданных ""%1"" не соответствует полному имени ""%2"".'"),
...
// Только для внутреннего использования.
// Параметры:
//  ОбъектМетаданныхРоль - ОбъектМетаданных
// Возвращаемое значение:
//  Строка
//  Неопределено
//
Функция КлючОбъектаМетаданныхРоль(ОбъектМетаданныхРоль) Экспорт
	Возврат КлючРоли(ОбъектМетаданныхРоль, Ложь);
КонецФункции

// Только для внутреннего использования.
// Параметры:
//  ИдентификаторыРоли - Массив из СправочникСсылка.ИдентификаторыОбъектовМетаданных
//                     - Массив из СправочникСсылка.ИдентификаторыОбъектовРасширений
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//   * Ключ - СправочникСсылка.ИдентификаторыОбъектовМетаданных
//          - СправочникСсылка.ИдентификаторыОбъектовРасширений
//   * Значение - см. КлючОбъектаМетаданныхРоль
//
Функция КлючиРолей(ИдентификаторыРоли) Экспорт
	
	Результат = Новый Соответствие;
	Значения = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ИдентификаторыРоли, "КлючОбъектаМетаданных");
	...
	Для Каждого КлючИЗначение Из Значения Цикл
		...
		Если ТипЗнч(КлючИЗначение.Значение) <> Тип("ХранилищеЗначения") Тогда
			Продолжить;
		КонецЕсли;
		КлючРоли = КлючОбъектаМетаданныхИзХранилища(КлючИЗначение.Значение,, КлючИЗначение.Ключ);
		Если ТипЗнч(КлючРоли) <> Тип("Строка")
		 Или Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(КлючРоли) Тогда
			Продолжить;
		КонецЕсли;
		Результат.Вставить(КлючИЗначение.Ключ, КлючРоли);
	КонецЦикла;
...
Процедура ВыполнитьОбновлениеДанных(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
	...
	Если Не ОбновлениеСправочниковИдентификаторов().ОбновлениеВыполняется Тогда
		УстановитьОбновлениеИдентификаторовВыполняется(Истина);
		Попытка
			ВыполнитьОбновлениеДанныхПослеПроверкиЗацикливания(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
				ЕстьКритичныеИзменения, СписокКритичныхИзменений, ОбъектыРасширений);
		Исключение
			УстановитьОбновлениеИдентификаторовВыполняется(Ложь);
			ВызватьИсключение;
		КонецПопытки;
		УстановитьОбновлениеИдентификаторовВыполняется(Ложь);
		Возврат;
	...
	ИменаСлужебныхОбъектов = Новый Массив;
	ИменаСлужебныхОбъектов.Добавить(Метаданные.Справочники.ВерсииРасширений.ПолноеИмя());
	ИменаСлужебныхОбъектов.Добавить(Метаданные.Справочники.ИдентификаторыОбъектовМетаданных.ПолноеИмя());
	ИменаСлужебныхОбъектов.Добавить(Метаданные.Справочники.ИдентификаторыОбъектовРасширений.ПолноеИмя());
	ИменаСлужебныхОбъектов.Добавить(Метаданные.РегистрыСведений.ИдентификаторыОбъектовВерсийРасширений.ПолноеИмя());
	ИменаСлужебныхОбъектов.Добавить(Метаданные.РегистрыСведений.ПараметрыРаботыПрограммы.ПолноеИмя());
	ИменаСлужебныхОбъектов.Добавить(Метаданные.РегистрыСведений.ПараметрыРаботыВерсийРасширений.ПолноеИмя());
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Недопустимое получение идентификаторов объектов метаданных в процессе их обновления.
		           |Следует исключать из подписок на события %1 служебные объекты, в частности:
		           |%2.'"),
		"ПередЗаписью" + "/" + "ПриЗаписи",
		"- " + СтрСоединить(ИменаСлужебныхОбъектов, "," + Символы.ПС + "- "));
	
	ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	
КонецПроцедуры

Процедура ВыполнитьОбновлениеДанныхПослеПроверкиЗацикливания(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
			ЕстьКритичныеИзменения, СписокКритичныхИзменений, ОбъектыРасширений)
...
#Область РеализацияПроцедурОбъявленныхВДругихМодулях
...
Процедура ДобавитьПереименование(Переименования, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	Если Переименования.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		
		СписокДопустимыхТипов = "";
		Для каждого КлючИЗначение Из Переименования.КоллекцииБезКлюча Цикл
	...
	ПорядокБиблиотеки = Переименования.ПорядокБиблиотек[ИдентификаторБиблиотеки];
	Если ПорядокБиблиотеки = Неопределено Тогда
		ПорядокБиблиотеки = Переименования.ПорядокБиблиотек.Количество();
		Переименования.ПорядокБиблиотек.Вставить(ИдентификаторБиблиотеки, ПорядокБиблиотеки);
	КонецЕсли;
	
	ВерсияБиблиотеки = Переименования.ВерсииБиблиотек[ИдентификаторБиблиотеки];
	Если ВерсияБиблиотеки = Неопределено Тогда
		...
		Переименования.ВерсииБиблиотек.Вставить(ИдентификаторБиблиотеки, ВерсияБиблиотеки);
	КонецЕсли;
	...
	Если Результат > 0 Тогда
		...
		ТаблицаПереименования = Переименования.Таблица; // см. ТаблицаПереименованияДляТекущейВерсии
...
#Область ДополнительныеПроцедурыИФункцииДляВызоваИзДругихМодулей
...
Функция ТаблицаПереименованияДляТекущейВерсии() Экспорт
	...
	Переименования.Вставить("Таблица", ТаблицаПереименования);
	Переименования.Вставить("КоллекцииБезКлюча", КоллекцииБезКлюча);
	Переименования.Вставить("ВерсииБиблиотек",  Новый Соответствие);
	Переименования.Вставить("ПорядокБиблиотек", Новый Соответствие);
	
	ОбщегоНазначенияПереопределяемый.ПриДобавленииПереименованийОбъектовМетаданных(Переименования);
	ИнтеграцияПодсистемБСП.ПриДобавленииПереименованийОбъектовМетаданных(Переименования);
...
Процедура ПередЗаписьюОбъекта(Объект) Экспорт
	...
	// АПК:75-выкл проверка ОбменДанными.Загрузка должна быть после обязательных действий.
	ОбъектыРасширений = ЭтоОбъектРасширений(Объект);
	...
	ДополнительныеСвойства = Новый Структура("СтандартныеПодсистемыСтарыеЗначения");
	ЗаполнитьЗначенияСвойств(ДополнительныеСвойства, Объект.ДополнительныеСвойства);
	Если ДополнительныеСвойства.СтандартныеПодсистемыСтарыеЗначения = Неопределено Тогда
		Объект.ДополнительныеСвойства.Вставить("СтандартныеПодсистемыСтарыеЗначения",
			ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Ссылка,
				Новый ФиксированнаяСтруктура(НоваяСтруктураСтарыхЗначений(ОбъектыРасширений))));
	КонецЕсли;
...
Процедура ПриЗаписиОбъекта(Объект) Экспорт
	...
	ДополнительныеСвойства = Новый Структура("СтандартныеПодсистемыОбъектМетаданных", "");
	ЗаполнитьЗначенияСвойств(ДополнительныеСвойства, Объект.ДополнительныеСвойства);
	Если ДополнительныеСвойства.СтандартныеПодсистемыОбъектМетаданных = "" Тогда
		Идентификаторы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
		Если ТипЗнч(Объект.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
			ИдентификаторыКонфигурации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
			ИдентификаторыРасширений = Новый Массив;
		Иначе
			ИдентификаторыКонфигурации = Новый Массив;
			ИдентификаторыРасширений = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
		КонецЕсли;
		Попытка
			ОбъектыМетаданных = ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
				ИдентификаторыКонфигурации, ИдентификаторыРасширений, Ложь);
			ОписаниеОбъектаМетаданных = ОбъектыМетаданных.Получить(Объект.Ссылка);
		Исключение
			ОписаниеОбъектаМетаданных = Null;
		КонецПопытки;
		
		Объект.ДополнительныеСвойства.Вставить("СтандартныеПодсистемыОбъектМетаданных",
			?(ТипЗнч(ОписаниеОбъектаМетаданных) = Тип("Структура"),
				ОписаниеОбъектаМетаданных.Объект, ОписаниеОбъектаМетаданных));
	КонецЕсли;
	
	СтарыеЗначения = Объект.ДополнительныеСвойства.СтандартныеПодсистемыСтарыеЗначения;
	
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()
	   И ПользователиСлужебный.РегистрироватьИзмененияПравДоступа()
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом")
	   И (ЭтоКлючРоли(КлючОбъектаМетаданныхИзХранилища(Объект.КлючОбъектаМетаданных, Объект))
	      Или ЭтоКлючРоли(КлючОбъектаМетаданныхИзХранилища(СтарыеЗначения.КлючОбъектаМетаданных, СтарыеЗначения))) Тогда
		
		МодульПрофилиГруппДоступа = ОбщегоНазначения.ОбщийМодуль("Справочники.ПрофилиГруппДоступа");
		МодульПрофилиГруппДоступа.ЗарегистрироватьИзменениеРолейПрофилей(Объект,
			СтарыеЗначения, Объект.ДополнительныеСвойства.СтандартныеПодсистемыОбъектМетаданных);
	КонецЕсли;
...
Процедура ФормаСпискаПриСозданииНаСервере(Форма) Экспорт
	...
	Иначе
		Форма.Элементы.Список.РежимВыбора = Ложь;
	КонецЕсли;
	
	Форма.ВыбиратьГруппыОбъектовМетаданных = Форма.Параметры.ВыбиратьГруппыОбъектовМетаданных;
...
Процедура ФормаЭлементаПриСозданииНаСервере(Форма) Экспорт
	...
	ОбъектыРасширений = ЭтоОбъектРасширений(Форма.Объект);
		...
		Если Не ЗначениеЗаполнено(Строка.Ссылка) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Данные справочника %1 повреждены.
				           |В конфигураторе в меню ""Администрирование"" выполните ""Тестирование и исправление..."".'"),
				?(ОбъектыРасширений, "ИдентификаторыОбъектовРасширений", "ИдентификаторыОбъектовМетаданных"));
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		Строка.КлючОбъектаМетаданных = КлючОбъектаМетаданныхИзХранилища(Строка.ХранилищеКлюча, Строка);
...
// Параметры:
//  ХранилищеПараметров - ХранилищеЗначения, Неопределено
//  Объект - Неопределено - в этом случае указан параметр Ссылка.
//         - Структура
...
//
Функция КлючОбъектаМетаданныхИзХранилища(ХранилищеКлюча, Объект = Неопределено, Ссылка = Неопределено)
	
	Если ТипЗнч(ХранилищеКлюча) <> Тип("ХранилищеЗначения") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Попытка
		Возврат ХранилищеКлюча.Получить();
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	КонецПопытки;
	
	ИменаСвойств = "Ссылка, ПолноеИмя, ПометкаУдаления, ИмяРасширения";
	Если Объект = Неопределено Тогда
		Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
			ИменаСвойств = "Ссылка, ПолноеИмя, ПометкаУдаления";
		КонецЕсли;
		Свойства = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, ИменаСвойств);
		Свойства.Ссылка = Ссылка;
	Иначе
		Свойства = Новый Структура(ИменаСвойств);
		Свойства.Вставить("ПолноеИмяКоллекции");
		ЗаполнитьЗначенияСвойств(Свойства, Объект);
		Если ТипЗнч(Свойства.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений")
		   И Не ЗначениеЗаполнено(Свойства.ПолноеИмя)
		   И ЗначениеЗаполнено(Свойства.ПолноеИмяКоллекции) Тогда
			Свойства.ПолноеИмя = Свойства.ПолноеИмяКоллекции;
		КонецЕсли;
	КонецЕсли;
	
	ОписаниеИдентификатора = Новый Массив;
	
	Если ТипЗнч(Свойства.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
	 Или ТипЗнч(Свойства.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		ОписаниеИдентификатора.Добавить("Ссылка" + ": " + ПолучитьНавигационнуюСсылку(Свойства.Ссылка));
	КонецЕсли;
	ОписаниеИдентификатора.Добавить("ПолноеИмя" + ": """ + Свойства.ПолноеИмя + """");
	ОписаниеИдентификатора.Добавить("ПометкаУдаления" + ": " + XMLСтрока(Свойства.ПометкаУдаления));
	
	Если ТипЗнч(Свойства.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		ИмяСобытия = НСтр("ru = 'Идентификаторы объектов расширений.Ключ объекта метаданных'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеИдентификатора.Добавить("ИмяРасширения" + ": """ + Свойства.ИмяРасширения + """");
	Иначе
		ИмяСобытия = НСтр("ru = 'Идентификаторы объектов метаданных.Ключ объекта метаданных'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	ОписаниеИдентификатора.Добавить("КлючОбъектаМетаданных" + ": " + ЗначениеВСтрокуВнутр(ХранилищеКлюча));
	
	КомментарийДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление идентификаторов продолжается корректно.
		           |
		           |При обработке идентификатора:
		           |- %1
		           |
		           |Произошла неблокирующая ошибка (в момент извлечения ключа объекта метаданных из хранилища значения):
		           |%2'"),
		СтрСоединить(ОписаниеИдентификатора, Символы.ПС + "- "),
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	
	ЗаписьЖурналаРегистрации(ИмяСобытия,
		УровеньЖурналаРегистрации.Предупреждение, , , КомментарийДляЖурнала,
...
Процедура ОбновитьИдентификаторыОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений,
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если Не Свойства.ЭтоНовый И Не Свойства.Обновлен Тогда
			Продолжить;
		КонецЕсли;
		...
		СтарыеЗначения = НоваяСтруктураСтарыхЗначений(ОбъектыРасширений);
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(СтарыеЗначения, ТаблицаОбъект);
		КонецЕсли;
		...
		ТаблицаОбъект.ДополнительныеСвойства.Вставить("СтандартныеПодсистемыСтарыеЗначения", СтарыеЗначения);
		ТаблицаОбъект.ДополнительныеСвойства.Вставить("СтандартныеПодсистемыОбъектМетаданных",
			?(ТипЗнч(Свойства.ОбъектМетаданных) = Тип("ОбъектМетаданных"), Свойства.ОбъектМетаданных, Null));
...
Процедура ОбновитьСвойстваПомеченногоНаУдаление(Свойства, СвойстваОбновлены = Ложь, ЕстьУдаленные = Ложь, ЭтоДубль = Ложь)
	...
	ОбъектыРасширений = ЭтоОбъектРасширений(Свойства);
	
	Если ЭтоДубль Тогда
		КлючОбъектаМетаданных = ?(ТипЗнч(Свойства.КлючОбъектаМетаданных) = Тип("ХранилищеЗначения"),
			КлючОбъектаМетаданныхИзХранилища(Свойства.КлючОбъектаМетаданных, Свойства),
			Свойства.КлючОбъектаМетаданных);
	КонецЕсли;
	...
	Если НЕ Свойства.ПометкаУдаления
	 ...
	 ИЛИ ЭтоДубль
	   И КлючОбъектаМетаданных <> Неопределено Тогда
		...
		Свойства.Родитель              = ПустаяСсылкаСправочника(ЭтоОбъектРасширений(Свойства));
...
Функция КлючиОбъектовМетаданныхСовпадают(Свойства, Объект)
	...
	Возврат Свойства.КлючОбъектаМетаданных = КлючОбъектаМетаданныхИзХранилища(Объект.КлючОбъектаМетаданных, Объект);
...
Функция КлючОбъектаМетаданныхСоответствуетПолномуИмени(СвойстваИдентификатора)
	...
	КлючОбъектаМетаданных = КлючОбъектаМетаданныхИзХранилища(СвойстваИдентификатора.КлючОбъектаМетаданных,
		СвойстваИдентификатора);
	ОбъектыРасширений = ЭтоОбъектРасширений(СвойстваИдентификатора);
...
Функция ЭтоКлючРоли(КлючОбъектаМетаданных)
	Возврат ТипЗнч(КлючОбъектаМетаданных) = Тип("Строка")
		И СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(КлючОбъектаМетаданных);
КонецФункции

Функция КлючРоли(ОбъектМетаданныхРоль, ВызыватьИсключение = Истина)
...
Процедура ДобавитьСвойстваОбъектовМетаданных(Знач КоллекцияОбъектовМетаданных,
	...
	Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
		...
		Синоним = ?(ЗначениеЗаполнено(ОбъектМетаданных.Синоним),
			ОбъектМетаданных.Синоним, ОбъектМетаданных.Имя);
		
		ПолныйСиноним = ПолныйСинонимРодителя
			+ СвойстваКоллекции.СинонимВЕдЧисле + ". " + Синоним;
		...
		Если ЗначениеЗаполнено(ИмяРасширения) = ОбъектыРасширений Тогда
			...
			НоваяСтрока.ПолныйСиноним     = ПолныйСиноним;
		...
		Если СвойстваКоллекции.Имя = "Подсистемы"
		   И (ОбъектыРасширений Или Не ЗначениеЗаполнено(ИмяРасширения)) Тогда
...
Функция ЭтоОбъектРасширений(ОписаниеОбъекта)
	
	Возврат ТипЗнч(ОписаниеОбъекта.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений");
...
Функция НоваяСтруктураСтарыхЗначений(ОбъектыРасширений)
	
	Результат = Новый Структура("ПометкаУдаления, Имя, Синоним, КлючОбъектаМетаданных, Ссылка, ПолноеИмя");
...
Функция ИдентификаторыОбъектовМетаданныхСПопыткойПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент)
	...
	Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(
		ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент, Истина);
	...
	Если Идентификаторы = Неопределено Тогда
		...
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(
			ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент);
	КонецЕсли;
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных,
			ВызыватьИсключение, ОдинЭлемент, ПерваяПопытка = Ложь)
	...
	Контекст = Новый Структура("КонфигурацияБазыДанныхИзмененаДинамически");
	...
	Для Каждого ПолноеИмяОбъектаМетаданных Из ПолныеИменаОбъектовМетаданных Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если КонфигурацияБазыДанныхИзмененаДинамическиИзКонтекста(Контекст) Тогда
		...
		Если РезультатПроверки.НеСоответствует Тогда
			НазваниеСправочника = НазваниеСправочника(ЭтоОбъектРасширений(СтрокаТаблицы));
	...
	Если ПерваяПопытка И ДобавитьУточнениеОшибкиПараметровРаботыПрограммыДляРазработчика Тогда
		Возврат Неопределено;
	КонецЕсли;
...
// Для функций ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора,
// ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора.
//
Функция КонфигурацияБазыДанныхИзмененаДинамическиИзКонтекста(Контекст)
	
	Если Контекст.КонфигурацияБазыДанныхИзмененаДинамически = Неопределено Тогда
		Контекст.КонфигурацияБазыДанныхИзмененаДинамически = КонфигурацияБазыДанныхИзмененаДинамически();
	КонецЕсли;
	
	Возврат Контекст.КонфигурацияБазыДанныхИзмененаДинамически;
...
Функция ИдентификаторыИзКлючей()
	...
	Пока Выборка.Следующий() Цикл
		Если ТипЗнч(Выборка.ХранилищеКлюча) <> Тип("ХранилищеЗначения") Тогда
			Продолжить;
		КонецЕсли;
		КлючОбъектаМетаданных = КлючОбъектаМетаданныхИзХранилища(Выборка.ХранилищеКлюча, Выборка);
...
//   * ИдентификаторыОбъектовМетаданных - Булево
//   * ИдентификаторыОбъектовРасширений - Булево
//   * ОбновлениеВыполняется - Булево
//
...
Процедура УстановитьОбновлениеИдентификаторовВыполняется(ОбновлениеВыполняется)
	
	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);
	
	ОбновлениеСправочников = Новый Структура(ПараметрыСеанса.ОбновлениеСправочниковИдентификаторов);
	
	ОбновлениеСправочников.ОбновлениеВыполняется = ОбновлениеВыполняется;
	
	ПараметрыСеанса.ОбновлениеСправочниковИдентификаторов = Новый ФиксированнаяСтруктура(ОбновлениеСправочников);
...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, ВызыватьИсключение)
	...
	Если ИдентификаторыКонфигурации.Количество() > 0 Или ИдентификаторыРасширений.Количество() > 0 Тогда
		ПерваяПопытка = Не ОбщегоНазначения.РазделениеВключено()
			Или Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных();
		
		ОбъектыМетаданных = ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
			ИдентификаторыКонфигурации, ИдентификаторыРасширений, ВызыватьИсключение, ПерваяПопытка);
	Иначе
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
			ИдентификаторыКонфигурации, ИдентификаторыРасширений, ВызыватьИсключение, ПерваяПопытка = Ложь)
	...
	Если Выгрузка.Количество() < ВсегоИдентификаторов Тогда
		...
		Если ВызыватьИсключение Тогда
			Если ИдентификаторыРасширений.Найти(Идентификатор) = Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Объект ""%1"" удален в этой версии приложения.
					           |Связанные с ним данные и настройки также более недоступны.'"),
					Строка(Идентификатор));
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Объект расширения ""%1"" удален,
					           |т.к. вместе с ним удалено расширение или объект удален из текущей версии расширения.
					           |Связанные с ним данные и настройки также более недоступны.'"),
					Строка(Идентификатор));
			КонецЕсли;
			ВызватьИсключение ТекстОшибки;
	...
	Для Каждого Свойства Из Выгрузка Цикл
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				...
				Если Свойства.ОбъектРасширений Тогда
					...
					Если УстановленныеРасширения.Количество() = 0 Тогда
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Объект ""%2"" не существует, т.к. удалено расширение ""%1"".
							           |Связанные с ним данные и настройки более недоступны.'"),
							ИмяРасширения,
							ПредставлениеИдентификатора);
						
					ИначеЕсли Не УстановленныеРасширения[0].Активно Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение ""%1"" установлено, но выключено.
					...
					ИначеЕсли ОтключенныеРасширения.Количество() > 0 И Не ОтключенныеРасширения[0].Активно
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение ""%1"" установлено после запуска сеанса, поэтому не подключено.
							           |Перезапустите сеанс.'"),
							ИмяРасширения);
						
					ИначеЕсли ОтключенныеРасширения.Количество() > 0 И ОтключенныеРасширения[0].Активно Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение ""%1"" установлено, но отключено при запуске сеанса.
							           |Это значит, что при его подключении возникла непредвиденная ситуация.'"),
							ИмяРасширения);
					...
					Иначе // АктивныеРасширения.Количество() > 0
						...
						Если РезультатПроверки.ОбъектМетаданныхУдаленного <> Неопределено
						   ...
						   И РезультатПроверки.ОбъектМетаданныхУдаленного.РасширениеКонфигурации().Имя = АктивныеРасширения[0].Имя Тогда
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Идентификатор объекта метаданных ""%2"" расширения ""%1"" помечен на удаление.
								           |Обычно это значит, что расширение было удалено и установлено заново (вместо обновления расширения на новую версию).
								           |Связанные с ним данные и настройки более недоступны.'"),
								ИмяРасширения,
								ПредставлениеИдентификатора);
						Иначе
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Объект ""%2"" удален в текущей версии расширения ""%1"".
								           |Связанные с ним данные и настройки более недоступны.'"),
								ИмяРасширения,
								ПредставлениеИдентификатора);
						КонецЕсли;
				...
				ИначеЕсли КонфигурацияБазыДанныхИзмененаДинамическиИзКонтекста(Контекст) Тогда
				...
				ИначеЕсли РезультатПроверки.ОбъектМетаданныхУдаленного <> Неопределено
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатор объекта метаданных ""%1"" помечен на удаление.
						           |Это значит, что объект метаданных был удален из конфигурации, а позже снова добавлен.
						           |Связанные с ним данные и настройки более недоступны.'"),
						ПредставлениеИдентификатора);
					ВызватьИсключение ТекстОшибки;
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект метаданных ""%1"" не существует, т.к. удален в этой версии приложении.
						           |Связанные с ним данные и настройки более недоступны.'"),
						ПредставлениеИдентификатора);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
		...
		Если Не Свойства.ОбъектРасширений
		   И Свойства.ПометкаУдаления
		   И Не КонфигурацияБазыДанныхИзмененаДинамическиИзКонтекста(Контекст) Тогда
			
			Если ПерваяПопытка Тогда
				Возврат Неопределено;
			КонецЕсли;
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1"" из справочника ""%2"" помечен на удаление.'"),
				Свойства.Представление,
				НазваниеСправочника(Ложь));
...
#Область ПроцедурыИФункцииЗаменыИдентификатораВБазеДанных
			...
			Если ОбрабатываемаяСсылка <> СтрокаТаблицы.Данные Тогда
				Если ОбрабатываемаяСсылка <> Неопределено И Параметры.Объект <> Неопределено Тогда
					...
					Исключение
						...
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не удалось записать объект ""%1"" по причине:
							           |%2'"),
				...
				Для Каждого Движение Из СтрокаТаблицы.Метаданные.Движения Цикл
					...
					Если НадоЗаписывать Тогда
						...
						Исключение
							...
							ИнформацияОбОшибке = ИнформацияОбОшибке();
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Не удалось записать движения объекта ""%1"" в ""%2"" по причине:
								           |%3'"),
								ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
						...
						Если НаборЗаписи.Количество() > 0 Тогда
							...
							Если НадоЗаписывать Тогда
								...
								Исключение
									...
									ИнформацияОбОшибке = ИнформацияОбОшибке();
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'Не удалось записать данные по регистратору ""%1"" в ""%2"" по причине:
											           |%3'"),
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				Попытка
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не удалена запись ""%1"" по причине:
							           |%2'"),
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не удалось добавить запись ""%1"" по причине:
							           |%2'"),
							ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(СтруктураИзмерений)),
							ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
