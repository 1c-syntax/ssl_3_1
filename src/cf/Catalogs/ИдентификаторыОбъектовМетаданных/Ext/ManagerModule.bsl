...
Функция МетаданныеРасширенияНайтиПоПолномуИмени(Свойства)
	...
	Если НРег(РасширениеОбъекта.УникальныйИдентификатор) = НРег(Сред(Свойства.ИдентификаторРасширения, 3))
	 Или НРег(РасширениеОбъекта.Имя) = НРег(Сред(Свойства.ИмяРасширения, 3))
	   И Base64Строка(РасширениеОбъекта.ХешСумма) = Сред(Свойства.ХешСуммаРасширения, 3) Тогда
...
Функция ИдентификаторыКлючейРасширений()
...
Процедура ОбновитьСвойстваПомеченногоНаУдаление(Свойства, СвойстваОбновлены = Ложь, ЕстьУдаленные = Ложь, ЭтоДубль = Ложь)
	...
	Если ЭтоДубль Тогда
		Если ТипЗнч(Свойства.КлючОбъектаМетаданных) = Тип("ХранилищеЗначения") Тогда
			КлючОбъектаМетаданныхДубля = Новый ХранилищеЗначения(Неопределено);
		Иначе
			КлючОбъектаМетаданныхДубля = Неопределено;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ Свойства.ПометкаУдаления
	 ...
	 ИЛИ ЭтоДубль
	   И XMLСтрока(Свойства.КлючОбъектаМетаданных) <> XMLСтрока(КлючОбъектаМетаданныхДубля) Тогда
		...
		Если ЭтоДубль Тогда
			Свойства.КлючОбъектаМетаданных = КлючОбъектаМетаданныхДубля;
		КонецЕсли;
...
Функция КлючОбъектаМетаданных(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = ВРег("Роль") Тогда
		Возврат КлючРоли(Метаданные.Роли[ИмяОМ]);
...
Функция ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, ИмяРасширения, ИдентификаторРасширения, ХешСуммаРасширения)
	...
	Если НРег(РасширениеОбъекта.УникальныйИдентификатор) = НРег(ИдентификаторРасширения)
	 ...
	 Или НРег(РасширениеОбъекта.Имя) = НРег(ИмяРасширения)
	   И Base64Строка(РасширениеОбъекта.ХешСумма) = ХешСуммаРасширения
	 Или НРег(РасширениеОбъекта.Имя) = НРег(Сред(ИмяРасширения, 3))
	   И Base64Строка(РасширениеОбъекта.ХешСумма) = Сред(ХешСуммаРасширения, 3) Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				...
				Если Свойства.ОбъектРасширений Тогда
					...
					Если СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ИдентификаторРасширения) Тогда
						...
						Если АктивныеРасширения.Количество() > 0 Тогда
							ИмяРасширения = УстановленныеРасширения[0].Имя;
						ИначеЕсли ОтключенныеРасширения.Количество() > 0 Тогда
							ИмяРасширения = УстановленныеРасширения[0].Имя;
						ИначеЕсли УстановленныеРасширения.Количество() > 0 Тогда
							ИмяРасширения = УстановленныеРасширения[0].Имя;
						КонецЕсли;
					КонецЕсли;
