...
Функция ОбъектыМетаданныхПоИдентификаторам(Идентификаторы, ВызыватьИсключение) Экспорт
	...
	Для Каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли СтрНачинаетсяС(Описание.Ключ, "Роль.") Тогда
			Результат.Вставить(Идентификатор, Метаданные.Роли.Найти(СтрРазделить(Описание.Ключ, ".")[1]));
		Иначе
...
Функция КлючОбъектаМетаданныхСоответствуетПолномуИмени(СвойстваИдентификатора)
	...
	Если ОбъектМетаданных = Неопределено
	   ...
	   И СтрНачинаетсяС(СвойстваИдентификатора.ПолноеИмяКоллекции, "? ") Тогда
		...
		РезультатПроверки.ОбъектМетаданныхУдаленного = Метаданные.НайтиПоПолномуИмени(
...
Функция МетаданныеНайтиПоПолномуИмени(ПолноеИмя)
	...
	Если СтрНачинаетсяС(ПолноеИмя, "Роль.") Тогда
		ОбъектМетаданных = Метаданные.Роли.Найти(СтрРазделить(ПолноеИмя, ".")[1]);
	Иначе
	...
	Если ВРег(ПолноеИмяБезИменКоллекций(ПолноеИмяОбъекта))
	  <> ВРег(ПолноеИмяБезИменКоллекций(ПолноеИмя)) Тогда
