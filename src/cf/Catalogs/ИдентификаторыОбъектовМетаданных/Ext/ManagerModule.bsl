...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных, ВызыватьИсключение) Экспорт
	...
	Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректное значение параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных.
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный тип параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных:
			           |""%1"".'"),
	...
	Идентификатор = ИдентификаторыОбъектовМетаданных(Массив,
		ВызыватьИсключение, Истина).Получить(ПолноеИмяОбъектаМетаданных);
...
Функция ИдентификаторыОбъектовМетаданных(ОписаниеОбъектовМетаданных, ВызыватьИсключение = Истина, ОдинЭлемент = Ложь) Экспорт
	...
	Для Каждого ОписаниеОбъектаМетаданных Из ОписаниеОбъектовМетаданных Цикл
		...
		Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
			...
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректное значение параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
					           |Указан несуществующий объект метаданных: ""%1"".'"),
					ОписаниеОбъектаМетаданных);
			КонецЕсли;
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректный тип параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
				           |""%1""'"),
				ТипОписанияОбъектаМетаданных);
		КонецЕсли;
...
//    ** Ссылка - СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений -  
//    
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
	...
	СвойстваРасширений.Вставить("ИменаПодключенныхРасширений",
		ИменаРасширений(ИсточникРасширенийКонфигурации.СеансАктивные));
	СвойстваРасширений.Вставить("ИменаНеподключенныхРасширений",
		ИменаРасширений(ИсточникРасширенийКонфигурации.СеансОтключенные));
...
//   * Родитель - СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений -
...
//   * НоваяСсылка - СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений -
...
Функция МетаданныеРасширенияНайтиПоПолномуИмени(Свойства)
	...
	Если РасширениеОбъекта = Неопределено
	 Или РасширениеОбъекта.Имя <> Сред(Свойства.ИмяРасширения, 3)
	 Или Base64Строка(РасширениеОбъекта.ХешСумма) <> Сред(Свойства.ХешСуммаРасширения, 3) Тогда
	...
	Если Не ОбъектыРасширений Или ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) Тогда
		Для каждого СвойстваКоллекции Из СвойстваКоллекций Цикл
			ДобавитьСвойстваОбъектовМетаданных(Метаданные[СвойстваКоллекции.Имя], СвойстваКоллекции,
				СвойстваОбъектовМетаданных, ОбъектыРасширений, ИменаПредопределенных);
		КонецЦикла;
...
Функция ИдентификаторыОбъектовМетаданныхСПопыткойПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент)
	...
	Попытка
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(
			ПолныеИменаОбъектовМетаданных, Истина, ОдинЭлемент);
	Исключение
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент)
	...
	Если ДоступныИдентификаторыОбъектовРасширений Тогда
		...
		|	И ВерсииИдентификаторов.ПолноеИмяОбъекта В (&ПолныеИмена)";
	...
	Если КоличествоОшибок > 0 Тогда
		...
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных().'");
		Иначе
			ЗаголовокОшибки = НСтр("ru = 'Ошибки при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных().'");
		КонецЕсли;
...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, ВызыватьИсключение)
	...
	Для Каждого ТекущийИдентификатор Из Идентификаторы Цикл
		Если ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
			...
			КонецЕсли;
		ИначеЕсли ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
	...
	Если ИдентификаторыРасширений.Количество() > 0 Тогда
		...
		|ГДЕ
		|	ВерсииИдентификаторов.ВерсияРасширений = &ВерсияРасширений
		|	И Идентификаторы.Ссылка В(&ИдентификаторыРасширений)";
	КонецЕсли;
	...
	Если Выгрузка.Количество() < ВсегоИдентификаторов Тогда
		...
		Если ВызыватьИсключение Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				НазваниеСправочника(ИдентификаторыРасширений.Найти(Идентификатор) <> Неопределено));
	...
	ВерсияПрограммыОбновленаДинамически = СтандартныеПодсистемыСервер.ВерсияПрограммыОбновленаДинамически();
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если РезультатПроверки.НеСоответствует Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				...
				Если Свойства.ОбъектРасширений И Не Свойства.ПометкаУдаления Тогда
					// Возможно расширение не удалось подключить.
					Если ВызыватьИсключение Тогда
						Отбор = Новый Структура("Имя", Свойства.ИмяРасширения);
						Если РасширенияКонфигурации.Получить(Отбор, ИсточникРасширенийКонфигурации.СеансОтключенные).Количество() > 0 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								...
								Свойства.ИмяРасширения);
						Иначе
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Расширение конфигурации %1 установлено, но не подключено.
				...
				ИначеЕсли Не Свойства.ОбъектРасширений И ВерсияПрограммыОбновленаДинамически Тогда
				...
				Иначе
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						НазваниеСправочника(Свойства.ОбъектРасширений));
				КонецЕсли;
			ИначеЕсли ВерсияПрограммыОбновленаДинамически Тогда
		...
		Если Не Свойства.ОбъектРасширений И Свойства.ПометкаУдаления И Не ВерсияПрограммыОбновленаДинамически Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1""
...
// Для процедуры ОбработкаПолученияПредставления.
Функция ПредставлениеИдентификатора(Ссылка) Экспорт
	...
	Если ОбъектыРасширений Тогда
		...
		КонецЕсли;
	Иначе
		ПолноеИмя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ПолноеИмя");
	КонецЕсли;
