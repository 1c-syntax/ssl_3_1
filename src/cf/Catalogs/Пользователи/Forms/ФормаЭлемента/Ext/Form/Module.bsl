...
&НаКлиенте
Перем ПропуститьНаименованиеПриИзменении, ПропуститьНаименованиеАвтоПодбор;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Параметры.ГруппаНовогоПользователя <> Пользователи.ГруппаВсеПользователи() Тогда
	...
	Иначе
		...
		ПрочитатьПользователяИБ();
	КонецЕсли;
	ЗаполнитьЗначенияПрямогоИзменения();
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		ВидКонтактнойИнформацииEmailПользователя   = ВидКонтактнойИнформации("EmailПользователя");
		ВидКонтактнойИнформацииТелефонПользователя = ВидКонтактнойИнформации("ТелефонПользователя");
		
		ИсключаемыеВиды = Новый Массив;
		ИсключаемыеВиды.Добавить(ВидКонтактнойИнформацииEmailПользователя);
		ИсключаемыеВиды.Добавить(ВидКонтактнойИнформацииТелефонПользователя);
	...
	Иначе
		Элементы.ГруппаКонтактнаяИнформацияПолеEmailПользователя.Видимость = Ложь;
		Элементы.ГруппаКонтактнаяИнформацияПолеТелефонПользователя.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("ПочтаДляВосстановленияПароляУстанавливаетсяВФорме");
	...
	Если ФотографияЗадана И ЭтоАдресВременногоХранилища(АдресФотографии) Тогда
		ТекущийОбъект.Фотография = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(АдресФотографии), Новый СжатиеДанных(9));
	Иначе 
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Обсуждения")
	   И ЗначениеЗаполнено(Объект.ИдентификаторПользователяИБ) Тогда
		...
		Для Каждого Элемент Из ЭлементыДерева Цикл
			...
			Если Элемент.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Несуществующая роль ""%1"".'"), Элемент.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Элемент),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Несуществующая роль ""%1"" в строке %2.'"), Элемент.Синоним, "%1"));
			КонецЕсли;
			Если Элемент.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"" недоступна для пользователей.'"), Элемент.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Элемент),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"" в строке %2 недоступна для пользователей.'"), Элемент.Синоним, "%1"));
			КонецЕсли;
		КонецЦикла;
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если ПропуститьНаименованиеПриИзменении = Истина Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура НаименованиеАвтоПодбор(Элемент, Текст, ДанныеВыбора,
			ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	Если Не ЗначениеЗаполнено(Текст)
	 Или Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПодборФИО")
	 Или ПропуститьНаименованиеАвтоПодбор = Истина Тогда
		ПропуститьНаименованиеАвтоПодбор = Ложь;
		Возврат;
	КонецЕсли;
	МодульПодборФИОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодборФИОКлиент");
	
	ДанныеФИО = Новый Структура("Представление", Текст);
	ДанныеКлассификатора = МодульПодборФИОКлиент.Подобрать("ФИО", ДанныеФИО);
	
	Если ДанныеКлассификатора.Количество() > 0 Тогда
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = Новый СписокЗначений;
		ДанныеВыбора.ЗагрузитьЗначения(ДанныеКлассификатора);
	КонецЕсли;
...
Процедура НаименованиеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПропуститьНаименованиеПриИзменении = Истина;
	
	ДанныеНаименования = СтрРазделить(ВыбранноеЗначение, " ", Ложь);
	Элементы.Наименование.ОбновитьТекстРедактирования();
	Если ДанныеНаименования.Количество() < 3 Тогда
		НовоеЗначение = ВыбранноеЗначение + " ";
	Иначе
		НовоеЗначение = ВыбранноеЗначение;
		ПропуститьНаименованиеАвтоПодбор = Истина;
	КонецЕсли;
	Элементы.Наименование.ВыделенныйТекст = НовоеЗначение;
	Если Объект.Наименование <> НовоеЗначение Тогда
		Объект.Наименование = ВыбранноеЗначение;
	КонецЕсли;
	
	ПропуститьНаименованиеПриИзменении = Ложь;
...
Процедура НедействителенПриИзменении(Элемент)
	...
	Если РазделениеВключено
	   И Не Объект.Недействителен Тогда
		ЗначенияПрямогоИзменения.ВходВПрограммуРазрешен = Истина;
	КонецЕсли;
	...
	Если Объект.Недействителен Тогда
		...
		Если Не ПользовательИБАутентификацияOpenID
		   ...
		   И Не ЗначенияПрямогоИзменения.АутентификацияСтандартная
		   И ПользовательИБАутентификацияСтандартная Тогда
			
			ПользовательИБАутентификацияСтандартная = Ложь;
		КонецЕсли;
	ИначеЕсли ЗначенияПрямогоИзменения.ВходВПрограммуРазрешен Тогда
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	...
	Если Объект.ПометкаУдаления И ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'Чтобы разрешить вход в приложение, снимите пометку на удаление с этого пользователя.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не УровеньДоступа.ИзменениеРазрешенияНаВход
	   И Не ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'После записи вход в приложение сможет разрешить только администратор.'"));
	КонецЕсли;
	
	ЗначенияПрямогоИзменения.ВходВПрограммуРазрешен = ВходВПрограммуРазрешен;
...
Процедура ПользовательИБИмяПриИзменении(Элемент)
	...
	ЗначенияПрямогоИзменения.Имя = ПользовательИБИмя;
...
Процедура ПользовательИБАутентификацияСтандартнаяПриИзменении(Элемент)
	...
	Если ПользовательИБАутентификацияСтандартная Тогда
		ПотребоватьСменуПароляПриВходе = ПотребоватьСменуПароляПриВходеБыло;
	Иначе
		ПотребоватьСменуПароляПриВходеБыло = ПотребоватьСменуПароляПриВходе;
		ОбновитьПотребоватьСменуПароляПриВходе();
	КонецЕсли;
	ОбновитьПользовательИБЗапрещеноВосстанавливатьПароль();
	
	АутентификацияПриИзменении();
	ЗначенияПрямогоИзменения.АутентификацияСтандартная = ПользовательИБАутентификацияСтандартная;
...
Процедура ПотребоватьСменуПароляПриВходеПриИзменении(Элемент)
	...
	ОбновитьПользовательИБЗапрещеноИзменятьПароль();
	ОбновитьПользовательИБЗапрещеноВосстанавливатьПароль();
	
	ЗначенияПрямогоИзменения.ПотребоватьСменуПароляПриВходе = ПотребоватьСменуПароляПриВходе;
...
Процедура ПользовательИБЗапрещеноИзменятьПарольПриИзменении(Элемент)
	...
	ЗначенияПрямогоИзменения.ЗапрещеноИзменятьПароль =
		ПользовательИБЗапрещеноИзменятьПароль;
...
&НаКлиенте
Процедура ПользовательИБЗапрещеноВосстанавливатьПарольПриИзменении(Элемент)
	
	ЗначенияПрямогоИзменения.ЗапрещеноВосстанавливатьПароль =
		ПользовательИБЗапрещеноВосстанавливатьПароль;
...
#Область ПоддержкаКонтактнойИнформации
...
Процедура Подключаемый_КонтактнаяИнформацияПриИзменении(Элемент)
	...
	Если Не ВозможноИзменитьЗначениеКонтактнойИнформации(Элемент) Тогда
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_КонтактнаяИнформацияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ВозможноИзменитьЗначениеКонтактнойИнформации(Элемент,, Истина) Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура РолиПометкаПриИзменении(Элемент)
	...
	Если СтрокаТаблицы.Пометка И СтрокаТаблицы.Имя = "ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок" Тогда
		...
		ПользователиСлужебныйКлиент.ПоказатьПредупреждениеБезопасности(Оповещение,
			ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередВыборомРоли);
	Иначе
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Попытка
			ДействияСПользователемСервиса = МодульПользователиСлужебныйВМоделиСервиса.ПолучитьДействияСПользователемСервиса(
				ТекущийОбъект.Ссылка, ДействияСПользователемСервиса);
		Исключение
...
Процедура ОбновитьИмяДляВхода(Форма, ПриИзмененииНаименования = Ложь)
	...
	Если Не ПоказатьИмяСОтметкойНезаполненного Тогда
		Если (Не ЗначениеЗаполнено(Форма.ЗначенияПрямогоИзменения.Имя)
		      Или Форма.ЗначенияПрямогоИзменения.Имя = КраткоеИмя)
...
&НаКлиенте
Процедура КонтактнаяИнформацияНачалоВыбораЗавершение(Результат, Контекст) Экспорт
	
	ТекущееЗначение = ЭтотОбъект[Результат.Элемент.Имя];
	
	ЭтотОбъект[Результат.Элемент.Имя] = Результат.НовоеЗначение;
	Если Не ВозможноИзменитьЗначениеКонтактнойИнформации(Результат.Элемент) Тогда
		Результат.Отказ = Истина;
	КонецЕсли;
	
	ЭтотОбъект[Результат.Элемент.Имя] = ТекущееЗначение;
	
КонецПроцедуры

&НаКлиенте
Функция ВозможноИзменитьЗначениеКонтактнойИнформации(Элемент, ПередОчисткой = Ложь, НачалоВыбора = Ложь)
	
	МодульУправлениеКонтактнойИнформациейКлиентСервер =
		ОбщегоНазначенияКлиент.ОбщийМодуль("УправлениеКонтактнойИнформациейКлиентСервер");
	
	ТаблицаКИ = МодульУправлениеКонтактнойИнформациейКлиентСервер.ОписаниеКонтактнойИнформацииНаФорме(ЭтотОбъект);
	ИмяРеквизитаКИ = Элемент.Имя;
	НайденныеСтроки = ТаблицаКИ.НайтиСтроки(Новый Структура("ИмяРеквизита", ИмяРеквизитаКИ));
	Если НайденныеСтроки.Количество() <> 1 Тогда
		Возврат Истина;
	КонецЕсли;
	
	ВидКИ = НайденныеСтроки[0].Вид;
	Если ВидКИ = ВидКонтактнойИнформацииEmailПользователя Тогда
		ИмяРеквизитаПервойКИ = "КонтактнаяИнформацияПолеEmailПользователя";
	ИначеЕсли ВидКИ = ВидКонтактнойИнформацииТелефонПользователя Тогда
		ИмяРеквизитаПервойКИ = "КонтактнаяИнформацияПолеТелефонПользователя";
	Иначе
		Возврат Истина;
	КонецЕсли;
	
	ВсеСтрокиЭтогоВида = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", ВидКИ));
	
	СтароеЗначение = "";
	ЕстьВтороеЗначение = Ложь;
	Для Каждого СтрокаВида Из ВсеСтрокиЭтогоВида Цикл
		Если СтрокаВида.ИмяРеквизита = ИмяРеквизитаКИ Тогда
			СтароеЗначение = СтрокаВида.Представление;
		ИначеЕсли ЗначениеЗаполнено(ЭтотОбъект[СтрокаВида.ИмяРеквизита]) Тогда
			ЕстьВтороеЗначение = Истина;
		КонецЕсли;
	КонецЦикла;
	
	НовоеЗначение = ЭтотОбъект[ИмяРеквизитаКИ];
	ПриИзменении = Не ПередОчисткой И Не НачалоВыбора;
	
	Если ИмяРеквизитаКИ = ИмяРеквизитаПервойКИ Тогда
		Если Не ЕстьВтороеЗначение
		 Или НачалоВыбора
		 Или Не ЗначениеЗаполнено(СтароеЗначение)
		 Или ПриИзменении
		   И ЗначениеЗаполнено(НовоеЗначение) Тогда
			Возврат Истина;
		КонецЕсли;
		Если ВидКИ = ВидКонтактнойИнформацииEmailПользователя Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Перед очисткой электронной почты на вкладке ""%1"", нужно очистить всю дополнительную электронную почту на вкладке ""%2"".'"),
				Элементы.Главное.Заголовок,
				Элементы.КонтактнаяИнформация.Заголовок));
		Иначе
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Перед очисткой телефона на вкладке ""%1"", нужно очистить все дополнительные телефоны на вкладке ""%2"".'"),
				Элементы.Главное.Заголовок,
				Элементы.КонтактнаяИнформация.Заголовок));
		КонецЕсли;
	Иначе
		Если ЕстьВтороеЗначение
		 Или ПередОчисткой
		 Или ПриИзменении
		...
		Если ВидКИ = ВидКонтактнойИнформацииEmailПользователя Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Перед заполнением дополнительной электронной почты на вкладке ""%1"", нужно заполнить электронную почту на вкладке ""%2"".'"),
				Элементы.КонтактнаяИнформация.Заголовок,
				Элементы.Главное.Заголовок));
		Иначе
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Перед заполнением дополнительного телефона на вкладке ""%1"", нужно заполнить телефон на вкладке ""%2"".'"),
				Элементы.КонтактнаяИнформация.Заголовок,
				Элементы.Главное.Заголовок));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПриИзменении Тогда
		ЭтотОбъект[ИмяРеквизитаКИ] = СтароеЗначение;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьПотребоватьСменуПароляПриВходе()
	
	ПотребоватьСменуПароляПриВходе =
		ПользовательИБАутентификацияСтандартная
		И Не ПользовательИБЗапрещеноИзменятьПароль
		И ЗначенияПрямогоИзменения.ПотребоватьСменуПароляПриВходе;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПользовательИБЗапрещеноИзменятьПароль()
	
	ПользовательИБЗапрещеноИзменятьПароль =
		Не ПотребоватьСменуПароляПриВходе
		И ЗначенияПрямогоИзменения.ЗапрещеноИзменятьПароль;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПользовательИБЗапрещеноВосстанавливатьПароль()
	
	ПользовательИБЗапрещеноВосстанавливатьПароль =
		Не ЗначениеЗаполнено(ИмяРеквизитаСПочтойДляВосстановленияПароля)
		Или Не ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизитаСПочтойДляВосстановленияПароля])
		Или Не ПользовательИБАутентификацияСтандартная
		Или ПользовательИБЗапрещеноИзменятьПароль
		Или ЗначенияПрямогоИзменения.ЗапрещеноВосстанавливатьПароль;
...
Процедура АутентификацияПриИзменении()
	...
	ИначеЕсли Не ВходВПрограммуРазрешен Тогда
		ВходВПрограммуРазрешен = ЗначенияПрямогоИзменения.ВходВПрограммуРазрешен;
	КонецЕсли;
	...
	Если УровеньДоступа.АдминистраторСистемы
	 Или УровеньДоступа.ПолныеПрава Тогда
		...
		Пользователи.СкопироватьСвойстваПользователяИБ(
			...
			"УникальныйИдентификатор,
			|Роли",
			ПользователиСлужебный.ИменаСвойствПользователяИБСПрефиксом());
...
Процедура ПереопределитьРедактированиеКонтактнойИнформацииВСервисе()
	...
	СтрокаEMail = ПолучитьСтрокуКонтактнойИнформацииПоВиду(КонтактнаяИнформация, ВидКонтактнойИнформацииEmailПользователя);
	...
	СтрокаТелефон = ПолучитьСтрокуКонтактнойИнформацииПоВиду(КонтактнаяИнформация, ВидКонтактнойИнформацииТелефонПользователя);
...
&НаСервереБезКонтекста
Функция ПолучитьСтрокуКонтактнойИнформацииПоВиду(КонтактнаяИнформация, ВидКонтактнойИнформации)
	
	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Вставить("Вид", ВидКонтактнойИнформации);
	
	НайденныеСтроки = КонтактнаяИнформация.НайтиСтроки(ПараметрыПоиска);
	
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось открыть информацию о пользователе из-за технической проблемы в настройках видов контактной информации (не найден вид ""%1"").
		| Пожалуйста, передайте это сообщение системному администратору для восстановления доступа.'");
		ВызватьИсключение СтрШаблон(ТекстОшибки, ВидКонтактнойИнформации);
	КонецЕсли;
...
Процедура ОбновитьСпособИзмененияEMailВСервисе()
	...
	СтрокаEMail = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид", ВидКонтактнойИнформацииEmailПользователя))[0];
	ЭлементEMail = Элементы[СтрокаEMail.ИмяРеквизита];
...
#Область ПоддержкаДополнительныхРеквизитов
...
#Область ОбработкаПользователяИБ
...
&НаСервере
Процедура ПрочитатьПользователяИБ(ПриКопированииЭлемента = Ложь, ПриСозданииНаСервере = Истина)
	...
	ПользовательИБПароль     = Неопределено;
	ВходВПрограммуРазрешен   = Ложь;
	
	Если ПриСозданииНаСервере И ПриКопированииЭлемента Тогда
	...
	Иначе
		...
		ИначеЕсли ПриСозданииНаСервере И ЗначениеЗаполнено(Параметры.ИдентификаторПользователяИБ) Тогда
...
&НаСервере
Процедура ЗаполнитьЗначенияПрямогоИзменения()
	
	ЗначенияПрямогоИзменения = Новый Структура;
	ЗначенияПрямогоИзменения.Вставить("ВходВПрограммуРазрешен",         ВходВПрограммуРазрешен);
	ЗначенияПрямогоИзменения.Вставить("Имя",                            ПользовательИБИмя);
	ЗначенияПрямогоИзменения.Вставить("АутентификацияСтандартная",      ПользовательИБАутентификацияСтандартная);
	ЗначенияПрямогоИзменения.Вставить("ПотребоватьСменуПароляПриВходе", ПотребоватьСменуПароляПриВходе);
	ЗначенияПрямогоИзменения.Вставить("ЗапрещеноИзменятьПароль",        ПользовательИБЗапрещеноИзменятьПароль);
	ЗначенияПрямогоИзменения.Вставить("ЗапрещеноВосстанавливатьПароль", ПользовательИБЗапрещеноВосстанавливатьПароль);
	
	Если Не ПользовательИБСуществует Тогда
		ЗначенияПрямогоИзменения.АутентификацияСтандартная = Ложь;
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПриЧтенииВходВПрограммуРазрешен И Объект.Недействителен Тогда
			...
			УточнениеСвойств.Вставить(0, НСтр("ru = 'Вход в приложение разрешен'"));
		КонецЕсли;
		...
		Если УровеньДоступа.ИзменениеТекущего Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				...
				СтрокаОписания = МодульУправлениеКонтактнойИнформацией.СтрокаОписанияПочтыДляВосстановленияПароляИзДанныхФормы(
					ЭтотОбъект, ВидКонтактнойИнформацииEmailПользователя, ПользовательИБАдресЭлектроннойПочты);
...
Процедура ЗаполнитьПолеПочтыДляВосстановленияПароляИзИБ()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если ЗначениеЗаполнено(ПочтаДляВосстановленияПароляИзОбъекта[ИмяРеквизитаСПочтойДляВосстановленияПароля])
			...
			ОписаниеПочты = МодульУправлениеКонтактнойИнформацией.СтрокаОписанияПочтыДляВосстановленияПароляИзДанныхФормы(ЭтотОбъект,
				ВидКонтактнойИнформацииEmailПользователя, ЭтотОбъект[ИмяРеквизитаСПочтойДляВосстановленияПароля]);
			...
			Если ОписаниеПочты <> Неопределено Тогда
				...
				ОписаниеПочты.Значение = МодульУправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(
					ПользовательИБАдресЭлектроннойПочты, ВидКонтактнойИнформацииEmailПользователя);
...
#Область НачальноеЗаполнениеПроверкаЗаполненияДоступностьСвойств
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.ПользовательИБЗапрещеноВосстанавливатьПароль.Доступность =
		Не Форма.ПользовательИБЗапрещеноИзменятьПароль
		И ЗначениеЗаполнено(Форма.ИмяРеквизитаСПочтойДляВосстановленияПароля)
		И ЗначениеЗаполнено(Форма[Форма.ИмяРеквизитаСПочтойДляВосстановленияПароля]);
	
	// Уточнение настроек в модели сервиса.
	Если ДействияСПользователемСервиса <> Неопределено Тогда
		
		// Доступность редактирования КИ.
		Отбор = Новый Структура("Вид", Форма.ВидКонтактнойИнформацииEmailПользователя);
