...
//                          ИСПОЛЬЗОВАНИЕ ФОРМЫ                               //
//
// Описание параметров формы см. ПользователиПереопределяемый.ПриОпределенииФормыПодбораПользователей
//
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Фотография = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
...
Процедура СтрокаПоискаОчисткаЗавершение()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ПользователиСписок,
			"Наименование",
			СтрокаПоискаТекст,
			ВидСравненияКомпоновкиДанных.Содержит,,
			ЗначениеЗаполнено(СтрокаПоискаТекст),
			РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный);
...
Процедура ВставитьИзБуфераОбмена(Команда)
	...
	ПараметрыПоиска.Вставить("ПредставлениеПоля", НСтр("ru='Выбор пользователей'"));
...
Процедура ЗаполнитьКонтактнуюИнформацию(Форма, ТекущийПользователь)
	...
	КонтактнаяИнформацияПользователя = ?(ЗначениеЗаполнено(ТекущийПользователь),
		КонтактнаяИнформацияПользователя(ТекущийПользователь, Форма.УникальныйИдентификатор),
		Новый Структура);
	Форма.Фотография = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КонтактнаяИнформацияПользователя, "Фотография", "");
	...
	ПредставлениеПользователя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КонтактнаяИнформацияПользователя, "Наименование", "")
...
Функция КонтактнаяИнформацияПользователя(Пользователь, УникальныйИдентификатор)
...
//    * Строка - Строка
//    * Перемещение - ДействиеПеретаскивания
//
&НаКлиенте
