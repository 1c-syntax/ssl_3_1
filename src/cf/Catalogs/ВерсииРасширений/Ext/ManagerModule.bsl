...
Функция УстановленныеРасширения(ПриЗапуске = Ложь) Экспорт
	...
	Если ПриЗапуске Тогда
		...
		Для Каждого Расширение Из НеподключенныеРасширения Цикл
			РасширенияПриЗапуске.Вставить(КонтрольнаяСуммаРасширения(Расширение), Расширение);
		КонецЦикла;
		ВсеПроблемыПрименения = РасширенияКонфигурации.ПолучитьИнформациюОПроблемахПримененияВСеансе();
		Для Каждого ПроблемаПрименения Из ВсеПроблемыПрименения Цикл
			КонтрольнаяСумма = КонтрольнаяСуммаРасширения(ПроблемаПрименения.Расширение);
			Если РасширенияПриЗапуске.Получить(КонтрольнаяСумма) = Неопределено Тогда
				РасширенияПриЗапуске.Вставить(КонтрольнаяСумма, ПроблемаПрименения.Расширение);
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗарегистрироватьИспользованиеВерсииРасширений() Экспорт
	...
	ВерсияРасширений = ВерсияРасширений(Истина);
	ПараметрыСеанса.ВерсияРасширений = ВерсияРасширений;
	...
	Запрос = ЗапросПроверкиДатыПоследнегоИспользования(ВерсияРасширений);
	Если Запрос.Выполнить().Пустой() Тогда
		ОбновитьПоследнююВерсиюРасширений(ВерсияРасширений);
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Или МонопольныйРежим() Тогда
		ОбновитьДатуПоследнегоИспользованияВерсииРасширений(ВерсияРасширений);
		ОбновитьПоследнююВерсиюРасширений(ВерсияРасширений);
		Возврат;
	КонецЕсли;
	...
	ИмяПроцедуры = "СтандартныеПодсистемыСервер.ОбновитьДатуПоследнегоИспользованияВерсииРасширений";
	Отбор = Новый Структура("ИмяМетода, Состояние", ИмяПроцедуры, СостояниеФоновогоЗадания.Активно);
	ВерсияСтрокой = НРег(ВерсияРасширений.УникальныйИдентификатор());
	
	НайденныеЗадания = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	Для Каждого НайденноеЗадание Из НайденныеЗадания Цикл
		Если СтрНайти(НайденноеЗадание.Наименование, ВерсияСтрокой) > 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Версии расширений: Обновление даты последнего использования %1'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		ВерсияСтрокой);
	
	ПараметрыЗадания = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВерсияРасширений);
	
	// Имя процедуры должно быть, как указано в переменной ИмяПроцедуры.
	ФоновыеЗадания.Выполнить("СтандартныеПодсистемыСервер.ОбновитьДатуПоследнегоИспользованияВерсииРасширений",
...
Функция ПоследняяВерсияРасширений() Экспорт
	...
	НовыеХранимыеСвойства = НовыеХранимыеСвойстваВерсииРасширений();
	
	Если ТипЗнч(ХранимыеСвойства) = Тип("Структура") Тогда
		Для Каждого КлючИЗначение Из НовыеХранимыеСвойства Цикл
			Если ХранимыеСвойства.Свойство(КлючИЗначение.Ключ)
			   И ТипЗнч(ХранимыеСвойства[КлючИЗначение.Ключ]) = ТипЗнч(КлючИЗначение.Значение) Тогда
				Продолжить;
			КонецЕсли;
			ХранимыеСвойства = Неопределено;
			Прервать;
		КонецЦикла;
	КонецЕсли;
	...
	НовыеХранимыеСвойства.ВерсияРасширений = Неопределено;
	
	Возврат НовыеХранимыеСвойства;
...
Процедура УдалитьУстаревшиеВерсииПараметров() Экспорт
	...
	НаименьшаяДатаНачалаСеанса = НаименьшаяДатаНачалаСеанса - 24*60*60;
...
//
Функция ВерсияРасширений(ПриРегистрацииИспользованияВерсииРасширений = Ложь)
	...
	Если Не ТранзакцияАктивна() Или Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Запрос = ЗапросАктивныхВерсийРасширений();
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Или МонопольныйРежим() Тогда
		...
		ДобавитьНовуюВерсиюРасширений(ОписаниеРасширений, ВерсияРасширений);
		Возврат ВерсияРасширений;
	КонецЕсли;
	
	ИмяПроцедуры = "СтандартныеПодсистемыСервер.ДобавитьНовуюВерсиюРасширений";
	Отбор = Новый Структура("ИмяМетода, Состояние", ИмяПроцедуры, СостояниеФоновогоЗадания.Активно);
	
	Хеширование = Новый ХешированиеДанных(ХешФункция.SHA512);
	Хеширование.Добавить(ОписаниеРасширений);
	ХешСуммаСтрокой = Base64Строка(Хеширование.ХешСумма);
	
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Версии расширений: Добавление новой версии %1'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		ХешСуммаСтрокой);
	
	ПараметрыЗадания = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеРасширений);
	ИдентификаторыЗаданий = Новый Соответствие;
	
	НомерПопытки = 1;
	Пока НомерПопытки <= 3 Цикл
		НайденныеЗадания = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
		Индекс = НайденныеЗадания.ВГраница();
		Пока Индекс >= 0 Цикл
			НайденноеЗадание = НайденныеЗадания[Индекс];
			Если СтрНайти(НайденноеЗадание.Наименование, ХешСуммаСтрокой) = 0 Тогда
				НайденныеЗадания.Удалить(Индекс);
			КонецЕсли;
			Индекс = Индекс - 1;
		КонецЦикла;
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если ВерсияНайдена(Выборка, ОписаниеРасширений) Тогда
			Если Не ПриРегистрацииИспользованияВерсииРасширений Тогда
				ОбновитьПоследнююВерсиюРасширений(Выборка.Ссылка);
			КонецЕсли;
			Возврат Выборка.Ссылка;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(НайденныеЗадания) Тогда
			// Имя процедуры должно быть, как указано в переменной ИмяПроцедуры.
			ФоновоеЗадание = ФоновыеЗадания.Выполнить("СтандартныеПодсистемыСервер.ДобавитьНовуюВерсиюРасширений",
				ПараметрыЗадания,, НаименованиеЗадания);
			НайденныеЗадания.Добавить(ФоновоеЗадание);
		КонецЕсли;
		
		ФоновыеЗадания.ОжидатьЗавершенияВыполнения(НайденныеЗадания, 15);
		Для Каждого НайденноеЗадание Из НайденныеЗадания Цикл
			ИдентификаторыЗаданий.Вставить(НайденноеЗадание.УникальныйИдентификатор);
		КонецЦикла;
	...
	ТекстОшибки = НСтр("ru = 'Попробуйте перезапустить сеанс.'");
	УточнениеДляАдминистратора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось создать новую версию расширений.
	...
	УточнениеДляАдминистратора = УточнениеДляАдминистратора
		+ Символы.ПС + ОписаниеСостоянияЗаданий(ИдентификаторыЗаданий);
...
Функция ОписаниеСостоянияЗаданий(ИдентификаторыЗаданий)
	
	ОписаниеЗаданий = Новый СписокЗначений;
	
	Для Каждого КлючИЗначение Из ИдентификаторыЗаданий Цикл
		ФоновоеЗадание = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(КлючИЗначение.Ключ);
		...
		Если ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.Активно Тогда
			ОписаниеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Задание %1 выполняется с %2'"),
				НРег(ФоновоеЗадание.УникальныйИдентификатор),
				Формат(ФоновоеЗадание.Начало, "ДЛФ=DT"));
			
		ИначеЕсли ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
			ОписаниеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Задание %1 с %2 отменено %3'"),
				НРег(ФоновоеЗадание.УникальныйИдентификатор),
				Формат(ФоновоеЗадание.Начало, "ДЛФ=DT"),
				Формат(ФоновоеЗадание.Конец, "ДЛФ=DT"));
			
		ИначеЕсли ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.Завершено Тогда
			ОписаниеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Задание %1 с %2 завершено %3'"),
				НРег(ФоновоеЗадание.УникальныйИдентификатор),
				Формат(ФоновоеЗадание.Начало, "ДЛФ=DT"),
				Формат(ФоновоеЗадание.Конец, "ДЛФ=DT"));
		...
		ИначеЕсли ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
			ОписаниеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Задание %1 с %2 завершено %3 с ошибкой:
				           |%4'"),
				НРег(ФоновоеЗадание.УникальныйИдентификатор),
				Формат(ФоновоеЗадание.Начало, "ДЛФ=DT"),
				Формат(ФоновоеЗадание.Конец, "ДЛФ=DT"),
				?(ТипЗнч(ФоновоеЗадание.ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке"),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ФоновоеЗадание.ИнформацияОбОшибке), ""));
		КонецЕсли;
		ОписаниеЗаданий.Добавить(ОписаниеЗадания, XMLСтрока(ФоновоеЗадание.Начало));
	КонецЦикла;
	
	ОписаниеЗаданий.СортироватьПоПредставлению();
	
	Возврат СтрСоединить(ОписаниеЗаданий.ВыгрузитьЗначения(), Символы.ПС
...
// Для функции ВерсияРасширений.
Процедура ДобавитьНовуюВерсиюРасширений(ОписаниеРасширений, ВерсияРасширений = Неопределено) Экспорт
...
// Для функции ВерсияРасширений и процедуры ДобавитьНовуюВерсиюРасширений.
Функция ЗапросАктивныхВерсийРасширений()
...
// Для функции ПоследняяВерсияРасширений.
//
// Возвращаемое значение:
//  Структура:
//   * ВерсияРасширений - СправочникСсылка.ВерсииРасширений
...
//   * ИдентификаторОбновления - УникальныйИдентификатор
//
Функция НовыеХранимыеСвойстваВерсииРасширений()
	
	Результат = Новый Структура;
	Результат.Вставить("ВерсияРасширений", ПустаяСсылка());
...
Процедура ОбновитьДатуПоследнегоИспользованияВерсииРасширений(ВерсияРасширений) Экспорт
	...
	ОкругленнаяДатаНачалаСеанса = ОкругленнаяДатаНачалаСеанса();
	Запрос = ЗапросПроверкиДатыПоследнегоИспользования(ВерсияРасширений, ОкругленнаяДатаНачалаСеанса);
...
// Для процедуры ЗарегистрироватьИспользованиеВерсииРасширений.
Функция ЗапросПроверкиДатыПоследнегоИспользования(ВерсияРасширений, ОкругленнаяДатаНачалаСеанса = Неопределено)
	
	Если ОкругленнаяДатаНачалаСеанса = Неопределено Тогда
		ОкругленнаяДатаНачалаСеанса = ОкругленнаяДатаНачалаСеанса();
	КонецЕсли;
...
// Для процедур ЗарегистрироватьИспользованиеВерсииРасширений,
// ОбновитьДатуПоследнегоИспользованияВерсииРасширений и ДобавитьНовуюВерсиюРасширений.
//
Процедура ОбновитьПоследнююВерсиюРасширений(ВерсияРасширений)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Или МонопольныйРежим() Тогда
		УстановитьПоследнююВерсиюРасширений(ВерсияРасширений);
		Возврат;
	КонецЕсли;
	
	ИмяПроцедуры = "СтандартныеПодсистемыСервер.УстановитьПоследнююВерсиюРасширений";
	Отбор = Новый Структура("ИмяМетода, Состояние", ИмяПроцедуры, СостояниеФоновогоЗадания.Активно);
	ВерсияСтрокой = НРег(ВерсияРасширений.УникальныйИдентификатор());
	...
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Версии расширений: Установка последней версии %1'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
		ВерсияСтрокой);
	...
	// Имя процедуры должно быть, как указано в переменной ИмяПроцедуры.
	ФоновыеЗадания.Выполнить("СтандартныеПодсистемыСервер.УстановитьПоследнююВерсиюРасширений",
		ПараметрыЗадания,, НаименованиеЗадания);
	
КонецПроцедуры

// Для процедуры ОбновитьПоследнююВерсиюРасширений.
Процедура УстановитьПоследнююВерсиюРасширений(ВерсияРасширений) Экспорт
	...
	Попытка
		Блокировка.Заблокировать();
		ХранимыеСвойства = ПоследняяВерсияРасширений();
		Если ХранимыеСвойства.ВерсияРасширений <> ВерсияРасширений
		   И Не КонфигурацияБазыДанныхИзмененаДинамически()
		   И Не РасширенияИзмененыДинамически() Тогда
