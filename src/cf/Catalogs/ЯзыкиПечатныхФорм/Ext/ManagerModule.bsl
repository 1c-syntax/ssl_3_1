...
Функция ДополнительныеЯзыкиПечатныхФорм() Экспорт
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЯзыкиПечатныхФорм.Код КАК КодЯзыка
	|ИЗ
	|	Справочник.ЯзыкиПечатныхФорм КАК ЯзыкиПечатныхФорм
	|ГДЕ
	|	НЕ ЯзыкиПечатныхФорм.ПометкаУдаления
	|	И НЕ ЯзыкиПечатныхФорм.Код В (&ЯзыкиКонфигурации)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЯзыкиПечатныхФорм.РеквизитДопУпорядочивания";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ЯзыкиКонфигурации", СтандартныеПодсистемыСервер.ЯзыкиКонфигурации());

	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("КодЯзыка");
