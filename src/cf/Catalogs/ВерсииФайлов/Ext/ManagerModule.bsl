...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	И ПОДСТРОКА(ВерсииФайлов.ПутьКФайлу, 1, 1) = &Разделитель
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка";
	...
	Запрос.УстановитьПараметр("Разделитель", ПолучитьРазделительПути());
...
Функция УдалитьЛишнийРазделитель(ВерсияСсылка)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать версию файла %1 по причине: %2'"),
			ВерсияСсылка, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.Справочники.ВерсииФайлов, ВерсияСсылка, ТекстСообщения);
