...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = Метаданные.Справочники.ВерсииФайлов.ПолноеИмя();
	...
	Запрос.УстановитьПараметр("ДатаМодификации", Дата(3999, 12, 31));
	Запрос.УстановитьПараметр("Ссылка", Справочники.ВерсииФайлов.ПолучитьСсылку(
		Новый УникальныйИдентификатор("FFFFFFFF-FFFF-FFFF-FFFF-FFFFFFFFFFFF")));
	...
	Пока Истина Цикл
		...
		ВерсииДляОбработки = ТаблицаВыборки.ВыгрузитьКолонку("Ссылка");
		ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, ВерсииДляОбработки);

		Если ТаблицаВыборки.Количество() < 1000 Тогда
			Прервать;
		КонецЕсли;
		
		ПоследняяСтрока = ТаблицаВыборки[ТаблицаВыборки.Количество() - 1];
		Запрос.УстановитьПараметр("ДатаМодификации", ПоследняяСтрока.ДатаМодификацииУниверсальная);
		Запрос.УстановитьПараметр("Ссылка", ПоследняяСтрока.Ссылка);
