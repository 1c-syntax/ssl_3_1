...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл"
		...
		Или Параметр.Событие = "ВерсияСохранена"
		Или Параметр.Событие = "ФайлыИзмененыПриРаботеСАрхивом") Тогда
...
Процедура СохранитьКак(Команда)
	...
	ПараметрыПолученияФайла.ПрисоединенныйФайл				= ТекущиеДанные.Владелец;
	ПараметрыПолученияФайла.ВерсияСсылка					= ТекущиеДанные.Ссылка;
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаВерсийФайлов)
	...
	ТекстЗапроса = 
		...
		|	ВЫБОР
		|		КОГДА ВерсииФайлов.ДатаПереносаВАрхив = ДАТАВРЕМЯ(1, 1, 1)
		|	ТОГДА -1
		|		ИНАЧЕ 0
...
Процедура ОткрытьВерсиюФайла()
	...
	ПараметрыПолученияФайла = РаботаСФайламиКлиент.ПараметрыАсинхронногоПолученияФайла("ОткрытьВерсиюФайла");
	ПараметрыПолученияФайла.ПрисоединенныйФайл				= ТекущиеДанные.Владелец;
	ПараметрыПолученияФайла.ВерсияСсылка					= ТекущиеДанные.Ссылка;
	ПараметрыПолученияФайла.ФормаВладелец					= ЭтотОбъект;
	ПараметрыПолученияФайла.ПроверятьНаличиеФайлаВАрхиве	= Ложь;
