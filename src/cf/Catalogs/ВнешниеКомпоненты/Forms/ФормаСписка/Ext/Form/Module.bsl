...
Процедура ДобавитьИзКаталога(Команда)
	...
	ТекстПредложения =  НСтр("ru = 'Для загрузки внешних компонент из каталога требуется расширение для работы с файлами'");
...
&НаСервере
Функция ЗагрузитьКомпонентыНаСервере(ПомещенныеФайлы)
...
&НаСервереБезКонтекста
Функция ЭтоФайлСервиса(АдресФайла)
