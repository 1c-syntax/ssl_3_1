...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.ОбновляемыеКомпоненты) <> Тип("Массив") 
		Или Параметры.ОбновляемыеКомпоненты.Количество() = 0 Тогда 
		ВызватьИсключение НСтр("ru = 'Некорректное значение параметра формы ОбновляемыеКомпоненты.'");
	КонецЕсли;
	...
	РеквизитыКомпонент = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Параметры.ОбновляемыеКомпоненты, "Идентификатор, Версия, ОбновлятьСПортала1СИТС");
	Для Каждого ОбновляемаяКомпонента Из Параметры.ОбновляемыеКомпоненты Цикл 
	...
	Если ПредложитьОбновление Тогда 
		...
		Элементы.ДекорацияПояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
			           |Проверить и загрузить обновление компоненты?'"),
			ТекстПояснения);
...
Функция НачатьОбновлениеКомпонентСПортала()
	...
	ПараметрыПроцедуры.Вставить("ОбновляемыеКомпоненты", ОбновляемыеКомпоненты.ВыгрузитьЗначения());
