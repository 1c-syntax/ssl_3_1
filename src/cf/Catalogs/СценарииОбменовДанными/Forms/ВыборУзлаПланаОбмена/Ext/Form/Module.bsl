...
Процедура ПрочитатьДеревоУзловОбмена()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НастройкиТранспорта.ИдентификаторТранспорта = &ИдентификаторТранспорта
		|ИТОГИ ПО
		|	ТипУзла";
	
	Запрос.УстановитьПараметр("ИдентификаторТранспорта", "SM");
	...
	Пока ВыборкаПоТипамУзлов.Следующий() Цикл
		...
		Пока ВыборкаПоУзлам.Следующий() Цикл
			...
			НастройкиТранспорта = ТранспортСообщенийОбмена.НастройкиТранспорта(ВыборкаПоУзлам.Ссылка, "SM");
			
			Если НастройкиТранспорта.ВнутренняяПубликация Тогда 
