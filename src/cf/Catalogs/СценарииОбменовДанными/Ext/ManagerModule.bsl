...
Процедура СоздатьСценарий(
	...
	СтрокаТаблицы.ИдентификаторТранспорта = ИдентификаторТранспорта;
...
// 
// Параметры:
//  НастройкаВыполненияОбмена - Структура - структура настроек выполнения обмена
// 
// Возвращаемое значение:
//  РасписаниеРегламентногоЗадания - получить расписание выполнения обмена данными
//
...
Процедура СоздатьОбновитьРегламентноеЗаданиеВСервисе(РасписаниеРегламентногоЗадания, ТекущийОбъект)
	...
	ЧасовойПояс = МодульРаботаВМоделиСервиса.ПолучитьЧасовойПоясОбластиДанных(ОбластьДанных);
	ЗапланированныйМоментЗапуска = МестноеВремя(ТекущаяУниверсальнаяДата(), ЧасовойПояс);
	...
	Если ВыполняемоеДействие = Перечисления.ДействияПриОбмене.ВыгрузкаДанных Тогда
		...
		Для Индекс = 0 По МаксимальныйИндекс Цикл
			...
			Если СтрокаТаблицы.ВыполняемоеДействие = ВыполняемоеДействие Тогда
				...
				НоваяСтрока.ИдентификаторТранспорта = ИдентификаторТранспорта;	
