...
Процедура ПередЗаписью(Отказ)
	...
	ИменаПолей = "Наименование, Родитель, ПометкаУдаления";
	ТекущаяПапка = ?(ЭтоНовый(), Новый Структура(ИменаПолей),
		ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, ИменаПолей));
	...
	Если ЭтоНовый() Или ТекущаяПапка.Родитель <> Родитель Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", ТекущаяПапка.Родитель) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				?(ЗначениеЗаполнено(ТекущаяПапка.Родитель), ТекущаяПапка.Родитель, НСтр("ru = 'Папки'")));
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", Родитель) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				?(ЗначениеЗаполнено(Родитель), Родитель, НСтр("ru = 'Папки'")));
			ВызватьИсключение(ТекстСообщения, КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЭтоНовый() Тогда
		Если Наименование <> ТекущаяПапка.Наименование Тогда // Переименована папка.
			ИзменитьРабочийКаталогПриПереименованииПапки(Ссылка, ТекущаяПапка);
		КонецЕсли;
		
		Если Родитель <> ТекущаяПапка.Родитель Тогда // Перенесена в другую папку.
			ИзменитьРабочийКаталогПриПеремещенииПапки(Ссылка, Родитель, ТекущаяПапка);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("РабочийКаталог", РабочийКаталог) 
		Или ДополнительныеСвойства.ПрошлыйЭтоНовый Тогда
		...
		Иначе
			...
			Если РабочийКаталогПапки <> "" Тогда
				РабочийКаталогПапки = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(РабочийКаталогПапки) 
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	НедопустимыеСимволы = ОбщегоНазначенияКлиентСервер.НайтиНедопустимыеСимволыВИмениФайла(Наименование);
...
Процедура ИзменитьРабочийКаталогПриПереименованииПапки(Ссылка, ТекущаяПапка)
	...
	Если РабочийКаталогРодителяПапки <> "" Тогда
		...
		РабочийКаталогПапкиУнаследованныйПрежний = РабочийКаталогРодителяПапки + ТекущаяПапка.Наименование
			+ ПолучитьРазделительПути();
			
		Если РабочийКаталогПапкиУнаследованныйПрежний = РабочийКаталогПапки Тогда
			НовыйРабочийКаталогПапки = РабочийКаталогРодителяПапки + Наименование + ПолучитьРазделительПути();
			РаботаСФайламиСлужебный.СохранитьРабочийКаталогПапки(Ссылка, НовыйРабочийКаталогПапки);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ИзменитьРабочийКаталогПриПеремещенииПапки(Ссылка, Родитель, ТекущаяПапка)
	...
	Если РабочийКаталогРодителяПапки <> "" ИЛИ РабочийКаталогНовогоРодителяПапки <> "" Тогда
		...
		Если РабочийКаталогПапкиУнаследованныйПрежний = РабочийКаталогПапки Тогда
			Если РабочийКаталогНовогоРодителяПапки <> "" Тогда
				НовыйРабочийКаталогПапки = РабочийКаталогНовогоРодителяПапки + Наименование 
					+ ПолучитьРазделительПути();
				РаботаСФайламиСлужебный.СохранитьРабочийКаталогПапки(Ссылка, НовыйРабочийКаталогПапки);
			Иначе
				РаботаСФайламиСлужебный.ОчиститьРабочийКаталог(Ссылка);
