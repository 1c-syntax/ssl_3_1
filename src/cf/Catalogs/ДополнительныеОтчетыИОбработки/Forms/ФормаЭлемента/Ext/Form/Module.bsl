...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии = Параметры.ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии;
...
Процедура РазмещениеКомандНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		СтандартныеПодсистемыКлиент.ВыбратьОбъектыМетаданных(ПараметрыФормы);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзменитьРегламентноеЗадание(РежимВыбора = Ложь, ИзмененФлажок = Ложь)
	...
	Контекст.Вставить("РасписаниеКоманды", РасписаниеКоманды);
	
	Обработчик = Новый ОписаниеОповещения("ПередВключениемЗапускаПоРасписанию", ЭтотОбъект, Контекст);
	ПользователиСлужебныйКлиент.ПоказатьПредупреждениеБезопасности(Обработчик,
		ПользователиСлужебныйКлиентСервер.ВидыПредупрежденийБезопасности().ПередВключениемРегламентногоЗаданияДополнительнойОбработки);
	
КонецПроцедуры

&НаКлиенте
Процедура ПередВключениемЗапускаПоРасписанию(Результат, Контекст) Экспорт
	...
	РедактированиеРасписания = Новый ДиалогРасписанияРегламентногоЗадания(Контекст.РасписаниеКоманды);
...
Функция НачатьВыполнениеСервернойКомандыВФоне(ВыполняемаяКоманда, УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ДополнительныеОтчетыИОбработки.ВыполнитьКоманду",
		ПараметрыПроцедуры, НастройкиЗапуска);
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.ФильтрПоОбъектамМетаданных = ФильтрПоОбъектамМетаданных;
	ПараметрыФормы.ВыбранныеОбъектыМетаданных = ВыбранныеОбъектыМетаданных;
	ПараметрыФормы.Заголовок = НСтр("ru = 'Назначение дополнительной обработки'");
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	СвойстваБезопасногоРежима = СвойстваБезопасногоРежима();
	БезопасныйРежим = СвойстваБезопасногоРежима.БезопасныйРежим;
	КоличествоРазрешений = СвойстваБезопасногоРежима.КоличествоРазрешений;
	ИспользуютсяПрофилиБезопасности = СвойстваБезопасногоРежима.ИспользуютсяПрофилиБезопасности;
	...
	Иначе
		...
		Если ИспользуютсяПрофилиБезопасности И БезопасныйРежим = Истина Тогда
			Элементы.СтраницыБезопасныйРежимСРазрешениями.ТекущаяСтраница = Элементы.СтраницаБезопасныйРежимСРазрешениями;
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Булево") Тогда
		...
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
			...
			Элементы.ДекорацияПерсональныйПрофильБезопасностиНадпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Дополнительный отчет или обработка будет подключаться к приложению с использованием ""персонального""
	...
	Элементы.СтраницыВариантыКомандыРазрешения.ОтображениеСтраниц = ОтображениеСтраницФормы[
		?(КоличествоВидимыхЗакладок > 1, "ЗакладкиСверху", "Нет")];
	
	ПредставлениеНазначения = ПредставлениеНазначения(ЭтоГлобальнаяОбработка);
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("ru = 'Дополнительный отчет (создание)'"), 
			НСтр("ru = 'Дополнительная обработка (создание)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, НСтр("ru = '(Дополнительный отчет)'"), 
			НСтр("ru = '(Дополнительная обработка)'"));
	КонецЕсли;
	
	Если КоличествоВариантов > 0 Тогда
		ВыводитьЗаголовокТаблицы = КоличествоВидимыхЗакладок <= 1 И Объект.Вид = ВидДополнительныйОтчет 
			И Объект.ИспользуетХранилищеВариантов;
		Элементы.ВариантыДополнительногоОтчета.ПоложениеЗаголовка = 
			ПоложениеЗаголовкаЭлементаФормы[?(ВыводитьЗаголовокТаблицы, "Верх", "Нет")];
	...
	Если КоличествоКоманд > 0 Тогда
		...
		Элементы.ОбъектКоманды.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы[
			?(ВыводитьЗаголовокТаблицы, "Верх", "Нет")];
...
&НаСервере
Функция СвойстваБезопасногоРежима()

	Результат = Новый Структура;
	Результат.Вставить("БезопасныйРежим", Объект.БезопасныйРежим);
	Результат.Вставить("КоличествоРазрешений", РазрешенияПрофиляБезопасности().Количество());
	Результат.Вставить("ИспользуютсяПрофилиБезопасности", Ложь);
		
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрофилиБезопасности") Тогда
		МодульРаботаВБезопасномРежиме = ОбщегоНазначения.ОбщийМодуль("РаботаВБезопасномРежиме");
		Результат.ИспользуютсяПрофилиБезопасности = МодульРаботаВБезопасномРежиме.ИспользуютсяПрофилиБезопасности();
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("РаботаВМоделиСервиса") Или Результат.ИспользуютсяПрофилиБезопасности Тогда
		МодульРаботаВБезопасномРежимеСлужебный = ОбщегоНазначения.ОбщийМодуль("РаботаВБезопасномРежимеСлужебный");
		Если Объект.РежимСовместимостиРазрешений = Перечисления.РежимыСовместимостиРазрешенийДополнительныхОтчетовИОбработок.Версия_2_1_3 Тогда
			Если Результат.БезопасныйРежим И Результат.КоличествоРазрешений > 0 И Результат.ИспользуютсяПрофилиБезопасности Тогда
				Результат.БезопасныйРежим = ?(ЭтоНовый, "", МодульРаботаВБезопасномРежимеСлужебный.РежимПодключенияВнешнегоМодуля(Объект.Ссылка));
			КонецЕсли;
		ИначеЕсли Результат.КоличествоРазрешений = 0 Тогда
			Результат.БезопасныйРежим = Истина;
		ИначеЕсли Результат.ИспользуютсяПрофилиБезопасности Тогда
			Результат.БезопасныйРежим = ?(ЭтоНовый, "", МодульРаботаВБезопасномРежимеСлужебный.РежимПодключенияВнешнегоМодуля(Объект.Ссылка));
		Иначе
...
&НаСервере
Функция ПредставлениеНазначения(Знач ЭтоГлобальнаяОбработка)
	Результат = "";
	Если ЭтоГлобальнаяОбработка Тогда
		Для Каждого СтрокаРаздел Из Объект.Разделы Цикл
			...
			Если ПредставлениеРаздела = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Результат = ?(ПустаяСтрока(Результат), ПредставлениеРаздела,
				Результат + ", " + ПредставлениеРаздела);
		КонецЦикла;
	Иначе
		Для Каждого СтрокаНазначение Из Объект.Назначение Цикл
			...
			Результат = ?(ПустаяСтрока(Результат), ПредставлениеОбъекта,
				Результат + ", " + ПредставлениеОбъекта);
		КонецЦикла;
	КонецЕсли;
	Если Результат = "" Тогда
		Результат = НСтр("ru = 'Не определено'");
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура СформироватьСписокРазрешений()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРазрешений Цикл
		Разрешение = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(МодульРаботаВБезопасномРежимеСлужебный.Пакет(), СтрокаТаблицы.ВидРазрешения));
		ЗаполнитьЗначенияСвойств(Разрешение, СтрокаТаблицы.Параметры.Получить());
