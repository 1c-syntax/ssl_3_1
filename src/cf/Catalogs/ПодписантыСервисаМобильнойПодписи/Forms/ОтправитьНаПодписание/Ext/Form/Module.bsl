...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйПараметрТипПодписи.Вставить("ПроверятьСертификат", 
...
Процедура ПодписантОткрытие(Элемент, СтандартнаяОбработка)
	...
	СервисМобильнойПодписиСлужебныйКлиент.ПодписантПриОткрытии(ЭтотОбъект, Элемент, СтандартнаяОбработка);
...
Процедура ПодписантСоздание(Элемент, СтандартнаяОбработка)
	...
	СервисМобильнойПодписиСлужебныйКлиент.ПодписантПриСоздании(ЭтотОбъект, Элемент, СтандартнаяОбработка);
...
Процедура ПодписантПриИзмененииНаСервере()
	СервисМобильнойПодписиСлужебный.ПодписантПриИзмененииНаСервере(ЭтотОбъект);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьНастройкуОтправки()
	СервисМобильнойПодписиСлужебный.ЗаполнитьНастройкуОтправки(ЭтотОбъект);
КонецПроцедуры
...
Процедура ОтправитьНаПодписаниеПослеОбработкиПередВыполнением(РезультатПослеОбработки, Контекст) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Не удалось отправить данные в сервис мобильной подписи'");
	
	Если РезультатПослеОбработки.Свойство("ОписаниеОшибки") Тогда
		ОбработатьОшибку(Контекст.Оповещение, НовоеОписаниеОшибки(ЗаголовокОшибки,
			РезультатПослеОбработки.ОписаниеОшибки), Новый Структура);
		Возврат;
	КонецЕсли;
	...
	ПараметрыПодписания.ТипПодписи = ОписаниеДанных.ТипПодписи;
	ПараметрыПодписания.Доверенность = ОписаниеДанных.ВыбраннаяДоверенность;
	...
	СервисМобильнойПодписиСлужебныйКлиент.ОтправитьНаПодписаниеВСервис(Новый ОписаниеОповещения("ОтправитьИОбработатьРезультат", ЭтотОбъект, Контекст),
		Контекст, ПараметрыПодписания);

КонецПроцедуры

&НаКлиенте
Процедура ОтправитьИОбработатьРезультат(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("Ошибка") Тогда
		Если ТипЗнч(Результат.Ошибка) = Тип("Структура") Тогда
			ОбработатьОшибку(Контекст.Оповещение, Результат.Ошибка);
		Иначе
			ОбработатьОшибку(Контекст.Оповещение, НовоеОписаниеОшибки(НСтр("ru='Не удалось отправить данные в сервис мобильной подписи'"),
				Результат.Ошибка));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция НовоеОписаниеОшибки(ЗаголовокОшибки, ТекстОшибки)
	
	Возврат СервисМобильнойПодписиКлиентСервер.НовоеОписаниеОшибки(ЗаголовокОшибки, ТекстОшибки);
