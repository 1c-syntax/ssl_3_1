...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ИННФЛ) И Не ЗначениеЗаполнено(Объект.ИНН) Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Объект.НомерТелефона = "7" + СервисМобильнойПодписиКлиентСервер.ТолькоЦифры(НомерТелефона);
...
Процедура УстановитьВидимостьИДоступность(Форма)
	...
	Элементы.ГруппаОрганизация.Видимость = Форма.ВидПодписанта = "ЮридическоеЛицо";
...
Функция АвтоНаименование()
	Возврат Справочники.ПодписантыСервисаМобильнойПодписи.АвтоНаименование(Объект);
КонецФункции
