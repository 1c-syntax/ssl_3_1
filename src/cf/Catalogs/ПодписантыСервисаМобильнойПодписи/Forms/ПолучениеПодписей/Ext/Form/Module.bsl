...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Подписант) Тогда
		Элементы.НадписьОтправлено.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru='Документы отправлены подписанту <b>%1</b> в мобильное приложение <b>Моя подпись</b> от ФНС. Зайдите в приложение и подпишите документы, затем нажмите кнопку <b>Документы подписаны</b>'"), 
			?(ЗначениеЗаполнено(Параметры.Подписант.Наименование), Параметры.Подписант.Наименование,
...
Процедура Отмена(Команда)
	...
	Для Каждого Строка Из ИдентификаторыДокументов Цикл
		Если ЗначениеЗаполнено(Строка.СвойстваПодписи) И ЗначениеЗаполнено(Строка.СвойстваПодписи.ДатаПодписи) Тогда
			...
			ЕстьПодписи = Истина;
		Иначе
			ДляУдаления.Добавить(Строка.ИдентификаторДокумента);
		КонецЕсли;
	КонецЦикла;
	
	Если ДляУдаления.Количество() > 0 Тогда
		УдалитьЗапросыНаПодписание(ДляУдаления);
	КонецЕсли;
...
Процедура УдалитьЗапросыНаПодписание(Знач ИдентификаторыДокументов)
	
	СервисМобильнойПодписиСлужебный.УдалитьОжидаемыеПодписи(ИдентификаторыДокументов, Истина);
