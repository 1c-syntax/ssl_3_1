...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВыгрузкаОбъектовВФайлы") Тогда
			...
			Элементы.ДекорацияПечатныеФормыИВложения.Заголовок = ЗаголовокПечатныеФормыИВложения;
...
&НаКлиенте
Процедура ПолноеИмяТипаПараметраВводаНаОснованииОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные,
	ВыборДобавлением, СтандартнаяОбработка)
	
	Если СтрСравнить(ВыбранноеЗначение, ШаблоныСообщенийКлиентСервер.ОбщийПредставление()) = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТипВыбора = ПолучитьТипПоПолномуИмени(ВыбранноеЗначение);
	
	Если ТипВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Строка Из Реквизиты.ПолучитьЭлементы() Цикл
	
		Если Строка.ПроизвольныйПараметр И Строка.Тип.СодержитТип(ТипВыбора) Тогда
	
			СтандартнаяОбработка = Ложь;
			ВыбранныйВариант =  Элемент.СписокВыбора.НайтиПоЗначению(ВыбранноеЗначение);
			Если ВыбранныйВариант <> Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru ='%1 уже выбран как произвольный параметр и не может быть назначением.'"),
					ВыбранныйВариант.Представление);
			Иначе
				ТекстОшибки = НСтр("ru ='Объект уже выбран как произвольный параметр и не может быть назначением.'");
			КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьТипПоПолномуИмени(Знач ПолноеИмя)
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя); 
	Возврат ?(МенеджерОбъекта = Неопределено, Неопределено, ТипЗнч(МенеджерОбъекта.ПустаяСсылка()));
КонецФункции
