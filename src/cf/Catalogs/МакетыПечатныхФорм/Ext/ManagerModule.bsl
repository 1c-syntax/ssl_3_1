...
Процедура УдалитьМакет(Ссылка, КодЯзыка = Неопределено) Экспорт
	...
	Иначе
		СтрокиМакета = Объект.Представления.НайтиСтроки(Новый Структура("КодЯзыка", КодЯзыка));
		Для Каждого СтрокаМакета Из СтрокиМакета Цикл
			Объект.Представления.Удалить(СтрокаМакета);
		КонецЦикла;
	КонецЕсли;
