...
Процедура ОшибкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ОшибкиПодробнее Тогда
		...
		Если ТекущиеДанные.Действие = "УстановитьБиблиотекиДляТокенов" Тогда
			...
			ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуУстановкиПрограммКриптопровайдеров(ПараметрыОткрытия,
				ЭтотОбъект, Новый ОписаниеОповещения("ПослеВыполненияДействийПодробнее", ЭтотОбъект));
		Иначе
...
Процедура ДобавитьОшибки(ОписаниеОшибок, ОшибкаНаСервере = Ложь)
	...
	Если ОписаниеОшибок.Свойство("Ошибки")
		...
		Для Каждого СвойстваОшибки Из СвойстваОшибок Цикл
			...
			Если ЗначениеЗаполнено(НовыеСвойстваОшибки.Токен)
				И ЭлектроннаяПодписьСлужебныйКлиентСервер.ОшибкаЗагрузкиБиблиотекиТокена(НовыеСвойстваОшибки.Описание) Тогда
				СтрокаОшибки.Подробнее = НСтр("ru = 'Установить'") + "...";
				СтрокаОшибки.Действие = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеУстановитьБиблиотекиДляТокенов();
				СтрокаОшибки.Параметры = Новый Структура("ВидТокена", НовыеСвойстваОшибки.Токен.Токен);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СтрокаОшибки.Подробнее) Тогда
...
Процедура ВыгрузитьТехническуюИнформацию(ВыгрузитьАрхив)
	...
	Иначе
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			ТекстОшибок, Новый Структура("Тема, Сообщение", Заголовок), , ОписаниеФайлов);
	КонецЕсли;
