...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ПоставляемаяНастройка Из ПоставляемыеНастройки Цикл
		Если ЕстьВОСКлиентаИлиВОССервера(ПоставляемаяНастройка) И Не ПоставляемаяНастройка.Неактуальная Тогда
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Параметры.Программа) Тогда
			...
			Элементы.ИмяПрограммы.ТолькоПросмотр = Истина;
			Элементы.ТипПрограммы.ТолькоПросмотр = Истина;
			Если ЗначениеЗаполнено(Параметры.РежимИспользования) Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Приложение с указанным именем и типом уже есть в списке.'"),
			, "Объект.ИмяПрограммы");
	КонецЕсли;
