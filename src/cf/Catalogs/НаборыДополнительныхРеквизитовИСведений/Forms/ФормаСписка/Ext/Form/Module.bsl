...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ВидСвойств) Тогда
...
Процедура СвойстваОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		Если ВыбранноеЗначение.Свойство("ВладелецДополнительныхЗначений") Тогда
			...
			ПараметрыФормы.Вставить("ВидСвойства", ВидСвойств);
...
Процедура ДобавитьИзНабора(Команда)
	...
	ОбщиеСвойства = Свойства.НайтиСтроки(Новый Структура("Общее", Истина));
	Для каждого Свойство Из ОбщиеСвойства Цикл
		ВыбранныеЗначения.Добавить(Свойство.Свойство);
	КонецЦикла;
...
Процедура ВыбратьУказанныеСтроки(Описание)
	...
	Если Описание.Свойство("Набор") Тогда
		...
		Если Описание.ВидСвойства <> ВидСвойств Тогда
			ВидСвойств = Описание.ВидСвойства;
			НастроитьОтображениеНаборов();
		КонецЕсли;
...
Процедура НастроитьОтображениеНаборов()
	...
	КнопкаСоздать            = Элементы.СвойстваСоздать;
	КнопкаСоздатьКонтекстная = Элементы.СвойстваКонтекстноеМенюСоздать;
	...
	Если ВидСвойств = Перечисления.ВидыСвойств.ДополнительныеСведения Тогда
		...
		КнопкаСоздать.Заголовок          = НСтр("ru = 'Новое'");
		КнопкаСоздатьКонтекстная.Заголовок = НСтр("ru = 'Новое'");
	...
	ИначеЕсли ВидСвойств = Перечисления.ВидыСвойств.Метки Тогда
		...
		КнопкаСоздать.Заголовок            = НСтр("ru = 'Новая'");
		КнопкаСоздатьКонтекстная.Заголовок = НСтр("ru = 'Новая'");
	...
	Иначе
		...
		КнопкаСоздать.Заголовок            = НСтр("ru = 'Новый'");
		КнопкаСоздатьКонтекстная.Заголовок = НСтр("ru = 'Новый'");
	...
	Для Каждого СтрокаТаблицы Из Наборы Цикл
		Если СтрНачинаетсяС(СтрокаТаблицы.ИмяПредопределенныхДанных, "Удалить") Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(СвойстваНабора, СтрокаТаблицы);
		...
		Если ВидСвойств = Перечисления.ВидыСвойств.ДополнительныеСведения И ВидыСвойствНабора.ДополнительныеСведения
			...
			ДоступныеНаборы.Добавить(СтрокаТаблицы.Ссылка);
			ДоступныеНаборыСписок.Добавить(СтрокаТаблицы.Ссылка);
		КонецЕсли;
	КонецЦикла;
...
Процедура ИзменитьПометкуУдаленияИВладельцаЗначений(ТекущееСвойство, ПометкаУдаленияСвойства)
	...
	// АПК:1328-выкл Блокировка устанавливается выше по стеку.
		...
		Если НЕ ЗначениеЗаполнено(СвойствоОбъект.ВладелецДополнительныхЗначений) Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				Для каждого СтрокаТаблицы Из Выгрузка Цикл
					ТекущийОбъект = СтрокаТаблицы.Ссылка.ПолучитьОбъект();
	...
	Если НоваяПометкаЗначений  = Неопределено  И НовыйВладелецЗначений = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из Выгрузка Цикл
		...
		Если НовыйВладелецЗначений <> Неопределено И ТекущийОбъект.Владелец <> НовыйВладелецЗначений Тогда
			ТекущийОбъект.Владелец = НовыйВладелецЗначений;
		КонецЕсли;
		
		Если НоваяПометкаЗначений <> Неопределено И ТекущийОбъект.ПометкаУдаления <> НоваяПометкаЗначений Тогда
