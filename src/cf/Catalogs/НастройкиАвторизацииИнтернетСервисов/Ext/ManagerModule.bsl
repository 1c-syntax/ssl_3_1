...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта"
		И Не Параметры.Свойство("Ключ") // АПК:1415 Динамический состав свойств.
		И Не Параметры.Свойство("ЗначениеКопирования") // АПК:1415 Динамический состав свойств.
		И ПравоДоступа("Редактирование", Метаданные.Справочники.НастройкиАвторизацииИнтернетСервисов) Тогда
		
		ВыбраннаяФорма = "ПомощникРегистрацииПриложения";
...
// Возвращает настройки авторизации интернет-сервиса.
//
// Параметры:
...
//   Структура - настройки авторизации интернет-сервиса
...
Функция ЕстьНастройкиАвторизации(ВладелецДанных, ИмяИнтернетСервиса = "") Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	НастройкиАвторизацииИнтернетСервисов.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.НастройкиАвторизацииИнтернетСервисов КАК НастройкиАвторизацииИнтернетСервисов
	|ГДЕ
	|	НастройкиАвторизацииИнтернетСервисов.ВладелецДанных = &ВладелецДанных
	|	И ВЫРАЗИТЬ(НастройкиАвторизацииИнтернетСервисов.ИдентификаторПриложения КАК СТРОКА(1024)) <> """"
	|	И НЕ НастройкиАвторизацииИнтернетСервисов.ПометкаУдаления
	|	И (&ИмяИнтернетСервиса = """"
	|	ИЛИ НастройкиАвторизацииИнтернетСервисов.ИмяИнтернетСервиса = &ИмяИнтернетСервиса)";
...
Процедура ЗаписатьНастройкиАвторизации(НастройкиАвторизации) Экспорт
	...
	ВыполнитьПроверкуПравДоступа("Изменение", Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты);
