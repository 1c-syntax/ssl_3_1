...
Функция СведенияОДоверенности(Объект) // СвДовТип
	...
	СведенияОбИнформационнойСистеме = "https://m4d.nalog.gov.ru/";
...
Функция СведенияОДоверителеЮридическогоЛица(РеквизитыОрганизации, РеквизитыУправляющихКомпаний, РеквизитыФизическихЛиц, ДляНалоговыхОрганов) // РосОргДоверТип
	...
	СведенияОЛицахДоверителя = СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности(
...
Функция СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности(РеквизитыОрганизации, РеквизитыУправляющихКомпаний, РеквизитыФизическихЛиц, ДляНалоговыхОрганов) // ЛицоБезДовТип
	
	СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности = Новый Массив;
	...
	Для Каждого РеквизитыУправляющейКомпании Из РеквизитыУправляющихКомпаний Цикл
		...
		КонецЕсли;
		СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности.Добавить(СведенияОЛицеДоверителя);
	КонецЦикла;
	...
	Для Каждого РеквизитыФизическогоЛица Из РеквизитыФизическихЛиц Цикл
		...
		Если РеквизитыФизическогоЛица.ЭтоИндивидуальныйПредприниматель Тогда // удалить ип?
			СведенияОЕдиноличномИсполнительномОрганеИндивидуальномПредпринимателе = СведенияОбИндивидуальномПредпринимателе(
	...
	Возврат СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности;
...
Функция СведенияОДолжностномЛице(РеквизитыФизическогоЛица, ДляНалоговыхОрганов) // СвФЛТип
	...
	Должность = РеквизитыФизическогоЛица.ДолжностьЛицаБезДоверенности;
	СведенияОФизическомЛице = СведенияОФизическомЛице(РеквизитыФизическогоЛица, ДляНалоговыхОрганов);
...
Функция ПечатьДоверенности(Доверенности, ОбъектыПечати, КодЯзыка)
	...
	Для Каждого Ссылка Из Доверенности Цикл
		...
		СведенияОДоверенности = Доверенность.Документ.Довер.СвДов;
		...
		СведенияОДоверителях = Доверенность.Документ.Довер.СвДоверит;
		Для Каждого СведенияОДоверителе Из СведенияОДоверителях Цикл
			...
			Если ТипДоверителя = "1" Тогда
				...
				Для Каждого СведенияОЛицеДоверителя Из СведенияОЛицахДоверителя Цикл
					...
					СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности = СведенияОЛицеДоверителя.СвФЛ;
					...
					Если СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности <> Неопределено Тогда
						Параметры = РеквизитыДолжностногоЛица(СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности);
		...
		СведенияОПредставителях = Доверенность.Документ.Довер.СвУпПред;
		Для Каждого СведенияОПредставителе Из СведенияОПредставителях Цикл
		...
		СведенияОПолномочияхПредставителей = Доверенность.Документ.Довер.СвПолн;
...
Функция ДоверенностьXDTO(Знач ДоверенностьXML)
...
Функция РеквизитыЮридическогоЛица(СведенияОЮридическомЛице)
	...
	ИдентификаторАдресаПоФИАС = АдресЮридическогоЛица.ИдФИАС;
	ЮридическийАдресЗначение = РаботаСАдресами.АдресПоИдентификатору(ИдентификаторАдресаПоФИАС);
...
Функция РеквизитыДолжностногоЛица(СведенияОДолжностномЛице)
	...
	Параметры.Вставить("ДолжностьЛицаБезДоверенности", Должность);
...
Функция РеквизитыФизическогоЛица(СведенияОФизическомЛице)
	...
	Если АдресМестаЖительства <> Неопределено Тогда
		...
		ИдентификаторАдресаПоФИАС = АдресМестаЖительства.ИдФИАС;
		АдресРегистрацииЗначение = РаботаСАдресами.АдресПоИдентификатору(ИдентификаторАдресаПоФИАС);
	КонецЕсли;
...
Функция РеквизитыИностранногоЮридическогоЛица(СведенияОбИностранномЮридическомЛице)
	...
	ФактическийАдрес = СведенияОбИностранномЮридическомЛице.АдрМНФакт;
	
	ИдентификаторАдресаПоФИАС = ФактическийАдрес.ИдФИАС;
	ФактическийАдресЗначение = РаботаСАдресами.АдресПоИдентификатору(ИдентификаторАдресаПоФИАС);
	ФактическийАдрес = ПредставленияАдреса(ФактическийАдрес);
	...
	Параметры.Вставить("ФактическийАдресЗначение", ФактическийАдресЗначение);
			...
			Для Каждого СведенияОЛицеДоверителя Из СведенияОЛицахДоверителя Цикл
				...
				Если СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности <> Неопределено Тогда
					ДобавитьДолжностноеЛицоДоверителя(Объект, ОрганизацияДоверитель,
						СведенияОЛицахДействующихОтИмениЮридическогоЛицаБезДоверенности);
				КонецЕсли;
...
Функция ДобавитьФизическоеЛицоДоверителя(Объект, СведенияОФизическомЛицеДоверителе)
	...
	Параметры = РеквизитыФизическогоЛица(СведенияОФизическомЛицеДоверителе);
