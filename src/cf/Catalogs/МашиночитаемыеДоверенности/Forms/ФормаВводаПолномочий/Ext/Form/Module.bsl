...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоПередоверие = ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности);
	ЭтоВыборИзКлассификатора = Параметры.РежимКлассификатора;
	...
	Иначе
		...
		Элементы.ПоказатьВыбранные.Пометка = Не ЭтоВыборИзКлассификатора И Не ЭтоПередоверие И ВыбранныеПолномочияТаблица.Количество() > 0;
		Элементы.ПоказатьВыбранные.Видимость = Не ЭтоВыборИзКлассификатора И Не ЭтоПередоверие;
		Элементы.ГруппаСохранение.Видимость = Не ЭтоВыборИзКлассификатора;
		Элементы.ГруппаОграниченияПолномочий.Видимость = Не ЭтоВыборИзКлассификатора;
		Элементы.ВыбранныеПолномочияТаблицаЗагрузитьНастройкуПолномочий.Видимость = Не ЭтоПередоверие;
		Элементы.ЗагрузитьНастройкуПолномочий.Видимость = Не ЭтоПередоверие;
		
	КонецЕсли;
	
	Элементы.ТекстПолномочий.ТолькоПросмотр = Параметры.ТолькоПросмотр Или ЭтоПередоверие;
	Элементы.ГруппаКнопокВидПолномочий.Видимость  = Не ЭтоВыборИзКлассификатора И Не Параметры.ТолькоПросмотр И Не ЭтоПередоверие;
	Элементы.ГруппаКнопокВидПолномочий1.Видимость = Элементы.ГруппаКнопокВидПолномочий.Видимость;
	Элементы.ГруппаКнопокВидПолномочий2.Видимость = Элементы.ГруппаКнопокВидПолномочий.Видимость;
	Элементы.ОбновитьКлассификаторыПолномочий.Видимость = Не ОбщегоНазначения.РазделениеВключено() И Не ЭтоПередоверие;
...
Процедура ВыбранныеПолномочияТаблицаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Нельзя добавлять полномочия у передоверия вручную.'"), , , , Отказ);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ОграниченияПолномочийПередУдалением(Элемент, Отказ)
	
	Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Нельзя удалять ограничения у передоверия.'"), , , , Отказ);
...
&НаКлиенте
Процедура ВыбранныеПолномочияТаблицаПередУдалением(Элемент, Отказ)
	
	СнятьОтметкуСтроки(Элементы.ВыбранныеПолномочияТаблица.ТекущиеДанные.ИдентификаторПолномочия);
...
Процедура УправлениеФормой(Форма)
	...
	Форма.КлючСохраненияПоложенияОкна = Форма.ВидПолномочий;
	Форма.КлючНазначенияИспользования = Форма.ВидПолномочий;
...
Процедура СформироватьДеревоПолномочий(АдресПолномочий)
	...
	Если Не ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
	...
	Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
		ПолномочияРодительскойДоверенности = ПолучитьИзВременногоХранилища(Параметры.ПолномочияРодительскойДоверенности);
		Для Каждого Полномочие Из ПолномочияРодительскойДоверенности.Полномочия Цикл
			НоваяСтрока = ДеревоЗначений.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Полномочие);
		КонецЦикла;
		Для Каждого Ограничение Из ПолномочияРодительскойДоверенности.Ограничения Цикл
			НоваяСтрока = ОграниченияПолномочийРодительскойДоверенности.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Ограничение);
			Если Не ЗначениеЗаполнено(НоваяСтрока.КодЗначения) Тогда
				НоваяСтрока.ЭтоТекстовоеЗначение = Истина;
...
&НаСервере
Процедура СнятьОтметкуСтроки(Знач ИдентификаторПолномочия)
	
	ДеревоЗначений = РеквизитФормыВЗначение("Полномочия");
	
	Найдено = ДеревоЗначений.Строки.Найти(ИдентификаторПолномочия, "ИдентификаторПолномочия", Истина);
	Если Найдено <> Неопределено Тогда
		Найдено.Пометка = Ложь;
		
		Если ЗначениеЗаполнено(Найдено.Родитель) Тогда
			СтрокиСПометками = Новый Соответствие;
			СтрокиСПометками.Вставить(Найдено.Родитель, Истина);
			ОбновитьФлажки(СтрокиСПометками);
		КонецЕсли;
...
&НаСервере
Процедура ОбновитьФлажки(СтрокиСПометками)
...
Процедура ОбновитьСтрокуВыбранныхПолномочий(Флажок, Строка)
	...
	Если Флажок = 1 Тогда
		Если Найдено.Количество() = 0 Тогда
			...
			Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
				НайденыОграничения = ОграниченияПолномочийРодительскойДоверенности.НайтиСтроки(
					Новый Структура("ИдентификаторПолномочия", НоваяСтрока.ИдентификаторПолномочия));
				НоваяСтрока.ЕстьОграничения = НайденыОграничения.Количество() > 0;
				Для Каждого НайденнаяСтрока Из НайденыОграничения Цикл
					НоваяСтрокаОграничений = ОграниченияПолномочий.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаОграничений, НайденнаяСтрока);
				КонецЦикла;
			КонецЕсли;
	...
	ИначеЕсли Найдено.Количество() > 0 Тогда
		Для Каждого НайденнаяСтрока Из Найдено Цикл
			...
			Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
				НайденыОграничения = ОграниченияПолномочий.НайтиСтроки(
					Новый Структура("ИдентификаторПолномочия", НайденнаяСтрока.ИдентификаторПолномочия));
				Для Каждого Ограничение Из НайденыОграничения Цикл
					ОграниченияПолномочий.Удалить(ОграниченияПолномочий.Индекс(Ограничение));
				КонецЦикла;
			КонецЕсли;
...
Процедура СохранитьНастройку(НастройкаПолномочий, Наименование)
	...
	НоваяЗапись.Настройка = Новый ХранилищеЗначения(НастройкаПолномочий, Новый СжатиеДанных(9));
