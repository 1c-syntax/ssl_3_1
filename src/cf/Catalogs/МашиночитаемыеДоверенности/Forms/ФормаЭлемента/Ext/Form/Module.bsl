...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьСервисМобильнойПодписи = Ложь;
	...
	ЕстьПравоРедактировать = ПравоДоступа("Изменение", Метаданные.Справочники.МашиночитаемыеДоверенности);
	...
	ПоказыватьПерсональныеДанные = Пользователи.ЭтоПолноправныйПользователь() 
		Или ПравоДоступа("Изменение", Метаданные.Справочники.МашиночитаемыеДоверенности)
		Или Справочники.МашиночитаемыеДоверенности.ПоказыватьПерсональныеДанные(Объект, ТекущийПользователь);
...
Процедура ПервоначальныйДоверительОткрытие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.ПоказыватьПерсональныеДанные = ПоказыватьПерсональныеДанные;
...
&НаКлиенте
Процедура ДекорацияРегистрацияВФНСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь; 

	Если (НавигационнаяСсылкаФорматированнойСтроки = "ХронологическиеДанныеОшибка"
		Или НавигационнаяСсылкаФорматированнойСтроки = "ОтказПриРегистрацииВНалоговомОргане")
		И ЭтоАдресВременногоХранилища(АдресХронологическихДанных) Тогда
		
		ХронологическиеДанные = ПолучитьИзВременногоХранилища(АдресХронологическихДанных);
		Если ТипЗнч(ХронологическиеДанные) = Тип("Структура") Тогда
			
			Если ЗначениеЗаполнено(ХронологическиеДанные.Ошибка) Тогда
				КонтекстОшибки = МашиночитаемыеДоверенностиФНССлужебныйКлиент.КонтекстДляОбработкиОшибкиРР();
				КонтекстОшибки.ЗаголовокПредупреждения = ХронологическиеДанные.Ошибка.ЗаголовокОшибки;
				КонтекстОшибки.Форма = ЭтотОбъект;
				МашиночитаемыеДоверенностиФНССлужебныйКлиент.ОбработатьОшибкуВзаимодействияРР(ХронологическиеДанные.Ошибка, КонтекстОшибки);
			ИначеЕсли ЗначениеЗаполнено(ХронологическиеДанные.ХронологическиеДанные.ПричиныОтказаВПриемеМЧД) Тогда
				ПараметрыФормы = Новый Структура;
				ПараметрыФормы.Вставить("ЗаголовокПредупреждения", НСтр("ru='Отказ при регистрации в налоговом органе'"));
				ПараметрыФормы.Вставить("ТекстОшибкиКлиент", СтрСоединить(ХронологическиеДанные.ХронологическиеДанные.ПричиныОтказаВПриемеМЧД, Символы.ПС));
				...
				ПараметрыФормы.Вставить("ТекстОшибкиСервер", "");
				ПараметрыФормы.Вставить("ТекстОшибки", "");
				ПараметрыФормы.Вставить("ИнформацияДляПоддержки", "");
				ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыФормы, ЭтотОбъект);
			КонецЕсли;
			
		КонецЕсли;
	ИначеЕсли ЭтоАдресВременногоХранилища(АдресХронологическихДанных) Тогда
		
		ХронологическиеДанные = ПолучитьИзВременногоХранилища(АдресХронологическихДанных);
		Если ТипЗнч(ХронологическиеДанные) = Тип("Структура") Тогда
			ИсторияСобытий = ХронологическиеДанные.ХронологическиеДанные.ИсторияСобытий;
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			Для Каждого Событие Из ИсторияСобытий Цикл
				Строка = СтрШаблон("%1 %2", Событие.ДатаВремяИзданияТранзакции, Событие.ОписаниеСобытия);
				ТекстовыйДокумент.ДобавитьСтроку(Строка);
			КонецЦикла;
			ТекстовыйДокумент.Показать(НСтр("ru='Хронологические данные регистрации доверенности в налоговом органе'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура Подписать(Команда)
	...
	Если СтандартнаяОбработка Тогда
		...
		Если ИспользоватьСервисМобильнойПодписи Тогда
			ПараметрыПодписания.РазрешитьОтправкуНаПодписание = Истина;
			ПараметрыПодписания.ПакетнаяОтправкаНаПодписание = Ложь;
		КонецЕсли;
		
		ЭлектроннаяПодписьКлиент.Подписать(ОписаниеДанных, ЭтотОбъект, Оповещение, ПараметрыПодписания);
	КонецЕсли;
...
Процедура ОтправитьНаПодписание(Команда)
	...
	СтандартнаяОбработка = Истина;
	Оповещение = Новый ОписаниеОповещения("ПослеПодписанияДоверенности", ЭтотОбъект, "ОтправкаНаПодписание");
	МашиночитаемыеДоверенностиФНСКлиентПереопределяемый.ПриНачалеПодписания(ОрганизацииДоверители(), 
		Объект.ФайлДоверенности, Объект.Полномочия, Оповещение, СтандартнаяОбработка);
	...
	Если СтандартнаяОбработка Тогда
		...
		Данные.Вставить("Данные", ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
		Данные.Вставить("Объект", Объект.ФайлДоверенности);

		Представление = Новый Структура;
		Представление.Вставить("Значение", Объект.ФайлДоверенности);
		Представление.Вставить("Представление", ДанныеФайла.ИмяФайла);
		Данные.Вставить("Представление", Представление);
		
		ОписаниеДанных.Вставить("НаборДанных", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Данные));
		ЭлектроннаяПодписьКлиент.ОтправитьНаПодписание(ОписаниеДанных, ЭтотОбъект, Оповещение);
	КонецЕсли;
...
Процедура ВыгрузитьВФайл(Команда)
	...
	МашиночитаемыеДоверенностиФНССлужебныйКлиент.ВыгрузитьДоверенностьВФайл(Объект.Ссылка, Не ПоказыватьПерсональныеДанные);
...
Процедура ПроверитьПодпись(Команда)
	...
	Если ЕстьПравоРедактировать Тогда
...
&НаКлиенте
Процедура ПросмотрКвитанций(Команда)
	ПараметрыФормы = Новый Структура("Доверенность", Объект.Ссылка);
	ОткрытьФорму("Справочник.МашиночитаемыеДоверенности.Форма.Квитанции", ПараметрыФормы, ЭтотОбъект);
КонецПроцедуры
...
Процедура ПослеПроверкиДоверенности(РезультатПроверкиДоверенности, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатПроверкиДоверенности.ОписаниеОшибок) Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Проверка доверенности'"),
			"",
			РезультатПроверкиДоверенности.ОписаниеОшибок,
			Новый Структура,
...
Функция ЗаполнениеПроверено()
	...
	Если Объект.ДляНалоговыхОрганов Тогда
		...
		Для Каждого СтрокаПолномочий Из Объект.Полномочия Цикл
			Если СтрНачинаетсяС(СтрокаПолномочий.Код, "1_FNS") Тогда
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Доверенность содержит устаревший код налоговых полномочий %1. Коды налоговых полномочий должны начинаться с 2_FNS.'"),
					СтрокаПолномочий.Код),,,, Отказ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПослеПодписанияДоверенности(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успех Тогда
		...
		Если ДополнительныеПараметры = "ОтправкаНаПодписание" Тогда
...
Функция ОбновитьСформироватьФайлДоверенности()
	...
	Если Не РеквизитыНедоступныДляРедактирования И ЭлектронныеПодписи.Количество() = 0 И Не Подписана И Не ОжидаетсяПодписание И Не ТолькоПросмотр Тогда
...
Процедура УстановитьВидимостьИДоступность(Форма, Условие = Неопределено)
	...
	Если Условие = Неопределено Или Условие = "Подписание" Тогда
		...
		РеквизитыНедоступныДляРедактирования = Не Форма.ЕстьПравоРедактировать 
			Или Форма.ЭлектронныеПодписи.Количество() > 0 Или ВнешнийСтатус(Форма);
...
Процедура ВидимостьПриИзмененииКоличестваПодписей(Форма)
	...
	Форма.Элементы.ФормаОтправитьНаПодписание.Видимость = Ложь;
	...
	Если РеквизитыНедоступныДляРедактирования Тогда
		...
		Иначе
			...
			Иначе
				...
				Форма.Элементы.ФормаОтправитьНаПодписание.Видимость = Форма.ИспользоватьСервисМобильнойПодписи И Не ПросроченаИлиОтменена;
	...
	Форма.Элементы.ФормаПросмотрФайловДоверенности.Видимость = ЗначениеЗаполнено(Форма.Объект.ФайлДоверенности)
...
Процедура ПроверитьРеквизитыРуководителяНаСовместимость(ИмяЭлементаОрганизация, ИмяЭлементаРуководитель = Неопределено)
	...
	ИначеЕсли ИмяЭлементаОрганизация = "Доверитель" И ДоверительТипСтрока <> "ФизическоеЛицо" Тогда
		...
		Если (ДоверительРеквизиты.ЭтоФизическоеЛицо 
			...
			ЗаполнитьТипИРеквизитыОрганизации(ИмяЭлементаРуководитель, "ФизическоеЛицо");
		ИначеЕсли ЭтотОбъект[ИмяЭлементаРуководитель + "Реквизиты"] = Неопределено Или ЛицоДоверителяТипСтрока = "ФизическоеЛицо" Тогда
			ЗаполнитьТипИРеквизитыОрганизации(ИмяЭлементаРуководитель, "ФизическоеЛицо");
		КонецЕсли;
...
Функция ЗаголовокДекорации(Реквизиты, ИмяЭлементаОрганизация = Неопределено, ТолькоОбязательные = Ложь)
	...
	ТолькоОсновные = Подписана Или ВнешнийСтатус(ЭтотОбъект) Или ОжидаетсяПодписание;
	
	Возврат МашиночитаемыеДоверенностиФНССлужебный.ЗаголовокДекорации(Назначение, Реквизиты, ИмяЭлементаОрганизация,
		ТолькоОсновные, ТолькоОбязательные, ПоказыватьПерсональныеДанные);
...
Функция ЗаголовокДекорацииТаблицы(Реквизиты, ИмяЭлементаОрганизация = Неопределено, ТолькоОбязательные = Ложь)
	...
	Возврат МашиночитаемыеДоверенностиФНССлужебный.ЗаголовокДекорацииДляТаблицы(Назначение, Реквизиты, ИмяЭлементаОрганизация,
	 Подписана Или ОжидаетсяПодписание Или ВнешнийСтатус(ЭтотОбъект), ТолькоОбязательные,
		ПоказыватьПерсональныеДанные);
...
Процедура ОбработчикОжиданияРеквизитыПриИзменении(ИмяЭлементаОрганизация)
	...
	Если Подписана Или ОжидаетсяПодписание Или РеквизитыНедоступныДляРедактирования Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьОбъект(ТекущийОбъект = Неопределено)
	...
	Для Каждого Строка Из ДоверенностьОбъект.Доверители Цикл
		...
		Строка.ДоверительРеквизиты = Новый ХранилищеЗначения(РеквизитыДоверителя, Новый СжатиеДанных(9));
	...
	Для Каждого Строка Из ДоверенностьОбъект.Представители Цикл
		...
		Строка.ПредставительРеквизиты = Новый ХранилищеЗначения(РеквизитыПредставителя, Новый СжатиеДанных(9));
...
Процедура ПрочитатьРеквизитыТабличнойЧасти(ТабличнаяЧастьОбъекта, ТабличнаяЧастьФормы, ИмяРеквизита)
	...
	Для Каждого Строка Из ТабличнаяЧастьОбъекта Цикл
		НайденыСтроки = ТабличнаяЧастьФормы.НайтиСтроки(Новый Структура("Идентификатор", Строка.Идентификатор));
		Если НайденыСтроки.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		СтрокаОбъекта = НайденыСтроки[0];
...
Процедура УдалитьПодпись(Команда)
	...
	ОчиститьРезультатыПроверки();
	Если ЗначениеЗаполнено(ТекущиеДанные.ПорядковыйНомер) Тогда
		УдалитьПодписьНаСервере(ТекущиеДанные.ПорядковыйНомер);
	Иначе
		УдалитьПодписьНаСервере(ТекущиеДанные.ИдентификаторПодписи);
	КонецЕсли;
	ОповеститьОбИзменении(Объект.Ссылка);
...
Процедура ПрочитатьСостояние(Форма)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И Объект.РегистрироватьВРеестре Тогда
		...
		Если ЭтоАдресВременногоХранилища(Форма.АдресХронологическихДанных) Тогда
			ПоместитьВоВременноеХранилище(ДанныеСостояния.ХронологическиеДанные, Форма.АдресХронологическихДанных);
		Иначе
			Форма.АдресХронологическихДанных = ПоместитьВоВременноеХранилище(ДанныеСостояния.ХронологическиеДанные, Форма.УникальныйИдентификатор);
		КонецЕсли;
...
Процедура ПрочитатьСостояниеВДругомРеестре(ПриЧтенииНаСервере = Ложь)
	...
	Элементы.РегистрироватьВДругомРеестре.Доступность = ЕстьПравоРедактировать;
...
Процедура ОбновитьСтатус(НаправитьЗапросВРР = Ложь)
	...
	Если Не Параметры.Ключ.Пустая() Тогда
		...
		Если ЭтоАдресВременногоХранилища(АдресХронологическихДанных) Тогда
			ПоместитьВоВременноеХранилище(ДанныеСостояния.ХронологическиеДанные, АдресХронологическихДанных);
		Иначе
			АдресХронологическихДанных = ПоместитьВоВременноеХранилище(ДанныеСостояния.ХронологическиеДанные, УникальныйИдентификатор);
		КонецЕсли;
...
#Иначе
	МодульСтроковыеФункции = СтроковыеФункцииКлиент;
#КонецЕсли
	Элементы.ГруппаРегистрацияВФНС.Видимость = Ложь;
	...
	Если ДанныеСостояния.ОтображатьСтатусВДокументе И Объект.РегистрироватьВРеестре И Объект.ДляНалоговыхОрганов Тогда
		Если ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные) Тогда
	
			Если ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные.Ошибка) Тогда
				
				Элементы.ГруппаРегистрацияВФНС.Видимость = Истина;
				Элементы.ДекорацияРегистрацияВФНС.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(НСтр(
				"ru='<a href = ""ХронологическиеДанныеОшибка"">Не удалось получить информацию о регистрации в налоговом органе</a>'"));
				Элементы.ДекорацияКартинкаРегистрацияВФНС.Картинка = БиблиотекаКартинок.ДиалогСтоп;
				
			ИначеЕсли ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные.ХронологическиеДанные) Тогда
				
				Элементы.ГруппаРегистрацияВФНС.Видимость = Истина;
				ИсторияСобытий = ДанныеСостояния.ХронологическиеДанные.ХронологическиеДанные.ИсторияСобытий;
				Если ИсторияСобытий.Количество() > 0 Тогда
					Событие = ИсторияСобытий[ИсторияСобытий.ВГраница()];
					ОписаниеСобытия = СтрШаблон("%1 %2", Формат(Событие.ДатаВремяИзданияТранзакции, "ДЛФ=D"), Событие.ОписаниеСобытия);
				Иначе
					ОписаниеСобытия = НСтр("ru='Не удалось получить информацию о регистрации в налоговом органе'");
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные.ХронологическиеДанные.ПричиныОтказаВПриемеМЧД) Тогда
					
					Элементы.ДекорацияРегистрацияВФНС.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(НСтр(
					"ru='<a href = ""ОтказПриРегистрацииВНалоговомОргане"">%1</a>'"), ОписаниеСобытия);
					Элементы.ДекорацияКартинкаРегистрацияВФНС.Картинка = БиблиотекаКартинок.ДиалогСтоп;
					
				Иначе
					
					Элементы.ДекорацияРегистрацияВФНС.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(НСтр(
					"ru='<a href = ""РегистрацияВНалоговомОргане"">%1</a>'"), ОписаниеСобытия);
					Элементы.ДекорацияКартинкаРегистрацияВФНС.Картинка = БиблиотекаКартинок.ДиалогИнформация;
				КонецЕсли;
...
Процедура ЗаполнитьСписокПодписей()
	...
	ПодписиОбъекта = ЭлектроннаяПодпись.ПодписиОбъекта(Объект.ФайлДоверенности, ПараметрыПолученияПодписей);
...
Процедура УстановитьУсловноеОформление()
	...
	Если Элементы.ГруппаПередоверие.Видимость Тогда
		...
		Шрифт = Новый Шрифт(Элементы.ЦепочкаПередоверия.Шрифт,,,Истина); // АПК:1345 Шрифт формируется на основании шрифта элемента.
