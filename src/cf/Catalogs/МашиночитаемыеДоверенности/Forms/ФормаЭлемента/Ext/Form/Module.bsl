...
Процедура ДоступностьПризнакаРегистрацииВРеестре(Форма)
	...
	Если Форма.Элементы.РегистрироватьВРеестре.ТолькоПросмотр 
		И Форма.Объект.РегистрироватьВРеестре И Форма.Объект.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыМЧД.Черновик") Тогда
		Форма.Элементы.РегистрироватьВРеестре.Заголовок = НСтр("ru='Зарегистрирована в Распределенном реестре ФНС России'");
	Иначе
		Форма.Элементы.РегистрироватьВРеестре.Заголовок = НСтр("ru='Регистрировать в Распределенном реестре ФНС России'");
	КонецЕсли;
	...
	Если Не Форма.Просрочена И (Форма.Подписана Или Форма.ТехническийСтатус = ПредопределенноеЗначение("Перечисление.ТехническиеСтатусыМЧД.Подписана")) Тогда
		Если Не Форма.Объект.РегистрироватьВРеестре Тогда
			Форма.Элементы.ФормаОтправить.Видимость = Форма.РегистрироватьВДругомРеестре;
		Иначе
			Форма.Элементы.ФормаОтправить.Видимость = Форма.ТехническийСтатус = ПредопределенноеЗначение("Перечисление.ТехническиеСтатусыМЧД.Подписана")
				Или Форма.РегистрироватьВДругомРеестре;
		КонецЕсли;
...
Процедура ПрочитатьСостояниеВДругомРеестре()
	...
	Элементы.ОписаниеСостоянияВДругомРеестре.Видимость = Ложь;
	...
	Если ПараметрыПереопределения.ПоказыватьРегистрироватьВРеестре Тогда
		...
		Элементы.РегистрироватьВДругомРеестре.Заголовок = ПараметрыПереопределения.ЗаголовокФлажкаРегистрации;
		Если Не ПараметрыПереопределения.ЗарегистрированаВРеестре Тогда
			Элементы.ДекорацияРегистрироватьВДругомРеестре.Заголовок = ПараметрыПереопределения.СостояниеРегистрации;
		Иначе
			Элементы.ДекорацияРегистрироватьВДругомРеестре.Заголовок = "";
		КонецЕсли;
		
		Если Не РеквизитыНедоступныДляРедактирования И ЗначениеЗаполнено(ПараметрыПереопределения.КодыПолномочийДляДобавления) Тогда
	...
	Если ПараметрыПереопределения.ЗарегистрированаВРеестре Тогда
		...
		Элементы.ОписаниеСостоянияВДругомРеестре.Видимость = Истина;
		Элементы.ОписаниеСостоянияВДругомРеестре.Заголовок = ПараметрыПереопределения.СостояниеРегистрации;
		Если РегистрироватьВДругомРеестре = Истина Тогда
			РегистрироватьВДругомРеестре = Ложь;
			ДоступностьПризнакаРегистрацииВРеестре(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
