...
Функция ЗаполнениеПроверено()
	...
	Для Каждого Строка Из Объект.Доверители Цикл
		...
		Если Не РеквизитыОрганизации.ЭтоФизическоеЛицо Тогда
			СтрокаОшибки = НСтр("ru = 'Организация %1 - %2'");
		Иначе
			СтрокаОшибки = НСтр("ru = 'Физическое лицо %1 - %2'");
		КонецЕсли;

		ШаблонОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки,
			Строка.Доверитель, " %1: ");
...
Функция ПроверитьДублированиеСтрокИДанныеПервоначальнойДоверенности()
	...
	Если ЗначениеЗаполнено(Объект.РодительскаяДоверенность) Тогда
		...
		РеквизитыРодительскойДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.РодительскаяДоверенность, "Представители, ДатаОкончания");
		...
		Для Каждого Строка Из ПредставителиРодительскойДоверенности Цикл
			...
			Если Не РеквизитыОрганизации.ЭтоФизическоеЛицо Тогда
				ШаблонОшибки = НСтр("ru = 'Организация %1 - %2'");
			...
			Иначе
				ШаблонОшибки = НСтр("ru = 'Физическое лицо %1 - %2'");
	...
	Для Каждого Строка Из Объект.Доверители Цикл
		...
		ПредставлениеДоверителя = ПредставлениеДоверителяПредставителяДляОшибки(Строка.Доверитель, ИНН, КПП);
		...
		Если Не ЗначениеЗаполнено(Строка.ИдентификаторРодителя) Тогда
			...
			Если ЗначениеЗаполнено(ИНН) И ТаблицаДляПроверкиПервоначальнойДоверенности.Количество() > 0 Тогда
				...
				Если ТаблицаДляПроверкиПервоначальнойДоверенности.НайтиСтроки(
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПредставлениеДоверителя, НСтр(
	...
	Для Каждого Строка Из Объект.Представители Цикл
		...
		ПредставлениеПредставителя = ПредставлениеДоверителяПредставителяДляОшибки(Строка.Представитель, ИНН, КПП);
...
&НаСервере
Функция ПредставлениеДоверителяПредставителяДляОшибки(Ссылка, ИНН, КПП)
	Если ЗначениеЗаполнено(Ссылка) Тогда
		
		Если ЗначениеЗаполнено(КПП) Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (ИНН: %2, КПП %3)'"), Ссылка, ИНН, КПП);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (ИНН: %2)'"), Ссылка, ИНН);
		КонецЕсли;
	Иначе
		
		Если ЗначениеЗаполнено(КПП) Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'с ИНН: %1, КПП %2'"), ИНН, КПП);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'с ИНН: %1'"), ИНН);
		КонецЕсли;
...
Процедура ОтобразитьДанныеПервоначальнойДоверенности()
	...
	Если ЗначениеЗаполнено(Объект.ФайлДоверенности) Тогда
		...
		Доверители = Справочники.МашиночитаемыеДоверенности.ДоверителиПервоначальнойДоверенностиПоФайлу(Объект.ФайлДоверенности);
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.РодительскаяДоверенность) Тогда
	...
	Иначе
		...
		Элементы.ДоверительПервоначальнойДоверенностиНаименование.Заголовок =
