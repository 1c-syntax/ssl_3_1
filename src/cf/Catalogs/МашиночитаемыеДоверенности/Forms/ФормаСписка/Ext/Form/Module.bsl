...
Процедура ДоверенностиПоРолиПриИзменении(Элемент)
	...
	Если Элементы.ДоверенностиПоРоли.Видимость = Ложь Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДоверительПредставительАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ПодходящиеЗначения.Вставить(0, Текст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Поля доверенности содержат: %1'"), Текст));
...
Функция ФизическоеЛицоПользователя(Знач АвторизованныйПользователь)
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(АвторизованныйПользователь, "ФизическоеЛицо");
КонецФункции 
...
Функция ПодходящиеЗначенияУниверсальногоПоиска(Запрос)
	...
	МаксимальноеКоличествоПодходящихЗначений = 49;
	...
	Если ПодходящиеЗначения.Количество() = 0 Тогда
		...
		Если СтрДлина(ПараметрыПоиска.СтрокаПоиска) > 2 Тогда
			Сообщение = НСтр("ru = 'Совпадений в справочниках не найдено'");
		Иначе
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			...
			СвойстваПоиска = СвойстваУниверсальногоПоиска(
				РеквизитПоля, ПараметрыПоиска.СтрокаПоиска);
...
Функция ШаблонФрагментаУсловия(Условие)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1<span style=""color: %2"">%3</span>%4'"), " ", "ПоясняющийТекст", 
