...
Процедура ДокументВидПриИзменении(Элемент)
	...
	Элементы.ДокументНомер.Маска = МаскаНомераДокумента(ДокументВид);
...
&НаКлиенте
Процедура ПредставлениеАдресаНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка, ИмяРеквизита, 
	ЗаголовокФормы)
...
Функция ПодготовитьИзмененияНаСервере()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		Если ЗначениеЗаполнено(ЭлектроннаяПочта) Тогда
			Если Не ЗначениеЗаполнено(ЭлектроннаяПочтаЗначение) Тогда
				МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
				...
				ЭлектроннаяПочтаЗначение = МодульУправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(
					ЭлектроннаяПочта, ПараметрыВидаКонтактнойИнформации.Тип);
			КонецЕсли;
		Иначе
			ЭлектроннаяПочтаЗначение = "";
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВидовДокументов()
	...
	Для Каждого ВидДокумента Из МашиночитаемыеДоверенностиФНССлужебный.ВидыДокументовУдостоверяющихЛичность() Цикл
		...
		МаскиНомеровДокументов.Добавить(ВидДокумента.Код, ВидДокумента.Маска);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Функция МаскаНомераДокумента(ДокументВид)
	
	Маска = МаскиНомеровДокументов.НайтиПоЗначению(ДокументВид);
	Если Маска <> Неопределено Тогда
		Возврат Маска.Представление;
	КонецЕсли;
