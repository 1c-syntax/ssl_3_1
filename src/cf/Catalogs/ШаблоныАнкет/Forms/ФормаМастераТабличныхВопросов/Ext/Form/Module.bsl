...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ЭлементМетаданных Из Метаданные.Перечисления.ТипыТабличныхВопросов.ЗначенияПеречисления Цикл
		Элементы.ТипТабличногоВопроса.СписокВыбора.Добавить(Перечисления.ТипыТабличныхВопросов[ЭлементМетаданных.Имя],
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если Не ВыполняетсяЗакрытие И ЭтоНоваяСтрока Тогда
		Оповестить("ОтменаВводаНовойСтрокиШаблонаАнкеты", , ВладелецФормы);
	КонецЕсли;
...
Процедура СформироватьРезультирующуюТаблицу()
	...
	Анкетирование.ОбновитьПревьюТабличныйВопрос(РеквизитФормыВЗначение("Вопросы"), Ответы, ТипТабличногоВопроса,
		ЭтотОбъект, "РезультирующаяТаблица", "");
	Элементы.КнопкаВперед.Заголовок = НСтр("ru = 'Готово'");
...
Процедура ЗакончитьРедактированиеИЗакрыть()
	...
	Оповестить("ОкончаниеРедактированияПараметровТабличногоВопроса",
		СформироватьСтруктуруПараметровДляПередачиВладельцу(), ВладелецФормы);
	Закрыть();
