...
Процедура ЗаполнитьПолучателейРассылкиСАдресомЭлектроннойПочты(ТаблицаПолучателей)
	...
	Пока ВыборкаПолучатели.Следующий() Цикл
		...
		СтрокаПолучатели.Получатель = ВыборкаПолучатели.Получатель;
		МассивПолучателей.Добавить(СтрокаПолучатели.Получатель);
		Пока Выборка.Следующий() Цикл
			ТекущийАдрес = ?(ПустаяСтрока(СтрокаПолучатели.АдресЭлектроннойПочты), "",
				СтрокаПолучатели.АдресЭлектроннойПочты + "; ");
			СтрокаПолучатели.АдресЭлектроннойПочты = ТекущийАдрес + Выборка.EMail;
		КонецЦикла;
	КонецЦикла;	
