...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	УстановитьВидимостьДоступностьПаролейСертификатов(ЭтотОбъект);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.ПрисоединяемыеОтчеты) = Тип("Массив") Тогда
	...
	Если ЭтоНовый И Не СозданКопированием Тогда
		...
		ЗаполнитьРасписаниеПоВарианту(Неопределено);
	Иначе
		ПрочитатьРасписаниеРегламентногоЗадания();
	КонецЕсли;
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект);
	ДобавитьКомандыДобавленияДополнительныхПараметровТекста();
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПодключитьОбработчикОжидания("ОбновитьЗначениеПараметраПолучателяПерсонализированнойРассылки", 0.1, Истина);
КонецПроцедуры
...
Процедура ПодготовленаПриИзменении(Элемент)
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "Подготовлена");
КонецПроцедуры

#Область СтраницаРасписание

&НаКлиенте
Процедура ВыполнятьПоРасписаниюПриИзменении(Элемент)
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ВыполнятьПоРасписанию");
КонецПроцедуры
...
Процедура МесяцыПриИзменении(Элемент)
	...
	КонецЕсли;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "Месяцы");
КонецПроцедуры
...
Процедура ДниНеделиПриИзменении(Элемент)
	...
	КонецЕсли;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ДниНедели");
КонецПроцедуры
...
Процедура ПериодичностьРасписанияПриИзменении(Элемент)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ПериодичностьРасписания");
...
Процедура ГиперссылкаНачалоКонецМесяцаНажатие(Элемент)
	...
	КонецЕсли;
	Модифицированность = Истина;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "НачалоКонецМесяца");
КонецПроцедуры

&НаКлиенте
Процедура ВремяНачалаПриИзменении(Элемент)
	Расписание.ВремяНачала = ВремяНачала;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ВремяНачала");
КонецПроцедуры
...
Процедура ИспользоватьПериодПовтораВТечениеДняПриИзменении(Элемент)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ПериодПовтораВТечениеДня");
КонецПроцедуры
...
Процедура ВремяОкончанияПриИзменении(Элемент)
	Расписание.ВремяКонца = ВремяОкончания;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ВремяОкончания");
КонецПроцедуры

&НаКлиенте
Процедура ПериодПовтораДнейПриИзменении(Элемент)
	Расписание.ПериодПовтораДней = ПериодПовтораДней;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ПериодПовтораДней");
КонецПроцедуры

&НаКлиенте
Процедура ДеньМесяцаПриИзменении(Элемент)
	Расписание.ДеньВМесяце = ?(Расписание.ДеньВМесяце >= 0, ДеньВМесяце, -ДеньВМесяце);
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ДеньВМесяце");
КонецПроцедуры
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ПолучателиРассылки");
...
Процедура FTPСерверИКаталогОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "FTPСерверИКаталог");
	Модифицированность = Истина;
КонецПроцедуры
...
Процедура ВидРассылкиПриИзменении(Элемент)
	...
	ИначеЕсли Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
		Если Элементы.ТипПолучателейРассылки.СписокВыбора.Количество() > 0 Тогда
			ТипПолучателейРассылки = Элементы.ТипПолучателейРассылки.СписокВыбора[0].Значение;
			ТипПолучателейРассылкиПриИзменении(Элементы.ТипПолучателейРассылки);
		КонецЕсли;
	КонецЕсли;
	
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ВидРассылки");
	ДобавитьКомандыДобавленияДополнительныхПараметровТекста();
...
Процедура ИспользоватьЭлектроннуюПочтуПриИзменении(Элемент)
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ИспользоватьЭлектроннуюПочту");
	
	Если Не Публиковать И Не Объект.ИспользоватьЭлектроннуюПочту Тогда
		Публиковать = Истина;
		ВычислитьФлажкиДополнительныхСпособовДоставки();
		УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "Публиковать");
...
Процедура ТолькоУведомитьПриИзменении(Элемент)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ТолькоУведомить");
КонецПроцедуры

&НаКлиенте
Процедура ДругойСпособДоставкиПриИзменении(Элемент)
	ВычислитьФлажкиДополнительныхСпособовДоставки();
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ДругойСпособДоставки");
КонецПроцедуры
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недостаточно прав для изменения файлов папки ""%1"".'"), 
...
Процедура АрхивироватьПриИзменении(Элемент)
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "Архивировать");
...
Процедура ИмяАрхиваОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = "НаименованиеДатаРассылки" Тогда
		...
		Переменные = ПеременныеВыбораШаблонаНаименованияОтчета("НаименованиеДатаРассылки", Ложь);
		Переменные.Элемент = ТекущийЭлемент;
		Переменные.Префикс = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 от %2'"), "[" + ПараметрыТекстаПисьмаИФайлов.НаименованиеРассылки + "]",
			"[" + ПараметрыТекстаПисьмаИФайлов.ДатаВыполнения + "(");
		Переменные.Постфикс = ")]";
		Переменные.ИзменятьШаблонНаименованияОтчета = Ложь;
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "Отчеты");
КонецПроцедуры
...
Процедура ОбработчикАктивизацииСтрокиТаблицыОтчеты()
	...
	ОбновитьЗначениеПараметраПолучателяПерсонализированнойРассылки();
...
Процедура УказатьПолучателяРассылки(Команда)
	...
	Если Настройка.СКД Тогда
		...
		Иначе
			ДоступныйПараметр = КомпоновщикНастроекКД.Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Настройка.СтрокаНастроек.Параметр);
			Если ДоступныйПараметр <> Неопределено И ДоступныйПараметр.ДоступенСписокЗначений Тогда
				ЗначениеСписком = Новый СписокЗначений;
				ЗначениеСписком.Добавить(ШаблонЗначенияПолучателяРассылки(ПараметрыТекстаПисьмаИФайлов),
					ШаблонЗначенияПолучателяРассылки(ПараметрыТекстаПисьмаИФайлов));
				Настройка.СтрокаНастроек.Значение = ЗначениеСписком;
			Иначе	
...
Процедура ДобавитьИзменитьШаблонДатыРассылки(Команда)
	...
	Переменные = ПеременныеВыбораШаблонаНаименованияОтчета("", Ложь);
	Переменные.Элемент = ТекущийЭлемент;
	Переменные.СтарыйТекст = Переменные.Элемент.ВыделенныйТекст;
	Переменные.Префикс = "[" + ПараметрыТекстаПисьмаИФайлов.ДатаВыполнения + "(";
	Переменные.Постфикс = ")]";
	Переменные.ИзменятьШаблонНаименованияОтчета = Ложь;
	...
	Переменные.СтарыйФрагментНайден = (ПозицияПрефикса > 0 И ПозицияПостфикса > ПозицияПрефикса);
...
Процедура ИзменитьТипТекстаНаHTML(Команда)
	...
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ПисьмоВФорматеHTML");
КонецПроцедуры
...
Процедура ВставлятьОтчетыВТекстПисьмаПриИзменении(Элемент)
	...
	Иначе
		Объект.ПрикреплятьОтчетыВоВложения = Истина;
		УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ПрикреплятьОтчетыВоВложения");
	КонецЕсли;
...
Процедура СброситьФорматПоУмолчанию(Команда)
	...
	ФорматыПоУмолчанию = СписокФорматовПоУмолчаниюПредставление;
	УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "ФорматыПоУмолчанию");
КонецПроцедуры
...
Процедура ДобавитьИзменитьШаблонДатыРассылкиЗавершение(СтрокаРезультат, Переменные) Экспорт
	...
	Если Переменные.ИзменятьШаблонНаименованияОтчетаМножественно Тогда
		Если Переменные.СтарыйФрагментНайден Тогда
	...
	ИначеЕсли Переменные.ИзменятьШаблонНаименованияОтчета Тогда
		Если Переменные.СтарыйФрагментНайден Тогда
...
Процедура ПослеИзмененияРасписания(РасписаниеРезультат, ДополнительныеПараметры) Экспорт
	Если РасписаниеРезультат <> Неопределено Тогда
		...
		УстановитьВидимостьДоступностьКорректность(ЭтотОбъект, "Расписание");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверкаШифрованияПередВключениемОтчетовВТелеПисьма()
	...
	Если Объект.Персонализирована И Объект.УстановитьПаролиЗашифровать Тогда
		...
		КонецЕсли;
		Если ДоступноШифрованиеВложений Или Объект.Архивировать Тогда
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
...
//
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Иначе
			Элементы.ПолучателиРассылки.Видимость = ЗначениеЗаполнено(Форма.ТипПолучателейРассылки);
			Если Не РассылкаОбщая Тогда
		...
		УстановитьВидимостьДоступностьПаролейСертификатов(Форма);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПолучателиРассылки" Или Изменения = "ВидРассылки" Тогда
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		Если Объект.ПериодичностьРасписания = ПредопределенноеЗначение("Перечисление.ПериодичностиРасписанийРассылокОтчетов.Ежедневно") Тогда
			...
			ИмяВидимойСтраницы = Элементы.СтраницаЕжедневно.Имя;
		ИначеЕсли Объект.ПериодичностьРасписания = ПредопределенноеЗначение("Перечисление.ПериодичностиРасписанийРассылокОтчетов.Еженедельно") Тогда
			...
			ИмяВидимойСтраницы = Элементы.СтраницаЕженедельно.Имя;
		ИначеЕсли Объект.ПериодичностьРасписания = ПредопределенноеЗначение("Перечисление.ПериодичностиРасписанийРассылокОтчетов.Ежемесячно") Тогда
			...
			ИмяВидимойСтраницы = Элементы.СтраницаЕжемесячно.Имя;
		Иначе
			ИмяПеречисления    = "Произвольное";
			ИмяВидимойСтраницы = "";
		КонецЕсли;
		
		Для Каждого Страница Из Элементы.СтраницыПериодичности.ПодчиненныеЭлементы Цикл
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьДоступностьПаролейСертификатов(Форма)
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
...
&НаКлиенте
Процедура ОбновитьЗначениеПараметраПолучателяПерсонализированнойРассылки()
	
	Если НЕ Объект.Персонализирована Или Элементы.Отчеты.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ШаблонЗначенияПолучателя = ШаблонЗначенияПолучателяРассылки(ПараметрыТекстаПисьмаИФайлов);
	ЗначениеСписком = Новый СписокЗначений;
	ЗначениеСписком.Добавить(ШаблонЗначенияПолучателя, ШаблонЗначенияПолучателя);

	Для каждого ЭлементНастройки Из КомпоновщикНастроекКД.ПользовательскиеНастройки.Элементы Цикл
		Если Не ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		Если ЭлементНастройки.Значение = ШаблонЗначенияПолучателя 
			Или ТипЗнч(ЭлементНастройки.Значение) = Тип("СписокЗначений")
			И ЭлементНастройки.Значение.НайтиПоЗначению(ШаблонЗначенияПолучателя) <> Неопределено Тогда
			ДоступныйПараметр = КомпоновщикНастроекКД.Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(ЭлементНастройки.Параметр);
			Если ДоступныйПараметр <> Неопределено И ДоступныйПараметр.ДоступенСписокЗначений Тогда
				ЭлементНастройки.Значение = ЗначениеСписком;
				Элементы.Отчеты.ТекущиеДанные.ВнесеныИзменения = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		Если ТипЗнч(СтрокаОтчетыПараметры.ВариантСсылка) = Тип("СправочникСсылка.ВариантыОтчетов")
...
Функция ПараметрыФормыПредпросмотраШаблона()
	...
	Если Объект.Архивировать Тогда
		...
		СтруктураИмениАрхива = Новый Структура("НаименованиеРассылки, ДатаВыполнения", Объект.Наименование,
			ТекущаяДатаСеанса());
		ИмяАрхива = РассылкаОтчетов.ЗаполнитьШаблон(Объект.ИмяАрхива, СтруктураИмениАрхива);
	...
	СпособДоставки = РассылкаОтчетов.ПредставлениеСпособовДоставки(ПараметрыДоставки);
	...
	Текст = РассылкаОтчетов.ЗаполнитьШаблон(Шаблон, ПараметрыШаблона);
...
#Область КопияКомандыВыполнитьСейчасДляПоддержкиАсинхронности
...
#Область КопияКомандыСобытияРассылкиДляПоддержкиАсинхронности
