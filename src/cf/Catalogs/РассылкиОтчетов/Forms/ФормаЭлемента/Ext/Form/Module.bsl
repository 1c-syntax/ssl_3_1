...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если СозданКопированием Тогда
			...
			Если АвторОснования = ТекущийПользователь Тогда
				...
				Пароли = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(РассылкаОснование, "ПарольАрхива, FTPПароль");
				УстановитьПривилегированныйРежим(Ложь);
				Если ЗначениеЗаполнено(Пароли.ПарольАрхива) Тогда
					ПарольАрхива = ПарольСкрыт();
	...
	Иначе
		...
		УстановитьПривилегированныйРежим(Ложь);
		ПарольАрхива = ?(ЗначениеЗаполнено(Пароли.ПарольАрхива), ПарольСкрыт(), "");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если РассылкаНеПерсонализирована И ОтчетПерсонализирован Тогда
			Отказ = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В настройках отчета ''%1'' задан отбор по получателю рассылки. Необходимо отключить этот отбор или изменить вид рассылки на ''Свой отчет для каждого получателя''.'"),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтруктураЗаполнения.Вставить("Форматы", ПредставлениеФорматовПоУмолчанию());
...
Процедура УказатьПолучателяРассылки(Команда)
	...
	Если Настройка.СКД Тогда
		Если Настройка.ЭтоЭлементОтбора Тогда
			...
			КонецЕсли;
			Настройка.СтрокаНастроек.ПравоеЗначение = "[Получатель]";
		Иначе
			Настройка.СтрокаНастроек.Значение = "[Получатель]";
		КонецЕсли;
	Иначе
		Настройка.СтрокаНастроек.Значение = "[Получатель]";
	КонецЕсли;
...
Процедура УдалитьПолучателяРассылки(Команда)
	...
	Если Настройка.СКД Тогда
		Если Настройка.ЭтоЭлементОтбора Тогда
			Если Настройка.СтрокаНастроек.ПравоеЗначение = "[Получатель]" Тогда
				Настройка.СтрокаНастроек.ПравоеЗначение = Неопределено;
				ВнесеныИзменения = Истина;
			КонецЕсли;
		Иначе
			Если Настройка.СтрокаНастроек.Значение = "[Получатель]" Тогда 
...
Процедура ПредпросмотрШаблона(Команда)
	...
	Для Каждого СтрокаОтчет Из Объект.Отчеты Цикл
		...
		+ ")";
	КонецЦикла;
	СформированныеОтчеты = СокрЛ(СформированныеОтчеты);
	
	ПараметрыДоставки = Новый Структура("ТолькоУведомить, ПисьмоВФорматеHTML, Папка, СетевойКаталогWindows, Сервер, Порт, Каталог");
	ЗаполнитьЗначенияСвойств(ПараметрыДоставки, Объект);
	ПараметрыДоставки.Вставить("ВыполненаВПапку", Объект.ИспользоватьПапку);
	ПараметрыДоставки.Вставить("ВыполненаВСетевойКаталог", Объект.ИспользоватьСетевойКаталог);
	ПараметрыДоставки.Вставить("ВыполненаНаFTP", Объект.ИспользоватьFTPРесурс);
...
#Если НЕ МобильныйКлиент Тогда
	...
	ТекстовыйДокумент.УстановитьТекст(РассылкаОтчетовКлиентСервер.ЗаполнитьШаблон(Шаблон, ПараметрыШаблона));
...
Процедура ПроверитьВозможностьРассылки(Команда)
	...
	ПараметрыДоставки.Вставить("ИспользоватьЭлектроннуюПочту", Объект.ИспользоватьЭлектроннуюПочту);
...
//     * Получатели - ТаблицаЗначений - где:
...
//   Параметр - Структура, Неопределено - 
//
&НаКлиенте
...
Функция ВыборПодборПеретаскиваниеЭлементаВТабличнуюЧасть(ЭлементПодбора, ТабличнаяЧасть, ИмяРеквизита, СтруктураЗаполнения, Уникальность = Истина)
	...
	Если (ТипЗнч(ЭлементПодбора) = Тип("Структура")) Или ТипЗнч(ЭлементПодбора) = Тип("ДанныеФормыЭлементКоллекции") Тогда
	...
	Если (ТипЗнч(ЭлементПодбора) = Тип("Структура")) Тогда
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ДругойСпособДоставки" Или Изменения = "Публиковать" Или Изменения = "ВидРассылки" Тогда
		...
		Элементы.ВключатьДатуВИмяФайла.Доступность = Форма.Публиковать;
	...
	Если Изменения = "" Или Изменения = "ФорматыПоУмолчанию" Тогда
		Элементы.ФорматыПоУмолчанию.КнопкаОчистки = (Форма.ФорматыПоУмолчанию <> Форма.СписокФорматовПоУмолчаниюПредставление);
	КонецЕсли;
...
Функция ПочтовыеАдресаПолучателя(Получатель, СписокЗначений)
	...
	Для Каждого ПочтовыйАдрес Из ПочтовыеАдреса Цикл
		Если ЗначениеЗаполнено(ПочтовыйАдрес.Представление) Тогда
			СписокЗначений.Добавить(ПочтовыйАдрес.Вид, ПочтовыйАдрес.Представление + " (" + Строка(ПочтовыйАдрес.Вид) + ")");
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыДоставки.Вставить("РежимТестирования",           Истина);
...
Процедура УстановитьУсловноеОформление()
	...
	// Оформление получателей рассылки
...
Функция ПолучитьКэш()
	...
	Шаблоны = Новый ФиксированнаяСтруктура("Тема, Текст, ИмяАрхива",
		РассылкаОтчетов.ШаблонТемы(),
		РассылкаОтчетов.ШаблонТекста(),
		РассылкаОтчетов.ШаблонИмениАрхива());
