...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.Переместить(Элементы.ФорматыПоУмолчанию, Элементы.ГруппаФорматыОтчетов, Элементы.ГруппаШапкаФорматыОтчетов);
...
Процедура ПредпросмотрШаблона(Команда)
	...
	Для Каждого СтрокаОтчет Из Объект.Отчеты Цикл
		СформированныеОтчеты = СформированныеОтчеты + Символы.ПС 
	...
	Если Форма.ДоступноШифрованиеВложений Тогда
		Если Объект.Персонализирована Тогда
			...
			Если Объект.Архивировать Тогда
				Элементы.ПаролиШифрование.Заголовок = НСтр("ru = 'Пароли и шифрование'");
			Иначе
				Элементы.ПаролиШифрование.Заголовок = НСтр("ru = 'Шифрование'");
			КонецЕсли;
			Элементы.ПаролиШифрование.Видимость   = Истина;
		...
		Иначе
			...
			Элементы.ПаролиШифрование.Видимость = Ложь;
		КонецЕсли;
	Иначе
		Если Объект.Персонализирована Тогда
			Элементы.ПаролиШифрование.Заголовок = НСтр("ru = 'Пароли'");
			Элементы.ПаролиШифрование.Видимость   = Истина;
			Элементы.ГруппаПарольАрхива.Видимость = Ложь;
			Элементы.ПаролиШифрование.Доступность = Объект.Архивировать;
...
Функция ПредставлениеПолучателей(Форма)
	...
	КоличествоПолучателей = КоличествоПолучателейСУчетомГрупп(Форма.Объект.Получатели,
		Форма.Объект.ТипПолучателейРассылки);
	...
	Если КоличествоПолучателей.Исключены > 0 Тогда
...
Функция КоличествоПолучателейСУчетомГрупп(Знач Получатели, Знач ИдентификаторОбъектаМетаданных)

	Возврат Справочники.РассылкиОтчетов.КоличествоПолучателейСУчетомГрупп(Получатели, ИдентификаторОбъектаМетаданных);
...
Процедура ЗаполнитьРасписаниеПоВарианту(Вариант, ОбновитьВидимость = Ложь)
	...
	Объект.ПериодичностьРасписания = Перечисления.ПериодичностиРасписанийРассылокОтчетов.Ежедневно; // "Каждый день"
...
Процедура СоздатьРеквизитЭлементСертификатДляШифрования()
	...
	Группа = Элементы.Добавить("ГруппаСертификатДляШифрования", Тип("ГруппаФормы"), Элементы.СтраницаДополнительно);
	Группа.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	Группа.Заголовок = НСтр("ru = 'Группа сертификат для шифрования'");
	...
	Элементы.Переместить(Группа, Элементы.СтраницаДополнительно, Элементы.ДополнительныеПараметры);
