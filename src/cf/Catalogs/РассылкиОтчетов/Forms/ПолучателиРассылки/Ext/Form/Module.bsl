...
Процедура ЗаполнитьПочтовыеАдреса()
	...
	ПараметрыПолучателей = Новый Структура("Ссылка, ВидПочтовогоАдресаПолучателей, Личная, Получатели, ТипПолучателейРассылки");
	...
	Для каждого ПолучательРассылки Из Получатели Цикл
		...
		КонецЕсли;
		Если ПолучательРассылки.Получатель.ЭтоГруппа Или ТипЗнч(ПолучательРассылки.Получатель) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
...
Процедура ОбновитьКоличествоПолучателей(Форма)
	...
	КоличествоПолучателей = КоличествоПолучателейСУчетомГрупп(Форма.Получатели,
		Форма.ИдентификаторОбъектаМетаданных);
