...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененыНастройкиТранспорта" Тогда
		Отбор = Новый Структура("ИдентификаторТранспорта", Параметр.ИдентификаторТранспорта);
		...
		Строка.ОбязательныеРеквизитыНастройкиЗаполнены = Параметр.ОбязательныеРеквизитыНастройкиЗаполнены;
	КонецЕсли;
...
Процедура ИспользоватьПоУмолчанию(Команда)
	...
	Если Не ТекущиеДанные.ЕстьНастройки Тогда
		...
		Текст = НСтр("ru = 'Невозможно установить тип транспорта, который не настроен используемым по умолчанию.'",
...
Процедура ИнициализацияРеквизитовФормы()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Строка.ЕстьНастройки Тогда
			Строка.ОбязательныеРеквизитыНастройкиЗаполнены = ТранспортСообщенийОбмена.ЗаполненыОбязательныеРеквизитыНастройкиТранспорта(Выборка.Настройки, Выборка.ИдентификаторТранспорта);
		ИначеЕсли Строка.НастройкаПоУмолчанию Тогда	
			Строка.ОбязательныеРеквизитыНастройкиЗаполнены = Истина;
		Иначе
			Строка.ОбязательныеРеквизитыНастройкиЗаполнены = Ложь;
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТипыТранспорта.ОбязательныеРеквизитыНастройкиЗаполнены");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТипыТранспорта.НастройкаПоУмолчанию");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;	
...
Процедура СохранитьНастройкиТранспорта(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбязательныеРеквизитыНастройкиЗаполнены = Истина;
	
	ТранспортСообщенийОбменаВызовСервера.ОбработатьИзменениеНастроекТранспорта(
		ДополнительныеПараметры.Корреспондент,
		ДополнительныеПараметры.ИдентификаторТранспорта,
		Результат,
		ОбязательныеРеквизитыНастройкиЗаполнены);
		
	ТекущиеДанные = Элементы.ТипыТранспорта.ТекущиеДанные;
	ТекущиеДанные.ЕстьНастройки = Истина;
	ТекущиеДанные.ОбязательныеРеквизитыНастройкиЗаполнены = ОбязательныеРеквизитыНастройкиЗаполнены;
