...
&НаКлиенте
Процедура ТаблицаПользователейБезИспользованияКлючаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.Пометка = Ложь Тогда
		ТекущиеДанные.БезИспользованияКлюча = Не ТекущиеДанные.БезИспользованияКлюча;
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо сначала выбрать пользователя.'"));
	КонецЕсли;
...
Процедура ОК(Команда)
	...
	Иначе	
			...
			Если Элементы.ТаблицаПользователей.Видимость Тогда
				Для Каждого СтрокаПользователя Из ТаблицаПользователей Цикл
					Если СтрокаПользователя.Пометка Тогда
						...
						ЗаполнитьЗначенияСвойств(ДанныеПользователя, СтрокаПользователя);
						Результат.Пользователи.Добавить(ДанныеПользователя);
					КонецЕсли;
				КонецЦикла;
			Иначе
		...
		Если Результат.Пользователи.Количество() = 1 Тогда
			Результат.Пользователь = Результат.Пользователи[0].Пользователь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПолныйСписок(ПользователиСертификата, ВладелецСертификата)
	...
	ИначеЕсли ЗначениеЗаполнено(ВладелецСертификата) И СпособВыбора = "СписокПользователей" Тогда
		ПараметрыДобавления = Новый Структура("Пользователь, БезИспользованияКлюча", ВладелецСертификата, Ложь);
		МассивПользователей.Добавить(ПараметрыДобавления);
	КонецЕсли;
	
	ПараметрПользователи = Новый Массив;
	Для Каждого ДанныеПользователя Из МассивПользователей Цикл
		ПараметрПользователи.Добавить(ДанныеПользователя.Пользователь);
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("Пользователи", ПараметрПользователи);
	...
	Если Результат.Количество() > 30 Тогда
		...
		Для Каждого ДанныеПользователя Из МассивПользователей Цикл
			...
			Если ДанныеПользователя.Пользователь = ВладелецСертификата Тогда
				НоваяСтрока.Основной = Истина;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		Для Каждого ДанныеПользователя Из МассивПользователей Цикл
			...
			ПараметрыПоиска.Вставить("Пользователь", ДанныеПользователя.Пользователь);
			СтрокаТаблицы = ТаблицаПользователей.НайтиСтроки(ПараметрыПоиска);
			Если СтрокаТаблицы.Количество() > 0 Тогда
				СтрокаТаблицы[0].БезИспользованияКлюча = ДанныеПользователя.БезИспользованияКлюча;
			КонецЕсли;
		КонецЦикла;
