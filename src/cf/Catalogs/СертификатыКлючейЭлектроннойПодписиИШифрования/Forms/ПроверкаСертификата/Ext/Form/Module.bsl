...
Процедура Подключаемый_НадписьНажатие(Элемент)
	...
	ПараметрыПредупреждения.Вставить("ТекстОшибкиКлиент",
		ПроверкиНаКлиенте[Лев(Элемент.Имя, СтрДлина(Элемент.Имя) - 7) + "Ошибка"]);
	
	ТекстОшибкиСервер = ?(Не ОперацииНаСервере, "",
		ПроверкиНаСервере[Лев(Элемент.Имя, СтрДлина(Элемент.Имя) - 7) + "Ошибка"]);
	ПараметрыПредупреждения.Вставить("ТекстОшибкиСервер", ТекстОшибкиСервер);
...
Процедура ИнформацияДляПоддержкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	ТекстОшибок = НСтр("ru = 'Результат поверки на клиенте'") + ":" + Символы.ПС;
	ДополнитьТекстОшибками(ТекстОшибок, СоставПроверок);
	
	ТекстОшибок = ТекстОшибок + Символы.ПС
		+ НСтр("ru = 'Результат поверки на сервере'") + ":" + Символы.ПС;
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
		ТекстОшибок, Неопределено, ДанныеСертификата(Сертификат));
...
Процедура ДобавитьГруппуОписанияРешения(ИмяПроверки, Родитель, НаСервере = Ложь)
	...
	Суффикс = ?(НаСервере, "Сервер", "Клиент");
	
	ГруппаРешение = ГруппаФормыБезОтображения(
		ИмяПроверки + "Решение" + Суффикс, Родитель);
	
	ДобавитьКартинку(ИмяПроверки + "Решение" + Суффикс + "Картинка",
		ГруппаРешение, БиблиотекаКартинок["Компьютер" + Суффикс]);
	
	Надпись = Элементы.Добавить(ИмяПроверки + "Решение" + Суффикс + "Надпись",
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере,
	...
	ЭлементНадпись = Форма.Элементы[НачалоЭлемента + "Надпись"];
	Если ОбъединятьРезультаты = "ОбъединятьПоИ"
		Или ОбъединятьРезультаты = "ОбъединятьПоИли" Тогда
		
		ЭлементКартинка = Форма.Элементы[НачалоЭлемента + "НаКлиентеКартинка"];
	Иначе
		ЭлементКартинка = Форма.Элементы[НачалоЭлемента
			+ ?(НаСервере, "НаСервере", "НаКлиенте") + "Картинка"];
	КонецЕсли;
		
	Проверки = Форма["Проверки" + ?(НаСервере, "НаСервере", "НаКлиенте")];
	ПроверкиВторогоКонтекста = Форма["Проверки" + ?(НаСервере, "НаКлиенте", "НаСервере")];
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не РезультатПроверки Тогда
			...
			ЭлементКартинка.Картинка = ?(ЭтоОшибка,
				БиблиотекаКартинок.КрасныйШар, БиблиотекаКартинок.ЖелтыйШар);
			...
			ИмяГруппыРешение = НачалоЭлемента + "Решение" + ?(НаСервере, "Сервер", "Клиент");
			Форма.Элементы[ИмяГруппыРешение].Видимость = ЭтоИзвестнаяОшибка;
			Если ЭтоИзвестнаяОшибка Тогда
				Форма.Элементы[ИмяГруппыРешение + "Надпись"].Заголовок = ОписаниеИзвестнойОшибки.Решение;
			КонецЕсли;
	...
	Иначе
		...
		ЭлементРешение = Форма.Элементы[НачалоЭлемента
			+ "Решение" + ?(НаСервере, "Сервер", "Клиент")];
		...
		Если (ОбъединятьРезультаты = "ОбъединятьПоИ"
			...
			Если ЗначениеПроверки = Неопределено
				...
				ЭлементКартинка.Картинка = БиблиотекаКартинок.СерыйШар;
			...
			ИначеЕсли ЗначениеПроверки <> Неопределено
				...
				ЭлементРешение.Видимость = Ложь;
				ЭлементКартинка.Картинка = БиблиотекаКартинок.ЗеленыйШар;
		...
		Иначе
			...
			ЭлементКартинка.Картинка = ?(ОписаниеОшибки = Неопределено,
				БиблиотекаКартинок.СерыйШар, БиблиотекаКартинок.ЗеленыйШар);
...
Процедура ДополнитьТекстОшибками(ТекстОшибок, Проверки, НаСервере = Ложь)
	...
	СоставПроверок = ЭтотОбъект["Проверки" + ?(НаСервере, "НаСервере", "НаКлиенте")];
	Для Каждого Проверка Из Проверки Цикл
		
		Если СоставПроверок = Неопределено Тогда
			ТекстОшибок = ТекстОшибок + Проверка + ": "
				+ НСтр("ru = 'Не запускалась'") + Символы.ПС;
		Иначе
			...
			ТекстОшибок = ТекстОшибок + Проверка + ": "
				+ ?(РезультатПроверки = Неопределено, НСтр("ru = 'Не выполнялась'"),
