...
Процедура ПроверитьНаСторонеКлиентаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	// Подписание.
	Если ПроверкиНаКлиенте.НаличиеСертификата = Истина Тогда
...
Процедура ПроверитьНаСторонеКлиентаВМоделиСервиса(Результат, Контекст)
	...
	// Подписание.
	Если ПроверкиНаСервере.НаличиеСертификата = Истина Тогда
		МодульСервисКриптографииКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииКлиент");
...
Процедура ОбновитьВидимостьНаСервере()
	...
	Для Каждого Проверка Из СтандартныеПроверки() Цикл
		Элементы[Проверка + "НаСервереКартинка"].Видимость = ВидимостьПроверкиНаСервере;
	КонецЦикла;
	
	Для каждого Проверка Из ДополнительныеПроверки Цикл
		Элементы[Проверка.Имя + "НаСервереКартинка"].Видимость = ВидимостьПроверкиНаСервере;
	КонецЦикла;
	
	Элементы.ПроверкиНаСервереКартинка.Видимость = ВидимостьПроверкиНаСервере;
	Элементы.ПроверкиНаКлиентеКартинка.Видимость = ОбъединятьРезультаты <> "ОбъединятьПоИ"
		И ОбъединятьРезультаты <> "ОбъединятьПоИли";
