...
// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Асинх Процедура ПроверитьНаСторонеКлиентаПослеСозданияЛюбогоМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Токен = Ждать ЭлектроннаяПодписьСлужебныйКлиент.ПолучитьТокенПоСертификату(Контекст.СертификатКриптографии);
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		ДанныеПодписи = Ждать ЭлектроннаяПодписьСлужебныйКлиент.ПодписьНаТокене(ПараметрыПодписи, Контекст.ДанныеСертификата);
		...
		Иначе
			Если ЗначениеЗаполнено(ТипПодписи) И ТипПодписи <> ПредопределенноеЗначение(
				...
				ПараметрыПодписи.ТипПодписи = ТипПодписи;
				ДанныеПодписи = Ждать ЭлектроннаяПодписьСлужебныйКлиент.ПодписьНаТокене(ПараметрыПодписи, Контекст.ДанныеСертификата);
				Если ТипЗнч(ДанныеПодписи) = Тип("Строка") Тогда
					ОписаниеОшибки = ДанныеПодписи;
				КонецЕсли;
...
Асинх Процедура ПослеПроверкиПодписиНаТокене(Результат, Контекст) Экспорт
	...
	Если Результат <> Неопределено Тогда
		Если Не Результат.ПодписьВерна Тогда
	...
	ЗашифрованныеДанные = Ждать ЭлектроннаяПодписьКлиентЛокализация.ШифрованиеНаТокене(ПараметрыШифрования, Контекст.ДанныеСертификата);
	...
	Иначе
		...
		Иначе
			...
			ПараметрыРасшифровки = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыРасшифровкиНаТокене(Контекст.Токен,
				Контекст.СертификатКриптографии);
			РасшифрованныеДанные = Ждать ЭлектроннаяПодписьКлиентЛокализация.РасшифровкаНаТокене(ПараметрыРасшифровки, ЗашифрованныеДанные);
