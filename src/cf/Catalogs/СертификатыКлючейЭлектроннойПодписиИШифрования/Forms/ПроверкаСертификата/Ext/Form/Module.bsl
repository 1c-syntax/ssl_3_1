...
&НаКлиенте
Процедура СкачатьТехническуюИнформацию(Команда)
...
Процедура ВыгрузитьТехническуюИнформацию(ВыгрузитьАрхив)
	...
	Если ВыгрузитьАрхив Тогда
		Элементы.СкачатьТехническуюИнформацию.Доступность = Ложь;
		Элементы.ГруппаФормируетсяТехническаяИнформация.Видимость = Истина;
	КонецЕсли;
	...
	Если ВыгрузитьАрхив Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			ТекстОшибок, Неопределено, Новый ОписаниеОповещения("ПослеВыгрузкиТехническойИнформации", ЭтотОбъект), ОписаниеФайлов);
	Иначе
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
...
&НаКлиенте
Процедура ПослеВыгрузкиТехническойИнформации(Результат, Контекст) Экспорт
	
	Элементы.СкачатьТехническуюИнформацию.Доступность = Истина;
	Элементы.ГруппаФормируетсяТехническаяИнформация.Видимость = Ложь;
...
Процедура ПроверитьСертификатПослеПроверкиНаКлиенте(Результат, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(ПроверитьСертификатВЛокальномХранилищеКлиента) Тогда
	
		ОшибкаПоКлассификатору = Новый Структура;
		ОшибкаПоКлассификатору.Вставить("ТекстОшибки", Элементы[ПроверитьСертификатВЛокальномХранилищеКлиента + "ОшибкаКлиентНадпись"].Заголовок);
		ОшибкаПоКлассификатору.Вставить("Причина", "");
		ОшибкаПоКлассификатору.Вставить("Решение", Элементы[ПроверитьСертификатВЛокальномХранилищеКлиента + "РешениеКлиентНадпись"].Заголовок);
				
		ДанныеДляДополнения = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДанныеДляДополненияОшибкиИзКлассификатора(Неопределено);
		ДанныеДляДополнения.ДанныеСертификата = ПолучитьИзВременногоХранилища(АдресСертификата);
		
		ПараметрыДополнения = Новый Структура("Результат, Контекст, Элемент",
			Результат, Контекст, ПроверитьСертификатВЛокальномХранилищеКлиента);
			
		ПараметрыДополненияТекстаРешения = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПараметрыДополненияТекстаРешенияОшибкиКлассификатора();
		ПараметрыДополненияТекстаРешения.ПроверитьСертификатВЛокальномХранилищеКлиента = Истина;
			
		ЭлектроннаяПодписьСлужебныйКлиент.ДополнитьРешениеКлассификатораОшибокПодробностями(
			Новый ОписаниеОповещения("ПослеДополненияРешенияКлассификатораОшибок", ЭтотОбъект, ПараметрыДополнения),
			ОшибкаПоКлассификатору, ПараметрыДополненияТекстаРешения, ДанныеДляДополнения);
		ПроверитьСертификатВЛокальномХранилищеКлиента = "";
		Возврат;
		
	КонецЕсли;
	
	ПроверитьСертификатПослеПроверкиНаКлиентеИДополнений(Результат, Контекст);
		
КонецПроцедуры

// Продолжение процедуры ПроверитьСертификат.
&НаКлиенте
Процедура ПослеДополненияРешенияКлассификатораОшибок(ОшибкаПоКлассификатору, ПараметрыДополнения) Экспорт
	
	Если Элементы[ПараметрыДополнения.Элемент + "РешениеКлиентНадпись"].Заголовок <> ОшибкаПоКлассификатору.Решение Тогда
		Элементы[ПараметрыДополнения.Элемент + "РешениеКлиентНадпись"].Заголовок = ОшибкаПоКлассификатору.Решение;
	КонецЕсли;
	
	ПроверитьСертификатПослеПроверкиНаКлиентеИДополнений(ПараметрыДополнения.Результат, ПараметрыДополнения.Контекст);

КонецПроцедуры

// Продолжение процедуры ПроверитьСертификат.
&НаКлиенте
Процедура ПроверитьСертификатПослеПроверкиНаКлиентеИДополнений(Результат, Контекст)
...
Процедура УстановитьЭлемент(Форма, Действие, НаСервере,
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		ОписаниеИзвестнойОшибки = ОшибкаПоКлассификатору(ОписаниеОшибки, НаСервере, Действие = "ПроверкаПодписи");
		...
		Если ЗначениеЗаполнено(Форма.Сертификат) И ЗначениеЗаполнено(ОписаниеИзвестнойОшибки.ДействияДляУстранения) Тогда
			...
			Дополнение = ЭлектроннаяПодписьСлужебныйВызовСервера.ДополнитьРешениеКлассификатораОшибокПодробностями(
				ОписаниеИзвестнойОшибки, ДанныеДляДополнения, 
				ПараметрыДополненияТекстаРешенияОшибкиКлассификатора, ?(НаСервере, "Сервер", "Клиент"));
			ОписаниеИзвестнойОшибки = Дополнение.ОшибкаПоКлассификатору;
			...
			Если ПараметрыДополненияТекстаРешенияОшибкиКлассификатора.ПроверитьСертификатВЛокальномХранилищеКлиента Тогда
				Форма.ПроверитьСертификатВЛокальномХранилищеКлиента = Действие;
			КонецЕсли;
		КонецЕсли;
...
Функция ОшибкаПоКлассификатору(ОписаниеОшибки, ОшибкаНаСервере, ОшибкаПроверкиПодписи)
	
	Возврат ЭлектроннаяПодписьСлужебный.ОшибкаПоКлассификатору(ОписаниеОшибки, ОшибкаНаСервере, ОшибкаПроверкиПодписи);
