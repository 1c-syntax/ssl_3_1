...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не СтандартныеПроверки И Проверки.Количество() = 0 Тогда
	...
	Если ЗначениеЗаполнено(Параметры.ТипПодписи) Тогда
		ТипПодписи = Параметры.ТипПодписи;
	КонецЕсли;
	ИгнорироватьПроверкуВСпискеОтозванныхСертификатов = Параметры.ИгнорироватьПроверкуВСпискеОтозванныхСертификатов;
	ВыполнятьПроверкуУдостоверяющегоЦентра = Параметры.ВыполнятьПроверкуУдостоверяющегоЦентра;
	Если ПустаяСтрока(ВыполнятьПроверкуУдостоверяющегоЦентра) Тогда
		ВыполнятьПроверкуУдостоверяющегоЦентра = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПроверятьКвалифицированные();
	КонецЕсли;
	...
	Для каждого Проверка Из Проверки Цикл
		...
		Группа = ГруппаФормыБезОтображения("Группа" + Проверка.Имя, Элементы.ГруппаДополнительныеПроверки);
		...
		ГруппаРезультатИРешение = ГруппаФормыБезОтображения(Проверка.Имя + "РезультатИРешение", Группа,
			ГруппировкаПодчиненныхЭлементовФормы.Вертикальная);
			
		Надпись = Элементы.Добавить(Проверка.Имя + "Надпись", Тип("ДекорацияФормы"), ГруппаРезультатИРешение);
	...
	Иначе
		...
		ИменаГруппПоНазваниямПроверок = ИменаГруппПоНазваниямПроверок(Элементы);
		
		Для Каждого Проверка Из СтандартныеПроверки() Цикл
			
			Элементы[ИменаГруппПоНазваниямПроверок[Проверка]].Видимость = Ложь;
			...
			Элементы[Проверка + "Надпись"].Заголовок = СтандартныйЗаголовокЭлемента(Проверка + "Надпись", ТипПодписи);
	...
	ИначеЕсли ЭтоВстроенныйКриптопровайдер Или (Не СтандартныеПроверки И Не ВводитьПароль) Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("БСП_ВыполнитьПроверкуСертификата") 
		И Параметр = АдресСертификата Или ВРег(ИмяСобытия) = ВРег("Установка_КомпонентаExtraCryptoAPI") Тогда
...
Процедура Подключаемый_НадписьНажатие(Элемент)
	...
	ПараметрыПредупреждения.Вставить("ЗаголовокПредупреждения", СтандартныйЗаголовокЭлемента(Элемент.Имя, ТипПодписи));
	ПараметрыПредупреждения.Вставить("ТекстОшибкиКлиент", ПроверкиНаКлиенте[ИмяОшибки]);
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыПредупреждения, ЭтотОбъект);
...
Процедура Подключаемый_НадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрЗаканчиваетсяНа(Элемент.Имя, "ОшибкаСерверНадпись") Или СтрЗаканчиваетсяНа(Элемент.Имя, "ОшибкаКлиентНадпись") Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияСертификатМеткиВремениОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьСертификат(НавигационнаяСсылкаФорматированнойСтроки);
	
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияПроверкиУспешныНажатие(Элемент)
	...
	Если СтандартныеПроверки Тогда
		...
		Для каждого Проверка Из ОсновныеПроверки Цикл
			Если Проверка = "ЗаконныйСертификат" Тогда
				Продолжить;
			КонецЕсли;
			Элементы[ИменаГруппПоНазваниямПроверок[Проверка]].Видимость = Истина;
		КонецЦикла;
		
	КонецЕсли;
	
	Для каждого Проверка Из ДополнительныеПроверки Цикл
		Элементы["Группа" + Проверка.Имя].Видимость = Истина;
	КонецЦикла;
...
&НаКлиенте
Процедура ВидимостьПриОкончанииПроверки()
	Элементы.ФормаПроверить.Доступность = Истина;
	ВидимостьСтраниц(ЭтотОбъект);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ИменаГруппПоНазваниямПроверок(ЭлементыФормы)
	
	ИменаГруппПоНазваниямПроверок = Новый Соответствие;
		
	ИменаГруппПоНазваниямПроверок.Вставить("ЗаконныйСертификат", ЭлементыФормы.ГруппаЗаконныйСертификат.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("НаличиеСертификата", ЭлементыФормы.ГруппаНаличиеСертификата.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("ДанныеСертификата",  ЭлементыФормы.ГруппаДанныеСертификата.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("НаличиеПрограммы",   ЭлементыФормы.ГруппаНаличиеПрограммы.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("Подписание",         ЭлементыФормы.ГруппаПодписание.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("Шифрование",         ЭлементыФормы.ГруппаШифрование.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("Расшифровка",        ЭлементыФормы.ГруппаРасшифровка.Имя);
	ИменаГруппПоНазваниямПроверок.Вставить("ПроверкаПодписи",    ЭлементыФормы.ГруппаПроверкаПодписи.Имя);
	
	Возврат ИменаГруппПоНазваниямПроверок;
...
Процедура ВыгрузитьТехническуюИнформацию(ВыгрузитьАрхив)
	...
	Для Каждого СертификатМеткиВремени Из СертификатыМетокВремени Цикл
		Сертификаты.Добавить(СертификатМеткиВремени.АдресСертификата);
	КонецЦикла;
	
	ЭлектроннаяПодписьСлужебныйВызовСервера.ДобавитьОписаниеДополнительныхДанных(
		Новый Структура("Сертификат", Сертификаты), ОписаниеФайлов, ТекстОшибок);
		
	ТемаСообщения = "";
	ТекстОшибок = ТекстОшибок + НСтр("ru = 'Результат проверки на клиенте'") + ":" + Символы.ПС;
	ДополнитьТекстОшибками(ТекстОшибок, СоставПроверок, Ложь, ТемаСообщения);
	...
	ДополнитьТекстОшибками(ТекстОшибок, СоставПроверок, Истина, ТемаСообщения);
	...
	Иначе
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			ТекстОшибок, Новый Структура("Тема, Сообщение",
				?(ПустаяСтрока(ТемаСообщения), НСтр("ru = 'Проблема при проверке сертификата электронной подписи'"), ТемаСообщения),
				ЭлектроннаяПодписьСлужебныйКлиент.ТекстОбращенияВТехническуюПоддержкуПриПроверкеСертификата()),
...
Процедура ПроверитьЗавершение(Неопределен, Контекст) Экспорт
	...
	Иначе
		Массив = Новый Массив;
		Для Каждого СертификатМеткиВремени Из СертификатыМетокВремени Цикл
			Если Не СертификатМеткиВремени.ПолученНаКлиенте Тогда
				Массив.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(
							НСтр("ru = 'Сертификат метки времени, полученной на сервере: <a href = %1>%2</a>.'"),
						СертификатМеткиВремени.АдресСертификата, СертификатМеткиВремени.Представление));
			ИначеЕсли Не СертификатМеткиВремени.ПолученНаСервере Тогда
				Массив.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(
							НСтр("ru = 'Сертификат метки времени, полученной на компьютере: <a href = %1>%2</a>.'"),
						СертификатМеткиВремени.АдресСертификата, СертификатМеткиВремени.Представление));
			Иначе
				Массив.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(
							НСтр("ru = 'Сертификат полученной метки времени: <a href = %1>%2</a>.'"),
						СертификатМеткиВремени.АдресСертификата, СертификатМеткиВремени.Представление));
			КонецЕсли;
		КонецЦикла;
		Элементы.ДекорацияСертификатМеткиВремени.Заголовок = Новый ФорматированнаяСтрока(Массив);
		Элементы.ДекорацияСертификатМеткиВремени.Видимость = Истина;
	КонецЕсли;
...
Процедура ПроверитьСертификат(Оповещение)
	...
	СертификатыМетокВремени.Очистить();
	Элементы.ДекорацияСертификатМеткиВремени.Видимость = Ложь;
	
	ПроверкиУспешны = Истина;
	Элементы.ГруппаСтраницы.Видимость = Истина;
	Элементы.ФормаПроверить.Доступность = Ложь;
	Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаВыполняетсяПроверка;
...
Процедура ПроверитьНаСторонеКлиентаПослеСозданияЛюбогоМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииТокенаПоСертификату(
		Новый ОписаниеОповещения("ПослеПолученияТокенаПоСертификату", ЭтотОбъект, 
			Новый Структура("Контекст, Результат", Контекст, Результат)),
		Контекст.СертификатКриптографии, СтандартнаяОбработка, Истина, Истина);
...
// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Асинх Процедура ПослеПолученияТокенаПоСертификату(Токен, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Токен) Тогда
		ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(ДополнительныеПараметры.Результат, ДополнительныеПараметры.Контекст);
		Возврат;
	КонецЕсли;
	...
	Контекст.Вставить("МенеджерКриптографии", Неопределено);
	Токен.Вставить("Пароль", СвойстваПароля.Значение);
	Контекст.Вставить("Токен", Токен);
	
	Если ДополнительныеПараметры.Результат = Истина Тогда
		ОписаниеОшибкиНаКлиенте = "";
	ИначеЕсли ТипЗнч(ДополнительныеПараметры.Результат) = Тип("Структура") Тогда
		ОписаниеОшибкиНаКлиенте = ДополнительныеПараметры.Результат.ОписаниеОшибкиНаКлиенте;
	Иначе
		ОписаниеОшибкиНаКлиенте = ДополнительныеПараметры.Результат;
	КонецЕсли;
	
	УстановитьЭлемент(ЭтотОбъект, "ДанныеСертификата", Ложь,
		НСтр("ru = 'Проверка корректности сертификата средствами токена появится в следующих версиях приложения.'"),
		Истина, ОбъединятьРезультаты);
	
	УстановитьЭлемент(ЭтотОбъект, "НаличиеПрограммы", Ложь, НСтр("ru = 'Проверка сертификата выполняется средствами токена.'" + Символы.ПС + ОписаниеОшибкиНаКлиенте),
		Ложь, ОбъединятьРезультаты);
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		ПараметрыПодписи = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыПодписиНаТокене(Токен, 
			Контекст.СертификатКриптографии, ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES"));
		
		СтандартнаяОбработка = Истина;
		ОповещениеПослеОперацииНаТокене = Новый ОписаниеОповещения("ПослеПодписанияНаТокене", ЭтотОбъект, Контекст);
		ЭлектроннаяПодписьКлиентЛокализация.ПриПодписанииНаТокене(ОповещениеПослеОперацииНаТокене, ПараметрыПодписи,
			Контекст.ДанныеСертификата, СтандартнаяОбработка);
		Если Не СтандартнаяОбработка Тогда
			Возврат;
		КонецЕсли;
		
		УстановитьЭлемент(ЭтотОбъект, "Подписание", Ложь, НСтр("ru = 'Не найдена процедура подписания для токена.'"), Истина, ОбъединятьРезультаты);
	Иначе
		УстановитьЭлемент(ЭтотОбъект, "Подписание", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
	КонецЕсли;
...
// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Асинх Процедура ПослеПодписанияНаТокене(ДанныеПодписи, Контекст) Экспорт

	Если ТипЗнч(ДанныеПодписи) <> Тип("Строка") И ЗначениеЗаполнено(ТипПодписи) И ТипПодписи <> ПредопределенноеЗначение(
			"Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES") И ТипПодписи <> ПредопределенноеЗначение(
			"Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS") Тогда
		ПараметрыПодписи = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыПодписиНаТокене(Контекст.Токен,
			Контекст.СертификатКриптографии, ТипПодписи);

		СтандартнаяОбработка = Истина;
		ОповещениеПослеОперацииНаТокене = Новый ОписаниеОповещения("ПослеПодписанияНаТокенеСТипомПодписи", ЭтотОбъект,
			Контекст);
		ЭлектроннаяПодписьКлиентЛокализация.ПриПодписанииНаТокене(ОповещениеПослеОперацииНаТокене, ПараметрыПодписи,
			Контекст.ДанныеСертификата, СтандартнаяОбработка);
		Если Не СтандартнаяОбработка Тогда
	...
	ПослеПодписанияНаТокенеСТипомПодписи(ДанныеПодписи, Контекст);
	
КонецПроцедуры

// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Асинх Процедура ПослеПодписанияНаТокенеСТипомПодписи(ДанныеПодписи, Контекст) Экспорт
	...
	Если ТипЗнч(ДанныеПодписи) = Тип("ДвоичныеДанные") Тогда
		...
		ПараметрыПроверкиПодписи.Вставить("Подпись", ДанныеПодписи);
		ПараметрыПроверкиПодписи.Вставить("АлгоритмПодписи", Контекст.АлгоритмПодписи);
		ПараметрыПроверкиПодписи.Вставить("РезультатПроверки", ЭлектроннаяПодписьКлиентСервер.РезультатПроверкиПодписи());
		ПараметрыПроверкиПодписи.Вставить("Оповещение", Новый ОписаниеОповещения("ПослеПроверкиПодписиНаТокене", ЭтотОбъект, Контекст));
		ПараметрыПроверкиПодписи.Вставить("ПроверятьСертификат",
			ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат());
		ПараметрыПроверкиПодписи.Вставить("ИсходныеДанные", Контекст.ДанныеСертификата);
		ЭлектроннаяПодписьСлужебныйКлиент.ПроверитьПодписьСредствамиТокена(ПараметрыПроверкиПодписи);
	Иначе
		ПослеПроверкиПодписиНаТокене(Неопределено, Контекст);
	КонецЕсли;
...
// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Асинх Процедура ПослеПроверкиПодписиНаТокене(Результат, Контекст) Экспорт
	
	Если Результат <> Неопределено Тогда
		Если Результат.ПодписьВерна <> Истина Тогда
			ОписаниеОшибки = ?(ЗначениеЗаполнено(Результат.ОшибкаМатематическойПроверкиПодписи),
				Результат.ОшибкаМатематическойПроверкиПодписи, Результат.ОшибкаПроверкиДополнительныхАтрибутов);
			УстановитьЭлемент(ЭтотОбъект, "ПроверкаПодписи", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
		Иначе
			УстановитьЭлемент(ЭтотОбъект, "ПроверкаПодписи", Ложь, "", Ложь, ОбъединятьРезультаты);
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыШифрования = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыШифрованияНаТокене(Контекст.Токен,
	...
	ЭлектроннаяПодписьКлиентЛокализация.ПриШифрованииНаТокене(ОповещениеПослеОперацииНаТокене,
		ПараметрыШифрования, Контекст.ДанныеСертификата, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, НСтр("ru = 'Не найдена процедура шифрования для токена.'"));
...
Процедура ПослеШифрованияНаТокене(ЗашифрованныеДанные, Контекст) Экспорт
	...
	Если ТипЗнч(ЗашифрованныеДанные) = Тип("Строка") Тогда
		УстановитьЭлемент(ЭтотОбъект, "Шифрование", Ложь, ЗашифрованныеДанные, Истина, ОбъединятьРезультаты);
		ПроверитьНаСторонеКлиентаДополнительныеПроверки(Контекст);
	Иначе
		
		Если ТипЗнч(ЗашифрованныеДанные) <> Тип("ДвоичныеДанные") Тогда
			УстановитьЭлемент(ЭтотОбъект, "Шифрование", Ложь, НСтр(
				"ru = 'Не удалось зашифровать данные средствами токена.'"), Истина, ОбъединятьРезультаты);
			ПроверитьНаСторонеКлиентаДополнительныеПроверки(Контекст);
		Иначе
			УстановитьЭлемент(ЭтотОбъект, "Шифрование", Ложь, "", Ложь, ОбъединятьРезультаты);
			
			ПараметрыРасшифровки = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыРасшифровкиНаТокене(Контекст.Токен, 
						Контекст.СертификатКриптографии);
			...
			ЭлектроннаяПодписьКлиентЛокализация.ПриРасшифровкеНаТокене(ОповещениеПослеОперацииНаТокене,
				ПараметрыРасшифровки, ЗашифрованныеДанные, СтандартнаяОбработка);
			Если Не СтандартнаяОбработка Тогда
				Возврат;
			КонецЕсли;
...
Процедура ПослеРасшифровкиНаТокене(РасшифрованныеДанные, Контекст) Экспорт
	Если ТипЗнч(РасшифрованныеДанные) = Тип("Строка") Тогда
		УстановитьЭлемент(ЭтотОбъект, "Расшифровка", Ложь, РасшифрованныеДанные, Истина, ОбъединятьРезультаты);

	Иначе
		УстановитьЭлемент(ЭтотОбъект, "Расшифровка", Ложь, "", Ложь, ОбъединятьРезультаты);
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'В сертификате не указано приложение для использования закрытого ключа.'");
		ПроверитьНаСторонеКлиентаПослеСозданияМенеджераКриптографии(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Асинх Процедура ПроверитьНаСторонеКлиентаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ПроверкиНаКлиенте.НаличиеСертификата = Истина И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Попытка
			ДанныеПодписи = Ждать Контекст.МенеджерКриптографии.ПодписатьАсинх(Контекст.ДанныеСертификата, Контекст.СертификатКриптографии);
		...
		Если ЗначениеЗаполнено(ТипПодписи) И ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES")
			И ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS") Тогда
			
			Попытка
				НастройкиПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.НастройкиСозданияПодписи(ТипПодписи,
					ЭлектроннаяПодписьКлиент.ОбщиеНастройки().АдресаСерверовМетокВремени);
				Контекст.МенеджерКриптографии.АдресаСерверовМетокВремени = НастройкиПодписи.АдресаСерверовМетокВремени;
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ПустыеДанныеПодписи(ДанныеПодписи, ОписаниеОшибки);
			Исключение
				ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			КонецПопытки;
		...
		ПроверитьНаСторонеКлиентаПослеПодписания(ДанныеПодписи, Контекст);
...
// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Процедура ПроверитьНаСторонеКлиентаПослеПодписания(ДанныеПодписи, Контекст)
	...
	Если ДанныеПодписи <> Null И ПроверкиНаКлиенте.НаличиеСертификата = Истина И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Контекст.Вставить("ДанныеПодписи", ДанныеПодписи);
...
// Продолжение процедуры ПроверитьНаСторонеКлиента.
&НаКлиенте
Асинх Процедура ПроверитьНаСторонеКлиентаПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат <> Null Тогда
		...
		Если Не ЗначениеЗаполнено(ОписаниеОшибки) И ЗначениеЗаполнено(ТипПодписи) И ТипПодписи
			<> ПредопределенноеЗначение(
		"Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES") И ТипПодписи <> ПредопределенноеЗначение(
		"Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS") Тогда

			Попытка
				СвойстваПодписи = Ждать ЭлектроннаяПодписьСлужебныйКлиент.СвойстваПодписиЧтениеМенеджеромКриптографии(
					Контекст.ДанныеПодписи, Контекст.МенеджерКриптографии, Истина);
			Исключение
				ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			КонецПопытки;

			Если Не ЗначениеЗаполнено(ОписаниеОшибки) И ЗначениеЗаполнено(СвойстваПодписи.ТекстОшибки) Тогда
				ОписаниеОшибки = СвойстваПодписи.ТекстОшибки;
			ИначеЕсли Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
				ДанныеСертификатаМеткиВремени = СвойстваПодписи.Сертификаты[СвойстваПодписи.Сертификаты.ВГраница()];
				СертификатМеткиВремени = Новый СертификатКриптографии();
				Ждать СертификатМеткиВремени.ИнициализироватьАсинх(ДанныеСертификатаМеткиВремени);
				СвойстваСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСертификата(
					СертификатМеткиВремени, ЭлектроннаяПодписьСлужебныйКлиент.РазницаСУниверсальнымВременем());
				
				Найдено = СертификатыМетокВремени.НайтиСтроки(Новый Структура("Отпечаток",
					СвойстваСертификата.Отпечаток));

				Если Найдено.Количество() = 0 Тогда
					НоваяСтрока = СертификатыМетокВремени.Добавить();
					НоваяСтрока.АдресСертификата = ПоместитьВоВременноеХранилище(ДанныеСертификатаМеткиВремени,
						УникальныйИдентификатор);
					НоваяСтрока.Отпечаток = СвойстваСертификата.Отпечаток;
					НоваяСтрока.ПолученНаКлиенте = Истина;
					НоваяСтрока.Представление = СвойстваСертификата.Представление;
				Иначе
					Найдено[0].ПолученНаКлиенте = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьНаСторонеСервера(Знач ЗначениеПароля)
	...
	Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
		ПараметрыСоздания.АлгоритмПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.ПредставлениеАлгоритмаПодписи(
			СертификатКриптографии.АлгоритмОткрытогоКлюча, Истина, Ложь);
	Иначе
		ПараметрыСоздания.АлгоритмПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмПодписиСертификата(ДанныеСертификата);
	КонецЕсли;
	...
	Если Отозван Тогда
		...
		УстановитьЭлемент(ЭтотОбъект, "Подписание", Истина, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
	Иначе
		Если ВыполнятьПроверкуУдостоверяющегоЦентра <> ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат() Тогда
			ПараметрыПроверки = Новый Структура;
			ПараметрыПроверки.Вставить("ЭтоПроверкаПодписи", Ложь);
			ПараметрыПроверки.Вставить("ПроверятьСертификат", ВыполнятьПроверкуУдостоверяющегоЦентра);
			
			РезультатПроверкиУЦ = ЭлектроннаяПодписьСлужебный.РезультатПроверкиУдостоверяющегоЦентраСертификата(
				СертификатКриптографии,, ПараметрыПроверки);
			Если Не РезультатПроверкиУЦ.Действует Тогда
				
				Если  ВыполнятьПроверкуУдостоверяющегоЦентра = ЭлектроннаяПодписьСлужебныйКлиентСервер.ТолькоКвалифицированные() Тогда
...
Функция ПроверитьПодписаниеНаСервере(МенеджерКриптографии, ДанныеСертификата, СертификатКриптографии, ОписаниеОшибки)
	...
	Если ЗначениеЗаполнено(ТипПодписи) И ТипПодписи <> ПредопределенноеЗначение(
		"Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES") И ТипПодписи <> ПредопределенноеЗначение(
		"Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS") Тогда
...
Процедура ПроверитьПодписьНаСервере(МенеджерКриптографии, ДанныеСертификата, ДанныеПодписи)
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) И ЗначениеЗаполнено(ТипПодписи) И ТипПодписи <> ПредопределенноеЗначение(
		"Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES") И ТипПодписи <> ПредопределенноеЗначение(
		"Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS") Тогда
		
		Попытка
			СвойстваПодписи = ЭлектроннаяПодписьСлужебный.СвойстваПодписиЧтениеМенеджеромКриптографии(
				ДанныеПодписи, МенеджерКриптографии, Истина);
		Исключение
			ОписаниеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
		...
		ИначеЕсли Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			ДанныеСертификатаМеткиВремени = СвойстваПодписи.Сертификаты[СвойстваПодписи.Сертификаты.ВГраница()];
			СертификатМеткиВремени = Новый СертификатКриптографии(ДанныеСертификатаМеткиВремени);
			СвойстваСертификата = ЭлектроннаяПодписьСлужебныйКлиентСервер.СвойстваСертификата(
				СертификатМеткиВремени, ЭлектроннаяПодписьСлужебный.РазницаСУниверсальнымВременем());
			
			Найдено = СертификатыМетокВремени.НайтиСтроки(Новый Структура("Отпечаток", СвойстваСертификата.Отпечаток));
			...
			Если Найдено.Количество() = 0 Тогда
					...
					НоваяСтрока.АдресСертификата = ПоместитьВоВременноеХранилище(ДанныеСертификатаМеткиВремени,
						УникальныйИдентификатор);
					НоваяСтрока.Отпечаток = СвойстваСертификата.Отпечаток;
					НоваяСтрока.ПолученНаСервере = Истина;
				
			Иначе
				Найдено[0].ПолученНаСервере = Истина;
			КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЭлемент(Форма, Действие, НаСервере,
	ОписаниеОшибки = Неопределено, ЭтоОшибка = Ложь, ОбъединятьРезультаты = "НеОбъединять")
	
	ИмяЭлементаНадпись = Действие + "Надпись";
	ИмяЭлементаНаКлиентеКартинка = Действие + "НаКлиентеКартинка";
	ИмяЭлементаНаСервереКартинка = Действие + "НаСервереКартинка"; 
	ИмяСвойстваОшибка = Действие + "Ошибка";
	
	ЭлементНадпись = Форма.Элементы[ИмяЭлементаНадпись]; // ДекорацияФормы, РасширениеДекорацииФормыДляНадписи
	ТребуетсяОбъединятьРезультаты = (ОбъединятьРезультаты = "ОбъединятьПоИ" Или ОбъединятьРезультаты = "ОбъединятьПоИли");  
	Если ТребуетсяОбъединятьРезультаты Тогда
		ЭлементКартинка = Форма.Элементы[ИмяЭлементаНаКлиентеКартинка]; // ДекорацияФормы
	Иначе
		ЭлементКартинка = Форма.Элементы[?(НаСервере, ИмяЭлементаНаСервереКартинка, ИмяЭлементаНаКлиентеКартинка)]; // ДекорацияФормы
	КонецЕсли;
	...
	Если ТипЗнч(ПроверкиВторогоКонтекста) = Тип("Структура") И ПроверкиВторогоКонтекста.Свойство(Действие) Тогда
		ЗначениеПроверкиВторогоКонтекста = ПроверкиВторогоКонтекста[Действие];
	КонецЕсли;
	
	ИзвестнаяОшибкаБезКлассификатора = Неопределено;
	Если ТипЗнч(ОписаниеОшибки) = Тип("Структура") Тогда
		ИзвестнаяОшибкаБезКлассификатора = ОписаниеОшибки;
		ОписаниеОшибки = ИзвестнаяОшибкаБезКлассификатора.ТекстОшибки;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		ЗначениеПроверки = Ложь;
		РезультатПроверки = ЗначениеПроверки;
		Если ОбъединятьРезультаты = "ОбъединятьПоИли" И ЗначениеПроверкиВторогоКонтекста <> Неопределено Тогда
	...
	Иначе
		...
		Если ЗначениеПроверки <> Неопределено И ЗначениеПроверкиВторогоКонтекста <> Неопределено 
			И ТребуетсяОбъединятьРезультаты Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Проверки.Вставить(Действие, ЗначениеПроверки);
	
	ИмяГруппыПоНазваниямПроверки = ИменаГруппПоНазваниямПроверок(Форма.Элементы)[Действие];
	Если ИмяГруппыПоНазваниямПроверки = Неопределено Тогда
		ИмяГруппыПоНазваниямПроверки = "Группа" + Действие;
	КонецЕсли;
	Форма.Элементы[ИмяГруппыПоНазваниямПроверки].Видимость = 
		ЗначениеПроверки = Ложь Или ЗначениеПроверкиВторогоКонтекста = Ложь;

	Проверки.Вставить(ИмяСвойстваОшибка, ?(ОписаниеОшибки = Неопределено, "", ОписаниеОшибки));
		
	ИмяГруппыОшибкаСервер = Действие + "ОшибкаСервер"; 
	ИмяГруппыОшибкаКлиент = Действие + "ОшибкаКлиент"; 
	ИмяЭлементаНадписьОшибкаСервер = Действие + "ОшибкаСерверНадпись"; 
	ИмяЭлементаНадписьОшибкаКлиент = Действие + "ОшибкаКлиентНадпись"; 
	ИмяЭлементаНадписьРешениеСервер = Действие + "РешениеСерверНадпись"; 
	ИмяЭлементаНадписьРешениеКлиент = Действие + "РешениеКлиентНадпись"; 
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Форма.ПроверкиУспешны = Ложь;
		
		ГруппаОшибка = Форма.Элементы[?(НаСервере, ИмяГруппыОшибкаСервер, ИмяГруппыОшибкаКлиент)];
		...
		ЭлементНадписьОшибка = Форма.Элементы[?(НаСервере, ИмяЭлементаНадписьОшибкаСервер, ИмяЭлементаНадписьОшибкаКлиент)]; // ДекорацияФормы, РасширениеДекорацииФормыДляНадписи
		ЭлементНадписьОшибка.Заголовок = ОписаниеОшибки;
		
		ИмяНадписиРешение = ?(НаСервере, ИмяЭлементаНадписьРешениеСервер, ИмяЭлементаНадписьРешениеКлиент);
		ЭлементФормыНадписьРешение = Форма.Элементы[ИмяНадписиРешение];
			
		ИмяЭлементаНадписьОшибкаПодписание = "Подписание" + ?(НаСервере, "ОшибкаСерверНадпись", "ОшибкаКлиентНадпись"); 
		ЭлементФормыНадписьОшибкаПодписание = Форма.Элементы[ИмяЭлементаНадписьОшибкаПодписание]; 

		ОписаниеИзвестнойОшибки = ОшибкаПоКлассификатору(ОписаниеОшибки, НаСервере, Действие = "ПроверкаПодписи", ЗначениеЗаполнено(Форма.Сертификат));
		...
		Если Не ЭтоИзвестнаяОшибка Тогда
			...
			Если ИзвестнаяОшибкаБезКлассификатора = Неопределено Тогда
				ЭлементНадписьОшибка.Заголовок = СформироватьЗаголовокПодробно(ОписаниеОшибки);
				Возврат;
			Иначе
				ЭлементФормыНадписьРешение.Видимость = Истина;
				ИмяРеквизитаФормы = ?(НаСервере, "ДополнительныеДанныеПроверокНаСервере", "ДополнительныеДанныеПроверокНаКлиенте");
				ДополнительныеДанныеПроверок = Форма[ИмяРеквизитаФормы];
				ДополнительныеДанныеПроверок.Вставить(Действие,
					Новый Структура("Причина, Решение", ИзвестнаяОшибкаБезКлассификатора.Причина, ИзвестнаяОшибкаБезКлассификатора.Решение));
				Форма[ИмяРеквизитаФормы] = ДополнительныеДанныеПроверок;
				ОписаниеИзвестнойОшибки = Новый Структура;
				ОписаниеИзвестнойОшибки.Вставить("Причина", ИзвестнаяОшибкаБезКлассификатора.Причина);
				ОписаниеИзвестнойОшибки.Вставить("Решение", ИзвестнаяОшибкаБезКлассификатора.Решение);
				ОписаниеИзвестнойОшибки.Вставить("ДействияДляУстранения");
			КонецЕсли;
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОписаниеИзвестнойОшибки.Причина) Тогда
			ЭлементНадписьОшибка.Заголовок = ОписаниеИзвестнойОшибки.Причина;
		КонецЕсли;
		
		Если Действие = "Расшифровка"
			И ЭлементФормыНадписьОшибкаПодписание.Видимость = Истина
			И ЭлементНадписьОшибка.Заголовок = ЭлементФормыНадписьОшибкаПодписание.Заголовок Тогда
			
			ЭлементФормыНадписьРешение.Видимость = Ложь;
			ЭлементНадписьОшибка.Видимость = Истина;
			ЭлементНадписьОшибка.Заголовок = НСтр("ru = 'См. пункт Подписание данных.'");
			Возврат;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Форма.Сертификат) И ЗначениеЗаполнено(ОписаниеИзвестнойОшибки.ДействияДляУстранения) Тогда
			
			ДанныеДляДополнения = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДанныеДляДополненияОшибкиИзКлассификатора(
				Новый Структура("Сертификат, ДанныеСертификата", Форма.Сертификат, Форма.АдресСертификата));
			
			ОписаниеИзвестнойОшибки = ЭлектроннаяПодписьСлужебныйВызовСервера.ДополнитьРешениеКлассификатораОшибокПодробностями(
				ОписаниеИзвестнойОшибки, ДанныеДляДополнения, ?(НаСервере, "Сервер", "Клиент"));
	...
	ГруппаОшибка = Форма.Элементы[?(НаСервере, ИмяГруппыОшибкаСервер, ИмяГруппыОшибкаКлиент)];
	ЭлементРешение = Форма.Элементы[?(НаСервере, ИмяЭлементаНадписьРешениеСервер, ИмяЭлементаНадписьРешениеКлиент)];
	
	Если ТребуетсяОбъединятьРезультаты Тогда
		
		Если ЗначениеПроверки = Неопределено И ЗначениеПроверкиВторогоКонтекста = Неопределено Тогда
			...
			ЭлементНадпись.Заголовок = ЗаголовокЭлемента(Форма.ДополнительныеПроверки, ЭлементНадпись.Имя, Форма.ТипПодписи);
		ИначеЕсли ЗначениеПроверки <> Неопределено И ЗначениеПроверкиВторогоКонтекста = Неопределено Тогда
			ГруппаОшибка.Видимость = Ложь;
			ЭлементРешение.Видимость = Ложь;
			...
			ЭлементНадпись.Гиперссылка = Ложь;
			ЭлементНадпись.Заголовок = ЗаголовокЭлемента(Форма.ДополнительныеПроверки, ЭлементНадпись.Имя, Форма.ТипПодписи);
		ИначеЕсли ЗначениеПроверки <> Неопределено И РезультатПроверки Тогда
			ГруппаОшибка.Видимость = Ложь;
			ЭлементРешение.Видимость = Ложь;
			ЭлементКартинка.Картинка = БиблиотекаКартинок.ПроверкаСертификатаУспех;
			ЭлементКартинка.Подсказка = НСтр("ru = 'Проверка выполнена успешно.'");
	...
	Если ПроверкиВторогоКонтекста = Неопределено
		Или Не ПроверкиВторогоКонтекста.Свойство(Действие + "Ошибка")
		Или ПустаяСтрока(ПроверкиВторогоКонтекста[Действие + "Ошибка"]) Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция СформироватьЗаголовокПодробно(ОписаниеОшибки)
	...
	Строка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '<a href = %1>Поиск решения...</a>'"), НавигационнаяСсылка);

#Если Сервер Тогда
	Массив.Добавить(СтроковыеФункции.ФорматированнаяСтрока(Строка));
#Иначе
	Массив.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(Строка));
#КонецЕсли
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗаголовокЭлемента(ДополнительныеПроверки, ИмяЭлемента, ТипПодписи)
	
	ЗаголовокЭлемента = СтандартныйЗаголовокЭлемента(ИмяЭлемента, ТипПодписи);
...
&НаКлиентеНаСервереБезКонтекста
Функция СтандартныйЗаголовокЭлемента(ИмяЭлемента, ТипПодписи)
	...
	ИначеЕсли ИмяЭлемента = "НаличиеПрограммыНадпись" Тогда
		Возврат НСтр("ru = 'Наличие приложения для подписания и расшифровки'");
		
	ИначеЕсли ИмяЭлемента = "ПодписаниеНадпись" Тогда
		
		Если ЗначениеЗаполнено(ТипПодписи) Тогда
			
			ТипПодписиСтрока = Строка(ТипПодписи);
			ТипПодписиСтрока = НРег(Лев(ТипПодписиСтрока, 1)) + Сред(ТипПодписиСтрока, 2);
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подписание данных типом подписи - %1'"), ТипПодписиСтрока);
...
&НаКлиенте
Процедура ДополнитьТекстОшибками(ТекстОшибок, Проверки, НаСервере = Ложь, ТемаСообщения = "")
	...
	Для Каждого Проверка Из Проверки Цикл
		...
		Иначе
			...
			Если ПустаяСтрока(ТемаСообщения) И Не ПустаяСтрока(ТекстОшибкиПроверки) Тогда
				ТемаСообщения = ТекстОшибкиПроверки;
			КонецЕсли;
...
Функция ОшибкаПоКлассификатору(ОписаниеОшибки, ОшибкаНаСервере, ОшибкаПроверкиПодписи, УказанСертификат)
	
	Возврат ЭлектроннаяПодписьСлужебный.ОшибкаПоКлассификатору(ОписаниеОшибки, ОшибкаНаСервере, ОшибкаПроверкиПодписи, УказанСертификат);
...
Асинх Функция ДополнительнаяПроверкаНаВозможностьПодписания(СертификатКриптографии, СторонаВыполнения)
	...
	ИначеЕсли ВыполнятьПроверкуУдостоверяющегоЦентра <> ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат() Тогда
		
		ПараметрыПроверки = Новый Структура;
		ПараметрыПроверки.Вставить("ЭтоПроверкаПодписи", Ложь);
		ПараметрыПроверки.Вставить("ПроверятьСертификат", ВыполнятьПроверкуУдостоверяющегоЦентра);
			
		РезультатПроверкиУдостоверяющегоЦентраСертификата = Ждать ЭлектроннаяПодписьСлужебныйКлиент.РезультатПроверкиУдостоверяющегоЦентраСертификата(
			СертификатКриптографии,, ПараметрыПроверки);
		...
		Если Не РезультатПроверкиУдостоверяющегоЦентраСертификата.Действует Тогда
			...
			Если ПодписаниеРазрешено = Неопределено Или Не ПодписаниеРазрешено Тогда
				...
				УстановитьЭлемент(ЭтотОбъект, "Подписание", СторонаВыполнения,
					РезультатПроверкиУдостоверяющегоЦентраСертификата.Предупреждение, Истина, ОбъединятьРезультаты);
			КонецЕсли;
		КонецЕсли;
