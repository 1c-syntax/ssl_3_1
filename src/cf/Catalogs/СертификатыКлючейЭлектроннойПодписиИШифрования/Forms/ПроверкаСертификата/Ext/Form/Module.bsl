...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого Проверка Из Проверки Цикл
		...
		ГруппаРешения = ГруппаФормыБезОтображения(
			Проверка.Имя + "Решения", ГруппаРезультатИРешение,
			ГруппировкаПодчиненныхЭлементовФормы.Вертикальная);
		
		ГруппаРешения.Видимость = Ложь;
		
		ДобавитьГруппуОписанияРешения(Проверка.Имя, ГруппаРешения);
		ДобавитьГруппуОписанияРешения(Проверка.Имя, ГруппаРешения, Истина);
	...
	Иначе
		...
		Для Каждого Проверка Из СтандартныеПроверки() Цикл
			...
			Элементы[Проверка + "РешениеКлиент"].Видимость = Ложь;
			Элементы[Проверка + "РешениеСервер"].Видимость = Ложь;
	...
	СвойстваСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сертификат,
		"ДанныеСертификата, Программа, ВводитьПарольВПрограммеЭлектроннойПодписи, Отпечаток");
	...
	ЭтоПрограммаОблачнойПодписи = ТипЗнч(Программа) = ЭлектроннаяПодписьСлужебный.ПолучитьТипОблачнойПодписи()
						И ЭлектроннаяПодписьСлужебный.ИспользоватьСервисОблачнойПодписи();
	ЭтоПрограммаОблачногоСервиса = Программа = ЭлектроннаяПодписьСлужебный.ПрограммаОблачногоСервиса();
	
	Если ЭтоПрограммаОблачнойПодписи Тогда
	...
	ИначеЕсли ЭтоПрограммаОблачногоСервиса Тогда
...
Процедура Подключаемый_НадписьНажатие(Элемент)
	...
	ИмяОшибки = Лев(Элемент.Имя, СтрДлина(Элемент.Имя) - СтрДлина("Надпись")) + "Ошибка";
	...
	ПараметрыПредупреждения.Вставить("ТекстОшибкиСервер",
		?(Не ОперацииНаСервере, "", ПроверкиНаСервере[ИмяОшибки]));
...
Процедура ИнформацияДляПоддержкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	ТекстОшибок = ТекстОшибок + НСтр("ru = 'Результат поверки на клиенте'") + ":" + Символы.ПС;
	...
	ТекстОшибок = ТекстОшибок + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Результат поверки на сервере ""%1"":'"), ИмяСервера) + Символы.ПС;
...
Процедура ПроверитьНаСторонеКлиентаПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если ЭтоОперацияОблачнойПодписи() Тогда
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(Результат.Сертификат) Тогда
		ОписаниеОшибки = НСтр("ru = 'Сертификат отсутствует в облачном сервисе (возможно удален).'");
	КонецЕсли;
	Если Не ПустаяСтрока(ОписаниеОшибки) Тогда
		ОписаниеОшибки = ОписаниеОшибки + Символы.ПС
			+ НСтр("ru = 'Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.'");
	КонецЕсли;	
	УстановитьЭлемент(ЭтотОбъект, "НаличиеСертификата", Ложь, ОписаниеОшибки, , ОбъединятьРезультаты);
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	КонецЕсли;
	УстановитьЭлемент(ЭтотОбъект, "ДанныеСертификата", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
...
Процедура ПроверитьНаСторонеКлиентаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	КонецЕсли;
	УстановитьЭлемент(ЭтотОбъект, "НаличиеПрограммы", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
...
Процедура ПроверитьНаСторонеКлиентаПослеШифрования(ЗашифрованныеДанные, Контекст) Экспорт
	...
	КонецЕсли;
	УстановитьЭлемент(ЭтотОбъект, "Шифрование", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
...
Процедура ПроверитьНаСторонеКлиентаПослеРасшифровки(РасшифрованныеДанные, Контекст) Экспорт
	...
	Если РасшифрованныеДанные <> Null Тогда
		...
		КонецЕсли;
		УстановитьЭлемент(ЭтотОбъект, "Расшифровка", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеАвторизацииВОблачнойПодписи(РезультатВызова, Контекст) Экспорт
	...
	ЭтотОбъект.НастройкиПользователя = РезультатВызова.НастройкиПользователя;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаОблачнойПодписи(РезультатВызова, Контекст) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(РезультатВызова.ДанныеСертификата) Тогда
		ОписаниеОшибки = НСтр("ru = 'Сертификат отсутствует в сервисе ""Облачная подпись"" (возможно удален).'");
	КонецЕсли;
	...
	// Проверка данных сертификата.
	Контекст.Вставить("НастройкиПользователя", ЭтотОбъект.НастройкиПользователя);
	...
	МодульСервисКриптографииDSSКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSКлиент");
	МодульСервисКриптографииDSSКлиент.ПроверитьСертификат(ОбработчикСледующий, Контекст.НастройкиПользователя, РезультатВызова.ДанныеСертификата.Сертификат);
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификатаОблачнойПодписи(Результат, Контекст) Экспорт
	...
	ИначеЕсли Результат.Выполнено И Результат.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = Результат.СтатусОшибки.ИсходныйТекст;
	КонецЕсли;
	УстановитьЭлемент(ЭтотОбъект, "ДанныеСертификата", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
...
Процедура ПроверитьНаСторонеКлиентаОблачнаяПодпись(Результат, Контекст)
	...
	Иначе
		ПроверитьНаСторонеКлиентаПослеПодписания(Неопределено, Контекст);
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиПодписиОблачнаяПодпись(РезультатВызова, Контекст) Экспорт
	...
	ИначеЕсли РезультатВызова <> Неопределено Тогда
		Если Не РезультатВызова.Выполнено Тогда
			ОписаниеОшибки = РезультатВызова.СтатусОшибки.ИсходныйТекст;
		Иначе
			ОписаниеОшибки = "";
		КонецЕсли;
		УстановитьЭлемент(ЭтотОбъект, "ПроверкаПодписи", Ложь, ОписаниеОшибки, Истина, ОбъединятьРезультаты);
...
&НаКлиенте
Функция ЭтоОперацияОблачнойПодписи()
	
	Результат = ТипЗнч(Программа) = ЭлектроннаяПодписьСлужебныйКлиент.ПолучитьТипОблачнойПодписи();
	Возврат Результат;
...
&НаСервере
Процедура ДобавитьГруппуОписанияРешения(ИмяПроверки, Родитель, НаСервере = Ложь)
	
	ГруппаРешение = ГруппаФормыБезОтображения(
		ИмяПроверки + ?(НаСервере, "РешениеСервер", "РешениеКлиент"), Родитель);
	
	ДобавитьКартинку(ИмяПроверки + ?(НаСервере, "РешениеСерверКартинка", "РешениеКлиентКартинка"),
		ГруппаРешение, БиблиотекаКартинок[?(НаСервере, "КомпьютерСервер", "КомпьютерКлиент")]);
	
	Надпись = Элементы.Добавить(ИмяПроверки + ?(НаСервере, "РешениеСерверНадпись", "РешениеКлиентНадпись"),
		Тип("ДекорацияФормы"), ГруппаРешение);
...
Процедура ОбновитьВидимостьНаСервере()
	...
	Для Каждого Проверка Из СтандартныеПроверки() Цикл
		Элементы[Проверка + "НаСервереКартинка"].Видимость = ОперацииНаСервере;
	КонецЦикла;
	
	Для каждого Проверка Из ДополнительныеПроверки Цикл
		Элементы[Проверка.Имя + "НаСервереКартинка"].Видимость = ОперацииНаСервере;
	КонецЦикла;
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере,
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не РезультатПроверки Тогда
			...
			ЭлементНадпись.Заголовок = ОписаниеОшибки;
			
			ОписаниеИзвестнойОшибки = ОшибкаПоКлассификатору(ОписаниеОшибки, Истина);
			ЭтоИзвестнаяОшибка = ОписаниеИзвестнойОшибки <> Неопределено;
			
			ИмяГруппыРешение = НачалоЭлемента + ?(НаСервере, "РешениеСервер", "РешениеКлиент");
			Форма.Элементы[ИмяГруппыРешение].Видимость = ЭтоИзвестнаяОшибка;
			Если ЭтоИзвестнаяОшибка Тогда
				ИмяГруппыРешениеНадпись = НачалоЭлемента + ?(НаСервере, "РешениеСерверНадпись", "РешениеКлиентНадпись");
				Форма.Элементы[ИмяГруппыРешениеНадпись].Заголовок = ОписаниеИзвестнойОшибки.Решение;
			КонецЕсли;
		КонецЕсли;
	Иначе
		ЭлементРешение = Форма.Элементы[НачалоЭлемента
			+ ?(НаСервере, "РешениеСервер", "РешениеКлиент")];
...
Функция ОшибкаПоКлассификатору(ОписаниеОшибки, Форматировать)
	
	Возврат ЭлектроннаяПодписьСлужебный.ОшибкаПоКлассификатору(ОписаниеОшибки, Форматировать);
