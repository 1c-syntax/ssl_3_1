...
// АПК:78-вкл: для безопасной передачи данных на клиенте между формами, не отправляя их на сервер.
	...
	ВидыФорм =
		...
		|,ВыборСертификатаДляПодписанияИлиРасшифровки,ПроверкаСертификата,ОтправитьНаПодписание,";
	...
	Если ВидФормы = "ОтправитьНаПодписание" Тогда
		ИмяНовойФормы = "";
		ЭлектроннаяПодписьКлиентЛокализация.ПриОпределенииФормыНаправленияНаПодписание(ИмяНовойФормы);
		Если ИмяНовойФормы = "" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в процедуре %1. %2 ""%3"" не поддерживается.'"),
				"ОткрытьНовуюФорму", "ВидФормы", ВидФормы);
		КонецЕсли;
...
Процедура ОткрытьНовуюФормуПродолжение(Результат, Контекст) Экспорт
	...
	Если Форма.Открыта() Тогда
		Возврат;
	ИначеЕсли Контекст.Форма.ИмяФормы =
		"Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.ВыборСертификатаДляПодписанияИлиРасшифровки"
		И Не ПустаяСтрока(Форма.ТекстПредупрежденияПриОткрытии) Тогда
