	...
	Если ЭлектроннаяПодписьСлужебныйКлиент.ЭтоОперацияОблачнойПодписи(ЭтотОбъект) Тогда
		...
		Если МодульСервисКриптографииDSSПодтверждениеКлиент.ПроверкаПередВыполнениемОперации(ЭтотОбъект, СвойстваПароля.Значение) Тогда 
			Если ТипЗнч(ОписаниеДанных) = Тип("Структура")
				И ТипЗнч(ОписаниеДанных.Данные) = Тип("Структура")
				И ОписаниеДанных.Данные.Свойство("ПараметрыCMS") Тогда
				ОписаниеДанных.Данные.ПараметрыCMS.ВключениеСертификатовВПодпись = Строка(ОписаниеДанных.Данные.ПараметрыCMS.ВключениеСертификатовВПодпись);
			КонецЕсли;
