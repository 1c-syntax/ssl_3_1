...
// Продолжение процедуры ПодписатьДанные.
&НаКлиенте
Асинх Процедура ПодписатьДанныеПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда // Ошибка при проверке сертификата.
		ПараметрыВыполнения.Вставить("СертификатВерен", Ложь);
		ПараметрыВыполнения.Вставить("ТребуетсяПроверка", Результат.ТребуетсяПроверка);
	...
	Иначе
		ПараметрыВыполнения.Вставить("СертификатВерен", Истина);
		ПараметрыВыполнения.Вставить("ТребуетсяПроверка", Ложь);
	КонецЕсли;
