...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйПараметрТипПодписи.Вставить("Отправитель");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		...
		Если Элементы.ТипПодписанта.СписокВыбора.НайтиПоЗначению("ОтправитьНаПодписание") <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Параметры.Подписант) Или ЗначениеЗаполнено(Подписант) Или (ТипЗнч(
				Параметры.ОтпечаткиСертификатовНаКлиенте) = Тип(
				...
				И Не ЭлектроннаяПодписьСлужебный.ИспользоватьСервисОблачнойПодписи()
				...
				Если ЗначениеЗаполнено(Параметры.Подписант) И ЗначениеЗаполнено(Подписант) И Параметры.БезПодтверждения Тогда
					БезПодтверждения = Истина;
				КонецЕсли;
				УстановитьСвязиПараметровВыбораДоверенности();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// СтандартныеПодсистемы.СервисМобильнойПодписи
	Если ВРег(ИмяСобытия) = ВРег("Запись_ПодписантыСервисаМобильнойПодписи") И Подписант = Источник Тогда
...
Процедура ВыборДоверенностиПриИзменении(Элемент)
	...
	Элементы.СтраницаБезДоверенности.Видимость = Не ПоДоверенности;
	Элементы.СтраницаВыборДоверенности.Видимость = ПоДоверенности;
...
Процедура ПодписантОткрытие(Элемент, СтандартнаяОбработка)
	...
	ЭлектроннаяПодписьКлиентЛокализация.ПодписантПриОткрытии(ЭтотОбъект, Элемент, СтандартнаяОбработка);
...
Процедура ПодписантОчистка(Элемент, СтандартнаяОбработка)
	...
	ЭлектроннаяПодписьКлиентЛокализация.ПодписантПриОчистке(ЭтотОбъект, Элемент, СтандартнаяОбработка);
...
&НаКлиенте
Процедура НастройкаОтправкиПриИзменении(Элемент)
...
Процедура ВидимостьСтраницТипаПодписания(Форма)
	...
	Если Форма.ТипПодписанта = "ПодписатьСертификатом" Тогда
		...
		Форма.Элементы.СтраницаОтправитьНаПодписание.Видимость = Ложь;
		Форма.Элементы.СтраницаПодписание.Видимость = Истина;
	Иначе
		Форма.Элементы.СтраницыПодписание.ТекущаяСтраница = Форма.Элементы.СтраницаОтправитьНаПодписание;
		Форма.Элементы.СтраницаПодписание.Видимость = Ложь;
		Форма.Элементы.СтраницаОтправитьНаПодписание.Видимость = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьСвязиПараметровВыбораДоверенности()

	МассивПараметровВыбора = Новый Массив;
	
	Если ТипПодписанта = "ПодписатьСертификатом" Тогда
		СвязьПараметраВыбора = Новый СвязьПараметраВыбора("СертификатПредставителя", "Сертификат", РежимИзмененияСвязанногоЗначения.Очищать);
		МассивПараметровВыбора.Добавить(СвязьПараметраВыбора);
	КонецЕсли;
	
	Элементы.МашиночитаемаяДоверенность.СвязиПараметровВыбора =  Новый ФиксированныйМассив(МассивПараметровВыбора);
...
Процедура УстановитьТипПодписи(Знач ПараметрТипПодписи)
	...
	ПараметрыОтправителя = ПараметрТипПодписи.Отправитель;
...
Процедура ПодобратьДоверенность(ДоверенностьНастройка = Неопределено)
	...
	Если Не ЭтоАдресВременногоХранилища(СертификатАдрес) И ТипПодписанта = "ПодписатьСертификатом"
		Или ТипПодписанта <> "ПодписатьСертификатом" И Не ЗначениеЗаполнено(Подписант) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодписатьДанныеПослеПроверкиВыбранногоСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда // Ошибка при проверке сертификата.
		...
		Если Результат.ТребуетсяПроверка = Истина Тогда
			ПодписаниеРазрешено = ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекЗагрузить(
					Сертификат, "РазрешитьПодписаниеСертификатомТребующимПроверки", Ложь);
			Если ПодписаниеРазрешено Тогда
...
Процедура ПодписатьДанныеПослеПредупрежденияПодписьНеверна(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("БольшеНеЗадаватьЭтотВопрос") И Результат.БольшеНеЗадаватьЭтотВопрос Тогда
		ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить(
			Сертификат, "РазрешитьПодписаниеСертификатомТребующимПроверки", Истина);
	КонецЕсли;
...
Процедура ПодписатьДанныеПослеВыполнения(Результат)
	...
	НастройкаПодписант = Неопределено;
	Если ТипПодписанта = "ПодписатьСертификатом" Тогда
		НастройкаПодписант = Сертификат;
	ИначеЕсли ЗначениеЗаполнено(Подписант) Тогда
		НастройкаПодписант = Подписант;
	КонецЕсли;

	Если ЗначениеЗаполнено(НастройкаПодписант) Тогда
		
		НастройкиДляСохранения = Новый Массив;
		
		// СтандартныеПодсистемы.МашиночитаемыеДоверенности
		Если СохранитьНастройкуДоверенностиПослеПодписания Тогда
			ДобавитьНастройкуПоДоверенности(НастройкиДляСохранения, НастройкаПодписант);
		КонецЕсли;
		// Конец СтандартныеПодсистемы.МашиночитаемыеДоверенности
		
		Если СохранитьНастройкуПодписантаПослеПодписания Тогда
			НастройкиДляСохранения.Добавить(Новый Структура("Объект, Настройка, Значение",
				ПользователиКлиент.ТекущийПользователь(), "ПодписаниеДанных_ПодписантПоУмолчанию", НастройкаПодписант));
				
			// СтандартныеПодсистемы.СервисМобильнойПодписи	
			Если ТипПодписанта <> "ПодписатьСертификатом" И ЗначениеЗаполнено(Подписант) Тогда
				ДобавитьНастройкуПодписанта(НастройкиДляСохранения, НастройкаПодписант);
			КонецЕсли;
			// Конец СтандартныеПодсистемы.СервисМобильнойПодписи
		КонецЕсли;
...
Процедура ПодписатьДанныеВСервисеМобильнойПодписи(Оповещение)
	...
	ОписаниеДанных.Вставить("ВыбраннаяДоверенность", МашиночитаемаяДоверенность);
	ОписаниеДанных.Вставить("ВидМобильнойПодписи", ВидМобильнойПодписи);
	ОписаниеДанных.Вставить("НастройкаОтправки", НастройкаОтправки);
...
Процедура ПодписатьДанныеВСервисеПослеОбработкиПередВыполнением(Результат, Контекст) Экспорт
	...
	ПараметрыПодписания = МодульСервисМобильнойПодписиКлиентСервер.ПараметрыПодписания();
	ПараметрыПодписания.ТипПодписи = ТипПодписи;
	ПараметрыПодписания.ПакетнаяОтправкаНаПодписание = ПакетнаяОтправка;
	ПараметрыПодписания.ВидМобильнойПодписи = ВидМобильнойПодписи;
	ПараметрыПодписания.НастройкаОтправки = НастройкаОтправки;
	ПараметрыПодписания.Комментарий = Комментарий;
	Если ПоДоверенности И ЗначениеЗаполнено(МашиночитаемаяДоверенность) Тогда
		ПараметрыПодписания.Доверенность = МашиночитаемаяДоверенность;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьНастройкуПодписанта(НастройкиДляСохранения, НастройкаПодписант)
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СервисМобильнойПодписи") Тогда
		МодульСервисМобильнойПодписиСлужебный = ОбщегоНазначения.ОбщийМодуль("СервисМобильнойПодписиСлужебный");
		МодульСервисМобильнойПодписиСлужебный.ДобавитьНастройкуПодписанта(ЭтотОбъект, НастройкиДляСохранения, НастройкаПодписант);
	КонецЕсли;
КонецПроцедуры
