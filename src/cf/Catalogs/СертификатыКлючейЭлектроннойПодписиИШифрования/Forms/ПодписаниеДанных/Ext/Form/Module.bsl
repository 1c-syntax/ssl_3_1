...
Процедура УстановитьТипПодписи(Знач ПараметрТипПодписи)
	...
	Иначе
		ЗаполнитьЗначенияСвойств(НовыйПараметрТипПодписи, ПараметрТипПодписи);
	КонецЕсли;
...
Процедура ПодобратьДоверенностьПоСертификату()
	...
	Иначе
		НастройкаПоДоверенности = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить(
