...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодписьСервисаDSS")
		...
		Элементы.Программа.Заголовок = НСтр("ru = 'Приложение или сервис'");
	КонецЕсли;
	...
	Если Метаданные.Обработки.Найти("ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата") <> Неопределено Тогда
		...
		КонецЕсли;
		Если Не ЭлектроннаяПодписьСлужебный.ЕстьПравоНаДобавлениеСертификатовВСправочник() Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
				"ФормаПеревыпуститьСертификат", "Видимость", Ложь);
		КонецЕсли;
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
				"ФормаПеревыпуститьСертификат", "Видимость", Ложь);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.ФормаОчиститьПерсональныеНастройки.Видимость = ПравоДоступа("СохранениеДанныхПользователя",
		Метаданные);
...
Асинх Процедура ПрограммаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ПрограммаАвто <> Неопределено Тогда
		...
		Иначе
			ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуПрограммы(ПрограммаАвто, ЭтотОбъект);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьДанныеСертификата(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ОткрытьФормуСертификата(ПараметрыФормы, ЭтотОбъект);
...
Процедура ПроверитьСертификат(Команда)
	...
	ЭлектроннаяПодписьКлиент.ПроверитьСертификатСправочника(Объект.Ссылка,
		Новый Структура("БезПодтверждения, ТипПодписи", Истина, ТипПодписиПоУмолчанию()));
...
Процедура СертификатОтозван(Команда)
	...
	Если Не Объект.Отозван Тогда
		...
		ПоказатьВопрос(ОписаниеСледующее, НСтр("ru = 'Сертификат будет помечен как отозванный, его нельзя будет использовать для подписи. 
		|Рекомендуется устанавливать этот признак, когда подано, но еще не исполнено заявление на отзыв сертификата.
		|Продолжить?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ОписаниеСледующее = Новый ОписаниеОповещения("ПослеОтветаНаВопросСертификатОтозван", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеСледующее, НСтр("ru = 'Если сертификат отозван в удостоверяющем центре, сделать подпись таким сертификатом будет невозможно, даже если снять отметку в приложении.
		|Продолжить?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
&НаКлиенте
Процедура ПеревыпуститьСертификат(Команда)
...
&НаКлиенте
Процедура ОчиститьПерсональныеНастройки(Команда)

	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекУдалить(Объект.Ссылка, Неопределено,
		ИмяПользователя());
...
Процедура ОбновитьВидимостьПредупреждения(Знач СертификатКриптографии = Неопределено)
	...
	Если Объект.Отозван Тогда
		...
		Элементы.Предупреждение.Заголовок = 
			НСтр("ru='Сертификат помечен в приложении как отозванный. Подписи, сделанные таким сертификатом верны, если содержат метку доверенного времени, добавленную ранее даты отзыва сертификата. Чтобы узнать причину и дату отзыва, обратитесь в удостоверяющий центр, выдавший сертификат.'");
	...
	ИначеЕсли Объект.ДействителенДо > ТекущаяДатаСеанса() Тогда
		...
		Если ЗначениеЗаполнено(АдресСертификата) Тогда
			...
			Иначе
				...
				Если ДанныеПредупреждения.РазрешитьПодписание
					И ЭлектроннаяПодпись.ДобавлениеИзменениеЭлектронныхПодписей() Тогда
...
Процедура СписокПользователейЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено И ТипЗнч(РезультатВыбора) = Тип("Структура") Тогда
		...
		Для Каждого ДанныеПользователя Из РезультатВыбора.Пользователи Цикл
			НоваяСтрока = Объект.Пользователи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеПользователя);
		КонецЦикла;
		ОформитьСписокПользователей();
...
Процедура ПослеСменыПинКода(РезультатВызова, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли НЕ МодульСервисКриптографииDSSКлиентСервер.ЭтоОшибкаОтказа(РезультатВызова.СтатусОшибки) Тогда
		МодульСервисКриптографииDSSКлиент.ВывестиОшибку(Неопределено, РезультатВызова.СтатусОшибки);
...
Процедура ОткрытьСписокПользователей(РежимПросмотра)
	...
	Для Каждого СтрокаТаблицы Из Объект.Пользователи Цикл
		ДанныеПользователя = Новый Структура("Пользователь, БезИспользованияКлюча");
		ЗаполнитьЗначенияСвойств(ДанныеПользователя, СтрокаТаблицы);
...
Функция ЗаполнитьСписокПрограмм(Знач ВыбраннаяПрограмма)
	...
	Результат.Добавить(Справочники.ПрограммыЭлектроннойПодписиИШифрования.ПустаяСсылка(), НСтр("ru = 'По умолчанию'"));
...
&НаСервереБезКонтекста
Функция ТипПодписиПоУмолчанию()
	
	Возврат Константы.ТипПодписиКриптографииПоУмолчанию.Получить();
