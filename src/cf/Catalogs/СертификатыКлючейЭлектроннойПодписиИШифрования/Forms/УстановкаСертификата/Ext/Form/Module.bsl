...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВариантУстановки.ТолькоПросмотр = СписокВыбора.Количество() = 1;
	...
	Если ЭтоАдресВременногоХранилища(Параметры.Сертификат) Тогда
		...
		АдресСертификата = Параметры.Сертификат;
	ИначеЕсли ЗначениеЗаполнено(Параметры.Сертификат) Тогда
		СертификатДвоичныеДанные = Base64Значение(Параметры.Сертификат);
		АдресСертификата = ПоместитьВоВременноеХранилище(СертификатДвоичныеДанные, УникальныйИдентификатор);
	КонецЕсли;
	...
	Элементы.ПоказатьДанныеСертификата.Видимость = ЗначениеЗаполнено(АдресСертификата);
...
Асинх Процедура НайтиКонтейнеры()
	...
	Если КонтейнерыПоСертификату = Неопределено Тогда
