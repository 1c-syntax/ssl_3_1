...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.СертификатыФизическоеЛицо.Видимость И ДляПоискаФизическихЛиц.Количество() > 0 Тогда
		...
		ФизическиеЛицаПоФИО = ЭлектроннаяПодписьСлужебный.ПолучитьФизическиеЛицаПоПолюСертификатаКомуВыдан(ДляПоискаФизическихЛиц);
		Для Каждого ТекущаяСтрока Из Сертификаты Цикл
			...
			МассивЗначений = ФизическиеЛицаПоФИО.Получить(ТекущаяСтрока.КомуВыдан);
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ТекущаяСтрока Из СтрокаСертификата Цикл
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
				...
				ЧтениеРазрешено = МодульУправлениеДоступом.ЧтениеРазрешено(Выборка.Ссылка);
			Иначе
				ЧтениеРазрешено = ПравоДоступа("Чтение", Выборка.Ссылка.Метаданные());
			КонецЕсли;
			Если ЧтениеРазрешено Тогда // Защита от ошибки в данных (дубли сертификатов).
				ТекущаяСтрока.Сертификат = Выборка.Ссылка;
			КонецЕсли;
			
			ТекущаяСтрока.УжеДобавлен = Истина;
...
Функция ФизическоеЛицоСертификата(Знач Сертификат)
...
Процедура НачалоВыбораФизическогоЛица(СтандартнаяОбработка)
	...
	ФизическиеЛицаДляФИО = Результат.ФизическиеЛица.Получить(ТекущиеДанные.КомуВыдан);
	...
	Если ФизическиеЛицаДляФИО.Количество() = 1 Тогда
		Если ТекущиеДанные.ФизическоеЛицо <> ФизическиеЛицаДляФИО[0] Тогда
			ТекущиеДанные.ФизическоеЛицо = ФизическиеЛицаДляФИО[0];
		Иначе
			ОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбораФизическогоЛица", ЭтотОбъект);
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("РежимВыбора", Истина);
	...
	Для Каждого ТекущаяСтрока Из Сертификаты Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Сертификат) Тогда
			...
			Иначе
				Продолжить;
			КонецЕсли;
		ИначеЕсли ТекущаяСтрока.УжеДобавлен Тогда
			Продолжить;
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьОписаниеДанныхСертификатаНаСервере(Знач АдресСертификата)
