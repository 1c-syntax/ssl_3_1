...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("ПодтверждениеВыполнитьОсновнуюОперацию") И Источник = УникальныйИдентификатор Тогда
		...
		Иначе
			...
			ОбработатьОшибку(Новый ОписаниеОповещения("РасшифроватьЗавершение", ЭтотОбъект),
...
Процедура РасшифроватьДанныеПослеВыполненияНаСторонеКлиента(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("Ошибка") Тогда
		...
		КонецЕсли;
		Контекст.ОшибкаНаКлиенте = Ошибка;
		ДополнительныеДанныеОшибки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			Ошибка, "ДополнительныеДанныеПроверокНаКлиенте", Неопределено);
...
Функция ЗаписатьСертификатыШифрования(ИдентификаторФормы, Ошибка)
	...
	Ошибка = ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, ИдентификаторФормы, Ошибка);
...
Функция ЗаписатьСертификатыШифрованияНаСервере(Знач ОписаниеОбъектов, Знач ИдентификаторФормы, Знач Ошибка)
	...
	Исключение
		...
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ru = 'Не удалось очистить сертификаты шифрования по причине:'")
...
&НаКлиенте
Процедура ОбработатьОшибку(Оповещение, ОшибкаНаКлиенте, ОшибкаНаСервере, ДополнительныеДанные = Неопределено)
