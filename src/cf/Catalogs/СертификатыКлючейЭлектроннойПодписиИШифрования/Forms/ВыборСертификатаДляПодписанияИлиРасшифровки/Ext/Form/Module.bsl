...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользуютсяСертификатыНаСервере = ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере() 
		И ВыполнятьНаСервере <> Ложь Или ЕстьОблачнаяПодпись;
	
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Если ИспользуютсяСертификатыНаСервере Тогда
			Если ВыполнятьНаСервере = Истина Тогда
				Элементы.ГруппаСертификаты.Заголовок = НСтр("ru = 'Личные сертификаты на сервере'");
			Иначе
				Элементы.ГруппаСертификаты.Заголовок = НСтр("ru = 'Личные сертификаты на устройстве и сервере'");
			КонецЕсли;
		Иначе
			Элементы.ГруппаСертификаты.Заголовок = НСтр("ru = 'Личные сертификаты на устройстве'");
		КонецЕсли;
	Иначе
		Если ИспользуютсяСертификатыНаСервере Тогда
			Если ВыполнятьНаСервере = Истина Тогда
				Элементы.ГруппаСертификаты.Заголовок = НСтр("ru = 'Личные сертификаты на сервере'");
			Иначе
				Элементы.ГруппаСертификаты.Заголовок = НСтр("ru = 'Личные сертификаты на компьютере и сервере'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ЕстьОрганизации = ЭлектроннаяПодпись.ОбщиеНастройки().ОрганизацияИспользуется;
	...
	УстановитьВидимостьКомандДобавленияСертификатов();
...
Процедура ПриОткрытии(Отказ)
	...
	Если Сертификаты.Количество() = 0
		И Не Элементы.ГруппаСертификатыНедоступныНаКлиенте.Видимость
		И Не Элементы.ГруппаСертификатыНедоступныНаСервере.Видимость
		И Не Элементы.ГруппаАвторизацииОблачнойПодписи.Видимость Тогда
		ТекстПредупрежденияПриОткрытии = ?(ДобавлениеВСписок,
			...
			НСтр("ru = 'Нет доступных сертификатов для выбора. Возможно, отсутствуют права на добавление сертификатов, обратитесь к администратору.'"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("Запись_СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
		...
		Если Параметр.Свойство("Установлен") И Параметр.Установлен И ТекущиеДанные <> Неопределено
			И ТипЗнч(Источник) = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования")
			И Источник = Сертификат  И Не ТекущиеДанные.НаСервере Тогда
			
			СвойстваСертификата = СвойстваДобавленногоСертификата(Источник);
			ЗаполнитьЗначенияСвойств(ТекущиеДанные, СвойстваСертификата);
			Далее(Неопределено);
...
Процедура Выбрать(Команда)
	...
	Иначе
		...
		ПараметрыСертификата.ФизическоеЛицо = СертификатФизическоеЛицо;
...
Процедура ДобавитьДляПодписанияИШифрованияИзФайлов(Команда)
	...
	ПараметрыСоздания = ЭлектроннаяПодписьСлужебныйКлиент.ПараметрыДобавленияСертификата();
	ПараметрыСоздания.ОбработчикЗавершения = Новый ОписаниеОповещения("ПослеДобавленияСертификата", ЭтотОбъект);
	ЭлектроннаяПодписьСлужебныйКлиент.ДобавитьСертификатПослеВыбораНазначения("ДляПодписанияШифрованияИРасшифровкиИзФайлов", ПараметрыСоздания);
...
Процедура ПродолжитьОткрытиеПослеПереходаКВыборуТекущегоСертификата(Результат, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(АдресСертификата) Тогда
		...
		ДанныеСертификата.Вставить("Сертификат", АдресСертификата);
		ПослеДобавленияСертификата(ДанныеСертификата, Неопределено);
	КонецЕсли;
...
Процедура ОбновитьСписокСертификатовНаСервере(Знач СвойстваСертификатовНаКлиенте)
	...
	ДополнительныеПараметры.Вставить("ДоступныТолькоСКлючом", Не ПоказыватьВсе);
	...
	Если ЗаявлениеНаВыпускСертификатаДоступно Тогда
		...
		МодульЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.ДополнитьТаблицуСертификатовЗаявлениями(Сертификаты);
	...
	Если ЗначениеЗаполнено(ОтпечатокВыбранногоСертификата)
		...
		Или Сертификаты.НайтиПоИдентификатору(Элементы.Сертификаты.ТекущаяСтрока).Отпечаток <> ОтпечатокВыбранногоСертификата) Тогда
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Элементы.ГруппаСертификатыНедоступныНаКлиенте.Видимость = Ложь;
	Иначе
...
Процедура ПерейтиКВыборуТекущегоСертификата(Оповещение)
	...
	Если ТекущиеДанные.ЭтоЗаявление Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
			...
			ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
		Иначе
	...
	Если Не ЕстьПравоНаДобавлениеВСправочник И Не ТекущиеДанные.ЕстьВСправочнике Тогда
		Результат.ОбновитьСписокСертификатов = Истина;
		Результат.ОписаниеОшибки =
			НСтр("ru = 'Отсутствуют права на добавление сертификатов в справочник. Обратитесь к администратору.'");
		ВыполнитьОбработкуОповещения(Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
#КонецЕсли
	...
	СертификатФизическоеЛицо = Контекст.СохраненныеСвойства.ФизическоеЛицо;
...
&НаСервере
Процедура УстановитьВидимостьКомандДобавленияСертификатов()
	
	Если Не ЭлектроннаяПодписьСлужебный.ЕстьПравоНаДобавлениеСертификатовВСправочник()
		Или Не Параметры.ОтображатьКомандыДобавления Или ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		
		Элементы.ДобавитьДляПодписанияИШифрованияИзФайлов.Видимость = Ложь;
...
&НаКлиенте
Асинх Процедура ПослеДобавленияСертификата(Результат, Контекст) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СертификатАдрес = Результат.Сертификат;
	
	Если ПустаяСтрока(СертификатАдрес) Тогда
		ПараметрыФормы = Новый Структура;
	...
	СертификатОтпечаток = СвойстваСертификата.Отпечаток;
	
	ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаполнитьОписаниеДанныхСертификата(СертификатОписаниеДанных, СвойстваСертификата);
	
	Контекст.СохраненныеСвойства = СохраненныеСвойстваСертификата(СвойстваСертификата.Отпечаток,
		СертификатАдрес, СертификатПараметрыРеквизитов);
...
&НаСервереБезКонтекста
Функция СвойстваДобавленногоСертификата(Знач Сертификат)
	
	Результат = Новый Структура;
	Результат.Вставить("ЭтоЗаявление", Ложь);
	Результат.Вставить("НаКлиенте", Истина);
	Результат.Вставить("ЕстьВСправочнике", Истина);
	Результат.Вставить("Отпечаток", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сертификат, "Отпечаток"));
