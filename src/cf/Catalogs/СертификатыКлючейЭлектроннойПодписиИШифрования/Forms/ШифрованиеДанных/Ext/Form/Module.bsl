...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("ПодтверждениеВыполнитьОсновнуюОперацию") И Источник = УникальныйИдентификатор Тогда
		Если Параметр.Выполнено Тогда
			...
			КонецЕсли;
		Иначе
			ОбработатьОшибку(Новый ОписаниеОповещения("ЗашифроватьЗавершение", ЭтотОбъект),
...
Процедура ЗаполнитьПрограммуШифрованияПослеЦикла(Контекст)
	...
	Если ЗначениеЗаполнено(Сертификат) И Не ЗначениеЗаполнено(СертификатПрограмма)
		И Не ЗначениеЗаполнено(ПрограммаАвто) Тогда
		СтандартнаяОбработка = Истина;
		ЭлектроннаяПодписьКлиентЛокализация.ПриПолученииТокенаПоСертификату(
			Новый ОписаниеОповещения("ЗаполнитьПрограммуШифрованияПослеПолученияТокенаПоСертификату", ЭтотОбъект, Контекст),
			Контекст.СертификатШифрования, СтандартнаяОбработка, Истина);
...
// Продолжение процедуры СоздатьМенеджерКриптографии.
&НаКлиенте
Процедура ЗаполнитьПрограммуШифрованияПослеПолученияТокенаПоСертификату(Токен, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(Сертификат) Тогда
		Если Не ЗначениеЗаполнено(СертификатПрограмма)
			...
			Контекст.ОшибкаНаКлиенте.Вставить("ОписаниеОшибки",
				НСтр("ru = 'У выбранного личного сертификата не указано приложение для закрытого ключа и его не удалось определить автоматически.
...
Процедура ОбработатьОшибку(Оповещение, ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	Иначе
		...
		Если СертификатыШифрования.Количество() > 0 Тогда
			...
			КонецЦикла;
		ИначеЕсли НаборСертификатов.Количество() > 0 Тогда
			Для Каждого Строка Из НаборСертификатов Цикл
				Если Не ЗначениеЗаполнено(Строка.АдресДанных)
					Или ВсеСертификаты.Найти(Строка.АдресДанных) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ВсеСертификаты.Добавить(Строка.АдресДанных);
			КонецЦикла;
		КонецЕсли;
