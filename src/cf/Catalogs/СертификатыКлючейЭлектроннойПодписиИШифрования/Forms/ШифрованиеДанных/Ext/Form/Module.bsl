...
&НаКлиенте
Асинх Процедура ЗаполнитьПрограммуШифрованияПослеЦикла(Контекст)
	
	Если ЗначениеЗаполнено(Сертификат) И Не ЗначениеЗаполнено(СертификатПрограмма)
		И Не ЗначениеЗаполнено(ПрограммаАвто) Тогда
		Токен = Ждать ЭлектроннаяПодписьСлужебныйКлиент.ПолучитьТокенПоСертификату(Контекст.СертификатШифрования);
