...
&НаКлиенте
Перем ВыборИПодборВнешнихПользователей;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПерейтиКЗначениюДоступа <> Неопределено Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Иначе
		Для каждого Элемент Из ПользователиГруппы.ПолучитьЭлементы() Цикл
			НоваяСтрока = ТекущийОбъект.Пользователи.Добавить();
			НоваяСтрока.Пользователь = Элемент.Пользователь;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПрофилиГруппДоступаНазначение.ТипПользователей КАК ТипПользователей
	...
	Для Каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		СрокДействия = ТекущаяСтрока.СрокДействия;
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СрокДействия) И СрокДействия <= НачалоДня Тогда
			Срок = Формат(СрокДействия, НСтр("ru = 'ДЛФ=D'"));
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"ПользователиГруппы[%1].Пользователь",
				УточнитьСообщение(НСтр("ru = 'Срок участия %2 должен быть до завтра или более.'"), Срок),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("ru = 'Срок участия %2 в строке %1 должен быть до завтра или более.'"), Срок));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Участник) Тогда
			Продолжить;
		КонецЕсли;
		...
		НедопустимыйПользователь = НедопустимыеПользователи.Найти(ТекущаяСтрока.Пользователь, "Ссылка");
		Если НедопустимыйПользователь <> Неопределено Тогда
			
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				Если НедопустимыйПользователь.Служебный Тогда
					ТекстДляОднойОшибки      = НСтр("ru = 'Права (роли) служебного пользователя ""%2"" не настраиваются интерактивно.'");
					ТекстДляНесколькихОшибок = НСтр("ru = 'Права (роли) служебного пользователя ""%2"" в строке %1 не настраиваются интерактивно.'");
				Иначе
...
Процедура ПользователиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если РежимПодбора Тогда
		ТипПользователя = ?(ВыборИПодборВнешнихПользователей,
			Новый ОписаниеТипов("СправочникСсылка.ВнешниеПользователи,СправочникСсылка.ГруппыВнешнихПользователей"),
			Новый ОписаниеТипов("СправочникСсылка.Пользователи,СправочникСсылка.ГруппыПользователей"));
		ЭлементыПользователей = ПользователиГруппы.ПолучитьЭлементы();
		Индекс = ЭлементыПользователей.Количество();
		Пока Индекс > 0 Цикл
			Индекс = Индекс - 1;
			ТекущийПользователь = ЭлементыПользователей[Индекс].Пользователь;
			Если Не ЗначениеЗаполнено(ТекущийПользователь)
			 Или ТипПользователя.СодержитТип(ТипЗнч(ТекущийПользователь))
			   И ВыбранноеЗначение.Найти(ТекущийПользователь) = Неопределено Тогда
				ЭлементыПользователей.Удалить(Индекс);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если РежимПодбора Тогда
		ТипВнешнийПользователь = Новый ОписаниеТипов("СправочникСсылка.ВнешниеПользователи,
			|СправочникСсылка.ГруппыВнешнихПользователей");
		Список = Новый СписокЗначений;
		ЭлементыПользователей = ПользователиГруппы.ПолучитьЭлементы();
		Для Каждого Элемент Из ЭлементыПользователей Цикл
			Список.Добавить(Элемент,
				?(ТипВнешнийПользователь.СодержитТип(ТипЗнч(Элемент.Пользователь)), "2", "1")
					+ Строка(Элемент.Пользователь));
		КонецЦикла;
		Список.СортироватьПоПредставлению();
		НовыйИндекс = 0;
		Для Каждого ЭлементСписка Из Список Цикл
			СтарыйИндекс = ЭлементыПользователей.Индекс(ЭлементСписка.Значение);
			ЭлементыПользователей.Сдвинуть(СтарыйИндекс, НовыйИндекс - СтарыйИндекс);
...
&НаКлиенте
Процедура ПодобратьЗначенияДоступа(Команда)
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СрокДействия.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПользователиГруппы.Уровень");
...
Процедура НачальнаяНастройкаПриЧтенииИСоздании(ТекущийОбъект)
	...
	Для каждого СтрокаТЧ Из ТекущийОбъект.Пользователи Цикл
		...
		НоваяСтрока.СрокДействия = СтрокаТЧ.СрокДействия;
	КонецЦикла;
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если Подобрать Тогда
		...
		ПараметрыФормы.Вставить("ПараметрыРасширеннойФормыПодбора",
			ПараметрыРасширеннойФормыПодбора(ВыборИПодборВнешнихПользователей));
	КонецЕсли;
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Если Не ИспользоватьВнешнихПользователей Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Ведение внешних пользователей отключено в настройках приложения.'"));
		...
		Иначе
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для выбора внешних пользователей.'"),
				КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
	Иначе
...
&НаСервере
Функция ПараметрыРасширеннойФормыПодбора(ВыборИПодборВнешнихПользователей)
	
	ПараметрыПодбора = ПользователиСлужебный.НовыеПараметрыРасширеннойФормыПодбора();
	ПараметрыПодбора.ЗаголовокФормыПодбора = НСтр("ru = 'Подбор участников группы доступа'");
	...
	Для каждого Элемент Из ЭлементыКоллекции Цикл
		Если ТипПользователя.СодержитТип(ТипЗнч(Элемент.Пользователь))
		   И ЗначениеЗаполнено(Элемент.Пользователь) Тогда
			ПараметрыПодбора.ВыбранныеПользователи.Добавить(Элемент.Пользователь);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьПользователейГрупп(ИдентификаторСтроки = Неопределено,
	...
	Для Каждого Элемент Из ЭлементыКоллекции Цикл
		ТекущийПользователь = ?(ТипЗнч(Элемент) = Тип("ДанныеФормыЭлементДерева"),
			Элемент.Пользователь, Элемент);
		
		Если ТипЗнч(ТекущийПользователь) = Тип("СправочникСсылка.ГруппыПользователей")
		 Или ТипЗнч(ТекущийПользователь) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
		
			УчастникиГруппыПользователей.Добавить(ТекущийПользователь);
		КонецЕсли;
	КонецЦикла;
	...
	|	И СоставыГруппПользователей.Пользователь.Служебный <> ИСТИНА";
