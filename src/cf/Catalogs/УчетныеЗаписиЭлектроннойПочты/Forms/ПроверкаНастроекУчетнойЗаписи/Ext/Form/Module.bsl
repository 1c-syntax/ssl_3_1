...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СообщенияОбОшибках) Тогда
		...
		ЗаполнитьПояснения();
		ЗаполнитьИнформациюДляПоддержки();
...
Функция ПоясненияПоОшибке()
	...
	ПараметрыПояснения.Контекст = РаботаСПочтовымиСообщениямиСлужебный.КонтекстДляПояснения().РучнаяНастройка;
...
&НаСервере
Процедура ЗаполнитьИнформациюДляПоддержки(ПодробноеОписаниеОшибки = "")
	
	Если ЗначениеЗаполнено(ПодробноеОписаниеОшибки) Тогда
		ИнформацияДляПоддержки = ПодробноеОписаниеОшибки;
		Возврат;
	КонецЕсли;
	...
	ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		СпособыУстранения,
		ВозможныеПричины,
		ТехническиеПодробностиДляПоддержки(Параметры.УчетнаяЗапись));
...
Функция ТехническиеПодробностиДляПоддержки(УчетнаяЗапись)
	
	Результат = Новый Массив;
	
	ПочтовыйПрофиль = РаботаСПочтовымиСообщениямиСлужебный.ИнтернетПочтовыйПрофиль(УчетнаяЗапись);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		МодульПолучениеФайловИзИнтернета = ОбщегоНазначения.ОбщийМодуль("ПолучениеФайловИзИнтернета");
		ДиагностикаСоединения = МодульПолучениеФайловИзИнтернета.ДиагностикаСоединения(ПочтовыйПрофиль.АдресСервераSMTP);
		Результат.Добавить(ДиагностикаСоединения.ЖурналДиагностики);
	КонецЕсли;
	
	ОписаниеНастроек = Справочники.УчетныеЗаписиЭлектроннойПочты.ОписаниеНастроек(ПочтовыйПрофиль, Неопределено);
	Результат.Добавить(ОписаниеНастроек);
	
	Результат.Добавить("");
	
	ИнформацияОПрограмме = Справочники.УчетныеЗаписиЭлектроннойПочты.ИнформацияОПрограмме();
	Результат.Добавить(ИнформацияОПрограмме);
