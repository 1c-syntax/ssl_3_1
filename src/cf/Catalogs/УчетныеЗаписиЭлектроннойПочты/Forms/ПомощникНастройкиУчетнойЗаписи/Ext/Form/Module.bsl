...
Процедура ПерейтиНаСледующуюСтраницу(Команда = Неопределено)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВводПароляУчетнойЗаписи Тогда
		...
		Если ПустаяСтрока(ПарольДляПолученияПисем) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Введите пароль учетной записи'"), , "ПарольДляПолученияПисем");
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьПереходПоАдресу(Знач АдресПерехода)
	...
	Если ЭтоВебКлиент() Или НастройкиАвторизации.ИспользоватьБраузерОперационнойСистемы Тогда
...
Функция ВложенныйАдресПеренаправления()
	...
	ВложенныйАдресПеренаправления = Новый Структура;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		...
		Если МодульРаботаВМоделиСервиса.РазделениеВключено() Тогда
			...
			// АПК:1403-выкл, АПК:1443-выкл - №644.3.5 Программный интерфейс предусмотрен в старших версиях.
			КлючОбластиДанных = Константы["КлючОбластиДанных"].Получить();
			// АПК:1403-вкл, АПК:1443-вкл
			
			Если ЗначениеЗаполнено(КлючОбластиДанных) Тогда
				
				ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.SHA256);
				ХешированиеДанных.Добавить(КлючОбластиДанных);
				
				ВложенныйАдресПеренаправления.Вставить("zone", ЗначениеРазделителяСеанса);
				ВложенныйАдресПеренаправления.Вставить("zone_key", Base64Строка(ХешированиеДанных.ХешСумма));
	...
	ВложенныйАдресПеренаправления.Вставить("session", НомерСеансаИнформационнойБазы());
	ВложенныйАдресПеренаправления.Вставить("request_id", ИдентификаторЗапроса);
	ВложенныйАдресПеренаправления.Вставить("user_id", Строка(ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор));
	
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
	ЗаписатьJSON(ЗаписьJSON, ВложенныйАдресПеренаправления);
	
	Хранилище = Новый ХранилищеЗначения(ЗаписьJSON.Закрыть(), Новый СжатиеДанных(9));
...
Процедура ПослеРучнойНастройкиУчетнойЗаписи(Результат, ДополнительныеПараметры) Экспорт
	...
	Протокол = Результат.ПротоколВходящейПочты;
	НазваниеУчетнойЗаписи = Результат.Наименование;
	ИмяОтправителяПисем = Результат.ИмяПользователя;
	ДлительностьОжиданияСервера = Результат.ВремяОжидания;
	ИмяПользователяДляПолученияПисем = Результат.Пользователь;
	ИмяПользователяДляОтправкиПисем = Результат.ПользовательSMTP;
	ОставлятьКопииПисемНаСервере = Результат.ОставлятьКопииСообщенийНаСервере;
	УдалятьПисьмаССервера = Результат.ПериодХраненияСообщенийНаСервере > 0;
	ВидУчетнойЗаписи = ?(ЗначениеЗаполнено(Результат.ВладелецУчетнойЗаписи), "Персональная", "Общая");
