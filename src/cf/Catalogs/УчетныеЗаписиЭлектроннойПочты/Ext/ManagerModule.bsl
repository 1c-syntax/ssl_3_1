...
Функция ОпределитьИменаПочтовыхСерверовДомена(Домен)
	...
	АдресаСерверовИмен = Новый Массив;
	АдресаСерверовИмен.Добавить(""); // Сервер по умолчанию.
	АдресаСерверовИмен.Добавить("8.8.8.8"); // dns.google
	АдресаСерверовИмен.Добавить("8.8.4.4"); // dns.google
	АдресаСерверовИмен.Добавить("78.88.8.8"); // dns.yandex
	АдресаСерверовИмен.Добавить("78.8.8.1"); // dns.yandex
	
	СтрокиКоманд = Новый Массив;
	
	Для Каждого АдресСервераИмен Из АдресаСерверовИмен Цикл
		ШаблонКоманды = "nslookup -type=mx %1 %2";
		СтрокаКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКоманды, Домен, АдресСервераИмен);
