...
Процедура ПриЗаписи(Отказ)
	...
	Если Не ИспользуетсяСОбъектамиАдресации И ИспользуетсяБезОбъектовАдресации Тогда
		
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировки = БлокировкаДанных.Добавить("РегистрСведений.ИсполнителиЗадач");
		ЭлементБлокировки.УстановитьЗначение("РольИсполнителя", Ссылка);
		БлокировкаДанных.Заблокировать();

		Набор = РегистрыСведений.ИсполнителиЗадач.СоздатьНаборЗаписей();
		Набор.Отбор.РольИсполнителя.Установить(Ссылка);
		Набор.Прочитать();
		Если Набор.Количество() > 0 Тогда
			Для Каждого ИсполнительЗадачи Из Набор Цикл
				ИсполнительЗадачи.ОсновнойОбъектАдресации       = Неопределено;
				ИсполнительЗадачи.ДополнительныйОбъектАдресации = Неопределено;
			КонецЦикла;
		КонецЕсли;
		Набор.Записать();
