...
Процедура ШаблонЗаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	НачатьЗаполнениеРасписанияДня(ВыбраннаяСтрока);
...
&НаКлиенте
Процедура НачатьЗаполнениеРасписанияДня(ВыбранныйДень)
	
	СтрокаШаблона = Объект.ШаблонЗаполнения.НайтиПоИдентификатору(ВыбранныйДень);
	
	КонтекстВыбора = ДополнительныеПараметрыЗаполненияРасписанияДня();
	КонтекстВыбора.НомерДня = СтрокаШаблона.НомерСтроки;
	КонтекстВыбора.ИдентификаторСтрокиШаблона = ВыбранныйДень;
