...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбрабатываемыйВидТомаХраненияФайлов = Параметры.ЗначениеОтбораПоВидуТомаХраненияФайлов;
	Если Не ЗначениеЗаполнено(ОбрабатываемыйВидТомаХраненияФайлов) Тогда
		ОбрабатываемыйВидТомаХраненияФайлов = Перечисления.ВидыХраненияФайлов.ОперационноеХранение;
		Параметры.Отбор.Вставить("ВидТомаХраненияФайлов", ОбрабатываемыйВидТомаХраненияФайлов);
	КонецЕсли;

	Элементы.ПеренестиВсеФайлыВТома.Видимость = ОбрабатываемыйВидТомаХраненияФайлов = Перечисления.ВидыХраненияФайлов.ОперационноеХранение;

	Элементы.СписокВидТомаХраненияФайлов.Видимость = Не ЗначениеЗаполнено(ОбрабатываемыйВидТомаХраненияФайлов);
	
	Элементы.ГруппаУправлениеХранилищамиДвоичныхДанных.Видимость = РаботаСФайловымАрхивомСервер.ДоступныХранилищаДвоичныхДанных();
	
	Если РаботаСФайловымАрхивомСервер.ИспользоватьФайловыйАрхив() Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", Заголовок, ОбрабатываемыйВидТомаХраненияФайлов);
	КонецЕсли
...
Процедура ПроверитьЦелостность(Команда)
	...
	Если Не ДоступнаПроверкаЦелостностиТома(ТекущиеДанные.Ссылка) Тогда
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		Если Команда.Имя = "ПодменюОтчетыОбычное__ПроверкаЦелостностиТома" Тогда
			ТекущиеДанные = Элементы.Список.ТекущиеДанные;
			Если ТекущиеДанные <> Неопределено И Не ДоступнаПроверкаЦелостностиТома(ТекущиеДанные.Ссылка) Тогда
				Возврат;
			КонецЕсли;			
		КонецЕсли;
...
&НаКлиенте
Процедура НастроитьХранилищаДвоичныхДанных(Команда)

	ОткрытьФорму("Обработка.УправлениеХранилищемДвоичныхДанных.Форма.СписокХранилищ", , ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
&НаКлиенте
Функция ДоступнаПроверкаЦелостностиТома(ТомХраненияФайлаСсылка)

	Результат = ЭтоТомХраненияНаДисках(ТомХраненияФайлаСсылка);
