...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Элементы.СпособХраненияФайлов.ОтображениеПредупрежденияПриРедактировании		= ОтображениеПредупреждения;
		Элементы.ИмяХранилищаДвоичныхДанных.ОтображениеПредупрежденияПриРедактировании	= ОтображениеПредупреждения;
	Иначе
	...
	Элементы.ФормаПроверитьЦелостностьТома.Видимость = Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды")
	...
	РаботаСФайловымАрхивомСервер.СпособХраненияФайловЗаполнитьСписокВыбора(Элементы.СпособХраненияФайлов.СписокВыбора, Объект.ВидТомаХраненияФайлов);
	ЗаполнитьСписокВыбораИменХранилищДвоичныхДанных(Элементы.ИмяХранилищаДвоичныхДанных.СписокВыбора);
...
Процедура СпособХраненияФайловПриИзменении(Элемент)
	...
	ВидимостьРеквизитовОтСпособаХраненияФайлов = ПолучитьВидимостьРеквизитовОтСпособаХраненияФайлов(Объект.СпособХраненияФайлов);
	РаботаСФайловымАрхивомКлиентСервер.ОбновитьВидимостьРеквизитовТомаХраненияФайлов(ЭтотОбъект, ВидимостьРеквизитовОтСпособаХраненияФайлов);
...
Процедура ПроверитьЦелостностьТома(Команда)
	...
	Если Не ЭтоТомХраненияНаДисках(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Проверка целостности предусмотрена только для томов со способом хранения ""В сетевых каталогах""'"));
		Возврат;
	КонецЕсли;	
...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	Если Не ЭтоТомХраненияНаДисках(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Удаление лишних файлов предусмотрено только для томов со способом хранения ""В сетевых каталогах""'"));
		Возврат;
	КонецЕсли;	
...
Функция НайтиМаксимальныйПорядок()
	...
	Запрос.УстановитьПараметр("ВидТомаХраненияФайлов", Объект.ВидТомаХраненияФайлов);
...
Процедура ОбновитьВидимостьНаСервере()
	...
	ВидимостьГруппыСпособХраненияФайлов = РаботаСФайловымАрхивомСервер.ДоступныХранилищаДвоичныхДанных();

	Элементы.ГруппаСпособХраненияФайлов.Видимость = ВидимостьГруппыСпособХраненияФайлов;

	Если ВидимостьГруппыСпособХраненияФайлов Тогда
...
Функция ПолучитьВидимостьРеквизитовОтСпособаХраненияФайлов(Знач СпособХраненияФайлов)

	ВидимостьКаталогов		= СпособХраненияФайлов = Перечисления.СпособыХраненияФайлов.ВСетевыхКаталогах;
	ВидимостьИмениХранилища	= Не ВидимостьКаталогов И СпособХраненияФайлов = Перечисления.СпособыХраненияФайлов.ВоВнешнемХранилищеПоПротоколуS3;

	Результат = РаботаСФайловымАрхивомКлиентСервер.ПараметрыВидимостиРеквизитовОтСпособаХраненияФайлов();
	Результат.ВидимостьГруппаПути					= ВидимостьКаталогов;
	Результат.ВидимостьИмяХранилищаДвоичныхДанных	= ВидимостьИмениХранилища;
...
&НаСервереБезКонтекста
Процедура ЗаполнитьСписокВыбораИменХранилищДвоичныхДанных(СписокВыбора)

	Для Каждого МенеджерВнешнегоХДД Из ВнешниеХранилищаДвоичныхДанных Цикл
		СписокВыбора.Добавить(МенеджерВнешнегоХДД.Имя);
	КонецЦикла;

КонецПроцедуры 

&НаСервереБезКонтекста
Функция ЭтоТомХраненияНаДисках(Знач ТомХраненияСсылка)

	Возврат Справочники.ТомаХраненияФайлов.ЭтоТомХраненияНаДисках(ТомХраненияСсылка);
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если РаботаСФайловымАрхивомСервер.ИспользоватьФайловыйАрхив() Тогда
			...
			Заголовок = НСтр(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ru = '%1 (%2) (Создание)'", ПредставлениеОбъекта, Объект.ВидТомаХраненияФайлов));
		КонецЕсли;
	ИначеЕсли Не АвтоЗаголовок Тогда
