	...
	Если ПроверкаПриЗаписи Тогда
		Если СпособХраненияФайлов = Перечисления.СпособыХраненияФайлов.ВоВнешнемХранилищеПоПротоколуS3 Тогда
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В системе уже существует том с видом %1, размещающий данные во внешнем хранилище двоичных данных (по протоколу S3) и не помеченный на удаление.'"),
				НРег(ВидТомаХраненияФайлов));
		ИначеЕсли СпособХраненияФайлов = Перечисления.СпособыХраненияФайлов.ВоВстроенномХранилище Тогда
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В системе уже существует том с видом %1, размещающий данные во встроенном хранилище двоичных данных и не помеченный на удаление.'"),
				НРег(ВидТомаХраненияФайлов));
		Иначе
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В системе уже существует том с видом %1, размещающий данные %2 и не помеченный на удаление'"),
				НРег(ВидТомаХраненияФайлов),
				Строка(СпособХраненияФайлов));
		КонецЕсли;
	Иначе
		Если СпособХраненияФайлов = Перечисления.СпособыХраненияФайлов.ВоВнешнемХранилищеПоПротоколуS3 Тогда
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В системе уже существует том с видом %1, размещающий данные во внешнем хранилище двоичных данных (по протоколу S3).'"),
				НРег(ВидТомаХраненияФайлов));
		ИначеЕсли СпособХраненияФайлов = Перечисления.СпособыХраненияФайлов.ВоВстроенномХранилище Тогда
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В системе уже существует том с видом %1, размещающий данные во встроенном хранилище двоичных данных.'"),
				НРег(ВидТомаХраненияФайлов));
		Иначе
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В системе уже существует том с видом %1, размещающий данные %2. Используйте другой том.'"),
				НРег(ВидТомаХраненияФайлов),
				Строка(СпособХраненияФайлов));
		КонецЕсли;
	КонецЕсли;
